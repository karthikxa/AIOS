...
class StreamManager:
    ...
    async def auto_detect(self) -> str:
        ...
        On Windows, fall back to screenshot polling via the desktop API.
        Otherwise fall back to FFmpeg HLS on port 7080.
        Returns the stream URL that is actually available.
        """
        import platform
        import aiohttp

        # On Windows, skip KasmVNC and go straight to screenshot polling
        if platform.system() == "Windows":
            self._mode = "screenshot"
            self._started = True
            return "http://127.0.0.1:7777/screenshot"
