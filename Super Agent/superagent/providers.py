...
class FreeLLMProvider(BaseProvider):
    """Provider that connects to the local LLM Proxy (freellmapi) on port 3002.

    Uses the OpenAI-compatible endpoint at http://localhost:3002/v1.
    No API key required — the proxy handles backend routing.
    """
    ...
    def __init__(self, model: str = "auto", *, api_key: str | None = None, base_url: str | None = None) -> None:
        super().__init__(model=model, api_key=api_key or "", base_url=base_url or self.base_url)
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{self.base_url.rstrip('/')}/chat/completions", json=payload, headers=headers) as resp:
                if resp.status != 200:
                    ...
                    raise RuntimeError(f"LLM Proxy error {resp.status}: {text}")
                data = await resp.json()
        content = data["choices"][0]["message"]["content"]
        return ProviderResponse(content=content, raw=data)


class FreeLLMVisionProvider(FreeLLMProvider):
    """Same as FreeLLMProvider but sends screenshot as base64 image."""

    provider_name = "freellm-vision"
