...
class RelayServer:
    ...
    async def _run_unix_server(self) -> None:
        """Listen for agent connections over Unix socket, fall back to TCP on Windows."""
        import platform
    ...
    async def _run_tcp_server(self) -> None:
        """TCP fallback for Windows where Unix sockets are unavailable."""
        host = self.ws_host
        port = 9201
        server = await asyncio.start_server(
            self._handle_agent_connection, host, port
        )
        logger.info("Relay TCP server listening at %s:%d", host, port)
        async with server:
            await server.serve_forever()
...
class RelayClient:
    ...
    def __init__(self, socket_path: str = _DEFAULT_SOCKET, host: str = "127.0.0.1", port: int = 9201) -> None:
        self.socket_path = socket_path
        self.host = host
        self.port = port
    ...
    async def connect(self) -> None:
        ...
        try:
            if platform.system() == "Windows":
                ...
                logger.debug("RelayClient: connected to %s:%d", self.host, self.port)
            else:
