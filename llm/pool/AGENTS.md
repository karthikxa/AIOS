...
External AI tools are configured in `.metaswarm/external-tools.yaml`.

freellmpool uses the polybot-v3-style lane model, reduced to:

- one Umans Kimi K2.7 implementation lane;
- one MiniMax M3 implementation lane through OpenCode;
- Codex as the implementation escalation lane;
- Claude Opus only for final pre-ship review, not routine implementation or review.

The orchestrator must still run deterministic validation independently.
...
- Python 3.11+ package under `src/freellmpool`
