...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with zed-agent)
2. User plugins: ``$ZED_HOME/plugins/model-providers/<name>/``

Each plugin directory contains:
  - ``__init__.py`` â€” calls ``register_provider(profile)`` at import
...
# Repo-root ``plugins/model-providers/`` â€” populated at discovery time.
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    Later registrations with the same name replace earlier ones â€” so user
    plugins under ``$ZED_HOME/plugins/model-providers/`` can override
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple ZED_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_zed_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$ZED_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins â€” shipped with zed-agent.
    ...
    # 2. User plugins â€” under $ZED_HOME/plugins/model-providers/<name>/.
    ...
    #    back-compat â€” if someone drops a ``providers/foo.py`` into an
