...
Registry and ABC for every inference provider Zed knows about.

Each provider is declared once as a `ProviderProfile`. Every other layer â€”
auth resolution, transport kwargs, model listing, runtime routing â€” reads from
...
```
providers/
â”œâ”€â”€ base.py         ProviderProfile dataclass + OMIT_TEMPERATURE sentinel
â”œâ”€â”€ __init__.py     Registry: register_provider(), get_provider_profile(), list_providers()
â””â”€â”€ README.md       This file
```

The **profiles themselves** live as plugins under
`plugins/model-providers/<name>/` (bundled in this repo) and
`$ZED_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
- `zed_cli/auth.py` extends `PROVIDER_REGISTRY` with every api-key
  profile it sees (skipping `copilot`, `kimi-coding`, `kimi-coding-cn`,
  `zai`, `openrouter`, `custom` â€” those need bespoke token resolution).
- `zed_cli/models.py` extends `CANONICAL_PROVIDERS` and calls
  `profile.fetch_models()` inside `provider_model_ids()`.
- `zed_cli/doctor.py` adds a `/models` health check for each
  `auth_type="api_key"` profile.
- `zed_cli/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `zed_cli/runtime_provider.py` reads `profile.api_mode` as a fallback
  when URL detection finds nothing.
- `agent/model_metadata.py` maps hostname â†’ provider via
...
See `plugins/model-providers/README.md` â€” drop a new directory there (or
under `$ZED_HOME/plugins/model-providers/` for a private plugin).
...
| Hook | Purpose |
|------|---------|
| `get_hostname()` | URL-based detection â€” default derives from `base_url`. |
...
| `build_api_kwargs_extras(**ctx)` | `(extra_body_additions, top_level_kwargs)` â€” Kimi puts reasoning_effort top-level, Qwen splits `enable_thinking`/`thinking_budget`. |
| `fetch_models(*, api_key)` | Live catalog fetch â€” default hits `{models_url or base_url}/models` with Bearer auth. Override for no-REST providers (Bedrock), OAuth catalogs (Anthropic), or public catalogs (OpenRouter). |
