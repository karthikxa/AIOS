#!/usr/bin/env bash
# GitHub environment detection helper for Zed Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _zed_env="${ZED_HOME:-$HOME/.zed}/.env"; [ -f "$_zed_env" ] && grep -q "^GITHUB_TOKEN=" "$_zed_env" 2>/dev/null; then
...
[ "$GH_AUTH_METHOD" = "none" ] && echo "âš  Not authenticated â€” see github-auth skill"
