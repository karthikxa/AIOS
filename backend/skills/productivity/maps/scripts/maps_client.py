...
USER_AGENT = "ZedAgent/1.0 (contact: zed@agent.ai)"
...
# outage doesn't break the skill â€” kumi.systems is a well-known mirror.
OVERPASS_URLS = [
...
def overpass_query(query):
    ...
    for url in OVERPASS_URLS:
        try:
            ...
            # error_exit inside http_post â€” keep trying the next mirror.
...
def cmd_nearby(args):
    ...
    multiple categories in one call â€” results are merged, deduplicated
...
def cmd_area(args):
    ...
    # Approximate area in kmÂ² using the bounding box
...
def build_parser():
    ...
    p_nearby.add_argument(
        ...
        metavar="CAT",
        help="POI category (repeatable â€” adds a type to the search).",
    )
    p_nearby.add_argument(
        "--radius", "-r",
