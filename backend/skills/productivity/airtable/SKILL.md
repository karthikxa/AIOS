...
4. Store the token in `${ZED_HOME:-~/.zed}/.env` (or via `zed setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `${ZED_HOME:-~/.zed}/.env` into the subprocess automatically** when this skill is loaded â€” no need to re-export it before each `curl` call.
