...
from ``zed_constants.py`` using only the stdlib.
...
try:
    from zed_constants import display_zed_home as display_zed_home
    from zed_constants import get_zed_home as get_zed_home
except (ModuleNotFoundError, ImportError):

    def get_zed_home() -> Path:
        """Return the Zed home directory (default: ~/.zed).

        Mirrors ``zed_constants.get_zed_home()``."""
    ...
    def display_zed_home() -> str:
        ...
        Mirrors ``zed_constants.display_zed_home()``."""
        home = get_zed_home()
        try:
