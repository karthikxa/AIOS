#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Zed Agent.

Fully non-interactive â€” designed to be driven by the agent via terminal commands.
...
Agent workflow:
  1. Run --check. If exit 0, auth is good â€” skip setup.
...
# Ensure sibling modules (_zed_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _zed_home import display_zed_home, get_zed_home

ZED_HOME = get_zed_home()
TOKEN_PATH = ZED_HOME / "google_token.json"
CLIENT_SECRET_PATH = ZED_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = ZED_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Zed profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Zed Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Zed Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  pip install 'zed-agent[google]'")
...
def check_auth(quiet: bool = False):
    ...
    try:
        # Don't pass scopes â€” user may have authorized only a subset.
    ...
    if creds.expired and creds.refresh_token:
        try:
            ...
            if "disabled_client" in err_str or "invalid_client" in err_str:
                ...
                print("    1. Check your Google Cloud Console â€” verify the OAuth client is not disabled")
                ...
                print("    4. Do NOT retry API calls with a disabled account â€” this may worsen the situation")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Zed home."""
...
def exchange_auth_code(code: str):
    ...
    try:
        # Accept partial scopes â€” user may deselect some permissions in the consent screen
    ...
    print(f"Profile-scoped token location: {display_zed_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Zed")
