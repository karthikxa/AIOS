#!/usr/bin/env python3
"""Google Workspace API CLI for Zed Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Zed-facing JSON contract and falls back to the Python client
...
from _zed_home import get_zed_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ZED_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Zed Agent")
