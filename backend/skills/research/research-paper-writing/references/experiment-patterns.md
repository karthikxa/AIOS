...
Patterns and best practices distilled from running research experiments at scale with the Zed agent. These cover experiment infrastructure, evaluation protocols, monitoring, and failure recovery.
...
- **CoT judges** match non-CoT quality at ~40% cost (1 CoT judge â‰ˆ 3 standard judges)
...
4. Compute pilot inter-annotator agreement â€” if low, revise guidelines
...
**Pairwise comparison** (preferred over absolute scoring â€” more reliable):
...
```python
# Krippendorff's alpha (preferred â€” handles missing data, any scale)
...
|----------|----------|------|---------|
| **Prolific** | Academic research, higher quality | $8-15/hr | High â€” academic participant pool |
| **MTurk** | Large-scale, fast turnaround | $2-10/hr | Variable â€” use qualifications |
| **Surge AI** | NLP-specific annotations | Premium | High â€” trained annotators |
| **Expert annotators** | Domain-specific (medical, legal) | Highest | Highest â€” but slow |
...
1. **Check processes first** â€” don't read results if the experiment is still running and results are incomplete
2. **Read the log tail** â€” look for errors, progress indicators, completion messages
3. **Count completed vs expected** â€” "45/150 problems done" is more useful than "some results exist"
4. **Report in structured tables** â€” always include key metrics in a table
5. **Answer the key question** â€” each experiment should have a specific analytical question to answer when done
6. **[SILENT] for no-news** â€” suppress notifications when nothing has changed
7. **Commit on completion** â€” every completed batch gets committed with a descriptive message
...
- [Not vague â€” "be comprehensive" is bad, "include exactly 6 sections" is good]
```
...
- **Fixed deliverable**: Specific format (pitch, postmortem, memo â€” not "improve this")
...
**Do NOT use word count as a scope constraint.** Word limits cause false convergence â€” outputs get rejected for length, not quality. Constrain scope (what to include) not length.
...
**Option A: SciencePlots styles** (recommended â€” handles most defaults automatically):
...
# Pick a style:
# 'science'        â€” clean, serif fonts, suitable for most venues
# 'science+ieee'   â€” IEEE-style (good for two-column papers)
# 'science+nature' â€” Nature-style
...
- **Always save as PDF**: `fig.savefig('fig.pdf')` â€” vector graphics, sharp at any zoom
- **Never save as PNG** for paper figures â€” raster PNGs look blurry when printed/zoomed
- **Exception**: Screenshots, photographs, or pixel-art visualizations â†’ PNG at 600 DPI
