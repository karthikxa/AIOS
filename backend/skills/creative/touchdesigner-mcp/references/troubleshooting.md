...
If it's unclear, use `td_read_textport` to see the full TD console output â€”
...
Workflow:
1. td_get_par_info(op_type='glslTOP') â€” get all params for a type
2. td_get_operator_info(path='/project1/mynode', detail='full') â€” get params for a specific instance
...
Use `td_get_perf` to see which operators are slow. Look at cook times â€”
...
`$ZED_HOME/config.yaml` (defaults to `~/.zed/config.yaml` when `ZED_HOME` is unset)
...
Restart the Zed session for changes to take effect. The MCP connection is
...
Check parentPath â€” use absolute paths like /project1. The default project
...
```python
rec.par.videocodec = 'prores'  # Preferred on macOS â€” lossless, Non-Commercial OK
# rec.par.videocodec = 'mjpa'  # Fallback â€” lossy, works everywhere
```
