...
Resend's website is a dark, cinematic canvas that treats email infrastructure like a luxury product. The entire page is draped in pure black (`#000000`) with text that glows in near-white (`#f0f0f0`), creating a theater-like experience where content performs on a void stage. This isn't the typical developer-tool darkness â€” it's the controlled darkness of a photography gallery, where every element is lit with intention and nothing competes for attention.
...
What makes Resend distinctive is its icy, blue-tinted border system. Instead of neutral gray borders, Resend uses `rgba(214, 235, 253, 0.19)` â€” a frosty, slightly blue-tinted line at 19% opacity that gives every container and divider a cold, crystalline quality against the black background. Combined with pill-shaped buttons (9999px radius), multi-color accent system (orange, green, blue, yellow, red â€” each with its own CSS variable scale), and OpenType stylistic sets (`"ss01"`, `"ss03"`, `"ss04"`, `"ss11"`), the result is a design system that feels premium, precise, and quietly confident.

**Key Characteristics:**
- Pure black background with near-white (`#f0f0f0`) text â€” theatrical, gallery-like darkness
- Three-font hierarchy: Domaine Display (serif hero), ABC Favorit (geometric sections), Inter (body/UI)
- Icy blue-tinted borders: `rgba(214, 235, 253, 0.19)` â€” every border has a cold, crystalline shimmer
- Multi-color accent system: orange, green, blue, yellow, red â€” each with numbered CSS variable scales
...
- Commit Mono for code â€” monospace as a design element, not an afterthought
...
### Accent Scale â€” Orange
- **Orange 4** (`#ff5900`): `--color-orange-4`, at 22% opacity â€” subtle warm glow
- **Orange 10** (`#ff801f`): `--color-orange-10`, primary orange accent â€” warm, energetic
- **Orange 11** (`#ffa057`): `--color-orange-11`, lighter orange for secondary use

### Accent Scale â€” Green
- **Green 3** (`#22ff99`): `--color-green-3`, at 12% opacity â€” faint emerald wash
- **Green 4** (`#11ff99`): `--color-green-4`, at 18% opacity â€” success indicator glow

### Accent Scale â€” Blue
- **Blue 4** (`#0075ff`): `--color-blue-4`, at 34% opacity â€” medium blue accent
- **Blue 5** (`#0081fd`): `--color-blue-5`, at 42% opacity â€” stronger blue
- **Blue 10** (`#3b9eff`): `--color-blue-10`, bright blue â€” links, interactive elements

### Accent Scale â€” Other
- **Yellow 9** (`#ffc53d`): `--color-yellow-9`, warm gold for warnings or highlights
- **Red 5** (`#ff2047`): `--color-red-5`, at 34% opacity â€” error states, destructive actions
...
### Borders & Shadows
- **Frost Border** (`rgba(214, 235, 253, 0.19)`): The signature â€” icy blue-tinted borders at 19% opacity
...
### Font Families
- **Display Serif**: `domaine` (Domaine Display by Klim Type Foundry) â€” hero headlines
- **Display Sans**: `aBCFavorit` (ABC Favorit by Dinamo), fallbacks: `ui-sans-serif, system-ui` â€” section headings
- **Body / UI**: `inter`, fallbacks: `ui-sans-serif, system-ui` â€” body text, buttons, navigation
- **Monospace**: `commitMono`, fallbacks: `ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas`
- **Secondary**: `Helvetica` â€” fallback for specific UI contexts
- **System**: `-apple-system, system-ui, Segoe UI, Roboto` â€” embedded content
...
| Nav Link | aBCFavorit | 14px (0.88rem) | 500 | 1.43 | 0.35px | `"ss01", "ss03", "ss04"` â€” positive tracking |
| Button / Link | inter | 14px (0.88rem) | 500â€“600 | 1.43 | normal | Buttons, nav, CTAs |
| Caption | inter | 14px (0.88rem) | 400 | 1.60 (relaxed) | normal | Descriptions |
| Helvetica Caption | Helvetica | 14px (0.88rem) | 400â€“600 | 1.00â€“1.71 | normal | UI elements |
| Small | inter | 12px (0.75rem) | 400â€“500 | 1.33 | normal | Tags, meta, fine print |
...
### Principles
- **Three-font editorial hierarchy**: Domaine Display (serif, hero), ABC Favorit (geometric sans, sections), Inter (readable body). Each font has a strict role â€” they never cross lanes.
- **Aggressive negative tracking on display**: Domaine at -0.96px, ABC Favorit at -2.8px. The display type feels compressed, urgent, and designed â€” like a magazine masthead.
- **Positive tracking on nav**: ABC Favorit nav links use +0.35px letter-spacing â€” the only positive tracking in the system. This creates airy, spaced-out navigation text that contrasts with the compressed headings.
...
- **Commit Mono as design element**: The monospace font isn't hidden in code blocks â€” it's used prominently for code examples and technical content, treated as a first-class visual element.
...
- Minimal styling â€” inherits dark theme
...
- Dark-themed screenshots on dark background â€” seamless integration
- Rounded corners: 12pxâ€“16px on images
...
- Badges use the accent color at low opacity (12â€“42%) for background, full opacity for text
...
- **Cinematic black space**: The black background IS the whitespace. Generous vertical spacing (80pxâ€“120px+) between sections creates a scroll-through-darkness experience where each section emerges like a scene.
- **Tight content, vast surrounds**: Text blocks and cards are compact internally, but float in vast dark space â€” creating isolated "islands" of content.
- **Typography-led rhythm**: The massive display fonts (96px) create their own vertical rhythm â€” each headline is a visual event that anchors the surrounding space.
...
| Level | Treatment | Use |
|-------|-----------|-----|
| Flat (Level 0) | No shadow, transparent background | Default â€” most elements on dark void |
| Ring (Level 1) | `rgba(176, 199, 217, 0.145) 0px 0px 0px 1px` | Shadow-as-border for cards, containers |
| Frost Border (Level 1b) | `1px solid rgba(214, 235, 253, 0.19)` | Explicit borders â€” buttons, dividers, tabs |
| Subtle (Level 2) | `rgba(0, 0, 0, 0.1) 0px 1px 3px, rgba(0, 0, 0, 0.1) 0px 1px 2px -1px` | Light card elevation |
| Focus (Level 3) | `rgb(0, 0, 0) 0px 0px 0px 8px` | Heavy black focus ring â€” accessibility |

**Shadow Philosophy**: Resend barely uses shadows at all. On a pure black background, traditional shadows are invisible â€” you can't cast a shadow into the void. Instead, Resend creates depth through its signature frost borders (`rgba(214, 235, 253, 0.19)`) â€” thin, icy blue-tinted lines that catch light against the darkness. This creates a "glass panel floating in space" aesthetic where borders are the primary depth mechanism.
...
- No gradient backgrounds â€” depth comes from border luminance and content contrast

## 7. Do's and Don'ts

### Do
- Use pure black (`#000000`) as the page background â€” the void is the canvas
- Apply frost borders (`rgba(214, 235, 253, 0.19)`) for all structural lines â€” they're the blue-tinted signature
...
- Keep shadows at ring level (`0px 0px 0px 1px`) â€” on black, traditional shadows don't work
- Use +0.35px letter-spacing on ABC Favorit nav links â€” the only positive tracking

### Don't
- Don't lighten the background above `#000000` â€” the pure black void is non-negotiable
- Don't use neutral gray borders â€” all borders must have the frost blue tint
- Don't apply Domaine Display to body text â€” it's a display-only serif
- Don't mix accent colors in the same component â€” each feature gets one accent color
- Don't use box-shadow for elevation on the dark background â€” use frost borders instead
- Don't skip the OpenType stylistic sets â€” they define the typographic character
- Don't use negative letter-spacing on nav links â€” ABC Favorit nav uses positive +0.35px
- Don't make buttons opaque on dark â€” transparency with frost border is the pattern
...
| Mobile | 480â€“600px | Standard mobile, stacked layout |
| Desktop | >600px | Full layout, 96px hero, expanded sections |

*Note: Resend uses a minimal breakpoint system â€” only 480px and 600px detected. The design is desktop-first with a clean mobile collapse.*
...
### Collapsing Strategy
- Hero: Domaine 96px â†’ 76.8px on mobile
- Navigation: horizontal â†’ hamburger
- Feature sections: side-by-side â†’ stacked
...
- Rounded corners (12pxâ€“16px) maintained across breakpoints
...
### Iteration Guide
1. Start with pure black â€” everything floats in the void
2. Frost borders (`rgba(214, 235, 253, 0.19)`) are the universal structural element â€” not gray, not neutral
3. Three fonts, three roles: Domaine (hero), ABC Favorit (sections), Inter (body) â€” never cross
4. OpenType stylistic sets are mandatory on display fonts â€” they define the character
5. Multi-color accents at low opacity (12â€“42%) for backgrounds, full opacity for text
6. Pill shape (9999px) for CTAs and badges, standard radius (4pxâ€“16px) for containers
7. No shadows â€” use frost borders for depth against the void
