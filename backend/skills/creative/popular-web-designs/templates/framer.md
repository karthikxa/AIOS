...
Framer's website is a cinematic, tool-obsessed dark canvas that radiates the confidence of a design tool built by designers who worship craft. The entire experience is drenched in pure black â€” not a warm charcoal or a cozy dark gray, but an absolute void (`#000000`) that makes every element, every screenshot, every typographic flourish feel like it's floating in deep space. This is a website that treats its own product UI as the hero art, embedding full-fidelity screenshots and interactive demos directly into the narrative flow.

The typography is the signature move: GT Walsheim with aggressively tight letter-spacing (as extreme as -5.5px on 110px display text) creates headlines that feel compressed, kinetic, almost spring-loaded â€” like words under pressure that might expand at any moment. The transition to Inter for body text is seamless, with extensive OpenType feature usage (`cv01`, `cv05`, `cv09`, `cv11`, `ss03`, `ss07`) that gives even small text a refined, custom feel. Framer Blue (`#0099ff`) is deployed sparingly but decisively â€” as link color, border accents, and subtle ring shadows â€” creating a cold, electric throughline against the warm-less black.
...
**Key Characteristics:**
- Pure black (`#000000`) void canvas â€” absolute dark, not warm or gray-tinted
- GT Walsheim display font with extreme negative letter-spacing (-5.5px at 110px)
- Framer Blue (`#0099ff`) as the sole accent color â€” cold, electric, precise
- Pill-shaped buttons (40pxâ€“100px radius) â€” no sharp corners on interactive elements
- Product screenshots as hero art â€” the tool IS the marketing
...
- **Framer Blue** (`#0099ff`): Primary accent color â€” links, borders, ring shadows, interactive highlights
...
### Gradient System
- No prominent gradient usage â€” Framer relies on pure flat black surfaces with occasional blue-tinted glows for depth
- Subtle radial glow effects behind product screenshots using Framer Blue at very low opacity

## 3. Typography Rules

### Font Family
- **Display**: `GT Walsheim Framer Medium` / `GT Walsheim Medium` â€” custom geometric sans-serif, weight 500. Fallbacks: `GT Walsheim Framer Medium Placeholder`, system sans-serif
- **Body/UI**: `Inter Variable` / `Inter` â€” variable sans-serif with extensive OpenType features. Fallbacks: `Inter Placeholder`, `-apple-system`, `system-ui`
- **Accent**: `Mona Sans` â€” GitHub's open-source font, used for select elements at ultra-light weight (100)
- **Monospace**: `Azeret Mono` â€” companion mono for code and technical labels
- **Rounded**: `Open Runde` â€” small rounded companion font for micro-labels
...
### Principles
- **Compression as personality**: GT Walsheim's extreme negative letter-spacing (-5.5px at 110px) is the defining typographic gesture â€” headlines feel spring-loaded, urgent, almost breathless
...
- **Weight restraint on display**: All GT Walsheim usage is weight 500 (medium) â€” never bold, never regular. This creates a confident-but-not-aggressive display tone
- **Ultra-tight line heights**: Display text at 0.85 line-height means letters nearly overlap vertically â€” intentional density that rewards reading at arm's length

## 4. Component Stylings

### Buttons
- **Frosted Pill**: `rgba(255, 255, 255, 0.1)` background, black text (`#000000`), pill shape (40px radius). The glass-effect button that lives on dark surfaces â€” translucent, ambient, subtle
- **Solid White Pill**: `rgb(255, 255, 255)` background, black text (`#000000`), full pill shape (100px radius), padding `10px 15px`. The primary CTA â€” clean, high-contrast on dark, unmissable
...
### Cards & Containers
- **Dark Surface Card**: Black or near-black (`#090909`) background, `rgba(0, 153, 255, 0.15) 0px 0px 0px 1px` blue ring shadow border, rounded corners (10pxâ€“15px radius)
- **Elevated Card**: Multi-layer shadow â€” `rgba(255, 255, 255, 0.1) 0px 0.5px 0px 0.5px` (subtle top highlight) + `rgba(0, 0, 0, 0.25) 0px 10px 30px` (deep ambient shadow)
- **Product Screenshots**: Full-width or padded within dark containers, 8pxâ€“12px border-radius for software UI previews
...
### Image Treatment
- **Product screenshots as hero art**: Full-width embedded UI screenshots with rounded corners (8pxâ€“12px)
...
- **No decorative imagery**: All images are functional â€” showing the tool, the output, or the workflow

### Trust & Social Proof
- Customer logos and testimonials in muted gray on dark surfaces
- Minimal ornamentation â€” the product screenshots serve as the trust signal
...
- **Section padding**: Large vertical spacing (80pxâ€“120px between sections)
- **Card padding**: 15pxâ€“30px internal padding
- **Component gaps**: 8pxâ€“20px between related elements
...
- **Breathe through darkness**: Generous vertical spacing between sections â€” the black background means whitespace manifests as void, creating dramatic pauses between content blocks
...
- **5pxâ€“7px**: Small UI elements, image thumbnails â€” subtly softened
- **8px**: Standard component radius â€” code blocks, buttons, interactive elements
- **10pxâ€“12px**: Cards, product screenshots â€” comfortably rounded
- **15pxâ€“20px**: Large containers, feature cards â€” generously rounded
- **30pxâ€“40px**: Navigation pills, pagination â€” noticeably rounded
- **100px**: Full pill shape â€” primary CTAs, tag elements
...
| Level 1 (Ring) | `rgba(0, 153, 255, 0.15) 0px 0px 0px 1px` | Card borders, interactive element outlines â€” Framer Blue glow ring |
| Level 2 (Contained) | `rgb(9, 9, 9) 0px 0px 0px 2px` | Near-black ring for subtle containment on dark surfaces |
| Level 3 (Floating) | `rgba(255, 255, 255, 0.1) 0px 0.5px 0px 0.5px, rgba(0, 0, 0, 0.25) 0px 10px 30px` | Elevated cards, floating elements â€” subtle white top-edge highlight + deep ambient shadow |
...
- **Blue-tinted ring shadows** at very low opacity (0.15) for containment â€” a signature move that subtly brands every bordered element
- **White edge highlights** (0.5px) on the top edge of elevated elements â€” simulating light hitting the top surface
- **Deep ambient shadows** for true floating elements â€” `rgba(0, 0, 0, 0.25)` at large spread (30px)

### Decorative Depth
- **Blue glow auras**: Subtle Framer Blue (`#0099ff`) radial gradients behind key interactive areas
- **No background blur/glassmorphism**: Despite the frosted button effect, there's no heavy glass blur usage â€” the translucency is achieved through simple rgba opacity

## 7. Do's and Don'ts

### Do
- Use pure black (`#000000`) as the primary background â€” not dark gray, not charcoal
- Apply extreme negative letter-spacing on GT Walsheim display text (-3px to -5.5px)
- Keep all buttons pill-shaped (40px+ radius) â€” never use squared or slightly-rounded buttons
- Use Framer Blue (`#0099ff`) exclusively for interactive accents â€” links, borders, focus states
- Deploy `rgba(255, 255, 255, 0.1)` for frosted glass surfaces on dark backgrounds
- Maintain GT Walsheim at weight 500 only â€” the medium weight IS the brand
- Use extensive OpenType features on Inter text (cv01, cv05, cv09, cv11, ss03, ss07)
- Let product screenshots be the visual centerpiece â€” the tool markets itself
...
- Apply bold (700+) weight to GT Walsheim display text â€” medium 500 only
- Introduce additional accent colors beyond Framer Blue â€” this is a one-accent-color system
- Use large border-radius on non-interactive elements (cards use 10pxâ€“15px, only buttons get 40px+)
- Add decorative imagery, illustrations, or icons â€” the product IS the illustration
- Use positive letter-spacing on headlines â€” everything is compressed, negative tracking
- Create heavy drop shadows â€” depth is communicated through subtle rings and minimal ambients
- Place light/white backgrounds behind content sections â€” the void is sacred
- Use serif or display-weight fonts â€” the system is geometric sans-serif only
...
|------|-------|-------------|
| Mobile | <809px | Single column, stacked feature sections, reduced hero text (62pxâ†’40px), hamburger nav |
| Tablet | 809pxâ€“1199px | 2-column features begin, nav links partially visible, screenshots scale down |
| Desktop | >1199px | Full layout, expanded nav with all links + CTA, 110px display hero, side-by-side features |

### Touch Targets
- Pill buttons: minimum 40px height with 10px vertical padding â€” exceeds 44px WCAG minimum
...
### Collapsing Strategy
- **Navigation**: Full horizontal nav â†’ hamburger menu at mobile breakpoint
- **Hero text**: 110px display â†’ 85px â†’ 62px â†’ ~40px across breakpoints, maintaining extreme negative tracking proportionally
- **Feature sections**: Side-by-side (text + screenshot) â†’ stacked vertically on mobile
- **Product screenshots**: Scale responsively within containers, maintaining aspect ratios
- **Section spacing**: Reduces proportionally â€” 120px desktop â†’ 60px mobile

### Image Behavior
- Product screenshots are responsive, scaling within their container boundaries
- No art direction changes â€” same crops across breakpoints
...
1. Focus on ONE component at a time â€” the dark canvas makes each element precious
2. Always verify letter-spacing on GT Walsheim headings â€” the extreme negative tracking is non-negotiable
3. Check that Framer Blue appears ONLY on interactive elements â€” never as decorative background or text color for non-links
4. Ensure all buttons are pill-shaped â€” any squared corner immediately breaks the Framer aesthetic
5. Test frosted glass surfaces by checking they have exactly `rgba(255, 255, 255, 0.1)` â€” too opaque looks like a bug, too transparent disappears
