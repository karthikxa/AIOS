...
Pinterest's website is a warm, inspiration-driven canvas that treats visual discovery like a lifestyle magazine. The design operates on a soft, slightly warm white background with Pinterest Red (`#e60023`) as the singular, bold brand accent. Unlike the cool blues of most tech platforms, Pinterest's neutral scale has a distinctly warm undertone â€” grays lean toward olive/sand (`#91918c`, `#62625b`, `#e5e5e0`) rather than cool steel, creating a cozy, craft-like atmosphere that invites browsing.

The typography uses Pin Sans â€” a custom proprietary font with a broad fallback stack including Japanese fonts, reflecting Pinterest's global reach. At display scale (70px, weight 600), Pin Sans creates large, inviting headlines. At smaller sizes, the system is compact: buttons at 12px, captions at 12â€“14px. The CSS variable naming system (`--comp-*`, `--sema-*`, `--base-*`) reveals a sophisticated three-tier design token architecture: component-level, semantic-level, and base-level tokens.

What distinguishes Pinterest is its generous border-radius system (12pxâ€“40px, plus 50% for circles) and warm-tinted button backgrounds. The secondary button (`#e5e5e0`) has a distinctly warm, sand-like tone rather than cold gray. The primary red button uses 16px radius â€” rounded but not pill-shaped. Combined with warm badge backgrounds (`hsla(60,20%,98%,.5)` â€” a subtle yellow-warm wash) and photography-dominant layouts, the result is a design that feels handcrafted and personal, not corporate and sterile.

**Key Characteristics:**
- Warm white canvas with olive/sand-toned neutrals â€” cozy, not clinical
- Pinterest Red (`#e60023`) as singular bold accent â€” never subtle, always confident
...
- Photography-first content â€” pins/images are the primary visual element
- Dark near-purple text (`#211922`) â€” warm, with a hint of plum

## 2. Color Palette & Roles

### Primary Brand
- **Pinterest Red** (`#e60023`): Primary CTA, brand accent â€” bold, confident red
...
### Text
- **Plum Black** (`#211922`): Primary text â€” warm near-black with plum undertone
...
### Surface & Border
- **Sand Gray** (`#e5e5e0`): Secondary button background â€” warm, craft-like
...
### Font Family
- **Primary**: `Pin Sans`, fallbacks: `-apple-system, system-ui, Segoe UI, Roboto, Oxygen-Sans, Apple Color Emoji, Segoe UI Emoji, Segoe UI Symbol, Ubuntu, Cantarell, Fira Sans, Droid Sans, Helvetica Neue, Helvetica, ãƒ’ãƒ©ã‚®ãƒŽè§’ã‚´ Pro W3, ãƒ¡ã‚¤ãƒªã‚ª, Meiryo, ï¼­ï¼³ ï¼°ã‚´ã‚·ãƒƒã‚¯, Arial`
...
| Caption | Pin Sans | 12px (0.75rem) | 400â€“500 | 1.50 | normal | Small text, tags |
| Button | Pin Sans | 12px (0.75rem) | 400 | normal | normal | Button labels |

### Principles
- **Compact type scale**: The range is 12pxâ€“70px with a dramatic jump â€” most functional text is 12â€“16px, creating a dense, app-like information hierarchy.
- **Warm weight distribution**: 600â€“700 for headings, 400â€“500 for body. No ultra-light weights â€” the type always feels substantial.
- **Negative tracking on headings**: -1.2px on 28px headings creates cozy, intimate section titles.
- **Single font family**: Pin Sans handles everything â€” no secondary display or monospace font detected.
...
- Text: `#000000` (black â€” unusual choice for contrast on red)
- Padding: 6px 14px
...
### Cards & Containers
- Photography-first pin cards with generous radius (12pxâ€“20px)
...
- Rounded corners: 12pxâ€“20px on images
- Photography as primary content â€” every pin is an image
...
- Large jumps: 32px â†’ 80px â†’ 100px for section spacing
...
- **Inspiration density**: The masonry grid packs pins tightly â€” the content density IS the value proposition. Whitespace exists between sections, not within the grid.
...
| Level | Treatment | Use |
|-------|-----------|-----|
| Flat (Level 0) | No shadow | Default â€” pins rely on content, not shadow |
...
### Do
- Use warm neutrals (`#e5e5e0`, `#e0e0d9`, `#91918c`) â€” the warm olive/sand tone is the identity
- Apply Pinterest Red (`#e60023`) only for primary CTAs â€” it's bold and singular
- Use Pin Sans exclusively â€” one font for everything
- Apply generous border-radius: 16px for buttons/inputs, 20px+ for cards
- Keep the masonry grid dense â€” content density is the value
- Use warm badge backgrounds (`hsla(60,20%,98%,.5)`) for subtle warm washes
- Use `#211922` (plum black) for primary text â€” it's warmer than pure black

### Don't
- Don't use cool gray neutrals â€” always warm/olive-toned
- Don't use pure black (`#000000`) as primary text â€” use plum black (`#211922`)
- Don't use pill-shaped buttons â€” 16px radius is rounded but not pill
- Don't add heavy shadows â€” Pinterest is flat by design, depth from content
- Don't use small border-radius (<12px) on cards â€” the generous rounding is core
- Don't introduce additional brand colors â€” red + warm neutrals is the complete palette
- Don't use thin font weights â€” Pin Sans at 400 minimum
...
| Mobile Large | 576â€“768px | 2-column pin grid |
| Tablet | 768â€“890px | Expanded grid |
| Desktop Small | 890â€“1312px | Standard masonry grid |
| Desktop | 1312â€“1440px | Full layout |
| Large Desktop | 1440â€“1680px | Expanded grid columns |
| Ultra-wide | >1680px | Maximum grid density |

### Collapsing Strategy
- Pin grid: 5+ columns â†’ 3 â†’ 2 â†’ 1
- Navigation: search bar + icons â†’ simplified mobile nav
- Feature sections: side-by-side â†’ stacked
- Hero: 70px â†’ scales down proportionally
- Footer: dark multi-column â†’ stacked
...
### Iteration Guide
1. Warm neutrals everywhere â€” olive/sand grays, never cool steel
2. Pinterest Red for CTAs only â€” bold and singular
3. 16px radius on buttons/inputs, 20px+ on cards â€” generous but not pill
4. Pin Sans is the only font â€” compact at 12px for UI, 70px for display
5. Photography carries the design â€” the UI stays warm and minimal
6. Plum black (#211922) for text â€” warmer than pure black
