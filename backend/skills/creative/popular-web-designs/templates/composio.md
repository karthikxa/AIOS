...
- Bioluminescent accent strategy â€” cyan and blue glows that feel like they're emitting light from within
...
### Primary
- **Composio Cobalt** (`#0007cd`): The core brand color â€” a deep, saturated blue used sparingly for high-priority interactive elements and brand moments. It anchors the identity with quiet intensity.

### Secondary & Accent
- **Electric Cyan** (`#00ffff`): The attention-grabbing accent â€” used at low opacity (`rgba(0,255,255,0.12)`) for glowing button backgrounds and card highlights. At full saturation, it serves as the energetic counterpoint to the dark canvas.
...
### Surface & Background
- **Void Black** (`#0f0f0f`): The primary page background â€” not pure black, but a hair warmer, reducing eye strain on dark displays.
...
- **Ghost White** (`rgba(255,255,255,0.6)`): Secondary body text and link labels â€” visible but deliberately receded.
...
### Semantic & Accent
- **Border Mist 12** (`rgba(255,255,255,0.12)`): Highest-opacity border treatment â€” used for prominent card edges and content separators.
...
- **Border Mist 04** (`rgba(255,255,255,0.04)`): The faintest border â€” used for atmospheric separation only.
...
- **Dual personality**: abcDiatype carries the marketing voice â€” geometric, precise, friendly. JetBrains Mono carries the technical voice â€” credible, functional, familiar to developers.
...
- **Uppercase is earned**: The `uppercase` + `letter-spacing` treatment is reserved exclusively for tiny overline labels and technical tags â€” never for headings.
...
- Shadow: select cards use the hard-offset brutalist shadow (`rgba(0,0,0,0.15) 4px 4px 0px 0px`) â€” a distinctive design choice that adds raw depth
...
### Inputs & Forms
- No explicit input token data extracted â€” inputs likely follow the dark-surface pattern with:
...
- Large monospace numbers (JetBrains Mono) â€” "10k+" style
...
- Oversized brand typography â€” likely the largest text on the page
...
### Border Radius Scale
- Nearly squared (2px): Inline code spans, small tags, pre blocks â€” the sharpest treatment, conveying technical precision
- Subtly rounded (4px): Content cards, images, standard containers â€” the workhorse radius
- Pill-shaped (37px): Select buttons and badges â€” creates a softer, more approachable feel for key CTAs
...
**Shadow Philosophy**: Composio uses shadows sparingly and with deliberate contrast. The hard-offset brutalist shadow is the signature â€” it breaks the sleek darkness with a raw, almost retro-computing feel. The soft diffuse shadow is reserved for truly floating elements. Most depth is communicated through border opacity gradations rather than shadows.
...
### Do
- Use Void Black (`#0f0f0f`) as the primary page background â€” never pure white for main surfaces
- Keep heading line-heights ultra-tight (0.87-1.0) for compressed, authoritative text blocks
- Use white-opacity borders (4-12%) for containment â€” they're more important than shadows here
- Reserve Electric Cyan (`#00ffff`) for high-signal moments only â€” CTAs, glows, interactive accents
...
- Keep button text dark (`oklch(0.145 0 0)`) even on the darkest backgrounds â€” buttons carry their own surface
...
- Don't apply heavy shadows everywhere â€” depth comes from border opacity, not box-shadow
- Don't use Composio Cobalt (`#0007cd`) as a text color â€” it's too dark on dark and too saturated on light
- Don't increase heading line-heights beyond 1.2 â€” the compressed feel is core to the identity
- Don't use bold (700) weight for body or heading text â€” 400-500 is the ceiling
- Don't mix warm colors â€” the palette is strictly cool (blue, cyan, white, black)
- Don't use border-radius larger than 4px on content cards â€” the precision of near-square corners is intentional
- Don't place Electric Cyan at full opacity on large surfaces â€” it's an accent, used at 12% max for backgrounds
- Don't use decorative serif or handwritten fonts â€” the entire identity is geometric sans + monospace
- Don't skip the monospace font for technical content â€” JetBrains Mono is not decorative, it's a credibility signal
...
- **Feature grids**: 3-column â†’ 2-column â†’ single-column stacking
- **Hero text**: 64px â†’ 40px â†’ 28px progressive scaling
...
2. Reference specific color names and hex codes from this document â€” "use Ghost White (rgba(255,255,255,0.6))" not "make it lighter"
3. Use natural language descriptions â€” "make the border barely visible" = Border Mist 04-06
4. Describe the desired "feel" alongside specific measurements â€” "compressed and authoritative heading at 48px with line-height 1.0"
5. For glow effects, specify "Electric Cyan at 12% opacity as a radial gradient behind the element"
6. Always specify which font â€” abcDiatype for marketing, JetBrains Mono for technical/code content
