...
Airbnb's website is a warm, photography-forward marketplace that feels like flipping through a travel magazine where every page invites you to book. The design operates on a foundation of pure white (`#ffffff`) with the iconic Rausch Red (`#ff385c`) â€” named after Airbnb's first street address â€” serving as the singular brand accent. The result is a clean, airy canvas where listing photography, category icons, and the red CTA button are the only sources of color.

The typography uses Airbnb Cereal VF â€” a custom variable font that's warm and approachable, with rounded terminals that echo the brand's "belong anywhere" philosophy. The font operates in a tight weight range: 500 (medium) for most UI, 600 (semibold) for emphasis, and 700 (bold) for primary headings. Slight negative letter-spacing (-0.18px to -0.44px) on headings creates a cozy, intimate reading experience rather than the compressed efficiency of tech companies.

What distinguishes Airbnb is its palette-based token system (`--palette-*`) and multi-layered shadow approach. The primary card shadow uses a three-layer stack (`rgba(0,0,0,0.02) 0px 0px 0px 1px, rgba(0,0,0,0.04) 0px 2px 6px, rgba(0,0,0,0.1) 0px 4px 8px`) that creates a subtle, warm lift. Combined with generous border-radius (8pxâ€“32px), circular navigation controls (50%), and a category pill bar with horizontal scrolling, the interface feels tactile and inviting â€” designed for browsing, not commanding.

**Key Characteristics:**
- Pure white canvas with Rausch Red (`#ff385c`) as singular brand accent
- Airbnb Cereal VF â€” custom variable font with warm, rounded terminals
...
- Photography-first listing cards â€” images are the hero content
- Near-black text (`#222222`) â€” warm, not cold
...
### Text Scale
- **Near Black** (`#222222`): `--palette-text-primary`, primary text â€” warm, not cold
...
| Tag | Airbnb Cereal VF | 12px (0.75rem) | 400â€“700 | 1.33 | normal | Tags, prices |
...
### Principles
- **Warm weight range**: 500â€“700 dominate. No weight 300 or 400 for headings â€” Airbnb's type is always at least medium weight, creating a warm, confident voice.
...
- 8pxâ€“14px radius on contained images
...
- Listing grid: responsive multi-column (3â€“5 columns on desktop)
- Full-width footer with link columns

### Whitespace Philosophy
- **Travel-magazine spacing**: Generous vertical padding between sections creates a leisurely browsing pace â€” you're meant to scroll slowly, like browsing a magazine.
- **Photography density**: Listing cards are packed relatively tightly, but each image is large enough to feel immersive.
- **Search bar prominence**: The search bar gets maximum vertical space in the header â€” finding your destination is the primary action.
...
### Do
- Use `#222222` (warm near-black) for text â€” never pure `#000000`
- Apply Rausch Red (`#ff385c`) only for primary CTAs and brand moments â€” it's the singular accent
- Use Airbnb Cereal VF at weight 500â€“700 â€” the warm weight range is intentional
...
- Use photography as the primary visual content â€” listings are image-first
...
### Don't
- Don't use pure black (`#000000`) for text â€” always `#222222` (warm)
- Don't apply Rausch Red to backgrounds or large surfaces â€” it's an accent only
- Don't use thin font weights (300, 400) for headings â€” 500 minimum
- Don't use heavy shadows (>0.1 opacity as primary layer) â€” keep them warm and graduated
- Don't use sharp corners (0â€“4px) on cards â€” the generous rounding (20px+) is core
- Don't introduce additional brand colors beyond the Rausch/Luxe/Plus system
- Don't override the palette token system â€” use `--palette-*` variables consistently
...
| Mobile | 375â€“550px | Standard mobile listing grid |
| Tablet Small | 550â€“744px | 2-column listings |
| Tablet | 744â€“950px | Search bar expansion |
| Desktop Small | 950â€“1128px | 3-column listings |
| Desktop | 1128â€“1440px | 4-column grid, full header |
| Large Desktop | 1440â€“1920px | 5-column grid |
| Ultra-wide | >1920px | Maximum grid width |

*Note: Airbnb has 61 detected breakpoints â€” one of the most granular responsive systems observed, reflecting their obsession with layout at every possible screen size.*
...
### Collapsing Strategy
- Listing grid: 5 â†’ 4 â†’ 3 â†’ 2 â†’ 1 columns
- Search: expanded bar â†’ compact bar â†’ overlay
- Category pills: horizontal scroll at all sizes
- Navigation: full header â†’ mobile simplified
- Map: side panel â†’ overlay/toggle
...
### Iteration Guide
1. Start with white â€” the photography provides all the color
2. Rausch Red (#ff385c) is the singular accent â€” use sparingly for CTAs only
3. Near-black (#222222) for text â€” the warmth matters
4. Three-layer shadows create natural, warm lift â€” always use all three layers
5. Generous radius: 8px buttons, 20px cards, 50% controls
6. Cereal VF at 500â€“700 weight â€” no thin weights for any heading
7. Photography is hero â€” every listing card is image-first
