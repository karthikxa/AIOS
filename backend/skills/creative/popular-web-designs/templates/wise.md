...
Wise's website is a bold, confident fintech platform that communicates "money without borders" through massive typography and a distinctive lime-green accent. The design operates on a warm off-white canvas with near-black text (`#0e0f0c`) and a signature Wise Green (`#9fe870`) â€” a fresh, lime-bright color that feels alive and optimistic, unlike the corporate blues of traditional banking.

The typography uses Wise Sans â€” a proprietary font used at extreme weight 900 (black) for display headings with a remarkably tight line-height of 0.85 and OpenType `"calt"` (contextual alternates). At 126px, the text is so dense it feels like a protest sign â€” bold, urgent, and impossible to ignore. Inter serves as the body font with weight 600 as the default for emphasis, creating a consistently confident voice.

What distinguishes Wise is its green-on-white-on-black material palette. Lime Green (`#9fe870`) appears on buttons with dark green text (`#163300`), creating a nature-inspired CTA that feels fresh. Hover states use `scale(1.05)` expansion rather than color changes â€” buttons physically grow on interaction. The border-radius system uses 9999px for buttons (pill), 30pxâ€“40px for cards, and the shadow system is minimal â€” just `rgba(14,15,12,0.12) 0px 0px 0px 1px` ring shadows.

**Key Characteristics:**
- Wise Sans at weight 900, 0.85 line-height â€” billboard-scale bold headlines
- Lime Green (`#9fe870`) accent with dark green text (`#163300`) â€” nature-inspired fintech
- Inter body at weight 600 as default â€” confident, not light
- Near-black (`#0e0f0c`) primary with warm green undertone
- Scale(1.05) hover animations â€” buttons physically grow
- OpenType `"calt"` on all text
- Pill buttons (9999px) and large rounded cards (30pxâ€“40px)
...
### Font Families
- **Display**: `Wise Sans`, fallback: `Inter` â€” OpenType `"calt"` on all text
...
| Button | Inter | 18pxâ€“22px | 600 | 1.00â€“1.44 | -0.108px | `"calt"` |
| Caption | Inter | 14px (0.88rem) | 400â€“600 | 1.50â€“1.86 | -0.084px to -0.108px | `"calt"` |
| Small | Inter | 12px (0.75rem) | 400â€“600 | 1.00â€“2.17 | -0.084px to -0.108px | `"calt"` |

### Principles
- **Weight 900 as identity**: Wise Sans Black (900) is used exclusively for display â€” the heaviest weight in any analyzed system. It creates text that feels stamped, pressed, physical.
...
- **"calt" everywhere**: Contextual alternates enabled on ALL text â€” both Wise Sans and Inter.
- **Weight 600 as body default**: Inter Semibold is the standard reading weight â€” confident, not light.
...
- Padding: 5px 16px
- Radius: 9999px
- Hover: scale(1.05) â€” button physically grows
- Active: scale(0.95) â€” button compresses
...
**Shadow Philosophy**: Wise uses minimal shadows â€” ring shadows only. Depth comes from the bold green accent against the neutral canvas.

## 7. Do's and Don'ts

### Do
- Use Wise Sans weight 900 for display â€” the extreme boldness IS the brand
- Apply line-height 0.85 on Wise Sans display â€” ultra-tight is intentional
...
### Don't
- Don't use light font weights for Wise Sans â€” only 900
- Don't relax the 0.85 line-height on display â€” the density is the identity
- Don't use the Wise Green as background for large surfaces â€” it's for buttons and accents
- Don't skip the scale animation on buttons
- Don't use traditional shadows â€” ring shadows only
...
| Tablet | 576â€“992px | 2-column |
| Desktop | 992â€“1440px | Full layout |
...
### Iteration Guide
1. Wise Sans 900 at 0.85 line-height â€” the extreme weight IS the brand
2. Lime Green for buttons only â€” dark green text on green background
3. Scale animations (1.05 hover, 0.95 active) on all interactive elements
4. "calt" on everything â€” contextual alternates are mandatory
5. Inter 600 for body â€” confident reading weight
