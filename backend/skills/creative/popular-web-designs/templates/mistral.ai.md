...
- The Mistral "M" block identity â€” a gradient from yellow to orange
- Dramatic landscape photography in warm golden tones
- Uppercase typography used strategically for section labels and CTAs
- Near-zero border-radius â€” sharp, architectural geometry
- French-European confidence: bold, warm, declarative

## 2. Color Palette & Roles

### Primary
- **Mistral Orange** (`#fa520f`): The core brand color â€” a vivid, saturated orange-red that anchors the entire identity. Used for primary emphasis, the brand block, and the highest-signal moments.
...
- **Block Orange** (`#ff8105`): A pure orange used in the gradient block system â€” warmer and less red than Mistral Orange.

### Secondary & Accent
- **Sunshine 900** (`#ff8a00`): Deep golden amber â€” the darkest sunshine tone, used for strong accent moments.
- **Sunshine 700** (`#ffa110`): Warm amber-gold â€” the core sunshine accent for backgrounds and interactive elements.
- **Sunshine 500** (`#ffb83e`): Medium golden â€” balanced warmth for mid-level emphasis.
- **Sunshine 300** (`#ffd06a`): Light golden â€” for subtle warm tints and secondary backgrounds.
- **Block Gold** (`#ffe295`): Pale gold â€” soft background accents and gentle warmth.
- **Bright Yellow** (`#ffd900`): The brightest tone in the gradient â€” used at the "top" of the block identity.

### Surface & Background
- **Warm Ivory** (`#fffaeb`): The lightest page background â€” barely tinted with warmth, the foundation canvas.
- **Cream** (`#fff0c2`): The primary warm surface and secondary button background â€” noticeably golden.
...
### Neutrals & Text
- **Mistral Black** (`#1f1f1f`): Primary text color and dark button backgrounds â€” a near-black that's warmer than pure #000.
...
### Semantic & Accent
- **Input Border** (defined as `hsl(240, 5.9%, 90%)`): A cool-tinted light gray for form borders â€” one of the few cool tones in the system.
- **White Overlay** (`oklab(1, 0, 0 / 0.088â€“0.1)`): Semi-transparent white for frosted glass effects and button overlays.

### Gradient System
- **Mistral Block Gradient**: The signature identity â€” a multi-step gradient flowing through Yellow (`#ffd900`) â†’ Gold (`#ffe295`) â†’ Amber (`#ffa110`) â†’ Orange (`#ff8105`) â†’ Flame (`#fb6424`) â†’ Mistral Orange (`#fa520f`). This gradient appears in the logo blocks, section backgrounds, and decorative elements.
...
### Principles
- **Single weight, maximum impact**: The entire system uses weight 400 (regular) â€” even at 82px. This creates a surprisingly elegant effect where the size alone carries authority without needing bold weight.
- **Ultra-tight at scale**: Line-heights of 0.95â€“1.00 at display sizes create text blocks where ascenders nearly touch descenders from the line above â€” creating dense, poster-like composition.
...
- **No weight variation**: Unlike most systems that use 300â€“700 weight range, Mistral uses 400 everywhere. Hierarchy comes from size and color, never weight.
...
- No visible border
- The primary action button â€” dark on warm
...
- Minimal styling â€” text link as button
- For tertiary navigation actions

### Cards & Containers
- Background: Warm Ivory (`#fffaeb`), Cream (`#fff0c2`), or Pure White
- Border: minimal to none â€” containers defined by background color
- Radius: near-zero â€” sharp, architectural corners
- Shadow: warm golden multi-layer (`rgba(127, 99, 21, 0.12) -8px 16px 39px, rgba(127, 99, 21, 0.1) -33px 64px 72px, rgba(127, 99, 21, 0.06) -73px 144px 97px, ...`) â€” a dramatic, cascading warm shadow
- Distinctive: the golden shadow creates a "golden hour" lighting effect

### Inputs & Forms
- Border: `hsl(240, 5.9%, 90%)` â€” the sole cool-toned element
...
### Image Treatment
- Dramatic landscape photography in warm golden tones
- The winding road through golden hills â€” a recurring visual motif
...
- A row of colored blocks forming the gradient: yellow â†’ amber â†’ orange â†’ burnt orange
- Each block gets progressively more orange/red
- The visual DNA of the brand â€” recognizable at any size
...
- Section vertical spacing: very generous (80pxâ€“100px)
...
- Card grids: 2â€“3 column layouts

### Whitespace Philosophy
- **Bold declarations**: Huge headlines surrounded by generous whitespace create billboard-like impact â€” each statement gets its own breathing space.
...
### Border Radius Scale
- Near-zero: The dominant radius â€” sharp, architectural corners on most elements
...
| Golden Float (Level 1) | Multi-layer warm shadow (5 layers, 12%â†’0% opacity, amber-tinted) | Feature cards, product showcases, elevated content |

**Shadow Philosophy**: Mistral uses a single but extraordinarily complex shadow â€” **five cascading layers** of amber-tinted shadow (`rgba(127, 99, 21, ...)`) that build from a close 16px offset to a distant 400px offset. The result is a rich, warm, "golden hour" lighting effect that makes elevated elements look like they're bathed in afternoon sunlight. This is the most distinctive shadow system in any major AI brand.
...
- Use the Mistral block gradient (yellow â†’ amber â†’ orange) for brand moments
- Apply warm golden shadows (amber-tinted rgba) for elevated elements
- Use Mistral Black (#1f1f1f) for text â€” never pure #000000
- Keep font weight at 400 throughout â€” let size and color carry hierarchy
- Use sharp, architectural corners â€” near-zero border-radius
...
### Don't
- Don't introduce cool colors (blue, green, purple) â€” the palette is exclusively warm
- Don't use bold (700+) weight â€” 400 is the only weight
- Don't round corners â€” the sharp geometry is intentional
- Don't use cool-toned shadows â€” shadows must carry amber warmth
- Don't use pure white as a page background â€” always warm-tinted (#fffaeb minimum)
- Don't reduce hero text below 48px on desktop â€” the billboard scale is core
- Don't use more than 2 font weights â€” size variation replaces weight variation
- Don't add gradients outside the warm spectrum â€” no blue-to-purple, no cool transitions
- Don't use generic gray for text â€” even neutrals should be warm-tinted
...
| Tablet | 640â€“768px | Minor layout adjustments |
| Small Desktop | 768â€“1024px | 2-column layouts begin |
| Desktop | 1024â€“1280px | Full layout with maximum typography scale |
...
- **Hero text**: 82px â†’ 56px â†’ 48px â†’ 32px progressive scaling
- **Feature sections**: Multi-column â†’ stacked
...
- No art direction changes â€” same warm composition at all sizes
...
### Iteration Guide
1. Keep the warm temperature â€” "shift toward amber" not "shift toward gray"
2. Use size for hierarchy â€” 82px â†’ 56px â†’ 48px â†’ 32px â†’ 24px â†’ 16px
3. Never add border-radius â€” sharp corners only
4. Shadows are always warm: "golden shadow with amber tones"
5. Font weight is always 400 â€” describe emphasis through size and color
