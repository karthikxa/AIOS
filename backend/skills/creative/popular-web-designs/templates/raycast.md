...
- Subtle rgba white borders (0.06â€“0.1 opacity) for containment on dark surfaces
- Keyboard shortcut styling with gradient key caps and heavy shadows

## 2. Color Palette & Roles

### Primary
- **Near-Black Blue** (`#07080a`): Primary page background â€” the foundational void with a subtle blue-cold undertone
- **Pure White** (`#ffffff`): Primary heading text, high-emphasis elements
- **Raycast Red** (`#FF6363` / `hsl(0, 100%, 69%)`): Brand accent â€” hero stripes, danger states, critical highlights

### Secondary & Accent
- **Raycast Blue** (`hsl(202, 100%, 67%)` / ~`#55b3ff`): Interactive accent â€” links, focus states, selected items
...
### Gradient System
- **Keyboard Key Gradient**: Linear gradient from `#121212` (top) to `#0d0d0d` (bottom) â€” simulates physical key depth
- **Warm Glow**: `rgba(215, 201, 175, 0.05)` radial spread â€” subtle warm ambient glow behind featured elements

## 3. Typography Rules

### Font Family
- **Primary**: `Inter` â€” humanist sans-serif, used everywhere. Fallbacks: `Inter Fallback`, system sans-serif
- **System**: `SF Pro Text` â€” Apple system font for select macOS-native UI elements. Fallbacks: `SF Pro Icons`, `Inter`, `Inter Fallback`
- **Monospace**: `GeistMono` â€” Vercel's monospace font for code elements. Fallbacks: `ui-monospace`, `SFMono-Regular`, `Roboto Mono`, `Menlo`, `Monaco`
...
### Principles
- **Positive tracking on dark**: Unlike most dark UIs that use tight or neutral letter-spacing, Raycast applies +0.2px to +0.4px â€” creating an airy, readable feel that compensates for the dark background
- **Weight 500 as baseline**: Most body text uses medium weight (500), not regular (400) â€” subtle extra heft improves legibility on dark surfaces
- **Display restraint**: Hero text at 64px/600 is confident but not oversized â€” Raycast avoids typographic spectacle in favor of functional elegance
...
- **Transition**: All buttons use opacity transition for hover rather than background-color change â€” a signature Raycast interaction pattern

### Cards & Containers
- **Standard Card**: `#101111` surface, `1px solid rgba(255, 255, 255, 0.06)` border, 12pxâ€“16px border-radius
- **Elevated Card**: Ring shadow `rgb(27, 28, 30) 0px 0px 0px 1px` outer + `rgb(7, 8, 10) 0px 0px 0px 1px inset` inner â€” creates a double-ring containment
- **Feature Card**: 16pxâ€“20px border-radius, subtle warm glow (`rgba(215, 201, 175, 0.05) 0px 0px 20px 5px`) behind hero elements
...
- **Nav links**: Gray text (`#9c9c9d`) â†’ white on hover, underline decoration on hover
...
### Image Treatment
- **Product screenshots**: macOS window chrome style â€” rounded corners (12px), deep shadows simulating floating windows
- **Full-bleed sections**: Dark screenshots blend seamlessly into the dark background
- **Hero illustration**: Diagonal stripe pattern in Raycast Red â€” abstract, geometric, brand-defining
- **App UI embeds**: Showing actual Raycast command palette and extensions â€” product as content

### Keyboard Shortcut Keys
- **Key cap styling**: Gradient background (`#121212` â†’ `#0d0d0d`), heavy multi-layer shadow (`rgba(0, 0, 0, 0.4) 0px 1.5px 0.5px 2.5px` + inset shadows), creating realistic physical key appearance
- Border-radius: 4pxâ€“6px for individual keys
...
- **Section padding**: 80pxâ€“120px vertical between major sections
...
- **Column patterns**: Single-column hero, 2â€“3 column feature grids, full-width showcase sections
...
- **Vertical rhythm**: Consistent 24pxâ€“32px gaps between elements within sections

### Border Radius Scale
- **2pxâ€“3px**: Micro-elements, code spans, tiny indicators
- **4pxâ€“5px**: Keyboard keys, small interactive elements
- **6px**: Buttons, badges, tags â€” the workhorse radius
- **8px**: Input fields, inline components
- **9pxâ€“11px**: Images, medium containers
...
| Level 3 (Button) | `rgba(255, 255, 255, 0.05) 0px 1px 0px 0px inset` + `rgba(255, 255, 255, 0.25) 0px 0px 0px 1px` + `rgba(0, 0, 0, 0.2) 0px -1px 0px 0px inset` | macOS-native button press â€” white highlight top, dark inset bottom |
| Level 4 (Key) | 5-layer shadow stack with inset press effects | Keyboard shortcut key caps â€” physical 3D appearance |
| Level 5 (Floating) | `rgba(0, 0, 0, 0.5) 0px 0px 0px 2px` + `rgba(255, 255, 255, 0.19) 0px 0px 14px` + insets | Command palette, floating panels â€” heavy depth with glow |
...
- **Inset top highlights** (`rgba(255, 255, 255, 0.05â€“0.25)`) simulating light source from above
...
### Decorative Depth
- **Warm glow**: `rgba(215, 201, 175, 0.05) 0px 0px 20px 5px` behind featured elements â€” a subtle warm aura on the cold dark canvas
...
### Do
- Use `#07080a` (not pure black) as the background â€” the blue-cold tint is essential to the Raycast feel
- Apply positive letter-spacing (+0.2px) on body text â€” this is deliberately different from most dark UIs
- Use multi-layer shadows with inset highlights for interactive elements â€” the macOS-native depth is signature
- Keep Raycast Red (`#FF6363`) as punctuation, not pervasive â€” reserve it for hero moments and error states
- Use `rgba(255, 255, 255, 0.06)` borders for card containment â€” barely visible, structurally essential
- Apply weight 500 as the body text baseline â€” medium weight improves dark-mode legibility
- Use pill shapes (86px+ radius) for primary CTAs, rectangular shapes (6pxâ€“8px) for secondary actions
...
### Don't
- Use pure black (`#000000`) as the background â€” the blue tint differentiates Raycast from generic dark themes
- Apply negative letter-spacing on body text â€” Raycast deliberately uses positive spacing for readability
- Use Raycast Blue as the primary accent for everything â€” blue is for interactive/info, red is the brand color
- Create single-layer flat shadows â€” the multi-layer inset system is core to the macOS-native aesthetic
- Use regular weight (400) for body text when 500 is available â€” the extra weight prevents dark-mode text from feeling thin
- Mix warm and cool borders â€” stick to the cool gray (`hsl(195, 5%, 15%)`) border palette
- Apply heavy drop shadows without inset companions â€” shadows always come in pairs (outer + inset)
- Use decorative elements, gradients, or colorful backgrounds â€” the dark void is the stage, content is the performer
...
| Small Tablet | 600pxâ€“768px | 2-column grid begins, nav partially visible |
| Tablet | 768pxâ€“1024px | 2â€“3 column features, nav expanding, screenshots scale |
| Desktop | 1024pxâ€“1200px | Full layout, all nav links visible, 64px hero display |
| Large Desktop | >1200px | Max-width container centered, generous side margins |

### Touch Targets
- Pill buttons: 86px radius with 20px padding â€” well above 44px minimum
...
### Collapsing Strategy
- **Navigation**: Full horizontal nav â†’ hamburger at mobile with slide-out menu
- **Hero**: 64px display â†’ 48px â†’ 36px across breakpoints
...
- No lazy-loading artifacts â€” images are critical to the product narrative
...
- "Create a keyboard shortcut display with key caps using gradient background (#121212â†’#0d0d0d), 5-layer shadow for physical depth, 4px radius, Inter 12px weight 600 text"
...
1. Check the background is `#07080a` not pure black â€” the blue tint is critical
2. Verify letter-spacing is positive (+0.2px) on body text â€” negative spacing breaks the Raycast aesthetic
3. Ensure shadows have both outer and inset layers â€” single-layer shadows look flat and wrong
4. Confirm Inter has OpenType features `calt`, `kern`, `liga`, `ss03` enabled
5. Test that hover states use opacity transitions (0.6) not color swaps â€” this is a core interaction pattern
