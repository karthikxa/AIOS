...
Linear's website is a masterclass in dark-mode-first product design â€” a near-black canvas (`#08090a`) where content emerges from darkness like starlight. The overall impression is one of extreme precision engineering: every element exists in a carefully calibrated hierarchy of luminance, from barely-visible borders (`rgba(255,255,255,0.05)`) to soft, luminous text (`#f7f8f8`). This is not a dark theme applied to a light design â€” it is darkness as the native medium, where information density is managed through subtle gradations of white opacity rather than color variation.

The typography system is built entirely on Inter Variable with OpenType features `"cv01"` and `"ss03"` enabled globally, giving the typeface a cleaner, more geometric character. Inter is used at a remarkable range of weights â€” from 300 (light body) through 510 (medium, Linear's signature weight) to 590 (semibold emphasis). The 510 weight is particularly distinctive: it sits between regular and medium, creating a subtle emphasis that doesn't shout. At display sizes (72px, 64px, 48px), Inter uses aggressive negative letter-spacing (-1.584px to -1.056px), creating compressed, authoritative headlines that feel engineered rather than designed. Berkeley Mono serves as the monospace companion for code and technical labels, with fallbacks to ui-monospace, SF Mono, and Menlo.

The color system is almost entirely achromatic â€” dark backgrounds with white/gray text â€” punctuated by a single brand accent: Linear's signature indigo-violet (`#5e6ad2` for backgrounds, `#7170ff` for interactive accents). This accent color is used sparingly and intentionally, appearing only on CTAs, active states, and brand elements. The border system uses ultra-thin, semi-transparent white borders (`rgba(255,255,255,0.05)` to `rgba(255,255,255,0.08)`) that create structure without visual noise, like wireframes drawn in moonlight.

**Key Characteristics:**
- Dark-mode-native: `#08090a` marketing background, `#0f1011` panel background, `#191a1b` elevated surfaces
- Inter Variable with `"cv01", "ss03"` globally â€” geometric alternates for a cleaner aesthetic
...
- Brand indigo-violet: `#5e6ad2` (bg) / `#7170ff` (accent) / `#828fff` (hover) â€” the only chromatic color in the system
...
### Background Surfaces
- **Marketing Black** (`#010102` / `#08090a`): The deepest background â€” the canvas for hero sections and marketing pages. Near-pure black with an imperceptible blue-cool undertone.
...
- **Secondary Surface** (`#28282c`): The lightest dark surface â€” used for hover states and slightly elevated components.

### Text & Content
- **Primary Text** (`#f7f8f8`): Near-white with a barely-warm cast. The default text color â€” not pure white, preventing eye strain on dark backgrounds.
...
- **Quaternary Text** (`#62666d`): The most subdued text â€” timestamps, disabled states, subtle labels.

### Brand & Accent
- **Brand Indigo** (`#5e6ad2`): Primary brand color â€” used for CTA button backgrounds, brand marks, and key interactive surfaces.
- **Accent Violet** (`#7170ff`): Brighter variant for interactive elements â€” links, active states, selected items.
...
- **Emerald** (`#10b981`): Secondary success â€” pill badges, completion states.
...
- **Border Subtle** (`rgba(255,255,255,0.05)`): Ultra-subtle semi-transparent border â€” the default.
...
### Overlay
- **Overlay Primary** (`rgba(0,0,0,0.85)`): Modal/dialog backdrop â€” extremely dark for focus isolation.
...
- **OpenType Features**: `"cv01", "ss03"` enabled globally â€” cv01 provides an alternate lowercase 'a' (single-story), ss03 adjusts specific letterforms for a cleaner geometric appearance.
...
| Caption Large | Inter Variable | 14px (0.88rem) | 510â€“590 | 1.50 | -0.182px | Sub-labels, category headers |
| Caption | Inter Variable | 13px (0.81rem) | 400â€“510 | 1.50 | -0.13px | Metadata, timestamps |
| Label | Inter Variable | 12px (0.75rem) | 400â€“590 | 1.40 | normal | Button text, small labels |
| Micro | Inter Variable | 11px (0.69rem) | 510 | 1.40 | normal | Tiny labels |
| Tiny | Inter Variable | 10px (0.63rem) | 400â€“510 | 1.50 | -0.15px | Overline text, sometimes uppercase |
...
| Link Caption | Inter Variable | 13px (0.81rem) | 400â€“510 | 1.50 | -0.13px | Footer, metadata links |
...
- **Compression at scale**: Display sizes use progressively tighter letter-spacing â€” -1.584px at 72px, -1.408px at 64px, -1.056px at 48px, -0.704px at 32px. Below 24px, spacing relaxes toward normal.
- **OpenType as identity**: `"cv01", "ss03"` aren't decorative â€” they transform Inter into Linear's distinctive typeface, giving it a more geometric, purposeful character.
...
### Cards & Containers
- Background: `rgba(255,255,255,0.02)` to `rgba(255,255,255,0.05)` (never solid â€” always translucent)
...
- Links: Inter Variable 13â€“14px weight 510, `#d0d6e0` text
...
- Feature sections: 2â€“3 column grids for feature cards
- Full-width dark sections with internal max-width constraints
- Changelog: single-column timeline layout

### Whitespace Philosophy
- **Darkness as space**: On Linear's dark canvas, empty space isn't white â€” it's absence. The near-black background IS the whitespace, and content emerges from it.
...
- **Section isolation**: Each feature section is separated by generous vertical padding (80px+) with no visible dividers â€” the dark background provides natural separation.
...
**Shadow Philosophy**: On dark surfaces, traditional shadows (dark on dark) are nearly invisible. Linear solves this by using semi-transparent white borders as the primary depth indicator. Elevation isn't communicated through shadow darkness but through background luminance steps â€” each level slightly increases the white opacity of the surface background (`0.02` â†’ `0.04` â†’ `0.05`), creating a subtle stacking effect. The inset shadow technique (`rgba(0,0,0,0.2) 0px 0px 12px 0px inset`) creates a unique "sunken" effect for recessed panels, adding dimensional depth that traditional dark themes lack.

## 7. Do's and Don'ts

### Do
- Use Inter Variable with `"cv01", "ss03"` on ALL text â€” these features are fundamental to Linear's typeface identity
- Use weight 510 as your default emphasis weight â€” it's Linear's signature between-weight
...
- Use `#f7f8f8` for primary text â€” not pure `#ffffff`, which would be too harsh
- Apply the luminance stacking model: deeper = darker bg, elevated = slightly lighter bg

### Don't
- Don't use pure white (`#ffffff`) as primary text â€” `#f7f8f8` prevents eye strain
- Don't use solid colored backgrounds for buttons â€” transparency is the system (rgba white at 0.02â€“0.05)
- Don't apply the brand indigo decoratively â€” it's reserved for interactive/CTA elements only
- Don't use positive letter-spacing on display text â€” Inter at large sizes always runs negative
- Don't use visible/opaque borders on dark backgrounds â€” borders should be whisper-thin semi-transparent white
- Don't skip the OpenType features (`"cv01", "ss03"`) â€” without them, it's generic Inter, not Linear's Inter
- Don't use weight 700 (bold) â€” Linear's maximum weight is 590, with 510 as the workhorse
- Don't introduce warm colors into the UI chrome â€” the palette is cool gray with blue-violet accent only
- Don't use drop shadows for elevation on dark surfaces â€” use background luminance stepping instead
...
| Mobile | 600â€“640px | Standard mobile layout |
| Tablet | 640â€“768px | Two-column grids begin |
| Desktop Small | 768â€“1024px | Full card grids, expanded padding |
| Desktop | 1024â€“1280px | Standard desktop, full navigation |
| Large Desktop | >1280px | Full layout, generous margins |

### Touch Targets
- Buttons use comfortable padding with 6px radius minimum
- Navigation links at 13â€“14px with adequate spacing
...
### Collapsing Strategy
- Hero: 72px â†’ 48px â†’ 32px display text, tracking adjusts proportionally
- Navigation: horizontal links + CTAs â†’ hamburger menu at 768px
...
- Footer: multi-column â†’ stacked single column
- Section spacing: 80px+ â†’ 48px on mobile
...
### Iteration Guide
1. Always set font-feature-settings `"cv01", "ss03"` on all Inter text â€” this is non-negotiable for Linear's look
...
4. Surface elevation via background opacity: `rgba(255,255,255, 0.02 â†’ 0.04 â†’ 0.05)` â€” never solid backgrounds on dark
5. Brand indigo (`#5e6ad2` / `#7170ff`) is the only chromatic color â€” everything else is grayscale
