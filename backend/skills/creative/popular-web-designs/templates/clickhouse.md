...
**Key Characteristics:**
- Pure black canvas (#000000) with neon yellow-green (#faff69) accent â€” maximum contrast
...
- All links hover to neon yellow-green â€” unified interactive signal
- Inset shadows on select elements creating "pressed into the surface" depth

## 2. Color Palette & Roles

### Primary
- **Neon Volt** (`#faff69`): The signature brand color â€” a vivid acid yellow-green that's the sole chromatic accent on the black canvas. Used for primary CTAs, accent borders, link hovers, and highlighted moments.
- **Forest Green** (`#166534`): Secondary CTA color â€” a deep, saturated green for "Get Started" and primary action buttons that need distinction from the neon.
- **Dark Forest** (`#14572f`): A darker green variant for borders and secondary accents.

### Secondary & Accent
- **Pale Yellow** (`#f4f692`): Active/pressed state text color â€” a softer, more muted version of Neon Volt for state feedback.
- **Border Olive** (`#4f5100`): A dark olive-yellow for ghost button borders â€” the neon's muted sibling.
- **Olive Dark** (`#161600`): The darkest neon-tinted color for subtle brand text.

### Surface & Background
- **Pure Black** (`#000000`): The primary page background â€” absolute black for maximum contrast.
- **Near Black** (`#141414`): Button backgrounds and slightly elevated dark surfaces.
- **Charcoal** (`#414141`): The primary border color at 80% opacity â€” the workhorse for card and container containment.
- **Deep Charcoal** (`#343434`): Darker border variant for subtle division lines.
- **Hover Gray** (`#3a3a3a`): Button hover state background â€” slightly lighter than Near Black.
...
### Gradient System
- **None in the traditional sense.** ClickHouse uses flat color blocks and high-contrast borders. The "gradient" is the contrast itself â€” neon yellow-green against pure black creates a visual intensity that gradients would dilute.
...
| Sub-heading | Inter / Basier | 24px (1.5rem) | 600â€“700 | 1.17â€“1.38 | normal | Card headings |
| Feature Title | Inter / Basier | 20px (1.25rem) | 600â€“700 | 1.40 | normal | Small feature titles |
| Body Large | Inter | 18px (1.13rem) | 400â€“700 | 1.56 | normal | Intro paragraphs, button text |
| Body / Button | Inter | 16px (1rem) | 400â€“700 | 1.50 | normal | Standard body, nav, buttons |
| Caption | Inter | 14px (0.88rem) | 400â€“700 | 1.43 | normal | Metadata, descriptions, links |
...
### Principles
- **Weight 900 is the weapon**: The display headline uses Inter Black (900) â€” a weight most sites never touch. Combined with 96px size, this creates text with a physical, almost architectural presence.
- **Full weight spectrum**: The system uses 400, 500, 600, 700, and 900 â€” covering the full gamut. Weight IS hierarchy.
- **Uppercase with maximum tracking**: Section overlines use 1.4px letter-spacing â€” wider than most systems â€” creating bold structural labels that stand out against the dense dark background.
...
- The eye-catching CTA â€” neon on black
...
- Border: `1px solid rgba(65, 65, 65, 0.8)` â€” the signature charcoal containment
...
- Shadow Level 3: inset (`rgba(0,0,0,0.06) 0px 4px 4px, rgba(0,0,0,0.14) 0px 4px 25px inset`) â€” the "pressed" effect
...
**Performance Stats**
- Oversized numbers (72px+, weight 700â€“900)
...
- Section vertical spacing: generous (48â€“64px)
...
- Full-dark page â€” no light sections

### Whitespace Philosophy
- **Dark void as canvas**: The pure black background provides infinite depth â€” elements float in darkness.
...
| Pressed/Inset (Level 4) | `0px 4px 25px rgba(0,0,0,0.14) inset` | Active/pressed elements â€” "sunk into the surface" |
| Neon Highlight (Level 5) | Neon Volt border (`#faff69`) | Featured/selected cards, maximum emphasis |

**Shadow Philosophy**: ClickHouse uses shadows on a black canvas, where they're barely visible â€” they exist more for subtle dimensionality than obvious elevation. The most distinctive depth mechanism is the **inset shadow** (Level 4), which creates a "pressed into the surface" effect unique to ClickHouse. The neon border highlight (Level 5) is the primary attention-getting depth mechanism.

## 7. Do's and Don'ts

### Do
- Use Neon Volt (#faff69) as the sole chromatic accent â€” it must pop against pure black
- Use Inter at weight 900 for hero display text â€” the extreme weight IS the personality
- Keep everything on pure black (#000000) â€” never use dark gray as the page background
- Use charcoal borders (rgba(65,65,65,0.8)) for all card containment
- Apply Forest Green (#166534) for primary CTA buttons â€” distinct from neon for action hierarchy
- Show performance stats as oversized display numbers â€” it's the core visual argument
- Use uppercase with wide letter-spacing (1.4px) for section labels
- Apply Pale Yellow (#f4f692) for active/pressed text states
- Link hovers should ALWAYS shift to Neon Volt â€” unified interactive feedback

### Don't
- Don't introduce additional colors â€” the palette is strictly black, neon, green, and gray
- Don't use the neon as a background fill â€” it's an accent and border color only (except on CTA buttons)
- Don't reduce display weight below 700 â€” heavy weight is core to the personality
- Don't use light/white backgrounds anywhere â€” the entire experience is dark
- Don't round corners beyond 8px â€” the sharp geometry reflects database precision
- Don't use soft/diffused shadows on black â€” they're invisible. Use border-based depth instead
- Don't skip the inset shadow on active states â€” the "pressed" effect is distinctive
- Don't use warm neutrals â€” all grays are perfectly neutral
...
| Small Tablet | 640â€“768px | Minor adjustments |
| Tablet | 768â€“1024px | 2-column grids |
| Desktop | 1024â€“1280px | Standard layout |
| Large Desktop | 1280â€“1536px | Expanded content |
| Ultra-wide | 1536â€“2200px | Maximum container width |
...
### Collapsing Strategy
- **Hero text**: 96px â†’ 72px â†’ 48px â†’ 36px
- **Feature grids**: Multi-column â†’ 2 â†’ 1 column
- **Stats**: Horizontal â†’ stacked
...
### Iteration Guide
1. Keep everything on pure black â€” no dark gray alternatives
2. Neon Volt (#faff69) is for accents and CTAs only â€” never large backgrounds
3. Weight 900 for hero, 700 for headings, 600 for labels, 400-500 for body
4. Active states use Pale Yellow (#f4f692) â€” not just opacity changes
5. All links hover to Neon Volt â€” consistent interactive feedback
