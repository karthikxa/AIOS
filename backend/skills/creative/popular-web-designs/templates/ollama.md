...
Ollama's interface is radical minimalism taken to its logical conclusion â€” a pure-white void where content floats without decoration, shadow, or color. The design philosophy mirrors the product itself: strip away everything unnecessary until only the essential tool remains. This is the digital equivalent of a Dieter Rams object â€” every pixel earns its place, and the absence of design IS the design.

The entire page exists in pure grayscale. There is zero chromatic color in the interface â€” no brand blue, no accent green, no semantic red. The only colors that exist are shades between pure black (`#000000`) and pure white (`#ffffff`), creating a monochrome environment that lets the user's mental model of "open models" remain uncolored by brand opinion. The Ollama llama mascot, rendered in simple black line art, is the only illustration â€” and even it's monochrome.

What makes Ollama distinctive is the combination of SF Pro Rounded (Apple's rounded system font) with an exclusively pill-shaped geometry (9999px radius on everything interactive). The rounded letterforms + rounded buttons + rounded containers create a cohesive "softness language" that makes a developer CLI tool feel approachable and friendly rather than intimidating. This is minimalism with warmth â€” not cold Swiss-style grid minimalism, but the kind where the edges are literally softened.

**Key Characteristics:**
- Pure white canvas with zero chromatic color â€” completely grayscale
...
- Zero shadows â€” depth comes exclusively from background color shifts and borders
- Pill-shaped geometry on all interactive elements (buttons, tabs, inputs, tags)
- The Ollama llama as the sole illustration â€” black line art, no color
- Extreme content restraint â€” the homepage is short, focused, and uncluttered
...
- **Darkest Surface** (`#090909`): The darkest possible surface â€” barely distinguishable from pure black, used for footer or dark containers.

### Surface & Background
- **Pure White** (`#ffffff`): The primary page background â€” not off-white, not cream, pure white. Button surfaces for secondary actions.
- **Snow** (`#fafafa`): The subtlest possible surface distinction from white â€” used for section backgrounds and barely-elevated containers.
...
### Semantic & Accent
- **Ring Blue** (`#3b82f6` at 50%): The ONLY non-gray color in the entire system â€” Tailwind's default focus ring, used exclusively for keyboard accessibility. Never visible in normal interaction flow.
...
*Note: SF Pro Rounded is Apple's system font â€” it renders with rounded terminals on macOS/iOS and falls back to the system sans-serif on other platforms.*
...
| Sub-heading | SF Pro Rounded / ui-sans-serif | 30px (1.88rem) | 400â€“500 | 1.20 (tight) | normal | Card headings, feature names |
| Card Title | ui-sans-serif | 24px (1.5rem) | 400 | 1.33 | normal | Medium emphasis headings |
| Body Large | ui-sans-serif | 18px (1.13rem) | 400â€“500 | 1.56 | normal | Hero descriptions, button text |
| Body / Link | ui-sans-serif | 16px (1rem) | 400â€“500 | 1.50 | normal | Standard body text, navigation |
...
| Code Small | ui-monospace | 12px (0.75rem) | 400â€“700 | 1.63 | normal | Tags, labels |
...
- **Weight restraint**: Only two weights matter â€” 400 (regular) for body and 500 (medium) for headings. No bold, no light, no black weight. This extreme restraint reinforces the minimal philosophy.
- **Tight display, comfortable body**: Headlines compress to 1.0 line-height, while body text relaxes to 1.43â€“1.56. The contrast creates clear hierarchy without needing weight contrast.
...
- The primary action button â€” understated, grayscale, always pill-shaped
...
- Radius: pill-shaped (9999px)
- Secondary action â€” visually lighter than Gray Pill
...
- Maximum emphasis â€” black on white

### Cards & Containers
- Background: Pure White or Snow (`#fafafa`)
- Border: thin solid Light Gray (`1px solid #e5e5e5`) when needed
- Radius: comfortably rounded (12px) â€” the ONLY non-pill radius in the system
- Shadow: **none** â€” zero shadows on any element
- Hover: likely subtle background shift or border darkening

### Inputs & Forms
- Background: Pure White
- Border: `1px solid #e5e5e5`
- Radius: pill-shaped (9999px) â€” search inputs and form fields are pill-shaped
...
- No borders, no background â€” transparent nav on white page

### Image Treatment
- The Ollama llama mascot is the only illustration â€” black line art on white
...
- Minimal styling â€” just a bordered 12px-radius container
...
- Section vertical spacing: very generous (88pxâ€“112px)

### Grid & Container
- Max container width: approximately 1024â€“1280px, centered
...
- **Emptiness as luxury**: The page is remarkably short and sparse â€” no feature section overstays its welcome. Each concept gets minimal but sufficient space.
...
### Border Radius Scale
- Comfortably rounded (12px): The sole container radius â€” code blocks, cards, panels
- Pill-shaped (9999px): Everything interactive â€” buttons, tabs, inputs, tags, badges
...
**Shadow Philosophy**: Ollama uses **zero shadows**. This is not an oversight â€” it's a deliberate design decision. Every other major AI product site uses at least subtle shadows. Ollama's flat, shadowless approach creates a paper-like experience where elements are distinguished purely by background color and single-pixel borders. Depth is communicated through **content hierarchy and typography weight**, not visual layering.

## 7. Do's and Don'ts

### Do
- Use pure white (`#ffffff`) as the page background â€” never off-white or cream
- Use pill-shaped (9999px) radius on all interactive elements â€” buttons, tabs, inputs, tags
- Use 12px radius on all non-interactive containers â€” code blocks, cards, panels
- Keep the palette strictly grayscale â€” no chromatic colors except the blue focus ring
- Use SF Pro Rounded at weight 500 for display headings â€” the rounded terminals are the brand expression
- Maintain zero shadows â€” depth comes from borders and background shifts only
- Keep content density low â€” each section should present one clear idea
- Use monospace for terminal commands and code â€” it's primary content, not decoration
- Keep all buttons at 10px 24px padding with pill shape â€” consistency is absolute

### Don't
- Don't introduce any chromatic color â€” no brand blue, no accent green, no warm tones
- Don't use border-radius between 12px and 9999px â€” the system is binary
- Don't add shadows to any element â€” the flat aesthetic is intentional
- Don't use font weights above 500 â€” no bold, no black weight
- Don't add decorative illustrations beyond the llama mascot
- Don't use gradients anywhere â€” flat blocks and borders only
- Don't overcomplicate the layout â€” two columns maximum, no complex grids
- Don't use borders heavier than 1px â€” containment is always the lightest possible touch
- Don't add hover animations or transitions â€” interactions should feel instant and direct
...
| Small Tablet | 640â€“768px | Minor adjustments to spacing |
| Tablet | 768â€“850px | 2-column layouts begin |
| Desktop | 850â€“1024px | Standard layout, expanded features |
| Large Desktop | 1024â€“1280px | Maximum content width |
...
- **Feature sections**: 2-column â†’ stacked single column
- **Hero text**: 48px â†’ 36px â†’ 30px progressive scaling
- **Integration grid**: Multi-column â†’ 2-column â†’ single column
...
2. Keep all values grayscale â€” "Stone (#737373)" not "use a light color"
3. Always specify pill (9999px) or container (12px) radius â€” nothing in between
4. Shadows are always zero â€” never add them
5. Weight is always 400 or 500 â€” never bold
6. If something feels too decorated, remove it â€” less is always more for Ollama
