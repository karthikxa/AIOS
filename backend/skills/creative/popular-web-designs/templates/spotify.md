...
Spotify's web interface is a dark, immersive music player that wraps listeners in a near-black cocoon (`#121212`, `#181818`, `#1f1f1f`) where album art and content become the primary source of color. The design philosophy is "content-first darkness" â€” the UI recedes into shadow so that music, podcasts, and playlists can glow. Every surface is a shade of charcoal, creating a theater-like environment where the only true color comes from the iconic Spotify Green (`#1ed760`) and the album artwork itself.

The typography uses SpotifyMixUI and SpotifyMixUITitle â€” proprietary fonts from the CircularSp family (Circular by Lineto, customized for Spotify) with an extensive fallback stack that includes Arabic, Hebrew, Cyrillic, Greek, Devanagari, and CJK fonts, reflecting Spotify's global reach. The type system is compact and functional: 700 (bold) for emphasis and navigation, 600 (semibold) for secondary emphasis, and 400 (regular) for body. Buttons use uppercase with positive letter-spacing (1.4pxâ€“2px) for a systematic, label-like quality.

What distinguishes Spotify is its pill-and-circle geometry. Primary buttons use 500pxâ€“9999px radius (full pill), circular play buttons use 50% radius, and search inputs are 500px pills. Combined with heavy shadows (`rgba(0,0,0,0.5) 0px 8px 24px`) on elevated elements and a unique inset border-shadow combo (`rgb(18,18,18) 0px 1px 0px, rgb(124,124,124) 0px 0px 0px 1px inset`), the result is an interface that feels like a premium audio device â€” tactile, rounded, and built for touch.

**Key Characteristics:**
- Near-black immersive dark theme (`#121212`â€“`#1f1f1f`) â€” UI disappears behind content
- Spotify Green (`#1ed760`) as singular brand accent â€” never decorative, always functional
- SpotifyMixUI/CircularSp font family with global script support
- Pill buttons (500pxâ€“9999px) and circular controls (50%) â€” rounded, touch-optimized
- Uppercase button labels with wide letter-spacing (1.4pxâ€“2px)
- Heavy shadows on elevated elements (`rgba(0,0,0,0.5) 0px 8px 24px`)
- Semantic colors: negative red (`#f3727f`), warning orange (`#ffa42b`), announcement blue (`#539df5`)
- Album art as the primary color source â€” the UI is achromatic by design

## 2. Color Palette & Roles

### Primary Brand
- **Spotify Green** (`#1ed760`): Primary brand accent â€” play buttons, active states, CTAs
...
| Button Uppercase | SpotifyMixUI | 14px (0.88rem) | 600â€“700 | 1.00 (tight) | 1.4pxâ€“2px | `text-transform: uppercase` |
...
| Caption Bold | SpotifyMixUI | 14px (0.88rem) | 700 | 1.50â€“1.54 | normal | Bold metadata |
...
- **Uppercase buttons as system**: Button labels use uppercase + wide letter-spacing (1.4pxâ€“2px), creating a systematic "label" voice distinct from content text.
- **Compact sizing**: The range is 10pxâ€“24px â€” narrower than most systems. Spotify's type is compact and functional, designed for scanning playlists, not reading articles.
...
- **Dark compression**: Spotify packs content densely â€” playlist grids, track lists, and navigation are all tightly spaced. The dark background provides visual rest between elements without needing large gaps.
...
- Medium (10pxâ€“20px): Panels, overlay elements
...
### Do
- Use near-black backgrounds (`#121212`â€“`#1f1f1f`) â€” depth through shade variation
- Apply Spotify Green (`#1ed760`) only for play controls, active states, and primary CTAs
- Use pill shape (500pxâ€“9999px) for all buttons â€” circular (50%) for play controls
- Apply uppercase + wide letter-spacing (1.4pxâ€“2px) on button labels
- Keep typography compact (10pxâ€“24px range) â€” this is an app, not a magazine
- Use heavy shadows (`0.3â€“0.5 opacity`) for elevated elements on dark backgrounds
- Let album art provide color â€” the UI itself is achromatic

### Don't
- Don't use Spotify Green decoratively or on backgrounds â€” it's functional only
- Don't use light backgrounds for primary surfaces â€” the dark immersion is core
- Don't skip the pill/circle geometry on buttons â€” square buttons break the identity
- Don't use thin/subtle shadows â€” on dark backgrounds, shadows need to be heavy to be visible
- Don't add additional brand colors â€” green + achromatic grays is the complete palette
- Don't use relaxed line-heights â€” Spotify's typography is compact and dense
- Don't expose raw gray borders â€” use shadow-based or inset borders instead
...
| Mobile | 425â€“576px | Standard mobile |
| Tablet | 576â€“768px | 2-column grid |
| Tablet Large | 768â€“896px | Expanded layout |
| Desktop Small | 896â€“1024px | Sidebar visible |
| Desktop | 1024â€“1280px | Full desktop layout |
| Large Desktop | >1280px | Expanded grid |

### Collapsing Strategy
- Sidebar: full â†’ collapsed â†’ hidden
- Album grid: 5 columns â†’ 3 â†’ 2 â†’ 1
- Now-playing bar: maintained at all sizes
- Search: pill input maintained, width adjusts
- Navigation: sidebar â†’ bottom bar on mobile
...
### Iteration Guide
1. Start with #121212 â€” everything lives in near-black darkness
2. Spotify Green for functional highlights only (play, active, CTA)
3. Pill everything â€” 500px for large, 9999px for small, 50% for circular
4. Uppercase + wide tracking on buttons â€” the systematic label voice
5. Heavy shadows (0.3â€“0.5 opacity) for elevation â€” light shadows are invisible on dark
6. Album art provides all the color â€” the UI stays achromatic
