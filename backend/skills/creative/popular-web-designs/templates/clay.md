...
Clay's website is a warm, playful celebration of color that treats B2B data enrichment like a craft rather than an enterprise chore. The design language is built on a foundation of warm cream backgrounds (`#faf9f7`) and oat-toned borders (`#dad4c8`, `#eee9df`) that give every surface the tactile quality of handmade paper. Against this artisanal canvas, a vivid swatch palette explodes with personality â€” Matcha green, Slushie cyan, Lemon gold, Ube purple, Pomegranate pink, Blueberry navy, and Dragonfruit magenta â€” each named like flavors at a juice bar, not colors in an enterprise UI kit.
...
- **Warm Cream** (`#faf9f7`): Page background â€” the warm, paper-like canvas

### Swatch Palette â€” Named Colors
...
### Surface & Border
- **Oat Border** (`#dad4c8`): Primary border â€” warm, cream-toned structural lines
...
### Shadows
- **Clay Shadow** (`rgba(0,0,0,0.1) 0px 1px 1px, rgba(0,0,0,0.04) 0px -1px 1px inset, rgba(0,0,0,0.05) 0px -0.5px 1px`): Multi-layer with inset highlight â€” the signature
- **Hard Offset** (`rgb(0,0,0) -7px 7px`): Hover state â€” playful hard shadow
...
| Body Medium | Roobert | 16px (1.00rem) | 500 | 1.20â€“1.40 | -0.16px to -0.32px | 4â€“5 stylistic sets |
...
| Caption | Roobert | 14px (0.88rem) | 400 | 1.50â€“1.60 | -0.14px | 4 stylistic sets |
...
| Badge | Roobert | 9.6px | 600 | â€” | â€” | Pill badges |

### Principles
- **Five stylistic sets as identity**: The combination of `"ss01"`, `"ss03"`, `"ss10"`, `"ss11"`, `"ss12"` on Roobert creates a distinctive typographic personality. `ss01` is reserved for headings and emphasis â€” body text omits it, creating a subtle hierarchy through glyph variation.
- **Aggressive display compression**: -3.2px at 80px, -2.4px at 60px â€” the most compressed display tracking alongside the most generous body spacing (1.60 line-height), creating dramatic contrast.
...
- Card grids: 2â€“3 columns on desktop
- Full-width colorful sections break the grid
- Footer with generous 40px radius container

### Whitespace Philosophy
- **Warm, generous breathing**: The cream background provides a warm rest between content blocks. Spacing is generous but not austere â€” it feels inviting, like a well-set table.
...
| Clay Shadow (Level 1) | `rgba(0,0,0,0.1) 0px 1px 1px, rgba(0,0,0,0.04) 0px -1px inset, rgba(0,0,0,0.05) 0px -0.5px` | Cards, buttons â€” multi-layer with inset highlight |
| Hover Hard (Level 2) | `rgb(0,0,0) -7px 7px` | Hover state â€” playful hard offset shadow |
| Focus (Level 3) | `rgb(20, 110, 245) solid 2px` | Keyboard focus ring |

**Shadow Philosophy**: Clay's shadow system is uniquely three-layered: a downward cast (`0px 1px 1px`), an upward inset highlight (`0px -1px 1px inset`), and a subtle edge (`0px -0.5px 1px`). This creates a "pressed into clay" quality where elements feel both raised AND embedded â€” like a clay tablet where content is stamped into the surface. The hover hard shadow (`-7px 7px`) is deliberately retro-graphic, referencing print-era drop shadows and adding physical playfulness.
...
### Do
- Use warm cream (`#faf9f7`) as the page background â€” the warmth is the identity
...
- Use warm oat borders (`#dad4c8`) â€” not neutral gray
...
### Don't
- Don't use cool gray backgrounds â€” the warm cream (`#faf9f7`) is non-negotiable
- Don't use neutral gray borders (`#ccc`, `#ddd`) â€” always use the warm oat tones
- Don't mix more than 2 swatch colors in the same section
- Don't skip the OpenType stylistic sets â€” they define Roobert's character
- Don't use subtle hover effects â€” the rotation + hard shadow is the signature interaction
- Don't use small border radius (<12px) on feature cards â€” the generous rounding is structural
- Don't use standard shadows (blur-based) â€” Clay uses hard offset and multi-layer inset
- Don't forget the uppercase labels with 1.08px tracking â€” they're the wayfinding system
...
| Mobile | 479â€“767px | Standard mobile, stacked layout |
| Tablet | 768â€“991px | 2-column grids, condensed nav |
...
### Collapsing Strategy
- Hero: 80px â†’ 60px â†’ smaller display text
- Navigation: horizontal â†’ hamburger at 767px
- Feature sections: multi-column â†’ stacked
- Colorful sections: maintain full-width but compress padding
- Card grids: 3-column â†’ 2-column â†’ single column
...
### Iteration Guide
1. Start with warm cream (#faf9f7) â€” never cool white
2. Swatch colors are for full sections, not small accents â€” go bold with matcha, slushie, ube
3. Oat borders (#dad4c8) everywhere â€” dashed variants for decoration
4. OpenType stylistic sets are mandatory â€” they make Roobert look like Roobert
5. Hover animations are the signature â€” rotation + hard shadow, not subtle fades
6. Generous radius: 24px cards, 40px sections â€” nothing looks sharp or corporate
7. Three weights: 600 (headings), 500 (UI), 400 (body) â€” strict roles
