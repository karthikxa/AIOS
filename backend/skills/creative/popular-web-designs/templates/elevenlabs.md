...
ElevenLabs' website is a study in restrained elegance â€” a near-white canvas (`#ffffff`, `#f5f5f5`) where typography and subtle shadows do all the heavy lifting. The design feels like a premium audio product brochure: clean, spacious, and confident enough to let the content speak (literally, given ElevenLabs makes voice AI). There's an almost Apple-like quality to the whitespace strategy, but warmer â€” the occasional warm stone tint (`#f5f2ef`, `#777169`) prevents the purity from feeling clinical.

The typography system is built on a fascinating duality: Waldenburg at weight 300 (light) for display headings creates ethereal, whisper-thin titles that feel like sound waves rendered in type â€” delicate, precise, and surprisingly impactful at large sizes. This light-weight display approach is the design's signature â€” where most sites use bold headings to grab attention, ElevenLabs uses lightness to create intrigue. Inter handles all body and UI text with workmanlike reliability, using slight positive letter-spacing (0.14pxâ€“0.18px) that gives body text an airy, well-spaced quality. WaldenburgFH appears as a bold uppercase variant for specific button labels.

What makes ElevenLabs distinctive is its multi-layered shadow system. Rather than simple box-shadows, elements use complex stacks: inset border-shadows (`rgba(0,0,0,0.075) 0px 0px 0px 0.5px inset`), outline shadows (`rgba(0,0,0,0.06) 0px 0px 0px 1px`), and soft elevation shadows (`rgba(0,0,0,0.04) 0px 4px 4px`) â€” all at remarkably low opacities. The result is a design where surfaces seem to barely exist, floating just above the page with the lightest possible touch. Pill-shaped buttons (9999px) with warm-tinted backgrounds (`rgba(245,242,239,0.8)`) and warm shadows (`rgba(78,50,23,0.04)`) add a tactile, physical quality.

**Key Characteristics:**
- Near-white canvas with warm undertones (`#f5f5f5`, `#f5f2ef`)
- Waldenburg weight 300 (light) for display â€” ethereal, whisper-thin headings
- Inter with positive letter-spacing (0.14â€“0.18px) for body â€” airy readability
- Multi-layered shadow stacks at sub-0.1 opacity â€” surfaces barely exist
- Pill buttons (9999px) with warm stone-tinted backgrounds
- WaldenburgFH bold uppercase for specific CTA labels
- Warm shadow tints: `rgba(78, 50, 23, 0.04)` â€” shadows have color, not just darkness
...
- **Warm Stone** (`#f5f2ef`): Button background (at 80% opacity) â€” the warm signature
...
### Interactive
- **Grid Cyan** (`#7fffff`): `--grid-column-bg`, at 25% opacity â€” decorative grid overlay
...
| Body | Inter | 18px (1.13rem) | 400 | 1.44â€“1.60 | 0.18px | Standard reading text |
| Body Standard | Inter | 16px (1.00rem) | 400 | 1.50 | 0.16px | UI text |
| Body Medium | Inter | 16px (1.00rem) | 500 | 1.50 | 0.16px | Emphasized body |
| Nav / UI | Inter | 15px (0.94rem) | 500 | 1.33â€“1.47 | 0.15px | Navigation links |
...
| Caption | Inter | 14px (0.88rem) | 400â€“500 | 1.43â€“1.50 | 0.14px | Metadata |
...
### Principles
- **Light as the hero weight**: Waldenburg at 300 is the defining typographic choice. Where other design systems use bold for impact, ElevenLabs uses lightness â€” thin strokes that feel like audio waveforms, creating intrigue through restraint.
...
- **WaldenburgFH for emphasis**: A bold (700) uppercase variant of Waldenburg appears only in specific CTA button labels with 0.7px letter-spacing â€” the one place where the type system gets loud.
...
- Use: Featured CTA, hero action â€” the signature warm button
...
- 20pxâ€“24px radius on image containers
...
- **Warm emptiness**: The whitespace isn't cold â€” the warm stone undertones and warm shadows give empty space a tactile, physical quality.
...
- Comfortable (10pxâ€“12px): Medium cards, dropdowns
- Card (16px): Standard cards, articles
- Large (18pxâ€“20px): Featured cards, code panels
...
- Use 9999px radius for primary buttons â€” pill shape is standard
...
### Don't
- Don't use bold (700) Waldenburg for headings â€” weight 300 is non-negotiable
- Don't use heavy shadows (>0.1 opacity) â€” the ethereal quality requires whisper-level depth
- Don't use cool gray borders â€” the system is warm-tinted throughout
- Don't skip the inset shadow component â€” half-pixel inset borders define edges
- Don't apply negative letter-spacing to body text â€” Inter uses positive tracking
- Don't use sharp corners (<8px) on cards â€” the generous radius is structural
- Don't introduce brand colors â€” the palette is intentionally achromatic with warm undertones
- Don't make buttons opaque and heavy â€” the warm translucent stone treatment is the signature
...
### Touch Targets
- Pill buttons with generous padding (12pxâ€“20px)
- Navigation links at 15px with adequate spacing
- Select dropdowns maintain comfortable sizing

### Collapsing Strategy
- Navigation: horizontal â†’ hamburger at 1024px
- Feature grids: multi-column â†’ stacked
...
### Iteration Guide
1. Start with white â€” the warm undertone comes from shadows and stone surfaces, not backgrounds
2. Waldenburg 300 for headings â€” never bold, the lightness is the identity
3. Multi-layer shadows: always include inset + outline + elevation at sub-0.1 opacity
4. Positive letter-spacing on Inter body (+0.14px to +0.18px) â€” the airy reading quality
5. Warm stone CTA is the signature â€” `rgba(245,242,239,0.8)` with `rgba(78,50,23,0.04)` shadow
6. Pill (9999px) for buttons, generous radius (16pxâ€“24px) for cards
