...
- Pill-shaped geometry everywhere â€” buttons, tabs, containers, images (24pxâ€“9999px radius)
- Massive display headlines (64px) with extreme negative letter-spacing (-1.6px to -3px)
- Inter as the sole typeface, used at weights 400â€“900 for full expressive range
...
### Primary
- **Expo Black** (`#000000`): The absolute anchor â€” used for primary headlines, CTA buttons, and the brand identity. Pure black on cool white creates maximum contrast without feeling aggressive.
- **Near Black** (`#1c2024`): The primary text color for body content â€” a barely perceptible blue-black that's softer than pure #000 for extended reading.

### Secondary & Accent
- **Link Cobalt** (`#0d74ce`): The standard link color â€” a trustworthy, saturated blue that signals interactivity without competing with the monochrome hierarchy.
- **Legal Blue** (`#476cff`): A brighter, more saturated blue for legal/footer links â€” slightly more attention-grabbing than Link Cobalt.
- **Widget Sky** (`#47c2ff`): A light, friendly cyan-blue for widget branding elements â€” the brightest accent in the system.
- **Preview Purple** (`#8145b5`): A rich violet used for "preview" or beta feature indicators â€” creating clear visual distinction from standard content.

### Surface & Background
- **Cloud Gray** (`#f0f0f3`): The primary page background â€” a cool off-white with the faintest blue-violet tint. Not warm, not sterile â€” precisely technological.
...
### Semantic & Accent
- **Warning Amber** (`#ab6400`): A warm, deep amber for warning states â€” deliberately not bright yellow, conveying seriousness.
- **Destructive Rose** (`#eb8e90`): A soft pink-coral for disabled destructive actions â€” gentler than typical red, reducing alarm fatigue.
- **Border Lavender** (`#e0e1e6`): Standard card/container borders â€” a cool lavender-gray that's visible without being heavy.
- **Input Border** (`#d9d9e0`): Button and form element borders â€” slightly warmer/darker than card borders for interactive elements.
- **Dark Focus Ring** (`#2547d0`): Deep blue for keyboard focus indicators in dark theme contexts.

### Gradient System
- The design is notably **gradient-free** in the interface layer. Visual richness comes from product screenshots, the React universe illustration, and careful shadow layering rather than color gradients. This absence IS the design decision â€” gradients would undermine the clinical precision.
...
|------|------|------|--------|-------------|----------------|-------|
| Display / Hero | Inter | 64px (4rem) | 700â€“900 | 1.10 (tight) | -1.6px to -3px | Maximum impact, extreme tracking |
...
| Body Large | Inter | 18px (1.13rem) | 400â€“500 | 1.40 | normal | Intro paragraphs, section descriptions |
| Body / Button | Inter | 16px (1rem) | 400â€“700 | 1.25â€“1.40 | normal | Standard text, nav links, buttons |
| Caption / Label | Inter | 14px (0.88rem) | 400â€“600 | 1.00â€“1.40 | normal | Descriptions, metadata, badge text |
| Tag / Small | Inter | 12px (0.75rem) | 500 | 1.00â€“1.60 | normal | Smallest sans-serif text, badges |
| Code Body | JetBrains Mono | 16px (1rem) | 400â€“600 | 1.40 | normal | Inline code, terminal commands |
| Code Caption | JetBrains Mono | 14px (0.88rem) | 400â€“600 | 1.40 | normal | Code snippets, technical labels |
...
- **Weight as hierarchy**: 700â€“900 for display, 600 for headings, 500 for emphasis, 400 for body. The jumps are decisive â€” no ambiguous in-between weights.
...
- The understated default â€” clean, professional, unheroic
...
- Pill-shaped (9999px) or generously rounded (32â€“36px)
- No border (black IS the border)
- The maximum-emphasis CTA â€” reserved for primary conversion actions

### Cards & Containers
- Background: Pure White (`#ffffff`) â€” clearly lifted from Cloud Gray page
- Border: thin solid Border Lavender (`1px solid #e0e1e6`) for standard cards
- Radius: comfortably rounded (8px) for standard cards; generously rounded (16â€“24px) for featured containers
- Shadow Level 1: Whisper (`rgba(0,0,0,0.08) 0px 3px 6px, rgba(0,0,0,0.07) 0px 2px 4px`) â€” barely perceptible lift
- Shadow Level 2: Standard (`rgba(0,0,0,0.1) 0px 10px 20px, rgba(0,0,0,0.05) 0px 3px 6px`) â€” clear floating elevation
...
- Links: Near Black (`#1c2024`) or Slate Gray (`#60646c`) at 14â€“16px Inter weight 500
...
- Green dot + text â€” compact trust signal
...
- Button padding: 0px 12px (unusually compact â€” height driven by line-height)
- Card internal padding: approximately 24â€“32px
- Section vertical spacing: enormous (estimated 96â€“144px between major sections)
- Component gap: 16â€“24px between sibling elements

### Grid & Container
- Max container width: approximately 1200â€“1400px, centered
...
- Card grids: 2â€“3 column for feature highlights
...
- **Breathing room is the design**: The generous whitespace IS the primary design element â€” it communicates confidence, quality, and that each feature deserves individual attention.
...
- Subtly rounded (6px): Buttons, form inputs, combo boxes â€” the functional interactive radius
- Comfortably rounded (8px): Standard content cards, containers
- Generously rounded (16px): Feature tabs, content panels
- Very rounded (24px): Buttons, video/image containers, tabpanels â€” the signature softness
- Highly rounded (32â€“36px): Hero CTAs, status badges, nav buttons
- Pill-shaped (9999px): Primary action buttons, tags, avatars â€” maximum friendliness
...
**Shadow Philosophy**: Expo uses shadows as gentle whispers rather than architectural statements. The primary depth mechanism is **background color contrast** â€” white cards floating on Cloud Gray â€” rather than shadow casting. When shadows appear, they're soft, diffused, and directional (downward), creating the feeling of paper hovering millimeters above a desk.

## 7. Do's and Don'ts

### Do
- Use Cloud Gray (`#f0f0f3`) as the page background and Pure White (`#ffffff`) for elevated cards â€” the two-tone light system is essential
- Keep display headlines at extreme negative letter-spacing (-1.6px to -3px at 64px) for the signature compressed look
- Use pill-shaped (9999px) radius for primary CTA buttons â€” the organic shape is core to the identity
- Reserve black (`#000000`) for headlines and primary CTAs â€” it carries maximum authority on the light canvas
- Use Slate Gray (`#60646c`) for secondary text â€” it's the precise balance between readable and receded
- Maintain enormous vertical spacing between sections (96px+) â€” the gallery pacing defines the premium feel
- Use product screenshots as the primary visual content â€” the interface stays monochrome, the products bring color
- Apply Inter at the full weight range (400â€“900) â€” weight contrast IS the hierarchy

### Don't
- Don't introduce decorative colors into the interface chrome â€” the monochromatic palette is intentional
- Don't use sharp corners (border-radius < 6px) on interactive elements â€” the pill/rounded geometry is the signature
- Don't reduce section spacing below 64px â€” the breathing room is the design
- Don't use heavy drop shadows â€” depth comes from background contrast and whisper-soft shadows
- Don't mix in additional typefaces â€” Inter handles everything from display to caption
- Don't use letter-spacing wider than -0.25px on body text â€” extreme tracking is reserved for display only
- Don't use borders heavier than 2px â€” containment is subtle, achieved through background color and gentle borders
- Don't add gradients to the interface â€” visual richness comes from content, not decoration
- Don't use saturated colors outside of semantic contexts â€” the palette is strictly grayscale + functional blue
...
| Tablet | 640â€“1024px | 2-column grids, condensed nav, medium hero text |
...
### Touch Targets
- Buttons use generous radius (24â€“36px) creating large, finger-friendly surfaces
...
- **Hero text**: 64px â†’ ~36px progressive scaling
- **Device previews**: Grid â†’ stacked/carousel
- **Cards**: Side-by-side â†’ vertical stacking
...
2. Reference specific color names and hex codes â€” "use Slate Gray (#60646c)" not "make it gray"
3. Use radius values deliberately â€” 6px for buttons, 8px for cards, 24px for images, 9999px for pills
4. Describe the "feel" alongside measurements â€” "enormous breathing room with 96px section spacing"
5. Always specify Inter and the exact weight â€” weight contrast IS the hierarchy
6. For shadows, specify "whisper shadow" or "standard elevation" from the elevation table
7. Keep the interface monochrome â€” let product content be the color
