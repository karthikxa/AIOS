...
MongoDB's website is a deep-forest-meets-terminal experience â€” a design system rooted in the darkest teal-black (`#001e2b`) that evokes both the density of a database and the depth of a forest canopy. Against this near-black canvas, a striking neon green (`#00ed64`) pulses as the brand accent â€” bright enough to feel electric, organic enough to feel alive. This isn't the cold neon of cyberpunk; it's the bioluminescent green of something growing in the dark.

The typography system is architecturally ambitious: MongoDB Value Serif for massive hero headlines (96px) creates an editorial, authoritative presence â€” serif type at database-company scale is a bold choice that says "we're not just another tech company." Euclid Circular A handles the heavy lifting of body and UI text with an unusually wide weight range (300â€“700), while Source Code Pro serves as the code and label font with distinctive uppercase treatments featuring very wide letter-spacing (1pxâ€“3px). This three-font system creates a hierarchy that spans editorial elegance â†’ geometric professionalism â†’ engineering precision.

What makes MongoDB distinctive is its dual-mode design: a dark hero/feature section world (`#001e2b` with neon green accents) and a light content world (white with teal-gray borders `#b8c4c2`). The transition between these modes creates dramatic contrast. The shadow system uses teal-tinted dark shadows (`rgba(0, 30, 43, 0.12)`) that maintain the forest-dark atmosphere even on light surfaces. Buttons use pill shapes (100pxâ€“999px radius) with MongoDB Green borders (`#00684a`), and the entire component system references the LeafyGreen design system.

**Key Characteristics:**
- Deep teal-black backgrounds (`#001e2b`) â€” forest-dark, not space-dark
- Neon MongoDB Green (`#00ed64`) as the singular brand accent â€” electric and organic
- MongoDB Value Serif for hero headlines â€” editorial authority at tech scale
- Euclid Circular A for body with weight 300 (light) as a distinctive body weight
- Source Code Pro with wide uppercase letter-spacing (1pxâ€“3px) for technical labels
- Teal-tinted shadows: `rgba(0, 30, 43, 0.12)` â€” shadows carry the forest color
...
### Primary Brand
- **Forest Black** (`#001e2b`): Primary dark background â€” the deepest teal-black
- **MongoDB Green** (`#00ed64`): Primary brand accent â€” neon green for highlights, underlines, gradients
- **Dark Green** (`#00684a`): Button borders, link text on light â€” muted green for functional use

### Interactive
- **Action Blue** (`#006cfa`): Secondary accent â€” links, interactive highlights
- **Hover Blue** (`#3860be`): All link hover states transition to this blue
- **Teal Active** (`#1eaedb`): Button hover background â€” bright teal
...
### Shadows
- **Forest Shadow** (`rgba(0, 30, 43, 0.12) 0px 26px 44px, rgba(0, 0, 0, 0.13) 0px 7px 13px`): Primary card elevation â€” teal-tinted
...
### Font Families
- **Display Serif**: `MongoDB Value Serif` â€” editorial hero headlines
- **Body / UI**: `Euclid Circular A` â€” geometric sans-serif workhorse
- **Code / Labels**: `Source Code Pro` â€” monospace with uppercase label treatments
...
| Body Light | Euclid Circular A | 16px (1.00rem) | 300 | 1.50â€“2.00 | normal | Light-weight reading text |
| Nav / UI | Euclid Circular A | 16px (1.00rem) | 500 | 1.00â€“1.88 | 0.16px | Navigation, emphasized |
| Body Bold | Euclid Circular A | 15px (0.94rem) | 700 | 1.50 | normal | Strong emphasis |
| Button | Euclid Circular A | 13.5pxâ€“16px | 500â€“700 | 1.00 | 0.135pxâ€“0.9px | CTA labels |
...
| Code Label | Source Code Pro | 14px (0.88rem) | 400â€“500 | 1.14 (tight) | 1pxâ€“2px | `text-transform: uppercase` |
| Code Micro | Source Code Pro | 9px (0.56rem) | 600 | 2.67 (relaxed) | 2.5px | `text-transform: uppercase` |

### Principles
- **Serif for authority**: MongoDB Value Serif at hero scale creates an editorial presence unusual in tech â€” it communicates that MongoDB is an institution, not a startup.
...
- **Wide-tracked monospace labels**: Source Code Pro uppercase at 1pxâ€“3px letter-spacing creates technical signposts that feel like database field labels â€” systematic, structured, classified.
- **Four-weight range**: 300 (light body) â†’ 400 (standard) â†’ 500 (UI/nav) â†’ 700 (bold CTA) â€” a wider range than most systems, enabling fine-grained hierarchy.
...
- Image containers: 30pxâ€“32px radius
...
### Border Radius Scale
- Minimal (1pxâ€“2px): Small spans, badges
...
- Image (30pxâ€“32px): Image containers
- Hero (48px): Hero cards
- Pill (100pxâ€“999px): Buttons, navigation pills
...
| Forest (Level 4) | `rgba(0,30,43,0.12) 0px 26px 44px, rgba(0,0,0,0.13) 0px 7px 13px` | Hero cards â€” teal-tinted |

**Shadow Philosophy**: MongoDB's shadow system is unique in that the primary elevation shadow uses `rgba(0, 30, 43, 0.12)` â€” a teal-tinted shadow that carries the forest-dark brand color into the depth system. This means even on white surfaces, shadows feel like they belong to the MongoDB color world rather than being generic neutral black.

## 7. Do's and Don'ts

### Do
- Use `#001e2b` (forest-black) for dark sections â€” not pure black
- Apply MongoDB Green (`#00ed64`) sparingly for maximum electric impact
- Use MongoDB Value Serif ONLY for hero/display headings â€” Euclid Circular A for everything else
- Apply Source Code Pro uppercase with wide tracking (1pxâ€“3px) for technical labels
- Use teal-tinted shadows (`rgba(0,30,43,0.12)`) for primary card elevation
- Maintain the dark/light section duality â€” dramatic contrast between modes
- Use weight 300 for body text â€” the light weight is the readable voice
- Apply pill radius (100px) to primary action buttons

### Don't
- Don't use pure black (`#000000`) for dark backgrounds â€” always use teal-black (`#001e2b`)
- Don't use MongoDB Green (`#00ed64`) on backgrounds â€” it's an accent for text, underlines, and small highlights
- Don't use standard gray shadows â€” always use teal-tinted (`rgba(0,30,43,...)`)
- Don't apply serif font to body text â€” MongoDB Value Serif is hero-only
- Don't use narrow letter-spacing on Source Code Pro labels â€” the wide tracking IS the identity
- Don't mix dark and light section treatments within the same section
- Don't use warm colors â€” the palette is strictly cool (teal, green, blue)
- Don't forget the green accent underlines â€” they're the signature decorative element
...
| Mobile | 425â€“768px | Standard mobile |
| Tablet | 768â€“1024px | 2-column grids begin |
| Desktop | 1024â€“1280px | Standard layout |
| Large Desktop | 1280â€“1440px | Expanded layout |
...
### Collapsing Strategy
- Hero: MongoDB Value Serif 96px â†’ 64px â†’ scales further
- Navigation: horizontal mega-menu â†’ hamburger
- Feature cards: multi-column â†’ stacked
...
2. MongoDB Green (#00ed64) is electric â€” use once per section for maximum impact
3. Serif headlines (MongoDB Value Serif) create the editorial authority â€” never use for body
4. Weight 300 body text creates the airy reading experience â€” don't default to 400
5. Source Code Pro uppercase with wide tracking for technical labels â€” the database voice
