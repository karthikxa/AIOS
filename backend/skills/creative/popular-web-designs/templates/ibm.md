...
IBM's website is the digital embodiment of enterprise authority built on the Carbon Design System â€” a design language so methodically structured it reads like an engineering specification rendered as a webpage. The page operates on a stark duality: a bright white (`#ffffff`) canvas with near-black (`#161616`) text, punctuated by a single, unwavering accent â€” IBM Blue 60 (`#0f62fe`). This isn't playful tech-startup minimalism; it's corporate precision distilled into pixels. Every element exists within Carbon's rigid 2x grid, every color maps to a semantic token, every spacing value snaps to the 8px base unit.

The IBM Plex type family is the system's backbone. IBM Plex Sans at light weight (300) for display headlines creates an unexpectedly airy, almost delicate quality at large sizes â€” a deliberate counterpoint to IBM's corporate gravity. At body sizes, regular weight (400) with 0.16px letter-spacing on 14px captions introduces the meticulous micro-tracking that makes Carbon text feel engineered rather than designed. IBM Plex Mono serves code, data, and technical labels, completing the family trinity alongside the rarely-surfaced IBM Plex Serif.

What defines IBM's visual identity beyond monochrome-plus-blue is the reliance on Carbon's component token system. Every interactive state maps to a CSS custom property prefixed with `--cds-` (Carbon Design System). Buttons don't have hardcoded colors; they reference `--cds-button-primary`, `--cds-button-primary-hover`, `--cds-button-primary-active`. This tokenized architecture means the entire visual layer is a thin skin over a deeply systematic foundation â€” the design equivalent of a well-typed API.

**Key Characteristics:**
- IBM Plex Sans at weight 300 (Light) for display â€” corporate gravitas through typographic restraint
- IBM Plex Mono for code and technical content with consistent 0.16px letter-spacing at small sizes
- Single accent color: IBM Blue 60 (`#0f62fe`) â€” every interactive element, every CTA, every link
- Carbon token system (`--cds-*`) driving all semantic colors, enabling theme-switching at the variable level
- 8px spacing grid with strict adherence â€” no arbitrary values, everything aligns
- Flat, borderless cards on `#f4f4f4` Gray 10 surface â€” depth through background-color layering, not shadows
- Bottom-border inputs (not boxed) â€” the signature Carbon form pattern
- 0px border-radius on primary buttons â€” unapologetically rectangular, no softening
...
### Interactive
- **Blue 60** (`#0f62fe`): Primary interactive â€” buttons, links, focus. `--cds-link-primary`, `--cds-button-primary`.
...
- **Focus Blue** (`#0f62fe`): `--cds-focus` â€” 2px inset border on focused elements.
- **Focus Inset** (`#ffffff`): `--cds-focus-inset` â€” white inner ring for focus on dark backgrounds.
...
- **Icon Font**: `ibm_icons` â€” proprietary icon glyphs at 20px
...
### Principles
- **Light weight at display sizes**: Carbon's expressive type set uses weight 300 (Light) at 42px+. This creates a distinctive tension â€” the content speaks with corporate authority while the letterforms whisper with typographic lightness.
- **Micro-tracking at small sizes**: 0.16px letter-spacing at 14px and 0.32px at 12px. These seemingly negligible values are Carbon's secret weapon for readability at compact sizes â€” they open up the tight IBM Plex letterforms just enough.
...
**Primary Button (Blue)**
- Background: `#0f62fe` (Blue 60) â†’ `--cds-button-primary`
- Text: `#ffffff` (White)
- Padding: 14px 63px 14px 15px (asymmetric â€” room for trailing icon)
- Border: 1px solid transparent
- Border-radius: 0px (sharp rectangle â€” the Carbon signature)
- Height: 48px (default), 40px (compact), 64px (expressive)
- Hover: `#0353e9` (Blue 60 Hover) â†’ `--cds-button-primary-hover`
- Active: `#002d9c` (Blue 80) â†’ `--cds-button-primary-active`
...
- Border: none (flat design â€” no border or shadow on most cards)
...
- Separation: background-color layering (white â†’ gray 10 â†’ white) rather than shadows

### Inputs & Forms
- Background: `#f4f4f4` (Gray 10) â€” `--cds-field`
- Text: `#161616` (Gray 100)
- Padding: 0px 16px (horizontal only)
- Height: 40px (default), 48px (large)
- Border: none on sides/top â€” `2px solid transparent` bottom
- Bottom-border active: `2px solid #161616` (Gray 100)
- Focus: `2px solid #0f62fe` (Blue 60) bottom-border â€” `--cds-focus`
...
- Border-radius: 0px (top) â€” inputs are sharp-cornered

### Navigation
- Background: `#161616` (Gray 100) â€” full-width dark masthead
- Height: 48px
...
- **Functional density**: Carbon favors productive density over expansive whitespace. Sections are tightly packed compared to consumer design systems â€” this reflects IBM's enterprise DNA.
- **Background-color zoning**: Instead of massive padding between sections, IBM uses alternating background colors (white â†’ gray 10 â†’ white) to create visual separation with minimal vertical space.
- **Consistent 48px rhythm**: Major section transitions use 48px vertical spacing. Hero sections may use 80pxâ€“96px.

### Border Radius Scale
- **0px**: Primary buttons, inputs, tiles, cards â€” the dominant treatment. Carbon is fundamentally rectangular.
- **2px**: Occasionally on small interactive elements (tags)
- **24px**: Tags/labels (pill shape â€” the sole rounded exception)
...
- Apply Blue 60 (`#0f62fe`) as the sole accent â€” one blue to rule them all

### Don't
- Don't round button corners â€” 0px radius is the Carbon identity
- Don't use shadows on cards or tiles â€” flatness is the point
- Don't introduce additional accent colors â€” IBM's system is monochromatic + blue
- Don't use weight 700 (Bold) â€” the scale stops at 600 (Semibold)
- Don't add letter-spacing to display-size text â€” tracking is only for 14px and below
- Don't box inputs with full borders â€” Carbon inputs use bottom-border only
- Don't use gradient backgrounds â€” IBM's surfaces are flat, solid colors
- Don't deviate from the 8px spacing grid â€” every value should be divisible by 8 (with 2px and 4px for micro-adjustments)
...
### Collapsing Strategy
- Hero: 60px display â†’ 42px â†’ 32px heading as viewport narrows
- Navigation: full horizontal masthead â†’ hamburger with slide-out panel
- Grid: 4-column â†’ 2-column â†’ single column
- Tiles/cards: horizontal grid â†’ vertical stack
- Images: maintain aspect ratio, max-width 100%
- Footer: multi-column link groups â†’ stacked single column
- Section padding: 48px â†’ 32px â†’ 16px
...
### Iteration Guide
1. Always use 0px border-radius on buttons, inputs, and cards â€” this is non-negotiable in Carbon
2. Letter-spacing only at small sizes: 0.16px at 14px, 0.32px at 12px â€” never on display text
3. Three weights: 300 (display), 400 (body), 600 (emphasis) â€” no bold
4. Blue 60 is the only accent color â€” do not introduce secondary accent hues
5. Depth comes from background-color layering (white â†’ #f4f4f4 â†’ #e0e0e0), not shadows
