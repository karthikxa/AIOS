...
HashiCorp's website is enterprise infrastructure made tangible â€” a design system that must communicate the complexity of cloud infrastructure management while remaining approachable. The visual language splits between two modes: a clean white light-mode for informational sections and a dramatic dark-mode (`#15181e`, `#0d0e12`) for hero areas and product showcases, creating a day/night duality that mirrors the "build in light, deploy in dark" developer workflow.

The typography is anchored by a custom brand font (HashiCorp Sans, loaded as `__hashicorpSans_96f0ca`) that carries substantial weight â€” literally. Headings use 600â€“700 weights with tight line-heights (1.17â€“1.19), creating dense, authoritative text blocks that communicate enterprise confidence. The hero headline at 82px weight 600 with OpenType `"kern"` enabled is not decorative â€” it's infrastructure-grade typography.

What distinguishes HashiCorp is its multi-product color system. Each product in the portfolio has its own brand color â€” Terraform purple (`#7b42bc`), Vault yellow (`#ffcf25`), Waypoint teal (`#14c6cb`), Vagrant blue (`#1868f2`) â€” and these colors appear throughout as accent tokens via a CSS custom property system (`--mds-color-*`). This creates a design system within a design system: the parent brand is black-and-white with blue accents, while each child product injects its own chromatic identity.

The component system uses the `mds` (Markdown Design System) prefix, indicating a systematic, token-driven approach where colors, spacing, and states are all managed through CSS variables. Shadows are remarkably subtle â€” dual-layer micro-shadows using `rgba(97, 104, 117, 0.05)` that are nearly invisible but provide just enough depth to separate interactive surfaces from the background.

**Key Characteristics:**
- Dual-mode: clean white sections + dramatic dark (`#15181e`) hero/product areas
- Custom HashiCorp Sans font with 600â€“700 weights and `"kern"` feature
...
- Micro-shadows: dual-layer at 0.05 opacity â€” depth through whisper, not shout
- Token-driven `mds` component system with semantic variable names
- Tight border radius: 2pxâ€“8px, nothing pill-shaped or circular
...
- **Cool Gray** (`#b2b6bd`): Border accents (at 0.1â€“0.4 opacity)
...
- **Focus Outline**: `3px solid var(--mds-color-focus-action-external)` â€” systematic focus ring
...
| Sub-heading | HashiCorp Sans | 34px (2.13rem) | 600â€“700 | 1.18 (tight) | normal | Feature blocks |
...
| Body Emphasis | HashiCorp Sans | 17px (1.06rem) | 600â€“700 | 1.18â€“1.35 | normal | Bold body text |
| Body Large | system-ui | 20px (1.25rem) | 400â€“600 | 1.50 | normal | Hero descriptions |
| Body | system-ui | 16px (1.00rem) | 400â€“500 | 1.63â€“1.69 (relaxed) | normal | Standard body text |
| Nav Link | system-ui | 15px (0.94rem) | 500 | 1.60 (relaxed) | normal | Navigation items |
| Small Body | system-ui | 14px (0.88rem) | 400â€“500 | 1.29â€“1.71 | normal | Secondary content |
| Caption | system-ui | 13px (0.81rem) | 400â€“500 | 1.23â€“1.69 | normal | Metadata, footer links |
...
- **Kern always on**: All HashiCorp Sans text enables OpenType `"kern"` â€” letterfitting is non-negotiable.
- **Tight headings**: Every heading uses 1.17â€“1.21 line-height, creating dense, stacked text blocks that feel infrastructural â€” solid, load-bearing.
- **Relaxed body**: Body text uses 1.50â€“1.69 line-height (notably generous), creating comfortable reading rhythm beneath the dense headings.
- **Uppercase labels as wayfinding**: 13px uppercase with 1.3px letter-spacing serves as the systematic category/section marker â€” always HashiCorp Sans weight 600.
...
### Links
- **Action Blue on Light**: `#2264d6`, hover â†’ blue-600 variable, underline on hover
- **Action Blue on Dark**: `#1060ff` or `#2b89ff`, underline on hover
- **White on Dark**: `#ffffff`, transparent underline â†’ visible underline on hover
- **Neutral on Light**: `#3b3d45`, transparent underline â†’ visible underline on hover
...
| Mobile | 375â€“480px | Standard mobile |
| Small Tablet | 480â€“600px | Minor adjustments |
| Tablet | 600â€“768px | 2-column grids begin |
| Small Desktop | 768â€“992px | Full nav visible |
| Desktop | 992â€“1120px | Standard layout |
| Large Desktop | 1120â€“1440px | Max-width content |
| Ultra-wide | >1440px | Centered, generous margins |

### Whitespace Philosophy
- **Enterprise breathing room**: Generous vertical spacing between sections (48pxâ€“80px+) communicates stability and seriousness.
...
| Focus (Level 2) | `3px solid var(--mds-color-focus-action-external)` outline | Focus rings â€” color-matched to context |

**Shadow Philosophy**: HashiCorp uses arguably the subtlest shadow system in modern web design. The dual-layer shadows at 5% opacity are nearly invisible â€” they exist not to create visual depth but to signal interactivity. If you can see the shadow, it's too strong. This restraint communicates the enterprise value of stability â€” nothing floats, nothing is uncertain.
...
- Maintain the tight-heading / relaxed-body rhythm (1.17â€“1.21 vs 1.50â€“1.69 line-heights)
...
- Don't increase shadow opacity above 0.1 â€” the whisper level is intentional
- Don't use pill-shaped buttons (>8px radius) â€” the sharp, minimal radius is structural
- Don't skip the `"kern"` feature on headings â€” the font requires it
- Don't use HashiCorp Sans for small body text â€” it's designed for 17px+ heading use
- Don't mix product colors in the same component â€” each product has one color
- Don't use pure black (`#000000`) for dark backgrounds â€” use `#15181e` or `#0d0e12`
- Don't forget the asymmetric button padding â€” 9px 9px 9px 15px is intentional
...
| Tablet | 768â€“992px | 2-column grids, nav begins expanding |
| Desktop | 992â€“1150px | Full layout, mega-menu nav |
| Large | >1150px | Max-width centered, generous margins |

### Collapsing Strategy
- Hero: 82px â†’ 52px â†’ 42px heading sizes
- Navigation: mega-menu â†’ hamburger
- Product cards: 3-column â†’ 2-column â†’ stacked
- Dark sections maintain full-width but compress padding
- Buttons: inline â†’ full-width stacked on mobile
...
- "Create a product-specific CTA button: Terraform â†’ #7b42bc background, Vault â†’ #ffcf25 with dark text, Waypoint â†’ #14c6cb. All: 5px radius, 500 weight text, 16px system-ui."
...
3. Shadows are at whisper level (0.05 opacity) â€” if visible, reduce
4. Product colors are sacred â€” each product owns exactly one color
5. Focus rings are always 3px solid, color-matched to product context
6. Uppercase labels are the systematic wayfinding pattern â€” 13px, 600, 1.3px tracking
