...
> **See also:** architecture.md Â· effects.md Â· scenes.md Â· shaders.md Â· optimization.md Â· troubleshooting.md
...
|---------|---------|----------|
| `rms` | `sqrt(mean(chunkÂ²))` | Overall loudness/energy |
| `sub`..`hi` | `sqrt(mean(band_magnitudesÂ²))` | Per-band energy |
...
| `bdecay` | Exponential decay from beats | Smooth beat pulse (0â†’1â†’0) |

**Band ratios are critical** â€” they decouple spectral shape from volume, so a quiet bass section and a loud bass section both read as "bassy" rather than just "loud" vs "quiet".
...
`bdecay` gives smooth 0â†’1â†’0 pulse per beat, decaying over ~0.5s. Use for flash/glitch/mirror triggers.
