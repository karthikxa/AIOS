...
> **See also:** composition.md Â· effects.md Â· scenes.md Â· shaders.md Â· inputs.md Â· optimization.md Â· troubleshooting.md
...
```python
RESOLUTION_PRESETS = {
    "landscape":  (1920, 1080),  # 16:9 â€” YouTube, default
    "portrait":   (1080, 1920),  # 9:16 â€” TikTok, Reels, Stories
    "square":     (1080, 1080),  # 1:1  â€” Instagram feed
    "ultrawide":  (2560, 1080),  # 21:9 â€” cinematic
    "landscape4k":(3840, 2160),  # 16:9 â€” 4K
    "portrait4k": (2160, 3840),  # 9:16 â€” 4K portrait
}
...
- Fewer columns (90 at `lg` vs 160) â€” lines must be shorter or wrap
- Many more rows (80 at `lg` vs 45) â€” vertical stacking is natural
...
**Grid sizing for text in portrait**: Use `lg` (20px) for 2-3 word lines. Max comfortable line length is ~25-30 chars. For longer quotes, break aggressively into many short lines stacked vertically â€” portrait has vertical space to spare. `xl` (24px) works for single words or very short phrases.
...
for pal in [PAL_DEFAULT, PAL_DENSE, PAL_BLOCKS, PAL_RUNE, PAL_KATA,
            ...
            PAL_CIRCUIT, PAL_ARROWS, PAL_ZED]:  # ... all palettes used in project
...
```python
PAL_DOTS     = " â‹…âˆ˜âˆ™â—â—‰â—Žâ—†âœ¦â˜…"                   # dot size progression
PAL_BRAILLE  = " â â ‚â ƒâ „â …â †â ‡â ˆâ ‰â Šâ ‹â Œâ â Žâ â â ‘â ’â “â ”â •â –â —â ˜â ™â šâ ›â œâ â žâ Ÿâ ¿"  # braille patterns
PAL_STARS    = " Â·âœ§âœ¦âœ©âœ¨â˜…âœ¶âœ³âœ¸"               # star progression
PAL_HALFFILL = " â—”â—‘â—•â—â—’â—“â—–â——â—™"               # directional half-fill progression
PAL_HATCH    = " â–£â–¤â–¥â–¦â–§â–¨â–©"                     # crosshatch density ramp
```

#### Project-Specific (examples -- invent new ones per project)
```python
PAL_ZED   = " .\u00b7~=\u2248\u221e\u26a1\u263f\u2726\u2605\u2295\u25ca\u25c6\u25b2\u25bc\u25cf\u25a0"   # mythology/tech blend
...
HSV hue is perceptually non-uniform: green occupies far more visual range than blue. OKLAB / OKLCH provide perceptually even color steps â€” hue increments of 0.1 look equally different regardless of starting hue. Use OKLAB for:
...
def lerp_oklab(color_a, color_b, t_array):
    ...
    t_array: float32 array [0,1] â€” interpolation parameter per pixel.
...
Every scene function: `(r, f, t, S) -> canvas_uint8` â€” where `r` = Renderer, `f` = features dict, `t` = time float, `S` = persistent state dict
...
Effects that need state across frames (particles, rain columns) use the `S` dict parameter (which is `r.S` â€” same object, but passed explicitly for clarity):
