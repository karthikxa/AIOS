...
Built for [Zed Agent](https://github.com/NousResearch/zed-agent). Usable in any coding agent. Canonical source lives here; synced to [`NousResearch/zed-agent/skills/creative/ascii-video`](https://github.com/NousResearch/zed-agent/tree/main/skills/creative/ascii-video) via PR.
...
|--------|----------|------|
| Density ramps | ` .:-=+#@â–ˆ` | Classic ASCII art gradient |
| Block elements | ` â–‘â–’â–“â–ˆâ–„â–€â–â–Œ` | Chunky, digital |
| Braille | ` â â ‚â ƒ...â ¿` | Fine-grained pointillism |
| Dots | ` â‹…âˆ˜âˆ™â—â—‰â—Ž` | Smooth, organic |
| Stars | ` Â·âœ§âœ¦âœ©âœ¨â˜…âœ¶` | Sparkle, celestial |
| Half-fills | ` â—”â—‘â—•â—â—’â—“â—–â——â—™` | Directional fill progression |
| Crosshatch | ` â–£â–¤â–¥â–¦â–§â–¨â–©` | Hatched density ramp |
| Math | ` Â·âˆ˜âˆ™â€¢Â°Â±Ã—Ã·â‰ˆâ‰ â‰¡âˆžâˆ«âˆ‘Î©` | Scientific, abstract |
| Box drawing | ` â”€â”‚â”Œâ”â””â”˜â”œâ”¤â”¬â”´â”¼` | Structural, circuit-like |
| Katakana | ` Â·ï½¦ï½§ï½¨ï½©ï½ªï½«ï½¬ï½­...` | Matrix rain |
| Greek | ` Î±Î²Î³Î´ÎµÎ¶Î·Î¸...Ï‰` | Classical, academic |
| Runes | ` áš áš¢áš¦áš±áš·á›á›‡á›’á›–á›šá›žá›Ÿ` | Mystical, ancient |
| Alchemical | ` â˜‰â˜½â™€â™‚â™ƒâ™„â™…â™†â™‡` | Esoteric |
| Arrows | ` â†â†‘â†’â†“â†”â†•â†–â†—â†˜â†™` | Directional, kinetic |
| Music | ` â™ªâ™«â™¬â™©â™­â™®â™¯â—‹â—` | Musical |
| Project-specific | ` .Â·~=â‰ˆâˆžâš¡â˜¿âœ¦â˜…âŠ•â—Šâ—†â–²â–¼â—â– ` | Themed per project |
...
Full OKLAB/OKLCH color system: sRGBâ†”linearâ†”OKLAB conversion pipeline, perceptually uniform gradient interpolation, and color harmony generation (complementary, triadic, analogous, split-complementary, tetradic).
...
| fBM | Fractal Brownian Motion â€” octaved noise for clouds, terrain, smoke |
| Domain warp | Inigo Quilez technique â€” fBM-driven coordinate distortion for flowing organic forms |
...
| Strange attractors | Clifford, De Jong, Bedhead â€” iterated point systems binned to density fields |
...
UV-space transforms applied before effect evaluation: rotate, scale, skew, tile (with mirror seaming), polar, inverse-polar, twist (rotation increasing with distance), fisheye, wave displacement, MÃ¶bius conformal transformation. `make_tgrid()` wraps transformed coordinates into a grid object.
...
10 easing functions (linear, quad, cubic, expo, elastic, bounce â€” in/out/in-out). Keyframe interpolation with eased transitions. Value field morphing (smooth crossfade between fields). Value field sequencing (cycle through fields with crossfade). Temporal noise (3D noise evolving smoothly in-place).
...
**Feedback buffer.** Temporal recursion â€” each frame blends with a transformed version of the previous frame. 7 spatial transforms: zoom, shrink, rotate CW/CCW, shift up/down, mirror. Optional per-frame hue shift for rainbow trails. Configurable decay, blend mode, and opacity per scene.
...
â—† Python 3.10+
â—† NumPy, Pillow, SciPy (audio modes)
â—† ffmpeg on PATH
â—† A monospace font (Menlo, Courier, Monaco, auto-detected)
â—† Optional: OpenCV, ElevenLabs API key (TTS mode)

## File structure

```
â”œâ”€â”€ SKILL.md                 # Modes, workflow, creative direction
â”œâ”€â”€ README.md                # This file
â””â”€â”€ references/
    â”œâ”€â”€ architecture.md      # Grid system, fonts, palettes, color, _render_vf()
    â”œâ”€â”€ effects.md           # Value fields, hue fields, backgrounds, particles
    â”œâ”€â”€ shaders.md           # 38 shaders, ShaderChain, tint presets, transitions
    â”œâ”€â”€ composition.md       # Blend modes, multi-grid, tonemap, FeedbackBuffer
    â”œâ”€â”€ scenes.md            # Scene protocol, SCENES table, render_clip(), examples
    â”œâ”€â”€ design-patterns.md   # Layer hierarchy, directional arcs, scene concepts
    â”œâ”€â”€ inputs.md            # Audio analysis, video sampling, text, TTS
    â”œâ”€â”€ optimization.md      # Hardware detection, vectorized patterns, parallelism
    â””â”€â”€ troubleshooting.md   # Broadcasting traps, blend pitfalls, diagnostics
```

## Projects built with this

âœ¦ 85-second highlight reel. 15 scenes (14Ã—5s + 15s crescendo finale), randomized order, directional parameter arcs, layer hierarchy composition. Showcases the full effect vocabulary: fBM, voronoi fragmentation, reaction-diffusion, cellular automata, dual counter-rotating spirals, wave collision, domain warping, tunnel descent, kaleidoscope symmetry, boid flocking, fire simulation, glitch corruption, and a 7-layer crescendo buildup.

âœ¦ Audio-reactive music visualizer. 3.5 min, 8 sections with distinct effects, beat-triggered particles and glitch, cycling palettes.

âœ¦ TTS narrated testimonial video. 23 quotes, per-quote ElevenLabs voices, background music at 15% wide stereo, per-clip re-rendering for iterative editing.
