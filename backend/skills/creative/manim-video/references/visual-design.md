...
1. **Geometry Before Algebra** â€” Show the shape first, the equation second.
2. **Opacity Layering** â€” PRIMARY=1.0, CONTEXT=0.4, GRID=0.15. Direct attention through brightness.
3. **One New Idea Per Scene** â€” Each scene introduces exactly one concept.
4. **Spatial Consistency** â€” Same concept occupies the same screen region throughout.
5. **Color = Meaning** â€” Assign colors to concepts, not mobjects. If velocity is blue, it stays blue.
6. **Progressive Disclosure** â€” Show simplest version first, add complexity incrementally.
7. **Transform, Don't Replace** â€” Use Transform/ReplacementTransform to show connections.
8. **Breathing Room** â€” `self.wait(1.5)` minimum after showing something new.
9. **Visual Weight Balance** â€” Don't cluster everything on one side.
10. **Consistent Motion Vocabulary** â€” Pick a small set of animation types and reuse them.
11. **Dark Background, Light Content** â€” #1C1C1C to #2D2B55 backgrounds maximize contrast.
12. **Intentional Empty Space** â€” Leave at least 15% of the frame empty.
...
Monospace fonts have fixed character widths â€” zero kerning issues by design.
...
| Math | Use `MathTex` (renders via LaTeX, not Pango) | â€” |
...
# Math â€” always use MathTex, not Text
equation = MathTex(r"\nabla L = \frac{\partial L}{\partial w}")
```

### When Proportional Fonts Are Acceptable

Large title text (font_size >= 48) with short strings (1-3 words) can use proportional fonts without visible kerning issues. For anything else â€” labels, descriptions, multi-word text, small sizes â€” use monospace.
...
`"Menlo"` is the safest default â€” pre-installed on macOS, and Linux systems fall back to DejaVu Sans Mono.
...
MarkupText('<span letter_spacing="6000">ZED</span>', font_size=18, font="Menlo")
