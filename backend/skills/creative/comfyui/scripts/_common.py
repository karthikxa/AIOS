"""
_common.py â€” Shared logic for ComfyUI skill scripts.
...
DEFAULT_HTTP_TIMEOUT = 60          # seconds â€” single-attempt request timeout
DEFAULT_RETRIES = 3                # total attempts including the first
RETRY_BASE_DELAY = 1.0             # seconds â€” exponential backoff base
RETRY_MAX_DELAY = 30.0             # seconds â€” cap on backoff
RETRY_STATUS_CODES = {408, 429, 500, 502, 503, 504, 522, 524}

# Streaming download chunk size (bytes)
DOWNLOAD_CHUNK_SIZE = 1 << 16  # 64 KiB

# Heuristic: workflows with these node types tend to be slow â†’ larger default timeout
SLOW_OUTPUT_NODES = {
...
# ---------------------------------------------------------------------------
# Output node catalog (extensible â€” community packs add their own)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Folder aliases â€” handle ComfyUI's gradual folder renames
# ---------------------------------------------------------------------------
...
# migrated to the new naming (unet â†’ diffusion_models, clip â†’ text_encoders).
...
MODEL_LOADERS: dict[str, list[tuple[str, str]]] = {
    ...
    # `provider` are enums (not file paths), so they're intentionally omitted â€”
...
# Order matters â€” first match wins for naming. Use _meta.title for disambiguation.
# ---------------------------------------------------------------------------
PARAM_PATTERNS: list[tuple[str, str, str]] = [
    ...
    # NB: SamplerCustomAdvanced takes a NOISE input (from RandomNoise) â€” no seed field directly.
...
def resolve_api_key(explicit: str | None) -> str | None:
    """Look up API key from CLI flag â†’ env var. Strips whitespace and quotes."""
...
def http_request(
    ...
    *,
    ...
    headers.setdefault("User-Agent", "zed-comfyui-skill/5.0")

    if files or form is not None:
        # Multipart upload â€” needs `requests`. The stdlib fallback lacks
...
if HAS_REQUESTS:
    class _StripSensitiveOnRedirectSession(requests.Session):
        ...
        `_SENSITIVE_HEADERS` when the destination is a different host â€”
...
def unwrap_workflow(payload: Any) -> dict:
    ...
    if isinstance(payload, dict) and "nodes" in payload and "links" in payload:
        raise ValueError(
            ...
            "Re-export from ComfyUI using 'Workflow â†’ Export (API)' (newer UI) "
