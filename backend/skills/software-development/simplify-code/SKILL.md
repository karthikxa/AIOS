...
version: 1.0.0
author: Zed Agent (inspired by Claude Code /simplify)
license: MIT
...
# Simplify Code â€” Parallel Review & Cleanup
...
deeply searches the codebase for a single class of problem â€” reuse, quality,
efficiency â€” without diluting its attention across all three. They run
...
Optional modifiers the user may add â€” honor them:

- **Focus:** "simplify focus on efficiency" â†’ run only the efficiency reviewer
  (or weight the aggregation toward it). Recognized focuses: `reuse`,
  `quality`, `efficiency`.
- **Dry run:** "simplify but don't change anything" / "just report" â†’ run the
...
- **Scope:** "simplify the last commit" / "simplify staged" / "simplify
  src/foo.py" â†’ narrow the diff source accordingly (see Phase 1).

Do NOT auto-run this after every edit. It costs three subagents' worth of
tokens â€” invoke it only when the user explicitly asks.

## The Process

### Phase 1 â€” Identify the changes
...
changed code, say so and stop â€” there's nothing to simplify.
...
### Phase 2 â€” Launch three reviewers in parallel

Use `delegate_task` **batch mode** â€” pass all three tasks in one `tasks`
...
Give **every** reviewer the **complete diff** (not fragments â€” cross-file
...
- **Apply Chesterton's Fence:** before flagging anything for removal, run
  ...
  the original purpose, mark it `confidence: low` â€” don't guess.
- Report findings as structured output with confidence and risk:
  ```
  file:line â†’ problem â†’ suggested fix | confidence: high/medium/low | risk: SAFE/CAREFUL/RISKY
  ```
  ...
  - **RISKY** = may change behavior or breaks public contracts (N+1
    ...
    human review â€” do NOT auto-apply.
...
> code (raw strings where a constant/enum/registry already exists â€” check the
...
> propagation gaps â€” these hide bugs and should at minimum log before
> swallowing). For each, give the concrete fix and why it's faster or safer.

### Phase 3 â€” Aggregate and apply
...
2. **Discard false positives** â€” you have the most context; you don't have to
...
4. **Apply in risk-tier order:**
   ...
   - **RISKY last** (flag for review â€” do NOT auto-apply): N+1 restructuring,
...
- **Give the WHOLE diff to each reviewer.** Splitting the diff across reviewers
  defeats the design â€” cross-file duplication and N+1s only show up with the
  full picture.
...
- **Apply â‰  rewrite.** This is cleanup of the user's recent changes, not a
...
- **Respect project conventions.** If the repo has AGENTS.md / CLAUDE.md /
  ZED.md or a linter config, fold those rules into the reviewer prompts so
  suggestions match house style instead of fighting it.
- **Large diffs blow context.** If the diff is huge, scope it down before
  delegating â€” three subagents each carrying a 5000-line diff is expensive and
  may truncate.
- **Over-trusting dead code tools.** `knip`, `ts-prune`, and `depcheck` flag
  ...
  grep for the symbol name before removing â€” a clean tool report is not proof.
- **Renaming without checking public contracts.** Export names, API route
  paths, DB column names, and config keys are contracts â€” even if the name is
...
- **Removing "unnecessary" error handling.** An empty catch block or ignored
  error might be intentional â€” the error is expected and benign in that
