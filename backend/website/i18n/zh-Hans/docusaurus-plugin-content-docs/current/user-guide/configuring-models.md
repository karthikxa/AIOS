...
[Nous Portal](/user-guide/features/tool-gateway) åœ¨å•ä¸€è®¢é˜…ä¸‹æä¾› 300+ ä¸ªæ¨¡åž‹ã€‚å…¨æ–°å®‰è£…åŽï¼Œè¿è¡Œ `zed setup --portal` å³å¯ç™»å½•å¹¶ä¸€é”®å°† Nous è®¾ä¸ºæä¾›å•†ã€‚ä½¿ç”¨ `zed portal info` æŸ¥çœ‹å½“å‰é…ç½®ã€‚
:::
...
- **å³åˆ—** â€” æ‰€é€‰æä¾›å•†çš„ç²¾é€‰æ¨¡åž‹åˆ—è¡¨ã€‚è¿™äº›æ˜¯ Zed é’ˆå¯¹è¯¥æä¾›å•†æŽ¨èçš„ agentic æ¨¡åž‹ï¼Œè€ŒéžåŽŸå§‹çš„ `/models` æŽ¥å£è¿”å›žç»“æžœï¼ˆOpenRouter çš„åŽŸå§‹åˆ—è¡¨åŒ…å« 400+ ä¸ªæ¨¡åž‹ï¼Œæ¶µç›– TTSã€å›¾åƒç”Ÿæˆå™¨å’Œé‡æŽ’åºå™¨ï¼‰ã€‚

åœ¨è¿‡æ»¤æ¡†ä¸­è¾“å…¥æä¾›å•†åç§°ã€slug æˆ–æ¨¡åž‹ ID è¿›è¡Œç­›é€‰ã€‚

é€‰æ‹©æ¨¡åž‹åŽç‚¹å‡» **Switch**ï¼ŒZed ä¼šå°†å…¶å†™å…¥ `~/.zed/config.yaml` çš„ `model` éƒ¨åˆ†ã€‚**æ­¤æ“ä½œä»…å¯¹æ–°ä¼šè¯ç”Ÿæ•ˆ** â€” å·²æ‰“å¼€çš„èŠå¤©æ ‡ç­¾é¡µå°†ç»§ç»­ä½¿ç”¨å¯åŠ¨æ—¶çš„æ¨¡åž‹ã€‚å¦‚éœ€åœ¨å½“å‰èŠå¤©ä¸­çƒ­åˆ‡æ¢ï¼Œè¯·åœ¨èŠå¤©å†…ä½¿ç”¨ `/model` æ–œæ å‘½ä»¤ã€‚
...
æ¯ä¸ªè¾…åŠ©ä»»åŠ¡é»˜è®¤ä¸º `auto`ï¼Œå³ Zed å¯¹è¯¥ä»»åŠ¡ä¹Ÿä½¿ç”¨ä¸»æ¨¡åž‹ã€‚å½“æŸä¸ªè¾¹ç¼˜ä»»åŠ¡éœ€è¦æ›´ä¾¿å®œæˆ–æ›´å¿«çš„æ¨¡åž‹æ—¶ï¼Œå¯å•ç‹¬è¦†ç›–è¯¥æ§½ä½ã€‚
...
| **Skills Hubï¼ˆæŠ€èƒ½ä¸­å¿ƒï¼‰** | `zed skills search` ä½¿ç”¨æ­¤æ§½ä½ã€‚é€šå¸¸ä¿æŒ `auto` å³å¯ã€‚ |
...
é€šè¿‡ä»ªè¡¨æ¿ä¿å­˜æ—¶ï¼ŒZed å†™å…¥ `~/.zed/config.yaml`ï¼š
...
`provider: auto` åŠ  `model: ''` è¡¨ç¤º Zed å¯¹è¯¥ä»»åŠ¡ä½¿ç”¨ä¸»æ¨¡åž‹ã€‚

## ä½•æ—¶ç”Ÿæ•ˆï¼Ÿ

- **CLI**ï¼ˆ`zed chat`ï¼‰ï¼šä¸‹æ¬¡æ‰§è¡Œ `zed chat` æ—¶ç”Ÿæ•ˆã€‚
- **Gateway**ï¼ˆTelegramã€Discordã€Slack ç­‰ï¼‰ï¼šä¸‹ä¸€ä¸ª*æ–°*ä¼šè¯ç”Ÿæ•ˆã€‚çŽ°æœ‰ä¼šè¯ä¿æŒåŽŸæœ‰æ¨¡åž‹ã€‚å¦‚éœ€å¼ºåˆ¶æ‰€æœ‰ä¼šè¯ä½¿ç”¨æ–°é…ç½®ï¼Œé‡å¯ gatewayï¼ˆ`zed gateway restart`ï¼‰ã€‚
...
Zed ä»…åˆ—å‡ºå…·æœ‰æœ‰æ•ˆå‡­æ®çš„æä¾›å•†ã€‚æ£€æŸ¥ä¾§è¾¹æ ä¸­çš„ **Keys** â€” åº”å­˜åœ¨ä»¥ä¸‹ä¹‹ä¸€ï¼šAPI keyã€æˆåŠŸçš„ OAuth æˆ–è‡ªå®šä¹‰ç«¯ç‚¹ URLã€‚è‹¥æ‰€éœ€æä¾›å•†ä¸åœ¨åˆ—è¡¨ä¸­ï¼Œè¿è¡Œ `zed setup` è¿›è¡Œé…ç½®ï¼Œæˆ–å‰å¾€ **Keys** æ·»åŠ çŽ¯å¢ƒå˜é‡ã€‚
...
### æˆ‘é€‰æ‹©äº†æ¨¡åž‹ï¼Œä½† Zed åˆ‡æ¢äº†æä¾›å•†
...
```bash
zed model            # äº¤äº’å¼æä¾›å•† + æ¨¡åž‹é€‰æ‹©å™¨ï¼ˆåˆ‡æ¢é»˜è®¤å€¼çš„æ ‡å‡†æ–¹å¼ï¼‰
```

`zed model` å¼•å¯¼ä½ é€‰æ‹©æä¾›å•†ã€å®Œæˆè®¤è¯ï¼ˆOAuth æµç¨‹ä¼šæ‰“å¼€æµè§ˆå™¨ï¼›API key æä¾›å•†ä¼šæç¤ºè¾“å…¥å¯†é’¥ï¼‰ï¼Œç„¶åŽä»Žè¯¥æä¾›å•†çš„ç²¾é€‰ç›®å½•ä¸­é€‰æ‹©å…·ä½“æ¨¡åž‹ã€‚é€‰æ‹©ç»“æžœå†™å…¥ `~/.zed/config.yaml` çš„ `model.provider` å’Œ `model.model` å­—æ®µã€‚

å¦‚éœ€åœ¨ä¸å¯åŠ¨é€‰æ‹©å™¨çš„æƒ…å†µä¸‹åˆ—å‡ºæä¾›å•†/æ¨¡åž‹ï¼Œè¯·ä½¿ç”¨ä»ªè¡¨æ¿æˆ–ä¸‹æ–¹çš„ REST ç«¯ç‚¹ã€‚æŸ¥çœ‹ CLI å½“å‰å®žé™…ä½¿ç”¨çš„é…ç½®ï¼š`zed config get model` å’Œ `zed status`ã€‚

### ç›´æŽ¥ç¼–è¾‘é…ç½®æ–‡ä»¶

ç¼–è¾‘ `~/.zed/config.yaml` åŽé‡å¯ç›¸å…³æœåŠ¡ã€‚å®Œæ•´ schema è¯·è§[é…ç½®å‚è€ƒ](./configuration.md)ã€‚
...
session token åœ¨å¯åŠ¨æ—¶æ³¨å…¥ä»ªè¡¨æ¿ HTMLï¼Œæ¯æ¬¡æœåŠ¡å™¨é‡å¯åŽè½®æ¢ã€‚å¦‚éœ€å¯¹è¿è¡Œä¸­çš„ä»ªè¡¨æ¿ç¼–å†™è„šæœ¬ï¼Œå¯ä»Žæµè§ˆå™¨å¼€å‘è€…å·¥å…·ä¸­èŽ·å–ï¼ˆ`window.__ZED_SESSION_TOKEN__`ï¼‰ã€‚