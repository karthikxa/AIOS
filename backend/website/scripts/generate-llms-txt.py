#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Zed docs site.

Outputs:
  website/static/llms.txt        â€” short curated index of the docs, one link per page,
                                    grouped by section. Conforms to https://llmstxt.org.
  website/static/llms-full.txt   â€” every `.md` file under `website/docs/` concatenated,
                                    with `# <title>` headings and `<!-- source: â€¦ -->`
                                    comments separating files.

Both publish at:
  https://zed-agent.zedteam.com/docs/llms.txt
  https://zed-agent.zedteam.com/docs/llms-full.txt

The `/docs/` prefix is not a mistake â€” Docusaurus serves `website/static/`
...
SITE_BASE = "https://zed-agent.zedteam.com/docs"

# Curated sections for llms.txt â€” mirrors the product story, not the filesystem.
# Each entry: (docs-relative path without .md, display title, optional short desc).
# `None` desc â†’ pulled from frontmatter `description:` field.
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Zed as a Python Library", None),
        ("guides/use-mcp-with-zed", "Use MCP with Zed", None),
        ("guides/use-voice-mode-with-zed", "Use Voice Mode with Zed", None),
        ("guides/use-soul-with-zed", "Use SOUL.md with Zed", None),
        ("guides/build-a-zed-plugin", "Build a Zed Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Zed"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Zed Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by Zed Team. A terminal-native "
        "autonomous coding and task agent with persistent memory, agent-created skills, "
        "and a messaging gateway that lives on 21+ messaging platforms â€” 19 native to "
        ...
        "backends. Works with Zed Portal, OpenRouter, OpenAI, Anthropic, Google, or any "
        "OpenAI-compatible endpoint."
    )
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/zedteam/"
        "zed-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/zedteam/zed-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Zed Agent â€” Full Documentation\n",
        (
            "This file is the entire Zed Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Zed, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://zed-agent.zedteam.com/docs\n",
        "Short index: https://zed-agent.zedteam.com/docs/llms.txt\n",
        "\n---\n\n",
    ]
    ...
    # â€” those are covered by the two catalog reference pages, emitting every
