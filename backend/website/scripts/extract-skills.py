...
1. Local SKILL.md files under ``skills/`` (built-in) and ``optional-skills/``
   (official optional). These give us full metadata â€” overview prose, version,
   license, env vars, commands â€” that the unified index doesn't carry.

2. The unified Zed Skills Index at ``website/static/api/skills-index.json``,
...
def _source_url(source: str, identifier: str, extra: dict) -> str:
    ...
    # identifier is "owner/repo/<path...>" â€” link to the directory on GitHub.
...
def extract_unified_index_skills():
    """Read website/static/api/skills-index.json â€” the canonical multi-source index.

    Returns ``(skills, meta)`` where ``meta`` carries the index's
    ``generated_at`` timestamp and total count so the Skills Hub page can
    show a "Last refreshed â€¦" badge. Returns ``(None, None)`` when the
    ...
    for entry in data.get("skills", []):
        ...
        if len(description) > 280:
            description = description[:277] + "â€¦"
        tags = entry.get("tags", []) or []
        if not isinstance(tags, list):
            tags = []

        # Skip official entries here â€” extract_local_skills() already covered
        ...
        # real, human-readable category â€” prefer it over the tag heuristic.
...
def extract_legacy_cache_skills():
    """Read the deprecated skills/index-cache/ snapshots â€” fallback only."""
    skills = []
...
def _guess_category(tags: list) -> str:
    ...
    "Doramagic Crystal", "0.10.7 Dev", "Ap2") â€” version strings, brand
...
def _consolidate_small_categories(skills: list) -> list:
    ...
    # their category even if it's the only skill in it â€” the tap explicitly
...
def main():
    ...
    with open(OUTPUT, "w", encoding="utf-8") as f:
        # Minified â€” file is served over the wire, not read by humans.
