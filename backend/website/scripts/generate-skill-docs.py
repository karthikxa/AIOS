...
Sidebar is updated to nest all per-skill pages under Skills â†’ Bundled / Optional.
"""
...
# ignore markers in every SKILL.md â€” the generator handles it defensively.
_BOX_DRAWING_CHARS = frozenset("â”Œâ”â””â”˜â”€â”‚â•â•‘â•”â•—â•šâ•â• â•£â•¦â•©â•¬â”œâ”¤â”¬â”´â”¼â•­â•®â•¯â•°â–¶â—€â–²â–¼")
...
def _wrap_ascii_art_code_blocks(code_segment: str) -> str:
    ...
    left alone â€” double-wrapping is harmless but we'd rather keep the output
    clean.
    """
        ...
        while i < len(text):
            ...
            if ch == "`":
                ...
                if ch == "{":
                    ...
                    # Preserve full HTML comments (e.g. ascii-guard ignore markers) â€” they
...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/zed-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    zed_meta = (fm.get("metadata") or {}).get("zed") or {}
    tags = zed_meta.get("tags") or []
    related = zed_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional â€” install with `zed skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
    ...
    fm_title = sanitize_yaml_string(display_name + " â€” " + (short_desc or name))
    if len(fm_title) > 120:
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Zed Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Zed ships with a large built-in skill library copied into `~/.zed/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Zed also syncs bundled skills on `zed update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.zed/skills/` tree, it is still shipped with Zed; restore it with `zed skills reset <name> --restore`.",
        "",
...
def build_sidebar_items(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> dict:
    ...
    Structure:
    Skills
    â”œâ”€â”€ (hand-written pages first: google-workspace)
    â”œâ”€â”€ Bundled
    â”‚   â”œâ”€â”€ apple
    â”‚   â”‚   â”œâ”€â”€ apple-apple-notes
    â”‚   â”‚   â””â”€â”€ ...
    â”‚   â””â”€â”€ ...
    â””â”€â”€ Optional
        â””â”€â”€ ...
...
def write_sidebar(entries):
    # Sidebar layout:
    #   Skills
    #   â”œâ”€â”€ reference/skills-catalog
    #   â”œâ”€â”€ reference/optional-skills-catalog
    #   â”œâ”€â”€ Bundled
    #   â”‚   â”œâ”€â”€ apple/
    #   â”‚   â”‚   â”œâ”€â”€ apple-apple-notes
    #   â”‚   â”‚   â””â”€â”€ ...
