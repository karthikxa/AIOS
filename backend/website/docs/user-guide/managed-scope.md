...
`~/.zed/config.yaml`, `~/.zed/.env`, and even the shell environment â€” for
exactly the keys it pins. Everything else stays fully user-controlled.

:::note Different from a package-managerâ€“locked install
A package-managerâ€“managed install (declarative-distro / formula) blocks *all*
...
Managed scope is read from a system-level directory, default `/etc/zed`:

```text
/etc/zed/
â”œâ”€â”€ config.yaml     # managed config layer (wins over ~/.zed/config.yaml)
â””â”€â”€ .env            # managed env layer (wins over ~/.zed/.env + shell)
```
...
filesystem permission is the enforcement mechanism** â€” a standard user can read
...
The location can be relocated with the `ZED_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob â€” like `ZED_HOME` â€” set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Zed.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export ZED_MANAGED_DIR=/opt/org/zed-policy
```

:::warning
A user who can set `ZED_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `zed doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/zed/config.yaml` (managed) | `/etc/zed/.env` (managed) |
| 2 | `~/.zed/config.yaml` (user) | `~/.zed/.env` (user) |
...
too â€” otherwise it would not be "managed." This is the one place that inverts the
...
```bash
zed config        # shows a header naming the managed source + the pinned keys
zed doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Zed refuses and names the source:

```bash
$ zed config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/zed/config.yaml) and cannot be changed.
```

The same applies to managed secrets â€” `zed config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/zed

# Pin some config values for every user on this machine
sudo tee /etc/zed/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/zed/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/zed
sudo chmod 0644 /etc/zed/config.yaml /etc/zed/.env
```

Changes take effect on the next Zed start (a malformed managed file is logged
loudly and ignored â€” it never blocks startup, but the admin should check
`zed doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Zed as `root`), managed scope is advisory.
