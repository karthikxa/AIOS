...
description: Control your smart home with Zed Agent via Home Assistant integration.
...
Zed Agent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Zed Agent"
5. Copy the token
...
Zed Agent registers four tools for smart home control:
...
**Parameters:**
- `domain` *(optional)* â€” Filter by entity domain: `light`, `switch`, `climate`, `sensor`, `binary_sensor`, `cover`, `fan`, `media_player`, etc.
- `area` *(optional)* â€” Filter by area/room name (matches against friendly names): `living room`, `kitchen`, `bedroom`, etc.
...
**Parameters:**
- `entity_id` *(required)* â€” The entity to query, e.g., `light.living_room`, `climate.thermostat`, `sensor.temperature`
...
**Parameters:**
- `domain` *(optional)* â€” Filter by domain, e.g., `light`, `climate`, `switch`
...
**Parameters:**
- `domain` *(required)* â€” Service domain: `light`, `switch`, `climate`, `cover`, `media_player`, `fan`, `scene`, `script`
- `service` *(required)* â€” Service name: `turn_on`, `turn_off`, `toggle`, `set_temperature`, `set_hvac_mode`, `open_cover`, `close_cover`, `set_volume_level`
- `entity_id` *(optional)* â€” Target entity, e.g., `light.living_room`
- `data` *(optional)* â€” Additional parameters as a JSON object
...
Configure which events the agent sees in `~/.zed/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Zed Agent".
...
The adapter reads credentials from `~/.zed/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Zed profile
home and that there's no stray quoting around the URL/token. Restart the gateway
after editing â€” env changes are only applied on process start.
...
agent. In HA, open **Settings â†’ Voice assistants â†’ Add assistant** and note the
...
page (**Profile â†’ Security â†’ Long-lived access tokens**). Short-lived UI
...
running Zed â€” `curl -H "Authorization: Bearer <token>" <url>/api/` should
