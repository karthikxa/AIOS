...
sidebar_position: 3
title: "Discord"
description: "Set up Zed Agent as a Discord bot"
---

# Discord Setup

Zed Agent integrates with Discord as a bot, letting you chat with your AI assistant through direct messages or server channels. The bot receives your messages, processes them through the Zed Agent pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, voice messages, file attachments, and slash commands.

Before setup, here's the part most people want to know: how Zed behaves once it's in your server.

## How Zed Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | Zed responds to every message. No `@mention` needed. Each DM has its own session. |
| **Server channels** | By default, Zed only responds when you `@mention` it. If you post in a channel without mentioning it, Zed ignores the message. |
| **Free-response channels** | You can make specific channels mention-free with `DISCORD_FREE_RESPONSE_CHANNELS`, or disable mentions globally with `DISCORD_REQUIRE_MENTION=false`. Messages in these channels are answered inline â€” auto-threading is skipped so the channel stays a lightweight chat. |
| **Threads** | Zed replies in the same thread. Mention rules still apply unless that thread or its parent channel is configured as free-response. Threads stay isolated from the parent channel for session history. |
| **Shared channels with multiple users** | By default, Zed isolates session history per user inside the channel for safety and clarity. Two people talking in the same channel do not share one transcript unless you explicitly disable that. |
| **Messages mentioning other users** | When `DISCORD_IGNORE_NO_MENTION` is `true` (the default), Zed stays silent if a message @mentions other users but does **not** mention the bot. This prevents the bot from jumping into conversations directed at other people. Set to `false` if you want the bot to respond to all messages regardless of who is mentioned. This only applies in server channels, not DMs. |

:::tip
If you want a normal bot-help channel where people can talk to Zed without tagging it every time, add that channel to `DISCORD_FREE_RESPONSE_CHANNELS`.
:::

### Discord Gateway Model

Zed on Discord is not a webhook that replies statelessly. It runs through the full messaging gateway, which means each incoming message goes through:

1. authorization (`DISCORD_ALLOWED_USERS`)
2. mention / free-response checks
3. session lookup
4. session transcript loading
5. normal Zed agent execution, including tools, memory, and slash commands
6. response delivery back to Discord

That matters because behavior in a busy server depends on both Discord routing and Zed session policy.
...
So if Alice and Bob both talk to Zed in `#research`, Zed treats those as separate conversations by default even though they are using the same visible Discord channel.
...
Zed tracks running agents by session key.
...
This guide walks you through the full setup process â€” from creating your bot on Discord's Developer Portal to sending your first message.

## Step 1: Create a Discord Application

1. Go to the [Discord Developer Portal](https://discord.com/developers/applications) and sign in with your Discord account.
2. Click **New Application** in the top-right corner.
3. Enter a name for your application (e.g., "Zed Agent") and accept the Developer Terms of Service.
4. Click **Create**.

You'll land on the **General Information** page. Note the **Application ID** â€” you'll need it later to build the invite URL.
...
3. Under **Authorization Flow**:
   - Set **Public Bot** to **ON** â€” required to use the Discord-provided invite link (recommended). This allows the Installation tab to generate a default authorization URL.
...
- Without **Message Content Intent**, your bot receives message events but the message text is empty â€” the bot literally cannot see what you typed.
...
If your bot is online but never responds to messages, the **Message Content Intent** is almost certainly disabled. Go back to the [Developer Portal](https://discord.com/developers/applications), select your application â†’ Bot â†’ Privileged Gateway Intents, and make sure **Message Content Intent** is toggled ON. Click **Save Changes**.
:::
...
The bot token is the credential Zed Agent uses to log in as your bot. Still on the **Bot** page:
...
The token is only displayed once. If you lose it, you'll need to reset it and generate a new one. Never share your token publicly or commit it to Git â€” anyone with this token has full control of your bot.
:::
...
- **View Channels** â€” see the channels it has access to
- **Send Messages** â€” respond to your messages
- **Embed Links** â€” format rich responses
- **Attach Files** â€” send images, audio, and file outputs
- **Read Message History** â€” maintain conversation context

### Recommended Additional Permissions

- **Send Messages in Threads** â€” respond in thread conversations
- **Add Reactions** â€” react to messages for acknowledgment
...
After authorizing, the bot will appear in your server's member list (it will show as offline until you start the Zed gateway).

## Step 7: Find Your Discord User ID

Zed Agent uses your Discord User ID to control who can interact with the bot. To find it:

1. Open Discord (desktop or web app).
2. Go to **Settings** â†’ **Advanced** â†’ toggle **Developer Mode** to **ON**.
3. Close settings.
4. Right-click your own username (in a message, the member list, or your profile) â†’ **Copy User ID**.

Your User ID is a long number like `284102345871466496`.

:::tip
Developer Mode also lets you copy **Channel IDs** and **Server IDs** the same way â€” right-click the channel or server name and select Copy ID. You'll need a Channel ID if you want to set a home channel manually.
:::

## Step 8: Configure Zed Agent
...
Add the following to your `~/.zed/.env` file:
...
The bot should come online in Discord within a few seconds. Send it a message â€” either a DM or in a channel it can see â€” to test.

:::tip
You can run `zed gateway` in the background or as a systemd service for persistent operation. See the deployment docs for details.
:::

## Configuration Reference

Discord behavior is controlled through two files: **`~/.zed/.env`** for credentials and env-level toggles, and **`~/.zed/config.yaml`** for structured settings. Environment variables always take precedence over config.yaml values when both are set.

### Environment Variables (`.env`)

| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `DISCORD_BOT_TOKEN` | **Yes** | â€” | Bot token from the [Discord Developer Portal](https://discord.com/developers/applications). |
| `DISCORD_ALLOWED_USERS` | **Yes** | â€” | Comma-separated Discord user IDs allowed to interact with the bot. Without this **or** `DISCORD_ALLOWED_ROLES`, the gateway denies all users. |
| `DISCORD_ALLOWED_ROLES` | No | â€” | Comma-separated Discord role IDs. Any member with one of these roles is authorized â€” OR semantics with `DISCORD_ALLOWED_USERS`. Auto-enables the **Server Members Intent** on connect. Useful when moderation teams churn: new mods get access as soon as the role is granted, no config push needed. |
| `DISCORD_HOME_CHANNEL` | No | â€” | Channel ID where the bot sends proactive messages (cron output, reminders, notifications). |
...
| `DISCORD_THREAD_REQUIRE_MENTION` | No | `false` | When `true`, the in-thread mention shortcut is disabled â€” threads are gated the same as channels, requiring `@mention` even after the bot has already participated. Use this when multiple bots share a thread and you want each to fire only on explicit `@mention`. |
| `DISCORD_FREE_RESPONSE_CHANNELS` | No | â€” | Comma-separated channel IDs where the bot responds without requiring an `@mention`, even when `DISCORD_REQUIRE_MENTION` is `true`. |
| `DISCORD_IGNORE_NO_MENTION` | No | `true` | When `true`, the bot stays silent if a message `@mentions` other users but does **not** mention the bot. Prevents the bot from jumping into conversations directed at other people. Only applies in server channels, not DMs. |
| `DISCORD_AUTO_THREAD` | No | `true` | When `true`, automatically creates a new thread for every `@mention` in a text channel, so each conversation is isolated (similar to Slack behavior). Messages already inside threads or DMs are unaffected. |
| `DISCORD_ALLOW_BOTS` | No | `"none"` | Controls how the bot handles messages from other Discord bots. `"none"` â€” ignore all other bots. `"mentions"` â€” only accept bot messages that `@mention` Zed. `"all"` â€” accept all bot messages. |
| `DISCORD_REACTIONS` | No | `true` | When `true`, the bot adds emoji reactions to messages during processing (ðŸ‘€ when starting, âœ… on success, âŒ on error). Set to `false` to disable reactions entirely. |
| `DISCORD_IGNORED_CHANNELS` | No | â€” | Comma-separated channel IDs where the bot **never** responds, even when `@mentioned`. Takes priority over all other channel settings. |
| `DISCORD_ALLOWED_CHANNELS` | No | â€” | Comma-separated channel IDs. When set, the bot **only** responds in these channels (plus DMs if allowed). Overrides `config.yaml` `discord.allowed_channels`. Combine with `DISCORD_IGNORED_CHANNELS` to express allow/deny rules. |
| `DISCORD_NO_THREAD_CHANNELS` | No | â€” | Comma-separated channel IDs where the bot responds directly in the channel instead of creating a thread. Only relevant when `DISCORD_AUTO_THREAD` is `true`. |
...
| `DISCORD_HISTORY_BACKFILL_LIMIT` | No | `50` | Maximum number of messages to scan backwards when assembling the backfill block. In practice the scan usually stops earlier â€” at the bot's own last message in the channel. |
| `DISCORD_REPLY_TO_MODE` | No | `"first"` | Controls reply-reference behavior: `"off"` â€” never reply to the original message, `"first"` â€” reply-reference on the first message chunk only (default), `"all"` â€” reply-reference on every chunk. |
...
| `DISCORD_PROXY` | No | â€” | Proxy URL for Discord connections (HTTP, WebSocket, REST). Overrides `HTTPS_PROXY`/`ALL_PROXY`. Supports `http://`, `https://`, and `socks5://` schemes. |
...
| `ZED_DISCORD_TEXT_BATCH_DELAY_SECONDS` | No | `0.6` | Grace window the adapter waits before flushing a queued text chunk. Useful for smoothing streamed output. |
| `ZED_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS` | No | `2.0` | Delay between split chunks when a single message exceeds Discord's length limit. |

### Config File (`config.yaml`)

The `discord` section in `~/.zed/config.yaml` mirrors the env vars above. Config.yaml settings are applied as defaults â€” if the equivalent env var is already set, the env var wins.
...
Free-response channels also **skip auto-threading** â€” the bot replies inline rather than spinning off a new thread per message. This keeps the channel usable as a lightweight chat surface. If you want threading behavior, don't list the channel as free-response (use normal `@mention` flow instead).

#### `discord.auto_thread`

**Type:** boolean â€” **Default:** `true`

When enabled, every `@mention` in a regular text channel automatically creates a new thread for the conversation. This keeps the main channel clean and gives each conversation its own isolated session history. Once a thread is created, subsequent messages in that thread don't require `@mention` â€” the bot knows it's already participating. Set [`thread_require_mention`](#discordthread_require_mention) to `true` to disable this in-thread shortcut for multi-bot setups.
...
- ðŸ‘€ added when the bot starts processing your message
- âœ… added when the response is delivered successfully
- âŒ added if an error occurs during processing
...
Channel IDs where the bot **never** responds, even when directly `@mentioned`. This takes the highest priority â€” if a channel is in this list, the bot silently ignores all messages there, regardless of `require_mention`, `free_response_channels`, or any other setting.
...
- If a message arrives inside a thread or forum post and that thread has no explicit entry, Zed falls back to the parent channel/forum ID.
...
- **Threads**: backfill scans the thread only â€” Discord's `channel.history()` on a thread returns only that thread's messages, not the parent channel. This is the right scope because threads are usually self-contained conversations.
- **DMs**: skipped. Every DM message triggers the bot, so the session transcript is already complete â€” there's no mention gap to fill.
- **Free-response channels** and **bot's own auto-created threads**: skipped for the same reason â€” no mention gating means no gap.
...
> **Note:** Messages that arrive *while* the bot is processing (between a trigger and its response) are not captured. This is an accepted simplification â€” the user can re-send or tag again.

#### `discord.history_backfill_limit`

**Type:** integer â€” **Default:** `50`

Maximum number of messages to scan backwards when recovering channel context. In practice the scan usually stops much earlier â€” at the bot's own last message in the channel, which is the natural boundary between turns. This limit is a safety cap for cold starts and long gaps where no prior bot message exists in recent history.
...
When `true`: Alice and Bob talking in `#research` each have their own separate conversation with Zed. When `false`: the entire channel shares one conversation transcript and one running-agent slot.
...
**Type:** string â€” **Default:** `"all"` â€” **Values:** `off`, `new`, `all`, `verbose`
...
- `off` â€” no progress messages
- `new` â€” only show the first tool call per turn
- `all` â€” show all tool calls (truncated to 40 characters in gateway messages)
- `verbose` â€” show full tool call details (can produce long messages)

#### `display.tool_progress_command`

**Type:** boolean â€” **Default:** `false`

When enabled, makes the `/verbose` slash command available in the gateway, letting you cycle through tool progress modes (`off â†’ new â†’ all â†’ verbose â†’ off`) without editing config.yaml.
...
- A user in `allow_admin_from` for a scope (DM or server channel) can run **every** registered slash command â€” built-in AND plugin-registered â€” through the live command registry.
...
1. **Provider selection** â€” a Select dropdown showing available providers (up to 25).
2. **Model selection** â€” a second dropdown with models for the chosen provider (up to 25).
...
Zed automatically registers installed skills as **native Discord Application Commands**. This means skills appear in Discord's autocomplete `/` menu alongside built-in commands.

- Each skill becomes a Discord slash command (e.g., `/code-review`, `/ascii-art`)
- Skills accept an optional `args` string parameter
- Discord has a limit of 100 application commands per bot â€” if you have more skills than available slots, extra skills are skipped with a warning in the logs
- Skills are registered during bot startup alongside built-in commands like `/model`, `/reset`, and `/background`

No extra configuration is needed â€” any skill installed via `zed skills install` is automatically registered as a Discord slash command on the next gateway restart.

### Disabling Slash Command Registration

If you run multiple Zed gateways against the same Discord application (e.g. staging + production), only one of them should own the global slash-command registration â€” otherwise the last startup wins and the registrations flap. Turn slash registration off on the "follower" gateway:
...
Leaving this at `true` on the "primary" gateway keeps the normal behavior â€” global `/`-menu commands for built-ins and installed skills.
...
| Video (MP4/MOV) | `send_video` â€” native video player |
| Audio / Voice | `send_voice` â€” native voice message when possible, file attachment otherwise |
| Documents (PDF/ZIP/docx/etc.) | `send_document` â€” native attachment with download button |

Discord's per-upload size limit depends on the server's boost tier (25 MB free, up to 500 MB). If Zed gets an HTTP 413, the adapter falls back to a link pointing at the local cache path rather than failing silently.

## Receiving Arbitrary File Types

By default the bot caches uploads that match a built-in allowlist â€” images, audio, video, PDF, text/markdown/csv/log, JSON/XML/YAML/TOML, zip, docx/xlsx/pptx. Anything else (a `.wav`, a `.bin`, a custom-extension dump) gets logged as `Unsupported document type` and dropped before the agent sees it.
...
discord:
  ...
  # Optional â€” raise/disable the per-file size cap. Default is 32 MiB.
...
When the agent calls the `clarify` tool â€” to ask which approach you prefer, get post-task feedback, or check before a non-trivial decision â€” Discord renders the question with **one button per choice**:
...
Replace the ID with the actual channel ID (right-click â†’ Copy Channel ID with Developer Mode on).

## Voice Messages

Zed Agent supports Discord voice messages:
...
- **Discord voice channels**: Zed can also join a voice channel, listen to users speaking, and talk back in the channel.

For the full setup and operational guide, see:
- [Voice Mode](/user-guide/features/voice-mode)
- [Use Voice Mode with Zed](/guides/use-voice-mode-with-zed)

### Voice Channel Audio Effects (ambient + verbal acks)

When the bot is in a voice channel, you can give it a more conversational feel: a short verbal acknowledgement ("let me look into that") before it starts working, and a subtle ambient "thinking" bed that plays underneath while tools run â€” the speech ducks the ambient down and swells it back when finished, similar to Grok voice mode.

discord.py plays only one audio stream per connection, so Zed installs a software mixer on the outgoing stream that sums an ambient loop, acknowledgements, and TTS replies into that single stream â€” they overlap instead of cutting each other off.
...
- **Attachments** ride along on the starter message of the new thread â€” no separate upload step, no partial sends.
...
**Fix**: Go to [Developer Portal](https://discord.com/developers/applications) â†’ your app â†’ Bot â†’ Privileged Gateway Intents â†’ enable **Message Content Intent** â†’ Save Changes. Restart the gateway.
...
**Fix**: In Discord, go to the channel's settings â†’ Permissions â†’ add the bot's role with **View Channel** and **Read Message History** enabled.

### 403 Forbidden errors

**Cause**: The bot is missing required permissions.

**Fix**: Re-invite the bot with the correct permissions using the URL from Step 5, or manually adjust the bot's role permissions in Server Settings â†’ Roles.

### Bot is offline

**Cause**: The Zed gateway isn't running, or the token is incorrect.

**Fix**: Check that `zed gateway` is running. Verify `DISCORD_BOT_TOKEN` in your `.env` file. If you recently reset the token, update it.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `DISCORD_ALLOWED_USERS`.

**Fix**: Add your User ID to `DISCORD_ALLOWED_USERS` in `~/.zed/.env` and restart the gateway.
...
**Fix**: Set this in `~/.zed/config.yaml` and restart the gateway:

```yaml
group_sessions_per_user: true
```

If you intentionally want a shared room conversation, leave it off â€” just expect shared transcript history and shared interrupt behavior.

## Security

:::warning
Always set `DISCORD_ALLOWED_USERS` (or `DISCORD_ALLOWED_ROLES`) to restrict who can interact with the bot. Without either, the gateway denies all users by default as a safety measure. Only authorize people you trust â€” authorized users have full access to the agent's capabilities, including tool use and system access.
:::

### Role-Based Access Control

For servers where access is managed by roles instead of individual user lists (moderator teams, support staff, internal tooling), use `DISCORD_ALLOWED_ROLES` â€” a comma-separated list of role IDs. Any member with one of those roles is authorized.

```bash
# ~/.zed/.env â€” works alongside or instead of DISCORD_ALLOWED_USERS
...
- **Server Members Intent auto-enabled.** When `DISCORD_ALLOWED_ROLES` is set, the bot enables the Members intent on connect â€” required for Discord to send role information with member records.
- **Role IDs, not names.** Grab them from Discord: **User Settings â†’ Advanced â†’ Developer Mode ON**, then right-click any role â†’ **Copy Role ID**.
...
This is the preferred pattern when the moderation team churns â€” new moderators get access the moment the role is granted, with no `.env` edit or gateway restart.

### Mention Control

By default, Zed blocks the bot from pinging `@everyone`, `@here`, and role mentions, even if its reply contains those tokens. This prevents a poorly-worded prompt or echoed user content from spamming a whole server. Individual `@user` pings and reply-reference pings (the little "replying toâ€¦" chip) stay enabled so normal conversation still works.
...
```bash
# ~/.zed/.env â€” env vars win over config.yaml
...
For more information on securing your Zed Agent deployment, see the [Security Guide](../security.md).
