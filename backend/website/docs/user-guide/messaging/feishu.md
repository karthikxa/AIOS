...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up Zed Agent as a Feishu or Lark bot"
---

# Feishu / Lark Setup

Zed Agent integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` â€” recommended; Zed opens the outbound connection and you do not need a public webhook endpoint
- `webhook` â€” useful when you want Feishu/Lark to push events into your gateway over HTTP

## How Zed Behaves

| Context | Behavior |
|---------|----------|
| Direct messages | Zed responds to every message. |
| Group chats | Zed responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. Zed will automatically create a bot application with the correct permissions and save the credentials.
...
5. Run `zed gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
2. In the **Event Configuration** section, subscribe to:
   - `im.message.receive_v1` â€” required for receiving messages
...
Use WebSocket mode when Zed runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run Zed behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, Zed starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
When Feishu sends a URL verification challenge (`type: url_verification`), the webhook responds automatically so you can complete the subscription setup in the Feishu developer console. The challenge response is gated on `FEISHU_VERIFICATION_TOKEN` when set â€” challenge requests with a missing or mismatched token are rejected so an unauthenticated remote cannot prove endpoint control by echoing attacker-controlled challenge data.
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how Zed responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | Zed responds to @mentions from any user in any group. |
| `allowlist` | Zed only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | Zed ignores all group messages entirely. |
...
Set `FEISHU_REQUIRE_MENTION=false` to let Zed read all group traffic without requiring an @mention:

```bash
FEISHU_REQUIRE_MENTION=false
```

For per-chat control, set `require_mention` on a `group_rules` entry â€” see [Per-Group Access Control](#per-group-access-control) below.

### Bot Identity

Zed auto-detects the bot's `open_id` and display name on startup. You only need to set these manually when auto-detection cannot reach the Feishu API, or when your app uses tenant-scoped user IDs:
...
By default Zed ignores messages sent by other bots. Enable bot-to-bot messaging when you want Zed to participate in A2A orchestration or receive notifications from other bots in the same group.
...
| `mentions` | Accept only when the peer bot @mentions Zed. |
| `all` | Accept every peer bot message. |

Also configurable as `feishu.allow_bots` in `config.yaml` (env wins when both are set).

Peer bots do not need to be added to `FEISHU_ALLOWED_USERS` â€” that allowlist applies to human senders only.
...
Gateway-driven update prompts use a native Feishu `Yes` / `No` card instead of falling back to plain text replies. When `zed update --gateway` needs confirmation, the adapter records the selected answer in Zed's `.update_response` file and replaces the card inline with a resolved state.

Card action events are dispatched with `MessageType.COMMAND`, so they flow through the normal command processing pipeline.

This is also how **command approval** works â€” when the agent needs to run a dangerous command, it sends an interactive card with Allow Once / Session / Always / Deny buttons. The user clicks a button, and the card action callback delivers the approval decision back to the agent.
...
:::warning
Without all three steps, Feishu will successfully *send* interactive cards (sending only requires `im:message:send` permission), but clicking any button will return error 200340. The card appears to work â€” the error only surfaces when a user interacts with it.
:::

## Document Comment Intelligent Reply

Beyond chat, the adapter can also answer `@`-mentions left on **Feishu/Lark documents**. When a user comments on a document (local text selection or whole-doc comment) and @-mentions the bot, Zed reads the document plus the surrounding comment thread and posts an LLM reply inline on the thread.
...
Document-comment replies are **explicit-grant only** â€” there is no implicit allow-all mode. Permissions resolve in this order (first match wins, per field):

1. **Exact doc** â€” rule scoped to a specific document token.
2. **Wildcard** â€” rule that matches a pattern of docs.
3. **Top-level** â€” default rule for the workspace.

Two policies are available per rule:

- **`allowlist`** â€” a static list of users / tenants.
- **`pairing`** â€” static list âˆª runtime-approved store. Useful for rollouts where moderators can grant access live.

Rules live in `~/.zed/feishu_comment_rules.json` (pairing grants in `~/.zed/feishu_comment_pairing.json`) with mtime-cached hot-reload â€” edits take effect on the next comment event without restarting the gateway.
...
You can invite the Zed Feishu/Lark bot into a video meeting the same way you invite a human participant. When the bot receives the meeting invitation event, Zed can automatically start an agent turn that attempts to join the meeting.

Powered by the `vc.bot.meeting_invited_v1` event, the flow is:

- A user invites the bot to a Feishu/Lark video meeting.
- Feishu/Lark sends Zed the meeting invitation event.
- Zed extracts the inviter, meeting topic, and meeting number.
...
- If the invite is malformed, or the agent cannot join, Zed drops the event or replies to the inviter with a concise explanation.
...
- Keep `im:message` and `im:message:send_as_bot` enabled so Zed can reply to the inviter.
...
- `.ogg`, `.opus` â†’ uploaded as `opus` audio
- `.mp4`, `.mov`, `.avi`, `.m4v` â†’ uploaded as `mp4` media
- `.pdf`, `.doc(x)`, `.xls(x)`, `.ppt(x)` â†’ uploaded with their document type
- Everything else â†’ uploaded as a generic stream file
...
|---------|---------|---------|
| Quiet period | `ZED_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| Max messages per batch | `ZED_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| Max characters per batch | `ZED_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| Setting | Env Var | Default |
|---------|---------|---------|
| Quiet period | `ZED_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.zed/feishu_seen_message_ids.json`.

| Setting | Env Var | Default |
|---------|---------|---------|
| Cache size | `ZED_FEISHU_DEDUP_CACHE_SIZE` | 2048 entries |

## All Environment Variables

| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `FEISHU_APP_ID` | âœ… | â€” | Feishu/Lark App ID |
| `FEISHU_APP_SECRET` | âœ… | â€” | Feishu/Lark App Secret |
| `FEISHU_DOMAIN` | â€” | `feishu` | `feishu` (China) or `lark` (international) |
| `FEISHU_CONNECTION_MODE` | â€” | `websocket` | `websocket` or `webhook` |
| `FEISHU_ALLOWED_USERS` | â€” | _(empty)_ | Comma-separated open_id list for user allowlist |
| `FEISHU_ALLOW_BOTS` | â€” | `none` | Accept messages from other bots: `none`, `mentions`, or `all` |
| `FEISHU_REQUIRE_MENTION` | â€” | `true` | Whether group messages must @mention the bot |
| `FEISHU_HOME_CHANNEL` | â€” | â€” | Chat ID for cron/notification output |
| `FEISHU_ENCRYPT_KEY` | â€” | _(empty)_ | Encrypt key for webhook signature verification |
| `FEISHU_VERIFICATION_TOKEN` | â€” | _(empty)_ | Verification token for webhook payload auth |
| `FEISHU_GROUP_POLICY` | â€” | `allowlist` | Group message policy: `open`, `allowlist`, `disabled` |
| `FEISHU_BOT_OPEN_ID` | â€” | _(empty)_ | Bot's open_id (for @mention detection) |
| `FEISHU_BOT_USER_ID` | â€” | _(empty)_ | Bot's user_id (for @mention detection) |
| `FEISHU_BOT_NAME` | â€” | _(empty)_ | Bot's display name (for @mention detection) |
| `FEISHU_WEBHOOK_HOST` | â€” | `127.0.0.1` | Webhook server bind address |
| `FEISHU_WEBHOOK_PORT` | â€” | `8765` | Webhook server port |
| `FEISHU_WEBHOOK_PATH` | â€” | `/feishu/webhook` | Webhook endpoint path |
| `ZED_FEISHU_DEDUP_CACHE_SIZE` | â€” | `2048` | Max deduplicated message IDs to track |
| `ZED_FEISHU_TEXT_BATCH_DELAY_SECONDS` | â€” | `0.6` | Text burst debounce quiet period |
| `ZED_FEISHU_TEXT_BATCH_MAX_MESSAGES` | â€” | `8` | Max messages merged per text batch |
| `ZED_FEISHU_TEXT_BATCH_MAX_CHARS` | â€” | `4000` | Max characters merged per text batch |
| `ZED_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | â€” | `0.8` | Media burst debounce quiet period |
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `zed gateway setup` |
| `Another local Zed gateway is already using this Feishu app_id` | Only one Zed instance can use the same app_id at a time. Stop the other gateway first. |
...
| Peer bot messages still ignored after enabling `FEISHU_ALLOW_BOTS` | Zed can't identify itself yet â€” set `FEISHU_BOT_OPEN_ID` (and `FEISHU_BOT_USER_ID` if your app uses `sender_id_type=user_id`). |
...
Feishu / Lark uses the `zed-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.
