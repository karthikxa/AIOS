...
> Run `zed gateway setup` and pick **SimpleX** for a guided walk-through.
...
| `SIMPLEX_ALLOWED_USERS` | Recommended | Comma-separated allowlist. Each entry can be a numeric `contactId` **or** a display name â€” both forms work. |
...
| `ZED_SIMPLEX_TEXT_BATCH_DELAY` | Optional | Quiet-period seconds (default: `0.8`) used to concatenate rapid-fire inbound text messages into one event |

## Find your contact ID or display name

After starting the daemon, open a conversation with your agent contact. The numeric `contactId` appears in session logs or via `zed send_message action=list`. If you'd rather use the display name shown in the SimpleX UI, that works too â€” `SIMPLEX_ALLOWED_USERS` accepts either form.
...
2. Use **DM pairing** â€” send any message to the bot and it will reply with a pairing code. Enter that code via `zed pairing approve simplex <CODE>`.

## Group chats

By default the adapter ignores group messages â€” a bot in a group otherwise
...
- **Inbound** â€” incoming images, voice notes, and files are accepted via
  the daemon's XFTP flow (`rcvFileDescrReady` â†’ `/freceive` â†’ wait for
  `rcvFileComplete`) and surfaced as `MessageEvent.media_urls` with the
  appropriate `MessageType` (`PHOTO`, `VOICE`, `TEXT` + document).
- **Outbound** â€” `send_image_file`, `send_voice`, `send_document`, and
...
Agent replies can also embed `MEDIA:/path/to/file` tags in plain text â€”
...
- The connection between Zed and the daemon is local WebSocket (`ws://127.0.0.1:5225`) â€” no data leaves your machine
