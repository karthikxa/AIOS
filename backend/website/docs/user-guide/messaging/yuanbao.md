...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Zed Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Zed to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.zed/.env`:
...
- **WebSocket gateway** â€” real-time bidirectional communication
- **HMAC authentication** â€” secure request signing with APP_ID/APP_SECRET
- **C2C messaging** â€” direct user-to-bot conversations
- **Group messaging** â€” conversations in group chats
- **Media support** â€” images, files, and voice messages via COS (Cloud Object Storage)
- **Markdown formatting** â€” messages are automatically chunked for Yuanbao's size limits
- **Message deduplication** â€” prevents duplicate processing of the same message
- **Heartbeat/keep-alive** â€” maintains WebSocket connection stability
- **Typing indicators** â€” shows "typingâ€¦" status while the agent processes
- **Automatic reconnection** â€” handles WebSocket disconnections with exponential backoff
- **Group information queries** â€” retrieve group details and member lists
- **Sticker/Emoji support** â€” send TIMFaceElem stickers and emoji in conversations
- **Auto-sethome** â€” first user to message the bot is automatically set as the home channel owner
- **Slow-response notification** â€” sends a waiting message when the agent takes longer than expected
...
All standard Zed commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.zed/logs/gateway.log`
...
4. Consider enabling verbose logging: `ZED_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Zed automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
| Reconnect backoff | 1s â†’ 60s (exponential) | Wait time between reconnect attempts |
