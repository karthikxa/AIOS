...
title: "SMS (Twilio)"
description: "Set up Zed Agent as an SMS chatbot via Twilio"
---

# SMS Setup (Twilio)

Zed connects to SMS through the [Twilio](https://www.twilio.com/) API. People text your Twilio phone number and get AI responses back â€” same conversational experience as Telegram or Discord, but over standard text messages.
...
- **Twilio account** â€” [Sign up at twilio.com](https://www.twilio.com/try-twilio) (free trial available)
- **A Twilio phone number** with SMS capability
- **A publicly accessible server** â€” Twilio sends webhooks to your server when SMS arrives
...
3. Go to **Phone Numbers â†’ Manage â†’ Active Numbers** â€” note your phone number in E.164 format (e.g., `+15551234567`)
...
3. Under **Messaging â†’ A MESSAGE COMES IN**, set:
   - **Webhook**: `https://your-server:8080/webhooks/twilio`
   - **HTTP Method**: `POST`

:::tip Exposing Your Webhook
If you're running Zed locally, use a tunnel to expose the webhook:
...
**Set `SMS_WEBHOOK_URL` to the same URL you configured in Twilio.** This is required for Twilio signature validation â€” the adapter will refuse to start without it:
...
Text your Twilio number â€” Zed will respond via SMS.
...
| `SMS_WEBHOOK_URL` | Yes | Public URL for Twilio signature validation â€” must match the webhook URL in your Twilio Console |
| `SMS_WEBHOOK_PORT` | No | Webhook listener port (default: `8080`) |
| `SMS_WEBHOOK_HOST` | No | Webhook bind address (default: `127.0.0.1`) |
| `SMS_INSECURE_NO_SIGNATURE` | No | Set to `true` to disable signature validation (local dev only â€” **not for production**) |
...
- **Plain text only** â€” Markdown is automatically stripped since SMS renders it as literal characters
- **1600 character limit** â€” Longer responses are split across multiple messages at natural boundaries (newlines, then spaces)
- **Echo prevention** â€” Messages from your own Twilio number are ignored to prevent loops
- **Phone number redaction** â€” Phone numbers are redacted in logs for privacy
...
Zed validates that inbound webhooks genuinely originate from Twilio by verifying the `X-Twilio-Signature` header (HMAC-SHA1). This prevents attackers from injecting forged messages.
...
3. Check the Twilio Console â†’ **Monitor â†’ Logs â†’ Messaging** for delivery errors
...
3. Check Zed gateway logs for Twilio API errors
