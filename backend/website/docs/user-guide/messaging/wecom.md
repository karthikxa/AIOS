...
description: "Connect Zed Agent to WeCom via the AI Bot WebSocket gateway"
---

# WeCom (Enterprise WeChat)

Connect Zed to [WeCom](https://work.weixin.qq.com/) (ä¼ä¸šå¾®ä¿¡), Tencent's enterprise messaging platform. The adapter uses WeCom's AI Bot WebSocket gateway for real-time bidirectional communication â€” no public endpoint or webhook needed.
...
Select **WeCom** and scan the QR code with your WeCom mobile app. Zed will automatically create a bot application with the correct permissions and save the credentials.
...
2. Navigate to **Applications** â†’ **Create Application** â†’ **AI Bot**
3. Configure the bot name and description
4. Copy the **Bot ID** and **Secret** from the credentials page
5. Run `zed gateway setup`, select **WeCom**, and enter the credentials when prompted
...
- **WebSocket transport** â€” persistent connection, no public endpoint needed
- **DM and group messaging** â€” configurable access policies
- **Per-group sender allowlists** â€” fine-grained control over who can interact in each group
- **Media support** â€” images, files, voice, video upload and download
- **AES-encrypted media** â€” automatic decryption for inbound attachments
- **Quote context** â€” preserves reply threading
- **Markdown rendering** â€” rich text responses
- **Reply correlation** â€” responses are correlated to the inbound message context
- **Auto-reconnect** â€” exponential backoff on connection drops

:::note Streaming and typing indicators
The WeCom adapter delivers each response as a single complete message â€” it does
...
|-----|---------|-------------|
| `bot_id` | â€” | WeCom AI Bot ID (required) |
| `secret` | â€” | WeCom AI Bot Secret (required) |
...
5. Entries can optionally use the `wecom:user:` or `wecom:group:` prefix format â€” the prefix is stripped automatically.
...
No configuration is needed â€” decryption happens transparently when encrypted media is received.
...
**Chunked upload:** Files are uploaded in 512 KB chunks through a three-step protocol (init â†’ chunks â†’ finish). The adapter handles this automatically.
...
- Images > 10 MB â†’ sent as file
- Videos > 10 MB â†’ sent as file
- Voice > 2 MB â†’ sent as file
- Non-AMR audio â†’ sent as file (WeCom only supports AMR for native voice)
...
The full response is delivered as a single message â€” the adapter does not stream tokens incrementally. If the inbound request context has expired or is unavailable, the adapter falls back to proactive message sending via `aibot_send_msg`.
...
| `WECOM_ALLOWED_USERS` | â€” | _(empty)_ | Comma-separated user IDs for the gateway-level allowlist |
| `WECOM_HOME_CHANNEL` | â€” | â€” | Chat ID for cron/notification output |
| `WECOM_WEBSOCKET_URL` | â€” | `wss://openws.work.weixin.qq.com` | WebSocket gateway URL |
| `WECOM_DM_POLICY` | â€” | `open` | DM access policy |
| `WECOM_GROUP_POLICY` | â€” | `open` | Group access policy |
