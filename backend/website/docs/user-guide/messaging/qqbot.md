...
Connect Zed to QQ via the **Official QQ Bot API (v2)** â€” supporting private (C2C), group @-mentions, guild, and direct messages with voice transcription.
...
1. **QQ Bot Application** â€” Register at [q.qq.com](https://q.qq.com):
...
2. **Dependencies** â€” The adapter requires `aiohttp` and `httpx`:
   ```bash
...
Set the required environment variables in `~/.zed/.env`:
...
|---|---|---|
| `QQ_APP_ID` | QQ Bot App ID (required) | â€” |
| `QQ_CLIENT_SECRET` | QQ Bot App Secret (required) | â€” |
| `QQBOT_HOME_CHANNEL` | OpenID for cron/notification delivery | â€” |
...
| `QQ_GROUP_ALLOWED_USERS` | Comma-separated group OpenIDs for group access | â€” |
...
| `QQ_STT_API_KEY` | API key for voice-to-text provider | â€” |
| `QQ_STT_BASE_URL` | (Not read directly â€” set `platforms.qqbot.extra.stt.baseUrl` in `config.yaml` instead) | n/a |
| `QQ_STT_MODEL` | STT model name | `glm-asr` |

## Advanced Configuration

For fine-grained control, add platform settings to `~/.zed/config.yaml`:
...
1. **QQ built-in ASR** (free, always tried first) â€” QQ provides `asr_refer_text` in voice message attachments, which uses Tencent's own speech recognition
2. **Configured STT provider** (fallback) â€” If QQ's ASR doesn't return text, the adapter calls an OpenAI-compatible STT API:
...
This usually means:
- **Invalid App ID / Secret** â€” Double-check your credentials at q.qq.com
- **Missing permissions** â€” Ensure the bot has the required intents enabled
- **Sandbox-only bot** â€” If the bot is in sandbox mode, it can only receive messages from QQ's sandbox test channel
