...
sidebar_position: 7
title: "Email"
description: "Set up Zed Agent as an email assistant via IMAP/SMTP"
---

# Email Setup

Zed can receive and reply to emails using standard IMAP and SMTP protocols. Send an email to the agent's address and it replies in-thread â€” no special client or bot API needed. Works with Gmail, Outlook, Yahoo, Fastmail, or any provider that supports IMAP/SMTP.
...
|---|---|---|
| Let people email the Zed agent and receive replies | Email gateway adapter on this page | None beyond an IMAP/SMTP email account |
...
- **A dedicated email account** for your Zed agent (don't use your personal email)
...
4. Copy the 16-character password â€” you'll use this instead of your regular password
...
- **Reply emails** (subject starting with `Re:`) skip the subject prefix â€” the thread context is already established
- **Attachments** are cached locally:
  - Images (JPEG, PNG, GIF, WebP) â†’ available to the vision tool
  - Documents (PDF, ZIP, etc.) â†’ available for file access
- **HTML-only emails** have tags stripped for plain text extraction
- **Self-messages** are filtered out to prevent reply loops
- **Automated/noreply senders** are silently ignored â€” `noreply@`, `mailer-daemon@`, `bounce@`, `no-reply@`, and emails with `Auto-Submitted`, `Precedence: bulk`, or `List-Unsubscribe` headers
...
Email access follows the same pattern as all other Zed platforms:

1. **`EMAIL_ALLOWED_USERS` set** â†’ only emails from those addresses are processed
2. **No allowlist set** â†’ unknown senders get a pairing code
3. **`EMAIL_ALLOW_ALL_USERS=true`** â†’ any sender is accepted (use with caution)
...
| Problem | Solution |
|---------|----------|
| **"IMAP connection failed"** at startup | Verify `EMAIL_IMAP_HOST` and `EMAIL_IMAP_PORT`. Ensure IMAP is enabled on the account. For Gmail, enable it in Settings â†’ Forwarding and POP/IMAP. |
...
| **Messages not received** | Check `EMAIL_ALLOWED_USERS` includes the sender's email. Check spam folder â€” some providers flag automated replies. |
| **"Authentication failed"** | For Gmail, you must use an App Password, not your regular password. Ensure 2FA is enabled first. |
| **Duplicate replies** | Ensure only one gateway instance is running. Check `zed gateway status`. |
...
:::warning
**Use a dedicated email account.** Don't use your personal email â€” the agent stores the password in `.env` and has full inbox access via IMAP.
:::
...
- The password is stored in `~/.zed/.env` â€” protect this file (`chmod 600`)
- IMAP uses SSL (port 993) and SMTP uses STARTTLS (port 587) by default â€” connections are encrypted
...
|----------|----------|---------|-------------|
| `EMAIL_ADDRESS` | Yes | â€” | Agent's email address |
| `EMAIL_PASSWORD` | Yes | â€” | Email password or app password |
| `EMAIL_IMAP_HOST` | Yes | â€” | IMAP server host (e.g., `imap.gmail.com`) |
| `EMAIL_SMTP_HOST` | Yes | â€” | SMTP server host (e.g., `smtp.gmail.com`) |
| `EMAIL_IMAP_PORT` | No | `993` | IMAP server port |
| `EMAIL_SMTP_PORT` | No | `587` | SMTP server port |
| `EMAIL_POLL_INTERVAL` | No | `15` | Seconds between inbox checks |
| `EMAIL_ALLOWED_USERS` | No | â€” | Comma-separated allowed sender addresses |
| `EMAIL_HOME_ADDRESS` | No | â€” | Default delivery target for cron jobs |
