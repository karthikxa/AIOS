...
sidebar_position: 15
title: "Weixin (WeChat)"
description: "Connect Zed Agent to personal WeChat accounts via the iLink Bot API"
---

# Weixin (WeChat)

Connect Zed to [WeChat](https://weixin.qq.com/) (å¾®ä¿¡), Tencent's personal messaging platform. The adapter uses Tencent's **iLink Bot API** for personal WeChat accounts â€” this is distinct from WeCom (Enterprise WeChat). Messages are delivered via long-polling, so no public endpoint or webhook is required.

:::info
This adapter is for **personal WeChat accounts** (å¾®ä¿¡). If you need enterprise/corporate WeChat, see the [WeCom adapter](./wecom.md) instead.
:::

:::warning iLink bot identity â€” ordinary WeChat groups may not work
QR login connects Zed to an **iLink bot identity** (e.g. `a5ace6fd482e@im.bot`), **not** a fully scriptable ordinary personal WeChat account. Consequences:
...
- `@`-mentioning the personal WeChat account used to scan the QR code is **not** the same as `@`-mentioning the iLink bot â€” the bot is a separate identity.
- The `WEIXIN_GROUP_POLICY` / `WEIXIN_GROUP_ALLOWED_USERS` settings below only take effect when iLink actually returns group events for your account type. If it doesn't, group messages will never reach Zed regardless of policy.

In practice, most deployments only get DMs to the iLink bot working reliably. If group delivery doesn't work after configuration, the limitation is on the iLink side, not in Zed. The gateway logs a `WARNING` at startup whenever `WEIXIN_GROUP_POLICY` is set to anything other than `disabled`.
:::
...
- Terminal QR rendering is included when Zed is installed with the `messaging` extra
...
5. Save the account credentials automatically to `~/.zed/weixin/accounts/`
...
After initial QR login, set at minimum the account ID in `~/.zed/.env`:
...
- **Long-poll transport** â€” no public endpoint, webhook, or WebSocket needed
- **QR code login** â€” scan-to-connect setup via `zed gateway setup`
- **DM messaging** â€” configurable access policies; group messaging depends on iLink actually delivering group events for the connected identity (often not the case for iLink bot accounts â€” see the warning above)
- **Media support** â€” images, video, files, and voice messages
- **AES-128-ECB encrypted CDN** â€” automatic encryption/decryption for all media transfers
- **Context token persistence** â€” disk-backed reply continuity across restarts
- **Markdown formatting** â€” preserves Markdown, including headers, tables, and code blocks, so WeChat clients that support Markdown can render it natively
- **Smart message chunking** â€” messages stay as a single bubble when under the limit; only oversized payloads split at logical boundaries
- **Typing indicators** â€” shows "typingâ€¦" status in the WeChat client while the agent processes
- **SSRF protection** â€” outbound media URLs are validated before download
- **Message deduplication** â€” 5-minute sliding window prevents double-processing
- **Automatic retry with backoff** â€” recovers from transient API errors
...
|-----|---------|-------------|
| `account_id` | â€” | iLink Bot account ID (required) |
| `token` | â€” | iLink Bot token (required, auto-saved from QR login) |
...
login connects one iLink bot identity to Zed. Other people do not scan the
Zed QR code with their own accounts; they must message the connected iLink
bot/contact through WeChat, and Zed will process the DM only if the sender's
Weixin user ID is present in `WEIXIN_ALLOWED_USERS`.

A practical setup flow is:

1. Pair Zed once with `zed gateway setup` and note the connected iLink bot
   account.
...
If only the account that scanned the QR code can talk to Zed, verify that the
...
Controls which groups the bot responds in **when iLink delivers group events for the connected identity**. For QR-login iLink bot identities (e.g. `...@im.bot`), group events are typically not delivered at all, so this policy may have no effect â€” see the iLink bot limitation warning at the top of the page.
...
- The AES key is 16 bytes (128-bit). Keys may arrive as raw base64 or hex-encoded â€” the adapter handles both formats.
- This requires the `cryptography` Python package.

No configuration is needed â€” encryption and decryption happen automatically.
...
- Tokens are saved per account+peer to `~/.zed/weixin/accounts/<account_id>.context-tokens.json`
...
| Condition | Behavior |
|-----------|----------|
| Transient error (1stâ€“2nd) | Retry after 2 seconds |
...
|----------|----------|---------|-------------|
| `WEIXIN_ACCOUNT_ID` | âœ… | â€” | iLink Bot account ID (from QR login) |
| `WEIXIN_TOKEN` | âœ… | â€” | iLink Bot token (auto-saved from QR login) |
| `WEIXIN_BASE_URL` | â€” | `https://ilinkai.weixin.qq.com` | iLink API base URL |
| `WEIXIN_CDN_BASE_URL` | â€” | `https://novac2c.cdn.weixin.qq.com/c2c` | CDN base URL for media transfer |
| `WEIXIN_DM_POLICY` | â€” | `open` | DM access policy: `open`, `allowlist`, `disabled`, `pairing` |
| `WEIXIN_GROUP_POLICY` | â€” | `disabled` | Group access policy: `open`, `allowlist`, `disabled` |
| `WEIXIN_ALLOWED_USERS` | â€” | _(empty)_ | Comma-separated user IDs for DM allowlist |
| `WEIXIN_GROUP_ALLOWED_USERS` | â€” | _(empty)_ | Comma-separated **group chat IDs** (not member user IDs) for group allowlist. The variable name is legacy â€” it expects group IDs, not user IDs. |
| `WEIXIN_HOME_CHANNEL` | â€” | â€” | Chat ID for cron/notification output |
| `WEIXIN_HOME_CHANNEL_NAME` | â€” | `Home` | Display name for the home channel |
| `WEIXIN_ALLOW_ALL_USERS` | â€” | â€” | Gateway-level flag to allow all users (used by setup wizard) |
...
| `Weixin startup failed: WEIXIN_TOKEN is required` | Run `zed gateway setup` to complete QR login, or set `WEIXIN_TOKEN` manually |
| `Weixin startup failed: WEIXIN_ACCOUNT_ID is required` | Set `WEIXIN_ACCOUNT_ID` in your `.env` or run `zed gateway setup` |
| `Another local Zed gateway is already using this Weixin token` | Stop the other gateway instance first â€” only one poller per token is allowed |
| Session expired (`errcode=-14`) | Your login session has expired. Re-run `zed gateway setup` to scan a new QR code |
| QR code expired during setup | The QR auto-refreshes up to 3 times. If it keeps expiring, check your network connection |
| Bot doesn't respond to DMs | Check `WEIXIN_DM_POLICY` â€” if set to `allowlist`, the sender must be in `WEIXIN_ALLOWED_USERS` |
| Bot ignores group messages | Group policy defaults to `disabled`. Set `WEIXIN_GROUP_POLICY=open` or `allowlist` â€” but note that QR-login iLink bot identities (`...@im.bot`) typically cannot receive ordinary WeChat group messages at all. If the gateway logs show no raw inbound events for group messages, the limitation is on the iLink side, not in Zed. |
...
| Terminal QR code doesn't render | Reinstall with the messaging extra: `pip install zed-agent[messaging]`. Alternatively, open the URL printed above the QR |
