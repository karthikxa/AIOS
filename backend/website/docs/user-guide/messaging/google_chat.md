...
sidebar_position: 12
title: "Google Chat"
description: "Set up Zed Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Zed Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Zed
process does not need a public URL, a tunnel, or a TLS certificate. It connects,
authenticates, and listens on a subscription â€” the same way a Telegram bot listens
on a token.

> Run `zed gateway setup` and pick **Google Chat** for a guided walk-through.
...
create it at [console.cloud.google.com](https://console.cloud.google.com) â€”
...
In the console, go to **APIs & Services â†’ Library** and enable:
...
- Skip the "Grant this service account access to project" step. IAM on the specific
  subscription is all you need â€” do **NOT** grant project-level Pub/Sub roles.

After creation, open the SA, go to **Keys â†’ Add Key â†’ Create new key â†’ JSON** and
download the file. Save it somewhere only Zed can read (e.g.,
`~/.zed/google-chat-sa.json`, `chmod 600`).
...
- Topic ID: `zed-chat-events`
- Leave the defaults for everything else.

After creation, the topic's detail page has a **Subscriptions** tab. Create one:

- Subscription ID: `zed-chat-events-sub`
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a zed restart)
...
- Principal: `zed-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription â€” Zed calls
...
Go to **APIs & Services â†’ Google Chat API â†’ Configuration**.

- **App name**: whatever you want users to see ("Zed" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/zed-chat-events`.
- **Visibility**: restrict to your workspace (or specific users) â€” do not publish
...
`ADDED_TO_SPACE` event that Zed uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Zed

Add the Google Chat section to `~/.zed/.env`:
...
# Authorization â€” paste the emails of people allowed to talk to the bot
...
GOOGLE_CHAT_MAX_BYTES=16777216                  # 16 MiB â€” cap on in-flight message bytes
```

The project ID also falls back to `GOOGLE_CLOUD_PROJECT`, and the SA path falls
back to `GOOGLE_APPLICATION_CREDENTIALS` â€” use whichever convention you prefer.

Install the dependencies the Google Chat adapter needs (no Zed extra is currently published â€” install them directly):
...
Send "hola" in the test DM. The bot posts a "Zed is thinkingâ€¦" marker, then
edits that same message in place with the real response â€” no "message deleted"
tombstones.
...
| Native file attachments (after `/setup-files` â€” see Step 10) | Native voice notes / circular video notes |

The agent's system prompt includes a Google Chatâ€“specific hint so it knows these
...
Thread support: when a user replies inside a thread, Zed detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Zed session.
...
for audio/video/documents. To deliver **native** Chat attachments â€” the same
file widget you get when a human drags-and-drops a file â€” each user authorizes
...
credentials. So the bot has to act *as a user* whenever it uploads a file â€”
specifically, as the user who asked for the file.

### One-time setup (per profile)

1. Go to **APIs & Services â†’ Credentials** in the same GCP project.
2. **Create credentials â†’ OAuth client ID â†’ Desktop app**.
3. Download the JSON. Move it onto the host that runs Zed.
4. Register the client with Zed (run under the profile you want it scoped to):
...
zed -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Zed home (e.g.
`~/.zed/google_chat_user_client_secret.json` for the default profile). The
client secret is **profile-scoped, not shared across profiles** â€” each profile
...
3. They open the URL, click **Allow**, and watch the browser fail to load
   `http://localhost:1/?...&code=...`. That failure is expected â€” the auth
...
The token lands at `~/.zed/google_chat_user_tokens/<sanitized_email>.json`.
...
`attachmentDataRef`. No Drive, no broader Chat scopes â€” this is least-privilege
on purpose.

### Multi-user behavior

When the asker has no per-user token yet, the bot falls back to a legacy
single-user token at `~/.zed/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Zed isn't authenticated â€” verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `zed gateway` logs for `[GoogleChat] Connected`. If you see
...
Check logs for `[GoogleChat] Pub/Sub stream died` â€” if these repeat, your SA
...
with exponential backoff â€” but you'll still see user-visible latency. Consider
...
- **Service Account scope**: the adapter requests `chat.bot` and `pubsub` scopes.
  IAM should be the actual enforcement â€” grant your SA the minimum
  (`roles/pubsub.subscriber` + `roles/pubsub.viewer` on the subscription), not
  project-level or org-level Pub/Sub roles.
- **Attachment download protection**: Zed will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  `chat.messages.create` â€” the minimum that covers `media.upload` plus the
  follow-up `messages.create`. Tokens are persisted as plain JSON at
  `~/.zed/google_chat_user_tokens/<sanitized_email>.json` (filesystem
  permissions are the protection â€” same model as the SA key file). Each
