...
1. Enable via `zed gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `zed webhook subscribe`
...
| `deliver_extra` | No | Additional delivery config â€” keys depend on `deliver` type (e.g. `repo`, `pr_number`, `chat_id`). Values support the same `{dot.notation}` templates as `prompt`. |
| `deliver_only` | No | If `true`, skip the agent entirely â€” the rendered `prompt` template becomes the literal message that gets delivered. Zero LLM cost, sub-second delivery. See [Direct Delivery Mode](#direct-delivery-mode) for use cases. Requires `deliver` to be a real target (not `log`). |
...
- `{__raw__}` â€” special token that dumps the **entire payload** as indented JSON (truncated at 4000 characters). Useful for monitoring alerts or generic webhooks where the agent needs the full context.
...
Add the `github-pr` route to your `~/.zed/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, Zed processes the event, and posts a review comment on the PR.
...
By default, every webhook POST triggers an agent run â€” the payload becomes a prompt, the agent processes it, and the agent's response is delivered. This costs LLM tokens on every event.

For use cases where you just want to **push a plain notification** â€” no reasoning, no agent loop, just deliver the message â€” set `deliver_only: true` on the route. The rendered `prompt` template becomes the literal message body, and the adapter dispatches it directly to the configured delivery target.

### When to use direct delivery

- **External service push** â€” Supabase/Firebase webhook fires on a database change â†’ notify a user in Telegram instantly
- **Monitoring alerts** â€” Datadog/Grafana alert webhook â†’ push to a Discord channel
- **Inter-agent pings** â€” Agent A notifies Agent B's user that a long-running task finished
- **Background job completion** â€” Cron job finishes â†’ post result to Slack

Benefits:

- **Zero LLM tokens** â€” the agent is never invoked
- **Sub-second delivery** â€” a single adapter call, no reasoning loop
- **Same security as agent mode** â€” HMAC auth, rate limits, idempotency, and body-size limits all still apply
- **Synchronous response** â€” the POST returns `200 OK` once delivery succeeds, or `502` if the target rejects it, so your upstream service can retry intelligently
...
- `deliver_only: true` requires `deliver` to be a real target. `deliver: log` (or omitting `deliver`) is rejected at startup â€” the adapter refuses to start if it finds a misconfigured route.
...
- Idempotency uses the same `X-GitHub-Delivery` / `X-Request-ID` header â€” retries with the same ID return `status=duplicate` and do NOT re-deliver.

---

## Dynamic Subscriptions (CLI) {#dynamic-subscriptions}

In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `zed webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.
...
- Subscriptions are stored in `~/.zed/webhook_subscriptions.json`
...
- No gateway restart required â€” subscribe and it's immediately live

### Agent-driven subscriptions

The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `zed webhook subscribe` command.
...
- **GitHub**: `X-Hub-Signature-256` header â€” HMAC-SHA256 hex digest prefixed with `sha256=`
- **GitLab**: `X-Gitlab-Token` header â€” plain secret string match
- **Generic**: `X-Webhook-Signature` header â€” raw HMAC-SHA256 hex digest

If a secret is configured but no recognized signature header is present, the request is rejected.

### Secret is required

Every route must have a secret â€” either set directly on the route or inherited from the global `secret`. Routes without a secret cause the adapter to fail at startup with an error. For development/testing only, you can set the secret to `"INSECURE_NO_AUTH"` to skip validation entirely.

`INSECURE_NO_AUTH` is only accepted when the gateway is bound to a loopback host (`127.0.0.1`, `localhost`, `::1`). If it is combined with a non-loopback bind such as `0.0.0.0` or a LAN IP, the adapter refuses to start â€” this prevents accidentally exposing an unauthenticated endpoint on a public interface.
...
:::warning
Webhook payloads contain attacker-controlled data â€” PR titles, commit messages, issue descriptions, etc. can all contain malicious instructions. Run the gateway in a sandboxed environment (Docker, VM) when exposed to the internet. Consider using the Docker or SSH terminal backend for isolation.
:::
...
- Check firewall rules â€” port `8644` (or your configured port) must be open
...
- For GitHub, the secret is HMAC-based â€” check `X-Hub-Signature-256`
- For GitLab, the secret is a plain token match â€” check `X-Gitlab-Token`
...
- Run the gateway in foreground to see logs: `zed gateway run`
- Check that the prompt template is rendering correctly
- Verify the delivery target is configured and connected

### Duplicate responses

- The idempotency cache should prevent this â€” check that the webhook source is sending a delivery ID header (`X-GitHub-Delivery` or `X-Request-ID`)
