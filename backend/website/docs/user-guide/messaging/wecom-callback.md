...
Connect Zed to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Zed supports two WeCom integration modes:
- **[WeCom Bot](wecom.md)** â€” bot-style, connects via WebSocket. Simpler setup, works in group chats.
- **WeCom Callback** (this page) â€” self-built app, receives encrypted XML callbacks. Shows as a first-class app in users' WeCom sidebar. Supports multi-corp routing.
:::

See also: [WeCom Bot](./wecom.md) for the bot-style integration.

> Run `zed gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Zed decrypts the message, queues it for the agent
4. Immediately acknowledges (silent â€” nothing displayed to the user)
5. The agent processes the request (typically 3â€“30 minutes)
...
1. Go to [WeCom Admin Console](https://work.weixin.qq.com/) â†’ **Applications** â†’ **Create App**
...
(Use `zed gateway start` only after `zed gateway install` has registered the systemd/launchd service.)
...
|---------|---------|-------------|
| `corp_id` | â€” | WeCom enterprise Corp ID (required) |
| `corp_secret` | â€” | Corp secret for the self-built app (required) |
| `agent_id` | â€” | Agent ID of the self-built app (required) |
| `token` | â€” | Callback verification token (required) |
| `encoding_aes_key` | â€” | 43-character AES key for callback encryption (required) |
...
- **No streaming** â€” replies arrive as complete messages after the agent finishes
- **No typing indicators** â€” the callback model doesn't support typing status
- **Text only** â€” currently supports text messages for input; image/file/voice input not yet implemented. The agent is aware of outbound media capabilities via the WeCom platform hint (images, documents, video, voice).
- **Response latency** â€” agent sessions take 3â€“30 minutes; users see the reply when processing completes

## Troubleshooting

**Signature verification failing.**
WeCom signs every request with the **Token** you registered in the admin
console. A mismatch between the token configured in Zed and the token the
admin console expects is the most common cause. Re-copy both the **Token** and
**EncodingAESKey** from the admin console â€” they're easy to truncate. Whitespace
in `~/.zed/.env` values around `=` will also break signature checks. After
fixing, restart `zed gateway run`.
...
3. From outside your network, `curl -i https://<your-domain>/wecom/callback`
   returns something other than a timeout (a 4xx without query params is fine â€”
   it just means the listener is reachable).

**Port not reachable / listener not bound.**
Check `zed gateway run` logs for the bound host/port. If the adapter bound to
`127.0.0.1` you must front it with a reverse proxy or tunnel â€” WeCom's servers
