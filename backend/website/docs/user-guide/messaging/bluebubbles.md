...
Connect Zed to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) â€” a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Zed and the BlueBubbles server

## Setup

### 1. Install BlueBubbles Server

Download and install from [bluebubbles.app](https://bluebubbles.app/). Complete the setup wizard â€” sign in with your Apple ID and configure a connection method (local network, Ngrok, Cloudflare, or Dynamic DNS).

### 2. Get your Server URL and Password

In BlueBubbles Server â†’ **Settings â†’ API**, note:
...
Or set environment variables directly in `~/.zed/.env`:
...
By default, Zed responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, Zed uses conservative defaults for `Zed` and `@Zed agent` variants.
...
When someone messages your iMessage, Zed automatically sends them a pairing code. Approve it with:
```bash
zed pairing approve bluebubbles <CODE>
```
Use `zed pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.zed/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.zed/.env`):
```bash
...
Zed will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.
...
- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling â€” instant delivery.
- **Outbound:** Zed sends messages via the BlueBubbles REST API.
...
|----------|----------|---------|-------------|
| `BLUEBUBBLES_SERVER_URL` | Yes | â€” | BlueBubbles server URL |
| `BLUEBUBBLES_PASSWORD` | Yes | â€” | Server password |
...
| `BLUEBUBBLES_HOME_CHANNEL` | No | â€” | Phone/email for cron delivery |
| `BLUEBUBBLES_ALLOWED_USERS` | No | â€” | Comma-separated authorized users |
...
| `BLUEBUBBLES_MENTION_PATTERNS` | No | Zed wake words | JSON array, newline-separated, or comma-separated regex patterns for group mention matching |

Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.zed/config.yaml` (default: `true`). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number â€” Zed resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check that the webhook is registered in BlueBubbles Server â†’ Settings â†’ API â†’ Webhooks
- Verify the webhook URL is reachable from the Mac
- Check `zed logs gateway` for webhook errors (or `zed logs -f` to follow in real-time)

### "Private API helper not connected"
- Install the Private API helper: [docs.bluebubbles.app](https://docs.bluebubbles.app/helper-bundle/installation)
- Basic messaging works without it â€” only reactions, typing, and read receipts require it
