...
sidebar_position: 4
title: "Slack"
description: "Set up Zed Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Zed Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Zed instance doesn't need to be publicly accessible â€” it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Zed uses the modern
...
The fastest path is to paste a manifest Zed generates for you. It
...
### Option A: From a Zed-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   zed slack manifest --write
   ```
   This writes `~/.zed/slack-manifest.json` and prints paste-in
   instructions.
...
4. Enter an app name (e.g., "Zed Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Zed can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `zed-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Zed configuration issue.
:::
...
Zed uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
zed gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
zed gateway              # Foreground
...
Every Zed command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Zed command with its description.

Under the hood: Zed ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/NousResearch/zed-agent/blob/main/zed_cli/commands.py)
...
When Zed adds new commands (e.g. after `zed update`), regenerate
...
3. Paste the new contents of `~/.zed/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/zed <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/zed btw run the tests` â€” Zed routes it the same way as `/btw
run the tests`. Free-form questions also work: `/zed what's the
...
Slack never delivers them to Zed.

As a workaround, Zed recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply â€”
Zed treats it identically to the slash form and replies in the same
thread.
...
Zed falls back to a text prompt, the prompt instructs you to reply
...
Understanding how Zed behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message â€” no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Zed Agent what time is it?`). In channels, Zed replies in a thread attached to that message. |
| **Threads** | If you @mention Zed inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** â€” the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.zed/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Zed in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Zed will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Zed will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Zed Agent`).

---

## Multi-Workspace Support

Zed can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Zed also loads tokens from an **OAuth token file** at:
...
- When a message arrives, Zed uses the correct workspace-specific client to respond.
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Zed Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Zed Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Zed now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
1. âœ… `message.channels` event is subscribed (for public channels)
2. âœ… `message.groups` event is subscribed (for private channels)
3. âœ… `app_mention` event is subscribed
4. âœ… `channels:history` scope is added (for public channels)
5. âœ… `groups:history` scope is added (for private channels)
6. âœ… App was **reinstalled** after adding scopes/events
7. âœ… Bot was **invited** to the channel (`/invite @Zed Agent`)
8. âœ… You are **@mentioning** the bot in your message
...
- Tokens should be stored in `~/.zed/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Zed config directory
