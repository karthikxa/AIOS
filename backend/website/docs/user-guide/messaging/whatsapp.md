...
sidebar_position: 5
title: "WhatsApp"
description: "Set up Zed Agent as a WhatsApp bot via the built-in Baileys bridge"
---

# WhatsApp Setup

Zed connects to WhatsApp through a built-in bridge based on **Baileys**. This works by emulating a WhatsApp Web session â€” **not** through the official WhatsApp Business API. No Meta developer account or Business verification is required.

> Run `zed gateway setup` and pick **WhatsApp** for a guided walk-through.
...
:::warning Unofficial API â€” Ban Risk
...
with third-party bridges. When this happens, Zed will update the bridge dependency. If the
bot stops working after a WhatsApp update, pull the latest Zed version and re-pair.
:::
...
| **Prepaid SIM** | $5â€“15 one-time | Any carrier. Activate, verify WhatsApp, then the SIM can sit in a drawer. Number must stay active (make a call every 90 days). |
| **VoIP services** | Freeâ€“$5/month | TextNow, TextFree, or similar. Some VoIP numbers are blocked by WhatsApp â€” try a few if the first doesn't work. |
...
3. Run `zed whatsapp` and scan the QR code from that WhatsApp account
...
# Access control â€” pick ONE of these options:
...
The Baileys bridge saves its session under `~/.zed/platforms/whatsapp/session`. This means:

- **Sessions survive restarts** â€” you don't need to re-scan the QR code every time
- The session data includes encryption keys and device credentials
- **Do not share or commit this session directory** â€” it grants full access to the WhatsApp account
...
Zed supports voice on WhatsApp:

- **Incoming:** Voice messages (`.ogg` opus) are automatically transcribed using the configured STT provider: local `faster-whisper`, Groq Whisper (`GROQ_API_KEY`), or OpenAI Whisper (`VOICE_TOOLS_OPENAI_KEY`)
- **Outgoing:** TTS responses are sent as MP3 audio file attachments
- Agent responses are prefixed with "âš• **Zed Agent**" by default. You can customize or disable this in `config.yaml`:
...
WhatsApp supports **streaming (progressive) responses** â€” the bot edits its message in real-time as the AI generates text, just like Discord and Telegram. Internally, WhatsApp is classified as a TIER_MEDIUM platform for delivery capabilities.

### Chunking

Long responses are automatically split into multiple messages at **4,096 characters** per chunk (WhatsApp's practical display limit). You don't need to configure anything â€” the gateway handles splitting and sends chunks sequentially.
...
| **QR code expires** | QR codes refresh every ~20 seconds. If it times out, restart `zed whatsapp`. |
| **Session not persisting** | Check that `~/.zed/platforms/whatsapp/session` exists and is writable. If containerized, mount it as a persistent volume. |
| **Logged out unexpectedly** | WhatsApp unlinks devices after long inactivity. Keep the phone on and connected to the network, then re-pair with `zed whatsapp` if needed. |
| **Bridge crashes or reconnect loops** | Restart the gateway, update Zed, and re-pair if the session was invalidated by a WhatsApp protocol change. |
| **Bot stops working after WhatsApp update** | Update Zed to get the latest bridge version, then re-pair. |
| **macOS: "Node.js not installed" but node works in terminal** | launchd services don't inherit your shell PATH. Run `zed gateway install` to re-snapshot your current PATH into the plist, then `zed gateway start`. See the [Gateway Service docs](./index.md#macos-launchd) for details. |
...
| **Bot replies to strangers with a pairing code** | Set `whatsapp.unauthorized_dm_behavior: ignore` in `~/.zed/config.yaml` if you want unauthorized DMs to be silently ignored instead. |
...
- The `~/.zed/platforms/whatsapp/session` directory contains full session credentials â€” protect it like a password
- Set file permissions: `chmod 700 ~/.zed/platforms/whatsapp/session`
- Use a **dedicated phone number** for the bot to isolate risk from your personal account
- If you suspect compromise, unlink the device from WhatsApp â†’ Settings â†’ Linked Devices
