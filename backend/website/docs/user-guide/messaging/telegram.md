...
sidebar_position: 1
title: "Telegram"
description: "Set up Zed Agent as a Telegram bot"
---

# Telegram Setup

Zed Agent integrates with Telegram as a full-featured conversational bot. Once connected, you can chat with your agent from any device, send voice memos that get auto-transcribed, receive scheduled task results, and use the agent in group chats. The integration is built on [python-telegram-bot](https://python-telegram-bot.org/) and supports text, voice, images, and file attachments.
...
2. Send `/newbot`
3. Choose a **display name** (e.g., "Zed Agent") â€” this can be anything
4. Choose a **username** â€” this must be unique and end in `bot` (e.g., `my_zed_bot`)
...
Telegram bots have no real online/offline presence dot â€” that green dot is a
...
Enable `status_indicator` and Zed sets that short description to **Online**
...
- Only a **clean** gateway shutdown (`/stop`, `disconnect`) writes "Offline".
  A hard crash leaves the last-known status â€” the inherent limitation of a
...
2. Send `/mybots`
3. Select your bot
4. Go to **Bot Settings â†’ Group Privacy â†’ Turn off**
...
Zed Agent uses numeric Telegram user IDs to control access. Your user ID is **not** your username â€” it's a number like `123456789`.

**Method 1 (recommended):** Message [@userinfobot](https://t.me/userinfobot) â€” it instantly replies with your user ID.

**Method 2:** Message [@get_id_bot](https://t.me/get_id_bot) â€” another reliable option.
...
| `TELEGRAM_WEBHOOK_SECRET` | **Yes** (when `TELEGRAM_WEBHOOK_URL` is set) | Secret token that Telegram echoes in every webhook request for verification. The gateway refuses to start without it â€” see [GHSA-3vpc-7q5r-276h](https://github.com/NousResearch/zed-agent/security/advisories/GHSA-3vpc-7q5r-276h). Generate with `openssl rand -hex 32`. |
| `TELEGRAM_WEBHOOK_PORT` | No | Local port the webhook server listens on (default: `8443`). |

When `TELEGRAM_WEBHOOK_URL` is set, the gateway starts an HTTP webhook server instead of polling. When unset, polling mode is used â€” no behavior change from previous versions.
...
You can also set it manually in `~/.zed/.env`:
...
If you have topic mode enabled in your bot DM, cron messages delivered to the root chat land in the system-only lobby â€” replying there opens no session and you see the "main chat is reserved for system commands" notice. Create a dedicated forum topic (e.g. `Cron`) and set:
...
Voice messages you send on Telegram are automatically transcribed by Zed's configured STT provider and injected as text into the conversation.

- `local` uses `faster-whisper` on the machine running Zed â€” no API key required
...
If you'd rather have the **agent itself** handle audio â€” for diarization, a custom transcription tool, or just archiving the recording â€” set `stt.enabled: false` in `~/.zed/config.yaml`:
...
With STT disabled, the gateway still downloads the voice/audio attachment into Zed's audio cache, but **does not transcribe it**. The agent receives the message with a marker like:
...
This pairs naturally with the [local Bot API server](#large-files-20mb-via-local-bot-api-server) section below, which lifts Telegram's 20MB getFile ceiling to 2GB â€” useful when the recordings you want to process are longer than a couple of minutes.

### Outgoing Voice (Text-to-Speech)

When the agent generates audio via TTS, it's delivered as native Telegram **voice bubbles** â€” the round, inline-playable kind.

- **OpenAI and ElevenLabs** produce Opus natively â€” no extra setup needed
...
Telegram's **public** Bot API caps `getFile` downloads at **20 MB**, so any voice note, audio file, video, or document larger than that is silently rejected by Zed with a "too large" reply. The documented way around this is to run a **local** [telegram-bot-api](https://github.com/tdlib/telegram-bot-api) daemon â€” the same server software Telegram uses, but running on your network. A local server raises the file ceiling to **2 GB** and Zed auto-lifts its own internal cap when it sees a custom `base_url` configured.
...
3. Copy the `api_id` and `api_hash` â€” both are required.
...
services:
  tg-bot-api:
    ...
    environment:
      ...
      TELEGRAM_LOCAL: "1"                # enable --local mode (raises 20MB â†’ 2GB)
    volumes:
...
:::warning Security
The local Bot API server takes your bot token in the URL path (e.g. `/bot<TOKEN>/getMe`) with **no additional auth**. Anyone who can reach the port can fully control your bot â€” read every message it can see, send messages as it, etc. Bind the container to `127.0.0.1` and/or front it with a reverse proxy on a private network. **Never expose port 8081 to the public internet.**
:::
...
This is a one-shot migration step â€” you don't repeat it on every restart. Telegram delivers any messages received after `logOut` through the new server instead.
...
### Step 4: Point Zed at the local server

Add the URLs under `platforms.telegram.extra` in `~/.zed/config.yaml`:
...
platforms:
  telegram:
    extra:
      ...
      local_mode: true        # see Step 5 below â€” only set this if the bot's data
                              # directory is readable by the Zed process
```
...
When `base_url` is set, Zed:

- Builds the python-telegram-bot client against the local server
- Auto-lifts its internal document/audio size cap from 20 MB â†’ 2 GB
...
### Step 5: `local_mode` â€” file access on disk
...
2. **With `--local`** (set via `TELEGRAM_LOCAL=1` above): files are written to the server's filesystem and the `getFile` response returns an **absolute path** instead of an HTTP URL. The 20MB ceiling is lifted. Zed must then read the bytes **from disk**, not over HTTP.

To make the disk-read path work, set `local_mode: true` in the config above **and** make sure the Zed process can read the path the server returns. Two scenarios:

- **Same machine** â€” telegram-bot-api and Zed run on the same host. Bind-mount the data volume to a directory that Zed can read (e.g., `/var/lib/telegram-bot-api`), and make sure the file ownership matches. The container drops privileges to its internal `telegram-bot-api` user (uid varies by image); the simplest fix is to add `user: "<UID>:<GID>"` to the compose service so files are owned by a uid Zed already runs as.
- **Different machines** â€” the bot server runs on one host (e.g., a NAS, a separate VM) and Zed on another. The server's data directory must be shared with the Zed machine at the **same absolute path** the server reports (typically `/var/lib/telegram-bot-api`). NFS works well for this; CIFS/SMB with `uid=` mount remapping is friendlier if you don't want to deal with uid mismatches at the filesystem level.

If `local_mode: true` is set but Zed can't `stat` the returned file path (permissions or wrong mount), python-telegram-bot silently falls back to an HTTP `getFile` against the local server â€” which in `--local` mode responds with `404 Not Found`. The symptom shows up in `gateway.log` as:
...
If you see that, the cap-lift is working but the file-share isn't. Verify `ls -la /var/lib/telegram-bot-api/<TOKEN>/voice/` from the Zed host as the user the gateway runs as, and confirm a single file is `cat`-able without a permission error.
...
You should see a `[Telegram] Cached user voice at /home/<user>/.zed/cache/audio/...` line and **no** "too large" rejection. Combined with `stt.enabled: false` (above), the path to the original audio file then lands in the agent's inbound message for downstream processing.

## Group Chat Usage

Zed Agent works in Telegram group chats with a few considerations:

- **Privacy mode** determines what messages the bot can see (see [Step 3](#step-3-privacy-mode-critical-for-groups))
- `TELEGRAM_ALLOWED_USERS` still applies â€” only authorized users can trigger the bot, even in groups
...
- In groups with multiple Zed bots, `telegram.exclusive_bot_mentions` keeps routing deterministic. When a message explicitly mentions one or more Telegram bot usernames, only the mentioned bot profiles process it; other Zed bots ignore it before reply and wake-word fallbacks run. This is enabled by default.
- Use `telegram.ignored_threads` to keep Zed silent in specific Telegram forum topics, even when the group would otherwise allow free responses or mention-triggered replies
- If `telegram.require_mention` is left unset or false, Zed keeps the previous open-group behavior and responds to normal group messages it can see

### Multiple Zed bots in one group

If you run several Zed profiles in the same Telegram group, create one Telegram bot token per profile and start one gateway per profile. Do not reuse the same bot token in multiple running gateways; Telegram will reject concurrent polling for the same token.
...
With this setup, a group message like `@research_bot @ops_bot summarize this` is processed by `research_bot` and `ops_bot` only. Other Zed bots in the group stay silent, even if the message is a reply to one of their earlier messages or would otherwise match a shared wake word.
...
For a small fixed fleet, use a shell loop or script that calls `zed gateway <action>` for the default profile and `zed -p <profile> gateway <action>` for each named profile. This is more reliable than assuming a single process-level command controls every named profile on every service manager.
...
1. **Telegram delivery:** turn off BotFather privacy mode, promote the bot to
   admin, or mention the bot directly. Zed cannot respond to group messages
...
3. **Zed authorization:** make sure the sender is listed in
...
5. **Multi-bot routing:** if a group contains several bots, make sure each
   Zed profile uses a unique bot token and keep `exclusive_bot_mentions`
...
Telegram Bot API 9.4 (February 2026) introduced **Private Chat Topics** â€” bots can create forum-style topic threads directly in 1-on-1 DM chats, no supergroup needed. This lets you run multiple isolated workspaces within your existing DM with Zed.

### Use case

If you work on several long-running projects, topics keep their context separate:

- **Topic "Website"** â€” work on your production web service
- **Topic "Research"** â€” literature review and paper exploration
- **Topic "General"** â€” miscellaneous tasks and quick questions

Each topic gets its own conversation session, history, and context â€” completely isolated from the others.
...
1. Open your private chat with the Zed bot in Telegram
2. Tap the bot's name at the top to open chat info
3. Enable **Topics** (the toggle to turn the chat into a forum)

Without this, Zed will log `The chat is not a forum` on startup and skip topic creation. This is a Telegram client-side setting â€” the bot cannot enable it programmatically.
:::

Add topics under `platforms.telegram.extra.dm_topics` in `~/.zed/config.yaml`:
...
| `thread_id` | No | Auto-populated after topic creation â€” don't set manually |

### How it works

1. On gateway startup, Zed calls `createForumTopic` for each topic that doesn't have a `thread_id` yet
2. The `thread_id` is saved back to `config.yaml` automatically â€” subsequent restarts skip the API call
...
normally. Set `ignore_root_dm: true` to turn the root DM into a lobby â€” normal
...
Topics with a `skill` field automatically load that skill when a new session starts in the topic. This works exactly like typing `/skill-name` at the start of a conversation â€” the skill content is injected into the first message, and subsequent messages see it in the conversation history.
...
:::tip
Topics created outside of the config (e.g., by manually calling the Telegram API) are discovered automatically when a `forum_topic_created` service message arrives. You can also add topics to the config while the gateway is running â€” they'll be picked up on the next cache miss.
:::

## Multi-session DM mode (`/topic`)

A ChatGPT-style multi-session DM â€” one bot, many parallel conversations. Unlike the operator-curated `extra.dm_topics` above, this mode is **user-driven**: no config, no pre-declared topic names. The end user flips it on with `/topic`, then taps the Telegram **+** button to create as many topics as they want, each one a fully independent Zed session.
...
| Topic names | Chosen by operator | Chosen by user; auto-renamed to match Zed session title |
...
Both features can coexist on the same bot â€” you'd run `/topic` from a user's DM, and `extra.dm_topics` continues to manage operator-declared topics for other chats.

### Prerequisites

In **@BotFather**, open your bot â†’ **Bot Settings â†’ Threads Settings**:
...
When the user first runs `/topic`, Zed calls `getMe` to verify both flags. If either is off, Zed sends a screenshot of the BotFather Threads Settings page and explains what to toggle â€” no activation happens until prerequisites are met.
...
4. Zed responds inside that topic â€” the topic is now a standalone session

Every topic gets its own conversation history, model state, tool execution, and session ID. The isolation key is `agent:main:telegram:dm:{chat_id}:{thread_id}` â€” identical to the config-driven DM topics isolation.

### Auto-renamed topics

When Zed generates a session title for a topic (via the auto-title pipeline, after the first exchange), the Telegram topic itself is renamed to match â€” e.g. "New Topic" becomes "Database migration plan". The rename is best-effort: failures are logged but don't break the session.
...
When this flag is on, Zed still generates an internal session title (used by `zed sessions`, the TUI, etc.) but never edits the Telegram topic name. Useful when you organise topics by hand under BotFather Threaded Mode and don't want every first reply to overwrite the title.

### `/new` inside a topic

Resets the current topic's session (new session ID, fresh history) without touching other topics. Zed replies with a reminder that for parallel work, creating another topic (via **All Messages**) is usually what you want.
...
This binds the current topic to an existing Zed session instead of starting fresh. Useful for continuing a conversation that started before topic mode was enabled. Restrictions:

- The target session must belong to the same Telegram user
- The target session must not already be bound to another topic

Zed confirms with the session title and replays the last assistant message for context.

To discover session IDs, send `/topic` (no argument) in the root DM â€” Zed lists the user's unlinked Telegram sessions.
...
- Each topic binding persists to `telegram_dm_topic_bindings(chat_id, thread_id, session_id, ...)` with `ON DELETE CASCADE` on `session_id` â€” pruning a session automatically clears its topic binding
...
- Topics declared in `extra.dm_topics` are **never auto-renamed** â€” the operator-chosen name is preserved even when multi-session mode is enabled
...
- Root-lobby reminders are rate-limited to one message per 30 seconds per chat â€” a user who forgets topic mode is on and types ten prompts in the root won't get ten replies
- BotFather setup screenshots are rate-limited to one send per 5 minutes per chat â€” repeated `/topic` attempts while Threads Settings are still disabled won't re-upload the same image
...
- `/topic` itself is gated by the bot's user authorization check â€” unauthorized DMs get a refusal instead of activation

### Disabling multi-session mode

Send `/topic off` in the root DM. Zed flips the row off, clears the chat's `(thread_id â†’ session_id)` bindings, and the root DM reverts to a normal Zed chat. Existing topics in Telegram aren't deleted â€” they just stop being gated as independent sessions. Re-run `/topic` later to turn it back on.
...
If you downgrade to a Zed version that predates `/topic`, the feature simply stops working â€” the `telegram_dm_topic_mode` and `telegram_dm_topic_bindings` tables remain in `state.db` but are ignored by older code. DMs revert to the native per-thread isolation (each `message_thread_id` still gets its own session via `build_session_key`), so your existing Telegram topics keep working as parallel sessions. The root DM is no longer a lobby â€” messages there go into the agent like they used to. Re-upgrading reactivates multi-session mode exactly where it was.

## Group Forum Topic Skill Binding

Supergroups with **Topics mode** enabled (also called "forum topics") already get session isolation per topic â€” each `thread_id` maps to its own conversation. But you may want to **auto-load a skill** when messages arrive in a specific group topic, just like DM topic skill binding works.

### Use case

A team supergroup with forum topics for different workstreams:

- **Engineering** topic â†’ auto-loads the `software-development` skill
- **Research** topic â†’ auto-loads the `arxiv` skill
- **General** topic â†’ no skill, general-purpose assistant

### Configuration

Add topic bindings under `platforms.telegram.extra.group_topics` in `~/.zed/config.yaml`:
...
| `thread_id` | Yes | Telegram forum topic ID â€” visible in `t.me/c/<group_id>/<thread_id>` links |
| `skill` | No | Skill to auto-load on new sessions in this topic |

### How it works

1. When a message arrives in a mapped group topic, Zed looks up the `chat_id` and `thread_id` in `group_topics` config
2. If a matching entry has a `skill` field, that skill is auto-loaded for the session â€” identical to DM topic skill binding
3. Topics without a `skill` key get session isolation only (existing behavior, unchanged)
4. Unmapped `thread_id` values or `chat_id` values fall through silently â€” no error, no skill
...
| Topic creation | Zed creates topics via API if `thread_id` is missing | Admin creates topics in Telegram UI |
...
| Skill binding | âœ“ | âœ“ |
| Session isolation | âœ“ | âœ“ (already built-in for forum topics) |

:::tip
To find a topic's `thread_id`, open the topic in Telegram Web or Desktop and look at the URL: `https://t.me/c/1234567890/5` â€” the last number (`5`) is the `thread_id`. The `chat_id` for supergroups is the group ID prefixed with `-100` (e.g., group `1234567890` becomes `-1001234567890`).
:::

## Recent Bot API Features

- **Bot API 9.4 (Feb 2026):** Private Chat Topics â€” bots can create forum topics in 1-on-1 DM chats via `createForumTopic`. Zed uses this for two distinct features: operator-curated [Private Chat Topics](#private-chat-topics-bot-api-94) (config-driven, fixed topic list) and user-driven [Multi-session DM mode](#multi-session-dm-mode-topic) (activated by `/topic`, unlimited user-created topics).
- **Privacy policy:** Telegram now requires bots to have a privacy policy. Set one via BotFather with `/setprivacy_policy`, or Telegram may auto-generate a placeholder. This is particularly important if your bot is public-facing.
- **Bot API 9.5 (Mar 2026): Native streaming via `sendMessageDraft`.** Zed supports Telegram's native streaming-draft API as an opt-in transport for private chats. The default remains the legacy `editMessageText` path because draft previews can visibly collapse and re-render on some Telegram clients.

### Streaming transport (`gateway.streaming.transport`)

When streaming is enabled (`gateway.streaming.enabled: true`), Zed picks one of four transports:
...
**What you'll see in DMs with `edit` (default)** â€” the gateway sends a normal preview message and progressively updates it via `editMessageText`, avoiding Telegram's draft-preview collapse/rollback effect.

**What you'll see in DMs with `auto` or `draft`** â€” Telegram shows an animated draft preview that updates token-by-token. When the reply finishes, it's delivered as a regular message and the draft preview clears naturally on the client. Drafts have no message id, so the final answer is what stays in your chat history.

**What about groups, supergroups, forum topics?** Telegram restricts `sendMessageDraft` to private chats (DMs). The gateway transparently falls back to the edit-based path for everything else â€” same UX as before.
...
When enabled, Zed attaches Telegram's `LinkPreviewOptions(is_disabled=True)` to every outgoing message and falls back to the legacy `disable_web_page_preview` parameter on older `python-telegram-bot` versions.

## Group Allowlisting

Telegram groups and forum chats have two orthogonal gates you can configure:

- **Sender user IDs** (`group_allow_from` / `TELEGRAM_GROUP_ALLOWED_USERS`) â€” sender-scoped allowlist that applies only to group/forum messages. Use this when you want specific users to be able to invoke the bot in groups without adding them to `TELEGRAM_ALLOWED_USERS` (which would also give them DM access).
- **Chat IDs** (`group_allowed_chats` / `TELEGRAM_GROUP_ALLOWED_CHATS`) â€” chat-scoped allowlist. Any member of these groups/forums can interact with the bot. Useful for team/support bots where group membership itself is the access signal.
  ...
  platforms:
    telegram:
      extra:
        ...
        group_allow_from:
          - "987654321"
        # Entire groups/forums â€” any member is authorized.
        group_allowed_chats:
          - "-1001234567890"
```
...
For more casual setups â€” friend group chats where you want the bot **mostly silent** but **occasionally available on explicit ping** â€” enable `guest_mode`:
...
With `guest_mode: true`, a message from a non-allowlisted group is processed **only** if it explicitly @mentions the bot. The mention is required every turn â€” there's no session stickiness for guest interactions, so the bot never auto-engages in a friend group thread it isn't pinged into.
  ...
  platforms:
    telegram:
      extra:
        ...
        # NEW â€” admins get all slash commands (built-in + plugin)
        allow_admin_from:
          - "123456789"

        # NEW â€” non-admin allowed users can only run these slash commands.
...
- A user listed in `allow_admin_from` for a scope (DM or group) can run **every** registered slash command â€” built-in commands AND plugin-registered ones â€” through the live registry.
...
When you send `/model` with no arguments in a Telegram chat, Zed shows an interactive inline keyboard for switching models:

1. **Provider selection** â€” buttons showing each available provider with model counts (e.g., "OpenAI (15)", "âœ“ Anthropic (12)" for the current provider).
2. **Model selection** â€” paginated model list with **Prev**/**Next** navigation, a **Back** button to return to providers, and **Cancel**.
...
5. Once a fallback IP succeeds, it becomes "sticky" â€” subsequent requests use it directly without retrying the primary path first.
...
Or add it to `~/.zed/.env`:

```bash
HTTPS_PROXY=http://proxy.example.com:8080
```

The proxy applies to both the primary transport and all fallback IP transports. No additional Zed configuration is needed â€” if the environment variable is set, it's used automatically.

:::note
This covers the custom fallback transport layer that Zed uses for Telegram connections. The standard `httpx` client used elsewhere already respects proxy env vars natively.
:::
...
- ðŸ‘€ when the bot starts processing your message
- âœ… when the response is delivered successfully
- âŒ if an error occurs during processing
...
:::note
Unlike Discord (where reactions are additive), Telegram's Bot API replaces all bot reactions in a single call. The transition from ðŸ‘€ to âœ…/âŒ happens atomically â€” you won't see both at once.
:::
...
Assign ephemeral system prompts to specific Telegram groups or forum topics. The prompt is injected at runtime on every turn â€” never persisted to transcript history â€” so changes take effect immediately.
...
- Message in topic `42` inside group `-1001234567890` â†’ uses topic `42`'s prompt
- Message in topic `99` (no explicit entry) â†’ falls back to group `-1001234567890`'s prompt
- Message in a group with no entry â†’ no channel prompt applied
...
| Problem | Solution |
|---------|----------|
| Bot not responding at all | Verify `TELEGRAM_BOT_TOKEN` is correct. Check `zed gateway` logs for errors. |
...
| Voice messages not transcribed | Verify STT is available: install `faster-whisper` for local transcription, or set `GROQ_API_KEY` / `VOICE_TOOLS_OPENAI_KEY` in `~/.zed/.env`. |
...
When the agent calls the `clarify` tool â€” to ask which approach you prefer, get post-task feedback, or check before a non-trivial decision â€” Telegram renders the question with **inline keyboard buttons**:
...
Configure the response timeout via `agent.clarify_timeout` in `~/.zed/config.yaml` (default `600` seconds). If you don't respond within the timeout, the agent unblocks with a sentinel message and adapts rather than hanging.
...
The Telegram adapter routes recurring agent status callbacks (e.g. "Compressing contextâ€¦", "Calling toolâ€¦") through `send_or_update_status()`, which keeps a `{(chat_id, status_key) â†’ message_id}` cache and **edits the existing bubble** on subsequent emits instead of appending a new one each time. Distinct `status_key` values get their own messages; distinct chats never collide. If the edit fails (e.g. the user deleted the message, or it's older than Telegram allows for edits), the cache entry is dropped and the next emit posts a fresh message and re-caches its ID. No config required â€” this is the default Telegram behavior. Other adapters that don't implement `send_or_update_status` fall through to plain `send()` unchanged.

## Pin incoming user message during agent turn

When a user sends a message that triggers an agent turn, the Telegram adapter pins that incoming message for the duration of the turn and unpins it when the response is finished â€” a lightweight visual indicator that the bot is actively working on the message rather than ignoring it. The pin uses `disable_notification=true` to avoid extra pings. No config required.
