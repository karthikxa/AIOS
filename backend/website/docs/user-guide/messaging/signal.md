...
sidebar_position: 6
title: "Signal"
description: "Set up Zed Agent as a Signal messenger bot via signal-cli daemon"
---

# Signal Setup

Zed connects to Signal through the [signal-cli](https://github.com/AsamK/signal-cli) daemon running in HTTP mode. The adapter streams messages in real-time via SSE (Server-Sent Events) and sends responses via JSON-RPC.
...
The Signal adapter uses `httpx` (already a core Zed dependency) for all communication. No additional Python packages are required. You just need signal-cli installed externally.
:::
...
DM access follows the same pattern as all other Zed platforms:

1. **`SIGNAL_ALLOWED_USERS` set** â†’ only those users can message
2. **No allowlist set** â†’ unknown users get a DM pairing code (approve via `zed pairing approve signal CODE`)
3. **`SIGNAL_ALLOW_ALL_USERS=true`** â†’ anyone can message (use with caution)
...
**Incoming** (user â†’ agent):

- **Images** â€” PNG, JPEG, GIF, WebP (auto-detected via magic bytes)
- **Audio** â€” MP3, OGG, WAV, M4A (voice messages transcribed if Whisper is configured)
- **Documents** â€” PDF, ZIP, and other file types

**Outgoing** (agent â†’ user):

The agent can send media files via `MEDIA:` tags in responses. The following delivery methods are supported:

- **Images** â€” `send_multiple_images` and `send_image_file` send PNG, JPEG, GIF, WebP as native Signal attachments
- **Voice** â€” `send_voice` sends audio files (OGG, MP3, WAV, M4A, AAC) as attachments
- **Video** â€” `send_video` sends MP4 video files
- **Documents** â€” `send_document` sends any file type (PDF, ZIP, etc.)
...
**Reply quotes.** When Zed replies to a specific message, it now posts a native reply that quotes the original â€” same UI affordance Signal users see when they use "Reply" themselves. This is automatic for replies generated in response to an inbound message.
...
None of this requires additional config â€” it ships on by default in recent signal-cli builds. If your `signal-cli` version is too old, Zed falls back to plaintext delivery and logs a one-time warning.
...
Signal does not support editing already-sent messages. Zed therefore suppresses gateway tool-progress bubbles on Signal, even when `/verbose` is enabled and saves a non-`off` mode for the platform.
...
- This applies to both Zed gateway logs and the global redaction system

### Note to Self (Single-Number Setup)

If you run signal-cli as a **linked secondary device** on your own phone number (rather than a separate bot number), you can interact with Zed through Signal's "Note to Self" feature.

Just send a message to yourself from your phone â€” signal-cli picks it up and Zed responds in the same conversation.
...
- Echo-back protection (sent-timestamp tracking) prevents infinite loops â€” the bot's own replies are filtered out automatically
...
| `SIGNAL_GROUP_ALLOWED_USERS` | No | â€” | Group IDs to monitor, or `*` for all (omit to disable groups) |
