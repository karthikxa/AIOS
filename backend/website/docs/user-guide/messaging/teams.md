...
description: "Set up Zed Agent as a Microsoft Teams bot"
---

# Microsoft Teams Setup

Connect Zed Agent to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint â€” either a dev tunnel (local dev) or a real domain (production).
...
> Run `zed gateway setup` and pick **Microsoft Teams** for a guided walk-through.
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which Zed strips automatically before processing.
...
The `@microsoft/teams.cli` automates bot registration â€” no Azure portal needed.
...
Teams cannot deliver messages to `localhost`. For local development, use any tunnel tool to get a public HTTPS URL. The default port is `3978` â€” change it with `TEAMS_PORT` if needed.

```bash
# devtunnel (Microsoft)
devtunnel create zed-bot --allow-anonymous
devtunnel port create zed-bot -p 3978 --protocol https  # replace 3978 with TEAMS_PORT if changed
...
Copy the `https://` URL from the output â€” you'll use it in the next step. Leave the tunnel running while developing.
...
The CLI outputs your `CLIENT_ID`, `CLIENT_SECRET`, and `TENANT_ID`, plus an install link for Step 6. Save the client secret â€” it won't be shown again.
...
Open the printed link in your browser â€” it opens directly in the Teams client. After installing, send a direct message to your bot â€” it's ready.
...
Alternatively, configure via `~/.zed/config.yaml`:
...
- **Allow Once** â€” approve this specific command
- **Allow Session** â€” approve this pattern for the rest of the session
- **Always Allow** â€” permanently approve this pattern
...
When the [Teams meeting pipeline plugin](/user-guide/messaging/msgraph-webhook) is enabled, this adapter also handles outbound delivery of meeting summaries â€” one Teams integration surface, not two. After a meeting's transcript is summarized, the writer posts the summary into your chosen Teams target.
...
platforms:
  teams:
    ...
    extra:
      ...
      # For delivery_mode: graph â€” pick ONE of:
...
If the `teams_pipeline` plugin is **not** enabled, these settings are inert â€” they only wire up when the pipeline runtime binds to the Graph webhook ingress.
...
Make sure your configured port (`TEAMS_PORT`, default `3978`) is reachable from the internet and that your TLS certificate is valid â€” Teams rejects self-signed certificates.
...
| `KeyError: 'teams'` in logs | Restart the container â€” this is fixed in the current version |
| Bot responds with auth errors | Verify `TEAMS_CLIENT_ID`, `TEAMS_CLIENT_SECRET`, and `TEAMS_TENANT_ID` are all set correctly |
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.zed/.env` |
...
| Tunnel URL changes on restart | devtunnel URLs are persistent if you use a named tunnel (`devtunnel create zed-bot`). ngrok and cloudflared generate a new URL each run unless you have a paid plan â€” update the bot endpoint with `teams app update` when it changes |
| Teams shows "This bot is not responding" | The webhook returned an error. Check `docker logs zed` for tracebacks |
...
Treat `TEAMS_CLIENT_SECRET` like a password â€” rotate it periodically via the Azure portal or Teams CLI.
:::

- Store credentials in `~/.zed/.env` with permissions `600` (`chmod 600 ~/.zed/.env`)
- The bot only accepts messages from users in `TEAMS_ALLOWED_USERS`; unauthorized messages are silently dropped
- Your public endpoint (`/api/messages`) is authenticated by the Teams Bot Framework â€” requests without valid JWTs are rejected
