...
sidebar_position: 9
title: "Matrix"
description: "Set up Zed Agent as a Matrix bot"
---

# Matrix Setup

Zed Agent integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org â€” either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the Zed Agent pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

Zed works with any Matrix homeserver â€” Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how Zed behaves once it's connected.

## How Zed Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | Zed responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, Zed requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | Zed supports Matrix threads (MSC3440). If you reply in a thread, Zed keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, Zed auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. Set `MATRIX_DM_AUTO_THREAD=true` (default false) to also auto-create threads for DM messages â€” this is distinct from `MATRIX_DM_MENTION_THREADS`, which only starts a thread when the bot is `@mentioned` in a DM. |
| **Commands** | Zed accepts normal `/commands` when your Matrix client sends them. If your client reserves `/` for local commands, use `!commands` instead; Zed normalizes known `!command` aliases to `/command`. |
...
| **Shared rooms with multiple users** | By default, Zed isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
MATRIX_REACTIONS=true          # default: true â€” emoji reactions during processing
MATRIX_ALLOW_ROOM_MENTIONS=false
```

:::tip Disabling reactions
`MATRIX_REACTIONS=false` turns off the processing-lifecycle emoji reactions (ðŸ‘€/âœ…/âŒ) the bot posts on inbound messages. Useful for rooms where reaction events are noisy or aren't supported by all participating clients.
:::

:::tip Room-wide mentions
Zed sends structured Matrix user mentions for explicit Matrix IDs such as `@alice:example.org`. Room-wide `@room` notifications are disabled by default; set `MATRIX_ALLOW_ROOM_MENTIONS=true` only in rooms where the bot is allowed to notify everyone.
:::
...
Zed now includes the current Matrix room name, room ID, topic, message ID,
...
This guide walks you through the full setup process â€” from creating your bot account to sending your first message.
...
2. Choose a username like `zed` â€” the full user ID will be `@zed:your-server.org`.

### Option B: Use matrix.org or Another Public Homeserver

1. Go to [Element Web](https://app.element.io) and create a new account.
2. Pick a username for your bot (e.g., `zed-bot`).

### Option C: Use Your Own Account

You can also run Zed as your own user. This means the bot posts as you â€” useful for personal assistants.

## Step 2: Get an Access Token

Zed needs an access token to authenticate with the homeserver. You have two options:
...
3. Scroll down and expand **Advanced** â€” the access token is displayed there.
...
The response includes an `access_token` field â€” copy it.
...
Instead of providing an access token, you can give Zed the bot's user ID and password. Zed will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.
...
Zed Agent uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
Bridge and appservice deployments need extra loop protection. Zed always
...
Optional behavior settings in `~/.zed/config.yaml`:
...
The bot should connect to your homeserver and start syncing within a few seconds. Send it a message â€” either a DM or in a room it has joined â€” to test.
...
Zed supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
When E2EE is enabled, Zed:

- Stores encryption keys in `~/.zed/platforms/matrix/store/` (legacy installs: `~/.zed/matrix/store/`)
...
In Matrix conversations, Zed exposes Matrix-specific tools to the agent:
...
Set `MATRIX_APPROVAL_REQUIRE_SENDER=false` if you intentionally want any authorized Matrix user in the room to operate an approval/model picker prompt. The default is requester-bound when Zed knows who requested the action.

### Media Limits

Zed uploads and downloads Matrix images, files, audio, and video through Matrix media APIs. Multiple generated images are sent as one ordered logical batch, preserving captions and thread context across the batch.
...
Inbound media must use Matrix `mxc://` content URIs. Zed rejects arbitrary
...
Zed includes an opt-in Synapse harness for local validation:

```bash
docker compose -f tests/e2e/matrix_synapse_gateway/docker-compose.yml up -d
ZED_MATRIX_SYNAPSE_INTEGRATION=1 \
...
**Where to find it:** In Element, go to **Settings** â†’ **Security & Privacy** â†’ **Encryption** â†’ your recovery key (also called the "Security Key"). This is the key you were asked to save when you first set up cross-signing.

On each startup, if `MATRIX_RECOVERY_KEY` is set, Zed imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

If Zed bootstraps a new Matrix recovery key, it never logs the raw key. Set
...
If you delete `~/.zed/platforms/matrix/store/crypto.db`, the bot loses its encryption identity. Simply restarting with the same device ID will **not** fully recover â€” the homeserver still holds one-time keys signed with the old identity key, and peers cannot establish new Olm sessions.

Zed detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
**Manual recovery** (advanced â€” keeps the same device ID):
...
2. Delete the local crypto store and restart Zed:
   ```bash
...
Restrict the bot to a fixed set of Matrix rooms. When set, the bot **only** responds in rooms whose ID appears in the list â€” messages from any other room are silently ignored, even if the bot is mentioned.
...
- Empty / unset â†’ no restriction (default).
- Non-empty â†’ room ID must be on the list. The check runs **before** any other gating (mention requirement, sender allowlist, etc.).
- Use the room's **internal ID** (`!abc...:server`), not its alias (`#room:server`). You can find a room's internal ID in Element via Room â†’ Settings â†’ Advanced.

See also: [admin/user slash command split](../../reference/slash-commands.md#permissions-and-adminuser-split).


:::tip
To find a Room ID: in Element, go to the room â†’ **Settings** â†’ **Advanced** â†’ the **Internal room ID** is shown there (starts with `!`).
:::

## Commands in Matrix

Zed supports the same gateway commands in Matrix that it supports on other
...
Zed only normalizes `!command` when the command is known to the gateway, a
...
**Fix**: Invite the bot to the room â€” it auto-joins on invite. Verify your User ID is in `MATRIX_ALLOWED_USERS` (use the full `@user:server` format) and the room ID is in `MATRIX_ALLOWED_ROOMS` if that allowlist is configured. In rooms, mention the bot or add the room to `MATRIX_FREE_RESPONSE_ROOMS`. Restart the gateway.

### Bot joins rooms but silently drops every message (clock skew)

**Cause**: The host's system clock is set ahead of real time. The Matrix adapter applies a 5-second startup-grace filter (`event_ts < startup_ts - 5`) to ignore events replayed from initial sync. When the wall clock is ahead, every incoming event looks "older than startup" and is dropped before reaching the message handler â€” the bot appears connected but never replies. See [#12614](https://github.com/NousResearch/zed-agent/issues/12614).
...
**Fix**: Verify `MATRIX_HOMESERVER` points to your homeserver (include `https://`, no trailing slash). Check that `MATRIX_ACCESS_TOKEN` is valid â€” try it with curl:
...
:::tip
If you also manually deleted `crypto.db`, see the "Deleting the crypto store" warning in the E2EE section above â€” there are additional steps to clear stale one-time keys from the homeserver.
:::

If you previously used Zed with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.zed/.env`.
...
3. **Set your recovery key** (if you use cross-signing â€” most Element users do). Add to `~/.zed/.env`:

   ```bash
   MATRIX_RECOVERY_KEY=EsT... your recovery key here
   ```

   This lets the bot self-sign with cross-signing keys on startup, so Element trusts the new device immediately. Without this, Element may see the new device as unverified and refuse to share encryption sessions. Find your recovery key in Element under **Settings** â†’ **Security & Privacy** â†’ **Encryption**.
...
a working E2EE setup with a previous version of Zed and are upgrading.
...
macOS (Host):
  â””â”€ zed gateway
       â”œâ”€ api_server adapter â† listens on 0.0.0.0:8642
       â”œâ”€ AIAgent â† single source of truth
       â”œâ”€ Sessions, memory, skills
       â””â”€ Local file access (Obsidian, projects, etc.)

Linux VM (Docker):
  â””â”€ zed gateway (proxy mode)
       â”œâ”€ Matrix adapter â† E2EE decryption/encryption
       â””â”€ HTTP forward â†’ macOS:8642/v1/chat/completions
           (no LLM API keys, no agent, no inference)
```

The Docker container only handles Matrix protocol + E2EE. When a message arrives, it decrypts it and forwards the text to the host via a standard HTTP request. The host runs the agent, calls tools, generates a response, and streams it back. The container encrypts and sends the response to Matrix. All sessions are unified â€” CLI, Matrix, Telegram, and any other platform share the same memory and conversation history.
...
services:
  zed-matrix:
    ...
    environment:
      ...
      # Proxy mode â€” forward to host agent
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote Zed API server (e.g., `http://192.168.1.100:8642`) |
...
Proxy mode is not limited to Matrix. Any platform adapter can use it â€” set `GATEWAY_PROXY_URL` on any gateway instance and it will forward to the remote agent instead of running one locally. This is useful for any deployment where the platform adapter needs to run in a different environment from the agent (network isolation, E2EE requirements, resource constraints).

:::tip
Session continuity is maintained via the `X-Zed-Session-Id` header. The host's API server tracks sessions by this ID, so conversations persist across messages just like they would with a local agent.
:::

:::note
**Limitations (v1):** Tool progress messages from the remote agent are not relayed back â€” the user sees the streamed final response only, not individual tool calls. Dangerous command approval prompts are handled on the host side, not relayed to the Matrix user. These can be addressed in future updates.
:::
...
**Fix**: Zed uses an explicit sync loop that calls `client.handle_sync()` on
both the initial sync and every incremental sync response. This matches the
diagnosis in upstream issue #7914 and closed PR #37807, but keeps Zed's own
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the Zed logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The Zed gateway isn't running, or it failed to connect.

**Fix**: Check that `zed gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.zed/.env` and restart the gateway. Use the full `@user:server` format.
