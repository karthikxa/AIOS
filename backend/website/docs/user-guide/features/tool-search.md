...
context window on every turn â€” even when only a few of them are relevant
to what the user actually asked for.

**Tool Search** is Zed' opt-in progressive-disclosure layer for that
...
:::info Built-in Zed tools never defer
The tools that make up Zed' core capability set (`terminal`,
`read_file`, `write_file`, `patch`, `search_files`, `todo`, `memory`,
`browser_*`, `web_search`, `web_extract`, `clarify`, `execute_code`,
`delegate_task`, `session_search`, `send_message`, and the rest of
`_ZED_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
```
tool_search(query, limit?)     â€” search the deferred-tool catalog
tool_describe(name)            â€” load the full schema for one tool
tool_call(name, arguments)     â€” invoke a deferred tool
```
...
Model: tool_search("create a github issue")
  â†’ { matches: [{ name: "mcp_github_create_issue", ... }, ...] }
Model: tool_describe("mcp_github_create_issue")
  â†’ { parameters: { type: "object", properties: { ... } } }
Model: tool_call("mcp_github_create_issue", { title: "...", body: "..." })
  â†’ { ok: true, issue_number: 42 }
```

When the model invokes `tool_call`, Zed **unwraps the bridge** and
...
post-tool-call hooks all run against the real tool name â€” not against
...
tools:
  tool_search:
    ...
    threshold_pct: 10   # percentage of context â€” only used in auto mode
...
| `threshold_pct` | `10` | Percentage of context length at which `auto` mode kicks in. Range 0â€“100. |
| `search_default_limit` | `5` | Hits returned when the model calls `tool_search` without a `limit`. |
| `max_search_limit` | `20` | Hard upper bound the model can request via `limit`. Range 1â€“50. |
...
schemas, ~300 tokens) and at least one extra round trip (search â†’
describe â†’ call) for the savings on the deferred schemas. It's a clear
...
These come from the prompt-cache integrity invariant â€” they are inherent
...
- **Model-quality dependence.** Tool Search assumes the model can write a
  ...
  less well; the published Anthropic numbers (49% â†’ 74% on Opus 4 with
...
- **Catalog is stateless across turns.** It rebuilds from the current
  tool-defs list every assembly â€” no session-keyed `Map`. This avoids
...
- **The catalog is scoped to the session's toolsets.** `tool_search`,
  ...
  discover or call a tool outside that subset â€” the deferred catalog is
  the deferrable slice of the session's own enabled/disabled toolsets,
  not the whole process registry.
- **No JS sandbox.** Zed uses the simpler "structured tools" mode
...
- `tools/tool_search.py` â€” the implementation
- `tests/tools/test_tool_search.py` â€” the regression suite
