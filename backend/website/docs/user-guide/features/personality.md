...
description: "Customize Zed Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Zed Agent's personality is fully customizable. `SOUL.md` is the **primary identity** â€” it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` â€” a durable persona file that lives in `ZED_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets â€” session-level system-prompt overlays

If you want to change who Zed is â€” or replace it with an entirely different agent persona â€” edit `SOUL.md`.

## How SOUL.md works now

Zed now seeds a default `SOUL.md` automatically in:

```text
~/.zed/SOUL.md
```

More precisely, it uses the current instance's `ZED_HOME`, so if you run Zed with a custom home directory, it will use:
...
- Zed creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Zed loads `SOUL.md` only from `ZED_HOME`
- Zed does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Zed falls back to a built-in default identity
- If `SOUL.md` has content, that content is injected verbatim after security scanning and truncation
- SOUL.md is **not** duplicated in the context files section â€” it appears only once, as the identity
...
If Zed loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `ZED_HOME`, the personality belongs to the Zed instance itself.

That also makes it easier to teach users:
- "Edit `~/.zed/SOUL.md` to change Zed' default personality."
...
- how Zed should handle uncertainty, disagreement, or ambiguity
...
## What Zed injects into the prompt

`SOUL.md` content goes directly into slot #1 of the system prompt â€” the agent identity position. No wrapper language is added around it.

The content goes through:
- prompt-injection scanning
- truncation if it is too large

If the file is empty, whitespace-only, or cannot be read, Zed falls back to a built-in default identity ("You are Zed Agent, an intelligent AI assistant created by Zed Team..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Zed ships with built-in personalities you can switch to with `/personality`.
...
| **kawaii** | Cute expressions, sparkles, and enthusiasm â˜… |
| **catgirl** | Neko-chan with cat-like expressions, nya~ |
| **pirate** | Captain Zed, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Zed its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.zed/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.zed/SOUL.md`
...
1. **SOUL.md** (agent identity â€” or built-in fallback if SOUL.md is unavailable)
...
`SOUL.md` is the foundation â€” everything else builds on top of it.
...
- [SOUL.md Guide](/guides/use-soul-with-zed)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Zed speaks
- `display.skin` and `/skin` affect how Zed looks in the terminal
