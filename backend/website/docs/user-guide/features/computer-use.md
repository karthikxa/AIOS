...
Zed Agent can drive your Mac's desktop â€” clicking, typing, scrolling,
dragging â€” in the **background**. Your cursor doesn't move, keyboard focus
...
model** â€” Claude, GPT, Gemini, or an open model on a local vLLM endpoint.
...
- Post synthesized events directly to target processes â€” no HID event tap,
  no cursor warp.
- Flip AppKit active-state without raising windows â€” no Space switching.
...
Pick whichever path is most convenient â€” both run the same upstream installer:
...
Use `zed computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `zed tools`, pick `ðŸ–±ï¸ Computer Use (macOS)` â†’ `cua-driver (background)`.
2. The setup runs the upstream installer (same as Option 1).

After installing, regardless of which path you took:

3. Grant macOS permissions when prompted:
   - **System Settings â†’ Privacy & Security â†’ Accessibility** â†’ allow the
     terminal (or Zed app).
   - **System Settings â†’ Privacy & Security â†’ Screen Recording** â†’ allow
     the same.
4. Start a session with the toolset enabled:
   ```
   zed -t computer_use chat
   ```
   or add `computer_use` to your enabled toolsets in `~/.zed/config.yaml`.

## Keeping cua-driver up to date

The cua-driver project ships fixes regularly (e.g. v0.1.6 fixed a Safari
window-focus bug for UTM workflows). Zed refreshes the binary in two
places so you don't get stuck on a stale release:

- **`zed update`** â€” when you update Zed itself, if `cua-driver` is
...
- **`zed computer-use install --upgrade`** â€” manual force-refresh.
...
`zed computer-use status` shows the installed version next to the
binary path.
...
1. `computer_use(action="capture", mode="som", app="Mail")` â€” gets a
   screenshot of Mail with every sidebar item, toolbar button, and message
   row numbered.
2. `computer_use(action="click", element=14)` â€” clicks the search field
   (element #14 from the capture).
3. `computer_use(action="type", text="from:stripe")`
4. `computer_use(action="key", keys="return", capture_after=True)` â€” submit
...
|---|---|---|---|
| Anthropic (Claude Sonnet/Opus 3+) | âœ… | âœ… | Best overall; SOM + raw coordinates. |
| OpenRouter (any vision model) | âœ… | âœ… | Multi-part tool messages supported. |
| OpenAI (GPT-4+, GPT-5) | âœ… | âœ… | Same as above. |
| Local vLLM / LM Studio (vision model) | âœ… | âœ… | If the model supports multi-part tool content. |
| Text-only models | âŒ | âœ… (degraded) | Use `mode="ax"` for accessibility-tree-only operation. |
...
Zed applies multi-layer guardrails:

- Destructive actions (click, type, drag, scroll, key, focus_app) require
  approval â€” either interactively via the CLI dialog or via the
...
Pair with `approvals.mode: manual` in `~/.zed/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Zed applies four layers of optimisation:

- **Screenshot eviction** â€” the Anthropic adapter keeps only the 3 most
  recent screenshots in context; older ones become `[screenshot removed
  to save context]` placeholders.
- **Client-side compression pruning** â€” the context compressor detects
  multimodal tool results and strips image parts from old ones.
- **Image-aware token estimation** â€” each image is counted as ~1500 tokens
  (Anthropic's flat rate) instead of its base64 char length.
- **Server-side context editing (Anthropic only)** â€” when active, the
  adapter enables `clear_tool_uses_20250919` via `context_management` so
  Anthropic's API clears old tool results server-side.

A 20-action session on a 1568Ã—900 display typically costs ~30K tokens
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `ZED_CUA_DRIVER_VERSION`
  env var if you want reproducibility across a macOS bump.
- **Performance.** Background mode is slower than foreground â€”
...
**`computer_use backend unavailable: cua-driver is not installed`** â€” Run
`zed computer-use install` to fetch the cua-driver binary, or run
`zed tools` and enable the Computer Use toolset.

**Clicks seem to have no effect** â€” Capture and verify. A modal you
didn't see may be blocking input. Dismiss it with `escape` or the close
button.

**Element indices are stale** â€” SOM indices are only valid until the
next `capture`. Re-capture after any state-changing action.

**"blocked pattern in type text"** â€” The text you tried to `type`
matches the dangerous-shell-pattern list. Break the command up or
reconsider.

## See also

- [Universal skill: `macos-computer-use`](https://github.com/NousResearch/zed-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
