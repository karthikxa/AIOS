...
description: "Generate agent trajectories at scale â€” parallel processing, checkpointing, and toolset distributions"
---

# Batch Processing

Batch processing lets you run the Zed agent across hundreds or thousands of prompts in parallel, generating structured trajectory data. This is primarily used for **training data generation** â€” producing ShareGPT-format trajectories with tool usage statistics that can be used for fine-tuning or evaluation.
...
Batch runs spin up many concurrent agent sessions, each making model calls and tool calls. A [Nous Portal](/user-guide/features/tool-gateway) subscription bundles model access plus web search, image gen, TTS, and cloud browsers under one bill â€” useful when you want stable cost-per-trajectory without juggling rate limits across five vendor accounts. Set up with `zed setup --portal`, then point `--model` at a Nous model.
:::
...
```text
data/my_run/
â”œâ”€â”€ trajectories.jsonl    # Combined final output (all batches merged)
â”œâ”€â”€ batch_0.jsonl         # Individual batch results
â”œâ”€â”€ batch_1.jsonl
â”œâ”€â”€ ...
â”œâ”€â”€ checkpoint.json       # Resume checkpoint
â””â”€â”€ statistics.json       # Aggregate tool usage stats
```
...
- **Failed prompts:** Only successfully completed prompts are marked as done â€” failed prompts will be retried on resume
