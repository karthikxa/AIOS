...
Schedule tasks to run automatically with natural language or cron expressions. Zed exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
- run in **no-agent mode** â€” a script on a schedule, its stdout delivered verbatim, zero LLM involvement (see the [no-agent mode](#no-agent-mode-script-only-jobs) section below)
...
:::warning
Cron-run sessions cannot recursively create more cron jobs. Zed disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
Zed will use the unified `cronjob` tool internally.
...
Cron jobs default to running detached from any repo â€” no `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` is loaded, and the terminal / file / code-exec tools run from whatever working directory the gateway started in. Pass `--workdir` (CLI) or `workdir=` (tool call) to change that:
...
- The path must be an absolute directory that exists â€” relative paths and missing directories are rejected at create / update time
...
:::tip Job reference
The `<job_id>` placeholder below (and in [Lifecycle actions](#lifecycle-actions)) also accepts the job's name (case-insensitive) â€” handy when you remember `morning-digest` but not the hex ID. An exact job ID takes precedence over name matches; if the reference is not an ID and a name matches more than one job, the command refuses and prints the candidate IDs so you can disambiguate.
:::
...
```bash
zed cron list
zed cron pause <job_id_or_name>
zed cron resume <job_id_or_name>
zed cron run <job_id_or_name>
zed cron remove <job_id_or_name>
zed cron edit <job_id_or_name> [...flags]
zed cron status
zed cron tick
```
...
sudo zed gateway install --system   # Linux: boot-time system service for servers
zed gateway             # Or run in foreground
...
1. loads jobs from `~/.zed/cron/jobs.json`
...
A file lock at `~/.zed/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.zed/cron/output/`) | Default on CLI |
...
When Telegram topic mode is enabled, the root DM is reserved as a system lobby â€” replies sent there are rebuffed with a lobby reminder and `reply_to_message_id` is dropped, so you cannot reply to a cron message that landed in the main chat.

Point cron at a dedicated forum topic instead:

1. In Telegram, open the bot DM and create a topic named e.g. `Cron`. Long-press the topic header â†’ **Copy link**; the trailing integer is the topic's `message_thread_id`.
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.zed/cron/output/`), but no message is sent to the delivery target.
...
Failed jobs always deliver regardless of the `[SILENT]` marker â€” only successful runs can be silenced. For quiet monitoring jobs, prompt the agent to reply with only `[SILENT]` when there is nothing to report.
...
Or set the `ZED_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var â†’ config.yaml â†’ 120s default.

## No-agent mode (script-only jobs)

For recurring jobs that don't need LLM reasoning â€” classic watchdogs, disk/memory alerts, heartbeats, CI pings â€” pass `no_agent=True` at creation time. The scheduler runs your script on schedule and delivers its stdout directly, skipping the agent entirely:
...
- Script stdout (trimmed) â†’ delivered verbatim as the message.
- **Empty stdout â†’ silent tick**, no delivery. This is the watchdog pattern: "only say something when something is wrong".
- Non-zero exit or timeout â†’ an error alert is delivered, so a broken watchdog can't fail silently.
- `{"wakeAgent": false}` on the last line â†’ silent tick (same gate LLM jobs use).
- No tokens, no model, no provider fallback â€” the job never touches the inference layer.

`.sh` / `.bash` files run under `/bin/bash`; anything else under the current Python interpreter (`sys.executable`). Scripts must live in `~/.zed/scripts/` (same sandboxing rule as the pre-run script gate).

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to Zed directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Zed will write the check script to `~/.zed/scripts/` via `write_file`, then call:
...
It picks `no_agent=True` automatically when the message content is fully determined by the script (watchdogs, threshold alerts, heartbeats). The same tool also lets the agent pause, resume, edit, and remove jobs â€” so the whole lifecycle is chat-driven without anyone touching the CLI.

See the [Script-Only Cron Jobs guide](/guides/cron-script-only) for worked examples.

## Chaining jobs with `context_from`

Cron jobs run in isolated sessions with no memory of previous runs. But sometimes one job's output is exactly what the next job needs. The `context_from` parameter wires that connection automatically â€” Job B's prompt gets Job A's most recent output prepended as context at runtime.
...
# Job 2: Triage â€” receives Job 1's output as context
...
# Job 3: Ship â€” receives Job 2's output as context
cronjob(
    action="create",
...
- When Job 2 fires, Zed reads Job 1's most recent output from `~/.zed/cron/output/{job1_id}/*.md`
- That output is prepended to Job 2's prompt automatically
- Job 2 doesn't need to hardcode "read this file" â€” it receives the content as context
- The chain can be any length: Job 1 â†’ Job 2 â†’ Job 3 â†’ ...
...
- Multi-stage pipelines (collect â†’ filter â†’ format â†’ deliver)
- Dependent tasks where step N's work depends on step Nâˆ’1's output
...
The agent's final response is automatically delivered â€” you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Zed skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `zed tools`** â€” not the CLI default, not everything under the sun.

```bash
zed tools
# â†’ pick the "cron" platform in the curses UI
# â†’ toggle toolsets on/off just like you would for Telegram/Discord/etc.
```
...
When `enabled_toolsets` is set on a job it wins; otherwise the `zed tools` cron-platform config wins; otherwise Zed falls back to the built-in defaults. This matters for cost control: carrying `moa`, `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Zed should even invoke the agent. Emit a final stdout line of the form:

```text
{"wakeAgent": false}
```

â€¦and cron skips the agent run entirely for this tick. Useful for frequent polls (every 1â€“5 min) that only need to wake the LLM when state actually changed â€” otherwise you pay for zero-content agent turns over and over.
...
**File-change gate** â€” only run when a watched file has new content since the last successful tick. The scheduler records each job's `last_run_at`; compare it against the file's mtime.
...
**External-flag gate** â€” only run when some other process has signalled readiness (e.g. a deploy hook drops a file, a CI job sets a value in your state store).
...
**SQL-count gate** â€” only run when there are new rows to process in your own database. The script can also pass the count through to the agent via `context`, so the agent knows how much it's looking at without re-querying.
...
The same pattern works for any data source you can query from a script â€” Postgres, an HTTP API, your own state store â€” without baking a SQL evaluator into the cron subsystem.
...
The referenced jobs' most recent completed outputs are injected above the prompt as context for this run. Each upstream entry must be a valid job ID or name (see `cronjob action="list"`). Note: chaining reads the *most recent completed* output â€” it does not wait for upstream jobs that are running in the same tick.

## Job storage

Jobs are stored in `~/.zed/cron/jobs.json`. Output from job runs is saved to `~/.zed/cron/output/{job_id}/{timestamp}.md`.

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Zed resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
