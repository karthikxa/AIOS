...
description: "External memory provider plugins â€” Honcho, OpenViking, Mem0, Hindsight, Holographic, RetainDB, ByteRover, Supermemory"
---

# Memory Providers

Zed Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time â€” the built-in memory is always active alongside it.

## Quick Start

```bash
zed memory setup      # interactive picker + configuration
zed memory status     # check what's active
zed memory off        # disable external provider
```

You can also select the active memory provider via `zed plugins` â†’ Provider Plugins â†’ Memory Provider.

Or set manually in `~/.zed/config.yaml`:
...
When a memory provider is active, Zed automatically:
...
**Tools (5):** `honcho_profile` (read/update peer card), `honcho_search` (semantic search), `honcho_context` (session context â€” summary, representation, card, messages), `honcho_reasoning` (LLM-synthesized), `honcho_conclude` (create/delete conclusions)

**Architecture:** Two-layer context injection â€” a base layer (session summary + representation + peer card, refreshed on `contextCadence`) plus a dialectic supplement (LLM reasoning, refreshed on `dialecticCadence`). The dialectic automatically selects cold-start prompts (general user facts) vs. warm prompts (session-scoped context) based on whether base context exists.

**Three orthogonal config knobs** control cost and depth independently:

- `contextCadence` â€” how often the base layer refreshes (API call frequency)
- `dialecticCadence` â€” how often the dialectic LLM fires (LLM call frequency)
- `dialecticDepth` â€” how many `.chat()` passes per dialectic invocation (1â€“3, depth of reasoning)

**Setup Wizard:**
```bash
zed memory setup        # select "honcho" â€” runs the Honcho-specific post-setup
```
...
| `dialecticCadence` | `2` | Minimum turns between `peer.chat()` LLM calls. Recommended 1â€“5. Only applies to `hybrid`/`context` modes |
| `dialecticDepth` | `1` | Number of `.chat()` passes per dialectic invocation. Clamped 1â€“3. Pass 0: cold/warm prompt, pass 1: self-audit, pass 2: reconciliation |
...
:::tip Migrating from `zed honcho`
If you previously used `zed honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages â€” one user peer plus one AI peer per Zed profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Zed profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Zed profile. Host key `zed` â†’ default; `zed.<profile>` for others. |
...
`--clone` creates a `zed.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Zed profile, creates host blocks for any profile without one, inherits settings from the default `zed` block, and creates the new AI peers eagerly. Idempotent â€” skips profiles that already have a host block.
...
- **`"directional"`** (default) â€” all four flags on. Full mutual observation; enables cross-peer dialectic.
- **`"unified"`** â€” user `observeMe: true`, AI `observeOthers: true`, rest false. Single-observer pool; AI models the user but not itself, user peer only self-models.

Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults â€” synced back at session init.
...
| Key | Effect |
|-----|--------|
| `pinUserPeer: true` | Every non-agent gateway user collapses to `peerName`. The pin is checked first, so it overrides all aliases â€” pick it only when no user-side identity needs its own peer |
| `userPeerAliases` | Maps specific runtime IDs to peers (`{"7654321": "alice"}`). The home for routing distinct identities â€” including agents that each carry their own peer |
| `runtimePeerPrefix` | Namespaces any unmapped runtime ID (`telegram_7654321`) so platforms with same-shaped IDs don't collide |

Off-gateway these keys do nothing. `zed memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
# Then configure Zed
zed memory setup    # select "openviking"
# Or manually:
...
echo "OPENVIKING_API_KEY=..." >> ~/.zed/.env
```

**Key features:**
- Tiered context loading: L0 (~100 tokens) â†’ L1 (~2k) â†’ L2 (full)
...
`OPENVIKING_AGENT` is Zed' peer ID in OpenViking for peer-scoped memories.
...
| | |
|---|---|
| **Best for** | Hands-off memory management â€” Mem0 handles extraction automatically |
...
**Config:** `$ZED_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `zed-user` | User identifier |
...
**Setup:**
```bash
zed memory setup    # select "hindsight"
# Or manually:
...
**Local mode UI:** `hindsight-embed -p zed ui start`

**Config:** `$ZED_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `zed` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Zed Agent and the User` | Context label for retained memories |
| `retain_tags` | â€” | Default tags applied to retained memories; merged with per-call tool tags |
| `retain_source` | â€” | Optional `metadata.source` attached to retained memories |
...
**Setup:**
```bash
zed memory setup    # select "holographic"
# Or manually:
zed config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.zed-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ZED_HOME/memory_store.db` | SQLite database path |
| `auto_extract` | `false` | Auto-extract facts at session end |
| `default_trust` | `0.5` | Default trust score (0.0â€“1.0) |

**Unique capabilities:**
- `probe` â€” entity-specific algebraic recall (all facts about a person/thing)
- `reason` â€” compositional AND queries across multiple entities
- `contradict` â€” automated detection of conflicting facts
...
**Setup:**
```bash
zed memory setup    # select "retaindb"
# Or manually:
...
Persistent memory via the `brv` CLI â€” hierarchical knowledge tree with tiered retrieval (fuzzy text â†’ LLM-driven search). Local-first with optional cloud sync.
...
# Then configure Zed
zed memory setup    # select "byterover"
# Or manually:
...
- Knowledge tree stored at `$ZED_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
zed memory setup    # select "supermemory"
# Or manually:
zed config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.zed/.env
```

**Config:** `$ZED_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `zed` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
**Key features:**
- Automatic context fencing â€” strips recalled memories from captured turns to prevent recursive memory pollution
- Full-session ingest â€” the entire conversation is sent once at session boundaries
- Session-end conversation ingest (to `/v4/conversations`) for richer profile + graph building in Supermemory
- Profile facts injected on first turn and at configurable intervals
- **Profile-scoped containers** â€” use `{identity}` in `container_tag` (e.g. `zed-{identity}` â†’ `zed-coder`) to isolate memories per Zed profile
- **Multi-container mode** â€” enable `enable_custom_container_tags` with a `custom_containers` list to let the agent read/write across named containers. Automatic operations stay on the primary container.
...
**Support:** [Discord](https://supermemory.link/discord) Â· [support@supermemory.com](mailto:support@supermemory.com)
...
| **Requires** | `pip install zed-memori` + `zed-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install zed-memori
zed-memori install
zed config set memory.provider memori
zed memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `zed-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$ZED_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$ZED_HOME/` so each profile has its own credentials
