...
Zed Agent includes a full browser automation toolset with multiple backend options:
...
- **Local Chromium-family CDP** â€” connect browser tools to your own Chrome, Brave, Chromium, or Edge instance using `/browser connect`
...
- **Local Chromium-family integration** â€” attach to your running Chrome, Brave, Chromium, or Edge browser via CDP for hands-on browsing
...
If you have a paid [Zed Portal](https://portal.zedteam.com) subscription, you can use browser automation through the **[Tool Gateway](tool-gateway.md)** without any separate API keys. New installs can run `zed setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Nous Subscription** as the browser provider via `zed model` or `zed tools`.
:::
...
When a cloud provider is configured, Zed auto-spawns a **local Chromium sidecar**
...
This solves the common "I'm developing locally but using Browserbase" workflow â€”
...
`browser.allow_private_urls: true` (which lets the cloud provider attempt them â€”
...
mode, so you need it installed (`zed setup tools â†’ Browser Automation`
...
Or configure via `zed tools` â†’ Browser Automation â†’ Camofox.
...
By default, each Camofox session gets a random identity â€” cookies and logins don't survive across agent restarts. To enable persistent browser sessions, add the following to `~/.zed/config.yaml`:
...
Then fully restart Zed so the new config is picked up.

:::warning Nested path matters
Zed reads `browser.camofox.managed_persistence`, **not** a top-level `managed_persistence`. A common mistake is writing:
...
If the flag is placed at the wrong path, Zed silently falls back to a random ephemeral `userId` and your login state will be lost on every session.
:::
...
- Scopes the `userId` to the active Zed profile, so different Zed profiles get different browser profiles (profile isolation).

##### What Zed does not do
- It does not force persistence on the Camofox server. Zed only sends a stable `userId`; the server must honor it by mapping that `userId` to a persistent Firefox profile directory.
- If your Camofox server build treats every request as ephemeral (e.g. always calls `browser.newContext()` without loading a stored profile), Zed cannot make those sessions persist. Make sure you are running a Camofox build that implements userId-based profile persistence.

##### Verify it's working

1. Start Zed and your Camofox server.
...
If step 5 logs you out, the Camofox server isn't honoring the stable `userId`. Double-check your config path, confirm you fully restarted Zed after editing `config.yaml`, and verify your Camofox server version supports persistent per-user profiles.

##### Where state lives

Zed derives the stable `userId` from the profile-scoped directory `~/.zed/browser_auth/camofox/` (or the equivalent under `$ZED_HOME` for non-default profiles). The actual browser profile data lives on the Camofox server side, keyed by that `userId`. To fully reset a persistent profile, clear it on the Camofox server and remove the corresponding Zed profile's state directory.

#### Externally managed Camofox sessions

When another app drives the visible Camofox browser (a desktop assistant, a custom integration, another agent), configure Zed to operate inside that same identity instead of spawning its own isolated profile.

Three knobs control the behavior:

| Setting | Env var | Effect |
|---------|---------|--------|
| `browser.camofox.user_id` | `CAMOFOX_USER_ID` | Camofox `userId` Zed uses when creating tabs. Setting this opts the session into "externally managed" mode. |
...
| `browser.camofox.adopt_existing_tab` | `CAMOFOX_ADOPT_EXISTING_TAB` | When true, Zed calls `GET /tabs?userId=<user_id>` on first use and reuses an existing tab before creating a new one. |
...
- Zed skips destructive cleanup at task end (same as `managed_persistence: true`). The other app's tab/cookies/profile survive.
- Zed does **not** call `DELETE /sessions/<user_id>` â€” that endpoint wipes all user data, so it would nuke the external app's session if it fired.

**How tab adoption works (when `adopt_existing_tab: true`):**

1. On the first browser tool call after a process start, Zed issues `GET /tabs?userId=<user_id>` (5-second timeout).
2. If any tab in the response has `listItemId == session_key`, Zed adopts the most recently created one in that group.
3. Otherwise, Zed adopts the most recently created tab for the user (any `listItemId`).
4. If no tabs exist or the request fails, Zed falls back to creating a new tab on the next operation.

Adoption only fires until `tab_id` is populated for the session. If the external app closes the adopted tab mid-run, the next browser tool call will surface a Camofox error â€” Zed does not re-poll for a fresh tab on every call.

**Picking `session_key`:** if you want Zed to reliably attach to a *specific* existing tab, set `session_key` to the `listItemId` the external app used when creating it. If you leave `session_key` unset and only set `user_id`, Zed generates a per-task `session_key` (`task_<id>`) â€” Zed will share cookies and the profile with the external app, but will open its own tab alongside instead of reusing one.

**Concurrency note:** the external app and Zed can drive the same Camofox `userId` simultaneously, but Camofox does not coordinate per-tab focus between clients. Coordinate ownership at the application layer (e.g. the external app pauses while Zed runs).

#### VNC live view

When Camofox runs in headed mode (with a visible browser window), it exposes a VNC port in its health check response. Zed automatically discovers this and includes the VNC URL in navigation responses, so the agent can share a link for you to watch the browser live.

### Local Chromium-family browser via CDP (`/browser connect`)

Instead of a cloud provider, you can attach Zed browser tools to your own running Chrome, Brave, Chromium, or Edge instance via the Chrome DevTools Protocol (CDP). This is useful when you want to see what the agent is doing in real-time, interact with pages that require your own cookies/sessions, or avoid cloud browser costs.

:::note
`/browser connect` is an **interactive-CLI slash command** â€” it is not dispatched by the gateway. If you try to run it inside a WebUI, Telegram, Discord, or other gateway chat, the message will be sent to the agent as plain text and the command will not execute. Start Zed from the terminal (`zed` or `zed chat`) and issue `/browser connect` there.
:::
...
If Zed runs inside WSL2 but the Chrome window you want to control runs on the Windows host, `/browser connect` is often not the best path.

Why:

- `/browser connect` expects Zed itself to reach a usable CDP endpoint
...
- even when Windows Chrome is debuggable, the cleanest integration is often to let a Windows-side browser MCP server attach to Chrome and let Zed talk to that MCP server

For that setup, prefer `chrome-devtools-mcp` through Zed MCP support.
...
If you do **not** set any cloud credentials and don't use `/browser connect`, Zed can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `zed config set toolsets '["zed-cli", "browser"]'`.
:::
...
Screenshots are stored in `~/.zed/cache/screenshots/` and automatically cleaned up after 24 hours.
...
`browser_console` also evaluates JavaScript when called with an `expression` argument â€” same shape as DevTools console, the result comes back parsed (JSON-serialized objects become dicts; primitive values stay primitive).
...
When a CDP supervisor is active for the current session (typical for any session that's run `browser_navigate` against a CDP-capable backend), evaluation runs over the supervisor's persistent WebSocket â€” no subprocess startup cost. Falls through to the standard agent-browser CLI path otherwise. Behaviour is identical either way; only latency changes.
...
**Only available when a CDP endpoint is reachable at session start** â€” meaning `/browser connect` has attached to a running Chrome, Brave, Chromium, or Edge browser, or `browser.cdp_url` is set in `config.yaml`. The default local agent-browser mode, Camofox, and cloud providers (Browserbase, Browser Use, Firecrawl) do not currently expose CDP to this tool â€” cloud providers have per-session CDP URLs but live-session routing is a follow-up.

**CDP method reference:** https://chromedevtools.github.io/devtools-protocol/ â€” the agent can `web_extract` a specific method's page to look up parameters and return shape.
...
|---|---|---|
| Local Chrome via `/browser connect` or `browser.cdp_url` | âœ“ | âœ“ full workflow |
| Browserbase | âœ“ | âœ“ full workflow (via injected XHR bridge) |
| Camofox / default local agent-browser | âœ— | âœ— (no CDP endpoint) |
...
When enabled, recording starts automatically on the first `browser_navigate` and saves to `~/.zed/browser_recordings/` when the session closes. Works in both local and cloud (Browserbase) modes. Recordings older than 72 hours are automatically cleaned up.
...
:::note
If paid features aren't available on your plan, Zed automatically falls back â€” first disabling `keepAlive`, then proxies â€” so browsing still works on free plans.
:::
