...
Type `@` followed by a reference to inject content directly into your message. Zed expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Zed env: `$ZED_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$ZED_HOME/skills/.hub/`
