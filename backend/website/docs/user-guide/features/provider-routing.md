...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Zed Agent supports **provider routing** â€” fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
:::tip
Traffic routed through [Nous Portal](/integrations/nous-portal) still respects per-model routing and priority configs â€” and Portal subscribers get 10% off token-billed providers.
:::

## Configuration

Add a `provider_routing` section to your `~/.zed/config.yaml`:
...
- **CLI mode** â€” configured in `~/.zed/config.yaml`, loaded at startup
- **Gateway mode** â€” same config file, loaded when the gateway starts

The routing config is read from `config.yaml` and passed as parameters when creating the `AIAgent`:

```
providers_allowed  â† from provider_routing.only
providers_ignored  â† from provider_routing.ignore
providers_order    â† from provider_routing.order
provider_sort      â† from provider_routing.sort
provider_require_parameters â† from provider_routing.require_parameters
provider_data_collection    â† from provider_routing.data_collection
```
