...
description: "How Zed Agent remembers across sessions â€” MEMORY.md, USER.md, and session search"
---

# Persistent Memory

Zed Agent has bounded, curated memory that persists across sessions. This lets it remember your preferences, your projects, your environment, and things it has learned.
...
|------|---------|------------|
| **MEMORY.md** | Agent's personal notes â€” environment facts, conventions, things learned | 2,200 chars (~800 tokens) |
| **USER.md** | User profile â€” your preferences, communication style, expectations | 1,375 chars (~500 tokens) |

Both are stored in `~/.zed/memories/` and are injected into the system prompt as a frozen snapshot at session start. The agent manages its own memory via the `memory` tool â€” it can add, replace, or remove entries.
...
silently dropping entries. The agent then makes room itself â€” consolidating or
...
- Individual entries separated by `Â§` (section sign) delimiters
- Entries can be multiline

**Frozen snapshot pattern:** The system prompt injection is captured once at session start and never changes mid-session. This is intentional â€” it preserves the LLM's prefix cache for performance. When the agent adds/removes memory entries during a session, the changes are persisted to disk immediately but won't appear in the system prompt until the next session starts. Tool responses always show the live state.

## Memory Tool Actions

The agent uses the `memory` tool with these actions:

- **add** â€” Add a new memory entry
- **replace** â€” Replace an existing entry with updated content (uses substring matching via `old_text`)
- **remove** â€” Remove an entry that's no longer relevant (uses substring matching via `old_text`)

There is no `read` action â€” memory content is automatically injected into the system prompt at session start. The agent sees its memories as part of its conversation context.

### Substring Matching

The `replace` and `remove` actions use short unique substring matching â€” you don't need the full entry text. The `old_text` parameter just needs to be a unique substring that identifies exactly one entry:
...
### `memory` â€” Agent's Personal Notes
...
The agent saves automatically â€” you don't need to ask. It saves when it learns:

- **User preferences:** "I prefer TypeScript over JavaScript" â†’ save to `user`
- **Environment facts:** "This server runs Debian 12 with PostgreSQL 16" â†’ save to `memory`
- **Corrections:** "Don't use `sudo` for Docker commands, user is in docker group" â†’ save to `memory`
- **Conventions:** "Project uses tabs, 120-char line width, Google-style docstrings" â†’ save to `memory`
- **Completed work:** "Migrated database from MySQL to PostgreSQL on 2026-01-15" â†’ save to `memory`
- **Explicit requests:** "Remember that my API key rotation happens monthly" â†’ save to `memory`

### Skip These

- **Trivial/obvious info:** "User asked about Python" â€” too vague to be useful
- **Easily re-discovered facts:** "Python 3.12 supports f-string nesting" â€” can web search this
- **Raw data dumps:** Large code blocks, log files, data tables â€” too big for memory
...
{
  "success": false,
  "error": "Memory at 2,100/2,200 chars. Adding this entry (250 chars) would exceed the limit. Consolidate now: use 'replace' to merge overlapping entries into shorter ones or 'remove' stale or less important entries (see current_entries below), then retry this add â€” all in this turn.",
  "current_entries": ["..."],
  "usage": "2,100/2,200"
}
```
...
- All CLI and messaging sessions are stored in SQLite (`~/.zed/state.db`) with FTS5 full-text search
- Search queries return actual messages from the DB â€” no LLM summarization, no truncation
- The agent can find things it discussed weeks ago, even if they're not in its active memory
- The agent can also scroll forward/backward inside any session it finds

```bash
zed sessions list    # Browse past sessions
```
...
| **Cost** | Token cost in every prompt | Free â€” no LLM calls |
| **Use case** | Key facts always available | Finding specific past conversations |
| **Management** | Manually curated by agent | Automatic â€” all sessions stored |
...
By default the agent saves memory freely â€” including from the background
...
|------------------|-----------|
| `false` (default) | Write freely â€” the gate is off (the pre-gate behaviour). |
| `true` | Require approval before anything is saved. In the interactive CLI, foreground writes prompt you inline (entries are small enough to read in full). Everywhere else â€” messaging platforms, scripts, and the background self-improvement review â€” writes are **staged** for review with `/memory pending`. |
...
`write_approval: true`, and every save â€” especially the unprompted background
ones â€” waits for your yes/no before it ever enters your profile.
...
or update a skill. By default it surfaces a short `ðŸ’¾ Memory updated` line in
...
| Value | Behaviour |
|-------|-----------|
| `off` | No chat notification. The review still runs and still writes â€” you just don't see a line for it. |
| `on` (default) | Generic line, e.g. `ðŸ’¾ Memory updated`, `ðŸ’¾ Skill 'foo' patched`. |
| `verbose` | Includes a compact preview of what changed, e.g. `ðŸ’¾ Memory âž• User prefers terse replies` or a `"old" â†’ "new"` skill diff snippet. |
...
`~/.zed/pending/skills/<id>.json` when you want to read the whole change.
Full details in [Gating agent skill writes](/user-guide/features/skills#gating-agent-skill-writes-skillswrite_approval).


## External Memory Providers

For deeper, persistent memory that goes beyond MEMORY.md and USER.md, Zed ships with 8 external memory provider plugins â€” including Honcho, OpenViking, Mem0, Hindsight, Holographic, RetainDB, ByteRover, and Supermemory.

External providers run **alongside** built-in memory (never replacing it) and add capabilities like knowledge graphs, semantic search, automatic fact extraction, and cross-session user modeling.

```bash
zed memory setup      # pick a provider and configure it
