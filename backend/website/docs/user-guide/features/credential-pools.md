...
:::tip
Credential pools are mainly for API-key providers (OpenRouter, Anthropic). A single [Zed Portal](/integrations/nous-portal) OAuth covers 300+ models, so most users don't need a pool when on Portal.
:::
