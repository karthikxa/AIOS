...
The `delegate_task` tool spawns child AIAgent instances with isolated context, restricted toolsets, and their own terminal sessions. Each child gets a fresh conversation and works independently â€” only its final summary enters the parent's context.
...
You can configure a different model for subagents via `config.yaml` â€” useful for delegating simple tasks to cheaper/faster models:
...
- `delegation` â€” blocked for leaf subagents (the default). Retained for `role="orchestrator"` children, bounded by `max_spawn_depth` â€” see [Depth Limit and Nested Orchestration](#depth-limit-and-nested-orchestration) below.
- `clarify` â€” subagents cannot interact with the user
- `memory` â€” no writes to shared persistent memory
- `code_execution` â€” children should reason step-by-step
- `send_message` â€” no cross-platform side effects (e.g., sending Telegram messages)
...
With a hard cap configured, if a subagent times out having made **zero** API calls (usually: provider unreachable, auth failure, or tool-schema rejection), `delegate_task` writes a structured diagnostic to `~/.zed/logs/subagent-timeout-<session>-<timestamp>.log` containing the subagent's config snapshot, credential-resolution trace, and any early error messages. Much easier to root-cause than the previous silent-timeout behavior.
:::
...
- Kill and pause controls â€” cancel a specific subagent mid-flight without interrupting its siblings
- Post-hoc review: step through each subagent's turn-by-turn history even after they've returned to the parent

The classic CLI just prints `/agents` as a text summary; the TUI is where the overlay shines. See [TUI â€” Slash commands](/user-guide/tui#slash-commands).
...
For multi-stage workflows (research â†’ synthesis, or parallel orchestration over sub-problems), a parent can spawn **orchestrator** children that *can* delegate their own workers:
...
**Cost warning:** With `max_spawn_depth: 3` and `max_concurrent_children: 3`, the tree can reach 3Ã—3Ã—3 = 27 concurrent leaf agents. Each extra level multiplies spend â€” raise `max_spawn_depth` intentionally.

## Lifetime and Durability

:::warning delegate_task is synchronous â€” not durable
...
- `cronjob` (action=`create`) â€” schedules a separate agent run; immune to parent-turn interrupts.
- `terminal(background=True, notify_on_complete=True)` â€” long-running shell commands that keep running while the agent does other things.
:::

## Key Properties

- Each subagent gets its **own terminal session** (separate from the parent)
- **Nested delegation is opt-in** â€” only `role="orchestrator"` children can delegate further, and only when `max_spawn_depth` is raised from its default of 1 (flat). Disable globally with `orchestrator_enabled: false`.
...
- **Interrupt propagation** â€” interrupting the parent interrupts all active children (including grandchildren under orchestrators)
...
When `base_url` points at an Anthropic-compatible endpoint â€” for example a path ending in `/anthropic`, an Azure Foundry Claude route, or a MiniMax `/anthropic` proxy â€” `api_mode` is auto-detected as `anthropic_messages` so the subagent uses the right wire format without you setting anything. Set `api_mode` explicitly when the auto-detection guess is wrong (rare).

:::tip
The agent handles delegation automatically based on the task complexity. You don't need to explicitly ask it to delegate â€” it will do so when it makes sense.
:::
