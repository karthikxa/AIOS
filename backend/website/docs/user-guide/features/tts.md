...
Zed Agent supports both text-to-speech output and voice message transcription across all messaging platforms.

:::tip Nous Subscribers
If you have a paid [Zed Portal](https://portal.zedteam.com) subscription, OpenAI TTS is available through the **[Tool Gateway](tool-gateway.md)** without a separate OpenAI API key. New installs can run `zed setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Nous Subscription** for just TTS via `zed model` or `zed tools`.
:::
...
| CLI | Saved to `~/.zed/audio_cache/` | MP3 |
...
If the file contains `{transcript}` or `{{ transcript }}`, Zed replaces that placeholder with the live TTS text. Otherwise, Zed appends a labeled `TRANSCRIPT` section automatically. The persona prompt stays local and is not shown in the chat reply.
...
tts:
  ...
  gemini:
    voice: Algieba
    persona_prompt_file: ~/.zed/tts/butler-voice.md
```

### Gemini Audio Tags

Gemini 3.1 Flash TTS supports freeform square-bracket audio tags such as `[whispers]`, `[excitedly]`, `[very slow]`, `[laughs]`, and other expressive delivery notes. Enable `tts.gemini.audio_tags` to have Zed run a hidden rewrite pass before Gemini TTS. The rewrite inserts inline tags into the TTS script only; the visible chat reply stays unchanged.
...
Each provider has a documented per-request input-character cap. Zed truncates text before calling the provider so requests never fail with a length error:
...
- **OpenAI, ElevenLabs, and Mistral** produce Opus natively â€” no extra setup
...
**Install via `zed tools`** â†’ Voice & TTS â†’ Piper â€” Zed runs `pip install piper-tts` for you. Or install manually: `pip install piper-tts`.
...
On the first TTS call for a voice that isn't cached locally, Zed runs `python -m piper.download_voices <name>` and downloads the model (~20-90MB depending on quality tier) into `~/.zed/cache/piper-voices/`. Subsequent calls reuse the cached model.

**Picking a voice.** The [full voice catalog](https://github.com/OHF-Voice/piper1-gpl/blob/main/docs/VOICES.md) covers English, Spanish, French, German, Italian, Dutch, Portuguese, Russian, Polish, Turkish, Chinese, Arabic, Hindi, and more â€” each with `x_low` / `low` / `medium` / `high` quality tiers. Sample voices at [rhasspy.github.io/piper-samples](https://rhasspy.github.io/piper-samples/).
...
Credentials come from your shell environment (`VOLCENGINE_APP_ID` / `VOLCENGINE_ACCESS_TOKEN`) or `~/.doubao-speech/config.yaml`. Pick a voice by adding `--voice zh-female-warm` (or any other alias from `doubao-speech list-voices`) to the command. `doubao-speech` also bundles streaming ASR â€” see the [STT section below](#example-doubao--volcengine-asr) for Zed integration. Source and full docs: [github.com/Hypnus-Yuan/doubao-speech](https://github.com/Hypnus-Yuan/doubao-speech).

#### Placeholders

Your command template can reference these placeholders. Zed substitutes them at render time and shell-quotes each value for the surrounding context (bare / single-quoted / double-quoted), so paths with spaces and other shell-sensitive characters are safe.

| Placeholder      | Meaning                                              |
|------------------|------------------------------------------------------|
| `{input_path}`   | Path to the temp UTF-8 text file Zed wrote        |
...
| `output_format`    | `mp3`   | One of `mp3` / `wav` / `ogg` / `flac`. Auto-inferred from the output extension if Zed picks a path.      |
| `voice_compatible` | `false` | When `true`, Zed converts MP3/WAV output to Opus/OGG via ffmpeg so Telegram renders a voice bubble.      |
...
Command-type providers run whatever shell command you configure, with your user's permissions. Zed quotes placeholder values and enforces the configured timeout, but the command template itself is trusted local input â€” treat it the same way you would a shell script on your PATH.

### Python plugin providers

For TTS engines that can't be expressed as a single shell command â€” Python SDKs without a CLI, streaming engines, voice-listing APIs, OAuth-refreshing auth â€” register a Python plugin via `ctx.register_tts_provider()`. The plugin **coexists with** (does not replace) the [Custom command providers](#custom-command-providers) registry; pick the surface that fits your engine.

#### When to pick which

| Your backend hasâ€¦ | Use |
|---|---|
...
| A Python SDK only â€” no CLI | **Plugin** |
| Streaming bytes you want to deliver chunked (mid-generation voice bubbles) | **Plugin** (override `stream()`) |
| A voice-listing API used by `zed setup` | **Plugin** (override `list_voices()`) |
| OAuth refresh flow (not a static bearer token) | **Plugin** |

Built-ins always win, and command providers win over a same-name plugin â€” so plugins are safe to register against any non-built-in name without worrying about shadowing your existing config.

#### Minimal plugin

Drop this in `~/.zed/plugins/my-tts/`:
...
Enable it (`zed plugins enable my-tts`), point `tts.provider` at it (`tts.provider: my-tts` in `config.yaml`), and the `text_to_speech` tool will route through your plugin.

#### Optional hooks

Override these on your provider class for richer integration:

- `list_voices()` â†’ list of `{id, display, language, gender, preview_url}` dicts shown in `zed tools`.
- `list_models()` â†’ list of `{id, display, languages, max_text_length}` dicts.
- `get_setup_schema()` â†’ return `{name, badge, tag, env_vars: [{key, prompt, url}]}` to power the picker row in `zed tools` / `zed setup`. Without this, the plugin still works but its row in the picker is minimal.
- `stream(text, *, voice, model, format, **extra)` â†’ iterator yielding audio bytes for streaming delivery (default raises `NotImplementedError`).
- `voice_compatible` property â†’ set `True` if your output is Opus-compatible and the gateway should deliver it as a voice bubble (default `False` = regular audio attachment).
...
| **Groq Whisper API** | Goodâ€“Best | Free tier | `GROQ_API_KEY` |
| **OpenAI Whisper API** | Goodâ€“Best | Paid | `VOICE_TOOLS_OPENAI_KEY` or `OPENAI_API_KEY` |

:::info Zero Config
Local transcription works out of the box when `faster-whisper` is installed. If that's unavailable, Zed can also use a local `whisper` CLI from common install locations (like `/opt/homebrew/bin`) or a custom command via `ZED_LOCAL_STT_COMMAND`.
:::
...
**Local (faster-whisper)** â€” Runs Whisper locally via [faster-whisper](https://github.com/SYSTRAN/faster-whisper). Uses CPU by default, GPU if available. Model sizes:
...
**Groq API** â€” Requires `GROQ_API_KEY`. Good cloud fallback when you want a free hosted STT option.

**OpenAI API** â€” Accepts `VOICE_TOOLS_OPENAI_KEY` first and falls back to `OPENAI_API_KEY`. Supports `whisper-1`, `gpt-4o-mini-transcribe`, and `gpt-4o-transcribe`.

**Mistral API (Voxtral Transcribe)** â€” Requires `MISTRAL_API_KEY`. Uses Mistral's [Voxtral Transcribe](https://docs.mistral.ai/capabilities/audio/speech_to_text/) models. Supports 13 languages, speaker diarization, and word-level timestamps. Install with `pip install zed-agent[mistral]`.

**xAI Grok STT** â€” Requires `XAI_API_KEY`. Posts to `https://api.x.ai/v1/stt` as multipart/form-data. Good choice if you're already using xAI for chat or TTS and want one API key for everything. Auto-detection order puts it after Groq â€” explicitly set `stt.provider: xai` to force it.

**Custom local CLI fallback** â€” Set `ZED_LOCAL_STT_COMMAND` if you want Zed to call a local transcription command directly. The command template supports `{input_path}`, `{output_dir}`, `{language}`, and `{model}` placeholders. Your command must write a `.txt` transcript somewhere under `{output_dir}`.
...
Zed writes the incoming voice message to `{input_path}`, runs the command, and reads the `.txt` file produced under `{output_dir}`. Language is auto-detected by the Volcengine bigmodel endpoint.

### Fallback Behavior

If your configured provider isn't available, Zed automatically falls back:
- **Local faster-whisper unavailable** â†’ Tries a local `whisper` CLI or `ZED_LOCAL_STT_COMMAND` before cloud providers
- **Groq key not set** â†’ Falls back to local transcription, then OpenAI
- **OpenAI key not set** â†’ Falls back to local transcription, then Groq
- **Mistral key/SDK not set** â†’ Skipped in auto-detect; falls through to next available provider
- **Nothing available** â†’ Voice messages pass through with an accurate note to the user

### STT custom command providers

If the STT engine you want isn't natively supported (Doubao ASR, NVIDIA Parakeet, a whisper.cpp build, an open-source SenseVoice CLI, anything else that exposes a shell command), wire it in as a **command-type provider** without writing any Python. Zed runs your shell command against the audio file and reads back the transcript.

Declare one or more providers under `stt.providers.<name>` and switch between them with `stt.provider: <name>` â€” same shape as the TTS [command-provider registry](#custom-command-providers), adapted for the input=audio â†’ output=transcript direction.
...
This complements the legacy `ZED_LOCAL_STT_COMMAND` escape hatch â€” that env var still works untouched via the built-in `local_command` path. Use `stt.providers.<name>` when you want **multiple** shell-driven STT engines, a name you can pick via `stt.provider`, or anything that needs per-provider `language` / `model` / `timeout`.

#### STT placeholders

Your command template can reference these placeholders. Zed substitutes them at render time and shell-quotes each value for the surrounding context (bare / single-quoted / double-quoted), so paths with spaces are safe.
...
1. If `{output_path}` exists and is non-empty â†’ Zed reads it as UTF-8 text.
2. Otherwise, if the command wrote to stdout â†’ Zed uses that.
3. Otherwise â†’ error: "Command STT provider wrote no output file and produced no stdout".

This lets you use the registry for both file-writing CLIs (`whisper-cli`, `parakeet-asr`) and curl-style one-liners that emit transcript to stdout (`curl â€¦ | jq -r .text`).

For `format: json` / `srt` / `vtt`, Zed returns the raw file content as the `transcript` field. Extracting `.text` from JSON is out of scope for the runner â€” either configure `format: txt`, or post-process JSON downstream.
...
- **Shell-quoting is automatic.** Placeholders inside `'â€¦'` get single-quote-safe escaping; inside `"â€¦"` get `$`/`` ` ``/`"` escaping; outside quotes get `shlex.quote`. Don't pre-quote placeholder values.

#### STT command-provider security

The shell command runs under the same user as Zed with full filesystem access â€” same trust model as `tts.providers.<name>: type: command` and `ZED_LOCAL_STT_COMMAND`. Only declare command providers from sources you trust.

### Python plugin providers (STT)

For STT engines that aren't built-in AND can't be expressed as a shell command (need a Python SDK, OAuth-refreshing auth, streaming chunks, etc.), register a Python plugin via `ctx.register_transcription_provider()`. The plugin **coexists with** the 6 built-in providers (`local`, `local_command`, `groq`, `openai`, `mistral`, `xai`) and the `stt.providers.<name>: type: command` registry â€” built-ins keep their native implementations and always win on name collision; command providers win over plugins of the same name (config is more local than plugin install).
...
| Only the legacy single-command escape hatch is wanted        | `ZED_LOCAL_STT_COMMAND` env var (preserved for back-compat)  |
...
| A built-in already covers it (`local`, `groq`, `openai`, â€¦)  | Set `stt.provider: <name>` â€” built-ins are inline               |

#### Resolution order

1. **`stt.provider` is a built-in name** â†’ built-in dispatch. **Always wins.**
2. **`stt.provider` matches `stt.providers.<name>` with `command:` set** â†’ command-provider runner (see [STT custom command providers](#stt-custom-command-providers)). Wins over a same-name plugin.
3. **`stt.provider` matches a plugin-registered `TranscriptionProvider`** â†’ plugin dispatch:
   - if the plugin's `is_available()` returns `False` (missing creds or SDK), the call surfaces an unavailability error envelope identifying the plugin â€” **not** the generic "No STT provider available" message.
...
4. **No match** â†’ "No STT provider available" error.
...
Drop this in `~/.zed/plugins/my-stt/`:
...
class MySTTProvider(TranscriptionProvider):
    ...
    def transcribe(self, file_path, *, model=None, language=None, **extra):
        ...
        # Do NOT raise â€” convert exceptions to the error envelope so the
...
Enable it (`zed plugins enable my-stt`), set `stt.provider: my-stt` in `config.yaml`, and voice-message transcription will route through your plugin.

#### Optional hooks

Override these on your provider class for richer integration:

- `list_models()` â†’ list of `{id, display, languages, max_audio_seconds}` dicts.
- `default_model()` â†’ string returned when the user doesn't override the model.
- `get_setup_schema()` â†’ return `{name, badge, tag, env_vars: [{key, prompt, url}]}` to power picker rows in `zed tools` / `zed setup` (the picker category for STT is not yet shipped â€” this metadata is available to plugins for forward compatibility).
