...
description: Search the web and extract page content with multiple backend providers â€” including free self-hosted SearXNG.
...
Zed Agent includes two model-callable web tools backed by multiple providers:

- **`web_search`** â€” search the web and return ranked results
- **`web_extract`** â€” fetch and extract readable content from one or more URLs

Both are configured through a single backend selection. Providers are chosen via `zed tools` or set directly in `config.yaml`.

## Backends

| Provider | Env Var | Search | Extract | Free tier |
|----------|---------|--------|---------|-----------|
| **Firecrawl** (default) | `FIRECRAWL_API_KEY` | âœ” | âœ” | 500 credits/mo |
| **SearXNG** | `SEARXNG_URL` | âœ” | â€” | âœ” Free (self-hosted) |
| **Brave Search (free tier)** | `BRAVE_SEARCH_API_KEY` | âœ” | â€” | 2 000 queries/mo |
| **DDGS (DuckDuckGo)** | â€” (no key) | âœ” | â€” | âœ” Free |
| **Tavily** | `TAVILY_API_KEY` | âœ” | âœ” | 1 000 searches/mo |
| **Exa** | `EXA_API_KEY` | âœ” | âœ” | 1 000 searches/mo |
| **Parallel** | `PARALLEL_API_KEY` | âœ” | âœ” | Paid |
| **xAI (Grok)** | `XAI_API_KEY` or `zed auth login xai-oauth` | âœ” | â€” | Paid (SuperGrok or per-token) |

Brave Search, DDGS, and xAI are **search-only** â€” pair any of them with Firecrawl/Tavily/Exa/Parallel when you also need `web_extract`. DDGS uses the [`ddgs` Python package](https://pypi.org/project/ddgs/) under the hood; if it isn't already installed, run `pip install ddgs` (or let Zed lazy-install it on first use). xAI runs Grok's server-side `web_search` tool on the Responses API â€” results are LLM-generated rather than index-backed, so titles, descriptions, and URL choice are all model output (see the [trust-model caveat](#xai-grok) below).

**Per-capability split:** you can use different providers for search and extract independently â€” for example SearXNG (free) for search and Firecrawl for extract. See [Per-capability configuration](#per-capability-configuration) below.

:::tip Nous Subscribers
If you have a paid [Zed Portal](https://portal.zedteam.com) subscription, web search and extract are available through the **[Tool Gateway](tool-gateway.md)** via managed Firecrawl â€” no API key needed. New installs can run `zed setup --portal` to log in and turn on all gateway tools at once; existing installs can flip just web via `zed tools`.
:::
...
|------------------------|--------------|
| Under 5 000 | Returned as-is â€” no LLM call, full markdown reaches the agent |
| 5 000 â€“ 500 000 | Single-pass summary via the `web_extract` auxiliary model, capped at ~5 000 chars of output |
| 500 000 â€“ 2 000 000 | Chunked: split into 100 k-char chunks, summarize each in parallel, then synthesize a final summary (~5 000 chars) |
| Over 2 000 000 | Refused with a hint to use a more focused source URL |

The summary keeps quotes, code blocks, and key facts in their original formatting â€” it's a content compressor, not a paraphraser. If summarization fails or times out, Zed falls back to the first ~5 000 chars of raw content rather than a useless error.

### Which model does the summarizing?

The `web_extract` auxiliary task. By default (`auxiliary.web_extract.provider: "auto"`), this is your **main chat model** â€” same provider, same model as `zed model`. That's fine for most setups, but on expensive reasoning models (Opus, MiniMax M2.7, etc.) every long-page extract adds meaningful cost.
...
Or pick interactively: `zed model` â†’ **Configure auxiliary models** â†’ `web_extract`.
...
If you specifically need raw, unsummarized page content â€” for example, you're scraping a structured page where the LLM summary would drop important fields â€” use `browser_navigate` + `browser_snapshot` instead. The browser tool returns the live accessibility tree without auxiliary-model rewriting (subject to its own 8 000-char snapshot cap on huge pages).

---

## Setup

### Quick setup via `zed tools`

Run `zed tools`, navigate to **Web Search & Extract**, and pick a provider. The wizard prompts for the required URL or API key and writes it to your config.
...
SearXNG is a privacy-respecting, open-source metasearch engine that aggregates results from 70+ search engines. **No API key required** â€” just point Zed at a running SearXNG instance.

SearXNG is **search-only** â€” `web_extract` requires a separate extract provider.

#### Option A â€” Self-host with Docker (recommended)
...
```yaml
# Before (default â€” JSON disabled):
formats:
  - html

# After (enable JSON for Zed):
formats:
  - html
  - json
```
...
You should see something like `10 results`. If you get a `403 Forbidden`, JSON format is still disabled â€” recheck step 4.
...
Then select SearXNG as the search backend in `~/.zed/config.yaml`:
...
Or set via `zed tools` â†’ Web Search & Extract â†’ SearXNG.

---

#### Option B â€” Use a public instance
...
With this config, Zed uses SearXNG for all search queries and Firecrawl for URL extraction â€” combining free search with high-quality extraction.
...
Works with either credential path â€” no new env vars, no new setup wizard:

```bash
# ~/.zed/.env (env-var path)
...
web:
  ...
  xai:
    ...
    allowed_domains:             # optional, max 5 â€” mutex with excluded_domains
      - arxiv.org
...
If no backend is explicitly configured, Zed picks the first available one based on which credentials are set:
...
xAI Web Search is **not** in the auto-detection chain â€” having `XAI_API_KEY` set (or being signed in via xAI Grok OAuth) does not automatically route web traffic through xAI, since those credentials are also used for inference / TTS / image gen and the user may want a different backend for web. Opt in explicitly with `web.backend: "xai"`.

---

## Verify your setup

Run `zed setup` to see which web backend is detected:
...
- If you get HTTP 403, JSON format is disabled â€” add `json` to the `formats` list in `settings.yml` and restart
...
- Switch the `web_extract` auxiliary task to a faster model (e.g. `google/gemini-3-flash-preview`) â€” see [How `web_extract` handles long pages](#how-web_extract-handles-long-pages)
