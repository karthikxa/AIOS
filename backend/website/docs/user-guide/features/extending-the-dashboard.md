...
description: "Build themes and plugins for the Zed web dashboard â€” palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# Extending the Dashboard

The Zed web dashboard (`zed dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:

1. **Themes** â€” YAML files that repaint the dashboard's palette, typography, layout, and per-component chrome. Drop a file in `~/.zed/dashboard-themes/`; it appears in the theme switcher.
2. **UI plugins** â€” a directory with `manifest.json` + a JavaScript bundle that registers a tab, replaces a built-in page, augments one via page-scoped slots, or injects components into named shell slots.
3. **Backend plugins** â€” a Python file inside that plugin directory that exposes a FastAPI `router`; routes are mounted under `/api/plugins/<name>/` and called from the plugin's UI.
...
If you just want to use the dashboard, see [Web Dashboard](./web-dashboard). If you want to reskin the terminal CLI (not the web dashboard), see [Skins & Themes](./skins) â€” the CLI skin system is unrelated to dashboard themes.

:::note How the pieces compose
Themes and plugins are independent but synergistic. A theme can stand alone (just a YAML file). A plugin can stand alone (just a tab). Together they let you build a complete visual reskin with custom HUDs â€” the example `strike-freedom-cockpit` demo (lives in the `zed-example-plugins` companion repo â€” see [Combined theme + plugin demo](#combined-theme--plugin-demo) for install steps) does exactly that.
:::
...
- [Themes](#themes)
  - [Quick start â€” your first theme](#quick-start--your-first-theme)
...
- [Plugins](#plugins)
  - [Quick start â€” your first plugin](#quick-start--your-first-plugin)
...
Themes are YAML files stored in `~/.zed/dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional â€” missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.

### Quick start â€” your first theme
...
These three blocks are the heart of a theme. Each is independent â€” override one, leave the others.
...
| Key | Description |
|-----|-------------|
| `palette.background` | Deepest canvas color â€” typically near-black. Drives the page background and card fill. |
...
| `palette.noiseOpacity` | 0â€“1.2 multiplier on the grain overlay. Lower = softer, higher = grittier. |

Each layer accepts either `{hex: "#RRGGBB", alpha: 0.0â€“1.0}` or a bare hex string (alpha defaults to 1.0).
...
| `fontUrl` | string | Optional external stylesheet URL. Injected as `<link rel="stylesheet">` in `<head>` on theme switch. Same URL is never injected twice. Works with Google Fonts, Bunny Fonts, self-hosted `@font-face` sheets â€” anything linkable. |
| `baseSize` | string | Root font size â€” controls the rem scale. E.g. `"14px"`, `"16px"`. |
...
theme is active â€” the choice is independent of the theme and persists across
...
free-text font URL â€” the font's stylesheet is injected as a `<link>`, so the
...
|-----|--------|-------------|
| `radius` | any CSS length (`"0"`, `"0.25rem"`, `"0.5rem"`, `"1rem"`, ...) | Corner-radius token. Maps to `--radius` and cascades into `--radius-sm/md/lg/xl` â€” every rounded element shifts together. |
| `density` | `compact` \| `comfortable` \| `spacious` | Spacing multiplier applied as the `--spacing-mul` CSS var. `compact = 0.85Ã—`, `comfortable = 1.0Ã—` (default), `spacious = 1.2Ã—`. Scales Tailwind's base spacing, so padding, gap, and space-between utilities all shift proportionally. |
...
| `cockpit` | Left sidebar rail (260px) + main content. Populated by plugins via the `sidebar` slot â€” see [Shell slots](#shell-slots). Without a plugin the rail shows a placeholder. |
...
- Bare URLs â€” wrapped in `url(...)` automatically.
- Pre-wrapped `url(...)`, `linear-gradient(...)`, `radial-gradient(...)` expressions â€” used as-is.
- `"none"` â€” explicit opt-out.
...
Property names use camelCase (`clipPath`) and are emitted as kebab (`clip-path`). Values are plain CSS strings â€” anything CSS accepts (`clip-path`, `border-image`, `background`, `box-shadow`, `animation`, ...).

### Color overrides

Most themes won't need this â€” the 3-layer palette derives every shadcn token. Use `colorOverrides` when you want a specific accent the derivation won't produce (a softer destructive red for a pastel theme, a specific success green for a brand).
...
Each key maps 1:1 to the `--color-<kebab>` CSS var (e.g. `primaryForeground` â†’ `--color-primary-foreground`). Any key set here wins over the palette cascade for the active theme only â€” switching to another theme clears the overrides.

### Raw `customCSS`

For selector-level chrome that `componentStyles` can't express â€” pseudo-elements, animations, media queries, theme-scoped overrides â€” drop raw CSS into `customCSS`:
...
The CSS is injected as a single scoped `<style data-zed-theme-css>` tag on theme apply and cleaned up on theme switch. **Capped at 32 KiB per theme.**

### Built-in themes

Each built-in ships its own palette, typography, and layout â€” switching produces visible changes beyond color alone.

| Theme | Palette | Typography | Layout |
|-------|---------|------------|--------|
| **Zed Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **Zed Teal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
| **RosÃ©** (`rose`) | Pink + ivory | Fraunces (serif) + DM Mono, 16px | 1rem radius, spacious |

Themes that reference Google Fonts (all except Zed Teal) load the stylesheet on demand â€” the first time you switch to them a `<link>` tag is injected into `<head>`.

### Full theme YAML reference

Every knob in one file â€” copy and trim what you don't need:
...
Refresh the dashboard after creating the file. Switch themes live from the header bar â€” click the palette icon. Selection persists to `config.yaml` under `dashboard.theme` and is restored on reload.

---

## Plugins

A dashboard plugin is a directory with a `manifest.json`, a pre-built JS bundle, and optionally a CSS file and a Python file with FastAPI routes. Plugins live next to other Zed plugins in `~/.zed/plugins/<name>/` â€” the dashboard extension is a `dashboard/` subfolder inside that plugin directory, so one plugin can extend both the CLI/gateway and the dashboard from a single install.

Plugins don't bundle React or UI components. They use the **Plugin SDK** exposed on `window.__ZED_PLUGIN_SDK__`. This keeps plugin bundles tiny (typically a few KB) and avoids version conflicts.

### Quick start â€” your first plugin
...
Write the JS bundle (a plain IIFE â€” no build step needed):
...
Refresh the dashboard â€” your tab appears in the nav bar, after **Skills**.
...
- `plugin.yaml` + `__init__.py` â€” CLI/gateway plugin ([see plugins page](./plugins)).
- `dashboard/manifest.json` + `dashboard/dist/index.js` â€” dashboard UI plugin.
- `dashboard/plugin_api.py` â€” dashboard backend routes.
...
| `tab.position` | No | Where to insert the tab. `"end"` (default), `"after:<path>"`, or `"before:<path>"` â€” value after the colon is the **path segment** of the target tab (no leading slash). Examples: `"after:skills"`, `"before:config"`. |
...
| `slots` | No | Named shell slots this plugin populates. **Documentation aid only** â€” actual registration happens from the JS bundle via `registerSlot()`. Listing slots here makes discovery surfaces more informative. |
...
Plugins use Lucide icon names. The dashboard maps these by name â€” unknown names silently fall back to `Puzzle`.
...
Need a different icon? Open a PR to `web/src/App.tsx`'s `ICON_MAP` â€” pure additive change.

### The Plugin SDK

Everything a plugin needs is on `window.__ZED_PLUGIN_SDK__`. Plugins should never import React directly.
...
#### Calling built-in Zed endpoints
...
See [Web Dashboard â†’ REST API](./web-dashboard#rest-api) for the full list.

### Shell slots

Slots let a plugin inject components into named locations of the app shell â€” the cockpit sidebar, the header, the footer, an overlay layer â€” without claiming a whole tab. Multiple plugins can populate the same slot; they render stacked in registration order.
...
| `header-left` | Before the Zed brand in the top bar. |
| `header-right` | Before the theme/language switchers in the top bar. |
| `header-banner` | Full-width strip below the nav. |
| `sidebar` | Cockpit sidebar rail â€” **only rendered when `layoutVariant === "cockpit"`**. |
...
**Page-scoped slots** (render only on the named built-in page â€” use these to inject widgets, cards, or toolbars into an existing page without overriding the whole route):
...
Example â€” add a banner card to the top of the Sessions page:
...
The shell only renders `<PluginSlot name="..." />` for the slots above. Additional names are accepted by the registry for nested plugin UIs â€” a plugin can expose its own slots via `SDK.components.PluginSlot`.

#### Re-registration and HMR

If the same `(plugin, slot)` pair is registered twice, the later call replaces the earlier one â€” this matches how React HMR expects plugin re-mounts to behave.
...
Full replacement via `tab.override` is heavy â€” your plugin now owns the entire page, including any future updates we ship to it. Most of the time you just want to add a banner, card, or toolbar to an existing page. That's what **page-scoped slots** are for.

Every built-in page exposes `<page>:top` and `<page>:bottom` slots rendered at the top and bottom of its content area. Your plugin populates one by calling `registerSlot()` â€” the built-in page keeps working normally, and your component renders alongside it.

Available slots: `sessions:*`, `analytics:*`, `logs:*`, `cron:*`, `skills:*`, `config:*`, `env:*`, `docs:*`, `chat:*` (each with `:top` and `:bottom`). See the full catalogue in [Shell slots â†’ Slot catalogue](#slot-catalogue).

Minimal example â€” pin a banner to the top of the Sessions page:
...
- `tab.hidden: true` keeps the plugin out of the sidebar â€” it has no standalone page.
...
A reference plugin (`example-dashboard` in [`zed-example-plugins`](https://github.com/NousResearch/zed-example-plugins/tree/main/example-dashboard)) ships a live demo that injects a banner into `sessions:top` â€” install it to see the pattern end-to-end.

### Slot-only plugins (`tab.hidden`)

When `tab.hidden: true`, the plugin registers its component (for direct URL visits) and any slots, but never adds a tab to the navigation. Used by plugins that only exist to inject into slots â€” a header crest, a sidebar HUD, an overlay.
...
Plugin API routes bypass session-token authentication since the dashboard server binds to localhost by default. **Don't expose the dashboard on a public interface with `--host 0.0.0.0` if you run untrusted plugins** â€” their routes become reachable too.

#### Accessing Zed internals

Backend routes run inside the dashboard process, so they can import from the zed-agent codebase directly:
...
|----------|-----------|--------------|
| 1 (wins on conflict) | `~/.zed/plugins/<name>/dashboard/` | `user` |
| 2 | `<repo>/plugins/memory/<name>/dashboard/` | `bundled` |
| 2 | `<repo>/plugins/<name>/dashboard/` | `bundled` |
| 3 | `./.zed/plugins/<name>/dashboard/` | `project` â€” only when `ZED_ENABLE_PROJECT_PLUGINS` is set |
...
â€¦or restart `zed dashboard`.

#### Plugin load lifecycle

1. Dashboard loads. `main.tsx` exposes the SDK on `window.__ZED_PLUGIN_SDK__` and the registry on `window.__ZED_PLUGINS__`.
2. `App.tsx` calls `usePlugins()` â†’ fetches `GET /api/dashboard/plugins`.
3. For each manifest: CSS `<link>` is injected (if declared), then a `<script>` tag loads the JS bundle.
4. The plugin's IIFE runs and calls `window.__ZED_PLUGINS__.register(name, Component)` â€” and optionally `.registerSlot(name, slot, Component)` for each slot.
...
Plugins have up to **2 seconds** after their script loads to call `register()`. After that the dashboard stops waiting and finishes initial render. If a plugin later registers, it still appears â€” the nav is reactive.
...
The [`strike-freedom-cockpit`](https://github.com/NousResearch/zed-example-plugins/tree/main/strike-freedom-cockpit) plugin (companion repo `zed-example-plugins`) is a complete reskin demo. It pairs a theme YAML with a slot-only plugin to produce a cockpit-style HUD without forking the dashboard.
...
- A slot-only plugin (`tab.hidden: true`) that registers into three slots:
  - `sidebar` â€” an MS-STATUS panel with live telemetry bars driven by `SDK.api.getStatus()`.
  - `header-left` â€” a faction crest that reads `--theme-asset-crest` from the active theme.
  - `footer-right` â€” a custom tagline replacing the default org line.
...
Open the dashboard, pick **Strike Freedom** from the theme switcher. The cockpit sidebar appears, the crest shows in the header, the tagline replaces the footer. Switch back to **Zed Teal** and the plugin remains installed but invisible (the `sidebar` slot only renders under the `cockpit` layout variant).
...
|--------|------|----------|
| `window.__ZED_PLUGIN_SDK__` | object | `registry.ts` â€” React, hooks, UI components, API client, utils. |
| `window.__ZED_PLUGINS__.register(name, Component)` | function | Register a plugin's main component. |
| `window.__ZED_PLUGINS__.registerSlot(name, slot, Component)` | function | Register into a named shell slot. |
...
Check that the file is in `~/.zed/dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` â€” your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/.zed/logs/`.

**My plugin's tab doesn't show up.**
1. Check the manifest is at `~/.zed/plugins/<name>/dashboard/manifest.json` (note the `dashboard/` subdirectory).
2. `curl http://127.0.0.1:9119/api/dashboard/plugins/rescan` to force re-discovery.
3. Open browser dev tools â†’ Network â€” confirm `manifest.json`, `index.js`, and any CSS loaded without 404s.
4. Open browser dev tools â†’ Console â€” look for errors during the IIFE or `window.__ZED_PLUGINS__ is undefined` (indicates the SDK didn't initialize, usually a React render crash earlier).
5. Verify your bundle calls `window.__ZED_PLUGINS__.register(...)` with the **same name** as `manifest.json:name`.
...
2. Restart `zed dashboard` â€” plugin API routes are mounted once at startup, **not** on rescan.
3. Check that `plugin_api.py` exports a module-level `router = APIRouter()`. Other export names are not picked up.
4. Tail `~/.zed/logs/errors.log` for `Failed to load plugin <name> API routes` â€” import errors are logged there.

**Theme change drops my color overrides.**
`colorOverrides` are scoped to the active theme and cleared on theme switch â€” that's by design. If you want overrides that persist, put them in your theme's YAML, not in the live switcher.
...
Dashboard plugins are installed by directory layout, not by pip entry point. The cleanest distribution path today is a git repo the user clones into `~/.zed/plugins/`. A pip-based installer for dashboard plugins is not currently wired up.
