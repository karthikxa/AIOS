...
When Zed Agent runs inside a messaging gateway (Slack, Discord, Telegram,
WhatsApp, Signal, etc.), it can deliver generated files directly into the
chat â€” not as paths the user has to copy, but as native attachments.
...
write a `MEDIA:` tag or do anything special â€” it just generates the file
...
The agent doesn't reach for artifacts by default â€” it has to know to.
...
persona in `~/.zed/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.zed/config.yaml` (switchable per session
via `/personality`).
...
If you use Zed' kanban multi-agent workflow, workers can attach
...
community servers for most popular tools â€” install whichever you need:
...
Install MCP servers via `~/.zed/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Zed Agent's deliverable
...
- Connector breadth comes via MCP rather than a curated catalog of 400
  hosted integrations â€” install the ones you actually use.
