...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Zed Agent â€” CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Zed Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Zed](/guides/use-voice-mode-with-zed).

## Prerequisites

Before using voice features, make sure you have:

1. **Zed Agent installed** â€” `pip install zed-agent` (see [Installation](/getting-started/installation))
2. **An LLM provider configured** â€” run `zed model` or set your preferred provider credentials in `~/.zed/.env`
3. **A working base setup** â€” run `zed` to verify the agent responds to text before enabling voice

:::tip
The `~/.zed/` directory and default `config.yaml` are created automatically the first time you run `zed`. You only need to create `~/.zed/.env` manually for API keys.
:::

:::tip Zed Portal covers both
A paid [Zed Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway â€” no separate OpenAI key needed. On a fresh install, `zed setup --portal` wires both up at once.
:::
...
| **ffmpeg** | Audio format conversion (MP3 â†’ Opus, PCM â†’ WAV) | All platforms |
...
```bash
# Speech-to-Text â€” local provider needs NO key at all
# pip install faster-whisper          # Free, runs locally, recommended
GROQ_API_KEY=your-key                 # Groq Whisper â€” fast, free tier (cloud)
VOICE_TOOLS_OPENAI_KEY=your-key       # OpenAI Whisper â€” paid (cloud)

# Text-to-Speech (optional â€” Edge TTS and NeuTTS work without any key)
ELEVENLABS_API_KEY=***           # ElevenLabs â€” premium quality
...
Voice mode is available in both the **classic CLI** (`zed chat`) and the **TUI** (`zed --tui`). Behavior is identical across both â€” same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.zed/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
1. Start the CLI with `zed` and enable voice mode with `/voice on`
2. **Press Ctrl+B** â€” a beep plays (880Hz), recording starts
3. **Speak** â€” a live audio level bar shows your input: `â— [â–â–‚â–ƒâ–…â–‡â–‡â–…â–‚] â¯`
4. **Stop speaking** â€” after 3 seconds of silence, recording auto-stops
...
8. Recording **automatically restarts** â€” speak again without pressing any key
...
:::tip
The record key is configurable via `voice.record_key` in `~/.zed/config.yaml` (default: `ctrl+b`).
:::
...
1. **Speech confirmation** â€” waits for audio above the RMS threshold (200) for at least 0.3s, tolerating brief dips between syllables
2. **End detection** â€” once speech is confirmed, triggers after 3.0 seconds of continuous silence
...
When TTS is enabled, the agent speaks its reply **sentence-by-sentence** as it generates text â€” you don't wait for the full response:
...
```bash
zed gateway        # Start the gateway (connects to configured platforms)
zed gateway setup  # Interactive setup wizard for first-time configuration
```
...
|------|------------|-----------------|-------|
| **Direct Message (DM)** | Open the bot's profile â†’ "Message" | No | Works immediately |
| **Server Channel** | Type in a text channel where the bot is present | Yes (`@botname`) | Bot must be invited to the server |

**DM (recommended for personal use):** Just open a DM with the bot and type â€” no @mention needed. Voice replies and all commands work the same as in channels.

**Server channels:** The bot only responds when you @mention it (e.g. `@zedbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.zed/.env`:
```bash
...
|----------|--------|-------|
| **Telegram** | Voice bubble (Opus/OGG) | Plays inline in chat. ffmpeg converts MP3 â†’ Opus if needed |
...
Go to the [Discord Developer Portal](https://discord.com/developers/applications) â†’ your application â†’ **Installation** â†’ **Default Install Settings** â†’ **Guild Install**:
...
In the [Developer Portal](https://discord.com/developers/applications) â†’ your application â†’ **Bot** â†’ **Privileged Gateway Intents**, enable all three:
...
**Message Content Intent** is required. **Server Members Intent** is only needed if your `DISCORD_ALLOWED_USERS` list uses usernames â€” if you use numeric user IDs, you can leave it OFF. Voice-channel SSRC â†’ user_id mapping comes from Discord's SPEAKING opcode on the voice websocket and does **not** require the Server Members Intent.
...
# STT â€” local provider needs no key (pip install faster-whisper)
# GROQ_API_KEY=your-key            # Alternative: cloud-based, fast, free tier

# TTS â€” optional. Edge TTS and NeuTTS need no key.
...
```bash
zed gateway        # Start with existing configuration
```
...
2. **Detects silence** â€” 1.5s of silence after at least 0.5s of speech triggers processing
...
# Speech-to-Text
stt:
  enabled: true                     # set to false to skip auto-transcription â€”
...
# pip install faster-whisper        # Free local STT â€” no API key needed
...
If you are running Zed inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
2. Or use DMs instead â€” no mention needed
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.zed/.env`
...
- The bot needs a SPEAKING event from Discord before it can map your audio â€” start speaking within a few seconds of joining
...
- Review gateway logs: `tail -f ~/.zed/logs/gateway.log`

### Bot responds in text but not in voice channel

- TTS provider may be failing â€” check API key and quota
