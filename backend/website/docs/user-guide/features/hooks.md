...
sidebar_position: 6
title: "Event Hooks"
description: "Run custom code at key lifecycle points â€” log activity, send alerts, post to webhooks"
---

# Event Hooks

Zed has three hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.zed/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.zed/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |

All three systems are non-blocking â€” errors in any hook are caught and logged, never crashing the agent.
...
Each hook is a directory under `~/.zed/hooks/` containing two files:

```text
~/.zed/hooks/
â””â”€â”€ my-hook/
    â”œâ”€â”€ HOOK.yaml      # Declares which events to listen for
...
- Can be `async def` or regular `def` â€” both work
...
### Tutorial: BOOT.md â€” Run a Startup Checklist on Every Gateway Boot

A popular pattern from the community: drop a Markdown checklist at `~/.zed/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."

This tutorial shows how to build it yourself as a user-defined hook. Zed does not ship a built-in BOOT.md hook â€” you wire up exactly the behavior you want.

#### What we're building

1. A file at `~/.zed/BOOT.md` with natural-language startup instructions.
...
Create `~/.zed/BOOT.md`. Write it as if you were giving instructions to a human assistant:
...
The agent sees this as part of its prompt, so anything you can describe in plain language works â€” tool calls, shell commands, sending messages, summarizing files.
...
- `_resolve_runtime_agent_kwargs()` resolves provider credentials the same way a normal gateway turn does â€” including API keys, base URLs, OAuth tokens, and credential pools.
...
Delete `~/.zed/BOOT.md` to disable the checklist â€” the hook stays loaded but silently skips when the file isn't there.
...
An earlier version of Zed shipped this as a built-in hook and silently spawned an agent with bare defaults on every gateway boot. That surprised users with custom endpoints and made the feature invisible to users who didn't know it was running. Keeping it as a documented pattern â€” built by you, in your hooks directory â€” means you see exactly what it does and opt in by writing the files.

### How It Works

1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.zed/hooks/`
2. Each subdirectory with `HOOK.yaml` + `handler.py` is loaded dynamically
3. Handlers are registered for their declared events
4. At each lifecycle point, `hooks.emit()` fires all matching handlers
5. Errors in any handler are caught and logged â€” a broken hook never crashes the agent
...
- Callbacks receive **keyword arguments**. Always accept `**kwargs` for forward compatibility â€” new parameters may be added in future versions without breaking your plugin.
...
Fires **immediately before** every tool execution â€” built-in tools and plugin tools alike.
...
**Fires:** In `model_tools.py`, inside `handle_function_call()`, before the tool's handler runs. Fires once per tool call â€” if the model calls 3 tools in parallel, this fires 3 times.

**Return value â€” veto the call:**
...
**Example â€” warn on dangerous tools:**
...
**Example â€” track tool usage metrics:**
...
Fires **once per turn**, before the tool-calling loop begins. This is the **only hook whose return value is used** â€” it can inject context into the current turn's user message.
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache â€” the system prompt stays identical across turns, so cached tokens are reused. The system prompt is Zed's territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.

All injected context is **ephemeral** â€” added at API call time only. The original user message in the conversation history is never mutated, and nothing is persisted to the session database.
...
Fires **once per turn**, after the tool-calling loop completes and the agent has produced a final response. Only fires on **successful** turns â€” does not fire if the turn was interrupted.
...
**Fires:** In `run_agent.py`, inside `run_conversation()`, after the tool loop exits with a final response. Guarded by `if final_response and not interrupted` â€” so it does **not** fire when the user interrupts mid-turn or the agent hits the iteration limit without producing a response.
...
**Example â€” sync to external memory:**
...
**Example â€” track response lengths:**
...
**Fires:** In `run_agent.py`, inside `run_conversation()`, during the first turn of a new session â€” specifically after the system prompt is built but before the tool loop starts. The check is `if not conversation_history` (no prior messages = new session).
...
**Example â€” initialize a session cache:**
...
**Fires:** In two places:
1. **`run_agent.py`** â€” at the end of every `run_conversation()` call, after all cleanup. Always fires, even if the turn errored.
2. **`cli.py`** â€” in the CLI's atexit handler, but **only** if the agent was mid-turn (`_agent_running=True`) when the exit occurred. This catches Ctrl+C and `/exit` during processing. In this case, `completed=False` and `interrupted=True`.
...
**Example â€” session duration tracking:**
...
Fires when the CLI or gateway **tears down** an active session â€” for example, when the user runs `/new`, the gateway GC'd an idle session, or the CLI quit with an active agent. This is the last chance to flush state tied to the outgoing session before its identity is gone.
...
Fires when the gateway **swaps in a new session key** for an active chat â€” the user invoked `/new`, `/reset`, `/clear`, or the adapter picked a fresh session after an idle window. This lets plugins react to the fact that conversation state has been wiped without waiting for the next `on_session_start`.
...
**Fires:** In `gateway/run.py`, immediately after the new session key is allocated but before the next inbound message is processed. On the gateway, the order is: `on_session_finalize(old_id)` â†’ swap â†’ `on_session_reset(new_id)` â†’ `on_session_start(new_id)` on the first inbound turn.
...
See the **[Build a Plugin guide](/guides/build-a-zed-plugin)** for the full walkthrough including tool schemas, handlers, and advanced hook patterns.
...
**Example â€” log orchestrator activity:**
...
:::info
With heavy delegation (e.g. orchestrator roles Ã— 5 leaves Ã— nested depth), `subagent_stop` fires many times per turn. Keep your callback fast; push expensive work to a background queue.
:::
...
**Fires:** In `gateway/run.py`, inside `GatewayRunner._handle_message()`, immediately after `is_internal` is computed. **Internal events skip the hook entirely** (they are system-generated â€” background-process completions, etc. â€” and must not be gate-kept by user-facing policy).
...
| Return | Effect |
|--------|--------|
| `{"action": "skip", "reason": "..."}` | Drop the message â€” no agent reply, no pairing flow, no auth. Plugin is assumed to have handled it (e.g. silent-ingested into the transcript). |
...
| `{"action": "allow"}` / `None` | Normal dispatch â€” runs the full auth / pairing / agent-loop chain. |
...
**Example â€” drop unauthorized DMs silently without triggering the pairing code:**
...
**Example â€” rewrite an ambient-message buffer into a single prompt on mention:**
...
Fires **immediately before** an approval request is shown to the user â€” covers every surface: interactive CLI, the Ink TUI, gateway platforms (Telegram, Discord, Slack, WhatsApp, Matrix, etc.), and ACP clients (VS Code, Zed, JetBrains).

This is the right place to wire a custom notifier â€” for example, a macOS menu-bar app that pops an allow/deny notification, or an audit log that records every approval request with context.
...
**Example â€” desktop notification on macOS:**
...
Fires **after** a tool returns and **before** the result is appended to the conversation. Lets a plugin rewrite ANY tool's result string â€” not just terminal output â€” before the model sees it.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `tool_name` | `str` | Tool that produced the result (`read_file`, `web_extract`, `delegate_task`, â€¦). |
...
Applies to every tool. For terminal-only rewriting see `transform_terminal_output` below â€” it's narrower and runs earlier in the pipeline (pre-truncation, pre-redaction).
...
| `output` | `str` | Raw combined stdout/stderr (may be very large â€” truncation happens after the hook). |
...
| `platform` | `str` | Delivery platform (`cli`, `telegram`, `discord`, â€¦; empty when unset). |

**Return value:** Non-empty `str` to replace the response text, `None` or empty string to leave it unchanged. **First non-empty string wins** when multiple plugins register â€” mirroring `transform_tool_result`.
...
The hook is guarded on a non-empty, non-interrupted response â€” it will not fire on stop-button interrupts or empty turns. Exceptions are logged as warnings and do not break agent execution.

---

## Shell Hooks

Declare shell-script hooks in your `cli-config.yaml` and Zed will run them as subprocesses whenever the corresponding plugin-hook event fires â€” in both CLI and gateway sessions. No Python plugin authoring required.

Use shell hooks when you want a drop-in, single-file script (Bash, Python, anything with a shebang) to:

- **Block a tool call** â€” reject dangerous `terminal` commands, enforce per-directory policies, require approval for destructive `write_file` / `patch` operations.
- **Run after a tool call** â€” auto-format Python or TypeScript files that the agent just wrote, log API calls, trigger a CI workflow.
- **Inject context into the next LLM turn** â€” prepend `git status` output, the current weekday, or retrieved documents to the user message (see [`pre_llm_call`](#pre_llm_call)).
- **Observe lifecycle events** â€” write a log line when a subagent completes (`subagent_stop`) or a session starts (`on_session_start`).

Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`zed_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks â€” both flow through the same dispatcher.

### Comparison at a glance

| Dimension | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.zed/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.zed/agent-hooks/` (by convention) | `~/.zed/plugins/<name>/` | `~/.zed/hooks/<name>/` |
| Language | Any (Bash, Python, Go binary, â€¦) | Python only | Python only |
...
Each time the event fires, Zed spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.

**stdin â€” payload the script receives:**
...
`tool_name` and `tool_input` are `null` for non-tool events (`pre_llm_call`, `subagent_stop`, session lifecycle). The `extra` dict carries all event-specific kwargs (`user_message`, `conversation_history`, `child_role`, `duration_ms`, â€¦). Unserialisable values are stringified rather than omitted.

**stdout â€” optional response:**
...
The agent's in-context view of the file is **not** re-read automatically â€” the reformat only affects the file on disk. Subsequent `read_file` calls pick up the formatted version.
...
Claude Code's `UserPromptSubmit` event is intentionally not a separate Zed event â€” `pre_llm_call` fires at the same place and already supports context injection. Use it here.
...
Each unique `(event, command)` pair prompts the user for approval the first time Zed sees it, then persists the decision to `~/.zed/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.

Three escape hatches bypass the interactive prompt â€” any one is sufficient:

1. `--accept-hooks` flag on the CLI (e.g. `zed --accept-hooks chat`)
2. `ZED_ACCEPT_HOOKS=1` environment variable
3. `hooks_auto_accept: true` in `cli-config.yaml`

Non-TTY runs (gateway, cron, CI) need one of these three â€” otherwise any newly-added hook silently stays un-registered and logs a warning.

**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `zed hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.

### The `zed hooks` CLI

| Command | What it does |
|---------|--------------|
| `zed hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `zed hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `zed hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `zed hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |

### Security

Shell hooks run with **your full user credentials** â€” same trust boundary as a cron entry or a shell alias. Treat the `hooks:` block in `config.yaml` as privileged configuration:

- Only reference scripts you wrote or fully reviewed.
- Keep scripts inside `~/.zed/agent-hooks/` so the path is easy to audit.
- Re-run `zed hooks doctor` after you pull a shared config to spot newly-added hooks before they register.
