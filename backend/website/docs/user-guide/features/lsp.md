---
sidebar_position: 16
title: "LSP â€” Semantic Diagnostics"
description: "Real language servers (pyright, gopls, rust-analyzer, â€¦) wired into the post-write lint check used by write_file and patch."
---

# Language Server Protocol (LSP)

Zed runs full language servers â€” pyright, gopls, rust-analyzer,
typescript-language-server, clangd, and ~20 more â€” as background
...
file, it sees exactly the errors that edit introduced â€” not just
...
This is the same architecture top-tier coding agents use. Zed
...
stays dormant â€” useful for messaging gateways where the cwd is the
...
language server can never break a write â€” every LSP failure path
falls back silently to the syntax-only result.

Concretely, on every successful `write_file` or `patch`:

1. Zed captures a baseline of current diagnostics for the file.
...
real language server. Two channels, independent signals â€” the
...
â€¦). Zed auto-detects the binary on PATH or in
`<ZED_HOME>/lsp/bin/`.
...
`node_modules` tree â€” Zed installs both packages together when you
run `zed lsp install typescript` or auto-install fires on first
use.

## CLI

```
zed lsp status          # service state + per-server install status
zed lsp list            # registry, optionally --installed-only
zed lsp install <id>    # eagerly install one server
zed lsp install-all     # try every server with a known recipe
zed lsp restart         # tear down running clients
zed lsp which <id>      # print resolved binary path
```

`zed lsp status` is the best starting point â€” it shows which
...
# config.yaml
lsp:
  # Master toggle. Disabling skips the entire subsystem â€” no servers
  ...
  #   auto    â€” install via npm/pip/go install into <ZED_HOME>/lsp/bin
...
* `disabled: true` â€” skip this server entirely even when its
  extensions match a file.
* `command: [bin, ...args]` â€” pin a custom binary path. Bypasses
  auto-install.
* `env: {KEY: value}` â€” extra env vars passed to the spawned process.
* `initialization_options: {...}` â€” merged into the LSP
  `initializationOptions` payload sent in the `initialize`
  handshake. Server-specific; consult the language server's docs.

## Installation locations

When `install_strategy: auto`, Zed installs binaries into
`<ZED_HOME>/lsp/bin/`. NPM packages land in
`<ZED_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location â€” the staging dir is fully Zed-owned and is
...
budget is `wait_timeout` seconds â€” typically the server responds in
tens of milliseconds for pyright/tsserver and a few seconds for
rust-analyzer mid-indexing.

Servers are kept alive for the life of the Zed process. There's
no idle-timeout reaper â€” the cost of restarting the server's index
...
**`zed lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<ZED_HOME>/lsp/bin/`. Run
`zed lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `zed lsp status`**

Some servers ship as thin wrappers around an external CLI for actual
diagnostics â€” they spawn cleanly and accept requests but never emit
...
When `zed lsp status` shows a `Backend warnings` section, install
...
Check `~/.zed/logs/agent.log` for `[agent.lsp.client]` entries â€”
...
the rest of the session. Run `zed lsp restart` to clear the set;
