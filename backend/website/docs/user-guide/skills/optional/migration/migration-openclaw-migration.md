---
title: "Openclaw Migration â€” Migrate a user's OpenClaw customization footprint into Zed Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Zed Agent"
---
...
Migrate a user's OpenClaw customization footprint into Zed Agent. Imports Zed-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Zed Agent (Zed Team) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Zed`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Zed Migration

Use this skill when a user wants to move their OpenClaw setup into Zed Agent with minimal manual cleanup.
...
**First-time setup:** The `zed setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_zed.py` to:

- import `SOUL.md` into the Zed home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Zed memory entries
- merge OpenClaw command approval patterns into Zed `command_allowlist`
- migrate Zed-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Zed working-directory configuration
- copy OpenClaw skills into `~/.zed/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Zed workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.zed/tts/`
- archive non-secret docs that do not have a direct Zed destination
...
Do not guess a shorter path like `~/.zed/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.zed/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Zed-compatible secrets, currently:
...
Zed CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Zed skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Zed unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Zed targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Zed destination. Unsupported auth blobs must still be reported as skipped.
...
11. Do not end the dry-run summary with a vague â€œWhat would you like to do?â€ if `clarify` is available. Use structured follow-up prompts instead.
...
- Zed persona state imported
- Zed memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.zed/skills/openclaw-imports/`
