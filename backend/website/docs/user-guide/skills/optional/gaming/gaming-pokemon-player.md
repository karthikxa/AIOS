---
title: "Pokemon Player â€” Play Pokemon via headless emulator + RAM reads"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/gaming/pokemon-player` |
...
The repo is zedteam/pokemon-agent on GitHub. Clone it, then
...
with a venv ready â€” just cd there and source .venv/bin/activate.

You also need a ROM file. Ask the user for theirs. On this machine
one exists at roms/pokemon_red.gb inside that directory.
NEVER download or provide ROM files â€” always ask the user.
...
The tunnel URL changes each time â€” give the user the new one if restarted.
...
### Step 1: OBSERVE â€” check state AND take a screenshot
GET /state for position, HP, battle, dialog.
GET /screenshot and save to /tmp/pokemon.png, then use vision_analyze.
Always do BOTH â€” RAM state gives numbers, vision gives spatial awareness.

### Step 2: ORIENT
- Dialog/text on screen â†’ advance it
- In battle â†’ fight or run
- Party hurt â†’ head to Pokemon Center
- Near objective â†’ navigate carefully

### Step 3: DECIDE
Priority: dialog > battle > heal > story objective > training > explore

### Step 4: ACT â€” move 2-4 steps max, then re-check
POST /action with a SHORT action list (2-4 actions, not 10-15).

### Step 5: VERIFY â€” screenshot after every move sequence
...
## Action Reference
- press_a â€” confirm, talk, select
- press_b â€” cancel, close menu
- press_start â€” open game menu
- walk_up/down/left/right â€” move one tile
- hold_b_N â€” hold B for N frames (use for speeding through text)
- wait_60 â€” wait about 1 second (60 frames)
- a_until_dialog_end â€” press A repeatedly until dialog clears
...
- Ledges, fences, signs, building doors, NPCs â€” only visible via screenshot
...
- Do not spam 10-15 movements â€” you will overshoot or get stuck
...
### Decision Tree
1. Want to catch? â†’ Weaken then throw Poke Ball
2. Wild you don't need? â†’ RUN
3. Type advantage? â†’ Use super-effective move
4. No advantage? â†’ Use strongest STAB move
5. Low HP? â†’ Switch or use Potion
...
- Boulder Badge â€” Brock (Rock) â†’ use Water/Grass
- Cascade Badge â€” Misty (Water) â†’ use Grass/Electric
- Thunder Badge â€” Lt. Surge (Electric) â†’ use Ground
- Rainbow Badge â€” Erika (Grass) â†’ use Fire/Ice/Flying
- Soul Badge â€” Koga (Poison) â†’ use Ground/Psychic
- Marsh Badge â€” Sabrina (Psychic) â†’ hardest gym
- Volcano Badge â€” Blaine (Fire) â†’ use Water/Ground
- Earth Badge â€” Giovanni (Ground) â†’ use Water/Grass/Ice
- Elite Four â†’ Champion!
...
- Dialog detection via RAM is unreliable â€” verify with screenshots
