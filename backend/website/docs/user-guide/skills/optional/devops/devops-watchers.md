---
title: "Watchers â€” Poll RSS, JSON APIs, and GitHub with watermark dedup"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/devops/watchers` |
...
The scripts below handle all three. The agent runs them via the terminal tool â€” from a cron job, a webhook, or an interactive chat â€” and reports what's new.

## Ready-made scripts

All three live in `$ZED_HOME/skills/devops/watchers/scripts/` once the skill is installed. Each reads `WATCHER_STATE_DIR` (defaults to `$ZED_HOME/watcher-state/`) for its state file, keyed by the `--name` argument.
...
- First run records a baseline â€” never replays existing feed
- Watermark is a bounded ID set (max 500) to cap memory
- Output format: `## <title>\n<url>\n\n<optional body>` per item
- Empty stdout on no-new â€” the caller treats that as silent
...
Watch a GitHub repo (set `GITHUB_TOKEN` in `~/.zed/.env` to avoid the 60 req/hr anonymous rate limit):
...
Every watcher writes `$ZED_HOME/watcher-state/<name>.json`. Inspect:
...
4. **Putting the state dir where the agent's sandbox can't write.** `$ZED_HOME/watcher-state/` is always writable. Docker/Modal backends may not see arbitrary host paths.
