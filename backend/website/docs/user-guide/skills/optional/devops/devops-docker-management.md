...
description: "Manage Docker containers, images, volumes, networks, and Compose stacks â€” lifecycle ops, debugging, cleanup, and Dockerfile optimization"
---
...
Manage Docker containers, images, volumes, networks, and Compose stacks â€” lifecycle ops, debugging, cleanup, and Dockerfile optimization.

## Skill metadata

| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/devops/docker-management` |
...
- **Container lifecycle** â†’ run, stop, start, restart, rm, pause/unpause
- **Container interaction** â†’ exec, cp, logs, inspect, stats
- **Image management** â†’ build, pull, push, tag, rmi, save/load
- **Docker Compose** â†’ up, down, ps, logs, exec, build, config
- **Volumes & networks** â†’ create, inspect, rm, prune, connect
- **Troubleshooting** â†’ log analysis, exit codes, resource issues
...
docker system prune -a --volumes       # EVERYTHING â€” named volumes too
```
...
- **Container started?** â†’ `docker ps` (check status is "Up")
- **Logs clean?** â†’ `docker logs --tail 20 NAME` (no errors)
- **Port accessible?** â†’ `curl -s http://localhost:PORT` or `docker port NAME`
- **Image built?** â†’ `docker images | grep TAG`
- **Compose stack healthy?** â†’ `docker compose ps` (all services "running" or "healthy")
- **Disk freed?** â†’ `docker system df` (compare before/after)

## Dockerfile Optimization Tips

When reviewing or creating a Dockerfile, suggest these improvements:

1. **Multi-stage builds** â€” separate build environment from runtime to reduce final image size
2. **Layer ordering** â€” put dependencies before source code so changes don't invalidate cached layers
3. **Combine RUN commands** â€” fewer layers, smaller image
4. **Use .dockerignore** â€” exclude `node_modules`, `.git`, `__pycache__`, etc.
5. **Pin base image versions** â€” `node:20-alpine` not `node:latest`
6. **Run as non-root** â€” add `USER` instruction for security
7. **Use slim/alpine bases** â€” `python:3.12-slim` not `python:3.12`
