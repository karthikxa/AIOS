---
title: "Pinggy Tunnel â€” Zero-install localhost tunnels over SSH via Pinggy"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/devops/pinggy-tunnel` |
...
Expose a local service (dev server, webhook receiver, MCP endpoint, demo) to the public internet using a Pinggy SSH reverse tunnel. No daemon to install â€” the user's stock SSH client connects to `a.pinggy.io:443` and Pinggy hands back a public HTTP/HTTPS URL.
...
If the host already has `cloudflared` configured, prefer the `cloudflared-quick-tunnel` skill â€” Cloudflare quick tunnels don't expire after 60 minutes.
...
# TLS tunnel (Pinggy can't decrypt â€” bring your own certs at origin)
...
## Procedure â€” Start a Tunnel and Get the URL
...
If nothing is listening yet, start it first (e.g. `python3 -m http.server 8000 --bind 127.0.0.1`). Pinggy will happily return a URL pointed at nothing â€” the user will see 502 until the origin comes up.
...
If you get `502 Bad Gateway`, the SSH session is up but the local origin isn't listening â€” fix step 1 first.
...
| `x:https` | Force HTTPS â€” auto-redirect HTTP to HTTPS |
...
- **`process(action='log')` may miss SSH banner output.** Pinggy prints the URLs and then the SSH session goes interactive. Always redirect to a logfile and `grep` the file directly â€” same pattern as `cloudflared-quick-tunnel`.
...
- **Pro mode requires the token as the username, not a flag.** Use `"$PINGGY_TOKEN+a.pinggy.io"` (no `free@`). With a token you can also add `:persistent` for a stable subdomain â€” see `pinggy.io/docs/`.

## Recipes

Composite patterns combining a local origin with a Pinggy tunnel. Each recipe is self-contained â€” start the origin, start the tunnel, parse the URL, hand it back to the user.

### Recipe 1 â€” Receive a webhook callback
...
# 2. Tunnel â€” bearer-token-gate so randos can't pollute the capture log
...
### Recipe 2 â€” Expose an MCP server over HTTP/SSE

Use when a remote MCP client (Claude Desktop on another machine, a teammate's editor, etc.) needs to reach an MCP server running on the local box. Only works for MCP servers that speak HTTP transport â€” stdio-mode servers can't be tunneled.
...
# 2. Tunnel with a bearer token â€” MCP traffic should not be open to the internet
...
The remote client connects to `$URL` with `Authorization: Bearer $TOKEN`. Zed' own native MCP client config: `{"transport": "http", "url": "<URL>", "headers": {"Authorization": "Bearer <TOKEN>"}}`.

### Recipe 3 â€” Expose a local LLM endpoint (Ollama / vLLM / llama.cpp)
