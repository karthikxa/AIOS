---
title: "Obliteratus â€” OBLITERATUS: abliterate LLM refusals (diff-in-means)"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/mlops/obliteratus` |
...
Remove refusal behaviors (guardrails) from open-weight LLMs without retraining or fine-tuning. Uses mechanistic interpretability techniques â€” including diff-in-means, SVD, whitened SVD, LEACE concept erasure, SAE decomposition, Bayesian kernel projection, and more â€” to identify and surgically excise refusal directions from model weights while preserving reasoning capabilities.

**License warning:** OBLITERATUS is AGPL-3.0. NEVER import it as a Python library. Always invoke via CLI (`obliteratus` command) or subprocess. This keeps Zed Agent's MIT license clean.

## Video Guide

Walkthrough of OBLITERATUS used by a Zed agent to abliterate Gemma:
...
| Experimental auto-detection       | `informed`         | Auto-detects alignment type â€” experimental, may not always outperform advanced |

### 9 CLI Methods
- **basic** â€” Single refusal direction via diff-in-means. Fast (~5-10 min for 8B).
- **advanced** (DEFAULT, RECOMMENDED) â€” Multiple SVD directions, norm-preserving projection, 2 refinement passes. Medium speed (~10-20 min).
- **aggressive** â€” Whitened SVD + jailbreak-contrastive + attention head surgery. Higher risk of coherence damage.
- **spectral_cascade** â€” DCT frequency-domain decomposition. Research/novel approach.
- **informed** â€” Runs analysis DURING abliteration to auto-configure. Experimental â€” slower and less predictable than advanced.
- **surgical** â€” SAE features + neuron masking + head surgery + per-expert. Very slow (~1-2 hrs). Best for reasoning models.
- **optimized** â€” Bayesian hyperparameter search (Optuna TPE). Longest runtime but finds optimal parameters.
- **inverted** â€” Flips the refusal direction. Model becomes actively willing.
- **nuclear** â€” Maximum force combo for stubborn MoE models. Expert-granular.

### Direction Extraction Methods (--direction-method flag)
- **diff_means** (default) â€” Simple difference-in-means between refused/complied activations. Robust.
- **svd** â€” Multi-direction SVD extraction. Better for complex alignment.
- **leace** â€” LEACE (Linear Erasure via Closed-form Estimation). Optimal linear erasure.

### 4 Python-API-Only Methods
(NOT available via CLI â€” require Python import, which violates AGPL boundary. Mention to user only if they explicitly want to use OBLITERATUS as a library in their own AGPL project.)
...
### Standard usage
```bash
# Default method (advanced) â€” recommended for most models
obliteratus obliterate <model_name> --method advanced --output-dir ./abliterated-models

# With 4-bit quantization (saves VRAM)
obliteratus obliterate <model_name> --method advanced --quantization 4bit --output-dir ./abliterated-models

# Large models (70B+) â€” conservative defaults
...
```bash
# Interactive guided mode (hardware â†’ model â†’ preset)
...
```python
# Python API only â€” for user's own projects
...
- **Embedding Ablation** â€” Target embedding layer components
- **FFN Ablation** â€” Feed-forward network block removal
- **Head Pruning** â€” Attention head pruning
- **Layer Removal** â€” Full layer removal
...
- **CUDA** â€” Full support (NVIDIA GPUs)
- **Apple Silicon (MLX)** â€” Supported via MLX backend
- **CPU** â€” Supported for tiny models (&lt; 1B params)

## YAML Config Templates

Load templates for reproducible runs via `skill_view`:
- `templates/abliteration-config.yaml` â€” Standard single-model config
- `templates/analysis-study.yaml` â€” Pre-abliteration analysis study
- `templates/batch-abliteration.yaml` â€” Multi-model batch processing

## Telemetry

OBLITERATUS can optionally contribute anonymized run data to a global research dataset.
Enable with `--contribute` flag. No personal data is collected â€” only model name, method, metrics.

## Common Pitfalls

1. **Don't use `informed` as default** â€” it's experimental and slower. Use `advanced` for reliable results.
2. **Models under ~1B respond poorly to abliteration** â€” their refusal behaviors are shallow and fragmented, making clean direction extraction difficult. Expect partial results (20-40% remaining refusal). Models 3B+ have cleaner refusal directions and respond much better (often 0% refusal with `advanced`).
3. **`aggressive` can make things worse** â€” on small models it can damage coherence and actually increase refusal rate. Only use it if `advanced` leaves > 10% refusals on a 3B+ model.
4. **Always check perplexity** â€” if it spikes > 15%, the model is damaged. Reduce aggressiveness.
5. **MoE models need special handling** â€” use `nuclear` method for Mixtral, DeepSeek-MoE, etc.
