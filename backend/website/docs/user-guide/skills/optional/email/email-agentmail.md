---
title: "Agentmail â€” Give the agent its own dedicated email inbox via AgentMail"
...
Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. zed-agent@agentmail.to).

## Skill metadata

| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/email/agentmail` |
...
Add to `~/.zed/config.yaml` (paste your actual key â€” MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `zed-agent`)
   - The agent gets address: `zed-agent@agentmail.to`
2. Send an email:
...
- Real-time inbound email (webhooks) requires a public server â€” use `list_threads` polling via cronjob instead for personal use
