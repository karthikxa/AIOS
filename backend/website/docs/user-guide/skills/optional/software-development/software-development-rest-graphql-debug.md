---
title: "Rest Graphql Debug â€” Debug REST/GraphQL APIs: status codes, auth, schemas, repro"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/software-development/rest-graphql-debug` |
...
Drive REST and GraphQL diagnosis through Zed tools â€” `terminal` for `curl`, `execute_code` for Python `requests`, `web_extract` for vendor docs. Isolate the failing layer before guessing at the fix.
...
```
1. Connectivity   â†’ can we reach the host at all?
1.5 Timeouts      â†’ connect-slow vs read-slow?
2. TLS/SSL        â†’ cert valid and trusted?
3. Auth           â†’ credentials correct and unexpired?
4. Request format â†’ payload shape match server expectations?
5. Response parse â†’ does our code accept what came back?
6. Semantics      â†’ does the data mean what we assume?
```
...
In Python, always pass a tuple timeout â€” `requests` has no default and will hang forever:
...
- API key in header vs query param (`?api_key=â€¦`)?

### Step 4 â€” Request Format
...
**Content-Type / body mismatch â€” the silent 415/400:**

```python
# WRONG â€” data= sends form-encoded, header lies
requests.post(url, data='{"k":"v"}', headers={"Content-Type": "application/json"})

# RIGHT â€” json= auto-sets header AND serializes
requests.post(url, json={"k": "v"})

# WRONG â€” Accept says XML, code calls .json()
requests.get(url, headers={"Accept": "text/xml"})

# RIGHT â€” let requests build multipart with boundary
...
### Step 6 â€” Semantic Validation

Parsed cleanly â€” but is the data *correct*?
...
### 401 Unauthorized â€” credentials missing or invalid
...
4. Some APIs use query param (`?api_key=â€¦`) instead of header.
...
### 404 Not Found â€” resource doesn't exist or URL is wrong
...
### 5xx â€” server-side, usually not your fault

- **500** â€” server bug. Capture correlation ID, file with provider.
- **502** â€” upstream down. Backoff + retry.
- **503** â€” overloaded / maintenance. Check status page.
- **504** â€” upstream timeout. Reduce payload or raise timeout.
...
- Offset (`?limit=100&offset=200`) â€” simple, can skip items if data shifts.
- Cursor (`?cursor=abc123`) â€” preferred for live or large datasets.
...
Always capture the provider's request ID â€” fastest path to vendor support:
...
- Never hardcode tokens in scripts. Read from env (`os.environ["API_TOKEN"]`) or `~/.zed/.env`.
...
- [ ] **Credentials in URLs.** API keys in query strings end up in server logs, browser history, referrer headers â€” use headers.
...
### terminal â€” for curl, dig, openssl
...
### execute_code â€” for multi-step Python flows

When debugging spans auth â†’ fetch â†’ paginate â†’ validate, use `execute_code`. Variables persist for the script, results print to stdout, no risk of token spam in your context:
...
### web_extract â€” for vendor API docs
...
### delegate_task â€” for full CRUD test sweeps
...
- `systematic-debugging` â€” once the failing API layer is isolated, root-cause your code
- `test-driven-development` â€” write the regression test before shipping the fix
