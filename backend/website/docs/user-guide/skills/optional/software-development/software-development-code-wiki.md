---
title: "Code Wiki â€” Generate wiki docs + Mermaid diagrams for any codebase"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/software-development/code-wiki` |
| Path | `optional-skills/software-development/code-wiki` |
| Version | `0.1.0` |
| Author | Teknium (teknium1), Zed Agent |
| License | MIT |
...
Invoke through the `terminal` tool from the target repo's root, then use `read_file` / `search_files` / `write_file` to produce the wiki. Default output location is `~/.zed/wikis/<repo-name>/`. Only write into the repo (`docs/wiki/`) when the user explicitly requests it.
...
```bash
OUTPUT_DIR="$HOME/.zed/wikis/$REPO_NAME"
...
`read_file` the actual project README plus the top 2â€“3 entry-point files. Then `write_file`:

````markdown
# <Project Name>

<One paragraph: what it is and what it's for. Self-contained â€” don't assume the
reader has the source README.>

## Key Concepts

- **<Concept 1>** â€” <one line>
- **<Concept 2>** â€” <one line>

## Entry Points

- [`path/to/main.py`](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/software-development/code-wiki/<link>) â€” <what runs when you start it>
- [`path/to/cli.py`](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/software-development/code-wiki/<link>) â€” <CLI surface>

## High-Level Architecture

<2-3 sentences. Detail goes in architecture.md.>

See [architecture.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/software-development/code-wiki/architecture.md).

## Module Map

| Module | Purpose |
|---|---|
| [`<module>`](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/software-development/code-wiki/modules/<module>.md) | <one-line purpose> |

## Getting Started

See [getting-started.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/software-development/code-wiki/getting-started.md).
````
...
- **<Component>** â€” <1-2 sentences>. See [`modules/<module>.md`](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/software-development/code-wiki/modules/<module>.md).
...
1. **<Step>** â€” [`<file>`](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/software-development/code-wiki/<link>)
2. **<Step>** â€” [`<file>`](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/software-development/code-wiki/<link>)
...
For each selected module, inspect its layout with `ls`, identify 3â€“5 most important files (by size, by being named `core.py` / `main.py` / `__init__.py`, by being imported a lot), then `read_file` those files (use `offset` / `limit` to read only what you need; prefer `search_files` for specific symbols).
...
- [`<module>/<file>`](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/software-development/code-wiki/<link>) â€” <what it does>
...
Pick the 5â€“10 most important classes/types. `read_file` them, then write:
...
<Anything the diagram can't express â€” lifecycle, threading, etc.>
````
...
Pick 2â€“4 of the most important workflows. Trace each call path through the code (read entry point, follow function calls), then:
...
1. **User input** â€” [`cli.py:ZedCLI.run_session`](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/software-development/code-wiki/<link>)
2. **Message dispatch** â€” [`run_agent.py:AIAgent.chat`](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/software-development/code-wiki/<link>)
````
...
<From manifest files + README. Be specific â€” versions if pinned.>
...
- `<config-file>` â€” <what it controls>
- Env var `<VAR>` â€” <what it controls>

## Where to Go Next

- Architecture: [architecture.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/software-development/code-wiki/architecture.md)
- Module reference: [README.md#module-map](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/software-development/code-wiki/README.md#module-map)
````
...
cat > "$OUTPUT_DIR/.codewiki-state.json" <<EOF
{
  ...
  "generator": "zed-agent code-wiki skill v0.1.0",
  "modules_documented": []
}
EOF
```
...
```
Generated wiki at ~/.zed/wikis/<repo-name>/:
...
Full incremental-regeneration is a future enhancement â€” for now, regenerating the whole thing is acceptable.
...
- **In-repo output without asking.** Default is `~/.zed/wikis/`. Only write into the repo when the user explicitly requests it.
...
- **GitHub Mermaid theme is fixed** â€” don't include `%%{init: ...}%%` blocks; they're stripped on render.

## Verification

After writing, verify:

1. **Mermaid blocks balance** â€” opens equal closes per file:
   ```bash
   ...
   ```
2. **All expected files exist** â€”
   ```bash
   ls "$OUTPUT_DIR"/{README.md,architecture.md,getting-started.md,.codewiki-state.json} \
      "$OUTPUT_DIR"/modules/ "$OUTPUT_DIR"/diagrams/
   ```
3. **Module count matches what you intended** â€” `ls "$OUTPUT_DIR/modules" | wc -l` should equal the number of modules you committed to in Step 3.
4. **No fabricated paths** â€” sanity-check 2â€“3 source links resolve to real files.
