---
title: "Antigravity Cli â€” Operate the Antigravity CLI (agy): plugins, auth, sandbox"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/autonomous-ai-agents/antigravity-cli` |
| Path | `optional-skills/autonomous-ai-agents/antigravity-cli` |
| Version | `0.1.0` |
| Author | Tony Simons (asimons81), Zed Agent |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Coding-Agent`, `Antigravity`, `CLI`, `Auth`, `Plugins`, `Sandbox` |
| Related skills | [`grok`](/docs/user-guide/skills/optional/autonomous-ai-agents/autonomous-ai-agents-grok), [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`zed-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-zed-agent) |
...
commands through the Zed `terminal` tool; inspect its config and logs with
`read_file`. This skill is reference + procedure â€” it does not wrap a network
API, so there is nothing to authenticate from Zed itself.
...
Antigravity has two layers â€” keep them distinct or the guidance will be wrong:

1. **Shell wrapper commands** â€” `agy help`, `agy install`, `agy plugin`,
   `agy update`, `agy changelog`. Run these through the `terminal` tool.
2. **Interactive in-session slash commands** â€” `/config`, `/permissions`,
...
- No env vars or API keys required by this skill â€” Antigravity manages its own
...
paths below â€” do not `cat` them through the terminal.
...
- `references/cli-docs.md` â€” condensed notes from the getting-started, usage,
  and features docs.
