---
title: "Grok â€” Delegate coding to xAI Grok Build CLI (features, PRs)"
sidebar_label: "Grok"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/autonomous-ai-agents/grok` |
| Path | `optional-skills/autonomous-ai-agents/grok` |
| Version | `0.1.0` |
| Author | Matt Maximo (MattMaximo), Zed Agent |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Coding-Agent`, `Grok`, `xAI`, `Code-Review`, `Refactoring`, `Automation` |
| Related skills | [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`zed-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-zed-agent) |
...
# Grok Build CLI â€” Zed Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Zed terminal. Grok
...
pattern is nearly identical â€” **prefer headless `-p` for one-shots**, use a PTY
...
- **Auth â€” SuperGrok / X Premium+ subscription (primary path):**
  - Run `grok login` once â†’ opens a browser for OAuth â†’ token cached in
...
- **No git repo required** â€” unlike Codex, Grok runs fine outside a git
  directory (good for scratch/throwaway tasks).
- **Claude Code / AGENTS.md compatible with zero config** â€” Grok auto-reads
...
### Mode 1: Headless (`-p`) â€” Non-Interactive (PREFERRED)

Runs a one-shot task, prints the result, and exits. No PTY, no interactive
dialogs to navigate. This is the cleanest integration path â€” the analog of
...
- `plain` â€” human-readable text (default)
- `json` â€” one JSON object at the end of the run (parse the result cleanly)
- `streaming-json` â€” newline-delimited JSON events as they arrive
...
2. **Don't conflate Zed' xAI auth with the `grok` CLI's auth.** Zed'
   ...
   `grok` is logged in.
3. **Always pass `--no-auto-update` in automation** â€” otherwise Grok phones home
   for update checks (and `x.ai`/`storage.googleapis.com` may be unreachable).
4. **Prefer npm install over the curl installer** â€” `npm install -g
...
8. **No git repo needed**, but for PR/commit workflows you still want one â€” use
...
1. **Prefer headless `-p`** for single tasks â€” cleanest integration, structured
...
7. **Verify auth before relying on it** â€” check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Zed' xAI auth carries
   over.
8. **Report results to the user** â€” summarize what Grok changed and what's left.
