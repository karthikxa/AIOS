...
description: "Configure and use Honcho memory with Zed -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Zed -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/autonomous-ai-agents/honcho` |
...
| Related skills | [`zed-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-zed-agent) |
...
# Honcho Memory for Zed

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Zed profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/zed#running-honcho-locally-with-zed
...
3. **AI peer card** -- the identity card for this Zed profile's AI peer
...
Honcho models conversations as interactions between **peers**. Zed creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Zed instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Zed run |
| `global` | Single session across all directories |

Manual override: `zed honcho map my-project-name`
...
| `dialecticCadence` | `2` | Min turns between dialectic API calls. Recommended 1â€“5 |
...
Each Zed profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
1. Creates a `zed.coder` host block in `honcho.json`
...
Guidelines for Zed when Honcho memory is active.
...
Config file: `$ZED_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
| `contextCadence` | `1` | Min turns between context API calls |
| `dialecticCadence` | `2` | Min turns between dialectic LLM calls (recommended 1â€“5) |
...
Run `zed honcho setup`. Ensure `memory.provider: honcho` is in `~/.zed/config.yaml`.

### Memory not persisting across sessions
Check `zed honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `zed profile create <name> --clone`. For existing profiles: `zed honcho sync`.
...
| Command | Description |
|---------|-------------|
| `zed honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `zed honcho status` | Show resolved config, connection test, peer info for active profile |
| `zed honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `zed honcho disable` | Disable Honcho for the active profile |
| `zed honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `zed honcho peers` | Show peer identities across all profiles |
| `zed honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `zed honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `zed honcho sessions` | List known directory-to-session-name mappings |
| `zed honcho map <name>` | Map current working directory to a Honcho session name |
| `zed honcho identity` | Seed AI peer identity or show both peer representations |
| `zed honcho sync` | Create host blocks for all Zed profiles that don't have one yet |
| `zed honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Zed + Honcho |
| `zed memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `zed memory status` | Show active memory provider and config |
| `zed memory off` | Disable external memory provider |
