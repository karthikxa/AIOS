...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/mcp/mcporter` |
...
# Machine-readable output (recommended for Zed)
...
- Ad-hoc servers (HTTP URL or `--stdio` command) work without any config â€” useful for one-off calls
- OAuth auth may require interactive browser flow â€” use `terminal(command="mcporter auth <server>", pty=true)` if needed
