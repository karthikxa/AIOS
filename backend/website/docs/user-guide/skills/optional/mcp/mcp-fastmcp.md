---
title: "Fastmcp â€” Build, test, inspect, install, and deploy MCP servers with FastMCP in Python"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Zed or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Zed. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
When the goal is Zed integration, either:

- configure the server in `~/.zed/config.yaml` using the `native-mcp` skill, or
...
### Zed cannot see the deployed server

The server-building part may be correct while the Zed config is not. Load the `native-mcp` skill and configure the server in `~/.zed/config.yaml`, then restart Zed.
