...
description: "Build leveraged buyout models in Excel â€” sources & uses, debt schedule, cash sweep, exit multiple, IRR/MOIC sensitivity"
---
...
Build leveraged buyout models in Excel â€” sources & uses, debt schedule, cash sweep, exit multiple, IRR/MOIC sensitivity. Pairs with excel-author. Use for PE screening, sponsor-case valuation, or illustrative LBO in a pitch.

## Skill metadata

| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/finance/lbo-model` |
...
## CRITICAL INSTRUCTIONS â€” READ FIRST
...
* **Work section by section, verify with user at each step** - Complete one section fully, show the user what was built, run the section's verification checks, and get confirmation BEFORE moving to the next section. Do NOT build the entire model end-to-end and then present it â€” later sections depend on earlier ones, so catching a mistake in Sources & Uses after the returns are already built means rework everywhere.
...
### Fill Color Palette â€” Professional Blues & Greys (Default unless user/template specifies otherwise)
* **Keep it minimal** â€” only use blues and greys for cell fills. Do NOT introduce greens, yellows, reds, or multiple accents. A professional LBO model uses restraint.
* **Default fill palette:**
  ...
  * **Input cells**: Light grey `#F2F2F2` (or just white) â€” the blue *font* is the signal, fill is secondary
...
* Note: The blue/black/purple/green **font** colors above are for distinguishing inputs vs formulas vs links. Those are separate from the **fill** palette here â€” both work together.
...
* Pattern: Interest â†’ Cash Flow â†’ Paydown â†’ Ending Balance (if interest uses ending balance, this circles back)
...
### Sensitivity Tables
* **Use ODD dimensions** (5Ã—5 or 7Ã—7) â€” never 4Ã—4 or 6Ã—6. Odd dimensions guarantee a true center cell.
* **Center cell = base case.** Build the row and column axis values symmetrically around the model's actual assumptions (e.g., if base entry multiple = 10.0x, axis = `[8.0x, 9.0x, 10.0x, 11.0x, 12.0x]`). The center cell's IRR/MOIC MUST then equal the model's actual IRR/MOIC output â€” this is the proof the table is wired correctly.
* **Highlight the center cell** â€” medium-blue fill (`#BDD7EE`) + bold font so the base case is visually anchored.
* Excel's DATA TABLE function may not work with openpyxl â€” instead write explicit formulas that reference row/column headers
* Each cell should show a DIFFERENT value â€” if all same, formulas aren't varying correctly
...
- [ ] Grid dimensions are ODD (5Ã—5 or 7Ã—7) â€” there is a true center cell
- [ ] Row and column axis values are symmetric around the base case (`[base-2Î”, base-Î”, base, base+Î”, base+2Î”]`)
- [ ] Center cell output equals the model's actual IRR/MOIC â€” confirms the table is wired correctly
...
- [ ] Values move in expected directions (higher exit multiple â†’ higher IRR, etc.)
...
| Roll-forwards don't tie | Beginning â‰  prior ending | Verify links between periods |
...
## WORKING WITH THE USER â€” SECTION-BY-SECTION CHECKPOINTS
...
* **After completing each major section**, STOP and verify with the user before continuing:
  - **After Sources & Uses** â†’ show the balanced table, confirm the plug is correct, get sign-off before building the operating model
  - **After Operating Model / Projections** â†’ show the projected P&L, confirm growth rates and margins look right, get sign-off before the debt schedule
  - **After Debt Schedule** â†’ show beginning/ending balances and interest, confirm the waterfall logic, get sign-off before returns
  - **After Returns (IRR/MOIC)** â†’ show the cash flow series and outputs, confirm signs and ranges, get sign-off before sensitivity tables
  - **After Sensitivity Tables** â†’ show that each cell varies, confirm the base case lands where expected
...
* **Never present a completed model without having checked in at each section** â€” it's faster to catch a wrong cell reference at the source than to trace it backwards from a broken IRR
