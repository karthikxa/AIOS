...
description: "Build institutional-quality DCF valuation models in Excel â€” revenue projections, FCF build, WACC, terminal value, Bear/Base/Bull scenarios, 5x5 sensitivity t..."
---
...
Build institutional-quality DCF valuation models in Excel â€” revenue projections, FCF build, WACC, terminal value, Bear/Base/Bull scenarios, 5x5 sensitivity tables. Pairs with excel-author. Use for intrinsic-value equity analysis.

## Skill metadata

| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/finance/dcf-model` |
| Path | `optional-skills/finance/dcf-model` |
| Version | `1.0.0` |
| Author | Anthropic (adapted by Zed Team) |
| License | Apache-2.0 |
...
This skill assumes **headless openpyxl** â€” you are producing an .xlsx file on disk.
...
- Every projection, margin, discount factor, PV, and sensitivity cell MUST be a live Excel formula â€” never a value computed in Python and written as a number
...
- If you catch yourself computing something in Python and writing the result â€” STOP. The model must flex when the user changes an assumption.

**Verify Step-by-Step With the User (DO NOT build end-to-end):**
- After data retrieval â†’ show the user the raw inputs block (revenue, margins, shares, net debt) and confirm before projecting
- After revenue projections â†’ show the projected top line and growth rates, confirm before building margin build
- After FCF build â†’ show the full FCF schedule, confirm logic before computing WACC
- After WACC â†’ show the calculation and inputs, confirm before discounting
- After terminal value + PV â†’ show the equity bridge (EV â†’ equity value â†’ per share), confirm before sensitivity tables
- Catch errors at each stage â€” a wrong margin assumption discovered after sensitivity tables are built means rebuilding everything downstream

**Sensitivity Tables:**
- **Use an ODD number of rows and columns** (standard: 5Ã—5, sometimes 7Ã—7) â€” this guarantees a true center cell
- **Center cell = base case.** Build the axis values so the middle row header and middle column header exactly equal the model's actual assumptions (e.g., if base WACC = 9.0%, the middle row is 9.0%; if terminal g = 3.0%, the middle column is 3.0%). The center cell's output must therefore equal the model's actual implied share price â€” this is the sanity check that the table is built correctly.
...
- Populate ALL cells (typically 3 tables Ã— 25 cells = 75) with full DCF recalculation formulas
...
**Formula structure:**
- Revenue(Year N) = Revenue(Year N-1) Ã— (1 + Growth Rate)
...
**IMPORTANT**: These are NOT Excel's "Data Table" feature. These are simple grids where you write regular formulas using openpyxl. Yes, this means ~75 formulas total (3 tables Ã— 25 cells each), but this is straightforward and required.
...
**Table Structure â€” 5Ã—5 grid (ODD dimensions, base case centered):**
...
**â˜… = the center cell.** Its formula output MUST equal the model's actual implied share price (from the valuation summary). Apply the medium-blue fill (`#BDD7EE`) and bold font to this cell so the base case is visually anchored.

**Rule for axis values:** `axis_values = [base - 2*step, base - step, base, base + step, base + 2*step]` â€” symmetric around the base, odd count guarantees a center.
...
- âŒ "Sensitivity tables need Excel's Data Table feature" (NO - that's a specific Excel tool we can't use)
- âœ… "Sensitivity tables are simple grids with formulas in each cell" (YES - this is what we build)
...
4. Now formulas point to wrong cells â†’ #REF! errors
...
1. **Formula row references off** â†’ Define ALL row positions BEFORE writing formulas
2. **Missing cell comments** â†’ Add comments AS cells are created, not at end
3. **Simplified sensitivity tables** â†’ Populate all cells with full DCF recalc formulas, not approximations
4. **Scenario block references wrong** â†’ Ensure IF formulas pull from correct Bear/Base/Bull blocks
5. **No borders** â†’ Add professional section borders for client-ready appearance
...
**Layer 2: Fill Colors â€” Professional Blue/Grey Palette (Default unless user specifies otherwise)**
- **Keep it minimal** â€” use only blues and greys for fills. Do NOT introduce greens, yellows, oranges, or multiple accent colors. A model with too many colors looks amateurish.
- **Default fill palette:**
  ...
  - **Input cells**: Light grey (RGB: 242,242,242 / `#F2F2F2`) background with blue font â€” or just white with blue font if you want maximum minimalism
...
- **That's it â€” 3 blues + 1 grey + white.** Resist the urge to add more.
...
**TERMINOLOGY REMINDER**: "Sensitivity tables" = simple 2D grids with row headers, column headers, and formulas in each data cell. NOT Excel's "Data Table" feature (Data â†’ What-If Analysis â†’ Data Table). You will use openpyxl to write regular Excel formulas into each cell.
...
**If you encounter errors or unreasonable results, read [TROUBLESHOOTING.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/finance/dcf-model/TROUBLESHOOTING.md) for detailed debugging guidance.**
...
3. **Check output**:
   - If `status` is `"success"` â†’ Continue to step 4
   - If `status` is `"errors_found"` â†’ Check `error_summary` and read [TROUBLESHOOTING.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/finance/dcf-model/TROUBLESHOOTING.md) for debugging guidance
...
## Data sources â€” MCP first, web fallback

Many passages below say "use the S&P Kensho MCP / Daloopa MCP / FactSet MCP". Those are commercial financial-data MCPs from the original Cowork plugin context. In Zed:

- **If you have any structured financial-data MCP configured** (Zed supports MCP â€” see `native-mcp` skill), prefer it for point-in-time comps, precedent transactions, and filings.
