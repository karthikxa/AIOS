...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/finance/3-statement-model` |
...
## âš ï¸ CRITICAL PRINCIPLES â€” Read Before Populating Any Template

**Formulas over hardcodes (non-negotiable):**
- Every projection cell, roll-forward, linkage, and subtotal MUST be an Excel formula â€” never a pre-computed value
...
- If you find yourself computing a value in Python and writing the result to a cell â€” STOP. Write the formula instead.
...
1. **After mapping the template** â†’ show the user which tabs/sections you've identified and confirm before touching any cells
2. **After populating historicals** â†’ show the user the historical block and confirm values/periods match source data
3. **After building IS projections** â†’ run the subtotal checks, show the user the projected IS, confirm before moving to BS
4. **After building BS** â†’ show the user the balance check (Assets = L+E) for every period, confirm before moving to CF
5. **After building CF** â†’ show the user the cash tie-out (CF ending cash = BS cash), confirm before finalizing
6. **Do NOT populate the entire model end-to-end and present it complete** â€” break at each statement, show the work, catch errors early

## Formatting â€” Professional Blue/Grey Palette (Default unless template/user specifies otherwise)

**Keep colors minimal.** Use only blues and greys for cell fills. Do NOT introduce greens, yellows, oranges, or multiple accent colors â€” a clean model uses restraint.
...
- Review existing named ranges in the template (Formulas â†’ Name Manager in Excel)
...
If the template specifically requires pulling data from SEC filings (10-K, 10-Q), see [references/sec-filings.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/finance/3-statement-model/references/sec-filings.md) for detailed extraction guidance. This reference is only needed when populating templates with public company data from regulatory filings.
...
- Use Excel's Trace Precedents/Dependents (Formulas â†’ Trace Precedents) to understand cell relationships
- Check for named ranges that may control key inputs (Formulas â†’ Name Manager)

**Map the Template's Flow**
- Identify which tabs feed into others (e.g., Assumptions â†’ IS â†’ BS â†’ CF)
...
| Check Type | Method |
|------------|--------|
| Trace precedents | Select a formula cell â†’ Formulas â†’ Trace Precedents to verify it references correct inputs |
| Trace dependents | Verify key inputs flow to expected output cells |
| Evaluate formula | Use Formulas â†’ Evaluate Formula to step through complex calculations |
...
See [references/formulas.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/finance/3-statement-model/references/formulas.md) for all formula details.
...
| Equity Financing | Î”Common Stock/APIC (BS) - Equity Issuance (CFF) | = 0 |
...
| CFO | Î”AR (increase) | Negative (use of cash) |
| CFO | Î”AP (increase) | Positive (source of cash) |
| CFI | CapEx | Negative |
...
Interest expense creates circularity: Interest â†’ Net Income â†’ Cash â†’ Debt Balance â†’ Interest

Enable iterative calculation in Excel: File â†’ Options â†’ Formulas â†’ Enable iterative calculation. Set maximum iterations to 100, maximum change to 0.001. Add a circuit breaker toggle in Assumptions tab.
...
- SBC ties to IS
- Î”AR, Î”Inventory, Î”AP tie to WC schedule
...
- Equity Raise Tie-Out: Î”Common Stock/APIC (BS) = Equity Issuance (CFF) (must = 0)
...
- NOL utilization â‰¤ 80% of EBT (post-2017 federal limitation)
- NOL balance is non-negative (cannot utilize more than available)
- NOL generated only when EBT &lt; 0
- Tax expense = 0 when taxable income â‰¤ 0
...
- If all sections pass â†’ "âœ“ ALL CHECKS PASS"
- If any section fails â†’ "âœ— ERRORS DETECTED - REVIEW BELOW"
