...
description: "Build auditable Excel workbooks headless with openpyxl â€” blue/black/green cell conventions, formulas over hardcodes, named ranges, balance checks, sensitivit..."
---
...
Build auditable Excel workbooks headless with openpyxl â€” blue/black/green cell conventions, formulas over hardcodes, named ranges, balance checks, sensitivity tables. Use for financial models, audit outputs, reconciliations.

## Skill metadata

| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/finance/excel-author` |
...
Adapted from Anthropic's `xlsx-author` and `audit-xls` skills in the [anthropics/financial-services](https://github.com/anthropics/financial-services) repo. The MCP / Office-JS / Cowork-specific branches of the originals are dropped â€” this skill assumes headless Python.
...
- **Blue** (`Font(color="0000FF")`) â€” hardcoded input a human entered. Revenue drivers, WACC inputs, terminal growth, market data.
- **Black** (default) â€” formula. Every derived cell is a live Excel formula.
- **Green** (`Font(color="006100")`) â€” link to another sheet or external file.
...
```python
# WRONG â€” silent bug waiting to happen
ws["D20"] = revenue_prior_year * (1 + growth)

# CORRECT â€” flexes when the user changes the assumption
ws["D20"] = "=D19*(1+$B$8)"
```
...
3. Current market data (share price, debt balance) â€” with a cell comment documenting source + date
...
- **Odd number of rows/cols** (5Ã—5 or 7Ã—7) â€” guarantees a true center cell.
...
- Populate every cell with a full recalculation formula â€” never an approximation.
...
Checkpoint pattern:
- After Inputs block â†’ show raw inputs, confirm before projecting
- After Revenue projections â†’ confirm top line + growth
- After FCF build â†’ confirm the full schedule
- After WACC â†’ confirm inputs
- After valuation â†’ confirm the equity bridge
- THEN build sensitivity tables

## When NOT to use this skill

- Users in a live Excel session with an Office MCP available â€” drive their live workbook instead.
- Pure tabular data export with no formulas â€” `csv` or `pandas.to_excel` is simpler.
- Dashboards / charts with heavy interactivity â€” use a real BI tool.
