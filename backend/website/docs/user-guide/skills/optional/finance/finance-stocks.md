---
title: "Stocks â€” Stock quotes, history, search, compare, crypto via Yahoo"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/finance/stocks` |
| Path | `optional-skills/finance/stocks` |
| Version | `0.1.0` |
| Author | Mibay (Mibayy), Zed Agent |
| License | MIT |
...
`history`, `compare`, `crypto`. Python stdlib only â€” no API key, no pip
...
All output is JSON on stdout â€” pipe through `jq` if you want to slice it.
...
- Yahoo Finance's API is unofficial. Endpoints can change or rate-limit
  without notice â€” if requests start failing, that's why.
...
- This is read-only â€” no order placement, no account integration.
