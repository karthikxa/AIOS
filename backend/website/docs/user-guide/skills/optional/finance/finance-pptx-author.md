---
title: "Pptx Author â€” Build PowerPoint decks headless with python-pptx"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/finance/pptx-author` |
...
Adapted from Anthropic's `pptx-author` and `pitch-deck` skills in [anthropics/financial-services](https://github.com/anthropics/financial-services). The MCP / Office-JS branches of the originals are dropped â€” this assumes headless Python.
...
Never transcribe numbers from memory or from a summary â€” open the workbook, read the named range, and bind the deck value to it programmatically when you can.
...
Remember to recalculate the workbook before reading it â€” openpyxl only sees computed values if something has already calculated the sheet. Run the recalc helper in the `excel-author` skill first, or open/save through a real Excel session.
...
7. Valuation summary (football field) â€” the money slide
...
- Users in a live PowerPoint session with an Office MCP available â€” drive their live doc instead.
- Non-financial slideware (quarterly all-hands, marketing decks) â€” use the broader `powerpoint` skill.
- Decks with heavy animation, transitions, or speaker notes â€” use the broader `powerpoint` skill.
