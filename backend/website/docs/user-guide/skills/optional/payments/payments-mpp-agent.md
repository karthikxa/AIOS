---
title: "Mpp Agent â€” Pay HTTP 402 APIs via Machine Payments Protocol (MPP)"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/payments/mpp-agent` |
...
Wraps the Machine Payments Protocol (MPP, https://mpp.dev) clients so Zed can pay for per-request API access against servers that respond with `HTTP 402 Payment Required`.
...
- A merchant API returns `HTTP 402` with a `www-authenticate` header â€” and the user wants to actually pay it, not just log the response.
...
- A Stripe Link spend has produced a Shared Payment Token (SPT) and the agent needs to attach it to the 402 challenge â€” in that flow, prefer `link-cli mpp pay` (see the `stripe-link-cli` skill).
...
Store the resulting account credentials wherever the CLI tells you (the CLI writes them under its own config â€” do not paste them into the agent transcript).
...
- **`HTTP 402` without `method="stripe"` cannot be paid by Stripe Link.** If the challenge advertises only Tempo / other methods, use `mppx` (or whichever wallet matches) â€” Link will reject it. Conversely, if it advertises `method="stripe"`, prefer Link via the `stripe-link-cli` skill so the spend goes through the user's approved card.
- **Multiple challenges in one header.** `www-authenticate` may list several methods (e.g. `tempo, stripe`). The Link CLI's `mpp decode` will pick the Stripe one; `mppx` will pick Tempo. There's no single "right" client â€” pick by which wallet the user has funded.
...
- **Server-side MPP is a different skill.** If the user wants to ADD 402 to their own API, this skill is wrong â€” point them at https://mpp.dev/quickstart/server and the `mppx/nextjs` / `mppx/hono` / `mppx/express` / `mppx/elysia` middlewares. A dedicated `mpp-server` skill may land later.
