---
title: "Stripe Projects â€” Provision SaaS services + sync creds via Stripe Projects"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/payments/stripe-projects` |
...
Wraps the [Stripe Projects](https://projects.dev) CLI plugin so Zed can provision SaaS services (Neon, Twilio, Vercel, etc.), generate and sync credentials into the user's `.env`, and manage billing across providers from one place.
...
Lists every provider Stripe Projects supports â€” databases, hosting, auth, AI, analytics, messaging, etc.
...
- **Per-project state.** `.projects/vault/vault.json` is per-project. Provisioning the same service in two different projects creates two separate resources â€” and two bills.
...
- **Credentials in vault are encrypted but `.env` is plaintext.** Standard `.env` hygiene applies â€” never commit it.
