---
title: "Stripe Link Cli â€” Agent payments via Stripe Link â€” cards, SPT, approvals"
sidebar_label: "Stripe Link Cli"
description: "Agent payments via Stripe Link â€” cards, SPT, approvals"
---
...
Agent payments via Stripe Link â€” cards, SPT, approvals.

## Skill metadata

| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/payments/stripe-link-cli` |
...
Wraps [@stripe/link-cli](https://github.com/stripe/link-cli) so Zed can complete purchases on the user's behalf using one-time-use virtual cards or Shared Payment Tokens (SPT). Every spend is gated by an in-app approval in the Link mobile/web app â€” Zed cannot self-approve.

US-only at the moment (Link account requirement). Windows is not supported by the upstream CLI â€” this skill is gated `[linux, macos]`.
...
If the user wants a paid API call (HTTP 402, no checkout form), the `card` path is wrong â€” use SPT via this same skill, or hand off to the `mpp-agent` skill.
...
The Link account, payment method, and spend-approval app do NOT need to be set up before Zed attempts to pay â€” the CLI walks the user through them on first run:

- A Link account at https://app.link.com â€” created/linked during first `link-cli` auth
- At least one payment method â€” added during first run at https://app.link.com/wallet
- The Link mobile/web app â€” opened to approve the first spend request when it's made

No env vars required â€” auth state is stored locally by the CLI under its own config directory.
...
All commands run through the `terminal` tool. The CLI auto-detects non-TTY callers and emits compact `toon` output by default â€” fine for the model. Pass `--format json` if a step needs structured fields.
...
```
link-cli auth login --client-name "Zed" --interval 5 --timeout 300
```
...
| Returns HTTP 402 without `method="stripe"` | unsupported â€” stop |
...
### 5. Retrieve the credential â€” SECURELY
...
`@stripe/link-cli --mcp` exposes the same commands as MCP tools over stdio. To register it with Zed' native MCP:

```
zed mcp add stripe-link --command "npx" --args "@stripe/link-cli --mcp"
```

Then `zed mcp list` should show `stripe-link`. The same approval rules apply â€” MCP doesn't bypass the Link app approval step.

## Pitfalls

- **US-only.** Outside the US, `auth login` will fail. Tell the user, don't keep retrying.
- **Card PAN must never enter agent context.** Use `--output-file` every time. If you've already retrieved without it, immediately `link-cli auth logout` is not enough â€” the card is one-time-use but rotate hygiene matters.
