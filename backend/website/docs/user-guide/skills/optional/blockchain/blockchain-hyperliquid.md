---
title: "Hyperliquid â€” Hyperliquid market data, account history, trade review"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain/hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Zed Agent |
| License | MIT |
...
Read-only â€” no API key, no signing, no order placement.
...
Stdlib only â€” no external packages, no API key.

The script reads `~/.zed/.env` for two optional defaults:

- `HYPERLIQUID_API_URL` â€” defaults to `https://api.hyperliquid.xyz`. Set to
  `https://api.hyperliquid-testnet.xyz` for testnet.
- `HYPERLIQUID_USER_ADDRESS` â€” default address for `state`, `spot-balances`,
...
Helper script: `~/.zed/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `~/.zed/.env`.
...
or windows â†’ pull `fills` and `orders` for that period â†’ pull `candles`
and `funding` for each traded coin â†’ judge decision quality separately
...
- `fills --hours ...` uses `userFillsByTime`, which only exposes a
  recent rolling window â€” not full archive history.
