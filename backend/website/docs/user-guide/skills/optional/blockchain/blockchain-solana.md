...
description: "Query Solana blockchain data with USD pricing â€” wallet balances, token portfolios with values, transaction details, NFTs, whale detection, and live network s..."
---
...
Query Solana blockchain data with USD pricing â€” wallet balances, token portfolios with values, transaction details, NFTs, whale detection, and live network stats. Uses Solana RPC + CoinGecko. No API key required.

## Skill metadata

| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/blockchain/solana` |
| Path | `optional-skills/blockchain/solana` |
| Version | `0.2.0` |
| Author | Deniz Alagoz (gizdusum), enhanced by Zed Agent |
| License | MIT |
...
Helper script path: ~/.zed/skills/blockchain/solana/scripts/solana_client.py
...
Flags:
- `--limit N` â€” show top N tokens (default: 20)
- `--all` â€” show all tokens, no dust filter, no limit
- `--no-prices` â€” skip CoinGecko price lookups (faster, RPC-only)
...
Note: scans the latest block only â€” point-in-time snapshot, not historical.
...
- **CoinGecko rate-limits** â€” free tier allows ~10-30 requests/minute.
  Price lookups use 1 request per token. Wallets with many tokens may
  not get prices for all of them. Use `--no-prices` for speed.
- **Public RPC rate-limits** â€” Solana mainnet public RPC limits requests.
  For production use, set SOLANA_RPC_URL to a private endpoint
  (Helius, QuickNode, Triton).
- **NFT detection is heuristic** â€” amount=1 + decimals=0. Compressed
  NFTs (cNFTs) and Token-2022 NFTs won't appear.
- **Whale detector scans latest block only** â€” not historical. Results
  vary by the moment you query.
- **Transaction history** â€” public RPC keeps ~2 days. Older transactions
  may not be available.
- **Token names** â€” ~25 well-known tokens are labeled by name. Others
  show abbreviated mint addresses. Use the `token` command for full info.
- **Retry on 429** â€” both RPC and CoinGecko calls retry up to 2 times
