---
title: "Oss Forensics â€” Supply chain investigation, evidence recovery, and forensic analysis for GitHub repositories"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/security/oss-forensics` |
...
## âš ï¸ Anti-Hallucination Guardrails
...
5. **Proof-Required Disproval**: A hypothesis cannot be dismissed without a specific, evidence-backed counter-argument. "No evidence found" is not sufficient to disproveâ€”it only makes a hypothesis inconclusive.
...
> resolve `SKILL_DIR` to the actual path â€” e.g. `~/.zed/skills/security/oss-forensics/`
...
**Reference**: See [evidence-types.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/security/oss-forensics/references/evidence-types.md) for IOC taxonomy.
...
- Each dangling commit SHA â†’ type: `git`
- Force-push evidence (reflog showing history rewrite) â†’ type: `git`
- Unsigned commits from verified contributors â†’ type: `git`
- Suspicious binary file additions â†’ type: `git`

**Reference**: See [recovery-techniques.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/security/oss-forensics/references/recovery-techniques.md) for accessing force-pushed commits.
...
- PR exists in archive but missing from API â†’ evidence of deletion
- Contributor in archive events but not in contributors list â†’ evidence of permission revocation
- Commit in archive PushEvents but not in API commit list â†’ evidence of force-push/deletion

**Reference**: See [evidence-types.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/security/oss-forensics/references/evidence-types.md) for GH event types.
...
**Reference**: See [github-archive-guide.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/security/oss-forensics/references/github-archive-guide.md) for CDX API parameters.
...
# payload.distinct_size = 0 AND payload.size > 0 â†’ force push indicator
...
**Reference**: See [github-archive-guide.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/security/oss-forensics/references/github-archive-guide.md) for all 12 event types and query patterns.
...
- Recover force-pushed commits using 3 methods (see [recovery-techniques.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/security/oss-forensics/references/recovery-techniques.md))
...
**Common hypothesis templates** (see [investigation-templates.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/security/oss-forensics/references/investigation-templates.md)):
...
2. Verify each ID exists in `evidence.json` (hard failure if any ID is missing â†’ hypothesis rejected as potentially fabricated).
...
Populate `investigation-report.md` using the template in [forensic-report.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/security/oss-forensics/templates/forensic-report.md).
...
This skill is designed for **defensive security investigation** â€” protecting open-source software from supply chain attacks. It must not be used for:

- **Harassment or stalking** of contributors or maintainers
- **Doxing** â€” correlating GitHub activity to real identities for malicious purposes
- **Competitive intelligence** â€” investigating proprietary or internal repositories without authorization
- **False accusations** â€” publishing investigation results without validated evidence (see anti-hallucination guardrails)
...
- For paginated endpoints, fetch all pages in sequence â€” don't parallelize against the same endpoint
- Check `X-RateLimit-Remaining` header; if below 100, pause for `X-RateLimit-Reset` timestamp
- BigQuery has its own quotas (10 TiB/day free tier) â€” always dry-run first
...
- [github-archive-guide.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/security/oss-forensics/references/github-archive-guide.md) â€” BigQuery queries, CDX API, 12 event types
- [evidence-types.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/security/oss-forensics/references/evidence-types.md) â€” IOC taxonomy, evidence source types, observation types
- [recovery-techniques.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/security/oss-forensics/references/recovery-techniques.md) â€” Recovering deleted commits, PRs, issues
- [investigation-templates.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/security/oss-forensics/references/investigation-templates.md) â€” Pre-built hypothesis templates per attack type
- [evidence-store.py](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/security/oss-forensics/scripts/evidence-store.py) â€” CLI tool for managing the evidence JSON store
- [forensic-report.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/security/oss-forensics/templates/forensic-report.md) â€” Structured report template
