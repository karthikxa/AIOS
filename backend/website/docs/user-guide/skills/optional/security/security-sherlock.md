---
title: "Sherlock â€” OSINT username search across 400+ social networks"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/security/sherlock` |
...
- **Do NOT** try multiple installation methods â€” pick one and proceed
...
- "Find accounts for nasa" â†’ username is `nasa`
- "Search for johndoe123" â†’ username is `johndoe123`
- "Check if alice exists on social media" â†’ username is `alice`
- "Look up user bob on social networks" â†’ username is `bob`
...
When extracting, take the **exact** username as stated â€” preserve case, numbers, underscores, etc.
...
- `--nsfw` â€” Include NSFW sites (only if user asks)
- `--tor` â€” Route through Tor (only if user asks for anonymity)

**Do NOT ask about options via clarify** â€” just run the default search. Users can request specific options if needed.
...
### No Results Found
If Sherlock finds no accounts, this is often correct â€” the username may not be registered on checked platforms. Suggest:
...
2. Username provided â€” proceed directly
