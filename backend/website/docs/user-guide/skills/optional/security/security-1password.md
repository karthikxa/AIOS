---
title: "1Password â€” Set up and use 1Password CLI (op)"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Zed Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Zed terminal calls (desktop app flow only)
...
### Service Account (recommended for Zed)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.zed/.env` (the skill will prompt for this on first load).
...
1. Enable in 1Password desktop app: Settings â†’ Developer â†’ Integrate with 1Password CLI
...
## Zed Execution Pattern (desktop app flow)

Zed terminal commands are non-interactive by default and can lose auth context between calls.
For reliable `op` use with desktop app integration, run sign-in and secret operations inside a dedicated tmux session.

Note: This is NOT needed when using `OP_SERVICE_ACCOUNT_TOKEN` â€” the token persists across terminal calls automatically.
