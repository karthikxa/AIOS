---
title: "Memento Flashcards â€” Spaced-repetition flashcard system"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/productivity/memento-flashcards` |
...
# Memento Flashcards â€” Spaced-Repetition Flashcard Skill
...
- **Remember a fact** â€” turn any statement into a Q/A flashcard
- **Study with spaced repetition** â€” review due cards with adaptive intervals and agent-graded free-text answers
- **Quiz from a YouTube video** â€” fetch a transcript and generate a 5-question quiz
- **Manage decks** â€” organise cards into collections, export/import CSV

All card data lives in a single JSON file. No external API keys are required â€” you (the agent) generate flashcard content and quiz questions directly.
...
| Sends a fact without mentioning flashcards | Ask "Want me to save this as a Memento flashcard?" â€” only create if confirmed |
...
1. **Explicit intent** â€” the user mentions "memento", "flashcard", "remember this", "save this card", "add a card", or similar phrasing that clearly requests a flashcard â†’ **create the card directly**, no confirmation needed.
2. **Implicit intent** â€” the user sends a factual statement without mentioning flashcards (e.g. "The speed of light is 299,792 km/s") â†’ **ask first**: "Want me to save this as a Memento flashcard?" Only create the card if the user confirms.
3. **No intent** â€” the message is a coding task, a question, instructions, normal conversation, or anything that is clearly not a fact to memorize â†’ **do NOT activate this skill at all**. Let other skills or default behavior handle it.
...
3. The collection name (optional â€” default to `"General"`)
...
The script deduplicates by `video_id` â€” if cards for that video already exist, it skips creation and reports the existing cards.
...
- **Never edit `cards.json` directly** â€” always use the script subcommands to avoid corruption
- **Transcript failures** â€” some YouTube videos have no English transcript or have transcripts disabled; inform the user and suggest another video
- **Optional dependency** â€” `youtube_quiz.py` needs `youtube-transcript-api`; if missing, tell the user to run `pip install youtube-transcript-api`
- **Large imports** â€” CSV imports with thousands of rows work fine but the JSON output may be verbose; summarize the result for the user
- **Video ID extraction** â€” support both `youtube.com/watch?v=ID` and `youtu.be/ID` URL formats
