---
title: "Telephony â€” Give Zed phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give Zed phone capabilities without core tool changes"
---
...
Give Zed phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.

## Skill metadata

| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/productivity/telephony` |
...
# Telephony â€” Numbers, Calls, and Texts without Core Tool Changes

This optional skill gives Zed practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `~/.zed/.env`
...
It does **not** turn Zed into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Zed memory
...
## Decision tree â€” which service to use?

Use this logic instead of hardcoded provider routing:

### 1) "I want Zed to own a real phone number"
Use **Twilio**.
...
- pairs well with Zed `text_to_speech` plus a public file host or tunnel
...
### Twilio â€” owned number, SMS/MMS, direct calls, inbound SMS polling

Sign up at:
- https://www.twilio.com/try-twilio

Then save credentials into Zed:
...
### Bland.ai â€” easiest outbound AI calling
...
### Vapi â€” better conversational voice quality
...
3. Buy it and save it into `~/.zed/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
This is the main answer to â€œhow do I access messages the number receives next time the skill is loaded?â€
...
This is the main path for reusing Zed's existing `text_to_speech` support.

Use this when:
- you want the call to use Zed's configured TTS voice rather than Twilio `<Say>`
...
Recommended Zed TTS -> Twilio Play workflow:

1. Generate the audio with Zed `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Zed to create the message audio with `text_to_speech`
...
Important note:
- Zed TTS is great for prerecorded outbound messages
...
- Zed STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Zed memory.
...
- Do not store arbitrary third-party phone numbers in Zed memory.
...
3. persist that number to `~/.zed/.env`
