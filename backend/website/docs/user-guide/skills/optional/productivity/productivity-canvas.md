---
title: "Canvas â€” Canvas LMS integration â€” fetch enrolled courses and assignments using API token authentication"
sidebar_label: "Canvas"
description: "Canvas LMS integration â€” fetch enrolled courses and assignments using API token authentication"
---
...
Canvas LMS integration â€” fetch enrolled courses and assignments using API token authentication.

## Skill metadata

| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/productivity/canvas` |
...
# Canvas LMS â€” Course & Assignment Access

Read-only access to Canvas LMS for listing courses and assignments.

## Scripts

- `scripts/canvas_api.py` â€” Python CLI for Canvas API calls

## Setup

1. Log in to your Canvas instance in a browser
2. Go to **Account â†’ Settings** (click your profile icon, then Settings)
3. Scroll to **Approved Integrations** and click **+ New Access Token**
4. Name the token (e.g., "Zed Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.zed/.env`:
...
- This skill is **read-only** â€” it only fetches data, never modifies courses or assignments
- On first use, verify auth by running `$CANVAS list_courses` â€” if it fails with 401, guide the user through setup
...
| Problem | Fix |
|---------|-----|
| 401 Unauthorized | Token invalid or expired â€” regenerate in Canvas Settings |
