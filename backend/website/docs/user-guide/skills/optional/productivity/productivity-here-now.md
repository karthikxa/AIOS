---
title: "Here.Now â€” Publish static sites to {slug}"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ZED_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ZED_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
**File structure:** For HTML sites, place `index.html` at the root of the directory you publish, not inside a subdirectory. The directory's contents become the site root. For example, publish `my-site/` where `my-site/index.html` exists â€” don't publish a parent folder that contains `my-site/`.
...
**IMPORTANT**: After receiving an API key, save it immediately â€” run the command above yourself. Do not ask the user to run it manually. Avoid passing the key via CLI flags (e.g. `--api-key`) in interactive sessions; the credentials file is the preferred storage method.
...
| `--client {name}`      | Agent name for attribution (e.g. `zed`)    |
...
For Drive operations, use `drive.sh` or the Drive API. For broader account and site management â€” delete, metadata, passwords, payments, domains, handles, links, variables, proxy routes, forking, duplication, and more â€” see the current docs:
