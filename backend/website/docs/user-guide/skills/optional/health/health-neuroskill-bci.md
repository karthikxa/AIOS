...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/health/neuroskill-bci` |
| Path | `optional-skills/health/neuroskill-bci` |
| Version | `1.0.0` |
| Author | Zed Agent + Zed Team |
| License | MIT |
...
Connect Zed to a running [NeuroSkill](https://neuroskill.com/) instance to read
...
> **âš ï¸ Research Use Only** â€” NeuroSkill is an open-source research tool. It is
...
- Check signal quality â€” green indicators in NeuroSkill (â‰¥0.7 per electrode)
...
| `interactive "query"` | Cross-modal 4-layer graph search (text â†’ EXG â†’ labels) |
...
The `scores` object contains all live metrics (0â€“1 scale unless noted):

```jsonc
{
  "scores": {
    "focus": 0.70,           // Î² / (Î± + Î¸) â€” sustained attention
    "relaxation": 0.40,      // Î± / (Î² + Î¸) â€” calm wakefulness
...
Also includes: `device` (state, battery, firmware), `signal_quality` (per-electrode 0â€“1),
...
numbers alone â€” always give them meaning:

**DO:**
> "Your focus is solid right now at 0.70 â€” that's flow state territory. Heart
...
- **Focus > 0.70** â†’ flow state territory, protect it
- **Focus &lt; 0.40** â†’ suggest a break or protocol
- **Drowsiness > 0.60** â†’ fatigue warning, micro-sleep risk
- **Relaxation &lt; 0.30** â†’ stress intervention needed
- **Cognitive Load > 0.70 sustained** â†’ mind dump or break
- **TBR > 1.5** â†’ theta-dominant, reduced executive control
- **FAA &lt; 0** â†’ withdrawal/negative affect â€” consider FAA rebalancing
- **SNR &lt; 3 dB** â†’ unreliable signal, suggest electrode repositioning
...
> "Your focus started at 0.64 and climbed to 0.76 by the end â€” a clear upward trend.
...
4-layer graph: query â†’ text labels â†’ EXG points â†’ nearby labels. Use `--k-text`,
...
Interpret comparisons with context â€” mention trends, not just deltas:
...
> but there have been more stress spikes â€” your stress index jumped 15% and
...
- **Healthy targets**: N3 15â€“25%, REM 20â€“25%, efficiency >85%, onset &lt;20 min
...
- **&lt; 0.5** â†’ Similar brain states across both sessions
...
> FAA is positive â€” approach motivation is engaged. Looks like a solid start."
...
session â€” silence is the correct response.
...
Always ask before starting â€” never interrupt flow state:

> "Your focus has been declining for the past 15 minutes and TBR is climbing past
> 1.5 â€” signs of theta dominance and mental fatigue. Want me to walk you through
...
Key triggers:
- **Focus &lt; 0.40, TBR > 1.5** â†’ Theta-Beta Neurofeedback Anchor or Box Breathing
- **Relaxation &lt; 0.30, stress_index high** â†’ Cardiac Coherence or 4-7-8 Breathing
- **Cognitive Load > 0.70 sustained** â†’ Cognitive Load Offload (mind dump)
- **Drowsiness > 0.60** â†’ Ultradian Reset or Wake Reset
- **FAA &lt; 0 (negative)** â†’ FAA Rebalancing
- **Flow State (focus > 0.75, engagement > 0.70)** â†’ Do NOT interrupt
- **High stillness + headache_index** â†’ Neck Release Sequence
- **Low RMSSD (&lt; 25ms)** â†’ Vagal Toning
...
npx neuroskill status --json
```
â†’ Interpret scores naturally, mentioning focus, relaxation, mood, and any notable
...
npx neuroskill status --json
```
â†’ Check if metrics confirm it (high theta, low beta, rising TBR, high drowsiness).
â†’ If confirmed, suggest an appropriate protocol from `references/protocols.md`.
â†’ If metrics look fine, the issue may be motivational rather than neurological.

**"Compare my focus today vs yesterday"**
```bash
npx neuroskill compare --json
```
â†’ Interpret trends, not just numbers. Mention what improved, what declined, and
  possible causes.
...
```
â†’ Report timestamps, associated metrics, and what the user was doing (from labels).

**"How did I sleep?"**
```bash
npx neuroskill sleep --json
```
â†’ Report sleep architecture (N3%, REM%, efficiency), compare to healthy targets,
  and note any issues (high wake epochs, low REM).

**"Mark this moment â€” I just had a breakthrough"**
```bash
npx neuroskill label "breakthrough"
```
â†’ Confirm label saved. Optionally note the current metrics to remember the state.

---

## References

- [NeuroSkill Paper â€” arXiv:2603.03212](https://arxiv.org/abs/2603.03212) (Kosmyna & Hauptmann, MIT Media Lab)
