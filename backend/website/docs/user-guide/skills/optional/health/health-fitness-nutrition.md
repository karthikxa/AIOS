---
title: "Fitness Nutrition â€” Gym workout planner and nutrition tracker"
...
Gym workout planner and nutrition tracker. Search 690+ exercises by muscle, equipment, or category via wger. Look up macros and calories for 380,000+ foods via USDA FoodData Central. Compute BMI, TDEE, one-rep max, macro splits, and body fat â€” pure Python, no pip installs. Built for anyone chasing gains, cutting weight, or just trying to eat better.

## Skill metadata

| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/health/fitness-nutrition` |
...
plus offline calculators â€” everything a gym-goer needs in one place.

**Data sources (all free, no pip dependencies):**

- **wger** (https://wger.de/api/v2/) â€” open exercise database, 690+ exercises with muscles, equipment, images. Public endpoints need zero authentication.
- **USDA FoodData Central** (https://api.nal.usda.gov/fdc/v1/) â€” US government nutrition database, 380,000+ foods. `DEMO_KEY` works instantly; free signup for higher limits.
...
**Step 1 â€” Identify what the user wants:**

- By muscle â†’ use `/api/v2/exercise/?muscles={id}&language=2&status=2&format=json`
- By category â†’ use `/api/v2/exercise/?category={id}&language=2&status=2&format=json`
- By equipment â†’ use `/api/v2/exercise/?equipment={id}&language=2&status=2&format=json`
- By name â†’ use `/api/v2/exercise/search/?term={query}&language=english&format=json`
- Full details â†’ use `/api/v2/exerciseinfo/{exercise_id}/?format=json`

**Step 2 â€” Reference IDs (so you don't need extra API calls):**
...
**Step 3 â€” Fetch and present results:**
...
- wger exercise endpoint returns **all languages by default** â€” always add `language=2` for English
- wger includes **unverified user submissions** â€” add `status=2` to only get approved exercises
- USDA `DEMO_KEY` has **30 req/hour** â€” add `sleep 2` between batch requests or get a free key
- USDA data is **per 100g** â€” remind users to scale to their actual portion size
- BMI does not distinguish muscle from fat â€” high BMI in muscular people is not necessarily unhealthy
- Body fat formulas are **estimates** (Â±3-5%) â€” recommend DEXA scans for precision
- 1RM formulas lose accuracy above 10 reps â€” use sets of 3-5 for best estimates
