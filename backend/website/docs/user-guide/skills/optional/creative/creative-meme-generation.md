---
title: "Meme Generation â€” Generate real meme images by picking a template and overlaying text with Pillow"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/creative/meme-generation` |
...
2. Use `image_generate` to create a scene that matches the meme concept. Do NOT include any text in the image prompt â€” text will be added by the script. Describe only the visual scene.
...
4. Run the script with `--image` to overlay text, choosing a mode:
   - **Overlay** (text directly on image, white with black outline):
     ...
     ```
   - **Bars** (black bars above/below with white text â€” cleaner, always readable):
     ```bash
...
- The joke lands â€” caption matches the template's intended structure
