---
title: "Pixel Art â€” Pixel art w/ era palettes (NES, Game Boy, PICO-8)"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/creative/pixel-art` |
...
- `scripts/pixel_art.py` â€” photo â†’ pixel-art PNG (Floyd-Steinberg dithering)
- `scripts/pixel_art_video.py` â€” pixel-art PNG â†’ animated MP4 (+ optional GIF)
...
clarify(
    ...
    choices=[
        "arcade â€” bold, chunky 80s cabinet feel (16 colors, 8px)",
        "nes â€” Nintendo 8-bit hardware palette (54 colors, 8px)",
        "gameboy â€” 4-shade green Game Boy DMG",
        "snes â€” cleaner 16-bit look (32 colors, 4px)",
    ],
)
```
...
### Step 2 â€” Offer animation (optional)
...
clarify(
    ...
    choices=[
        "night â€” stars + fireflies + leaves",
        "urban â€” rain + neon pulse",
        "snow â€” falling snowflakes",
        "skip â€” just the image",
    ],
)
```
...
the complete list â€” 28 named palettes total). Any preset can be overridden:
...
```python
import sys
sys.path.insert(0, "/home/teknium/.zed/skills/creative/pixel-art/scripts")
...
```bash
cd /home/teknium/.zed/skills/creative/pixel-art/scripts
...
4. Quantize with Floyd-Steinberg dithering â€” against either an adaptive
...
- ffmpeg on PATH (only needed for video â€” Zed installs package this)
...
- Fractional `block` or `palette` will break quantization â€” keep them positive ints.
