---
title: "Kanban Video Orchestrator â€” Plan, set up, and monitor a multi-agent video production pipeline backed by Zed Kanban"
sidebar_label: "Kanban Video Orchestrator"
description: "Plan, set up, and monitor a multi-agent video production pipeline backed by Zed Kanban"
---
...
Plan, set up, and monitor a multi-agent video production pipeline backed by Zed Kanban. Use when the user wants to make ANY video â€” narrative film, product/marketing, music video, explainer, ASCII/terminal art, abstract/generative loop, comic, 3D, real-time/installation â€” and the work warrants decomposition into specialized profiles (writer, designer, animator, renderer, voice, editor, etc.) coordinated through a kanban board. Performs adaptive discovery to scope the brief, designs an appropriate team for the requested style, generates the setup script that creates Zed profiles + initial kanban task, then helps monitor execution and intervene when tasks stall or fail. Routes scenes to whichever Zed rendering / audio / design skill fits each beat (`ascii-video`, `manim-video`, `p5js`, `comfyui`, `touchdesigner-mcp`, `blender-mcp`, `pixel-art`, `baoyu-comic`, `claude-design`, `excalidraw`, `songsee`, `heartmula`, â€¦) plus external APIs for TTS, image-gen, and image-to-video as needed.

## Skill metadata

| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/creative/kanban-video-orchestrator` |
...
Wrap any video request â€” from a 15-second product teaser to a 5-minute narrative
short to a music video to an ASCII loop â€” in a Zed Kanban pipeline that
...
3. **Generates** a setup script that creates Zed profiles, project workspace, and the initial kanban task
...
existing skills + tools fit the scenes â€” `ascii-video`, `manim-video`, `p5js`,
...
- The user wants a quick one-shot conversion (e.g. "convert this mp4 to a GIF") â€” use ffmpeg directly.
- The output is a static image, GIF, or audio-only artifact â€” use the matching specific skill (`ascii-art`, `gifs`, `meme-generation`, `songwriting-and-ai-music`).
- The work fits a single existing skill cleanly (e.g. a pure ASCII video â€” just use `ascii-video`).
...
### Step 1 â€” Discover (ask the right questions)
...
**[references/intake.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/intake.md)**.
...
1. **Concept** â€” the one-sentence pitch + emotional north star
2. **Scope** â€” duration, aspect, platform, deadline
3. **Style** â€” visual references, brand constraints, tone
4. **Scenes** â€” beat-by-beat breakdown (durations, content, target tool)
5. **Audio** â€” narration / music / SFX / silent (per scene if needed)
6. **Deliverables** â€” file format, resolution, optional alternates (vertical cut, GIF, etc.)

Show the brief to the user for confirmation before designing the team. **The
brief is the contract** â€” every downstream task references it.

### Step 3 â€” Team design
...
**[references/role-archetypes.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/role-archetypes.md)**.

For mapping role â†’ which Zed skills + toolsets it loads, see
**[references/tool-matrix.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/tool-matrix.md)**.
...
3. Creates each Zed profile via `zed profile create --clone`
...
7. Fires the initial `zed kanban create` task assigned to the director

Use `scripts/bootstrap_pipeline.py` to generate setup.sh from a brief +
team-design JSON. See **[references/kanban-setup.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/kanban-setup.md)**
...
```bash
zed kanban watch --tenant <project-tenant>     # live events
zed kanban list  --tenant <project-tenant>     # board snapshot
zed dashboard                                   # visual board UI
```
...
### Step 6 â€” Monitor and intervene

Stay engaged â€” the kanban runs autonomously but a stuck task or bad output
...
playbook, see **[references/monitoring.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/monitoring.md)**.

## Reference: worked examples

Six concrete pipelines covering very different video styles â€” narrative film,
product/marketing, music video, math/algorithm explainer, ASCII video, real-time
installation â€” showing how the same workflow yields very different teams and
task graphs. See **[references/examples.md](https://github.com/NousResearch/zed-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/examples.md)**.
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   work itself. It decomposes and routes only â€” every concrete task becomes
   a `zed kanban create` call to a specialist profile. The
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `~/.zed/.env` or the user's secret store.
...
```
SKILL.md                            â† this file (workflow + rules)
references/
  intake.md                         â† discovery question banks per style
  role-archetypes.md                â† role library (writer, designer, animator, â€¦)
  tool-matrix.md                    â† skill + toolset mapping per role
  kanban-setup.md                   â† setup script structure & profile config
  monitoring.md                     â† watch + intervene patterns
  examples.md                       â† six worked pipelines
assets/
  brief.md.tmpl                     â† brief skeleton
  setup.sh.tmpl                     â† setup script skeleton
  soul.md.tmpl                      â† profile personality skeleton
scripts/
  bootstrap_pipeline.py             â† generate setup.sh from brief + team JSON
  monitor.py                        â† polling + intervention helpers
```
