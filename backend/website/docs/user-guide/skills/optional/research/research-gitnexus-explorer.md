...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/research/gitnexus-explorer` |
...
- **Node.js** (v18+) â€” required for GitNexus and the proxy
- **git** â€” repo must have a `.git` directory
- **cloudflared** â€” for tunneling (auto-installed to ~/.local/bin if missing)
...
at any scale â€” only the web visualization has this limit.
...
Add `--embeddings` for semantic search (slower â€” minutes instead of seconds).
...
web UI and proxies `/api/*` to the GitNexus backend â€” same origin, no CORS issues,
no sudo, no nginx.
...
### 6. Tunnel with Cloudflare (optional â€” for remote access)
...
Share it â€” anyone with the link can explore the graph.
