---
title: "Searxng Search â€” Free meta-search via SearXNG â€” aggregates results from 70+ search engines"
sidebar_label: "Searxng Search"
description: "Free meta-search via SearXNG â€” aggregates results from 70+ search engines"
---
...
Free meta-search via SearXNG â€” aggregates results from 70+ search engines. Self-hosted or use a public instance. No API key needed. Falls back automatically when the web search toolset is unavailable.

## Skill metadata

| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/research/searxng-search` |
...
Free meta-search using [SearXNG](https://searxng.org/) â€” a privacy-respecting, self-hosted search aggregator that queries 70+ search engines simultaneously.
...
SearXNG returns titles, URLs, and snippets â€” not full page content. To get full page content, search first and then extract the most relevant URL with `web_extract`, browser tools, or `curl`.
...
- **Results freshness**: Meta-search aggregates external engines â€” result freshness depends on those engines.
