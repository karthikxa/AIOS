---
title: "Darwinian Evolver â€” Evolve prompts/regex/SQL/code with Imbue's evolution loop"
...
| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Zed Agent |
| License | MIT |
...
Run Imbue's [darwinian_evolver](https://github.com/imbue-ai/darwinian_evolver) â€” an
LLM-driven evolutionary search loop â€” to optimize a **prompt, regex, SQL query,
...
import upstream classes into Zed itself.
...
- You have a scorer (exact match, regex pass-rate, unit test, LLM-judge, runtime
  ...
  and define one first â€” that's the hard part.
- Cost is OK: a typical run is 50â€“500 LLM calls. On gpt-4o-mini that's pennies;
  on Claude Sonnet it can be a few dollars.

Do **not** use this when:
- The optimization target is differentiable (use gradient descent / DSPy).
- You only need to try 2â€“3 variants â€” just write them by hand.
...
## Quick Start â€” The Built-In Parrot Example
...
Outputs:
- `/tmp/parrot_demo/snapshots/iteration_N.pkl` â€” pickled population per iteration
- `/tmp/parrot_demo/<jsonl>` â€” per-iteration JSON log (path printed at end)

Open `~/.zed/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
in a browser and load the JSON log to see the evolutionary tree.

## Quick Start â€” OpenRouter Driver (No Anthropic Key)

The skill ships `scripts/parrot_openrouter.py` â€” same parrot problem, but the
...
landing around 0.6â€“0.8 (the seed `Say {{ phrase }}` scored 0.000).

## Defining a Custom Problem

The skill ships `templates/custom_problem_template.py` â€” copy, edit, run.
Three things you must define:

1. **`Organism`** â€” a Pydantic `BaseModel` subclass holding the artifact being
   evolved (`prompt_template: str`, `regex_pattern: str`, `sql_query: str`,
   `code_block: str`, etc.). Add a `run(*args)` method that exercises it.

2. **`Evaluator`** â€” `.evaluate(organism) -> EvaluationResult(score=..., trainable_failure_cases=[...], holdout_failure_cases=[...], is_viable=True)`.
   - **`score`** is in `[0, 1]`. Higher is better.
   - **`trainable_failure_cases`** â€” what the mutator sees. Include enough
     context (input, expected, actual) for the LLM to diagnose.
   - **`holdout_failure_cases`** â€” kept out of the mutator's view. Use these
     to detect overfitting.
   - **`is_viable=True`** unless the organism is completely broken (raises,
     returns None, etc.). A 0-score viable organism is fine â€” it just gets
     down-weighted in parent selection.

3. **`Mutator`** â€” `.mutate(organism, failure_cases, learning_log_entries) -> list[Organism]`.
   ...
   a new `Organism`. Return `[]` on parse failure â€” the loop handles it.

Then write a driver script that wires `Problem(initial_organism, evaluator, [mutators])`
into `EvolveProblemLoop` and iterates over `loop.run(num_iterations=N)` â€” the
...
|---|---|---|
| `--num_iterations` | 5 | bump to 10â€“20 once you trust the evaluator |
| `--num_parents_per_iteration` | 4 | drop to 2 for cheap exploration |
| `--mutator_concurrency` | 10 | drop to 2â€“4 to avoid rate limits |
| `--evaluator_concurrency` | 10 | same; evaluator hits the LLM too |
| `--batch_size` | 1 | raise to 3â€“5 once your mutator handles multiple failures |
| `--verify_mutations` | off | turn on once mutator is wasteful (>10Ã— cost saving on later runs per Imbue) |
| `--midpoint_score` | `p75` | leave alone unless scores cluster |
| `--sharpness` | 10 | leave alone |

## Pitfalls

1. **`Initial organism must be viable`** â€” set `is_viable=True` in your
...
2. **Provider content filters kill runs.** Azure-backed OpenRouter models
   ...
   the LLM call in `try/except` and return `f"<LLM_ERROR: {e}>"` â€” the
   evolver will just score that organism 0 and move on.
3. **`loop.run()` is a generator** â€” calling it doesn't run anything until
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Zed core.
   Custom driver scripts under `~/.zed/skills/...` are user-side and fine.
