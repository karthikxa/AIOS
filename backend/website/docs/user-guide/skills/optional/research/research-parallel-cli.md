...
description: "Optional vendor skill for Parallel CLI â€” agent-native web search, extraction, deep research, enrichment, FindAll, and monitoring"
---
...
Optional vendor skill for Parallel CLI â€” agent-native web search, extraction, deep research, enrichment, FindAll, and monitoring. Prefer JSON output and non-interactive flows.

## Skill metadata

| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Zed core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Zed native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Zed native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Zed native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Zed usage patterns
...
- Do not silently choose Parallel when Zed native tools are already sufficient.
