...
description: "Public-records OSINT investigation framework â€” SEC EDGAR filings, USAspending contracts, Senate lobbying, OFAC sanctions, ICIJ offshore leaks, NYC property r..."
---
...
Public-records OSINT investigation framework â€” SEC EDGAR filings, USAspending contracts, Senate lobbying, OFAC sanctions, ICIJ offshore leaks, NYC property records (ACRIS), OpenCorporates registries, CourtListener court records, Wayback Machine archives, Wikipedia + Wikidata, GDELT news monitoring. Entity resolution across sources, cross-link analysis, timing correlation, evidence chains. Python stdlib only.

## Skill metadata

| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/research/osint-investigation` |
| Path | `optional-skills/research/osint-investigation` |
| Version | `0.1.0` |
| Author | Zed Agent (adapted from ShinMegamiBoson/OpenPlanter, MIT) |
...
# OSINT Investigation â€” Public Records Cross-Reference
...
- "follow the money" â€” government contracts, lobbying â†’ legislation, sanctions
- corporate due diligence â€” who controls company X, where are they
  incorporated, who serves on their boards, what filings have they made
- sanctions screening â€” is entity X on OFAC SDN, ICIJ offshore leaks
- pay-to-play investigation â€” contractors with offshore ties, lobbying
  clients winning awards
- property ownership â€” find recorded deeds/mortgages by name or address
  (NYC; for other counties point users at the relevant recorder)
- litigation history â€” find federal + state court opinions and PACER dockets
...
- "what's been said about X" â€” international news (GDELT) + Wikipedia
  narrative + Wayback Machine to recover dead URLs

Do NOT use this skill for:

- general web research â†’ `web_search` / `web_extract`
- domain/infrastructure OSINT â†’ `domain-intel` skill
- academic literature â†’ `arxiv` skill
- social-media profile discovery â†’ `sherlock` skill (optional)
- US **federal** campaign finance â€” FEC is intentionally NOT covered here
...
The **cross-reference potential** section maps join keys between sources â€” read
...
# ICIJ Offshore Leaks â€” downloads ~70 MB bulk CSV on first use,
# then searches it locally. Cached for 30 days under
# $ZED_OSINT_CACHE/icij/ (default: ~/.cache/zed-osint/icij/).
...
```bash
# NYC property records (deeds, mortgages, liens) â€” ACRIS via Socrata
...
# OpenCorporates â€” 130+ jurisdiction corporate registry
...
| `token_overlap` | â‰¥60% token overlap, â‰¥2 shared tokens, tokens â‰¥4 chars | low |
...
Test whether two time series cluster suspiciously close together â€” e.g.
lobbying filings near contract awards â€” using a permutation test:
...
The script's column flags are intentionally generic â€” the original tool was
...
with mean nearest-award distance â‰¤ observed. Minimum 3 events per (payer,
...
- Statistical significance â‰  wrongdoing. p &lt; 0.05 means the timing pattern
...
- `fetch_*.py` scripts use `urllib.request` and respect `Retry-After`. Heavy
  bulk usage may still violate ToS â€” read each source's legal section first.
