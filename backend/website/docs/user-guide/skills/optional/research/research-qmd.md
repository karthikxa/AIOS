...
title: "Qmd"
sidebar_label: "Qmd"
description: "Search personal knowledge bases, notes, docs, and meeting transcripts locally using qmd â€” a hybrid retrieval engine with BM25, vector search, and LLM reranking"
---
...
Search personal knowledge bases, notes, docs, and meeting transcripts locally using qmd â€” a hybrid retrieval engine with BM25, vector search, and LLM reranking. Supports CLI and MCP integration.

## Skill metadata

| | |
|---|---|
| Source | Optional â€” install with `zed skills install official/research/qmd` |
...
LLM-powered reranking â€” all running locally with no cloud dependencies.

Created by [Tobi LÃ¼tke](https://github.com/tobi/qmd). MIT licensed.
...
No models loaded â€” near-instant results.
...
Uses all 3 models â€” query expansion, parallel BM25+vector, reranking.
...
Zed Agent via the native MCP client. This is the preferred
integration â€” once configured, the agent gets qmd tools automatically
...
Start the qmd daemon separately â€” it keeps models warm in memory:
...
Then configure Zed Agent to connect via HTTP:
...
1. **Query Expansion** â€” A fine-tuned 1.7B model generates 2 alternative
   queries. The original gets 2x weight in fusion.
2. **Parallel Retrieval** â€” BM25 (SQLite FTS5) and vector search run
   simultaneously across all query variants.
3. **RRF Fusion** â€” Reciprocal Rank Fusion (k=60) merges results.
   Top-rank bonus: #1 gets +0.05, #2-3 get +0.02.
4. **LLM Reranking** â€” qwen3-reranker scores top 30 candidates (0.0-1.0).
5. **Position-Aware Blending** â€” Ranks 1-3: 75% retrieval / 25% reranker.
...
1. **Always add context descriptions** â€” `qmd context add` dramatically
   improves retrieval accuracy. Describe what each collection contains.
2. **Re-embed after adding documents** â€” `qmd embed` must be re-run when
   new files are added to collections.
3. **Use `qmd search` for speed** â€” when you need fast keyword lookup
   (code identifiers, exact names), BM25 is instant and needs no models.
4. **Use `qmd query` for quality** â€” when the question is conceptual or
   the user needs the best possible results, use hybrid search.
5. **Prefer MCP integration** â€” once configured, the agent gets native
   tools without needing to load this skill each time.
6. **Daemon mode for frequent users** â€” if the user searches their
   knowledge base regularly, recommend the HTTP daemon setup.
7. **First query in structured search gets 2x weight** â€” put the most
   important/certain query first when combining lex and vec.

## Troubleshooting

### "Models downloading on first run"
Normal â€” qmd auto-downloads ~2GB of GGUF models on first use.
