---
title: "Plan â€” Plan mode: write an actionable markdown plan to"
sidebar_label: "Plan"
...
Plan mode: write an actionable markdown plan to .zed/plans/, no execution. Bite-sized tasks, exact paths, complete code.
...
| Version | `2.0.0` |
| Author | Zed Agent (writing-craft adapted from obra/superpowers) |
| License | MIT |
...
- Your deliverable is a markdown plan saved inside the active workspace under `.zed/plans/`.
...
Treat that as relative to the active working directory / backend workspace. Zed file tools are backend-aware, so using this relative path keeps the plan with the workspace on local, docker, ssh, modal, and daytona backends.

If the runtime provides a specific target path, use that exact path.
If not, create a sensible timestamped filename yourself under `.zed/plans/`.
...
> **For Zed:** Use subagent-driven-development skill to implement this plan task-by-task.
...
Use Zed tools to understand the project:
