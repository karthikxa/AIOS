...
description: "Drive the macOS desktop in the background â€” screenshots, mouse, keyboard, scroll, drag â€” without stealing the user's cursor, keyboard focus, or Space"
---
...
Drive the macOS desktop in the background â€” screenshots, mouse, keyboard,
scroll, drag â€” without stealing the user's cursor, keyboard focus, or
...
Everything here works with any tool-capable model â€” Claude, GPT, Gemini, or
an open model running through a local OpenAI-compatible endpoint. There is
no Anthropic-native schema to learn.

## The canonical workflow

**Step 1 â€” Capture first.** Almost every task starts with:
...
**Step 2 â€” Click by element index.** This is the single most important
habit:
...
**Step 3 â€” Verify.** After any state-changing action, re-capture. You can
...
2. **Scope captures to an app** (`app="Safari"`) â€” less noisy, fewer
...
focus explicitly â€” passing `app=...` to `capture` / `click` / `type` will
...
On CLI, you can just describe what you see â€” the screenshot data stays in
your conversation context.

## Safety â€” these are hard rules
...
- Some system shortcuts are hard-blocked at the tool level â€” log out,
...
- File edits â€” use `read_file` / `write_file` / `patch`, not `type` into
  an editor window.
- Shell commands â€” use `terminal`, not `type` into Terminal.app.
