---
title: "Codex â€” Delegate coding to OpenAI Codex CLI (features, PRs)"
sidebar_label: "Codex"
...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`zed-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-zed-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Zed terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Zed itself, `model.provider: openai-codex` uses Zed-managed Codex
OAuth from `~/.zed/auth.json` after `zed auth add openai-codex`. For the
...
When invoking the Codex CLI from a Zed gateway/service context (for example,
