---
title: "Zed Agent â€” Configure, extend, or contribute to Zed Agent"
sidebar_label: "Zed Agent"
description: "Configure, extend, or contribute to Zed Agent"
---
...
Configure, extend, or contribute to Zed Agent.
...
| Path | `skills/autonomous-ai-agents/zed-agent` |
| Version | `2.1.0` |
| Author | Zed Agent + Teknium |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `zed`, `setup`, `configuration`, `multi-agent`, `spawning`, `cli`, `gateway`, `development` |
...
Zed Agent is an open-source AI agent framework by Zed Team that runs in your terminal, messaging platforms, and IDEs. It belongs to the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw â€” autonomous coding and task-execution agents that use tool calling to interact with your system. Zed works with any LLM provider (OpenRouter, Anthropic, OpenAI, DeepSeek, local models, and 15+ others) and runs on Linux, macOS, and WSL.

What makes Zed different:

- **Self-improving through skills** â€” Zed learns from experience by saving reusable procedures as skills. When it solves a complex problem, discovers a workflow, or gets corrected, it can persist that knowledge as a skill document that loads into future sessions. Skills accumulate over time, making the agent better at your specific tasks and environment.
...
- **Profiles** â€” run multiple independent Zed instances with isolated configs, sessions, skills, and memory.
- **Extensible** â€” plugins, MCP servers, custom tools, webhook triggers, cron scheduling, and the full Python ecosystem.

People use Zed for software development, research, system administration, data analysis, content creation, home automation, and anything else that benefits from an AI agent with persistent context and full system access.
...
zed auth                 Interactive credential manager
zed auth add PROVIDER    Add OAuth or API-key credential (e.g. nous, openai-codex, qwen-oauth)
zed auth list            List stored credentials
zed auth remove PROVIDER Remove a stored credential
...
(`zed mcp install <name>`): `skill_view(name="zed-agent", file_path="references/native-mcp.md")`.
...
patterns: `skill_view(name="zed-agent", file_path="references/webhooks.md")`.
...
~/.zed/sessions/         Gateway routing index, request dumps, *.jsonl transcripts (and optional per-session JSON snapshots when sessions.write_json_snapshots: true)
~/.zed/state.db          Canonical session store (SQLite + FTS5)
~/.zed/logs/             Gateway and error logs
~/.zed/auth.json         OAuth tokens and credential pools
~/.zed/zed-agent/     Source code (if git-installed)
```

Profiles use `~/.zed/profiles/<name>/` with the same layout.

### Config Sections

Edit with `zed config edit` or `zed config set section.key value`.
...
20+ providers supported. Set via `zed model` or `zed setup`.
...
Enable/disable via `zed tools` (interactive) or `zed tools enable/disable NAME`.
...
Full enumeration lives in `toolsets.py` as the `TOOLSETS` dict; `_ZED_CORE_TOOLS` is the default bundle most platforms inherit from.
...
Common "why is Zed doing X to my output / tool calls / commands?" toggles â€” and the exact commands to change them. Most of these need a fresh session (`/reset` in chat, or start a new `zed` invocation) because they're read once at startup.
...
```bash
zed config set security.redact_secrets true       # keep enabled globally
```

**Restart required.** `security.redact_secrets` is snapshotted at import time â€” toggling it mid-session (e.g. via `export ZED_REDACT_SECRETS=false` from a tool call) will NOT take effect for the running process. Tell the user to change it in config from a terminal, then start a new session. This is deliberate â€” it prevents an LLM from flipping the toggle on itself mid-task.
...
```bash
zed config set privacy.redact_pii true    # enable
zed config set privacy.redact_pii false   # disable (default)
```

### Command approval prompts

By default (`approvals.mode: manual`), Zed prompts the user before running shell commands flagged as destructive (`rm -rf`, `git reset --hard`, etc.). The modes are:
...
```bash
zed config set approvals.mode smart       # recommended middle ground
zed config set approvals.mode off         # bypass everything (not recommended)
```
...
Some shell-hook integrations require explicit allowlisting before they fire. Managed via `~/.zed/shell-hooks-allowlist.json` â€” prompted interactively the first time a hook wants to run.

### Disabling the web/browser/image-gen tools

To keep the model away from network or media tools entirely, open `zed tools` and toggle per-platform. Takes effect on next session (`/reset`). See the Tools & Skills section above.
...
## Spawning Additional Zed Instances

Run additional Zed processes as fully independent subprocesses â€” separate sessions, tools, and environments.

### When to Use This vs delegate_task

| | `delegate_task` | Spawning `zed` process |
|-|-----------------|--------------------------|
...
Zed uses prompt_toolkit, which requires a real terminal. Use tmux for interactive spawning:
...
- **Use `zed chat -q` for fire-and-forget** â€” no PTY needed
...
the `cronjob` tool, the `zed cron` CLI (`list`, `add`, `edit`,
...
- **CLI:** `zed curator <verb>` â€” `status`, `run`, `pause`, `resume`,
...
- **Telemetry:** sidecar at `~/.zed/skills/.usage.json` holds
...
Users drive it via `zed kanban <verb>`; dispatcher-spawned workers
see a focused `kanban_*` toolset gated by `ZED_KANBAN_TASK`, and
...
Zed runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
without BOM. `zed config edit` writes without BOM; manual edits in
...
Zed's own env scrubber dropping `SYSTEMROOT` / `WINDIR` / `COMSPEC`
...
POSIX venv layouts (`.venv/bin/activate`). The Zed-installed venv at
...
every Zed tool and most Windows APIs. Prefer forward slashes in code
...
### Tool not available
1. `zed tools` â€” check if toolset is enabled for your platform
2. Some tools need env vars (check `.env`)
3. `/reset` after enabling tools

### Model/provider issues
1. `zed doctor` â€” check config and dependencies
2. `zed auth` â€” re-authenticate OAuth providers (or `zed auth add <provider>`)
3. Check `.env` has the right API key
4. **Copilot 403**: `gh auth login` tokens do NOT work for Copilot API. You must use the Copilot-specific OAuth device code flow via `zed model` â†’ GitHub Copilot.
...
### Skills not showing
1. `zed skills list` â€” verify installed
2. `zed skills config` â€” check platform enablement
3. Load explicitly: `/skill name` or `zed -s name`
...
- **Gateway crash loop**: Reset the failed state: `systemctl --user reset-failed zed-gateway`
...
Config: `~/.zed/config.yaml` (settings), `~/.zed/.env` (API keys).
...
**2. Add to `toolsets.py`** â†’ `_ZED_CORE_TOOLS` list.
...
All handlers must return JSON strings. Use `get_zed_home()` for paths, never hardcode `~/.zed`.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `zed_cli/commands.py`
...
- POSIX file modes (0o600, etc.) â†’ `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/zed_cli/test_auth_toctou_file_modes.py`)
- `signal.SIGALRM` â†’ Unix-only (see `tests/conftest.py::_enforce_test_timeout`)
...
- Use `get_zed_home()` from `zed_constants` for all paths (profile-safe)
