---
title: "Notion â€” Notion API + ntn CLI: pages, databases, markdown, Workers"
...
Add those exports to your shell profile (or to `~/.zed/.env`) so every session inherits them.
...
- Notion also ships an MCP server now (`Notion MCP`, ~91% more token-efficient on DB ops than the previous version) â€” wire it via Zed' MCP support if you want streaming Notion access from inside a session, but the paths above are enough for most one-shot tasks.
