...
description: "Use when building creative browser demos with @chenglou/pretext â€” DOM-free text layout for ASCII art, typographic flow around obstacles, text-as-geometry gam..."
---
...
Use when building creative browser demos with @chenglou/pretext â€” DOM-free text layout for ASCII art, typographic flow around obstacles, text-as-geometry games, kinetic typography, and text-powered generative art. Produces single-file HTML demos by default.
...
[`@chenglou/pretext`](https://github.com/chenglou/pretext) is a 15KB zero-dependency TypeScript library by Cheng Lou (React core, ReasonML, Midjourney) for **DOM-free multiline text measurement and layout**. It does one thing: given `(text, font, width)`, return the line breaks, per-line widths, per-grapheme positions, and total height â€” all via canvas measurement, no reflow.
...
This skill exists so Zed can make **cool demos** with it â€” the kind people post to X. See `pretext.cool` and `chenglou.me/pretext` for the community demo corpus.
...
Don't use for:
- Static SVG/HTML pages where CSS already solves layout â€” just use CSS
- Rich text editors, general inline formatting engines (pretext is intentionally narrow)
- Image â†’ text (use `ascii-art` / `ascii-video` skills)
- Pure canvas generative art with no text role â€” use `p5js`
...
- **Proportional fonts are the point.** Pretext's whole vibe is "not monospaced" â€” lean into it. Use Iowan Old Style, Inter, JetBrains Mono, Helvetica Neue, or a variable font. Never default sans.
- **Real source/text, not lorem ipsum.** The corpus should mean something. Short manifestos, poetry, real source code, a found text, the library's own README â€” never `lorem ipsum`.
...
| Interaction | Raw DOM events | Mouse / touch / wheel â€” no framework |
...
Pin the version. `@0.0.6` at time of writing â€” check [npm](https://www.npmjs.com/package/@chenglou/pretext) for the latest if demo behavior is off.

## The Two Use Cases

Almost everything reduces to one of these two shapes. Learn both.

### Use-case 1 â€” measure, then render with CSS/DOM
...
### Use-case 2 â€” measure *and* render yourself
...
This is the most important pattern in the whole library. It's what unlocks "text flowing around a dragged sprite" â€” the demo that went viral on X.

### Helpers worth knowing

- `measureLineStats(prepared, maxWidth)` â†’ `{ lineCount, maxLineWidth }` â€” the widest line, i.e. multiline shrink-wrap width.
- `walkLineRanges(prepared, maxWidth, callback)` â€” iterate lines without allocating strings. Use for stats/physics over graphemes when you don't need the characters.
- `@chenglou/pretext/rich-inline` â€” the same system but for paragraphs mixing fonts / chips / mentions. Import from the subpath.

## Demo Recipe Patterns

The community corpus (see `references/patterns.md`) clusters into a handful of strong patterns. Pick one and riff â€” don't invent a new category unless asked.
...
| **Shatter / particles** | `walkLineRanges` â†’ per-grapheme (x,y) â†’ physics | Sentence that explodes into letters on click |
...
2. **Start from a template**:
   - `templates/hello-orb-flow.html` â€” text reflowing around a moving orb (reflow-around-obstacle pattern)
   - `templates/donut-orbit.html` â€” advanced example: measured ASCII logo obstacles, draggable wire sphere/cube, morphing shape fields, selectable DOM text, and dev-only controls
...
4. **Tune the aesthetic** â€” font, palette, composition, interaction. This is the work; don't skip it.
5. **Verify locally**:
   ```sh
   cd <dir-with-html> && python3 -m http.server 8765
   # then open http://localhost:8765/<file>.html
   ```
6. **Check the console** â€” pretext will throw if `prepareWithSegments` is called with a bad font string; `Intl.Segmenter` is available in every modern browser.
7. **Show the user the file path**, not just the code â€” they want to open it.

## Performance Notes

- `prepare()` / `prepareWithSegments()` is the expensive call. Do it **once** per text+font pair. Cache the handle.
- On resize, only rerun `layout()` / `layoutWithLines()` â€” never re-prepare.
...
3. **Forgetting `Intl.Segmenter` for grapheme splits.** Emoji, combining marks, CJK â€” `"Ã©".split("")` gives you two chars. Use `new Intl.Segmenter(undefined, { granularity: "grapheme" })` when sampling individual visible glyphs.

4. **`break: 'never'` chips without `extraWidth`.** In `rich-inline`, if you use `break: 'never'` for an atomic chip/mention, you must also supply `extraWidth` for the pill padding â€” otherwise chip chrome overflows the container.

5. **Using `@chenglou/pretext` from `unpkg` with TypeScript-only entry.** Use `esm.sh` â€” it compiles the TS exports to browser-ready ESM automatically. `unpkg` will 404 or serve raw TS.
...
7. **Skipping rows vs adjusting width** when flowing around a shape. If the corridor on this row is too narrow to fit a line, *skip the row* (`y += lineHeight; continue;`) rather than passing a tiny maxWidth to `layoutNextLineRange` â€” pretext will return one-grapheme lines that look broken.
...
- [ ] Demo is a single self-contained `.html` file â€” opens by double-click or `python3 -m http.server`
...
- [ ] Dark background + considered palette â€” not the default white canvas
...
- **Pretext Breaker** â€” breakout with word-bricks â€” `github.com/rinesh/pretext-breaker`
- **Tetris Ã— Pretext** â€” `github.com/shinichimochizuki/tetris-pretext`
- **Dragon animation** â€” `github.com/qtakmalay/PreTextExperiments`
- **Somnai editorial engine** â€” `github.com/somnai-dreams/pretext-demos`
- **Bad Apple!! ASCII** â€” `github.com/frmlinn/bad-apple-pretext`
- **Drag-sprite reflow** â€” `github.com/dokobot/pretext-demo`
- **Alarmy editorial clock** â€” `github.com/SmisLee/alarmy-pretext-demo`

Official playground: [chenglou.me/pretext](https://chenglou.me/pretext/) â€” accordion, bubbles, dynamic-layout, editorial-engine, justification-comparison, masonry, markdown-chat, rich-note.
