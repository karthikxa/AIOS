...
title: "Google Workspace â€” Gmail, Calendar, Drive, Sheets & Docs"
description: "Send email, manage calendar events, search Drive, read/write Sheets, and access Docs â€” all through OAuth2-authenticated Google APIs"
---

# Google Workspace Skill

Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration for Zed. Uses OAuth2 with automatic token refresh. Prefers the [Google Workspace CLI (`gws`)](https://github.com/googleworkspace/cli) when available for broader coverage, and falls back to Google's Python client libraries otherwise.

**Skill path:** `skills/productivity/google-workspace/`

## Setup

The setup is fully agent-driven â€” ask Zed to set up Google Workspace and it walks you through each step. The flow:
...
3. **Authorize** â€” Zed generates an auth URL, you approve in the browser, paste back the redirect URL
4. **Done** â€” token auto-refreshes from that point on

:::tip Email-only users
If you only need email (no Calendar/Drive/Sheets), use the **himalaya** skill instead â€” it works with a Gmail App Password and takes 2 minutes. No Google Cloud project needed.
:::
...
**Important:** If you use a *different email address* in `--from` (not the authenticated account), Gmail requires that address to be configured as a [Send As alias](https://support.google.com/mail/answer/22370) in Gmail Settings â†’ Accounts â†’ Send mail as.
...
| Problem | Fix |
|---------|-----|
| `NOT_AUTHENTICATED` | Run setup (ask Zed to set up Google Workspace) |
| `REFRESH_FAILED` | Token revoked â€” re-run authorization steps |
| `HttpError 403: Insufficient Permission` | Missing scope â€” revoke and re-authorize with the right services |
