...
Zed Agent is designed with a defense-in-depth security model. This page covers every security boundary â€” from command approval to container isolation to user authorization on messaging platforms.

## Overview

The security model has seven layers:

1. **User authorization** â€” who can talk to the agent (allowlists, DM pairing)
2. **Dangerous command approval** â€” human-in-the-loop for destructive operations
3. **Container isolation** â€” Docker/Singularity/Modal sandboxing with hardened settings
4. **MCP credential filtering** â€” environment variable isolation for MCP subprocesses
5. **Context file scanning** â€” prompt injection detection in project files
6. **Cross-session isolation** â€” sessions cannot access each other's data or state; cron job storage paths are hardened against path traversal attacks
7. **Input sanitization** â€” working directory parameters in terminal tool backends are validated against an allowlist to prevent shell injection

## Dangerous Command Approval

Before executing any command, Zed checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.zed/config.yaml`:

```yaml
approvals:
  mode: manual                    # manual | smart | off
  timeout: 60                     # seconds to wait for user response (default: 60)
  cron_mode: deny                 # deny | approve â€” what cron jobs do when they hit a dangerous command
...
|---|---|---|
| `mode` | `manual` | Approval policy for dangerous shell commands â€” see the table below. |
| `timeout` | `60` | Seconds Zed waits for an approval reply before timing out. |
...
| **off** | Disable all approval checks â€” equivalent to running with `--yolo`. All commands execute without prompts. |
...
1. **CLI flag**: Start a session with `zed --yolo` or `zed chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `ZED_YOLO_MODE=1`

The `/yolo` command is a **toggle** â€” each use flips the mode on or off:
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `ZED_YOLO_MODE` environment variable which is checked before every command execution.

When YOLO is active, Zed shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:

- A red banner line at session start when YOLO is already active: `âš  YOLO mode â€” all approval prompts bypassed`. Hidden when YOLO is off so the default banner stays uncluttered.
- A `âš  YOLO` fragment in the status bar across all width tiers, updated live as you toggle YOLO on or off (rich-text renderer and plain-text fallback).

:::danger
YOLO mode disables **all** dangerous command safety checks for the session â€” **except** the hardline blocklist (see below). Use only when you fully trust the commands being generated (e.g., well-tested automation scripts in disposable environments).
:::

For destructive session slash commands (`/clear`, `/new` / `/reset`, `/undo`, `/quit --delete` â€” `/exit --delete` is an alias), the CLI also prompts for confirmation before running them. See [Slash Commands â€” Confirmation prompts for destructive commands](../reference/slash-commands.md#confirmation-prompts-for-destructive-commands).

### Hardline Blocklist (Always-On Floor)

Some commands are so catastrophic â€” irreversible filesystem wipes, fork bombs, direct block-device writes â€” that Zed refuses to run them **regardless** of:
...
Configure the timeout in `~/.zed/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.zed/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.zed/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` zed/gateway | Self-termination prevention |
...
- **once** â€” allow this single execution
- **session** â€” allow this pattern for the rest of the session
- **always** â€” add to permanent allowlist (saved to `config.yaml`)
- **deny** (default) â€” block the command
...
The `ZED_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.zed/config.yaml`:
...
:::tip
Use `zed config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Zed controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.zed/.env`:
...
For more flexible authorization, Zed includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `zed pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.zed/config.yaml`:
...
| Lockout | 5 failed approval attempts â†’ 1-hour lockout |
...
**Storage:** Pairing data is stored in `~/.zed/pairing/` with per-platform JSON files:
- `{platform}-pending.json` â€” pending pairing requests
- `{platform}-approved.json` â€” approved users
- `_rate_limits.json` â€” rate limit and lockout tracking

## Container Isolation

When using the `docker` terminal backend, Zed applies strict security hardening to every container.
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.zed/sandboxes/docker/<task_id>/`
- **Ephemeral mode** (`container_persistent: false`): Uses tmpfs for workspace â€” everything is lost on cleanup
...
|---------|-----------|-------------------|----------|
| **local** | None â€” runs on host | âœ… Yes | Development, trusted users |
| **ssh** | Remote machine | âœ… Yes | Running on a separate server |
| **docker** | Container | âŒ Skipped (container is boundary) | Production gateway |
| **singularity** | Container | âŒ Skipped | HPC environments |
| **modal** | Cloud sandbox | âŒ Skipped | Scalable cloud isolation |
| **daytona** | Cloud sandbox | âŒ Skipped | Persistent cloud workspaces |
...
After loading this skill, `TENOR_API_KEY` passes through to `execute_code`, `terminal` (local), **and remote backends (Docker, Modal)** â€” no manual configuration needed.

:::info Docker & Modal
Prior to v0.5.1, Docker's `forward_env` was a separate system from the skill passthrough. They are now merged â€” skill-declared env vars are automatically forwarded into Docker containers and Modal sandboxes without needing to add them to `docker_forward_env` manually.
:::
...
Some skills need **files** (not just env vars) in the sandbox â€” for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `ZED_HOME`. Skills declare these in frontmatter:
...
When loaded, Zed checks if these files exist in the active profile's `ZED_HOME` and registers them for mounting:
...
Paths are relative to `~/.zed/`. Files are mounted to `/root/.zed/` inside the container. This list is read by `tools/credential_files.py` (`terminal.credential_files`) â€” it lives under the `terminal:` block but is loaded by the credential-files module, not the core terminal backend, so it isn't part of the bundled `DEFAULT_CONFIG` snapshot.

### What Each Sandbox Filters

| Sandbox | Default Filter | Passthrough Override |
|---------|---------------|---------------------|
| **execute_code** | Blocks vars containing `KEY`, `TOKEN`, `SECRET`, `PASSWORD`, `CREDENTIAL`, `PASSWD`, `AUTH` in name; only allows safe-prefix vars through | âœ… Passthrough vars bypass both checks |
| **terminal** (local) | Blocks explicit Zed infrastructure vars (provider keys, gateway tokens, tool API keys) | âœ… Passthrough vars bypass the blocklist |
| **terminal** (Docker) | No host env vars by default | âœ… Passthrough vars + `docker_forward_env` forwarded via `-e` |
| **terminal** (Modal) | No host env/files by default | âœ… Credential files mounted; env passthrough via sync |
| **MCP** | Blocks everything except safe system vars + explicitly configured `env` | âŒ Not affected by passthrough (use MCP `env` config instead) |

### Security Considerations

- The passthrough only affects vars you or your skills explicitly declare â€” the default security posture is unchanged for arbitrary LLM-generated code
...
- Zed infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` â€” they have dedicated mechanisms
...
Some setups legitimately need private/internal URL access â€” home networks that resolve `home.arpa` to RFC 1918 space, LAN-only Ollama/llama.cpp endpoints, internal wikis, cloud metadata debugging, and the like. For those cases there's a global opt-out:
...
Zed integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped â€” pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Zed under WSL.

Tirith's verdict integrates with the approval flow: safe commands pass through, while both suspicious and blocked commands trigger user approval with the full tirith findings (severity, title, description, safer alternatives). Users can approve or deny â€” the default choice is deny to keep unattended scenarios secure.
...
1. **Set explicit allowlists** â€” never use `GATEWAY_ALLOW_ALL_USERS=true` in production
2. **Use container backend** â€” set `terminal.backend: docker` in config.yaml
3. **Restrict resource limits** â€” set appropriate CPU, memory, and disk limits
4. **Store secrets securely** â€” keep API keys in `~/.zed/.env` with proper file permissions
5. **Enable DM pairing** â€” use pairing codes instead of hardcoding user IDs when possible
6. **Review command allowlist** â€” periodically audit `command_allowlist` in config.yaml
7. **Set `terminal.cwd`** â€” don't let the agent operate from sensitive directories
8. **Run as non-root** â€” never run the gateway as root
9. **Monitor logs** â€” check `~/.zed/logs/` for unauthorized access attempts
10. **Keep updated** â€” run `zed update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.zed/.env`:
...
Zed ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `zed_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `zed doctor` for the full remediation.
- **`zed doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
The ack is persisted to `config.security.acked_advisories` and survives restart. Old advisories are intentionally **not** removed from the catalog â€” leaving them in place keeps fresh installs warned about historically poisoned versions that might still be cached in a private mirror.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, â€¦) depend on Python packages that not every user needs. Zed installs these **lazily** on first use rather than eagerly under `zed-agent[all]`. The implementation lives in `tools/lazy_deps.py`.

The trade-off this fixes:

- **Fragility.** When one extra's transitive dependency becomes unavailable on PyPI (quarantined for malware, yanked, broken upload), the entire `[all]` resolve would fail and fresh installs would silently fall back to a stripped tier â€” losing 10+ unrelated extras at once. Lazy install isolates each backend so one poisoned dep can't break unrelated features.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `zed tools`.

Security guarantees enforced by `tools/lazy_deps.py`:

| Guarantee | What it means |
|---|---|
| Venv-scoped only | Installs target `sys.executable` in the active venv â€” never the system Python |
| PyPI by name only | Specs accept `"package>=1.0,<2"` syntax. No `--index-url`, `git+https://`, or file: paths â€” a malicious `config.yaml` cannot redirect the install |
...
| No silent retries | Failures surface as `FeatureUnavailable` â€” no caching of bad state, no retry storms |
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install â€¦`) or pick a different backend via `zed tools`.
