...
A **profile distribution** packages a complete Zed agent â€” personality, skills, cron jobs, MCP connections, config â€” as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing a Zed agent meant sending someone:
...
â€¦and hoping they assembled it correctly. Every version bump or bug fix meant repeating the handoff.
...
â€¦and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` â†’ `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `zed profile update my-research-agent` and pull your changes â€” their memories and sessions stay put.
...
- **Private repos work for free.** SSH keys, `git credential` helpers, GitHub CLI stored credentials â€” whatever auth your terminal is already set up for applies transparently.
- **Reproducibility is a commit SHA.** The same thing pip and npm record.

The tradeoff: recipients need git installed. On any machine running Zed in 2026, that's already true.

## When should you use a distribution?

Good fits:

- **You're sharing a specialized agent** â€” a compliance monitor, a code reviewer, a research assistant, a customer-support bot â€” with a team or with the community.
...
- **You're building an agent as a product** â€” opinionated defaults, curated skills, tuned prompts â€” that other people should use as a starting point.

Not a fit:

- **You just want to back up a profile on your own machine.** Use [`zed profile export` / `import`](../reference/profile-commands.md#zed-profile-export) â€” that's what those are for.
...
### Step 1 â€” Start from a working profile
...
### Step 2 â€” Add a `distribution.yaml`

Create `~/.zed/profiles/research-bot/distribution.yaml`:
...
### Step 4 â€” Tag versioned releases
...
Recipients who run `zed profile update research-bot` will pull the latest.
...
| **Config override** | `config.yaml` | Actually preserved by default â€” the installer may have tuned model or provider. Pass `--force-config` on update to reset. |
...
When omitted, the defaults above apply â€” which is what most distributions want.
...
3. Checks each required env var against your shell environment and the target profile's existing `.env`. Marks each as `âœ“ set` or `needs setting` so you know exactly what to configure.
4. Asks for confirmation. Pass `-y` / `--yes` to skip.
5. Copies distribution-owned files into `~/.zed/profiles/research-bot/` (or wherever the manifest's `name` resolves).
6. Writes `.env.EXAMPLE` with the required keys commented out â€” copy to `.env` and fill in.
...
Required keys that were already in your shell environment (e.g. `OPENAI_API_KEY` exported in your `~/.zshrc`) are marked `âœ“ set` during install â€” you don't need to duplicate them in `.env`.
...
`zed profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
3. **Preserves** your `config.yaml` â€” you may have tuned the model, temperature, or other settings. Pass `--force-config` to overwrite.
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `zed profile update research-bot`. Memories stay per-machine â€” the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
When the lead ships v1.1 (better SOUL, new skill), engineers run `zed profile update pr-reviewer` and everyone's on the new version within minutes.

### Community: publish a public agent

You built something novel â€” maybe a "Polymarket trader" or an "academic paper summarizer" or a "Minecraft server ops assistant." You want to share it.

```bash
# You
cd ~/.zed/profiles/polymarket-trader
# Write a solid README.md at the repo root â€” GitHub shows it on the repo page
...
Tweet the install command. People who try it send you issues and PRs. If someone wants to customize, they fork â€” same git workflow everyone already knows.

### Product: ship an opinionated agent

You built Zed-on-top â€” maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
You're the ops lead. You want a temporary agent that diagnoses a production incident â€” a canned SOUL with the right tools and MCP connections â€” and runs on three on-call engineers' laptops for the next week.
...
# Incident resolved â€” tear it down
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release â€” install currently tracks the default branch. Track your installed version via `zed profile info <name>` and hold off on updates until you're ready.
:::
...
â€¦and reference it from `config.yaml` or your SOUL as needed.
...
The installer hard-excludes these paths even if an author accidentally ships them. No config option lets you override this â€” the safety guard is a regression-tested invariant:

- `auth.json` â€” OAuth tokens, platform credentials
- `.env` â€” API keys, secrets
- `memories/` â€” conversation memory
- `sessions/` â€” conversation history
- `state.db`, `state.db-shm`, `state.db-wal` â€” session metadata
- `logs/` â€” agent and error logs
- `workspace/` â€” generated working files
- `plans/` â€” scratch plans
- `home/` â€” user's home mount in Docker backends
- `*_cache/` â€” image / audio / document caches
- `local/` â€” user-reserved customization namespace

When you clone a distribution, these simply aren't there. When you update, they stay put. If you installed the same distribution on five machines, you have five isolated sets of this data â€” one per machine.
...
Cron jobs from a distribution are **not auto-scheduled** â€” the installer prints `zed -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.

Rough analogy: installing a distribution is like installing a browser extension or a VS Code extension. Low friction, high power, trust the source. For internal company distributions, use a private repo and your normal git auth â€” nothing new to configure.
...
- `install`, `update`, `info` live inside `zed profile` â€” not a parallel command tree.
...
- After clone, `.git/` is stripped â€” the installed profile isn't itself a git checkout, avoiding "oh my, I accidentally committed my `.env` to the distribution's git history" traps.
- Reserved profile names (`zed`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.

## See also

- [Profiles: Running Multiple Agents](./profiles.md) â€” the base concept
- [Profile Commands reference](../reference/profile-commands.md) â€” every flag, every option
- [`zed profile export` / `import`](../reference/profile-commands.md#zed-profile-export) â€” local backup / restore (not distribution)
- [Using SOUL with Zed](../guides/use-soul-with-zed.md) â€” authoring personalities
- [Personality & SOUL](./features/personality.md) â€” how SOUL fits into the agent
- [Skills catalog](../reference/skills-catalog.md) â€” skills you can bundle
