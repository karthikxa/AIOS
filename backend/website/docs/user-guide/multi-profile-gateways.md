...
If you only run one Zed agent, you don't need this page â€” see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Zed agents that should all
...
(`ai.zed.gateway-<name>.plist`) or systemd user service
(`zed-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
zed profile create coder
zed profile create personal-bot
zed profile create research
...
the right choice for most setups. But on a host with many profiles â€” or a
container deployment where one process per profile is operationally heavy â€” you
...
changes â€” every behavior below is inert.
...
```bash
zed config set gateway.multiplex_profiles true
zed gateway restart
```

Equivalently, in the default profile's `~/.zed/config.yaml`:
...
keys** â€” credentials are never shared across profiles.

You do **not** run `zed gateway start` for the secondary profiles â€” the
...
With a multiplexer running, a named-profile `zed gateway start` / `run` is a
...
therefore **not** used in multiplex mode â€” you only manage the default gateway.
...
must not enable a port-binding platform itself** â€” doing so is a config error
...
Polling/connection platforms (Telegram, Discord, Slack, Matrix, Signal, â€¦) work
fine multiplexed, but each profile that enables one must supply its **own** bot
token â€” the same token cannot be polled by two profiles at once. If two profiles
configure the same `(platform, token)`, startup fails fast naming both profiles
(see [Token-conflict safety](#token-conflict-safety) â€” the rule is unchanged,
it's just enforced inside the one process now).

#### 4. Session keys are namespaced by profile

Each profile's sessions live under an `agent:<profile>:â€¦` namespace so two
profiles on the same platform/chat never collide in the shared session store.
The **default** profile keeps the historical `agent:main:â€¦` namespace
byte-for-byte, so existing default-profile sessions are unaffected â€” no
...
home). `zed status` reports the multiplexer and the profiles it serves;
`zed status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/zed-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: zed-gateways {start|stop|restart|status|list}"
}
...
```bash
zed-gateways start      # start every configured profile
zed-gateways stop       # stop every configured profile
zed-gateways restart    # restart all
zed-gateways status     # status across all
zed-gateways list       # delegates to `zed gateway list`
```

:::tip
The `default` profile is targeted with `zed gateway <action>` (no `-p`),
not `zed -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `zed -p coder gateway <action>` â€” useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.zed.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/zed-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.zed.gateway.plist` /
`zed-gateway.service`.
...
tail -f ~/.zed/logs/gateway.error.log

# Named profile
tail -f ~/.zed/profiles/<name>/logs/gateway.log
tail -f ~/.zed/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.zed/logs/gateway.log ~/.zed/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
zed logs -f                  # follow default profile
zed -p coder logs -f         # follow one profile
zed logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
zed profile list             # profiles + model + gateway state
zed-gateways status          # full status across every profile
launchctl list | grep zed    # macOS â€” PIDs and labels
systemctl --user list-units 'zed-gateway-*'   # Linux â€” units
```
...
```
~/.zed/profiles/<name>/
â”œâ”€â”€ .env              # API keys, bot tokens (chmod 600)
â”œâ”€â”€ config.yaml       # model, provider, toolsets, gateway settings
â””â”€â”€ SOUL.md           # personality / system prompt
```

The default profile uses `~/.zed/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
zed config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.zed/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=zed --why="gateways running" \
  sleep infinity &
...
`zed-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.zed/.env ~/.zed/profiles/*/.env
```

## Updating the code

`zed update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `zed gateway start` after a previous `zed gateway stop`. The
...
```bash
ps -ef | grep "zed_cli.*-p <profile>"
cat ~/.zed/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.zed.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.zed.gateway-<profile>.plist

# Linux
systemctl --user restart zed-gateway-<profile>.service
```

### Health check

```bash
zed doctor                  # default profile
zed -p <profile> doctor     # one profile
```
