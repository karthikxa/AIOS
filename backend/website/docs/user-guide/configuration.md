...
sidebar_position: 2
title: "Configuration"
description: "Configure Zed Agent â€” config.yaml, providers, models, API keys, and more"
---
...
â””â”€â”€ logs/           # Logs (errors.log, gateway.log â€” secrets auto-redacted)
```

## Managing Configuration

```bash
zed config              # View current configuration
zed config edit         # Open config.yaml in your editor
zed config set KEY VAL  # Set a specific value
zed config check        # Check for missing options (after updates)
zed config migrate      # Interactively add missing options

# Examples:
zed config set model anthropic/claude-opus-4
zed config set terminal.backend docker
zed config set OPENROUTER_API_KEY sk-or-...  # Saves to .env
```

:::tip
The `zed config set` command automatically routes values to the right file â€” API keys are saved to `.env`, everything else to `config.yaml`.
:::
...
1. **CLI arguments** â€” e.g., `zed chat --model anthropic/claude-sonnet-4` (per-invocation override)
2. **`~/.zed/config.yaml`** â€” the primary config file for all non-secret settings
3. **`~/.zed/.env`** â€” fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
Multiple references in a single value work: `url: "${HOST}:${PORT}"`. If a referenced variable is not set, the placeholder is kept verbatim (`${UNDEFINED_VAR}` stays as-is). Only the `${VAR}` syntax is supported â€” bare `$VAR` is not expanded.
...
`zed update` settings live under `updates` in `config.yaml`:

```yaml
updates:
  pre_update_backup: false       # Create a full ZED_HOME zip before every update
  backup_keep: 5                 # Keep this many pre-update backup zips
  non_interactive_local_changes: stash  # stash | discard
```

For git installs, Zed auto-stashes dirty tracked files and untracked files before checking out the update branch or pulling. Interactive terminal updates prompt before restoring that stash. Non-interactive updates (desktop/chat app, gateway, or `--yes`) use `updates.non_interactive_local_changes`: `stash` restores local source edits after a successful pull, while `discard` drops the update-created stash after a successful pull. Use `discard` only on managed installs where local source edits are never meant to persist.

Before that stash step, Zed also restores tracked `package-lock.json` diffs left by npm install/build churn. Commit or manually stash intentional lockfile edits before updating.

## Terminal Backend Configuration

Zed supports six terminal backends. Each determines where the agent's shell commands actually execute â€” your local machine, a Docker container, a remote server via SSH, a Modal cloud sandbox (direct or via the Nous-managed gateway), a Daytona workspace, or a Singularity/Apptainer container.
...
terminal:
  ...
  home_mode: auto   # auto | real | profile â€” subprocess HOME policy
...
For cloud sandboxes such as Modal and Daytona, `container_persistent: true` means Zed will try to preserve filesystem state across sandbox recreation. It does not promise that the same live sandbox, PID space, or background processes will still be running later.
...
find the credentials and config they already use in your normal shell. Zed
state is still profile-scoped through `ZED_HOME`; `HOME` is not how profiles
select config, memory, sessions, or skills.

Zed does **not** change your system-wide `HOME`, your shell startup files, or
the operating system account home. This setting only controls the environment
passed to subprocesses that Zed launches through tools such as `terminal`,
...
|---|---|---|---|
| `auto` | Keep the real OS-user `HOME` | Use `{ZED_HOME}/home` | Recommended default. Host CLIs keep working; container state persists. |
...
| `profile` | Use `{ZED_HOME}/home` when it exists | Use `{ZED_HOME}/home` when it exists | Strict per-profile CLI config isolation, but normal `~/.ssh`, `~/.gitconfig`, `~/.azure`, `~/.config/gh`, Claude/Codex auth, npm state, etc. will not be visible unless you initialize or link them inside the profile home. |
...
In that mode tool subprocesses use `{ZED_HOME}/home` as `HOME`. Zed also
sets `ZED_REAL_HOME` so scripts can still locate the actual user home when
they need it. Container backends keep using `{ZED_HOME}/home` in `auto` mode
because that directory lives on the persistent Zed data volume.

Scripts that need to distinguish profile state from the real user home should
prefer `ZED_HOME` for Zed data and `ZED_REAL_HOME` for the account home:

```python
from pathlib import Path
import os

zed_home = Path(os.environ["ZED_HOME"])
real_home = Path(os.environ.get("ZED_REAL_HOME", os.environ["HOME"]))
```

:::warning
The agent has the same filesystem access as your user account. Use `zed tools` to disable tools you don't want, or switch to Docker for sandboxing.
:::
...
**Single persistent container, shared across Zed processes.** Zed starts ONE long-lived container on first use and routes every terminal, file, and `execute_code` call through `docker exec` into that same container â€” across sessions, `/new`, `/reset`, and `delegate_task` subagents. Working-directory changes, installed packages, files in `/workspace`, and **background processes** all carry over from one tool call to the next, and from one Zed process to the next. When you close a TUI session, run `/quit`, or start a new `zed` invocation, the container keeps running and the next Zed process reuses it via a labeled lookup. See **Container lifecycle** below for the exact teardown rules.
...
terminal:
  ...
  # container shared across sessions" contract â€” see Container lifecycle).
  docker_persist_across_processes: true   # Reuse container across Zed restarts
  ...
  timeout: 180                     # Per-command timeout in seconds
  lifetime_seconds: 300            # Idle-reaper window; also feeds 2Ã— orphan-reaper threshold
```

**`docker_env`** vs **`docker_forward_env`**: the former injects literal `KEY=value` pairs you specify in the config (the values live in your `config.yaml` or are passed as a JSON dict via `TERMINAL_DOCKER_ENV='{"DEBUG":"1"}'`). The latter forwards values from your shell or `~/.zed/.env`, so the actual secret never appears in the config file. Use `docker_forward_env` for tokens and `docker_env` for static knobs the container needs.

**`terminal.docker_extra_args`** (also overridable via `TERMINAL_DOCKER_EXTRA_ARGS='["--gpus=all"]'`) lets you pass arbitrary `docker run` flags that Zed doesn't surface as first-class keys â€” `--gpus`, `--network`, `--add-host`, alternative `--security-opt` overrides, etc. Each entry must be a string; the list is appended last to the assembled `docker run` invocation so it can override Zed' defaults if needed. Use sparingly â€” flags that conflict with the sandbox hardening (capability drops, `--user`, the workspace bind mount) will silently weaken isolation.

**Requirements:** Docker Desktop or Docker Engine installed and running. Zed probes `$PATH` plus common macOS install locations (`/usr/local/bin/docker`, `/opt/homebrew/bin/docker`, Docker Desktop app bundle). Podman is supported out of the box: set `ZED_DOCKER_BINARY=podman` (or the full path) to force it when both are installed.

#### Container lifecycle

Every Zed-managed container is tagged with three labels so subsequent processes (and the orphan reaper) can identify it:

- `zed-agent=1` â€” marks it as Zed-managed
- `zed-task-id=<sanitized task_id>` â€” keys the per-task reuse probe
- `zed-profile=<sanitized profile name>` â€” scopes reuse and reaping to the active Zed profile

On startup, Zed runs `docker ps --filter label=zed-task-id=<id> --filter label=zed-profile=<profile>` and **attaches to the existing container** when it finds one. If the container is `exited` (e.g. after a Docker daemon restart), it's `docker start`'d and reused â€” filesystem state and any installed packages survive, but in-container background processes do not.

When a Zed process exits â€” `/quit`, closing a TUI session, gateway shutdown, even SIGKILL â€” the cleanup path is a **no-op for the container in default mode**. The container keeps running. The next Zed process attaches to it in milliseconds via the label probe. This is the behavior the "one long-lived container shared across sessions" contract requires: it's the only way background processes (npm watchers, dev servers, long-running pytest) survive across sessions.
...
| Orphan reaper at next startup | Sweeps **Exited** zed-labeled containers older than `2 Ã— lifetime_seconds` (default 600s = 10 min), scoped to the current profile. **Running containers are never touched** â€” sibling-process safety. Set `docker_orphan_reaper: false` to disable. |
...
- **Switching profiles** isolates containers from each other â€” a container labeled `zed-profile=work` is invisible to a Zed process running under `zed-profile=research`. The orphan reaper is profile-scoped too, so cross-profile containers don't get reaped accidentally, but they also won't get cleaned up automatically until you start Zed again under their original profile.

Parallel subagents spawned via `delegate_task(tasks=[...])` share this one container â€” concurrent `cd`, env mutations, and writes to the same path will collide. If a subagent needs an isolated sandbox, it must register a per-task image override via `register_task_env_overrides()`, which RL and benchmark environments (TerminalBench2, ZedSweEnv, etc.) do automatically for their per-task Docker images.
...
**Credential forwarding:** Env vars listed in `docker_forward_env` are resolved from your shell environment first, then `~/.zed/.env`. Skills can also declare `required_environment_variables` which are merged automatically.
...
| `TERMINAL_DOCKER_PERSIST_ACROSS_PROCESSES` | `docker_persist_across_processes` | `true` / `false` â€” default `true` |
| `TERMINAL_DOCKER_ORPHAN_REAPER` | `docker_orphan_reaper` | `true` / `false` â€” default `true` |
...
| `TERMINAL_CONTAINER_PERSISTENT` | `container_persistent` | `true` / `false` â€” controls the bind-mount workspace dirs, distinct from `docker_persist_across_processes` |
| `TERMINAL_LIFETIME_SECONDS` | `lifetime_seconds` | Idle reaper window |
| `TERMINAL_TIMEOUT` | `timeout` | Per-command timeout |
| `ZED_DOCKER_BINARY` | _none_ | Force a specific docker/podman binary path |

### SSH Backend

Runs commands on a remote server over SSH. Uses ControlMaster for connection reuse (5-minute idle keepalive). Persistent shell is enabled by default â€” state (cwd, env vars) survives across commands.
...
**Persistence:** When enabled, the sandbox filesystem is snapshotted on cleanup and restored on next session. Snapshots are tracked in `~/.zed/modal_snapshots.json`. This preserves filesystem state, not live processes, PID space, or background jobs.

**Credential files:** Automatically mounted from `~/.zed/` (OAuth tokens, etc.) and synced before each command.
...
terminal:
  backend: daytona
  container_cpu: 1                 # CPU cores
  container_memory: 5120           # MB â†’ converted to GiB
  container_disk: 10240            # MB â†’ converted to GiB (max 10 GiB)
  container_persistent: true       # Stop/resume instead of delete
```

**Required:** `DAYTONA_API_KEY` environment variable.

**Persistence:** When enabled, sandboxes are stopped (not deleted) on cleanup and resumed on next session. Sandbox names follow the pattern `zed-{task_id}`.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` â†’ `TERMINAL_SANDBOX_DIR/singularity` â†’ `/scratch/$USER/zed-agent` (HPC convention) â†’ `~/.zed/sandboxes/singularity`.
...
- **Local** â€” No special requirements. The safest default when getting started.
- **Docker** â€” Run `docker version` to verify Docker is working. If it fails, fix Docker or `zed config set terminal.backend local`.
- **SSH** â€” Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set. Zed logs a clear error if either is missing.
- **Modal** â€” Needs `MODAL_TOKEN_ID` env var or `~/.modal.toml`. Run `zed doctor` to check.
- **Daytona** â€” Needs `DAYTONA_API_KEY`. The Daytona SDK handles server URL configuration.
- **Singularity** â€” Needs `apptainer` or `singularity` in `$PATH`. Common on HPC clusters.

When in doubt, set `terminal.backend` back to `local` and verify that commands run there first.

### Remote-to-Host File Sync on Teardown

For the **SSH**, **Modal**, and **Daytona** backends (anywhere the agent's working tree lives on a different machine than the host running Zed), Zed tracks files the agent touched inside the remote sandbox and, on session teardown / sandbox cleanup, **syncs the modified files back to the host** under `~/.zed/cache/remote-syncs/<session-id>/`.
...
- The remote sandbox may have been torn down by the time you go looking; the local `~/.zed/cache/remote-syncs/â€¦` copy is the authoritative record of what the agent changed.
- Large binary outputs (model checkpoints, raw datasets) are capped by size â€” the sync skips files over `file_sync_max_mb` (default `100`). Bump that if you expect bigger artifacts to come back.

```yaml
terminal:
  file_sync_max_mb: 100     # default â€” sync files up to 100 MB each
  file_sync_enabled: true   # default â€” set false to skip the sync entirely
```
...
terminal:
  ...
  docker_volumes:
    ...
    - "/home/user/.zed/cache/documents:/output" # Gateway-visible exports
```
...
Zed resolves each listed variable from your current shell first, then falls back to `~/.zed/.env` if it was saved with `zed config set`.
...
When enabled, Zed appends `--user $(id -u):$(id -g)` to the `docker run` command so files written into bind-mounted directories (`/workspace`, `/root`, anything in `docker_volumes`) are owned by your host user, not root. The trade-off: the container can no longer `apt install` or write to root-owned paths like `/root/.npm` â€” use a base image whose `HOME` is owned by a non-root user (or add your required tooling at image build time) if you need both.
...
Docker sandboxes stay isolated by default. Zed does **not** pass your current host working directory into the container unless you explicitly opt in.
...
When enabled:
- if you launch Zed from `~/projects/my-app`, that host directory is bind-mounted to `/workspace`
...
- `true` gives the sandbox direct access to the directory you launched Zed from

Use the opt-in only when you intentionally want the container to work on live host files.

### Persistent Shell

By default, each terminal command runs in its own subprocess â€” working directory, environment variables, and shell variables reset between commands. When **persistent shell** is enabled, a single long-lived bash process is kept alive across `execute()` calls so that state survives between commands.
...
```yaml
terminal:
  persistent_shell: true   # default â€” enables persistent shell for SSH
```
...
skills:
  config:
    myplugin:
      path: ~/myplugin-data   # Example â€” each skill defines its own keys
```

**How skill settings work:**

- `zed config migrate` scans all enabled skills, finds unconfigured settings, and offers to prompt you
- `zed config show` displays all skill settings under "Skill Settings" with the skill they belong to
...
For details on declaring config settings in your own skills, see [Creating Skills â€” Config Settings](/developer-guide/creating-skills#config-settings-configyaml).

### Guard on agent-created skill writes

When the agent uses `skill_manage` to create, edit, patch, or delete a skill, Zed can optionally scan the new/updated content for dangerous keyword patterns (credential harvesting, obvious prompt injection, exfil instructions). The scanner is **off by default** â€” real agent workflows that legitimately touch `~/.ssh/` or mention `$OPENAI_API_KEY` were tripping the heuristic too often. Turn it back on if you want the scanner to prompt you before the agent's skill writes land:
...
Independent of the content scanner above, `skills.write_approval` gates **every** agent skill write (create / edit / patch / delete / supporting files) behind your explicit approval â€” the same approve/deny mechanism as dangerous commands:
...
When on, skill writes are staged under `~/.zed/pending/skills/` and reviewed with `/skills pending`, `/skills diff <id>`, `/skills approve <id>`, `/skills reject <id>` â€” from the CLI or any messaging platform. Toggle at runtime with `/skills approval on|off`. Memory has the same gate (`memory.write_approval`, below). Full walkthrough: [Gating agent skill writes](/user-guide/features/skills#gating-agent-skill-writes-skillswrite_approval).
...
With `memory.write_approval: true`, memory writes need your approval before they land: interactive CLI turns prompt inline; messaging sessions and the background self-improvement review stage the write for `/memory pending` â†’ `/memory approve <id>` / `/memory reject <id>` review. Toggle at runtime with `/memory approval on|off`. See [Controlling memory writes](/user-guide/features/memory#controlling-memory-writes-write_approval).

## Context File Truncation

Controls how much content Zed loads from each automatic context file before applying head/tail truncation. This applies to files injected into the system prompt such as `SOUL.md`, `.zed.md`, `AGENTS.md`, `CLAUDE.md`, and `.cursorrules`. It does **not** affect the `read_file` tool.
...
```yaml
file_read_max_chars: 100000  # default â€” ~25-35K tokens
```
...
The agent also deduplicates file reads automatically â€” if the same file region is read twice and the file hasn't changed, a lightweight stub is returned instead of re-sending the content. This resets on context compression so the agent can re-read files after their content is summarized away.

## Tool Output Truncation Limits

Three related caps control how much raw output a tool can return before Zed truncates it:
...
- **`max_bytes`** â€” When a `terminal` command produces more than this many characters of combined stdout/stderr, Zed keeps the first 40% and last 60% and inserts a `[OUTPUT TRUNCATED]` notice between them. Default `50000` (â‰ˆ12-15K tokens across typical tokenisers).
- **`max_lines`** â€” Upper bound on the `limit` parameter of a single `read_file` call. Requests above this are clamped so a single read can't flood the context window. Default `2000`.
- **`max_line_length`** â€” Per-line cap applied when `read_file` emits the line-numbered view. Lines longer than this are truncated to this many chars followed by `... [truncated]`. Default `2000`.
...
`zed tools`), so a toolset listed here is always removed â€” even if a
...
```yaml
worktree: true    # Always create a worktree (same as zed -w)
# worktree: false # Default â€” only when -w flag is passed
```
...
Zed automatically compresses long conversations to stay within your model's context window. The compression summarizer is a separate LLM call â€” you can point it at any provider or endpoint.
...
compression:
  ...
  hygiene_hard_message_limit: 400                   # Gateway safety valve â€” see below
...
`hygiene_hard_message_limit` is a gateway-only **pre-compression safety valve**. Runaway sessions with thousands of messages can hit model context limits before the normal percent-of-context threshold fires; when message count crosses this ceiling, Zed forces compression regardless of token usage. Default `400` â€” raise it for platforms where very long sessions are normal, lower it to force more aggressive compression. Editing this value on a running gateway takes effect on the next message (see below).

`protect_first_n` controls how many **non-system** head messages are pinned across every compaction. Default `3` â€” the opening user/assistant exchange survives every summarizer pass so the original goal stays visible. On long-running rolling-compaction sessions where the opening turn is no longer relevant, set `protect_first_n: 0` to pin nothing but the system prompt + summary + tail. The system prompt itself is always preserved regardless of this setting.

:::tip Gateway hot-reload of compression and context length
As of recent releases, editing `model.context_length` or any `compression.*` key in `config.yaml` on a running gateway takes effect on the next message â€” no gateway restart, no `/reset`, no session rotation required. The cached-agent signature includes these keys, so the gateway transparently rebuilds the agent when it sees a change. API keys and tool/skill config still require the usual reload paths.
:::

### Common setups

**Default (auto-detect) â€” no configuration needed:**
```yaml
compression:
  enabled: true
  threshold: 0.50
...
Plugin engines are **never auto-activated** â€” you must explicitly set `context.engine` to the plugin name. Available engines can be browsed and selected via `zed plugins` â†’ Provider Plugins â†’ Context Engine.
...
Warnings are injected into the last tool result's JSON (as a `_budget_warning` field) rather than as separate messages â€” this preserves prompt caching and doesn't disrupt the conversation structure.
...
When the iteration budget is fully exhausted, the CLI shows a notification to the user: `âš  Iteration budget reached (90/90) â€” response may be incomplete`. If the budget runs out during active work, the agent generates a summary of what was accomplished before stopping.

`agent.api_max_retries` controls how many times Zed retries a provider API call on transient errors (rate limits, connection drops, 5xx) **before** fallback-provider switching engages. The default is `3` â€” four attempts total. If you have [fallback providers](/user-guide/features/fallback-providers) configured and want to fail over faster, drop this to `0` so the first transient error on your primary immediately hands off to the fallback instead of churning retries against the flaky endpoint.

### API Timeouts

Zed has separate timeout layers for streaming, plus a stale detector for non-streaming calls. The stale detectors auto-adjust for local providers only when you leave them at their implicit defaults.

| Timeout | Default | Local providers | Config / env |
|---------|---------|----------------|--------------|
| Socket read timeout | 120s | Auto-raised to 1800s | `ZED_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Auto-disabled | `ZED_STREAM_STALE_TIMEOUT` |
| Stale non-stream detection | 300s | Auto-disabled when left implicit | `providers.<id>.stale_timeout_seconds` or `ZED_API_CALL_STALE_TIMEOUT` |
| API call (non-streaming) | 1800s | Unchanged | `providers.<id>.request_timeout_seconds` / `timeout_seconds` or `ZED_API_TIMEOUT` |

The **socket read timeout** controls how long httpx waits for the next chunk of data from the provider. Local LLMs can take minutes for prefill on large contexts before producing the first token, so Zed raises this to 30 minutes when it detects a local endpoint. If you explicitly set `ZED_STREAM_READ_TIMEOUT`, that value is always used regardless of endpoint detection.
...
The **stale non-stream detection** kills non-streaming calls that produce no response for too long. By default Zed disables this on local endpoints to avoid false positives during long prefills. If you explicitly set `providers.<id>.stale_timeout_seconds`, `providers.<id>.models.<model>.stale_timeout_seconds`, or `ZED_API_CALL_STALE_TIMEOUT`, that explicit value is honored even on local endpoints.

## Context Pressure Warnings

Separate from iteration budget pressure, context pressure tracks how close the conversation is to the **compaction threshold** â€” the point where context compression fires to summarize older messages. This helps both you and the agent understand when the conversation is getting long.

| Progress | Level | What happens |
|----------|-------|-------------|
| **â‰¥ 60%** to threshold | Info | CLI shows a cyan progress bar; gateway sends an informational notice |
| **â‰¥ 85%** to threshold | Warning | CLI shows a bold yellow bar; gateway warns compaction is imminent |
...
Context pressure is automatic â€” no configuration needed. It fires purely as a user-facing notification and does not modify the message stream or inject anything into the model's context.
...
No knob exists to disable this â€” caching is always-on and saves money even on single-turn conversations because the system prompt alone is a meaningful fraction of the input token count.
...
Instead of hand-editing YAML, run `zed model` and pick **"Configure auxiliary models"** from the menu. You'll get an interactive per-task picker:
...
Select a task, pick a provider (OAuth flows open a browser; API-key providers prompt), pick a model. The change persists to `auxiliary.<task>.*` in `config.yaml`. Same machinery as the main-model picker â€” no extra syntax to learn.
...
Every model slot in Zed â€” auxiliary tasks, compression, fallback â€” uses the same three knobs:
...
When `base_url` is set, Zed ignores the provider and calls that endpoint directly (using `api_key` or `OPENAI_API_KEY` for auth). When only `provider` is set, Zed uses that provider's built-in auth and base URL.

Available providers for auxiliary tasks: `auto`, `main`, plus any provider in the [provider registry](/reference/environment-variables) â€” `openrouter`, `nous`, `openai-codex`, `copilot`, `copilot-acp`, `anthropic`, `gemini`, `google-gemini-cli`, `qwen-oauth`, `zai`, `kimi-coding`, `kimi-coding-cn`, `minimax`, `minimax-cn`, `minimax-oauth`, `deepseek`, `nvidia`, `xai`, `xai-oauth`, `ollama-cloud`, `alibaba`, `bedrock`, `huggingface`, `arcee`, `xiaomi`, `kilocode`, `opencode-zen`, `opencode-go`, `azure-foundry` â€” or any named custom provider from your `custom_providers` list (e.g. `provider: "beans"`).

:::tip MiniMax OAuth
`minimax-oauth` logs in via browser OAuth (no API key needed). Run `zed model` and select **MiniMax (OAuth)** to authenticate. Auxiliary tasks use `MiniMax-M2.7-highspeed` automatically. See the [MiniMax OAuth guide](../guides/minimax-oauth.md).
:::

:::tip xAI Grok OAuth
`xai-oauth` logs in via browser OAuth for SuperGrok and X Premium+ subscribers (no API key needed). Run `zed model` and select **xAI Grok OAuth (SuperGrok / Premium+)** to authenticate. The same OAuth token is reused for every direct-to-xAI surface (chat, auxiliary tasks, TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md), and if Zed is on a remote host see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md).
:::

:::warning `"main"` is for auxiliary tasks only
The `"main"` provider option means "use whatever provider my main agent uses" â€” it's only valid inside `auxiliary:`, `compression:`, and primary fallback entries (`fallback_providers:` or legacy `fallback_model:`). It is **not** a valid value for your top-level `model.provider` setting. If you use a custom OpenAI-compatible endpoint, set `provider: custom` in your `model:` section. See [AI Providers](/integrations/providers) for all main model provider options.
:::
...
auxiliary:
  ...
  vision:
    ...
    timeout: 120               # seconds â€” LLM API call timeout; vision payloads need generous timeout
    download_timeout: 30       # seconds â€” image HTTP download; increase for slow connections
  ...
  web_extract:
    ...
    base_url: ""
    api_key: ""
    timeout: 360               # seconds (6min) â€” per-attempt LLM summarization
  ...
  compression:
    timeout: 120               # seconds â€” compression summarizes long conversations, needs more time
  ...
  # Skills hub â€” skill matching and search
  skills_hub:
    provider: "auto"
    model: ""
    base_url: ""
  ...
  # Kanban triage specifier â€” `zed kanban specify <id>` (or the
  # dashboard's âœ¨ Specify button on Triage-column cards) uses this
  ...
  compression:
    ...
    extra_body:
      ...
      plugins:                             # OpenRouter Pareto Code router knob
        - id: pareto-router
          min_coding_score: 0.5            # 0.0â€“1.0; higher = stronger coders
```

The shape mirrors what OpenRouter accepts in the chat completions request body. Zed forwards the entire `extra_body` verbatim, so any other OpenRouter request-body field documented at [openrouter.ai/docs](https://openrouter.ai/docs) works the same way.
...
Or via environment variable (in `~/.zed/.env`):
...
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `zed model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. **Auxiliary tasks only â€” not valid for `model.provider`.** | Custom endpoint credentials + base URL |
...
`base_url` takes precedence over `provider`, so this is the most explicit way to route an auxiliary task to a specific endpoint. For direct endpoint overrides, Zed uses the configured `api_key` or falls back to `OPENAI_API_KEY`; it does not reuse `OPENROUTER_API_KEY` for that custom endpoint.
...
**Using Codex OAuth** (ChatGPT Pro/Plus account â€” no API key needed):
```yaml
auxiliary:
  vision:
...
```
Run `zed model` and select **MiniMax (OAuth)** to log in and set this automatically. For the China region, the base URL will be `https://api.minimaxi.com/anthropic`. See the [MiniMax OAuth guide](../guides/minimax-oauth.md) for the full walkthrough.
...
`provider: "main"` uses whatever provider Zed uses for normal chat â€” whether that's a named custom provider (e.g. `beans`), a built-in provider like `openrouter`, or a legacy `OPENAI_BASE_URL` endpoint.

:::tip
If you use Codex OAuth as your main model provider, vision works automatically â€” no extra configuration needed. Codex is included in the auto-detection chain for vision.
:::

:::warning
**Vision requires a multimodal model.** If you set `provider: "main"`, make sure your endpoint supports multimodal/vision â€” otherwise image analysis will fail.
:::

### Environment Variables (legacy)

Auxiliary models can also be configured via environment variables. However, `config.yaml` is the preferred method â€” it's easier to manage and supports all options including `base_url` and `api_key`.
...
:::tip
Run `zed config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
When unset (default), reasoning effort defaults to "medium" â€” a balanced level that works well for most tasks. Setting a value overrides it â€” higher reasoning effort gives better results on complex tasks at the cost of more tokens and latency.
...
field â€” OpenRouter ignores it for them. Zed transparently routes your
...
knob keeps working â€” no extra configuration needed. `none` (or unset) leaves the
...
1. **General tool-use enforcement** (all matched models) â€” instructs the model to make tool calls immediately instead of describing intentions, keep working until the task is complete, and never end a turn with a promise of future action.

2. **OpenAI execution discipline** (GPT and Codex models only) â€” additional guidance addressing GPT-specific failure modes: abandoning work on partial results, skipping prerequisite lookups, hallucinating instead of using tools, and declaring "done" without verification.

3. **Google operational guidance** (Gemini and Gemma models only) â€” conciseness, absolute paths, parallel tool calls, and verify-before-edit patterns.
...
tts:
  ...
  edge:
    ...
    speed: 1.0                  # Speed multiplier (converted to rate percentage, e.g. 1.5 â†’ +50%)
  elevenlabs:
  ...
  openai:
    ...
    speed: 1.0                  # Speed multiplier (clamped to 0.25â€“4.0 by the API)
...
**Speed fallback hierarchy:** provider-specific speed (e.g. `tts.edge.speed`) â†’ global `tts.speed` â†’ `1.0` default. Set the global `tts.speed` to apply a uniform speed across all providers, or override per-provider for fine-grained control.
...
display:
  ...
  tool_progress_overrides: {}  # DEPRECATED â€” use display.platforms instead
...
When `display.file_mutation_verifier` is `true` (default), Zed appends a one-line advisory to the assistant's final response whenever a `write_file` or `patch` call failed during the turn and was never superseded by a successful write to the same path. This catches the "batch of parallel patches, half silently fail, model summarises success" class of over-claim without requiring you to manually run `git status` after every edit.
...
Set `file_mutation_verifier: false` (or `ZED_FILE_MUTATION_VERIFIER=0`) to suppress the footer. The verifier only fires when real failures are outstanding at turn end â€” a model that retries a failed patch and succeeds within the same turn will not trigger it for that file.

### UI language for static messages

The `display.language` setting translates a small set of static user-facing messages â€” the CLI approval prompt, a handful of gateway slash-command replies (e.g. restart-drain notices, "approval expired", "goal cleared"). It does **not** translate agent responses, log lines, tool output, error tracebacks, or slash-command descriptions â€” those stay in English. If you want the agent itself to reply in another language, just tell it in your prompt or system message.
...
You can also set this per-session with the `ZED_LANGUAGE` env var, which overrides the config value.
...
When `display.runtime_footer.enabled: true`, Zed appends a small runtime-context footer to the **final** message of each gateway turn. The current footer can show the model, context-window percentage, and current working directory. Off by default; opt in per-gateway if your team wants every reply to include this provenance.
...
`interim_assistant_messages` is gateway-only. When enabled, Zed sends completed mid-turn assistant updates as separate chat messages. This is independent from `tool_progress` and does not require gateway streaming.
...
Hashes are deterministic â€” the same user always maps to the same hash, so the model can still distinguish between users in group chats. Routing and delivery use the original values internally.
...
If the requested provider is unavailable, Zed falls back automatically in this order: `local` â†’ `groq` â†’ `openai`.
...
streaming:
  ...
  cursor: " â–‰"            # Cursor shown during streaming
  fresh_final_after_seconds: 0    # Opt in to fresh final (Telegram) when preview is this old
```

When enabled, the bot sends a message on the first token, then progressively edits it as more tokens arrive. Platforms that don't support message editing (Signal, Email, Home Assistant) are auto-detected on the first attempt â€” streaming is gracefully disabled for that session with no flood of messages.
...
The master `streaming.enabled` switch is `false` by default â€” nothing streams until you flip it. Once enabled, streaming is decided **per platform**: Telegram ships with `display.platforms.telegram.streaming: true` (streams) and Discord with `display.platforms.discord.streaming: false` (does not). So after enabling streaming, Telegram streams out of the box and Discord stays on whole-message replies until you change its toggle. You can adjust these per-platform switches from the dashboard's **Channels** toggles or directly in `~/.zed/config.yaml`.
:::
...
When the cap is reached, Zed returns a direct limit message for new sessions.
Existing active sessions keep their normal behavior.

The canonical key is top-level `max_concurrent_sessions`. Zed also accepts
`gateway.max_concurrent_sessions` as a fallback, but the top-level key wins when
both are set.

The cap is enforced with a local runtime lease file and is best-effort: Zed
fails open if the registry cannot be read or locked so users are not stranded.
It is intended for a single host/profile runtime, not a shared `$ZED_HOME`
...
- `false` reverts to the old shared-room behavior. That can be useful if you explicitly want Zed to treat a channel like one collaborative conversation, but it also means users share context, token costs, and interrupt state.
- Direct messages are unaffected. Zed still keys DMs by chat/DM ID as usual.
...
Control what Zed does when an unknown user sends a direct message:
...
- `pair` is the default. Zed denies access, but replies with a one-time pairing code in DMs.
...
Usage: type `/status`, `/disk`, `/update`, `/gpu`, or `/restart` in the CLI or any messaging platform. `exec` commands run locally on the host and return the output directly â€” no LLM call, no tokens consumed. `alias` commands rewrite to the configured slash command target.

- **30-second timeout** â€” long-running commands are killed with an error message
- **Priority** â€” quick commands are checked before skill commands, so you can override skill names
- **Autocomplete** â€” quick commands are resolved at dispatch time and are not shown in the built-in slash-command autocomplete tables
- **Type** â€” supported types are `exec` and `alias`; other types show an error
- **Works everywhere** â€” CLI, Telegram, Discord, Slack, WhatsApp, Signal, Email, Home Assistant
...
- **`project`** (default) â€” scripts run in the session's working directory with the active virtualenv/conda env's python. Project deps (`pandas`, `torch`, project packages) and relative paths (`.env`, `./data.csv`) resolve naturally, matching what `terminal()` sees.
- **`strict`** â€” scripts run in a temp staging directory with `sys.executable` (Zed's own python). Maximum reproducibility, but project deps and relative paths won't resolve.

Environment scrubbing (strips `*_API_KEY`, `*_TOKEN`, `*_SECRET`, `*_PASSWORD`, `*_CREDENTIAL`, `*_PASSWD`, `*_AUTH`) and the tool whitelist apply identically in both modes â€” switching mode does not change the security posture.

## Web Search Backends

The `web_search` and `web_extract` tools support five backend providers. Configure the backend in `config.yaml` or via `zed tools`:
...
|---------|---------|--------|---------|
| **Firecrawl** (default) | `FIRECRAWL_API_KEY` | âœ” | âœ” |
...
**Parallel search modes:** Set `PARALLEL_SEARCH_MODE` to control search behavior â€” `fast`, `one-shot`, or `agentic` (default: `agentic`).

**Exa:** Set `EXA_API_KEY` in `~/.zed/.env`. Supports `category` filtering (`company`, `research paper`, `news`, `people`, `personal site`, `pdf`) and domain/date filters.
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.zed/browser_recordings/
  # Optional CDP override â€” when set, Zed attaches directly to your own
  # Chromium-family browser (via /browser connect) rather than starting a headless browser.
  cdp_url: ""
  # Dialog supervisor â€” controls how native JS dialogs (alert / confirm / prompt)
  ...
  camofox:
    ...
    session_key: ""              # Optional session key sent when Zed creates a tab
    adopt_existing_tab: false    # Reuse an existing tab for this identity before creating one
```

**Dialog policies:**

- `must_respond` (default) â€” capture the dialog, surface it in `browser_snapshot.pending_dialogs`, and wait for the agent to call `browser_dialog(action=...)`. After `dialog_timeout_s` seconds with no response, the dialog is auto-dismissed to prevent the page's JS thread from stalling forever.
- `auto_dismiss` â€” capture, dismiss immediately. The agent still sees the dialog record in `browser_snapshot.recent_dialogs` with `closed_by="auto_policy"` after the fact.
- `auto_accept` â€” capture, accept immediately. Useful for pages with aggressive `beforeunload` prompts.
...
- `require_mention` â€” when `true` (default), the bot only responds in server channels when mentioned with `@BotName`. DMs always work without mention.
- `free_response_channels` â€” comma-separated list of channel IDs where the bot responds to every message without requiring a mention.
- `auto_thread` â€” when `true` (default), mentions in channels automatically create a thread for the conversation, keeping channels clean (similar to Slack threading).
...
- `redact_secrets` â€” when `true`, automatically detects and redacts patterns that look like API keys, tokens, and passwords in tool output before it enters the conversation context and logs. **On by default**. Set to `false` explicitly only when you need raw credential-like strings for debugging or redactor development.
- `tirith_enabled` â€” when `true`, terminal commands are scanned by [Tirith](https://github.com/sheeki03/tirith) before execution to detect potentially dangerous operations.
- `tirith_path` â€” path to the tirith binary. Set this if tirith is installed in a non-standard location.
- `tirith_timeout` â€” maximum seconds to wait for a tirith scan. Commands proceed if the scan times out.
- `tirith_fail_open` â€” when `true` (default), commands are allowed to execute if tirith is unavailable or fails. Set to `false` to block commands when tirith cannot verify them.
...
Control how Zed handles potentially dangerous commands:
...
| `off` | Skip all approval checks. Equivalent to `ZED_YOLO_MODE=true`. **Use with caution.** |

Smart mode is particularly useful for reducing approval fatigue â€” it lets the agent work more autonomously on safe operations while still catching genuinely destructive commands.
...
```yaml
checkpoints:
  enabled: false                 # Enable automatic checkpoints (also: zed chat --checkpoints). Default: false (opt-in).
...
delegation:
  ...
  # api_mode: ""                            # Wire protocol for base_url: "chat_completions", "codex_responses", or "anthropic_messages". Empty = auto-detect from URL (e.g. /anthropic suffix â†’ anthropic_messages). Set explicitly for non-standard endpoints the heuristic can't detect.
...
**Direct endpoint override:** If you want the obvious custom-endpoint path, set `delegation.base_url`, `delegation.api_key`, and `delegation.model`. That sends subagents directly to that OpenAI-compatible endpoint and takes precedence over `delegation.provider`. If `delegation.api_key` is omitted, Zed falls back to `OPENAI_API_KEY` only.

**Wire protocol (`api_mode`):** Zed auto-detects the wire protocol from `delegation.base_url` (e.g. paths ending in `/anthropic` â†’ `anthropic_messages`; Codex / native Anthropic / Kimi-coding hostnames keep their existing detection). For endpoints the heuristic can't classify â€” for example Azure AI Foundry, MiniMax, Zhipu GLM, or LiteLLM proxies fronting an Anthropic-shaped backend â€” set `delegation.api_mode` explicitly to one of `chat_completions`, `codex_responses`, or `anthropic_messages`. Leave it empty (the default) to keep auto-detection.
...
Zed uses two different context scopes:

| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** â€” defines who the agent is (slot #1 in the system prompt) | `~/.zed/SOUL.md` or `$ZED_HOME/SOUL.md` |
| `.zed.md` / `ZED.md` | Project-specific instructions (highest priority) | Walks to git root |
...
- If SOUL.md is missing, empty, or cannot be loaded, Zed falls back to a built-in default identity.
- **Project context files use a priority system** â€” only ONE type is loaded (first match wins): `.zed.md` â†’ `AGENTS.md` â†’ `CLAUDE.md` â†’ `.cursorrules`. SOUL.md is always loaded independently.
- **AGENTS.md** is hierarchical: if subdirectories also have AGENTS.md, all are combined.
- Zed automatically seeds a default `SOUL.md` if one does not already exist.
...
| Context | Default |
|---------|---------|
| **CLI (`zed`)** | Current directory where you run the command |
| **Messaging gateway** | `terminal.cwd` from `~/.zed/config.yaml`; if unset, home directory `~` |
...
`MESSAGING_CWD` and direct `TERMINAL_CWD` entries in `~/.zed/.env` are legacy compatibility fallbacks. New configurations should use `terminal.cwd`.
