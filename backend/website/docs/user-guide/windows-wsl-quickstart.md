...
description: "Run Zed Agent on Windows via WSL2 â€” setup, filesystem access between Windows and Linux, networking, and common pitfalls"
...
Zed Agent now supports **both** native Windows and WSL2.  This page covers the WSL2 path; for the native PowerShell install see the dedicated **[Windows (Native) Guide](./windows-native.md)**.
...
- You're doing POSIX-heavy development work and want your Zed sessions to share the same filesystem / paths as your dev tools.
- You already have a WSL2 environment and don't want to maintain a second install.

**When native is fine (or better):**
- Interactive chat, gateway (Telegram/Discord/etc.), cron scheduler, browser tool, MCP servers, and most Zed features all run natively on Windows.
...
This guide covers the parts of that split that specifically affect Zed: installing WSL2, getting files back and forth between Windows and Linux, networking in both directions, and the pitfalls people actually hit.

:::info ç®€ä½“ä¸­æ–‡
A Chinese-language walkthrough of the minimum install path is maintained on this same page â€” switch via the **language** menu (top right) and select **ç®€ä½“ä¸­æ–‡**.
:::

## Why WSL2 (vs. native Windows)

The native Windows install runs in Windows directly: your Windows terminal (PowerShell, Windows Terminal, etc.), Windows filesystem paths (`C:\Users\â€¦`), and Windows processes.  Zed uses Git Bash to run shell commands, which is how Claude Code and other agents handle Windows today â€” it sidesteps the POSIX-vs-Windows gap without a full rewrite.

WSL2 runs a real Linux kernel in a lightweight VM, so Zed inside it is essentially identical to running on Ubuntu.  That's valuable when you want a real POSIX environment: `fork`, `/tmp`, UNIX sockets, signal semantics, PTY-backed terminals, shells like `bash`/`zsh`, and tools like `rg`, `git`, `ffmpeg` that behave the way they do on Linux.

Practical consequences of WSL2:

- The Zed CLI, gateway, sessions, memory, skills, and tool runtimes all live inside the Linux VM.
- Windows programs (browsers, native apps, Chrome with your logged-in profile) live outside it.
- Every time you want the two to talk â€” share files, open URLs, control Chrome, hit a local model server, expose the Zed gateway to your phone â€” you cross a boundary. Those boundaries are what this guide is about.
...
Zed does not work reliably on WSL1 â€” WSL1 translates Linux syscalls on the fly and some behaviors (procfs, signals, network) diverge from real Linux.

### Distro choice

Ubuntu (LTS) is what we test against. Debian works. Arch and NixOS work for people who want them, but the one-line installer assumes a Debian-derived `apt` system â€” see the [Nix setup guide](/getting-started/nix-setup) for that path.

### Enable systemd (recommended)

The zed gateway (and anything else you want to keep running) is easier to manage with systemd. On modern WSL, enable it once inside your distro:
...
The installer treats WSL2 as plain Linux â€” nothing WSL-specific is needed. See [Installation](/getting-started/installation) for the full layout.

## Filesystem: crossing the Windows â†” WSL2 boundary
...
### Where to put Zed and your projects

**Rule of thumb: keep everything Linux-ish inside the Linux filesystem.**

- Your Zed install (`~/.zed/`) â€” Linux side. The installer already does this.
...
- **Fast I/O.** Operations on `/mnt/c/...` go through 9P and are 10â€“100Ã— slower than native ext4. `git status` on a 10k-file repo that feels instant under `~/code` can take 15+ seconds under `/mnt/c`.
...
```bash
sudo apt install wslu     # once â€” gives you wslview, wslpath, wslopen, etc.
...
Clone inside WSL. Always, unless you have a specific reason not to. A typical Zed workflow (`zed chat`, tool calls that `rg`/`ripgrep` the repo, file watchers, background gateway) will be dramatically faster and more reliable against `~/code/myrepo` than `/mnt/c/Users/you/myrepo`.

One exception: **MCP bridges that launch Windows binaries.** If you're using `chrome-devtools-mcp` through `cmd.exe` (see [MCP guide: WSL â†’ Windows Chrome](/guides/use-mcp-with-zed#wsl2-bridge-zed-in-wsl-to-windows-chrome)), Windows may complain with a `UNC` warning if Zed's current working directory is `~`. In that case, start Zed from somewhere under `/mnt/c/` so the Windows process has a drive-letter cwd.

## Networking: WSL â†” Windows
...
### Case 1 â€” Zed in WSL talks to a service on Windows

Most common: you're running **Ollama, LM Studio, or a llama-server on Windows**, and Zed (inside WSL) needs to hit it.

The canonical how-to for this lives in the providers guide: **[WSL2 Networking for Local Models â†’](/integrations/providers#wsl2-networking-windows-users)**
...
### Case 2 â€” Something on Windows (or your LAN) talks to Zed in WSL

This is the reverse direction and is less documented elsewhere, but it's what you need for:

- Using the Zed **web dashboard** from a Windows browser.
- Using the **OpenAI-compatible API server** (exposed by `zed gateway` when `API_SERVER_ENABLED=true`) from a Windows-side tool. See the [API Server feature page](/user-guide/features/api-server).
...
On **NAT mode** (Windows 10 / older Windows 11), the default "localhost forwarding" in WSL2 will generally forward Linux-side `127.0.0.1` binds to Windows `localhost`, so a Zed service started with `--host 127.0.0.1` is usually reachable as `http://localhost:PORT` from Windows. If it isn't:
...
2. **Port-forward Windows â†’ WSL VM.** In mirrored mode this is automatic. In NAT mode you have to do it yourself, per port, in Admin PowerShell:
   ...
   # Forward Windows port 8080 â†’ WSL:8080
...
For webhooks from cloud messaging providers (Telegram `setWebhook`, Slack events, etc.), don't fight port-forwarding â€” use `cloudflared` tunnels. See the [webhooks guide](/user-guide/messaging/webhooks).

## Running Zed services long-term on Windows

The Zed [Tool Gateway](/user-guide/features/tool-gateway) and the API server are long-lived processes. In WSL2 you have a few options for keeping them up.

### Desktop shortcut for opening Zed quickly

If you just want a double-click launcher for an interactive Zed shell, create
...
2. For the target, use your distro name (replace `Ubuntu` if needed):
   ...
   ```text
   wt.exe -w 0 -p "Ubuntu" wsl.exe -d Ubuntu --cd ~ -- bash -ic "zed"
   ```

3. Name it something obvious like `Zed`.

That opens Windows Terminal, starts your WSL distro, drops you in your Linux
home directory, and launches Zed. If `zed` is not on PATH yet, open WSL
once manually and run `source ~/.bashrc`, or replace the command with
`uv run zed` inside your project checkout.

Optional polish:

- **Custom icon:** open **Properties -> Change Icon** and point it at an `.ico`
  file, such as the Zed favicon from the repo.
...
If you enabled systemd per the setup section above, `zed gateway` and the API server work the way they do on any Linux machine. Use the gateway setup wizard:
...
AMD ROCm and Intel Arc support inside WSL2 is still evolving and outside Zed's test matrix â€” it may work with current drivers but we don't have a recipe to recommend.
...
**Massive slowness on `git status` / `zed chat` in a repo.**
...
Zed's cwd is inside the Linux filesystem, and Windows `cmd.exe` doesn't know what to do with it. Start Zed from `/mnt/c/...` for that session, or use a wrapper that `cd`s to a Windows-reachable path before invoking the Windows executable.
...
Mirrored mode proxies host network settings into WSL â€” if Windows DNS is funky (VPN split-tunnel, corporate resolver), WSL inherits that. Workaround: override `resolv.conf` manually (set `generateResolvConf=false` in `/etc/wsl.conf`, then write your own `/etc/resolv.conf` with `1.1.1.1` or your VPN's DNS).

**`zed` not found after running the installer.**
...
- **[Installation](/getting-started/installation)** â€” actual install steps (Linux/WSL2/Termux all use the same installer).
- **[Integrations â†’ Providers â†’ WSL2 Networking](/integrations/providers#wsl2-networking-windows-users)** â€” the canonical networking deep-dive for local model servers.
- **[MCP guide â†’ WSL â†’ Windows Chrome](/guides/use-mcp-with-zed#wsl2-bridge-zed-in-wsl-to-windows-chrome)** â€” controlling your signed-in Windows Chrome from Zed in WSL.
- **[Tool Gateway](/user-guide/features/tool-gateway)** and **[Web Dashboard](/user-guide/features/web-dashboard)** â€” the long-lived services you'll most often want to expose from WSL to the rest of your network.
