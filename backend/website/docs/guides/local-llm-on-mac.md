...
Both expose an OpenAI-compatible `/v1/chat/completions` endpoint. Zed works with either one â€” just point it at `http://localhost:8080` or `http://localhost:8000`.

:::info Apple Silicon only
This guide targets Macs with Apple Silicon (M1 and later). Intel Macs will work with llama.cpp but without GPU acceleration â€” expect significantly slower performance.
:::
...
For getting started, we recommend **Qwen3.5-9B** â€” it's a strong reasoning model that fits comfortably in 8GB+ of unified memory with quantization.
...
| `-np 1` | Number of parallel slots. Keep at 1 for single-user use â€” more slots split your memory budget. |
...
On an 8 GB Mac, use `q4_0` KV cache and choose a smaller model that can still fit Zed' 64K minimum context. On 16 GB, you can comfortably do 128K context. On 32 GB+, you can run larger models or multiple parallel slots.

If you're still running out of memory, reduce context only while staying at or above Zed' 64K minimum; otherwise switch to a smaller model or smaller quantization (Q3_K_M instead of Q4_K_M).
...
- **llama.cpp** excels at prompt processing â€” its flash attention + quantized KV cache pipeline gets you the first token in ~66ms. If you're building interactive applications where perceived responsiveness matters (chatbots, autocomplete), this is a meaningful advantage.
...
- Both backends are **extremely consistent** â€” variance across runs was negligible. You can rely on these numbers.
...
Select **Custom endpoint** and follow the prompts. It will ask for the base URL and model name â€” use the values from whichever backend you set up above.

---

## Timeouts

Zed automatically detects local endpoints (localhost, LAN IPs) and relaxes its streaming timeouts. No configuration needed for most setups.

If you still hit timeout errors (e.g. very large contexts on slow hardware), you can override the streaming read timeout:

```bash
# In your .env â€” raise from the 120s default to 30 minutes
ZED_STREAM_READ_TIMEOUT=1800
```

| Timeout | Default | Local auto-adjustment | Env var override |
|---------|---------|----------------------|------------------|
| Stream read (socket-level) | 120s | Raised to 1800s | `ZED_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Disabled entirely | `ZED_STREAM_STALE_TIMEOUT` |
| API call (non-streaming) | 1800s | No change needed | `ZED_API_TIMEOUT` |

The stream read timeout is the one most likely to cause issues â€” it's the socket-level deadline for receiving the next chunk of data. During prefill on large contexts, local models may produce no output for minutes while processing the prompt. The auto-detection handles this transparently.
