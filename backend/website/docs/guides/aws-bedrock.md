...
sidebar_position: 14
title: "AWS Bedrock"
description: "Use Zed Agent with Amazon Bedrock â€” native Converse API, IAM authentication, Guardrails, and cross-region inference"
---

# AWS Bedrock

Zed Agent supports Amazon Bedrock as a native provider using the **Converse API** â€” not the OpenAI-compatible endpoint. This gives you full access to the Bedrock ecosystem: IAM authentication, Guardrails, cross-region inference profiles, and all foundation models.

## Prerequisites

- **AWS credentials** â€” any source supported by the [boto3 credential chain](https://boto3.amazonaws.com/v1/documentation/api/latest/guide/credentials.html):
  - IAM instance role (EC2, ECS, Lambda â€” zero config)
  - `AWS_ACCESS_KEY_ID` + `AWS_SECRET_ACCESS_KEY` environment variables
  - `AWS_PROFILE` for SSO or named profiles
  - `aws configure` for local development
- **boto3** â€” install with `pip install zed-agent[bedrock]`
- **IAM permissions** â€” at minimum:
...
:::tip EC2 / ECS / Lambda
On AWS compute, attach an IAM role with `AmazonBedrockFullAccess` and you're done. No API keys, no `.env` configuration â€” Zed detects the instance role automatically.
:::
...
zed model
# â†’ Choose "More providers..." â†’ "AWS Bedrock"
# â†’ Select your region and model
...
Zed auto-discovers available models via the Bedrock control plane. You can customize discovery:
...
Bedrock models use **inference profile IDs** for on-demand invocation. The `zed model` picker shows these automatically, with recommended models at the top:

| Model | ID | Notes |
|-------|-----|-------|
| Claude Sonnet 4.6 | `us.anthropic.claude-sonnet-4-6` | Recommended â€” best balance of speed and capability |
...
Bedrock works with all Zed gateway platforms (Telegram, Discord, Slack, Feishu, etc.). Configure Bedrock as your provider, then start the gateway normally:
...
Zed checks for credentials in this order:
...
You've hit the Bedrock per-model rate limit. Zed automatically retries with backoff. To increase limits, request a quota increase in the [AWS Service Quotas console](https://console.aws.amazon.com/servicequotas/).

## One-Click AWS Deployment

For a fully automated deployment on EC2 with CloudFormation:

**[sample-zed-agent-on-aws-with-bedrock](https://github.com/JiaDe-Wu/sample-zed-agent-on-aws-with-bedrock)** â€” creates VPC, IAM role, EC2 instance, and configures Bedrock automatically. Deploy in any region with one click.
