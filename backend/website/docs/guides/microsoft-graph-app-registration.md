...
The Teams meeting pipeline reads meeting transcripts, recordings, and related artifacts from Microsoft Graph using **app-only** (daemon) authentication â€” no user sign-in, no interactive consent per meeting. That requires an Azure AD application registration with admin-consented application permissions.
...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect â€” they go into `~/.zed/.env` at the end.
...
4. Fill in:
   - **Name:** `Zed Teams Meeting Pipeline` (or any name you'll recognize).
   - **Supported account types:** *Accounts in this organizational directory only (Single tenant)*.
   - **Redirect URI:** leave blank â€” app-only auth does not need one.
5. Click **Register**.
...
3. **Description:** `zed-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
5. Copy the **Value** column immediately â€” it's only shown once. That value is `MSGRAPH_CLIENT_SECRET`.
...
- `OnlineMeetings.ReadWrite.All` / `Chat.ReadWrite` without `.All` â€” broader than the pipeline needs.
- Delegated permissions â€” the pipeline uses app-only (client-credentials) flow; delegated permissions won't work without user sign-in.
...
Without the policy, **any** user's meetings are readable â€” that's what the permission technically grants. Don't skip this step on a production tenant.

## Step 5: Write the Credentials to Your Env File

Put the three values you collected into `~/.zed/.env`:
...
Zed ships a Graph auth smoke-test. From your Zed install:
...
A successful run prints a long token string and a health dict showing `cached: True` and an `expires_in_seconds` value near 3600. Failures produce a `MicrosoftGraphTokenError` with the Azure error code â€” the most common are:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.zed/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `zed gateway restart`.
...
- **Webhook listener setup** â€” stand up the `msgraph_webhook` gateway platform that receives Graph change notifications.
- **Pipeline configuration** â€” configure the Teams meeting pipeline runtime and operator CLI.
- **Outbound delivery** â€” wire summaries back into a Teams channel or chat.
