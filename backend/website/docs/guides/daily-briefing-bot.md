...
By the end, you'll have a fully automated workflow combining **web search**, **cron scheduling**, **delegation**, and **messaging delivery** â€” no code required.

:::tip
This recipe hits web search, summarization, and optional TTS â€” all bundled in a Portal subscription. The fastest setup is `zed setup --portal`. See [Nous Portal](/integrations/nous-portal).
:::
...
1. **8:00 AM** â€” The cron scheduler triggers your job
2. **Zed spins up** a fresh agent session with your prompt
...
- **Gateway running** â€” the gateway daemon handles cron execution:
  ```bash
  ...
  ```
- **Firecrawl API key** â€” set `FIRECRAWL_API_KEY` in your environment for web search
- **Messaging configured** (optional but recommended) â€” [Telegram](/user-guide/messaging/telegram) or Discord set up with a home channel

:::tip No messaging? No problem
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.zed/cron/output/` and you can read them anytime.
:::
...
Zed will search the web, read through results, and produce something like:
...
Before creating cron jobs, ensure Zed has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Zed what you want:
...
Zed will create the cron job for you using the unified `cronjob` tool.
...
Cron jobs run in a **completely fresh session** â€” no memory of your previous conversations, no context about what you "set up earlier." Your prompt must contain **everything** the agent needs to do the job.
:::
...
For faster briefings, tell Zed to delegate each topic to a sub-agent:
...
Don't need briefings on weekends? Use a cron expression that targets Mondayâ€“Friday:
...
If you have [memory](/user-guide/features/memory) enabled, you can store preferences that persist across sessions. But remember â€” cron jobs run in fresh sessions without conversational memory. To add personal context, bake it directly into the prompt:
...
Zed will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
...
- **[Scheduled Tasks (Cron)](/user-guide/features/cron)** â€” Full reference for schedule formats, repeat limits, and delivery options
- **[Delegation](/user-guide/features/delegation)** â€” Deep dive into parallel sub-agent workflows
- **[Messaging Platforms](/user-guide/messaging)** â€” Set up Telegram, Discord, or other delivery targets
- **[Memory](/user-guide/features/memory)** â€” Persistent context across sessions
- **[Tips & Best Practices](/guides/tips)** â€” More prompt engineering advice
