...
description: "Practical advice to get the most out of Zed Agent â€” prompt tips, CLI shortcuts, context files, memory, cost optimization, and security"
---
...
Vague prompts produce vague results. Instead of "fix the code," say "fix the TypeError in `api/handlers.py` on line 47 â€” the `process_request()` function receives `None` from `parse_body()`." The more context you give, the fewer iterations you need.

### Provide Context Up Front

Front-load your request with the relevant details: file paths, error messages, expected behavior. One well-crafted message beats three rounds of clarification. Paste error tracebacks directly â€” the agent can parse them.

### Use Context Files for Recurring Instructions

If you find yourself repeating the same instructions ("use tabs not spaces," "we use pytest," "the API is at `/api/v2`"), put them in an `AGENTS.md` file. The agent reads it automatically every session â€” zero effort after setup.

### Let the Agent Use Its Tools

Don't try to hand-hold every step. Say "find and fix the failing test" rather than "open `tests/test_foo.py`, look at line 42, then..." The agent has file search, terminal access, and code execution â€” let it explore and iterate.
...
The CLI auto-detects multi-line pastes. Just paste a code block or error traceback directly â€” it won't send each line as a separate message. The paste is buffered and sent as one message.
...
Forgot something from your last session? Run `zed -c` to resume exactly where you left off, with full conversation history restored. You can also resume by title: `zed -r "my research project"`.

### Clipboard Image Paste

Press **Ctrl+V** to paste an image from your clipboard directly into the chat. The agent uses vision to analyze screenshots, diagrams, error popups, or UI mockups â€” no need to save to a file first.

### Slash Command Autocomplete

Type `/` and press **Tab** to see all available commands. This includes built-in commands (`/compress`, `/model`, `/title`) and every installed skill. You don't need to memorize anything â€” Tab completion has you covered.

:::tip
Use `/verbose` to cycle through tool output display modes: **off â†’ new â†’ all â†’ verbose**. The "all" mode is great for watching what the agent does; "off" is cleanest for simple Q&A.
:::
...
Want Zed to have a stable default voice? Edit `~/.zed/SOUL.md` (or `$ZED_HOME/SOUL.md` if you use a custom Zed home). Zed now seeds a starter SOUL automatically and uses that global file as the instance-wide personality source.

For a full walkthrough, see [Use SOUL.md with Zed](/guides/use-soul-with-zed).
...
Already have a `.cursorrules` or `.cursor/rules/*.mdc` file? Zed reads those too. No need to duplicate your coding conventions â€” they're loaded automatically from the working directory.

### Discovery

Zed loads the top-level `AGENTS.md` from the current working directory at session start. Subdirectory `AGENTS.md` files are discovered lazily during tool calls (via `subdirectory_hints.py`) and injected into tool results â€” they are not loaded upfront into the system prompt.
...
Memory is intentionally bounded (~2,200 chars for MEMORY.md, ~1,375 chars for USER.md). When it fills up, the agent consolidates entries. You can help by saying "clean up your memory" or "replace the old Python 3.9 note â€” we're on 3.12 now."
...
:::warning
Memory is a frozen snapshot â€” changes made during a session don't appear in the system prompt until the next session starts. The agent writes to disk immediately, but the prompt cache isn't invalidated mid-session.
:::
...
Need to research three topics at once? Ask the agent to use `delegate_task` with parallel subtasks. Each subagent runs independently with its own context, and only the final summaries come back â€” massively reducing your main conversation's token usage.
...
Name your sessions with `/title auth-refactor` or `/title research-llm-quantization`. Named sessions are easy to find with `zed sessions list` and resume with `zed -r "auth-refactor"`. Unnamed sessions pile up and become impossible to distinguish.

### DM Pairing for Team Access

Instead of manually collecting user IDs for allowlists, enable DM pairing. When a teammate DMs the bot, they get a one-time pairing code. You approve it with `zed pairing approve telegram XKGH5N7P` â€” simple and secure.

### Tool Progress Display Modes

Use `/verbose` to control how much tool activity you see. In messaging platforms, less is usually more â€” keep it on "new" to see just new tool calls. In the CLI, "all" gives you a satisfying live view of everything the agent does.

:::tip
On messaging platforms, sessions auto-reset after idle time (default: 24 hours) or daily at 4 AM. Adjust per-platform in `~/.zed/config.yaml` if you need longer sessions.
:::
...
When the agent triggers a dangerous command approval (`rm -rf`, `DROP TABLE`, etc.), you get four options: **once**, **session**, **always**, **deny**. Think carefully before choosing "always" â€” it permanently allowlists that pattern. Start with "session" until you're comfortable.

### Command Approval Is Your Safety Net

Zed checks every command against a curated list of dangerous patterns before execution. This includes recursive deletes, SQL drops, piping curl to shell, and more. Don't disable this in production â€” it exists for good reasons.
...
*Have a tip that should be on this page? Open an issue or PR â€” community contributions are welcome.*
