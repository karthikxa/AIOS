---
sidebar_position: 5
title: "Using Zed as a Python Library"
description: "Embed AIAgent in your own Python scripts, web apps, or automation pipelines â€” no CLI required"
---

# Using Zed as a Python Library

Zed isn't just a CLI tool. You can import `AIAgent` directly and use it programmatically in your own Python scripts, web applications, or automation pipelines. This guide shows you how.
...
:::tip
The same environment variables used by the CLI are required when using Zed as a library. At minimum, set `OPENROUTER_API_KEY` (or `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` if using direct provider access).
:::
...
The simplest way to use Zed is the `chat()` method â€” pass a message, get a string back:
...
`chat()` handles the full conversation loop internally â€” tool calls, retries, everything â€” and returns just the final text response.

:::warning
Always set `quiet_mode=True` when embedding Zed in your own code. Without it, the agent prints CLI spinners, progress indicators, and other terminal output that will clutter your application's output.
:::
...
- **`final_response`** â€” The agent's final text reply
- **`messages`** â€” The complete message history (system, user, assistant, tool calls)
...
For running many prompts in parallel, Zed includes `batch_runner.py`. It manages concurrent `AIAgent` instances with proper resource isolation:
...
|-----------|------|---------|-------------|
| `model` | `str` | `""` | Model in OpenRouter format (defaults to empty; resolved from your zed config at runtime) |
