...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `zed send`."
---

# Pipe Script Output to Messaging Platforms

`zed send` is a small, scriptable CLI that pushes a message to any
messaging platform Zed is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | zed send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `zed
...
Any platform Zed ships adapters for works as a target:
...
`zed send` resolves the message body in this order:

1. **Positional argument** â€” `zed send --to telegram "hi"`
2. **`--file PATH`** â€” `zed send --to telegram --file msg.txt`
3. **Piped stdin** â€” `echo hi | zed send --to telegram`

When stdin is a TTY (no pipe), Zed does **not** wait for input â€” you'll
...
Because `zed send` reuses your Zed config, the same script works on
any host where Zed is installed â€” no need to export bot tokens into
...
`zed send`. If the Python interpreter can't load because the box is
...
## Does `zed send` Need the Gateway Running?

**Usually no.** For any bot-token platform â€” Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others â€” `zed send` calls
the platform's REST endpoint directly using credentials from
`~/.zed/.env` and `~/.zed/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `zed gateway start` and retry.
...
The listing is built from `~/.zed/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`zed gateway
...
| Approach | Multi-platform | Reuses Zed creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `zed send` | âœ… | âœ… | No (bot-token) | Everything below |
...
| `send_message` agent tool | âœ… | âœ… | No | Inside an agent loop |

`zed send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `zed send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) â€”
  the delivery router that `zed send` shares with cron delivery.
