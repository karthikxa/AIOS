---
sidebar_position: 6
title: "Use MCP with Zed"
description: "A practical guide to connecting MCP servers to Zed Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Zed

This guide shows how to actually use MCP with Zed Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Zed tool
- you want Zed to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Zed to internal APIs, databases, or company systems without modifying Zed core

Do not use MCP when:
- a built-in Zed tool already solves the job well
...
- Zed remains the agent
- MCP servers contribute tools
- Zed discovers those tools at startup or reload time
- the model can use them like normal tools
- you control how much of each server is visible

That last part matters. Good MCP usage is not just â€œconnect everything.â€ It is â€œconnect the right thing, with the smallest useful surface.â€

## Step 1: install MCP support

If you installed Zed with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Zed banner/status should show MCP integration when configured
- ask Zed what tools it has available
...
## WSL2: bridge Zed in WSL to Windows Chrome

This is the practical setup when:

- Zed runs inside WSL2
...
In this setup, Zed does **not** connect to Chrome directly. Instead:

- Zed runs in WSL
- Zed starts a local stdio MCP server
...
- Zed stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Zed core browser transport
...
Then start a fresh Zed session or run:
...
Once loaded, Zed can use the MCP-prefixed browser tools directly. For example:
...
If Zed runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Zed from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Zed from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Zed:
...
So Zed will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Zed to reason over a bounded workspace.
...
Now Zed can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Zed core.
...
Because Zed now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
