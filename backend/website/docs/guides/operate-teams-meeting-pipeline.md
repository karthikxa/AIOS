...
#### Option 1: Zed cron (recommended if you already run the Zed gateway)

Zed ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.zed/scripts/`. First create the script:
...
Create `/etc/systemd/system/zed-teams-pipeline-maintain.service`:
...
And `/etc/systemd/system/zed-teams-pipeline-maintain.timer`:
...
Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.zed/.env` at the top of a wrapper script that crontab calls.
...
```bash
zed teams-pipeline subscriptions   # should show expirationDateTime advanced
zed teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
- run `zed teams-pipeline maintain-subscriptions --dry-run`
- inspect `zed teams-pipeline list --status failed`
...
- run `zed teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `zed teams-pipeline run`
...
- [ ] `zed teams-pipeline validate` returns an OK snapshot
- [ ] `zed teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Zed cron, systemd timer, or crontab â€” see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
