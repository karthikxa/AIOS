---
sidebar_position: 8
title: "Use Voice Mode with Zed"
description: "A practical guide to setting up and using Zed voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Zed
...
:::tip
[Nous Portal](/integrations/nous-portal) bundles both the LLM and TTS through one OAuth â€” voice mode works end-to-end with no extra credentials.
:::
...
- you want Zed sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Zed.
...
## Step 1: make sure normal Zed works first
...
Why these matter:
- `portaudio` â†’ microphone input / playback for CLI voice mode
- `ffmpeg` â†’ audio conversion for TTS and messaging delivery
- `opus` â†’ Discord voice codec support
- `espeak-ng` â†’ phonemizer backend for NeuTTS

## Step 4: choose STT and TTS providers

Zed supports both local and cloud speech stacks.
...
- `local` â†’ best default for privacy and zero-cost use
- `groq` â†’ very fast cloud transcription
- `openai` â†’ good paid fallback

#### Text-to-speech

- `edge` â†’ free and good enough for most users
- `neutts` â†’ free local/on-device TTS
- `elevenlabs` â†’ best quality
- `openai` â†’ good middle ground
- `mistral` â†’ multilingual, native Opus

### If you use `zed setup`

If you choose NeuTTS in the setup wizard, Zed checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Zed transcribes and responds
...
- asking Zed to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Zed loop.

## Tuning CLI behavior

### Silence threshold

If Zed starts/stops too aggressively, tune:
...
Zed stays a normal chat bot, but can speak replies.
...
- you want Zed to function like a portable research or ops assistant
...
Zed joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Zed detects speech boundaries
- transcripts are posted in the associated text channel
- Zed responds in text and audio
...
1. get text Zed working
2. install `zed-agent[voice]`
