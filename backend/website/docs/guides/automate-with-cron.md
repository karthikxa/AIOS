...
description: "Real-world automation patterns using Zed cron â€” monitoring, reports, pipelines, and multi-skill workflows"
---

# Automate Anything with Cron

The [daily briefing bot tutorial](/guides/daily-briefing-bot) covers the basics. This guide goes further â€” five real-world automation patterns you can adapt for your own workflows.

For the full feature reference, see [Scheduled Tasks (Cron)](/user-guide/features/cron).

:::info Key Concept
Cron jobs run in fresh agent sessions with no memory of your current chat. Prompts must be **completely self-contained** â€” include everything the agent needs to know.
:::

:::tip Don't need the LLM? You have two zero-token options.
- **Recurring watchdog** where the script already produces the exact message (memory alerts, disk alerts, heartbeats): use [script-only cron jobs](/guides/cron-script-only). Same scheduler, no LLM. You can ask Zed to set one up for you in chat â€” the `cronjob` tool knows when to pick `no_agent=True` and writes the script for you.
- **One-shot from a script that's already running** (CI step, post-commit hook, deploy script, externally-scheduled monitor): use [`zed send`](/guides/pipe-script-output) to pipe stdout or a file straight to Telegram / Discord / Slack / etc. without setting up a cron entry.
:::
...
For cron monitoring jobs, instruct the agent to respond with only `[SILENT]` when nothing changed. Cron delivery treats `[SILENT]` as the quiet marker, so you only get notified when something actually happens â€” no spam on quiet hours.
:::
...
```bash
/cron add "every 6h" "Check the GitHub repository zedteam/zed-agent for:
- New issues opened in the last 6 hours
- New PRs opened or merged in the last 6 hours
- Any new releases

Use the terminal to run gh commands:
  gh issue list --repo zedteam/zed-agent --state open --json number,title,author,createdAt --limit 10
  gh pr list --repo zedteam/zed-agent --state all --json number,title,author,createdAt,mergedAt --limit 10
...
Notice how the prompt includes the exact `gh` commands. The cron agent has no memory of previous runs or your preferences â€” spell everything out.
:::
...
Skills are loaded in order â€” `arxiv` first (teaches the agent how to search papers), then `obsidian` (teaches how to write notes). The prompt ties them together.
...
**Use `[SILENT]` deliberately.** For monitoring jobs, include instructions like "if nothing changed, respond with only `[SILENT]`." Do not ask the agent to explain the token in quiet cases â€” cron treats `[SILENT]` as the delivery-suppression marker.
...
**Schedule expressions.** Supported formats: relative delays (`30m`), intervals (`every 2h`), standard cron expressions (`0 9 * * *`), and ISO timestamps (`2025-06-15T09:00:00`). Natural language like `daily at 9am` is not supported â€” use `0 9 * * *` instead.

---

*For the complete cron reference â€” all parameters, edge cases, and internals â€” see [Scheduled Tasks (Cron)](/user-guide/features/cron).*
