...
title: "Build a Zed Plugin"
description: "Step-by-step guide to building a complete Zed plugin with tools, hooks, data files, and skills"
---

# Build a Zed Plugin

This guide walks through building a complete Zed plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill â€” everything the plugin system supports.

:::info Not sure which guide you need?
Zed has several distinct pluggable interfaces â€” some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| A **TTS backend** (any CLI â€” Piper, VoxCPM, Kokoro, voice cloning, â€¦) | [TTS custom command providers](/user-guide/features/tts#custom-command-providers) â€” config-driven, no Python needed |
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) â€” set `ZED_LOCAL_STT_COMMAND` to a shell template |
| **External tools via MCP** (filesystem, GitHub, Linear, any MCP server) | [MCP](/user-guide/features/mcp) â€” declare `mcp_servers.<name>` in `config.yaml` |
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) â€” drop `HOOK.yaml` + `handler.py` into `~/.zed/hooks/<name>/` |
| **Shell hooks** (run a shell command on events) | [Shell Hooks](/user-guide/features/hooks#shell-hooks) â€” declare under `hooks:` in `config.yaml` |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) â€” `zed skills tap add <repo>` Â· [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
This tells Zed: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.

Optional fields you could add:
```yaml
author: Your Name
requires_env:          # gate loading on env vars; prompted during install
  - SOME_API_KEY       # simple format â€” plugin disabled if missing
  - name: OTHER_KEY    # rich format â€” shows description/url during install
...
4. **Accept `**kwargs`:** Zed may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `zed my-plugin <subcommand>`)
- `ctx.register_command()` registers an in-session slash command (e.g. `/myplugin <args>` inside CLI / gateway chat) â€” see [Register slash commands](#register-slash-commands) below
...
- If this function crashes, the plugin is disabled but Zed continues fine
...
If your plugin doesn't show up â€” or shows up but isn't loading â€” set `ZED_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:
...
The same logs are always written to `~/.zed/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:
...
- **Not enabled in config** â€” plugins are opt-in. Run `zed plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout** â€” must be `~/.zed/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.zed/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
â”œâ”€â”€ plugin.yaml      # "I'm calculator, I provide tools and hooks"
â”œâ”€â”€ __init__.py      # Wiring: schemas â†’ handlers, register hooks
â”œâ”€â”€ schemas.py       # What the LLM reads (descriptions + parameter specs)
â””â”€â”€ tools.py         # What runs (calculate, unit_convert functions)
```
...
```python
skill_view("my-plugin:my-workflow")   # â†’ plugin's version
skill_view("my-workflow")              # â†’ built-in version (unchanged)
```

**Key properties:**
- Plugin skills are **read-only** â€” they don't enter `~/.zed/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.zed/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
```yaml
# plugin.yaml â€” simple format (backwards-compatible)
requires_env:
  - WEATHER_API_KEY
```
...
When users run `zed plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.

For a better install experience, use the rich format with descriptions and signup URLs:

```yaml
# plugin.yaml â€” rich format
requires_env:
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Zed into installing arbitrary packages â€” only specs Zed itself ships are eligible |
...
For third-party plugins distributed via pip, declare the optional deps as `[project.optional-dependencies]` extras in your own `pyproject.toml` and tell users to `pip install your-plugin[backend]` â€” that path doesn't go through `lazy_deps`. The lazy-install dance is most useful for **bundled** plugins where shipping a hard dependency on every install would bloat the base Zed footprint.

When `security.allow_lazy_installs: false` is set globally, `ensure()` raises `FeatureUnavailable` immediately with a remediation hint â€” your plugin should catch it and degrade gracefully (return an error result, not crash the tool loop).



### Thread-safe lazy singletons

Plugins often cache an expensive object â€” an SDK client, an HTTP session, a connection pool â€” in a module-level variable built on first use:
...
def get_client():
    ...
    if _client is not None:
        return _client
    _client = ExpensiveClient(...)   # â† TOCTOU race
    return _client
```

This is a footgun. Zed runs multiple threads in one process (delegated tool calls, background workers, the self-improvement fork), so two threads can hit `get_client()` before `_client` is set, **both** pass the `is not None` check, **both** run the expensive build, and the second write clobbers the first â€” leaking whatever resource the loser opened (connection, file handle, background thread).
...
# Zero-arg accessor â†’ decorate it:
@lazy_singleton
def get_client():
...
# Accessor that takes a build argument â†’ use a slot:
...
shadow an existing tool from a different toolset â€” this prevents
accidental overwrites. The override is logged at INFO level so it's
auditable in `~/.zed/logs/agent.log`. Plugins load after built-in
...
Each hook is documented in full on the **[Event Hooks reference](/user-guide/features/hooks#plugin-hooks)** â€” callback signatures, parameter tables, exactly when each fires, and examples. Here's the summary:
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Zed injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.
...
# Return None or don't return â†’ no injection (observer-only)
return None
```
...
- **The system prompt is Zed's territory** â€” it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `zed <plugin>` subcommand tree:
...
After registration, users can run `zed my-plugin status`, `zed my-plugin config`, etc.

**Memory provider plugins** use a convention-based approach instead: add a `register_cli(subparser)` function to your plugin's `cli.py` file. The memory plugin discovery system finds it automatically â€” no `ctx.register_cli_command()` call needed. See the [Memory Provider Plugin guide](/developer-guide/memory-provider-plugin#adding-cli-commands) for details.
...
|---|---|---|
| Invoked as | `/name` in a session | `zed name` in a terminal |
...
- **Gateway mode:** There is no CLI agent, so tools degrade gracefully â€” workspace is read from the configured terminal working directory and no spinner is shown.
...
Plugins that post Block Kit messages with interactive elements (buttons, overflow menus, datepickers, etc.) can register the click handlers directly with the Slack adapter â€” no monkey-patching of `slack_bolt.AsyncApp` required.

```python
def register(ctx):
    async def _on_approve(ack, body, action):
        # ack within 3 seconds â€” slack_bolt requirement.
        await ack()
...
- Standard slack_bolt rules apply â€” `await ack()` within 3 seconds, then do longer work.
...
Zed has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.zed/plugins/<category>/<name>/` (user). The contract differs by category â€” pick the one you need, then read its full guide.

### Model provider plugins â€” add an LLM backend
...
**Full guide:** [Model Provider Plugins](/developer-guide/model-provider-plugin) â€” field reference, overridable hooks (`prepare_messages`, `build_extra_body`, `build_api_kwargs_extras`, `fetch_models`), api_mode selection, auth types, testing.

### Platform plugins â€” add a gateway channel
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `zed gateway status` without SDK instantiation.
...
**Full guide:** [Adding Platform Adapters](/developer-guide/adding-platform-adapters) â€” complete `BasePlatformAdapter` contract, message routing, auth gating, setup wizard integration. Look at `plugins/platforms/irc/` for a stdlib-only working example.

### Memory provider plugins â€” add a cross-session knowledge backend
...
class MyMemoryProvider(MemoryProvider):
    ...
    def get_tool_schemas(self) -> list[dict]:
        return []   # required @abstractmethod â€” see full guide
...
**Full guide:** [Memory Provider Plugins](/developer-guide/memory-provider-plugin) â€” full `MemoryProvider` ABC, threading contract, profile isolation, CLI command registration via `cli.py`.

### Context engine plugins â€” replace the context compressor
...
**Full guide:** [Image Generation Provider Plugins](/developer-guide/image-gen-provider-plugin) â€” full `ImageGenProvider` ABC, `list_models()` / `get_setup_schema()` metadata, `success_response()`/`error_response()` helpers, base64 vs URL output, user overrides, pip distribution.
...
Zed also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers â€” register external tools

Model Context Protocol (MCP) servers register their own tools into Zed without any Python plugin. Declare them in `~/.zed/config.yaml`:
...
Zed connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks â€” fire on lifecycle events

Drop a manifest + handler into `~/.zed/hooks/<name>/`:
...
### Shell hooks â€” run a shell command on tool calls
...
### Skill sources â€” add a custom skill registry
...
**Full guides:** [Skills Hub](/user-guide/features/skills#skills-hub) Â· [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) (repo layout, minimal example, non-default paths, trust levels).
...
For STT, point `ZED_LOCAL_STT_COMMAND` at a shell template. Supported placeholders: `{input_path}`, `{output_path}`, `{format}`, `{voice}`, `{model}`, `{speed}` (TTS); `{input_path}`, `{output_dir}`, `{language}`, `{model}` (STT). Any path-interacting CLI is automatically a plugin.

**Full guides:** [TTS custom command providers](/user-guide/features/tts#custom-command-providers) Â· [STT](/user-guide/features/tts#voice-message-transcription-stt).
...
# Plugin auto-discovered on next zed startup
```
...
```python
# Wrong â€” returns a dict
def handler(args, **kwargs):
    return {"result": 42}

# Right â€” returns a JSON string
def handler(args, **kwargs):
...
```python
# Wrong â€” will break if Zed passes extra context
def handler(args):
    ...
...
```python
# Wrong â€” exception propagates, tool call fails
def handler(args, **kwargs):
    result = 1 / int(args["value"])  # ZeroDivisionError!
    return json.dumps({"result": result})

# Right â€” catch and return error JSON
def handler(args, **kwargs):
    try:
...
```python
# Bad â€” model doesn't know when to use it
"description": "Does stuff"

# Good â€” model knows exactly when and how
