...
description: "Diagnose and fix common Zed cron issues â€” jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
Look for the job and confirm its state is `[active]` (not `[paused]` or `[completed]`). If it shows `[completed]`, the repeat count may be exhausted â€” edit the job to reset it.
...
If the job fires once and then disappears from the list, it's a one-shot schedule (`30m`, `1d`, or an ISO timestamp) â€” expected behavior.
...
If you're expecting jobs to fire automatically, you need a running gateway (`zed gateway` for foreground, or `zed gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `zed cron tick`.
...
```bash
date
zed cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.zed/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.zed/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.zed/.env` |
...
| `local` | Write access to `~/.zed/cron/output/` |
...
If delivery fails, the job still runs â€” it just won't send anywhere. Check `zed cron list` for updated `last_error` field (if available).

### Check 2: Check `[SILENT]` usage

If your cron job produces no output, delivery is suppressed. If the agent response includes the cron quiet marker `[SILENT]`, delivery is also suppressed. This is intentional for monitoring jobs â€” but make sure your prompt is not accidentally suppressing everything.
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `zed skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `zed skills list`.
...
2. `~/.zed/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `zed cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Zed config directory). Verify:
```bash
ls ~/.zed/scripts/your-script.py   # Must exist
zed cron edit <job_id> --script ~/.zed/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync â€” reinstall them with `zed skills install <skill-name>`.
...
Jobs are stored in `~/.zed/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.zed/cron/jobs.json
chmod 600 ~/.zed/cron/jobs.json   # Your user should own it
```
...
Scripts that dump megabytes of output will slow down the agent and may hit token limits. Filter/summarize at the script level â€” emit only what the agent needs to reason about.
...
```bash
zed cron list                    # Show all jobs, states, next_run times
zed cron run <job_id>            # Schedule for next tick (for testing)
zed cron edit <job_id>           # Fix configuration issues
zed logs                         # View recent Zed logs
zed skills list                  # Verify installed skills
```
...
1. Run the job with `zed cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.zed/logs/agent.log` for scheduler messages and `~/.zed/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/zed-agent](https://github.com/NousResearch/zed-agent) with:
