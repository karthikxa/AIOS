...
description: "When and how to use subagent delegation â€” patterns for parallel research, code review, and multi-file work"
---

# Delegation & Parallel Work

Zed can spawn isolated child agents to work on tasks in parallel. Each subagent gets its own conversation, terminal session, and toolset. Only the final summary comes back â€” intermediate tool calls never enter your context window.
...
**Use something else:**
- Single tool call â†’ just use the tool directly
- Mechanical multi-step work with logic between steps â†’ `execute_code`
- Tasks needing user interaction â†’ subagents can't use `clarify`
- Quick file edits â†’ do them directly
- Durable long-running work that must outlive the current turn â†’ `cronjob` or `terminal(background=True, notify_on_complete=True)`. `delegate_task` is **synchronous**: if the parent turn is interrupted, active children are cancelled and their work is discarded.
...
The key is the `context` field â€” it must include everything the subagent needs:
...
Each subagent researches one option independently. Because they're isolated, there's no cross-contamination â€” each evaluation stands on its own merits. The parent agent gets all three summaries and makes the comparison.
...
:::tip
Each subagent gets its own terminal session. They can work on the same project directory without stepping on each other â€” as long as they're editing different files. If two subagents might touch the same file, handle that file yourself after the parallel work completes.
:::
...
```python
# Step 1: Mechanical gathering (execute_code is better here â€” no reasoning needed)
execute_code("""
...
- **Separate terminals** â€” each subagent gets its own terminal session with separate working directory and state
- **No conversation history** â€” subagents see only the `goal` and `context` the parent agent passes when calling `delegate_task`
- **Default 50 iterations** â€” set `max_iterations` lower for simple tasks to save cost
- **Not durable** â€” `delegate_task` is synchronous and runs inside the parent turn. If the parent is interrupted (new user message, `/stop`, `/new`), all active children are cancelled (`status="interrupted"`) and their work is discarded. For work that must outlive the current turn, use `cronjob` or `terminal(background=True, notify_on_complete=True)`.
...
**Check results.** Subagent summaries are just that â€” summaries. If a subagent says "fixed the bug and tests pass," verify by running the tests yourself or reading the diff.

---

*For the complete delegation reference â€” all parameters, ACP integration, and advanced configuration â€” see [Subagent Delegation](/user-guide/features/delegation).*
