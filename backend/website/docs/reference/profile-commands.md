...
This page covers all commands related to [Zed profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `zed profile` without a subcommand shows help.
...
Sets `<name>` as the active profile. All subsequent `zed` commands (without `-p`) will use this profile.
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `zed profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `zed update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.zed`), use `zed skills opt-out` / `zed skills opt-in`. |
...
This shows the profile's Zed home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Zed installation.
...
:::tip
**New to distributions?** Start with the [Profile Distributions user guide](../user-guide/profile-distributions.md) â€” it covers the why, when, and how with full examples. The sections below are a dry CLI reference for when you know what you want.
:::
...
`auth.json` and `.env` are never part of a distribution â€” they stay on the
...
:::info
`zed profile export` / `import` are still the right commands for
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` â†’ `zed -p telemetry`). |
...
Zed version, author, env var requirements, the source URL/path, and
...
`zed profile list` also shows the distribution name and version in a
`Distribution` column, and `zed profile show <name>` / `delete <name>`
...
configuration â€” the install shells out to your normal `git` binary, so
...
`zed_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Zed version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Zed can clone from.
3. Tell recipients to run `zed profile install <your-repo-url>`.

Use git tags for versioned releases â€” recipients who clone `HEAD` get your
...
Global flag to run any Zed command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
- `zed profile <TAB>` â€” subcommands (list, use, create, etc.)
- `zed profile use <TAB>` â€” profile names
- `zed -p <TAB>` â€” profile names
