...
description: "How to create skills for Zed Agent â€” SKILL.md format, guidelines, and publishing"
---

# Creating Skills

Skills are the preferred way to add new capabilities to Zed Agent. They're easier to create than tools, require no code changes to the agent, and can be shared with the community.
...
metadata:
  zed:
    ...
    blueprint:                              # Optional â€” marks this skill a runnable automation
...
Each entry supports:
- `name` (required) â€” the environment variable name
- `prompt` (optional) â€” prompt text when asking the user for the value
- `help` (optional) â€” help text or URL for obtaining the value
- `required_for` (optional) â€” describes which feature needs this variable
...
Use `required_environment_variables` when a skill needs an API key or token. Missing values do **not** hide the skill from discovery. Instead, Zed prompts for them securely when the skill is loaded in the local CLI.
...
The user can skip setup and keep loading the skill. Zed never exposes the raw secret value to the model. Gateway and messaging sessions show local setup guidance instead of collecting secrets in-band.

:::tip Sandbox Passthrough
When your skill is loaded, any declared `required_environment_variables` that are set are **automatically passed through** to `execute_code` and `terminal` sandboxes â€” including remote backends like Docker and Modal. Your skill's scripts can access `$TENOR_API_KEY` (or `os.environ["TENOR_API_KEY"]` in Python) without the user needing to configure anything extra. See [Environment Variable Passthrough](/user-guide/security#environment-variable-passthrough) for details.
:::
...
Each entry supports:
- `key` (required) â€” dotpath for the setting (e.g., `myplugin.path`)
- `description` (required) â€” explains what the setting controls
- `default` (optional) â€” default value if the user doesn't configure it
- `prompt` (optional) â€” prompt text shown during `zed config migrate`; falls back to `description`
...
2. **Discovery:** `zed config migrate` scans all enabled skills, finds unconfigured settings, and prompts the user. Settings also appear in `zed config show` under "Skill Settings."
...
Use `required_environment_variables` for API keys, tokens, and other **secrets** (stored in `~/.zed/.env`, never shown to the model). Use `config` for **paths, preferences, and non-sensitive settings** (stored in `config.yaml`, visible in config show).
:::

### Credential File Requirements (OAuth tokens, etc.)

Skills that use OAuth or file-based credentials can declare files that need to be mounted into remote sandboxes. This is for credentials stored as **files** (not env vars) â€” typically OAuth token files produced by a setup script.
...
Each entry supports:
- `path` (required) â€” file path relative to `~/.zed/`
- `description` (optional) â€” explains what the file is and how it's created

When loaded, Zed checks if these files exist. Missing files trigger `setup_needed`. Existing files are automatically:
...
:::tip When to use which
Use `required_environment_variables` for simple API keys and tokens (strings stored in `~/.zed/.env`). Use `required_credential_files` for OAuth token files, client secrets, service account JSON, certificates, or any credential that's a file on disk.
:::
...
Prefer stdlib Python, curl, and existing Zed tools (`web_extract`, `terminal`, `read_file`). If a dependency is needed, document installation steps in the skill.
...
| Token | Replaced with |
|---|---|
| `${ZED_SKILL_DIR}` | Absolute path to the skill's directory |
| `${ZED_SESSION_ID}` | The active session id (left in place if there is no session) |
...
The agent sees the substituted absolute path and invokes the `terminal` tool with a ready-to-run command â€” no path math, no extra `skill_view` round-trip. Disable substitution globally with `skills.template_vars: false` in `config.yaml`.
...
This is **off by default** â€” any snippet in a SKILL.md runs on the host without approval, so only enable it for skill sources you trust:
...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** â€” it ships with the repo, is discoverable via `zed skills browse` (labeled "official"), and installs with built-in trust.

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** â€” upload it to a registry and share it via `zed skills install`.

## Blueprints: skills that are also automations

A **blueprint** is an ordinary skill that additionally declares a schedule in its frontmatter. Add a `metadata.zed.blueprint` block and the skill becomes a shareable, runnable automation:
...
Because a blueprint **is** a skill, it flows through the entire skills pipeline unchanged â€” search, inspect, install, security scan, provenance, taps, the centralized index, and `zed skills publish` for sharing. Nothing new to learn.

**Installing a blueprint.** When you install a skill that carries a `blueprint:` block, Zed registers it as a **suggested cron job** rather than scheduling it. Scheduling is **opt-in** â€” installing never silently creates a recurring job. You review and accept it via `/suggestions`:

```bash
zed skills install owner/morning-brief
# â†’ Blueprint: 'morning-brief' is an automation (schedule 0 8 * * *).
#   Added to your suggestions â€” run /suggestions to schedule or dismiss it.
...
Blueprints are one **source** of the unified Suggested Cron Jobs surface â€” the same place curated starter automations and (later) usage-pattern and integration suggestions appear. See [Suggested Cron Jobs](#suggested-cron-jobs) below.

**Sharing an automation you built.** A blueprint loaded by a cron job (`zed cron create --skill <name> ...`) can be exported back to a SKILL.md and published like any other skill, so an automation you tuned for yourself becomes a one-command install for someone else.

The blueprint layer adds no new object type, store, or transport â€” the blueprint is a skill, the schedule is a cron job, and sharing is the existing publish/tap/index path.

## Suggested Cron Jobs

Zed can *propose* automations and let you accept them with one tap, instead of making you assemble cron jobs by hand. Every proposal flows through one surface â€” the `/suggestions` command â€” regardless of where it came from:

| Source | Trigger |
|--------|---------|
| `catalog` | Curated starter automations (`/suggestions catalog`) â€” daily briefing, important-mail monitor, weekly review, workday-start reminder |
...
/suggestions dismiss N   # dismiss it â€” latched, never re-offered
/suggestions catalog     # add the curated starter automations
```

Accepting a suggestion calls the same `cron.jobs.create_job` the `cronjob` tool uses â€” there is no second job engine. Suggestions **never** auto-create jobs; acceptance is always explicit. Dismissed suggestions latch by a stable key so the same proposal is never re-offered. The pending list is capped so it never becomes a nag wall.

The **important-mail monitor** catalog entry is the pollâ†’classifyâ†’surface pattern: it scores inbox items with a cheap classifier model (`auxiliary.monitor` in `config.yaml`) and delivers only the ones above an urgency threshold, staying silent otherwise.
...
Trust levels:
- `builtin` â€” ships with Zed (always trusted)
- `official` â€” from `optional-skills/` in the repo (built-in trust, no third-party warning)
- `trusted` â€” from openai/skills, anthropics/skills, huggingface/skills
- `community` â€” non-dangerous findings can be overridden with `--force`; `dangerous` verdicts remain blocked

Zed can now consume third-party skills from multiple external discovery models:
