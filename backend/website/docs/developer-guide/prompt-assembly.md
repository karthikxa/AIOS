...
description: "How Zed builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Zed deliberately separates:
...
1. **stable** â€” identity (`SOUL.md` or fallback), tool/model guidance, skills prompt, environment hints, platform hints
2. **context** â€” caller-supplied `system_message` plus project context files (`.zed.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`)
3. **volatile** â€” built-in memory snapshot (`MEMORY.md`), user profile snapshot (`USER.md`), external memory-provider block, timestamp/session/model/provider line

The final system prompt is then joined as: `stable` â†’ `context` â†’ `volatile`.
...
The platform hint (Layer 10 above) is the per-surface guidance Zed
injects for Telegram, WhatsApp, Slack, CLI, and other platforms â€” for
...
- `append` â€” keep the built-in hint and add the extra text after it.
- `replace` â€” substitute the built-in hint entirely.
- A bare string â€” shorthand for `append`.
...
alongside the built-in hint and does not break prompt caching â€” it is
not a live mid-session mutation of a frozen prompt.

## How SOUL.md appears in the prompt

`SOUL.md` lives at `~/.zed/SOUL.md` and serves as the agent's identity â€” the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
`build_context_files_prompt()` uses a **priority system** â€” only one project context type is loaded (first match wins):
...
|----------|-------|-------------|-------|
| 1 | `.zed.md`, `ZED.md` | CWD up to git root | Zed-native project config |
| 2 | `AGENTS.md` | CWD only | Common agent instruction file |
| 3 | `CLAUDE.md` | CWD only | Claude Code compatibility |
| 4 | `.cursorrules`, `.cursor/rules/*.mdc` | CWD only | Cursor compatibility |

All context files are:
- **Security scanned** â€” checked for prompt injection patterns (invisible unicode, "ignore previous instructions", credential exfiltration attempts)
- **Truncated** â€” capped at `context_file_max_chars` characters (default 20,000) using 70/20 head/tail ratio with a truncation marker
- **YAML frontmatter stripped** â€” `.zed.md` frontmatter is removed (reserved for future config overrides)
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, Zed concatenates those context blocks (see [Hooks â†’ `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
`agent/prompt_builder.py` scans and sanitizes project context files using a **priority system** â€” only one type is loaded (first match wins):

1. `.zed.md` / `ZED.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Zed already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.zed/SOUL.md` â€” replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.zed/MEMORY.md` and `~/.zed/USER.md` â€” provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.zed.md`, `ZED.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` â€” inject repo-specific working rules.
- Skills â€” package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides â€” add deployment-specific instruction text without forking Zed.
- Ephemeral overlays such as `ZED_EPHEMERAL_SYSTEM_PROMPT` or prefill messages â€” add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how Zed assembles prompts for everyone, change Python and treat it as a code contribution
