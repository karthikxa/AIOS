...
Plugin engines are **never auto-activated** â€” the user must explicitly set `context.engine` to the plugin's name.
...
â”œâ”€â”€ __init__.py      # exports the ContextEngine subclass
â”œâ”€â”€ plugin.yaml      # metadata (name, description, version)
â””â”€â”€ ...              # any other modules your engine needs
```
...
Engine tools are injected into the agent's tool list at startup and dispatched automatically â€” no registry registration needed.
...
2. on_session_start() â€” conversation begins
3. update_from_response() â€” after each API call
4. should_compress() â€” checked each turn
5. compress() â€” called when should_compress() returns True
6. on_session_end() â€” session boundary (CLI exit, /reset, gateway expiry)
```
...
Users select your engine via `zed plugins` â†’ Provider Plugins â†’ Context Engine, or by editing `config.yaml`:
...
- [Context Compression and Caching](/developer-guide/context-compression-and-caching) â€” how the built-in compressor works
- [Memory Provider Plugins](/developer-guide/memory-provider-plugin) â€” analogous single-select plugin system for memory
- [Plugins](/user-guide/features/plugins) â€” general plugin system overview
