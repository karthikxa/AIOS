...
description: "How to add a new inference provider to Zed Agent â€” auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Zed can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `zed model` menu entries
...
1. `zed_cli/auth.py` decides how credentials are found.
2. `zed_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `zed_cli/models.py` and `zed_cli/main.py` make the provider show up in the CLI. (`zed_cli/setup.py` delegates to `main.py` automatically â€” no changes needed there.)
...
### Path A â€” OpenAI-compatible provider
...
:::tip
`zed_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` â€” any provider added there is automatically available in `zed setup`.
:::
...
1. A plugin directory under `plugins/model-providers/<your-provider>/` containing:
   - `__init__.py` â€” calls `register_provider(profile)` at module-level
   - `plugin.yaml` â€” manifest (name, kind: model-provider, version, description)
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` â€” bundled plugins (this repo) and user plugins at `$ZED_HOME/plugins/model-providers/` both get picked up.
...
7. `zed model` menu includes the provider
8. `zed setup` wizard delegates to `main.py` automatically
...
User plugins at `$ZED_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) â€” so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- OAuth or token refresh (Zed Portal, Codex, Google Gemini, Qwen Portal, Copilot)
...
- Provider-specific `zed model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `zed_cli/auth.py`
- `_PROVIDER_LABELS` in `zed_cli/models.py`
- `_PROVIDER_ALIASES` in both `zed_cli/auth.py` and `zed_cli/models.py`
- CLI `--provider` choices in `zed_cli/main.py`
...
## Step 2: Add auth metadata in `zed_cli/auth.py`
...
- What env vars should Zed check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `zed_cli/models.py`
...
## Step 4: Resolve runtime data in `zed_cli/runtime_provider.py`
...
Be careful with API-key precedence. Zed already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `zed_cli/main.py`

A provider is not discoverable until it shows up in the interactive `zed model` flow.

Update these in `zed_cli/main.py`:
...
:::tip
`zed_cli/setup.py` does not need changes â€” it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `zed model` and `zed setup` automatically.
:::
...
When you add a native provider, double-check that Zed is only sending fields that provider actually understands.
...
- `tests/zed_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/zed_cli/test_model_switch_custom_providers.py` (and adjacent `tests/zed_cli/test_model_switch_*.py`)
- `tests/zed_cli/test_setup_model_provider.py`
...
python -m pytest tests/zed_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/zed_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -n0 -q
```
...
- [ ] `ProviderConfig` added in `zed_cli/auth.py`
- [ ] aliases added in `zed_cli/auth.py` and `zed_cli/models.py`
- [ ] model catalog added in `zed_cli/models.py`
- [ ] runtime branch added in `zed_cli/runtime_provider.py`
- [ ] CLI wiring added in `zed_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `zed model` but not `zed setup`
