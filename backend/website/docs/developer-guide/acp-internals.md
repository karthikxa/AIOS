...
The ACP adapter wraps Zed' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
The Zed ACP Registry path launches the same adapter through `uvx --from 'zed-agent[acp]==<version>' zed-acp`, pointed at the `zed-agent` PyPI release.
...
- reject options -> Zed `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Zed tools to ACP tool kinds and builds editor-facing content.
...
Instead it reuses Zed' runtime resolver:

- `acp_adapter/auth.py`
- `zed_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Zed provider/credentials. It also always advertises a terminal setup auth method (`zed-setup`, args `--setup`) so first-run registry clients can open Zed' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.zed/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` â€” `zed-acp` toolset definition
- `zed_cli/main.py` â€” `zed acp` CLI subcommand
- `pyproject.toml` â€” `[acp]` optional dependency + `zed-acp` script
