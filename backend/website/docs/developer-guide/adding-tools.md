...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Zed Agent â€” schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Zed tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Zed core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a Zed Plugin](/guides/build-a-zed-plugin)
...
1. **`tools/your_tool.py`** â€” handler, schema, check function, `registry.register()` call
2. **`toolsets.py`** â€” add tool name to `_ZED_CORE_TOOLS` (or a specific toolset)

Any `tools/*.py` file with a top-level `registry.register()` call is auto-discovered at startup â€” no manual import list required.
...
- The `check_fn` is called when building tool definitions â€” if it returns `False`, the tool is silently excluded
...
Tool modules with a top-level `registry.register()` call are auto-discovered by `discover_builtin_tools()` in `tools/registry.py`. No manual import list to maintain â€” just create your file in `tools/` and it's picked up at startup.
...
The registry handles async bridging transparently â€” you never call `asyncio.run()` yourself.
...
If your tool requires an API key, add it to `zed_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `zed_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `zed chat -q "Use the weather tool for London"`
