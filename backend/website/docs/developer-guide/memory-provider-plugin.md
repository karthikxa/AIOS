...
description: "How to build a memory provider plugin for Zed Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Zed Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `zed plugins`.
:::
...
â”œâ”€â”€ __init__.py      # MemoryProvider implementation + register() entry point
â”œâ”€â”€ plugin.yaml      # Metadata (name, description, hooks)
â””â”€â”€ README.md        # Setup instructions, config reference, tools
```
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          zed_home (str): Active ZED_HOME path. Use for storage.
        """
...
| `is_available()` | Agent init, before activation | **Yes** â€” no network calls |
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `zed memory setup` | **Yes** |
| `save_config(values, zed_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
`get_config_schema()` returns a list of field descriptors used by `zed memory setup`:
...
Every field in `get_config_schema()` is prompted during `zed memory setup`. Providers with many options should keep the schema minimal â€” only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$ZED_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example â€” it only prompts for the API key; all other options live in `supermemory.json`.
:::
...
the `messages` parameter; Zed will continue calling them with the legacy
signature.
...
All storage paths **must** use the `zed_home` kwarg from `initialize()`, not hardcoded `~/.zed`:

```python
# CORRECT â€” profile-scoped
from zed_constants import get_zed_home
data_dir = get_zed_home() / "my-provider"

# WRONG â€” shared across all profiles
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `zed my-provider status`, `zed my-provider config`). This uses a convention-based discovery system â€” no changes to core files needed.
...
4. Your commands appear under `zed <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `zed --help`.
...
def register_cli(subparser) -> None:
    """Build the zed my-provider argparse tree.
...
```
plugins/memory/my-provider/
â”œâ”€â”€ __init__.py      # MemoryProvider implementation + register()
â”œâ”€â”€ plugin.yaml      # Metadata
â”œâ”€â”€ cli.py           # register_cli(subparser) â€” CLI commands
