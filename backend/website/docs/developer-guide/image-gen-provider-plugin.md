...
description: "How to build an image-generation backend plugin for Zed Agent"
---
...
:::tip
Image-gen is one of several **backend plugins** Zed supports. The others (with more specialized ABCs) are [Memory Provider Plugins](/developer-guide/memory-provider-plugin), [Context Engine Plugins](/developer-guide/context-engine-plugin), and [Model Provider Plugins](/developer-guide/model-provider-plugin). General tool/hook/CLI plugins live in [Build a Zed Plugin](/guides/build-a-zed-plugin).
:::

## How discovery works

Zed scans for image-gen backends in three places:

1. **Bundled** â€” `<repo>/plugins/image_gen/<name>/` (auto-loaded with `kind: backend`, always available)
2. **User** â€” `~/.zed/plugins/image_gen/<name>/` (opt-in via `plugins.enabled`)
3. **Pip** â€” packages declaring a `zed_agent.plugins` entry point

Each plugin's `register(ctx)` function calls `ctx.register_image_gen_provider(...)` â€” that puts it into the registry in `agent/image_gen_registry.py`. The active provider is picked by `image_gen.provider` in `config.yaml`; `zed tools` walks users through selection.

The `image_generate` tool wrapper asks the registry for the active provider and dispatches there. If no provider is registered, the tool surfaces a helpful error pointing at `zed tools`.

## Directory structure

```
plugins/image_gen/my-backend/
â”œâ”€â”€ __init__.py      # ImageGenProvider subclass + register()
â””â”€â”€ plugin.yaml      # Manifest with kind: backend
```

A bundled plugin is complete at this point. User plugins at `~/.zed/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `zed plugins enable <name>`).
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `zed plugins install`.
...
|---|---|---|---|
| `name` | âœ… | â€” | Stable id used in `image_gen.provider` config |
| `display_name` | â€” | `name.title()` | Label shown in `zed tools` |
| `is_available()` | â€” | `True` | Gate for missing creds/deps |
| `list_models()` | â€” | `[]` | Catalog for `zed tools` model picker |
...
| `generate(prompt, aspect_ratio, **kwargs)` | âœ… | â€” | The call |
...
Some backends return image URLs (fal, Replicate); others return base64 payloads (OpenAI gpt-image-2). For the base64 case, use `save_b64_image()` â€” it writes to `$ZED_HOME/cache/images/<prefix>_<timestamp>_<uuid>.<ext>` and returns the absolute `Path`. Pass that path (as `str`) as `image=` in `success_response()`. Gateway delivery (Telegram photo bubble, Discord attachment) recognizes both URLs and absolute paths.

## User overrides

Drop a user plugin at `~/.zed/plugins/image_gen/<name>/` with the same `name` property as a bundled one and enable it via `zed plugins enable <name>` â€” the registry is last-writer-wins, so your version replaces the built-in. Useful for pointing an `openai` plugin at a private proxy, or swapping in a custom model catalog.
...
Or interactively: `zed tools` â†’ "Image Generation" â†’ select `my-backend` â†’ enter API key if prompted.
...
`my_backend_imggen_package` must expose a top-level `register` function. See [Distribute via pip](/guides/build-a-zed-plugin#distribute-via-pip) in the general plugin guide for the full setup.

## Related pages

- [Image Generation](/user-guide/features/image-generation) â€” user-facing feature documentation
