...
description: "How to build a model provider (inference backend) plugin for Zed Agent"
---

# Building a Model Provider Plugin

Model provider plugins declare an inference backend â€” an OpenAI-compatible endpoint, an Anthropic Messages server, a Codex-style Responses API, or a Bedrock-native surface â€” that Zed can route `AIAgent` calls through. Every built-in provider (OpenRouter, Anthropic, GMI, DeepSeek, Nvidia, â€¦) ships as one of these plugins. Third parties can add their own by dropping a directory under `$ZED_HOME/plugins/model-providers/` with zero changes to the repo.
...
1. **Bundled plugins** â€” `<repo>/plugins/model-providers/<name>/` â€” ship with Zed
2. **User plugins** â€” `$ZED_HOME/plugins/model-providers/<name>/` â€” drop in any directory; no restart required for subsequent sessions
3. **Legacy single-file** â€” `<repo>/providers/<name>.py` â€” back-compat for out-of-tree editable installs

**User plugins override bundled plugins of the same name** because `register_provider()` is last-writer-wins. Drop a `$ZED_HOME/plugins/model-providers/gmi/` directory to replace the built-in GMI profile without touching the repo.

## Directory structure

```
plugins/model-providers/my-provider/
â”œâ”€â”€ __init__.py       # Calls register_provider(profile) at module-level
â”œâ”€â”€ plugin.yaml       # kind: model-provider + metadata (optional but recommended)
â””â”€â”€ README.md         # Setup instructions (optional)
```

The only required file is `__init__.py`. `plugin.yaml` is used by `zed plugins` for introspection and by the general PluginManager to route the plugin to the right loader; without it, the general loader falls back to a source-text heuristic.

## Minimal example â€” a simple API-key provider
...
|---|---|---|
| Credential resolution | `zed_cli/auth.py` | `PROVIDER_REGISTRY["acme-inference"]` populated from profile |
| `--provider` CLI flag | `zed_cli/main.py` | Accepts `acme-inference` |
| `zed model` picker | `zed_cli/models.py` | Appears in `CANONICAL_PROVIDERS`, model list fetched from `{base_url}/models` |
| `zed doctor` | `zed_cli/doctor.py` | Health check for `ACME_API_KEY` + `{base_url}/models` probe |
| `zed setup` | `zed_cli/config.py` | `ACME_API_KEY` appears in `OPTIONAL_ENV_VARS` and the setup wizard |
| URL reverse-mapping | `agent/model_metadata.py` | Hostname â†’ provider name for auto-detection |
| Auxiliary model | `agent/auxiliary_client.py` | Uses `default_aux_model` for compression / summarization |
| Runtime resolution | `zed_cli/runtime_provider.py` | Returns correct `base_url`, `api_key`, `api_mode` |
...
| Field | Type | Purpose |
|---|---|---|
| `name` | str | Canonical id â€” matches `model.provider` in `config.yaml` and the `--provider` flag |
| `aliases` | `tuple[str, ...]` | Alternative names resolved by `get_provider_profile()` (e.g. `grok` â†’ `xai`) |
| `api_mode` | str | `chat_completions` \| `codex_responses` \| `anthropic_messages` \| `bedrock_converse` |
| `display_name` | str | Human label shown in `zed model` picker |
...
Say you want to point `gmi` at your private staging endpoint for testing. Create `~/.zed/plugins/model-providers/gmi/__init__.py`:
...
Four values are recognized. Zed picks one based on:
...
Set `profile.api_mode` to match the default your provider ships â€” it acts as a hint. User URL overrides still win.
...
| `oauth_external` | User signs in elsewhere, tokens land in `auth.json` | Anthropic OAuth, MiniMax OAuth, Gemini Cloud Code, Qwen Portal, Zed Portal |
...
`auth_type` gates which codepaths treat your provider as a "simple api-key provider" â€” if it's not `api_key`, the PluginManager still records the manifest but Zed' CLI-level automation (doctor checks, `--provider` flag, setup wizard delegation) may skip over it.

## Discovery timing

Provider discovery is **lazy** â€” triggered by the first `get_provider_profile()` or `list_providers()` call in the process. In practice this happens early at startup (`auth.py` module load extends `PROVIDER_REGISTRY` eagerly). If you need to verify your plugin loaded, run:

```bash
zed doctor
```

â€” a successful `auth_type="api_key"` profile appears under the Provider Connectivity section with a `/models` probe.
...
Point `ZED_HOME` at a temp directory so you don't pollute your real config:
...
The general `PluginManager` (the thing `zed plugins` operates on) **sees** model-provider plugins but does not import them â€” `providers/__init__.py` owns their lifecycle. The manager records the manifest for introspection and categorizes by `kind: model-provider`. When you drop an unlabeled user plugin into `$ZED_HOME/plugins/` that happens to call `register_provider` with a `ProviderProfile`, the manager auto-coerces it to `kind: model-provider` via a source-text heuristic â€” so the plugin still routes correctly even without `plugin.yaml`.

## Distribute via pip

Like any Zed plugin, model providers can ship as a pip package. Add an entry point to your `pyproject.toml`:
...
â€¦where `acme_zed_plugin:register` is a function that calls `register_provider(profile)`. The general PluginManager picks up entry-point plugins during `discover_and_load()`. For `kind: model-provider` pip plugins, you still need to declare the kind in your manifest (or rely on the source-text heuristic).

See [Building a Zed Plugin](/guides/build-a-zed-plugin#distribute-via-pip) for the full entry-points setup.

## Related pages

- [Provider Runtime](/developer-guide/provider-runtime) â€” resolution precedence + where each layer reads the profile
- [Adding Providers](/developer-guide/adding-providers) â€” end-to-end checklist for new inference backends (covers both the fast plugin path and the full CLI/auth integration)
...
- [Building a Zed Plugin](/guides/build-a-zed-plugin) â€” general plugin authoring
