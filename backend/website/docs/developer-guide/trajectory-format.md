...
Zed Agent saves conversation trajectories in ShareGPT-compatible JSONL format
...
3. **Empty think blocks**: Every `gpt` turn is guaranteed to have a `<think>`
   ...
   `<think>\n</think>\n` â€” this ensures consistent format for training data.
...
- If JSON parsing fails (shouldn't happen â€” validated during conversation),
...
It follows the Zed function-calling prompt template with:
...
Trajectories are standard JSONL â€” load with any JSON-lines reader:
