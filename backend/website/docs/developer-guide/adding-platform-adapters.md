...
This guide covers adding a new messaging platform to the Zed gateway. A platform adapter connects Zed to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.

:::tip
There are two ways to add a platform:
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.zed/plugins/` â€” zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
```
User â†” Messaging Platform â†” Platform Adapter â†” Gateway Runner â†” AIAgent
```

Every adapter extends `BasePlatformAdapter` from `gateway/platforms/base.py` and implements:

- **`connect()`** â€” Establish connection (WebSocket, long-poll, HTTP server, etc.) *(abstract)*
- **`disconnect()`** â€” Clean shutdown *(abstract)*
- **`send()`** â€” Send a text message to a chat *(abstract)*
- **`send_typing()`** â€” Show typing indicator (optional override)
- **`get_chat_info()`** â€” Return chat metadata (optional override)
...
The plugin system lets you add a platform adapter without modifying any core Zed code. Your plugin is a directory with two files:
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `zed config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-zed-config) below).
...
def register(ctx):
    ...
    ctx.register_platform(
        ...
        # Env-driven auto-configuration â€” seeds PlatformConfig.extra from
...
When you call `ctx.register_platform()`, the following integration points are handled for you â€” no core code changes needed:
...
| `zed config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `zed status` | Shows plugin platforms with `(plugin)` tag |
| `zed gateway setup` | Plugin platforms appear in setup menu |
| `zed tools` / `zed skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.zed/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `zed gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
def _env_enablement() -> dict | None:
    ...
    Return None when the platform isn't minimally configured â€” the
...
def _apply_yaml_config(yaml_cfg: dict, platform_cfg: dict) -> dict | None:
    ...
    yaml_cfg     â€” the full top-level parsed config.yaml dict
    platform_cfg â€” the platform's own sub-dict (yaml_cfg.get("my_platform", {}))
...
Exceptions raised by the hook are swallowed and logged at debug level â€” a misbehaving plugin never aborts gateway config load.
...
The scheduler reads this env var when resolving the home target for `deliver=my_platform` jobs, and also treats the platform as a valid cron target in `_KNOWN_DELIVERY_PLATFORMS`-style checks. If your `env_enablement_fn` seeds a `home_channel` dict (see above), that takes precedence â€” `cron_deliver_env_var` is the fallback for cron jobs that run before env seeding.

### Out-of-process cron delivery

`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `zed cron run` separate from `zed gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `zed config`

`zed_cli/config.py` scans `plugins/platforms/*/plugin.yaml` at import time and auto-populates `OPTIONAL_ENV_VARS` from `requires_env` and (optional) `optional_env` blocks. Use the rich-dict form to contribute proper descriptions, prompts, password flags, and URLs â€” the CLI setup UI picks them up for free.
...
Bare-string entries (`- MY_PLATFORM_TOKEN`) still work â€” they get a generic description auto-derived from the plugin's `label`. If a hardcoded entry for the same var already exists in `OPTIONAL_ENV_VARS`, it wins (back-compat); the plugin.yaml form acts as the fallback.
...
- **SMS** has no concept of typing indicators or progressive updates â€” long responses just look like the bot is offline.
    ...
    async def _keep_typing(self, chat_id: str, *args, **kwargs) -> None:
        ...
        async def _fire_at_threshold() -> None:
            try:
                ...
                raise
            # Platform-specific work here â€” for LINE, send a Template
...
- **Always `await super()._keep_typing(...)`.** The typing heartbeat is independently useful â€” don't replace it, layer on top of it.
...
1. **Pending postback active for this chat** â†’ cache the response under the request_id, don't send anything visible.
2. **System busy-ack** (`âš¡ Interrupting`, `â³ Queued`, `â© Steered`) â†’ bypass the cache and send visibly so the user sees the gateway's response to their input.
3. **Normal response** â†’ send via reply-token-or-push as usual.
...
`_SYSTEM_BYPASS_PREFIXES` are the gateway's own busy-acknowledgment prefixes (`âš¡`, `â³`, `â©`, `ðŸ’¾`). Always let those through visibly, regardless of cached UX state.
...
- The user community prefers "loadingâ€¦ loadingâ€¦ DONE" silence-then-response over an interactive intermediate bubble.
...
:::note
This checklist is for adding a platform directly to the Zed core codebase â€” typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
...
1. **`get_connected_platforms()`** â€” Add a check for your platform's required credentials
2. **`load_gateway_config()`** â€” Add token env map entry: `Platform.NEWPLAT: "NEWPLAT_TOKEN"`
3. **`_apply_env_overrides()`** â€” Map all `NEWPLAT_*` env vars to config

### 4. Gateway Runner (`gateway/run.py`)

Five touchpoints:

1. **`_create_adapter()`** â€” Add an `elif platform == Platform.NEWPLAT:` branch
2. **`_is_user_authorized()` allowed_users map** â€” `Platform.NEWPLAT: "NEWPLAT_ALLOWED_USERS"`
3. **`_is_user_authorized()` allow_all map** â€” `Platform.NEWPLAT: "NEWPLAT_ALLOW_ALL_USERS"`
4. **Early env check `_any_allowlist` tuple** â€” Add `"NEWPLAT_ALLOWED_USERS"`
5. **Early env check `_allow_all` tuple** â€” Add `"NEWPLAT_ALLOW_ALL_USERS"`
6. **`_UPDATE_ALLOWED_PLATFORMS` frozenset** â€” Add `Platform.NEWPLAT`

### 5. Cross-Platform Delivery

1. **`gateway/platforms/webhook.py`** â€” Add `"newplat"` to the delivery type tuple
2. **`cron/scheduler.py`** â€” Add to `_KNOWN_DELIVERY_PLATFORMS` frozenset and `_deliver_result()` platform map

### 6. CLI Integration

1. **`zed_cli/config.py`** â€” Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`zed_cli/gateway.py`** â€” Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`zed_cli/platforms.py`** â€” Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`zed_cli/setup.py`** â€” Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`zed_cli/status.py`** â€” Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`zed_cli/dump.py`** â€” Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict

### 7. Tools

1. **`tools/send_message_tool.py`** â€” Add `"newplat": Platform.NEWPLAT` to platform map
2. **`tools/cronjob_tools.py`** â€” Add `newplat` to the delivery target description string

### 8. Toolsets

1. **`toolsets.py`** â€” Add `"zed-newplat"` toolset definition with `_ZED_CORE_TOOLS`
2. **`toolsets.py`** â€” Add `"zed-newplat"` to the `"zed-gateway"` includes list

### 9. Optional: Platform Hints

**`agent/prompt_builder.py`** â€” If your platform has specific rendering limitations (no markdown, message length limits, etc.), add an entry to the `_PLATFORM_HINTS` dict. This injects platform-specific guidance into the system prompt:
...
Not all platforms need hints â€” only add one if the agent's behavior should differ.
...
Repeat for `.md` and `.ts` files. Investigate each gap â€” is it a platform enumeration (needs updating) or a platform-specific reference (skip)?
...
For platforms with tight response deadlines (e.g., WeCom's 5-second limit), always acknowledge immediately and deliver the agent's reply proactively via API later. Agent sessions run 3â€“30 minutes â€” inline replies within a callback response window are not feasible.
