...
description: "Run any LLM call from inside a plugin via ctx.llm â€” chat or structured, sync or async. Host-owned auth, fail-closed trust gate, optional JSON Schema validation."
---
...
images â€” same surface, same trust gate, same host-owned credentials.
...
model the user is currently using â€” when they switch providers, the
...
`purpose` is a free-form audit string â€” it shows up in `agent.log`
...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  credential pool, per-task aux overrides â€” every credential
  concept Zed already has applies. The plugin never sees a
...
Two complete plugins below â€” one chat, one structured. Both ship
...
### Structured extraction â€” `/paste-to-tasks`
...
Everything else â€” provider selection, model resolution, auth, fallback,
timeout, vision routing â€” is the same across all four.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated â€” Zed provider id (e.g. "openrouter")
    model=None,            # optional, gated â€” whatever string that provider expects
    temperature=None,
    max_tokens=None,
    timeout=None,          # seconds
    agent_id=None,         # optional, gated
    profile=None,          # optional, gated â€” explicit auth-profile name
    purpose="optional-audit-string",
)
# â†’ PluginLlmCompleteResult(text, provider, model, agent_id, usage, audit)
```

Plain chat completion. `messages` is the standard OpenAI shape â€” a
...
result = ctx.llm.complete_structured(
    ...
    json_schema={...},     # optional â€” triggers parsed result + validation
...
* `result.content_type == "json"` â€” `result.parsed` is a Python
  object that matches your schema.
* `result.content_type == "text"` â€” parsing or validation failed;
...
â€¦and that's it. `provider=`, `model=`, `agent_id=`, and `profile=`
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Zed provider
        # (must be one Zed already knows about â€” same names as
        # `zed model` and config.yaml model.provider).
        ...
        # plugin sends â€” Zed does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
| â†³ allowlist     | â€”       | `allowed_providers: [...]`       |
| `model=`        | denied  | `allow_model_override: true`     |
| â†³ allowlist     | â€”       | `allowed_models: [...]`          |
...
does **not** also grant `allow_provider_override` â€” a plugin trusted
...
* Request-shaping arguments â€” `temperature`, `max_tokens`,
  `timeout`, `system_prompt`, `purpose`, `messages`, `instructions`,
  `input`, `json_schema`, `schema_name`, `json_mode` â€” are always
  allowed; they don't pick credentials or routes.
* The default deny posture means an unconfigured plugin can still do
  useful work â€” it just runs against the active provider and model.
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.zed/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Zed' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Zed subsystem:
...
own model call â€” for any reason, structured or not â€” `ctx.llm`.
