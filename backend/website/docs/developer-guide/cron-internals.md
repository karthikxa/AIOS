...
description: "How Zed stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---

# Cron Internals

The cron subsystem provides scheduled task execution â€” from simple one-shot delays to recurring cron-expression jobs with skill injection and cross-platform delivery.
...
| `cron/scheduler.py` | Scheduler loop â€” due-job detection, execution, repeat tracking |
| `tools/cronjob_tools.py` | Model-facing `cronjob` tool registration and handler |
| `gateway/run.py` | Gateway integration â€” cron ticking in the long-running loop |
| `zed_cli/cron.py` | CLI `zed cron` subcommands |
...
Jobs are stored in `~/.zed/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
| `paused` | Suspended â€” won't fire until resumed |
| `completed` | Repeat count exhausted or one-shot that has fired |
| `running` | Currently executing (transient state) |

### Backward Compatibility

Older jobs may have a single `skill` field instead of the `skills` array. The scheduler normalizes this at load time â€” single `skill` is promoted to `skills: [skill]`.
...
tick()
  ...
  4. For each due job:
     ...
     g. If repeat count exhausted â†’ state = "completed"
     h. Otherwise â†’ state = "scheduled"
...
fires â€” "Axis B") is selected through a pluggable `CronScheduler` provider. The
...
- **empty (default)** â†’ the built-in `InProcessCronScheduler`, which runs the
...
- **a named provider** (e.g. `chronos`, a managed-cron provider for
  scale-to-zero deployments) â†’ discovered from `plugins/cron/<name>/` or
  `$ZED_HOME/plugins/<name>/`.

If a named provider is missing, fails to load, or reports `is_available() ==
False`, the resolver falls back to the built-in with a warning â€” **cron is
...
providers â€” it stays in `scheduler.run_job()` / `scheduler._deliver_result()`.
A provider only controls the trigger, never execution.

In CLI mode, cron jobs only fire when `zed cron` commands are run or during active CLI sessions.
...
then re-arms the next one-shot. Between fires the process can be fully stopped â€”
...
create/update a cron job
  â†’ Chronos asks Nous to arm a one-shot at the job's next_run_at
      (authenticated with the agent's existing Nous token)
  â†’ at fire time Nous calls the gateway: POST {callback_url}/api/cron/fire
      (authenticated with a short-lived, purpose-scoped Nous-minted JWT)
  â†’ the gateway verifies the token, claims the job (store compare-and-set so
...
`resolve_cron_scheduler()` falls back to the built-in ticker (logged warning) â€”
...
agentâ†”Nous wire contract lives in `docs/chronos-managed-cron-contract.md`.
...
- The prompt must be self-contained â€” cron jobs cannot ask clarifying questions
...
```
Create a daily funding report â†’ attach "ai-funding-daily-report" skill
```
...
# Print summary to stdout â€” agent analyzes and reports
```

The script timeout defaults to 120 seconds. `_get_script_timeout()` resolves the limit through a three-layer chain:

1. **Module-level override** â€” `_SCRIPT_TIMEOUT` (for tests/monkeypatching). Only used when it differs from the default.
2. **Environment variable** â€” `ZED_CRON_SCRIPT_TIMEOUT`
3. **Config** â€” `cron.script_timeout_seconds` in `config.yaml` (read via `load_config()`)
4. **Default** â€” 120 seconds

### Provider Recovery

`run_job()` passes the user's configured fallback providers and credential pool into the `AIAgent` instance:

- **Fallback providers** â€” reads `fallback_providers` (list) or `fallback_model` (legacy dict) from `config.yaml`, matching the gateway's `_load_fallback_model()` pattern. Passed as `fallback_model=` to `AIAgent.__init__`, which normalizes both formats into a fallback chain.
- **Credential pool** â€” loads via `load_pool(provider)` from `agent.credential_pool` using the resolved runtime provider name. Only passed when the pool has credentials (`pool.has_credentials()`). Enables same-provider key rotation on 429/rate-limit errors.

This mirrors the gateway's behavior â€” without it, cron agents would fail on rate limits without attempting recovery.
...
| Local file | `local` | Save to `~/.zed/cron/output/` |
...
The `[SILENT]` prefix in a cron response suppresses delivery entirely â€” useful for jobs that only need to write to files or perform side effects.
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice â€” even between the gateway's in-process ticker and a standalone `zed cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `zed cron` CLI provides direct job management:

```bash
zed cron list                    # Show all jobs
zed cron create                  # Interactive job creation (alias: add)
zed cron edit <job_id>           # Edit job configuration
zed cron pause <job_id>          # Pause a running job
zed cron resume <job_id>         # Resume a paused job
zed cron run <job_id>            # Trigger immediate execution
zed cron remove <job_id>         # Delete a job
```
