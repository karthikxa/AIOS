...
The core orchestration engine is `run_agent.py`'s `AIAgent` class â€” a large file that handles everything from prompt assembly to tool dispatch to provider failover.
...
# Full interface â€” returns dict with messages, metadata, usage stats
...
Zed supports three API execution modes, resolved from provider selection, explicit args, and base URL heuristics:
...
2. Provider-specific detection (e.g., `anthropic` provider â†’ `anthropic_messages`)
3. Base URL heuristics (e.g., `api.anthropic.com` â†’ `anthropic_messages`)
...
- After the system message: `User â†’ Assistant â†’ User â†’ Assistant â†’ ...`
- During tool calling: `Assistant (with tool_calls) â†’ Tool â†’ Tool â†’ ... â†’ Assistant`
...
â”‚                                                    â”‚
â”‚   wait on:                     HTTP POST           â”‚
â”‚    - response ready     â”€â”€â”€â–¶   to provider         â”‚
â”‚    - interrupt event                               â”‚
â”‚    - timeout                                       â”‚
...
- **Single tool call** â†’ executed directly in the main thread
- **Multiple tool calls** â†’ executed concurrently via `ThreadPoolExecutor`
...
- Each agent gets its own budget. Subagents get independent budgets capped at `delegation.max_iterations` (default 50) â€” total iterations across parent + subagents can exceed the parent's cap
...
The fallback system also covers auxiliary tasks independently â€” vision, compression, and web extraction each have their own fallback chain configurable via the `auxiliary.*` config section.
...
After each turn:
- Messages are saved to the session store (SQLite via `zed_state.py`)
- Memory changes are flushed to `MEMORY.md` / `USER.md`
- The session can be resumed later via `/resume` or `zed chat --resume`

## Key Source Files

| File | Purpose |
|------|---------|
| `run_agent.py` | AIAgent class â€” the complete agent loop |
| `agent/prompt_builder.py` | System prompt assembly from memory, skills, context files, personality |
| `agent/context_engine.py` | ContextEngine ABC â€” pluggable context management |
| `agent/context_compressor.py` | Default engine â€” lossy summarization algorithm |
