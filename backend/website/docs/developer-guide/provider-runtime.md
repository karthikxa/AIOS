...
description: "How Zed resolves providers, credentials, API modes, and auxiliary models at runtime"
---

# Provider Runtime Resolution

Zed has a shared provider runtime resolver used across:
...
- `zed_cli/runtime_provider.py` â€” credential resolution, `_resolve_custom_runtime()`
- `zed_cli/auth.py` â€” provider registry, `resolve_provider()`
- `zed_cli/model_switch.py` â€” shared `/model` switch pipeline (CLI + gateway)
- `agent/auxiliary_client.py` â€” auxiliary model routing
- `providers/` â€” ABC + registry entry points (`ProviderProfile`, `register_provider`, `get_provider_profile`, `list_providers`)
- `plugins/model-providers/<name>/` â€” per-provider plugins (bundled) that declare `api_mode`, `base_url`, `env_vars`, `fallback_models` and register themselves into the registry on first access. User plugins at `$ZED_HOME/plugins/model-providers/<name>/` override bundled ones of the same name.

`get_provider_profile()` in `providers/` returns a `ProviderProfile` for a given provider id. `runtime_provider.py` calls this at resolution time to get the canonical `base_url`, `env_vars` priority list, `api_mode`, and `fallback_models` without needing to duplicate that data in multiple files. Adding a new plugin under `plugins/model-providers/<your-provider>/` (or `$ZED_HOME/plugins/model-providers/<your-provider>/`) that calls `register_provider()` is enough for `runtime_provider.py` to pick it up â€” no branch needed in the resolver itself.
...
That ordering matters because Zed treats the saved model/provider choice as the source of truth for normal runs. This prevents a stale shell export from silently overriding the endpoint a user last selected in `zed model`.
...
- LM Studio
- Tencent TokenHub
- Custom (`provider: custom`) â€” first-class provider for any OpenAI-compatible endpoint
...
This resolver is the main reason Zed can share auth/runtime logic between:
...
Zed contains logic to avoid leaking the wrong API key to a custom endpoint when multiple provider keys exist (e.g. `OPENROUTER_API_KEY` and `OPENAI_API_KEY`).
...
Zed also distinguishes between:
...
When provider resolution selects `anthropic`, Zed uses:
...
- Zed preflights Anthropic credential refresh before native Messages API calls
- Zed still retries once on a 401 after rebuilding the Anthropic client, as a fallback path
...
When an auxiliary task is configured with provider `main`, Zed resolves that through the same shared runtime path as normal chat. In practice that means:

- env-driven custom endpoints still work
- custom endpoints saved via `zed model` / `config.yaml` also work
- auxiliary routing can tell the difference between a real saved custom endpoint and the OpenRouter fallback

## Fallback models

Zed supports a configured fallback provider chain â€” a list of `(provider, model)` entries tried in order when the primary model encounters errors. The legacy single-pair `fallback_model` dict is still accepted for back-compat (and migrated on first write).
...
4. **Config flow**:
   - CLI: `cli.py` reads `CLI_CONFIG["fallback_model"]` â†’ passes to `AIAgent(fallback_model=...)`
   - Gateway: `gateway/run.py._load_fallback_model()` reads `config.yaml` â†’ passes to `AIAgent`
...
- `tests/run_agent/test_fallback_credential_isolation.py` â€” credential isolation between primary and fallback
- `tests/zed_cli/test_fallback_cmd.py` â€” the `/fallback` CLI command
- `tests/gateway/test_fallback_eviction.py` â€” gateway eviction of failed providers
