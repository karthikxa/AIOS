...
description: "How Zed detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in Zed' browser tooling:
...
1. **Native JS dialogs** (`alert`/`confirm`/`prompt`/`beforeunload`) block the
   ...
   dialog is open â€” subsequent tool calls hang or throw opaque errors.
...
|---|---|---|---|---|
| Local Chrome (`--remote-debugging-port`) / `/browser connect` | âœ“ | âœ“ full workflow | âœ“ | âœ“ |
| Browserbase | âœ“ (via bridge) | âœ“ full workflow (via bridge) | âœ“ | âœ“ |
| Camofox | âœ— no CDP (REST-only) | âœ— | partial via DOM snapshot | âœ— |
...
(`zed-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
they touch the network â€” the dialog becomes a `Fetch.requestPaused` event the
...
Camofox is unsupported â€” no CDP surface, REST-only.

## Architecture

### CDPSupervisor

One `asyncio.Task` running in a background daemon thread per Zed `task_id`.
Holds a persistent WebSocket to the backend's CDP endpoint. Maintains:

- **Dialog queue** â€” `List[PendingDialog]` with `{id, type, message, default_prompt, session_id, opened_at}`
- **Frame tree** â€” `Dict[frame_id, FrameInfo]` with parent relationships, URL, origin, whether cross-origin child session
- **Session map** â€” `Dict[session_id, SessionInfo]` so interaction tools can route to the right attached session for OOPIF operations
- **Recent console errors** â€” ring buffer of the last 50 for diagnostics

Subscribes on attach:

- `Page.enable` â€” `javascriptDialogOpening`, `frameAttached`, `frameNavigated`, `frameDetached`
- `Runtime.enable` â€” `executionContextCreated`, `consoleAPICalled`, `exceptionThrown`
- `Target.setAutoAttach {autoAttach: true, flatten: true}` â€” surfaces child OOPIF targets; supervisor enables `Page`+`Runtime` on each
...
- **Start:** `SupervisorRegistry.get_or_start(task_id, cdp_url)` â€” called by
...
- **Rebind:** if the CDP URL changes (user reconnects to a new Chrome), the
  old supervisor is stopped and a fresh one started â€” state is never reused
  across endpoints.
...
- **`must_respond`** (default) â€” capture, surface in `browser_snapshot`, wait
  ...
  forever.
- `auto_dismiss` â€” record and dismiss immediately; agent sees it after the
  fact via `browser_state` inside `browser_snapshot`.
- `auto_accept` â€” record and accept (useful for `beforeunload` where the
...
- `action="accept"` / `"dismiss"` â†’ responds to the specified or sole pending dialog (required)
- `prompt_text=...` â†’ text to supply to a `prompt()` dialog
- `dialog_id=...` â†’ disambiguate when multiple dialogs are queued (rare)
...
- **`pending_dialogs`** â€” dialogs currently blocking the page's JS thread.
...
- **`recent_dialogs`** â€” ring buffer of up to 20 recently-closed dialogs with
...
- **`frame_tree`** â€” frame structure including cross-origin (OOPIF) children.
...
No new tool schema surface for any of these â€” the agent reads the snapshot it
already requests.
...
hidden and the snapshot omits the new fields â€” no schema bloat.
...
`Runtime.evaluate` instead â€” the supervisor surfaces an error pointing at that
fallback when `frame_id` belongs to a non-OOPIF.

On Browserbase, this is the only reliable path for iframe interaction â€”
...
- `tools/browser_supervisor.py` â€” `CDPSupervisor`, `SupervisorRegistry`, `PendingDialog`, `FrameInfo`
- `tools/browser_dialog_tool.py` â€” `browser_dialog` tool handler
- `tools/browser_tool.py` â€” `browser_navigate` start-hook, `browser_snapshot` merge, `/browser connect` reattach, `_cleanup_browser_session` teardown
- `toolsets.py` â€” registers `browser_dialog` in `browser`, `zed-acp`, `zed-api-server`, and core toolsets (gated on CDP reachability)
- `zed_cli/config.py` â€” `browser.dialog_policy` and `browser.dialog_timeout_s` defaults
...
- Replacing `browser_cdp` â€” it stays as the escape hatch for the long tail (cookies, viewport, network throttling)
...
Chromium-family browser) is manual â€” exercise via `/browser connect` to a
