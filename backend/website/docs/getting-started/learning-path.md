...
First-time users almost always want `zed setup --portal` â€” one OAuth covers a model plus the four Tool Gateway tools (search/image/TTS/browser). See [Nous Portal](/integrations/nous-portal).
:::
...
Use reinforcement learning to fine-tune model behavior with Zed Agent's RL training pipeline (powered by [Atropos](https://github.com/NousResearch/atropos)).
