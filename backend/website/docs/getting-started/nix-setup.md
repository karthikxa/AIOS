...
description: "Install and deploy Zed Agent with Nix â€” from quick `nix run` to fully declarative NixOS module with container mode"
---

# Nix & NixOS Setup

Zed Agent ships a Nix flake with three levels of integration:
...
**For non-NixOS users**, this only changes the install step. Everything after (`zed setup`, `zed gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage zed the same way you manage any other NixOS service.
:::
...
After `nix profile install`, `zed`, `zed-agent`, and `zed-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) â€” `zed setup` walks you through provider selection, `zed gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.zed/`.
...
```bash
nix profile install github:NousResearch/zed-agent#messaging
```

For all optional extras (voice, all providers, all platforms):

```bash
nix profile install github:NousResearch/zed-agent#full
```
...
That's it. `nixos-rebuild switch` creates the `zed` user, generates `config.yaml`, wires up secrets, and starts the gateway â€” a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
Setting `addToSystemPackages = true` does two things: puts the `zed` CLI on your system PATH **and** sets `ZED_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `zed` in your shell creates a separate `~/.zed/` directory.
:::

### Container-aware CLI

:::info
When `container.enable = true` and `addToSystemPackages = true`, **every** `zed` command on the host automatically routes into the managed container. This means your interactive CLI session runs inside the same environment as the gateway service â€” with access to all container-installed packages and tools.

- The routing is transparent: `zed chat`, `zed sessions list`, `zed version`, etc. all exec into the container under the hood
...
- For developers working on the zed codebase, set `ZED_DEV=1` to bypass container routing and run the local checkout directly

Set `container.hostUsers` to create a `~/.zed` symlink to the service state directory, so the host CLI and the container share sessions, config, and memories:
...
Users listed in `hostUsers` are automatically added to the `zed` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo zed chat` manually.
:::
...
zed version
zed config       # shows the generated config
```
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, Zed defaults to OpenRouter.
:::
...
This bypasses `settings` entirely â€” no merge, no generation. The file is copied as-is to `$ZED_HOME/config.yaml` on each activation.
...
| Add API keys | `environmentFiles` | `[ config.sops.secrets."zed-env".path ]` |
| Give the agent a personality | `${services.zed-agent.stateDir}/.zed/SOUL.md` | manage the file directly |
...
| Override the zed package | `package` | `inputs.zed-agent.packages.${system}.default.override { ... }` |
| Change state directory | `stateDir` | `"/opt/zed"` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$ZED_HOME/.env` at activation time (`nixos-rebuild switch`). Zed reads this file on every startup, so changes take effect with a `systemctl restart zed-agent` â€” no container recreation needed.
...
```yaml
# secrets/zed.yaml (encrypted with sops)
zed-env: |
...
The `documents` option installs files into the agent's working directory (the `workingDirectory`, which the agent reads as its workspace). Zed looks for specific filenames by convention:

- **`USER.md`** â€” context about the user the agent is interacting with.
- Any other files you place here are visible to the agent as workspace files.

The agent identity file is separate: Zed loads its primary `SOUL.md` from `$ZED_HOME/SOUL.md`, which in the NixOS module is `${services.zed-agent.stateDir}/.zed/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.
...
:::tip
Environment variables in `env` values are resolved from `$ZED_HOME/.env` at runtime. Use `environmentFiles` to inject secrets â€” never put tokens directly in Nix config.
:::
...
Set `auth = "oauth"` for servers using OAuth 2.1. Zed implements the full PKCE flow â€” metadata discovery, dynamic client registration, token exchange, and automatic refresh.
...
Tokens are stored in `$ZED_HOME/mcp-tokens/<server-name>.json` and persist across restarts and rebuilds.

<details>
<summary><strong>Initial OAuth authorization on headless servers</strong></summary>

The first OAuth authorization requires a browser-based consent flow. In a headless deployment, Zed prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** â€” run the flow once via `docker exec` (container) or `sudo -u zed` (native):
...
# Ensure: chown zed:zed, chmod 0600
```
...
When zed runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `zed setup` | Config is declarative â€” edit `settings` in your Nix config |
| `zed config edit` | Config is generated from `settings` |
| `zed config set <key> <value>` | Config is generated from `settings` |
| `zed gateway install` | The systemd service is managed by NixOS |
| `zed gateway uninstall` | The systemd service is managed by NixOS |

This prevents drift between what Nix declares and what's on disk. Detection uses two signals:

1. **`ZED_MANAGED=true`** environment variable â€” set by the systemd service, visible to the gateway process
2. **`.managed` marker file** in `ZED_HOME` â€” set by the activation script, visible to interactive shells (e.g., `docker exec -it zed-agent zed config set ...` is also blocked)
...
When container mode is enabled, zed runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:

```
Host                                    Container
â”€â”€â”€â”€                                    â”€â”€â”€â”€â”€â”€â”€â”€â”€
/nix/store/...-zed-agent-0.1.0  â”€â”€â–º  /nix/store/... (ro)
~/.zed -> /var/lib/zed/.zed       (symlink bridge, per hostUsers)
/var/lib/zed/                    â”€â”€â–º  /data/          (rw)
  â”œâ”€â”€ current-package -> /nix/store/...    (symlink, updated each rebuild)
  â”œâ”€â”€ .gc-root -> /nix/store/...           (prevents nix-collect-garbage)
  â”œâ”€â”€ .container-identity                  (sha256 hash, triggers recreation)
  â”œâ”€â”€ .zed/                             (ZED_HOME)
  â”‚   â”œâ”€â”€ .env                             (merged from environment + environmentFiles)
  â”‚   â”œâ”€â”€ config.yaml                      (Nix-generated, deep-merged by activation)
  â”‚   â”œâ”€â”€ .managed                         (marker file)
  â”‚   â”œâ”€â”€ .container-mode                  (routing metadata: backend, exec_user, etc.)
  â”‚   â”œâ”€â”€ state.db, sessions/, memories/   (runtime state)
  â”‚   â””â”€â”€ mcp-tokens/                      (OAuth tokens for MCP servers)
  â”œâ”€â”€ home/                                â”€â”€â–º  /home/zed    (rw)
  â””â”€â”€ workspace/                           (agent working directory)
      â”œâ”€â”€ SOUL.md                          (from documents option)
      â””â”€â”€ (agent-created files)

Container writable layer (apt/pip/npm):   /usr, /usr/local, /tmp
```

The Nix-built binary works inside the Ubuntu container because `/nix/store` is bind-mounted â€” it brings its own interpreter and all dependencies, so there's no reliance on the container's system libraries. The container entrypoint resolves through a `current-package` symlink: `/data/current-package/bin/zed gateway run --replace`. On `nixos-rebuild switch`, only the symlink is updated â€” the container keeps running.

### What Persists Across What

| Event | Container recreated? | `/data` (state) | `/home/zed` | Writable layer (`apt`/`pip`/`npm`) |
|---|---|---|---|---|
| `systemctl restart zed-agent` | No | Persists | Persists | Persists |
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the zed package itself do **not** trigger recreation.

:::warning Writable layer loss
When the identity hash changes (image upgrade, new volumes, new container options), the container is destroyed and recreated from a fresh pull of `container.image`. Any `apt install`, `pip install`, or `npm install` packages in the writable layer are lost. State in `/data` and `/home/zed` is preserved (these are bind mounts).

If the agent relies on specific packages, consider baking them into a custom image (`container.image = "my-registry/zed-base:latest"`) or scripting their installation in the agent's SOUL.md.
:::

### GC Root Protection

The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current zed package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation â€” no imperative `zed plugins install` needed.

### Directory Plugins (`extraPlugins`)

For plugins that are just a source tree with `plugin.yaml` + `__init__.py` (e.g., [zed-lcm](https://github.com/stephenschoettler/zed-lcm)):
...
Plugins are symlinked into `$ZED_HOME/plugins/` at activation time. Zed discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.

### Entry-Point Plugins (`extraPythonPackages`)

For pip-packaged plugins that register via `[project.entry-points."zed_agent.plugins"]` (e.g., [rtk-zed](https://github.com/ogallotti/rtk-zed)):
...
The package's `site-packages` is added to PYTHONPATH in the zed wrapper. `importlib.metadata` discovers the entry point at session start.

### Optional Dependency Groups (`extraDependencyGroups`)

For optional extras declared in zed-agent's `pyproject.toml`, use `extraDependencyGroups` to include them in the sealed venv at build time. This is required for any extra not in the default `[all]` set â€” on Nix, runtime installation into the read-only store is not possible.

```nix
# Enable Discord, Telegram, Slack
services.zed-agent.extraDependencyGroups = [ "messaging" ];
```
...
:::note
A build-time collision check prevents plugin packages from shadowing core zed dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
nix build .#checks.x86_64-linux.managed-guard       # ZED_MANAGED blocks mutation
...
| Check | What it tests |
|---|---|
| `package-contents` | `zed` and `zed-agent` binaries exist and `zed version` runs |
| `entry-points-sync` | Every `[project.scripts]` entry in `pyproject.toml` has a wrapped binary in the Nix package |
| `cli-commands` | `zed --help` exposes `gateway` and `config` subcommands |
| `managed-guard` | `ZED_MANAGED=true zed config set ...` prints the NixOS error |
| `bundled-skills` | Skills directory exists, contains SKILL.md files, `ZED_BUNDLED_SKILLS` is set in wrapper |
...
|---|---|---|---|
| `enable` | `bool` | `false` | Enable the zed-agent service |
| `package` | `package` | `zed-agent` | The zed-agent package to use |
| `user` | `str` | `"zed"` | System user |
| `group` | `str` | `"zed"` | System group |
| `createUser` | `bool` | `true` | Auto-create user/group |
| `stateDir` | `str` | `"/var/lib/zed"` | State directory (`ZED_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory |
| `addToSystemPackages` | `bool` | `false` | Add `zed` CLI to system PATH and set `ZED_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | Paths to env files with secrets. Merged into `$ZED_HOME/.env` at activation time |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `zed gateway` |
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the zed user's per-user profile so terminal commands, skills, and cron jobs all see them |
| `extraPlugins` | `listOf package` | `[]` | Directory plugin packages to symlink into `$ZED_HOME/plugins/`. Each must contain `plugin.yaml` |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.zed` symlink to the service stateDir and are auto-added to the `zed` group |
...
```
/var/lib/zed/                     # stateDir (owned by zed:zed, 0750)
â”œâ”€â”€ .zed/                         # ZED_HOME
â”‚   â”œâ”€â”€ config.yaml                  # Nix-generated (deep-merged each rebuild)
â”‚   â”œâ”€â”€ .managed                     # Marker: CLI config mutation blocked
â”‚   â”œâ”€â”€ .env                         # Merged from environment + environmentFiles
â”‚   â”œâ”€â”€ auth.json                    # OAuth credentials (seeded, then self-managed)
â”‚   â”œâ”€â”€ gateway.pid
â”‚   â”œâ”€â”€ state.db
â”‚   â”œâ”€â”€ mcp-tokens/                  # OAuth tokens for MCP servers
â”‚   â”œâ”€â”€ sessions/
â”‚   â”œâ”€â”€ memories/
â”‚   â”œâ”€â”€ skills/
â”‚   â”œâ”€â”€ cron/
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Zed binary + all Nix deps |
| `/data` | `/var/lib/zed` | `rw` | All state, config, workspace |
| `/home/zed` | `${stateDir}/home` | `rw` | Persistent agent home â€” `pip install --user`, tool caches |
...
| `No adapter available for discord` (or telegram/slack) | Messaging deps missing from the sealed Nix venv | Install `#messaging` variant: `nix profile install ...#messaging`. For NixOS module: `extraDependencyGroups = [ "messaging" ]`. Check `journalctl -u zed-agent` for `FeatureUnavailable` or `requirements not met` for the underlying error. |
...
| `zed version` shows old version | Container not restarted | `systemctl restart zed-agent` |
| Permission denied on `/var/lib/zed` | State dir is `0750 zed:zed` | Use `docker exec` or `sudo -u zed` |
| `nix-collect-garbage` removed zed | GC root missing | Restart the service (preStart recreates the GC root) |
| `no container with name or ID "zed-agent"` (Podman) | Podman rootful container not visible to regular user | Add passwordless sudo for podman (see [Container Mode](#container-mode) section) |
| `unable to find user zed` | Container still starting (entrypoint hasn't created user yet) | Wait a few seconds and retry â€” the CLI retries automatically |
| Tool added via `extraPackages` not found in terminal | Requires `nixos-rebuild switch` to update the per-user profile | Rebuild and restart: `nixos-rebuild switch && systemctl restart zed-agent` |
