...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Zed Agent â€” from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Zed setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands â€” a good companion to this page if you'd rather follow along on video. For more, see the full [Zed Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.
...
- Setting up Zed for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Zed working on my machine | `zed setup` | Run a real chat and verify it responds |
| I already know my provider | `zed model` | Save the config, then start chatting |
| I want a bot or always-on setup | `zed gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `zed model` â†’ custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `zed model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Zed cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.
...
The single most important setup step. Use `zed model` to walk through the choice interactively:
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth â€” no API key needed (model name in `zed_cli/models.py` may change between releases) | `zed model` â†’ MiniMax (OAuth) |
...
| **Google Gemini (OAuth)** | Gemini via the `google-gemini-cli` OAuth flow â€” no key needed | `zed model` â†’ Google Gemini (OAuth) |
| **xAI** | Grok models via direct API | Set `XAI_API_KEY` |
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `zed model` â†’ xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth â€” no API key needed | `zed model` â†’ Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `zed model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `zed model` (requires `copilot` CLI + `copilot login`) |
...
Zed Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `zed model` â€” no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Zed separates secrets from normal config:

- **Secrets and tokens** â†’ `~/.zed/.env`
- **Non-secret settings** â†’ `~/.zed/config.yaml`
...
```bash
zed            # classic CLI
zed --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Zed ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config â€” try each with `zed` vs `zed --tui`.
:::
...
```bash
zed --continue    # Resume the most recent session
zed -c            # Short form
```
...
```bash
zed gateway setup    # Interactive platform configuration
```
...
- `zed tools` â€” tune tool access per platform
- `zed skills` â€” browse and install reusable workflows
...
```bash
zed config set terminal.backend docker    # Docker isolation
zed config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Zed install directory (the curl installer placed it at
# ~/.zed/zed-agent on Linux/macOS or %LOCALAPPDATA%\zed\zed-agent on Windows):
cd ~/.zed/zed-agent
...
Skills are on-demand instruction documents that teach Zed how to do a specific task â€” deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Zed ships with a catalog of bundled skills already installed in `~/.zed/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
zed skills browse                      # list everything available
zed skills search kubernetes           # find skills by keyword
zed skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub â€” `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `zed skills browse` shows the exact slugs to use.

**Use a skill** â€” every installed skill becomes a slash command automatically:

```bash
/k8s deploy the staging manifest          # run the skill with a request
/k8s                                       # load it and let Zed ask what you need
```

This works in the CLI and in any connected messaging platform. You don't have to install everything up front â€” the agent picks the right bundled skill on its own during normal conversation when a task matches one.
...
(If you installed without `[all]`, run `cd ~/.zed/zed-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Zed opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `zed model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `zed gateway setup` and check `zed gateway status` |
| `zed --continue` can't find old session | Switched profiles or session never saved | Check `zed sessions list` and confirm you're in the right profile |
...
| `zed doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
| `zed tools` | Configure which tools are enabled per platform |
| `zed setup` | Full setup wizard (configures everything at once) |
| `zed doctor` | Diagnose issues |
| `zed update` | Update to latest version |
| `zed gateway` | Start the messaging gateway |
| `zed --continue` | Resume last session |
