...
description: "Run Zed Agent directly on an Android phone with Termux"
---

# Zed on Android with Termux

This is the tested path for running Zed Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Zed from working well as a phone-native CLI agent â€” it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- attempts the broad `.[termux-all]` extra first and falls back to the smaller `.[termux]` extra (then a base install) â€” the curl installer matches this order automatically
- links `zed` into `$PREFIX/bin` so it stays on your Termux PATH
...
Why these packages?
- `python` â€” runtime + venv support
- `git` â€” clone/update the repo
- `clang`, `rust`, `make`, `pkg-config`, `libffi`, `openssl` â€” needed to build a few Python dependencies on Android
- `nodejs` â€” optional Node runtime for experiments beyond the tested core path
- `ripgrep` â€” fast file search
- `ffmpeg` â€” media / TTS conversions
...
### 5. Put `zed` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/zed" "$PREFIX/bin/zed"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `zed` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.zed/.env`.
...
### `zed doctor` says ripgrep or Node is missing
