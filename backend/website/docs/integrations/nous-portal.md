...
The Portal proxies a curated catalog of agentic models from across the ecosystem â€” billed against your Nous subscription instead of one credit balance per lab.
...
| **Zed** | Zed-4-70B, Zed-4-405B (chat, see [note below](#a-note-on-zed-4)) |
| **+ everything else** | 280+ additional models â€” the full agentic frontier |

Routing happens through OpenRouter under the hood, so model availability and failover behavior matches what you'd get with an OpenRouter key â€” just billed against your Nous subscription instead. Switch between Claude Sonnet 4.6 for code and Gemini 3 Pro for long context with `/model` mid-session â€” no new credentials, no top-ups, no surprise zero-balance errors.

### The Nous Tool Gateway

The same subscription unlocks the [Tool Gateway](/user-guide/features/tool-gateway), which routes Zed Agent's tool calls through Nous-managed infrastructure. Five backends, one login:
...
Without the gateway, hooking each of those up means a Firecrawl account, a FAL account, a Browser Use account, an OpenAI key, and a Modal account â€” five separate signups, five separate dashboards, five separate top-up flows. With the gateway, all of it routes through one subscription.

You can also enable just specific gateway tools (e.g. web search but not image generation) â€” see [Mixing the gateway with your own backends](#mixing-the-gateway-with-your-own-backends) below.
...
Because everything routes through one OAuth-authenticated Portal session, you don't accumulate a `.env` file with a dozen long-lived API keys. The refresh token at `~/.zed/auth.json` is the only credential on disk, and Zed mints short-lived JWTs from it per request â€” see [Token handling](#token-handling) below.

### Cross-platform parity

[Native Windows](/user-guide/windows-native) makes per-tool API key setup its rough edge â€” installing a Firecrawl account, a FAL account, a Browser Use account, an OpenAI key from Windows is the highest-friction part of getting a useful agent. A Portal subscription smooths that out: one OAuth covers the model and every gateway tool, so Windows users get the same experience as macOS/Linux without manually configuring four backends.
...
If you already have Zed configured with OpenRouter, Anthropic, or any other provider and you want to add the Portal alongside them:
...
Your existing providers stay configured. You can switch between them with `/model` mid-session or `zed model` between sessions â€” the Portal becomes one of your available providers, not your only one.

### Headless / SSH / remote setup

OAuth needs a browser, but the loopback callback runs on the machine where Zed is running. For remote hosts, see [OAuth over SSH / Remote Hosts](/guides/oauth-over-ssh) â€” the same patterns work for the Portal as for any other OAuth-based provider (`ssh -L` port forwarding, `--manual-paste` for browser-only environments like Cloud Shell / Codespaces).

### Profile setup

If you use [Zed profiles](/user-guide/profiles), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically â€” no need to repeat the OAuth flow per profile.
...
zed portal open       # open the subscription management page in your browser
```

`zed portal` (with no subcommand) is the human-readable alias for `zed auth add nous --type oauth` â€” it logs you in, lets you pick a Nous model, sets Nous as your inference provider, and offers the Tool Gateway opt-in (identical to `zed setup --portal`, and the same Nous flow as the first-time quick setup).
...
If you already have, say, a Browserbase account and want to keep using it while routing web search and image generation through Nous, that's supported. Use `zed tools` to pick backends per tool:

```bash
zed tools
# â†’ Web search       â†’ "Nous Subscription"
# â†’ Image generation â†’ "Nous Subscription"
...
After `zed setup --portal`, `~/.zed/config.yaml` will look like:
...
The OAuth refresh token is stored separately at `~/.zed/auth.json` (not in `config.yaml` â€” credentials and configuration are kept separate by design).

## Token handling

Zed mints a short-lived JWT from your stored Portal refresh token on each inference call rather than reusing a long-lived API key. The token lifecycle is fully automatic â€” refresh, mint, retry on transient 401 â€” and you never see it.

If the Portal invalidates the refresh token (password change, manual revoke, session expiry), the invalid refresh token is **quarantined locally** so Zed stops replaying it and you don't see a stream of identical 401s. The next call surfaces a clear "re-authentication required" message. Run `zed auth add nous` to log in again; the quarantine clears on the next successful login.

## Troubleshooting

### `zed portal info` shows "not logged in"
...
Your Portal refresh token was invalidated (password change, manual revoke, or session expiry). Run `zed auth add nous` and your next request will use the new credentials. Any quarantine on the old token clears automatically on successful re-login.
...
- **[Tool Gateway](/user-guide/features/tool-gateway)** â€” Full details on every gateway tool, per-tool config, and pricing
- **[Subscription proxy](/user-guide/features/subscription-proxy)** â€” Use your Portal subscription from non-Zed tools (other agents, scripts, third-party clients)
- **[Voice mode](/user-guide/features/voice-mode)** â€” Voice conversations using the Portal's OpenAI TTS
- **[AI Providers](/integrations/providers)** â€” Full provider catalog if you want to compare alternatives
- **[OAuth over SSH](/guides/oauth-over-ssh)** â€” Login from remote hosts or browser-only environments
- **[Profiles](/user-guide/profiles)** â€” Multiple Zed configurations sharing one Portal login
