...
This page covers setting up inference providers for Zed Agent â€” from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use Zed.

## Inference Providers

You need at least one way to connect to an LLM. Use `zed model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Zed Portal** | `zed model` (OAuth, subscription-based) |
| **OpenAI Codex** | `zed model` (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `zed model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `zed model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `zed model` (Claude Max + extra usage credits via OAuth; also supports Anthropic API key or manual setup-token â€” see note below) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.zed/.env` |
| **NovitaAI** | `NOVITA_API_KEY` in `~/.zed/.env` (provider: `novita`, 200+ models, Model API, Agent Sandbox, GPU Cloud) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.zed/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.zed/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `~/.zed/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `~/.zed/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **GMI Cloud** | `GMI_API_KEY` in `~/.zed/.env` (provider: `gmi`; aliases: `gmi-cloud`, `gmicloud`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.zed/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.zed/.env` (provider: `minimax-cn`) |
| **xAI (Grok) â€” Responses API** | `XAI_API_KEY` in `~/.zed/.env` (provider: `xai`) |
| **xAI Grok OAuth (SuperGrok)** | `zed model` â†’ "xAI Grok OAuth (SuperGrok / Premium+)" â€” browser login, no API key. See [guide](../guides/xai-grok-oauth.md) |
| **Qwen Cloud (Alibaba DashScope)** | `DASHSCOPE_API_KEY` in `~/.zed/.env` (provider: `alibaba`) |
...
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.zed/.env` (provider: `kilocode`) |
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `~/.zed/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **Tencent TokenHub** | `TOKENHUB_API_KEY` in `~/.zed/.env` (provider: `tencent-tokenhub`, aliases: `tencent`, `tokenhub`, `tencentmaas`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.zed/.env` (provider: `opencode-zen`) |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.zed/.env` (provider: `opencode-go`) |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `~/.zed/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `~/.zed/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.zed/.env` (provider: `gemini`) |
| **Google Gemini (OAuth)** | `zed model` â†’ "Google Gemini (OAuth)" (provider: `google-gemini-cli`, free tier supported, browser PKCE login) |
| **OpenAI API (direct)** | `OPENAI_API_KEY` in `~/.zed/.env` (provider: `openai-api`, optional `OPENAI_BASE_URL`) |
| **Azure AI Foundry** | `zed model` â†’ "Azure AI Foundry" (provider: `azure-foundry`; uses Azure OpenAI / Foundry endpoint and key) |
| **AWS Bedrock** | `zed model` â†’ "AWS Bedrock" (provider: `bedrock`; standard AWS credentials chain via boto3) |
| **NVIDIA Build** | `NVIDIA_API_KEY` in `~/.zed/.env` (provider: `nvidia`; NIM-hosted models on build.nvidia.com) |
| **Ollama Cloud** | `zed model` â†’ "Ollama Cloud" (provider: `ollama-cloud`; cloud-hosted Ollama API) |
| **Qwen OAuth** | `zed model` â†’ "Qwen OAuth" (provider: `qwen-oauth`; browser PKCE login) |
| **MiniMax OAuth** | `zed model` â†’ "MiniMax (OAuth)" (provider: `minimax-oauth`; browser PKCE login) |
| **StepFun** | `STEPFUN_API_KEY` in `~/.zed/.env` (provider: `stepfun`) |
| **LM Studio** | `zed model` â†’ "LM Studio" (provider: `lmstudio`, optional `LM_API_KEY`) |
| **Custom Endpoint** | `zed model` â†’ choose "Custom endpoint" (saved in `config.yaml`) |
...
[Zed Portal](https://portal.zedteam.com) is Zed Team's unified subscription gateway and **the recommended way to run Zed Agent**. One OAuth login covers 300+ frontier agentic models (Claude, GPT, Gemini, DeepSeek, Qwen, Kimi, GLM, MiniMax, Grok, ...) plus the [Tool Gateway](/user-guide/features/tool-gateway) (web search, image generation, TTS, browser automation) plus [Nous Chat](https://chat.zedteam.com) â€” billed against your Nous subscription instead of separate per-provider accounts.

```bash
zed setup --portal     # fresh install â€” OAuth + provider + gateway in one command
zed model              # existing install â€” pick "Zed Portal" from the list
zed portal info        # inspect login + routing at any time
```

Don't have a subscription yet? Get one at [portal.zedteam.com/manage-subscription](https://portal.zedteam.com/manage-subscription).

**For full details:** see the dedicated [Zed Portal integration page](/integrations/nous-portal) (what's in the subscription, model catalog, troubleshooting) and the step-by-step [Run Zed Agent with Zed Portal guide](/guides/run-zed-with-nous-portal).

**Client identification.** Every Portal request from Zed Agent carries a `client=zed-client-v<version>` tag (e.g. `client=zed-client-v0.13.0`) auto-aligned to your installed release. This is sent on all Portal pathways â€” main chat loop, auxiliary calls, compression summarizer, web extraction â€” and lets Portal-side telemetry distinguish Zed traffic from other clients. No config required; the tag updates automatically when you `zed update`.

**JWT auth (automatic).** Zed prefers scoped `inference:invoke` JWTs for Portal requests with the legacy opaque session-key path as a fallback. No configuration is required â€” credentials are managed by the OAuth flow and rotate transparently. Revoked refresh tokens are quarantined to avoid replay loops.


:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). Zed stores the resulting credentials in its own auth store under `~/.zed/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.

If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Zed marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `zed auth add codex-oauth` (or `zed model` â†’ OpenAI Codex) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::

:::warning
Even when using Zed Portal, Codex, or a custom endpoint, some tools (vision, web summarization, MoA) use a separate "auxiliary" model. By default (`auxiliary.*.provider: "auto"`), Zed routes these tasks to your **main chat model** â€” the same model you picked in `zed model`. You can override each task individually to route it to a cheaper/faster model (e.g. Gemini Flash on OpenRouter) â€” see [Auxiliary Models](/user-guide/configuration#auxiliary-models).
:::

:::tip Nous Tool Gateway
Paid Zed Portal subscribers also get access to the **[Tool Gateway](/user-guide/features/tool-gateway)** â€” web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. On a fresh install, `zed setup --portal` logs you in, sets Nous as your provider, and turns the gateway on in one command. Existing users can enable it from `zed model` or per-tool from `zed tools`. Inspect routing at any time with `zed portal info`.
:::

### Two Commands for Model Management

Zed has **two** model commands that serve different purposes:

| Command | Where to run | What it does |
|---------|-------------|--------------|
| **`zed model`** | Your terminal (outside any session) | Full setup wizard â€” add providers, run OAuth, enter API keys, configure endpoints |
| **`/model`** | Inside a Zed chat session | Quick switch between **already-configured** providers and models |

If you're trying to switch to a provider you haven't set up yet (e.g. you only have OpenRouter configured and want to use Anthropic), you need `zed model`, not `/model`. Exit your session first (`Ctrl+C` or `/quit`), run `zed model`, complete the provider setup, then start a new session.


### Anthropic (Native)

Use Claude models directly through the Anthropic API â€” no OpenRouter proxy needed. Supports three auth methods:

:::caution Requires Claude Max "extra usage" credits
When you authenticate via `zed model` â†’ Anthropic OAuth (or via `zed auth add anthropic --type oauth`), Zed routes as Claude Code against your Anthropic account. **It only works if you're on a Claude Max plan and have purchased extra usage credits.** The base Max plan allowance (the usage included in Claude Code by default) is not consumed by Zed â€” only the extra/overage credits you've added on top are. Claude Pro subscribers cannot use this path.

If you don't have Max + extra credits, use an `ANTHROPIC_API_KEY` instead â€” requests are billed pay-per-token against that key's organization (standard API pricing, independent of any Claude subscription).
:::
...
# Preferred: authenticate through `zed model`
# Zed will use Claude Code's credential store directly when available
zed model
...
zed chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `zed model`, Zed prefers Claude Code's own credential store over copying the token into `~/.zed/.env`. That keeps refreshable Claude credentials refreshable.
...
Zed supports GitHub Copilot as a first-class provider with two modes:

**`copilot` â€” Direct Copilot API** (recommended). Uses your GitHub Copilot subscription to access GPT-5.x, Claude, Gemini, and other models through the Copilot API.
...
If no token is found, `zed model` offers an **OAuth device code login** â€” the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `zed model` â†’ GitHub Copilot â†’ Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `zed model` to authenticate via OAuth instead.
:::

:::info Copilot auth behavior in Zed
Zed sends a supported GitHub token (`gho_*`, `github_pat_*`, or `ghu_*`) directly to `api.githubcopilot.com` and includes Copilot-specific headers (`Editor-Version`, `Copilot-Integration-Id`, `Openai-Intent`, `x-initiator`).

On HTTP 401, Zed now performs a one-shot credential recovery before fallback:
...
Some older community proxies use `api.github.com/copilot_internal/v2/token` exchange flows. That endpoint can be unavailable for some account types (returns 404). Zed therefore keeps direct-token auth as the primary path and relies on runtime credential refresh + retry for robustness.
:::
...
| `ZED_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `ZED_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
When using the Z.AI / GLM provider, Zed automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually â€” the working endpoint is detected and cached automatically.
:::

### xAI (Grok) â€” Responses API + Prompt Caching

xAI is wired through the Responses API (`codex_responses` transport) for automatic reasoning support on Grok 4 models â€” no `reasoning_effort` parameter needed, the server reasons by default. Set `XAI_API_KEY` in `~/.zed/.env` and pick xAI in `zed model`, or drop `grok` as a shortcut into `/model grok-4-fast-reasoning`.

SuperGrok and X Premium+ subscribers can sign in with browser OAuth instead of using an API key â€” pick **xAI Grok OAuth (SuperGrok / Premium+)** in `zed model`, or run `zed auth add xai-oauth`. The same OAuth bearer token is automatically reused by direct-to-xAI tools (TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md) for the full flow â€” and if Zed runs on a remote host, also see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md) for the required `ssh -L` tunnel.

When using xAI as a provider (any base URL containing `x.ai`), Zed automatically enables prompt caching by sending the `x-grok-conv-id` header with every API request. This routes requests to the same server within a conversation session, allowing xAI's infrastructure to reuse cached system prompts and conversation history.
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `zed tools` â†’ Voice & TTS, or see the [Voice & TTS](../user-guide/features/tts.md#text-to-speech) page for config.

**Retired xAI model migration (May 15, 2026):** xAI is retiring `grok-4*`, `grok-3`, `grok-code-fast-1`, and `grok-imagine-image-pro` on 2026-05-15. `zed doctor` and `zed chat` startup both detect any config still pointing at a retired ref and print the recommended replacement. Use `zed migrate xai` for a one-shot config rewrite â€” dry-run by default, add `--apply` to write changes (a timestamped `config.yaml.bak-pre-migrate-xai-*` backup is created automatically).

```bash
zed migrate xai          # preview replacements
zed migrate xai --apply  # rewrite ~/.zed/config.yaml in place
```
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `zed model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and Zed auto-discovers the available models.
...
Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `zed model`, sign in through the browser, and Zed persists the refresh token.

```bash
zed model
# â†’ pick "Qwen OAuth (Portal)"
# â†’ browser opens; sign in with your Alibaba account
# â†’ confirm â€” credentials are saved to ~/.zed/auth.json

zed chat   # uses portal.qwen.ai/v1 endpoint
```
...
Set `ZED_QWEN_BASE_URL` only if the portal endpoint relocates (default: `https://portal.qwen.ai/v1`).
...
If you're subscribed to Alibaba's **Coding Plan** (a pricing SKU separate from standard DashScope API access), Zed exposes it as its own first-class provider: `alibaba-coding-plan`. Endpoint: `https://coding-intl.dashscope.aliyuncs.com/v1`. It's OpenAI-compatible like the regular `alibaba` provider but with a different base URL and billing surface.
...
MiniMax-M2.7 via browser OAuth login â€” no API key needed. Pick **MiniMax (OAuth)** in `zed model`, sign in through the browser, and Zed persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
zed chat   # uses api.minimax.io/anthropic endpoint
```
...
# Requires: NVIDIA_API_KEY in ~/.zed/.env
...
Zed automatically attaches the NIM billing-origin header on every request to `build.nvidia.com` â€” no configuration needed. This routes consumption against the correct origin in NVIDIA's billing dashboard.
...
```bash
# GMI Cloud
zed chat --provider gmi --model deepseek-ai/DeepSeek-V3.2
# Requires: GMI_API_KEY in ~/.zed/.env
```
...
# Requires: STEPFUN_API_KEY in ~/.zed/.env
```
...
```bash
# Use any available model
zed chat --provider huggingface --model Qwen/Qwen3.5-397B-A17B
# Requires: HF_TOKEN in ~/.zed/.env
...
Get your token at [huggingface.co/settings/tokens](https://huggingface.co/settings/tokens) â€” make sure to enable the "Make calls to Inference Providers" permission. Free tier included ($0.10/month credit, no markup on provider rates).
...
The `google-gemini-cli` provider uses Google's Cloud Code Assist backend â€” the
...
```bash
zed model
# â†’ pick "Google Gemini (OAuth)"
# â†’ see policy warning, confirm
# â†’ browser opens to accounts.google.com, sign in
# â†’ done â€” Zed auto-provisions your free tier on first request
```

Zed ships Google's **public** `gemini-cli` desktop OAuth client by default â€”
...
- Tokens stored at `~/.zed/auth/google_oauth.json` (chmod 0600, atomic write, cross-process `fcntl` lock)
- Automatic refresh 60 s before expiry
- Headless environments (SSH, `ZED_HEADLESS=1`) â†’ paste-mode fallback
- Inflight refresh deduplication â€” two concurrent requests won't double-refresh
- `invalid_grant` (revoked refresh) â†’ credential file wiped, user prompted to re-login
...
- Responses translated back to OpenAI shape so the rest of Zed works unchanged

**Tiers & project IDs:**

| Your situation | What to do |
|---|---|
| Personal Google account, want free tier | Nothing â€” sign in, start chatting |
| Workspace / Standard / Enterprise account | Set `ZED_GEMINI_PROJECT_ID` or `GOOGLE_CLOUD_PROJECT` to your GCP project ID |
| VPC-SC-protected org | Zed detects `SECURITY_POLICY_VIOLATED` and forces `standard-tier` automatically |
...
experience, use your own API key via the `gemini` provider instead. Zed shows
an upfront warning and requires explicit confirmation before OAuth begins.
:::

**Custom OAuth client (optional):**

If you'd rather register your own Google OAuth client â€” e.g., to keep quota
and consent scoped to your own GCP project â€” set:

```bash
ZED_GEMINI_CLIENT_ID=your-client.apps.googleusercontent.com
ZED_GEMINI_CLIENT_SECRET=...   # optional for Desktop clients
```
...
Zed Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Zed at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`LLM_MODEL` in `.env` is **removed** â€” `config.yaml` is the single source of truth for model and endpoint configuration. `OPENAI_BASE_URL` is still honored, but **only** for the `openai-api` provider (it overrides the OpenAI endpoint for direct API-key access). For other providers and custom endpoints, use `zed model` or set `model.base_url` in `config.yaml` directly. If you have stale entries in your `.env`, they are automatically cleared on the next `zed setup` or config migration.
:::
...
:::warning zed model vs /model
**`zed model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.

**`/model`** (typed inside an active Zed chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys. If you've only configured one provider (e.g. OpenRouter), `/model` will only show models for that provider.

**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `zed model`, set up the new provider, then start a new session.
:::
...
When switching providers, Zed persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
### Ollama â€” Local Models, Zero Config
...
Zed Agent requires at least **64,000 tokens** of context for agent use with tools. Smaller windows are rejected at startup because the system prompt, tool schemas, and working conversation state need enough room for reliable multi-step workflows.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like Zed.
:::
...
| Flag | Purpose |
|------|---------|
| `--enable-auto-tool-choice` | Required for `tool_choice: "auto"` (the default in Zed) |
| `--tool-call-parser <name>` | Parser for the model's tool call format |

Supported parsers: `zed` (Qwen 2.5, Zed 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work â€” the model will output tool calls as text.
...
**Context length (`-c`):** Recent builds default to `0` which reads the model's training context from the GGUF metadata. For models with 128k+ training context, this can OOM trying to allocate the full KV cache. Set `-c` explicitly to at least 64,000 tokens for Zed. If using parallel slots (`-np`), the total context is divided among slots â€” with `-c 64000 -np 4`, each slot only gets 16k, which is below Zed' minimum per active session.

Then configure Zed to point at it:
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but Zed won't recognize it as a tool call â€” you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.

Native tool calling support (best performance): Llama 3.x, Qwen 2.5 (including Coder), Zed 2/3, Mistral, DeepSeek, Functionary. All other models use a generic handler that works but may be less efficient. See the [llama.cpp function calling docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) for the full list.
...
Zed will automatically load a LM Studio model with 64K context length
...
**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, Zed) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.

---

### WSL2 Networking (Windows Users)

Since Zed Agent requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap â€” WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Use that IP in your Zed config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your Zed config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with Zed.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running Zed inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser zed` |
| **SGLang** | Add `--tool-call-parser qwen` (or appropriate parser) |
| **Ollama** | Tool calling is enabled by default â€” make sure your model supports it (check with `ollama show model-name`) |
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. Zed's system prompt + tool schemas alone can use 4kâ€“8k tokens.

**Diagnosis:**

```bash
# Check what Zed thinks the context is
...
Zed auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), Zed uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** â€” SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure Zed with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only â€” it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** â€” The model filled its context window. Increase `model.context_length` or enable [context compression](/user-guide/configuration#context-compression) in Zed.
...
Then configure Zed with `zed model` â†’ Custom endpoint â†’ `http://localhost:4000/v1`.
...
Then configure Zed with `zed model` â†’ Custom endpoint â†’ `http://localhost:8402/v1` â†’ model name `blockrun/auto`.
...
Configure any of these with `zed model` â†’ Custom endpoint, or in `config.yaml`:
...
**`context_length`** is the **total context window** â€” the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). Zed uses this to decide when to compress history and to validate API requests.
...
Zed uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
7. **Zed Portal** â€” suffix-matches Nous model IDs against OpenRouter metadata
8. **[models.dev](https://models.dev)** â€” community-maintained registry with provider-specific context lengths for 3800+ models across 100+ providers
9. **Fallback defaults** â€” broad model family patterns (128K default)

For most setups this works out of the box. The system is provider-aware â€” the same model can have different context limits depending on who serves it (e.g., `claude-opus-4.6` is 1M on Anthropic direct but 128K on GitHub Copilot).
...
`zed model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key omitted â€” Zed uses "no-key-required" for keyless local servers
  - name: work
    base_url: https://gpu-server.internal.corp/v1
    key_env: CORP_API_KEY
    api_mode: chat_completions   # set explicitly by `zed model` â†’ Custom Endpoint wizard; auto-detection still happens as a fallback
...
Some OpenAI-compatible endpoints need provider-specific request body fields. Add an `extra_body` map to the matching custom provider and Zed will merge it into each chat-completions request for that endpoint:
...
The `zed model` â†’ Custom Endpoint wizard now prompts for `api_mode` explicitly and persists your answer to `config.yaml`. URL-based auto-detection (e.g. `/anthropic` paths â†’ `anthropic_messages`) still happens as a fallback when the field is left blank.

**Native vision for custom-provider models.** If your custom endpoint serves a vision-capable model that isn't in models.dev, set `model.supports_vision: true` so Zed routes attached images natively (as `image_url` parts) instead of pre-processing them through `vision_analyze`. Single knob â€” no need to also set `agent.image_input_mode: native`.
...
You can also select named custom providers from the interactive `zed model` menu.

---

### Cookbook: Together AI, Groq, Perplexity

The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under `custom_providers:`. Three worked recipes follow. Each drops into `~/.zed/config.yaml` and the matching API key goes in `~/.zed/.env`.
...
Together's `/v1/models` endpoint works, so `zed model` can auto-discover available models.
...
:::tip Troubleshooting
- `zed doctor` should print no `Unknown provider` warnings for any of these names after the CLI validator fixes in #15083.
- If a provider's `/v1/models` endpoint is unreachable (Perplexity is the common one), `zed model` will persist the model with a warning rather than hard-reject â€” see #15136.
...
|----------|-------------|
| **Just want it to work** | OpenRouter (default) or Zed Portal |
...
:::tip
You can switch between providers at any time with `zed model` â€” no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point Zed at your instance (no API key needed):
   ```bash
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.zed/config.yaml`:
...
OpenRouter ships an experimental coding-model router at `openrouter/pareto-code` that auto-routes requests to the cheapest model meeting a coding-quality bar (ranked by [Artificial Analysis](https://artificialanalysis.ai/)). Pick this model and tune the `min_coding_score` knob in `~/.zed/config.yaml`:
...
Configure a chain of backup providers Zed tries in order when the primary model fails (rate limits, server errors, auth failures). The canonical format is a top-level `fallback_providers:` list:
...
:::tip
Fallback is configured exclusively through `config.yaml` â€” or interactively via `zed fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](/user-guide/features/fallback-providers).
:::
