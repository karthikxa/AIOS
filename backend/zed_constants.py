...
_UNSET = object()
_ZED_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_ZED_HOME_OVERRIDE", default=_UNSET
)


def set_zed_home_override(path: str | Path | None) -> Token:
    """Set a context-local Zed home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _ZED_HOME_OVERRIDE.set(value)


def reset_zed_home_override(token: Token) -> None:
    """Restore the previous context-local Zed home override."""
    _ZED_HOME_OVERRIDE.reset(token)


def get_zed_home_override() -> str | None:
    """Return the active context-local Zed home override, if any."""
    override = _ZED_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_zed_home() -> Path:
    """Return the platform-native default Zed home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "zed"
    return Path.home() / ".zed"
...
def get_zed_home() -> Path:
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_zed_home()
    ...
    return _get_platform_default_zed_home()


def get_default_zed_root() -> Path:
    """Return the root Zed directory for profile-level operations.

    In standard deployments this is the platform-native Zed home
    (``~/.zed`` on POSIX, ``%LOCALAPPDATA%\\zed`` on native Windows).
    ...
    try:
        ...
        # ZED_HOME is under ~/.zed (normal or profile mode)
        return native_home
    except ValueError:
        pass
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Zed is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``ZED_OPTIONAL_SKILLS``.
    """
    override = os.getenv("ZED_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_zed_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    """
    override = os.getenv("ZED_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_zed_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``ZED_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<ZED_HOME>/skills`` last-resort
    """
    override = os.getenv("ZED_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_zed_home() / "skills"


def get_zed_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Zed subdirectory with backward compatibility.
...
def iter_zed_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Zed-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\zed\\node``. POSIX installs use
    ``$ZED_HOME/node/bin``. Include both shapes on every platform so mixed
...
def with_zed_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Zed-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_zed_node_dirs() if path.is_dir()]
...
def display_zed_home() -> str:
    """Return a user-friendly display string for the current ZED_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.zed``
        profile:  ``~/.zed/profiles/coder``
        custom:   ``/opt/zed-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.zed``.  For code that needs a real ``Path``, use
    :func:`get_zed_home` instead.
    """
    home = get_zed_home()
    try:
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{ZED_HOME}/home`` when the profile-home directory exists."""
    zed_home = get_zed_home_override() or (env or {}).get("ZED_HOME") or os.getenv("ZED_HOME")
    if not zed_home:
        return None
    profile_home = os.path.join(zed_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("ZED_REAL_HOME") or os.getenv("ZED_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Zed profile HOME.

    ``ZED_HOME`` scopes Zed state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={ZED_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{ZED_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{ZED_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Zed' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["ZED_REAL_HOME"] = real_home
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under ZED_HOME.

    Replaces the ``get_zed_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, zed_logging.py, zed_time.py, etc.).
    """
    return get_zed_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under ZED_HOME."""
    return get_zed_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under ZED_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_zed_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._zed_ipv4_patched = True  # type: ignore[attr-defined]
