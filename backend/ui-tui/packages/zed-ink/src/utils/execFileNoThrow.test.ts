...
// timeout â€” exactly the production wl-copy bug. With resolveOnExit, the
...
function trackSleeperPid(pidFile: string): void {
  try {
    ...
    // PID file not written or unreadable â€” sleeper may have already exited.
  }
}

beforeEach(() => {
  sleeperPids = []
  scriptDir = join(tmpdir(), `zed-execfile-test-${process.pid}-${Date.now()}`)
...
describe.skipIf(onWindows)('execFileNoThrow with daemon-style children', () => {
  ...
  // SIGTERM at the timeout doesn't help â€” the daemon survives it. To
  ...
  it('settles on timeout=124 when the child itself never exits, even with resolveOnExit', async () => {
    ...
    // Child process never exits on its own â†’ timer fires â†’ SIGTERM â†’
    // child exits â†’ 'exit' fires with non-null signal. The settle()
