  ...
  it('honors explicit env override', () => {
    expect(
      shouldEmitClipboardSequence({
        ZED_TUI_CLIPBOARD_OSC52: '1',
        TMUX: '/tmp/tmux-1/default,1,0'
      } as NodeJS.ProcessEnv)
    ).toBe(true)
    expect(
      shouldEmitClipboardSequence({ ZED_TUI_COPY_OSC52: '0', TERM: 'xterm-256color' } as NodeJS.ProcessEnv)
    ).toBe(false)
  })

  it('ZED_TUI_FORCE_OSC52 takes precedence over TMUX suppression', () => {
    ...
    expect(
      shouldEmitClipboardSequence({
        ZED_TUI_FORCE_OSC52: '1',
        TMUX: '/tmp/t,1,0'
      } as NodeJS.ProcessEnv)
    ).toBe(true)
  })

  it('ZED_TUI_FORCE_OSC52=0 suppresses OSC 52 even for remote or plain terminals', () => {
    expect(
      shouldEmitClipboardSequence({
        ZED_TUI_FORCE_OSC52: '0',
        SSH_CONNECTION: '1'
      } as NodeJS.ProcessEnv)
    ).toBe(false)
  })
...
describe('supportsOsc52Clipboard', () => {
  ...
  // detectTerminal() in utils/env.ts returns â€” TERM=xterm-ghostty normalises
  ...
  // Intentionally conservative â€” iTerm2 disables OSC 52 by default; Alacritty
...
// existing style â€” tests pass env/terminal as arguments instead of using
// vi.mock â€” and gives broader coverage of the env x terminal matrix.
describe('shouldUseNativeClipboard', () => {
  ...
  it('returns true on plain local terminals (existing behaviour)', () => {
    // Non-allowlisted terminals â€” xterm, GNOME Terminal, Apple_Terminal,
  ...
  it('returns false on allowlisted local terminals (the race-fix case)', () => {
    // Ghostty / kitty / WezTerm / Windows Terminal / VS Code â€” OSC 52
  ...
  it('returns true inside tmux even on allowlisted outer terminal', () => {
    ...
    // through tmux load-buffer there, not raw OSC 52 â€” the wl-copy race
  ...
  it('returns true inside GNU screen even on allowlisted outer terminal', () => {
    // Same reasoning as TMUX â€” STY indicates we're inside screen, which
  ...
  it('returns true when OSC 52 emission is disabled via ZED_TUI_FORCE_OSC52=0', () => {
    ...
    // remaining path â€” keep it on regardless of terminal allowlist.
    expect(shouldUseNativeClipboard({ ZED_TUI_FORCE_OSC52: '0' } as NodeJS.ProcessEnv, 'ghostty')).toBe(true)
    expect(shouldUseNativeClipboard({ ZED_TUI_FORCE_OSC52: '0' } as NodeJS.ProcessEnv, 'kitty')).toBe(true)
    expect(shouldUseNativeClipboard({ ZED_TUI_CLIPBOARD_OSC52: '0' } as NodeJS.ProcessEnv, 'WezTerm')).toBe(true)
    expect(shouldUseNativeClipboard({ ZED_TUI_COPY_OSC52: 'no' } as NodeJS.ProcessEnv, 'vscode')).toBe(true)
  })

  it('returns true under TMUX even with ZED_TUI_FORCE_OSC52=1 on an allowlisted terminal (tmux load-buffer path)', () => {
    ...
    expect(
      shouldUseNativeClipboard({ ZED_TUI_FORCE_OSC52: '1', TMUX: '/tmp/t,1,0' } as NodeJS.ProcessEnv, 'ghostty')
      // TMUX guard wins â€” native still fires because we're going through
  ...
  it('SSH_CONNECTION takes precedence over allowlisted terminal', () => {
    ...
    // remote machine â€” the user's clipboard is on the other end.
    expect(shouldUseNativeClipboard({ SSH_CONNECTION: '1' } as NodeJS.ProcessEnv, 'ghostty')).toBe(false)
  })
