 ...
 * through to the -256color regex â†’ level 2. At level 2, chalk.rgb()
 * downgrades to the nearest 6Ã—6Ã—6 cube color: rgb(215,119,87) â†’ idx 174
 * rgb(215,135,135) â€” washed-out salmon.
 *
 * Gated on level === 2 (not < 3) to respect NO_COLOR / FORCE_COLOR=0 â€”
 ...
 *
 * Must run BEFORE the tmux clamp â€” if tmux is running inside a VS Code
...
export function shouldUseRichEightBitDowngradeForLegacyAppleTerminal(
  ...
  const truecolorOverride = /^(?:1|true|yes|on)$/i.test((env.ZED_TUI_TRUECOLOR ?? '').trim())
 ...
 * WITHOUT the bg sequence â€” outer terminal's buffer has bg=default â†’ black on
...
// Computed once at module load â€” terminal/tmux environment doesn't change mid-session.
// Order matters: boost first; then tmux can still clamp RGB to 256.
// Exported for debugging â€” tree-shaken if unused.
