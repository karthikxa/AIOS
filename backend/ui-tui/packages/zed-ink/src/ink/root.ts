...
export type RenderOptions = {
   ...
   * the actual open â€” `child_process.spawn` with an argv array (NOT
   * shell-mode) to the platform's native opener: `open` on macOS,
   * `xdg-open` on Linux/BSD, `explorer.exe` on Windows. Avoid
   * `cmd.exe /c start` â€” `start` is a cmd builtin that reparses the URL
   ...
   * caller side. Zed wires this in `entry.tsx`; library users who
 ...
 * Like react-dom's createRoot â€” call root.render() to mount a tree.
 */
export async function createRoot({
  ...
  // See wrappedRender â€” preserve microtask boundary from the old WASM await.
  await Promise.resolve()
