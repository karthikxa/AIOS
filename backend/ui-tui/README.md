...
React + Ink terminal UI for Zed. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ZED_PYTHON` â†’ `PYTHON` â†’ `$VIRTUAL_ENV/bin/python` â†’ `./.venv/bin/python` â†’ `./venv/bin/python` â†’ `python3` (or `python` on Windows).
...
| `Ctrl+D`                        | Exit                                                                                                                                                    |
| `Cmd/Ctrl+G` / `Alt+G`          | Open `$EDITOR` with the current draft (use `Alt+G` in VSCode/Cursor â€” they bind the primary keystroke to Find Next)                                     |
...
- Input history is stored in `~/.zed/.zed_history` or under `ZED_HOME`.
...
```text
ui-tui/
  packages/zed-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      zed-ink.d.ts    type declarations for @zed/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ZedCLI subprocess for slash commands
```
