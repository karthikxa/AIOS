import type { MouseTrackingMode, ScrollBoxHandle } from '@zed/ink'
...
// `Usage` type â€” no camelCase mapping layer. The `text` already carries its
// own leading glyph (âš  â€¢ âœ• âœ“) from the Python policy, so the renderer only
...
// line â€” `useConfigSync` (validation) and `session.ts` (slash arg
...
// â”€â”€ Billing overlay (Phase 2b: full-modal TUI parity) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// state machine (overview â†’ buy|autoreload|limit â†’ confirm) that performs
 ...
 * billing.ts (single source of truth) â€” the overlay only renders + routes.
 */
export interface BillingOverlayCtx {
  /** Run `billing.auto_reload` (enabled/threshold/top_up) â†’ resolve ok/false. */
