import type { RunExternalProcess } from '@zed/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`zed ${args.join(' ')}\`â€¦`)
  patchUiState({ status: 'setup runningâ€¦' })
  ...
  if (result.error) {
    transcript.sys(`error launching zed: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`zed ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
