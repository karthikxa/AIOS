import { type ScrollBoxHandle, useApp, useHasSelection, useSelection, useStdout, useTerminalTitle } from '@zed/ink'
...
export function useMainApp(gw: GatewayClient) {
  ...
  // mixed markdown â€” we deliberately remount every row so yoga re-measures
  ...
  // Index of the first user-role message â€” separator-rendering in
  ...
  const die = useCallback(() => {
    ...
    // handler in entry.tsx â€” which sends the final resetTerminalModes() â€” never
  ...
  useEffect(() => {
    if (ui.busy) {
      ...
      // Only stamp the idle marker when a turn was actually live â€” busy is
  ...
  // Tab title: `âš ` waiting on approval/sudo/secret/clarify, `â³` busy, `âœ“` idle.
  // Format: `<marker> <session name> Â· <model> Â· <cwd>` â€” name/cwd omitted when absent.
  const model = ui.info?.model?.replace(/^.*\//, '') ?? ''

  const marker = overlay.approval || overlay.sudo || overlay.secret || overlay.clarify ? 'âš ' : ui.busy ? 'â³' : 'âœ“'

  const tabCwd = ui.info?.cwd

  useTerminalTitle(
    model ? composeTabTitle(marker, ui.sessionTitle, model, tabCwd ? shortCwd(tabCwd, 24) : '') : 'Zed'
  )
  ...
  useEffect(() => {
    ...
    // this timer fires. Re-check isSticky() inside the timeout â€” a manual
    ...
    (quiet = false) =>
      rpc<ClipboardPasteResponse>('clipboard.paste', { session_id: getUiState().sid }).then(r => {
        ...
        if (r.attached) {
          ...
          return sys(`ðŸ“Ž Image #${r.count} attached from clipboard${meta ? ` Â· ${meta}` : ''}`)
        }
  ...
  // Drain one queued message whenever the session settles (busy â†’ false):
  ...
  useEffect(() => {
    ...
    if (next) {
      patchUiState({ busy: true, status: 'runningâ€¦' })
      sendQueued(next)
    }
  ...
  useEffect(() => {
    ...
    const exitHandler = () => {
      ...
      // death â€” a user /quit exits Node before this fires, and a replaced child
      ...
      if (plan.recover && plan.sid) {
        ...
        turnController.pushActivity('gateway exited Â· recovering sessionâ€¦', 'warn')
        sys('gateway exited â€” recovering your session (any in-flight reply was lost)')
        gw.start()
      ...
      turnController.pushActivity('gateway exited Â· /logs to inspect', 'error')
  ...
  const closeLiveSession = useCallback(
    async (id: string) => {
      patchUiState({ status: 'closing sessionâ€¦' })
  ...
  // Per-section overrides win over the global mode â€” when every section is
  ...
  const appActions = useMemo(
    () => ({
      ...
      // running, so those stay unguarded â€” that's the orchestrator's purpose.)
  ...
  // Pass current progress through unfrozen â€” streaming update throttling
  ...
  const cwd = ui.info?.cwd || process.env.ZED_CWD || process.cwd()
  ...
  const appStatus = useMemo(
    () => ({
      ...
      voiceLabel: voiceRecording ? 'â— REC' : voiceProcessing ? 'â—‰ STT' : `voice ${voiceEnabled ? 'on' : 'off'}${voiceTts ? ' [tts]' : ''}`
    }),
    [
      cwd,
      gitBranch,
