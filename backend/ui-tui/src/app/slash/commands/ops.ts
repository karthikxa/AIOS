...
export const opsCommands: SlashCommand[] = [
        ...
        .then(
          ctx.guarded<ReloadMcpResponse>(r => {
            ...
            if (r.status === 'reloaded') {
              ctx.transcript.sys(
                params.always
                  ? 'MCP servers reloaded Â· future /reload-mcp will run without confirmation'
  ...
  {
    help: 're-read ~/.zed/.env into the running gateway (CLI parity)',
    name: 'reload',
    run: (_arg, ctx) => {
      ctx.gateway
...

  {
    ...
    run: (arg, ctx) => {
      if (!ctx.sid) {
        return ctx.transcript.sys('no active session â€” nothing to rollback')
      }
              ...
              ctx.transcript.panel('Rollback checkpoints', [
                {
                  rows: checkpoints.map((c, idx) => [
                    ...
                    [c.timestamp, c.message].filter(Boolean).join(' Â· ') || '(no metadata)'
                  ])
                }
              ])
            })
...

  {
    ...
    run: (arg, ctx) => {
      ...
      // Stay compatible with the gateway `/agents [pause|resume|status]` CLI â€”
      ...
      if (sub === 'pause' || sub === 'resume' || sub === 'unpause') {
        ...
        ctx.gateway.gw
          ...
          .then(r => {
            ...
            ctx.transcript.sys(`delegation Â· ${r?.paused ? 'paused' : 'resumed'}`)
          })
          .catch(ctx.guardedErr)
      ...
      if (sub === 'status') {
        ...
        ctx.transcript.sys(
          `delegation Â· ${d.paused ? 'paused' : 'active'} Â· caps d${d.maxSpawnDepth ?? '?'}/${d.maxConcurrentChildren ?? '?'}`
        )
  ...
  {
    help: 'replay a completed spawn tree Â· `/replay [N|last|list|load <path>]`',
    name: 'replay',
    run: (arg, ctx) => {
      ...
      // â”€â”€ Disk-backed listing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      ...
      // â”€â”€ Disk-backed load by path â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      ...
      // â”€â”€ In-memory nav (same-session) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      if (!history.length) {
        return ctx.transcript.sys('no completed spawn trees this session Â· try /replay list')
      }
      ...
      if (raw && lower !== 'last') {
        ...
        if (Number.isNaN(parsed) || parsed < 1 || parsed > history.length) {
          return ctx.transcript.sys(`replay: index out of range 1..${history.length} Â· use /replay list for disk`)
        }
  ...
  {
    help: 'diff two completed spawn trees Â· `/replay-diff <baseline> <candidate>` (indexes from /replay list or history N)',
    name: 'replay-diff',
    run: (arg, ctx) => {
      ...
      if (!baseline || !candidate) {
        return ctx.transcript.sys(`replay-diff: could not resolve indices Â· history has ${history.length} entries`)
      }
...

  {
    ...
    run: (arg, ctx, cmd) => {
      ...
      if (sub === 'browse') {
        ...
        sys('fetching community skills (scans 6 sources, may take ~15s)â€¦')
          ...
          .then(
            ctx.guarded<SkillsBrowseResponse>(r => {
              ...
              const rows: [string, string][] = items.map(s => [
                s.trust ? `${s.name} Â· ${s.trust}` : s.name,
              ...
              panel(`Browse Skills${pageNum > 1 ? ` â€” p${pageNum}` : ''}`, [
                { rows },
                ...(footer.length ? [{ text: footer.join(' Â· ') }] : [])
              ])
            })
          )
          .catch(ctx.guardedErr)
...

  {
    ...
    name: 'plugins',
    run: (arg, ctx, cmd) => {
      // No argument â†’ open the interactive Plugins Hub overlay. Any
      // subcommand (enable/disable/list/install/â€¦) falls through to the
      // text slash worker so it stays at parity with `zed plugins`.
      if (!arg.trim()) {
