import { forceRedraw, type MouseTrackingMode } from '@zed/ink'
...
export const DASHBOARD_EXIT_DISABLED_MESSAGE =
  'exit is disabled in hosted dashboard chat â€” use /new to start a fresh session'

export const DASHBOARD_UPDATE_DISABLED_MESSAGE =
  'update is disabled in hosted dashboard chat â€” the hosted environment is managed separately'
...
export const coreCommands: SlashCommand[] = [
  {
    ...
    run: (_arg, ctx) => {
      ...
      if (ctx.local.catalog?.skillCount) {
        sections.push({ text: `${ctx.local.catalog.skillCount} skill commands available â€” /skills to browse` })
      }
  ...
  {
    help: 'update Zed Agent to the latest version (exits TUI)',
    name: 'update',
    run: (_arg, ctx) => {
      if (DASHBOARD_TUI_MODE) {
      ...
      // Exit code 42 signals the Python wrapper to exec `zed update`.
    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys(
            'clipboard copy failed â€” try ZED_TUI_FORCE_OSC52=1 to force the escape sequence'
          )
        }
      }
      ...
      if (!target) {
        return sys('nothing to copy â€” start a conversation first')
      }
...

  {
    ...
    run: (arg, ctx) => {
      ...
      // that never sees the TUI's turns â€” it only surfaces whatever was
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `Zed #${i + 1}`
        const body = m.text.trim() || (m.tools?.length ? `(${m.tools.length} tool calls)` : '(empty)')
        const clipped = body.length > preview ? `${body.slice(0, preview).trimEnd()}â€¦` : body
...

  {
    ...
    run: (_arg, ctx) => {
      ...
      if (!ctx.sid) {
        return ctx.transcript.sys('no active session â€” nothing to save')
      }
...

  {
    ...
    run: (arg, ctx) => {
      ...
      ctx.transcript.sys(`queued: "${arg.slice(0, 50)}${arg.length > 50 ? 'â€¦' : ''}"`)
    }
  },

  {
    ...
    run: (arg, ctx) => {
      ...
      // message isn't lost â€” identical semantics to the gateway handler.
      if (!ctx.ui.busy || !ctx.sid) {
        ctx.composer.enqueue(payload)
        ctx.transcript.sys(
          `no active turn â€” queued for next: "${payload.slice(0, 50)}${payload.length > 50 ? 'â€¦' : ''}"`
        )
      ...
      ctx.gateway
        ...
        .then(
          ctx.guarded<SessionSteerResponse>(r => {
            if (r?.status === 'queued') {
              ctx.transcript.sys(
                `steer queued â€” arrives after next tool call: "${payload.slice(0, 50)}${payload.length > 50 ? 'â€¦' : ''}"`
              )
            } else {
