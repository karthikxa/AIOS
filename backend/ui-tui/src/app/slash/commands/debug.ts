...
export const debugCommands: SlashCommand[] = [
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see ZED_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      const { heapUsed, rss } = process.memoryUsage()

      ctx.transcript.sys(`writing heap dump (heap ${formatBytes(heapUsed)} Â· rss ${formatBytes(rss)})â€¦`)
