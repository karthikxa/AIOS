...
const statusFromBusy = () => (getUiState().busy ? 'runningâ€¦' : 'ready')
...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: GatewayEvent) => void {
  ...
  // left set until the next turn's idle() silently nulls it â€” so the question
  ...
  turnController.persistSpawnTree = async (subagents, sessionId) => {
    try {
      ...
      const label = top.length ? top.join(' Â· ') : `${subagents.length} subagents`
  ...
  // â”€â”€ Shared full-config read â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  //
  ...
  // â”€â”€ Nudge toward /agents on delegation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  //
  ...
  // ("subagents working Â· /agents to watch live") so the user discovers the
  // spawn-tree dashboard instead of staring at a quiet transcript â€” without
  // hijacking the screen by force-opening an overlay.  Guards:
  //   â€¢ fires at most once per turn (`agentsNudgedThisTurn`)
  //   â€¢ silent if the overlay is already open (nothing to advertise)
  ...
  const maybeNudgeAgents = () => {
    ...
    // Already watching â†’ no point advertising the dashboard.  Don't burn the
    ...
    turnController.pushActivity('subagents working Â· /agents to watch live', 'info')
  }
  ...
  // Terminal statuses are never overwritten by late-arriving live events â€”
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // resumes the session that was live, not a brand-new one. One-shot â€” the
    ...
    if (recoverSidRef && recoverSid) {
      ...
      // After resumeById: it synchronously sets status to 'resumingâ€¦' on entry,
      ...
      patchUiState({ status: 'recovering sessionâ€¦' })

      return
    }

    if (STARTUP_RESUME_ID) {
      patchUiState({ status: 'resumingâ€¦' })
    ...
    // forging a brand-new one.  Mirrors classic CLI's `zed -c` /
    // `zed --tui` muscle memory and addresses the audit's "session
    ...
    getFullConfigOnce()
      .then(cfg => {
        if (!cfg?.config?.display?.tui_auto_resume_recent) {
          patchUiState({ status: 'forging sessionâ€¦' })
          newSession()
          scheduleStartupPrompt()
        ...
        return rpc<SessionMostRecentResponse>('session.most_recent', {}).then(r => {
          ...
          if (target) {
            patchUiState({ status: 'resuming most recentâ€¦' })
            resumeById(target)
            scheduleStartupPrompt()
    ...
    switch (ev.type) {
      ...
      case 'session.info': {
        ...
        patchUiState(state => ({
          ...state,
          info,
          status: state.status === 'starting agentâ€¦' ? 'ready' : state.status,
      ...
      case 'status.update': {
        ...
        if (p.kind === 'goal') {
          ...
          const brief = p.text.startsWith('âœ“')
            ? 'âœ“ goal complete'
            : p.text.startsWith('â†»')
              ? 'â†» goal continuing'
              : p.text.startsWith('â¸')
      ...
      case 'notification.clear':
        // Key-matched clear only â€” a stale/late clear must not wipe a newer
      ...
      case 'billing.step_up.verification': {
        ...
        // 120s timeout), so the link â€” not the RPC result â€” is the source of truth.
        ...
        sys('ðŸ’³ Open this link to grant terminal billing access:')
        sys(url)
        if (code) {
      ...
      case 'voice.transcript': {
        ...
        // the transcript to the agent as its next turn â€” draft handling
      ...
      case 'gateway.start_timeout': {
        ...
        const trace = python || cwd ? ` Â· ${String(python || '')} ${String(cwd || '')}`.trim() : ''

        setStatus('gateway startup timeout')
        turnController.pushActivity(`gateway startup timed out${trace} Â· /logs to inspect`, 'error')
      ...
      case 'gateway.protocol_error':
        ...
        if (!turnController.protocolWarned) {
          ...
          turnController.pushActivity('protocol noise detected Â· /logs to inspect', 'info')
        }
      ...
      case 'tool.generating':
        if (ev.payload?.name) {
          turnController.pushTrail(`drafting ${ev.payload.name}â€¦`)
        }
      ...
      case 'review.summary': {
        ...
        // flash. Python-side already formats it as "ðŸ’¾ Self-improvement
        // review: â€¦".
        const text = String(ev.payload?.text ?? '').trim()
      ...
      case 'subagent.spawn_requested':
        ...
        // First sign of delegation this turn â†’ nudge toward /agents.
        maybeNudgeAgents()
      ...
      case 'subagent.start':
        ...
        // CLIâ†’gateway path), so nudge here too.  Once-per-turn guarded, so
