import { forceRedraw, useInput } from '@zed/ink'
 ...
 * overlays was up, which silently disabled transcript scrolling â€” the user
 ...
 *
 * Modifier-held wheel (precision mode) is included â€” a user who wants to
...
export function useInputHandlers(ctx: InputHandlerContext): InputHandlerResult {
  ...
  // rows = wheelStep Ã— accelMult. State mutates in place across renders.
  ...
  const voiceRecordToggle = () => {
    if (!voice.enabled) {
      return actions.sys('voice: mode is off â€” enable with /voice on')
    }

    const starting = !voice.recording
    const action = starting ? 'start' : 'stop'

    // Optimistic UI â€” flip the REC badge immediately so the user gets
  ...
  useInput((ch, key) => {
    ...
    if (isBlocked) {
      ...
      // intercept Ctrl+C here so the user can deny/dismiss â€” all other keys
      // fall through to the component-level handlers.
      //
      // Scroll inputs (wheel / PageUp / PageDown / Shift+â†‘â†“) are special:
      ...
      if (overlay.pager) {
        ...
        if (key.return || ch === ' ' || key.pageDown) {
          patchOverlayState(prev => {
            ...
            // Auto-close only when already at the last page â€” otherwise clamp
            // to `max` so the offset matches what the line/page-back handlers
            // can reach (prevents a snap-back jump on the next â†‘/â†“/PgUp).
            return offset >= max
              ? { ...prev, pager: null }
