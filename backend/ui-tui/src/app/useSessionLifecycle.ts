...
import { evictInkCaches } from '@zed/ink'
...
const statusFromLiveSession = (status?: string, running = false) => {
  ...
  if (status === 'starting') {
    return 'starting agentâ€¦'
  }

  return running || status === 'working' ? 'runningâ€¦' : 'ready'
}

export const writeActiveSessionFile = (sessionId: null | string, file = process.env.ZED_TUI_ACTIVE_SESSION_FILE) => {
  if (!file || !sessionId) {
    return
  }
...
export function useSessionLifecycle(opts: UseSessionLifecycleOptions) {
  ...
  const startNewSession = useCallback(
    async (msg?: string, title?: string, keepCurrent = false) => {
      ...
      patchUiState({
        info,
        sid: r.session_id,
        status: info?.version ? 'ready' : 'starting agentâ€¦',
  ...
  const activateLiveSession = useCallback(
    (id: string) => {
      patchOverlayState({ sessions: false })
      patchUiState({ status: 'switching sessionâ€¦' })
