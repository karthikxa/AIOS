...
import { useStdin, withInkSuspended } from '@zed/ink'
 ...
 * validation. Keep in sync with _detect_file_drop() in cli.py â€” see that
...
export function looksLikeDroppedPath(text: string): boolean {
  ...
  // Bare absolute paths (start with /) â€” require a second '/' or a '.' to avoid
...
export function useComposerState({
  ...
  const openEditor = useCallback(async () => {
    const dir = mkdtempSync(join(tmpdir(), 'zed-'))
