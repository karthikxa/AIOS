 ...
 *
 * Symptom: in `zed --tui`, typing into the composer until the input
 ...
 * disagreed on many real inputs â€” wrap-ansi would keep "branch
 * investigate" on one row while cursorLayout claimed it had wrapped,
 * etc. â€” so the declared cursor position drifted from where the text
...
const USER_REPORT_MESSAGE =
  ...
  'im in cursor terminal using zed --tui and as i type multiline my caret at the end will often ' +
...
describe('cursor-drift regression â€” composer cursorLayout matches Ink rendering', () => {
  ...
  it('keeps cursor on the same row when text exactly fills the terminal width', () => {
    ...
    // The previous algorithm did â€” that's what produced the visible
  ...
  it('agrees with wrap-ansi for word-wrap that pushes a word onto the next line', () => {
    ...
    // The cursor at end-of-text must land at line=1, col=5 â€” where Ink
