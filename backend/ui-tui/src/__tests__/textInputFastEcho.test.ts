...
//
// Regression intent: any non-ASCII text â€” Vietnamese precomposed letters
...
// that could be produced by an in-flight IME composition â€” must NOT
...
describe('canFastAppendShape', () => {
  ...
  it('rejects Vietnamese precomposed letter á» (U+1EC1) â€” IME composition path', () => {
    // 'á»' is one grapheme, length 1, width 1, but Vietnamese Telex/IME
    ...
    // arrives.
    expect(canFastAppendShape('hello', 5, 'á»', COLS, 5)).toBe(false)
  })

  it('rejects Vietnamese tone marks Äƒ, Æ¡, Æ° (Latin-Extended-A/B)', () => {
    for (const ch of ['Äƒ', 'áº¯', 'Æ¡', 'á»', 'Æ°', 'á»±']) {
  ...
  it('rejects NFD combining marks (U+0300 grave, U+0301 acute, U+0302 circumflex)', () => {
    ...
    // = 'á»'. Each combining mark is zero-width but length 1; without the
  ...
  it('rejects CJK (East-Asian wide) characters', () => {
    expect(canFastAppendShape('hello', 5, 'ä½ ', COLS, 5)).toBe(false)
    expect(canFastAppendShape('hello', 5, 'æ—¥æœ¬', COLS, 5)).toBe(false)
  })

  it('rejects emoji', () => {
    expect(canFastAppendShape('hello', 5, 'ðŸ™‚', COLS, 5)).toBe(false)
  })
  ...
  it('rejects NBSP and Latin-1 letters that would change the line shape', () => {
    expect(canFastAppendShape('hello', 5, '\u00a0', COLS, 5)).toBe(false)
    expect(canFastAppendShape('hello', 5, 'Ã©', COLS, 5)).toBe(false)
    expect(canFastAppendShape('hello', 5, 'Ã±', COLS, 5)).toBe(false)
  })
})
...
describe('canFastBackspaceShape', () => {
  ...
  it('rejects deleting Vietnamese precomposed letter á»', () => {
    ...
    expect(canFastBackspaceShape('helloá»', 'helloá»'.length)).toBe(false)
  })

  it('rejects deleting a CJK character (2 cells)', () => {
    expect(canFastBackspaceShape('hiä½ ', 'hiä½ '.length)).toBe(false)
  })

  it('rejects deleting a NFD-composed grapheme with combining marks', () => {
    // 'e' + U+0302 (circumflex) + U+0300 (grave) â€” final grapheme is one
  ...
  it('rejects deleting an emoji', () => {
    expect(canFastBackspaceShape('hiðŸ™‚', 'hiðŸ™‚'.length)).toBe(false)
  })
  ...
  // moves up to the end of the previous visual line â€” desyncing both
  ...
  it('rejects fast-backspace at a soft-wrap boundary when columns is known', () => {
    // value width 6 in a column of 6 â†’ cursorLayout produces (line 1, col 0)
  ...
  it('rejects fast-backspace at an exact multiple of columns (wide wrap)', () => {
    // 12 chars at width 6 â†’ two full visual rows, caret at (line 2, col 0).
    const value = 'abcdefghijkl'
    expect(canFastBackspaceShape(value, value.length, 6)).toBe(false)
  })

  it('still accepts fast-backspace inside a wrapped line', () => {
    // Caret mid-visual-line â€” "\b \b" can move the cursor one cell left
    // without crossing a wrap boundary.
    expect(canFastBackspaceShape('hello world', 'hello world'.length, 20)).toBe(true)
    expect(canFastBackspaceShape('abcdefghi', 9, 6)).toBe(true) // visual line 1, col 3 â†’ ok
  })
  ...
  it('skips the wrap-boundary check when columns is omitted (legacy contract)', () => {
    ...
    // behavior â€” the function does NOT magically reject anything that
