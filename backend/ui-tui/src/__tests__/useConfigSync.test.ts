...
describe('normalizeBusyInputMode', () => {
  ...
  it('defaults to queue for missing/unknown values (TUI-only override)', () => {
    ...
    // (see zed_cli/config.py + tui_gateway/server.py::_load_busy_input_mode);
...
describe('applyDisplay â†’ busy_input_mode', () => {
  beforeEach(() => {
    resetUiState()
  })
...
describe('applyDisplay â†’ tui_status_indicator', () => {
  beforeEach(() => {
    resetUiState()
  })
...
describe('applyDisplay â†’ voice.record_key (#18994)', () => {
  beforeEach(() => {
    resetUiState()
  })
  ...
  it('does not reset voiceRecordKey when cfg is null (transient RPC failure)', () => {
    ...
    // runs â€” we specifically only skip voiceRecordKey.
    expect(setBell).toHaveBeenCalledWith(false)
  })
})
...
describe('hydrateFullConfig', () => {
  ...
  it('leaves cached voiceRecordKey untouched when the RPC fails', async () => {
    ...
    expect(setVoiceRecordKey).not.toHaveBeenCalled()
    // bell setter still fires â€” applyDisplay's null-cfg path applies
  ...
  it('threads through without a voice setter (back-compat call sites)', async () => {
    ...
    // No third arg â€” applyDisplay must not throw and must still apply
