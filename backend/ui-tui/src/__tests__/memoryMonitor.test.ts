...
// memory.js performs real heap dumps / fs work â€” stub it so the monitor's
// dump path is a no-op in tests.
vi.mock('../lib/memory.js', () => ({
  performHeapDump: vi.fn(async () => null)
}))

// @zed/ink is dynamically imported only on the dump path; stub the eviction.
vi.mock('@zed/ink', () => ({ evictInkCaches: vi.fn() }))
...
describe('startMemoryMonitor thresholds (#34095)', () => {
  ...
  it('fires onWarn once on fast sub-threshold heap growth, then re-arms', async () => {
    ...
    // below `high` â€” the silent-death regime.
    ...
    await vi.advanceTimersByTimeAsync(2) // jumped 700MB â†’ above floor + steep
    ...
    // Falls back below the floor â†’ re-armed, then climbs again â†’ fires again.
  ...
  it('does not warn on slow growth below the steep-growth step', async () => {
    ...
    await vi.advanceTimersByTimeAsync(2)
    // +50MB per tick â€” above the floor but gentle, not a render-tree blowup.
