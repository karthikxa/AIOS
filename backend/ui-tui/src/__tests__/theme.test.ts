...
// ZED_TUI_THEME=light (or ZED_TUI_BACKGROUND set to something
...
// statically â€” but we stay consistent and dynamic-import it too.
const RELEVANT_ENV = [
  'ZED_TUI_LIGHT',
  'ZED_TUI_THEME',
  'ZED_TUI_BACKGROUND',
...
describe('DEFAULT_THEME', () => {
  it('has brand defaults', async () => {
    ...
    expect(DEFAULT_THEME.brand.name).toBe('Zed Agent')
    expect(DEFAULT_THEME.brand.prompt).toBe('â¯')
    expect(DEFAULT_THEME.brand.tool).toBe('â”Š')
  })
...
describe('detectLightMode', () => {
  ...
  it('honors ZED_TUI_LIGHT on/off', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ ZED_TUI_LIGHT: '1' })).toBe(true)
    expect(detectLightMode({ ZED_TUI_LIGHT: 'true' })).toBe(true)
    expect(detectLightMode({ ZED_TUI_LIGHT: 'on' })).toBe(true)
    expect(detectLightMode({ ZED_TUI_LIGHT: '0' })).toBe(false)
    expect(detectLightMode({ ZED_TUI_LIGHT: 'off' })).toBe(false)
  })
  ...
  it('lets ZED_TUI_LIGHT=0 override a light COLORFGBG', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ COLORFGBG: '0;15', ZED_TUI_LIGHT: '0' })).toBe(false)
  })

  it('honors ZED_TUI_THEME=light/dark as a symmetric explicit override', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ ZED_TUI_THEME: 'light' })).toBe(true)
    expect(detectLightMode({ ZED_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '0;15', ZED_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '15;0', ZED_TUI_THEME: 'light' })).toBe(true)
  })

  it('uses ZED_TUI_BACKGROUND luminance when COLORFGBG is missing', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ ZED_TUI_BACKGROUND: '#ffffff' })).toBe(true)
    expect(detectLightMode({ ZED_TUI_BACKGROUND: '#000000' })).toBe(false)
    expect(detectLightMode({ ZED_TUI_BACKGROUND: '#1e1e1e' })).toBe(false)
    // Three-char hex normalises like CSS.
    expect(detectLightMode({ ZED_TUI_BACKGROUND: '#fff' })).toBe(true)
    // Garbage falls through to the default-dark path.
    expect(detectLightMode({ ZED_TUI_BACKGROUND: 'not-a-colour' })).toBe(false)
  })

  it('rejects partially-invalid hex instead of silently truncating', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()
    // `parseInt('fffgff'.slice(2,4), 16)` would return 15 â€” the strict
    // regex must reject these inputs so they fall through to default-
    // dark instead of producing a false-positive light reading.
    expect(detectLightMode({ ZED_TUI_BACKGROUND: '#fffgff' })).toBe(false)
    expect(detectLightMode({ ZED_TUI_BACKGROUND: 'ffggff' })).toBe(false)
    expect(detectLightMode({ ZED_TUI_BACKGROUND: '#xyz' })).toBe(false)
    // Wrong length also rejected (no implicit padding/truncation).
    expect(detectLightMode({ ZED_TUI_BACKGROUND: '#fffff' })).toBe(false)
    expect(detectLightMode({ ZED_TUI_BACKGROUND: '#fffffff' })).toBe(false)
  })
...
describe('fromSkin', () => {
  ...
  // must dynamic-import it after sterilizing env â€” otherwise an ambient
  // ZED_TUI_THEME=light would flip the base palette and make these
  ...
  it('normalizes skin prompt symbols to trimmed single-line text', async () => {
    ...
    expect(fromSkin({}, { prompt_symbol: ' âš” â¯ \n' }).brand.prompt).toBe('âš” â¯')
    expect(fromSkin({}, { prompt_symbol: ' Î¨ > \n' }).brand.prompt).toBe('Î¨ >')
