...
describe('cursorLayout â€” word-wrap parity with wrap-ansi', () => {
  ...
  it('does not push exact-fill text onto a phantom next line', () => {
    ...
    // it on the same row (no soft-wrap), so the cursor must too â€” otherwise
...
describe('input metrics helpers', () => {
  ...
  it('counts the prompt gap as its own cell', () => {
    ...
    expect(composerPromptWidth('â¯')).toBe(2)
    expect(composerPromptWidth('Î¨ >')).toBe(4)
  })
...
describe('offsetFromPosition â€” word-wrap inverse of cursorLayout', () => {
