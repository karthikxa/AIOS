...
describe('createGatewayEventHandler', () => {
  ...
  it('prints compaction progress status into the transcript', () => {
    ...
    onEvent({
      payload: { kind: 'compressing', text: 'compressing 968 messages (~123,400 tok)â€¦' },
      type: 'status.update'
    } as any)

    expect(ctx.system.sys).toHaveBeenCalledWith('compressing 968 messages (~123,400 tok)â€¦')
  })
  ...
  it('keeps goal verdict text in transcript but shows a brief idle status (#goal statusbar)', () => {
    ...
    const ctx = buildCtx(appended)
    const onEvent = createGatewayEventHandler(ctx)
    const verdict = 'âœ“ Goal achieved: long judge reason goes only in transcript, not merged with cwd label.'
    ...
    try {
      ...
      expect(getUiState().status).toBe('âœ“ goal complete')
  ...
  it('maps goal status.update prefixes to short status strings', () => {
    ...
    onEvent({
      payload: { kind: 'goal', text: 'â†» Continuing toward goal (1/10): reason' },
      type: 'status.update'
    } as any)
    expect(getUiState().status).toBe('â†» goal continuing')

    onEvent({
      payload: { kind: 'goal', text: 'â¸ Goal paused â€” budget exhausted.' },
      type: 'status.update'
    } as any)
    expect(getUiState().status).toBe('â¸ goal paused')
  })
  ...
  it('surfaces self-improvement review summaries as a persistent system line', () => {
    ...
    onEvent({
      payload: { text: "ðŸ’¾ Self-improvement review: Skill 'zed-release' patched" },
      type: 'review.summary'
    } as any)

    expect(ctx.system.sys).toHaveBeenCalledWith(
      "ðŸ’¾ Self-improvement review: Skill 'zed-release' patched"
    )
  })
  ...
  it('filters spinner/status-only reasoning noise from completed thinking', () => {
    const appended: Msg[] = []
    const streamed = '(Â¬_Â¬) synthesizing...\nactual plan\n( Í¡Â° ÍœÊ– Í¡Â°) pondering...\nnext step'
  ...
  it('annotates gateway.start_timeout with stderr tail lines so users can diagnose without /logs', () => {
    ...
    onEvent({
      payload: {
        ...
        stderr_tail:
          '[startup] timed out\nModuleNotFoundError: No module named openai\nFileNotFoundError: ~/.zed/config.yaml'
      },
  ...
  it('anchors inline_diff as its own segment where the edit happened', () => {
    ...
    // Narration â†’ tool â†’ tool-complete â†’ more narration â†’ message-complete.
    ...
    expect(turnController.segmentMessages).toEqual([
      { role: 'assistant', text: 'Editing the file' },
      {
        ...
        role: 'assistant',
        text: block,
        tools: [expect.stringMatching(/^Patch\("foo\.ts"\)(?: \([^)]+\))? âœ“$/)]
      }
    ])
  ...
  it('drops the diff segment when the final assistant text narrates the same diff', () => {
    ...
    // Only the final message â€” diff-only segment dropped so we don't
  ...
  it('strips the CLI "â”Š review diff" header from inline diff segments', () => {
    const appended: Msg[] = []
    const onEvent = createGatewayEventHandler(buildCtx(appended))
    const raw = '  \u001b[33mâ”Š review diff\u001b[0m\n--- a/foo.ts\n+++ b/foo.ts\n@@\n-old\n+new'
    ...
    expect(appended[0]?.text).not.toContain('â”Š review diff')
  ...
  it('shows setup panel for missing provider startup error', () => {
    ...
    onEvent({
      payload: {
        message:
          'agent init failed: No LLM provider configured. Run `zed model` to select a provider, or run `zed setup` for first-time configuration.'
      },
      type: 'error'
    } as any)
  ...
  it('on gateway.ready after a crash, resumes the recovered session once and skips forge', async () => {
    ...
    // "recovering sessionâ€¦" label is applied *after* (and survives) it.
    ctx.session.resumeById = resumeById.mockImplementation(() => patchUiState({ status: 'resumingâ€¦' }))
    ctx.session.STARTUP_RESUME_ID = ''
    ...
    expect(getUiState().status).toBe('recovering sessionâ€¦')
  })
  ...
  it('keeps gateway noise informational and approval out of Activity', async () => {
    ...
    expect(getTurnState().activity).toMatchObject([
      ...
      { text: 'protocol noise detected Â· /logs to inspect', tone: 'info' },
  ...
  it('nudges toward /agents on subagent.start (spawn_requested dropped in CLI path)', () => {
    ...
    // In the real CLIâ†’gateway path the delegate callback drops
  ...
  it('nudges at most once per turn and resets on the next message.start', () => {
    ...
    // Multiple spawns in one turn â†’ a single hint.
    onEvent({
      payload: { goal: 'child a', subagent_id: 'sa-a', task_index: 0 },
      type: 'subagent.start'
    } as any)
    ...
    // New turn clears activity AND the once-per-turn guard â†’ nudges again.
    onEvent({ payload: {}, type: 'message.start' } as any)
    onEvent({
  ...
  it('does not nudge when the /agents overlay is already open', () => {
    ...
    // User already has the dashboard open â†’ nothing to advertise.
  ...
  it('nudges if the /agents overlay is closed mid-turn while delegation continues', () => {
    ...
    // Overlay open on the first delegation event â†’ suppressed, but the
    ...
    // User closes the dashboard mid-turn â†’ the next delegation event nudges.
  ...
  it('does not nudge when display.tui_agents_nudge is false', async () => {
    const appended: Msg[] = []
    const ctx = buildCtx(appended)
    // config.get â†’ full returns the disable flag.
    ctx.gateway.rpc = vi.fn(async (method: string) =>
    ...
    // spawn (mirrors real usage â€” config lands well before first delegation).
    onEvent({ payload: {}, type: 'gateway.ready' } as any)
    await Promise.resolve()
    await Promise.resolve()
  ...
  it('drops stale reasoning/tool/todos events after ctrl-c until the next message starts', () => {
    ...
    // its cooldown â€” without flushing it inside this test, the timeout
    ...
    try {
      ...
      onEvent({ payload: { text: 'still thinkingâ€¦' }, type: 'reasoning.delta' } as any)
      // Post-interrupt tool.start with a todos payload â€” must NOT mutate todos.
      onEvent({
        payload: {
          context: 'post',
          name: 'browser',
      ...
      // Late tool.generating must NOT push a 'drafting â€¦' line into the trail.
      ...
      // (This test does not assert that pre-interrupt todos are cleared â€”
  ...
  it('keepBusy interrupt holds busy until the gateway settles and suppresses the cancelled turnâ€™s final_response', () => {
    // Force-send: interrupt holds busy so the drain waits for the real settle
    // instead of racing it (the race duplicated the bubble, leaked a "queued: â€¦"
    // note, and surfaced the cancelled turn's "Operation interruptedâ€¦" reply).
    const appended: Msg[] = []
    const ctx = buildCtx(appended)
    ctx.gateway.gw.request = vi.fn(async () => ({ status: 'interrupted' }))
    const onEvent = createGatewayEventHandler(ctx)
    ...
    patchUiState({ sid: 'sess-1' })
    onEvent({ payload: {}, type: 'message.start' } as any)
    onEvent({ payload: { text: 'thinkingâ€¦' }, type: 'reasoning.delta' } as any)
    ...
    // Held busy: the drain effect keys off busyâ†’false, so it must not fire yet.
    expect(getUiState().busy).toBe(true)
    ...
    // "Operation interruptedâ€¦" line is suppressed (not appended).
    expect(getUiState().busy).toBe(false)
  ...
  it('persists an abandoned (timed-out) clarify into the transcript when the clarify tool completes', () => {
    ...
    expect(record?.text).toContain('timed out â€” no selection')
  ...
  it('does not flush the clarify overlay when a non-clarify tool completes', () => {
    ...
    // A clarify is live, but it's a *different* tool that just completed â€” the
  ...
  // â”€â”€ Credits notice (Strategy B) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  describe('credits notice', () => {
    it('shows a notice immediately when idle (no turn in flight)', () => {
      const onEvent = createGatewayEventHandler(buildCtx([]))

      onEvent({
        payload: { key: 'credits.depleted', kind: 'sticky', level: 'error', text: 'âœ• credits exhausted' },
    ...
    it('holds a notice arriving mid-turn (busy) and flushes it at message.complete', () => {
      ...
      onEvent({
        payload: { key: 'credits.90', kind: 'sticky', level: 'warn', text: 'âš  90% used' },
        type: 'notification.show'
      } as any)
      ...
      expect(getUiState().notice).toMatchObject({ key: 'credits.90', text: 'âš  90% used' })
    })
    ...
    it('flushes a held notice at interruptTurn (turn-end via ctrl-c)', () => {
      ...
      try {
        ...
        onEvent({ payload: {}, type: 'message.start' } as any)
        onEvent({
          payload: { key: 'credits.depleted', kind: 'sticky', level: 'error', text: 'âœ• out' },
          type: 'notification.show'
        } as any)
        expect(getUiState().notice).toBeNull()
        ...
        expect(getUiState().notice).toMatchObject({ key: 'credits.depleted', text: 'âœ• out' })
      } finally {
        vi.runAllTimers()
        vi.useRealTimers()
      }
    ...
    it('latest-wins: a second mid-turn notice replaces the first held one', () => {
      ...
      onEvent({
        payload: { key: 'credits.depleted', kind: 'sticky', level: 'error', text: 'âœ• exhausted' },
        type: 'notification.show'
      } as any)
      ...
      expect(getUiState().notice).toMatchObject({ key: 'credits.depleted', text: 'âœ• exhausted' })
    })
    ...
    it('clears a visible notice only when the clear key matches (no-op otherwise)', () => {
      ...
      onEvent({
        payload: { key: 'credits.grant_spent', kind: 'sticky', level: 'warn', text: 'âš  grant spent' },
        type: 'notification.show'
      } as any)
    ...
    it('drops a held pending notice on a matching clear before it can surface', () => {
      ...
      // Nothing surfaces â€” the pending notice was dropped by the matching clear.
      expect(getUiState().notice).toBeNull()
    })
    ...
    it('a ttl notice self-expires after ttl_ms when applied while idle', () => {
      ...
      try {
        ...
        onEvent({
          payload: { key: 'credits.restored', kind: 'ttl', level: 'success', text: 'âœ“ access restored', ttl_ms: 8000 },
          type: 'notification.show'
        } as any)
    ...
    it('R3-C2: a ttl notice self-expires even when statusTimer is also armed (timer isolation)', () => {
      ...
      try {
        ...
        // 1. While idle, show a ttl notice â†’ applies immediately, arms noticeTimer.
        onEvent({
          payload: { key: 'credits.restored', kind: 'ttl', level: 'success', text: 'âœ“ restored', ttl_ms: 8000 },
          type: 'notification.show'
        } as any)
        expect(getUiState().notice).toMatchObject({ key: 'credits.restored' })
        ...
        onEvent({
          payload: { kind: 'goal', text: 'âœ“ Goal achieved: some reason' },
          type: 'status.update'
        } as any)
        // statusTimer is now live; notice must still be visible.
        expect(getUiState().notice).toMatchObject({ key: 'credits.restored' })

        // 3. Advance past the TTL â€” the notice's own dedicated timer fires.
    ...
    it('latest-wins cancels a prior ttl timer so it cannot wipe the newer notice', () => {
      ...
      try {
        ...
        onEvent({
          payload: { id: 'a', key: 'credits.restored', kind: 'ttl', level: 'success', text: 'âœ“ a', ttl_ms: 5000 },
          type: 'notification.show'
        } as any)
        ...
        onEvent({
          payload: { id: 'b', key: 'credits.depleted', kind: 'sticky', level: 'error', text: 'âœ• b' },
          type: 'notification.show'
        } as any)
        ...
        expect(getUiState().notice).toMatchObject({ id: 'b', text: 'âœ• b' })
      } finally {
        vi.useRealTimers()
      }
    })
    ...
    it('reset()/fullReset() clears pending + timer + visible notice (no cross-session leak)', () => {
      ...
      try {
        ...
        onEvent({
          payload: { key: 'credits.depleted', kind: 'sticky', level: 'error', text: 'âœ• A cut' },
          type: 'notification.show'
        } as any)
        onEvent({ payload: {}, type: 'message.start' } as any)
        onEvent({
          payload: { key: 'credits.90', kind: 'sticky', level: 'warn', text: 'âš  A 90%' },
          type: 'notification.show'
        } as any)
        expect(getUiState().notice).toMatchObject({ key: 'credits.depleted' })
        ...
        // Session B: a turn ends with nothing held â€” A's notice must not bleed in.
        onEvent({ payload: {}, type: 'message.start' } as any)
