...
// ~/.zed. To exercise the real persistence path we clear that gate, point
// ZED_HOME at a temp dir, and re-import the module fresh (path + enabled
// flag are captured at module load).
const loadFresh = async (home: string) => {
  vi.resetModules()
  vi.stubEnv('VITEST', '')
  vi.stubEnv('ZED_HOME', home)
...
describe('recordParentLifecycle', () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), 'zed-parentlog-'))
  })
  ...
  it('appends a timestamped breadcrumb to logs/tui_gateway_crash.log', async () => {
    ...
    recordParentLifecycle('graceful-exit received signal=SIGHUP â†’ killing gateway')

    const contents = readFileSync(join(home, 'logs', 'tui_gateway_crash.log'), 'utf8')

    expect(contents).toContain('[tui-parent]')
    expect(contents).toContain('graceful-exit received signal=SIGHUP â†’ killing gateway')
  ...
  it('collapses embedded newlines so a value stays one breadcrumb', async () => {
    ...
    expect(lines[0]).toContain('boom â†µ   at foo() â†µ   at bar()')
  })
