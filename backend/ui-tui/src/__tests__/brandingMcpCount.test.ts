...
import { renderSync } from '@zed/ink'
...
// zed_cli/banner.py) and the "connected" label on the MCP collapse toggle.
//
...
describe('branding MCP headline count', () => {
  it('counts only connected servers, not configured-but-disabled ones', async () => {
    ...
    // One connected server â†’ "1 MCP", never "2 MCP".
  ...
  it('drops the MCP segment entirely when no server is connected', async () => {
    ...
    // Matches the classic CLI, which only appends "Â· N MCP" when N > 0.
