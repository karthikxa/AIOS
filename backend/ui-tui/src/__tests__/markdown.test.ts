...
import { Box, renderSync } from '@zed/ink'
...
describe('INLINE_RE emphasis', () => {
  ...
  it('keeps intraword underscores literal', () => {
    const path = '/home/me/.zed/cache/screenshots/browser_screenshot_ecc1c3feab.png'
...
describe('protocol sentinels', () => {
  it('captures MEDIA: paths with surrounding quotes or backticks', () => {
    ...
    expect('  MEDIA: /home/me/.zed/cache/screenshots/browser_screenshot_ecc.png  '.match(MEDIA_LINE_RE)?.[1]).toBe(
      '/home/me/.zed/cache/screenshots/browser_screenshot_ecc.png'
    )
    expect('`MEDIA:/tmp/a.png`'.match(MEDIA_LINE_RE)?.[1]).toBe('/tmp/a.png')
...
describe('Md wrapping', () => {
  ...
  it('keeps nested list and quote indentation out of trim-sensitive text', () => {
    ...
    expect(lines).toContain('  â€¢ nested bullet')
    expect(lines).toContain('  â”‚ nested quote')
  })
...
describe('renderTable CJK width alignment', () => {
  it('column starts share the same display offset across CJK rows', async () => {
    const { stringWidth } = await import('@zed/ink')

    const md = [
      '| é…ç½® | Config | çŠ¶æ€ |',
      '|------|--------|------|',
      '| Vicuna (report) | dense | Ã— |',
      '| ChatGLM | chat | âœ“ |',
      '| é€šä¹‰åƒé—® | qwen | Ã— |'
    ].join('\n')
