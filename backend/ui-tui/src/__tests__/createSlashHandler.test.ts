...
// DASHBOARD_TUI_MODE resolves once at module load from ZED_TUI_DASHBOARD,
...
describe('createSlashHandler', () => {
  ...
  it('routes /status to live session.status instead of slash worker', async () => {
    ...
    const rpc = vi.fn(() => Promise.resolve({ output: 'Zed TUI Status' }))
    ...
    expect(ctx.gateway.gw.request).not.toHaveBeenCalled()
    await vi.waitFor(() => {
      expect(ctx.transcript.page).toHaveBeenCalledWith('Zed TUI Status', 'Status')
    })
  })
  ...
  it('keeps visible scrollback when branching a TUI session', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(ctx.transcript.sys).toHaveBeenCalledWith('branched â†’ branch title')
    })
  ...
  it('falls through to command.dispatch for skill commands and sends the message', async () => {
      ...
      gateway: {
        gw: {
          ...
          request: vi.fn((method: string) => {
            ...
            if (method === 'command.dispatch') {
              return Promise.resolve({ type: 'skill', message: skillMessage, name: 'zed-agent-dev' })
            }
    ...
    const h = createSlashHandler(ctx)
    expect(h('/zed-agent-dev')).toBe(true)
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('âš¡ loading skill: zed-agent-dev')
    })
  ...
  it('handles command.dispatch payloads returned directly by slash.exec', async () => {
          ...
          request: vi.fn((method: string) => {
            if (method === 'slash.exec') {
              return Promise.resolve({
                ...
                notice: 'âŠ™ Goal set (20-turn budget): complete all the steps and provide a final report',
                type: 'send'
              })
            }
    ...
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith(
        'âŠ™ Goal set (20-turn budget): complete all the steps and provide a final report'
      )
    })
  ...
  it('/save forwards to session.save RPC and reports the returned file', async () => {
    ...
    const rpc = vi.fn(() => Promise.resolve({ file: '/tmp/zed_conversation_test.json' }))
    ...
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('conversation saved to: /tmp/zed_conversation_test.json')
    })
  })
  ...
  it('/save without an active session tells the user instead of hitting the RPC', () => {
    ...
    expect(rpc).not.toHaveBeenCalled()
    expect(ctx.transcript.sys).toHaveBeenCalledWith('no active session â€” nothing to save')
  })
  ...
  it('/rollback without an active session tells the user instead of hitting the RPC', () => {
    ...
    expect(rpc).not.toHaveBeenCalled()
    expect(ctx.transcript.sys).toHaveBeenCalledWith('no active session â€” nothing to rollback')
  })
