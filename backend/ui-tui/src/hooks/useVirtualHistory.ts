import type { ScrollBoxHandle } from '@zed/ink'
import {
  type RefObject,
  useCallback,
  useDeferredValue,
...
// edge â†’ smaller fiber tree â†’ less buffer-compose work per frame.  HN/CC
...
// rows/item, so 120 leaves >4Ã— headroom and never blanks the viewport
// even when items are tiny.
const MAX_MOUNTED = 120
const COLD_START = 30
// Floor on unmeasured row height used when computing coverage â€” guarantees
...
// Half of OVERSCAN keeps â‰¥20 rows of cushion before the mounted range
...
// PESSIMISTIC=1 coverage â€” each row running marked lexer + syntax
...
// from 25 â†’ 12: each new item adds ~100 fibers / Yoga nodes, and a
...
export function useVirtualHistory(
  ...
  // Ref (not state) â€” checked during render phase, zero extra commits.
  ...
  // a fresh Array(n+1) every render â€” at n=10k that's ~80KB/render of GC
  ...
  // a fresh marked.lexer + syntax highlight â‰ˆ 3ms). Freeze the mount range
  ...
  // key â†’ React.Object.is short-circuits the commit entirely. The key includes
  ...
  // (/clear, compaction) â€” clamping would collapse to an empty mount and
  ...
  if (frozenRange) {
    ...
    if (vp <= 0) {
      ...
      // covered. Claude-code caps the span at 3Ã—viewport so pendingDelta
      ...
      // Binary search â€” offsets is monotone. Linear walk was O(n) at n=10k+,
  ...
  // Coverage guarantee: ensure sum(real or pessimistic heights) â‰¥
  // viewportH + 2*overscan so the viewport is physically covered even when
  // items are tiny. Pessimistic because uncached items use a floor of 1 â€”
  ...
  // 2Ã—viewport â€” key-repeat PageUp moves ~viewport/2 per press). Covers
  ...
  // deferred range lagging briefly. Only deferral range GROWTH â€” shrinking
  ...
  // not bottomSpacer) â€” skip deferral.
  if (effStart > effEnd || sticky) {
    effStart = start
    effEnd = end
  }

  // Scrolling DOWN â€” bypass effEnd deferral so the tail mounts immediately.
  ...
  // (not pendingDelta direction â€” that flips mid-settle under concurrent
  ...
  const measureRef = useCallback((key: string) => {
    ...
    if (!fn) {
      fn = (el: unknown) => {
        ...
        // calls ref(null) before removeChild â†’ freeRecursive), so we grab
  ...
  useLayoutEffect(() => {
    ...
    // would drain past the deferred children's span â†’ viewport lands in
    // spacer â†’ white flash.
    if (s && shouldSetVirtualClamp({ itemCount: n, liveTailActive, sticky, viewportHeight: vp })) {
      ...
      // At effEnd=n there's no bottomSpacer â€” use Infinity so render-node-
      ...
      // streaming the tail item's cached height lags its real height â€”
