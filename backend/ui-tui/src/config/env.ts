import type { MouseTrackingMode } from '@zed/ink'
...
export const STARTUP_RESUME_ID = (process.env.ZED_TUI_RESUME ?? '').trim()
export const STARTUP_QUERY = (process.env.ZED_TUI_QUERY ?? '').trim()
export const STARTUP_IMAGE = (process.env.ZED_TUI_IMAGE ?? '').trim()

// Mouse tracking mode resolution at startup. Per-mode selection (off|wheel|
// buttons|all) lives in display.mouse_tracking in config.yaml â€” these env
// vars only set the boot-time default before that config is applied.
//
// Precedence (highest first):
//
// - ZED_TUI_MOUSE_TRACKING (truthy/falsy) explicitly overrides everything.
//   This is the "force a value" knob and intentionally beats the legacy
//   kill-switch and the Termux default.
// - ZED_TUI_DISABLE_MOUSE=1 forces mouse off â€” the legacy kill switch.
...
//   behavior.
const mouseTrackingOverride = parseToggle(process.env.ZED_TUI_MOUSE_TRACKING)
const mouseTrackingDisabledLegacy = truthy(process.env.ZED_TUI_DISABLE_MOUSE)
...
export const NO_CONFIRM_DESTRUCTIVE = truthy(process.env.ZED_TUI_NO_CONFIRM)
...
export const DASHBOARD_TUI_MODE = truthy(process.env.ZED_TUI_DASHBOARD)

// ZED_DEV_CREDITS â€” dev-only live-spend readout (Î” status segment + "(dev credits)"
// banner). Throwaway dev scaffolding; the whole readout gates on this one flag.
export const DEV_CREDITS_MODE = truthy(process.env.ZED_DEV_CREDITS)

const inlineOverride = parseToggle(process.env.ZED_TUI_INLINE)

// Skip AlternateScreen â€” TUI renders into the primary buffer so the host
...
// less fragile. Override explicitly with ZED_TUI_INLINE=0/1.
...
// synthetic timer).
export const SHOW_FPS = truthy(process.env.ZED_TUI_FPS)
