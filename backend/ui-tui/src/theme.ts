...
// â”€â”€ Color math â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// Mirrors @zed/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @zed/ink is bundled separately from packages/.
...
export const DARK_THEME: Theme = {
  color: {
    ...
    // new value sits ~60% luminance â€” readable without losing the "muted /
    ...
    prompt: '#FFF8DC',
    // sessionLabel/sessionBorder intentionally track the `dim` value â€” they
...
// Terminal. Explicit ZED_TUI_THEME / COLORFGBG signals above still win,
...
// Best-effort RGB â†’ luminance check.  Currently only accepts a 3- or
// 6-digit hex value (with or without a leading `#`); the env var name
// `ZED_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
function backgroundLuminance(raw: string): null | number {
  ...
  // Rec. 709 luma â€” close enough for "is this background bright".
  return (0.2126 * rgb[0]! + 0.7152 * rgb[1]! + 0.0722 * rgb[2]!) / 255
}

// Pick light vs dark with ordered, explainable signals (#11300):
//
//   1. `ZED_TUI_LIGHT` boolean â€” `1`/`true`/`yes`/`on` â†’ light;
//      `0`/`false`/`no`/`off` â†’ dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `ZED_TUI_THEME` named override â€” `light` / `dark` win over
//      every signal below.
//   3. `ZED_TUI_BACKGROUND` hex hint (3- or 6-digit) â€” luminance
//      â‰¥ LUMA_LIGHT_THRESHOLD â†’ light.
//   4. `COLORFGBG` last field â€” XFCE / rxvt / Terminal.app emit
//      slot 7 or 15 on light profiles; 0â€“15 ranges are otherwise
...
//
// Anything we can't decide stays dark â€” the default Zed palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.ZED_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.ZED_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.ZED_TUI_BACKGROUND ?? '')
  ...
  if (colorfgbg) {
    // Validate as a decimal integer before coercing â€” `Number('')` is 0,
    ...
    if (/^\d+$/.test(lastField)) {
      ...
      // Slots 0â€“6 and 8â€“14 are the dark half of the 0â€“15 ANSI range.
      // When COLORFGBG is set we trust it as authoritative â€” a non-light
...
// â”€â”€ Skin â†’ Theme â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
