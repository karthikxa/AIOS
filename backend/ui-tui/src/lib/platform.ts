 ...
 *     keys (Cmd+Left â†’ Ctrl+A, Cmd+Right â†’ Ctrl+E, Cmd+Backspace â†’ Ctrl+U).
...
/**
 * Voice recording toggle key â€” configurable via ``voice.record_key`` in
 ...
 * macOS action modifier (Cmd+B) â€” custom bindings like ``ctrl+o``
...
export interface ParsedVoiceRecordKey {
   ...
   * otherwise the named-key token (``'space'``, ``'enter'``â€¦). Kept as
 ...
 * zed-ink sets ``key.meta`` for plain Alt/Option on every platform
 ...
 * modifier would produce a display/binding mismatch â€” a config like
 ...
 * (``zed_cli/voice.py::normalize_voice_record_key_for_prompt_toolkit``)
 * so one ``voice.record_key`` value binds the same shortcut in both
 * runtimes (Copilot round-9 review on #19835). The ``super`` /
 * ``win`` / ``windows`` spellings are TUI-only â€” prompt_toolkit has no
 ...
 *
 * Aliases mirror what prompt_toolkit accepts (``return`` â†” ``enter``,
 * ``esc`` â†” ``escape``) so a config that round-trips through the CLI also
 * binds in the TUI. */
 ...
 *
 * ``ctrl+x`` is intentionally NOT here â€” it's only claimed during
 ...
 *  - super+c â†’ copy
 *  - super+d â†’ exit
 *  - super+l â†’ clear screen
 *  - super+v â†’ paste (also claimed at the TextInput layer)
 ...
 * modifier â€” but zed-ink reports Alt as ``key.meta`` on many
 ...
 * ``delete``) â€” matching the keys prompt_toolkit accepts on the CLI
 * side via the ``c-<name>`` rewrite in ``cli.py``.
 *
 * Accepts ``unknown`` because the source is raw YAML via
 * ``config.get full`` â€” a hand-edited ``voice.record_key: 1`` or
...
export const parseVoiceRecordKey = (raw: unknown): ParsedVoiceRecordKey => {
  ...
  // than silently dropping the extra modifier â€” the previous
  ...
  // check â€” ``ctrl+c`` / ``ctrl+d`` / ``ctrl+l`` would never actually
  ...
  // macOS only â€” those are copy / exit / clear / paste and get claimed
  ...
  // On macOS zed-ink reports Alt as ``key.meta``, which ``isActionMod``
  // accepts as the mac action modifier. So ``alt+c`` / ``alt+d`` / ``alt+l``
  // collide with copy / exit / clear in ``useInputHandlers()`` before the
  // voice check. Reject at parse time on darwin only â€” non-mac ``alt+<letter>``
  ...
  // Unknown multi-character token (e.g. typo'd ``ctrl+spcae``) â€” fall back
...
export const isVoiceToggleKey = (
  ...
  // so at match time Shift must always be clear â€” otherwise
  ...
  switch (configured.mod) {
    case 'alt':
      ...
      // alt binding.
      //
      // Bare Escape on zed-ink can arrive as ``key.meta=true`` on some
    ...
    case 'ctrl':
      ...
      // explicit ``key.super`` bit on macOS for Cmd+B muscle memory â€”
      // but ONLY ``key.super`` (kitty-style), never ``key.meta``, since
      // ``key.meta`` is zed-ink's Alt signal and accepting it would
    ...
    case 'super':
      ...
      // ``key.meta`` need a kitty-protocol terminal â€” see the
