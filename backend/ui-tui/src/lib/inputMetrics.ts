import { stringWidth, wrapAnsi } from '@zed/ink'
...
// boundaries â€” it never drops, reorders, or substitutes existing characters â€”
...
// delegates to wrap-ansi â€” so any drift between the two algorithms parks the
...
function visualLines(value: string, cols: number): VisualLine[] {
  ...
  for (let i = 0; i < wrapped.length; i += 1) {
    ...
    // never substitute, drop, or reorder source characters â€” so under those
 ...
 * hardware cursor several cells off the last rendered character â€” see the
