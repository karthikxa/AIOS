...
const ENV_KEYS = ['ZED_AUTO_HEAPDUMP', 'ZED_HEAPDUMP_DIR', 'ZED_HEAPDUMP_MAX_BYTES'] as const
...
describe('performHeapDump auto opt-in gate (#21767)', () => {
  ...
  beforeEach(() => {
    ...
    dir = mkdtempSync(join(tmpdir(), 'zed-heapdump-test-'))
    process.env.ZED_HEAPDUMP_DIR = dir
  })
  ...
  it('writes diagnostics only for auto-high without ZED_AUTO_HEAPDUMP', async () => {
  ...
  it('writes diagnostics only for auto-critical without ZED_AUTO_HEAPDUMP', async () => {
  ...
  it('writes both diagnostics and snapshot for auto-high when ZED_AUTO_HEAPDUMP=1', async () => {
    process.env.ZED_AUTO_HEAPDUMP = '1'
  ...
  it('accepts truthy spellings (true|yes|on, case-insensitive) as opt-in', async () => {
    for (const value of ['true', 'YES', 'On']) {
      process.env.ZED_AUTO_HEAPDUMP = value
      const result = await performHeapDump('auto-high')
  ...
  it('writes both for manual triggers regardless of ZED_AUTO_HEAPDUMP', async () => {
...
describe('heapdump retention guard (#21767)', () => {
  ...
  beforeEach(() => {
    savedDir = process.env.ZED_HEAPDUMP_DIR
    savedMax = process.env.ZED_HEAPDUMP_MAX_BYTES
    delete process.env.ZED_AUTO_HEAPDUMP
    dir = mkdtempSync(join(tmpdir(), 'zed-heapdump-prune-'))
    process.env.ZED_HEAPDUMP_DIR = dir
  })

  afterEach(() => {
    if (savedDir === undefined) {delete process.env.ZED_HEAPDUMP_DIR}
    else {process.env.ZED_HEAPDUMP_DIR = savedDir}

    if (savedMax === undefined) {delete process.env.ZED_HEAPDUMP_MAX_BYTES}
    else {process.env.ZED_HEAPDUMP_MAX_BYTES = savedMax}
  ...
  it('evicts oldest files when total bytes exceed the cap, retaining the newest', async () => {
    ...
    // Cap at 2KB â†’ a fresh diagnostics write should trigger a prune down to ~cap.
    process.env.ZED_HEAPDUMP_MAX_BYTES = String(2 * 1024)
