...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ZED_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Zed on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ZED_TUI_TRUECOLOR=1.
