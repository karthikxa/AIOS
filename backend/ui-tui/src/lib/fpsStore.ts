...
// frame (React commit + drain-only frames) â€” the right notion for
// user-perceived motion.
//
// Zero-cost when ZED_TUI_FPS is unset: trackFrame is undefined so the
...
export type FpsState = {
  fps: number
  /** Wraps at JS-safe int â€” diff pairs in a debug overlay safely. */
  totalFrames: number
