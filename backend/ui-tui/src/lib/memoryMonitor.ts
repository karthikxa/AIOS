...
export interface MemoryMonitorOptions {
  ...
  // critical exit threshold â€” the regime where the TUI used to die silently
...
// constant killed the process â€” and silently closed the gateway's stdin â€” at
...
// Deferred @zed/ink import: loading `@zed/ink` at module top-level
...
// only when heap pressure crosses the high-water mark â€” by then Ink has
...
async function _ensureEvictInkCaches(): Promise<(level: 'all' | 'half') => unknown> {
  ...
  _evictInkCachesPromise ??= import('@zed/ink')
    .then(mod => {
...
export function startMemoryMonitor({
  ...
  // climbing steeply (â‰¥150MB between 10s ticks) â€” the signature of a render-
  // tree blowup â€” so the user gets a visible heads-up before Node OOMs under
  ...
  // threshold (issue #21767). `dumped` alone is not enough â€” it clears on
  // every transition back to `normal`.
  const cooldownRaw = process.env.ZED_AUTO_HEAPDUMP_COOLDOWN_MS?.trim()
  ...
  const tick = async () => {
    ...
    // sample â€” we need a prior reading to measure a delta against.
    ...
    // Prune Ink content caches before dump/exit â€” half on 'high' (recoverable),
    // full on 'critical' (post-dump RSS reduction, keeps user running).
    // Deferred import keeps `@zed/ink` off the cold-start critical path;
