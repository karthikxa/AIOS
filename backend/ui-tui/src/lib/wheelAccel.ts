...
//
//   gap < 5ms                 â†’ same-batch burst â†’ 1 row/event
//   gap < 40ms (native)       â†’ ramp +0.3, cap 6
//   gap 80-500ms (xterm.js)   â†’ mult = 1 + (mult-1)Â·0.5^(gap/150) + 5Â·decay
//                               cap 3 slow / 6 fast
//   gap > 500ms               â†’ reset (deliberate click stays responsive)
//   flip + flip-back â‰¤200ms   â†’ encoder bounce â†’ engage wheel-mode (sticky cap)
//   5 consecutive <5ms events â†’ trackpad flick â†’ disengage wheel-mode
//
...
import { isXtermJs } from '@zed/ink'

// â”€â”€ Native (ghostty, iTerm2, WezTerm, â€¦) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const WHEEL_ACCEL_WINDOW_MS = 40
const WHEEL_ACCEL_STEP = 0.3
const WHEEL_ACCEL_MAX = 6

// â”€â”€ Encoder bounce / wheel-mode (mechanical wheels) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ xterm.js (VS Code / Cursor / browser terminals) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/** ZED_TUI_SCROLL_SPEED (or CLAUDE_CODE_SCROLL_SPEED for portability).
 *  Default 1, clamped (0, 20]. */
export function readScrollSpeedBase(): number {
  const n = parseFloat(process.env.ZED_TUI_SCROLL_SPEED ?? process.env.CLAUDE_CODE_SCROLL_SPEED ?? '')
 ...
 *  direction flip is deferred for bounce detection â€” call sites should
 *  no-op on 0. */
...
function nativeStep(state: WheelAccelState, dir: -1 | 1, now: number): number {
  ...
  if (state.pendingFlip) {
    ...
    if (dir !== state.dir || now - state.time > WHEEL_BOUNCE_GAP_MAX_MS) {
      ...
      // The deferred event's 1 row is lost â€” acceptable latency.
      state.dir = dir
      state.time = now
      state.mult = state.base
  ...
  // Trackpad / hi-res native: tight 40ms window â€” sub-window ramps,
...
function xtermJsStep(state: WheelAccelState, dir: -1 | 1, now: number): number {
  ...
  if (!sameDir || gap > WHEEL_DECAY_IDLE_MS) {
    // Reversal or long idle â€” start at 2 so first click after a pause moves visibly.
    state.mult = 2
    state.frac = 0
  } else {
    const m = Math.pow(0.5, gap / WHEEL_DECAY_HALFLIFE_MS)
