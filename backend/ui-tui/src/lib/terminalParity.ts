...
export async function terminalParityHints(
  ...
  if (
    ...
    hints.push({
      key: 'ide-setup',
      tone: 'info',
      message: `Detected ${ctx.vscodeLike} terminal Â· run /terminal-setup for best Cmd+Enter / undo parity`
    })
  }
  ...
  if (ctx.isAppleTerminal) {
    hints.push({
      ...
      tone: 'warn',
      message:
        'Apple Terminal detected Â· use /paste for image-only clipboard fallback, and try Ctrl+A / Ctrl+E / Ctrl+U if Cmd+â†/â†’/âŒ« gets rewritten'
    })
  }
  ...
  if (ctx.isTmux) {
    hints.push({
      ...
      tone: 'warn',
      message:
        'tmux detected Â· clipboard copy/paste uses passthrough when available; allow-passthrough improves OSC52 reliability'
    })
  }
  ...
  if (ctx.isRemote) {
    hints.push({
      ...
      tone: 'warn',
      message:
        'SSH session detected Â· text clipboard can bridge via OSC52, but image clipboard and local screenshot paths still depend on the machine running Zed'
    })
  }
