...
describe('openCommand', () => {
  ...
  it('routes through explorer.exe on win32 â€” not cmd.exe â€” so URLs with & | ^ < > stay safe', () => {
    // win32 must not route through cmd.exe â€” see comment in openCommand.
  ...
  it('returns null for unknown platforms (aix, sunos, cygwin, etc.)', () => {
    ...
    // probably isn't installed â€” the caller's `if (!command) return false`
...
describe('openExternalUrl', () => {
  ...
  // ChildProcess â€” the latter's `unref` signature is strictly `() => void`
  ...
  function mockSpawn(): {
    ...
    const spawn = vi.fn((command: string, args: readonly string[]) => {
      ...
      // test. The cast is the same one Node uses internally â€” ChildProcess
  ...
  it('on win32, a URL with & | ^ < > is forwarded as a single argv element via explorer.exe', () => {
    ...
    // parser percent-encodes `<` and `>` (which is fine â€” encoded forms
    ...
    // The URL must arrive as exactly one argv element â€” not split on &/|/^/etc.
  ...
  it('on win32, common http URLs with & query params are forwarded intact', () => {
    const { spawn, calls } = mockSpawn()
    const url = 'https://example.com/search?q=foo&page=2&utm_source=zed'
  ...
  it('does not crash the host when the spawned process emits an async error', () => {
    ...
    // re-throws the event as an uncaught exception â†’ TUI dies. We attach
