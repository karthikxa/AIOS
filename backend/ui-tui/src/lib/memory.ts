...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because ZED_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function captureMemoryDiagnostics(trigger: MemoryTrigger): Promise<MemoryDiagnostics> {
  ...
  // Real growth rate since STARTED_AT (captured at module load) â€” NOT a lifetime
  ...
  const potentialLeaks = [
    heapStats.number_of_detached_contexts > 0 &&
      `${heapStats.number_of_detached_contexts} detached context(s) â€” possible component/closure leak`,
    activeHandles > 100 && `${activeHandles} active handles â€” possible timer/socket leak`,
    nativeMemory > usage.heapUsed && 'Native memory > heap â€” leak may be in native addons',
    mbPerHour > 100 && `High memory growth rate: ${mbPerHour.toFixed(1)} MB/hour`,
    openFileDescriptors && openFileDescriptors > 500 && `${openFileDescriptors} open FDs â€” possible file/socket leak`
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    // Diagnostics first â€” heap-snapshot serialization can crash on very large
    // heaps, and the JSON sidecar is the most actionable artifact if so.
    const diagnostics = await captureMemoryDiagnostics(trigger)
    const dir = process.env.ZED_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.zed', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `zed-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.ZED_AUTO_HEAPDUMP ?? '').trim())

    if (isAuto && !autoEnabled) {
      await pruneHeapdumps(dir).catch(() => undefined)

      // Not an error: the dump did its job â€” it wrote the lightweight
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.ZED_HEAPDUMP_MAX_BYTES?.trim()
