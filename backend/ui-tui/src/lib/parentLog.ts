...
//
// A backend SIGTERM is *usually* a parent action â€” `gw.kill()` (graceful-exit on
// a signal to Node, or an explicit /quit) or `start()` replacing a live child â€”
...
// parent kill; its absence *suggests* an external signal â€” not definitive,
...
// distinction (and a memory-critical `process.exit(137)`, which closes stdin â†’
// clean EOF, not SIGTERM) diagnosable after the fact.
const logDir = join(process.env.ZED_HOME?.trim() || join(homedir(), '.zed'), 'logs')
...
// a real ~/.zed (tests must stay hermetic â€” see AGENTS.md).
const enabled = !process.env.VITEST
// Slice a single breadcrumb's value to MAX_BREADCRUMB chars (a short
// "[truncated â€¦]" marker is appended, so the written line is slightly longer)
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    const capped =
      oneLine.length > MAX_BREADCRUMB ? `${oneLine.slice(0, MAX_BREADCRUMB)}â€¦ [truncated ${oneLine.length} chars]` : oneLine
    ...
    if (!warned) {
      warned = true
      process.stderr.write('zed-tui: parent lifecycle log unavailable\n')
    }
  }
}
