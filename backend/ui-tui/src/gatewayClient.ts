...
const STARTUP_TIMEOUT_MS = Math.max(5000, parseInt(process.env.ZED_TUI_STARTUP_TIMEOUT_MS ?? '15000', 10) || 15000)
const REQUEST_TIMEOUT_MS = Math.max(30000, parseInt(process.env.ZED_TUI_RPC_TIMEOUT_MS ?? '120000', 10) || 120000)
...
const truncateLine = (line: string) =>
  line.length > MAX_LOG_LINE_BYTES ? `${line.slice(0, MAX_LOG_LINE_BYTES)}â€¦ [truncated ${line.length} bytes]` : line
...
const resolveGatewayAttachUrl = () => {
  const raw = process.env.ZED_TUI_GATEWAY_URL?.trim()

  return raw ? raw : null
}

const resolveSidecarUrl = () => {
  const raw = process.env.ZED_TUI_SIDECAR_URL?.trim()

  return raw ? raw : null
}

const resolvePython = (root: string) => {
  const configured = process.env.ZED_PYTHON?.trim() || process.env.PYTHON?.trim()
...
// Matches `<scheme>://user:pass@hostâ€¦` style user-info segments in
...
export class GatewayClient extends EventEmitter {
  ...
  private startSpawnedGateway(root: string) {
    ...
    const cwd = process.env.ZED_CWD || root
    ...
    this.proc.on('error', err => {
      ...
      // 'exit' twice). Then run the full teardown â€” clears the
  ...
  private startAttachedGateway(attachUrl: string) {
    ...
    try {
      ...
      ws.addEventListener('close', ev => {
        ...
        // replaced â€” start() / closeGatewaySocket() can swap `this.ws`
  ...
  start() {
    const root = process.env.ZED_PYTHON_SRC_ROOT ?? resolve(import.meta.dirname, '../../')
  ...
  // Death-explaining breadcrumbs (spawn / exit / kill / replace) â€” kept in the
  ...
  // Arrow class-field â€” stable identity, so `setTimeout(this.onTimeout, â€¦, id)`
  ...
  request<T = unknown>(method: string, params: Record<string, unknown> = {}): Promise<T> {
    ...
    if (attachUrl) {
      if (this.attachUrl !== attachUrl) {
        // The env var rotated at runtime â€” restart the transport so
