# Contributing to Zed Agent

Thank you for contributing to Zed Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
1. **Bug fixes** â€” crashes, incorrect behavior, data loss. Always top priority.
2. **Cross-platform compatibility** â€” macOS, different Linux distros, and WSL2 on Windows. We want Zed to work everywhere.
3. **Security hardening** â€” shell injection, prompt injection, path traversal, privilege escalation. See [Security](#security-considerations).
4. **Performance and robustness** â€” retry logic, error handling, graceful degradation.
5. **New skills** â€” but only broadly useful ones. See [Should it be a Skill or a Tool?](#should-it-be-a-skill-or-a-tool)
6. **New tools** â€” rarely needed. Most capabilities should be skills. See below.
7. **Documentation** â€” fixes, clarifications, new examples.
...
Bundled skills (in `skills/`) ship with every Zed install. They should be **broadly useful to most users**:
...
**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.zed/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) â€” `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(zed_home, config)` for setup-wizard integration
- Use the same discovery system â€” `discover_memory_providers()` picks them up from user/project plugin directories and pip entry points
- Integrate with `zed memory setup` via `post_setup()` â€” no need to touch core code
...
This isn't a quality bar â€” it's a coupling-and-maintenance decision. Memory providers are the most common plugin type and they shouldn't all live in this tree.
...
| **Node.js 20+** | Optional â€” needed for browser tools and WhatsApp bridge (matches root `package.json` engines) |

### Install with the standard installer

For most contributors, the best development bootstrap is the same path users
take: run the standard installer, then work inside the repository it cloned.
The installer creates the Zed venv, wires the `zed` command, stamps the
install method for `zed update`, and clones the full git project into
`$ZED_HOME/zed-agent` (usually `~/.zed/zed-agent`). That keeps your
...
Use this only if you intentionally do not want Zed' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `zed` entrypoint from this venv; running the
system `python3 -m zed_cli.main` can pick up unrelated system Python
packages.
...
echo "OPENROUTER_API_KEY=***" >> ~/.zed/.env
```

### Run

```bash
# The standard installer already put `zed` on PATH.
zed doctor
zed chat -q "Hello"
```

If you used the manual clone fallback, run `./zed` from the checkout or
...
```bash
# Preferred â€” matches CI (hermetic env, 4 xdist workers); see AGENTS.md
scripts/run_tests.sh
...
```
zed-agent/
â”œâ”€â”€ run_agent.py              # AIAgent class â€” core conversation loop, tool dispatch, session persistence
â”œâ”€â”€ cli.py                    # ZedCLI class â€” interactive TUI, prompt_toolkit integration
â”œâ”€â”€ model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
â”œâ”€â”€ toolsets.py               # Tool groupings and presets (zed-cli, zed-telegram, etc.)
â”œâ”€â”€ zed_state.py           # SQLite session database with FTS5 full-text search, session titles
â”œâ”€â”€ batch_runner.py           # Parallel batch processing for trajectory generation
â”‚
â”œâ”€â”€ agent/                    # Agent internals (extracted modules)
â”‚   â”œâ”€â”€ prompt_builder.py         # System prompt assembly (identity, skills, context files, memory)
â”‚   â”œâ”€â”€ context_compressor.py     # Auto-summarization when approaching context limits
â”‚   â”œâ”€â”€ auxiliary_client.py       # Resolves auxiliary OpenAI clients (summarization, vision)
â”‚   â”œâ”€â”€ display.py                # KawaiiSpinner, tool progress formatting
â”‚   â”œâ”€â”€ model_metadata.py         # Model context lengths, token estimation
â”‚   â””â”€â”€ trajectory.py             # Trajectory saving helpers
â”‚
â”œâ”€â”€ zed_cli/               # CLI command implementations
...
â”‚   â”œâ”€â”€ callbacks.py              # Interactive callbacks (clarify, sudo, approval)
â”‚   â”œâ”€â”€ doctor.py                 # Diagnostics
â”‚   â”œâ”€â”€ skills_hub.py             # Skills Hub CLI + /skills slash command
â”‚   â””â”€â”€ skin_engine.py            # Skin/theme engine â€” data-driven CLI visual customization
â”‚
â”œâ”€â”€ tools/                    # Tool implementations (self-registering)
â”‚   â”œâ”€â”€ registry.py               # Central tool registry (schemas, handlers, dispatch)
â”‚   â”œâ”€â”€ approval.py               # Dangerous command detection + per-session approval
â”‚   â”œâ”€â”€ terminal_tool.py          # Terminal orchestration (sudo, env lifecycle, backends)
â”‚   â”œâ”€â”€ file_operations.py        # read_file, write_file, search, patch, etc.
â”‚   â”œâ”€â”€ web_tools.py              # web_search, web_extract (Parallel/Firecrawl + Gemini summarization)
â”‚   â”œâ”€â”€ vision_tools.py           # Image analysis via multimodal models
â”‚   â”œâ”€â”€ delegate_tool.py          # Subagent spawning and parallel task execution
â”‚   â”œâ”€â”€ code_execution_tool.py    # Sandboxed Python with RPC tool access
â”‚   â”œâ”€â”€ session_search_tool.py    # Search past conversations with FTS5 + anchored windows
â”‚   â”œâ”€â”€ cronjob_tools.py          # Scheduled task management
â”‚   â”œâ”€â”€ skill_tools.py            # Skill search, load, manage
â”‚   â””â”€â”€ environments/             # Terminal execution backends
â”‚       â”œâ”€â”€ base.py                   # BaseEnvironment ABC
â”‚       â”œâ”€â”€ local.py, docker.py, ssh.py, singularity.py, modal.py, daytona.py
â”‚
â”œâ”€â”€ gateway/                  # Messaging gateway
â”‚   â”œâ”€â”€ run.py                    # GatewayRunner â€” platform lifecycle, message routing, cron
â”‚   â”œâ”€â”€ config.py                 # Platform configuration resolution
â”‚   â”œâ”€â”€ session.py                # Session store, context prompts, reset policies
â”‚   â””â”€â”€ platforms/                # Platform adapters
â”‚       â”œâ”€â”€ telegram.py, discord_adapter.py, slack.py, whatsapp.py
â”‚
â”œâ”€â”€ scripts/                  # Installer and bridge scripts
â”‚   â”œâ”€â”€ install.sh                # Linux/macOS installer
â”‚   â”œâ”€â”€ install.ps1               # Windows PowerShell installer
â”‚   â””â”€â”€ whatsapp-bridge/          # Node.js WhatsApp bridge (Baileys)
â”‚
...
### User configuration (stored in `~/.zed/`)
...
| `~/.zed/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.zed/cron/` | Scheduled job data |
| `~/.zed/whatsapp/session/` | WhatsApp bridge credentials |
...
```
User message â†’ AIAgent._run_agent_loop()
  â”œâ”€â”€ Build system prompt (prompt_builder.py)
  â”œâ”€â”€ Build API kwargs (model, messages, tools, reasoning config)
  â”œâ”€â”€ Call LLM (OpenAI-compatible API)
  â”œâ”€â”€ If tool_calls in response:
  â”‚     â”œâ”€â”€ Execute each tool via registry dispatch
  â”‚     â”œâ”€â”€ Add tool results to conversation
  â”‚     â””â”€â”€ Loop back to LLM call
  â”œâ”€â”€ If text response:
  â”‚     â”œâ”€â”€ Persist session to DB
  â”‚     â””â”€â”€ Return final_response
  â””â”€â”€ Context compression if approaching token limit
```
...
- **Comments**: Only when explaining non-obvious intent, trade-offs, or API quirks. Don't narrate what the code does â€” `# increment counter` adds nothing
- **Error handling**: Catch specific exceptions. Log with `logger.warning()`/`logger.error()` â€” use `exc_info=True` for unexpected errors so stack traces appear in logs
...
```python
"""my_tool â€” Brief description of what this tool does."""
...
(for example `_ZED_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
required_environment_variables:    # Optional â€” secure setup-on-load metadata
  - name: MY_API_KEY
    prompt: API key
...
metadata:
  zed:
    ...
    fallback_for_toolsets: [web]       # Optional â€” show only when toolset is unavailable
    requires_toolsets: [terminal]      # Optional â€” show only when toolset is available
---
...
Skills can declare conditions that control when they appear in the system prompt, based on which tools and toolsets are available in the current session. This is primarily used for **fallback skills** â€” alternatives that should only be shown when a primary tool is unavailable.

Four fields are supported under `metadata.zed`:
...
```yaml
# DuckDuckGo search â€” shown when Firecrawl (web toolset) is unavailable
metadata:
  zed:
    fallback_for_toolsets: [web]

# Smart home skill â€” only useful when terminal is available
metadata:
  zed:
    requires_toolsets: [terminal]

# Local browser fallback â€” shown when Browserbase is unavailable
metadata:
  zed:
...
The user may skip setup and keep loading the skill. Zed only exposes metadata (`stored_as`, `skipped`, `validated`) to the model â€” never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `zed setup` or update `~/.zed/.env` locally.
...
Every new or modernized skill â€” bundled, optional, or contributed â€” must meet these standards before merge. Reviewers reject PRs that violate them.
...
2. **Tools referenced in SKILL.md prose must be native Zed tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
3. **`platforms:` gating audited against actual script imports.** Skills that use POSIX-only primitives (`fcntl`, `termios`, `os.setsid`, `os.kill(pid, 0)` for liveness, `/proc`, hardcoded `/tmp` paths, `signal.SIGKILL`, bash heredocs, `osascript`, `apt`, `systemctl`) must declare their supported platforms via the `platforms:` frontmatter. Default posture is to fix it cross-platform first â€” `tempfile.gettempdir()`, `pathlib.Path`, `psutil.pid_exists()`, Python-level filtering instead of `grep`. Gate to a narrower set only when the dependency is genuinely platform-bound (e.g. `osascript` is macOS-only, `/proc` is Linux-only).

4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Zed Agent" is the secondary collaborator. If the contributor's commit shows "Zed Agent" as author because they used Zed to draft the skill, replace it with their actual name â€” credit the human, not the tool.
...
5. **SKILL.md body uses the modern section order.** `# <Skill> Skill` title, 2-3 sentence intro stating what it does and what it doesn't do, then:
   ...
   - `## Prerequisites` â€” env vars, install steps, MCP setup, API key sourcing
   - `## How to Run` â€” canonical invocation through the `terminal` tool
   - `## Quick Reference` â€” flat command/API reference
   - `## Procedure` â€” numbered steps with copy-paste commands
   - `## Pitfalls` â€” known limits, rate limits, things that look broken but aren't
   - `## Verification` â€” single command that proves the skill works
...
6. **Scripts go in `scripts/`, references in `references/`, templates in `templates/`.** Don't expect the model to inline-write parsers, XML walkers, or non-trivial logic every call â€” ship a helper script. Reference scripts from SKILL.md by path relative to the skill directory.
...
8. **`.env.example` additions are isolated to a clearly delimited block.** Don't touch the surrounding file â€” contributor-supplied `.env.example` versions are usually stale, and edits outside the skill's own block will be dropped during salvage. Comment all values with `#` (it's documentation, not live config).

### Skill guidelines

- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Zed tools (`web_extract`, `terminal`, `read_file`).
- **Progressive disclosure.** Put the most common workflow first. Edge cases and advanced usage go at the bottom.
- **Include helper scripts** for XML/JSON parsing or complex logic â€” don't expect the LLM to write parsers inline every time.
- **Test it.** Run `zed --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Zed uses a data-driven skin system â€” no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.zed/skins/<name>.yaml`:
...
branding:
  ...
  response_label: " âš” Agent "
  prompt_symbol: "âš”"

tool_prefix: "â•Ž"             # Tool output line prefix
```

All fields are optional â€” missing values inherit from the default skin.

**Option B: Built-in skin**

Add to `_BUILTIN_SKINS` dict in `zed_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `zed_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Zed runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   is a standard POSIX idiom to check "is this PID alive" â€” the signal 0
   ...
   (open since 2012 â€” will never be fixed for compat reasons).

   **Preferred:** use `psutil` (a core dependency â€” always available):

   ```python
   import psutil
   if psutil.pid_exists(pid):
       # process is alive â€” safe on every platform
       ...
   ```

   If you specifically need the zed wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out â€” don't assume Windows has
   ...
   `shutil.which("tool")` and fall back to a Windows-native equivalent â€”
   ...
   `zed_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
4. **File encoding.** Windows may save `.env` files in `cp1252`. Always
   ...
   similar editors â€” use `encoding="utf-8-sig"` when reading files that
...
5. **Process management.** `os.setsid()`, `os.killpg()`, `os.fork()`,
   ...
   **Preferred:** for killing a process AND its children (what `os.killpg`
   does on POSIX), use `psutil` â€” it works on every platform:
   ```python
   import psutil
   try:
...
6. **Signals that don't exist on Windows: `SIGALRM`, `SIGCHLD`, `SIGHUP`,
   ...
   raises `NotImplementedError` on Windows â€” always catch it.
...
7. **Path separators.** Use `pathlib.Path` instead of string concatenation
   ...
   require backslashes â€” convert with `str(path)` at the subprocess boundary,
...
9. **POSIX file modes (0o600, 0o644, etc.) are NOT enforced on NTFS** by
   ...
   Windows â€” the concept doesn't translate. Use ACLs (`icacls`, `pywin32`)
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `zed_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
11. **`subprocess.Popen` with `.cmd` or `.bat` shims needs `shutil.which`
    ...
    `CreateProcessW` can't execute â€” you'll get `WinError 193 "not a valid
...
13. **Shell commands in installers.** If you change `scripts/install.sh`,
    ...
    Windows" and have drifted multiple times â€” keep them in lockstep.
...
14. **Known paths that are OneDrive-redirected on Windows:** Desktop,
    ...
    `Shell Folders` registry key â€” never assume `~/Desktop`.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `zed_cli/gateway_windows.py::
...
- Symlinks â†’ `@pytest.mark.skipif(sys.platform == "win32", ...)`
- `0o600` file modes â†’ `@pytest.mark.skipif(sys.platform.startswith("win"), ...)`
- `signal.SIGALRM` â†’ Unix-only (see `tests/conftest.py::_enforce_test_timeout`)
- `os.setsid` / `os.fork` â†’ Unix-only
- Live Winsock / Windows-specific regression tests â†’
  `@pytest.mark.skipif(sys.platform != "win32", reason="Windows-specific regression")`

If you monkeypatch `sys.platform` for cross-platform tests, also patch
`platform.system()` / `platform.release()` / `platform.mac_ver()` â€” each
...
Zed has terminal access. Security matters.
...
- If the package is at version `0.x.y` (pre-1.0), use `<0.(current_minor + 2)` â€” e.g. if current is `0.29.x`, use `<0.32`. This gives ~2 minor versions of headroom while keeping the window small enough that a hostile takeover version is unlikely to land inside it.
- Exception: packages with very stable APIs (e.g. `aiohttp-socks`) can use `<1` at reviewer discretion.

**Examples:**
```toml
# âœ… Correct â€” post-1.0
"openai>=2.21.0,<3"
"pydantic>=2.12.5,<3"

# âœ… Correct â€” pre-1.0 (tight minor window)
"asyncpg>=0.29,<0.32"
"aiosqlite>=0.20,<0.23"
"hindsight-client>=0.4.22,<0.5"

# âŒ Rejected â€” no upper bound
"some-package>=1.2.3"

# âŒ Rejected â€” too tight (blocks legitimate patches)
"some-package==1.2.3"

# âŒ Rejected â€” too loose for pre-1.0 (allows 80 minor versions)
"some-package>=0.20,<1"
```
...
2. **Test manually**: Run `zed` and exercise the code path you changed
