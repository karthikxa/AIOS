...
Synthesized from:
- jenslys/opencode-gemini-auth (MIT) â€” overall flow shape, public OAuth creds, request format
- clawdbot/extensions/google/ â€” refresh-token rotation, VPC-SC handling reference
- PRs #10176 (@sliverp) and #10779 (@newarthur) â€” PKCE module structure, cross-process lock

Storage (``~/.zed/auth/google_oauth.json``, chmod 0o600):
...
The packed format stays parseable even if no project IDs are present â€” just
...
gemini-cli npm package and are NOT confidential â€” desktop OAuth clients have
...
from hermes_constants import get_zed_home, secure_parent_dir
...
#
# Resolution order:
#   1. ZED_GEMINI_CLIENT_ID / ZED_GEMINI_CLIENT_SECRET env vars (power users)
#   2. Shipped defaults â€” Google's public gemini-cli desktop OAuth client
#      (baked into every copy of Google's open-source gemini-cli; NOT
#      confidential â€” desktop OAuth clients use PKCE, not client_secret, for
...
ENV_CLIENT_ID = "ZED_GEMINI_CLIENT_ID"
ENV_CLIENT_SECRET = "ZED_GEMINI_CLIENT_SECRET"
...
_HEADLESS_ENV_VARS = ("SSH_CONNECTION", "SSH_CLIENT", "SSH_TTY", "ZED_HEADLESS")
...
def _credentials_path() -> Path:
    return get_zed_home() / "auth" / "google_oauth.json"
...
def _require_client_id() -> str:
    ...
    if not cid:
        raise GoogleOAuthError(
            ...
            "Zed looks for a locally installed gemini-cli to source the OAuth client. "
            "Either:\n"
            "  1. Install it: npm install -g @google/gemini-cli  (or brew install gemini-cli)\n"
            "  2. Set ZED_GEMINI_CLIENT_ID and ZED_GEMINI_CLIENT_SECRET in ~/.zed/.env\n"
            "\n"
...
def get_valid_access_token(*, force_refresh: bool = False) -> str:
    ...
    if creds is None:
        raise GoogleOAuthError(
            "No Google OAuth credentials found. Run `zed auth add google-gemini-cli` first.",
    ...
    if not owner:
        # Another thread is refreshing â€” wait, then re-read from disk.
        ...
        try:
            ...
            if exc.code == "google_oauth_invalid_grant":
                logger.warning(
                    ...
                    "Clearing credentials at %s â€” user must re-login.",
                    _credentials_path(),
                )
                clear_credentials()
            raise
...
class _OAuthCallbackHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if state != type(self).expected_state:
            ...
            self._respond_html(400, _ERROR_PAGE.format(message="State mismatch â€” aborting for safety."))
        elif error:
...
<html><head><meta charset="utf-8"><title>Zed â€” signed in</title>
<style>
...
<html><head><meta charset="utf-8"><title>Zed â€” sign-in failed</title>
<style>
body {{ font: 16px/1.5 system-ui, sans-serif; margin: 10vh auto; max-width: 32rem; text-align: center; color: #222; }}
h1 {{ color: #b42318; }} p {{ color: #555; }}
</style></head>
<body><h1>Sign-in failed</h1><p>{message}</p>
<p>Return to your terminal â€” Zed will walk you through a manual paste fallback.</p></body></html>
"""
...
def start_oauth_flow(
    *,
    ...
    auth_url = AUTH_ENDPOINT + "?" + urllib.parse.urlencode(params) + "#zed"

    server_thread = threading.Thread(target=server.serve_forever, daemon=True)
    server_thread.start()

    print()
    print("Opening your browser to sign in to Googleâ€¦")
    ...
    try:
        if ready.wait(timeout=callback_wait_seconds):
            ...
            logger.info("Callback server timed out â€” offering manual paste fallback.")
