#!/usr/bin/env python3
"""
Zed Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Zed Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills zed-agent-dev,github-auth
...
os.environ["ZED_QUIET"] = "1"  # Our own modules
...
load_zed_dotenv(zed_home=_zed_home, project_env=_project_env)
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("ZED_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.zed/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If ZED_IGNORE_USER_CONFIG=1 is set (via ``zed chat --ignore-user-config``),
    the user config at ``~/.zed/config.yaml`` is skipped entirely and only the
    ...
    """
    # Check user config first ({ZED_HOME}/config.yaml)
    user_config_path = _zed_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("ZED_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Zed, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Zed - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume â€” see zed_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and ZedCLI.__init__ checks "default" first.
    ...
    # Normalize config key: the new config system (zed_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && zed` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _ZED_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_ZED_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["ZED_REDACT_SECRETS"] = str(redact).lower()
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``zed --help`` paths inside the chat command flow).  See
...
# one-shot CLI runs â€” which also register _run_cleanup via atexit â€” don't emit
...
def _prepare_deferred_agent_startup() -> None:
    ...
    if _deferred_agent_startup_done:
        return
    if os.environ.get("ZED_DEFER_AGENT_STARTUP") != "1":
        return
    _deferred_agent_startup_done = True
    _accept_hooks = os.environ.get("ZED_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _reset_terminal_input_modes_on_exit() -> None:
    ...
    ``os._exit(0)`` path bypasses ``atexit``; neither runs this â€” but both are
    ...
    if not _tui_input_modes_active:
        return
    # About to disable the modes â€” clear the flag so a re-armed _run_cleanup (or
    ...
    # /dev/tty while stdout was redirected â€” reset there instead of nowhere.
    try:
        stream = sys.stdout
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run zed -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"zed-{short_id}"
    branch_name = f"zed/{wt_name}"
    ...
    try:
        ...
        if result.returncode != 0:
            print(f"\033[31mâœ— Failed to create worktree: {result.stderr.strip()}\033[0m")
            return None
    except Exception as e:
    ...
    try:
        subprocess.run(
            ["git", "worktree", "lock", "--reason", f"zed pid={os.getpid()}", str(wt_path)],
...
def _cleanup_worktree(info: Dict[str, str] = None) -> None:
    ...
    # placed at creation time.  Fail-soft â€” never block cleanup.
    try:
        subprocess.run(
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_zed_home_maint / "sessions"
                )
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``zed/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("zed-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``zed/zed-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``zed -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("zed/zed-") or b.startswith("pr-"))
    ]
...
#
# Detection priority:
#   1. ZED_LIGHT / ZED_TUI_LIGHT env (true/false) â€” explicit override
#   2. ZED_TUI_THEME=light|dark â€” explicit theme
#   3. ZED_TUI_BACKGROUND=#RRGGBB â€” explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) â€” bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) â€” ask the terminal directly
#   6. Default: assume dark (matches the legacy Zed assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("ZED_LIGHT", "ZED_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("ZED_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("ZED_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    SkinConfig._zed_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Zed output it's common to display cron schedules verbatim.
    #
...
def _cprint(text: str):
    ...
    background review's ``ðŸ’¾ â€¦`` summary, curator summaries, and other
...
def _prepend_note_to_message(message, note: str):
    ...
    then raises ``TypeError: can only concatenate str (not "list") to str`` â€”
    ...
    Returns the message with ``note`` prepended:
      ...
      * ``list`` â†’ note folded into the first text part, or inserted as a new
    ...
    if isinstance(message, list):
        ...
        for i, part in enumerate(parts):
            if isinstance(part, dict) and part.get("type") == "text":
                ...
                parts[i] = merged
                return parts
        # No text part (image-only) â€” insert the note as a leading text block.
...
def _format_image_attachment_badges(attached_images: list[Path], image_counter: int, width: int | None = None) -> str:
    ...
    if width < 52:
        if len(attached_images) == 1:
            return f"[ðŸ“Ž {_trunc(attached_images[0].name, 20)}]"
        return f"[ðŸ“Ž {len(attached_images)} images attached]"

    if width < 80:
        if len(attached_images) == 1:
            return f"[ðŸ“Ž {_trunc(attached_images[0].name, 32)}]"
        first = _trunc(attached_images[0].name, 20)
        extra = len(attached_images) - 1
        return f"[ðŸ“Ž {first}] [+{extra}]"
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_zed_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_zed_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._zed_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_zed_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._zed_bp_start = now
        ...
        _vt100_mod._zed_bp_timeout_patched = True
...
def _estimate_tui_input_height(
    ...
    *,
    ...
    prompt such as ``âš” ``, while continuation rows use the full terminal width.
...
# ANSI parser can't handle â€” it strips \x1b but passes the payload through
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``ZedCLI._busy_command()``.
        """
        yield self

# ASCII Art - ZED-AGENT logo (full width, single line - requires ~95 char terminal)
ZED_AGENT_LOGO = """[bold #FFD700]â–ˆâ–ˆâ•—  â–ˆâ–ˆâ•—â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•—â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•— â–ˆâ–ˆâ–ˆâ•—   â–ˆâ–ˆâ–ˆâ•—â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•—â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•—       â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•—  â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•— â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•—â–ˆâ–ˆâ–ˆâ•—   â–ˆâ–ˆâ•—â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•—[/]
...
# ASCII Art - Zed Caduceus (compact, fits in left panel)
ZED_CADUCEUS = """[#CD7F32]â €â €â €â €â €â €â €â €â €â €â¢€â£€â¡€â €â£€â£€â €â¢€â£€â¡€â €â €â €â €â €â €â €â €â €â €[/]
...
[#B8860B]â €â €â €â €â €â €â €â €â €â €â €â €â ˆâ ³â ˆâ£¡â žâ â €â €â €â €â €â €â €â €â €â €â €â €[/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "âš• NOUS ZED - AI Agent Framework"
        tiny_line = "âš• NOUS ZED"
    else:
        agent_name = _skin.get_branding("agent_name", "Zed Agent") if _skin else "Zed Agent"
    ...
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Zed Team[/]\n"
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.zed/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.zed/config.yaml on first use)
...
class ZedCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    Interactive CLI for the Zed Agent.
    ...
    def __init__(
        ...
        # Read max_tokens from config (env var override: ZED_MAX_TOKENS)
        ...
        self.requested_provider = (
            ...
            or os.getenv("ZED_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("ZED_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("ZED_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `zed chat --ignore-rules` in zed_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("ZED_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("ZED_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        try:
            ...
            # persisted to state.db â€” the live chat looks healthy but resume
            ...
            logger.warning("Failed to initialize SessionDB â€” session will NOT be indexed for search: %s", e)
            try:
                ...
                Console(stderr=True).print(
                    "[bold yellow]âš  Session store unavailable[/bold yellow] â€” "
                    ...
                    f"  Reason: {e}\n"
                    "  Fix the state.db store (e.g. `zed update` to rebuild the venv) to restore persistence."
                )
            except Exception:
                # Never let the warning path itself break startup.
                print(
                    "WARNING: Session store unavailable â€” this conversation will NOT be "
        ...
        # it's shared across all Zed processes for this ZED_HOME.
        ...
        self._history_file = _zed_home / ".zed_history"
        ...
        # width change (column reflow â†’ possible ghost chrome, needs a viewport
    ...
    def _invalidate(self, min_interval: float = 0.25) -> None:
        ...
        repaints that can fire many times per second â€” the throttle prevents
        ...
        within the 250ms window â€” or an in-flight resize â€” silently drop it, so
    ...
    def _paint_now(self) -> None:
        ...
        bypasses the ``_invalidate`` throttle and resize-recovery guard â€” a
        modal the user is actively waiting on must never be dropped â€” mirroring
    ...
    def _recover_after_resize(self, app, original_on_resize) -> None:
        ...
        â€” the duplicated-status-bar report (two bars, two elapsed readings).
        ...
        *old* one, so the ghost survives the whole suppression window.
        ...
        we do not send here â€” ``rebuild_scrollback=False``), so the startup
        ...
        *submitted* user input, so a resize/reflow (tmux pane change, SSH
        ...
        # the SAME recovery: wipe the visible viewport (banner-safe â€” CSI 2J
        # only, never CSI 3J) and replay the transcript so nothing is lost.
        # Row-count-only changes skip this (no reflow â†’ no ghost) to avoid an
    ...
    def _format_idle_since(last_finished_at: Optional[float], turn_live: bool) -> str:
        ...
        Compact read-out: ``âœ“ 42s`` / ``âœ“ 3m`` / ``âœ“ 1h 12m``.
        """
        if turn_live or last_finished_at is None:
            return ""
        idle = max(0.0, time.time() - last_finished_at)
        return f"âœ“ {format_duration_compact(idle)}"
    ...
    def _get_voice_status_fragments(self, width: Optional[int] = None):
        ...
        if compact:
            return [("class:voice-status", f" ðŸŽ¤ {label} ")]
        tts = " | TTS on" if self._voice_tts else ""
        cont = " | Continuous" if self._voice_continuous else ""
        return [("class:voice-status", f" ðŸŽ¤ Voice mode{tts}{cont}  â€”  {label} to record ")]
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"âš• {self.model if getattr(self, 'model', None) else 'Zed'}"
    ...
    def _on_notice(self, notice) -> None:
        ...
        per-turn _capture_credits after the API call) â€” printing immediately races the
    ...
    def _on_notice_clear(self, key: str) -> None:
        ...
        it's a no-op for rendering â€” kept so the agent's clear callback is bound
    ...
    def _format_submitted_user_message_preview(self, user_input: str) -> str:
        ...
        if len(lines) <= 1:
            return f"[bold {_accent_hex()}]â—[/] [bold]{_escape(user_input)}[/]{ts_suffix}"
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - ZedCLI._status_bar_display_width(label)
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``zed doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``zed doctor`` so the banner stays
        small.
        """
        try:
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Zed needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        if is_nous_zed_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]âš   Zed Team Zed 3 & 4 models are NOT agentic and are not "
                "designed for use with Zed Agent.[/]"
            )
            self._console_print(
    ...
    def _restore_session_cwd(self, session_meta: dict, *, quiet: bool = False) -> None:
        ...
        and relative-path resolution all land in the same place â€” the local
        ...
        crash â€” repos get moved and deleted.
        """
        ...
        if current and os.path.realpath(recorded) == os.path.realpath(current):
            return  # Already where the session lived â€” nothing to announce.

        if not os.path.isdir(recorded):
            msg = f"âš  Session's working directory is gone: {recorded} â€” staying in {current or '.'}"
            if quiet:
        ...
        try:
            ...
            msg = f"âš  Could not enter session's working directory {recorded}: {e}"
            if quiet:
                print(msg, file=sys.stderr)
            else:
                self._console_print(f"[dim]{_escape(msg)}[/dim]")
        ...
        msg = f"â†» Working directory: {recorded}"
        if quiet:
            print(msg, file=sys.stderr)
        else:
            self._console_print(f"[dim]{_escape(msg)}[/dim]")
    ...
    def _preprocess_images_with_vision(self, text: str, images: list, *, announce: bool = True) -> str:
        ...
        for img_path in images:
            ...
            if announce:
                _cprint(f"  {_DIM}ðŸ‘ï¸  analyzing {img_path.name} ({size_kb}KB)...{_RST}")
            try:
                ...
                if result.get("success"):
                    ...
                    if announce:
                        _cprint(f"  {_DIM}âš  vision analysis failed â€” path included for retry{_RST}")
            except Exception as e:
                enriched_parts.append(
                ...
                if announce:
                    _cprint(f"  {_DIM}âš  vision analysis error â€” path included for retry{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                self._console_print()
                self._console_print("[yellow]âš ï¸  Some tools disabled (missing API keys):[/]")
                for item in api_key_missing:
                    tools_str = ", ".join(item["tools"][:2])  # Show first 2 tools
                    if len(item["tools"]) > 2:
                        tools_str += f", +{len(item['tools'])-2} more"
                    self._console_print(f"   [dim]â€¢ {item['name']}[/] [dim italic]({', '.join(item['missing_vars'])})[/]")
                self._console_print("[dim]   Run 'zed setup' to configure[/]")
        except Exception:
    ...
    def _show_status(self):
        ...
        provider_info = f" [dim {separator_color}]Â·[/] [dim]provider: {self.provider}[/]"
        if self._provider_source:
            provider_info += f" [dim {separator_color}]Â·[/] [dim]auth: {self._provider_source}[/]"
    ...
    def show_help(self):
        ...
        if quick_commands:
            _cprint(f"\n  âš¡ {_BOLD}Quick Commands{_RST} ({len(quick_commands)} configured):")
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Zed!{_RST}")
    ...
    def show_tools(self):
        ...
        print(f"  Total: {len(tools)} tools  ãƒ½(^o^)ãƒŽ")
        print()
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        print()
        if reason == "history":
            print("(._.) No messages in the current chat yet â€” here are recent sessions you can resume:")
        else:
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        ``/resume`` and ``zed sessions list``. Delegates the
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self._session_db and old_session_id:
            ...
            # when _flush_messages_to_session_db() has not yet run â€” without
            ...
            # /resume and `zed sessions list` (gemini-cli#27770 port).
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("ZED_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
                ...
                if title and self._session_db:
                    ...
                    if sanitized:
                        ...
                        _cprint("  Title is empty after cleanup â€” session started untitled.")
                        title = None
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.zed/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``zed --resume <id>``
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: zed --resume {self.session_id}")
        except Exception as e:
    ...
    def _prompt_text_input(self, prompt_text: str) -> str | None:
        ...
        # thread, so cancel cleanly (None) instead of hanging â€” mirrors the
    ...
    def _prompt_text_input_modal(
        ...
        **Platform note (Windows â€” issue #33961):**
        ...
        the main thread and deadlocked against prompt_toolkit's stdin ownership â€”
    ...
    def _get_slash_confirm_display_fragments(self):
        ...
        for idx, (_value, label, desc) in enumerate(choices):
            ...
            preview_lines.extend(_wrap_panel_text(f"{marker} [{idx + 1}] {label} â€” {desc}", 72, subsequent_indent="    "))
        ...
        for idx, (_value, label, desc) in enumerate(choices):
            marker = "â¯" if idx == selected else " "
            for wrapped in _wrap_panel_text(f"{marker} [{idx + 1}] {label} â€” {desc}", inner_text_width, subsequent_indent="    "):
        ...
        _append_blank_panel_line(lines, 'class:approval-border', box_width)
        _append_panel_line(lines, 'class:approval-border', 'class:approval-cmd', 'Type 1/2/3 or use â†‘/â†“ then Enter. ESC/Ctrl+C cancels.', box_width)
        lines.append(('class:approval-border', 'â•°' + ('â”€' * box_width) + 'â•¯\n'))
        return lines
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        choices = [
            ("once", "Switch anyway", "Use this model for the current Zed session."),
    ...
    def _confirm_and_apply_model_switch_result(self, result, persist_global: bool) -> None:
        try:
            ...
            _cprint(f"  âœ— Model selection failed: {exc}")
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        if self.agent is not None:
            try:
                ...
                _cprint(f"  âš  Agent swap failed ({exc}); change applied to next session.")
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `zed model` and gateway pickers).
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        Usage:
            ...
            /codex-runtime auto                  â€” Zed default (chat_completions)
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_zed_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_zed_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        if canonical in {"quit", "exit"}:
            ...
            if _args in {"--delete", "-d"}:
                ...
                _cprint(f"  {_DIM}âœ— Unknown argument: {_escape(_args)}. Use /exit --delete to also remove session history.{_RST}")
                return True
            return False
        elif canonical == "help":
            self.show_help()
            ...
            if self._confirm_destructive_slash(
                ...
                return True  # confirmation cancelled â€” command handled, keep REPL alive
            ...
            if len(_undo_parts) > 1:
                try:
                    ...
                    print(f"(._.) Invalid count {_undo_parts[1]!r} â€” use /undo or /undo N.")
                    return
                if _undo_n < 1:
                    _undo_n = 1
            _undo_desc = (
            ...
            try:
                # Discover from disk (bundled + user), matching `zed plugins
                # list` â€” so installed-but-not-enabled plugins are visible here
                ...
                # `/plugins` is a quick glance â€” default to user-installed
                # plugins (what the user actually added). Bundled provider/
                # platform plugins are summarized on one line; the full
                # catalog lives behind `zed plugins list`.
                user_entries = [e for e in entries if e[3] != "bundled"]
                bundled_count = len(entries) - len(user_entries)

                if not user_entries:
                    print("No user plugins installed.")
                    print("  Install one: zed plugins install owner/repo")
                    print(f"  Or drop a plugin directory into {display_zed_home()}/plugins/")
                    if bundled_count:
                        print(f"  ({bundled_count} bundled plugins available â€” see: zed plugins list)")
                else:
                    ...
                    for name, version, _desc, source, _dir, key in sorted(user_entries):
                        ...
                        glyph = {"enabled": "âœ“", "disabled": "âœ—"}.get(state, "â—‹")
                        ...
                        error = f" â€” {info['error']}" if info.get("error") else ""
                        print(f"  {glyph} {name}{ver}{label}{detail}{error}")
                    if bundled_count:
                        print(f"  (+{bundled_count} bundled â€” see: zed plugins list)")
                    print("  Enable/disable: zed plugins enable/disable <name>")
            except Exception as e:
            ...
            if not payload:
                ...
                try:
                    ...
                    if accepted:
                        _cprint(f"  â© Steer queued â€” arrives after the next tool call: {payload[:80]}{'...' if len(payload) > 80 else ''}")
                    else:
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``ZED_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``ZED_YOLO_MODE`` here â€” that env var is read once at module import
        ...
        * ``/compress here [N]`` â€” boundary-aware compression. Summarize
        ...
        with self._busy_command("Compressing context..."):
            try:
                ...
                if partial:
                    print(f"ðŸ—œï¸  Summarizing up to here: compressing {len(head)} of "
                          f"{original_count} messages (~{approx_tokens:,} tokens), "
                          f"keeping last {keep_last} exchange(s) verbatim...")
                elif focus_topic:
                    print(f"ðŸ—œï¸  Compressing {original_count} messages (~{approx_tokens:,} tokens), "
                          f"focus: \"{focus_topic}\"...")
                else:
                    print(f"ðŸ—œï¸  Compressing {original_count} messages (~{approx_tokens:,} tokens)...")
                ...
                icon = "ðŸ—œï¸" if summary["noop"] else "âœ…"
    ...
    def _show_usage(self):
        ...
        with no live agent â€” important for the TUI, where /usage runs in a slash-worker
        ...
        print("  ðŸ“Š Session Token Usage")
        print(f"  {'â”€' * 40}")
        ...
        # Nous credits magnitudes + monthly-grant gauge (agent-independent â€” also
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by zed_logging not
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        Delegates to the shared ``agent.account_usage.nous_credits_lines`` helper â€”
        ...
        wall-clock-bounded inside the helper; also honors ZED_DEV_CREDITS_FIXTURE
        for offline testing â€” same behavior as every other surface.
        """
    ...
    def _show_credits(self):
        """`/credits` â€” focused Nous credit balance + top-up handoff.

        Interactive CLI: balance block + identity line + a 3-button panel
        (Open top-up / Copy link / Cancel). Non-interactive contexts â€” the TUI
        slash-worker subprocess and any place without a live prompt_toolkit app
        (``self._app is None``) â€” render a text variant (balance + tappable
        ...
        if not view.logged_in:
            print()
            _cprint(f"  ðŸ’³ {_d('Not logged into Nous Portal.')}")
            print("  Run `zed portal` to log in, then /credits.")
            return

        print()
        print("  ðŸ’³ Nous credits")
        print(f"  {'â”€' * 41}")
        for line in view.balance_lines:
            # Drop the helper's own "ðŸ“ˆ Nous credits" header â€” we print our own.
        ...
        # prompt_toolkit modal can't run here â€” it would read the worker's
        ...
        if not getattr(self, "_app", None):
            ...
            print("  Complete your top-up in the browser â€” credits will appear in /credits shortly.")
            return
        ...
        if choice == "open":
            ...
            try:
                ...
                print(f"  ðŸ“‹ Copied: {view.topup_url}")
            except Exception:
                print(f"  Top-up URL: {view.topup_url}")
        else:
            print("  ðŸŸ¡ Cancelled. No credits added.")

    # ------------------------------------------------------------------
    # /billing â€” Phase 2b terminal billing (CLI surface, all 5 screens)
    # ------------------------------------------------------------------

    def _show_billing(self, command: str = "/billing"):
        """`/billing` â€” terminal billing for Nous (one interactive modal).
        ...
        UX spec Â§0.4 â€” ``/billing buy`` etc. are gone; we don't error on a stray
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  ðŸ’³ {_d(_msg)}")
            else:
                _cprint(f"  ðŸ’³ {_d('Not logged into Nous Portal.')}")
                print("  Run `zed portal` to log in, then /billing.")
            return

        # Any sub-arg is intentionally ignored â€” always open the menu.
    ...
    def _billing_overview(self, state):
        """Screen 1 â€” overview: balance, spend bar, role-gated action menu."""
        from agent.billing_view import format_money

        print()
        _cprint(f"  ðŸ’³ {_b('Usage credits')}")
        print(f"  {'â”€' * 41}")
        ...
        if ar is not None:
            if ar.enabled:
                print(
                    f"  Auto-reload: on â€” below {format_money(ar.threshold_usd)} "
                    f"â†’ reload to {format_money(ar.reload_to_usd)}"
                )
            else:
        ...
        if state.org_name:
            ...
            _org_line = f"Org: {state.org_name}{f' Â· {role}' if role else ''}"
        ...
        # Optimistic funnel: no card on file â†’ a charge will 403 no_payment_method.
        # Surface that up front (with the portal link) but DON'T hide Buy â€” /state.card
        # can't fully prove CLI-chargeability, so we advise rather than gate.
        if state.card is None:
            _cprint(
                f"  {_d('No saved card for terminal charges yet â€” set one up on the portal first.')}"
            )
            self._billing_portal_hint(state)

        # Non-interactive (slash-worker / no live app): no modal, no sub-command
        # advertising â€” just the portal funnel (the URL is the affordance).
        if not getattr(self, "_app", None):
            self._billing_portal_hint(state)
            return
        ...
        # present the action menu â€” repeating the title/balance reads as a dupe.
        raw = self._prompt_text_input_modal(
            title="ðŸ’³ Choose an action", detail="",
            choices=choices,
        )
        choice = self._normalize_slash_confirm_choice(raw, choices)
        if choice == "buy":
    ...
    def _billing_spend_bar(self, spent, limit, *, cells: int = 10):
        """Render a 10-cell `â–ˆ`/`â–‘` spend bar + integer percent from spent/limit.

        Returns ``(bar, pct)`` where ``bar`` is like ``[â–ˆâ–ˆâ–ˆâ–ˆâ–‘â–‘â–‘â–‘â–‘â–‘]`` and ``pct``
    ...
    def _billing_buy_flow(self, state):
        ...
        preset_choices.append(("custom", "Custom amountâ€¦", "enter your own amount"))
        preset_choices.append(("cancel", "Cancel", "do nothing"))

        card = state.card
        detail = f"Payment: {card.masked}" if card else "No saved card on file"
        raw = self._prompt_text_input_modal(
            title="ðŸ’³ Buy usage credits", detail=detail, choices=preset_choices,
        )
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        print(f"  {'â”€' * 41}")
        _consent = (
            "By confirming, you allow Zed Team to charge your card."
        )
        ...
        if not getattr(self, "_app", None):
            ...
            return
        raw = self._prompt_text_input_modal(
            title=f"ðŸ’³ Pay {format_money(amount)}?",
        ...
        if not charge_id:
            print("  ðŸ”´ No charge id returned; please check the portal.")
            return
        _cprint(f"  {_d('Charge submitted â€” confirming settlementâ€¦')}")
    ...
    def _billing_poll_charge(self, state, charge_id, amount):
        ...
        while _time.time() < deadline:
            try:
                ...
                # Retry-after, NOT a failure â€” back off and keep polling.
                wait = exc.retry_after or 5
                _time.sleep(min(wait, 30))
                continue
            except BillingError as exc:
                print(f"  ðŸ”´ Could not check the charge: {exc}")
                return
            ...
            if state_str == "settled":
                ...
                print(f"  âœ… {shown} in credits added.")
                return
            if state_str == "failed":
                self._billing_render_charge_failed(state, status.get("reason"))
                return
            # pending â†’ wait and poll again
            _time.sleep(interval)

        # Past the cap with no terminal state = timeout (not an error).
        print(f"  ðŸŸ¡ Still processing after 5 minutes â€” this is a timeout, not a "
    ...
    def _billing_render_charge_failed(self, state, reason):
        ...
        if reason == "authentication_required":
            print("  ðŸ”´ Your bank requires verification (3DS). Complete it on the "
                  "portal to finish this purchase.")
        elif reason == "payment_method_expired":
            print("  ðŸ”´ Your card has expired. Update it on the portal.")
        elif reason == "card_declined":
            print("  ðŸ”´ Your card was declined. Try another card on the portal.")
        else:
            print(f"  ðŸ”´ The charge didn't go through ({reason or 'processing_error'}).")
        self._billing_portal_hint(state)
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        if code == "no_payment_method":
            print("  ðŸ’³ No saved card for terminal charges yet. Set one up on the "
                  "portal (one-time credit buys don't save a reusable card).")
        elif code == "cli_billing_disabled":
            print("  ðŸ”´ Terminal billing is turned off for this org â€” an admin must enable it on the portal.")
        elif code == "monthly_cap_exceeded":
            remaining = (getattr(exc, "payload", {}) or {}).get("remainingUsd")
            if remaining is not None:
                print(f"  ðŸ”´ Monthly spend cap reached â€” ${remaining} headroom left.")
            else:
                print("  ðŸ”´ Monthly spend cap reached.")
        elif isinstance(exc, BillingRateLimited):
            wait = getattr(exc, "retry_after", None)
            mins = f" (try again in ~{max(1, round(wait / 60))} min)" if wait else ""
            print(f"  ðŸŸ¡ Too many charges right now{mins}. This isn't a payment failure.")
        else:
            print(f"  ðŸ”´ {exc}")
        if portal_url:
    ...
    def _billing_handle_scope_required(self, state):
        """403 insufficient_scope â†’ lazy step-up re-auth (plan D-A)."""
        print()
        print("  ðŸ’³ Terminal billing needs an extra permission (billing:manage).")
        _scope_msg = (
        ...
        if not getattr(self, "_app", None):
            print("  Run `zed portal` and approve terminal billing, then retry.")
            return
        confirm_choices = [
        ...
        raw = self._prompt_text_input_modal(
            title="ðŸ’³ Grant terminal billing access?",
        ...
        try:
            ...
            print(f"  ðŸ”´ Re-authorization failed: {exc}")
            return
        if granted:
            print("  âœ… Billing permission granted.")
            ...
            if fresh.logged_in and fresh.cli_billing_enabled:
                ...
                print("  ðŸŸ¡ Permission granted, but terminal billing is still turned "
                      "off for this org. Enable it in the portal, then run /billing again.")
                self._billing_portal_hint(fresh)
        else:
            print("  ðŸŸ¡ Terminal billing was not granted (an admin must tick the box).")

    def _billing_auto_reload_flow(self, state):
        """Screen 4 â€” auto-reload config: threshold + reload-to â†’ PATCH.
        ...
        print()
        _cprint(f"  ðŸ’³ {_b('Auto-reload')}")
        print(f"  {'â”€' * 41}")
        _cprint(f"  {_d('Automatically buy more credits when your balance is low.')}")
        if card:
            print(f"  Card on file: {card.masked}")
        else:
            print("  No saved card â€” set one up on the portal first.")
            self._billing_portal_hint(state)
            return
        if currently_on:
            print(
                f"  Currently: below {format_money(ar.threshold_usd)} â†’ "
        ...
        if currently_on:
            ...
            raw = self._prompt_text_input_modal(
                title="ðŸ’³ Auto-reload",
                detail=(
                    f"On â€” below {format_money(ar.threshold_usd)} â†’ "
                    f"reload to {format_money(ar.reload_to_usd)}"
                ),
                choices=top_choices,
            )
        ...
        # Field 1 â€” threshold (prefilled when editing an existing config).
        ...
        # Field 2 â€” reload-to (prefilled when editing an existing config).
        ...
        if reload_amt is None or threshold_amt is None or reload_amt <= threshold_amt:
            print("  ðŸ”´ Reload-to amount must be greater than the threshold.")
            return

        print()
        _ar_consent = (
            f"By confirming, you authorize Zed Team to charge {card.masked} "
        ...
        raw = self._prompt_text_input_modal(
            title="ðŸ’³ Turn on auto-reload?",
            detail=f"Below {format_money(threshold_amt)} â†’ reload to {format_money(reload_amt)}",
            choices=confirm_choices,
        )
        choice = self._normalize_slash_confirm_choice(raw, confirm_choices)
        if choice != "agree":
        ...
        try:
            ...
            self._billing_render_charge_error(state, exc)
            return
        print(f"  âœ… Auto-reload on: below {format_money(threshold_amt)} â†’ "
    ...
    def _billing_auto_reload_disable(self, state):
        ...
        try:
            ...
            self._billing_render_charge_error(state, exc)
            return
        print("  âœ… Auto-reload turned off.")

    def _billing_limit_screen(self, state):
        """Screen 5 â€” monthly spend limit (read-only; cap is portal-only)."""
        from agent.billing_view import format_money

        print()
        _cprint(f"  ðŸ’³ {_b('Monthly spend limit')}")
        print(f"  {'â”€' * 41}")
        cap = state.monthly_cap
        ...
        _limit_note = (
            "The monthly limit is set on the portal â€” the terminal shows "
            "it read-only."
        )
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        print()
        print("ðŸ”„ MCP server config changed â€” reloading connections...")
        ...
        if _reload_thread.is_alive():
            print("  âš ï¸  MCP reload timed out (30s). Some servers may not have reconnected.")
    ...
    # â€” lets users self-serve without flipping approvals.destructive_slash_confirm
    # in config. (Native Windows now drives the modal normally â€” see #33961.)
    ...
    def _confirm_destructive_slash(
        ...
        drives the modal normally â€” see #33961). Callers are responsible
        ...
        if raw is None:
            print(f"ðŸŸ¡ /{command} cancelled (no input).")
            return None
        choice = self._normalize_slash_confirm_choice(raw, choices)
        if choice is None:
            print(f"ðŸŸ¡ Unrecognized choice '{raw}'. /{command} cancelled.")
            return None

        if choice == "cancel":
            print(f"ðŸŸ¡ /{command} cancelled. Conversation unchanged.")
            return None

        if choice == "always":
            if save_config_value("approvals.destructive_slash_confirm", False):
                print("ðŸ”’ Future /clear, /new, /reset, and /undo will run without confirmation.")
    ...
    def _confirm_and_reload_mcp(self, cmd_original: str = "") -> None:
        ...
        if raw is None:
            print("ðŸŸ¡ /reload-mcp cancelled (no input).")
            return
        choice = self._normalize_slash_confirm_choice(raw, choices)
        if choice is None:
            print(f"ðŸŸ¡ Unrecognized choice '{raw}'. /reload-mcp cancelled.")
            return

        if choice == "cancel":
            print("ðŸŸ¡ /reload-mcp cancelled. MCP tools unchanged.")
            return

        if choice == "always":
            if save_config_value("approvals.mcp_reload_confirm", False):
                print("ðŸ”’ Future /reload-mcp calls will run without confirmation.")
    ...
    def _reload_mcp(self):
        ...
        try:
            ...
            if not self._command_running:
                print("ðŸ”„ Reloading MCP servers...")
            ...
            if added:
                print(f"  âž• Added: {', '.join(sorted(added))}")
            if removed:
                print(f"  âž– Removed: {', '.join(sorted(removed))}")
            if not connected_servers:
                print("  No MCP servers connected.")
            else:
                print(f"  ðŸ”§ {len(new_tools)} tool(s) available from {len(connected_servers)} server(s)")
            ...
            # (name-diff, thread-safe, and â€” critically â€” additive-preserving so
            ...
            print(f"  âœ… Agent updated â€” {len(self.agent.tools if self.agent else [])} tool(s) available")

        except Exception as e:
            print(f"  âŒ MCP reload failed: {e}")

    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.zed/skills/ and queue a note for the
        next user turn.
        ...
        try:
            ...
            if not added and not removed:
                ...
                print(f"  ðŸ“š {total} skill(s) available")
                return
            ...
            if added:
                print("  âž• Added Skills:")
                for item in added:
                    print(f"  {_fmt_line(item)}")
            if removed:
                print("  âž– Removed Skills:")
                for item in removed:
                    print(f"  {_fmt_line(item)}")
            print(f"  ðŸ“š {total} skill(s) available")
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "zed_voice"), exist_ok=True)
    ...
    def _persist_prompt_summary(self, icon: str, label: str, detail: str, outcome: str) -> None:
        ...
        if len(outcome) > 120:
            outcome = outcome[:119] + "â€¦"
        _cprint(f"\n{_DIM}{icon} {label}: {detail} â†’ {outcome}{_RST}")
    ...
    def _clarify_callback(self, question, choices):
        ...
        # _invalidate throttle / resize guard â€” see _paint_now / _invalidate (#41098).
        self._paint_now()
        ...
        # while we wait. Selection changes (â†‘/â†“) trigger instant repaints via
        # the key bindings.
        _last_countdown_refresh = _time.monotonic()
        while True:
            try:
    ...
    def _sudo_password_callback(self) -> str:
        ...
        # Modal prompt â€” paint immediately, bypassing the throttle/resize guard
    ...
    def _approval_callback(self, command: str, description: str,
        ...
        with self._approval_lock:
            ...
            # Modal prompt â€” paint immediately, bypassing the throttle/resize
    ...
    def _submit_secret_response(self, value: str) -> None:
        ...
        self._secret_state = None
        self._secret_deadline = 0
        # Modal teardown â€” paint directly so the secret panel clears at once and
            ...
            if _img_mode == "native":
                try:
                    ...
                    if any(p.get("type") == "image_url" for p in _parts):
                        ...
                        _cprint(
                            f"  {_DIM}ðŸ“Ž attaching {len(images)} image(s) natively "
        ...
        try:
            ...
            def run_agent():
                ...
                try:
                    ...
                    # seed / per-turn capture) now that the response is done â€” printing
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _zed_home / "interrupt_debug.log"
    ...
    def _clear_terminal_on_exit(self):
        ...
        prompt_toolkit has torn down its renderer + restored terminal modes â€”
        ...
        try:
            ...
            pass
        # Fallback: shell clear command (rarely needed â€” escapes work on every
    ...
    def _print_exit_summary(self):
        ...
        # visible screen, ESC[H homes the cursor â€” so the summary prints at a
        ...
        if msg_count > 0:
            ...
            print(f"  zed --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  zed -c \"{session_title}\"{profile_flag}")
            print()
    ...
    def _get_tui_prompt_fragments(self):
        ...
        if self._sudo_state:
            return _state_fragment("class:sudo-prompt", "ðŸ”")
        ...
        if self._clarify_freetext:
            return _state_fragment("class:clarify-selected", "âœŽ")
        ...
        if self._agent_running:
            return _state_fragment("class:prompt-working", "âš•")
        if self._voice_mode:
            return _state_fragment("class:voice-prompt", "ðŸŽ¤")
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = "Welcome to Zed Agent! Type your message or /help for commands."
        ...
        try:
            ...
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    ...
                    f"(ZED_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClawâ†’Zed migration (especially migrations done by
        ...
        try:
            ...
            pass  # Tips are non-critical â€” never break startup
                    ...
                    if _effective_mode == "steer":
                        ...
                        if images or not text:
                            ...
                            try:
                                ...
                                _cprint(f"  {_DIM}Steer failed ({exc}) â€” queued for next turn.{_RST}")
                                accepted = False
                            if accepted:
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Zed binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        # Dynamic prompt: shows Zed symbol when agent is working,
        ...
        input_area = TextArea(
            ...
            # completer does blocking work â€” fuzzy @-file indexing shells out to
            # rg/fd (up to a 2s timeout) and path completion hits os.listdir/stat
            # â€” so running it inline would stall the render loop on each key (very
        ...
        def get_hint_text():
            ...
            if cli_ref._command_running:
                frame = cli_ref._command_spinner_frame()
                return [
                    ('class:hint', f'  {frame} command in progress Â· input temporarily disabled'),
                ]
        ...
        def _get_clarify_display():
            ...
            if other_num < 10:
                ...
                other_num_prefix = ' '
            other_label = (
                f"â¯ {other_num_prefix}. Other (type below)" if cli_ref._clarify_freetext
                else f"â¯ {other_num_prefix}. Other (type your answer)" if selected == len(choices)
                else f"  {other_num_prefix}. Other (type your answer)"
            )
            preview_lines.extend(_wrap_panel_text(other_label, 60, subsequent_indent="    "))
            box_width = _panel_box_width("Zed needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Zed needs your input'))
            lines.append(('class:clarify-border', ' ' + ('â”€' * max(0, box_width - len("Zed needs your input") - 3)) + 'â•®\n'))
            if not use_compact_chrome:
        ...
        def _get_sudo_display():
            ...
            lines.append(('class:sudo-border', ' ' + ('â”€' * max(0, box_width - len(title) - 3)) + 'â•®\n'))
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = ZedCLI._compute_model_picker_viewport(
            ...
            lines.append(('class:clarify-border', ' ' + ('â”€' * max(0, box_width - len(title) - 3)) + 'â•®\n'))
            _append_blank_panel_line(lines, 'class:clarify-border', box_width)
        ...
        # Image attachment indicator â€” shows badges like [ðŸ“Ž Image #1] above input
        cli_ref = self
        ...
        app = Application(
            ...
            # Set to 0 to suppress background redraws entirely â€” avoids
            ...
            # summary â€” so a dead status bar + empty prompt sit between the
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_zed_osd_patched", False):
                ...
                _pt_renderer._zed_osd_patched = True
        except Exception:
            pass
                ...
                try:
                    ...
                    if _file_drop:
                        ...
                        if _file_drop["is_image"]:
                            ...
                            _cprint(f"  ðŸ“Ž Auto-attached image: {_drop_path.name}")
                        else:
                            _cprint(f"  ðŸ“„ Detected file: {_drop_path.name}")
                            user_input = (
                    ...
                    if submit_images:
                        n = len(submit_images)
                        _cprint(f"  {_DIM}ðŸ“Ž {n} image{'s' if n > 1 else ''} attached{_RST}")
                    ...
                    try:
                        ...
                        try:
                            ...
                            pass  # Non-fatal â€” don't break the main loop
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``ZED_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("ZED_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the zed process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: zed setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                # drop the empty row so /resume and `zed sessions list`
                ...
                if getattr(self, '_delete_session_on_exit', False):
                    try:
                        ...
                        if self._session_db.delete_session(_sid, sessions_dir=_sessions_dir):
                            ...
                            _cprint(f"  {_DIM}âœ— Session {_escape(_sid)} not found for deletion{_RST}")
                    except (Exception, KeyboardInterrupt) as e:
...
def _run_kanban_goal_loop_q(cli: "ZedCLI", first_response: str) -> None:
    """Drive a kanban goal_mode worker through the Ralph-style goal loop.

    Called from the quiet single-query path AFTER the worker's first turn,
    only when ``ZED_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("ZED_KANBAN_TASK") or "").strip()
    if not task_id:
        return
...
def main(
    ...
    """
    Zed Agent CLI - Interactive AI Assistant
    ...
    if gateway:
        ...
        print("Starting Zed Gateway (messaging platforms)...")
    ...
    # Default to zed-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
        ...
        # Coding posture (base Zed): with no explicit --toolsets, collapse
    ...
    # mode registers its own inside ZedCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  ZED_SIGTERM_GRACE overrides the 1.5 s
    ...
    def _signal_handler_q(signum, frame):
        ...
        if os.environ.get("ZED_KANBAN_TASK"):
            try:
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``zed chat -q "work kanban task <id>"``;
            ...
            if quiet:
                ...
                if cli._ensure_runtime_credentials():
                    ...
                    if cli._init_agent(
                        ...
                        # machine-readable (no styled "Zed" box, no tool-gen
                        ...
                        if os.environ.get("ZED_KANBAN_GOAL_MODE") == "1":
                            try:
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            _exit_code = 1
                            if os.environ.get("ZED_KANBAN_TASK") and result.get(
                                "failure_reason"
                ...
                # Single-query mode (`zed chat -q "â€¦"`): skip the welcome
