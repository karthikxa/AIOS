...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_zed_session_injector", False):
    ...
    _session_record_factory._zed_session_injector = True  # type: ignore[attr-defined]
...
def setup_logging(
    ...
    *force* is ``True``.
    ...
    ----------
    zed_home
        Override for the Zed home directory.  Falls back to
        ``get_zed_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_zed_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._zed_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from zed_cli.config import is_managed
