...
tracked in a :class:`contextvars.ContextVar` so handlers â€” including those
dispatched onto the worker pool â€” route their writes to the right peer.
...
_DISABLE_FLUSH = (os.environ.get("ZED_TUI_GATEWAY_NO_FLUSH", "") or "").strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
...
class StdioTransport:
    ...
    underlying stream continue to work â€” this preserves the behaviour the
    ...
    def write(self, obj: dict) -> bool:
        ...
        â€” ``entry.py`` calls ``sys.exit(0)`` when ``write_json`` reports
        ...
        with self._lock:
            ...
            try:
                ...
                # else â€” including UnicodeEncodeError, which is a
                # ValueError subclass for misconfigured locales â€”
            ...
            # a half-closed pipe holds the lock until it returns â€” see
...
class TeeTransport:
    ...
    The primary's return value (and exceptions) determine the result â€”
