...
The dashboard's `/api/pty` spawns `zed --tui` as a child process, which
spawns its own ``tui_gateway.entry``.  Tool/reasoning/status events fire on
*that* gateway's transport â€” three processes removed from the dashboard
...
already passes to ``write``).  No JSON-RPC envelope here â€” the dashboard's
