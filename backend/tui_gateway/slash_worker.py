"""Persistent slash-command worker â€” one ZedCLI per TUI session.
...
def _env_float(name: str, default: float) -> float:
    ...
    import time on a typo (e.g. ``ZED_SLASH_WATCHDOG_POLL_S=2s``) and kill
...
_WATCHDOG_POLL_S = max(0.05, _env_float("ZED_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, _env_float("ZED_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _run(cli: ZedCLI, command: str) -> str:
...
def main():
    ...
    os.environ["ZED_SESSION_KEY"] = args.session_key
    os.environ["ZED_INTERACTIVE"] = "1"

    # Start before the (hundreds-of-ms) ZedCLI build â€” that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = ZedCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
