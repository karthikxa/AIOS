...
load_zed_dotenv(
    zed_home=_zed_home, project_env=Path(__file__).parent.parent / ".env"
)


# â”€â”€ Panic logger â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# appends every unhandled exception to ~/.zed/logs/tui_gateway_crash.log
# AND re-emits a one-line summary to stderr so the TUI can surface it in
# Activity â€” exactly what was missing when the voice-mode turns started
# exiting the gateway mid-TTS.
_CRASH_LOG = os.path.join(_zed_home, "logs", "tui_gateway_crash.log")
...
def _panic_hook(exc_type, exc_value, exc_tb):
    ...
    try:
        ...
        with open(_CRASH_LOG, "a", encoding="utf-8") as f:
            f.write(
                f"\n=== unhandled exception Â· {time.strftime('%Y-%m-%d %H:%M:%S')} ===\n"
            )
            f.write(trace)
    except Exception:
        pass
    # Stderr goes through to the TUI as a gateway.stderr Activity line â€”
...
def _thread_panic_hook(args):
    ...
    try:
        ...
        with open(_CRASH_LOG, "a", encoding="utf-8") as f:
            f.write(
                f"\n=== thread exception Â· {time.strftime('%Y-%m-%d %H:%M:%S')} "
                f"Â· thread={args.thread.name} ===\n"
            )
            f.write(trace)
    except Exception:
        pass
...
try:
    _slash_timeout = float(os.environ.get("ZED_TUI_SLASH_TIMEOUT_S") or "45")
...
# lingers forever â€” one leaked python process per refresh (#38591 fallout).
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("ZED_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
# â”€â”€ Async RPC dispatch (#12546) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# A handful of handlers block the dispatcher loop in entry.py for seconds
# to minutes (slash.exec, cli.exec, shell.exec, session.resume,
# session.branch, session.compress, skills.manage).  While they're running, inbound RPCs â€”
# notably approval.respond and session.interrupt â€” sit unread in the
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("ZED_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
# Module-level stdio transport â€” fallback sink when no transport is bound via
...
class _SlashWorker:
    """Persistent ZedCLI subprocess for slash commands."""
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    # Use session_id (from agent.session_id) not session_key â€” after compression,
    ...
    # _finalize_session directly â€” without the surrounding _teardown_session /
    # _shutdown_sessions worker.close() â€” can't reintroduce the #38095 leak.
...
def _teardown_session(session: dict | None, *, end_reason: str = "tui_close") -> None:
    ...
    # We deliberately do NOT re-close it here â€” _teardown_session's job beyond
...
def _attach_worker(sid: str, session: dict, worker) -> None:
    """Store worker on session iff sid still maps to it, else close it â€” a
...
def _schedule_ws_orphan_reap(sid: str) -> None:
    ...
    def _reap() -> None:
        ...
        # _sessions_lock â€” consistent with every other _sessions mutator
...
def _close_sessions_for_transport(
    ...
    the single WS-disconnect teardown entry point â€” there is no second
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `zed --tui` keeps real _stdio.
...
# viewing â€” running/pending/starting/live-transport are hard exemptions instead.
try:
    _SESSION_TTL_S = float(os.environ.get("ZED_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `zed --tui`, so it must NOT count as dead here (doing
...
# â”€â”€ Plumbing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _get_db():
    ...
    if _db is None:
        ...
        try:
            ...
            _db_error = str(exc)
            logger.warning(
                "TUI session store unavailable â€” continuing without state.db features: %s",
                exc,
            )
            return None
    return _db
...
# â”€â”€ per-session profile scoping (global remote mode) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# ``profile`` on those calls; we open that profile's db and bind its ZED_HOME
# (a ContextVar override) for the duration of the call so config/skills/model and
# message persistence all resolve to the right profile. Omitted/own profile â†’ the
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_zed_home).resolve():
        return None
    return home if (home / "state.db").exists() or home.exists() else None


# Placeholder ``terminal.cwd`` values that don't name a real directory â€” the
...
def write_json(obj: dict) -> bool:
    ...
    1. Event frames with a session id â†’ the transport stored on that session,
...
def _status_update(sid: str, kind: str, text: str | None = None):
    ...
    # drivers (desktop app) can show an explicit "Summarizingâ€¦" indicator â€”
...
def dispatch(req: dict, transport: Optional[Transport] = None) -> dict | None:
    """Route inbound RPCs â€” long handlers to the pool, everything else inline.

    Returns a response dict when handled inline. Returns None when the
    handler was scheduled on the pool; the worker writes its own response
    via the bound transport when done.

    *transport* (optional): pins every write produced by this request â€”
    including any events emitted by the handler â€” to the given transport.
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `zed` shows the prompt before constructing AIAgent; the TUI used
    ...
    with lock:
        ...
        # An upgrading lazy session is now genuinely mid-construction â€” restore
    ...
    def _build() -> None:
        ...
        try:
            ...
            # ZED_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_zed_home_override(profile_home)
                try:
                    from zed_state import SessionDB
            ...
            try:
                ...
                # id â€” pass it through so the upgrade continues that session
            ...
            # was built without those tools. Catch up once they land â€” see
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    a message â€” abandoned drafts never leave an empty "Untitled" session behind.
    ...
    we don't stamp that onto the row â€” otherwise every session the user never
    ...
    # launch profile's â€” otherwise the row lands in the wrong state.db, the
    ...
    if db is None:
        return
    # The session's own model/effort/fast pick â€” the composer override shipped on
    # session.create, or a restored /model switch â€” must own the row's model +
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from zed_cli.runtime_provider import canonical_custom_identity
...
def _set_session_cwd(session: dict, cwd: str) -> str:
    ...
    # An explicit user choice â€” persist it as the workspace (and let a later
...
# â”€â”€ Config I/O â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


# Keep aligned with `INDICATOR_STYLES` / `DEFAULT_INDICATOR_STYLE` in
# ``ui-tui/src/app/interfaces.ts`` â€” both ends validate against the
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _zed_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_zed_home_override()
        home = override if isinstance(override, str) and override else _zed_home
...
def _apply_managed(cfg: dict) -> dict:
    ...
    zed_cli.config.load_config, so without this a managed skin / reasoning_effort
    / service_tier / provider_routing would be silently ignored here. Read-side
    only â€” the raw user config is what gets cached and saved. Fail-open.
    """
    try:
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["ZED_GATEWAY_SESSION"] = "1"
    os.environ["ZED_EXEC_ASK"] = "1"
    os.environ["ZED_INTERACTIVE"] = "1"


# â”€â”€ Blocking prompt factory â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _clear_pending(sid: str | None = None) -> None:
    ...
    released â€” critical for session.interrupt, which must not
...
# â”€â”€ Agent factory â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


def resolve_skin() -> dict:
    try:
        from zed_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    env = (
        os.environ.get("ZED_MODEL", "")
        or os.environ.get("ZED_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _config_model_target() -> tuple[str, str]:
    ...
    config.yaml wins over ZED_MODEL / ZED_INFERENCE_MODEL here, the
    reverse of `_resolve_model()`'s startup order. Those env vars are a
    provision-time seed (hosted instances set ZED_INFERENCE_MODEL in the
    ...
    never reach an open chat â€” the exact bug this sync exists to fix.
    """
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    model = _resolve_model()
    explicit_provider = os.environ.get("ZED_TUI_PROVIDER", "").strip()
    ...
    try:
        from zed_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("ZED_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _stored_session_runtime_overrides(row: dict | None) -> dict:
    ...
    # ``billing_provider`` is only the billing bucket â€” for a custom endpoint it is the
    ...
    # the resolved billing class, not a routable identity â€” restoring it as the
...
def _runtime_model_config(agent, existing: dict | None = None) -> dict:
    ...
    if provider:
        if provider.strip().lower() == "custom":
            ...
            # string "custom" â€” persisting it loses the entry identity, so a
            ...
            # else from the configured provider â€” this second fallback is the
...
def _display_mouse_tracking(display: dict) -> str:
    ...
    Boolean values keep their legacy meaning (``True`` â†’ ``all``, ``False`` â†’
    ``off``). The ``wheel`` preset (DEC 1000+1006) is the tmux-friendly
    subset â€” wheel + click only, no hover events to trigger prompt-row
...
def _load_memory_notifications() -> str:
    ...
    "ðŸ’¾ Self-improvement review: â€¦" summary is surfaced. Without this the
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("ZED_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("ZED_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base Zed): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `zed --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] ZED_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from zed_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown ZED_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in ZED_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid ZED_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        # config-editing variant â€” used when we need to persist a toolset
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid ZED_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _restart_slash_worker(sid: str, session: dict):
    ...
    # close_on_disconnect reap can pop this session â€” a bare store would orphan
    ...
    *,
    ...
    if agent:
        ...
        if not explicit_provider:
            ...
            # provider) unchanged â€” ``str(...)`` would produce
    ...
    custom_provs = None
    try:
        from zed_cli.config import get_compatible_custom_providers, load_config
    ...
    #
    # We deliberately do NOT write process-global env vars (ZED_MODEL /
    # ZED_INFERENCE_MODEL / ZED_TUI_PROVIDER / ZED_INFERENCE_PROVIDER)
    ...
    # into every OTHER live session's next agent rebuild â€” switching the model
...
def _compress_session_history(
    ...
    # below does NOT hold history_lock for the duration of the request â€”
    ...
    with session["history_lock"]:
        if int(session.get("history_version", 0)) != history_version:
            # External mutation during compaction â€” drop the compressed
...
def _get_usage(agent) -> dict:
    ...
    # ZED_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("ZED_DEV_CREDITS")):
        try:
...
def _probe_credentials(agent) -> str:
    """Light credential check at session creation â€” returns warning or ''."""
    try:
...
def _probe_config_health(cfg: dict) -> str:
    """Flag bare YAML keys (`agent:` with no value â†’ None) that silently
    ...
    if not null_keys:
        ...
        warnings.append(
            ...
            f"Remove the line(s) or set them to `{{}}` â€” "
...
# backend reporting less than its required value (or none at all â€” a pre-GUI
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    # Effective approval-bypass state â€” the same three sources that
    ...
    try:
        ...
        pass
    try:
        from zed_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from zed_cli.banner import get_update_result
...
# all session and expanded by default â€” so shipping more than that is pure pipe
...
# "[omitted â€¦]" label is still informative when output is genuinely large).
...
def _on_tool_progress(
    ...
    **_kwargs,
    ...
    if event_type.startswith("subagent."):
        ...
        # Identity fields for the TUI spawn tree.  All optional â€” older
        ...
        # skip the parent emit â€” sending hundreds of ignored token frames there
...
# â”€â”€ Child-session live mirror â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# A delegated child is not a live gateway session â€” it runs synchronously
...
# window already renders â€” emitted on the CHILD sid, routed to its transport
# by write_json â€” so the window shows a real midstream turn.
...
# parent crashed) â€” don't let a leaked entry pin "running" forever.
...
def _mirror_subagent_to_child(event_type: str, payload: dict) -> None:
    ...
    if not child_key:
        return
    # Liveness registry first â€” it must be accurate even when no window is
    ...
    # No window / closed â†’ nothing to mirror; an upgraded session owns a real
    ...
    with _child_mirrors_lock:
        ...
        if event_type == "subagent.thinking":
            ...
            # The child's streamed reply text â€” the actual "agent talking".
...
def _agent_cbs(sid: str) -> dict:
    return {
        ...
        # read_terminal tool (desktop GUI): same blocking bridge as clarify â€” the
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from zed_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from zed_cli.config import load_config as _load_full_cfg
...
def _apply_personality_to_session(
    ...
    Returns (history_reset, info) â€” history_reset is always False since we
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("ZED_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("ZED_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``ZedCLI.__init__`` and ``gateway/run.py``:
...
def _preview_restart_history(session: dict, max_messages: int = 24, max_tool_chars: int = 1200) -> list[dict]:
    ...
    guess â€” usually starting the wrong thing.
...
def _schedule_mcp_late_refresh(sid: str, agent) -> None:
    ...
    already-spawning servers land in that snapshot â€” but a server that takes
    ...
    the agent's callable tools and the banner count catch up â€” the same
    rebuild ``/reload-mcp`` performs, but automatic.

    Cache safety: the rebuild only runs while the session is still pre-first-
    turn (no API call made yet â†’ nothing cached to invalidate). If the user
    has already sent a message, we leave the snapshot frozen rather than
    invalidate the prompt cache mid-conversation â€” those late tools then
    ...
    def _wait_then_refresh() -> None:
        # Bounded but generous â€” a server still not connected after this is
        ...
        with _sessions_lock:
            ...
            # has started â€” that would invalidate the cached prompt prefix.
            if (
            ...
            try:
                ...
                return
            # No new tools landed (discovery added nothing) â†’ don't churn the client.
            if not added:
                return
            info = _session_info(agent, session)
        # Emit outside the lock â€” write_json must not block under _sessions_lock.
        _emit("session.info", sid, info)
...
def _make_agent(
    ...
    # to land before building â€” bounded, so a slow/dead server still can't
    # block. Dashboard /api/ws uses zed_cli.mcp_startup; TUI stdio keeps
    ...
    if isinstance(model_override, dict) and model_override.get("model"):
        ...
        if str(requested_provider or "").strip().lower() == "custom":
            ...
            # a named ``providers:`` / ``custom_providers:`` entry â€” the
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("ZED_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("ZED_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("ZED_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("ZED_IGNORE_RULES")),
...
def _init_session(
    ...
    try:
        ...
        pass
    # Surface the self-improvement background review's "ðŸ’¾ â€¦" summary as a
    ...
    try:
        ...
        # messaging gateway and CLI do â€” otherwise the review always behaved as
...
def _coerce_message_text(content: Any) -> str:
    ...
    instead of the URL â€” and it stops the resume payload from disagreeing
...
def _history_to_messages(history: list[dict]) -> list[dict]:
    ...
    for m in history:
        ...
        # reasoning so the "Thinkingâ€¦" block still shows. (#44022)
        reasoning_keys = (
            "reasoning",
            "reasoning_content",
            "reasoning_details",
...
# â”€â”€ Methods: session â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind ZED_HOME. None/own profile â†’ launch (unchanged).
    profile = (params.get("profile") or "").strip() or None
    profile_home = _profile_home(profile)

    # The desktop composer owns its model/effort/fast as plain UI state and ships
    # it on every session.create. Honor each as a PER-SESSION override (built into
    # the agent below) â€” never a global config write, so picking a model/effort
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # user-facing surface â€” CLI, TUI, all gateway platforms (including new
        # ones not enumerated here), ACP adapter clients, webhook sessions,
        # custom `ZED_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    # local profile's state.db. None/own profile â†’ the launch profile (unchanged).
    profile = (params.get("profile") or "").strip() or None
    profile_home = _profile_home(profile)
    ...
    if not found:
        ...
        if found:
            ...
            # â€” the frontend then 404'd on the REST messages fallback and the
    ...
    # transcript and the response generated after compression is missing â€” the
    ...
    def _reuse_live_payload(sid: str, session: dict) -> dict:
        ...
        # flag is always False â€” overlay the child-run registry so a reconnecting
    ...
    # Used by the desktop's subagent windows â€” the child runs inside the
    ...
    if is_truthy_value(params.get("lazy", False)):
        ...
        try:
            ...
            # parent's entire transcript â€” a watch window opened on a subagent
        ...
        now = time.time()
        # A delegated child mid-run emits no native session events of its own â€”
    ...
    # Build the agent OUTSIDE the lock â€” _make_agent can block for seconds
    ...
    _enable_gateway_prompts()
    home_token = (
        set_zed_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
        history = db.get_messages_as_conversation(target)
    ...
    with _session_resume_lock:
        ...
        try:
            init_home_token = (
                set_zed_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            try:
                ...
                if init_home_token is not None:
                    reset_zed_home_override(init_home_token)
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds ZED_HOME (the
                # agent persists to its own db, but mid-turn home reads â€” memory,
                # skills â€” must resolve to the resumed profile too).
                if profile_home is not None:
...
def _(rid, params: dict) -> dict:
    ...
    # is dead â€” its agent/worker are being released and it is no longer
    # attachable â€” but it can briefly remain in ``_sessions`` until the reaper
    ...
    # ``zed --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    that is currently active in this gateway process â€” those rows are
    ...
    # pool â€” iterating the dict directly can raise ``RuntimeError:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # intent, not an abandoned draft â€” so persist the row NOW and set the
        ...
        with _session_db(session) as scoped_db:
            if scoped_db is not None and scoped_db.set_session_title(key, title):
                session["pending_title"] = None
                return _ok(rid, {"pending": False, "title": title})
        # Row creation didn't take (DB unavailable, or a concurrent writer) â€”
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``zed gateway`` process, whose
    ...
    if session.get("running"):
        return _err(
            rid,
            4009,
            "session busy â€” wait for the current turn to finish, then retry the handoff",
        )
    ...
    # Validate against the live gateway config â€” an unconfigured platform or a
    ...
    except Exception as e:  # pragma: no cover â€” gateway pkg always ships
    ...
    if not home or not home.chat_id:
        return _err(
            rid,
            4026,
            f"no home channel configured for {platform_name} â€” set one with "
    ...
    if not ok:
        return _err(
            rid,
            4027,
            "session is already in flight for handoff â€” wait for it to settle, then retry",
        )
    return _ok(
        rid,
        {
    ...
    try:
        ...
        return _ok(
            rid,
            {
                ...
                "balance_lines": [
                    line for line in view.balance_lines if not line.lstrip().startswith("ðŸ“ˆ")
                ],
...
# rate_limited, no_payment_method, â€¦) to render the right affordance instead of
# landing in a generic catch. The data-building lives in the shared core
# (agent/billing_view.py + zed_cli/nous_billing.py) â€” same as /credits.
...
def _serialize_billing_state(state) -> dict:
    """Serialize a BillingState for the wire (Decimals â†’ strings, money-safe)."""
...
def _(rid, params: dict) -> dict:
    """GET /api/billing/state â†’ serialized BillingState (Screen 1 + 5).
...
def _(rid, params: dict) -> dict:
    """POST /api/billing/charge â†’ {ok, chargeId} or a typed error envelope.
    ...
    """
    from zed_cli.nous_billing import BillingError, post_charge
...
def _(rid, params: dict) -> dict:
    """GET /api/billing/charge/{id} â†’ {ok, status, ...} or typed error.

    The poll. Caller drives the 2s/5-min cadence; this is a single status read.
    """
    from zed_cli.nous_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    """PATCH /api/billing/auto-top-up â†’ {ok:true} or typed error (Screen 2).

    params: {enabled: bool, threshold: number, top_up_amount: number}.
    """
    from zed_cli.nous_billing import BillingError, patch_auto_top_up
...
def _(rid, params: dict) -> dict:
    """Run the lazy billing:manage step-up device flow â†’ {ok, granted}.
    ...
    pipe), and the browser is opened TUI-side via openExternalUrl â€” never with the
...
def _(rid, params: dict) -> dict:
    ...
    # Neither is what the user wants â€” make them /interrupt first.
    if session.get("running"):
        return _err(
            rid, 4009, "session busy â€” /interrupt the current turn before /undo"
        )
    removed = 0
    with session["history_lock"]:
...
def _(rid, params: dict) -> dict:
    ...
    if session.get("running"):
        return _err(
            rid, 4009, "session busy â€” /interrupt the current turn before /compress"
        )
    ...
    try:
        ...
        if before_count >= 4:
            ...
            _status_update(
                sid,
                "compressing",
                f"â ‹ compressing {before_count} messages "
                f"(~{before_tokens:,} tok){focus_suffix}â€¦",
            )
        ...
        try:
            ...
            # Re-read system prompt + tools after compression â€” _compress_context
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Zed profile home
    # (~/.zed/sessions/saved/) rather than the project/workspace CWD, and
    # include the system prompt so the export matches the dashboard save.
    saved_dir = get_zed_home() / "sessions" / "saved"
    try:
    ...
    path = saved_dir / f"zed_conversation_{timestamp}.json"
...
def _(rid, params: dict) -> dict:
    ...
    if not history:
        return _err(rid, 4008, "nothing to branch â€” send a message first")
    ...
    try:
        ...
        db.create_session(
            ...
            # end_reason heuristic never matches it â€” the marker is the only
...
# â”€â”€ Delegation: subagent tree observability + controls â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Powers the TUI's /agents overlay (see ui-tui/src/components/agentsOverlay).
# The registry lives in tools/delegate_tool â€” these handlers are thin
...
# â”€â”€ Spawn-tree snapshots: TUI-written, disk-persisted â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
#
# Layout:  $ZED_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _append_spawn_tree_index(session_dir, entry: dict) -> None:
    try:
        ...
        # Index is a cache â€” losing a line just means list() falls back
...
def _(rid, params: dict) -> dict:
    ...
    Mirrors AIAgent.steer(). Safe to call while a turn is running â€” the text
...
# â”€â”€ Methods: prompt â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _(rid, params: dict) -> dict:
    ...
    with session["history_lock"]:
        ...
        # flag is False â€” without this, typing mid-run builds a second agent
        # racing the in-flight child on the same stored session (interleaved
        # transcript, stale fork). After the run completes, submitting is fine:
        # the upgrade resumes the child's transcript as a normal conversation.
        if session.get("lazy") and _child_run_active(str(session.get("session_key") or "")):
            return _err(rid, 4009, "subagent still running â€” wait for it to finish")
...
def _notification_event_belongs_elsewhere(session: dict, evt: dict) -> bool:
    ...
    background job's completion surfaces in the session that launched it â€” not
...
def _notification_poller_loop(
    ...
    while not stop_event.is_set() and not session.get("_finalized"):
        ...
        # consume events that belong to *this* session â€” otherwise a background
        ...
        # Only emit the same notification identity to TUI once â€” re-queued
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn ZED_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_zed_home_override(_profile_home_str)
            # The sudo password callback is thread-local (tools.terminal_tool
            # _callback_tls), so wiring it on the build thread doesn't reach this
            # turn thread â€” terminal sudo prompts would fall through to /dev/tty
            ...
            # "native" â†’ pass pixels to the main model as OpenAI-style content
            # parts (adapters translate for Anthropic/Gemini/Bedrock/etc.).
            # "text"   â†’ pre-analyze with vision_analyze and prepend the text.
            ...
            if images:
                try:
                    ...
                    from zed_cli.config import load_config as _tui_load_config
                ...
                if isinstance(result.get("messages"), list):
                    with session["history_lock"]:
                        ...
                        if current_version == history_version:
                            ...
                            # dropping the agent's output â€” the UI can
                            # show the response and warn that it was
                            # not persisted.
                            print(
                                f"[tui_gateway] prompt.submit: history_version mismatch "
                                f"(expected={history_version} current={current_version}) â€” "
                                f"agent output NOT written to session history",
                                file=sys.stderr,
                            )
                            status_note = (
                                "History changed during this turn â€” the response above is visible "
            ...
            # â”€â”€ /goal continuation (Ralph-style loop) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            # After every TUI turn, if a /goal is active, ask the judge
            # whether the goal is done and â€” if not and we're still under
            # budget â€” queue a continuation prompt to run after this
            # thread releases session["running"]. The verdict message
            # ("âœ“ Goal achieved" / "â¸ budget exhausted") is surfaced as
            ...
            if _pending and status == "complete":
                ...
                if _pdb:
                    ...
                    try:
                        ...
                        # Invalid/duplicate title â€” non-retryable, drop it.
                        ...
                        # Transient DB failure â€” keep pending_title for retry.
                        pass
            ...
            # (cli.py:_voice_speak_response).  Only the final text â€” tool
            ...
            if (
                ...
                try:
                    from zed_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: zed_cli.voice unavailable")
                except Exception as e:
            ...
            try:
                ...
                with open(_CRASH_LOG, "a", encoding="utf-8") as f:
                    f.write(
                        f"\n=== turn-dispatcher exception Â· "
                        f"{time.strftime('%Y-%m-%d %H:%M:%S')} Â· sid={sid} ===\n"
                    )
                    f.write(trace)
            except Exception:
                pass
        ...
        if goal_followup:
            with session["history_lock"]:
                if session.get("running"):
                    # User already sent something â€” their turn wins,
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from zed_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _zed_home / "images"
...
# Leading magic bytes â†’ file extension, for filename-less uploads.
...
def _(rid, params: dict) -> dict:
    ...
    gateway can't hand us a local path â€” that file only exists on the client's
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".zed" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _decode_attachment_data_url(data_url: str) -> bytes:
    ...
    media type â€” text/csv, application/pdf, etc. â€” so non-image file uploads
...
def _stage_session_file_attachment(
    ...
    *,
    ...
    Three cases:
      1. The path resolves to a file already INSIDE the session workspace â€” use
         it as-is (no copy, ``uploaded=False``).
      2. The path resolves to a gateway-visible file OUTSIDE the workspace â€” copy
         it into ``.zed/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         path on the CLIENT's disk) â€” decode the uploaded ``data_url`` bytes and
         write them into ``.zed/desktop-attachments/``.
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Zed/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Zed Desktop. If it serves Zed/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    # not blow up the whole restart â€” treat it as "no validated cwd".
    try:
    ...
    def run():
        # Pin the validated preview cwd, else the parent workspace â€” never an
...
# â”€â”€ Methods: respond â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Methods: config â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _(rid, params: dict) -> dict:
            ...
            if session:
                ...
                if session.get("running"):
                    return _err(
                        rid,
                        4009,
                        "session busy â€” /interrupt the current turn before switching models",
                    )
                from zed_cli.model_switch import parse_model_flags
    ...
    if key == "yolo":
        ...
        #   scope="session" (default) â€” same as the TUI's Shift+Tab. Toggles
        #     ONLY this session's _session_yolo flag; never touches global
        #     config, so CLI / TUI / cron behavior is unaffected.
        #   scope="global" (Shift+click the zap) â€” flips the persistent global
        ...
        try:
            ...
            if scope == "global":
                ...
                # not try to restore a prior "smart"/custom mode â€” the zap is a
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("ZED_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
                    os.environ["ZED_YOLO_MODE"] = "1"
                    nv = "1"
                else:
                    os.environ.pop("ZED_YOLO_MODE", None)
                    nv = "0"
    ...
    if key == "mouse":
        ...
        # inputs (0, False) reach the alias map as themselves â€” both map to
        # 'off' via _MOUSE_TRACKING_ALIASES â€” instead of being collapsed to
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from zed_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_zed_home), "display": display_zed_home()})
    if key == "project":
...
def _(rid, params: dict) -> dict:
    try:
        from zed_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Zed provider is configured.",
                },
            )
...
# â”€â”€ Methods: tools & system â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _session_processes(session: dict) -> list:
    ...
    for entry in process_registry.list_sessions():
        ...
        # terminal viewer â€” ship a real tail alongside it.
...
@method("process.kill")
def _(rid, params: dict) -> dict:
    """Kill ONE background process â€” scoped to the caller's session so one
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Respect the ``approvals.mcp_reload_confirm`` config toggle â€” if
            ...
            if _confirm_required:
                ...
                return _ok(
                    rid,
                    {
                        ...
                        "message": (
                            "âš ï¸  /reload-mcp invalidates the prompt cache (next "
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.zed/.env`` into the gateway process via
    ``zed_cli.config.reload_env``, matching classic CLI's ``/reload``
    handler.  Newly added API keys take effect on the next agent call
    without restarting the TUI.

    The credential pool / provider routing for any *already-constructed*
    agent does not auto-rebuild â€” that's the same behaviour as classic
...
def _(rid, params: dict) -> dict:
    """Registry-backed slash metadata for the TUI â€” categorized, no aliases."""
    try:
        ...
        try:
            ...
            for k, info in sorted(scan_skill_commands().items()):
                ...
                all_pairs.append([k, d[:120] + ("â€¦" if len(d) > 120 else "")])
                skill_count += 1
        except Exception as e:
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `zed` is interactive â€” use `/zed chat -q â€¦` or run `zed` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`zed setup` needs a full terminal â€” run it outside the TUI"
    if a0 == "gateway":
        return "`zed gateway` is long-running â€” run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`zed sessions browse` is interactive â€” use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`zed config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m zed_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "zed_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    # â”€â”€ Commands that queue messages onto _pending_input in the CLI â”€â”€â”€
    # In the TUI the slash worker subprocess has no reader for that queue,
    ...
    if name == "retry":
        ...
        if session.get("running"):
            return _err(
                rid, 4009, "session busy â€” /interrupt the current turn before /retry"
            )
        history = session.get("history", [])
        if not history:
            ...
            try:
                ...
                if accepted:
                    return _ok(
                        ...
                        {
                            "type": "exec",
                            "output": f"â© Steer queued â€” arrives after the next tool call: {arg[:80]}{'...' if len(arg) > 80 else ''}",
                        },
                    )
            except Exception:
                pass
    ...
    if name == "goal":
        ...
        if lower == "pause":
            ...
            out = "No goal set." if state is None else f"â¸ Goal paused: {state.goal}"
        ...
        if lower == "resume":
            ...
            return _ok(
                rid,
                {
                    "type": "exec",
                    "output": (
                        f"â–¶ Goal resumed: {state.goal}\n"
                        "Send any message to continue, or wait â€” I'll take the next step on the next turn."
                    ),
                },
            )
        if lower in {"clear", "stop", "done"}:
            ...
            return _ok(
                ...
                {
                    "type": "exec",
                    "output": "âœ“ Goal cleared." if had else "No active goal.",
                },
            )
        ...
        notice = (
            f"âŠ™ Goal set ({state.max_turns}-turn budget): {state.goal}\n"
            "I'll keep working until the goal is done, you pause/clear it, or the budget is exhausted.\n"
            "Controls: /goal status Â· /goal pause Â· /goal resume Â· /goal clear"
        )
        # Send the goal text as the kickoff prompt. The TUI client sees
        # {type: send, notice, message} â†’ renders `notice` as a sys line,
    ...
    if name == "undo":
        ...
        # Parse the optional count argument (e.g. "/undo 3" â†’ 3).
        n = 1
        ...
        if arg_str:
            try:
                ...
                return _err(rid, 4004, f"undo: invalid count {arg_str!r} â€” use /undo or /undo N")
        if n < 1:
            n = 1
        try:
        ...
        with session["history_lock"]:
            ...
            session["history_version"] = int(session.get("history_version", 0)) + 1
        # Notify memory providers â€” same hook /branch fires, plus the
        ...
        notice = (
            f"â†¶ Undid {turns_undone} {turn_word} ({rewound_count} message(s)). "
...
# â”€â”€ Methods: paste â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Methods: complete â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _list_repo_files(root: str) -> list[str]:
        ...
        if top_result.returncode == 0:
            ...
            if list_result.returncode == 0:
                for p in list_result.stdout.decode("utf-8", "replace").split("\0"):
                    ...
                    # Skip parents/siblings of cwd â€” keep the picker scoped
    ...
    if not files:
        ...
        # tractable. Dotfiles themselves survive â€” the ranker decides based
...
def _fuzzy_basename_rank(name: str, query: str) -> tuple[int, int] | None:
    ...
    Tiers (kind):
      0 â€” exact basename
      1 â€” basename prefix (e.g. `app` â†’ `appChrome.tsx`)
      2 â€” word-boundary / camelCase hit (e.g. `chrome` â†’ `appChrome.tsx`)
      3 â€” substring anywhere in basename
      4 â€” subsequence match (every query char appears in order)
    ...
    # Word-boundary split: `foo-bar_baz.qux` â†’ ["foo","bar","baz","qux"].
    # camelCase split: `appChrome` â†’ ["app","Chrome"]. Cheap approximation;
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # name with no path separator â€” `@appChrome` surfaces every file
        ...
        for entry in sorted(os.listdir(search_dir)):
            ...
            # Explicit `@folder:` / `@file:` â€” honour the user's filter.  Skip
        ...
        items = [
            {
                ...
                # plain strings â€” the TUI's CompletionItem.display contract
...
def _(rid, params: dict) -> dict:
    try:
        ...
        session = _sessions.get(params.get("session_id", ""))
        agent = session.get("agent") if session else None
        # Layer agent-session state on top of disk config â€” once an agent
        ...
        # Curated model lists are preserved â€” list_authenticated_providers
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from zed_cli.config import is_managed, save_env_value
        from zed_cli.inventory import build_models_payload, load_picker_context
        ...
        if is_managed():
            return _err(rid, 4006, "managed install â€” credentials are read-only")
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `zed model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.zed/.env
        ...
        if provider_data is None:
            # Key was saved but provider didn't appear â€” still return success.
            provider_data = {
                "slug": slug,
                "name": pconfig.name,
                "is_current": False,
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from zed_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from zed_cli.config import remove_env_value
...
# â”€â”€ Methods: slash.exec â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _mirror_slash_side_effects(sid: str, session: dict, command: str) -> str:
    ...
    if name in _MUTATES_WHILE_RUNNING and session.get("running"):
        return f"session busy â€” /interrupt the current turn before running /{name}"
...
def _(rid, params: dict) -> dict:
    ...
    # slash worker â€” see _PENDING_INPUT_COMMANDS definition above. Plugin
...
# â”€â”€ Methods: voice â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("ZED_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("ZED_VOICE_TTS", "").strip() == "1"
...
def _voice_cfg_dict() -> dict:
    ...
    a scalar would otherwise break ``.get("â€¦")`` and take every
...
def _(rid, params: dict) -> dict:
    ...
    * ``status`` â€” report mode + TTS flags (default when action is unknown).
    * ``on`` / ``off`` â€” flip voice *mode* (the umbrella bit). Turning it
    ...
    * ``tts`` â€” toggle speech-output of agent replies. Requires mode on
    ...
    if action == "status":
        ...
        # it in /voice status â€” previously the TUI hardcoded Ctrl+B and
        ...
        try:
            ...
            # check_voice_requirements pulls optional transcription deps â€”
    ...
    if action == "tts":
        ...
        # Runtime-only flag (CLI parity) â€” see voice.toggle on/off above.
        os.environ["ZED_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            if not _voice_mode_enabled():
                return _err(rid, 4015, "voice mode is off â€” enable with /voice on")

            with _voice_sid_lock:
                global _voice_event_sid
                _voice_event_sid = params.get("session_id") or _voice_event_sid

            from zed_cli.voice import start_continuous

            # Shape-safe lookups: malformed ``voice:`` YAML (bool/scalar/list)
            # must not crash /voice with a 5025 â€” fall back to VAD defaults.
            #
            # Exclude ``bool`` from the numeric check since Python's bool is
            # a subclass of int â€” a hand-edit like ``silence_threshold: true``
        ...
        return _err(
            rid, 5025, "voice module not available â€” install audio dependencies"
        )
    except Exception as e:
...
# â”€â”€ Methods: insights â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Methods: rollback â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _(rid, params: dict) -> dict:
    ...
    if not file_path and session.get("running"):
        return _err(
            rid,
            4009,
            "session busy â€” /interrupt the current turn before full rollback.restore",
        )
    try:
...
# â”€â”€ Methods: browser / plugins / cron / skills â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


def _resolve_browser_cdp_url() -> str:
    """Return the configured browser CDP override without network I/O.

    ``/browser status`` must be fast â€” calling
...
def _is_default_local_cdp(parsed) -> bool:
    ...
    real, connectable endpoint â€” collapsing it to bare ``http://...:9222``
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from zed_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from zed_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from zed_cli.browser_connect import try_launch_chrome_debug
...
def _browser_disconnect(rid) -> dict:
    # Reap, drop the env override, reap again â€” closes the same swap
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("ZED_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("ZED_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_zed_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from zed_cli.skills_hub import do_install
        ...
        if action == "browse":
            from zed_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from zed_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``zed plugins`` / the dashboard, so the three surfaces
    agree on what's installed and what's enabled.

    Actions:
      - ``list``   â†’ {"plugins": [{name, version, description, source,
                       status}], "user_count": N, "bundled_count": M}
      - ``toggle`` â†’ flip ``name`` based on ``enable`` (bool). Returns the
