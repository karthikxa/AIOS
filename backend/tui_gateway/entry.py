...
# zed modules.  zed_cli sets ZED_PYTHON_SRC_ROOT before spawning this
# subprocess; inserting it first ensures the installed packages win.
_src_root = os.environ.get("ZED_PYTHON_SRC_ROOT", "")
if _src_root and _src_root not in sys.path:
    sys.path.insert(0, _src_root)
# Strip '' and '.' â€” both resolve to CWD at import time and can let a local
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `ZED_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("ZED_TUI_SIDECAR_URL")
...
# tested; override via ``ZED_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("ZED_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    handler the gateway-exited banner in the TUI has no trace â€” the
    ...
    pool â€” a thread holding ``_stdout_lock`` mid-flush would block the
    interpreter shutdown indefinitely.  We now log the stack, give the
    process the configured shutdown grace
    (``ZED_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
    ...
    """
    # SIGPIPE and SIGHUP don't exist on Windows â€” build the lookup
    ...
    try:
        ...
        with open(_CRASH_LOG, "a", encoding="utf-8") as f:
            f.write(
                f"\n=== {name} received Â· {time.strftime('%Y-%m-%d %H:%M:%S')} ===\n"
            )
            if frame is not None:
                f.write("main-thread stack at signal delivery:\n")
                traceback.print_stack(frame, file=f)
            # All live threads â€” signal may have been triggered by a
...
# quiet on â€” even though the main thread was perfectly fine waiting on
...
# ``zed --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def _log_exit(reason: str) -> None:
    ...
    triggered it â€” the main reason voice-mode turns look like phantom
    ...
    try:
        ...
        with open(_CRASH_LOG, "a", encoding="utf-8") as f:
            f.write(
                f"\n=== gateway exit Â· {time.strftime('%Y-%m-%d %H:%M:%S')} "
                f"Â· reason={reason} ===\n"
            )
    except Exception:
        pass
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``zed_cli.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is None or not thread.is_alive():
        return
    try:
        from zed_cli.mcp_startup import _resolve_discovery_timeout
...
def main():
    ...
    # MCP tool discovery â€” runs in a background daemon thread so a slow or
    ...
    # server stalled the whole shell on "summoning zedâ€¦" for the full
    # connect-retry backoff (e.g. a dead stdio/http server burns 1+2+4s of
    # retries â†’ ~7s of dead air before the composer appears).  Discovery is
    ...
    # bounded) so already-spawning fast servers land in the tool snapshot â€”
    ...
    # full MCP SDK (mcp, pydantic, httpx, jsonschema, starlette parsers â€”
