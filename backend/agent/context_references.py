...
_SENSITIVE_ZED_DIRS = (Path("skills") / ".hub",)
    ...
    *,
    ...
    try:
        ...
        if ref.kind == "url":
            ...
            return None, f"ðŸŒ {ref.raw} ({estimate_tokens_rough(content)} tokens)\n{content}"
...
def _expand_file_reference(
    ...
    *,
    ...
    if _is_binary_file(path):
        ...
        # tools run where this resolves â€” the local cwd, or the staged copy in a
        ...
        # actionable block â€” the path, type, size, and a nudge to use its tools â€”
    ...
    label = ref.raw
    return None, f"ðŸ“„ {label} ({estimate_tokens_rough(text)} tokens)\n```{lang}\n{text}\n```"
...
def _expand_folder_reference(
    ...
    *,
    ...
    return None, f"ðŸ“ {ref.raw} ({estimate_tokens_rough(listing)} tokens)\n{listing}"
...
def _expand_git_reference(
    ...
    if not content:
        content = "(no output)"
    return None, f"ðŸ§¾ {label} ({estimate_tokens_rough(content)} tokens)\n```diff\n{content}\n```"
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    zed_home = get_zed_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(zed_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(zed_home / rel for rel in _SENSITIVE_ZED_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Zed path and cannot be attached")
...
def _binary_reference_block(ref: ContextReference, path: Path) -> str:
    ...
    try:
        ...
        size = "unknown size"
    return (
        f"ðŸ“Ž {ref.raw} ({mime}, {size}) â€” binary file, not inlined as text. "
