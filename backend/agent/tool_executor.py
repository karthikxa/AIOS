"""Tool-call execution â€” sequential and concurrent dispatch.
...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    # â”€â”€ Pre-flight: interrupt check â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if agent._interrupt_requested:
        print(f"{agent.log_prefix}âš¡ Interrupt: skipping {num_tools} tool call(s)")
        for tc in tool_calls:
            messages.append(make_tool_result_message(
                tc.function.name,
                f"[Tool execution cancelled â€” {tc.function.name} was skipped due to user interrupt]",
                tc.id,
            ))
        return

    # â”€â”€ Parse args + pre-execution bookkeeping â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    for tool_call in tool_calls:
        ...
        # â”€â”€ Tool Search unwrap â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # callback) sees the underlying tool â€” not the bridge. This is
        ...
        # â”€â”€ Block evaluation (BEFORE checkpoint preflight) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ Checkpoint preflight (only for tools that will execute) â”€â”€
    ...
    # â”€â”€ Logging / callbacks â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    tool_names_str = ", ".join(name for _, name, _, _, _, _ in parsed_calls)
    if not agent.quiet_mode and getattr(agent, "tool_progress_mode", "all") != "off":
        print(f"  âš¡ Concurrent: {num_tools} tool calls â€” {tool_names_str}")
        for i, (tc, name, args, middleware_trace, block_result, blocked_by_guardrail) in enumerate(parsed_calls, 1):
            args_str = json.dumps(args, ensure_ascii=False)
            if agent.verbose_logging:
                print(f"  ðŸ“ž Tool {i}: {name}({list(args.keys())})")
                print(agent._wrap_verbose("Args: ", json.dumps(args, indent=2, ensure_ascii=False)))
            else:
                args_preview = args_str[:agent.log_prefix_chars] + "..." if len(args_str) > agent.log_prefix_chars else args_str
                print(f"  ðŸ“ž Tool {i}: {name}({list(args.keys())}) - {args_preview}")
    ...
    # â”€â”€ Concurrent execution â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _run_tool(index, tool_call, function_name, function_args, middleware_trace):
        ...
        # to it â€” see AIAgent.interrupt().  Must happen first thing, and
    ...
    if agent._should_emit_quiet_tool_messages() and agent._should_start_quiet_spinner():
        ...
        spinner = KawaiiSpinner(f"{face} âš¡ running {num_tools} tools concurrently", spinner_type='dots', print_fn=agent._print_fn)
        spinner.start()
    ...
    try:
        ...
        if runnable_calls:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                while True:
                    ...
                    # Check for interrupt â€” the per-thread interrupt signal
                    ...
                    if agent._interrupt_requested:
                        if not _interrupt_logged:
                            ...
                            agent._vprint(
                                f"{agent.log_prefix}âš¡ Interrupt: cancelling "
                    ...
                    # Heartbeat every ~30s (6 Ã— 5s poll intervals)
        ...
        if spinner:
            ...
            spinner.stop(f"âš¡ {completed}/{num_tools} tools completed in {total_dur:.1f}s total")

    # â”€â”€ Post-execution: display per-tool results â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    for i, (tc, name, args, middleware_trace, block_result, blocked_by_guardrail) in enumerate(parsed_calls):
        r = results[i]
        blocked = False
        if r is None:
            ...
            if agent._interrupt_requested:
                function_result = f"[Tool execution cancelled â€” {name} was skipped due to user interrupt]"
                _emit_terminal_post_tool_call(
                    agent,
                    function_name=name,
                    function_args=args,
            ...
            # `blocked` calls never actually ran â€” don't let a guardrail
        ...
        if agent._should_emit_quiet_tool_messages():
            ...
            if agent.verbose_logging:
                print(f"  âœ… Tool {i+1} completed in {tool_duration:.2f}s")
                ...
                print(f"  âœ… Tool {i+1} completed in {tool_duration:.2f}s - {response_preview}")
        ...
        # (mlx-vlm, GPT-4o, â€¦) accept image_url in tool messages natively.
        ...
        # â”€â”€ Per-tool /steer drain â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        # Same as the sequential path: drain between each collected
        # result so the steer lands as early as possible.
        agent._apply_pending_steer_to_tool_results(messages, 1)

    # â”€â”€ Per-turn aggregate budget enforcement â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    num_tools = len(parsed_calls)
    if num_tools > 0:
        turn_tool_msgs = messages[-num_tools:]
        enforce_turn_budget(turn_tool_msgs, env=get_active_env(effective_task_id))

    # â”€â”€ /steer injection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if agent._interrupt_requested:
            ...
            if remaining_calls:
                agent._vprint(f"{agent.log_prefix}âš¡ Interrupt: skipping {len(remaining_calls)} tool call(s)", force=True)
            for skipped_tc in remaining_calls:
                skipped_name = skipped_tc.function.name
                skip_msg = {
                    "role": "tool",
                    "name": skipped_name,
                    "content": f"[Tool execution cancelled â€” {skipped_name} was skipped due to user interrupt]",
        ...
        # Tool Search unwrap â€” see execute_tool_calls_concurrent for full
        ...
        if _execution_blocked:
            # Tool blocked by plugin or guardrail policy â€” skip counters,
        ...
        if not agent.quiet_mode and getattr(agent, "tool_progress_mode", "all") != "off":
            ...
            if agent.verbose_logging:
                print(f"  ðŸ“ž Tool {i}: {function_name}({list(function_args.keys())})")
        ...
        if _block_msg is not None:
            # Tool blocked by plugin policy â€” return error without executing.
            ...
            # Tool blocked by tool-loop guardrail â€” synthesize exactly one
            ...
            if tasks_arg and isinstance(tasks_arg, list):
                spinner_label = f"ðŸ”€ delegating {len(tasks_arg)} tasks Â· (/agents to monitor)"
            else:
                goal_preview = (function_args.get("goal") or "")[:30]
                spinner_label = (
                    f"ðŸ”€ {goal_preview} Â· (/agents to monitor)"
                    if goal_preview
                    else "ðŸ”€ delegating Â· (/agents to monitor)"
                )
            spinner = None
            if agent._should_emit_quiet_tool_messages() and agent._should_start_quiet_spinner():
                face = random.choice(KawaiiSpinner.get_waiting_faces())
            ...
            # These are not in the tool registry â€” route through MemoryManager.
            spinner = None
            if agent._should_emit_quiet_tool_messages() and agent._should_start_quiet_spinner():
                face = random.choice(KawaiiSpinner.get_waiting_faces())
                emoji = _get_tool_emoji(function_name)
        ...
        if isinstance(function_result, str):
            ...
            # Multimodal dict result (_multimodal=True) â€” not sliceable as string
        ...
        # dispatched inline â€” they never reach handle_function_call, so the
        ...
        # injection lands as soon as a tool finishes â€” not after the
        # entire batch.  The model sees it on the next API iteration.
        agent._apply_pending_steer_to_tool_results(messages, 1)

        if not agent.quiet_mode and getattr(agent, "tool_progress_mode", "all") != "off":
            if agent.verbose_logging:
                print(f"  âœ… Tool {i} completed in {tool_duration:.2f}s")
        ...
        if agent._interrupt_requested and i < len(assistant_message.tool_calls):
            ...
            agent._vprint(f"{agent.log_prefix}âš¡ Interrupt: skipping {remaining} remaining tool call(s)", force=True)
            for skipped_tc in assistant_message.tool_calls[i:]:
                skipped_name = skipped_tc.function.name
                messages.append(make_tool_result_message(
                    skipped_name,
                    f"[Tool execution skipped â€” {skipped_name} was not started. User sent a new message]",
                    skipped_tc.id,
                ))
            break
