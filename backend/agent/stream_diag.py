"""Stream diagnostics â€” per-attempt counters, exception chains, retry logging.
...
def stream_diag_capture_response(agent: Any, diag: Dict[str, Any], http_response: Any) -> None:
    ...
    are swallowed â€” diag is best-effort.
    """
...
def flatten_exception_chain(error: BaseException) -> str:
    ...
    site â€” but the underlying ``RemoteProtocolError`` /
...
def log_stream_retry(
    ...
    *,
    ...
    retries no longer spam the parent's terminal â€” but the file log keeps
    ...
    try:
        ...
        logger.warning(
            "Stream %s on attempt %s/%s â€” retrying. "
...
def emit_stream_drop(
    ...
    *,
    ...
    Both top-level agents and subagents announce drops in the UI â€” the
    ...
    mid-flight.  Full diagnostic detail goes to ``agent.log`` only â€”
    ``zed logs --level WARNING | grep "Stream drop"`` to inspect.
    """
    ...
    # Compose a brief "after Xs" suffix when we have timing data â€” helps
    ...
    try:
        agent._buffer_status(
            f"âš ï¸ {provider} stream {kind} ({type(error).__name__}){_suffix} "
            f"â€” reconnecting, retry {attempt}/{max_attempts}"
        )
