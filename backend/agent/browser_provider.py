...
(Browserbase, Browser Use, Firecrawl, â€¦). Providers register instances via
...
``kind: backend``) or ``~/.zed/plugins/browser/<name>/`` (user, opt-in via
``plugins.enabled``).

This ABC mirrors :class:`agent.web_search_provider.WebSearchProvider` (PR
#25182) â€” same shape, same registration flow, same picker integration. The
...
:mod:`tools.browser_tool` â€” it holds the provider's session ID regardless of
...
class BrowserProvider(abc.ABC):
    ...
    registry lookup â€” no per-provider conditionals, no shape translation.
    """
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``zed tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        calls â€” this runs at tool-registration time and on every
        ``zed tools`` paint.
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        the rest of :mod:`tools.browser_tool` â€” it holds the provider's
    ...
    def close_session(self, session_id: str) -> bool:
        ...
        Returns True on success, False on failure. Should not raise â€” log and
    ...
    def emergency_cleanup(self, session_id: str) -> None:
        ...
        credentials, network errors, etc. â€” log and move on. Must not raise.
        """

    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``zed tools`` picker.

        Used by :mod:`zed_cli.tools_config` to inject this provider as a
