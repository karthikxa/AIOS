...
# Terminal failure â€” the credential will never recover on its own.  Used for
...
# without losing recoverability â€” the user always has the option to re-add
# via ``zed auth add``.
#
...
# Fields that are only round-tripped through JSON â€” never used for logic as attributes.
...
class PooledCredential:
    ...
    def from_dict(cls, provider: str, payload: Dict[str, Any]) -> "PooledCredential":
        ...
        # Rehydrated last_status_at may be an ISO string from to_dict() â€” normalize to float epoch
...
class CredentialPool:
    ...
    def _is_terminal_auth_failure(
        ...
        Returns False for non-401 status codes â€” 429 rate limits and 402
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``zed model`` / ``zed auth`` which
        performs a fresh device-code login and writes new tokens to
        ``auth.json`` under ``_auth_store_lock``.  Without this sync the pool
        entry stays frozen until ``last_error_reset_at`` elapses â€” even
        though fresh credentials are sitting on disk â€” and every request
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Zed process
    ...
    def _sync_nous_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        avoiding a "refresh token reuse" revocation on the Zed Portal.
        """
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        stale state and can overwrite the fresh pool entry â€” potentially
        ...
        setup silently flips the ``active_provider`` flag â€” the next
        ``zed`` invocation that defaults to the active provider
        (e.g. setup wizard, ``zed auth status``) would land on
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("zed_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token â€” single-use tokens consumed by another Zed
                ...
                # Adopt fresher tokens from auth.json before spending the
                # refresh_token â€” single-use tokens consumed by another
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("zed_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
                    ...
                    # Credentials file had a valid (non-expired) token â€” use it directly
                    logger.debug("Credentials file has valid token, using without refresh")
                    return synced
            # For xai-oauth: same race as nous â€” another process may have
            ...
            if self.provider == "xai-oauth":
                ...
                if synced.refresh_token != entry.refresh_token:
                    logger.debug(
                        "xAI OAuth refresh failed but auth.json has newer tokens â€” adopting"
                    )
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
                    ...
                    self._persist()
                    return updated
                # Terminal error: auth.json has no newer tokens â€” the stored
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/nous â€” another Zed process
            ...
            if self.provider == "openai-codex":
                ...
                if synced.refresh_token != entry.refresh_token:
                    logger.debug(
                        "Codex OAuth refresh failed but auth.json has newer tokens â€” adopting"
                    )
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
            ...
            if self.provider == "nous":
                synced = self._sync_nous_entry_from_auth_store(entry)
                if synced.refresh_token != entry.refresh_token:
                    logger.debug("Nous refresh failed but auth.json has newer tokens â€” adopting")
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
                        last_status_at=None,
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Zed profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `zed model` / `zed auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `zed model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``zed auth add``.  Singleton-
                # seeded DEAD entries are kept so the audit trail (label,
                # last_error_reason, timestamps) stays visible â€” pruning them
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) â€” re-add via `zed auth add %s`",
                            _label,
        ...
        *,
        ...
        with self._lock:
            ...
            if api_key_hint:
                ...
                # _select_unlocked() would return the NEXT key â€” the wrong one.
                entry = next(
            ...
            if updated_entry.last_status == STATUS_DEAD:
                logger.warning(
                    "credential pool: marking %s DEAD (status=%s, reason=%s) â€” "
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Shared suppression gate â€” used at every upsert site so
    # `zed auth remove <provider> <N>` is stable across all source types.
    try:
        from zed_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Zed PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `zed setup` ("Claude
        ...
        # AND no OAuth env vars set â€” `save_anthropic_api_key()` writes the
        ...
        # Claude Code CLI, zed_pkce creds from a previous OAuth login)
        # into the anthropic pool â€” otherwise rotation on a 401/429 silently
        ...
        # that masquerade.  Prefer ~/.zed/.env over os.environ for the
        # same reason `_seed_from_env` does â€” that's the authoritative file
        # that `zed setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"zed_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_zed_oauth_credentials

        for source_name, creds in (
            ("zed_pkce", read_zed_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `zed auth add nous --label <name>`).  Fall back to the
        ...
        # Zed auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from zed_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.zed/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `zed auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression â€” `zed auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Zed auth store.  Without this gate the removal is instantly
        ...
        # Zed owns its own Codex auth state â€” we do NOT auto-import from
        ...
        # When the user logs in via ``zed model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``zed auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.zed/.env over os.environ â€” the user's config file is the
    # authoritative source for Zed credentials. Stale env vars from parent
    ...
    # Honour user suppression â€” `zed auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.zed/.env.
    ...
    try:
        from zed_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from zed_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.zed/.env over os.environ
...
def _seed_custom_pool(pool_key: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Shared suppression gate â€” same pattern as _seed_from_env/_seed_from_singletons.
    try:
        from zed_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
...
def load_pool(provider: str) -> CredentialPool:
    ...
    if provider.startswith(CUSTOM_POOL_PREFIX):
        # Custom endpoint pool â€” seed from custom_providers config and model config
