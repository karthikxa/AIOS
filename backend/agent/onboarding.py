...
time a user hits a behavior fork â€” message-while-running, first long-running
...
# -------------------------------------------------------------------------
# Flag names (stable â€” used as config.yaml keys under onboarding.seen)
# -------------------------------------------------------------------------
...
def busy_input_hint_gateway(mode: str) -> str:
    ...
    message matches reality ("I just interruptedâ€¦" vs "I just queuedâ€¦").
    """
    if mode == "queue":
        return (
            "ðŸ’¡ First-time tip â€” I queued your message instead of interrupting. "
            "Send `/busy interrupt` to make new messages stop the current task "
            "immediately, or `/busy status` to check. This notice won't appear again."
        )
    if mode == "steer":
        return (
            "ðŸ’¡ First-time tip â€” I steered your message into the current run; "
            "it will arrive after the next tool call instead of interrupting. "
            "Send `/busy interrupt` or `/busy queue` to change this, or "
            "`/busy status` to check. This notice won't appear again."
        )
    return (
        "ðŸ’¡ First-time tip â€” I just interrupted my current task to answer you. "
...
def tool_progress_hint_gateway() -> str:
    return (
        "ðŸ’¡ First-time tip â€” that tool took a while and I'm streaming every step. "
        "If the progress messages feel noisy, send `/verbose` to cycle modes "
        "(all â†’ new â†’ off). This notice won't appear again."
    )
...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Zed starts and finds ``~/.openclaw/``.

    Points users at ``zed claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``zed claw cleanup`` is mentioned as the
    follow-up step for users who have already migrated and want to archive
    the old directory â€” with a warning that archiving breaks OpenClaw.
    """
    return (
        "A legacy OpenClaw directory was detected at ~/.openclaw/.\n"
        "To port your config, memory, and skills over to Zed, run "
        "`zed claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `zed claw cleanup` (renames it to ~/.openclaw.pre-migration â€” "
...
def detect_openclaw_residue(home: Optional[Path] = None) -> bool:
    ...
    Pure filesystem check â€” no side effects. ``home`` override exists for tests.
    """
...
def profile_build_mode(config: Mapping[str, Any]) -> str:
    ...
    Returns one of:
      ``"ask"``  â€” on first contact, OFFER to build a profile (default).
      ``"off"``  â€” never offer; the first-message note stays a plain intro.
    ...
    conversation â€” this setting only governs whether the offer is made.
    """
...
def profile_build_directive() -> str:
    ...
    lookup and never silently reads connected accounts â€” directly addressing
    ...
    return (
        ...
        "OFFER â€” do not assume â€” to build a short profile of them so you can "
        ...
        "accounts (email, calendar, etc.) silently â€” ask each time.\n"
        ...
        "target=\"user\" â€” keep entries compact and high-signal.\n"
...
def mark_seen(config_path: Path, flag: str) -> bool:
    ...
    (including the config file being absent â€” onboarding is best-effort).
    """
    try:
        import yaml
        from utils import atomic_yaml_write
    except Exception as e:  # pragma: no cover â€” dependency issue
    ...
    try:
        ...
        if seen.get(flag) is True:
            return True  # already marked â€” nothing to do
        seen[flag] = True
