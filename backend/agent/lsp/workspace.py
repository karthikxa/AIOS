...
1. **Workspace gate** â€” the upper-level "is this directory a project?"
   check.  Zed only runs LSP when the cwd (or the file being edited)
...
2. **NearestRoot** â€” the per-server project-root walk.  Each language
...
# Cache: cwd â†’ (worktree_root, is_git) so repeated calls don't re-stat.
...
def normalize_path(path: str) -> str:
    ...
    NOT resolve symlinks here â€” symlink stability matters for some
...
def find_git_worktree(start: str) -> Optional[str]:
    ...
    worktree set up via ``git worktree add`` â€” both forms count.
    """
    try:
        ...
        # Pathological input (loop in symlinks, encoding error, etc.) â€”
    ...
    for _ in range(64):
        ...
        try:
            ...
            # Permission error on a parent dir â€” bail out cleanly.
            break
        parent = cur.parent
        if parent == cur:
            break
...
def is_inside_workspace(path: str, workspace_root: str) -> bool:
    ...
    Uses absolute paths but does not resolve symlinks â€” a file accessed
...
def nearest_root(
    ...
    *,
    ...
    in the upward walk, returns ``None`` â€” the server is gated off
    ...
    for _ in range(64):
        # Check excludes first â€” if an exclude is found at this level,
...
def resolve_workspace_for_file(
    ...
    *,
    ...
    The cwd path takes precedence â€” if the agent was launched in a
    ...
    if cwd_root is not None:
        ...
        # File is outside the cwd's worktree â€” try the file's own
