...
pairs touched in one Python process â€” bytes of memory in even an
...
Grep recipe::
    ...
    tail -f ~/.zed/logs/agent.log | rg 'lsp\\['
"""
...
event_log = logging.getLogger("zed.lint.lsp")
...
# ---------------------------------------------------------------------------
# Public event helpers â€” call these from the LSP layer.
# ---------------------------------------------------------------------------
...
def log_diagnostics(server_id: str, file_path: str, count: int) -> None:
    """Diagnostics arrived for a file.  INFO every time â€” these are the
...
def log_server_unavailable(server_id: str, binary_or_pkg: str) -> None:
    ...
    if _announce_once(_announced_unavailable, key):
        _emit(
            ...
            "(install via `zed lsp install <id>` or set lsp.servers.<id>.command)",
        )
    else:
...
def log_timeout(server_id: str, file_path: str, kind: str = "diagnostics") -> None:
    """A request to the server timed out.  WARNING every time â€” these are
