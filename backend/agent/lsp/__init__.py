"""Language Server Protocol (LSP) integration for Zed Agent.

Zed runs full language servers (pyright, gopls, rust-analyzer,
...
LSP is **gated on git workspace detection** â€” if the agent's cwd is
...
The bulk of the wiring is internal â€” most callers only need the layer
...
def get_service() -> Optional[LSPService]:
    ...
    with _service_lock:
        ...
        if not _atexit_registered:
            ...
            # subprocesses â€” losing them on SIGKILL is fine; they'll
            ...
            # ``zed chat`` exit would leak pyright processes that
...
def _atexit_shutdown() -> None:
    ...
    atexit fires the user has already seen the agent's final output â€”
