...
appropriate.  All installs go to a Zed-owned bin staging dir,
``<ZED_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.

Strategies:

- ``auto`` â€” attempt to install with the best available package
  manager.  This is the default.
- ``manual`` â€” never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``zed lsp
  status``.
- ``off`` â€” same as ``manual`` for now (kept distinct so we can
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # Rust â€” too heavy (hundreds of MB to bootstrap).  We do NOT
    # auto-install rust-analyzer; users install via rustup.
    "rust-analyzer": {"strategy": "manual", "pkg": "", "bin": "rust-analyzer"},
    # C/C++ â€” manual (clangd ships with LLVM, very heavy)
    "clangd": {"strategy": "manual", "pkg": "", "bin": "clangd"},
    # Lua â€” manual (LuaLS is platform-specific binaries from GitHub
...
def zed_lsp_bin_dir() -> Path:
    """Return the Zed-owned bin staging dir for LSP servers."""
    home = os.environ.get("ZED_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".zed")
    p = Path(home) / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(zed_lsp_bin_dir() / name):
...
def try_install(pkg: str, strategy: str = "auto") -> Optional[str]:
    ...
    The install is cached per-package â€” a second call returns the
...
def _do_install(pkg: str) -> Optional[str]:
    recipe = INSTALL_RECIPES.get(pkg)
    if recipe is None:
        # Not in our registry â€” best-effort: just probe PATH.
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = zed_lsp_bin_dir().parent  # <ZED_HOME>/lsp/
    ...
    for c in _native_binary_candidates(nm_bin):
        if c.exists():
            ...
            link = zed_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
                except (OSError, NotImplementedError):
                    # Symlinks fail on some Windows setups â€” copy instead.
                    try:
                        shutil.copy2(c, link)
                    except OSError:
                        return str(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a zed-owned target dir.
    ...
    console script.
    """
    pip_target = zed_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = zed_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``zed lsp status`` CLI to give users a quick
