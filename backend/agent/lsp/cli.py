"""``zed lsp`` CLI subcommand.

Subcommands:

- ``status`` â€” show service state, configured servers, install status.
- ``install <server_id>`` â€” eagerly install one server's binary.
- ``install-all`` â€” try to install every server with a known recipe.
- ``restart`` â€” tear down running clients so the next edit re-spawns.
- ``which <server_id>`` â€” print the resolved binary path for one server.
- ``list`` â€” print the registry of supported servers.

The handlers are kept here (rather than in
``zed_cli/main.py``) so the LSP module ships self-contained.
"""
...
def register_subparser(subparsers: argparse._SubParsersAction) -> None:
    """Wire the ``zed lsp`` subcommand tree into the main argparse."""
...
def run_lsp_command(args: argparse.Namespace) -> int:
    """Top-level dispatcher for ``zed lsp <subcommand>``."""
...
def _cmd_status(emit_json: bool) -> int:
    ...
    for s in SERVERS:
        ...
        if len(s.extensions) > 5:
            ext_summary += f", â€¦ (+{len(s.extensions) - 5})"
        out.append(
...
def _recipe_pkg_for(server_id: str) -> str:
    ...
    # key, but a few differ (e.g. ``vue-language-server`` â†’
    # ``@vue/language-server``).
    aliases = {
...
def _backend_warnings() -> list:
    ...
    actual diagnostics â€” they spawn cleanly but never emit any errors when
    ...
    if bash_installed and _shutil.which("shellcheck") is None:
        notes.append(
            "bash-language-server is installed but shellcheck is missing â€” "
