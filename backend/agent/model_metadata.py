...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Zed also honours ZED_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("ZED_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
# Only these are stripped â€” Ollama-style "model:tag" colons (e.g. "qwen3.5:27b")
...
def _strip_provider_prefix(model: str) -> str:
    ...
    ``"local:my-model"`` â†’ ``"my-model"``
    ``"qwen3.5:27b"``   â†’ ``"qwen3.5:27b"``  (unchanged â€” not a provider prefix)
    ``"qwen:0.5b"``     â†’ ``"qwen:0.5b"``    (unchanged â€” Ollama model:tag)
    ``"deepseek:latest"``â†’ ``"deepseek:latest"``(unchanged â€” Ollama model:tag)
    """
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_zed_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Zed Agent.  Models with fewer
# tokens cannot maintain enough working memory for tool-calling workflows.
# Sessions, model switches, and cron jobs should reject models below this.
MINIMUM_CONTEXT_LENGTH = 64_000

# Thin fallback defaults â€” only broad model family patterns.
# These fire only when provider is unknown AND models.dev/OpenRouter/Anthropic
# all miss. Replaced the previous 80+ entry dict.
# For provider-specific context lengths, models.dev is the primary source.
DEFAULT_CONTEXT_LENGTHS = {
    # Anthropic Claude 4.6 (1M context) â€” bare IDs only to avoid
    ...
    "claude": 200000,
    # OpenAI â€” GPT-5 family (most have 400k; specific overrides first)
    ...
    # DeepSeek â€” V4 family ships with a 1M context window. The legacy
    ...
    # Meta
    "llama": 131072,
    # Qwen â€” specific model families before the catch-all.
    # Official docs: https://help.aliyun.com/zh/model-studio/developer-reference/
    "qwen3.6-plus": 1048576,      # 1M context (DashScope/Alibaba & OpenRouter)
    "qwen3-coder-plus": 1000000,  # 1M context
    "qwen3-coder": 262144,        # 256K context
    "qwen": 131072,
    # MiniMax â€” M3 is 1M context (max output 512K); M2.x series is 204,800.
    ...
    "minimax-m3": 1000000,
    "minimax": 204800,
    # GLM â€” GLM-5.2 ships with a 1M context window (verified empirically:
    ...
    "glm-5.2": 1_048_576,
    "glm": 202752,
    # xAI Grok â€” xAI /v1/models does not return context_length metadata,
    # so these hardcoded fallbacks prevent Zed from probing-down to
    ...
    "grok-4.3": 1000000,        # grok-4.3, grok-4.3-latest â€” 1M context per docs.x.ai
    ...
    # Kimi
    "kimi": 262144,
    # Tencent â€” Hy3 Preview (Hunyuan) with 256K context window.
    # OpenRouter live metadata reports 262144 (256 Ã— 1024); align the
    # static fallback so cache and offline both agree (issue #22268).
    "hy3-preview": 262144,
    # Nemotron â€” NVIDIA's open-weights series (128K context across all sizes)
    "nemotron": 131072,
    # Arcee
    "trinity": 262144,
    # OpenRouter
    "elephant": 262144,
    # Hugging Face Inference Providers â€” model IDs use org/name format
...
#                    grok-code-fast-1
#
# REJECTS-side models still reason natively â€” they just don't expose an
# effort dial â€” so callers should send no `reasoning` key at all rather
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference-api.zedteam.com": "nous",
    "api.deepseek.com": "deepseek",
    "api.githubcopilot.com": "copilot",
    "models.github.ai": "copilot",
    # GitHub Models free tier (Azure-hosted prototyping endpoint) â€” same
    # canonical provider as the Copilot API.  Hard per-request token cap
    # (often 8K) makes it unusable for Zed' system prompt, but mapping
...
def is_local_endpoint(base_url: str) -> bool:
    ...
    if any(host.endswith(suffix) for suffix in _CONTAINER_LOCAL_SUFFIXES):
        return True
    # Unqualified hostnames (no dots) are local by definition â€” Docker
    ...
    # or Tailscale CGNAT (100.64.x.xâ€“100.127.x.x).
    parts = host.split(".")
    if len(parts) == 4:
        try:
...
def detect_local_server_type(base_url: str, api_key: str = "") -> Optional[str]:
    ...
    try:
        with httpx.Client(timeout=2.0, headers=headers) as client:
            # LM Studio exposes /api/v1/models â€” check first (most specific)
            try:
...
def _get_context_cache_path() -> Path:
    ...
    from zed_constants import get_zed_home
    return get_zed_home() / "context_length_cache.yaml"
...
def get_context_length_from_provider_error(
    ...
    down through guessed probe tiers (1M â†’ 256K â†’ 128K â†’ ...).
    """
...
def parse_available_output_tokens_from_error(error_msg: str) -> Optional[int]:
    ...
    Background â€” two distinct context errors exist:
      1. "Prompt too long"  â€” the INPUT itself exceeds the context window.
           Fix: compress history, and only reduce context_length if the
           provider explicitly reports the actual lower limit.
      2. "max_tokens too large" â€” input is fine, but input + requested_output > window.
           Fix: reduce max_tokens (the output cap) for this call.
           Do NOT touch context_length â€” the window hasn't shrunk.
    ...
    is_output_cap_error = (
        ...
        # problem (the input itself fits) â€” reduce max_tokens, don't compress.
    ...
    # Anthropic format: "â€¦ = available_tokens: 10000"
    patterns = [
    ...
    # OpenRouter/Nous format: "maximum context length is N â€¦ (A of text input,
...
def _query_ollama_api_show(model: str, base_url: str, api_key: str = "") -> Optional[int]:
    ...
    of hostname â€” local Ollama, Ollama Cloud (``ollama.com``), custom Ollama
    ...
    Resolution order for hosted Ollama:
      1. ``model_info.*.context_length`` â€” GGUF training max (authoritative)
      2. ``parameters`` â†’ ``num_ctx`` â€” server-side Modelfile override
    ...
    try:
        with httpx.Client(timeout=5.0, headers=headers) as client:
            ...
            # Hosted Ollama: GGUF model_info is the real max â€” prefer it over
...
def _query_local_context_length(model: str, base_url: str, api_key: str = "") -> Optional[int]:
    ...
    # Strip recognised provider prefix (e.g., "local:model-name" â†’ "model-name").
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # which can be larger than num_ctx â€” using it here would let
                    # Zed grow conversations past the runtime limit and Ollama
...
# `context_window` values, which are what Codex actually enforces â€” the
...
_CODEX_OAUTH_CONTEXT_FALLBACK: Dict[str, int] = {
    ...
    # longest-key-first fallback resolves it correctly â€” substring match
...
def _resolve_nous_context_length(
    ...
    """Resolve Zed Portal model context length.

    Tries the live Zed inference endpoint first (authoritative), then falls
    ...
    normalization (dotâ†”dash) is applied to handle name drifts.

    Returns ``(context_length, source)`` where ``source`` is one of:
      - ``"portal"``    â€” live /v1/models response (authoritative)
      - ``"openrouter"`` â€” OpenRouter cache fallback (non-authoritative;
        callers must NOT persist this to the on-disk cache or a single
        portal blip will freeze the wrong value in forever)
      - ``""``           â€” could not resolve
    """
    # Portal first â€” the Nous /models endpoint is authoritative for what our
...
def get_model_context_length(
    ...
    # 0. Explicit config override â€” user knows best
    if config_context_length is not None and isinstance(config_context_length, int) and config_context_length > 0:
        return config_context_length

    # 0b. custom_providers per-model override â€” check before any probe.
    ...
    # Normalise provider-prefixed model names (e.g. "local:model-name" â†’
    # "model-name") so cache lookups and server queries use the bare ID that
    # local servers actually know about.  Ollama "model:tag" colons are preserved.
    model = _strip_provider_prefix(model)

    # 1. Check persistent cache (model+provider)
    # LM Studio is excluded â€” its loaded context length is transient (the
    ...
    if base_url and provider != "lmstudio":
        ...
        if cached is not None:
            ...
            # Invalidate stale 32k cache entries for Kimi-family models.
                ...
                _invalidate_cached_context_length(model, base_url)
            # Invalidate stale â‰¤204,800 cache entries for MiniMax-M3.  Pre-catalog
            ...
            # so any sub-256K cached value for an M3 slug is a leftover â€” drop it
            # and fall through to the hardcoded default.
                ...
                _invalidate_cached_context_length(model, base_url)
            # Invalidate stale â‰¤256,000 cache entries for Grok-4.3.  The
            ...
            # leftover â€” drop it and fall through to the hardcoded default.
                ...
                _invalidate_cached_context_length(model, base_url)
            # Zed Portal: the portal /v1/models endpoint is authoritative.
            # Bypass the persistent cache so step 5b can always reconcile
            # against it â€” this corrects pre-fix entries seeded from the
            ...
            # cost amortise to ~0 within a process.
                ...
                logger.debug(
                    "Bypassing persistent cache for %s@%s (Zed Portal authoritative)",
                    model, base_url,
                )
    ...
    # 1b. AWS Bedrock â€” use static context length table.
    ...
    # step 2 â€” bedrock-runtime.<region>.amazonaws.com is not in
    ...
    if provider == "bedrock" or (
        ...
        try:
            ...
            pass  # boto3 not installed â€” fall through to generic resolution
    ...
    # Known providers (Copilot, OpenAI, Anthropic, etc.) skip this â€” their
    ...
    if _is_custom_endpoint(base_url) and not _is_known_provider_base_url(base_url):
        ...
        if not _is_known_provider_base_url(base_url):
            # 2b. Ollama native /api/show â€” any URL might be an Ollama server
            ...
            if is_local_endpoint(base_url):
                ...
                if local_ctx and local_ctx > 0:
                    ...
                    return local_ctx
            logger.info(
                "Could not detect context length for model %r at %s â€” "
            ...
            # DEFAULT_CONTEXT_LENGTHS (e.g. "claude-opus-4-8" â†’ 1M).
    ...
    # 4b. (Bedrock handled earlier at step 1b â€” before custom-endpoint probe.)
    ...
    # 5a. Copilot live /models API â€” max_prompt_tokens from the user's account.
    # This catches account-specific models (e.g. claude-opus-4.6-1m) that
    # don't exist in models.dev. For models that ARE in models.dev, this
    # returns the provider-enforced limit which is what users can actually use.
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from zed_cli.models import get_copilot_model_context
    ...
    if effective_provider == "nous":
        ...
        if ctx:
            ...
            # Kimi/Qwen DEFAULT_CONTEXT_LENGTHS overrides exist â€” we don't
    ...
    if effective_provider == "gmi" and base_url:
        ...
        if ctx is not None:
            return ctx
    # 5e. Ollama native /api/show probe â€” runs for ANY provider with a
    ...
    if base_url:
        ...
        if ctx is not None:
            save_context_length(model, base_url, ctx)
            return ctx
    # 5f. OpenRouter live /models metadata â€” authoritative for OpenRouter-routed
    ...
    # 6. OpenRouter live API metadata â€” provider-unaware fallback.
    ...
    # 8. Hardcoded defaults (fuzzy match â€” longest key first for specificity)
...
def estimate_messages_tokens_rough(messages: List[Dict[str, Any]]) -> int:
    ...
    image â€” the Anthropic pricing model â€” instead of counting raw base64
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Zed sends to providers:
    system prompt, conversation messages, and tool schemas.  With 50+
    tools enabled, schemas alone can add 20-30K tokens â€” a significant
