"""MemoryManager â€” orchestrates memory providers for the agent.

Single integration point in run_agent.py. Replaces scattered per-backend
code with one manager that delegates to registered providers.

Only ONE external plugin provider is allowed at a time â€” attempting to
...
# teardown indefinitely â€” the worker threads are daemon, so anything still
        ...
        while buf:
            if self._in_span:
                ...
                if idx == -1:
                    ...
                    return "".join(out)
                # Found close â€” skip span content + tag, continue
                buf = buf[idx + len(self._CLOSE_TAG):]
                self._in_span = False
            else:
                idx = self._find_boundary_open_tag(buf)
                if idx == -1:
                    # No open tag â€” hold back a potential partial open tag
                    held = (
...
def build_memory_context_block(raw_context: str) -> str:
    ...
    return (
        ...
        "NOT new user input. Treat as authoritative reference data â€” "
...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        Only **one** external (non-builtin) provider is allowed â€” a second
        ...
        if not is_builtin:
            if self._has_external:
                ...
                logger.warning(
                    "Rejected memory provider '%s' â€” external provider '%s' is "
        ...
        # Core tool names are reserved â€” a memory provider must never register
        ...
        # table at all â€” matching the built-ins-always-win invariant used by
        # the TTS/browser/search provider registries.
        from toolsets import _ZED_CORE_TOOLS

        _core_tool_names = set(_ZED_CORE_TOOLS)

        # Index tool names â†’ provider for routing
        for schema in provider.get_tool_schemas():
            tool_name = schema.get("name", "")
            if tool_name in _core_tool_names:
                logger.warning(
                    "Memory provider '%s' tool '%s' shadows a reserved core "
                    "tool name; registration ignored. Core tools always win â€” "
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Zed expands the turn into
        ...
        just the user's instruction here, once, for every provider â€” so this
        ...
        - Bare skill invocations (no instruction) return None â†’ callers skip
    ...
    def sync_all(
        ...
        *,
        ...
        broken provider can never stall the turn â€” the sync simply
    ...
    def _submit_background(self, fn) -> None:
        ...
        runs inline as a last-resort fallback â€” losing the async benefit
        ...
        if executor is None:
            # Executor unavailable (shut down / creation failed) â€” run
    ...
    def flush_pending(self, timeout: Optional[float] = None) -> bool:
        ...
        try:
            ...
            # Executor already shut down â€” nothing pending.
            return True
        try:
    ...
    def get_all_tool_schemas(self) -> List[Dict[str, Any]]:
        ...
        skipped â€” they are rejected from the routing table in
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        context compression â€” any path that reassigns
    ...
    def _drain_sync_executor(self) -> None:
        ...
        try:
            ...
            # do NOT block here â€” cancel_futures cancels not-yet-started
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``zed_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_zed_home()`` themselves.
        """
        if "zed_home" not in kwargs:
            from zed_constants import get_zed_home
            kwargs["zed_home"] = str(get_zed_home())
        for provider in self._providers:
            try:
