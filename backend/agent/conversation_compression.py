"""Context compression â€” extract the AIAgent methods that drive summarisation.

Three concerns live here:

* :func:`check_compression_model_feasibility` â€” startup probe of the
...
* :func:`replay_compression_warning` â€” re-emit a stored warning through
  the gateway ``status_callback`` once it's wired up (the callback is
  set after :class:`AIAgent` construction).

* :func:`compress_context` â€” the actual compression call.  Runs the
...
* :func:`try_shrink_image_parts_in_messages` â€” image-too-large recovery
...
# drivers like the desktop app can show an explicit "Summarizingâ€¦" indicator
...
COMPACTION_STATUS = (
    f"ðŸ—œï¸ {COMPACTION_STATUS_MARKER} â€” summarizing earlier conversation so I can continue..."
)
    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                msg = (
                    "âš  Configured auxiliary compression provider "
                    f"'{_aux_cfg_provider}' is unavailable â€” context "
                ...
                msg = (
                    "âš  No auxiliary LLM provider configured â€” context "
                    "compression will drop middle turns without a summary. "
                    "Run `zed setup` or set OPENROUTER_API_KEY."
                )
            agent._compression_warning = msg
            agent._emit_status(msg)
            logger.warning(
                "No auxiliary LLM provider for compression â€” "
        ...
        # fallbacks, which don't require auth â€” pass empty string rather
        ...
        # __init__), so the compression model must too â€” otherwise it
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Zed "
        ...
        if aux_context < threshold:
            ...
            logger.warning(
                ...
                "tokens â€” auto-lowered session threshold to %d to "
...
def replay_compression_warning(agent: Any) -> None:
    ...
    ``run_conversation()`` â€” by then the gateway has set the callback,
...
def compress_context(
    ...
    *,
    ...
    Args:
        ...
        focus_topic: Optional focus string for guided compression â€” the
    ...
    Returns:
        ...
        prompt â€” the session is NOT rotated.  Callers should detect the
        no-op via ``len(returned) == len(input)`` and stop the retry loop.
    """
    # Lazy feasibility check â€” run the auxiliary-provider probe + context
    ...
    # â”€â”€ Compression lock â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # Atomic, state.db-backed lock per session_id.  Without this, two
    # AIAgent instances that share the same session_id (most commonly the
    # parent-turn agent and its background-review fork â€” see
    ...
    # that silently accumulates writes â€” Damien's repro shape.
    #
    ...
    # and stops retrying for this cycle. The session is NOT corrupted â€”
    ...
    # long-lived ``zed_state.SessionDB`` class still bound to the
    # pre-#34351 version in memory) has the call site but not the method.
    # In that case ``try_acquire_compression_lock`` raises AttributeError â€”
    # NOT a ``sqlite3.Error`` â€” so the method's own fail-open guard never
    ...
    if _lock_db is not None and _lock_sid:
        ...
        try:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "(%s: %s) â€” proceeding without lock. This usually means a "
                    "stale in-memory module after an update; restart the "
                    "process (or `zed update`) to resync.",
        ...
        if not _lock_acquired:
            ...
            logger.warning(
                ...
                "(holder=%s) â€” returning messages unchanged to avoid session fork",
                _lock_sid, existing,
            )
            _lock_holder = None  # don't release a lock we don't own
            # Surface to the user once â€” quiet for downstream auto-compress loops
            if getattr(agent, "_last_compression_lock_warning_sid", None) != _lock_sid:
                agent._last_compression_lock_warning_sid = _lock_sid
                try:
                    agent._emit_warning(
                        "âš  Skipping concurrent compression â€” another path "
    ...
    try:
        ...
        # focus_topic / force â€” fall back to calling without them.
    ...
    if getattr(agent.context_compressor, "_last_compress_aborted", False):
        ...
        if getattr(agent, "_last_compression_summary_warning", None) != _err:
            ...
            agent._emit_warning(
                f"âš  Compression aborted: {_err}. "
                "No messages were dropped â€” conversation continues unchanged. "
                "Run /compress to retry, or /new to start a fresh session."
            )
        _existing_sp = getattr(agent, "_cached_system_prompt", None)
        if not _existing_sp:
            _existing_sp = agent._build_system_prompt(system_message)
        _release_lock()  # compression aborted â€” no rotation will happen
        return messages, _existing_sp
    ...
    if summary_error:
        if getattr(agent, "_last_compression_summary_warning", None) != summary_error:
            ...
            agent._emit_warning(
                f"âš  Compression summary failed: {summary_error}. "
                "Inserted a fallback context marker."
            )
    else:
        # No hard failure â€” but did the configured aux model error out
        ...
        if _aux_fail_model:
            ...
            if getattr(agent, "_last_aux_fallback_warning_key", None) != _aux_key:
                ...
                agent._emit_warning(
                    f"â„¹ Configured compression model '{_aux_fail_model}' failed "
                    f"({_aux_fail_err or 'unknown error'}). Recovered using main model â€” "
    ...
    if agent._session_db:
        try:
            ...
            try:
                ...
                pass  # best-effort â€” don't block compression on a flush error
            ...
            try:
                ...
                os.environ["ZED_SESSION_ID"] = agent.session_id
            # The gateway/tools session context (ContextVar + env) and the
            # logging session context are SEPARATE mechanisms. The call above
            # moves the former; the ``[session_id]`` tag on log lines comes
            # from ``zed_logging._session_context`` (set once per turn in
            # conversation_loop.py). Without this, post-rotation log lines in
            # the same turn keep the STALE old id while the message/DB/gateway
            # state carry the new one â€” breaking log correlation exactly at the
            ...
            agent._session_db.create_session(
                ...
                source=agent.platform or os.environ.get("ZED_SESSION_SOURCE", "cli"),
                model=agent.model,
            ...
            # Reset flush cursor â€” new session starts with no messages written
            agent._last_flushed_db_idx = 0
        except Exception as e:
            logger.warning("Session DB compression split failed â€” new session will NOT be indexed: %s", e)

    # Notify the context engine that the session_id rotated because of
    # compression (not a fresh /new). Plugin engines (e.g. zed-lcm) use
    # boundary_reason="compression" to preserve DAG lineage across the
    # rollover instead of re-initializing fresh per-session state.
    # See zed-lcm#68. Built-in ContextCompressor ignores kwargs.
    try:
    ...
    if _cc >= 2:
        agent._vprint(
            f"{agent.log_prefix}âš ï¸  Session compressed {_cc} times â€” "
    ...
    # read content is summarised away â€” if the model re-reads the same
    ...
    # acquire on that â€” no race against our just-finished work.
    _release_lock()
...
def try_shrink_image_parts_in_messages(
    ...
    *,
    ...
    Non-data-URL images (http/https URLs) are not touched â€” the provider
    ...
    try:
        ...
        logger.warning("image-shrink recovery: vision_tools unavailable â€” %s", exc)
        return False
    ...
    # If any survive, retrying is pointless â€” the same oversized payload will
    ...
    def _shrink_data_url(url: str) -> tuple:
        ...
        to satisfy *that same* constraint â€” so the caller knows retrying is
        ...
        # the original (PNG compression is non-monotonic in image size â€” a
        ...
        if not needs_shrink:
            # Bytes are fine â€” check pixel dimensions against the provider's
            # reported per-side cap.  A screenshot can be tiny in bytes yet
            # too large in pixels.
            dims = _decode_pixels(url)
            if dims is None:
                # Pillow missing or corrupt data â€” fall back to byte-only.
                return None, False
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="zed_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
                ...
                try:
                    ...
                    pass
            if not resized:
                # Resize returned nothing â€” Pillow couldn't help.
                return None, True
            if triggered_by == "bytes":
                # Byte budget is the binding constraint â€” bytes must shrink.
                ...
                # exhausts its halving budget â€” it freezes the long side once
                ...
                # be decoded â€” preserves historical byte-only behaviour.)
            ...
            if new_dims is not None:
                if max(new_dims) <= max_dimension:
                    return resized, False
                # Still over the per-side cap â€” the resize didn't satisfy it.
                return None, True
            ...
            logger.warning("image-shrink recovery: re-encode failed â€” %s", exc)
    ...
    if unshrinkable_oversized:
        ...
        # progress" even if other parts shrank â€” the caller will surface the
        ...
        logger.warning(
            ...
            "shrunk under %.0f MB â€” not retrying (would re-send rejected payload)",
