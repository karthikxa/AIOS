...
Three coexisting TTS extension surfaces â€” in resolution order:

1. **Built-in providers** (``BUILTIN_TTS_PROVIDERS`` in
   :mod:`tools.tts_tool`) â€” native Python implementations (edge, openai,
   elevenlabs, â€¦). **Always win** â€” plugins cannot shadow them.
2. **Command-type providers** declared under ``tts.providers.<name>:
   type: command`` (PR #17843, commit ``2facea7f7``). Wire any local
   CLI into Zed with shell-template placeholders. **Wins over a
   same-name plugin** â€” config is more local than plugin install.
...
shipped today) or ``~/.zed/plugins/tts/<name>/`` (user-installed).
None ship in-tree as of issue #30398 â€” the hook is additive
infrastructure waiting for a real consumer (Cartesia, Fish Audio, â€¦).
...
failure â€” the dispatcher converts exceptions into the standard
``{success: False, error: â€¦}`` JSON envelope the rest of Zed
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    Everything else has sane defaults â€” override only what your provider
    needs.
    """
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``zed tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise â€” used by the picker and ``zed setup`` for
    ...
    def list_voices(self) -> List[Dict[str, Any]]:
        ...
        Each entry::

            {
                ...
                "display": "Aria â€” neutral female",    # optional; defaults to id
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``zed tools`` picker.
    ...
    def synthesize(
        ...
        **extra: Any,
        ...
        (typically just echoes ``output_path``). Raises on failure â€”
    ...
    def voice_compatible(self) -> bool:
        ...
        Default: False (safe â€” providers opt in explicitly).
        """
        return False
