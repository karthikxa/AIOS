...
#
# When a user invokes a /skill (or /bundle), Zed expands the turn into a
# model-facing message that embeds the full skill body plus scaffolding. That
# expanded text is what flows into the agent loop â€” and into memory providers
...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_zed_scaffolding.
# ---------------------------------------------------------------------------
...
def extract_user_instruction_from_skill_message(content: Any) -> Optional[str]:
    ...
    Returns:
        ...
        - ``None`` when the content is skill scaffolding with no user
          ...
          memory providers should skip the turn in that case â€” there is no
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``ZED_PLATFORM`` env var and
    ``ZED_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = os.getenv("ZED_PLATFORM")
...
def _load_skill_payload(skill_identifier: str, task_id: str | None = None) -> tuple[dict[str, Any], Path | None, str] | None:
    ...
    try:
        ...
        if identifier_path.is_absolute():
            ...
            # find a skill via ~/.zed/skills/<name> where <name> is a
    ...
    skill_dir = None
    # Prefer the absolute skill_dir returned by skill_view() â€” this is
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.zed.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_zed_home()}/config.yaml):"]
...
def _build_skill_message(
    ...
    # â”€â”€ Template substitution and inline-shell expansion â”€â”€
    ...
    # â”€â”€ Inject the absolute skill directory so the agent can reference
    #    bundled scripts without an extra skill_view() round-trip. â”€â”€
    if skill_dir:
        parts.append("")
    ...
    # â”€â”€ Inject resolved skill config values â”€â”€
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.zed/skills/ and return a mapping of /command -> skill info.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.zed/skills/`` and any ``skills.external_dirs`` so the
    ...
    â€” they don't need to be in the system prompt for the model to use them.
    ...
    Returns:
        ...
        ``description:`` field â€” the same string the system prompt renders
...
def build_skill_invocation_message(
    ...
    try:
        ...
        pass  # Non-critical â€” skill invocation proceeds regardless
