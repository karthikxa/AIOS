...
Two coexisting STT extension surfaces â€” in resolution order:

1. **Built-in providers** (``BUILTIN_STT_PROVIDERS`` in
   :mod:`tools.transcription_tools`) â€” native Python implementations
   for the 6 backends shipped today (faster-whisper, local_command,
   Groq, OpenAI, Mistral, xAI). **Always win** â€” plugins cannot
   shadow them. The single-env-var shell escape hatch
   ``ZED_LOCAL_STT_COMMAND`` is preserved via the built-in
   ``local_command`` path.
2. **Plugin-registered providers** (this ABC). For new STT backends â€”
   OpenRouter, SenseAudio, Gemini-STT, custom proprietary engines â€”
...
``~/.zed/plugins/transcription/<name>/`` (user-installed).
...
class TranscriptionProvider(abc.ABC):
    ...
    def transcribe(
        ...
        **extra: Any,
        ...
        Implementations should NOT raise â€” convert exceptions to the
        ...
        Args:
            ...
            language: Optional BCP-47 language hint (e.g. ``"en"``,
                ``"ja"``) â€” providers without language hints should
