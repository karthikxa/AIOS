...
Zed keeps ``api_mode='chat_completions'`` for the ``gemini`` provider so the
...
Why this exists
---------------
Google's OpenAI-compatible endpoint has been brittle for Zed's multi-turn
...
def probe_gemini_tier(
    ...
    *,
    ...
    - ``"free"``    -- key is on the free tier (unusable with Zed)
...
_FREE_TIER_GUIDANCE = (
    ...
    "gemini-2.5-flash). Zed typically makes 3-10 API calls per user turn, "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Zed retry/error classification."""
...
def build_gemini_request(
    *,
    ...
    if max_tokens is not None:
        ...
        # maxOutputTokens as "use the model's full output budget" â€” it applies
        # a low internal default and the model stops early with
        # finishReason=MAX_TOKENS, truncating tool calls mid-stream (Zed
        # then retries 3Ã— and refuses the incomplete call). Every current
        ...
        # the field genuinely means full budget â€” that assumption does not
...
class GeminiNativeClient:
        ...
        **_: Any,
        ...
        if not (api_key or "").strip():
            raise RuntimeError(
                ...
                "Set GOOGLE_API_KEY or GEMINI_API_KEY in your environment / ~/.zed/.env "
                "(get one at https://aistudio.google.com/app/apikey), or run `zed setup` "
    ...
    def _headers(self) -> Dict[str, str]:
        headers = {
            ...
            "User-Agent": "zed-agent (gemini-native)",
        }
