...
``invoke_hook()`` site dispatches to the configured shell scripts â€” with
zero changes to call sites.

Design notes
------------
* Python plugins and shell hooks compose naturally: both flow through
  :func:`zed_cli.plugins.invoke_hook` and its aggregators.  Python
  plugins are registered first (via ``discover_and_load()``) so their
  block decisions win ties over shell-hook blocks.
* Subprocess execution uses ``shlex.split(os.path.expanduser(command))``
  with ``shell=False`` â€” no shell injection footguns.  Users that need
  pipes/redirection wrap their logic in a script.
* First-use consent is gated by the allowlist under
  ``~/.zed/shell-hooks-allowlist.json``.  Non-TTY callers must pass
  ``accept_hooks=True`` (resolved from ``--accept-hooks``,
  ``ZED_ACCEPT_HOOKS``, or ``hooks_auto_accept: true`` in config)
  for registration to succeed without a prompt.
* Registration is idempotent â€” safe to invoke from both the CLI entry
  point (``zed_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
**stdout** (JSON, optional â€” anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # Zed-canonical
...
# it triggers ``_record_approval`` â€” reusing it here would self-deadlock
...
class ShellHookSpec:
    ...
    def __post_init__(self) -> None:
        ...
        # folding) â€” a matcher of " terminal" would otherwise silently fail
        ...
        if self.matcher:
            try:
                ...
                logger.warning(
                    "shell hook matcher %r is invalid (%s) â€” treating as "
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``zed_cli.config.load_config``
    output).  The ``hooks:`` key is read out of it.  Missing, empty, or
    non-dict ``hooks`` is treated as zero configured hooks.

    ``accept_hooks=True`` skips the TTY consent prompt â€” the caller is
    promising that the user has opted in via a flag, env var, or config
    setting.  ``ZED_ACCEPT_HOOKS=1`` and ``hooks_auto_accept: true`` are
    ...
    # Import lazily â€” avoids circular imports at module-load time.
    ...
    for spec in specs:
        ...
        if not already_allowlisted:
            if not _prompt_and_record(
                ...
                logger.warning(
                    "shell hook for %s (%s) not allowlisted â€” skipped. "
                    "Use --accept-hooks / ZED_ACCEPT_HOOKS=1 / "
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``zed hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    Malformed entries warn-and-skip â€” we never raise from config parsing
    ...
    for event_name, entries in hooks_cfg.items():
        if event_name not in VALID_HOOKS:
            ...
            if suggestion:
                logger.warning(
                    "unknown hook event %r in hooks: config â€” did you mean %r?",
...
def _parse_single_entry(
    ...
    if matcher is not None and event not in {"pre_tool_call", "post_tool_call"}:
        logger.warning(
            "hooks.%s[%d].matcher=%r will be ignored at runtime â€” the "
...
def _spawn(spec: ShellHookSpec, stdin_json: str) -> Dict[str, Any]:
    ...
    subprocess is actually invoked â€” both the live callback path
...
def _make_callback(spec: ShellHookSpec) -> Callable[..., Optional[Dict[str, Any]]]:
    ...
    def _callback(**kwargs: Any) -> Optional[Dict[str, Any]]:
        # Matcher gate â€” only meaningful for tool-scoped events.
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a Zed wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical Zed
    ``{"action": "block", "message": "..."}`` shape expected by
    :func:`zed_cli.plugins.get_pre_tool_call_block_message`.  This is
    the single most important correctness invariant in this module â€”
...
def allowlist_path() -> Path:
    ...
    return get_zed_home() / ALLOWLIST_FILENAME
...
def save_allowlist(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            ...
            "runs without --accept-hooks / ZED_ACCEPT_HOOKS).",
            p, exc,
        )
...
def _locked_update_approvals() -> Iterator[Dict[str, Any]]:
    ...
    20â€“50 simultaneous writers).  Falls back to an in-process lock on
    ...
    if fcntl is None:  # pragma: no cover â€” non-POSIX fallback
...
def _prompt_and_record(
    ...
    print(
        f"\nâš  Zed is about to register a shell hook that will run a\n"
...
def revoke(command: str) -> int:
    ...
    process â€” restart the CLI / gateway to drop them.
    """
    with _locked_update_approvals() as data:
...
def _resolve_effective_accept(
    ...
    Precedence (any truthy source flips us on):
      ...
      2. ``ZED_ACCEPT_HOOKS`` env var
      3. ``hooks_auto_accept: true`` in ``cli-config.yaml``
    """
    if accept_hooks_arg:
        return True
    env = os.environ.get("ZED_ACCEPT_HOOKS", "").strip().lower()
...
# ---------------------------------------------------------------------------
# Introspection (used by `zed hooks` CLI)
# ---------------------------------------------------------------------------
...
def script_is_executable(command: str) -> bool:
    ...
    to be readable â€” the interpreter doesn't care about the ``X_OK``
...
def run_once(
    ...
    Used by ``zed hooks test`` and ``zed hooks doctor``.

    ``kwargs`` is the same dict that :func:`zed_cli.plugins.invoke_hook`
    ...
    produce â€” otherwise scripts tested via ``zed hooks test`` could
    diverge silently from production behaviour.

    Returns the :func:`_spawn` diagnostic dict plus a ``parsed`` field
    holding the canonical Zed-wire-shape response."""
