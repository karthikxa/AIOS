...
from zed_cli.auth import _read_codex_tokens, resolve_codex_runtime_credentials
...
def render_account_usage_lines(snapshot: Optional[AccountUsageSnapshot], *, markdown: bool = False) -> list[str]:
    if not snapshot:
        return []
    header = f"ðŸ“ˆ {'**' if markdown else ''}{snapshot.title}{'**' if markdown else ''}"
    lines = [header]
    if snapshot.plan:
    ...
    for window in snapshot.windows:
        ...
        if window.reset_at:
            base += f" â€¢ resets {_format_reset(window.reset_at)}"
        elif window.detail:
            base += f" â€¢ {window.detail}"
        lines.append(base)
...
def _is_finite_num(v: Any) -> TypeGuard[float]:
    ...
    number in the positive branch â€” callers can then do arithmetic / pass it to
...
def build_nous_credits_snapshot(account_info) -> Optional[AccountUsageSnapshot]:
    ...
    """
    try:
        from zed_cli.nous_account import nous_portal_topup_url
        ...
        # Subscription usage gauge â€” only when the portal supplies a positive
        ...
        #     parses bare NaN/Infinity by default) â†’ would render "$nan"/"$inf"
        #     and a falsely-confident gauge;
        #   - remaining > cap (rollover balance spanning the period) â†’ monthly_credits
        ...
        if paid is False:
            details.append("Status: access depleted â€” top up to restore")
...
def nous_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    ...
    Dev override: when ZED_DEV_CREDITS_FIXTURE selects a fixture state, /usage
    ...
    """
    # Dev fixture short-circuit â€” render /usage from the injected state, no portal.
    try:
    ...
    try:
        ...
        logger.debug("credits â–¸ /usage portal fetch/render failed (fail-open)", exc_info=True)
        return []
...
def _snapshot_from_credits_state(state) -> Optional[AccountUsageSnapshot]:
    ...
    so ZED_DEV_CREDITS_FIXTURE can exercise /usage without a live account. The
    *_usd strings are mock display values here (not server balance to compute on);
    the % comes from CreditsState.used_fraction (micros math). Fail-open â†’ None.
    """
    try:
        if state is None:
            return None
        ...
        details.append("(dev fixture â€” ZED_DEV_CREDITS_FIXTURE)")
        return AccountUsageSnapshot(
            provider="nous",
            source="dev-fixture",
            fetched_at=_utc_now(),
...
class CreditsView:
    ...
    One portal fetch, one parse â€” consumed identically by the CLI panel, the
...
def build_credits_view(*, markdown: bool = False, timeout: float = 10.0) -> CreditsView:
    ...
    supplies its own affordance). Fail-open â†’ ``CreditsView(logged_in=False)``.
    """
    ...
    try:
        ...
        logger.debug("credits â–¸ /credits portal fetch failed (fail-open)", exc_info=True)
        return not_logged_in
    ...
    # Identity line â€” shown before any open (roadmap Â§4.4).
...
def _fetch_openrouter_account_usage(base_url: Optional[str], api_key: Optional[str]) -> Optional[AccountUsageSnapshot]:
    ...
    if isinstance(usage, (int, float)):
        ...
        details.append(" â€¢ ".join(usage_parts))
    return AccountUsageSnapshot(
        provider="openrouter",
        source="credits_api",
        fetched_at=_utc_now(),
