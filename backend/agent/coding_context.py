"""Coding-context awareness â€” base Zed, every interactive surface.

When the user runs Zed inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Zed shifts into a **coding posture**. This module is the
...
Architecture â€” one seam, many consumers
----------------------------------------
...
is *data* â€” it declares the toolset to collapse to, the operating brief to
...
domain reads the same resolved object instead of probing git/config itself:
  ...
  * **System prompt** â€” ``RuntimeMode.system_blocks()`` â†’ the operating brief +
    a live git/workspace snapshot (``agent/system_prompt.py``).
  * **Toolset** â€” ``RuntimeMode.toolset_selection()`` â†’ the ``coding`` toolset
  ...
  * **Delegation** â€” subagents inherit the parent's toolset and run through the
    same prompt builder, so the coding posture propagates to children for free.
  * **Model / memory / compression** â€” declared on the profile
...
once at prompt-build time and baked into the *stable* system-prompt tier â€” never
...
Activation (config ``agent.coding_context``):
  ...
  * ``auto`` (default) â€” posture (brief + snapshot) on an interactive coding
  ...
  * ``focus`` â€” like ``auto``, but additionally collapses the toolset to the
  ...
  * ``on`` â€” force the posture anywhere (incl. non-workspaces). Prompt-only.
...
logger = logging.getLogger("zed.coding_context")

CODING_TOOLSET = "coding"

# Surfaces where a coding posture makes sense under ``auto``. Messaging
# platforms (telegram, discord, slack, â€¦) are intentionally absent â€” a chat bot
...
# isn't (yet) a git repo. Cheap filename checks â€” no parsing.
_PROJECT_MARKERS = (
...
# Lockfile â†’ package manager, checked in priority order.
...
# patch-style diffs best; Anthropic models â€” and most open-weight coding
# models, whose RL scaffolds use str_replace-style editors â€” do best with
...
# apply_patch (V4A â€” apply_patch.lark) is the ONLY file editor, no
# str_replace-style tool exists, and the shipped model prompts say to use
# apply_patch even "for single file edits" â€” so a replace-mode nudge would
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    "patch": (
        ...
        "existing code use `patch` with `mode='patch'` (V4A diff) â€” including "
        "single-file edits. It's the edit format you handle most reliably.",
    ),
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         "gemini", "gemma", "deepseek", "qwen", "kimi", "glm", "grok",
         "zed", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
        "existing code prefer `patch` in `mode='replace'` â€” match a unique "
...
# in _ZED_CORE_TOOLS, so they're present on every surface this fires on.
CODING_AGENT_GUIDANCE = (
    ...
    "the repo, go look. Don't assume a library is available â€” check the project "
    ...
    "a substitute for editing â€” apply the change, then summarise it. Only show "
    ...
    "only what the task needs â€” no drive-by refactors, renames, or reformatting "
    "â€” and add any imports/dependencies your code requires.\n"
    "- If an edit fails to apply, re-read the file to get the current exact "
    "contents before retrying â€” don't repeat a stale patch. If the same region "
    ...
    "asked, and never read, print, or commit secrets â€” leave `.env` and "
    "credential files alone unless the user explicitly asks. The Workspace "
    "block below is a snapshot from session start â€” re-run `git status`/"
    "`git branch` before relying on it. Be concise: lead with the change or "
    "answer, not a preamble."
)


# â”€â”€ Context profiles (declarative posture definitions) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


@dataclass(frozen=True)
class ContextProfile:
    """A named operating posture. Pure data â€” consumers read these fields.

    ``toolset``      â€” collapse to this toolset (+ enabled MCP) when no explicit
                       selection is pinned; ``None`` keeps the platform default.
    ``guidance``     â€” operating brief injected into the stable system prompt;
                       ``""`` injects nothing.
    ``model_hint``   â€” routing preference key for smart model routing
                       (extension seam; not yet consumed by the router).
    ``memory_policy``â€” memory namespace/weighting hint (extension seam).
    ``compact_skill_categories`` â€” skill categories DEMOTED to names-only in
                       the system-prompt skill index under the opt-in ``focus``
                       mode. Never hidden: every skill name stays visible
                       (so memory-anchored recall keeps working) â€” only the
...
# (deny-list â€” anything not listed here, incl. custom user categories, keeps
# full entries). Coding-adjacent categories (devops, github, mcp,
# data-science, diagramming, research, security, â€¦) are intentionally absent.
...
def _marker_root(cwd: Path) -> Optional[Path]:
    ...
    even when the user is in a subdirectory. ``$HOME`` itself is skipped â€” a
...
def _detect_profile_name(mode: str, platform: str, cwd_str: str) -> str:
    ...
    signal â€” without the guard, every session anywhere under a dotfiles-managed
    ...
    if git_root is not None and git_root == _home():
        git_root = None  # dotfiles repo at $HOME â€” not a code workspace
    if git_root is not None or _marker_root(cwd) is not None:
        return CODING_PROFILE.name
    return GENERAL_PROFILE.name


# â”€â”€ RuntimeMode (the seam) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class RuntimeMode:
    ...
    mid-session â€” that would break the prompt cache.
    """
    ...
    # only to steer edit-format guidance toward the model's family â€” see
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        messaging for build notifications, â€¦) keeps it while coding.

        Callers apply this only when the user hasn't pinned an explicit
        selection (``--toolsets``, ``ZED_TUI_TOOLSETS``, â€¦); they never
    ...
    def system_blocks(self) -> list[str]:
        ...
        toward the `patch` mode it handles best â€” see ``_edit_format_line``.
        """
    ...
    def compact_skill_categories(self) -> frozenset[str]:
        ...
        for a lean prompt keep full entries for every category â€” index changes
        ...
        Demoted â€” never hidden â€” even under ``focus``. An earlier revision
        ...
        pitfalls, â€¦), and models do not reliably reach for ``skills_list`` to
...
def resolve_runtime_mode(
    *,
    ...
    """Resolve the operating posture once. Cheap â€” a handful of ``stat`` calls.
...
# â”€â”€ Back-compat surface (thin wrappers over RuntimeMode) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def is_coding_context(
    *,
    ...
    """Whether Zed should operate in its coding posture right now."""
...
def coding_selection(
    *,
    ...
    active â€” the default coding posture never overrides configured toolsets.
    """
...
def coding_compact_skill_categories(
    *,
    ...
    Empty outside the coding posture and outside the opt-in ``focus`` mode â€”
    the default posture never touches the skill index. Under ``focus``,
    demoted â€” never hidden: every skill name stays in the index and remains
...
def _enabled_mcp_servers(config: Optional[dict[str, Any]]) -> list[str]:
    """Names of MCP servers the user has enabled â€” kept in the coding posture.

    MCP servers (figma, browser, tophat, â€¦) are explicitly configured and part
...
# â”€â”€ git/workspace probe â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _read_small(path: Path) -> str:
    """Read a small text file, or ``""`` â€” never raises, never reads huge files."""
    try:
...
def _project_facts(root: Path) -> list[str]:
    ...
    The point is to hand the model its *verify loop* up front â€” which manifest,
    which package manager, and the exact test/lint/build commands â€” instead of
...
def build_coding_workspace_block(cwd: Optional[str | Path] = None) -> str:
    ...
    â€” so marker-only (non-git) projects still get a snapshot.
    """
    ...
    lines = ["Workspace (snapshot at session start â€” re-check with `git` before acting on it):"]
    ...
    if git_root is not None:
        ...
        # are shared state) but deliberately do NOT expose the primary tree path â€”
