"""Background memory/skill review â€” fork the agent to evaluate the turn.
...
See the ``zed-agent-dev`` skill (``references/self-improvement-loop.md``)
...
# Review-prompt strings â€” used by ``spawn_background_review_thread`` to build
...
_MEMORY_REVIEW_PROMPT = (
    ...
    "Focus on:\n"
    "1. Has the user revealed things about themselves â€” their persona, desires, "
...
_SKILL_REVIEW_PROMPT = (
    ...
    "ACTIVE â€” most sessions produce at least one skill update, even if "
    ...
    "  â€¢ User corrected your style, tone, format, legibility, or "
    ...
    "  â€¢ User corrected your workflow, approach, or sequence of steps. "
    "Encode the correction as a pitfall or explicit step in the skill "
    "that governs that class of task.\n"
    "  â€¢ Non-trivial technique, fix, workaround, debugging path, or "
    "tool-usage pattern emerged that a future session would benefit "
    "from. Capture it.\n"
    "  â€¢ A skill that got loaded or consulted this session turned out "
    "to be wrong, missing a step, or outdated. Patch it NOW.\n\n"
    "Preference order â€” prefer the earliest action that fits, but do "
    ...
    "packaged with three kinds of support files â€” use the right "
    "directory per kind:\n"
    "     â€¢ `references/<topic>.md` â€” session-specific detail (error "
    "transcripts, reproduction recipes, provider quirks) AND "
    "condensed knowledge banks: quoted research, API docs, external "
    "authoritative excerpts, or domain notes you found while working "
    "on the problem. Write it concise and for the value of the task, "
    "not as a full mirror of upstream docs.\n"
    "     â€¢ `templates/<name>.<ext>` â€” starter files meant to be "
    "copied and modified (boilerplate configs, scaffolding, a "
    "known-good example the agent can `reproduce with modifications`).\n"
    "     â€¢ `scripts/<name>.<ext>` â€” statically re-runnable actions "
    ...
    "today's task, it's wrong â€” fall back to (1), (2), or (3).\n\n"
    ...
    "reply â€” the background curator handles consolidation at scale.\n\n"
    "Protected skills (DO NOT edit these):\n"
    "  â€¢ Bundled skills (shipped with Zed, e.g. 'zed-agent').\n"
    "  â€¢ Hub-installed skills (installed via 'zed skills install').\n"
    "Pinned skills (marked via 'zed curator pin') CAN be improved â€” "
    ...
    "  â€¢ Environment-dependent failures: missing binaries, fresh-install "
    "errors, post-migration path mismatches, 'command not found', "
    "unconfigured credentials, uninstalled packages. The user can fix "
    "these â€” they are not durable rules.\n"
    "  â€¢ Negative claims about tools or features ('browser tools do not "
    "work', 'X tool is broken', 'cannot use Y from execute_code'). These "
    "harden into refusals the agent cites against itself for months "
    "after the actual problem was fixed.\n"
    "  â€¢ Session-specific transient errors that resolved before the "
    "conversation ended. If retrying worked, the lesson is the retry "
    "pattern, not the original failure.\n"
    "  â€¢ One-off task narratives. A user asking 'summarize today's "
    "market' or 'analyze this PR' is not a class of work that warrants "
    "a skill.\n\n"
    "If a tool failed because of setup state, capture the FIX (install "
    "command, config step, env var to set) under an existing setup or "
    "troubleshooting skill â€” never 'this tool does not work' as a "
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "**Skills**: how to do this class of task. Be ACTIVE â€” most "
    ...
    "  â€¢ User corrected your style, tone, format, legibility, "
    "verbosity, or approach. Frustration is a FIRST-CLASS skill "
    "signal, not just a memory signal. 'stop doing X', 'don't format "
    "like this', 'I hate when you Y' â€” embed the lesson in the skill "
    "that governs that task so the next session starts fixed.\n"
    "  â€¢ Non-trivial technique, fix, workaround, or debugging path "
    "emerged.\n"
    "  â€¢ A skill that was loaded or consulted turned out wrong, "
    "missing, or outdated â€” patch it now.\n\n"
    "Preference order for skills â€” pick the earliest that fits:\n"
    ...
    "Name at the class level â€” NOT a PR number, error string, "
    "codename, library-alone name, or 'fix-X / debug-Y' session "
    "artifact. If the name only fits today's task, fall back to (1), "
    "(2), or (3).\n\n"
    "User-preference embedding: when the user complains about how "
    "you handled a task, update the skill that governs that task â€” "
    "memory alone isn't enough. Memory says 'who the user is and "
    "what the current situation and state of your operations are'; "
    "skills say 'how to do this class of task for this user'. Both "
    "should carry user-preference lessons when relevant.\n\n"
    "If you notice overlapping existing skills, mention it â€” the "
    ...
    "and stop â€” but don't reach for that conclusion as a default."
)
            ...
            if is_skill:
                ...
                if action == "patch" and (old_string or new_string):
                    old_preview = old_string[:80].replace("\n", " ") + (
                        "â€¦" if len(old_string) > 80 else ""
                    )
                    new_preview = new_string[:80].replace("\n", " ") + (
                        "â€¦" if len(new_string) > 80 else ""
                    )
                    actions.append(
                        f"ðŸ“ Skill '{skill_name}' patched: "
                        f"\"{old_preview}\" â†’ \"{new_preview}\""
                    )
                elif action == "create" and description:
                    actions.append(f"ðŸ“ Skill '{skill_name}' created: {description}")
                elif action == "edit" and description:
                    actions.append(f"ðŸ“ Skill '{skill_name}' rewritten: {description}")
                else:
                    actions.append(f"ðŸ“ {message}" if message else f"Skill {action}")
            elif operations:
                for op in operations:
                    ...
                    if op_act == "add" and op_content:
                        preview = op_content[:max_preview] + ("â€¦" if len(op_content) > max_preview else "")
                        actions.append(f"{label} âž• {preview}")
                    elif op_act == "replace" and op_content:
                        preview = op_content[:max_preview] + ("â€¦" if len(op_content) > max_preview else "")
                        actions.append(f"{label} âœï¸ {preview}")
                    elif op_act == "remove" and op_old:
                        preview = op_old[:60] + ("â€¦" if len(op_old) > 60 else "")
                        actions.append(f"{label} âž– {preview}")
            elif action == "add" and content:
                preview = content[:max_preview] + ("â€¦" if len(content) > max_preview else "")
                actions.append(f"{label} âž• {preview}")
            elif action == "replace" and content:
                preview = content[:max_preview] + ("â€¦" if len(content) > max_preview else "")
                actions.append(f"{label} âœï¸ {preview}")
            elif action == "remove" and old_text:
                preview = old_text[:60] + ("â€¦" if len(old_text) > 60 else "")
                actions.append(f"{label} âž– {preview}")
            else:
...
def build_memory_write_metadata(
    ...
    *,
    ...
    metadata: Dict[str, Any] = {
        ...
        "platform": agent.platform or os.environ.get("ZED_SESSION_SOURCE", "cli"),
        "tool_name": "memory",
    }
    if task_id:
...
def _run_review_in_thread(
    ...
    try:
        with open(os.devnull, "w", encoding="utf-8") as _devnull, \
            ...
            # skill_manage). Those tools require Zed' own dispatch,
            # which the codex_app_server runtime bypasses entirely
            # (it runs the turn inside codex's subprocess). So when
            # the parent is on codex_app_server, downgrade the review
            # fork to codex_responses â€” same auth/credentials, but
            # talks to the OpenAI Responses API directly so Zed
            ...
            # in the request body â€” Anthropic's cache key includes it.
            ...
            # scratch (fresh _zed_now() timestamp, fresh
            # session_id, narrower toolset â†’ different skills_prompt)
            ...
            # produce a good memory/skill summary â€” compressing would strip
            ...
            try:
                review_agent.run_conversation(
                    user_message=(
                        ...
                        "at runtime â€” do not attempt them."
                    ),
        ...
        if actions:
            summary = " Â· ".join(dict.fromkeys(actions))
            agent._safe_print(
                f"  ðŸ’¾ Self-improvement review: {summary}"
            )
