"""
Session Insights Engine for Zed Agent.

Analyzes historical session data from the SQLite state database to produce
comprehensive usage insights â€” token consumption, cost estimates, tool usage
patterns, activity trends, model/platform breakdowns, and session metrics.

Inspired by Claude Code's /insights command, adapted for Zed Agent's
...
def _bar_chart(values: List[int], max_width: int = 20) -> List[str]:
    ...
    return ["â–ˆ" * max(1, int(v / peak * max_width)) if v > 0 else "" for v in values]
...
class InsightsEngine:
    ...
    def __init__(self, db):
        ...
        Args:
            db: A SessionDB instance (from zed_state.py)
        """
        self.db = db
        self._conn = db._conn
    ...
    def format_terminal(self, report: Dict) -> str:
        ...
        lines.append("  â•‘                    ðŸ“Š Zed Insights                    â•‘")
        ...
        if act.get("by_day"):
            lines.append("  ðŸ“… Activity Patterns")
            lines.append("  " + "â”€" * 56)
        ...
        if report.get("top_sessions"):
            lines.append("  ðŸ† Notable Sessions")
            lines.append("  " + "â”€" * 56)
    ...
    def format_gateway(self, report: Dict) -> str:
        ...
        lines.append(f"ðŸ“Š **Zed Insights** â€” Last {days} days\n")
        ...
        # Models (top 5)
        if report["models"]:
            lines.append("**ðŸ¤– Models:**")
            for m in report["models"][:5]:
                lines.append(f"  {m['model'][:25]} â€” {m['sessions']} sessions, {m['total_tokens']:,} tokens")
            lines.append("")
        ...
        if skills.get("top_skills"):
            ...
            for skill in skills["top_skills"][:5]:
                ...
                lines.append(
                    f"  {skill['skill']} â€” {skill['view_count']:,} loads, {skill['manage_count']:,} edits{suffix}"
                )
            lines.append("")
        ...
        if act.get("busiest_day") and act.get("busiest_hour"):
            ...
            ampm = "AM" if hr < 12 else "PM"
            display_hr = hr % 12 or 12
            lines.append(f"**ðŸ“… Busiest:** {act['busiest_day']['day']}s ({act['busiest_day']['count']} sessions), {display_hr}{ampm} ({act['busiest_hour']['count']} sessions)")
            if act.get("active_days"):
                lines.append(f"**Active days:** {act['active_days']}", )
