"""
Plugin LLM facade â€” host-owned LLM access for trusted plugins.
==============================================================

Plugins built on Zed Agent often need to make their own LLM calls
out-of-band â€” a hook that rewrites a tool error before the user sees
...
Today the only stable plugin surfaces extend an existing Zed
...
:class:`~zed_cli.plugins.PluginContext`:

* ``complete(messages, ...)`` â€” chat completion against the user's
  active model + auth.
* ``complete_structured(instructions=..., input=[...], json_schema=...)``
  â€” bounded structured inference with optional image inputs, JSON
...
Provider/model/agent_id/profile are explicit keyword arguments â€” no
embedded slugs, no shorthands. This mirrors Zed' main config
...
Untrusted plugins still get the default surface â€” they just can't
...
handles every provider, fallback chain, and per-task override Zed
supports.
"""
...
same shape â€” dicts are normalized internally. Dict shape::
...
class PluginLlmUsage:
    """Token + cost usage for a completion. All fields optional â€” providers
...
def _coerce_allowlist(raw: Any) -> tuple[Optional[frozenset], bool]:
    ...
    ``["*"]`` (or any list containing ``"*"``) â†’ ``(frozenset(), True)``.
    Any other list â†’ ``(frozenset({...}), False)``.
    Missing / non-list â†’ ``(None, False)`` meaning "no allowlist."
    """
...
def _resolve_trust_policy(plugin_id: str) -> _TrustPolicy:
    ...
    Missing config â†’ fully restrictive policy (default deny on every
    ...
    except Exception:  # pragma: no cover â€” config IO failure
...
def _extract_usage(response: Any) -> PluginLlmUsage:
    ...
    Tolerant of provider differences â€” Anthropic via the auxiliary
...
def _resolve_attribution(
    *,
    ...
    1. Explicit overrides win â€” if the plugin asked for ``provider="x"``
    ...
    3. ``response.model`` (if present) overrides the recorded model
       ...
       different model id than the request (e.g. ``gpt-4o`` â†’
...
class PluginLlm:
    ...
    Instances are constructed by :class:`zed_cli.plugins.PluginContext`
    and exposed as ``ctx.llm``. Plugins should not instantiate this
    directly â€” the constructor binds plugin identity for trust-gate
    enforcement.
    """
