...
Resolution order for text tasks (auto mode):
  ...
  3. Zed Portal (~/.zed/auth.json active provider)
...
# default 50% compaction trigger fires at ~136K â€” wasteful, since the model
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Zed triggers summarization.  Higher values delay
    compression and preserve more raw context.

    Per-model/route overrides:
      - Arcee Trinity Large Thinking â†’ 0.75 (preserve reasoning context).
      - gpt-5.5 on the Codex OAuth route â†’ 0.85, because Codex caps the window
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://zed-agent.zedteam.com",
    "X-Title": "Zed Agent",
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``ZED_OPENROUTER_CACHE`` â€” truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``ZED_OPENROUTER_CACHE_TTL`` â€” integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("ZED_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("ZED_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "ZedAgent",
}
...
# Zed Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by Zed Portal.
#
# The tags are computed from agent.portal_tags so the client= marker stays
# in lockstep with zed_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    """Return a fresh Zed Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``zed_cli.__version__`` is
...
# Set at resolve time â€” True if the auxiliary client points to Zed Portal
auxiliary_is_nous: bool = False

# Default auxiliary models per provider
_OPENROUTER_MODEL = "google/gemini-3-flash-preview"
_NOUS_MODEL = "google/gemini-3-flash-preview"
_NOUS_DEFAULT_BASE_URL = "https://inference-api.zedteam.com/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_zed_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Zed Agent)",
...
def _to_openai_base_url(base_url: str) -> str:
    ...
    land on ``/anthropic/chat/completions`` â€” a 404.
    """
    url = str(base_url or "").strip().rstrip("/")
    if url.endswith("/anthropic"):
        # ZAI (open.bigmodel.cn) uses /api/anthropic for Anthropic wire
        # but /api/paas/v4 for OpenAI wire â€” the generic /v1 rewrite is wrong.
        ...
        logger.debug("Auxiliary client: rewrote base URL %s â†’ %s", url, rewritten)
        return rewritten
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``zed_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.zed/auth.json for an active Nous provider.
    ...
    try:
        ...
        logger.debug("Could not read Zed auth: %s", exc)
        return None


def _zed_api_key(provider: dict) -> str:
    """Extract a usable Zed inference JWT from stored auth state."""
    from zed_cli.auth import _nous_invoke_jwt_is_usable
...
def _nous_base_url() -> str:
    """Resolve the Zed inference base URL from env or default."""
    return os.getenv("NOUS_INFERENCE_BASE_URL", _NOUS_DEFAULT_BASE_URL)


def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Return fresh Zed runtime credentials when available.
    ...
    try:
        ...
        logger.debug("Auxiliary Zed runtime credential resolution failed: %s", exc)
        return None
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``zed auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``zed_cli.auth``'s singleton runtime resolver for older
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Zed auth store.
    ...
    try:
        from zed_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from zed_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from zed_cli.auth import is_provider_explicitly_configured
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        ...
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping Zed Portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
    ...
    if runtime is None and not nous:
        logger.warning(
            "Auxiliary Nous client unavailable: no Zed authentication found "
            "(run: zed auth)."
        )
    ...
    logger.debug("Auxiliary client: Zed Portal")
    ...
    model = _NOUS_MODEL
    try:
        from zed_cli.models import get_nous_recommended_aux_model
    ...
    if runtime is not None:
        ...
        api_key = _zed_api_key(nous or {})
        if not api_key:
            logger.warning(
                "Auxiliary Nous client unavailable: no usable inference JWT found "
                "(run: zed auth add nous)."
            )
            _mark_provider_unhealthy("nous", ttl=60)
            return None, None
...
def _refresh_nous_recommended_model(
    *, vision: bool, stale_model: Optional[str]
    ...
    """Re-fetch the Zed Portal's recommended model after a stale-model 404.
    ...
    try:
        ...
        logger.debug(
            "Zed recommended-model refresh failed (%s); using default %s",
            exc, _NOUS_MODEL,
        )
...
# per turn â€” no lock needed. Cleared by ``clear_runtime_main()``.
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `zed setup` or `zed model` and enter a valid http(s) base URL."
        ) from exc


def _try_zed_pro() -> Tuple[Optional[Any], Optional[str]]:
    """Try Zed Pro inference engine.

    Connects through the deploy server proxy on localhost:3000,
    which handles authentication automatically. No API key needed
    — the proxy injects its own auth header.
    """
    base_url = os.environ.get("ZED_PRO_BASE_URL", "").strip() or "http://localhost:3000/v1"
    model = _read_main_model() or "auto"
    logger.debug("Auxiliary client: Zed Pro (%s, auto-routing)", base_url)
    _clean_base, _dq = _extract_url_query_params(base_url)
    _extra = {"default_query": _dq} if _dq else {}
    client = OpenAI(api_key="zed-pro", base_url=_clean_base, **_extra)
    return client, model
...
def _try_azure_foundry(
    *,
    ...
    :func:`zed_cli.runtime_provider._resolve_azure_foundry_runtime` â€”
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from zed_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _try_anthropic(explicit_api_key: str = None) -> Tuple[Optional[Any], Optional[str]]:
    ...
    # when the configured provider is anthropic â€” otherwise a non-Anthropic
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second zed
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``zed config aux reset``)."""
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        ...
        logger.debug("Auxiliary Zed paid-entitlement refresh check failed: %s", exc)
        return False
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.zedteam.com"):
        return "nous"
...
def _try_main_agent_model_fallback(
    ...
    if main_provider.lower() == skip:
        # The thing that failed IS the main model â€” nothing to fall back to.
        return None, None, ""
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Zed' built-in discovery
...
def _resolve_auto(
    ...
    #    scenario where a user switches providers via `zed model` but the
    #    old OPENAI_BASE_URL lingers in ~/.zed/.env. â”€â”€
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: zed model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.zed/.env",
...
def resolve_provider_client(
    ...
    # reach this function without an explicit model â€” the user picked their
    ...
    # â”€â”€ Zed Portal (OAuth) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Zed Portal not configured (run: zed auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: zed model)")
                return None, None
    ...
    # â”€â”€ xAI Grok OAuth (loopback PKCE â†’ Responses API) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: zed model -> xAI Grok OAuth â€” SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        from zed_cli.runtime_provider import _get_named_custom_provider
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: zed doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("zed_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "external_process":
        ...
        # AWS SDK providers (Bedrock) â€” use the Anthropic Bedrock client via
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes Zed Portal product tags when the auxiliary client is backed
    by Zed Portal. Returns empty dict otherwise.
    """
...
def _refresh_nous_auxiliary_client(
    *,
    ...
    """Refresh Zed runtime creds, rebuild the client, and replace the cache entry."""
...
def shutdown_cached_clients() -> None:
    ...
    with _client_cache_lock:
        for key, entry in list(_client_cache.items()):
            ...
            # Async clients: skip â€” we already neutered __del__ above.
            try:
                close_fn = getattr(client, "close", None)
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`zed_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def call_llm(
    ...
    *,
        ...
        if client is None:
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                raise RuntimeError(
                    ...
                    f"variable, or switch to a different provider with `zed model`."
                )
    ...
    try:
        ...
        # â”€â”€ Stale-model self-heal (Zed Portal recommendation drift) â”€â”€â”€
        ...
        _heal_is_nous = (
            ...
            or base_url_host_matches(_base_info, "inference-api.zedteam.com")
        )
        if _is_model_not_found_error(first_err) and _heal_is_nous:
            ...
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s: model %r no longer in Zed catalog; "
        ...
        # â”€â”€ Zed auth refresh parity with main agent â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        client_is_nous = (
            resolved_provider == "nous"
            or base_url_host_matches(_base_info, "inference-api.zedteam.com")
        )
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s: refreshed Zed runtime credentials after paid account check, retrying",
                    task or "call",
                )
        ...
        if _is_auth_error(first_err) and client_is_nous:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s: refreshed Zed runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        _heal_is_nous = (
            resolved_provider == "nous"
            or base_url_host_matches(_client_base, "inference-api.zedteam.com")
        )
        if _is_model_not_found_error(first_err) and _heal_is_nous:
            healed_model = _refresh_nous_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s (async): model %r no longer in Zed catalog; "
                    "retrying with refreshed recommendation %r",
                    task or "call", kwargs.get("model"), healed_model,
                )
                kwargs["model"] = healed_model
        ...
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s (async): refreshed Zed runtime credentials after paid account check, retrying",
                    task or "call",
                )
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
        ...
        if _is_auth_error(first_err) and client_is_nous:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s (async): refreshed Zed runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
        ...
        # gate â€” the provider cannot serve the request regardless of user intent.
