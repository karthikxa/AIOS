...
â†’ workload identity â†’ managed identity â†’ VS Code â†’ Azure CLI â†’ azd â†’
PowerShell â†’ broker).
...
* SDK-callable contract. The public entry point ``build_token_provider``
  returns a zero-arg callable produced by ``get_bearer_token_provider`` â€”
...
* Three explicit consumer-side helpers (display / cache / http-bearer)
  rather than one generic "materialize" function â€” splitting them by
...
* No persisted JWT. ``azure-identity`` caches in-process and (where
  available) in the OS keychain or ``~/.IdentityService``. Zed does
...
Requires: ``azure-identity`` (optional dependency â€” only needed when
...
# resources â€” users with that requirement override via
# ``model.entra.scope`` in config.yaml.
SCOPE_AI_AZURE_DEFAULT = "https://ai.azure.com/.default"

# ---------------------------------------------------------------------------
# Lazy SDK import â€” only loaded when the Entra path is actually used.
# ---------------------------------------------------------------------------
...
def has_azure_identity_installed() -> bool:
    ...
    Cheap check â€” does not walk the credential chain.
    """
    try:
...
def reset_credential_cache() -> None:
    ...
    ``build_credential`` with a plain (non-lru-cached) function â€” those
...
class EntraIdentityConfig:
    ...
    Captures the Zed-managed Entra knobs we need outside Azure SDK
    ...
    ``AZURE_*`` env vars â€” see the Bedrock pattern in
    ``zed_cli/runtime_provider.py:1310-1377`` for the analogous
...
def build_credential(config: EntraIdentityConfig) -> Any:
    ...
    Zed processes use exactly one Entra config at a time (the
    ...
    rarely), the LRU eviction handles it correctly â€” each call still
...
def build_token_provider(scope: Optional[str] = None,
    ...
    ``azure-identity``'s standard ``AZURE_*`` environment variables â€”
...
def has_azure_identity_credentials(scope: Optional[str] = None,
    ...
    error â€” never raises. Use for ``zed doctor`` /
    ``zed auth status`` / wizard preflight.
    ...
    False to make this strictly an "is installed?" check â€” used on hot
    paths like CLI startup where we never want pip to run.

    NOT used by ``is_provider_configured()`` â€” that path is structural
...
def describe_active_credential(config: Optional[EntraIdentityConfig] = None,
    ...
    Designed for ``zed doctor`` and the wizard preflight â€” never
...
# ---------------------------------------------------------------------------
# Consumer-side helpers â€” split by purpose to prevent accidental token
...
def materialize_bearer_for_http(value: Any) -> str:
    ...
    header outside the OpenAI SDK (e.g. ``zed_cli/azure_detect.py``).
    ...
    **Anthropic SDK integration:** the Anthropic Python SDK does not
    ...
    ``Authorization`` header per request â€” and that client is passed to
...
def build_bearer_http_client(token_provider: Callable[[], str], **httpx_kwargs: Any) -> Any:
    ...
    The Anthropic SDK (â‰¤ 0.86.0 at the time of writing) stores
    ...
    ``http_client=...``. The hook:
      ...
      1. Calls :func:`materialize_bearer_for_http` to mint a fresh JWT
         (azure-identity caches internally â€” this is cheap when the
    ...
    ``token_provider`` must be a zero-arg callable returning a string â€”
    ...
    try:
        import httpx
    except ImportError as exc:  # pragma: no cover â€” httpx ships with openai/anthropic
        raise ImportError(
    ...
    def _inject_bearer(request: "httpx.Request") -> None:
        try:
            ...
            # may have set â€” including our own placeholder sentinel
            # ``entra-id-bearer-via-http-hook`` from
            # ``_build_anthropic_client_with_bearer_hook`` â€” so the
            ...
            logger.warning(
                ...
                "â€” stripping Authorization headers. Azure will respond 401. "
                "Run `zed doctor` or `az login` to recover.",
                exc,
            )
