...
decomposition campaign (``~/.zed/plans/god-file-decomposition.md``, Phase 1
step 4 â€” the post-loop ``TurnFinalizer`` seam). ``run_conversation``'s tail
...
to the caller. The function is synchronous with a single return â€” mirroring the
...
def finalize_turn(
    ...
    *,
    ...
    if final_response is None and (
        ...
        # Budget exhausted â€” ask the model for a summary via one extra
        # API call with tools stripped.  _handle_max_iterations injects a
        # user message and makes a single toolless request.
        _turn_exit_reason = f"max_iterations_reached({api_call_count}/{agent.max_iterations})"
        agent._emit_status(
            f"âš ï¸ Iteration budget exhausted ({api_call_count}/{agent.max_iterations}) "
            "â€” asking model to summarise"
        )
        if not agent.quiet_mode:
            agent._safe_print(
                f"\nâš ï¸  Iteration budget exhausted ({api_call_count}/{agent.max_iterations}) "
                "â€” requesting summary..."
            )
        ...
        # itself â€” we must do it on its behalf.
        #
        ...
        _kanban_task = os.environ.get("ZED_KANBAN_TASK")
        if _kanban_task:
            try:
                ...
                try:
                    _kb._record_task_failure(
                        ...
                        error=(
                            ...
                            f"({api_call_count}/{agent.max_iterations}) â€” "
    ...
    # â”€â”€ Turn-exit diagnostic log â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # Always logged at INFO so agent.log captures WHY every turn ended.
    # When the last message is a tool result (agent was mid-work), log
    # at WARNING â€” this is the "just stops" scenario users report.
    ...
    if _last_msg_role == "tool" and not interrupted:
        # Agent was mid-work â€” this is the "just stops" case.
        logger.warning(
    ...
    # catches the specific case â€” reported by Ben Eng (#15524-adjacent)
    # â€” where a model issues a batch of parallel patches, half of them
    ...
    # When a turn ends abnormally after substantive work â€” empty content
    # after retries, a partial/truncated stream, a still-pending tool
    # result, or an iteration/budget limit â€” the user otherwise gets a
        ...
        try:
            if agent._turn_completion_explainer_enabled():
                ...
                _is_partial_fragment = (
                    ...
                    and _stripped[-1:] not in {".", "!", "?", "ã€‚", "ï¼", "ï¼Ÿ", "`", ")"}
                )
    ...
    # but stop at the user message that started this turn â€” anything
    ...
    for msg in reversed(messages):
        if msg.get("role") == "user":
            break  # turn boundary â€” don't cross into prior turns
    ...
    # Check skill trigger NOW â€” based on how many tool iterations THIS turn used.
    ...
    # Background memory/skill review â€” runs AFTER the response is delivered
    ...
    # called here â€” run_conversation() is called once per user message in
