...
def _build_codex_gpt55_autoraise_notice(autoraise: Dict[str, float]) -> str:
    ...
    return (
        f"â„¹ Codex gpt-5.5 caps context at 272K, so auto-compaction was raised "
        f"to {to_pct}% (from {from_pct}%) to use more of the window before "
        f"summarizing.\n"
        f"  Opt back out: zed config set compression.codex_gpt55_autoraise false"
    )
...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.zed/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from zed_cli.config import load_config as _load_pc_cfg
    ...
    # Credits tracking (dev-only, L0 usage-aware-credits) â€” updated from
    ...
    # report cumulative micros spent.  Surfaced behind ZED_DEV_CREDITS.
    ...
    # both live under ~/.zed/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from zed_logging import setup_logging, setup_verbose_logging
    setup_logging(zed_home=_ra()._zed_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that zed_logging.setup_logging() does not
        ...
        # handler level inside zed_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from zed_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `zed model` to "
                        "select a provider, or run `zed setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff ZED_KANBAN_TASK is set).
    ...
    # Session logs go into ~/.zed/sessions/ alongside gateway sessions
    zed_home = get_zed_home()
    agent.logs_dir = zed_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.zed/sessions/session_{sid}.json)
    ...
    try:
        from zed_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from zed_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "zed"
                    except Exception:
                        pass
    ...
    # Per-platform prompt-hint overrides (config.yaml â†’ platform_hints).
    # Lets an enterprise admin append to or replace Zed' built-in
    ...
    # compact at ~136K â€” half the usable context). Gated by an opt-out config
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from zed_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Zed Agent.  Choose a model with at least "
