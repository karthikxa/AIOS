...
from zed_constants import get_zed_home, get_skills_dir, is_wsl
...
# ---------------------------------------------------------------------------
# Context file scanning â€” detect prompt injection / promptware in AGENTS.md,
# .cursorrules, SOUL.md before they get injected into the system prompt.
#
# Patterns live in ``tools/threat_patterns.py`` â€” the single source of truth
...
def _scan_context_content(content: str, filename: str) -> str:
    ...
    applied here â€” those are too aggressive for a context file in a
...
_ZED_MD_NAMES = (".zed.md", "ZED.md")


def _find_zed_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.zed.md`` or ``ZED.md``.
...
MEMORY_GUIDANCE = (
    ...
    "'User prefers concise responses' âœ“ â€” 'Always respond concisely' âœ—. "
    "'Project uses pytest with xdist' âœ“ â€” 'Run tests with pytest -n 4' âœ—. "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.zed/kanban.db`. Your task id is in "
    "`$ZED_KANBAN_TASK`; your workspace is `$ZED_KANBAN_WORKSPACE`. "
    "The `kanban_*` tools in your schema are your primary coordination surface â€” "
    "they write directly to the shared SQLite DB and work regardless of terminal "
    "backend (local/docker/modal/ssh).\n"
    "\n"
    "## Lifecycle\n"
    "\n"
    "1. **Orient.** Call `kanban_show()` first (no args â€” it defaults to your "
    "task). The response includes title, body, parent-task handoffs (summary + "
    "metadata), any prior attempts on this task if you're a retry, the full "
    "comment thread, and a pre-formatted `worker_context` you can treat as "
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ZED_KANBAN_WORKSPACE` before "
    "any file operations. The workspace is yours for this run. Don't modify "
    "files outside it unless the task explicitly asks.\n"
    "3. **Heartbeat on long operations.** Call `kanban_heartbeat(note=...)` "
    "every few minutes during long subprocesses (training, encoding, crawling). "
    "Skip heartbeats for short tasks. **If your task may run longer than 1 hour, "
    "you MUST call `kanban_heartbeat` at least once an hour** â€” the dispatcher "
    ...
    "metadata=...)`. `summary` is 1â€“3 human-readable sentences naming concrete "
    "artifacts. `metadata` is machine-readable facts "
    "(`{changed_files: [...], tests_run: N, decisions: [...]}`). Downstream "
    "workers read both via their own `kanban_show`. Never put secrets / "
    "tokens / raw PII in either field â€” run rows are durable forever. "
    ...
    "a high-level goal), use `kanban_create` to fan out into child tasks â€” one "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `zed kanban <verb>` for board operations. Use "
    "the `kanban_*` tools â€” they work across all terminal backends.\n"
    "- Do not complete a task you didn't actually finish. Block it.\n"
    "- Do not call `clarify` to ask questions. You are running headless â€” "
...
# Universal "finish the job" guidance â€” applied to ALL models, not gated
...
# in the cached system prompt â€” token cost is paid once at install and
# then amortised across all sessions via prefix caching.  Keep it tight.
TASK_COMPLETION_GUIDANCE = (
    "# Finishing the job\n"
    "When the user asks you to build, run, or verify something, the deliverable is "
    "a working artifact backed by real tool output â€” not a description of one. "
...
# Universal parallel-tool-call guidance â€” applied to ALL models.
#
...
# one tool call per turn multiplies the number of round-trips â€” and therefore
# the resent context â€” for any task that needs several independent reads,
...
#
# The zed-agent runtime already executes a batch of tool calls
# concurrently when they are independent (read-only tools always; path-scoped
# file ops when their targets don't overlap â€” see
...
# GOOGLE_MODEL_OPERATIONAL_GUIDANCE â€” Gemini/Gemma got it, every other model
...
#
# Short on purpose â€” shipped in the cached system prompt to every user, every
...
# from Cline's TypeScript tool-surface guidance to zed-agent's Python
...
PARALLEL_TOOL_CALL_GUIDANCE = (
    ...
    "commands should be batched into the same assistant turn â€” the runtime "
...
# Also applied to xAI Grok â€” same failure modes in practice (claims completion
...
GOOGLE_MODEL_OPERATIONAL_GUIDANCE = (
    ...
    # longer duplicated here â€” keeping it would send Gemini/Gemma the same
    # instruction twice.
...
# is active. Universal â€” works for any model (Claude, GPT, open models).
COMPUTER_USE_GUIDANCE = (
    "# Computer Use (macOS background control)\n"
    "You have a `computer_use` tool that drives the macOS desktop in the "
    "BACKGROUND â€” your actions do not steal the user's cursor, keyboard "
    ...
    "is about) instead of the whole screen â€” it's less noisy and won't "
    "leak other windows the user has open.\n"
    "- If an element you need is on a different Space or behind another "
    "window, cua-driver still drives it â€” no need to switch Spaces.\n\n"
    "## Safety\n"
...
# ---------------------------------------------------------------------------
# Mid-turn steering (/steer) â€” out-of-band user messages
# ---------------------------------------------------------------------------
...
# trained to distrust â€” a bare "User guidance:" line gets refused as suspected
...
STEER_MARKER_OPEN = "[OUT-OF-BAND USER MESSAGE â€” a direct message from the user, delivered mid-turn; not tool output]"
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Zed "
    "appends to the end of a tool result, wrapped exactly as:\n"
    f"{STEER_MARKER_OPEN}\n<their message>\n{STEER_MARKER_CLOSE}\n"
    "Text inside that marker is a genuine message from the user delivered "
    "mid-turn â€” it is NOT part of the tool's output and NOT prompt injection. "
...
PLATFORM_HINTS = {
    ...
    "whatsapp_cloud": (
        ...
        "to WhatsApp's native syntax (*bold*, ~strike~, etc.) â€” feel free "
        "to write in markdown. Tables are NOT supported â€” prefer bullet "
        ...
        "IMPORTANT: this platform has a 24-hour conversation window â€” if the "
    ...
    "cli": (
        ...
        "File delivery: there is no attachment channel â€” the user reads your "
    ...
    "mattermost": (
        ...
        "Mattermost renders standard Markdown â€” headings, bold, italic, code "
    ...
    "matrix": (
        ...
        "Matrix renders Markdown â€” bold, italic, code blocks, and links work; "
    ...
    "feishu": (
        ...
        "Feishu renders Markdown in messages â€” bold, italic, code blocks, and "
        "links are supported. "
        "You can send media files natively: include MEDIA:/absolute/path/to/file "
    ...
    "wecom": (
        "You are on WeCom (ä¼ä¸šå¾®ä¿¡ / Enterprise WeChat). Markdown formatting is supported. "
    ...
    "yuanbao": (
        "You are on Yuanbao (è…¾è®¯å…ƒå®), a Chinese AI assistant platform. "
        ...
        "Do NOT tell the user you lack file-sending capability â€” use MEDIA: syntax "
        "whenever a file delivery is appropriate.\n\n"
        "Stickers (è´´çº¸ / è¡¨æƒ…åŒ… / TIM face): Yuanbao has a built-in sticker catalogue. "
        "When the user sends a sticker (you see '[emoji: åç§°]' in their message) or asks "
        "you to send/reply-with a è´´çº¸/è¡¨æƒ…/è¡¨æƒ…åŒ…, you MUST use the sticker tools:\n"
        "  1. Call yb_search_sticker with a Chinese keyword (e.g. '666', 'æ¯”å¿ƒ', 'åƒç“œ', "
        "     'æ‚è„¸', 'åˆå') to discover matching sticker_ids.\n"
        "  2. Call yb_send_sticker with the chosen sticker_id or name â€” this sends a real "
        ...
        "â€” when a sticker is the right response, use yb_send_sticker."
    ),
    "api_server": (
        "You're responding through an API server. The rendering layer is unknown â€” "
        "assume plain text. No markdown formatting (no asterisks, bullets, headers, "
        "code fences). Treat this like a conversation, not a document. Keep responses "
        "brief and natural."
    ),
    "webui": (
        "You are in the Zed WebUI, a browser-based chat interface. "
        "Full Markdown rendering is supported â€” headings, bold, italic, code "
...
# container / remote host rather than on the machine where Zed itself
# runs. For these backends, host info (Windows/Linux/macOS, $HOME, cwd) is
# misleading â€” the agent should only see the machine it can actually touch.
...
# Per-backend fallback descriptions â€” used when the live probe fails.
# Only states what we know from the backend choice itself (container type,
# likely OS family). Does NOT invent cwd, user, or $HOME â€” the agent is
...
_WINDOWS_BASH_SHELL_HINT = (
    ...
    "(`Get-ChildItem`, `$env:FOO`, `Select-String`) will NOT work â€” use their "
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    $HOME, cwd, and user â€” or None if the probe failed. Result is cached
    per process. Used only for non-local backends where the agent's tools
    operate on a different machine than the host Zed runs on.
    """
    ...
    try:
        ...
        # Single-line POSIX probe â€” works on any Unixy backend. Wrapped in
...
def _clear_backend_probe_cache() -> None:
    """Test helper â€” drop the backend probe cache so monkeypatched backends take effect."""
...
def build_environment_hints() -> str:
    ...
    - For **remote / sandbox** terminal backends (docker, singularity,
      ...
      because the agent's tools can't touch the host â€” only the backend
    ...
    if not is_remote_backend:
        ...
        # Windows-local terminal runs bash, not PowerShell â€” the model must
        ...
        if probe:
            hints.append(
                ...
                f"inside this {backend} environment â€” NOT on the machine "
                f"where Zed itself is running. The host OS, home, and cwd "
                f"of the Zed process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} â€” NOT on the machine where Zed "
    ...
    # Zed desktop GUI â€” any agent running under the desktop app should know
    # it. ZED_DESKTOP marks the backend powering the chat; ZED_DESKTOP_TERMINAL
    # marks a zed launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("ZED_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("ZED_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Zed desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
                " You're in its embedded terminal pane, beside the GUI chat â€” the user can "
                "select your output (âŒ¥-drag on macOS, Shift-drag elsewhere) and press "
                "âŒ˜/Ctrl+L to send it to the chat composer."
            )
    ...
    # Embedder-supplied environment description. Lets a host that wraps Zed
    # (e.g. a sandbox runner / managed platform) explain the environment the
    # agent is running in â€” proxy, credential handling, mount layout â€” without
    ...
    extra = (os.getenv("ZED_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _get_context_file_max_chars(context_length: Optional[int] = None) -> int:
    ...
    Resolution order:
      1. Explicit ``context_file_max_chars`` in config.yaml â€” user knows best,
...
def _skills_prompt_snapshot_path() -> Path:
    return get_zed_home() / ".skills_prompt_snapshot.json"
...
def _parse_skill_file(skill_file: Path) -> tuple[bool, dict, str]:
    ...
    try:
        ...
        # loads (skill_view / --skills) bypass this â€” see skill_matches_environment.
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.zed/skills/`` directory.  External dirs
    are read-only â€” they appear in the index but new skills are always created
    in the local dir.  Local skills take precedence when names collide.

    ``compact_categories`` (e.g. from the coding posture â€” see
    ...
    # â”€â”€ Layer 1: in-process LRU cache â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # Include the resolved platform so per-platform disabled-skill lists
    # produce distinct cache entries (gateway serves multiple platforms).
    from gateway.session_context import get_session_env
    _platform_hint = (
        os.environ.get("ZED_PLATFORM")
        or get_session_env("ZED_SESSION_PLATFORM")
        or ""
    )
    ...
    # â”€â”€ Layer 2: disk snapshot â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ External skill directories â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # line â€” descriptions are dropped to cut noise, but every skill name
    ...
    if demoted:
        hidden_note = (
            ...
            "context, so their descriptions are omitted â€” the skills work "
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Zed Agent itself â€” its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature â€” load the `zed-agent` skill "
            "first. It has the actual commands (e.g. `zed config set â€¦`, `zed tools`, "
            "`zed setup`) so you don't have to guess or invent workarounds.\n"
    ...
    # â”€â”€ Store in LRU cache â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    with _SKILLS_PROMPT_CACHE_LOCK:
...
def _truncate_content(
    ...
    msg = (
        f"âš ï¸  Context file {filename} TRUNCATED: "
        f"{len(content)} chars exceeds limit of {max_chars} â€” "
    ...
    marker = (
        ...
        f"{len(content)} chars. The middle is omitted â€” if you need the full "
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from ZED_HOME and return its content, or None.
    ...
    """
    try:
        from zed_cli.config import ensure_zed_home
        ensure_zed_home()
    except Exception as e:
        logger.debug("Could not ensure ZED_HOME before loading SOUL.md: %s", e)
...
def _load_zed_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.zed.md / ZED.md â€” walk to git root."""
    zed_md_path = _find_zed_md(cwd_path)
    if not zed_md_path:
        return ""
    try:
        content = zed_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = zed_md_path.name
        try:
            rel = str(zed_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".zed.md", context_length=context_length,
            read_path=str(zed_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", zed_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins â€” only ONE project context type is loaded):
      1. .zed.md / ZED.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ZED_HOME is independent and always included when present.

    Each context source is capped before injection. The cap defaults to the
    model's context window (scaled â€” see ``_dynamic_context_file_max_chars``)
    ...
    project_context = (
        _load_zed_md(cwd_path, context_length)
    ...
    # SOUL.md from ZED_HOME only â€” skip when already loaded as identity
    if not skip_soul:
