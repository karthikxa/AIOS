"""Unified removal contract for every credential source Zed reads from.

Zed seeds its credential pool from many places:

    env:<VAR>     â€” os.environ / ~/.zed/.env
    claude_code   â€” ~/.claude/.credentials.json
    zed_pkce   â€” ~/.zed/.anthropic_oauth.json
    device_code   â€” auth.json providers.<provider> (nous, openai-codex, ...)
    qwen-cli      â€” ~/.qwen/oauth_creds.json
    gh_cli        â€” gh auth token
    config:<name> â€” custom_providers config entry
    model_config  â€” model.api_key when model.provider == "custom"
    manual        â€” user ran `zed auth add`

Each source has its own reader inside ``agent.credential_pool._seed_from_*``
(which keep their existing shape â€” we haven't restructured them).  What we
unify here is **removal**:

    ``zed auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all â€” so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), zed_pkce, copilot gh_cli, and
...
class RemovalResult:
    ...
    Attributes:
        ...
        suppress: Whether to call ``suppress_credential_source`` after
            ...
            Default True â€” almost every source needs this to stay sticky.
...
class RemovalStep:
    ...
    Attributes:
        provider: Provider pool key (``"xai"``, ``"anthropic"``, ``"nous"``, ...).
            Special value ``"*"`` means "matches any provider" â€” used for
...
def find_removal_step(provider: str, source: str) -> Optional[RemovalStep]:
    ...
    correct behaviour for ``manual`` entries â€” they were only ever stored
...
# ---------------------------------------------------------------------------
# Individual RemovalStep implementations â€” one per source.
# ---------------------------------------------------------------------------
# Each remove_fn is intentionally small and single-purpose.  Adding a new
# credential source means adding ONE entry here â€” no other changes to
# auth_remove_command.


def _remove_env_source(provider: str, removed) -> RemovalResult:
    """env:<VAR> â€” the most common case.

    Handles three user situations:
      1. Var lives only in ~/.zed/.env  â†’ clear it
      2. Var lives only in the user's shell (shell profile, systemd
         EnvironmentFile, launchd plist) â†’ hint them where to unset it
      3. Var lives in both â†’ clear from .env, hint about shell
    """
    from zed_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Zed.",
            f"  The pool entry is now suppressed â€” Zed will ignore "
            f"{env_var} until you run `zed auth add {provider}`.",
        ])
    else:
        result.hints.append(
            f"Suppressed env:{env_var} â€” it will not be re-seeded even "
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    We don't delete it â€” the user's Claude Code install still needs to
    work.  We just suppress it so Zed stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential â€” it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `zed auth add anthropic` to re-enable if needed.",
    ])


def _remove_zed_pkce(provider: str, removed) -> RemovalResult:
    """~/.zed/.anthropic_oauth.json is ours â€” delete it outright."""
    from zed_constants import get_zed_home

    result = RemovalResult()
    oauth_file = get_zed_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Zed Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    """Nous OAuth lives in auth.json providers.nous â€” clear it and suppress.

    We suppress in addition to clearing because nothing else stops a future
    `zed auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `zed auth add nous` to re-engage, which is the
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    """MiniMax OAuth lives in auth.json providers.minimax-oauth â€” clear it.
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    """xAI OAuth tokens live in auth.json providers.xai-oauth â€” clear them.

    Without this step, ``zed auth remove xai-oauth <N>`` silently undoes
    ...
    entry from the still-present singleton â€” credentials reappear with no
    user feedback. Clearing the singleton in step with the suppression set
    by the central dispatcher makes the removal stick.

    Belt-and-braces against the manual entry path: ``zed auth add
    xai-oauth`` produces a ``manual:xai_pkce`` entry whose removal step
    falls through to "unregistered â†’ nothing to clean up" (correct â€”
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `zed model` â†’ xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Zed auth store is not enough â€” _seed_from_singletons() would
    ...
    via ``zed auth add openai-codex``), but in both cases the re-seed
    gate lives at the ``"device_code"`` suppression key.  We suppress
    that canonical key here; the central dispatcher also suppresses
    ``removed.source`` which is fine â€” belt-and-suspenders, idempotent.
    """
    ...
    result.hints.extend([
        "Suppressed openai-codex device_code source â€” it will not be re-seeded.",
        "Note: Codex CLI credentials still live in ~/.codex/auth.json",
        "Run `zed auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as claude_code â€” suppress, don't delete.  The user's
    Qwen CLI install still reads from that file.
    """
    return RemovalResult(hints=[
        "Suppressed qwen-cli credential â€” it will not be re-seeded.",
        "Note: Qwen CLI credentials still live in ~/.qwen/oauth_creds.json",
        "Run `zed auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    We don't touch the user's gh CLI or shell state â€” just suppress so
    Zed stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        "Suppressed all copilot token sources (gh_cli + env vars) â€” they will not be re-seeded.",
        "Note: Your gh CLI / shell environment is unchanged.",
        "Run `zed auth add copilot` to re-enable if needed.",
    ])
...
def _remove_custom_config(provider: str, removed) -> RemovalResult:
    ...
    model.api_key.  Both are in config.yaml â€” modifying that from here
    ...
    return RemovalResult(hints=[
        f"Suppressed {source_label} â€” it will not be re-seeded.",
...
def _register_all_sources() -> None:
    ...
    ORDER MATTERS â€” ``find_removal_step`` returns the first match.  Put
    ...
    register(RemovalStep(
        provider="anthropic", source_id="zed_pkce",
        remove_fn=_remove_zed_pkce,
        description="~/.zed/.anthropic_oauth.json",
    ))
    register(RemovalStep(
