...
SUMMARY_PREFIX = (
    ...
    f"'{HISTORICAL_REMAINING_WORK_HEADING}' entirely â€” do not 'wrap up' or "
...
# heuristics. See https://github.com/NousResearch/zed-agent/issues/38389
#
...
# poisoning every subsequent request in the session â€” a bare key like
...
_HISTORICAL_SUMMARY_PREFIXES = (
    # Carveout era (#41607/#38364/#42812): "consistent â†’ use as background"
    ...
    "Asks' / '## Remaining Work', the latest message WINS â€” discard those "
    ...
    "summary â€” resume exactly from there. "
...
# dimensions (Anthropic â‰ˆ widthÃ—height/750, GPT-4o up to ~1700 for
# high-detail 2048Ã—2048, Gemini 258/tile), but 1600 is a realistic ceiling
...
# MEDIA delivery directives must not reach the summarizer â€” if one leaks into
...
def _content_length_for_budget(raw_content: Any) -> int:
    ...
    for p in raw_content:
        ...
        if ptype in {"image_url", "input_image", "image"}:
            ...
            # dicts â€” dimensions don't matter, only whether it's an image.
...
def _truncate_tool_call_args_json(args: str, head_chars: int = 200) -> str:
    ...
    appended ``...[truncated]`` â€” which routinely produced strings like::
    ...
    to begin with â€” some model backends use non-JSON tool arguments â€” the
...
def _strip_images_from_content(content: Any) -> Any:
    ...
    - List content: image parts become ``{"type": "text", "text": "[Attached
      image â€” stripped after compression]"}``; other parts are preserved as-is.
    ...
    for p in content:
        if _is_image_part(p):
            new_parts.append({
                "type": "text",
                "text": "[Attached image â€” stripped after compression]",
            })
        else:
            new_parts.append(p)
    return new_parts
...
def _strip_historical_media(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    shape the zed compressor emits).
    """
    if not messages:
        return messages
    ...
    # image â€” matching the problem kilocode#9434 set out to solve.
    anchor = -1
    ...
    if anchor <= 0:
        # No image-bearing user message, or it's the very first message â€”
...
class ContextCompressor(ContextEngine):
    """Default context engine â€” compresses conversation context via lossy summarization.
    ...
    def on_session_end(self, session_id: str, messages: List[Dict[str, Any]]) -> None:
        ...
        ``on_session_end()`` instead â€” which inherited a no-op from
    ...
    def update_model(
        ...
        # compressor stays calibrated after a model switch (e.g. 200K â†’ 32K).
    ...
    def should_defer_preflight_to_real_usage(self, rough_tokens: int) -> bool:
        ...
        overestimates schema-heavy requests so Zed compresses before a
    ...
    def should_compress(self, prompt_tokens: int = None) -> bool:
        ...
        if self._ineffective_compression_count >= 2:
            if not self.quiet_mode:
                logger.warning(
                    "Compression skipped â€” last %d compressions saved <10%% each. "
    ...
    def _prune_old_tool_results(
        ...
        for i in range(len(result) - 1, -1, -1):
            ...
            # Multimodal content â€” dedupe by the text summary if available.
            if isinstance(content, list):
                continue
            ...
            if h in content_hashes:
                # This is an older duplicate â€” replace with back-reference
        ...
        for i in range(prune_boundary):
            ...
            # payload â€” keep a lightweight text placeholder in its place.
        ...
        # result remains valid JSON â€” otherwise downstream providers 400
    ...
    # each message the summary model sees â€” the budget is the *summary*
...
Summary generation was unavailable, so this is a best-effort deterministic fallback for {len(turns_to_summarize)} compacted message(s).{reason_text}"""
    ...
    def _fallback_to_main_for_compression(self, e: Exception, reason: str) -> None:
        ...
        self._summary_failure_cooldown_until = 0.0  # no cooldown â€” retry immediately
    ...
    def _generate_summary(
        ...
        Returns None if all attempts fail â€” the caller should drop
        ...
        # user's configured timezone via zed_time.now(). The compaction summary
        # is a mid-conversation message that is NOT part of the cached prefix, so a
        # date here never affects prompt-cache stability. Resolved defensively â€”
        # a clock failure must never block compaction.
        try:
            from zed_time import now as _zed_now

            _today_str = _zed_now().strftime("%Y-%m-%d")
        ...
        _summarizer_preamble = (
            ...
            "or connection strings in the summary â€” replace any that appear "
...
input verbatim â€” the exact words they used. This includes:
...
"User asked: 'Waarom stond provider ineens op openrouter?' â€” needs investigation + answer"
...
verify the current diff' â€” earlier i18n in-flight work is cancelled."
...
{HISTORICAL_IN_PROGRESS_HEADING}
[Work currently underway â€” what was being done when compaction fired]
...
{HISTORICAL_PENDING_ASKS_HEADING}
[Questions or requests from the user that have NOT yet been answered or fulfilled. These are STALE â€” they were from the compacted turns. Write them here for reference only. The agent must NOT act on them unless the latest user message explicitly requests it. If none, write "None."]

## Relevant Files
[Files read, modified, or created â€” with brief note on each]

{HISTORICAL_REMAINING_WORK_HEADING}
[What remains to be done â€” framed as STALE context for reference only. The agent must NOT resume this work unless the latest user message explicitly asks for it.]

## Critical Context
[Any specific values, error messages, configuration details, or data that would be lost without explicit preservation. NEVER include API keys, tokens, passwords, or credentials â€” write [REDACTED] instead.]

Target ~{summary_budget} tokens. Be CONCRETE â€” include file paths, command outputs, error messages, line numbers, and specific values. Avoid vague descriptions like "made some changes" â€” say exactly what changed.
{_temporal_anchoring_rule}
...
Update the summary using this exact structure. PRESERVE all existing information that is still relevant. ADD new completed actions to the numbered list (continue numbering). Move items from "In Progress" to "Completed Actions" when done. Move answered questions to "Resolved Questions". Update "Active State" to reflect current state. Remove information only if it is clearly obsolete. CRITICAL: Update "## Active Task" to reflect the user's most recent unfulfilled input â€” this includes any question, decision request, or discussion turn that the assistant has not yet answered. Only write "None" if the last exchange was fully resolved.
...
This compaction should PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail â€” exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials â€” use [REDACTED]."""
        ...
        try:
            ...
            # Redact the summary output as well â€” the summarizer LLM may
            ...
            # No provider configured â€” long cooldown, unlikely to self-resolve
            ...
            # streaming premature-close) â€” shorter cooldown for JSON decode and
    ...
    def _strip_summary_prefix(summary: str) -> str:
        ...
        for prefix in (SUMMARY_PREFIX, LEGACY_SUMMARY_PREFIX, *_HISTORICAL_SUMMARY_PREFIXES):
            if text.startswith(prefix):
                ...
                break
        # Strip the trailing end marker too â€” a rehydrated handoff body that
    ...
    def _has_compressed_summary_metadata(message: Any) -> bool:
        ...
        relying on content-prefix heuristics.  The flag is in-process only â€”
                ...
                if msg.get("role") == "assistant":
                    for tc in msg.get("tool_calls") or []:
                        ...
                        if cid in missing_results:
                            patched.append({
                                "role": "tool",
                                "content": "[Result from earlier conversation â€” see context summary above]",
                                "tool_call_id": cid,
                            })
            messages = patched
            if not self.quiet_mode:
    ...
    def _protect_head_size(self, messages: List[Dict[str, Any]]) -> int:
        ...
        is always implicitly protected â€” it's load-bearing context that
        ...
        Examples:
          protect_first_n=0 â†’ system prompt only (or nothing if no system msg)
          protect_first_n=3 â†’ system + first 3 non-system messages
        """
        head = 0
    ...
    def _find_last_assistant_message_idx(
        ...
        textual content â€” i.e. one that the WebUI / TUI / SessionsPage
        ...
        in the compressible region â€” typically a fresh session that
    ...
    def _ensure_last_assistant_message_in_tail(
        ...
        visible assistant response inside the compressed middle region â€”
        ...
        into the single ``[CONTEXT COMPACTION â€” REFERENCE ONLY]`` block
        ...
        group that immediately precedes the anchored message â€” orphaned
    ...
    def _ensure_last_user_message_in_tail(
        ...
        messages *after* the summary â€” so the task effectively disappears from
        ...
        if last_user_idx < 0:
            # No user message found beyond head â€” nothing to anchor.
            return cut_idx
        ...
        # Pull cut_idx back to it directly â€” a user message is already a
    ...
    def _find_tail_cut_by_tokens(
        ...
        # or compresses a single message â€” both of which trigger the infinite
        ...
        if cut_idx <= head_end and accumulated <= soft_ceiling and accumulated > 0:
            ...
            # transcript), fall through â€” the existing fallback logic below
        ...
        # ``[CONTEXT COMPACTION â€” REFERENCE ONLY]`` block (fixes #29824).
        # Each anchor only walks ``cut_idx`` backward, so chaining them is
        # monotonic â€” the tail can only grow, never shrink.
    ...
    def compress(self, messages: List[Dict[str, Any]], current_tokens: int = None, focus_topic: str = None, force: bool = False) -> List[Dict[str, Any]]:
        ...
        """
        # Reset per-call summary failure state â€” callers inspect these fields
        ...
        if compress_start >= compress_end:
            # No compressable window â€” the entire transcript fits within
            ...
            if not self.quiet_mode:
                logger.warning(
                    ...
                    "â€” transcript fits within tail budget, nothing to compress. "
        ...
        if summary_idx is not None:
            ...
            # _previous_summary is non-empty â€” it was set by a different
        ...
        #   True  â†’ ABORT compression entirely. Return messages unchanged
        #           and set _last_compress_aborted=True so callers can warn
        #           the user and stop the auto-compress retry loop.
        #   False â†’ Fall through to the default fallback path below: insert
        ...
        if not summary and self.abort_on_summary_failure:
            ...
            if not self.quiet_mode:
                logger.warning(
                    "Summary generation failed â€” aborting compression "
        ...
        if summary_role == first_tail_role:
            ...
            if flipped != last_head_role:
                ...
                # (e.g. head=assistant, tail=user â€” neither role works).
