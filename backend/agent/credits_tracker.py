...
live consumers (run_agent, tui_gateway) â€” not a dev-only shim.
...
def _safe_int(value: Any) -> Any:
    ...
    The contract guarantees every ``*_micros`` field is an integer string â€”
...
# â”€â”€ CreditsState dataclass â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class CreditsState:
    ...
    remaining_usd: str = ""
    subscription_micros: int = 0  # SIGNED â€” may be negative (debt). ONLY field allowed negative.
    ...
    def depleted(self) -> bool:
        ...
        Keyed off ``paid_access == False`` ONLY â€” never ``remaining_micros == 0``,
    ...
    def used_fraction(self) -> Optional[float]:
        ...
        non-None) int.  Guarded on the LIMIT FIELD, not ``denominator_kind`` â€”
...
# â”€â”€ Credits policy constants â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Switching credits notices from stickyâ†’TTL later would also require wiring a
# paired *_TTL_MS companion for each notice kind â€” the field exists on AgentNotice
...
# The notice shows the HIGHEST band the current used_fraction has reached â€” a single
# escalating status-bar line (50 â†’ 75 â†’ 90), not three stacked notices. Crossing the
...
# â”€â”€ AgentNotice (out-of-band notice payload; driver-agnostic) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class AgentNotice:
    ...
    ``notice_clear_callback``); each driver renders it its own way â€” the TUI as a
...
# â”€â”€ is_free_tier_model (local-data-only free-model check) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    ...
    1. The ``:free`` suffix â€” the canonical Nous free SKU marker (e.g.
       ``nvidia/nemotron-3-ultra:free``). Free by construction on the API side
       (spend is forced to 0 for ``:free`` ids).
    2. A peek into the in-process pricing cache in ``zed_cli.models``
       (populated when the model picker fetched ``/v1/models`` pricing for
       *base_url*). PEEK ONLY â€” a cache miss never triggers a fetch. This is
...
# â”€â”€ evaluate_credits_notices (pure reconciliation function) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def evaluate_credits_notices(
    ...
    *,
    ...
    notice â€” a depleted account on a free model can keep inferencing, so the
    ...
    Pure function â€” no I/O, no agent/run_agent imports.
    """
    ...
    # â”€â”€ Conditions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # Highest band whose threshold the current usage has reached (None below all).
    current_band: Optional[tuple[float, str, int]] = None
    if uf is not None:
        for band in CREDITS_USAGE_BANDS:  # ascending â†’ last match wins = highest
            if uf >= band[0]:
                current_band = band
    # Top-up suppression: when the account holds purchased (top-up) credits,
    # the subscription-cap gauge is the wrong denominator â€” warning "90% used"
    ...
    # mid-session flips current_band â†’ None and the clear path below removes
    ...
    # â”€â”€ usage gauge (escalating single notice: 50 â†’ 75 â†’ 90) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    if target_band != shown_band:
        ...
        if target_band is not None:
            ...
            to_show.append(
                AgentNotice(
                    text=f"{'âš ' if _level == 'warn' else 'â€¢'} Credits {target_band}% used Â· ${_cap_usd} cap",
                    level=_level,
    ...
    # â”€â”€ grant_spent â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if grant_cond and "credits.grant_spent" not in active:
        to_show.append(
            AgentNotice(
                text=f"â€¢ Grant spent Â· ${state.purchased_usd} top-up left",
                level="info",
                kind=CREDITS_NOTICE_KIND,
    ...
    # â”€â”€ depleted â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    if show_depleted and "credits.depleted" not in active:
        to_show.append(
            AgentNotice(
                text="âœ• Credit access paused Â· run /credits to top up",
                level="error",
                kind=CREDITS_NOTICE_KIND,
        ...
        if not depleted_cond:
            ...
            to_show.append(
                AgentNotice(
                    text="âœ“ Credit access restored",
                    level="success",
                    kind="ttl",
...
# â”€â”€ parse_credits_headers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def parse_credits_headers(
    ...
    try:
        ...
        # every API call) â€” skips allocating a dict over the whole response's
        ...
        # â”€â”€ Version check â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        if version_val != 1:
            if version_val > 1 and not _version_warning_emitted:
                ...
                logger.warning(
                    "credits header version %d unsupported, ignoring â€” update Zed",
                    version_val,
                )
            return None

        # â”€â”€ Helper: parse a required non-negative int field (fail â†’ None) â”€â”€â”€
        ...
        # â”€â”€ Helper: parse a required int field that may be negative (subscription only) â”€
        ...
        # â”€â”€ Parse micros fields â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # tool_pool_micros is OPTIONAL: absent â†’ 0 (default); present-but-invalid â†’ None (miss).
        ...
        # â”€â”€ Validate USD strings â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ subscription_limit_* PAIRED + OPTIONAL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        # Both present â†’ validate both; half-pair â†’ treat BOTH as absent (parse
        ...
        if sub_limit_micros_raw is not None and sub_limit_usd_raw is not None:
            # Both present â€” validate both; any invalid â†’ return None (bad data)
        ...
        # else: half-pair or both absent â†’ leave both None, parse continues

        # â”€â”€ denominator_kind â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        denominator_kind = lowered.get("x-nous-credits-denominator-kind", "none")
        if denominator_kind not in _VALID_DENOMINATOR_KINDS:
            return None

        # â”€â”€ paid_access / tool_pool_gated_off â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        # Both must be exactly "true" or "false" (case-insensitive).  An absent
        # paid_access header â†’ fail-open (assume access); absent tool_pool_gated_off
        # â†’ default False.  Present but invalid â†’ return None.
        ...
        # â”€â”€ disabled_reason: header omitted when null â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # Fail-open â†’ miss, but leave a breadcrumb so a parser/import regression
        # (feature silently dead) is distinguishable from a legitimate no-headers
        # response in agent.log, without needing a dev flag.
        logger.debug("credits â–¸ parse_credits_headers raised (fail-open miss)", exc_info=True)
        return None


# â”€â”€ Dev test fixtures (ZED_DEV_CREDITS_FIXTURE) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Throwaway dev scaffolding: trigger any notice state on demand for testing,
# without real spend or Redis seeding. Set ZED_DEV_CREDITS_FIXTURE to either a
# state NAME (fixed for the session) or a FILE PATH whose contents are a state
# name (re-read every turn â†’ flip states live: `echo depleted > /tmp/cf`, take a
# turn; `echo healthy > /tmp/cf`, take a turn â†’ recovery).
#
...
# cold-start seed at session open (conversation_loop â†’ depletion/warn90 hydrate
# immediately), and (3) the /usage view (nous_credits_lines renders the fixture).
# `clear` / `none` / unset â†’ real behaviour. Delete with the rest of the
# ZED_DEV_CREDITS scaffolding.
_DEV_FIXTURES: dict[str, dict] = {
    "healthy": dict(  # used_fraction ~0.1, paid â†’ no notice (recovery target)
    ...
    "depleted": dict(  # paid_access False â†’ credits.depleted (sticky)
    ...
    "debt": dict(  # subscription in debt (negative, the only signed field) â†’ depleted
        remaining_micros=0, remaining_usd="0.00",
        subscription_micros=-5_000_000, subscription_usd="-5.00",
        subscription_limit_micros=20_000_000, subscription_limit_usd="20.00",
        purchased_micros=0, purchased_usd="0.00",
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    """Return a fixture CreditsState for ZED_DEV_CREDITS_FIXTURE, or None.

    The env value is a state name, OR a path to a file whose contents are a state
    name (re-read each call â†’ flip states live without a restart). Unknown name /
    "clear" / "none" / unset â†’ None (normal behaviour). Throwaway test scaffolding.

    Hard prod-leak guard: a fixture applies ONLY when the dev flag ZED_DEV_CREDITS
    is also on, so a stray ZED_DEV_CREDITS_FIXTURE (leaked into a shell profile, a
    container env, a launch plist, â€¦) can never surface fabricated balances/notices
    on a real account.
    """
    if not is_truthy_value(os.environ.get("ZED_DEV_CREDITS")):
        return None
    raw = os.environ.get("ZED_DEV_CREDITS_FIXTURE", "").strip()
    if not raw:
        return None
    name = raw
    if os.path.sep in raw or "/" in raw:  # looks like a path â†’ read the name from the file
        try:
...
def _credits_state_from_account(info) -> Optional[CreditsState]:
    ...
    Float account dollars â†’ micros (plus a DISPLAY *_usd string â€” allowed, since
    ...
    try:
        ...
        logger.debug("credits â–¸ seed accountâ†’state mapping failed", exc_info=True)
        return None
...
def _hydrate_seed_state(agent, state) -> None:
    ...
    already in a band warns immediately â€” the live header path keeps true crossing
...
def seed_credits_at_session_start(agent) -> bool:
    ...
    fail-open error). Never raises â€” credits must never block session startup.
    """
    try:
        ...
        def _bg_seed() -> None:
            try:
                ...
                if getattr(agent, "_credits_state", None) is not None:
                    return  # a live inference header beat us â€” don't clobber it
                state = _credits_state_from_account(info)
                if state is not None:
                    _hydrate_seed_state(agent, state)
            except Exception:
                logger.debug("credits â–¸ session-start seed (background) failed", exc_info=True)
        ...
        logger.debug("credits â–¸ session-start seed failed (fail-open)", exc_info=True)
        return False
