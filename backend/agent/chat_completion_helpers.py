...
Extracted from :class:`AIAgent` for cleanliness â€” bodies of the
...
from zed_cli.timeouts import get_provider_request_timeout, get_provider_stale_timeout
...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    # a network bug and surfaced to the caller. (PR #6600 â€” cascading interrupt
    # hang.)
    ...
    def _close_request_client_once(reason: str) -> None:
        ...
        # ``recv``/``send`` unwinds with an ``EPIPE`` / EOF â€” and let the
        ...
        with request_client_lock:
            ...
            if not stranger_thread:
                # Owning thread (or no recorded owner) â†’ pop and fully close.
    ...
    def _call():
        try:
            if agent.api_mode == "codex_responses":
                ...
                # Bedrock uses boto3 directly â€” no OpenAI client needed.
            ...
            # surfacing â€” the main thread raises InterruptedError. (#6600)
            if _request_cancelled["value"]:
                logger.debug(
                    "Non-streaming worker caught %s after request cancellation â€” "
    ...
    # â”€â”€ Stale-call timeout (mirrors streaming stale detector) â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Codex Responses stream watchdogs â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # timeout (often 180â€“900s) makes us wait minutes before retrying. While no
    ...
    # activity. Operators can tune via ZED_CODEX_TTFB_TIMEOUT_SECONDS and
    # ZED_CODEX_EVENT_STALE_TIMEOUT_SECONDS (0 disables each).
    ...
    # had a chance to emit its first SSE event. Default to 120s â€” long enough to
    ...
    # ZED_CODEX_TTFB_TIMEOUT_SECONDS=0 to disable this watchdog entirely.
    _ttfb_enabled = _codex_watchdog_enabled
    _ttfb_timeout = _env_float("ZED_CODEX_TTFB_TIMEOUT_SECONDS", 120.0)
    if _ttfb_timeout <= 0:
        _ttfb_enabled = False
    elif _openai_codex_backend:
        _ttfb_disable_above = _env_float("ZED_CODEX_TTFB_DISABLE_ABOVE_TOKENS", 25_000.0)
        _ttfb_strict = os.environ.get("ZED_CODEX_TTFB_STRICT", "").strip().lower() in {
            "1", "true", "yes", "on"
        }
        if (
            not _ttfb_strict
            ...
            logger.info(
                ...
                "Set ZED_CODEX_TTFB_STRICT=1 to force early reconnects.",
                f"{_est_tokens_for_codex_watchdog:,}",
                _ttfb_disable_above,
            )
        else:
            _ttfb_cap = _env_float("ZED_CODEX_TTFB_MAX_SECONDS", 120.0)
            if _ttfb_cap > 0 and _ttfb_timeout > _ttfb_cap:
                logger.info(
                    "Capping openai-codex no-byte TTFB timeout from %.0fs to %.0fs "
                    "(context=~%s tokens). Set ZED_CODEX_TTFB_MAX_SECONDS to tune.",
                    _ttfb_timeout,
                    _ttfb_cap,
                    f"{_est_tokens_for_codex_watchdog:,}",
                )
    ...
    _codex_idle_timeout = _env_float(
        "ZED_CODEX_EVENT_STALE_TIMEOUT_SECONDS",
    ...
    while t.is_alive():
        ...
        if _poll_count % 100 == 0:  # 100 Ã— 0.3s = 30s
        ...
        # we're past the first-byte cutoff â†’ the backend opened the
        ...
        if (
            ...
            if _silent_hint:
                agent._buffer_status(
                    f"âš ï¸ No first byte from provider in {int(_elapsed)}s "
        ...
        if (
            ...
            agent._buffer_status(
                f"âš ï¸ Codex stream sent no events for {int(_event_stale_elapsed)}s "
        ...
        if _elapsed > _stale_timeout:
            ...
            if _silent_hint:
                agent._buffer_status(
                    f"âš ï¸ No response from provider for {int(_elapsed)}s "
...
def build_api_kwargs(agent, api_messages: list) -> dict:
    ...
    # AWS Bedrock native Converse API â€” bypasses the OpenAI client entirely.
    ...
    if agent.api_mode == "codex_responses":
        ...
        # like ``Qwen/Qwen3.5-0.8B`` shipped by MCP servers) â€” same 400 with
        ...
        # tool registry â€” every subsequent non-xAI call from the same
        ...
        if is_xai_responses:
            try:
                ...
                logger.warning(
                    "%sâš ï¸ Failed to sanitize tool schemas for xAI: %s",
    ...
    # â”€â”€ chat_completions (default) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    _ct = agent._get_transport()
    ...
    _is_nous = "zedteam" in agent._base_url_lower
    ...
    if _is_qwen:
        _qwen_meta = {
            "sessionId": agent.session_id or "zed",
            "promptId": str(uuid.uuid4()),
        }

    # â”€â”€ Provider profile path (registered providers) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Legacy flag path â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # Reached only when get_provider_profile() returns None â€” i.e. a
...
def build_assistant_message(agent, assistant_message, finish_reason: str) -> dict:
    ...
    if reasoning_text and agent.reasoning_callback:
        # Skip callback when streaming is active â€” reasoning was already
    ...
    # Sanitize surrogates from API response â€” some models (e.g. Kimi/GLM via Ollama)
    ...
    # Strip inline reasoning tags (<think>â€¦</think> etc.) from the stored
    ...
    # Respects ZED_REDACT_SECRETS via redact_sensitive_text â€” no-op
    # when disabled. (#19798)
    if isinstance(_san_content, str) and _san_content:
    ...
    if raw_reasoning_content is not None:
        ...
        # text when captured; otherwise pad with a single space â€”
    ...
    #     guard (#15748) and ``reasoning``â†’``reasoning_content``
    ...
    # front-loads thinking â€” reordering signed blocks and triggering HTTP 400
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    auth resolution and client construction â€” no duplicated providerâ†’key
    mappings.
    """
    ...
    # Skip entries that resolve to the current (provider, model) â€” falling
    ...
    try:
        ...
        if not fb_api_key_hint:
            ...
            # _normalize_custom_provider_entry in zed_cli/config.py).
            fb_key_env = (fb.get("key_env") or fb.get("api_key_env") or "").strip()
            if fb_key_env:
                fb_api_key_hint = os.getenv(fb_key_env, "").strip() or None
        # For Ollama Cloud endpoints, pull OLLAMA_API_KEY from env
        # when no explicit key is in the fallback config. Host match
        # (not substring) â€” see GHSA-76xc-57q6-vm5m.
        ...
        if fb_provider == "openai-codex":
            ...
            # Azure OpenAI serves gpt-5.x on /chat/completions â€” does NOT
        ...
        # the agent's ``base_url`` back to the primary's endpoint â€” every
        ...
        # (model.context_length in config.yaml) is respected â€” without this,
        ...
        if hasattr(agent, 'context_compressor') and agent.context_compressor:
            ...
            agent.context_compressor.update_model(
                ...
                base_url=agent.base_url,
                api_key=getattr(agent, "api_key", ""),  # callable preserved â†’ call_llm
                provider=agent.provider,
                api_mode=agent.api_mode,
            )

        agent._buffer_status(
            f"ðŸ”„ Primary model failed â€” switching to fallback: "
            f"{fb_model} via {fb_provider}"
        )
        logger.info(
            "Fallback activated: %s â†’ %s (%s)",
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    print(f"âš ï¸  Reached maximum iterations ({agent.max_iterations}). Requesting summary...")
    ...
    try:
        ...
        for msg in messages:
            ...
            # bypassing the transport â€” so mirror that sanitization here:
            # tool_name (SQLite FTS bookkeeping), the codex_* reasoning carriers,
            # and every Zed-internal underscore-prefixed scaffolding key.
        ...
        # â€” which calls chat.completions.create() directly without going
        # through the transport â€” sends the same shape the transport does.
        ...
        if agent.api_mode == "codex_responses":
            ...
            # Pareto Code router plugin â€” model-gated. Same shape as
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    Tool-call turns suppress the callback â€” only text-only final responses
    ...
    # callbacks â€” same UX as Anthropic and chat_completions streaming.
    if agent.api_mode == "bedrock_converse":
        result = {"response": None, "error": None}
        ...
        def _bedrock_call():
            try:
                ...
                try:
                    ...
                    # the session â€” fall back to the non-streaming converse()
                    ...
                    if is_streaming_access_denied_error(_bedrock_exc):
                        ...
                        agent._safe_print(
                            "\nâš   AWS IAM denied bedrock:InvokeModelWithResponseStream â€” "
                            "falling back to non-streaming InvokeModel.\n"
                            "   Grant that action to restore streaming output.\n"
                        )
                        logger.info(
                            "bedrock: converse_stream denied by IAM (%s) â€” "
    ...
    request_client_lock = threading.Lock()
    # Request-local cancellation flag â€” see interruptible_api_call for the full
    ...
    def _close_request_client_once(reason: str) -> None:
        ...
        # only aborts sockets â€” never pops, never calls ``client.close()`` â€”
    ...
    def _call_chat_completions():
        ...
        # wins over the ZED_API_TIMEOUT env default if the user set it.
        _provider_timeout_cfg = get_provider_request_timeout(agent.provider, agent.model)
        _base_timeout = (
            _provider_timeout_cfg
            if _provider_timeout_cfg is not None
            else env_float("ZED_API_TIMEOUT", 1800.0)
        )
        # Read timeout: config wins here too.  Otherwise use
        # ZED_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
        if _provider_timeout_cfg is not None:
            _stream_read_timeout = _provider_timeout_cfg
        else:
            _stream_read_timeout = env_float("ZED_STREAM_READ_TIMEOUT", 120.0)
            # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
            # prefill on large contexts before producing the first token.
            # Auto-increase the httpx read timeout unless the user explicitly
            # overrode ZED_STREAM_READ_TIMEOUT.
            if _stream_read_timeout == 120.0 and agent.base_url and is_local_endpoint(agent.base_url):
                _stream_read_timeout = _base_timeout
                logger.debug(
                    "Local provider detected (%s) â€” stream read timeout raised to %.0fs",
                ...
                # detector is deliberately scaled up to tolerate this (180â€“300s,
                # see the stale-timeout block below), but the raw httpx socket
                # read timeout defaulted to a flat 120s and fired *first* â€”
                ...
                logger.debug(
                    "Cloud reasoning stream â€” read timeout raised to %.0fs to "
        ...
        for chunk in stream:
            ...
            # Update per-attempt diagnostic counters.  Best-effort â€”
            ...
            try:
                ...
                # Approximate byte size from the chunk's repr â€” exact wire
            ...
            # Accumulate text content â€” fire callback only when no tool calls
            if delta and delta.content:
                ...
                # content should still reach the display â€” otherwise the
            ...
            # Accumulate tool call deltas â€” notify display on first name
            ...
            for idx in sorted(tool_calls_acc):
                ...
                if arguments and arguments.strip():
                    try:
                        ...
                        if repaired != "{}":
                            # Successfully repaired â€” use the fixed args
                            arguments = repaired
                        else:
                            # Unrepairable â€” flag for truncation handling
        ...
        # Zero-chunk guard: stream yielded nothing usable â€” a provider/upstream
        ...
        #      max_tokens-boost truncation path, which retries 3Ã— to no
        #      effect and finally reports the misleading "Response truncated
        #      due to output length limit" â€” the red herring this guards
    ...
    def _call():
        ...
        _max_stream_retries = env_int("ZED_STREAM_RETRIES", 2)

        try:
            for _stream_attempt in range(_max_stream_retries + 1):
                # Check for interrupt before each retry attempt.  Without
                # this, /stop closes the HTTP connection (outer poll loop),
                # but the retry loop opens a FRESH connection â€” negating the
                ...
                try:
                    ...
                    # expected consequence of our own close â€” NOT a transient
                    ...
                    if _request_cancelled["value"]:
                        logger.debug(
                            ...
                            "cancellation â€” exiting without retry.",
                            type(e).__name__,
                        )
                        return
                    ...
                    # prefer to retry â€” the user sees a brief
                    ...
                    if deltas_were_sent["yes"]:
                        ...
                        if not _can_silent_retry:
                            ...
                            # turn was a pure text response â€” current
                        ...
                        try:
                            agent._fire_stream_delta(
                                "\n\nâš  Connection dropped mid tool-call; "
                                "reconnectingâ€¦\n\n"
                            )
                        except Exception:
                            pass
                    ...
                    # semantically identical to httpx connection drops â€”
                    # the upstream stream died â€” and should be retried with
                    ...
                    if _is_timeout or _is_conn_err or _is_sse_conn_err or _is_stream_parse_err:
                        ...
                        # mid-flight retries â€” subagent lines get the
                        ...
                        agent._buffer_status(
                            "âŒ Provider returned malformed streaming data after "
                            f"{_max_stream_retries + 1} attempts. "
                            "The provider may be experiencing issues â€” "
                            "try again in a moment."
                            if _is_stream_parse_err else
                            "âŒ Connection to provider failed after "
                            f"{_max_stream_retries + 1} attempts. "
                            "The provider may be experiencing issues â€” "
                            "try again in a moment."
                        )
                        ...
                        # Permanent for the session â€” flip to non-streaming
                        ...
                        if (
                            ...
                            # adapter â€” bedrock_adapter triggers a lazy boto3
                        ...
                        if _is_stream_unsupported or _is_bedrock_stream_denied:
                            agent._disable_streaming = True
                            agent._safe_print(
                                "\nâš   AWS IAM denied bedrock:InvokeModelWithResponseStream. "
                                "Switching to non-streaming.\n"
                                "   Grant that action to restore streaming output.\n"
                                if _is_bedrock_stream_denied else
                                "\nâš   Streaming is not supported for this "
                    ...
                    # backoff, and â€” for "stream not supported" â€” will switch
    ...
    if _cfg_stale is not None:
        ...
        _stream_stale_timeout_base = env_float("ZED_STREAM_STALE_TIMEOUT", 180.0)
    # Local providers (Ollama, oMLX, llama-cpp) can take 300+ seconds
    # for prefill on large contexts.  Disable the stale detector unless
    # the user explicitly set ZED_STREAM_STALE_TIMEOUT.
    if _stream_stale_timeout_base == 180.0 and agent.base_url and is_local_endpoint(agent.base_url):
        _stream_stale_timeout = float("inf")
        logger.debug("Local provider detected (%s) â€” stale stream timeout disabled", agent.base_url)
    else:
    ...
    while t.is_alive():
        ...
        # and first chunk can exceed the gateway timeout â€” especially
        ...
        if _stale_elapsed > _stream_stale_timeout:
            _est_ctx = estimate_request_context_tokens(api_kwargs)
            logger.warning(
                "Stream stale for %.0fs (threshold %.0fs) â€” no chunks received. "
                "model=%s context=~%s tokens. Killing connection.",
                _stale_elapsed, _stream_stale_timeout,
                api_kwargs.get("model", "unknown"), f"{_est_ctx:,}",
            )
            agent._buffer_status(
                f"âš ï¸ No response from provider for {int(_stale_elapsed)}s "
            ...
            try:
                ...
                pass
            # Rebuild the primary client too â€” its connection pool
    ...
    if result["error"] is not None:
        if deltas_were_sent["yes"]:
            ...
            if _partial_names:
                ...
                _warn = (
                    f"\n\nâš  Stream stalled mid tool-call "
...
# â”€â”€ Provider fallback â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
