...
def _hermes_meta(fm: dict[str, Any]) -> dict[str, Any]:
    """``metadata.zed`` as a dict, tolerant of the string-valued frontmatter
...
def _load_usage() -> dict[str, dict[str, Any]]:
    try:
        ...
        path = get_zed_home() / "skills" / ".usage.json"
        try:
...
def _skill_roots() -> list[tuple[str, Path]]:
    ...
    home_skills = get_zed_home() / "skills"
