"""Anthropic Messages API adapter for Zed Agent.

Translates between Zed's internal OpenAI-style message format and
Anthropic's Messages API. Follows the same pattern as the codex_responses
adapter â€” all provider-specific logic is isolated here.

Auth supports:
  - Regular API keys (sk-ant-api*) â†’ x-api-key header
  - OAuth setup-tokens (sk-ant-oat*) â†’ Bearer auth + beta header
  - Claude Code credentials (~/.claude.json or ~/.claude/.credentials.json) â†’ Bearer auth
"""
...
# NOTE: `import anthropic` is deliberately NOT at module top â€” the SDK pulls
...
_anthropic_sdk: Any = ...  # sentinel â€” None means "tried and missing"
...
def _get_anthropic_sdk():
    ...
    if _anthropic_sdk is ...:
        try:
            ...
            # FeatureUnavailable â€” fall through to ImportError handling below
            pass
        try:
...
# Zed effort â†’ Anthropic adaptive-thinking effort (output_config.effort).
# Anthropic exposes 5 levels on 4.7+: low, medium, high, xhigh, max.
# Opus/Sonnet 4.6 only expose 4 levels: low, medium, high, max â€” no xhigh.
...
# â”€â”€ Anthropic thinking-mode classification â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# claude-fable-5) follow the same modern contract â€” but they share no common
# version substring, so an allowlist of version numbers ("4.6", "4.7", â€¦) goes
...
#
# Non-Claude Anthropic-Messages models (minimax, qwen3, GLM, â€¦) are NOT Claude,
# so they fall through to the legacy path automatically â€” exactly what those
...
# â”€â”€ Max output token limits per Anthropic model â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Source: Anthropic docs + Cline model catalog.  Anthropic's API requires
# max_tokens as a mandatory field.  Previously we hardcoded 16384, which
# starves thinking-enabled models (thinking tokens count toward the limit).
_ANTHROPIC_OUTPUT_LIMITS = {
    # Mythos-class named models (claude-fable-5, â€¦) â€” 1M context, reasoning
    ...
    # DashScope enforces max_tokens âˆˆ [1, 65536]
    "qwen3":               65_536,
}
...
def _resolve_positive_anthropic_max_tokens(value) -> Optional[int]:
    ...
    """
    # Booleans are a subclass of int â€” exclude explicitly so ``True`` doesn't
...
def _resolve_anthropic_messages_max_tokens(
    ...
    Separately, callers apply a context-window clamp â€” this resolver does
...
def _supports_adaptive_thinking(model: str) -> bool:
    ...
    models (minimax, qwen3, â€¦) return False so they keep the manual path.
    """
...
def _supports_xhigh_effort(model: str) -> bool:
    ...
    and reject xhigh with an HTTP 400. Callers should downgrade xhighâ†’max
...
# requests. As of Opus 4.7 (2026-04-16), these are GA on Claude 4.6+ â€” the
...
# Fast mode beta â€” enables the ``speed: "fast"`` request parameter for
...
# Claude Code identity â€” required for OAuth requests to be routed correctly.
# Without these, Anthropic's infrastructure intermittently 500s OAuth traffic.
# The version must stay reasonably current â€” Anthropic rejects OAuth requests
...
def _is_oauth_token(key: str) -> bool:
    ...
    - ``sk-ant-`` prefix (but NOT ``sk-ant-api``) â†’ setup tokens, managed keys
    - ``eyJ`` prefix â†’ JWTs from the Anthropic OAuth flow
    - ``cc-`` prefix â†’ Claude Code OAuth access tokens (from CLAUDE_CODE_OAUTH_TOKEN)
    ...
    if not key:
        return False
    # Regular Anthropic Console API keys â€” x-api-key auth, never OAuth
...
def _is_third_party_anthropic_endpoint(base_url: str | None) -> bool:
    ...
    if "anthropic.com" in normalized:
        return False  # Direct Anthropic API â€” OAuth applies
...
def _model_name_is_kimi_family(model: str | None) -> bool:
    ...
    if not m:
        return False
    # Strip vendor prefix (e.g. ``moonshotai/kimi-k2.5`` â†’ ``kimi-k2.5``)
    if "/" in m:
        m = m.rsplit("/", 1)[-1]
...
def _is_kimi_family_endpoint(base_url: str | None, model: str | None = None) -> bool:
    ...
    Broader than ``_is_kimi_coding_endpoint`` â€” matches:
    ...
    - Custom or proxied endpoints whose *model* name is in the Kimi / Moonshot
      family (``kimi-*``, ``moonshot-*``, ``k1.*``, ``k2.*``, â€¦).  Users with
      ``api_mode: anthropic_messages`` on a private gateway fronting Kimi
      fall into this branch â€” the upstream still enforces Kimi's thinking
...
def _is_deepseek_anthropic_endpoint(base_url: str | None) -> bool:
    ...
    from prior assistant turns to round-trip on subsequent requests â€” the
...
def _common_betas_for_base_url(
    ...
    *,
    ...
    that include Anthropic's ``fine-grained-tool-streaming`` beta â€” every
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    ``auth_token`` is set â€” but the hook overrides it per-request so
    ...
    # Strip any trailing /v1 â€” the Anthropic SDK appends /v1/messages.
...
def build_anthropic_client(
    ...
    * a static ``str`` â€” the historical contract for all key-based and
      OAuth flows.
    * a ``Callable[[], str]`` â€” an Entra ID bearer token provider from
    ...
    # Callable api_key â†’ Entra ID bearer provider path. Delegated to a
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # own API keys with x-api-key auth. Skip OAuth detection â€” their keys
        ...
        # OAuth access token / setup-token â†’ Bearer auth + Claude Code identity.
        ...
        # Regular API key â†’ x-api-key header + common betas
        kwargs["api_key"] = api_key
        if common_betas:
            kwargs["default_headers"] = {"anthropic-beta": ",".join(common_betas)}
...
def build_anthropic_bedrock_client(region: str):
    ...
    thinking, fast mode â€” features not available via the Converse API.
    ...
    unlocks the 1M context window for Opus 4.6/4.7 on Bedrock â€” without
...
def read_claude_code_credentials() -> Optional[Dict[str, Any]]:
    ...
    Checks two sources in order:
      1. macOS Keychain (Darwin only) â€” "Claude Code-credentials" entry
...
def is_claude_code_token_valid(creds: Dict[str, Any]) -> bool:
    ...
    if not expires_at:
        # No expiry set (managed keys) â€” valid if token is present
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    if not refresh_token:
        logger.debug("No refresh token available â€” cannot refresh")
        return None
...
def _resolve_claude_code_token_from_credentials(creds: Optional[Dict[str, Any]] = None) -> Optional[str]:
    ...
    if creds:
        logger.debug("Claude Code credentials expired â€” attempting refresh")
        refreshed = _refresh_oauth_token(creds)
        if refreshed:
            return refreshed
        logger.debug("Token refresh failed â€” re-run 'claude setup-token' to reauthenticate")
    return None
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Zed historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Zed)
      2. CLAUDE_CODE_OAUTH_TOKEN env var
      3. Claude Code credentials (~/.claude.json or ~/.claude/.credentials.json)
         â€” with automatic refresh if expired and a refresh token is available
    ...
    # 1. Zed-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Zed configs.
...
def run_oauth_setup_token() -> Optional[str]:
    ...
    # Run interactively â€” stdin/stdout/stderr inherited so the user can
...
# â”€â”€ Zed-native PKCE OAuth flow â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.zed/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_ZED_OAUTH_FILE = get_zed_home() / ".anthropic_oauth.json"
...
def run_zed_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Zed-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Zed with your Claude Pro/Max subscription.")
    print()
    print("â•­â”€ Claude Pro/Max Authorization â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â•®")
    print("â”‚                                                   â”‚")
    print("â”‚  Open this link in your browser:                  â”‚")
    ...
    try:
        from zed_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
        _can_open_gui = lambda: True  # noqa: E731 â€” degrade to prior behavior
    ...
    # Validate state to prevent CSRF (RFC 6749 Â§10.12)
    if received_state != oauth_state:
        logger.warning("OAuth state mismatch â€” possible CSRF, aborting")
        return None
...
def read_zed_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Zed-managed OAuth credentials from ~/.zed/.anthropic_oauth.json."""
    if _ZED_OAUTH_FILE.exists():
        try:
            data = json.loads(_ZED_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Zed OAuth credentials: %s", e)
    return None
...
def normalize_model_name(model: str, preserve_dots: bool = False) -> str:
    ...
    - Converts dots to hyphens in version numbers (OpenRouter uses dots,
      Anthropic uses hyphens: claude-opus-4.6 â†’ claude-opus-4-6), unless
...
def _convert_content_part_to_anthropic(part: Any) -> Optional[Dict[str, Any]]:
    ...
    if ptype == "input_text":
        ...
        # A stored Anthropic text block. Rebuild from whitelisted fields only â€”
...
def _content_parts_to_anthropic_blocks(parts: Any) -> List[Dict[str, Any]]:
    """Convert OpenAI-style tool-message content parts â†’ Anthropic tool_result inner blocks.
...
def _sanitize_replay_block(b: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    *input* schema forbids ("Extra inputs are not permitted"): text blocks get
    ...
    so these leak back as input on the next turn â†’ HTTP 400.
    ...
    # Unknown/unsupported block type on the input path â€” drop rather than risk
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # SDK fields (parsed_output, caller, citations=None, â€¦) that the Messages
    # INPUT schema forbids â€” replaying them verbatim caused HTTP 400 "Extra
    ...
    # can validate the message history.  See zed-agent#13848.
    #
    # Accept empty string "" â€” _copy_reasoning_content_for_api()
    ...
    # signed thinking blocks â€” adding another unsigned one from
...
def _convert_tool_message_to_result(
    ...
    Mutates ``result`` in place â€” either appends a new user message or extends
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for m in result:
        if m["role"] == "assistant" and isinstance(m["content"], list):
            ...
            # replaying it verbatim.  See zed-agent: extended-thinking + parallel
            # tool batch interrupted mid-flight â†’ non-retryable 400 crash-loop.
    ...
    for m in result:
        if fixed and fixed[-1]["role"] == m["role"]:
            if m["role"] == "user":
                ...
                # Consecutive assistant messages â€” merge text content.
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See zed-agent#13848 (Kimi) and
    zed-agent#16748 (DeepSeek).
    ...
    for idx, m in enumerate(result):
        ...
        if _preserve_unsigned_thinking:
            ...
            for b in m["content"]:
                ...
                if b.get("signature") or b.get("data"):
                    # Signed (or redacted-with-data) â€” upstream can't validate, strip.
                    continue
                new_content.append(b)
            ...
            # rejects the turn either way â€” replaying the signed block 400s with
            ...
            for b in m["content"]:
                ...
                if b.get("type") == "redacted_thinking":
                    # Redacted blocks use 'data' for the signature payload â€”
        ...
        # blocks â€” cache markers interfere with signature validation.
        for b in m["content"]:
            if isinstance(b, dict) and b.get("type") in _THINKING_TYPES:
                b.pop("cache_control", None)

        # Drop the internal bookkeeping flag â€” it must never reach the API payload.
...
def convert_messages_to_anthropic(
    ...
    Anthropic-proprietary â€” third-party endpoints cannot validate them and will
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    assistant tool-call messages â€” Kimi requires the field to exist, even
    if empty.
    """
    system = None
...
def build_anthropic_kwargs(
    ...
    Naming note â€” two distinct concepts, easily confused:
      ...
      context_length = TOTAL context window (input tokens + output tokens).
                       The API enforces: input_tokens + max_tokens â‰¤ context_length.
    ...
    clamped to context_length âˆ’ 1.  This only kicks in for unusually small
    ...
    NOTE: this clamping does not account for prompt size â€” if the prompt is
    ...
    # effective_max_tokens = output cap for this call (â‰  total context window)
    ...
    # â”€â”€ OAuth: Claude Code identity â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if is_oauth:
        ...
        # 2. Sanitize system prompt â€” replace product name references
        #    to avoid Anthropic's server-side content filters.
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Zed Agent", "Claude Code")
                text = text.replace("Zed agent", "Claude Code")
                text = text.replace("zed-agent", "claude-code")
                text = text.replace("Zed Team", "Anthropic")
                block["text"] = text
        ...
        #      a) bare Zed-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    if anthropic_tools:
        ...
        if tool_choice == "auto" or tool_choice is None:
            ...
            # Anthropic has no tool_choice "none" â€” omit tools entirely to prevent use
    ...
    # not adaptive).  Haiku does NOT support extended thinking â€” skip entirely.
    #
    ...
    # on third-party endpoints â€” so the request fails with HTTP 400
    # "thinking is enabled but reasoning_content is missing in assistant
    # tool call message at index N".  Kimi's reasoning is driven server-side
    # on the /coding route, so skip Anthropic's thinking parameter entirely
    # for that host.  (Kimi on chat_completions enables thinking via
    # extra_body in the ChatCompletionsTransport â€” see #13503.)
    #
    # On 4.7+ the `thinking.display` field defaults to "omitted", which
    # silently hides reasoning text that Zed surfaces in its CLI. We
    # request "summarized" so the reasoning blocks stay populated â€” matching
    ...
    if reasoning_config and isinstance(reasoning_config, dict) and not _is_kimi_coding:
        if reasoning_config.get("enabled") is not False and "haiku" not in model.lower():
            ...
            if _supports_adaptive_thinking(model):
                ...
                # Downgrade xhighâ†’max on models that don't list xhigh as a
    ...
    # â”€â”€ Strip sampling params on 4.7+ â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # Opus 4.7 rejects any non-default temperature/top_p/top_k with a 400.
    # Callers (auxiliary_client, etc.) may set these for older models;
    # drop them here as a safety net so upstream 4.6 â†’ 4.7 migrations
    # don't require coordinated edits everywhere.
    if _forbids_sampling_params(model):
        for _sampling_key in ("temperature", "top_p", "top_k"):
            kwargs.pop(_sampling_key, None)

    # â”€â”€ Fast mode (Opus 4.6 only) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # Adds extra_body.speed="fast" + the fast-mode beta header for ~2.5x
    # output speed. Per Anthropic docs, fast mode is only supported on
    # Opus 4.6 â€” Opus 4.7 and other models 400 on the speed parameter.
    # Only for native Anthropic endpoints â€” third-party providers would
...
def sanitize_anthropic_kwargs(api_kwargs: Any, *, log_prefix: str = "") -> Any:
    ...
    if leaked:
        ...
        logger.warning(
            ...
            "call (api_mode flip race â€” see #31673). The call will proceed; "
