...
In ``auto`` mode:
  - If the user has explicitly configured ``auxiliary.vision.provider``
    ...
    regardless of the main model â€” they've opted in to a specific vision
...
# Image extensions used by extract_image_refs(). Kept tight on purpose â€” we
...
def extract_image_refs(text: str) -> Tuple[List[str], List[str]]:
    ...
    for match in _LOCAL_IMAGE_PATH_RE.finditer(text):
        ...
        try:
            ...
            # ENAMETOOLONG / EINVAL on pathological inputs â€” skip rather than crash.
            continue
...
# a user writing ``supports_vision: "false"`` (quoted â€” a common YAML mistake)
...
def _supports_vision_override(
    ...
    # (zed_cli/runtime_provider.py:_resolve_named_custom_runtime), so the
...
def _lookup_supports_vision(
    ...
    try:
        ...
        logger.debug("image_routing: caps lookup failed for %s:%s â€” %s", provider, model, exc)
        return None
    if caps is None:
        return None
...
def _file_to_data_url(path: Path) -> Optional[str]:
    ...
    try:
        ...
        logger.warning("image_routing: failed to read %s â€” %s", path, exc)
        return None
...
def build_native_content_parts(
    ...
    # No images successfully attached â€” fall back to plain text-only behaviour.
    parts = []
    if text:
