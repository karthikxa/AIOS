"""The agent conversation loop â€” extracted from ``run_agent.AIAgent``.
...
from zed_constants import PARTIAL_STREAM_STUB_ID
from zed_logging import set_session_context
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Zed tool use: "
    ...
    return (
        ...
        f"context, but Zed needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Zed config, set `model.ollama_num_ctx: 65536` "
...
def _print_nous_entitlement_guidance(agent, capability: str) -> bool:
    ...
    for line in message.splitlines():
        agent._vprint(f"{agent.log_prefix}   ðŸ’¡ {line}", force=True)
    return True
...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    if conversation_history and agent._session_db:
        try:
            ...
            logger.warning(
                ...
                "(session=%s): %s. Falling back to fresh build â€” prefix "
                "cache will miss for this turn.",
                agent.session_id, exc,
            )

    if stored_prompt and _stored_prompt_matches_runtime(agent, stored_prompt):
        # Continuing session â€” reuse the exact system prompt from the
    ...
    if conversation_history and stored_state in ("null", "empty"):
        ...
        # string â€” either way every turn now rebuilds and the prefix
    ...
    # prompt) â€” build from scratch.
    agent._cached_system_prompt = agent._build_system_prompt(system_message)

    # Plugin hook: on_session_start â€” fired once when a brand-new
    ...
    # Cold-start credits seed (L3) â€” fallback for the first-turn path. The TUI/
    ...
    # gateway path (fresh AIAgent per turn â†’ reads from this row every
    # subsequent turn).
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `zed fallback add`."
)
...
def _content_policy_blocked_result(
    ...
    *,
    ...
    exception-path handler return the identical shape â€” a failed, non-completed
    turn carrying the user-facing message and a ``content_policy_blocked:``
    prefixed error â€” so they funnel through this one builder.
    """
    return {
...
def run_conversation(
    ...
    """
    # â”€â”€ Per-turn setup (the prologue) â”€â”€
    # All once-per-turn setup â€” stdio guarding, retry-counter resets, user
    # message sanitization, todo/nudge hydration, system-prompt restore-or-
    # build, crash-resilience persistence, preflight compression, the
    # ``pre_llm_call`` plugin hook, and external-memory prefetch â€” lives in
    ...
    # all run inside Codex). Default Zed path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        if agent._budget_grace_call:
            ...
            if not agent.quiet_mode:
                agent._safe_print(f"\nâš ï¸  Iteration budget exhausted ({agent.iteration_budget.used}/{agent.iteration_budget.max_total} iterations used)")
            break
        ...
        # â”€â”€ Pre-API-call /steer drain â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        # If a /steer arrived during the previous API call (while the model
        # was thinking), drain it now â€” before we build api_messages â€” so
        ...
        # tool batch â€” injecting into a user message would break role
            ...
            for _si in range(len(messages) - 1, -1, -1):
                ...
                if isinstance(_sm, dict) and _sm.get("role") == "tool":
                    ...
                    if isinstance(existing, str):
                        ...
                        # Multimodal content blocks â€” append text block
                        try:
                    ...
                    break
            if not _injected:
                # No tool message to inject into â€” put it back so
        ...
        # ``tool â†’ user`` or ``user â†’ user`` tail (e.g. after empty-
        ...
        for idx, msg in enumerate(messages):
            ...
            # API-call-time only â€” the original message in `messages` is
        ...
        # This is intentional â€” system prompt modifications break the prompt
        # cache prefix.  The system prompt is reserved for Zed internals.
        #
        # Zed invariant: the system prompt is built ONCE per session
        ...
        # results before sending to the API.  Runs unconditionally â€” not
        # gated on context_compressor â€” so orphans from session loading or
        ...
        # a thinking-only turn. Runs on the per-call copy only â€” the
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("âŒ Ollama runtime context is too small for Zed tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        if not agent.quiet_mode:
            agent._vprint(f"\n{agent.log_prefix}ðŸ”„ Making API call #{api_call_count}/{agent.max_iterations}...")
            agent._vprint(f"{agent.log_prefix}   ðŸ“Š Request size: {len(api_messages)} messages, ~{approx_tokens:,} tokens (~{total_chars:,} chars)")
            agent._vprint(f"{agent.log_prefix}   ðŸ”§ Available tools: {len(agent.tools) if agent.tools else 0}")
        else:
        ...
        while retry_count < max_retries:
                ...
                try:
                    ...
                    if _nous_remaining is not None and _nous_remaining > 0:
                        ...
                        agent._buffer_vprint(
                            f"â³ {_nous_msg} Trying fallback..."
                        )
                        agent._buffer_status(f"â³ {_nous_msg}")
                        if agent._try_activate_fallback():
                            retry_count = 0
                            compression_attempts = 0
                            _retry.primary_recovery_attempted = False
                            continue
                        # No fallback available â€” surface buffered context
            ...
            try:
                ...
                try:
                    from zed_cli.middleware import apply_llm_request_middleware
                ...
                try:
                    ...
                    if has_hook("pre_api_request"):
                        ...
                        # sanitised â€” secrets are not expected here because
                ...
                if env_var_enabled("ZED_DUMP_REQUESTS"):
                    agent._dump_api_request_debug(api_kwargs, reason="preflight")

                # Always prefer the streaming path â€” even without stream
                ...
                # attempt â€” switch to non-streaming for the rest of this
                # session instead of re-failing every retry.
                if getattr(agent, "_disable_streaming", False):
                    _use_streaming = False
                # CopilotACPClient communicates via subprocess stdio and
                # returns a plain SimpleNamespace â€” not an iterable
                ...
                from zed_cli.middleware import run_llm_execution_middleware
                ...
                if not agent.quiet_mode:
                    agent._vprint(f"{agent.log_prefix}â±ï¸  API call completed in {api_duration:.2f}s")
                ...
                if response_invalid:
                    ...
                    # Stop spinner silently â€” retry status is now buffered
                    ...
                    # Invalid response â€” could be rate limiting, provider timeout,
                    ...
                    if agent._fallback_index < len(agent._fallback_chain):
                        agent._buffer_status("âš ï¸ Empty/malformed response â€” switching to fallback...")
                    if agent._try_activate_fallback():
                        retry_count = 0
                        compression_attempts = 0
                        _retry.primary_recovery_attempted = False
                    ...
                    if _resp_error_code == 524:
                        ...
                        _failure_hint = f"fast response ({api_duration:.1f}s) â€” likely rate limited"
                    elif api_duration > 60:
                        _failure_hint = f"slow response ({api_duration:.0f}s) â€” likely upstream timeout"
                    else:
                        _failure_hint = f"response time {api_duration:.1f}s"

                    agent._buffer_vprint(f"âš ï¸  Invalid API response (attempt {retry_count}/{max_retries}): {', '.join(error_details)}")
                    agent._buffer_vprint(f"   ðŸ¢ Provider: {provider_name}")
                    cleaned_provider_error = agent._clean_error_message(error_msg)
                    agent._buffer_vprint(f"   ðŸ“ Provider message: {cleaned_provider_error}")
                    agent._buffer_vprint(f"   â±ï¸  {_failure_hint}")
                    
                    if retry_count >= max_retries:
                        # Try fallback before giving up
                        if agent._has_pending_fallback():
                            agent._buffer_status(f"âš ï¸ Max retries ({max_retries}) for invalid responses â€” trying fallback...")
                        if agent._try_activate_fallback():
                            retry_count = 0
                            compression_attempts = 0
                            _retry.primary_recovery_attempted = False
                            continue
                        # Terminal â€” flush buffered retry trace so user sees what happened.
                        agent._flush_status_buffer()
                        agent._emit_status(f"âŒ Max retries ({max_retries}) exceeded for invalid responses. Giving up.")
                    ...
                    # Backoff before retry â€” jittered exponential: 5s base, 120s cap
                    wait_time = jittered_backoff(retry_count, base_delay=5.0, max_delay=120.0)
                    agent._buffer_vprint(f"â³ Retrying in {wait_time:.1f}s ({_failure_hint})...")
                    ...
                    while time.time() < sleep_end:
                        if agent._interrupt_requested:
                            agent._vprint(f"{agent.log_prefix}âš¡ Interrupt detected during retry wait, aborting.", force=True)
                            agent._persist_session(messages, conversation_history)
                        ...
                        if _backoff_touch_counter % 150 == 0:  # 150 Ã— 0.2s = 30s
                ...
                if agent.api_mode == "codex_responses":
                    ...
                    # Bedrock response already normalized at dispatch â€” use transport
                    ...
                    if agent._should_treat_stop_as_truncated(
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}âš ï¸  Treating suspicious Ollama/GLM stop response as truncated",
                            force=True,
                        )
                        finish_reason = "length"

                # â”€â”€ Content-policy refusal (HTTP 200) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                # The model â€” or the provider's safety system â€” returned a
                # *successful* response whose stop/finish reason is a refusal:
                # Anthropic ``stop_reason="refusal"`` â†’ ``content_filter``;
                ...
                # retries" â€” burning paid attempts reproducing a deterministic
                ...
                if finish_reason == "content_filter":
                    ...
                    # Deterministic for the unchanged prompt â€” never retry.
                    # Try a configured fallback once (a different model may not
                    # refuse); otherwise surface the refusal terminally.
                    if agent._has_pending_fallback():
                        agent._buffer_status(
                            "âš ï¸ Model declined to respond (safety refusal) â€” trying fallback..."
                        )
                    if agent._try_activate_fallback():
                        retry_count = 0
                        compression_attempts = 0
                    ...
                    agent._emit_status(
                        "âš ï¸ The model declined to respond to this request (safety refusal)."
                    )
                    ...
                    _refusal_response = (
                        "âš ï¸  The model declined to respond to this request "
                        "(safety refusal â€” not a Zed/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if finish_reason == "length":
                    if getattr(response, "id", "") == PARTIAL_STREAM_STUB_ID:
                        agent._vprint(
                            f"{agent.log_prefix}âš ï¸  Stream interrupted by network error "
                            f"(finish_reason='length' on partial-stream-stub)",
                            force=True,
                        )
                    else:
                        agent._vprint(
                            f"{agent.log_prefix}âš ï¸  Response truncated "
                    ...
                    # â”€â”€ Detect thinking-budget exhaustion â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                    ...
                    # string for unrelated reasons â€” treat those as normal
                    ...
                    if _thinking_exhausted:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}ðŸ’­ Reasoning exhausted the output token budget â€” "
                        ...
                        _exhaust_response = (
                            "âš ï¸ **Thinking Budget Exhausted**\n\n"
                            "The model used all its output tokens on reasoning "
                            "and had none left for the actual response.\n\n"
                            "To fix this:\n"
                            "â†’ Lower reasoning effort: `/thinkon low` or `/thinkon minimal`\n"
                            "â†’ Or switch to a larger/non-reasoning model with `/model`"
                        )
                        agent._cleanup_task_resources(effective_task_id)
                        agent._persist_session(messages, conversation_history)
                        return {
                        ...
                        if assistant_message is not None and not _trunc_has_tool_calls:
                            ...
                            if length_continue_retries < 3:
                                ...
                                if _is_partial_stream_stub and _dropped_tools:
                                    ...
                                    agent._vprint(
                                        f"{agent.log_prefix}â†» Stream interrupted mid "
                                        f"tool-call ({_tool_list}) â€” requesting "
                                        f"chunked retry "
                                        f"({length_continue_retries}/3)..."
                                    )
                                elif _is_partial_stream_stub:
                                    agent._vprint(
                                        f"{agent.log_prefix}â†» Stream interrupted â€” "
                                        f"requesting continuation "
                                        f"({length_continue_retries}/3)..."
                                    )
                                else:
                                    agent._vprint(
                                        f"{agent.log_prefix}â†» Requesting continuation "
                                        f"({length_continue_retries}/3)..."
                                    )
                        ...
                        if assistant_message is not None and _trunc_has_tool_calls:
                            ...
                            if truncated_tool_call_retries < 3:
                                ...
                                if _is_stub_stall:
                                    ...
                                    # cap â€” say so instead of "max output tokens".
                                    agent._buffer_vprint(
                                        f"âš ï¸  Stream interrupted mid tool-call â€” "
                                        f"retrying ({truncated_tool_call_retries}/3)..."
                                    )
                                else:
                                    agent._buffer_vprint(
                                        f"âš ï¸  Truncated tool call detected â€” "
                                        f"retrying API call "
                            ...
                            if _is_stub_stall:
                                agent._vprint(
                                    f"{agent.log_prefix}âš ï¸  Stream kept dropping mid tool-call after 3 retries â€” the action was not executed.",
                                    force=True,
                                )
                            else:
                                agent._vprint(
                                    f"{agent.log_prefix}âš ï¸  Truncated tool call response detected again â€” refusing to execute incomplete tool arguments.",
                                    force=True,
                                )
                            agent._cleanup_task_resources(effective_task_id)
                            agent._persist_session(messages, conversation_history)
                    ...
                    if len(messages) > 1:
                        agent._vprint(f"{agent.log_prefix}   âª Rolling back to last complete assistant turn")
                        ...
                        agent._flush_status_buffer()
                        agent._vprint(f"{agent.log_prefix}âŒ First response truncated - cannot recover", force=True)
                        agent._persist_session(messages, conversation_history)
                        return {
                            "final_response": None,
                            "messages": messages,
                ...
                if hasattr(response, 'usage') and response.usage:
                    ...
                    if getattr(agent.context_compressor, "_context_probed", False):
                        ...
                        if getattr(agent.context_compressor, "_context_probe_persistable", False):
                            ...
                            agent._safe_print(f"{agent.log_prefix}ðŸ’¾ Cached context length: {ctx:,} tokens for {agent.model}")
                    ...
                    if agent._session_db and agent.session_id:
                        try:
                            ...
                            # visible in agent.log â€” silent loss here is
                    ...
                    # them â€” not just those where we inject cache_control
                    ...
                    if (cached or written) and not agent.quiet_mode:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}   ðŸ’¾ Cache: "
                            f"{cached:,}/{prompt:,} tokens "
                            f"({hit_pct:.0f}% hit, {written:,} written)"
                        )
                
                _retry.has_retried_429 = False  # Reset on success
                # Note: don't clear the retry buffer here â€” an "API call
                ...
                agent._vprint(f"{agent.log_prefix}âš¡ Interrupted during API call.", force=True)
                agent._persist_session(messages, conversation_history)
                interrupted = True
                ...
                # Stop spinner silently â€” retry status is buffered and
                ...
                #      (Google Docs, rich-text editors) â€” sanitize and retry.
                #   2. ASCII codec on systems with LANG=C or non-UTF-8 locale
                #      (e.g. Chromebooks) â€” any non-ASCII character fails.
                ...
                if isinstance(api_error, UnicodeEncodeError) and getattr(agent, '_unicode_sanitization_passes', 0) < 2:
                    ...
                    # Detect surrogate errors â€” utf-8 codec refusing to
                    ...
                    # from `reasoning` â€” fields the canonical list doesn't
                    ...
                    # found anything â€” _force_ascii_payload / the extended
                    ...
                    if _surrogates_found or _is_surrogate_error:
                        ...
                        if _surrogates_found:
                            agent._buffer_vprint(
                                f"âš ï¸  Stripped invalid surrogate characters from messages. Retrying..."
                            )
                        else:
                            agent._buffer_vprint(
                                f"âš ï¸  Surrogate encoding error â€” retrying after full-payload sanitization..."
                            )
                        continue
                    if _is_ascii_codec:
                        ...
                        # Sanitize the API key â€” non-ASCII characters in
                        # credentials (e.g. Ê‹ instead of v from a bad
                        ...
                        # Entra ID bearer providers are callables â€” their
                        ...
                        if _raw_key and isinstance(_raw_key, str):
                            ...
                            if _clean_key != _raw_key:
                                ...
                                # Also update the live client â€” it holds its
                                ...
                                agent._vprint(
                                    f"{agent.log_prefix}âš ï¸  API key contained non-ASCII characters "
                                    f"(bad copy-paste?) â€” stripped them. If auth fails, "
                                    f"re-copy the key from your provider's dashboard.",
                                    force=True,
                                )

                        # Always retry on ASCII codec detection â€”
                        ...
                        if _any_sanitized:
                            agent._vprint(
                                f"{agent.log_prefix}âš ï¸  System encoding is ASCII â€” stripped non-ASCII characters from request payload. Retrying...",
                                force=True,
                            )
                        else:
                            agent._vprint(
                                f"{agent.log_prefix}âš ï¸  System encoding is ASCII â€” enabling full-payload sanitization for retry...",
                                force=True,
                            )
                        continue

                # â”€â”€ Image-rejection recovery â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                #
                # Detection is best-effort English phrase matching â€” a
                ...
                _IMAGE_REJECTION_PHRASES = (
                    ...
                    # purpose â€” keyed on the field-path apostrophe so
                ...
                # said no to images" â€” those are transient and must
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}âš ï¸  Server rejected image content â€” "
                ...
                # â”€â”€ Classify the error for structured recovery decisions â”€â”€
                ...
                if (
                    ...
                    if agent._try_shrink_image_parts_in_messages(
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}ðŸ“ Image(s) exceeded provider size limit â€” "
                ...
                if (
                    ...
                    if agent._try_strip_image_parts_from_tool_messages(api_messages):
                        agent._vprint(
                            f"{agent.log_prefix}ðŸ“ Provider rejected list-type tool content â€” "
                ...
                # subscriptions never hit this branch â€” they accept the
                ...
                if (
                    ...
                    if not getattr(agent, "_oauth_1m_beta_disabled", False):
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}ðŸ”• OAuth subscription doesn't support "
                            f"the 1M-context beta â€” disabled for this session and retrying...",
                            force=True,
                        )
                        continue
                ...
                if (
                    ...
                    if agent._try_refresh_codex_client_credentials(force=True):
                        ...
                        agent._buffer_vprint(f"ðŸ” {_label} auth refreshed after 401. Retrying request...")
                        continue
                if (
                    ...
                    if agent._try_refresh_nous_client_credentials(force=True):
                        print(f"{agent.log_prefix}ðŸ” Nous agent key refreshed after 401. Retrying request...")
                        continue
                    # Credential refresh didn't help â€” show diagnostic info.
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix}ðŸ” Nous 401 â€” Portal authentication failed.")
                    if _body_text:
                ...
                if (
                    ...
                    if agent._try_refresh_copilot_client_credentials():
                        agent._buffer_vprint(f"ðŸ” Copilot credentials refreshed after 401. Retrying request...")
                        continue
                if (
                    ...
                    if agent._try_refresh_anthropic_client_credentials():
                        print(f"{agent.log_prefix}ðŸ” Anthropic credentials refreshed after 401. Retrying request...")
                        continue
                    # Credential refresh didn't help â€” show diagnostic info
                    key = agent._anthropic_api_key
                    print(f"{agent.log_prefix}ðŸ” Anthropic 401 â€” authentication failed.")
                    if is_token_provider(key):
                        # Azure Foundry Entra ID â€” the bearer token is
                        ...
                        print(f"{agent.log_prefix}   Run `zed doctor` for credential-chain diagnostics, or")
                    ...
                    from zed_constants import display_zed_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     â€¢ Check ANTHROPIC_TOKEN in {_dhh}/.env for Zed-managed OAuth/setup tokens")
                    print(f"{agent.log_prefix}     â€¢ Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                    print(f"{agent.log_prefix}     â€¢ For API keys: verify at https://platform.claude.com/settings/keys")
                    print(f"{agent.log_prefix}     â€¢ For Claude Code: run 'claude /login' to refresh, then retry")
                    print(f"{agent.log_prefix}     â€¢ Legacy cleanup: zed config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     â€¢ Clear stale keys: zed config set ANTHROPIC_API_KEY \"\"")
                ...
                if (
                    ...
                    for _m in api_messages:
                        if isinstance(_m, dict) and "reasoning_details" in _m:
                            ...
                            _api_stripped += 1
                    agent._vprint(
                        f"{agent.log_prefix}âš ï¸  Thinking block signature invalid, "
                ...
                # â”€â”€ Invalid encrypted reasoning replay recovery â”€â”€â”€â”€â”€â”€â”€
                ...
                # One-shot â€” if a second 400 fires we fall through to the
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}âš ï¸  Encrypted reasoning replay was rejected by the provider â€” "
                ...
                # â”€â”€ llama.cpp grammar-parse recovery â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                if (
                    ...
                    except Exception as _strip_exc:  # pragma: no cover â€” defensive
                        logger.warning(
                    ...
                    if _stripped:
                        agent._vprint(
                            f"{agent.log_prefix}âš ï¸  llama.cpp rejected tool schema grammar â€” "
                        ...
                        continue
                    # No keywords found to strip â€” fall through to normal
                    # retry path rather than loop forever on the same error.
                    logger.warning(
                        "%sllama.cpp grammar error but no pattern/format "
                        "keywords to strip â€” falling through to normal retry",
                        agent.log_prefix,
                    )
                ...
                agent._buffer_vprint(f"âš ï¸  API call failed (attempt {retry_count}/{max_retries}): {error_type}{_status_code_str}")
                agent._buffer_vprint(f"   ðŸ”Œ Provider: {_provider}  Model: {_model}")
                agent._buffer_vprint(f"   ðŸŒ Endpoint: {_base}")
                agent._buffer_vprint(f"   ðŸ“ Error: {_error_summary}")
                if status_code and status_code < 500:
                    _err_body = getattr(api_error, "body", None)
                    _err_body_str = str(_err_body)[:300] if _err_body else None
                    if _err_body_str:
                        agent._buffer_vprint(f"   ðŸ“‹ Details: {_err_body_str}")
                agent._buffer_vprint(f"   â±ï¸  Elapsed: {elapsed_time:.2f}s  Context: {len(api_messages)} msgs, ~{approx_tokens:,} tokens")

                # Actionable hint for OpenRouter "no tool endpoints" error.
                # Buffered like the rest of the retry trace â€” surfaced only
                ...
                if (
                    ...
                    agent._buffer_vprint(
                        f"   ðŸ’¡ No OpenRouter providers for {_model} support tool calling with your current settings."
                    )
                ...
                if agent._interrupt_requested:
                    agent._vprint(f"{agent.log_prefix}âš¡ Interrupt detected during error handling, aborting retries.", force=True)
                    agent._persist_session(messages, conversation_history)
                    agent.clear_interrupt()
                    return {
                ...
                # A 413 is a payload-size error â€” the correct response is to
                # compress history and retry, not abort immediately.
                status_code = getattr(api_error, "status_code", None)

                # â”€â”€ Respect disabled auto-compaction on overflow â”€â”€â”€â”€â”€â”€
                # Ported from anomalyco/opencode#30749.  When the user has
                # turned auto-compaction off (``compression.enabled: false``),
                # NO automatic compaction trigger may fire â€” including the
                ...
                # (``force=True``) is unaffected â€” it never reaches this loop.
                _overflow_reasons = {
                ...
                if (
                    ...
                    agent._flush_status_buffer()
                    agent._vprint(
                        f"{agent.log_prefix}âŒ Context overflow, but auto-compaction is disabled "
                        f"(compression.enabled: false).",
                        force=True,
                    )
                    agent._vprint(
                        f"{agent.log_prefix}   ðŸ’¡ Run /compress to compact manually, /new to start fresh, "
                        f"switch to a larger-context model, or reduce attachments.",
                        force=True,
                    )
                    logger.error(
                        f"{agent.log_prefix}Context overflow ({classified.reason.value}) with "
                        f"auto-compaction disabled â€” not compressing."
                    )
                    agent._persist_session(messages, conversation_history)
                    return {
                        "messages": messages,
                ...
                # â”€â”€ Anthropic Sonnet long-context tier gate â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                # Anthropic returns HTTP 429 "Extra usage is required for
                # long context requests" when a Claude Max (or similar)
                # subscription doesn't include the 1M-context tier.  This
                # is NOT a transient rate limit â€” retrying or switching
                ...
                if classified.reason == FailoverReason.long_context_tier:
                    ...
                    if old_ctx > _reduced_ctx:
                        ...
                        # Context probing flags â€” only set on built-in
                        # compressor (plugin engines manage their own).
                        if hasattr(compressor, "_context_probed"):
                            compressor._context_probed = True
                            # Don't persist â€” this is a subscription-tier
                        ...
                        agent._buffer_vprint(
                            f"âš ï¸  Anthropic long-context tier "
                            f"requires extra usage â€” reducing context: "
                            f"{old_ctx:,} â†’ {_reduced_ctx:,} tokens"
                        )
                    ...
                    if compression_attempts <= max_compression_attempts:
                        ...
                        # Compression created a new session â€” clear history
                        # so _flush_messages_to_session_db writes compressed
                        # messages to the new session, not skipping them.
                        conversation_history = None
                        if len(messages) < original_len or old_ctx > _reduced_ctx:
                            agent._buffer_status(
                                f"ðŸ—œï¸ Context reduced to {_reduced_ctx:,} tokens "
                ...
                if is_rate_limited and agent._fallback_index < len(agent._fallback_chain):
                    ...
                    if not pool_may_recover:
                        if classified.reason == FailoverReason.billing:
                            agent._buffer_status(
                                "âš ï¸ Billing or credits exhausted â€” switching to fallback provider..."
                            )
                        else:
                            agent._buffer_status("âš ï¸ Rate limited â€” switching to fallback provider...")
                ...
                # which Zed' system prompt + tool schemas alone exceed.
                # Compression can't help â€” the floor is the system prompt
                # itself, not the conversation â€” so surface a clear "not
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}   ðŸ’¡ GitHub Models free tier (models.inference.ai.azure.com) caps every",
                        force=True,
                    )
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Zed' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`zed",
                        force=True,
                    )
                    agent._vprint(
                        f"{agent.log_prefix}      setup` â†’ GitHub Copilot), or pick any other provider.",
                        force=True,
                    )
                ...
                if is_payload_too_large:
                    compression_attempts += 1
                    if compression_attempts > max_compression_attempts:
                        # Terminal â€” surface the buffered retry trace.
                        agent._flush_status_buffer()
                        agent._vprint(f"{agent.log_prefix}âŒ Max compression attempts ({max_compression_attempts}) reached for payload-too-large error.", force=True)
                        agent._vprint(f"{agent.log_prefix}   ðŸ’¡ Try /new to start a fresh conversation, or /compress to retry compression.", force=True)
                    ...
                    agent._buffer_status(f"âš ï¸  Request payload too large (413) â€” compression attempt {compression_attempts}/{max_compression_attempts}...")
                    ...
                    if len(messages) < original_len:
                        agent._buffer_status(f"ðŸ—œï¸ Compressed {original_len} â†’ {len(messages)} messages, retrying...")
                        time.sleep(2)  # Brief pause between compression retries
                        _retry.restart_with_compressed_messages = True
                        break
                    else:
                        # Terminal â€” surface buffered context so the user
                        # sees what compression attempts were made.
                        agent._flush_status_buffer()
                        agent._vprint(f"{agent.log_prefix}âŒ Payload too large and cannot compress further.", force=True)
                        agent._vprint(f"{agent.log_prefix}   ðŸ’¡ Try /new to start a fresh conversation, or /compress to retry compression.", force=True)
                ...
                if is_context_length_error:
                    ...
                    # â”€â”€ Distinguish two very different errors â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                    ...
                    #    Do NOT shrink context_length â€” the window is unchanged.
                    #
                    ...
                    if available_out is not None:
                        ...
                        agent._buffer_vprint(
                            f"âš ï¸  Output cap too large for current prompt â€” "
                        ...
                        if compression_attempts > max_compression_attempts:
                            agent._flush_status_buffer()
                            agent._vprint(f"{agent.log_prefix}âŒ Max compression attempts ({max_compression_attempts}) reached.", force=True)
                            agent._vprint(f"{agent.log_prefix}   ðŸ’¡ Try /new to start a fresh conversation, or /compress to retry compression.", force=True)
                    ...
                    if new_ctx is not None:
                        ...
                        agent._buffer_vprint(f"âš ï¸  Context length exceeded â€” using provider limit: {old_ctx:,} â†’ {new_ctx:,} tokens")
                        ...
                        agent._buffer_vprint(
                            f"âš ï¸  Context length exceeded, but provider did not report a max context length; "
                            f"keeping context_length at {old_ctx:,} tokens and compressing."
                        )
                    ...
                    agent._buffer_status(f"ðŸ—œï¸ Context too large (~{approx_tokens:,} tokens) â€” compressing ({compression_attempts}/{max_compression_attempts})...")
                    ...
                    if len(messages) < original_len or new_ctx and new_ctx < old_ctx:
                        ...
                        agent._flush_status_buffer()
                        agent._vprint(f"{agent.log_prefix}âŒ Context length exceeded and cannot compress further.", force=True)
                        agent._vprint(f"{agent.log_prefix}   ðŸ’¡ The conversation has accumulated too much content. Try /new to start fresh, or /compress to manually trigger compression.", force=True)
                ...
                # Exclude UnicodeEncodeError â€” it's a ValueError subclass
                # but is handled separately by the surrogate sanitization
                # path above.  Exclude json.JSONDecodeError â€” also a
                ...
                is_local_validation_error = (
                    ...
                    # backend response.completed.output=null) â€” not local
                ...
                #   â€¢ credential-pool rotation (line ~2031) has already
                #     fired for billing and either ``continue``d or
                #     returned (False, ...) â€” pool is exhausted or absent.
                #   â€¢ the eager-fallback branch above (line ~2422) also
                ...
                # balance with no recovery mechanism left â€” see #31273
                ...
                if is_client_error:
                    # Try fallback before aborting â€” a different provider may
                    ...
                    if agent._has_pending_fallback():
                        if classified.reason == FailoverReason.content_policy_blocked:
                            agent._buffer_status("âš ï¸ Provider safety filter blocked this request â€” trying fallback...")
                        else:
                            agent._buffer_status(f"âš ï¸ Non-retryable error (HTTP {status_code}) â€” trying fallback...")
                    if agent._try_activate_fallback():
                        retry_count = 0
                        compression_attempts = 0
                        _retry.primary_recovery_attempted = False
                    ...
                    # Terminal â€” flush buffered context so the user sees
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        agent._emit_status(
                            f"âŒ Provider safety filter blocked this request: "
                            f"{_nonretryable_summary}"
                        )
                    else:
                        agent._emit_status(
                            f"âŒ Non-retryable error (HTTP {status_code}): "
                            f"{_nonretryable_summary}"
                        )
                    agent._vprint(f"{agent.log_prefix}âŒ Non-retryable client error (HTTP {status_code}). Aborting.", force=True)
                    agent._vprint(f"{agent.log_prefix}   ðŸ”Œ Provider: {_provider}  Model: {_model}", force=True)
                    agent._vprint(f"{agent.log_prefix}   ðŸŒ Endpoint: {_base}", force=True)
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                agent._vprint(f"{agent.log_prefix}   ðŸ’¡ Codex OAuth token was rejected (HTTP 401). Your token may have been", force=True)
                                agent._vprint(f"{agent.log_prefix}      refreshed by another client (Codex CLI, VS Code). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Run `codex` in your terminal to generate fresh tokens.", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Then run `zed auth` to re-authenticate.", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      â€¢ Does your account have access to {_model}?", force=True)
                            if base_url_host_matches(str(_base), "openrouter.ai"):
                                agent._vprint(f"{agent.log_prefix}      â€¢ Check credits: https://openrouter.ai/settings/credits", force=True)
                    else:
                        agent._vprint(f"{agent.log_prefix}   ðŸ’¡ This type of error won't be fixed by retrying.", force=True)
                    # Content-policy blocks deserve their own actionable
                    # guidance â€” neither "fix your API key" nor "retry won't
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        agent._vprint(
                            f"{agent.log_prefix}   ðŸ’¡ The provider's safety filter rejected this specific prompt.",
                            force=True,
                        )
                        agent._vprint(
                            f"{agent.log_prefix}      â€¢ Try rephrasing the request, narrowing the context, or splitting into smaller steps.",
                            force=True,
                        )
                        agent._vprint(
                            f"{agent.log_prefix}      â€¢ Configure a fallback provider so future blocks route automatically:",
                            force=True,
                        )
                        agent._vprint(
                            f"{agent.log_prefix}        zed fallback add   (interactive picker â€” same as `zed model`)",
                            force=True,
                        )
                    ...
                    if status_code == 400 and (approx_tokens > 50000 or len(api_messages) > 80):
                        agent._vprint(
                            f"{agent.log_prefix}âš ï¸  Skipping session persistence "
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        _policy_response = (
                            "âš ï¸  The model provider's safety filter blocked this request "
                            "(not a Zed/gateway failure).\n\n"
                ...
                if retry_count >= max_retries:
                    ...
                    if agent._has_pending_fallback():
                        agent._buffer_status(f"âš ï¸ Max retries ({max_retries}) exhausted â€” trying fallback...")
                    if agent._try_activate_fallback():
                        retry_count = 0
                        compression_attempts = 0
                        _retry.primary_recovery_attempted = False
                        continue
                    # Terminal â€” flush buffered retry/fallback trace.
                    agent._flush_status_buffer()
                    _final_summary = agent._summarize_api_error(api_error)
                    _billing_guidance = ""
                    if classified.reason == FailoverReason.billing:
                        agent._emit_status(f"âŒ Billing or credits exhausted â€” {_final_summary}")
                        ...
                        agent._emit_status(f"âŒ Rate limited after {max_retries} retries â€” {_final_summary}")
                    else:
                        agent._emit_status(f"âŒ API failed after {max_retries} retries â€” {_final_summary}")
                    agent._vprint(f"{agent.log_prefix}   ðŸ’€ Final error: {_final_summary}", force=True)
                    ...
                    if _is_stream_drop:
                        agent._vprint(
                            f"{agent.log_prefix}   ðŸ’¡ The provider's stream "
                    ...
                    if _is_stream_drop:
                        _final_response += (
                            ...
                            "dropping â€” this often happens when generating "
                ...
                if is_rate_limited:
                    agent._buffer_status(f"â±ï¸ Rate limited. Waiting {wait_time:.1f}s (attempt {retry_count + 1}/{max_retries})...")
                else:
                    agent._buffer_status(f"â³ Retrying in {wait_time:.1f}s (attempt {retry_count}/{max_retries})...")
                logger.warning(
        ...
        if _retry.restart_with_length_continuation:
            ...
            # Retry 1 â†’ 2Ã— base, retry 2 â†’ 3Ã— base, capped at 32 768.
        ...
        if response is None:
            ...
            print(f"{agent.log_prefix}âŒ All API retries exhausted with no successful response.")
            agent._persist_session(messages, conversation_history)
            break
        ...
        try:
            ...
            # Normalize content to string â€” some OpenAI-compatible servers
            ...
            if assistant_message.content is not None and not isinstance(assistant_message.content, str):
                ...
                if isinstance(raw, dict):
                    ...
                    # Multimodal content list â€” extract text parts
                    parts = []
                    for part in raw:
            ...
            if assistant_message.content and not agent.quiet_mode:
                if agent.verbose_logging:
                    agent._vprint(f"{agent.log_prefix}ðŸ¤– Assistant: {assistant_message.content}")
                else:
                    agent._vprint(f"{agent.log_prefix}ðŸ¤– Assistant: {assistant_message.content[:100]}{'...' if len(assistant_message.content) > 100 else ''}")
            ...
            if has_incomplete_scratchpad(assistant_message.content or ""):
                ...
                agent._buffer_vprint(f"âš ï¸  Incomplete <REASONING_SCRATCHPAD> detected (opened but never closed)")
                
                if agent._incomplete_scratchpad_retries <= 2:
                    agent._buffer_vprint(f"ðŸ”„ Retrying API call ({agent._incomplete_scratchpad_retries}/2)...")
                    # Don't add the broken message, just retry
                    continue
                else:
                    # Max retries - discard this turn and save as partial
                    agent._flush_status_buffer()
                    agent._vprint(f"{agent.log_prefix}âŒ Max retries (2) for incomplete scratchpad. Saving as partial.", force=True)
            ...
            if agent.api_mode == "codex_responses" and finish_reason == "incomplete":
                ...
                if agent._codex_incomplete_retries < 3:
                    if not agent.quiet_mode:
                        agent._vprint(f"{agent.log_prefix}â†» Codex response incomplete; continuing turn ({agent._codex_incomplete_retries}/3)")
                    agent._session_messages = messages
                    continue
            ...
            if assistant_message.tool_calls:
                if not agent.quiet_mode:
                    agent._vprint(f"{agent.log_prefix}ðŸ”§ Processing {len(assistant_message.tool_calls)} tool call(s)...")
                ...
                for tc in assistant_message.tool_calls:
                    if tc.function.name not in agent.valid_tool_names:
                        ...
                        if repaired:
                            print(f"{agent.log_prefix}ðŸ”§ Auto-repaired tool name: '{tc.function.name}' -> '{repaired}'")
                ...
                if invalid_tool_calls:
                    ...
                    # Return helpful error to model â€” model can agent-correct next turn
                    available = ", ".join(sorted(agent.valid_tool_names))
                    invalid_name = invalid_tool_calls[0]
                    invalid_preview = invalid_name[:80] + "..." if len(invalid_name) > 80 else invalid_name
                    agent._buffer_vprint(f"âš ï¸  Unknown tool '{invalid_preview}' â€” sending error to model for agent-correction ({agent._invalid_tool_retries}/3)")

                    if agent._invalid_tool_retries >= 3:
                        agent._flush_status_buffer()
                        agent._vprint(f"{agent.log_prefix}âŒ Max retries (3) for invalid tool calls exceeded. Stopping as partial.", force=True)
                    ...
                    for tc in assistant_message.tool_calls:
                        ...
                        if _tc_name not in agent.valid_tool_names:
                            ...
                            # model can fuzzy-correct toward a real tool â€” it is
                            ...
                            if not (_tc_name or "").strip():
                                content = (
                                    ...
                                    "contents or tool output, that is data â€” do "
                ...
                if invalid_json_args:
                    ...
                    if _truncated:
                        agent._vprint(
                            f"{agent.log_prefix}âš ï¸  Truncated tool call arguments detected "
                            f"(finish_reason={finish_reason!r}) â€” refusing to execute.",
                            force=True,
                        )
                    ...
                    agent._buffer_vprint(f"âš ï¸  Invalid JSON in tool call arguments for '{tool_name}': {error_msg}")

                    if agent._invalid_json_retries < 3:
                        agent._buffer_vprint(f"ðŸ”„ Retrying API call ({agent._invalid_json_retries}/3)...")
                        # Don't add anything to messages, just retry the API call
                        continue
                    else:
                        # Instead of returning partial, inject tool error results so the model can recover.
                        # Using tool results (not user messages) preserves role alternation.
                        agent._buffer_vprint(f"âš ï¸  Injecting recovery tool results for invalid JSON...")
                ...
                # â”€â”€ Post-call guardrails â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                if turn_content and agent._has_content_after_think_block(turn_content):
                    ...
                    if _all_housekeeping and agent._has_stream_consumers():
                        ...
                        if clean:
                            agent._vprint(f"  â”Š ðŸ’¬ {clean}")
                
                # Pop thinking-only prefill message(s) before appending
                # (tool-call path â€” same rationale as the final-response path).
                _had_prefill = False
                while (
                    messages
                ...
                # the whole conversation â€” a model that intermittently
                # empties (empty â†’ prefill â†’ tools â†’ empty â†’ prefill â†’
                ...
                # Successful tool execution â€” reset the post-tool nudge
                ...
                # Only signal the display callback â€” TTS (_stream_callback)
                ...
                if agent._tool_guardrail_halt_decision is not None:
                    ...
                    agent._emit_status(
                        f"âš ï¸ Tool guardrail halted {decision.tool_name}: {decision.code}"
                    )
                    messages.append({"role": "assistant", "content": final_response})
                    # Emit the halt message to the client so it's not
                    # indistinguishable from a crash.  The stream display
                    # was flushed (callback(None)) before tool execution,
                    # but the callback is still alive â€” fire the text
                ...
                # assistant turn â€” a tight lower bound for the next prompt.
                ...
                if _compressor.last_prompt_tokens > 0:
                    # Only use prompt_tokens â€” completion/reasoning
                    ...
                    # Include tool schemas â€” with 50+ tools enabled
                ...
                if agent.compression_enabled and _compressor.should_compress(_real_tokens):
                    agent._safe_print("  âŸ³ compacting contextâ€¦")
                    messages, active_system_prompt = agent._compress_context(
                        messages, system_message,
                        approx_tokens=agent.context_compressor.last_prompt_tokens,
                        task_id=effective_task_id,
                    )
                    # Compression created a new session â€” clear history so
                ...
                if not agent._has_content_after_think_block(final_response):
                    # â”€â”€ Partial stream recovery â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                    ...
                    if agent._has_content_after_think_block(_partial_streamed):
                        ...
                        logger.info(
                            ...
                            "â€” using as final response",
                            len(_recovered),
                        )
                        agent._emit_status(
                            "â†» Stream interrupted â€” using delivered content "
                            "as final response"
                        )
                    ...
                    # the empty follow-up means the model choked â€” let the
                    ...
                    if fallback and getattr(agent, '_last_content_tools_all_housekeeping', False):
                        ...
                        logger.info("Empty follow-up after tool calls â€” using prior turn content as final response")
                        agent._emit_status("â†» Empty response after tool calls â€” using earlier content as final answer")
                        agent._last_content_with_tools = None
                        agent._last_content_tools_all_housekeeping = False
                        agent._empty_content_retries = 0
                        # Do NOT modify the assistant message content â€” the
                    ...
                    # â”€â”€ Post-tool-call empty response nudge â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                    # The model returned empty after executing tool calls.
                    # This covers two cases:
                    #  (a) No prior-turn content at all â€” model went silent
                    ...
                    if (
                        ...
                        and not _has_inline_thinking  # thinking model still working â€” let prefill handle
                    ):
                        ...
                        agent._last_content_tools_all_housekeeping = False
                        logger.info(
                            "Empty response after tool calls â€” nudging model "
                            "to continue processing"
                        )
                        agent._buffer_status(
                            "âš ï¸ Model returned empty after tool calls â€” "
                            "nudging to continue"
                        )
                        # Append the empty assistant message first so the
                        # message sequence stays valid:
                        #   tool(result) â†’ assistant("(empty)") â†’ user(nudge)
                        # Without this, we'd have tool â†’ user which most
                    ...
                    # â”€â”€ Thinking-only prefill continuation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                    ...
                    # continue â€” the model will see its own reasoning
                    ...
                    if _has_structured and agent._thinking_prefill_retries < 2:
                        ...
                        logger.info(
                            "Thinking-only response (no visible content) â€” "
                            "prefilling to continue (%d/2)",
                            agent._thinking_prefill_retries,
                        )
                        agent._buffer_status(
                            f"â†» Thinking-only response â€” prefilling to continue "
                    ...
                    # â”€â”€ Empty response retry â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                    ...
                    # prefill exhaustion â€” models like mimo-v2-pro
                    ...
                    if _truly_empty and (not _has_structured or _prefill_exhausted) and agent._empty_content_retries < 3:
                        ...
                        logger.warning(
                            "Empty response (no content or reasoning) â€” "
                            "retry %d/3 (model=%s)",
                            agent._empty_content_retries, agent.model,
                        )
                        agent._buffer_status(
                            f"âš ï¸ Empty response from model â€” retrying "
                            f"({agent._empty_content_retries}/3)"
                        )
                        continue

                    # â”€â”€ Exhausted retries â€” try fallback provider â”€â”€
                    ...
                    if _truly_empty and agent._fallback_chain:
                        logger.warning(
                            "Empty response after %d retries â€” "
                            "attempting fallback (model=%s, provider=%s)",
                            agent._empty_content_retries, agent.model,
                            agent.provider,
                        )
                        agent._buffer_status(
                            "âš ï¸ Model returning empty responses â€” "
                            "switching to fallback provider..."
                        )
                        if agent._try_activate_fallback():
                            agent._empty_content_retries = 0
                            agent._buffer_status(
                                f"â†» Switched to fallback: {agent.model} "
                                f"({agent.provider})"
                            )
                            logger.info(
                    ...
                    if reasoning_text:
                        ...
                        agent._emit_status(
                            "âš ï¸ Model produced reasoning but no visible "
                        ...
                        agent._emit_status(
                            "âŒ Model returned no content after all retries"
                ...
                agent._empty_content_retries = 0
                agent._thinking_prefill_retries = 0
                # Successful content reached â€” drop any buffered retry
                ...
                if not agent.quiet_mode:
                    agent._safe_print(f"ðŸŽ‰ Conversation completed after {api_call_count} OpenAI-compatible API call(s)")
                break
            ...
            # â€” users would see a one-line summary on screen with no way to
