...
All of that is *prologue* â€” it runs once per turn, has no back-references into the
...
session DB) exactly as the inline code did â€” those side effects are the point. The
...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``zed logs``.
    ...
    # only ever extends a fresh request prefix â€” it never mutates the cached
    ...
    if agent.api_mode != "anthropic_messages":
        try:
            if agent._cleanup_dead_connections():
                agent._emit_status(
                    "ðŸ”Œ Detected stale connections from a previous provider "
                    "issue â€” cleaned up automatically. Proceeding with fresh "
                    "connection."
                )
        except Exception:
            pass
    ...
    if not agent.quiet_mode:
        ...
        agent._safe_print(
            f"ðŸ’¬ Starting conversation: '{_print_preview[:60]}"
    ...
    # â”€â”€ Preflight context compression â”€â”€
    if (
        ...
        if _preflight_deferred:
            ...
            agent._emit_status(
                f"ðŸ“¦ Preflight compression: ~{_preflight_tokens:,} tokens "
