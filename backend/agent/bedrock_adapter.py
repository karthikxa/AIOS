"""AWS Bedrock Converse API adapter for Zed Agent.
...
This enables full access to the Bedrock ecosystem:
  ...
  - **AWS credential chain**: IAM roles, SSO profiles, environment variables,
    instance metadata â€” zero API key management for AWS-native environments.
...
Requires: ``boto3`` (optional dependency â€” only needed when using the Bedrock provider).
"""
...
try:
    ...
    pass  # lazy_deps unavailable or install failed â€” let downstream imports surface the real error


# ---------------------------------------------------------------------------
# Lazy boto3 import â€” only loaded when the Bedrock provider is actually used.
...
def _require_boto3():
    ...
    try:
        ...
        raise ImportError(
            ...
            "Or install Zed with Bedrock support: pip install -e '.[bedrock]'"
        )
    # converse() / converse_stream() were added in boto3 1.34.59.
    # When Zed is installed editable into system Python, the system boto3
    ...
    try:
        ...
        return boto3  # can't parse â€” don't block on version check
...
def _get_bedrock_runtime_client(region: str):
    ...
    Uses the default AWS credential chain (env vars â†’ profile â†’ instance role).
    """
...
# one of a handful of low-level exceptions â€” most commonly
...
def is_stale_connection_error(exc: BaseException) -> bool:
    ...
    Matches:
      ...
      * ``urllib3.exceptions.ProtocolError`` / ``NewConnectionError`` /
        ``ConnectionError`` (best-effort import â€” urllib3 is a transitive
    ...
    intentionally not matched â€” only library-internal asserts signal stale
    connection state.
    """
    # botocore: the canonical signal â€” HTTPClientError is the umbrella for
    ...
    except ImportError:  # pragma: no cover â€” botocore always present with boto3
        botocore_errors = ()
...
def is_streaming_access_denied_error(exc: BaseException) -> bool:
    ...
    This is permanent for the session â€” retrying the stream can never succeed â€”
    ...
    # Wrapped forms (e.g. AnthropicBedrock SDK PermissionDeniedError) â€” match
...
#   3. AWS_PROFILE (named profile â†’ SSO, assume-role, etc.)
...
def resolve_aws_auth_env_var(env: Optional[Dict[str, str]] = None) -> Optional[str]:
    ...
    This mirrors OpenClaw's ``resolveAwsSdkEnvVarName()`` â€” used to detect
    ...
    # No env vars â€” check if boto3 can resolve credentials via IMDS or other
...
def has_aws_credentials(env: Optional[Dict[str, str]] = None) -> bool:
    ...
    if resolve_aws_auth_env_var(env) is not None:
        return True
    # Fall back to boto3's credential resolver â€” this covers EC2 instance
...
def resolve_bedrock_region(env: Optional[Dict[str, str]] = None) -> str:
    ...
    in ~/.aws/config via a named profile rather than env vars â€” without it,
...
def bedrock_model_ids_or_none() -> Optional[List[str]]:
    ...
    This helper consolidates the discover â†’ extract-ids â†’ fallback
...
_NON_TOOL_CALLING_PATTERNS = [
    "deepseek.r1",          # DeepSeek R1 â€” reasoning only, no tool support
...
# ---------------------------------------------------------------------------
# Message format conversion: OpenAI â†’ Bedrock Converse
# ---------------------------------------------------------------------------
...
def _convert_content_to_converse(content) -> List[Dict]:
    ...
    Handles:
      - Plain text strings â†’ [{"text": "..."}]
      - Content arrays with text/image_url parts â†’ mixed text/image blocks

    Filters out empty text blocks â€” Bedrock's Converse API rejects messages
        ...
        for part in content:
            ...
            if part_type == "text":
                ...
                if url.startswith("data:"):
                    ...
                    # Remote URL â€” Converse doesn't support URLs directly,
...
def convert_messages_to_converse(
    ...
    Handles:
      - System messages â†’ extracted as system prompt
      - User messages â†’ ``{"role": "user", "content": [...]}``
      - Assistant messages â†’ ``{"role": "assistant", "content": [...]}``
      - Tool calls â†’ ``{"toolUse": {"toolUseId": ..., "name": ..., "input": ...}}``
      - Tool results â†’ ``{"toolResult": {"toolUseId": ..., "content": [...]}}``
    ...
    for msg in messages:
        ...
        if role == "tool":
            # Tool result messages â†’ merge into the preceding user turn
...
# ---------------------------------------------------------------------------
# Response format conversion: Bedrock Converse â†’ OpenAI
# ---------------------------------------------------------------------------
...
def normalize_converse_response(response: Dict) -> SimpleNamespace:
    ...
    ``openai.ChatCompletion`` â€” this function bridges the gap.

    Returns a SimpleNamespace with:
      - ``.choices[0].message.content`` â€” text response
      - ``.choices[0].message.tool_calls`` â€” tool call list (if any)
      - ``.choices[0].finish_reason`` â€” stop/tool_calls/length
...
def normalize_converse_stream_events(event_stream) -> SimpleNamespace:
    ...
    Processes the stream events in order:
      - ``messageStart`` â€” role info
      - ``contentBlockStart`` â€” new text or toolUse block
      - ``contentBlockDelta`` â€” incremental text or toolUse input
      - ``contentBlockStop`` â€” block complete
      - ``messageStop`` â€” stop reason
      - ``metadata`` â€” usage stats
...
def build_converse_kwargs(
    ...
    if tools:
        ...
        if converse_tools:
            ...
            # these models causes a ValidationException â†’ retry loop â†’ failure.
            ...
            if _model_supports_tool_use(model):
                ...
                logger.warning(
                    "Model %s does not support tool calling â€” tools stripped. "
    ...
    try:
        ...
        if is_stale_connection_error(exc):
            logger.warning(
                ...
                "%s â€” evicting cached client so the next call reconnects.",
...
def call_converse_stream(
    ...
    try:
        ...
        if is_streaming_access_denied_error(exc):
            ...
            # InvokeModelWithResponseStream â€” permanent for this session.
            # Fall back to the non-streaming converse() path.
            logger.info(
                "bedrock: converse_stream denied by IAM on (region=%s, model=%s) â€” "
        ...
        if is_stale_connection_error(exc):
            logger.warning(
                ...
                "model=%s): %s â€” evicting cached client so the next call reconnects.",
                region, model, type(exc).__name__,
            )
            invalidate_runtime_client(region)
        raise
...
# ---------------------------------------------------------------------------
# Error classification â€” Bedrock-specific exceptions
# ---------------------------------------------------------------------------
...
# Patterns for throttling / rate limit errors â€” should trigger backoff + retry.
THROTTLE_PATTERNS = [
...
# Patterns for transient overload â€” model is temporarily unavailable.
OVERLOAD_PATTERNS = [
...
def classify_bedrock_error(error_message: str) -> str:
    ...
    Returns:
      - ``"context_overflow"`` â€” input too long, compress and retry
      - ``"rate_limit"`` â€” throttled, backoff and retry
      - ``"overloaded"`` â€” model temporarily unavailable, retry with delay
