...
into a frozen dataclass; **fail open** â€” when not logged in or the portal is
...
def parse_money(value: Any) -> Optional[Decimal]:
    ...
    defensively, float â€” though the server always sends strings).
    """
    if value is None:
        return None
    try:
...
def format_money(value: Optional[Decimal]) -> str:
    ...
    ``Decimal("142.5")`` â†’ ``"$142.50"``, ``Decimal("100")`` â†’ ``"$100"``,
    ``Decimal("0.01")`` â†’ ``"$0.01"``.
    """
    if value is None:
        return "â€”"
    if value == value.to_integral_value():
        # Whole dollars â€” no decimal point. format(..., "f") avoids 1E+3 for 1000.
        return f"${format(value.to_integral_value(), 'f')}"
    # Fractional â€” always show 2dp.
...
class CardInfo:
    ...
    @property
    def masked(self) -> str:
        return f"{self.brand} Â·Â·Â·Â·{self.last4}"
...
@dataclass(frozen=True)
class BillingState:
    """Parsed ``GET /api/billing/state`` â€” the overview screen's data.
    ...
    def is_admin(self) -> bool:
        """True for OWNER/ADMIN â€” the roles that can manage billing."""
...
def build_billing_state(*, timeout: float = 15.0) -> BillingState:
    ...
    try:
        ...
        logger.debug("billing â–¸ /state fetch failed (fail-open)", exc_info=True)
        return BillingState(logged_in=False, error=str(exc))
    except Exception:
        logger.debug("billing â–¸ /state unexpected error (fail-open)", exc_info=True)
...
def validate_charge_amount(
    ...
    # multipleOf 0.01 â€” reject sub-cent precision.
