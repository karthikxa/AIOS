...
as ``kind: backend``) or ``~/.zed/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
Unified surface
---------------
One tool â€” ``video_generate`` â€” covers **text-to-video** and **image-to-video**.
...
Video edit and video extend are intentionally NOT exposed in this surface â€”
...
# a narrower or wider set â€” they are responsible for clamping.
...
class VideoGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``zed tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``zed tools`` model picker.
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``zed tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$ZED_HOME/cache/videos/``, creating parents as needed."""
    from zed_constants import get_zed_home

    path = get_zed_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$ZED_HOME/cache/videos/``.
...
def success_response(
    *,
    ...
    ``modality`` is ``"text"`` (text-to-video) or ``"image"`` (image-to-video) â€”
