...
Owns one Codex thread per Zed session. Drives `turn/start`, consumes
...
Lifecycle:
    ...
    # result.final_text          â†’ assistant text returned to caller
    # result.projected_messages  â†’ list of {role, content, ...} for messages list
    # result.tool_iterations     â†’ how many tool-shaped items completed (skill nudge counter)
    # result.interrupted         â†’ True if Ctrl+C / interrupt_requested fired mid-turn
...
# Zed' tools.terminal.security_mode â†’ Codex's permissions profile id.
# Defaults if config is missing â†’ workspace-write (matches Codex's own default).
_ZED_TO_CODEX_PERMISSION_PROFILE = {
...
@dataclass
class TurnResult:
    """Result of one userâ†’assistantâ†’tool turn through the codex app-server."""
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Zed/OpenAI rich content into app-server text input.
...
def _classify_oauth_failure(*parts: str) -> Optional[str]:
    ...
    purpose â€” we only redirect users to `codex login` when the signal
    ...
    for needle in _OAUTH_REFRESH_FAILURE_HINTS:
        if needle in haystack:
            return (
                "Codex authentication failed â€” your ChatGPT/Codex login "
...
class CodexAppServerSession:
    """One Codex thread per Zed session, lifetime owned by AIAgent.

    Not thread-safe â€” one caller drives it at a time, matching how AIAgent's
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ZED_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ZED_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        thread. Returns the codex thread id. Idempotent â€” repeated calls
        ...
        self._client.initialize(
            client_name="zed",
            client_title="Zed Agent",
            client_version=_get_zed_version(),
        )
        # Permission selection is intentionally NOT sent on thread/start.
        # Two reasons (live-tested against codex 0.130.0):
        #   1. `thread/start.permissions` is gated behind the experimentalApi
        #      capability on this codex version â€” we'd have to opt in during
        ...
        # Cross-fill thread.id/sessionId â€” different codex versions have
    ...
    def run_turn(
        ...
        *,
        ...
        # the same way per-turn failures do â€” with a TurnResult.error string
        # the caller can render â€” instead of bubbling raw codex exceptions
        ...
        try:
            ...
            # Subprocess almost certainly unhealthy â€” retire so the next
        ...
        # supports rich content but Zed' text path is the common case).
        try:
            ...
            if hint is not None:
                ...
                # token store is broken â€” retire so the next turn does a
        ...
        while time.monotonic() < deadline and not turn_complete:
            ...
            # the process), we won't get any more notifications â€” bail out
            ...
            if sreq is not None:
                ...
                # Activity counts as live signal â€” reset the post-tool
            ...
            if projection.is_tool_iteration:
                ...
                # output â€” clear the quiet timer so we don't fast-fail.
        ...
        if not turn_complete and not result.interrupted:
            ...
            # tell the caller to retire the session â€” a turn that never
    ...
    def _issue_interrupt(self, turn_id: Optional[str]) -> None:
        ...
        try:
            ...
            # "no active turn to interrupt" is fine â€” already done.
            logger.debug("turn/interrupt non-fatal: %s", exc)
        except TimeoutError:
            logger.warning("turn/interrupt timed out")

    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Zed' approval
        flow, then send the response.

        Method names verified live against codex 0.130.0 (Apr 2026):
          item/commandExecution/requestApproval â€” exec approvals
          item/fileChange/requestApproval       â€” apply_patch approvals
          item/permissions/requestApproval      â€” permissions changes
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # always decline â€” the user already chose their permission
            ...
            # OAuth, form data). For our own zed-tools callback we
            # auto-accept â€” the user already approved Zed' tools
            ...
            # Unknown server request â€” codex can extend this surface. Reject
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        # Codex's CommandExecutionRequestApprovalParams has cwd as Optional â€”
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        if self._approval_callback is not None:
            ...
            # item which the projector has already cached for us â€” look it
    ...
    def _track_pending_file_change(self, note: dict) -> None:
        ...
        actually changing â€” codex's approval params don't carry the data."""
        method = note.get("method", "")
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Zed approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Zed returns 'once', 'session', 'always', or 'deny'.
...
def _get_zed_version() -> str:
    """Best-effort Zed version string for codex's userAgent line."""
    try:
