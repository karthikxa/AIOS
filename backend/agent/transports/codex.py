...
This transport owns format conversion and normalization â€” NOT client lifecycle,
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        **params,
        ...
        params:
            instructions: str â€” system prompt (extracted from messages[0] if not given)
            reasoning_config: dict | None â€” {effort, enabled}
            session_id: str | None â€” used for prompt_cache_key + xAI conv header
            max_tokens: int | None â€” max_output_tokens
            timeout: float | None â€” per-request timeout forwarded to the SDK
            request_overrides: dict | None â€” extra kwargs merged in
            provider: str | None â€” provider name for backend-specific logic
            base_url: str | None â€” endpoint URL
            base_url_hostname: str | None â€” hostname for backend detection
            is_github_responses: bool â€” Copilot/GitHub models backend
            is_codex_backend: bool â€” chatgpt.com/backend-api/codex
            is_xai_responses: bool â€” xAI/Grok backend
            github_reasoning_extra: dict | None â€” Copilot reasoning params
        """
        ...
        # the intent to that native engine â€” but because the tool is
        ...
        # web_extract, MCP tools, â€¦) are untouched and continue to dispatch
        # through Zed's agent loop.
        #
        # Scope: we ONLY swap in the native built-in when the client
        # ``web_search`` was actually present.  We do NOT force-enable Grok
        # server-side search on turns where the user never had web enabled â€”
        # that would silently route around Zed's web-provider config and
        ...
        # native search engine for xAI sessions instead of Zed's
        # configured web provider (Tavily/etc.), and those results bypass
        # Zed's tool-trace / citation plumbing (they arrive baked into the
        ...
        # supported: service_tier") â€” hit when ``/fast`` priority-processing
        ...
        if is_xai_responses and session_id:
            ...
            # xAI Responses cache-routing â€” body-level field per
    ...
    def validate_response(self, response: Any) -> bool:
        ...
        Does NOT check output_text fallback â€” the caller handles that
