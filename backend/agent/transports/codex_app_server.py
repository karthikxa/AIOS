...
projection into Zed' display, approval bridging, transcript projection into
...
"codex_app_server"`. Zed' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        if spawn_env.get("ZED_KANBAN_TASK"):
            kanban_db = spawn_env.get("ZED_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "ZED_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("ZED_HOME", os.path.expanduser("~/.zed")),
                        "kanban",
                    ),
                )
            )
