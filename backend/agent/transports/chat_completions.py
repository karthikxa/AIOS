...
Messages and tools are already in OpenAI format â€” convert_messages and
...
def _build_gemini_thinking_config(model: str, reasoning_config: dict | None) -> dict | None:
    """Translate Zed/OpenRouter-style reasoning config to Gemini thinkingConfig."""
    ...
    # Cannot find field" â€” including the polite ``{"includeThoughts": False}``
    ...
    # Gemini 2.5 accepts thinkingBudget; don't guess a budget from Zed'
    ...
    # is stricter (low/high). Clamp Zed' wider effort set to what each
...
def _model_consumes_thought_signature(model: Any) -> bool:
    ...
    otherwise â€” including when a non-Gemini model inherits stale Gemini
...
class ChatCompletionsTransport(ProviderTransport):
    ...
    def convert_messages(
        ...
        """Messages are already in OpenAI format â€” strip internal fields
        ...
        - ``extra_content`` on ``tool_calls`` (Gemini thought_signature) â€”
        ...
        - ``tool_name`` on tool-result messages â€” written by
        ...
        - Zed-internal scaffolding markers â€” any top-level message key
        ...
        for msg in sanitized:
            ...
            # Drop all Zed-internal scaffolding markers (``_``-prefixed).
    ...
    def convert_tools(self, tools: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Tools are already in OpenAI format â€” identity."""
        return tools
    ...
    def build_kwargs(
        ...
        **params,
        ...
        params (all optional):
            timeout: float â€” API call timeout
            max_tokens: int | None â€” user-configured max tokens
            ephemeral_max_output_tokens: int | None â€” one-shot override
            max_tokens_param_fn: callable â€” returns {max_tokens: N} or {max_completion_tokens: N}
            reasoning_config: dict | None
            request_overrides: dict | None
            session_id: str | None
            model_lower: str â€” lowercase model name for pattern matching
            # Provider profile path (all per-provider quirks live in providers/)
            provider_profile: ProviderProfile | None â€” when present, delegates to
                _build_kwargs_from_profile(); all flag params below are bypassed.
            # Legacy-path flags â€” only used when provider_profile is None
            ...
            # Qwen-specific
            qwen_prepare_fn: callable | None â€” runs AFTER codex sanitization
            qwen_prepare_inplace_fn: callable | None â€” in-place variant for deepcopied lists
            qwen_session_metadata: dict | None
            # Temperature
            fixed_temperature: Any â€” from _fixed_temperature_for_model()
        ...
        # â”€â”€ Provider profile: single-path when present â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ Legacy fallback (unregistered / unknown provider) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # max_tokens resolution â€” priority: ephemeral > user > provider default
        ...
        # Pareto Code router plugin â€” model-gated. Same shape as the
    ...
    def _build_kwargs_from_profile(self, profile, model, sanitized, tools, params):
        """Build API kwargs using a ProviderProfile â€” single path, no legacy flags.
        ...
        # Developer role swap â€” model-name-based, applies to all providers
        ...
        # Tools â€” apply Moonshot/Kimi schema sanitization regardless of path
        if tools:
            if is_moonshot_model(model):
                tools = sanitize_moonshot_tools(tools)
            api_kwargs["tools"] = tools

        # max_tokens resolution â€” priority: ephemeral > user > profile default
        max_tokens_fn = params.get("max_tokens_param_fn")
        ephemeral = params.get("ephemeral_max_output_tokens")
        user_max = params.get("max_tokens")
        anthropic_max = params.get("anthropic_max_output")
        # Per-model default cap â€” profiles override get_max_tokens() when
        ...
        if extra_body:
            ...
            # keys (tags, reasoning, provider, plugins, â€¦) are unknown fields
            ...
            # Gemini base_url â€” typical when only Google credentials are set and
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        For chat_completions, this is near-identity â€” the response is already
        ...
        if msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # thought_signature â€” without replay on the next turn the API
        ...
        # Bedrock (e.g. Nous Portal) surface a Claude refusal this way â€” or via
        # ``finish_reason="content_filter"`` â€” instead of the native
        ...
        if isinstance(refusal, str) and refusal.strip():
            # Record the refusal explanation regardless â€” it's useful provider
            ...
            # the *sole* payload â€” no visible text and no tool calls. A response
