"""Projects codex app-server events into Zed' messages list.

The translator that lets Zed' memory/skill review keep working under the
...
Codex emits items with a discriminator field `type`:
  - userMessage         â†’ {role: "user", content}
  - agentMessage        â†’ {role: "assistant", content}
  - reasoning           â†’ stashed in the assistant's "reasoning" field
  - commandExecution    â†’ assistant tool_call(name="exec") + tool result
  - fileChange          â†’ assistant tool_call(name="apply_patch") + tool result
  - mcpToolCall         â†’ assistant tool_call(name=f"mcp.{server}.{tool}") + tool result
  - dynamicToolCall     â†’ assistant tool_call(name=tool) + tool result
  - plan/hookPrompt/collabAgentToolCall â†’ recorded as opaque assistant notes

Each item maps to AT MOST one assistant entry + one tool entry, preserving
Zed' message-alternation invariants (system â†’ user â†’ assistant â†’ user/tool
â†’ assistant â†’ ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Zed
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Zed' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Zed stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list â€” same way Zed already only writes
        ...
        # â€” record as opaque assistant note so memory review can still see
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) â€” Zed' messages store text only.
        text_parts: list[str] = []
    ...
    def _project_file_change(self, item: dict, item_id: str) -> ProjectionResult:
        ...
        # without inlining full file contents â€” those can be huge.
        changes_summary = []
