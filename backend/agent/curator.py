"""Curator â€” background skill maintenance orchestrator.
...
Strict invariants:
  ...
  - Never auto-deletes â€” only archives. Archive is recoverable.
...
# ---------------------------------------------------------------------------
# .curator_state â€” persistent scheduler + status
# ---------------------------------------------------------------------------

def _state_file() -> Path:
    return get_zed_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.zed/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `zed update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    forked aux-model review entirely â€” no consolidation, no umbrella-building,
    ...
    The explicit ``zed curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``zed update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``zed curator run`` (with or without
    ``--dry-run``) explicitly â€” that path bypasses this gate.

    The idle check (min_idle_hours) is applied at the call site where we know
    whether an agent is actively running â€” here we only enforce the static
    gates.
    """
    if not is_enabled():
        return False
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                "deferred first run â€” curator seeded, will run after one "
                "interval; use `zed curator run --dry-run` to preview now"
            )
            save_state(state)
        except Exception as e:  # pragma: no cover â€” best-effort persistence
...
def apply_automatic_transitions(now: Optional[datetime] = None) -> Dict[str, int]:
    ...
    clock starts NOW rather than at epoch â€” a long-unused built-in is therefore
    ...
    for row in _u.agent_created_report():
        ...
        if anchor <= archive_cutoff and current != _u.STATE_ARCHIVED:
            ...
            # Skill got used again after being marked stale â€” reactivate.
...
CURATOR_DRY_RUN_BANNER = (
    ...
    "DRY-RUN â€” REPORT ONLY. DO NOT MUTATE THE SKILL LIBRARY.\n"
    "â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•\n"
    "\n"
    "This is a PREVIEW pass. Follow every instruction below EXCEPT:\n"
    "\n"
    "  â€¢ DO NOT call skill_manage with action=patch, create, delete, "
    "write_file, or remove_file.\n"
    "  â€¢ DO NOT call terminal to mv skill directories into .archive/.\n"
    "  â€¢ DO NOT call terminal to mv, cp, rm, or rewrite any file under "
    "~/.zed/skills/.\n"
    "  â€¢ skills_list and skill_view are FINE â€” read as much as you need.\n"
    "\n"
    "Your output IS the deliverable. Produce the exact same "
    "human-readable summary and structured YAML block you would "
    "produce on a live run â€” but describe the actions you WOULD take, "
    "not actions you took. A downstream reviewer will read the report "
    "and decide whether to approve a live run with "
    "`zed curator run` (no flag).\n"
    "\n"
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Zed' background skill CURATOR. This is an "
    "UMBRELLA-BUILDING consolidation pass, not a passive audit and not a "
    "duplicate-finder.\n\n"
    "The goal of the skill collection is a LIBRARY OF CLASS-LEVEL "
    "INSTRUCTIONS AND EXPERIENTIAL KNOWLEDGE. A collection of hundreds of "
    "narrow skills where each one captures one session's specific bug is "
    "a FAILURE of the library â€” not a feature. An agent searching skills "
    ...
    "into ~/.zed/skills/.archive/) is the maximum destructive action. "
    ...
    "tips) and are filtered out of the candidate list below â€” never resurrect "
    ...
    "to find: zed-config-*, zed-dashboard-*, gateway-*, codex-*, "
    ...
    "   a. MERGE INTO EXISTING UMBRELLA â€” one skill in the cluster is "
    ...
    "siblings.\n"
    "   b. CREATE A NEW UMBRELLA SKILL.md â€” no existing member is broad "
    ...
    "   c. DEMOTE TO REFERENCES/TEMPLATES/SCRIPTS â€” a sibling has "
    "narrow-but-valuable session-specific content. Move it into the "
    "umbrella's appropriate support directory:\n"
    "      â€¢ `references/<topic>.md` for session-specific detail OR "
    "condensed knowledge banks (quoted research, API docs excerpts, "
    "domain notes, provider quirks, reproduction recipes)\n"
    "      â€¢ `templates/<name>.<ext>` for starter files meant to be "
    "copied and modified\n"
    "      â€¢ `scripts/<name>.<ext>` for statically re-runnable actions "
    "(verification scripts, fixture generators, probes)\n"
    "      Then archive the old sibling. Use `terminal` with `mkdir -p "
    "~/.zed/skills/<umbrella>/references/ && mv ... <umbrella>/"
    "references/<topic>.md` (or templates/ / scripts/).\n\n"
    "Package integrity â€” not optional:\n"
    ...
    "   â€¢ keep it as a standalone skill, OR\n"
    "   â€¢ fully merge it by re-homing every needed support file into the "
    "umbrella's canonical `references/`, `templates/`, `scripts/`, or "
    "`assets/` directories AND rewrite the destination instructions to "
    "the new paths, OR\n"
    "   â€¢ archive the entire original skill package unchanged.\n"
    ...
    "merges.\n\n"
    "Your toolset:\n"
    "  - skills_list, skill_view        â€” read the current landscape\n"
    "  - skill_manage action=patch      â€” add sections to the umbrella\n"
    "  - skill_manage action=create     â€” create a new umbrella SKILL.md\n"
    "  - skill_manage action=write_file â€” add a references/, templates/, "
    "or scripts/ file under an existing skill (the skill must already "
    "exist)\n"
    "  - skill_manage action=delete     â€” archive a skill. MUST pass "
    "`absorbed_into=<umbrella>` when you've merged its content into another "
    "skill, or `absorbed_into=\"\"` when you're truly pruning with no "
    "forwarding target. This drives cron-job skill-reference migration â€” "
    "guessing from your YAML summary after the fact is fragile.\n"
    "  - terminal                       â€” mv a sibling into the archive "
    "OR move its content into a support subfile\n\n"
    "'keep' is a legitimate decision ONLY when the skill is already a "
    "class-level umbrella and none of the proposed merges would improve "
    "discoverability. 'This is narrow but distinct from its siblings' "
    "is NOT a reason to keep â€” it's a reason to move it under an "
    ...
    "stopped too early â€” go back and look at the clusters you left "
    "alone.\n\n"
    ...
    "    reason: <one short sentence â€” why merged, not just 'similar'>\n"
    "prunings:\n"
    "  - name: <skill-name>\n"
    "    reason: <one short sentence â€” why archived with no merge target>\n"
    "```\n\n"
    ...
    "no absorption â€” truly stale, irrelevant, or obsolete â€” X goes under "
...
# ---------------------------------------------------------------------------
# Per-run reports â€” {YYYYMMDD-HHMMSS}/run.json + REPORT.md under logs/curator/
# ---------------------------------------------------------------------------

def _reports_root() -> Path:
    """Directory where curator run reports are written.

    Lives under the profile-aware logs dir (``~/.zed/logs/curator/``)
    ...
    authored skill data in ``~/.zed/skills/``.

    ``ensure_zed_home()`` pre-creates this dir on every CLI launch and
    the v22â†’v23 migration backfills it for existing profiles, but we
    ...
    that bypasses both.
    """
    root = get_zed_home() / "logs" / "curator"
    try:
...
def _classify_removed_skills(
    ...
    into another skill (an umbrella) during this run â€” the content still
    ...
    for tc in tool_calls or []:
        ...
        if isinstance(raw, dict):
            ...
            try:
                ...
                # Truncated or malformed â€” fall back to substring match on
    ...
    for name in removed:
        ...
        for args in parsed_calls:
            ...
            if target == name:
                continue
            # The target must be a surviving or newly-created skill â€”
            ...
            #   file_path â€” needle must be a complete path component
            #     (filename stem or directory name), so "api" does NOT
            #     falsely match "references/api-design.md".
            #   content fields â€” word-boundary regex so "test" does NOT
...
def _parse_structured_summary(
    ...
    - Missing block â†’ returns empty lists (we'll fall back to heuristic).
    - Malformed YAML â†’ returns empty lists and we rely on heuristic.
    - Partial block (e.g. only consolidations) â†’ returns what we could parse.
    ...
    # Prefer PyYAML when available â€” every zed install already has it
...
def _extract_absorbed_into_declarations(
    ...
    signal for classification â€” the model's own declaration at the moment of
    ...
    ``absorbed_into``, are not in the returned dict â€” caller falls back to
...
def _reconcile_classification(
    ...
    - **Model-declared `absorbed_into` at delete time is authoritative.** Any
      ...
      ``into != ""`` and target exists â†’ consolidated. ``into == ""`` â†’
      pruned. ``into != ""`` but target doesn't exist â†’ hallucination; fall
    ...
    - Model-declared pruning is accepted unless the heuristic has
      tool-call evidence that contradicts it (rare â€” the heuristic would
    ...
    for name in removed:
        ...
        if dec is not None:
            ...
            # in practice â€” but if it slips through (e.g. the umbrella was
            # deleted LATER in the same run), fall through to the usual
            # signals rather than trusting a broken reference.

        # Model says consolidated â€” trust it if the destination is real.
        ...
        # Model says consolidated but the umbrella doesn't exist â€”
...
def _build_rename_summary(
    *,
    ...
    nothing was archived this run â€” most ticks are no-op and shouldn't add
    extra log noise.

    Format::

        archived 4 skill(s):
          â€¢ pdf-extraction â†’ document-tools
          â€¢ docx-extraction â†’ document-tools
          â€¢ flaky-thing â€” pruned (stale)
          â€¢ old-utility â†’ spreadsheet-ops
        full report: zed curator status
        keep an umbrella stable: zed curator pin document-tools
    ...
    for entry in consolidated:
        ...
        name = entry.get("name", "?")
        into = entry.get("into", "?")
        lines.append(f"  â€¢ {name} â†’ {into}")
        shown += 1
    for entry in pruned:
        if shown >= SHOW:
            break
        name = entry.get("name", "?") if isinstance(entry, dict) else str(entry)
        lines.append(f"  â€¢ {name} â€” pruned (stale)")
        shown += 1
    if total > SHOW:
        lines.append(f"  â€¦ and {total - SHOW} more")
    lines.append("full report: zed curator status")
    # Pin hint â€” only surface it when there's actually a destination skill
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: zed curator pin {example}"
            )
    return "\n".join(lines)
...
def _write_run_report(
    *,
    ...
    couldn't happen (caller logs and continues â€” reporting is best-effort).
    """
    root = _reports_root()
    try:
        root.mkdir(parents=True, exist_ok=True)
    ...
    # the substring heuristic â€” the model is telling us directly, at the
    ...
    # it at run time â€” the scheduler skips it and the job runs without
    ...
    # run.json â€” machine-readable, full fidelity
    try:
    ...
    # cron_rewrites.json â€” only when at least one job was touched, to
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    lines.append(f"# Curator run â€” {started}\n")
    model = p.get("model") or "(not resolved)"
    prov = p.get("provider") or "(not resolved)"
    counts = p.get("counts") or {}
    lines.append(
        f"Model: `{model}` via `{prov}`  Â·  Duration: {dur_label}  Â·  "
        f"Agent-created skills: {counts.get('before', 0)} â†’ {counts.get('after', 0)} "
        f"({counts.get('delta', 0):+d})\n"
    )

    error = p.get("llm_error")
    if error:
        lines.append(f"> âš  LLM pass error: `{error}`\n")
    ...
    lines.append(f"- state transitions (active â†” stale â†” archived): "
                 f"**{counts.get('state_transitions', 0)}**")
    lines.append("")

    # Consolidated list â€” content absorbed into an umbrella. The directory
    # on disk still lives under ~/.zed/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            "_These skills were **absorbed into another skill** during this run â€” "
            "their content still lives, just under a different name. "
            "The original directory was moved to `~/.zed/skills/.archive/` for "
            "safety and can be restored via `zed curator restore <name>` if the "
        ...
        if len(consolidated) > SHOW:
            lines.append(f"- â€¦ and {len(consolidated) - SHOW} more (see `run.json`)")
        lines.append("")

    # Pruned list â€” archived without consolidation. These are the
    # "stale skill pruned" cases the UI should mark clearly.
    pruned = p.get("pruned") or []
    if pruned:
        lines.append(f"### Pruned â€” archived for staleness ({len(pruned)})\n")
        lines.append(
            "_These skills were archived without being merged into an umbrella "
            "(e.g. stale, unused, or judged irrelevant). "
            "Directories live under `~/.zed/skills/.archive/`. "
            "Restore any via `zed curator restore <name>`._\n"
        )
        SHOW = 50
        ...
        if len(pruned) > SHOW:
            lines.append(f"- â€¦ and {len(pruned) - SHOW} more (see `run.json`)")
        lines.append("")
    ...
    if trans:
        ...
        for t in trans:
            lines.append(f"- `{t.get('name')}`: {t.get('from')} â†’ {t.get('to')}")
        lines.append("")

    # Cron job rewrites â€” show which scheduled jobs had their skill
    ...
    if cron_rewrites_list:
        ...
        for entry in cron_rewrites_list[:SHOW]:
            ...
            lines.append(
                f"- `{job_name}`: `{', '.join(before)}` â†’ `{', '.join(after) or '(none)'}`"
            )
            for old, new in mapped.items():
                lines.append(f"    - `{old}` â†’ `{new}` (consolidated)")
            for name in dropped:
                lines.append(f"    - `{name}` dropped (pruned)")
        if len(cron_rewrites_list) > SHOW:
            lines.append(
                f"- â€¦ and {len(cron_rewrites_list) - SHOW} more "
    ...
    lines.append("- Restore an archived skill: `zed curator restore <name>`")
    lines.append("- All archives live under `~/.zed/skills/.archive/` and are recoverable by `mv`")
...
# ---------------------------------------------------------------------------
# Orchestrator â€” spawn a forked AIAgent for the LLM review pass
# ---------------------------------------------------------------------------
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``True``/``False`` overrides the config for this invocation â€” used by the
    ``zed curator run --consolidate`` flag. When consolidation is off, only
    ...
    and the LLM review pass is instructed to produce a report only â€” no
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    if dry_run:
        ...
        # Pre-mutation snapshot â€” best-effort, never blocks the run. A
    ...
    # last_run_at or run_count â€” a preview shouldn't push the next scheduled
    # real pass out. We still record a summary so `zed curator status`
    ...
    def _llm_pass():
        ...
        # deterministic inactivity prune above â€” no forked aux-model review, no
        ...
        try:
            ...
            if "No agent-created skills" in candidate_list:
                ...
                # bundled" rule for them â€” but only archiving is permitted, and
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `zed update` only if "
        ...
        # Append the rename map (`old-name â†’ umbrella`) to the user-visible
        ...
        # recorded in state so `zed curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_runtime(cfg: Dict[str, Any]) -> _ReviewRuntimeBinding:
    ...
    if _legacy_provider and _legacy_model:
        logger.info(
            ...
            "config â€” please migrate to auxiliary.curator.{provider,model}"
        )
        return _ReviewRuntimeBinding(
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    Curator is a regular auxiliary task slot â€” ``auxiliary.curator.{provider,model}``
    â€” so it participates in the canonical aux-model plumbing (``zed model`` â†’
    ...
    means "use the main chat model" â€” same default as every other aux task.
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `zed model` â†’ auxiliary
    ...
    try:
        review_agent = AIAgent(
            ...
            # high iteration ceiling â€” the pass typically takes 50-100
        ...
        # Disable recursive nudges â€” the curator must never spawn its own review.
        ...
        result_meta["summary"] = (final[:240] + "â€¦") if len(final) > 240 else (final or "no change")
