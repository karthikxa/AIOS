"""Models.dev registry integration â€” primary database for providers and models.

Fetches from https://models.dev/api.json â€” a community-maintained database
...
Data resolution order (like TypeScript OpenCode):
  1. Bundled snapshot (ships with the package â€” offline-first)
  2. Disk cache (~/.zed/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Dataclasses â€” rich metadata for providers and models
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Zed â†” models.dev
# ---------------------------------------------------------------------------

# Zed provider names â†’ models.dev provider IDs
...
# Reverse mapping: models.dev â†’ Zed (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_zed_home() / "models_dev_cache.json"
...
def _disk_cache_age_seconds() -> Optional[float]:
    ...
    denied, weird filesystem) all return None â€” callers fall through
    ...
    try:
        ...
        # or system clock reset). Treat as "unknown freshness" â†’ fall
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    """Fetch models.dev registry. Cache hierarchy: in-mem â†’ disk â†’ network.

    Returns the full registry dict keyed by provider ID, or empty dict on failure.

    Cache hierarchy (when ``force_refresh=False``):
      1. In-memory cache, populated and < TTL old â†’ return immediately.
      2. **Disk cache file < TTL old by mtime â†’ load, populate in-mem, return.**
      ...
      3. Network fetch â†’ on success, save to disk + in-mem and return.
      4. Network fails â†’ fall back to ANY available disk cache (even stale)
         with a short 5 min in-mem grace period before retrying network.

    When ``force_refresh=True`` (used by ``zed config refresh``, the
    ...
    # long-lived processes â€” no I/O, no system calls.
    if (
        not force_refresh
    ...
    # Stage 4: network failed â€” fall back to whatever disk cache exists,
...
def lookup_models_dev_context(provider: str, model: str) -> Optional[int]:
    ...
    # reporting 32768 â€” tripping the 64k minimum-context guard.
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Zed provider ID to its models dict from models.dev.
...
def get_model_capabilities(provider: str, model: str) -> Optional[ModelCapabilities]:
    ...
    Extracts from model entry fields:
      - reasoning  (bool)  â†’ supports_reasoning
      - tool_call  (bool)  â†’ supports_tools
      - attachment (bool)  â†’ supports_vision
      - limit.context (int) â†’ context_window
      - limit.output  (int) â†’ max_output_tokens
      - family     (str)   â†’ model_family
    """
...
def list_provider_models(provider: str) -> List[str]:
    ...
    """
    from zed_cli.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal Zed agent traffic.
...
# ---------------------------------------------------------------------------
# Rich dataclass constructors â€” parse raw models.dev JSON into dataclasses
# ---------------------------------------------------------------------------
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Zed provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Zed ID â†’ models.dev ID
...
def get_model_info(
    ...
    Accepts Zed or models.dev provider ID.  Tries exact match then
