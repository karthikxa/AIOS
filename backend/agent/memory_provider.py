...
Lifecycle (called by MemoryManager, wired in run_agent.py):
  initialize()          â€” connect, create resources, warm up
  system_prompt_block()  â€” static text for the system prompt
  prefetch(query)        â€” background recall before each turn
  sync_turn(user, asst)  â€” async write after each turn
  get_tool_schemas()     â€” tool schemas to expose to the model
  handle_tool_call()     â€” dispatch a tool call
  shutdown()             â€” clean exit

Optional hooks (override to opt in):
  on_turn_start(turn, message, **kwargs) â€” per-turn tick with runtime context
  on_session_end(messages)               â€” end-of-session extraction
  on_session_switch(new_session_id, **kwargs) â€” mid-process session_id rotation
  on_pre_compress(messages) -> str       â€” extract before context compression
  on_memory_write(action, target, content, metadata=None) â€” mirror built-in memory writes
  on_delegation(task, result, **kwargs)  â€” parent-side observation of subagent work
"""
...
class MemoryProvider(ABC):
    ...
    def is_available(self) -> bool:
        ...
        Should not make network calls â€” just check config and installed deps.
        """
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - zed_home (str): The active ZED_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.zed``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "zed").
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        should be fast â€” use background threads for the actual recall
    ...
    def queue_prefetch(self, query: str, *, session_id: str = "") -> None:
        ...
        by prefetch() on the next turn. Default is no-op â€” providers
    ...
    def sync_turn(
        ...
        *,
        ...
        Called after each turn. Should be non-blocking â€” queue for
    ...
    def on_session_end(self, messages: List[Dict[str, Any]]) -> None:
        ...
        NOT called after every turn â€” only at actual session boundaries
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        gateway equivalents, and context compression â€” any path that
        ...
        parent_session_id:
            The previous session_id, if meaningful â€” set for ``/branch``
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'zed memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], zed_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'zed memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``zed_home`` is the active ZED_HOME directory path.
