...
def _zed_home_path() -> Path:
    """Resolve the active ZED_HOME (profile-aware) without circular imports."""
    try:
        from zed_constants import get_zed_home  # local import to avoid cycles
        return get_zed_home()
    except Exception:
        return Path(os.path.expanduser("~/.zed"))


def _zed_root_path() -> Path:
    """Resolve the Zed root dir (always the parent of any profile, never per-profile)."""
    try:
        from zed_constants import get_default_zed_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    zed_home = _zed_home_path()
    zed_root = _zed_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(zed_home / ".env"),
            # Top-level .env, even when running under a profile â€” overwriting it
            # leaks credentials across every profile that inherits from root (#15981).
            str(zed_root / ".env"),
            # Active profile Anthropic PKCE credential store.
            str(zed_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(zed_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved ZED_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("ZED_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    zed_dirs = []
    for base in (_zed_home_path(), _zed_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in zed_dirs:
                zed_dirs.append(real)
        except Exception:
            continue

    for base_real in zed_dirs:
        try:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Zed path.

    Three categories are blocked:

      * Internal Zed cache files under ``ZED_HOME/skills/.hub`` â€”
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under ZED_HOME and the global Zed
        root: ``auth.json``, ``auth.lock``, ``.anthropic_oauth.json``,
        ``.env``, ``webhook_subscriptions.json``, ``auth/google_oauth.json``,
        and anything under ``mcp-tokens/``. These hold plaintext provider keys,
        OAuth tokens, and HMAC secrets that the agent never needs to read
        directly â€” provider tools / gateway adapters consume them through
        internal channels.
      * Project-local environment files anywhere on disk: ``.env``,
        ...
        need to read these â€” ``.env.example`` is the documented-shape
        substitute.

    **This is NOT a security boundary.** The terminal tool runs as the
    same OS user with shell access; the agent can still ``cat auth.json``
    or ``cat ~/.zed/.env`` and exfiltrate the file. The read-deny exists
    as defense-in-depth that:
      ...
      * Surfaces a visible audit trail when something tries to read
        credentials â€” easier to spot in logs than a generic ``cat``.
    ...
    # Resolve BOTH the active ZED_HOME (profile-aware) AND the global
    # Zed root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (ZED_HOME points at
    ...
    for hd in zed_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Zed cache file "
    ...
    for hd in zed_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Zed credential store "
                    "and cannot be read directly. Provider tools consume "
                    "these credentials through internal channels. "
                    "(Defense-in-depth â€” not a security boundary; the "
                    "terminal tool can still bypass.)"
                )

    # mcp-tokens/: directory prefix match â€” anything inside is OAuth
    # token material.
    for hd in zed_dirs:
        try:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Zed MCP token directory "
                "and cannot be read directly. (Defense-in-depth â€” not a "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Zed MCP token file "
            "and cannot be read directly. (Defense-in-depth â€” not a "
            "security boundary; the terminal tool can still bypass.)"
        )
    ...
    # .env contents â€” .env.example is the documented-shape substitute. The
    ...
    if resolved.name in _BLOCKED_PROJECT_ENV_BASENAMES:
        return (
            ...
            "(Defense-in-depth â€” not a security boundary; the terminal tool can still bypass.)"
        )
...
#
# Zed profiles are separate ZED_HOME dirs under
...
# ANOTHER profile's directories is almost always wrong â€” those skills /
...
# as the dangerous-command approval flow â€” the agent is told the boundary
# exists, and explicit user direction is required to cross it.
#
# Reference: May 2026 incident where a zed-security profile session
# edited skills under both ``~/.zed/profiles/zed-security/skills/``
# AND ``~/.zed/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under ZED_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from ZED_HOME.
    ...
    """
    try:
        home_real = _zed_home_path().resolve()
        root_real = _zed_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Zed scope, or is inside
    ...
    The caller decides what to do with the result â€” surface a warning to
    ...
    if parts[0] in PROFILE_SCOPED_AREAS:
        ...
        # ``<root>/profiles/<name>/<area>/...`` â†’ named profile.
    ...
    if target_profile == active_profile:
        # In-profile write â€” not a cross-profile event.
        return None
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Zed entirely. Caller is expected to surface the warning to the
    agent as a tool-result error, NOT to silently allow the write â€” the
    ...
    return (
        ...
        f"belongs to Zed profile {info['target_profile']!r}, but the "
        ...
        f"``cross_profile=True``. (Defense-in-depth â€” not a security "
...
#
#   <ZED_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.zed/...
#
...
# mirror â€” never read by the host process â€” while the host file is left
...
# ``â€¦/sandboxes/<backend>/<task>/home/.zed/â€¦`` segment and warns
# regardless of which Zed profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.zed/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.zed`` part in a sandbox-mirror path.

    Matches ``â€¦/sandboxes/<backend>/<task>/home/.zed/â€¦`` and returns the
    index where the inner Zed-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .zed / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".zed":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Zed state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``â€¦/sandboxes/<backend>/<task>/home/.zed``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.zed`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only â€” does not require any Zed resolver to
    succeed, so it works correctly even when called from contexts where
    ZED_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Zed process never "
        f"reads â€” the authoritative file is likely {info['inner_path']!r} "
        f"under the real ZED_HOME. Use the host-side tool for "
        ...
        f"(Defense-in-depth â€” not a security boundary; the terminal tool "
        f"can still bypass.)"
    )


# ---------------------------------------------------------------------------
# Container-context mirror guard (inner-container case â€” #32049 follow-up)
#
# Brian's shape-based detector (#32213) catches paths that still carry the
# full ``â€¦/sandboxes/<backend>/<task>/home/.zed/â€¦`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.zed/â€¦``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host ZED_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Zed state.
    ...
    return (
        ...
        f"bind-mounted home â€” a per-task mirror that the host Zed "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real ZED_HOME. Use the "
