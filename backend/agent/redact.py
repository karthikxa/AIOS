...
# Ported from nearai/ironclaw#2529 â€” catches tokens whose values don't match
...
# Exact match, NOT substring â€” "token_count" and "session_id" must NOT match.
...
# `export ZED_REDACT_SECRETS=false`) cannot disable redaction
# mid-session.  ON by default â€” secure default per issue #17691. Users who
# need raw credential values in tool output (e.g. working on the redactor
# itself) can opt out via `security.redact_secrets: false` in config.yaml
# (bridged to this env var in zed_cli/main.py, gateway/run.py, and
# cli.py) or `ZED_REDACT_SECRETS=false` in ~/.zed/.env. An opt-out
# warning is logged at gateway and CLI startup so operators see the
# downgrade â€” see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("ZED_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}
...
# JWT tokens: header.payload[.signature] â€” always start with "eyJ" (base64 for "{")
...
# URLs containing query strings â€” matches `scheme://...?...[# or end]`.
...
# URLs containing userinfo â€” `scheme://user:password@host` for ANY scheme
...
# Form-urlencoded body detection: conservative â€” only applies when the entire
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across Zed â€” used by
    ``zed config``, ``zed status``, ``zed dump``, and anywhere
    ...
    Args:
        ...
        floor:       Values shorter than ``head + tail + floor_margin`` are
                     fully masked (returns ``placeholder``). Default 12 â€”
...
def _mask_token(token: str) -> str:
    """Mask a log token â€” conservative 18-char floor, preserves 6 prefix / 4 suffix."""
...
def _redact_url_query_params(text: str) -> str:
    ...
    `https://example.com/cb?code=ABC123&state=xyz` â†’ `...?code=***&state=xyz`.
    """
...
def _redact_form_body(text: str) -> str:
    ...
    text passes through unchanged. This is a conservative pass â€” the
...
def redact_sensitive_text(text: str, *, force: bool = False, code_file: bool = False) -> str:
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical zed log line
    (no secrets) this drops the 13-pattern scan from ~5.6us to ~1.8us per
    record (-68%). The pre-checks are conservative â€” false positives
    ...
    # Known prefixes (sk-, ghp_, etc.) â€” gate on substring presence
    if _has_known_prefix_substring(text):
        text = _PREFIX_RE.sub(lambda m: _mask_token(m.group(1)), text)

    # ENV assignments: OPENAI_API_KEY=***  (skip for code files â€” false positives)
    if not code_file:
        if "=" in text:
            def _redact_env(m):
        ...
        # JSON fields: "apiKey": "***"  (skip for code files â€” false positives)
    ...
    # Authorization headers â€” _AUTH_HEADER_RE is "Authorization: Bearer ..."
    ...
    # Telegram bot tokens â€” pattern requires ":<token>" with digits prefix
    if ":" in text:
    ...
    # JWT tokens (eyJ... â€” base64-encoded JSON headers)
    if "eyJ" in text:
    ...
    # opaque tokens through query strings â€” magic-link checkouts, OAuth
    # callbacks the agent is meant to follow, pre-signed share URLs â€” and
...
# nothing) â€” the bound is "no false negatives" and that holds because every
