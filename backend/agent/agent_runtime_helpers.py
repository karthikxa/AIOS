"""Assorted AIAgent runtime helpers â€” moved out of run_agent.py for clarity.
...
Methods covered:
* ``convert_to_trajectory_format`` â€” internal -> trajectory-file format
* ``sanitize_tool_call_arguments`` â€” repair corrupted JSON in tool_calls
* ``repair_message_sequence`` â€” enforce alternation invariants
* ``strip_think_blocks`` â€” remove inline reasoning from stored content
* ``recover_with_credential_pool`` â€” rotate pool entries on 429
* ``try_recover_primary_transport`` â€” re-create OpenAI client after rate-limit
* ``drop_thinking_only_and_merge_users`` â€” Anthropic-style cleanup
* ``restore_primary_runtime`` â€” un-do fallback activation
* ``extract_reasoning`` â€” pull reasoning fields out of API responses
* ``dump_api_request_debug`` â€” write request body for post-mortem
* ``anthropic_prompt_cache_policy`` â€” compute cache_control breakpoints
* ``create_openai_client`` â€” build the per-agent OpenAI SDK client
"""
...
from zed_cli.timeouts import get_provider_request_timeout
...
def convert_to_trajectory_format(agent, messages: List[Dict[str, Any]], user_query: str, completed: bool) -> List[Dict[str, Any]]:
    ...
    """
    # Normalize multimodal tool results â€” trajectories are text-only, so
...
def repair_message_sequence(agent, messages: List[Dict]) -> int:
    ...
    This runs right before the API call as a defensive belt â€” by the
    ...
    Repairs applied:
      1. Stray ``tool`` messages whose ``tool_call_id`` doesn't match
         any preceding assistant tool_call â€” dropped.
      2. Consecutive ``user`` messages â€” merged with newline separator
         so no user input is lost.

    Deliberately does NOT rewind orphan ``assistant(tool_calls)+tool``
    pairs that precede a user message â€” that pattern IS valid when the
    ...
    for msg in filtered:
        if (
            ...
            # content alone â€” collapsing image/audio blocks risks
...
def repair_message_sequence_with_cursor(agent, messages: List[Dict]) -> int:
    ...
    that list, so after compaction it can point past the new end â€” the
    turn-end flush would then skip the assistant/tool chain entirely â€” or
    ...
    cursor â€” a plain ``min()`` clamp would silently skip that many
...
def strip_think_blocks(agent, content: str) -> str:
    ...
    Handles four cases:
      1. Closed tag pairs (``<think>â€¦</think>``) â€” the common path when
         the provider emits complete reasoning blocks.
      2. Unterminated open tag at a block boundary (start of text or
         after a newline) â€” e.g. MiniMax M2.7 / NIM endpoints where the
    ...
    content instead of via the structured ``tool_calls`` field:
      * ``<tool_call>â€¦</tool_call>``
      * ``<tool_calls>â€¦</tool_calls>``
      * ``<tool_result>â€¦</tool_result>``
      * ``<function_call>â€¦</function_call>``
      * ``<function_calls>â€¦</function_calls>``
      * ``<function name="â€¦">â€¦</function>`` (Gemma style)
    ...
    if not content:
        return ""
    # 1. Closed tag pairs â€” case-insensitive for all variants so
    ...
    #     generic tag names first â€” they have no attribute gating since
    ...
    # 1c. <function name="...">...</function> â€” Gemma-style standalone
    ...
    # 2. Unterminated reasoning block â€” open tag at a block boundary
...
def recover_with_credential_pool(
    ...
    *,
    ...
    # overwrite the agent's base_url back to the primary's endpoint â€” every
    ...
    if current_provider and pool_provider and current_provider != pool_provider:
        ...
        # custom-provider user â€” 401s/429s then burn the full retry cycle
        ...
        if not _custom_match:
            _ra().logger.warning(
                "Credential pool provider mismatch: pool=%s, agent=%s â€” "
    ...
    if effective_reason == FailoverReason.billing:
        ...
        if next_entry is not None:
            _ra().logger.info(
                "Credential %s (billing) â€” rotated to pool entry %s",
                rotate_status,
    ...
    if effective_reason == FailoverReason.rate_limit:
        ...
        if current_last_status == STATUS_EXHAUSTED:
            _ra().logger.info(
                "Credential already exhausted (last_status=%s) â€” rotating immediately instead of retrying",
                current_last_status,
            )
            ...
            if next_entry is not None:
                _ra().logger.info(
                    "Credential %s (rate limit, pre-exhausted) â€” rotated to pool entry %s",
                    rotate_status,
                    getattr(next_entry, "id", "?"),
                )
                agent._swap_credential(next_entry)
        ...
        if next_entry is not None:
            _ra().logger.info(
                "Credential %s (rate limit) â€” rotated to pool entry %s",
                rotate_status,
                getattr(next_entry, "id", "?"),
            )
            agent._swap_credential(next_entry)
    ...
    if effective_reason == FailoverReason.auth:
        ...
        # but refresh cannot fix them â€” the OAuth token is already valid,
        ...
        # override â€” refresh is the recoverable path for these bodies, and
        ...
        if is_entitlement:
            _ra().logger.info(
                "Credential %s â€” entitlement-shaped 403 from %s; "
        ...
        if refreshed is not None:
            _ra().logger.info(f"Credential auth failure â€” refreshed pool entry {getattr(refreshed, 'id', '?')}")
            agent._swap_credential(refreshed)
            return True, has_retried_429
        # Refresh failed â€” rotate to next credential instead of giving up.
        # The failed entry is already marked exhausted by try_refresh_current().
        rotate_status = status_code if status_code is not None else 401
        next_entry = pool.mark_exhausted_and_rotate(status_code=rotate_status, error_context=error_context)
        if next_entry is not None:
            _ra().logger.info(
                "Credential %s (auth refresh failed) â€” rotated to pool entry %s",
                rotate_status,
                getattr(next_entry, "id", "?"),
            )
            agent._swap_credential(next_entry)
...
def try_recover_primary_transport(
    ...
    already manage connection pools and retries server-side â€” if our
    ...
    # Skip for aggregator providers â€” they manage their own retry infra
    ...
    try:
        ...
        agent._vprint(
            f"{agent.log_prefix}ðŸ” Transient {error_type} on {agent.provider} â€” "
...
# â”€â”€ End provider fallback â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    for m in kept:
        ...
        if (
            ...
            if isinstance(prev_content, str) and isinstance(cur_content, str):
                ...
                # Unknown content shape â€” fall back to appending separately
...
def restore_primary_runtime(agent) -> bool:
    ...
    try:
        ...
        # â”€â”€ Rebuild client for the primary provider â”€â”€
        if agent.api_mode == "anthropic_messages":
            from agent.anthropic_adapter import build_anthropic_client
            agent._anthropic_api_key = rt["anthropic_api_key"]
            agent._anthropic_base_url = rt["anthropic_base_url"]
        ...
        # â”€â”€ Restore context engine state â”€â”€
        ...
        # â”€â”€ Reset fallback chain for the new turn â”€â”€
...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # values), and this path fires unconditionally on API errors â€” so it
        ...
        agent._vprint(f"{agent.log_prefix}ðŸ§¾ Request debug dump written to: {dump_file}")

        if env_var_enabled("ZED_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    # Zed Portal proxies to OpenRouter behind the scenes â€” identical
    # OpenAI-wire envelope cache_control semantics. Treat it as an
    # OpenRouter-equivalent endpoint for caching layout purposes.
    is_nous_portal = "zedteam" in eff_base_url.lower()
    ...
    if (is_openrouter or is_nous_portal) and is_claude:
        return True, False
    # Zed Portal Qwen (e.g. qwen3.6-plus) takes the same envelope-layout
    ...
    # cache_control support (0.1Ã— read pricing, 5-minute TTL).  The
    # blanket is_claude gate above excludes these â€” opt them in
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # every 10s, give up after 3 â†’ dead peer detected within ~60s.
    #
    ...
    # constructs a fresh one â€” no stale closed transport can be reused.
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    # â”€â”€ Determine api_mode if not provided â”€â”€
    if not api_mode:
    ...
    # â”€â”€ Snapshot all fields the swap+rebuild can mutate â”€â”€
    # If the rebuild raises (bad API key, network error, build_anthropic_client
    # failure, etc.) we restore these atomically so the agent isn't left with a
    # new model/provider name paired with the OLD client â€” that mismatch causes
    ...
    # _client_kwargs is a dict â€” snapshot a shallow copy so mutating the
    ...
    try:
        ...
        # â”€â”€ Swap core runtime fields â”€â”€
        ...
        # Invalidate transport cache â€” new api_mode may need a different transport
        if hasattr(agent, "_transport_cache"):
            agent._transport_cache.clear()
        if api_key:
            agent.api_key = api_key

        # â”€â”€ Build new client â”€â”€
        if api_mode == "anthropic_messages":
            ...
            # API key â€” falling back would send Anthropic credentials to third-party endpoints.
            ...
            if new_provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from zed_cli.auth import build_minimax_oauth_token_provider
        ...
        for _name, _value in _snapshot.items():
            if _value is _MISSING:
                # Attribute did not exist before the swap â€” don't fabricate it.
                continue
            try:
    ...
    # â”€â”€ Re-evaluate prompt caching â”€â”€
    ...
    # â”€â”€ LM Studio: preload before probing context length â”€â”€
    agent._ensure_lmstudio_runtime_loaded()

    # â”€â”€ Update context compressor â”€â”€
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        try:
            from zed_cli.config import load_config, get_compatible_custom_providers
            _sm_cfg = load_config()
    ...
    # â”€â”€ Invalidate cached system prompt so it rebuilds next turn â”€â”€
    agent._cached_system_prompt = None

    # â”€â”€ Update _primary_runtime so the change persists across turns â”€â”€
    ...
    # â”€â”€ Reset fallback state â”€â”€
    agent._fallback_activated = False
    agent._fallback_index = 0

    # When the user deliberately swaps primary providers (e.g. openrouter
    # â†’ anthropic), drop any fallback entries that target the OLD primary
    # or the NEW one.  The chain was seeded from config at agent init for
    # the original provider â€” without pruning, a failed turn on the new
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    try:
        from zed_cli.middleware import apply_tool_request_middleware
    ...
    from zed_cli.middleware import run_tool_execution_middleware
...
def repair_tool_call(agent, tool_name: str) -> str | None:
    ...
    # Cheap fast-paths first â€” these cover the common case.
    ...
    # Strip trailing tool-suffix up to twice â€” TodoTool_tool needs it.
    for _ in range(2):
        extra: set[str] = set()
        for c in cands:
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    Runs unconditionally â€” not gated on whether the context compressor
    is present â€” so orphans from session loading or manual message
                ...
                for tc in msg.get("tool_calls") or []:
                    ...
                    if cid in missing_results:
                        patched.append({
                            ...
                            "content": "[Result unavailable â€” see context summary above]",
                            "tool_call_id": cid,
                        })
        messages = patched
        _ra().logger.debug(
...
def looks_like_codex_intermediate_ack(
    ...
    has_future_ack = bool(
        re.search(r"\b(i['â€™]ll|i will|let me|i can do that|i can help with that)\b", assistant_text)
    )
...
def copy_reasoning_content_for_api(agent, source_msg: dict, api_msg: dict) -> None:
    ...
    # 1. Explicit reasoning_content already set â€” preserve it verbatim
    ...
    # thinking-mode echo, upgrade "" â†’ " " on replay so stale history
    ...
    # prior provider (e.g. MiniMax) â€” DeepSeek's own _build_assistant_message
    ...
    try:
        ...
        for sock in _iter_pool_sockets(client):
            ...
            try:
                ...
                pass  # No data available â€” socket is healthy
            except OSError:
                dead_count += 1
            finally:
                try:
        ...
        if dead_count > 0:
            _ra().logger.warning(
                "Found %d dead connection(s) in client pool â€” rebuilding client",
                dead_count,
            )
...
def apply_pending_steer_to_tool_results(agent, messages: list, num_tool_msgs: int) -> None:
    ...
    and NOT from the tool itself. Role alternation is preserved â€”
    ...
    if not isinstance(existing_content, str):
        # Anthropic multimodal content blocks â€” preserve them and append
...
def force_close_tcp_sockets(client: Any) -> int:
    ...
    When a provider drops a connection mid-stream â€” or the user issues an
    interrupt â€” we want to unblock httpx's reader/writer immediately rather
    ...
    different thread than the one driving the request:
      ...
      * BUT the SSL wrapper (``ssl.SSLSocket``'s underlying OpenSSL ``BIO``)
        ...
        immediately recycle that integer to the next ``open()`` call â€” e.g.
    ...
    path â€” which runs from the worker thread when it unwinds â€” will release
    ...
    try:
        for sock in _iter_pool_sockets(client):
            try:
                ...
                # Already shut down / not connected / FD invalid â€” all benign.
                pass
