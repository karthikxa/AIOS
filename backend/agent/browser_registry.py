...
# Legacy auto-detect order â€” used when no ``browser.cloud_provider`` is set.
...
def _resolve(configured: Optional[str]) -> Optional[BrowserProvider]:
    ...
    plugins added under ``~/.zed/plugins/browser/<vendor>/`` are subject
    ...
    def _is_available_safe(p: BrowserProvider) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Browser provider %s.is_available() raised %s â€” treating as unavailable",
    ...
    # 2. Explicit config wins â€” return regardless of is_available() so the
    ...
    # 3. Legacy preference walk â€” only providers in _LEGACY_PREFERENCE are
