...
(3 SDK retries x 3 Zed retries), and every one of those calls counts
...
def _state_path() -> str:
    ...
    try:
        ...
        base = os.path.join(os.path.expanduser("~"), ".zed")
...
def _parse_reset_seconds(headers: Optional[Mapping[str, str]]) -> Optional[float]:
    ...
    Priority:
      1. x-ratelimit-reset-requests-1h  (hourly RPH window â€” most useful)
    ...
    *,
    ...
    MiMo, Zed, ...) behind one endpoint.  A 429 can mean either:
      ...
      (a) The caller's own RPM / RPH / TPM / TPH bucket on Nous is
          exhausted â€” a genuine rate limit that will last until the
          bucket resets.
      (b) The upstream provider is out of capacity for a specific model
          â€” transient, clears in seconds, and has nothing to do with
