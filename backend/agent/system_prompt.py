...
turns â€” only context compression triggers a rebuild.  This keeps the
upstream prefix cache warm.  See ``zed-agent-dev``'s
...
* ``stable``   â€” identity (SOUL.md or DEFAULT_AGENT_IDENTITY), tool
  guidance, computer-use guidance, Zed subscription block, tool-use
  enforcement guidance + per-model operational guidance, skills prompt,
  alibaba model-name workaround, environment hints, platform hints.
* ``context``  â€” caller-supplied ``system_message`` plus context files
  (AGENTS.md / .cursorrules / etc.) discovered under ``TERMINAL_CWD``.
* ``volatile`` â€” memory snapshot, USER.md profile, external memory
...
def _resolve_platform_hint(agent: Any, platform_key: str, default_hint: str) -> str:
    ...
    effective hint for *platform_key*:
      ...
      * ``replace`` â€” substitute the default hint entirely.
      * ``append``  â€” keep the default and append the extra text.
      * a bare string value â€” treated as ``append`` (convenience shorthand).

    Precedence: ``replace`` wins over ``append`` if both are present.
    Override text is added on top of (not instead of) the SOUL/context/
    memory tiers â€” it only affects the platform-hint segment, so other
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    Returns a dict with three keys:
      * ``stable``   â€” identity, tool guidance, skills prompt,
        environment hints, platform hints, model-family operational
        guidance.
      * ``context``  â€” context files (AGENTS.md, .cursorrules, etc.)
        and caller-supplied system_message.
      * ``volatile`` â€” memory snapshot, user profile, external
        memory provider block, timestamp line.

    Joined into a single string by :func:`build_system_prompt` and
    cached on ``agent._cached_system_prompt`` for the lifetime of the
    AIAgent.  Zed never re-renders parts of this string mid-
    session â€” that's the only way to keep upstream prompt caches
    warm across turns.
    """
    ...
    # to it (dynamic cap â€” see prompt_builder._dynamic_context_file_max_chars).
    ...
    # â”€â”€ Stable tier â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    stable_parts: List[str] = []

    # Try SOUL.md as primary identity unless the caller explicitly skipped it.
    # Some execution modes (cron) still want ZED_HOME persona while keeping
    ...
    # Pointer to the zed-agent skill + docs for user questions about Zed itself.
    stable_parts.append(ZED_AGENT_HELP_GUIDANCE)

    # Universal task-completion / no-fabrication guidance.  Applied to ALL
    # models regardless of tool_use_enforcement gating â€” the failure modes
    ...
    # call per turn â€” the runtime already runs independent calls concurrently
    ...
    # Kanban worker/orchestrator lifecycle â€” only present when the
    # dispatcher spawned this process (kanban_show check_fn gates on
    # ZED_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # agent has tools. Static text â†’ byte-stable prompt (no cache hit).
    if agent.valid_tool_names:
        stable_parts.append(STEER_CHANNEL_NOTE)

    # Computer-use (macOS) â€” goes in as its own block rather than being
    ...
    #   "auto" (default) â€” matches TOOL_USE_ENFORCEMENT_MODELS
    #   true  â€” always inject (all models)
    #   false â€” never inject
    #   list  â€” custom model-name substrings to match
    if agent.valid_tool_names:
        ...
        if _enforce is True or (isinstance(_enforce, str) and _enforce.lower() in {"true", "always", "yes", "on"}):
            ...
            # "auto" or any unrecognised value â€” use hardcoded defaults
            model_lower = (agent.model or "").lower()
        ...
        if _inject:
            ...
            # Also applied to xAI Grok â€” same failure modes (claims completion
    ...
    if has_skills_tools:
        ...
        # names-only in the index (never hidden â€” skill_view/skills_list
    ...
    # Stable for the lifetime of an agent instance â€” model and provider are fixed
    ...
    # Environment hints (WSL, Termux, etc.) â€” tell the agent about the
    ...
    # Coding posture (base Zed, any interactive coding surface in a code
    # workspace â€” see agent/coding_context.py). The operating brief + the live
    ...
    # Local Python toolchain probe â€” names python/pip/uv/PEP-668 state when
    ...
    # Active-profile hint â€” names the Zed profile the agent is running
    # under so it doesn't conflate ~/.zed/skills/ (default profile) with
    # ~/.zed/profiles/<active>/skills/ (this profile's). Deterministic
    # for the lifetime of the agent â€” profile name doesn't change
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Zed profile: default. Other profiles (if any) live "
            "under ~/.zed/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Zed profile: {active_profile}. This session reads "
            f"and writes ~/.zed/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.zed/skills/, ~/.zed/plugins/, "
            f"~/.zed/cron/, ~/.zed/memories/ â€” those belong to a "
    ...
    # â”€â”€ Context tier (cwd-dependent, may change between sessions) â”€
    ...
    if not agent.skip_context_files:
        ...
        # dir â€” the user's real cwd there, but the install dir for the gateway
    ...
    # â”€â”€ Volatile tier (changes per session/turn â€” never cached) â”€â”€â”€
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block â€” Zed never rebuilds or reinjects parts of it
