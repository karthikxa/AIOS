"""Skill bundles â€” aliases that load multiple skills under one slash command.
...
``/<skill-name>`` does â€” but for N skills at once.

Storage
-------
Bundles live in ``~/.zed/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ZED_HOME``). Each file looks like::

    name: backend-dev
    description: Backend feature work â€” code review, testing, PR workflow.
    skills:
      - github-code-review
...
This is the intended behavior â€” a user who names a bundle ``research``
explicitly wants ``/research`` to mean their bundle, not whatever skill
happens to share the slug.

Public API
----------
- :func:`get_skill_bundles` â€” return ``{"/slug": bundle_info}``
- :func:`resolve_bundle_command_key` â€” map a user-typed command to its slug
- :func:`build_bundle_invocation_message` â€” produce the full user message
- :func:`reload_bundles` â€” re-scan disk and return a diff
- :func:`list_bundles` â€” return rich info for display (``zed bundles``)
- :func:`save_bundle` / :func:`delete_bundle` â€” file-level operations
"""
...
# Slug normalization â€” matches agent/skill_commands.py so a bundle and a
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ZED_HOME.

    Honors ``ZED_BUNDLES_DIR`` for tests; falls back to
    ``<ZED_HOME>/skill-bundles``.
    """
    override = os.environ.get("ZED_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_zed_home() / "skill-bundles"
...
def _load_bundle_file(path: Path) -> Optional[Dict[str, Any]]:
    ...
    Errors are logged at WARNING level. We don't raise â€” a broken bundle
...
def scan_bundles() -> Dict[str, Dict[str, Any]]:
    ...
    Returns the same mapping as :func:`get_skill_bundles` â€” ``"/slug"`` â†’
...
def build_bundle_invocation_message(
    ...
    loads â€” the agent gets a note about which ones were skipped. This is
    ...
    # Header â€” tells the agent this is a bundle, lists the skills, and
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers â€” used by `zed bundles` CLI subcommand.
# ---------------------------------------------------------------------------
