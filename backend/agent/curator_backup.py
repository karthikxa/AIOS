...
A pre-run snapshot of ``~/.zed/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.zed/skills/.curator_backups/<utc-iso>/`` with a
...
The snapshot does NOT include:
  ...
  - ``.hub/`` (hub-installed skills â€” managed by the hub, not us)

It DOES include:
  - all SKILL.md files + their directories (``scripts/``, ``references/``,
    ``templates/``, ``assets/``)
  - ``.usage.json`` (usage telemetry â€” needed to rehydrate state cleanly)
  - ``.archive/`` (so rollback restores previously-archived skills too)
  - ``.curator_state`` (so rolling back also restores the last-run-at
    pointer â€” otherwise the curator would immediately re-fire on the next
    tick)
...
``~/.zed/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
fidelity but rollback only touches the ``skills``/``skill`` fields â€” the
...
# invariants. .curator_backups is the backup dir itself â€” recursion bomb.
...
def _backups_dir() -> Path:
    return get_zed_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.zed/cron/jobs.json``)."""
    return get_zed_home() / "cron" / "jobs.json"
...
def _backup_cron_jobs_into(dest: Path) -> Dict[str, Any]:
    ...
    fold it into the manifest. Never raises â€” if the cron file is missing
    ...
    try:
        ...
        return info
    # Count jobs as a nice diagnostic â€” but don't fail the snapshot if the
    ...
    # `{"jobs": [...], "updated_at": ...}` â€” we count via that shape, and
...
def _utc_id(now: Optional[datetime] = None) -> str:
    ...
    # isoformat â†’ "2026-05-01T13:05:42.123456+00:00"; strip subseconds and tz.
...
def is_enabled() -> bool:
    """Default ON â€” the whole point of the backup is safety by default."""
...
def snapshot_skills(reason: str = "manual", *, protect_ids: Optional[Set[str]] = None) -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.zed/skills/`` and prune old ones.

    Returns the snapshot directory path, or ``None`` if the snapshot was
    skipped (backup disabled, skills dir missing, or an IO error occurred â€”
    ...
    if not skills.exists():
        logger.debug("No ~/.zed/skills/ directory â€” nothing to back up")
        return None
    ...
    try:
        # Stream into the tarball â€” no tempdir copy needed.
        ...
        # snapshot â€” the skills side is the core guarantee; cron is
...
def _prune_old(keep: int, protect: Optional[Set[str]] = None) -> List[str]:
    ...
    outside the keep window â€” rollback() uses this so the mandatory
...
def list_backups() -> List[Dict[str, Any]]:
    ...
    real ``skills.tar.gz`` tarball are listed â€” transient
...
def _restore_cron_skill_links(snapshot_dir: Path) -> Dict[str, Any]:
    ...
    current file (matched by ``id``). Everything else about the job â€”
    schedule, next_run_at, last_run_at, enabled, prompt, workdir, hooks â€”
    ...
    Rules:
    - Jobs present in backup AND live, with differing skills â†’ skills restored.
    - Jobs present in backup AND live, with matching skills â†’ no-op.
    - Jobs present in backup but gone from live (user deleted the job
      after the snapshot) â†’ skipped, noted in the return report.
    - Jobs present in live but not in backup (user created a new cron
      job after the snapshot) â†’ left untouched.
    ...
    try:
        with _jobs_lock():
            ...
            if changed:
                save_jobs(live_jobs)
    except Exception as e:  # noqa: BLE001 â€” rollback must not die mid-restore
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.zed/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.zed/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `zed curator rollback --list` to see available snapshots)",
            None,
        )
    archive = target / "skills.tar.gz"
    if not archive.exists():
        return (False, f"snapshot {target.name} has no skills.tar.gz â€” corrupted?", None)
    ...
    # out before touching anything â€” otherwise a failed extract could leave
    ...
    # This dir is implementation detail â€” not listed as a restorable
    ...
    # Extract succeeded â€” the staging dir has served its purpose. The
    ...
    # alone. Failures here don't fail the overall rollback â€” the skills
    ...
    if cron_report.get("attempted"):
        ...
        if cron_report.get("error"):
            summary_bits.append(f"cron links: error â€” {cron_report['error']}")
        elif restored_n == 0 and skipped_n == 0 and cron_report.get("unchanged", 0) == 0:
            # Attempted but nothing matched â€” empty snapshot or no overlapping ids.
            pass
        else:
            parts = []
            if restored_n:
