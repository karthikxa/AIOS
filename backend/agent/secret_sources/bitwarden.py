...
Zed pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.zed/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<zed_home>/bin/bws`` on
  first use.  Zed pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.zed/.env`` as
  ``BWS_ACCESS_TOKEN`` (or whatever name the user picked in
  ``secrets.bitwarden.access_token_env``).  This is the one
  bootstrap secret â€” every other provider key can live in Bitwarden.
* Pulling secrets is a single ``bws secret list <project_id>
  --output json`` call.  We cache the result in-process for
  ``cache_ttl_seconds`` so back-to-back ``zed`` invocations don't
  hammer the API.
* Failures NEVER block Zed startup.  Missing binary, no network,
...
# Pinned upstream version.  Bump in a follow-up PR â€” never auto-resolve
...
# In-process cache so repeated load_zed_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, "_CachedFetch"] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `zed chat -q ...`
...
# <zed_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.zed/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under zed_home/cache/.

    `home_path` is what `load_zed_dotenv()` already resolved; falling back
    to `$ZED_HOME` / `~/.zed` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("ZED_HOME", Path.home() / ".zed"))
...
def _read_disk_cache(cache_key: _CacheKey, ttl_seconds: float,
    ...
    if not isinstance(secrets, dict) or not isinstance(fetched_at, (int, float)):
        return None
    # Coerce all values to strings â€” JSON allows numbers but env vars need strings
...
def _write_disk_cache(cache_key: _CacheKey, entry: "_CachedFetch",
    ...
    try:
        ...
        pass  # best-effort â€” disk cache miss on next invocation is fine
...
def _zed_bin_dir() -> Path:
    """Where Zed stores its managed binaries.  Profile-aware."""
    from zed_constants import get_zed_home

    return get_zed_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<zed_home>/bin/bws``  (our managed copy â€” preferred)
    ...
    """
    managed = _zed_bin_dir() / _platform_binary_name()
    ...
    if install_if_missing:
        try:
            return install_bws()
        except Exception as exc:  # noqa: BLE001 â€” never block startup
...
def _platform_asset_name() -> str:
    """Map (uname, arch, libc) â†’ the upstream asset filename.
    ...
    if system == "Darwin":
        # Universal binary works on both Intel and Apple Silicon â€” no
    ...
    if system == "Linux":
        ...
        # don't need bullet-proof detection â€” getting it wrong falls
...
def install_bws(*, force: bool = False) -> Path:
    ...
    failure (network, checksum, extraction) â€” callers in the auto-install
    path catch these; the user-facing ``zed secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _zed_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="zed-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "zed-agent"})
    try:
...
def _token_fingerprint(token: str) -> str:
    """SHA-256 prefix used as a cache key â€” never logged, never displayed."""
...
def fetch_bitwarden_secrets(
    *,
    ...
    self-hosted instance â€” e.g. ``https://vault.bitwarden.eu`` for EU
    ...
    ``<zed_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
    ...
    if bws is None:
        raise RuntimeError(
            "bws binary not available â€” auto-install failed and `bws` is "
...
def _run_bws_list(
    ...
    try:
        proc = subprocess.run(  # noqa: S603 â€” bws path is trusted
            cmd,
            env=env,
            capture_output=True,
            text=True,
...
# ---------------------------------------------------------------------------
# Public entry point â€” called from zed_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_zed_dotenv()`` calls after the .env
    files have loaded.  It is intentionally defensive â€” any failure
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `zed secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `zed secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `zed secrets bitwarden setup` to install."
        )
        return result
    ...
    for key, value in secrets.items():
        if key == access_token_env:
            ...
            # itself â€” that would be a footgun if someone stored the
...
# ---------------------------------------------------------------------------
# Test hook â€” used by hermetic tests to flush the cache between cases.
# ---------------------------------------------------------------------------
