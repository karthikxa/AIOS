"""Google Code Assist API client â€” project discovery, onboarding, quota.
...
1. ``load_code_assist()`` â€” probe the user's account to learn what tier they're on
   and whether a ``cloudaicompanionProject`` is already assigned.
2. ``onboard_user()`` â€” if the user hasn't been onboarded yet (new account, fresh
   free tier, etc.), call this with the chosen tier + project id. Supports LRO
   polling for slow provisioning.
3. ``retrieve_user_quota()`` â€” fetch the ``buckets[]`` array showing remaining
...
documented nowhere public â€” we copy them from the reference implementations.
"""
...
class CodeAssistError(RuntimeError):
    ...
    def __init__(
        ...
        *,
        ...
        # Parsed ``Retry-After`` seconds (kept separately for convenience â€”
...
def _client_metadata() -> Dict[str, str]:
    """Match Google's gemini-cli exactly â€” unrecognized metadata may be rejected."""
    return {
...
# =============================================================================
# load_code_assist â€” discovers current tier + assigned project
# =============================================================================
...
def load_code_assist(
    ...
    *,
    ...
    """Call ``POST /v1internal:loadCodeAssist`` with prod â†’ sandbox fallback.
    ...
    for endpoint in endpoints:
        ...
        try:
            ...
            if exc.code == "code_assist_vpc_sc":
                logger.info("VPC-SC violation on %s â€” defaulting to standard-tier", endpoint)
...
# =============================================================================
# onboard_user â€” provisions a new user on a tier (with LRO polling)
# =============================================================================
...
def onboard_user(
    ...
    *,
    ...
    For free tiers, ``project_id`` is optional â€” Google will assign one.

    Returns the final operation response. Polls ``/v1internal/<name>`` for up
    to ``_ONBOARDING_POLL_ATTEMPTS`` Ã— ``_ONBOARDING_POLL_INTERVAL_SECONDS``
    (default: 12 Ã— 5s = 1 min).
    """
    if tier_id != FREE_TIER_ID and tier_id != LEGACY_TIER_ID and not project_id:
        raise ProjectIdRequiredError(
            ...
            "Set ZED_GEMINI_PROJECT_ID or GOOGLE_CLOUD_PROJECT."
        )
...
# =============================================================================
# retrieve_user_quota â€” for /gquota
# =============================================================================
...
def resolve_project_context(
    ...
    *,
    ...
    if not tier:
        # User hasn't been onboarded â€” provision them on free tier
