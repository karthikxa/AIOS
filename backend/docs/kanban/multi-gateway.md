...
Zed supports multiple gateway processes running concurrently â€” one per profile
...
needed. On every other profile gateway, add to `~/.zed/config.yaml`:
...
Or set the env var: `ZED_KANBAN_DISPATCH_IN_GATEWAY=false`
...
(Telegram, Discord, etc.) â€” they just don't poll kanban boards.
