...
Zed middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Zed'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Zed calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Zed retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Zed guardrails,
...
- `telemetry_schema_version`: currently `zed.observer.v1`
- `middleware_schema_version`: currently `zed.middleware.v1`
...
Zed stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Zed runs
them as a nested chain in registration order. Middleware failures are fail-open:
Zed logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Zed applies middleware in this order:
...
For each tool call, Zed applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Zed pre-execution path against the effective arguments:
...
```bash
zed plugins enable <plugin-name>
```

For isolated local testing, use one `ZED_HOME` for plugin enablement and the
agent run:

```bash
export ZED_HOME=/tmp/zed-middleware-test
mkdir -p "$ZED_HOME"
zed plugins enable <plugin-name>
zed chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run zed plugins enable <plugin-name>
uv run zed chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["zed_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/zed-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Zed expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Zed treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Zed preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Zed does not convert downstream
