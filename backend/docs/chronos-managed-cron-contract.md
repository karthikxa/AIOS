# Chronos managed-cron â€” agent â†” NAS wire contract
...
Chronos lets a hosted Zed gateway **scale to zero** while idle and still
...
process can be fully stopped â€” it wakes only on a genuine fire.
...
create/update/pause/resume/remove a cron job (agent side)
  â”‚
  â–¼
ChronosCronScheduler.reconcile()        â”€â”€ agent computes next_run_at
  â”‚  POST {portal}/api/agent-cron/provision   (auth: agent's Nous access token)
  â–¼
NAS arms a one-shot for fire_at         â”€â”€ NAS owns the scheduler + its creds
  â”‚
  â° at fire_at
  â–¼
scheduler â†’ POST {portal}/api/agent-cron/relay   (auth: scheduler signature, NAS-verified)
  â”‚
  â–¼
NAS mints a short-lived agent-audience JWT (purpose=cron_fire)
  â”‚  POST {agent_callback_url}/api/cron/fire        (auth: that JWT)
  â–¼
agent verifies the NAS JWT â†’ store CAS claim â†’ run_one_job â†’ re-arm next one-shot
```
...
Why NAS-mediated rather than schedulerâ†’agent direct: the scheduler signs with
**NAS's** keys, which the agent does not (and should not) hold. The agent can
only verify a **NAS-minted** token â€” a trust path it already has. This keeps
...
## Endpoint 1 â€” `POST /api/agent-cron/provision`  (agent â†’ NAS)
...
- **Request body:**
  ...
  ```
  - `fire_at` â€” ISO 8601, **agent-computed**. May be sub-minute in the future;
    NAS must honor second-granularity (the agent owns the time, so there is no
    1-minute scheduler floor).
  - `agent_callback_url` â€” the agent's own publicly-reachable base URL. NAS
    POSTs `{agent_callback_url}/api/cron/fire` at fire time.
  - `dedup_key` â€” `"{job_id}:{fire_at}"`. NAS **upserts by `(agent_id, job_id)`**
...
- **Action:** arm one one-shot to fire at `fire_at`, destined for the NAS
  **relay** route (Endpoint 3) â€” NOT the agent directly, so NAS stays in the
  loop to mint the agent JWT. Persist `(agent_id, job_id, schedule_id,
  agent_callback_url)`.
- **Response:** `200 {"schedule_id": "<opaque>"}`.

## Endpoint 2 â€” `POST /api/agent-cron/cancel`  (agent â†’ NAS)

- **Auth:** same as Endpoint 1.
- **Body:** `{"job_id": "ab12cd34"}`.
- **Action:** cancel the armed one-shot for `(agent_id, job_id)` and delete the
  row. Idempotent â€” cancelling an unknown job is a 200 no-op.
- **Response:** `200 {"ok": true}`.

## Endpoint 3 â€” `POST /api/agent-cron/relay`  (scheduler â†’ NAS, the fire relay)

- **Auth:** the scheduler's request **signature**, verified by NAS with the
  signature path it already has. This is the trust boundary for the fire â€” a
  forged relay call must be rejected here.
- **Action:**
  1. Look up `(agent_id, job_id) â†’ agent_callback_url` from the persisted row.
  2. Mint a **short-lived** JWT: `aud = "agent:{instance_id}"`,
     `iss = {portal_url}`, `purpose = "cron_fire"`, small `exp` (â‰ˆ60â€“120s),
...
## Inbound `POST /api/cron/fire`  (NAS â†’ agent) â€” agent side, already implemented

This is the agent endpoint NAS calls in Endpoint 3 step 3. Served by the
**dashboard app** (`zed_cli/web_server.py`) â€” the agent's always-reachable
...
- **Auth:** `Authorization: Bearer <NAS-minted JWT>`. The agent verifies:
  ...
  - `exp` / `nbf` (30s leeway),
  - `purpose == "cron_fire"` â€” a general agent JWT (no/other purpose) is
    rejected so it can't be replayed against this endpoint.
- **Body:** `{"job_id": "ab12cd34", "fire_at": "..."}` (only `job_id` is used).
- **Behavior:**
  - invalid/missing/forged/expired/wrong-aud/wrong-purpose token â†’ **401**, no
    execution.
  - missing `job_id` â†’ **400**.
  - valid â†’ **202 `{"status": "accepted", "job_id": "..."}`** immediately, and
...
- **`repeat.times = N`:** `mark_job_run` deletes the job at the limit, so
  `get_job` returns `None` after the final fire â†’ the agent does **not** re-arm
  â†’ the schedule stops cleanly with no orphaned one-shot.
- **Multi-replica agents:** the store CAS makes the fire at-most-once across N
  gateway replicas sharing one `ZED_HOME` â€” exactly one replica runs each
  fire.
...
periodic wake** of a sleeping agent â€” that would negate scale-to-zero.
...
| `cron.chronos.callback_url` | the agent's own public base URL for NASâ†’agent fires |
...
relay volume through NAS ever saturates, a direct schedulerâ†’agent mode with a
