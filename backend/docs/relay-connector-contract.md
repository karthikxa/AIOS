...
| `emoji` | string | no | Display emoji (default ðŸ”Œ). |
...
note below) â€” the connector pushes an `inbound` frame down the socket the
gateway already dialed. The gateway keys the session via `build_session_key()`
from the embedded `SessionSource` â€” so populating the right discriminators is
...
handshake + outbound actions (Â§4) + its own `/stop` egress (Â§5). Inbound rides
the **same socket** in the other direction: the connector pushes an `inbound`
frame (and `interrupt_inbound` for Â§5) down the gateway's outbound WS. There is
**no gateway-side inbound HTTP endpoint** â€” a gateway need not (and, when hosted,
...
Frames (connector â†’ gateway, over the WS):

- `{"type":"inbound", "event": <MessageEvent>, "bufferId"?}`
- `{"type":"interrupt_inbound", "session_key", "chat_id"}` (Â§5)

**Trust.** The WS upgrade is authenticated with the gateway's per-gateway secret
(Â§6.1), so the channel is trusted end to end â€” inbound frames are not separately
...
> every gateway to expose a reachable inbound URL â€” impossible for hosted
...
token). Per Â§6 the connector strips that at the edge and binds it in its
...
(`session_key`) plus the capability `kind` (e.g. `discord.interaction_token`) â€”
...
doesn't match â€” the gateway has nothing to retry with, by design (a leaked
...
- **Connector â†’ gateway:** an inbound interrupt for a `session_key` is delivered
  as an `interrupt_inbound` frame down the gateway's outbound WS (Â§3 transport
  note) â€” routed cross-instance via the relay bus to whichever instance holds
  the socket â€” and bridged by the adapter's `on_interrupt(session_key, chat_id)`
  into the existing per-session interrupt mechanism, cancelling exactly that turn
  (siblings untouched).

Both directions ride the gateway's outbound WS: the gatewayâ†’connector `/stop`
egresses over it, and the connectorâ†’gateway interrupt rides the same `inbound`
...
- **normalizes** the payload into a tenant-scoped `MessageEvent` (Â§3),
- **strips any shared-identity capability** out of the payload and binds it in
  its capability vault, keyed by the session (see Â§4 `follow_up`),
- **forwards only the sanitized `MessageEvent`** â€” never the raw signed body.
...
- Re-validating Twilio HMAC / WeCom crypto would require handing the gateway the
  **shared signing secret** â€” which is itself the leak, and on a shared bot it's
  a *cross-tenant* leak.
...
- A Discord interaction token lives **inside** the signed JSON body â€” you cannot
...
relay planes â€” both are "verify at the edge â†’ emit a normalized event," differing
...
### 6.1 Channel authentication (the connectorâ‡„gateway link itself)

A2 makes the connector the sole holder of platform secrets while the gateway may
be **customer-managed and internet-exposed**, so the connectorâ‡„gateway channel
is itself authenticated. The gateway holds an enrollment- or provision-issued
**per-gateway secret** (`zed gateway enroll` â†’ connector `/relay/enroll`, or
managed self-provision â†’ `/relay/provision`) that authenticates its outbound WS
...
|-----|-----------|-----------|
| Gateway â†’ connector WS upgrade | per-gateway secret | An `Authorization` bearer header on the `/relay` upgrade. The token is `base64url(payload:exp:sig)` where `payload = gatewayId` and `sig = HMAC(payload:exp, secret)`. Connector verifies and rejects the upgrade (**close 4401**) on mismatch/absence/revocation. The authenticated tenant comes from the connector's store, never the `hello` frame. |
| Connector â†’ gateway inbound (`inbound` / `interrupt_inbound` frames) | â€” (rides the authenticated WS) | Inbound is pushed down the gateway's already-authenticated outbound socket (Â§3), so no per-message signature is needed. A **per-tenant delivery key** is still issued at enroll/provision and retained for forward-compat, but is no longer used to sign inbound. |

This is the **channel** authenticator â€” distinct from platform crypto, which the
relay path still sheds entirely (Â§6). The gateway holds zero platform secrets;
