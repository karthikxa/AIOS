# Zed Agent Security Policy

This document describes Zed Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/NousResearch/zed-agent/security/advisories/new)
or **security@zedteam.com**. Do not open public issues for
security vulnerabilities. **Zed Agent does not operate a bug
bounty program.**
...
- Environment details (`zed version`, commit SHA, OS, Python
  version).
- A reproduction against `main` or the latest release.
- A statement of which trust boundary in Â§2 is crossed.

Please read Â§2 and Â§3 before submitting. Reports that demonstrate
limits of an in-process heuristic this policy does not treat as a
boundary will be closed as out-of-scope under Â§3 â€” but see Â§3.2:
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Zed Agent access to by running it â€” typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Zed Agent's documentation
  or code about how a consuming layer (adapter, UI, file writer,
  shell) should treat agent output â€” e.g. "the dashboard renders
...
containment â€” not the approval gate, not output redaction, not any
...
Zed Agent supports two OS-level isolation postures. They address
...
since they are implemented on top of the shell contract â€” they
...
sandbox. Every code path â€” shell, code-execution, MCP, file tools,
plugins, hooks, skill loading â€” is subject to the same filesystem,
network, process, and (where applicable) inference policy.

Zed Agent supports this in two ways:

- **Zed Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Zed Agent's in-process heuristics
(Â§2.4) function as accident-prevention layered on top of a real
boundary. This is the supported posture when the agent ingests
content from surfaces the operator does not control â€” the open web,
inbound email, multi-user channels, untrusted MCP servers â€” and for
...
Zed Agent filters the environment it passes to its lower-trust
...
in-process component is operator review before install (Â§2.4,
Â§2.5), not environment scrubbing.
...
- **Skills Guard** scans installable skill content for injection
  ...
  description â€” skills execute arbitrary Python at import time.
...
operator review before install â€” the same rule as skills (Â§2.4),
...
A malicious or buggy plugin is not a vulnerability in Zed Agent
itself. Bugs in Zed Agent's plugin-install or plugin-discovery
path that prevent the operator from seeing what they're installing
are in scope under Â§3.1.
...
4. **Within the authorized set, all callers are equally trusted.**
   Zed Agent does not model per-caller capabilities inside a
...
- Escape from a declared OS-level isolation posture (Â§2.2): an
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Zed Agent's own documentation, or reasonable
  operator expectations would predict â€” including cases where
  Zed Agent has documented a stance about how its output should
...
regular issues or pull requests â€” the approval gate can always catch
...
- **Bypasses of in-process heuristics (Â§2.4)** â€” approval-gate regex
...
- **Prompt injection per se.** Getting the LLM to emit unusual
  output â€” via injected content, hallucination, training artifacts,
  or any other cause â€” is not itself a vulnerability. "I achieved
  prompt injection" without a chained Â§3.1 outcome is not an
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  zed-home security, and similar. Reports against those
  configurations are not vulnerabilities â€” that's the flag's job.
- **Community-contributed skills and plugins.** Third-party skills
  (including the community skills repository) and third-party
  plugins are in the operator's review surface, not Zed Agent's
  trust surface (Â§2.4, Â§2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Zed Agent. Bugs in Zed
...
(Â§2.2) to the trust of the content the agent will ingest. Beyond
that:
...
- Configure a caller allowlist for every network-exposed adapter
  you enable (Â§2.6).
- Review third-party skills and plugins before install (Â§2.4,
  Â§2.5). For skills, this means reading the Python and scripts,
  not just SKILL.md. Skills Guard reports and the install audit
  log are the review surface.
- Zed Agent includes supply-chain guards for MCP server
