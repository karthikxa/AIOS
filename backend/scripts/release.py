...
VERSION_FILE = REPO_ROOT / "zed_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "kiranvk2011@gmail.com": "kiranvk-2011",  # PR #24815 salvage (image documents â†’ vision)
    ...
    "274096618+zed-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "shannon@zedteam.com": "shannonsands",
    ...
    # â”€â”€ bulk addition: 75 emails resolved via API, PR salvage bodies, noreply
    #    crossref, and GH contributor list matching (April 2026 audit) â”€â”€
    ...
    "zed@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    # no-github-match â€” keep as display names
    ...
    "archer@ouyangdeMac-mini.local": "Archerouyang",  # display name æ¬§é˜³
    "batosk2@gmail.com": "Sarbai",  # git email for PR #33438 author (display: Ð‘Ñ€Ð°Ð³Ð°Ñ€Ð½Ð¸Ðº Ð”Ð¼Ð¸Ñ‚Ñ€Ð¾)
    ...
    "isaachuang@Mac.localdomain": "isaachuangGMICLOUD",  # salvage of PR #11955 â†’ #16663
    ...
    "github@nadyazed.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (ZED_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (ZED_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: zed-eval community link)
    ...
    "zedagent26@gmail.com": "zedagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+zedagent26@users.noreply.github.com": "zedagent26",
    ...
    "clement@zedteam.com": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@zed": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    # batch salvage (May 2026 LHF run, group 6 â€” final)
    ...
    "timothy.b.dixon@gmail.com": "Codename-11",  # PR #29302 (API server session controls â€” sessions/chat/fork/stream)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Zed runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level ZED_HOME state files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Zed
    ...
    # with pyproject â€” enforced by tests/acp/test_registry_manifest.py).
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    Skips silently if the manifest is missing â€” older release branches predate
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"zed-agent[acp]=={semver}"
...
def build_release_artifacts(semver: str) -> list[Path]:
    ...
    if result.returncode != 0:
        print("  âš  Could not build Python release artifacts.")
    ...
    if not matching:
        print("  âš  Built artifacts did not match the expected release version.")
        return []
    return matching
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "zed@zedteam.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/ZedTeam/zed-agent",
    ...
    lines.append(f"# Zed Agent v{semver} ({tag_name})")
    lines.append("")
    lines.append(f"**Release Date:** {date_str}")
    lines.append("")

    if first_release:
        lines.append("> ðŸŽ‰ **First official release!** This marks the beginning of regular weekly releases")
        lines.append("> for Zed Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Zed Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Zed Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        if args.bump:
            ...
            print(f"  âœ“ Updated version files to v{new_version} ({calver_date})")
            ...
            if add_result.returncode != 0:
                print(f"  âœ— Failed to stage version files: {add_result.stderr.strip()}")
                return
            ...
            if commit_result.returncode != 0:
                print(f"  âœ— Failed to commit version bump: {commit_result.stderr.strip()}")
                return
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Zed Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        if tag_result.returncode != 0:
            print(f"  âœ— Failed to create tag {tag_name}: {tag_result.stderr.strip()}")
            return
        ...
        if push_result.returncode == 0:
            ...
            print(f"  âœ— Failed to push to origin: {push_result.stderr.strip()}")
        ...
        if artifacts:
            print("  âœ“ Built release artifacts:")
        ...
        gh_cmd = [
            ...
            "--title", f"Zed Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(f"  âœ“ GitHub release created: {result.stdout.strip()}")
            print(f"\n  ðŸŽ‰ Release v{new_version} ({tag_name}) published!")
        else:
            if result is None:
                print("  âœ— GitHub release skipped: `gh` CLI not found.")
            else:
                print(f"  âœ— GitHub release failed: {result.stderr.strip()}")
            print(f"    Release notes kept at: {changelog_file}")
            print(f"    Tag was created locally. Create the release manually:")
            print(
                f"    gh release create {tag_name} --title 'Zed Agent v{new_version} ({calver_date})' "
                f"--notes-file .release_notes.md {' '.join(str(path) for path in artifacts)}"
            )
            print(f"\n  âœ“ Release artifacts prepared for manual publish: v{new_version} ({tag_name})")
    else:
        print(f"\n{'='*60}")
