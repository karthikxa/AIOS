#!/usr/bin/env python3
"""Live test harness for Zed Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any zed imports.
ORIGINAL_HOME = os.environ.get("ZED_HOME")
ORIGINAL_AUTH = Path.home() / ".zed" / "auth.json"
...
# ---------------------------------------------------------------------------
# Fake MCP tools â€” realistic shape, varied difficulty for retrieval
# ---------------------------------------------------------------------------
...
SCENARIOS: List[Dict[str, Any]] = [
    ...
    {
        "id": "E_no_tool_needed",
        "description": "Question doesn't need any tool â€” model should just answer",
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.zed/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.zed/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="zed_ts_live_"))
    zed_home = home_dir / ".zed"
    zed_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, zed_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".zed" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, zed_home / ".env")
    ...
    (zed_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return zed_home
...
def reset_module_state():
    """Drop cached modules so the new ZED_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "zed_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def _redact_secrets(text: str) -> str:
    ...
    full traceback that â€” for an auth failure â€” may echo a request header or
...
def main():
    ...
    # Restore original ZED_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ZED_HOME"] = ORIGINAL_HOME
    else:
