#!/usr/bin/env python3
"""Build the Zed Skills Index â€” a centralized JSON catalog of all skills.
...
`zed skills search/install` can use it without hitting the GitHub API.
...
# Ensure ZED_HOME is set (needed by tools/skills_hub.py imports)
os.environ.setdefault("ZED_HOME", os.path.join(os.path.expanduser("~"), ".zed"))
...
def crawl_skills_sh(source: SkillsShSource) -> list:
    ...
    50 results) which yielded ~850 unique skills â€” about 4% of the real catalog.
...
def batch_resolve_paths(skills: list, auth: GitHubAuth) -> list:
    ...
    Instead of resolving each skill individually (NÃ—M API calls), we:
...
def main():
    print("Building Zed Skills Index...", flush=True)
    overall_start = time.time()

    auth = GitHubAuth()
    print(f"GitHub auth: {auth.auth_method()}")
    if auth.auth_method() == "anonymous":
        print("WARNING: No GitHub authentication â€” rate limit is 60/hr. "
    ...
    # Per-source soft caps â€” sources stop returning when they run out, so these
    ...
    # rate-limited token zeroes all three at once â€” surfaced below so the
    ...
    EXPECTED_FLOORS = {
        ...
        # â€” better to fail loudly than ship a regression to the 858-skill
        # popular-queries era.
        "skills.sh": 10000,
        "lobehub": 100,
        # ClawHub had 49,698+ skills as of May 2026 â€” anything under 20k means
    ...
    if health_errors:
        print(
            "\nERROR: skills index health check failed â€” refusing to ship "
        ...
        # gitignored, so a fresh deploy checkout has no copy on disk â€” leaving
    ...
    # Healthy â€” only now write the index out for the docs build to consume.
    index = {
