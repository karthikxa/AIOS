...
IGNORED_PATTERNS = [
    ...
    re.compile(r"^Zed\s+(Agent|Audit)$", re.IGNORECASE),
...
def gh_pr_list():
    ...
    try:
        ...
        print("  [warn] 'gh' CLI not found â€” skipping salvaged PR scan.", file=sys.stderr)
        return []
    except subprocess.TimeoutExpired:
        print("  [warn] gh pr list timed out â€” skipping salvaged PR scan.", file=sys.stderr)
        return []
    except json.JSONDecodeError:
        print("  [warn] gh pr list returned invalid JSON â€” skipping salvaged PR scan.", file=sys.stderr)
        return []
...
def collect_commit_authors(since_tag, until="HEAD"):
    ...
    for line in log.split("\n"):
        ...
        if handle.startswith("@"):
            ...
            # Could not resolve â€” record as unknown
    ...
    for pr in prs:
        ...
        for pattern in SALVAGE_PATTERNS:
            for match in pattern.finditer(body):
                value = match.group(1)
                # If it's a number, it's a PR reference â€” skip for now
