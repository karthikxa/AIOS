...
DEFAULT_DATASETS = [
    "zedteam/swe-terminus-agent-glm-kimi-minimax",
    "zedteam/zed-agent-megascience-sft1",
    "zedteam/zed-agent-Thinking-GLM-4.7-SFT2",
    "zedteam/zed-agent-Thinking-GLM-4.7-SFT1",
    "zedteam/terminal-tasks-glm-zed-agent"
]
...
def load_dataset_from_hf(dataset_name: str) -> List[Dict[str, Any]]:
    ...
    Args:
        dataset_name: HuggingFace dataset name (e.g., "zedteam/dataset-name")
...
def main(
    ...
    if not skip_download:
        ...
        if not samples:
            print("âŒ No samples collected. Exiting.")
            return
    ...
    if not config_path.exists():
        print(f"âŒ Config not found: {config_path}")
        return
    ...
    print(f"   huggingface-cli upload zedteam/{output_name} {final_output}")
