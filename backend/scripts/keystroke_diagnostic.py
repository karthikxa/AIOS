...
Useful when adding a keybinding to Zed (or any prompt_toolkit app) and you
need to know what the terminal actually delivers â€” particularly on Windows,
...
Common questions this answers:
    - Does my terminal distinguish Ctrl+Enter from plain Enter?
      (On Windows Terminal: yes, Ctrl+Enter â†’ c-j, Enter â†’ c-m.)
    ...
    - Does Shift+Enter register as a separate key?
      (Almost never â€” most terminals collapse it to Enter.)
...
Then in Zed, bind the newline behaviour to that key:
    @kb.add('c-j')
...
def _header() -> list[str]:
    return [
        "Keystroke diagnostic â€” press keys to see how prompt_toolkit sees them.",
...
def main() -> None:
    ...
    @kb.add("<any>")
    def _on_any(event):  # noqa: ANN001 â€” prompt_toolkit event type
        parts = []
