...
When Zed runs in TUI mode, the gateway child process communicates with
...
# stdin â€” e.g. an interactive login the user explicitly invokes. Travels with
...
def find_subprocess_calls(content: str, filepath: str) -> list[dict]:
    ...
    # Match only actual function calls â€” not comments, docstrings, or prose.
            ...
            for ch in lines[j]:
                if ch == "(":
                    ...
                    if found_open and paren_depth == 0:
                        ...
                        # Already has stdin= â†’ safe.
                        if "stdin=" in call_text:
                            break

                        # Has input= â†’ creates a pipe, safe.
                        if "input=" in call_text:
                            break

                        # Inline exemption marker on the call itself or within
                        # the few comment lines immediately above it â†’ the call
...
def main() -> int:
    ...
    if all_violations:
        print(f"âŒ {len(all_violations)} subprocess calls missing stdin=:")
        ...
        print("âœ… All TUI-context subprocess calls have explicit stdin=")
        return 0
