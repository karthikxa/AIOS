# ============================================================================
# Zed Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$ZedHome = $(if ($env:ZED_HOME) { $env:ZED_HOME } else { "$env:LOCALAPPDATA\zed" }),
    [string]$InstallDir = $(if ($env:ZED_HOME) { "$env:ZED_HOME\zed-agent" } else { "$env:LOCALAPPDATA\zed\zed-agent" }),
    ...
    # builds apps/desktop into a launchable Zed.exe.
    #
    # Why opt-in:
    #   * Zed-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Zed.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Zed.exe
    ...
    #     `zed desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:zedteam/zed-agent.git"
$RepoUrlHttps = "https://github.com/zedteam/zed-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Zed Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Zed Team.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $ZedHome)) {
        New-Item -ItemType Directory -Force -Path $ZedHome | Out-Null
    }
    $envFile = Join-Path $ZedHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $ZedHome "node"
...
function Install-Uv {
    # Zed owns its own uv at $ZedHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hermes_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `zed update` stay in sync.
    $managedUv = Join-Path $ZedHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $ZedHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $ZedHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $ZedHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $ZedHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and ZED_HOME wasn't propagated).
...
function Install-Git {
    ...
    which Zed uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\zed\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Zed needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Zed copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\zed\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ZED_GIT_BASH_PATH`` (User scope) so Zed can find it in a fresh
    ...
    # Download PortableGit into $ZedHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $ZedHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Zed features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Zed needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``ZED_GIT_BASH_PATH`` (User env scope) so Zed can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $ZedHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $ZedHome\git\usr\bin\bash.exe
    $candidates += "$ZedHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$ZedHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ZED_GIT_BASH_PATH", $candidate, "User")
            $env:ZED_GIT_BASH_PATH = $candidate
            Write-Info "Set ZED_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Zed may not find Git Bash."
    Write-Info "If needed, set ZED_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        ...
        $env:Path = "$ZedHome\node;$env:Path"
        Write-Success "Node.js $version found (Zed-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Zed-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $ZedHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $ZedHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\zed-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$ZedHome\node") { Remove-Item -Recurse -Force "$ZedHome\node" }
                Move-Item $extractedDir.FullName "$ZedHome\node"
                ...
                $version = & "$ZedHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $ZedHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\zed-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `zed
                    # update` path (#4735).
                    ...
                    $stashName = "zed-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Zed behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running zed processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/zedteam/zed-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/zedteam/zed-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/zedteam/zed-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\zed-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\zed-agent-extract"
    ...
    # next `zed update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    if (Test-Path "venv") {
        ...
        # DLLs by any running zed process. Windows denies deletion of loaded
        # DLLs, so kill any zed.exe tree before removing the venv.
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running zed processes before recreating venv..."
            & taskkill /F /T /IM zed.exe /FI "PID ne $myPid" 2>$null | Out-Null
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `zed.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install zed-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and zed.exe broken with
    ...
    # here, not 30 seconds later when the user runs `zed`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `zed dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `zed dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `zed dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up zed command..."
    
    if ($NoVenv) {
        $zedBin = "$InstallDir"
    } else {
        $zedBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so zed is globally available
    # On Windows, the zed.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$zedBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$zedBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $zedBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ZED_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\zed instead
    # of the Unix default ~/.hermes
    $currentZedHome = [Environment]::GetEnvironmentVariable("ZED_HOME", "User")
    if (-not $currentZedHome -or $currentZedHome -ne $ZedHome) {
        [Environment]::SetEnvironmentVariable("ZED_HOME", $ZedHome, "User")
        Write-Success "Set ZED_HOME=$ZedHome"
    }
    $env:ZED_HOME = $ZedHome
    
    # Update current session
    $env:Path = "$zedBin;$env:Path"
    
    Write-Success "zed command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.zed-bootstrap-complete which tells the Zed
    ...
    $markerPath = Join-Path $InstallDir ".zed-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the ZED_HOME directory structure ($ZedHome, default %LOCALAPPDATA%\zed)
    New-Item -ItemType Directory -Force -Path "$ZedHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZedHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZedHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZedHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZedHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZedHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZedHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZedHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZedHome\skills" | Out-Null
    ...
    # Create config.yaml
    $configPath = "$ZedHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Zed's prompt-injection scanner
...
"@
    ...
    Write-Success "Configuration directory ready: $ZedHome"
    
    # Seed bundled skills into $ZedHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $ZedHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $ZedHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ZedHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $ZedHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Zed-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'zed setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\zed-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\zed-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\zed-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Zed` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Zed.exe. Only called from
    ...
    # resolves apps/desktop/release/win-unpacked/Zed.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Zed-managed
    ...
    # The --dir mode produces an unpacked Zed.exe in
    ...
    # are NOT trying to work around). The Zed icon + product name are
    # stamped onto Zed.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\zed-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Zed.exe",
        "$desktopDir\release\win-arm64-unpacked\Zed.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Zed.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Zed icon + identity are stamped onto Zed.exe by the
    ...
    #    Zed.exe. We deliberately do NOT point them at `zed desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Zed.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Zed.lnk')
        )
        ...
        # with the Zed icon, but without this the shortcut can keep drawing
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'zed setup'."
        return
    }
...
function Start-GatewayIfConfigured {
    ...
    $zedCmd = "$InstallDir\venv\Scripts\zed.exe"
    if (-not (Test-Path $zedCmd)) {
        $zedCmd = "zed"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ZedHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'zed whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: zed gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ZedHome\logs\gateway.log"
            Start-Process -FilePath $zedCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ZedHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: zed gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: zed gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ZedHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZedHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZedHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZedHome\zed-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   zed              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   zed setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   zed config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   zed config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   zed gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   zed update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Zed repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Zed-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Zed to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://zed-agent.zedteam.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
