...
# Bootstrap Open WebUI against Zed Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.zed/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-zed.sh
...
#   ZED_API_PORT=8642
#   ZED_API_HOST=127.0.0.1
#   ZED_API_MODEL_NAME='Zed Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Zed Agent WebUI}"
...
ZED_ENV_FILE="${ZED_ENV_FILE:-$HOME/.zed/.env}"
ZED_API_PORT="${ZED_API_PORT:-8642}"
ZED_API_HOST="${ZED_API_HOST:-127.0.0.1}"
ZED_API_CONNECT_HOST="${ZED_API_CONNECT_HOST:-127.0.0.1}"
ZED_API_MODEL_NAME="${ZED_API_MODEL_NAME:-Zed Agent}"
ZED_API_BASE_URL="http://${ZED_API_CONNECT_HOST}:${ZED_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-zed.sh"
LOG_DIR="$HOME/.zed/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$ZED_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$ZED_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.zed.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.zed</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.zed"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.zed"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-zed.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Zed Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-zed.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.zed/logs/openwebui.log
StandardError=append:%h/.zed/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-zed.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$ZED_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Zed API server is configured...'
  upsert_env API_SERVER_ENABLED true "$ZED_ENV_FILE"
  upsert_env API_SERVER_HOST "$ZED_API_HOST" "$ZED_ENV_FILE"
  upsert_env API_SERVER_PORT "$ZED_API_PORT" "$ZED_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$ZED_API_MODEL_NAME" "$ZED_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$ZED_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Zed gateway so API server settings take effect...'
  zed gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${ZED_API_CONNECT_HOST}:${ZED_API_PORT}/health" >/dev/null; then
    log 'Zed API server did not answer on the first check. Trying to start gateway in the background...'
    nohup zed gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${ZED_API_CONNECT_HOST}:${ZED_API_PORT}/health" >/dev/null
  ...
  log "Zed API endpoint: ${ZED_API_BASE_URL}"
