...
Why per-file rather than per-test?
    Per-test spawn overhead (~250ms Ã— 17k tests = 70min CPU minimum)
    swamped the actual work. Per-file spawn (~250ms Ã— ~850 files = ~3.5min)
    fits in the budget while still giving every file a fresh Python
    interpreter â€” the only isolation boundary that actually matters
...
Environment:
    ZED_TEST_WORKERS  Override worker count (default: os.cpu_count())
    ZED_TEST_PATHS    Override discovery roots (colon-sep, default: 'tests')
...
# Directories to skip during discovery â€” these suites require real
# external services (a model gateway, a docker daemon with a prebuilt
# image, etc.) and are run in their own dedicated CI jobs:
#
#   tests/e2e/         â€” .github/workflows/tests.yml :: e2e job
#   tests/integration/ â€” historical; legacy --ignore flags
#   tests/docker/      â€” .github/workflows/docker-publish.yml ::
#                        build-amd64 job (runs against the freshly-loaded
#                        nousresearch/zed-agent:test image, via
#                        ``ZED_TEST_IMAGE`` so the fixture skips
...
# via --file-timeout or ZED_TEST_FILE_TIMEOUT.
...
def _count_tests(
    ...
    collected tests are omitted from the dict (not an error â€” e.g. the
    ...
    skip â€” matching what the per-file runs will actually execute.
    """
...
def _discover_files(roots: List[Path]) -> List[Path]:
    ...
    ``test_*`` prefix â€” caller knows what they want).
    ...
    ``pytest tests/docker/`` does â€” the CI-level skip exists to keep
...
def _kill_tree(proc: "subprocess.Popen", pgid: int | None = None) -> None:
    ...
    POSIX: the caller must pass ``pgid`` â€” the process group id captured
...
def _run_one_file(
    ...
    ``summary_counts`` is the result of ``_parse_pytest_summary(output)`` â€”
    ...
    # even though grandchildren in that group are still alive â€” defeating
    ...
    try:
        ...
        # KeyboardInterrupt / runner crash â€” make sure no zombie
    ...
    if rc == 5:
        # No tests collected â€” every test in the file was filtered out.
...
def _parse_pytest_summary(output: str) -> dict[str, int]:
    ...
    result: dict[str, int] = {}
    # Walk backwards from the end â€” the summary line is always near the tail.
    for line in reversed(output.splitlines()):
        ...
        # Also match "N error" (singular â€” pytest uses this sometimes).
        for m in re.finditer(r"(\d+)\s+error\b", line):
            result.setdefault("errors", result.get("errors", 0) + int(m.group(1)))
        if result:
            # Found the counts line â€” done.
            break
        # Stop at the short test summary header (if any) â€” everything above
...
def _format_file(file: Path, repo_root: Path) -> str:
    ...
    ``/home/runner/work/zed-agent/zed-agent/tests/acp/test_auth.py``.
...
def _print_progress(
    ...
    msg = (
        ...
        f" | âœ“{tests_passed:>{fw}} | âœ—{tests_failed:>{fw}}] "
...
def _print_inline_failure(
    ...
    # Grab just the failure lines (last ~30 lines of pytest output â€”
    # typically the FAILED summary + short test info).
    lines = output.rstrip().splitlines()
    tail = "\n".join(lines[-30:])

    print(flush=True)
    print(f"  â•”â• Failed: {rel} â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•", flush=True)
    for line in tail.splitlines():
        print(f"  â•‘ {line}", flush=True)
    print(f"  â•‘", flush=True)
    print(f"  â•‘  Repro: {repro}", flush=True)
    print(f"  â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•", flush=True)
    print(flush=True)
...
def _load_durations(repo_root: Path) -> dict[str, float]:
    ...
    the last run. Missing or corrupt file â†’ empty dict (safe fallback).
    """
...
def _slice_files(
    ...
    ``slice_index`` is 1-indexed (1..slice_count) for ergonomics â€”
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "--jobs",
        type=int,
        default=int(os.environ.get("ZED_TEST_WORKERS") or (os.cpu_count() or 4) * 2),
        help="Parallel worker count (default: $ZED_TEST_WORKERS or cpu_count*2)",
    )
    parser.add_argument(
        "--paths",
        default=os.environ.get("ZED_TEST_PATHS", ":".join(_DEFAULT_ROOTS)),
    ...
    parser.add_argument(
        ...
        type=float,
        default=float(
            os.environ.get("ZED_TEST_FILE_TIMEOUT", _DEFAULT_FILE_TIMEOUT_SECONDS)
        ),
        help=(
            "Per-file wall-clock cap in seconds. On timeout, the pytest "
            "subprocess and its full process tree are SIGKILL'd. "
            f"Default: {_DEFAULT_FILE_TIMEOUT_SECONDS}s ({round(_DEFAULT_FILE_TIMEOUT_SECONDS/60)} min), env: ZED_TEST_FILE_TIMEOUT."
        ),
    )
    parser.add_argument(
        "--slice",
    ...
    # Parse --slice (or ZED_TEST_SLICE) early so we can exit on bad input
    # before doing any expensive discovery.
    slice_raw = args.slice or os.environ.get("ZED_TEST_SLICE")
    ...
    if args.paths_positional:
        ...
        # fine â€” _discover_files handles both via rglob('test_*.py') for
    ...
    if args.include_integration:
        # Caller takes responsibility â€” typically used via explicit -k filter.
        global _SKIP_PARTS  # noqa: PLW0603 â€” config knob
        _SKIP_PARTS = set()
    ...
    # Apply slicing if requested â€” distribute files across CI jobs by
    ...
    # Capture and print on completion (out-of-order is fine â€” keeps the
    ...
    def _on_done(file: Path, started_at: float, fut: "Future[Tuple[Path, int, str, dict[str, int], float]]") -> None:
        ...
        except Exception as exc:  # noqa: BLE001 â€” must always advance counter
            with lock:
                files_done += 1
                tests_done += n_tests
                fail_count += 1
    ...
    # Per-file time distribution (throwaway diagnostic â€” shows how
    ...
    if file_times:
        ...
        # Top 10 slowest files â€” likely the ones dragging the run.
