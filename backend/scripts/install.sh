...
# Python-driven tool session (e.g. Zed terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/zed-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/zed-agent.git"
ZED_HOME="${ZED_HOME:-$HOME/.zed}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory â€” if so we never override it.
if [ -n "${ZED_INSTALL_DIR:-}" ]; then
...
#   code at /usr/local/lib/zed-agent, command at /usr/local/bin/zed,
#   data still at /root/.zed (ZED_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --no-skills    Start with a blank slate â€” seed no bundled skills, and"
            echo "                   write \$ZED_HOME/.no-bundled-skills so future"
            echo "                   'zed update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Zed.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.zed/zed-agent"
            echo "                   default (root, Linux): /usr/local/lib/zed-agent"
            echo "  --zed-home PATH  Data directory (default: ~/.zed, or \$ZED_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Zed installs the code under"
            echo "  /usr/local/lib/zed-agent and links the command into"
            echo "  /usr/local/bin/zed (FHS layout â€” matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$ZED_HOME"
            echo "  (default /root/.zed).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$ZED_HOME/zed-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs zed setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "â”‚             âš• Zed Agent Installer                    â”‚"
    echo "â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤"
    echo "â”‚  An open source AI agent by Zed Team.              â”‚"
...
# intentional, but they leave the checkout dirty â€” which forces `zed update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Zed-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Zed Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install zed command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `zed` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ZED_HOME/zed-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ZED_HOME/zed-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/zed-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $ZED_HOME/zed-agent â€” then preserve it)
#
# Always no-op when the user set --dir or $ZED_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ZED_HOME.
    if is_termux; then
        INSTALL_DIR="$ZED_HOME/zed-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$ZED_HOME/zed-agent/.git" ]; then
            INSTALL_DIR="$ZED_HOME/zed-agent"
            log_info "Existing install detected at $INSTALL_DIR â€” keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/zed-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/zed-agent"
        ...
        # which non-root users can't traverse â€” leaving the shared
        # /usr/local/bin/zed wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/zed"
        log_info "  Data:    $ZED_HOME (unchanged)"
...
# Point a Zed-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $ZED_HOME/node/bin â€” which is NOT on
...
# Zed's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Zed-managed npm, so calling it on
...
configure_managed_node_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$ZED_HOME/node/etc/npmrc"
}

get_zed_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/zed" ]; then
        echo "$link_dir/zed"
    else
        echo "zed"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://zed-agent.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Zed owns its own uv at $ZED_HOME/bin/uv.  Always install there â€”
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (zed_cli/managed_uv.py) looks in the same
    # place, so install.sh and `zed update` stay in sync.
    local _managed_uv="$ZED_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $ZED_HOME/bin ..."
    mkdir -p "$ZED_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/zed-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/zed-uv-installer.$$.sh")"
    ...
    # directly into $ZED_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$ZED_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
attempt_install_git() {
    case "$OS" in
        macos)
            # Prefer Homebrew â€” fully headless when present.
            ...
            # system dialog (Apple gates CLT behind it â€” it cannot be fully
...
# `^20.19 || >=22.12` â€” older Node lacks `node:util.styleText`, so `vite build`
# crashes with a SyntaxError that surfaces only as the opaque "Build desktop
# app â€¦ exit code 1" install failure. Returns 0 when the given `node --version`
# string clears that floor; anything below it is replaced with the Zed-
...
check_node() {
    ...
    # Repair pre-existing Zed-managed installs where `npm install -g` lands
    # off PATH. No-op when there's no managed Node, so this is safe to run on
    # every install â€” including re-runs that skip the Node (re)install below.
    configure_managed_node_npm_prefix
    ...
    # Prefer a Zed-managed Node from a previous run over a too-old system one.
    if [ -x "$ZED_HOME/node/bin/node" ] && node_satisfies_build "$("$ZED_HOME/node/bin/node" --version)"; then
        export PATH="$ZED_HOME/node/bin:$PATH"
        log_success "Node.js $("$ZED_HOME/node/bin/node" --version) found (Zed-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) â€” installing Zed-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.zed/node/..."
    ...
    # Place into ~/.zed/node/ and symlink binaries into the same bin dir
    # the zed command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$ZED_HOME/node"
    mkdir -p "$ZED_HOME"
    mv "$extracted_dir" "$ZED_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$ZED_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$ZED_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$ZED_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$ZED_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.zed/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Zed install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password â€” ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Zed Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `zed update` path
                # (#4735).
                ...
                local stash_name
                stash_name="zed-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # branches â€” on a non-single-branch checkout that turns each update
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Zed behaves unexpectedly."
                    else
...
setup_venv() {
    ...
    # version â€” building Rust transitives that have no wheel for that
...
setup_path() {
    log_info "Setting up zed command..."

    if [ "$USE_VENV" = true ]; then
        ZED_BIN="$INSTALL_DIR/venv/bin/zed"
    else
        ZED_BIN="$(which zed 2>/dev/null || echo "")"
        if [ -z "$ZED_BIN" ]; then
            log_warn "zed not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$ZED_BIN" ]; then
        log_warn "zed entry point not found at $ZED_BIN"
    ...
    # Create a user-facing shim for the zed command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $ZED_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim â€” making `exec "$ZED_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/zed"
    cat > "$command_link_dir/zed" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ZED_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/zed"
    log_success "Installed zed launcher â†’ $command_link_display_dir/zed"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "zed command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v zed' \
                >/dev/null 2>&1; then
        ...
        log_info "zed not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Zed Agent â€” ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Zed Agent â€” ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Zed Agent â€” ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so zed works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.zed directory structure (config at top level, code in subdir)
    mkdir -p "$ZED_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.zed/.env (top level, easy to find)
    if [ ! -f "$ZED_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ZED_HOME/.env"
            log_success "Created ~/.zed/.env from template"
        else
            touch "$ZED_HOME/.env"
            log_success "Created ~/.zed/.env"
        fi
    else
        log_info "~/.zed/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$ZED_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.zed/config.yaml (top level, easy to find)
    if [ ! -f "$ZED_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ZED_HOME/config.yaml"
            log_success "Created ~/.zed/config.yaml from template"
        fi
    else
        log_info "~/.zed/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$ZED_HOME/SOUL.md" ]; then
        cat > "$ZED_HOME/SOUL.md" << 'SOUL_EOF'
# Zed Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Zed communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.zed/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.zed/"

    # Seed bundled skills into ~/.zed/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `zed update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'zed update'." \
            > "$ZED_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $ZED_HOME/.no-bundled-skills"
        log_info "  Future 'zed update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.zed/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.zed/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ZED_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$ZED_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.zed/skills/"
            fi
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Zed Agent browser tools â€” use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Zed will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (zed --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `zed update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'zed setup' after install."
        return 0
    fi
    ...
    # Run zed setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m zed_cli.main setup < /dev/tty
    else
        python -m zed_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$ZED_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Zed to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ZED_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'zed whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ZED_CMD="$(get_zed_command_path)"
                $ZED_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'zed whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'zed gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ZED_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ZED_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: zed gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: zed gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ZED_CMD gateway > "$ZED_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.zed/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: zed gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: zed gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ZED_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ZED_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ZED_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}zed${NC}              Start chatting"
    echo -e "   ${GREEN}zed setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}zed config${NC}       View/edit configuration"
    echo -e "   ${GREEN}zed config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}zed gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}zed update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}âš¡ 'zed' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}âš¡ 'zed' was linked into /usr/local/bin and is ready to use â€” no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}âš¡ Reload your shell to use 'zed' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$ZED_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$ZED_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$ZED_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$ZED_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$ZED_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Zed for pip install"
    ...
    ZED_CMD="$(command -v zed 2>/dev/null || echo "")"
    if [ -n "$ZED_CMD" ]; then
        log_info "Running zed setup..."
        "$ZED_CMD" setup
    else
        log_warn "zed command not found on PATH"
        log_info "Try: python -m zed_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`Zed` rename
...
# _purge_electron_build_cache() used by `zed desktop`; install.sh was the only
...
# keychain (a real signed/notarized .dmg needs Apple credentials â€” a separate
...
# Electron package dir â€” workspace-local nest first, then root hoist.
_electron_dir() {
    local install_dir="$1"
...
install_desktop() {
    ...
    # failure, not a silent skip â€” a silent skip yields a "complete" install
    ...
    # floor (^20.19 || >=22.12) and prepends the Zed-managed Node to PATH, so
    # the build never runs on a too-old system Node â€” the cause of the opaque
    # "Build desktop app â€¦ exit code 1" failure (Vite crashes on old Node).
    check_node
    ...
    #    flake) â€” leaving tsc/typescript unresolved and `npm run pack`'s
    ...
    #         retry (matches install.ps1 / `zed desktop`),
    #      c) on still-failing, fall back to a public Electron mirror â€” this is
    ...
    # (c) GitHub blocked â†’ mirror fallback (#47266).
    if [ "$pack_ok" = false ] && [ -z "${ELECTRON_MIRROR:-}" ]; then
        log_warn "Desktop build still failing â€” the Electron download from GitHub looks blocked."
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Zed" ]; then
            app="$desktop_dir/release/linux-unpacked/Zed"
        elif [ -x "$desktop_dir/release/linux-unpacked/zed" ]; then
            app="$desktop_dir/release/linux-unpacked/zed"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Zed.app" \
            "$desktop_dir/release/mac/Zed.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Zed is damaged and can't be opened").
    ...
    # checkout stays clean for the next `zed update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Zed-managed Node
            # provisioned during prerequisites/node-deps (at $ZED_HOME/node/bin)
...
run_stage_protocol() {
    ...
    # flow) only exits the subshell â€” the parent still reaches the JSON result
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $ZED_HOME.
    # $ZED_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'zed update' on this host install.
