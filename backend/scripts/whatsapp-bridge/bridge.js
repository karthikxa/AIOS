 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.zed/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.zed', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (ZED_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.ZED_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.zed', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.ZED_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.zed', 'document_cache');
const AUDIO_CACHE_DIR = process.env.ZED_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.zed', 'audio_cache');
...
// `zed update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = 'âš• *Zed Agent*\nâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€\n';
...
function formatOutgoingMessage(message) {
  ...
  // redundant â€” the sender identity is already clear.  Only prepend in
...
// Build LID â†’ phone reverse map from session files (lid-mapping-{phone}.json)
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Zed Agent', 'Chrome', '120.0'],
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      if (reason === DisconnectReason.loggedOut) {
        console.log('âŒ Logged out. Delete session and restart to re-authenticate.');
        process.exit(1);
      } else {
        // 515 = restart requested (common after pairing). Always reconnect.
        if (reason === 515) {
          console.log('â†» WhatsApp requested restart (code 515). Reconnecting...');
        } else {
          console.log(`âš ï¸  Connection closed (reason: ${reason}). Reconnecting in 3s...`);
        }
      ...
      if (PAIR_ONLY) {
        console.log('âœ… Pairing complete. Credentials saved.');
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          // Bot mode: separate number. ALL fromMe are echo-backs of our own replies â€” skip.
          continue;
        }
      ...
      // themselves â€” stranger DMs / group pings must never reach the
      ...
      // Ignore Zed' own reply messages in self-chat mode to avoid loops.
...
// Host-header validation â€” defends against DNS rebinding.
...
app.use((req, res, next) => {
  ...
  // Strip port suffix: "localhost:3000" â†’ "localhost"
  ...
  try {
    ...
    switch (type) {
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `zed_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execSync(
            ...
            // ffmpeg not available or conversion failed â€” fall back to original format
...
if (PAIR_ONLY) {
  ...
  console.log('ðŸ“± WhatsApp pairing mode');
  console.log(`ðŸ“ Session: ${SESSION_DIR}`);
  console.log();
  startSocket();
} else {
  app.listen(PORT, '127.0.0.1', () => {
    console.log(`ðŸŒ‰ WhatsApp bridge listening on port ${PORT} (mode: ${WHATSAPP_MODE})`);
    console.log(`ðŸ“ Session stored in: ${SESSION_DIR}`);
    if (ALLOWED_USERS.size > 0) {
      console.log(`ðŸ”’ Allowed users: ${Array.from(ALLOWED_USERS).join(', ')}`);
    } else if (WHATSAPP_MODE === 'self-chat') {
      console.log(`ðŸ”’ Self-chat mode â€” only your own messages to yourself are processed.`);
    } else {
      console.log(`ðŸ”’ No WHATSAPP_ALLOWED_USERS set â€” incoming messages are rejected.`);
