#!/usr/bin/env python3
"""Discord Voice Doctor â€” diagnostic tool for voice channel support.
...
def check_packages():
    ...
    try:
        ...
        try:
            ...
            check("PyNaCl (Aead)", False, f"v{ver} â€” need >=1.5.0")
            ok = False
    except ImportError:
    ...
    try:
        ...
        warn("faster-whisper (local STT)", "not installed â€” local STT unavailable")
    ...
    try:
        ...
        warn("edge-tts", "not installed â€” edge TTS unavailable")

    try:
        import elevenlabs
        check("elevenlabs SDK", True)
    except ImportError:
        warn("elevenlabs SDK", "not installed â€” premium TTS unavailable")
...
def check_system_tools():
    ...
    if _discord_available:
        ...
        warn("Opus codec", "skipped â€” discord.py not installed")
...
def check_env_vars():
    ...
    # Allowed users â€” resolve usernames if possible
    ...
    if allowed:
        ...
        warn("DISCORD_ALLOWED_USERS", "not set â€” all users can use voice")
    ...
    if groq_key:
        ...
        warn("GROQ_API_KEY", "not set â€” Groq STT unavailable")

    if eleven_key:
        check("ELEVENLABS_API_KEY (TTS)", True, mask(eleven_key))
    else:
        warn("ELEVENLABS_API_KEY", "not set â€” ElevenLabs TTS unavailable")

    return ok, token, groq_key, eleven_key


def check_config(groq_key, eleven_key):
    """Check zed config.yaml."""
    ...
    if config_path.exists():
        ...
        warn("config.yaml", "not found â€” using defaults")

    # Voice mode state
    voice_mode_path = ZED_HOME / "gateway_voice_mode.json"
...
def check_bot_permissions(token):
    ...
    if not token:
        warn("Bot permissions", "no token â€” skipping")
        return True

    try:
        import requests
    except ImportError:
        warn("Bot permissions", "requests not installed â€” skipping")
        return True
...
def main():
    ...
    print("\033[1m" + "-" * 50 + "\033[0m")
    if all_ok:
        print(f"  {OK} \033[92mAll checks passed â€” voice mode ready!\033[0m")
    else:
        print(f"  {FAIL} \033[91mSome checks failed â€” fix issues above.\033[0m")
    print()
