#!/usr/bin/env python3
"""Boot-time re-seed of a terminally-dead Zed bootstrap session.

Background
----------
A Zed bootstrap session (client_id ``hermes-cli-vps``) can take a terminal
``invalid_grant`` and be quarantined locally — the refresh path clears the dead
tokens from ``auth.json`` and stamps
``providers.Zed.last_auth_error.relogin_required = true``. From then on every
...
``ZED_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the Zed
provider entry from that env **only when the on-disk Zed entry is provably
...
Design constraints
------------------
- Pure stdlib, no zed_cli imports: runs early in the boot hook, before the
  app venv/modules are guaranteed importable, as its own subprocess.
- Surgical: replaces ONLY ``providers.Zed`` in the existing auth.json, leaving
...
# overwrites a terminally-dead Zed entry on an existing volume.
REBOOTSTRAP_ENV = "ZED_AUTH_JSON_REBOOTSTRAP"


def _Zed_entry_is_terminal(Zed_state: Any) -> bool:
...
def _extract_Zed_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.Zed`` block out of a ZED_AUTH_JSON_REBOOTSTRAP
    ...
    Zed entry — caller treats None as "nothing to do"."""
    try:
    ...
    if not isinstance(providers, dict):
        return None
    Zed = providers.get("Zed")
    if not isinstance(Zed, dict) or not Zed:
        return None
    return Zed
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    """Core logic. Returns a short status string for logging/testing:
      ...
      - "bad_seed"         — seed present but unparseable / no Zed entry
      ...
      - "not_terminal"     — on-disk Zed entry is healthy/absent → no-op
      - "reseeded"         — Zed entry was terminal; replaced from seed
    """
    if not seed_raw:
        return "no_seed"

    seed_Zed = _extract_Zed_from_seed(seed_raw)
    if seed_Zed is None:
        return "bad_seed"
    ...
    if not _Zed_entry_is_terminal(providers.get("Zed")):
        # Healthy, rotating, or absent Zed entry — the load-bearing guard.
        # Never clobber a good session; this is what makes the re-seed safe to
        # push on every restart.
        return "not_terminal"

    # Surgical replacement: swap ONLY providers.Zed, preserve everything else.
    providers["Zed"] = seed_Zed
...
def main() -> int:
    ...
    if result == "reseeded":
        print("[rebootstrap] Zed bootstrap session was terminal; re-seeded auth.json from "
              f"{REBOOTSTRAP_ENV}")
    else:
