#!/usr/bin/env python3
"""Drive the Zed TUI under ZED_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.zed/perf.log on exit.

The --tui build must exist (run `npm run build` in ui-tui first). This script
launches `node dist/entry.js` directly with ZED_TUI_RESUME set so it
bypasses the zed_cli wrapper â€” we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  ZED_PERF_LOG     (default ~/.zed/perf.log)
  ZED_PERF_NODE    (default node from $PATH)
  ZED_TUI_DIR      (default: <repo>/ui-tui relative to this script)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests ZED_DEV_PERF wiring is broken).
"""
...
DEFAULT_TUI_DIR = Path(
    os.environ.get("ZED_TUI_DIR")
    or str(Path(__file__).resolve().parent.parent / "ui-tui")
)
DEFAULT_LOG = Path(os.environ.get("ZED_PERF_LOG", str(get_zed_home() / "perf.log")))
DEFAULT_STATE_DB = get_zed_home() / "state.db"
...
KEYS = {
    "page_up": b"\x1b[5~",
    "page_down": b"\x1b[6~",
    "wheel_up": b"\x1b[M`!!",      # mouse wheel up (SGR-less) â€” best-effort
    "shift_up": b"\x1b[1;2A",
...
def format_report(data: dict[str, Any]) -> str:
    ...
    out.append("â•â•â• React Profiler â•â•â•")
    if not react:
        out.append("  (no react events â€” ZED_DEV_PERF wired? threshold too high?)")
    else:
    ...
    out.append("")
    out.append("â•â•â• Ink pipeline â•â•â•")
    if not frames:
        out.append("  (no frame events â€” onFrame wiring broken?)")
    else:
        ...
        if phases_present:
            ...
            if sum_ps:
                ...
                out.append(
                    f"  {'dur-Î£phases':<10} {pct(deltas,0.50):>8.2f} {pct(deltas,0.95):>8.2f} "
            ...
            # Patch counts â€” proxy for "how much changed each frame"
            ...
            # Write bytes + drain telemetry â€” the outer-terminal bottleneck gauge.
            bytes_written = [
            ...
            if backpressure:
                out.append(
                    ...
                    f"({100*backpressure/len(frames):.0f}%)   (Node stdout buffer full â€” terminal slow)"
                )
        ...
        if flicker_frames:
            out.append("")
            out.append(f"  âš  flickers detected in {len(flicker_frames)} frames")
...
def key_metrics(data: dict[str, Any]) -> dict[str, float]:
    ...
    if frames:
        ...
        if len(ts) >= 2:
            ...
            # Interframe gaps distribution â€” complementary view to throughput:
...
def format_diff(before: dict[str, float], after: dict[str, float]) -> str:
    ...
    for k in keys:
        ...
        pct_str = "â€”" if pct_change == float("inf") else f"{pct_change:+6.1f}%"
        lines.append(
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not entry.exists():
        sys.exit(f"{entry} missing â€” run `npm run build` in {tui_dir} first")

    sid = args.session or pick_longest_session(DEFAULT_STATE_DB)
    print(f"â€¢ session: {sid}")
    print(f"â€¢ hold: {args.hold} x {args.rate}Hz for {args.seconds}s after {args.warmup}s warmup")
    print(f"â€¢ terminal: {args.cols}x{args.rows}")
    ...
    env = os.environ.copy()
    env["ZED_DEV_PERF"] = "1"
    env["ZED_DEV_PERF_MS"] = str(args.threshold_ms)
    env["ZED_DEV_PERF_LOG"] = str(log)
    env["ZED_TUI_RESUME"] = sid
    ...
    node = os.environ.get("ZED_PERF_NODE", "node")
    ...
    try:
        ...
        print(f"â€¢ pid: {pid}  fd: {fd}")
        print(f"â€¢ warmup {args.warmup}s (drain startup output)â€¦")
        drain(fd, args.warmup)

        print(f"â€¢ holding {args.hold}â€¦")
        ...
        try:
            ...
            for _ in range(10):
                ...
                os.kill(pid, signal.SIGKILL)  # windows-footgun: ok â€” POSIX-only script (imports pty at top)
                os.waitpid(pid, 0)
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="ZED_DEV_PERF_MS (0 = capture all)")
    ...
    if args.compare:
        ...
        if not path.exists():
            print(f"\nâš  no baseline at {path} â€” run with --save {args.compare} first")
        else:
            before = json.loads(path.read_text())
            print(f"\nâ•â•â• A/B diff vs /tmp/perf-{args.compare}.json â•â•â•")
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    reports delta vs the last one â€” visibility into whether the last
    ...
    pkg_root = tui_dir / "packages" / "zed-ink" / "src"
    ...
    print(f"â€¢ loop mode â€” watching {src_root} + {pkg_root} for *.ts(x) changes")
    print("â€¢ edit any TS file, the harness rebuilds + reruns automatically")
        ...
        while True:
            ...
            if iteration > 1:
                ...
                if result.returncode != 0:
                    ...
                    print("\nâ€¢ waiting for source changes to retryâ€¦")
            ...
            if previous_metrics is not None:
                print(f"\nâ•â•â• A/B diff vs iteration {iteration - 1} â•â•â•")
                print(format_diff(previous_metrics, metrics))

            previous_metrics = metrics

            print("\nâ€¢ waiting for source changesâ€¦")
            previous_mtimes = wait_for_change(previous_mtimes, collect_mtimes)
...
def wait_for_change(prev: dict[str, float], collect) -> dict[str, float]:
    ...
    while True:
        ...
        if changed:
            print(f"  â†» {len(changed)} file(s) changed:")
            for path in changed[:5]:
                print(f"    {path}")
            # Debounce â€” editor save bursts can take ~500ms to settle
            time.sleep(0.5)
            return collect()
