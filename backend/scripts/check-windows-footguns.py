...
Flags common patterns that break silently on Windows. Run before PRs â€”
...
Exit status:
    0 â€” no Windows footguns found (or all matches suppressed)
    1 â€” at least one unsuppressed match

Suppress an intentional use (e.g. tests or platform-gated code) with:
    os.kill(pid, 0)  # windows-footgun: ok â€” only called on POSIX
"""
...
# the programmer wrote the line in full awareness of the Windows pitfall â€”
# e.g. `if hasattr(os, 'setsid'): ... os.setsid()`, or the classic
# `getattr(signal, 'SIGKILL', signal.SIGTERM)`, or `shutil.which("wmic")`.
# False negatives are fine here â€” the inline `# windows-footgun: ok` marker
...
# Files we never scan (self-referential â€” this script mentions the
# patterns it detects â€” and the CONTRIBUTING docs that list them).
EXCLUDED_FILES = {
...
FOOTGUNS: list[Footgun] = [
    Footgun(
        ...
        # Match builtins.open() specifically â€” NOT os.open(), .open()
        ...
        message=(
            ...
            "(UTF-8 on POSIX, cp1252/mbcs on Windows) â€” files round-tripped "
        ...
        post_filter=lambda m, line: (
            ...
            # Skip open(path, **kwargs) patterns â€” encoding may be in the dict.
    ...
    Footgun(
        ...
        message=(
            "os.kill(pid, 0) is NOT a no-op on Windows â€” it sends "
        ...
        fix=(
            ...
            "Or gateway.status._pid_exists(pid) for the zed wrapper "
    ...
    Footgun(
        ...
        message=(
            "Running a script via './scriptname' doesn't work on Windows â€” "
    ...
    Footgun(
        ...
        # Match wmic appearing as a subprocess argument â€” NOT the
    ...
    Footgun(
        ...
        message=(
            ...
            "Windows â€” always wrap in try/except or gate with a "
            "platform check."
        ),
        fix=(
            "try:\n"
...
def should_scan_file(path: Path) -> bool:
    ...
    if rel in EXCLUDED_FILES:
        return False
    # Only scan text files (rough heuristic â€” .py, .md, .sh, .ps1, .yaml, etc.)
...
def _strip_code(line: str) -> str:
    """Return just the code portion of a line â€” strip trailing comments and
    ...
    # Line starts with # â€” entirely a comment.
    if stripped.startswith("#"):
        return ""
    # Remove trailing "# ..." inline comment. Naive â€” doesn't handle `#`
    # inside strings â€” but on balance reduces noise far more than it adds.
...
def _find_unquoted_hash(line: str) -> int | None:
    ...
    Simple state machine â€” good enough for the 99% case of "code, then
...
def scan_file(path: Path, footguns: list[Footgun]) -> list[tuple[int, str, Footgun]]:
    ...
    # Simple state machine â€” handles both ''' and """, toggled by the FIRST
    ...
    for i, line in enumerate(text.splitlines(), start=1):
        ...
        code_for_scan = line
        if in_triple:
            # We're inside a docstring â€” skip the whole line's scan.
        ...
        for delim in ('"""', "'''"):
            if delim in code_for_scan:
                # Count occurrences â€” even count means single-line docstring,
                # odd means we've entered a multi-line one.
                count = code_for_scan.count(delim)
                if count % 2 == 1:
                    # Odd â€” we're now inside the triple-quoted block.
                    ...
                    # Even â€” entire docstring fits on one line. Strip it
        ...
        if SUPPRESS_MARKER.search(line):
            continue
        # Skip if the line has an obvious guard â€” e.g. hasattr/getattr/
        ...
        for fg in footguns:
            ...
            if fg.post_filter is not None:
                try:
                    ...
                    # Post-filter assumed a named group that isn't there â€” skip.
                    continue
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument(
        "--all",
        action="store_true",
        help="Scan the full repository (zed_cli/, gateway/, tools/, cron/, etc.).",
    )
    p.add_argument(
        "--diff",
        metavar="REF",
...
def main(argv: list[str]) -> int:
    # Windows terminals default to cp1252, which can't encode the âœ“/âœ—
    ...
    if total_matches:
        print(
            f"\nâœ— {total_matches} Windows footgun(s) found across "
    ...
    print(
        f"âœ“ No Windows footguns found ({files_scanned} file(s) scanned)."
    )
    return 0
