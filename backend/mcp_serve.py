"""
Zed MCP Server â€” expose messaging conversations as MCP tools.
...
Plus: channels_list (Zed-specific extra)

Usage:
    zed mcp serve
    zed mcp serve --verbose
...
logger = logging.getLogger("zed.mcp_serve")
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = Path(
            os.environ.get("ZED_HOME", Path.home() / ".zed")
...
class EventBridge:
    ...
    This is the Zed equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def __init__(self):
        ...
        # mtime cache â€” skip expensive work when files haven't changed
    ...
    def _poll_once(self, db):
        ...
        when nothing has changed â€” makes 200ms polling essentially free.
        """
        # Check if sessions.json has changed (mtime check is ~1Î¼s)
        sessions_file = _get_sessions_dir() / "sessions.json"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Zed MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "zed",
        instructions=(
            "Zed Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Zed MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
