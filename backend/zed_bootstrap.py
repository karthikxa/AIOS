"""Windows UTF-8 bootstrap for Zed entry points.

Python on Windows has two long-standing text-encoding footguns:

1. ``sys.stdout`` / ``sys.stderr`` are bound to the console code page
   (``cp1252`` on US-locale installs), so ``print("cafÃ©")`` crashes with
...
2. Child processes spawned via ``subprocess`` don't know to use UTF-8
   ...
   environment â€” so any Python subprocess (the execute_code sandbox,
   delegation children, linter subprocesses, etc.) inherits the same
   cp1252 defaults and hits the same UnicodeEncodeError.

This module fixes both on Windows *only* â€” POSIX is untouched.  It
should be imported at the very top of every Zed entry point
(``zed``, ``zed-agent``, ``zed-acp``, ``python -m gateway.run``,
...
What this module does on POSIX:
  ...
  - Nothing.  POSIX systems are already UTF-8 by default in 99% of cases,
    ...
    Linux, they can export ``PYTHONUTF8=1`` themselves â€” we won't override.
...
def apply_windows_utf8_bootstrap() -> bool:
    ...
    advisory â€” callers normally don't need it, but tests may want to
    ...
    #    â€” those were bound at interpreter startup based on the console
    ...
    for stream_name in ("stdout", "stderr"):
        ...
        if reconfigure is None:
            ...
            # Skip silently â€” the env-var fix is still in effect for
    ...
    # stdin is reconfigured separately with errors="replace" too â€” input
...
# Apply on import â€” entry points just need ``import zed_bootstrap``
# (or ``from zed_bootstrap import apply_windows_utf8_bootstrap``) at
