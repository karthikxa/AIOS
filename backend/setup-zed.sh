...
# 5. Symlinks the 'zed' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}âš• Zed Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/zed-uv-install.$$.log")"
...
if [ ! -f ".env" ]; then
    if [ -f ".env.example" ]; then
        cp .env.example .env
        # .env holds API keys â€” restrict to owner-only access (matches
...
# ============================================================================
# PATH setup â€” symlink zed into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}â†’${NC} Setting up zed command..."

ZED_BIN="$SCRIPT_DIR/venv/bin/zed"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ZED_BIN" "$COMMAND_LINK_DIR/zed"
echo -e "${GREEN}âœ“${NC} Symlinked zed â†’ $COMMAND_LINK_DISPLAY_DIR/zed"
...
# ============================================================================
# Seed bundled skills into ~/.zed/skills/
# ============================================================================

ZED_SKILLS_DIR="${ZED_HOME:-$HOME/.zed}/skills"
mkdir -p "$ZED_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.zed/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    ...
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ZED_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  zed status        # Check configuration"
if is_termux; then
    echo "  zed gateway       # Run gateway in foreground"
else
    echo "  zed gateway install # Install gateway service (messaging + cron)"
fi
echo "  zed cron list     # View scheduled jobs"
echo "  zed doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m zed_cli.main setup
fi
