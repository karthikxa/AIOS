 ...
 *
 * Plugins call window.__ZED_PLUGINS__.register(name, Component)
...
// ---------------------------------------------------------------------------
// Plugin registry â€” plugins call register() to add their component.
// ---------------------------------------------------------------------------
 ...
 * Exposed at runtime as ``window.__ZED_PLUGIN_SDK__.sdkVersion`` so a
 * plugin (or a future host-side compatibility gate) can read it.
 */
export const SDK_CONTRACT_VERSION = "1.1.0";

// Window globals for the plugin SDK are declared in ``plugins/sdk.d.ts`` â€”
...
export function exposePluginSDK() {
  window.__ZED_PLUGINS__ = {
    register: registerPlugin,
    registerSlot,
  };

  window.__ZED_PLUGIN_SDK__ = {
    ...
    // React core â€” plugins use these instead of importing react
    React,
    hooks: {
      useState,
      useEffect,
    ...
    // window.__ZED_SESSION_TOKEN__ directly.
    authedFetch,
    ...
    // UI components â€” Nous DS where available, shadcn/ui primitives elsewhere.
    components: {
      Card,
      CardHeader,
      CardTitle,
