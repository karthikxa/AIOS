/**
 * usePlugins hook â€” discovers and loads dashboard plugins.
 *
...
import { api, ZED_BASE_PATH } from "@/lib/api";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${ZED_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${ZED_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?zed_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-zed-plugin", manifest.name);
      script.src = scriptSrc;
      script.async = true;
      // SRI integrity verification â€” defense against compromised plugin
