 ...
 * `window.__ZED_PLUGINS__.registerSlot(pluginName, slotName, Component)`
 * from their JS bundle. Multiple plugins can populate the same slot â€” they
 ...
 *
 *  Shell-wide slots:
 *  - `backdrop`         â€” rendered inside `<Backdrop />`, above the noise layer
 *  - `header-left`      â€” injected before the Zed brand in the top bar
 *  - `header-right`     â€” injected before the theme/language switchers
 *  - `header-banner`    â€” injected below the top nav bar, full-width
 *  - `sidebar`          â€” the cockpit sidebar rail (only rendered when
 *                         `layoutVariant === "cockpit"`)
 *  - `pre-main`         â€” rendered above the route outlet (inside `<main>`)
 *  - `post-main`        â€” rendered below the route outlet (inside `<main>`)
 *  - `footer-left`      â€” replaces the left footer cell content
 *  - `footer-right`     â€” replaces the right footer cell content
 *  - `overlay`          â€” fixed-position layer above everything else;
 *                         useful for chrome (scanlines, vignettes) the
 *                         theme's customCSS can't achieve alone
 *
 *  Page-scoped slots (rendered inside a specific built-in page â€” use these
 *  to inject widgets, cards, or toolbars into existing pages without
 *  overriding the whole route):
 *  - `sessions:top`     â€” top of /sessions page (above session list)
 *  - `sessions:bottom`  â€” bottom of /sessions page
 *  - `analytics:top`    â€” top of /analytics page
 *  - `analytics:bottom` â€” bottom of /analytics page
 *  - `logs:top`         â€” top of /logs page (above filter toolbar)
 *  - `logs:bottom`      â€” bottom of /logs page (below log viewer)
 *  - `cron:top`         â€” top of /cron page
 *  - `cron:bottom`      â€” bottom of /cron page
 *  - `skills:top`       â€” top of /skills page
 *  - `skills:bottom`    â€” bottom of /skills page
 *  - `plugins:top`       â€” top of /plugins page
 *  - `plugins:bottom`    â€” bottom of /plugins page
 *  - `config:top`       â€” top of /config page
 *  - `config:bottom`    â€” bottom of /config page
 *  - `env:top`          â€” top of /env (Keys) page
 *  - `env:bottom`       â€” bottom of /env (Keys) page
 *  - `docs:top`         â€” top of /docs page (above the docs iframe)
 *  - `docs:bottom`      â€” bottom of /docs page
 *  - `chat:top`         â€” top of /chat page (above the composer, when embedded chat is on)
 *  - `chat:bottom`      â€” bottom of /chat page
 */
 ...
 *  `window.__ZED_PLUGINS__.registerSlot(...)`.
 *
 *  If the same (plugin, slot) pair is registered twice, the later call
 *  replaces the earlier one â€” this matches how React HMR expects plugin
 *  re-mounts to behave. */
 ...
 *  plugin reload flows â€” not wired in by default. */
