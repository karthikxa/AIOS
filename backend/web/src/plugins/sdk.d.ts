/**
 * Zed Dashboard Plugin SDK â€” typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__ZED_PLUGIN_SDK__`` and
 * ``window.__ZED_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__ZED_PLUGIN_SDK__`` because:
 *   1. The runtime object is assembled from many internal modules
 *      (``@/lib/api``, ``@nous-research/ui``, â€¦). Deriving the type would
 ...
 *   2. A hand-authored contract is the *versioned API boundary* â€” changing
 *      it is a deliberate act, visible in review, not an accidental
 *      consequence of refactoring an internal helper.
 *
 * Versioning: bump ``ZedPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@zed/dashboard-plugin-sdk`` types package, or
 ...
 * 401 â†’ /login redirect in gated mode. Use for all JSON plugin endpoints.
 */
 ...
 * of calling ``fetch`` with a hand-read ``window.__ZED_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__ZED_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__ZED_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface ZedPluginSDK {
  /** Contract version of this SDK surface (see SDK_CONTRACT_VERSION). */
  readonly sdkVersion: string;

  /** React core â€” use instead of importing/bundling react. */
   ...
   * at the boundary (methods vary in arity and return type â€” most return
...
declare global {
  interface Window {
    __ZED_PLUGIN_SDK__?: ZedPluginSDK;
    __ZED_PLUGINS__?: PluginRegistry;
  }
}
