...
interface ZEDConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function ZEDConsoleModal({ open, onClose }: ZEDConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("ZED> ");
  const inputPromptRef = useRef("ZED> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "ZED> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to ZED Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="ZED-console-title"
    >
      <div
        className={cn(
          themedBody,
