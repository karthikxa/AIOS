 ...
 *
 *   1. `palette`       â€” the 3-layer color triplet (background/midground/
 ...
 *   2. `typography`    â€” font families, base font size, line height,
 ...
 *   3. `layout`        â€” corner radius and density (spacing multiplier).
 *
...
/** A color layer: hex base + alpha (0â€“1). */
...
export interface ThemePalette {
  ...
  /** Top-layer highlight. In LENS_0 this is white @ alpha 0 â€” invisible by
   *  default but still drives `--color-ring`-style accents. */
  foreground: ThemeLayer;
  /** Warm vignette color for <Backdrop />, as an rgba() string. */
  warmGlow: string;
  /** Scalar multiplier (0â€“1.2) on the noise overlay. Lower for softer themes
   *  like Mono and RosÃ©, higher for grittier themes like Cyberpunk. */
  noiseOpacity: number;
}
 ...
 *  plugin-facing â€” a cockpit sidebar plugin reads `--theme-asset-hero`
...
export interface ThemeAssets {
  ...
  bg?: string;
  /** Hero render (Gundam, mascot, wallpaper) â€” for plugin sidebars/overlays. */
  hero?: string;
  /** Logo mark â€” header slot consumers use this. */
  logo?: string;
  /** Faction/brand crest â€” header-left decoration. */
  crest?: string;
 ...
 *  strings â€” we don't parse them, so themes can use `clip-path`,
 ...
 *  `index.css` (Zed-teal `#ffe6cb` for input, `#34d399` for output).
 *
 ...
 *  difference layer flips them (`out = 255 âˆ’ channel`). E.g. to make
 *  output paint as Nous-blue `#0053FD` on screen, set
 *  `outputTokenAccent: "#FFAC02"` â€” the difference math reverses it. */
...
export interface DashboardTheme {
   ...
   *  palette's raw hex values don't reflect what users see on screen â€”
 ...
 *
 * The `themes` list is intentionally partial â€” built-in themes are fully
...
export interface ThemeListEntry {
   ...
   *  `~/.zed/dashboard-themes/*.yaml`; undefined for built-ins (the
