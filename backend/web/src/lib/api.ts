...
// (e.g. https://mission-control.tilos.com/zed/). The Python backend
// injects ``window.__ZED_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__ZED_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const ZED_BASE_PATH = readBasePath();
const BASE = ZED_BASE_PATH;

import type { DashboardTheme } from "@/themes/types";

// Ephemeral session token for protected endpoints.
// Injected into index.html by the server â€” never fetched via API.
declare global {
  interface Window {
    __ZED_SESSION_TOKEN__?: string;
    __ZED_BASE_PATH__?: string;
    ...
    __ZED_AUTH_REQUIRED__?: boolean;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-Zed-Session-Token";
...
// â”€â”€ Global management-profile scope â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// (e.g. ProfileBuilder writes) are left untouched â€” explicit beats global.
...
// _profile_scope or explicit per-profile DB opens). Anything else â€” ops,
// pairing, cron (which has its own per-job profile params), profiles
// themselves â€” is machine-global or self-scoped and must NOT be rewritten.
...
export async function fetchJSON<T>(
  ...
  const token = window.__ZED_SESSION_TOKEN__;
  if (token) {
  ...
  const res = await fetch(`${BASE}${url}`, {
    ...
    // for any fetch routed through here. Loopback mode is unaffected â€” the
  ...
  if (res.status === 401) {
    ...
    // and only redirect on the known error codes â€” domain-level 401s
    ...
    try {
      ...
      /* non-JSON 401 â€” let it fall through */
    }
    if (
      ...
      // SPA route the gate never saw â€” e.g. a hash route or a client-side
      ...
      try {
        ...
        /* SSR / privacy mode â€” ignore */
      }
      window.location.assign(body.login_url);
      // Never resolve â€” the page is about to unload.
      return new Promise<T>(() => {});
    }
    // Loopback mode: ``_SESSION_TOKEN`` rotates on every server restart
    // (``zed update``, ``zed gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__ZED_SESSION_TOKEN__`` from the previous HTML render,
    ...
    // that reload once on the first stale-token 401 â€” gated mode is
    // handled above, so reaching here in gated mode means a real
    // middleware failure that should not reload-loop.
    if (!window.__ZED_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("zed.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode â€” fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("zed.tokenReloadAttempted", "1");
        } catch {
          /* SSR / privacy mode â€” best effort */
        }
  ...
  if (res.ok) {
    ...
    // current ``window.__ZED_SESSION_TOKEN__`` is valid, so the next
    // 401 â€” if any â€” should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("zed.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode â€” ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__ZED_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available â€” page must be served by the Zed dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``zed_cli.web_server._ws_auth_ok``)
 ...
 *
 * Tickets are single-use and TTL=30s â€” every WS connect attempt must
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__ZED_AUTH_REQUIRED__) {
 ...
 * plain JSON â€” file uploads (``FormData``), binary downloads (blobs), etc.
 ...
 *  - loopback / ``--insecure``: attach the ``X-Zed-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``zed_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 * Unlike ``fetchJSON`` this does NOT parse the body, does NOT throw on
 * non-2xx (the caller decides â€” a 404 on a download is meaningful), and
 * does NOT run the global 401 â†’ /login redirect (binary endpoints aren't
 ...
 * ``window.__ZED_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
   ...
   * valid cookie is attached. Loopback mode is unaffected â€” the endpoint
  ...
  logout: () =>
    fetch(`${BASE}/auth/logout`, {
      ...
      // /auth/logout returns 302 â†’ /login. Follow that with a full-page
      // navigation rather than letting fetch() opaquely consume the
      // redirect â€” the SPA needs to leave the protected area.
  ...
  uploadFile: (path: string, file: File, overwrite = true) => {
    // Stream the raw bytes as multipart/form-data. Do NOT set Content-Type â€”
  ...
  // Automation Blueprints â€” parameterized automation blueprints
  ...
  // any profile's skills/toolsets â€” not just the one the dashboard process
  ...
  updateZed: () =>
    fetchJSON<ActionResponse>("/api/zed/update", { method: "POST" }),
  checkZedUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/zed/update/check${force ? "?force=true" : ""}`,
    ),
  ...
  // â”€â”€ Admin: MCP servers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Admin: Pairing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Admin: Webhooks â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Admin: Credential pool â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  getCredentialPool: () =>
  ...
  // â”€â”€ Admin: Memory provider â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Admin: Gateway lifecycle â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  startGateway: () =>
  ...
  // â”€â”€ Admin: Curator â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Admin: Diagnostics (backgrounded) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  runPromptSize: () =>
  ...
  // â”€â”€ Admin: Skills hub â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
 ...
 * Nous Portal contract V1 (the access token has no email/name claims â€”
...
export interface SkillHubSource {
  ...
  /** zed-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
// â”€â”€ Admin types â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export interface McpCatalogEntry {
  ...
  // Transport details â€” what actually connects (http) or runs (stdio).
  command: string | null;
  args: string[];
  url: string | null;
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``zed update``. */
  can_update_zed?: boolean;
  config_path: string;
...
export interface AutomationBlueprintField {
  ...
  options: string[];
  optional: boolean;
  /** When false, options are suggestions â€” any value is accepted. */
  strict?: boolean;
  help: string;
}
...
// â”€â”€ Model info types â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Model options / assignment types â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ OAuth provider types â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/** Discriminated union â€” the shape of /start depends on the flow. */
...
// â”€â”€ Dashboard theme types â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Dashboard plugin types â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
