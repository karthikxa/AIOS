...
import { ZED_BASE_PATH, getWsTicket } from "@/lib/api";
...
export class GatewayClient {
  ...
  async connect(token?: string): Promise<void> {
    ...
    } else if (window.__ZED_AUTH_REQUIRED__) {
      const { ticket } = await getWsTicket();
      authParamName = "ticket";
      authParamValue = ticket;
    } else {
      authParamName = "token";
      authParamValue = window.__ZED_SESSION_TOKEN__ ?? "";
      if (!authParamValue) {
        this.setState("error");
        throw new Error(
          "Session token not available â€” page must be served by the Zed dashboard",
        );
      }
    }
    ...
    const ws = new WebSocket(
      `${scheme}//${location.host}${ZED_BASE_PATH}/api/ws?${authParamName}=${encodeURIComponent(authParamValue)}`,
    );
    this.ws = ws;

    // Register message + close BEFORE awaiting open â€” the server emits
