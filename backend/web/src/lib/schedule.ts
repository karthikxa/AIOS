 ...
 *
 * The zed-agent backend (cron/jobs.py::parse_schedule) accepts a
...
/** Picker modes â€” each renders a different set of inputs in the UI but
...
export interface ScheduleBuilderState {
   ...
   * still valid â€” we send "*" for the day-of-week cron field. */
  weekdays: Weekday[];

  /** Monthly mode: 1..31 (no support for "last day of month" sugar â€” the
...
/** Default state â€” "every 30 minutes" is the most-common-cron-pattern
 ...
 * state is incomplete enough that the backend would 400 â€” the caller
...
export function buildScheduleString(state: ScheduleBuilderState): string {
  switch (state.mode) {
    ...
    case "weekly": {
      const parsed = parseTimeOfDay(state.timeOfDay);
      if (!parsed) return "";
      // Empty weekday selection â†’ "*" (every day) rather than a backend
...
export interface ScheduleDescribeStrings {
  ...
  none: string;
  /** "Every {n} minute(s)" â€” caller pluralises via {n}. */
  everyMinutes: string;
  everyHours: string;
  everyDays: string;
  /** "Daily at {time}" */
...
export function describeSchedule(
  ...
  // Try the raw expression as a last attempt â€” for legacy jobs stored
 ...
 * "humanize" â€” caller falls back to displaying the raw expression so
...
function describeCronExpression(
  ...
  // Star minutes/hours would mean "every minute" / "every hour" â€” we'd
  ...
  // OR-semantics for that combo are confusing â€” fall back to raw.
  return null;
}
