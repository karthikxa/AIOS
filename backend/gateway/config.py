...
class SessionResetPolicy:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SessionResetPolicy":
        # Handle both missing keys and explicit null values (YAML null â†’ None)
        mode = data.get("mode")
...
class PlatformConfig:
    ...
    # Whether the gateway is allowed to send "â™»ï¸ Gateway online" /
    # "â™» Gateway restarted" lifecycle notifications on this platform.
...
# Streaming defaults â€” single source of truth so both StreamingConfig and
...
DEFAULT_STREAMING_CURSOR: str = " â–‰"
...
class StreamingConfig:
    ...
    enabled: bool = False
    # Transport selection:
    #   "auto"  â€” prefer native streaming-draft updates when the platform
    #             supports them (Telegram sendMessageDraft, Bot API 9.5+);
    #             fall back to edit-based when not.
    #   "draft" â€” explicitly request native drafts; falls back to edit when
    #             the platform/chat doesn't support them.
    #   "edit"  â€” progressive editMessageText only (legacy behaviour).
    #   "off"   â€” disable streaming entirely.
    #
    ...
    # â€¦) report supports_draft_streaming() == False and transparently use the
    # edit path â€” so "auto" never regresses non-Telegram platforms, it only
...
_PLATFORM_CONNECTED_CHECKERS: dict[Platform, Callable[[PlatformConfig], bool]] = {
    ...
    # signal in the experimental phase. EXPERIMENTAL â€” may change.
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_zed_home() / "sessions")
    
    # Delivery settings
    always_log_local: bool = True  # Always save cron outputs to local files
    # Drop outbound "silence narration" messages (e.g. *(silent)*, ðŸ”‡, a bare
    ...
    # gateway behaves exactly as before â€” single ZED_HOME, no profile stamping.
    ...
    # months.  Pruning is invisible to users â€” if they resume, they get a
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.zed/config.yaml (primary user-facing config)
    3. ~/.zed/gateway.json (legacy â€” provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_zed_home()
    gw_data: dict = {}
    ...
    if gateway_json_path.exists():
        try:
            ...
            logger.info(
                "Loaded legacy %s â€” consider moving settings to config.yaml",
                gateway_json_path,
            )
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through zed_cli.config.load_config, so without this a managed
            ...
            # Map config.yaml keys â†’ GatewayConfig.from_dict() schema.
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``zed config set gateway.streaming.*``
            ...
            for plat in _shared_loop_targets:
                ...
                # require_mention, free_response_channels, â€¦) still runs when
            ...
            # Plugin-owned YAMLâ†’env config bridges (#24836).  See
            # ``PlatformEntry.apply_yaml_config_fn`` for the hook contract.
            # Order: shared-key loop (above) â†’ this dispatch â†’ legacy hardcoded
            # blocks (below; no-op when a hook already set their env var) â†’
            ...
            # Slack settings â†’ env vars (env vars take precedence)
            ...
            # Telegram settings â†’ env vars (env vars take precedence)
            ...
            # Signal settings â†’ env vars (env vars take precedence)
            ...
            # DingTalk settings â†’ env vars (env vars take precedence)
            ...
            # adapter.py::_apply_yaml_config â€” see #25443 (apply_yaml_config_fn).

            # Matrix settings â†’ env vars (env vars take precedence)
            ...
            # Feishu settings â†’ env vars (env vars take precedence)
        ...
        logger.warning(
            "Failed to process config.yaml â€” falling back to .env / gateway.json values. "
...
def _apply_env_overrides(config: GatewayConfig) -> None:
    ...
    if Platform.WHATSAPP in config.platforms:
        # YAML config exists â€” respect explicit disable
    ...
    if whatsapp_cloud_phone_id and whatsapp_cloud_token:
        ...
        # Webhook verify token â€” Meta hub.verify_token shared secret
    ...
    if slack_token:
        if Platform.SLACK not in config.platforms:
            # No yaml config for Slack â€” env-only setup, enable it
    ...
    # Yuanbao â€” YUANBAO_APP_ID preferred
    ...
    # their PlatformEntry â€” called here BEFORE adapter construction.
    #
    ...
    # ``check_fn`` alone â€” which for adapter plugins typically just
    # verifies the SDK is importable / lazy-installs it â€” silently enables
    ...
    try:
        ...
        for entry in platform_registry.plugin_entries():
            ...
            # env-var bridge enabled it â€” keep that decision).
            if existing_cfg is None or not existing_cfg.enabled:
                if entry.is_connected is not None:
                    try:
                        ...
                        logger.debug(
                            "is_connected for %s raised: %s â€” skipping enablement",
                            entry.name, exc,
                        )
                        configured = False
                    if not configured:
                        logger.debug(
                            ...
                            "(is_connected returned False) â€” skipping enable",
                            entry.name,
                        )
                        continue
            if platform not in config.platforms:
    ...
    # and dials OUT to the connector â€” so, like Telegram/Matrix, it has no public
