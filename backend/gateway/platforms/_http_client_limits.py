...
drains â€” which, multiplied across 7 long-lived adapters plus the LLM
...
* ``max_keepalive_connections=10`` â€” plenty for any single adapter;
  platform APIs rarely parallelise beyond this.
* ``keepalive_expiry=2.0`` â€” close idle sockets aggressively so a
  proxy's lingering CLOSE_WAIT window can't starve the process.

Override via ``ZED_GATEWAY_HTTPX_KEEPALIVE_EXPIRY`` /
``ZED_GATEWAY_HTTPX_MAX_KEEPALIVE`` env vars when tuning under load.
"""
...
def platform_httpx_limits() -> "httpx.Limits | None":
    ...
    keepalive_expiry = _env_float(
        "ZED_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", _DEFAULT_KEEPALIVE_EXPIRY_S
    )
    max_keepalive = _env_int(
        "ZED_GATEWAY_HTTPX_MAX_KEEPALIVE", _DEFAULT_MAX_KEEPALIVE
    )

    return httpx.Limits(
        max_keepalive_connections=max_keepalive,
        # Leave max_connections at httpx default (100) â€” plenty of headroom.
