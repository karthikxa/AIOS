...
# DingTalk message type â†’ runtime content type
...
class DingTalkAdapter(BasePlatformAdapter):
    ...
    def REQUIRES_EDIT_FINALIZE(self) -> bool:  # noqa: N802
        ...
        are configured â€” webhook-only DingTalk doesn't need it.
        """
        return bool(self._card_template_id and self._card_sdk)
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Chats for which we've already fired the Done reaction â€” prevents
        ...
        # allows streaming_update on a finalized card â€” it flips back to
        # streaming).  We track those reopened cards so the next `send()` can
        # auto-close them as siblings â€” otherwise tool-progress cards get
    ...
    def _should_process_message(self, message: "ChatbotMessage", text: str, is_group: bool, chat_id: str) -> bool:
        ...
        When ``allowed_chats`` is non-empty, it acts as a hard gate â€” messages
    ...
    def _fire_done_reaction(self, chat_id: str) -> None:
        """Swap ðŸ¤”Thinking â†’ ðŸ¥³Done on the original user message.

        Idempotent per chat_id â€” safe to call from segment-break flushes
        ...
        async def _swap() -> None:
            await self._send_emotion(
                msg_id, conversation_id, "ðŸ¤”Thinking", recall=True,
            )
            await self._send_emotion(
                msg_id, conversation_id, "ðŸ¥³Done", recall=False,
            )
    ...
    async def _on_message(
        ...
        # gets its own Thinkingâ†’Done cycle.
        if chat_id:
    ...
    def _extract_text(message: "ChatbotMessage") -> str:
        ...
        Handles both legacy and current dingtalk-stream SDK payload shapes:
          ...
          * >= 0.20: ``message.text`` is a ``TextContent`` dataclass whose
            ``__str__`` returns ``"TextContent(content=...)"`` â€” never fall
        ...
        # LLM see the raw text â€” it handles "@bot hello" cleanly.
        return content
                        ...
                        if dl_code:
                            ...
                            if mapped == "image":
                                ...
                                if msg_type == MessageType.TEXT:
                                    ...
                                    # native voice note â€” route through STT.
    ...
    async def send(
        ...
        # to an inbound user message â€” only `base.py:_send_with_retry` sets
        ...
        #      updates don't flicker closedâ†’streamingâ†’closed repeatedly).
        ...
        if self._card_template_id and current_message and self._card_sdk:
            ...
            # before creating a new one (handles tool-progress â†’ final-
            ...
            if result and result.success:
                if is_final_reply:
                    # Final reply: card closed, swap Thinking â†’ Done.
        ...
        payload = {
            "msgtype": "markdown",
            "markdown": {"title": "Zed", "text": normalized},
        }
    ...
    async def _create_and_stream_card(
        ...
        *,
        ...
        try:
            ...
            out_track_id = f"zed_{uuid.uuid4().hex[:12]}"
        ...
        *,
        ...
        try:
            ...
            if finalize:
                ...
                # Non-final edit reopens the card into streaming state â€”
...
class _IncomingHandler(
    ...
    async def process(self, message: "CallbackMessage"):
        ...
        method returns the ACK immediately â€” blocking here would prevent the
        ...
        try:
            ...
            # Thinking reaction â€” fire-and-forget, tracked
            if msg_id and conversation_id:
                self._adapter._spawn_bg(
                    self._adapter._send_emotion(
                        msg_id, conversation_id, "ðŸ¤”Thinking", recall=False,
                    )
                )
