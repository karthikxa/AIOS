        ...
        for elem in elements:
            ...
            if elem_type == "rich_text_section":
                ...
                for idx, item in enumerate(elem.get("elements", [])):
                    item_bullet = "â€¢ " if list_style == "bullet" else f"{idx + 1}. "
...
class SlackAdapter(BasePlatformAdapter):
    ...
    # the prefix that works everywhere â€” instruction text must show it.
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._team_clients: Dict[str, Any] = {}  # team_id â†’ WebClient
        ...
        # clear them (chat_id â†’ thread_ts).
    ...
    async def _socket_watchdog_loop(self) -> None:
        ...
        disable self-healing â€” the loop logs and keeps polling.
        """
        while self._running:
            try:
    ...
    _SLASH_CTX_TTL = 120.0  # seconds â€” response_url is valid for 30 min;
    ...
    def _pop_slash_context(
        ...
        from a non-slash code path â€” should not match anything).
        """
        now = time.monotonic()
        # Clean up stale entries on every lookup â€” dict is small.
        stale_keys = [
            k
    ...
    async def _send_slash_ephemeral(
        ...
        lets us swap the "Running /cmdâ€¦" placeholder with the real reply,
        and the message stays ephemeral ("Only visible to you").

        Falls back to a simple ``True`` SendResult if the POST fails â€”
        ...
        # Truncate to MAX_MESSAGE_LENGTH and use only the first chunk â€” the
        ...
        # Non-fatal â€” the user saw the initial ack already.
        return SendResult(success=True, message_id=None)
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_zed_home() / "slack_tokens.json"
        ...
        try:
            ...
            # responses â€” the same bug that affected DiscordAdapter (#18187).
            await self._stop_socket_mode_handler()
            self._app = None
            ...
            # @mention, they share the same event ts â€” the dedup in
            ...
            # Zed does not currently need to route them into the agent loop.
            ...
            # /model work directly without /zed prefix). A single regex
            ...
            # manifest (see `zed slack manifest`). In Socket Mode, Slack
            ...
            from zed_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/zed$")

            @self._app.command(_slash_pattern)
            async def handle_zed_command(ack, command):
                slash = (command.get("command") or "").lstrip("/")
                await ack(
                    response_type="ephemeral",
                    text=f"Running `/{slash}`â€¦",
                )
            ...
            # Closure factory â€” keeps the wrapper's signature limited to
    ...
    async def create_handoff_thread(
        ...
        channel and return its ``ts`` â€” the watcher uses that as the
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Zed handoff â€” *{(name or 'session').strip()[:80]}*"
            )
    ...
    async def send(
        ...
        try:
            ...
            # already showed an ephemeral "Running /cmdâ€¦" message.  If we have
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Zed session â€” matching the per-thread behavior channels already
    ...
    def format_message(self, content: str) -> str:
        ...
        #     Single *text* â†’ _text_ (Slack italic), but only when the
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if slack_attachments:
            ...
            for att in slack_attachments:
                ...
                if att_title and att_url:
                    header = f"ðŸ“Ž [{att_title}]({att_url})"
                elif att_title:
                    header = f"ðŸ“Ž {att_title}"
                elif att_url:
                    header = f"ðŸ“Ž {att_url}"
                else:
                    header = None
        ...
        if is_dm:
            ...
            #
            # Three cases:
            #   (a) genuine thread reply   â†’ scope session per thread
            #   (b) top-level, reply_in_thread=true (the default)  â†’
            ...
            if event_thread_ts_raw and event_thread_ts_raw != ts:
                ...
                # reply_in_thread=false: no thread key â†’ session manager
        ...
        if not is_dm and bot_uid:
            # Check allowed channels â€” if set, only respond in these channels (whitelist)
    ...
    async def send_exec_approval(
        ...
        try:
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "zed_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "zed_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "zed_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
            ...
            kwargs: Dict[str, Any] = {
                "channel": chat_id,
                "text": f"âš ï¸ Command approval required: {cmd_preview[:100]}",
                "blocks": blocks,
            }
            if thread_ts:
                kwargs["thread_ts"] = thread_ts
    ...
    async def send_slash_confirm(
        ...
        try:
            ...
            # the block over the limit (overflow â†’ invalid_blocks â†’ no buttons).
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "zed_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "zed_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        # Authorization â€” reuse the exec-approval allowlist.
        allowed_csv = ""  # Interactive auth already ran above.
        if allowed_csv:
            allowed_ids = {uid.strip() for uid in allowed_csv.split(",") if uid.strip()}
            if "*" not in allowed_ids and user_id not in allowed_ids:
                logger.warning(
                    "[Slack] Unauthorized slash-confirm click by %s (%s) â€” ignoring",
                    user_name,
                    user_id,
                )
                return
        ...
        choice_map = {
            "zed_confirm_once": "once",
            "zed_confirm_always": "always",
            "zed_confirm_cancel": "cancel",
        }
        choice = choice_map.get(action_id, "cancel")

        label_map = {
            "once": f"âœ… Approved once by {user_name}",
            "always": f"ðŸ”’ Always approved by {user_name}",
            "cancel": f"âŒ Cancelled by {user_name}",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "zed_approve_once": "once",
            "zed_approve_session": "session",
            "zed_approve_always": "always",
            "zed_deny": "deny",
        }
        ...
        label_map = {
            "once": f"âœ… Approved once by {user_name}",
            "session": f"âœ… Approved for session by {user_name}",
            "always": f"âœ… Approved permanently by {user_name}",
            "deny": f"âŒ Denied by {user_name}",
        }
        decision_text = label_map.get(choice, f"Resolved by {user_name}")
    ...
    async def _fetch_thread_parent_text(
        ...
        Returns empty string on any failure â€” callers should treat an empty
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/zed <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/zed
        what's the weather`` â€” non-slash text is treated as a regular
        message).
        """
        ...
        if slash_name in {"zed", ""}:
            # Legacy /zed <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from zed_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/zed   ``).
            ...
            # Native slash â€” /<slash_name> [args].  Route directly through the
        ...
        # "Running /cmdâ€¦" ack shown by handle_zed_command).
        # Only stash for COMMAND events (text starts with "/") â€” free-form
        # questions via "/zed <question>" must produce public replies so
    ...
    # â”€â”€ Channel mention gating â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _slack_allowed_channels(self) -> set:
        ...
        ignored â€” even if the bot is @mentioned.  DMs are never filtered.
