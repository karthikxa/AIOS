...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.zed to the active profile home."""
    ...
    if raw == "~/.zed":
        return zed_home
    if raw.startswith("~/.zed/"):
        return zed_home / raw.removeprefix("~/.zed/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return zed_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under ZED_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from hermes_constants import get_zed_home

    scripts_root = (get_zed_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.zed" or raw_text.startswith("~/.zed/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__zed_ignore__") is True
        ):
            return False, None
