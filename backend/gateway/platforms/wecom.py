...
class WeComAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("ZED_WECOM_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("ZED_WECOM_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
    ...
    async def _on_message(self, payload: Dict[str, Any]) -> None:
        ...
        # this chat can fall back to APP_CMD_RESPONSE (required for groups â€”
    ...
    async def _flush_text_batch(self, key: str) -> None:
        ...
        try:
            ...
            # This check is synchronous â€” no await between the sleep and
            # the pop â€” so no other coroutine can modify the task registry
            # in between.
    ...
    def _remember_chat_req_id(self, chat_id: str, req_id: str) -> None:
        ...
        without an explicit ``reply_to`` â€” WeCom AI Bots are blocked from
    ...
    def _apply_file_size_limits(file_size: int, detected_type: str, content_type: Optional[str] = None) -> Dict[str, Any]:
        ...
        if file_size > ABSOLUTE_MAX_BYTES:
            return {
                ...
                "rejected": True,
                "reject_reason": (
                    f"æ–‡ä»¶å¤§å° {file_size_mb:.2f}MB è¶…è¿‡äº†ä¼ä¸šå¾®ä¿¡å…è®¸çš„æœ€å¤§é™åˆ¶ 20MBï¼Œæ— æ³•å‘é€ã€‚"
                    "è¯·å°è¯•åŽ‹ç¼©æ–‡ä»¶æˆ–å‡å°æ–‡ä»¶å¤§å°ã€‚"
                ),
                "downgraded": False,
                "downgrade_note": None,
            }
        ...
        if normalized_type == "image" and file_size > IMAGE_MAX_BYTES:
            return {
                ...
                "reject_reason": None,
                "downgraded": True,
                "downgrade_note": f"å›¾ç‰‡å¤§å° {file_size_mb:.2f}MB è¶…è¿‡ 10MB é™åˆ¶ï¼Œå·²è½¬ä¸ºæ–‡ä»¶æ ¼å¼å‘é€",
            }
        ...
        if normalized_type == "video" and file_size > VIDEO_MAX_BYTES:
            return {
                ...
                "reject_reason": None,
                "downgraded": True,
                "downgrade_note": f"è§†é¢‘å¤§å° {file_size_mb:.2f}MB è¶…è¿‡ 10MB é™åˆ¶ï¼Œå·²è½¬ä¸ºæ–‡ä»¶æ ¼å¼å‘é€",
            }
        ...
        if normalized_type == "voice":
            if normalized_content_type and normalized_content_type not in VOICE_SUPPORTED_MIMES:
                return {
                    ...
                    "downgraded": True,
                    "downgrade_note": (
                        f"è¯­éŸ³æ ¼å¼ {normalized_content_type} ä¸æ”¯æŒï¼Œä¼å¾®ä»…æ”¯æŒ AMR æ ¼å¼ï¼Œå·²è½¬ä¸ºæ–‡ä»¶æ ¼å¼å‘é€"
                    ),
                }
            if file_size > VOICE_MAX_BYTES:
                return {
                    ...
                    "reject_reason": None,
                    "downgraded": True,
                    "downgrade_note": f"è¯­éŸ³å¤§å° {file_size_mb:.2f}MB è¶…è¿‡ 2MB é™åˆ¶ï¼Œå·²è½¬ä¸ºæ–‡ä»¶æ ¼å¼å‘é€",
                }
...
_QR_CODE_PAGE = "https://work.weixin.qq.com/ai/qc/gen?source=zed&scode="
...
def qr_scan_for_bot_info(
    *,
    ...
    used here are not part of WeCom's public developer API â€” they back the
    ...
    generate_url = f"{_QR_GENERATE_URL}?source=zed"

    # â”€â”€ Step 1: Fetch QR code â”€â”€
    print("  Connecting to WeCom...", end="", flush=True)
    try:
        req = urllib.request.Request(generate_url, headers={"User-Agent": "ZedAgent/1.0"})
    ...
    # â”€â”€ Step 2: Render QR code in terminal â”€â”€
    print()
    qr_rendered = False
    try:
    ...
    # â”€â”€ Step 3: Poll for result â”€â”€
    ...
    while time.monotonic() < deadline:
        try:
            req = urllib.request.Request(query_url, headers={"User-Agent": "ZedAgent/1.0"})
