...
https://open.feishu.cn/document/home/user-identity-introduction/introduction):
  ...
  open_id  (ou_xxx)  â€” **App-scoped**.  The same person gets a different
                        open_id under each Feishu app.  Always available in
                        event payloads without extra permissions.
  user_id  (u_xxx)   â€” **Tenant-scoped**.  Stable within a company but
                        requires the ``contact:user.employee_id:readonly``
                        scope.  May not be present.
  union_id (on_xxx)  â€” **Developer-scoped**.  Same across all apps owned by
                        one developer/ISV.  Best cross-app stable ID.

For bots specifically:

  app_id              â€” The application's canonical credential identifier.
  bot open_id         â€” Returned by ``/bot/v3/info``.  This is the bot's own
...
In single-bot mode (what Zed currently supports), open_id works as a
...
# aiohttp/websockets are independent optional deps â€” import outside lark_oapi
...
_FEISHU_DEDUP_TTL_SECONDS = 24 * 60 * 60          # 24 hours â€” matches openclaw
...
_FEISHU_WEBHOOK_RATE_LIMIT_MAX = 120               # max requests per window per IP â€” matches openclaw
...
_FEISHU_WEBHOOK_ANOMALY_TTL_SECONDS = 6 * 60 * 60  # anomaly tracker TTL (6 hours) â€” matches openclaw
...
_FEISHU_REPLY_FALLBACK_CODES = frozenset({230011, 231003})  # reply target withdrawn/missing â†’ create fallback

# Feishu reactions render as prominent badges, unlike Discord/Telegram's
# small footer emoji â€” a success badge on every message would add noise, so
...
# Bound on the (message_id â†’ reaction_id) handle cache. Happy-path entries
...
class FeishuAdapterSettings:
    ...
    # Bot's own open_id (app-scoped) â€” returned by /bot/v3/info.  Used only for
    # @mention matching: Feishu puts this value in mentions[].id.open_id when
    # a user @-mentions the bot in a group chat.
    bot_open_id: str
    # Bot's user_id (tenant-scoped) â€” optional, used as fallback mention match.
    bot_user_id: str
    bot_name: str
...
def _sender_identity(sender: Any) -> frozenset:
    # Take any non-empty id variant â€” tenant sender_id_type decides which are populated.
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    # =========================================================================
    # Lifecycle â€” init / settings / connect / disconnect
    # =========================================================================
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._seen_message_ids: Dict[str, float] = {}  # message_id â†’ seen_at (time.time())
        self._seen_message_order: List[str] = []
        self._dedup_state_path = get_zed_home() / "feishu_seen_message_ids.json"
        self._dedup_lock = threading.Lock()
        self._sender_name_cache: Dict[str, tuple[str, float]] = {}  # sender_id â†’ (name, expire_at)
        self._webhook_rate_counts: Dict[str, tuple[int, float]] = {}  # rate_key â†’ (count, window_start)
        self._webhook_anomaly_counts: Dict[str, tuple[int, str, float]] = {}  # ip â†’ (count, last_status, first_seen)
        self._card_action_tokens: Dict[str, float] = {}  # token â†’ first_seen_time
        ...
        self._chat_locks: "collections.OrderedDict[str, asyncio.Lock]" = collections.OrderedDict()  # chat_id â†’ lock (per-chat serial processing, LRU-bounded)
        self._sent_message_ids_to_chat: Dict[str, str] = {}  # message_id â†’ chat_id (for reaction routing)
        ...
        # Exec approval button state (approval_id â†’ {session_key, message_id, chat_id})
        self._approval_state: Dict[int, Dict[str, str]] = {}
        self._approval_counter = itertools.count(1)
        # Update prompt button state (prompt_id â†’ {session_key, message_id, chat_id})
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        if isinstance(raw_group_rules, dict):
            for chat_id, rule_cfg in raw_group_rules.items():
                if not isinstance(rule_cfg, dict):
                    continue
                # Only override when the key is explicitly set â€” missing vs false
                # must not collapse.
        ...
        return FeishuAdapterSettings(
            ...
            dedup_cache_size=max(
                32,
                env_int("ZED_FEISHU_DEDUP_CACHE_SIZE", _DEFAULT_DEDUP_CACHE_SIZE),
            ),
            text_batch_delay_seconds=env_float(
                "ZED_FEISHU_TEXT_BATCH_DELAY_SECONDS", _DEFAULT_TEXT_BATCH_DELAY_SECONDS
            ),
            text_batch_split_delay_seconds=env_float(
                "ZED_FEISHU_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0
            ),
            text_batch_max_messages=max(
                1,
                env_int("ZED_FEISHU_TEXT_BATCH_MAX_MESSAGES", _DEFAULT_TEXT_BATCH_MAX_MESSAGES),
            ),
            text_batch_max_chars=max(
                1,
                env_int("ZED_FEISHU_TEXT_BATCH_MAX_CHARS", _DEFAULT_TEXT_BATCH_MAX_CHARS),
            ),
            media_batch_delay_seconds=env_float(
                "ZED_FEISHU_MEDIA_BATCH_DELAY_SECONDS", _DEFAULT_MEDIA_BATCH_DELAY_SECONDS
            ),
            webhook_host=str(
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local Zed gateway is already using this Feishu app_id"
    ...
    # =========================================================================
    # Outbound â€” send / edit / send_image / send_voice / â€¦
    # =========================================================================
    ...
    async def send_exec_approval(
        ...
        The buttons carry ``zed_action`` in their value dict so that
        ...
        try:
            ...
            def _btn(label: str, action_name: str, btn_type: str = "default") -> dict:
                return {
                    ...
                    "type": btn_type,
                    "value": {"zed_action": action_name, "approval_id": approval_id},
                }

            card = {
                "config": {"wide_screen_mode": True},
                "header": {
                    "title": {"content": "âš ï¸ Command Approval Required", "tag": "plain_text"},
                    "template": "orange",
                },
                "elements": [
                    {
                    ...
                    },
                    {
                        "tag": "action",
                        "actions": [
                            _btn("âœ… Allow Once", "approve_once", "primary"),
                            _btn("âœ… Session", "approve_session"),
                            _btn("âœ… Always", "approve_always"),
                            _btn("âŒ Deny", "deny", "danger"),
                        ],
                    },
                ],
            }
    ...
    def _build_update_prompt_card(*, prompt: str, default: str, prompt_id: int) -> Dict[str, Any]:
        ...
        def _btn(label: str, answer: str, btn_type: str) -> dict:
            return {
                ...
                "type": btn_type,
                "value": {
                    "zed_update_prompt_action": answer,
        ...
        return {
            "config": {"wide_screen_mode": True},
            "header": {
                "title": {"content": "âš• Update Needs Your Input", "tag": "plain_text"},
                "template": "orange",
            },
            "elements": [
    ...
    def _build_resolved_approval_card(*, choice: str, user_name: str) -> Dict[str, Any]:
        ...
        icon = "âŒ" if choice == "deny" else "âœ…"
    ...
    def _build_resolved_update_prompt_card(*, answer: str, user_name: str) -> Dict[str, Any]:
        ...
        return {
            "config": {"wide_screen_mode": True},
            "header": {
                "title": {"content": f"{'âœ…' if yes else 'âŒ'} Update prompt answered: {label}", "tag": "plain_text"},
    ...
    def _write_update_prompt_response(answer: str) -> None:
        response_path = get_zed_home() / ".update_response"
    ...
    def _enqueue_pending_inbound_event(self, data: Any) -> bool:
        ...
        with self._pending_inbound_lock:
            if len(self._pending_inbound_events) >= self._pending_inbound_max_depth:
                # Queue full â€” drop the oldest to make room. This happens only
        ...
        try:
            while True:
                if not getattr(self, "_running", True):
                    # Adapter shutting down â€” drop queued events rather than
                ...
                if self._loop_accepts_callbacks(loop):
                    ...
                    for event in batch:
                        if self._submit_on_loop(
                            ...
                            # Loop closed/unavailable â€” requeue and poll again.
                    ...
                    if not requeue:
                        ...
                        with self._pending_inbound_lock:
                            if not self._pending_inbound_events:
                                return
                    # More events queued or requeue pending â€” loop again.
                    continue
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that Zed does not act on."""
        event = getattr(data, "event", None)
        message = getattr(event, "message", None)
    ...
    def _on_card_action_trigger(self, data: Any) -> Any:
        ...
        zed_action = action_value.get("zed_action") if isinstance(action_value, dict) else None
        update_prompt_action = (
            action_value.get("zed_update_prompt_action")
    ...
    def _handle_approval_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        if not state:
            ...
            return P2CardActionTriggerResponse() if P2CardActionTriggerResponse else None
        choice = _APPROVAL_CHOICE_MAP.get(action_value.get("zed_action"), "deny")
    ...
    def _handle_update_prompt_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        answer = str(action_value.get("zed_update_prompt_action", "") or "").strip().lower()
    ...
    async def _handle_reaction_event(self, event_type: str, data: Any) -> None:
        ...
        try:
            ...
            if not msg:
                return
            # GET im/v1/messages returns sender.id=app_id for bot messages â€”
    ...
    async def on_processing_complete(
        ...
        if start_reaction_id:
            if not await self._remove_reaction(message_id, start_reaction_id):
                ...
                # remove â€” UI would read as both "working" and "done/failed"
    ...
    def _record_webhook_anomaly(self, remote_ip: str, status: str) -> None:
        ...
        if entry is not None:
            ...
            if now - first_seen < _FEISHU_WEBHOOK_ANOMALY_TTL_SECONDS:
                ...
                return
        # Either first occurrence or TTL expired â€” start fresh.
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        # Rate limiting â€” composite key: app_id:path:remote_ip (matches openclaw key structure).
        ...
        # Content-Type guard â€” Feishu always sends application/json.
        ...
        # Body size guard â€” reject early via Content-Length when present.
        ...
        # Verification token check â€” second layer of defence beyond signature (matches openclaw).
        ...
        # URL verification challenge â€” Feishu includes the verification token in
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by Zed webhook mode")
    ...
    def _check_webhook_rate_limit(self, rate_key: str) -> bool:
        ...
        The rate_key is composed as "{app_id}:{path}:{remote_ip}" â€” matching openclaw's key
        ...
        if entry is not None:
            ...
            if now - window_start < _FEISHU_WEBHOOK_RATE_WINDOW_SECONDS:
                ...
                return True
        # New window for an existing key, or a brand-new key â€” prune stale entries first.
    ...
    async def _flush_text_batch(self, key: str) -> None:
        ...
        try:
            ...
            # a continuation is almost certain â€” wait longer.
    ...
    # =========================================================================
    # Static helpers â€” extension / media-type guessing
    # =========================================================================
    ...
    async def _resolve_sender_profile(
        ...
        *,
        ...
        """Map Feishu's three-tier user IDs onto Zed' SessionSource fields.

        Preference order for the primary ``user_id`` field:
          1. user_id  (tenant-scoped, most stable â€” requires permission scope)
          2. open_id  (app-scoped, always available â€” different per bot app)
    ...
    async def _resolve_sender_name_from_api(
        ...
        *,
        ...
        """Bots divert to bot/basic_batch â€” contact API doesn't return bot names.
    ...
    def _admit(self, sender: Any, message: Any) -> Optional[RejectReason]:
        ...
        # Defensive only â€” Feishu doesn't echo our outbound back as inbound,
        ...
        for mention in mentions:
            ...
            if mention_open_id and self._bot_open_id:
                if mention_open_id == self._bot_open_id:
                    return True
                continue  # IDs differ â€” not the bot; skip name fallback.
    ...
    async def _hydrate_bot_identity(self) -> None:
        ...
        # Primary probe: /open-apis/bot/v3/info â€” returns bot_name + open_id, no
    ...
    # =========================================================================
    # Deduplication â€” seen message ID cache (persistent)
    # =========================================================================
    ...
    async def _send_raw_message(
        ...
        *,
        ...
        # For topic/thread messages that fell back from replyâ†’create, use
    ...
    # =========================================================================
    # Connection internals â€” websocket / webhook setup
    # =========================================================================
                ...
                if active_reply_to and not self._response_succeeded(response):
                    ...
                    if code in _FEISHU_REPLY_FALLBACK_CODES:
                        if (metadata or {}).get("thread_id"):
                            logger.warning(
                                "[Feishu] Reply to %s failed in thread %s (code %s â€” message withdrawn/missing); "
                            ...
                            return response
                        logger.warning(
                            "[Feishu] Reply to %s failed (code %s â€” message withdrawn/missing); "
...
# Called by `zed gateway setup` via _setup_feishu() in zed_cli/gateway.py.
# =============================================================================
    ...
    *,
    ...
    while time.monotonic() < deadline:
        ...
        if tenant_brand == "lark" and not domain_switched:
            ...
            # Fall through â€” server may return credentials in this same response.
        ...
        # authorization_pending or unknown â€” keep polling
        time.sleep(interval)
...
def probe_bot(app_id: str, app_secret: str, domain: str) -> Optional[dict]:
    ...
    Note: ``bot_open_id`` here is the bot's app-scoped open_id â€” the same ID
...
def _parse_bot_response(data: dict) -> Optional[dict]:
    # /bot/v3/info returns bot.app_name; legacy paths used bot_name â€” accept both.
    if data.get("code") != 0:
        return None
...
def _qr_register_inner(
    *,
    ...
    """Run init â†’ begin â†’ poll â†’ probe. Raises on network/protocol errors."""
    ...
    # Probe bot â€” best-effort, don't fail the registration
