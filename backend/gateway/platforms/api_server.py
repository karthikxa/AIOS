...
- POST /v1/chat/completions        â€” OpenAI Chat Completions format (stateless; opt-in session continuity via X-Zed-Session-Id header; opt-in long-term memory scoping via X-Zed-Session-Key header)
- POST /v1/responses               â€” OpenAI Responses API format (stateful via previous_response_id; X-Zed-Session-Key supported)
- GET  /v1/responses/{response_id} â€” Retrieve a stored response
- DELETE /v1/responses/{response_id} â€” Delete a stored response
- GET  /v1/models                  â€” lists zed-agent as an available model
- GET  /v1/capabilities            â€” machine-readable API capabilities for external UIs
- GET  /api/sessions               â€” list client-visible Zed sessions
- POST /api/sessions               â€” create an empty Zed session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to zed-agent
...
def _normalize_multimodal_content(content: Any) -> Any:
    ...
    Raises ``ValueError`` with an OpenAI-style code on invalid input:
      * ``unsupported_content_type`` â€” file/input_file/file_id parts, or
        non-image ``data:`` URLs.
      * ``invalid_image_url`` â€” missing URL or unsupported scheme.
      * ``invalid_content_part`` â€” malformed text/image objects.
    ...
    for part in items:
        ...
        # Unknown part type â€” reject explicitly so clients get a clear error
...
class ResponseStore:
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        # rather than after every commit â€” chmod-on-every-write is wasted
...
def _derive_chat_session_id(
    ...
    the same Zed session (and therefore the same Docker container sandbox
...
def _notify_cron_provider_jobs_changed() -> None:
    ...
    mutation (no-op for the built-in). Best-effort â€” never breaks the handler."""
    try:
...
# API_SERVER_KEY), so this is not the trust boundary â€” it's parity with the
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through zed-agent's AIAgent.
    """
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Zed-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Zed-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Zed-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Zed-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Zed-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``zed sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the zed-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-Zed-Session-Key``).  Unlike ``session_id``
        which scopes the short-term transcript and rotates on /new, this
        key is meant to persist across transcripts so long-term memory
        providers (e.g. Honcho) can scope their per-chat state correctly
        â€” matching the semantics of the native gateway's ``session_key``.
        """
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "zed-agent", "version": _zed_version()}
        )
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models â€” return zed-agent as an available model."""
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        """GET /v1/capabilities â€” advertise the stable API surface.

        External UIs and orchestrators use this endpoint to discover the API
        server's plugin-safe contract without scraping docs or assuming that
        every Zed version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side Zed AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-Zed-Session-Id",
                "session_key_header": "X-Zed-Session-Key",
    ...
    async def _handle_skills(self, request: "web.Request") -> "web.Response":
        """GET /v1/skills â€” list installed skills visible to the API-server agent.
    ...
    async def _handle_toolsets(self, request: "web.Request") -> "web.Response":
        """GET /v1/toolsets â€” list toolsets and their resolved tools.
    ...
    # ------------------------------------------------------------------
    # /api/sessions â€” thin client/session resource API
    # ------------------------------------------------------------------
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions â€” list persisted Zed sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions â€” create an empty Zed session row."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({"object": "zed.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "zed.session", "session": self._session_response(session)})

    async def _handle_patch_session(self, request: "web.Request") -> "web.Response":
        """PATCH /api/sessions/{session_id} â€” update client-safe session metadata."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "zed.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions/{session_id}/fork â€” branch via current SessionDB primitives."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({"object": "zed.session", "session": self._session_response(fork)}, status=201)

    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions/{session_id}/chat â€” one synchronous agent turn."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        headers = {"X-Zed-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-Zed-Session-Key"] = gateway_session_key
        return web.json_response(
            {
                "object": "zed.session.chat.completion",
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        """POST /api/sessions/{session_id}/chat/stream â€” SSE wrapper over _run_agent."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        gateway_session_key, key_err = self._parse_session_key_header(request)
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        """POST /v1/chat/completions â€” OpenAI Chat Completions format."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        # stable per-channel identifier via X-Zed-Session-Key.  This
        # is independent of X-Zed-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Zed-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Zed-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Zed-Session-Id rejected: "
            ...
            # conversation map to the same Zed session.  The first user
        ...
        if stream:
            ...
            def _on_delta(delta):
                # Filter out None â€” the agent fires stream_delta_callback(None)
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``zed.tool.progress`` with ``status: running``.
                ...
                events (``_thinking``, â€¦) stay off the wire â€” matching
        ...
        response_headers = {
            "X-Zed-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Zed-Session-Key"] = gateway_session_key

        # Hard-fail path: no usable assistant text AND a real failure â†’ 5xx
        ...
        if not final_response and (is_failed or is_partial):
            ...
            response_headers["X-Zed-Completed"] = "false"
            response_headers["X-Zed-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Zed-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Zed-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Zed-Error"] = err_msg[:200]
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Zed-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Zed-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            # Helper â€” route a queue item to the correct SSE event.
            async def _emit(item):
                """Write a single queue item to the SSE stream.

                Plain strings are sent as normal ``delta.content`` chunks.
                Tagged tuples ``("__tool_progress__", payload)`` are sent
                as a custom ``event: zed.tool.progress`` SSE event so
    ...
    async def _write_sse_responses(
        ...
        - ``response.created`` â€” initial envelope (status=in_progress)
        - ``response.output_text.delta`` / ``response.output_text.done`` â€”
          streamed assistant text
        - ``response.output_item.added`` / ``response.output_item.done``
          with ``item.type == "function_call"`` â€” when the agent invokes a
          tool (both events fire; the ``done`` event carries the finalized
          ``arguments`` string)
        - ``response.output_item.added`` with
          ``item.type == "function_call_output"`` â€” tool result with
          ``{call_id, output, status}``
        - ``response.completed`` â€” terminal event carrying the full
          response object with all output items + usage (same payload
          shape as the non-streaming path for parity)
        - ``response.failed`` â€” terminal event on agent error
        ...
        # delta events â€” the spec ties deltas to a specific item.
        ...
        try:
                ...
                if call_id:
                    for i, p in enumerate(pending_tool_calls):
                        if p["call_id"] == call_id:
                            ...
                            break
                if pending is None:
                    # Completion without a matching start â€” skip to avoid
            ...
            # Main drain loop â€” thread-safe queue fed by agent callbacks.
            async def _dispatch(it) -> None:
                """Route a queue item to the correct SSE emitter.

                Plain strings are text deltas â€” they are batched (50ms)
                        ...
                        if isinstance(_args, dict):
                            for _k in ("content", "query", "pattern", "old_string", "new_string"):
                                if isinstance(_args.get(_k), str) and len(_args[_k]) > 500:
                                    _args[_k] = "[" + str(len(_args[_k])) + " chars â€” truncated for response.completed]"
            ...
            # Client disconnected â€” interrupt the agent so it stops
            ...
            # Server-side cancellation (e.g. shutdown, request timeout) â€”
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        """POST /v1/responses â€” OpenAI Responses API format."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        if conversation:
            ...
            # No error if conversation doesn't exist yet â€” it's a new conversation
        ...
        stream = _coerce_request_bool(body.get("stream"), default=False)
        if stream:
            # Streaming branch â€” emit OpenAI Responses SSE events as the
        ...
        response_headers = {"X-Zed-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-Zed-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    # Allowed fields for update â€” prevents clients injecting arbitrary keys
    ...
    async def _handle_list_jobs(self, request: "web.Request") -> "web.Response":
        """GET /api/jobs â€” list all cron jobs."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_job(self, request: "web.Request") -> "web.Response":
        """POST /api/jobs â€” create a new cron job."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        cron_err = self._check_jobs_available()
        ...
        try:
            ...
            if len(name) > self._MAX_NAME_LENGTH:
                return web.json_response(
                    {"error": f"Name must be â‰¤ {self._MAX_NAME_LENGTH} characters"}, status=400,
                )
            if not schedule:
                return web.json_response({"error": "Schedule is required"}, status=400)
            if len(prompt) > self._MAX_PROMPT_LENGTH:
                return web.json_response(
                    {"error": f"Prompt must be â‰¤ {self._MAX_PROMPT_LENGTH} characters"}, status=400,
                )
    ...
    async def _handle_get_job(self, request: "web.Request") -> "web.Response":
        """GET /api/jobs/{job_id} â€” get a single cron job."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        cron_err = self._check_jobs_available()
    ...
    async def _handle_update_job(self, request: "web.Request") -> "web.Response":
        """PATCH /api/jobs/{job_id} â€” update a cron job."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        cron_err = self._check_jobs_available()
    ...
    async def _handle_delete_job(self, request: "web.Request") -> "web.Response":
        """DELETE /api/jobs/{job_id} â€” delete a cron job."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        cron_err = self._check_jobs_available()
    ...
    async def _handle_pause_job(self, request: "web.Request") -> "web.Response":
        """POST /api/jobs/{job_id}/pause â€” pause a cron job."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        cron_err = self._check_jobs_available()
    ...
    async def _handle_resume_job(self, request: "web.Request") -> "web.Response":
        """POST /api/jobs/{job_id}/resume â€” resume a paused cron job."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        cron_err = self._check_jobs_available()
    ...
    async def _handle_run_job(self, request: "web.Request") -> "web.Response":
        """POST /api/jobs/{job_id}/run â€” trigger immediate execution."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        cron_err = self._check_jobs_available()
    ...
    async def _handle_cron_fire(self, request: "web.Request") -> "web.Response":
        """POST /api/cron/fire â€” Chronos managed-cron fire webhook (NAS â†’ agent).

        Authenticated by a NAS-minted JWT (verified via the pluggable
        fire-verifier), NOT API_SERVER_KEY â€” NAS holds no API server key, and
    ...
    def _turn_transcript_messages(
        ...
        tool calls â€” so when the page is re-opened mid/post-stream those
    ...
    async def _run_agent(
        ...
        def _run():
            ...
            try:
                ...
                # (e.g. X-Zed-Session-Id header) can track compression-
    ...
    # ------------------------------------------------------------------
    # /v1/runs â€” structured event streaming
    # ------------------------------------------------------------------
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        """POST /v1/runs â€” start an agent run, return run_id immediately."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        async def _run_and_close():
            try:
                ...
                # block below never fires â€” issue #15561).
        ...
        response_headers = (
            {"X-Zed-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_get_run(self, request: "web.Request") -> "web.Response":
        """GET /v1/runs/{run_id} â€” return pollable run status for external UIs."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_run_events(self, request: "web.Request") -> "web.StreamResponse":
        """GET /v1/runs/{run_id}/events â€” SSE stream of structured agent lifecycle events."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        try:
            while True:
                try:
                    ...
                    continue
                if event is None:
                    # Run finished â€” send final SSE comment and close
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "zed.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def _handle_stop_run(self, request: "web.Request") -> "web.Response":
        ...
        if task is not None and not task.done():
            ...
            # executor thread which task.cancel() cannot preempt â€” we rely on
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Chronos managed-cron fire webhook (NAS â†’ agent). Authenticated by a
            ...
            # Store the adapter after native routes are registered. Local Zed-Relay
    ...
    async def disconnect(self) -> None:
        ...
        2 file descriptors (the database file and its WAL sidecar) â€” the
        reconnect loop in ``gateway.run`` constructs a fresh adapter on
        every retry, so 2 fds/retry Ã— 300s backoff cap â‰ˆ 12 fds/hour, which
    ...
    async def send(
        ...
        """
        Not used â€” HTTP request/response cycle handles delivery directly.
        """
