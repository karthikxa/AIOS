...
def _kill_stale_bridge_by_pidfile(session_path: Path) -> None:
    ...
    process becomes orphaned â€” this helper finds and kills it.
    """
    ...
    try:
        ...
        try:
            ...
            pass
        return
    # ``os.kill(pid, 0)`` is NOT a no-op on Windows (bpo-14484) â€” use the
...
def check_whatsapp_requirements() -> bool:
    ...
    """
    # Prefer Zed-managed Node/npm so Windows installs are not broken by a
...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    - dm_policy: "open" | "allowlist" | "disabled" â€” how DMs are handled (default: "open")
    - allow_from: List of sender IDs allowed in DMs (when dm_policy="allowlist")
    - group_policy: "open" | "allowlist" | "disabled" â€” which groups are processed (default: "open")
    ...
    # Default bridge location relative to the zed-agent install
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._session_path: Path = Path(config.extra.get(
            "session_path",
            get_zed_dir("platforms/whatsapp/session", "whatsapp/session")
        ))
        ...
        # notification before the normal "âœ“ whatsapp disconnected" fires.
        ...
        # (e.g. forwarded batches, paste-splits) â€” without debounce each
    ...
    async def connect(self) -> bool:
        ...
        if not check_whatsapp_requirements():
            ...
            self._set_fatal_error(
                ...
                "Node.js is not installed â€” install Node.js and re-run `zed gateway`.",
                retryable=False,
            )
            return False
        ...
        # clear "run zed whatsapp" message instead of the watcher
        ...
        if not creds_path.exists():
            logger.warning(
                ...
                "Run `zed whatsapp` to pair, or remove WHATSAPP_ENABLED from "
                "your .env to disable.",
                self.name, creds_path,
            )
            self._set_fatal_error(
                "whatsapp_not_paired",
                "WhatsApp enabled but not paired â€” run `zed whatsapp` to pair.",
                retryable=False,
            )
            return False
        ...
        try:
            ...
            # `zed update` bumps the Baileys pin).  The stamp file records
            # the package.json hash of the last successful install.
            bridge_dir = bridge_path.parent
            _pkg_json = bridge_dir / "package.json"
            _dep_stamp = bridge_dir / "node_modules" / ".zed-pkg-hash"
            ...
            if not _deps_fresh:
                ...
                # Zed-managed portable Node before falling back to PATH.
                    ...
                    async with session.get(
                        ...
                        if resp.status == 200:
                            ...
                            if bridge_status == "connected":
                                ...
                                # `zed update`, so without this check it
            ...
            bridge_env = with_zed_node_path()
            if self._reply_prefix is not None:
                bridge_env["WHATSAPP_REPLY_PREFIX"] = self._reply_prefix
            # Pass the profile-aware cache directories so the bridge writes
            # media where the Python side reads it.  Without these the bridge
            # hardcodes ~/.zed/{image,audio,document}_cache, which diverges
            # under ZED_HOME overrides, profiles, and the new cache/ layout.
            from gateway.platforms.base import (
                get_audio_cache_dir as _get_audio_dir,
                get_document_cache_dir as _get_doc_dir,
                get_image_cache_dir as _get_img_dir,
            )
            bridge_env["ZED_IMAGE_CACHE_DIR"] = str(_get_img_dir())
            bridge_env["ZED_AUDIO_CACHE_DIR"] = str(_get_audio_dir())
            bridge_env["ZED_DOCUMENT_CACHE_DIR"] = str(_get_doc_dir())
            ...
            if data.get("status") != "connected":
                ...
                for attempt in range(15):
                    ...
                    # Still not connected â€” warn but proceed (bridge may
                    # auto-reconnect later, e.g. after a code 515 restart).
                    print(f"[{self.name}] âš  WhatsApp not connected after 30s")
                    print(f"[{self.name}]   Bridge log: {self._bridge_log}")
                    print(f"[{self.name}]   If session expired, re-pair: zed whatsapp")
    ...
    async def send_image(
        ...
        ``metadata`` is accepted to honor the base-class contract â€” the
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        """Send a video natively via bridge â€” plays inline in WhatsApp."""
    ...
    async def send_typing(self, chat_id: str, metadata=None) -> None:
        ...
        try:
            ...
            # Must wrap in `async with` â€” a bare `await session.post(...)`
    ...
    # â”€â”€ Text debounce batching â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        try:
            ...
            for url in raw_urls:
                if msg_type == MessageType.PHOTO and url.startswith(("http://", "https://")):
                    ...
                    # Local file path â€” bridge already downloaded the image
                    cached_urls.append(url)
                    media_types.append("image/jpeg")
