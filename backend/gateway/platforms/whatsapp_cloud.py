"""
WhatsApp Cloud API adapter â€” official Meta WhatsApp Business Platform.

This adapter is a *complement* to ``whatsapp.py`` (the Baileys bridge), not
a replacement. The two are independent:

- ``whatsapp.py``      â€” unofficial Baileys bridge, personal accounts, no
                         public URL needed, account-ban risk.
- ``whatsapp_cloud.py`` (this file) â€” official Meta Cloud API, Business
...
- Phase 2 â€” outbound text via Graph API + webhook server with verify-token
            handshake.
- Phase 3 â€” X-Hub-Signature-256 HMAC verification (raw body, constant-time)
            + wamid replay protection + dispatch via handle_message. Phase 3
            adapter is end-to-end usable for text DMs.
- Phase 4 â€” media upload + send (image/video/audio/document), inbound
...
- Phase 5 â€” 24-hour conversation window + template fallback.
...
# every wamid for the full retry window â€” the practical risk is duplicate
...
# extensions for some types (audio/ogg â†’ .oga since RFC 5334; audio/mp4
# â†’ .mp4 instead of the de-facto .m4a for voice notes). Our downstream
...
_WHATSAPP_MIME_EXTENSION_OVERRIDES: Dict[str, str] = {
    # WhatsApp voice notes â€” opus codec inside an Ogg container.
    "audio/ogg": ".ogg",
    "audio/x-opus+ogg": ".ogg",
    "audio/opus": ".ogg",
    # iOS voice memos â€” AAC inside an MP4 container; STT tools expect .m4a.
    "audio/mp4": ".m4a",
    "audio/x-m4a": ".m4a",
    # Image â€” mimetypes occasionally returns .jpe (legacy IANA) instead
...
# Inbound media cache lives under the user's zed dir so it survives
# restarts and gateway reloads â€” same convention the Baileys bridge uses.
_INBOUND_MEDIA_CACHE = Path(get_zed_dir("platforms/whatsapp_cloud/media", "whatsapp_cloud/media"))
...
def check_whatsapp_cloud_requirements() -> bool:
    ...
    for Graph API calls (outbound). Both ship with zed-agent's default
...
class WhatsAppCloudAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Webhook dedup state â€” wamid â†’ True. OrderedDict gives O(1) FIFO
        ...
        # outbound button payload) â†’ the session/correlation key needed
        ...
        # evicted first â€” an evicted button tap degrades to the plain-text
        # fallback path, same as after a gateway restart).
        #   _clarify_state:        clarify_id â†’ session_key (resolves via
        #                          tools.clarify_gateway.resolve_gateway_clarify)
        #   _exec_approval_state:  approval_id â†’ session_key (resolves via
        #                          tools.approval.resolve_gateway_approval)
        #   _slash_confirm_state:  confirm_id â†’ session_key (resolves via
    ...
    def _effective_reply_prefix(self) -> str:
        """Cloud API has no self-chat concept â€” never prepend a reply prefix.
    ...
    async def connect(self) -> bool:
        if not check_whatsapp_cloud_requirements():
            self._set_fatal_error(
                ...
                "aiohttp and httpx are required for whatsapp_cloud â€” "
                "reinstall zed-agent.",
                retryable=False,
            )
            return False
        ...
        if not self._verify_token:
            logger.warning(
                "[whatsapp_cloud] WHATSAPP_CLOUD_VERIFY_TOKEN is not set â€” "
                "the GET subscription handshake will fail until it is."
            )
        if not self._app_secret:
            logger.warning(
                "[whatsapp_cloud] WHATSAPP_CLOUD_APP_SECRET is not set â€” "
    ...
    async def send(
        ...
        ``chat_id`` is the recipient's WhatsApp ID (``wa_id``) â€” typically
        ...
        for idx, chunk in enumerate(chunks):
            ...
            if resp.status_code != 200:
                # Meta returns structured errors in the body â€” surface them
    ...
    # whichever comes first â€” so this matches "I see your message and
    ...
    async def send_typing(self, chat_id: str, metadata=None) -> None:
        ...
        if not wamid:
            ...
            # restart) â€” skip. The next inbound message will repopulate.
            return
        ...
        try:
            ...
            # Network / connection error â€” silent fail. Typing UX must
        ...
        # > 30 days old) â€” common after a long-quiet conversation, log
        # at info not warning.
        if resp.status_code != 200:
            try:
                body = resp.json()
    ...
    #   * ``interactive.type=button`` â€” up to 3 quick-reply buttons. Each
    #     button has an ``id`` (â‰¤256 chars, returned verbatim on tap) and
    #     a ``title`` (â‰¤20 chars, the label shown). Used for clarify with
    #     â‰¤3 choices, exec_approval, and slash_confirm.
    #   * ``interactive.type=list``   â€” a single "Tap to choose" button
    ...
    # approval. They only work *inside* the 24-hour conversation window â€”
    ...
    async def _post_interactive(
        ...
        ``interactive_body`` is the inner ``interactive: {...}`` dict â€”
    ...
    async def send_clarify(
        ...
        - 1â€“3 choices â†’ ``interactive.type=button`` (inline pill buttons).
        - 4+ choices â†’ ``interactive.type=list`` (tap-to-open sheet with
        ...
        - 0 choices (open-ended) â†’ plain text question; the next message
        ...
        # Open-ended â†’ just send the question, gateway captures next msg.
        if not choices:
            return await self.send(chat_id, f"â“ {question}", reply_to=reply_to)

        # Mirror Telegram: render full choice text in body so long
        # options aren't truncated to the 20-char button label cap.
        # Truncate choices to MAX_CHOICES (4) â€” the tool layer enforces
        ...
        body_text = self._truncate_body(f"â“ {question}\n\n{option_lines}")
        ...
        if len(choices_list) <= 3:
            ...
            # List mode: rows must each have id + title (â‰¤24 chars).
            # Description (â‰¤72 chars) renders below the title â€” we put
    ...
    async def send_exec_approval(
        ...
        waiting agent via ``tools.approval.resolve_gateway_approval`` â€”
        ...
        body_text = self._truncate_body(
            f"âš ï¸ *Command Approval Required*\n\n"
            f"```\n{cmd_preview}\n```\n\n"
        ...
        interactive = {
            ...
            "action": {
                "buttons": [
                    {
                        "type": "reply",
                        "reply": {"id": f"appr:{approval_id}:approve", "title": "âœ… Approve"},
                    },
                    {
                        "type": "reply",
                        "reply": {"id": f"appr:{approval_id}:deny", "title": "âŒ Deny"},
                    },
                ],
            },
        }
    ...
    async def send_slash_confirm(
        ...
        handler) â€” we just store the session_key mapping for the inbound
        resolver to look up.
        """
        if self._http_client is None:
            return SendResult(success=False, error="Not connected")
        ...
        interactive = {
            ...
            "action": {
                "buttons": [
                    {
                        "type": "reply",
                        "reply": {"id": f"sc:once:{confirm_id}", "title": "âœ… Approve Once"},
                    },
                    {
                        "type": "reply",
                        "reply": {"id": f"sc:always:{confirm_id}", "title": "ðŸ”’ Always"},
                    },
                    {
                        "type": "reply",
                        "reply": {"id": f"sc:cancel:{confirm_id}", "title": "âŒ Cancel"},
                    },
                ],
            },
        }
    ...
    async def get_chat_info(self, chat_id: str) -> Dict[str, Any]:
        ...
        # Slack/Discord do â€” we just echo the wa_id. Profile name (when
    ...
    async def _upload_media(
        ...
        "sticker" â€” selects size cap + default mime fallback.
        """
        if self._http_client is None:
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        """Send a video. Local path â†’ upload; HTTPS URL â†’ link mode."""
        return await self._send_media_from_path_or_link(
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        a generic audio attachment. Zed TTS produces MP3, so we try
        ...
        if is_local_mp3:
            ...
            if opus_path:
                try:
                    ...
                    # the source MP3 â€” clean it up after upload so voice
    ...
    def _warn_once_no_ffmpeg(self) -> None:
        ...
        logger.warning(
            "[whatsapp_cloud] ffmpeg not found on PATH â€” voice messages will "
    ...
    async def _download_media_to_cache(
        ...
        *,
        ...
        """Two-step Graph media download: ``GET /<id>`` â†’ temp URL â†’ bytes.
        ...
        # a cache filename below â€” refuse anything that isn't a plain
        ...
        # Step 1 â€” metadata (gives us a temporary signed URL + mime)
        try:
        ...
        # Step 2 â€” bytes (auth required even though URL is signed; Meta
        # documents this explicitly â€” the URL alone is not enough).
        try:
    ...
    async def _handle_verify(self, request: "web.Request") -> "web.Response":
        ...
        """
        if not self._verify_token:
            # Misconfigured server â€” refuse rather than silently accepting
    ...
    async def _handle_webhook(self, request: "web.Request") -> "web.Response":
        ...
        Lifecycle:
          1. Read raw bytes (signature is over the raw body â€” JSON parsing
          ...
          6. Always respond 200 once we've ack'd a valid request â€” Meta
        ...
        # Parse only AFTER signature passes â€” bad JSON from an attacker is
    ...
    def _dedup_wamid(self, wamid: str) -> bool:
        ...
        """
        if not wamid:
            # No wamid means we can't dedup â€” let it through. Meta should
    ...
    async def _dispatch_payload(self, payload: Dict[str, Any]) -> None:
        ...
        â€” the agent doesn't currently consume delivery receipts and
        ...
        for entry in payload.get("entry") or []:
            ...
            for change in entry.get("changes") or []:
                ...
                # Build a wa_id â†’ profile-name index for the messages we're
                # about to surface.
                ...
                for raw_message in value.get("messages") or []:
                    ...
                    try:
                        ...
                        # Dispatch errors must not bubble out â€” Meta would
                ...
                # Log status updates at debug level â€” useful for diagnosing
    ...
    async def _dispatch_interactive_reply(
        ...
        state entry, or the resolver itself reports no waiter â€” in
        ...
        Dispatch table:
          ``cl:<clarify_id>:<idx|other>``  â†’ resolve_gateway_clarify
          ``appr:<approval_id>:approve|deny`` â†’ resolve_gateway_approval
          ``sc:<once|always|cancel>:<confirm_id>`` â†’ slash_confirm.resolve
        """
        ...
        if button_id.startswith("cl:"):
            ...
            if not session_key:
                logger.info(
                    ...
                    "(clarify_id=%s) â€” likely stale; falling back to text",
                    clarify_id,
                )
                return False
            try:
            ...
            if choice == "other":
                ...
                # ``get_pending_for_session`` won't return the entry â€”
                ...
                if not flipped:
                    ...
                    return False
                # Put state back since we popped it earlier â€” keep the
                # clarify_id â†’ session_key mapping live in case future
            ...
            # full choice from the original prompt â€” but we didn't
            ...
            if not resolved:
                ...
                logger.info(
                    ...
                    "(clarify_id=%s) â€” falling back to text", clarify_id,
                )
                return False
            return True
        ...
        if button_id.startswith("appr:"):
            ...
            if not session_key:
                logger.info(
                    ...
                    "(approval_id=%s) â€” likely stale; falling back to text",
                    approval_id,
                )
                return False
            ...
            if not count:
                logger.info(
                    ...
                    "(session_key=%s) â€” likely already resolved",
                    session_key,
                )
            # Send confirmation message â€” paralleling Telegram's UX.
            try:
                confirm_text = (
                    "âœ… Approved." if choice == "approve" else "âŒ Denied."
                )
        ...
        if button_id.startswith("sc:"):
            ...
            if not session_key:
                logger.info(
                    ...
                    "(confirm_id=%s) â€” likely stale", confirm_id,
                )
                return False
        ...
        # Unknown prefix â€” let text dispatch handle the title as a
    ...
    async def _build_message_event_from_cloud(
        ...
        """Convert a Cloud-API message object into a Zed MessageEvent.
        ...
        up to 100KB â€” same heuristic the Baileys adapter uses.
        ...
        # gateway resolver BEFORE falling through to text dispatch â€” the
        ...
        # Cloud API doesn't have a separate "chat" entity for DMs â€” chat_id
        ...
        # object identifying the group JID â€” its absence signals DM.
        chat_field = raw_message.get("chat")
        if chat_field:
            logger.warning(
                "[whatsapp_cloud] received group-shaped message (chat=%s, "
                "wamid=%s) â€” group support is not yet implemented; dropping. "
        ...
        if msg_type_str in {"image", "video", "audio", "voice", "document", "sticker"}:
            ...
            if media_id:
                ...
                if local_path:
                    ...
                    logger.warning(
                        "[whatsapp_cloud] failed to download inbound %s (id=%s) â€” "
