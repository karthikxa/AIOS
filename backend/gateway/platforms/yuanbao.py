...
_APP_VERSION = _ZED_VERSION
_BOT_VERSION = _ZED_VERSION
_YUANBAO_INSTANCE_ID = str(ZED_INSTANCE_ID)  # single source: yuanbao_proto.ZED_INSTANCE_ID
...
# teardown fast â€” a responsive server completes the handshake in well under a
...
# Close codes that indicate permanent errors â€” do NOT reconnect.
NO_RECONNECT_CLOSE_CODES = {4012, 4013, 4014, 4018, 4019, 4021}

# Heartbeat timeout threshold â€” N consecutive missed pongs trigger reconnect.
...
SLOW_RESPONSE_MESSAGE = "ä»»åŠ¡æœ‰ç‚¹å¤æ‚ï¼Œæ­£åœ¨åŠªåŠ›å¤„ç†ä¸­ï¼Œè¯·è€å¿ƒç­‰å¾…..."
...
#   [file: report.pdf â†’ /opt/data/.../report.pdf]
...
class MarkdownProcessor:
    ...
    def chunk_markdown_text(
        ...
        # Phase 3: Post-processing â€” split still-oversized chunks at paragraph boundaries
        result: list[str] = []
    ...
    def infer_block_separator(cls, prev_chunk: str, next_chunk: str) -> str:
        ...
        - Previous chunk ends with code fence or next chunk starts with fence â†’ single newline '\\n'
        - Previous chunk ends with table row and next chunk starts with table row â†’ single newline '\\n' (continued table)
        - Otherwise â†’ double newline '\\n\\n' (paragraph separator)
...
class SignManager:
    ...
    # key: app_key â†’ {"token", "bot_id", "expire_ts", ...}
    _cache: dict[str, dict[str, Any]] = {}

    # Per-app_key refresh locks â€” prevents concurrent duplicate sign-token
...
class InboundPipeline:
    ...
    def use(self, name_or_mw, handler=None, when=None) -> "InboundPipeline":
        ...
        Accepts either:
          - ``pipeline.use(SomeMiddleware())``  â€” OOP style
          - ``pipeline.use("name", some_fn)``   â€” functional style
        """
        ...
        async def next_fn() -> None:
            ...
            while index < len(chain):
                ...
                try:
                    ...
                    raise
                return
            # End of chain â€” nothing more to do
...
class DecodeMiddleware(InboundMiddleware):
    ...
    async def handle(self, ctx: InboundContext, next_fn) -> None:
        ...
        if not data_list:
            return  # Stop pipeline â€” nothing to decode
...
class RecallGuardMiddleware(InboundMiddleware):
    ...
    Branch A: message in transcript (observed, not yet consumed) â†’ redact content
    Branch B: message not in transcript â†’ append system note
    Branch C: message currently being processed â†’ silent interrupt + delayed redact
    """
    ...
    def _interrupt_for_recall(cls, adapter, session_key: str, recalled_id: str,
        ...
        recall_text = (
            f"[CRITICAL â€” MESSAGE RECALLED] The user message that triggered "
            ...
            f"tool results â€” the original request is void. "
        ...
        # May overwrite a user message pending in the same ~200ms window â€” acceptable.
        ...
        # interrupt â€” schedule a delayed redaction to clean it up.
    ...
    def _schedule_content_redact(cls, adapter, session_key: str, recalled_text: str,
        ...
        async def _redact() -> None:
            ...
            try:
                ...
                return
            # Poll until the recalled content appears in transcript â€” the
    ...
    def _patch_transcript(cls, adapter, recalled_id: str, group_code: str,
        ...
        # for any message that was observed with one â€” Branch A1 (exact id
        ...
        # platform id on the row â€” e.g. agent-processed @bot messages
        ...
        # Branch B: not found in transcript â†’ append system note
    ...
    def _extract_text(cls, msg_body: list) -> str:
        ...
        for elem in msg_body:
            ...
            if elem_type == "TIMTextElem":
                ...
                # Unknown element type â€” include type as placeholder
...
class OwnerCommandMiddleware(InboundMiddleware):
    ...
    def _detect_owner_command(
        ...
        *,
        ...
        # privileged â€” leaking them to non-owners lets any group member approve
    ...
    async def handle(self, ctx: InboundContext, next_fn) -> None:
        ...
        if matched_cmd and not is_owner:
            # Non-owner tried an owner-only command â€” reject and stop
            logger.info(
            ...
            adapter._track_task(asyncio.create_task(
                adapter.send(ctx.chat_id, f"âš ï¸ {matched_cmd} is only available to the creator in private chat mode"),
...
class GroupAtGuardMiddleware(InboundMiddleware):
    ...
    async def handle(self, ctx: InboundContext, next_fn) -> None:
        ...
        if ctx.chat_type == "group" and not ctx.owner_command and not self._is_at_bot(ctx.msg_body, adapter._bot_id):
            ...
            return  # Stop pipeline â€” message observed but not dispatched
        await next_fn()
...
class ForwardedRecordsParseMiddleware(InboundMiddleware):
    ...
    the transcript at observe time â€” there is no run-time summary fallback
    here.

    On any failure the middleware leaves ``ctx.raw_text`` untouched
    (graceful degradation, design Â§2.8).
    """
    ...
    def _media_marker(
        ...
        # media_id is directly usable as a ybres RID (design Â§2.10.9);
        ...
        if media_type == "url":
            # Link share (e.g. WeChat article) â€” keep URL for the agent.
    ...
    # Per-record combined-text cap; record count is NOT capped (design Â§2.10.3).
    ...
    def _walk_forward_msgs(
        ...
        entries in textual order â€” the order PatchAnchorsMiddleware relies on
        (design Â§2.10.6). Headers / footers are the caller's job.
        """
        for msg in (forward_data.get("msg") if isinstance(forward_data, dict) else None) or []:
                    ...
                    if mc_type == 1:  # TEXT
                        ...
                        for media in mc.get("multimedia", []) or []:
                            if isinstance(media, dict):
                                ...
                                if ref is not None:
                                    refs.append(ref)
                    elif mc_type == 3:  # nested FORWARD_MSG (design Â§2.10.10)
                        parts.append("[åµŒå¥—èŠå¤©è®°å½•]")
                    else:
                        if plain_text:
            ...
            if len(rendered) > cls.FORWARD_MSG_TEXT_MAX_CHARS:
                rendered = rendered[: cls.FORWARD_MSG_TEXT_MAX_CHARS] + "â€¦(å·²æˆªæ–­)"
    ...
    def build_forward_text(
        ...
        Body lines are ``å‘é€äººï¼šæ­£æ–‡`` with full ``[kind|ybres:RID]`` media
        markers preserved. When ``is_dispatch`` is true, refs are appended to
        ``ctx.media_refs`` for downstream resolution and a ``ç”¨æˆ·é™„è¨€ï¼š
        {ctx.raw_text}`` footer is added; observed callers skip both since
        no later middleware runs.
        """
        nickname = ctx.sender_nickname or "ç”¨æˆ·"
        lines = [f"å½“å‰ç”¨æˆ·çš„æ˜µç§°ä¸º{nickname}", "ä»¥ä¸‹ä¸ºç”¨æˆ·çš„èŠå¤©è®°å½•"]
        for sender, body, refs in cls._walk_forward_msgs(forward_data):
            lines.append(f"{sender}ï¼š{body}")
            if is_dispatch:
                ctx.media_refs.extend(refs)
        text = "\n".join(lines)
        if is_dispatch and ctx.raw_text.strip():
            text += f"\n\nç”¨æˆ·é™„è¨€ï¼š{ctx.raw_text.strip()}"
        return text
...
class MediaResolveMiddleware(InboundMiddleware):
    ...
    async def _collect_observed_media(
        ...
        # Walk the most recent LOOKBACK messages newestâ†’oldest so that when we
        ...
        # Final ``order`` is reversed back to chronological (oldâ†’new) before
        ...
        # Restore chronological order (oldestâ†’newest) for downstream resolution.
        order.reverse()
    ...
    def _collect_quote_local_media(ctx: InboundContext) -> Tuple[List[str], List[str]]:
        ...
        ``[image: /path]`` / ``[file: name â†’ /path]``. Unresolved anchors are an
        original-turn resolution failure and belong to that turn's handling, not
        this quote fallback â€” so no re-download happens here.
    ...
    async def handle(self, ctx: InboundContext, next_fn) -> None:
        ...
        # 2) Second source â€” quoted media takes priority; otherwise fall back
        ...
        if ctx.reply_to_message_id is not None:
            if ctx.quote_media_refs:
                ...
                # from the adapter msg cache. Patched on its original turn â€” no
...
class ConnectionManager:
    ...
    async def open(self) -> bool:
        """Open WebSocket connection: sign-token â†’ WS connect â†’ AUTH_BIND â†’ start loops.
    ...
    async def _handle_frame(self, raw: bytes) -> None:
        ...
        # Fire-and-forget heartbeat ACKs â€” server always responds but callers don't
        ...
        if cmd_type == CMD_TYPE["Push"]:
            ...
            # Genuine inbound message â€” dispatch to AI
            if data:
                logger.info(
    ...
    def _flush_inbound_buffer(self, key: str) -> None:
        """Flush the debounce buffer for a given key â€” execute the pipeline."""
    ...
    async def _reconnect_with_backoff(self) -> bool:
        """Reconnect with exponential backoff (1s, 2s, 4s, â€¦ up to 60s)."""
    ...
    async def _cleanup_ws(self) -> None:
        ...
        if ws is not None:
            try:
                ...
                logger.debug(
                    "[%s] WS close handshake exceeded %.1fs â€” dropping connection",
...
class MediaSendHandler(ABC):
    ...
    The shared flow (check ws â†’ cancel notifier â†’ validate â†’ COS upload
    â†’ lock â†’ dispatch) is handled by the base handle() template method.
    """
...
class ImageUrlHandler(MediaSendHandler):
    """Strategy: send image from a URL (download â†’ COS â†’ TIMImageElem)."""
...
class ImageFileHandler(MediaSendHandler):
    """Strategy: send image from a local file path (read â†’ COS â†’ TIMImageElem)."""
...
class FileUrlHandler(MediaSendHandler):
    """Strategy: send file from a URL (download â†’ COS â†’ TIMFileElem)."""
...
class DocumentHandler(MediaSendHandler):
    """Strategy: send local file/document (read â†’ COS â†’ TIMFileElem)."""
...
class OutboundManager:
    ...
    Composes:
      - MessageSender   â€” core text/media sending
      - HeartbeatManager â€” reply heartbeat (RUNNING / FINISH) lifecycle
      - SlowResponseNotifier â€” delayed 'please wait' notifications
...
class YuanbaoAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs: Any) -> None:
        ...
        # Inbound dispatch tasks â€” tracked so disconnect() can cancel them
        self._inbound_tasks: set[asyncio.Task] = set()

        # Set of background tasks â€” prevent GC from collecting fire-and-forget tasks
        ...
        # Group chat sequential dispatch queue (session_key â†’ asyncio.Queue).
        ...
        # Bounded cache of msg_id â†’ attributed content for recent messages.
        ...
        # upgrade â€” the first DM will override it with direct:xxx.
        # ------------------------------------------------------------------
    ...
    async def get_chat_info(self, chat_id: str) -> Dict[str, Any]:
        ...
        chat_id conventions:
          "group:<group_code>"  â†’ group chat
          "direct:<account>"   â†’ C2C / direct message (default)
