...
Mixin contract â€” the adapter must set these on ``self`` before any of the
mixin's methods are called (typically in ``__init__``):

    self.config        # gateway.config.PlatformConfig
    self.name          # str â€” adapter name (used in log lines)
...
class WhatsAppBehaviorMixin:
    ...
    satisfy. This mixin owns no state of its own â€” every value it touches
    is either a class attribute or set by the adapter's ``__init__``.
    """

    # WhatsApp message limits â€” practical UX limit, not protocol max.
    # WhatsApp allows ~65K but long messages are unreadable on mobile.
    MAX_MESSAGE_LENGTH: int = 4096
    supports_code_blocks = True  # WhatsApp renders fenced code blocks (monospace)

    DEFAULT_REPLY_PREFIX: str = "âš• *Zed Agent*\nâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€\n"
    ...
    def _is_broadcast_chat(chat_id: str) -> bool:
        ...
        never reply â€” answering a Story update spams the contact's status
    ...
    def _should_process_message(self, data: Dict[str, Any]) -> bool:
        ...
        # and the agent should never reply to them â€” even in self-chat mode
    ...
    def format_message(self, content: str) -> str:
        ...
        result = re.sub(r"~~(.+?)~~", r"~\1~", result)
        # Italic: *text* is already WhatsApp italic â€” leave as-is
        # _text_ is already WhatsApp italic â€” leave as-is

        # --- 4. Convert markdown headers to bold text ---
        # # Header â†’ *Header*. Strip any *...* wrapping already produced
        # by step 3 (e.g. "# **Title**" â†’ "*Title*", not "**Title**",
        ...
        # --- 5. Convert markdown links: [text](url) â†’ text (url) ---
