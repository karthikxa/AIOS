...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = env_float("ZED_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", 0.8)
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "ZED_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "ZED_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
        ...
        # Off by default â€” this mutates the bot's GLOBAL profile, visible to all
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Zed-created private-chat topic lanes are marked with
    ...
    # older PTB/clients. Streaming edits stay on Zed' existing MarkdownV2
    ...
    def _has_telegram_desktop_details_math_crash_shape(self, content: str) -> bool:
        ...
        Zed must skip rich delivery up front and use the legacy MarkdownV2
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `zed update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other Zed or OpenClaw instance is running "
            "with this token, then restart the gateway with 'zed gateway restart'."
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # readable by the Zed process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("ZED_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("ZED_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("ZED_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("ZED_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("ZED_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }

            disable_fallback = (os.getenv("ZED_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"})
    ...
    async def _set_status_indicator(self, online: bool) -> None:
        ...
        profile. It is the closest Bot API surface to a presence indicator â€”
    ...
    async def send_update_prompt(
        ...
        try:
            ...
            text = self.format_message(f"âš• *Update needs your input:*\n\n{prompt}{default_hint}")
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            keyboard = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("âœ… Allow Once", callback_data=f"ea:once:{approval_id}"),
                    InlineKeyboardButton("âœ… Session", callback_data=f"ea:session:{approval_id}"),
                ],
                [
                    InlineKeyboardButton("âœ… Always", callback_data=f"ea:always:{approval_id}"),
                    InlineKeyboardButton("âŒ Deny", callback_data=f"ea:deny:{approval_id}"),
                ],
            ])
    ...
    async def send_slash_confirm(
        ...
        try:
            ...
            keyboard = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("âœ… Approve Once", callback_data=f"sc:once:{confirm_id}"),
                    InlineKeyboardButton("ðŸ”’ Always Approve", callback_data=f"sc:always:{confirm_id}"),
                ],
                [
                    InlineKeyboardButton("âŒ Cancel", callback_data=f"sc:cancel:{confirm_id}"),
                ],
            ])
    ...
    def _build_provider_keyboard(self, providers: list):
        ...
        CLI ``zed model`` picker via the shared ``group_providers`` fold,
    ...
    def _build_model_keyboard(self, models: list, page: int) -> tuple:
        ...
        if total_pages > 1:
            nav: list = []
            if page > 0:
                nav.append(InlineKeyboardButton("â—€ Prev", callback_data=f"mg:{page - 1}"))
            nav.append(InlineKeyboardButton(f"{page + 1}/{total_pages}", callback_data="mx:noop"))
            if page < total_pages - 1:
                nav.append(InlineKeyboardButton("Next â–¶", callback_data=f"mg:{page + 1}"))
            rows.append(nav)
    ...
    async def _handle_model_picker_callback(
        ...
        if data.startswith("mp:"):
            ...
            shown = len(models)
            extra = f"\n_{total - shown} more available â€” type `/model <name>` directly_" if total > shown else ""
    ...
    async def _handle_callback_query(
        ...
        if data.startswith("ea:"):
            ...
            if len(parts) == 3:
                ...
                if not self._is_callback_user_authorized(
                    ...
                    await query.answer(text="â›” You are not authorized to approve commands.")
                    return
        ...
        if data.startswith("sc:"):
            ...
            if len(parts) == 3:
                ...
                if not self._is_callback_user_authorized(
                    ...
                    await query.answer(text="â›” You are not authorized to answer this prompt.")
                    return
        ...
        if not self._is_callback_user_authorized(
            ...
            await query.answer(text="â›” You are not authorized to answer update prompts.")
            return
    ...
    # Scripts live in ~/.zed/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".zed" / "scripts" / "gmail-triage" / script_name
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@zed_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several Zed bot profiles. A message like
    ...
    async def on_processing_complete(self, event: MessageEvent, outcome: ProcessingOutcome) -> None:
        ...
        interrupted mid-flight), we explicitly clear the ðŸ‘€ in-progress
        reaction so it doesn't linger on the user's message indefinitely.
        Without this clear, the only way to remove the ðŸ‘€ was to wait for
        another agent run to swap it to ðŸ‘/ðŸ‘Ž â€” which never happens if the
