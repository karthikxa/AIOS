...
# group mention gating without custom aliases, use conservative Zed wake
...
class BlueBubblesAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> List[re.Pattern]:
        ...
        JSON list, or comma/newline-separated), or None (use Zed defaults).
        """
        if raw is None:
    ...
    async def _register_webhook(self) -> bool:
        ...
        # Crash resilience â€” reuse an existing registration if present
    ...
    def truncate_message(content: str, max_length: int = MAX_TEXT_LENGTH) -> List[str]:
        # Use the base splitter but skip pagination indicators â€” iMessage
