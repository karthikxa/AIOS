...
# Audio file extensions Zed recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Zed' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Zed-created Telegram private-chat
...
def utf16_len(s: str) -> int:
    ...
    **not** Unicode code-points.  Characters outside the Basic Multilingual
    Plane (emoji like ðŸ˜€, CJK Extension B, musical symbols, â€¦) are encoded as
...
def proxy_kwargs_for_aiohttp(proxy_url: str | None) -> tuple[dict, dict]:
    ...
    Returns ``(session_kwargs, request_kwargs)`` where:
      - With aiohttp-socks â†’ ``({"connector": ProxyConnector(...)}, {})``
        for *all* proxy schemes (SOCKS **and** HTTP/HTTPS).
      - HTTP without aiohttp-socks â†’ ``({}, {"proxy": url})``.
      - None â†’ ``({}, {})``.
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.zed/.env manually."
)
...
# Default location: {ZED_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_zed_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; ZedAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_zed_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_zed_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_zed_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_zed_dir("cache/screenshots", "browser_screenshots")
_ZED_HOME = get_zed_home()
_ZED_ROOT = get_default_zed_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "ZED_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "ZED_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "ZED_MEDIA_TRUST_RECENT_SECONDS"
# Strict mode toggles the original allowlist+recency path-validation behavior.
# Off by default â€” symmetric with inbound (we accept any document type the
...
MEDIA_DELIVERY_STRICT_ENV = "ZED_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _ZED_HOME / "image_cache",
    _ZED_HOME / "audio_cache",
    _ZED_HOME / "video_cache",
    _ZED_HOME / "document_cache",
    _ZED_HOME / "browser_screenshots",
    # Canonical cache layout â€” listed alongside the legacy *_cache dirs so
    # generated artifacts deliver on installs that have both (#31733).
    _ZED_HOME / "cache" / "images",
    _ZED_HOME / "cache" / "audio",
    _ZED_HOME / "cache" / "videos",
    _ZED_HOME / "cache" / "documents",
    _ZED_HOME / "cache" / "screenshots",
)
...
# stray credentials) have mtimes measured in days or months â€” well outside this
# window â€” so prompt-injection paths pointing at pre-existing host files are
# still rejected.
...
# file looks. The cache-dir allowlist still beats this â€” an operator-configured
...
def _media_delivery_strict_mode() -> bool:
    ...
    system-path denylist â€” restoring the pre-#29523 behavior for the
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Zed profile and shared Zed root both contain control
    # files and credentials. Only cache subdirectories under them are
    # explicitly allowlisted above.
    for zed_root in (_ZED_HOME, _ZED_ROOT):
        denied.append(zed_root / ".env")
        denied.append(zed_root / "auth.json")
        denied.append(zed_root / "credentials")
        denied.append(zed_root / "config.yaml")
    return denied
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Zed
    secrets (``~/.zed/.env``, ``auth.json``) are *separate, more-specific*
    denied paths, so they stay blocked regardless of this exception â€” it can
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    This matches the symmetry of inbound delivery â€” Telegram/Discord/Slack
    ...
    ``ZED_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Zed-managed cache, under an operator-allowlisted root
    (``ZED_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # Cache / operator allowlist is always honored â€” these are unconditionally
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.zed/.env,
    # ~/.zed/auth.json, etc. â€” so the obvious prompt-injection sites
    ...
    # credential locations remain blocked even when "recent" â€” see
...
# ---------------------------------------------------------------------------
# Media-delivery extension allowlist â€” SINGLE SOURCE OF TRUTH
#
# Both extractors that turn response text into native attachments derive their
# extension set from this tuple:
#   * ``extract_media()``       â€” explicit ``MEDIA:<path>`` tags
#   * ``extract_local_files()`` â€” bare absolute/home paths the agent mentions
#
...
# invisible to extract_local_files â€” the file was never delivered (issue
...
# ``X:\\`` or ``X:/`` (Windows drive-letter absolute â€” #34632).
...
#
# One entry point for "I have raw attachment bytes from a platform â€” cache them
...
# platform â€” not Telegram-specific.
# ---------------------------------------------------------------------------
...
def cache_media_bytes(
    ...
    *,
    ...
    when the extension/MIME are ambiguous â€” e.g. a Telegram native photo whose
...
class MessageEvent:
    ...
    def get_command_args(self) -> str:
        ...
        # iOS auto-corrects -- to â€” (em dash) and - to â€“ (en dash)
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?zed\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+zed[.!?\s]*$", re.IGNORECASE),
)
...
class EphemeralReply(str):
    ...
    silently ignore the TTL â€” the message is sent normally and left in
...
class BasePlatformAdapter(ABC):
    ...
    # block (no language tag â€” Slack mrkdwn would print the tag as a literal
    ...
    # Zed commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    # users the form that actually works everywhere.  Capability flag â€”
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # maps session â†’ the specific Task currently processing it so that
        ...
        self._busy_text_mode: str = (
            os.environ.get("ZED_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "ZED_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "ZED_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def enforces_own_access_policy(self) -> bool:
        ...
        documented config-driven access surface â€” ``dm_policy`` / ``group_policy`` /
        ``allow_from`` / ``group_allow_from`` in ``PlatformConfig.extra`` â€” and
        ...
        for the chat type is an actual ``"allowlist"`` restriction â€” never for
        ``"open"`` (that would be the network-exposed fail-open SECURITY.md Â§2.6
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Zed sends
        ...
        Default implementation returns False â€” legacy platforms keep the
    ...
    # â”€â”€ Structured stream-event rendering â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    #
    ...
    def render_message_event(self, event: Any, sink: Any) -> None:
        ...
        if isinstance(event, MessageChunk):
            ...
            # An intermediate stop (text â†’ tool â†’ text) is a segment break;
    ...
    def format_tool_event(self, event: Any, *, mode: str = "all",
        ...
        emoji = get_tool_emoji(event.tool_name, default="âš™ï¸")
        ...
        # progress bubbles compact â€” they persist as permanent messages).
    ...
    def _should_auto_tts_for_chat(self, chat_id: str) -> bool:
        ...
        Decision layers (Issue #16007):
          1. Explicit ``/voice on`` or ``/voice tts`` â†’ always fire (even if
             ``voice.auto_tts`` is False).
          2. Explicit ``/voice off`` â†’ never fire.
    ...
    async def create_handoff_thread(
        ...
        session to a thread-capable platform â€” the new thread isolates the
        ...
        Default implementation returns ``None`` â€” adapters that support
    ...
    async def edit_message(
        ...
        *,
        ...
        of message lifecycle state â€” an edit is an edit.  Platforms that
        ...
        transition the UI out of the streaming indicator â€” those should
    ...
    async def delete_message(
        ...
        """
        Delete a previously sent message.  Optional â€” platforms that don't
    ...
    def _schedule_ephemeral_delete(
        ...
        Best-effort â€” failures (gateway restart, permission denied, message
    ...
    async def send_slash_confirm(
        ...
        acknowledge â€” the current caller is ``/reload-mcp``, which
    ...
    async def send_clarify(
        ...
        Two render modes:
          ...
          * **Multiple choice** (``choices`` is a non-empty list) â€” adapters
          ...
          * **Open-ended** (``choices`` is None or empty) â€” render the
        ...
        which works on every platform â€” the user replies with a number
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.zed/media/generated/stale.png"}
        ...
        * Within such a span, only a ``MEDIA:`` followed by a **bare** path
          ...
          tag â€” a real LLM output format the extractor supports â€” is not bare and
          is left alone.
    ...
    def extract_media(content: str) -> Tuple[List[Tuple[str, bool]], str]:
        ...
        user-visible text. Per-file granularity is intentionally not exposed â€”
        ...
        # Strip [[as_document]] directive â€” callers inspect the original
        ...
        # *unmasked* ``cleaned``. Masking is only a locator â€” protected spans
    ...
    async def _keep_typing(
        ...
        the next tick fire a fresh send_typing on schedule â€” as long as
            ...
            while True:
                ...
                if chat_id not in self._typing_paused:
                    try:
                        ...
                        # Slow network â€” abandon this tick, keep the loop
    ...
    def register_post_delivery_callback(
        ...
        *,
        ...
        is already registered, the new callback is chained â€” both fire, in
    ...
    # â”€â”€ Processing lifecycle hooks â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # Subclasses override these to react to message processing events
    # (e.g. Discord adds ðŸ‘€/âœ…/âŒ reactions).
    ...
    def _session_task_is_stale(self, session_key: str) -> bool:
        ...
        install guards directly) â€” don't treat that as stale.  The on-entry
    ...
    def _heal_stale_session_lock(self, session_key: str) -> bool:
        ...
        for: without it, a split-brain â€” adapter still thinks the session is
        active, but nothing is actually processing â€” traps the chat in
    ...
    async def cancel_session_processing(
        ...
        *,
        ...
        stall the calling dispatch coroutine â€” particularly under pytest-
    ...
    async def _drain_pending_after_session_command(
        ...
        command-scoped guard, then â€” if a follow-up message landed while the
        command was running â€” spawns a fresh processing task for it.
        """
        await self._flush_text_debounce_now(session_key)
    ...
    async def _dispatch_active_session_command(
        ...
        try:
            ...
            # condition fix â€” issue #18912).  Previously the send happened
            ...
            # been sent â€” keeps ordering deterministic and avoids the race.
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        # normal dispatch so the user isn't trapped behind a dead guard â€”
        ...
        if session_key in self._active_sessions:
            ...
            if should_bypass_active_session(cmd):
                ...
                # /background, /restart) just need direct dispatch â€” they
            ...
            # user's answer.
            # Same shape as the /approve deadlock fix (PR #4926) â€” both
            ...
            if self._is_queue_text_debounce_candidate(event):
                logger.debug(
                    "[%s] New text message while session %s is active â€” "
                ...
                logger.debug(
                    "[%s] New message while session %s is active â€” queuing follow-up "
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          ZED_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          ZED_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          ZED_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("ZED_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
            min_ms, max_ms = 800, 2500
            return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
        # custom mode â€” tolerate malformed env vars instead of crashing.
        try:
            min_ms = int(os.getenv("ZED_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("ZED_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # for system notices like "âœ¨ New session started!" that the user
            ...
            if session_key in self._pending_messages:
                ...
                # and only CLEAR the interrupt Event â€” do NOT delete the entry.
            ...
            # not at the top of this task.  _zed_run_generation is set on
            # the interrupt event by GatewayRunner._bind_adapter_run_generation
            # during _handle_message_with_agent â€” which happens DURING the
            ...
            if late_pending is not None:
                ...
                if (
                    ...
                    # task picks it up â€” avoids spawning two concurrent
                    ...
                    logger.debug(
                        "[%s] Late-arrival pending message during cleanup â€” spawning drain task",
                        self.name,
                    )
                    _active = self._active_sessions.get(session_key)
                    if _active is not None:
                    ...
                    try:
                        ...
                        # Tests stub create_task() with non-hashable sentinels; tolerate.
                        pass
                # Leave _active_sessions[session_key] populated â€” the drain
                ...
                # running â€” letting a concurrent inbound message pass
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        # would drop the reference â€” the task runs untracked against a
