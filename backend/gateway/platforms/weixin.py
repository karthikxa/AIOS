...
Connects Zed Agent to WeChat personal accounts via Tencent's iLink Bot API.
...
RATE_LIMIT_ERRCODE = -2  # iLink frequency limit â€” backoff and retry
...
def _account_dir(zed_home: str) -> Path:
    path = Path(zed_home) / "weixin" / "accounts"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _account_file(zed_home: str, account_id: str) -> Path:
    return _account_dir(zed_home) / f"{account_id}.json"
...
def save_weixin_account(
    ...
    *,
    ...
    path = _account_file(zed_home, account_id)
...
def load_weixin_account(zed_home: str, account_id: str) -> Optional[Dict[str, Any]]:
...
class ContextTokenStore:
    ...
    def __init__(self, zed_home: str):
        self._root = _account_dir(zed_home)
...
def _looks_like_heading_line_for_weixin(line: str) -> bool:
    ...
    if _HEADER_RE.match(stripped):
        return True
    return len(stripped) <= 24 and stripped.endswith((":", "ï¼š"))
    ...
    for item in item_list:
        if item.get("type") == ITEM_TEXT:
            ...
            if ref_type in {ITEM_IMAGE, ITEM_VIDEO, ITEM_FILE, ITEM_VOICE}:
                ...
                prefix = f"[å¼•ç”¨åª’ä½“: {title}]\n" if title else "[å¼•ç”¨åª’ä½“]\n"
...
def _sync_buf_path(zed_home: str, account_id: str) -> Path:
    return _account_dir(zed_home) / f"{account_id}.sync.json"


def _load_sync_buf(zed_home: str, account_id: str) -> str:
    path = _sync_buf_path(zed_home, account_id)
    if not path.exists():
        return ""
    try:
...
def _save_sync_buf(zed_home: str, account_id: str, sync_buf: str) -> None:
    path = _sync_buf_path(zed_home, account_id)
    ...
    *,
    ...
    async with aiohttp.ClientSession(trust_env=True, connector=_make_ssl_connector()) as session:
        ...
        try:
            ...
            print(f"ï¼ˆç»ˆç«¯äºŒç»´ç æ¸²æŸ“å¤±è´¥: {_qr_exc}ï¼Œè¯·ç›´æŽ¥æ‰“å¼€ä¸Šé¢çš„äºŒç»´ç é“¾æŽ¥ï¼‰")
        ...
        while time.monotonic() < deadline:
            ...
            if status == "wait":
                ...
                refresh_count += 1
                if refresh_count > 3:
                    print("\näºŒç»´ç å¤šæ¬¡è¿‡æœŸï¼Œè¯·é‡æ–°æ‰§è¡Œç™»å½•ã€‚")
                    return None
                print(f"\näºŒç»´ç å·²è¿‡æœŸï¼Œæ­£åœ¨åˆ·æ–°... ({refresh_count}/3)")
                try:
                    qr_resp = await _api_get(
                        session,
                        base_url=ILINK_BASE_URL,
                ...
                print(f"\nå¾®ä¿¡è¿žæŽ¥æˆåŠŸï¼Œaccount_id={account_id}")
                return {
                    "account_id": account_id,
                    "token": token,
                    "base_url": base_url,
...
class WeixinAdapter(BasePlatformAdapter):
    """Native Zed adapter for Weixin personal accounts."""
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._zed_home = zed_home
        self._token_store = ContextTokenStore(zed_home)
        ...
        if self._account_id and not self._token:
            persisted = load_weixin_account(zed_home, self._account_id)
            if persisted:
    ...
    async def connect(self) -> bool:
        ...
        if self._group_policy != "disabled":
            logger.warning(
                ...
                "these accounts, so group messages may never reach Zed regardless of this "
    ...
    async def _poll_loop(self) -> None:
        ...
        sync_buf = _load_sync_buf(self._zed_home, self._account_id)
        ...
        while self._running:
            try:
                ...
                if new_sync_buf:
                    ...
                    _save_sync_buf(self._zed_home, self._account_id, sync_buf)
    ...
    async def _send_text_chunk(
        ...
        *without* ``context_token`` â€” iLink accepts tokenless sends as a
            ...
            try:
                ...
                if resp and isinstance(resp, dict):
                    ...
                    if (ret is not None and ret not in {0,}) or (errcode is not None and errcode not in {0,}):
                        ...
                        # Session expired â€” strip token and retry once
                        if is_session_expired and not retried_without_token and context_token:
                            ...
                            continue
                        # Rate limit (-2) â€” backoff and retry
                        is_rate_limited = (
    ...
    async def send(
        ...
        try:
            ...
            for idx, chunk in enumerate(chunks):
                client_id = f"zed-weixin-{uuid.uuid4().hex}"
    ...
    async def _ensure_typing_ticket(self, chat_id: str) -> Optional[str]:
        ...
        ``send_typing`` and ``stop_typing`` silently no-op â€” leaving the
        ...
        if not self._send_session or not self._token:
            return None
        # Ticket expired or never fetched â€” refresh via getConfig.
    ...
    async def _send_file(
        ...
        if caption:
            last_message_id = f"zed-weixin-{uuid.uuid4().hex}"
            await _send_message(
                self._send_session,
                base_url=self._base_url,
                token=self._token,
...
async def send_weixin_direct(
    *,
    ...
    token_store = ContextTokenStore(str(get_zed_home()))
