...
class SmsAdapter(BasePlatformAdapter):
    """
    Twilio SMS <-> Zed gateway adapter.

    Each inbound phone number gets its own Zed session (multi-tenant).
    ...
    async def connect(self) -> bool:
        ...
        if not self._from_number:
            msg = "[sms] TWILIO_PHONE_NUMBER not set â€” cannot send replies"
            logger.error(msg)
