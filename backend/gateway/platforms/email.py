"""
Email platform adapter for the Zed gateway.

Allows users to interact with Zed by sending emails.
Uses IMAP to receive and SMTP to send messages.

Environment variables:
    EMAIL_IMAP_HOST     â€” IMAP server host (e.g., imap.gmail.com)
    EMAIL_IMAP_PORT     â€” IMAP server port (default: 993)
    EMAIL_SMTP_HOST     â€” SMTP server host (e.g., smtp.gmail.com)
    EMAIL_SMTP_PORT     â€” SMTP server port (default: 587)
    EMAIL_ADDRESS       â€” Email address for the agent
    EMAIL_PASSWORD      â€” Email password or app-specific password
    EMAIL_POLL_INTERVAL â€” Seconds between mailbox checks (default: 15)
    EMAIL_ALLOWED_USERS â€” Comma-separated list of allowed sender addresses
"""
...
# Automated sender patterns â€” emails from these are silently ignored
_NOREPLY_PATTERNS = (
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "zed-agent" "version" "{_zed_version}" '
            '"vendor" "zedteam" '
            '"support-email" "noreply@zedteam.com")',
        )
    except Exception as e:  # noqa: BLE001 â€” best-effort, never fatal
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Skip attachments â€” configured via config.yaml:
        #   platforms:
        #     email:
    ...
    def _trim_seen_uids(self) -> None:
        ...
        beyond the cap, we keep only the highest half â€” old UIDs are safe to
        ...
        if len(self._seen_uids) <= self._seen_uids_max:
            return
        try:
            # UIDs are bytes like b'1234' â€” sort numerically and keep top half
    ...
    def _connect_smtp(self) -> smtplib.SMTP:
        ...
        Returns a connected SMTP object with TLS established â€” callers
    ...
    async def _dispatch_message(self, msg_data: Dict[str, Any]) -> None:
        ...
        # Skip senders not in EMAIL_ALLOWED_USERS â€” prevents the adapter
        ...
        for att in attachments:
            ...
            if att["type"] == "image" and msg_type == MessageType.TEXT:
                ...
                # gates strictly on MessageType.DOCUMENT â€” so DOCUMENT is the
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Zed Agent")
        ...
        msg_id = f"<zed-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
    ...
    async def send_typing(self, chat_id: str, metadata: Optional[Dict[str, Any]] = None) -> None:
        """Email has no typing indicator â€” no-op."""
    ...
    async def send_multiple_images(
        ...
        images). No hard cap â€” email clients handle dozens of
