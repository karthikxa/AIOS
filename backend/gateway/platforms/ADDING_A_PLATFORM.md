...
There are two ways to add a platform to the Zed gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.zed/plugins/` (or under `plugins/platforms/`
...
- `env_enablement_fn: () -> Optional[dict]` â€” seeds `PlatformConfig.extra`
  ...
  `zed gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
- `apply_yaml_config_fn: (yaml_cfg, platform_cfg) -> Optional[dict]` â€”
...
- `cron_deliver_env_var: str` â€” name of the `*_HOME_CHANNEL` env var.  When
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries â€”
  auto-populate `OPTIONAL_ENV_VARS` in `zed_cli/config.py` so the setup
...
two transport modes the user picks between â€” unofficial vs official
APIs, polling vs websocket, library A vs library B â€” the right
...
filters, and the WhatsApp-flavored markdown conversion â€” everything
...
come **first** in the bases list â€” `class WhatsAppAdapter(Mixin,
BasePlatformAdapter)` â€” so the mixin's `format_message` overrides
...
Checklist for integrating a platform directly into the Zed core.
Use this as a reference when building a built-in adapter â€” every item here
...
See `gateway/platforms/telegram.py`, `discord.py`, and `whatsapp_cloud.py` for reference implementations. The button-callback id convention (`cl:<id>:<idx>`, `appr:<id>:<choice>`, `sc:<choice>:<id>`) is shared across adapters â€” match it so the gateway-side resolvers work without modification.
...
And add it to the `zed-gateway` composite:

```python
"zed-gateway": {
    "includes": [..., "zed-your-platform"]
}
```
...
## 12. Status Display (`zed_cli/status.py`)
...
## 13. Gateway Setup Wizard (`zed_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ zed_cli/ toolsets.py \
  --include="*.py" -l | sort -u
