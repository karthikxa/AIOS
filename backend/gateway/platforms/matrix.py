...
Environment variables:
    ...
    MATRIX_USER_ID              Full user ID (@bot:server) â€” required for password login
...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Zed command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Zed accepts ``!command`` as a Matrix-friendly alias, but only for
    ...
    *name* only by underscoreâ†’hyphen normalization, e.g. ``reload_skills`` â†’
    ...
    left as-is â€” the gateway dispatcher resolves them to their canonical name.
    """
    if not name:
        return None
    ...
    # alias passthrough â€” the gateway dispatcher canonicalizes aliases itself.
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Zed ``/command`` text."""
...
# Uses get_zed_home() so each profile gets its own Matrix store.
from zed_constants import get_zed_dir as _get_zed_dir

_STORE_DIR = _get_zed_dir("platforms/matrix/store", "matrix/store")
...
def _check_e2ee_deps() -> bool:
    ...
    store backend (mautrix.crypto.store.asyncpg.PgCryptoStore â€” yes, the
...
def check_matrix_requirements() -> bool:
    ...
    missing â€” not just mautrix itself.  Previously this short-circuited on
...
class _CryptoStateStore:
    ...
    async def find_shared_rooms(self, user_id: str) -> list:
        # Return all joined rooms â€” simple but correct for a single-user bot.
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches Zed
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # See #12614 â€” the symptom is "bot joins rooms but never replies".
        ...
        # Mention/thread gating â€” parsed once from config.extra or env vars.
        ...
        # errors in some clients.  5s is empirically safe; not user-tunable â€”
        ...
        # Proxy support â€” resolve once at init, reuse for all HTTP traffic.
        self._proxy_url: str | None = resolve_proxy_url(platform_env_var="MATRIX_PROXY")
        if self._proxy_url:
            logger.info("Matrix: proxy configured â€” %s", self._proxy_url)
        try:
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("ZED_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("ZED_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
    ...
    def _parse_thread_require_mention(config) -> bool:
        ...
        if configured is not None:
            ...
            if isinstance(configured, str):
                return configured.lower() not in {"false", "0", "no", "off"}
            # int, float, etc. â€” truthiness fallback
            return bool(configured)
        return os.getenv(
    ...
    async def _verify_device_keys_on_server(self, client: Any, olm: Any) -> bool:
        ...
        try:
            ...
            logger.error(
                "Matrix: cannot verify device keys on server: %s â€” refusing E2EE",
                exc,
                exc_info=True,
            )
            return False
        ...
        if not our_keys:
            logger.warning("Matrix: device keys missing from server â€” re-uploading")
        ...
        if server_ed25519 != local_ed25519:
            if olm.account.shared:
                logger.error(
                    "Matrix: server has different identity keys for device %s â€” "
            ...
            logger.warning(
                "Matrix: server has stale keys for device %s â€” attempting re-upload",
                client.device_id,
            )
            try:
    ...
    async def connect(self) -> bool:
        ...
        if self._access_token:
            ...
            try:
                ...
                logger.error(
                    "Matrix: whoami failed â€” check MATRIX_ACCESS_TOKEN and MATRIX_HOMESERVER: %s",
                    exc,
                    exc_info=True,
                )
            ...
            try:
                ...
                logger.error("Matrix: login failed â€” %s", exc)
                await api.session.close()
                return False
        else:
            logger.error(
        ...
        if self._encryption:
            if not _check_e2ee_deps():
                if self._e2ee_mode == "optional":
                    ...
                    logger.error(
                        "Matrix: E2EE is required but dependencies are missing. %s. "
                        "Refusing to connect â€” encrypted rooms would silently fail.",
                        _E2EE_INSTALL_HINT,
                    )
                    await api.session.close()
                    return False
                            ...
                            if output_error == "not_configured":
                                ...
                                try:
                                    ...
                                    logger.warning(
                                        ...
                                        "(non-fatal â€” Element will show 'not verified by its owner'): %s",
                                        exc,
                                    )
    ...
    async def send_exec_approval(
        ...
        text = (
            "âš ï¸ **Dangerous command requires approval**\n"
            f"```\n{cmd_preview}\n```\n"
    ...
    async def send_model_picker(
        ...
        for emoji, model_id, provider_slug, provider_name in flat_choices:
            ...
            lines.append(f"{emoji} `{model_id}` â€” {provider_name}")
    ...
    def format_message(self, content: str) -> str:
        """Pass-through â€” Matrix supports standard Markdown natively."""
            ...
            try:
                ...
                if _sync_msg and isinstance(_sync_msg, str):
                    ...
                    if "m_unknown_token" in _lower or "unknown_token" in _lower:
                        logger.error(
                            "Matrix: permanent auth error from sync: %s â€” stopping",
                            _sync_msg,
                        )
                        return
                ...
                if (
                    ...
                    logger.error(
                        "Matrix: permanent auth error: %s â€” stopping sync", exc
                    )
                    return
                logger.warning("Matrix: sync error: %s â€” retrying in 5s", exc)
    ...
    def _is_self_sender(self, sender: str) -> bool:
        ...
        lowercasing â€” some homeservers normalize the localpart case
        ...
        True defensively â€” an unidentified bot dropping its own events
    ...
    def _is_system_or_bridge_sender(sender: str) -> bool:
        ...
        authorization â€” and every outbound message relayed by the bridge
        would then loop back into the agent as an "authorized user
        message", which is the root of issue #15763.

        Matches:
            ``@_something:server``   â€” appservice namespace convention
            ``@:server``             â€” malformed / empty localpart
            ``:server``              â€” malformed, no leading ``@``
        """
        s = (sender or "").strip()
        if not s:
            return True
    ...
    async def _on_room_message(self, event: Any) -> None:
        ...
        logger.debug(
            "Matrix: callback fired â€” event %s from %s in %s",
        ...
        # user_id hasn't been resolved yet â€” see _is_self_sender docstring
        # and issue #15763).
        ...
        if event_ts and event_ts < self._startup_ts - _STARTUP_GRACE_SECONDS:
            ...
            # real time â€” every live event then looks "older than startup".
            ...
            #    room can deliver events spanning hours/days â€” those skews
            ...
            if not self._clock_skew_warned and (
                ...
                if 5 < skew < 86400:
                    if self._late_grace_drops == 0:
                        ...
                        # Consistent offset â†’ likely real clock skew.
                        self._late_grace_drops += 1
                    else:
                        # Varied skew â†’ likely backfill, restart sampling.
                        self._late_grace_skew = skew
                        self._late_grace_drops = 1
                    if self._late_grace_drops >= 3:
                        logger.warning(
                            ...
                            "â‰ˆ %.0fs). The host system clock is likely set "
        ...
        # Get msgtype â€” either from content object or raw dict.
    ...
    async def _resolve_message_context(
        ...
        # m.mentions.user_ids (MSC3952 / Matrix v1.7) â€” authoritative mention signal.
        ...
        if not is_dm:
            # allowed_rooms check (whitelist â€” must pass before other gating).
            ...
            if self._allowed_rooms and room_id not in self._allowed_rooms:
                logger.debug(
                    "Matrix: ignoring message %s in %s â€” room not in "
            ...
            if self._require_mention and not is_free_room and not in_bot_thread:
                if not is_mentioned and not is_command:
                    logger.debug(
                        "Matrix: ignoring message %s in %s â€” no @mention "
            ...
            # where multiple bots all participate in the same thread.
                ...
                if not is_mentioned:
                    logger.debug(
                        "Matrix: ignoring message %s in thread %s â€” "
    ...
    def _is_bot_mentioned(
        ...
        field with the bot's user-id, we trust it â€” even when the visible
        body text does not contain an explicit ``@bot`` string (some clients
        only render mention "pills" in ``formatted_body`` or use display
        names).
        """
        # m.mentions.user_ids â€” authoritative per MSC3952 / Matrix v1.7.
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Zed Agent" become "Agent".
        """
        if not body:
            return ""
    ...
    def _markdown_to_html(self, text: str) -> str:
        ...
        links, blockquotes, lists, and horizontal rules â€” everything the
