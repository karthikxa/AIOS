...
Config: ~/.zed/feishu_comment_rules.json (mtime-cached, hot-reload).
Pairing store: ~/.zed/feishu_comment_pairing.json.
"""
...
# ---------------------------------------------------------------------------
#
# Uses the canonical ``get_zed_home()`` helper (ZED_HOME-aware and
...
RULES_FILE = get_zed_home() / "feishu_comment_rules.json"
PAIRING_FILE = get_zed_home() / "feishu_comment_pairing.json"
...
# ---------------------------------------------------------------------------
# Rule resolution  (Â§8.4 field-by-field fallback)
# ---------------------------------------------------------------------------
...
def resolve_rule(
    ...
    """Resolve effective rule: exact doc â†’ wiki key â†’ wildcard â†’ top-level â†’ defaults."""
...
def _print_status() -> None:
    ...
    if cfg.documents:
        ...
        for key, rule in sorted(cfg.documents.items()):
            ...
            print(f"  [{key}] {', '.join(parts) if parts else '(empty â€” inherits all)'}")
    else:
