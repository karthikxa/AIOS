...
(``~/.zed/plans/god-file-decomposition.md``, Phase 3 mechanical mixin lifts).
...
class GatewayAuthorizationMixin:
    ...
    def _adapter_group_has_sender_allowlist(
        ...
        restricting which senders inside that group can invoke Zed. If such a
    ...
    def _get_unauthorized_dm_behavior(self, platform: Optional[Platform]) -> str:
        ...
        if config and hasattr(config, "unauthorized_dm_behavior"):
            if config.unauthorized_dm_behavior != "pair":  # non-default â†’ explicit override
