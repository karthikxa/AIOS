...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `zed tools` (opt-in, default OFF).
      2. `DISCORD_BOT_TOKEN` is set â€” the tool's `check_fn` gates on it
         at registry time, so the toolset being enabled in config is not
         enough if the token isn't configured.

    Returns False (safe default â€” keeps the stale-API disclaimer) on any
...
def build_session_context_prompt(
    ...
    *,
    ...
    # conversation.  Don't pin a single user name in the system prompt â€” it
    ...
    if context.shared_multi_user_session:
        ...
        lines.append(
            f"**Session type:** {session_label} â€” messages are prefixed "
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # Discord tools loaded this session â€” i.e. the user opted into
        # `discord` / `discord_admin` via `zed tools` AND the bot
        ...
        lines.append(
            ...
            "Keep responses short and conversational â€” think texts, not essays. "
            ...
            "one idea per bubble, 1â€“3 sentences each. "
    ...
    lines.append(
        f"- `\"local\"` â†’ Save to local files only ({display_zed_home()}/cron/output/)"
    )
...
class SessionEntry:
    ...
    # context-note prepend â€” both wrong for an explicit manual reset.
    # See issue #6508.
    ...
    # restarts â€” prevents redundant finalization runs.
    ...
    # ``resume_pending`` preserves the existing session_id on next access â€”
...
def _session_key_namespace(profile: Optional[str]) -> str:
    ...
    ``main`` is a static namespace literal (NOT a branch name â€” branching keys
    off ``session_id``, not this slot). Multi-profile multiplexing reuses this
    slot to carry the profile:

    - default profile (or ``None``/``""``/``"default"``) â†’ ``agent:main`` â€”
    ...
    - named profile ``coder`` â†’ ``agent:coder`` â€” keeps the same positional
...
def build_session_key(
    ...
    It defaults to ``None`` â‡’ the legacy ``agent:main`` namespace, so callers
    ...
    if source.chat_type == "dm":
        ...
        # No chat_id â€” fall back to the sender's own identifier before the
        ...
        # single cached agent ends up serving multiple people's conversations â€”
...
class SessionStore:
    ...
    def _resolve_profile_for_key(self, source: Optional[SessionSource] = None) -> Optional[str]:
        ...
        keys stay in the legacy ``agent:main`` namespace â€” byte-identical to
        before. When enabled, prefers the profile the inbound source was routed
        to (``source.profile`` â€” set by the /p/<profile>/ URL prefix or
    ...
    def get_or_create_session(
        ...
        with self._lock:
            ...
            if session_key in self._entries and not force_new:
                ...
                # broke a stuck loop â€” #7536).  ``suspended`` is the hard
                ...
                if entry.suspended:
                    ...
                    # means a re-interrupted retry keeps trying â€” the
    ...
    def mark_resume_pending(
        ...
        with self._lock:
            ...
            if session_key in self._entries:
                entry = self._entries[session_key]
                # Never override an explicit ``suspended`` â€” that is a hard
    ...
    def prune_old_entries(self, max_age_days: int) -> int:
        ...
        how old it is.  Entries marked ``suspended`` are kept â€” the user
        ...
        the transcript in SQLite stays, but the session_key â†’ session_id
        ...
        with self._lock:
            ...
            for key, entry in list(self._entries.items()):
                ...
                # attached â€” the user may still be waiting on output.
        ...
        if self._db and not skip_db:
            try:
                self._db.append_message(
                    ...
                    # Platform-side message id (yuanbao msg_id, telegram update_id, â€¦).
    ...
    def load_transcript(self, session_id: str) -> List[Dict[str, Any]]:
        ...
        in spec 002 â€” pre-DB sessions on existing disks have already been
