...
``self`` state, so they live on a mixin that ``GatewayRunner`` inherits â€” the
...
def _acquire_singleton_lock(lock_path) -> "tuple[Optional[object], str]":
    ...
    runs its own ``release_stale_claims`` â†’ promote â†’ dispatch loop), double
    claim-attempt events in the event log, and â€” with ``wal_autocheckpoint=0`` â€”
    ...
    Returns ``(handle, "held")`` on success â€” the caller keeps the file handle
    ...
    without flock, or the status.py helpers are unimportable) â€” caller falls
...
class GatewayKanbanWatchersMixin:
    ...
    async def _kanban_notifier_watcher(self, interval: float = 5.0) -> None:
        ...
        **Multi-board:** iterates every board discovered on disk per
        ...
        cross boards, so delivery semantics are unchanged â€” this is
        purely a fan-out of the single-DB poll.
        """
        # Gate: only the dispatch-owning gateway opens kanban DBs for notifier polling.
        # Non-dispatch gateways have no subscriptions to deliver â€” all kanban state lives
        ...
        try:
            ...
            return
        env_override = os.environ.get("ZED_KANBAN_DISPATCH_IN_GATEWAY", "").strip().lower()
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban notifier: disabled via ZED_KANBAN_DISPATCH_IN_GATEWAY env")
            return
        try:
            cfg = _load_config()
        ...
        # means the chat is dead (deleted, bot kicked, etc.) â€” after N
        ...
        while self._running:
            try:
                def _collect():
                    ...
                    # ZED_KANBAN_DB pins the board path; without this guard
                    ...
                    for board_meta in boards:
                        ...
                        try:
                            ...
                            # â€” issue #21378) on every gateway start against
                            ...
                            for sub in subs:
                                ...
                                logger.debug(
                                    "kanban notifier: claimed %d event(s) for %s on board %s cursor %sâ†’%s",
                    ...
                    for ev in d["events"]:
                        ...
                        if kind == "completed":
                            ...
                            if payload_summary:
                                ...
                                handoff = f"\n{r}"
                            msg = (
                                f"âœ” {tag}Kanban {sub['task_id']} done"
                                f" â€” {title}{handoff}"
                            )
                        elif kind == "blocked":
                            reason = ""
                            ...
                            msg = f"â¸ {tag}Kanban {sub['task_id']} blocked{reason}"
                        elif kind == "gave_up":
                            err = ""
                            if ev.payload and ev.payload.get("error"):
                                err = f"\n{str(ev.payload['error'])[:200]}"
                            msg = (
                                f"âœ– {tag}Kanban {sub['task_id']} gave up "
                                f"after repeated spawn failures{err}"
                            )
                        elif kind == "crashed":
                            msg = (
                                f"âœ– {tag}Kanban {sub['task_id']} worker crashed "
                            ...
                            msg = (
                                f"â± {tag}Kanban {sub['task_id']} timed out "
                        ...
                        # is the dedup mechanism â€” it prevents re-delivery
    ...
    async def _deliver_kanban_artifacts(
        ...
        *,
        ...
        Sources scanned, in priority order:
          1. ``event_payload['artifacts']`` (explicit list â€” preferred)
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        """Embedded kanban dispatcher â€” one tick every `dispatch_interval_seconds`.

        Gated by `kanban.dispatch_in_gateway` in config.yaml (default True).
        When true, the gateway hosts the single dispatcher for this profile:
        no separate `zed kanban daemon` process needed. When false, the
        loop exits immediately and an external daemon is expected.

        Each tick calls :func:`kanban_db.dispatch_once` inside
        ``asyncio.to_thread`` so the SQLite WAL lock never blocks the
        event loop. Failures in one tick don't stop subsequent ticks â€”
        ...
        env_override = os.environ.get("ZED_KANBAN_DISPATCH_IN_GATEWAY", "").strip().lower()
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban dispatcher: disabled via ZED_KANBAN_DISPATCH_IN_GATEWAY env")
            return
        ...
        # frequency, double claim-attempt events, and â€” with
        # wal_autocheckpoint=0 â€” concurrent manual WAL checkpoints can corrupt
        ...
        try:
            ...
            interval = 60.0
        interval = max(interval, 1.0)  # sanity floor â€” tighter than this is a footgun
        ...
        # Read stale_timeout_seconds â€” 0 disables stale detection.
        ...
        # Read kanban.default_assignee â€” fallback profile for tasks
        ...
        # (the schema default) means "no fallback, keep skipping" â€”
        ...
        # Read kanban.max_in_progress_per_profile â€” per-profile concurrency
        ...
        # queue is non-empty but spawns are 0 for N consecutive ticks â€”
            ...
            try:
                ...
                if _is_corrupt_board_db_error(exc):
                    ...
                    logger.error(
                        ...
                        "then run `zed kanban init` if you need a fresh board.",
                        slug,
                        fingerprint[0],
                    )
                    return None
        ...
        def _ready_nonempty() -> bool:
            ...
            task on ANY board whose assignee maps to a real Zed profile
            ...
            PATH, missing venv, credential loss for a real Zed profile).
            """
            try:
                boards = _kb.list_boards(include_archived=False)
            except Exception:
        ...
        def _auto_decompose_tick() -> int:
            ...
            for b in boards:
                ...
                if attempted >= auto_decompose_per_tick:
                    break
                # Pin this board for the duration of the call â€” same
                # pattern as the dashboard specify endpoint. The
                # decomposer module connects with no board kwarg and
                # relies on the env var.
                prev_env = os.environ.get("ZED_KANBAN_BOARD")
                try:
                    os.environ["ZED_KANBAN_BOARD"] = slug
                    try:
                    ...
                    for tid in triage_ids:
                        ...
                        if outcome.ok:
                            ...
                            if outcome.fanout and outcome.child_ids:
                                logger.info(
                                    "kanban auto-decompose [%s]: %s â†’ %d children",
                                    slug, tid, len(outcome.child_ids),
                                )
                            else:
                                logger.info(
                                    "kanban auto-decompose [%s]: %s â†’ single task (no fanout)",
                                    slug, tid,
                                )
                        else:
                    ...
                    if prev_env is None:
                        os.environ.pop("ZED_KANBAN_BOARD", None)
                    else:
                        os.environ["ZED_KANBAN_BOARD"] = prev_env
            return successes
        ...
        while self._running:
            ...
            try:
                ...
                for slug, res in (results or []):
                    if res is not None and getattr(res, "spawned", None):
                        any_spawned = True
                        # Quiet by default â€” only log when something actually
            ...
            # Sleep in 1s slices so shutdown is snappy â€” otherwise a stop()
