...
Module-level run.py helpers a handler needs (``_zed_home``,
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Zed commands.
            ...
            if failed:
                for p, info in failed.items():
                    if info.get("paused"):
                        ...
                        lines.append(
                            f"  Â· {p.value} â€” retrying (attempt {attempts})"
                        )
            else:
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`zed gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
            ...
            if not failed[platform].get("paused"):
                return (
                    f"{platform.value} is already retrying â€” "
                    f"no resume needed."
                )
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        try:
            ...
            atomic_json_write(
                _zed_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        if _cost_warning is not None:
            async def _on_cost_confirm(choice: str) -> str:
                if choice == "cancel":
                    return (
                        f"ðŸŸ¡ Model switch cancelled. Current model unchanged "
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             â€” Zed default runtime
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return t("gateway.personality.none_configured", path=display_zed_home())
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _zed_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _zed_home, _platform_config_key
        import yaml
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.zed/pending/skills/``. (Note this is
        ...
        # the real review surface. (Note: `zed skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\nâ€¦ (truncated â€” full diff in "
                     f"~/.zed/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _zed_home, _load_gateway_config, _platform_config_key
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _zed_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_compress_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            lines = [f"ðŸ—œï¸ {summary['headline']}"]
            if focus_topic:
    ...
    async def _handle_credits_command(self, event: MessageEvent) -> str:
        ...
        if view.topup_url:
            ...
            lines.append("Complete your top-up in the browser â€” credits will appear in /credits shortly.")
        return "\n".join(lines)
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `zed bundles create <name> --skill <s1> --skill <s2>`\n"
        ...
        for info in bundles:
            ...
            lines.append(
                f"â€¢ `/{info['slug']}` â€” {desc} _({skill_count} skills)_"
            )
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command â€” update Zed Agent to the latest version.

        Spawns ``zed update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``zed update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from gateway.run import _zed_home, _resolve_zed_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        ...
        if is_managed():
            return f"âœ— {format_managed_message('update Zed Agent')}"
        ...
        if not zed_cmd:
            return t("gateway.update.zed_cmd_not_found")
        ...
        # Spawn `zed update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `zed update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                zed_cmd_str = " ".join(shlex.quote(part) for part in zed_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {zed_cmd_str} update --gateway"
