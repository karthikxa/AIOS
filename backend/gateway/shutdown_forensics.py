...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if zed_home_str:
            takeover_path = Path(zed_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(zed_home_str) / ".gateway-planned-stop.json"
            if planned_stop_path.exists():
                try:
                    ...
                    pass
    except Exception:  # noqa: BLE001 â€” never raise from a signal handler
        pass
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded zed-agent but never re-ran ``zed setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../zed-gateway.service"
    ...
    # that's the common case for zed.
    ...
    # constant in zed_cli/gateway.py.
    headroom = 30.0
