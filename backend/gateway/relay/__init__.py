"""Relay/connector support package for the Zed gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``zed gateway enroll`` writes them to
    ``~/.zed/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``ZED_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def _provision_url(relay_dial_url: str) -> str:
    """Map the ``ws(s)://â€¦/relay`` dial URL to the ``http(s)://â€¦/relay/provision`` POST URL."""
...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory â€” never written to ``~/.zed/.env``.
    ...
    sets neither ``ZED_MANAGED`` nor a ``.managed`` marker), so gating on it
    blocked the exact hosted case this is for. The real signal is "you pointed me
    at a connector and didn't pin a secret" â€” which is both NAS-independent and
    self-guarding:

      - A NAS-hosted agent: has ``GATEWAY_RELAY_URL``, no pinned secret, and a
        bootstrapped NAS token -> self-provisions.
      - A self-hosted operator who ran ``zed gateway enroll``: has a PINNED
    ...
    or config) is RESPECTED â€” self-provision skips so an operator pin isn't
    stomped.
    ...
    # Respect an already-present (pinned/stamped) secret â€” don't stomp it. This
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'zed'}"
