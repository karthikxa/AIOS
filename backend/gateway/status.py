...
The PID file lives at ``{ZED_HOME}/gateway.pid``.  ZED_HOME defaults to
``~/.zed`` but can be overridden via the environment variable.  This means
separate ZED_HOME directories naturally get separate PID files â€” a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting ZED_HOME."""
    home = get_zed_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("ZED_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "zed" / _LOCKS_DIRNAME
...
def looks_like_gateway_command_line(command: str | None) -> bool:
    ...
    (``"C:\\Program Files\\...\\zed-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Zed's
    ...
    for token in tokens:
        ...
        if basename in ("zed-gateway", "zed-gateway.exe"):
            return True
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return True  # bare `zed gateway` defaults to `run`
        return filtered[i + 1] == "run"
    return False


def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Zed gateway."""
...
def _pid_exists(pid: int) -> bool:
    ...
    if _IS_WINDOWS:
        ...
        try:
            os.kill(int(pid), 0)  # windows-footgun: ok â€” POSIX-only branch (the whole point of _pid_exists)
            return True
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `zed status` show per-profile
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different ZED_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``zed.service`` + ``zed-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``zed gateway stop`` on Windows would be misclassified as an
