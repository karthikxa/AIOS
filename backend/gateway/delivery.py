...
# ðŸ”‡, a bare ".", "â€¦", and the whitespace/marker-padded variants seen in the
...
class DeliveryRouter:
    ...
    def __init__(self, config: GatewayConfig, adapters: Dict[Platform, Any] = None):
        ...
        self.output_dir = get_zed_home() / "cron" / "output"
    ...
    def _save_full_output(self, content: str, job_id: str) -> Path:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        out_dir = get_zed_home() / "cron" / "output"
    ...
    def _filter_silence_narration_enabled(self) -> bool:
        ...
        ``ZED_FILTER_SILENCE_NARRATION`` env var overrides config when set;
        otherwise the ``gateway.filter_silence_narration`` config flag wins
        (default True).
        """
        env = os.getenv("ZED_FILTER_SILENCE_NARRATION")
        if env is not None:
    ...
    async def _deliver_to_platform(
        ...
        if len(content) > MAX_PLATFORM_OUTPUT:
            ...
            logger.info("Cron output truncated (%d chars) â€” full output: %s", len(content), saved_path)
            content = (
        ...
        # (*(silent)*, ðŸ”‡, a bare ".", etc.) before it ever reaches the adapter.
