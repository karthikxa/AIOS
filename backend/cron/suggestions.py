"""Suggested cron jobs â€” proposed automations the user accepts with one tap.

A *suggestion* is a ready-to-run cron job spec that Zed surfaces to the
...
flows through, regardless of where it came from:
  ...
  * ``catalog``  â€” a curated starter automation (daily briefing, important-mail
                   monitor, weekly digest, ...).
  * ``blueprint``   â€” the user installed a skill that carries a ``blueprint:`` block
                   (see ``tools/blueprints.py``); installing it registers a
                   suggestion instead of auto-scheduling.
  * ``usage``    â€” the background self-improvement review noticed a recurring
                   ask that a scheduled job would serve.
  * ``integration`` â€” the user connected an account (Gmail, GitHub, ...) and
...
the stored ``job_spec`` â€” there is NO second job engine. Suggestions never
...
Storage mirrors ``cron/jobs.py``: ``~/.zed/cron/suggestions.json``, atomic
...
CRON_DIR = get_zed_home().resolve() / "cron"
...
def _save_raw(suggestions: List[Dict[str, Any]]) -> None:
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            json.dump(
                {"suggestions": suggestions, "updated_at": _zed_now().isoformat()},
                f,
                indent=2,
            )
            f.flush()
...
def add_suggestion(
    *,
    ...
    ``job_spec`` is a dict of kwargs for ``cron.jobs.create_job`` â€” accepting
    ...
    with _suggestions_lock:
        ...
        for s in suggestions:
            if s.get("id") == suggestion_id:
                s["status"] = status
                s["resolved_at"] = _zed_now().isoformat()
                changed = True
                break
        if changed:
            _save_raw(suggestions)
...
def dismiss_suggestion(ref: str) -> bool:
    """Dismiss a suggestion (latched â€” never re-offered for its dedup_key)."""
