"""Automation Blueprints â€” parameterized automation blueprints with typed slots.
...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``zed://`` deep-link
...
class BlueprintSlot:
    ...
    optional: bool = False
    help: str = ""
    # When False, ``options`` are suggestions rather than a closed set â€”
...
CATALOG: List[AutomationBlueprint] = [
    ...
    AutomationBlueprint(
        ...
        slots=[
            ...
            BlueprintSlot(
                ...
                label="Only notify me if the mailâ€¦",
    ...
    AutomationBlueprint(
        ...
        slots=[
            BlueprintSlot(name="what", type="text", label="Remind me toâ€¦",
    ...
    AutomationBlueprint(
        ...
        prompt_template=(
            ...
            "loose ends from today. Keep it calm and brief â€” one message. If no "
    ...
    AutomationBlueprint(
        ...
        description="A recurring digest on a topic you care about â€” deduped "
        "against what was already sent, so only genuinely new items land.",
        category="general",
        schedule_template="{minute} {hour} * * {dow}",
        prompt_template=(
            "Search the web for new and noteworthy items about: {topic}. "
            "Dedupe against what you sent in previous runs â€” only include "
    ...
    AutomationBlueprint(
        ...
        "renewal, or due date â€” so nothing auto-charges by surprise.",
        category="general",
        schedule_template="{minute} {hour} * * {dow}",
        prompt_template=(
    ...
    AutomationBlueprint(
        ...
        "and stretch.",
        category="general",
        # NOTE: cron minute-field steps (*/90) wrap per hour â€” */90 and */120
    ...
    AutomationBlueprint(
        ...
        slots=[
            BlueprintSlot(
                name="topic", type="text", label="Learn aboutâ€¦",
    ...
    AutomationBlueprint(
        ...
        prompt_template=(
            ...
            "day â€” invite them to note one thing that went well, one thing they "
    ...
    AutomationBlueprint(
        ...
        schedule_template="{minute} {hour} * * *",
        prompt_template=(
            "Give the user one interesting '{flavor}' item for today â€” keep it "
...
def blueprint_slash_command(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the flattened ``/blueprint <key> slot=val â€¦`` command string.
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``zed://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"zed://blueprint/{quote(blueprint.key)}{qs}"
...
def blueprint_catalog_entry(blueprint: AutomationBlueprint) -> Dict[str, Any]:
    """Unified serializable shape for a blueprint â€” used by the docs generator
...
def _resolve_schedule(blueprint: AutomationBlueprint, values: Dict[str, Any]) -> str:
    ...
    if "{minute}" in sched or "{hour}" in sched:
        ...
        if not m:
            raise BlueprintFillError(f"invalid time {time_val!r} â€” use HH:MM (24h)")
        ...
        if "recurrence" in values:
            ...
            if preset not in WEEKDAY_PRESETS:
                raise BlueprintFillError(
                    f"unknown recurrence {preset!r} â€” one of {', '.join(WEEKDAY_PRESETS)}"
                )
    ...
    if "{interval_min}" in sched:
        ...
        if not iv.isdigit() or int(iv) <= 0:
            raise BlueprintFillError(f"invalid interval {iv!r} â€” minutes as a positive integer")
...
def fill_blueprint(
    ...
    *,
    ...
    options. The result is passed straight to ``create_job`` â€” no second schema.
    """
    ...
    if unknown:
        raise BlueprintFillError(
            ...
            f"{', '.join(unknown)} â€” valid: {', '.join(s.name for s in blueprint.slots)}"
        )
    ...
    for s in blueprint.slots:
        ...
        if s.type == "enum" and s.strict and s.options and str(raw) not in {str(o) for o in s.options}:
            raise BlueprintFillError(
                f"{s.name}={raw!r} not allowed â€” one of {', '.join(map(str, s.options))}"
            )
        resolved[s.name] = raw
