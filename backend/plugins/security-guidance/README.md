...
This is layer 1 of Anthropic's `security-guidance` plugin design â€” a fast
...
carries a per-extension `path_filter` lambda â€” Python-only rules skip `.js`,
...
```bash
zed plugins enable security-guidance
# or edit ~/.zed/config.yaml manually:
plugins:
  enabled:
    - security-guidance
```
...
|---|---|---|
| (none) | warn | Appends a `âš ï¸ Security guidance` block to the tool result. The file is written. |
...
| `SECURITY_GUIDANCE_DISABLE=1` | unset | Kill switch â€” plugin loads but does nothing. |

## What it does **not** do (yet)

* **No LLM diff review.** Anthropic's layer 2 spawns an auxiliary LLM call
  on every agent turn that touched files. On zed that would route
...
* **No project-local rules file.** Anthropic's `.claude/claude-security-guidance.md`
  ...
  add an analogous `.zed/security-guidance.md` once layer 2 lands.
...
* `__init__.py`, `plugin.yaml`, `README.md`, and tests are original work by
  NousResearch, MIT-licensed alongside the rest of zed-agent.
