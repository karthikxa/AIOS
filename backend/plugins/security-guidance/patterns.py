...
Pure data + one pure helper. No env-var reads, no I/O â€” kept side-effect-free
...
Modifications by NousResearch for the Zed Agent plugin port:
  - none to the pattern data itself; this file is byte-for-byte the upstream
    patterns.py at commit 0bde168 (2026-05-26). Zed-side wiring lives in
    __init__.py.
"""
...
_UNSAFE_DESERIALIZATION_REMINDER = """âš ï¸ Security Warning: Loading pickle data (or equivalents: cPickle, cloudpickle, dill, marshal, shelve, joblib, pandas.read_pickle, numpy with allow_pickle=True) from untrusted sources allows arbitrary code execution.
...
_UNSAFE_YAML_LOAD_REMINDER = """âš ï¸ Security Warning: yaml.load() / yaml.unsafe_load() execute arbitrary Python via !!python/object tags.

Use yaml.safe_load() if the file only contains simple data structures (dicts, lists, strings, numbers). If you need typed objects, parse with safe_load and validate the result against a schema (pydantic, msgspec, marshmallow) â€” never use a custom Loader that constructs arbitrary types."""

_UNSAFE_TORCH_LOAD_REMINDER = """âš ï¸ Security Warning: torch.load() defaults to weights_only=False, which unpickles arbitrary Python objects and allows arbitrary code execution.
...
SECURITY_PATTERNS = [
    {
        ...
        "reminder": """âš ï¸ Security Warning: You are editing a GitHub Actions workflow file. Be aware of these security risks:
...
- github.event.client_payload.* (repository_dispatch events â€” attacker can set any field)
...
- github.head_ref""",
    },
    {
        ...
        # Gate to JS/TS files â€” bare `exec(` otherwise fires on Python's
        ...
        "reminder": """âš ï¸ Security Warning: Using child_process.exec() can lead to command injection vulnerabilities.
...
Only use exec() if you absolutely need shell features and the input is guaranteed to be safe.""",
    ...
    },
    {
        ...
        "reminder": "âš ï¸ Security Warning: eval() executes arbitrary code and is a major security risk. Use JSON.parse() for data, ast.literal_eval() for Python literals, or a safe expression parser. If this is safe or is explicitly needed, briefly document that in a comment before continuing.",
    },
    {
        "ruleName": "react_dangerously_set_html",
        "substrings": ["dangerouslySetInnerHTML"],
        "reminder": "âš ï¸ Security Warning: dangerouslySetInnerHTML can lead to XSS vulnerabilities if used with untrusted content. Ensure all content is properly sanitized using an HTML sanitizer library like DOMPurify, or use safe alternatives.",
    },
    {
        "ruleName": "document_write_xss",
        "substrings": ["document.write"],
        "reminder": "âš ï¸ Security Warning: document.write() can be exploited for XSS attacks and has performance issues. Use DOM manipulation methods like createElement() and appendChild() instead.",
    },
    {
        "ruleName": "innerHTML_xss",
        "substrings": [".innerHTML =", ".innerHTML="],
        "reminder": "âš ï¸ Security Warning: Setting innerHTML with untrusted content can lead to XSS vulnerabilities. Use textContent for plain text or safe DOM methods for HTML content. If you need HTML support, consider using an HTML sanitizer library such as DOMPurify.",
    },
    {
    ...
    },
    {
        ...
        "reminder": "âš ï¸ Security Warning: os.system() runs a shell and is a command-injection sink. Use subprocess.run([...]) with a list of arguments instead. If this is safe or is explicitly needed, briefly document that in a comment before continuing.",
    },
    {
        "ruleName": "python_subprocess_shell",
        "regex": r"subprocess\.(?:run|call|Popen|check_output|check_call)\(.*shell\s*=\s*True",
        "reminder": """âš ï¸ Security Warning: Using subprocess with shell=True enables command injection.
...
When arguments are passed as a list without shell=True, special characters cannot be interpreted as shell metacharacters.""",
    ...
    # =====================================================================
    {
        ...
        "reminder": """âš ï¸ Security Warning: Using exec.Command with a shell interpreter (sh/bash) enables command injection.
...
- For numeric values: parse to int/float first""",
    ...
    },
    {
        ...
        "reminder": "âš ï¸ Security Warning: Use crypto.createCipheriv() / createDecipheriv(). createCipher was removed in Node 22 and derives the key insecurely (no IV, MD5-based KDF).",
    },
    {
        "ruleName": "aes_ecb_mode",
        "regex": r"\bAES\.MODE_ECB\b|\bmodes\.ECB\s*\(|[\x22\x27]aes-\d+-ecb[\x22\x27]",
        "reminder": "âš ï¸ Security Warning: Use AES-GCM or AES-CBC with HMAC. ECB mode leaks plaintext structure (identical blocks encrypt to identical ciphertext).",
    },
    {
        ...
        "reminder": "âš ï¸ Security Warning: Don't disable TLS verification. This allows MITM attacks. For self-signed dev certs, add the CA to your trust store or use a properly-issued cert.",
    },
    {
    ...
    },
    {
        ...
        "reminder": "âš ï¸ Security Warning: Use defusedxml.ElementTree. Python's stdlib XML parsers are vulnerable to XXE (external entity) and billion-laughs attacks by default.",
    },
    {
    ...
    },
    {
        ...
        "reminder": "âš ï¸ Security Warning: Use textContent or sanitize with DOMPurify. outerHTML assignment is an XSS sink equivalent to innerHTML.",
    },
    {
        "ruleName": "insertAdjacentHTML_xss",
        "substrings": [".insertAdjacentHTML("],
        "reminder": "âš ï¸ Security Warning: Use insertAdjacentText() or sanitize with DOMPurify. insertAdjacentHTML is an XSS sink.",
    },
    {
        ...
        "reminder": 'âš ï¸ Security Warning: Add integrity="sha384-..." crossorigin="anonymous" to external script tags. Loading scripts without Subresource Integrity exposes you to CDN compromise.',
    },
    {
        "ruleName": "torch_unsafe_load",
        # Suppressed by weights_only=True on the same line (within 200 chars). weights_only=False
        # still triggers. Multi-line calls false-positive â€” same known limitation as unsafe_yaml_load.
