...
Mounted at /api/plugins/zed-achievements/ by Hermes dashboard.
"""
...
try:
    ...
    def get_zed_home() -> Path:  # type: ignore[misc]
        val = (_os.environ.get("ZED_HOME") or "").strip()
...
def state_path() -> Path:
    return get_zed_home() / "plugins" / "zed-achievements" / "state.json"


def snapshot_path() -> Path:
    return get_zed_home() / "plugins" / "zed-achievements" / "scan_snapshot.json"


def checkpoint_path() -> Path:
    return get_zed_home() / "plugins" / "zed-achievements" / "scan_checkpoint.json"
