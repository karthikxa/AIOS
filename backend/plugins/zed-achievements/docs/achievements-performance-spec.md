# Zed Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: zed-achievements plugin
...
### `GET /api/plugins/zed-achievements/achievements`
...
### `POST /api/plugins/zed-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/zed-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/zed-achievements/overview`
