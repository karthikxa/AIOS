...
class KimiProfile(ProviderProfile):
    """Kimi/Moonshot â€” temperature omitted, thinking xor reasoning_effort."""
    ...
    def build_api_kwargs_extras(
        ...
        and a top-level ``reasoning_effort`` as mutually exclusive â€” sending
        ...
        back to ``extra_body.thinking`` â€” never both.
        """
        extra_body = {}
        top_level = {}

        if not reasoning_config or not isinstance(reasoning_config, dict):
            # No config â†’ thinking enabled, let the server pick the depth.
...
kimi = KimiProfile(
    ...
    default_headers={"User-Agent": "zed-agent/1.0"},
