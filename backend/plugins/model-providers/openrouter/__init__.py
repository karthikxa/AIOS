...
# for ``reasoning: {enabled: false}``). Everything Claude 4.6 and newer â€”
# including future date-stamped / named models (fable, mythos-class, â€¦) â€”
...
def _anthropic_reasoning_is_mandatory(model: str | None) -> bool:
    ...
    switch â€” sending ``reasoning: {enabled: false}`` makes OpenRouter emit
...
class OpenRouterProfile(ProviderProfile):
    """OpenRouter aggregator â€” provider preferences, reasoning config passthrough."""
    ...
    def fetch_models(
        ...
        *,
        ...
        """Fetch from public OpenRouter catalog â€” no auth required.

        Note: Tool-call capability filtering is applied by zed_cli/models.py
        via fetch_openrouter_models() â†’ _openrouter_model_supports_tools(), not
    ...
    def build_extra_body(
        ...
        # Pareto Code router â€” model-gated. The plugins block is only
    ...
    def build_api_kwargs_extras(
        ...
        **context: Any,
        ...
        if supports_reasoning:
            ...
            #   - ``{enabled: false}`` â†’ OpenRouter emits Anthropic's manual
            ...
            #     emit ``thinking: {type: "disabled"}`` â†’ the same 400 on every
            #     turn after the first tool call.
            # The only reliable behavior is to omit ``reasoning`` and let the
            # model default to adaptive. See zed-agent#42991 (disable case)
            # and the tool-replay follow-up.
            #
            # ``reasoning.effort`` being ignored does NOT mean these models have
            # no effort lever â€” OpenRouter honors the requested effort on the
            # top-level ``verbosity`` field instead (it maps to Anthropic's
            # ``output_config.effort``; ``reasoning.effort`` is accepted but
            # ignored â€” confirmed by OpenRouter's Claude migration docs and a
            # live token-spend probe in zed-agent#43432). Route the existing
...
openrouter = OpenRouterProfile(
    ...
    description="OpenRouter â€” unified API for 200+ models",
