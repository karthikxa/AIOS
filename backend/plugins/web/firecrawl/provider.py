"""Firecrawl web search + extract â€” plugin form.
...
Env vars::
    ...
    FIRECRAWL_GATEWAY_URL=...        # Zed tool-gateway (subscribers)
...
# module â€” so the patched name still references the same proxy instance.

if TYPE_CHECKING:
    from firecrawl import Firecrawl as FirecrawlSDK  # noqa: F401 â€” type hints only
...
def _load_firecrawl_cls() -> type:
    ...
    if _FIRECRAWL_CLS_CACHE is None:
        try:
            ...
            pass
        except Exception as exc:  # noqa: BLE001 â€” surface install hint
            raise ImportError(str(exc))
        from firecrawl import Firecrawl as _cls  # noqa: WPS433 â€” deliberately lazy
...
# state. The plugin reads/writes through that public module â€” see
...
def _is_tool_gateway_ready() -> bool:
    """Return True when gateway URL + Zed Subscriber token are available.
...
def check_firecrawl_api_key() -> bool:
    ...
    existing tests and the ``zed tools`` setup flow.
    """
...
def _firecrawl_backend_help_suffix() -> str:
    ...
    if not _wt.managed_nous_tools_enabled():
        return ""
    return (
        ", or use the Zed Tool Gateway via your subscription "
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if _wt.managed_nous_tools_enabled():
        message += (
            " With your Zed Subscription you can also use the Tool Gateway. "
            "run `zed tools` and select Zed Subscription as the web provider."
        )
    else:
...
def _get_firecrawl_client() -> Any:
    ...
    :mod:`tools.web_tools` for the same reason â€” see
...
class FirecrawlWebSearchProvider(WebSearchProvider):
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        top-level handler, which wraps them as ``tool_error(...)`` â€”
        ...
        # _get_firecrawl_client() raises ValueError on unconfigured systems â€”
                    ...
                    results.append(
                        {
                            ...
                            "content": "",
                            "error": (
                                "Scrape timed out after 60s â€” page may be too large "
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            "name": "Firecrawl",
            "badge": "paid Â· optional gateway",
            "tag": (
                "Full search + extract; supports direct API and "
                "Zed tool-gateway routing."
            ),
            "env_vars": [
                {
