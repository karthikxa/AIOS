...
``{title, url, description, position}`` rows every other Zed web
provider produces.
...
Optional knobs (under ``web.xai`` in ``config.yaml``)::
    ...
    web:
      xai:
        ...
        allowed_domains: ["x.ai"]     # max 5 â€” mutually exclusive with excluded_domains
        excluded_domains: ["bad.com"] # max 5 â€” mutually exclusive with allowed_domains
        timeout: 90                   # seconds (default 90)

Auth: reuses :func:`tools.xai_http.resolve_xai_http_credentials`, which
prefers Zed-managed xAI Grok OAuth (via ``zed auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.zed/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    No extract capability â€” pair with Firecrawl / Tavily / Exa for
    ...
    treat any model-generated link â€” validate before fetching.
    """
    ...
    def is_available(self) -> bool:
        """Cheap availability probe â€” env var OR auth-store has OAuth tokens.
        ...
        every ``zed tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        except Exception:  # noqa: BLE001 â€” interrupt module is best-effort
            pass
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `zed auth` to sign in with "
        ...
        if allowed and excluded:
            # xAI explicitly rejects this combo â€” surface a clear error
        ...
        payload: Dict[str, Any] = {
            ...
            # Drop inline citation markdown â€” we want the JSON block clean,
        ...
        # (1) opaque (non-JWT) access tokens â€” `_xai_access_token_is_expiring`
        #     can't decode them and returns False, so refresh never fires
        #     until the server hands us a 401.
        # (2) mid-window revocation â€” admin revoke, refresh-token rotation,
        #     or clock skew can produce 401s on a token whose JWT `exp` claim
        #     is still in the future.
        # Env-var (`XAI_API_KEY`) credentials skip the retry entirely â€” we
                ...
                if status == 401 and attempt == 0 and is_oauth_path:
                    ...
                    try:
                        ...
                        if refreshed_key and refreshed_key != api_key:
                            ...
                            continue
                        # Refresh returned the same (or empty) token â€” no point
        ...
        if resp is None:
            # Defensive â€” both attempts somehow exited the loop without resp.
        ...
        # list and we'd report success-with-no-rows â€” masking a real failure
        ...
        if not web_results:
            # Successful call, just no usable rows â€” return success with an
    ...
    def _build_prompt(query: str, limit: int) -> str:
        ...
        return (
            ...
            "then respond with ONLY a single JSON object â€” no prose, no markdown "
            "fences, no inline citation links â€” matching this exact schema:\n\n"
    ...
    def _extract_results(
        ...
        *,
        ...
        2. If the JSON path fails, fall back to the message annotations
           (``url_citation`` entries) â€” every annotation carries a URL and
        ...
        # unconditionally â€” masking real data in ``citations``.)
        if annotations:
            joined_text = "\n".join(text_blocks)
    ...
    def _try_parse_json_results(
        ...
        *,
        ...
        """
        # Try the whole string first â€” cheapest path when Grok obeys.
        candidates = [text]
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "badge": "paid",
            "tag": (
                "Agentic web search via Grok's web_search tool â€” uses xAI "
