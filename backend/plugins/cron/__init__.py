...
1. Bundled providers: ``plugins/cron/<name>/`` (shipped with zed-agent)
...
This is a near-verbatim clone of ``plugins/memory/__init__.py`` â€” the same
...
_USER_NAMESPACE = "_zed_user_cron"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_zed_user_cron.<name>``, a dotted
    ...
    ``ModuleNotFoundError: No module named '_zed_user_cron'`` â€” the same
...
def discover_cron_schedulers() -> List[Tuple[str, str, bool]]:
    ...
    Returns list of (name, description, is_available) tuples. May be empty â€”
...
def load_cron_scheduler(name: str) -> Optional["CronScheduler"]:  # noqa: F821
    ...
    (``$ZED_HOME/plugins/<name>/``) directories. Bundled takes precedence
    on name collisions.
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["CronScheduler"]:  # noqa: F821
    ...
    - A top-level class that extends CronScheduler â€” we instantiate it
    """
