"""Chronos â€” NAS-mediated managed cron provider (scale-to-zero).

Chronos (the Greek god of time, alongside Zed) is the first non-default
...
class ChronosCronScheduler(CronScheduler):
    ...
    def __init__(self) -> None:
        # In-memory map of job_id â†’ fire_at we've asked NAS to arm. Best-effort
    ...
    def start(self, stop_event, *, adapters=None, loop=None, interval=60):
        ...
        Does NOT block and does NOT spawn a 60s wake (DQ-1) â€” that is the whole
        point of scale-to-zero. The machine wakes only on a NASâ†’agent fire.
        """
        try:
            self.reconcile()
        except Exception as e:
            logger.warning("Chronos start() reconcile failed: %s", e)
        # Intentionally return â€” no loop, no periodic wake.

    def stop(self) -> None:
        return None

    def on_jobs_changed(self) -> None:
        """A job was created/updated/removed/paused/resumed â€” reconcile the NAS
    ...
    def _list_armed(self) -> Dict[str, str]:
        """Observed armed one-shots: job_id â†’ fire_at.

        Prefer the in-memory map (warm process); on a cold/empty map, ask NAS
        (best-effort). If NAS list fails, return what we have â€” reconcile then
    ...
    def fire_due(self, job_id: str, *, adapters: Any = None, loop: Any = None) -> bool:
        ...
        returns None â†’ nothing to re-arm (the schedule naturally stops).
        """
...
def register(ctx) -> None:
    """Plugin entrypoint â€” register the Chronos provider with the loader.
