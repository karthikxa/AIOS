"""
Microsoft Teams platform adapter for Zed Agent.
...
# httpx is imported lazily â€” only the ``_write_summary_via_incoming_webhook``
...
class TeamsSummaryWriter:
    ...
    async def _write_summary_via_incoming_webhook(
        ...
        # Lazy import â€” see module-level note. The teams plugin loads on
...
# ``check_fn`` and by ``gateway status``) â€” it must never trigger a pip
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Maps chat_id â†’ ConversationReference captured from incoming messages.
    ...
    async def connect(self) -> bool:
        ...
        try:
            # Set up aiohttp app first â€” the bridge adapter wires SDK routes into it
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Zed"}),
            )
    ...
    async def _on_message(self, ctx: ActivityContext[MessageActivity]) -> None:
        ...
        # Extract text â€” strip bot @mentions
        text = ""
        ...
        # attachments â€” run.py's image handling keys off the per-path image/*
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        zed_action = data.get("zed_action", "")
        session_key = data.get("session_key", "")

        if not zed_action or not session_key:
        ...
        # bot silently treated every clicker as authorized â€” meaning any
        ...
        if not allow_all:
            if not allowed_csv:
                logger.warning(
                    ...
                    "and TEAMS_ALLOW_ALL_USERS not set â€” default deny"
                )
                return InvokeResponse(
                    status=200,
                    body=AdaptiveCardActionMessageResponse(
                        value="â›” Approval buttons require TEAMS_ALLOWED_USERS to be configured."
                    ),
                )
            ...
            if "*" not in allowed_ids and clicker_id not in allowed_ids:
                logger.warning("[teams] Unauthorized card action by %s â€” ignoring", clicker_id)
                return InvokeResponse(
                    status=200,
                    body=AdaptiveCardActionMessageResponse(value="â›” Not authorized."),
                )
        ...
        choice = choice_map.get(zed_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        if not has_blocking_approval(session_key):
            return InvokeResponse(
                ...
                body=AdaptiveCardActionCardResponse(
                    value=AdaptiveCard()
                    .with_version("1.4")
                    .with_body([TextBlock(text="âš ï¸ Approval already resolved or expired.", wrap=True)])
                ),
            )
        ...
        label_map = {
            "once": "âœ… Allowed (once)",
            "session": "âœ… Allowed (session)",
            "always": "âœ… Always allowed",
            "deny": "âŒ Denied",
        }
        cmd = data.get("cmd", "")
        desc = data.get("desc", "")
        body = []
        if cmd:
            body.append(TextBlock(text="âš ï¸ Command Approval Required", wrap=True, weight="Bolder"))
    ...
    async def send_exec_approval(
        ...
        # Truncated for button data payload â€” just enough to reconstruct the card body.
        btn_data_base = {
        ...
        card = (
            ...
            .with_version("1.4")
            .with_body([
                TextBlock(text="âš ï¸ Command Approval Required", wrap=True, weight="Bolder"),
                TextBlock(text=f"```\n{cmd_preview}\n```", wrap=True),
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="zed_approve",
                    data={**btn_data_base, "zed_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="zed_approve",
                    data={**btn_data_base, "zed_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="zed_approve",
                    data={**btn_data_base, "zed_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="zed_approve",
                    data={**btn_data_base, "zed_action": "deny"},
                    style="destructive",
                ),
            ])
        )
    ...
    async def _send_media_attachment(
        ...
        try:
            ...
            if source.startswith("http://") or source.startswith("https://"):
                ...
                # Local path â€” encode as base64 data URI
...
# â”€â”€ Interactive setup â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Zed\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    if not client_id:
        print_warning("Client ID is required â€” skipping Teams setup")
        return
    ...
    if not client_secret:
        print_warning("Client secret is required â€” skipping Teams setup")
        return
    ...
    if not tenant_id:
        print_warning("Tenant ID is required â€” skipping Teams setup")
        return
    ...
    if prompt_yes_no("Restrict access to specific users? (recommended)", True):
        ...
        print_warning("âš ï¸  Open access â€” anyone who can message the bot can command it.")

    print()
    print_success("Teams configuration saved to ~/.zed/.env")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # Env-driven auto-configuration â€” seeds PlatformConfig.extra with
        ...
        platform_hint=(
            ...
            "markdown â€” bold (**text**), italic (*text*), and inline code "
