...
required â€” uses aiohttp which is already a Zed dependency.
...
# practical limit for readable messages â€” matching OpenClaw's choice).
...
_CHANNEL_TYPE_MAP = {
    "D": "dm",
    "G": "group",
    "P": "group",   # private channel â†’ treat as group
    "O": "channel",
}
...
class MattermostAdapter(BasePlatformAdapter):
        ...
        try:
            async with self._session.get(url, headers=self._headers(), timeout=aiohttp.ClientTimeout(total=30)) as resp:
                if resp.status >= 400:
                    ...
                    logger.error("MM API GET %s â†’ %s: %s", path, resp.status, body[:200])
                    return {}
        ...
        try:
            async with self._session.post(
                ...
                if resp.status >= 400:
                    ...
                    logger.error("MM API POST %s â†’ %s: %s", path, resp.status, body[:200])
                    return {}
                return await resp.json()
        except aiohttp.ClientError as exc:
    ...
    async def _post_preserving_thread(
        ...
        flat_payload["message"] = (
            "âš ï¸ Mattermost thread delivery failed; posting final reply in channel.\n\n"
            + original
        ).strip()
        logger.warning(
    ...
    async def _api_put(
        ...
        try:
            async with self._session.put(
                ...
                if resp.status >= 400:
                    body = await resp.text()
                    logger.error("MM API PUT %s â†’ %s: %s", path, resp.status, body[:200])
                    return {}
                return await resp.json()
        except aiohttp.ClientError as exc:
    ...
    async def _upload_file(
        ...
        async with self._session.post(url, headers=headers, data=form, timeout=aiohttp.ClientTimeout(total=60)) as resp:
            if resp.status >= 400:
                body = await resp.text()
                logger.error("MM file upload â†’ %s: %s", resp.status, body[:200])
                return None
    ...
    async def connect(self) -> bool:
        ...
        if not me or "id" not in me:
            logger.error("Mattermost: failed to authenticate â€” check MATTERMOST_TOKEN and MATTERMOST_URL")
    ...
    def format_message(self, content: str) -> str:
        """Mattermost uses standard Markdown â€” mostly pass through.

        Strip image markdown into plain links (files are uploaded separately).
        """
        # Convert ![alt](url) to just the URL â€” Mattermost renders
    ...
    async def _ws_loop(self) -> None:
        ...
        while not self._closing:
            try:
                ...
                # Clean disconnect â€” reset delay.
                delay = _RECONNECT_BASE_DELAY
                ...
                # succeed on retry â€” stop reconnecting instead of looping forever.
                import aiohttp
                err_str = str(exc).lower()
                if isinstance(exc, aiohttp.WSServerHandshakeError) and exc.status in {401, 403}:
                    logger.error("Mattermost WS auth failed (HTTP %d) â€” stopping reconnect", exc.status)
                    return
                if "401" in err_str or "403" in err_str or "unauthorized" in err_str:
                    logger.error("Mattermost WS permanent error: %s â€” stopping reconnect", exc)
                    return
                logger.warning("Mattermost WS error: %s â€” reconnecting in %.0fs", exc, delay)
    ...
    async def _ws_connect_and_listen(self) -> None:
        ...
        # Build WS URL: https:// â†’ wss://, http:// â†’ ws://
    ...
    async def _handle_ws_event(self, event: Dict[str, Any]) -> None:
        ...
        if channel_type_raw != "D":
            # allowed_channels check (whitelist â€” must pass before other gating).
...
async def _standalone_send(
    ...
    *,
    ...
    on the ``POST /posts`` payload â€” pass ``thread_id`` when threading is
    ...
    standalone senders but unused â€” Mattermost stores every uploaded file
...
def interactive_setup() -> None:
    ...
    print_info("   1. In Mattermost: Integrations â†’ Bot Accounts â†’ Add Bot Account")
    ...
    print_info("   To find your user ID: click your avatar â†’ Profile")
    ...
    print()
    print_info("ðŸ“¬ Home Channel: where Zed delivers cron job results and notifications.")
    print_info("   To get a channel ID: click channel name â†’ View Info â†’ copy the ID")
...
# ---------------------------------------------------------------------------
# YAML â†’ env config bridge (apply_yaml_config_fn, #25443)
# ---------------------------------------------------------------------------
...
def _apply_yaml_config(yaml_cfg: dict, mattermost_cfg: dict) -> dict | None:
    ...
    model and merely owns the YAMLâ†’env translation here, next to the
    adapter that consumes it.

    Env vars take precedence over YAML â€” every assignment is guarded
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # zed_cli/setup.py::_setup_mattermost function.
        setup_fn=interactive_setup,
        # YAMLâ†’env config bridge â€” owns the translation of
