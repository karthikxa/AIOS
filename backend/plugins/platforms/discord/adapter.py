...
class _Snowflake:
    """Minimal object exposing ``.id`` â€” satisfies discord.py's Snowflake
...
# every slash command â€” not just the overflow ones. We keep the desired set
...
_DISCORD_NONCONVERSATIONAL_HISTORY_MESSAGE_PATTERNS = (
    re.compile(r"^\s*ðŸ’¾\s*Self-improvement review:\s+\S[\s\S]*$", re.IGNORECASE),
    ...
    re.compile(
        r"^\s*ðŸ’¾\s+Skill\s+['\"].+?['\"]\s+(?:created|updated|improved|patched)\.?\s*$",
        re.IGNORECASE,
    ),
    re.compile(r"^\s*â³\s+Working\s+â€”\s+\d+\s+min(?:\s|$)", re.IGNORECASE),
    re.compile(
    ...
    re.compile(
        r"^\s*(?:âœ…|âŒ)\s+Zed update\s+"
        r"(?:finished|failed|timed out)[\s\S]*$",
        re.IGNORECASE,
    ),
    re.compile(r"^\s*â™»ï¸?\s+Gateway\s+(?:restarted successfully|online\b)[\s\S]*$", re.IGNORECASE),
)
...
class VoiceReceiver:
    ...
    def _on_packet(self, data: bytes):
        ...
        try:
            import nacl.secret  # noqa: E402 â€” delayed import, only in voice path
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.zed/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("ZED_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("ZED_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Zed gateway
        ...
        try:
            ...
            try:
                ...
                if retry_after is None:
                    # Rate-limited but no retry-after signal â€” back off for a
            ...
            logger.warning(
                "[%s] Slash command sync timed out â€” Discord rate-limit bucket "
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Zed manages."""
    ...
    async def send(
        ...
        try:
            ...
            # backfill â€” avoids a full channel.history() scan on hot paths.
            if message_ids:
    ...
    async def _send_to_forum(self, forum_channel: Any, content: str) -> SendResult:
        ...
        # module â€” no cross-module import needed.
    ...
    def _load_voice_fx_config(self) -> Dict[str, Any]:
        ...
        .env file â€” these are behavioral, not secrets).  The feature is OFF by
    ...
    async def join_voice_channel(self, channel) -> bool:
        ...
        async with self._voice_locks.setdefault(guild_id, asyncio.Lock()):
            ...
            # speech).  Best-effort â€” if it fails we fall back to the legacy
    ...
    async def play_in_voice_channel(self, guild_id: int, audio_path: str) -> bool:
        ...
        # â”€â”€ Mixer path (overlap + ducking) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ Legacy one-shot path (no mixer) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            while receiver._running:
                ...
                for user_id, pcm_data in completed:
                    if not self._is_allowed_user(
                        ...
                        continue
                    # A user speaking to the bot is activity too â€” not just the
        ...
        *,
        ...
        if direct_roles and (author_guild is None or author_guild.id == guild.id):
            if any(getattr(r, "id", None) in allowed_roles for r in direct_roles):
                return True
        # 2) Fallback: resolve the Member in the message's guild only â€” NEVER
    ...
    # â”€â”€ Slash command authorization â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # historically ran with NO authorization check â€” bypassing every gate
    ...
    # set â€” ``_is_allowed_user`` returns True and the channel checks early-out
    # â€” preserving the existing "single-tenant, all guild members trusted"
    ...
    def _evaluate_slash_authorization(
        ...
        # â”€â”€ Channel scope (mirrors on_message lines 3374-3388) â”€â”€
        # DMs aren't channel-gated â€” DMs follow on_message's DM lockdown
        ...
        # â”€â”€ User / role allowlist (mirrors on_message line 681) â”€â”€
    ...
    async def _check_slash_authorization(
        ...
        alert â€” the caller must stop on False (the interaction has already
        been responded to).
        """
        ...
        for target in (Platform.TELEGRAM, Platform.SLACK):
            try:
                ...
                if not home or not getattr(home, "chat_id", None):
                    continue
                msg = (
                    "âš ï¸ Unauthorized Discord slash attempt\n"
    ...
    async def send_typing(self, chat_id: str, metadata=None) -> None:
        ...
        default), and continues â€” it does NOT die on a single rate-limit
            ...
            try:
                while True:
                    try:
                        ...
                        # Don't die on 429 â€” backoff and continue
                        retry_after = self._extract_discord_retry_after(e)
    ...
    async def _run_simple_slash(
        ...
        # Auth gate â€” must run before defer() so an ephemeral rejection can
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Zed session")
        ...
        @tree.command(name="status", description="Show Zed session status")
        ...
        @tree.command(name="stop", description="Stop the running Zed agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.zed/skills/ for new or removed skills")
        ...
        @discord.app_commands.choices(mode=[
            ...
            # gateway/run.py â€” expose both in the slash UI so autocomplete
            # matches what the docs advertise and what the runner accepts when
            # the command is typed as plain text.
            discord.app_commands.Choice(name="join â€” join your voice channel", value="join"),
            discord.app_commands.Choice(name="channel â€” join your voice channel (alias)", value="channel"),
        ...
        @tree.command(name="update", description="Update Zed Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Zed gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Zed session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Zed in the thread",
        ...
        # zed_cli/commands.py automatically appear as Discord slash
    ...
    def _apply_owner_only_visibility(self, tree) -> None:
        ...
        can re-grant per user/role via Server Settings â†’ Integrations â†’
        <bot> â†’ Permissions.
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            async def _autocomplete_name(
                ...
                empty list here is intentional â€” sending a per-keystroke
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        ``/reload-skills``. No Discord API calls are made â€” autocomplete
    ...
    def refresh_skill_group(self) -> tuple[int, int]:
        ...
        at process start â€” new skills stay invisible and deleted skills
        ...
        callbacks â€” no ``tree.sync()`` is required.
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Zed session in the thread
    ...
    def _discord_thread_require_mention(self) -> bool:
        ...
        mentioned again â€” useful for one-on-one conversations.
    ...
    def _discord_history_backfill_limit(self) -> int:
        ...
        In practice the scan usually stops much earlier â€” at the bot's own
    ...
    async def _fetch_channel_context(
        ...
        agent sees the conversation surrounding what the user pointed at â€”
        ...
        # trigger â€” Discord snowflake IDs are monotonically increasing, so
        ...
        try:
            ...
            pass  # Malformed cache entry â€” fall back to cold-start scan
        ...
        try:
            def _keep(msg) -> Optional[str]:
                ...
                identical rules.  Does NOT enforce the self-message partition â€”
                ...
                # context, "mentions" is treated as "all" â€” we are deciding
            ...
            # â”€â”€ Primary window: recent channel activity since the last bot turn â”€â”€
            ...
            async for msg in channel.history(
                ...
                # skipped BEFORE the partition check â€” otherwise a delayed
                ...
                if (
                    ...
                    continue
                # Stop at our own (conversational) message â€” this is the
            ...
            # â”€â”€ Reply window: context around the message the user pointed at â”€â”€
            ...
            # NOT partitioned on the self-message boundary â€” the whole point is
            ...
            if reply_target is not None and reply_target_id and reply_target_id not in seen_ids:
                ...
                # window modest â€” it's anchored context, not a full backfill.
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Zed: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "Zed"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by Zed: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Zed handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_clarify(
        ...
        plus a final "âœï¸ Other (type answer)" button. Picking "Other" flips
        ...
        plain embed text â€” no buttons. The gateway's text-intercept captures
        ...
        try:
            ...
            def _flatten_choice(c):
                ...
                if isinstance(c, dict):
                    ...
                    # rather than picking some random field â€” a garbage
            ...
            if clean_choices:
                embed.add_field(
                    name="Choices",
                    value="Pick one below, or click âœï¸ Other to type a custom answer.",
                    inline=False,
                )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``zed update --gateway``
    ...
    async def _handle_message(self, message: DiscordMessage, role_authorized: bool = False) -> None:
        ...
        if not isinstance(message.channel, discord.DMChannel):
            ...
            # â€” UNLESS thread_require_mention is enabled, in which case threads
        ...
        if normalized_content.startswith("/"):
            ...
            for att in all_attachments:
                if att.content_type:
                    ...
                    # No content_type at all (rare â€” discord usually fills it
        ...
        # â”€â”€ History backfill â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # because every DM message triggers the bot â€” there's no mention gap
        ...
        # and response) are not captured â€” this is an accepted simplification
        ...
        if not _is_dm and self._discord_history_backfill():
            ...
            #     no mention gap exists â€” otherwise replies get only the short
            ...
            # Auto-threaded messages also skip â€” we just created the thread,
            ...
            # rather than isinstance(discord.Message) â€” under test doubles the
        ...
        # â€” the context IS the message, so skip the placeholder.
...
def _define_discord_view_classes() -> None:
            ...
            if msg:
                try:
                    ...
                    if embed:
                        ...
                        embed.set_footer(text="â± Prompt expired â€” no action taken")
    ...
    class SlashConfirmView(discord.ui.View):
        ...
        gateway's three choices:
          ...
          * "Approve Once"   â†’ ``choice="once"``
          * "Always Approve" â†’ ``choice="always"``
          * "Cancel"         â†’ ``choice="cancel"``
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``zed update`` prompts.
    ...
    class ModelPickerView(discord.ui.View):
        ...
        async def _on_model_selected(self, interaction: discord.Interaction):
            ...
            if warning is not None:
                ...
                await interaction.response.edit_message(
                    embed=discord.Embed(
                        title="âš  Expensive Model Warning",
        ...
        async def on_timeout(self):
            ...
            if msg:
                try:
                    embed = discord.Embed(
                        title="âš™ Model Configuration",
                        description="â± Selection expired â€” no model change.",
                        color=discord.Color.greyple(),
                    )
                    await msg.edit(embed=embed, view=self)
                except Exception:
    ...
    class ClarifyChoiceView(discord.ui.View):
        ...
        Renders one button per choice (max 24) plus a final ``âœï¸ Other`` button.
        ...
        Auth gating mirrors ``ExecApprovalView`` â€” only users/roles in the
        ...
        def __init__(
            ...
            for index, choice in enumerate(self.choices):
                ...
                if len(choice) <= budget:
                    ...
                    if space >= budget // 2:
                        cut_at = space
                    # 2. Soft boundary â€” only if no word boundary found.
        ...
        async def _resolve_choice(
            ...
            # Resolve via the gateway clarify primitive â€” same mechanism as
        ...
        async def _on_other(self, interaction: "discord.Interaction") -> None:
            ...
            # Don't pop the entry â€” the gateway's text-intercept needs it
            ...
            if embed:
                ...
                embed.set_footer(
                    text=f"Awaiting typed response from {display_name}â€¦",
                )
...
# â”€â”€ Standalone (out-of-process) sender â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Used by ``tools/send_message_tool._send_via_adapter`` when the gateway runner
# is not in this process (e.g. ``zed cron`` running standalone) and no live
...
# logic lives next to the adapter â€” same shape as Teams' ``_standalone_send``.
...
def _standalone_sanitize_error(text) -> str:
    """Local copy of tools.send_message_tool._sanitize_error_text â€” strips bot
...
async def _standalone_send(
    ...
    *,
    ...
    Forum channels (type 15) reject ``POST /messages`` â€” a thread post is
    ...
    ``force_document`` is accepted for signature parity but unused â€” Discord
    ...
    try:
        ...
        if thread_id:
            ...
            # Forum channels reject POST /messages â€” create a thread post instead.
            # Three-layer detection: directory cache â†’ process-local probe
            # cache â†’ GET /channels/{id} probe (with result memoized).
            _channel_type = None
            try:
            ...
            if is_forum:
                ...
                async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=60), **_sess_kw) as session:
                    if valid_media:
                        ...
                        # No media â€” simple JSON POST creates the thread with
...
def interactive_setup() -> None:
    ...
    print_info("   To get a channel ID: right-click a channel â†’ Copy Channel ID")
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    env-driven model and merely owns the YAMLâ†’env translation here, next to
    the adapter that consumes it.

    Env vars take precedence over YAML â€” every assignment is guarded by
    ...
    # YAML 1.1 parses bare 'off' as boolean False â€” coerce to string "off".
...
def register(ctx) -> None:
    """Plugin entry point â€” called by the Zed plugin system."""
    ctx.register_platform(
        ...
        install_hint="pip install 'zed-agent[messaging]'",
        # Interactive setup wizard â€” replaces the central
        # zed_cli/setup.py::_setup_discord function.  Same shape as Teams.
        setup_fn=interactive_setup,
        # YAMLâ†’env config bridge â€” owns the translation of ``config.yaml``
