...
``add_done_callback`` (never ``.result()`` â€” that would block the callback
...
# ``model_tools`` import time, so ``zed status``, ``zed chat``, etc.
...
# ``try`` block executes (which it does â€” ``__init__`` calls it), the
...
def _load_google_modules() -> bool:
    ...
    googleapiclient, grpc, and friends â€” about 33MB RSS and 110ms wall
...
# the in-tree â†’ plugin migration. ``__name__`` resolves to
# ``zed_plugins.platforms__google_chat.adapter`` once the plugin
...
# SA scopes â€” chat.bot is sufficient for the bot's own messaging operations
# (messages.create / patch / delete, spaces metadata, memberships,
# media.download for inbound user attachments). The bot CANNOT call
# media.upload â€” Google requires user OAuth for that endpoint, no scope
# adjustment changes it.
#
# Native attachment delivery (bot â†’ user) is handled via a separate user-
...
# and 429 occasionally â€” without a retry wrapper, single hiccups drop
...
def _is_retryable_error(exc: BaseException) -> bool:
    ...
    and well-formed non-retryable failures are NOT retried â€” those
...
#   * ``send_typing`` checks for any value before posting â€” sentinel keeps
#     ``_keep_typing`` (running on the base-class timer) from creating a
#     fresh "Zed is thinkingâ€¦" card during the small window between
#     ``send()`` finishing and the base-class cancelling its typing_task.
#   * ``stop_typing`` checks for the sentinel and skips the API delete â€”
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a zed MessageType.
...
class _ThreadCountStore:
    ...
    - prev_count == 0 (first time we see this thread) â†’ "main flow":
    ...
    - prev_count >= 1 (we've already seen this thread) â†’ "side thread":
    ...
    (the bug RamÃ³n reported across 4 iterations of the in-memory
    version).
    ...
    def load(self) -> None:
        ...
        Missing file â†’ empty store. Corrupt JSON â†’ empty store + warn.
        """
        self._loaded = True
        ...
        try:
            ...
            self._counts = {}
            return
        # Validate shape â€” anything off-schema gets dropped silently.
    ...
    def incr(self, chat_id: str, thread_name: str) -> int:
        ...
        PRE-increment value (the heuristic input â€” "have we seen this
    ...
    def _save(self) -> None:
        ...
        Failure is non-fatal â€” log warning and continue. The in-memory
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        # ``Platform("google_chat")`` resolves via ``_missing_()`` â†’ pseudo-member
        # cached in ``_value2member_map_``.  We deliberately do NOT add an enum
        # attribute to ``gateway.config.Platform`` â€” bundled platform plugins
        ...
        # ~110ms / ~33MB on every CLI invocation. Idempotent â€” pays the cost
        ...
        # but now hold the LEGACY single-user token (if any) â€” used as a
        ...
        # chat_id â†’ most-recent inbound sender's email. Populated in
        ...
        #       stays stable across top-level messages â€” see
        ...
        # Persisted to ${ZED_HOME}/google_chat_thread_counts.json so
        ...
        self._thread_count_store = _ThreadCountStore(
            _zed_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Zed is thinkingâ€¦".
    ...
    def _load_sa_credentials(self) -> Any:
        ...
        Priority:
          ...
          3. Application Default Credentials via ``google.auth.default()``
             â€” works on Cloud Run / GCE / GKE with a workload identity
        ...
        # No explicit SA configured â€” try ADC. This is the Cloud Run / GCE
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ZED_HOME", str(_Path.home() / ".zed"))
    ...
    async def _resolve_bot_user_id(self) -> Optional[str]:
        ...
        precise â€” own messages and other bots look alike).
        """
    ...
    async def connect(self) -> bool:
        ...
        # First call into the heavy google-cloud stack â€” trigger the lazy
        # import. ``_load_google_modules()`` is idempotent and rebinds the
        # module globals (``pubsub_v1``, ``service_account``, ``HttpError``,
        # â€¦) used throughout this file. Anything that runs *before* this
        ...
        try:
            ...
            if user_creds is not None:
                ...
                logger.info(
                    "[GoogleChat] Legacy user OAuth loaded â€” fallback "
            ...
            if authorized:
                ...
                logger.info(
                    "[GoogleChat] No user OAuth tokens at setup â€” file "
    ...
    def _extract_message_payload(
        ...
        Format 1 â€” Workspace Add-ons (canonical, ce-type-driven)::

            {"chat": {"messagePayload": {"message": {...}, "space": {...}}}}

        Format 2 â€” Native Chat API Pub/Sub (alternative configuration where
        the Chat app publishes events directly without the Workspace
        Add-ons wrapper)::

            {"type": "MESSAGE", "message": {...}, "space": {...}}

        Format 3 â€” Relay / flat (a custom Cloud Run relay that flattens the
        ...
        dict so downstream code (``_dispatch_message`` â†’
    ...
    def _on_pubsub_message(self, message: Any) -> None:
        """Pub/Sub callback â€” parse envelope and dispatch to asyncio loop.
        ...
        try:
            ...
            if "membership" in ce_type or "MEMBERSHIP" in ce_type:
                ...
                if "created" in ce_type:
                    # ADDED_TO_SPACE for this bot â€” resolve self user_id.
            ...
            # Dedup guard â€” Pub/Sub is at-least-once.
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        sees it â€” that's a bot-local OAuth setup flow, not a prompt.
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # key â€” the bot stores this user's token at
                # ${ZED_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _handle_setup_files_command(
        ...
        Subcommands:
          /setup-files                  â†’ show status + next step
          /setup-files start            â†’ print OAuth URL
          /setup-files revoke           â†’ revoke and delete stored token
          /setup-files <CODE_OR_URL>    â†’ exchange auth code for token
        ...
        if not arg:
            ...
            if creds is not None:
                ...
                await _reply(
                    "âœ… Native attachment delivery is **active** for "
                    f"`{who}`.\n"
                    f"Token: `{token_path}`\n"
            ...
            if not client_secret_present:
                await _reply(
                    "ðŸ”§ Native attachment delivery is **not configured**.\n"
                    "**Step 1 (one-time, on the host):** create OAuth client "
                    "credentials at "
                    "https://console.cloud.google.com/apis/credentials â†’ "
                ...
                return True
            await _reply(
                "ðŸ”§ Client credentials are stored but you haven't "
        ...
        if arg == "start":
            if not oauth_helper._client_secret_path().exists():
                await _reply(
                    "âš ï¸ No client credentials stored for this profile. Send "
            ...
            try:
                ...
                await _reply(
                    "âŒ Couldn't generate the OAuth URL. Check the gateway "
            ...
            await _reply(
                ...
                "Tip: the URL contains your access grant â€” keep it private."
            )
            return True
        ...
        try:
            ...
            await _reply(
                "âŒ Token exchange failed. The code may have expired or "
        ...
        try:
            ...
            if new_creds is not None:
                ...
                await _reply(
                    "âœ… Authorized! Native attachment delivery is now "
        ...
        await _reply(
            "âš ï¸ Token exchanged but the gateway couldn't load the new "
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a zed MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
        ...
        # - prev_count == 0  â†’ first message in this thread. Google Chat
        ...
        #   reply ALSO must NOT thread with the user message â€” if we
        ...
        # - prev_count >= 1  â†’ user explicitly engaged a thread that
        ...
        source = self.build_source(
            ...
            # resource name when sender has no email (rare â€” bot-to-bot
    ...
    async def _download_attachment(
        ...
        Priority for bot Service Accounts:
          ...
          1. ``attachmentDataRef.resourceName`` via ``chat.media.download`` â€”
          ...
          3. Direct HTTP fetch of ``downloadUri`` only as a last resort â€”
        ...
        # can use â€” otherwise try the bot path first.
    ...
    async def send(
        ...
        ``messages.patch`` â€” NO delete+create. Google Chat shows a tombstone
        ...
        try:
            ...
            # and strip invisible Unicode that renders as tofu (â–¡). Runs
            ...
            # Treat any earlier sentinel as "no real card to patch" â€” defensive.
    ...
    async def edit_message(
        ...
        *,
        ...
        tool activity (no "ðŸ” web_searchâ€¦", no progressive token edits).

        ``message_id`` is the Google Chat resource name
        ``spaces/X/messages/Y``. ``finalize`` is unused here â€” Google
        ...
        if len(content) > _MAX_TEXT_LENGTH:
            content = content[: _MAX_TEXT_LENGTH - 1] + "â€¦"
        try:
    ...
    async def delete_message(self, chat_id: str, message_id: str) -> bool:
        """Delete a message â€” used sparingly (deletion creates a tombstone).
    ...
    # Outbound formatting
    # ------------------------------------------------------------------
    # Invisible Unicode codepoints that render as tofu (â–¡) in Google
    ...
    _INVISIBLE_RE = re.compile(
        ...
        "â€"          # Zero-Width Joiner (ZWJ)
        "â€Žâ€"    # LTR / RTL marks
        "â "          # Word Joiner
        "ï»¿"          # BOM / Zero-Width No-Break Space
        "ï¸€-ï¸"   # Variation Selectors 1-16 (VS1â€“VS16)
    ...
    def format_message(cls, content: str) -> str:
        ...
        # Headers (## Title) â†’ *Title* (Chat has no header support).
        text = re.sub(
            r"^#{1,6}\s+(.+)$",
        ...
        # Bold: **text** â†’ *text* (Chat uses single asterisks).
        text = re.sub(
            r"\*\*(.+?)\*\*",
        ...
        # Markdown links [text](url) â†’ <url|text> (Slack-style angle-bracket).
        text = re.sub(
            r"\[([^\]]+)\]\(([^)]+)\)",
    ...
    def _resolve_thread_id(
        ...
        Priority:
          1. ``metadata['thread_id']`` â€” populated by the gateway's session
             plumbing from ``SessionSource.thread_id`` (the inbound
             ``thread.name``). Canonical path for groups.
          2. ``metadata['thread_name']`` / ``metadata['thread_ts']`` â€” Slack
          ...
          4. ``self._last_inbound_thread[chat_id]`` â€” Google Chat DMs spawn
    ...
    async def _call_with_retry(
        ...
        *,
        ...
        attempt â€” see :func:`_is_retryable_error`. Cancellation propagates
        ...
        # Defensive â€” the loop above always either returns or re-raises.
    ...
    async def _create_message(
        ...
        ``messageReplyOption=REPLY_MESSAGE_FALLBACK_TO_NEW_THREAD`` â€”
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Zed is thinkingâ€¦' marker message.
        ...
        IMPORTANT â€” must place the typing card in the user's thread:
        ...
        top-level card in place â€” leaving the bot's whole response
        stranded outside the user's thread. We resolve the thread the
        same way send() does.

        IMPORTANT â€” cancellation safety:
        ...
        ``send_typing`` mid-flight â€” but the underlying ``asyncio.to_thread``
        ...
        "Zed is thinkingâ€¦" stuck in chat forever, plus one card that
        ...
        Cancellation of THIS coroutine no longer cancels the create â€”
        the task runs to completion and the msg_id lands in the slot
        regardless.
        """
        # Already have a card (real msg_id, sentinel, or in-flight) â€” bail.
        ...
        if chat_id in self._typing_card_inflight:
            ...
            # is up by the time we return". Bounded wait â€” if the
        ...
        body: Dict[str, Any] = {"text": "Zed is thinkingâ€¦"}
        if thread_id:
            body["thread"] = {"name": thread_id}
            ...
            try:
                ...
                if result.success and result.message_id:
                    ...
                    if chat_id not in self._typing_messages:
                        ...
                        # Slot already populated â€” likely send() patched
        ...
        # but the task continues in the background â€” so the msg_id
    ...
    async def stop_typing(self, chat_id: str) -> None:
        """Stop the typing indicator â€” NO-OP when a live card is tracked.

        Google Chat has no separate typing API: the "Zed is thinkingâ€¦"
        ...
        ``stop_typing`` at three moments per turn â€” typically BEFORE
        ...
        Three cases:
          * Slot empty â†’ nothing to do.
          * Slot holds SENTINEL â†’ ``send()`` already patched the card;
            pop the sentinel so the next turn starts clean.
          * Slot holds a real ``message_name`` â†’ leave it for ``send()``
            to consume. NO-OP.

        Stranded cards on error / cancellation paths (where ``send()``
        never runs) are reaped by ``on_processing_complete`` â€” see that
        ...
        if current == _TYPING_CONSUMED_SENTINEL:
            ...
            return
        # Real message_name â€” leave it for send() to patch. Deliberate no-op.
        return
    ...
    async def on_processing_complete(
        ...
        patched it), the slot holds the SENTINEL â€” pop and exit.
        ...
        see "Zed is thinkingâ€¦" stuck forever.
        """
        ...
        try:
            ...
            if current and current != _TYPING_CONSUMED_SENTINEL:
                # Real message_name still in slot â€” send() never ran. Patch
    ...
    async def _consume_typing_card_with_text(
        ...
        try:
            ...
            status = getattr(getattr(exc, "resp", None), "status", None)
            if status == 404:
                # Card disappeared â€” caller should create a new message.
                return None
            raise
    ...
    async def send_image(
        ...
        the image (caption + URL) â€” same anti-tombstone pattern used by
    ...
    def _is_app_auth_attachment_error(exc: HttpError) -> bool:
        ...
        still see). Used to flag a misuse â€” calling ``media.upload``
    ...
    async def _acquire_user_chat_api(
        ...
        Lookup order:
          1. Per-user token for ``sender_email`` â€” the asker's identity.
          2. Legacy single-user fallback (``self._user_chat_api``) for
             pre-multi-user installs.
          3. None â€” caller posts the setup-instructions text notice.
        ...
        if self._user_chat_api is not None:
            ...
            if refreshed is None:
                logger.warning(
                    "[GoogleChat] legacy user-OAuth refresh returned None â€” "
    ...
    def _invalidate_user_creds(self, identity: Optional[str]) -> None:
        ...
        ``identity`` comes from ``_acquire_user_chat_api`` â€” either the
    ...
    async def _send_file(
        ...
        BOTH calls go through a user-authed Chat API client â€” the
        ...
        # No user OAuth â†’ can't upload natively. Surface clear setup
        ...
        try:
            ...
            if status in {401, 403}:
                logger.warning(
                    ...
                    "(token revoked or scope missing) â€” falling back to "
        ...
        # thread.name in body to actually be honored â€” see
        ...
        try:
            ...
            # comment for why â€” same reasoning applies to the
    ...
    async def _post_attachment_fallback(
        ...
        lines.extend([
            f"âš ï¸ No he podido adjuntar **{filename}**.",
            "Google Chat sÃ³lo permite adjuntar archivos cuando el bot tiene "
            "permiso explÃ­cito tuyo (OAuth de usuario). Es un consentimiento "
            "Ãºnico que se hace desde este chat.",
            "**Para activarlo:** envÃ­a `/setup-files` y sigue las instrucciones.",
            f"Mientras tanto el archivo estÃ¡ en el host: `{path}`",
        ])
        ...
        return SendResult(
            success=False,
            error="google_chat: native attachment requires user OAuth â€” "
...
def _check_for_registry() -> bool:
    """``check_fn`` for the platform registry pass â€” stricter than the
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    core hook â€” it becomes a proper ``HomeChannel`` dataclass on the
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``zed setup``.

    The setup wizard at ``zed_cli/gateway.py`` calls this for plugin
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. zed-chat-events) and a Pull subscription.")
    ...
    print_info("  7. Google Chat API console â†’ Configuration: connection = Cloud Pub/Sub,")
    ...
    if not project:
        print_warning("Project ID is required â€” skipping Google Chat setup")
        return
    ...
    if not subscription:
        print_warning("Subscription is required â€” skipping Google Chat setup")
        return
    ...
    if prompt_yes_no("Restrict access to specific users? (recommended)", True):
        ...
        print_warning("âš ï¸  Open access â€” anyone who can DM the bot can command it.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.zed/.env")
    print_info("Restart the gateway: zed gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``zed cron`` running as a
    separate process from ``zed gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point â€” called by the Zed plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'zed-agent[google_chat]'",
        setup_fn=interactive_setup,
        # Env-driven auto-configuration â€” the core env-populator hook calls
        ...
        # the "Zed is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "once in their own DM â€” until they do, file requests fall back "
            "to a text notice with the host path. Do NOT generate interactive "
            "Card v2 buttons â€” Google Chat interactivity is not yet supported "
            "by this gateway; ask for typed confirmations instead. While you "
            "are generating a response, a 'Zed is thinkingâ€¦' marker message "
            "appears in the space and is deleted once your response is ready. "
            "You do NOT have access to Google Chat-specific APIs â€” you cannot "
