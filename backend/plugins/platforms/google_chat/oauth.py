...
For the bot to deliver native file attachments â€” the same drag-and-drop
file widget the user gets when they upload manually â€” each user must
...
- Per-user tokens (keyed by sender email):
    ``${ZED_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${ZED_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start â†’ exchange:
    ``${ZED_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${ZED_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped â€” each profile registers its own):
    ``${ZED_HOME}/google_chat_user_client_secret.json``
"""
...
# after the in-tree â†’ plugin migration. See adapter.py for context.
...
def _zed_home() -> Path:
    """Resolve ZED_HOME at call time (NOT module import).

    Tests and ``ZED_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.zed/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _zed_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _zed_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _zed_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _zed_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _zed_home() / "google_chat_user_client_secret.json"
...
# do NOT request drive.file or other scopes â€” least privilege.
SCOPES: List[str] = [
...
# =============================================================================
# Library API â€” called from the adapter at runtime
# =============================================================================
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    Does NOT raise on the no-token case â€” that's expected.
    """
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install zed-agent[google_chat]."
        )
        return None

    try:
        # Don't pass scopes â€” user may have authorized only a subset, and
...
def refresh_or_none(creds: Any, email: Optional[str] = None) -> Optional[Any]:
    ...
    token is current. Returns ``None`` if refresh fails â€” caller falls
...
def list_authorized_emails() -> List[str]:
    ...
    the ``+suffix`` part of plus-addressed emails â€” accept that and use
...
# =============================================================================
# CLI commands â€” driven by the agent via /setup-files
# =============================================================================
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'zed-agent[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into ZED_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    try:
        # Accept partial scopes â€” user may deselect items in the consent screen.
    ...
    rel_label = (
        f"{display_zed_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_zed_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Zed (native attachment delivery)"
    )
