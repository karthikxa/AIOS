"""SimpleX Chat platform adapter (Zed plugin).
...
This adapter ships as a Zed platform plugin under
``plugins/platforms/simplex/``. The Zed plugin loader scans the
directory at startup, calls ``register(ctx)``, and the platform
becomes available to ``gateway/run.py`` and ``tools/send_message_tool``
through the registry â€” no edits to core files are required.
...
Optional environment variables:
    ...
    ZED_SIMPLEX_TEXT_BATCH_DELAY
                               Quiet-period seconds (default: 0.8) used to
                               concatenate rapid-fire inbound text messages
                               into a single MessageEvent â€” same pattern as
                               Telegram's text batching.

The ``websockets`` Python package is imported lazily â€” the plugin is
discoverable and ``zed setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Zed â€” no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        # Contact-request auto-accept (on by default â€” matches the way most
        ...
        # are ignored entirely (safer default â€” a bot in a group otherwise
        ...
        # Text message batching â€” concatenate rapid-fire messages into one
        # event before dispatching, mirroring Telegram's batching.
        self._text_batch_delay = float(
            os.getenv("ZED_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
    ...
    async def connect(self) -> bool:
        ...
        # Quick connectivity check â€” try to open and immediately close
        try:
    ...
    async def _handle_event(self, event: dict) -> None:
        ...
        if resp_type == "rcvFileDescrReady":
            ...
            if file_id is not None:
                logger.debug(
                    "SimpleX: rcvFileDescrReady for fileId=%s â€” sending /freceive",
                    file_id,
                )
                await self._send_fire_and_forget(f"/freceive {file_id}")
            return

        # New messages â€” simplex-chat sends "newChatItems" with an array
        ...
        # Singular variant â€” some daemon versions emit this
        ...
        # File transfer completion â€” deliver any deferred chat item
    ...
    def _make_corr_id(self) -> str:
        ...
        ever invoking ``_handle_chat_item``. The set is bounded â€” when
    ...
    async def _send_ws(self, payload: dict) -> None:
        ...
        caller never has to handle reconnection â€” the ``_ws_listener``
    ...
    async def _send_command(
        ...
        try:
            ...
            logger.warning("SimpleX: command failed: %s â€” %s", command[:50], e)
            self._pending_responses.pop(corr_id, None)
            return None
    ...
    async def send_typing(self, chat_id: str, metadata=None) -> None:
        """SimpleX has no typing-indicator API â€” no-op."""
...
def _env_enablement() -> Optional[dict]:
    ...
    The special ``home_channel`` key is handled by the core hook â€” it
...
async def _standalone_send(
    ...
    *,
    ...
    accepted but only the text body is delivered â€” SimpleX file transfers
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``zed setup gateway`` â†’ SimpleX.
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        max_message_length=MAX_MESSAGE_LENGTH,
        emoji="ðŸ”’",
        # SimpleX uses opaque contact IDs only â€” no phone numbers or email
        # addresses to redact.
        pii_safe=True,
