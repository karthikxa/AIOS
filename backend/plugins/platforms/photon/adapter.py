"""
Photon Spectrum (iMessage) platform adapter for Zed Agent.

Both directions of traffic flow through a small supervised Node sidecar
(see ``sidecar/index.mjs``) that runs the ``spectrum-ts`` SDK â€” the SDK is
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Zed dep
...
# Dedup parameters â€” the gRPC stream is at-least-once, and a sidecar
...
# messages are ignored unless they match one of these patterns â€” same
...
# ---------------------------------------------------------------------------
# Module-level helpers â€” also used by check_fn / standalone send
...
def check_requirements() -> bool:
    ...
    if not (_SIDECAR_DIR / "node_modules").exists():
        # spectrum-ts not installed yet â€” `zed photon setup` will
        # install it.  check_fn still returns False so the gateway
        # surfaces the missing-deps state in `zed setup` / status.
        return False
    return True
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # ``project_id`` here is the project's spectrumProjectId â€” the value
        ...
        # these â€” a tapback on a humanâ†”human message is not addressed to us.
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Zed defaults).
    ...
    async def connect(self) -> bool:
        ...
        if self._autostart_sidecar:
            ...
            logger.warning(
                "[photon] sidecar autostart disabled â€” inbound + outbound will fail"
            )
        ...
        logger.info(
            "[photon] connected â€” sidecar on %s:%d, streaming inbound over gRPC",
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Zed-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _is_duplicate(self, msg_id: str) -> bool:
        ...
        # the dict without limit â€” not just the expired-only prune.
        if msg_id in seen:
    ...
    async def _dispatch_inbound(self, event: Dict[str, Any]) -> None:
        ...
        # iMessage spaces carry their type directly â€” no id string-sniffing.
        ...
        if ctype == "reaction":
            # Route only tapbacks on messages WE sent â€” those are implicitly
            # addressed to the bot (feishu precedent: synthetic text event).
            # Reactions on humanâ†”human messages are not for us. Checked before
            ...
            return
        # Anything past here is a real (reactable) message â€” remember it as
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Zed checkout's sidecar entry point.
        return "photon/sidecar/index.mjs" in out.stdout

    @staticmethod
    def _pid_alive(pid: int) -> bool:
        try:
            os.kill(pid, 0)  # windows-footgun: ok â€” only called from _reap_stale_sidecar which win32-guards early
            return True
        except OSError:
            return False
    ...
    async def _reap_stale_sidecar(self) -> None:
        ...
        know, so it can't be told to ``/shutdown`` â€” and every replacement
        ...
        try:
            ...
            return  # nothing listening â€” the normal case
        ...
        if not stale:
            raise RuntimeError(
                ...
                f"(pids: {foreign or 'unknown'}, not a Photon sidecar) â€” "
        ...
        for pid in stale:
            if self._pid_alive(pid):
                try:
                    os.kill(pid, signal.SIGKILL)  # windows-footgun: ok â€” unreachable on win32 (early return above)
                except OSError:
                    pass
        ...
        if foreign:
            raise RuntimeError(
                ...
                f"processes (pids: {foreign}) â€” free it or set "
    ...
    async def _start_sidecar(self) -> None:
        if not (_SIDECAR_DIR / "node_modules").exists():
            raise RuntimeError(
                ...
                f"cd {_SIDECAR_DIR} && npm install   (or `zed photon setup`)"
            )
        ...
        # gateway death of ANY kind â€” including SIGKILL, where disconnect()
        # never runs â€” can't leave it orphaned on the port.
        ...
        # Wait for /healthz to come up â€” give it up to 15s on cold start.
    ...
    async def send_image(
        ...
        try:
            ...
            # Couldn't fetch the URL â€” fall back to sending it as text.
    ...
    # -- Reactions (tapbacks) -----------------------------------------------
    #
    # Same lifecycle-hook pattern as Telegram/Discord: ðŸ‘€ while processing,
    # swapped for ðŸ‘/ðŸ‘Ž on completion. Opt-in via PHOTON_REACTIONS â€” iMessage
    ...
    # A DM space is addressable two ways â€” the chat GUID (`any;-;+1555...`)
    ...
    async def add_reaction(
        ...
        maps â¤ï¸ðŸ‘ðŸ‘ŽðŸ˜‚â€¼ï¸â“ to native tapbacks; anything else uses Apple's
        ...
        if not target:
            return {
                "success": False,
                "error": "no message to react to â€” pass message_id (no "
    ...
    async def remove_reaction(
        ...
        if not target:
            return {
                "success": False,
                "error": "no message to unreact â€” pass message_id",
            }
    ...
    async def on_processing_start(self, event: MessageEvent) -> None:
        """Tapback ðŸ‘€ on the triggering message while the agent works."""
    ...
    async def on_processing_complete(
        ...
        """Swap the ðŸ‘€ progress tapback for a ðŸ‘/ðŸ‘Ž result.
    ...
    def format_message(self, content: str) -> str:
        # Markdown is passed through verbatim â€” the sidecar sends it with the
    ...
    async def _send_with_retry(
        ...
        text under ``PHOTON_MARKDOWN=false`` â€” either way the gateway's
    ...
    async def _sidecar_call(self, path: str, body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # _http_client directly â€” it always runs on the gateway's loop.
...
# MIME â†’ file-extension maps for caching inbound attachment bytes. These mirror
    ...
    try:
        if mime.startswith("image/"):
            ...
            try:
                ...
                # Bytes don't look like a supported image (e.g. HEIC magic) â€”
        ...
        # Video, application/*, and everything else â†’ document cache.
        return cache_document_from_bytes(raw, name)
    except Exception as exc:
...
async def _standalone_send(
    ...
    message: str,
    *,
    thread_id: Optional[str] = None,  # noqa: ARG001 â€” Spectrum has no threads yet
    media_files: Optional[list] = None,
    force_document: bool = False,  # noqa: ARG001 â€” iMessage auto-detects file kind
) -> Dict[str, Any]:
    if not HTTPX_AVAILABLE:
    ...
    headers = {"X-Zed-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Zed plugin loader at startup."""
    # Local import to avoid argparse work at module load; reused for both the
    # gateway-setup hook and the `zed photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: zed photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `zed gateway setup` alongside every other
        # channel â€” same unified onboarding wizard, no Photon-only detour.
        ...
        max_message_length=_MAX_MESSAGE_LENGTH,
        emoji="ðŸ“±",
        # iMessage carries E.164 phone numbers â€” treat session descriptions
        ...
        platform_hint=(
            ...
            "Treat replies like regular text messages â€” short and friendly. "
    ...
    # Register CLI subcommands â€” `zed photon ...`
