...
Small Node helper that bridges Zed Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Zed is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Zed-Sidecar-Token`)
...
The Zed plugin's `zed photon setup` command runs `npm install`
here automatically.
