...
This plugin connects Zed Agent to iMessage (and other Spectrum
interfaces) through [Photon][photon] â€” a managed service that handles
...
Like Discord and Slack, Photon is a **persistent-connection** channel â€” no
...
TypeScript-only, Zed runs it inside a small supervised Node sidecar and
talks to it over loopback.

```
                         gRPC (spectrum-ts)
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â” â—„â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â–º â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  Photon Spectrum cloud  â”‚   app.messages    â”‚  Node sidecar        â”‚
â”‚  (iMessage line owner)  â”‚   space.send()    â”‚  (plugins/â€¦/sidecar) â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜                   â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                                       GET /inbound (NDJSON) â”‚  â–² POST /send
                                       inbound events        â–¼  â”‚ /typing
                                              â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
                                              â”‚  PhotonAdapter        â”‚
                                              â”‚  (Python, in gateway) â”‚
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Zed-Sidecar-Token`.
...
`zed photon setup` does, in order:

1. **Device login** (RFC 8628, `client_id=photon-cli`) â€” opens
   `https://app.photon.codes/` for approval and stores the bearer token.
2. **Find or create** the `Zed Agent` project on the Photon dashboard.
3. **Provision the project secret** â€” mint a fresh project secret (the
   dashboard reveals it only once) and persist it to `~/.zed/.env` so the
   sidecar can authenticate `spectrum-ts`. Spectrum is always on, so there's no
   separate enable step.
4. **Register your phone number** as a Spectrum user (idempotent â€” skipped if
   a user with that number already exists).
5. **Print the assigned iMessage line** â€” the number you text to reach your
   agent.
6. **Install the sidecar deps** (`npm ci` â€” installs the committed lockfile
   verbatim, so every setup runs the exact `spectrum-ts` version this plugin
   was written against).

There is no separate `login` command; like every other Zed channel,
...
Run `zed photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.zed/.env` (the same place every other
...
Management metadata lives in `~/.zed/auth.json` under `credential_pool`:
...
| `PHOTON_HOME_CHANNEL`     | your number (set by setup) | Default space for cron delivery â€” a space id, or a bare E.164 number (resolved to a DM) |
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry â€” toggle with `zed photon telemetry on\|off` (restart the gateway to apply) |
| `PHOTON_MARKDOWN`         | true                       | Send agent replies as markdown (iMessage renders natively). `false` strips formatting to plain text |
| `PHOTON_REACTIONS`        | false                      | Tapback ðŸ‘€/ðŸ‘/ðŸ‘Ž as processing status; tapbacks on bot messages reach the agent as `reaction:added:<emoji>` |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  voice note â€” parity with the BlueBubbles iMessage channel. Mixed iMessage
  ...
  received: â€¦]` or `[Photon voice received: â€¦]`) so the agent still knows
  something arrived.
...
- **Reactions (tapbacks) are supported** behind `PHOTON_REACTIONS` (default
  off): the adapter tapbacks ðŸ‘€ while processing and swaps it for ðŸ‘/ðŸ‘Ž on
  ...
  restart is best-effort â€” the live reaction handle is lost, so a stale
...
- **Message effects, polls** â€” supported by `spectrum-ts` but not yet
...
3. Migrate `sidecar/index.mjs` against the new typings
   (`sidecar/node_modules/spectrum-ts/dist/*.d.ts` is the source of truth â€”
   the hosted docs can lag).
4. Run `pytest tests/plugins/platforms/photon/`.
5. Verify end-to-end: `zed photon status`, a DM and a group roundtrip,
