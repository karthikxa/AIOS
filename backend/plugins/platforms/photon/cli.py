"""
``zed photon ...`` CLI subcommands â€” registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Zed
gateway channel onboards through a single setup surface).

Photon uses the spectrum-ts gRPC stream for inbound â€” there is no webhook
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `zed photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Zed Agent')")
...
def dispatch(args: argparse.Namespace) -> int:
    ...
    if sub is None:
        # No subcommand given â€” show status by default.
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    Internal helper â€” invoked as the first step of ``setup``. There is
    no standalone ``zed photon login`` command; Photon onboards
    ...
    def _print_code(code):
        ...
        print()
        print("â”Œâ”€ Photon device login â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€")
        print(f"â”‚  Open this URL:  {target}")
        print(f"â”‚  Enter the code: {code.user_code}")
        print("â”‚  (waiting for approval â€” Ctrl-C to cancel)")
    ...
    try:
        ...
        return 1
    # Don't print any portion of the token â€” even a prefix can help a
    # shoulder-surfer or accidentally leak into a screen recording.
    _ = token
    print(f"âœ“ logged in â€” token saved to {photon_auth._auth_json_path()}")
    return 0
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    if not token:
        print("[1/5] No Photon token found â€” running device login...")
    ...
    # 2. Find or create the "Zed Agent" project.
    ...
    # 3. Rotate the project secret and persist creds (runtime -> ~/.zed/.env,
    ...
    #    (ids unified), so there's nothing to enable â€” the id we already have is
    #    the Spectrum id.
    try:
        ...
        print(f"  âœ“ Spectrum ready (project id {spectrum_id}) â€” secret saved")
    except Exception as e:
    ...
    if not phone:
        ...
        # Name/email are optional and never prompted for â€” pass --first-name /
        ...
        try:
            ...
            return 1
        print("  âœ“ phone registered" if created else "  âœ“ phone already registered")
        ...
        # truth â€” and we already have it from the (reused) user object.
        agent_number = photon_auth.user_assigned_line(user)
        # Allowlist the operator and make their DM the cron home channel â€”
    ...
    if not agent_number:
        # No per-user assignment â€” fall back to a dedicated line if the project
    ...
    if agent_number:
        print()
        print(color("â”Œâ”€ Your agent's iMessage number â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€", Colors.GREEN))
        print(
            color("â”‚  ðŸ“± ", Colors.GREEN)
            + color(str(agent_number), Colors.GREEN, Colors.BOLD)
        )
        print(color("â”‚  Text this number from your phone to talk to your agent.", Colors.GREEN))
        print(color("â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€", Colors.GREEN))
    else:
        print("      No iMessage line assigned yet â€” check the Photon dashboard.")
    if registered_phone:
        try:
    ...
    print()
    print("âœ“ Photon setup complete.")
    print("  Start the gateway:  zed gateway start")
    return 0
    ...
    for key, label in (
        ...
        try:
            if get_env_value(key):
                print(f"      {key} already set â€” leaving it as-is.")
                continue
            save_env_value(key, phone)
            print(f"  âœ“ {label} ({key})")
        except Exception as e:
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    # Defer the credential rows to auth.print_credential_summary â€” its emit
    ...
    print(f"  node binary         : {node_bin or 'âœ— missing (install Node 18+)'}")
    print(f"  sidecar deps        : {'âœ“ installed' if sidecar_installed else 'âœ— run `zed photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`zed photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.zed/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `zed photon telemetry on` / `zed photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"âœ“ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.zed/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  zed gateway restart")
    return 0
...
def _install_sidecar() -> int:
    ...
    # migrate sidecar/index.mjs, re-run the photon tests â€” never `@latest`
    ...
    if proc.returncode != 0:
        print(f"  npm ci failed â€” falling back to:  {npm} install")
        proc = subprocess.run(  # noqa: S603
            [npm, "install"],
            cwd=str(_SIDECAR_DIR),
            check=False,
...
#
# `zed gateway setup` discovers platforms via the registry and calls each
# entry's zero-arg ``setup_fn``. Photon registers this function so it appears
# in the unified setup wizard alongside every other channel â€” same onboarding
# surface, no Photon-specific detour. It runs the identical device-login +
# project + user + sidecar flow as ``zed photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `zed gateway setup` wizard."""
