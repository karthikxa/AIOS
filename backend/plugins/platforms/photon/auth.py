...
Credential storage mirrors every other Zed channel:

    * runtime SDK creds  -> ``~/.zed/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.zed/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a zed dependency
...
# endpoint â€” an unregistered client_id is rejected with
...
# registers Zed as its own client_id.
...
# Default name of the project Zed provisions for the operator.
DEFAULT_PROJECT_NAME = "Zed Agent"

# Polling defaults per RFC 8628.  Photon overrides via `interval` /
# `expires_in` in the device-code response â€” those win.
...
# ---------------------------------------------------------------------------
# auth.json helpers â€” share the file with the rest of zed-agent.
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.zed/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.zed/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.zed/.env`` (canonical runtime store).
    ...
    *,
    ...
    while time.time() < deadline:
        ...
        if resp.status_code == 429:
            # RFC 8628 Â§3.5 â€” treat 429 as slow_down.
            sleep += 10
            if on_pending:
                _safe(on_pending)
            continue
...
def get_session(token: str) -> Dict[str, Any]:
    """GET ``/api/auth/get-session`` â€” confirm the token + fetch the user."""
    if httpx is None:
...
def list_projects(token: str) -> List[Dict[str, Any]]:
    """GET ``/api/projects`` â€” return the caller's projects."""
    if httpx is None:
        raise RuntimeError("httpx is required for Photon")
...
def regenerate_project_secret(token: str, project_id: str) -> str:
    """POST ``/api/projects/{id}/regenerate-secret`` â†’ the new project secret.
...
def list_users(project_id: str, project_secret: str) -> List[Dict[str, Any]]:
    """GET Spectrum Cloud ``/projects/{id}/users/`` â†’ ``SpectrumUser[]``."""
    if httpx is None:
        raise RuntimeError("httpx is required for Photon")
...
def list_lines(token: str, project_id: str) -> List[Dict[str, Any]]:
    """GET ``/api/projects/{id}/lines`` â†’ ``[{id, platform, phoneNumber, status}]``."""
    if httpx is None:
        raise RuntimeError("httpx is required for Photon")
...
# ---------------------------------------------------------------------------
# Credential status (display-only â€” never emits raw secret material)
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels: Dict[str, str] = {}
    labels["device_token"] = (
        "âœ“ stored" if load_photon_token()
        else "âœ— missing (run `zed photon setup`)"
    )
    ...
    labels["project_id"] = sid if sid else "âœ— missing"
    labels["project_key"] = "âœ“ stored" if sec else "âœ— missing"
    phone, assigned = load_user_numbers()
    labels["phone_number"] = (
        phone if phone else "âœ— missing (run `zed photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "âœ— missing (run `zed photon setup`)"
    )
