"""
IRC Platform Adapter for Zed Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Zed agent.  Zero external dependencies â€” uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "zed-bot")
        ...
        # IRC nicks are case-insensitive â€” normalise for lookups
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            logger.error("IRC: failed to connect to %s:%s â€” %s", self.server, self.port, e)
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Zed Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Zed Agent shutting down")
    ...
    # â”€â”€ Sending â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async def send_typing(self, chat_id: str, metadata=None) -> None:
        """IRC has no typing indicator â€” no-op."""
        pass
    ...
    # â”€â”€ Message splitting â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def _strip_markdown(text: str) -> str:
        ...
        text = re.sub(r"`(.+?)`", r"\1", text)
        # Code blocks: ```...``` â†’ content
        text = re.sub(r"```\w*\n?", "", text)
        # Images: ![alt](url) â†’ url  (must come BEFORE links)
        text = re.sub(r"!\[([^\]]*)\]\(([^)]+)\)", r"\2", text)
        # Links: [text](url) â†’ text (url)
        text = re.sub(r"\[([^\]]+)\]\(([^)]+)\)", r"\1 (\2)", text)
        return text

    # â”€â”€ Raw IRC I/O â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async def _receive_loop(self) -> None:
        """Main receive loop â€” reads lines and dispatches them."""
        buffer = b""
        try:
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        # RPL_WELCOME (001) â€” registration complete
        if command == "001":
        ...
        # ERR_NICKNAMEINUSE (433) â€” nick collision during registration
        if command == "433":
            # Retry with incrementing suffix: zed_, zed_1, zed_2...
        ...
        # PRIVMSG â€” incoming message (channel or DM)
        if command == "PRIVMSG" and len(params) >= 2:
            ...
            # CTCP ACTION (/me) â€” convert to text
        ...
        # NICK â€” track our own nick changes
...
def check_requirements() -> bool:
    ...
    Only requires the server and channel â€” no external pip packages needed.
    """
    ...
    # zed setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `zed gateway setup` flow for the IRC platform.

    Lazy-imports ``zed_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Zed to an IRC network. Uses Python stdlib â€” no extra packages needed.")
    ...
    if not server:
        print_warning("Server is required â€” skipping IRC setup")
        return
    ...
    if port:
        try:
            ...
            print_warning(f"Invalid port â€” using default {default_port}")
    elif get_env_value("IRC_PORT"):
        # User cleared the prompt; drop the override so the default applies.
        save_env_value("IRC_PORT", "")

    nickname = prompt(
        "Bot nickname (e.g. zed-bot)",
        default=get_env_value("IRC_NICKNAME") or "",
    )
    if not nickname:
        print_warning("Nickname is required â€” skipping IRC setup")
        return
    save_env_value("IRC_NICKNAME", nickname.strip())

    channel = prompt(
        "Channel to join (e.g. #zed â€” comma-separate for multiple)",
        default=get_env_value("IRC_CHANNEL") or "",
    )
    if not channel:
        print_warning("Channel is required â€” skipping IRC setup")
        return
    save_env_value("IRC_CHANNEL", channel.strip())

    print()
    print_info("ðŸ”‘ Optional authentication")
    ...
    print()
    print_info("ðŸ”’ Access control: restrict who can message the bot")
    print_info("   IRC nicks are not authenticated â€” anyone can claim any nick.")
    ...
    if allow_all:
        ...
        print_warning("âš ï¸  Open access â€” any nick in the channel can command the bot.")
    else:
        ...
        if allowed:
            ...
            save_env_value("IRC_ALLOWED_USERS", "")
            print_info("No nicks allowed â€” the bot will ignore all messages until you add nicks.")

    print()
    print_success("IRC configuration saved to ~/.zed/.env")
    print_info("Restart the gateway for changes to take effect: zed gateway restart")
...
def _env_enablement() -> dict | None:
    ...
    the core hook â€” it becomes a proper ``HomeChannel`` dataclass on the
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "zed-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "zed-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Zed Agent (cron)")
...
def register(ctx):
    """Plugin entry point: called by the Zed plugin system."""
    ctx.register_platform(
        ...
        platform_hint=(
            ...
            "â€” use plain text only. Messages are limited to ~450 characters per "
