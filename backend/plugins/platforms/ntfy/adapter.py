"""ntfy platform adapter (Zed plugin).
...
replies via HTTP POST. No external SDK â€” only httpx, which is already
a Zed dependency.

This adapter ships as a Zed platform plugin under
``plugins/platforms/ntfy/``. The Zed plugin loader scans the
...
the registry â€” no edits to core files required.
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "zed-in"              # subscribe topic (incoming)
          publish_topic: "zed-out"     # optional â€” defaults to topic
          token: "..."                    # optional Bearer / Basic auth token
          markdown: true                  # optional â€” enable markdown (default: false)
...
config.yaml ``extra``):
    ...
    NTFY_ALLOW_ALL_USERS       Allow any topic â€” dev only
...
authorization. Each topic is treated as a single trusted channel â€”
...
_ECHO_TAG = "zed-agent"  # tag added to outgoing messages for echo-loop prevention
...
def _build_auth_header(token: str) -> Dict[str, str]:
    ...
    Tokens are stripped of surrounding whitespace â€” pasted tokens often
...
class NtfyAdapter(BasePlatformAdapter):
    ...
    publishes replies via HTTP POST. No external SDK â€” only httpx.
    """
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            logger.info("[%s] Connected â€” subscribing to %s/%s", self.name, self._server, self._topic)
            return True
    ...
    async def _consume_stream(self, url: str, headers: Dict[str, str]) -> None:
        ...
        async with self._http_client.stream(
            ...
            if response.status_code == 401:
                logger.error(
                    "[%s] Authentication failed (401) â€” stopping reconnect loop. Check NTFY_TOKEN.",
                    self.name,
                )
            ...
            if response.status_code == 404:
                logger.error(
                    "[%s] Topic not found (404): %s â€” stopping reconnect loop.",
    ...
    async def _on_message(self, event: Dict[str, Any]) -> None:
        ...
        # publisher-controlled and must NOT be used for authorization â€” any
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Zed dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `zed gateway status` without
        # instantiating the HTTP client.
        env_enablement_fn=_env_enablement,
        # Cron home-channel delivery support â€” `deliver=ntfy` cron jobs
        ...
        max_message_length=MAX_MESSAGE_LENGTH,
        emoji="ðŸ””",
        # ntfy publishers have no persistent identity â€” topic names are
        # the only identifier, no phone numbers / emails to redact.
        pii_safe=True,
        allow_update_command=True,
        platform_hint=(
            "You are communicating via ntfy push notifications. "
            "Use plain text by default â€” ntfy supports optional markdown "
