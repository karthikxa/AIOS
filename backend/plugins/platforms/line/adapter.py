"""
LINE Messaging API platform adapter for Zed Agent.
...
Buttons bubble â€” the user taps it later to receive the cached answer via a
*fresh* reply token (also free). State machine: PENDING â†’ READY â†’ DELIVERED,
...
binary uploads â€” images, audio, and video must be reachable HTTPS URLs.
...
to Zed Agent. It deliberately ports the *strongest* idea from each into
a single plugin-form module that requires zero core edits:

* PR #18153 (leepoweii)   â€” Template Buttons postback cache state machine,
  Markdown URL preservation, system-message bypass.
* PR #8398  (yuga-hashimoto) â€” media URL serving with traversal guard,
  send_voice / send_video, ``LINE_PUBLIC_URL`` env, macOS ``/tmp`` root.
* PR #16832 (jethac)      â€” config wiring style, voice/image tests.
* PR #21023 (perng)       â€” plugin-form skeleton (the only one already
  modeled on ``ADDING_A_PLATFORM.md``), replyâ†’push fallback at 50s TTL,
  loading-animation indicator, source dispatcher.
* PR #14942 (soichiyo)    â€” Cloudflare-tunnel operating model (docs only).
* PR #14988 (David-0x221Eight) â€” text-first scope discipline.
* PR #6676  (liyoungc)    â€” Push-only mode (used as the ``threshold=0``
  fallback path here).
"""
...
# ---------------------------------------------------------------------------
# Lazy / function-level imports for gateway internals are NOT used here â€”
...
# Webhook hardening
WEBHOOK_BODY_MAX_BYTES = 1_048_576  # 1 MiB â€” webhooks are tiny JSON
...
DEFAULT_DELIVERED_TEXT = "Already replied âœ…"
...
# routes on. LINE has no separate "voice" type â€” audio messages are recorded
...
# A 1Ã—1 transparent PNG used as fallback video preview thumbnail when no
# explicit preview is supplied â€” LINE requires ``previewImageUrl`` for
...
def strip_markdown_preserving_urls(text: str) -> str:
    ...
    LINE's text bubble has zero Markdown support â€” bold, italics, code
    ...
    Source: PR #18153 (leepoweii) â€” adapted to keep code-block content
    ...
    # Code blocks first â€” keep the inner content, drop the fences.
    ...
    # Markdown links â†’ "label (url)"
    text = _MD_LINK_RE.sub(lambda m: f"{m.group(1)} ({m.group(2)})", text)

    # Bold/italic markers â€” strip.
    text = _MD_BOLD_RE.sub(r"\1", text)
    text = _MD_ITAL_RE.sub(r"\1", text)

    # Headings (#, ##) and bullet markers â€” strip the prefix only.
    text = _MD_HEADING_RE.sub("", text)
    text = _MD_BULLET_RE.sub("â€¢ ", text)
...
def split_for_line(text: str, max_chars: int = LINE_SAFE_BUBBLE_CHARS) -> List[str]:
    ...
    if remaining:
        # Truncate gracefully â€” caller already burned its 5-bubble budget.
        if chunks:
            tail = chunks[-1]
            if len(tail) > max_chars - 1:
                tail = tail[: max_chars - 1]
            chunks[-1] = tail.rstrip() + "â€¦"
        else:
            chunks.append(remaining[: max_chars - 1] + "â€¦")
    return chunks
...
# ---------------------------------------------------------------------------
# Cache state machine â€” slow-LLM postback flow
# ---------------------------------------------------------------------------
...
class RequestCache:
    ...
    PRs #18153 originally combined two TTLs â€” one for PENDING (24h) and
...
def _resolve_chat(source: Dict[str, Any]) -> Tuple[str, str]:
    ...
    LINE sources are one of:
      * ``{"type": "user",  "userId":  "U..."}``  â†’ 1:1 DM
      * ``{"type": "group", "groupId": "C...", "userId": "U..."}``  â†’ group chat
      * ``{"type": "room",  "roomId":  "R...", "userId": "U..."}``  â†’ multi-user room
...
def _allowed_for_source(
    ...
    *,
    ...
    """Three-list gate â€” credit PR #18153."""
    if allow_all:
        return True
    src_type = (source or {}).get("type", "")
    if src_type == "user":
...
def _text_message(text: str) -> Dict[str, Any]:
    ...
    if len(text) > LINE_PER_BUBBLE_CHARS:
        text = text[: LINE_PER_BUBBLE_CHARS - 1] + "â€¦"
...
def build_postback_button_message(
    ...
    """Template Buttons message â€” the slow-LLM postback bubble.
    ...
    LINE limits: ``text`` â‰¤ 160 chars, ``altText`` â‰¤ 400 chars.
    """
...
class LineAdapter(BasePlatformAdapter):
    ...
    # LINE has its own message-edit story (none) â€” we always send fresh
    ...
    def __init__(self, config, **kwargs):
        ...
        # Public base URL â€” required for media sending when bind isn't
        # publicly reachable.
        ...
        self._reply_tokens: Dict[str, Tuple[str, float]] = {}  # chat_id â†’ (token, expiry)
        ...
        # Media state
        self._media_tokens: Dict[str, Tuple[str, float]] = {}  # token â†’ (path, expiry)
        self._media_temp_paths: Set[str] = set()
        self._media_ttl = MEDIA_TOKEN_TTL_SECONDS

        # Pending-button slot per chat â€” ensures one outstanding postback
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._set_fatal_error(
                "missing_dep",
                "aiohttp is required for the LINE adapter â€” install with `pip install aiohttp`",
                retryable=False,
            )
            return False
        ...
        # Public health probe â€” useful for tunnel/proxy verification.
    ...
    async def _handle_message_event(self, event: Dict[str, Any]) -> None:
        ...
        # Handle media inbound â€” fetch the binary, cache it, and surface a
    ...
    async def _handle_postback_event(self, event: Dict[str, Any]) -> None:
        """User tapped the slow-LLM postback button â€” deliver cached payload."""
        ...
        if entry.state is State.READY:
            ...
            # Still working â€” re-issue the wait notice.
            try:
    ...
    async def get_chat_info(self, chat_id: str) -> Dict[str, Any]:
        ...
        LINE's chat-info APIs are limited and per-source-type â€” instead of
    ...
    # ------------------------------------------------------------------
    # Slow-LLM postback button â€” driven by _keep_typing
    # ------------------------------------------------------------------
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ZED_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            zed_home = Path.home().joinpath(".zed").resolve()

        allowed_roots = {
            Path(tempfile.gettempdir()).resolve(),
            Path("/tmp").resolve(),  # â†’ /private/tmp on macOS
            zed_home,
        }
    ...
    async def send_video(
        ...
        # write a stdlib 1Ã—1 PNG to /tmp and serve it. PR #8398.
...
def _is_relative_to(child: Path, parent: Path) -> bool:
    """Backport for Path.is_relative_to (Python 3.9+) â€” defensive against
...
def is_connected(config) -> bool:
    """Surface in ``zed status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``zed status`` reflect a LINE configuration that lives entirely
...
async def _standalone_send(
    ...
    *,
    ...
    ``thread_id`` is accepted for signature parity but ignored â€” LINE has
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``zed setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.zed/.env`` via ``zed_cli.config``.
    """
    print()
    ...
    try:
        from zed_cli.config import get_env_var, set_env_var
    except ImportError:
        print("zed_cli.config not available; set LINE_* vars manually in ~/.zed/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        platform_hint=(
            ...
            "Markdown â€” text bubbles show ** and # literally. Bare URLs are "
