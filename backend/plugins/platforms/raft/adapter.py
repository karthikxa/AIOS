...
and injects content-free wake hints into Zed' normal gateway session pipeline.
...
def check_raft_requirements() -> bool:
    ...
    Intentionally silent on failure â€” this is a passive probe registered as
...
class RaftAdapter(BasePlatformAdapter):
    ...
    def _spawn_bridge(self, port: int) -> None:
        ...
        if not raft_bin:
            logger.warning("[raft] raft CLI not found in PATH; bridge not spawned â€” wake-only polling mode")
            return
    ...
    async def _handle_wake(self, request: "web.Request") -> "web.Response":
        ...
        # Zed only verifies that wake hints are content-free.
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        """Accept Raft wake hints without interrupting an active Zed turn."""
        if not self._message_handler:
            return
