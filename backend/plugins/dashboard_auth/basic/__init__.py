"""BasicAuthProvider â€” username/password dashboard auth (no OAuth IDP).

A self-hosted "just put a password on my dashboard" provider. It plugs
into the same ``DashboardAuthProvider`` framework as the Zed OAuth
...
provider mints and verifies itself. That keeps it zero-infrastructure â€”
appropriate for a single-box self-hosted dashboard.

Configuration surfaces (env wins over config.yaml when set non-empty),
mirroring the Zed provider's precedence convention:
  ...
  ``config.yaml`` â€” canonical surface::
      ...
      dashboard:
        basic_auth:
          ...
          # Provide EITHER a precomputed scrypt hash (preferred â€” no
          # plaintext at rest) ...
  ...
  Environment overrides::
      ...
      ZED_DASHBOARD_BASIC_AUTH_PASSWORD_HASH   # preferred
      ZED_DASHBOARD_BASIC_AUTH_PASSWORD        # plaintext fallback
      ZED_DASHBOARD_BASIC_AUTH_SECRET
      ZED_DASHBOARD_BASIC_AUTH_TTL_SECONDS
...
multiple worker processes â€” set an explicit ``secret`` for stable
...
Skip reasons:
  Like the Zed provider, this exposes a module-level ``LAST_SKIP_REASON``
...
# signed tokens (no separator â€” binary HMAC bytes can't be confused with
# a delimiter).
...
class BasicAuthProvider(DashboardAuthProvider):
    ...
    def revoke_session(self, *, refresh_token: str) -> None:
        # Stateless tokens â€” nothing to revoke server-side. The session
...
def _resolve_secret(cfg_section: dict) -> bytes:
    ...
    restart or span multiple workers â€” logged at INFO).
    """
    raw = _resolve(
        "ZED_DASHBOARD_BASIC_AUTH_SECRET", cfg_section, "secret"
    )
    if not raw:
        logger.info(
            ...
            "secret (or ZED_DASHBOARD_BASIC_AUTH_SECRET) for stable "
            "sessions."
        )
...
def register(ctx) -> None:
    """Plugin entry â€” registers BasicAuthProvider when credentials exist.
    ...
    username = _resolve(
        "ZED_DASHBOARD_BASIC_AUTH_USERNAME", section, "username"
    )
    password_hash = _resolve(
        "ZED_DASHBOARD_BASIC_AUTH_PASSWORD_HASH", section, "password_hash"
    )
    plaintext = _resolve(
        "ZED_DASHBOARD_BASIC_AUTH_PASSWORD", section, "password"
    )
    ttl_raw = _resolve(
        "ZED_DASHBOARD_BASIC_AUTH_TTL_SECONDS", section, "session_ttl_seconds"
    )

    if not username:
        LAST_SKIP_REASON = (
            "dashboard.basic_auth.username is not set (and "
            "ZED_DASHBOARD_BASIC_AUTH_USERNAME is empty). Set a username "
    ...
    if not password_hash and not plaintext:
        LAST_SKIP_REASON = (
            ...
            "is preferred â€” compute it with "
    ...
    # ZED_DASHBOARD_BASIC_AUTH_PASSWORD env var overrides a config.yaml
    ...
    # over a config-only plaintext password at the same tier â€” it's the
    # preferred at-rest form. Concretely:
    #   * env password set        â†’ hash it (overrides any config hash)
    #   * else config password_hash set â†’ use it
    #   * else config plaintext password â†’ hash it in-memory
    plaintext_from_env = os.environ.get(
        "ZED_DASHBOARD_BASIC_AUTH_PASSWORD", ""
    ).strip()
