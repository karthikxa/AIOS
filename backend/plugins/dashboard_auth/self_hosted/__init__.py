...
same precedence convention as the ``nous`` plugin)::
    ...
    ZED_DASHBOARD_OIDC_ISSUER
    ZED_DASHBOARD_OIDC_CLIENT_ID
    ZED_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
...
# HS256 is deliberately excluded â€” it implies a shared secret we don't have in
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def __init__(
        ...
        *,
        ...
        # issuer the IDP advertises in discovery â€” we pin against the
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # Same flat ``state=â€¦;verifier=â€¦`` cookie shape every provider uses;
    ...
    def verify_session(self, *, access_token: str) -> Optional[Session]:
        ...
        try:
            ...
            # Expired / invalid token â€” protocol says return None, not raise.
            return None
        except ProviderError:
            raise
        # No refresh token available on this path; "" is fine â€” the middleware
    ...
    def revoke_session(self, *, refresh_token: str) -> None:
        ...
        # Must never raise â€” logout is client-side cookie clearing regardless.
        if not refresh_token:
            return None
        try:
            disco = self._get_discovery()
    ...
    def _exchange(
        ...
        *,
        ...
        if not id_token or not isinstance(id_token, str):
            raise ProviderError(
                "OIDC token response missing id_token â€” ensure the 'openid' "
        ...
        # previous (some IDPs don't rotate). Empty string if neither â€” the
    ...
    def _get_discovery(self) -> Dict[str, Any]:
        ...
        with self._discovery_lock:
            ...
            # New issuer/keys â†’ drop the JWKS client so it re-binds to the
    ...
    def _verify_id_token(self, id_token: str) -> Dict[str, Any]:
        import jwt  # lazy import â€” keeps startup fast for the ungated path
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored â€” Zed does not call
        ...
        # rather than dropping the info â€” org_id is a free-form string.
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``ZED_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "ZED_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "ZED_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("ZED_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "issuer and a client_id â€” either as env vars "
            "(ZED_DASHBOARD_OIDC_ISSUER + ZED_DASHBOARD_OIDC_CLIENT_ID) "
            "or under dashboard.oauth.self_hosted.{issuer,client_id} in "
            "config.yaml â€” or pass --insecure to skip the OAuth gate "
