"""NousDashboardAuthProvider â€” Zed Portal OAuth (authorization-code + PKCE).
...
its provider when a client_id is configured â€” either via ``config.yaml`` or
via the Portal-injected env var â€” so loopback / ``--insecure`` operators
are unaffected.
...
Configuration surfaces (env wins over config.yaml when set non-empty):
  ...
  Environment overrides â€” used by Fly.io's platform-secret injection so
  per-deploy values don't need to bake into ``config.yaml``:

      ZED_DASHBOARD_OAUTH_CLIENT_ID  â€” shape ``agent:{agent_instance_id}``
      ZED_DASHBOARD_PORTAL_URL       â€” defaults to
                                          ``https://portal.zedteam.com``
...
Key contract points encoded here:
  ...
  - the dashboard auth-code grant issues a 24h rotating refresh token
    ...
    reuse-detected refresh token Portal returns 400 â†’ ``RefreshExpiredError``
    â†’ middleware redirects to ``/auth/login``.
  - audience claim is the bare ``client_id`` (no ``zed-cli:`` prefix).
  - tolerant ``oauth_contract_version`` check: missing â†’ warn + proceed;
    present and ``!= 1`` â†’ refuse.
...
of cookie names; this provider just hands back ``{"code_verifier": â€¦,
"state": â€¦}`` and the route serializes those into the ``zed_session_pkce``
cookie.
...
Skip reasons:
  ...
  ("Set ZED_DASHBOARD_OAUTH_CLIENT_ID â€¦") instead of the bare "no
...
# Production Portal URL. Override via ZED_DASHBOARD_PORTAL_URL for
# staging (portal.rewbs.uk) or a custom deployment. Contract docs name
# this as the production issuer.
_DEFAULT_PORTAL_URL = "https://portal.zedteam.com"
...
def _b64url_no_pad(raw: bytes) -> str:
    """Base64url-encode without ``=`` padding (RFC 7636 Â§4)."""
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    """Zed Portal OAuth via authorization-code + PKCE (S256)."""
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"zed_session_pkce\"]``
        ...
        cookie_payload = {
            "zed_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def complete_login(
        ...
        *,
        ...
        # we just receive it for symmetry with the protocol. Zed Portal
        ...
        # (24h session, reuse-detected) â€” Portal NAS PR #293. A 400 here means
    ...
    def refresh_session(self, *, refresh_token: str) -> Session:
        ...
        so it never lands in Portal's request-body access logs â€” mirroring the
        ...
        next refresh replays a rotated token and â€” outside Portal's 60s grace
        â€” trips reuse-detection and revokes the whole session.
        ...
        if not refresh_token:
            # No RT to present â€” treat as a dead session so middleware
        ...
        try:
            response = httpx.post(
                ...
                # additionally reconciles the header against the body â€” sending
                ...
                # against the NAS #293 preview deploy: header-only â†’ 400
                # invalid_request; body â†’ accepted.)
                data={
    ...
    def _token_response_to_session(
        ...
        *,
        ...
        400 â€” ``InvalidCodeError`` for the auth-code path, ``RefreshExpiredError``
        for the refresh path â€” so the middleware's distinct handling
    ...
    def verify_session(self, *, access_token: str) -> Optional[Session]:
        ...
        try:
            ...
            # Expired/invalid token â€” middleware contract is None, not raise.
            return None
        ...
        # pass "" here â€” the RT-driven rotation path is middleware's job.
    ...
    def _validate_redirect_uri(self, redirect_uri: str) -> None:
        ...
        over plain HTTP â€” LAN IPs, internal hostnames, reverse proxies that
        terminate TLS upstream â€” are not rejected here; Portal makes the final
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        # Lazy import â€” keeps startup fast for operators who never trigger
        # the gated path.
        import jwt
        ...
        try:
            ...
            # between ZED_DASHBOARD_PORTAL_URL / ZED_DASHBOARD_OAUTH_CLIENT_ID
            ...
            # these values â€” they're surfaced for diagnostics only.
            details = ""
            try:
    ...
    def _check_agent_instance_id(self, claims: Dict[str, Any]) -> None:
        ...
        if token_instance_id is None:
            # Tolerated â€” the claim is documented as "should" not "must".
    ...
    def _check_contract_version(self, claims: Dict[str, Any]) -> None:
        """Contract C11 â€” tolerant treatment per OQ-C2."""
        contract_version = claims.get("oauth_contract_version")
        if contract_version is None:
            logger.warning(
                "Zed Portal token missing oauth_contract_version claim "
...
def _load_config_oauth_section() -> dict:
    ...
    config.yaml absent â€” common in fresh installs), (b) the
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``ZED_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
         after strip â€” empty values are treated as unset so a
         provisioned-but-not-populated Fly secret can't shadow a valid
         config.yaml entry).
      2. ``dashboard.oauth.client_id`` in ``config.yaml``.
      3. Empty string â€” signals "no client_id configured" to the caller.
    """
    env = os.environ.get("ZED_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``ZED_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("ZED_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    """Plugin entry â€” called by the plugin loader at startup.

    Registers ``NousDashboardAuthProvider`` only when a client_id is
    configured (either via ``ZED_DASHBOARD_OAUTH_CLIENT_ID`` env var
    or via ``dashboard.oauth.client_id`` in ``config.yaml``). The env
    var wins when set non-empty â€” Fly.io's platform-secret injection
    ...
    can surface "Set ZED_DASHBOARD_OAUTH_CLIENT_ID â€¦" instead of the
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            "ZED_DASHBOARD_OAUTH_CLIENT_ID is not set (and "
            "dashboard.oauth.client_id in config.yaml is empty). The "
            "Zed Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys a Zed Agent "
            "instance â€” set it to your provisioned client id (either "
    ...
    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"ZED_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
            f"the contract shape 'agent:{{instance_id}}'. The Zed Portal "
