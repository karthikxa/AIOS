"""Spotify integration plugin â€” bundled, auto-loaded.
...
``_check_spotify_available()`` â€” when the user has not run ``zed auth
spotify``, the tools remain registered (so they appear in ``zed
...
- Bundled + ``kind: backend`` auto-loads on startup just like image_gen
  backends â€” no user opt-in needed, no ``plugins.enabled`` config.

The Spotify auth flow (``zed auth spotify``), CLI plumbing, and docs
...
_TOOLS = (
    ("spotify_playback",  SPOTIFY_PLAYBACK_SCHEMA,  _handle_spotify_playback,  "ðŸŽµ"),
    ("spotify_devices",   SPOTIFY_DEVICES_SCHEMA,   _handle_spotify_devices,   "ðŸ”ˆ"),
    ("spotify_queue",     SPOTIFY_QUEUE_SCHEMA,     _handle_spotify_queue,     "ðŸ“»"),
    ("spotify_search",    SPOTIFY_SEARCH_SCHEMA,    _handle_spotify_search,    "ðŸ”Ž"),
    ("spotify_playlists", SPOTIFY_PLAYLISTS_SCHEMA, _handle_spotify_playlists, "ðŸ“š"),
    ("spotify_albums",    SPOTIFY_ALBUMS_SCHEMA,    _handle_spotify_albums,    "ðŸ’¿"),
    ("spotify_library",   SPOTIFY_LIBRARY_SCHEMA,   _handle_spotify_library,   "â¤ï¸"),
)
