...
exposed in this unified surface â€” xAI is the only backend that supports
...
Authentication: xAI Grok OAuth tokens (preferred â€” billed against the
...
def _resolve_xai_credentials() -> Tuple[str, str]:
    ...
    Order: runtime provider (xai-oauth pool entry) â†’ singleton ``auth.json``
    OAuth tokens â†’ ``XAI_API_KEY`` env var. ``api_key`` is empty when no
...
def _xai_user_agent() -> str:
    try:
        ...
        return zed_xai_user_agent()
    except Exception:
        return "zed-agent/video_gen"
...
async def _submit(
    ...
    *,
    ...
    """POST to /videos/generations â€” xAI's only public endpoint for our
...
class XAIVideoGenProvider(VideoGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        # hook (``zed_cli/tools_config.py``) so the picker doesn't blindly
        # prompt for an API key when the user is already signed in via xAI
        # Grok OAuth (SuperGrok / Premium+) â€” TTS / image gen / video gen
    ...
    async def _generate_async(
        ...
        *,
        ...
        if not api_key:
            return error_response(
                error=(
                    "No xAI credentials found. Sign in via `zed auth add xai-oauth` "
