...
``image_url`` is provided. The agent never sees the routing â€” it just
...
Authentication via ``FAL_KEY`` or the managed Zed gateway. Output is an
...
# drive which keys get added to the request payload â€” keys a family doesn't
...
FAL_FAMILIES: Dict[str, Dict[str, Any]] = {
    # â”€â”€â”€ Cheap / fast tier â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    "ltx-2.3": {
        "display": "LTX 2.3 (22B)",
        "speed": "~30-60s",
        "price": "cheap",
        ...
        # LTX docs don't expose duration/aspect/resolution enums â€” leave
    ...
    # â”€â”€â”€ Expensive / premium tier â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    "veo3.1": {
        "display": "Veo 3.1",
        "speed": "~60-120s",
        "price": "premium",
    ...
    "seedance-2.0": {
        ...
        # Seedance accepts "auto" too â€” we omit it from the enum so the
    ...
    "happy-horse": {
        ...
        "speed": "~60-120s",
        "price": "premium",
        "strengths": "Alibaba. New model, sparse public docs â€” conservative defaults.",
        "tier": "premium",
        "text_endpoint": "alibaba/happy-horse/text-to-video",
        "image_endpoint": "alibaba/happy-horse/image-to-video",
        # Docs don't expose duration/aspect/resolution â€” let the endpoint
...
def _submit_fal_video_request(endpoint: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                f"Zed Subscription gateway rejected endpoint '{endpoint}' "
                f"(HTTP {status}). This model may not yet be enabled on "
                f"the Zed Portal's FAL proxy. Either:\n"
                f"  â€¢ Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                f"  â€¢ Pick a different model via `zed tools` â†’ Video Generation."
            ) from exc
        raise
...
class FALVideoGenProvider(VideoGenProvider):
    ...
    def is_available(self) -> bool:
        ...
        except Exception:  # noqa: BLE001 â€” never break the picker
            return False
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            "name": "FAL",
            "badge": "paid",
            "tag": "LTX, Pixverse, Veo 3.1, Seedance 2.0, Kling 4K, Happy Horse â€” text-to-video & image-to-video",
            "env_vars": [
                {
                    "key": "FAL_KEY",
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not _check_fal_video_available():
            return error_response(
                error=(
                    ...
                    "(run `zed tools` â†’ Video Generation â†’ FAL to configure) "
                    "or sign in to Zed (`zed setup`) for managed gateway access."
                ),
        ...
        # Route: image_url â†’ image-to-video endpoint; else â†’ text-to-video.
            ...
            if not endpoint:
                return error_response(
                    error=(
                        ...
                        f"via `zed tools` â†’ Video Generation."
                    ),
...
def register(ctx) -> None:
    """Plugin entry point â€” wire ``FALVideoGenProvider`` into the registry."""
