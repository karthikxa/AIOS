"""disk-cleanup plugin â€” auto-cleanup of ephemeral Zed session files.

Wires three behaviours:

1. ``post_tool_call`` hook â€” inspects ``write_file`` and ``terminal``
   tool results for newly-created paths matching test/temp patterns
   under ``ZED_HOME`` and tracks them silently.  Zero agent
   compliance required.

2. ``on_session_end`` hook â€” when any test files were auto-tracked
   during the just-finished turn, runs :func:`disk_cleanup.quick` and
   logs a single line to ``$ZED_HOME/disk-cleanup/cleanup.log``.

3. ``/disk-cleanup`` slash command â€” manual ``status``, ``dry-run``,
...
# cleanup.  Guarded by a lock â€” post_tool_call can fire concurrently on
# parallel tool calls.
...
def _extract_paths_from_patch(args: Dict[str, Any]) -> Set[str]:
    ...
    # most of its use is editing existing files â€” we only care about new
...
def _extract_paths_from_terminal(args: Dict[str, Any], result: str) -> Set[str]:
    ...
    if isinstance(cmd, str) and cmd:
        # Tokenise the command â€” catches `touch /tmp/zed-x/test_foo.py`
        try:
...
_HELP_TEXT = """\
/disk-cleanup â€” ephemeral-file cleanup
...
All operations are scoped to ZED_HOME and /tmp/zed-*.
...
def _handle_slash(raw_args: str) -> Optional[str]:
    ...
    if sub == "deep":
        # In-session deep can't prompt the user interactively â€” show what
    ...
    if sub == "track":
        ...
        return (
            f"Not tracked (already present, missing, or outside ZED_HOME): "
            f"{path_arg}"
        )
...
def register(ctx) -> None:
    ...
    ctx.register_command(
        ...
        description="Track and clean up ephemeral Zed session files.",
    )
