"""disk_cleanup â€” ephemeral file cleanup for Zed Agent.
...
tracking and cleanup happen automatically â€” the agent never needs to
call a tool or remember a skill.

Rules:
  - test files    â†’ delete immediately at task end (age >= 0)
  - temp files    â†’ delete after 7 days
  - cron-output   â†’ delete after 14 days
  - empty dirs    â†’ always delete (under ZED_HOME)
  - research      â†’ keep 10 newest, prompt for older (deep only)
  - chrome-profileâ†’ prompt after 14 days (deep only)
  - >500 MB files â†’ prompt always (deep only)

Scope: strictly ZED_HOME and /tmp/zed-*
Never touches: ~/.zed/logs/ or any system directory.
"""
...
try:
    ...
    def get_zed_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ZED_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".zed").resolve()
...
def get_state_dir() -> Path:
    """State dir â€” separate from ``$ZED_HOME/logs/``."""
    return get_zed_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log â€” intentionally NOT under ``$ZED_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ZED_HOME or ``/tmp/zed-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    zed_home = get_zed_home()
    try:
        path.resolve().relative_to(zed_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/zed-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("zed-"):
        return True
    return False
...
# ---------------------------------------------------------------------------
# tracked.json â€” atomic read/write, backup scoped to tracked.json only
# ---------------------------------------------------------------------------
    ...
    try:
        ...
        if bak.exists():
            try:
                ...
                _log("WARN: tracked.json corrupted â€” restored from .bak")
                return data
            except Exception:
                pass
        _log("WARN: tracked.json corrupted, no backup â€” starting fresh")
        return []


def save_tracked(tracked: List[Dict[str, Any]]) -> None:
    """Atomic write: ``.tmp`` â†’ backup old â†’ rename."""
    tf = get_tracked_file()
    tf.parent.mkdir(parents=True, exist_ok=True)
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "zed-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $ZED_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    (``jobs.json``, ``.tick.lock``) â€” it does NOT blanket-protect
    everything under ``cron/`` because ``cron/output/`` is disposable.
    """
    # Lazily build the set once per process so ZED_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        zed_home = get_zed_home()
        for parent in ("cron", "cronjobs"):
            base = zed_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ZED_HOME)")
        return False
    ...
    for item in tracked:
        ...
        if cat == "cron-output":
            ...
            if re_cat != "cron-output":
                # Stale entry â€” would be skipped by quick(); omit from
                # dry-run output too.
                continue
...
def quick() -> Dict[str, Any]:
    """Safe deterministic cleanup â€” no prompts.
    ...
    for item in tracked:
        ...
        if cat == "cron-output":
            ...
            if re_cat != "cron-output":
                ...
                # Drop the stale entry â€” it was misclassified.
                continue
    ...
    # Remove empty dirs under ZED_HOME, but never recurse into known
    # durable state trees.  Some installs place the Zed checkout, venv,
    # and desktop build under ZED_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    zed_home = get_zed_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in zed_home.iterdir():
            if (
                top.is_dir()
...
# ---------------------------------------------------------------------------
# Deep cleanup (interactive â€” not called from plugin hooks)
# ---------------------------------------------------------------------------
...
def deep(
    ...
    if confirm is None:
        # No interactive confirmer â€” deep stops after the quick pass.
...
def guess_category(path: Path) -> Optional[str]:
    ...
    zed_home = get_zed_home()
    try:
        rel = path.resolve().relative_to(zed_home)
        ...
        if top == "cron" or top == "cronjobs":
            ...
            # auto-tracked â€” deleting it wipes the live scheduler
        ...
        # Path isn't under ZED_HOME (e.g. /tmp/zed-*) â€” fall through.
        pass
