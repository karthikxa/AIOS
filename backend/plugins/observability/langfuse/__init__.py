"""langfuse â€” Zed plugin for Langfuse observability.

Traces Zed conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Zed plugin system â€” standalone plugins only
load when listed in ``plugins.enabled`` (via ``zed plugins enable
observability/langfuse`` or ``zed tools â†’ Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``zed tools`` or ~/.zed/.env):
  ZED_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  ZED_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  ZED_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  ZED_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  ZED_LANGFUSE_RELEASE     - release/version tag
  ZED_LANGFUSE_SAMPLE_RATE - sampling rate 0.0â€“1.0 (default: 1.0)
  ZED_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  ZED_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
# never reaches that state â€” interrupted, a tool-only final step, or empty
# final content â€” would otherwise linger forever, so over the cap we evict
...
# Langfuse-issued keys always carry these prefixes (cloud or self-hosted â€”
...
# See #23823 â€” the silent-failure bug this guard fixes.
...
def _debug_enabled() -> bool:
    return _env_bool("ZED_LANGFUSE_DEBUG")
...
def _validate_langfuse_key(env_name: str, value: str) -> Optional[str]:
    ...
    single log line â€” it names the env var and shows a safe preview.
    """
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Zed plugin system â€”
    ...
    public_key = _env("ZED_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("ZED_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    ...
    # keys at construction time â€” it queues traces in memory and only
    ...
    placeholder_issues = [
        msg
        for msg in (
            _validate_langfuse_key("ZED_LANGFUSE_PUBLIC_KEY", public_key),
            _validate_langfuse_key("ZED_LANGFUSE_SECRET_KEY", secret_key),
        )
        if msg
    ]
    if placeholder_issues:
        logger.warning(
            ...
            "or unset ZED_LANGFUSE_PUBLIC_KEY / ZED_LANGFUSE_SECRET_KEY to "
    ...
    base_url = _env("ZED_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("ZED_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("ZED_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("ZED_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid ZED_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _trace_key(
    ...
    *,
    ...
    Older Zed paths only expose ``task_id``/``session_id``. Newer paths
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("ZED_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
...
def _evict_stale_locked() -> None:
    ...
    at ``_MAX_TRACE_STATE`` â€” a true ceiling. The evicted entry's root span is
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Zed branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Zed also has a turn-scoped
    ...
    # messages is a list only for legacy Zed branches that fired
    # pre_llm_call with API messages directly. Current Zed fires
    # pre_llm_call for context injection (conversation_history/user_message,
    # no messages list) â€” tracing that would create orphan traces.
    task_key = _trace_key(
        task_id,
        session_id,
        turn_id=turn_id,
...
def on_post_llm_call(*, task_id: str = "", session_id: str = "", provider: str = "", base_url: str = "",
    ...
    if assistant_message is not None:
        ...
        # post_api_request path â€” reconstruct from summary kwargs
        output = {
    ...
    # where ``getattr(response, "usage", None)`` is always None â€” so usage and
...
def register(ctx) -> None:
    ...
    # Zed versions.  pre_api_request / post_api_request fire per API
