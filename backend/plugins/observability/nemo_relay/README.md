...
Optional Zed observability plugin that maps Zed observer hooks to
...
not replace Zed Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Zed emit NeMo Relay lifecycle events for
the work Zed already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Zed Agent can:

- Preserve Zed execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
zed plugins enable observability/nemo_relay
```

The `ZED_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `ZED_HOME` that the
agent run will use:

```bash
env ZED_HOME=/tmp/zed-nemo-relay-test \
  zed plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`ZED_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`ZED_HOME` is the Zed profile/config home used by both
`zed plugins enable ...` and the later `zed chat ...` run. If unset,
Zed uses the user's default home, usually `~/.zed`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export ZED_HOME=/tmp/zed-nemo-relay-test
zed plugins enable observability/nemo_relay
zed chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `zed` command you run is built from the
...
uv run zed plugins enable observability/nemo_relay
uv run zed chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/zed_agent-*.whl
...
The plugin can configure exporters directly from `ZED_NEMO_RELAY_*`
...
```bash
export ZED_NEMO_RELAY_ATOF_ENABLED=1
export ZED_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export ZED_NEMO_RELAY_ATIF_ENABLED=1
export ZED_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `ZED_NEMO_RELAY_ATOF_FILENAME`
- `ZED_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `ZED_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `ZED_NEMO_RELAY_ATIF_AGENT_NAME`
- `ZED_NEMO_RELAY_ATIF_AGENT_VERSION`
- `ZED_NEMO_RELAY_ATIF_MODEL_NAME`
- `ZED_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)
...
```bash
export ZED_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `ZED_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`ZED_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`ZED_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Zed does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Zed keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Zed routes LLM and tool
...
For the full generic Zed middleware contract, see
...
export ZED_HOME=/tmp/zed-nemo-relay-docs/zed-home
mkdir -p "$ZED_HOME"

cat > "$ZED_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Zed session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export ZED_NEMO_RELAY_ATOF_ENABLED=1
export ZED_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/zed-nemo-relay-docs/subagent/atof
export ZED_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export ZED_NEMO_RELAY_ATOF_MODE=overwrite
export ZED_NEMO_RELAY_ATIF_ENABLED=1
export ZED_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/zed-nemo-relay-docs/subagent/atif
export ZED_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export ZED_NEMO_RELAY_ATIF_AGENT_NAME='Zed Agent E2E'
export ZED_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export ZED_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"zed.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Zed Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Zed dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/zed-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/zed-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/zed-nemo-relay-docs/workdir/beta.txt
cd /tmp/zed-nemo-relay-docs/workdir

export ZED_NEMO_RELAY_ATOF_ENABLED=1
export ZED_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/zed-nemo-relay-docs/parallel/atof
export ZED_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export ZED_NEMO_RELAY_ATOF_MODE=overwrite
export ZED_NEMO_RELAY_ATIF_ENABLED=1
export ZED_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/zed-nemo-relay-docs/parallel/atif
export ZED_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export ZED_NEMO_RELAY_ATIF_AGENT_NAME='Zed Agent E2E'
export ZED_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Zed sessions map to `agent` scopes.
- Zed API request hooks map to `llm` scope start/end events.
- Zed tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Zed execution middleware to hand
...
```bash
export ZED_NEMO_RELAY_PLUGINS_TOML=/tmp/zed-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Zed routes execution
...
Zed provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Zed provider adapter next_call(...)

Zed tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Zed tool dispatcher next_call(...)
```
...
middleware for a local Zed run. This path requires a NeMo Relay runtime that
...
```bash
export ZED_HOME=/tmp/zed-middleware-test/zed-home
mkdir -p "$ZED_HOME" /tmp/zed-middleware-test/nemo-relay
...
cat > /tmp/zed-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/zed-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/zed-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Zed Middleware E2E"
agent_version = "local"
...
export ZED_NEMO_RELAY_PLUGINS_TOML=/tmp/zed-middleware-test/nemo-relay/plugins.toml
