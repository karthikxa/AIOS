...
def _load_supermemory_config(zed_home: str) -> dict:
    config = _default_config()
    config_path = Path(zed_home) / "supermemory.json"
    ...
    # Keep raw container_tag â€” template variables like {identity} are resolved
...
def _save_supermemory_config(values: dict, zed_home: str) -> None:
    config_path = Path(zed_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "zed"},
        )
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        ...
        merged = {"sm_source": "zed", **(metadata or {})}
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `zed memory setup`.
        # All other options are documented for $ZED_HOME/supermemory.json
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._turn_count = 0
        self._config = _load_supermemory_config(self._zed_home)
