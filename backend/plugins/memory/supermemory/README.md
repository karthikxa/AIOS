...
Config file: `$ZED_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `zed` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `zed-{identity}` â†’ `zed-coder`). |
...
All Supermemory API calls send `x-sm-source: zed`, and document writes stamp
`metadata.sm_source: zed`. This is a **functional routing key, not telemetry**:
it groups Zed-written memories into a dedicated "Zed" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Zed profile:

```json
{
  "container_tag": "zed-{identity}"
}
```

For a profile named `coder`, this resolves to `zed-coder`. The default profile resolves to `zed-default`. Without `{identity}`, all profiles share the same container.
