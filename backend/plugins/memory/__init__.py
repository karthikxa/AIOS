...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with zed-agent)
2. User-installed providers: ``$ZED_HOME/plugins/<name>/``
...
_USER_NAMESPACE = "_zed_user_memory"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_zed_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_zed_user_memory'`` â€” the
...
def _is_memory_provider_dir(path: Path) -> bool:
    ...
    ``__init__.py`` source.  Cheap text scan â€” no import needed.
    """
...
def discover_memory_providers() -> List[Tuple[str, str, bool]]:
    ...
    for name, child in _iter_provider_dirs():
        ...
        # Quick availability check â€” try loading and calling is_available()
        available = True
        try:
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$ZED_HOME/plugins/<name>/``) directories.  Bundled takes
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    - A register(ctx) function (plugin-style) â€” we simulate a ctx
    - A top-level class that extends MemoryProvider â€” we instantiate it
    """
...
def _get_active_memory_provider() -> Optional[str]:
    ...
    external provider is configured.  Lightweight â€” only reads config,
    no plugin loading.
    """
    try:
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    This is a lightweight scan â€” it only imports ``cli.py``, not the
    ...
    try:
        # Import the CLI module (lightweight â€” no SDK needed)
        if module_name in sys.modules:
            cli_mod = sys.modules[module_name]
        else:
            if not _is_bundled:
                # cli.py imports as _zed_user_memory.<name>.cli, usually
