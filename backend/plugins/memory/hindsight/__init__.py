...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                â€” memory bank identifier (default: zed)
  ...
  HINDSIGHT_TIMEOUT                â€” API request timeout in seconds (default: 120)
  HINDSIGHT_IDLE_TIMEOUT           â€” embedded daemon idle timeout seconds; 0 disables shutdown (default: 300)
  HINDSIGHT_RETAIN_TAGS            â€” comma-separated tags attached to retained memories
  HINDSIGHT_RETAIN_OBSERVATION_SCOPES â€” observation scoping for retained memories: per_tag/combined/all_combinations, or a JSON list of tag-lists for custom scopes
  HINDSIGHT_RETAIN_SOURCE          â€” metadata source value attached to retained memories
  HINDSIGHT_RETAIN_USER_PREFIX     â€” label used before user turns in retained transcripts
  HINDSIGHT_RETAIN_ASSISTANT_PREFIX â€” label used before assistant turns in retained transcripts

Or via $ZED_HOME/hindsight/config.json (profile-scoped), falling back to
...
_DEFAULT_TIMEOUT = 120  # seconds â€” cloud API can take 30-40s per request
_DEFAULT_IDLE_TIMEOUT = 300  # seconds â€” Hindsight embedded daemon default
# Mirrors hindsight-integrations/openclaw â€” Hindsight 0.5.0 added
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Zed can degrade gracefully instead of repeatedly trying to start
...
# ---------------------------------------------------------------------------
# Hindsight API capability probe â€” mirrors hindsight-integrations/openclaw.
# ---------------------------------------------------------------------------

# Cache of API_URL -> bool (whether that API supports update_mode='append').
# Probed once per URL per process â€” every provider talking to the same API
...
def _meets_minimum_version(actual: str | None, required: str) -> bool:
    """Return True if *actual* â‰¥ *required* (semver). False on missing/invalid."""
    if not actual:
        return False
    try:
...
def _fetch_hindsight_api_version(api_url: str, api_key: str | None = None,
    ...
    Any failure (timeout, 404, malformed JSON, missing key) â†’ None, which
...
def _check_api_supports_update_mode_append(api_url: str,
    ...
    Probes once per URL per process. Returns False on any probe failure â€”
    ...
    if not supported:
        logger.warning(
            ...
            "Falling back to per-process document_id â€” retains across "
...
# ---------------------------------------------------------------------------
# Backward-compatible alias â€” instances use self._run_sync() instead.
# ---------------------------------------------------------------------------
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $ZED_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_zed_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "zed": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "zed"),
...
def _normalize_observation_scopes(value: Any) -> Any:
    ...
    Returns one of:
      * ``None`` â€” nothing configured; Hindsight applies its ``combined`` default.
      * a keyword string â€” ``"per_tag"`` / ``"combined"`` / ``"all_combinations"``.
      * ``list[list[str]]`` â€” custom scopes, one inner list per consolidation pass.
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Zed config."""
    profile = config.get("profile", "zed")
    return str(profile or "zed")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   â€” active Zed profile name (from agent_identity)
      {workspace} â€” Zed workspace name (from agent_workspace)
      {platform}  â€” "cli", "telegram", "discord", etc.
      {user}      â€” platform user id (gateway sessions)
      {session}   â€” current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed â€” e.g. ``zed-{user}`` with no user becomes ``zed``.
    ...
    try:
        ...
        logger.warning("Invalid bank_id_template %r: %s â€” using fallback %r",
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # Legacy alias â€” older tests/callers reference _sync_thread directly.
        ...
        self._retain_context = "conversation between Zed Agent and the User"
        ...
        # consolidated knowledge layer â€” deduplicated, evidence-grounded
    ...
    def save_config(self, values, zed_home):
        """Write config to $ZED_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(zed_home) / "hindsight"
    ...
    def post_setup(self, zed_home: str, config: dict) -> None:
        ...
        from zed_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        provider_config.setdefault("bank_id", "zed")
        ...
        self.save_config(provider_config, zed_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(zed_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(zed_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "zed"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: zed-{profile}", "default": ""},
            ...
            {"key": "observation_scopes", "description": "How observations are scoped during consolidation: 'combined' (default â€” one pass over all tags), 'per_tag' (one isolated observation per tag), 'all_combinations' (every tag subset â€” expensive), or a JSON list of tag-lists for explicit custom scopes. Empty uses Hindsight's 'combined' default.", "default": ""},
            ...
            {"key": "recall_types", "description": "Fact types to surface on recall â€” applies to both auto-recall and the hindsight_recall tool (comma-separated or list). Defaults to observation-only â€” observations are Hindsight's consolidated, deduplicated, evidence-grounded knowledge layer; raw world/experience facts are the supporting evidence observations already summarize. Set to e.g. 'observation,world,experience' to also include raw facts.", "default": "observation"},
            {"key": "auto_recall", "description": "Automatically recall memories before each turn", "default": True},
            {"key": "auto_retain", "description": "Automatically retain conversation turns", "default": True},
            {"key": "retain_every_n_turns", "description": "Retain every N turns (1 = every turn)", "default": 1},
            {"key": "retain_async","description": "Process retain asynchronously on the Hindsight server", "default": True},
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Zed Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "zed"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "zed"),
    ...
    def _resolve_retain_target(self, fallback_document_id: str) -> tuple[str, str | None]:
        ...
        On Hindsight â‰¥ 0.5.0 the API supports ``update_mode='append'``,
        ...
        and don't pass ``update_mode`` at all â€” that's the only way the
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # alone caused overwrites on /resume â€” the reloaded session starts
        ...
        banks = cfg_get(self._config, "banks", "zed", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "zed")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Zed Agent and the User")
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        further sync_turn() calls are dropped â€” this prevents post-exit
        ...
        # accumulated since the last retain â€” the server appends them to the
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        ``_turn_index``) â€” even for /resume and /branch, the new session's
        ...
        moment of switch â€” the same data-loss class as the shutdown race,
        ...
        ``reset`` is accepted but not needed for Hindsight's state model â€”
        ...
        if self._session_turns:
            ...
            # â‰¥0.5.0: stable session-scoped + append).
            ...
            # shutdown has already fired â€” the writer is draining/gone.
    ...
    def shutdown(self) -> None:
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Zed created/used that client on the shared async loop,
        ...
        # HindsightMemoryProvider instance in the process â€” the plugin
