"""Honcho memory plugin â€” MemoryProvider for Honcho AI-native memory.
...
Config: Uses the existing Honcho config chain:
  1. $ZED_HOME/honcho.json (profile-scoped)
...
PROFILE_SCHEMA = {
    "name": "honcho_profile",
    "description": (
        "Retrieve or update a peer card from Honcho â€” a curated list of key facts "
        ...
        "fresh peer, dialectic layer still warming up, etc.) â€” this is NOT an "
...
SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "Returns raw excerpts ranked by relevance â€” no LLM synthesis. "
...
REASONING_SCHEMA = {
    ...
    "description": (
        ...
        "Uses Honcho's LLM (dialectic reasoning) â€” higher cost than honcho_profile or honcho_search. "
...
CONTEXT_SCHEMA = {
    "name": "honcho_context",
    "description": (
        "Retrieve full session context from Honcho â€” summary, peer representation, "
...
CONCLUDE_SCHEMA = {
    ...
    "description": (
        ...
        "Deletion is only for PII removal â€” Honcho self-heals incorrect conclusions over time."
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # B1: recall_mode â€” set during initialize from config
        self._recall_mode = "hybrid"  # "context", "tools", or "hybrid"

        # Base context cache â€” refreshed on context_cadence, not frozen
        ...
        # Port #4053: cron guard â€” when True, plugin is fully inactive
    ...
    def is_available(self) -> bool:
        ...
        try:
            ...
            # Port #2645: baseUrl-only verification â€” api_key OR base_url suffices
            return cfg.enabled and bool(cfg.api_key or cfg.base_url)
        except Exception:
            return False

    def save_config(self, values, zed_home):
        """Write config to $ZED_HOME/honcho.json (Honcho SDK native format)."""
        import json
        import os
        from pathlib import Path
        config_path = Path(zed_home) / "honcho.json"
        existing = {}
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if not cfg.enabled or not (cfg.api_key or cfg.base_url):
                logger.debug("Honcho not configured â€” plugin inactive")
                return
            ...
            try:
                ...
                # behave as they did before. New setups via `zed honcho setup`
            ...
            if self._recall_mode == "tools":
                if cfg.init_on_session_start:
                    ...
                    return
                logger.debug("Honcho tools-only mode â€” deferring session init until first tool call")
                return

            self._start_session_init_background(wait_timeout=0.1)

        except ImportError:
            logger.debug("honcho-ai package not installed â€” plugin inactive")
        except Exception as e:
    ...
    def _start_session_init_background(self, *, wait_timeout: float = 0.0) -> None:
        ...
        This keeps Zed CLI/gateway startup responsive when Honcho is down,
        ...
        with self._init_lock:
            ...
            init_session_id = self._lazy_init_session_id or "zed-default"
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        # Skip under per-session strategy: every Zed run creates a fresh
    ...
    def _ensure_session(self) -> bool:
        ...
        try:
            self._do_session_init(
                self._config,
                self._lazy_init_session_id or "zed-default",
    ...
    def _format_first_turn_context(self, ctx: dict) -> str:
        ...
        # Session summary â€” session-scoped context, placed first for relevance
    ...
    def system_prompt_block(self) -> str:
        ...
        Returns only the mode header and tool instructions â€” static text
        ...
        if self._recall_mode == "context":
            header = (
                ...
                "injected before each turn. No memory tools are available â€” context is "
            ...
            header = (
                ...
                "minimal/low/medium/high/max â€” you pick the depth per call), "
                "honcho_conclude to save facts about the user. "
                "No automatic context injection â€” you must use tools to access memory."
            )
        else:  # hybrid
            header = (
                "# Honcho Memory\n"
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        1. Base context from peer.context() â€” cached, refreshed on context_cadence
        2. Dialectic supplement â€” cached, refreshed on dialectic_cadence

        B1: Returns empty when recall_mode is "tools" (no injection).
        B5: Respects injection_frequency â€” "first-turn" returns cached/empty after turn 0.
        Port #3265: Truncates to context_tokens budget.
        """
        if self._cron_skipped:
            return ""

        # B1: tools-only mode â€” no auto-injection
        if self._recall_mode == "tools":
            return ""
        ...
        # B5: injection_frequency â€” if "first-turn" and past first turn, return empty.
        ...
        #
        # Skip if the session-start prewarm already filled _prefetch_result â€”
        ...
        if self._last_dialectic_turn == -999 and query:
            ...
            if self._prefetch_thread.is_alive():
                logger.debug(
                    "Honcho first-turn dialectic still running after %.1fs â€” "
        ...
        # cadence Ã— multiplier turns ago (e.g. a run of trivial-prompt turns
    ...
    def queue_prefetch(self, query: str, *, session_id: str = "") -> None:
        ...
        if self._cron_skipped:
            return
        # B1: tools-only mode â€” no prefetch
        if self._recall_mode == "tools":
            return
        ...
        # ----- Context refresh (base layer) â€” independent cadence -----
        ...
        # older than timeout Ã— multiplier is treated as dead so it can't
    ...
    # Index: (depth, pass) â†’ level relative to base.
    ...
    # Liveness constants. A thread older than timeout Ã— multiplier is treated
    # as dead so a hung Honcho call can't block future retries indefinitely.
    _STALE_THREAD_MULTIPLIER = 2.0
    # Pending result whose fire-turn is older than cadence Ã— multiplier is
    ...
    def _thread_is_live(self) -> bool:
        ...
        if age > timeout * self._STALE_THREAD_MULTIPLIER:
            logger.debug(
                ...
                "%.1fs â€” treating as dead", age, timeout * self._STALE_THREAD_MULTIPLIER,
            )
            return False
        return True

    def _effective_cadence(self) -> int:
        """Cadence plus empty-streak backoff, capped at _BACKOFF_MAX Ã— base."""
    ...
    def _resolve_pass_level(self, pass_idx: int, query: str = "") -> str:
        ...
        Precedence:
          1. dialecticDepthLevels (explicit per-pass) â€” wins absolutely
    ...
    def _run_dialectic_depth(self, query: str) -> str:
        ...
        Each pass is conditional â€” bails early if prior pass returned strong signal.
    ...
    # Prompts that carry no semantic signal â€” trivial acknowledgements, slash
    ...
    def _empty_profile_hint(self, peer: str) -> Dict[str, Any]:
        ...
        Ordered by likelihood for a typical deployment:
          ...
          2. Card hasn't accumulated yet (fresh peer, not enough dialectic
             cycles â€” dialectic cadence runs every N turns)
        ...
        if not reasons:
            reasons.append(
                "peer card has no facts yet â€” Honcho's dialectic layer builds "
