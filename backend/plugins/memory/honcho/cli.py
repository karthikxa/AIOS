...
Handles: zed honcho setup | status | sessions | map | peer
"""
...
def cmd_sync(args) -> None:
    ...
    Scans all Zed profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'zed honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'zed honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `zed update` -- no output, no exceptions.
    """
    try:
        from zed_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Zed profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $ZED_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_zed_home() / "honcho.json"
...
def _resolve_api_key(cfg: dict) -> str:
    ...
    config shapes, e.g. ``localhost:8000``) still pass â€” the Honcho SDK
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        zed_host.get("pinUserPeer"),
        zed_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in zed_host:
        aliases_src = zed_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in zed_host:
        prefix_src = zed_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(zed_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom zed_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    zed_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        zed_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(zed_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = zed_host.get("userPeerAliases")
    prefix = zed_host.get("runtimePeerPrefix")
    print("  resolved â†’")
    print(f"    pinUserPeer       = {bool(zed_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    pin_in = _prompt(
        "pinUserPeer â€” pin all gateway users to your peer? (true/false)",
        default=str(bool(current_pin)).lower(),
    ).strip().lower()
    pin = pin_in in {"true", "t", "yes", "y", "1"}
    _scrub_identity_mapping(zed_host)
    zed_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    print("  userPeerAliases â€” 'runtime_id=peer' pairs (blank line to finish):")
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        zed_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        zed_host, current_prefix, prefix_from_root,
        "runtimePeerPrefix â€” namespace for unknown IDs (blank for none)",
    )
...
def _prompt(label: str, default: str | None = None, secret: bool = False) -> str:
    ...
    if secret:
        if sys.stdin.isatty():
            ...
            # Non-TTY (piped input, test runners) â€” read plaintext
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "â”€" * 40)
    print("  Honcho gives Zed persistent cross-session memory.")
    ...
    if is_local:
        ...
        current_host_key = zed_host.get("apiKey", "")
        masked = (
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'zed honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = zed_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        zed_host["peerName"] = new_peer

    current_ai = zed_host.get("aiPeer") or cfg.get("aiPeer", "zed")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        zed_host["aiPeer"] = new_ai

    current_workspace = zed_host.get("workspace") or cfg.get("workspace", "zed")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        zed_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the Zed GATEWAY (Telegram/Discord/Slack/...),
    ...
    # moot off-gateway â€” gate it behind detection.
    #
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, zed_host)
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Zed gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
        ).strip().lower() in {"y", "yes"}
    elif not gw_platforms:
        print("\n  No gateway platforms connected â€” identity mapping only affects")
    ...
    if run_mapping:
        peer_target = zed_host.get("peerName") or current_peer or "user"
        default_choice = {"single": "1", "hybrid": "2", "multi": "3"}.get(current_shape, "3")
        print("\n  How should gateway users map to memory peers?")
        print("    [1] just me â€” every non-agent user collapses to your peer")
        print("    [2] me + other people â€” keep mine pooled, others separate")
        print("    [3] only other people â€” everyone gets their own peer")
        ...
        if current_pin and shape == "multi":
            print(
                f"\n  âš  Un-pinning will orphan memory accumulated under peer\n"
        ...
        if shape == "single":
            _scrub_identity_mapping(zed_host)
            zed_host["pinUserPeer"] = True
            print(f"  All non-agent gateway users route to '{peer_target}' (pin overrides aliases).")
            _echo_identity_mapping(zed_host)
        elif shape == "multi":
            # Preserve operator-curated host-level aliases across multi â†’ multi
            # re-runs.  Root-sourced aliases cascade naturally and are NOT
            # copied down â€” an empty host map would mask a root baseline.
            prior_aliases = (
                dict(current_aliases)
                if isinstance(current_aliases, dict) and not aliases_from_root
                else {}
            ...
            _scrub_identity_mapping(zed_host)
            zed_host["pinUserPeer"] = False
            if prior_aliases:
                zed_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                zed_host, current_prefix, prefix_from_root,
                "Runtime peer prefix (e.g. 'telegram_', blank for none)",
            )
            print("  Each gateway user â†’ own peer.")
            _echo_identity_mapping(zed_host)
        elif shape == "hybrid":
            ...
            if merged:
                zed_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                zed_host, current_prefix, prefix_from_root,
                "Runtime peer prefix for unknown users (e.g. 'telegram_', blank for none)",
            )
            print(f"  Your runtime IDs â†’ '{peer_target}', others â†’ own peer.")
            _echo_identity_mapping(zed_host)
        elif shape == "raw":
            _configure_raw_identity_mapping(
                zed_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = zed_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        zed_host["observationMode"] = new_obs
    else:
        zed_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(zed_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        zed_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        zed_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = zed_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        zed_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = zed_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        zed_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                zed_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(zed_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            zed_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        zed_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        zed_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        zed_host["dialecticReasoningLevel"] = new_reasoning
    else:
        zed_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = zed_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        zed_host["sessionStrategy"] = new_strat

    zed_host["enabled"] = True
    zed_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        zed_config = load_config()
        zed_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(zed_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: zed config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    zed honcho status     -- show full config")
    print("    zed honcho mode       -- change recall/observation mode")
    print("    zed honcho tokens     -- tune context and dialectic budgets")
    print("    zed honcho peer       -- update peer names")
    print("    zed honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Zed profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: zed honcho setup\n")
        return
    ...
    # Config paths â€” show where config was read from and where writes go
    ...
    if active_path == global_path:
        print(f"  Fallback:       (none â€” using global ~/.honcho/config.json)")
...
def cmd_sessions(args) -> None:
    """List known directory â†’ session name mappings."""
    cfg = _read_config()
    sessions = cfg.get("sessions", {})

    if not sessions:
        print("  No session mappings configured.\n")
        print("  Add one with: zed honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        zed = hosts.get(_host_key(), {})
        user = zed.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = zed.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = zed.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = zed.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "â”€" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Zed' identity in Honcho. Seed with 'zed honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: zed honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: zed honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = zed.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = zed.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = zed.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "â”€" * 40)
        print()
        ...
        print("    AI-to-AI inference. Zed asks Honcho's AI peer a question")
        ...
        print("\n  Set with: zed honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'zed honcho setup' first.\n")
        return
    ...
    if show:
        # â”€â”€ User peer â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ AI peer â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ai_rep = mgr.get_ai_representation(session_key)
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'zed honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    zed honcho identity --show        â€” show both peer representations")
        print("    zed honcho identity <file>        â€” seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory â†’ Zed + Honcho."""
    from pathlib import Path

    # â”€â”€ Detect OpenClaw native memory files â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    cwd = Path(os.getcwd())
    ...
    print("\nHoncho migration: OpenClaw native memory â†’ Zed\n" + "â”€" * 50)
    print()
    ...
    # â”€â”€ Step 1: Honcho account â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Zed persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  zed honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'zed honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'zed honcho setup' when ready, then re-run this walkthrough.")

    # â”€â”€ Step 2: Detected files â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    print()
    print("Step 2  Detected OpenClaw memory files")
    print()
    if user_files or agent_files:
        if user_files:
            print(f"  User memory ({len(user_files)} file(s)) â€” will go to Honcho user peer:")
            for f in user_files:
                print(f"    {f}")
        if agent_files:
            print(f"  Agent identity ({len(agent_files)} file(s)) â€” will go to Honcho AI peer:")
        ...
        print("  or seed them manually:  zed honcho identity <path/to/file>")

    # â”€â”€ Step 3: Migrate user memory â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    print()
    print("Step 3  Migrate user memory files â†’ Honcho user peer")
    print()
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'zed'")
        print("  with Honcho configured and no prior session history.")
        print("  (Zed calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    zed honcho migrate  â€” this step handles it interactively")
        if has_key:
            ...
            print("  Run 'zed honcho setup' first, then re-run this step.")
    else:
        print("  No user memory files detected. Nothing to migrate here.")

    # â”€â”€ Step 4: Seed AI identity â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    print()
    print("Step 4  Seed AI identity files â†’ Honcho AI peer")
    print()
    ...
    print()
    print("  In Zed, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'zed honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    zed honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  zed honcho identity <path/to/SOUL.md>")

    # â”€â”€ Step 5: What changes â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    print()
    ...
    print("    Zed:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Zed:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Zed:   Honcho observes every message and updates representations")
    ...
    print("    honcho_context   â€” session context: summary, representation, card, messages")
    print("    honcho_search        â€” semantic search over stored context")
    print("    honcho_profile       â€” fast peer card snapshot")
    print("    honcho_reasoning     â€” ask Honcho a question, synthesized answer")
    print("    honcho_conclude      â€” write a conclusion/fact back to memory")
    print()
    print("  Session naming")
    print("    OpenClaw: no persistent session concept â€” files are global.")
    print("    Zed:   per-session by default â€” each run gets its own session")
    print("              Map a custom name:  zed honcho map <session-name>")

    # â”€â”€ Step 6: Next steps â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    print()
    print("Step 6  Next steps")
    print()
    if not has_key:
        print("  1. zed honcho setup              â€” configure API key (required)")
        print("  2. zed honcho migrate            â€” re-run this walkthrough")
    else:
        print("  1. zed honcho status             â€” verify Honcho connection")
        print("  2. zed                           â€” start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. zed honcho identity --show    â€” verify AI peer representation")
        print("  4. zed honcho tokens             â€” tune context and dialectic budgets")
        print("  5. zed honcho mode               â€” view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        # Redirect to memory setup â€” honcho setup goes through the unified path
        print("\n  Honcho is configured via the memory provider system.")
        print("  Running 'zed memory setup'...\n")
        from zed_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``zed honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``zed honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to zed memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Zed Honcho",
    )
