...
Resolution order for config file:
  1. $ZED_HOME/honcho.json  (instance-local, enables isolated Zed instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Zed profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Zed profile.

    Resolution order:
      1. ZED_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``zed.<profile>``
      3. Fallback: ``"zed"`` (default profile)
    """
    explicit = os.environ.get("ZED_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $ZED_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.zed/honcho.json          (default profile â€” shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_zed_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config â€” host blocks accumulate here via setup/clone
    default_path = _get_default_zed_home() / "honcho.json"
...
def _normalize_recall_mode(val: str) -> str:
    """Normalize legacy recall mode values (e.g. 'auto' â†’ 'hybrid')."""
...
# Observation presets â€” granular booleans derived from legacy string mode.
...
class HonchoClientConfig:
    ...
    # identity (Telegram UID, Discord ID, â€¦) when resolving the user peer.
    # This keeps memory unified across platforms for single-user deployments
    # where Honcho's one peer-name is an unambiguous identity â€” otherwise
    ...
    # Max chars of dialectic result to inject into Zed system prompt
    ...
    # Honcho API limits â€” configurable for self-hosted instances
    ...
    # "hybrid"  â€” auto-injected context + Honcho tools available (model decides)
    # "context" â€” auto-injected context only, Honcho tools removed
    # "tools"   â€” Honcho tools only, no auto-injected context
    recall_mode: str = "hybrid"
    # Eager init in tools mode â€” when true, initializes session during
    ...
    # Per-peer observation booleans â€” maps 1:1 to Honcho's SessionPeerConfig.
    ...
    # True when Honcho was explicitly configured for this host (hosts.zed
    ...
    def from_global_config(
        ...
        Resolution: $ZED_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Zed profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.zed block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Zed session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy â€” Zed session_id ({timestamp}_{hex})
          5. per-repo strategy â€” git repo root directory name
          6. per-directory strategy â€” directory basename
          7. global strategy â€” workspace name
        """
        import re
        ...
        # per-session: inherit Zed session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'zed honcho setup' or set HONCHO_API_KEY. "
    ...
    # race the first call â€” the slot's double-checked lock serializes them and
    ...
    def _build() -> "Honcho":
        ...
        # the "go run zed honcho setup" hint they used to.
        try:
            from tools.lazy_deps import FeatureUnavailable, ensure as _lazy_ensure
            _lazy_ensure("memory.honcho", prompt=False)
        except ImportError:
            # lazy_deps module missing â€” fall through to the raw import below.
            pass
        except Exception:
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `zed honcho setup` to configure)."
            )
        ...
        # the two produces "/v3/v3/workspaces" â†’ 404 on every call.  This is a pure
        # routing concern independent of host, so strip a trailing version segment
        # from ANY base_url â€” loopback, LAN, custom domain, or cloud alike.  The
