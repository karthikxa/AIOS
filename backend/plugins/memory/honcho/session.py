...
class HonchoSessionManager:
    ...
    Runs alongside zed' existing SQLite state and file-based memory,
    ...
    def __init__(
        ...
        # Prefetch cache: session_key â†’ last context result (consumed once per turn).
        ...
        # Async write queue â€” started lazily on first enqueue
    ...
    def get_or_create(self, key: str) -> HonchoSession:
        ...
        # Determine peer IDs â€” no lock needed (read-only, no shared state mutation).
        ...
        assistant_peer_id = self._sanitize_id(
            self._config.ai_peer if self._config else "zed-assistant"
        )

        # All expensive I/O outside the lock â€” Honcho's persistence is source of truth
        ...
        # Write to cache under lock â€” only one writer wins
        with self._cache_lock:
    ...
    def save(self, session: HonchoSession) -> None:
        ...
        write_frequency modes:
          "async"   â€” enqueue for background thread (zero blocking, zero token cost)
          "turn"    â€” flush synchronously every turn
          "session" â€” defer until flush_session() is called explicitly
          N (int)   â€” flush every N turns
        """
    ...
    def dialectic_query(
        ...
        representation. Higher latency than context() â€” callers run this in
        ...
        Args:
            ...
            peer: Which peer to query â€” "user" (default) or "ai".
        ...
        try:
            if self._ai_observe_others:
                # AI peer can observe other peers â€” use assistant as observer.
            ...
            # Apply Zed-side char cap before caching
            if result and self._dialectic_max_chars and len(result) > self._dialectic_max_chars:
                result = result[:self._dialectic_max_chars].rsplit(" ", 1)[0] + " â€¦"
            return result
        except Exception as e:
    ...
    def get_prefetch_context(self, session_key: str, user_message: str | None = None) -> dict[str, str]:
        ...
        # Session summary â€” provides session-scoped context.
        # Fresh sessions (per-session cold start, or first-ever per-directory)
        # return null summary â€” the guard below handles that gracefully.
        ...
        # Also fetch AI peer's own representation so Zed knows itself.
        try:
    ...
    def migrate_memory_files(self, session_key: str, memory_dir: str) -> bool:
        ...
        Args:
            ...
            memory_dir: Path to the memories directory (~/.zed/memories/).
    ...
    def get_peer_card(self, session_key: str, peer: str = "user") -> list[str]:
        """
        Fetch a peer card â€” a curated list of key facts.
    ...
    def search_context(
        ...
        reasoning â€” cheaper and faster than dialectic_query. Good for
    ...
    def create_conclusion(self, session_key: str, content: str, peer: str = "user") -> bool:
        ...
        Conclusions are facts a peer observes about another peer or itself â€”
