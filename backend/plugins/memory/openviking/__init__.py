"""OpenViking memory plugin â€” full bidirectional MemoryProvider interface.
...
or a linked OpenViking CLI config:
  OPENVIKING_ENDPOINT  â€” Server URL (default: http://127.0.0.1:1933)
  OPENVIKING_API_KEY   â€” API key (required for authenticated servers)
  OPENVIKING_ACCOUNT   â€” Tenant account for local/trusted mode (default: default)
  OPENVIKING_USER      â€” Tenant user for local/trusted mode (default: default)
  OPENVIKING_AGENT     â€” Zed peer ID in OpenViking (default: zed)
...
_DEFAULT_AGENT = "zed"
_AGENT_PROMPT_LABEL = "Zed peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "ZED_OPENVIKING_SYNC_TRACE"
...
# for on_memory_write mirroring. User profile facts â†’ preferences; agent
# notes / observations â†’ patterns. Anything unknown falls back to the default.
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Zed slash-skill scaffolding before sending content to OpenViking.
    ...
    ``agent.skill_commands`` â€” no duplicated marker literals, no drift risk.
    """
...
# ---------------------------------------------------------------------------
# Process-level atexit safety net â€” ensures pending sessions are committed
...
# ---------------------------------------------------------------------------
# HTTP helper â€” uses httpx to avoid requiring the openviking SDK
# ---------------------------------------------------------------------------
...
READ_SCHEMA = {
    ...
    "description": (
        ...
        "  abstract â€” ~100 token summary (L0)\n"
        "  overview â€” ~2k token key points (L1)\n"
        "  full â€” complete content (L2)\n"
...
BROWSE_SCHEMA = {
    ...
    "description": (
        ...
        "  list â€” show directory contents\n"
        "  tree â€” show hierarchy\n"
        "  stat â€” show metadata for a URI"
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
# Recall tools (read-only) whose results we never re-ingest into OpenViking â€”
...
def _write_ovcli_config(path: Path, values: dict) -> None:
    ...
    # it into place â€” no half-written config on crash and no chmod-after-write
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory disabled for this Zed run."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Zed session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Zed only", "write values only to Zed .env"),
    ...
    _save_zed_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Zed .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See zed-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_zed_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Zed peer ID in OpenViking, sent as the actor peer and "
    ...
    def _finish_runtime_openviking_start(
        ...
        *,
        ...
        try:
            ...
            logger.warning("httpx not installed â€” OpenViking plugin disabled")
            return
        except Exception as e:
            _emit_runtime_warning(
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_zed_openviking_config())
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Zed run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _session_needs_commit(self, sid: str, turn_count: int) -> bool:
        ...
        # the turn counter â€” a racing sync_turn can re-increment _turn_count
        ...
        def _finalize() -> None:
            try:
                ...
                if not self._drain_writers(sid, timeout=_DEFERRED_COMMIT_TIMEOUT):
                    logger.warning(
                        "OpenViking writer for %s still alive after drain â€” "
    ...
    def _invalidate_prefetch_state(self) -> None:
        ...
        with self._prefetch_lock:
            ...
            self._prefetch_result = ""
            # Join EVERY tracked prefetch thread, not just the latest slot â€” a
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Zed' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Zed canonical messages into OpenViking batch payloads."""
    ...
    def on_session_end(self, messages: List[Dict[str, Any]]) -> None:
        ...
        if not self._drain_writers(sid, timeout=_SESSION_DRAIN_TIMEOUT):
            logger.warning(
                "OpenViking writer for %s still alive after drain â€” skipping commit",
                sid,
            )
            return
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See zed-agent#28296.
        ...
        # after (counted under new) â€” never split. The OLD session's commit
        ...
        # Invalidate stale prefetch OUTSIDE the session lock â€” it takes its own
        ...
        if not rotate:
            ...
            # counter reset â€” just the prefetch invalidation above.
            logger.debug(
    ...
    def _tool_search(self, args: dict) -> str:
        ...
        # Format results for the model â€” keep it concise
        scored_entries = []
    ...
    def _tool_read(self, args: dict) -> str:
        ...
        # (e.g. viking://user/zed/.overview.md).
    ...
    def _tool_remember(self, args: dict) -> str:
        ...
        # in a single call â€” no dependency on session commit / VLM extraction.
        try:
