"""ByteRover memory plugin â€” MemoryProvider interface.

Persistent memory via the ByteRover CLI (``brv``). Organizes knowledge into
a hierarchical context tree with tiered retrieval (fuzzy text â†’ LLM-driven
...
Config via environment variables (profile-scoped via each profile's .env):
  BRV_API_KEY   â€” ByteRover API key (for cloud features, optional for local)

Working directory: $ZED_HOME/byterover/ (profile-scoped context tree)
"""
...
# Timeouts
_QUERY_TIMEOUT = 10   # brv query â€” should be fast
_CURATE_TIMEOUT = 120  # brv curate â€” may involve LLM processing
...
CURATE_SCHEMA = {
    ...
    "description": (
        ...
        "patterns â€” anything worth remembering across sessions. ByteRover's LLM "
...
STATUS_SCHEMA = {
    "name": "brv_status",
    "description": "Check ByteRover status â€” CLI version, context tree stats, cloud sync state.",
