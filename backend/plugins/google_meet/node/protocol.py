"""Wire protocol for gateway â†” node RPC.
...
``zed meet node approve`` on the gateway and read off disk on the
...
def make_response(req_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    For simplicity we don't require the type here â€” clients usually just
