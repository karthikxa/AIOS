...
the zed-agent gateway. The gateway speaks a small JSON-over-WebSocket
RPC protocol to the remote node; the node wraps the existing
``plugins.google_meet.process_manager`` API.

Topology
--------
    gateway (Linux)  â”€â”€ ws://mac.local:18789 â”€â”€â–¶  node server (Mac)
                                                  â””â”€ process_manager
                                                     â””â”€ meet_bot (Playwright)
...
Public surface
--------------
    NodeClient     â€” gateway-side RPC client (short-lived sync WS per call)
    NodeServer     â€” long-running server that hosts the bot
    NodeRegistry   â€” local JSON registry of approved nodes (name â†’ url+token)
    protocol       â€” message envelope helpers (make_request, encode, decode, ...)
"""
