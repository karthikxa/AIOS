"""`zed meet node ...` subcommand tree.

Wired into the existing ``zed meet`` parser by the plugin's top-level
CLI. This module only defines the subparsers and their dispatch â€” it
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``zed meet node`` argparse object â€” typically
    ...
    run.add_argument("--display-name", default="zed-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    ...
    if cmd == "run":
        ...
        print(f"             zed meet node approve <name> ws://<host>:{args.port} {token}")
        try:
    ...
    if cmd == "list":
        ...
        if not nodes:
            ...
            return 0
        for n in nodes:
            print(f"{n['name']}\t{n['url']}\ttoken={n['token'][:6]}â€¦")
        return 0
    ...
    if cmd in {"status", "ping"}:
        ...
        try:
            result = client.ping()
        except Exception as exc:  # noqa: BLE001 â€” surface any connection error
