...
Launched by ``zed meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$ZED_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``zed meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_zed_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "zed-meet-node",
    ) -> None:
        self.host = host
        self.port = port
    ...
    def ensure_token(self) -> str:
        ...
        # Restrict to owner-read-write only â€” the token grants full RPC
    ...
    async def _handle_request(self, msg: Dict[str, Any]) -> Dict[str, Any]:
        ...
        does) rather than being re-encoded as error envelopes â€” the
        ...
        except Exception as exc:  # noqa: BLE001 â€” surface any pm crash to client
