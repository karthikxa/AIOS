...
Tools:
  meet_join        â€” join a Google Meet URL (spawns Playwright bot locally
                     OR on a remote node host via node=<name>)
  meet_status      â€” report bot liveness + transcript progress
  meet_transcript  â€” read the current transcript (optional last-N)
  meet_leave       â€” signal the bot to leave cleanly
  meet_say         â€” (v2) speak text through the realtime audio bridge.
...
def check_meet_requirements() -> bool:
    ...
    ``websockets`` dep on the gateway side â€” Chromium lives on the node.
...
def _resolve_node_client(node: Optional[str]):
    ...
    if entry is None:
        raise RuntimeError(
            f"no registered meet node matches {node!r} â€” "
            "run `zed meet node approve <name> <url> <token>` first"
        )
...
MEET_JOIN_SCHEMA: Dict[str, Any] = {
    ...
    "description": (
        ...
        "runs in parallel with the agent loop â€” returns immediately. Poll "
        ...
        "properties": {
            ...
            "node": {
                ...
                "description": (
                    ...
                    "approved via `zed meet node approve`."
                ),
            },
        },
        "required": ["url"],
...
MEET_STATUS_SCHEMA: Dict[str, Any] = {
    "name": "meet_status",
    "description": (
        "Report the current Meet session state â€” whether the bot is alive, "
...
MEET_LEAVE_SCHEMA: Dict[str, Any] = {
    ...
    "description": (
        ...
        "active â€” returns ok=false with a reason."
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
MEET_SAY_SCHEMA: Dict[str, Any] = {
    ...
    "description": (
        ...
        "immediately â€” the actual speech lags by a couple of seconds."
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
def handle_meet_join(args: Dict[str, Any], **_kw) -> str:
    ...
    if client is not None:
        # Remote path â€” delegate to the node host.
        try:
            res = client.start_bot(
                url=url,
                guest_name=str(args.get("guest_name") or "Zed Agent"),
    ...
    # Local path â€” same as v1, with v2 params.
    if not check_meet_requirements():
        return _err(
            "google_meet plugin prerequisites missing â€” install with "
