...
session-scoped state file under ``$ZED_HOME/workspace/meetings/.active.json``
so tool calls across turns can find the bot, and ``on_session_end`` can clean
it up.

The bot runs as a detached subprocess â€” we don't hold file descriptors open,
...
def _root() -> Path:
    return Path(get_zed_home()) / "workspace" / "meetings"
...
def _pid_alive(pid: int) -> bool:
    # ``os.kill(pid, 0)`` is NOT a no-op on Windows (bpo-14484) â€” it
...
# ---------------------------------------------------------------------------
# Public API â€” used by tool handlers + CLI
# ---------------------------------------------------------------------------
...
def start(
    ...
    *,
    ...
    If a bot is already running for this zed install, leave it first â€”
    ...
    env = os.environ.copy()
    env["ZED_MEET_URL"] = url
    env["ZED_MEET_OUT_DIR"] = str(out)
    env["ZED_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ZED_MEET_HEADED"] = "1"
    if auth_state:
        env["ZED_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ZED_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ZED_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ZED_MEET_MODE"] = mode
    if realtime_model:
        env["ZED_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ZED_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ZED_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["ZED_MEET_REALTIME_KEY"] = realtime_api_key

    log_path = out / "bot.log"
    # Detach: stdin=devnull, stdout/stderr â†’ log file, new session so parent
...
def enqueue_say(text: str) -> Dict[str, Any]:
    ...
    if active.get("mode") != "realtime":
        return {
            "ok": False,
            "reason": (
                "active meeting is in transcribe mode â€” pass mode='realtime' "
...
def stop(*, reason: str = "requested") -> Dict[str, Any]:
    ...
    if pid and _pid_alive(pid):
        ...
        if _pid_alive(pid):
            try:
                os.kill(pid, signal.SIGKILL)  # windows-footgun: ok â€” POSIX-only plugin (google_meet registers no-op on Windows; see __init__.py)
            except ProcessLookupError:
                pass
