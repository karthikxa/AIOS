"""Headless Google Meet bot â€” Playwright + live-caption scraping.
...
``$ZED_HOME/workspace/meetings/<meeting-id>/``. The main zed process
reads those files via the ``meet_*`` tools â€” no IPC beyond filesystem.
...
Run standalone for debugging::
    ...
    ZED_MEET_URL=https://meet.google.com/abc-defg-hij \\
    ZED_MEET_OUT_DIR=/tmp/meet-debug \\
    ZED_MEET_HEADED=1 \\
    python -m plugins.google_meet.meet_bot

No meet.google.com URL â†’ exits non-zero. Any URL that doesn't start with
...
# Match ``https://meet.google.com/abc-defg-hij`` or ``.../lookup/...`` â€” the
...
# Filenames the bot reads/writes in ``ZED_MEET_OUT_DIR``.
...
def _meeting_id_from_url(url: str) -> str:
    ...
    For ``https://meet.google.com/abc-defg-hij`` â†’ ``abc-defg-hij``.
    For ``.../lookup/<id>`` or ``/new`` we fall back to a timestamped id â€” the
...
class _BotState:
    ...
    def record_caption(self, speaker: str, text: str) -> None:
        ...
        # Atomic-ish append â€” good enough for a single-writer.
...
# and exposes ``window.__zedMeetDrain()`` to pull new entries. This
# mirrors the OpenUtter caption scraping approach.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__zedMeetInstalled) return;
  window.__zedMeetInstalled = true;
  window.__zedMeetQueue = [];
  ...
  // Try now and retry on interval â€” the caption region only appears after
  // captions are enabled and someone speaks.
  if (!attach()) {
    const iv = setInterval(() => { if (attach()) clearInterval(iv); }, 1500);
  }

  window.__zedMeetDrain = () => {
    const out = window.__zedMeetQueue.slice();
    window.__zedMeetQueue = [];
    return out;
  };
})();
...
def _enable_captions_js() -> str:
    ...
    Best-effort â€” Meet's caption toggle is keyboard-accessible via ``c``. We
...
def _start_realtime_speaker(
    *,
    ...
    # platform, but the contract is the same â€” block-read the growing
    # PCM file and stream it to the device in near-real-time.
    platform_tag = (bridge_info or {}).get("platform")
    if platform_tag == "linux":
        import subprocess as _sp

        sink = (bridge_info or {}).get("write_target") or "zed_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
            ...
            state.set(error="paplay not found â€” install pulseaudio-utils for realtime on Linux")
        ...
        # as the default input in System Settings â†’ Sound for Chrome to
        ...
        if _shutil.which("ffmpeg"):
            try:
                ...
                state.set(error="ffmpeg not found â€” install via `brew install ffmpeg` for realtime on macOS")
            except Exception as e:
...
def _mac_audio_device_index(device_name: str) -> str:
    ...
    Defaults to ``"0"`` if the device can't be found â€” caller will get a
...
def run_bot() -> int:  # noqa: C901 â€” orchestration, explicit branches
    url = os.environ.get("ZED_MEET_URL", "").strip()
    out_dir_env = os.environ.get("ZED_MEET_OUT_DIR", "").strip()
    headed = os.environ.get("ZED_MEET_HEADED", "").lower() in {"1", "true", "yes"}
    auth_state = os.environ.get("ZED_MEET_AUTH_STATE", "").strip()
    guest_name = os.environ.get("ZED_MEET_GUEST_NAME", "Zed Agent")
    duration_s = _parse_duration(os.environ.get("ZED_MEET_DURATION", ""))
    # v2: optional realtime mode. Enabled when ZED_MEET_MODE=realtime.
    mode = os.environ.get("ZED_MEET_MODE", "transcribe").strip().lower()
    realtime_model = os.environ.get("ZED_MEET_REALTIME_MODEL", "gpt-realtime")
    realtime_voice = os.environ.get("ZED_MEET_REALTIME_VOICE", "alloy")
    realtime_instructions = os.environ.get("ZED_MEET_REALTIME_INSTRUCTIONS", "")
    realtime_api_key = os.environ.get("ZED_MEET_REALTIME_KEY") or os.environ.get("OPENAI_API_KEY", "")

    if not url or not _is_safe_meet_url(url):
        sys.stderr.write(
            "google_meet bot: refusing to launch â€” ZED_MEET_URL must be a "
            "meet.google.com URL. got: %r\n" % url
        )
        return 2
    if not out_dir_env:
        sys.stderr.write("google_meet bot: ZED_MEET_OUT_DIR is required\n")
        return 2
    ...
    # SIGTERM â†’ exit cleanly so the parent ``meet_leave`` gets a finalized
    ...
    if rt["enabled"]:
        if not realtime_api_key:
            state.set(error="realtime mode requested but no API key in ZED_MEET_REALTIME_KEY/OPENAI_API_KEY â€” falling back to transcribe")
            rt["enabled"] = False
        else:
            try:
                ...
                state.set(error=f"audio bridge setup failed: {e} â€” falling back to transcribe")
                rt["enabled"] = False
    ...
    if not rt["enabled"]:
        # v1-style fake device (silence) â€” we don't care about mic content
    ...
    try:
        with sync_playwright() as pw:
            ...
            #   * detecting admission (Leave button visible â†’ in_call=True)
            ...
            lobby_deadline = time.time() + float(
                os.environ.get("ZED_MEET_LOBBY_TIMEOUT", "300")
            )
            ...
            while not stop_flag["stop"]:
                ...
                if not state.in_call and (now - last_admission_check) > 3.0:
                    ...
                    if admitted:
                        ...
                        state.set(
                            error=(
                                "lobby timeout â€” host never admitted the bot "
                ...
                try:
                    queued = page.evaluate("window.__zedMeetDrain && window.__zedMeetDrain()")
                    ...
                    # Meet reloaded or we got booted â€” try to detect and
            ...
            # Try to leave cleanly â€” click "Leave call" button if present.
            try:
                page.evaluate(
...
def _detect_admission(page) -> bool:
    ...
    Conservative by default â€” returns False on any error.
    """
    probe = r"""
    (() => {
      const leave = document.querySelector('button[aria-label*="eave call" i]');
      if (leave) return true;
      if (window.__zedMeetInstalled) {
...
def _detect_denied(page) -> bool:
    ...
    (() => {
      ...
      // English only â€” matches what shows up when the host denies or
      // removes a guest.
...
def _looks_like_human_speaker(speaker: str, bot_guest_name: str) -> bool:
    ...
    barge-in. Anything else â€” real participant names â€” does.
...
def _parse_duration(raw: str) -> Optional[float]:
    """Parse ``30m`` / ``2h`` / ``90`` (seconds) â†’ float seconds, or None."""
    if not raw:
        return None
    raw = raw.strip().lower()
    try:
...
if __name__ == "__main__":  # pragma: no cover â€” subprocess entry point
    sys.exit(run_bot())
