...
Easiest path â€” run the built-in installer:

```bash
zed plugins enable google_meet
zed meet install                 # pip deps + Chromium (transcribe only)
zed meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
zed meet auth                    # optional; skips guest-lobby wait
zed meet setup                   # preflight checks
```

`zed meet install --realtime` prompts before running `sudo apt-get` (Linux)
or `brew install` (macOS). Pass `--yes` to skip the prompt. It will NOT touch
your macOS default-input setting â€” you have to select BlackHole 2ch in
...
#           â†’ System Settings â†’ Sound â†’ Input â†’ BlackHole 2ch
#   Then set OPENAI_API_KEY or ZED_MEET_REALTIME_KEY in ~/.zed/.env
```
...
pip install playwright websockets && python -m playwright install chromium
zed plugins enable google_meet
zed meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
zed meet node approve my-mac ws://<mac-ip>:18789 <token>
zed meet node ping my-mac                   # confirm reachable
```

Run `zed meet setup` to preflight local prereqs.

## Flow

1. **Join** â€” call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** â€” no auto-consent. Say (in whatever channel the user is watching): "A Zed agent bot is in this call taking notes."
3. **Poll** â€” `meet_status()` for liveness, `meet_transcript(last=20)` for recent captions. Don't re-read the whole transcript every turn.
4. **Speak (realtime only)** â€” `meet_say(text="...")` queues text for TTS. The speech lags by ~2s. Don't spam it.
5. **Leave** â€” `meet_leave()` when done, or set `duration="30m"` on `meet_join` for auto-leave.
6. **Follow up** â€” read `meet_transcript()` in full, summarize, and use regular tools to send the recap, file issues, schedule followups.
...
- Guest mode sits in the lobby until a host admits. Warn the user; `zed meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `ZED_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `zed_meet_src`). |
...
| `error` | Last error (soft â€” bot may still be running). |

## Transcript location

Local:
```
$ZED_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$ZED_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `zed meet node approve`.
- `meet_say` text is rate-limited by the OpenAI Realtime session; spam-protection is the bot's problem, not yours, but still â€” don't queue hundreds of lines.
