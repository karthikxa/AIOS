...
Wires ``zed meet <subcommand>``:
  setup       â€” preflight playwright, chromium, auth file, print fixes
  auth        â€” open a browser to sign into Google, save storage state
  join <url>  â€” join a Meet URL synchronously (also callable from the agent)
  status      â€” print current bot state
  transcript  â€” print the transcript
  stop        â€” leave the current meeting
"""
...
def _auth_state_path() -> Path:
    return Path(get_zed_home()) / "workspace" / "meetings" / "auth.json"
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    """Build the ``zed meet`` argparse tree.
    ...
    join_p.add_argument("--guest-name", default="Zed Agent")
    ...
    try:
        ...
        def _node_unavailable(args):
            print(f"zed meet node: module unavailable ({e})")
            return 1
...
def meet_command(args: argparse.Namespace) -> int:
    ...
    if not sub:
        print("usage: zed meet {setup,auth,join,status,transcript,say,stop,node}")
        return 2
    if sub == "setup":
        return _cmd_setup()
    if sub == "install":
    ...
    if sub == "node":
        ...
        if fn is None or fn is meet_command:
            print("usage: zed meet node {run,list,approve,remove,status,ping}")
            return 2
        return fn(args)
...
def _cmd_setup() -> int:
    ...
    try:
        ...
        pw_ok = False
        pw_msg = "NOT installed â€” run: pip install playwright"
    ...
    print(
        "  google auth    : "
        + (f"ok ({auth_path})" if auth_ok else "not saved â€” run: zed meet auth")
    )
    ...
    if all_ok:
        print(
            ...
            "zed meet join https://meet.google.com/abc-defg-hij"
        )
    else:
        print("not ready yet â€” fix the items above.")
...
def _cmd_install(*, realtime: bool, assume_yes: bool) -> int:
    ...
    # 1) pip deps â€” always safe, venv-scoped.
    ...
    # 2) Playwright browsers â€” pulls chromium (~300MB first run).
        ...
        if system == "Linux":
            if _shutil.which("paplay") and _shutil.which("pactl"):
                ...
                if not _confirm(
                    ...
                    if res.returncode != 0:
                        print("  apt install failed â€” install pulseaudio-utils manually")
        elif system == "Darwin":
            have_bh = False
            try:
            ...
            if not needs:
                ...
                if not _confirm(f"  install via brew: {' '.join(needs)}?"):
                    ...
                    res = _sp.run(cmd, check=False)
                    if res.returncode != 0:
                        print("  brew install failed â€” install them manually")
            print(
                ...
                "  zed will not switch your default input for you."
            )
    else:
        print("\n[3/3] skipped (pass --realtime to install audio tooling too)")

    print("\ndone. verify with: zed meet setup")
    return 0
...
def _cmd_auth() -> int:
    ...
    print(f"opening Chromium â€” sign in to Google, then return here and press Enter.")
    ...
    try:
        ...
        return 1
    print("saved. you can now run: zed meet join <url>")
    return 0
...
def _cmd_stop() -> int:
    res = pm.stop(reason="zed meet stop")
    print(json.dumps(res, indent=2))
    return 0 if res.get("ok") else 1
