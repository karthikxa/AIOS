"""google_meet plugin â€” let the agent join a Meet call, transcribe it, follow up.
...
_TOOLS = (
    ("meet_join",       MEET_JOIN_SCHEMA,       handle_meet_join,       "ðŸ“ž"),
    ("meet_status",     MEET_STATUS_SCHEMA,     handle_meet_status,     "ðŸŸ¢"),
    ("meet_transcript", MEET_TRANSCRIPT_SCHEMA, handle_meet_transcript, "ðŸ“"),
    ("meet_leave",      MEET_LEAVE_SCHEMA,      handle_meet_leave,      "ðŸ‘‹"),
    ("meet_say",        MEET_SAY_SCHEMA,        handle_meet_say,        "ðŸ—£ï¸"),
)


def _on_session_end(**kwargs) -> None:
    """Best-effort cleanup â€” if a meet bot is still running when the session
    ends, leave the call so we don't orphan a headless Chromium.

    No-ops when nothing is active. Swallows all exceptions â€” session end must
...
def register(ctx) -> None:
    ...
    """
    # Windows is not supported in v1 â€” audio routing for v2 doesn't have a
    ...
    ctx.register_cli_command(
        ...
        description=(
            "Let the zed agent join a Google Meet call and scrape live "
            "captions into a transcript. See: zed meet setup"
        ),
    )
