"""Firecrawl cloud browser provider â€” plugin form.
...
This is the cloud-browser path â€” distinct from the firecrawl WEB plugin at
...
Config keys this provider responds to::
    ...
    browser:
      cloud_provider: "firecrawl"   # explicit selection only â€” not in the
...
class FirecrawlBrowserProvider(BrowserProvider):
    ...
    Cloud-browser path only â€” search/extract/crawl live in the separate
    ...
    def emergency_cleanup(self, session_id: str) -> None:
        if not self.is_available():
            logger.warning(
                "Cannot emergency-cleanup Firecrawl session %s â€” missing credentials",
                session_id,
            )
            return
        try:
