"""Browser Use cloud browser provider â€” plugin form.
...
``BROWSER_USE_API_KEY`` for self-billed users, or the managed Zed tool
gateway (which Zed uses to bill Browser Use sessions to a Zed
subscription). The dispatch order â€” direct API key first, managed gateway
second â€” preserves the pre-migration behaviour in
...
Auth env vars (one of)::
    ...
    # OR a managed Zed gateway entry (configured via 'zed setup')
"""
...
# Idempotency tracking for managed-mode session creation. The managed Zed
...
def _should_preserve_pending_create_key(response: requests.Response) -> bool:
    ...
    in progress") â€” in either case, retrying with the same key lets the
    gateway deduplicate.

    Drop the key on any other 4xx (auth failure, bad request, etc.) â€” those
...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    to the managed Zed Tool Gateway when ``tool_gateway.browser`` config
    ...
    # ------------------------------------------------------------------
    # Config resolution (direct API key OR managed Zed gateway)
    # ------------------------------------------------------------------

    def _get_config_or_none(self, *, refresh_token: bool = True) -> Optional[Dict[str, Any]]:
        # Import here to avoid a hard dependency at module-import time â€”
        # managed_tool_gateway pulls in the Zed auth stack which can be
        ...
        # managed Zed gateway via ``tool_gateway.browser: gateway``.
        ...
        # Keep availability scans off the synchroZed OAuth refresh path.
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        # default to a long Browser-Use timeout when Zed only needs a task-
        ...
        if managed_mode:
            _clear_pending_create_key(task_id)
        session_name = f"zed_{task_id}_{uuid.uuid4().hex[:8]}"
        external_call_id = (
    ...
    def close_session(self, session_id: str) -> bool:
        try:
            ...
            logger.warning(
                "Cannot close Browser Use session %s â€” missing credentials", session_id
            )
            return False
    ...
    def emergency_cleanup(self, session_id: str) -> None:
        ...
        if config is None:
            logger.warning(
                "Cannot emergency-cleanup Browser Use session %s â€” missing credentials",
                session_id,
            )
            return
        try:
