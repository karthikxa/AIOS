"""OpenAI image generation backend â€” ChatGPT/Codex OAuth variant.
...
Output is saved as PNG under ``$ZED_HOME/cache/images/``.
"""
...
# ---------------------------------------------------------------------------
# Model catalog â€” mirrors the ``openai`` plugin so the picker UX is identical.
# ---------------------------------------------------------------------------
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "badge": "free",
            "tag": "gpt-image-2 via ChatGPT/Codex OAuth â€” no API key required (text-to-image only)",
            "env_vars": [],
            "post_setup_hint": (
                "Sign in with `zed auth codex` (or `zed setup` â†’ Codex) "
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        # only here. Image-to-image / editing via Codex OAuth is not wired â€”
        ...
        if not _read_codex_access_token():
            return error_response(
                error=(
                    ...
                    "`zed auth codex` (or `zed setup` â†’ Codex) to sign in."
                ),
...
def register(ctx) -> None:
    """Plugin entry point â€” register the Codex-backed image-gen provider."""
