...
4. :data:`DEFAULT_MODEL` â€” ``gpt-image-2-medium``
"""
...
class OpenAIImageGenProvider(ImageGenProvider):
    """OpenAI ``images.generate`` / ``images.edit`` backend â€” gpt-image-2."""
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            "name": "OpenAI",
            "badge": "paid",
            "tag": "gpt-image-2 at low/medium/high quality tiers â€” text-to-image & image editing",
            "env_vars": [
                {
                    "key": "OPENAI_API_KEY",
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not os.environ.get("OPENAI_API_KEY"):
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `zed tools` â†’ Image "
                    "Generation â†’ OpenAI to configure, or `zed setup` "
                    "to add the key."
                ),
