...
Exposes Krea's `Krea 2` foundation image model family â€” Krea 2 Medium and
Krea 2 Large â€” as an :class:`ImageGenProvider` implementation.
...
4. :data:`DEFAULT_MODEL` â€” ``krea-2-medium`` (Krea's "start here" recommendation)
...
# Zed uses 3 abstract aspect ratios. Map to Krea's enum (which is wider).
...
# 402, 403, 404, other 4xx) is a permanent failure â€” surface it immediately
...
class KreaImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            "name": "Krea",
            "badge": "paid",
            "tag": "Krea 2 foundation model â€” Medium ($0.03) + Large ($0.06). Style transfer, moodboards, reference-guided generation.",
            "env_vars": [
                {
                    "key": "KREA_API_KEY",
                    "prompt": "Krea API key",
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        # transfer) via image_style_references â€” up to 10 refs.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        #   2. legacy image_style_references kwarg â€” may be plain URL strings OR
        ...
        if isinstance(legacy_refs, list):
            for ref in legacy_refs:
                if isinstance(ref, str):
                    ...
                    # Non-string ref object (dict, etc.) â€” pass through as-is.
                    style_refs.append(ref)
        ...
        if not api_key:
            return error_response(
                error=(
                    "KREA_API_KEY not set. Run `zed tools` â†’ Image "
                    "Generation â†’ Krea to configure, or get a key at "
        ...
        # Optional forward-compat passthroughs â€” the Krea API accepts these
        ...
        headers = {
            ...
            "User-Agent": "Zed-Agent/1.0 (krea-image-gen)",
        }
        ...
        while True:
            ...
            try:
                ...
                # Otherwise keep trying â€” transient 5xx (and a few retryable
            ...
            # over time â€” backlogged/scheduled/sampling â€” and we don't
        ...
        # Successful path â€” pull URL out of the result.
        ...
        # Materialise locally â€” Krea result URLs may expire, mirroring
