"""Kanban dashboard plugin â€” backend API routes.
...
``zed_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
(``web_server.auth_middleware``) just like core API routes â€” every
...
own pages inject it via ``window.__ZED_SESSION_TOKEN__`` so logged-in
...
the in-browser PTY bridge in ``zed_cli/web_server.py``.

This means ``zed dashboard --host 0.0.0.0`` is safe to run on a LAN:
plugin routes are no longer an unauthenticated exception. The auth still
isn't multi-user â€” anyone who can read the printed URL+token gets full
dashboard access â€” but they can't ride along just because they can reach
the port.
"""
...
# ---------------------------------------------------------------------------
# Auth helper â€” WebSocket only (HTTP routes live behind the dashboard's
# existing plugin-bypass; this is documented above).
# ---------------------------------------------------------------------------

def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    """Authorize a WebSocket upgrade by delegating to the dashboard's canonical
    WS auth gate (``zed_cli.web_server._ws_auth_ok``).
    ...
    """
    try:
        from zed_cli import web_server as _ws
    except Exception:
...
def _conn(board: Optional[str] = None):
    ...
    via the resolution chain (env var â†’ ``current`` file â†’ ``default``).
    """
    try:
...
# Hallucination-warning event kinds â€” see complete_task() in kanban_db.py.
...
def _compute_task_diagnostics(
    ...
    Uses ``zed_cli.kanban_diagnostics`` â€” see that module for the
    rule definitions.
    """
    from zed_cli import kanban_diagnostics as kd
    ...
    # slurp a lot â€” acceptable on the dashboard's typical working set
...
def _warnings_summary_from_diagnostics(
    ...
    â€” same shape additions plus ``highest_severity`` so the UI can color
    ...
    if not diagnostics:
        return None
    from zed_cli.kanban_diagnostics import SEVERITY_ORDER
...
def get_board(
    ...
    through to the active board (``ZED_KANBAN_BOARD`` env â†’ on-disk
    ``current`` pointer â†’ ``default``).
    """
    ...
    try:
        ...
        # One pass over task_links joined with child status â€” cheaper than
        ...
        # Diagnostics rollup for this board â€” see kanban_diagnostics.
        ...
        # preview here â€” the full text is available via /tasks/:id.
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from zed_cli.kanban import _check_dispatcher_presence
...
# ---------------------------------------------------------------------------
# Attachments â€” upload / list / download / delete (#35338)
# ---------------------------------------------------------------------------

# Cap a single upload so a runaway request can't fill the disk. 25 MB
# comfortably covers PDFs, images, and source docs â€” the kanban use case.
...
async def upload_task_attachment(
    ...
    try:
        ...
        # Resolve name collisions: foo.pdf â†’ foo (1).pdf, foo (2).pdf, â€¦
...
def download_attachment(attachment_id: int, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        # before serving â€” defense in depth against a tampered DB row.
...
class UpdateTaskBody(BaseModel):
    ...
    # Structured handoff fields â€” forwarded to complete_task when status
    # transitions to 'done'. Dashboard parity with ``zed kanban
...
def bulk_update(payload: BulkTaskBody, board: Optional[str] = Query(None)):
    ...
    This is an *independent* iteration â€” per-task failures don't abort
    ...
    try:
        for tid in ids:
            ...
            except Exception as e:  # defensive â€” one bad id shouldn't kill the batch
                entry.update(ok=False, error=str(e))
            results.append(entry)
...
# ---------------------------------------------------------------------------
# Diagnostics â€” fleet-wide distress signals (hallucinations, crashes,
# spawn failures, stuck-blocked). See zed_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def list_diagnostics(
    ...
    AND for ``zed kanban diagnostics`` which shells to this
...
# ---------------------------------------------------------------------------
# Worker visibility â€” cross-task active-worker list and per-run inspection
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Recovery actions â€” reclaim a running claim, reassign to a new profile
# ---------------------------------------------------------------------------
...
def reclaim_task_endpoint(
    ...
    ``zed kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
class SpecifyBody(BaseModel):
    ...
    dashboard â€” model + prompt come from ``auxiliary.triage_specifier``
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``zed kanban specify <task_id>``.

    Returns the outcome shape used by the CLI: ``{ok, task_id, reason,
    new_title}``. A non-OK outcome is NOT an HTTP error â€” the UI renders
    ...
    # ZED_KANBAN_BOARD env var â€” this endpoint runs in FastAPI's
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from zed_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``zed kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.zed/config.yaml.
...
# ---------------------------------------------------------------------------
#
# Home channels are a first-class gateway concept â€” each configured platform
...
#
# The wire format mirrors kanban_db.add_notify_sub â€” (task_id, platform,
# chat_id, thread_id) â€” so toggle-on creates exactly the same row the
...
def _configured_home_channels() -> list[dict]:
    ...
    # Stable order for deterministic UI â€” platform name alphabetical.
    result.sort(key=lambda r: r["platform"])
    return result


def _active_profile_name() -> str:
    """Return the current Zed profile name for notify-sub ownership."""
    try:
...
def get_home_channels(
    ...
    â€” useful for the "no task selected" state of the UI.
    """
...
def subscribe_home(task_id: str, platform: str, board: Optional[str] = Query(None)):
    ...
    Idempotent â€” re-subscribing is a no-op at the DB layer. 404 if the
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.zed/profiles/*`` on disk and every
...
def get_task_log(
    ...
    ``_rotate_worker_log`` â€” a single ``.log.1`` is kept, no further
...
def create_board_endpoint(payload: CreateBoardBody):
    """Create a new board. Idempotent â€” ``slug`` collision returns existing."""
    try:
...
def rename_board(slug: str, payload: RenameBoardBody):
    """Update a board's display metadata (slug is immutable â€” create a new one to rename the directory)."""
    try:
...
def switch_board(slug: str):
    ...
    Dashboard users pick boards via a client-side ``localStorage`` â€” this
...
def list_profile_roster():
    ...
    description still appear here â€” they're routable on name alone,
    just less precisely.
    """
    try:
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from zed_constants import get_zed_home  # type: ignore
            from pathlib import Path as _Path
            profile_dir = _Path(get_zed_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``zed profile describe <name> --auto``. Non-OK
    outcomes are NOT HTTP errors â€” the UI renders the reason inline
    (e.g. "no auxiliary client configured") so the operator can fix
    config and retry without a page reload.
    """
    try:
        from zed_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    1:1 to ``zed kanban decompose <task_id>``.

    Returns the outcome shape used by the CLI: ``{ok, task_id, reason,
    fanout, child_ids, new_title}``. A non-OK outcome is NOT an HTTP
    error â€” the UI renders the reason inline.
    ...
    # ZED_KANBAN_BOARD env var would let concurrent requests for
    # different boards race and cross-write (issue #38323).
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        from zed_cli import kanban_decompose  # noqa: WPS433 (intentional)
...
# auto_decompose) â€” surfaced to the dashboard's settings panel
# ---------------------------------------------------------------------------
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.zed/config.yaml.

    Each field is optional â€” only fields explicitly passed are
...
async def stream_events(ws: WebSocket):
    ...
    # string â€” the browser SDK's buildWsUrl() assembles it.
    ...
    try:
        ...
        # Board selection â€” pinned at the WS handshake; re-subscribe to
