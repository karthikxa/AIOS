#!/usr/bin/env python3
"""
SWE Runner with Zed Trajectory Format

A runner that uses Zed-Agent's built-in execution environments
(local, docker, modal) and outputs trajectories in the Zed-Agent format
compatible with batch_runner.py and trajectory_compressor.py.

Features:
- Uses Zed-Agent's Docker, Modal, or Local environments for command execution
- Outputs trajectories in Zed format (from/value pairs with <tool_call>/<tool_response> XML)
...
# ============================================================================
# Terminal Tool Definition (matches Zed-Agent format)
# ============================================================================
...
def create_environment(
    ...
    **kwargs
    ...
    """
    Create an execution environment using Zed-Agent's built-in backends.
...
# ============================================================================
# Mini-SWE Runner with Zed Trajectory Format
# ============================================================================

class MiniSWERunner:
    """
    Agent runner that uses Zed-Agent's built-in execution environments
    and outputs trajectories in Zed-Agent format.
    """
    ...
    def __init__(
        ...
        print("ðŸ¤– Mini-SWE Runner initialized")
    ...
    def _create_env(self):
        ...
        print(f"ðŸ”§ Creating {self.env_type} environment...")
    ...
    def _convert_to_zed_format(
        ...
        """
        Convert internal message format to Zed trajectory format.
...
Complete the user's task step by step."""
        ...
        try:
            while api_call_count < self.max_iterations:
                api_call_count += 1
                print(f"\nðŸ”„ API call #{api_call_count}/{self.max_iterations}")
                ...
                if assistant_message.content:
                    print(f"ðŸ¤– Assistant: {assistant_message.content[:100]}...")
                
                # Check for tool calls
                if assistant_message.tool_calls:
                    print(f"ðŸ”§ Tool calls: {len(assistant_message.tool_calls)}")
                    ...
                    for tc in assistant_message.tool_calls:
                        ...
                        print(f"   ðŸ“ž terminal: {command[:60]}...")
                        ...
                        print(f"   âœ… exit_code={result['exit_code']}, output={len(result['output'])} chars")
                    ...
                    completed = True
                    print("ðŸŽ‰ Agent finished (no more tool calls)")
                    break
        ...
        # Convert to Zed trajectory format
        trajectory = self._convert_to_zed_format(messages, task, completed)
    ...
    def run_batch(
        ...
        print(f"\nðŸ“¦ Running batch of {len(prompts)} tasks")
        print(f"ðŸ“ Output: {output_file}")
        
        with open(output_file, 'w', encoding='utf-8') as f:
            for i, prompt in enumerate(prompts, 1):
                print(f"\n{'='*60}")
                print(f"ðŸ“‹ Task {i}/{len(prompts)}")
                print(f"{'='*60}")
...
def main(
    ...
    """
    Run SWE tasks with Zed trajectory format output.
    ...
    """
    print("ðŸš€ Mini-SWE Runner with Zed Trajectory Format")
    print("=" * 60)
