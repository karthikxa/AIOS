...
Provides speech-to-text transcription with six providers:
  ...
  - **local** (default, free) â€” faster-whisper running locally, no API key needed.
    Auto-downloads the model (~150 MB for ``base``) on first use.
  - **groq** (free tier) â€” Groq Whisper API, requires ``GROQ_API_KEY``.
  - **openai** (paid) â€” OpenAI Whisper API, requires ``VOICE_TOOLS_OPENAI_KEY``.
  - **mistral** â€” Mistral Voxtral Transcribe API, requires ``MISTRAL_API_KEY``.
  - **xai** â€” xAI Grok STT API, requires ``XAI_API_KEY``. High accuracy,
    Inverse Text Normalization, diarization, 21 languages.
  - **elevenlabs** â€” ElevenLabs Scribe API, requires ``ELEVENLABS_API_KEY``.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``zed_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so STT does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from zed_cli.config import get_env_value as _get_env_value
    except ImportError:
...
LOCAL_STT_COMMAND_ENV = "ZED_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "ZED_LOCAL_STT_LANGUAGE"
...
# Singleton for the local model â€” loaded once, reused across calls
...
# Kept in sync with ``agent.transcription_registry._BUILTIN_NAMES`` â€”
...
# ---------------------------------------------------------------------------
#
# Mirrors the TTS command-provider registry shipped in PR #17843 â€” same
...
#      ``mistral``, ``xai``)              â†’ native handler. **Always wins.**
#   2. ``stt.providers.<name>: type: command``  â†’ command-provider runner.
#   3. Plugin-registered TranscriptionProvider  â†’ plugin dispatch.
#   4. No match                                 â†’ "No STT provider available".
#
# The single-env-var ``ZED_LOCAL_STT_COMMAND`` escape hatch is preserved
...
def _get_named_stt_provider_config(
    ...
    Built-in names are NOT special-cased here â€” the caller short-circuits
...
def _shell_quote_context_stt(command_template: str, position: int) -> Optional[str]:
    ...
    Mirrors ``tools.tts_tool._shell_quote_context`` â€” kept local to avoid
...
def _render_command_stt_template(
    ...
    def replace_match(match: "re.Match[str]") -> str:
        ...
        token = f"__ZED_STT_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _terminate_command_stt_process_tree(proc: subprocess.Popen) -> None:
    ...
    try:
        ...
        # psutil is optional â€” fall back to single-process terminate/kill
        proc.terminate()
        try:
            proc.wait(timeout=2)
...
def _read_command_stt_output(output_path: Path, stdout: str, fmt: str) -> str:
    ...
    Resolution:
      1. If ``output_path`` exists and is non-empty â†’ read it (raw text).
      2. Else if ``stdout`` is non-empty â†’ use stdout (lets users write
         curl-style one-liners that emit transcript to stdout instead of
         writing a file).
      3. Else â†’ raise RuntimeError (no usable output produced).

    For JSON format, we still return the raw bytes â€” extracting a
...
def _transcribe_command_stt(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"zed-cmd-stt-{provider_name}-") as tmpdir:
...
def _get_provider(stt_config: dict) -> str:
    ...
    honoured â€” no silent cloud fallback.  When no provider is configured,
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set ZED_LOCAL_STT_COMMAND)"
            )
            return "none"
        ...
        return provider  # Unknown â€” let it fail downstream
    ...
    if _HAS_OPENAI and _has_openai_audio_backend():
        ...
        return "openai"
    # Only auto-select Mistral if the SDK is already present â€” don't trigger a
...
# ---------------------------------------------------------------------------
# Plugin provider dispatch (issue follow-up to #30398 â€” STT pluggability)
# ---------------------------------------------------------------------------
...
def _dispatch_to_plugin_provider(
    ...
    *,
    ...
    1. Built-in provider names short-circuit â€” never reach the plugin
    ...
    4. Availability gating: when the matched plugin reports
       ...
       plugin as unavailable â€” **not** ``None`` â€” because the user
    ...
    error envelope (matches the legacy built-in error shapes â€” the
    ...
    try:
        ...
        if plugin_provider is None:
            ...
            plugin_provider = get_provider(key)
    except Exception as exc:  # noqa: BLE001 â€” discovery failure is non-fatal
    ...
    # ``stt.provider: <plugin>`` in config â€” surfacing the plugin's
    ...
    try:
        ...
        logger.warning(
            "STT plugin provider '%s' is_available() raised: %s â€” "
    ...
    if not available:
        ...
        return {
            ...
            "transcript": "",
            "error": (
                f"STT plugin '{key}' is not available â€” check that its "
...
# longer be used â€” we fall back to CPU and reload.
#
...
# memory" â€” those should surface to the user, not silently fall back to CPU
...
def _load_local_whisper_model(model_name: str):
    """Load faster-whisper with graceful CUDA â†’ CPU fallback.
    ...
    (``libcublas.so.12`` / ``libcudnn*``) isn't installed â€” common on WSL2
    ...
    try:
        ...
        if not _looks_like_cuda_lib_error(exc):
            raise
        logger.warning(
            "faster-whisper CUDA load failed (%s) â€” falling back to CPU (int8). "
    ...
    try:
        ...
        try:
            ...
            if not _looks_like_cuda_lib_error(exc):
                raise
            logger.warning(
                "faster-whisper CUDA runtime failed mid-transcribe (%s) â€” "
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="zed-local-stt-") as output_dir:
...
# ---------------------------------------------------------------------------
# Provider: groq (Whisper API â€” free tier)
# ---------------------------------------------------------------------------
...
def _transcribe_xai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    if not api_key:
        return {
            "success": False,
            "transcript": "",
            "error": "No xAI credentials found. Configure xAI OAuth in `zed model` or set XAI_API_KEY",
        }
    ...
    language = str(
        ...
        or os.getenv("ZED_LOCAL_STT_LANGUAGE")
        or DEFAULT_LOCAL_STT_LANGUAGE
    ).strip()
    ...
    try:
        import requests
        from tools.xai_http import zed_xai_user_agent
...
def transcribe_audio(file_path: str, model: Optional[str] = None) -> Dict[str, Any]:
    ...
    if provider == "xai":
        # xAI Grok STT doesn't use a model parameter â€” pass through for logging
    ...
    # elif chain â€” built-in names short-circuit upstream so a user's
    # ``stt.providers.openai.command`` can't override the real OpenAI
    # handler â€” and BEFORE the plugin dispatcher, because config is more
