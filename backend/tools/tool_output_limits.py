...
as tool-output truncation thresholds. Zed-agent had the same
...
# Hardcoded defaults â€” these match the pre-existing values, so adding
...
def _reset_tool_output_limits_cache() -> None:
    """Reset the cached limits â€” for tests or after config hot-reload."""
    global _cached_limits
    _cached_limits = None
