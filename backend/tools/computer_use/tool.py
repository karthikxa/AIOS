...
approach â€” the schema here is standard OpenAI function-calling so every
...
# Hard-blocked key combinations. Mirrored from #4562 â€” these are destructive
# regardless of approval level (e.g. logout kills the session Zed runs in).
_BLOCKED_KEY_COMBOS = {
...
_KEY_ALIASES = {"command": "cmd", "control": "ctrl", "alt": "option", "âŒ˜": "cmd", "âŒ¥": "option"}
...
# ---------------------------------------------------------------------------
# Backend selection â€” env-swappable for tests
# ---------------------------------------------------------------------------
...
def _get_backend() -> ComputerUseBackend:
    ...
    with _backend_lock:
        if _backend is None:
            backend_name = os.environ.get("ZED_COMPUTER_USE_BACKEND", "cua").lower()
            if backend_name in {"cua", "cua-driver", ""}:
                ...
                raise RuntimeError(f"Unknown ZED_COMPUTER_USE_BACKEND={backend_name!r}")
            _backend.start()
        return _backend


def reset_backend_for_tests() -> None:  # pragma: no cover
    """Test helper â€” tear down the cached backend."""
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    """Main entry point â€” dispatched by tools.registry.
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "Run `zed tools` and enable Computer Use to install cua-driver.",
        })
...
def _request_approval(action: str, args: Dict[str, Any]) -> Optional[str]:
    ...
    if cb is None:
        # No CLI approval wired â€” default allow. Gateway approval is handled
...
def _capture_response(cap: CaptureResult, max_elements: int = _DEFAULT_MAX_ELEMENTS) -> Any:
    ...
    # Index only what's actually surfaced in the response â€” otherwise the
    ...
    if cap.png_b64 and cap.mode != "ax" and not image_too_small:
        ...
        if _should_route_through_aux_vision():
            ...
            # call raised, etc.). Fall through to the multimodal envelope â€”
        ...
        # note would be inaccurate â€” skip it on this branch.
        return {
            "_multimodal": True,
            "content": [
...
def _should_route_through_aux_vision() -> bool:
    ...
    returned â€” fail open on the routing decision so a broken config can
...
def _route_capture_through_aux_vision(
    ...
    The captured base64 PNG is materialised to ``$ZED_HOME/cache/vision/``
    ...
    try:
        ...
        cache_dir = get_zed_dir("cache/vision", "temp_vision_images")
...
def _maybe_follow_capture(
    ...
    if isinstance(resp, dict) and resp.get("_multimodal"):
        prefix = f"[{res.action}] ok={res.ok}" + (f" â€” {res.message}" if res.message else "")
