...
can break on OS updates. Pin the installed version via `ZED_CUA_DRIVER_
...
PINNED_CUA_DRIVER_VERSION = os.environ.get("ZED_CUA_DRIVER_VERSION", "0.5.0")

_CUA_DRIVER_CMD = os.environ.get("ZED_CUA_DRIVER_CMD", "cua-driver")
...
def cua_driver_binary_available() -> bool:
    """True if `cua-driver` is on $PATH or ZED_CUA_DRIVER_CMD resolves."""
...
def cua_driver_install_hint() -> str:
    return (
        ...
        "Or run `zed tools` and enable the Computer Use toolset to install it automatically."
    )
...
# ---------------------------------------------------------------------------
# Asyncio bridge â€” one long-lived loop on a background thread
# ---------------------------------------------------------------------------
...
class _CuaDriverSession:
    ...
    def call_tool(self, name: str, args: Dict[str, Any], timeout: float = 30.0) -> Dict[str, Any]:
        ...
        try:
            ...
            # retry exactly one more time â€” never loop, to avoid hammering a
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    (MCP spec Â§2024-11-05+) and takes precedence for structured data like
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self) -> None:
        ...
        # Sticky context â€” updated by capture(), used by action tools.
    ...
    def capture(self, mode: str = "som", app: Optional[str] = None) -> CaptureResult:
        ...
        Maps zed `capture(mode, app)` â†’ cua-driver `list_windows` +
        ...
        # silently capturing the frontmost window â€” on macOS the `app_name`
        # returned by list_windows is the localized name (e.g. "è¨ˆç®—æ©Ÿ"), so
            ...
            if not filtered:
                return CaptureResult(
                    ...
                    window_title=(
                        ...
                        f"(macOS reports localized names, e.g. 'è¨ˆç®—æ©Ÿ' "
        ...
        if mode == "vision":
            ...
            # Parse element count from summary e.g. "âœ… AppName â€” 42 elements, turn 3..."
            m = re.search(r'(\d+)\s+elements?', summary)
            if tree and not gws_out["images"]:
                # ax mode â€” no screenshot
    ...
    # â”€â”€ Pointer â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    def click(
        self,
        *,
        ...
        if pid is None:
            return ActionResult(ok=False, action="click",
                                message="No active window â€” call capture() first.")
    ...
    # â”€â”€ Value setter â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def list_apps(self) -> List[Dict[str, Any]]:
        ...
        # list_apps returns plain text â€” parse app lines.
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        list_windows. We implement focus_app as a pure window-selector â€”
        ...
        # matches nothing â€” that hides the real failure (often a localized
        # macOS app name mismatch, e.g. caller passed "Calculator" but
        # list_windows returns "è¨ˆç®—æ©Ÿ").
