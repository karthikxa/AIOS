...
Copies bundled skills from the repo's skills/ directory into ~/.zed/skills/
...
The manifest lives at ~/.zed/skills/.bundled_manifest.
"""
...
from zed_constants import get_bundled_skills_dir, get_zed_home, get_optional_skills_dir
...
SKILLS_DIR = ZED_HOME / "skills"
MANIFEST_FILE = SKILLS_DIR / ".bundled_manifest"

# Marker file written by `zed profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.zed profile).
# When present in ZED_HOME, sync_skills() is a no-op so neither the
# installer, `zed update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# zed_cli.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _get_bundled_dir() -> Path:
    ...
    Checks ZED_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    """Built-in skills the curator pruned â€” must NOT be re-seeded on sync.

    Delegates to ``tools.skill_usage`` (single source of truth) and falls back
    to reading ``~/.zed/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.zed/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.zed/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.zed) that wrote the
    ...
    if (ZED_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
            print("  (skipped â€” profile opted out of bundled skills via .no-bundled-skills)")
        return {
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.zed/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `zed update` would resurrect a skill the user
        ...
        # dest is gone â€” without this, the "in manifest but not on disk"
        ...
        if skill_name not in manifest:
            # â”€â”€ New skill â€” never offered before â”€â”€
            try:
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            f"  âš  {skill_name}: bundled version shipped but you "
                            f"already have a local skill by this name â€” yours "
                            f"was kept. Run `zed skills reset {skill_name}` "
            ...
            # â”€â”€ Existing skill â€” in manifest AND on disk â”€â”€
            ...
            if bundled_hash != origin_hash:
                try:
                    ...
                    # dest is the authoritative copy â€” clear the leftover.
                    if backup.exists():
                    ...
                    try:
                        ...
                        # not shadow the user's copy or block the restore â€”
            ...
            # â”€â”€ In manifest but not on disk â€” user deleted it â”€â”€
            skipped += 1
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``ZED_HOME/skills/`` to prevent the catastrophic wipe of
    # ``~/.zed/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    # skills, scripts, â€¦) that an earlier incident observed. Five call
    # sites in this file invoke this helper; if any one of them ever
    # computes a destination outside the skills root â€” through a bad
    # path join, a missing ``ZED_HOME`` default, a malicious
    # bundled-manifest entry, or a mid-flight exception that leaves a
    # stale path in scope â€” this guard turns the resulting
    # ``shutil.rmtree(~/.zed)`` into a loud, recoverable ``ValueError``
    # instead of silently destroying the user's install.
    target = Path(path).resolve()
    skills_root = SKILLS_DIR.resolve()
    # Every legitimate caller passes a skill directory or its ``.bak``
    # sibling â€” always a strict child of the skills root. The skills root
    ...
    # ``ZED_HOME``. Require a strict-child relationship so both escape
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    ``user_modified`` and skip it forever â€” even if the user later copies
    ...
    Args:
        ...
        restore: If True, also delete the user's copy in SKILLS_DIR and let
                 ...
                 (default), only clear the manifest entry â€” the user's
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `zed skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore:
        if not is_bundled:
            return {
                ...
                "message": (
                    f"'{name}' has no bundled source â€” manifest entry preserved "
            ...
            try:
                ...
                return {
                    ...
                    "message": (
                        ...
                        f"Manifest entry preserved â€” nothing was changed."
                    ),
                    "synced": None,
                }
    ...
    if restore and deleted_user_copy:
        ...
        # Nothing on disk to delete, but we re-synced â€” acts like a fresh install
        action = "restored"
        message = f"Restored '{name}' (no prior user copy, re-copied from bundled)."
    else:
        action = "manifest_cleared"
        message = (
            f"Cleared manifest entry for '{name}'. Future `zed update` runs "
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``zed update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``zed update`` keeps because the user
    edited them locally.

    A skill counts as user-modified when its on-disk copy no longer matches the
    origin hash recorded in the manifest the last time it was synced â€” the exact
    ...
    for skill_name, skill_dir in _discover_bundled_skills(bundled_dir):
        ...
        # modification â€” the next sync handles it.
        if not origin_hash:
            continue
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``zed skills reset`` back to upstream.

    Returns a dict:
        ``ok`` (bool), ``name`` (str), ``found`` (bool â€” bundled source exists),
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `zed skills inspect`."
            ),
        }
    ...
    for rel in sorted(user_files | stock_files):
        ...
        if in_user and in_stock:
            if user_text is None or stock_text is None:
                # At least one side is binary â€” report only if bytes differ
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes ZED_HOME/.no-bundled-skills so the
    installer, ``zed update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``zed skills opt-out`` /
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = ZED_HOME / NO_BUNDLED_SKILLS_MARKER
    ...
    try:
        if enabled:
            ...
            message = (
                ...
                if changed
                else "Already opted out â€” marker was already present."
            )
        else:
            if existed:
                marker.unlink()
            changed = existed
            message = (
                "Opted back in. The next `zed update` (or `zed skills "
                "opt-in --sync`) will re-seed bundled skills."
                if changed
                else "Not opted out â€” no marker to remove."
            )
    except OSError as e:
        return {
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (ZED_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
...
def remove_pristine_bundled_skills(dry_run: bool = False) -> dict:
    ...
    for name, origin_hash in sorted(manifest.items()):
        ...
        if src is None:
            # Tracked but no longer bundled upstream â€” leave it; not ours to judge.
        ...
        if on_disk != origin_hash:
            ...
            continue
        # Pristine bundled copy â€” safe to remove.
        if dry_run:
            removed.append(name)
            continue
        try:
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.zed/skills/ ...")
