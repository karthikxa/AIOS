...
~/.zed/skills/. Existing skills (bundled, hub-installed, or user-created)
...
# Import security scanner â€” external hub installs always get scanned;
...
def _security_scan_skill(skill_dir: Path) -> Optional[str]:
    ...
    try:
        ...
        if allowed is None:
            # "ask" verdict â€” for agent-created skills this means dangerous
...
# All skills live in ~/.zed/skills/ (single source of truth)
...
def _containing_skills_root(skill_path: Path) -> Path:
    ...
    match is found (defensive â€” callers should have located the skill via
    ``_find_skill`` first).
    """
...
def _pinned_guard(name: str) -> Optional[str]:
    ...
    Pin protects a skill from **deletion** â€” both the curator's auto-archive
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`zed curator unpin {name}` if they want to delete it. "
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.zed/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Zed profiles.
    ...
    when profile discovery fails â€” fail-quiet, the caller falls back to
    ...
    # Default profile (~/.zed/skills) â€” only consider when active is non-default.
    ...
    # All named profiles (~/.zed/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`zed -p "
            ...
            base += (
                ...
                f"Switch profiles (`zed -p <name>`) to edit there, or "
...
def _validate_file_path(file_path: str) -> Optional[str]:
    ...
    # under an allowed subdirectory. Accept its two natural spellings â€”
    # 'SKILL.md' and '<skill-name>/SKILL.md' â€” so callers can target the main
...
def _patch_skill(
    ...
    result["_change"] = {
        "old": old_string[:200] + ("â€¦" if len(old_string) > 200 else ""),
        "new": new_string[:200] + ("â€¦" if len(new_string) > 200 else ""),
    }
    return result
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``absorbed_into`` declares intent:
      - ``None`` / missing  â†’ caller didn't declare (legacy / non-curator path);
      ...
      - ``""`` (empty)      â†’ explicit "truly pruned, no forwarding target".
      - ``"<skill-name>"``  â†’ content was absorbed into that umbrella; the
...
def _apply_skill_write_gate(action, name, **payload_kwargs):
    ...
    # stage â€” record the full skill_manage kwargs so approval can replay it.
...
def skill_manage(
    ...
    if result.get("success"):
        ...
        # review fork creates it â€” foreground `skill_manage(create)` calls are
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_zed_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "exist â€” create/patch the umbrella first, then delete.\n\n"
        ...
        "Pinned skills are protected from deletion only â€” skill_manage(action='delete') "
        "will refuse with a message pointing the user to `zed curator unpin <name>`. "
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "absorbed_into": {
                "type": "string",
                "description": (
                    "For 'delete' only â€” declares intent so the curator can "
