...
* ``XAI_API_KEY`` is set in ``~/.zed/.env`` or the process environment
  (paid xAI API key), OR
* The user is signed in via xAI Grok OAuth â€” SuperGrok subscription â€”
  i.e. ``zed auth add xai-oauth`` has been run and the stored refresh
  token still works.
...
from tools.xai_http import zed_xai_user_agent, resolve_xai_http_credentials
...
def _resolve_xai_bearer() -> Tuple[str, str, str]:
    ...
    credential is available â€” the registered :func:`check_x_search_requirements`
    ...
    if not api_key:
        raise RuntimeError(
            "No xAI credentials available. Run `zed auth add xai-oauth` "
...
def _validate_date_range(from_date: str, to_date: str) -> None:
    ...
    Rules:
      ...
      * ``from_date`` must not be later than today UTC â€” no posts can exist
