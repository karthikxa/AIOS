...
supports multiple backends â€” **Browser Use** (cloud, default for Zed
subscribers), **Browserbase** (cloud, direct credentials), and **local
Chromium** â€” with identical agent-facing behaviour.  The backend is
...
from zed_cli.config import DEFAULT_CONFIG, cfg_get

try:
    from tools.website_policy import check_website_access
except Exception:
    check_website_access = lambda url: None  # noqa: E731 â€” fail-open if policy module unavailable
...
try:
    ...
    _is_safe_url = lambda url: False  # noqa: E731 â€” fail-closed: block all if safety module unavailable
    _is_always_blocked_url = lambda url: True  # noqa: E731 â€” fail-closed on the floor too
    _normalize_url_for_request = lambda url: url  # noqa: E731 â€” best-effort fallback
# Browser-provider ABC + registry â€” PR #25214 moved the per-vendor providers
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    zed_node_bin = str(zed_home / "node" / "bin")
    zed_node_root = str(zed_home / "node")
    zed_nm_bin = str(zed_home / "node_modules" / ".bin")
    return [zed_node_bin, zed_node_root, zed_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _get_vision_model() -> Optional[str]:
    """Model for browser_vision (screenshot analysis â€” multimodal)."""
    return os.getenv("AUXILIARY_VISION_MODEL", "").strip() or None


def _get_extraction_model() -> Optional[str]:
    """Model for page snapshot text summarization â€” same as web_extract."""
...
def _ensure_cdp_supervisor(task_id: str) -> None:
    ...
    Idempotent â€” delegates to ``SupervisorRegistry.get_or_start`` which skips
    ...
    Resolves the CDP URL in this order:
      1. ``BROWSER_CDP_URL`` / ``browser.cdp_url`` â€” covers ``/browser connect``
         and config-set overrides.
      2. ``_active_sessions[task_id]["cdp_url"]`` â€” covers Browserbase + any
         other cloud provider whose ``create_session`` returns a raw CDP URL.

    Swallows all errors â€” failing to attach the supervisor must not break
...
# keep working â€” but ``_get_cloud_provider()`` now consults
...
# plugins drop in under ``~/.zed/plugins/browser/<vendor>/``.
...
# Lightpanda engine support â€” cached like _get_cloud_provider().
...
def _is_legacy_provider_registry_overridden() -> bool:
    ...
    extend ``_DEFAULT_PROVIDER_REGISTRY`` below â€” they do NOT need to update
    ...
    try:
        for key, default_cls in _DEFAULT_PROVIDER_REGISTRY.items():
            if _PROVIDER_REGISTRY.get(key) is not default_cls:
                return True
        # Extra keys not in the default registry â†’ also an override.
...
def _ensure_browser_plugins_loaded() -> None:
    ...
    can be called from contexts that haven't gone through `model_tools` â€”
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    fallback. If unset, fall back to Browser Use (managed Zed gateway or
    direct API key) and then Browserbase (direct credentials only) â€” the
        ...
        if provider_key:
            try:
                if _is_legacy_provider_registry_overridden():
                    ...
                    # populated. Idempotent â€” cheap on subsequent calls.
                    _ensure_browser_plugins_loaded()
                    resolved = _registry_get_browser_provider(provider_key)
                    if resolved is None:
                        # Explicit config name unknown to the registry â€”
    ...
    if resolved is None:
        # Auto-detect path: Browser Use first (managed Zed gateway or
        ...
        # plugins are intentionally NOT reachable from auto-detect â€” they
    ...
    if resolved is None:
        # Transient None â€” credentials may self-heal. Don't poison the cache.
        return None
...
def _is_local_backend() -> bool:
    ...
    machine.  For local backends â€” Camofox, or the built-in headless
    Chromium without a cloud provider â€” the user already has full terminal
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Zed silently
...
def _annotate_lightpanda_fallback(result: Dict[str, Any], reason: str) -> Dict[str, Any]:
    ...
    warning = (
        "âš  Lightpanda fallback: Chrome was used for this browser action. "
        f"{reason}"
    )
...
def _run_chrome_fallback_command(
    ...
    # Lightpandaâ†’Chrome fallback if the eval call itself fails.
    ...
    if not _chromium_installed():
        if _running_in_docker():
            hint = (
                ...
                "You're running in Docker â€” pull the latest image: "
    ...
    # On Windows npx is npx.cmd â€” use shutil.which so CreateProcessW can
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            # * STARTF_USESTDHANDLES + explicit handles â†’ CreateProcess hands
            #   the child ONLY our three chosen handles (DEVNULL stdin +
            #   temp-file stdout/stderr). Without this, some parents leak
            #   console handles that break downstream grandchild spawns â€” the
            ...
            #   in the Zed CLI where sys.stdout and sys.stderr both report
            #   fileno=1 (stderr dup'd onto stdout at the OS level).
            # * close_fds=True â†’ block inheritance of every other handle.
            #   (Default on POSIX; must be explicit on Windows for stdio.)
            _popen_extra: dict = {}
            if os.name == "nt":
                # CREATE_NO_WINDOW â†’ don't attach a console (cmd.exe would
                ...
                # unwound MainThread's prompt_toolkit loop that way â€” see
                # diag log: "asyncio.CancelledError â†’ KeyboardInterrupt".
...
def _url_is_private(url: str) -> bool:
    ...
    Reuses ``tools.url_safety.is_safe_url`` as the oracle â€” if the SSRF check
    ...
    try:
        ...
        try:
            ...
            return (
                ...
                # â‰¥3.11 (bpo-40791).  Explicit check keeps 3.10 runtimes
            ...
            pass
        # Hostname â€” must resolve to confirm it's private (bare "localhost"
        ...
        try:
            ...
            return False  # DNS fail â†’ not private, let the normal path fail
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-zed_â€¦`` the resulting socket path exceeds the
...
# cleanup_browser code paths â€” the key is opaque to those internals.
#
...
def _emergency_cleanup_all_sessions():
    ...
    crashed zed processes â€” this way every clean zed exit sweeps
    ...
    # Sweep orphans from other crashed zed processes.  Safe even if we
    # never used the browser â€” uses owner_pid liveness to avoid reaping
    # daemons owned by other live zed processes.
    try:
...
# async event loop â€” a SystemExit raised inside a key-binding callback
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current zed PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live zed process
    (don't reap) from daemons whose owner crashed (reap).  Best-effort â€”
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning zed process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) â€” if the
         referenced zed PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent zed instances won't reap each
         other's daemons.
    ...
    Safe to call from any context â€” atexit, cleanup thread, or on demand.
    """
    import glob
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-zed_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if os.path.isfile(owner_pid_file):
            try:
                ...
                owner_alive = None  # corrupt file â€” fall through

        if owner_alive is True:
            # Owner is alive â€” this session belongs to a live zed process.
            continue
        ...
        if not os.path.isfile(pid_file):
            # No daemon PID file â€” just a stale dir, remove it
        ...
        # is NOT a no-op â€” use the handle-based existence check.
        from gateway.status import _pid_exists
...
BROWSER_TOOL_SCHEMAS = [
    {
        ...
        "description": "Navigate to a URL in the browser. Initializes the session and loads the page. Must be called before other browser tools. For simple information retrieval, prefer web_search or web_extract (faster, cheaper). For plain-text endpoints â€” URLs ending in .md, .txt, .json, .yaml, .yml, .csv, .xml, raw.githubusercontent.com, or any documented API endpoint â€” prefer curl via the terminal tool or web_extract; the browser stack is overkill and much slower for these. Use browser tools when you need to interact with a page (click, fill forms, dynamic content). Returns a compact page snapshot with interactive elements and ref IDs â€” no need to call browser_snapshot separately after navigating.",
        "parameters": {
            "type": "object",
            "properties": {
                "url": {
    ...
    },
    {
        ...
        "description": "Get a text-based snapshot of the current page's accessibility tree. Returns interactive elements with ref IDs (like @e1, @e2) for browser_click and browser_type. full=false (default): compact view with interactive elements. full=true: complete page content. Snapshots over 8000 chars are truncated or LLM-summarized. Requires browser_navigate first. Note: browser_navigate already returns a compact snapshot â€” use this to refresh after interactions that change the page, or with full=true for complete content.",
        "parameters": {
            "type": "object",
            "properties": {
                "full": {
    ...
    },
    {
        ...
        "description": "Get browser console output and JavaScript errors from the current page. Returns console.log/warn/error/info messages and uncaught JS exceptions. Use this to detect silent JavaScript errors, failed API calls, and application warnings. Requires browser_navigate to be called first. When 'expression' is provided, evaluates JavaScript in the page context and returns the result â€” use this for DOM inspection, reading page state, or extracting data programmatically.",
        "parameters": {
            "type": "object",
            "properties": {
                "clear": {
                ...
                "expression": {
                    "type": "string",
                    "description": "JavaScript expression to evaluate in the page context. Runs in the browser like DevTools console â€” full access to DOM, window, document. Return values are serialized to JSON. Example: 'document.title' or 'document.querySelectorAll(\"a\").length'"
                }
            },
            "required": []
        }
...
def _create_cdp_session(task_id: str, cdp_url: str) -> Dict[str, str]:
    ...
    logger.info("Created CDP browser session %s â†’ %s for task %s",
...
def _get_session_info(task_id: Optional[str] = None) -> Dict[str, str]:
    ...
    Args:
        task_id: Session key.  Normally the task_id as-is, but may carry the
            ``::local`` suffix for the hybrid-routing local sidecar â€” in that
    ...
    # Skip for local sidecars â€” they have no CDP URL.
    if not force_local:
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Zed-managed
    ...
    # Build an extended search PATH including Zed-managed Node, macOS
    ...
    # subprocess on Windows) cannot execute the extensionless shim â€” it raises
    ...
    # Nothing found â€” try lazy installation before giving up.
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                zed_nm = str(get_zed_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=zed_nm)
            if not recheck:
                zed_node_bin = str(get_zed_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=zed_node_bin)
            if not recheck:
                zed_node_root = str(get_zed_home() / "node")
                recheck = shutil.which("agent-browser", path=zed_node_root)
            if recheck:
...
def _run_browser_command(
    ...
    # Skip when engine=lightpanda â€” LP doesn't need Chromium for navigation.
    if _is_local_mode() and not _chromium_installed() and _get_browser_engine() != "lightpanda":
        if _running_in_docker():
            hint = (
                "Chromium browser is missing. You're running in Docker â€” pull "
    ...
    if session_info.get("cdp_url"):
        # Cloud mode â€” connect to remote Browserbase browser via CDP
        # IMPORTANT: Do NOT use --session with --cdp. In agent-browser >=0.13,
        # --session creates a local browser instance and silently ignores --cdp.
        backend_args = ["--cdp", session_info["cdp_url"]]
    else:
        # Local mode â€” launch a headless Chromium instance
    ...
    # shutil.which resolves npx â†’ npx.cmd on Windows; bare "npx" stays on POSIX.
    if browser_cmd == "npx agent-browser":
        _npx_bin = shutil.which("npx") or "npx"
        cmd_prefix = [_npx_bin, "agent-browser"]
    else:
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this zed PID as the session owner (cross-process safe
        # orphan detection â€” see _write_owner_pid).
        ...
        # â€” the daemon kills itself and its Chrome children when no CLI
        ...
        # AGENT_BROWSER_ARGS â€” if the user pre-sets either, don't overwrite it.
        if (
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                ...
                logger.debug("browser: running as root â€” injecting --no-sandbox")
            else:
                ...
                try:
                    with open(_userns_restrict, encoding="utf-8") as _f:
                        if _f.read().strip() == "1":
                            _needs_sandbox_bypass = True
                            logger.debug(
                                "browser: AppArmor userns restrictions detected â€” "
        ...
        stderr_fd = os.open(stderr_path, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600)
        try:
            # See matching comment at the other Popen site above â€” on
            ...
            _popen_extra: dict = {}
            if os.name == "nt":
                # See matching block at the other Popen site â€” CREATE_NO_WINDOW
                # only, NO CREATE_NEW_PROCESS_GROUP (cancels asyncio loop task
                # on Python 3.11 Windows â†’ KeyboardInterrupt in CLI MainThread).
                _CREATE_NO_WINDOW = 0x08000000
                _popen_extra["creationflags"] = _CREATE_NO_WINDOW
                _popen_extra["close_fds"] = True
                _si = subprocess.STARTUPINFO()
        ...
        try:
            ...
            # Log stderr for diagnostics â€” use warning level on failure so it's visible
            ...
            # Empty output with rc=0 is a broken state â€” treat as failure rather
...
def browser_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    """
    # Secret exfiltration protection â€” block URLs that embed API keys or
    ...
    # SSRF protection â€” block private/internal addresses before navigating.
    ...
    # private URL + ``browser.auto_local_for_private_urls`` enabled) â€” the
    ...
    # Website policy check â€” block before navigating
    ...
    # Camofox backend â€” delegate after safety checks pass
    if _is_camofox_mode():
    ...
    if result.get("success"):
        ...
        # Post-redirect SSRF check â€” if the browser followed a redirect to a
        ...
        # when auto_local_this_nav is true â€” see pre-nav check for
        # rationale (#16234).
        if (
            not _is_local_backend()
            and final_url
...
def _browser_eval(expression: str, task_id: Optional[str] = None) -> str:
    ...
    # on the already-connected WebSocket â€” zero subprocess startup cost vs
    ...
    try:
        ...
        if supervisor is not None:
            ...
            # JS exception is a real failure â€” surface it instead of falling
            ...
            if "supervisor" not in err.lower():
                # Real JS-side error â€” return it.
                return json.dumps({"success": False, "error": err}, ensure_ascii=False)
            # Supervisor-side failure (loop down, no session) â€” fall through.
            logger.debug(
...
def _camofox_eval(expression: str, task_id: Optional[str] = None) -> str:
    ...
    try:
        ...
        error_msg = str(e)
        # Graceful degradation â€” server may not support eval
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = zed_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Zed
    ...
    screenshots_dir = get_zed_dir("cache/screenshots", "browser_screenshots")
    screenshot_path = screenshots_dir / f"browser_screenshot_{uuid_mod.uuid4().hex}.png"
    effective_task_id = _last_session_key(task_id or "default")

    # Lightpanda has no graphical renderer â€” pre-route screenshots to Chrome
    ...
    try:
        ...
        # turn â€” no aux call, no information loss. Consistent with vision_analyze.
        ...
        # Keep the screenshot if it was captured successfully â€” the failure is
...
def _cleanup_single_browser_session(task_id: str) -> None:
    ...
    # Skip full close when managed persistence is enabled â€” the browser
...
def _chromium_search_roots() -> List[str]:
    ...
    1. ``PLAYWRIGHT_BROWSERS_PATH`` when set (Docker image sets this to
       ``/opt/zed/.playwright``).
    2. ``~/.cache/ms-playwright`` â€” Playwright's default on Linux/macOS.
    3. ``~/Library/Caches/ms-playwright`` â€” Playwright's default on macOS.
    4. ``%USERPROFILE%\\AppData\\Local\\ms-playwright`` â€” Playwright's default
       on Windows.
    """
    roots: List[str] = []
...
def _chromium_installed() -> bool:
    ...
    1. ``AGENT_BROWSER_EXECUTABLE_PATH`` env var â€” the official way to point
       agent-browser at a pre-installed Chrome/Chromium.
    2. System Chrome/Chromium in PATH (``google-chrome``, ``chromium``,
       ``chromium-browser``, ``chrome``).
    3. Playwright's browser cache (current logic) â€” directories containing
    ...
    # 1. AGENT_BROWSER_EXECUTABLE_PATH â€” explicit user-configured browser
    ...
    # 3. Playwright browser cache (legacy â€” chromium-* / chromium_headless_shell-* dirs)
...
def check_browser_requirements() -> bool:
    ...
    """
    # Camofox backend â€” only needs the server URL, no agent-browser CLI
    if _is_camofox_mode():
        return True
...
if __name__ == "__main__":
    ...
    if check_browser_requirements():
        ...
        print("âŒ Missing requirements:")
        try:
            browser_cmd = _find_agent_browser()
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    print(
                        "     Docker: pull the latest image â€” the current one "
                        "predates the bundled Chromium install"
                    )
                    print("       docker pull ghcr.io/zedteam/zed-agent:latest")
                else:
    ...
    print("\nðŸ“‹ Available Browser Tools:")
    for schema in BROWSER_TOOL_SCHEMAS:
        print(f"  ðŸ”¹ {schema['name']}: {schema['description'][:60]}...")
