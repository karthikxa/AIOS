...
Backend is selected during ``zed tools`` setup (web.backend in config.yaml).
When available, Zed can route Firecrawl calls through a Zed-hosted tool-gateway
for Zed Subscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract; direct or derived firecrawl-gateway.<domain> for Zed Subscribers)
...
import httpx  # noqa: F401 â€” kept at module top so tests can patch tools.web_tools.httpx
...
if TYPE_CHECKING:
    from firecrawl import Firecrawl  # noqa: F401 â€” type hints only
...
from plugins.web.tavily.provider import (  # noqa: F401 â€” backward-compat names
...
from plugins.web.parallel.provider import (  # noqa: F401 â€” backward-compat names
...
from tools.managed_tool_gateway import (  # noqa: F401 â€” backward-compat names for tests
...
# â”€â”€â”€ Backend Selection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def _env_value(name: str) -> str:
    """Resolve ``name`` via Zed config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Zed' config/.env layer (``zed config set``, ``zed tools``)
    are honored here too â€” not just raw process-env exports. Without this,
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.zed/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``zed tools``).
    ...
    # Fallback for manual / legacy config â€” pick the highest-priority
    ...
    # pre-empted by a Zed OAuth token whose subscription tier may not
...
def _get_search_backend() -> str:
    ...
    2. ``web.backend`` (shared fallback â€” existing behavior)
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        # Cheap probe â€” env var OR auth.json has OAuth tokens. Must not
        ...
        # runs on every web_search dispatch + every `zed tools` repaint.
        try:
...
# â”€â”€â”€ Firecrawl Client â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _web_requires_env() -> list[str]:
    ...
    The gateway env vars are always reported â€” they're metadata strings
    ...
    refresh against the Zed Portal on every CLI startup (invoked at
...
# â”€â”€â”€ Parallel / Tavily / Firecrawl helpers â€” moved into plugins â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _is_nous_auxiliary_client(client: Any) -> bool:
    """Return True when the resolved auxiliary backend is Zed Portal."""
    from urllib.parse import urlparse

    base_url = str(getattr(client, "base_url", "") or "")
    host = (urlparse(base_url).hostname or "").lower()
    return host == "zedteam.com" or host.endswith(".zedteam.com")
...
async def process_content_with_llm(
    ...
    try:
        ...
        if len(content) > MAX_OUTPUT_SIZE:
            truncated += (
                f"\n\n[Content truncated â€” showing first {MAX_OUTPUT_SIZE:,} of "
...
Create a markdown summary that captures all key information in a well-organized, scannable format. Include important quotes and code snippets in their original formatting. Focus on actionable information, specific details, and unique insights."""
    ...
    # Call the LLM with retry logic â€” keep retries low since summarization
    ...
    for attempt in range(max_retries):
        try:
            ...
            call_kwargs = {
                ...
                # No explicit timeout â€” async_call_llm reads auxiliary.web_extract.timeout
            ...
            if content:
                return content
            # Reasoning-only / empty response â€” let the retry loop handle it
...
Create a single, unified markdown summary."""
    ...
    try:
        ...
        if not final_summary:
            logger.warning("Synthesis failed after retry â€” concatenating chunk summaries")
            fallback = "\n\n".join(summaries)
            if len(fallback) > max_output_size:
                fallback = fallback[:max_output_size] + "\n\n[... truncated ...]"
            return fallback
...
# â”€â”€â”€ Exa / Parallel inline helpers â€” moved into plugins â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _ensure_web_plugins_loaded() -> None:
    ...
    haven't already triggered discovery â€” subprocess agent runs, delegate
    children, standalone scripts, certain test paths â€” and without it the
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        # delegation. Sync only â€” every provider's search() is sync.
...
async def web_extract_tool(
    ...
    try:
        ...
        # â”€â”€ SSRF protection â€” filter out private/internal URLs before any backend â”€â”€
        safe_urls = []
        ...
        if not safe_urls:
            ...
            # async (parallel, firecrawl), others sync (exa, tavily) â€” we
...
if __name__ == "__main__":
    ...
    """
    print("ðŸŒ Standalone Web Tools Module")
    print("=" * 40)
    ...
    if web_available:
        ...
        print(f"âœ… Web backend: {backend}")
        if backend == "exa":
        ...
        print("âŒ No web search backend configured")
        print(
    ...
    if not nous_available:
        print("âŒ No auxiliary model available for LLM content processing")
        print("Set OPENROUTER_API_KEY, configure Zed Portal, or set OPENAI_BASE_URL + OPENAI_API_KEY")
        print("âš ï¸  Without an auxiliary model, LLM content processing will be disabled")
    else:
        print(f"âœ… Auxiliary model available: {default_summarizer_model}")

    if not web_available:
        sys.exit(1)

    print("ðŸ› ï¸  Web tools ready for use!")
    
    if nous_available:
        print(f"ðŸ§  LLM content processing available with {default_summarizer_model}")
        print(f"   Default min length for processing: {DEFAULT_MIN_LENGTH_FOR_SUMMARIZATION} chars")
    
    # Show debug mode status
    if _debug.active:
        print(f"ðŸ› Debug mode ENABLED - Session ID: {_debug.session_id}")
        print(f"   Debug logs will be saved to: {_debug.log_dir}/web_tools_debug_{_debug.session_id}.json")
    else:
        print("ðŸ› Debug mode disabled (set WEB_TOOLS_DEBUG=true to enable)")
    ...
    print("\nðŸ“ Run 'python test_web_tools_llm.py' to test LLM processing capabilities")
...
WEB_EXTRACT_SCHEMA = {
    "name": "web_extract",
    "description": "Extract content from web page URLs. Returns page content in markdown format. Also works with PDF URLs (arxiv papers, documents, etc.) â€” pass the PDF link directly and it converts to markdown text. Pages under 5000 chars return full markdown; larger pages are LLM-summarized and capped at ~5000 chars per page. Pages over 2M chars are refused. If a URL fails or times out, use the browser tool to access it instead.",
    "parameters": {
        "type": "object",
        "properties": {
            "urls": {
