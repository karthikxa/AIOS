...
# (W...) are NOT valid chat.postMessage channel values â€” posting to them fails
...
# verbatim â€” they must NOT fall through to home-channel resolution.
_WHATSAPP_JID_RE = re.compile(
    r"^\s*[\w-]+@(?:g\.us|s\.whatsapp\.net|lid|broadcast|newsletter)\s*$",
    re.IGNORECASE,
)
# Email addresses â€” a valid email like "user@domain.com" should be treated as
...
SEND_MESSAGE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "message": {
                "type": "string",
                "description": "The message text to send. To send an image or file, include MEDIA:<local_path> (e.g. 'MEDIA:/tmp/report.pdf') in the message â€” the platform will deliver it as a native media attachment."
            },
            "emoji": {
                "type": "string",
                "description": "For action='react': the emoji to react with (e.g. 'â¤ï¸'). On iMessage, â¤ï¸ðŸ‘ðŸ‘ŽðŸ˜‚â€¼ï¸â“ render as native tapbacks; other emoji use custom-emoji reactions."
            },
            "message_id": {
                "type": "string",
...
def _handle_react(args, remove=False):
    ...
    process â€” there is no standalone fallback, since reacting needs the
    ...
    if target_ref:
        ...
        if not chat_id:
            ...
            # entry â€” pass them through verbatim; the adapter validates.
...
def _handle_send(args):
    ...
    # Accept any platform name â€” built-in names resolve to their enum
    ...
    if not pconfig or not pconfig.enabled:
        ...
        if platform_name == "weixin":
            ...
            if wx_token and wx_account:
                ...
                return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.zed/config.yaml or environment variables.")
        else:
            return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.zed/config.yaml or environment variables.")
    ...
    if not chat_id:
        ...
        if home:
            ...
            return json.dumps({
                ...
                f"or set a home channel via: zed config set {home_env} <channel_id>"
            })
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            try:
                ...
                source_label = get_session_env("ZED_SESSION_PLATFORM", "cli")
...
def _parse_target_ref(platform_name: str, target_ref: str):
    ...
    if platform_name == "slack":
        ...
        if match:
            ...
            # explicit send targets â€” chat.postMessage rejects them. A DM
            # must be opened first via conversations.open to get a D...
            # conversation ID. Caller still gets the chat_id so the Uâ†’D
    ...
    if platform_name == "whatsapp":
        ...
        # are explicit targets â€” pass through verbatim. E.164 '+' numbers fall
    ...
    if platform_name in _PHONE_PLATFORMS:
        ...
        if match:
            # Preserve the leading '+' â€” signal-cli and sms/whatsapp adapters
...
def _get_cron_auto_delivery_target():
    ...
    from gateway.session_context import get_session_env
    platform = get_session_env("ZED_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()
    chat_id = get_session_env("ZED_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not platform or not chat_id:
        return None
    thread_id = get_session_env("ZED_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None
    return {
        "platform": platform,
        "chat_id": chat_id,
        "thread_id": thread_id,
...
async def _send_telegram(token, chat_id, message, media_files=None, thread_id=None, disable_link_previews=False, force_document=False):
    ...
    Applies markdownâ†’MarkdownV2 formatting (same as the gateway adapter)
    ...
    try:
        ...
        # Auto-detect HTML tags â€” if present, skip MarkdownV2 and send as HTML.
        # Inspired by github.com/ashaney â€” PR #1568.
        ...
        if _has_html:
            ...
            # Reuse the gateway adapter's format_message for markdownâ†’MarkdownV2
            try:
                from gateway.platforms.telegram import TelegramAdapter
        ...
        if formatted.strip():
            try:
                ...
                # Thread not found â€” retry without message_thread_id so the
                ...
                with open(media_path, "rb") as f:
                    ...
                    try:
                        ...
                        if _is_telegram_thread_not_found(media_err) and media_kwargs.get("message_thread_id"):
                            # Thread not found for media â€” retry without
...
async def _send_signal(extra, chat_id, message, media_files=None):
    ...
    SignalAttachmentScheduler â€” same bucket the gateway adapter uses, so
            ...
            if n > 0:
                ...
                if estimated >= SIGNAL_BATCH_PACING_NOTICE_THRESHOLD:
                    await _send_inline_notice(
                        f"(More images coming â€” pausing ~{_format_wait(estimated)} "
...
async def _send_matrix(token, extra, chat_id, message):
    ...
    try:
        ...
        txn_id = f"zed_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
...
async def _send_feishu(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    try:
        ...
        if not FEISHU_AVAILABLE:
            return {"error": "Feishu dependencies not installed. Run: pip install 'zed-agent[feishu]'"}
...
def _check_send_message():
    ...
    Also passes for kanban workers â€” the dispatcher sets ``ZED_KANBAN_TASK``
    on every spawned worker, but those workers run with the assignee profile's
    ``ZED_HOME`` which has no ``gateway.pid``, so the gateway-running check
...
async def _send_qqbot(pconfig, chat_id, message):
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            ...
            # If channel endpoint failed (likely "é¢‘é“ä¸å­˜åœ¨"), try C2C endpoint
            ...
            # All endpoints failed â€” return the most informative error
...
async def _send_yuanbao(chat_id, message, media_files=None):
    ...
    Yuanbao uses a persistent WebSocket â€” unlike HTTP-based platforms, we
...
#   - the ``zed send`` CLI command (zed_cli/send_cmd.py)
