...
provider** â€” turn on a backend by enabling a plugin (``zed plugins
enable video_gen/<name>``) and selecting it in ``zed tools`` â†’ Video
Generation.

Unified surface
---------------
One tool covers the common cases â€” text-to-video, image-to-video, video
edit, video extend â€” with a compact schema:
...
do its own clamping inside :meth:`VideoGenProvider.generate` â€” that keeps
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    "name": "video_generate",
    # Placeholder â€” the real description is built dynamically at
    ...
    # skill at github/zed-agent-dev/references/dynamic-tool-schemas.md.
    "description": "(rebuilt at get_definitions() time â€” see _build_dynamic_video_schema)",
    "parameters": {
        "type": "object",
        "properties": {
            "prompt": {
            ...
            "negative_prompt": {
                "type": "string",
                "description": (
                    "Optional negative prompt â€” content to avoid in the "
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `zed tools` "
                    "â†’ Video Generation) is used. Models that the active "
...
def _resolve_active_provider():
    ...
    Forces plugin discovery before checking the registry â€” handles cases
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `zed tools` â†’ Video "
    ...
    msg = (
        "No video generation backend is configured. Run `zed tools` â†’ "
...
def _handle_video_generate(args: Dict[str, Any], **_kw: Any) -> str:
    ...
    # Soft validation â€” providers do their own. Prompt is required by the
    ...
    try:
        ...
        # caller error â€” log and surface a clear contract message.
        logger.warning(
...
# ---------------------------------------------------------------------------
# Dynamic schema â€” reflect the active backend's actual capabilities
# ---------------------------------------------------------------------------
#
...
# `zed tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`zed tools` â†’ Video Generation; the agent does not pick them. "
    "Long-running generations may take 30 seconds to several minutes â€” "
...
def _format_model_caveats(
    ...
    overall capabilities â€” repeating defaults is noise.
    """
    caveats: List[str] = []
    ...
    if "image" in modalities and "text" not in modalities:
        caveats.append(
            "this model is image-to-video only â€” image_url is REQUIRED; "
            "text-only calls will be rejected"
        )
    elif "text" in modalities and "image" not in modalities:
        caveats.append(
            "this model is text-to-video only â€” image_url is not supported"
        )
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `zed tools` â†’ Video Generation."
        )
        return {"description": "\n".join(parts)}
    ...
    if provider is None:
        parts.append(
            f"\nActive backend: {configured} (plugin not yet loaded â€” the "
    ...
    # Backend modality summary â€” only useful when the backend supports
    ...
    if "text" in modalities and "image" in modalities and not model_meta.get("modality"):
        parts.append(
            ...
            "image-to-video (pass image_url) â€” routes automatically"
        )
