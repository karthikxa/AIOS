...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/zed-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/zed-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/zed-results"
HEREDOC_MARKER = "ZED_PERSIST_EOF"
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"ZED_PERSIST_{uuid.uuid4().hex[:8]}"
