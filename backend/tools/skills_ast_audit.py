"""
AST-level deep audit for skill Python files â€” opt-in diagnostic, not a security gate.

Per SECURITY.md Â§2.4, Skills Guard is in-process heuristics ("useful â€” not
    ...
    class V(ast.NodeVisitor):
        def visit_Call(self, node):
            ...
            if isinstance(f, ast.Attribute) and f.attr == "import_module":
                findings.append((rel_path, node.lineno, "dynamic_import",
                                 "importlib.import_module() â€” loads arbitrary modules at runtime"))
            # __import__(<computed>)
        ...
        def visit_Subscript(self, node):
            ...
            if (isinstance(node.value, ast.Attribute)
                ...
                findings.append((rel_path, node.lineno, "dict_access",
                                 "__dict__[<computed>] â€” dynamic attribute access"))
            self.generic_visit(node)
            ...
            for a in node.names:
                if a.name == "importlib" or a.name.startswith("importlib."):
                    findings.append((rel_path, node.lineno, "importlib_import",
                                     f"import {a.name} â€” enables dynamic module loading"))
            self.generic_visit(node)
        ...
        def visit_ImportFrom(self, node):
            ...
            if m == "importlib" or m.startswith("importlib."):
                findings.append((rel_path, node.lineno, "importlib_import",
                                 f"from {m} import ... â€” enables dynamic module loading"))
            self.generic_visit(node)
...
def format_ast_report(findings: List[Finding], skill_name: str = "") -> str:
    ...
    for f, line, pid, desc in sorted(findings):
        if f != current:
            current = f
            lines.append(f"  {f}")
        lines.append(f"    L{line}  {pid}  â€” {desc}")
    lines.append("")
