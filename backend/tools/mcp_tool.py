...
transport, discovers their tools, and registers them into the zed-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.zed/config.yaml under the ``mcp_servers`` key.
...
# the terminal while prompt_toolkit / Rich is rendering the TUI â€” which
# corrupts the display and can hang the session.
#
# Instead we redirect every stdio MCP subprocess's stderr into a shared
# per-profile log file (~/.zed/logs/mcp-stderr.log), tagged with the
...
# JSON-RPC "method not found" â€” the error a server returns when it does not
...
except Exception:  # pragma: no cover â€” older/newer SDK without the constant
...
# These are WARNING-level â€” we log but don't block, since false positives
...
def _scan_mcp_description(server_name: str, tool_name: str, description: str) -> List[str]:
    ...
    if findings:
        logger.warning(
            "MCP server '%s' tool '%s': suspicious description content â€” %s. "
            "Description: %.200s",
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            zed_home = os.path.expanduser(
                os.getenv(
                    "ZED_HOME", os.path.join(os.path.expanduser("~"), ".zed")
                )
            )
            candidates = [
                os.path.join(zed_home, "node", "bin", resolved_command),
                ...
                # image (which the Zed Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block â†’ Zed MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Zed gateways know how to render.
    ...
    try:
        ...
        # without gateway deps). Fall back to silently dropping â€” callers
        # get any text blocks that did parse.
        logger.debug("MCP image caching skipped â€” gateway.platforms.base unavailable")
        return ""
...
def _validate_remote_mcp_url(server_name: str, url: Any) -> str:
    ...
    - Non-http(s) schemes (``file://``, ``ws://``, ``stdio:`` â€” stdio servers
    ...
    except Exception as exc:  # urlparse is very permissive â€” belt and braces
        raise InvalidMcpUrlError(
    ...
    # Reject that â€” we need a real host.
...
def _resolve_client_cert(server_name: str, config: dict):
    ...
    no client certificate is configured:
      ...
      - A ``(cert_path, key_path, password)`` tuple when ``client_cert`` is
        a 3-element list/tuple â€” the third element is the key passphrase.
    ...
    # Tuple/list form for client_cert â€” (cert, key) or (cert, key, password).
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Zed' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (ZED_SESSION_PLATFORM etc.). When
        ...
        def _invoke_consent() -> str:
            ...
            # Context.run can only execute a context once â€” copy to allow
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit ZED_SESSION_PLATFORM, so the
    ...
    def _advertises_tools(self) -> bool:
        ...
        them raises ``McpError(-32601 Method not found)`` â€” which previously
    ...
    async def _refresh_tools(self):
        ...
        â€” atomic from the event loop's perspective.
        """
        from tools.registry import registry

        if not self._advertises_tools():
            # A server that doesn't implement tools/* should never send
            # tools/list_changed, but guard anyway â€” calling tools/list
        ...
        async with self._refresh_lock:
            ...
            if changes:
                logger.warning(
                    "MCP server '%s': tools changed dynamically â€” %s. "
    ...
    async def _keepalive_probe(self) -> None:
        ...
        ``_ping_unsupported`` and fall back to the pre-ping probe â€” capability
        ...
        if not self._ping_unsupported:
            try:
                ...
                # a real liveness failure â€” propagate so we reconnect.
                if not _is_method_not_found_error(exc):
                    raise
                if not self._advertises_tools():
                    # No ping, no tools â†’ no cheaper probe to fall back to.
                    raise
    ...
    async def _wait_for_lifecycle_event(self) -> str:
        ...
        ping utility â€” see :meth:`_keepalive_probe`) to prevent TCP/session
        ...
        # stays a few bytes regardless of how many tools the server exposes â€”
        ...
        # ``notifications/tools/list_changed`` â†’ ``_refresh_tools``.
        ...
        try:
            while True:
                ...
                # Timeout â€” no lifecycle event fired.  Probe the connection
    ...
    async def _run_stdio(self, config: dict):
        ...
        try:
            async with stdio_client(server_params, errlog=_errlog) as (
                ...
                if new_pids:
                    # Capture pgid while the child is alive â€” once it exits we
                ...
                with _lock:
                    ...
                    for pid in new_pids:
                        ...
                        if not pid_alive and pgid is not None and _killpg is not None:
                            ...
                            # signal 0 â€” succeeds iff any pgroup member is alive.
                            try:
                                _killpg(pgid, 0)
                                pgroup_alive = True
                        ...
                        if pid_alive or pgroup_alive:
                            ...
                            # Nothing left to reap â€” drop the pgid entry so
    ...
    async def _preflight_content_type(
        ...
        *,
        ...
        here catches that in â‰¤ ``timeout`` seconds and raises
        ...
        through silently â€” the probe is strictly best-effort, and the real
        ...
        try:
            ...
            return  # No httpx â†’ skip probe; SDK import would have failed first.
        ...
        try:
            ...
            return  # DNS/connect/timeout/transport error â€” let the SDK try.
        ...
        if not ct_base:
            return  # No content type advertised â€” don't second-guess the SDK.
        ...
        raise NonMcpEndpointError(
            ...
            "points at a web page rather than an MCP endpoint â€” check it "
    ...
    async def _run_http(self, config: dict):
        ...
        if config.get("transport") == "sse":
            ...
            if _oauth_auth is not None:
                ...
                # forwarded â€” SSE OAuth would silently fail with 401s.
            ...
            async with sse_client(**_sse_kwargs) as (read_stream, write_stream):
                async with ClientSession(
                    ...
                    if reason == "reconnect":
                        logger.info(
                            "MCP server '%s': reconnect requested â€” "
        ...
        if _MCP_NEW_HTTP:
            ...
            # Caller owns the client lifecycle â€” the SDK skips cleanup when
    ...
    async def _discover_tools(self):
        ...
        which previously aborted the connection â€” those servers could never
        stay connected for their prompts/resources. Skip the call when the
        server doesn't advertise the ``tools`` capability.
        (Ported from anomalyco/opencode#31271.)
        """
        # Fresh transport connection â†’ re-probe with the cheap ``ping`` path.
        ...
        if not self._advertises_tools():
            logger.info(
                "MCP server '%s': does not advertise 'tools' capability â€” "
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Zed' approval system.
        ...
        # means a typo in config.yaml fails fast with a clear error â€” and
        ...
        if self._is_http():
            ...
            # before surfacing an opaque CancelledError. Probing here â€” once,
            # outside the SDK task group â€” fails fast and non-retryably with
        ...
        while True:
            try:
                ...
                #    touch the retry counters â€” this is not a failure.
                if self._shutdown_event.is_set():
                    break
                logger.info(
...
#
# State machine:
#   closed    â€” error count below threshold; all calls go through.
#   open      â€” threshold reached; calls short-circuit until the
#               cooldown elapses.
#   half-open â€” cooldown elapsed; the next call is a probe that
#               actually hits the session. Probe success â†’ closed.
#               Probe failure â†’ reopens (cooldown re-armed).
#
...
# this state â€” they keep the count and timestamp in sync.
...
def _get_auth_error_types() -> tuple:
    ...
    Cached after first call. Includes:
      - ``mcp.client.auth.OAuthFlowError`` / ``OAuthTokenError`` â€” raised by
        the SDK's auth flow when discovery, refresh, or full re-auth fails.
      - ``mcp.client.auth.UnauthorizedError`` (older MCP SDKs) â€” kept as an
        optional import for forward/backward compatibility.
      - ``tools.mcp_oauth.OAuthNonInteractiveError`` â€” raised by our callback
        handler when no user is present to complete a browser flow.
      - ``httpx.HTTPStatusError`` â€” caller must additionally check
...
def _handle_auth_error_and_retry(
    ...
    if recovered:
        ...
        # server is viable again, so close the circuit breaker here â€”
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `zed mcp login {server_name}` (or delete the tokens "
            f"file under ~/.zed/mcp-tokens/ and restart). Do NOT retry "
            f"this tool â€” ask the user to re-authenticate."
        ),
        "needs_reauth": True,
        "server": server_name,
    }, ensure_ascii=False)
...
# was garbage-collected.  The caller's OAuth token is still valid â€”
...
def _is_session_expired_error(exc: BaseException) -> bool:
    ...
    state while the OAuth token remains valid â€” idle TTL, server
    ...
    needed is a transport reconnect â€” tear down and rebuild the
...
def _handle_session_expired_and_retry(
    ...
    the OAuth manager's ``handle_401`` â€” the access token is still
...
def _mcp_loop_exception_handler(loop, context):
    ...
    if isinstance(exc, RuntimeError) and "Event loop is closed" in str(exc):
        return  # benign shutdown race â€” suppress
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local ZED_HOME override into ``coro``.
    ...
    context on the MCP loop and reset when it completes â€” concurrent calls
    ...
    try:
        ...
        home_override = get_zed_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_zed_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local ZED_HOME override onto the MCP loop.
    # Tasks scheduled via run_coroutine_threadsafe are created INSIDE the
    # loop thread, so they copy the loop thread's context â€” not the
    ...
    # vanish here: OAuth token stores and any other get_zed_home()
    ...
    # task's own context (task-local â€” concurrent calls carrying different
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Zed config file.
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
            ...
            if age < _CIRCUIT_BREAKER_COOLDOWN_SEC:
                ...
                return json.dumps({
                    "error": (
                        ...
                        f"Do NOT retry this tool yet â€” use alternative "
                        f"approaches or ask the user to check the MCP server."
                    )
                }, ensure_ascii=False)
            # Cooldown elapsed â†’ fall through as a half-open probe.
        ...
        async def _call():
            ...
            # flow through Zed' MEDIA: tag convention and out to messaging
            ...
            # Zed' MEDIA tag + cache_image_from_bytes) was the cleaner of
            # the two â€” plugs into existing infrastructure.
            parts: List[str] = []
            for block in (result.content or []):
        ...
        try:
            ...
            try:
                ...
                if "error" in parsed:
                    ...
                    _reset_server_error(server_name)  # success â€” reset
            except (json.JSONDecodeError, TypeError):
            ...
            # still valid â€” only the server-side session is stale.
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Zed's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Zed registry schema format.
...
# Source of truth: MCP spec â€” capabilities.resources / capabilities.prompts
...
def _register_server_tools(name: str, server: MCPServerTask, config: dict) -> List[str]:
    ...
    #   tools.include â€” whitelist: only these tool names are registered
    #   tools.exclude â€” blacklist: all tools EXCEPT these are registered
    #   include takes precedence over exclude
    #   Neither set â†’ register all tools (backward-compatible default)
    tools_filter = config.get("tools") or {}
    ...
    for mcp_tool in server._tools:
        ...
        if existing_toolset and not existing_toolset.startswith("mcp-"):
            logger.warning(
                "MCP server '%s': tool '%s' (â†’ '%s') collides with built-in "
                "tool in toolset '%s' â€” skipping to preserve built-in",
...
def get_mcp_status() -> List[dict]:
    ...
    for name, cfg in configured.items():
        ...
        if server and server.session is not None:
            ...
            # A server with enabled: false is intentionally not connected â€” it is
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``zed tools`` interactive configuration â€” connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Zed registry.
...
def has_registered_mcp_tools() -> bool:
    ...
    Cheap â€” checks the global MCP-toolâ†’server name map under ``_lock``, no
...
def refresh_agent_mcp_tools(
    ...
    *after* that snapshot â€” a slow HTTP/OAuth server that misses the bounded
    startup wait, or a ``/reload-mcp`` â€” their tools are invisible until the
    ...
    tool **name** (not count â€” a count compare misses an equal-size add/remove
    swap).
    ...
    families directly onto ``agent.tools`` *after* that â€” external
    memory-provider tools (mem0/honcho/â€¦) and context-engine tools
    ...
    # engine tools). Staged entirely on LOCALS â€” the live ``agent.tools`` /
    ...
    with _agent_tools_lock:
        ...
        if snapshot_generation < published_gen:
            # A newer snapshot already won; our set is stale â€” drop it.
            return set()
        current = {
            t["function"]["name"]
        ...
        if new_names == current:
            # No change â†’ leave the live snapshot untouched (no churn), but
...
def _reinject_post_build_tools(agent, tools_list: list, name_set: set) -> set:
    ...
    rebuild â€” matching ``agent_init``'s dedup behavior (a name already provided
    ...
    # Memory-provider tools (mem0/honcho/byterover/supermemory/â€¦).
    try:
    ...
    # Context-engine tools (lcm_grep/lcm_describe/â€¦) â€” the `context_engine`
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    did not terminate the process â€” common on Linux when stdio children escape
    ...
    survivors, avoiding shared-resource collisions when multiple zed
    ...
    With ``include_active=True`` also kills every PID in ``_stdio_pids`` â€”
    ...
    # dance entirely â€” otherwise every MCP-free shutdown pays a 2s sleep tax.
    if not pids:
        return
    ...
    def _send_signal(pid: int, sig: int, server_name: str) -> None:
        ...
        if pgid is not None and killpg is not None:
            try:
                ...
                # Pgroup gone (all members exited) or refused â€” fall back to
    ...
    for pid, server_name in pids.items():
        if not _pid_exists(pid):
            continue  # Good â€” exited after SIGTERM
...
def _stop_mcp_loop(*, only_if_idle: bool = False) -> bool:
    ...
    if loop is not None:
        ...
        # graceful shutdown are now orphaned â€” include active PIDs too
