...
check in ``run_agent._should_parallelize_tool_batch`` â€” this module catches
...
A process-wide singleton ``FileStateRegistry`` tracks, per resolved path:
  ...
  * per-path ``threading.Lock`` for readâ†’modifyâ†’write critical sections

Three public hooks are used by the file tools:

  * ``record_read(task_id, path, *, partial)`` â€” called by read_file
  * ``note_write(task_id, path)`` â€” called after write_file / patch
  * ``check_stale(task_id, path)`` â€” called BEFORE write_file / patch

Plus ``lock_path(path)`` â€” a context-manager returning a per-path lock to
wrap the whole readâ†’modifyâ†’write block. And ``writes_since(task_id,
since_ts, paths)`` for the subagent-completion reminder in delegate_tool.

All methods are no-ops when ``ZED_DISABLE_FILE_STATE_GUARD=1`` is set.

This module is intentionally separate from ``_read_tracker`` in
``file_tools.py`` â€” that tracker is per-task and handles consecutive-read
...
# â”€â”€ Public stamp type â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# (mtime, read_ts, partial).  partial=True when read_file returned a
# windowed view (offset > 1 or limit < total_lines) â€” writes that happen
...
class FileStateRegistry:
    ...
    # â”€â”€ Path lock management â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def lock_path(self, resolved: str):
        """Acquire the per-path lock for a readâ†’modifyâ†’write section.

        Same process, same filesystem â€” threads on the same path serialize.
    ...
    # â”€â”€ Read/write accounting â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    def record_read(
        self,
        task_id: str,
        resolved: str,
    ...
    def note_write(
        ...
        *,
        ...
        (a write is an implicit read â€” the agent now knows the current
        content).
        """
        if _disabled():
            return
    ...
    def check_stale(self, task_id: str, resolved: str) -> Optional[str]:
        ...
        Returns ``None`` when the write is safe.  Does not raise â€” callers
        ...
        # Case 3: never read AND we have no write record â€” net-new file or
        ...
        try:
            ...
            # File doesn't exist â€” write will create it; not stale.
            return None
        ...
        if last_writer is not None:
            ...
            if writer_tid != task_id:
                ...
                if writer_ts > read_ts:
                    return (
                        f"{resolved} was modified by sibling subagent "
                        f"{writer_tid!r} at {_fmt_ts(writer_ts)} â€” after "
    ...
    # â”€â”€ Reminder helper for delegate_tool â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    def writes_since(
        self,
        exclude_task_id: str,
        since_ts: float,
    ...
    # â”€â”€ Testing hooks â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    def clear(self) -> None:
...
# â”€â”€ Module-level singleton + helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _disabled() -> bool:
    ...
    return os.environ.get("ZED_DISABLE_FILE_STATE_GUARD", "").strip() == "1"
...
def _cap_dict(d: dict, limit: int) -> None:
    ...
    if over <= 0:
        return
    # dict preserves insertion order (PY>=3.7) â€” pop the oldest keys.
    it = iter(d)
    for _ in range(over):
        try:
            d.pop(next(it))
...
# â”€â”€ Convenience wrappers (short names used at call sites) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
