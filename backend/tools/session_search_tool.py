...
the get_anchored_view / get_messages_around primitives in zed_state.
...
# Third-party integrations tag their sessions with ZED_SESSION_SOURCE=tool;
# delegate subagent runs are tagged "subagent" â€” neither belongs in the
...
def _read_session(db, session_id: str, head: int = 20, tail: int = 10) -> str:
    ...
    Serves the linked-session case â€” the user dropped an @session reference and
...
def _scroll(
    ...
    No FTS5, no bookends â€” just the slice. The discovery shape's lineage
    ...
    # Reject scrolling inside the active session lineage â€” those messages are
    # already in context.
...
def _discover(
    ...
    # row â€” only that pairs validly with the FTS5 match id for the anchored
...
def session_search(
    ...
    Read:      pass ``session_id`` (no anchor) â€” dumps the whole session.
    Browse:    pass nothing.
    ...
    anchor is set â€” the agent has asked for a specific slice.
    """
    if db is None:
        try:
            from zed_state import SessionDB
    ...
    # Session ids never contain "/", so a slash unambiguously means profile/id â€”
    ...
    # Scroll shape takes precedence â€” explicit anchor beats any query.
    ...
    # Read shape: a session_id with no anchor â†’ dump the whole session.
    if isinstance(session_id, str) and session_id.strip():
        ...
        # Miss in the target profile â€” the model may have dropped the owning
    ...
    # Browse shape: no query â†’ recent sessions.
...
def check_session_search_requirements() -> bool:
    ...
    try:
        from zed_state import DEFAULT_DB_PATH
...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "FTS5-backed retrieval over the SQLite message store. No LLM calls â€” every "
        ...
        "(the goal / kickoff)\n"
        "       - messages: Â±5 messages around the FTS5 match, with the anchor message "
        "flagged (the hit in context)\n"
        "       - bookend_end: last 3 user+assistant messages of the session "
        "(the resolution / decisions)\n"
        "       - match_message_id, messages_before, messages_after\n"
        "     Bookends + window together let you reconstruct goal â†’ match â†’ resolution "
        "without paying for the whole transcript.\n\n"
        "  2) SCROLL â€” pass `session_id` + `around_message_id`:\n"
        "     session_search(session_id=\"...\", around_message_id=12345, window=10)\n"
        "     Returns a window of Â±`window` messages centered on the anchor. No FTS5, "
        "no bookends â€” just the slice. Use after a discovery call when you need more "
        "context than the Â±5 default window.\n"
        ...
        "       - The boundary message appears in both windows â€” orientation marker.\n"
        "       - When messages_before or messages_after is < window, you're at the "
        "start or end of the session.\n\n"
        "  3) READ â€” pass `session_id` only (no around_message_id):\n"
        ...
        "FTS5 SYNTAX\n\n"
        "  AND is the default â€” multi-word queries require all terms. Use OR explicitly "
        ...
        "\"find the session where Z\" question â€” before gh, web search, or filesystem "
        ...
        "properties": {
            ...
            "limit": {
                ...
                "description": (
                    ...
                    "Bump to 5â€“10 when the topic likely spans several sessions and you "
            ...
            "sort": {
                ...
                "description": (
                    ...
                    "to keep relevance-only ordering (suitable for exploratory recall â€” "
            ...
            "profile": {
                "type": "string",
                "description": (
                    "Optional. Read sessions from another Zed profile's database "
