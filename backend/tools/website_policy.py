...
This module loads a user-managed website blocklist from ~/.zed/config.yaml
...
def _iter_blocklist_file_rules(path: Path) -> List[str]:
    ...
    rather than raising â€” a bad file path should not disable all web tools.
    """
    try:
...
def _load_policy_config(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("PyYAML not installed â€” website blocklist disabled")
        return dict(_DEFAULT_WEBSITE_BLOCKLIST)
...
def load_website_blocklist(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    for shared_file in raw_shared_files:
        ...
        if not path.is_absolute():
            path = (get_zed_home() / path).resolve()
    ...
    # Cache the result (only for the default path â€” explicit paths are tests)
...
def check_website_access(url: str, config_path: Optional[Path] = None) -> Optional[Dict[str, str]]:
    ...
    Never raises on policy errors â€” logs a warning and returns ``None``
    ...
    try:
        ...
        if config_path is not None:
            raise  # Tests pass explicit paths â€” let errors propagate
    ...
    for rule in policy.get("rules", []):
        ...
        if _match_host_against_rule(host, pattern):
            logger.info("Blocked URL %s â€” matched rule '%s' from %s",
