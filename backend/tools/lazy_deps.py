"""
Lazy dependency installer for opt-in Zed Agent backends.

Many Zed features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``zed-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
1. **Fragility.** When one extra's transitive dependency becomes
   ...
   back to a stripped tier â€” losing 10+ unrelated extras at once.
...
remediation hint pointing at ``zed tools`` or the manual pip command.
...
* **Offline detection.** If the install fails (offline, mirror down,
  ...
  :class:`FeatureUnavailable` with the actual pip stderr â€” no silent
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    # â”€â”€â”€ Inference providers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # Native Anthropic SDK â€” needed when provider=anthropic (not via
    # OpenRouter / aggregators which use the openai SDK).
    "provider.anthropic": ("anthropic==0.87.0",),  # CVE-2026-34450, CVE-2026-34452
    # AWS Bedrock provider
    "provider.bedrock": ("boto3==1.42.89",),
    # Microsoft Foundry â€” Entra ID auth (managed identity, workload identity,
    ...
    # â”€â”€â”€ Web search backends â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    "search.exa": ("exa-py==2.10.2",),
    "search.firecrawl": ("firecrawl-py==4.17.0",),
    "search.parallel": ("parallel-web==0.4.2",),

    # â”€â”€â”€ TTS providers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€â”€ Speech-to-text providers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€â”€ Image generation backends â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    "image.fal": ("fal-client==0.13.1",),

    # â”€â”€â”€ Memory providers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    "memory.honcho": ("honcho-ai==2.0.1",),
    "memory.hindsight": ("hindsight-client==0.6.1",),

    # â”€â”€â”€ Messaging platforms (lazy-installable on demand) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # "Can not decode content-encoding: br" â€” see #12511 / #15744.
    ...
    # WeCom callback-mode adapter â€” parses untrusted XML POST bodies. Pulls
    ...
    # Microsoft Teams adapter â€” microsoft-teams-apps pulls a heavy tree
    ...
    # â”€â”€â”€ Terminal backends â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    "terminal.modal": ("modal==1.3.4",),
    "terminal.daytona": ("daytona==0.155.0",),

    # â”€â”€â”€ Skills â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # Dashboard (`zed dashboard`)
    "tool.dashboard": (
        "fastapi==0.133.1",
        "uvicorn[standard]==0.41.0",
        "starlette==1.0.1",  # CVE-2026-48710 (BadHost) â€” keep lazy-install in sync with pyproject [web]
...
# Conservative regex for spec validation â€” package name plus optional
...
def _pkg_name_from_spec(spec: str) -> str:
    ...
    ``"slack-bolt>=1.18.0,<2"`` â†’ ``"slack-bolt"``
    ``"mautrix[encryption]>=0.20"`` â†’ ``"mautrix"``
    """
...
def _specifier_from_spec(spec: str) -> str:
    ...
    ``"honcho-ai==2.0.1"`` â†’ ``"==2.0.1"``
    ``"mautrix[encryption]>=0.20,<1"`` â†’ ``">=0.20,<1"``
    ``"package"`` â†’ ``""`` (no version constraint)
    """
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``zed update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
    ...
    if not spec_tail:
        # Bare ``"package"`` â€” no version constraint, presence is enough.
        return True
    ...
    try:
        ...
        # packaging unavailable â€” fall back to "installed counts as satisfied".
        return True

    try:
        return Version(installed) in SpecifierSet(spec_tail)
    except (InvalidSpecifier, InvalidVersion, Exception):
        # Malformed spec or installed version we can't parse â€” don't churn.
        return True
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    # Tier 1: uv (preferred â€” fast, doesn't need pip in the venv)
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    batch) get prompt=False and skip the confirmation â€” config flag is
    ...
    # braces â€” the keys-in-LAZY_DEPS check above already constrains this.
    for spec in missing:
    ...
    # Under the TUI we skip the prompt and proceed â€” lazy installs are
    ...
    if not result.success:
        ...
        if snippet:
            # Clip to a readable size â€” pip can dump pages of resolution traces.
...
def active_features() -> list[str]:
    ...
    Used by ``zed update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Returns a ``{feature: status}`` map where status is one of:
        ``"current"``  â€” pins already satisfied, no install run
        ``"refreshed"`` â€” pins were stale, reinstall succeeded
        ``"failed: <reason>"`` â€” install attempt failed; caller decides
                                  whether to surface it (we don't raise)
        ``"skipped: <reason>"`` â€” gated off (config flag, user decline)

    Intended for ``zed update``. Never raises; lazy-install failures
