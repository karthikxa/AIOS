...
machine as Zed itself), we surface a single deterministic line about
Python tooling state so models don't have to discover it by hitting
walls.  Common failure modes this addresses:

* Zed ships under one Python (e.g. 3.11 in a bundled venv) while the
  user's login shell has a different one (e.g. 3.12 system).  ``pip``
  resolved from PATH may not match ``python3 -m pip``.
* The bundled-venv Python has no pip module installed â†’ ``python3 -m
  pip`` returns ``No module named pip``.
* The system Python is PEP-668 externally-managed â†’ naive
...
PEP 668), it emits nothing â€” no token cost.

Remote terminal backends (docker, modal, ssh, â€¦) are skipped: the
...
# lifetime of the process â€” Python install state doesn't change
...
# Remote backends â€” keep in sync with agent/prompt_builder.py:_REMOTE_TERMINAL_BACKENDS.
...
def _build_probe_line() -> str:
    ...
    Emit only when SOMETHING is off â€” the goal is to save the model from
    ...
    # version mismatch between `pip` and `python3` â†’ environment is
    ...
    if py_ver and py_ver != py3_ver:
        ...
        # Common on Debian/Ubuntu â€” call it out so the model doesn't
        # type `python` and hit "command not found".
        bits.append("python=missing (use python3)")

    if pip_bound_to:
        if mismatch:
            bits.append(f"pipâ†’python{pip_bound_to} (mismatch)")
        elif not py3_has_pip:
            # pip exists but `python3 -m pip` doesn't â€” the script
            # works but the module path doesn't.
            bits.append(f"pipâ†’python{pip_bound_to}")
    elif py3_has_pip:
...
def get_environment_probe_line(*, force_refresh: bool = False) -> str:
    ...
    Returns "" when the environment is clean â€” the system prompt
...
def _reset_cache_for_tests() -> None:
    """Test helper â€” clear the cache between probe scenarios."""
    global _CACHED_LINE
    with _CACHE_LOCK:
        _CACHED_LINE = None
