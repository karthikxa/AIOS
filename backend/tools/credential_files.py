...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.zed",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``ZED_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside ZED_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    zed_home = _resolve_zed_home()

    # Reject absolute paths â€” they bypass the ZED_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to ZED_HOME)",
            relative_path,
        )
        return False

    host_path = zed_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape ZED_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, zed_home)
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="zed-skills-safe-"))
...
def get_cache_directory_mounts(
    ...
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_zed_dir(new_subpath, old_name)
        if host_dir.is_dir():
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.zed``,
    SSH ``<remote_home>/.zed``, etc.) and whether translation is wanted.
