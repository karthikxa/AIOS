...
One ``CDPSupervisor`` runs per Zed ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "zed-dialog-bridge.invalid"
...
(() => {
  if (window.__zedDialogBridgeInstalled) return;
  window.__zedDialogBridgeInstalled = true;
  const ENDPOINT = "http://zed-dialog-bridge.invalid/";
    ...
    def evaluate_runtime(
        ...
        *,
        ...
        try:
            ...
            # Chrome returns the object's description string instead â€” the same
            # graceful degradation path used for ``document.querySelector(...)``
            # results â€” rather than crashing the eval.
