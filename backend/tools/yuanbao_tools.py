"""
yuanbao_tools.py - å…ƒå®å¹³å°å·¥å…·é›†

æä¾›ä»¥ä¸‹å·¥å…·å‡½æ•°ï¼Œä¾› zed-agent çš„ "zed-yuanbao" toolset ä½¿ç”¨ï¼š
  - get_group_info        : æŸ¥è¯¢ç¾¤åŸºæœ¬ä¿¡æ¯ï¼ˆç¾¤åã€ç¾¤ä¸»ã€æˆå‘˜æ•°ï¼‰
  - query_group_members   : æŸ¥è¯¢ç¾¤æˆå‘˜ï¼ˆæŒ‰åæœç´¢ã€åˆ—ä¸¾ botã€åˆ—ä¸¾å…¨éƒ¨ï¼‰
  - search_sticker        : æŒ‰å…³é”®è¯æœç´¢å†…ç½®è´´çº¸ï¼ˆè¿”å›žå€™é€‰åˆ—è¡¨ï¼Œå« sticker_id/name/descriptionï¼‰
  - send_sticker          : å‘å½“å‰ä¼šè¯æˆ–æŒ‡å®š chat_id å‘é€è´´çº¸ï¼ˆTIMFaceElemï¼‰
  - send_dm               : å‘é€ç§èŠæ¶ˆæ¯ï¼ˆæŒ‰æ˜µç§°æŸ¥æ‰¾ç”¨æˆ·å¹¶å‘é€ï¼‰

å¯¹é½ chatbot-web/yuanbao-openclaw-plugin çš„ sticker-search/sticker-send è¡Œä¸ºï¼š
LLM åº”å…ˆç”¨ search_sticker æ‰¾åˆ°åˆé€‚çš„ sticker_idï¼ˆæˆ–ç›´æŽ¥ä¼ ä¸­æ–‡ nameï¼‰ï¼Œå†ç”¨ send_sticker
å‘é€ã€‚ä¸è¦åœ¨æ–‡æœ¬ä¸­å¤¹æ‚è£¸çš„ Unicode emoji å½“ä½œè´´çº¸ã€‚
...
async def get_group_info(group_code: str) -> dict:
    """æŸ¥è¯¢ç¾¤åŸºæœ¬ä¿¡æ¯ï¼ˆç¾¤åã€ç¾¤ä¸»ã€æˆå‘˜æ•°ï¼‰ã€‚"""
    if not group_code:
    ...
    try:
        ...
        return {
            ...
            "note": 'The group is called "æ´¾ (Pai)" in the app.',
        }
...
async def query_group_members(
    ...
    """
    ç»Ÿä¸€çš„ç¾¤æˆå‘˜æŸ¥è¯¢å·¥å…·ï¼ˆå¯¹é½ TS query_session_membersï¼‰ã€‚

    action:
      - find      : æŒ‰æ˜µç§°æ¨¡ç³Šæœç´¢
      - list_bots : åˆ—å‡º bot å’Œå…ƒå® AI
      - list_all  : åˆ—å‡ºå…¨éƒ¨æˆå‘˜
    """
    if not group_code:
        return {"success": False, "error": "group_code is required"}
...
async def search_sticker(query: str = "", limit: int = 10) -> dict:
    """
    åœ¨å†…ç½®è´´çº¸è¡¨ä¸­æŒ‰å…³é”®è¯æ¨¡ç³Šæœç´¢ï¼Œè¿”å›ž Top-N å€™é€‰ã€‚

    è¿”å›žæ¯æ¡å€™é€‰çš„ sticker_id / name / description / package_idï¼Œ
    ä¾› LLM é€‰æ‹©åŽä¼ ç»™ send_stickerã€‚ç©º query æ—¶è¿”å›žå‰ N æ¡ã€‚
    """
...
async def send_sticker(
    ...
    """
    å‘ chat_idï¼ˆç¼ºçœå–å½“å‰ä¼šè¯ï¼‰å‘é€ä¸€å¼ å†…ç½®è´´çº¸ï¼ˆTIMFaceElemï¼‰ã€‚

    Args:
        sticker:   è´´çº¸åç§°ï¼ˆå¦‚ "å…­å…­å…­"ï¼‰æˆ– sticker_idï¼ˆå¦‚ "278"ï¼‰ã€‚ä¸ºç©ºæ—¶éšæœºå‘é€ä¸€å¼ ã€‚
        chat_id:   ç›®æ ‡ä¼šè¯ï¼›ç¼ºçœæ—¶ä½¿ç”¨å½“å‰ä¼šè¯ä¸Šä¸‹æ–‡ï¼ˆZED_SESSION_CHAT_IDï¼‰ã€‚
                   æ ¼å¼ï¼š``direct:{account_id}`` / ``group:{group_code}`` / æˆ–è£¸ account_idã€‚
        reply_to:  ç¾¤èŠåœºæ™¯çš„å¼•ç”¨æ¶ˆæ¯ IDï¼ˆå¯é€‰ï¼‰ã€‚
    ...
    target = (chat_id or "").strip() or get_session_env("ZED_SESSION_CHAT_ID", "")
    if not target:
        return {
            "success": False,
...
async def send_dm(
    ...
    if not resolved_user_id:
        ...
        try:
            ...
            if len(matched) > 1:
                # Multiple matches â€” return candidates for disambiguation
                candidates = [
                    {
                        "user_id": m.get("user_id", ""),
                        "nickname": m.get("nickname", m.get("nick_name", "")),
...
def _check_yuanbao():
    """Toolset availability check â€” True when running in a yuanbao gateway session."""
    try:
        from gateway.session_context import get_session_env
        if get_session_env("ZED_SESSION_PLATFORM", "") == "yuanbao":
            return True
    except Exception:
        pass
...
async def _handle_yb_send_dm(args, **kw):
    ...
    if not group_code:
        try:
            ...
            # chat_id format: "group:<code>" â†’ extract the code part
            if chat_id.startswith("group:"):
                group_code = chat_id.split(":", 1)[1]
        except Exception:
            pass

    # Parse media_files: list of {{"path": str, "is_voice": bool}} â†’ List[Tuple[str, bool]]
...
registry.register(
    ...
    schema={
        "name": "yb_query_group_info",
        "description": (
            "Query basic info about a group (called 'æ´¾/Pai' in the app), "
...
registry.register(
    ...
    schema={
        "name": "yb_query_group_members",
        "description": (
            "Query members of a group (called 'æ´¾/Pai' in the app). "
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "action": {
                    ...
                    "description": (
                        "find â€” search a user by name (use when you need to @mention or look up someone); "
                        "list_bots â€” list bots and Yuanbao AI assistants; "
                        "list_all â€” list all members."
                    ),
                },
                "name": {
                    "type": "string",
...
registry.register(
    ...
    schema={
        ...
        "description": (
            ...
            "and sends the message. Use this when someone asks to privately message / ç§ä¿¡ / DM a user. "
            ...
            "properties": {
                "group_code": {
                    ...
                    "description": (
                        ...
                        "Extract from chat_id: 'group:328306697' â†’ '328306697'. "
...
registry.register(
    ...
    schema={
        "name": "yb_search_sticker",
        "description": (
            "Search the built-in Yuanbao sticker (TIM face / è¡¨æƒ…åŒ…) catalogue by keyword. "
            ...
            "Sticker = è´´çº¸ = TIM face â€” NOT a message reaction. "
        ...
        "parameters": {
            ...
            "properties": {
                "query": {
                    "type": "string",
                    "description": (
                        "Search keyword (Chinese or English, e.g. '666', 'æ¯”å¿ƒ', 'cool', 'åƒç“œ'). "
...
registry.register(
    ...
    schema={
        "name": "yb_send_sticker",
        "description": (
            "Send a built-in sticker (TIMFaceElem / è´´çº¸è¡¨æƒ…) to the current Yuanbao chat. "
            "Call yb_search_sticker first if you don't know the sticker_id/name. "
            "Sticker = è´´çº¸ = TIM face â€” NOT a message reaction. "
            "CRITICAL: Whenever the user asks you to send a sticker / è´´çº¸ / è¡¨æƒ…åŒ…, you MUST "
            "use this tool. DO NOT draw a PNG via execute_code / Pillow / matplotlib and "
            "then call send_image_file â€” that produces a fake 'sticker' image instead of a "
        ...
        "parameters": {
            ...
            "properties": {
                "sticker": {
                    "type": "string",
                    "description": (
                        "Sticker name (e.g. 'å…­å…­å…­', 'æ¯”å¿ƒ', 'ok') or numeric sticker_id "
