#!/usr/bin/env python3
"""Read the in-app terminal pane in the Zed desktop GUI.

The embedded terminal's buffer lives in the desktop renderer (xterm.js), so this
tool round-trips through the gateway's blocking-prompt bridge â€” the same one
...
def read_terminal_tool(
    ...
    if callback is None:
        return tool_error("read_terminal is only available in the Zed desktop app.")
...
def check_read_terminal_requirements() -> bool:
    """Desktop GUI only â€” ZED_DESKTOP is set on the gateway the app spawns."""
    return (os.getenv("ZED_DESKTOP") or "").strip().lower() in ("1", "true", "yes")


READ_TERMINAL_SCHEMA = {
    "name": "read_terminal",
    "description": (
        "Read what's currently shown in the in-app terminal pane of the Zed "
