...
- ``"all"``  â€” applied everywhere (classic prompt injection, exfiltration)
- ``"context"`` â€” applied to context files + memory + tool results
  (promptware / C2 / behavioral hijack; broader detection)
- ``"strict"`` â€” applied to memory writes + skill installs only
...
and MCP responses â€” content the user did not author â€” and we want broad
...
# scope âˆˆ {"all", "context", "strict"}
_PATTERNS: List[Tuple[str, str, str]] = [
    # â”€â”€ Classic prompt injection (applies everywhere) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Role-play / identity hijack (context + strict; common attack
    #    surface in scraped web content and poisoned context files) â”€â”€
    ...
    # "name yourself X" is a Brainworm-specific tell â€” identity override
    # via spec instead of jailbreak.  Anchored on the verb pair so it
    # doesn't match "name your variables" etc.
    (r'\bname\s+yourself\s+\w+', "identity_override", "context"),

    # â”€â”€ C2 / Brainworm-style promptware (context scope) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # Verb-anchored "you must register/connect/report/beacon" â€” the verbs
    ...
    # â€” extremely unusual in legitimate content; near-zero false positive.
    (r'only\s+use\s+one[\s\-]?liners?\b', "anti_forensic_oneliner", "context"),
    (r'never\s+(?:\w+\s+)*(?:create|write)\s+(?:\w+\s+)*(?:script|file)\s+(?:\w+\s+)*disk', "anti_forensic_disk", "context"),
    # Environment-variable unsetting targeting known agent runtimes â€”
    # this is pure attack behavior (Brainworm sub-session bypass).
    (r'unset\s+\w*(?:CLAUDE|CODEX|ZED|AGENT|OPENAI|ANTHROPIC)\w*', "env_var_unset_agent", "context"),

    # â”€â”€ Known C2 / red-team framework names (near-zero false positive
    #    outside security research; warn-only by default) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    (r'\b(?:praxis|cobalt\s*strike|sliver|havoc|mythic|metasploit|brainworm)\b', "known_c2_framework", "context"),
    (r'\bc2\s+(?:server|channel|infrastructure|beacon)\b', "c2_explicit", "context"),
    (r'\bcommand\s+and\s+control\b', "c2_explicit_long", "context"),

    # â”€â”€ Exfiltration via curl/wget/cat with secrets (applies everywhere) â”€â”€
    ...
    # â”€â”€ Persistence / SSH backdoor (strict scope â€” memory + skills) â”€â”€
    (r'authorized_keys', "ssh_backdoor", "strict"),
    (r'\$HOME/\.ssh|\~/\.ssh', "ssh_access", "strict"),
    (r'\$HOME/\.zed/\.env|\~/\.zed/\.env', "zed_env", "strict"),
    (r'(update|modify|edit|write|change|append|add\s+to)\s+.*(?:AGENTS\.md|CLAUDE\.md|\.cursorrules|\.clinerules)', "agent_config_mod", "strict"),
    (r'(update|modify|edit|write|change|append|add\s+to)\s+.*\.zed/(config\.yaml|SOUL\.md)', "zed_config_mod", "strict"),

    # â”€â”€ Hardcoded secrets â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    (r'(?:api[_-]?key|token|secret|password)\s*[=:]\s*["\'][A-Za-z0-9+/=_-]{20,}', "hardcoded_secret", "strict"),
]

# Invisible / bidirectional unicode characters used in injection attacks.
# Aligned with skills_guard.py INVISIBLE_CHARS â€” directional isolates
...
def scan_for_threats(content: str, scope: str = "context") -> List[str]:
    ...
    - ``"all"`` (narrow): classic injection + exfil only â€” minimal false
      positives, suitable for any text.
    - ``"context"`` (default): adds promptware / C2 / role-play patterns â€”
      suitable for context files, memory entries, and tool results.
    - ``"strict"`` (broad): adds persistence / SSH backdoor / exfil-URL
      patterns â€” appropriate for user-mediated writes (memory tool,
    ...
    # Invisible unicode â€” single pass through the content set, not 17
    # ``in`` lookups.
