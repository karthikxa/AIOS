...
The agent writes to two persistent stores that survive across sessions:
  ...
  * **memory** â€” MEMORY.md / USER.md, small (~200 char) declarative entries
  * **skills** â€” SKILL.md + supporting files, potentially huge (10-100 KB)

Both stores are written from two origins:

  * **foreground** â€” a normal agent turn (user is present / chatting)
  * **background_review** â€” the self-improvement review fork that runs after a
...
``write_approval``:
  ...
  * ``false`` (default) â€” write freely (the pre-gate behaviour)
  * ``true``            â€” require approval: do not commit the write; either
...
(see ``zed_cli`` slash handlers): memory shows full content, skills show
...
channel â€” review happens via ``/memory pending``). Foreground CLI memory
writes prompt inline via the dangerous-command approval callback; skill
writes always stage (too big to eyeball mid-loop).

Pending records live under ``<ZED_HOME>/pending/{memory,skills}/<id>.json``
...
# "block all writes" state â€” to disable a subsystem entirely use its own
...
def write_approval_enabled(subsystem: str) -> bool:
    ...
    ``False`` (gate off â€” writes flow freely) for any unset / invalid value so
...
def _pending_dir(subsystem: str) -> Path:
    return get_zed_home() / "pending" / subsystem
...
def stage_write(subsystem: str, payload: Dict[str, Any],
    ...
    Args:
        ...
        summary: a one-line human-readable description shown in pending lists.
            ...
            entry text itself.
        origin: ``foreground`` or ``background_review`` â€” recorded for audit.
    ...
    safe failure for an approval gate â€” nothing is silently committed).
    """
...
class GateDecision:
    ...
    Exactly one of the boolean flags is True:
      * ``allow``  â€” proceed with the real write (gate off, or an inline
        approval was granted).
      * ``blocked`` â€” refuse the write (the user denied an inline approval
        prompt). ``message`` explains why; surface it to the agent.
      * ``stage``  â€” do not write; the caller should stage the payload via
        ``stage_write`` (gate on, and no inline prompt is available â€” gateway,
...
def evaluate_gate(subsystem: str, *, inline_summary: str = "",
    ...
    Decision matrix:
        gate off (default)                    â†’ allow (writes flow freely)
        gate on, memory + interactive CLI     â†’ inline approve/deny prompt
        gate on, memory + gateway/script/bg   â†’ stage
        gate on, skills (any origin)          â†’ stage (too big to review inline)

    Note: there is no config-driven "blocked" outcome â€” the gate only ever
    ...
    # Skills always stage â€” a SKILL.md is too large to review inline, and a
    ...
    if subsystem == SKILLS or background:
        ...
        return GateDecision(
            ...
            message=(
                ...
                f"Not yet saved â€” review with {where}."
            ),
        )

    # Memory + foreground: if an interactive approval channel exists (a CLI
    # approval callback registered on this thread), prompt inline â€” entries
    ...
    if _interactive_approval_available():
        ...
        # granted is None â†’ prompt failed; fall through to staging.

    return GateDecision(
        stage=True,
        message=(
            "Staged for approval (memory.write_approval is on). "
            "Not yet saved â€” review with /memory pending."
        ),
    )
...
def _interactive_approval_available() -> bool:
    ...
    * **Gateway/API sessions** â€” the dangerous-command ``/approve`` round-trip
    ...
    * Scripts, cron, and background threads â€” no user present.
    """
    try:
...
def _prompt_inline_memory_approval(summary: str, detail: str) -> Optional[bool]:
    ...
    available / prompt failed â†’ caller should stage instead).
    ...
    invoked directly â€” NOT via ``prompt_dangerous_approval`` â€” because that
    ...
    if callback is None:
        # No interactive channel on this thread â€” stage rather than risk the
    ...
    if choice == "deny":
        return False
    # Any other outcome (e.g. timeout that returns "deny" already handled) â†’
...
def skill_gist(action: str, name: str, *, content: str = "",
    ...
    Heuristic, no model call â€” the gist surfaces enough to decide approve/reject
    ...
    if action in {"create", "edit"} and content:
        ...
        if desc:
            return f"{verb} '{name}' â€” {desc} ({size})"
...
def skill_pending_diff(record: Dict[str, Any]) -> str:
    ...
    if action == "edit":
        ...
        new = current.replace(old_s, new_s) if current else f"(patch {old_s!r} â†’ {new_s!r})"
