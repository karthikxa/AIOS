...
This module provides the glue:
    - ``ZedTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Zed Agent"
"""
...
# Skip tokens accepted at the paste prompt â€” exit OAuth without auth.
...
_USER_SKIPPED_SENTINEL = "__zed_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses ZED_HOME so each profile gets its own OAuth tokens.
...
def _can_open_browser() -> bool:
    ...
    # Explicit SSH session â†’ no local display
...
# ---------------------------------------------------------------------------
# ZedTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class ZedTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        ZED_HOME/mcp-tokens/<server_name>.json         -- tokens
        ZED_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ZED_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Zed records an absolute wall-clock ``expires_at`` alongside the
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    # the *remote* machine â€” not the user's local machine where the browser
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"    1. Easiest â€” when your browser shows a connection error after\n"
            ...
            f"\n"
            f"  See: https://zed-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
    ...
    if _can_open_browser():
        try:
            ...
            if opened:
                ...
                print("  (Could not open browser â€” please open the URL manually.)\n", file=sys.stderr)
        except Exception:
            print("  (Could not open browser â€” please open the URL manually.)\n", file=sys.stderr)
    else:
        print("  (Headless environment detected â€” open the URL manually.)\n", file=sys.stderr)
...
async def _wait_for_callback() -> tuple[str, str | None]:
    ...
    Raises:
        ...
        RuntimeError: If ``_oauth_port`` has not been set, which would indicate
            that ``build_oauth_auth`` was skipped â€” the asserting form below
            was a silent bug when running Python with ``-O``/``-OO``.
    """
    if _oauth_port is None:
        raise RuntimeError(
            "OAuth callback port not set â€” build_oauth_auth must be called "
    ...
    try:
        ...
        # Port already in use â€” the server from build_oauth_auth is running.
        # Fall back to polling the server started by build_oauth_auth.
        raise OAuthNonInteractiveError(
            "OAuth callback timed out â€” could not bind callback port. "
    ...
    if result["auth_code"] is None:
        raise OAuthNonInteractiveError(
            "OAuth callback timed out â€” no authorization code received. "
...
def _paste_callback_reader(result: dict) -> None:
    ...
    Accepts any of:
      ...
      - A skip token (``skip``, ``cancel``, ``s``, ``n``, ``no``, ``q``, ``quit``)
        â€” exits the OAuth flow cleanly without auth. Caller raises
    ...
    Failures to parse, EOF, or interrupts are swallowed â€” this is best-effort
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `zed mcp login <server>` later to "
    ...
    try:
        ...
        print(
            "  Could not parse pasted input as an OAuth redirect â€” ignoring.",
            file=sys.stderr,
        )
        return
    ...
    if not code and not error:
        print(
            "  Pasted input did not contain ``code=`` or ``error=`` â€” ignoring.",
            file=sys.stderr,
        )
        return
    ...
    result["error"] = error
    if code:
        print("  Got authorization code from paste â€” completing flow.", file=sys.stderr)
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = ZedTokenStorage(server_name)
    storage.remove()
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Zed Agent")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    cfg = dict(oauth_config or {})  # copy â€” we mutate _resolved_port
    storage = ZedTokenStorage(server_name)
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`zed mcp login {server_name}` interactively first to complete "
