...
CHECKPOINT_PATH = get_zed_home() / "processes.json"
...
# Watch pattern rate limiting â€” PER SESSION.
...
WATCH_STRIKE_LIMIT = 3            # Strikes in a row â†’ disable watch + promote to notify_on_complete

# Global circuit breaker â€” across all sessions. Secondary safety net so concurrent
...
class ProcessSession:
    ...
    watcher_message_id: str = ""                # Triggering message id â€” reply anchor for topic routing
    watcher_interval: int = 0                   # 0 = no watcher configured
    notify_on_complete: bool = False             # Queue agent notification on exit
    # Watch patterns â€” trigger agent notification when output matches any pattern
    ...
    # between â€” a strike is a window, not a match). After WATCH_STRIKE_LIMIT strikes
...
class ProcessRegistry:
    ...
    def __init__(self):
        ...
        # Notification queue â€” unified queue for all background process events.
        ...
        # Global watch-match circuit breaker â€” across all sessions.
    ...
    def _check_watch_patterns(self, session: ProcessSession, new_text: str) -> None:
        ...
        notify_on_complete semantics â€” one notification when the process
        ...
        with session._lock:
            ...
            if session._watch_cooldown_until and now < session._watch_cooldown_until:
                ...
                if not session._watch_strike_candidate:
                    # First drop in this window â€” count one strike.
                ...
                # cooldown, reset the consecutive-strike counter â€” we're back to
        ...
        if return_early:
            if should_disable:
                ...
                self.completion_queue.put({
                    ...
                    "message": (
                        f"Watch patterns disabled for process {session.id} â€” "
        ...
        # Global circuit breaker â€” across all sessions (secondary safety net).
    ...
    def _global_watch_admit(self, now: float) -> bool:
        ...
        with self._global_watch_lock:
            ...
            # Still in cooldown â€” drop and count.
            if self._global_watch_tripped_until and now < self._global_watch_tripped_until:
    ...
    def _is_host_pid_alive(pid: Optional[int]) -> bool:
        ...
        if not pid:
            return False
        # ``os.kill(pid, 0)`` is NOT a no-op on Windows (bpo-14484) â€” use
    ...
    def _terminate_host_pid(pid: int) -> None:
        ...
        reuse the POSIX psutil path on Windows because:
          ...
          1. Windows doesn't maintain a Unix-style process tree â€”
          ...
          2. ``psutil.Process.terminate()`` on Windows is
             ...
             and is a hard kill â€” there is no Windows equivalent of a
    ...
    def spawn_local(
        ...
        try:
            ...
            # Post-Popen setup failed â€” kill the orphaned subprocess (and any
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/zed_bg_{session.id}.log"
        pid_path = f"{temp_dir}/zed_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/zed_bg_{session.id}.exit"
    ...
    def _move_to_finished(self, session: ProcessSession):
        ...
        with the reader thread), the second call is a no-op â€” no duplicate
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `zed update` restarting the gateway) is still
        holding the stdout pipe open, the reader blocks forever and poll()
        keeps returning "running" indefinitely (issue #17327 â€” 74 polls over
        7 minutes on Feishu).
        ...
        try:
            ...
            return
        if rc is None:
            return  # Direct child still running â€” reader block is legitimate.
    ...
    def wait(self, session_id: str, timeout: int = None) -> dict:
        ...
        while time.monotonic() < deadline:
            ...
            if session is None:
                return {"status": "not_found", "error": f"No process with ID {session_id}"}
            # Reconcile against real child state â€” guards against orphaned-
    ...
    def _prune_if_needed(self):
        ...
        # tracked at all â€” belt-and-suspenders against module-lifetime growth
...
def _format_async_delegation(evt: dict) -> str:
    ...
    existed, so the block is written to stand entirely on its own â€” enough to
    ...
    lines = [
        f"[ASYNC DELEGATION COMPLETE â€” {deleg_id}]",
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    if _exit in {-15, 143, "-15", "143"}:
        _signal = ", SIGTERM"
    if _reason == "killed":
        _status = f"terminated by {_source or 'Zed'}"
    elif _reason == "lost":
...
def _handle_process(args, **kw):
    ...
    # Coerce to string â€” some models send session_id as an integer
