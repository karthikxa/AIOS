...
skills-hub pipeline for free â€” search, inspect, quarantine, security scan,
install, lock-file provenance, audit log, taps, the centralized index, and
`zed skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.zed.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.zed.blueprint`` is a mapping containing
    ...
    zed = meta.get("zed") if isinstance(meta, dict) else None
    blueprint = zed.get("blueprint") if isinstance(zed, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.zed.blueprint must be a mapping")
...
def register_blueprint_suggestion(spec: BlueprintSpec) -> Optional[Dict[str, Any]]:
    ...
    a skill that carries a ``blueprint:`` block does NOT auto-schedule it â€” it
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.zed.blueprint`` block) they can hand
    to ``zed skills publish`` to share. ``body`` is the plain-language
