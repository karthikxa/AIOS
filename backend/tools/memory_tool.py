...
# (ZED_HOME env var changes) are always respected.  The old module-level
# constant was cached at import time and could go stale if a profile switch
# happened after the first import.
def get_memory_dir() -> Path:
    """Return the profile-scoped memories directory."""
    return get_zed_home() / "memories"
...
def _drift_error(path: "Path", bak_path: str) -> Dict[str, Any]:
    ...
    through the tool's parser/serializer â€” flushing would discard the
    ...
    return {
        ...
        "error": (
            ...
            f"Resolve the drift first â€” either rewrite the file as a clean "
            f"Â§-delimited list of entries, or move the extra content out â€” "
...
class MemoryStore:
    ...
    def load_from_disk(self):
        ...
        entry for injection/promptware patterns at snapshot-build time â€”
        ANY hit replaces the entry text in the snapshot with a placeholder
        like ``[BLOCKED: â€¦]``, so a poisoned-on-disk memory file (supply
        ...
        see poisoned entries by inspecting the source files directly, and remove them â€” silently dropping them would hide the attack from the user.
    ...
    def _sanitize_entries_for_snapshot(entries: List[str], filename: str) -> List[str]:
        ...
        contained threat pattern: <ids>. Removed from system prompt.]"`` â€”
    ...
    def _reload_target(self, target: str) -> Optional[str]:
        ...
        When drift is detected the caller must abort the mutation â€”
    ...
    def add(self, target: str, content: str) -> Dict[str, Any]:
        ...
        with self._file_lock(self._path_for(target)):
            ...
            # â€” refuse the mutation so we don't clobber the un-roundtrippable
            ...
            if new_total > limit:
                ...
                return {
                    ...
                    "error": (
                        ...
                        f"current_entries below), then retry this add â€” all in this turn."
                    ),
            ...
            if new_total > limit:
                ...
                return {
                    ...
                    "error": (
                        ...
                        f"entries to make room (see current_entries below), then retry â€” all "
                        f"in this turn."
                    ),
                    "current_entries": entries,
                    "usage": f"{current:,}/{limit:,}",
    ...
    def _success_response(self, target: str, message: str = None) -> Dict[str, Any]:
        ...
        resp["note"] = "Write saved. This update is complete â€” do not repeat it."
        return resp
    ...
    def _detect_external_drift(self, target: str) -> Optional[str]:
        ...
        wrote, joined by Â§. Detect drift via two signals:

        1. Round-trip mismatch â€” re-parsing and re-serializing the file
           doesn't produce identical bytes (rare; would catch oddly-encoded
           delimiters).
        2. Entry-size overflow â€” any single parsed entry exceeds the
           ...
           content, discarding the appended bytes â€” issue #26045.
        ...
        # Drift confirmed â€” snapshot the file so the operator can recover
        ...
        try:
            ...
            return str(bak_path) + " (BACKUP FAILED â€” file unchanged on disk)"
        return str(bak_path)
...
MEMORY_SCHEMA = {
    ...
    "description": (
        ...
        "checked only on the FINAL result â€” so a single call can remove/replace stale entries "
