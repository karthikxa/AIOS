...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Zed exit
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of zed directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "zed-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = os.getenv("ZED_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("ZED_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (ZED_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (ZED_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # interactive Zed password prompt or the sudo -S password-pipe path.
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, ZedSweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.zed/.env or environment variables."
        )
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior Zed processes that hit SIGKILL / OOM / a closed terminal
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "Zed Tool Gateway access is not currently available and no direct "
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so Zed can track the process, then run "
    ...
    try:
            ...
            if _contains_gateway_lifecycle_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Run `zed gateway restart` from a separate shell outside "
                        "the running gateway."
                    ),
                    "status": "error",
                }, ensure_ascii=False)
        ...
        if background:
            ...
            try:
                ...
                # zed-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("ZED_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("ZED_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("ZED_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("ZED_SESSION_USER_ID", "")
                        _gw_user_name = _gse("ZED_SESSION_USER_NAME", "")
                        _gw_message_id = _gse("ZED_SESSION_MESSAGE_ID", "")
