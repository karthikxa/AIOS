"""
Checkpoint Manager â€” Transparent filesystem snapshots via a single shared
shadow git store.
...
This is NOT a tool â€” the LLM never sees it.  It's transparent infrastructure
...
-----------------------------------------------------------------------------
    ...
    ~/.zed/checkpoints/
        store/                          â€” single bare-ish git repo
            HEAD, config, objects/      â€” standard git internals (shared)
            refs/zed/<hash16>        â€” per-project branch tip
            indexes/<hash16>            â€” per-project git index
            projects/<hash16>.json      â€” {workdir, created_at, last_touch}
            info/exclude                â€” default excludes (shared)
        .last_prune                     â€” auto-prune idempotency marker
        legacy-<timestamp>/             â€” archived pre-v2 per-project shadow
...
CHECKPOINT_BASE = get_zed_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/zed"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Zed convention â€” don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("ZED_CHECKPOINT_TIMEOUT", 30)))

# Max files to snapshot â€” skip huge directories to avoid slowdowns.
_MAX_FILES = 50_000

# Valid git commit hash pattern: 4â€“40 hex chars (short or full SHA-1/SHA-256).
...
def _shadow_repo_path(working_dir: str) -> Path:  # pragma: no cover â€” kept for BC
    ...
    projects â€” per-project isolation lives in refs and indexes, not in
...
def _git_env(
    ...
    The shared store is internal Zed infrastructure â€” it must NOT inherit
    ...
    Isolation strategy:
    * ``GIT_CONFIG_GLOBAL=<os.devnull>`` â€” ignore ``~/.gitconfig`` (git 2.32+).
    * ``GIT_CONFIG_SYSTEM=<os.devnull>`` â€” ignore ``/etc/gitconfig`` (git 2.32+).
    * ``GIT_CONFIG_NOSYSTEM=1`` â€” legacy belt-and-suspenders for older git.
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``zed checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `zed checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    # Use the base dir as the working_dir for config commands â€” it always
    # exists since we just created the store inside it.
    cfg_wd = str(base)
    _run_git(["config", "user.email", "zed@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Zed Checkpoint"], store, cfg_wd)
...
# Backwards-compatibility shim â€” some tests import ``_init_shadow_repo`` and
# look for ``HEAD``/``info/exclude``/``ZED_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at ZED_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "ZED_WORKDIR").write_text(
...
class CheckpointManager:
    ...
    def ensure_checkpoint(self, working_dir: str, reason: str = "auto") -> bool:
        ...
        Never raises â€” all errors are silently logged.
        """
        if not self.enabled:
            return False
    ...
    def _take(self, working_dir: str, reason: str) -> bool:
        ...
        # Quick size guard â€” don't try to snapshot enormous directories
        ...
        # via ``core.bigFileThreshold`` is not what we want â€” instead, we
        ...
        # Compare against the current ref tip (not HEAD â€” HEAD points to a
        ...
        if has_ref:
            ...
            # No ref yet â€” skip only if the index is empty.
        ...
        # Real pruning â€” drop old commits beyond max_snapshots.
    ...
    def _prune(self, store: Path, working_dir: str, ref: str) -> None:
        ...
        was supposed to handle it, but only the log view was limited â€” loose
        ...
        # Collect commits oldest â†’ newest, take last N.
    ...
    def _enforce_size_cap(self, store: Path) -> None:
        ...
        if size <= cap_bytes:
            return
        logger.info(
            "Checkpoint store exceeded %d MB (actual %d MB) â€” pruning oldest",
...
def format_checkpoint_list(checkpoints: List[Dict], directory: str) -> str:
    ...
    lines = [f"ðŸ“¸ Checkpoints for {directory}:\n"]
    ...
    try:
        ...
        if marker.exists():
            try:
                ...
                pass  # corrupt marker â€” treat as no prior run
...
# ---------------------------------------------------------------------------
# Public helpers for `zed checkpoints` CLI
# ---------------------------------------------------------------------------
