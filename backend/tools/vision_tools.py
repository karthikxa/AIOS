...
Zed, Codex, native Anthropic, or a custom OpenAI-compatible endpoint.
...
# Resolution: config.yaml auxiliary.vision.download_timeout â†’ env var â†’ 30s default.
def _resolve_download_timeout() -> float:
    env_val = os.getenv("ZED_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
def _is_retryable_download_error(error: Exception) -> bool:
    ...
    Non-retryable (fail-fast):
      ...
      - ValueError: image too large or blocked redirect â€” deterministic.
...
async def _download_image(image_url: str, destination: Path, max_retries: int = 3) -> Path:
    ...
    for attempt in range(max_retries):
        try:
            ...
            # client error (404/403/410, etc.) will never succeed on retry â€”
            # the resource isn't there or we're not allowed â€” so burning 3
...
# Absolute hard ceiling for vision API payloads (20 MB) â€” above this, no major
...
# can't clear (the bad bytes are immutable history).  Capping at embed time â€”
# with headroom under 5 MB â€” is the only durable fix.  Matches the post-failure
...
# 8000px per-side ceiling INDEPENDENTLY of the 5 MB byte cap â€” a tall full-page
# screenshot can be well under 5 MB yet far over 8000px (e.g. 1200Ã—12000 at
...
def _image_exceeds_dimension(image_path: Path, max_dimension: int) -> bool:
    ...
    unavailable or the file can't be read as an image â€” the byte-based checks
...
def _resize_image_for_vision(image_path: Path, mime_type: Optional[str] = None,
    ...
    if not needs_resize_for_bytes and not needs_resize_for_dims:
        # Small enough â€” just encode directly.
    ...
    try:
        ...
        try:
            ...
            logger.info("Pillow not installed â€” cannot auto-resize oversized image")
            if data_url is None:
                data_url = _image_to_base64_data_url(image_path, mime_type=mime_type)
    ...
    # For PNG, quality is irrelevant â€” only dimension reduction helps.
...
def _supports_media_in_tool_results(provider: str, model: str) -> bool:
    ...
    Providers covered today (per spec docs verified Apr-2026):
      ...
      * Anthropic Messages API (``anthropic`` provider, plus aggregators that
        proxy Claude â€” ``openrouter``, ``nous``, ``vertex``, ``bedrock``):
    ...
    For unknown / legacy providers we conservatively return False â€” the
    ...
    # Aggregators that route to multiple vendors â€” assume support since
    ...
    # Gemini â€” gate on model name; older Gemini variants did not support
...
def _build_native_vision_tool_result(
    ...
    text_part = (
        "Image loaded into your context â€” you can see it natively now. "
...
async def _vision_analyze_native(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            temp_dir = get_zed_dir("cache/vision", "temp_vision_images")
        ...
        # permanently wedges the session â€” retries can't clear bytes (or
        ...
        if _over_bytes or _over_dims:
            ...
            # there's nothing more we can do â€” reject rather than embed a
        ...
        # Only delete temp files we created â€” never user-provided paths.
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $ZED_HOME/cache/vision/ and cleaned up
    ...
    try:
        ...
        # Convert image to base64 â€” send at full resolution first.
        ...
        # Hard limit (20 MB) â€” no provider accepts payloads this large.
        if len(image_data_url) > _MAX_BASE64_BYTES:
        ...
        # Extract the analysis â€” fall back to reasoning if content is empty
        ...
        # Detect vision capability errors â€” give the model a clear message
...
def check_vision_requirements() -> bool:
    ...
    and if that fails, the auto chain (main provider â†’ openrouter â†’ nous).
...
if __name__ == "__main__":
    ...
    """
    print("ðŸ‘ï¸ Vision Tools Module")
    print("=" * 40)
    ...
    if not api_available:
        print("âŒ No auxiliary vision model available")
        print("Configure a supported multimodal backend (OpenRouter, Zed, Codex, Anthropic, or a custom OpenAI-compatible endpoint).")
        sys.exit(1)
    else:
        print("âœ… Vision model available")
    
    print("ðŸ› ï¸ Vision tools ready for use!")
    ...
    if _debug.active:
        ...
        print("ðŸ› Debug mode disabled (set VISION_TOOLS_DEBUG=true to enable)")
...
VISION_ANALYZE_SCHEMA = {
    ...
    "description": (
        ...
        "and you read the pixels yourself on the next turn â€” call this "
...
def _handle_vision_analyze(args: Dict[str, Any], **kw: Any) -> Awaitable[str]:
    ...
    # model sees the pixels directly on its next turn â€” no aux call, no
...
# Extension â†’ MIME. avi/mkv fall back to mp4.
_VIDEO_MIME_TYPES = {
    ".mp4": "video/mp4",
    ".webm": "video/webm",
    ".mov": "video/mov",
...
async def video_analyze_tool(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            if blocked:
                raise PermissionError(blocked["message"])
            temp_dir = get_zed_dir("cache/video", "temp_video_files")
        ...
        if video_size_bytes > _VIDEO_SIZE_WARN_BYTES:
            logger.warning("Video is %.1f MB â€” may be slow or rejected", video_size_mb)
...
VIDEO_ANALYZE_SCHEMA = {
    ...
    "description": (
        ...
        "Use this for video files â€” for images, use vision_analyze instead. "
