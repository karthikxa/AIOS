...
_zed_config_resolved: str | None = None
_zed_config_resolved_loaded = False


def _get_zed_config_resolved() -> str | None:
    """Return the resolved absolute path of the Zed config file (cached)."""
    global _zed_config_resolved, _zed_config_resolved_loaded
    if _zed_config_resolved_loaded:
    ...
    try:
        ...
        try:
            _zed_config_resolved = str(Path("~/.zed/config.yaml").expanduser().resolve())
        except Exception:
            _zed_config_resolved = None
    return _zed_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Zed config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    zed_config = _get_zed_config_resolved()
    if zed_config and (resolved == zed_config or normalized == zed_config):
        return (
            f"Refusing to write to Zed config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.zed/config.yaml directly or use 'zed config' instead."
        )
    return None


def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Zed mirror prefix for Docker file tools."""
    try:
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Zed
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Zed state.
    ...
    * container-mirror (#32049 follow-up) Ã¢â‚¬â€ writes from inside a Docker
    ...
    # in a session that cd'd into ``~/.zed/profiles/other/`` is
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Zed-profile guard for
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Zed profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
