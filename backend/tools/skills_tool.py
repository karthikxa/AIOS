...
# All skills live in ~/.zed/skills/ (seeded from bundled skills/ on install).
...
def _skill_lookup_path_error(name: str) -> Optional[str]:
    ...
    on-disk lookup path, so it must stay relative and free of ``..`` segments â€”
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from ZED_HOME/.env."""
...
# Prompt injection detection â€” shared by local-skill and plugin-skill paths.
...
def skill_matches_environment(frontmatter: Dict[str, Any]) -> bool:
    ...
    Delegates to ``agent.skill_utils.skill_matches_environment`` â€” kept here
...
def _capture_required_environment_variables(
    ...
    # â€” the desktop app / TUI â€” set ZED_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (ZED_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("ZED_INTERACTIVE"):
        return {
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("ZED_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_zed_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.zed/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``ZED_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``ZED_PLATFORM`` environment variable
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.zed/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``zed skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_zed_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
# â”€â”€ Plugin skill serving â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _serve_plugin_skill(
    ...
    *,
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: zed plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    # Injection scan â€” log but still serve (matches local-skill behaviour)
    ...
    # Bundle context banner â€” tells the agent about sibling skills
    try:
        siblings = [
...
def skill_view(
    ...
    try:
        ...
        # â”€â”€ Qualified name dispatch (plugin skills) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        if ":" in name:
            ...
            if plugin_skill_md is not None:
                if not plugin_skill_md.exists():
                    # Stale registry entry â€” file deleted out of band
                    ...
                    return json.dumps(
                        {
                            ...
                            "error": (
                                ...
                                f"been cleaned up â€” try again after the "
            ...
            # Plugin itself not found â€” fall through to flat-tree scan.
        ...
        # the caller â€” silent shadowing of a local skill by a same-named
        ...
        if len(candidates) > 1:
            ...
            logging.getLogger(__name__).warning(
                "Skill name collision for '%s': %d candidates â€” %s",
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Refusing to guess â€” load one explicitly by its categorized path."
                    ),
                    "matches": paths,
                    "hint": (
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.zed/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `zed skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.zed.* first (agentskills.io convention), fall back to top-level
        zed_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            zed_meta = metadata.get("zed", {}) or {}

        tags = _parse_tags(zed_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            zed_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
...
def _skill_view_with_bump(args, **kw):
    ...
    try:
        ...
        if isinstance(parsed, dict) and parsed.get("success"):
            # Use the resolved skill name from the payload when present â€”
            ...
            if resolved:
                ...
                # to act on it â€” that counts as use, not just a browse/view.
