...
Tracks per-skill usage metadata in a sidecar JSON file (~/.zed/skills/.usage.json)
...
# throughout this module. Keep this list tiny and intentional â€” it is not a
...
# ---------------------------------------------------------------------------
# Provenance â€” which skills are agent-created (and thus eligible for curation)
# ---------------------------------------------------------------------------

def _read_bundled_manifest_names() -> Set[str]:
    """Return the set of skill names that were seeded from the bundled repo.

    Reads ~/.zed/skills/.bundled_manifest (format: "name:hash" per line).
...
def _read_hub_installed_names() -> Set[str]:
    ...
    Reads ~/.zed/skills/.hub/lock.json (see tools/skills_hub.py :: HubLockFile).
    """
...
def _prune_builtins_enabled() -> bool:
    ...
    except Exception as e:  # pragma: no cover â€” best-effort config read
...
def read_suppressed_names() -> Set[str]:
    """Built-in skills the curator pruned â€” the re-seeder must leave archived.

    One skill name per line in ``~/.zed/skills/.curator_suppressed``. This is
    what makes pruning a built-in durable: without it, ``zed update`` would
...
def list_agent_created_skill_names() -> List[str]:
    ...
    agent-created usage record â€” their inactivity clock is anchored on first
    ...
    for skill_md in base.rglob("SKILL.md"):
        # Skip Zed metadata, VCS, virtualenv/dependency, and cache dirs
        ...
        if name in hub:
            continue
        # Protected built-ins are never curation candidates â€” exempt from the
...
def list_archived_skill_names() -> List[str]:
    """Enumerate skills in ``~/.zed/skills/.archive/``.

    Archive layout is flat (``.archive/<skill>/``) as set by ``archive_skill``,
    so the directory name is the skill name. Used by ``zed curator
    list-archived`` to help users pass a name to ``zed curator restore``.
    """
...
def is_curation_eligible(skill_name: str) -> bool:
    ...
    NEVER eligible â€” they have an external upstream owner. Protected built-ins
    (``PROTECTED_BUILTIN_SKILLS``) are NEVER eligible regardless of any flag â€”
...
def save_usage(data: Dict[str, Dict[str, Any]]) -> None:
    """Write the usage map atomically. Best-effort â€” errors are logged, not raised."""
    path = _usage_file()
    try:
        path.parent.mkdir(parents=True, exist_ok=True)
...
def seed_record_if_missing(skill_name: str) -> None:
    ...
    archive/stale clock measures non-use FROM THEN â€” not from epoch. No-op when
...
def _mutate(skill_name: str, mutator, *, require_curation_eligible: bool = False) -> None:
    ...
    By default this records telemetry for ANY skill â€” bundled, hub-installed,
    or agent-created â€” because usage tracking is pure observability and is
...
# ---------------------------------------------------------------------------
# Public counter-bump helpers â€” telemetry for ALL skills (observability only)
# ---------------------------------------------------------------------------

def bump_view(skill_name: str) -> None:
    """Bump view_count and last_viewed_at. Called from skill_view().

    Tracks every skill regardless of provenance â€” built-ins and hub skills
...
def archive_skill(skill_name: str) -> Tuple[bool, str]:
    """Move a curator-eligible skill directory to ~/.zed/skills/.archive/.
    ...
    try:
        ...
        # Cross-device â€” fall back to shutil.move
        import shutil
        try:
...
def restore_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an archived skill back to ~/.zed/skills/. Restores to the flat
    ...
    skill â€” that would shadow the upstream version. Also refuses to restore
    ...
    """
    # Hub skills always have an external upstream owner â€” never shadow them.
    if is_hub_installed(skill_name):
        return False, (
    ...
    if not candidates:
        ...
        # startswith(f"{skill_name}-") also swallows unrelated sibling skills â€”
...
def _find_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    Handles both flat (~/.zed/skills/<skill>/SKILL.md) and category-nested
    (~/.zed/skills/<category>/<skill>/SKILL.md) layouts.
    """
    base = _skills_dir()
    if not base.exists():
        return None
...
# ---------------------------------------------------------------------------
# Reporting â€” for the curator CLI / slash command
# ---------------------------------------------------------------------------
...
def provenance(skill_name: str) -> str:
    ...
    'agent' covers both agent-authored and local manually-authored skills â€”
...
def usage_report() -> List[Dict[str, Any]]:
    ...
    candidates), this surfaces all skills â€” bundled built-ins and
    hub-installed included â€” so callers can answer "how often is this skill
