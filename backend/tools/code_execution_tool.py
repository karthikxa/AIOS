...
Lets the LLM write a Python script that calls Zed tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `zed_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `zed_tools.py` with file-based RPC stubs
...
# sandbox RPC transport (AF_UNIX is unreliable on Windows Python) â€” see
# ``_use_tcp_rpc`` in ``_execute_local`` below.  That makes execute_code
# available on every platform Zed itself runs on.
...
# Resource limit defaults (overridable via config.yaml â†’ code_execution.*)
...
# match a safe prefix, the operational ZED_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "ZED_" prefix was deliberately removed (#27303) â€” it leaked
# ZED_*-named config that lacks a secret substring (e.g. ZED_BASE_URL,
# ZED_KANBAN_DB, ZED_*_WEBHOOK).  The child only needs the few
# location/profile vars in _ZED_CHILD_ALLOWED below; ZED_RPC_SOCKET /
# ZED_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational ZED_* vars the child legitimately needs by exact name â€” these
# are non-secret runtime-location flags (the same set zed_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_ZED_CHILD_ALLOWED = frozenset({
    "ZED_HOME",
    "ZED_PROFILE",
    "ZED_CONFIG",
    "ZED_ENV",
...
_WINDOWS_ESSENTIAL_ENV_VARS = frozenset({
    "SYSTEMROOT",       # %SYSTEMROOT%\System32 â€” Winsock needs this
    "SYSTEMDRIVE",      # C: (or wherever Windows lives)
    "WINDIR",           # usually same as SYSTEMROOT
    "COMSPEC",          # cmd.exe path â€” subprocess shell=True needs it
    "PATHEXT",          # .COM;.EXE;.BAT;... â€” shell lookup
    "OS",               # "Windows_NT" â€” some tools gate on this
    "PROCESSOR_ARCHITECTURE",
    "NUMBER_OF_PROCESSORS",
    "PUBLIC",           # C:\Users\Public
    "ALLUSERSPROFILE",  # C:\ProgramData â€” some stdlib paths use it
    "PROGRAMDATA",      # C:\ProgramData
    "PROGRAMFILES",
    "PROGRAMFILES(X86)",
    "PROGRAMW6432",
    "APPDATA",          # %USERPROFILE%\AppData\Roaming â€” Python uses it
    "LOCALAPPDATA",     # %USERPROFILE%\AppData\Local
    "USERPROFILE",      # C:\Users\<name> â€” Python's expanduser uses it
    "USERDOMAIN",
    "USERNAME",
    "HOMEDRIVE",        # C:
    "HOMEPATH",         # \Users\<name>
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational ZED_* vars (_ZED_CHILD_ALLOWED) pass by exact name.
      5. On Windows, a small OS-essential allowlist passes by exact name
         â€” without these the child can't even create a socket or spawn a
         subprocess.
    ...
    scrubbed = {}
    # Non-secret ZED_* vars dropped by the tightened allowlist (#27303). The
    # broad "ZED_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like ZED_KANBAN_DB / ZED_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _ZED_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("ZED_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist â€” a deliberately-dropped ZED_* var.
            _dropped_zed.append(k)
    if _dropped_zed:
        logger.debug(
            "execute_code: dropped %d non-allowlisted ZED_* var(s) from the "
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Zed-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Zed-profile soft guard."""',
...
def generate_zed_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the zed_tools.py stub module.
...
"""Auto-generated Zed tools RPC stubs."""
...
def _connect():
    ...
    ZED_RPC_SOCKET can be either:
      - a filesystem path (POSIX Unix domain socket â€” the default on
        Linux and macOS)
      - a string of the form ``tcp://127.0.0.1:<port>`` (Windows, where
        AF_UNIX is unreliable â€” the parent falls back to loopback TCP)
    """
    global _sock
    if _sock is None:
        endpoint = os.environ["ZED_RPC_SOCKET"]
        if endpoint.startswith("tcp://"):
            # tcp://host:port  (host is always 127.0.0.1 in practice â€” we
...
"""Auto-generated Zed tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("ZED_RPC_DIR") or os.path.join(tempfile.gettempdir(), "zed_rpc")
_seq = 0
...
def _ship_file_to_remote(env, remote_path: str, content: str) -> None:
    ...
    Uses ``echo â€¦ | base64 -d`` rather than stdin piping because some
...
def _execute_remote(
    ...
    The script and the generated zed_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/zed_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_zed_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/zed_tools.py", tools_src)
        _ship_file_to_remote(env, f"{sandbox_dir}/script.py", code)

        # Wrapped so the thread inherits the turn's approval context + callbacks
        # (see tools.thread_context) â€” else sandbox RPC tool calls lose approval
        # routing (#33057).
        ...
        env_prefix = (
            f"ZED_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("ZED_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
    ...
    if status == "timeout":
        ...
        # to the user (see local path comment â€” same reasoning, #10807).
        if stdout_text:
            result["output"] = stdout_text + f"\n\nâ° {timeout_msg}"
        else:
            result["output"] = f"â° {timeout_msg}"
        logger.warning(
        ...
        result["output"] = (
            stdout_text + "\n[execution interrupted â€” user sent a new message]"
        )
    elif exit_code != 0:
        result["status"] = "error"
...
def execute_code(
    ...
    # --- Set up temp directory with zed_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="zed_sandbox_")
    ...
    # on the same temp drive as the script).  Fall back to loopback TCP â€”
    ...
    # selector from ZED_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"zed_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated zed_tools module.
        # encoding="utf-8" is required on Windows â€” the stub and user code
        ...
        # tools, or SANDBOX_ALLOWED_TOOLS as fallback â€” see lines above).
        tools_src = generate_zed_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "zed_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  ZED_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        # Wrapped so the thread inherits the turn's approval context + callbacks
        # (see tools.thread_context) â€” else gateway sandbox tool calls silently
        ...
        # passed through â€” without those, the child can't create a socket
        # or spawn a subprocess.  See ``_scrub_child_env`` for the rules.
        child_env = _scrub_child_env(os.environ)
        child_env["ZED_RPC_SOCKET"] = rpc_endpoint
        ...
        # ``print("cafÃ©")`` or ``print("â†’")`` crashes with:
        #
        ...
        # Ensure the zed-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from zed_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _zed_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _zed_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set â€”
        # ZED_TIMEZONE is an internal Zed setting and must not leak
        # into child processes.
        _tz_name = os.getenv("ZED_TIMEZONE", "").strip()
        if _tz_name:
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.zed/.env')).
...
def _load_config() -> dict:
    ...
    ``zed --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    """Return the active execute_code mode â€” 'project' or 'strict'.
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (zed-agent's python). Reproducible and the
...
def _resolve_child_python(mode: str) -> str:
    ...
    In ``strict`` mode, always ``sys.executable`` â€” guaranteed to work and
        ...
        for subdir in subdirs:
            for exe in exe_names:
                ...
                if _is_usable_python(candidate):
                    return candidate
                # Found the interpreter but it failed the version check â€”
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``zed tools`` (e.g. web is turned off),
    the schema description should NOT mention web_search / web_extract â€”
    ...
    # temp-dir staging and zed-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD â€” use absolute paths "
            "(os.path.expanduser('~/.zed/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Zed tools programmatically. "
        ...
        f"Available via `from zed_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed â€” built into zed_tools):\n"
        "  json_parse(text: str) â€” json.loads with strict=False; use for terminal() output with control chars\n"
        "  shell_quote(s: str) â€” shlex.quote(); use when interpolating dynamic strings into shell commands\n"
        "  retry(fn, max_attempts=3, delay=2) â€” retry with exponential backoff for transient failures"
    )
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from zed_tools import {import_str}` "
