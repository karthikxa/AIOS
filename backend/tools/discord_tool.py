...
with the bot token â€” no dependency on the gateway adapter's client.

Only included in the zed-discord toolset, so it has zero cost
...
1. Privileged intents detected from GET /applications/@me at schema
   ...
   (search_members / member_info â†’ GUILD_MEMBERS intent) are hidden.
...
Per-guild permissions (MANAGE_ROLES etc.) are NOT pre-checked â€” Discord
...
# Application flag bits (from GET /applications/@me â†’ "flags").
...
def _detect_capabilities(token: str, *, force: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        caps["detected"] = True
    except Exception as exc:  # nosec â€” detection is best-effort
        logger.info(
...
# Single-source-of-truth manifest: action â†’ (signature, one-line description).
...
def _build_schema(
    ...
    Returns ``None`` when *actions* is empty â€” callers should drop the
...
def get_dynamic_schema() -> Optional[Dict[str, Any]]:
    """Backward-compat wrapper â€” returns core schema."""
...
_ACTION_403_HINT = {
    ...
    "search_members": (
        "Likely missing the Server Members privileged intent â€” enable it in the "
...
def _run_discord_action(
    ...
    # Config-level allowlist gate (defense in depth â€” schema already filtered,
