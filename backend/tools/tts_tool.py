...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.zed/config.yaml``. Zed
...
Configuration is loaded from ~/.zed/config.yaml under the 'tts:' key.
...
def _import_elevenlabs():
    ...
    try:
        ...
        # lazy_deps module itself missing â€” fall through to the raw import
...
def _get_default_output_dir() -> str:
    ...
    return str(get_zed_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.zed/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.zed/config.yaml.
    ...
    try:
        ...
        logger.debug("zed_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
# custom voice-cloning scripts, etc.) into Zed without any Python code
...
#           output_format: wav
#
# Zed writes the input text to a temp UTF-8 file, runs the command with
...
def _dispatch_to_plugin_provider(
    ...
    2. Command-type providers declared under
       ...
       own command-provider check returned None â€” we re-verify here so
    ...
    Plugin exceptions are caught and re-raised â€” the outer
    ...
    # Resolve voice / model / format from tts_config â€” providers should
    ...
    # Defensive against a provider returning None or a non-string â€”
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__ZED_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
_XAI_FIRST_SENTENCE_RE = re.compile(r"^(.{12,120}?[.!?â€¦])\s+(?=\S)", flags=re.DOTALL)
...
def _apply_xai_auto_speech_tags(text: str) -> str:
    ...
    system_prompt = (
        ...
        "- Do not use angle-bracket tags like `<tag>...</tag>` â€” xAI uses BBCode-style closing tags with `[/tag]`.\n"
        "- Do not use SSML.\n"
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `zed model` or set XAI_API_KEY.")
    ...
    # misconfigured agent can't 400 the request â€” the API would reject
    ...
    # output_format when Zed actually needs a non-default format/override.
...
def _generate_neutts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # NeuTTS outputs WAV natively â€” use a .wav path for generation,
    ...
    if wav_path != output_path:
        ...
        if ffmpeg:
            ...
            # No ffmpeg â€” just rename the WAV to the expected path
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Zed caches Piper voice models.

    Resolves to ``~/.zed/cache/piper-voices/`` under the active
    ZED_HOME so voice downloads follow profile boundaries.
    """
    from zed_constants import get_zed_dir
    root = Path(get_zed_dir("cache/piper-voices", "piper_voices_cache"))
...
def _resolve_piper_voice_path(voice: str, download_dir: Path) -> str:
    ...
    if not cached.exists():
        raise RuntimeError(
            f"Piper voice download completed but {cached} is missing â€” "
...
def _generate_piper_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # to 0 (Piper's own default). Booleans are rejected outright â€” True/False
    ...
    # speaker_id is applied per-call via syn_config.speaker_id â€” the same
    ...
    # Optional synthesis knobs â€” only pass a SynthesisConfig when at least
    ...
    if has_advanced:
        try:
            ...
            logger.warning(
                ...
                "version â€” advanced knobs ignored"
            )
    ...
    if wav_path != output_path:
        ...
        if ffmpeg:
            ...
            # No ffmpeg â€” keep WAV and return that path
            os.rename(wav_path, output_path)
...
def _generate_kittentts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if wav_path != output_path:
        ...
        if ffmpeg:
            ...
            # No ffmpeg â€” rename the WAV to the expected path
            os.rename(wav_path, output_path)
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.zed/config.yaml (tts: section).
    ...
    platform = get_session_env("ZED_SESSION_PLATFORM", "").lower()
    ...
    if output_path:
        ...
        # prompt-injection-controlled â€” e.g.
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'zed-agent[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run zed setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'zed setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'zed tools' and select Piper under TTS, or install manually: "
...
if __name__ == "__main__":
    print("ðŸ”Š Text-to-Speech Tool Module")
    print("=" * 50)
    ...
    print(f"  ffmpeg:     {'âœ… found' if _has_ffmpeg() else 'âŒ not found (needed for Telegram Opus)'}")
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_zed_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
