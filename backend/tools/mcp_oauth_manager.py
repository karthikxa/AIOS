...
- **Reconnect signalling** for long-lived MCP sessions. The manager itself
  does not drive reconnection â€” the `MCPServerTask` in `mcp_tool.py` does â€”
...
`mcp_tool.py`, and `zed_cli/mcp_config.py`. This module is the ONLY place
that instantiates the MCP SDK's `OAuthClientProvider` â€” all other code paths
...
# ---------------------------------------------------------------------------
# ZedMCPOAuthProvider â€” OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_zed_provider_class() -> Optional[type]:
    ...
    except ImportError:  # pragma: no cover â€” SDK required in CI
        return None

    class ZedMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._zed_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            whose AS is a different origin â€” BetterStack's MCP lives at
            ...
            Paired with :class:`ZedTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, ZedTokenStorage)
            ...
            # metadata â€” i.e. the exact scenario where the SDK's built-in
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import ZedTokenStorage
                        if isinstance(storage, ZedTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._zed_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import ZedTokenStorage
            if not isinstance(storage, ZedTokenStorage):
                return
        ...
        async def async_auth_flow(self, request):  # type: ignore[override]
            ...
            # Any failure here is non-fatal â€” we just log and proceed with
    ...
    return ZedMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_ZED_PROVIDER_CLS: Optional[type] = _make_zed_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`ZedMCPOAuthProvider` directly using the helpers
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `zed mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``zed mcp remove <name>`` and (indirectly) by
        ``zed mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
    ...
    async def handle_401(
        ...
        Returns:
            True  if a (possibly new) access token is now available â€” caller
                  should trigger a reconnect and retry the operation.
            False if no recovery path exists â€” caller should surface a
            ...
            if pending is None:
                ...
                async def _do_handle() -> None:
                    try:
                        ...
                        # Step 2: No disk change â€” if the SDK can refresh
