...
# ---------------------------------------------------------------------------
# Write-path deny list â€” blocks writes to sensitive system/credential files
# ---------------------------------------------------------------------------
...
_FENCE_MARKER_RE = re.compile(r"'?\x07?__ZED_FENCE_[A-Za-z0-9]+__\x07?'?")
...
def _strip_terminal_fence_leaks(text: str) -> str:
    ...
    for line in text.splitlines(keepends=True):
        had_terminal_wrapper = "__ZED_FENCE_" in line or "\x1b]" in line
...
def _detect_line_ending(sample: str) -> Optional[str]:
    ...
    patch operations â€” without this the agent's bare-LF tool args
    silently normalize Windows-line-ending files, and patch produces
    mixed endings when only a substituted region changes.
    """
    if not sample:
        return None
    # Look at the first chunk â€” enough to tell, cheap to scan.
    head = sample[:4096]
    if "\r\n" in head:
        return "\r\n"
    if "\n" in head:
...
# write when the original file had one â€” exactly mirroring the line-ending
...
class SearchResult:
    ...
    def _densify_matches(self) -> Optional[str]:
        ...
        lossless â€” every path, line number, and content byte is preserved â€” and
...
def _split_tool_diagnostics(output: str) -> tuple[str, str]:
    ...
    that look like real search output â€” a match line (``file:line:content``),
    ...
    exit-2 guard distinguish a pure failure (no usable payload â†’ surface the
    error) from a partial failure (some files matched, one was unreadable â†’
...
#   "cannot find package" â€” already partially handled by
...
# extensions â€” the ``lsp_diagnostics`` channel carries the real signal.
...
# couldn't actually run â€” e.g. ``npx tsc`` when tsc isn't installed in
...
def _lint_json_inproc(content: str) -> tuple[bool, str]:
    ...
    except Exception as e:  # noqa: BLE001 â€” any parse failure is a lint failure
...
def _lint_yaml_inproc(content: str) -> tuple[bool, str]:
    ...
    Skipped gracefully if PyYAML isn't installed â€” YAML parsing is optional.
    """
    try:
        import yaml as _yaml
    except ImportError:
        # PyYAML not available â€” skip silently, caller treats as no linter.
...
def _lint_python_inproc(content: str) -> tuple[bool, str]:
    ...
    ast module rejects â€” matching py_compile's scope but with no
...
# present â€” no subprocess overhead, microseconds per call.  Each callable
...
class ShellFileOperations(FileOperations):
    ...
    def __init__(self, terminal_env, cwd: str = None):
        ...
        Args:
            ...
            cwd: Optional explicit fallback cwd when the terminal env has
                 no cwd attribute (rare â€” most backends track cwd live).
        ...
        Note:
            ...
            when the env has no cwd at all â€” it is NOT the authoritative
    ...
    def _exec(self, command: str, cwd: str = None, timeout: int = None,
        ...
        Cwd resolution order (critical â€” see class docstring):
        ...
        terminal's current directory â€” not the directory this file_ops was
    ...
    def _add_line_numbers(self, content: str, start_line: int = 1) -> str:
        ...
        (the model hand-counted and was off-by-one, 3/4) â€” so we keep the
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        ``.zed-tmp`` file next to the user's data, and the original file
        ...
        tmpl = self._escape_shell_arg(".zed-tmp.XXXXXX")
        ...
        #    silent best-effort â€” a perms-copy failure must not abort the
        ...
        script = (
            ...
            '|| mktemp "$d/.zed-tmp.$$.XXXXXX" 2>/dev/null '
            '|| { tmp="$d/.zed-tmp.$$"; : > "$tmp" && echo "$tmp"; })"; '
    ...
    def _detect_file_line_ending(self, path: str, pre_content: Optional[str] = None) -> Optional[str]:
        ...
        for lint/LSP purposes), inspect that â€” zero extra exec calls.
        ...
        # File may not exist (new write) â€” `head` exits 0 with empty
    ...
    def read_file(self, path: str, offset: int = 1, limit: int = 500) -> ReadResult:
        ...
        # Images are never inlined â€” redirect to the vision tool
    ...
    def _suggest_similar_files(self, path: str) -> ReadResult:
        ...
        scored: list = []  # (score, filepath) â€” higher is better
    ...
    def read_file_raw(self, path: str) -> ReadResult:
        ...
        if self._is_likely_binary(path, sample_output):
            return ReadResult(
                ...
                error="Binary file â€” cannot display as text."
            )
        ...
        # back out â€” it re-probes the on-disk file, which still has the
        # marker â€” so the round-trip preserves it.
    ...
    def delete_file(self, path: str) -> WriteResult:
        ...
        don't ship ``rm``. Directories are rejected here â€” use
    ...
    def _python_delete(self, path: str, recursive: bool) -> WriteResult:
        ...
        # We can't shell out to ``rm`` here â€” it doesn't exist on Windows
        ...
        snippet = (
            ...
            "    else:\n"
            # NOTE: avoid ``unlink(missing_ok=True)`` â€” that kwarg lands in
    ...
    def write_file(self, path: str, content: str) -> WriteResult:
        ...
        files. The content never appears in the shell command string â€”
        ...
        write are surfaced â€” pre-existing problems are filtered out so
        ...
        #      map â€” pre-existing diagnostics below the edit point shift
        ...
        # â”€â”€ Line-ending preservation (Roo Code pattern) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ BOM preservation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        # If the file on disk started with a UTF-8 BOM, keep it. read_file
        # strips the BOM so the agent never sees it, which means the
        # content it hands back to write_file / patch has no BOM either â€”
        ...
        # of a half-written corrupt file. Same-directory is load-bearing â€”
        ...
        # clean the temp up on any failure so we never leak a ``.zed-tmp``
        ...
        # Semantic diagnostics from the LSP layer â€” separate channel.
    ...
    def patch_replace(self, path: str, old_string: str, new_string: str,
        ...
        # â”€â”€ Line-ending preservation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # Post-write verification â€” re-read the file and confirm the bytes we
    ...
    def _check_lint(self, path: str, content: Optional[str] = None) -> LintResult:
        ...
        TOML) when possible â€” those parse via the Python stdlib in
        ...
        if inproc is not None:
            # Need content â€” either passed in or read from disk.
            if content is None:
                read_cmd = f"cat {self._escape_shell_arg(path)} 2>/dev/null"
                read_result = self._exec(read_cmd)
                if read_result.exit_code != 0:
        ...
        if ext in _SHELL_LINTER_LSP_REDUNDANT and self._lsp_will_handle(path):
            return LintResult(
                skipped=True,
                message=f"LSP server handles {ext} â€” shell linter skipped",
            )
        ...
        if result.exit_code != 0 and _looks_like_linter_unusable(base_cmd, result.stdout):
            ...
            # not a real lint failure â€” surface it as ``skipped`` so the
            # write doesn't get flagged AND so the LSP tier still runs.
            from tools.ansi_strip import strip_ansi
            cleaned = strip_ansi(result.stdout).strip()
            # Collapse to a single line â€” the npx banner is multi-line ASCII.
            first_line = next(
    ...
    def _check_lint_delta(self, path: str, pre_content: Optional[str],
        ...
        Args:
            ...
            pre_content: File content BEFORE the write.  Pass None for new
                         files or when the pre-state isn't available â€” the
        ...
        if pre.success or pre.skipped or not pre.output:
            # Pre-write was clean (or we couldn't lint it) â€” post errors
        ...
        # later ones â€” if the pre-existing error blocks parsing before
        ...
        # new on top â€” the agent knows it's inherited state, not fresh
        ...
        if not post_lines:
            # Every error in post was also in pre â€” this edit didn't make
            # anything obviously worse, but the file remains broken and
            # the agent should know.
            return LintResult(
                success=False,
                output=post.output,
                message="Pre-existing lint errors â€” this edit didn't introduce new ones but the file is still broken.",
            )
    ...
    def _lsp_local_only(self) -> bool:
        ...
        LSP servers run on the host process â€” they need access to the
    ...
    def _lsp_handles_extension(self, ext: str) -> bool:
        ...
        Safe to call on remote backends â€” the registry is purely
    ...
    def _lsp_will_handle(self, path: str) -> bool:
        ...
        Stronger than :meth:`_lsp_handles_extension` â€” that one only checks
        ...
        runs as before â€” never suppress lint based on an LSP probe that
    ...
    def _snapshot_lsp_baseline(self, path: str) -> None:
        ...
        Best-effort.  Silent on every failure path â€” LSP is an
        enrichment layer and must never break a write.

        Skipped entirely on non-local backends (Docker, Modal, SSH,
        etc.) â€” the server can't see files inside the sandbox.
        """
        if not self._lsp_local_only():
            return
        try:
    ...
    def _maybe_lsp_diagnostics(
        ...
        *,
        ...
        â€” the calling tier already returned a clean syntax result, so
        ``""`` here just means "no extra info to add".

        Skipped entirely on non-local backends (Docker, Modal, SSH,
        etc.) â€” same reasoning as ``_snapshot_lsp_baseline``.
        """
        if not self._lsp_local_only():
            return ""
        try:
        ...
        # Build a line-shift map when we have both pre and post â€” it
