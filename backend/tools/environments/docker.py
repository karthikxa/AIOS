...
from tools.environments.local import _ZED_PROVIDER_ENV_BLOCKLIST
...
# Docker label values must match [a-zA-Z0-9_.-] and stay â‰¤63 chars to round-trip
...
def _sanitize_label_value(value: str) -> str:
    """Coerce *value* into a Docker label-safe form (alnum + ``_.-``, â‰¤63 chars).
...
def _get_active_profile_name() -> str:
    """Return the active Zed profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale zed-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=zed-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped â€” they may
      belong to a sibling Zed process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=zed-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a zed process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Zed
    process leaves its container behind permanently because there's no
    subsequent Zed process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=zed-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=zed-profile={_sanitize_label_value(profile_filter)}"])
    ...
    for cid in candidate_ids:
        ...
        if finished_at is None:
            # Couldn't determine age â€” be conservative and leave it alone.
            continue
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``ZED_DOCKER_BINARY`` env var â€” explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("ZED_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using ZED_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'zed'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``zed-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.zed/sandboxes/). Non-persistent
        ...
        try:
            ...
            for mount_entry in get_credential_file_mounts():
                ...
                if src.is_dir():
                    ...
                    logger.warning(
                        "Docker: skipping credential mount â€” source is a directory "
                ...
                if not src.is_file():
                    logger.warning(
                        "Docker: skipping credential mount â€” source not found: %s", src,
                    )
                    continue
                volume_args.extend([
                    "-v",
            ...
            for skills_mount in get_skills_directory_mount():
                ...
                if not src.is_dir():
                    logger.warning(
                        "Docker: skipping skills mount â€” source is not a directory: %s",
                        src,
                    )
                    continue
                volume_args.extend([
            ...
            # cached media from inside the container.  Read-only â€” the
            # container reads these but the host gateway manages writes.
            for cache_mount in get_cache_directory_mounts():
                src = Path(cache_mount["host_path"])
                if not src.is_dir():
                    logger.warning(
                        "Docker: skipping cache mount â€” source is not a directory: %s",
                        src,
                    )
                    continue
                volume_args.extend([
        ...
        # Explicit environment variables (docker_env config) â€” set at container
        ...
        if run_as_host_user:
            ...
            if user_spec is not None:
                ...
                # Fall back to the full cap set â€” without --user, an image's
        ...
        # s6-overlay images (e.g. zed-agent:latest) already use /init as PID 1
        ...
        if image_uses_s6_init:
            logger.info(
                "Docker: image %s uses /init (s6-overlay) as entrypoint â€” "
        ...
        container_name = f"zed-{uuid.uuid4().hex[:8]}"
        # Labels make zed-created containers identifiable to:
        #   * the orphan reaper (`zed-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`zed-task-id`, `zed-profile`)
        #   * operators running `docker ps --filter label=zed-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Zed profile is captured at
        ...
        label_args = [
            "--label", "zed-agent=1",
            "--label", f"zed-task-id={task_label}",
            "--label", f"zed-profile={profile_name}",
        ]
        ...
        self._labels = {
            "zed-agent": "1",
            "zed-task-id": task_label,
            "zed-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 â€” docs claim "ONE long-lived
        # container shared across sessions").  If a prior Zed process
        ...
        #
        # Reuse matches on labels only â€” we deliberately do NOT compare image
                ...
                if state != "running":
                    try:
                        ...
                        logger.warning(
                            ...
                            "%s â€” falling back to a fresh container.",
        ...
        if not reused:
            # tini/catatonit as PID 1 reaps zombie children â€” but s6-overlay
            ...
            run_cmd = [
                ...
                *all_run_args,
                image,
                "sleep", "infinity",  # no fixed lifetime â€” idle reaper handles cleanup
            ]
            ...
            try:
                ...
                # "Created" state â€” which the exited-only orphan reaper
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Zed secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _ZED_PROVIDER_ENV_BLOCKLIST)
        zed_env = _load_zed_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = zed_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        logger.warning(
            "Container %s appears to be gone â€” attempting recovery", old_id,
        )
        self._container_id = None

        # 1. Try label-based reuse (another process may have recreated it).
        task_label = self._labels.get("zed-task-id", "")
        profile_label = self._labels.get("zed-profile", "")
        ...
        # 2. No reusable container â€” create a fresh one.
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"zed-{_uuid.uuid4().hex[:8]}"
        ...
        logger.info("Recovery successful â€” new container %s", (self._container_id or "")[:12])
        return True
    ...
    def _storage_opt_supported() -> bool:
        ...
        try:
            ...
            # Probe by attempting a dry-ish run â€” the fastest reliable check.
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``zed-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=zed-agent=1",
                    "--filter", f"label=zed-task-id={task_label}",
                    "--filter", f"label=zed-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
            ...
            logger.debug("docker ps probe failed: %s â€” will start a fresh container", e)
            return None
        if result.returncode != 0:
            logger.debug(
                "docker ps probe returned %d: %s â€” will start a fresh container",
        ...
        # container) but can happen if a previous Zed process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Zed exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Zed process touches a labeled container for ``2 Ã— lifetime_seconds``
        it gets ``docker rm -f``'d at the next Zed startup. That covers the
        ...
        #
        #   force_remove=True             â†’ stop + rm (explicit teardown)
        #   persist_across_processes=True â†’ no-op (leave container running)
        #   persist_across_processes=False â†’ stop + rm (per-process isolation)
        #
        # The persist-mode no-op is the issue-#20561 contract: the container
        # outlives Zed processes, processes inside it stay alive, and
        ...
        # Capture state needed by the worker before we null out the attrs â€”
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"zed-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits â€” without this, ``zed /quit``
