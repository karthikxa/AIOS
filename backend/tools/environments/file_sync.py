...
try:
    ...
    fcntl = None  # Windows â€” file locking skipped
...
_FORCE_SYNC_ENV = "ZED_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.zed") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.zed to *container_base* because the remote
...
_SYNC_BACK_MAX_BYTES = 2 * 1024 * 1024 * 1024  # 2 GiB â€” refuse to extract larger tars
...
class FileSyncManager:
    ...
    Not used by bind-mount backends (Docker, Singularity) â€” those get
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``ZED_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, zed_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.zed/`` directory as a tar archive,
        ...
        # Nothing was ever committed through this manager â€” the initial
        # push failed or never ran. Skip sync_back to avoid retry storms
        # against an uninitialized remote .zed/ directory.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state â€” skipping")
            return

        lock_path = (zed_home or get_zed_home()) / ".sync.lock"
    ...
    def _sync_back_once(self, lock_path: Path) -> None:
        ...
        # contexts cleanup() may run from a worker thread â€” skip SIGINT
    ...
    def _sync_back_locked(self, lock_path: Path) -> None:
        ...
        if fcntl is None:
            # Windows: no flock â€” run without serialization
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            if tar_size > _SYNC_BACK_MAX_BYTES:
                logger.warning(
                    "sync_back: remote tar is %d bytes (cap %d) â€” skipping extraction",
                    tar_size, _SYNC_BACK_MAX_BYTES,
                )
                return

            with tempfile.TemporaryDirectory(prefix="zed-sync-back-") as staging:
                    ...
                    for fname in filenames:
                        ...
                        if os.path.exists(host_path) and pushed_hash is not None:
                            ...
                            if host_hash != pushed_hash:
                                logger.warning(
                                    "sync_back: conflict on %s â€” host modified "
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.zed/skills/a.md`` â†’
        ``~/.zed/skills/a.md``, a new remote file at
        ``/root/.zed/skills/b.md`` maps to ``~/.zed/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
