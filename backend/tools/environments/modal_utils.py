"""Shared Zed-side execution flow for Modal transports.

This module deliberately stops at the Zed boundary:
- command preparation
...
def wrap_modal_stdin_heredoc(command: str, stdin_data: str) -> str:
    ...
    marker = f"ZED_EOF_{uuid.uuid4().hex[:8]}"
...
class BaseModalExecutionEnvironment(BaseEnvironment):
    ...
    ``_wait_for_process`` / snapshot machinery does not apply here â€” the
