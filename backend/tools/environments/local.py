"""Local execution environment â€” spawn-per-call with session snapshot."""
...
def _msys_to_windows_path(cwd: str) -> str:
    ...
    idempotent â€” calling it on an already-Windows path returns it as-is.
    """
...
def _resolve_safe_cwd(cwd: str) -> str:
    ...
    Used by ``_run_bash`` to recover when the configured cwd is gone â€” most
    ...
    while parent:
        ...
        if next_parent == parent:
            # Reached the filesystem root and it doesn't exist either â€”
...
# Zed-internal env vars that should NOT leak into terminal subprocesses.
_ZED_PROVIDER_ENV_FORCE_PREFIX = "_ZED_FORCE_"

# Zed-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Zed inference secret exactly
# analogous to ``OPENAI_API_KEY`` â€” nobody drives the ``aws``/``terraform``/
...
# left inheritable.  Per SECURITY.md Â§3.2 the local terminal is the user's
...
# the agent terminal â€” not just Bedrock users, since the registry is iterated
# unconditionally â€” and (b) be unrecoverable, because env_passthrough.py
...
_ZED_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Zed-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_ZED_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _ZED_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_ZED_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_ZED_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _ZED_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("ZED_GIT_BASH_PATH")
    if custom and os.path.isfile(custom):
        return custom

    # Prefer our own portable Git install first â€” this way a broken or
    ...
    #   PortableGit: %LOCALAPPDATA%\zed\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\zed\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _zed_portable_git = os.path.join(_local_appdata, "zed", "git") if _local_appdata else ""
    if _zed_portable_git:
        for candidate in (
            os.path.join(_zed_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_zed_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    raise RuntimeError(
        "Git Bash not found. Zed Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set ZED_GIT_BASH_PATH to your bash.exe location."
    )


# Backward compat â€” process_registry.py imports this name
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    - **Empty entries are stripped.** A leading/trailing/double ``:`` encodes
      ...
      working directory â€” a mild foot-gun in a default terminal environment.
    ...
    # against the caller entries is sufficient â€” no need to track `seen` here.
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_ZED_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_ZED_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _ZED_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
...
def _read_terminal_shell_init_config() -> tuple[list[str], bool]:
    ...
    Best-effort â€” returns sensible defaults on any failure so terminal
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list â€” once they have, Zed trusts them.
    """
    ...
    if explicit:
        ...
        # interactivity guard â€” installers like ``n`` and ``nvm`` append
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways â€” native Python
        ...
        ``ZED_HOME`` instead â€” single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under ZED_HOME.  Using
            # forward slashes makes the same string work unchanged in bash
            # command interpolations AND in Python ``open()`` â€” Windows
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        # (nvm, asdf, pyenv, â€¦) end up on PATH in the captured snapshot.
        ...
        # Recover when the cwd has been deleted out from under us â€” usually by
        ...
        if safe_cwd != self.cwd:
            # MSYS â†’ Windows translation alone shouldn't surface as a warning
        ...
        if not _IS_WINDOWS:
            try:
                proc._zed_pgid = os.getpgid(proc.pid)
    ...
    def _kill_process(self, proc):
        ...
        def _group_alive(pgid: int) -> bool:
            try:
                ...
                os.killpg(pgid, 0)  # windows-footgun: ok â€” POSIX process-group alive probe
                return True
            except ProcessLookupError:
                return False
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_zed_pgid", None)
                    if pgid is None:
                        raise

                try:
                    os.killpg(pgid, signal.SIGTERM)  # windows-footgun: ok â€” POSIX process-group SIGTERM (guarded by _IS_WINDOWS above)
                except ProcessLookupError:
                    return
                ...
                try:
                    ...
                    os.killpg(pgid, signal.SIGKILL)  # windows-footgun: ok â€” POSIX process-group SIGKILL
                except ProcessLookupError:
                    return
    ...
    def _update_cwd(self, result: dict):
        ...
        Skip the assignment when the path no longer exists as a directory â€”
    ...
    def _extract_cwd_from_output(self, result: dict):
        ...
        the directory exists before assigning to ``self.cwd`` â€” otherwise
        ...
        if self.cwd != prev_cwd:
            ...
            if normalized and os.path.isdir(normalized):
                ...
                # Stale / non-existent path â€” keep previous cwd; _run_bash
