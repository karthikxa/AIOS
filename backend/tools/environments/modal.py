...
wrapper, while preserving Zed' persistent snapshot behavior across sessions.
"""
...
_SNAPSHOT_STORE = get_zed_home() / "modal_snapshots.json"
...
def _ensure_modal_sdk() -> None:
    """Lazy-install modal on demand. Idempotent â€” fast no-op once installed."""
    try:
...
class ModalEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        async def _create_sandbox(image_spec: Any):
            app = await _modal.App.lookup.aio("zed-agent", create_if_missing=True)
    ...
    _STDIN_CHUNK_SIZE = 1 * 1024 * 1024  # 1 MB â€” safe for both transport paths
    ...
    def _modal_bulk_download(self, dest: Path) -> None:
        ...
        Modal sandboxes always run as root, so /root/.zed is hardcoded
        (consistent with iter_sync_files call on line 269).
        """
        async def _download():
            proc = await self._sandbox.exec.aio(
                "bash", "-c", "tar cf - -C / root/.zed"
            )
