...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "zed-ssh"
        ...
        # Unix domain sockets. A raw ``user@host:port`` â€” especially with an
        # IPv6 host â€” plus the 16-byte random suffix SSH appends in
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(f"{self._remote_home}/.zed"),
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.zed directory tree on remote in one SSH call."""
        base = f"{self._remote_home}/.zed"
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="zed-ssh-bulk-") as staging:
    ...
    def _ssh_bulk_download(self, dest: Path) -> None:
        """Download remote .zed/ as a tar archive."""
        # Tar from / with the full path so archive entries preserve absolute
        # paths (e.g. home/user/.zed/skills/f.py), matching _pushed_hashes keys.
        rel_base = f"{self._remote_home}/.zed".lstrip("/")
        ssh_cmd = self._build_ssh_command()
