"""Base class for all Zed execution environment backends.
...
# ZED_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("ZED_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {ZED_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_zed_home() / "sandboxes"
...
def _pipe_stdin(proc: subprocess.Popen, data: str) -> None:
    ...
    translates ``\\n`` â†’ ``\\r\\n`` as the data flows through the pipe â€”
    ...
    on POSIX â€” the byte sequence is identical to what text-mode would
    produce there.
    """
    ...
    def _write():
        try:
            ...
            # no ``.buffer`` attribute â€” fall back to .write() directly.
...
def _cwd_marker(session_id: str) -> str:
    return f"__ZED_CWD_{session_id}__"
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Zed backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/zed-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/zed-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        # caused ``C:/Users/.../zed-snap-*.sh: No such file or directory``
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"ZED_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(self, proc: ProcessHandle, timeout: int = 120) -> dict:
        ...
        def _drain():
            ...
            # ``fileno()`` entirely or return a non-integer â€” in that case fall
            ...
            # Use blocking os.read in a daemon thread instead â€” safe because
        ...
        # --- Debug tracing (opt-in via ZED_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __ZED_CWD_{session}__ marker from stdout output.
