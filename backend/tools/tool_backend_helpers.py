...
def managed_nous_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Return True when the user is entitled to the Zed Tool Gateway.

    Entitlement is paid Zed Portal service access OR a live free tool pool
...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    """Return account-aware guidance for an unavailable Zed Tool Gateway path."""
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `zed model` to refresh your "
        "Zed Portal login and billing status."
    )
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.zed/.env`` (via
    ``zed_cli.config.get_env_value`` when available) so tool-side
