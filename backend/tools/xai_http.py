...
def has_xai_credentials() -> bool:
    """Cheap probe â€” return True when xAI credentials are *likely* usable.

    Deliberately avoids :func:`resolve_xai_http_credentials` so callers in
    hot-paint paths (``zed tools`` repaint, tool-registration scans,
    ``WebSearchProvider.is_available()``) don't incur disk locks or â€” in
    the OAuth path â€” a network token refresh. The ABC contract on
    ...
    2. ``~/.zed/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_zed_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.zed/.env`` first, then ``os.environ``.

    Wraps :func:`zed_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from zed_cli.config import get_env_value as _zed_get_env_value

        value = _zed_get_env_value(name)
...
def zed_xai_user_agent() -> str:
    """Return a stable Zed-specific User-Agent for xAI HTTP calls."""
    try:
        from zed_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Zed-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Zed-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``zed_cli.config.get_env_value`` so keys
    stored in ``~/.zed/.env`` (the standard Zed location) are honored â€”
    ...
    tokens where the proactive JWT check is a no-op, etc.), not as a default â€”
