...
configured â€” either via ``/browser connect`` (sets ``BROWSER_CDP_URL``) or
``browser.cdp_url`` in ``config.yaml`` â€” or when a CDP-backed cloud provider
session is active.
...
``browser_console``, etc.) â€” handling native dialogs, iframe-scoped
...
# ``websockets`` is a direct zed-agent dependency because the browser CDP
...
async def _cdp_call(
    ...
    When ``target_id`` is None, ``method`` is sent at browser level â€” which
...
def _browser_cdp_via_supervisor(
    ...
    if not child_sid:
        # Not an OOPIF â€” fall back to top-level session (evaluating at page
...
def browser_cdp(
    ...
    Args:
        ...
        frame_id: Optional cross-origin (OOPIF) iframe ``frame_id`` from
            ...
            WebSocket â€” which is how you Runtime.evaluate *inside* an
    ...
    if not endpoint.startswith(("ws://", "wss://")):
        return tool_error(
            ...
            "Expected ws://... or wss://... â€” the /browser connect "
    ...
    try:
        ...
        return tool_error(
            ...
            "browser may have disconnected â€” try '/browser connect' again.",
            method=method,
        )
...
BROWSER_CDP_SCHEMA: Dict[str, Any] = {
    ...
    "description": (
        ...
        "Firecrawl) â€” those expose CDP per session but live-session routing is "
        ...
        f"**CDP method reference:** {CDP_DOCS_URL} â€” use web_extract on a "
        ...
        "supervisor's live connection â€” the only reliable way on "
        "Browserbase where stateless CDP calls hit signed-URL expiry.\n"
        "- Each stateless call (without frame_id) is independent â€” sessions "
...
def _browser_cdp_check() -> bool:
    ...
    endpoint right now â€” meaning a static URL is set via ``/browser connect``
    (``BROWSER_CDP_URL``) or ``browser.cdp_url`` in ``config.yaml``.

    Backends that do *not* currently expose CDP to us â€” Camofox (REST-only),
    ...
    surfaced â€” are gated out so the model doesn't see a tool that would
    ...
    except ImportError as exc:  # pragma: no cover â€” defensive
