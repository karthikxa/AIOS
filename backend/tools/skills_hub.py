#!/usr/bin/env python3
"""
Skills Hub â€” Source adapters and hub state management for the Zed Skills Hub.
...
Used by zed_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _normalize_lock_install_path(install_path: str, skill_name: str) -> str:
    ...
    entry â€” empty string, ``"."``, an absolute path, ``../..`` traversal,
    or anything whose final component doesn't match the skill name â€” would
...
def _resolve_lock_install_path(install_path: str, skill_name: str) -> Path:
    ...
    2. After resolve(), reject not just escape-out but also ``resolved == SKILLS_DIR``
       â€” an empty/``"."``/``""`` install_path resolves to the skills root itself,
...
class GitHubAuth:
    ...
    GitHub API authentication. Tries methods in priority order:
      1. GITHUB_TOKEN / GH_TOKEN env var (PAT â€” the default)
...
class GitHubSource(SkillSource):
    ...
    DEFAULT_TAPS = [
        ...
        # (governance card), and an `evals/` directory â€” synced daily from
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            zed_meta = metadata.get("zed", {})
            if isinstance(zed_meta, dict):
                tags = zed_meta.get("tags", [])
        if not tags:
    ...
    def _get_repo_tree(self, repo: str) -> Optional[Tuple[str, List[dict]]]:
        ...
        ``_find_skill_in_repo_tree`` multiple times for the same repo â€” this
    ...
    def _github_get(
        ...
        *,
        ...
        Retries on:
          - 403/429 with ``X-RateLimit-Remaining: 0`` â€” waits until the
          ...
          - 5xx and connection/timeout errors â€” exponential backoff.
        ...
        for attempt in range(max_retries):
            ...
            if resp.status_code in (403, 429):
                ...
                if is_rl and attempt < max_retries - 1:
                    ...
                    backoff = min(backoff * 2, 30.0)
                    continue
                # Out of retries (or not a rate-limit 403) â€” flag and return.
                self._check_rate_limit_response(resp)
                return resp

            # 5xx â€” retry; 4xx (other than rate limit) â€” return immediately.
    ...
    def _download_directory_via_tree(self, repo: str, path: str) -> Optional[Dict[str, str]]:
        ...
        if not has_entries:
            # Path definitively doesn't exist in the repo â€” return empty
    ...
    def _get_skillsh_groupings(self, repo: str) -> Optional[Dict[str, str]]:
        ...
        tap that ships this file gets categorization for free â€” this is not
        NVIDIA-specific.
...
class UrlSource(SkillSource):
    ...
    Only single-file skills are supported â€” multi-file skills with
    ...
    # Search is meaningless for a direct URL â€” skip (return empty).
    def search(self, query: str, limit: int = 10) -> List[SkillMeta]:
        return []
    ...
    def _resolve_skill_name(cls, fm: dict, url: str) -> Optional[str]:
        ...
        # 2. URL-slug heuristic: ``.../<name>/SKILL.md`` â†’ ``<name>``;
        #    ``.../<name>.md`` â†’ ``<name>``. Validate each candidate.
        try:
        ...
        # Nothing usable â€” let the caller handle it.
        return None
...
class SkillsShSource(SkillSource):
    ...
    # Sitemap index â€” the real catalog source. The homepage scrape only
    ...
    def _sitemap_catalog(self, limit: int) -> List[SkillMeta]:
        ...
        # Step 1: fetch the sitemap index â†’ list of skill-sitemap URLs.
    ...
    def _meta_from_search_item(self, item: dict) -> Optional[SkillMeta]:
        ...
        installs_label = f" Â· {int(installs):,} installs" if isinstance(installs, int) else ""
    ...
    def _finalize_inspect_meta(self, meta: SkillMeta, canonical: str, detail: Optional[dict]) -> SkillMeta:
        ...
        if isinstance(detail, dict):
            ...
            if body_summary:
                ...
                meta.description = f"{meta.description} Â· {weekly_installs} weekly installs on skills.sh"
        return meta
...
class ClawHubSource(SkillSource):
    ...
    All skills are treated as community trust â€” ClawHavoc incident showed
    ...
    def search(self, query: str, limit: int = 10) -> List[SkillMeta]:
        ...
        if query:
            ...
            # walking the entire 50k+ catalog (max_items=0 â†’ unbounded, used
    ...
    def _load_catalog_index(self, max_items: int = 0) -> List[SkillMeta]:
        ...
        cold-start fallback wants â€” it only renders one page, so walking the
        ...
        # terminates well before this on `nextCursor` going None â€” the cap is
        ...
        # catalog â€” a 12s cap there ships ~3k skills and trips the deploy
        # health floor (20k).
        deadline = (
...
class LobeHubSource(SkillSource):
    ...
    LobeHub agents are system prompt templates â€” we convert them to SKILL.md on fetch.
...
class BrowseShSource(SkillSource):
    ...
    pointing at a CDN-hosted blob â€” the catalog's ``sourceUrl`` field is a GitHub
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Zed Team) but not activated
    by default â€” they don't appear in the system prompt and aren't copied to
...
class HubLockFile:
    """Manages skills/.hub/lock.json â€” tracks provenance of installed hub skills."""
...
class TapsManager:
    """Manages the taps.json file â€” custom GitHub repo sources."""
...
def uninstall_skill(skill_name: str) -> Tuple[bool, str]:
    ...
    # the destructive boundary â€” anything that falls through to the rmtree
...
def _load_zed_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ZED_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if ZED_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - ZED_INDEX_CACHE_FILE.stat().st_mtime
            if age < ZED_INDEX_TTL:
                return json.loads(ZED_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(ZED_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Zed index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Zed index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        ZED_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        ZED_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class ZedIndexSource(SkillSource):
    """Skill source backed by the centralized Zed Skills Index.
    ...
    def search(self, query: str, limit: int = 10) -> List[SkillMeta]:
        ...
        if not query.strip():
            # No query â€” return featured/popular
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "zed-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "zed-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        ZedIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    invoked as each source completes â€” useful for progress indicators.
    """
    ...
    # clawhub, etc.) â€” the index already has their data.  This avoids
    ...
    # worker finishes â€” so a single slow source (e.g. ClawHub) keeps the
