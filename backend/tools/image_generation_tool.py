...
selectable via ``zed tools`` â†’ Image Generation; the active model is
...
- Upscaling via FAL's Clarity Upscaler is gated per-model via the ``upscale``
  flag â€” on for FLUX 2 Pro (backward-compat), off for all faster/newer models
...
# fal_client is imported lazily â€” see _load_fal_client(). Pulling it
...
def _load_fal_client() -> Any:
    ...
    Skips the import if the global is already truthy â€” this preserves the
...
from tools.fal_common import (
    ...
    _normalize_fal_queue_url_format,  # noqa: F401 â€” re-exported for tests
)
...
#
#   "image_size_preset" â€” preset enum ("square_hd", "landscape_16_9", ...)
#                          used by the flux family, z-image, qwen, recraft,
#                          ideogram.
#   "aspect_ratio"      â€” aspect ratio enum ("16:9", "1:1", ...) used by
#                          nano-banana (Gemini).
#   "gpt_literal"       â€” literal dimension strings ("1024x1024", etc.)
#                          used by gpt-image-1.5.
#
# ``supports`` is a whitelist of keys allowed in the outgoing payload â€” any
...
FAL_MODELS: Dict[str, Dict[str, Any]] = {
    ...
    "fal-ai/gpt-image-2": {
        ...
        "price": "$0.04â€“0.06/image",
        # GPT Image 2 uses FAL's standard preset enum (unlike 1.5's literal
        # dimensions). We map to the 4:3 variants â€” the 16:9 presets
        ...
        "supports": {
            "prompt", "image_size", "quality", "num_images", "output_format",
            "sync_mode",
            # openai_api_key (BYOK) intentionally omitted â€” all users go
    ...
    "fal-ai/recraft/v4/pro/text-to-image": {
        ...
        "defaults": {
            # V4 Pro dropped V3's required `style` enum â€” defaults handle taste now.
            "enable_safety_checker": False,
        },
        "supports": {
    ...
    "fal-ai/krea/v2/medium/text-to-image": {
        ...
        "size_style": "aspect_ratio",
        # Krea natively accepts 1:1, 4:3, 3:2, 16:9, 2.35:1, 4:5, 2:3, 9:16 â€”
...
# ---------------------------------------------------------------------------
# Upscaler (Clarity Upscaler â€” unchanged from previous implementation)
# ---------------------------------------------------------------------------
...
def _get_managed_fal_client(managed_gateway):
    ...
    with _managed_fal_client_lock:
        ...
        # Resolve fal_client on the legacy module â€” preserves the test
...
def _submit_fal_request(model: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        # â€” surface a clearer message with actionable remediation instead
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        # â€” it's an extension hook, not a typo. The getattr/callable guards make
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.zed"
    ...
    # Zed cache roots can be translated without side effects. SSH can still
    ...
    try:
        ...
        if source_images and not edit_endpoint:
            raise ValueError(
                ...
                f"via `zed tools` â†’ Image Generation."
            )
        ...
        if use_edit:
            ...
            logger.info(
                "Editing image with %s (%s) â€” %d source image(s), prompt: %s",
            ...
            endpoint = model_id
            logger.info(
                "Generating image with %s (%s) â€” prompt: %s",
        ...
        if not result or "images" not in result:
            raise ValueError("Invalid response from FAL.ai API â€” no images returned")
...
def _build_no_backend_setup_message() -> str:
    ...
    lines.append(
        "  3. Configure a different image_gen provider via `zed tools` "
        "â†’ Image Generation (run `zed plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping zed with a FAL key configured
...
if __name__ == "__main__":
    print("ðŸŽ¨ Image Generation Tools â€” FAL.ai multi-model support")
    print("=" * 60)

    if not check_fal_api_key():
        print("âŒ FAL_KEY environment variable not set")
        print("   Set it via: export FAL_KEY='your-key-here'")
        print("   Get a key: https://fal.ai/")
        raise SystemExit(1)
    print("âœ… FAL.ai API key found")

    try:
        import fal_client  # noqa: F401
        print("âœ… fal_client library available")
    except ImportError:
        print("âŒ fal_client library not found â€” pip install fal-client")
        raise SystemExit(1)

    model_id, meta = _resolve_fal_model()
    print(f"ðŸ¤– Active model: {meta.get('display', model_id)} ({model_id})")
    print(f"   Speed: {meta.get('speed', '?')}  Â·  Price: {meta.get('price', '?')}")
    print(f"   Upscaler: {'on' if meta.get('upscale') else 'off'}")

    print("\nAvailable models:")
    for mid, m in FAL_MODELS.items():
        marker = " â† active" if mid == model_id else ""
        print(f"  {mid:<32}  {m.get('speed', '?'):<6}  {m.get('price', '?')}{marker}")

    if _debug.active:
        print(f"\nðŸ› Debug mode enabled â€” session {_debug.session_id}")
...
IMAGE_GENERATE_SCHEMA = {
    "name": "image_generate",
    # Placeholder â€” the real description is rebuilt dynamically at
        ...
        "properties": {
            ...
            "image_url": {
                ...
                "description": (
                    ...
                    "support editing â€” the description above indicates whether "
...
def _read_configured_image_provider():
    ...
    We only consult the plugin registry when this is explicitly set â€” an
    ...
    back into this module's pipeline via call-time indirection â€” see
    issue #26241).
    """
    try:
        from hermes_cli.config import load_config
...
def _dispatch_to_plugin_provider(
    ...
    Dispatch fires when ``image_gen.provider`` is explicitly set â€” including
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `zed plugins list` to see "
    ...
    try:
        ...
        # (third-party plugin not yet updated) â€” retry without the new kwargs
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `zed tools` â†’ Image Generation."
                ),
...
# ---------------------------------------------------------------------------
# Dynamic schema â€” reflect the active backend's image-to-image capability
# ---------------------------------------------------------------------------
#
...
# model up front ("the active model is text-to-image only â€” image_url will be
...
# model/provider via `zed tools` or `/skills`.
...
def _build_dynamic_image_schema() -> Dict[str, Any]:
    ...
    if "image" in modalities and "text" in modalities:
        ...
        parts.append(
            ...
            f"image-to-image / editing (pass image_url){ref_note} â€” "
            "routes automatically"
        )
    elif "image" in modalities and "text" not in modalities:
        parts.append(
            "- this model is image-to-image / edit only â€” image_url is REQUIRED"
        )
    else:
        parts.append(
            "- this model is text-to-image only â€” it is NOT capable of "
