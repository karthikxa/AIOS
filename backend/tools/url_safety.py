"""URL safety checks â€” blocks requests to private/internal network addresses.
...
def normalize_url_for_request(url: str) -> str:
    """Return an ASCII-safe HTTP URL for Zed-owned URL tools.
...
# endpoints â€” the #1 SSRF target â€” and the link-local range where
# they all live.
#
...
_ALWAYS_BLOCKED_IPS = frozenset({
    ...
    # IPv4-mapped IPv6 variants â€” same endpoints reachable via ::ffff:x.x.x.x
...
# ipaddress.is_private â€” it returns False for both is_private and is_global.
...
def _global_allow_private_urls() -> bool:
    ...
    1. ``ZED_ALLOW_PRIVATE_URLS`` env var  (``true``/``1``/``yes``)
    ...
    env_val = os.getenv("ZED_ALLOW_PRIVATE_URLS", "").strip().lower()
    if env_val in {"true", "1", "yes"}:
        _cached_allow_private = True
        return _cached_allow_private
    if env_val in {"false", "0", "no"}:
        # Explicit false â€” don't fall through to config
        return _cached_allow_private
    ...
    try:
        ...
        # Config unavailable (e.g. tests, early import) â€” keep default
        pass
...
def _reset_allow_private_cache() -> None:
    """Reset the cached toggle â€” only for tests."""
    global _allow_private_resolved, _cached_allow_private
    _allow_private_resolved = False
...
def is_always_blocked_url(url: str) -> bool:
    ...
    try:
        ...
        # Literal IP â†’ check directly against the always-blocked set
        try:
        ...
        # Hostname â†’ resolve and check every answer.  DNS failure is NOT
        ...
        # Parse failures or unexpected errors â€” don't claim the URL is
...
def is_safe_url(url: str) -> bool:
    ...
    ``ZED_ALLOW_PRIVATE_URLS=true``), private-IP blocking is skipped.
    ...
    try:
        ...
        if scheme not in {"http", "https"}:
            logger.warning("Blocked request â€” unsupported URL scheme: %s", scheme or "<empty>")
            return False
        if not hostname:
            return False

        # Block known internal hostnames â€” ALWAYS, even with toggle on
        if hostname in _BLOCKED_HOSTNAMES:
        ...
        try:
            ...
            # DNS resolution failed â€” fail closed. If DNS can't resolve it,
        ...
        # Fail closed on unexpected errors â€” don't let parsing edge cases
        # become SSRF bypass vectors
        logger.warning("Blocked request â€” URL safety check error for %s: %s", url, exc)
        return False
