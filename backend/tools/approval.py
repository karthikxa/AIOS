...
# instantly bypass all approval checks â€” a prompt-injection escalation path.
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("ZED_YOLO_MODE", ""))
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("ZED_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("ZED_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("ZED_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set ZED_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind ZED_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds ZED_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("ZED_CRON_SESSION"):
        return False
    if env_var_enabled("ZED_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $ZED_HOME, or by the resolved absolute
# active profile home path such as /home/zed/.zed/config.yaml. The
# resolved-absolute form is folded into the ~/.zed/ patterns at detection
# time by _normalize_command_for_detection() â€” see the rewrite step there â€” so
# these static patterns stay free of any import-time path snapshot (which would
# go stale when ZED_HOME is set after this module is imported, e.g. under the
...
# ~/.zed/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# `cp`, etc. targeting it are gated too â€” otherwise the deny is unpaired
# theater. Mirrors _ZED_ENV_PATH; matches the ZED_HOME override form as
# well as ~/.zed/.
_ZED_CONFIG_PATH = (
    r'(?:~\/\.zed/|'
    r'(?:\$home|\$\{home\})/\.zed/|'
...
# alone.
#
# The list is deliberately tiny â€” only things with no recovery path:
...
# them through â€” that's what yolo is for.
#
...
HARDLINE_PATTERNS = [
    ...
    # System shutdown / reboot â€” anchor to command position (start of line,
...
# =========================================================================
# Sudo stdin guard â€” block password guessing via "sudo -S"
# =========================================================================
...
# Treat this as an unconditional block â€” there is never a legitimate
...
def _hardline_block_result(description: str) -> dict:
    ...
    return {
        ...
        "message": (
            ...
            "be executed via the agent â€” not even with --yolo, /yolo, "
...
def _sudo_stdin_block_result(description: str) -> dict:
    ...
    return {
        ...
        "message": (
            ...
            "Do not pipe passwords to 'sudo -S' â€” this is a brute-force "
...
DANGEROUS_PATTERNS = [
    ...
    # find -exec rm / -execdir rm â€” the -execdir variant (same semantics,
    ...
    (r'\bzed\s+gateway\s+(stop|restart)\b', "stop/restart zed gateway (kills running agents)"),
    (r'\bzed\s+update\b', "zed update (restarts gateway, kills running agents)"),
    # Docker container lifecycle â€” any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `zed gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart zed-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart zed-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(zed|gateway|cli\.py)\b', "kill zed/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill zed` but not
    # `kill -9 $(pgrep -f zed)` because the substitution is opaque
    ...
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Zed file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.zed/config.yaml/.env), but cp/mv/install was only paired for /etc and
    # project-relative env/config â€” so `cp evil ~/.ssh/authorized_keys` (key
    ...
    # DESTINATION (last arg) only â€” `cp evil ~/.ssh/authorized_keys` is gated,
    ...
    # In-place edit of a Zed-managed security file (~/.zed/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_ZED_CONFIG_PATH}|{_ZED_ENV_PATH})', "in-place edit of Zed config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_ZED_CONFIG_PATH}|{_ZED_ENV_PATH})', "in-place edit of Zed config/env (long flag)"),
    ...
    # anywhere in the args, not just the first token â€” `perl -e '...'` (code
    # eval, no -i) does not trip because it has no `-...i` flag token.
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_ZED_CONFIG_PATH}|{_ZED_ENV_PATH})', "in-place edit of Zed config/env (perl/ruby)"),
    # Script execution via heredoc â€” bypasses the -e/-c flag patterns above.
    ...
    # Script execution after chmod +x â€” catches the two-step pattern where
    ...
    # matching, so case variants of S/s and A/a collapse â€” both forms
    ...
    # Combined short-flag form: -nS, -ns, -sa, -las â€” sudo flags packed
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # Strip shell backslash-escapes: r\m â†’ rm. Prevents \-injection bypass.
    command = re.sub(r'\\([^\n])', r'\1', command)
    # Strip empty-string literals that split tokens: r''m â†’ rm, r"\"m â†’ rm.
    ...
    # ~/.zed/ form so the Zed config/env patterns catch it. In Docker and
    # gateway deployments the agent often references the resolved absolute path
    # directly (e.g. `sed -i ... /home/zed/.zed/config.yaml`) rather than
    # ~, $HOME, or $ZED_HOME. Done at detection time (not via an import-time
    # pattern snapshot) so it tracks the live ZED_HOME even when that is set
    # after this module is imported â€” as the hermetic test conftest does.
    command = _rewrite_resolved_zed_home(command)
    return command
...
def _rewrite_resolved_zed_home(command: str) -> str:
    """Rewrite the resolved absolute Zed home prefix to ``~/.zed/``.

    Resolves the active ``ZED_HOME`` at call time (and its symlink-resolved
    form) and replaces an occurrence of ``<home>/`` in *command* with
    ``~/.zed/`` so the static ``_ZED_CONFIG_PATH`` / ``_ZED_ENV_PATH``
    patterns match. No-op when the path can't be resolved or doesn't appear.
    """
    try:
        from hermes_constants import get_zed_home
        home = get_zed_home().expanduser()
        candidates = [
            str(home).rstrip("/"),
    ...
    for path in candidates:
        if not path or path in seen:
            continue
        seen.add(path)
        # Guard against a degenerate ZED_HOME (e.g. "/" or "") rewriting
        ...
        # /home/zed/.zed or a per-test tempdir, never a bare root.
        normalized = path.rstrip("/")
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.zed/")
    return command
...
# subagents, execute_code RPC handlers) can block concurrently â€” each gets
...
class _ApprovalEntry:
    ...
    def __init__(self, data: dict):
        ...
        self.data = data          # command, description, pattern_keys, â€¦
        self.result: Optional[str] = None  # "once"|"session"|"always"|"deny"


_gateway_queues: dict[str, list] = {}        # session_key â†’ [_ApprovalEntry, â€¦]
_gateway_notify_cbs: dict[str, object] = {}  # session_key â†’ callable(approval_data)
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["ZED_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "ZED_SPINNER_PAUSE" in os.environ:
            del os.environ["ZED_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("ZED_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s â€” set ZED_INTERACTIVE or ZED_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("ZED_EXEC_ASK"):
        ...
        return {
            ...
            "description": description,
            "message": (
                f"âš ï¸ This command is potentially dangerous ({description}). "
...
def _format_tirith_description(tirith_result: dict) -> str:
    ...
    return "Security scan â€” " + "; ".join(parts)
...
def _await_gateway_decision(session_key: str, notify_cb, approval_data: dict,
    ...
    # Notify the user (bridges sync agent thread â†’ async gateway)
    try:
    ...
    # inactivity tracker â€” otherwise the gateway watchdog kills the agent
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("ZED_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("ZED_EXEC_ASK")
    ...
    # Tirith check â€” wrapper guarantees no raise for expected failures.
    ...
    try:
        ...
        pass  # tirith module not installed â€” allow
    ...
    # Tirith block/warn â†’ approvable warning with rich findings.
    ...
    if approval_mode == "smart":
        ...
        # verdict == "escalate" â†’ fall through to manual prompt
    ...
    # Gateway/async approval â€” block the agent thread until the user
    ...
    if is_gateway or is_ask:
        ...
        if notify_cb is not None:
            ...
            if not resolved or choice is None or choice == "deny":
                ...
                # deny is also a hard halt â€” both produce a BLOCKED outcome
            ...
            # User approved â€” persist based on scope (same logic as CLI)
            for key, _, is_tirith in warnings:
                if choice == "session" or (choice == "always" and is_tirith):
                    ...
                    save_permanent_allowlist(_permanent_approved)
                # choice == "once": no persistence â€” command allowed this
        ...
        return {
            ...
            "description": combined_desc,
            "message": (
                f"âš ï¸ {combined_desc}. Asking the user for approval.\n\n**Command:**\n```\n{command}\n```"
            ),
        }
...
def check_execute_code_guard(code: str, env_type: str) -> dict:
    ...
    # Isolated backends already sandbox the child â€” matches the container skip
    ...
    # Check session/permanent approval â€” same gate as check_all_command_guards.
    ...
    if approval_mode == "smart":
        ...
        # verdict == "escalate" â†’ fall through to manual approval
    ...
    if notify_cb is None:
        ...
        return {
            ...
            "description": description,
            "message": (
                f"âš ï¸ {description}. Asking the user for approval.\n\n"
                f"**Code:**\n```python\n{code}\n```"
            ),
        }
    ...
    # Approved â€” persist based on scope (same logic as check_all_command_guards).
    if choice == "session":
        approve_session(session_key, pattern_key)
    elif choice == "always":
        approve_session(session_key, pattern_key)
        approve_permanent(pattern_key)
        save_permanent_allowlist(_permanent_approved)
    # choice == "once": no persistence â€” approval lasts this single call only.
...
def request_elicitation_consent(
    ...
    *,
    ...
    if _is_gateway_approval_context():
        ...
        if notify_cb is None:
            logger.warning(
                ...
                "notify_cb is registered â€” failing closed",
                session_key,
            )
            return "decline"
    ...
    # per-call confirmation â€” there is no pattern to remember.
    try:
