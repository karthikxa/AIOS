...
running under the dispatcher (env var ``ZED_KANBAN_TASK`` set) or when
the active profile explicitly enables the ``kanban`` toolset for
orchestrator work. A normal ``zed chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``zed kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``zed kanban complete â€¦``
   inside the container, where ``zed`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.zed/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``zed kanban â€¦``), the dashboard
(``zed dashboard``), and the slash command (``/kanban â€¦``) â€” all
...
def _check_kanban_mode() -> bool:
    ...
    1. ``ZED_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    2. The current profile has ``kanban`` in its toolsets config
       (orchestrator profiles like techlead that route work via Kanban).

    Humans running ``zed chat`` without the kanban toolset see zero
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if arg:
        return arg
    env_tid = os.environ.get("ZED_KANBAN_TASK")
...
def _stamp_worker_session_metadata(
    ...
    if os.environ.get("ZED_KANBAN_TASK") != task_id:
        return metadata
    session_id = os.environ.get("ZED_SESSION_ID")
    if not session_id:
        return metadata
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``ZED_KANBAN_TASK`` set
    ...
    ``ZED_KANBAN_TASK`` in env) aren't subject to this check â€” their
...
def _connect(board: Optional[str] = None):
    ...
    (``ZED_KANBAN_DB`` â†’ ``ZED_KANBAN_BOARD`` env â†’ current symlink
...
#   - No-op outside dispatcher-spawned worker context (no ``ZED_KANBAN_TASK``).
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    Identity comes from:
      * ``ZED_KANBAN_TASK`` â€” task id (required; absence means no-op)
      * ``ZED_KANBAN_RUN_ID`` â€” pins the run row so we don't heartbeat
        a stale run that may have already been reclaimed
      * ``ZED_KANBAN_CLAIM_LOCK`` â€” claim lock for ``heartbeat_claim``;
    ...
    tid = os.environ.get("ZED_KANBAN_TASK")
    if not tid:
        return False
    ...
    try:
        kb, conn = _connect()
        try:
            claim_lock = os.environ.get("ZED_KANBAN_CLAIM_LOCK")
            try:
                kb.heartbeat_claim(conn, tid, claimer=claim_lock)
            except Exception:
                logger.debug("auto-heartbeat: heartbeat_claim failed", exc_info=True)
            run_id_raw = os.environ.get("ZED_KANBAN_RUN_ID")
            run_id: Optional[int]
            try:
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set ZED_KANBAN_TASK in the env)"
        )
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # ZED_KANBAN_CLAIM_LOCK in the worker env at spawn time
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``zed-system`` and poison
    ...
    author = os.environ.get("ZED_PROFILE") or "worker"
    board = args.get("board")
    try:
        kb, conn = _connect(board=board)
        try:
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("ZED_TENANT")
    # Stamp the originating session id when the agent loop runs under
    # ACP (which sets ZED_SESSION_ID before invoking tools). NULL on
    # CLI / dashboard paths and on legacy hosts that don't set the env.
    session_id = args.get("session_id") or os.environ.get("ZED_SESSION_ID")
    priority = args.get("priority")
    # Resolve workspace. If the caller passed one explicitly, honor it.
    # Otherwise, a dispatcher-spawned worker (ZED_KANBAN_TASK set)
    ...
    # (kanban toolset, no ZED_KANBAN_TASK) and CLI/dashboard callers
    ...
    try:
        ...
        try:
            ...
            if _inherit_workspace:
                _self_tid = os.environ.get("ZED_KANBAN_TASK")
                if _self_tid:
            ...
            new_tid = kb.create_task(
                ...
                created_by=os.environ.get("ZED_PROFILE") or "worker",
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    toggle (see ``zed dashboard``).

    Subscription paths:

    - **Gateway** (telegram/discord/slack/etc): ``ZED_SESSION_PLATFORM``
      and ``ZED_SESSION_CHAT_ID`` are set in ContextVars by the
    ...
    - **TUI** (herm desktop / herm TUI): the platform/chat_id ContextVars
      ...
      ``ZED_SESSION_KEY`` from the parent session. We subscribe with
    ...
    try:
        ...
        # If config can't load we still default to True â€” this is the
    ...
    try:
        ...
        if not platform or not chat_id:
            ...
            # ZED_SESSION_KEY into the subprocess env. Treat that
            # as a "tui" subscription so the TUI notification poller
            # (tui_gateway/server.py) can pick it up.
            #
            # ZED_SESSION_ID is intentionally NOT a fallback here:
            ...
            # poller keys on ZED_SESSION_KEY.
            session_key = (
                get_session_env("ZED_SESSION_KEY", "")
                or os.environ.get("ZED_SESSION_KEY", "")
            )
            if not session_key:
                return False  # CLI / cron / test â€” no persistent channel
            platform = "tui"
            chat_id = session_key
        thread_id = get_session_env("ZED_SESSION_THREAD_ID", "") or None
        user_id = get_session_env("ZED_SESSION_USER_ID", "") or None
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to ZED_KANBAN_TASK from the env "
    "(the task the dispatcher spawned you to work on)."
)

_DESC_BOARD = (
    "Kanban board slug to target. When omitted, the call resolves the "
    "active board the usual way: ZED_KANBAN_DB env â†’ "
    "ZED_KANBAN_BOARD env â†’ the 'current' symlink under the kanban "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to ZED_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
