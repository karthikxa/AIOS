...
thread until they respond.  In CLI mode this is trivial â€” ``input()`` is
...
Two delivery paths from the adapter:
  ...
  1. **Button UI** â€” adapters override ``send_clarify`` to render inline
  ...
  2. **Text fallback** â€” adapters without rich UI render a numbered list.
...
# clarify_id â†’ _ClarifyEntry  (primary lookup for button callbacks)
_entries: Dict[str, _ClarifyEntry] = {}
# session_key â†’ list[clarify_id]  (FIFO; for text-fallback intercept and session cleanup)
_session_index: Dict[str, List[str]] = {}


# =========================================================================
# Public API â€” agent-thread side
# =========================================================================
...
def register(
    ...
    entry = _ClarifyEntry(
        ...
        # Open-ended (no choices) â†’ next message IS the response, no buttons needed.
...
def wait_for_response(clarify_id: str, timeout: float) -> Optional[str]:
    ...
    firing â€” without this, ``Event.wait(timeout=600)`` blocks the thread
...
# =========================================================================
# Public API â€” gateway / adapter side
# =========================================================================
...
def get_pending_for_session(session_key: str) -> Optional[_ClarifyEntry]:
    ...
    Used by the text-fallback intercept in ``_handle_message`` â€” when a
...
def clear_session(session_key: str) -> int:
    ...
    for entry in entries:
        if entry is None:
            continue
        # Empty string sentinel â€” agent code can distinguish from a real
...
def get_clarify_timeout() -> int:
    ...
    Defaults to 600 (10 minutes) â€” long enough for the user to type a
...
# =========================================================================
# Per-session notify hook (gateway â†’ adapter bridge)
# =========================================================================
...
# callback bridges syncâ†’async (runs on the agent thread; schedules the
