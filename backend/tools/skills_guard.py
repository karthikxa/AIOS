...
Trust levels:
  - builtin:   Ships with Zed. Never scanned, always trusted.
...
INSTALL_POLICY = {
    ...
    # skills.guard_agent_created is enabled (off by default) â€” see
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.zed/\.env|\~/\.zed/\.env',
     "zed_env_access", "critical", "exfiltration",
     "directly references Zed secrets file"),
    ...
    # secret-shaped identifier â€” `os.environ.get("OPENAI_API_KEY")` still trips
    ...
    # spec â€” every compliant skill declares it, so it cannot be a threat
    ...
    (r'\.zed/config\.yaml|\.zed/SOUL\.md',
     "zed_config_mod", "critical", "persistence",
     "references Zed configuration files directly"),
...
def content_hash(skill_path: Path) -> str:
    ...
    ``tools.skills_hub.bundle_content_hash`` â€” both functions need to
...
# scan. `.skillignore` is the Zed-native name; `.clawhubignore` is honored
