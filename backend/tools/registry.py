"""Central registry for all zed-agent tools.
...
def _module_registers_tools(module_path: Path) -> bool:
    ...
    """
    try:
        # utf-8-sig strips a leading BOM (\ufeff) if present. Many tool files
        # in this repo were saved with a BOM, which utf-8 preserves as a literal
        # character and ast.parse() then rejects ("invalid non-printable
        # character U+FEFF") -- causing the file to be silently skipped by tool
        # discovery. utf-8-sig accepts both BOM and no-BOM files.
...
# every get_definitions() is pure waste â€” external state changes on human
# timescales. Cache results for ~30 s so env-var flips via ``zed tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``zed tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
        ...
        with self._lock:
            ...
            if existing and existing.toolset != toolset:
                ...
                if both_mcp:
                    ...
                    # Reject shadowing â€” prevent plugins/MCP from overwriting
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``zed tools enable foo``)
        still take effect in near-real-time without forcing a full cache
        flush on every call.
        """
        result = []
        # Per-call cache on top of the 30 s TTL â€” handles repeat probes of the
        ...
        for name in sorted(tool_names):
            ...
            if entry.check_fn:
                ...
                if not check_results[entry.check_fn]:
                    ...
                    continue
            # Ensure schema always has a "name" field â€” use entry.name as fallback
    ...
    def get_schema(self, name: str) -> Optional[dict]:
        ...
        doesn't matter â€” only the schema content does.
        """
        entry = self.get_entry(name)
