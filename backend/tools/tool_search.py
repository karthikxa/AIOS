"""Progressive tool disclosure ("tool search") for Zed Agent.

When enabled, MCP and non-core plugin tools are replaced in the model-visible
tools array by three bridge tools â€” ``tool_search``, ``tool_describe``,
``tool_call`` â€” and surfaced on demand. Core Zed tools never defer.

Design constraints this module is built around (see ``openclaw-tool-search-report``
for the full rationale):

* Core tools defined in ``toolsets._ZED_CORE_TOOLS`` are *never* deferred.
...
# user/plugin/MCP tool â€” registration of any tool with these names is
...
class ToolSearchConfig:
    ...
    threshold_pct: float  # 0..100 â€” only used when enabled == "auto"
...
def _core_tool_names() -> frozenset[str]:
    ...
    try:
        ...
        return frozenset(_ZED_CORE_TOOLS)
    except Exception:
        return frozenset()
...
def is_deferrable_tool_name(name: str) -> bool:
    ...
    OR it is not in ``_ZED_CORE_TOOLS``. Core tools are never deferred
    ...
    try:
        ...
        if entry.toolset.startswith("mcp-"):
            return True
        # Non-MCP, non-core â†’ plugin tool, eligible.
        return True
    except Exception:
        return False
...
def classify_tools(tool_defs: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for td in tool_defs:
        ...
        name = fn.get("name", "")
        if name in BRIDGE_TOOL_NAMES:
            # Should never happen â€” bridge tools are added after classification â€”
            # but be defensive.
            continue
...
def estimate_tokens_from_schemas(tool_defs: Iterable[Dict[str, Any]]) -> int:
    ...
    Cheap and stable across providers. The number doesn't need to be exact â€”
...
def should_activate(
    ...
    (as long as there is at least one deferrable tool â€” there's no point
    ...
    if not context_length or context_length <= 0:
        ...
        # â€” the cliff above which Anthropic and OpenAI both saw quality drops.
...
def _entry_search_text(td: Dict[str, Any]) -> str:
    ...
    top-level parameters. Schema bodies are deliberately excluded â€”
...
def _bm25_score(query_tokens: List[str], doc_tokens: List[str],
    ...
    is fine â€” the catalog is bounded by N (tools) typically < 500, and we
...
def search_catalog(catalog: List[CatalogEntry], query: str, limit: int = 5) -> List[CatalogEntry]:
    ...
    where every tool is named ``github_*`` still returns results â€” BM25
...
def bridge_tool_schemas(deferred_count: int) -> List[Dict[str, Any]]:
    ...
    The schemas are intentionally short â€” every byte added here is a byte
