...
it is automatically downloaded from GitHub releases to $ZED_HOME/bin/tirith.
...
verification only â€” still secure via HTTPS + checksum, just without supply
...
# Cosign provenance verification â€” pinned to the specific release workflow
...
# _INSTALL_FAILED means "we tried and failed" â€” prevents retry on every command.
...
# Warning de-duplication. The spawn/path warnings live in the hot path â€”
...
def _reset_spawn_warning_state() -> None:
    ...
    (re)installed so a subsequent failure surfaces again â€” e.g. user
    deletes the binary mid-session.
    """
    with _warned_lock:
        _warned_messages.clear()

# Disk-persistent failure marker â€” avoids retry across process restarts
_MARKER_TTL = 86400  # 24 hours


def _get_zed_home() -> str:
    """Return the Zed home directory, respecting ZED_HOME env var."""
    return str(get_zed_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_zed_home(), ".tirith-install-failed")
...
def _zed_bin_dir() -> str:
    """Return $ZED_HOME/bin, creating it if needed."""
    d = os.path.join(_get_zed_home(), "bin")
...
def _detect_target() -> str | None:
    ...
    Windows is intentionally unsupported â€” tirith does not ship a Windows
    ...
    # Android (Termux) is ABI-compatible with Linux â€” reuse Linux binaries.
    if system == "Darwin":
        plat = "apple-darwin"
...
def is_platform_supported() -> bool:
    ...
    install" from "tirith was never going to install here" â€” the latter
...
def _verify_cosign(checksums_path: str, sig_path: str, cert_path: str) -> bool | None:
    ...
    Returns:
        True  â€” cosign verified successfully
        False â€” cosign found but verification failed
        None  â€” cosign not available (not on PATH, or execution failed)

    The caller treats both False and None as "abort auto-install" â€” only
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ZED_HOME/bin/tirith.
    ...
    try:
        ...
        logger.info("tirith not found â€” downloading latest release for %s...", target)
        ...
        # Cosign provenance verification â€” preferred but not mandatory.
        ...
        dest = os.path.join(_zed_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    default), that path is authoritative â€” we never fall through to
    auto-download a different binary.

    For the default "tirith":
    1. PATH lookup via shutil.which
    2. $ZED_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases â†’ $ZED_HOME/bin/tirith
    ...
    # return the unexpanded configured path â€” the spawn loop will fail-open
    ...
    # Default "tirith" â€” always re-run cheap local checks so a manual
    ...
    zed_bin = os.path.join(_zed_bin_dir(), "tirith")
    if os.path.isfile(zed_bin) and os.access(zed_bin, os.X_OK):
        _resolved_path = zed_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return zed_bin

    # Local checks failed.  If a previous install attempt already failed,
    # skip the network retry â€” UNLESS the failure was "cosign_missing" and
    ...
    if install_failed:
        if _install_failure_reason == "cosign_missing" and shutil.which("cosign"):
            # Retryable cause resolved â€” clear sentinel and fall through to retry
            _resolved_path = None
            _install_failure_reason = ""
            _clear_install_failed()
    ...
    # If a background install thread is running, don't start a parallel one â€”
    ...
    # Install failed â€” cache the miss and persist reason to disk
    _resolved_path = _INSTALL_FAILED
...
def ensure_installed(*, log_failures: bool = True):
    ...
    # Platform has no tirith build (e.g. Windows) â€” don't probe PATH,
    ...
    # Default "tirith" â€” quick local checks first (no network)
    found = shutil.which("tirith")
    if found:
        _resolved_path = found
        _install_failure_reason = ""
    ...
    # the cosign_missing reason was resolved â€” handled by _is_install_failed_on_disk).
    ...
    # Need to download â€” launch background thread so startup doesn't block
...
def check_command_security(command: str) -> dict:
    ...
    # Unsupported platform (Windows etc.) â€” tirith has no binary here and
    ...
    try:
        ...
        # surfaces once but doesn't drown the log on every command â€”
    ...
    if exit_code == 0:
        ...
        # Unknown exit code â€” respect fail_open
