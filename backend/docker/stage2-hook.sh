...
#
# Wired into the image as /etc/cont-init.d/01-zed-setup by the
...
ZED_HOME="${ZED_HOME:-/opt/data}"
INSTALL_DIR="/opt/zed"

# Drop to zed via s6-setuidgid, but skip it when already non-root.
as_zed() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid zed "$@"; }
...
# starts non-root. The baked install tree under /opt/zed is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $ZED_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass ZED_UID/ZED_GID â€” or the PUID/PGID aliases â€” which the
...
# `id -u zed` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# zed UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u zed)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-zed UID).
...
and the baked /opt/zed install tree is intentionally root-owned and
...
# --- Bootstrap ZED_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid zed mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom ZED_HOME paths whose parents only
# root can create (e.g. `ZED_HOME=/home/zed/.zed` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for ZED_UID/ZED_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  ZED_UID/
# ZED_GID still win when both are set.  See #15290, salvages #25872.
ZED_UID="${ZED_UID:-${PUID:-}}"
ZED_GID="${ZED_GID:-${PGID:-}}"

if [ -n "${ZED_UID:-}" ] && validate_uid_gid "$ZED_UID" && [ "$ZED_UID" != "$(id -u zed)" ]; then
    echo "[stage2] Changing zed UID to $ZED_UID"
    usermod -u "$ZED_UID" zed
fi
if [ -n "${ZED_GID:-}" ] && validate_uid_gid "$ZED_GID" && [ "$ZED_GID" != "$(id -g zed)" ]; then
    echo "[stage2] Changing zed GID to $ZED_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$ZED_GID" zed 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised zed user (UID 10000)
...
# zed process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes zed. Idempotent across container
...
#   - zed is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G zed 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] zed already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" zed 2>/dev/null; then
        echo "[stage2] Added zed to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group zed failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When ZED_UID is remapped or the top-level $ZED_HOME isn't owned by
# the runtime zed UID, restore ownership to zed â€” but ONLY for the
# directories zed actually writes to. The full $ZED_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of zed-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_zed_uid=$(id -u zed)
needs_chown=false
if [ "$(stat -c %u "$ZED_HOME" 2>/dev/null)" != "$actual_zed_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $ZED_HOME (targeted) to zed ($actual_zed_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID â€” chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $ZED_HOME: chown the directory itself (not its contents)
    # so zed can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    chown zed:zed "$ZED_HOME" 2>/dev/null || \
        echo "[stage2] Warning: chown $ZED_HOME failed (rootless container?) â€” continuing"
    # Zed-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by zed (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing; do
        if [ -e "$ZED_HOME/$sub" ]; then
            chown -R zed:zed "$ZED_HOME/$sub" 2>/dev/null || \
                echo "[stage2] Warning: chown $ZED_HOME/$sub failed (rootless container?) â€” continuing"
        fi
    done
fi
...
# zed user. Hosted/container instances keep mutable state under
# $ZED_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# ZED_DISABLE_LAZY_INSTALLS=1. Keeping /opt/zed root-owned and
...
# Always reset ownership of $ZED_HOME/profiles to zed on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> zed â€¦` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as zed and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$ZED_HOME/profiles" ]; then
    chown -R zed:zed "$ZED_HOME/profiles" 2>/dev/null || true
fi

# Always reset ownership of $ZED_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged zed runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$ZED_HOME/cron" ]; then
    chown -R zed:zed "$ZED_HOME/cron" 2>/dev/null || true
fi

# Reset ownership of zed-owned top-level state files on every boot.
# The targeted data-volume chown above only covers zed-owned
# *subdirectories*; loose state files living directly under $ZED_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> zed â€¦` (root unless `-u` is passed) they
# land root-owned, and the unprivileged zed runtime then hits
...
# sweep so host-owned files in a bind-mounted $ZED_HOME are never
...
# entries of zed_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$ZED_HOME/$f" ]; then
        chown zed:zed "$ZED_HOME/$f" 2>/dev/null || true
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the zed runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$ZED_HOME/config.yaml" ]; then
    chown zed:zed "$ZED_HOME/config.yaml" 2>/dev/null || true
    chmod 640 "$ZED_HOME/config.yaml" 2>/dev/null || true
fi

# --- Seed directory structure as zed user ---
# Run as zed via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter â€” defends against $ZED_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_zed mkdir -p \
    "$ZED_HOME/cron" \
    "$ZED_HOME/sessions" \
    "$ZED_HOME/logs" \
    "$ZED_HOME/logs/gateways" \
    "$ZED_HOME/hooks" \
    "$ZED_HOME/memories" \
    "$ZED_HOME/skills" \
    "$ZED_HOME/skins" \
    ...
    "$ZED_HOME/home" \
    "$ZED_HOME/pairing" \
    "$ZED_HOME/platforms/pairing"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/zed/.install_method (see Dockerfile), NOT written here into
# $ZED_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $ZED_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.zed:/opt/data) and sometimes shared with a
...
# 'zed update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $ZED_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/zed stamp, so deleting the data-dir copy is safe).
if [ -f "$ZED_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$ZED_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$ZED_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$ZED_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        as_zed cp "$INSTALL_DIR/$src" "$ZED_HOME/$dest"
    fi
}
...
if [ -f "$ZED_HOME/.env" ]; then
    chown zed:zed "$ZED_HOME/.env" 2>/dev/null || true
    chmod 600 "$ZED_HOME/.env" 2>/dev/null || true
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $ZED_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `zed update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set ZED_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$ZED_HOME/config.yaml" ]; then
    s6-setuidgid zed "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$ZED_HOME/auth.json" ] && [ -n "${ZED_AUTH_JSON_BOOTSTRAP:-}" ]; then
    printf '%s' "$ZED_AUTH_JSON_BOOTSTRAP" > "$ZED_HOME/auth.json"
    chown zed:zed "$ZED_HOME/auth.json" 2>/dev/null || true
    chmod 600 "$ZED_HOME/auth.json"
fi
...
# can set ZED_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# ZED_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$ZED_HOME/gateway_state.json" ] && \
        [ "${ZED_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    printf '{"gateway_state":"running"}\n' > "$ZED_HOME/gateway_state.json"
    chown zed:zed "$ZED_HOME/gateway_state.json" 2>/dev/null || true
    chmod 644 "$ZED_HOME/gateway_state.json"
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_zed "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/zed/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Zed spawns for the
...
# main-wrapper.sh propagates it into the supervised ``zed`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-zed, dashboard, etc.).
