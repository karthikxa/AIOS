...
# by /etc/cont-init.d/01-zed-setup (installed by the Dockerfile). This
# file exists so external references to docker/entrypoint.sh still work,
# but it's no longer the ENTRYPOINT â€” /init is.
#
...
# on the pre-s6 contract "entrypoint.sh sets up state then execs zed"
...
echo "[zed] WARNING: docker/entrypoint.sh is a deprecated shim under " \
    ...
    "as your ENTRYPOINT, drop the override â€” docker will use the image's " \
    "default ENTRYPOINT (/init), which handles bootstrap AND CMD." >&2
exec /opt/zed/docker/stage2-hook.sh "$@"
