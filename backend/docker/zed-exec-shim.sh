#!/bin/sh
# shellcheck shell=sh
# /opt/zed/bin/zed â€” `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `zed` user (UID 10000). When an operator runs `docker exec <c> zed ...`
# the default UID is root (0), and any file the command writes under
# $ZED_HOME â€” auth.json, .env, config.yaml â€” ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> zed login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Zed is not logged into Nous
# Portal" on every incoming message â€” even though `docker exec <c> zed
...
# Fix
# ---
# This shim sits at /opt/zed/bin/zed and is placed earliest on PATH.
# When invoked as root, it drops to the zed user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $ZED_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID â€” including the supervised processes themselves,
# `docker exec --user zed`, kanban subagents, etc. â€” it short-circuits
...
# (/opt/zed/.venv/bin/zed), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set ZED_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
# to keep running as root. Reserved for diagnostic sessions where the
# operator deliberately wants root semantics â€” e.g. inspecting root-only
# state via the zed CLI. Default is to drop.

set -e

REAL=/opt/zed/.venv/bin/zed

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "zed-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user zed`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${ZED_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the zed user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "zed-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "zed-shim: re-run with --user zed or set ZED_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the zed user's home before dropping privileges. Without
