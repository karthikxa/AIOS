...
from zed_cli.active_sessions import active_session_registry_snapshot
...
def test_session_create_rejects_at_active_session_limit(monkeypatch, tmp_path):
    ...
    token = set_zed_home_override(home)
    ...
    try:
        ...
        assert second["error"]["message"] == (
            "Zed is at the active session limit (1/1). "
...
def test_completion_cwd_prefers_profile_over_stale_env(monkeypatch, tmp_path):
    ...
    # No profile â†’ unchanged fallback to the launch env var.
...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["ZED_VOICE"]``
    ...
    # review on #19835).
    monkeypatch.setenv("ZED_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("ZED_VOICE", "1")
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("ZED_VOICE", "1")
    monkeypatch.delenv("ZED_VOICE_TTS", raising=False)
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("ZED_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("ZED_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
        "zed_cli.plugins",
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("ZED_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("ZED_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "zed_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    # whose tools live in zed-cli's universe (see toolsets.py).
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("ZED_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "zed_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("ZED_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("ZED_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("ZED_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("ZED_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "zed_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown ZED_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_session_resume_profile_uses_profile_db_cwd(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("zed_state.SessionDB", lambda db_path=None: profile_db)
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    monkeypatch.delenv("ZED_MODEL", raising=False)
    monkeypatch.setenv("ZED_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_resolve_model_strips_config_model(monkeypatch):
    monkeypatch.delenv("ZED_MODEL", raising=False)
    monkeypatch.delenv("ZED_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(
        server, "_load_cfg", lambda: {"model": {"default": " nous/zed-test "}}
    )

    assert server._resolve_model() == "nous/zed-test"
...
def test_config_sync_config_wins_over_env_seed(monkeypatch):
    # Hosted instances set ZED_INFERENCE_MODEL as a provision-time seed;
    # the per-turn sync must follow config.yaml edits, not stay pinned to it.
    monkeypatch.setenv("ZED_INFERENCE_MODEL", "seed/model")
    monkeypatch.delenv("ZED_MODEL", raising=False)
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("ZED_MODEL", "nous/zed-test")
    monkeypatch.setenv("ZED_TUI_PROVIDER", "nous")
    monkeypatch.delenv("ZED_INFERENCE_PROVIDER", raising=False)

    assert server._resolve_startup_runtime() == ("nous/zed-test", "nous")


def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    monkeypatch.setenv("ZED_MODEL", "nous/zed-test")
    monkeypatch.delenv("ZED_TUI_PROVIDER", raising=False)
    monkeypatch.setenv("ZED_INFERENCE_PROVIDER", "nous")
    monkeypatch.setattr(
        "zed_cli.models.detect_static_provider_for_model",
        lambda model, provider: None,
    )

    assert server._resolve_startup_runtime() == ("nous/zed-test", None)


def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("ZED_MODEL", "sonnet")
    monkeypatch.delenv("ZED_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("ZED_INFERENCE_PROVIDER", raising=False)
    ...
    monkeypatch.setattr(
        "zed_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_load_fallback_model_merges_chain_providers_first(monkeypatch):
    # Parity with ZedCLI / gateway: fallback_providers stays first and keeps
...
def test_startup_runtime_resolves_short_alias_without_network(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "zed_cli.models.fetch_openrouter_models",
...
def test_config_set_yolo_global_scope_writes_approvals_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_zed_home", tmp_path)
...
def test_config_set_fast_updates_live_agent_and_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "zed_cli.models.resolve_fast_mode_overrides",
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("ZED_YOLO_MODE", "false")
    ...
    assert resp["result"]["value"] == "1"
    assert os.environ.get("ZED_YOLO_MODE") == "1"
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    monkeypatch.delenv("ZED_EXEC_ASK", raising=False)
    monkeypatch.delenv("ZED_GATEWAY_SESSION", raising=False)
    monkeypatch.delenv("ZED_INTERACTIVE", raising=False)

    server._enable_gateway_prompts()

    assert server.os.environ["ZED_GATEWAY_SESSION"] == "1"
    assert server.os.environ["ZED_EXEC_ASK"] == "1"
    assert server.os.environ["ZED_INTERACTIVE"] == "1"


def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("zed_cli.main._has_any_provider_configured", lambda: False)
...
def test_setup_runtime_check_rejects_empty_runtime_key(monkeypatch):
    monkeypatch.setattr("zed_cli.main._has_any_provider_configured", lambda: True)
    monkeypatch.setattr(
        "zed_cli.runtime_provider.resolve_runtime_provider",
...
def test_config_set_model_requires_confirmation_for_expensive_model(monkeypatch):
    ...
    server._sessions["sid"] = _session(agent=agent)
    monkeypatch.setattr(
        "zed_cli.model_switch.switch_model", lambda **_kwargs: result
    )
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    monkeypatch.setattr("zed_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    monkeypatch.setattr("zed_cli.config.save_config", lambda cfg: saved.update(cfg))
...
def test_config_set_model_explicit_provider_skips_broken_default_init(monkeypatch):
    ...
    monkeypatch.setattr("zed_cli.runtime_provider.resolve_runtime_provider", fake_runtime_provider)
...
def test_config_set_model_does_not_leak_inference_provider_env(monkeypatch):
    ...
    process; writing ZED_INFERENCE_PROVIDER on a switch leaked the new
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("ZED_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setattr(
        "zed_cli.model_switch.switch_model", lambda **_kwargs: result
    )
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    ...
    try:
        ...
        assert os.environ["ZED_INFERENCE_PROVIDER"] == "openrouter"
...
def test_config_set_model_records_per_session_override_not_env(monkeypatch):
    ...
    try:
        ...
        assert "ZED_TUI_PROVIDER" not in os.environ
        assert "ZED_INFERENCE_PROVIDER" not in os.environ
...
def test_config_set_model_switches_agent_without_touching_env(monkeypatch):
    ...
    a per-session override; it does NOT write ZED_MODEL / ZED_TUI_PROVIDER
    ...
    session = _session(agent=agent)
    server._sessions["sid"] = session
    monkeypatch.setenv("ZED_TUI_PROVIDER", "openai-codex")
    monkeypatch.delenv("ZED_MODEL", raising=False)
    monkeypatch.delenv("ZED_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    monkeypatch.setattr("zed_cli.model_switch.switch_model", fake_switch_model)
    ...
    try:
        ...
        assert os.environ["ZED_TUI_PROVIDER"] == "openai-codex"
        assert "ZED_MODEL" not in os.environ
        assert "ZED_INFERENCE_MODEL" not in os.environ
    finally:
        server._sessions.clear()
...
def test_session_compress_syncs_session_key_after_rotation(monkeypatch):
    ...
    continuation session â€” mirrors ZedCLI._manual_compress's
...
def test_file_attach_uploads_remote_file_into_session_workspace(monkeypatch, tmp_path):
    """Remote case: client path doesn't exist on gateway â†’ decode data_url bytes."""
    ...
    try:
        ...
        stored = workspace / ".zed" / "desktop-attachments" / "report.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["path"] == str(stored)
        assert resp["result"]["ref_text"] == "@file:.zed/desktop-attachments/report.txt"
...
def test_file_attach_copies_gateway_visible_file_outside_workspace(monkeypatch, tmp_path):
    """Local case: gateway can see the file but it's outside the workspace â†’ copy in."""
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    ...
    try:
        ...
        stored = workspace / ".zed" / "desktop-attachments" / "outside.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["ref_text"] == "@file:.zed/desktop-attachments/outside.txt"
...
def test_file_attach_uses_in_workspace_file_without_copying(monkeypatch, tmp_path):
    """Local case: file already inside the workspace â†’ ref it directly, no copy."""
    workspace = tmp_path / "workspace"
    ...
    try:
        ...
        assert not (workspace / ".zed" / "desktop-attachments").exists()
    finally:
        server._sessions.pop("sid", None)


def test_file_attach_errors_when_unresolvable_and_no_bytes(monkeypatch, tmp_path):
    """Remote path not on gateway and no data_url â†’ actionable error, not a stage."""
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    fake_cli = types.ModuleType("cli")
    fake_cli._detect_file_drop = lambda raw: None
...
def test_file_attach_quotes_ref_with_spaces(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert resp["result"]["attached"] is True
        assert resp["result"]["ref_text"] == "@file:`.zed/desktop-attachments/my exam schedule.csv`"
    finally:
        server._sessions.pop("sid", None)
...
def test_commands_catalog_surfaces_quick_commands(monkeypatch):
    ...
    assert pairs["/git"].startswith("alias â†’")
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("zed_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "zed_cli.commands.SlashCommandCompleter",
...
# â”€â”€ session.steer â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# output (version mismatch) â€” both produce UI<->backend state desync.
# ---------------------------------------------------------------------------


def test_session_undo_rejects_while_running():
    """Fix for TUI silent-drop #1: /undo must not mutate history
    while the agent is mid-turn â€” would either clobber the undo or
...
def test_prompt_submit_history_version_mismatch_surfaces_warning(monkeypatch):
    ...
    try:
        ...
        assert "warning" in payload, (
            ...
            "history_version mismatch â€” otherwise the UI silently "
...
# session â€” it must not blast-resolve clarify/sudo/secret prompts on
...
def test_interrupt_only_clears_own_session_pending():
    ...
    try:
        ...
        # Session B's pending MUST remain untouched â€” no cross-session blast.
        assert not ev_b.is_set(), (
            "CRITICAL: session.interrupt on sid_a released a pending prompt "
            "belonging to sid_b â€” other sessions' clarify/sudo/secret "
...
def test_clear_pending_without_sid_clears_all():
    """_clear_pending(None) is the shutdown path â€” must still release
...
# self.base_url, self.client etc. in place â€” the worker thread running
...
def test_config_set_model_rejects_while_running(monkeypatch):
    ...
    try:
        ...
        assert not seen["called"], (
            "_apply_model_switch was called mid-turn â€” would race with "
...
def test_mirror_slash_side_effects_rejects_mutating_commands_while_running(monkeypatch):
    ...
    config.set â€” mutates live agent state while run_conversation is
    reading it."""
    import types
...
def test_mirror_slash_side_effects_allowed_when_idle(monkeypatch):
    ...
    # Should NOT contain "session busy" â€” the switch went through.
...
def test_session_create_close_race_does_not_orphan_worker(monkeypatch):
    ...
    resources leak â€” the subprocess stays alive until atexit and the
    ...
    # Make _build block until we release it â€” simulates slow agent init.
    ...
    # _make_agent â€” otherwise session.close pops _sessions[sid] before
    ...
    # NOW â€” this pops _sessions[sid] before _build can install the
    # worker/notify.
    ...
    # and let it finish â€” it should detect the orphan and clean up the
    ...
    ), f"orphan worker was not cleaned up â€” closed_workers={closed_workers}"
    ...
    assert len(unregistered_keys) >= 1, (
        f"orphan notify registration was not unregistered â€” "
...
def test_session_create_no_race_keeps_worker_alive(monkeypatch):
    ...
    try:
        ...
        # Build finished without a close race â€” nothing should have been
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("zed_state")
    ...
    monkeypatch.setitem(sys.modules, "zed_state", fake_mod)
...
# --------------------------------------------------------------------------
# session.delete â€” TUI resume picker `d` key
# --------------------------------------------------------------------------
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    """Happy path â€” DB delete succeeds, response carries the deleted id
    ...
    # too â€” not just the SQLite row.  The autouse _isolate_zed_home
    # fixture pins ZED_HOME to a temp dir; the handler should append
    # /sessions to it.
...
# --------------------------------------------------------------------------
# model.options â€” curated-list parity with `zed model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `zed model` and the classic CLI /model picker.
    ...
    with patch(
        "zed_cli.model_switch.list_authenticated_providers",
        return_value=curated_providers,
    ) as listing:
        # If provider_model_ids gets called at all, the handler is still
        # overwriting curated with live â€” that's the regression we're
        # guarding against.
        with patch("zed_cli.models.provider_model_ids") as live_fetch:
    ...
    # Handler must not consult the live catalog â€” curated is the truth.
...
def test_prompt_submit_surfaces_backend_error_as_visible_text(monkeypatch):
    ...
    â†’ provider 4xx), the TUI must surface result['error'] as visible text
...
def test_prompt_submit_preserves_empty_response_without_error(monkeypatch):
    """An empty final_response with NO backend error must stay empty â€” do not
    ...
    # Text stays empty â€” we did NOT fabricate an "Error:" string
    text = payload.get("text", "")
    assert text in {"", None}, f"expected empty text, got {text!r}"


# â”€â”€ active live TUI sessions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_session_active_list_excludes_finalized_sessions(monkeypatch):
    ...
    standalone ``zed --tui`` case) must still be reported.
    """
    class _DB:
        def get_session_title(self, key):
...
# â”€â”€ session.most_recent â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ browser.manage â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _stub_urlopen(monkeypatch, *, ok: bool):
    ...
    def _opener(_url, timeout=2.0):  # noqa: ARG001 â€” match urllib signature
        if not ok:
...
def _stub_urlopen_capture(monkeypatch, *, ok: bool):
    ...
    def _opener(url, timeout=2.0):  # noqa: ARG001 â€” match urllib signature
        urls.append(url)
        if not ok:
            raise OSError("probe failed")
        return _Resp()
...
def test_browser_manage_status_falls_back_to_config_cdp_url(monkeypatch):
    ...
    with patch.dict(sys.modules, {"zed_cli.config": fake_cfg}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_status_does_not_call_get_cdp_override(monkeypatch):
    ...
    fake = types.SimpleNamespace(
        _get_cdp_override=lambda: pytest.fail(  # noqa: PT015 â€” fail loudly if called
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "zed_cli.browser_connect.try_launch_chrome_debug", return_value=False
            ),
            patch(
                "zed_cli.browser_connect.get_chrome_debug_candidates",
                return_value=[],
            ),
        ):
            resp = server.handle_request(
...
def test_browser_manage_connect_default_local_retries_after_launch(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with patch(
            "zed_cli.browser_connect.try_launch_chrome_debug", return_value=True
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_browser_manage_connect_preserves_devtools_browser_endpoint(monkeypatch):
    ...
    survive verbatim â€” we only collapse discovery-style paths."""
    monkeypatch.delenv("BROWSER_CDP_URL", raising=False)
    fake = types.SimpleNamespace(
        cleanup_all_browsers=lambda: None,
        _get_cdp_override=lambda: os.environ.get("BROWSER_CDP_URL", ""),
...
def test_browser_manage_connect_concrete_ws_skips_http_probe(monkeypatch):
    ...
    # wss â†’ port 443, host preserved verbatim.
...
def test_browser_manage_connect_concrete_ws_tcp_unreachable(monkeypatch):
    ...
    return a clear 5031 error â€” no fallback to the HTTP probe (which
...
# â”€â”€ config.get indicator normalization â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_config_get_indicator_normalizes_casing_and_whitespace(monkeypatch):
    ...
    must do the same â€” otherwise `/indicator` prints 'EMOJI ' while the
...
# â”€â”€ config.set indicator validation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_config_set_indicator_falsy_non_string_surfaces_in_error(monkeypatch):
    ...
    must still tell the user what they sent â€” `value or ""` would have
    ...
    for bad in (0, False, []):
        ...
        # `[]` becomes an empty list â€” what matters is the diagnostic
...
def test_config_set_indicator_none_keeps_blank_repr(monkeypatch):
    """`None` is the genuine 'no value' case â€” empty raw is acceptable."""
    monkeypatch.setattr(server, "_write_config_key", lambda *a, **k: None)
    resp = server.handle_request(
        {
            "id": "1",
...
# â”€â”€ reload.env â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


def test_reload_env_rpc_calls_zed_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` â€” re-reads ~/.zed/.env
    ...
    with patch.dict(sys.modules, {"zed_cli.config": fake}):
...
# â”€â”€ max_iterations config reading â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_make_agent_waits_for_shared_mcp_discovery(monkeypatch):
    ...
    from zed_cli import mcp_startup
...
def test_notification_poller_delivers_completion(monkeypatch):
    ...
    # worker) is allowed to dequeue and dispatch it to its own session â€” whose
...
def test_session_save_writes_under_zed_home_with_system_prompt(monkeypatch, tmp_path):
    """TUI /save (session.save RPC) must snapshot under the Zed profile
    home â€” not the project/workspace CWD â€” and include the system prompt,
    mirroring the classic CLI /save and the dashboard save export.

    Regression: the gateway handler wrote ``zed_conversation_*.json`` to
    ...
    agent = types.SimpleNamespace(
        ...
        _cached_system_prompt="You are Zed.",
    )
    history = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
    ...
    assert not list(work.glob("zed_conversation_*.json"))
    ...
    assert payload["model"] == "zed-test"
    assert payload["session_id"] == "20260101_120000_abc123"
    assert payload["session_start"] == "2026-01-01T12:00:00"
    assert payload["system_prompt"] == "You are Zed."
...
def test_slash_worker_close_reaps_zombie_and_closes_fds():
    """A hung worker is SIGKILLed, the zombie reaped, all pipes closed â€” once."""
...
def test_session_create_records_ui_model_as_session_override(monkeypatch):
    ...
    into the agent), never a global config write â€” picking a model for a new chat
    ...
    try:
        ...
        # No knobs â†’ no overrides; the session builds from the profile default.
...
def test_start_agent_build_passes_session_model_override(monkeypatch):
    ...
    composer) must reach _make_agent so the first build runs on it directly â€”
