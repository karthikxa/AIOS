...
`zed update` keeps (does not overwrite) bundled skills the user edited
locally, but historically only printed a *count* â€” there was no way to find
...
* ``list_user_modified_bundled_skills()`` â€” the discovery half of the exact
  test the sync loop uses to decide what to skip.
* ``diff_bundled_skill()`` â€” a unified diff of the user copy vs the stock copy.
