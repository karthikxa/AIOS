...
to LF whenever it edits them â€” and patch produces a mixed-ending file
...
@pytest.fixture
def zed_home(monkeypatch, tmp_path):
    """Isolate ZED_HOME so the tests don't pollute the real config.
...
class TestPatchCRLFPreservation:
    def test_patch_on_crlf_file_stays_pure_crlf(self, zed_home, tmp_path):
    ...
    def test_patch_on_lf_file_stays_lf(self, zed_home, tmp_path):
        """LF file with LF new_string stays LF â€” no spurious CRLF added."""
        from tools.file_tools import _handle_patch
    ...
    def test_patch_multiline_replacement_on_crlf(self, zed_home, tmp_path):
...
class TestWriteFileCRLFPreservation:
    ...
    def test_new_file_written_as_is(self, zed_home, tmp_path):
        """No pre-existing file â†’ write content verbatim (LF by default)."""
        from tools.file_tools import _handle_write_file
    ...
    def test_overwrite_lf_file_stays_lf(self, zed_home, tmp_path):
...
class TestLineEndingHelpers:
    """Direct unit tests for the pure helpers â€” easier to debug than the
    ...
    def test_detect_mixed_picks_crlf(self):
        """Mixed-ending content (any CRLF in the head) returns CRLF â€”
