...
1. Tool resolution â€” terminal + file tools load correctly
2. CWD fix â€” host paths get replaced with /root for container backends
3. ephemeral_disk version check
4. ensurepip fix in Modal image builder
5. No swe-rex dependency â€” uses native Modal SDK
...
try:
    ...
    pytest.skip("zed-agent tools not importable (missing deps)", allow_module_level=True)
...
class TestCwdHandling:
    ...
    def test_home_path_replaced_for_modal(self, monkeypatch):
        ...
        monkeypatch.setenv("TERMINAL_CWD", "/home/dakota/github/zed-agent")
