"""Regression test for the ``ZedMCPOAuthProvider.async_auth_flow`` bidirectional
generator bridge.
...
``httpx``'s auth_flow contract is a **bidirectional** async generator â€” the
...
the full ``.asend()`` round-trip â€” the integration tests in
...
@pytest.mark.asyncio
async def test_zed_provider_forwards_asend_values(tmp_path, monkeypatch):
    ...
    from tools.mcp_oauth_manager import _ZED_PROVIDER_CLS, reset_manager_for_tests

    assert _ZED_PROVIDER_CLS is not None, "SDK OAuth types must be available"

    monkeypatch.setenv("ZED_HOME", str(tmp_path))
    reset_manager_for_tests()

    # Seed a valid-looking token so the SDK's _initialize loads something and
    # can_refresh_token() is True (though we don't exercise refresh here â€” we
    # go straight through the 200 path).
    storage = ZedTokenStorage("srv")
    ...
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="Zed Agent",
    )
    provider = _ZED_PROVIDER_CLS(
        server_name="srv",
...
@pytest.mark.asyncio
async def test_zed_provider_forwards_401_triggers_refresh(tmp_path, monkeypatch):
    ...
    assert _ZED_PROVIDER_CLS is not None
    ...
    # back â€” any request. The broken bridge would have crashed with
    ...
    # Clean up the generator â€” we don't need to complete the full dance.
    await flow.aclose()
