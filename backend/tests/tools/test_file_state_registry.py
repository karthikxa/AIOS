...
def _tmp_file(content: str = "initial\n") -> str:
    fd, path = tempfile.mkstemp(prefix="zed_file_state_test_", suffix=".txt")
...
class FileStateRegistryUnitTests(unittest.TestCase):
    ...
    def test_own_write_updates_stamp_so_next_write_is_clean(self):
        ...
        # Second write by the same agent â€” should not be flagged.
        self.assertIsNone(file_state.check_stale("A", p))
    ...
    def test_writes_since_filters_by_parent_read_set(self):
        ...
        since = time.time()
        time.sleep(0.01)
        file_state.note_write("child", foo)  # parent read this â€” report
        file_state.note_write("child", baz)  # parent never saw â€” skip
    ...
    def test_kill_switch_env_var(self):
        p = self._mk()
        os.environ["ZED_DISABLE_FILE_STATE_GUARD"] = "1"
        try:
            file_state.record_read("A", p)
            file_state.note_write("B", p)
            self.assertIsNone(file_state.check_stale("A", p))
            ...
            del os.environ["ZED_DISABLE_FILE_STATE_GUARD"]


class FileToolsIntegrationTests(unittest.TestCase):
    """Integration through the real file_tools handlers.

    These exercise the wiring: read_file_tool â†’ registry.record_read,
    write_file_tool / patch_tool â†’ check_stale + lock_path + note_write.
    """

    def setUp(self) -> None:
        file_state.get_registry().clear()
        self._tmpdir = tempfile.mkdtemp(prefix="zed_file_state_int_")
    ...
    def test_patch_tool_also_surfaces_sibling_warning(self):
        ...
        # match) or succeed â€” either way, the cross-agent warning should be
