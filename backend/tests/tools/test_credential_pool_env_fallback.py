...
- _seed_from_env reads API keys from ~/.zed/.env when not in os.environ
...
@pytest.fixture
def isolated_zed_home(tmp_path, monkeypatch):
    """Point ZED_HOME at a temp dir and clear known API key env vars.
...
def _write_env_file(home: Path, **kwargs) -> None:
    """Write key=value pairs to ~/.zed/.env."""
    lines = [f"{k}={v}" for k, v in kwargs.items()]
    (home / ".env").write_text("\n".join(lines) + "\n")


class TestCredentialPoolSeedsFromDotEnv:
    """_seed_from_env must read keys from ~/.zed/.env, not just os.environ.
    ...
    load_zed_dotenv, the credential pool must still discover it.
    """

    def test_deepseek_key_from_dotenv_only(self, isolated_zed_home):
        """Key in .env but not os.environ â†’ _seed_from_env adds a pool entry."""
        _write_env_file(isolated_zed_home, DEEPSEEK_API_KEY="sk-dotenv-only-12345")
    ...
    def test_openrouter_key_from_dotenv_only(self, isolated_zed_home):
        """OpenRouter path has its own branch â€” verify it also reads .env."""
        _write_env_file(isolated_zed_home, OPENROUTER_API_KEY="sk-or-dotenv-abc")
    ...
    def test_empty_dotenv_no_entries(self, isolated_zed_home):
        """No .env file, no env vars â†’ no entries seeded (and no crash)."""
        from agent.credential_pool import _seed_from_env
        entries = []
        changed, active_sources = _seed_from_env("deepseek", entries)
...
class TestAuthResolvesFromDotEnv:
    """_resolve_api_key_provider_secret must also read from ~/.zed/.env."""

    def test_key_from_dotenv_only(self, isolated_zed_home):
        """Key in .env but not os.environ â†’ _resolve returns it with the env var source."""
        _write_env_file(isolated_zed_home, DEEPSEEK_API_KEY="sk-dotenv-resolve-789")
        assert "DEEPSEEK_API_KEY" not in os.environ

        from zed_cli.auth import _resolve_api_key_provider_secret
...
class TestAuthCredentialPoolFallback:
    ...
    def test_credential_pool_fallback_structure(self, isolated_zed_home):
        """Empty env + empty .env â†’ auth falls back to credential pool."""
    ...
    def test_credential_pool_empty_returns_empty(self, isolated_zed_home):
        """Empty env + empty .env + empty pool â†’ empty string."""
        mock_pool = MagicMock()
    ...
    def test_env_var_takes_priority_over_pool(self, isolated_zed_home, monkeypatch):
        """os.environ key wins â€” credential pool is NEVER consulted."""
        ...
        assert source == "DEEPSEEK_API_KEY"
        # Pool should not even have been loaded â€” env var satisfied the request first
        mp.assert_not_called()

    def test_dotenv_takes_priority_over_pool(self, isolated_zed_home):
        """Key in .env beats credential pool â€” pool only fires when both env sources are empty."""
        _write_env_file(isolated_zed_home, DEEPSEEK_API_KEY="sk-dotenv-priority-xyz")
        assert "DEEPSEEK_API_KEY" not in os.environ
