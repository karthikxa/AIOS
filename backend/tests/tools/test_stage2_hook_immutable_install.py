...
Hosted/container Zed keeps user-writable state under ZED_HOME
(/opt/data). The installed source, venv, TUI bundle, and node_modules under
/opt/zed must remain root-owned/non-writable by the runtime zed user so
...
def test_stage2_does_not_chown_install_tree_to_zed(stage2_text: str) -> None:
    ...
    for install_tree in (
        ...
        assert install_tree not in stage2_text, (
            f"stage2 must not chown {install_tree} back to zed; "
            "the Dockerfile keeps /opt/zed immutable and writable state "
            "belongs under ZED_HOME"
        )
...
def test_stage2_documents_immutable_install_contract(stage2_text: str) -> None:
    ...
    assert "ZED_DISABLE_LAZY_INSTALLS=1" in stage2_text
    assert "/opt/zed" in stage2_text
