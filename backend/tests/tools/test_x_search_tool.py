...
# retry behavior) â€” preserved verbatim. Uses XAI_API_KEY env var via the
...
def test_x_search_posts_responses_request(monkeypatch):
    ...
    assert captured["headers"]["User-Agent"] == f"Zed-Agent/{__version__}"
...
# ---------------------------------------------------------------------------
# Credential-resolution coverage â€” the OAuth-or-API-key gating contract.
# ---------------------------------------------------------------------------

def _no_xai_env(monkeypatch):
    """Strip any XAI_* env vars so the resolver doesn't see a leaked dev key."""
    for var in ("XAI_API_KEY", "XAI_BASE_URL", "ZED_XAI_BASE_URL"):
...
def test_x_search_prefers_oauth_when_both_available(monkeypatch):
    ...
    â€” OAuth runtime first, fallback OAuth resolver second, ``XAI_API_KEY`` third.
...
def test_x_search_returns_tool_error_when_no_credentials(monkeypatch):
    ...
    assert "zed auth add xai-oauth" in result
...
def test_x_search_honors_config_model_and_timeout(monkeypatch, tmp_path):
    ...
    # Patch the in-module config loader so tests don't touch ~/.zed/config.yaml.
    monkeypatch.setattr(
...
def test_x_search_registered_in_registry_with_check_fn():
    ...
    import tools.x_search_tool  # noqa: F401 â€” ensures registration runs
...
# ---------------------------------------------------------------------------
# Date validation â€” fail fast before burning an API call on a window that
...
def _no_post_allowed(monkeypatch):
    ...
    def _fail(*_, **__):
        raise AssertionError("requests.post must not be called â€” validation should reject first")
...
def test_x_search_rejects_future_from_date(monkeypatch):
    """``from_date`` in the future can never match any post â†’ reject."""
...
def test_x_search_allows_future_to_date(monkeypatch):
    """``to_date`` in the future is fine â€” caller may want posts as they arrive."""
    import datetime as _dt
...
# ---------------------------------------------------------------------------
# Degraded-result flag â€” distinguish citation-backed answers from
...
def test_x_search_marks_degraded_when_handle_filter_returns_no_citations(monkeypatch):
    """allowed_x_handles set + zero citations â†’ degraded=True."""
    from tools.x_search_tool import x_search_tool
