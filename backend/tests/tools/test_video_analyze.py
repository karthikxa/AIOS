...
# ---------------------------------------------------------------------------
# video_analyze_tool â€” integration-style tests with mocked LLM
# ---------------------------------------------------------------------------
...
class TestVideoAnalyzeTool:
    ...
    def test_local_file_success(self, tmp_path, monkeypatch):
        """Analyze a local video file â€” happy path."""
    ...
    def test_video_too_large(self, tmp_path, monkeypatch):
        ...
        # Don't actually write 50MB â€” mock the stat
        video.write_bytes(b"\x00" * 100)
...
class TestVideoToolsetRegistration:
    ...
    def test_not_in_core_tools(self):
        """video_analyze should NOT be in _ZED_CORE_TOOLS (default disabled)."""
        from toolsets import _ZED_CORE_TOOLS
        assert "video_analyze" not in _ZED_CORE_TOOLS
