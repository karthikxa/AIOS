...
at startup, by THREE separate code paths:
  ...
  2. gateway/run.py    -> ``_terminal_env_map`` dict (gateway / messaging
                          platforms)
  3. zed_cli/config.py:set_config_value
                       -> bridges via the canonical ``TERMINAL_CONFIG_ENV_MAP``
                          (one-shot when the user runs ``zed config set â€¦``)
...
dicts) keeps the test independent of the user's ~/.zed/config.yaml and
mirrors the pattern used in tests/zed_cli/test_config_drift.py.
"""
...
def _save_config_env_sync_keys() -> set[str]:
    """terminal config keys bridged by ``zed config set foo bar``.

    ``set_config_value`` no longer carries its own ``_config_to_env_sync``
    dict â€” it bridges through the canonical ``TERMINAL_CONFIG_ENV_MAP`` via
    ...
    """
    from zed_cli import config as hc_config
...
_CLI_ONLY_OK = frozenset({
    ...
    "env_type",
    # sudo_password is not a terminal-backend option â€” it's a credential
...
def test_cli_and_gateway_env_maps_agree():
    ...
    mode" (or vice-versa) â€” the bug class that shipped twice already.
    """
    ...
    # `backend`.  Since cli.py copies `backend` â†’ `env_type` before the
...
def test_save_config_set_supports_critical_bridged_keys():
    """``zed config set terminal.X true`` must propagate to .env for
    ...
    gaps, are now bridged â€” see the dedicated tests below.)
    """
    ...
    assert not missing, (
        f"`zed config set terminal.X` doesn't sync these load-bearing "
        f"keys to .env: {sorted(missing)}.  Add them to TERMINAL_CONFIG_ENV_MAP "
        f"in zed_cli/config.py (set_config_value bridges through it)."
    )
...
def test_docker_mount_cwd_to_workspace_is_bridged_everywhere():
    """Same regression class â€” docker_mount_cwd_to_workspace was missing from
...
def test_docker_persist_across_processes_is_bridged_everywhere():
    ...
    container on Zed exit or just stops it (keeping it for the next
    process).  Same four-bridge invariant as docker_run_as_host_user /
    docker_env / docker_mount_cwd_to_workspace â€” drift between any of the
...
def test_docker_orphan_reaper_is_bridged_everywhere():
    ...
    ``terminal.docker_orphan_reaper`` controls whether Zed sweeps stale
    Exited containers from prior SIGKILL'd processes at startup.  Same
    four-site bridge invariant â€” drift means
...
def test_docker_volumes_is_bridged_everywhere():
    ...
    ``zed config set terminal.docker_volumes '["/host:/workspace"]'`` wrote
    config.yaml yet left the running process's TERMINAL_DOCKER_VOLUMES stale â€”
...
def test_docker_forward_env_is_bridged_everywhere():
    """Regression pin for ``terminal.docker_forward_env`` â€” the sibling gap to
    docker_volumes.
    ...
    ``zed config set terminal.docker_forward_env '["GITHUB_TOKEN"]'`` had no
