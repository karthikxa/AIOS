...
Three calling shapes:
  1. DISCOVERY â€” pass query â†’ FTS5 + anchored window + bookends per hit
  2. SCROLL    â€” pass session_id + around_message_id â†’ just the window
  3. BROWSE    â€” no args â†’ recent sessions chronologically
...
def _seed_modpack_sessions(db):
    ...
    now = int(time.time())
    # Older session â€” modpack origin
    ...
    # Middle session â€” modpack quest coverage
    ...
    db.append_message("s_middle", role="assistant", content="Modpack version bumped 0.4 â†’ 0.8.5; quest coverage page added.")

    # Newest session â€” modpack mob spawn fix
...
class TestSchema:
    ...
    def test_no_mode_parameter(self):
        # Mode is inferred from which args are set â€” no explicit mode param
        params = SESSION_SEARCH_SCHEMA["parameters"]["properties"]
...
class TestShapePrecedence:
    def test_scroll_args_beat_query(self, db):
        ...
        anchor_sid = disc["results"][0]["session_id"]
        anchor_mid = disc["results"][0]["match_message_id"]
        # Pass both query and scroll args â€” scroll should win
        result = json.loads(session_search(
    ...
    def test_session_id_without_anchor_reads(self, db):
        _seed_modpack_sessions(db)
        # session_id alone (no anchor, no query) â†’ read shape, not browse.
        result = json.loads(session_search(session_id="s_oldest", db=db))
        assert result["mode"] == "read"


# =========================================================================
# Read shape â€” dump a whole session by id (serves @session links)
# =========================================================================
