"""Tests for tools/vision_tools.py â€” URL validation, type hints, error logging."""
...
# ---------------------------------------------------------------------------
# _validate_image_url â€” urlparse-based validation
# ---------------------------------------------------------------------------
...
class TestValidateImageUrl:
    ...
    def test_rejects_http_without_netloc(self):
        """http:// alone has no network location â€” urlparse catches this."""
...
# ---------------------------------------------------------------------------
# _handle_vision_analyze â€” type signature & behavior
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Error logging with exc_info â€” verify tracebacks are logged
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# _resize_image_for_vision â€” auto-resize oversized images
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# _image_exceeds_dimension â€” proactive embed-time pixel-cap detector
# ---------------------------------------------------------------------------
...
class TestImageExceedsDimension:
    ...
    def test_missing_pillow_returns_false(self, tmp_path):
        # Without Pillow we can't inspect dimensions â€” return False so the
...
# ---------------------------------------------------------------------------
# _is_image_size_error â€” detect size-related API errors
# ---------------------------------------------------------------------------
...
class TestDownloadRetryClassification:
    ...
    async def test_404_fails_fast_without_retry(self, tmp_path):
        """A 404 must raise on the first attempt â€” no backoff sleep, no extra GETs."""
        import httpx
        from tools.vision_tools import _download_image
