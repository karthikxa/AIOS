...
Background
==========
For every connected MCP server, zed-agent used to register four "utility"
...
all four methods on the class â€” independent of what the remote server
supports.

Tools-only servers like ``@upstash/context7-mcp`` advertise
``{\"tools\": {\"listChanged\": true}}`` in their ``initialize`` response â€”
no ``prompts`` or ``resources`` keys â€” and they return JSON-RPC
...
def _make_fake_server(*, initialize_result):
    ...
    loop for the ``Event``/``Lock`` init â€” overkill for unit scope.
    """
    server = MagicMock()
...
class TestCapabilityGatedRegistration:
    def test_tools_only_server_gets_no_utility_schemas(self):
        ...
        get zero utility stubs registered â€” this is the exact scenario
