...
The original implementation only had two states â€” closed and open â€” with
...
def test_circuit_breaker_half_opens_after_cooldown(monkeypatch, tmp_path):
    ...
    try:
        ...
        # fail at the .get() inside the gate (correct â€” the fix is
        ...
        # Advance past cooldown â†’ next call is a half-open probe that
...
def test_circuit_breaker_reopens_on_probe_failure(monkeypatch, tmp_path):
    ...
    try:
        ...
        # The probe failure must have re-armed the cooldown â€” another
...
def test_circuit_breaker_cleared_on_reconnect(monkeypatch, tmp_path):
    ...
    stale failure count â€” even if the post-reconnect retry itself fails.
    ...
    try:
        ...
        # count to 1 â€” still below threshold.
        def _retry_call():
        ...
        # bumped it to exactly 1 â€” well below threshold.
