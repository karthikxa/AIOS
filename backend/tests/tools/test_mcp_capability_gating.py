...
is non-None iff the server supports it. Before the capability gate, Zed
...
class TestAdvertisesTools:
    ...
    def test_legacy_fallback_no_initialize_result(self):
        """No captured capabilities â†’ preserve old always-list_tools behavior."""
        task = MCPServerTask("test")
...
class TestKeepaliveProbe:
    ...
    async def test_keepalive_uses_ping_legacy_fallback(self):
        """No captured capabilities â†’ still pings (no spurious list_tools)."""
        task = MCPServerTask("test")
        assert task.initialize_result is None
        task.session = SimpleNamespace(
            list_tools=AsyncMock(),
...
class TestMethodNotFoundDetection:
    """``_is_method_not_found_error`` underpins the pingâ†’list_tools fallback."""
...
class TestKeepaliveProbeFallback:
    ...
    that don't implement the optional ping utility â€” without reconnect-looping,
    ...
    async def test_no_ping_no_tools_propagates_method_not_found(self):
        ...
        probe â€” the -32601 must propagate rather than calling list_tools on a
