...
def test_yolo_env_var_cannot_bypass_hardline(clean_session, monkeypatch):
    """ZED_YOLO_MODE=1 must not bypass the hardline floor."""
...
def test_approvals_mode_off_cannot_bypass_hardline(clean_session, monkeypatch, tmp_path):
    ...
    # _get_approval_mode() reads from zed config; simplest path: monkeypatch the helper.
...
def test_container_backends_still_bypass(clean_session):
    """Containerized backends remain bypass-approved â€” they can't touch the host.
...
def test_recoverable_dangerous_commands_still_pass_yolo(clean_session, monkeypatch):
    ...
    This confirms we haven't broken the yolo escape hatch â€” only narrowed it.
    """
    monkeypatch.setenv("ZED_YOLO_MODE", "1")

    # These are dangerous but NOT hardline â€” yolo should still pass them.
...
def test_hardline_list_is_small():
    ...
    If you're adding a 20th+ pattern, reconsider â€” it probably belongs in
...
# =========================================================================
# Sudo stdin guard â€” blocks "sudo -S" without SUDO_PASSWORD
# =========================================================================
...
_SUDO_STDIN_ALLOW = [
    # Plain sudo without -S â€” goes through normal approval
    "sudo whoami",
    "sudo apt-get update",
    "sudo -u root whoami",
...
def test_sudo_stdin_guard_container_bypass(clean_session):
    """Containerized backends still bypass â€” they can't touch the host."""
    for env in ("docker", "singularity", "modal", "daytona"):
        for cmd in _SUDO_STDIN_BLOCK:
