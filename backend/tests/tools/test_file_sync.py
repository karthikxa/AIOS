"""Tests for FileSyncManager â€” mtime tracking, deletion detection, transactional rollback."""
...
def _make_get_files(tmp_files, remote_base="/root/.zed"):
...
def _make_manager(tmp_files, remote_base="/root/.zed", upload=None, delete=None):
...
class TestTransactionalRollback:
    def test_upload_failure_rolls_back(self, tmp_files):
        ...
        # State should be empty (rolled back) â€” next sync retries all files
...
class TestEdgeCases:
    ...
    def test_file_disappears_between_list_and_upload(self, tmp_path):
        ...
        upload = MagicMock()
        mgr = FileSyncManager(
            get_files_fn=lambda: [(str(f), "/root/.zed/ephemeral.txt")],
            upload_fn=upload,
            delete_fn=MagicMock(),
        )
