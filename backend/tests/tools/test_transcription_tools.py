"""Tests for tools.transcription_tools â€” three-provider STT pipeline.
...
def clean_env(monkeypatch):
    ...
    monkeypatch.delenv("ZED_LOCAL_STT_COMMAND", raising=False)
    monkeypatch.delenv("ZED_LOCAL_STT_LANGUAGE", raising=False)


# ============================================================================
# _get_provider â€” full permutation matrix
# ============================================================================
...
class TestGetProviderGroq:
    ...
    def test_groq_explicit_no_fallback(self, monkeypatch):
        """Explicit groq with no key returns none â€” no cross-provider fallback."""
        monkeypatch.delenv("GROQ_API_KEY", raising=False)
...
class TestGetProviderFallbackPriority:
    ...
    def test_explicit_openai_no_key_returns_none(self, monkeypatch):
        """Explicit openai with no key returns none â€” no cross-provider fallback."""
        monkeypatch.delenv("VOICE_TOOLS_OPENAI_KEY", raising=False)
        monkeypatch.delenv("GROQ_API_KEY", raising=False)
        with patch("tools.transcription_tools._HAS_FASTER_WHISPER", False), \
             patch("tools.transcription_tools._HAS_OPENAI", True):
...
class TestExplicitProviderRespected:
    ...
    def test_explicit_local_uses_local_command_fallback(self, monkeypatch):
        """Local-to-local_command fallback is fine â€” both are local."""
        monkeypatch.setenv(
...
# ============================================================================
# _transcribe_openai â€” additional tests
# ============================================================================
...
class TestTranscribeLocalCommand:
    ...
    def test_command_fallback_with_template(self, monkeypatch, sample_ogg, tmp_path):
        ...
        monkeypatch.setenv("ZED_LOCAL_STT_LANGUAGE", "en")
...
# ============================================================================
# _transcribe_local â€” additional tests
# ============================================================================
...
class TestTranscribeLocalExtended:
    ...
    def test_load_time_cuda_lib_failure_falls_back_to_cpu(self, tmp_path):
        """Missing libcublas at load time â†’ reload on CPU, succeed."""
        audio = tmp_path / "test.ogg"
        audio.write_bytes(b"fake")
    ...
    def test_runtime_cuda_lib_failure_evicts_cache_and_retries_on_cpu(self, tmp_path):
        """libcublas dlopen fails at transcribe() â†’ evict cache, reload CPU, retry."""
        audio = tmp_path / "test.ogg"
        audio.write_bytes(b"fake")
    ...
    def test_cuda_out_of_memory_does_not_trigger_cpu_fallback(self, tmp_path):
        """'CUDA out of memory' is a real error, not a missing lib â€” surface it."""
        audio = tmp_path / "test.ogg"
        audio.write_bytes(b"fake")
        ...
        # Single call â€” no CPU retry, because OOM isn't a missing-lib symptom.
        assert mock_whisper_cls.call_count == 1
        assert result["success"] is False
        assert "CUDA out of memory" in result["error"]
...
# ============================================================================
# _validate_audio_file â€” edge cases
# ============================================================================
...
# ============================================================================
# transcribe_audio â€” end-to-end dispatch
# ============================================================================
...
class TestGetProviderMistral:
    ...
    def test_mistral_explicit_no_key_returns_none(self, monkeypatch):
        """Explicit mistral with no key returns none â€” no cross-provider fallback."""
        monkeypatch.delenv("MISTRAL_API_KEY", raising=False)
        with patch("tools.transcription_tools._HAS_MISTRAL", True):
            from tools.transcription_tools import _get_provider
...
# ============================================================================
# transcribe_audio â€” Mistral dispatch
# ============================================================================
...
def mock_xai_http_module():
    ...
    fake_module = MagicMock()
    fake_module.zed_xai_user_agent = MagicMock(return_value="zed-xai/test")
...
class TestTranscribeXAI:
    ...
    def test_sends_language_and_format(self, monkeypatch, sample_ogg, mock_xai_http_module):
        ...
        monkeypatch.setenv("ZED_LOCAL_STT_LANGUAGE", "fr")
...
class TestGetProviderXAI:
    ...
    def test_xai_explicit_no_key_returns_none(self, monkeypatch):
        """Explicit xai with no key returns none â€” no cross-provider fallback."""
        monkeypatch.delenv("XAI_API_KEY", raising=False)
        from tools.transcription_tools import _get_provider
...
# ============================================================================
# transcribe_audio â€” xAI dispatch
# ============================================================================
...
class TestGetProviderElevenLabs:
    ...
    def test_elevenlabs_explicit_no_key_returns_none(self, monkeypatch):
        """Explicit elevenlabs with no key returns none â€” no cross-provider fallback."""
        monkeypatch.delenv("ELEVENLABS_API_KEY", raising=False)
        from tools.transcription_tools import _get_provider
...
# ============================================================================
# transcribe_audio â€” ElevenLabs dispatch
# ============================================================================
...
# Shell safety â€” shlex.split on auto-detected templates
# ============================================================================
class TestShellSafety:
    ...
    def test_env_var_template_uses_shell_path(self, monkeypatch):
        """When ZED_LOCAL_STT_COMMAND is set, use_shell should be True."""
        import os
