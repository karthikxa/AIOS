...
1. ``ZedTokenStorage.set_tokens`` persisted only relative ``expires_in``,
...
â€” some return HTTP 401 (caught by the consolidation's 401 handler, which
...
- ``ZedMCPOAuthProvider._initialize`` seeds ``context.token_expiry_time``
...
# ---------------------------------------------------------------------------
# ZedTokenStorage â€” absolute expiry persistence
# ---------------------------------------------------------------------------
...
class TestGetTokensReconstructsExpiresIn:
    ...
    def test_get_tokens_returns_zero_ttl_for_expired_token(
        ...
        monkeypatch.setenv("ZED_HOME", str(tmp_path))
        from tools.mcp_oauth import ZedTokenStorage, _get_token_dir
...
# ---------------------------------------------------------------------------
# ZedMCPOAuthProvider._initialize â€” seed token_expiry_time
# ---------------------------------------------------------------------------
...
async def test_initialize_prefetches_oauth_metadata_when_missing(
    ...
    and we drop into full browser re-auth â€” visible to the user as an
...
async def test_initialize_skips_prefetch_when_no_tokens(tmp_path, monkeypatch):
    ...
    storage = ZedTokenStorage("srv")  # empty â€” no tokens on disk
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="Zed Agent",
    )
