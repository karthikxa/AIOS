...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        zed_home.mkdir()
        (zed_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"ZED_HOME": str(zed_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(zed_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.zed/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' â€” both should work."""
        zed_home = tmp_path / ".zed"
        zed_home.mkdir()
        (zed_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        zed_home = tmp_path / ".zed"
        zed_home.mkdir()
        (zed_home / "secret.key").write_text("key")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.zed/skills"
    ...
    def test_returns_none_when_no_skills_dir(self, tmp_path):
        ...
        # No local skills dir â†’ no local mount (external dirs may still appear)
        local_mounts = [m for m in mounts if m["container_path"].endswith("/skills")]
        assert local_mounts == []

    def test_custom_container_base(self, tmp_path):
        zed_home = tmp_path / ".zed"
        (zed_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"ZED_HOME": str(zed_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.zed")

        assert mounts[0]["container_path"] == "/home/user/.zed/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.zed/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.zed/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape ZED_HOME."""
        monkeypatch.setenv("ZED_HOME", str(tmp_path / ".zed"))
        (tmp_path / ".zed").mkdir()

        # Create a sensitive file one level above zed_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside zed_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside ZED_HOME must still be registered."""
        zed_home = tmp_path / ".zed"
        zed_home.mkdir()
        monkeypatch.setenv("ZED_HOME", str(zed_home))
        (zed_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_zed_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of ZED_HOME must be allowed."""
        zed_home = tmp_path / ".zed"
        zed_home.mkdir()
        subdir = zed_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside ZED_HOME pointing outside must be rejected."""
        zed_home = tmp_path / ".zed"
        zed_home.mkdir()
        monkeypatch.setenv("ZED_HOME", str(zed_home))

        # Create a sensitive file outside zed_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside zed_home pointing outside
        symlink = zed_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes ZED_HOME â€” must be rejected
        assert result is False
        assert get_credential_file_mounts() == []


# ---------------------------------------------------------------------------
# Config-based credential files â€” same containment checks
# ---------------------------------------------------------------------------

class TestConfigPathTraversal:
    """terminal.credential_files in config.yaml must also reject traversal."""

    def _write_config(self, zed_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape ZED_HOME."""
        zed_home = tmp_path / ".zed"
        zed_home.mkdir()
        monkeypatch.setenv("ZED_HOME", str(zed_home))
        ...
        sensitive.write_text("{}")
        self._write_config(zed_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(zed_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside ZED_HOME via config must still mount."""
        zed_home = tmp_path / ".zed"
        zed_home.mkdir()
        monkeypatch.setenv("ZED_HOME", str(zed_home))

        (zed_home / "oauth.json").write_text("{}")
        self._write_config(zed_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        zed_home = tmp_path / ".zed"
        zed_home.mkdir()
        (zed_home / "cache" / "documents").mkdir(parents=True)
        (zed_home / "cache" / "audio").mkdir(parents=True)
        (zed_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("ZED_HOME", str(zed_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.zed/cache/documents" in paths
        assert "/root/.zed/cache/audio" in paths
        assert "/root/.zed/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.zed/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        zed_home = tmp_path / ".zed"
        zed_home.mkdir()
        # Use legacy dir name â€” get_zed_dir prefers old if it exists
        (zed_home / "document_cache").mkdir()
        (zed_home / "image_cache").mkdir()
        monkeypatch.setenv("ZED_HOME", str(zed_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(zed_home / "document_cache") in host_paths
        assert str(zed_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.zed/cache/documents" in container_paths
        assert "/root/.zed/cache/images" in container_paths

    def test_empty_zed_home(self, tmp_path, monkeypatch):
        """No cache dirs â†’ empty list."""
        zed_home = tmp_path / ".zed"
        zed_home.mkdir()
        monkeypatch.setenv("ZED_HOME", str(zed_home))
...
class TestMapCachePathToContainer:
    """Tests for map_cache_path_to_container() â€” the backend-agnostic mapper."""

    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        zed_home = tmp_path / ".zed"
        img_dir = zed_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.zed/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.zed")
            == "/home/agent/.zed/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        zed_home = tmp_path / ".zed"
        (zed_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("ZED_HOME", str(zed_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(zed_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        zed_home = tmp_path / ".zed"
        doc_dir = zed_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        zed_home = tmp_path / ".zed"
        ss_dir = zed_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.zed/cache/screenshots/session_abc/screen1.png"
