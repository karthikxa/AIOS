...
def _restore_tool_modules():
    original_zed_home = os.environ.get("ZED_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_zed_home is None:
            os.environ.pop("ZED_HOME", None)
        else:
            os.environ["ZED_HOME"] = original_zed_home
...
def _install_modal_test_modules(
    ...
    *,
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="zed-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": zed_home / "modal_snapshots.json",
