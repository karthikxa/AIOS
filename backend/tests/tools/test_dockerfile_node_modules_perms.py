...
Older images made /opt/zed/ui-tui and /opt/zed/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV ZED_TUI_DIR=/opt/zed/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R zed:zed /opt/zed/ui-tui" not in text
    assert "chown -R zed:zed /opt/zed/node_modules" not in text
