...
Mirrors ``tests/tools/test_tts_command_providers.py`` â€” same shape, same
invariants, adapted for the input=audio â†’ output=transcript flow.
...
class TestGetNamedSTTProviderConfig:
    ...
    def test_builtin_name_is_not_legacy_resolved(self):
        # stt.openai has model/language but no command â€” must NOT be
...
class TestSTTCommandHelpers:
    ...
    def test_iter_command_providers_excludes_builtins(self):
        # Defense in depth â€” a user trying to register a built-in name as
...
class TestRenderCommandSTTTemplate:
    ...
    def test_preserves_doubled_braces(self):
        ...
        # Doubled braces collapse to single braces â€” JSON snippets survive.
    ...
    def test_placeholder_not_in_dict_passes_through(self):
        # Unknown placeholder isn't replaced â€” preserves literal text.
        rendered = _render_command_stt_template(
            "echo {unknown_name}",
            {"input_path": "x"},
        )
...
class TestTranscribeCommandSTT:
    ...
    def test_language_from_provider_config_wins(self, tmp_path):
        ...
        # stt.language is "es" but provider config says "fr" â€” provider wins.
        result = _transcribe_command_stt(
...
class TestTranscribeAudioDispatchToCommandProvider:
    ...
    ZED_LOCAL_STT_COMMAND) by patching ``_load_stt_config`` directly.
    """
    ...
    def test_builtin_name_shadow_does_not_route_to_command(self, tmp_path):
        # User mis-configures stt.providers.openai as a command â€” must NOT
        ...
        with patch("tools.transcription_tools._load_stt_config", return_value=cfg):
            # openai dispatch will likely fail with no API key â€” that's fine,
...
class TestCommandWinsOverPlugin:
    ...
    command provider must win â€” same precedence rule as TTS PR #17843
