...
class TestPrependShellInit:
    ...
    def test_prepends_guarded_source_lines(self):
        ...
        # Each file is sourced through a guarded [ -r â€¦ ] && . 'â€¦' || true
...
class TestSnapshotEndToEnd:
    ...
    def test_exported_env_changes_persist_between_commands(self, tmp_path):
        ...
        try:
            first = env.execute(
                'export ZED_SESSION_ENV_PROBE="sticky"; '
                'export PATH="/tmp/zed-session-bin:$PATH"; '
                'echo "first=$ZED_SESSION_ENV_PROBE"'
            )
            second = env.execute(
                'echo "second=$ZED_SESSION_ENV_PROBE"; echo "PATH=$PATH"'
            )
        finally:
            env.cleanup()
        ...
        assert "/tmp/zed-session-bin" in output
    ...
    def test_snapshot_picks_up_init_file_exports(self, tmp_path, monkeypatch):
        ...
        init_file.write_text(
            'export ZED_SHELL_INIT_PROBE="probe-ok"\n'
        ...
        with patch(
            ...
            try:
                result = env.execute(
                    'echo "PROBE=$ZED_SHELL_INIT_PROBE"; echo "PATH=$PATH"'
                )
            finally:
                env.cleanup()
    ...
    def test_profile_path_export_survives_bashrc_interactive_guard(
        ...
        Setup:
          - ``~/.bashrc`` starts with ``case $- in *i*) ;; *) return;; esac``
            ...
            entry below that guard â€” but never gets there because a
        ...
        # bashrc short-circuited on the interactive guard â€” its export never ran
