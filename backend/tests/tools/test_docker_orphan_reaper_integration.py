...
Issue #20561 â€” without these gates, parallel subagents would each fire the
...
def test_maybe_reap_runs_once_per_process(monkeypatch):
    ...
    daemon and waste 5â€“10s of startup."""
    _reset_reaper_gate()
    call_count = {"reap": 0}
...
def test_maybe_reap_respects_disable_flag(monkeypatch):
    ...
    skip the sweep entirely â€” no docker ps, no inspect, no rm. The escape
    hatch for operators running multiple Zed processes in the same
    profile."""
    _reset_reaper_gate()
    call_count = {"reap": 0}
    ...
    # disabled â€” that would prevent a subsequent toggle to true from working.
    assert terminal_tool._docker_orphan_reaper_ran is False


def test_maybe_reap_doubles_lifetime_for_max_age(monkeypatch):
    """The reaper's age threshold is ``2 Ã— lifetime_seconds`` (with a 60s
    floor). Generous default â€” gives sibling Zed processes ample grace
    ...
    assert captured_args.get("max_age_seconds") == 600, (
        f"expected 2 Ã— 300 = 600, got {captured_args.get('max_age_seconds')}"
    )
...
def test_maybe_reap_floors_at_60_seconds(monkeypatch):
    ...
    user's own just-started container creation. Floor at 60s Ã— 2 = 120s."""
    _reset_reaper_gate()
    captured_args = {}
    ...
    assert captured_args.get("max_age_seconds") == 120, (
        f"expected floored 60 Ã— 2 = 120, got {captured_args.get('max_age_seconds')}"
    )


def test_maybe_reap_passes_current_profile_as_filter(monkeypatch):
    """The reaper must be scoped to the current Zed profile â€” a research
