...
``~/.zed/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.zed/.env`` would be told
    ...
    def test_import_after_config_env_patch_uses_restored_dotenv_loader(self):
        """Importing STT while zed_cli.config.get_env_value is patched must
    ...
    def test_auto_detect_sees_dotenv_groq(self):
        """No local backend, no explicit provider â€” auto-detect should fall
        ...
        with patch.object(tt, "_HAS_FASTER_WHISPER", False), \
             ...
             patch("zed_cli.config.load_env",
                   return_value={"GROQ_API_KEY": "dotenv-secret"}):
            # No "provider" key â†’ explicit=False â†’ auto-detect branch
...
class TestEndToEndRegressionGuard:
    """End-to-end probe: patch ``zed_cli.config.load_env`` to simulate
    ``~/.zed/.env`` carrying the key while ``os.environ`` does not.
    ...
    def test_xai_key_only_in_dotenv_before_fix(self, monkeypatch):
        ...
        with patch("zed_cli.config.load_env",
                   return_value={"XAI_API_KEY": "dotenv-secret"}):
