...
def zed_home(monkeypatch, tmp_path):
    """Isolate ZED_HOME and clear module-level caches afterward so the
...
class TestPatchFailureEscalation:
    def test_first_two_failures_use_normal_hint(self, zed_home, tmp_path, fresh_tracker):
    ...
    def test_third_consecutive_failure_escalates(self, zed_home, tmp_path, fresh_tracker):
        from tools.file_tools import _handle_patch
    ...
    def test_success_clears_failure_counter(self, zed_home, tmp_path, fresh_tracker):
        from tools.file_tools import _handle_patch
        ...
        # Next failure should be back to "attempt 1" â€” generic hint only.
        result = _handle_patch(
            {
                "mode": "replace",
                "path": str(target),
    ...
    def test_different_paths_have_independent_counters(
        self, zed_home, tmp_path, fresh_tracker
    ):
        from tools.file_tools import _handle_patch
        ...
        # One failure on b.py â€” should NOT inherit a.py's count.
        result = _handle_patch(
            {
                "mode": "replace",
                "path": str(b),
    ...
    def test_different_tasks_have_independent_counters(
        ...
        # First failure under task B â€” should NOT see escalation.
        result = _handle_patch(
            {
                "mode": "replace",
                "path": str(target),
