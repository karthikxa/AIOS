...
class TestExitCodeMapping:
    ...
    def test_exit_1_block_with_findings(self, mock_cfg, mock_run):
        ...
        result = check_command_security("curl http://gÐ¾Ð¾gle.com")
...
class TestPathExpansion:
    def test_tilde_expanded_in_resolve(self):
        ...
        _tirith_mod._resolved_path = None
        # Explicit path â€” won't auto-download, just expands and caches miss
...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._zed_bin_dir", return_value="/nonexistent"), \
...
# ---------------------------------------------------------------------------
# Unsupported platform (Windows etc.) â€” silent fast-path everywhere
# ---------------------------------------------------------------------------
...
class TestUnsupportedPlatform:
    ...
    def test_ensure_installed_unsupported_returns_none_no_thread(self, mock_cfg):
        ...
        failure marker â€” just cache the verdict and return None."""
        mock_cfg.return_value = {"tirith_enabled": True, "tirith_path": "tirith",
                                 "tirith_timeout": 5, "tirith_fail_open": True}
        _tirith_mod._resolved_path = None
    ...
    def test_check_command_security_unsupported_allows_silently(self, mock_cfg):
        """Windows: skip the resolver and spawn entirely â€” return allow with
    ...
    def test_explicit_path_still_honored_on_unsupported_platform(self, mock_cfg):
        ...
        NOT override that â€” explicit config wins."""
...
class TestFailedDownloadCaching:
    ...
    def test_failed_install_scan_uses_fail_open(self, mock_cfg, mock_run,
                                                 ...
                                                 mock_disk_check, mock_mark):
        """After cached miss, check_command_security hits OSError â†’ fail_open."""
        _tirith_mod._resolved_path = None
        mock_cfg.return_value = {"tirith_enabled": True, "tirith_path": "tirith",
                                 "tirith_timeout": 5, "tirith_fail_open": True}
        mock_run.side_effect = FileNotFoundError("No such file: tirith")
        ...
        # Second command: no install retry, just hits OSError â†’ allow
...
class TestCosignVerification:
    ...
    def test_cosign_pass(self, mock_which, mock_run):
        """cosign verify-blob exits 0 â†’ returns True."""
    ...
    def test_cosign_fail_aborts(self, mock_which, mock_run):
        """cosign verify-blob exits non-zero â†’ returns False (abort install)."""
        from tools.tirith_security import _verify_cosign
    ...
    def test_cosign_not_found_returns_none(self, mock_which):
        """cosign not on PATH â†’ returns None (proceed with SHA-256 only)."""
        from tools.tirith_security import _verify_cosign
        result = _verify_cosign("/tmp/checksums.txt", "/tmp/checksums.txt.sig",
                                "/tmp/checksums.txt.pem")
        assert result is None
    ...
    def test_cosign_timeout_returns_none(self, mock_which, mock_run):
        """cosign times out â†’ returns None (proceed with SHA-256 only)."""
        from tools.tirith_security import _verify_cosign
        result = _verify_cosign("/tmp/checksums.txt", "/tmp/checksums.txt.sig",
                                "/tmp/checksums.txt.pem")
        assert result is None
    ...
    def test_cosign_os_error_returns_none(self, mock_which, mock_run):
        """cosign OSError â†’ returns None (proceed with SHA-256 only)."""
        from tools.tirith_security import _verify_cosign
        result = _verify_cosign("/tmp/checksums.txt", "/tmp/checksums.txt.sig",
                                "/tmp/checksums.txt.pem")
        assert result is None
    ...
    def test_install_proceeds_when_cosign_passes(self, mock_target, mock_dl,
        ...
        from tools.tirith_security import _install_tirith
        # Mock tarfile â€” empty archive means "binary not found" return
        mock_tar = MagicMock()
        mock_tar.__enter__ = MagicMock(return_value=mock_tar)
        mock_tar.__exit__ = MagicMock(return_value=False)
        mock_tar.getmembers.return_value = []
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(zed_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(zed_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_zed_bin(self):
        """After _INSTALL_FAILED, manual install in ZED_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        zed_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(zed_bin, "w") as f:
            f.write("#!/bin/sh\n")
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._zed_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == zed_bin
            assert _tirith_mod._resolved_path == zed_bin
            mock_clear.assert_called_once()
    ...
    def test_cosign_missing_disk_marker_allows_retry(self):
        ...
        # _is_install_failed_on_disk sees "cosign_missing" + cosign on PATH â†’ returns False
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._zed_bin_dir", return_value="/nonexistent"), \
             patch("tools.tirith_security._is_install_failed_on_disk", return_value=False), \
    ...
    def test_disk_marker_reason_preserved_in_memory(self):
        ...
        # Second call: cosign now on PATH â†’ in-memory retry fires
        def _which_side_effect(name):
            if name == "tirith":
                return None
            if name == "cosign":
...
class TestZedHomeIsolation:
    def test_zed_bin_dir_respects_zed_home(self):
        """_zed_bin_dir must use ZED_HOME, not hardcoded ~/.zed."""
        from tools.tirith_security import _zed_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ZED_HOME": tmpdir}):
            result = _zed_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_zed_home(self):
        """_failure_marker_path must use ZED_HOME, not hardcoded ~/.zed."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"ZED_HOME": "/custom/zed"}):
            result = _failure_marker_path()
        assert result == "/custom/zed/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ZED_HOME; verify it's active."""
        zed_home = os.getenv("ZED_HOME")
        assert zed_home is not None, "ZED_HOME should be set by conftest"
        assert "zed_test" in zed_home, "Should point to test temp dir"

    def test_get_zed_home_fallback(self):
        """Without ZED_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_zed_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove ZED_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".zed")
            result = _get_zed_home()
...
class TestSpawnWarningDedup:
    ...
    def test_repeated_spawn_failure_logs_once(self, mock_cfg, mock_run, caplog):
        ...
        # Fresh dedupe state â€” clear any keys left by other tests.
        _tirith_mod._reset_spawn_warning_state()

        with caplog.at_level("WARNING", logger="tools.tirith_security"):
            for _ in range(15):
                result = check_command_security("echo hi")
                # Behavior must remain the same on every call â€”
    ...
    def test_path_none_logs_once(self, mock_cfg, caplog):
        ...
        but resolver returned None â€” unusual) should not spam the log
        either."""
        mock_cfg.return_value = {
            "tirith_enabled": True, "tirith_path": "tirith",
            "tirith_timeout": 5, "tirith_fail_open": True,
...
class TestAppTldSuppression:
    ...
    def test_multiple_app_tld_findings_all_suppressed(self, mock_cfg, mock_run):
        """All findings being .app lookalike_tld â†’ allow."""
        mock_cfg.return_value = _CFG
        findings = [
            {"rule_id": "lookalike_tld", "value": ".app"},
