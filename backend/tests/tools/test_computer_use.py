...
def _reset_backend():
    ...
    with patch.dict(os.environ, {"ZED_COMPUTER_USE_BACKEND": "noop"}, clear=False):
        yield
    reset_backend_for_tests()
...
class TestSchema:
    ...
    def test_schema_does_not_use_anthropic_native_types(self):
        """Generic OpenAI schema â€” no `type: computer_20251124`."""
        from tools.computer_use.schema import COMPUTER_USE_SCHEMA
    ...
    def test_schema_max_elements_documents_default_and_upper_bound(self):
        ...
        had no upper bound â€” both Copilot findings.
        """
        from tools.computer_use.schema import COMPUTER_USE_SCHEMA
...
class TestDispatch:
    ...
    def test_set_value_routes_to_backend(self, noop_backend):
        """set_value must reach the backend â€” regression for missing _NoopBackend stub."""
        from tools.computer_use.tool import handle_computer_use
...
class TestCaptureResponse:
    ...
    def test_capture_multimodal_summary_omits_truncation_note(self):
        ...
        `elements` array â€” so a "response truncated to N of M elements"
...
class TestCompressorScreenshotPruning:
    def _make_compressor(self):
        ...
        # Minimal constructor â€” _prune_old_tool_results doesn't need a real client.
...
class TestImageAwareTokenEstimator:
    def test_image_block_counts_as_flat_1500_tokens(self):
        ...
        assert tokens < 5000, f"image-aware counter returned {tokens} tokens â€” too high"
...
class TestUniversality:
    ...
    def test_no_provider_gating_in_tool_registration(self):
        """Anthropic-only gating was a #4562 artefact â€” must not recur."""
        import tools.computer_use_tool  # noqa: F401
        from tools.registry import registry
        entry = registry._tools["computer_use"]
...
class TestElementLabelParsing:
    ...
    cua-driver â‰¥0.1.6 emits ``[N] AXRole (order) id=Label`` instead of
...
class TestCaptureAfterAppContext:
    ...
    def test_capture_after_without_prior_app_uses_none(self):
        ...
        # No app context â€” should pass None so cua-driver picks the frontmost window
        assert len(captured_app_args) == 1
...
class TestCaptureAppFilterNoMatch:
    ...
    when X matches nothing â€” on a non-English macOS, list_windows returns
    localized app names (e.g. "è¨ˆç®—æ©Ÿ"), so an English `app="Calculator"`
    ...
    def test_app_filter_no_match_returns_empty_capture_with_diagnostic(self):
        # Simulates a localized macOS where Calculator's app_name is "è¨ˆç®—æ©Ÿ".
        windows = [
            {"app_name": "Fuwari", "pid": 100, "window_id": 1,
             "is_on_screen": True, "title": "menu bar", "z_index": 0},
            {"app_name": "è¨ˆç®—æ©Ÿ", "pid": 200, "window_id": 2,
    ...
    def test_app_filter_match_still_works(self):
        ...
        backend._session.call_tool.side_effect = [
            {"data": "", "images": [], "isError": False,
             "structuredContent": {"windows": windows}},
            {"data": 'âœ… è¨ˆç®—æ©Ÿ â€” 0 elements\n', "images": [], "isError": False,
             "structuredContent": None},
        ]

        cap = backend.capture(mode="ax", app="è¨ˆç®—æ©Ÿ")
    ...
    def test_no_app_filter_still_picks_frontmost(self):
        ...
        window â€” the no-match early-return must not fire on the empty case."""
        windows = [
            {"app_name": "Fuwari", "pid": 100, "window_id": 1,
             "is_on_screen": True, "title": "menu bar", "z_index": 0},
        ]
...
class TestFocusAppFilterNoMatch:
    """focus_app(app=X) must return ok=False when X matches nothing â€”
    ...
    def test_focus_app_match_still_works(self):
        ...
        res = backend.focus_app("è¨ˆç®—æ©Ÿ")
...
class TestCuaEnvironmentScrubbing:
    ...
    def test_cua_session_sanitizes_provider_env_vars(self):
        ...
        The cua-driver MCP subprocess should not inherit Zed-managed credentials
        or other sensitive environment variables â€” only runtime-required vars.
