...
``~/.zed/logs/subagent-timeout-<sid>-<ts>.log``. This gives users a
...
# â”€â”€ _dump_subagent_timeout_diagnostic â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

class TestDumpSubagentTimeoutDiagnostic:

    def test_writes_log_with_expected_sections(self, zed_home):
        ...
        p = Path(path)
        assert p.is_file()
        # File lives under ZED_HOME/logs/
        assert p.parent == zed_home / "logs"
        ...
        # The thread is parked inside _hang.wait â†’ cond.wait â†’ waiter.acquire
        assert "acquire" in content or "wait" in content

    def test_truncates_very_long_goal(self, zed_home):
        from tools.delegate_tool import _dump_subagent_timeout_diagnostic
        child = _StubChild()
        huge_goal = "x" * 5000
    ...
    def test_missing_worker_thread_is_handled(self, zed_home):
        from tools.delegate_tool import _dump_subagent_timeout_diagnostic
        child = _StubChild()
        path = _dump_subagent_timeout_diagnostic(
            child=child,
    ...
    def test_exited_worker_thread_is_handled(self, zed_home):
        from tools.delegate_tool import _dump_subagent_timeout_diagnostic
        child = _StubChild()
    ...
    def test_returns_none_on_unwritable_logs_dir(self, tmp_path, monkeypatch):
        # Point ZED_HOME at an unwritable path so logs/ can't be created
        # (simulates permission-denied). Helper must not raise.
        from tools.delegate_tool import _dump_subagent_timeout_diagnostic
        bogus = tmp_path / "does-not-exist" / ".zed"
        monkeypatch.setenv("ZED_HOME", str(bogus))
        child = _StubChild()

        # Make the logs dir itself unwritable by creating it as a FILE
        # so mkdir(exist_ok=True) â†’ NotADirectoryError and we fall through.
        ...
        # We assert no exception propagates â€” the return value is advisory.
        assert result is None or Path(result).exists()


# â”€â”€ _run_single_child timeout branch wiring â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestRunSingleChildTimeoutDump:
    ...
    def test_zero_api_calls_writes_dump_and_surfaces_path(self, zed_home, monkeypatch):
        ...
        assert dump_path.parent == zed_home / "logs"
    ...
    def test_nonzero_api_calls_skips_dump_and_uses_old_message(self, zed_home, monkeypatch):
        ...
        # actual API calls â€” the old generic "stuck on slow call" message
        # still applies.
