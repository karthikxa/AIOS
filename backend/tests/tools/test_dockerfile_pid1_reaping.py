...
These tests assert invariants about how the Dockerfile composes its runtime â€”
...
- Signal forwarding runs through the init so ``docker stop`` triggers
  zed's own graceful-shutdown path.
...
checks below accept either family â€” the contract is behavioural, not
nominal.
"""
...
# satisfies the zombie-reaping invariant â€” see issue #15012.
...
def _instruction_text(dockerfile_text: str) -> str:
    ...
    string. Crucially excludes comments â€” otherwise the historical
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, zed accumulates zombie
    ...
    assert installed, (
        ...
        "reap orphaned subprocesses, zed accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    Installing the init is only half the fix â€” the container must actually
    run with it as PID 1.  If the ENTRYPOINT executes the shell script
    directly, the shell becomes PID 1 and will ``exec`` into zed,
    which then runs as PID 1 without any zombie reaping.
    """
    # Find the last uncommented ENTRYPOINT line â€” Docker honours the final one.
    ...
    assert routes_through_init, (
        ...
        "not wired into ENTRYPOINT, zed still runs as PID 1 and zombies "
...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    # resolve dependencies. The bundled ``zed-ink`` workspace package is
    ...
    # ``ui-tui/package.json`` â€” copying the tree avoids npm stopping at a
    ...
    assert "ui-tui/packages/zed-ink/" in dockerfile_text
...
def test_dockerfile_preinstalls_hindsight_memory_dependency(dockerfile_text):
    ...
    assert any("--extra hindsight" in step for step in sync_steps), (
        ...
        "into /opt/zed/.venv. It lazy-installs into the image layer (not the "
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    # ``zed-ink`` is a bundled workspace package referenced from
    # ``ui-tui/package.json`` via ``file:`` â€” not pulled from the npm
    # registry. The contract this test pins is just that the image
    # actually carries the package source so ``await import('@zed/ink')``
    # can resolve at runtime; the previous, much pickier assertion (manual
    # ``rm -rf`` + ``npm install --omit=dev --prefix node_modules/@zed/ink``)
    ...
    assert "ui-tui/packages/zed-ink/" in dockerfile_text, (
        "Dockerfile must COPY the bundled zed-ink workspace package "
        "so ``await import('@zed/ink')`` resolves at runtime."
    )
