...
closed the pipe â€” causing the terminal tool to hang for the full lifetime
...
class TestBackgroundChildDoesNotHang:
    ...
    def test_plain_background_returns_promptly(self, local_env):
        ...
        try:
            ...
            assert elapsed < 4.0, (
                f"terminal_tool hung for {elapsed:.1f}s â€” drain thread "
    ...
    def test_foreground_streaming_output_still_captured(self, local_env):
        ...
        # Loop body sleeps ~0.6s total â€” elapsed should be close to that.
    ...
    def test_utf8_output_decoded_correctly(self, local_env):
        ...
        result = local_env.execute("echo æ—¥æœ¬èªž cafÃ© rÃ©sumÃ©", timeout=5)
        assert result["returncode"] == 0
        assert "æ—¥æœ¬èªž" in result["output"]
        assert "cafÃ©" in result["output"]
        assert "rÃ©sumÃ©" in result["output"]
    ...
    def test_utf8_multibyte_across_read_boundary(self, local_env):
        ...
        must be decoded correctly via the incremental decoder â€” not lost to a
        ...
        """
        # 10000 "æ—¥" chars = 30000 bytes â€” guaranteed to cross multiple 4096
