...
class TestModalBulkUpload:
    ...
    def test_tar_archive_contains_all_files(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src_a), "/root/.zed/credentials/a.json"),
            (str(src_b), "/root/.zed/skills/b.py"),
        ]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            ...
            assert "root/.zed/credentials/a.json" in names
            assert "root/.zed/skills/b.py" in names

            # Verify content
            a_content = tar.extractfile("root/.zed/credentials/a.json").read()
            assert a_content == b"cred_content"
            b_content = tar.extractfile("root/.zed/skills/b.py").read()
    ...
    def test_mkdir_includes_all_parents(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src), "/root/.zed/credentials/f.txt"),
            (str(src), "/root/.zed/skills/deep/nested/f.txt"),
        ]
        ...
        cmd = exec_calls[0][2]
        assert "/root/.zed/credentials" in cmd
        assert "/root/.zed/skills/deep/nested" in cmd
    ...
    def test_single_exec_call(self, monkeypatch, tmp_path):
        ...
        for i in range(20):
            ...
            files.append((str(src), f"/root/.zed/cache/file_{i}.txt"))
    ...
    def test_stdin_chunked_for_large_payloads(self, monkeypatch, tmp_path):
        ...
        files = [(str(src), "/root/.zed/large.bin")]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            names = tar.getnames()
            assert "root/.zed/large.bin" in names
