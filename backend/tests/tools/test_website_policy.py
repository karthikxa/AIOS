...
def test_check_website_access_uses_dynamic_zed_home(monkeypatch, tmp_path):
    ...
    # Invalidate the module-level cache so the new ZED_HOME is picked up.
    # A prior test may have cached a default policy (enabled=False) under the
    # old ZED_HOME set by the autouse _isolate_zed_home fixture.
...
def test_browser_navigate_allows_when_shared_file_missing(monkeypatch, tmp_path):
    ...
    # check_website_access should return None (allow) â€” missing file is skipped
...
def test_check_website_access_fails_open_on_malformed_config(tmp_path, monkeypatch):
    ...
    # Simulate default path by pointing ZED_HOME to tmp_path
