"""Tests for tools/process_registry.py â€” ProcessRegistry query methods, pruning, checkpoint."""
...
from tools.environments.local import _ZED_PROVIDER_ENV_FORCE_PREFIX
...
class TestOrphanedPipeReconciliation:
    ...
    `zed update` in Feishu spawned a background subprocess that restarted
    ...
    def test_reconcile_flips_exited_when_direct_child_done(self, registry):
        ...
        # Approach: spawn `sh -c 'sleep 10 &'` with setsid â€” sh forks the
        ...
        assert result["status"] == "exited", (
            ...
            "This is issue #17327 â€” reader is blocked on orphaned pipe."
        )
        assert result["exit_code"] == 0
    ...
    def test_wait_returns_when_reader_blocked(self, registry):
        """wait() must also reconcile â€” not just poll()."""
        proc = subprocess.Popen(
...
class TestStdinHelpers:
    ...
    def test_close_stdin_allows_eof_driven_process_to_finish(self, registry, tmp_path):
        ...
        lockout (#17959). For interactive stdin â†’ PTY mode is now the only
        supported path.
        """
...
class TestSpawnEnvSanitization:
    def test_spawn_local_strips_blocked_vars_from_background_env(self, registry):
        ...
        with patch.dict(os.environ, {
            ...
            registry.spawn_local(
                ...
                env_vars={
                    ...
                    f"{_ZED_PROVIDER_ENV_FORCE_PREFIX}TELEGRAM_BOT_TOKEN": "forced-bot-token",
                },
            )
        ...
        assert f"{_ZED_PROVIDER_ENV_FORCE_PREFIX}TELEGRAM_BOT_TOKEN" not in env
    ...
    def test_spawn_via_env_uses_backend_temp_dir_for_artifacts(self, registry):
        ...
        assert "/data/data/com.termux/files/usr/tmp/zed_bg_" in bg_command
        assert ".exit" in bg_command
        assert "rc=$?;" in bg_command
        assert " > /tmp/zed_bg_" not in bg_command
        assert "cat /tmp/zed_bg_" not in bg_command
    ...
    def test_env_poller_quotes_temp_paths_with_spaces(self, registry):
        ...
        with patch("tools.process_registry.time.sleep", return_value=None), \
            ...
            registry._env_poller_loop(
                session,
                env,
                "/path with spaces/zed_bg.log",
                "/path with spaces/zed_bg.pid",
                "/path with spaces/zed_bg.exit",
            )

        assert env.commands[0][0] == "cat '/path with spaces/zed_bg.log' 2>/dev/null"
        assert env.commands[1][0] == "kill -0 \"$(cat '/path with spaces/zed_bg.pid' 2>/dev/null)\" 2>/dev/null; echo $?"
        assert env.commands[2][0] == "cat '/path with spaces/zed_bg.exit' 2>/dev/null"
...
class TestPopenLeakOnSetupFailure:
    ...
    def test_popen_killed_when_thread_creation_fails(self, registry):
        """If Thread() raises after Popen, proc must be killed â€” not orphaned."""
        killed = []
        ...
        # real process group and proc.kill() is never reached â€” flaky failure,
...
class TestCheckpoint:
    ...
    def test_recover_enqueues_watchers(self, registry, tmp_path):
        ...
        checkpoint.write_text(json.dumps([{
            ...
            "command": "sleep 999",
            "pid": os.getpid(),  # current process â€” guaranteed alive
            "task_id": "t1",
            "session_key": "sk1",
...
class TestKillProcess:
    ...
    def test_kill_detached_session_uses_host_pid(self, registry):
        ...
        try:
            ...
            # ``ProcessRegistry._is_host_pid_alive`` (â†’
...
# ---------------------------------------------------------------------------
# _terminate_host_pid â€” cross-platform process-tree termination
# ---------------------------------------------------------------------------


class TestTerminateHostPidWindows:
    """Windows branch uses ``taskkill /T /F`` â€” the documented MS tree-kill
    ...
    def test_windows_does_not_call_psutil(self, monkeypatch):
        ...
        (it's unreliable on Windows â€” see the function docstring)."""
        from tools import process_registry as pr
        import psutil
