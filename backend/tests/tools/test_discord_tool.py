...
# ---------------------------------------------------------------------------
# Toolset: discord / discord_admin only in zed-discord
# ---------------------------------------------------------------------------

class TestToolsetInclusion:
    def test_discord_tools_in_zed_discord_toolset(self):
        from toolsets import TOOLSETS
        assert "discord" in TOOLSETS["zed-discord"]["tools"]
        assert "discord_admin" in TOOLSETS["zed-discord"]["tools"]

    def test_discord_tools_not_in_core_tools(self):
        from toolsets import _ZED_CORE_TOOLS
        assert "discord" not in _ZED_CORE_TOOLS
        assert "discord_admin" not in _ZED_CORE_TOOLS

    def test_discord_tools_not_in_other_toolsets(self):
        from toolsets import TOOLSETS
        for name, ts in TOOLSETS.items():
            if name in {"zed-discord", "zed-gateway", "discord", "discord_admin"}:
                continue
            tools = ts.get("tools", [])
...
class TestCapabilityDetection:
    ...
    def test_cache_is_keyed_by_token(self, mock_req):
        ...
        call populated it and every subsequent call â€” regardless of token â€”
...
class TestConfigAllowlist:
    ...
    def test_empty_string_returns_none(self, monkeypatch):
        """Empty config means no allowlist â€” all actions visible."""
        monkeypatch.setattr(
    ...
    def test_config_load_failure_is_permissive(self, monkeypatch):
        ...
        monkeypatch.setattr("zed_cli.config.load_config", bad_load)
        assert _load_allowed_actions_config() is None
...
class TestAvailableActions:
    ...
    def test_no_members_intent_hides_member_actions(self):
        ...
        # fetch_messages stays â€” MESSAGE_CONTENT affects content field but action works
    ...
    def test_allowlist_intersects_with_intents(self):
        """Allowlist can only narrow â€” not re-enable intent-gated actions."""
        caps = {"detected": True, "has_members_intent": False, "has_message_content": True}
        allowlist = ["list_guilds", "search_members", "fetch_messages"]
        actions = _available_actions(caps, allowlist)
        # search_members gated by intent â†’ stripped even though allowlisted
...
# ---------------------------------------------------------------------------
# Runtime allowlist enforcement (defense in depth â€” schema already filtered)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# model_tools integration â€” dynamic schema replaces static
# ---------------------------------------------------------------------------
...
class TestModelToolsIntegration:
    ...
    def test_discord_admin_schema_rebuilt_by_get_tool_definitions(
        ...
        tools = get_tool_definitions(enabled_toolsets=["zed-discord"], quiet_mode=True)
