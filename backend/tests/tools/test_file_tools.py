...
class TestWriteFileHandler:
    ...
    def test_missing_content_key_returns_error(self):
        """#19096 â€” handler must reject tool calls where 'content' key is absent."""
    ...
    def test_missing_path_key_returns_error(self):
        """#19096 â€” handler must reject tool calls where 'path' key is absent."""
        from tools.file_tools import _handle_write_file
    ...
    def test_explicit_empty_content_is_allowed(self):
        """#19096 â€” explicit empty string content (file truncation) must still work."""
        from tools.file_tools import _handle_write_file
    ...
    def test_non_string_content_returns_error(self):
        """#19096 â€” content must be a string, not a dict or list."""
        from tools.file_tools import _handle_write_file
...
class TestSensitivePathCheck:
    ...
    def test_zed_config_blocked_for_write_file(self, tmp_path, monkeypatch):
        fake_config = tmp_path / "config.yaml"
        monkeypatch.setattr("tools.file_tools._zed_config_resolved", str(fake_config))
        monkeypatch.setattr("tools.file_tools._zed_config_resolved_loaded", True)

        from tools.file_tools import write_file_tool
        result = json.loads(write_file_tool(str(fake_config), "approvals:\n  mode: off\n"))
        assert "error" in result
        assert "Zed config" in result["error"]

    def test_zed_config_blocked_via_tilde_path(self, tmp_path, monkeypatch):
        fake_config = tmp_path / "config.yaml"
        monkeypatch.setattr("tools.file_tools._zed_config_resolved", str(fake_config))
        monkeypatch.setattr("tools.file_tools._zed_config_resolved_loaded", True)
    ...
    def test_zed_config_blocked_for_patch(self, tmp_path, monkeypatch):
        fake_config = tmp_path / "config.yaml"
    ...
    def test_system_path_still_blocked(self, monkeypatch):
        monkeypatch.setattr("tools.file_tools._zed_config_resolved", "/some/other/path")
        monkeypatch.setattr("tools.file_tools._zed_config_resolved_loaded", True)
    ...
    def test_normal_file_not_blocked(self, mock_get, monkeypatch):
        monkeypatch.setattr("tools.file_tools._zed_config_resolved", "/home/user/.zed/config.yaml")
        monkeypatch.setattr("tools.file_tools._zed_config_resolved_loaded", True)
        mock_ops = MagicMock()
        result_obj = MagicMock()
...
class TestPatchSchemaShape:
    ...
    def test_no_anyof_required_stays_mode_only(self):
        ...
        # Moonshot/Kimi schema sanitizer â€” description-level guidance is the
