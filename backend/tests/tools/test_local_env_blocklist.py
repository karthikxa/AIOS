...
Verifies that Zed-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed Zed secrets.

See: https://github.com/NousResearch/zed-agent/issues/1002
See: https://github.com/NousResearch/zed-agent/issues/1264
"""
...
from tools.environments.local import (
    LocalEnvironment,
    _ZED_PROVIDER_ENV_BLOCKLIST,
    _ZED_PROVIDER_ENV_FORCE_PREFIX,
)
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.zed/.env must not leak."""
    ...
    def test_registry_derived_vars_are_stripped(self):
        ...
        must also be blocked â€” not just the hand-written extras."""
        registry_vars = {
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a Zed inference secret
        ...
        models`` run inside a Zed terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        subprocesses â€” this is the no-regression guard for #32314.

        Per SECURITY.md Â§3.2 the local terminal is the user's trusted operator
        ...
        agent terminal â€” not just Bedrock users, since the registry is iterated
        unconditionally â€” and (b) be unrecoverable, because env_passthrough.py
        refuses to re-allow anything in _ZED_PROVIDER_ENV_BLOCKLIST
        (GHSA-rhgp-j443-p4rf). Only the Bedrock inference bearer token is
        Zed-managed; the rest belongs to the user.
        """
        general_chain = {
        ...
        for var, value in general_chain.items():
            assert result_env.get(var) == value, (
                f"{var} was stripped from subprocess env â€” this is a "
    ...
    def test_tool_and_gateway_vars_are_stripped(self):
        ...
        leaked_vars = {
            ...
            "ZED_DASHBOARD_SESSION_TOKEN": "dashboard-session-secret",
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _ZED_FORCE_ prefix."""

    def test_force_prefix_passes_blocked_var(self):
        """_ZED_FORCE_OPENAI_API_KEY in self.env should inject OPENAI_API_KEY."""
        result_env = _run_with_env(self_env={
            f"{_ZED_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-explicit",
        })
        ...
        assert f"{_ZED_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result_env
    ...
    def test_force_prefix_overrides_os_environ_block(self):
        ...
        result_env = _run_with_env(
            ...
            self_env={f"{_ZED_PROVIDER_ENV_FORCE_PREFIX}OPENAI_BASE_URL": "http://intended/v1"},
        )
...
class TestBlocklistCoverage:
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their Zed-managed
        ...
        assert "AWS_BEARER_TOKEN_BEDROCK" in _ZED_PROVIDER_ENV_BLOCKLIST

    def test_general_aws_chain_not_in_blocklist(self):
        """The general AWS credential chain must NOT be in the blocklist â€”
        no-regression guard for #32314. These belong to the user's trusted
        operator shell (SECURITY.md Â§3.2), not to Zed, and blocklisting
        ...
        leaked_block = general_chain & _ZED_PROVIDER_ENV_BLOCKLIST
    ...
    def test_extra_auth_vars_covered(self):
        ...
        assert extras.issubset(_ZED_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_optional_tool_and_messaging_vars_are_in_blocklist(self):
        ...
        for name, metadata in OPTIONAL_ENV_VARS.items():
            ...
            if category in {"tool", "messaging"}:
                assert name in _ZED_PROVIDER_ENV_BLOCKLIST, (
