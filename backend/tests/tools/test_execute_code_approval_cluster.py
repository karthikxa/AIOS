...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  1. tools.thread_context.propagate_context_to_thread â€” propagates the agent
  ...
  3. tools.approval.check_execute_code_guard â€” the entry-point guard decision
     matrix (isolated backends, yolo/off, cron-deny, headless-local,
     gateway approve/deny/timeout/missing-notify, smart mode).
  4. tools.code_execution_tool._scrub_child_env â€” broad ZED_ prefix dropped,
...
def test_helper_clears_callbacks_on_teardown():
    ...
    try:
        ...
        def second():  # NOT wrapped â€” runs on the same recycled worker thread
...
def test_both_rpc_threads_use_propagation_helper():
    ...
    assert "propagate_context_to_thread(_rpc_server_loop)" in src, (
        ...
        "propagate_context_to_thread â€” gateway approval routing will be lost."
    )
...
def gw_session(monkeypatch):
    """A clean gateway session: ZED_GATEWAY_SESSION set, a bound session
    key, and isolated gateway queues/callbacks. Yields the session_key."""
    monkeypatch.setenv("ZED_GATEWAY_SESSION", "1")
    monkeypatch.delenv("ZED_INTERACTIVE", raising=False)
    monkeypatch.delenv("ZED_CRON_SESSION", raising=False)
...
def test_guard_isolated_backend_approved():
    # Container backends already sandbox the child â€” no-op approve.
    assert A.check_execute_code_guard("import os", "docker")["approved"] is True


def test_guard_headless_local_approved(monkeypatch):
    # Documented #30882 limitation: no approval surface â†’ preserve auto-run.
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("ZED_CRON_SESSION", "1")
    monkeypatch.delenv("ZED_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_guard_gateway_missing_notify_is_pending(gw_session):
    # No notify callback registered â†’ backward-compat pending approval.
    res = A.check_execute_code_guard("import os", "local")
    assert res["approved"] is False
...
def test_guard_smart_mode(gw_session, monkeypatch):
    ...
    # escalate â†’ falls through to manual gateway approval
...
def test_env_scrub_zed_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist â†’ kept
        "ZED_HOME": "/h", "ZED_PROFILE": "p",
        "ZED_CONFIG": "/c.yaml", "ZED_ENV": "/e",
        # other ZED_* â†’ dropped (broad prefix removed)
        "ZED_BASE_URL": "https://x", "ZED_INTERACTIVE": "1",
        "ZED_KANBAN_DB": "postgres://u:p@h/db",
        # secret substrings (incl. new DSN/WEBHOOK) â†’ dropped
        "SENTRY_DSN": "https://a@s.io/1", "SLACK_WEBHOOK": "https://h/x",
        "OPENAI_API_KEY": "sk", "GITHUB_TOKEN": "ghp",
        # safe prefix â†’ kept; uncategorized â†’ dropped
        "PATH": "/usr/bin", "RANDOM_X": "y",
    }
    out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    for kept in ("ZED_HOME", "ZED_PROFILE", "ZED_CONFIG", "ZED_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ZED_BASE_URL", "ZED_INTERACTIVE", "ZED_KANBAN_DB",
...
def test_execute_code_entry_blocks_before_spawn_when_guard_denies(monkeypatch, tmp_path):
    ...
    denial, returns the block message WITHOUT spawning the child â€” proven by a
...
def test_env_scrub_logs_dropped_zed_vars(caplog):
    """Dropping a non-allowlisted, non-secret ZED_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset ZED_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "ZED_HOME": "/h",          # allowlisted â†’ kept, not logged
        "ZED_BASE_URL": "https://x",   # dropped â†’ logged
        "ZED_KANBAN_DB": "postgres://u:p@h/db",  # dropped â†’ logged
        "ZED_API_KEY": "sk",       # secret â†’ dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix â†’ kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "ZED_HOME" in out and "PATH" in out
    assert "ZED_BASE_URL" not in out and "ZED_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "ZED_BASE_URL" in msgs and "ZED_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "ZED_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ZED_* vars."""
    import logging
