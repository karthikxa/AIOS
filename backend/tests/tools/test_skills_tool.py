    ...
    def test_skill_view_applies_template_vars(self, tmp_path):
        with (
            ...
            skill_dir = _make_skill(
                tmp_path,
                "templated",
                body="Run ${ZED_SKILL_DIR}/scripts/do.sh in ${ZED_SESSION_ID}",
            )
        ...
        assert "${ZED_SKILL_DIR}" not in result["content"]
...
# ---------------------------------------------------------------------------
# _find_all_skills â€” platform filtering integration
# ---------------------------------------------------------------------------
...
class TestSkillViewCollisionDetection:
    ...
    shadowing â€” where ``/skills`` shows the local version but
    ``skill_view`` loads the external one â€” is the bug class this guards
    ...
    def test_top_level_local_collides_with_external(self, tmp_path):
        ...
        refuses â€” same-name shadowing is ambiguous regardless of nesting."""
        local_dir = tmp_path / "local"
        external_dir = tmp_path / "external"
        local_dir.mkdir()
        external_dir.mkdir()
    ...
    def test_collision_resolvable_via_categorized_path(self, tmp_path):
        ...
        categorized path â€” the bare name is ambiguous, the path is not."""
        local_dir = tmp_path / "local"
        external_dir = tmp_path / "external"
        local_dir.mkdir()
        external_dir.mkdir()
    ...
    def test_two_externals_same_name_also_refuse(self, tmp_path):
        """Collision detection is symmetric â€” two external dirs with
