...
class TestWriteDenyExactPaths:
    ...
    def test_zed_env(self):
        # ``.env`` under the active ZED_HOME (profile-aware, not just
    ...
    def test_zed_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.zed/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "zed_root"
