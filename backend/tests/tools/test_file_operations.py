...
class TestIsWriteDenied:
    ...
    def test_pairing_dir_denied(self, tmp_path, monkeypatch):
        ...
        PR #30383 introduced ~/.zed/pairing/{platform}-approved.json as the
        ...
        gateway access without going through the pairing code flow â€” the same
        ...
        # Root pairing entries (profile mode â€” same shape as mcp-tokens gap)
...
class TestSearchResultDensify:
    ...
    def _matches(self, n, paths=None):
        ...
        # mirrors that â€” group by path, then enumerate lines within each.
        paths = paths or ["a.py"]
        out = []
    ...
    def test_densify_content_is_lossless(self, content):
        ...
        # (trailing whitespace is the one documented transform â€” rstrip).
...
class TestShellFileOpsHelpers:
    ...
    def test_read_file_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "'\x07__ZED_FENCE_a9f7b3__\x1b]0;cat "
            "'/tmp/test/a.py' 2> /dev/null\x07\n"
            "print('ok')\n"
            "__ZED_FENCE_a9f7b3__\x07'\n"
        )
        ...
        assert "ZED_FENCE" not in result.content
    ...
    def test_read_file_raw_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "__ZED_FENCE_a9f7b3__\x07'\n"
            "alpha\n"
            "\x1b]0;cat '/tmp/test/a.txt'\x07__ZED_FENCE_a9f7b3__\n"
        )
...
class TestPatchReplacePostWriteVerification:
    ...
    def test_patch_replace_fails_when_file_not_persisted(self, mock_env):
        ...
        def side_effect(command, **kwargs):
            # cat reads the file â€” both the initial read and the verify read
            if command.startswith("cat "):
            ...
            # but DOESN'T update file_contents â€” simulates silent failure
            return {"output": "", "returncode": 0}
    ...
    def test_patch_replace_succeeds_when_file_persisted(self, mock_env):
        ...
        def side_effect(command, stdin_data=None, **kwargs):
            # A write is the only call that pipes content over stdin â€” key
...
class _DeletedTestGitBaselineCheck:
    """Removed May 2026 â€” these tests asserted on a ``_check_git_baseline``
