...
session (idle TTL, server restart, pod rotation, â€¦) it rejects
subsequent requests with a JSON-RPC error containing phrases like
``"Invalid or expired session"``.  The OAuth token remains valid â€”
...
# ---------------------------------------------------------------------------
# _is_session_expired_error â€” unit coverage
# ---------------------------------------------------------------------------
...
def test_is_session_expired_rejects_interrupted_error():
    """InterruptedError is the user-cancel signal â€” must never route
...
# ---------------------------------------------------------------------------
# Handler integration â€” verify the recovery plumbing wires end-to-end
# ---------------------------------------------------------------------------
...
def _install_stub_server(name: str = "wpcom"):
    ...
    server = MagicMock()
    server.name = name
    # _reconnect_event is called via loop.call_soon_threadsafe(â€¦set); use
    ...
    # Immediately "ready" â€” simulates a fast reconnect (_ready.is_set()
...
def test_call_tool_handler_reconnects_on_session_expired(monkeypatch, tmp_path):
    ...
    try:
        ...
        # Retry succeeded â€” no error surfaced to caller.
        ...
        assert reconnect_flag.is_set(), (
            ...
            "error â€” the reconnect flow is the whole point of this fix."
        )
...
def test_call_tool_handler_non_session_expired_error_falls_through(
    ...
    NOT trigger reconnect â€” it must fall through to the generic error
    ...
    async def _raises(*a, **kw):
        raise RuntimeError("Tool execution failed â€” unrelated error")
    ...
    try:
        ...
        assert not reconnect_flag.is_set(), (
            "Reconnect must not fire for non-session-expired errors â€” "
...
def test_session_expired_handler_returns_none_without_loop(monkeypatch):
    ...
    try:
        ...
        assert out is None, (
            ...
            "through to caller's generic error path â€” not hang or raise."
        )
    finally:
...
def test_session_expired_handler_returns_none_without_server_record():
    ...
    through cleanly â€” nothing to reconnect to."""
    from tools.mcp_tool import _handle_session_expired_and_retry
    out = _handle_session_expired_and_retry(
        "does-not-exist",
        RuntimeError("Invalid or expired session"),
...
def test_session_expired_handler_returns_none_when_retry_also_fails(
    ...
    try:
        ...
        assert out is None, (
            ...
            "so the caller's generic error path runs â€” no retry loop."
        )
    finally:
...
# prompts/get â€” all four handlers share the same exception path.
# ---------------------------------------------------------------------------
...
def test_non_tool_handlers_also_reconnect_on_session_expired(
    ...
    async def _sequence(*a, **kw):
        ...
        # Explicitly set primitive attrs â€” MagicMock's default auto-attr
