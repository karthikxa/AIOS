"""Tests for tools/skill_usage.py â€” sidecar telemetry + provenance filtering."""
...
def _bump_view_many(zed_home: str, skill_name: str, iterations: int) -> None:
...
def skills_home(tmp_path, monkeypatch):
    """Isolated ZED_HOME with a clean skills/ dir for each test.
...
# ---------------------------------------------------------------------------
# Provenance filter â€” the load-bearing safety check
# ---------------------------------------------------------------------------
...
""",
    ...
    names = list_agent_created_skill_names()
    assert "my-skill" in names
    assert "Getç¬”è®°" not in names
    assert is_agent_created("Getç¬”è®°") is False
...
def test_restore_skill_finds_nested_archive_subdir(skills_home):
    ...
    .archive/<category>/<skill>/) â€” left behind by older archive layouts or
    external imports â€” must still be restorable by name."""
...
def test_archive_collision_gets_suffix(skills_home):
    ...
    assert ok
    # Two entries under .archive/ â€” second should have a timestamp suffix
...
def test_restore_does_not_pull_unrelated_sibling_out_of_archive(skills_home):
    ...
    it to ``git``, and report success â€” destroying the sibling's only copy."""
    from tools.skill_usage import (
...
def test_agent_created_report_includes_marked_skills_with_defaults(skills_home):
    ...
    # b has only the provenance marker â€” activity fields still default.
...
# ---------------------------------------------------------------------------
# Telemetry vs curation â€” usage is tracked for ALL skills; curation is not
# ---------------------------------------------------------------------------
...
def test_bump_view_tracks_bundled_skill(skills_home):
    ...
    # Pruning is off by default in this fixture â†’ not a curation candidate.
...
def test_end_to_end_telemetry_tracked_but_lifecycle_refused(skills_home):
    ...
    for name in ("bundled-one", "hub-one"):
        ...
        # But lifecycle mutators were refused â€” state stays the default, never
