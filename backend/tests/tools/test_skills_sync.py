"""Tests for tools/skills_sync.py â€” manifest-based skill seeding and updating."""
...
class TestRmtreeWritableScopeGuard:
    ...
    root â€” through a bad join, a missing default, a malicious
    bundled-manifest entry, or a stale path in scope after an
    exception â€” the result is a silent ``shutil.rmtree(~/.zed/)``
    ...
    def test_refuses_root_path(self, tmp_path):
        """``Path('/')`` is the entire filesystem â€” must always be rejected."""
    ...
    def test_refuses_zed_home_itself(self, tmp_path):
        """``~/.zed/`` itself is what the #48200 wipe destroyed."""
        from tools.skills_sync import _rmtree_writable

        zed = tmp_path / "home"
        zed.mkdir()
        (zed / "skills").mkdir()
        with patch("tools.skills_sync.SKILLS_DIR", zed / "skills"):
            with pytest.raises(ValueError, match="refusing to rmtree"):
                _rmtree_writable(zed)
    ...
    def test_refuses_sibling_directory(self, tmp_path):
        ...
        skills = zed / "skills"
        skills.mkdir()
        not_skills = zed / "kanban.db"  # any non-skills path
        not_skills.mkdir()
        with patch("tools.skills_sync.SKILLS_DIR", skills):
            with pytest.raises(ValueError, match="refusing to rmtree"):
    ...
    def test_refuses_skills_root_itself(self, tmp_path):
        ...
        No caller in skills_sync.py ever passes SKILLS_DIR directly â€” every
...
class TestSyncSkills:
    ...
    def test_suppressed_builtin_not_reseeded(self, tmp_path):
        ...
        re-copied on sync â€” that's what makes the prune durable across updates.
        """
        ...
        # old-skill is suppressed â†’ skipped, not copied.
    ...
    def test_user_modified_skill_not_overwritten(self, tmp_path):
        ...
        # Should NOT update â€” user modified it
    ...
    def test_v1_migration_then_bundled_update_detected(self, tmp_path):
        ...
        with self._patches(bundled, skills_dir, manifest_file):
            # First sync: migration â€” sets baseline
            sync_skills(quiet=True)

            # Now change bundled content
            (bundled / "old-skill" / "SKILL.md").write_text("# Old v2 â€” improved")

            # Second sync: should detect bundled changed + user unmodified â†’ update
            result = sync_skills(quiet=True)

        assert "old-skill" in result["updated"]
        assert (user_skill / "SKILL.md").read_text() == "# Old v2 â€” improved"
    ...
    def test_collision_does_not_poison_manifest(self, tmp_path):
        ...
        'user-modified' â€” even though the user never touched a bundled copy.
        """
        bundled = self._setup_bundled(tmp_path)
        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"
        ...
        user_skill = skills_dir / "category" / "new-skill"
        user_skill.mkdir(parents=True)
        (user_skill / "SKILL.md").write_text("# From hub â€” unrelated to bundled")
        ...
        assert (user_skill / "SKILL.md").read_text() == (
            "# From hub â€” unrelated to bundled"
        )

        # Manifest must NOT contain the skill â€” it was never synced from bundled.
        with patch("tools.skills_sync.MANIFEST_FILE", manifest_file):
            manifest = _read_manifest()
    ...
    def test_collision_does_not_trigger_false_user_modified_on_resync(self, tmp_path):
        ...
        assert "new-skill" not in result2["user_modified"], (
            ...
            "as 'user-modified' â€” the manifest was poisoned on the first sync."
        )
    ...
    def test_collision_prints_reset_hint(self, tmp_path, capsys):
        ...
        assert "zed skills reset new-skill" in captured
    ...
    def test_failed_copy_does_not_poison_manifest(self, tmp_path):
        ...
        with self._patches(bundled, skills_dir, manifest_file):
            ...
            manifest = _read_manifest()
            assert "new-skill" not in manifest, (
                "Failed copy was recorded in manifest â€” next sync will "
                "treat it as 'user deleted' and never retry"
            )

            # Now run sync again (copytree works this time) â€” it should retry
    ...
    def test_failed_update_does_not_destroy_user_copy(self, tmp_path):
        ...
        with self._patches(bundled, skills_dir, manifest_file):
            ...
            # but copytree failed to replace it â€” this is data loss)
...
class TestGetBundledDir:
    def test_env_var_override(self, tmp_path, monkeypatch):
        """ZED_BUNDLED_SKILLS env var overrides the default path resolution."""
        custom_dir = tmp_path / "custom_skills"
        custom_dir.mkdir()
        monkeypatch.setenv("ZED_BUNDLED_SKILLS", str(custom_dir))
    ...
    def test_default_without_env_var(self, monkeypatch):
        ...
        monkeypatch.delenv("ZED_BUNDLED_SKILLS", raising=False)
        result = _get_bundled_dir()
        assert result.name == "skills"

    def test_env_var_empty_string_ignored(self, monkeypatch):
        """Empty ZED_BUNDLED_SKILLS should fall back to default."""
        monkeypatch.setenv("ZED_BUNDLED_SKILLS", "")
        result = _get_bundled_dir()
        assert result.name == "skills"


class TestResetBundledSkill:
    """Covers reset_bundled_skill() â€” the escape hatch for the 'user-modified' trap."""
    ...
    def test_reset_clears_stuck_user_modified_flag(self, tmp_path):
        ...
        # Stale origin_hash â€” from some prior bundled version. User "restored" by pasting
        # the current bundled contents, so user_hash == current bundled_hash, but manifest
        # still points at the stale hash â†’ treated as user_modified forever.
    ...
    def test_reset_no_op_when_already_clean(self, tmp_path):
        ...
        # Simulate a clean state â€” do a fresh sync first
        with self._patches(bundled, skills_dir, manifest_file):
            sync_skills(quiet=True)
    ...
    def test_reset_restore_succeeds_on_readonly_nix_tree(self, tmp_path):
        ...
        # Read-only files AND directories â€” the real Nix-store case.
        ro_dir = (
    ...
    def test_reset_restore_preserves_manifest_on_rmtree_failure(self, tmp_path):
        ...
        entry must NOT be deleted â€” otherwise the skill enters a limbo state
...
class TestNoBundledSkillsOptOut:
    ...
    This is what `zed profile create --no-skills` (named profiles) and the
    installer's `--no-skills` flag (default ~/.zed) rely on so bundled
    skills are never seeded at install time NOR re-injected by `zed update`.
    """
    ...
    def test_marker_skips_sync(self, tmp_path):
        ...
        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"
        zed_home = tmp_path / "home"
        zed_home.mkdir()
        (zed_home / ".no-bundled-skills").write_text("opted out\n")

        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), \
             patch("tools.skills_sync.SKILLS_DIR", skills_dir), \
             patch("tools.skills_sync.MANIFEST_FILE", manifest_file), \
             patch("tools.skills_sync.ZED_HOME", zed_home):
            result = sync_skills(quiet=True)
...
class TestOptOutToggleAndRemove:
    """`zed skills opt-out/opt-in` core: marker toggle + safe removal."""
    ...
    def test_marker_toggle(self, tmp_path):
        ...
        home = tmp_path / "home"
        home.mkdir()
        with patch("tools.skills_sync.ZED_HOME", home):
    ...
    def test_remove_keeps_user_modified(self, tmp_path):
        ...
        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), \
             ...
             patch("tools.skills_sync.SKILLS_DIR", skills_dir), \
             patch("tools.skills_sync.MANIFEST_FILE", manifest_file), \
             patch("tools.skills_sync.ZED_HOME", home):
            sync_skills(quiet=True)
            # User edits 'beta'
...
class TestUpdateBackupRecovery:
    ...
    def test_stale_backup_does_not_poison_failed_update(self, tmp_path):
        ...
        to dest â€” corrupting the skill and wedging it as "user-modified".
        """
    ...
    def test_orphaned_backup_is_recovered_not_treated_as_deleted(self, tmp_path):
        ...
        branch reads that as a deliberate user deletion and skips â€” the
