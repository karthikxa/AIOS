...
``reload.mcp`` RPC, the gateway reload, and the late-binding refresh thread â€”
...
def test_refresh_adds_late_landing_tools(monkeypatch):
    """A server that registers after build â†’ its tools land in the snapshot."""
...
def test_refresh_no_change_returns_empty_and_leaves_agent_untouched(monkeypatch):
    """No new tools â†’ empty set, and the snapshot object is not swapped."""
    agent = _agent(["read_file", "terminal"])
    ...
    assert added == set()
    assert agent.tools is original_tools  # not replaced â†’ no churn / no cache thrash
...
def test_refresh_preserves_memory_provider_and_context_engine_tools(monkeypatch):
    ...
    appends memory-provider tools (mem0/honcho/â€¦) and context-engine tools
...
def test_refresh_is_thread_safe_under_concurrent_calls(monkeypatch):
    ...
    no-change early return â€” this actually exercises the lock. The invariant:
...
# â”€â”€ discovery-wait bound (mcp_discovery_timeout config) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_resolve_discovery_timeout_falls_back_on_bad_value(monkeypatch):
    ...
    # Non-positive / unparsable â†’ DEFAULT_CONFIG value, never hang.
