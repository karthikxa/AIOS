...
class TestWebProviderABCs:
    ...
    same PR â€” providers now advertise capabilities via
    ...
    def test_search_only_provider_skips_extract(self):
        ...
        # Should instantiate fine â€” extract has default supports_*()
...
class TestPerCapabilityBackendSelection:
    ...
    def test_fully_backward_compatible_with_web_backend_only(self, monkeypatch):
        ...
        monkeypatch.setenv("TAVILY_API_KEY", "test-key")
        # No search_backend or extract_backend set â€” both fall through
        assert web_tools._get_search_backend() == "tavily"
...
class TestUnconfiguredErrorEnvelopeParity:
    ...
    def test_unconfigured_search_emits_top_level_error(self, monkeypatch):
        ...
        â€” matching main's ``tool_error()`` envelope, not a per-result shape.
        """
        from tools import web_tools
...
class TestDispatchersTriggerPluginDiscovery:
    ...
    Mirrors :func:`tools.browser_tool._ensure_browser_plugins_loaded` â€”
    ...
    def test_web_extract_tool_runs_discovery_before_registry_lookup(self, monkeypatch):
        ...
        try:
            ...
            # also assert the dispatcher actually invoked the hook â€” if
            ...
            # underlying ``zed_cli.plugins._ensure_plugins_discovered`` so
            ...
            # The hook must have been called BEFORE the registry lookup â€”
