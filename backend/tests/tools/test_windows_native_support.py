...
code paths Zed takes on native Windows.

Runs on Linux CI â€” every test mocks ``sys.platform``, ``subprocess.run``,
...
class TestConfigureWindowsStdio:
    ...
    - respect ZED_DISABLE_WINDOWS_UTF8 opt-out
    """
    ...
    def test_windows_path_sets_env_and_reconfigures_streams(self, monkeypatch):
        ...
        monkeypatch.delenv("ZED_DISABLE_WINDOWS_UTF8", raising=False)
        ...
        # but not on the Linux CI runner â€” mock it so the editor default
        # survives).
...
# ---------------------------------------------------------------------------
# terminate_pid â€” the centralized kill primitive
# ---------------------------------------------------------------------------
...
class TestTerminatePidRoutingOnWindows:
    ...
    def test_graceful_on_windows_uses_os_kill_sigterm(self, monkeypatch):
        ...
        and uses ``os.kill`` directly â€” so platform doesn't actually matter
...
class TestProcessRegistryOSErrorWidening:
    ...
    def test_oserror_treated_as_not_alive(self, monkeypatch):
        """_pid_exists â†’ False propagates as _is_host_pid_alive â†’ False."""
    ...
    def test_permission_error_treated_as_alive(self, monkeypatch):
        ...
        this test (which asserted PermissionError â†’ not-alive). The old
...
class TestTzdataDependencyDeclared:
    ...
    def test_pyproject_declares_tzdata_for_win32(self):
        ...
        # â€” only that tzdata is declared with a win32 marker. This is an
        # invariant check, not a snapshot test.
        import re
        # Match `"tzdata` â€¦ `; sys_platform == 'win32'"` allowing any version
...
class TestReadmeNoLongerSaysWindowsUnsupported:
    ...
    def test_readme_does_not_say_not_supported(self):
        ...
        assert "Native Windows is not supported" not in source, (
            "README.md still says native Windows is not supported â€” update the "
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "zed_cli" / "web_server.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """cli.py, zed_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "zed_cli/main.py", "gateway/run.py"],
    )
...
class TestSubprocessCompatHelpers:
    ...
    def test_resolve_node_command_fallback_when_absent(self):
        ...
        # Must fall back to the bare name â€” NOT return None, NOT crash.
    ...
    def test_windows_detach_flags_includes_breakaway_from_job(self, monkeypatch):
        ...
        Without it, the gateway-respawn watcher spawned by ``zed update``
        (which runs under zed-setup.exe, itself a grandchild of the
...
class TestTuiGatewayEntrySignalGuards:
    ...
    def test_module_imports_cleanly(self):
        """Importing the module must not raise â€” verifies the guards work."""
...
# ---------------------------------------------------------------------------
# zed_cli/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------


class TestKanbanWaitpidWindowsGuard:
    """os.WNOHANG doesn't exist on Windows â€” the dispatcher tick reap loop
    must be gated behind ``os.name != "nt"``."""

    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "zed_cli" / "kanban_db.py").read_text(encoding="utf-8")
...
class TestCodeExecutionTransportTcpFallback:
    ...
    AF_INET endpoints based on the ZED_RPC_SOCKET format.
    """
...
class TestCronSchedulerBashResolution:
    """cron.scheduler must NOT hardcode /bin/bash â€” .sh scripts need a
    ...
    def test_no_bare_npm_or_npx_in_popen_argv(self, relpath):
        ...
        for pat in forbidden_patterns:
            ...
            while True:
                ...
                if idx < 0:
                    break
                # Look at the preceding 120 chars â€” if "shutil.which" appears
                ...
                raise AssertionError(
                    f"{relpath}: bare {pat!r} still present at offset {idx} â€” "
...
class TestLocalEnvironmentWindowsTempDir:
    ...
    def test_posix_path_preserved_on_linux(self):
        """Linux/macOS behaviour MUST be unchanged â€” return / tmp or
        tempfile.gettempdir()-derived POSIX path.  This is the 'do no harm'
        test â€” regressions here break every Unix user's terminal tool."""
    ...
    def test_source_has_windows_branch_using_zed_home(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "tools" / "environments" / "local.py").read_text(encoding="utf-8")
        assert "if _IS_WINDOWS:" in source
        assert "get_zed_home" in source
        assert 'cache_dir = get_zed_home() / "cache" / "terminal"' in source
...
# ---------------------------------------------------------------------------
# Gateway detached watcher â€” Windows creationflags
# ---------------------------------------------------------------------------
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_zed_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "zed_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "zed_cli/gateway.py must use the platform-aware detach helper"
        )
    ...
    def test_launch_detached_profile_gateway_restart_inlined_watcher_uses_breakaway(self):
        ...
        Static check â€” the watcher source is built at import time and embedded
        ...
        """
        root = Path(__file__).resolve().parents[2]
        text = (root / "zed_cli" / "gateway.py").read_text(encoding="utf-8")
        ...
        assert "0x01000000" in block, (
            ...
            "(0x01000000) on the respawned gateway â€” without it, the new "
    ...
    def test_launch_detached_profile_gateway_restart_outer_popen_has_access_denied_fallback(
        ...
        Static check â€” without standing up a real Windows job object
        ...
        # And the inlined watcher's respawn must also handle the denial â€”
