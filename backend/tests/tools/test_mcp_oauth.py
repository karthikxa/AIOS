"""Tests for tools/mcp_oauth.py â€” OAuth 2.1 PKCE support for MCP servers."""
...
class TestZedTokenStorage:
    def test_roundtrip_tokens(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ZED_HOME", str(tmp_path))
        storage = ZedTokenStorage("test-server")
    ...
    def test_token_file_created_with_0o600(self, tmp_path, monkeypatch):
        ...
        """
        monkeypatch.setenv("ZED_HOME", str(tmp_path))
        storage = ZedTokenStorage("perm-test-server")
        ...
        assert mode == 0o600, f"token file mode {oct(mode)} != 0o600 â€” TOCTOU race regressed"

        parent_mode = stat.S_IMODE(token_path.parent.stat().st_mode)
        assert parent_mode == 0o700, (
            f"token parent dir mode {oct(parent_mode)} != 0o700 â€” siblings can traverse"
        )
    ...
    def test_has_cached_tokens(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ZED_HOME", str(tmp_path))
        storage = ZedTokenStorage("my-server")
    ...
    def test_corrupt_tokens_returns_none(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ZED_HOME", str(tmp_path))
        storage = ZedTokenStorage("bad-server")
...
class TestPathTraversal:
    ...
    def test_path_traversal_blocked(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ZED_HOME", str(tmp_path))
        storage = ZedTokenStorage("../../.ssh/config")
    ...
    def test_dots_and_slashes_sanitized(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ZED_HOME", str(tmp_path))
        storage = ZedTokenStorage("../../../etc/passwd")
        path = storage._tokens_path()
...
class TestWaitForCallbackNoBlocking:
    """_wait_for_callback() must never call input() â€” it raises instead."""
...
def test_build_oauth_auth_preserves_server_url_path():
    ...
    ``OAuthContext.get_authorization_base_url``; Zed must not pre-strip.
    """
    ...
    with patch.object(mcp_oauth, "_OAUTH_AVAILABLE", True), \
         ...
         patch.object(mcp_oauth, "ZedTokenStorage") as mock_storage_cls:
