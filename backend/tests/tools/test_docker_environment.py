...
def test_non_persistent_cleanup_removes_container(monkeypatch):
    ...
    docker rm so containers don't leak across zed processes.
...
def _make_execute_only_env(forward_env=None):
    ...
    env._snapshot_path = "/tmp/zed-snap-test123.sh"
    env._cwd_file = "/tmp/zed-cwd-test123.txt"
    env._cwd_marker = "__ZED_CWD_test123__"
...
def test_init_env_args_uses_zed_dotenv_for_allowlisted_env(monkeypatch):
    """_build_init_env_args picks up forwarded env vars from .env file at init time."""
    # Use a var that is NOT in _ZED_PROVIDER_ENV_BLOCKLIST (GITHUB_TOKEN
    # is in the copilot provider's api_key_env_vars and gets stripped).
    env = _make_execute_only_env(["DATABASE_URL"])

    monkeypatch.delenv("DATABASE_URL", raising=False)
    monkeypatch.setattr(docker_env, "_load_zed_env_vars", lambda: {"DATABASE_URL": "value_from_dotenv"})
...
def test_init_env_args_prefers_shell_env_over_zed_dotenv(monkeypatch):
...
def test_init_env_args_uses_zed_dotenv_for_empty_shell_env(monkeypatch):
    ...
    MY_SECRET=`, clobbering the correct value sitting in ~/.zed/.env.
    """
    env = _make_execute_only_env(["MY_SECRET"])

    monkeypatch.setenv("MY_SECRET", "")
    monkeypatch.setattr(docker_env, "_load_zed_env_vars", lambda: {"MY_SECRET": "value_from_dotenv"})
...
def test_init_env_args_never_forwards_blank_secret(monkeypatch):
    ...
    monkeypatch.setenv("MY_SECRET", "")
    monkeypatch.setattr(docker_env, "_load_zed_env_vars", lambda: {})

    args = env._build_init_env_args()

    # The key must not appear at all â€” not even as an empty -e MY_SECRET= flag.
    assert not any(a.startswith("MY_SECRET=") for a in args)
    assert "MY_SECRET" not in " ".join(args)


# â”€â”€ docker_env tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_security_args_include_setuid_setgid_for_privdrop(monkeypatch):
    ...
    (e.g. via ``s6-setuidgid`` in the bundled Zed image, or ``gosu``/``su``
    ...
    escalate back to root after the drop â€” the drop is a one-way transition
    ...
    assert "SETUID" in added, "SETUID cap missing â€” image privilege-drop will fail"
    assert "SETGID" in added, "SETGID cap missing â€” image privilege-drop will fail"


# â”€â”€ run_as_host_user tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_run_as_host_user_drops_setuid_setgid_caps(monkeypatch):
    ...
    assert "SETUID" not in added, (
        "SETUID cap should be dropped when running as host user â€” no privilege drop is needed"
    )
    assert "SETGID" not in added, (
        "SETGID cap should be dropped when running as host user â€” no privilege drop is needed"
    )
...
def test_run_as_host_user_default_off(monkeypatch):
    """Without the opt-in, no --user flag is emitted â€” preserving existing behavior."""
    monkeypatch.setattr(docker_env, "find_docker", lambda: "/usr/bin/docker")
    calls = _mock_subprocess_run(monkeypatch)
...
# â”€â”€ Docker labels (issue #20561) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_run_command_tags_zed_agent_label(monkeypatch):
    """Every container zed-agent starts must carry the zed-agent=1 label
    so the orphan reaper (and external operators) can identify them with a
    single ``docker ps --filter label=zed-agent=1`` call. Regression test
    for issue #20561 â€” without the label there is no global sweep target."""
    monkeypatch.setattr(docker_env, "find_docker", lambda: "/usr/bin/docker")
    calls = _mock_subprocess_run(monkeypatch)
    ...
    assert "zed-agent=1" in labels, (
        f"zed-agent=1 label missing; got labels: {sorted(labels)}"
    )
...
def test_run_command_tags_task_and_profile_labels(monkeypatch):
    ...
    returns ``"default"`` for the root Zed home."""
    monkeypatch.setattr(docker_env, "find_docker", lambda: "/usr/bin/docker")
    ...
    labels = _labels_in_run_args(_run_args_from_calls(calls))
    assert "zed-task-id=kanban-42" in labels, (
        f"zed-task-id=kanban-42 missing; got: {sorted(labels)}"
    )
    assert "zed-profile=research-bot" in labels, (
        f"zed-profile=research-bot missing; got: {sorted(labels)}"
    )


def test_label_sanitizer_rejects_invalid_characters():
    """Docker label values must be alnum + ``_.-`` and â‰¤63 chars. Profile or
    ...
    assert docker_env._sanitize_label_value("emoji-ðŸ˜€-here") == "emoji-_-here"
...
def test_run_command_sanitizes_unsafe_task_id(monkeypatch):
    ...
    assert "zed-task-id=task_with_weird_chars" in labels, (
...
# â”€â”€ Cross-process container reuse (issue #20561) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _mock_subprocess_run_with_reuse(monkeypatch, ps_state: str | None,
    ...
    ``ps_state`` controls what ``docker ps -a --filter ...`` returns:
      * ``None`` â†’ no match (empty stdout). Forces a fresh ``docker run``.
      * ``"running"`` / ``"exited"`` / ... â†’ emit ``CID\\tSTATE`` so the reuse
...
def test_reuse_attaches_to_running_container_without_docker_run(monkeypatch):
    ...
    issue #20561 root cause: every Zed process spawning a new container
...
def test_reuse_starts_stopped_container_before_attaching(monkeypatch):
    ...
    ``docker start`` before the new Zed process uses it. Without this
...
def test_reuse_falls_back_to_fresh_run_when_start_fails(monkeypatch):
    ...
    user with a broken environment. Defensive recovery â€” the probe is best-
...
def test_failed_docker_run_cleans_up_orphaned_container(monkeypatch):
    ...
    assert rm_cmd[3].startswith("zed-"), "should remove the container by its generated name"
...
def test_no_reuse_when_persist_across_processes_disabled(monkeypatch):
    ...
    monkeypatch.setattr(docker_env, "find_docker", lambda: "/usr/bin/docker")
    monkeypatch.setattr(docker_env, "_get_active_profile_name", lambda: "default")
    # ps_state=running would trigger reuse if the probe ran â€” assert it doesn't.
    calls = _mock_subprocess_run_with_reuse(monkeypatch, ps_state="running")
...
# â”€â”€ Cleanup correctness (issue #20561) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_cleanup_with_persist_is_noop_for_container(monkeypatch):
    ...
    NOR remove the container â€” the docs promise "ONE long-lived container
    ...
    Zed startup, not on graceful exit. Issue #20561 â€” the first iteration
...
def test_cleanup_force_remove_stops_and_rms_even_in_persist_mode(monkeypatch):
    ...
    fresh container without also flipping the global config â€” too coarse for
    a per-task reset.
    """
    monkeypatch.setattr(docker_env, "find_docker", lambda: "/usr/bin/docker")
    monkeypatch.setattr(docker_env, "_get_active_profile_name", lambda: "default")
...
def test_cleanup_vm_force_remove_tears_down_persist_container(monkeypatch):
    ...
    container â€” the explicit-teardown path for ``/reset``-style flows.
...
def test_cleanup_with_persist_disabled_stops_and_rms(monkeypatch):
    ...
    ``persistent_filesystem`` setting â€” the original code only rm'd when
    ...
    assert rms, (
        ...
        "persistent_filesystem=True â€” that gating was the leak source in #20561."
    )
...
def test_cleanup_uses_subprocess_run_not_detached_shell(monkeypatch):
    ...
    ``force_remove=True`` so cleanup actually issues docker calls â€” the
...
def test_wait_for_cleanup_returns_true_when_no_thread_started():
    ...
    (or the env has no live cleanup thread) â€” atexit calls it unconditionally
    across all active envs, so a False return would falsely flag healthy
    shutdowns."""
    env = docker_env.DockerEnvironment.__new__(docker_env.DockerEnvironment)
    # No _cleanup_thread set â€” simulates an env that was never cleanup()'d.
...
def test_wait_for_cleanup_after_cleanup_returns_true(monkeypatch):
    ...
    Uses ``force_remove=True`` so cleanup actually starts a worker thread â€”
...
def test_cleanup_on_env_with_no_container_id_does_not_raise(monkeypatch):
    ...
    cleanup() â€” the post-creation failure path in callers always tries.
...
# â”€â”€ Orphan reaper (issue #20561) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _now_iso(offset_seconds: int = 0) -> str:
    ...
    # Format like Docker emits â€” with nanoseconds-style trailing digits.
...
def _reaper_run_mock(monkeypatch, ps_ids: list[str], inspect_responses: dict[str, str],
    ...
    * ``ps_ids`` â€” what ``docker ps -a --filter ... --format '{{.ID}}'`` returns
    * ``inspect_responses[cid]`` â€” what ``docker inspect ... FinishedAt`` returns
      for each cid; ``""`` means "field unset".
    * ``rm_succeeds`` â€” whether ``docker rm -f`` returns 0.
...
def test_reap_orphan_returns_zero_when_no_matches(monkeypatch):
    """No labeled containers â†’ no rm calls, returns 0. Establishes the
...
def test_reap_orphan_removes_stale_exited_container(monkeypatch):
    ...
    This is the core repair path for issue #20561 â€” without the reaper,
    SIGKILL'd Zed processes leak containers permanently."""
...
def test_reap_orphan_spares_recently_exited_container(monkeypatch):
    """A container exited within max_age_seconds must NOT be reaped â€” that
    container belongs to a Zed process that just finished and may be
...
def test_reap_orphan_scopes_to_profile_filter_via_label(monkeypatch):
    """The reaper must pass ``--filter label=zed-profile=<profile>`` to
    ...
    assert "label=zed-profile=research-bot" in flat, (
        f"profile filter not applied to docker ps; got args: {ps_calls[0][0]}"
    )
    assert "label=zed-agent=1" in flat, (
        f"zed-agent label filter must also be applied; got: {ps_calls[0][0]}"
    )
    assert "status=exited" in flat, (
        "must filter to exited containers only â€” running containers may "
        "belong to a sibling Zed process and must NEVER be reaped"
    )
...
def test_reap_orphan_skips_container_with_unparseable_finished_at(monkeypatch):
    ...
    container alone. Defensive â€” never reap a container whose age we can't
    determine."""
    calls = _reaper_run_mock(
        monkeypatch,
...
def test_reap_orphan_handles_docker_ps_failure_gracefully(monkeypatch):
    ...
    not a critical path â€” it must never block container creation."""
...
def test_container_finished_at_parses_nanosecond_timestamp(monkeypatch):
    ...
    The helper must trim the extra digits without raising â€” otherwise every
...
def test_container_finished_at_returns_none_on_zero_value():
    ...
    # Direct test of the parsing helper â€” no subprocess needed since the
...
def test_credential_mount_skipped_when_source_is_directory(monkeypatch, tmp_path, caplog):
    ...
    fake_mounts = [
        {"host_path": str(corrupted_dir), "container_path": "/root/.zed/google_token.json"},
    ]
    monkeypatch.setattr(
...
def test_credential_mount_skipped_when_source_missing(monkeypatch, tmp_path, caplog):
    ...
    # Don't create the file â€” it's "missing"

    monkeypatch.setattr(docker_env, "find_docker", lambda: "/usr/bin/docker")
    calls = _mock_subprocess_run(monkeypatch)

    fake_mounts = [
        {"host_path": str(missing_path), "container_path": "/root/.zed/deleted_token.json"},
    ]
    monkeypatch.setattr(
        "tools.credential_files.get_credential_file_mounts",
        lambda: fake_mounts,
...
def test_credential_mount_works_when_source_is_valid_file(monkeypatch, tmp_path):
    ...
    fake_mounts = [
        {"host_path": str(valid_file), "container_path": "/root/.zed/token.json"},
    ]
    monkeypatch.setattr(
        "tools.credential_files.get_credential_file_mounts",
        lambda: fake_mounts,
...
# â”€â”€ s6-overlay /init image handling (issue #34628) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_image_uses_init_entrypoint_detects_s6_init(monkeypatch):
    ...
    monkeypatch.setattr(docker_env.subprocess, "run", _run)
    assert docker_env._image_uses_init_entrypoint("/usr/bin/docker", "zed-agent:latest") is True
...
def test_image_uses_init_entrypoint_false_on_exception(monkeypatch):
    """A subprocess error never raises out of detection â€” defaults kept."""
    def _run(cmd, **kwargs):
...
def test_s6_image_skips_docker_init_and_mounts_run_exec(monkeypatch):
    ...
    _make_dummy_env(image="zed-agent:latest")
...
def test_is_container_gone_matches_removal_errors(monkeypatch):
    ...
    assert env._is_container_gone(
        "Error response from daemon: No such container: zed-abc123"
    )
    ...
    # the container being gone â€” otherwise every non-zero exit would trigger a
...
def test_execute_recovers_from_out_of_band_removal(monkeypatch):
    ...
    the "No such container" error, recreates the container, and retries once â€”
    ...
    outputs = iter([
        {"output": "Error response from daemon: No such container: zed-x", "returncode": 1},
...
def test_execute_does_not_recover_when_not_persistent(monkeypatch):
    ...
    "No such container" error â€” recovery is only meaningful for the persistent,
