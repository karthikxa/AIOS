...
Verifies:
  - Tools are gated on ZED_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `zed chat` sessions (no ZED_KANBAN_TASK) must have
    zero kanban_* tools in their schema."""
    monkeypatch.delenv("ZED_KANBAN_TASK", raising=False)
    home = tmp_path / ".zed"
    home.mkdir()
    ...
    schema = registry.get_definitions(set(resolve_toolset("zed-cli")), quiet=True)
...
def test_kanban_tools_visible_with_env_var(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("ZED_KANBAN_TASK", "t_fake")
    home = tmp_path / ".zed"
    home.mkdir()
    monkeypatch.setenv("ZED_HOME", str(home))
...
def test_worker_with_kanban_toolset_still_hides_board_routing(monkeypatch, tmp_path):
    ...
    in its config, the ZED_KANBAN_TASK env var means it's a focused
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: ZED_HOME isolated, ZED_KANBAN_TASK set
    after we've created the task."""
    home = tmp_path / ".zed"
    home.mkdir()
    monkeypatch.setenv("ZED_HOME", str(home))
    monkeypatch.setenv("ZED_PROFILE", "test-worker")
    monkeypatch.delenv("ZED_SESSION_ID", raising=False)
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("ZED_KANBAN_TASK", tid)
    return tid
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        "changed_files": ["zed_cli/kanban.py"],
...
def test_complete_stamps_worker_session_id_from_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("ZED_SESSION_ID", "session-trusted")
...
def test_complete_phantom_card_message_advertises_retry(worker_env):
    ...
    tells the worker the task is still in-flight and how to retry â€” the
    ...
    # The retry-is-supported phrasing â€” these are the literal cues a
    ...
    # Critically: the task is genuinely still in-flight â€” the gate
...
def test_complete_retry_with_corrected_created_cards_succeeds(worker_env):
    ...
    # First attempt mixes real + phantom â€” gate rejects.
    rejected = json.loads(kt._handle_complete({
        "summary": "oops",
...
def test_heartbeat_extends_claim_expires(worker_env):
    ...
    update last_heartbeat_at â€” otherwise long-running workers loop the
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to ZED_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    """``args["author"]`` is no longer honored â€” the author is always
    derived from ``ZED_PROFILE`` so a worker can't forge a comment
    under an authoritative-looking name like ``zed-system`` and
    ...
    from tools import kanban_tools as kt
    out = kt._handle_comment({
        "task_id": worker_env, "body": "hi", "author": "zed-system",
    })
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from ZED_PROFILE in the fixture, not the
        # caller-supplied "zed-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_create_inherits_worker_dir_workspace(monkeypatch, worker_env):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("ZED_KANBAN_TASK", self_tid)
...
def test_create_no_worker_task_stays_scratch(monkeypatch, worker_env):
    """Orchestrator/CLI callers (no ZED_KANBAN_TASK) still default to
    scratch â€” inheritance only applies to task-scoped workers."""
    from tools import kanban_tools as kt
    from hermes_cli import kanban_db as kb
...
def test_create_session_id_arg_overrides_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("ZED_SESSION_ID", "from-env")
    from tools import kanban_tools as kt
    from hermes_cli import kanban_db as kb
    out = kt._handle_create({
...
def test_create_session_id_absent_when_env_unset(monkeypatch, worker_env):
    """No env var, no arg â†’ session_id stays NULL. Important for backwards
...
def test_worker_lifecycle_through_tools(worker_env):
    ...
    try:
        ...
        # have promoted it â€” complete_task runs recompute internally).
...
def test_kanban_guidance_not_in_normal_prompt(monkeypatch, tmp_path):
    """A normal chat session (no ZED_KANBAN_TASK) must NOT have
...
def test_kanban_guidance_in_worker_prompt(monkeypatch, tmp_path):
    """A worker session (ZED_KANBAN_TASK set) MUST have the full
    ...
    prompt = a._build_system_prompt()
    # Header phrase (identity-free â€” SOUL.md owns identity, layer 3 is protocol)
    ...
    # Anti-shell guidance
    assert "Do not shell out" in prompt or "tools â€” they work" in prompt
...
def test_kanban_guidance_prompt_size_bounded(monkeypatch, tmp_path):
    ...
    assert 1_500 < len(KANBAN_GUIDANCE) < 4_096, (
        f"KANBAN_GUIDANCE is {len(KANBAN_GUIDANCE)} chars â€” too short (missing?) or too long"
    )
...
# ---------------------------------------------------------------------------
#
# A worker process has ZED_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no ZED_KANBAN_TASK in env) are intentionally
# exempt â€” their job is routing, and they sometimes close out child
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    did NOT add an ownership gate to kanban_comment â€” comments are the
    ...
    # ZED_PROFILE â€” never to a caller-controlled string.
    conn = kb.connect()
    try:
...
def test_worker_unblock_rejects_foreign_task_id(worker_env):
    """A worker cannot unblock any task â€” kanban_unblock is orchestrator-only.

    The check fires before the per-task ownership check, so the error
    surface is the orchestrator-only refusal rather than the
    cross-task-ownership refusal. Either is fine â€” the property we're
...
def test_worker_complete_rejects_stale_run_id(worker_env, monkeypatch):
    ...
    # for this test â€” we WANT immediate reclamation here.
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("ZED_KANBAN_RUN_ID", str(run1.id))
    ...
    monkeypatch.setenv("ZED_KANBAN_RUN_ID", str(run2.id))
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no ZED_KANBAN_TASK) can still complete
...
# ---------------------------------------------------------------------------
# Optional ``board`` parameter â€” per-call DB override
# ---------------------------------------------------------------------------
#
# The dispatcher pins the active board via ZED_KANBAN_BOARD env var,
...
# restarting Zed. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    """Isolated Zed home with two distinct kanban boards seeded.

    Returns ``("default", "alt")`` slugs. The default board has one
    pre-existing task ``seed_default``; ``alt`` has ``seed_alt``. No
    ZED_KANBAN_TASK is pinned (orchestrator context) â€” workers test
    the env-task case via the existing ``worker_env`` fixture.
    """
    home = tmp_path / ".zed"
    home.mkdir()
    monkeypatch.setenv("ZED_HOME", str(home))
    # Make sure neither ZED_KANBAN_DB nor ZED_KANBAN_BOARD pin a
    # board â€” the test is specifically about the per-call override.
    monkeypatch.delenv("ZED_KANBAN_DB", raising=False)
    monkeypatch.delenv("ZED_KANBAN_BOARD", raising=False)
    monkeypatch.delenv("ZED_KANBAN_TASK", raising=False)
    monkeypatch.setenv("ZED_PROFILE", "test-orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)

    from hermes_cli import kanban_db as kb
    kb._INITIALIZED_PATHS.clear()
    # Default board â€” implicit
    conn = kb.connect()
    try:
        ...
        conn.close()
    # Alt board â€” explicit slug routes the connection to a separate DB
...
def test_board_param_routes_list_to_alt_board(multi_board_env):
    ...
    # Default â€” sees seed-default, not seed-alt.
    ...
    # Alt â€” sees seed-alt, not seed-default.
...
def test_board_param_routes_assign_via_create_to_alt(multi_board_env):
    """Workflow test for the 'assign' UX â€” create with assignee on a
...
def test_board_param_routes_heartbeat_to_alt_board(monkeypatch, tmp_path):
    ...
    use the worker-env style fixture inline (pinning ZED_KANBAN_TASK
    to a task that exists in the alt board)."""
    home = tmp_path / ".zed"
    home.mkdir()
    monkeypatch.setenv("ZED_HOME", str(home))
    monkeypatch.setenv("ZED_PROFILE", "alt-worker")
    monkeypatch.delenv("ZED_KANBAN_DB", raising=False)
    monkeypatch.delenv("ZED_KANBAN_BOARD", raising=False)
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
def test_board_param_none_falls_back_to_env(worker_env):
    ...
    before this feature â€” calls land on whatever the env resolves to.
...
def test_board_param_in_all_schemas():
    ...
    parameter. This pins the contract surfaced to the LLM â€” adding a
    ...
    for schema in schemas:
        ...
        # board is optional everywhere â€” never in required.
...
# - Gateway sessions: ZED_SESSION_PLATFORM + ZED_SESSION_CHAT_ID set.
# - TUI sessions: ZED_SESSION_KEY (or ZED_SESSION_ID) set, with
...
def test_create_subscribes_gateway_session(monkeypatch, worker_env):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("ZED_SESSION_PLATFORM", "telegram")
    monkeypatch.setenv("ZED_SESSION_CHAT_ID", "chat-42")
    monkeypatch.setenv("ZED_SESSION_THREAD_ID", "thread-7")
    monkeypatch.setenv("ZED_SESSION_USER_ID", "user-9")
...
def test_create_subscribes_tui_session_via_session_key(monkeypatch, worker_env):
    ...
    local channel), but the parent process exports ZED_SESSION_KEY.
    We should still auto-subscribe, with platform='tui' and
    chat_id=<key>."""
    from tools import kanban_tools as kt
    monkeypatch.delenv("ZED_SESSION_PLATFORM", raising=False)
    monkeypatch.delenv("ZED_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("ZED_SESSION_THREAD_ID", raising=False)
    monkeypatch.delenv("ZED_SESSION_USER_ID", raising=False)
    monkeypatch.setenv("ZED_SESSION_KEY", "tui-session-abc")
    monkeypatch.delenv("ZED_SESSION_ID", raising=False)
...
def test_create_does_not_subscribe_in_cli_session(monkeypatch, worker_env):
    ...
    monkeypatch.delenv("ZED_SESSION_PLATFORM", raising=False)
    monkeypatch.delenv("ZED_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("ZED_SESSION_KEY", raising=False)
    monkeypatch.delenv("ZED_SESSION_ID", raising=False)
...
def test_create_respects_auto_subscribe_on_create_false(monkeypatch, worker_env, tmp_path):
    ...
    concern from PR #19718 (reverted in #19721) â€” users who want
    explicit kanban_notify-subscribe calls per task get that."""
    # worker_env already created <tmp>/.zed; use a fresh sibling
    # home to avoid mkdir() colliding with the worker's directory.
    home = tmp_path / "gate-home" / ".zed"
    home.mkdir(parents=True)
    (home / "config.yaml").write_text(
        "kanban:\n  auto_subscribe_on_create: false\n"
    )
    monkeypatch.setenv("ZED_HOME", str(home))
    monkeypatch.setenv("ZED_SESSION_PLATFORM", "discord")
    monkeypatch.setenv("ZED_SESSION_CHAT_ID", "channel-1")
...
def test_create_partial_session_context_no_subscribe(monkeypatch, worker_env):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("ZED_SESSION_PLATFORM", "slack")
    monkeypatch.delenv("ZED_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("ZED_SESSION_KEY", raising=False)
    monkeypatch.delenv("ZED_SESSION_ID", raising=False)
