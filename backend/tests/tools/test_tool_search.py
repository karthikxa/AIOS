"""Tests for tools/tool_search.py â€” progressive tool disclosure.

Coverage targets â€” these mirror the issues called out in the OpenClaw tool
...
# ---------------------------------------------------------------------------
# Classification â€” the hard invariant: core tools NEVER defer.
# ---------------------------------------------------------------------------
...
class TestClassification:
    def test_core_tools_never_defer(self):
        ...
        # Sample of core tools from _ZED_CORE_TOOLS.
    ...
    def test_classify_keeps_unknown_in_visible(self):
        """A tool we can't classify stays visible â€” never silently dropped.
...
# ---------------------------------------------------------------------------
# Assembly â€” the full passthrough/activate decision.
# ---------------------------------------------------------------------------
...
class TestBridgeDispatch:
    ...
    def test_tool_describe_rejects_non_deferrable(self):
        """If the model asks to describe a core tool, refuse â€” it's already
        in the visible list."""
        from tools.tool_search import dispatch_tool_describe
...
class TestRegression_OpenClawCron84141:
    ...
    resulted in the agent receiving only ``sessions_send`` â€” the catalog
    ...
    def test_core_tool_survives_alongside_many_mcp_tools(self):
        ...
        # Pad with fake "deferrable" tools â€” without registry registration,
...
class TestRegression_ToolsetScoping:
    ...
    def test_search_catalog_is_scoped_to_session_toolsets(self):
        ...
        assert parsed["total_available"] == 12, (
            ...
            "â€” catalog leaked tools outside the session's toolsets"
        )
