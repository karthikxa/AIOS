...
â€” the logic is conditional on a passed-in ``is_windows`` flag, not on
...
``zed_tools.py`` and ``script.py`` into a temp dir, and those files
must be written as UTF-8 on every platform â€” the generated stub contains
...
class TestWindowsEssentialAllowlist:
    """The allowlist itself â€” contents, shape, and invariants."""
    ...
    def test_no_overlap_with_secret_substrings(self):
        ...
        for name in _WINDOWS_ESSENTIAL_ENV_VARS:
            assert not any(s in name for s in _SECRET_SUBSTRINGS), (
                f"{name!r} looks secret-like â€” would be blocked before the "
...
class TestScrubChildEnvWindows:
    ...
    def _sample_windows_env(self):
        ...
        return {
            ...
            "MY_PASSWORD": "hunter2",
            # Not matched by any rule â€” should be dropped on both OSes:
    ...
    def test_essentials_blocked_when_is_windows_false(self):
        """On POSIX hosts, Windows-specific vars should not pass â€” they
...
@pytest.mark.skipif(
    sys.platform != "win32",
    reason="Winsock-specific regression â€” only meaningful on Windows",
)
class TestWindowsSocketSmokeTest:
    ...
    regression that motivated the fix â€” without SYSTEMROOT the child
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    """Independent oracle for TestPosixEquivalence â€” a from-scratch reimpl of
    _scrub_child_env's POSIX behavior, used to prove the production helper does
    what we think it does.

    Deliberately updated for #27303 (the broad ``ZED_`` prefix was dropped
    ...
    visible in review â€” that is what this change is.
    """
...
class TestPosixEquivalence:
    ...
    behavior â€” maybe on purpose, maybe not.  Either way it should land
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # ZED_* handling (#27303): only the operational allowlist passes;
        # every other ZED_* is dropped (the broad prefix was removed).
        "ZED_HOME": "/home/alice/.zed",        # allowlisted â†’ kept
        "ZED_PROFILE": "default",                 # allowlisted â†’ kept
        "ZED_INTERACTIVE": "1",                   # not allowlisted â†’ dropped
        "ZED_BASE_URL": "https://api.internal",   # not allowlisted â†’ dropped
        "ZED_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted â†’ dropped
        # Secret-substring blocks
        "OPENAI_API_KEY": "sk-xxx",
        "GITHUB_TOKEN": "ghp_xxx",
        "AWS_SECRET_ACCESS_KEY": "yyy",
        "MY_PASSWORD": "hunter2",
        "SENTRY_DSN": "https://abc@sentry.io/1",     # DSN substring â†’ blocked
        "SLACK_WEBHOOK": "https://hooks.slack/x",    # WEBHOOK substring â†’ blocked
        # Uncategorized â€” must be dropped
    ...
    def test_posix_behavior_unchanged(self, env_name, env, pt_name, pt):
        """For every combination of (env shape Ã— passthrough rule), the
    ...
    def test_windows_mode_is_strict_superset_of_posix_mode(self):
        ...
        â€” if it did, we'd have broken same-host reuse of the scrubber."""
...
# ---------------------------------------------------------------------------
#
# The sandbox writes two Python files into a temp dir â€” the generated
# ``zed_tools.py`` stub, and the LLM's ``script.py``.  Both contain
# non-ASCII characters in practice: the stub has em-dashes in docstrings
# ("``tcp://host:port`` â€” the parent falls back..."), and user scripts
...
class TestSandboxWritesUtf8:
    ...
    context â€” but the code inspection is deterministic and fast."""

    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``zed_tools.py`` and ``script.py`` writes in
    ...
    def test_file_rpc_stub_uses_utf8(self):
        ...
        from tools.code_execution_tool import generate_zed_tools_module
        stub = generate_zed_tools_module(["terminal"], transport="file")
        # The generated stub should open response + request files as UTF-8.
        assert 'encoding="utf-8"' in stub, (
            "File-based RPC stub does not specify encoding=\"utf-8\" â€” "
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        em-dashes (which it does â€” check the transport-header docstring).
        """
        from tools.code_execution_tool import generate_zed_tools_module
        import tempfile, ast
        stub = generate_zed_tools_module(
        ...
        assert non_ascii, (
            "Generated stub is pure ASCII â€” test is meaningless.  If the "
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        requirement may be obsolete â€” reconsider the fix."""
        from tools.code_execution_tool import generate_zed_tools_module
        import tempfile

        stub = generate_zed_tools_module(["terminal"], transport="uds")
        # Find a non-ASCII character we can use to prove the corruption.
        non_ascii = [c for c in stub if ord(c) > 127]
        if not non_ascii:
            pytest.skip("stub has no non-ASCII chars â€” nothing to corrupt")
        ...
        with tempfile.NamedTemporaryFile(
            ...
            try:
                ...
                # Default encoding can't even encode it â€” that's the bug
            ...
            with open(tmp_path, encoding="utf-8") as fh:
                try:
                    ...
                    pytest.skip(
                        ...
                        "this Windows build â€” explicit encoding= is no "
...
# let the child import zed_tools, a user script that printed non-ASCII
...
# em-dashes, arrows, accented chars, emoji â€” all of which break.
#
...
class TestChildStdioIsUtf8:
    ...
    def test_live_child_can_print_non_ascii(self):
        ...
        Runs on every OS â€” on POSIX the fix is belt-and-suspenders but
        ...
        result = subprocess.run(
            ...
            capture_output=True,
            timeout=15,
            # Don't decode at the subprocess boundary â€” we want to check
    ...
    def test_windows_child_without_utf8_env_would_fail(self):
        ...
        stdio encoding on Windows and the fix may be obsolete â€” but
        ...
        # it anyway â€” in which case the fix is still defensive but no
        ...
        if result.returncode == 0 and b"\xe2\x80\x94" in result.stdout:
            pytest.skip(
                ...
                "without PYTHONIOENCODING/PYTHONUTF8 â€” fix is defensive "
                "but no longer strictly load-bearing.  Keep the env vars "
                "for older Python builds and C.ASCII-locale containers."
            )
        # Otherwise: crash OR garbled output â€” both count as proving the
