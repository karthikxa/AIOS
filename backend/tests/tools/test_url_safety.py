...
class TestNormalizeUrlForRequest:
    def test_percent_encodes_non_ascii_path(self):
        assert (
            normalize_url_for_request("https://wttr.in/KÃ¶ln")
    ...
    def test_preserves_reserved_query_syntax(self):
        assert (
            normalize_url_for_request("https://example.com/search?q=KÃ¶ln&lang=de")
            == "https://example.com/search?q=K%C3%B6ln&lang=de"
        )

    def test_idna_encodes_hostname(self):
        assert (
            normalize_url_for_request("https://mÃ¼nich.example/KÃ¶ln")
...
class TestGlobalAllowPrivateUrls:
    ...
    def test_default_is_false(self, monkeypatch):
        ...
        monkeypatch.delenv("ZED_ALLOW_PRIVATE_URLS", raising=False)
        with patch("zed_cli.config.read_raw_config", side_effect=Exception("no config")):
            assert _global_allow_private_urls() is False

    def test_env_var_true(self, monkeypatch):
        """ZED_ALLOW_PRIVATE_URLS=true enables the toggle."""
        monkeypatch.setenv("ZED_ALLOW_PRIVATE_URLS", "true")
        assert _global_allow_private_urls() is True

    def test_env_var_1(self, monkeypatch):
        """ZED_ALLOW_PRIVATE_URLS=1 enables the toggle."""
        monkeypatch.setenv("ZED_ALLOW_PRIVATE_URLS", "1")
        assert _global_allow_private_urls() is True

    def test_env_var_yes(self, monkeypatch):
        """ZED_ALLOW_PRIVATE_URLS=yes enables the toggle."""
        monkeypatch.setenv("ZED_ALLOW_PRIVATE_URLS", "yes")
        assert _global_allow_private_urls() is True

    def test_env_var_false(self, monkeypatch):
        """ZED_ALLOW_PRIVATE_URLS=false keeps it disabled."""
        monkeypatch.setenv("ZED_ALLOW_PRIVATE_URLS", "false")
        assert _global_allow_private_urls() is False
    ...
    def test_config_security_section(self, monkeypatch):
        ...
        with patch("zed_cli.config.read_raw_config", return_value=cfg):
            assert _global_allow_private_urls() is True
    ...
    def test_result_is_cached(self, monkeypatch):
        ...
        monkeypatch.setenv("ZED_ALLOW_PRIVATE_URLS", "true")
        assert _global_allow_private_urls() is True
        # Change env after first call â€” should still be True (cached)
        monkeypatch.setenv("ZED_ALLOW_PRIVATE_URLS", "false")
        assert _global_allow_private_urls() is True
...
class TestAllowPrivateUrlsIntegration:
    ...
    def test_benchmark_ip_allowed_when_toggle_on(self, monkeypatch):
        ...
        with patch("socket.getaddrinfo", return_value=[
            ...
            assert is_safe_url("https://zedteam.com") is True
...
class TestIsAlwaysBlockedUrl:
    """The always-blocked floor â€” cloud metadata only, narrower than is_safe_url."""
    ...
    @pytest.mark.parametrize("url", [
        ...
        "http://10.0.0.5/",
        "http://172.16.0.1/",
        "http://100.64.0.1/",  # CGNAT â€” blocked by is_safe_url but not by the floor
    ])
    def test_ordinary_private_urls_not_in_floor(self, url):
        """Floor is narrower than is_safe_url â€” ordinary private URLs pass."""
    ...
    def test_empty_url_not_in_floor(self):
        """Empty URL falls through â€” caller decides what to do with a malformed URL."""
...
class TestIPv4MappedIPv6SSRF:
    ...
    # â”€â”€ _is_blocked_ip direct tests â”€â”€
    ...
    # â”€â”€ is_safe_url integration tests: always-blocked metadata IPs â”€â”€
