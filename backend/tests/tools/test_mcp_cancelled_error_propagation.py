...
``task.cancel()``) caused the reconnect loop to exit silently â€” the MCP
server stayed dead until Zed was restarted. See #9930.
...
class TestCancelledErrorPropagation:
    def test_cancelled_error_is_not_swallowed_by_except_exception(self):
        ...
        so the reconnect loop terminates cleanly on cancel â€” not stay wedged."""
        ...
        async def drive():
            with patch.object(MCPServerTask, "_run_stdio", _hanging_run), \
                ...
                try:
                    ...
                    # and stayed wedged â€” the exact #9930 bug.
                    task.cancel()
                    try:
                        await task
                    except Exception:
        ...
        assert outcome in {"cancelled_cleanly", "clean_return"}, (
            f"MCPServerTask.run wedged on cancel (outcome={outcome}) â€” "
            f"#9930 regression"
        )

    def test_shutdown_completes_promptly_when_task_is_cancelled(self):
        """``shutdown()`` falls through to ``task.cancel()`` + ``await self._task``
        after a grace period. That cancel must unwedge the reconnect loop â€”
        ...
        assert done, "MCPServerTask did not finish after cancel â€” #9930 regression"
