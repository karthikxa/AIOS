...
Security-critical invariants â€” env scrubbing, tool whitelist, resource caps â€”
must apply identically in both modes. These tests guard all three layers.

Mode is sourced exclusively from ``code_execution.mode`` in config.yaml â€”
...
def _force_local_terminal(monkeypatch):
    """Mirror test_code_execution.py â€” guarantee local backend under xdist."""
...
class TestGetExecutionMode(unittest.TestCase):
    ...
    def test_none_config_falls_back_to_default(self):
        with patch("tools.code_execution_tool._load_config",
            ...
            # str(None).lower() = "none" â†’ not in EXECUTION_MODES â†’ default
            self.assertEqual(_get_execution_mode(), DEFAULT_EXECUTION_MODE)

    def test_execution_modes_tuple(self):
        """Canonical set of modes â€” tests + config layer rely on this shape."""
...
class TestResolveChildPython(unittest.TestCase):
    """_resolve_child_python â€” picks the right interpreter per mode."""
    ...
    def test_project_with_no_venv_falls_back(self):
        """Project mode without VIRTUAL_ENV or CONDA_PREFIX â†’ sys.executable."""
    ...
    def test_project_with_virtualenv_picks_venv_python(self):
        """Project mode + VIRTUAL_ENV pointing at a real venv â†’ that python."""
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch.dict(os.environ, {"VIRTUAL_ENV": str(fake_venv)}):
                # Clear cache â€” _is_usable_python memoizes on path
    ...
    def test_project_with_broken_venv_falls_back(self):
        """VIRTUAL_ENV set but bin/python missing â†’ sys.executable."""
        import tempfile
        with tempfile.TemporaryDirectory() as td:
            # No bin/python inside â€” broken venv
...
@pytest.mark.skipif(
    ...
    reason=(
        ...
        "privileges.  execute_code itself works on Windows â€” these "
...
class TestExecuteCodeModeIntegration(unittest.TestCase):
    ...
    def test_strict_mode_runs_in_tmpdir(self):
        ...
        self.assertIn("zed_sandbox_", result["output"])
    ...
    def test_project_mode_runs_in_session_cwd(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(result["status"], "success")
            # Resolve symlinks (macOS /tmp â†’ /private/tmp) on both sides
            self.assertEqual(
    ...
    def test_project_mode_interpreter_is_venv_python(self):
        ...
        # The zed-agent venv is always active during tests, so this also
    ...
    def test_project_mode_can_still_import_zed_tools(self):
        """Regression: zed_tools still importable from non-tmpdir CWD.

        This is the PYTHONPATH fix â€” without it, switching to session CWD
        breaks `from zed_tools import terminal`.
        """
        import tempfile
        with tempfile.TemporaryDirectory() as td:
            code = (
    ...
    def test_strict_mode_can_still_import_zed_tools(self):
