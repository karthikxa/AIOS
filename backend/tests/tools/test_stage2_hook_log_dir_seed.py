"""Contract test: the s6-overlay stage2 hook seeds $ZED_HOME/logs/gateways
as the zed user.
...
zed user and s6-log crash-loops on `mkdir: Permission denied`.

Seeding `logs/gateways` in stage2 (cont-init runs before any service starts)
guarantees the parent already exists zed-owned by the time the first
...
def _seed_mkdir_block(text: str) -> str:
    """Extract the `as_zed mkdir -p \\ ...` seed block."""
    m = re.search(r"as_zed mkdir -p \\\n(?:[^\n]*\\\n)*[^\n]*\n", text)
    assert m, "stage2-hook.sh must contain the as_zed mkdir -p seed block"
    return m.group(0)


def test_logs_gateways_is_seeded(stage2_text: str) -> None:
    block = _seed_mkdir_block(stage2_text)
    assert '"$ZED_HOME/logs/gateways"' in block, (
        "logs/gateways must be seeded zed-owned in stage2 so profiles "
    ...
    assert '"$ZED_HOME/logs"' in block


def test_logs_subtree_is_healed_when_chown_needed(stage2_text: str) -> None:
    """The needs_chown repair loop must cover the logs subtree recursively â€”
    ...
    assert "logs" in m.group(1).split(), (
        "the needs_chown loop must recursively chown logs/ â€” it covers "
