...
def fake_zed(tmp_path, monkeypatch):
    """Build a two-profile Zed layout and point ZED_HOME at
    the zed-security profile (matching the original-incident shape).
    """
    ...
    monkeypatch.setattr(fs, "_zed_home_path", lambda: sec_home)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_zed):
        from tools.file_tools import write_file_tool
        target = fake_zed["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_zed):
        """The May 2026 incident â€” security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_zed["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "zed-security" in result["error"]
        # File untouched.
        assert target.read_text() == original

    def test_cross_profile_True_bypass(self, fake_zed):
    ...
    def test_non_zed_path_unaffected(self, fake_zed, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_zed):
    ...
    def test_cross_profile_patch_bypass(self, fake_zed):
        from tools.file_tools import patch_tool
        target = fake_zed["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_zed):
...
# ---------------------------------------------------------------------------
# skill_manage â€” error message naming other profile (item D)
# ---------------------------------------------------------------------------
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_zed, monkeypatch
    ):
        """The original incident shape â€” model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_zed["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up ZED_HOME (set in
        ...
        assert "not found in active profile 'zed-security'" in err
        assert "default" in err
        assert "cross_profile=True" in err

    def test_error_names_multiple_profiles(self, fake_zed, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_zed["root"], "everywhere-skill")
        self._make_skill_in_profile(fake_zed["coder_home"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.zed/profiles/<name>/."""
        # Don't set ZED_HOME â€” falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_zed_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_zed_root_path", lambda: tmp_path / "fake")

        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "default"
        # Build the line manually to pin the contract â€” the prompt builder
        # is too heavy to instantiate end-to-end in a unit test.
        # See agent/system_prompt.py for the exact wording.

    def test_named_profile_line_in_prompt_text(self, fake_zed):
        """When active profile is 'zed-security', the prompt warns
        explicitly about NOT modifying default's skills/plugins/cron/memories."""
        # Spot-check by reading the source â€” the contract is:
        ...
        assert "Active Zed profile" in src
        assert "cross_profile=True" in src
        assert "~/.zed/profiles/" in src
