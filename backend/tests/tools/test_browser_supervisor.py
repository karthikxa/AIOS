...
â€” these are the tests that actually verify the CDP wire protocol
...
Automated: skipped in CI unless ``ZED_E2E_BROWSER=1`` is set.
"""
...
def chrome_cdp(request):
    ...
    profile = tempfile.mkdtemp(prefix="zed-supervisor-test-")
...
def test_browser_cdp_frame_id_real_oopif_smoke_documented():
    """Document that real-OOPIF E2E was manually verified â€” see PR #14540.
    ...
    smoke scripts in /tmp/dialog-iframe-test/:
      ...
      * smoke_local_oopif.py   â€” local Chrome + 2 http servers on
      ...
      * smoke_bb_iframe_agent_path.py â€” Browserbase + real cross-origin
    ...
    pytest.skip(
        ...
        "smoke_bb_iframe_agent_path.py â€” pytest version hits an asyncio "
...
def test_bridge_captures_prompt_and_returns_reply_text(chrome_cdp, supervisor_registry):
    ...
    Proves the bridge isn't just catching dialogs â€” it's properly round-
    ...
    async def nav_and_read():
        async with _ws_mod.connect(cdp_url, max_size=50 * 1024 * 1024) as ws:
            ...
            try:
                ...
                # Fire navigate but don't await â€” prompt() blocks the page
...
def test_evaluate_runtime_dom_node_returns_empty_object(chrome_cdp, supervisor_registry):
    ...
    This is honest â€” DOM nodes can't be deeply JSON-serialized â€” and matches
    ...
    assert out["ok"] is True
    assert out["result_type"] == "object"
    # Empty dict â€” Chrome can't deeply-serialize a DOM node through returnByValue.
    assert out["result"] == {}
