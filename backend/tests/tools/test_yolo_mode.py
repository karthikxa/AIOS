"""Tests for --yolo (ZED_YOLO_MODE) approval bypass."""
...
class TestYoloMode:
    """When ZED_YOLO_MODE is set, all dangerous commands are auto-approved."""
    ...
    def test_dangerous_command_blocked_normally(self, monkeypatch):
        ...
        monkeypatch.setenv("ZED_SESSION_KEY", "test-session")
    ...
    def test_dangerous_command_approved_in_yolo_mode(self, monkeypatch):
        """With ZED_YOLO_MODE, dangerous commands are auto-approved."""
        ...
        # regardless of yolo â€” see test_hardline_blocklist.py.
        result = check_dangerous_command("rm -rf /tmp/stuff", "local")
    ...
    def test_yolo_mode_works_for_all_patterns(self, monkeypatch):
        ...
        # Dangerous but recoverable â€” yolo should bypass.
    ...
    def test_combined_guard_bypasses_yolo_mode(self, monkeypatch):
        ...
        # Non-hardline dangerous command â€” yolo should bypass tirith+dangerous.
    ...
    def test_yolo_mode_not_set_by_default(self):
        """ZED_YOLO_MODE should not be set by default."""
        # Clean env check â€” if it happens to be set in test env, that's fine,
        # we just verify the mechanism exists
        assert os.getenv("ZED_YOLO_MODE") is None or True  # no-op, documents intent

    def test_yolo_mode_empty_string_does_not_bypass(self, monkeypatch):
        """Empty string for ZED_YOLO_MODE should not trigger bypass."""
        monkeypatch.setenv("ZED_YOLO_MODE", "")
        monkeypatch.setenv("ZED_INTERACTIVE", "1")
        monkeypatch.setenv("ZED_SESSION_KEY", "test-session")

        # Empty string is falsy in Python, so getenv("ZED_YOLO_MODE") returns ""
        # which is falsy â€” bypass should NOT activate
    ...
    def test_false_like_yolo_values_do_not_bypass_dangerous_command(self, monkeypatch, value):
        ...
        monkeypatch.setenv("ZED_YOLO_MODE", value)
        monkeypatch.setenv("ZED_INTERACTIVE", "1")
        monkeypatch.setenv("ZED_SESSION_KEY", "test-session")
    ...
    def test_session_scoped_yolo_only_bypasses_current_session(self, monkeypatch):
        ...
        # Dangerous-but-not-hardline â€” the yolo bypass applies here.
