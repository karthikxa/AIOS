...
Covers:
- XAIWebSearchProvider.is_available() â€” cheap probe (env var + auth.json)
- search() â€” JSON happy path, annotation fallback, citations fallback, empty results
- search() error paths â€” HTTP error, request error, missing creds, mutually-exclusive domain filters,
  200-OK error envelope
- Request payload shape â€” model, tools list, allowed_domains/excluded_domains filters
...
class TestXAIProviderIsAvailable:
    """``is_available()`` MUST be cheap â€” no network, no token refresh, no
    auth-store lock. It runs on every ``zed tools`` repaint and at
    ...
    def test_available_via_auth_store(self, monkeypatch, tmp_path):
        """Cheap probe should detect xai-oauth tokens in ~/.zed/auth.json
...
class TestXAIProviderSearchFallbacks:
    ...
    def test_annotations_without_url_citations_fall_through_to_citations(self):
        ...
        consulted â€” otherwise we'd silently report success-with-no-rows
        and mask real data the API provided.
        """
        from plugins.web.xai import provider as xai_provider

        body = "Some narration about xAI."
        # Non-url_citation annotations only â€” the fallback shouldn't extract
...
class TestXAIProviderSearchErrors:
    ...
    def test_401_on_oauth_path_triggers_force_refresh_and_retry(self):
        """OAuth credentials â†’ 401 must force-refresh and retry once.
    ...
    def test_401_on_env_var_path_does_not_retry(self):
        """Env-var (XAI_API_KEY) creds can't be refreshed â€” must not retry."""
        import httpx
        from plugins.web.xai import provider as xai_provider
    ...
    def test_401_retry_gives_up_when_refresh_returns_same_token(self):
        ...
        dead), don't loop â€” surface the 401 to the caller."""
        import httpx
        from plugins.web.xai import provider as xai_provider
    ...
    def test_non_401_http_error_is_not_retried(self):
        """Only 401 is retryable â€” 429 / 500 / 503 must fail fast so the
...
class TestXAIBackendWiring:
    ...
    def test_is_backend_available_does_not_call_resolver(self, monkeypatch):
        """Regression guard â€” `_is_backend_available` runs on every web_search
        dispatch and every `zed tools` repaint. It must not invoke the
    ...
    def test_xai_not_in_legacy_backend_candidate_chain(self, monkeypatch):
        ...
        does not include xAI â€” by design, since the no-config legacy
        chain is for users who set env vars but never ran ``zed tools``,
