...
blocks looking only for ``block.text`` â€” image blocks were silently dropped
...
Zed' existing ``cache_image_from_bytes`` so a ``MEDIA:<path>`` tag
...
def _png_bytes():
    ...
    Zed' ``cache_image_from_bytes`` has a format-sniff guard that rejects
    non-image payloads â€” use a real PNG signature so the test exercises the
...
class TestCacheMcpImageBlock:
    def test_returns_media_tag_for_valid_image_block(self, tmp_path, monkeypatch):
        ...
        # The cached file should be in Zed' image cache dir
        from gateway.platforms.base import get_image_cache_dir
        cache_dir = str(get_image_cache_dir().resolve())
        assert tag.startswith(f"MEDIA:{cache_dir}"), (
            f"cached file not under ZED_HOME image cache dir. "
    ...
    def test_returns_empty_on_malformed_base64(self, tmp_path, monkeypatch):
        """A server that sends garbage base64 shouldn't crash the handler â€”
