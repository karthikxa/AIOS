...
class TestDelegateRequirements(unittest.TestCase):
    ...
    def test_schema_valid(self):
        ...
        # max_iterations is intentionally NOT exposed to the model â€” it's
        # config-authoritative via delegation.max_iterations so users get
        # predictable budgets.
        self.assertNotIn("max_iterations", props)
        self.assertNotIn("maxItems", props["tasks"])  # removed â€” limit is now runtime-configurable
    ...
    def test_build_child_agent_does_not_raise_name_error(self):
        ...
        with patch("run_agent.AIAgent"):
            try:
                ...
                self.fail(
                    f"_build_child_agent raised NameError â€” "
...
class TestSubagentCostRollup(unittest.TestCase):
    """Port of Kilo-Org/kilocode#9448 â€” parent's session_estimated_cost_usd
    ...
    def test_batch_children_costs_sum_into_parent(self):
        ...
        # 0.15 + 0.27 + 0.03 even though one child failed â€” the API calls it
    ...
    def test_zero_cost_children_leave_parent_source_untouched(self):
        ...
        not invent a fake 'subagent' source â€” the parent's 'none' stays."""
        parent = self._make_parent_with_cost_counters(starting_cost=0.00)
...
class TestDelegationCredentialResolution(unittest.TestCase):
    ...
    def test_direct_endpoint_no_raise_when_only_provider_env_key_present(self):
        # Even if OPENAI_API_KEY is absent, no ValueError â€” _build_child_agent uses parent key.
        parent = _make_mock_parent(depth=0)
        cfg = {
            "model": "qwen2.5-coder",
            "base_url": "http://localhost:1234/v1",
    ...
    def test_custom_provider_with_empty_configured_provider_falls_back_to_runtime(self, mock_resolve):
        ...
        creds = _resolve_delegation_credentials(cfg, parent)
        # Empty provider â†’ early return with None (child inherits parent)
        self.assertIsNone(creds["provider"])
...
class TestDelegationProviderIntegration(unittest.TestCase):
    """Integration tests: delegation config â†’ _run_single_child â†’ AIAgent construction."""
    ...
    def test_cross_provider_delegation(self, mock_creds, mock_cfg):
        """Parent on Nous, subagent on OpenRouter â€” full credential switch."""
        mock_cfg.return_value = {
            "max_iterations": 45,
            "model": "google/gemini-3-flash-preview",
            "provider": "openrouter",
        ...
        parent.base_url = "https://inference-api.zedteam.com/v1"
...
class TestChildCredentialPoolResolution(unittest.TestCase):
    ...
    def test_custom_unregistered_endpoint_returns_none(self):
        ...
        must NOT inherit the parent's pool â€” return None so the child keeps its
...
class TestDelegateEventEnum(unittest.TestCase):
    ...
    def test_progress_callback_normalises_thinking(self):
        ...
        assert any("ðŸ’­" in str(c) for c in parent._delegate_spinner.print_above.call_args_list)
    ...
    def test_progress_callback_accepts_new_style_string(self):
        """cb('delegate.task_thinking', ...) â€” the string form of the
        enum value â€” must route to the thinking branch too, so new-style
    ...
    def test_progress_callback_task_progress_not_misrendered(self):
        ...
        cb = _build_child_progress_callback(0, "test goal", parent, task_count=1)
        cb("subagent_progress", tool_name="ðŸ”€ [1] terminal, file")

        # Spinner gets a distinct ðŸ”€-prefixed line, NOT a tool emoji
        # followed by the summary string as if it were a tool name.
        calls = parent._delegate_spinner.print_above.call_args_list
        self.assertTrue(any("ðŸ”€ ðŸ”€ [1] terminal, file" in str(c) for c in calls))
        # Parent callback receives the relay (pass-through, no re-batching).
        parent.tool_progress_callback.assert_called_once()
        # No 'âš¡' tool-start emoji should appear â€” that's the pre-fix bug.
        self.assertFalse(any("âš¡" in str(c) for c in calls))
...
class TestConcurrencyDefaults(unittest.TestCase):
    ...
    def test_no_upper_ceiling(self, mock_cfg):
        """Users can raise concurrency as high as they want â€” no hard cap."""
...
class TestMaxSpawnDepth(unittest.TestCase):
    ...
    def test_max_spawn_depth_no_upper_ceiling(self, mock_cfg):
        """No upper ceiling â€” high values pass through unchanged (cost is the limiter)."""
        from tools.delegate_tool import _get_max_spawn_depth
...
class TestOrchestratorRoleSchema(unittest.TestCase):
    ...
    def test_unknown_role_coerces_to_leaf(self):
        """role='nonsense' â†’ _normalize_role warns and returns 'leaf'."""
        import logging
        with self.assertLogs("tools.delegate_tool", level=logging.WARNING) as cm:
    ...
    def test_acp_command_description_has_no_claude_as_example(self):
        # Descriptions must not list 'claude' as a canonical example value â€”
...
class TestOrchestratorRoleBehavior(unittest.TestCase):
    ...
    def test_orchestrator_role_keeps_delegation_at_depth_1(
        ...
        """role='orchestrator' + depth-0 parent with max_spawn_depth=2 â†’
    ...
    def test_orchestrator_blocked_at_default_flat_depth(
        ...
        the floor â€” the role degrades to 'leaf' and the delegation
    ...
    # â”€â”€ Role-aware system prompt â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def test_orchestrator_prompt_at_depth_floor_says_children_are_leaves(self):
        ...
        own children would be at depth 2 (the floor) â†’ must be leaves."""
        prompt = _build_child_system_prompt(
    ...
    # â”€â”€ Batch mode and intersection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    @patch("tools.delegate_tool._resolve_delegation_credentials")
    @patch("tools.delegate_tool._load_config",
           return_value={"max_spawn_depth": 2})
    def test_batch_mode_per_task_role_override(self, mock_cfg, mock_creds):
        """Per-task role beats top-level; no top-level role â†’ "leaf".

        tasks=[{role:'orchestrator'},{role:'leaf'},{}] â†’ first gets
        ...
        with patch("run_agent.AIAgent", side_effect=_factory):
            delegate_task(
                tasks=[
                    ...
                    {"goal": "B", "role": "leaf"},
                    {"goal": "C"},  # no role â†’ falls back to top_role (leaf)
                ],
                parent_agent=parent,
            )
    ...
    def test_intersection_preserves_delegation_bound(
        ...
        orchestrator child â€” the re-add in _build_child_agent runs
...
class TestOrchestratorEndToEnd(unittest.TestCase):
    ...
    that keys on the child's ephemeral_system_prompt â€” orchestrator
...
class TestSubagentApprovalCallback(unittest.TestCase):
    ...
    Governed by delegation.subagent_auto_approve:
      false (default) â†’ _subagent_auto_deny
      true            â†’ _subagent_auto_approve
    """
    ...
    def test_executor_initializer_installs_callback_in_worker(self):
        ...
        not the parent's â€” verifies the fix actually scopes to workers.
        """
