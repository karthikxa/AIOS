"""Tests for MCP stability fixes â€” event loop handler, PID tracking, shutdown robustness."""
...
class TestStdioPidTracking:
    ...
    def test_kill_orphaned_uses_sigkill_when_available(self, monkeypatch):
        ...
        # ``gateway.status._pid_exists`` (so it's safe on Windows â€” see
...
class TestStdioPgroupReaping:
    ...
    def test_grandchild_reaped_via_pgroup(self, tmp_path):
        ...
        parent_script.write_text(
            ...
            # Parent exits â€” grandchild reparents to init.
        )
        ...
        # Grandchild should be gone â€” SIGTERM via killpg in phase 1 reached it.
...
class TestMCPReloadTimeout:
    ...
    def test_reload_timeout_does_not_block_forever(self, tmp_path, monkeypatch):
        ...
        # Create a mock ZedCLI-like object with the needed attributes
        class FakeCLI:
            _config_mtime = 0.0
            _config_mcp_servers = {}
            ...
            def _reload_mcp(self):
                # Simulate a hang â€” sleep longer than the timeout
                time.sleep(60)
        ...
        source = inspect.getsource(ZedCLI._check_config_mcp_changes)
    ...
    def test_initial_connect_retry_succeeds_on_second_attempt(self):
        ...
        async def _run():
            ...
            async def fake_run_stdio(self_inner, config):
                ...
                # Second attempt: success â€” set ready and "run" until shutdown
