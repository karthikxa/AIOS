...
def _make_voice_cli(**overrides):
    """Create a minimal ZedCLI with only voice-related attrs initialized.
...
# ============================================================================
# Markdown stripping â€” import real function from tts_tool
# ============================================================================
...
class TestMarkdownStripping:
    ...
    def test_long_text_not_truncated(self):
        """_strip_markdown_for_tts does NOT truncate â€” that's the caller's job."""
        text = "a" * 5000
        result = _strip_markdown_for_tts(text)
...
class TestVprintForceParameter:
    ...
    def test_error_messages_use_force_in_run_agent(self):
        ...
        # streaming suppression â€” every âŒ-prefixed _vprint must pass force=True.
    ...
    def test_generate_edge_tts_calls_lazy_import(self):
        ...
        for node in _ast.walk(tree):
            if isinstance(node, _ast.AsyncFunctionDef) and node.name == "_generate_edge_tts":
                ...
                assert bare_refs == [], (
                    f"_generate_edge_tts uses bare 'edge_tts' name â€” "
...
class TestDisableVoiceModeStopsTTS:
    ...
    def test_disable_voice_mode_calls_stop_playback(self):
        ...
        source = inspect.getsource(ZedCLI._disable_voice_mode)
        ...
        for line in lines:
            if "def _show_voice_status" in line:
                ...
                assert 'Record key: Ctrl+B"' not in line, (
                    "_show_voice_status hardcodes 'Ctrl+B' â€” "
...
class TestBrowserToolSignalHandlerRemoved:
    ...
    sys.exit() â€” this conflicts with prompt_toolkit's event loop and causes
...
class TestKeyHandlerNeverBlocks:
    ...
    def test_start_recording_not_called_directly_in_handler(self):
        ...
        directly â€” it must wrap it in a Thread to avoid blocking the UI."""
        import ast as _ast
                ...
                for child in _ast.iter_child_nodes(node):
                    ...
                    if isinstance(child, _ast.Expr) and isinstance(child.value, _ast.Call):
                        ...
                        assert "_voice_start_recording" not in call_src, (
                            ...
                            "â€” must dispatch to a daemon thread"
                        )
                break
...
# ============================================================================
# Real behavior tests â€” CLI voice methods via _make_voice_cli()
# ============================================================================
...
class TestEnableVoiceModeReal:
    ...
    @patch("cli._cprint")
    @patch("zed_cli.config.load_config", return_value={"voice": {}})
    ...
    @patch("cli._cprint")
    @patch("zed_cli.config.load_config", return_value={"voice": {"auto_tts": True}})
    @patch("tools.voice_mode.check_voice_requirements",
           return_value={"available": True, "details": "OK"})
    @patch("tools.voice_mode.detect_audio_environment",
           return_value={"available": True, "warnings": []})
    ...
    @patch("cli._cprint")
    @patch("zed_cli.config.load_config", side_effect=Exception("broken config"))
    @patch("tools.voice_mode.check_voice_requirements",
           return_value={"available": True, "details": "OK"})
    @patch("tools.voice_mode.detect_audio_environment",
           return_value={"available": True, "warnings": []})
...
class TestVoiceBeepConfigReal:
    ...
    @patch("zed_cli.config.load_config", return_value={"voice": {"beep_enabled": False}})
...
class TestVoiceStopAndTranscribeReal:
    ...
    @patch("cli.os.unlink")
    @patch("cli.os.path.isfile", return_value=True)
    @patch("zed_cli.config.load_config", return_value={"stt": {}})
    ...
    @patch("cli.os.unlink")
    @patch("cli.os.path.isfile", return_value=True)
    @patch("zed_cli.config.load_config", return_value={"stt": {"model": "whisper-large-v3"}})
    @patch("tools.voice_mode.transcribe_recording",
