"""Tests for tools/skills_hub.py â€” source adapters, lock file, taps, dedup logic."""
...
class TestParseFrontmatterQuick:
    ...
    def test_nested_yaml(self):
        content = "---\nname: test\nmetadata:\n  zed:\n    tags: [a, b]\n---\n\nBody.\n"
        fm = GitHubSource._parse_frontmatter_quick(content)
        assert fm["metadata"]["zed"]["tags"] == ["a", "b"]
...
class TestSkillsShGroupings:
    ...
    mechanism behind NVIDIA-style categorization â€” not NVIDIA-specific.
    """
...
class TestTrustLevelFor:
    ...
    def test_two_part_identifier(self):
        ...
        # No path part â€” still resolves repo correctly
    ...
    def test_nvidia_skills_tap_is_registered_and_trusted(self):
        ...
        # browseable/searchable through `zed skills browse` must also
    ...
    def test_browseable_trusted_repos_have_taps(self):
        ...
        # is the deliberate exception â€” it has two taps (`.curated/` and
        # `.system/`) â€” so we just assert membership not path equality.
        from tools.skills_guard import TRUSTED_REPOS
        ...
        for repo in TRUSTED_REPOS:
            assert repo in tap_repos, (
                ...
                "from GitHubSource.DEFAULT_TAPS â€” its skills will not be "
                "browsable via `zed skills browse`."
            )
    ...
    def test_fetch_falls_back_to_tree_search_for_deeply_nested_skills(
        ...
        def _httpx_get_side_effect(url, **kwargs):
            ...
            if url.endswith("/contents/"):
                # Root listing for shallow scan â€” return empty so it falls through
                resp.status_code = 200
                resp.json = lambda: []
                return resp
            if "/contents/" in url:
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  <url><loc>https://www.skills.sh/anthropics/skills/frontend-design</loc></url>
        ...
        # Homepage was NOT fetched â€” the sitemap path is taken on empty query.
...
class TestUrlSource:
    ...
    # â”€â”€ _matches â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ inspect â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    @patch("tools.skills_hub.httpx.get")
    ...
    def test_inspect_returns_none_when_url_not_md(self, mock_get):
        # _matches filters first â€” no HTTP call.
    ...
    def test_fetch_falls_back_to_url_directory_name(self, mock_get):
        # Frontmatter has no ``name:`` â€” we slug from the URL directory.
        mock_get.return_value = MagicMock(
            status_code=200,
            text="---\ndescription: No name.\n---\n\n# Body\n",
        )
    ...
    def test_fetch_awaiting_name_when_unresolvable(self, mock_get):
        ...
        # File content still present â€” CLI will reuse it after picking a name.
        assert bundle.files["SKILL.md"].startswith("---\n")

    @patch("tools.skills_hub.httpx.get")
    def test_fetch_awaiting_name_rejects_sentinel_slug(self, mock_get):
        # Frontmatter has no name AND the URL filename slug is ``README`` â€”
    ...
    # â”€â”€ _is_valid_skill_name â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestUnifiedSearchDedup:
    ...
    def test_dedup_keeps_first_seen(self):
        # Same identifier from two sources â€” only the first (community) is kept when equal trust.
    ...
    def test_dedup_prefers_trusted_over_community(self):
        # Same identifier â€” trusted wins over community.
...
# ---------------------------------------------------------------------------
# GitHubSource._download_directory â€” tree API + fallback (#2940)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Install-path safety (lock-file â†’ uninstall rmtree boundary)
# ---------------------------------------------------------------------------


class TestInstallPathSafety:
    """Guard the lock-file â†’ ``uninstall_skill`` rmtree path.
    ...
    def test_uninstall_rejects_empty_install_path(self, tmp_path, isolated_skills_dir, patch_lock_file):
        """Empty install_path resolves to SKILLS_DIR itself â€” must be refused."""
        from tools.skills_hub import uninstall_skill
...
# ---------------------------------------------------------------------------
# parallel_search_sources â€” overall_timeout must be honoured even when a
