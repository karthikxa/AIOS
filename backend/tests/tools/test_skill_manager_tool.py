"""Tests for tools/skill_manager_tool.py â€” skill creation, editing, and deletion."""
...
def _skill_dir(tmp_path):
    ...
    only the temp directory â€” not the real ~/.zed/skills/."""
...
# ---------------------------------------------------------------------------
# _validate_file_path â€” path traversal prevention
# ---------------------------------------------------------------------------
...
class TestDeleteSkill:
    ...
    def test_delete_with_absorbed_into_empty_string_means_pruned(self, tmp_path):
        ...
        assert result["success"] is True
        # Empty absorbed_into is explicit prune â€” no "absorbed into" suffix in message
    ...
    def test_delete_with_absorbed_into_whitespace_only_treated_as_prune(self, tmp_path):
        # Leading/trailing whitespace only: .strip() â†’ "" â†’ pruned path
        with _skill_dir(tmp_path):
            _create_skill("narrow", VALID_SKILL_CONTENT)
    ...
    def test_delete_without_absorbed_into_backward_compat(self, tmp_path):
        # Legacy callers that don't pass the arg still work â€” the curator
...
class TestSkillManageDispatcher:
    ...
    def test_full_create_via_dispatcher(self, tmp_path):
        ...
        result = json.loads(raw)
        assert result["success"] is True
        # No provenance marker on a foreground create â€” record either missing
...
class TestSecurityScanGate:
    ...
    def test_scan_runs_when_flag_on(self, tmp_path):
        ...
        # Fake a safe scan result â€” caller should return None (allow)
    ...
    def test_scan_blocks_dangerous_when_flag_on(self, tmp_path):
        """Dangerous verdict + flag on â†’ returns an error string for the agent."""
        from tools.skill_manager_tool import _security_scan_skill
    ...
    def test_guard_flag_reads_config_default_false(self):
        ...
        with patch("zed_cli.config.load_config", return_value={"skills": {}}):
    ...
    def test_guard_flag_reads_config_when_set(self):
        ...
        with patch("zed_cli.config.load_config",
    ...
    def test_guard_flag_handles_config_error(self):
        ...
        with patch("zed_cli.config.load_config", side_effect=RuntimeError("boom")):
            assert _guard_agent_created_enabled() is False
...
# ---------------------------------------------------------------------------
# External skills directories (skills.external_dirs) â€” mutations in place
# ---------------------------------------------------------------------------
...
class TestExternalSkillMutations:
    ...
    under skills.external_dirs â€” in place, without duplicating to local.
    ...
    caused agents to create duplicate copies in ~/.zed/skills/ as a
    workaround.
    """
    ...
    def test_create_still_writes_to_local_root(self, tmp_path):
        ...
        external_dirs â€” create is unchanged by this PR."""
        local = tmp_path / "local"
        external = tmp_path / "vault"
        local.mkdir(); external.mkdir()
...
# ---------------------------------------------------------------------------
# Pinned-skill guard â€” skill_manage refuses only `delete` on pinned skills.
# Patches and edits go through so pinned skills can still evolve as pitfalls
# come up. The user unpins via `zed curator unpin <name>` to delete.
# ---------------------------------------------------------------------------
...
class TestPinnedGuard:
    ...
    def test_edit_allowed_when_pinned(self, tmp_path):
        """Pin does NOT block edit â€” agent can still improve pinned skills."""
        with _skill_dir(tmp_path):
            _create_skill("my-skill", VALID_SKILL_CONTENT)
    ...
    def test_delete_refuses_pinned(self, tmp_path):
        """Delete is the one action pin still blocks â€” it's the irrecoverable one."""
        with _skill_dir(tmp_path):
            _create_skill("my-skill", VALID_SKILL_CONTENT)
            with self._pin("my-skill"):
                result = _delete_skill("my-skill")
        ...
        assert "zed curator unpin my-skill" in result["error"]
        # Skill still exists
        assert (tmp_path / "my-skill" / "SKILL.md").exists()
...
# ---------------------------------------------------------------------------
# _delete_skill â€” recursive-delete safety (port of Kilo Code #11240)
# ---------------------------------------------------------------------------
...
class TestDeleteSkillRmtreeGuard:
    ...
    def test_symlinked_skill_dir_refused(self, tmp_path):
        """A skill dir that is a symlink must not be rmtree'd â€” rmtree would
    ...
    def test_skills_root_itself_refused(self, tmp_path):
        """If discovery ever hands back the skills root, refuse â€” rmtree would
