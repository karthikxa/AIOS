...
SSRF checks entirely â€” the agent already has full local-network access via
the terminal tool.
...
class TestPreNavigationSsrf:
    ...
    def test_local_allows_private_url(self, monkeypatch, _common_patches):
        """Local backends skip SSRF â€” private URLs are always allowed."""
    ...
    def test_cloud_blocks_imds_even_when_routing_to_local_sidecar(
        ...
        # main pre-fix â€” cloud provider configured, _url_is_private â†’ True,
    ...
    def test_cloud_allows_ordinary_private_url_via_sidecar(
        ...
        """Hybrid routing still works for ordinary private URLs â€” floor
...
class TestIsLocalBackend:
    ...
    def test_no_cloud_provider_is_local(self, monkeypatch):
        """No cloud provider configured â†’ local backend."""
        monkeypatch.setattr(browser_tool, "_is_camofox_mode", lambda: False)
    ...
    def test_cloud_provider_is_not_local(self, monkeypatch):
        """Cloud provider configured and not Camofox â†’ NOT local."""
        monkeypatch.setattr(browser_tool, "_is_camofox_mode", lambda: False)
...
class TestPostRedirectSsrf:
    ...
    def test_cloud_blocks_redirect_to_imds_even_via_sidecar(
        ...
        routing â€” even the hybrid local sidecar path can't return IMDS
