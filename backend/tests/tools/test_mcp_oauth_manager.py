...
def test_zed_provider_subclass_exists():
    """ZedMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _ZED_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _ZED_PROVIDER_CLS is not None
    assert issubclass(_ZED_PROVIDER_CLS, OAuthClientProvider)
...
async def test_disk_watch_invalidates_on_mtime_change(tmp_path, monkeypatch):
    ...
    # _initialized flipped â€” next async_auth_flow will re-read from disk
    assert provider._initialized is False


def test_manager_builds_zed_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns ZedMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ZED_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ZED_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _ZED_PROVIDER_CLS is not None
    assert isinstance(provider, _ZED_PROVIDER_CLS)
    assert provider._zed_server_name == "srv"
