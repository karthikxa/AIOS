...
and the shared slash-command handlers (zed_cli/write_approval_commands.py).
...
@pytest.fixture
def zed_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="zed_wa_test_")
    home = os.path.join(d, ".zed")
    os.makedirs(home)
    monkeypatch.setenv("ZED_HOME", home)
    yield home
...
def test_default_gate_is_off(zed_home):
    from tools import write_approval as wa
    # Default: gate off â†’ writes flow freely.
    assert wa.write_approval_enabled("memory") is False
    assert wa.write_approval_enabled("skills") is False


def test_invalid_subsystem_is_off(zed_home):
    from tools import write_approval as wa
...
def test_normalize_enabled_coerces_values():
    ...
    # Truthy strings â†’ True (incl. legacy 'approve').
    assert wa._normalize_enabled("on") is True
    assert wa._normalize_enabled("approve") is True
    assert wa._normalize_enabled("true") is True
    # Everything else â†’ False (gate off is the safe default).
...
def test_memory_gate_off_allows_write(zed_home):
    # Default (gate off) â†’ write straight through, no staging.
...
def test_memory_gate_on_no_interactive_stages(zed_home):
    # Gate on, no approval callback / not a gateway context â†’ stage.
    from tools.memory_tool import memory_tool, MemoryStore
    from tools import write_approval as wa
...
def test_memory_gate_on_then_apply(zed_home):
...
def test_skill_gate_off_allows_create(zed_home):
    # Default (gate off) â†’ skill is created normally, not staged.
    import importlib
...
def test_skill_gate_on_always_stages(zed_home):
...
def test_skill_gate_on_then_apply_writes_file(zed_home):
    # SKILLS_DIR is resolved at import time, so reload the skill module under
    # this test's ZED_HOME to exercise the real on-disk write path.
    import importlib
    import tools.skill_manager_tool as smt
    importlib.reload(smt)
    from tools import write_approval as wa
...
def test_skill_create_diff_is_full_content(zed_home):
    from tools.skill_manager_tool import skill_manage
    from tools import write_approval as wa
    _set_approval("skills", True)
...
def test_pending_store_roundtrip(zed_home):
    from tools import write_approval as wa
...
# ---------------------------------------------------------------------------
# Inline (interactive CLI) approval path â€” regression for the bug where the
...
def test_memory_inline_approve_writes(zed_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(zed_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_callback_error_stages(zed_home, approval_callback_cleanup):
    # If the prompt machinery fails, fall back to staging â€” never drop silently.
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_gateway_context_stages_not_prompts(zed_home, monkeypatch):
...
def test_skills_never_prompt_inline_even_with_callback(zed_home, approval_callback_cleanup):
    # Skills always stage â€” even when an interactive callback is registered.
    from tools.skill_manager_tool import skill_manage
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("skills", True)
...
def test_memory_invalid_params_rejected_before_staging(zed_home):
