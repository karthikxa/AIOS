...
class TestFileDedup(unittest.TestCase):
    ...
    def setUp(self):
        _read_tracker.clear()
        self._tmpdir = _make_safe_tempdir("zed-dedup-")
    ...
    def test_write_rejects_status_text_with_small_framing(self, mock_ops):
        ...
        Real-world corruption shapes aren't always the verbatim message â€” the
    ...
    def test_write_allows_large_file_that_quotes_status_text(self, mock_ops):
        ...
        Zed' own docs / SKILL.md files may legitimately mention the dedup
        message verbatim.  Only short, status-dominated writes are rejected â€”
        ...
        # so the status doesn't "dominate" â€” this is a legitimate file.
        large_content = (
            "# Skill reference\n\n"
...
class TestDedupStubLoopGuard(unittest.TestCase):
    ...
    of ``read_file â†’ stub â†’ read_file â†’ stub â†’ ...``"""
    ...
    def test_third_read_is_blocked(self, mock_ops):
        """read â†’ stub â†’ BLOCKED.  Second stub escalates to hard error."""
        mock_ops.return_value = _make_fake_ops(
            content="line one\nline two\n", file_size=20,
        )
        # 1. Real read â€” full content
        ...
        # 3. Dedup stub (second hit) â€” escalates to BLOCKED
    ...
    def test_file_modification_clears_block(self, mock_ops):
        """Real file change should break out of the block â€” new content
    ...
    def test_other_tool_call_clears_hits(self, mock_ops):
        ...
        # Agent did something else â€” e.g. terminal, write_file â€” so the
    ...
    def test_different_ranges_tracked_independently(self, mock_ops):
        ...
        # Different range â€” fresh read, should go through
    ...
    def test_reset_file_dedup_clears_hits(self, mock_ops):
        ...
        # Fresh session â€” real read, no stub, no block
        r4 = json.loads(read_file_tool(self._tmpfile, task_id="loop"))
        self.assertNotIn("error", r4)
        self.assertNotIn("dedup", r4)
...
class TestDedupResetOnCompression(unittest.TestCase):
    ...
    def test_reset_clears_dedup(self, mock_ops):
        ...
        # First read â€” populates dedup cache
        ...
        # Read again â€” should get full content
    ...
    def test_reset_all_tasks(self, mock_ops):
        ...
        reset_file_dedup()  # no task_id â€” clear all
    ...
    def test_reset_preserves_loop_detection(self, mock_ops):
        ...
        read_file_tool(self._tmpfile, task_id="loop")
        # 2nd read is deduped â€” doesn't increment consecutive counter
        read_file_tool(self._tmpfile, task_id="loop")

        reset_file_dedup("loop")

        # 3rd read â€” counter should still be at 2 from before reset
        ...
        r3 = json.loads(read_file_tool(self._tmpfile, task_id="loop"))
        # Should NOT be blocked or warned â€” counter restarted since dedup
...
class TestConfigOverride(unittest.TestCase):
    ...
    @patch("tools.file_tools._get_file_ops")
    @patch("zed_cli.config.load_config", return_value={"file_read_max_chars": 50})
    ...
    @patch("tools.file_tools._get_file_ops")
    @patch("zed_cli.config.load_config", return_value={"file_read_max_chars": 500_000})
...
class TestWriteInvalidatesDedup(unittest.TestCase):
    ...
    cache for the written path.  Without this, a readâ†’writeâ†’read sequence
    within the same mtime second returns a stale 'File unchanged' stub.

    Regression test for https://github.com/NousResearch/zed-agent/issues/13144
    """

    def setUp(self):
        _read_tracker.clear()
        self._tmpdir = _make_safe_tempdir("zed-write-dedup-")
    ...
    def test_write_invalidates_dedup_same_second(self, mock_ops):
        """readâ†’writeâ†’read within the same mtime second returns fresh content.

        This is the core #13144 scenario: on filesystems with â‰¥1ms mtime
        ...
        # 1. Read â€” populates dedup cache.
        r1 = json.loads(read_file_tool(self._tmpfile, task_id="wr"))
        self.assertNotEqual(r1.get("dedup"), True)

        # 2. Write â€” must invalidate dedup for this path.
        #    (No sleep â€” we intentionally stay in the same mtime second.)
        write_file_tool(self._tmpfile, "new content\n", task_id="wr")

        # 3. Read again â€” should get full content, NOT dedup stub.
        fake.read_file = lambda path, offset=1, limit=500: _FakeReadResult(
    ...
    def test_write_invalidates_all_offsets(self, mock_ops):
        ...
        # Write â€” should invalidate BOTH dedup entries.
    ...
    def test_write_does_not_invalidate_other_tasks(self, mock_ops):
        ...
        # Task B still sees dedup (its cache is separate â€” the file
