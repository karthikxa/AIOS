"""Regression tests for ZED_HOME override propagation onto the MCP loop.

Tasks scheduled via run_coroutine_threadsafe are created inside the MCP
event-loop thread, so they copy THAT thread's context â€” not the scheduling
...
get_zed_home() inside the coroutine, most visibly OAuth token-store
...
def test_override_propagates_to_mcp_loop(tmp_path, monkeypatch, mcp_loop):
    ...
    process_home.mkdir()
    profile_home.mkdir()
    monkeypatch.setenv("ZED_HOME", str(process_home))
    ...
    token = set_zed_home_override(str(profile_home))
    try:
...
def test_oauth_token_paths_follow_override(tmp_path, monkeypatch, mcp_loop):
    """The actual symptom path: ZedTokenStorage resolving inside the
    ...
    async def token_path():
        ...
        return str(ZedTokenStorage("probe-srv")._tokens_path())
...
def test_concurrent_scopes_do_not_interfere(tmp_path, monkeypatch, mcp_loop):
    ...
    loop must each see their own home â€” the wrapper is task-local."""
    import threading
    ...
    def scoped_call(key, home):
        token = set_zed_home_override(str(home))
        try:
...
def test_wrap_is_noop_without_override(mcp_loop):
    """No active override â†’ the coroutine passes through unwrapped."""
