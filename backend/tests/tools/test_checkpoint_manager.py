"""Tests for tools/checkpoint_manager.py â€” CheckpointManager (v2 single-store)."""
...
def checkpoint_base(tmp_path):
    """Isolated checkpoint base â€” never writes to ~/.zed/."""
...
class TestStoreInit:
    ...
    def test_bc_init_shadow_repo_shim(self, work_dir, checkpoint_base, monkeypatch):
        ...
        assert err is None
        assert (store / "HEAD").exists()
        assert (store / "ZED_WORKDIR").exists()
    ...
    def test_legacy_migration_archives_prev2_repos(
        ...
        (fake_repo / "ZED_WORKDIR").write_text(str(work_dir) + "\n")
        (fake_repo / "objects").mkdir()

        # Init store â€” should migrate the fake pre-v2 repo
...
# =========================================================================
# CheckpointManager â€” taking checkpoints
# =========================================================================
...
class TestRealPruning:
    def test_max_snapshots_trims_history(self, work_dir, checkpoint_base, monkeypatch):
        ...
        # Newest first â€” step-5, step-4, step-3
...
# =========================================================================
# CheckpointManager â€” working dir resolution
# =========================================================================
...
class TestTouchProjectMalformedMeta:
    ...
    ``TypeError: list indices must be integersâ€¦``.  This TypeError propagates
    uncaught out of ``_touch_project`` and up through ``_take`` into
    ``ensure_checkpoint``, where it is swallowed by the broad ``except
    Exception`` safety net â€” but the effect is that the checkpoint is silently
...
def _seed_legacy_repo(base: Path, name: str, workdir: Path, mtime: float = None) -> Path:
    ...
    (shadow / "ZED_WORKDIR").write_text(str(workdir) + "\n")
    (shadow / "info").mkdir()
