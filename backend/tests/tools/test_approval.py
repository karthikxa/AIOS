...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("zed_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("zed_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
def _clear_session(key):
    """Replace for removed clear_session() â€” directly clear internal state."""
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"ZED_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_zed_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.zed/.env")
        assert dangerous is True
        assert key is not None
    ...
    def test_tee_custom_zed_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $ZED_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_zed_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$ZED_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestZedConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.zed/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.zed/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.zed/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.zed/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.zed/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.zed/config.yaml")
        assert dangerous is True
        assert "zed config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.zed/config.yaml")
        assert dangerous is True

    def test_sed_in_place_absolute_zed_home_config(self):
    ...
    def test_sed_in_place_absolute_zed_home_env(self):
        env_path = get_zed_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_custom_zed_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $ZED_HOME/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.zed/config.yaml"
        )
        assert dangerous is True
        assert "in-place" in desc.lower() or "perl" in desc.lower()

    def test_perl_in_place_absolute_zed_home_config(self):
        config_path = get_zed_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.zed/config.yaml"
        )
        assert dangerous is True

    def test_ruby_in_place_absolute_zed_home_env(self):
        env_path = get_zed_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.zed/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.zed/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.zed/config.yaml"
        )
        assert dangerous is True

    def test_perl_eval_no_inplace_safe(self):
        # `perl -e` with no -i flag is code evaluation, not file mutation â€”
        # the perl/ruby -i pattern must not fire on it.
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.zed/config.yaml"
        )
        assert dangerous is False

    def test_read_is_safe(self):
        # Reading config is not a write â€” must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.zed/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-Zed config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_zed_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $ZED_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.zed/config.yaml/.env must require approval â€” the
    ...
    def test_cp_to_zed_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.zed/config.yaml")
        assert dangerous is True
...
class TestPatternKeyUniqueness:
    ...
    def test_find_exec_rm_and_find_delete_have_different_keys(self):
        ...
        assert key_exec != key_delete, (
            f"find -exec rm and find -delete share key {key_exec!r} â€” "
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.zed/zed-agent && source venv/bin/activate && python -m zed_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m zed_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m zed_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "zed_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart zed-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_pkill_zed_detected(self):
        """pkill targeting zed/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_zed_detected(self):
        cmd = "killall zed"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestNormalizationBypass:
    ...
    def test_fullwidth_safe_command_not_flagged(self):
        """Fullwidth 'ï½Œï½“ -ï½Œï½' is safe and must not be flagged."""
        cmd = "\uff4c\uff53 -\uff4c\uff41 /tmp"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False
...
class TestHeredocScriptExecution:
    ...
    def test_python3_heredoc_detected(self):
        ...
        # The heredoc pattern also matches â€” either detection is correct.
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep zed) bypasses the pkill/killall name-matching
    ...
    def test_pkill_zed_still_detected(self):
...
class TestGitDestructiveOps:
    ...
    def test_git_branch_lowercase_d_also_flagged(self):
        """git branch -d triggers approval too â€” IGNORECASE is global.
...
class TestChmodExecuteCombo:
    ...
    def test_chmod_semicolon_execute_detected(self):
        ...
        dangerous, _, _ = detect_dangerous_command(cmd)
        # Semicolon variant â€” pattern uses && but full-string match
...
class TestKillallKillSignals:
    ...
    def test_killall_list_signals_is_safe(self):
        """`killall -l` lists signals and is harmless â€” must not fire."""
...
class TestEtcPatternsUnaffectedByRefactor:
    ...
    def test_cat_etc_hostname_is_safe(self):
        """Reading /etc/ files is safe â€” only writes require approval."""
...
#
# These tests pin:
#   1. Gateway timeout â†’ approved=False, with a message strong enough that
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("ZED_GATEWAY_SESSION", "ZED_CRON_SESSION",
                      "ZED_YOLO_MODE",
                      "ZED_SESSION_KEY", "ZED_INTERACTIVE")
        }
        os.environ.pop("ZED_YOLO_MODE", None)
        os.environ.pop("ZED_INTERACTIVE", None)
        # ZED_CRON_SESSION takes priority over ZED_GATEWAY_SESSION in
        ...
        os.environ.pop("ZED_CRON_SESSION", None)
        os.environ["ZED_GATEWAY_SESSION"] = "1"
        os.environ["ZED_SESSION_KEY"] = self.SESSION_KEY
    ...
    def test_timeout_returns_approved_false_with_no_consent(self, monkeypatch):
        """The reported #24912 scenario â€” user never responds, agent must see BLOCKED."""
        from tools import approval as mod
        ...
        # The notify_cb DID fire â€” we did try to ask the user.
    ...
    def test_timeout_message_is_emphatic_against_retry_and_rephrase(self, monkeypatch):
        ...
        msg = result["message"]
        # Explicit halt signals â€” these are the model-facing contract.
    ...
    def test_explicit_deny_carries_same_no_consent_shape(self):
        """An explicit /deny must produce the same shape as timeout â€”
