...
class TestExpandParentToolsets(unittest.TestCase):
    ...
    def test_composite_zed_cli_expands_web(self):
        """zed-cli includes web_search/web_extract â†’ 'web' should be in expansion."""
        expanded = _expand_parent_toolsets({"zed-cli"})
        self.assertIn("web", expanded)
        self.assertIn("terminal", expanded)
        self.assertIn("browser", expanded)
        # Original composite is preserved
        self.assertIn("zed-cli", expanded)
    ...
    def test_intersection_with_expanded_composite(self):
        """End-to-end: requesting ['web'] from parent with ['zed-cli'] yields ['web']."""
        parent_toolsets = {"zed-cli"}
