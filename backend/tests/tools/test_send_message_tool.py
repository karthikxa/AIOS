...
# python-telegram-bot is an optional dep â€” skip the entire module when
...
async def _send_discord(
    ...
    *,
    ...
    """Pre-migration ``(token, chat_id, message, â€¦)`` adapter around the
    plugin's ``_standalone_send(pconfig, â€¦)``.  Lets test bodies continue
...
class TestSendMessageTool:
    ...
    def test_ntfy_topic_target_bypasses_channel_directory(self):
        ...
        ntfy_cfg = SimpleNamespace(enabled=True, token=None, extra={"topic": "zed-in"})
    ...
    def test_cron_duplicate_target_is_skipped_and_explained(self):
        ...
        with patch.dict(
            os.environ,
            {
                "ZED_CRON_AUTO_DELIVER_PLATFORM": "telegram",
                "ZED_CRON_AUTO_DELIVER_CHAT_ID": "-1001",
            },
            clear=False,
        ), \
...
class TestSendToPlatformChunking:
    ...
    def test_slack_messages_are_formatted_before_send(self, monkeypatch):
        ...
        with patch("tools.send_message_tool._send_slack", send):
            result = asyncio.run(
                _send_to_platform(
                    ...
                    SimpleNamespace(enabled=True, token="***", extra={}),
                    "C123",
                    "**hello** from [Zed](<https://example.com>)",
                )
            )
        ...
        send.assert_awaited_once_with(
            "***",
            "C123",
            "*hello* from <https://example.com|Zed>",
            thread_ts=None,
        )
...
class TestSendToPlatformWhatsapp:
    def test_whatsapp_routes_via_local_bridge_sender(self):
        ...
        assert result["success"] is True
        async_mock.assert_awaited_once_with({"bridge_port": 3000}, chat_id, "hello from zed")
...
class TestParseTargetRefMatrix:
    ...
    def test_matrix_user_mxid_is_explicit(self):
        ...
        chat_id, thread_id, is_explicit = _parse_target_ref("matrix", "@zed:matrix.org")
        assert chat_id == "@zed:matrix.org"
        assert thread_id is None
        assert is_explicit is True

    def test_matrix_alias_is_not_explicit(self):
        """Matrix room aliases (#) are NOT explicit â€” they need resolution."""
...
class TestParseTargetRefWhatsAppJID:
    ...
    def test_non_jid_whatsapp_target_falls_through(self):
        """A bare friendly name is not a JID â€” it must fall through to
...
class TestParseTargetRefSlack:
    ...
    def test_user_id_is_not_explicit(self):
        ...
        targets. chat.postMessage rejects them â€” a DM must be opened first via
...
class TestSendDiscordForum:
    ...
    def test_directory_forum_creates_thread(self):
        """Directory says 'forum' â€” creates a thread post."""
        thread_data = {
            "id": "t123",
            "message": {"id": "m456"},
        }
    ...
    def test_directory_forum_skips_probe(self):
        ...
        # get() should never be called â€” directory resolved the type
...
class TestSendDiscordForumMedia:
    ...
    def test_forum_with_media_uses_multipart(self, tmp_path, monkeypatch):
        """Forum + media â†’ single multipart POST to /threads carrying the starter + files."""
        ...
        # Exactly one POST â€” the combined thread-creation + attachments call
    ...
    def test_forum_without_media_still_json_only(self, tmp_path, monkeypatch):
        """Forum + no media â†’ JSON POST (no multipart overhead)."""
        monkeypatch.setattr(
            "gateway.channel_directory.lookup_channel_type", lambda p, cid: "forum"
        )
...
class TestForumProbeCache:
    ...
    def test_probe_result_is_memoized(self, monkeypatch):
        ...
        # Only one session opened (thread creation) â€” no probe session this time
        # (verified by not raising from our side_effect exhaustion)


# ---------------------------------------------------------------------------
# _send_signal â€” chunking + 429 retry (mirrors gateway adapter behavior)
# ---------------------------------------------------------------------------
...
class TestSendSignalChunking:
    ...
    def test_full_followup_batch_emits_pacing_notice(self, tmp_path, monkeypatch):
        """64 attachments â†’ 2 full batches. Batch 1 needs 14 more tokens
        than the 18 remaining after batch 0 â€” 56s wait crossing the 10s
        notice threshold."""
        from gateway.platforms.signal_rate_limit import (
            SIGNAL_MAX_ATTACHMENTS_PER_MSG,
        ...
        # Batch 1 deficit: 32 - (50 - 32) = 14 tokens Ã— 4s = 56s
        expected = (
            SIGNAL_MAX_ATTACHMENTS_PER_MSG
    ...
    def test_429_with_retry_after_drives_exact_backoff(self, tmp_path, monkeypatch):
        """signal-cli â‰¥ v0.14.3 surfaces Retry-After under
    ...
    def test_non_rate_limit_error_returns_immediately(self, tmp_path, monkeypatch):
        """A non-429 RPC error should not retry â€” it returns an error result."""
        p = tmp_path / "img.png"
        p.write_bytes(b"\x89PNG" + b"\x00" * 16)
...
# â”€â”€ _send_via_adapter standalone fallback â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestSendViaAdapterStandaloneFallback:
    ...
    When the gateway runner is not in this process (e.g. ``zed cron``
    runs separately from ``zed gateway``), ``_send_via_adapter`` should
...
# ---------------------------------------------------------------------------
# _check_send_message â€” availability gating
# ---------------------------------------------------------------------------
...
class TestCheckSendMessage:
    ...
    1. ``ZED_KANBAN_TASK`` is set (worker spawned by the kanban dispatcher
       â€” parent gateway is by definition running, but the worker's
       ``ZED_HOME`` may be a profile dir without a ``gateway.pid``).
    2. ``ZED_SESSION_PLATFORM`` resolves to a non-empty, non-``local`` value
    ...
    3. ``is_gateway_running()`` returns True (CLI / orchestrator profile with
       a live gateway colocated under the same ``ZED_HOME``).
    4. None of the above â†’ False, tool is hidden.
    """

    def test_kanban_task_env_grants_access(self, monkeypatch):
        """Workers spawned by the dispatcher (ZED_KANBAN_TASK set) must be
        allowed regardless of session_platform / gateway-pid state."""
        from tools.send_message_tool import _check_send_message

        monkeypatch.setenv("ZED_KANBAN_TASK", "t_abc12345")
    ...
    def test_kanban_task_env_short_circuits_before_gateway_check(self, monkeypatch):
        """Honoring ZED_KANBAN_TASK must not depend on importing or calling
        gateway.status â€” the worker may run with a ZED_HOME that has no
    ...
    def test_local_platform_falls_through_to_gateway_check(self, monkeypatch):
        """``ZED_SESSION_PLATFORM=local`` means CLI-style â€” must defer to
