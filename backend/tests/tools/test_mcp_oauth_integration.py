...
Exercises the full chain â€” manager, provider subclass, disk watch, 401
dedup â€” with real file I/O and real imports (no transport mocks, no
...
and the running Zed session picks up the new tokens on the next auth
...
async def test_external_refresh_picked_up_without_restart(tmp_path, monkeypatch):
    ...
    1. A running Zed session has OAuth tokens loaded in memory.
    ...
    # automatically via the ZedMCPOAuthProvider.async_auth_flow
...
async def test_handle_401_deduplicates_concurrent_callers(tmp_path, monkeypatch):
    ...
    Mirrors Claude Code's pending401Handlers dedup pattern â€” prevents N MCP
    ...
    # Count how many times invalidate_if_disk_changed is called â€” proxy for
    ...
    # Exactly ONE recovery ran â€” the rest awaited the same pending future.
...
async def test_invalidate_if_disk_changed_handles_missing_file(tmp_path, monkeypatch):
    ...
    # No tokens file exists yet â€” this is the pre-auth state
