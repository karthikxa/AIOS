"""Tests for tools.env_passthrough â€” skill and config env var passthrough."""
...
class TestTerminalIntegration:
    ...
    def test_blocklisted_var_blocked_by_default(self):
        from tools.environments.local import _sanitize_subprocess_env, _ZED_PROVIDER_ENV_BLOCKLIST

        # Pick a var we know is in the blocklist
        blocked_var = next(iter(_ZED_PROVIDER_ENV_BLOCKLIST))
    ...
    def test_passthrough_cannot_override_provider_blocklist(self):
        ...
        Zed provider credentials â€” that was the bypass where a skill
        ...
        blocked_var = next(iter(_ZED_PROVIDER_ENV_BLOCKLIST))
        # Attempt to register â€” must be silently refused (logged warning).
    ...
    def test_make_run_env_blocklist_override_rejected(self):
        ...
        try:
            # Without passthrough â€” blocked
            result_before = _make_run_env({})
            assert blocked_var not in result_before

            # Skill tries to register it â€” must be refused, so still blocked
            register_env_passthrough([blocked_var])
    ...
    def test_non_zed_api_key_still_registerable(self):
        """Third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
        Zed provider credentials and must still pass through â€” skills
        that legitimately wrap third-party APIs must keep working."""
        # TENOR_API_KEY is a real example â€” used by the gif-search skill
        register_env_passthrough(["TENOR_API_KEY"])
        assert is_env_passthrough("TENOR_API_KEY")
