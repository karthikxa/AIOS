...
before the exception propagates â€” otherwise the local backend's use of
os.setsid leaves an orphan with PPID=1.

The live repro that motivated this: zed chat -q ... 'sleep 300', SIGTERM
...
def _wait_for_pgid_exit(pgid: int, timeout: float = 30.0) -> bool:
    ...
    The cleanup chain is: SIGTERM â†’ 3s TimeoutStopSec â†’ SIGKILL â†’ reap.
...
def test_wait_for_process_kills_subprocess_on_keyboardinterrupt():
    ...
    try:
        ...
        def worker():
            ...
            # to observe the cleanup, via env.execute(...) â€” the normal path
            # that goes through _wait_for_process.
            try:
                result_holder["result"] = env.execute("sleep 30", timeout=60)
            except BaseException as e:  # noqa: BLE001 â€” we want to observe it
        ...
        while time.monotonic() < deadline:
            ...
            try:
                import psutil  # optional â€” fall back if absent
        ...
        # xdist load the SIGTERM â†’ 3s wait â†’ SIGKILL chain can take longer
        ...
        # just the bash wrapper â€” the 'sleep 30' child too. Under xdist load,
        ...
        assert _wait_for_pgid_exit(pgid), (
            ...
            f"KeyboardInterrupt â€” orphan bug regressed.  This is the "
        ...
        assert result_holder.get("exception") == "KeyboardInterrupt", (
            f"worker result: {result_holder!r} â€” expected KeyboardInterrupt "
