"""Regression test for #34192 â€” Dockerfile must keep the tini compat shim
...
'Zed WebUI' catalog updates to /init) and not by accident.
"""
...
def test_tini_compat_symlink_present():
    ...
    assert "ln -sf /init /usr/bin/tini" in df, (
        ...
        "/usr/bin/tini as the entrypoint (Hostinger 'Zed WebUI' "
...
def test_entrypoint_still_init_not_tini():
    ...
    runtime â€” that path must continue to use the canonical /init."""
    df = _dockerfile_text()
