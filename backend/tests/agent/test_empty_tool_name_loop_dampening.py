"""Regression for #47967 â€” empty-name phantom tool calls.
...
calls that mimic the payload â€” usually with an empty/whitespace ``name``. Those
...
``AIAgent.run_conversation`` against an in-process mock provider â€” not a snapshot
...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated ZED_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="zed_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".zed"))
    prev_home = os.environ.get("ZED_HOME")
    os.environ["ZED_HOME"] = os.path.join(test_home, ".zed")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("zed_"):
            del sys.modules[mod]
