...
Reports live under ``~/.zed/logs/curator/{YYYYMMDD-HHMMSS}/`` alongside
...
def curator_env(tmp_path, monkeypatch):
    """Isolated ZED_HOME with a skills/ dir + reset curator module state."""
    home = tmp_path / ".zed"
    home.mkdir()
    (home / "skills").mkdir()
    (home / "logs").mkdir()
...
def test_reports_root_is_under_logs_not_skills(curator_env):
    """Reports live in logs/curator/, not skills/ â€” operational telemetry
...
def test_state_transitions_captured_in_report(curator_env):
    """When a skill moves active â†’ stale or stale â†’ archived between
    ...
    assert "active â†’ stale" in md


# ---------------------------------------------------------------------------
# Cron job skill reference rewriting (curator â†” cron integration)
# ---------------------------------------------------------------------------
#
...
# run time â€” the scheduler logs a warning and skips it, so the scheduled
...
def curator_env_with_cron(curator_env, monkeypatch):
    ...
    monkeypatch.setattr(jobs_mod, "ZED_DIR", home)
...
def test_curator_rewrites_cron_skills_when_skill_consolidated(curator_env_with_cron):
    ...
    # Simulate a curator pass that consolidated `foo` â†’ `foo-umbrella`
...
def test_curator_drops_pruned_skill_from_cron_job(curator_env_with_cron):
    ...
    job's skill list entirely â€” there's no forwarding target."""
    curator = curator_env_with_cron["curator"]
    jobs = curator_env_with_cron["jobs"]
    ...
    run_dir = curator._write_run_report(
        ...
        after_report=after,
        llm_meta=_make_llm_meta(),  # no tool calls â†’ classifier marks it pruned
    )
...
def test_curator_report_has_no_cron_section_when_nothing_changes(curator_env_with_cron):
    ...
    # No rewrites â†’ no separate file, no section in md
