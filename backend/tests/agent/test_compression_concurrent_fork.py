...
Damien's incident (Discord, 2026-05-28): a long Zed session in a Discord
...
``_spawn_background_review(...)`` â€” which builds a forked ``AIAgent`` that
...
a synthetic ``Background process proc_â€¦ completed`` event arrived and
...
def _build_agent_with_db(db: SessionDB, session_id: str):
    ...
    # actually overlap â€” without that the OS could happen to serialize them
    # and hide the bug.
...
def test_concurrent_compression_does_not_fork_session(tmp_path: Path) -> None:
    ...
    # Two agents on the same session_id, both wired to the same db â€”
    ...
    def run(agent):
        try:
            ...
            # Surface to the test if either raises â€” should not happen.
            raise
    ...
    # Exactly one canonical child â€” not two orphans.
    assert _count_children(db, parent_sid) == 1, (
        "Compression lock failed: parent session has multiple children in state.db "
        "(transcript fork). This is Damien's incident shape â€” see the test docstring."
    )
...
def test_missing_lock_subsystem_fails_open_not_infinite_loop(tmp_path: Path) -> None:
    ...
    assert len(compressed) < len(messages), (
        "Compression made no progress despite failing open â€” loop would still spin."
    )
...
def test_review_fork_disables_compression_to_prevent_stale_parent_fork() -> None:
    ...
    next turn from the stale parent and compress it AGAIN â€” leaving the same
    ...
    ``agent.compression_enabled`` BEFORE calling ``_compress_context``:
      â€¢ preflight (``if agent.compression_enabled and len(messages) > ...``)
      â€¢ mid-loop  (``if agent.compression_enabled and _compressor.should_compress(...)``)
    ...
    assert captured, (
        "_run_review_in_thread never reached run_conversation â€” the spawn path "
