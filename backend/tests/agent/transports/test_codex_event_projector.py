"""Tests for CodexEventProjector â€” codex item/* events â†’ Zed messages list.
...
class TestProjectionInvariants:
    ...
    def test_streaming_deltas_dont_materialize(self) -> None:
        ...
        for delta_method in (
            ...
            assert r.messages == [], (
                f"{delta_method} should NOT produce messages â€” only "
...
class TestCommandExecutionProjection:
    """Real captured notification â†’ assistant tool_call + tool result."""
    ...
    def test_deterministic_call_id_across_replay(self) -> None:
        # Same item id â†’ same call_id (prefix cache must stay valid).
...
class TestAgentMessageProjection:
    """assistant text â†’ final_text + assistant message."""
...
class TestRoleAlternationInvariant:
    ...
    one item â€” that breaks Zed' message alternation invariant."""
