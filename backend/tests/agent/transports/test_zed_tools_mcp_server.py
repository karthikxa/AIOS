"""Tests for the zed-tools-as-MCP server module surface.

We don't run a live MCP session in unit tests â€” that requires the codex
...
class TestModuleSurface:
    def test_module_imports_clean(self):
        from agent.transports import zed_tools_mcp_server as m
    ...
    def test_exposed_tools_are_safe_subset(self):
        ...
        patch â€” those are codex's built-in tools."""
        from agent.transports.zed_tools_mcp_server import EXPOSED_TOOLS
        forbidden = {
    ...
    def test_expected_zed_specific_tools_listed(self):
        """The Zed-specific tools should be present so users on the
    ...
    def test_kanban_worker_tools_exposed(self):
        """Kanban workers run as `zed chat -q` subprocesses; if they
        ...
        from agent.transports.zed_tools_mcp_server import EXPOSED_TOOLS
        # Worker handoff tools â€” every dispatched worker uses at least
        ...
        for worker_tool in (
            ...
            assert worker_tool in EXPOSED_TOOLS, (
                f"{worker_tool!r} missing from codex callback â€” kanban "
...
class TestMain:
    def test_main_returns_2_when_mcp_unavailable(self, monkeypatch):
        ...
        import agent.transports.zed_tools_mcp_server as m
