...
module. They do NOT require the `codex` CLI to be installed â€” that's
...
class TestMaybeApplyCodexAppServerRuntime:
    """The opt-in helper that rewrites api_mode â†’ codex_app_server."""
    ...
    def test_other_providers_never_rerouted(self, provider) -> None:
        """Non-OpenAI providers MUST NOT be rerouted even with the flag set â€”
...
class TestSpawnEnvIsolation:
    """The codex spawn must NOT rewrite HOME â€” codex's shell tool spawns
    ...
    OpenClaw hit this footgun (openclaw/openclaw#81562) â€” they were
    ...
    def test_spawn_env_sets_CODEX_HOME_when_provided(self, monkeypatch):
        """CODEX_HOME isolation must still work â€” that's the whole point
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Zed passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("ZED_HOME", "/users/alice/.zed/profiles/backend-worker")
        monkeypatch.setenv("ZED_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "ZED_KANBAN_DB",
            "/users/alice/.zed/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.zed/kanban/boards/smoke"]'
            in cmd
        )
