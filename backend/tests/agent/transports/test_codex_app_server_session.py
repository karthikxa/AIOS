"""Tests for CodexAppServerSession â€” drive turns through a mock client.
...
class TestRunTurn:
    ...
    def test_tool_iteration_counter_ticks(self):
        ...
        # Each tool item produces (assistant, tool) â€” 2*2 + final assistant = 5 msgs
    ...
    def test_turn_start_failure_attaches_redacted_stderr_tail(self):
        ...
        # Non-OAuth â†’ should NOT retire (subprocess JSON-RPC is still healthy).
    ...
    def test_interrupt_during_turn_issues_turn_interrupt(self):
        client = FakeClient()
        # Don't queue turn/completed â€” the loop has to interrupt out
        client.queue_notification(
            "item/completed",
    ...
    def test_deadline_exceeded_records_error(self):
        client = FakeClient()
        # No notifications and no completion â†’ must hit deadline
        s = make_session(client)
...
class TestServerRequestRouting:
    ...
    def test_mcp_elicitation_for_zed_tools_auto_accepts(self):
        """When codex elicits on behalf of zed-tools (our own callback),
        accept automatically â€” the user already opted in by enabling the
        runtime."""
        client = FakeClient()
        client.queue_server_request(
            "mcpServer/elicitation/request", request_id="elic-1",
            threadId="t", turnId="tu1",
            serverName="zed-tools",
            mode="form",
            message="confirm",
...
class TestSessionRetirement:
    ...
    def test_post_tool_quiet_watchdog_trips_and_retires(self):
        client = FakeClient()
        # One tool completion, then total silence â€” no further events,
    ...
    def test_post_tool_watchdog_resets_on_further_activity(self):
        ...
        the watchdog â€” further activity = codex still alive."""
        client = FakeClient()
        client.queue_notification(
            "item/completed",
            item={
        ...
        # Non-tool activity immediately after â€” resets watchdog.
        client.queue_notification(
            "item/completed",
    ...
    def test_turn_aborted_marker_in_text_is_terminal(self):
        ...
        # Should have exited fast â€” not waited for the full 2s deadline.
    ...
    def test_generic_turn_failure_does_not_trigger_oauth_hint(self):
        ...
        # Generic model failures don't retire â€” the session itself is fine
        assert r.should_retire is False
