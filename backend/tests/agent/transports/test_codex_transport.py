...
class TestCodexBuildKwargs:
    ...
    def test_xai_responses_sends_cache_key_via_extra_body(self, transport):
        ...
        signature ever drops the field â€” the body field still serializes
    ...
    def test_xai_reasoning_effort_passed(self, transport):
        ...
        # across turns for cross-turn coherence â€” xAI explicitly relies
    ...
    def test_xai_does_not_inject_native_web_search_without_client_web_search(self, transport):
        ...
        already-requested client ``web_search`` â€” NOT an additive grant.  A
        ...
        would silently bypass Zed's web-provider config and tool-trace
    ...
    def test_xai_drops_clientside_web_search_to_avoid_duplicate(self, transport):
        ...
        path must drop it and rely on the native built-in â€” otherwise xAI
    ...
    def test_non_xai_path_does_not_inject_native_web_search(self, transport):
        """Native web_search injection is scoped to xAI â€” Codex/GitHub paths
    ...
    # ``reasoning.encrypted_content`` back from xAI on every model â€”
    ...
    def test_xai_grok_3_non_mini_omits_reasoning_effort(self, transport):
        """Plain grok-3 rejects reasoning.effort â€” only grok-3-mini accepts it."""
        messages = [{"role": "user", "content": "Hi"}]
        kw = transport.build_kwargs(
            model="grok-3", messages=messages, tools=[],
            is_xai_responses=True,
...
class TestCodexValidateResponse:
    ...
    def test_output_text_fallback_not_valid(self, transport):
        """validate_response is strict â€” output_text doesn't make it valid.
...
class TestCodexTransportTimeout:
    ...
    def test_bool_timeout_dropped(self, transport):
        """``True`` is technically int but must not survive â€” caller bug guard."""
        kw = transport.build_kwargs(
            model="gpt-5.5",
            messages=[{"role": "user", "content": "hi"}],
            tools=[],
...
class TestCodexTransportXaiServiceTierStrip:
    ...
    in config.yaml â€” both leak paths plumb through ``request_overrides``
    ...
    def test_non_xai_codex_preserves_service_tier(self, transport):
        """The strip is xAI-only â€” native Codex DOES accept
...
class TestPreflightSlashEnumStrip:
    ...
    those â€” gated on model name so we don't unnecessarily strip on
    non-xAI providers.
    """
    ...
    def test_grok_model_strips_slash_enum_values(self):
        ...
        # semantics â€” it removes the constraint entirely when any value
        # contains /).
