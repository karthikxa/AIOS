...
These tests do NOT require AWS credentials or boto3 â€” all AWS calls
are mocked.

Note: Tests that import ``zed_cli.auth`` or ``zed_cli.runtime_provider``
...
class TestResolveProvider:
    ...
    def test_auto_detect_with_aws_credentials(self, monkeypatch):
        ...
        with patch("zed_cli.auth._load_auth_store", return_value={}):
...
class TestRuntimeProvider:
    ...
    def test_bedrock_runtime_resolution(self, monkeypatch):
        ...
        with patch("zed_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("zed_cli.runtime_provider._get_model_config", return_value={"provider": "bedrock"}):
    ...
    def test_bedrock_runtime_no_credentials_raises_on_auto_detect(self, monkeypatch):
        ...
        with patch("zed_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("zed_cli.runtime_provider._get_model_config", return_value={"provider": "bedrock"}), \
             patch("zed_cli.runtime_provider.resolve_requested_provider", return_value="auto"), \
    ...
    def test_bedrock_runtime_explicit_skips_credential_check(self, monkeypatch):
        ...
        # No AWS env vars set â€” but explicit bedrock request should not raise
        for var in ["AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY", "AWS_PROFILE",
...
class TestProvidersModule:
    """Verify bedrock is wired into zed_cli/providers.py."""

    def test_bedrock_alias_in_providers(self):
        from zed_cli.providers import ALIASES
    ...
    def test_bedrock_transport_mapping(self):
        from zed_cli.providers import TRANSPORT_TO_API_MODE
...
class TestPackaging:
    ...
    def test_bedrock_is_not_eager_installed_by_all_extra(self):
        extras = self._optional_dependencies()
        assert "zed-agent[bedrock]" not in extras["all"]


# ---------------------------------------------------------------------------
# Model ID dot preservation â€” regression for #11976
# ---------------------------------------------------------------------------
...
class TestBedrockPreserveDotsFlag:
    ...
    def test_non_bedrock_aws_url_does_not_preserve_dots(self):
        ...
        must not accidentally activate the dot-preservation heuristic â€”
    ...
    def test_anthropic_native_still_does_not_preserve_dots(self):
        ...
        existing Anthropic native behaviour â€” ``claude-sonnet-4.6`` still
...
class TestBedrockModelNameNormalization:
    ...
    def test_apac_anthropic_haiku_inference_profile_preserved(self):
        """APAC inference profile â€” same structural-dot shape."""
        from agent.anthropic_adapter import normalize_model_name
        assert normalize_model_name(
...
class TestBedrockBuildAnthropicKwargsEndToEnd:
    ...
    model string in the outgoing kwargs â€” the exact body sent to the
...
# ---------------------------------------------------------------------------
# auxiliary_client Bedrock resolution â€” fix for #13919
# ---------------------------------------------------------------------------
...
class TestAuxiliaryClientBedrockResolution:
    ...
    def test_bedrock_returns_client_with_credentials(self, monkeypatch):
        ...
        assert client is not None, (
            "resolve_provider_client('bedrock') returned None â€” "
