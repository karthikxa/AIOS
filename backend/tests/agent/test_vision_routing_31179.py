...
def isolated_home(monkeypatch):
    """Temp ZED_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="zed_test_31179_")
    zed_home = os.path.join(test_home, ".zed")
    os.makedirs(zed_home)
...
def _fresh_modules():
    """Drop cached zed modules so each test reloads against current env."""
...
# ---------------------------------------------------------------------------
# Fix 1: provider=openai â†’ custom + api.openai.com/v1
# ---------------------------------------------------------------------------
...
""")
        ...
        # Exact hostname comparison (not substring) â€” defends against URLs
...
class TestTextOnlyMainSkippedForVision:
    ...
    def test_text_only_main_skipped_when_no_aggregator(self, isolated_home, monkeypatch):
        """DeepSeek main + no aggregator credentials â†’ no client built.
...
""")
    ...
    def test_unknown_capability_does_not_block(self, isolated_home, monkeypatch):
        ...
        This keeps new/custom providers working â€” only providers we have
        cataloged as text-only are skipped.
        """
        _fresh_modules()
        from agent.auxiliary_client import _main_model_supports_vision
        # Bogus provider/model â€” capability lookup returns None â†’ permissive.
...
""")
        ...
        with patch.object(tools.browser_tool, "check_browser_requirements", return_value=False):
            # Vision available but browser missing â†’ still False.
            assert tools.browser_tool.check_browser_vision_requirements() is False
