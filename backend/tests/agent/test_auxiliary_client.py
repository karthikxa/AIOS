...
class TestBuildCallKwargsMaxTokens:
    ...
    the one exception â€” max_tokens is a mandatory field there.
    """
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("nous", "zed-4", "https://inference-api.zedteam.com/v1"),
...
class TestReadCodexAccessToken:
    ...
    def test_pool_without_selected_entry_falls_back_to_auth_store(self, tmp_path, monkeypatch):
        ...
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(True, None)), \
             patch("zed_cli.auth._read_codex_tokens", return_value={
...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(self, tmp_path, monkeypatch):
        ...
        ``zed auth status`` already reports these as logged in; compression
        ...
        from zed_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
    ...
    def test_pool_backed_credentials_honor_base_url_env_override(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZED_HOME", str(zed_home))
        monkeypatch.setenv("ZED_XAI_BASE_URL", "https://example.x.ai/v1/")
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    reach this function without an explicit model â€” the user's main
    provider was picked via ``zed model``, no per-task override is
    ...
    ``(None, None)`` on an empty model â€” both lack a catalog default
    ...
    def test_empty_model_for_oauth_provider_falls_back_to_main_model(self):
        """xai-oauth: no catalog default â†’ uses main model."""
    ...
    def test_empty_model_for_codex_also_uses_main_model(self):
        """openai-codex: symmetric with xai-oauth â€” same universal fallback."""
        from agent.auxiliary_client import resolve_provider_client
    ...
    def test_empty_model_for_catalog_provider_uses_catalog_default(self):
        ...
        providers\" behaviour â€” users on anthropic for their main chat
        ...
        # Catalog default takes precedence â€” main_model was a no-op
    ...
    def test_explicit_model_takes_precedence_over_fallbacks(self):
        ...
        (``auxiliary.<task>.model``) routes here â€” when the user
...
class TestExpiredCodexFallback:
    ...
    def test_zed_oauth_file_sets_oauth_flag(self, monkeypatch):
        """OAuth-style tokens should get is_oauth=*** (token is not sk-ant-api-*)."""
        # Mock resolve_anthropic_token to return an OAuth-style token
        with patch("agent.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-oat-zed-token"), \
             patch("agent.anthropic_adapter.build_anthropic_client") as mock_build, \
             patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)):
            mock_build.return_value = MagicMock()
            from agent.auxiliary_client import _try_anthropic
...
class TestGetTextAuxiliaryClient:
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            ...
            patch("zed_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from agent.auxiliary_client import _build_codex_client
...
class TestAuxiliaryPoolAwareness:
    ...
    def test_try_nous_uses_portal_recommendation_for_text(self):
        ...
        fresh_base = "https://inference-api.zedteam.com/v1"
        with (
            ...
            patch("zed_cli.models.get_nous_recommended_aux_model", return_value="minimax/minimax-m2.7") as mock_rec,
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
        ):
            from agent.auxiliary_client import _try_nous
    ...
    def test_try_nous_uses_portal_recommendation_for_vision(self):
        ...
        with (
            patch("agent.auxiliary_client._read_nous_auth", return_value={"access_token": "***"}),
            patch("agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("zed_cli.models.get_nous_recommended_aux_model", return_value="google/gemini-3-flash-preview") as mock_rec,
            patch("agent.auxiliary_client.OpenAI"),
        ):
            from agent.auxiliary_client import _try_nous
    ...
    def test_try_nous_falls_back_when_recommendation_lookup_raises(self):
        ...
        with (
            patch("agent.auxiliary_client._read_nous_auth", return_value={"access_token": "***"}),
            patch("agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("zed_cli.models.get_nous_recommended_aux_model", side_effect=RuntimeError("portal down")),
            patch("agent.auxiliary_client.OpenAI"),
        ):
            from agent.auxiliary_client import _try_nous
            client, model = _try_nous()
    ...
    def test_call_llm_retries_nous_after_401(self):
        ...
        stale_client.base_url = "https://inference-api.zedteam.com/v1"
        stale_client.chat.completions.create.side_effect = _Auth401("stale nous key")

        fresh_client = MagicMock()
        fresh_client.base_url = "https://inference-api.zedteam.com/v1"
        ...
        with (
            ...
            patch("agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", "https://inference-api.zedteam.com/v1")),
        ):
            result = call_llm(
                task="compression",
                messages=[{"role": "user", "content": "hi"}],
    ...
    async def test_async_call_llm_retries_nous_after_401(self):
        ...
        fresh_async_client.base_url = "https://inference-api.zedteam.com/v1"
...
# â”€â”€ Payment / credit exhaustion fallback â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestIsPaymentError:
    ...
    def test_404_free_tier_model_block_is_payment(self):
        exc = Exception(
            ...
            "Upgrade at https://portal.zedteam.com or pick a free model."
        )
        exc.status_code = 404
        assert _is_payment_error(exc) is True
    ...
    # â”€â”€ Daily / monthly quota exhaustion (#26803) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestIsModelNotFoundError:
    ...
    def test_billing_404_is_not_model_not_found(self):
        """Free-tier / credit 404s belong to _is_payment_error, not here â€”
    ...
    def test_out_of_funds_404_is_not_model_not_found(self):
        ...
        exc.status_code = 404
        # billing keyword wins â€” payment owns it
        assert _is_model_not_found_error(exc) is False
...
class TestRefreshNousRecommendedModel:
    ...
    def test_returns_fresh_portal_recommendation(self, monkeypatch):
        monkeypatch.setattr(
            "zed_cli.models.get_nous_recommended_aux_model",
    ...
    def test_falls_back_to_default_when_portal_unavailable(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "zed_cli.models.get_nous_recommended_aux_model", _boom)
        out = _refresh_nous_recommended_model(
...
class TestIsRateLimitError:
    ...
    def test_openai_ratelimiterror_classname(self):
        """OpenAI SDK RateLimitError may omit .status_code â€” detect by class name."""
...
class TestGetProviderChain:
    ...
    def test_returns_four_entries(self):
        ...
        # Codex is deliberately NOT in this chain â€” see _get_provider_chain
...
class TestTryPaymentFallback:
    ...
    def _clear_unhealthy_cache(self):
        ...
        to return valid clients â€” the patched function is never called.
        """
    ...
    def test_codex_not_in_fallback_chain(self):
        ...
        When OR/Nous/custom/api-key all fail, payment-fallback returns None â€”
...
class TestAuxiliaryFallbackLayering:
    """Explicit-provider users get layered fallback: configured_chain â†’ main agent â†’ warn."""
    ...
    def test_explicit_provider_uses_configured_chain_first(self, monkeypatch, caplog):
        ...
        # Main agent fallback should NOT have been consulted â€” chain succeeded first
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    from zed_cli.auth import ProviderConfig
    ...
    monkeypatch.setattr("zed_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
...
class TestTransientTransportRetry:
    ...
    def test_retries_streaming_close_once_same_provider(self):
        ...
        assert result == {"ok": True}
        # Same client called twice â€” no provider fallback needed.
...
class TestKimiTemperatureOmitted:
    ...
    active mode (thinking â†’ 1.0, non-thinking â†’ 0.6).  Sending any temperature
...
class TestAuxiliaryTaskExtraBody:
    def test_sync_call_merges_task_extra_body_from_config(self):
        ...
        with patch("zed_cli.config.load_config", return_value=config), patch(
            "agent.auxiliary_client._get_cached_client",
...
class TestCodexAdapterReasoningTranslation:
    ...
    def test_reasoning_default_effort_when_only_enabled_flag(self):
        """extra_body={"reasoning": {}} (truthy enabled by omission) â†’ default 'medium'."""
        adapter, captured = self._build_adapter()
        adapter.create(
            messages=[{"role": "user", "content": "hi"}],
    ...
    def test_non_dict_reasoning_value_is_ignored_gracefully(self):
        ...
        adapter.create(
            messages=[{"role": "user", "content": "hi"}],
            extra_body={"reasoning": "medium"},  # wrong shape â€” must not crash
        )
        assert "reasoning" not in captured

    def test_reasoning_effort_null_falls_back_to_medium(self):
        """Parity with agent/transports/codex.py::build_kwargs() â€” falsy
    ...
    def test_reasoning_effort_zero_falls_back_to_medium(self):
        """Numeric ``0`` is also falsy â€” the docstring lists it explicitly,
...
class TestVisionAutoSkipsKimiCoding:
    ...
    def test_kimi_coding_skipped_falls_through_to_openrouter(self, monkeypatch):
        """kimi-coding as main + vision auto â†’ OpenRouter (not kimi)."""
        ...
        # and resolve_provider_client both would try kimi-coding â€” detect
    ...
    def test_explicit_override_to_kimi_coding_still_honored(self, monkeypatch):
        ...
        routes to kimi-coding â€” only the auto branch applies the skip.
        """
        monkeypatch.setattr(
...
class TestCodexAuxiliaryAdapterNullOutputRecovery:
    ...
    def test_handles_final_output_is_none_after_consumer(self):
        """Regression for #33368 â€” defense against ``final.output`` being ``None``.
...
# ---------------------------------------------------------------------------
# Issue #23432 â€” auxiliary timeout poisons cached client; later aux calls fail
# ---------------------------------------------------------------------------
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    See https://github.com/ZedTeam/zed-agent/issues/23432.
    """
    ...
    def test_evict_cached_client_instance_walks_async_wrapper(self):
        ...
        timeout that closes the leaf must evict BOTH â€” otherwise the async
        ...
        try:
            ...
            assert ("openai-codex", True, None, None, None) not in _client_cache, (
                "async cache entry survived eviction â€” wrapper is missing _real_client"
            )
        finally:
            with _client_cache_lock:
                _client_cache.clear()
    ...
    def test_call_llm_evicts_on_connection_error_with_explicit_provider(self):
        ...
        connection) DO trigger fallback even on explicit providers â€” so we
...
# ---------------------------------------------------------------------------
# _build_call_kwargs â€” tool dedup at API boundary
# ---------------------------------------------------------------------------
...
class TestBuildCallKwargsToolDedup:
    ...
    regresses.  See: https://github.com/ZedTeam/zed-agent/issues/18478
    """
    ...
    def test_duplicate_tool_names_are_deduplicated(self):
        """RED test â€” must fail until dedup guard is added."""
        tools = [
        ...
        # Must be deduplicated â€” no repeated names
...
class TestNvidiaBillingHeaders:
    ...
    def test_resolve_provider_client_cloud_adds_billing_origin_header(self, monkeypatch):
        ...
        assert headers["X-BILLING-INVOKE-ORIGIN"] == "ZedAgent"
...
class TestAnthropicExplicitApiKey:
    ...
    updated â€” _try_anthropic() always fell back to resolve_anthropic_token()
...
# â”€â”€ Auxiliary unhealthy-provider TTL cache (issue #23570) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestAuxUnhealthyCache:
    ...
    def test_resolve_auto_skips_unhealthy_step2(self):
        ...
        # Mark OpenRouter unhealthy â†’ chain should skip it and pick nous.
        _mark_provider_unhealthy("openrouter")
    ...
    def test_resolve_auto_skips_unhealthy_main_in_step1(self):
        ...
        with patch("agent.auxiliary_client._read_main_provider", return_value="openrouter"), \
             ...
             patch("agent.auxiliary_client._resolve_api_key_provider", return_value=(None, None)):
            client, model = _resolve_auto()
        # Step-1 was bypassed â€” resolve_provider_client never invoked
...
# â”€â”€ auxiliary_max_tokens_param â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestAuxiliaryMaxTokensParam:
    ...
    # Model-name fallback â€” this is the regression guard.
    ...
    def test_empty_model_falls_back_to_url_only(self):
        """No model hint â†’ only the URL-based rule applies."""
        with (
            patch("agent.auxiliary_client._current_custom_base_url",
                  return_value="https://my-gateway.example.com/v1"),
            patch("agent.auxiliary_client._read_nous_auth", return_value=None),
