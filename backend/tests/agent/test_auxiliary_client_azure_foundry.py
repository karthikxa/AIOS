...
that delegates to :func:`zed_cli.runtime_provider._resolve_azure_foundry_runtime`
...
Pinned scenarios:
  ...
  * ``auth_mode: api_key`` â†’ plain OpenAI client with the static string
    key for ``chat_completions``.
  * ``auth_mode: entra_id`` + ``chat_completions`` â†’ plain OpenAI
    client with a callable ``api_key`` (the bearer-token provider) â€”
    confirms the callable survives the auxiliary path end-to-end.
  * ``auth_mode: entra_id`` + GPT-5.x model â†’ CodexAuxiliaryClient
    wrapping the OpenAI client (api_mode auto-upgrades to
    codex_responses).
  * Anthropic-style + entra_id â†’ rejected at the runtime resolver,
...
# ---------------------------------------------------------------------------
# auth_mode: api_key (default) â€” regression for the legacy path
# ---------------------------------------------------------------------------
...
class TestAuxAzureFoundryApiKey:
    ...
    def test_codex_responses_wraps_in_codex_aux_client(self, monkeypatch, patch_load_config):
        ...
        # GPT-5.x â†’ runtime auto-upgrades to codex_responses
    ...
    def test_no_model_returns_none(self, monkeypatch, patch_load_config):
        """Azure has no fallback aux model â€” fail soft so the auto chain
...
# ---------------------------------------------------------------------------
# auth_mode: entra_id â€” callable api_key survives end-to-end
# ---------------------------------------------------------------------------
...
class TestAuxAzureFoundryEntra:
    def test_callable_api_key_reaches_openai_constructor(
        ...
        intact â€” never stringified to ``"no-key-required"`` or to the
        ...
        # in this path â€” that's a separate concern handled by the
    ...
    def test_codex_responses_with_entra_wraps_correctly(
        ...
        """GPT-5.x deployment on Entra ID â€” auto-upgraded to
        ...
        # callable api_key â€” verify against the SDK constructor record,
    ...
    def test_entra_anthropic_messages_uses_bearer_hook(
        ...
        api_key; ``_maybe_wrap_anthropic`` â†’ ``build_anthropic_client``
...
# ---------------------------------------------------------------------------
# resolve_provider_client â†’ azure-foundry dispatch
# ---------------------------------------------------------------------------
...
class TestResolveProviderClientAzureFoundry:
    def test_dispatches_to_azure_branch_not_generic_api_key_path(
        ...
        assert client is not None
        assert resolved == "gpt-4o"
        # The callable made it through resolve_provider_client â†’ _try_azure_foundry
        # â†’ OpenAI(api_key=...).
        assert callable(received["api_key"])
    ...
    def test_warns_and_returns_none_on_failure(
        ...
        clear warning pointing at ``zed doctor``."""
        import logging
        ...
        patch_load_config({
            ...
            "base_url": "https://r.openai.azure.com/openai/v1",
            "api_mode": "chat_completions",
            # No default â†’ resolver yields no model â†’ bail
        })
        ...
        assert resolved is None
        assert any(
            "azure-foundry" in rec.message and "zed doctor" in rec.message
