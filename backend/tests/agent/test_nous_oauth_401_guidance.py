...
to a generic "Your API key was rejected... run zed setup" message, which is
...
def test_nous_provider_is_in_oauth_401_set():
    ...
    # Be flexible about set element ordering â€” assert all three are listed
...
def test_nous_401_guidance_strings_present():
    ...
    # (Nous Portal has no API key path â€” auth_type=oauth_device_code only).
    assert "Nous Portal OAuth token was rejected" in source

    # Must give a concrete re-auth command, not a generic "zed setup".
    assert "zed portal" in source

    # Must point at the portal so users can check account/credit status.
    assert "portal.zedteam.com" in source
