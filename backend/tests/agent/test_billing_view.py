...
- Error-code â†’ typed-exception mapping (the live-verified contract matrix).
...
@pytest.mark.parametrize(
    "raw,expected",
    [
        ("142.5", Decimal("142.5")),   # decimal string, NOT 2dp â€” the headline case
        ("100", Decimal("100")),
        ("10000", Decimal("10000")),
        ("0.01", Decimal("0.01")),
        (250, Decimal("250")),
...
def test_state_owner_tier_parse():
    ...
    assert s.card is not None and s.card.masked == "visa Â·Â·Â·Â·4242"
...
# ---------------------------------------------------------------------------
# Error-code â†’ typed-exception mapping (live-verified contract)
# ---------------------------------------------------------------------------
...
def test_rate_limited_maps_with_retry_after(status):
    ...
    # Critically: a rate limit is NOT a generic BillingError-only â€” surfaces branch on type.
...
def test_other_403s_map_to_base_error_with_portal_url(error):
    ...
    # Not a scope/auth/rate subclass â€” the generic gate-denial path.
...
def test_portal_base_url_env_override(monkeypatch):
    monkeypatch.setenv("ZED_PORTAL_BASE_URL", "https://preview.example.com/")
    assert resolve_portal_base_url() == "https://preview.example.com"


def test_portal_base_url_falls_back_to_state(monkeypatch):
    monkeypatch.delenv("ZED_PORTAL_BASE_URL", raising=False)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("0", "greater than"),
        ("-5", "greater than"),
        ("10.005", "cent"),       # multipleOf 0.01 â€” sub-cent rejected
