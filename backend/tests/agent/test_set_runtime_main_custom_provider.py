...
Fixes https://github.com/ZedTeam/zed-agent/issues/34777
"""
import pytest
...
class TestResolveAutoCustomEndToEnd:
    """End-to-end routing assertions â€” build a *real* client (no mock on
    ...
    def test_config_less_custom_endpoint_routes_via_global(self, tmp_path, monkeypatch):
        ...
        set_runtime_main() must build a real client at that endpoint â€” not
        fall through to Step 2 (the regression in #34777)."""
        import agent.auxiliary_client as mod

        # Hermetic: no aggregator creds, no stale OPENAI_BASE_URL.
        for var in ("OPENROUTER_API_KEY", "ZED_API_KEY", "OPENAI_API_KEY",
                    "OPENAI_BASE_URL"):
        ...
        try:
            ...
            assert client is not None, (
                "config-less custom endpoint fell through to Step 2 â€” "
    ...
    def test_named_custom_with_config_entry_still_routes(self, tmp_path, monkeypatch):
        ...
        # No live base_url carried â€” resolution must come from config alone,
