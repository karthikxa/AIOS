...
- "consolidated" â€” absorbed into an umbrella; content still lives
  under another skill's files
- "pruned" â€” archived for staleness; content not preserved elsewhere
...
def test_classify_handles_malformed_arguments_string(curator_env):
    ...
    # even though json.loads fails â€” but it can't identify target="umbrella"
    ...
    # (This is a correctness floor â€” better to prune-label than hallucinate
...
def test_report_md_splits_consolidated_and_pruned_sections(curator_env):
    ...
    assert "Pruned â€” archived for staleness" in md
    assert "`absorbed-skill` â†’ merged into `umbrella`" in md
...
# ---------------------------------------------------------------------------
# _parse_structured_summary â€” extracting the model's required YAML block
# ---------------------------------------------------------------------------
...
def test_parse_structured_summary_happy_path(curator_env):
    text = (
        ...
        "  - from: openai-api\n"
        "    into: llm-providers\n"
        "    reason: same â€” merged with sibling\n"
        "prunings:\n"
...
# ---------------------------------------------------------------------------
# _reconcile_classification â€” merging model block with heuristic
# ---------------------------------------------------------------------------


def test_reconcile_model_wins_when_umbrella_exists(curator_env):
    """Model claim + umbrella in destinations â†’ model authority (with reason)."""
...
def test_reconcile_model_hallucinates_umbrella(curator_env):
    """Model names a non-existent umbrella â€” downgrade, prefer heuristic if any."""
    out = curator_env._reconcile_classification(
        removed=["thing"],
        heuristic={
...
def test_reconcile_model_hallucinates_with_no_heuristic_evidence(curator_env):
    """Model names a non-existent umbrella AND no tool-call evidence â†’ prune."""
    out = curator_env._reconcile_classification(
        removed=["ghost"],
...
def test_reconcile_model_block_visible_in_full_report(curator_env):
    """End-to-end: LLM final response with the YAML block â†’ reasons in REPORT.md."""
...
# ---------------------------------------------------------------------------
# _extract_absorbed_into_declarations â€” authoritative signal from delete calls
# ---------------------------------------------------------------------------
...
def test_extract_absorbed_into_missing_arg_ignored(curator_env):
    """Delete call without absorbed_into is skipped â€” fallback to heuristic."""
    declarations = curator_env._extract_absorbed_into_declarations([
        {
            "name": "skill_manage",
            "arguments": json.dumps({
...
def test_extract_absorbed_into_accepts_dict_arguments(curator_env):
    """arguments can arrive as a dict (defensive path) â€” still works."""
    declarations = curator_env._extract_absorbed_into_declarations([
        {
            "name": "skill_manage",
            "arguments": {
...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    """Model declared absorbed_into at delete; YAML/heuristic disagree â€” declaration wins.
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"zed-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "zed-agent-dev", "declared": True},
        },
    )
    assert len(out["consolidated"]) == 1
    assert out["pruned"] == []
...
def test_reconcile_without_declarations_preserves_legacy_behavior(curator_env):
    """Backward compat: no absorbed_declarations arg â†’ all existing logic intact."""
    out = curator_env._reconcile_classification(
        removed=["thing"],
        heuristic={
        ...
        model_block={"consolidations": [], "prunings": []},
        destinations={"umbrella"},
        # no absorbed_declarations â€” defaults to None â†’ behaves identically to pre-change
    )
    assert len(out["consolidated"]) == 1
...
# ---------------------------------------------------------------------------
# _build_rename_summary â€” surfaces the "where did my skills go?" map to the
# user-visible curator summary (gateway ðŸ’¾ line, CLI Rich panel,
# `zed curator status`). The full data has always been in REPORT.md on
...
def test_rename_summary_consolidation_shows_target(curator_env):
    """Consolidated skills render as `name â†’ umbrella` with the actual target."""
    result = curator_env._build_rename_summary(
    ...
    assert "pdf-extraction â†’ document-tools" in result
    assert "docx-extraction â†’ document-tools" in result
    assert "full report: zed curator status" in result
...
def test_rename_summary_pruned_marked_explicitly(curator_env):
    ...
    assert "old-flaky-thing â€” pruned (stale)" in result
    assert "â†’" not in result.split("old-flaky-thing")[1].splitlines()[0]


def test_rename_summary_caps_at_ten_with_more_indicator(curator_env):
    """Large consolidations don't blow up the log line â€” cap + `â€¦ and N more`."""
    ...
    assert "â€¦ and 5 more" in result
    # Exactly 10 bullets shown
    bullet_count = sum(1 for ln in result.splitlines() if ln.startswith("  â€¢ "))
    assert bullet_count == 10


def test_rename_summary_mixed_consolidation_and_pruning(curator_env):
    """Consolidated entries come first, pruned entries follow â€” matches REPORT.md ordering."""
    result = curator_env._build_rename_summary(
    ...
    assert "merge-me â†’ umbrella" in lines[merge_idx]
    assert "drop-me â€” pruned (stale)" in lines[drop_idx]


# ---------------------------------------------------------------------------
# Pin hint â€” surfaces `zed curator pin <umbrella>` in the rename block so
...
# one umbrella destination â€” pruned-only runs skip it.
# ---------------------------------------------------------------------------
...
def test_rename_summary_pin_hint_appears_when_consolidation_produced_umbrella(curator_env):
    ...
    assert "zed curator pin document-tools" in result
    assert "keep an umbrella stable" in result


def test_rename_summary_pin_hint_skipped_for_pruned_only_runs(curator_env):
    """Pruned-only runs have nothing surviving to pin â€” hint should not appear."""
    result = curator_env._build_rename_summary(
    ...
    assert "archived 2 skill(s):" in result
    assert "zed curator pin" not in result
...
def test_rename_summary_pin_hint_picks_one_umbrella_when_multiple_absorbed(curator_env):
    """Multiple umbrellas â†’ hint shows one example (alphabetically first), not a list."""
    result = curator_env._build_rename_summary(
    ...
    assert "zed curator pin umbrella-alpha" in result
