...
Covers:
- agent/google_oauth.py â€” PKCE, credential I/O with packed refresh format,
  token refresh dedup, invalid_grant handling, headless paste fallback
- agent/google_code_assist.py â€” project discovery, VPC-SC fallback, onboarding
  with LRO polling, quota retrieval
- agent/gemini_cloudcode_adapter.py â€” OpenAIâ†”Gemini translation, request
  envelope wrapping, response unwrapping, tool calls bidirectional, streaming
- Provider registration â€” registry entry, aliases, runtime dispatch, auth
...
# =============================================================================
# google_oauth.py â€” PKCE + packed refresh format
# =============================================================================
...
class TestClientCredResolution:
    def test_env_override(self, monkeypatch):
        ...
        monkeypatch.setenv("ZED_GEMINI_CLIENT_ID", "custom-id.apps.googleusercontent.com")
...
class TestAccessTokenExpired:
    ...
    def test_near_expiry_considered_expired(self):
        """60s skew â€” a token with 30s left is considered expired."""
        from agent.google_oauth import GoogleCredentials
...
class TestProjectIdResolution:
    ...
    def test_priority_order(self, monkeypatch):
        ...
        monkeypatch.setenv("ZED_GEMINI_PROJECT_ID", "higher-priority")
...
class TestHeadlessDetection:
    ...
    def test_detects_zed_headless(self, monkeypatch):
        from agent.google_oauth import _is_headless

        monkeypatch.setenv("ZED_HEADLESS", "1")
        assert _is_headless() is True
...
# =============================================================================
# google_code_assist.py â€” project discovery, onboarding, quota, VPC-SC
# =============================================================================
...
# =============================================================================
# gemini_cloudcode_adapter.py â€” request/response translation
# =============================================================================
...
class TestTranslateStreamEvent:
    def test_parallel_calls_to_same_tool_get_unique_indices(self):
        ...
        ``index`` â€” otherwise downstream aggregators collapse them into one.
        """
...
class TestGeminiHttpErrorParsing:
    ...
    def test_malformed_body_still_produces_structured_error(self):
        """Non-JSON body must not swallow status_code â€” we still want the classifier path."""
        from agent.gemini_cloudcode_adapter import _gemini_http_error
    ...
    def test_status_code_flows_through_error_classifier(self):
        ...
        This is the whole point of adding status_code to CodeAssistError â€”
...
class TestProviderRegistration:
    ...
    def test_google_gemini_alias_still_goes_to_api_key_gemini(self):
        """Regression guard: don't shadow the existing google-gemini â†’ gemini alias."""
    ...
    def test_oauth_capable_set_preserves_existing(self):
        from zed_cli.auth_commands import _OAUTH_CAPABLE_PROVIDERS
