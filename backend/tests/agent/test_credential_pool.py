...
def test_round_robin_strategy_rotates_priorities(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "zed" / "config.yaml"
...
def test_mark_exhausted_and_rotate_persists_status(tmp_path, monkeypatch):
    ...
    auth_payload = json.loads((tmp_path / "zed" / "auth.json").read_text())
...
def test_token_invalidated_marks_credential_dead(tmp_path, monkeypatch):
    ...
    again with the same 401 â€” surfacing as "Failed to generate context
    ...
    # Rotation still works â€” we hand off to the healthy credential.
    assert next_entry is not None
...
def test_dead_credential_never_re_enters_rotation_after_ttl(tmp_path, monkeypatch):
    ...
    A DEAD credential has no recovery TTL â€” it stays dead until either
    ...
    """
    monkeypatch.setenv("ZED_HOME", str(tmp_path / "zed"))
    # DEAD entry from 2 hours ago â€” well past the exhausted TTLs (5min/1h)
    ...
    pool = load_pool("openai-codex")
    selected = pool.select()
    # Should skip the dead entry and pick the healthy one â€” even though
    ...
    # The DEAD entry is still marked dead on disk â€” not cleared by TTL.
    auth_payload = json.loads((tmp_path / "zed" / "auth.json").read_text())
...
def test_429_rate_limit_still_uses_exhausted_not_dead(tmp_path, monkeypatch):
    ...
    auth_payload = json.loads((tmp_path / "zed" / "auth.json").read_text())
    persisted = auth_payload["credential_pool"]["openai-codex"][0]
    # 429 stays exhausted (transient) â€” NOT dead.
...
def test_generic_401_without_terminal_reason_still_uses_exhausted(tmp_path, monkeypatch):
    ...
    # 401 with no specific reason â€” stays exhausted, NOT dead.
...
def test_dead_manual_entry_pruned_after_24h(tmp_path, monkeypatch):
    ...
    window without losing recoverability â€” the user can always re-add
    via ``zed auth add``.
    """
    monkeypatch.setenv("ZED_HOME", str(tmp_path / "zed"))
...
def test_dead_manual_entry_kept_within_24h(tmp_path, monkeypatch):
    ...
    """
    monkeypatch.setenv("ZED_HOME", str(tmp_path / "zed"))
    # DEAD entry from only an hour ago â€” well within the 24h window
    ...
    # On-disk pool should still have BOTH entries â€” recent dead is preserved.
    auth_payload = json.loads((tmp_path / "zed" / "auth.json").read_text())
    persisted = auth_payload["credential_pool"]["openai-codex"]
...
def test_dead_singleton_seeded_entry_not_pruned(tmp_path, monkeypatch):
    ...
    every ``load_pool()``, so pruning them is pointless â€” they reappear
...
def test_load_pool_does_not_persist_env_seeded_secret_value(tmp_path, monkeypatch):
    ...
    auth_text = (tmp_path / "zed" / "auth.json").read_text()
...
def test_write_credential_pool_sanitizes_borrowed_payload_at_disk_boundary(tmp_path, monkeypatch):
    ...
    assert borrowed["source"] == "systemd://zed/openrouter"
...
def test_write_credential_pool_preserves_known_provider_owned_oauth_state(tmp_path, monkeypatch):
    ...
    persisted = json.loads((tmp_path / "zed" / "auth.json").read_text())["credential_pool"]["nous"][0]
...
def test_load_pool_prefers_dotenv_over_stale_os_environ(tmp_path, monkeypatch):
    ...
    from a parent shell) must NOT shadow the fresh key in ~/.zed/.env when
    ...
    # User edited ~/.zed/.env with the fresh key
...
def test_load_pool_falls_back_to_os_environ_when_dotenv_empty(tmp_path, monkeypatch):
    """When ~/.zed/.env does not define OPENROUTER_API_KEY (typical Docker /
    ...
    (zed_home / ".env").write_text("SOME_OTHER_VAR=unrelated\n")
...
def test_nous_pool_terminal_refresh_removes_device_code_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("ZED_HOME", str(tmp_path / "zed"))
    monkeypatch.setenv("ZED_SHARED_AUTH_DIR", str(tmp_path / "shared"))
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    assert {entry.source for entry in entries} == {"manual:zed_pkce", "zed_pkce"}
...
def test_load_pool_api_key_path_skips_oauth_autodiscovery(tmp_path, monkeypatch):
    ...
    When the user picks "Anthropic API key" at `zed setup`,
    ...
    + claude-cli user-agent).  Autodiscovered Claude Code / Zed PKCE
    tokens from other tools' credential files must NOT be silently mixed
    into the anthropic pool â€” otherwise rotation on a 401/429 could flip
    ...
    monkeypatch.setattr("agent.anthropic_adapter.read_zed_oauth_credentials", _fake_pkce)
...
def test_load_pool_api_key_path_prunes_stale_oauth_entries(tmp_path, monkeypatch):
    ...
    `zed_pkce` into auth.json) and later switches to the API key at
    `zed setup` would still have those OAuth entries dormant on disk.
...
def test_load_pool_oauth_path_still_autodiscovers(tmp_path, monkeypatch):
    ...
    OAuth path at `zed setup`.  When ANTHROPIC_TOKEN is set (and
...
def test_nous_seed_from_singletons_preserves_obtained_at_timestamps(tmp_path, monkeypatch):
        ...
        tmp_path,
        {
            ...
            "providers": {
                "nous": {
                    ...
                    "inference_base_url": "https://inference.zedteam.com/v1",
                    "token_type": "Bearer",
    ...
    # Direct dataclass fields â€” must survive the singleton â†’ pool copy.
    ...
    # Extra fields â€” this is what regressed.  These must be carried through
...
class TestLeastUsedStrategy:
    ...
    def test_request_count_increments(self):
        ...
        # First select should pick entry with lowest count (both 0 â†’ first)
        e1 = pool.select()
...
# â”€â”€ PR #10160 salvage: Nous OAuth cross-process sync tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ OpenAI Codex OAuth cross-process sync tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_sync_codex_entry_from_auth_store_adopts_newer_tokens(tmp_path, monkeypatch):
    ...
    # Simulate `zed auth openai-codex` replacing the token pair on disk.
...
def test_codex_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    ...
    rate-limit reset the user ran `zed model` to reauth, but the pool
    entry stayed marked EXHAUSTED with last_error_reset_at many hours in
    the future â€” so `_available_entries` kept returning empty and every
    ...
    # Simulate `zed model` / `zed auth` refreshing the tokens.
...
def test_codex_exhausted_entry_stays_stuck_without_auth_store_update(tmp_path, monkeypatch):
    ...
    entry must stay stuck behind its reset window â€” sync must not spuriously
    ...
    # auth.json unchanged â†’ sync returns same entry â†’ exhausted_until check
    # still skips it.
    available = pool._available_entries(clear_expired=True, refresh=False)
    assert available == []
...
def test_is_terminal_xai_oauth_refresh_error():
    ...
    # transient 429/5xx: relogin_required=False â†’ not terminal
