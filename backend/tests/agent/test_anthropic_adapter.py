"""Tests for agent/anthropic_adapter.py â€” Anthropic Messages API adapter."""
...
class TestWriteClaudeCodeCredentials:
    ...
    def test_credentials_file_created_with_0o600(self, tmp_path, monkeypatch):
        ...
        assert mode == 0o600, f"creds file mode {oct(mode)} != 0o600 â€” TOCTOU race regressed"
...
class TestRunOauthSetupToken:
    ...
    def test_returns_token_from_credential_files(self, monkeypatch, tmp_path):
        ...
        # Don't assert exact call count â€” the contract is "credentials flow
...
class TestConvertMessages:
    ...
    def test_strips_orphaned_tool_result(self):
        ...
        _, result = convert_messages_to_anthropic(messages)
        # tc_gone has no matching tool_use â€” its tool_result should be stripped
        for m in result:
    ...
    def test_empty_user_message_string_gets_placeholder(self):
        ...
        Regression test for #3143 â€” Discord @mention-only messages.
        """
        messages = [
            {"role": "user", "content": ""},
        ]
...
class TestBuildAnthropicKwargs:
    ...
    def test_reasoning_config_downgrades_xhigh_to_max_for_4_6_models(self):
        ...
        # xhigh/max). Opus 4.6 only supports low/medium/high/max â€” sending
        # "xhigh" there returns an API 400. Preserve the pre-migration
        # behavior of aliasing xhighâ†’max on pre-4.7 adaptive models so users
    ...
    def test_reasoning_config_preserves_xhigh_for_4_7_models(self):
        ...
        # coding/agentic work â€” keep it distinct from max.
        kwargs = build_anthropic_kwargs(
    ...
    def test_supports_fast_mode_predicate(self):
        """Fast mode is Opus 4.6 only â€” Opus 4.7 and others must be excluded.
    ...
    def test_fable_class_models_route_as_adaptive_thinking(self):
        ...
        contract â€” adaptive thinking, xhigh-capable, sampling-params-forbidden â€”
        ...
        # New / unknown Claude models â†’ modern contract by default.
        for m in (
            "claude-fable-5",
        ...
        # 1M-context reasoning model â†’ highest output ceiling.
    ...
    def test_fast_mode_still_applied_on_opus_46(self):
        """Regression guard â€” fast mode must still work on Opus 4.6."""
        kwargs = build_anthropic_kwargs(
            model="claude-opus-4-6",
            messages=[{"role": "user", "content": "hi"}],
            tools=None,
...
class TestThinkingBlockSignatureManagement:
    ...
    def test_redacted_thinking_without_data_dropped(self):
        """Redacted thinking without 'data' is dropped â€” can't be validated."""
        messages = [
            {
                "role": "assistant",
                "content": "Response.",
    ...
    def test_empty_content_after_strip_gets_placeholder(self):
        ...
        messages = [
            {
                ...
                "reasoning_details": [
                    ...
                    # Unsigned â€” will be downgraded, but content was empty string
                ],
            },
        ...
        # The original content was empty and thinking was unsigned â†’ placeholder
    ...
    def test_orphan_stripped_tool_use_demotes_dead_signed_thinking(self):
        ...
        comes back. On replay, the orphaned tool_use is stripped â€” which mutates
        ...
        # No signed thinking block survives â€” the signature is dead.
        assert not any(
    ...
    def test_signed_thinking_preserved_when_no_tool_use_stripped(self):
        ...
        This guards against the orphan-strip fix over-firing â€” when no tool_use
...
# ---------------------------------------------------------------------------
# max_tokens resolver â€” openclaw/openclaw#66664 port
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# convert_tools_to_anthropic â€” tool dedup at API boundary
# ---------------------------------------------------------------------------
...
class TestConvertToolsToAnthropicDedup:
    ...
    https://github.com/NousResearch/zed-agent/issues/18478
    """
