"""Tests for agent/curator.py â€” orchestrator, idle gating, state transitions.

LLM spawning is never exercised here â€” `_run_llm_review` is monkeypatched so
...
def curator_env(tmp_path, monkeypatch):
    """Isolated ZED_HOME + freshly reloaded curator + skill_usage modules."""
    home = tmp_path / ".zed"
    ...
    # Neutralize the real LLM pass by default â€” tests opt in per-case.
    monkeypatch.setattr(curator, "_run_llm_review", lambda prompt: "llm-stub")

    # Default: no config file â†’ curator defaults. Tests can override.
...
def test_first_run_defers(curator_env):
    ...
    """
    c = curator_env["curator"]
    # No state file â€” should defer and seed last_run_at.
    assert c.should_run_now() is False
    state = c.load_state()
...
def test_old_run_eligible(curator_env):
    ...
    default â€” bumping DEFAULT_INTERVAL_HOURS shouldn't break it."""
    c = curator_env["curator"]
...
def test_new_skill_without_last_used_not_immediately_archived(curator_env):
    ...
    # Bump nothing â€” record doesn't exist yet. Curator should create it
...
def test_prune_builtins_default_on(curator_env):
    ...
    c = curator_env["curator"]
    # _load_config returns {} (fixture) â†’ default True surfaces.
...
def test_prune_builtins_off_excludes_bundled(curator_env, monkeypatch):
    ...
    # Explicitly off â†’ bundled is not a candidate (the opt-out path).
...
def test_prune_builtins_seeds_clock_on_first_sight(curator_env, monkeypatch):
    ...
    # First pass: built-in has no record yet â†’ it's seeded, NOT archived,
...
def test_protected_builtin_never_archived_even_when_stale(curator_env, monkeypatch):
    ...
    stale bundled skill under prune_builtins â€” it backs a load-bearing slash
    ...
    counts = c.apply_automatic_transitions()
    assert counts["archived"] == 0
    # Not even enumerated as a candidate â†’ not "checked".
...
def test_dry_run_does_not_advance_state(curator_env, monkeypatch):
    ...
    `zed curator status`. Fixes #18373.
    """
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
...
def test_dry_run_injects_report_only_banner(curator_env, monkeypatch):
    ...
    call any mutating tool. This is defense in depth â€” the caller also
    skips automatic transitions â€” but the LLM prompt is the only guard
...
def test_dry_run_skips_automatic_transitions(curator_env, monkeypatch):
    """Dry-run must not call apply_automatic_transitions â€” the auto pass
...
def test_run_review_skips_llm_when_no_candidates(curator_env, monkeypatch):
    c = curator_env["curator"]
    # No skills in the dir â†’ no candidates
    calls = []
    monkeypatch.setattr(
        c, "_run_llm_review",
...
def test_consolidate_default_off(curator_env):
    """Consolidation (the LLM umbrella pass) is OFF by default â€” only the
...
def test_run_review_skips_llm_when_consolidate_off(curator_env, monkeypatch):
    ...
    prune but never spawns the LLM consolidation fork â€” even with candidates
...
def test_run_review_consolidate_override_runs_llm(curator_env, monkeypatch):
    ...
    the LLM consolidation pass â€” mirrors `zed curator run --consolidate`."""
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
    _write_skill(skills_dir, "a")
...
def test_maybe_run_curator_runs_when_eligible(curator_env, monkeypatch):
    ...
    _write_skill(skills_dir, "a")
    u.mark_agent_created("a")
    # Seed last_run_at far in the past so the interval gate opens â€” the
...
def test_maybe_run_curator_defers_on_fresh_install(curator_env):
    ...
    skills_dir = curator_env["home"] / "skills"
    _write_skill(skills_dir, "a")
    # Infinite idle â€” the only thing that should block the run is the new
...
def test_curator_review_prompt_points_at_existing_tools_only():
    ...
    # These would be nice but aren't actually registered as tools â€” the
...
def test_curator_review_prompt_is_umbrella_first():
    ...
    assert "umbrella" in lower, (
        "must use UMBRELLA framing â€” the class-first abstraction the curator "
...
def test_curator_review_prompt_offers_support_file_actions():
    ...
    # existing skill â€” this is the create-adjacent action the curator needs
...
# `zed model` â†’ auxiliary picker, the dashboard Models tab, and the full
...
def test_review_model_defaults_to_main_when_slot_is_auto(curator_env):
    """auxiliary.curator absent (or auto/empty) â†’ use main model.provider/model."""
...
def test_review_model_auxiliary_curator_partial_override_falls_back(curator_env):
    """Only one of slot provider/model set â†’ fall back to the main pair.
...
def test_review_model_legacy_curator_auxiliary_still_works(curator_env, caplog):
    """Pre-unification users set curator.auxiliary.{provider,model} â€” honor it.
...
def test_review_model_handles_missing_sections(curator_env):
    """Missing auxiliary/curator sections never raise â€” fall back cleanly."""
    curator = curator_env["curator"]
    ...
    # Completely empty config â†’ ("auto", "") â€” resolve_runtime_provider
...
def test_curator_slot_is_canonical_aux_task():
    ...
    # 2. zed_cli/main.py _AUX_TASKS â€” CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. zed_cli/web_server.py _AUX_TASK_SLOTS â€” REST API allowlist
