...
construction and tool registration â€” on a typical install that's 120+
...
@pytest.fixture
def zed_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.zed/`` with a config.yaml referencing one external dir."""
    home = tmp_path / ".zed"
    home.mkdir()
...
def test_returns_configured_external_dir(zed_home_with_config):
    _home, external, _cfg = zed_home_with_config
    result = get_external_skills_dirs()
    assert result == [external.resolve()]


def test_cache_reuses_result_without_reparsing(zed_home_with_config):
    """Subsequent calls hit the cache and skip YAML parsing entirely."""
    _home, _external, _cfg = zed_home_with_config

    # Prime cache
    get_external_skills_dirs()

    # Patch yaml_load to raise â€” if cache works, it's never called again.
    with patch.object(
        skill_utils,
        "yaml_load",
...
def test_cache_invalidates_on_mtime_change(zed_home_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = zed_home_with_config
...
def test_returns_empty_when_config_missing(tmp_path, monkeypatch):
    """No config file â†’ empty list, cached as empty."""
    home = tmp_path / ".zed"
    home.mkdir()
    monkeypatch.setenv("ZED_HOME", str(home))
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_returned_list_is_a_copy(zed_home_with_config):
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different ZED_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".zed"
    ...
    monkeypatch.setenv("ZED_HOME", str(home_a))
    assert get_external_skills_dirs() == [ext_a.resolve()]

    monkeypatch.setenv("ZED_HOME", str(home_b))
    assert get_external_skills_dirs() == [ext_b.resolve()]

    # And switching back still works â€” both entries coexist in the cache.
    monkeypatch.setenv("ZED_HOME", str(home_a))
    assert get_external_skills_dirs() == [ext_a.resolve()]
