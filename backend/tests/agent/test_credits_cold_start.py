...
ZED_DEV_CREDITS_FIXTURE) so depletion AND the 90% grant warning fire immediately
...
def test_cold_start_opens_already_at_90pct_warns():
    """A session that OPENS already â‰¥90% must warn immediately â€” the seed primes
...
def test_cold_start_grant_exhausted_grant_spent_only():
    """Cap reached but top-up funds remain â†’ grant_spent info notice ONLY.
...
def test_cold_start_depleted_warns():
    ...
    assert s.used_fraction is None  # no cap â†’ no %, depletion keys off paid_access
    assert _cold_start_notices(s) == ["credits.depleted"]


def test_cold_start_debt_warns_and_depleted():
    """Negative subscription balance (the only signed field) â†’ 100% used + depleted."""
    s = _state(
...
def test_cold_start_no_cap_degrades_to_depletion_only():
    """Without monthly_credits (older portals) the seed sets no limit â†’ used_fraction
    None â†’ only depletion can fire, never warn90."""
...
def test_dev_fixtures_drive_cold_start():
    """Every ZED_DEV_CREDITS_FIXTURE state produces a valid seed CreditsState."""
    import os
    ...
    for name, want in expected.items():
        os.environ["ZED_DEV_CREDITS"] = "1"  # fixtures gate on the dev flag
        os.environ["ZED_DEV_CREDITS_FIXTURE"] = name
        try:
            fx = dev_fixture_credits_state()
            assert fx is not None, name
            assert _cold_start_notices(fx) == want, (name, _cold_start_notices(fx))
        finally:
            os.environ.pop("ZED_DEV_CREDITS_FIXTURE", None)
            os.environ.pop("ZED_DEV_CREDITS", None)


# â”€â”€ seed_credits_at_session_start: the shared session-open hydrator â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _seed(agent, fixture):
    ...
    os.environ["ZED_DEV_CREDITS"] = "1"  # fixtures gate on the dev flag
    os.environ["ZED_DEV_CREDITS_FIXTURE"] = fixture
    try:
...
def test_seed_depleted_suppressed_on_free_model():
    ...
    the depleted banner â€” inference works fine on the free tier."""
