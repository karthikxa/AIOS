...
byte-stable across turns (fresh AIAgent â†’ must restore from session DB
instead of rebuilding).  Covers:
  ...
  * Silent-failure recovery paths:
      - DB read raises â†’ WARNING + fresh build
      - Row has system_prompt=NULL â†’ WARNING + fresh build
      - Row has system_prompt="" â†’ WARNING + fresh build
      - DB write fails â†’ WARNING (subsequent turns will miss cache)
"""
...
class TestStoredPromptReuse:
    def test_present_row_is_reused_verbatim(self, caplog):
        """Continuing session with a stored prompt â†’ reuse byte-for-byte."""
        stored = "Stored prompt from turn 1 â€” byte-identical reuse"
        db = MagicMock()
    ...
    def test_present_row_with_unicode_preserved(self):
        ...
        stored = "Stored prompt with unicode: â˜¤ âš— â—† â€” and emoji ðŸ¦Š"
        db = MagicMock()
        db.get_session.return_value = {"system_prompt": stored}
        agent = _make_agent(session_db=db)
...
class TestLegitimateFreshBuild:
    def test_no_history_skips_db_and_builds_fresh(self, caplog):
        """First turn with empty history â†’ build fresh, don't touch the DB."""
        db = MagicMock()
        agent = _make_agent(session_db=db)
        ...
        # No history â†’ DB read skipped entirely
...
# ---------------------------------------------------------------------------
# Silent-failure recovery â€” these are the new A/B logging paths
# ---------------------------------------------------------------------------


class TestSilentFailureWarnings:
    def test_db_read_exception_warns_and_rebuilds(self, caplog):
        """DB read raising â†’ WARNING + fall through to fresh build."""
        db = MagicMock()
    ...
    def test_null_system_prompt_warns_about_unusable_stored_state(self, caplog):
        """Row exists but system_prompt is NULL â†’ WARNING + fresh build."""
        db = MagicMock()
    ...
    def test_empty_system_prompt_warns_about_silent_persistence_bug(self, caplog):
        """Row exists but system_prompt is '' â†’ WARNING about silent write bug."""
        db = MagicMock()
    ...
    def test_db_write_failure_warns_loudly(self, caplog):
        """update_system_prompt raising â†’ WARNING (was DEBUG before)."""
        db = MagicMock()
    ...
    def test_no_history_with_null_row_does_not_warn(self, caplog):
        """First turn (no history) hitting a null row is not surprising â€” no warn."""
        db = MagicMock()
        db.get_session.return_value = {"system_prompt": None}
        agent = _make_agent(session_db=db)

        with caplog.at_level(logging.WARNING, logger="agent.conversation_loop"):
            # Empty history â†’ DB read is skipped entirely
            _restore_or_build_system_prompt(agent, None, [])
...
class TestPromptStabilityInvariant:
    def test_restored_prompt_is_byte_identical_to_stored(self):
        """The restored prompt must equal the stored bytes exactly â€” no
        ...
        # Identity check â€” must be the same object reference for maximum
