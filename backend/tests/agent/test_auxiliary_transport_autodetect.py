...
endpoint only speaks Anthropic Messages â€” sending ``kimi-for-coding`` over
...
@pytest.mark.parametrize("url,expected,label", [
    ...
    ("https://inference-api.zedteam.com/v1", False, "Nous"),
    ("", False, "empty"),
    (None, False, "None"),
])
...
def test_maybe_wrap_anthropic_skips_openai_wire_urls():
    ...
    plain_client = MagicMock(name="plain_openai")
    # No patch on build_anthropic_client â€” if the function tried to call it,
...
def test_resolve_provider_client_kimi_coding_wraps_anthropic(monkeypatch, tmp_path):
    ...
    assert isinstance(client, AnthropicAuxiliaryClient), (
        ...
        "Messages â€” aux client MUST be AnthropicAuxiliaryClient, got "
