"""Tests for ZED_HOME credential-file read blocking in file_safety.

Regression for https://github.com/NousResearch/zed-agent/issues/17656 â€”
``read_file`` was previously only sandboxed against ``ZED_HOME`` itself,
...
for the credential stores while leaving arbitrary ``ZED_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_zed_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "zed_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_zed_home_path", lambda: home)
    return home
...
def test_google_oauth_json_blocked(fake_home):
    """Gemini OAuth tokens live under auth/google_oauth.json â€” blocked."""
    from agent.file_safety import get_read_block_error
...
def test_arbitrary_zed_home_file_not_blocked(fake_home):
    """Non-credential files inside ZED_HOME stay readable."""
    from agent.file_safety import get_read_block_error
...
def test_skills_hub_block_still_applies(fake_home):
    ...
    assert err is not None
    assert "internal Zed cache file" in err


def test_path_traversal_resolves_to_blocked(fake_home, tmp_path):
    """A path that traverses through a sibling dir back into ZED_HOME's
    auth.json must still be caught â€” the check resolves through realpath."""
    from agent.file_safety import get_read_block_error

    _create(fake_home, "auth.json")
    sibling = tmp_path / "elsewhere"
    sibling.mkdir()
    traversal = sibling / ".." / "zed_home" / "auth.json"
...
def test_symlink_to_auth_json_blocked(fake_home, tmp_path):
    """A symlink pointing at ZED_HOME/auth.json from outside the home
    must be blocked â€” readlink-resolution catches the indirection."""
    from agent.file_safety import get_read_block_error
...
def test_read_file_tool_blocks_relative_path_under_terminal_cwd(
    ...
    ``read_file_tool`` against ``TERMINAL_CWD == ZED_HOME`` must still
    ...
    _create(fake_home, "auth.json")
    # Force the file_tools resolver to anchor relative paths at ZED_HOME
...
def test_dotenv_blocked(fake_home):
    """.env in ZED_HOME holds API keys â€” blocked."""
    from agent.file_safety import get_read_block_error
...
def test_webhook_subscriptions_blocked(fake_home):
    """webhook_subscriptions.json holds per-route HMAC secrets â€” blocked."""
    from agent.file_safety import get_read_block_error
...
def test_mcp_tokens_file_blocked(fake_home):
    """Files under mcp-tokens/ hold OAuth tokens â€” blocked."""
    from agent.file_safety import get_read_block_error
...
def test_identically_named_zed_files_outside_home_not_blocked(
    fake_home, tmp_path
):
    """Zed-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside ZED_HOME must remain readable â€” the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside ZED_HOME â€” readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside ZED_HOME should NOT be blocked"
    )
...
def test_config_yaml_not_blocked(fake_home):
    """config.yaml is NOT a credential file â€” agent should still be
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, ZED_HOME = <root>/profiles/<name>, but
    <root>/auth.json must ALSO be blocked â€” credentials at root are
    ...
    monkeypatch.setattr(fs, "_zed_home_path", lambda: profile)
