"""Tests for agent/skill_commands.py â€” skill slash command scanning and platform filtering."""
...
class TestScanSkillCommands:
    ...
    def test_get_skill_commands_rescans_when_platform_scope_changes(self, tmp_path):
        ...
        def _disabled_skills():
            platform = os.getenv("ZED_PLATFORM")
        ...
        with (
            ...
            with patch.dict(os.environ, {"ZED_PLATFORM": "telegram"}):
            ...
            with patch.dict(os.environ, {"ZED_PLATFORM": "discord"}):
            ...
            # Switching back to telegram must also rescan â€” not re-serve
    ...
    def test_get_skill_commands_rescans_when_session_platform_changes(self, tmp_path):
        """``ZED_SESSION_PLATFORM`` from the gateway session context must
        also trigger a rescan, not just ``ZED_PLATFORM`` (#14536).

        Exercises the real ContextVar path: the gateway sets the active
        adapter via ``set_session_vars(platform=...)`` and the resolver
        reads it via ``get_session_env``. Setting ``ZED_SESSION_PLATFORM``
        in ``os.environ`` would only test ``get_session_env``'s legacy
        env-var fallback â€” a regression that swapped ``get_session_env``
    ...
    def test_get_skill_commands_rescans_when_leaving_platform_scope(self, tmp_path, monkeypatch):
        ...
        def _disabled_skills():
            if os.getenv("ZED_PLATFORM") == "telegram":
                return {"telegram-only"}
            return set()
        ...
        with (
            ...
            monkeypatch.setenv("ZED_PLATFORM", "telegram")
            telegram_commands = dict(get_skill_commands())
            assert "/telegram-only" not in telegram_commands

            # Drop back to no platform scope â€” bare CLI / cron / RL rollouts.
            monkeypatch.delenv("ZED_PLATFORM", raising=False)
    ...
    def test_get_skill_commands_does_not_rescan_when_platform_unchanged(self, tmp_path):
        ...
        with (
            ...
            patch.object(sc_mod, "_skill_commands", {}),
            patch.object(sc_mod, "_skill_commands_platform", None),
            patch.dict(os.environ, {"ZED_PLATFORM": "telegram"}),
        ):
            _make_skill(tmp_path, "shared")
            # Prime the cache.
            get_skill_commands()
...
class TestTemplateVarSubstitution:
    """``${ZED_SKILL_DIR}`` and ``${ZED_SESSION_ID}`` in SKILL.md body
    ...
    def test_substitutes_skill_dir(self, tmp_path):
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            skill_dir = _make_skill(
                tmp_path,
                "templated",
                body="Run: node ${ZED_SKILL_DIR}/scripts/foo.js",
            )
            scan_skill_commands()
        ...
        assert "${ZED_SKILL_DIR}" not in msg.split("[Skill directory:")[0]
    ...
    def test_leaves_session_id_token_when_missing(self, tmp_path):
        ...
        assert msg is not None
        # No session â€” token left intact so the author can spot it.
        assert "Session: ${ZED_SESSION_ID}" in msg
    ...
    def test_disable_template_vars_via_config(self, tmp_path):
        ...
        assert "${ZED_SKILL_DIR}/scripts/foo.js" in msg


class TestInlineShellExpansion:
    """Inline ``!`cmd`` snippets in SKILL.md run before the agent sees the
    content â€” but only when the user has opted in via config."""
    ...
    def test_inline_shell_is_off_by_default(self, tmp_path):
        ...
        assert msg is not None
        # Default config has inline_shell=False â€” snippet must stay literal.
    ...
    def test_inline_shell_timeout_does_not_break_message(self, tmp_path):
        ...
        # The command's intended stdout never made it through â€” only the
