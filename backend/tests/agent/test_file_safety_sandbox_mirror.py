...
Those paths look like ``â€¦/sandboxes/<backend>/<task>/home/.zed/â€¦`` and
they accumulate divergent copies of authoritative profile state (SOUL.md,
config.yaml, memories/*.md) because the host Zed process never reads
them. Soft guard â€” defense in depth, NOT a security boundary.

Reference: #32049 â€” under ``terminal.backend: docker``, the agent's
...
# ---------------------------------------------------------------------------
# classify_sandbox_mirror_target â€” pure path-shape detection
# ---------------------------------------------------------------------------
...
class TestClassifySandboxMirrorTarget:
    def test_docker_mirror_soul_md_classified(self, tmp_path):
        ...
        target = (
            tmp_path
            / "profiles" / "group1"
            / "sandboxes" / "docker" / "default" / "home" / ".zed"
    ...
    def test_other_backends_and_inner_files_match(self, tmp_path, backend, inner):
        """The detector is backend-agnostic â€” sandbox-mirror shape is what matters."""
        from agent.file_safety import classify_sandbox_mirror_target

        target = (
            tmp_path
            / "sandboxes" / backend / "task-42" / "home" / ".zed"
            / Path(inner)
        )
    ...
    def test_path_outside_sandbox_returns_none(self, tmp_path):
        """A plain Zed path is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target

        target = tmp_path / ".zed" / "profiles" / "group1" / "SOUL.md"
        target.parent.mkdir(parents=True)
        target.write_text("# real SOUL\n")

        assert classify_sandbox_mirror_target(str(target)) is None

    def test_sandboxes_segment_without_home_zed_returns_none(self, tmp_path):
        """A ``sandboxes/`` directory unrelated to Zed-state mirroring (e.g.
    ...
    def test_sandboxes_segment_with_home_but_no_zed_returns_none(self, tmp_path):
        """``sandboxes/<backend>/<task>/home/anything-not-zed`` is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target
    ...
    def test_truncated_sandbox_path_returns_none(self, tmp_path):
        """``â€¦/sandboxes/<backend>/<task>`` without ``home/.zed/<thing>`` is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target
    ...
    def test_non_existent_path_still_classifies_by_shape(self, tmp_path):
        """Detection is path-shape only â€” it must not require the file to exist
...
# ---------------------------------------------------------------------------
# get_sandbox_mirror_warning â€” the model-facing string
# ---------------------------------------------------------------------------
...
class TestGetSandboxMirrorWarning:
    ...
    def test_mirror_warning_names_mirror_root_and_inner_path(self, tmp_path):
        ...
        assert "sandboxes/docker/default/home/.zed" in warn
...
class TestSandboxMirrorIsOrthogonalToCrossProfile:
    ...
    in-profile from the host's view â€” the bug is the mirror, not the
    profile mismatch."""

    def test_same_profile_mirror_still_flagged(self, tmp_path, monkeypatch):
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_zed_root_path", lambda: tmp_path)
        monkeypatch.setattr(fs, "_zed_home_path", lambda: tmp_path / "profiles" / "group1")
