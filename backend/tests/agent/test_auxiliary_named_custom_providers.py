...
def _isolate(tmp_path, monkeypatch):
    """Redirect ZED_HOME and clear module caches."""
...
def _write_config(tmp_path, config_dict):
    """Write a config.yaml to the test ZED_HOME."""
    import yaml
...
class TestResolveProviderClientMainAlias:
    ...
    def test_main_resolves_github_copilot_alias(self, tmp_path):
        ...
        with (
            patch("zed_cli.auth.resolve_api_key_provider_credentials", return_value={
...
class TestCustomProviderAliasCollision:
    ...
    *alias* (not a canonical provider) must win over the built-in.
    ...
    rewrote ``kimi`` â†’ ``kimi-coding`` before the named-custom lookup.
    """
