"""Tests for agent/curator_backup.py â€” snapshot + rollback of the skills tree."""
...
def backup_env(monkeypatch, tmp_path):
    """Isolate ZED_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".zed"
    home.mkdir()
    (home / "skills").mkdir()
...
def test_snapshot_excludes_backups_dir_itself(backup_env):
    """The backup must NOT contain .curator_backups/ â€” that would recurse
...
def test_rollback_is_itself_undoable(backup_env):
    ...
    is a real tarball with reason='pre-rollback to <id>' â€” it's
...
def test_rollback_rejects_unsafe_tarball(backup_env, monkeypatch):
    ...
    if someone crafts a malicious snapshot. Defense in depth â€” normal
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd zed_constants
    from agent import curator
    importlib.reload(curator)

    # Stub out LLM review and auto transitions â€” we only care about the
...
def test_dry_run_skips_snapshot(backup_env, monkeypatch):
    """Dry-run previews must not spend disk on a snapshot â€” they don't
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under ZED_HOME. Returns the path.
...
def test_snapshot_without_cron_jobs_file_still_succeeds(backup_env):
    """No cron/jobs.json on disk â†’ snapshot succeeds, manifest records absence."""
    cb = backup_env["cb"]
    _write_skill(backup_env["skills"], "alpha")
    # Deliberately do not create ~/.zed/cron/jobs.json
...
def test_rollback_restores_cron_skill_links(backup_env):
    ...
    rewrite to [umbrella], then rollback â†’ skills restored to [alpha,beta]."""
    cb = backup_env["cb"]
...
def test_rollback_only_touches_skill_fields(backup_env):
    ...
    Schedule, enabled, prompt, timestamps â€” all live state, hands off."""
    cb = backup_env["cb"]
    home = backup_env["home"]
    _write_skill(backup_env["skills"], "alpha")

    # Hand-rolled jobs.json with varied fields (no real create_job â€” we want
    # exact field control).
...
def test_rollback_skips_jobs_the_user_deleted(backup_env):
    ...
    NOT resurrect it â€” the user's delete is a later, explicit choice."""
    cb = backup_env["cb"]
    home = backup_env["home"]
    _write_skill(backup_env["skills"], "alpha")
...
def test_rollback_with_snapshot_missing_cron_succeeds(backup_env):
    ...
    # No cron/jobs.json at snapshot time â€” simulates a pre-feature snapshot
    snap = cb.snapshot_skills(reason="test")
    assert snap is not None
    assert not (snap / cb.CRON_JOBS_FILENAME).exists()
...
def test_rollback_does_not_delete_the_snapshot_it_restores_from(backup_env, monkeypatch):
    """The snapshot a rollback restores from must still exist afterwards â€” the
