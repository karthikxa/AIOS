...
class TestCompress:
    ...
    def test_compression_increments_count(self, compressor):
        ...
        # Default config (abort_on_summary_failure=False) â€” fallback path
...
class TestNonStringContent:
    ...
    def test_none_content_coerced_to_empty(self):
        ...
        with patch("agent.context_compressor.call_llm", return_value=mock_response):
            summary = c._generate_summary(messages)
        # None content â†’ empty string â†’ standardized compaction handoff prefix added
...
class TestSummaryFallbackToMainModel:
    ...
    giving up â€” losing N turns of context is almost always worse than one
    ...
    def test_model_not_found_404_falls_back_to_main_and_succeeds(self):
        ...
        a model the main provider doesn't serve â†’ 404 â†’ retry on main."""
        mock_ok = MagicMock()
        ...
        # Second call used the main model (no model kwarg â†’ call_llm uses main)
        assert "model" not in mock_call.call_args_list[1].kwargs
        assert result is not None
        assert "summary via main model" in result
        # Aux-model failure is recorded even though retry succeeded â€” this is
    ...
    def test_unknown_error_falls_back_to_main_and_succeeds(self):
        ...
        # A 400 from OpenRouter / Nous portal with an opaque message â€” does
    ...
    def test_no_fallback_when_summary_model_equals_main_model(self):
        ...
        to â€” go straight to cooldown, don't loop retrying the same call."""
        ...
        # Only one attempt â€” retry gate blocks fallback when models match
        assert mock_call.call_count == 1
        assert result is None
        # Not flagged as fallen back â€” the retry condition was never met
        assert getattr(c, "_summary_model_fallen_back", False) is False

    def test_fallback_only_happens_once_per_compressor(self):
        """If the retry-on-main ALSO fails, don't loop forever â€” enter
    ...
    def test_json_decode_error_substring_match_in_wrapped_exception(self):
        ...
        # A plain Exception with the canonical JSON decode error text â€” what
    ...
    def test_json_decode_error_on_main_uses_short_cooldown(self):
        ...
        30s cooldown, not the default 60s â€” provider bodies tend to
        ...
        with patch("agent.context_compressor.get_model_context_length", return_value=100000):
            c = ContextCompressor(
                model="main-model",
                # No summary_model_override â†’ already on main, no fallback path.
                quiet_mode=True,
            )
...
class TestStreamingClosedFallback:
    ...
    inside its body).  We test the *wiring* â€” that `_generate_summary` calls
    ``_is_connection_error`` and acts on its result â€” not the classifier itself
    ...
    def test_streaming_closed_on_main_uses_short_cooldown(self):
        ...
        the 30s cooldown, not the default 60s â€” these errors are transient."""
        err = Exception("RemoteProtocolError: response ended prematurely")

        with patch("agent.context_compressor.get_model_context_length", return_value=100000):
            c = ContextCompressor(
                model="main-model",
                # No summary_model_override â†’ no fallback path.
                quiet_mode=True,
            )
...
class TestAuxModelFallbackSurfacedToCallers:
    ...
    broken auxiliary.compression.model config â€” silent recovery would hide
    ...
    def test_compress_exposes_aux_failure_fields_after_successful_fallback(self):
        ...
        # Recovery succeeded â†’ no fallback placeholder
...
class TestAbortOnSummaryFailure:
    ...
    summary-generation failure ABORTS compression entirely â€” returns the
...
class TestCompressWithClient:
    ...
    def test_user_role_summary_carries_end_marker(self):
        ...
        # existing consecutive-user test) â†’ role resolves to "user".
        msgs = [
    ...
    def test_summary_role_avoids_consecutive_user_messages(self):
        ...
        # Last head message (index 1) is "assistant" â†’ summary should be "user".
        # With min_tail=3, tail = last 3 messages (indices 5-7).
        # head_last=assistant, tail_first=assistant â†’ summary_role="user", no collision.
    ...
    def test_summary_role_avoids_consecutive_user_when_head_ends_with_user(self):
        ...
        # Last head message (index 2) is "user" â†’ summary should be "assistant"
    ...
    def test_summary_role_flips_to_avoid_tail_collision(self):
        ...
        # Default: tool â†’ summary_role="user" â†’ collides with tail.
        # Flip to "assistant" â†’ toolâ†’assistant is fine.
        msgs = [
            {"role": "user", "content": "msg 0"},
    ...
    def test_double_collision_merges_summary_into_tail(self):
        ...
        # Head: [system, user, assistant]  â†’  last head = assistant
        # Tail: [user, assistant, user]    â†’  first tail = user
        # summary_role="user" collides with tail, "assistant" collides with head â†’ merge
        # NOTE: protect_first_n=2 preserves 2 non-system messages in addition to
    ...
    def test_double_collision_user_head_assistant_tail(self):
        ...
        summary='assistant' collides with tail, 'user' collides with head â†’ merge."""
        mock_response = MagicMock()
        mock_response.choices = [MagicMock()]
        mock_response.choices[0].message.content = "summary text"
        ...
        # Head: [system, user]        â†’ last head = user
        # Tail: [assistant, user, assistant] â†’ first tail = assistant
        # summary_role="assistant" collides with tail, "user" collides with head â†’ merge
    ...
    def test_no_collision_scenarios_still_work(self):
        ...
        # Head=assistant, Tail=assistant â†’ summary_role="user", no collision.
        # With min_tail=3, tail = last 3 messages (indices 5-7).
        # Need 8 messages: min_for_compress = 2+3+1 = 6, must have > 6.
        msgs = [
            {"role": "user", "content": "msg 0"},
...
class TestSummaryTargetRatio:
    ...
    def test_default_protect_first_n_is_3(self):
        ...
        new semantics, the constructor default is 3 â€” the system prompt is
    ...
    def test_protect_first_n_override(self):
        """protect_first_n=0 should be honoured â€” for users who rely on rolling
    ...
    def test_protect_first_n_0_preserves_only_system_prompt(self):
        ...
        sessions â€” no user/assistant turn gets pinned verbatim forever just
        ...
        # as head verbatim â€” those would have been summarized or absorbed.
    ...
    def test_protect_first_n_semantics_stable_without_system_prompt(self):
        ...
        in both paths â€” "N non-system head messages" â€” so configuring
        ...
        session forever â€” a user-reported complaint that a week-old
        ...
        # No system prompt â€” this is what the gateway passes to compress().
        msgs = [
            {"role": "user" if i % 2 == 0 else "assistant", "content": f"msg {i}"}
            for i in range(10)
        ]
        head_size = c._protect_head_size(msgs)
        # With no system prompt and protect_first_n=0 â†’ head is empty.
        # The first user message is NOT pinned as head.
        assert head_size == 0

        # And with protect_first_n=3 on the same no-system-prompt list â†’
...
class TestTokenBudgetTailProtection:
    ...
    def test_large_tool_outputs_no_longer_block_compaction(self, budget_compressor):
        ...
        # Add 20 messages with large tool outputs (~5K chars each â‰ˆ 1250 tokens)
        for i in range(10):
            messages.append({
                "role": "assistant", "content": None,
        ...
        assert tail_size < 20, f"Tail {tail_size} messages â€” large tool outputs are blocking compaction"
    ...
    def test_soft_ceiling_allows_oversized_message(self, budget_compressor):
        ...
        c = budget_compressor
        # Set a small budget â€” 500 tokens
    ...
    def test_multimodal_message_accumulates_text_chars_not_block_count(self, budget_compressor):
        ...
        at index 1 has 500 chars of text â†’ 135 tokens (correct) or 10 tokens (bug).
        ...
        """
        c = budget_compressor
        # 500 chars â†’ 500//4 + 10 = 135 tokens; len([text, image]) // 4 + 10 = 10 (bug)
        big_text = "x" * 500
        multimodal_content = [
        ...
        # With the bug: head_end safeguard fires â†’ cut = n - min_tail = 3, only 3 in tail.
        assert len(messages) - cut >= 4, (
            f"Expected â‰¥4 messages in tail (got {len(messages) - cut}, cut={cut}). "
            "The multimodal message was underestimated â€” len(list) used instead of text chars."
        )
    ...
    def test_plain_string_content_unchanged(self, budget_compressor):
        ...
        # the walk stops at index 1 and tail has 4 messages â€” same as the fix path.
        big_plain = "x" * 500
        messages = [
        ...
        cut = c._find_tail_cut_by_tokens(messages, head_end)
        assert len(messages) - cut >= 4, (
            f"Plain string regression: expected â‰¥4 messages in tail, got {len(messages) - cut}"
        )
    ...
    def test_image_only_block_contributes_zero_text_chars(self, budget_compressor):
        ...
        messages = [
            ...
            {"role": "user", "content": image_only},   # 0 text chars â†’ 10 tokens overhead
            {"role": "assistant", "content": "ok"},
        ]
        head_end = 0
        cut = c._find_tail_cut_by_tokens(messages, head_end)
    ...
    def test_mixed_list_with_bare_strings_does_not_crash(self, budget_compressor):
        """Content list may contain bare strings (not dicts) â€” must not raise AttributeError."""
        c = budget_compressor
        c.tail_token_budget = 500
        # Bare string item alongside a dict item â€” normalisation elsewhere allows this.
    ...
    def test_generous_budget_protects_everything_floor_does_not_override(
        ...
        """A budget that covers the whole transcript must prune nothing â€”
        ...
        assert pruned == 0, (
            ...
            f"{pruned} messages â€” protect_tail_count is acting as a ceiling, "
            "not a minimum floor"
        )
...
class TestUpdateModelBudgets:
    ...
    def test_tail_budget_recalculated(self):
        ...
        assert comp.tail_token_budget < old_tail, "smaller context â†’ smaller budget"
...
class TestTruncateToolCallArgsJson:
    ...
    shape matches the original â€” shrunken, not corrupted.
    """
    ...
    def test_shrunken_args_remain_valid_json(self):
        ...
        original = _json.dumps({
            "path": "~/.zed/skills/shopping/browser-setup-notes.md",
        ...
        assert parsed["path"] == "~/.zed/skills/shopping/browser-setup-notes.md"
    ...
    def test_pass3_emits_valid_json_for_downstream_provider(self):
        ...
        # Must parse â€” otherwise downstream provider returns 400
