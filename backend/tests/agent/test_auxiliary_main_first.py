"""Regression tests for the ``auto`` â†’ main-model-first policy.
...
and surprising â€” users picked Claude but got Gemini Flash summaries.
...
# â”€â”€ Text aux tasks â€” _resolve_auto â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestResolveAutoMainFirst:
    ...
    def test_openrouter_main_uses_main_model_for_aux(self, monkeypatch):
        """OpenRouter main user â†’ aux uses their picked OR model, not Gemini Flash."""
    ...
    def test_nous_main_uses_main_model_for_aux(self, monkeypatch):
        """Nous Portal main user â†’ aux uses their picked Nous model, not free-tier MiMo."""
        # No OPENROUTER_API_KEY â†’ ensures if main failed we'd fall to chain
        with patch(
    ...
    def test_non_aggregator_main_still_uses_main(self, monkeypatch):
        """Non-aggregator main (DeepSeek) â†’ unchanged behavior, main model used."""
    ...
    def test_main_unavailable_falls_through_to_chain(self, monkeypatch):
        """Main provider with no working client â†’ fall back to aux chain."""
    ...
    def test_no_main_config_uses_chain_directly(self):
        """No main provider configured â†’ skip step 1, use chain (no regression)."""
        chain_client = MagicMock()
        with patch(
...
# â”€â”€ Vision â€” resolve_vision_provider_client â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestResolveVisionMainFirst:
    ...
    def test_openrouter_main_vision_uses_main_model(self, monkeypatch):
        """OpenRouter main with vision-capable model â†’ aux vision uses main model."""
        monkeypatch.setenv("OPENROUTER_API_KEY", "or-key")
    ...
    def test_nous_main_vision_uses_paid_nous_vision_backend(self):
        """Paid Nous main â†’ aux vision uses the dedicated Nous vision backend."""
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="nous",
        ), patch(
            "agent.auxiliary_client._read_main_model",
    ...
    def test_nous_main_vision_uses_free_tier_nous_vision_backend(self):
        """Free-tier Nous main â†’ aux vision uses MiMo omni, not the text main model."""
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="nous",
        ), patch(
            "agent.auxiliary_client._read_main_model",
    ...
    def test_exotic_provider_with_vision_override_preserved(self):
        """xiaomi â†’ mimo-v2.5 override still wins over main_model."""
        with patch(
    ...
    def test_main_unavailable_vision_falls_through_to_aggregators(self):
        """Main provider fails â†’ fall back to OpenRouter/Nous strict backends."""
    ...
    def test_explicit_provider_override_still_wins(self):
        ...
        # Explicit "nous" override â†’ uses strict backend, NOT main model path
        assert provider == "nous"
        mock_strict.assert_called_once_with("nous", None)


# â”€â”€ Constant cleanup â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
