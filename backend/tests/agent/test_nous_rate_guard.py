"""Tests for agent/nous_rate_guard.py â€” cross-session Nous Portal rate limit guard."""
...
def rate_guard_env(tmp_path, monkeypatch):
    ...
    zed_home = str(tmp_path / ".zed")
    os.makedirs(zed_home, exist_ok=True)
...
class TestAuxiliaryClientIntegration:
    ...
    def test_try_nous_works_when_not_rate_limited(self, rate_guard_env, monkeypatch):
        ...
        # No rate limit recorded â€” _try_nous should proceed normally
...
class TestIsGenuineNousRateLimit:
    ...
    Nous Portal multiplexes upstreams (DeepSeek, Kimi, MiMo, Zed).
