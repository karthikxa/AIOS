...
class TestUserInstalledProviderDiscovery:
    """Memory providers installed to $ZED_HOME/plugins/ should be found.
    ...
    def test_load_user_plugin(self, tmp_path, monkeypatch):
        """load_memory_provider() can load from $ZED_HOME/plugins/."""
        from plugins.memory import load_memory_provider
        self._make_user_memory_plugin(tmp_path, "myexternal")
        monkeypatch.setattr(
            "plugins.memory._get_user_plugins_dir",
    ...
    def test_load_user_plugin_with_relative_import(self, tmp_path, monkeypatch):
        ...
        synthetic ``_zed_user_memory.<name>`` package but never registered
        that parent namespace in sys.modules, so any relative import inside
        the plugin raised
        ``ModuleNotFoundError: No module named '_zed_user_memory'``.
        """
        from plugins.memory import load_memory_provider
    ...
    def test_load_user_plugin_with_nested_subpackage(self, tmp_path, monkeypatch):
        ...
        impl_dir = plugin_dir / "adapters" / "zed"  # adapters/ has no __init__.py
        ...
        (plugin_dir / "__init__.py").write_text(
            "from .adapters.zed import MyProvider\n"
            "def register(ctx):\n"
...
class TestUserInstalledProviderCli:
    ...
    ``_zed_user_memory.<name>.cli`` without registering the parent
...
class TestSetupFieldFiltering:
    ...
    memory setup wizard in zed_cli/memory_setup.py.

    These features are generic â€” any memory plugin can use them in
...
class TestFlattenMessageContent:
    ...
    plain string before reaching providers â€” a raw list crashes their regex
...
# ---------------------------------------------------------------------------
# AIAgent.commit_memory_session â€” routes to MemoryManager.on_session_end
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# on_memory_write bridge â€” must fire from both concurrent AND sequential paths
# ---------------------------------------------------------------------------
...
class TestOnMemoryWriteBridge:
    ...
    def test_on_memory_write_remove_not_bridged(self):
        """The bridge intentionally skips 'remove' â€” only add/replace notify."""
        ...
        # Manager itself doesn't filter â€” run_agent.py does.
    ...
    def test_memory_manager_tool_injection_deduplicates(self):
        ...
        mnemosyne_stats in tools array â†’ 400 from Nous Portal.
        """
        mgr = MemoryManager()
        ...
        # (as if it was registered via ctx.register_tool â†’ get_tool_definitions)
        existing_tools = [
...
class TestMemoryToolToolsetGate:
    ...
    to AIAgent.tools unconditionally in agent_init.py â€” bypassing the
    ...
    causing 10x latency on local models (Qwen3-30B: 1.7s â†’ 42s) and
    ...
    The gate condition is:
        ...
        selected toolsets include memory â†’ user opted in, inject
        otherwise (incl. [])            â†’ skip injection
    """
    ...
    def test_none_toolsets_injects(self):
        """enabled_toolsets=None (no filter) injects memory tools â€” backward compat."""
    ...
    def test_composite_toolset_with_memory_injects(self):
        ...
        tools, names = self._run_memory_injection(["zed-acp"], mgr)
    ...
    def test_multiple_schemas_all_blocked_together(self):
        """When the gate is closed, no memory tools leak â€” not even partially."""
...
class TestContextEngineToolsetGate:
    ...
    def test_none_toolsets_injects(self):
        """enabled_toolsets=None injects context-engine tools â€” backward compat."""
