"""Tests for agent.models_dev â€” models.dev registry integration."""
...
class TestFetchModelsDev:
    ...
    def test_stale_disk_cache_falls_through_to_network(self, mock_get):
        ...
        # Disk cache exists but is older than the TTL â€” must NOT short-circuit.
    ...
    def test_force_refresh_skips_disk_cache(self, mock_get):
        ...
        disk-cache fast path. Used by ``zed config refresh`` and
...
# ---------------------------------------------------------------------------
# get_model_capabilities â€” vision via modalities.input
# ---------------------------------------------------------------------------
