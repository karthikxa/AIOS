"""Tests for agent/model_metadata.py â€” token estimation, context lengths,
probing, caching, and error parsing.

Coverage levels:
  Token estimation       â€” concrete value assertions, edge cases
  Context length lookup  â€” resolution order, fuzzy match, cache priority
  API metadata fetch     â€” caching, TTL, canonical slugs, stale fallback
  Probe tiers            â€” descending, boundaries, extreme inputs
  Error parsing          â€” OpenAI, Ollama, Anthropic, edge cases
  Persistent cache       â€” save/load, corruption, update, provider isolation
"""
...
class TestEstimateTokensRough:
    ...
    def test_unicode_multibyte(self):
        """Unicode chars are still 1 Python char each â€” 4 chars/token holds."""
        text = "ä½ å¥½ä¸–ç•Œ"  # 4 CJK characters
...
class TestDefaultContextLengths:
    ...
    def test_openrouter_live_metadata_beats_hardcoded_catchall(self):
        ...
        with mock_patch("agent.model_metadata.fetch_model_metadata", return_value=live), \
            ...
            # A genuinely-200k model still resolves to its real OR value â€” the
    ...
    def test_openrouter_kimi_32k_underreport_still_guarded(self):
        ...
        a bogus 32768 from OpenRouter for a Kimi slug must NOT win â€” it falls
...
class TestCodexOAuthContextLength:
    ...
    def test_non_codex_providers_unaffected(self):
        ...
        272k override â€” OpenRouter / direct OpenAI API have different limits.
        """
        from agent.model_metadata import get_model_context_length

        # OpenRouter â€” should hit its own catalog path first; when mocked
        # empty, falls through to hardcoded DEFAULT_CONTEXT_LENGTHS (1.05M,
        # matching the real direct-API value â€” Codex OAuth's 272k cap is
    ...
    def test_fresh_codex_cache_under_400k_is_respected(self, tmp_path, monkeypatch):
        """Codex entries at the correct 272k must NOT be invalidated â€”
...
class TestNousPortalContextResolution:
    ...
    Invariants this class pins down:
      ...
      3. OR-fallback values are NEVER persisted â€” otherwise a single portal
      ...
      5. Pre-fix persistent-cache entries SURVIVE on disk when the portal
         is unreachable â€” no opportunistic invalidation that loses the only
         value we have.
    """
    ...
    def test_openrouter_fallback_is_not_persisted(
        ...
        keeps working â€” but we must NOT write the OR value to disk.  Once
        ...
        assert not cache_file.exists() or not yaml.safe_load(
            ...
            "OR-fallback values must NOT be persisted â€” a single portal blip "
    ...
    def test_stale_cache_is_bypassed_and_overwritten_by_portal(
        ...
        """Users upgrading from pre-fix builds have ``qwen3.6-plus@â€¦nousâ€¦ =
        1000000`` (OR-derived) sitting in their cache file.  Step 1 must
        NOT short-circuit on that entry â€” step 5b reconciles against the
    ...
    def test_stale_cache_survives_when_portal_unreachable(
        ...
        on-disk cache entry, the entry must survive untouched â€” we don't
...
# =========================================================================
# get_model_context_length â€” resolution order
# =========================================================================
...
class TestGetModelContextLength:
    ...
    def test_api_missing_context_length_key(self, mock_fetch):
        """Model in API but without context_length â†’ defaults to the top
    ...
    def test_no_base_url_skips_cache(self, mock_fetch, tmp_path):
        ...
        with patch("agent.model_metadata._get_context_cache_path", return_value=cache_file):
            ...
            # No base_url â†’ cache skipped â†’ falls to probe tier
...
# =========================================================================
# Bedrock context resolution â€” must run BEFORE custom-endpoint probe
# =========================================================================
...
class TestBedrockContextResolution:
    ...
    the custom-endpoint probe at step 2 ran first â€” fetching ``/models`` from
...
# =========================================================================
# _strip_provider_prefix â€” Ollama model:tag vs provider:model
# =========================================================================
...
class TestStripProviderPrefix:
    ...
    def test_ollama_model_tag_not_mangled_in_context_lookup(self, mock_fetch):
        ...
        We mock a custom endpoint that knows 'qwen3.5:27b' â€” the full name
...
# =========================================================================
# fetch_model_metadata â€” caching, TTL, slugs, failures
# =========================================================================
...
class TestFetchModelMetadata:
    ...
    def test_malformed_json_no_data_key(self, mock_get):
        """API returns JSON without 'data' key â€” empty cache, no crash."""
        self._reset_cache()
        mock_response = MagicMock()
...
class TestGrok43StaleCacheGuard:
    ...
    def test_grok_4_not_clobbered(self, tmp_path, monkeypatch):
        ...
        importlib.reload(mm)
        base = "https://api.x.ai/v1"
        # 256,000 is the CORRECT value for plain grok-4 â€” guard must not touch it.
