...
assistant turn can emit content blocks in an interleaved order::
    ...
    thinking_1 (signed) Â· tool_use_1 Â· thinking_2 (signed) Â· tool_use_2
...
splits the turn into two *parallel* lists â€” ``reasoning_details`` (thinking
blocks) and ``tool_calls`` (tool_use blocks) â€” discarding the cross-type
...
def _stored_assistant_message(normalized) -> dict:
    ...
    run_agent.py L1513-1516 and zed_state.py.
    """
...
class TestInterleavedThinkingBlockOrder:
    ...
    def test_interleaved_order_preserved_on_replay(self):
        ...
        thinking blocks, producing
            thinking_1 Â· thinking_2 Â· tool_use_1 Â· tool_use_2
        instead of the original
            thinking_1 Â· tool_use_1 Â· thinking_2 Â· tool_use_2
        """
...
class TestInterleavedReplayCredentialRedaction:
    ...
    the wire â€” even though it is redacted everywhere else in history. The fix
