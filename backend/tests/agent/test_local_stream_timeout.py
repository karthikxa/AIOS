...
default 60s to ZED_API_TIMEOUT (1800s) to avoid premature connection
...
class TestLocalStreamReadTimeout:
    ...
    def test_local_endpoint_bumps_read_timeout(self, base_url):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZED_STREAM_READ_TIMEOUT", None)
            _base_timeout = float(os.getenv("ZED_API_TIMEOUT", 1800.0))
            _stream_read_timeout = float(os.getenv("ZED_STREAM_READ_TIMEOUT", 120.0))
    ...
    def test_user_override_respected_for_local(self):
        """User sets ZED_STREAM_READ_TIMEOUT -> keep their value even for local."""
        with patch.dict(os.environ, {"ZED_STREAM_READ_TIMEOUT": "300"}, clear=False):
            _base_timeout = float(os.getenv("ZED_API_TIMEOUT", 1800.0))
            _stream_read_timeout = float(os.getenv("ZED_STREAM_READ_TIMEOUT", 120.0))
...
class TestIsLocalEndpoint:
    ...
    @pytest.mark.parametrize("url", [
        ...
        "http://zed-litellm:8080",
        "http://vllm:8000",
    ])
    def test_unqualified_docker_hostnames(self, url):
        """Unqualified hostnames (no dots) are local â€” Docker Compose, /etc/hosts, etc."""
        assert is_local_endpoint(url) is True
