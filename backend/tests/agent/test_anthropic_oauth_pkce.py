...
History:
  - PR #1775 first fixed this on ``run_zed_oauth_login()``.
  - PR #2647 (b17e5c10) added ``run_zed_oauth_login_pure()`` and silently
...
def _patch_oauth_flow(
    ...
    *,
    ...
    """Wire up monkeypatches that let ``run_zed_oauth_login_pure()`` run
...
def test_authorization_url_state_is_not_pkce_verifier(monkeypatch, tmp_path):
    ...
    Referer headers, and auth-server access logs â€” defeating RFC 7636.
    """
    ...
    from agent.anthropic_adapter import run_zed_oauth_login_pure

    result = run_zed_oauth_login_pure()
    ...
    assert state_in_url != verifier_sent, (
        "PKCE code_verifier was reused as OAuth state â€” regression of #10693 / "
    ...
    assert verifier_sent not in url, (
        "PKCE verifier leaked into authorization URL â€” regression of #10693"
    )
...
def test_callback_state_mismatch_aborts(monkeypatch, tmp_path, caplog):
    ...
    crafted ``<code>#<state>`` string can complete the token exchange â€” the
    CSRF protection that ``state`` is supposed to provide (RFC 6749 Â§10.12)
    would be absent.
    """
    monkeypatch.setenv("ZED_HOME", str(tmp_path))
