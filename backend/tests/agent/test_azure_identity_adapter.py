...
# Ensure we always import a fresh adapter module â€” credential caches in
...
class TestMaterializeBearerForHttp:
    """The only helper that mints a real bearer JWT â€” must call the
...
# ---------------------------------------------------------------------------
# build_bearer_http_client â€” the Anthropic-on-Foundry bridge
# ---------------------------------------------------------------------------
...
class TestBuildBearerHttpClient:
    ...
    def test_hook_overrides_authorization_header(self):
        ...
        try:
            ...
            # The static-key headers must be stripped â€” sending both
    ...
    def test_hook_strips_auth_headers_and_warns_when_token_provider_fails(self, caplog):
        ...
        login expired), the hook must:
          ...
          2. Strip any pre-set Authorization headers â€” including the
        ...
        def bad_provider():
            return ""  # empty token â†’ materialize_bearer_for_http raises
        ...
        try:
            ...
            # Pre-set auth headers stripped â€” no sentinel makes it to Azure.
...
class TestIsTokenProvider:
    ...
    def test_string_is_not_token_provider(self):
        ...
        # â€” confirm they're never classified as token providers.
...
class TestEntraIdentityConfig:
    """The serializable config that crosses multiprocessing boundaries â€”
    ...
    def test_from_dict_ignores_legacy_identity_keys(self):
        ...
        tenant_id / authority should not crash from_dict â€” those values
        ...
        # Legacy keys silently ignored â€” no crash, no surprise field on the dataclass.
...
class _FakeAzureIdentity:
    ...
    def DefaultAzureCredential(self, **kwargs):  # noqa: N802 â€” match SDK
...
class TestBuildCredential:
    def test_default_kwargs_are_minimal(self, fake_azure_identity):
        ...
        azure-identity directly), not Zed config kwargs."""
        ...
        # Default config should produce empty kwargs â€” SDK uses its own
...
class TestBuildTokenProvider:
    ...
    def test_falls_back_to_default_scope_when_unspecified(self, fake_azure_identity):
        ...
        ``build_token_provider`` uses ``SCOPE_AI_AZURE_DEFAULT`` â€”
...
class TestHasAzureIdentityCredentials:
    ...
    def test_lazy_install_triggered_when_package_missing(self, monkeypatch):
        ...
        lazy-install path before bailing â€” otherwise the wizard's
...
# ---------------------------------------------------------------------------
# describe_active_credential â€” used by zed doctor + zed auth
# ---------------------------------------------------------------------------
