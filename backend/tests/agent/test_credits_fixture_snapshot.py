"""Tests for _snapshot_from_credits_state â€” the dev-fixture /usage renderer.

``build_nous_credits_snapshot`` maps a live portal account; ``_snapshot_from_credits_state``
maps a header-shaped CreditsState (e.g. a ZED_DEV_CREDITS_FIXTURE) into the SAME
...
def test_renders_gauge_magnitudes_and_fixture_marker():
    # used_fraction = (20 - 10) / 20 = 0.5  â†’ a 50%-used gauge window
...
def test_no_cap_yields_no_gauge_window():
    # No subscription cap â†’ used_fraction is None â†’ no gauge window, magnitudes only.
    snap = _snapshot_from_credits_state(_state(
