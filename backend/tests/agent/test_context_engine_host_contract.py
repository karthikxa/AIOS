...
engine plugins (e.g. zed-lcm) rely on:

1. ``_transition_context_engine_session`` drives the full lifecycle
   (on_session_end â†’ on_session_reset â†’ on_session_start â†’ optional
...
def test_transition_runs_full_lifecycle_in_order():
    """End â†’ reset â†’ start â†’ carry_over, in that order, when all inputs apply."""
    events: list[str] = []
    engine = MagicMock()
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``zed plugins`` picker."""
    from zed_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "zed_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "zed_cli.plugins.get_plugin_context_engine",
...
def test_discover_context_engines_dedupes_by_name(monkeypatch):
    ...
    engines = plugins_cmd._discover_context_engines()
    # Only one entry â€” the repo-shipped one. Description is preserved.
