...
on BOTH paths â€” otherwise the main turn succeeds but auxiliary calls to the
...
def _isolate(tmp_path, monkeypatch):
    """Redirect ZED_HOME so load_config() reads our test config.yaml."""
...
def _write_config(tmp_path, config_dict):
    import yaml
    (tmp_path / ".zed" / "config.yaml").write_text(yaml.dump(config_dict))
...
class TestAuxClientHonorsUserDefaultHeaders:
    ...
    def test_custom_provider_no_override_sends_no_user_agent(self, tmp_path):
        """Without config, the aux client injects nothing â€” SDK defaults apply."""
        _write_config(tmp_path, {
            "model": {
                "default": "my-custom-model",
                "provider": "custom",
    ...
    def test_named_custom_provider_honors_override(self, tmp_path):
        ...
        `model.provider: custom` path â€” both must apply the global override.
        """
        _write_config(tmp_path, {
            "model": {
                "default": "test-model",
