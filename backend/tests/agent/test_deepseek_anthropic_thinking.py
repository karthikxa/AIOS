...
blocks that Zed synthesises from ``reasoning_content``.
...
class TestDeepSeekAnthropicPreservesThinking:
    ...
    def test_unsigned_thinking_block_survives_replay(self, base_url: str) -> None:
        ...
        assert len(thinking_blocks) == 1, (
            ...
            "blocks synthesised from reasoning_content â€” upstream rejects "
            "replayed tool-call messages without them."
        )
        assert thinking_blocks[0]["thinking"] == "planning the tool call"
        # Synthesised block â€” never has a signature
    ...
    def test_signed_anthropic_thinking_block_is_stripped(self) -> None:
        ...
        DeepSeek issues its own signatures and cannot validate Anthropic's â€”
        ...
        assert thinking_blocks == [], (
            "Signed Anthropic thinking blocks must be stripped on DeepSeek â€” "
    ...
    def test_cache_control_stripped_from_thinking_block(self) -> None:
        ...
        as ignored â€” cache markers interfere with signature validation on
        upstreams that do check them, so Zed strips them everywhere.
        """
        from agent.anthropic_adapter import convert_messages_to_anthropic
    ...
    def test_openai_compat_deepseek_base_is_not_matched(self) -> None:
        ...
        DeepSeek /anthropic branch â€” it never reaches this adapter, but the
    ...
    def test_non_deepseek_third_party_still_strips_all_thinking(self) -> None:
        ...
        assert thinking_blocks == [], (
            ...
            "strip-all-thinking behaviour â€” unsigned blocks get rejected."
        )
