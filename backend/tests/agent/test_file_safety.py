"""Tests for agent/file_safety.py read guards â€” env file blocking.
...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_env_example(self):
        """"The .env.example file is explicitly allowed â€” it's documentation, not a secret."""
    ...
    def test_allowed_zed_env(self):
        """Zed' own .env inside ZED_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: zed internal .env is in ~/.zed/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional â€”
        # even ~/.zed/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.zed/.env"))
        assert error is not None
...
# ---------------------------------------------------------------------------
# Existing cache-file blocking (regression â€” must still work)
# ---------------------------------------------------------------------------


class TestCacheFileReadBlocking:
    """Internal Zed cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        zed_home = tmp_path / ".zed"
        cache = zed_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._zed_home_path", return_value=zed_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Zed cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        zed_home = tmp_path / ".zed"
        hub = zed_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_zed_home(self, tmp_path):
        """The env basename guard does not depend on ZED_HOME resolution."""
        zed_home = tmp_path / ".zed"
        zed_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        zed_home = tmp_path / ".zed"
        cache = zed_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
