...
It does NOT invalidate the skills system-prompt cache â€” skills are invoked
...
def zed_home(monkeypatch):
    """Isolate ZED_HOME for ``reload_skills`` tests.
    ...
    module-level ``ZED_HOME`` / ``SKILLS_DIR`` constants in place so the
    isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="zed-reload-skills-")
    monkeypatch.setenv("ZED_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "ZED_HOME", home, raising=False)
...
class TestReloadSkillsHelper:
    ...
    def test_returns_expected_keys(self, zed_home):
    ...
    def test_detects_newly_added_skill_with_description(self, zed_home):
        from agent.skill_commands import reload_skills, get_skill_commands

        # Prime the cache so subsequent diff is meaningful
        get_skill_commands()

        _write_skill(zed_home / "skills", "demo", "a demo skill")
        result = reload_skills()
    ...
    def test_detects_removed_skill_carries_description(self, zed_home):
        from agent.skill_commands import reload_skills

        skill_dir = _write_skill(zed_home / "skills", "demo", "soon to be gone")
        # First reload: demo present
        first = reload_skills()
        assert first["total"] == 1
        assert first["added"] == [{"name": "demo", "description": "soon to be gone"}]

        # Remove and reload â€” the description must survive the removal diff
    ...
    def test_description_passes_through_verbatim(self, zed_home):
        """``description`` must be the full SKILL.md frontmatter string â€” no
        ...
        long_desc = "x" * 200
        _write_skill(zed_home / "skills", "longdesc", long_desc)

        result = reload_skills()
        assert len(result["added"]) == 1
        assert result["added"][0]["description"] == long_desc

    def test_unchanged_skills_appear_in_unchanged_list(self, zed_home):
        from agent.skill_commands import reload_skills, get_skill_commands

        _write_skill(zed_home / "skills", "alpha")
        # Prime cache
        get_skill_commands()
    ...
    def test_does_not_invalidate_prompt_cache_snapshot(self, zed_home):
        """reload_skills must NOT delete the skills prompt-cache snapshot.

        Skills are called at runtime â€” the system prompt doesn't need to
        mention them for the model to use them â€” so reloading them should
        ...
        assert snapshot.exists(), (
            "prompt cache snapshot should be preserved â€” skills don't live "
