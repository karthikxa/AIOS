...
process leak â€” without the atexit hook, ``zed chat`` exits while
...
def test_atexit_shutdown_calls_shutdown_service(monkeypatch):
    ...
    swallows any exception â€” by the time atexit fires, the user has
...
def test_shutdown_service_idempotent(monkeypatch):
    """Calling shutdown twice must be safe â€” first call cleans up,
...
def test_shutdown_service_swallows_exception(monkeypatch):
    """An exception during ``svc.shutdown()`` must not propagate â€”
...
def test_get_service_returns_none_for_inactive_service(monkeypatch):
    ...
    not running â€” callers see ``None`` and fall back."""
    fake_svc = MagicMock()
    ...
    # Subsequent call returns None too â€” but the inactive instance is
