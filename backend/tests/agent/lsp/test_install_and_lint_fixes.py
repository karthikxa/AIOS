...
2. ``zed lsp status`` surfaces a ``Backend warnings`` section when
   bash-language-server is installed but ``shellcheck`` is missing.
3. ``_check_lint`` returns ``skipped`` (not ``error``) when the linter
   command exists on PATH but couldn't actually run â€” e.g. ``npx tsc``
...
def test_typescript_recipe_includes_typescript_sdk():
    ...
    assert "typescript" in extras, (
        ...
        "sibling install â€” without it `initialize` fails with "
...
def test_install_npm_passes_extras_to_npm_command(tmp_path, monkeypatch):
    ...
    def fake_run(cmd, **kwargs):
        captured["cmd"] = cmd
        # Pretend npm succeeded but binary doesn't exist â€” install code
...
def test_install_npm_works_without_extras(tmp_path, monkeypatch):
    ...
    install_targets = [c for c in cmd if not c.startswith("-") and c not in {
        "install", "--prefix", str(install_mod.zed_lsp_bin_dir().parent),
        "/usr/bin/npm",
    }]
...
def test_existing_binary_finds_windows_wrapper_in_staging(tmp_path, monkeypatch):
    ...
    wrapper = install_mod.zed_lsp_bin_dir() / "pyright-langserver.cmd"
...
def test_install_pip_finds_windows_scripts_launcher(tmp_path, monkeypatch):
    ...
    def fake_run(cmd, **kwargs):
        scripts_dir = install_mod.zed_lsp_bin_dir().parent / "python-packages" / "Scripts"
    ...
    assert (install_mod.zed_lsp_bin_dir() / "fake-language-server.exe").exists()


# ---------------------------------------------------------------------------
# Fix 2: ``zed lsp status`` surfaces shellcheck-missing for bash
# ---------------------------------------------------------------------------


def test_backend_warnings_quiet_when_bash_not_installed(tmp_path, monkeypatch):
    """No bash â†’ no warning."""
    monkeypatch.setenv("ZED_HOME", str(tmp_path))
...
def test_npx_tsc_missing_treated_as_skipped():
    ...
    semantic tier (gated on ``success or skipped``) is skipped â€” the user
