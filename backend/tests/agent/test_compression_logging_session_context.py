...
which moves ``ZED_SESSION_ID`` env + ContextVar). The ``[session_id]`` tag on
log lines comes from a SEPARATE mechanism â€” ``zed_logging._session_context``
...
def test_logging_session_context_follows_compression_rotation(tmp_path: Path) -> None:
    ...
    zed_logging.set_session_context(parent_sid)
    try:
        messages = [{"role": "user", "content": f"m{i}"} for i in range(20)]
        agent._compress_context(messages, "sys", approx_tokens=120_000)

        # The id actually rotated (sanity â€” otherwise the assertion is vacuous).
        assert agent.session_id != parent_sid

        # The logging context must now match the NEW id, not the stale one.
        current = getattr(zed_logging._session_context, "session_id", None)
