"""Tests for evaluate_credits_notices â€” pure threshold reconciliation policy (L4.1).
...
def state_with_fraction(
    ...
    *,
    ...
    # We want (limit - sub) / limit == uf  â†’  sub = limit * (1 - uf)
...
# â”€â”€ Scenario 1: crossing 90% threshold â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestWarn90Crossing:
    ...
    def test_crossing_to_90_fires_once(self):
        latch = fresh_latch()
        # First call: uf < 0.5 â€” sets seen_below_90 (below lowest band)
        s1 = state_with_fraction(0.10)
        evaluate_credits_notices(s1, latch)
        # Second call: uf >= 0.9 â€” should fire the usage band at 90
    ...
    def test_no_refire_on_repeated_over_90(self):
        ...
        # Third call: still â‰¥ 0.9 â€” must NOT re-fire
        to_show, to_clear = evaluate_credits_notices(s_over, latch)
        assert all(n.key != "credits.usage" for n in to_show)
        assert "credits.usage" not in to_clear


# â”€â”€ Scenario 2: recovery + re-cross â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestWarn90RecoveryReCross:
    def test_recovery_clears_warn90(self):
        ...
        # Recovery: uf drops back below ALL bands â†’ usage notice clears entirely
    ...
    def test_recross_after_recovery_fires_again(self):
        ...
        # Re-cross: uf >= 0.9 again â€” should fire again because the band is clearable
        to_show, to_clear = evaluate_credits_notices(state_with_fraction(0.95), latch)
        keys = [n.key for n in to_show]
        assert "credits.usage" in keys


# â”€â”€ Scenario 3: open-already-over (hybrid Q3 gate) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


class TestOpenAlreadyOver:
    def test_warn90_does_not_fire_without_seen_below_90(self):
        """First call ufâ‰¥0.9 with seen_below_90=False â€” warn90 must NOT fire."""
        latch = fresh_latch()
...
# â”€â”€ Scenario 3b: boundary â€” exact 0.9 and just-below-1.0 â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestBoundaryFractions:
    def test_exact_0_9_fires_warn90(self):
        ...
        # â†’ used = 18_000_000 / 20_000_000 = 0.9 exactly
        s = CreditsState(
    ...
    def test_just_below_1_0_does_not_fire_grant_spent(self):
        ...
        s = CreditsState(
            subscription_limit_micros=limit,
            subscription_limit_usd="20.00",
            subscription_micros=1,           # limit - 1 â†’ used_fraction < 1.0
            denominator_kind="subscription_cap",
...
# â”€â”€ Scenario 4: grant_spent â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestGrantSpent:
    ...
    def test_grant_spent_fires_on_first_obs(self):
        """No crossing gate for grant_spent â€” fires immediately on first obs."""
        latch = fresh_latch()
    ...
    def test_grant_spent_clears_when_purchased_zero(self):
        latch = fresh_latch()
        evaluate_credits_notices(self._grant_state(), latch)
        # Now purchased â†’ 0: grant_cond becomes False
...
# â”€â”€ Scenario 5: depleted + recovery â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Scenario 5b: free-model suppression of the depleted notice â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestDepletedFreeModelSuppression:
    ...
    def test_switch_to_free_model_clears_without_restored(self):
        latch = fresh_latch()
        # Depleted on a paid model â†’ notice fires
        evaluate_credits_notices(CreditsState(paid_access=False), latch)
        assert "credits.depleted" in latch["active"]
        # Same depleted account, but now on a free model â†’ clear, NO "restored"
    ...
    def test_switch_back_to_paid_model_while_depleted_reshows(self):
        ...
        # Back on a paid model, still depleted â†’ notice re-fires
        to_show, to_clear = evaluate_credits_notices(CreditsState(paid_access=False), latch)
        keys = [n.key for n in to_show]
        assert "credits.depleted" in keys
        assert "credits.depleted" in latch["active"]
    ...
    def test_genuine_recovery_on_free_model_no_spurious_restored(self):
        """Recovery observed while suppressed (notice never shown) â†’ nothing to
    ...
    def test_genuine_recovery_still_emits_restored_when_notice_active(self):
        """paid_access flip back to True with the notice showing â†’ clear + restored
...
# â”€â”€ Scenario 5c: is_free_tier_model (local-data-only check) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestIsFreeTierModel:
    def test_free_suffix_is_free(self):
        ...
        assert is_free_tier_model("Zed-4-70B:free", "https://inference-api.zedteam.com") is True

    def test_empty_or_paid_model_is_not_free(self):
        from agent.credits_tracker import is_free_tier_model

        assert is_free_tier_model("") is False
        assert is_free_tier_model("Zed-4-405B") is False
    ...
    def test_pricing_cache_peek_zero_priced_model(self, monkeypatch):
        ...
        assert is_free_tier_model("some/zero-priced", "https://inference-api.zedteam.com/") is True
        assert is_free_tier_model("some/zero-priced", "https://inference-api.zedteam.com/v1/") is True
    ...
    def test_cache_miss_is_not_free_and_no_fetch(self, monkeypatch):
        ...
        assert is_free_tier_model("some/model", "https://inference-api.zedteam.com/v1") is False
    ...
    def test_exception_fails_open_to_false(self, monkeypatch):
        ...
        assert is_free_tier_model("some/model", "https://inference-api.zedteam.com") is False


# â”€â”€ Scenario 6: denominator none (uf is None) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestDenominatorNone:
    ...
    def test_warn90_clears_when_uf_becomes_none(self):
        ...
        latch = fresh_latch()
        # Establish usage notice active: cross below â†’ above
        evaluate_credits_notices(state_with_fraction(0.10), latch)
        evaluate_credits_notices(state_with_fraction(0.95), latch)
...
# â”€â”€ Scenario 7: copy / verbatim USD strings â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Scenario 8: severity order in a single call â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestSeverityOrder:
    def test_multiple_new_notices_ordered_ascending_severity(self):
        ...
        (usage is suppressed here: purchased>0 â€” see TestTopUpSuppression.
        ...
        # grant_cond: subscription_cap + uf >= 1.0 + purchased > 0 âœ“
        # depleted_cond: not paid_access âœ“
...
# â”€â”€ Scenario 8b: top-up suppression of the usage gauge â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestTopUpSuppression:
    ...
    def test_topup_landing_mid_session_clears_active_band(self):
        """A showing 90% warn must clear when a top-up lands (purchased 0 â†’ >0)."""
        latch = fresh_latch()
        evaluate_credits_notices(state_with_fraction(0.10), latch)
        evaluate_credits_notices(state_with_fraction(0.95), latch)
    ...
    def test_band_resumes_after_topup_spent(self):
        """purchased back to 0 with usage still in-band â†’ gauge resumes."""
        latch = fresh_latch()
        evaluate_credits_notices(state_with_fraction(0.10), latch)
        evaluate_credits_notices(
            state_with_fraction(0.95, purchased_micros=10_000_000, purchased_usd="10.00"),
    ...
    def test_grant_spent_still_fires_with_topup(self):
        """Suppression only affects the gauge â€” grant_spent (which NEEDS purchased>0)
        is untouched."""
        latch = fresh_latch()
        s = state_with_fraction(
            1.0,
...
# â”€â”€ Invariant: never fire + clear same key in one call â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Scenario 9: escalating usage bands (50 â†’ 75 â†’ 90) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestUsageBands:
    ...
    def test_climb_replaces_band(self):
        """Climbing 50â†’75â†’90 replaces the single line (clear old + show new)."""
        latch = fresh_latch()
        evaluate_credits_notices(state_with_fraction(0.10), latch)
        # 55% â†’ 50 band
        evaluate_credits_notices(state_with_fraction(0.55), latch)
        assert latch["usage_band"] == 50
        # 80% â†’ climbs to 75, clearing the 50 line
    ...
    def test_step_down_on_recovery(self):
        ...
        evaluate_credits_notices(state_with_fraction(0.95), latch)
        assert latch["usage_band"] == 90
        # drop to 80% â†’ steps down to 75
        to_show, to_clear = evaluate_credits_notices(state_with_fraction(0.80), latch)
        assert "credits.usage" in to_clear
        assert "75%" in self._band_text(to_show)
        # drop to 55% â†’ steps down to 50
        to_show, _ = evaluate_credits_notices(state_with_fraction(0.55), latch)
        assert "50%" in self._band_text(to_show)
        # drop below 50% â†’ clears entirely
        to_show, to_clear = evaluate_credits_notices(state_with_fraction(0.10), latch)
        assert "credits.usage" in to_clear
        assert latch["usage_band"] is None
    ...
    def test_no_refire_same_band(self):
        ...
        # still 80% â†’ same band, no re-emit, no clear
        to_show, to_clear = evaluate_credits_notices(state_with_fraction(0.80), latch)
        assert all(n.key != "credits.usage" for n in to_show)
        assert "credits.usage" not in to_clear
