...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Zed Tests")
...
def test_binary_file_yields_actionable_block_not_a_dead_warning(sample_repo: Path):
    ...
    # warning that makes the model give up â€” it gets an actionable content block.
    assert not result.warnings
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_zed_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("ZED_HOME", str(tmp_path / ".zed"))

    zed_env = tmp_path / ".zed" / ".env"
    zed_env.parent.mkdir(parents=True)
    zed_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.zed/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
