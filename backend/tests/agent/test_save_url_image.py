...
These exercise the helper against a real in-process HTTP server â€” no
``requests.get`` mocking â€” so we catch the kinds of issues a mocked
...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ZED_HOME under tmp_path."""
    monkeypatch.setenv("ZED_HOME", str(tmp_path / ".zed"))
    (tmp_path / ".zed").mkdir()

    # Force the constants/image cache helpers to re-read ZED_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("zed_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_zed_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under ZED_HOME â€” gateway cleanup
    ...
    def test_extension_inferred_from_content_type(self, http_server):
        ...
        assert path.suffix == ".jpg", "image/jpeg â†’ .jpg"

    def test_extension_falls_back_to_url_suffix(self, http_server):
        """Some CDNs send ``application/octet-stream`` â€” the URL suffix wins then."""
        base, _ = http_server
        from agent.image_gen_provider import save_url_image
    ...
    def test_404_raises(self, http_server):
        """HTTP errors must propagate â€” caller decides whether to fall back."""
        base, _ = http_server
        from agent.image_gen_provider import save_url_image
    ...
    def test_empty_body_raises_without_writing_file(self, http_server):
        """0-byte responses are not images â€” refuse to cache."""
        base, _ = http_server
        from agent.image_gen_provider import save_url_image
