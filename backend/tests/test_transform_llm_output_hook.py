...
def _make_enabled_plugin(zed_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <zed_home>/plugins/<name> and opt it in."""
    plugin_dir = zed_home / "plugins" / name
...
def test_hook_exception_does_not_replace_response(tmp_path, monkeypatch):
    ...
    warning, and continues â€” so a raising plugin contributes no entry
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ZED_HOME", str(tmp_path / "zed_empty"))
