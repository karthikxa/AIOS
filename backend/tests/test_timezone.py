"""
Tests for timezone support (zed_time module + integration points).
...
def _reset_zed_time_cache():
    """Reset the zed_time module cache (replacement for removed reset_cache)."""
    zed_time._cached_tz = None
    zed_time._cached_tz_name = None
    zed_time._cache_resolved = False


# =========================================================================
# zed_time.now() â€” core helper
# =========================================================================

class TestZedTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_zed_time_cache()

    def teardown_method(self):
        _reset_zed_time_cache()
        os.environ.pop("ZED_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["ZED_TIMEZONE"] = "Asia/Kolkata"
        result = zed_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["ZED_TIMEZONE"] = "UTC"
        result = zed_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["ZED_TIMEZONE"] = "America/New_York"
        result = zed_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["ZED_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="zed_time"):
            result = zed_time.now()
    ...
    def test_empty_timezone_uses_local(self):
        """No timezone configured â†’ server-local time (still tz-aware)."""
        os.environ.pop("ZED_TIMEZONE", None)
        result = zed_time.now()
        assert result.tzinfo is not None
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["ZED_TIMEZONE"] = "Europe/London"
        tz = zed_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["ZED_TIMEZONE"] = "Not/A/Timezone"
        tz = zed_time.get_timezone()
        assert tz is None
...
# =========================================================================
# execute_code child env â€” TZ injection
# =========================================================================
...
class TestCodeExecutionTZ:
    ...
    def _import_execute_code(self, monkeypatch):
        ...
        # Force local backend â€” other tests in the same xdist worker may leak
    ...
    def test_tz_injected_when_configured(self):
        """When ZED_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) ZED_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("ZED_TIMEZONE=" + os.environ.get("ZED_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "ZED_TIMEZONE=NOT_SET" in result["output"], (
            "ZED_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When ZED_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("ZED_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_get_due_jobs_handles_naive_timestamps(self, tmp_path, monkeypatch):
        ...
        # Should not crash â€” _ensure_aware handles the naive timestamp
        due = get_due_jobs()
        assert len(due) == 1
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=zed_tz) which shifted
        absolute time when system-local tz != Zed tz.  The fix interprets
        ...
        # Create a naive datetime â€” will be interpreted as system-local time
    ...
    def test_ensure_aware_normalizes_aware_to_zed_tz(self):
        """Already-aware datetimes should be normalized to Zed tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Zed tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Zed tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Zed is UTC.
        ...
        # system running in UTC+5:30 â€” 5 minutes in the past (local time)
        naive_past = (datetime.now() - timedelta(seconds=30)).isoformat()
        jobs[0]["next_run_at"] = naive_past
        save_jobs(jobs)
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Overdue job was skipped â€” _ensure_aware likely shifted absolute time"
        )

    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Zed tz
        is behind system local tz â€” the scenario that triggered #806."""
        import cron.jobs as jobs_module
        monkeypatch.setattr(jobs_module, "CRON_DIR", tmp_path / "cron")
        monkeypatch.setattr(jobs_module, "JOBS_FILE", tmp_path / "cron" / "jobs.json")
        monkeypatch.setattr(jobs_module, "OUTPUT_DIR", tmp_path / "cron" / "output")

        # Use a Zed timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _zed_now's wall time â€” this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["ZED_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_zed_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Zed timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["ZED_TIMEZONE"] = "US/Eastern"
        _reset_zed_time_cache()
