...
The root ``package.json`` is installed by ``zed update`` on every user,
...
everyone â€” including binary-fetching backends, on every update.
...
* ``@askjo/camofox-browser`` is NOT eager. It is an explicit opt-in
  ...
  ``zed tools`` â†’ Browser Automation â†’ Camofox, and only used at
  runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
  Firefox-fork binary, which silently blocked ``zed update`` for
...
to root ``dependencies``, this test fails â€” read the lazy-install
guidance in the ``zed-agent-dev`` skill before changing the
expectations.
"""
...
def test_camofox_is_not_in_root_dependencies() -> None:
    ...
    assert "@askjo/camofox-browser" not in deps, (
        ...
        "Camofox post_setup handler in zed_cli/tools_config.py so it "
        "only installs when the user explicitly selects Camofox via "
        "`zed tools` â†’ Browser Automation â†’ Camofox."
    )
...
def test_agent_browser_stays_eager() -> None:
    ...
    assert "agent-browser" in deps, (
        ...
        "after `zed setup` / `zed update` without an explicit "
        "post_setup step."
    )
