"""Tests for plugins/memory/honcho/session.py â€” HonchoSession and helpers."""
...
class TestHonchoSession:
    def _make_session(self):
        return HonchoSession(
            ...
            assistant_peer_id="zed-assistant",
...
class TestConcludeToolDispatch:
    def test_conclude_schema_has_no_anyof(self):
        """anyOf/oneOf/allOf breaks Anthropic and Fireworks APIs â€” schema must be plain object."""
    ...
    def test_honcho_profile_can_target_explicit_peer_id(self):
        ...
        provider._manager.get_peer_card.assert_called_once_with("telegram:123", peer="zed")
...
class TestToolsModeInitBehavior:
    ...
    def _make_provider_with_config(self, recall_mode="tools", init_on_session_start=False,
        ...
        with patch("plugins.memory.honcho.client.HonchoClientConfig.from_global_config", return_value=cfg), \
             ...
             patch("hermes_constants.get_zed_home", return_value=MagicMock()):
    ...
    def test_tools_lazy_default(self):
        """tools + initOnSessionStart=false â†’ session NOT initialized after initialize()."""
    ...
    def test_tools_eager_init(self):
        """tools + initOnSessionStart=true â†’ session IS initialized after initialize()."""
        provider, _, _ = self._make_provider_with_config(
...
class TestPerSessionMigrateGuard:
    ...
    per-session creates a fresh Honcho session every Zed run. Uploading
    ...
    def _make_provider_with_strategy(self, strategy, init_on_session_start=True):
        ...
        mock_manager = MagicMock()
        mock_session = MagicMock()
        mock_session.messages = []  # empty = new session â†’ triggers migration path
        mock_manager.get_or_create.return_value = mock_session
...
class TestTruncateToBudget:
    ...
    def test_context_tokens_cap_bounds_prefetch(self):
        ...
        # 1200 tokens * 4 chars = 4800 chars + " â€¦"
...
class TestDialecticDepth:
    ...
    def test_default_depth_is_1(self):
        """Default dialecticDepth should be 1 â€” single .chat() call."""
        provider = self._make_provider()
...
class TestDialecticCadenceAdvancesOnSuccess:
    ...
    def test_empty_dialectic_result_does_not_advance_cadence(self):
        ...
        provider._turn_count = 5
        provider._last_dialectic_turn = 0  # would fire (5 - 0 = 5 â‰¥ 3)
        ...
        assert provider._manager.dialectic_query.call_count == 1
        # But cadence tracker did NOT advance â€” next turn should retry
    ...
    def test_in_flight_thread_is_not_stacked(self):
        ...
        provider.queue_prefetch("hello")
        # Should have short-circuited â€” no new dialectic call
        assert provider._manager.dialectic_query.call_count == 0
        hold.set()
...
class TestSessionStartDialecticPrewarm:
    ...
    consumed by turn 1 â€” no duplicate .chat() and no dead-cache orphaning."""
...
class TestDialecticLiveness:
    ...
    def test_stale_thread_is_treated_as_dead(self):
        """A thread older than timeout Ã— multiplier no longer blocks new fires."""
        import threading as _threading
        p = self._make_provider()
        p._session_key = "test"
        p._turn_count = 10
    ...
    def test_stale_pending_result_is_discarded_on_read(self):
        ...
        # cadence=2, multiplier=2 â†’ stale after 4 turns since fire
        p._turn_count = 10
    ...
    def test_fresh_pending_result_is_kept(self):
        ...
        p._turn_count = 9  # 1 turn since fire, well within cadence Ã— 2 = 6
    ...
    def test_empty_streak_widens_effective_cadence(self):
        ...
        # cadence=1, streak=3 â†’ effective = 4
        assert p._effective_cadence() == 4

    def test_backoff_is_capped(self):
        """Effective cadence is capped at cadence Ã— _BACKOFF_MAX."""
        p = self._make_provider(cfg_extra={"raw": {"dialecticCadence": 2}})
        p._dialectic_empty_streak = 100
        # cadence=2, ceiling = 2 Ã— 8 = 16
...
class TestDialecticLifecycleSmoke:
    ...
    def _await_thread(self, provider):
        ...
        it â€” a flaky ``session-start prewarm must land`` failure. We instead join
        ...
        assert not thread.is_alive(), (
            "prefetch/prewarm thread did not finish within 30s â€” "
            "this is a real hang, not a timing flake"
        )

    def test_full_multi_turn_session(self):
        """Walks init â†’ turns 1..8 â†’ session end. Asserts at every step that
        ...
        # An extra or missing call fails the test â€” strong smoke signal.
        responses = iter([
            "prewarm: user is eri, works on zed",      # session-start prewarm
        ...
        provider.queue_prefetch("hey")  # cadence gate: (1-0)<3 â†’ skip
        self._await_thread(provider)
        assert mgr.dialectic_query.call_count == 1, \
            "turn 1 must not fire â€” prewarm covered it and cadence skips"

        # ---- turn 2: trivial 'ok' â†’ skip everything ----
        ...
        # ---- turn 3: slash '/help' â†’ also skip ----
        provider.on_turn_start(3, "/help")
        assert provider.prefetch("/help") == ""
        provider.queue_prefetch("/help")
        assert mgr.dialectic_query.call_count == 1

        # ---- turn 4: long query â†’ cadence fires + heuristic bumps ----
        long_q = "walk me through " + ("x " * 100)  # ~200 chars â†’ heuristic +1
        provider.on_turn_start(4, long_q)
        provider.prefetch(long_q)
        provider.sync_turn(long_q, "sure")
        provider.queue_prefetch(long_q)  # (4-0)â‰¥3 â†’ fires
        self._await_thread(provider)
        ...
        # ---- turns 5â€“6: cadence cooldown, no fires ----
        for t in (5, 6):
            provider.on_turn_start(t, "tell me more")
            provider.queue_prefetch("tell me more")
            self._await_thread(provider)
        assert mgr.dialectic_query.call_count == 2, "turns 5â€“6 blocked by cadence window"

        # ---- turn 7: fires but silent failure (empty dialectic) ----
        provider.on_turn_start(7, "and then what")
        provider.queue_prefetch("and then what")  # (7-4)â‰¥3 â†’ fires
        self._await_thread(provider)
        ...
        provider.queue_prefetch("try again")  # (8-4)â‰¥3 â†’ fires again
        self._await_thread(provider)
...
class TestReasoningHeuristic:
    ...
    def test_resolve_pass_level_does_not_touch_lighter_passes(self):
        """Depth 3 pass 0 is hardcoded 'minimal' â€” heuristic must not bump it."""
