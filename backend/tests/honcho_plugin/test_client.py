"""Tests for plugins/memory/honcho/client.py â€” Honcho client configuration."""
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "zed"
        assert config.workspace_id == "zed"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.zed/honcho.json
        monkeypatch.setenv("ZED_HOME", str(tmp_path / "isolated"))
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"zed": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="zed-agent"
        ):
            result = config.resolve_session_name("/home/user/zed-agent/subdir")
        assert result == "zed-agent"
...
class TestResolveConfigPath:
    def test_prefers_zed_home_when_exists(self, tmp_path):
        zed_home = tmp_path / "zed"
        zed_home.mkdir()
        local_cfg = zed_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: ZED_HOME points at ~/.zed/profiles/<name>, so
        # _get_default_zed_home() must resolve back to ~/.zed â€” that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".zed"
        ...
        assert _get_default_zed_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_zed_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".zed".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".zed"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "zed_coder"
        assert profile_host_key("default") == "zed"

    def test_default_returns_zed(self):
        with patch.dict(os.environ, {}, clear=True):
    ...
    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"ZED_HONCHO_HOST": "zed.coder"}):
            assert resolve_active_host() == "zed.coder"
...
class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="zed_coder")
        assert config.host == "zed_coder"
        assert config.workspace_id == "zed"  # shared workspace
        assert config.ai_peer == "zed_coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="zed")
        assert config.host == "zed"
        assert config.workspace_id == "zed"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config = HonchoClientConfig.from_global_config(
            host="zed_coder", config_path=config_file,
        )
        assert config.host == "zed_coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "zed_dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="zed_dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "zed_dreamer"
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "zed.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        config = HonchoClientConfig.from_global_config(
            host="zed_dreamer",
        ...
        assert config.host == "zed_dreamer"
        assert config.peer_name == "dreamer-user"
        assert config.workspace_id == "zed_dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        """Config with host block but no observationMode â†’ 'unified' (old default)."""
        cfg_file = tmp_path / "config.json"
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"zed": {"enabled": True, "aiPeer": "zed"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "unified"

    def test_new_config_defaults_to_directional(self, tmp_path):
        """Config with no host block and no credentials â†’ 'directional' (new default)."""
        cfg_file = tmp_path / "config.json"
        cfg_file.write_text(json.dumps({}))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
    ...
    def test_explicit_directional_respected(self, tmp_path):
        """Existing config with explicit observationMode â†’ uses what's set."""
        cfg_file = tmp_path / "config.json"
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"zed": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"

    def test_explicit_unified_respected(self, tmp_path):
        """Existing config with explicit observationMode unified â†’ stays unified."""
        cfg_file = tmp_path / "config.json"
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "observationMode": "unified",
            "hosts": {"zed": {"enabled": True}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "unified"
...
class TestGetHonchoClient:
    ...
    def test_zed_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="zed",
    ...
    def test_zed_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="zed",
...
class TestResetHonchoClient:
    def test_reset_clears_singleton(self):
        ...
        # module global anymore â€” see #24759.)
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    """Regression tests for #20688 â€” Honcho SDK double-prefixing of /v3 for
