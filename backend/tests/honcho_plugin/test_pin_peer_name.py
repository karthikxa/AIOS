...
Under a gateway (Telegram, Discord, Slack, ...) Zed passes the
...
class TestPinPeerNameConfigParsing:
    def test_default_is_false(self):
        """Default preserves existing behaviour â€” multi-user bots unaffected."""
    ...
    def test_host_block_overrides_root(self, tmp_path, monkeypatch):
        """Host block wins over root â€” matches how every other flag behaves."""
        config_file = tmp_path / "honcho.json"
        config_file.write_text(json.dumps({
            "apiKey": "k",
            "peerName": "Igor",
...
def _patch_manager_for_resolution_test(mgr: HonchoSessionManager) -> None:
    ...
    to the network â€” we only care about the user_peer_id chosen before
    those calls happen.
    """
    fake_peer = MagicMock()
...
class TestPeerResolutionOrder:
    """Matrix of (runtime_id, pin_peer_name, peer_name) â†’ expected user_peer_id."""
    ...
    def _config(
        ...
        *,
        ...
        # The test doesn't need auth / Honcho â€” disable the provider so
    ...
    def test_runtime_wins_when_pin_is_false(self):
        ...
        assert session.user_peer_id == "7654321", (
            "pin_peer_name=False is the multi-user default â€” the gateway's "
    ...
    def test_pin_noop_when_peer_name_missing(self):
        ...
        nothing to pin to â€” fall through to runtime mapping."""
        mgr = HonchoSessionManager(
            honcho=MagicMock(),
            config=self._config(
                peer_name=None,
    ...
    def test_runtime_missing_falls_back_to_peer_name(self):
        """CLI-mode (no gateway runtime identity) uses config peer_name â€”
    ...
    def test_pin_does_not_affect_assistant_peer(self):
        """The flag only pins the USER peer â€” the assistant peer continues
        to come from ``ai_peer`` and must not be touched."""
        cfg = HonchoClientConfig(
            api_key="k",
            peer_name="Igor",
            pin_peer_name=True,
            ai_peer="zed-assistant",
            enabled=False,
        ...
        session = mgr.get_or_create("telegram:7654321")
        assert session.user_peer_id == "Igor"
        assert session.assistant_peer_id == "zed-assistant"


class TestCrossPlatformMemoryUnification:
    """The same physical user talking to Zed via Telegram AND Discord
    ...
    def test_telegram_and_discord_collapse_to_one_peer_when_pinned(self):
        ...
        # Discord turn (separate manager instance â€” simulates a fresh
        ...
        assert telegram_session.user_peer_id == discord_session.user_peer_id, (
            ...
            "pinPeerName â€” both platforms must land on the same Honcho peer"
        )
    ...
    def test_multiuser_default_keeps_platforms_separate(self):
        ...
        assert sess_a.user_peer_id != sess_b.user_peer_id, (
            "multi-user default MUST keep users separate â€” a regression "
...
class TestPinUserPeerAlias:
    ...
    ``pin_peer_name`` field.  Precedence when both appear: host pinUserPeer â†’
    host pinPeerName â†’ root pinUserPeer â†’ root pinPeerName â†’ default.
    """
...
class TestPinTransition:
    """Behavior when honcho.json flips ``pinPeerName`` true â†’ false.
    ...
    def test_fresh_manager_after_flip_resolves_to_runtime(self):
        ...
        assert after.user_peer_id == "7654321", (
            ...
            "to its own peer â€” otherwise multi-user mode silently merges users."
        )
    ...
    def test_cached_session_survives_config_flip_in_same_manager(self):
        ...
        assert second.user_peer_id == "Igor", (
            ...
            "resolved peer.  In-process flips don't invalidate it â€” the "
    ...
    def test_cache_busting_signature_reflects_ai_peer(self, tmp_path, monkeypatch):
        ...
        ``HonchoSessionManager`` freezes ``cfg.ai_peer`` at construction â€”
...
class TestProfilePeerUniqueness:
    """Each Zed profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `zed -p partner` pinned to a different person's peer).
    ...
    def test_two_profiles_pinned_to_different_peer_names_resolve_distinctly(self):
        ...
        assert sess_a.user_peer_id != sess_b.user_peer_id, (
            ...
            "the same Honcho peer â€” otherwise profile isolation is fictional."
        )
    ...
    def test_host_peer_name_overrides_root_when_pinned(self, tmp_path, monkeypatch):
        ...
        cfg = HonchoClientConfig.from_global_config(
            host="zed.partner", config_path=config_file,
        )
