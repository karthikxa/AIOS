...
The ``zed update`` Python path already clears the conflict with ``git reset``
...
def _make_unmerged_repo(repo: Path) -> None:
    ...
    # Conflicting merge â€” exits non-zero and leaves the index unmerged.
...
def test_install_ps1_clears_unmerged_index_before_stash() -> None:
    ...
    so *before* the stash push (order matters â€” the fix is a no-op otherwise)."""
