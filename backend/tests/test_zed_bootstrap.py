"""Tests for zed_bootstrap â€” Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Zed entry point
(zed, zed-agent, zed-acp, gateway, batch_runner, cli.py).  It
fixes Python's Windows UTF-8 defaults so print("cafÃ©") doesn't crash and
...
Key invariants covered by these tests:
  ...
  5. Load order: every Zed entry point imports zed_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported zed_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("zed_bootstrap", None)
    import zed_bootstrap  # noqa: WPS433
    return zed_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Zed CLI
        # runs on Windows with a pytest console that's cp1252 by default).
        # If reconfigure succeeded, sys.stdout.encoding is 'utf-8'.
        _fresh_import()
        # pytest may capture stdout, which makes encoding check flaky â€”
        ...
        assert out.encoding.lower() in {"utf-8", "utf8"}, (
            f"stdout encoding is {out.encoding!r} â€” bootstrap should have "
    ...
    def test_child_process_inherits_utf8_mode(self):
        ...
        script = textwrap.dedent("""
            ...
            sys.exit(0)
        """).strip()
        # Don't pass env= â€” let the child inherit os.environ, which
...
class TestStdioReconfigureErrorHandling:
    ...
    def test_reconfigure_oserror_is_caught(self, monkeypatch):
        ...
        the error â€” the env-var half of the fix still applies."""
        hb = _fresh_import()
        hb._IS_WINDOWS = True
        hb._bootstrap_applied = False
...
class TestEntryPointsImportBootstrap:
    """Every Zed entry point must import zed_bootstrap as its
    ...
    # Entry points that invoke Zed as a process.  Each one must
    # import zed_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "zed_cli/main.py",   # zed CLI (console_script)
        "run_agent.py",          # zed-agent (console_script)
        "acp_adapter/entry.py",  # zed-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        ...
        half-finished ``zed update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``zed_bootstrap`` as a top-level module) leaves zed
        ...
        """
        # Resolve relative to the zed-agent repo root.  Tests live
        # at tests/test_zed_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # Import node â€” this is the recovery-friendly form that lets
            # zed start even when zed_bootstrap hasn't been
        ...
        assert first_import_name == "zed_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'zed_bootstrap' so UTF-8 stdio is "
