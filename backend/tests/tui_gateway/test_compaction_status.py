"""Auto-compaction status re-tagging for the desktop "Summarizingâ€¦" indicator.
...
def test_other_lifecycle_status_stays_lifecycle(server, monkeypatch):
    events = _capture(server, monkeypatch)
    server._status_update("sid", "lifecycle", "âŒ Rate limited after 5 retries")
...
def test_manual_compressing_kind_is_preserved(server, monkeypatch):
    events = _capture(server, monkeypatch)
    server._status_update("sid", "compressing", "â ‹ compressing 40 messagesâ€¦")
