...
# â”€â”€ JSON-RPC envelope â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ write_json â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_write_json_unicode_encode_error_re_raises(server):
    ...
    subclass).  It must NOT be swallowed as 'peer gone' â€” that would let
...
def test_write_json_non_serializable_payload_re_raises(server):
    """Non-JSON-safe payloads are programming errors â€” they must NOT be
...
def test_write_json_non_peer_gone_oserror_re_raises(server):
    """Host I/O failures (ENOSPC, EACCES, EIO â€¦) are NOT peer-gone â€” they
...
def test_write_json_skips_flush_when_disable_flush_true(monkeypatch):
    ...
    var â†’ module constant wiring is covered by the dedicated env test
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `ZED_TUI_GATEWAY_NO_FLUSH=1` and importing
    `tui_gateway.transport` fresh actually flips `_DISABLE_FLUSH` true.

    Reloads only the transport module â€” server.py is untouched so its
    atexit hooks/worker pool stay intact."""
    import importlib

    monkeypatch.setenv("ZED_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("ZED_TUI_GATEWAY_NO_FLUSH", raising=False)
        importlib.reload(transport_mod)


# â”€â”€ _emit â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Blocking prompt round-trip â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_block_and_respond(capture):
    ...
    # _pending values are (sid, Event) tuples â€” unpack to set the Event
...
# â”€â”€ Session lookup â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ session.resume payload â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_session_resume_lazy_registers_watch_session_without_agent(server, monkeypatch):
    ...
    â€” keyed for the child mirror, on this transport â€” WITHOUT building an
...
def test_session_resume_lazy_reports_running_for_inflight_child(server, monkeypatch):
    ...
    live from the resume response itself â€” the child can sit silent inside a
...
def test_session_resume_lazy_tolerates_missing_row_for_active_child(server, monkeypatch):
    ...
    class _DB:
        def get_session(self, _sid):
            # Row not flushed yet â€” the whole point of the race.
            return None
...
def test_session_resume_missing_row_non_lazy_still_errors(server, monkeypatch):
    ...
    # Non-lazy resume, no active child â†’ hard error.
    resp = server.handle_request(
        {
            "id": "r1",
            "method": "session.resume",
    ...
    # Lazy resume but the child is NOT live â†’ still an error (no live mirror to
...
def test_session_resume_reuses_existing_live_session(server, monkeypatch):
    ...
    # Both resumes resolve to the SAME single live session â€” the core invariant.
...
def test_session_branch_persists_branched_from_marker(server, monkeypatch):
    ...
    heuristic never surfaces it â€” the stable model_config marker is the only
    ...
    # The marker â€” without it the branch is invisible in /resume and /sessions.
...
# â”€â”€ Config I/O â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


def test_config_load_missing(server, tmp_path):
    server._zed_home = tmp_path
...
# â”€â”€ _cli_exec_blocked â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ slash.exec skill command interception â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/zed-agent-dev": {"name": "zed-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "zed-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_routes_pending_input_commands_to_dispatch(server, cmd):
    ...
    command.dispatch would for the same command â€” no fragile retry hop.
    """
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "zed-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "zed-agent-dev"
...
# â”€â”€ dispatch(): pool routing for long handlers (#12546) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
