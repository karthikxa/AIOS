...
def test_make_agent_provider_routing_defaults_when_unset():
    """No ``provider_routing`` section â†’ no routing prefs forwarded (None /
...
def test_make_agent_honors_tui_launch_env_flags():
    ...
    with (
        patch.dict(
            ...
            {
                "ZED_TUI_MAX_TURNS": "7",
                "ZED_TUI_CHECKPOINTS": "1",
                "ZED_TUI_PASS_SESSION_ID": "1",
                "ZED_IGNORE_RULES": "1",
            },
        ),
        patch("tui_gateway.server._load_cfg", return_value=fake_cfg),
...
def test_make_agent_tolerates_null_config_sections():
    """Bare `agent:` / `display:` keys in ~/.zed/config.yaml parse as
...
def test_make_agent_honors_per_session_model_override():
    ...
    rebuilt agent's model/provider/base_url, NOT global config â€” and without
...
def test_apply_model_switch_does_not_leak_process_env():
    ...
    with (
        patch("zed_cli.model_switch.parse_model_flags",
              return_value=("glm-5.1", None, False, False, True)),
        patch("zed_cli.model_switch.resolve_persist_behavior",
              return_value=False),
        patch("zed_cli.model_switch.switch_model", return_value=_FakeResult()),
    ...
    assert before == after
    # persist_global was False â†’ config untouched.
