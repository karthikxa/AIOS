...
def test_no_live_child_session_no_mirror(server, emits):
    ...
    # Only the parent-sid relay event â€” nothing mirrored, no state retained.
...
def test_live_child_session_gets_native_stream(server, emits):
    ...
    # Synthetic turn: start â†’ tool â†’ reasoning â†’ tool rotation â†’ close + summary.
...
def test_window_closed_midrun_drops_state_then_fresh_turn_on_reopen(server, emits):
    ...
    # Window closes â†’ live session gone â†’ state dropped on the next event.
    server._sessions.clear()
    _relay(server, "subagent.tool", tool_name="read_file", child_session_id="child-1")
    assert server._child_mirrors == {}

    # Reopen under a new live sid â†’ a fresh synthetic turn starts.
    emits.clear()
...
def test_upgraded_child_session_not_mirrored(server, emits):
    ...
    native stream â€” mirroring on top would interleave two turns on one sid."""
    ...
    assert [(e, s) for e, s, _ in emits] == [("subagent.tool", "parent-sid")]
    assert server._child_mirrors == {}
    # Liveness registry still updates â€” it serves resume, not the mirror.
...
def test_stale_child_run_not_reported_active(server, emits):
    ...
    server._active_child_runs["child-1"] = 0.0  # epoch â€” ancient
...
def test_prompt_submit_rejected_while_child_run_active(server, emits):
    ...
    the in-flight child on the same stored session â€” busy error instead."""
    import threading
    ...
    # Run completes â†’ the same submit upgrades into a real conversation
...
def test_active_child_runs_registry_tracks_liveness(server, emits):
    ...
    open), and completion clears it â€” lazy watch resumes read this registry to
...
def test_start_mirrors_as_immediate_header_line(server, emits):
    ...
    # rollups) no longer pollutes the message body â€” tools mirror natively via
...
def test_text_mirrors_as_message_delta(server, emits):
    ...
    message.delta on the live child sid â€” the watch window streams it as the
...
def test_text_routes_to_watch_transport_without_contextvar(server, monkeypatch):
    ...
    # thread never inherits the parent's contextvar) â€” mirrors the async case.
