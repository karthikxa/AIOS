...
lost â€” and the api_key is deliberately never persisted. On ``session.resume``
or ``_reset_session_agent``, ``_stored_session_runtime_overrides`` fed
provider="custom" back into ``_make_agent`` â†’
...
"No LLM provider configured. Run `zed model`..." (resume failed) or
silently resolved placeholder credentials ("no-key-required") against the
patched-back base_url.

Fix: persist the REQUESTED/entry identity â€” ``_runtime_model_config`` maps
...
def _make_agent_with_override(override, monkeypatch, config, model_cfg=None):
    ...
    # Keep credential-pool resolution off the developer's real ZED home.
...
class TestResumeRoundTrip:
    def test_round_trip_restores_entry_credentials(self, monkeypatch):
        """persist â†’ stored-overrides â†’ _make_agent resolves the entry's
...
# point-fix above recovers the entry identity from the persisted base_url â€”
...
class TestBareCustomNoBaseUrlHealsFromConfig:
    ...
    config identifies the active entry â€” even when no base_url survived."""
    ...
    def test_canonical_identity_recovers_from_config_when_no_base_url(
        ...
        # No base_url to reverse-lookup â†’ must fall back to config.model.provider.
        assert (
    ...
    def test_canonical_identity_returns_none_without_a_real_entry(
        ...
        # config.model.provider is bare "custom" and no entry is named â†’ no
    ...
    def test_persist_recovers_entry_when_agent_has_no_base_url(self, monkeypatch):
        ...
        # Bare "custom" must NOT be persisted â€” it heals to the entry identity.
        assert config["provider"] == "custom:mimo-v2.5-pro"
    ...
    def test_restore_drops_bare_custom_when_config_cannot_heal(self, monkeypatch):
        ...
        override â€” leave it unset so resume falls back to the configured
    ...
    def test_first_db_row_persists_entry_identity_not_bare_custom(self, monkeypatch):
        ...
        resolved value is bare "custom" â€” persisting that verbatim seeds the
