"""Regression test for #25676 â€” nested gateway.streaming config must be loaded."""
...
def _load_with_yaml_dict(yaml_dict: dict):
    ...
    fake_home = Path("/tmp/fake_zed_home_25676")

    def fake_exists(self):
        return str(self).endswith("config.yaml")

    with patch("gateway.config.get_zed_home", return_value=fake_home), \
