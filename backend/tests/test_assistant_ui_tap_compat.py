...
The Zed desktop app (``apps/desktop``) is built from source on every
install/update via ``scripts/install.ps1`` â†’ ``npm ci``/``npm install`` â†’
...
* ``@assistant-ui/store@0.2.18`` bumped its tap peer to ``^0.9.0`` and started
  importing ``@assistant-ui/tap/react-shim`` â€” an entry point that only exists
...
def _hoisted_tap_version(packages: dict) -> str:
    ...
    assert entry is not None, (
        ...
        "entry â€” the @assistant-ui cluster should resolve a single shared "
        "tap version."
    )
...
def test_assistant_ui_cluster_agrees_on_one_tap() -> None:
    ...
    store, and any sibling â€” otherwise the missing ``./react-shim`` export
    ...
    assert not offenders, (
        ...
        + "; ".join(offenders)
        + ". The @assistant-ui cluster has split tap requirements â€” pin the "
