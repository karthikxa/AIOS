...
Spawns one subprocess per (version, scenario) cell â€” pinned to either
``origin/main`` (no plugin hook, no STT command-provider registry; only
the legacy ``ZED_LOCAL_STT_COMMAND`` escape hatch exists) or this PR's
...
Where ``dispatch_kind`` âˆˆ
``{"builtin_local", "builtin_groq", "builtin_openai", ...,
"plugin", "plugin_unavailable", "command_provider",
"no_provider_error", "stt_disabled"}``.

Acceptable diffs:
- ``no_provider_error â†’ plugin`` for the ``plugin-installed`` scenario.
- ``no_provider_error â†’ plugin_unavailable`` for the
...
- ``no_provider_error â†’ command_provider`` for the
  ``command-provider-installed`` scenario (registry shipped with this PR).
- ``no_provider_error â†’ command_provider`` for
...
def _resolve_main_dir() -> Path:
    ...
    if (candidate / "tools" / "transcription_tools.py").exists() and candidate != REPO_ROOT:
        return candidate
    sibling = REPO_ROOT.parent / "zed-agent-main"
...
assert (PR_DIR / "tools" / "transcription_tools.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a zed-agent checkout"
)
...
# Isolated ZED_HOME so the config write is hermetic.
...
# Fresh import â€” must not have anything cached from prior runs.
...
# Try importing transcription_registry â€” only exists on PR side.
...
if explicit:
    # Bypass the gating for test purposes â€” _get_provider would
...
SCENARIOS: list[tuple[str, str, dict[str, str], str]] = [
    ...
    ("explicit-local", "stt:\n  provider: local\n", {}, "no"),
    ("explicit-xai", "stt:\n  provider: xai\n", {}, "no"),
    # Mistral is quarantined â†’ _get_provider returns "none" today, hence no_provider_error.
    ("explicit-mistral-quarantine", "stt:\n  provider: mistral\n", {}, "no"),
    # Unknown name + no plugin â†’ both: no_provider_error
    ("unknown-no-plugin", "stt:\n  provider: openrouter\n", {}, "no"),
    # Unknown name + plugin installed â†’ main: no_provider_error, PR: plugin
    ("plugin-installed", "stt:\n  provider: openrouter\n", {}, "yes"),
    # Unknown name + plugin reports unavailable â†’ main: no_provider_error,
    # PR: plugin_unavailable (cleaner envelope, names the plugin)
    ("plugin-installed-unavailable", "stt:\n  provider: openrouter\n", {}, "unavailable"),
    # Built-in name + plugin tries to shadow â†’ both: built-in
    ...
    # than plugin install). The plugin emits "PLUGIN:..." â€” assertion is
    ...
    (
        ...
        {},
        "yes",  # also register a plugin under "openrouter" â€” must NOT fire
    ),
    # NEW (this PR): built-in name with a command provider declared under
    # it â†’ built-in still wins (built-in elif chain has precedence).
    # The command would write "CMD: HIJACK" if it fired â€” assertion is
...
# scenarios also work on origin/main â€” the subprocess just executes the
...
def main() -> int:
    ...
    if MAIN_DIR == PR_DIR:
        print(
            "WARN: MAIN_DIR == PR_DIR â€” diffs will be trivially identical.\n"
            "      Set up a sibling 'zed-agent-main' checkout pinned to "
    ...
    for label, config_yaml, env, plugin_register in SCENARIOS:
        ...
        if no_provider_to_plugin:
            print(f"  [DIFF] {label}: no_provider_error â†’ plugin â€” expected")
            intentional_diffs.append((label, main_reduced, pr_reduced))
        elif no_provider_to_unavailable:
            print(f"  [DIFF] {label}: no_provider_error â†’ plugin_unavailable â€” expected")
            intentional_diffs.append((label, main_reduced, pr_reduced))
        elif no_provider_to_command:
            print(f"  [DIFF] {label}: no_provider_error â†’ command_provider â€” expected")
            intentional_diffs.append((label, main_reduced, pr_reduced))
        else:
            print(f"  [FAIL] {label}")
    ...
    if intentional_diffs:
        print(
            ...
            f"no_provider_error â†’ plugin dispatch when a plugin is registered."
        )
