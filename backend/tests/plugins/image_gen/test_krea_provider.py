...
class TestGenerate:
    ...
    def test_successful_generation(self):
        """Happy path: submit â†’ one poll â†’ completed â†’ URL downloaded."""
        from plugins.image_gen.krea import KreaImageGenProvider
    ...
    def test_aspect_ratio_mapping(self):
        """Zed 'square' must map to Krea '1:1' in the wire payload."""
        from plugins.image_gen.krea import KreaImageGenProvider
...
class TestGenerateErrors:
    ...
    def test_url_download_failure_falls_back_to_bare_url(self):
        """Mirror of xAI behaviour â€” if local cache fails, return the URL."""
        import requests as req_lib
        from plugins.image_gen.krea import KreaImageGenProvider
    ...
    def test_polling_picks_up_completed_at_with_unknown_status(self):
        """``completed_at`` set + unrecognised pending status â†’ still terminal."""
        from plugins.image_gen.krea import KreaImageGenProvider

        # Use a status value that is NOT in our terminal set ("intermediate-complete")
        # but with completed_at populated â€” Krea's spec says completed_at is the
...
class TestPollRetryPolicy:
    ...
    def test_poll_fails_fast_on_401(self):
        ...
        assert result["error_type"] == "api_error"
        assert "401" in result["error"]
        # One call â€” no retry on permanent auth failure.
