...
* ``dispatch_kind`` âˆˆ ``{"legacy_fal", "plugin", "error", None}`` â€”
...
* ``provider_name`` â€” when ``dispatch_kind == "plugin"``, the
  resolved provider name. ``None`` otherwise.
* ``model`` â€” the resolved FAL model id when applicable.

The parent process diffs the shapes per scenario. A diff means the
migration introduced an observable behaviour change vs origin/main â€”
...
# worktree), ``MAIN_DIR`` falls back to a sibling ``zed-agent-main``
...
# Fresh import â€” must not have anything cached.
...
try:
    ...
    if model is None:
        ...
        # when dispatch goes to a non-FAL plugin â€” used for the diff
...
def _reduce(shape: dict) -> dict:
    ...
    Both shapes are functionally equivalent â€” the plugin's ``generate()``
    re-enters the same in-tree pipeline via ``_it`` indirection â€” but
...
def main() -> int:
    ...
    for label, config_yaml, env in SCENARIOS:
        ...
        # dispatch. That's the only acceptable diff â€” flag everything
        ...
        if legacy_to_plugin_fal:
            print(f"  [DIFF] {label}: legacy_fal â†’ plugin (fal) â€” expected")
    ...
    if intentional_diffs:
        print(
            ...
            f"legacy_fal â†’ plugin dispatch for explicit FAL paths."
        )
