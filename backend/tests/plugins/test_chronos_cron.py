...
All NAS calls are mocked â€” ZERO live network. These prove:
  - is_available is config-only (no network), false without config.
  - one-shot arming sends the right provision payload (incl. sub-minute fires â€”
...
def test_arm_one_shot_preserves_sub_minute_fire(chronos):
    """Sub-minute fire times survive â€” the agent owns the time, so there's no
...
def test_reconcile_skips_already_armed_same_time(temp_home, chronos, monkeypatch):
    ...
    prov.reconcile()
    assert fake.provisions == []  # already armed at the same time â†’ no re-arm
...
def test_fire_due_no_rearm_when_job_gone(chronos, monkeypatch):
    """repeat-N exhausted / one-shot completed â†’ mark_job_run deleted the job â†’
    get_job None â†’ no re-arm (the schedule stops cleanly)."""
    prov, fake = chronos
    monkeypatch.setattr("cron.scheduler_provider.CronScheduler.fire_due",
                        lambda self, jid, **kw: True)
