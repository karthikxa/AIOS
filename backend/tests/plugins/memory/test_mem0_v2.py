"""Tests for Mem0 API v2 compatibility â€” filters param and dict response unwrapping.
...
class TestMem0FiltersV2:
    ...
    def test_read_filters_no_agent_id(self):
        """Read filters should use user_id only â€” cross-session recall across agents."""
        provider = Mem0MemoryProvider()
        provider._user_id = "u123"
        provider._agent_id = "zed"
    ...
    def test_write_filters_include_agent_id(self):
        ...
        provider._user_id = "u123"
        provider._agent_id = "zed"
        assert provider._write_filters() == {"user_id": "u123", "agent_id": "zed"}
...
class TestMem0ResponseUnwrapping:
    ...
    def test_profile_list_response_backward_compat(self, monkeypatch):
        """Old API returned bare lists â€” still works."""
...
class TestMem0Defaults:
    ...
    def test_default_user_id_zed_user(self, monkeypatch, tmp_path):
        ...
        assert provider._user_id == "zed-user"

    def test_default_agent_id_zed(self, monkeypatch, tmp_path):
        monkeypatch.setenv("MEM0_API_KEY", "test-key")
