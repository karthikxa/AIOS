...
def test_post_setup_existing_profile_picker_validates_and_links_saved_profile(tmp_path, monkeypatch):
    ...
    OpenVikingMemoryProvider().post_setup(str(zed_home), config)
...
def test_post_setup_create_remote_user_can_keep_zed_only(tmp_path, monkeypatch):
    _clear_openviking_env(monkeypatch)
    zed_home = tmp_path / "zed"
    zed_home.mkdir()
    monkeypatch.setenv("ZED_HOME", str(zed_home))
    ...
    monkeypatch.setattr(
        ...
        _prompt_from_values({
            "OpenViking server URL": "https://openviking.example",
            "OpenViking user API key": "user-secret",
            "Zed peer ID in OpenViking": "agent",
        }),
    )
    config = {"memory": {}}
    ...
    env_text = (zed_home / ".env").read_text(encoding="utf-8")
...
def test_post_setup_user_key_path_can_route_detected_root_key_to_root_setup(tmp_path, monkeypatch):
    ...
    assert prompt_events.count("Zed peer ID in OpenViking") == 1
    env_text = (zed_home / ".env").read_text(encoding="utf-8")
...
def test_start_local_openviking_server_writes_output_to_log(tmp_path, monkeypatch):
    ...
    def fake_popen(args, **kwargs):
        ...
        assert kwargs["stdout"].name == str(zed_home / "logs" / "openviking-server.log")
    ...
    assert started is True
    assert str(zed_home / "logs" / "openviking-server.log") in message
    assert popen_calls
...
def test_runtime_openviking_waiter_warns_when_background_start_times_out(monkeypatch):
    ...
    assert warnings == [
        ...
        "within 60 seconds. OpenViking memory disabled for this Zed run."
    ]
...
def test_viking_client_headers_include_bearer_when_api_key_set():
    ...
    assert headers["X-OpenViking-Actor-Peer"] == "zed"
    assert "X-OpenViking-Agent" not in headers
    assert "X-OpenViking-Account" not in headers
    assert "X-OpenViking-User" not in headers
...
def test_viking_client_health_sends_auth_headers(monkeypatch):
    ...
    assert captured["headers"]["X-OpenViking-Actor-Peer"] == "zed"
...
# ---------------------------------------------------------------------------
# on_session_switch â€” flush + commit + rotate behavior (zed-agent#28296)
# ---------------------------------------------------------------------------
...
def test_on_session_switch_skips_commit_for_empty_old_session():
    """No turns accumulated â†’ nothing to extract â†’ no commit call."""
...
def test_on_session_switch_waits_for_inflight_sync_thread():
    """In-flight sync_turn write must drain before the commit fires â€”
...
def test_on_session_switch_noop_when_client_missing():
    ...
    # State stays untouched â€” provider is effectively disabled.
    assert provider._session_id == "old-sid"
...
def test_sync_turn_captures_session_id_before_worker_runs():
    ...
    re-read self._session_id later â€” otherwise a delayed worker can write the
    ...
    assert captured_payloads == [{
        "messages": [
            ...
            {"role": "assistant", "parts": [{"type": "text", "text": "a"}], "peer_id": "zed"},
        ]
    }]
...
def test_sync_turn_structured_messages_include_assistant_peer_id():
    ...
    assert captured == [(
        "/api/v1/sessions/sid-structured/messages/batch",
        {
            "messages": [
                {"role": "user", "parts": [{"type": "text", "text": "u"}]},
                {"role": "assistant", "parts": [{"type": "text", "text": "Looking."}], "peer_id": "zed"},
                {
                    "role": "assistant",
                    "parts": [
                        {
...
def test_end_then_switch_does_not_double_commit():
    ...
    (â†’ on_session_end) immediately followed by on_session_switch. The switch
...
def test_session_needs_commit_guard_wins_over_stale_turn_count():
    """Regression for zed-agent#28296 review (M3): once a session is marked
...
# the commit boundary â€” they would never be extracted.
# ---------------------------------------------------------------------------
...
class _HungThread:
    ...
    def join(self, timeout=None):
        # Pretend the join timed out â€” worker still running.
        return None
...
def test_on_session_end_skips_commit_when_sync_worker_outlives_join():
    ...
    be skipped â€” late writes from the worker would otherwise land in an
...
# still-alive previous worker â€” that dropped writer keeps POSTing under the
...
def test_on_session_switch_does_not_block_caller_on_slow_drain():
    """Regression for zed-agent#28296 review (H1): on_session_switch must
    ...
    # No commit yet â€” drain is still blocked off-thread.
    provider._client.post.assert_not_called()
...
def test_on_session_switch_defers_old_commit_to_finalizer_thread():
    ...
    -_DEFERRED_COMMIT_TIMEOUT drain or the commit POST. See zed-agent#28296
    ...
    # Rotation is synchronous and immediate â€” the new session is live at once.
    assert provider._session_id == "new-sid"
    assert provider._turn_count == 0
...
def test_on_memory_write_uses_content_write_independent_of_session_rotation():
    ...
    assert captured_payloads[0]["uri"].startswith(
        "viking://user/peers/zed/memories/preferences/mem_"
    )
