...
from zed_cli.memory_setup import _CANCELLED
...
def _provider_for_mode(tmp_path, monkeypatch, mode: str):
    ...
    monkeypatch.setattr(
        "plugins.memory.hindsight.get_zed_home", lambda: tmp_path
    )

    provider = HindsightMemoryProvider()
    provider.initialize(session_id="test-session", zed_home=str(tmp_path), platform="cli")
    return provider
...
def provider(tmp_path, monkeypatch):
    ...
    p.initialize(session_id="test-session", zed_home=str(tmp_path), platform="cli")
...
def test_normalize_retain_tags_accepts_csv_and_dedupes():
    assert _normalize_retain_tags("agent:fakeassistantname, source_system:zed-agent, agent:fakeassistantname") == [
        "agent:fakeassistantname",
        "source_system:zed-agent",
    ]


def test_normalize_retain_tags_accepts_json_array_string():
    value = json.dumps(["agent:fakeassistantname", "source_system:zed-agent"])
    assert _normalize_retain_tags(value) == ["agent:fakeassistantname", "source_system:zed-agent"]
...
class TestConfig:
    ...
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between Zed Agent and the User"
    ...
    def test_config_from_env_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "plugins.memory.hindsight.get_zed_home",
        ...
        assert cfg["banks"]["zed"]["bankId"] == "env-bank"
        assert cfg["banks"]["zed"]["budget"] == "high"
...
class TestPostSetup:
    def test_setup_cancel_at_mode_picker_writes_nothing(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("plugins.memory.hindsight.get_zed_home", lambda: zed_home)

        save_config = MagicMock()
        which = MagicMock(return_value="/usr/bin/uv")
        run = MagicMock()
        monkeypatch.setattr("zed_cli.memory_setup._curses_select", lambda *args, **kwargs: _CANCELLED)
        ...
        monkeypatch.setattr("zed_cli.config.save_config", save_config)

        provider = HindsightMemoryProvider()
        provider.post_setup(str(zed_home), {"memory": {"provider": "builtin"}})

        save_config.assert_not_called()
        which.assert_not_called()
        run.assert_not_called()
        assert not (zed_home / ".env").exists()
        assert not (zed_home / "hindsight" / "config.json").exists()
        assert not (user_home / ".hindsight" / "profiles" / "zed.env").exists()
    ...
    def test_local_embedded_setup_cancel_at_llm_picker_writes_nothing(self, tmp_path, monkeypatch):
        ...
        which = MagicMock(return_value="/usr/bin/uv")
        run = MagicMock()
        monkeypatch.setattr("zed_cli.memory_setup._curses_select", lambda *args, **kwargs: next(selections))
        monkeypatch.setattr("shutil.which", which)
        monkeypatch.setattr("subprocess.run", run)
        monkeypatch.setattr("builtins.input", MagicMock(side_effect=AssertionError("prompt should not run")))
        monkeypatch.setattr("getpass.getpass", MagicMock(side_effect=AssertionError("prompt should not run")))
    ...
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        ...
        saved_configs = []
        monkeypatch.setattr("zed_cli.config.save_config", lambda cfg: saved_configs.append(cfg.copy()))

        provider = HindsightMemoryProvider()
        provider.post_setup(str(zed_home), {"memory": {}})

        assert saved_configs[-1]["memory"]["provider"] == "hindsight"
        env_text = (zed_home / ".env").read_text()
        assert "HINDSIGHT_LLM_API_KEY=sk-local-test\n" in env_text
        assert "HINDSIGHT_TIMEOUT=120\n" in env_text
        assert "HINDSIGHT_IDLE_TIMEOUT=300\n" in env_text

        profile_env = user_home / ".hindsight" / "profiles" / "zed.env"
    ...
    def test_local_embedded_setup_respects_existing_profile_name(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config({"profile": "coder"}, str(zed_home))
        provider.post_setup(str(zed_home), {"memory": {}})

        coder_env = user_home / ".hindsight" / "profiles" / "coder.env"
        zed_env = user_home / ".hindsight" / "profiles" / "zed.env"
        assert coder_env.exists()
        assert not zed_env.exists()
    ...
    def test_local_embedded_setup_blank_inputs_preserve_existing_config(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config(existing_config, str(zed_home))
        ...
        monkeypatch.setattr("zed_cli.memory_setup._curses_select", lambda *args, **kwargs: kwargs.get("default", 0))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "")
        ...
        saved = json.loads((zed_home / "hindsight" / "config.json").read_text())
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config):
        ...
        item = call_kwargs["items"][0]
        assert item["context"] == "conversation between Zed Agent and the User"
        ...
        assert item["metadata"]["source"] == "zed"
    ...
    def test_sync_turn_uses_aretain_batch(self, provider):
        ...
        assert call_kwargs["retain_async"] is True
        assert len(call_kwargs["items"]) == 1
        assert call_kwargs["items"][0]["context"] == "conversation between Zed Agent and the User"
    ...
    def test_sync_turn_appends_only_delta_when_append_supported(self, provider_with_config, monkeypatch):
        ...
        try:
            ...
            # Only the delta â€” the already-retained turns must NOT be resent.
    ...
    def test_resume_creates_new_document(self, tmp_path, monkeypatch):
        ...
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        monkeypatch.setattr("plugins.memory.hindsight.get_zed_home", lambda: tmp_path)

        p1 = HindsightMemoryProvider()
        p1.initialize(session_id="resumed-session", zed_home=str(tmp_path), platform="cli")
        ...
        p2.initialize(session_id="resumed-session", zed_home=str(tmp_path), platform="cli")
    ...
    def test_sync_turn_preserves_unicode(self, provider_with_config):
        ...
        p = provider_with_config()
        p._client = _make_mock_client()
        p.sync_turn("ì•ˆë…• ã“ã‚“ã«ã¡ã¯ ä½ å¥½", "ðŸ‘¨â€ðŸ‘©â€ðŸ‘§â€ðŸ‘¦ family")
        p._retain_queue.join()
        p._client.aretain_batch.assert_called_once()
        item = p._client.aretain_batch.call_args.kwargs["items"][0]
        ...
        assert "ì•ˆë…•" in raw_json
        assert "ã“ã‚“ã«ã¡ã¯" in raw_json
        assert "ä½ å¥½" in raw_json
        assert "ðŸ‘¨â€ðŸ‘©â€ðŸ‘§â€ðŸ‘¦" in raw_json
...
class TestShutdownRace:
    def test_sync_turn_uses_single_writer_thread(self, provider):
        ...
        provider._retain_queue.join()
        # Same thread reused â€” no ad-hoc thread per call.
    ...
    def test_sync_turn_after_shutdown_is_dropped(self, provider):
        ...
        during interpreter teardown â€” that's what causes the
        ...
        # No new enqueue â€” the retain queue stays empty.
    ...
    def test_shutdown_drains_pending_retains(self, provider):
        ...
        Otherwise the LAST in-flight turn â€” typically the most important â€”
        is silently lost.
        """
        client = provider._client
        provider.sync_turn("a", "b")
...
# ---------------------------------------------------------------------------
# on_session_switch â€” flush + prefetch reset behavior
# ---------------------------------------------------------------------------
...
class TestSessionSwitchBufferFlush:
    def test_buffered_turns_flushed_before_clear(self, provider_with_config):
        ...
        # writer hasn't been started either â€” sync_turn's early return
        ...
        # Switch â€” flush should fire under OLD document_id via the writer queue.
    ...
    def test_no_flush_when_buffer_empty(self, provider):
        ...
        # Nothing enqueued â€” join is immediate.
        provider._retain_queue.join()
    ...
    def test_in_flight_prefetch_thread_drained_on_switch(self, provider, monkeypatch):
        ...
        # call on_session_switch â€” it must join the thread before clearing.
        gate.set()
        provider.on_session_switch("new-sid")
    ...
    def test_flush_serializes_behind_pending_retains_via_writer_queue(
        ...
        # behind it. This proves ordering â€” the flush MUST wait.
        ...
        # Turn 1+2 â†’ boundary hit â†’ retain enqueued (will block).
        p.sync_turn("turn1-user", "turn1-asst")
        p.sync_turn("turn2-user", "turn2-asst")
...
class TestUpdateModeAppendCapability:
    ...
    def test_legacy_api_falls_back_to_per_process_doc_id(self, provider, monkeypatch):
        """API returns no /version (or pre-0.5.0) â€” sync_turn must use the
    ...
    def test_modern_api_uses_stable_doc_id_with_append(self, provider, monkeypatch):
        """API on >=0.5.0 â€” retain uses stable session_id and sets update_mode='append'."""
        self._clear_capability_cache()
        monkeypatch.setattr(
            "plugins.memory.hindsight._fetch_hindsight_api_version",
            lambda *a, **kw: "0.5.6",
    ...
    def test_legacy_warning_emitted_once(self, provider, monkeypatch, caplog):
        ...
        # Cache hit on the second call â†’ no second warn.
        assert len(warns) == 1
...
class TestBankIdTemplate:
    def test_sanitize_bank_segment_passthrough(self):
        assert _sanitize_bank_segment("zed") == "zed"
    ...
    def test_resolve_collapses_empty_placeholders(self):
        # When user is empty, "zed-{user}" becomes "zed-" -> trimmed to "zed"
        result = _resolve_bank_id_template(
            "zed-{user}", fallback="default",
    ...
    def test_provider_uses_bank_id_template_from_config(self, tmp_path, monkeypatch):
        ...
        assert p._bank_id == "zed-coder"
        assert p._bank_id_template == "zed-{profile}"
    ...
    def test_provider_template_with_missing_profile_falls_back(self, tmp_path, monkeypatch):
        ...
        p = HindsightMemoryProvider()
        # No agent_identity passed â€” template renders to "zed-" which collapses to "zed"
        p.initialize(session_id="s1", zed_home=str(tmp_path), platform="cli")
        assert p._bank_id == "zed"
...
class TestSharedEventLoopLifecycle:
    """Regression tests for #11923 â€” Hindsight leaking aiohttp ClientSession /
    ...
    def test_shutdown_does_not_stop_shared_event_loop(self, provider_with_config):
        ...
        # Prime the shared loop by scheduling a trivial coroutine â€” mirrors
