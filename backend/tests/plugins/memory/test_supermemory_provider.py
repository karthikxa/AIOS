...
def provider(monkeypatch, tmp_path):
    ...
    p.initialize("session-1", zed_home=str(tmp_path), platform="cli")
    return p
...
def test_load_and_save_config_round_trip(tmp_path):
    ...
    # container_tag is kept raw â€” sanitization happens in initialize() after template resolution
...
def test_format_prefetch_context_deduplicates_overlap():
    result = _format_prefetch_context(
        ...
        dynamic_facts=["Jordan prefers short answers", "Uses Zed"],
        search_results=[{"memory": "Uses Zed", "similarity": 0.9}],
        max_results=10,
    )
    assert result.count("Jordan prefers short answers") == 1
    assert result.count("Uses Zed") == 1
...
def test_prefetch_includes_profile_on_first_turn(provider):
    provider._client.profile_response = {
        ...
        "search_results": [{"memory": "Working on Zed memory provider", "similarity": 0.88}],
    }
...
def test_sync_turn_buffers_short_messages(provider):
    # Trivial filtering is no longer applied at sync time â€” every non-empty turn
...
def test_sync_turn_buffers_cleaned_exchange(provider):
    ...
    # Buffering only â€” no per-turn writes to the client
    assert provider._client.add_calls == []
...
def test_merge_metadata_stamps_sm_source():
    # sm_source routes Zed writes into the "Zed" Space in the Supermemory
    # app (functional routing, not telemetry) â€” must always be present.
    from plugins.memory.supermemory import _SupermemoryClient

    client = _SupermemoryClient.__new__(_SupermemoryClient)
    merged = client._merge_metadata({"type": "explicit_memory"})
    assert merged["sm_source"] == "zed"
...
def test_shutdown_joins_threads_and_flushes_buffer(provider, monkeypatch):
    ...
    # sync_turn now only buffers â€” no thread is spawned.
    provider.sync_turn(
...
def test_identity_template_resolved_in_container_tag(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("SUPERMEMORY_API_KEY", "test-key")
    monkeypatch.setattr("plugins.memory.supermemory._SupermemoryClient", FakeClient)
    _save_supermemory_config({"container_tag": "zed-{identity}"}, str(tmp_path))
    p = SupermemoryMemoryProvider()
    p.initialize("s1", zed_home=str(tmp_path), platform="cli", agent_identity="coder")
    assert p._container_tag == "zed_coder"
...
def test_identity_template_default_profile(monkeypatch, tmp_path):
    ...
    _save_supermemory_config({"container_tag": "zed-{identity}"}, str(tmp_path))
    p = SupermemoryMemoryProvider()
    p.initialize("s1", zed_home=str(tmp_path), platform="cli")
    assert p._container_tag == "zed_default"
...
def test_multi_container_enabled_adds_schema_param(monkeypatch, tmp_path):
    ...
    assert p._allowed_containers == ["zed", "project_alpha", "shared"]
