...
def _isolate_env(tmp_path, monkeypatch):
    """Isolate ZED_HOME for each test.

    The global hermetic fixture already redirects ZED_HOME to a tempdir,
    but we want the plugin to work with a predictable subpath. We reset
    ZED_HOME here for clarity.
    """
...
def _load_plugin_init():
    ...
    spec = importlib.util.spec_from_file_location(
        "zed_plugins.disk_cleanup",
        plugin_dir / "__init__.py",
        submodule_search_locations=[str(plugin_dir)],
    )
    # Ensure parent namespace package exists for the relative `. import disk_cleanup`
    import types
    if "zed_plugins" not in sys.modules:
        ns = types.ModuleType("zed_plugins")
        ns.__path__ = []
        sys.modules["zed_plugins"] = ns
    mod = importlib.util.module_from_spec(spec)
    mod.__package__ = "zed_plugins.disk_cleanup"
    mod.__path__ = [str(plugin_dir)]
    sys.modules["zed_plugins.disk_cleanup"] = mod
    spec.loader.exec_module(mod)
    return mod
...
class TestIsSafePath:
    def test_accepts_path_under_zed_home(self, _isolate_env):
        dg = _load_lib()
    ...
    def test_rejects_outside_zed_home(self, _isolate_env):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/etc/passwd")) is False

    def test_accepts_tmp_zed_prefix(self, _isolate_env, tmp_path):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/tmp/zed-abc/x.log")) is True
...
class TestStaleCronEntryMigration:
    """Regression tests for #37721 â€” stale cron-output entries in tracked.json."""
    ...
    def test_quick_skips_protected_cron_paths_defense_in_depth(self, _isolate_env):
        ...
        # be auto-deleted) â€” the protected path guard must still block it.
        tracked_file = _isolate_env / "disk-cleanup" / "tracked.json"
        tracked_file.parent.mkdir(parents=True, exist_ok=True)
        tracked_file.write_text(json.dumps([{
            "path": str(tick_lock),
...
class TestTrackForgetQuick:
    ...
    def test_quick_does_not_descend_into_protected_top_level_dirs(
        ...
        dg = _load_lib()
        protected_empty = (
            _isolate_env / "zed-agent" / "node_modules" / "pkg" / "empty"
        )
        ...
        def guarded_iterdir(path):
            if path == _isolate_env / "zed-agent":
                raise AssertionError("quick() descended into protected zed-agent/")
...
class TestBundledDiscovery:
    def _write_enabled_config(self, zed_home, names):
    ...
    def test_disk_cleanup_discovered_but_not_loaded_by_default(self, _isolate_env):
        ...
        from zed_cli import plugins as pmod
