...
Covers three layers:
  * ``zed_cli.kanban_db`` accessors (add/list/get/delete + path helpers)
...
The plugin router is attached to a bare FastAPI app â€” same approach as
``test_kanban_dashboard_plugin.py`` â€” so we exercise the real HTTP path
...
def _load_plugin_router():
    ...
    spec = importlib.util.spec_from_file_location(
        "zed_dashboard_plugin_kanban_attach_test", plugin_file,
    )
...
def test_attachments_root_is_per_board(kanban_home, monkeypatch):
    ...
    monkeypatch.delenv("ZED_KANBAN_ATTACHMENTS_ROOT", raising=False)
...
def test_attachments_root_env_override(kanban_home, monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("ZED_KANBAN_ATTACHMENTS_ROOT", str(override))
...
# ---------------------------------------------------------------------------
# REST surface â€” upload / list / download / delete round-trip
# ---------------------------------------------------------------------------
