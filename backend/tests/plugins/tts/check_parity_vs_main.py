...
* ``builtin_<name>`` â€” config selects a built-in handler that exists
  on both main and PR (no diff expected)
* ``command`` â€” config selects a ``tts.providers.<name>: type: command``
  entry (PR #17843; no diff expected)
* ``plugin`` â€” config selects a plugin-registered provider (PR only)
* ``fallback_edge`` â€” config selects an unknown name with no matching
  plugin or command entry â†’ Edge TTS default fallback
* ``error`` â€” explicit fatal error (e.g. mistral quarantine)

The parent process diffs the reduced shape per scenario. The only
acceptable diff is ``fallback_edge â†’ plugin`` for the
``unknown-name-with-plugin-installed`` scenario â€” everything else is
a regression.

Run from the PR worktree (it auto-resolves ``MAIN_DIR`` from the parent
of the worktree directory, or falls back to a sibling
``zed-agent-main`` checkout)::
...
# The subprocess script â€” runs INSIDE either the main checkout or PR
...
# Try importing tts_registry â€” only exists on PR side.
...
SCENARIOS: list[tuple[str, str, dict[str, str], str]] = [
    ...
    # Scenario 1: unset tts.provider â†’ both: Edge default
    ("unset-defaults-to-edge", "", {}, "no"),

    # Scenario 2: built-in name â†’ both: that built-in
    ("explicit-edge", "tts:\n  provider: edge\n", {}, "no"),
    ("explicit-openai", "tts:\n  provider: openai\n", {}, "no"),
    ("explicit-elevenlabs", "tts:\n  provider: elevenlabs\n", {}, "no"),

    # Scenario 3: command-type provider â†’ both: command dispatch
    (
        "command-provider",
    ...
    # Scenario 4: unknown name with NO plugin installed â†’ both: fallback to Edge
    ...
    # Scenario 6: built-in name + plugin tries to shadow â†’ both: built-in
    # The plugin registers under name "cartesia", not "edge", so this is
    # effectively the same as scenario 2 â€” but we exercise the with-plugin
    # path to ensure the built-in branch still takes priority.
    ("explicit-edge-with-plugin-registered", "tts:\n  provider: edge\n", {}, "yes"),

    # Scenario 7: mistral quarantine â€” both surface the explicit error
...
def main() -> int:
    ...
    for label, config_yaml, env, plugin_register in SCENARIOS:
        ...
        if fallback_to_plugin:
            print(f"  [DIFF] {label}: fallback_edge â†’ plugin â€” expected")
    ...
    if intentional_diffs:
        print(
            ...
            f"fallback_edge â†’ plugin dispatch when a plugin is registered."
        )
