...
# ---------------------------------------------------------------------------
# Fake httpx â€” we don't want to hit the real Photon API in unit tests.
...
@pytest.fixture
def tmp_zed_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    home = tmp_path / "zed"
    home.mkdir()
...
def test_store_and_load_photon_token(tmp_zed_home: Path) -> None:
    photon_auth.store_photon_token("abc123def456")
    assert photon_auth.load_photon_token() == "abc123def456"

    auth_json = json.loads((tmp_zed_home / "auth.json").read_text())
    assert auth_json["credential_pool"]["photon"][0]["access_token"] == "abc123def456"


def test_store_project_credentials_round_trip(
    tmp_zed_home: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
    # Don't touch .env / os.environ here â€” exercise the auth.json path.
...
def test_request_device_code_uses_photon_cli(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Hosted Photon allowlists registered device clients â€” an unregistered
...
def test_list_projects_unwraps_list(monkeypatch: pytest.MonkeyPatch) -> None:
    def fake_get(url: str, **kwargs: Any) -> _FakeResponse:
        return _FakeResponse(json_body=[{"id": "p1", "name": "Zed Agent"}])
...
def test_find_project_by_name_case_insensitive(monkeypatch: pytest.MonkeyPatch) -> None:
    def fake_get(url: str, **kwargs: Any) -> _FakeResponse:
        return _FakeResponse(json_body={"data": [
            {"id": "p1", "name": "Other"},
            {"id": "p2", "name": "zed agent"},
        ]})

    monkeypatch.setattr(photon_auth.httpx, "get", fake_get)
    proj = photon_auth.find_project_by_name("tok", "Zed Agent")
...
def test_create_project_omits_spectrum_flag(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(photon_auth.httpx, "post", fake_post)
    data = photon_auth.create_project("tok", name="Zed Agent")
    ...
    assert captured["body"]["name"] == "Zed Agent"
...
def test_register_user_if_absent_dedup(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(photon_auth.httpx, "get", fake_get)
    monkeypatch.setattr(photon_auth.httpx, "post", fake_post)
    # Same number, different formatting â€” should match and NOT create.
...
def test_credential_summary_no_secret_leak(
    ...
    assert summary["device_token"].startswith("âœ“")
    assert summary["project_key"].startswith("âœ“")
    # Unified id: dashboard id == Spectrum id, surfaced as one project id.
    assert summary["project_id"] == "sp-uuid"
    assert summary["phone_number"].startswith("âœ— missing")
    assert summary["assigned_phone_number"].startswith("âœ— missing")
