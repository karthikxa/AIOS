...
modules â€” no mocking of provider classes themselves â€” so the test
...
class TestIsAvailable:
    ...
    def test_ddgs_always_available_when_package_importable(self) -> None:
        """DDGS is the always-on fallback â€” no API key required.

        It may report unavailable if the ``ddgs`` package itself isn't
        installed in the env (legitimate â€” the plugin's post_setup hook
...
# ---------------------------------------------------------------------------
# Registry resolution semantics (Option B â€” conservative smart fallback)
# ---------------------------------------------------------------------------
...
class TestRegistryResolution:
    ...
    def test_explicit_configured_provider_returned_even_when_unavailable(
        ...
        # Confirm it's the unavailable one â€” dispatcher will surface
    ...
    def test_unknown_configured_name_falls_back_to_available_provider(
        ...
        """Typo / uninstalled plugin â†’ walk legacy preference, pick available."""
        _ensure_plugins_loaded()
        from agent.web_search_registry import _resolve
        ...
        # premium provider) â€” both are valid. The point is the unknown
    ...
    def test_explicit_search_only_provider_for_extract_falls_back(
        ...
        """Asking for extract via a search-only backend â†’ fall back.
    ...
    def test_no_config_no_credentials_returns_none(
        ...
        """No backend configured AND no available providers â†’ typically None.
