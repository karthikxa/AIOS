...
Covers ``plugins/security-guidance/``:
  ...
  * ``patterns.py`` data integrity â€” every rule has a ``RuleId``, the
    fail-loud import assertion is wired.
  * ``_scan_content`` â€” true positives (pickle.load, yaml.load, eval,
    ...
    (``path_filter``).
  * Hooks â€” ``transform_tool_result`` appends a warning block in warn
...
def _load_plugin_init():
    ...
    spec = importlib.util.spec_from_file_location(
        "zed_plugins.security_guidance",
        plugin_dir / "__init__.py",
        submodule_search_locations=[str(plugin_dir)],
    )
    mod = importlib.util.module_from_spec(spec)
    mod.__package__ = "zed_plugins.security_guidance"
    mod.__path__ = [str(plugin_dir)]
    sys.modules["zed_plugins.security_guidance"] = mod
    spec.loader.exec_module(mod)
    return mod
...
class TestPatternsData:
    ...
    def test_every_rule_has_required_fields(self):
        ...
        for rule in p.SECURITY_PATTERNS:
            ...
            # At least one of substrings/regex/path_check must be present â€”
    ...
    def test_rule_names_to_mask_packs_bits(self):
        p = _load_patterns()
        # PICKLE_DESERIALIZATION = 8, EVAL_INJECTION = 4 â†’ bits 8 and 4 set.
...
class TestScanContent:
    ...
    def test_github_workflow_path_check_fires_on_path_alone(self):
        """github_actions_workflow has no regex/substring â€” fires on path."""
        mod = _load_plugin_init()
        findings = mod._scan_content(
    ...
    def test_huge_content_skipped(self):
        mod = _load_plugin_init()
        # 1 MB of content with a dangerous pattern at the end â€” scanner caps
...
class TestTransformToolResultHook:
    ...
    def test_no_warn_when_result_is_error(self):
        ...
        # top â€” the model has bigger problems to solve.
        assert (
            mod._on_transform_tool_result(
