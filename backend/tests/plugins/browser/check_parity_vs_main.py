...
Run from the PR worktree:
    ...
    cd ~/.zed/zed-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.zed/zed-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.zed/zed-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a zed-agent checkout"
)
...
# Reduced shape comparison â€” exact instance addresses obviously differ
# between subprocesses, so we compare the parts that matter for users.
SUBPROCESS_SCRIPT = r"""
import json, os, sys, tempfile
sys.path.insert(0, sys.argv[1])

# Isolated ZED_HOME for the config write.
...
# Fresh import â€” must not have any browser modules cached.
...
# returning display_name). Both shapes resolve to the same string â€”
# 'Browserbase' / 'Browser Use' / 'Firecrawl' â€” so we can compare safely.
provider_name = None
is_available = None
...
def _reduce_for_comparison(shape: dict) -> dict:
    ...
    We compare ``(is_local, provider_name, is_available)`` â€” the trio that
