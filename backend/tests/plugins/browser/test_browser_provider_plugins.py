...
- The browser_registry resolves an active provider in the documented
  scenarios:
    ...
    * legacy preference walk: browser-use â†’ browserbase (filtered by
      availability)
    * firecrawl is NOT in the legacy walk â€” explicit-only
    * unknown name falls through to auto-detect
    * ``local`` short-circuits to None

These tests use *real* imports from the plugin modules â€” no mocking of
provider classes themselves â€” so the test catches drift in the ABC
...
class TestBundledPluginsRegister:
    ...
    def test_each_plugin_implements_full_lifecycle(self, plugin_name: str) -> None:
        ...
        # default â€” we check by comparing the function reference.
...
class TestRegistryResolution:
    ...
    def test_resolve_none_with_no_creds_returns_none(self) -> None:
        """No config, no env â†’ local mode (None)."""
        _ensure_plugins_loaded()
    ...
    def test_explicit_browserbase_returns_provider_even_when_unavailable(self) -> None:
        ...
        This is critical â€” the dispatcher needs to surface a typed
    ...
    def test_explicit_unknown_falls_back_to_auto_detect(self) -> None:
        """Rule 1 miss: unknown name â†’ fall through to legacy walk."""
        _ensure_plugins_loaded()
        from agent.browser_registry import _resolve
    ...
    def test_legacy_walk_prefers_browser_use_over_browserbase(
        ...
        """Rule 3: walk order is browser-use â†’ browserbase."""
        _ensure_plugins_loaded()
        from agent.browser_registry import _resolve

        # Both available â€” browser-use should win.
    ...
    def test_legacy_walk_falls_through_to_browserbase(
        ...
        """Rule 3: browser-use unavailable â†’ browserbase picked."""
        _ensure_plugins_loaded()
        from agent.browser_registry import _resolve
    ...
    def test_firecrawl_not_in_legacy_walk_even_when_only_one_available(
        ...
        with the *web* firecrawl plugin â€” auto-routing a web-extract user
        ...
        # Only firecrawl is_available() â€” but it's not in the legacy walk.
        assert _resolve(None) is None
...
class TestPickerIntegration:
    ...
    def test_picker_rows_match_registered_plugins(self) -> None:
        _ensure_plugins_loaded()
        from zed_cli.tools_config import _plugin_browser_providers
