...
plugin SDK (``window.__ZED_PLUGIN_SDK__``): ``fetchJSON`` (JSON REST),
``authedFetch`` (uploads / blob downloads), and ``buildWsUrl`` /
``buildWsAuthParam`` (WebSockets). These handle BOTH dashboard auth modes â€”
loopback (``X-Zed-Session-Token`` header) and gated OAuth
(``zed_session_at`` cookie / single-use ``?ticket=``).

Plugins that hand-roll ``fetch`` / ``WebSocket`` and read
``window.__ZED_SESSION_TOKEN__`` directly send an empty token in gated mode
...
the enforcement half of the "single sanctioned auth surface" design â€” the SDK
...
# Repo root: tests/plugins/<this file> â†’ ../../
_REPO_ROOT = Path(__file__).resolve().parents[2]
_PLUGINS_DIR = _REPO_ROOT / "plugins"

# The forbidden global. Reading it directly bypasses the gated-mode auth path.
_FORBIDDEN = "__ZED_SESSION_TOKEN__"

# Files explicitly allowed to mention the token (none today). Map path â†’
...
def test_there_are_plugin_bundles_to_check() -> None:
    ...
    # kanban + zed-achievements are bundled today; assert at least one is
    # found so the guard can't pass vacuously.
    assert bundles, "no plugin dashboard bundles found â€” glob/layout drift?"
...
def test_plugin_bundle_does_not_read_session_token(bundle: Path) -> None:
    ...
    pytest.fail(
        f"{rel} reads {_FORBIDDEN} directly â€” this bypasses gated-mode auth "
