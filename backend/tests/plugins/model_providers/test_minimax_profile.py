...
Refs:
  ...
  - PR #6082: M2.7-highspeed â†’ M2.7 for aux model (half-price fix)
...
def minimax_profile(request):
    ...
    Going through ``providers.get_provider_profile`` keeps the test honest â€”
...
class TestMinimaxAuxModelM3:
    ...
    ``zed_cli.models._PROVIDER_MODELS['minimax']`` and the
    ...
    â€” picking it as the aux default matches the forward-looking
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # M2.7 â€” matching PR #6082's intent.
    ...
    def test_consumer_api_returns_non_empty_for_each_provider(self, minimax_profile):
        ...
        assert resolved != "", (
            f"_get_aux_model_for_provider({provider_id!r}) returned empty â€” "
        ...
        assert resolved == profile.default_aux_model, (
            ...
            "â€” the consumer API and the profile have drifted out of sync"
        )
...
class TestMinimaxAuxModelNotHighspeed:
    ...
    def test_default_aux_model_is_not_highspeed(self, provider_id):
        ...
        assert "highspeed" not in profile.default_aux_model.lower(), (
            ...
            "is a -highspeed variant â€” that costs 2x for the same model and "
