...
Covers four shapes from Phase 4 of ``.zed/plans/2026-05-21-dashboard-oauth-auth.md``:
...
4. ``verify_session`` JWT verification â€” RSA keypair, audience/issuer pinning,
...
# ---------------------------------------------------------------------------
# RSA keypair fixture (module-scope â€” keygen is slow)
# ---------------------------------------------------------------------------
...
class TestConstruction:
    ...
    def test_name_and_display(self):
        ...
        assert p.name == "nous"
        assert p.display_name == "Zed Team"
    ...
    def test_rejects_malformed_client_id(self):
        with pytest.raises(ValueError, match="agent:"):
            nous_plugin.NousDashboardAuthProvider(
                client_id="zed-dashboard", portal_url="https://x"
            )
...
class TestPluginRegister:
    def test_skips_when_client_id_missing(self, monkeypatch):
        monkeypatch.delenv("ZED_DASHBOARD_OAUTH_CLIENT_ID", raising=False)
        monkeypatch.delenv("ZED_DASHBOARD_PORTAL_URL", raising=False)
        ctx = MagicMock()
        nous_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_not_called()
        # Skip reason is surfaced for the gate's fail-closed message.
        assert "ZED_DASHBOARD_OAUTH_CLIENT_ID" in nous_plugin.LAST_SKIP_REASON

    def test_registers_with_default_portal_url_when_only_client_id_set(
        self, monkeypatch
    ):
        """Phase 7 follow-up: ZED_DASHBOARD_PORTAL_URL is optional â€”
        defaults to the production Nous Portal. The user shouldn't have
        to set it for the common production deployment path."""
        monkeypatch.setenv("ZED_DASHBOARD_OAUTH_CLIENT_ID", "agent:inst1")
        monkeypatch.delenv("ZED_DASHBOARD_PORTAL_URL", raising=False)
        ctx = MagicMock()
        nous_plugin.register(ctx)
        ...
        assert registered._portal_url == "https://portal.zedteam.com"
    ...
    def test_skips_when_client_id_malformed(self, monkeypatch):
        monkeypatch.setenv("ZED_DASHBOARD_OAUTH_CLIENT_ID", "zed-dashboard")
        monkeypatch.setenv("ZED_DASHBOARD_PORTAL_URL", "https://p.example")
        ctx = MagicMock()
        nous_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_not_called()
        # Skip reason names the offending value + contract shape.
        assert "agent:" in nous_plugin.LAST_SKIP_REASON
        assert "zed-dashboard" in nous_plugin.LAST_SKIP_REASON
    ...
    def test_strips_whitespace_from_env_vars(self, monkeypatch):
        monkeypatch.setenv("ZED_DASHBOARD_OAUTH_CLIENT_ID", "  agent:x  ")
        monkeypatch.setenv("ZED_DASHBOARD_PORTAL_URL", "  https://p.example  ")
        ctx = MagicMock()
        nous_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
    ...
    def test_empty_portal_url_env_uses_default(self, monkeypatch):
        ...
        default â€” same handling as 'unset' so an empty Fly secret can't
        accidentally point the dashboard at nowhere."""
        monkeypatch.setenv("ZED_DASHBOARD_OAUTH_CLIENT_ID", "agent:inst1")
        monkeypatch.setenv("ZED_DASHBOARD_PORTAL_URL", "")
        ctx = MagicMock()
        nous_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
        assert registered._portal_url == "https://portal.zedteam.com"
...
class TestConfigYamlSource:
    ...
    canonical surface for these settings. ``ZED_DASHBOARD_OAUTH_CLIENT_ID``
    and ``ZED_DASHBOARD_PORTAL_URL`` are operator overrides that win when
    set â€” this is the contract Fly.io's platform-secret injection relies on,
    ...
    def patch_config(self, monkeypatch):
        """Yield a callable that replaces ``zed_cli.config.load_config``
    ...
    def test_config_yaml_only_client_id_registers(self, patch_config, monkeypatch):
        """No env var, only config.yaml â€” plugin reads from config and
    ...
    def test_env_overrides_config_client_id(self, patch_config, monkeypatch):
        ...
        ZED_DASHBOARD_OAUTH_CLIENT_ID at deploy time and we MUST
        honour it even if a stale config.yaml ships in the image."""
        monkeypatch.setenv("ZED_DASHBOARD_OAUTH_CLIENT_ID", "agent:from-env")
        patch_config({"client_id": "agent:from-config"})
        ctx = MagicMock()
        nous_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
    ...
    def test_env_overrides_config_portal_url(self, patch_config, monkeypatch):
        monkeypatch.setenv("ZED_DASHBOARD_OAUTH_CLIENT_ID", "agent:x")
        monkeypatch.setenv(
            "ZED_DASHBOARD_PORTAL_URL", "https://env.portal.example",
        )
        patch_config({
            "client_id": "agent:x",
    ...
    def test_empty_env_string_does_not_shadow_config(
        ...
        """``ZED_DASHBOARD_OAUTH_CLIENT_ID=`` (set but empty) is
        ...
        string â€” operators would lose the gate."""
        monkeypatch.setenv("ZED_DASHBOARD_OAUTH_CLIENT_ID", "")
        patch_config({"client_id": "agent:from-config"})
        ctx = MagicMock()
        nous_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
    ...
    def test_neither_source_skips_with_helpful_reason(
        ...
        """Neither env nor config.yaml set â€” skip with a reason that
        ...
        assert "dashboard.oauth.client_id" in nous_plugin.LAST_SKIP_REASON, (
            f"skip reason omits the config.yaml surface â€” operators "
    ...
    def test_config_yaml_load_failure_falls_through_cleanly(
        ...
        plugin must not crash â€” it falls through to the env-only path
        ...
        monkeypatch.setattr(
            "zed_cli.config.load_config", _broken_load
        )
        ctx = MagicMock()
        # Must not raise.
        nous_plugin.register(ctx)
...
class TestStartLogin:
    ...
    def test_returns_login_start(self, provider):
        result = provider.start_login(
            redirect_uri="https://zed.fly.dev/auth/callback"
        )
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https://zed.fly.dev/auth/callback"
    ...
    def test_code_verifier_in_cookie_payload_43_to_128_chars(self, provider):
        ...
        assert "zed_session_pkce" in result.cookie_payload
        pkce = result.cookie_payload["zed_session_pkce"]
        # Shape: ``state=â€¦;verifier=â€¦`` (matches stub-provider convention so
    ...
    def test_two_calls_produce_different_state_and_verifier(self, provider):
        ...
        assert a.cookie_payload["zed_session_pkce"] != b.cookie_payload[
            "zed_session_pkce"
        ]
    ...
    def test_allows_http_with_arbitrary_host(self, provider):
        # http:// is permitted for any host now, not just localhost â€” the
        ...
        provider.start_login(redirect_uri="http://zed.fly.dev/auth/callback")
...
class TestVerifySession:
    ...
    def test_verification_failure_message_surfaces_token_claims(
        ...
        config drift between ZED_DASHBOARD_PORTAL_URL/CLIENT_ID and Portal."""
        token = _mint_token(rsa_keypair, iss="https://evil.example")
    ...
    def test_missing_sub_raises(self, provider, rsa_keypair):
        ...
        # InvalidTokenError â†’ ProviderError before we ever touch _session_from_claims.
...
class TestRefreshAndRevoke:
    ...
    def test_refresh_400_raises_refresh_expired(self, provider):
        # Expired / revoked / reuse-detected RT â†’ Portal 400 â†’ force re-login.
        mock_resp = self._mock_post(400, {"error": "invalid_grant"})
        with patch("plugins.dashboard_auth.nous.httpx.post", return_value=mock_resp):
    ...
    def test_refresh_empty_token_raises_refresh_expired_without_network(self, provider):
        # No RT present â€” fail fast as a dead session, never hit the network.
