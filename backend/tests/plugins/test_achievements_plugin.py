"""Tests for the bundled zed-achievements dashboard plugin.

These target the two behaviors that matter for official integration:

* The 200-session scan cap is removed â€” the plugin now walks the entire
...
``plugins/zed-achievements/tests/`` covering the achievement engine
internals (tier math, secret-state handling, catalog invariants). These
tests live at the zed-agent level and focus on the integration
...
def plugin_api(tmp_path, monkeypatch):
    """Load plugin_api with isolated ~/.zed so state/snapshot files don't collide.
...
def _install_fake_session_db(plugin_api, fake_db):
    ...
    ``sys.modules['zed_state']`` swap is auto-restored at test teardown
    and cannot leak into unrelated tests in the same xdist worker.
    """
    fake_module = type(sys)("zed_state")
    fake_module.SessionDB = lambda: fake_db
    plugin_api._test_monkeypatch.setitem(sys.modules, "zed_state", fake_module)
...
def test_evaluate_all_first_run_returns_pending_and_starts_background_scan(plugin_api):
    ...
    large DBs take minutes â€” blocking the dashboard request path is not
    acceptable.
    """
    ...
    # Immediate return â€” should not block waiting for the scan.
...
def test_evaluate_all_force_runs_synchronously(plugin_api):
    """Manual /rescan (force=True) blocks the caller â€” users clicking
    ...
    # Synchronous â€” snapshot is fresh on return.
...
def test_background_scan_publishes_partial_snapshots(plugin_api):
    ...
    # scan 750 sessions with progress_every=250 â†’ expect 2 intermediate
...
def test_partial_snapshots_do_not_persist_unlock_timestamps(plugin_api):
    """Intermediate snapshots must not write to state.json â€” an unlock
