...
# ---------------------------------------------------------------------------
# Imports â€” guarded since plugins/memory lives outside the standard test path
# ---------------------------------------------------------------------------

@pytest.fixture(autouse=True)
def _isolate_env(tmp_path, monkeypatch):
    """Ensure ZED_HOME and RETAINDB vars are isolated."""
...
def _cap_retaindb_sleeps(monkeypatch):
    ...
    up. Cap the module's bound ``time.sleep`` to 0.05s â€” tests don't care
...
class TestWriteQueue:
    ...
    def test_enqueue_creates_row(self, tmp_path):
        ...
        # shutdown() blocks until the writer thread drains the queue â€” no need
    ...
    def test_enqueue_persists_to_sqlite(self, tmp_path):
        ...
        # 0.5s is plenty â€” the test just needs the flush to still be in-flight.
        ...
        # Check SQLite directly â€” row should exist since flush is slow
    ...
    def test_crash_recovery_replays_pending(self, tmp_path):
        ...
        # Now create a new queue â€” it should replay the pending rows
        client2 = MagicMock()
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", zed_home=str(tmp_path / ".zed"))
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", zed_home=profile_home)
        assert p._client.project == "zed-coder"
        p.shutdown()

    def test_initialize_seeds_soul_md(self, tmp_path, monkeypatch):
        p = self._make_provider(tmp_path, monkeypatch)
        soul_path = tmp_path / ".zed" / "SOUL.md"
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        zed_home = tmp_path / ".zed"
        zed_home.mkdir(exist_ok=True)
        monkeypatch.setenv("ZED_HOME", str(zed_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", zed_home=str(zed_home))
        return p
    ...
    def test_thread_accumulation_guard(self, tmp_path, monkeypatch):
        ...
        with patch.object(p, "_prefetch_context", side_effect=lambda q: time.sleep(0.5)), \
            ...
            # Call again â€” should join first batch before spawning new
