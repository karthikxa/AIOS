...
Covers the safety-gated pieces that don't require Playwright:
  ...
  * URL regex â€” only ``https://meet.google.com/`` URLs pass
...
Does NOT spawn a real Chromium â€” we mock ``subprocess.Popen`` where needed.
"""
...
# ---------------------------------------------------------------------------
# _BotState â€” transcript + status file round-trip
# ---------------------------------------------------------------------------
...
def test_bot_state_dedupes_captions_and_flushes_status(tmp_path):
    ...
    state.record_caption("Alice", "Hey everyone")  # dup â€” ignored
    ...
    # dedup â€” Alice line appears exactly once
...
# ---------------------------------------------------------------------------
# process_manager â€” refuses unsafe URLs, manages active pointer
# ---------------------------------------------------------------------------
...
def test_start_spawns_subprocess_and_writes_active_pointer(tmp_path):
    ...
    assert captured_env["ZED_MEET_URL"] == "https://meet.google.com/abc-defg-hij"
    assert captured_env["ZED_MEET_GUEST_NAME"] == "Test Bot"
    assert captured_env["ZED_MEET_DURATION"] == "15m"
...
def test_transcript_reads_last_n_lines(tmp_path):
    ...
    meeting_dir = Path(os.environ["ZED_HOME"]) / "workspace" / "meetings" / "abc-defg-hij"
...
# ---------------------------------------------------------------------------
# Tool handlers â€” JSON shape + safety gates
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# _on_session_end â€” defensive cleanup
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Plugin register() â€” platform gating + tool registration
# ---------------------------------------------------------------------------
...
def test_enqueue_say_rejects_transcribe_mode(tmp_path):
    ...
    out_dir = Path(os.environ["ZED_HOME"]) / "workspace" / "meetings" / "abc-defg-hij"
...
def test_start_realtime_env_vars_threaded_through():
    ...
    assert captured_env["ZED_MEET_MODE"] == "realtime"
    assert captured_env["ZED_MEET_REALTIME_MODEL"] == "gpt-realtime"
    assert captured_env["ZED_MEET_REALTIME_VOICE"] == "alloy"
    assert captured_env["ZED_MEET_REALTIME_INSTRUCTIONS"] == "Be brief."
    assert captured_env["ZED_MEET_REALTIME_KEY"] == "sk-test"
...
def test_cli_register_includes_node_subcommand():
    """`zed meet` argparse tree includes the node subtree."""
    import argparse
    from plugins.google_meet.cli import register_cli

    parser = argparse.ArgumentParser(prog="zed meet")
    register_cli(parser)
...
def test_looks_like_human_speaker():
    ...
    # Blank, "unknown", "you", and the bot's own name â†’ not human (no barge-in)
    for s in ("", "   ", "Unknown", "unknown", "You", "you", "Zed Agent", "zed agent"):
        assert not _looks_like_human_speaker(s, "Zed Agent"), f"{s!r} should NOT be human"
    # Real names â†’ human (barge-in)
    for s in ("Alice", "Bob Lee", "@teknium"):
        assert _looks_like_human_speaker(s, "Zed Agent"), f"{s!r} SHOULD be human"
...
def test_realtime_session_cancel_response_when_disconnected():
    ...
    # No _ws yet â€” cancel should no-op and return False.
...
def test_cmd_install_realtime_skips_when_deps_present(capsys):
    ...
    assert rc == 0
    # No sudo apt-get call â€” paplay was already on PATH.
