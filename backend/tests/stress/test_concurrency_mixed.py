...
def reclaimer_loop(zed_home: str, result_file: str) -> None:
...
def main():
    home = tempfile.mkdtemp(prefix="zed_mixed_stress_")
    ...
    for comp in completions:
        ...
        if claim is None:
            ...
            # â€” that's still a bug: the worker shouldn't be able to complete
    ...
    # DB final state â€” every task should be in a clean terminal state.
    conn = kb.connect()
    try:
        ...
        # Counts â€” should roughly balance.
        status_counts = dict(
    ...
    if failures:
        ...
        print()
        print("âœ” ALL INVARIANTS HELD UNDER MIXED STRESS")
