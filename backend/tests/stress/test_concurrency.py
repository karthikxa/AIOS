...
def worker_loop(worker_id: int, zed_home: str, result_file: str) -> None:
    ...
    while time.monotonic() - start < WORKER_TIMEOUT_S:
        conn = kb.connect()
        try:
            # Find any ready task (non-deterministic order intentional â€” we
            ...
            try:
                ...
                continue
            if claimed is None:
                # Someone else beat us â€” expected contention, not an error.
...
def main():
    home = tempfile.mkdtemp(prefix="zed_concurrency_")
    print(f"ZED_HOME = {home}")
    ...
    for i, f in enumerate(result_files):
        if not os.path.isfile(f):
            print(f"  WORKER {i} produced no result file â€” died?")
            continue
        with open(f) as fh:
            events = json.load(fh)
    ...
    # Check 3: DB state â€” every task should be in 'done', no dangling claims
    conn = kb.connect()
    try:
        ...
        # Check 5: event counts â€” exactly NUM_TASKS completed events
