...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves zed_cli.kanban_db on its own
  ...
  - worker logs are captured to ZED_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "ZED_KANBAN_TASK": task.id,
            "ZED_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="zed_e2e_")
    os.environ["ZED_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from zed_cli import kanban_db as kb

    # Point the `zed` CLI child processes will run at the worktree
    # zed_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "zed")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m zed_cli.main "$@"
""")
    ...
    print("\n  âœ” Scenario A: all 3 real-subprocess workers completed cleanly")
    ...
    # Simulate next dispatcher tick â€” should detect the crashed PID
    ...
    if len(crashed) < 1:
        print("  âœ— crash NOT detected")
        sys.exit(1)
    if task.status != "ready":
        print(f"  âœ— task should be back to ready, got {task.status}")
        sys.exit(1)
    if runs[0].outcome != "crashed":
        print(f"  âœ— run outcome should be 'crashed', got {runs[0].outcome!r}")
        sys.exit(1)
    print("\n  âœ” Scenario B: crash detected, task re-queued, run outcome=crashed")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/zed_e2e_*/worker_*.log
    import glob
