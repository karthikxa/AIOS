...
normal tests assume away:
  ...
  - Filesystem: ZED_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own ZED_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            home = tempfile.mkdtemp(prefix=f"zed_atyp_{name}_")
            os.environ["ZED_HOME"] = home
            os.environ["HOME"] = home
            for m in list(sys.modules.keys()):
                if m.startswith(("zed_cli", "plugins", "gateway")):
                    del sys.modules[m]
            sys.path.insert(0, str(WT))
            from zed_cli import kanban_db as kb  # noqa: F401
            print(f"\nâ•â•â• {name} â•â•â•")
            try:
                fn(home, kb)
                print(f"  âœ” {name}")
...
def _(home, kb):
    ...
    try:
        ...
        cases = [
            ("ðŸ“‹ buy groceries ðŸŽ", "shopping"),
            ("è®¾è®¡è®¤è¯æ¨¡å¼", "implement"),
            ("××™×ž×•×ª ×ž×©×ª×ž×© ×—×“×©", "auth-rtl"),  # Hebrew RTL
            ("Ù…Ù‡Ù…Ø© ØªØµØ­ÙŠØ­ Ø§Ù„Ø£Ø®Ø·Ø§Ø¡", "bug-arabic"),
            ("ðŸ‘¨â€ðŸ‘©â€ðŸ‘§â€ðŸ‘¦ family emoji ZWJ sequences ðŸ³ï¸â€ðŸŒˆ", "emoji-stress"),
        ...
        for title, kind in cases:
            ...
            assert back.title == title, (
                f"[{kind}] round-trip mismatch: {title!r} â†’ {back.title!r}"
            )
        ...
        meta = {
            "ä½œè€…": "å¼ ä¸‰",
            "summary_fr": "rÃ©sumÃ© avec des caractÃ¨res accentuÃ©s",
            "emoji": "ðŸŽ‰ðŸ”¥ðŸ’¯",
            "mixed_list": ["normal", "æ—¥æœ¬èªž", "ðŸ‡ºðŸ‡¸"],
        }
        kb.complete_task(
            conn, tid,
            summary="å®Œæˆäº† ðŸ“ rÃ©sumÃ©",
            metadata=meta,
        )
        run = kb.latest_run(conn, tid)
        assert run.summary == "å®Œæˆäº† ðŸ“ rÃ©sumÃ©", f"summary round-trip failed"
...
def _(home, kb):
    ...
    The notifier truncates to first line â€” verify that's right, not
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "ZED_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
...
def _(home, kb):
    """A â†’ B â†’ A should be refused. If it's allowed, recompute_ready
    ...
    try:
        ...
        # Try to link A back to B â€” creating the cycle
        try:
...
def _(home, kb):
    """Root â†’ (A, B) â†’ leaf. Leaf should promote to ready only when
    ...
    try:
        ...
        # A done but B not â†’ leaf stays todo
...
def _(home, kb):
    ...
    try:
        ...
        print(f"  500 parents â†’ 1 child promotion works")
    finally:
        conn.close()
...
def _(home, kb):
    ...
    conn = kb.connect()
    try:
        # Direct kernel API â€” create with an attacker-ish path
        tid = kb.create_task(
        ...
        # Document what actually happens â€” is the path stored verbatim?
        ...
        # allow escape.
        try:
            from zed_cli.kanban_db import resolve_workspace
            ...
            home_abs = str(Path(os.environ["ZED_HOME"]).resolve())
            if not resolved_abs.startswith(home_abs) and resolved_abs.startswith("/tmp"):
                ...
                print(f"  âš  workspace resolved OUTSIDE zed_home: {resolved}")
                print(f"    (not necessarily a bug â€” dir: workspaces are intentionally arbitrary, but worth documenting)")
        except Exception as e:
...
def _(home, kb):
    ...
    try:
        ...
        if task.status == "ready":
            ...
            # Workspace not checked before spawn â€” the worker would hit
...
def _(home, kb):
    ...
    try:
        ...
        conn.commit()
        # Complete normally â€” ended_at will be now, < started_at
        ...
        if run.ended_at < run.started_at:
            ...
            print(f"  âš  kernel stores this; UI should clamp to 0 or handle")
            # Don't fail â€” document the behavior.
        else:
...
@scenario("zed_home_with_spaces")
def _(home, kb):
    """ZED_HOME at a path with spaces â€” should work but catches
    ...
    weird = tempfile.mkdtemp(prefix="zed with spaces ")
    os.environ["ZED_HOME"] = weird
    ...
    try:
        ...
        print(f"  ZED_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("zed_home_with_unicode")
def _(home, kb):
    """ZED_HOME with non-ASCII chars."""
    # Pre-create directly since tempfile doesn't love unicode prefixes
    weird = f"/tmp/zed_hÃ©llo_Ã©mÃ¶ji_{os.getpid()}"
    ...
    try:
        ...
        print(f"  ZED_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("zed_home_via_symlink")
def _(home, kb):
    """ZED_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    uses Path.resolve() â€” two different symlink names pointing at the
    same dir should NOT double-init."""
    real = tempfile.mkdtemp(prefix="zed_real_")
    link1 = real + "_link1"
    link2 = real + "_link2"
    os.symlink(real, link1)
    os.symlink(real, link2)
    try:
        os.environ["ZED_HOME"] = link1
        ...
        conn2.close()
        print("  symlinks to same ZED_HOME share DB correctly")
    finally:
...
def _(home, kb):
    """1000 reclaim cycles on a single task â†’ 1000 run rows. Verify
    ...
    try:
        ...
        print(f"  1000 runs â†’ list_runs OK; build_worker_context = {elapsed:.0f}ms, {len(ctx)} chars")
        if len(ctx) > 200_000:
            print(f"  âš  build_worker_context unbounded on retry-heavy tasks "
                  f"({len(ctx)} chars) â€” worker context will be huge")
    finally:
        conn.close()
...
def _idempotency_race_worker(zed_home: str, key: str, result_file: str,
...
def _(home, kb):
    ...
    idempotency_key â€” should both get back the SAME task id, not two
    different ones."""
    kb.init_db()
...
def _(home, kb):
    ...
    try:
        ...
        for a in assignees:
            ...
            if a:
                assert back.assignee == a, f"assignee round-trip: {a!r} â†’ {back.assignee!r}"
...
def _(home, kb):
    """A task in 'done' should NOT be reclaimable â€” reclaim/claim paths
    must refuse."""
    kb.init_db()
    conn = kb.connect()
    try:
...
def _(home, kb):
    """1000 comments on a single task â€” build_worker_context should still
    be reasonable."""
    kb.init_db()
    conn = kb.connect()
    try:
        ...
        if len(ctx) > 200_000:
            print(f"  âš  comment thread unbounded in worker context")
    finally:
        conn.close()
...
def _(home, kb):
    ...
    store what it gets â€” but queries filtering by tenant should still
    ...
    try:
        ...
        # board_stats groups by tenant â€” verify it doesn't fall over
        stats = kb.board_stats(conn)
...
def _(home, kb):
    ...
    try:
        ...
        for parent, expected in [
            (p_ready, "todo"),     # parent not done â†’ child stays todo
            (p_running, "todo"),
            (p_blocked, "todo"),
            (p_triage, "todo"),
...
def _(home, kb):
    ...
    try:
        from zed_cli import web_server as ws  # noqa
    except Exception:
        pass
    ...
    # Should succeed â€” kernel doesn't cap title length
    assert r.status_code == 200

    # Unicode + emoji
    r = client.post("/api/plugins/kanban/tasks", json={
        "title": "ðŸ“‹ deploy ðŸš€ to ç”Ÿäº§",
        "body": "æ—¥æœ¬èªž body",
        "assignee": "deploy-bot",
    })
    assert r.status_code == 200
    tid = r.json()["task"]["id"]
    assert r.json()["task"]["title"] == "ðŸ“‹ deploy ðŸš€ to ç”Ÿäº§"

    # Invalid JSON schema â€” unknown field, pydantic should either ignore or 422
    r = client.post("/api/plugins/kanban/tasks", json={
...
def main():
    ...
    if FAILURES:
        ...
        print("\nâœ” ALL ATYPICAL SCENARIOS HANDLED CORRECTLY")
