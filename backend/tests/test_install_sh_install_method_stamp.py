...
($INSTALL_DIR), not into the shared $ZED_HOME.

Background (shared-$ZED_HOME bug)
------------------------------------
$ZED_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.zed:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$ZED_HOME the two installs would fight over one slot â€” and the container,
...
``~/.zed/zed-agent``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$ZED_HOME/\.install_method"', text), (
        "install.sh must not stamp $ZED_HOME/.install_method â€” that data "
        "dir may be shared with a Docker gateway whose 'docker' stamp would "
        "clobber it and block host-side `zed update`"
    )
