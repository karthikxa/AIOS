...
Phase 4 wires `zed -p <profile> gateway start/stop` through the s6
ServiceManager dispatch path inside the container â€” so the lifecycle
commands now bring up an s6-supervised gateway rather than refusing
with the pre-Phase-4 informational message.

These tests were marked ``xfail(strict=True)`` through Phase 0â€“3 and
flip to plain ``test_â€¦`` once Phase 4 lands (now).
...
or exit fast and get throttled by s6 (and svstat reports ``down â€¦,
want up``). Both states are valid "user asked for gateway up" results
â€” what we assert is the *want* intent the lifecycle command set, NOT
...
def _svstat_wants_up(container: str) -> bool:
    ...
    s6-svstat formats the output to elide redundancies â€” when the
    ...
    # Currently down â€” ``want up`` only shows up when explicitly set.
...
def test_profile_create_then_gateway_start(
    ...
    r = _sh(container_name, f"zed profile create {PROFILE}")
    ...
    r = _sh(container_name, f"zed -p {PROFILE} gateway start", timeout=60)
    assert r.returncode == 0, (
        f"gateway start failed: stderr={r.stderr!r} stdout={r.stdout!r}"
    )

    # After start, s6's intent is "up" â€” even if the supervised gateway
    # process spin-fails (no model/auth in the test profile), the
    # supervision-state contract holds. See ``_svstat_wants_up`` for
    # why we accept both ``up â€¦`` (currently up) and ``down â€¦, want
    ...
    r = _sh(container_name, f"zed -p {PROFILE} gateway stop", timeout=30)
    assert r.returncode == 0
...
def test_profile_delete_stops_gateway(
    ...
    _sh(container_name, f"zed profile create {PROFILE}")
    _sh(container_name, f"zed -p {PROFILE} gateway start", timeout=60)
    time.sleep(3)

    r = _sh(
        container_name,
        f"zed profile delete {PROFILE} --yes",
        timeout=30,
    )
