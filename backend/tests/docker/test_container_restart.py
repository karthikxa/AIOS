...
$ZED_HOME/profiles/<name>/gateway_state.json on every boot and
...
def _wait_for_reconcile_log_mention(
    ...
    *,
    ...
    signal that the reconciler has finished â€” much more reliable
...
def restart_container(request, built_image: str):
    ...
    preserves $ZED_HOME/profiles/."""
    safe = request.node.name.replace("[", "_").replace("]", "_")
    name = f"zed-restart-{safe}"
    volume = f"zed-restart-vol-{safe}"
    _docker("rm", "-f", name)
    ...
    # 'default' line on every boot (PR #30136 item I1) â€” that's our
    # readiness signal.
    deadline = time.monotonic() + 30.0
    while time.monotonic() < deadline:
        r = _docker(
            "exec", "-u", "zed", name, "sh", "-c",
...
def test_running_gateway_survives_container_restart(restart_container: str) -> None:
    ...
    r = _exec(container, "zed", "profile", "create", "coder")
    assert r.returncode == 0, f"profile create failed: {r.stderr}"

    r = _exec(container, "zed", "-p", "coder", "gateway", "start", timeout=60)
    ...
    # via gateway/status.py â€” but we don't want to wait for or assert
    ...
    # No `down` marker â€” we asked for auto-start.
...
def test_stopped_gateway_stays_stopped_after_restart(restart_container: str) -> None:
    ...
    _exec(container, "zed", "profile", "create", "writer").check_returncode()
...
def test_stale_gateway_pid_cleaned_up_on_restart(restart_container: str) -> None:
    ...
    survive the restart â€” a numerically-equal live PID in the new
    container is a different process and would confuse the gateway
    process-mismatch checks."""
    container = restart_container

    _exec(container, "zed", "profile", "create", "ghost").check_returncode()
...
def test_live_gateway_autostarts_after_real_restart_without_manual_state_stamp(
    ...
    `docker restart` sends â€” no manual state stamp â€” and must come back up
    on the next boot.
    ...
    and registered the slot DOWN â€” the gateway silently stayed dark after
    ...
    _exec(container, "zed", "profile", "create", "live").check_returncode()
    r = _exec(container, "zed", "-p", "live", "gateway", "start", timeout=60)
    assert r.returncode == 0, f"gateway start failed: {r.stderr}"
    ...
    # Real restart â€” Docker sends SIGTERM to PID 1; s6 propagates it to the
    # supervised gateway. No planned-stop marker is written (this is not an
    # operator `zed gateway stop`), so the shutdown is signal-initiated.
    _docker("restart", container, timeout=60).check_returncode()
    ...
    assert r.returncode != 0, (
        "down marker present despite a live gateway being restarted â€” "
