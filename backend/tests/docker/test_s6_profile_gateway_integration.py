...
The unit tests in tests/zed_cli/test_service_manager.py exercise the
...
Phase 3 only registers the service slot â€” it doesn't depend on the
gateway actually starting (the binary will refuse to start without a
valid profile config). The full register â†’ start â†’ supervised-restart
â†’ unregister cycle is covered by Phase 4 once profile create/delete
hooks land.

Every ``docker exec`` here runs as the unprivileged ``zed`` user
(via :func:`docker_exec` in conftest); see the conftest module
docstring. ``/run/service`` is chowned zed-writable by the
...
_REGISTER_SCRIPT = """
import sys
sys.path.insert(0, "/opt/zed")
from zed_cli.service_manager import S6ServiceManager
S6ServiceManager().register_profile_gateway("phase3test")
# Don't worry about whether the gateway actually starts â€” we only care
...
def test_s6_register_creates_service_dir_in_live_container(
    ...
    # s6-svscan picked it up â€” s6-svstat works against the dir.
