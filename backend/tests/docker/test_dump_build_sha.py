"""Regression test: ``zed dump`` reports a real git SHA inside the container.

Background: ``.dockerignore`` excludes ``.git``, so ``git rev-parse HEAD``
fails inside the published image and ``zed dump`` used to report
``version: ... [(unknown)]``.  The Dockerfile now writes the build-time
``$ZED_GIT_SHA`` build-arg to ``/opt/zed/.zed_build_sha`` and
``zed_cli/build_info.py`` reads it as a fallback.

CI (``.github/workflows/docker-publish.yml``) always sets the build-arg
to ``${{ github.sha }}``.  Local ``docker build`` (the ``built_image``
fixture in ``tests/docker/conftest.py``) does NOT â€” so locally the file
is absent and ``zed dump`` correctly falls back to ``(unknown)``.

This test handles both cases:

* If ``/opt/zed/.zed_build_sha`` exists in the image, assert that
  ``zed dump`` surfaces its content as the version SHA (not
  ``(unknown)``).
* If the file is absent, assert the legacy behaviour (``(unknown)``)
  still holds â€” defensive guard against the helper accidentally
...
def _run_dump(image: str) -> str:
    ...
    Dockerfile) so the container's zed user (UID 10000) can bootstrap
    ...
    assert r.returncode == 0, (
        f"zed dump exited {r.returncode}: "
...
def _read_baked_sha_from_image(image: str) -> str | None:
    """Return the ``/opt/zed/.zed_build_sha`` content, or None if absent."""
    r = subprocess.run(
        [
            "docker", "run", "--rm", "--entrypoint", "cat", image,
            "/opt/zed/.zed_build_sha",
        ],
...
def test_dump_reports_baked_sha_when_present(built_image: str) -> None:
    """When the image was built with ``ZED_GIT_SHA``, dump must surface it.
    ...
    if baked is None:
        ...
        # fallback ``(unknown)`` is intact â€” guards against the helper
