...
Override the image with ``ZED_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ZED_TEST_IMAGE", "zed-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ZED_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ZED_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec â€” default to the unprivileged zed user
# ---------------------------------------------------------------------------
#
# Background: every Zed runtime path inside the container drops to UID
# 10000 (the ``zed`` user) via ``s6-setuidgid zed``. ``docker exec``
...
# this way â€” ``Path('/proc/1/exe').resolve()`` works as root and silently
# fails (PermissionError swallowed) for zed, so a test that ran as root
...
# helpers below run every probe under ``-u zed`` unless a specific
# test explicitly opts into ``user="root"`` (rare â€” e.g. inspecting
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: zed).
