"""Harness: dashboard opt-in via ZED_DASHBOARD.

Today (tini): dashboard starts once when ZED_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``zed`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without ZED_DASHBOARD, no dashboard process should be running."""
    subprocess.run(
    ...
    time.sleep(5)
    r = docker_exec(container_name, "pgrep", "-f", "zed dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without ZED_DASHBOARD"
    )
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    """Without ZED_DASHBOARD, s6-svstat should report the dashboard
    slot as DOWN (not up-with-sleep-infinity, which would
    false-positive `zed doctor` and any other health check).
    ...
    ZED_DASHBOARD is unset, so the slot reflects reality.
    """
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name, built_image,
         "sleep", "60"],
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without ZED_DASHBOARD; "
...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    """Symmetry: with ZED_DASHBOARD=1, s6-svstat reports the slot as up."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "ZED_DASHBOARD=1",
         # The default dashboard host is 0.0.0.0, which now engages the
         # OAuth auth gate. Without a provider registered (no
         # ZED_DASHBOARD_OAUTH_CLIENT_ID in this test env), start_server
...
def test_dashboard_opt_in_starts(
    ...
    """With ZED_DASHBOARD=1, a dashboard process should be visible."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "ZED_DASHBOARD=1",
    ...
    # Poll for the dashboard subprocess to appear â€” the entrypoint
    # backgrounds it and bootstrap (skills sync etc.) can take a few
    # seconds before the python process actually launches.
    ok, _ = _poll(
        container_name, "pgrep -f 'zed dashboard'", deadline_s=30.0,
    )
    assert ok, "Dashboard should be running with ZED_DASHBOARD=1"


def test_dashboard_port_override(
    built_image: str, container_name: str,
) -> None:
    """ZED_DASHBOARD_PORT changes the dashboard's listen port."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "ZED_DASHBOARD=1", "-e", "ZED_DASHBOARD_PORT=9120",
         # Default bind is 0.0.0.0; pin insecure opt-in so the auth gate
    ...
    # to the port yet â€” uvicorn takes another second or two to come up.
...
def test_dashboard_restarts_after_crash(
    ...
    Pre-s6 (tini) behavior was "stays dead" â€” the test wouldn't have
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "zed dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as zed, so the
    # zed user can kill it (same UID).
...
# ---------------------------------------------------------------------------
# OAuth auth-gate behaviour â€” regression guard for the dashboard-insecure
# auto-injection bug. Pre-fix, the s6 run script appended `--insecure`
# whenever `ZED_DASHBOARD_HOST` was non-loopback, silently disabling
...
def _http_probe(
    ...
    *,
    ...
    HTTP response â€” 200, 401, 503, anything. The image doesn't ship
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``ZED_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    1. ``/api/auth/providers`` (publicly reachable through the gate so
       ...
       provider list â€” proves the bundled provider registered.
    2. ``/api/sessions`` (a gated route under both the legacy
       ``_SESSION_TOKEN`` middleware and the OAuth gate) returns 401
       to an unauthenticated caller â€” proves the OAuth gate is actively
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "ZED_DASHBOARD=1",
         "-e", "ZED_DASHBOARD_HOST=0.0.0.0",
         "-e", "ZED_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
         built_image, "sleep", "120"],
        check=True, capture_output=True, timeout=30,
    )
    ...
    assert "nous" in provider_names, (
        ...
        f"ZED_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )

    # (2) A gated route (``/api/sessions``) returns 401 to an
    #     unauthenticated caller â€” the OAuth gate is intercepting.
    status_code, body = _http_probe(container_name, "/api/sessions")
    assert status_code == 401, (
        "OAuth gate must intercept gated /api/* routes on 0.0.0.0 bind "
        "when a provider is registered and ZED_DASHBOARD_INSECURE "
        f"is unset. Got: status={status_code} body={body!r}"
    )

    # (3) ``/api/status`` remains 200 under the gate â€” it's in the shared
    ...
    assert status_code == 200, (
        ...
        "â€” the portal uses it as the wildcard-subdomain liveness probe. "
...
def test_dashboard_insecure_env_var_opts_out_of_gate(
    ...
    """``ZED_DASHBOARD_INSECURE=1`` re-enables the legacy no-gate mode
    ...
    assert status.get("auth_required") is False, (
        "ZED_DASHBOARD_INSECURE=1 must disable the auth gate (explicit "
