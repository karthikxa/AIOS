...
Before the s6 migration, ``docker run nousresearch/zed-agent gateway
run`` was the standard pattern â€” the gateway ran as the container's
...
crash and a supervised dashboard alongside (when ``ZED_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``ZED_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse â€” the
     ``ZED_S6_SUPERVISED_CHILD`` sentinel breaks the loop.

Every ``docker exec`` runs as ``zed`` per the conftest module
...
def test_gateway_run_redirects_to_supervised(
    ...
    The CMD process itself shouldn't be the gateway â€” it should be
    ...
    # s6 image â€” the foreground gateway would crash without config,
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    all â€” the gateway IS the CMD process.

    Three positive assertions confirm we took the pre-s6 path:

      * The CMD process is a python ``zed gateway run`` invocation
        (not ``sleep infinity``).
    ...
    # semantics â€” what's NOT correct is the supervised behavior
    ...
    if status == "running":
        # Gateway running in foreground â€” the CMD process should be
        ...
        # but should NOT have its want-state set to "up" â€” the
...
def test_gateway_run_no_supervise_env_var(
    ...
    if inspect.stdout.strip() == "running":
        assert not _svstat_wants_up(container_name, "gateway-default"), (
            "ZED_GATEWAY_NO_SUPERVISE=1: gateway-default has "
...
def test_supervised_gateway_does_not_recurse(
    ...
    """The ZED_S6_SUPERVISED_CHILD sentinel must prevent the
    supervised ``zed gateway run`` from re-entering the redirect.

    If recursion happened, every supervised gateway start would itself
    re-dispatch to s6 and exec ``sleep infinity`` â€” so the supervised
    gateway slot would never actually run a python ``zed gateway
    ...
    via the redirect â†’ start â†’ run â†’ redirect â†’ ... loop.
    """
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name, built_image,
         "gateway", "run"],
    ...
    # Count python processes running `zed gateway run`. If the
    ...
    r = _sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*zed.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `zed gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-zed`
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``ZED_DASHBOARD=1`` is set, ``docker run <image> gateway
    run`` should result in BOTH the gateway and the dashboard being
    supervised by s6 â€” the dashboard slot was always there but only
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    """The supervised gateway's stdout â€” including the rich-console
    startup banner â€” must reach ``docker logs``, not just the rotated
    log file under ``${ZED_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Zed Gateway Startingâ€¦"
    banner with the âš• symbol is the most visible "yes, your gateway
    ...
    We assert by looking for the literal banner glyph (``âš•``) â€” a
    ...
    # Banner is printed during gateway startup â€” give it time to
    ...
    # The banner âš• symbol is the load-bearing assertion â€” it's unique
    # to gateway startup stdout output and won't appear in stderr
    # (Python logging) or s6 boot messages.
    assert "âš•" in combined or "Zed Gateway Starting" in combined, (
    ...
    assert "âš•" in file_contents or "Zed Gateway Starting" in file_contents, (
        "Banner also missing from rotated log file â€” the file "
