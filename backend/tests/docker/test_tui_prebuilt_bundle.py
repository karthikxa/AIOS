...
package-lock.json. Without ZED_TUI_DIR set, ``_make_tui_argv`` falls
...
concurrent /api/pty connections â†’ ENOTEMPTY.

The fix is ``ENV ZED_TUI_DIR=/opt/zed/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the zed user, return stdout."""
    inner = (
        "source /opt/zed/.venv/bin/activate && "
        "cd /opt/zed && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the zed user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as â€” not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "zed", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_zed_tui_dir_env_is_set(built_image: str) -> None:
    """ZED_TUI_DIR must point at the prebuilt bundle dir in the image."""
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$ZED_TUI_DIR"'],
        capture_output=True, text=True, timeout=60,
    )
    assert r.returncode == 0, r.stderr[-2000:]
    assert r.stdout.strip() == "/opt/zed/ui-tui", (
        f"ZED_TUI_DIR={r.stdout.strip()!r} (expected /opt/zed/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    npm install â€” i.e. it takes the same path as a nix/packaged release."""
    py = (
        "import json\n"
        "from pathlib import Path\n"
        "from zed_cli.main import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/zed/ui-tui')\n"
    ...
    # With ZED_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE
    # ever reaching the install check â€” so the resolved argv is what matters.
