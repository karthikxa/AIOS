...
def test_matrix_extra_not_in_all():
    ...
    `tools/lazy_deps.py` (LAZY_DEPS["platform.matrix"]) â€” installs at
    first use, where the user is expected to have a toolchain.
    """
    optional_dependencies = _load_optional_dependencies()

    assert "matrix" in optional_dependencies, "[matrix] extra must still exist for explicit `pip install zed-agent[matrix]`"
    # Must NOT appear in [all] in any form â€” neither unconditional nor
    ...
    assert not matrix_in_all, (
        "matrix must not appear in [all] â€” it's lazy-installed via "
...
def test_lazy_installable_extras_excluded_from_all():
    ...
    backend in BOTH [all] and LAZY_DEPS defeats that â€” fresh installs
    ...
    # imports tools/lazy_deps.py so the test stays a contract â€” if
    ...
    lazy_covered_extras = {
        ...
        "honcho", "hindsight",
        "mistral",  # mistralai â€” Voxtral STT/TTS, lazy-installed (stt.mistral / tts.mistral)
    }
    all_extra_specs = optional_dependencies["all"]
    for extra in lazy_covered_extras:
        offending = [
            spec for spec in all_extra_specs
            if f"zed-agent[{extra}]" in spec
        ]
        assert not offending, (
            f"[{extra}] is in [all] but also in LAZY_DEPS. "
            f"Remove it from [all] in pyproject.toml â€” it lazy-installs "
...
def test_pyproject_aiohttp_pins_match_lazy_slack_pin():
    ...
    If the two drift, `zed update` resolves the pyproject pin and
    downgrades aiohttp, reopening the CVEs the lazy pin fixed (#31817) â€”
    ...
    assert not mismatches, (
        ...
        "LAZY_DEPS['platform.slack'] to avoid zed update downgrading "
...
def test_pyproject_pins_match_lazy_deps_pins():
    ...
    places. When they drift, `zed update` resolves the pyproject extra
    ...
    CVE-2026-34450/34452 case found alongside it) â€” only for the lazy
    ...
    assert not drift, (
        ...
        "for every shared package â€” otherwise `zed update` downgrades the "
...
def test_dingtalk_extra_includes_qrcode_for_qr_auth():
    """DingTalk's QR-code device-flow auth (zed_cli/dingtalk_auth.py)
