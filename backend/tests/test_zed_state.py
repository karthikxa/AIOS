"""Tests for zed_state.py â€” SessionDB SQLite CRUD, FTS5 search, export."""

import sqlite3
import time
import pytest

from zed_state import SCHEMA_SQL, SCHEMA_VERSION, SessionDB
...
class TestSessionLifecycle:
    ...
    def test_end_session_preserves_original_end_reason(self, db):
        """The first end_reason wins â€” compression splits must not be
    ...
    def test_db_initializes_without_fts5_module(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("zed_state.sqlite3.connect", connect_without_fts)

        db = SessionDB(db_path=tmp_path / "state.db")
        try:
            assert db._fts_enabled is False
            # Neither FTS5 virtual table should have been created on a build
            # that lacks the fts5 module â€” both init paths must degrade.
    ...
    def test_fts_runtime_restores_triggers_after_no_fts_open(
        ...
        monkeypatch.setattr("zed_state.sqlite3.connect", real_connect)
    ...
    def test_base_fts_rebuilds_after_trigger_repair_without_trigram(
        ...
        monkeypatch.setattr("zed_state.sqlite3.connect", connect_without_trigram)
        restored = SessionDB(db_path=db_path)
        try:
            assert restored._fts_enabled is True
    ...
    def test_is_fts5_unavailable_error_catches_trigram_tokenizer(self):
        ...
        # Generic tokenizer errors should NOT match â€” only trigram.
    ...
    def test_v11_migration_backfills_base_fts_when_trigram_unavailable(
        ...
        # Phase 2: reopen with trigram disabled â€” migration should still
    ...
    def test_cjk_search_falls_back_to_like_when_trigram_unavailable(
        ...
        try:
            db.create_session(session_id="s1", source="cli")
            db.append_message("s1", role="user", content="å¤§åˆ«å±±é¡¹ç›®è®¡åˆ’ä¹¦")
            db.append_message("s1", role="user", content="é•¿æ±Ÿå¤§æ¡¥è®¾è®¡æ–¹æ¡ˆ")

            # 3+ CJK chars would normally use trigram, but it's unavailable.
            # Must fall back to LIKE and still return results.
            results = db.search_messages("å¤§åˆ«å±±")
            assert len(results) == 1
            # Note: search_messages strips 'content' from results; use 'snippet'.
            assert "å¤§åˆ«å±±" in results[0]["snippet"]
        finally:
            db.close()
...
class TestMessageStorage:
    ...
    def test_string_content_unchanged_by_encoding(self, db):
        """Plain strings must not be wrapped â€” FTS search and legacy
    ...
    def test_platform_message_id_round_trips(self, db):
        """Platform-side message ids (yuanbao msg_id, telegram update_id, â€¦)
        survive append â†’ get_messages_as_conversation under the
        ...
        # Assistant row had no platform id â€” must not gain one spuriously.
...
class TestFTS5Search:
    ...
    def test_search_special_chars_do_not_crash(self, db):
        ...
        for query in dangerous_queries:
            # Must not raise â€” should return list (possibly empty)
    ...
    def test_sanitize_fts5_quotes_hyphenated_terms(self):
        ...
        # Already-quoted hyphenated term â€” no double quoting
        assert s('"chat-send"') == '"chat-send"'
    ...
    def test_sanitize_fts5_quotes_dotted_terms(self):
        ...
        # Already quoted â€” no double quoting
        assert s('"P2.2"') == '"P2.2"'
        ...
        # Mixed dots and hyphens â€” single pass avoids double-quoting
    ...
    def test_sanitize_fts5_quotes_underscored_terms(self):
        ...
        # Mixed underscores and hyphens/dots â€” single pass avoids double-quoting
        ...
        result = s('sp_new and è¡€ç®¡ç˜¤')
        assert '"sp_new"' in result
        assert 'è¡€ç®¡ç˜¤' in result
...
class TestCJKSearchFallback:
    ...
    token ("å’Œå…¶ä»–agentçš„èŠå¤©è®°å½•" â†’ one token), so substring queries like
    "è®°å¿†æ–­è£‚" return 0 rows despite the data being present. SessionDB falls
    ...
    def test_cjk_detection_covers_all_ranges(self):
        ...
        assert f("è®°å¿†æ–­è£‚") is True
        # Japanese Hiragana + Katakana
        assert f("ã“ã‚“ã«ã¡ã¯") is True
        assert f("ã‚«ã‚¿ã‚«ãƒŠ") is True
        # Korean Hangul syllables (both early and late â€” guards against
        # the \ud7a0-\ud7af typo seen in one of the duplicate PRs)
        assert f("ì•ˆë…•í•˜ì„¸ìš”") is True
        assert f("ê¸°ì–µ") is True
        # Non-CJK
        assert f("hello world") is False
        assert f("æ—¥æœ¬èªžmixedwithenglish") is True
        assert f("") is False
    ...
    def test_chinese_multichar_query_returns_results(self, db):
        ...
        db.append_message(
            "s1", role="user",
            content="æ˜¨å¤©å’Œå…¶ä»–Agentçš„èŠå¤©è®°å½•ï¼Œè®°å¿†æ–­è£‚é—®é¢˜å¤çŽ°äº†",
        )
        results = db.search_messages("è®°å¿†æ–­è£‚")
        assert len(results) == 1
        assert results[0]["session_id"] == "s1"

    def test_chinese_bigram_query(self, db):
        db.create_session(session_id="s1", source="telegram")
        db.append_message("s1", role="user", content="ä»Šå¤©è®¨è®ºA2Aé€šä¿¡åè®®çš„å®žçŽ°")
        results = db.search_messages("é€šä¿¡")
        assert len(results) == 1

    def test_korean_query_returns_results(self, db):
        """Guards against Hangul range typos (\\uac00-\\ud7af, not \\ud7a0-)."""
        db.create_session(session_id="s1", source="cli")
        db.append_message("s1", role="user", content="ì•ˆë…•í•˜ì„¸ìš” ë°˜ê°‘ìŠµë‹ˆë‹¤")
        results = db.search_messages("ì•ˆë…•")
        assert len(results) == 1

    def test_japanese_query_returns_results(self, db):
        db.create_session(session_id="s1", source="cli")
        db.append_message("s1", role="user", content="ã“ã‚“ã«ã¡ã¯ä¸–ç•Œ")
        assert len(db.search_messages("ã“ã‚“ã«ã¡ã¯")) == 1
        assert len(db.search_messages("ä¸–ç•Œ")) == 1
    ...
    def test_cjk_fallback_preserves_source_filter(self, db):
        ...
        db.create_session(session_id="s1", source="cli")
        db.create_session(session_id="s2", source="telegram")
        db.append_message("s1", role="user", content="è®°å¿†æ–­è£‚åœ¨CLI")
        db.append_message("s2", role="user", content="è®°å¿†æ–­è£‚åœ¨Telegram")

        results = db.search_messages("è®°å¿†æ–­è£‚", source_filter=["telegram"])
        assert len(results) == 1
    ...
    def test_cjk_fallback_preserves_exclude_sources(self, db):
        ...
        db.append_message("s1", role="user", content="è®°å¿†æ–­è£‚åœ¨CLI")
        db.append_message("s2", role="assistant", content="è®°å¿†æ–­è£‚åœ¨tool")

        results = db.search_messages("è®°å¿†æ–­è£‚", exclude_sources=["tool"])
        sources = {r["source"] for r in results}
    ...
    def test_cjk_fallback_preserves_role_filter(self, db):
        db.create_session(session_id="s1", source="cli")
        db.append_message("s1", role="user", content="ç”¨æˆ·è¯´çš„è®°å¿†æ–­è£‚")
        db.append_message("s1", role="assistant", content="åŠ©æ‰‹è¯´çš„è®°å¿†æ–­è£‚")

        results = db.search_messages("è®°å¿†æ–­è£‚", role_filter=["assistant"])
        assert len(results) == 1
    ...
    def test_cjk_snippet_is_centered_on_match(self, db):
        ...
        db.create_session(session_id="s1", source="cli")
        long_prefix = "è¿™æ˜¯ä¸€æ®µå¾ˆé•¿çš„å‰ç¼€ç”¨æ¥æŠŠåŒ¹é…ä½ç½®æŽ¨åˆ°æ–‡æ¡£ä¸­é—´" * 3
        long_suffix = "è¿™æ˜¯ä¸€æ®µå¾ˆé•¿çš„åŽç¼€å†…å®¹å¡«å……å‰©ä½™ç©ºé—´" * 3
        db.append_message(
            "s1", role="user",
            content=f"{long_prefix}è®°å¿†æ–­è£‚{long_suffix}",
        )
        results = db.search_messages("è®°å¿†æ–­è£‚")
        assert len(results) == 1
        # The centered substr() snippet must include the matched term.
        assert "è®°å¿†æ–­è£‚" in results[0]["snippet"]
    ...
    def test_english_query_still_uses_fts5_fast_path(self, db):
        ...
        results = db.search_messages("docker")
        assert len(results) == 1
        # No CJK in query â†’ LIKE fallback must not run. We don't assert this
    ...
    def test_mixed_cjk_english_query(self, db):
        ...
        db.create_session(session_id="s1", source="cli")
        db.append_message("s1", role="user", content="è®¨è®ºAgenté€šä¿¡åè®®")
        # "Agenté€šä¿¡" is CJK+English â€” FTS5 default tokenizer indexes the
        # whole CJK run with embedded "agent" as separate tokens; the LIKE
        # fallback handles the substring correctly.
        results = db.search_messages("Agenté€šä¿¡")
        assert len(results) == 1
    ...
    def test_cjk_partial_fts5_results_supplemented_by_like(self, db):
        ...
        db.create_session(session_id="s1", source="cli")
        db.create_session(session_id="s2", source="telegram")
        db.append_message("s1", role="user", content="æ˜¨æ™šè®¨è®ºäº†è®°å¿†ç³»ç»Ÿ")
        db.append_message("s2", role="user", content="æ˜¨æ™šçš„ä¼šè®®çºªè¦å·²å‘é€")
        results = db.search_messages("æ˜¨æ™š")
        assert len(results) == 2
    ...
    def test_cjk_like_dedup_no_duplicates(self, db):
        ...
        db.create_session(session_id="s1", source="cli")
        db.append_message("s1", role="user", content="æµ‹è¯•åŽ»é‡é€»è¾‘")
        results = db.search_messages("æµ‹è¯•")
        assert len(results) == 1
    ...
    def test_cjk_like_escapes_wildcards(self, db):
        ...
        db.create_session(session_id="s1", source="cli")
        db.create_session(session_id="s2", source="cli")
        db.append_message("s1", role="user", content="è¾¾æˆ100%å®ŒæˆçŽ‡")
        db.append_message("s2", role="user", content="è¾¾æˆ100å®ŒæˆçŽ‡æ˜¯ç›®æ ‡")
        # The % in the query must be literal â€” should only match s1
        results = db.search_messages("100%å®Œæˆ")
        assert len(results) == 1
        assert results[0]["session_id"] == "s1"
    ...
    def test_cjk_trigram_preserves_boolean_operators(self, db):
        ...
        db.create_session(session_id="s1", source="cli")
        db.create_session(session_id="s2", source="cli")
        db.append_message("s1", role="user", content="è®°å¿†ç³»ç»Ÿå¾ˆå¥½ç”¨")
        db.append_message("s2", role="user", content="æ–­è£‚è¿žæŽ¥éœ€è¦ä¿®å¤")
        results = db.search_messages("è®°å¿†ç³»ç»Ÿ OR æ–­è£‚è¿žæŽ¥")
        assert len(results) == 2
        session_ids = {r["session_id"] for r in results}
        assert session_ids == {"s1", "s2"}
    ...
    def test_cjk_or_combined_short_tokens_returns_results(self, db):
        ...
        OR-combined 2-char CJK tokens (e.g. "å¹¿è¥¿ OR æ¡‚æž— OR æ¼“æ±Ÿ OR æ—…æ¸¸")
        ...
        db.append_message("s1", role="user", content="å¹¿è¥¿æ˜¯ä¸ªå¥½åœ°æ–¹ï¼ŒåŽ»è¿‡æ¡‚æž—")
        db.append_message("s2", role="user", content="æ¼“æ±Ÿé£Žæ™¯å¾ˆç¾Žï¼Œå€¼å¾—æ—…æ¸¸")
        db.append_message("s3", role="user", content="unrelated English content")

        results = db.search_messages("å¹¿è¥¿ OR æ¡‚æž— OR æ¼“æ±Ÿ OR æ—…æ¸¸")
        session_ids = {r["session_id"] for r in results}
        assert "s1" in session_ids, "å¹¿è¥¿/æ¡‚æž— terms not matched"
        assert "s2" in session_ids, "æ¼“æ±Ÿ/æ—…æ¸¸ terms not matched"
    ...
    def test_cjk_short_token_or_query_preserves_filters(self, db):
        ...
        db.create_session(session_id="s1", source="cli")
        db.create_session(session_id="s2", source="telegram")
        db.append_message("s1", role="user", content="å¹¿è¥¿æ—…æ¸¸æ”»ç•¥cli")
        db.append_message("s2", role="user", content="å¹¿è¥¿æ—…æ¸¸æ”»ç•¥telegram")

        results = db.search_messages("å¹¿è¥¿ OR æ—…æ¸¸", source_filter=["telegram"])
        assert len(results) == 1
        assert results[0]["source"] == "telegram"
...
class TestPruneSessions:
    ...
    def test_prune_entire_old_chain(self, db):
        """All sessions in a chain are old â€” entire chain is pruned."""
        old_ts = time.time() - 200 * 86400
...
class TestBulkDeleteSessions:
    """``delete_sessions(ids)`` â€” the bulk-delete primitive backing the
    ...
    1. Returns the real deleted count (existing intersection), not
       just ``len(session_ids)`` â€” selection state in the UI can race
       against another tab's delete.
    2. Unknown IDs are silently skipped, never raise.
    3. ``message_count > 0`` sessions are deleted too â€” unlike
    ...
    def test_returns_real_count_skipping_unknown_ids(self, db):
        """Unknown IDs are silently skipped â€” the return value reflects
    ...
    def test_deletes_archived_and_active_when_selected(self, db):
        ...
        explicit bulk-select trusts the user â€” archived sessions and
    ...
    def test_cleans_up_transcript_files(self, db, tmp_path):
        ...
        swept as part of the bulk operation â€” mirrors the per-row
...
class TestDeleteEmptySessions:
    ...
    button â€” see ``SessionsPage.tsx`` + ``DELETE /api/sessions/empty``
    in ``zed_cli/web_server.py``.

    Invariants this class locks in:

    1. Only ``message_count = 0`` rows are touched.
    2. Active (un-ended) sessions are skipped even if they're empty â€”
       the agent might be mid-handshake, and yanking the row would
       race the live runtime.
    3. Archived sessions are skipped â€” the user already filed them away.
    4. Children of a deleted parent are orphaned (parent_session_id â†’
    ...
    def test_count_and_delete_empties_only(self, db):
        # Two empty + ended sessions â†’ both should be in the kill list.
        ...
        # One non-empty + ended session â†’ must survive.
    ...
    def test_skips_active_empty_sessions(self, db):
        ...
        sweep must not delete it â€” that would yank a session out from
        under the agent before its first reply persists."""
        db.create_session(session_id="live", source="cli")
        # Deliberately no end_session() â€” session is "active".
    ...
    def test_returns_zero_when_nothing_to_delete(self, db):
        """No-op path: no candidate rows â†’ return 0, no error."""
        db.create_session(session_id="hasmsg", source="cli")
        db.append_message("hasmsg", role="user", content="Hello")
        db.end_session("hasmsg", end_reason="done")
    ...
    def test_orphans_children_of_deleted_empty_parent(self, db):
        ...
        must orphan that child, not cascade-delete it â€” same contract
    ...
    def test_cleans_up_on_disk_transcript_files(self, db, tmp_path):
        ...
        transcripts, but the request-dump path is real â€” the gateway
...
class TestSessionTitle:
    ...
    def test_title_with_special_characters(self, db):
        db.create_session(session_id="s1", source="cli")
        title = "PR #438 â€” fixing the 'auth' middleware"
...
class TestSessionTitleLineage:
    ...
    def test_rename_continuation_back_to_base_transfers_title(self, db):
        ...
        # User renames the visible tip back to the base name â€” must succeed.
        assert db.set_session_title("tip", "fingerprint-scanner") is True
        assert db.get_session("tip")["title"] == "fingerprint-scanner"
        # Title transferred off the hidden ancestor â€” no duplicate titles.
    ...
    def test_non_compression_child_still_conflicts(self, db):
        ...
        # compression reason â€” not a continuation edge.
...
class TestSanitizeTitle:
    ...
    def test_rtl_override_stripped(self):
        # Right-to-left override (U+202E) â€” used in filename spoofing attacks
        assert SessionDB.sanitize_title("hello\u202eworld") == "helloworld"
    ...
    def test_unicode_emoji_allowed(self):
        assert SessionDB.sanitize_title("ðŸš€ My Project ðŸŽ‰") == "ðŸš€ My Project ðŸŽ‰"

    def test_cjk_characters_allowed(self):
        assert SessionDB.sanitize_title("æˆ‘çš„é¡¹ç›®") == "æˆ‘çš„é¡¹ç›®"

    def test_accented_characters_allowed(self):
        assert SessionDB.sanitize_title("RÃ©sumÃ© Ã©diting") == "RÃ©sumÃ© Ã©diting"
...
class TestSchemaInit:
    ...
    def test_schema_version(self, db):
        from zed_state import SCHEMA_VERSION
    ...
    def test_topic_mode_schema_is_not_auto_migrated_on_open(self, tmp_path):
        ...
        The gateway must remain rollback-safe: simply upgrading Zed and starting
    ...
    def test_migration_from_v2(self, tmp_path):
        ...
        # Open with SessionDB â€” should migrate to v9
        migrated_db = SessionDB(db_path=db_path)
    ...
    def test_v9_migration_skips_v10_trigram_backfill_before_v11_rebuild(self, tmp_path, monkeypatch):
        """Direct v9â†’current migration should do only the v11 FTS rebuild.
        ...
        monkeypatch.setattr("zed_state.sqlite3.connect", connect_with_trace)
        migrated_db = SessionDB(db_path=db_path)
        try:
    ...
    def test_reconciliation_adds_missing_columns(self, tmp_path):
        ...
        created â€” causing 'no such column' on /continue.
        """
        import sqlite3
        ...
        # Open with SessionDB â€” reconciliation should add the missing column
        migrated_db = SessionDB(db_path=db_path)
    ...
    def test_schema_sql_is_source_of_truth(self, db):
        ...
        """
        from zed_state import SCHEMA_SQL
...
class TestTitleUniqueness:
    ...
    def test_same_session_can_keep_title(self, db):
        ...
        db.create_session("s1", "cli")
        db.set_session_title("s1", "my project")
        # Should not raise â€” it's the same session
    ...
    def test_null_titles_not_unique(self, db):
        ...
        db.create_session("s1", "cli")
        db.create_session("s2", "cli")
        # Both have NULL titles â€” no error
        assert db.get_session("s1")["title"] is None
        assert db.get_session("s2")["title"] is None
...
class TestListSessionsRich:
    ...
    def test_order_by_last_active_uses_compression_tip_activity(self, db):
        ...
        # Bunch of newer, uncompressed sessions â€” fresher start_at but older
    ...
    def test_branch_session_visible_after_parent_reopen_and_reend(self, db):
        ...
        re-ended with a different end_reason â€” tui_shutdown overwriting
        'branched' â€” which broke the legacy end_reason heuristic. The stable
        ...
        # Branch must STILL be visible â€” the marker survives the parent's
...
class TestCompressionChainProjection:
    """Tests for lineage-aware list_sessions_rich â€” compressed conversations
    ...
    def test_get_compression_tip_skips_delegate_children(self, db):
        ...
        continuations â€” the started_at >= ended_at guard handles this.
        """
        import time as _time
        self._build_compression_chain(db, _time.time() - 3600)
    ...
    def test_list_without_projection_returns_raw_root(self, db):
        ...
        rows â€” useful for admin/debug UIs.
        """
        import time as _time
        self._build_compression_chain(db, _time.time() - 3600)
        sessions = db.list_sessions_rich(
    ...
    def test_list_handles_broken_chain_gracefully(self, db):
        ...
        crash the list â€” it should fall back to surfacing the root as-is.
        """
        import time as _time
        t0 = _time.time() - 100
        db.create_session("orphan", "cli")
        ...
        # No tip means no projection â€” row stays raw.
...
class TestExcludeSources:
    ...
    def test_search_messages_source_include_and_exclude(self, db):
        ...
        # Include cli+tool, but exclude tool â†’ should only return cli
...
class TestConcurrentWriteSafety:
    def test_create_session_insert_or_ignore_is_idempotent(self, db):
        ...
        # Second call should be silent â€” no IntegrityError
    ...
    def test_ensure_session_is_idempotent(self, db):
        ...
        # First write wins â€” ensure_session must not overwrite
    ...
    def test_ensure_session_allows_append_message_after_failed_create(self, db):
        ...
        """
        # Simulate failed create_session â€” row absent
    ...
    def test_sqlite_timeout_is_at_least_30s(self, db):
        ...
        import inspect
        from zed_state import SessionDB as _SessionDB
...
class TestOptimizeFts:
    ...
    def test_optimize_preserves_search_and_snippet(self, db):
        ...
        # IDs and snippets are identical before/after â€” pure layout change.
...
class TestAutoMaintenance:
    ...
    def test_auto_prune_without_sessions_dir_preserves_files(self, db, tmp_path):
        ...
        result = db.maybe_auto_prune_and_vacuum(retention_days=90)
        assert result["pruned"] == 1
        # File stays â€” caller didn't opt in
...
class TestFTS5ToolCallMigration:
    ...
    def test_v10_to_v11_upgrade_backfills_tool_fields(self, tmp_path):
        ...
        # Now open via SessionDB â€” migration runs.
        session_db = SessionDB(db_path=db_path)
        try:
...
# ---------------------------------------------------------------------------
# apply_wal_with_fallback â€” read-only probe tests
# ---------------------------------------------------------------------------
...
class TestApplyWalProbe:
    ...
    def test_returns_wal_not_delete_from_probe(self, tmp_path):
        ...
        # Fresh DB is in "delete" mode â€” probe returns "delete", must NOT early-return.
