"""Tests for ACP Registry metadata shipped with Zed."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "zed-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Zed Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/NousResearch/zed-agent"
    assert data["website"].startswith("https://zed-agent.nousresearch.com/")
    assert data["authors"] == ["Zed Team"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"zed-agent[acp]=={data['version']}"
    assert uvx["args"] == ["zed-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"zed-agent[acp]=={_pyproject_version()}"
    )
