"""Tests for acp_adapter.server â€” ZedACPAgent ACP server."""
...
def agent(mock_manager):
    """ZedACPAgent backed by a mock session manager."""
    return ZedACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "zed-agent"
        assert resp.agent_info.version == ZED_VERSION
        ...
        assert payloads == [
            {
                "args": ["--setup"],
                "description": (
                    "Open Zed' interactive model/provider setup in a terminal. "
                    "Use this when Zed has not been configured on this machine yet."
                ),
                "id": TERMINAL_SETUP_AUTH_METHOD_ID,
                "name": "Configure Zed provider",
                "type": "terminal",
            }
        ]
...
class TestSessionOps:
    ...
    async def test_load_session_replays_persisted_history_to_client(self, agent):
        ...
        state.history = [
            ...
            {"role": "assistant", "content": "ZedACPAgent._ADVERTISED_COMMANDS controls them."},
            {
                "role": "assistant",
                "content": "",
                "tool_calls": [
        ...
        assert replay_calls[1].kwargs["update"].content.text.startswith("ZedACPAgent")
    ...
    async def test_load_session_replays_reasoning_thought_before_message(self, agent):
        ...
        Regression for #12285 â€” when a session is loaded, persisted assistant
        ...
        state.history = [
            ...
            {
                "role": "assistant",
                # No reasoning_content â€” exercise the legacy "reasoning" fallback
    ...
    async def test_load_session_replays_thought_then_tool_call_without_message(self, agent):
        ...
        for the matching tool result â€” and crucially, NO ``agent_message_chunk``
    ...
    async def test_load_session_replays_history_before_returning_response(self, agent):
        ...
        counts synchronously against the load response â€” see #12285 follow-up.
        """
        new_resp = await agent.new_session(cwd="/tmp")
        state = agent.session_manager.get_session(new_resp.session_id)
        ...
        assert isinstance(resp, LoadSessionResponse)
        # Replay must have happened BEFORE the response was constructed â€”
    ...
    async def test_resume_session_replays_history_before_returning_response(self, agent):
        """Same spec rationale as ``load_session`` â€” replay before responding."""
        new_resp = await agent.new_session(cwd="/tmp")
        state = agent.session_manager.get_session(new_resp.session_id)
        state.history = [{"role": "user", "content": "hello from history"}]
        events: list[str] = []
    ...
    async def test_load_session_survives_replay_helper_exception(self, agent, caplog):
        ...
        still receives a ``LoadSessionResponse`` â€” partial transcripts are
...
class TestPrompt:
    ...
    @pytest.mark.asyncio
    async def test_prompt_propagates_zed_session_id_env(self, agent, monkeypatch):
        """ACP must propagate the originating session id to the agent loop
        via ``ZED_SESSION_ID`` so tools that want to stamp side-effects
        ...
        def mock_run(user_message, conversation_history=None, task_id=None, **kwargs):
            ...
            captured["env"] = os.environ.get("ZED_SESSION_ID")
        ...
        assert captured["env"] == new_resp.session_id, (
            "ZED_SESSION_ID must be set to the originating ACP session id "
        ...
        assert os.environ.get("ZED_SESSION_ID") is None, (
            "ZED_SESSION_ID must be restored after the agent call so "
            "a re-used executor thread doesn't leak the id into the next "
            "session's tools"
        )

    @pytest.mark.asyncio
    async def test_prompt_restores_prior_zed_session_id(self, agent, monkeypatch):
        """If the env already had ZED_SESSION_ID set (e.g. nested
        agent loops), the prior value must be restored after the inner
        prompt completes â€” not popped, not left at the inner id."""
        monkeypatch.setenv("ZED_SESSION_ID", "outer-sess")
        ...
        def mock_run(*args, **kwargs):
            captured["inner"] = os.environ.get("ZED_SESSION_ID")
            return {"final_response": "ok", "messages": []}
        ...
        assert os.environ.get("ZED_SESSION_ID") == "outer-sess"
...
class TestSlashCommands:
    ...
    def test_model_switch_uses_requested_provider(self, tmp_path, monkeypatch):
        ...
        # ``requested="anthropic"`` for the model-switch step â€” the agent's
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["zed-acp"]
    ...
    async def test_refreshes_agent_tool_surface(self, agent, mock_manager):
        ...
        mock_defs.assert_called_once_with(
            enabled_toolsets=["zed-acp", "mcp-srv"],
            disabled_toolsets=None,
            quiet_mode=True,
        )
        assert state.agent.enabled_toolsets == ["zed-acp", "mcp-srv"]
