...
- 96vc: ACP didn't set ZED_EXEC_ASK, so `check_all_command_guards`
...
1. Setting ZED_EXEC_ASK inside _run_agent (wraps the agent call).
...
class TestThreadLocalApprovalCallback:
    ...
    def test_callback_not_visible_in_different_thread(self):
        ...
        # Each thread must see ONLY its own callback â€” not the other's
    ...
    def test_main_thread_callback_not_leaked_to_worker(self):
        ...
        # Worker thread has no callback set â€” TLS is empty for it
    ...
    def test_sudo_password_cache_isolated_across_acp_sessions_on_same_pool_thread(self):
        ...
        binds ZED_SESSION_KEY per session, so the cache scope key differs
...
class TestAcpExecAskGate:
    """GHSA-96vc-wcxf-jjff: ACP's _run_agent must set ZED_INTERACTIVE so
    ...
    (ZED_EXEC_ASK takes the gateway-queue path which requires a
    notify_cb registered in _gateway_notify_cbs â€” not applicable to ACP,
    which uses a direct callback shape.)"""

    def test_interactive_env_var_routes_to_callback(self, monkeypatch):
        """When ZED_INTERACTIVE is set and an approval callback is
        ...
        # Without ZED_INTERACTIVE: takes auto-approve path, callback NOT called
        result = check_all_command_guards(
            "rm -rf /tmp/test-exec-ask", "local", approval_callback=fake_cb,
        )
        assert result["approved"] is True
        assert called_with == [], (
            "without ZED_INTERACTIVE the non-interactive auto-approve "
            "path should fire without consulting the callback"
        )

        # With ZED_INTERACTIVE: callback IS called, approval flows through it
        ...
        assert called_with, (
            "with ZED_INTERACTIVE the approval path should consult the "
            "registered callback â€” this was the ACP bypass in "
            "GHSA-96vc-wcxf-jjff"
        )
        assert result["approved"] is True
