...
Exercises the full flow through the ACP server layer:
  new_session(mcpServers) â†’ MCP tools registered â†’ prompt() â†’
    tool_progress_callback (ToolCallStart) â†’
    step_callback with results (ToolCallUpdate with rawOutput) â†’
...
from acp_adapter.server import ZedACPAgent
...
# ---------------------------------------------------------------------------
# E2E: MCP registration â†’ prompt â†’ tool events
# ---------------------------------------------------------------------------


class TestMcpRegistrationE2E:
    """Full flow: session with MCP servers â†’ prompt with tool calls â†’ ACP events."""

    @pytest.mark.asyncio
    async def test_session_with_mcp_servers_registers_tools(self, acp_agent, mock_manager):
        """new_session with mcpServers converts them to Zed config and registers."""
        servers = [
            McpServerStdio(
                name="test-fs",
    ...
    async def test_prompt_with_tool_calls_emits_acp_events(self, acp_agent, mock_manager):
        """Prompt â†’ agent fires callbacks â†’ ACP ToolCallStart + ToolCallUpdate events."""
        ...
        for call in mock_conn.session_update.call_args_list:
            # session_update(session_id, update) â€” grab the update
...
class TestMcpSanitizationE2E:
    ...
    async def test_slashed_server_name_registers_cleanly(self, acp_agent, mock_manager):
        """Server name 'ai.exa/exa' should not crash â€” tools get sanitized names."""
        servers = [
            McpServerHttp(
                name="ai.exa/exa",
