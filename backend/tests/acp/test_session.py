"""Tests for acp_adapter.session â€” SessionManager and SessionState."""
...
class TestCreateSession:
    ...
    def test_register_task_cwd_translates_windows_drive_for_wsl_tools(self, monkeypatch):
        ...
        monkeypatch.setattr("zed_constants._wsl_detected", True)
        monkeypatch.setattr(
...
class TestWslCwdTranslation:
    ...
    def test_translate_acp_cwd_leaves_windows_drive_path_unchanged_off_wsl(self, monkeypatch):
        monkeypatch.setattr("zed_constants._wsl_detected", False)
...
class TestForkSession:
    def test_fork_session_deep_copies_history(self, manager):
        ...
        # But a deep copy â€” mutating one doesn't affect the other
...
# ---------------------------------------------------------------------------
# persistence â€” sessions survive process restarts (via SessionDB)
# ---------------------------------------------------------------------------
...
class TestPersistence:
    ...
    def test_create_session_includes_registered_mcp_toolsets(self, tmp_path, monkeypatch):
        ...
        assert captured["enabled_toolsets"] == ["zed-acp", "mcp-olympus", "mcp-exa"]
