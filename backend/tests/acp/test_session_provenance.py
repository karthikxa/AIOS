...
Exercises acp_adapter.provenance against a real SessionDB â€” no mocks â€” covering
...
def test_root_session_no_compression(db):
    ...
    assert prov["currentZedSessionId"] == "root1"
    assert prov["rootZedSessionId"] == "root1"
    assert prov["parentZedSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_zed_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentZedSessionId"] == "old"
    assert prov["rootZedSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousZedSessionId"] == "old"
    # Head rotated this turn â†’ reason/creatorKind flagged.
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootZedSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootZedSessionId"] == "p"  # lineage root still walked


def test_no_false_rotation_when_head_unchanged(db):
    _mk(db, "s")
    # previous == current â†’ no rotation reason emitted.
    prov = build_session_provenance(
        db, "acp-1", "s", previous_zed_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousZedSessionId"] == "s"
