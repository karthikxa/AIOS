"""Tests for MiniMax OAuth provider (zed_cli/auth.py).

Covers:
- PKCE pair generation (S256 challenge)
- _minimax_request_user_code happy path and state-mismatch error
- _minimax_poll_token: pendingâ†’success flow, error status, timeout
...
def test_poll_token_timeout_raises():
    ...
    import zed_cli.auth as auth_module
...
def test_refresh_updates_access_token():
    ...
    with patch("httpx.Client") as mock_client_class:
        ...
        with patch("zed_cli.auth._minimax_save_auth_state"):
            result = _refresh_minimax_oauth_state(state)
...
def test_resolve_credentials_requires_login():
    ...
    with patch("zed_cli.auth.get_provider_auth_state", return_value=None):
        with pytest.raises(AuthError) as exc_info:
            resolve_minimax_oauth_runtime_credentials()
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure():
    ...
    with patch("zed_cli.auth.get_provider_auth_state", return_value=stale_state), \
         patch("zed_cli.auth._refresh_minimax_oauth_state", side_effect=_terminal_refresh), \
         patch("zed_cli.auth._minimax_save_auth_state", side_effect=_capture_save):
        with pytest.raises(AuthError) as exc_info:
            resolve_minimax_oauth_runtime_credentials()
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure():
    ...
    dead-token quarantine path must NOT fire â€” tokens stay on disk for the
    next attempt.
    """
    stale_state = {
    ...
    with patch("zed_cli.auth.get_provider_auth_state", return_value=stale_state), \
         patch("zed_cli.auth._refresh_minimax_oauth_state", side_effect=_transient_refresh), \
         patch("zed_cli.auth._minimax_save_auth_state", side_effect=lambda s: saved_states.append(dict(s))):
        with pytest.raises(AuthError) as exc_info:
            resolve_minimax_oauth_runtime_credentials()
...
def test_get_minimax_oauth_auth_status_logged_in():
    ...
    with patch("zed_cli.auth.get_provider_auth_state", return_value=state):
        status = get_minimax_oauth_auth_status()
...
# ---------------------------------------------------------------------------
# build_minimax_oauth_token_provider â€” per-request callable bearer
# ---------------------------------------------------------------------------
...
def test_token_provider_returns_current_access_token_when_fresh():
    ...
    with patch("zed_cli.auth.get_provider_auth_state", return_value=state), \
        ...
        token = provider()
        # No network call should happen â€” token is fresh.
...
def test_token_provider_refreshes_when_near_expiry():
    ...
    with patch("zed_cli.auth.get_provider_auth_state", return_value=state), \
         patch("httpx.Client") as mock_client_class, \
         patch("zed_cli.auth._minimax_save_auth_state"):
...
def test_token_provider_rereads_state_each_call():
    ...
    persisted by another zed process are immediately visible."""
    from zed_cli.auth import build_minimax_oauth_token_provider
    ...
    provider = build_minimax_oauth_token_provider()
    with patch("zed_cli.auth.get_provider_auth_state", side_effect=states):
        first = provider()
        second = provider()
...
def test_token_provider_raises_not_logged_in_when_state_missing():
    """No state in auth.json â†’ AuthError(not_logged_in, relogin_required=True)."""
    from zed_cli.auth import build_minimax_oauth_token_provider
