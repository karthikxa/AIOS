"""Tests for zed_constants module."""
...
class TestGetDefaultZedRoot:
    """Tests for get_default_zed_root() â€” Docker/custom deployment awareness."""

    def test_no_zed_home_returns_native(self, tmp_path, monkeypatch):
        """When ZED_HOME is not set, returns ~/.zed."""
        monkeypatch.delenv("ZED_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_zed_root() == tmp_path / ".zed"

    def test_zed_home_is_native(self, tmp_path, monkeypatch):
        """When ZED_HOME = ~/.zed, returns ~/.zed."""
        native = tmp_path / ".zed"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZED_HOME", str(native))
        assert get_default_zed_root() == native

    def test_zed_home_is_profile(self, tmp_path, monkeypatch):
        """When ZED_HOME is a profile under ~/.zed, returns ~/.zed."""
        native = tmp_path / ".zed"
    ...
    def test_zed_home_is_docker(self, tmp_path, monkeypatch):
        """When ZED_HOME points outside ~/.zed (Docker), returns ZED_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZED_HOME", str(docker_home))
        assert get_default_zed_root() == docker_home

    def test_zed_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any ZED_HOME outside ~/.zed is treated as the root."""
        custom = tmp_path / "my-zed-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZED_HOME", str(custom))
        assert get_default_zed_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (ZED_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZED_HOME", str(profile))
        assert get_default_zed_root() == docker_root

    def test_no_zed_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\zed, not ~/.zed."""
    ...
    def test_no_zed_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/zed without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("ZED_HOME", raising=False)
...
class TestGetZedHome:
    """Tests for get_zed_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When ZED_HOME is unset on Windows, use %LOCALAPPDATA%\\zed."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("ZED_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
...
class TestZedManagedNode:
    ...
    def test_with_zed_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "zed"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
...
class TestIsContainer:
    """Tests for is_container() â€” Docker/Podman detection."""

    def _reset_cache(self, monkeypatch):
        """Reset the cached detection result before each test."""
        monkeypatch.setattr(zed_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(zed_constants, "_container_detected", True)
        assert is_container() is True
...
class TestParseReasoningEffort:
    """Tests for parse_reasoning_effort() â€” string â†’ reasoning config dict."""
...
class TestSecureParentDir:
    ...
    def test_top_level_dir_skipped(self, monkeypatch):
        ...
        # Path("/usr/foo").parent == Path("/usr") â€” depth 2
