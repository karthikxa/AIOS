...
The official ACP Registry manifest must match ``pyproject.toml`` exactly â€”
        ...
        json.dumps(
            {
                ...
                "distribution": {
                    "uvx": {
                        "package": f"zed-agent[acp]=={version}",
                        "args": ["zed-acp"],
                    }
                },
            },
...
def test_update_acp_registry_versions_bumps_manifest_and_pin(monkeypatch, tmp_path):
    ...
    assert manifest["distribution"]["uvx"]["package"] == "zed-agent[acp]==0.14.0"
    # args stay untouched so we don't accidentally rewrite them.
    assert manifest["distribution"]["uvx"]["args"] == ["zed-acp"]


def test_update_acp_registry_versions_is_silent_when_manifest_missing(
    monkeypatch, tmp_path
):
    """Older release branches predate the ACP Registry asset â€” must no-op."""
    module = _load_release_module(monkeypatch, tmp_path)
