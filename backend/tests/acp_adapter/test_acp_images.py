...
from acp_adapter.server import ZedACPAgent, _content_blocks_to_openai_user_content
...
async def test_initialize_advertises_image_prompt_capability():
    response = await ZedACPAgent().initialize()
...
# 1x1 transparent PNG â€” smallest valid image payload for inlining tests.
...
def test_acp_resource_link_image_mime_inferred_from_suffix(tmp_path):
    """No mimeType sent â€” should still be recognised as image by file suffix."""
