...
class TestSlackNativeSlashes:
    ...
    def test_zed_catchall_is_first(self):
        """``/zed`` must be reserved as the first slot so the legacy
        ``/zed <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "zed"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /zed <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
    ...
    def test_includes_aliases_as_first_class_slashes(self):
        """Aliases (/btw, /bg, â€¦) must be registered as standalone
        slashes â€” this is the whole point of native-slashes parity.

        Asserts the contract (aliases are surfaced as first-class slashes),
        not a specific alias's survival of Slack's 50-slash clamp â€” which alias
        ...
        fills the cap â€” canonical commands win the contest, and clamped
        aliases stay reachable via ``/zed <alias>``.
        """
        slashes = slack_native_slashes()
        ...
        # carries the "Alias for /â€¦" marker), proving the alias pass ran.
    ...
    def test_telegram_parity(self):
        ...
        # Commands deliberately routed through /zed <command> on Slack only
        # (Slack's 50-slash cap) are expected to be absent from native slashes.
        via_zed_norm = {_norm(n) for n in _SLACK_VIA_ZED_ONLY}
        missing = (tg_norm - slack_norm) - reserved_norm - via_zed_norm
        assert not missing, (
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `zed slack manifest`)."""
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_completes_toolset_names(self, monkeypatch):
        ...
        from zed_cli import commands as commands_mod

        # `web` is enabled, `spotify` is disabled â€” enabling should only offer
        # the disabled ones.
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(
            "zed_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
...
from zed_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestDiscordSkillCommandsByCategory:
    ...
    def test_root_level_skills_are_uncategorized(self, tmp_path, monkeypatch):
        """Skills directly under SKILLS_DIR (only 1 path component) â†’ uncategorized."""
        from unittest.mock import patch
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch zed_cli.plugins.get_plugin_commands() to a fixed dict."""
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/zed metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",
