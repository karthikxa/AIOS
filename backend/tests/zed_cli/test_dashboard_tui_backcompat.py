"""Regression test: `zed dashboard --tui` must not hard-crash.

Older Zed desktop app shells (<= 0.15.x) spawn the backend as::

    zed dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
with ``error: unrecognized arguments: --tui`` and ``exit(2)`` â€” the backend
died before it became ready and the desktop GUI showed only "Zed couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real zed_cli.main parser in a subprocess.
    ...
    return subprocess.run(
        [sys.executable, "-m", "zed_cli.main", *args],
        cwd=REPO_ROOT,
        env=env,
        capture_output=True,
        text=True,
