"""Tests for issue #26670 â€” concurrent zed.exe detection and improved
quarantine retry / reboot-deferred fallback during `zed update` on Windows.
...
# Tests in this module either exercise the REAL _detect_concurrent_zed_instances
# helper (and need the autouse stub in tests/zed_cli/conftest.py disabled),
...
# ---------------------------------------------------------------------------
# _detect_concurrent_zed_instances
# ---------------------------------------------------------------------------


def _make_proc(pid: int, exe: str, name: str = "zed.exe"):
...
def test_detect_concurrent_returns_empty_when_no_other_processes(_winp, tmp_path):
    ...
    (scripts_dir / "zed.exe").write_bytes(b"")
    (scripts_dir / "zed-gateway.exe").write_bytes(b"")

    fake_psutil = types.SimpleNamespace(process_iter=lambda attrs: iter([]))
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_zed_instances(scripts_dir)
...
def test_detect_concurrent_excludes_self_pid(_winp, tmp_path):
    ...
    procs = [_make_proc(my_pid, str(shim), "zed.exe")]
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_concurrent_finds_other_zed_process(_winp, tmp_path):
    scripts_dir = tmp_path
    shim = scripts_dir / "zed.exe"
    shim.write_bytes(b"")
    ...
    procs = [
        _make_proc(other_pid, str(shim), "zed.exe"),
    ...
    assert result == [(other_pid, "zed.exe")]
...
def test_detect_concurrent_matches_case_insensitively(_winp, tmp_path):
    ...
    # Simulate the desktop spawning zed.EXE (uppercase ext) from same path
    upper = str(shim).replace("zed.exe", "ZED.EXE")
    procs = [_make_proc(9999, upper, "ZED.EXE")]
    fake_psutil = types.SimpleNamespace(process_iter=lambda attrs: iter(procs))
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_zed_instances(scripts_dir)

    assert result == [(9999, "ZED.EXE")]
...
def test_detect_concurrent_no_psutil_returns_empty(_winp, tmp_path):
    ...
    # Block psutil import â€” simulate environment without it.
...
def test_detect_concurrent_is_noop_off_windows(_winp, tmp_path):
    ...
    assert cli_main._detect_concurrent_zed_instances(tmp_path) == []


# ---------------------------------------------------------------------------
# Parent-chain exclusion (issue #30768 follow-up â€” the setuptools .exe
...
def _fake_psutil_with_parent_chain(
    ...
    *,
    ...
    whose exe can't be read (psutil error) â€” it stays in the candidate set.
    """
...
def test_detect_concurrent_excludes_parent_chain(_winp, tmp_path):
    ...
    Simulates the real Windows topology: zed.exe launcher (PID L) spawns
    ...
    shim.write_bytes(b"")
    me = os.getpid()
    launcher_pid = me + 100  # the .exe launcher â€” our parent

    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(launcher_pid, str(shim), "zed.exe"),
    ]
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_concurrent_still_finds_unrelated_other_zed(_winp, tmp_path):
    """A sibling zed.exe outside our ancestor chain must still be reported."""
    scripts_dir = tmp_path
    shim = scripts_dir / "zed.exe"
    shim.write_bytes(b"")
    me = os.getpid()
    launcher_pid = me + 100  # our .exe launcher (parent â€” must be excluded)
    sibling_pid = me + 200  # an UNRELATED zed.exe (must still be reported)

    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(launcher_pid, str(shim), "zed.exe"),
        _make_proc(sibling_pid, str(shim), "zed.exe"),
    ]
    fake_psutil = _fake_psutil_with_parent_chain(
        parent_chain=[launcher_pid],
        proc_iter_rows=rows,
    ...
    assert result == [(sibling_pid, "zed.exe")]


@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_concurrent_parent_chain_walks_deep(_winp, tmp_path):
    """Multi-level ancestry (shell â†’ launcher â†’ python) is fully excluded."""
    scripts_dir = tmp_path
    shim = scripts_dir / "zed.exe"
    shim.write_bytes(b"")
    me = os.getpid()
    ...
    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(parent_pid, str(shim), "zed.exe"),
        _make_proc(grandparent_pid, str(shim), "zed.exe"),
        _make_proc(greatgrandparent_pid, str(shim), "zed.exe"),
    ]
    fake_psutil = _fake_psutil_with_parent_chain(
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    # ancestor_exe=None â†’ every ancestor's .exe() raises OSError. The helper
    ...
    assert result == [(launcher_pid, "zed.exe")]
...
def test_detect_concurrent_parent_walk_handles_stub_without_process(_winp, tmp_path):
    ...
    rows = [
        _make_proc(me, str(shim), "zed.exe"),
        _make_proc(other_pid, str(shim), "zed.exe"),
    ]
    # SimpleNamespace with ONLY process_iter â€” no Process / NoSuchProcess.
...
# ---------------------------------------------------------------------------
# _quarantine_running_zed_exe â€” retry + reboot-deferred fallback
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    shim = tmp_path / "zed.exe"
    shim.write_bytes(b"old")

    pairs = cli_main._quarantine_running_zed_exe(tmp_path)

    assert len(pairs) == 1
    orig, quarantine = pairs[0]
    assert orig == shim
    assert quarantine.name.startswith("zed.exe.old.")
...
def test_quarantine_retries_then_succeeds(_winp, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli_main, "_zed_exe_shims", lambda d: [shim])
...
def test_quarantine_falls_back_to_reboot_schedule(_winp, tmp_path, capsys, monkeypatch):
    ...
    # out â€” don't undo a deferred operation).
    assert pairs == []
...
def test_quarantine_actionable_warning_when_everything_fails(
    ...
    assert "Zed Desktop" in captured or "gateway" in captured.lower()
...
# ---------------------------------------------------------------------------
# cmd_update integration â€” concurrent-instance gate
# ---------------------------------------------------------------------------


@patch.object(cli_main, "_is_windows", return_value=True)
def test_cmd_update_aborts_on_concurrent_instance(_winp, tmp_path, capsys):
    """If another zed.exe is running, the update bails out before
...
def test_cmd_update_force_bypasses_concurrent_check(_winp, tmp_path):
    ...
    args = SimpleNamespace(
        ...
        gateway=False,
        yes=False,
        force=True,  # â† the bypass
        backup=False,
        no_backup=True,
    )

    detect = MagicMock(return_value=[(9, "zed.exe")])
    ...
    with patch.object(
        ...
        cli_main, "_detect_concurrent_zed_instances", detect
    ), patch.object(
