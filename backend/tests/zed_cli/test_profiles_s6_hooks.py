"""Tests for the Phase 4 s6 hooks in zed_cli.profiles.
...
tests/zed_cli/test_profiles.py; here we only exercise the new
...
def test_register_noop_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    # NOTE: deliberately DO NOT patch detect_service_manager â€” we want
...
def test_register_calls_through_on_s6(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    mgr = _S6Manager()
    monkeypatch.setattr(
        "zed_cli.service_manager.get_service_manager", lambda: mgr,
    )
...
def test_register_swallows_duplicate_value_error(
    ...
    is a benign condition â€” register must not propagate ValueError."""
    _patch_detect_s6(monkeypatch)
    mgr = _S6Manager()
...
def test_register_swallows_arbitrary_error(
    ...
    down `zed profile create` â€” print and continue."""
    _patch_detect_s6(monkeypatch)
    mgr = _S6Manager()
...
def test_register_swallows_no_backend_runtime_error(
    ...
    monkeypatch.setattr(
        "zed_cli.service_manager.get_service_manager", _no_backend,
    )
    # Should NOT raise
