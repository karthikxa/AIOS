"""Tests for zed_cli.managed_uv â€” one path, no guessing."""
...
class TestManagedUvPath:
    def test_posix(self, tmp_path):
        with patch("zed_cli.managed_uv.get_zed_home", return_value=tmp_path), \
             patch("zed_cli.managed_uv.platform.system", return_value="Linux"):
            from zed_cli.managed_uv import managed_uv_path
            assert managed_uv_path() == tmp_path / "bin" / "uv"

    def test_windows(self, tmp_path):
        with patch("zed_cli.managed_uv.get_zed_home", return_value=tmp_path), \
             patch("zed_cli.managed_uv.platform.system", return_value="Windows"):
            from zed_cli.managed_uv import managed_uv_path
...
class TestEnsureUv:
    ...
    def test_installs_if_missing(self, tmp_path):
        with patch("zed_cli.managed_uv.get_zed_home", return_value=tmp_path), \
             patch("zed_cli.managed_uv._install_uv") as mock_install:
    ...
    def test_install_failure_returns_falsy(self, tmp_path):
        with patch("zed_cli.managed_uv.get_zed_home", return_value=tmp_path), \
             patch("zed_cli.managed_uv._install_uv", side_effect=RuntimeError("network down")):
            from zed_cli.managed_uv import ensure_uv
            path = ensure_uv()
            # Failure is a falsy sentinel (not None) so legacy 2-target call
            # sites can still unpack it without raising â€” see
...
class TestEnsureUvUpdateBoundary:
    ...
    ``(path, fresh_bootstrap)`` call conventions â€” **on POSIX**.

    ``zed update`` runs the call site from the old, already-imported
    ``zed_cli.main`` against the freshly pulled ``managed_uv``. A release
    ...
    The dual contract is intentionally **not** offered on Windows â€” see
    ``TestEnsureUvWindowsSafe`` for why â€” so these tests pin ``platform.system``
    to a POSIX value.
    """
...
class TestEnsureUvWindowsSafe:
    ...
    char-iteration â€” both use the iterator protocol â€” so Windows opts out of
    ...
    def test_uvresult_would_break_windows_list2cmdline(self):
        ...
        import subprocess
        from zed_cli.managed_uv import _UvResult
        with pytest.raises(TypeError):
            subprocess.list2cmdline([_UvResult("C:\\zed\\uv.exe"), "pip"])
    ...
    def test_windows_returns_plain_str_safe_for_subprocess(self, tmp_path):
        ...
        with patch("zed_cli.managed_uv.get_zed_home", return_value=tmp_path), \
             patch("zed_cli.managed_uv.platform.system", return_value="Windows"):
            from zed_cli.managed_uv import _UvResult, ensure_uv
            uv_bin = ensure_uv()
...
class TestUpdateManagedUv:
    def test_no_uv_returns_none(self, tmp_path):
        with patch("zed_cli.managed_uv.get_zed_home", return_value=tmp_path):
            from zed_cli.managed_uv import update_managed_uv
            assert update_managed_uv() is None

    def test_self_update_success(self, tmp_path):
        _make_executable(tmp_path / "bin" / "uv")
        with patch("zed_cli.managed_uv.get_zed_home", return_value=tmp_path), \
             patch("zed_cli.managed_uv.subprocess.run") as mock_run:
    ...
    def test_self_update_failure_non_fatal(self, tmp_path):
        ...
        with patch("zed_cli.managed_uv.get_zed_home", return_value=tmp_path), \
            ...
            from zed_cli.managed_uv import update_managed_uv
            result = update_managed_uv()
            # Still returns the path â€” failure is non-fatal
            assert result == str(tmp_path / "bin" / "uv")
...
class TestInstallUvInternals:
    def test_posix_sets_uv_unmanaged_install(self, tmp_path):
        ...
        with patch("zed_cli.managed_uv._install_uv_posix") as mock_posix:
            from zed_cli.managed_uv import _install_uv
            _install_uv(target)
    ...
    def test_windows_sets_uv_install_dir(self, tmp_path):
        ...
        with patch("zed_cli.managed_uv.platform.system", return_value="Windows"), \
             patch("zed_cli.managed_uv._install_uv_windows") as mock_windows:
            from zed_cli.managed_uv import _install_uv
            _install_uv(target)
