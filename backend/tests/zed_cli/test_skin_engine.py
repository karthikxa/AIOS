"""Tests for zed_cli.skin_engine â€” the data-driven skin/theme system."""
...
def reset_skin_state():
    ...
    from zed_cli import skin_engine
...
class TestSkinConfig:
    def test_default_skin_has_required_fields(self):
        from zed_cli.skin_engine import load_skin
        skin = load_skin("default")
        assert skin.name == "default"
        assert skin.tool_prefix == "â”Š"
    ...
    def test_get_branding_with_fallback(self):
        from zed_cli.skin_engine import load_skin
        skin = load_skin("default")
        assert skin.get_branding("agent_name") == "Zed Agent"
...
class TestBuiltinSkins:
    ...
    def test_all_builtin_skins_have_complete_colors(self):
        from zed_cli.skin_engine import _BUILTIN_SKINS, _build_skin_config
...
class TestSkinManagement:
    def test_set_active_skin(self):
        from zed_cli.skin_engine import set_active_skin, get_active_skin, get_active_skin_name
    ...
    def test_init_skin_from_config(self):
        from zed_cli.skin_engine import init_skin_from_config, get_active_skin_name
...
class TestUserSkins:
    def test_load_user_skin_from_yaml(self, tmp_path, monkeypatch):
        ...
        # Patch skins dir
        monkeypatch.setattr("zed_cli.skin_engine._skins_dir", lambda: skins_dir)
...
class TestDisplayIntegration:
    def test_get_skin_tool_prefix_default(self):
        ...
        assert get_skin_tool_prefix() == "â”Š"

    def test_get_skin_tool_prefix_custom(self):
        from zed_cli.skin_engine import set_active_skin
        from agent.display import get_skin_tool_prefix
        set_active_skin("ares")
        assert get_skin_tool_prefix() == "â•Ž"
    ...
    def test_tool_message_uses_skin_prefix(self):
        ...
        assert msg.startswith("â•Ž")
        assert "â”Š" not in msg

    def test_tool_message_default_prefix(self):
        from agent.display import get_cute_tool_message
        msg = get_cute_tool_message("terminal", {"command": "ls"}, 0.5)
        assert msg.startswith("â”Š")
...
class TestCliBrandingHelpers:
    ...
    def test_active_prompt_symbol_ares(self):
        from zed_cli.skin_engine import set_active_skin, get_active_prompt_symbol

        set_active_skin("ares")
        assert get_active_prompt_symbol() == "âš” "

    def test_active_help_header_ares(self):
        from zed_cli.skin_engine import set_active_skin, get_active_help_header

        set_active_skin("ares")
        assert get_active_help_header() == "(âš”) Available Commands"

    def test_active_goodbye_ares(self):
        from zed_cli.skin_engine import set_active_skin, get_active_goodbye

        set_active_skin("ares")
        assert get_active_goodbye() == "Farewell, warrior! âš”"

    def test_prompt_toolkit_style_overrides_cover_tui_classes(self):
        from zed_cli.skin_engine import set_active_skin, get_prompt_toolkit_style_overrides
        set_active_skin("ares")
    ...
    def test_prompt_toolkit_style_overrides_use_skin_colors(self):
        from zed_cli.skin_engine import (
            set_active_skin,
            get_active_skin,
