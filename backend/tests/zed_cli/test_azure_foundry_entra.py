...
``azure-foundry``:
  ...
  * ``_get_azure_foundry_auth_status`` is structural â€” never mints a
...
def fake_azure_identity(monkeypatch):
    """Identical fake to test_azure_identity_adapter â€” keeps Azure SDK
...
class TestResolveAzureFoundryRuntimeEntra:
    ...
    def test_entra_inherits_codex_responses_for_gpt5_family(self, fake_azure_identity):
        ...
        The runtime auto-upgrades api_mode regardless of auth mode â€” this is
        the same behaviour as the static-key path (see
        ``zed_cli/models.py::azure_foundry_model_api_mode``)."""
        from zed_cli.runtime_provider import _resolve_azure_foundry_runtime
        runtime = _resolve_azure_foundry_runtime(
            requested_provider="azure-foundry",
            model_cfg={
        ...
        # GPT-5.x is upgraded to codex_responses â€” Entra path inherits.
        assert runtime["api_mode"] == "codex_responses"
        assert callable(runtime["api_key"])
        assert runtime["auth_mode"] == "entra_id"

    def test_entra_propagates_scope_only(self, fake_azure_identity):
        """``model.entra.scope`` is the only Zed-managed Azure SDK
        ...
        _resolve_azure_foundry_runtime(
            ...
            model_cfg={
                ...
                "entra": {
                    ...
                    # Legacy keys must not crash â€” they are accepted in
    ...
    def test_entra_default_scope_when_unset(self, fake_azure_identity):
        ...
        Microsoft's documented inference scope â€”
        ``https://ai.azure.com/.default`` â€” regardless of whether the
    ...
    def test_entra_with_anthropic_messages_is_supported(self, fake_azure_identity):
        ...
        # Callable api_key â€” the anthropic_adapter detects this and
...
class TestAzureFoundryAuthStatus:
    def test_entra_status_does_not_mint_token(self, monkeypatch, tmp_path):
        ...
        # token provider â€” if the code path tried to mint, the SDK
        # missing would raise.
        monkeypatch.setattr(
