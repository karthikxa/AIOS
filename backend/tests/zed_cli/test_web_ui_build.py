"""Tests for _web_ui_build_needed â€” staleness check for the web UI dist.

Critical invariant: the dashboard Vite build outputs to zed_cli/web_dist/
(vite.config.ts: outDir: "../../zed_cli/web_dist"), NOT web/dist/.
...
from zed_cli.main import _web_ui_build_needed, _build_web_ui, _run_npm_install_deterministic
...
def _make_web_dir(tmp_path: Path) -> tuple[Path, Path]:
    ...
    dist_dir = tmp_path / "zed_cli" / "web_dist"
...
class TestWebUIBuildNeeded:
    ...
    def test_web_dist_dir_not_web_dist_subdir(self, tmp_path):
        """Regression: sentinel must be in zed_cli/web_dist/, NOT web/dist/."""
        web_dir, dist_dir = _make_web_dir(tmp_path)
        _touch(web_dir / "src" / "App.tsx", offset=-10)
        # Place manifest in wrong location (web/dist/) â€” should NOT count as fresh
        wrong_dist = web_dir / "dist" / ".vite" / "manifest.json"
        _touch(wrong_dist)
        # Correct location is empty â†’ still needs build
        assert _web_ui_build_needed(web_dir) is True
...
class TestBuildWebUISkipsWhenFresh:
    ...
    def test_runs_npm_when_dist_missing(self, tmp_path):
        ...
        with patch("zed_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("zed_cli.main.subprocess.run", return_value=mock_cp) as mock_run, \
             patch("zed_cli.main._run_with_idle_timeout", return_value=build_ok) as mock_idle:
            result = _build_web_ui(web_dir)
    ...
    def test_npm_install_uses_utf8_replace_output_decoding(self, tmp_path):
        ...
        with patch("zed_cli.main.subprocess.run", return_value=mock_cp) as mock_run:
    ...
    def test_npm_install_uses_workspace_web_scope(self, tmp_path):
        ...
        # --workspace would be dropped â€” see test below and #42973.)
        (tmp_path / "package-lock.json").write_text("{}", encoding="utf-8")
        mock_cp = __import__("subprocess").CompletedProcess([], 0, stdout="", stderr="")
        build_ok = __import__("subprocess").CompletedProcess([], 0, stdout="", stderr="")
        with patch("zed_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("zed_cli.main.subprocess.run", return_value=mock_cp) as mock_run, \
             patch("zed_cli.main._run_with_idle_timeout", return_value=build_ok):
            result = _build_web_ui(web_dir)
        assert result is True
    ...
    def test_web_install_omits_workspace_when_web_has_own_lockfile(
        ...
        with patch("zed_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("zed_cli.main.subprocess.run", return_value=install_cp) as mock_run, \
             patch("zed_cli.main._run_with_idle_timeout", return_value=build_cp):
            result = _build_web_ui(web_dir)
    ...
    def test_web_build_uses_idle_timeout_helper(self, tmp_path):
        ...
        build_cp = __import__("subprocess").CompletedProcess([], 0, stdout="", stderr="")
        with patch("zed_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("zed_cli.main.subprocess.run", return_value=install_cp), \
             patch("zed_cli.main._run_with_idle_timeout", return_value=build_cp) as mock_idle:
            result = _build_web_ui(web_dir)
...
class TestBuildWebUIRetryAndStaleFallback:
    ...
    def test_retries_build_once_on_failure(self, tmp_path):
        ...
        with patch("zed_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("zed_cli.main._time.sleep") as mock_sleep, \
             patch("zed_cli.main.subprocess.run", return_value=install_ok), \
             patch("zed_cli.main._run_with_idle_timeout",
    ...
    def test_falls_back_to_stale_dist_when_retry_also_fails(self, tmp_path, capsys):
        ...
        with patch("zed_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("zed_cli.main._time.sleep"), \
             patch("zed_cli.main.subprocess.run", return_value=install_ok), \
             patch("zed_cli.main._run_with_idle_timeout",
                   side_effect=[build_fail, build_fail]):
            result = _build_web_ui(web_dir, fatal=True)

        # MUST return True (serve stale) â€” issue #23817 â€” even with fatal=True,
