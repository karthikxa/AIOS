"""Regression tests for zed_cli._ensure_utf8().

Covers the crash class where the setup wizard (and other banner-printing
commands) emit box-drawing characters and the âš• glyph, which raise
...
in ``zed setup`` because the repair returned early. See the Raspberry Pi
report: latin-1 locale â†’ UnicodeEncodeError before the wizard could start.
"""
...
def test_latin1_stdout_is_repaired_to_utf8(monkeypatch):
    ...
    assert "âš•".encode("utf-8") in sys.stdout.getvalue()


def test_ascii_posix_locale_is_repaired(monkeypatch):
    """C/POSIX locale resolves to ascii stdout â€” also must be repaired."""
...
def test_fallback_when_reconfigure_unavailable(monkeypatch, tmp_path):
    ...
    sys.stdout.flush()
    fh.close()
    assert "âš•".encode("utf-8") in real_path.read_bytes()
