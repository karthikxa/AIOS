...
through the REAL ``gated_auth_middleware`` (session-cookie mint â†’
authenticated request â†’ transparent refresh), the login-page credential
...
# ---------------------------------------------------------------------------
# Test password provider â€” minimal, in-memory, signed tokens.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# /auth/password-login â€” end-to-end through the real middleware
# ---------------------------------------------------------------------------
...
class TestPasswordLoginRoute:
    def test_valid_credentials_set_session_cookies_and_return_next(
        ...
        # HTTPS request â†’ __Host- prefixed access-token cookie is set.
    ...
    def test_session_cookie_then_grants_authenticated_access(self, gated_app):
        ...
        # the TestClient retains â€” proving the minted session is accepted
    ...
    def test_wrong_password_returns_generic_401(self, gated_app):
        ...
        # Generic detail â€” no user-vs-password distinction.
    ...
    def test_oauth_provider_rejects_password_login_with_404(self):
        ...
        # reachable via the password route â€” same 404 as unknown, so the
    ...
    def test_open_redirect_next_is_dropped(self, gated_app):
        ...
        assert resp.status_code == 200
        # Malicious absolute URL dropped â†’ lands at root.
        assert resp.json()["next"] == "/"

    def test_route_is_public_unauthenticated(self, gated_app):
        # The login route itself must be reachable without a session â€”
...
# ---------------------------------------------------------------------------
# Transparent refresh â€” expired access token, live refresh token
# ---------------------------------------------------------------------------


class TestPasswordSessionRefresh:
    def test_expired_access_token_refreshes_via_rt_cookie(self):
        # TTL=0 â†’ access token born expired; the RT cookie should drive a
