"""
Tests for zed_cli.mcp_config â€” ``zed mcp`` subcommands.
...
def _isolate_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "zed_cli.config.get_zed_home", lambda: tmp_path
    )
    config_path = tmp_path / "config.yaml"
    env_path = tmp_path / ".env"
    monkeypatch.setattr(
        "zed_cli.config.get_config_path", lambda: config_path
    )
    monkeypatch.setattr(
        "zed_cli.config.get_env_path", lambda: env_path
    )
    return tmp_path
...
class TestMcpList:
    def test_list_empty_config(self, tmp_path, capsys):
        from zed_cli.mcp_config import cmd_mcp_list
...
class TestMcpRemove:
    def test_remove_existing_server(self, tmp_path, capsys, monkeypatch):
        ...
        from zed_cli.mcp_config import cmd_mcp_remove
    ...
    def test_remove_cleans_oauth_tokens(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr("builtins.input", lambda _: "y")
        # Also patch get_zed_home in the mcp_config module namespace
        monkeypatch.setattr(
            "zed_cli.mcp_config.get_zed_home", lambda: tmp_path
        )
...
class TestMcpAdd:
    def test_add_no_transport(self, capsys):
        ...
        from zed_cli.mcp_config import cmd_mcp_add
    ...
    def test_add_http_server_all_tools(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "zed_cli.mcp_config._probe_single_server", mock_probe
        )
    ...
    def test_add_connection_failure_save_disabled(
        ...
        """Failed connection â†’ option to save as disabled."""

        def mock_probe_fail(name, config, **kw):
            raise ConnectionError("Connection refused")

        monkeypatch.setattr(
            "zed_cli.mcp_config._probe_single_server", mock_probe_fail
        )
    ...
    def test_add_preset_fills_transport(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "zed_cli.mcp_config._MCP_PRESETS",
...
class TestMcpTest:
    def test_test_not_found(self, tmp_path, capsys):
        _seed_config(tmp_path, {})
        from zed_cli.mcp_config import cmd_mcp_test
...
class TestProbeEnvResolution:
    ...
    where `zed mcp add --auth header` sent a literal
    ``Authorization: Bearer ${MCP_X_API_KEY}`` and got 401."""

    def test_resolve_interpolates_header(self, monkeypatch):
        from zed_cli.mcp_config import _resolve_mcp_server_config
    ...
    def test_resolve_leaves_unset_var_literal(self, monkeypatch):
        ...
        # Unresolved placeholder stays literal (no crash) â€” matches
    ...
    def test_probe_resolves_before_connect(self, monkeypatch):
        ...
        import zed_cli.mcp_config as mc
...
class TestStripBearerPrefix:
    ...
    def test_bare_token_unchanged(self):
        from zed_cli.mcp_config import _strip_bearer_prefix
...
class TestConfigHelpers:
    def test_save_and_load_mcp_server(self, tmp_path):
        from zed_cli.mcp_config import _save_mcp_server, _get_mcp_servers
    ...
    def test_remove_mcp_server(self, tmp_path):
        from zed_cli.mcp_config import (
            _save_mcp_server,
            _remove_mcp_server,
            _get_mcp_servers,
        )
    ...
    def test_env_key_for_server(self):
        from zed_cli.mcp_config import _env_key_for_server
...
# ---------------------------------------------------------------------------
# Tests: Task 7 consolidation â€” cmd_mcp_remove evicts manager cache,
...
class TestMcpLogin:
    def test_login_rejects_unknown_server(self, tmp_path, capsys):
        _seed_config(tmp_path, {})
        from zed_cli.mcp_config import cmd_mcp_login
    ...
    def test_login_false_success_no_token(self, tmp_path, capsys, monkeypatch):
        ...
        succeeds yet no OAuth token exists. Login must NOT claim success â€” it
        ...
        monkeypatch.setattr(
            "zed_cli.mcp_config._probe_single_server",
            lambda name, cfg: [("search_files", "d"), ("read_file_content", "d")],
        )
        # No token file is created â†’ _oauth_tokens_present() returns False.
        from zed_cli.mcp_config import cmd_mcp_login
    ...
    def test_login_genuine_success_with_token(self, tmp_path, capsys, monkeypatch):
        """Probe lists tools AND a token exists â†’ report real success."""
        _seed_config(tmp_path, {
        ...
        assert "Authenticated â€” 3 tool(s) available" in out
