...
class TestTomlValueFormatter:
    ...
    def test_string_with_newline_escaped(self):
        """TOML basic strings don't allow literal newlines â€” a path or
...
class TestMigrate:
    ...
    def test_no_servers_still_writes_permissions_default(self, tmp_path):
        ...
        report = migrate({}, codex_home=tmp_path, discover_plugins=False, expose_zed_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        fake_response = {
            "marketplaces": [{
                ...
                "plugins": [
                    ...
                    # Plugin without availability field â€” pass through
        ...
        assert "legacy-plugin" in names  # no field â†’ don't skip
    ...
    def test_plugin_discovery_failure_non_fatal(self, tmp_path, monkeypatch):
        ...
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, discover_plugins=True, expose_zed_tools=False)
        assert report.written
    ...
    def test_discover_plugins_false_skips_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, discover_plugins=False, expose_zed_tools=False)
    ...
    def test_dry_run_skips_plugin_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, dry_run=True, discover_plugins=True, expose_zed_tools=False)
        assert called["yes"] is False
    ...
    def test_expose_zed_tools_writes_callback_mcp_entry(self, tmp_path):
        """When expose_zed_tools=True (production default), an
        [mcp_servers.zed-tools] entry is written so codex calls back
        into Zed for browser/web/delegate_task/vision/memory tools.

        This is the fix for 'all other tools that codex doesn't provide
        should be useable by zed' â€” quirk #7."""
        report = migrate({}, codex_home=tmp_path,
                         discover_plugins=False,
                         default_permission_profile=None,
                         expose_zed_tools=True)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.zed-tools]" in text
        assert "zed_tools_mcp_server" in text
        ...
        assert "zed-tools" in report.migrated

    def test_expose_zed_tools_disabled_skips_entry(self, tmp_path):
        """expose_zed_tools=False suppresses the callback registration."""
        migrate({}, codex_home=tmp_path,
                discover_plugins=False,
                default_permission_profile=None,
                expose_zed_tools=False)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.zed-tools]" not in text
        assert "zed_tools_mcp_server" not in text

    def test_dry_run_doesnt_write(self, tmp_path):
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, dry_run=True, expose_zed_tools=False)
    ...
    def test_full_migration_round_trip(self, tmp_path):
        ...
        report = migrate(zed_cfg, codex_home=tmp_path, expose_zed_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_idempotent_re_run_replaces_managed_block(self, tmp_path):
        # First migration
        migrate({"mcp_servers": {"a": {"command": "x"}}}, codex_home=tmp_path, expose_zed_tools=False)
        first_text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.a]" in first_text
        # Second migration with different servers
        migrate({"mcp_servers": {"b": {"command": "y"}}}, codex_home=tmp_path, expose_zed_tools=False)
    ...
    def test_managed_root_keys_stay_top_level_when_config_ends_in_table(self, tmp_path):
        """TOML has no explicit 'leave current table' syntax. If Zed appends
        ...
        migrate({}, codex_home=tmp_path, discover_plugins=False, expose_zed_tools=False)
        new_text = target.read_text()
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside Zed' managed block, re-running
        ...
        # First migrate â€” adds managed block below user content
        migrate({"mcp_servers": {"zed-mcp": {"command": "npx"}}},
        ...
        # Re-migrate â€” both should survive
        migrate({"mcp_servers": {"zed-mcp": {"command": "npx"}}},
                codex_home=tmp_path, discover_plugins=False,
                expose_zed_tools=False)
        ...
        assert "[mcp_servers.zed-mcp]" in final
    ...
    def test_skipped_keys_reported(self, tmp_path):
        ...
        }, codex_home=tmp_path, expose_zed_tools=False)
    ...
    def test_invalid_mcp_servers_value(self, tmp_path):
        report = migrate({"mcp_servers": "notadict"}, codex_home=tmp_path, expose_zed_tools=False)
...
class TestStripUnmanagedPluginTables:
    ...
    ``zed codex-runtime migrate`` would re-emit them inside the managed
    ...
    def test_multi_line_array_in_plugin_table_does_not_leak(self):
        ...
        prematurely exit the strip region â€” otherwise array fragments
        ...
        stripped = _strip_unmanaged_plugin_tables(text)
        # Everything inside the plugin table â€” including the multi-line
        # array's continuation lines starting with `[` â€” should be gone.
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        new_text = target.read_text()
        # Only ONE [plugins."tasks@openai-curated"] header should remain â€” inside
        # the managed block â€” not the original outside-the-block copy.
    ...
    def test_migrate_preserves_plugin_tables_when_plugin_list_fails(self, tmp_path, monkeypatch):
        ...
        so we must NOT strip the user's existing [plugins.X] tables â€” that
        ...
        migrate({}, codex_home=tmp_path, discover_plugins=True, expose_zed_tools=False)
        new_text = target.read_text()
        # User's plugin table preserved verbatim â€” we can't re-emit it.
        assert '[plugins."tasks@openai-curated"]' in new_text


# ---- Bug C: ZED_HOME tempdir leak into ~/.codex/config.toml ----


class TestZedHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_zed_tools_mcp_entry()`` read ``ZED_HOME``
    ...
    once codex spawned the zed-tools MCP subprocess.
    """

    def test_tempdir_detector_recognizes_pytest_paths(self):
        assert _looks_like_test_tempdir(
            "/private/var/folders/abc/pytest-of-kshitij/pytest-137/popen-gw2/test_X/zed_test"
        )
        assert _looks_like_test_tempdir(
            "/tmp/pytest-of-user/pytest-12/test_X/zed"
        )
        assert _looks_like_test_tempdir(
            "/private/var/folders/zz/T/pytest-of-bob/pytest-1"
        )

    def test_tempdir_detector_accepts_real_zed_home(self):
        assert not _looks_like_test_tempdir("/Users/alice/.zed")
        assert not _looks_like_test_tempdir("/home/bob/.zed")
        assert not _looks_like_test_tempdir("/opt/zed")
        assert not _looks_like_test_tempdir("")

    def test_pytest_tempdir_not_burned_into_mcp_env(self, monkeypatch):
        """The headline regression: even when ZED_HOME points at a pytest
        tempdir, _build_zed_tools_mcp_entry() must NOT propagate it."""
        monkeypatch.setenv(
            "ZED_HOME",
            "/private/var/folders/xx/pytest-of-user/pytest-99/test_x/zed_test",
        )
        entry = _build_zed_tools_mcp_entry()
        env = entry.get("env", {})
        assert "ZED_HOME" not in env, (
            f"pytest-tempdir ZED_HOME leaked into codex MCP entry: "
            f"{env.get('ZED_HOME')!r}"
        )

    def test_real_zed_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate ZED_HOME (not a tempdir path) DOES propagate so the
        MCP subprocess sees the same config as the parent CLI."""
        # Use a path that looks real â€” under /Users or /home, not /var/folders.
        ...
        real_path = "/Users/alice/.zed"
        monkeypatch.setenv("ZED_HOME", real_path)
        entry = _build_zed_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("ZED_HOME") == real_path

    def test_unset_zed_home_omits_env_key(self, monkeypatch):
        """When ZED_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever ZED_HOME its launcher (systemd, gateway, shell)
