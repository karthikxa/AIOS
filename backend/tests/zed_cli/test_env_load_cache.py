...
The cache exists to keep `zed tools` â†’ "All Platforms" fast: every
...
def test_load_env_caches_on_repeat_calls():
    ...
    from zed_cli.config import invalidate_env_cache, load_env
    ...
    try:
        with patch("zed_cli.config.get_env_path", return_value=env_path):
            first = load_env()
            # Even if a writer outside our cache mutates the file, an
            # mtime/size match means the cache still wins. We simulate that
            # by writing identical bytes back â€” sanity check that the cache
...
def test_invalidate_env_cache_forces_reread():
    ...
    try:
        with patch("zed_cli.config.get_env_path", return_value=env_path):
            ...
            # Rewrite WITHOUT bumping mtime â€” simulates same-second write.
...
def test_save_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    from zed_cli import config as config_mod
    from zed_cli.config import invalidate_env_cache, load_env, save_env_value
    ...
    monkeypatch.setattr(config_mod, "ensure_zed_home", lambda: None)
...
def test_load_env_handles_missing_file():
    ...
    nonexistent = Path(tempfile.gettempdir()) / "zed-test-no-such-env-xyz123.env"
    nonexistent.unlink(missing_ok=True)

    try:
        with patch("zed_cli.config.get_env_path", return_value=nonexistent):
            assert load_env() == {}
