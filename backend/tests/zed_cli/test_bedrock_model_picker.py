...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  1. provider_model_ids("bedrock") â€” uses live discover_bedrock_models() instead
     of the static _PROVIDER_MODELS table, with curated fallback.

  2. list_authenticated_providers() Section 2 (ZED_OVERLAYS) â€” bedrock
  ...
  3. Region resolution â€” resolve_bedrock_region() reads from botocore profile
     when no AWS_REGION / AWS_DEFAULT_REGION env vars are set, so EU/AP users
     in eu-central-1 get eu.* profile IDs, not us.* ones.

All Bedrock API calls are mocked â€” no real AWS credentials needed.
"""
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() â€” bedrock via ZED_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestListAuthenticatedProvidersBedrock:
    ...
    def test_bedrock_uses_live_discovery_not_static_list(self, monkeypatch):
        ...
        # All returned model IDs should have eu.* prefix â€” live discovery result
    ...
    def test_bedrock_falls_back_to_curated_when_discovery_fails(self, monkeypatch):
        ...
        # Should not raise â€” bedrock entry may or may not appear depending on
    ...
    def test_bedrock_no_duplicate_entries(self, monkeypatch):
        """Bedrock must appear at most once â€” not in both Section 1 and Section 2."""
        from zed_cli.model_switch import list_authenticated_providers
...
class TestBedrockOverlayRegistration:
    """bedrock entry in ZED_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from zed_cli.providers import ZED_OVERLAYS
        assert "bedrock" in ZED_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from zed_cli.providers import ZED_OVERLAYS
        assert ZED_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from zed_cli.providers import ZED_OVERLAYS
        assert ZED_OVERLAYS["bedrock"].auth_type == "aws_sdk"
