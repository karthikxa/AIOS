"""Tests for zed_cli.gui_uninstall â€” GUI-only uninstall + install discovery.
...
import zed_cli.gui_uninstall as gu


def _make_agent(zed_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = zed_home / "zed-agent"
    (agent_root / "zed_cli").mkdir(parents=True)
    (agent_root / "zed_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(zed_home: Path) -> None:
    """Create the source-built GUI artifacts a `zed desktop` run produces."""
    desktop = zed_home / "zed-agent" / "apps" / "desktop"
    ...
    (zed_home / "zed-agent" / "node_modules").mkdir(parents=True)
    (zed_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(zed_home: Path) -> None:
    (zed_home / "config.yaml").write_text("x: 1\n")
    (zed_home / ".env").write_text("KEY=secret\n")
    (zed_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    zed_home = tmp_path / ".zed"
    zed_home.mkdir()
    assert gu.agent_is_installed(zed_home) is False
    _make_agent(zed_home)
    assert gu.agent_is_installed(zed_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    zed_home = tmp_path / ".zed"
    (zed_home / "zed-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(zed_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    zed_home = tmp_path / ".zed"
    _make_gui_build(zed_home)
    artifacts = gu.source_built_gui_artifacts(zed_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(zed_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(zed_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    zed_home = tmp_path / ".zed"
    agent_root = _make_agent(zed_home)
    _make_gui_build(zed_home)
    _make_user_data(zed_home)
    ...
    removed = gu.uninstall_gui(zed_home)
    ...
    assert not (zed_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "zed_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (zed_home / "config.yaml").exists()
    assert (zed_home / ".env").exists()
    assert (zed_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    zed_home = tmp_path / ".zed"
    _make_agent(zed_home)
    userdata = tmp_path / "Zed-userdata"
    userdata.mkdir()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(zed_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(zed_home)
    ...
    assert summary["zed_home"] == str(zed_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Zed"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Zed"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Zed"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Zed"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    test checkout) â€” we call run_uninstall in-process against a throwaway
    ZED_HOME with all the destructive externals stubbed out.
    """
    import zed_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_zed_home", lambda: zed_home)
    ...
    from zed_cli import gui_uninstall as gu_mod
    ...
    assert not (desktop / "release").exists()
    assert (zed_home / "config.yaml").exists()
    assert zed_home.exists()


def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole ZED_HOME non-interactively."""
    import zed_cli.uninstall as uninstall

    zed_home = tmp_path / ".zed"
    (zed_home / "zed-agent" / "zed_cli").mkdir(parents=True)
    (zed_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not zed_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m zed_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_zed_home", lambda: zed_home)
    ...
    assert not (desktop / "release").exists()
    assert not (zed_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "zed_cli").exists()
    assert (zed_home / "config.yaml").exists()
...
def test_uninstall_args_namespace_mode_mapping():
    """_UninstallArgs maps mode â†’ the gui/full flags run_uninstall reads."""
    import zed_cli.uninstall as uninstall
