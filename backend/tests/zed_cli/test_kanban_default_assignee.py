"""Regression tests for #27145 â€” kanban.default_assignee for unassigned ready tasks.
...
def isolated_kanban_home(monkeypatch):
    """Spin up a fresh ZED_HOME with a clean kanban DB."""
    test_home = tempfile.mkdtemp(prefix="kanban_default_assignee_test_")
    monkeypatch.setenv("ZED_HOME", test_home)
    # Force-reimport so the fresh ZED_HOME is picked up.
    ...
    # gives each test its own monkeypatched ZED_HOME so no cross-test
    # contamination.


def _fake_spawn(*args, **kwargs):
    """Stand-in for the real worker spawn â€” returns a fake PID."""
    return 12345
...
def test_dry_run_with_default_assignee_reports_without_mutating(isolated_kanban_home):
    ...
    `zed kanban dispatch --dry-run` see the routing decision before
    committing."""
    kb, _home = isolated_kanban_home
    with kb.connect_closing() as conn:
        kb.create_board(slug="default", name="Test")
    ...
    with kb.connect_closing() as conn:
        row = conn.execute("SELECT assignee FROM tasks WHERE id = ?", (task_id,)).fetchone()
    # DB unchanged â€” dry_run did not commit the assignment.
    assert row["assignee"] is None
...
def test_explicitly_assigned_task_untouched_by_default_assignee(isolated_kanban_home):
    ...
    default_assignee logic â€” that fallback only applies to genuinely
    unassigned rows."""
    kb, _home = isolated_kanban_home
    with kb.connect_closing() as conn:
        kb.create_board(slug="default", name="Test")
