...
The fix: Step c now skips colonâ†’slash conversion when the model name already
...
# Shared mock context â€” skip network calls, credential resolution, catalog lookups
...
def _run_switch(raw_input: str, current_provider: str = "openrouter") -> str:
    ...
    with patch("zed_cli.model_switch.resolve_alias", return_value=None), \
         ...
         patch("zed_cli.models.validate_requested_model", return_value=_MOCK_VALIDATION), \
...
class TestVariantTagPreservation:
    ...
    def test_legacy_colon_format_with_tag_converts_first_colon_only(self):
        """vendor:model:free (no slash) â†’ vendor/model:free â€” first colon becomes slash."""
