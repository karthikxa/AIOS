"""Tests for Codex auth â€” tokens stored in Zed auth store (~/.zed/auth.json)."""
...
def _setup_zed_auth(zed_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Zed auth store."""
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_zed_auth(zed_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("ZED_HOME", str(zed_home))
    ...
    monkeypatch.setattr("zed_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    zed_home = tmp_path / "zed"
    _setup_zed_auth(zed_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("ZED_HOME", str(zed_home))
...
def test_resolve_codex_runtime_credentials_falls_back_to_pool_when_singleton_empty(tmp_path, monkeypatch):
    """Regression for #32992 â€” chat path returns 401 when singleton is empty but pool has creds.
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((zed_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``zed auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``zed auth add openai-codex``.
    ...
    (zed_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Legacy alias from the #33000 workaround era â€” its tokens
                ...
                # Independent account from `zed auth add openai-codex` â€”
    ...
    # manual:api_key entry: untouched â€” independent credential.
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``zed auth add openai-codex`` for accounts B and C
    ...
    (zed_home / "auth.json").write_text(json.dumps({
        ...
        "providers": {
            "openai-codex": {
                # Old singleton tokens â€” represent "account A" which the user
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``zed auth add openai-codex`` (account B and account C).
    ...
    # User re-authenticates account A â€” fresh device-code login produces new
    ...
    # acctB: INDEPENDENT entry â€” must NOT be overwritten.
    ...
    # acctC: INDEPENDENT entry â€” must NOT be overwritten.
...
def test_save_codex_tokens_still_refreshes_legacy_manual_alias(tmp_path, monkeypatch):
    ...
    ``zed auth add openai-codex`` as a workaround when there was no
    singleton entry â€” that created a ``manual:device_code`` pool entry that
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                },
                {
                    ...
                    "source": "manual:device_code",
                    "auth_type": "oauth",
                    # Token material matches the singleton â€” this is a true
...
def test_save_codex_tokens_handles_missing_previous_singleton_tokens(tmp_path, monkeypatch):
    ...
    state gracefully â€” fall back to "no previous tokens", which means no
...
def test_save_codex_tokens_alias_match_uses_access_token_only(tmp_path, monkeypatch):
            ...
            "openai-codex": [
                {
                    ...
                    "auth_type": "oauth",
                    "access_token": "shared-at",
                    # No refresh_token at all â€” legacy schema.
                },
            ],
        },
    }))
    ...
    # Treated as alias â†’ refreshed with new tokens.
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Zed auth store, not ~/.codex/."""
    zed_home = tmp_path / "zed"
    codex_home = tmp_path / "codex-cli"
    zed_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "zed-at", "refresh_token": "zed-rt"})

    # ~/.codex/auth.json should NOT exist â€” _save_codex_tokens only touches Zed store
    assert not (codex_home / "auth.json").exists()

    # Zed auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "zed-at"


def test_resolve_returns_zed_auth_store_source(tmp_path, monkeypatch):
    zed_home = tmp_path / "zed"
    _setup_zed_auth(zed_home)
    monkeypatch.setenv("ZED_HOME", str(zed_home))
...
def test_refresh_parses_openai_nested_error_shape_refresh_token_reused(monkeypatch):
    ...
    â€” parser must surface relogin_required and the dedicated message.
    """
...
def test_refresh_falls_back_to_generic_message_on_unparseable_body(monkeypatch):
    """No JSON body â†’ generic 'with status 401' message; 401 always forces relogin."""
    ...
    # invalid/expired â€” force relogin even without a parseable error body.
    assert err.relogin_required is True
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run zed auth".
    """
    from zed_cli.auth import (
    ...
    assert "zed auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from zed_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from zed_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "zed_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("zed_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("zed_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch zed_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("zed_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
