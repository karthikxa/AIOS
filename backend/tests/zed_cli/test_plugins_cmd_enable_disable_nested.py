...
the *mutation* side: `zed plugins enable/disable` must resolve a bare name
OR a full path-derived key (e.g. `observability/nemo_relay`) to the canonical
registry key and write THAT â€” the same string PluginManager gates on â€” so a
...
# ---------------------------------------------------------------------------
# cmd_enable / cmd_disable â€” write the canonical key
# ---------------------------------------------------------------------------
...
class TestEnableDisableNested:
    @patch("zed_cli.plugins.get_bundled_plugins_dir")
    @patch("zed_cli.plugins_cmd._plugins_dir")
    @patch("zed_cli.plugins_cmd._save_disabled_set")
    @patch("zed_cli.plugins_cmd._save_enabled_set")
    @patch("zed_cli.plugins_cmd._get_disabled_set", return_value=set())
    @patch("zed_cli.plugins_cmd._get_enabled_set", return_value=set())
    def test_enable_bare_name_writes_key(
        ...
        from zed_cli.plugins_cmd import cmd_enable
        mock_user.return_value = nested_plugin_env
        mock_bundled.return_value = nested_plugin_env / "nonexistent"
        ...
        # The canonical key â€” NOT the bare name â€” must be persisted, because
    ...
    def test_disable_bare_name_writes_key_and_clears_alias(
        ...
        from zed_cli.plugins_cmd import cmd_disable
        mock_user.return_value = nested_plugin_env
        mock_bundled.return_value = nested_plugin_env / "nonexistent"
        # Simulate an existing config where the plugin was enabled under the
        # legacy bare name â€” disabling must clear that too, or the plugin would
    ...
    def test_enable_flat_plugin_unchanged(
        ...
        """Flat plugins keep writing their bare name (key == name) â€” no regression."""
        from zed_cli.plugins_cmd import cmd_enable
        mock_user.return_value = nested_plugin_env
        mock_bundled.return_value = nested_plugin_env / "nonexistent"
