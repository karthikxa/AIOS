"""Phase 6 â€” 401 re-auth + ``next=`` propagation tests.
...
Verifies the contract documented in Phase 6 v2 of the plan:
  ...
  - ``set_session_cookies(refresh_token="")`` does NOT emit the
    ``zed_session_rt`` cookie (contract V1: no RT to persist).
  - ``/auth/callback?next=â€¦`` honours the same-origin landing path.
"""
...
# ``app.state``) â€” the marker name is shared across all dashboard-auth test
...
from zed_cli.dashboard_auth.cookies import (
    SESSION_AT_COOKIE,
    SESSION_RT_COOKIE,
...
class TestApi401Envelope:
    ...
    # ``/api/status`` â€” status is in the shared ``PUBLIC_API_PATHS``
    ...
    def test_invalid_cookie_clears_dead_cookie(self, gated_app):
        """Dead-cookie cleanup â€” Phase 6 requirement so the browser
...
class TestTransparentRefreshOnAccessTokenEviction:
    ...
    ALREADY EVICTED must still transparently refresh via the RT cookie â€”
    not bounce to /login.
    ...
    the browser deletes ``zed_session_at`` the instant the token lapses,
    while ``zed_session_rt`` lives for 30 days. From that moment the
    browser sends ONLY the refresh-token cookie. The original gate bailed at
    ``if not at: return _unauth_response(...)`` â€” bouncing the user to
    ...
    REFRESH_SUCCESS â€” the refresh code never ran. With the fix it returns
    ...
    def _build_rt_only_app(self):
        ...
        """
        import time as _t
        from tests.zed_cli.conftest_dashboard_auth import _sign
    ...
    def test_at_evicted_rt_present_refreshes_transparently(self, gated_app):
        ...
        # Browser sends ONLY the RT cookie â€” the AT cookie has aged out.
        ...
        # Transparent refresh â€” request served, NOT bounced.
        assert r.status_code == 200, (
            f"expected 200 (transparent refresh) got {r.status_code} "
            f"â€” the AT-evicted/RT-present case bounced to login"
        )
    ...
    def test_dead_rt_only_bounces_to_login(self, gated_app):
        ...
        refresh raises RefreshExpiredError â†’ clear + relogin), not 500."""
        clear_providers()
        # default_ttl=0 â†’ the stub treats the minted RT as born-expired,
...
class TestHtmlRedirectNext:
    ...
    def test_auth_loop_avoided(self, gated_app):
        ...
        # /api/auth/me requires auth. Without cookie â†’ 401 with login_url
...
class TestNextSameOriginValidation:
    def test_protocol_relative_path_dropped(self, gated_app):
        # `//evil.com/foo` parses to a protocol-relative URL â€” browser
        ...
        # gate may see "/evil.com" â€” either way the encoded value
    ...
    def test_safe_next_validator_accepts_same_origin(self):
        from zed_cli.dashboard_auth.middleware import _safe_next_target
    ...
    def test_safe_next_validator_rejects_api_paths(self):
        ...
        # Exact ``/api`` (no trailing slash) also rejected â€” the dashboard
    ...
    def test_safe_next_validator_does_not_reject_api_prefix_lookalikes(self):
        ...
        # ``/apidocs`` or ``/api-keys`` lookalike SPA routes â€” we must
...
class TestAuthCallbackNext:
    ...
    These tests drive the real flow exactly as the gate produces it:
      ...
      1. unauth GET /sessions  â†’ 302 /login?next=%2Fsessions
      2. GET /login?next=%2Fsessions â†’ HTML with provider buttons that
         carry next=%2Fsessions in their hrefs
      3. GET /auth/login?provider=stub&next=%2Fsessions â†’ 302 to IDP +
         PKCE cookie carrying provider/state/verifier/next
      4. IDP returns to /auth/callback?code=...&state=... (NO next on
         the callback URL â€” real IDPs only echo back code+state)
    ...
    def _drive_oauth_via_login(
        ...
        """Walk /login â†’ /auth/login â†’ IDP-bounce â†’ /auth/callback like
        ...
        provider button â€” False for cases where the same-origin
        ...
        # Stub IDP "returns" code+state on the callback URL â€” same shape
    ...
    def test_attacker_callback_next_param_is_ignored(self, gated_app):
        ...
        assert r.status_code == 302
        # No next= was in the PKCE cookie, so landing must be "/" â€”
        # NOT /internal-admin.
        assert r.headers["location"] == "/"
    ...
    def test_callback_with_api_next_lands_at_root(self, gated_app):
        ...
        Drive ``/auth/login?next=/api/analytics/models?days=30`` â€”
        ...
        assert r.status_code == 302
        # Landing falls back to "/" â€” NOT the API URL.
        assert r.headers["location"] == "/"
...
class TestValidatePostLoginTarget:
    """Cover ``_validate_post_login_target`` directly â€” it's the second
    ...
    def test_accepts_same_origin_paths(self):
        from zed_cli.dashboard_auth.routes import _validate_post_login_target
    ...
    def test_rejects_api_paths(self):
        ...
        # URL-encoded form â€” what the cookie actually carries.
        assert (
    ...
    def test_does_not_reject_api_prefix_lookalikes(self):
        from zed_cli.dashboard_auth.routes import _validate_post_login_target
        # SPA route lookalikes â€” must NOT be dropped.
...
class TestRenderLoginHtmlNext:
    ...
    def test_next_threaded_url_encoded(self):
        ...
        # next= is URL-encoded â€” quote(safe='') turns "/" into "%2F",
