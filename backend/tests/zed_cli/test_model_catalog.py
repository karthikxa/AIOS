"""Tests for zed_cli.model_catalog â€” remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate ZED_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".zed"
    home.mkdir()
    ...
    import importlib
    from zed_cli import model_catalog
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from zed_cli.model_catalog import _validate_manifest
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in zed_cli.models."""

    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from zed_cli import model_catalog
        from zed_cli.models import get_curated_nous_model_ids, _PROVIDER_MODELS
    ...
    def test_curated_nous_ids_prefers_manifest(self, isolated_home):
        from zed_cli import model_catalog
        from zed_cli.models import get_curated_nous_model_ids
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order â€” matching the ``zed model`` CLI â€” not the live
        ...
        # ``ZED_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` ZED_HOME directly instead.
        import importlib
        from zed_cli import model_catalog
        from zed_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            from zed_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["ZED_HOME"])
            ...
            # stays an invariant â€” it can't rot as the curated/manifest list grows.
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("zed_cli.models.check_nous_free_tier", return_value=False), patch(
                "zed_cli.models.union_with_portal_free_recommendations",
    ...
    def test_picker_max_models_cap_semantics(self, tmp_path, monkeypatch):
        ...
        the ``is not None`` distinction the cap-removal follow-up introduced â€”
        ...
        zero_row = _nous(zero)
        # 0 means an empty model list â€” NOT unlimited. total_models still real.
...
# -----------------------------------------------------------------------------
# Drift guard â€” prevent the in-repo curated lists from going out of sync with
