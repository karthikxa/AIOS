...
Covers:
 - Normal path (tokens already in Zed auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 â€” Zed
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `zed auth openai-codex`. The old
...
@pytest.fixture()
def zed_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Zed auth store (no Codex CLI needed)."""
...
def test_codex_picker_uses_live_codex_catalog(zed_auth_only_env, tmp_path, monkeypatch):
    ...
    # Force the cache fallback path â€” without this the test issues a real
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) â€” not in env vars or Zed
    auth store."""
    zed_home = tmp_path / ".zed"
    zed_home.mkdir()
