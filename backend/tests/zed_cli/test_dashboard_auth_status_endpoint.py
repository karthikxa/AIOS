"""Phase 7 â€” /api/status exposes auth-gate state + AuthWidget integration.
...
The AuthWidget itself is .tsx â€” no Python test here. The widget's
...
def test_status_reports_auth_required_in_gated_mode(gated_client):
    # No ``_login()`` call â€” ``/api/status`` is in the shared
...
def test_status_preserves_existing_fields(loopback_client):
    ...
    body = r.json()
    expected_keys = {
        "version", "release_date", "zed_home", "config_path", "env_path",
...
_HOST_DETAIL_FIELDS = frozenset({
    "zed_home", "config_path", "env_path", "gateway_pid",
    "gateway_health_url",
})
...
def test_status_withholds_host_detail_in_gated_mode(gated_client):
    ...
    expose only the liveness + auth-gate shape â€” never absolute host paths,
...
def test_status_includes_host_detail_in_loopback_mode(loopback_client):
    ...
    full payload (including host paths and PID) is still served â€” preserving
    the StatusPage / ``zed status`` experience for local operators."""
    r = loopback_client.get("/api/status")
    assert r.status_code == 200
    body = r.json()
