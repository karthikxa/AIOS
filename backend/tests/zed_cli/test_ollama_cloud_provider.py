...
from zed_cli.auth import PROVIDER_REGISTRY, resolve_provider, resolve_api_key_provider_credentials
from zed_cli.models import _PROVIDER_MODELS, _PROVIDER_LABELS, _PROVIDER_ALIASES, normalize_provider
...
class TestOllamaCloudAliases:
    ...
    def test_bare_ollama_stays_local(self):
        """Bare 'ollama' alias routes to 'custom' (local) â€” not cloud."""
...
# â”€â”€ Model Catalog (dynamic â€” no static list) â”€â”€

class TestOllamaCloudModelCatalog:
    def test_no_static_model_list(self):
        """Ollama Cloud models are fetched dynamically â€” no static list to maintain."""
    ...
    def test_provider_model_ids_returns_dynamic_models(self, tmp_path, monkeypatch):
        ...
        with patch("zed_cli.models.fetch_api_models", return_value=["qwen3.5:397b"]), \
...
# â”€â”€ Model Picker (list_authenticated_providers) â”€â”€
...
# â”€â”€ Merged Model Discovery â”€â”€
...
class TestOllamaCloudMergedDiscovery:
    def test_merges_live_and_models_dev(self, tmp_path, monkeypatch):
        ...
        with patch("zed_cli.models.fetch_api_models", return_value=["qwen3.5:397b", "glm-5"]), \
             patch("agent.models_dev.fetch_models_dev", return_value=mock_mdev):
    ...
    def test_uses_disk_cache(self, tmp_path, monkeypatch):
        ...
        with patch("zed_cli.models.fetch_api_models", return_value=["model-a"]) as mock_api, \
             patch("agent.models_dev.fetch_models_dev", return_value={}):
            first = fetch_ollama_cloud_models(force_refresh=True)
            assert first == ["model-a"]
            assert mock_api.call_count == 1

            # Second call â€” should use disk cache, not call API
    ...
    def test_stale_cache_used_on_total_failure(self, tmp_path, monkeypatch):
        ...
        from zed_cli.models import fetch_ollama_cloud_models, _save_ollama_cloud_cache
        ...
        with patch("zed_cli.models.fetch_api_models", return_value=None), \
             patch("agent.models_dev.fetch_models_dev", return_value={}):
            result = fetch_ollama_cloud_models(force_refresh=True)
...
# â”€â”€ Model Normalization â”€â”€

class TestOllamaCloudModelNormalization:
    def test_passthrough_bare_name(self):
        """Ollama Cloud is a passthrough provider â€” model names used as-is."""
...
# â”€â”€ URL-to-Provider Mapping â”€â”€
...
# â”€â”€ models.dev Integration â”€â”€
...
# â”€â”€ Agent Init (no SyntaxError) â”€â”€
...
class TestOllamaCloudAgentInit:
    ...
    def test_ollama_cloud_agent_uses_chat_completions(self, monkeypatch):
        """Ollama Cloud falls through to chat_completions â€” no special elif needed."""
        monkeypatch.setenv("OLLAMA_API_KEY", "test-key")
...
class TestOllamaCloudProvidersNew:
    ...
    def test_get_provider(self):
        from zed_cli.providers import get_provider
...
# â”€â”€ Cloud Suffix Stripping â”€â”€
...
class TestOllamaCloudSuffixStripping:
    ...
    def test_no_duplicate_when_live_clean_and_mdev_suffixed(self, tmp_path, monkeypatch):
        """Live API returns clean ID; mdev has :cloud variant â€” result has exactly one entry."""
        from zed_cli.models import fetch_ollama_cloud_models
        ...
        with patch("zed_cli.models.fetch_api_models", return_value=["kimi-k2.6", "glm-5.1"]), \
             patch("agent.models_dev.fetch_models_dev", return_value=mock_mdev):
            result = fetch_ollama_cloud_models(force_refresh=True)
    ...
    def test_strip_suffix_helper(self):
        ...
        from zed_cli.models import _strip_ollama_cloud_suffix
