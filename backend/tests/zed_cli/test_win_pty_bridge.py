"""Unit tests for zed_cli.win_pty_bridge â€” ConPTY spawning + byte forwarding.

Windows-only counterpart to tests/zed_cli/test_pty_bridge.py.  Drives
``WinPtyBridge`` with minimal Windows processes (``cmd.exe``, ``python -c â€¦``)
...
The bridge is the ConPTY backend behind the dashboard ``/chat`` tab â€” see
``zed_cli/web_server.py`` ``/api/pty`` handler â€” so these tests are the
...
# WinPtyBridge can be imported on every platform â€” ``is_available`` just
...
from zed_cli.win_pty_bridge import PtyUnavailableError, WinPtyBridge
...
class TestWinPtyBridgeUnavailable:
    ...
    def test_bridge_class_is_importable(self):
        ...
        #     from zed_cli.win_pty_bridge import WinPtyBridge, PtyUnavailableError
...
class TestWinPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = WinPtyBridge.spawn(["cmd.exe", "/c", "echo zed-ok"])
        try:
            output = _read_until(bridge, b"zed-ok")
            assert b"zed-ok" in output
        finally:
            bridge.close()
    ...
    def test_write_after_close_is_silent(self):
        bridge = WinPtyBridge.spawn(["cmd.exe", "/c", "exit 0"])
        bridge.close()
        # Must not raise â€” the dashboard WebSocket reader sometimes writes
...
class TestWinPtyBridgeResize:
    ...
    def test_resize_clamps_garbage_dimensions(self):
        ...
        # propagate to the ConPTY API.  131072 > unsigned short max â€” the
    ...
    def test_resize_after_close_is_silent(self):
        bridge = WinPtyBridge.spawn(["cmd.exe", "/c", "exit 0"])
        bridge.close()
        # Must not raise â€” closed bridges still receive late resize escapes
...
class TestClampDimension:
    """The clamp helper is the load-bearing piece â€” the dashboard sends
    ...
    def test_clamps_above_max(self):
        from zed_cli.win_pty_bridge import _MAX_COLS, _MAX_ROWS, _clamp

        assert _clamp(131072, _MAX_COLS) == _MAX_COLS
        assert _clamp(131072, _MAX_ROWS) == _MAX_ROWS

    def test_floors_at_one(self):
        from zed_cli.win_pty_bridge import _MAX_COLS, _clamp
...
class TestWinPtyBridgeClose:
    ...
    def test_close_terminates_long_running_child(self):
        ...
        # forwarding bytes â€” verifying that flips to False is the contract
...
class TestWinPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = WinPtyBridge.spawn(
            [
                sys.executable,
                "-c",
                "import os; print('ZED_PTY_TEST=' + os.environ.get('ZED_PTY_TEST',''))",
            ],
            env={**os.environ, "ZED_PTY_TEST": "pty-env-works"},
        )
        try:
    ...
    def test_spawn_defaults_term_when_not_set(self):
        ...
        # doesn't already carry one â€” xterm.js expects ANSI/SGR sequences.
