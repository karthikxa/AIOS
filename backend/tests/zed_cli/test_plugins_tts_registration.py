...
``$ZED_HOME/plugins/``, runs ``PluginManager().discover_and_load()``,
and asserts the registration result.

Mirrors the structure of
``tests/zed_cli/test_plugin_scanner_recursion.py::TestRegisterImageGenProvider``.
"""
...
class TestRegisterTTSProvider:
    ...
    def test_accepts_valid_provider(self):
        ...
        _enable(zed_home, "my-tts-plugin")
    ...
    def test_rejects_non_provider(self, caplog):
        ...
        _enable(zed_home, "bad-tts-plugin")
    ...
    def test_rejects_builtin_shadow(self, caplog):
        ...
        _enable(zed_home, "shadow-tts-plugin")

        with caplog.at_level("WARNING"):
            mgr = PluginManager()
            mgr.discover_and_load()

        # Plugin still loaded normally â€” built-in shadowing is a warning,
