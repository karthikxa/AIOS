"""Regression test for #28181 â€” kanban worker SIGTERM must terminate the process.
...
for interactive ``zed chat -q`` invocations, but kanban workers spawned by
...
The fix: when the process is a dispatcher-spawned worker (``ZED_KANBAN_TASK``
...
def _synthetic_worker_script() -> str:
    ...
    shape without needing the full zed_cli boot path (config, providers,
    ...
    return textwrap.dedent(
        ...
        # Non-daemon thread that blocks forever â€” simulates the worker
...
def _is_alive_like_dispatcher(pid: int) -> bool:
    """Mirrors zed_cli/kanban_db.py:_pid_alive on Linux.

    A zombie is treated as dead â€” the dispatcher's _pid_alive checks
...
def test_sigterm_with_kanban_task_env_terminates_quickly():
    """With ZED_KANBAN_TASK set, SIGTERM should kill the process in <2s
    even when a non-daemon thread is still alive."""
    proc = _spawn_synthetic({"ZED_KANBAN_TASK": "t_test_28181"})
    try:
        t0 = time.time()
        ...
        while time.time() < deadline:
            ...
            time.sleep(0.02)
        pytest.fail(
            f"process still alive 2s after SIGTERM with ZED_KANBAN_TASK set "
            f"(dispatcher would keep extending claim) â€” fix regressed"
        )
    finally:
        _cleanup(proc)
...
def test_sigterm_without_kanban_task_env_uses_keyboard_interrupt_path():
    """Without ZED_KANBAN_TASK, the original KeyboardInterrupt path runs.

    This is the contrast case proving the fix is gated on the env var: in
    interactive ``zed chat -q`` (no env var), behavior is unchanged. The
...
def test_real_handler_uses_os_exit_for_kanban_workers():
    ...
    os._exit(0) when ZED_KANBAN_TASK is set.
    ...
    assert "ZED_KANBAN_TASK" in body, (
        "_signal_handler_q must gate its kanban-worker exit path on "
        "ZED_KANBAN_TASK â€” see #28181"
    )
    assert "os._exit(0)" in body, (
        "_signal_handler_q must call os._exit(0) for kanban workers â€” "
