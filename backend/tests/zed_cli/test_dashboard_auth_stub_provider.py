...
def _pkce_payload(ls) -> dict:
    ...
    return dict(
        item.split("=", 1)
        for item in ls.cookie_payload["zed_session_pkce"].split(";")
    )
...
def test_stub_start_login_returns_callback_redirect():
    ...
    assert "zed_session_pkce" in ls.cookie_payload
...
def test_stub_refresh_round_trips():
    ...
    # same wall-clock second as the original â€” payload contents are
