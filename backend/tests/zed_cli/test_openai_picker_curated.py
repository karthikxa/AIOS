...
Bug 1 â€” OpenAI picker dumped the raw ``/v1/models`` catalog
    ...
    moderation, gpt-3.5, â€¦). The ``zed model`` CLI shows only the curated
...
Bug 2 â€” OpenRouter appeared authenticated whenever OPENAI_API_KEY was set
    OpenRouter's ZedOverlay carried ``extra_env_vars=("OPENAI_API_KEY",)``.
...
def test_openrouter_overlay_does_not_list_openai_api_key():
    overlay = ZED_OVERLAYS["openrouter"]
