...
def test_exec_schtasks_decodes_with_replace_errors(monkeypatch):
    ...
    code, out, err = gateway_windows._exec_schtasks(["/Query", "/TN", "Zed_Gateway"])
...
def test_write_task_script_anchors_cmd_cd_at_zed_home(monkeypatch, tmp_path):
    project = tmp_path / "project"
    zed_home = tmp_path / "zed-home"
    zed_home.mkdir()
    ...
    assert f"cd /d {gateway_windows._quote_cmd_script_arg(str(zed_home.resolve()))}" in content
...
def _arrange_startup_fallback(monkeypatch, tmp_path, running_pids):
    script_path = tmp_path / "Zed_Gateway_alice.cmd"
    startup_entry = tmp_path / "Startup" / "Zed_Gateway_alice.cmd"
    calls = []

    monkeypatch.setattr(gateway_windows, "_prompt_install_choices", lambda *args, **kwargs: (False, True))
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Zed_Gateway_alice")
...
def test_gateway_cmd_script_uses_pythonw_without_replace_or_start_churn(monkeypatch):
    ...
    content = gateway_windows._build_gateway_cmd_script(
        r"C:\\Zed\\zed-agent\\venv\\Scripts\\python.exe",
        r"C:\\Zed\\zed-agent",
        r"C:\\ZedHome\\profiles\\alice",
        "--profile alice",
    )
...
def test_elevated_gateway_command_uses_pythonw_hidden_console(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows, "_derive_venv_pythonw", lambda exe: exe.replace("python.exe", "pythonw.exe"))
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Zed\venv\Scripts\python.exe")
...
def test_install_scheduled_task_recreates_instead_of_change(monkeypatch, tmp_path):
    ...
    ok, detail = gateway_windows._install_scheduled_task("Zed_Gateway_alice", script_path)

    assert ok is True
    assert "/Change" not in [arg for call in calls for arg in call]
    assert calls[0][:4] == ("/Delete", "/F", "/TN", "Zed_Gateway_alice")
...
def test_install_scheduled_task_success_start_now_uses_direct_spawn_not_task_run(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        gateway_windows,
        "_install_scheduled_task",
        lambda task_name, script_path: (True, "Created Scheduled Task 'Zed_Gateway_alice'"),
    )
...
def test_install_access_denied_launches_elevated_install_before_startup_fallback(monkeypatch, tmp_path, capsys):
    ...
    assert "Launched elevated Zed gateway install prompt" in out
...
def test_install_startup_fallback_does_not_auto_spawn_when_gateway_stopped(monkeypatch, tmp_path, capsys):
    ...
    assert "zed --profile alice gateway start" in out
...
def test_uninstall_access_denied_prompts_before_elevating(monkeypatch, tmp_path, capsys):
    ...
    assert "UAC is Windows' admin approval prompt" in out
    assert "Launched elevated Zed gateway uninstall prompt" in out
...
# ---------------------------------------------------------------------------
# stop() drain semantics â€” issue #33778
#
...
# resume_pending=True) never fires when `zed gateway stop` kills the
...
def test_stop_waits_for_graceful_drain_before_force_kill(monkeypatch):
    ...
    marker â€” escalating to taskkill /F afterwards would be wasted
    ...
    # NOT with force=True â€” drain succeeded, gateway is already gone.
...
def test_stop_escalates_to_force_kill_when_drain_times_out(monkeypatch):
    ...
    `zed gateway start` fails with "another instance is running".
    """
    pid = 77777
    events = []
    ...
    from gateway import status as status_mod
    monkeypatch.setattr(status_mod, "write_planned_stop_marker", lambda p: True)
    # PID never exits â€” drain times out.
...
def test_stop_no_running_gateway_skips_drain(monkeypatch):
    ...
    # (defensive â€” covers the case where a stray gateway is alive without
...
def test_drain_helper_still_waits_if_marker_write_fails(monkeypatch):
    ...
    gateway can't drain â€” but the wait still happens so a slow-shutdown
