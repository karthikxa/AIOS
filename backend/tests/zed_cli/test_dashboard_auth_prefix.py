...
prefix (e.g. ``mission-control.tilos.com/zed/*`` -> local Caddy ->
:9119), injecting ``X-Forwarded-Prefix: /zed`` on every request.

The dashboard already honours this for the SPA bundle (rewriting asset
URLs and the bootstrap ``__ZED_BASE_PATH__``). The OAuth gate must
honour it too:

  1. The gate's ``Location:`` redirect to /login (in
     ``_unauth_response``) needs to be ``/zed/login`` so the browser
  ...
  3. ``_redirect_uri`` (the OAuth callback URL handed to the IDP) must
     ...
     ``/zed/auth/callback`` and the user gets 404.
  4. Cookies must use ``Path=/zed`` when behind a prefix so they
  ...
  5. The ``__Host-`` cookie prefix requires ``Path=/`` â€” when behind an
...
# Same xdist group as the other dashboard-auth tests â€” they all mutate
...
class TestGateRedirectsCarryPrefix:
    def test_html_redirect_to_login_carries_prefix(self, gated_app_proxied):
        r = gated_app_proxied.get(
            "/sessions",
            headers={"x-forwarded-prefix": "/zed"},
        ...
        assert r.headers["location"].startswith("/zed/login"), (
    ...
    def test_api_401_envelope_login_url_carries_prefix(self, gated_app_proxied):
        ...
        # include the prefix.
        assert body["login_url"].startswith("/zed/login"), (
    ...
    def test_no_prefix_header_keeps_unprefixed_paths(self, gated_app_direct):
        ...
        NOT gain a phantom prefix â€” the Fly-direct deploy shape has no
        proxy at all."""
...
class TestOAuthRedirectUriRespectsPrefix:
    def test_redirect_uri_includes_prefix_in_authorize_url(
        ...
        ``https://mission-control.tilos.com/zed/auth/callback`` â€” the
        ...
        # Stub returns ``{redirect_uri}?code=stub_code&state=...`` â€” so
        ...
        assert parsed.path == "/zed/auth/callback", (
...
# ---------------------------------------------------------------------------
# ZED_DASHBOARD_PUBLIC_URL / dashboard.public_url override
# ---------------------------------------------------------------------------


class TestPublicUrlOverride:
    """``dashboard.public_url`` (env override:
    ``ZED_DASHBOARD_PUBLIC_URL``) lets an operator force the absolute
    base URL the OAuth ``redirect_uri`` is built from.

    When set, it is the *complete authority* â€” scheme + host + optional
    ...
    correctly (or at all) â€” manual nginx setups, on-prem ingresses,
    ...
    def _redirect_uri(self, gated_app, *, headers=None) -> str:
        ...
        # Location header (`{redirect_uri}?code=stub_code&state=â€¦`).
    ...
    def test_public_url_config_yaml_used_when_env_unset(
        ...
        monkeypatch.delenv("ZED_DASHBOARD_PUBLIC_URL", raising=False)
    ...
    def test_env_overrides_config_public_url(
        ...
        """Precedence pin â€” env wins over config.yaml. Fly.io / CI
        secret injection depends on this ordering."""
        monkeypatch.setenv(
            "ZED_DASHBOARD_PUBLIC_URL", "https://from-env.example",
        )
        patch_config("https://from-config.example")
        redirect_uri = self._redirect_uri(gated_app_direct)
        assert redirect_uri == "https://from-env.example/auth/callback", (
            "env var must override config.yaml â€” Fly secret injection "
    ...
    def test_public_url_with_path_prefix_baked_in(
        ...
        (``https://example.com/zed``), the OAuth callback URL is
        the path appended verbatim. The operator is declaring the
        whole authority; we trust them."""
        patch_config(None)
        monkeypatch.setenv(
            "ZED_DASHBOARD_PUBLIC_URL", "https://example.com/zed",
        )
        redirect_uri = self._redirect_uri(gated_app_direct)
        assert redirect_uri == "https://example.com/zed/auth/callback"
    ...
    def test_public_url_ignores_x_forwarded_prefix(
        ...
        patch_config(None)
        monkeypatch.setenv(
            "ZED_DASHBOARD_PUBLIC_URL", "https://example.com/already-prefixed",
        )
    ...
    def test_public_url_strips_trailing_slash(
        ...
        produce identical results â€” no ``//auth/callback`` double slash."""
        patch_config(None)
        monkeypatch.setenv(
            "ZED_DASHBOARD_PUBLIC_URL", "https://example.com/",
        )
        redirect_uri = self._redirect_uri(gated_app_direct)
    ...
    def test_malformed_public_url_falls_through_to_reconstruction(
        ...
        for bad in [
            ...
            monkeypatch.setenv("ZED_DASHBOARD_PUBLIC_URL", bad)
            redirect_uri = self._redirect_uri(gated_app_direct)
            # Fell through to request reconstruction â€” netloc is the
    ...
    def test_empty_public_url_env_treated_as_unset(
        ...
        plugin â€” an empty env var doesn't shadow a valid config.yaml
        entry."""
        monkeypatch.setenv("ZED_DASHBOARD_PUBLIC_URL", "")
        patch_config("https://from-config.example")
        redirect_uri = self._redirect_uri(gated_app_direct)
        assert redirect_uri == "https://from-config.example/auth/callback"
    ...
    def test_scheme_less_public_url_env_warns_operator(
        ...
        of "I set ZED_DASHBOARD_PUBLIC_URL but the callback is still
        ...
        patch_config(None)
        monkeypatch.setenv("ZED_DASHBOARD_PUBLIC_URL", "zed.domain.com")
        ...
        assert any(
            "ZED_DASHBOARD_PUBLIC_URL" in m
            and "zed.domain.com" in m
            and "scheme" in m
            for m in warnings
    ...
    def test_scheme_less_public_url_warning_is_deduplicated(
        ...
        scheme_warnings = [
            ...
            for r in caplog.records
            if r.levelno == logging.WARNING
            and "zed.domain.com" in r.getMessage()
        ]
    ...
    def test_valid_public_url_emits_no_warning(
        ...
        patch_config(None)
        monkeypatch.setenv(
            "ZED_DASHBOARD_PUBLIC_URL", "https://zed.domain.com"
        )

        with caplog.at_level(logging.WARNING, logger=prefix_mod.__name__):
            result = prefix_mod.resolve_public_url()

        assert result == "https://zed.domain.com"
        assert not [
...
class TestCookiePathRespectsPrefix:
    ...
    the ``__Host-`` cookie prefix for additional hardening â€” it binds
    ...
    def test_pkce_cookie_uses_prefix_path(self, gated_app_proxied):
        ...
        pkce = next(c for c in cookies if "zed_session_pkce" in c)
        # Browser only sends cookie back if the request path is under
        # the cookie's Path attribute, so we need /zed here. Bare
        # /-rooted cookies would still be sent but would also be sent
        # to /billing/... etc.
        assert "Path=/zed" in pkce, (
    ...
    def test_pkce_cookie_uses_secure_prefix_when_proxied(
        ...
        pkce_candidates = [
            c for c in cookies
            if c.startswith("__Secure-zed_session_pkce=")
        ]
    ...
    def test_pkce_cookie_uses_host_prefix_when_direct(
        ...
        exact origin (no Domain attribute) â€” best practice for
        ...
        pkce_candidates = [
            c for c in cookies
            if c.startswith("__Host-zed_session_pkce=")
        ]
        assert pkce_candidates, (
    ...
    def test_loopback_cookies_unprefixed(self):
        ...
        The bare cookie name is the right choice â€” neither prefix is
        ...
        assert any(c.startswith("zed_session_pkce=") for c in cookies), (
    ...
    def test_cookies_read_back_round_trip_through_prefix(
        ...
        __Secure- prefix AND Path=/zed, so the next request under
        the same prefix is authenticated.

        Note on TestClient semantics: starlette's TestClient sees the
        literal request path (``/auth/login``, ``/auth/callback``) â€”
        not the public path the proxy displays to the browser
        (``/zed/auth/login``, ``/zed/auth/callback``). A cookie
        set with ``Path=/zed`` would therefore NOT be sent back on
        the second request through TestClient even though it WOULD be
        sent by a real browser hitting ``/zed/auth/callback``. To
        ...
        TestClient's jar â€” we drive /auth/callback with an explicit
        ...
        pkce_set = next(
            ...
            if "zed_session_pkce" in c
        )
        # Parse "__Secure-zed_session_pkce=...; HttpOnly; ...".
        pkce_kv = pkce_set.split(";", 1)[0]  # "__Secure-zed_session_pkce=value"
        state = r1.headers["location"].split("state=")[1]

        # Round-trip the cookie by hand because TestClient's jar won't
        # automatically send a Path=/zed cookie to a /auth/callback
        # request path.
        r2 = gated_app_proxied.get(
            f"/auth/callback?code=stub_code&state={state}",
            headers={
                "x-forwarded-prefix": "/zed",
                "cookie": pkce_kv,
            },
            follow_redirects=False,
        )
        ...
        at_cookies = [
            c for c in cookies
            if c.startswith("__Secure-zed_session_at=")
        ]
        assert at_cookies, (
