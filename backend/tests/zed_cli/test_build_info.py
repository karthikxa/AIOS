"""Tests for zed_cli.build_info â€” baked-in build SHA resolution.

The build SHA is written by the Dockerfile's ``ZED_GIT_SHA`` build-arg
into ``<project_root>/.zed_build_sha``.  These tests cover the read-side
...
def test_get_build_sha_returns_none_when_file_absent(tmp_path):
    """Source installs: no file present â†’ None, callers fall back to git."""
    from zed_cli import build_info

    missing = tmp_path / ".zed_build_sha"  # never created
...
def test_get_build_sha_reads_baked_file(tmp_path):
    """Docker image case: file exists with full 40-char SHA â†’ truncated to 8."""
    from zed_cli import build_info

    sha_file = tmp_path / ".zed_build_sha"
...
def test_get_build_sha_swallows_read_errors(tmp_path):
    """Any IO exception from the read returns None â€” never raises."""
    from zed_cli import build_info
