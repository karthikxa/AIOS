...
class TestApply:
    ...
    def test_disable_does_not_check_binary(self):
        ...
        with patch.object(crs, "check_codex_binary_ok") as bin_check:
            r = crs.apply(cfg, "auto")
        assert r.success
        # Binary check is irrelevant when disabling â€” should not be called
    ...
    def test_enable_triggers_mcp_migration(self):
        """Enabling codex_app_server should auto-migrate Zed mcp_servers
        ...
        with patch.object(crs, "check_codex_binary_ok",
                          return_value=(True, "0.130.0")), \
             patch("zed_cli.codex_runtime_plugin_migration.migrate") as mig:
            mig.return_value.migrated = ["filesystem", "zed-tools"]
        ...
        # User MCP servers are reported (excluding internal zed-tools)
        ...
        # Zed tool callback announcement
    ...
    def test_disable_does_not_trigger_migration(self):
        ...
        with patch("zed_cli.codex_runtime_plugin_migration.migrate") as mig:
            r = crs.apply(cfg, "auto")
        assert r.success
        assert not mig.called  # disabling does not migrate

    def test_migration_failure_does_not_block_enable(self):
        """If MCP migration raises, the runtime change still proceeds â€”
