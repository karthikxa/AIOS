"""Tests for --ignore-user-config and --ignore-rules flags on `zed chat`.
...
files. In Zed the equivalent isolation is:

* ``--ignore-user-config`` â†’ skip ``~/.zed/config.yaml`` in ``load_cli_config()``
  (credentials in ``.env`` are still loaded).
* ``--ignore-rules`` â†’ skip AGENTS.md / SOUL.md / .cursorrules auto-injection
...
argparse â†’ cmd_chat â†’ cli.main() â†’ ZedCLI â†’ AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    ``cmd_chat`` logic), so ``monkeypatch.delenv`` alone isn't enough â€”
    ...
    """
    for var in ("ZED_IGNORE_USER_CONFIG", "ZED_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``ZED_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<zed_home>/config.yaml`` is
    skipped even if present â€” the function returns only the built-in defaults
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._zed_home at tmp_path and return a fresh load_cli_config."""
        import cli
        monkeypatch.setattr(cli, "_zed_home", tmp_path)
    ...
    def test_user_config_skipped_when_flag_set(self, tmp_path, monkeypatch):
        ...
        # User-set model.default MUST NOT leak through â€” either the built-in
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("ZED_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``ZedCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting ZED_IGNORE_RULES=1 flips ZedCLI.ignore_rules True."""
        monkeypatch.setenv("ZED_IGNORE_RULES", "1")

        # Import ZedCLI lazily â€” cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of ZedCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.ZedCLI)
        # Replicate the exact logic from cli.py ZedCLI.__init__:
        ignore_rules = False  # constructor default
        obj.ignore_rules = ignore_rules or os.environ.get("ZED_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True

    def test_constructor_flag_alone_enables_ignore_rules(self, monkeypatch):
        monkeypatch.delenv("ZED_IGNORE_RULES", raising=False)
        import cli
        obj = object.__new__(cli.ZedCLI)
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``zed_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("ZED_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("ZED_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("ZED_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("ZED_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("ZED_IGNORE_USER_CONFIG") == "1"
        assert "ZED_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        class FakeArgs:
            pass  # no attributes at all â€” getattr fallback must handle

        self._simulate_cmd_chat_env_setup(FakeArgs())

        assert "ZED_IGNORE_USER_CONFIG" not in os.environ
        assert "ZED_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_flags_present_in_chat_parser(self):
        ...
        # test keeps passing in isolation â€” but the E2E test below catches it.
        import argparse
