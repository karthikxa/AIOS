...
user turn â€” matching the CLI/gateway behaviour for inbound images.

The dispatcher spawns the worker as
``zed -p <profile> chat -q "work kanban task <id>"``. The task body
itself never appears in argv; the worker has to read it from the kanban
DB during startup. These tests cover the round-trip:

  task body  â†’  kanban_db.get_task  â†’  extract_image_refs  â†’
  build_native_content_parts  â†’  multimodal user turn
"""
...
# Tiny 1Ã—1 transparent PNG used to back any path the tests stick into a
...
def kanban_home(tmp_path: Path, monkeypatch):
    """Isolated ZED_HOME with a fresh kanban DB for each test."""
    home = tmp_path / ".zed"
    home.mkdir()
...
class TestExtractFromTaskBody:
    ...
    def test_url_in_body_round_trips(self, kanban_home):
        tid = _add_task_with_body(
            "The design lives at https://example.com/mock/v3.png â€” "
...
class TestBuildPartsFromTaskBody:
    ...
    def test_local_path_becomes_native_image_part(self, kanban_home, tmp_path):
        ...
        img.write_bytes(_PNG)
        tid = _add_task_with_body(f"Check out {img} â€” what's broken?")
        body = _read_body(tid)
        paths, urls = extract_image_refs(body)
    ...
    def test_url_becomes_image_url_part(self, kanban_home):
        tid = _add_task_with_body(
            "Reference: https://example.com/target.jpg â€” match it."
        )
        body = _read_body(tid)
        paths, urls = extract_image_refs(body)
    ...
    def test_body_with_both_yields_two_image_parts(self, kanban_home, tmp_path):
        ...
        img.write_bytes(_PNG)
        tid = _add_task_with_body(
            f"Diff {img} vs https://example.com/target.png â€” explain it."
        )
        body = _read_body(tid)
        paths, urls = extract_image_refs(body)
    ...
    def test_body_with_no_images_leaves_query_untouched(self, kanban_home):
        ...
        # No images â†’ plain text-only return (single part, no list mutation).
        assert skipped == []
        assert len(parts) == 1
        assert parts[0]["type"] == "text"
