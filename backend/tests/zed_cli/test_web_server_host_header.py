"""Tests for GHSA-ppp5-vxwm-4cf7 â€” Host-header validation.
...
that TTL-flips to 127.0.0.1. Same-origin / CORS checks won't help â€”
...
class TestHostHeaderValidator:
    """Unit test the _is_accepted_host helper directly â€” cheaper and
    more thorough than spinning up the full FastAPI app."""

    def test_loopback_bind_accepts_loopback_names(self):
        from zed_cli.web_server import _is_accepted_host
    ...
    def test_zero_zero_bind_accepts_anything(self):
        ...
        (requires --insecure). No Host-layer defence is possible â€” rely
    ...
    def test_explicit_non_loopback_bind_requires_exact_match(self):
        ...
        # Different host â€” reject
        assert not _is_accepted_host("evil.example", "my-server.corp.net")
        # Loopback â€” reject (we bound to a specific non-loopback name)
        assert not _is_accepted_host("localhost", "my-server.corp.net")

    def test_case_insensitive_comparison(self):
        """Host headers are case-insensitive per RFC â€” accept variations."""
        from zed_cli.web_server import _is_accepted_host
...
class TestHostHeaderMiddleware:
    """End-to-end test via the FastAPI app â€” verify the middleware
    ...
    def test_rebinding_request_rejected(self):
        ...
        try:
            ...
            # The TestClient sends Host: testserver by default â€” which is
    ...
    def test_legit_loopback_request_accepted(self):
        ...
        try:
            client = TestClient(app)
            # /api/status is in _PUBLIC_API_PATHS â€” passes auth â€” so the
            # only thing that can reject is the host header middleware
            resp = client.get(
                "/api/status",
                headers={"Host": "localhost:9119"},
            )
            # Either 200 (endpoint served) or some other non-400 â€”
