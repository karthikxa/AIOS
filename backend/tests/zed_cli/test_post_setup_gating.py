...
empty `env_vars` â€” causing `zed tools` to write the toolset to config
and exit `âœ“ Saved` without ever invoking the post_setup install. These
...
class TestPostSetupGate:
    ...
    def test_cua_driver_installed_skips_setup(self, monkeypatch, tmp_path):
        ...
        so a re-save through `zed tools` doesn't re-prompt the user."""
        from zed_cli import tools_config
