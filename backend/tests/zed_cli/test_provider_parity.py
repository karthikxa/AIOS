...
the SAME provider universe as ``zed model`` (the CLI/TUI picker).

This is the single load-bearing invariant of the unified provider catalog:

    keys(/api/env provider rows) âˆª ids(/api/providers/oauth) âŠ‡ CANONICAL_PROVIDERS
...
def test_every_zed_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys âˆª accounts) âŠ‡ `zed model` universe."""
    ...
    assert not missing, (
        "providers shown in `zed model` but not configurable in the desktop "
...
def test_no_provider_appears_on_both_tabs():
    """A provider should be configured exactly one way â€” not duplicated across
