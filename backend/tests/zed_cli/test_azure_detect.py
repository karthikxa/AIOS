"""Tests for zed_cli.azure_detect â€” transport & model auto-detection."""
...
def test_detect_anthropic_path_wins_without_http():
    """URL path sniff short-circuits â€” no HTTP call happens."""
...
def test_detect_openai_models_probe_success():
    """/models probe returning a model list â†’ chat_completions."""
...
def test_detect_openai_models_probe_empty_list_still_counts():
    """Endpoint returned OpenAI shape but no models â†’ still chat_completions."""
    def _fake_get(url, api_key, timeout=6.0, **kwargs):
...
def test_detect_all_probes_fail_returns_none():
    """Every probe fails â†’ api_mode is None and caller falls back to manual."""
...
def test_http_get_json_on_urlerror_returns_zero_none():
    ...
    import urllib.error
    with patch("zed_cli.azure_detect.urllib_request.urlopen",
...
def test_http_get_json_on_http_error_returns_code_none():
    ...
    with patch("zed_cli.azure_detect.urllib_request.urlopen", side_effect=err):
