"""Tests for ``_prompt_api_key`` â€” the shared Keep/Replace/Clear menu used by
``zed setup`` / ``zed model`` when an API key already exists in ``.env``.
...
def test_keep_on_unrecognised_input(profile_env):
    """Garbage input falls through to keep â€” never destroys the user's key."""
