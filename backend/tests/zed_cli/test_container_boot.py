...
These tests run against a fake $ZED_HOME under tmp_path; no real
...
def _make_profile(
    ...
    *,
    ...
    """Create a fake profile directory under zed_home/profiles/<name>/."""
    p = zed_home / "profiles" / name
    p.mkdir(parents=True)
    if config:
        # SOUL.md is what the reconciler keys on â€” it's always seeded by
        # `zed profile create`. See container_boot._render_run_script.
...
def _seed_default_root(
    ...
    *,
    ...
    ZED_HOME root (the implicit default profile)."""
    if state is not None:
        (zed_home / "gateway_state.json").write_text(json.dumps({
            "gateway_state": state, "timestamp": 1234567890,
        }))
    if with_pid:
        (zed_home / "gateway.pid").write_text(json.dumps(
            {"pid": 99999, "host": "old-container"},
        ))
        (zed_home / "processes.json").write_text("[]")
...
def test_running_profile_is_registered_and_autostarted(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        zed_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_directory_without_marker_file_is_skipped(tmp_path: Path) -> None:
    ...
    SOUL.md â€” the marker the reconciler keys on) should be skipped."""
    scandir = tmp_path / "run-service"; scandir.mkdir()
...
def test_corrupt_state_file_treated_as_no_prior_state(tmp_path: Path) -> None:
    ...
    reconciliation â€” register the slot in the down state."""
    scandir = tmp_path / "run-service"; scandir.mkdir()
...
def test_dry_run_makes_no_filesystem_changes(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        zed_home=tmp_path, scandir=scandir, dry_run=True,
    )
...
def test_missing_profiles_root_still_registers_default_slot(
    ...
    """When $ZED_HOME/profiles doesn't exist (fresh install), the
    ...
    so `zed gateway start` (no -p) has somewhere to land."""
    scandir = tmp_path / "run-service"; scandir.mkdir()
    actions = reconcile_profile_gateways(
        zed_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_register_service_publishes_atomically(tmp_path: Path) -> None:
    ...
    tmp directory and rename it into place â€” never leaving a half-
...
def test_register_service_overwrites_existing_slot(tmp_path: Path) -> None:
    ...
    # Down marker now present (state went from running â†’ stopped).
...
# ---------------------------------------------------------------------------
# Default-profile slot â€” always registered (PR #30136 review item I1)
# ---------------------------------------------------------------------------


def test_default_slot_always_registered_on_empty_home(tmp_path: Path) -> None:
    """Bare ZED_HOME with nothing under it still produces a
...
def test_default_slot_run_script_omits_profile_flag(tmp_path: Path) -> None:
    """The default slot's run script must NOT pass `-p default` â€”
    that would resolve to $ZED_HOME/profiles/default/ instead of
    the root profile. It must call `zed gateway run` directly."""
    scandir = tmp_path / "run-service"; scandir.mkdir()
...
@pytest.mark.parametrize(
    ...
    [
        ("gateway", "run"),
        ("/init", "/opt/zed/docker/main-wrapper.sh", "gateway", "run"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("/init", "/opt/zed/docker/main-wrapper.sh", "gateway", "run", "--no-supervise"),
    ],
)
...
def test_legacy_gateway_run_env_no_supervise_does_not_seed_s6_state(
    ...
    scandir = tmp_path / "run-service"; scandir.mkdir()
    monkeypatch.setenv("ZED_GATEWAY_NO_SUPERVISE", "1")
...
def test_default_slot_cleans_up_stale_runtime_files_at_root(
    ...
    """gateway.pid and processes.json at the ZED_HOME root (left
...
def test_profiles_default_subdir_is_skipped_with_warning(
    ...
    root-profile slot â€” the named entry is skipped (with a warning)
    ...
    # Only the root-profile default slot appears â€” not the colliding
    # named profile.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Through s6 /init + the main-wrapper that re-execs `zed`.
        ("/init", "/opt/zed/docker/main-wrapper.sh", "dashboard"),
        (
            "/init",
            "/opt/zed/docker/main-wrapper.sh",
            "dashboard",
            "--host",
            "127.0.0.1",
            "--no-open",
        ),
        # Wrapper that kept the explicit `zed` argv0.
        ("/init", "/opt/zed/docker/main-wrapper.sh", "zed", "dashboard"),
    ],
)
...
@pytest.mark.parametrize(
    "container_argv",
    [
        (),  # empty (/proc/1/cmdline unreadable) â€” not the dashboard
        ("gateway", "run"),
        ("/init", "/opt/zed/docker/main-wrapper.sh", "gateway", "run"),
        ("/init", "/opt/zed/docker/main-wrapper.sh", "zed", "gateway", "run"),
        ("chat",),
        # A profile literally named "dashboard" must NOT match â€” the token
...
def test_main_skips_reconcile_in_dashboard_container(
    ...
    monkeypatch.setattr(
        ...
        lambda: ("/init", "/opt/zed/docker/main-wrapper.sh", "dashboard"),
    )
...
def test_main_reconciles_in_gateway_container(
    ...
    monkeypatch.setattr(
        container_boot,
        "_read_container_argv",
        lambda: ("/init", "/opt/zed/docker/main-wrapper.sh", "gateway", "run"),
    )
...
def test_main_ignores_removed_skip_reconcile_env_var(
    ...
    monkeypatch.setenv("ZED_HOME", str(tmp_path))
    monkeypatch.setenv("S6_PROFILE_GATEWAY_SCANDIR", str(scandir))
    monkeypatch.setenv("ZED_SKIP_PROFILE_RECONCILE", "1")
    monkeypatch.setattr(
        container_boot,
        "_read_container_argv",
        lambda: ("/init", "/opt/zed/docker/main-wrapper.sh", "gateway", "run"),
