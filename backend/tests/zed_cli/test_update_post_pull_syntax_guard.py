"""Tests for the post-pull syntax guard in ``zed update``.

When a bad commit lands on ``main`` with a syntax error in a critical file
(e.g. orphan merge-conflict markers in ``zed_cli/config.py``), the CLI
becomes unbootable â€” every ``zed`` invocation imports those files at
...
markers in ``zed_cli/config.py``; users who ran ``zed update`` in
the 7-minute window before #28458 landed could not run any ``zed``
command afterward.
"""
...
def test_capture_head_sha_returns_stripped_sha(monkeypatch, tmp_path):
    ...
    assert zed_main._capture_head_sha(["git"], tmp_path) == "deadbeefcafe"
...
def test_capture_head_sha_returns_none_on_git_failure(monkeypatch, tmp_path):
    ...
    assert zed_main._capture_head_sha(["git"], tmp_path) is None
...
def _populate_critical_tree(root: Path, *, broken_file: str | None = None) -> None:
    ...
    broken_payload = (
        ...
        '    "b": 2,\n'
        "=======\n"
        '    "c": 0b6d673e7,\n'  # invalid binary literal â€” the actual error users saw
        ">>>>>>> 0b6d673e7\n"
        "}\n"
    )
...
def test_validate_critical_files_syntax_ok_when_all_files_parse(tmp_path):
    ...
    ok, failing_path, error = zed_main._validate_critical_files_syntax(tmp_path)
...
def test_validate_critical_files_syntax_detects_conflict_markers(tmp_path):
    ...
    _populate_critical_tree(tmp_path, broken_file="zed_cli/config.py")

    ok, failing_path, error = zed_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("zed_cli/config.py")
    assert error is not None
    # The error mentions either the syntax error itself or the file path â€”
...
def test_validate_critical_files_syntax_detects_break_in_main_py(tmp_path):
    _populate_critical_tree(tmp_path, broken_file="zed_cli/main.py")

    ok, failing_path, _ = zed_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("zed_cli/main.py")
...
# ---------------------------------------------------------------------------
# Repo invariant â€” the production tree itself must always pass the guard.
# This catches the case where ``main`` ships a syntax error before the next
# release; if a future ``zed update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
...
def test_production_tree_passes_syntax_guard():
    ...
    ok, failing_path, error = zed_main._validate_critical_files_syntax(repo_root)
