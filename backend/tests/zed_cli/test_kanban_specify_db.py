"""Tests for kb.specify_triage_task â€” the DB-layer atomic promotion
...
def test_specify_promotes_triage_to_todo(kanban_home):
    ...
    # No parents â†’ recompute_ready should have flipped it past todo to ready.
...
def test_specify_with_open_parent_lands_in_todo_not_ready(kanban_home):
    # Parent-gated specified tasks must not jump the dispatcher â€” they go
    ...
    # Parent still open â†’ specified child sits in 'todo', not 'ready'.
...
def test_specify_records_audit_comment_only_when_author_given(kanban_home):
    ...
    # Without author â†’ no comment (silent).
    with kb.connect() as conn:
