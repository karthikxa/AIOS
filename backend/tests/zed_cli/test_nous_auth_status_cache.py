...
The cache avoids re-validating Nous credentials on every menu paint â€”
`zed tools` â†’ "All Platforms" used to fire ~31 OAuth refresh POSTs
against portal.zedteam.com during one render. The cache is keyed
...
def _seed_auth_file(tmp_path):
    """Drop a placeholder auth.json into the test ZED_HOME.

    The exact content doesn't matter for cache-key purposes â€” only that
...
def test_get_nous_auth_status_caches_consecutive_calls(tmp_path, monkeypatch):
    ...
    assert call_count["n"] == 1, (
        f"_compute_nous_auth_status was called {call_count['n']}Ã— â€” "
...
def test_get_nous_auth_status_caches_failure_path(tmp_path, monkeypatch):
    """Logged-out snapshots are cached too â€” that's where the cost was.

    Teknium's case: ~31 cache misses per `zed tools` "All Platforms"
