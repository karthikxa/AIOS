"""Regression tests for #34107 â€” Docker UID/GID handling in ensure_zed_home.

When Zed runs in Docker with ``ZED_UID=1000`` / ``ZED_GID=911``,
the entrypoint chowns the top-level ``ZED_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_zed_home()`` â€” especially
for profile namespaces under ``profiles/<name>/`` spawned by kanban
workers â€” were landing as ``root:root`` and blocking subsequent
uid-mapped worker invocations with ``PermissionError [Errno 13]``.

The fix is a ``_chown_to_zed_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_zed_uid_gid
# ---------------------------------------------------------------------------


class TestResolveZedUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("ZED_UID", "1000")
        monkeypatch.setenv("ZED_GID", "911")
        from zed_cli.config import _resolve_zed_uid_gid
        uid, gid = _resolve_zed_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("ZED_UID", raising=False)
        monkeypatch.delenv("ZED_GID", raising=False)
        from zed_cli.config import _resolve_zed_uid_gid
        uid, gid = _resolve_zed_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("ZED_UID", "not-a-number")
        monkeypatch.setenv("ZED_GID", "911")
        from zed_cli.config import _resolve_zed_uid_gid
        uid, gid = _resolve_zed_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("ZED_UID", "")
        monkeypatch.setenv("ZED_GID", "")
        from zed_cli.config import _resolve_zed_uid_gid
        uid, gid = _resolve_zed_uid_gid()
        assert uid is None
        assert gid is None
...
class TestChownToZedUid:
    ...
    def test_eperm_is_silently_swallowed(self, tmp_path, monkeypatch):
        """When running as non-root, os.chown raises EPERM. That's fine â€”
        ...
        with patch.object(cfg.os, "chown", side_effect=_raises_eperm):
            # Must not raise â€” the catch is non-fatal.
            cfg._chown_to_zed_uid(d)
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_zed_uid(d)  # must not raise
