...
def profile_home(tmp_path, monkeypatch):
    """Redirect $ZED_HOME and ~ to a tmp dir for the duration of the test."""
    home = tmp_path / ".zed"
    home.mkdir()
    monkeypatch.setenv("ZED_HOME", str(home))
    # Some code paths fall back to Path.home() â€” patch that too.
...
def test_audit_write_failure_does_not_raise(monkeypatch, tmp_path):
    ...
    # Point ZED_HOME at a file (not a dir) so mkdir/open will fail.
    broken = tmp_path / "not-a-dir"
    broken.write_text("blocking file")
    monkeypatch.setenv("ZED_HOME", str(broken))
    # Should NOT raise.
