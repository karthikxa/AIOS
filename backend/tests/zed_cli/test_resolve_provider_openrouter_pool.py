...
A credential added via `zed auth add openrouter` lives in the credential
...
requests went out with no Authorization header â€” OpenRouter's
...
def _seed_openrouter_pool(token: str = "sk-or-FAKEKEY123") -> None:
    """Mimic `zed auth add openrouter <token>` â€” a manual pool entry."""
...
def test_no_credentials_still_raises(tmp_path, monkeypatch):
    """Empty pool + no env var must still fail to resolve â€” no false positive."""
    monkeypatch.setenv("ZED_HOME", str(tmp_path / "zed"))
    (tmp_path / "zed").mkdir(parents=True, exist_ok=True)
