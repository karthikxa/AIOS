...
requests hit ``https://opencode.ai/zen/go/v1/v1/messages`` â€” a double
``/v1`` that returns OpenCode's website 404 page with HTML body.

``zed_cli.runtime_provider.resolve_runtime_provider`` already strips
``/v1`` at fresh agent init (PR #4918), but the ``/model`` mid-session
switch path in ``zed_cli.model_switch.switch_model`` was missing the
same logic â€” these tests guard against that regression.
"""
...
def _run_opencode_switch(
    ...
    with (
        patch("zed_cli.model_switch.resolve_alias", return_value=None),
...
class TestOpenCodeGoV1Strip:
    ...
    def test_switch_to_minimax_m27_strips_v1(self):
        """GLM-5 â†’ MiniMax-M2.7: base_url loses trailing /v1."""
...
class TestOpenCodeZenV1Strip:
    ...
    def test_switch_to_claude_sonnet_strips_v1(self):
        """Gemini â†’ Claude on opencode-zen: /v1 stripped."""
        result = _run_opencode_switch(
    ...
    def test_switch_to_gpt_uses_codex_responses_keeps_v1(self):
        """GPT on opencode-zen uses codex_responses api_mode â€” /v1 kept."""
        result = _run_opencode_switch(
            raw_input="gpt-5.4",
            current_provider="opencode-zen",
            current_model="claude-sonnet-4-6",
...
class TestAgentSwitchModelDefenseInDepth:
    ...
    def test_agent_switch_model_strips_v1_for_anthropic_messages(self):
        ...
        # Intercept the expensive client rebuild â€” we only need to verify
        ...
        def _fake_build_anthropic_client(api_key, base_url, **kwargs):
            ...
            return object()  # placeholder client â€” no real calls expected
...
class TestStaleConfigDefaultDoesNotWedgeResolver:
    ...
    def test_kimi_switch_keeps_v1_despite_claude_config_default(self, tmp_path, monkeypatch):
        ...
        # Re-import with the new ZED_HOME so config cache is fresh.
        import zed_cli.config as _cfg_mod
        importlib.reload(_cfg_mod)
        import zed_cli.runtime_provider as _rp_mod
        importlib.reload(_rp_mod)
        import zed_cli.model_switch as _ms_mod
