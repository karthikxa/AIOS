...
ZED_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot") while
_PROVIDER_MODELS and config.yaml use Zed IDs ("copilot").  The slug
...
def test_copilot_uses_zed_slug():
...
def test_copilot_no_duplicate_entries():
    """Copilot must appear only once â€” not as both 'copilot' (section 1) and 'github-copilot' (section 2)."""
    providers = list_authenticated_providers(current_provider="copilot")
...
def test_kimi_for_coding_overlay_uses_zed_slug():
...
def test_kilo_overlay_uses_zed_slug():
