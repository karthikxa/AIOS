"""Regression coverage for GHSA-5qr3-c538-wm9j (#29156) â€” Remote Code
Execution via the ``ZED_ENABLE_PROJECT_PLUGINS`` bypass in the web
server's dashboard plugin loader.

Two primitives combined into the original advisory chain:

1. ``zed_cli.web_server._discover_dashboard_plugins`` opted into
   the untrusted ``./.zed/plugins/`` source via
   ``os.environ.get("ZED_ENABLE_PROJECT_PLUGINS")`` â€” truthy for
...
2. ``zed_cli.web_server._mount_plugin_api_routes`` then imported
...
def _reset_plugin_cache(monkeypatch):
    ...
    mask a regression â€” and so the production cache the import-time
...
# ---------------------------------------------------------------------------
# Layer 1 â€” ZED_ENABLE_PROJECT_PLUGINS env gate uses truthy semantics.
# ---------------------------------------------------------------------------


class TestProjectPluginsEnvGate:
    """Project plugins must only be discovered when the env var is set
    to a documented truthy value.  Pre-#29156 any non-empty string â€”
    including ``0`` / ``false`` / ``no`` â€” silently enabled the source."""

    @pytest.fixture
    def project_plugin(self, tmp_path, monkeypatch):
        """Plant a project-source plugin under CWD's ``.zed/plugins``
        and isolate the user-plugins dir to an empty tmp tree."""
        monkeypatch.setenv("ZED_HOME", str(tmp_path / "home"))
        (tmp_path / "home").mkdir()
    ...
    def test_falsy_values_keep_project_plugins_disabled(
        ...
        if value == "":
            monkeypatch.delenv("ZED_ENABLE_PROJECT_PLUGINS", raising=False)
        else:
            monkeypatch.setenv("ZED_ENABLE_PROJECT_PLUGINS", value)

        plugins = web_server._get_dashboard_plugins(force_rescan=True)
        names = {p["name"] for p in plugins}
        assert "evil" not in names, (
            f"ZED_ENABLE_PROJECT_PLUGINS={value!r} must NOT enable the "
            "project source â€” that's the GHSA-5qr3-c538-wm9j env bypass."
        )
...
# ---------------------------------------------------------------------------
# Layer 2 â€” _safe_plugin_api_relpath rejects path-traversal payloads.
# ---------------------------------------------------------------------------
...
class TestApiPathSanitizer:
    ...
    @pytest.mark.parametrize("payload", [
        ...
        "/tmp/payload.py",
        "/usr/bin/python",
        # NT-style absolute on POSIX is a relative path â€” covered by traversal below.
    ])
...
# ---------------------------------------------------------------------------
# Layer 3 â€” _discover_dashboard_plugins scrubs ``_api_file`` early.
# ---------------------------------------------------------------------------
...
class TestDiscoveryScrubsApiField:
    ...
    def test_safe_api_path_survives(self, user_plugin_factory, tmp_path):
        ...
        # in principle proceed â€” we're only testing the discovery scrub.
...
# ---------------------------------------------------------------------------
# Layer 4 â€” _mount_plugin_api_routes refuses project-source + traversal.
# ---------------------------------------------------------------------------


class TestMountApiRoutesRefusesUntrusted:
    """The mount routine is the actual ``importlib`` call site â€” these
    ...
    def test_project_source_api_is_not_imported(self, tmp_path):
        ...
        assert spec.call_count == 0, (
            "project-source plugin's api file was imported â€” "
...
# ---------------------------------------------------------------------------
# Layer 5 â€” End-to-end: the original PoC manifest no longer triggers RCE.
# ---------------------------------------------------------------------------
...
class TestEndToEndPocBlocked:
    ...
    ``ZED_ENABLE_PROJECT_PLUGINS=0`` (so the operator believed the
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZED_ENABLE_PROJECT_PLUGINS", "0")
