"""Tests for the Phase 4 s6 dispatch helper in zed_cli.gateway.

`_dispatch_via_service_manager_if_s6` decides whether a
`zed gateway start/stop/restart` invocation should be routed to
...
def test_dispatch_returns_false_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    return False and not invoke a manager â€” callers continue with
    their existing systemd/launchd/windows path."""
    from zed_cli import gateway as gw
    monkeypatch.setattr(
        "zed_cli.service_manager.detect_service_manager", lambda: "systemd",
    )
    # Should not even attempt to construct a manager.
    monkeypatch.setattr(
        "zed_cli.service_manager.get_service_manager",
...
def test_dispatch_returns_true_and_calls_start_on_s6(
    ...
    monkeypatch.setattr(
        "zed_cli.service_manager.get_service_manager", lambda: rec,
    )
...
def test_dispatch_unknown_action_returns_false(
    ...
    succeed â€” return False so the host code path handles it."""
    from zed_cli import gateway as gw
    rec = _CallRecorder()
    monkeypatch.setattr(
        "zed_cli.service_manager.detect_service_manager", lambda: "s6",
...
def test_dispatch_defaults_profile_to_default(
    ...
    monkeypatch.setattr(
        "zed_cli.gateway._profile_suffix", lambda: "",
    )
...
# ---------------------------------------------------------------------------
# _dispatch_all_via_service_manager_if_s6 â€” --all under s6
# ---------------------------------------------------------------------------
...
def test_dispatch_all_empty_list_reports_and_returns_true(
    ...
    dispatch (returns True) and prints a friendly message â€” the host
...
# ---------------------------------------------------------------------------
# Friendly error rendering â€” GatewayNotRegisteredError / S6CommandError
...
def test_dispatch_renders_gateway_not_registered_friendly(
    ...
    """`zed -p typo gateway start` should print a clear message and
    exit 1 â€” not dump a traceback at the user."""
    from zed_cli import gateway as gw
    from zed_cli.service_manager import GatewayNotRegisteredError
    ...
    monkeypatch.setattr(
        "zed_cli.service_manager.get_service_manager", lambda: _RaisesMissing(),
    )
    ...
    assert "zed profile create typo" in out
...
def test_dispatch_renders_s6_command_error_friendly(
    ...
    monkeypatch.setattr(
        "zed_cli.service_manager.get_service_manager", lambda: _RaisesS6Error(),
    )
...
def _stub_s6(monkeypatch: pytest.MonkeyPatch, *, on_s6: bool) -> _CallRecorder:
    ...
    rec = _CallRecorder()
    monkeypatch.setattr(
        "zed_cli.service_manager.detect_service_manager",
...
def test_redirect_noop_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # If execvp got called we'd raise â€” keep it bound so test fails loudly.
    monkeypatch.setattr(
        "zed_cli.gateway.os.execvp",
        lambda *a, **kw: pytest.fail("execvp should not be called on host"),
    )
    monkeypatch.delenv("ZED_S6_SUPERVISED_CHILD", raising=False)
    monkeypatch.delenv("ZED_GATEWAY_NO_SUPERVISE", raising=False)
...
def test_redirect_fires_inside_s6_container(
    ...
    monkeypatch.setattr("zed_cli.gateway._profile_suffix", lambda: "")
    ...
    monkeypatch.setattr("zed_cli.gateway.os.execvp", fake_execvp)
    # If the fallback ran, the normal sleep path was wrongly skipped.
    monkeypatch.setattr(
        "zed_cli.gateway._block_until_terminated",
    ...
    assert "ZED_GATEWAY_NO_SUPERVISE" in err
...
def test_redirect_falls_back_when_sleep_missing(
    ...
    without it), the redirect must NOT crash the container â€” it falls
    ...
    monkeypatch.setattr("zed_cli.gateway.os.execvp", missing_sleep)
    ...
    # Must not raise FileNotFoundError â€” that was the #36208 crash.
...
def test_block_until_terminated_installs_sigterm_handler_and_blocks(
    ...
    `docker stop` exits cleanly) and then block on signal.pause() â€” never
    ...
    monkeypatch.setattr("zed_cli.gateway.signal.pause", fake_pause)
...
def test_redirect_short_circuits_supervised_child(
    ...
    running execs `zed gateway run --replace`, the
    ZED_S6_SUPERVISED_CHILD sentinel must short-circuit the redirect
    so the gateway actually starts foreground. Without this guard the
    supervised process would re-dispatch `start` â†’ re-exec `run` â†’ ...
    in an infinite loop.
    """
    from zed_cli import gateway as gw
...
def test_redirect_respects_no_supervise_env(
    ...
    """`ZED_GATEWAY_NO_SUPERVISE=1` (env var) must skip the redirect.

    Truthiness mirrors the dashboard service's own env var parsing â€”
    ...
    monkeypatch.delenv("ZED_S6_SUPERVISED_CHILD", raising=False)
    monkeypatch.setenv("ZED_GATEWAY_NO_SUPERVISE", value)
...
def test_redirect_no_supervise_env_falsy_values_dont_opt_out(
    ...
    """Falsy / unrecognized values of ZED_GATEWAY_NO_SUPERVISE must
    NOT opt out. We're strict about what counts as "yes" so a typo
    like `ZED_GATEWAY_NO_SUPERVISE=0` doesn't silently enable the
    ...
    for falsy in ("", "0", "false", "no", "off", "garbage"):
        execvp_calls.clear()
        monkeypatch.setenv("ZED_GATEWAY_NO_SUPERVISE", falsy)
