"""Tests for ``zed debug`` CLI command and debug utilities."""
...
def zed_home(tmp_path, monkeypatch):
    """Set up an isolated ZED_HOME with minimal logs."""
    home = tmp_path / ".zed"
    home.mkdir()
...
class TestCaptureLogSnapshot:
    ...
    def test_empty_primary_reports_file_empty(self, zed_home):
    ...
    def test_truncates_large_file(self, zed_home):
        ...
        big_content = "x" * 100 + "\n"
        (zed_home / "logs" / "agent.log").write_text(big_content * 200)
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, zed_home):
        ...
        (zed_home / "logs" / "agent.log").write_text(line * num_lines)

        # max_bytes = 1000 = 100 * 10 â†’ cut at byte 20000 - 1000 = 19000,
        # and byte 19000 - 1 is '\n'.  Boundary hit â†’ keep all 10 lines.
    ...
    def test_drops_partial_when_truncation_mid_line(self, zed_home):
        ...
        # max_bytes = 950 doesn't divide evenly into 100 â†’ mid-line cut.
        snap = _capture_log_snapshot("agent", tail_lines=5, max_bytes=950)
        assert snap.full_text is not None
        assert "truncated" in snap.full_text
        raw = snap.full_text.split("\n", 1)[1]
        kept = [l for l in raw.strip().splitlines() if l.startswith("A")]
        # 950 / 100 = 9.5 â†’ 9 complete lines after dropping partial
        assert len(kept) == 9
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of ZED_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def zed_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated ZED_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".zed"
        home.mkdir()
        monkeypatch.setenv("ZED_HOME", str(home))
    ...
    def test_force_true_works_when_redaction_disabled(
        self, zed_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when ZED_REDACT_SECRETS=false, and the share-time
...
# ---------------------------------------------------------------------------
# CLI entry point â€” run_debug_share
# ---------------------------------------------------------------------------
...
class TestScheduleAutoDelete:
    ...
    and accumulated forever under repeated use â€” 15+ orphaned interpreters
    were observed in production.

    The new implementation is stateless: it records pending deletions to
    ``~/.zed/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``zed debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, zed_home):
        ...
        assert "Popen" not in code_only, (
            "_schedule_auto_delete must not spawn subprocesses â€” "
        ...
        if before is not None:
            ...
            # It's fine if unrelated processes appeared â€” we just need to make
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_survives_network_failure(self, zed_home):
        ...
        # Failure within 24h grace â†’ kept for retry
        assert deleted == 0
        assert remaining == 1
    ...
    def test_sweep_drops_entries_past_grace_window(self, zed_home):
        ...
        # Expired 25 hours ago â†’ past the 24h grace window
...
# ---------------------------------------------------------------------------
# build_debug_share â€” structured core used by the dashboard endpoint
# ---------------------------------------------------------------------------
...
class TestBuildDebugShare:
    ...
    Backs both ``zed debug share`` (CLI) and ``POST /api/ops/debug-share``
