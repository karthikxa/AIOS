...
def test_runtime_health_lines_include_fatal_platform_and_startup_reason(monkeypatch):
    ...
    assert "âš  telegram: another poller is active" in lines
    assert "âš  Last startup issue: telegram conflict" in lines
...
def test_runtime_status_running_pid_validates_live_gateway_record(monkeypatch):
    ...
    runtime = {
        "pid": 12345,
        "kind": "zed-gateway",
        "argv": ["/opt/zed/zed_cli/main.py", "gateway", "run", "--replace"],
        "start_time": None,
