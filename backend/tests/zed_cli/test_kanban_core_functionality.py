...
Complements tests/zed_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/zed_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def kanban_home(tmp_path, monkeypatch):
    ...
    # tests in tests/zed_cli/test_kanban_db.py.
    monkeypatch.setenv("ZED_KANBAN_CRASH_GRACE_SECONDS", "0")
    kb.init_db()
    return home
...
def test_idempotency_key_returns_existing_task(kanban_home):
    ...
    try:
        ...
        # And body wasn't overwritten â€” first create wins.
...
def test_spawn_failure_auto_blocks_after_limit(kanban_home, all_assignees_spawnable):
    """N consecutive spawn failures on the same task â†’ auto_blocked."""
    ...
    try:
        ...
        # One default-limit failure â†’ still ready, counter grows.
...
def test_successful_spawn_does_not_reset_failure_counter(kanban_home, all_assignees_spawnable):
    ...
    does NOT reset the counter â€” past failures stay on the books until
    ...
    def _flaky_spawn(task, ws):
        ...
        return 99999  # pid value â€” harmless; crash detection will clear it
    ...
    try:
        ...
        kb.dispatch_once(conn, spawn_fn=_flaky_spawn, failure_limit=5)
        task = kb.get_task(conn, tid)
        # Counter STAYS at 2 â€” spawn succeeded but run isn't complete yet.
        assert task.consecutive_failures == 2
...
def test_successful_completion_resets_failure_counter(kanban_home, all_assignees_spawnable):
    """A successful kb.complete_task wipes the counter â€” the task+profile
...
def test_per_task_max_retries_overrides_dispatcher_limit(kanban_home, all_assignees_spawnable):
    ...
    Three-tier resolution order:
      1. ``task.max_retries`` (set via ``create_task(max_retries=N)`` /
         ``zed kanban create --max-retries N``)
...
def test_per_task_max_retries_allows_more_than_default(kanban_home, all_assignees_spawnable):
    ...
    limit of 2 â€” it must reach the per-task override first."""
    conn = kb.connect()
    try:
        tid = kb.create_task(
            conn, title="flaky-retry", assignee="worker", max_retries=5,
        )
        # Four failures â€” still below the per-task threshold, should stay ready.
        for i in range(1, 5):
            kb.claim_task(conn, tid)
            tripped = kb._record_task_failure(
                conn, tid,
...
def test_max_retries_none_falls_through_to_dispatcher_limit(kanban_home, all_assignees_spawnable):
    ...
    supplied ``failure_limit`` â€” the gateway config tier."""
    conn = kb.connect()
    try:
...
def test_workspace_resolution_failure_also_counts(kanban_home, all_assignees_spawnable):
    ...
    count against the failure budget â€” not just crash the tick."""
    conn = kb.connect()
    try:
...
def test_notify_cursor_advances(kanban_home):
    ...
    try:
        ...
        assert events == []
        # Complete the task â†’ new `completed` event.
        ...
        # Advance cursor â€” next call returns empty.
...
def test_cli_log_missing_task(kanban_home):
    # No such task â†’ exit-style (no log for...) message on stderr, returned
    # in combined output.
...
# ---------------------------------------------------------------------------
# run_slash parity â€” every verb returns a sensible, non-crashy string
# ---------------------------------------------------------------------------
...
def test_enforce_max_runtime_integrates_with_dispatch(kanban_home, monkeypatch):
    """enforce_max_runtime + dispatch_once integrate cleanly â€” a timed-out
    task goes through ``timed_out`` â†’ ``ready`` and dispatch_once can then
    ...
    try:
        ...
        # Use enforce_max_runtime directly with our signal stub â€” dispatch_once
...
def test_migration_renames_legacy_event_kinds(tmp_path, monkeypatch):
    ...
    try:
        ...
        # Re-run init_db â€” the migration pass should rename them.
        kb.init_db()
        rows = conn.execute(
...
def test_list_profiles_on_disk(tmp_path, monkeypatch):
    ...
    named profiles under ~/.zed/profiles/ that contain a config.yaml."""
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.delenv("ZED_HOME", raising=False)
    profiles = tmp_path / ".zed" / "profiles"
...
def test_list_profiles_on_disk_custom_root(tmp_path, monkeypatch):
    """list_profiles_on_disk respects a custom ZED_HOME root."""
...
def test_parse_duration_accepts_formats():
    from zed_cli.kanban import _parse_duration
...
def test_cli_create_max_runtime_via_duration(kanban_home):
    """`zed kanban create --max-runtime 2h` should persist 7200 seconds."""
...
def test_run_on_spawn_failure_records_failed_runs(kanban_home, all_assignees_spawnable):
    ...
    try:
        ...
        runs = kb.list_runs(conn, tid)
        # 5 claim attempts â†’ 5 runs. Final one is gave_up, earlier ones
        # are spawn_failed.
        assert len(runs) == 5
...
def test_event_rows_carry_run_id(kanban_home):
    ...
    try:
        tid = kb.create_task(conn, title="x", assignee="worker")
        # task-scoped: 'created' â€” no run yet
...
def test_migration_backfills_inflight_run_for_legacy_db(kanban_home):
    ...
    try:
        ...
        # Re-run init_db â€” migration backfill should kick in.
        kb.init_db()
        conn2 = kb.connect()
        try:
...
def test_archive_of_ready_task_does_not_create_spurious_run(kanban_home):
    """No active run â†’ archive shouldn't synthesize one."""
    conn = kb.connect()
    try:
        tid = kb.create_task(conn, title="x", assignee="worker")
...
def test_cli_bulk_complete_with_summary_rejects(kanban_home):
    ...
    # Note: zed_cli.main doesn't propagate sub-command exit codes
    ...
    env = os.environ.copy()
    r = _run(
        [sys.executable, "-m", "zed_cli.main", "kanban",
...
def test_cli_bulk_complete_without_summary_still_works(kanban_home):
    """Bulk close with no per-task handoff is allowed â€” the common case."""
    conn = kb.connect()
    try:
        a = kb.create_task(conn, title="a", assignee="worker")
        b = kb.create_task(conn, title="b", assignee="worker")
...
def test_claim_task_recovers_from_invariant_leak(kanban_home):
    ...
    try:
        ...
        # Now re-claim â€” the defensive recovery must close the leak.
        claimed = kb.claim_task(conn, tid)
        assert claimed is not None
...
def test_cli_create_on_fresh_home_auto_inits(tmp_path, monkeypatch):
    """First CLI action on an empty ZED_HOME must not error with
    'no such table: tasks' â€” init_db auto-runs now."""
    home = tmp_path / ".zed"
    home.mkdir()
    monkeypatch.setenv("ZED_HOME", str(home))
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    env = {**os.environ, "ZED_HOME": str(home),
           "PYTHONPATH": str(worktree_root)}
    r = _sp.run(
        [_sys.executable, "-m", "zed_cli.main", "kanban",
...
def test_connect_auto_inits_fresh_db(tmp_path, monkeypatch):
    """Calling connect() on a fresh ZED_HOME must create the
    ...
    # Direct connect() without init_db() â€” used to raise "no such table".
    conn = kb.connect()
    try:
        tid = kb.create_task(conn, title="x")
...
def test_cli_show_json_carries_runs(kanban_home):
    """zed kanban show --json must include runs[] so scripts that
...
# -------------------------------------------------------------------------
# Pre-merge audit by @erosika (issue #16102 comment 4331125835) â€” fixes
# -------------------------------------------------------------------------
...
def test_unblock_invariant_recovery(kanban_home):
    ...
    try:
        ...
        # leave current_run_id pointing at the open run â€” simulate the
        ...
        # Unblock â€” the defensive recovery must close the leaked run.
...
def test_migration_backfill_idempotent_under_re_run(tmp_path, monkeypatch):
    ...
    try:
        ...
        # Re-run init_db 3x â€” each should detect the orphan-inflight and
...
def test_cli_show_clamps_negative_elapsed(kanban_home):
    ...
    try:
        ...
        kb.claim_task(conn, tid)
        # Force a future started_at via raw SQL â€” simulates NTP jump.
...
def test_resolve_workspace_rejects_relative_dir_path(kanban_home):
    ...
    CWD â€” a confused-deputy escape vector."""
    conn = kb.connect()
    try:
        tid = kb.create_task(
        ...
        task = kb.get_task(conn, tid)
        # Storage is verbatim â€” that's fine.
...
def test_build_worker_context_caps_prior_attempts(kanban_home):
    ...
    try:
        ...
        # Total size is bounded â€” empirically we expect << 100KB even
...
def test_build_worker_context_renders_author_with_safe_framing(kanban_home):
    ...
    + "comment from worker" prefix so a misleading ZED_PROFILE name
    (e.g. "zed-system", "operator") can't be misread as a system
    directive above the comment body. Defense-in-depth â€” see #22452."""
    conn = kb.connect()
    try:
        tid = kb.create_task(conn, title="t", assignee="worker")
        kb.add_comment(conn, tid, author="zed-system", body="some note")
        ctx = kb.build_worker_context(conn, tid)

        # No bold-author rendering anywhere in the context.
        assert "**zed-system**" not in ctx
        # Explicit provenance prefix is present.
        assert "comment from worker `zed-system` at " in ctx
...
def test_build_worker_context_caps_comments(kanban_home):
    """Same cap for comments â€” comment-storm tasks stay bounded."""
    conn = kb.connect()
    try:
...
def test_default_spawn_auto_loads_kanban_worker_skill(kanban_home, monkeypatch):
    ...
    zed subprocess (which would hang trying to call an LLM).
    """
    # Pretend the bundled kanban-worker skill resolves for this isolated
    # ZED_HOME â€” the fixture creates an empty tmpdir without the
    ...
    env = captured["env"]
    assert env.get("ZED_KANBAN_TASK") == tid
    assert env.get("ZED_PROFILE") == "some-profile"
...
def test_create_task_skills_rejects_comma_embedded(kanban_home):
    """Comma in a skill name is rejected â€” force caller to pass a list."""
    conn = kb.connect()
    try:
...
def test_cli_create_skill_flag_repeatable(kanban_home):
    """`zed kanban create --skill a --skill b` persists the list."""
    out = run_slash(
...
def test_cli_create_without_skill_flag_leaves_none(kanban_home):
    """No --skill on the CLI means Task.skills stays None (not []) â€”
...
def test_cli_show_renders_skills(kanban_home):
    """`zed kanban show <id>` prints a skills row when present."""
    out = run_slash(
...
def test_legacy_db_without_skills_column_migrates(tmp_path):
    ...
    # Run the migrator directly â€” the same function connect() calls.
...
def test_legacy_migration_no_legacy_columns_at_all(tmp_path):
    ...
    This is the exact crash scenario from issue #20842 â€” a very old DB that
...
def test_legacy_migration_both_columns_already_present(tmp_path):
    ...
    # Schema must be unchanged â€” no spurious ADD or DROP.
...
def test_check_dispatcher_presence_warns_when_no_gateway(monkeypatch):
    ...
    assert "zed gateway start" in msg
...
def test_cli_create_warns_when_no_gateway(kanban_home, monkeypatch, capsys):
    ...
    assert "zed gateway start" in captured.err
...
def test_cli_create_silent_when_gateway_up(kanban_home, monkeypatch, capsys):
    ...
    assert kb_cli._cmd_create(ns) == 0
    captured = capsys.readouterr()
    assert "zed gateway start" not in captured.err
...
def test_cli_create_no_warn_on_triage(kanban_home, monkeypatch, capsys):
    ...
    assert "zed gateway start" not in err
...
def test_cli_daemon_without_force_prints_deprecation_exits_2(kanban_home, capsys):
    """`zed kanban daemon` (no --force) is a deprecation stub."""
    from zed_cli import kanban as kb_cli
    ns = argparse.Namespace(
    ...
    assert "zed gateway start" in err
...
def test_cli_daemon_help_marks_deprecated():
    ...
    assert found_deprecation, (
        ...
        "the migration guidance in `zed kanban --help` output"
    )
...
def test_gateway_dispatcher_watcher_respects_env_override(monkeypatch):
    """ZED_KANBAN_DISPATCH_IN_GATEWAY=0 disables without touching config."""
    import asyncio
    from gateway.run import GatewayRunner
    monkeypatch.setenv("ZED_KANBAN_DISPATCH_IN_GATEWAY", "0")
...
def test_gateway_dispatcher_watcher_env_truthy_uses_config(monkeypatch):
    """Truthy env value doesn't force-enable â€” config still decides.
    ...
    monkeypatch.setenv("ZED_KANBAN_DISPATCH_IN_GATEWAY", "yes")
    monkeypatch.setattr(
        _cfg_mod, "load_config",
        lambda: {"kanban": {"dispatch_in_gateway": False}},
    )
    ...
    runner = object.__new__(GatewayRunner)
    runner._running = True
    # config says false, env is truthy â€” watcher should still exit
...
def test_complete_accepts_cross_worker_card_when_linked_as_child(kanban_home):
    ...
    a child of the completing task is accepted â€” the worker took
    ...
    (Relaxation salvaged from #20022 @LeonSGP43 â€” stricter version
...
def test_complete_can_retry_after_phantom_rejection(kanban_home):
    ...
    retry kanban_complete on the same task â€” both with a corrected
...
# ---------------------------------------------------------------------------
# Unified failure counter â€” timeout + crash paths increment the same counter
...
def test_detect_crashed_workers_increments_counter(kanban_home):
    ...
    try:
        ...
        kb.claim_task(conn, tid)
        kb._set_worker_pid(conn, tid, 99999)  # fake pid â€” not alive
...
def test_detect_crashed_workers_protocol_violation_auto_blocks(kanban_home):
    ...
    try:
        ...
        # The ``crashed`` event would be misleading here â€” the worker
...
def test_detect_crashed_workers_nonzero_exit_uses_default_limit(kanban_home):
    ...
    normal counter path â€” one failure doesn't trip the breaker.
    """
    import zed_cli.kanban_db as _kb
    conn = kb.connect()
    try:
        ...
        # W_EXITCODE(1, 0) == 256 â€” WIFEXITED True, WEXITSTATUS == 1.
...
def test_dispatch_once_integrates_stale_detection(kanban_home, monkeypatch):
    ...
    with kb.connect() as conn:
        ...
        kb._set_worker_pid(conn, t, 99999)  # fake PID â€” avoid killing test
...
def test_dispatch_once_stale_disabled_when_timeout_zero(kanban_home, monkeypatch):
    ...
    # Use os.getpid() so _pid_alive â†’ True, preventing detect_crashed_workers
