...
Zed keeps its OWN copy of the Codex OAuth token (per profile + top-level),
separate from the Codex CLI's ``~/.codex/auth.json``. OAuth refresh_tokens are
single-use, so when the Codex CLI (or another Zed process) rotates the shared
...
surfacing a hard 401 â€” but ONLY for relogin-required failures, never for transient
...
from zed_cli.auth import AuthError, _refresh_codex_auth_tokens, resolve_codex_runtime_credentials

STALE = {"access_token": "stale-access", "refresh_token": "stale-refresh"}


def test_self_heals_on_stale_refresh_token(monkeypatch):
    """invalid_grant (relogin-required) â†’ reimport from ~/.codex and persist it."""
    saved = {}
    fresh = {
    ...
    # the recovered token was persisted to the Zed auth store
    assert saved["access_token"] == "fresh-access"


def test_does_not_self_heal_on_rate_limit(monkeypatch):
    """429 quota keeps relogin_required=False â€” token still valid, must NOT reimport."""
    import_calls = {"n": 0}
...
def test_reraises_when_codex_cli_token_absent(monkeypatch):
    """relogin-required but ~/.codex unavailable/expired â†’ propagate original error."""
...
def test_happy_path_unchanged(monkeypatch):
    """Normal refresh succeeds â†’ rotated tokens persisted, ~/.codex never consulted."""
    saved = {}
    import_calls = {"n": 0}
...
def test_reraises_when_imported_token_lacks_refresh_token(monkeypatch):
    """relogin-required, but ~/.codex returns an access_token with NO refresh_token â†’
...
def test_self_heals_missing_singleton_access_token_from_codex_cli(tmp_path, monkeypatch):
    """Exact cron failure path: Zed auth has refresh_token but missing access_token."""
    ...
    assert resolved["source"] == "zed-auth-store"
    stored = json.loads((zed_home / "auth.json").read_text())
