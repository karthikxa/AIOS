...
still opt-in; exclusive kind skipped; unknown kinds â†’ standalone warning).
"""
...
def _enable(zed_home: Path, name: str) -> None:
    """Append ``name`` to ``plugins.enabled`` in ``<zed_home>/config.yaml``."""
...
# â”€â”€ Scanner recursion â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestCategoryNamespaceRecursion:
    def test_category_namespace_discovered(self, tmp_path, monkeypatch):
        ...
        import os
        zed_home = Path(os.environ["ZED_HOME"])  # set by hermetic conftest fixture
        user_plugins = zed_home / "plugins"

        _write_plugin(user_plugins, ["image_gen", "openai"])
        _enable(zed_home, "image_gen/openai")
    ...
    def test_depth_cap_two(self, tmp_path, monkeypatch):
        ...
        ``<root>/a/b/c/plugin.yaml`` should NOT be picked up â€” cap is
        two segments.
        """
        import os
        zed_home = Path(os.environ["ZED_HOME"])  # set by hermetic conftest fixture
    ...
    def test_category_dir_with_manifest_is_leaf(self, tmp_path, monkeypatch):
        ...
        # parent has a manifest â†’ stop recursing
        _write_plugin(user_plugins, ["image_gen"])
        # child also has a manifest â€” should NOT be found because we stop
        # at the parent.
        _write_plugin(user_plugins, ["image_gen", "openai"])
        _enable(zed_home, "image_gen")
        _enable(zed_home, "image_gen/openai")

        mgr = PluginManager()
        mgr.discover_and_load()

        # The bundled plugins/image_gen/openai/ exists in the repo â€” filter
...
# â”€â”€ Kind parsing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestKindField:
    def test_default_kind_is_standalone(self, tmp_path, monkeypatch):
        import os
        zed_home = Path(os.environ["ZED_HOME"])  # set by hermetic conftest fixture
        _write_plugin(zed_home / "plugins", ["p1"])
        _enable(zed_home, "p1")
...
# â”€â”€ Gate logic â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


class TestBackendGate:
    def test_user_backend_still_gated_by_enabled(self, tmp_path, monkeypatch):
        """User-installed ``kind: backend`` plugins still require opt-in â€”
    ...
    def test_user_backend_loads_when_enabled(self, tmp_path, monkeypatch):
        ...
        _enable(zed_home, "image_gen/fancy")
    ...
    def test_exclusive_kind_skipped(self, tmp_path, monkeypatch):
        """``kind: exclusive`` plugins are recorded but not loaded â€” the
...
# â”€â”€ Bundled backend auto-load (integration with real bundled plugin) â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestBundledBackendAutoLoad:
    def test_bundled_image_gen_openai_autoloads(self, tmp_path, monkeypatch):
        ...
        any opt-in â€” it's ``kind: backend`` and shipped in-repo."""
        import os
        zed_home = Path(os.environ["ZED_HOME"])  # set by hermetic conftest fixture
...
# â”€â”€ PluginContext.register_image_gen_provider â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestRegisterImageGenProvider:
    def test_accepts_valid_provider(self, tmp_path, monkeypatch):
        ...
        _enable(zed_home, "my-img-plugin")
    ...
    def test_rejects_non_provider(self, tmp_path, monkeypatch, caplog):
        ...
        _enable(zed_home, "bad-img-plugin")
