...
def kanban_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ZED_MEDIA_ALLOW_DIRS", str(tmp_path))
    kb.init_db()
    return home
...
async def test_notifier_unsubs_after_abnormal_events(kind, kanban_home):
    ...
    a truly final status (done / archived) â€” see the comment on
...
async def test_notifier_second_blocked_delivers(kanban_home):
    ...
    # Cycle 2: unblock â†’ block run again
    runner._running = True
    tick_count = 0
...
# the explicit `init_db()` was redundant â€” and worse, `init_db()`
...
async def test_notifier_does_not_call_init_db(kanban_home):
    ...
    with patch("gateway.run.asyncio.sleep", side_effect=_fast_sleep), \
         patch("zed_cli.kanban_db.init_db", side_effect=_spy_init_db):
        await asyncio.wait_for(
            runner._kanban_notifier_watcher(interval=1),
            timeout=10.0,
        )
    ...
    assert init_db_calls == [], (
        "_kanban_notifier_watcher must not call init_db on every tick â€” "
...
def test_dispatcher_tick_does_not_call_init_db(kanban_home, monkeypatch):
    ...
    assert "_kb.init_db(board=slug)" not in src, (
        "_kanban_dispatcher_watcher must not call _kb.init_db(board=slug) â€” "
    ...
    assert "_kb.init_db(board=slug)" not in notifier_src, (
        "_kanban_notifier_watcher must not call _kb.init_db(board=slug) â€” "
        "see issue #21378."
    )
...
async def test_notifier_uploads_artifacts_on_completion(kanban_home, tmp_path, monkeypatch):
    ...
    # via ``ZED_MEDIA_ALLOW_DIRS``. Test fixtures live under ``tmp_path``,
    ...
    # â†’ metadata.artifacts â†’ event payload promotion.
    import os
    os.environ["ZED_KANBAN_TASK"] = tid
    try:
        ...
        os.environ.pop("ZED_KANBAN_TASK", None)
...
async def test_notifier_artifact_delivery_skips_missing_files(kanban_home, tmp_path, monkeypatch):
    """Missing artifact paths are silently skipped â€” they may have been
