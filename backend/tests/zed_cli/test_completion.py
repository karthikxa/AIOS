"""Tests for zed_cli/completion.py â€” shell completion script generation."""
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real zed structure."""
    p = argparse.ArgumentParser(prog="zed")
    ...
    # alias â€” should NOT appear as a duplicate in completions
...
class TestWalk:
    ...
    def test_aliases_not_duplicated(self):
        """'foreground' is an alias of 'run' â€” must not appear as separate entry."""
        tree = _walk(_make_parser())
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_zed_completion()" in out
        assert "complete -F _zed_completion zed" in out
...
class TestGenerateZsh:
    ...
    def test_registers_compdef_instead_of_invoking_completion_function(self):
        out = generate_zsh(_make_parser())
        assert 'compdef _zed zed' in out
        assert '_zed "$@"' not in out
    ...
    def test_preserves_valid_zsh_arguments_alias_syntax(self):
        ...
        assert "'(-)'{-p,--profile}'[Profile name]:profile:_zed_profiles'" in out
        ...
        try:
            result = subprocess.run(
                [
                    "zsh",
                    "-fc",
                    f"autoload -Uz compinit && compinit -D; source {path}; [[ ${{_comps[zed]}} == _zed ]]",
                ],
                capture_output=True,
                text=True,
            )
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c zed -f" in out
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        out = generate_bash(_make_parser())
        assert "_zed_profiles()" in out
        assert 'profiles_dir="$HOME/.zed/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'zed profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _zed_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_zed_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _zed_profiles"

    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_zed_profiles()" in out
        assert "$HOME/.zed/profiles" in out
