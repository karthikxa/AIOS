...
`ZED_REDACT_SECRETS` at module-import time. `zed_cli/main.py` at
line ~174 calls `setup_logging(mode="cli")` which transitively imports
`agent.redact` â€” BEFORE any config bridge ran. So if a user set
`security.redact_secrets: false` in config.yaml (instead of as an env var
in .env), the toggle was silently ignored in both `zed chat` and
`zed gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml â†’ `ZED_REDACT_SECRETS`
env var in `zed_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    redaction â€” even though it's set in YAML, not as an env var."""
    ...
    # Spawn a fresh Python process that imports zed_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess â€” we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("ZED_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import zed_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('ZED_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["ZED_HOME"] = str(zed_home)
    env.pop("ZED_REDACT_SECRETS", None)
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env ZED_REDACT_SECRETS takes precedence over config.yaml."""
    zed_home = tmp_path / ".zed"
    zed_home.mkdir()
    (zed_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (zed_home / ".env").write_text("ZED_REDACT_SECRETS=true\n")
