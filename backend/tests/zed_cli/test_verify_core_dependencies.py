...
base deps) during ``zed update``, leaving the venv in a broken state
...
def temp_pyproject(tmp_path, monkeypatch):
    """Point zed_cli.main.PROJECT_ROOT at a tmp dir with a minimal pyproject.
...
class TestVerifyCoreDependencies:
    def test_no_action_when_all_deps_present(self, temp_pyproject, fake_venv_python):
        ...
        with patch("zed_cli.main._resolve_install_target_python", return_value=py), \
             patch("zed_cli.main.subprocess.run") as mock_run, \
             patch("zed_cli.main._run_install_with_heartbeat") as mock_install:
            mock_run.return_value = MagicMock(returncode=0, stdout="", stderr="")

            from zed_cli.main import _verify_core_dependencies_installed
    ...
    def test_triggers_reinstall_when_dep_missing(self, temp_pyproject, fake_venv_python):
        """The regression: one base dep is missing â†’ trigger --reinstall."""
        py, venv_root = fake_venv_python
        env = {"VIRTUAL_ENV": str(venv_root)}
        ...
        with patch("zed_cli.main._resolve_install_target_python", return_value=py), \
             patch("zed_cli.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("zed_cli.main._run_install_with_heartbeat") as mock_install:
    ...
    def test_skips_deps_excluded_by_environment_markers(self, temp_pyproject, fake_venv_python):
        ...
        with patch("zed_cli.main._resolve_install_target_python", return_value=py), \
             patch("zed_cli.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("zed_cli.main._run_install_with_heartbeat"), \
             patch("sys.platform", "win32"):

            from zed_cli.main import _verify_core_dependencies_installed
            _verify_core_dependencies_installed(["uv", "pip"], env=env)

        # Find the probe argv â€” it's the call that passed the dep names.
        probe = next(
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch("zed_cli.main._resolve_install_target_python") as mock_resolve, \
             patch("zed_cli.main._run_install_with_heartbeat") as mock_install:
            from zed_cli.main import _verify_core_dependencies_installed
    ...
    def test_repair_reinstall_quarantines_running_shim_on_windows(
        ...
        quarantine the running ``zed.exe`` on Windows before installing.

        That reinstall rewrites the editable entry-point shims, and on Windows
        pip can't overwrite the live launcher â€” so without quarantine the shim
        is left missing and ``zed`` drops off PATH. Previously this path
        ...
        def fake_subprocess_run(cmd, **kwargs):
            probe_calls["count"] += 1
            # 1st probe: pathspec missing â†’ triggers --reinstall repair.
            # 2nd probe (after repair): clean â†’ stops before per-package path.
            if probe_calls["count"] == 1:
                return MagicMock(returncode=0, stdout="pathspec\n", stderr="")
            return MagicMock(returncode=0, stdout="", stderr="")
        ...
        with patch("zed_cli.main._resolve_install_target_python", return_value=py), \
             patch("zed_cli.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("zed_cli.main._is_windows", return_value=True), \
             patch("zed_cli.main._venv_scripts_dir", return_value=fake_scripts), \
             patch("zed_cli.main._run_install_with_heartbeat"), \
             patch("zed_cli.main._quarantine_running_zed_exe", return_value=[]) as mock_quar:
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        venv's interpreter â€” not the outer Python that drove `zed update`.
        ...
        with patch("zed_cli.main._is_windows", return_value=True):
            from zed_cli.main import _resolve_install_target_python
