...
The Python launcher (`zed --tui â€¦`) has a ~100â€“300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Zed session
left DEC mouse-tracking asserted, any mouse motion during that window
echoes literal ``^[[<â€¦M`` text into the user's scrollback.
...
# scope. Under the test runner argv (`pytest â€¦`) it's a no-op, but we import
# at file scope so individual tests don't race the import side-effect with
# their `patch("os.write")` context.
from zed_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["zed", "--tui", "-c", "abc"])
        monkeypatch.delenv("ZED_TUI", raising=False)
        monkeypatch.delenv("ZED_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_writes_disable_sequence_when_zed_tui_env_set(self, monkeypatch):
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        ...
        monkeypatch.delenv("ZED_TUI", raising=False)
        monkeypatch.setenv("ZED_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_skips_when_stdout_is_not_a_tty(self, monkeypatch):
        # `zed --tui â€¦ >log` or CI capture: pipe is fd 1, not a TTY. The
    ...
    def test_oserror_is_swallowed(self, monkeypatch):
        ...
        with patch("os.isatty", return_value=True), patch("os.write", side_effect=boom):
            # Must not propagate â€” startup hot path can never break.
            _suppress_mouse_residue_early()
