"""Tests for ``zed gui`` desktop launcher wiring."""
...
from zed_cli import main as cli_main
...
def _make_packaged_executable(root: Path, monkeypatch, platform: str = "darwin") -> Path:
    ...
    desktop_dir = root / "apps" / "desktop"
    if platform == "darwin":
        exe = desktop_dir / "release" / "mac-arm64" / "Zed.app" / "Contents" / "MacOS" / "Zed"
    elif platform == "win32":
        exe = desktop_dir / "release" / "win-unpacked" / "Zed.exe"
    else:
        exe = desktop_dir / "release" / "linux-unpacked" / "zed"
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("zed_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("zed_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("zed_cli.main._desktop_build_needed", return_value=True), \
         patch("zed_cli.main._write_desktop_build_stamp"), \
         patch("zed_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("zed_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
...
def test_gui_forwards_desktop_environment_overrides(tmp_path, monkeypatch):
    ...
    with patch("zed_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("zed_cli.main._run_npm_install_deterministic", return_value=ok), \
         patch("zed_cli.main._desktop_build_needed", return_value=True), \
         patch("zed_cli.main._write_desktop_build_stamp"), \
         patch("zed_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("zed_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns(
            fake_boot=True,
            ignore_existing=True,
            zed_root=str(zed_root),
            cwd=str(cwd),
        ))

    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["ZED_DESKTOP_BOOT_FAKE"] == "1"
    assert launch_env["ZED_DESKTOP_IGNORE_EXISTING"] == "1"
    assert launch_env["ZED_DESKTOP_ZED_ROOT"] == str(zed_root)
    assert launch_env["ZED_DESKTOP_CWD"] == str(cwd)
...
def test_gui_exits_when_npm_missing(tmp_path, monkeypatch, capsys):
    ...
    with patch("zed_cli.main.shutil.which", return_value=None), \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_skip_build_launches_existing_packaged_app_without_npm(tmp_path, monkeypatch):
    ...
    with patch("zed_cli.main.shutil.which", return_value=None), \
         patch("zed_cli.main._run_npm_install_deterministic") as mock_install, \
         patch("zed_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_configures_sandbox_before_launch(tmp_path, monkeypatch):
    ...
    with patch("zed_cli.main.shutil.which", return_value="/usr/bin/sudo"), \
         patch("zed_cli.main.subprocess.run", return_value=ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_rejects_symlink_sandbox(tmp_path, monkeypatch):
    ...
    with patch("zed_cli.main.shutil.which", return_value="/usr/bin/sudo"), \
         patch("zed_cli.main.subprocess.run") as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_skips_fixup_when_already_configured(tmp_path, monkeypatch):
    ...
    sandbox = packaged_exe.parent / "chrome-sandbox"
    sandbox.write_text("", encoding="utf-8")
    # Simulate root-owned 4755 â€” lstat().st_uid==0 and mode==0o4755
    ...
    assert exc.value.code == 0
    # Only the launch call â€” no sudo chown/chmod
    mock_run.assert_called_once()
    assert mock_run.call_args.args[0] == [str(packaged_exe)]
...
def test_gui_source_mode_uses_renderer_build_and_electron(tmp_path, monkeypatch):
    ...
    with patch("zed_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("zed_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("zed_cli.main._desktop_build_needed", return_value=True), \
         patch("zed_cli.main._write_desktop_build_stamp"), \
         patch("zed_cli.main.subprocess.run", side_effect=[build_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
...
# â”€â”€ Content-hash stamp tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_desktop_build_stamp_skips_build_when_up_to_date(tmp_path, monkeypatch):
    ...
    with patch("zed_cli.main._desktop_build_needed", return_value=False), \
         patch("zed_cli.main._run_npm_install_deterministic") as mock_install, \
         patch("zed_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         patch("zed_cli.main._desktop_macos_relaunchable_fixup"), \
         pytest.raises(SystemExit) as exc:
...
def test_desktop_force_build_overrides_stamp(tmp_path, monkeypatch):
    ...
    with patch("zed_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("zed_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("zed_cli.main._desktop_build_needed", return_value=False), \
         patch("zed_cli.main._write_desktop_build_stamp") as mock_stamp, \
         patch("zed_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("zed_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
...
def test_compute_desktop_content_hash_stable(tmp_path, monkeypatch):
    ...
    (root / "package.json").write_text('{"name":"zed"}', encoding="utf-8")
...
def test_desktop_build_needed_detects_missing_artifact(tmp_path, monkeypatch):
    ...
    # No packaged executable exists â†’ build needed
...
def test_compute_desktop_content_hash_works_without_gitignore(tmp_path, monkeypatch):
    ...
    # No .gitignore â†’ pathspec matches nothing â†’ all files hashed
    h = cli_main._compute_desktop_content_hash(root)
    assert len(h) == 64  # valid sha256 hex

    # Edit a file â†’ hash changes
    (root / "apps" / "desktop" / "main.js").write_text("v2", encoding="utf-8")
    h2 = cli_main._compute_desktop_content_hash(root)
    assert h != h2
...
def test_compute_desktop_content_hash_respects_gitignore(tmp_path, monkeypatch):
    ...
    # Change the .env file (ignored) â€” hash should NOT change
    ...
    # Change the .js file (not ignored) â€” hash SHOULD change
...
# â”€â”€ Electron build-cache recovery tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_purge_electron_build_cache_clears_all_zips_and_unpacked_dir(tmp_path, monkeypatch):
    ...
    validator â€” not a self-rolled one."""
    cache = tmp_path / "electron-cache"
    # A "clean" zip and a prepended-junk zip â€” the latter is the real-world
...
def test_purge_electron_build_cache_empty_when_nothing_present(tmp_path, monkeypatch):
    """No cached zips and no unpacked dir â†’ nothing removed, so the caller
...
def test_gui_retries_pack_once_after_purging_build_cache(tmp_path, monkeypatch):
    ...
    with patch("zed_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("zed_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("zed_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("zed_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("zed_cli.main._write_desktop_build_stamp"), \
         patch("zed_cli.main._purge_electron_build_cache", return_value=[Path("/c/electron.zip")]) as mock_purge, \
         patch("zed_cli.main._electron_dist_ok", return_value=False), \
         patch("zed_cli.main._redownload_electron_dist", return_value=True), \
         patch("zed_cli.main.subprocess.run", side_effect=[pack_fail, pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())

    assert exc.value.code == 0
    mock_purge.assert_called_once()
    # pack(fail) â†’ repair succeeds â†’ pack(ok) â†’ launch = 3 subprocess.run calls
...
def test_gui_redownloads_electron_via_mirror_then_repacks(tmp_path, monkeypatch, capsys):
    """Purge clears nothing and the pinned electronDist (#38673) is missing â†’
    ...
    with patch("zed_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("zed_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("zed_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("zed_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("zed_cli.main._electron_dist_ok", return_value=False), \
         patch("zed_cli.main._redownload_electron_dist", side_effect=[False, True]) as mock_dl, \
         patch("zed_cli.main.subprocess.run", side_effect=[pack_fail, pack_fail]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_retries_pack_under_mirror_even_when_prefetch_blocked(tmp_path, monkeypatch, capsys):
    ...
    with patch("zed_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("zed_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("zed_cli.main._electron_dist_ok", return_value=False), \
         patch("zed_cli.main._redownload_electron_dist", return_value=False), \
         patch("zed_cli.main.subprocess.run", side_effect=[pack_fail, pack_fail]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_install_failure_self_heals_electron_and_continues(tmp_path, monkeypatch, capsys):
    ...
    with patch("zed_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("zed_cli.main._run_npm_install_deterministic", return_value=install_fail), \
         patch("zed_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("zed_cli.main._write_desktop_build_stamp"), \
         patch("zed_cli.main._electron_dist_ok", return_value=False), \
         patch("zed_cli.main._try_redownload_electron_dist", return_value=True) as mock_dl, \
         patch("zed_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
    ...
    # pack + launch ran â€” the install failure did NOT abort the build.
    assert mock_run.call_count == 2
...
def test_gui_install_failure_hard_fails_when_electron_not_staged(tmp_path, monkeypatch, capsys):
    ...
    error (not a blocked binary download) â€” hard-fail with guidance, don't try to
...
def test_gui_install_failure_hard_fails_when_electron_dist_exists(tmp_path, monkeypatch, capsys):
    ...
    with patch("zed_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("zed_cli.main._run_npm_install_deterministic", return_value=install_fail), \
         patch("zed_cli.main._electron_dist_ok", return_value=True), \
         patch("zed_cli.main.subprocess.run") as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_does_not_override_user_electron_mirror(tmp_path, monkeypatch, capsys):
    ...
    with patch("zed_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("zed_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("zed_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("zed_cli.main._purge_electron_build_cache", return_value=[]) as mock_purge, \
         patch("zed_cli.main.subprocess.run", side_effect=[pack_fail]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
# â”€â”€ electronDist (re)download helper tests (#47266) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_redownload_electron_dist_noop_when_present(tmp_path, monkeypatch):
    """Already-healthy dist â†’ no download, so an unrelated build failure can't
    ...
    with patch("zed_cli.main.subprocess.run") as mock_run:
...
def test_redownload_electron_dist_missing_installer(tmp_path, monkeypatch):
    """No electron/install.js (deps never installed) â†’ nothing to run."""
    monkeypatch.setattr(cli_main.sys, "platform", "linux")
    (tmp_path / "node_modules" / "electron").mkdir(parents=True)

    with patch("zed_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("zed_cli.main.subprocess.run") as mock_run:
...
def test_redownload_electron_dist_runs_installer_with_mirror(tmp_path, monkeypatch):
    """Missing dist â†’ wipe any partial dist + version marker, run electron's own
    ...
    with patch("zed_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("zed_cli.main.subprocess.run", side_effect=fake_run):
...
def test_redownload_electron_dist_returns_false_when_download_fails(tmp_path, monkeypatch):
    """install.js ran but produced no binary (still blocked) â†’ False, so the
    ...
    with patch("zed_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("zed_cli.main.subprocess.run",
...
def test_stop_desktop_build_lock_terminates_only_release_procs(tmp_path, monkeypatch):
    ...
    locker_exe = release / "Zed.exe"
    locker_exe.write_text("", encoding="utf-8")
    other_exe = tmp_path / "elsewhere" / "Zed.exe"
    ...
    selfish = _FakeProc(999, str(locker_exe))  # our own PID â€” never killed
