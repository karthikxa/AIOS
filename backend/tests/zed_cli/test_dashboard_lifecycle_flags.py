"""Tests for ``zed dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``zed update``
...
from zed_cli.main import cmd_dashboard
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        ...
        assert "No zed dashboard processes running" in out
    ...
    def test_status_with_processes(self, capsys):
        with patch("zed_cli.main._find_stale_dashboard_pids",
             ...
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(status=True))
        # Status is informational â€” always exits 0.
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "2 zed dashboard process(es) running" in out
        assert "PID 12345" in out
        assert "PID 12346" in out

    def test_status_does_not_try_to_import_fastapi(self):
        """`--status` must not require dashboard runtime deps â€” it's a
...
class TestDashboardStop:
    ...
    def test_stop_kills_and_exits_zero_when_all_killed(self, capsys):
        ...
        with patch("zed_cli.main._find_stale_dashboard_pids",
             ...
             patch("zed_cli.main._kill_stale_dashboard_processes") as mock_kill, \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(stop=True))
    ...
    def test_stop_exits_nonzero_if_kill_leaves_survivors(self):
        ...
        with patch("zed_cli.main._find_stale_dashboard_pids",
                   side_effect=lambda: next(scans)), \
             patch("zed_cli.main._kill_stale_dashboard_processes"), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(stop=True))
...
class TestLifecycleFlagsTakePrecedence:
    ...
    server-start path, which is the critical safety property â€” a user
    who typed ``zed dashboard --stop`` must not end up ALSO starting
    a new server."""
    ...
    def test_stop_does_not_fall_through_to_server_start(self):
        ...
        with patch("zed_cli.main._find_stale_dashboard_pids",
                   return_value=[]), \
             patch.dict(sys.modules, {"zed_cli.web_server": fake_ws}), \
             pytest.raises(SystemExit):
            cmd_dashboard(_ns(stop=True))
...
class TestArgparseWiring:
    ...
    ``zed dashboard --stop`` / ``--status`` actually parse."""

    def test_flags_are_registered(self):
        from zed_cli.main import main as _cli_main  # noqa: F401
