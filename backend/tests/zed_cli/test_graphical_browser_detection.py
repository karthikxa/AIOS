"""Tests for `_can_open_graphical_browser()` in zed_cli.auth.

Guards the fix for the May 2026 report where `zed auth add xai-oauth`
launched a text-mode browser (w3m) INSIDE the terminal on a headless Linux
box â€” `_is_remote_session()` only checked SSH/cloud-shell env vars, so a plain
...
from zed_cli.auth import _can_open_graphical_browser
...
def _force_platform_linux(monkeypatch):
    monkeypatch.setattr("zed_cli.auth.sys.platform", "linux")
...
def test_headless_linux_no_display_refuses(monkeypatch):
    """The reported bug: headless Linux, no display server â†’ don't auto-open."""
...
def test_graphical_browser_with_display_allows(monkeypatch):
    """Real GUI browser + display server â†’ auto-open is fine."""
    _force_platform_linux(monkeypatch)
    monkeypatch.setenv("DISPLAY", ":0")
...
def test_webbrowser_get_raises_refuses(monkeypatch):
    """No resolvable browser at all â†’ don't auto-open."""
    _force_platform_linux(monkeypatch)
    monkeypatch.setenv("DISPLAY", ":0")
...
def test_non_linux_with_gui_allows(monkeypatch):
    ...
    monkeypatch.setattr("zed_cli.auth.sys.platform", "darwin")
