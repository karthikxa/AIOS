...
from zed_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from zed_cli.auth import _resolve_verify
    ...
    def test_valid_ca_bundle_in_auth_state_is_returned(self, tmp_path, monkeypatch):
        ...
        # Avoid loading actual PEM â€” just verify the return type
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("ZED_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_zed_ca_bundle_env_falls_back(self, monkeypatch):
        from zed_cli.auth import _resolve_verify

        monkeypatch.setenv("ZED_CA_BUNDLE", "/nonexistent/zed-ca.pem")
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ZED_HOME", str(zed_home))
    monkeypatch.setenv("ZED_AGENT_USE_LEGACY_SESSION_KEYS", "true")
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.INFO, logger="zed_cli.auth")
    auth_mod.resolve_nous_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    even when the auth store has no Nous provider entry â€” this is the
    ...
    zed_home = tmp_path / "zed"
    zed_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store â€” no Nous provider entry
...
def test_get_nous_auth_status_pool_opaque_key_is_not_inference_credential(tmp_path, monkeypatch):
    from zed_cli.auth import get_nous_auth_status, invalidate_nous_auth_status_cache
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    zed_home = tmp_path / "zed"
    _setup_nous_auth(zed_home, access_token="at-123")
    monkeypatch.setenv("ZED_HOME", str(zed_home))
    monkeypatch.setattr(
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    zed_home = tmp_path / "zed"
    _setup_nous_auth(zed_home, access_token="at-fresh")
    monkeypatch.setenv("ZED_HOME", str(zed_home))
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zed_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_refresh_token_persisted_when_refreshed_jwt_lacks_invoke_scope(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zed_cli.auth._refresh_access_token", _fake_refresh_access_token)
...
def test_managed_access_token_refresh_failure_quarantines_tokens(
    ...
    zed_home = tmp_path / "zed"
    _setup_nous_auth(zed_home, refresh_token="refresh-old")
    monkeypatch.setenv("ZED_HOME", str(zed_home))
    from agent.credential_pool import load_pool
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `zed model` â†’ Nous Portal â†’ Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return zed_home, config_path, auth_path
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        fake_auth_state = {
            ...
            "inference_base_url": "https://inference-api.zedteam.com",
            "portal_base_url": "https://portal.zedteam.com",
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        """User picks Skip â†’ config.yaml untouched, Nous creds still saved."""
        import argparse
        import yaml
        from zed_cli.auth import PROVIDER_REGISTRY, _login_nous

        zed_home, config_path, auth_path = self._setup_home_with_openrouter(
    ...
    def test_picking_model_switches_to_nous(self, tmp_path, monkeypatch):
        """User picks a Nous model â†’ provider flips to nous with that model."""
        import argparse
        import yaml
        from zed_cli.auth import PROVIDER_REGISTRY, _login_nous
    ...
    def test_skip_with_no_prior_active_provider_clears_it(self, tmp_path, monkeypatch):
        """Fresh install (no prior active_provider) â†’ Skip clears active_provider
        ...
        # No auth.json yet â€” simulates first-run before any OAuth
        self._patch_login_internals(monkeypatch, prompt_returns=None)
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `zed auth add nous`
    ...
    """
    from zed_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "Zed is not logged into Nous Portal").

    This is the exact path that run_agent.py's `_try_refresh_nous_client_credentials`
    calls after a Nous 401 â€” before the fix it would raise AuthError because
    ...
    # portal â€” the point of this test is that state lookup succeeds and
    # doesn't raise "Zed is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_idempotent_no_duplicate_pool_entries(tmp_path, monkeypatch):
    """Re-running persist must upsert â€” not accumulate duplicate device_code rows.
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `zed auth add nous --type oauth --label <name>` silently
...
def test_persist_nous_credentials_no_label_uses_auto_derived(tmp_path, monkeypatch):
    ...
    is used (unchanged default behaviour â€” regression guard).
    """
    from zed_cli.auth import persist_nous_credentials
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Zed must surface an
    ...
    re-authenticate" string led users to report this as a Zed persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from zed_cli.auth import _refresh_access_token
    ...
    assert "zed auth add nous" in message.lower()
...
# =============================================================================
# Shared Nous token store â€” cross-profile persistence (Codex-style auto-import)
# =============================================================================


@pytest.fixture
def shared_store_env(tmp_path, monkeypatch):
    """Redirect ZED_SHARED_AUTH_DIR to a tmp_path.

    Required for every test that exercises the shared Nous store â€” the
    ...
    monkeypatch.setenv("ZED_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without ZED_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.zed/shared/`` across CI runs.
    """
    from zed_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("ZED_SHARED_AUTH_DIR", raising=False)
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """ZED_SHARED_AUTH_DIR must redirect the path."""
    from zed_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("ZED_SHARED_AUTH_DIR", str(custom_dir))
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    """Missing file â†’ ``_read_shared_nous_state()`` returns None."""
    from zed_cli.auth import _read_shared_nous_state

    assert _read_shared_nous_state() is None


def test_shared_store_read_malformed_returns_none(shared_store_env):
    """Unreadable / non-JSON file â†’ None, not an exception."""
    from zed_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_read_missing_required_fields_returns_none(shared_store_env):
    """Payload without refresh_token â†’ None (nothing worth importing)."""
    from zed_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_and_read_roundtrip(shared_store_env):
    """Write â†’ read must preserve refresh_token + OAuth URLs."""
    from zed_cli.auth import (
        _nous_shared_store_path,
    ...
    # (24h TTL, profile-specific â€” only long-lived OAuth tokens are
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from zed_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `zed auth add nous
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    """No shared store â†’ no rehydrate (fall through to device-code)."""
    from zed_cli.auth import _try_import_shared_nous_state
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "zed-cli"


def test_shared_store_survives_across_profile_switch(
    tmp_path, monkeypatch, shared_store_env,
):
    """End-to-end: profile A logs in â†’ shared store populated â†’ profile B
    (different ZED_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("ZED_HOME", str(profile_a))
    ...
    # Profile B: fresh ZED_HOME, no auth yet, but the shared store
    # persists â€” _read_shared_nous_state() must still return the tokens.
    profile_b = tmp_path / "profile_b"
    profile_b.mkdir(parents=True, exist_ok=True)
    (profile_b / "auth.json").write_text(
        json.dumps({"version": 1, "providers": {}})
    )
    monkeypatch.setenv("ZED_HOME", str(profile_b))
