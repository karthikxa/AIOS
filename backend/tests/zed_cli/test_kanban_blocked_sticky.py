"""Regression tests for #28712 â€” kanban dispatcher must not auto-promote
...
got recorded as a ``protocol_violation`` â†’ ``gave_up`` â†’ promote â†’ loop
...
* Circuit-breaker blocks (``gave_up`` event, status flipped via
  ``_record_task_failure``) still auto-recover â€” the original intent
  of #40c1decb3 is preserved.
* An explicit ``kanban_unblock`` clears the sticky state.
* The full block â†’ promote â†’ crash â†’ ``gave_up`` loop is broken after
...
def test_worker_block_is_not_auto_promoted_by_recompute_ready(kanban_home: Path) -> None:
    ...
    with kb.connect() as conn:
        ...
        # Hammer the promotion code â€” exactly the dispatcher loop's
...
def test_circuit_breaker_block_still_auto_promotes(kanban_home: Path) -> None:
    ...
    must get auto-promoted when its parents complete â€” preserves the
    pre-#28712 recovery semantics for genuinely transient failures.

    The complementary case â€” a block whose failure count has *reached*
    the limit must stay blocked â€” is covered by
    ...
    with kb.connect() as conn:
        ...
        # status without emitting a ``blocked`` event â€” exactly what
...
def test_gave_up_event_alone_does_not_make_block_sticky(kanban_home: Path) -> None:
    ...
    as sticky â€” otherwise we'd regress the safety net for genuinely
    transient crashes."""
    with kb.connect() as conn:
        parent = kb.create_task(conn, title="parent")
        child = kb.create_task(conn, title="child", parents=[parent])
...
def test_unblock_clears_sticky_state_and_lets_block_recover(kanban_home: Path) -> None:
    """``zed kanban unblock`` (or the ``kanban_unblock`` tool) is
    ...
    with kb.connect() as conn:
        ...
        # block/unblock event is ``unblocked`` â†’ guard does not fire
        # â†’ recompute can recover.
        conn.execute(
...
# ---------------------------------------------------------------------------
# Full bug-shaped loop: block â†’ promote â†’ crash â†’ gave_up â†’ next tick
# ---------------------------------------------------------------------------
...
def test_protocol_violation_loop_is_broken(kanban_home: Path) -> None:
    ...
    1. Worker calls ``kanban_block`` â†’ status='blocked',
       ``task_runs.outcome='blocked'``, ``blocked`` event.
    2. (Bug) Dispatcher promotes back to ``ready``.
    3. Fresh worker exits cleanly without terminal tool call â†’
       ``protocol_violation`` event.
    4. ``_record_task_failure(failure_limit=1)`` â†’ ``gave_up`` event,
       status='blocked' again.
    5. (Bug) Dispatcher promotes again â†’ infinite loop.

    With the fix in place, step 2 never happens â€” the test simulates
    ...
    with kb.connect() as conn:
        ...
        # First dispatcher tick â€” must NOT promote.
...
# tests/zed_cli/test_kanban_db.py::test_connect_migrates_legacy_db_before_optional_column_indexes
