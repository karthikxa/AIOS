...
Covers the fix for issue #6843 â€” API keys containing Unicode lookalike
characters (e.g. Ê‹ U+028B instead of v) cause UnicodeEncodeError when
...
from zed_cli.config import _check_non_ascii_credential
...
class TestCheckNonAsciiCredential:
    ...
    def test_strips_unicode_v_lookalike(self, capsys):
        """The exact scenario from issue #6843: Ê‹ instead of v."""
        key = "sk-proj-abc" + "Ê‹" + "def"  # \u028b
...
class TestEnvLoaderSanitization:
    ...
    def test_strips_non_ascii_from_api_key(self, monkeypatch):
        from zed_cli.env_loader import _sanitize_loaded_credentials, _WARNED_KEYS

        _WARNED_KEYS.discard("OPENROUTER_API_KEY")
        monkeypatch.setenv("OPENROUTER_API_KEY", "sk-proj-abcÊ‹def")
    ...
    def test_strips_non_ascii_from_token(self, monkeypatch):
        ...
        monkeypatch.setenv("DISCORD_BOT_TOKEN", "tokÃ©nvalue")
        _sanitize_loaded_credentials()
        assert os.environ["DISCORD_BOT_TOKEN"] == "toknvalue"

    def test_ignores_non_credential_vars(self, monkeypatch):
        from zed_cli.env_loader import _sanitize_loaded_credentials

        monkeypatch.setenv("MY_UNICODE_VAR", "hÃ©llo wÃ¶rld")
        _sanitize_loaded_credentials()
        # Not a credential suffix â€” should be left alone
        assert os.environ["MY_UNICODE_VAR"] == "hÃ©llo wÃ¶rld"
    ...
    def test_ascii_control_chars_not_stripped(self, monkeypatch, capsys):
        ...
        This is intentional â€” they're valid ASCII for HTTP headers even if the
