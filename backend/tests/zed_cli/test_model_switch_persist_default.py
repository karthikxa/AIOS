...
from zed_cli.model_switch import parse_model_flags, resolve_persist_behavior
...
class TestResolvePersistBehavior:
    ...
    def test_default_persists_when_config_missing(self):
        # No model section at all â†’ built-in default (True).
        with _config({}):
    ...
    def test_default_when_model_is_flat_string(self):
        # Fresh install: ``model: ""`` (not a dict) â†’ built-in default True.
        with _config({"model": ""}):
            assert resolve_persist_behavior(False, False) is True
