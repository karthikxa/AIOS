"""Tests for zed_cli configuration management."""
...
class TestGetZedHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZED_HOME", None)
            home = get_zed_home()
            assert home == Path.home() / ".zed"

    def test_env_override(self):
        with patch.dict(os.environ, {"ZED_HOME": "/custom/path"}):
            home = get_zed_home()
            assert home == Path("/custom/path")


class TestEnsureZedHome:
    def test_creates_subdirs(self, tmp_path):
        with patch.dict(os.environ, {"ZED_HOME": str(tmp_path)}):
            ensure_zed_home()
...
class TestLoadConfigParseFailure:
    ...
    on the first invocation â€” users saw aux-fallback misbehavior with no clue
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``zed logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        with patch.dict(os.environ, {"ZED_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="zed_cli.config"):
                config = load_config()

            # Falls back to defaults â€” confirms the silent-fallback we're warning about
            assert config["model"] == DEFAULT_CONFIG["model"]
            ...
            # stderr also got a user-visible message (with the âš ï¸ marker so it
            # stands out at zed startup before logging is configured)
            captured = capsys.readouterr()
            assert "zed config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"ZED_HOME": str(tmp_path)}):
            ...
            # Edit the file (still broken, but different content) â€” mtime changes
            time.sleep(0.05)
            (tmp_path / "config.yaml").write_text("\tstill broken differently:\n")
            load_config()
            after_edit = capsys.readouterr().err
            assert "zed config:" in after_edit, "edited file should re-warn"
    ...
    def test_corrupt_symlink_config_not_backed_up(self, tmp_path):
        ...
        guard) â€” avoids clobbering whatever the symlink points at."""
        import sys as _sys
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"ZED_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ZED_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ZED_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"ZED_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"ZED_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        mode-restore branch and re-tightened to 0600 â€” the same bug fixed
        ...
        with patch.dict(os.environ, {"ZED_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_unknown_keys_not_split(self):
        ...
        result = _sanitize_env_lines(lines)
        # Unknown keys stay on one line â€” no false split
        assert len(result) == 1
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from zed_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """ZED_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `zed setup`) lets a
        ...
        via config.yaml; ZED_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from zed_cli.config import OPTIONAL_ENV_VARS
        assert "ZED_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestAnthropicTokenMigration:
    """Test that config version 8â†’9 clears ANTHROPIC_TOKEN."""
    ...
    def test_skips_on_version_9_or_later(self, tmp_path):
        """Already at v9 â€” ANTHROPIC_TOKEN is not touched."""
...
class TestCustomProviderCompatibility:
    ...
    def test_v11_upgrade_moves_custom_providers_into_providers(self, tmp_path):
        ...
        # custom_providers removed by migration â€” runtime reads via compat layer
...
class TestEnvWriteDenylist:
    ...
    influence how subprocesses execute â€” ``LD_PRELOAD``, ``PYTHONPATH``,
    ``PATH``, ``EDITOR``, etc. â€” or any ``ZED_*`` runtime flag.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Zed
    process on next startup via the dotenv â†’ ``os.environ`` chain in
    ``zed_cli/env_loader.py``.
    ...
    def _zed_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "ZED_SPOTIFY_CLIENT_ID",
            "ZED_QWEN_BASE_URL",
            "ZED_MAX_ITERATIONS",
        ],
    )
    def test_zed_integration_keys_still_writable(self, allowed_key):
        """``ZED_*`` overall is NOT blocked â€” only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``ZED_*`` convention must keep
        working or we'd regress every provider setup wizard that
        currently writes one of these (auth.py, Spotify, Langfuse, â€¦)."""
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``ZED_*``) keep working. The denylist
    ...
    def test_save_env_value_secure_inherits_denylist(self):
        ...
        it inherits the gate â€” verify, don't assume."""
        with pytest.raises(ValueError, match="denylist"):
    ...
    def test_pre_existing_value_in_env_file_is_left_alone(self, tmp_path):
        ...
        change shipped, or hand-edited), we don't blow up â€” we just
...
class TestWriteApprovalMigration:
    """Version 28â†’29 renames memory/skills write_mode â†’ write_approval (bool).

    Only an explicit ``approve`` carried gating intent and maps to ``True``;
    ``on``/``off``/unset map to ``False`` (gate off). The old ``write_mode`` key
    is removed. Only a persisted key is rewritten â€” never invented.
    """
    ...
    def test_on_and_off_map_to_false(self, tmp_path):
        with patch.dict(os.environ, {"ZED_HOME": str(tmp_path)}):
            # YAML 1.1 parses bare on/off as bools â€” write_mode could be either
            # the string or the bool; both legacy "not gating" values â†’ False.
            self._write(tmp_path,
                        "_config_version: 28\nmemory:\n  write_mode: 'on'\n"
                        "skills:\n  write_mode: 'off'\n")
            migrate_config(interactive=False, quiet=True)
