"""Tests for zed_cli.dump._get_git_commit â€” git SHA resolution for ``zed dump``.

``zed dump`` prints the running commit so support bug reports identify the
...
written by the Dockerfile's ``ZED_GIT_SHA`` build-arg.
...
def test_get_git_commit_uses_live_git_when_available(tmp_path):
    ...
    with patch("zed_cli.dump.subprocess.run", return_value=git_result) as mock_run, \
         patch("zed_cli.build_info.get_build_sha") as mock_build:
...
def test_get_git_commit_falls_back_to_build_sha_when_live_git_fails(tmp_path):
    """Docker image case: live git returns non-zero â†’ use baked SHA."""
    from zed_cli import dump
    ...
    with patch("zed_cli.dump.subprocess.run", return_value=failed), \
         patch("zed_cli.build_info.get_build_sha", return_value="cafef00d"):
        commit = dump._get_git_commit(repo_dir)
...
def test_get_git_commit_falls_back_when_git_returns_empty_stdout(tmp_path):
    """Edge case: git exits 0 but prints nothing â€” still try the baked SHA."""
    from zed_cli import dump
    ...
    with patch("zed_cli.dump.subprocess.run", return_value=empty), \
         patch("zed_cli.build_info.get_build_sha", return_value="abcdef12"):
        commit = dump._get_git_commit(repo_dir)
...
def test_get_git_commit_falls_back_when_git_raises(tmp_path):
    """git binary missing (e.g. minimal container w/o git) â†’ baked SHA path."""
    from zed_cli import dump

    repo_dir = tmp_path / "repo"
    repo_dir.mkdir()

    with patch("zed_cli.dump.subprocess.run", side_effect=FileNotFoundError("git")), \
         patch("zed_cli.build_info.get_build_sha", return_value="feedface"):
        commit = dump._get_git_commit(repo_dir)
...
def test_get_git_commit_returns_unknown_when_neither_source_available(tmp_path):
    """Pip-installed wheel: no git, no baked SHA â†’ '(unknown)' (legacy contract)."""
    from zed_cli import dump
    ...
    failed = MagicMock(returncode=128, stdout="")
    with patch("zed_cli.dump.subprocess.run", return_value=failed), \
         patch("zed_cli.build_info.get_build_sha", return_value=None):
        commit = dump._get_git_commit(repo_dir)
...
def test_get_git_commit_output_format_identical_between_sources(tmp_path):
    ...
    so support tooling that parses ``zed dump`` doesn't have to special-case
    container builds.  Both paths must return a bare 8-char SHA â€” no prefix,
    ...
    with patch("zed_cli.dump.subprocess.run", return_value=git_result):
        live = dump._get_git_commit(repo_dir)

    # Baked-SHA path.
    failed = MagicMock(returncode=128, stdout="")
    with patch("zed_cli.dump.subprocess.run", return_value=failed), \
         patch("zed_cli.build_info.get_build_sha", return_value="b2f477a3"):
        baked = dump._get_git_commit(repo_dir)

    assert live == baked == "b2f477a3"
    # Same length, same charset â€” no decoration in either branch.
    assert len(live) == 8
...
def test_get_git_commit_date_empty_when_git_fails(tmp_path):
    """Docker image / pip wheel: no git â†’ '' so the dump line drops the date."""
    from zed_cli import dump
...
def test_get_git_commit_date_empty_when_git_raises(tmp_path):
    """git binary missing â†’ '' (no crash, suffix simply omitted)."""
    from zed_cli import dump
