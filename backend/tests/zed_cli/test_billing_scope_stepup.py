...
def test_step_up_requests_billing_scope_and_reuses_prior_urls(monkeypatch, _stub_persist):
    ...
    def _fake_login(**kw):
        captured.update(kw)
        # Simulate the admin ticking the box â†’ token comes back WITH the scope.
    ...
    assert captured["client_id"] == "zed-cli"


def test_step_up_returns_false_when_downscoped(monkeypatch, _stub_persist):
    # Non-admin / unticked â†’ the server silently downscopes; token comes back WITHOUT scope.
...
def test_device_login_fires_on_verification_before_polling(monkeypatch):
    ...
    # synthetic token â€” swallow it; the ordering assertion is what matters.
    try:
