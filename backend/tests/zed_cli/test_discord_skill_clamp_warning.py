...
drop was silent â€” the ``hidden`` count incremented but nothing named
...
def test_clamp_collision_emits_warning_naming_both_skills(
    ...
    """Two skills with identical first 32 chars â€” warning names both."""
    ...
    with caplog.at_level(logging.WARNING, logger="zed_cli.commands"), (
...
def test_clamp_collision_with_reserved_name_emits_distinct_warning(
    ...
    The reserved-vs-skill case is operationally different â€” the fix is
    still "rename the skill," but there's no second skill to also
    rename. The warning should say so explicitly.
    """
    from zed_cli.commands import discord_skill_commands_by_category

    # Reserved name 'help' is 4 chars â€” make a skill whose slug
