"""Tests for `zed curator status` output.
...
def curator_status_env(tmp_path, monkeypatch):
    """Isolated ZED_HOME with real agent-created skills on disk."""
    home = tmp_path / ".zed"
    skills = home / "skills"
    ...
    from zed_cli import curator as curator_cli
...
def test_status_hides_most_active_when_all_zero(curator_status_env):
    """If no skills have any activity, skip the most-active block â€” it's noise.
    ...
    # least-active still renders â€” it's part of the catalog overview
    assert "least active (top 5):" in out
