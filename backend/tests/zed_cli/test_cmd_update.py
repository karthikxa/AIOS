...
from zed_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdatePip:
    ...
    def test_update_pip_exports_virtualenv_from_sys_prefix(
        ...
        from zed_cli import main as hm

        mock_run.return_value = subprocess.CompletedProcess([], 0, stdout="", stderr="")
        monkeypatch.delenv("VIRTUAL_ENV", raising=False)
        monkeypatch.setattr(hm.sys, "prefix", "/tmp/zed-launcher-venv")
        monkeypatch.setattr(hm.sys, "base_prefix", "/usr")

        hm._cmd_update_pip(mock_args)

        assert mock_run.call_count == 1
        assert mock_run.call_args.args[0] == ["/usr/bin/uv", "pip", "install", "--upgrade", "zed-agent"]
        assert mock_run.call_args.kwargs["env"]["VIRTUAL_ENV"] == "/tmp/zed-launcher-venv"
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        "Already up to date!" â€” otherwise a fork that's caught up to its own
        origin but behind NousResearch/zed-agent silently misses updates.
        """
        from zed_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/zed-agent.git",
    ...
    def test_update_refreshes_repo_and_tui_node_dependencies(
        ...
        #   1. repo root  â€” root-only install (--workspaces=false)
        #   2. repo root  â€” workspace install (--workspace ui-tui --workspace web)
        #   3. web/       â€” npm ci --silent (if lockfile not at root)
        #                  via _build_web_ui (subprocess.run)
        #   4. web/       â€” npm run build (_run_with_idle_timeout)
        #
        ...
        # `@askjo/camofox-browser`'s browser-binary fetch) print progress â€”
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "zed_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "zed_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("zed_cli.config.check_config_version", return_value=(1, 2)), patch(
            "zed_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("zed_cli.main.sys") as mock_sys:
            ...
            from zed_cli.config import migrate_config
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        """Only the version moved â†’ apply non-interactively, never prompt."""
        with patch("shutil.which", return_value=None), patch(
            "subprocess.run"
        ) as mock_run, patch("builtins.input") as mock_input, patch(
            "zed_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "zed_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "zed_cli.config.check_config_version", return_value=(5, 24)
        ), patch(
            "zed_cli.config.migrate_config",
            ...
            assert "Updating config format (v5 â†’ v24)" in out
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "zed_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "zed_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "zed_cli.config.check_config_version", return_value=(1, 24)
        ), patch(
            "zed_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("zed_cli.main.sys") as mock_sys:
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.zed"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.zed/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.zed/profiles/work"))
        ...
        with (
            patch("zed_cli.profiles.list_profiles", return_value=all_profiles),
            patch("zed_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("zed_cli.profiles.list_profiles", return_value=[default_p]),
            patch("zed_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateBranchFlag:
    """``zed update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``zed update --check --branch <name>`` honors the branch override.
    ...
    @patch("zed_cli.config.detect_install_method", return_value="git")
    @patch("subprocess.run")
    ...
    @patch("zed_cli.config.detect_install_method", return_value="pip")
    @patch("zed_cli.banner.check_via_pypi", return_value=0)
    @patch("subprocess.run")
...
class TestCmdUpdateZipBranchRefusal:
    """``zed update --branch=<non-main>`` must refuse on the ZIP fallback path.
    ...
    fallback can't easily do. Refusing is the right move â€” silently lying
