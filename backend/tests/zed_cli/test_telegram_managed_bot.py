...
class TestGenerateBotUsername:
    def test_secure_default_format(self):
        ...
        assert name.startswith("zed_")
        assert name.endswith("_bot")
        assert len(name) == len("zed_") + 16 + len("_bot")
        assert len(name) <= 32
    ...
    def test_slug_uses_telegram_safe_base32_chars(self):
        name = generate_bot_username()
        slug = name.removeprefix("zed_").removesuffix("_bot")
        assert len(slug) == 16
...
class TestGenerateDeepLink:
    ...
    def test_name_url_encoded(self):
        link = generate_deep_link(
            ...
            suggested_name="Zed & Friends",
        )
        assert "Zed+%26+Friends" in link
...
class TestCreatePairing:
    def test_success(self):
        ...
        mock_resp.json.return_value = {
            ...
            "suggested_username": "zed_abcdefghijklmnop_bot",
            "deep_link": "https://t.me/newbot/ZedSetupBot/zed_abcdefghijklmnop_bot?name=Zed+Agent",
            "qr_payload": "https://t.me/newbot/ZedSetupBot/zed_abcdefghijklmnop_bot?name=Zed+Agent",
...
class TestPollForToken:
    def pairing(self):
        return TelegramPairing(
            ...
            poll_token="secret-token",
            suggested_username="zed_abcdefghijklmnop_bot",
            deep_link="https://t.me/newbot/ZedSetupBot/zed_abcdefghijklmnop_bot",
            qr_payload="https://t.me/newbot/ZedSetupBot/zed_abcdefghijklmnop_bot",
        )
    ...
    def test_immediate_success(self):
        ...
        mock_resp.json.return_value = {
            "bot_username": "zed_abcdefghijklmnop_bot",
            "owner_user_id": 42,
            "status": "ready",
            "token": VALID_TOKEN,
        }
...
class TestSetupTelegramAuto:
    ...
    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(setup, "get_zed_home", lambda: profile_home)
    ...
    def test_profile_name_from_home_path_handles_windows_separators(self):
        ...
        assert (
            _profile_name_from_zed_home(
                PureWindowsPath(r"C:\Users\test\AppData\Local\zed\profiles\oracle")
            )
            == "oracle"
        )
