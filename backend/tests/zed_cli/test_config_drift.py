...
def test_delegation_default_toolsets_removed_from_cli_config():
    """delegation.default_toolsets was dead config â€” never read by
    ...
    the user's ~/.zed/config.yaml over the defaults (cli.py:359-366).
    A contributor who still has the legacy key set in their own config
    would cause a false failure, and ZED_HOME patching via conftest
    doesn't help because cli._zed_home is frozen at module import time
    (cli.py:76) â€” before any autouse fixture can fire. Source inspection
