...
def _client():
    ...
    # Keep the state DB under the isolated ZED_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_zed_home):
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_zed_home):
        ...
        (get_zed_home() / "memories").mkdir(parents=True, exist_ok=True)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "zed_version"):
...
class TestSkillsHubSourcesEndpoint:
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        # requiring any live network â€” monkeypatch the router.
        class _Src:
            is_available = False
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("zed-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # zed-index source advertises availability + featured search.
            idx = _Src("zed-index")
        ...
        assert {"official", "github", "zed-index"} <= ids
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "zed_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestAdminEndpointsAuthGate:
    ...
    def _setup(self, _isolate_zed_home):
        ...
        # No session header â†’ must be rejected.
...
class TestUpdateCheckEndpoint:
    """``GET /api/zed/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/zed/update`` runs ``zed update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        r = self.client.get("/api/zed/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_up_to_date(self, monkeypatch):
        ...
        body = self.client.get("/api/zed/update/check").json()
    ...
    def test_docker_is_not_applyable(self, monkeypatch):
        ...
        body = self.client.get("/api/zed/update/check").json()
        # Docker images are immutable â€” the dashboard can't apply an update.
    ...
    def test_check_failure_is_soft(self, monkeypatch):
        ...
        # A failed check must not 500 â€” it returns behind=null with guidance.
        r = self.client.get("/api/zed/update/check")
        assert r.status_code == 200
        body = r.json()
        assert body["behind"] is None
...
class TestDebugShareEndpoint:
    ...
    def test_returns_structured_urls(self, monkeypatch):
        ...
        monkeypatch.setattr("zed_cli.dump.run_dump", lambda a: None)
    ...
    def test_default_body_redacts(self, monkeypatch):
        ...
        # No JSON body at all â€” should default redact=True.
...
class TestToolsConfigEndpoints:
    """Provider selection, API-key save, and post-setup spawn for toolsets â€”
    the dashboard surface that replicates the `zed tools` configurator."""
    ...
    def test_toolset_config_provider_matrix(self):
        # `web` has a TOOL_CATEGORIES entry â†’ providers list populated.
    ...
    def test_post_setup_spawns_action(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_zed_action", _fake_spawn)
        r = self.client.post(
