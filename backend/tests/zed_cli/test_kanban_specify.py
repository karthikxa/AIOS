"""Tests for the specifier module + `zed kanban specify` CLI surface.

The auxiliary LLM client is mocked â€” these tests don't hit any network or
...
def test_specify_task_happy_path(kanban_home):
    ...
    # Parent-free â†’ recompute_ready promotes to ready.
...
def test_specify_task_rejects_non_triage_task(kanban_home):
    ...
    # LLM must not be invoked for a non-triage task â€” fail cheap.
...
def test_specify_task_no_aux_client_configured(kanban_home):
    ...
    # Task must stay in triage â€” we never touched it.
    with kb.connect() as conn:
...
def test_list_triage_ids(kanban_home):
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="c")  # not triage â€” excluded
...
# ---------------------------------------------------------------------------
# CLI wiring â€” argparse + _cmd_specify
# ---------------------------------------------------------------------------

def _run_cli(*argv: str) -> int:
    """Invoke the `zed kanban â€¦` argparse surface directly."""
...
def test_cli_specify_single_id_success(kanban_home, capsys):
    ...
    assert tid in out
    assert "â†’ todo" in out or "-> todo" in out or "â†’" in out
...
def test_cli_specify_all_returns_1_when_every_task_fails(kanban_home, capsys):
    ...
    with patch(
        "agent.auxiliary_client.get_text_auxiliary_client",
        return_value=(None, ""),  # no aux client â†’ every task fails
    ):
        rc = _run_cli("specify", "--all")
