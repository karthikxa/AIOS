"""Regression tests for TOCTOU-safe credential file writers in ``zed_cli.auth``.
...
end up at ``0o600`` / ``0o700`` after every write. POSIX-only â€” the mode-bit
...
# ---------------------------------------------------------------------------
# _save_auth_store  (~/.zed/auth.json â€” every native OAuth provider)
# ---------------------------------------------------------------------------
...
def test_save_auth_store_writes_0o600_with_0o700_parent(tmp_path, monkeypatch):
    ...
    assert mode == 0o600, (
        f"auth.json mode 0o{mode:o} != 0o600 â€” TOCTOU race regressed"
    )
    assert parent_mode == 0o700, (
        f"auth.json parent dir mode 0o{parent_mode:o} != 0o700 â€” siblings can traverse"
    )
...
def test_save_qwen_cli_tokens_writes_0o600_with_0o700_parent(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ZED_HOME", str(tmp_path))
    # The Qwen CLI auth path lives under $HOME/.qwen by default â€” isolate it.
    ...
    assert mode == 0o600, (
        f"Qwen token file mode 0o{mode:o} != 0o600 â€” TOCTOU race regressed"
    )
    assert parent_mode == 0o700, (
...
def test_shared_nous_store_writes_0o600_with_0o700_parent(tmp_path, monkeypatch):
    ...
    # home" reference â€” which currently tracks ZED_HOME via
    # get_default_zed_root() â€” can't collide with our override and
    # falsely claim we're writing to the real user's shared store.
    monkeypatch.setenv("ZED_SHARED_AUTH_DIR", str(tmp_path / "shared_override"))
    old_umask = os.umask(0o022)
    try:
        from zed_cli import auth as auth_mod
    ...
    assert mode == 0o600, (
        f"Nous shared store mode 0o{mode:o} != 0o600 â€” TOCTOU race regressed"
    )
    assert parent_mode == 0o700, (
...
def test_save_auth_store_uses_os_open_with_0o600_mode(tmp_path, monkeypatch):
    ...
    mode so the file is created at 0o600 atomically â€” closing the TOCTOU
    ...
    for path, flags, mode in auth_tmp_opens:
        ...
        assert flags & os.O_EXCL, (
            f"auth.json temp open missing O_EXCL â€” TOCTOU-safe pattern regressed: "
            f"path={path}, flags={flags}"
        )
        # Must be exactly S_IRUSR | S_IWUSR (0o600) â€” no group/other bits.
        expected = stat.S_IRUSR | stat.S_IWUSR
        assert mode == expected, (
            f"auth.json temp open mode 0o{mode:o} != 0o{expected:o} â€” "
