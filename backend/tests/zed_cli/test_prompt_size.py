"""Tests for the ``zed prompt-size`` diagnostic (issue #34667)."""

import json

import pytest

from zed_cli.prompt_size import (
    _SKILLS_BLOCK_RE,
...
def _seed_memory(zed_home, memory_text="", user_text=""):
    mem_dir = zed_home / "memories"
...
def _seed_skill(zed_home, name, description):
    skill_dir = zed_home / "skills" / "demo" / name
...
def test_runs_offline_without_credentials(isolated_home, monkeypatch):
    """No provider credentials configured â†’ still produces a breakdown."""
    for var in ("OPENROUTER_API_KEY", "OPENAI_API_KEY", "ZED_API_KEY",
                "ANTHROPIC_API_KEY"):
...
def test_render_breakdown_is_plain_text(isolated_home):
    ...
    # Plain text â€” no JSON braces leaking in.
