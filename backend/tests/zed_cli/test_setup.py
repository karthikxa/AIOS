...
from zed_cli.setup import setup_model_provider
...
def test_setup_cancel_preserves_existing_config(tmp_path, monkeypatch):
    ...
    def fake_select():
        pass  # user cancelled â€” nothing written to disk
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("zed_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("zed_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "zed_cli.main._model_flow_named_custom",
...
def test_select_provider_and_model_accepts_named_provider_from_providers_section(
    ...
    monkeypatch.setattr(
        "zed_cli.main._prompt_provider_choice",
...
def test_modal_setup_can_use_nous_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("zed_cli.setup.managed_nous_tools_enabled", lambda: True)
    monkeypatch.setenv("ZED_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr("zed_cli.setup.prompt", fake_prompt)
    monkeypatch.setattr("zed_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
    ...
    from zed_cli.setup import setup_terminal_backend
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zed_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("zed_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("zed_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "zed_cli.setup.get_nous_subscription_features",
        lambda config: type("Features", (), {"nous_auth_present": True})(),
