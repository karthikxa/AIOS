"""Tests for ``zed_cli.voice`` â€” the TUI gateway's voice wrapper.
...
class TestNormalizeVoiceRecordKeyForPromptToolkit:
    ...
    def test_ctrl_and_alt_map_to_prompt_toolkit_form(self):
        from zed_cli.voice import normalize_voice_record_key_for_prompt_toolkit
    ...
    def test_typoed_named_keys_fall_back_to_default(self):
        ...
        prompt_toolkit would reject it at startup â€” fall back instead."""
        from zed_cli.voice import normalize_voice_record_key_for_prompt_toolkit
    ...
    # parser rejects alt+c/d/l because zed-ink reports Alt as
...
class TestVoiceRecordKeyFromConfig:
    ...
    (``normalize_â€¦`` / ``format_â€¦``) surfaces the documented default.
    """

    def test_dict_voice_with_string_record_key(self):
        from zed_cli.voice import voice_record_key_from_config
    ...
    def test_normalizer_accepts_extractor_output_directly(self):
        """voice_record_key_from_config + normalize_â€¦ must compose â€”
...
class TestContinuousAPI:
    """Continuous (VAD) mode API â€” CLI-parity loop entry points."""
    ...
    def test_stop_continuous_idempotent_when_inactive(self, monkeypatch):
        """stop_continuous must not raise when no loop is active â€” the
    ...
    def test_double_start_is_idempotent(self, monkeypatch):
        """A second start_continuous while already active is a no-op â€” prevents
...
class TestContinuousLoopSimulation:
    ...
    Proves auto-restart works: the silence callback must trigger transcribe â†’
    on_transcript â†’ re-call rec.start(on_silence_stop=same_cb). Also covers
    ...
    def test_silent_limit_halts_loop_after_three_strikes(self, fake_recorder, monkeypatch):
        ...
        # Transcription returns no speech â€” fake_recorder.stop() returns the
    ...
    def test_stop_during_transcription_discards_restart(self, fake_recorder, monkeypatch):
        ...
        # Loop is stopped â€” no auto-restart
