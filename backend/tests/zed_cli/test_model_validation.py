"""Tests for provider-aware `/model` validation in zed_cli.models."""
...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("zed_cli.models.fetch_api_models", return_value=api_models), \
         patch("zed_cli.models.probe_api_models", return_value=probe_payload):
...
class TestParseModelInput:
    ...
    def test_nous_provider_switch(self):
        provider, model = parse_model_input("nous:zed-3", "openrouter")
    ...
    def test_custom_triple_empty_model_falls_back(self):
        """custom:name: with no model â†’ treated as custom:name (bare)."""
        provider, model = parse_model_input("custom:name:", "openrouter")
        # Empty model after second colon â†’ no triple match, falls through
        assert provider == "custom"
        assert model == "name:"
...
class TestProviderModelIds:
    ...
    def test_stepfun_prefers_live_catalog(self):
        with patch(
            ...
            "zed_cli.models.fetch_api_models",
    ...
    def test_copilot_prefers_live_catalog(self):
        with patch("zed_cli.auth.resolve_api_key_provider_credentials", return_value={"api_key": "gh-token"}), \
             patch("zed_cli.models._fetch_github_models", return_value=["gpt-5.4", "claude-sonnet-4.6"]):
    ...
    def test_anthropic_provider_uses_configured_base_url_for_live_catalog(self):
        ...
        with patch(
            ...
            "zed_cli.models.urllib.request.urlopen",
            return_value=_Resp(),
        ) as mock_urlopen:
...
class TestFetchApiModels:
    ...
    def test_returns_none_on_network_error(self):
        with patch("zed_cli.models.urllib.request.urlopen", side_effect=Exception("timeout")):
    ...
    def test_probe_api_models_tries_v1_fallback(self):
        ...
        with patch("zed_cli.models.urllib.request.urlopen", side_effect=_fake_urlopen):
    ...
    def test_probe_api_models_uses_copilot_catalog(self):
        ...
        with patch("zed_cli.models.urllib.request.urlopen", return_value=_Resp()) as mock_urlopen:
...
class TestCopilotNormalization:
    ...
    def test_copilot_api_mode_gpt5_mini_uses_chat(self):
        """gpt-5-mini is the exception â€” uses Chat Completions."""
...
# -- validate â€” API unreachable â€” soft-accept via catalog or warning --------

class TestValidateApiFallback:
    """When /models is unreachable, the validator must accept the model (with
    a warning) rather than reject it outright â€” otherwise provider switches
    ...
    def test_known_model_accepted_via_catalog_when_api_down(self):
        ...
        with patch(
            "zed_cli.models.provider_model_ids",
    ...
    def test_unknown_provider_soft_accepted_when_api_down(self):
        # No catalog for unknown providers â€” soft-accept with a Note.
        with patch("zed_cli.models.provider_model_ids", return_value=[]):
    ...
    def test_custom_endpoint_warns_with_probed_url_and_v1_hint(self):
        ...
        # Unreachable /models on a custom endpoint no longer hard-rejects â€”
    ...
    def test_fetch_lmstudio_models_filters_embedding_type(self):
        ...
        with patch("zed_cli.models.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_fetch_lmstudio_models_raises_auth_error_on_401(self):
        ...
        with patch("zed_cli.models.urllib.request.urlopen", side_effect=http_error):
...
# -- validate â€” Codex auto-correction ------------------------------------------
...
class TestValidateCodexAutoCorrection:
    ...
    def test_missing_dash_auto_corrects(self):
        ...
        with patch("zed_cli.models.provider_model_ids", return_value=codex_models):
...
# -- probe_api_models â€” Cloudflare UA mitigation --------------------------------

class TestProbeApiModelsUserAgent:
    """Probing custom /v1/models must send a Zed User-Agent.
    ...
    "Could not reach the ... API to validate ..." error â€” even though the
    ...
    def test_probe_sends_zed_user_agent(self):
        ...
        assert ua.startswith("zed-cli/"), (
            f"User-Agent must advertise zed-cli, got {ua!r}"
        )
        # Must not fall back to urllib's default â€” that's what Cloudflare 1010 blocks.
    ...
    def test_probe_user_agent_sent_without_api_key(self):
        ...
        assert ua and ua.startswith("zed-cli/")
