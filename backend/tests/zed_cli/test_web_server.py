...
# tests/fixtures/ so the bundled-plugins directory stays clean â€” stock
...
@pytest.fixture
def _install_example_plugin(_isolate_zed_home):
    """Drop the example-dashboard fixture into the per-test ZED_HOME
    ...
    ``ZED_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins â€” kanban, zed-achievements, model providers
    â€” being available, and globally swapping that root would yank them
    ...
    #   2. Restore the original list on teardown â€” otherwise leftover
    #      ``/api/plugins/example/*`` routes leak into subsequent tests
    #      and start serving requests against a torn-down ZED_HOME.
    app = web_server.app
    ...
    # ``app.router.routes``. That works fine at import time â€” the SPA
    # catch-all ``mount_spa(app)`` registers AFTER the initial mount
    # call â€” but when we mount mid-flight the catch-all is already in
    ...
    try:
        ...
        # Restore the original route list â€” drops the example plugin's
        # routes so the next test sees a clean app â€” and clear the
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Zed vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Zed env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestSessionTokenInjection:
    """The desktop shell mints ZED_DASHBOARD_SESSION_TOKEN and signs its
    ...
    dropped this read â€” this guards the contract, not a literal value.
    """
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_zed_home):
        """Create a TestClient and isolate the state DB under the test ZED_HOME."""
        try:
    ...
    def test_get_memory_provider_config_returns_safe_defaults(self):
        ...
        assert fields["bank_id"]["value"] == "zed"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        ...
        config_path = get_zed_home() / "hindsight" / "config.json"
    ...
    # â”€â”€ GET /api/media (remote image display) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Dashboard font override â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def test_update_zed_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn zed update")

        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_zed_action", fail_spawn)
        web_server._ACTION_PROCS.pop("zed-update", None)
        web_server._ACTION_RESULTS.pop("zed-update", None)

        resp = self.client.post("/api/zed/update")
    ...
    def test_update_zed_returns_managed_runtime_guidance_without_spawning(self, monkeypatch):
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("managed runtime update guard should not spawn zed update")
    ...
    def test_update_zed_spawns_on_non_docker_install(self, monkeypatch):
        ...
        monkeypatch.setattr(web_server, "_spawn_zed_action", fake_spawn)
        web_server._ACTION_PROCS.pop("zed-update", None)
        web_server._ACTION_RESULTS.pop("zed-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "zed-update"}
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("zed-update", None)
        web_server._ACTION_RESULTS.pop("zed-update", None)
        web_server._ACTION_PROCS["zed-update"] = proc

        resp = self.client.get("/api/actions/zed-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "zed-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["zed-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_get_env_vars_surfaces_catalog_providers(self):
        ...
        Regression for the GUIâ‡„CLI drift: openai-api, kilocode, novita,
        ...
        for d in provider_catalog():
            ...
            # existing tool category and not hijacked â€” see the copilot test.)
    ...
    def test_model_set_normalizes_vendor_slug_for_native_provider(self, monkeypatch):
        """'Use as â†’ Main' with an OpenRouter slug + native provider must not
    ...
    def test_ops_import_passes_force_flag(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_zed_action", fake_spawn)
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "Zed Agent"}
        assert kwargs["headers"]["Accept"] == "application/json"
        assert kwargs["headers"]["Authorization"] == "Bearer poll-secret"
        assert kwargs["headers"]["Content-Type"] == "application/json"
        assert kwargs["headers"]["User-Agent"].startswith("ZedDashboard/")
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "suggested_username": "zed_pair123_bot",
                "deep_link": "https://t.me/newbot/ZedSetupBot/zed_pair123_bot",
                "qr_payload": "https://t.me/newbot/ZedSetupBot/zed_pair123_bot",
    ...
    def test_telegram_onboarding_ready_and_apply_never_returns_bot_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "zed_pair_ready_bot",
                    "deep_link": "https://t.me/newbot/ZedSetupBot/zed_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/ZedSetupBot/zed_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "zed_pair_ready_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_zed_action", fake_spawn_action)
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "zed_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/ZedSetupBot/zed_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/ZedSetupBot/zed_pair_restart_fails_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "zed_pair_restart_fails_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
        ...
        monkeypatch.setattr(ws, "_spawn_zed_action", fail_spawn_action)
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "zed_pair_reuse_bot",
                    "deep_link": "https://t.me/newbot/ZedSetupBot/zed_pair_reuse_bot",
                    "qr_payload": "https://t.me/newbot/ZedSetupBot/zed_pair_reuse_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            return {
                "status": "ready",
                "bot_username": "zed_pair_reuse_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "zed_pair_waiting_bot",
                "deep_link": "https://t.me/newbot/ZedSetupBot/zed_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/ZedSetupBot/zed_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "zed_pair_cancel_bot",
                "deep_link": "https://t.me/newbot/ZedSetupBot/zed_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/ZedSetupBot/zed_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/zed_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_spa_assets_are_read_as_utf8(self, monkeypatch, tmp_path):
        ...
        css_resp = spa_client.get("/assets/app.css", headers={"x-forwarded-prefix": "/zed"})
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "nous", "model": "zed-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        # Switching providers (custom â†’ openrouter) â†’ stale base_url cleared.
        out = _apply_main_model_assignment(
        ...
        # Same provider, no new base_url â†’ existing custom endpoint preserved.
        ...
        # Switching providers without a base_url â†’ don't invent one, clear stale.
        out = _apply_main_model_assignment(
        ...
        # same provider, no new key â†’ existing key preserved (re-picking a model
        ...
        # switching providers without a new key â†’ stale key cleared.
        out = _apply_main_model_assignment(
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        # Non-success / malformed / exception â†’ [] (never raises).
    ...
    def test_set_model_main_reports_stale_auxiliary_pins(self):
        ...
        cfg["auxiliary"] = {
            # Pinned to nous â€” same as the OLD main, becomes stale after switch.
            "compression": {"provider": "nous", "model": "anthropic/claude-sonnet-4.6"},
            # Auto â€” follows main, never stale.
            "vision": {"provider": "auto", "model": ""},
            # Pinned to a third provider â€” also stale vs the new main.
...
class TestNewEndpoints:
    ...
    def test_cron_blueprints_list(self):
        ...
        assert first["appUrl"].startswith("zed://")
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        ...
        (zed_home / "config.yaml").write_text(
            "model:\n  provider: openrouter\n  name: anthropic/claude-sonnet-4.6\n",
            encoding="utf-8",
        )
        named = zed_home / "profiles" / "multi-agent"
    ...
    def test_profiles_create_creates_wrapper_alias_when_safe(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(profiles_mod.shutil, "which", lambda name: "/opt/zed/bin/zed")
    ...
    def test_profiles_create_with_clone_from_copies_source_skills(self, monkeypatch):
        ...
        default_skill = get_zed_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_root = get_zed_home() / "profiles" / "cloned"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_zed_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_zed_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_clone_all_from_named_source(self, monkeypatch):
        ...
        source_dir = get_zed_home() / "profiles" / "full-src"
        ...
        assert resp.status_code == 200
        target_dir = get_zed_home() / "profiles" / "full-copy"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_zed_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        prof_dir = get_zed_home() / "profiles" / "builder"
        token = set_zed_home_override(str(prof_dir))
        try:
            cfg = load_config()
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_zed_home() / "profiles" / "model-prof" / "config.yaml"
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() â€” scans ~/.zed/dashboard-themes/."""
...
class TestDeleteSessionEndpoint:
    """Tests for ``DELETE /api/sessions/{session_id}`` â€” the single-row delete
    ...
    empty-session hygiene, or removed by a concurrent client â€” both common amid
    ...
    def test_delete_absent_session_is_idempotent(self):
        ...
        # 404 â€” the desktop would resurrect the ghost row and show
...
class TestBulkDeleteSessionsEndpoint:
    """Tests for ``POST /api/sessions/bulk-delete`` â€” backs the
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``zed_cli/web_server.py``).
    2. Behaviour parity with :meth:`SessionDB.delete_sessions` â€” real
    ...
    def test_unknown_ids_silently_skipped(self):
        """The endpoint never 404s on a missing ID â€” it returns the
    ...
    def test_empty_list_is_noop(self):
        """``ids: []`` returns ``deleted: 0`` (200, not 400) â€” the UI
    ...
    def test_payload_cap_enforced(self):
        """501 IDs returns 400 â€” a hard cap stops a runaway selection
        ...
        # deleted=0 â€” but it's not the cap path).
        resp = self.auth_client.post(
            "/api/sessions/bulk-delete",
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "being shadowed by /api/sessions/{session_id} â€” check "
            "registration order in zed_cli/web_server.py."
        )


class TestDeleteEmptySessionsEndpoint:
    """Tests for ``GET /api/sessions/empty/count`` and
    ``DELETE /api/sessions/empty`` â€” the bulk-delete endpoints backing
    ...
    def _seed(self):
        ...
        * ``empty1`` / ``empty2`` â€” ended, no messages â†’ should delete
        * ``hasmsg``  â€” ended, has one message â†’ must survive
    ...
    def test_delete_endpoint_requires_auth(self):
        ...
        Regression guard for issue #19533 â€” the bulk-delete is a strictly
    ...
    def test_count_returns_only_empty_ended_unarchived(self):
        """With the standard corpus, the count is exactly 2 â€” only
    ...
    def test_delete_with_no_empties_returns_zero(self):
        """No empty sessions â†’ endpoint returns ``deleted: 0`` (200,
    ...
    def test_route_order_empty_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "{session_id} route â€” check registration order in "
            "zed_cli/web_server.py."
        )
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_zed_home, _install_example_plugin):
        """Create a TestClient without the session token header.

        Pulls in ``_install_example_plugin`` so ``test_plugin_route_allows_auth``
        has the ``/api/plugins/example/hello`` endpoint available â€” the
        example plugin is no longer a bundled plugin, so the fixture
        installs it into the per-test ``ZED_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into ZED_HOME by the class-level
        ``_install_example_plugin`` fixture) â€” a stable, side-effect-free
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface â€” kanban, zed-achievements, future
        plugins â€” must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (zed-achievements is loaded by default).
        resp = self.client.get("/api/plugins/zed-achievements/overview")
        assert resp.status_code == 401
...
# ---------------------------------------------------------------------------
# /api/pty WebSocket â€” terminal bridge for the dashboard "Chat" tab.
#
# These tests drive the endpoint with a tiny fake command (typically ``cat``
# or ``sh -c 'printf â€¦'``) instead of the real ``zed --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        assert env["ZED_TUI_DASHBOARD"] == "1"
        assert env["ZED_TUI_INLINE"] == "1"
        assert env["ZED_TUI_DISABLE_MOUSE"] == "1"
    ...
    def test_resolve_chat_argv_applies_terminal_backend_config(
        ...
        config_path.write_text(
            "\n".join(
                [
                    "terminal:",
                    "  backend: docker",
                    "  docker_image: example/zed-tools:latest",
                    "  docker_extra_args:",
                    "    - --network=host",
                ]
            ),
        ...
        assert env["TERMINAL_DOCKER_IMAGE"] == "example/zed-tools:latest"
    ...
    def _assert_pty_propagates(self, monkeypatch, raising_resolver, *, profile=None, expect_detail=None):
        ...
        contract â€” we assert the close code, not the exact notice wording)."""
        from starlette.websockets import WebSocketDisconnect
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"zed-ws-ok" in buf:
                    break
            assert b"zed-ws-ok" in buf

    def test_client_input_reaches_child_stdin(self, monkeypatch):
        # ``cat`` echoes stdin back, so a write â†’ read round-trip proves
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        ZED_TUI_SIDECAR_URL env var pointing back at /api/pub on the
        same channel â€” which is how tool events reach the dashboard sidebar."""
        captured: dict = {}
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        subscriber registered on that channel â€” and not to subscribers on
        other channels.
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("ZED_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    """``/dashboard-plugins/<name>/<path>`` is unauthenticated by design â€”
    ...
    def _setup_test_client(self, monkeypatch, _isolate_zed_home, _install_example_plugin):
        ...
        The static-asset allowlist tests need a plugin to point at â€”
        ...
        the per-test ``ZED_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_python_source_is_404(self):
        ...
        dashboard directory. Suffix not in the allowlist â†’ 404."""
    ...
    def test_manifest_json_still_served(self):
        """JSON files remain browser-fetchable â€” manifests, localized
    ...
    def test_unknown_plugin_is_404(self):
        """Existing behaviour preserved: nonexistent plugin name â†’ 404."""
        resp = self.client.get(
    ...
    def test_path_traversal_still_blocked(self):
        ...
        ``is_relative_to()`` check â€” a ``.js`` named file at an
...
class TestValidateProviderCredential:
    ...
    def test_unknown_provider_is_not_validated(self):
        # No probe for this key â†’ don't block (ok True, reachable False).
    ...
    def test_local_endpoint_without_key_sends_no_auth_header(self, monkeypatch):
        """No key â†’ no Authorization header (keyless local servers unaffected)."""
        captured = {}
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_zed_home):
        import threading
        ...
        with self._client():
            assert called.wait(3.0), "expected cron tick under ZED_DESKTOP=1"

    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_zed_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.delenv("ZED_DESKTOP", raising=False)
