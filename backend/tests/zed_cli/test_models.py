"""Tests for the zed_cli models module."""
...
import zed_cli.models as _models_mod
...
class TestModelIds:
    def test_returns_non_empty_list(self):
        with patch("zed_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
            ids = model_ids()
...
class TestFetchOpenRouterModels:
    def test_live_fetch_recomputes_free_tags(self, monkeypatch):
        ...
        with patch("zed_cli.models.urllib.request.urlopen", return_value=_Resp()):
    ...
    def test_falls_back_to_static_snapshot_on_fetch_failure(self, monkeypatch):
        monkeypatch.setattr(_models_mod, "_openrouter_catalog_cache", None)
        with patch("zed_cli.models.urllib.request.urlopen", side_effect=OSError("boom")):
            models = fetch_openrouter_models(force_refresh=True)
    ...
    def test_filters_out_models_without_tool_support(self, monkeypatch):
        ...
        zed-agent is tool-calling-first â€” surfacing a non-tool model leads to
        ...
        class _Resp:
            ...
            def read(self):
                # opus-4.6 advertises tools â†’ kept
                # nano-image has explicit supported_parameters that OMITS tools â†’ dropped
                # qwen3.7-max advertises tools â†’ kept
                return (
                    b'{"data":['
        ...
        # Image-only model advertised supported_parameters WITHOUT tools â†’ must be dropped.
    ...
    def test_permissive_when_supported_parameters_missing(self, monkeypatch):
        ...
        as 'unknown â†’ allow' prevents the picker from silently emptying on
        those gateways.
        """
        class _Resp:
            def __enter__(self):
...
class TestOpenRouterToolSupportHelper:
    ...
    def test_tools_in_supported_parameters(self):
        from zed_cli.models import _openrouter_model_supports_tools
    ...
    def test_supported_parameters_absent_is_permissive(self):
        """Missing field â†’ allow (so older / non-OR gateways still work)."""
        from zed_cli.models import _openrouter_model_supports_tools
    ...
    def test_supported_parameters_malformed_is_permissive(self):
        """Malformed (non-list) value â†’ allow rather than silently drop."""
        from zed_cli.models import _openrouter_model_supports_tools
        assert _openrouter_model_supports_tools(
    ...
    def test_empty_supported_parameters_list_drops_model(self):
        """Explicit empty list â†’ no tools â†’ drop."""
        from zed_cli.models import _openrouter_model_supports_tools
        assert _openrouter_model_supports_tools(
...
class TestFindOpenrouterSlug:
    def test_exact_match(self):
        from zed_cli.models import _find_openrouter_slug
        with patch("zed_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
...
class TestIsNousFreeTier:
    """Tests for is_nous_free_tier â€” account tier detection."""
    ...
    def test_plan_name_alone_not_free(self):
        """Plan name alone is not enough â€” monthly_charge is required."""
...
class TestPartitionNousModelsByTier:
    """Tests for partition_nous_models_by_tier â€” free vs paid tier model split."""
...
class TestUnionWithPortalFreeRecommendations:
    ...
    what's free *right now* â€” the in-repo curated list and docs-hosted
    ...
    def test_adds_portal_free_model_missing_from_curated(self):
        ...
        with patch(
            "zed_cli.models.fetch_nous_recommended_models",
    ...
    def test_empty_payload_returns_inputs_unchanged(self):
        ...
        with patch("zed_cli.models.fetch_nous_recommended_models", return_value={}):
            ids, p = union_with_portal_free_recommendations(curated, pricing, "")
        assert ids == curated
        assert p == pricing
...
class TestUnionWithPortalPaidRecommendations:
    ...
    docs-hosted manifest can lag â€” this helper guarantees newly-launched
    ...
    def test_invalid_entries_skipped(self):
        ...
        # No synthetic entry â€” pricing is untouched.
        assert "openai/gpt-5.4" not in p
...
class TestCheckNousFreeTierCache:
    ...
    @patch("zed_cli.nous_account.get_nous_portal_account_info")
...
class TestNousRecommendedModels:
    ...
    def test_fetch_caches_per_portal_url(self):
        from zed_cli.models import fetch_nous_recommended_models
    ...
    def test_fetch_cache_is_keyed_per_portal(self):
        ...
        with patch("urllib.request.urlopen", return_value=mock_cm) as mock_urlopen:
            ...
            fetch_nous_recommended_models("https://portal.staging-zedteam.com")
        assert mock_urlopen.call_count == 2  # different portals â†’ separate fetches
    ...
    def test_get_aux_model_returns_vision_recommendation(self):
        ...
        with patch(
            ...
            # Free tier â†’ free vision recommendation.
    ...
    def test_paid_tier_prefers_paid_recommendation(self):
        ...
        with patch("zed_cli.models.fetch_nous_recommended_models", return_value=payload):
    ...
    def test_free_tier_never_uses_paid_recommendation(self):
        ...
        with patch("zed_cli.models.fetch_nous_recommended_models", return_value=payload):
            model = get_nous_recommended_aux_model(vision=False, free_tier=True)
        # Free tier must return None â€” never leak the paid model.
        assert model is None
    ...
    def test_auto_detects_tier_when_not_supplied(self):
        ...
        with (
            patch("zed_cli.models.fetch_nous_recommended_models", return_value=payload),
            patch("zed_cli.models.check_nous_free_tier", return_value=True),
        ):
            assert get_nous_recommended_aux_model(vision=False) == "free-model"
        with (
            patch("zed_cli.models.fetch_nous_recommended_models", return_value=payload),
            patch("zed_cli.models.check_nous_free_tier", return_value=False),
        ):
    ...
    def test_tier_detection_error_defaults_to_paid(self):
        ...
        with (
            patch("zed_cli.models.fetch_nous_recommended_models", return_value=payload),
            patch("zed_cli.models.check_nous_free_tier", side_effect=RuntimeError("boom")),
        ):
            assert get_nous_recommended_aux_model(vision=False) == "paid-model"
