"""Tests for the Zed plugin system (zed_cli.plugins)."""
...
from zed_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<zed_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<zed_home>/plugins/``; we derive
    ``<zed_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <ZED_HOME>/config.yaml.
        # Config is always read from ZED_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        zed_home_str = os.environ.get("ZED_HOME")
        if zed_home_str:
            zed_home = Path(zed_home_str)
        else:
            zed_home = base.parent
        zed_home.mkdir(parents=True, exist_ok=True)
        cfg_path = zed_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
# â”€â”€ TestPluginDiscovery â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.zed/plugins/ are discovered."""
        plugins_dir = tmp_path / "zed_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("ZED_HOME", str(tmp_path / "zed_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZED_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".zed" / "plugins"
    ...
    def test_discover_is_idempotent(self, tmp_path, monkeypatch):
        ...
        # Filter out bundled plugins â€” they're always discovered.
        non_bundled = {
...
# â”€â”€ TestPluginLoading â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under zed_plugins.<name>."""
        plugins_dir = tmp_path / "zed_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("ZED_HOME", str(tmp_path / "zed_test"))

        # Clean up any prior namespace module
        sys.modules.pop("zed_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "zed_plugins.ns_plugin" in sys.modules
    ...
    def test_user_memory_plugin_auto_coerced_to_exclusive(self, tmp_path, monkeypatch):
        ...
        PluginManager â€” they belong to plugins/memory discovery.
        ...
        plugin_dir.mkdir(parents=True)
        # No explicit `kind:` â€” the heuristic should kick in.
    ...
    def test_explicit_standalone_kind_not_coerced(self, tmp_path, monkeypatch):
        ...
        manifest, the memory-provider heuristic must NOT override it â€”
...
# â”€â”€ TestPluginHooks â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="zed_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
# â”€â”€ TestPluginContext â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestPluginContext:
    ...
    def test_register_tool_rejects_shadow_without_override(self, tmp_path, monkeypatch, caplog):
        ...
        try:
            ...
            # Original handler must still be in place â€” registration was rejected.
...
# â”€â”€ TestPluginToolVisibility â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ TestPluginManagerList â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `zed plugins list`. Attribution now counts what each plugin's own
...
# â”€â”€ TestPluginCommands â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("zed_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "zed_test" / "plugins"
        # Set ZED_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("zed_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("zed_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("zed_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)

        with pytest.raises(TimeoutError):
            resolve_plugin_command_result(_slow_handler())


# â”€â”€ TestPluginDispatchTool â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


class TestPluginDispatchTool:
    """Tests for PluginContext.dispatch_tool() â€” tool dispatch with agent context."""
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("zed_cli.plugins.PluginContext.dispatch_tool.__module__", "zed_cli.plugins"):
...
class TestPluginDebugLogging:
    """ZED_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("ZED_PLUGINS_DEBUG", raising=False)
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With ZED_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("ZED_PLUGINS_DEBUG", "1")
        from zed_cli import plugins as plugins_mod
