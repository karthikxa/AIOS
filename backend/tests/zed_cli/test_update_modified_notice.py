"""Guard: every `zed update` path that reports user-modified skills must
also tell the user how to find them.

`zed update` keeps (does not overwrite) bundled skills the user edited and
prints a ``~ N user-modified (kept)`` count. There are two independent update
code paths in ``zed_cli/main.py`` that print this notice (the git-pull path
in ``_cmd_update_impl`` and the unpack/install path). Both must point the user
at ``zed skills list-modified`` so the count is actionable â€” otherwise,
...
snapshot: it asserts the relationship "count line â‡’ discovery hint", so it
...
_HINT_RE = re.compile(r"zed skills list-modified")
...
def test_every_user_modified_notice_points_at_list_modified():
    ...
    assert count_sites, (
        "no 'user-modified (kept)' notice found in main.py â€” the update "
