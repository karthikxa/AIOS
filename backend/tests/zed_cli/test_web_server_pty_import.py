"""Test the platform-branched PTY bridge import in zed_cli.web_server.

The /api/pty WebSocket handler in web_server.py picks its bridge at import
time via ``sys.platform.startswith("win")`` â€” Windows gets the ConPTY
backend, POSIX gets the fcntl/termios one.  Both branches must:
  ...
  3. Never raise at import time when the platform-native dependency is
     missing â€” the dashboard's non-chat tabs must keep loading.
...
def test_web_server_exposes_pty_bridge_symbols():
    ...
    # PtyUnavailableError is always an exception class â€” either the real
...
def test_web_server_uses_win_pty_bridge_on_windows():
    ...
    from zed_cli.win_pty_bridge import WinPtyBridge
    ...
    from zed_cli.win_pty_bridge import PtyUnavailableError as WinErr
...
def test_web_server_uses_posix_pty_bridge_on_posix():
    ...
    from zed_cli.pty_bridge import PtyBridge as PosixBridge
    from zed_cli.pty_bridge import PtyUnavailableError as PosixErr
...
def test_pty_bridge_import_block_is_platform_branched():
    ...
    fails the same way on every OS â€” the runtime symbol checks above can
    ...
    #       try:
    #           from zed_cli.win_pty_bridge import WinPtyBridge as PtyBridge, ...
    #       except ImportError:
    #           PtyBridge = None
    #           ...
    #   else:
    ...
    assert "from zed_cli.win_pty_bridge import" in src
    assert "from zed_cli.pty_bridge import" in src
