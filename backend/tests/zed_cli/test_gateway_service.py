...
class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "zed-gateway.service"
    ...
    def test_systemd_stop_timeout_prints_status_guidance(self, monkeypatch, capsys):
        ...
        assert "zed gateway status" in output

    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`zed gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `zed gateway restart` in the background; when a
        ...
        def fake_run_systemctl(args, **kwargs):
            # reset-failed is a pre-step (check=False, 30s) â€” let it pass.
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `zed gateway restart`)."""
        unit_path = tmp_path / "zed-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``ZED_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../zed_test`` ZED_HOME â€” silently breaking
        ...
        # Realistic generated unit referencing a pytest tmpdir ZED_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="ZED_HOME=/tmp/pytest-of-alice/pytest-42/'
            'popen-gw0/test_x/zed_test"\n'
        )
        monkeypatch.setattr(
            gateway_cli,
        ...
        # If the guard fails, daemon-reload would be called â€” record it.
        ran = []
    ...
    def test_refresh_refuses_to_bake_any_tempdir_home_into_real_user_unit(
        ...
        """Structural guard: a manual E2E ZED_HOME like
        ``/tmp/zed-e2e-41264`` carries none of the pytest markers but
        poisons the unit identically (seen live 2026-06-11 â€” an E2E probe ran
        ``zed gateway restart`` with a /tmp ZED_HOME exported; the
        ...
        must refuse ANY temp-dir ZED_HOME, not just pytest-shaped ones.
        """
        unit_path = tmp_path / "zed-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
        ...
        polluted_unit = (
            "[Service]\n"
            'Environment="ZED_HOME=/tmp/zed-e2e-41264"\n'
            "WorkingDirectory=/tmp/zed-e2e-41264\n"
        )
        monkeypatch.setattr(
            gateway_cli,
            "generate_systemd_unit",
...
class TestTempHomeServiceDefinitionGuard:
    """_temp_home_in_service_definition() â€” structural temp-dir detection."""
    ...
    def test_detects_var_tmp_home(self):
        unit = '[Service]\nEnvironment="ZED_HOME=/var/tmp/zed-x"\n'
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="ZED_HOME={tmp_path}/zed-home"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is not None

    def test_detects_tmp_home_in_launchd_plist(self):
        plist = (
            "<dict>\n  <key>ZED_HOME</key>\n"
            "  <string>/tmp/zed-e2e-99999</string>\n</dict>\n"
        )
        assert (
    ...
    def test_tmp_prefixed_non_temp_path_is_accepted(self):
        # /tmpfs-data is NOT under /tmp â€” prefix matching must be
        # component-wise, not string startswith.
        unit = '[Service]\nEnvironment="ZED_HOME=/tmpfs-data/.zed"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is None
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "zed gateway install" in out
...
class TestGeneratedSystemdUnits:
    ...
    def test_user_unit_avoids_recursive_execstop_and_uses_extended_stop_timeout(self, monkeypatch):
        ...
        # (tool subprocess kill, adapter disconnect) runs â€” issue #8202.
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_zed_home_for_target_user", lambda home: "/home/alice/.zed")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        monkeypatch.delenv("ZED_RESTART_DRAIN_TIMEOUT", raising=False)
        ...
        monkeypatch.setenv("ZED_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("ZED_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.zed.gateway.plist"
        plist_path.write_text("<plist>old content</plist>", encoding="utf-8")

        monkeypatch.setattr(gateway_cli, "get_launchd_plist_path", lambda: plist_path)
        # Patch the generator with synthetic content carrying a real-looking
        # home â€” the temp-home guard refuses to write plists whose
        # ZED_HOME resolves under the (pytest tmp) test ZED_HOME.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist>--replace\n<key>ZED_HOME</key>"
                "<string>/Users/alice/.zed</string></plist>"
            ),
        )
        ...
        monkeypatch.setattr(gateway_cli.subprocess, "run", fake_run)
        # Not running inside the gateway tree â†’ direct bootout/bootstrap path.
    ...
    def test_refresh_uses_direct_reload_when_not_inside_gateway_tree(self, tmp_path, monkeypatch):
        ...
        assert result is True
        # No detached helper â€” direct path taken.
    ...
    def test_launchd_stop_tolerates_already_unloaded(self, monkeypatch, capsys):
        ...
        # Should not raise â€” exit code 3 means already unloaded
        gateway_cli.launchd_stop()
    ...
    def test_launchctl_domain_unsupported_recognizes_macos26_codes(self):
        # Codes that persist after a fresh bootstrap â†’ launchd truly unavailable.
    ...
    def test_launchd_start_reloads_on_kickstart_exit_code_125(self, tmp_path, monkeypatch):
        """Exit code 125 means the job is absent from the domain â†’ bootstrap recovery."""
        plist_path = tmp_path / "ai.zed.gateway.plist"
        plist_path.write_text(gateway_cli.generate_launchd_plist(), encoding="utf-8")
        label = gateway_cli.get_launchd_label()
    ...
    def test_launchd_install_falls_back_to_detached_on_bootstrap_5(self, tmp_path, monkeypatch, capsys):
        ...
        # (which would trip on the pytest-tmp test ZED_HOME) stays out of
        # the way â€” this test exercises the bootstrap-error fallback.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist><key>ZED_HOME</key>"
                "<string>/Users/alice/.zed</string></plist>"
            ),
        )
    ...
    def test_launchd_fallback_exits_when_spawn_fails(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "nohup zed gateway run" in out
...
class TestLaunchdDomainDetection:
    ...
    def test_prefers_gui_domain_when_service_loaded_there(self, monkeypatch):
        ...
        _launchd_domain() must return ``gui/<uid>`` â€” not ``user/<uid>``."""
...
class TestGatewaySystemServiceRouting:
    ...
    def test_systemd_status_surfaces_planned_restart_failure(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_zed_units", lambda: False)
        monkeypatch.setattr(gateway_cli, "systemd_unit_is_current", lambda system=False: True)
        monkeypatch.setattr(gateway_cli, "_runtime_health_lines", lambda: ["âš  Last shutdown reason: Gateway restart requested"])
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: zed gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup zed gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.zed.gateway"])
            ),
        )
...
class TestSystemUnitZedHome:
    """ZED_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'ZED_HOME=/home/alice/.zed' in unit
        assert '/root/.zed' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: ZED_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("ZED_HOME", "/root/.zed/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'ZED_HOME=/home/alice/.zed/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_zed_home(self, monkeypatch):
        # Custom ZED_HOME not under any user's home â€” keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("ZED_HOME", "/opt/zed-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'ZED_HOME=/opt/zed-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's ZED_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        zed_home = str(gateway_cli.get_zed_home().resolve())
        assert f'ZED_HOME={zed_home}' in unit


class TestZedHomeForTargetUser:
    """Unit tests for _zed_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("ZED_HOME", raising=False)

        result = gateway_cli._zed_home_for_target_user("/home/alice")
        assert result == "/home/alice/.zed"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._zed_home_for_target_user("/home/alice")
        assert result == "/home/alice/.zed/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("ZED_HOME", "/opt/zed")

        result = gateway_cli._zed_home_for_target_user("/home/alice")
        assert result == "/opt/zed"
...
class TestPreflightUserSystemd:
    """Tests for _preflight_user_systemd() â€” D-Bus reachability before systemctl --user.
    ...
    def test_noop_when_bus_socket_exists(self, monkeypatch):
        """Socket already there (desktop / linger + prior login) â†’ no-op."""
        monkeypatch.setattr(
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        """Rick's scenario: no D-Bus, no linger, non-root SSH â†’ clear error."""
        monkeypatch.setattr(
            gateway_cli, "_user_dbus_socket_path",
            lambda: type("P", (), {"exists": lambda self: False})(),
        )
        ...
        assert "zed gateway run" in msg  # foreground fallback mentioned
        assert "Interactive authentication required" in msg

    def test_raises_when_loginctl_missing(self, monkeypatch):
        """No loginctl binary at all â†’ suggest sudo install + manual fix."""
        monkeypatch.setattr(
            gateway_cli, "_user_dbus_socket_path",
            lambda: type("P", (), {"exists": lambda self: False})(),
        )
...
class TestProfileArg:
    """Tests for _profile_arg â€” returns '--profile <name>' for named profiles."""

    def test_default_zed_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.zed should not produce a --profile flag."""
        zed_home = tmp_path / ".zed"
        zed_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZED_HOME", str(zed_home))
        result = gateway_cli._profile_arg(str(zed_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.zed/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".zed" / "profiles" / "mybot"
    ...
    def test_named_profile_under_target_user_root_returns_flag(self, tmp_path):
        ...
        target_root = tmp_path / "home" / "alice" / ".zed"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile ZED_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "zed"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.zed/profiles/mybot/subdir should NOT match â€” too deep."""
        nested = tmp_path / ".zed" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".zed" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_zed_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
        assert "--profile mybot" in unit
        assert "gateway run" in unit
        # Under a process supervisor (Restart=always), --replace makes each
        # restart kill its predecessor â†’ self-kill loop. The systemd unit must
        # NOT use --replace; the supervisor owns the lifecycle. (--replace stays
        # on the manual launchd fallback path â€” see test_launchd_plist_includes_profile.)
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_profile = root_home / ".zed" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("ZED_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_zed_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
        ...
        assert f'ZED_HOME={target_home / ".zed" / "profiles" / "mybot"}' in unit
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".zed" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.zed.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".zed" / "zed-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".zed" / "zed-agent")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/zed", str(tmp_path / "alice"))
        assert result == "/opt/zed"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".zed" / "zed-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".zed" / "zed-agent"
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("ZED_HOME", str(root_home / ".zed"))
        monkeypatch.setattr(gateway_cli, "get_zed_home", lambda: root_home / ".zed")
        ...
        # WorkingDirectory is anchored at the target user's ZED_HOME (stable,
        # always exists) â€” NOT the source checkout under it. Pinning cwd to the
        # checkout is the rot bug fixed alongside this: a relocated/removed
        # checkout would crash-loop the unit on CHDIR (status=200).
        assert "WorkingDirectory=/home/alice/.zed" in unit
        assert "WorkingDirectory=/home/alice/.zed/zed-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "zed-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "zed-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'zed gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'zed gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'zed gateway start' inside Docker exits 0 with container guidance."""
        import pytest
        ...
        out = capsys.readouterr().out
        assert "docker" in out.lower()
        assert "zed gateway run" in out


class TestLegacyZedUnitDetection:
    """Tests for _find_legacy_zed_units / has_legacy_zed_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``zed.service`` unit behind when the service was
    renamed to ``zed-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``zed.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``zed-gateway-<profile>.service``) or unrelated third-party services.
    """
    ...
    def test_detects_legacy_zed_service_in_user_scope(self, tmp_path, monkeypatch):
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = user_dir / "zed.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_zed_units()

        assert len(results) == 1
        name, path, is_system = results[0]
        assert name == "zed.service"
        assert path == legacy
        assert is_system is False
        assert gateway_cli.has_legacy_zed_units() is True

    def test_detects_legacy_zed_service_in_system_scope(self, tmp_path, monkeypatch):
        _, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = system_dir / "zed.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_profile_unit_zed_gateway_coder(self, tmp_path, monkeypatch):
        """CRITICAL: profile units must NOT be flagged as legacy.

        Teknium's concern â€” ``zed-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "zed-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "zed-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "zed-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
        ...
        assert results == []
        assert gateway_cli.has_legacy_zed_units() is False

    def test_ignores_unrelated_zed_service(self, tmp_path, monkeypatch):
        """Third-party ``zed.service`` that isn't ours stays untouched.

        If a user has some other package named ``zed`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "zed.service").write_text(
            "[Unit]\nDescription=Some Other Zed\n[Service]\n"
            "ExecStart=/opt/other-zed/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_returns_empty_when_no_legacy_files_exist(self, tmp_path, monkeypatch):
        ...
        assert gateway_cli._find_legacy_zed_units() == []
        assert gateway_cli.has_legacy_zed_units() is False
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        user_dir, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "zed.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "zed.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        for i, execstart in enumerate(variants):
            name = f"zed.service" if i == 0 else f"zed.service"  # same name
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "zed.service" in out
        assert "zed gateway migrate-legacy" in out
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "zed.service"
        unreadable.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        # Simulate a read failure â€” monkeypatch Path.read_text to raise
...
class TestRemoveLegacyZedUnits:
    """Tests for remove_legacy_zed_units (the migration action)."""
    ...
    def _setup(tmp_path, monkeypatch, as_root=False):
        ...
        # Mock systemctl â€” return success for everything
    ...
    def test_returns_zero_when_no_legacy_units(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_zed_units(interactive=False)
    ...
    def test_dry_run_lists_without_removing(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_zed_units(
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        # Must have invoked stop â†’ disable â†’ daemon-reload on user scope
        cmds_joined = [" ".join(c) for c in calls]
        assert any("--user stop zed.service" in c for c in cmds_joined)
        assert any("--user disable zed.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        assert removed == 0
        assert remaining == [legacy]
        assert legacy.exists()  # Not removed â€” requires sudo
        out = capsys.readouterr().out
        assert "sudo zed gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop zed.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable zed.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "zed.service"
        system_legacy = system_dir / "zed.service"
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (zed-gateway-coder.service)
        ...
        profile_unit = user_dir / "zed-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "zed-gateway.service"
    ...
    def test_interactive_prompt_no_skips_removal(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_zed_units(interactive=True)
...
class TestMigrateLegacyCommand:
    """Tests for the `zed gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        # a namespace through argparse directly â€” but if build_parser isn't
        # public, just confirm that `zed gateway --help` shows it.
        import subprocess
        import sys
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_zed_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_zed_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_zed_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_zed_units", fake_remove)
    ...
    def test_install_skips_legacy_check_when_none_present(
        ...
        """No legacy â†’ no prompt, no helper call."""
...
class TestSystemScopeRequiresRootError:
    ...
    def test_error_is_runtime_error_subclass(self):
        """Wizards use ``except Exception`` guards â€” the error must be a
...
class TestSystemScopeWizardPreCheck:
    """Tests for _system_scope_wizard_would_need_root â€” the guard the
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "zed-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "zed-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "zed-gateway.service",
        )
    ...
    def test_non_root_with_user_unit_present_returns_false(self, tmp_path, monkeypatch):
        # User-scope unit present â€” user can start it themselves, no sudo needed.
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``zed gateway start --system``).
        self._setup_units(tmp_path, monkeypatch, system_present=False, user_present=False)
        monkeypatch.setattr(gateway_cli.os, "geteuid", lambda: 1000)
...
class TestSystemScopeRemediationOutput:
    """Tests for _print_system_scope_remediation â€” the actionable guidance
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "zed-gateway")
        ...
        assert "sudo systemctl start zed-gateway" in out
        assert "sudo zed gateway uninstall --system" in out
        assert "zed gateway install" in out
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart zed-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop zed-gateway" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``zed gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (ZED_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """

    def test_stable_working_dir_uses_zed_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".zed"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_zed_home", lambda: home)
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        # ZED_HOME points somewhere that does not exist -> fall back.
        missing = tmp_path / "does-not-exist" / ".zed"
        monkeypatch.setattr(gateway_cli, "get_zed_home", lambda: missing)
        assert gateway_cli._stable_service_working_dir() == str(gateway_cli.PROJECT_ROOT)

    def test_user_unit_workingdirectory_is_zed_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".zed"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_zed_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_zed_home(self, tmp_path, monkeypatch):
        import re
