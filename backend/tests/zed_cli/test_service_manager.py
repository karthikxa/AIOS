"""Tests for zed_cli.service_manager â€” the abstract ServiceManager
...
def test_detect_service_manager_returns_known_value() -> None:
    ...
    advertised literals â€” anything else means a new platform branch
...
def test_detect_service_manager_s6_keys_off_s6_running_not_is_container(
    ...
    the dispatch path inert on Fly, so `zed gateway restart` spawned a
    foreground gateway that fought the supervised one. Detection must key off
    s6 being PID 1 (`_s6_running`) alone."""
    monkeypatch.setattr(
        "zed_cli.service_manager._s6_running", lambda: True,
    )
    assert detect_service_manager() == "s6"


# ---------------------------------------------------------------------------
# _s6_running â€” must work for unprivileged users, not just root
# ---------------------------------------------------------------------------
...
def test_s6_running_true_when_comm_and_basedir_match(
    ...
    from zed_cli.service_manager import _s6_running
...
def test_s6_running_false_when_basedir_missing(
    ...
    # The comm matches but the basedir is missing â€” e.g. an unrelated
...
def test_s6_running_false_when_comm_unreadable(
    ...
    always-False inside the container under the zed user. The new
    probe must FAIL CLOSED â€” not raise â€” when /proc/1/comm can't be
    read.
    """
    from zed_cli.service_manager import _s6_running
...
# ---------------------------------------------------------------------------
# Backend wrappers â€” kind + registration unsupported on hosts
# ---------------------------------------------------------------------------
...
def test_systemd_manager_kind_and_registration_unsupported() -> None:
    ...
    # Protocol conformance â€” runtime_checkable lets us assert this.
...
# ---------------------------------------------------------------------------
# Lifecycle delegation â€” wrappers must call through to module-level fns
# ---------------------------------------------------------------------------
...
def test_systemd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "zed_cli.gateway.systemd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "zed_cli.gateway.systemd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "zed_cli.gateway.systemd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "zed_cli.gateway._probe_systemd_service_running",
...
def test_launchd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "zed_cli.gateway.launchd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "zed_cli.gateway.launchd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "zed_cli.gateway.launchd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "zed_cli.gateway._probe_launchd_service_running", lambda: False,
    )
    mgr = LaunchdServiceManager()
    mgr.start("ignored")
    mgr.stop("ignored")
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `zed_cli` package succeeds â€” gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import zed_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("zed_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
        "zed_cli.gateway.find_gateway_pids",
        lambda **kw: [12345],
    )
    mgr = WindowsServiceManager()
    mgr.start("ignored")
...
def test_get_service_manager_returns_correct_backend(
    ...
    monkeypatch.setattr(
        "zed_cli.service_manager.detect_service_manager", lambda: kind,
    )
...
def test_get_service_manager_raises_when_unsupported(
    ...
    monkeypatch.setattr(
        "zed_cli.service_manager.detect_service_manager", lambda: "none",
    )
...
def test_get_service_manager_returns_s6_instance(
    ...
    """The s6 backend ships in Phase 3 â€” the factory must return an
    S6ServiceManager when running inside a container."""
    monkeypatch.setattr(
        "zed_cli.service_manager.detect_service_manager", lambda: "s6",
    )
    assert isinstance(get_service_manager(), S6ServiceManager)


# ---------------------------------------------------------------------------
# S6ServiceManager â€” unit tests against a tmp-path scandir (no real s6)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# _seed_supervise_skeleton â€” unit tests
# ---------------------------------------------------------------------------
#
...
# unprivileged zed user from every lifecycle op. These tests run
# against tmp_path and assert the produced layout â€” the live-container
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from zed_cli.service_manager import _seed_supervise_skeleton
    ...
    # Top-level event/ â€” s6-svlisten1 event subscription dir.
    event = svc_dir / "event"
...
def test_s6_register_creates_service_dir_and_triggers_scan(
    ...
    assert "zed -p coder gateway run" in run_text
    assert "s6-setuidgid zed" in run_text
...
def test_render_run_script_resets_home_before_exec() -> None:
    ...
    assert "export HOME=/opt/data" in run_text
    assert "exec s6-setuidgid zed zed -p coder gateway run --replace" in run_text
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    ``zed gateway run``, an agent action, the Open WebUI helper) holds
    the per-ZED_HOME PID lock; the supervised slot then execs a bare
    ``gateway run``, hits the "Another gateway instance is already
    running" guard, exits non-zero, and s6 restarts it â€” a restart loop
    that never binds. ``--replace`` makes the supervised gateway reap the
    stale holder and win, so s6 is authoritative for the slot.

    Covers both the default (root ZED_HOME, no ``-p``) and named-profile
    render paths.
    """
    default_text = S6ServiceManager._render_run_script("default", {})
    # Root profile: bare `zed gateway run --replace` (no -p flag).
    assert "zed gateway run --replace" in default_text
    assert "zed -p default" not in default_text
...
def test_s6_lifecycle_persists_named_profile_desired_state(
    ...
    profile_dir = zed_home / "profiles" / "coder"
...
def test_s6_lifecycle_persists_default_profile_desired_state(
    ...
    monkeypatch.setenv("ZED_HOME", str(zed_home / "profiles" / "coder"))

    mgr = S6ServiceManager(scandir=s6_scandir)
    mgr.start("gateway-default")
    state = json.loads((zed_home / "gateway_state.json").read_text())
    assert state["desired_state"] == "running"


# ---------------------------------------------------------------------------
# Lifecycle errors â€” friendly messages, not raw CalledProcessError
# ---------------------------------------------------------------------------
...
def test_lifecycle_raises_gateway_not_registered_for_missing_slot(
    ...
    mgr = S6ServiceManager(scandir=s6_scandir)
    # No gateway-typo/ directory exists â€” slot is missing.
    ...
    assert "'typo'" in msg
    assert "zed profile create typo" in msg
...
def test_lifecycle_raises_s6_command_error_on_subprocess_failure(
    ...
    """When s6-svc itself fails (non-zero exit) â€” e.g. EACCES on the
    supervise control FIFO â€” the lifecycle methods translate the
    CalledProcessError into a named S6CommandError carrying the
    return code and stderr."""
    import subprocess as _sp
    from zed_cli.service_manager import S6CommandError
...
#
# `zed gateway stop` inside a container dispatches through
...
def test_s6_log_run_chowns_gateways_parent(s6_scandir, fake_subprocess_run) -> None:
    ...
    dropped zed user and s6-log crash-loops on `mkdir: Permission denied`.
    """
    mgr = S6ServiceManager(scandir=s6_scandir)
    mgr.register_profile_gateway("coder")
    ...
    parent_chown = 'chown zed:zed "$ZED_HOME/logs/gateways"'
    ...
    assert 'chown -R zed:zed "$ZED_HOME/logs/gateways"' not in log_text

    # Ordering: mkdir creates the parent, then the parent chown repairs its
    # ownership, then the leaf chown â€” all before s6-log execs.
    mkdir_idx = log_text.index('mkdir -p "$log_dir"')
    parent_idx = log_text.index(parent_chown)
    leaf_idx = log_text.index('chown -R zed:zed "$log_dir"')
