...
Regression coverage for [#26923](https://github.com/NousResearch/zed-agent/issues/26923):
...
* ``_xai_oauth_loopback_login(manual_paste=True)`` skips the HTTP
  ...
  token exchange â€” proving the paste path actually wires up.
"""
...
# ---------------------------------------------------------------------------
# _is_remote_session â€” broadened detection (#26923)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# _parse_pasted_callback â€” accept every plausible paste form
# ---------------------------------------------------------------------------
...
def test_parse_malformed_url_does_not_crash():
    ...
    # Malformed URLs return all-None rather than raising â€” the caller
    # (state check) will reject the empty payload with a clear error.
    assert out["code"] is None


# ---------------------------------------------------------------------------
# _prompt_manual_callback_paste â€” stdin handling
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# _xai_oauth_loopback_login(manual_paste=True) â€” full integration
# ---------------------------------------------------------------------------
...
def test_xai_loopback_login_manual_paste_skips_http_server(monkeypatch):
    ...
    def _fake_prompt(_redirect_uri):
        # Zed generates state internally; we won't know it ahead of
        # time, so capture the state Zed baked into the authorize
...
def test_xai_wait_for_callback_accepts_ready_stdin_code(monkeypatch):
    """Users can paste the Grok Build code while Zed is still waiting."""
    class _StubServer:
...
# ---------------------------------------------------------------------------
# _print_loopback_ssh_hint â€” now also mentions --manual-paste
# ---------------------------------------------------------------------------
