...
def kanban_home(tmp_path, monkeypatch):
    """Isolated ZED_HOME with an empty kanban DB."""
    home = tmp_path / ".zed"
    home.mkdir()
...
def test_connect_honors_kanban_busy_timeout_env(kanban_home, monkeypatch):
    ...
    """
    monkeypatch.setenv("ZED_KANBAN_BUSY_TIMEOUT_MS", "123456")
...
def test_connect_rejects_tls_record_in_sqlite_header(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("ZED_HOME", str(home))
    monkeypatch.delenv("ZED_KANBAN_DB", raising=False)
    monkeypatch.delenv("ZED_KANBAN_HOME", raising=False)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_connect_migrates_legacy_db_before_optional_column_indexes(tmp_path):
    ...
    # And their indexes â€” the regression scope of this test:
...
def test_recompute_ready_promotes_blocked_with_done_parents(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        # recompute_ready should promote blocked â†’ ready
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    ...
    with kb.connect() as conn:
        ...
        # Worker PID has died â€” exactly the case ``release_stale_claims``
...
def test_stale_claim_deferred_when_live_worker_survives_termination(
    ...
    worker â€” the runaway seen when a cgroup memory.high throttle parks a worker
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    with kb.connect() as conn:
        ...
        conn.execute(
            ...
            ("pid 1 exited rate-limited (quota wall) â€” requeued", tid),
        )
        conn.commit()

        # Inside cooldown â†’ defer with the rate-limit-specific reason.
        monkeypatch.setattr(_kb.time, "time", lambda: now + 100)
        assert kb.check_respawn_guard(conn, tid) == "rate_limit_cooldown"

        # Past cooldown â†’ allowed (None), NOT trapped by blocker_auth even
...
def test_resolve_rate_limit_cooldown_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv(
            "ZED_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", bad_val
        )
        assert (
...
def test_recompute_ready_skips_tasks_at_failure_limit(kanban_home):
    ...
    budget would cycle forever: block â†’ auto-recover (counter reset)
    â†’ respawn â†’ budget exhausted â†’ block â†’ â€¦
    """
    with kb.connect() as conn:
        parent = kb.create_task(conn, title="parent", assignee="a")
        ...
        # recompute_ready must NOT promote this task â€” the circuit
...
def test_recompute_ready_recovers_below_limit(kanban_home):
    ...
    failure limit yet â€” the counter is preserved across recovery."""
    with kb.connect() as conn:
...
def test_recompute_ready_honours_dispatcher_failure_limit(kanban_home):
    ...
    as the circuit breaker (#35072): per-task max_retries â†’ dispatcher
    failure_limit â†’ DEFAULT_FAILURE_LIMIT.
    ...
    breaker â€” sticking a task prematurely (config limit > default) or
    ...
    with kb.connect() as conn:
        ...
        assert kb.get_task(conn, t).status == "blocked"
        # Dispatcher configured a higher limit â†’ recover, preserve counter.
...
def test_recompute_ready_per_task_max_retries_overrides_dispatcher(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        conn.commit()
        # failures(2) < per-task limit(4) â†’ recover, despite dispatcher=2.
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="zed-task", assignee="daily")
        assert kb.has_spawnable_ready(conn) is True


def test_has_spawnable_ready_false_on_empty_queue(kanban_home):
    """Empty queue is the trivial false case â€” no ready tasks at all."""
    with kb.connect() as conn:
        assert kb.has_spawnable_ready(conn) is False
...
def test_scratch_workspace_created_under_zed_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "Zed-managed scratch dir should be cleaned up"
...
def test_cleanup_workspace_honors_workspaces_root_env_override(tmp_path, monkeypatch):
    """``ZED_KANBAN_WORKSPACES_ROOT`` extends the managed-scratch set.
    ...
    monkeypatch.setenv("ZED_KANBAN_WORKSPACES_ROOT", str(workspaces_override))
    kb.init_db()
...
def test_dir_child_completion_unblocks_deferred_scratch_parent(kanban_home, tmp_path):
    ...
    non-scratch task and never ran the parent sweep â€” leaking the parent's
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Zed' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Zed' DB/metadata/logs on task completion.
    """
    ...
    # The board root itself is NOT a managed scratch dir â€” only the
    ...
    # Now create the board's workspaces dir and a task scratch dir under it â€”
    ...
    # The workspaces root itself is also NOT managed â€” deleting it would
...
def test_session_id_compose_with_tenant_filter(kanban_home):
    """A client may want both `tenant=scarf:foo` AND `session=acp-x` â€”
...
# spawned with `zed -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's ZED_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's ZED_HOME."""

    def _set_home(self, monkeypatch, tmp_path, zed_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZED_HOME", str(zed_home))
        monkeypatch.delenv("ZED_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_zed(
        self, tmp_path, monkeypatch
    ):
        # Standard install: ZED_HOME == ~/.zed, no profile active.
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.zed/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.zed/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.zed/kanban.db.
        default_home = tmp_path / ".zed"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # ZED_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `zed -p coder`).
    ...
    def test_docker_custom_zed_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: ZED_HOME points outside ~/.zed.
        # `get_default_zed_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".zed"`.
        custom_root = tmp_path / "opt" / "zed"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: ZED_HOME=/opt/zed/profiles/coder;
        # `get_default_zed_root()` walks up to /opt/zed because
    ...
    def test_explicit_override_via_zed_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: ZED_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".zed"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZED_HOME", str(profile_home))
        monkeypatch.setenv("ZED_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZED_HOME", str(default_home))
        monkeypatch.setenv("ZED_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # ZED_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile ZED_HOME and reads.
        monkeypatch.setenv("ZED_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_zed_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # ZED_KANBAN_DB pins the file path directly and beats both
        # ZED_KANBAN_HOME and the `get_default_zed_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZED_HOME", str(default_home))
        monkeypatch.setenv("ZED_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("ZED_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows ZED_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_zed_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # ZED_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".zed"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("ZED_HOME", str(default_home))
        monkeypatch.setenv("ZED_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("ZED_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows ZED_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZED_HOME", str(default_home))
        monkeypatch.setenv("ZED_KANBAN_DB", "   ")
        monkeypatch.setenv("ZED_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject ZED_KANBAN_DB
        # and ZED_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites ZED_HOME.
        default_home = tmp_path / ".zed"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["ZED_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["ZED_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["ZED_KANBAN_TASK"] == "t_dispatch_env"
        assert env["ZED_KANBAN_BRANCH"] == "wt/t_dispatch_env"


# ---------------------------------------------------------------------------
# latest_summary / latest_summaries â€” surface task_runs.summary handoffs
# ---------------------------------------------------------------------------
...
def test_latest_summary_picks_newest_when_multiple_runs(kanban_home):
    """When a task has been re-run (block â†’ unblock â†’ complete), the
    ...
    with kb.connect() as conn:
        ...
        # Move back to ready by direct SQL â€” block_task / unblock_task
        ...
        time.sleep(1.05)
        kb.complete_task(conn, t, summary="second attempt â€” final")
        assert kb.latest_summary(conn, t) == "second attempt â€” final"


def test_latest_summary_skips_empty_string(kanban_home):
    """A run with an empty-string summary should not mask an earlier
    populated one â€” empty strings carry no information."""
    with kb.connect() as conn:
...
def test_latest_summaries_batch_omits_tasks_without_summary(kanban_home):
    """``latest_summaries`` is the dashboard's N+1 escape hatch â€” it
    ...
    with kb.connect() as conn:
        ...
        # Empty input â†’ empty dict, no SQL syntax error from "IN ()".
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    retried forever â€” which is what the real-world user report shows
    (see zed-agent issue #22032).

    NOTE: We do NOT use the ``kanban_home`` fixture here because that
    fixture pre-initializes the DB via ``kb.init_db()`` â€” putting the
    ...
    # DB still usable end-to-end â€” create + list a task
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    to ready when all remaining parents are done â€” same contract as
    complete_task and unblock_task.

    Before the fix, child stayed 'todo' indefinitely after unlink; only the
    next dispatcher tick or a manual 'zed kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
        ...
        # C is running (not done) â€” blocks child B.
        c = kb.create_task(conn, title="parent-running")
        kb.claim_task(conn, c, claimer="worker:1")

        # B depends on both A (done) and C (running) â†’ stays todo.
        b = kb.create_task(conn, title="child", parents=[a, c])
        assert kb.get_task(conn, b).status == "todo"

        # Remove the blocking dependency C â†’ B.
        removed = kb.unlink_tasks(conn, c, b)
        assert removed is True

        # B's only remaining parent is A (done) â†’ must be ready immediately.
        assert kb.get_task(conn, b).status == "ready", (
...
def test_add_column_if_missing_is_idempotent_on_race(kanban_home):
    ...
    # First call adds the column â€” returns True.
    added = kb._add_column_if_missing(conn, "tasks", "extra_col", "extra_col TEXT")
    assert added is True
    cols = {row["name"] for row in conn.execute("PRAGMA table_info(tasks)")}
    assert "extra_col" in cols

    # Second call on same connection â€” column already exists â€” must return
...
def test_migrate_add_optional_columns_tolerates_concurrent_migration(kanban_home):
    ...
    exist (issue #21708 race window â€” two connections migrate concurrently)."""
    import sqlite3
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation â€” _resolve_zed_argv()
#
# Workers spawned by the dispatcher must use a `zed` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["zed", ...]`
...
def test_resolve_zed_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    ...
    assert kb._resolve_zed_argv() == [os.path.abspath(".\\zed.exe")]
...
def test_resolve_zed_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "zed.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("ZED_BIN", raising=False)
...
def test_resolve_zed_argv_honors_zed_bin_path_override(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("ZED_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_zed_argv() == [str(shim)]
...
def test_resolve_zed_argv_zed_bin_bare_name_uses_path(monkeypatch, tmp_path):
    ...
    cwd_zed.chmod(cwd_zed.stat().st_mode | stat.S_IXUSR)
    path_zed = tmp_path / "bin" / "zed"
    path_zed.parent.mkdir()
    path_zed.write_text("right\n", encoding="utf-8")
    path_zed.chmod(path_zed.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_zed.parent))
    monkeypatch.setenv("ZED_BIN", "zed")

    assert kb._resolve_zed_argv() == [str(path_zed)]
...
def test_resolve_zed_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    ...
    `python -m zed` which fails with `No module named zed` on every
    invocation.
    """
    import shutil
    import sys
...
def test_resolve_zed_argv_module_actually_runs():
    ...
    assert "Zed Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"


# ---------------------------------------------------------------------------
# task_age â€” guard against corrupt timestamp values
#
...
def test_safe_int_accepts_int_and_int_string():
    ...
    # PR d8ad431de renamed _safe_int â†’ _to_epoch (now also handles ISO-8601).
...
def test_safe_int_returns_none_on_corrupt_inputs():
    ...
    # None â€” common when the column was never written
    assert kb._to_epoch(None) is None
    # Unsubstituted format string â€” the literal case the PR title cites
    assert kb._to_epoch("%s") is None
    # Arbitrary non-numeric strings
    assert kb._to_epoch("abc") is None
    assert kb._to_epoch("") is None
    # Float-ish strings: int("1.5") raises ValueError too â€” caller wants None.
    assert kb._to_epoch("1.5") is None
    # Random object â€” covered by TypeError branch
...
def test_task_dict_survives_corrupt_created_at(tmp_path, monkeypatch):
    ...
    # Re-read and pass through task_age â€” must not raise.
    conn = kb.connect()
    try:
...
def test_create_task_scratch_without_workspace_ignores_board_default_workdir(kanban_home, monkeypatch):
    """Scratch tasks must NOT inherit board.default_workdir â€” would point auto-cleanup
...
def test_create_task_without_workspace_no_default_stays_none(kanban_home):
    """Board without default_workdir â†’ create_task without workspace_path â†’ stays None."""
...
def test_create_task_with_explicit_workspace_ignores_board_default(kanban_home):
    """create_task with explicit workspace_path â†’ ignores board default."""
...
# ---------------------------------------------------------------------------
# dispatch_once â€” max_in_progress
# ---------------------------------------------------------------------------
...
def test_dispatch_max_in_progress_skips_when_at_limit(kanban_home, all_assignees_spawnable):
    ...
    with kb.connect() as conn:
        ...
        # Two more ready to spawn â€” but cap is 2 so none should fire.
...
def test_dispatch_max_in_progress_spawns_up_to_cap(kanban_home, all_assignees_spawnable):
    ...
    with kb.connect() as conn:
        ...
        t1 = kb.create_task(conn, title="a", assignee="alice")
        kb.claim_task(conn, t1)
        # Three ready tasks â€” only the first 2 should be spawned.
...
def test_dispatch_max_in_progress_none_is_unlimited(kanban_home, all_assignees_spawnable):
    """Default None means no limit â€” all ready tasks are spawned."""
    spawns = []
...
# Stale detection â€” detect_stale_running
# ---------------------------------------------------------------------------
...
def test_detect_stale_returns_running_task_with_no_heartbeat(kanban_home, monkeypatch):
    ...
    with kb.connect() as conn:
        ...
        # No heartbeat set â€” last_heartbeat_at stays NULL.
...
def test_detect_stale_skips_recently_started_task(kanban_home, monkeypatch):
    ...
    with kb.connect() as conn:
        ...
        # Started only 1 hour ago â€” well within the 4h threshold.
...
def test_init_db_refuses_corrupt_existing_file(tmp_path):
    ...
    # Original bytes untouched â€” no schema was written on top.
...
def test_repeated_corrupt_open_reuses_single_backup(tmp_path):
    ...
    # Mutate the corrupt bytes â€” fingerprint changes, separate backup preserved.
...
def test_locked_healthy_db_does_not_classify_as_corrupt(tmp_path, monkeypatch):
    ...
    def flaky_connect(*args, **kwargs):
        # First call is the integrity probe â€” simulate a lock.
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    Subsequent scratch workspaces on the SAME install stay silent â€” the
...
def test_maybe_emit_scratch_tip_skips_non_scratch_workspaces(kanban_home, caplog):
    ...
    # Sentinel stays unset â€” these workspaces are preserved by design,
...
# write_txn â€” rollback handler must not mask the original exception
# ---------------------------------------------------------------------------
...
def test_write_txn_preserves_original_exception_when_rollback_fails(kanban_home):
    ...
    must NOT be masked by the secondary rollback failure â€” operators rely
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    with open(db, "wb") as f:
        f.write(truncated)
    # Now open and check â€” should raise
...
# Regression coverage for #33159 (kanban.db FD leak â€” gateway crashes after
...
def test_connect_closing_closes_connection_on_exit(tmp_path):
    ...
    # After exit, the connection MUST be closed â€” subsequent execute
