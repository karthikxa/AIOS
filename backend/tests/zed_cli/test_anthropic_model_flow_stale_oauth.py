"""Tests for Bug #12905 fix â€” stale OAuth token detection in zed model flow.

Bug 3: `zed model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.zed/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(
            ...
            lambda: {
                ...
                "refreshToken": "",          # No refresh â€” can't recover
        ...
        monkeypatch.setattr("zed_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from zed_cli.main import _model_flow_anthropic
        cfg = {}
    ...
    def test_valid_api_key_skips_stale_check(self, tmp_path, monkeypatch, capsys):
        ...
        # Regular API key â€” NOT an OAuth token
...
class TestStaleOAuthGuardLogic:
    ...
    def test_stale_oauth_flag_logic_no_cc_creds(self):
        ...
        existing_is_stale_oauth should be True â†’ has_creds = False.
        """
    ...
    def test_stale_oauth_flag_logic_with_valid_cc_creds(self):
        ...
        has_creds should be True â€” the cc_creds will be used instead.
        """
