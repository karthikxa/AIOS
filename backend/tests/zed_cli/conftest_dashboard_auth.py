...
NOT a pytest conftest.py â€” this is an importable helper module. Phase 2
...
Tokens are HMAC-signed JSON blobs (not real JWTs) â€” just enough structure
...
# digest happens to contain that byte â€” ``bytes.rsplit`` then splits at
...
def _sign(payload: dict) -> str:
    ...
    Not a real JWT â€” just a base64(JSON || HMAC-SHA256) blob with enough
...
class StubAuthProvider(DashboardAuthProvider):
    ...
    def __init__(self, default_ttl: int = 3600):
        ...
        # state â†’ verifier mapping, cleared on complete_login
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        return LoginStart(
            ...
            cookie_payload={
                "zed_session_pkce": f"state={state};verifier={verifier}",
            },
        )
    ...
    def refresh_session(self, *, refresh_token: str) -> Session:
        ...
        # ``<=`` for symmetry with verify_session â€” a 0-TTL token is
        # treated as expired.
        if payload is None or payload.get("exp", 0) <= int(time.time()):
