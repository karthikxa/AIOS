...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and ZED_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.zed/profiles)
    * ZED_HOME  -> tmp_path/.zed  (so get_zed_home() agrees)
    * Creates the bare-minimum ~/.zed directory.
    """
...
class TestValidateProfileName:
    ...
    def test_uppercase_rejected(self):
        # validate_profile_name is strict â€” callers normalize first, then validate.
    ...
    @pytest.mark.parametrize("name", ["zed", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the Zed install itself or with
        common system binaries â€” reject them at validate time so
...
class TestGetProfileDir:
    ...
    def test_default_returns_zed_home(self, profile_env):
    ...
    def test_named_profile_returns_profiles_subdir(self, profile_env):
        ...
        assert result == tmp_path / ".zed" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".zed" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_seeds_placeholder_env_file(self, profile_env):
        ...
        # Placeholder only â€” no credentials leak in from anywhere.
        assert all(
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.zed must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".zed"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes zed-agent, .worktrees,
        bin, node_modules at root, plus __pycache__/*.pyc/*.pyo/*.sock/*.tmp
        at any depth.  Profile data (config, env, skills, logs) must be
        preserved â€” clone-all means "complete snapshot minus infrastructure
        and per-profile history."
        """
        tmp_path = profile_env
        default_home = tmp_path / ".zed"
        # Simulate infrastructure dirs that only the default profile has
        (default_home / "zed-agent" / ".git").mkdir(parents=True)
        (default_home / "zed-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "zed-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "zed-agent").exists()
    ...
    def test_clone_all_excludes_history_artifacts(self, profile_env):
        ...
        snapshots â€” a clone is a fresh workspace, and these can reach tens
...
class TestNoSkillsOptOut:
    """Tests for `zed profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `zed update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)

        # Call seed_profile_skills() directly â€” it should NOT invoke subprocess,
        ...
        # skills/ stays empty â€” no subprocess ran
        assert list((profile_dir / "skills").iterdir()) == []
    ...
    def test_delete_marker_re_enables_seeding(self, profile_env, monkeypatch):
        ...
        # Delete marker â†’ next call runs the real path
...
class TestBackfillProfileEnvs:
    """Tests for backfill_profile_envs() â€” the `zed update` pass that
    ...
    def test_copies_default_env_into_envless_profiles(self, profile_env):
        import stat
        tmp_path = profile_env
        (tmp_path / ".zed" / ".env").write_text("OPENROUTER_API_KEY=root-key\n")
    ...
    def test_never_overwrites_existing_profile_env(self, profile_env):
        tmp_path = profile_env
        (tmp_path / ".zed" / ".env").write_text("KEY=root\n")
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".zed" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_zed_home_returns_default(self, profile_env):
        # ZED_HOME points to tmp_path/.zed which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".zed" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom ZED_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("ZED_HOME", str(custom))
        # With Docker-aware roots, a custom ZED_HOME is the default â€”
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("zed")
        assert result is not None
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        ...
        assert "exec /opt/zed/bin/zed -p mybot" in content
    ...
    def test_custom_alias_target_on_posix(self, profile_env, monkeypatch):
        ...
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "zed -p redqueen" in content
...
# ===================================================================
# TestFindAliasForProfile â€” display-side reverse lookup
# ===================================================================
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".zed" / "profiles" / "oldname"
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "zed.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["zed_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["zed_heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "zed.ssi_health": {"workspace": "zed", "enabled": True}
            }
        }))
        ...
        assert "zed.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["zed_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["zed_heimdall"]["workspace"] == "zed"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "zed.ssi_health": {"aiPeer": "ssi_health"},
                "zed_heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["zed.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["zed_heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("zed-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".zed_history",
        ...
        excluded_prefixes = [
            "default/zed-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.zed_history",
    ...
    def test_import_default_export_with_new_name_roundtrip(self, profile_env, tmp_path):
        """Export default â†’ import under a different name â†’ data preserved."""
        default_dir = get_profile_dir("default")
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultZedHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_zed_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".zed" / "profiles"

    def test_default_zed_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_zed_home()
        assert home == tmp_path / ".zed"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (ZED_HOME outside ~/.zed), profiles go under ZED_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZED_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_zed_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_zed_home() returns ZED_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZED_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (ZED_HOME under ~/.zed), profiles root is still ~/.zed/profiles."""
        native = tmp_path / ".zed"
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under ZED_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZED_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZED_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".zed" / "profiles" / "mybot"
        assert result == expected
...
class TestProfilesToServe:
    """profiles_to_serve(multiplex) â€” the gateway's profile-enumeration chokepoint."""
    ...
    def test_off_returns_only_active_default(self, profile_env):
        ...
        name, home = serve[0]
        assert name == "default"
        assert home == _get_default_zed_home()

    def test_off_returns_only_active_named(self, profile_env, monkeypatch):
        # A named profile's gateway runs with ZED_HOME pointing at the
        # profile dir; get_active_profile_name() infers the name from there.
        create_profile("coder", no_alias=True)
        monkeypatch.setenv("ZED_HOME", str(get_profile_dir("coder")))
        serve = profiles_to_serve(multiplex=False)
        assert len(serve) == 1
    ...
    def test_on_returns_default_plus_all_named(self, profile_env):
        ...
        assert serve["default"] == _get_default_zed_home()
