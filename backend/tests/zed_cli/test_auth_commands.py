...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (zed_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from zed_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "zed" / "auth.json").read_text())
...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_zed_oauth_login_pure",
        lambda: {
            "access_token": token,
            "refresh_token": "refresh-token",
    ...
    entry = next(item for item in entries if item["source"] == "manual:zed_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:zed_pkce"
...
def test_auth_add_google_gemini_cli_sets_active_provider(tmp_path, monkeypatch):
    """zed auth add google-gemini-cli must set active_provider in auth.json.
    ...
    provider â€” without storing tokens that would become stale.
    """
    monkeypatch.setenv("ZED_HOME", str(tmp_path / "zed"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    monkeypatch.setattr(
    ...
    # Only email stored â€” no access_token/refresh_token (those live in
    ...
    # pool entry from pool.add_entry() still present for zed auth list
...
def test_auth_add_qwen_oauth_sets_active_provider(tmp_path, monkeypatch):
    """zed auth add qwen-oauth must set active_provider in auth.json.

    Tokens are managed by the Qwen CLI credential file via
    resolve_qwen_runtime_credentials(). The auth.json entry must record
    active_provider â€” without storing tokens that would become stale.
    """
    monkeypatch.setenv("ZED_HOME", str(tmp_path / "zed"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    _fake_creds = {
    ...
    # Only base_url stored â€” no api_key (that lives in the Qwen CLI file).
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "zed_cli.auth._nous_device_code_login",
        lambda **kwargs: {
    ...
    # Pool has exactly one canonical `device_code` entry â€” not a duplicate
    ...
    # `zed auth add nous` must also populate providers.nous so the
    ...
    # raises "Zed is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "zed_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`zed auth add nous --type oauth --label <name>` must preserve the
    custom label end-to-end â€” it was silently dropped in the first cut of the
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "zed_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
    ...
    # (which collapsed multiple accounts into the latest login â€” #39236).
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``zed auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``zed auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("zed_cli.auth._codex_device_code_login", lambda: next(logins))
    ...
    payload = json.loads((tmp_path / "zed" / "auth.json").read_text())
    # No singleton block â€” the add path is now pool-only.
    assert "openai-codex" not in payload.get("providers", {})
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from zed_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from zed_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """zed auth add xai-oauth must write providers singleton and set active_provider.
    ...
    monkeypatch.setattr(
        "zed_cli.auth._xai_oauth_loopback_login",
        lambda **kwargs: {
            "tokens": {
    ...
    payload = json.loads((tmp_path / "zed" / "auth.json").read_text())
    # active_provider must be set â€” the core of this regression
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from zed_cli.auth_commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from zed_cli.auth_commands import auth_reset_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`zed logout --provider openai-codex` must still clear model.provider.
    ...
    config_text = (zed_home / "config.yaml").read_text()
...
def test_logout_defaults_to_configured_codex_when_no_active_provider(tmp_path, monkeypatch, capsys):
    """Bare `zed logout` should target configured Codex if auth has no active provider."""
    zed_home = tmp_path / "zed"
    monkeypatch.setenv("ZED_HOME", str(zed_home))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "credential_pool": {}})
    (zed_home / "config.yaml").write_text(
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((zed_home / "auth.json").read_text())
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from zed_cli.auth import _reset_config_provider
    ...
    with patch("zed_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from zed_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "zed_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("zed_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("zed_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("zed_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "zed_cli.auth_commands.time.time",
...
def test_auth_remove_env_seeded_does_not_resurrect(tmp_path, monkeypatch):
    ...
    # Now reload the pool â€” the entry should NOT come back
    from agent.credential_pool import load_pool
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    (zed_home / "auth.json").write_text(json.dumps(auth_store))

    from types import SimpleNamespace
    from zed_cli.auth_commands import auth_remove_command
    auth_remove_command(SimpleNamespace(provider="anthropic", target="1"))

    updated = json.loads((zed_home / "auth.json").read_text())
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from zed_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from zed_cli.auth import unsuppress_credential_source
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `zed auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("ZED_HOME", str(tmp_path / "zed"))
    zed_home = tmp_path / "zed"
    zed_home.mkdir(parents=True, exist_ok=True)

    # Pre-existing suppression (simulating a prior `zed auth remove`)
    (zed_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
    ...
    payload = json.loads((zed_home / "auth.json").read_text())
    # Suppression marker must be cleared
    assert "openai-codex" not in payload.get("suppressed_sources", {})
    # New pool entry must be present (distinct manual:device_code entry â€” #39236)
    entries = payload["credential_pool"]["openai-codex"]
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    # Make _import_codex_cli_tokens return tokens â€” these would normally trigger
    ...
    monkeypatch.setattr("zed_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((zed_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`zed auth remove xai 1` must stick even when the env var is exported
    by the shell (not written into ~/.zed/.env).  Before PR for #13371 the
    ...
    from agent.credential_pool import load_pool
    pool = load_pool("xai")
    assert not pool.has_credentials(), "pool must stay empty â€” env:XAI_API_KEY suppressed"


def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.zed/.env (not the shell), the
    shell-hint should NOT be printed â€” avoid scaring the user about a
    ...
    (zed_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (zed_home / ".env").read_text().strip() == ""


def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `zed auth add <provider>` clears any
    env:<VAR> suppression marker â€” strong signal the user wants auth back.
    ...
    from types import SimpleNamespace
    from zed_cli.auth import is_source_suppressed
    from zed_cli.auth_commands import auth_add_command
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `zed auth remove`.  This is the gate that prevents shell-exported
...
# =============================================================================
# Unified credential-source stickiness â€” every source Zed reads from has a
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import zed_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_zed_pkce_suppression(tmp_path, monkeypatch):
    """anthropic zed_pkce must not re-seed from ~/.zed/.anthropic_oauth.json when suppressed."""
    zed_home = tmp_path / "zed"
    zed_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("ZED_HOME", str(zed_home))

    import yaml
    (zed_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (zed_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["zed_pkce"]},
    }))

    # Stub the readers so only zed_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_zed_oauth_credentials", lambda: {
    ...
    # zed_pkce suppressed, claude_code returns None â†’ nothing should be seeded
    assert entries == []
    assert "zed_pkce" not in active
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (zed_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
...
def test_credential_sources_registry_has_expected_steps():
    """Sanity check â€” the registry contains the expected RemovalSteps.
    ...
    change-detector tests" rule â€” they break every time someone adds a
    provider.
    """
    ...
    # Core steps must be present â€” these are the ones the rest of the code
...
def test_credential_sources_find_step_returns_none_for_manual():
    """Manual entries have nothing external to clean up â€” no step registered."""
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    # The copilot pool entry is no longer persisted directly in auth.json â€”
    ...
    with patch(
        ...
        "zed_cli.copilot_auth.get_copilot_api_token",
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `zed auth add <provider>` clears ALL
    suppression markers for the provider, not just env:*.  This matches
    the single "re-engage" semantic â€” the user wants auth back, period.
    """
    zed_home = tmp_path / "zed"
    zed_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("ZED_HOME", str(zed_home))
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `zed auth add openai-codex`)
