...
See: NousResearch/zed-agent#7622
"""
...
_GATEWAY_CMD = "python -m zed_cli.main gateway run"
_OTHER_CMD = "python -m some_other_thing"


def _fake_proc_dir(entries: dict):
    """Return side_effects that simulate /proc: isdir â†’ True, listdir â†’ pids,
    open(cmdline) â†’ null-delimited command bytes."""
    def _isdir(path):
...
class TestProcFallback:
    ...
    def test_detects_gateway_pid_via_proc(self):
        my_pid = os.getpid()
        entries = {
            my_pid: "python -m zed_cli.main",   # own process â€” excluded
            12345: _GATEWAY_CMD,
            99999: _OTHER_CMD,
        }
        ...
        with (
            patch("zed_cli.gateway.is_windows", return_value=False),
            patch("os.path.isdir", side_effect=_isdir),
            patch("os.listdir", side_effect=_listdir),
            patch("builtins.open", side_effect=_open),
            patch("zed_cli.gateway._get_ancestor_pids", return_value=set()),
    ...
    def test_proc_permission_error_skips_pid(self):
        ...
        # PermissionError swallowed â€” empty result, no crash
