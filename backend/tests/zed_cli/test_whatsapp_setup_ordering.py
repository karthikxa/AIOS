...
Before the fix, ``zed whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
step 2 â€” before npm install (step 4) and before QR pairing (step 6).
...
``zed gateway`` then paid a 30s bridge-bootstrap timeout and queued
WhatsApp for indefinite retries â€” looking like "the gateway is broken."
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    # _require_tty calls sys.stdin.isatty â€” make it pass.
    monkeypatch.setattr("zed_cli.main._require_tty", lambda *_a, **_kw: None)
    # No node, no bridge script â€” we shouldn't reach those steps anyway.
    ...
    assert _env_value(isolated_home, "WHATSAPP_ENABLED") is None, (
        "Setup aborted before pairing â€” WHATSAPP_ENABLED must not be set. "
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``zed whatsapp`` with an existing paired session and
    ...
    # mode already set â†’ skip mode prompt; users already set â†’ skip update
    # prompt with "no"; pairing exists â†’ "no, keep session" â†’ return.
    inputs = iter(["n", "n"])
    ...
    monkeypatch.setattr("builtins.input", fake_input)
    monkeypatch.setattr("zed_cli.main._require_tty", lambda *_a, **_kw: None)
    # Skip the bridge npm install â€” we're testing setup-ordering, not bridge
