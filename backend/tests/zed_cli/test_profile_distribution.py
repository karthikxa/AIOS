"""Tests for zed_cli.profile_distribution â€” git-based profile installs.
...
E2E runs, not unit tests â€” git itself is tested upstream, and subprocess-
...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/zed_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestVersionRequires:
    ...
    def test_check_matrix(self, spec, cur, ok):
        if ok:
            check_zed_requires(spec, cur)
        else:
            with pytest.raises(DistributionError, match="requires Zed"):
                check_zed_requires(spec, cur)
...
class TestEnvTemplate:
    ...
    def test_empty_env_requires_is_header_only(self):
        m = DistributionManifest(name="x")
        out = _env_template_from_manifest(m)
        assert "Zed distribution" in out
...
# ===========================================================================
# Install â€” fresh and force (from a local-directory source)
# ===========================================================================
...
class TestInstall:
    ...
    def test_install_enforces_zed_requires(self, profile_env, monkeypatch):
        # Pin current Zed version to something well below the requirement
        import zed_cli
        monkeypatch.setattr(zed_cli, "__version__", "0.1.0", raising=False)
...
# ===========================================================================
# Update â€” preserves user data, preserves config by default
# ===========================================================================
...
# ===========================================================================
# describe_distribution â€” info subcommand
# ===========================================================================
...
# ===========================================================================
# Security â€” USER_OWNED_EXCLUDE covers the right paths
# ===========================================================================
...
class TestSecurity:
    ...
    def test_install_rejects_symlinked_distribution_files(self, profile_env, tmp_path):
        ...
        from zed_cli.profiles import get_profile_dir
...
class TestNestedUserOwnedExcludeNotFiltered:
    ...
    def test_top_level_user_owned_still_skipped(self, profile_env):
        """Top-level entries in USER_OWNED_EXCLUDE must still be skipped â€”
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "zed_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from zed_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_plain_profile_has_no_distribution_fields(self, profile_env):
        from zed_cli.profiles import create_profile, list_profiles
        create_profile(name="plain", no_alias=True)
        rows = {p.name: p for p in list_profiles()}
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from zed_cli.profiles import create_profile, list_profiles, get_profile_dir
...
class TestErrorSurfaces:
    ...
    def test_bad_profile_name_raises_valueerror_not_traceback(self, profile_env, tmp_path):
        ...
        should raise ValueError from validate_profile_name â€” the CLI handler
