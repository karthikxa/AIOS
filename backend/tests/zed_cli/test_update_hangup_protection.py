"""Tests for SIGHUP protection and stdout mirroring in ``zed update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``zed_cli/main.py``.  These exist so
that ``zed update`` survives a terminal disconnect mid-install
...
class TestInstallHangupProtection:
    def test_gateway_mode_is_noop(self):
        """In gateway mode the process is already detached â€” don't touch stdio or signals."""
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            assert "zed update started" in contents
        finally:
            _finalize_update_output(state)
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_zed_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
