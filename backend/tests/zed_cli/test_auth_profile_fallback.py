...
When ``ZED_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.zed/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.zed            (has its own auth.json fixture)
    * Profile     -> tmp_path/.zed/profiles/coder   (active, ZED_HOME points here)
...
# ---------------------------------------------------------------------------
# read_credential_pool â€” provider-slice reads
# ---------------------------------------------------------------------------
...
def test_per_provider_shadowing_is_independent(profile_env):
    ...
    _write(profile_env["profile"] / "auth.json", _make_auth_store(pool={
        # Profile has openrouter only â€” anthropic should still fall back.
        "openrouter": [{
            "id": "prof-or",
            "label": "profile-or",
            "auth_type": "api_key",
...
# ---------------------------------------------------------------------------
# read_credential_pool â€” whole-pool reads (provider_id=None)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# get_provider_auth_state â€” singleton fallback
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# _load_provider_state â€” internal global fallback (issue #18594 follow-up)
#
...
# ``_load_provider_state`` itself, those helpers raise ``"Zed is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from zed_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    zed_home = tmp_path / "classic"
    zed_home.mkdir()
    monkeypatch.setenv("ZED_HOME", str(zed_home))

    _write(zed_home / "auth.json", _make_auth_store(providers={
...
# ---------------------------------------------------------------------------
# Classic mode â€” no fallback path should ever trigger
# ---------------------------------------------------------------------------


def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (ZED_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.zed as the "real home" â€” which is NOT equal
    # to the ZED_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    zed_home = tmp_path / "classic"
    ...
    _write(zed_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from zed_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: ZED_HOME is set to a custom path that is NOT under
    # ~/.zed/profiles/ â€” get_default_zed_root() returns ZED_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from zed_cli.auth import read_credential_pool, write_credential_pool
