"""Tests for `zed chat --safe-mode` â€” pristine troubleshooting runs.
...
_VARS = ("ZED_SAFE_MODE", "ZED_IGNORE_USER_CONFIG", "ZED_IGNORE_RULES")
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in zed_cli/main.py.
        class Args:
            safe_mode = True
...
class TestSafeModePluginDiscovery:
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("ZED_SAFE_MODE", raising=False)
        from zed_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under ZED_SAFE_MODE=1."""
...
class TestSafeModeParser:
    """--safe-mode must parse on both the root parser and `zed chat`."""
