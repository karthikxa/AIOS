...
def _touch_ink(root: Path) -> None:
    ink = root / "node_modules" / "@zed" / "ink" / "package.json"
...
def test_no_install_when_only_peer_annotation_differs(tmp_path: Path, main_mod) -> None:
    ...
    *also* declared as peers.  That's a cosmetic difference â€” the package is
    installed at the requested version â€” so it must not trigger a reinstall.
    Regression for the TUI-in-Docker failure where 16 such mismatches caused
    `Installing TUI dependenciesâ€¦` â†’ EACCES on every launch.
    """
    _touch_ink(tmp_path)
    (tmp_path / "package-lock.json").write_text(
        '{"packages":{'
...
def test_no_install_prebuilt_bundle_mode(tmp_path: Path, main_mod) -> None:
    """dist/entry.js present and no package-lock.json â†’ prebuilt bundle, skip npm install."""
...
def test_make_tui_argv_decodes_dev_prebuild_with_utf8_replace(
    ...
    ink_dir = tmp_path / "packages" / "zed-ink"
    ink_dir.mkdir(parents=True)
...
# â”€â”€ _workspace_root helper â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


def test_workspace_root_returns_parent_when_subpackage(tmp_path: Path, main_mod) -> None:
    """Sub-package has package.json, no lockfile; parent has lockfile â†’ parent."""
    sub = tmp_path / "ui-tui"
    sub.mkdir()
...
def test_workspace_root_returns_dir_when_standalone(tmp_path: Path, main_mod) -> None:
    """No package.json â†’ not a sub-package, return dir itself."""
...
def test_workspace_root_returns_dir_when_own_lockfile(tmp_path: Path, main_mod) -> None:
    """Has package.json AND its own lockfile â†’ standalone, return dir."""
...
def test_workspace_root_returns_dir_when_no_parent_lockfile(
    ...
    """Has package.json, no own lockfile, but parent also has no lockfile â†’ standalone."""
    sub = tmp_path / "ui-tui"
    sub.mkdir()
    (sub / "package.json").write_text("{}")
...
def test_workspace_root_consistent_with_need_npm_install(
    ...
    sub.mkdir()
    (sub / "package.json").write_text("{}")
    # Both sub and parent have lockfiles â€” accidental state
    (sub / "package-lock.json").write_text("{}")
    (tmp_path / "package-lock.json").write_text("{}")

    ws = main_mod._workspace_root(sub)
    # _workspace_root sees sub has its own lockfile â†’ treats it as standalone
    assert ws == sub
    ...
    # hidden lockfile to compare against, and ink is missing â†’ True.
    # But the key invariant is: ws_root for the need-check == ws_root
    # for the install cwd â€” both use _workspace_root(sub).
...
def test_no_stray_lockfiles_in_workspace_subdirs(main_mod) -> None:
    ...
    # Also sweep ui-tui/packages/* (zed-ink etc.)
