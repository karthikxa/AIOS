"""Tests for zed_cli.security_advisories.
...
isolation â€” no real package metadata, no real config, no real cache.
"""
...
import zed_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect ZED_HOME so banner cache and config writes are sandboxed."""
    home = tmp_path / ".zed"
    home.mkdir()
    (home / "cache").mkdir()
...
class TestDetectCompromised:
    ...
    def test_empty_compromised_set_matches_any_version(
        ...
        # wildcard â€” used when an entire maintainer namespace is owned.
...
class TestAck:
    def test_get_acked_ids_empty_when_no_config(self, monkeypatch):
        # load_config raises â†’ returns empty set, doesn't crash.
        monkeypatch.setattr(
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full zed_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "zed_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "zed_cli.config.save_config",
...
class TestBannerCache:
    ...
    def test_second_call_within_window_suppresses(
        ...
        # Same banner inside repeat window â†’ suppressed.
    ...
    def test_call_after_window_re_banners(
        ...
        # than 24h ago â€” should re-banner.
...
class TestRendering:
    ...
    def test_render_doctor_section_clean_state(self):
        # No hits â†’ success message, has_problems=False.
