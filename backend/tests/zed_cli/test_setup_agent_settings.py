...
def test_setup_agent_settings_uses_displayed_max_iterations_value(tmp_path, monkeypatch, capsys):
    ...
    After PR#18413 max_turns is read exclusively from config.yaml â€” the
    .env `ZED_MAX_ITERATIONS` fallback was removed because it was
...
def test_setup_agent_settings_prefers_config_over_stale_env(tmp_path, monkeypatch, capsys):
    ...
    Regression guard for the bug where `.env ZED_MAX_ITERATIONS=60`
    from an old `zed setup` run shadowed `agent.max_turns: 500` in
