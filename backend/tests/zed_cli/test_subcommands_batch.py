...
light â€” the byte-identical ``--help`` verification done at extraction time is
...
def test_single_handler_builders(name, builder, kw, argv):
    parser = argparse.ArgumentParser(prog="zed")
...
# â”€â”€ deprecated `zed login` fails gracefully, not with argparse error â”€â”€â”€â”€
#
# `zed login` is a removed command; its handler (`login_command` in
# `zed_cli/auth.py`) prints a deprecation notice pointing at `zed auth` /
# `zed model` and exits 0.  Two behavior contracts guard the UX:
#   1. ANY `--provider <value>` (including ones the user actually wants, like
#      `anthropic`) must parse and reach the handler â€” never crash in argparse
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `zed --help`.
