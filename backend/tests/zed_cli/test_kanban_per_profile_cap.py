"""Regression tests for #21582 â€” per-profile concurrency cap in dispatcher.
...
def isolated_kanban_home_with_profiles(monkeypatch):
    """Spin up a fresh ZED_HOME with kanban DB + alpha/beta profiles."""
...
def test_invalid_cap_treated_as_no_cap(isolated_kanban_home_with_profiles, cap):
    ...
    'no cap' â€” silently falling through rather than crashing the dispatcher."""
    kb = isolated_kanban_home_with_profiles
    with kb.connect_closing() as conn:
        kb.create_board(slug="default", name="Test")
        for i in range(3):
...
def test_capped_tasks_dispatched_on_subsequent_tick(isolated_kanban_home_with_profiles):
    ...
    # Simulate the running task completing â€” set it back to done so the
    # 'running' count drops
