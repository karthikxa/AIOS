"""Tests for the stale-dashboard handling run at the end of ``zed update``.

``zed update`` detects ``zed dashboard`` processes left over from the
...
def _refresh_bindings_against_live_module():
    """Rebind module-level names to the *current* ``zed_cli.main``.

    Other tests in the suite (notably ``test_env_loader.py`` and
    ``test_skills_subparser.py``) reload or delete ``zed_cli.main`` from
    ``sys.modules``.  When that happens on the same xdist worker before we
    run, our top-of-file ``from zed_cli.main import ...`` bindings end
    up pointing at the *old* module object.  ``patch(\"zed_cli.main.X\")``
    ...
    object â€” and keeping them consistent â€” makes the tests immune to
    ...
    live = sys.modules.get("zed_cli.main")
    if live is None:
        live = importlib.import_module("zed_cli.main")
...
def _ps_runner(stdout: str):
    ...
    def _side_effect(args, *a, **kw):
        ...
        # Any other subprocess.run (e.g. taskkill) â€” benign success stub.
...
class TestFindStaleDashboardPids:
    ...
    def test_matches_running_dashboard(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=_ps_line(12345, "python3 -m zed_cli.main dashboard --port 9119") + "\n",
                stderr="",
            )
    ...
    def test_multiple_matches(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(12345, "python3 -m zed_cli.main dashboard --port 9119"),
                    _ps_line(12346, "zed dashboard --port 9120 --no-open"),
                    _ps_line(12347, "python /home/x/zed_cli/main.py dashboard"),
                ]) + "\n",
                stderr="",
            )
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m zed_cli.main dashboard"),
                    _ps_line(12345, "zed dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
    ...
    def test_unrelated_process_containing_word_dashboard_not_matched(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                ...
                stdout="\n".join([
                    _ps_line(12345, "python3 -m zed_cli.main dashboard --port 9119"),
                    _ps_line(22222, "python3 -m zed_cli.main chat -q 'rewrite my dashboard'"),
    ...
    def test_grep_lines_ignored(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(99999, "grep zed dashboard"),
                    _ps_line(12345, "zed dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
    ...
    def test_exclude_pids_filters_specified_pids(self):
        """exclude_pids removes specific PIDs from the result â€” used by
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(11111, "zed dashboard --port 9119"),
                    _ps_line(22222, "zed dashboard --port 9120"),
                    _ps_line(33333, "zed dashboard --port 9121"),
                ]) + "\n",
                stderr="",
            )
    ...
    def test_exclude_pids_none_is_noop(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=_ps_line(12345, "zed dashboard --port 9119") + "\n",
                stderr="",
            )
...
class TestKillStaleDashboardPosix:
    ...
    def test_no_stale_processes_is_a_noop(self, capsys):
        with patch("zed_cli.main._find_stale_dashboard_pids", return_value=[]):
            _kill_stale_dashboard_processes()
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        def fake_kill(pid, sig):
            ...
            if sig == 0:
                # Probe after SIGTERM â†’ "process gone".
                raise ProcessLookupError
            # SIGTERM itself: succeed silently.

        with patch("zed_cli.main._find_stale_dashboard_pids",
        ...
        assert "âœ“ stopped PID 12345" in out
        assert "âœ“ stopped PID 12346" in out
    ...
    def test_permission_error_is_reported_not_raised(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "âœ— failed to stop PID 12345" in out
    ...
    def test_process_already_gone_counts_as_stopped(self, capsys):
        ...
        already exited between detection and the kill â€” treat as success."""
        def fake_kill(pid, sig):
            raise ProcessLookupError
...
class TestWindowsWmicEncoding:
    """Regression tests for #17049 â€” the Windows wmic branch must not crash
    `zed update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
    ...
    def test_wmic_invoked_with_utf8_ignore_errors(self, monkeypatch):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "CommandLine=python -m zed_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
    ...
    def test_wmic_returns_none_stdout_does_not_crash(self, monkeypatch):
        """If subprocess.run returns successfully but stdout is None â€” which
        is what Python 3.11 leaves behind when the reader thread silently
        crashed on UnicodeDecodeError before this fix landed â€” detection
        must short-circuit instead of raising AttributeError on
        ``None.split('\\n')`` and aborting `zed update` (#17049)."""
        monkeypatch.setattr(sys, "platform", "win32")
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
