...
``auth.json`` under ``credential_pool.copilot[]`` â€” placed there by
``zed auth add copilot`` or by ``_seed_from_env`` when the env var
is set in ``~/.zed/.env`` â€” the picker was silently dropping back to
...
from zed_cli.models import _resolve_copilot_catalog_api_key
...
class TestCopilotCatalogApiKeyResolution:
    def test_env_var_token_wins_over_pool(self):
        ...
        with patch(
            ...
            "zed_cli.auth.read_credential_pool",
        ) as mock_pool:
            assert _resolve_copilot_catalog_api_key() == "env-token"
            mock_pool.assert_not_called()

    def test_falls_back_to_pool_oauth_token(self):
        """Empty env â†’ walk credential_pool.copilot[] for an OAuth access_token."""
        with patch(
            "zed_cli.auth.resolve_api_key_provider_credentials",
            ...
            "zed_cli.copilot_auth.exchange_copilot_token",
    ...
    def test_skips_classic_pat_in_pool(self):
        """Classic PATs (``ghp_â€¦``) are unsupported by the Copilot API â€” skip them."""
        with patch(
            "zed_cli.auth.resolve_api_key_provider_credentials",
            return_value={"api_key": ""},
        ), patch(
    ...
    def test_skips_pool_entry_that_fails_to_exchange(self):
        """If the first entry won't exchange, try the next â€” an unsupported pool[0]
    ...
    def test_all_pool_entries_fail_exchange_returns_empty(self):
        """All exchanges fail â†’ return "" so the caller falls back to curated."""
        with patch(
            "zed_cli.auth.resolve_api_key_provider_credentials",
            return_value={"api_key": ""},
        ), patch(
    ...
    def test_returns_empty_string_when_no_credentials_anywhere(self):
        """No env, no pool â†’ empty string (caller falls back to curated list)."""
        with patch(
            "zed_cli.auth.resolve_api_key_provider_credentials",
            return_value={"api_key": ""},
        ), patch(
