"""Tests for the `zed memory reset` CLI command.
...
- Profile-scoped reset (uses ZED_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ZED_HOME with memory files."""
    zed_home = tmp_path / ".zed"
    memories = zed_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ZED_HOME", str(zed_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "Â§\nZed repo is at ~/.zed/zed-agent\nÂ§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        zed_home = tmp_path / ".zed"
        (zed_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ZED_HOME", str(zed_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's ZED_HOME."""
