...
``ZedCLI._apply_model_switch_result()`` printed ``ModelInfo.context_window``
straight from models.dev, which always reports the vendor-wide value (e.g.
gpt-5.5 = 1,050,000 on ``openai``). That ignored provider-specific caps â€” in
...
def _run_display(monkeypatch, result):
    ...
    # Avoid writing to ~/.zed/config.yaml during the test.
    monkeypatch.setattr(cli_mod, "save_config_value", lambda *a, **k: None)
    cli_mod.ZedCLI._apply_model_switch_result(_StubCLI(), result, False)
    return captured
...
def test_picker_path_shows_vendor_value_when_no_provider_cap(monkeypatch):
    ...
    should surface the real 1.05M context for gpt-5.5 â€” resolver and models.dev
    agree here.
    """
    result = ModelSwitchResult(
        success=True,
...
def test_picker_path_falls_back_to_model_info_when_resolver_empty(monkeypatch):
    """If ``get_model_context_length`` returns nothing (rare â€” truly unknown
