...
from zed_cli.dashboard_auth import clear_providers, get_provider
from zed_cli.dashboard_auth.base import (
    DashboardAuthProvider, LoginStart, Session,
)
from zed_cli.plugins import PluginContext, PluginManifest
...
class _MinimalManager:
    ...
    ``ctx.manifest``, so the manager attributes don't matter â€” but we set
...
def test_plugin_ctx_silently_ignores_non_provider(caplog):
    ...
    We do NOT raise â€” a misbehaving plugin must not crash the host.
    """
    import logging
