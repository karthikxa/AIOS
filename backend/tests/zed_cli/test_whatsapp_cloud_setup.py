...
Covers:
- Field-shape validators (catch the #1 setup mistake â€” phone number in
  the Phone Number ID field â€” plus the OpenAI / Slack / GitHub token
  paste-by-mistake cases)
- Wizard end-to-end flow with mocked stdin/stdout â€” verifies each step
...
# ---------------------------------------------------------------------------
# Validator tests â€” the cheap, exhaustive coverage layer
# ---------------------------------------------------------------------------
...
class TestPhoneNumberIdValidator:
    ...
    def test_rejects_actual_phone_number_with_helpful_message(self):
        """The #1 setup trap â€” pasting the phone number instead of the ID."""
...
def isolated_home(tmp_path, monkeypatch):
    """Redirect ZED_HOME so save_env_value writes into a temp .env."""
    home = tmp_path / "home"
    zed = home / ".zed"
    zed.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("ZED_HOME", str(zed))
...
def _env_value(zed_home: Path, key: str) -> str | None:
...
class TestWizardFlow:
    ...
    def test_phone_number_id_validator_catches_phone_number(self, isolated_home, monkeypatch):
        """The trap test â€” user pastes their phone number into the
        Phone Number ID field. Wizard MUST reject with a helpful
        explanation, not pass through."""
        inputs = iter([
            "",                                       # press Enter to continue
            "15556422442",                            # phone number â€” rejected
            "",                                       # empty â€” gives up
        ])
        monkeypatch.setattr("builtins.input", lambda *a, **kw: next(inputs))
        buf = io.StringIO()
    ...
    def test_access_token_validator_catches_openai_key(self, isolated_home, monkeypatch):
        ...
        inputs = iter([
            ...
            "sk-proj-" + "x" * 100,                   # OpenAI key â€” rejected
            "",                                       # give up
        ])
        monkeypatch.setattr("builtins.input", lambda *a, **kw: next(inputs))
        buf = io.StringIO()
    ...
    def test_existing_token_preserved_on_rerun(self, isolated_home, monkeypatch):
        ...
        inputs = iter([
            "",                                       # continue
            "",                                       # Phone ID â€” keep existing
            "",                                       # Token â€” keep existing
            "",                                       # App Secret â€” keep existing
            "",                                       # App ID â€” skip
            "",                                       # WABA ID â€” skip
            "",                                       # verify token: regenerate? [y/N] â€” no
            "",                                       # Allowed users â€” keep
        ])
        monkeypatch.setattr("builtins.input", lambda *a, **kw: next(inputs))
        buf = io.StringIO()
        with redirect_stdout(buf):
...
# =========================================================================
# Profile polish block (SETUP COMPLETE â†’ optional WhatsApp profile setup)
# =========================================================================


class TestProfilePolishGuidance:
    """The wizard can't set the bot's WhatsApp display name or profile
    picture via the API â€” those go through Meta's Business Manager UI.
    ...
    def test_polish_block_present_and_points_at_business_manager(
        ...
        assert "24-48h" in out or "24â€“48h" in out
