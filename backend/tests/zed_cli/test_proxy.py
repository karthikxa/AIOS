"""Tests for the `zed proxy` subcommand and its upstream adapters."""
...
from zed_cli.proxy.adapters import ADAPTERS, get_adapter
from zed_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from zed_cli.proxy.adapters.nous_portal import NousPortalAdapter
from zed_cli.proxy.adapters.xai import XAIGrokAdapter
...
def _write_auth_store(zed_home: Path, nous_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given nous state into a hermetic ZED_HOME."""
...
def test_nous_adapter_not_authenticated_when_no_auth_file(tmp_path, monkeypatch):
    # ZED_HOME is already set by conftest, but make doubly sure
...
def test_nous_adapter_authenticated_with_agent_key(tmp_path, monkeypatch):
    ...
    _write_auth_store(tmp_path, {
        ...
        "inference_base_url": "https://inference-api.zedteam.com/v1",
    })
...
def test_nous_adapter_get_credential_uses_runtime_resolver(tmp_path, monkeypatch):
    ...
    with patch(
        "zed_cli.proxy.adapters.nous_portal.resolve_nous_runtime_credentials",
    ...
    assert cred.base_url == "https://inference-api.zedteam.com/v1"
...
def test_nous_adapter_get_credential_raises_when_not_logged_in(tmp_path, monkeypatch):
    monkeypatch.setenv("ZED_HOME", str(tmp_path))
    adapter = NousPortalAdapter()
    with pytest.raises(RuntimeError, match="zed auth add nous"):
...
def test_nous_adapter_concurrent_refresh_serialized(tmp_path, monkeypatch):
    ...
    assert not overlap_detected.is_set(), "refresh calls overlapped â€” lock is broken"
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic ZED_HOME."""
    auth_path = zed_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_refreshes_current_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zed_cli.auth.refresh_xai_oauth_pure", fake_refresh)
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    rate-limited bearer stayed active for the next request â€” defeating
    the whole point of pool rotation.

    Post-fix: 429 lands on ``mark_exhausted_and_rotate`` (no refresh â€”
    ...
    # Refresh must NOT be called on the 429 path â€” guard against
    ...
    monkeypatch.setattr("zed_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
def test_xai_adapter_retry_returns_none_on_429_when_pool_exhausted(tmp_path, monkeypatch):
    """Single-entry pool: 429 has nowhere to rotate to â†’ return None
...
def test_xai_adapter_retry_returns_none_for_unrelated_status(tmp_path, monkeypatch):
    """Non-{401, 429} statuses must NOT trigger any retry â€” pool
...
from zed_cli.proxy.server import create_app  # noqa: E402
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("ZED_HOME", str(tmp_path))
    from zed_cli.proxy.cli import cmd_proxy_status
...
def test_cmd_proxy_providers_runs(capsys):
    from zed_cli.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from zed_cli.proxy.cli import cmd_proxy_start
...
def test_cmd_proxy_start_refuses_when_unauthenticated(capsys, tmp_path, monkeypatch):
    ...
    assert rc == 2
    err = capsys.readouterr().err
    assert "zed auth add nous" in err
