...
def test_format_banner_version_label_without_git_state():
    ...
    assert value == f"Zed Agent v{banner.VERSION} ({banner.RELEASE_DATE})"
...
def test_format_banner_version_label_on_upstream_main():
    ...
    assert value.endswith("Â· upstream b2f477a3")
...
def test_get_git_banner_state_falls_back_to_build_sha_when_no_repo():
    """Docker image case: no .git checkout â€” baked build SHA fills the gap.

    ``_resolve_repo_dir`` returns None when neither the running code's
    parent nor ``$ZED_HOME/zed-agent/`` is a git repo (the canonical
...
def test_get_git_banner_state_returns_none_when_no_repo_and_no_build_sha():
    """Pip-installed wheel with neither git checkout nor baked SHA â†’ None.
...
def test_get_git_banner_state_falls_back_when_live_git_returns_nothing(tmp_path):
    """Shallow clone without origin/main â†’ still surface build SHA if baked.
    ...
    with patch("zed_cli.banner.subprocess.run", return_value=failed), \
