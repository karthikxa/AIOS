...
# ---------------------------------------------------------------------------
# POST /api/auth/ws-ticket â€” the mint endpoint
# ---------------------------------------------------------------------------
...
class TestWsTicketEndpoint:
    ...
    def test_unauthenticated_returns_401_or_redirect(self, gated_app):
        ...
        # gated_auth_middleware short-circuits before the route â€” it
    ...
    def test_get_method_is_not_allowed(self, gated_app):
        ...
        # <img src=â€¦> from a malicious origin). Accepted responses:
        #   401 â€” gated middleware allowlist-miss
        #   404 â€” SPA catch-all swallowed it
        #   405 â€” Method Not Allowed (route only registered for POST)
        #   200 â€” SPA index.html was served (catch-all caught the path)
...
# ---------------------------------------------------------------------------
# _ws_auth_ok â€” unit-level (synthetic WebSocket-shaped object)
# ---------------------------------------------------------------------------
...
def insecure_explicit_host_app():
    ...
    `tailscale serve`) â€” a specific address rather than the all-interfaces
    0.0.0.0 wildcard.
    """
    _reset_for_tests()
    clear_providers()
...
class TestWsAuthOkGated:
    ...
    def test_consumed_ticket_rejected(self, gated_app):
        ...
        # Single-use â€” second consumption fails.
    ...
    def test_rejection_audit_logs(self, gated_app, tmp_path, monkeypatch):
        ...
        # logger may not have been initialized in this process â€” that's
    ...
    def test_internal_credential_not_accepted_in_loopback(self, loopback_app):
        """Outside gated mode, ?internal= is meaningless â€” only ?token= works.
...
class TestWsRequestIsAllowedGated:
    ...
    ``ws.client.host`` is rewritten to the X-Forwarded-For value â€” the
    ...
    def test_non_loopback_peer_rejected_in_loopback_mode(self, loopback_app):
        """Loopback mode still enforces the peer-IP guard â€” the legacy
    ...
    def test_peer_allowed_on_explicit_non_loopback_bind(self, insecure_explicit_host_app):
        ...
        non-loopback opt-in too â€” not just the 0.0.0.0 wildcard.
    ...
    def test_host_origin_guard_still_runs_in_gated_mode(self, gated_app):
        ...
        Host header guard â€” that one still protects against attacker
...
class TestWsHostOriginGuardOrigins:
    ...
    http(s) origins â€” a malicious web page can never forge a non-web origin.
    ...
    def test_loopback_app_scheme_origin_allowed(self, loopback_app):
        ws = self._ws(origin="app://zed", host="127.0.0.1:8080")
        assert web_server._ws_host_origin_is_allowed(ws) is True
    ...
    def test_explicit_non_loopback_file_origin_allowed(self, insecure_explicit_host_app):
        """Packaged Zed Desktop also uses file:// when connecting to a
    ...
    def test_gated_file_origin_allowed(self, gated_app):
        ...
        # it through. This is the regression that broke desktop â†’ hosted
        # gateway connections â€” every WS upgrade got HTTP 403 even with a valid
        # ticket.
...
# ---------------------------------------------------------------------------
# _build_gateway_ws_url â€” the TUI child's primary JSON-RPC backend WS.
# Loopback uses ?token=; gated mode uses the multi-use internal credential
# (NOT a single-use ticket â€” the child reuses this URL across reconnects).
# ---------------------------------------------------------------------------
