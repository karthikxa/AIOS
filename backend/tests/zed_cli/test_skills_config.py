"""Tests for zed_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestGetDisabledSkills:
    ...
    def test_platform_list_unions_with_global(self):
        ...
        # skill (matches issue #46201 â€” global disables hold everywhere).
...
class TestSaveDisabledSkills:
    @patch("zed_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from zed_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("zed_cli.config.load_config")
    ...
    def test_empty_platform_list_keeps_global_disabled(self, mock_load):
        ...
        from tools.skills_tool import _is_skill_disabled
        # An explicit empty platform list does NOT re-enable a globally-disabled
        # skill â€” global disables hold on every platform.
        assert _is_skill_disabled("skill-a", platform="telegram") is True
    ...
    @patch("zed_cli.config.load_config")
    @patch.dict("os.environ", {"ZED_PLATFORM": "discord"})
...
# ---------------------------------------------------------------------------
# get_disabled_skill_names â€” explicit platform param & env var fallback
# ---------------------------------------------------------------------------
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """ZED_SESSION_PLATFORM should be used when ZED_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_zed_platform_takes_precedence(self, tmp_path, monkeypatch):
        """ZED_PLATFORM should win over ZED_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
# ---------------------------------------------------------------------------
# _find_all_skills â€” disabled filtering
# ---------------------------------------------------------------------------
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from zed_cli.skills_config import _get_categories
        skills = [
