...
def test_disabling_tool_writes_include_list(capsys):
    ...
    Standardized on tools.include (whitelist) across the codebase â€” the
    catalog flow, `zed mcp configure`, and this UI all write the same
...
def test_enabling_all_clears_filters(capsys):
    ...
    # User checks all tools â€” pre_selected would be {0} (include mode),
...
def test_modifying_include_stays_in_include_mode(capsys):
    """Changing the selection updates the include list â€” never switches
