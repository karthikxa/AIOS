...
from zed_cli import runtime_provider as rp
...
def test_resolve_runtime_provider_lmstudio_honors_saved_base_url(monkeypatch):
    ...
    the saved `base_url` from `model_cfg` must still win â€” otherwise this
    ...
    # Don't mock resolve_api_key_provider_credentials â€” exercise the real
...
def test_custom_endpoint_uses_saved_config_base_url_when_env_missing(monkeypatch):
    ...
    (issue #28660) â€” local LLM servers get no-key-required instead."""
    monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "openrouter")
    monkeypatch.setattr(
        rp,
        "_get_model_config",
    ...
    assert resolved["base_url"] == "http://127.0.0.1:1234/v1"
    # OPENAI_API_KEY must not leak to an unrelated host â€” local servers get
...
def test_bare_custom_uses_loopback_model_base_url_when_provider_not_custom(monkeypatch):
    ...
    # 127.0.0.1 is not openai.com â€” OPENAI_API_KEY must not leak here
    assert resolved["api_key"] == "no-key-required"
...
def test_bare_custom_resolves_providers_dict_entry_named_custom(monkeypatch):
    ...
    # named-custom path was bypassed â€” that is the bug we are fixing.
    monkeypatch.setattr(
        rp,
        "resolve_provider",
        lambda *a, **k: (_ for _ in ()).throw(
...
def test_bare_custom_without_named_entry_still_falls_through(monkeypatch):
    """No literal providers.custom entry â†’ bare custom keeps the legacy
...
def test_named_custom_provider_uses_providers_dict_when_list_missing(monkeypatch):
    """After v11â†’v12 migration deletes custom_providers, resolution should
...
def test_named_custom_provider_falls_back_to_openai_api_key(monkeypatch):
    ...
    # localhost is not openai.com â€” OPENAI_API_KEY must not leak to local endpoints (#28660)
    assert resolved["api_key"] == "no-key-required"
...
def test_named_custom_provider_does_not_shadow_builtin_provider(monkeypatch):
    ...
    assert resolved["base_url"] == "https://inference-api.zedteam.com/v1"
...
def test_named_custom_provider_wins_over_builtin_alias(monkeypatch):
    ...
    ``provider: kimi``, the built-in alias rewriting (``kimi`` â†’ ``kimi-coding``)
...
def test_named_custom_provider_skipped_for_canonical_built_in(monkeypatch):
    ...
    should NOT be returned â€” the built-in wins as before.
    """
    monkeypatch.setattr(
        rp,
        "load_config",
...
def test_explicit_openrouter_honors_openrouter_base_url_over_pool(monkeypatch):
    ...
    # mirror.example.com is set via OPENROUTER_BASE_URL env â€” api_key should come from env too
...
# â”€â”€ api_mode config override tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_opencode_go_model_derivation_beats_stale_persisted_api_mode(monkeypatch):
    ...
    ``anthropic_messages`` â€” the model dictates the mode, not the stale
    persisted setting.
    """
    monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "opencode-go")
    monkeypatch.setattr(
...
# ---------------------------------------------------------------------------
# GHSA-76xc-57q6-vm5m â€” Ollama URL substring leak
#
...
class TestOllamaUrlSubstringLeak:
    ...
    def test_ollama_key_not_leaked_to_path_injection(self, monkeypatch):
        """http://127.0.0.1:9000/ollama.com/v1 â€” attacker endpoint with
    ...
    def test_ollama_key_not_leaked_to_lookalike_host(self, monkeypatch):
        """ollama.com.attacker.test â€” look-alike host. OLLAMA_API_KEY
        must not be sent."""
        monkeypatch.setenv("OPENAI_API_KEY", "oa-secret")
        monkeypatch.setenv("OLLAMA_API_KEY", "ol-SECRET-should-not-leak")
        monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "custom")
    ...
    def test_ollama_key_sent_to_genuine_ollama_com(self, monkeypatch):
        """https://ollama.com/v1 â€” legit Ollama Cloud. OLLAMA_API_KEY
        should be used."""
        monkeypatch.setenv("OPENAI_API_KEY", "oa-secret")
    ...
    def test_ollama_key_sent_to_ollama_subdomain(self, monkeypatch):
        """https://api.ollama.com/v1 â€” legit subdomain."""
        monkeypatch.setenv("OPENAI_API_KEY", "oa-secret")
        monkeypatch.setenv("OLLAMA_API_KEY", "ol-legit-key")
        monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "custom")
        monkeypatch.setattr(rp, "_get_model_config", lambda: self._make_cfg(
...
# =============================================================================
# Azure Foundry â€” both OpenAI-style and Anthropic-style endpoints
# =============================================================================
...
class TestAzureFoundryResolution:
    ...
    def test_azure_foundry_openai_style_explicit(self, monkeypatch):
        """OpenAI-style Azure Foundry â†’ chat_completions, keeps base_url as-is."""
    ...
    def test_azure_foundry_anthropic_style_strips_v1_suffix(self, monkeypatch):
        """Anthropic-style Azure Foundry â†’ anthropic_messages, /v1 stripped
    ...
    def test_azure_foundry_missing_api_key_raises(self, monkeypatch):
        ...
        # `get_env_value` reads from ~/.zed/.env â€” mock it to return None
    ...
    # ``400 "The requested operation is unsupported."`` â€” the resolver must
    ...
    def test_gpt4o_stays_on_chat_completions(self, monkeypatch):
        """gpt-4o-pure worked on Bob's endpoint â€” must not get upgraded."""
        monkeypatch.setenv("AZURE_FOUNDRY_API_KEY", "az-key")
        monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "azure-foundry")
        monkeypatch.setattr(rp, "_get_model_config",
    ...
    def test_target_model_downgrade_path(self, monkeypatch):
        """/model switch gpt-5.3-codex â†’ gpt-4o: api_mode follows new model."""
        monkeypatch.setenv("AZURE_FOUNDRY_API_KEY", "az-key")
        monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "azure-foundry")
        ...
        # override" â€” the persisted codex_responses survives.)
        assert resolved["api_mode"] == "codex_responses"
...
# Azure Anthropic â€” honor user-specified env var hints (key_env / api_key_env)
#
...
# `key_env: MY_CUSTOM_VAR` on the model config was silently ignored â€” and
# the Azure Foundry docs that showed `api_key_env:` were broken as a result.
#
# These tests lock in the priority chain:
#   1. model_cfg.key_env â†’ os.getenv(value)
#   2. model_cfg.api_key_env â†’ os.getenv(value) (docs alias)
...
class TestAzureAnthropicEnvVarHint:
    ...
    def test_key_env_hint_picks_custom_var(self, monkeypatch):
        """model.key_env names a non-default env var â†’ that var's value is used."""
    ...
    def test_non_azure_anthropic_path_ignores_key_env(self, monkeypatch):
        """key_env is only consulted on Azure endpoints â€” non-Azure Anthropic
        ...
        # The normal chain runs â€” key_env is not consulted off-Azure.
        assert called["resolve_anthropic_token"] is True
        assert resolved["api_key"] == "token-from-resolver"


# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# custom_providers / providers normalizer â€” api_key_env alias for key_env
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestProviderEntryApiKeyEnvAlias:
    ...
    def test_valid_fields_set_lists_key_env(self):
        ...
        from zed_cli.config import _VALID_CUSTOM_PROVIDER_FIELDS
...
# =============================================================================
# Tencent TokenHub â€” API-key provider runtime resolution
# =============================================================================
...
def test_minimax_oauth_runtime_returns_anthropic_messages_mode(monkeypatch):
    ...
    from zed_cli.auth import MINIMAX_OAUTH_GLOBAL_INFERENCE
...
def test_minimax_oauth_runtime_uses_inference_base_url(monkeypatch):
    ...
    from zed_cli.auth import MINIMAX_OAUTH_CN_INFERENCE
...
# ----------------------------------------------------------------------
# GitHub #27132 â€” provider aliases (ollama/vllm/llamacpp/llama-cpp) must
...
def test_custom_aliases_with_lan_base_url_route_to_custom_not_openrouter(
    ...
    # otherwise succeed â€” we want to prove the alias short-circuits before
    # reaching it.
    monkeypatch.setenv("OPENROUTER_API_KEY", "sk-or-fake-test")
    # No custom credential pool â€” exercise the bare-alias path.
    monkeypatch.setattr(rp, "load_pool", lambda provider: None)
...
# ----------------------------------------------------------------------
# Issue #28660 â€” bonus: `<VENDOR>_API_KEY` derivation from host.
...
def test_host_derived_key_does_not_leak_to_lookalike_host(monkeypatch):
    ...
    DEEPSEEK â€” and any real DEEPSEEK_API_KEY stays put."""
    monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "openrouter")
    monkeypatch.setattr(
        rp,
        "_get_model_config",
...
def test_host_derived_key_ignored_for_loopback(monkeypatch):
    ...
    env var â€” there's no meaningful vendor label."""
    monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "openrouter")
    monkeypatch.setattr(
        rp,
        "_get_model_config",
...
def test_host_derived_key_skips_already_handled_vendors(monkeypatch):
    ...
    OLLAMA env vars â€” those are owned by their explicit host-gated paths.
    ...
    monkeypatch.setattr(
        ...
        lambda: {
            "provider": "custom",
            # Hosts like proxy.openai.evil should derive nothing â€” but even
    ...
    # on their own hosts â€” chain falls through to no-key-required.
    assert resolved["api_key"] == "no-key-required"


def test_host_derived_key_helper_basic_cases():
    """Direct unit tests for the host-derive helper itself."""
    # Standard provider hosts â†’ derives correctly.
    import os as _os
    ...
    # Empty / malformed â†’ empty.
    assert rp._host_derived_api_key("") == ""
    assert rp._host_derived_api_key("not a url") == ""

    # Already-handled vendors â†’ empty (guards against bypass of host-gate).
