"""Tests for the kanban CLI surface (zed_cli.kanban)."""
...
from zed_cli import kanban as kc
...
def test_run_slash_session_filter(kanban_home):
    """`zed kanban list --session <id>` filters by the originating
...
def test_board_override_is_isolated_per_concurrent_call(kanban_home, monkeypatch):
    ...
    parser = argparse.ArgumentParser(prog="zed", add_help=False)
...
def test_kanban_in_autocomplete_table():
    from zed_cli.commands import COMMANDS, SUBCOMMANDS
...
# ---------------------------------------------------------------------------
# /kanban specify â€” slash surface (same entry point CLI + gateway use)
# ---------------------------------------------------------------------------
...
def test_run_slash_specify_help_is_reachable(kanban_home):
    """`-h`/`--help` on a subcommand returns the actual help text â€” see
...
def test_run_slash_bare_returns_curated_help(kanban_home):
    """Bare `/kanban` returns the curated short-help block â€” not a 5KB
    argparse usage dump."""
    out = kc.run_slash("")
...
def test_run_slash_unknown_action_friendly_error(kanban_home):
    ...
    with our marker â€” no `(usage error: 2)` wrapping, no doubled
...
def test_run_slash_board_override_restores_prior_env(kanban_home, monkeypatch):
    kb.create_board("alpha")
    kb.create_board("beta")
    monkeypatch.setenv("ZED_KANBAN_BOARD", "beta")

    kc.run_slash("--board alpha list")

    assert os.environ.get("ZED_KANBAN_BOARD") == "beta"
