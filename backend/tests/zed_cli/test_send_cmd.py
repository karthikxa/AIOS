...
def test_load_zed_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``zed send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``zed config set``) lives
    in config.yaml, not in .env â€” and the gateway's config loader reads via
    ``os.getenv(...)``.
    """
    import os
    ...
    zed_home.mkdir()
    (zed_home / ".env").write_text("SOME_TOKEN=abc123\n")
...
def test_load_zed_env_does_not_override_existing(tmp_path, monkeypatch):
    """Existing env vars must not be clobbered by config.yaml values."""
    import os

    zed_home = tmp_path / ".zed"
    zed_home.mkdir()
    (zed_home / "config.yaml").write_text("TELEGRAM_HOME_CHANNEL: yaml_value\n")
...
def test_load_zed_env_handles_missing_files(tmp_path, monkeypatch):
