"""Tests for ``zed update`` / ``--check`` inside the Docker container.
...
fix, ``zed update`` would fall through to ``"âœ— Not a git repository.
Please reinstall: curl ... install.sh"`` â€” that script installs a *new*
host-side Zed, not an update to the running container, so the message
...
from zed_cli.main import _cmd_update_check, cmd_update
...
@patch("zed_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``zed update`` inside Docker â†’ friendly message + exit 1, no git calls."""
    with pytest.raises(SystemExit) as excinfo:
        cmd_update(SimpleNamespace(check=False))

    assert excinfo.value.code == 1
    out = capsys.readouterr().out
    # Spot-check the key guidance â€” exhaustive wording is locked in by the
    ...
    assert "docker pull nousresearch/zed-agent:latest" in out

    # No git invocations â€” the early-return must beat every git command.
...
def test_cmd_update_check_in_docker_prints_guidance_and_exits(
    ...
    """``zed update --check`` inside Docker â†’ same message + exit 1, no fetch."""
    with pytest.raises(SystemExit) as excinfo:
...
def test_cmd_update_on_git_install_does_not_print_docker_message(
    ...
    SystemExit / unrelated errors from the rest of the update flow â€”
    ...
    out to ``git fetch upstream`` / ``git fetch origin`` â€” on CI runners
    ...
    try:
        ...
        # Update flow may exit for unrelated reasons in a stubbed env â€”
...
# ---------- format_docker_update_message â€” content lock ----------
...
def test_format_docker_update_message_contents():
    ...
    # Primary command â€” the entire reason this message exists.
    assert "docker pull nousresearch/zed-agent:latest" in msg
    ...
    assert "ZED_HOME" in msg or "/opt/data" in msg, (
