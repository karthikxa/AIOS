...
Bug â€” newly-routed curated aliases vanished on a native Anthropic setup
    ...
    curated ``_PROVIDER_MODELS["anthropic"]`` list with the live catalog â€”
    curated entries first, live-only models appended, deduped â€” mirroring the
...
def test_anthropic_curated_alias_survives_when_live_omits_it():
    ...
    # Live catalog the API would actually return â€” no fable-5.
