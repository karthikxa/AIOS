"""Tests for parentâ†’subparser flag propagation.

When flags like --yolo, -w, -s exist on both the parent parser and the 'chat'
subparser, placing the flag BEFORE the subcommand (e.g. 'zed --yolo chat')
...
def _build_parser():
    """Build the zed argument parser from the real code.
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("zed_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "zed_cli.banner", fake_banner)
...
class TestYoloEnvVar:
    """Verify --yolo sets ZED_YOLO_MODE regardless of flag position.

    This tests the actual cmd_chat logic pattern (getattr â†’ os.environ).
    """
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `zed gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `zed <argv>` must exit 0 (help) rather than
