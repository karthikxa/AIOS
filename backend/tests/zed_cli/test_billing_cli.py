...
Focus on the non-interactive (no live prompt_toolkit app) path â€” the same
...
@pytest.fixture
def cli():
    obj = ZedCLI.__new__(ZedCLI)  # bypass __init__ (no full app needed)
...
def test_billing_overview_non_interactive_renders_text_not_modal(cli, monkeypatch, capsys):
    ...
    assert "%" in out and ("â–ˆ" in out or "â–‘" in out)
...
def test_billing_limit_screen_readonly(cli, monkeypatch, capsys):
    ...
    # sub-command â€” call it directly the way the overview menu would.
...
def test_billing_sub_arg_ignored_opens_overview(cli, monkeypatch, capsys):
    # A stray sub-arg must NOT error and must NOT dispatch to a sub-screen â€”
    # it just opens the overview (spec Â§0.4: zero sub-commands).
    monkeypatch.setattr(ZedCLI, "_prompt_text_input_modal", _boom_modal, raising=False)
    state = BillingState(
