...
each worker has its own module instance â€” no cross-worker bleed â€” but we
    ...
    def test_mint_and_consume_concurrent(self):
        ...
        def worker(i: int):
            try:
                ...
                with lock:
                    results.append(info)
            except Exception as exc:  # noqa: BLE001 â€” collect for assert
                with lock:
                    errors.append(exc)
...
# â€” _ws_auth_ok exercises these indirectly, but the mint-once, unminted, and
...
class TestInternalCredential:
    ...
    def test_independent_of_ticket_store(self):
        """The internal credential is not a ticket â€” minting tickets doesn't
