"""Unit tests for zed_cli.pty_bridge â€” PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf zed-ok"])
        try:
...
class TestPtyBridgeResize:
    ...
    def test_resize_clamps_wsl_garbage_dimensions(self):
        ...
        # resize() â€” uncaught, since only OSError was handled â€” and broke the
...
class TestClampDimension:
    def test_clamps_above_max(self):
        from zed_cli.pty_bridge import _MAX_COLS, _MAX_ROWS, _clamp_dimension

        assert _clamp_dimension(131072, _MAX_COLS) == _MAX_COLS
        assert _clamp_dimension(131072, _MAX_ROWS) == _MAX_ROWS

    def test_floors_at_one(self):
        from zed_cli.pty_bridge import _MAX_COLS, _clamp_dimension
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$ZED_PTY_TEST\""],
...
class TestPtyBridgeUnavailable:
    """Platform fallback semantics â€” PtyUnavailableError is importable and
