...
import pytest
from zed_cli.model_switch import list_authenticated_providers, switch_model
...
def test_list_authenticated_providers_includes_full_models_list_from_user_providers(monkeypatch):
    ...
    monkeypatch.setattr("zed_cli.providers.ZED_OVERLAYS", {})
...
def test_list_authenticated_providers_enumerates_dict_format_models(monkeypatch):
    ...
    (canonical Zed write format) should surface every key in the picker.

    Regression: the ``providers:`` dict path previously only accepted
    list-format ``models:`` and silently dropped dict-format entries,
    even though Zed's own writer and downstream readers use dict format.
    """
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr("zed_cli.providers.ZED_OVERLAYS", {})
...
def test_list_authenticated_providers_uses_live_models_for_user_provider(monkeypatch):
    ...
    monkeypatch.setattr("zed_cli.models.fetch_api_models", fake_fetch_api_models)
...
def test_list_authenticated_providers_openai_alias_not_emitted_as_phantom(monkeypatch):
    ...
    config entry â€” both of which carry api.openai.com. See model-picker bug."""
    ...
    assert row is None, (
        "bare 'openai' alias must not appear as a standalone picker row â€” "
...
def test_resolve_provider_full_user_config_openai_beats_alias():
    ...
    'openai' â†’ 'openrouter' alias. Regression for the model-picker bug
...
def test_list_authenticated_providers_accepts_base_url_and_singular_model(monkeypatch):
    """providers: dict entries written in canonical Zed shape
    ...
    ``default_model``, so new-shape entries written by Zed's own writer
...
def test_list_authenticated_providers_dedupes_when_user_and_custom_overlap(monkeypatch):
    ...
    # providers: dict wins â€” legacy-only-model is suppressed.
...
def test_list_authenticated_providers_no_duplicate_labels_across_schemas(monkeypatch):
    ...
    emitted ``custom:openrouter`` rows for the same endpoint â€” both labelled
    ...
    # Expect one row per shared entry â€” not two.
...
def test_list_authenticated_providers_hides_custom_shadowing_builtin_endpoint(monkeypatch):
    ...
    # Custom shadow row should be hidden â€” its base_url matches the built-in's.
...
def test_list_authenticated_providers_dedup_honors_base_url_env_override(monkeypatch):
    """The dedup must track the EFFECTIVE endpoint â€” if DASHSCOPE_BASE_URL
...
def test_switch_model_resolves_user_provider_credentials(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "zed_cli.models.validate_requested_model",
...
def test_get_named_custom_provider_reads_transport_field(monkeypatch):
    ...
    â€” silently downgrading every codex_responses / anthropic_messages
...
def test_get_named_custom_provider_legacy_api_mode_field_still_works(monkeypatch):
    ...
    the v12+ providers: dict shape must keep working â€” the migration writer
...
def _run_user_provider_override_case(
    *,
    ...
    with patch("zed_cli.model_switch.resolve_alias", return_value=None), \
         patch("zed_cli.model_switch.list_provider_models", return_value=[]), \
         patch("zed_cli.model_switch.normalize_model_for_provider", side_effect=lambda model, provider: model), \
         patch("zed_cli.models.validate_requested_model", return_value=_REJECTED_VALIDATION), \
         patch("zed_cli.models.detect_provider_for_model", return_value=None), \
         patch("zed_cli.model_switch.get_model_info", return_value=None), \
         patch("zed_cli.model_switch.get_model_capabilities", return_value=None), \
         patch("zed_cli.runtime_provider.resolve_runtime_provider", return_value={"api_key": "***", "base_url": base_url, "api_mode": "anthropic_messages"}):
        return switch_model(
            raw_input=raw_input,
...
def test_section3_probes_no_key_endpoint_without_explicit_models(monkeypatch):
    ...
    still probe /v1/models for live discovery â€” mirroring section 4's policy.
    ...
    monkeypatch.setattr("zed_cli.models.fetch_api_models", _fake_fetch)

    user_providers = {
        "local-llamacpp": {
            "name": "Local llama.cpp",
            "api": "http://localhost:8080/v1",
            # No api_key, no models list â€” bare local endpoint.
        }
    }
...
def test_section3_skips_probe_when_no_key_but_explicit_models(monkeypatch):
    ...
    public endpoint to a subset â€” skip live discovery and keep the list."""
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr("zed_cli.providers.ZED_OVERLAYS", {})
    ...
    monkeypatch.setattr("zed_cli.models.fetch_api_models", _fail_fetch)
