...
catalog exposes, plus how each provider's ``auth_type`` maps to a desktop tab â€”
...
def test_catalog_covers_every_zed_model_provider():
    """PARITY CONTRACT: the catalog == the `zed model` universe."""
    slugs = {d.slug for d in provider_catalog()}
    for entry in CANONICAL_PROVIDERS:
        assert entry.slug in slugs, (
            f"{entry.slug} is shown in `zed model` but missing from provider_catalog()"
        )


def test_catalog_has_no_providers_outside_zed_model():
    """The catalog must not invent providers `zed model` doesn't show."""
...
def test_api_key_providers_route_to_keys_oauth_to_accounts():
    ...
    # account / sign-in flows â†’ accounts
...
def test_copilot_surfaces_as_a_provider_with_its_own_token_var():
    ...
    keys tab â€” but its PRIMARY credential var must be the provider-owned
...
def test_api_key_providers_expose_a_credential_env_var():
    ...
    Exemptions: ``aws_sdk`` (bedrock â€” uses AWS_REGION/AWS_PROFILE) and the
