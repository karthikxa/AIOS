...
# ---------------------------------------------------------------------------
# _offer_openclaw_migration â€” unit tests
# ---------------------------------------------------------------------------
...
class TestOfferOpenclawMigration:
    ...
    def test_runs_migration_when_user_accepts(self, tmp_path):
        ...
        # Create a fake zed home with config
        ...
        fake_mod = ModuleType("openclaw_to_zed")
        ...
        fake_migrator.migrate.return_value = {
            ...
            "output_dir": str(zed_home / "migration"),
        }
        ...
        with (
            ...
            result = setup_mod._offer_openclaw_migration(zed_home)
    ...
    def test_creates_config_if_missing(self, tmp_path):
        ...
        with (
            ...
            setup_mod._offer_openclaw_migration(zed_home)
...
# ---------------------------------------------------------------------------
# Integration with run_setup_wizard â€” first-time flow
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# _get_section_config_summary / _skip_configured_section â€” unit tests
# ---------------------------------------------------------------------------
...
class TestGetSectionConfigSummary:
    ...
    def test_model_ignores_bare_gh_token(self):
        ...
        configured inference provider on its own â€” mirrors the copilot
    ...
    def test_model_ignores_claude_code_oauth_token(self):
        ...
        trigger skip â€” mirrors the _IMPLICIT_ENV_VARS guard in
    ...
    def test_model_copilot_recognised_when_explicitly_chosen(self):
        """If the user picked copilot in config, GH_TOKEN *does* count â€”
...
class TestSetupWizardSkipsConfiguredSections:
    ...
    def test_sections_skipped_when_migration_imported_settings(self, tmp_path):
        ...
        # Track whether migration has "run" â€” after it does, API key is available
        ...
        # Model has API key â†’ skip offered, user said No â†’ section NOT called
        mock_model.assert_not_called()
        # Terminal/agent always have a summary â†’ skip offered, user said No
        mock_terminal.assert_not_called()
        mock_agent.assert_not_called()
        # Gateway has no tokens (env_side returns "" for gateway keys) â†’ section runs
