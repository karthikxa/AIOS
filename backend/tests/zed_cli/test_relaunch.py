...
class TestResolveZedBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        ...
        assert relaunch_mod.resolve_zed_bin() == fake
    ...
    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        ...
        # Ensure we don't accidentally match a real 'zed' on PATH
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_zed_bin() == str(fake)

    def test_falls_back_to_path_which(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/zed" if name == "zed" else None
        )
        assert relaunch_mod.resolve_zed_bin() == "/usr/bin/zed"
...
class TestBuildRelaunchArgv:
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/zed", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/zed", ["/usr/bin/zed", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        zed).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead."""
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_zed_bin", lambda: r"C:\Users\test\zed.exe")
        ...
        # execvp MUST NOT be called on Windows â€” route must go through subprocess
        execvp_calls = []
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\zed.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_zed_bin", lambda: r"C:\zed.exe")
    ...
    def test_windows_surfaces_oserror_with_help(self, monkeypatch, capsys):
        ...
        we must NOT let it bubble up as a cryptic traceback â€” print a
        user-readable hint and sys.exit(1)."""
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_zed_bin", lambda: r"C:\missing.exe")
...
class TestResolveZedBinWindowsPyGuard:
    """On Windows, resolve_zed_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a zed.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\zed.exe" if name == "zed" else None,
        )

        bin_path = relaunch_mod.resolve_zed_bin()
        # Must NOT be the .py â€” must be the zed.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\zed.exe"
    ...
    def test_posix_still_accepts_py_argv0(self, monkeypatch, tmp_path):
        ...
        script.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(script), "chat"])
        assert relaunch_mod.resolve_zed_bin() == str(script)
