...
- Bare `zed setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `zed setup --quick` runs the narrower "fill in missing items" flow.
- `zed setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.

On a fresh install, all three are no-ops â€” fall through to first-time setup.
"""
...
class TestExistingInstallDefault:
    """Bare `zed setup` on an existing install = full reconfigure wizard."""

    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        """No menu, no prompt_choice â€” just run every section in sequence."""
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`zed setup --reconfigure` behaves the same as bare `zed setup`."""
