...
from zed_cli.skills_hub import do_check, do_install, do_list, do_update, handle_skills_slash
...
def _capture_update(monkeypatch, results) -> tuple[str, list[tuple[str, str, bool]]]:
    import tools.skills_hub as hub
    import zed_cli.skills_hub as cli_hub
...
def test_do_list_platform_env_is_ignored(three_source_env, monkeypatch):
    """`zed skills list` reads the active profile's config via
    ZED_HOME (swapped by -p), so it must NOT pass a platform arg to
    ``get_disabled_skill_names`` â€” otherwise per-platform overrides
    would silently leak in from ZED_PLATFORM env."""
    from agent import skill_utils
...
def test_url_install_actionable_error_on_non_interactive_with_no_name(monkeypatch, tmp_path, hub_env):
    ...
    do_install(
        "https://example.com/SKILL.md",
        console=console, skip_confirm=True,
        # No name_override â€” should error out with a retry hint.
    )
...
def test_url_install_prompts_category_and_uses_typed_value(monkeypatch, tmp_path, hub_env):
    ...
    # Name is already resolved (from frontmatter) â†’ only category prompt fires.
...
def test_url_install_cancel_name_prompt_aborts(monkeypatch, tmp_path, hub_env):
    ...
    # Empty input with no default â†’ name prompt returns None â†’ abort.
...
# â”€â”€ _existing_categories â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


def test_existing_categories_skips_top_level_skills(monkeypatch, tmp_path, hub_env):
    import tools.skills_hub as hub
    from zed_cli.skills_hub import _existing_categories
    ...
    # Empty dir (NOT a category â€” no SKILL.md below).
...
def test_existing_categories_returns_empty_when_skills_dir_missing(monkeypatch, tmp_path, hub_env):
    # hub_env creates tmp_path/skills/.hub â€” we point SKILLS_DIR at a missing sibling.
    import tools.skills_hub as hub
...
# ---------------------------------------------------------------------------
# browse_skills â€” dedup by identifier, not name
# ---------------------------------------------------------------------------
...
def test_browse_skills_dedup_uses_identifier_not_name(monkeypatch):
    ...
    # so the patch must target the source module, not zed_cli.skills_hub.
...
def test_do_browse_reports_live_per_source_progress():
    ...
    from zed_cli.skills_hub import do_browse
    from tools.skills_hub import SkillMeta
    ...
    def fake_parallel(sources, query="", per_source_limits=None,
                      ...
                      on_source_done=None):
        # Simulate two sources completing â€” the callback must be wired through.
...
# ---------------------------------------------------------------------------
# Regression: full identifier must be recoverable from `zed skills search`
...
def test_do_search_identifier_column_does_not_truncate_long_slug():
    ...
    Rich would render `browse-sh/weatherâ€¦` and lose the hash.
    """
    from zed_cli.skills_hub import do_search

    sink = StringIO()
    # Narrow width forces Rich to apply overflow rules â€” exactly the scenario
    ...
    # the table by walking lines that look like table rows ("â”‚ ... â”‚") and
    # taking the last `â”‚...â”‚` cell. Concatenating those chunks must yield
    # the full slug.
    chunks = []
    for line in output.splitlines():
        # Table data rows start and end with the box-drawing vertical bar.
        if not line.startswith("â”‚") or not line.rstrip().endswith("â”‚"):
            continue
        # Last `â”‚ ... â”‚` cell on the row is the Identifier column.
        last_cell = line.rstrip().rsplit("â”‚", 2)[-2].strip()
        if last_cell:
...
def test_do_search_json_flag_emits_full_identifiers(capsys):
    ...
    # Table render must be suppressed â€” sink should be empty (no "Searching for:" header).
