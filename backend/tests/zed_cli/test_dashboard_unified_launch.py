"""Tests for the unified profileâ†’machine dashboard launch routing.
...
class TestUnifiedDashboardRouting:
    ...
    def test_reexec_pins_docker_machine_root(self, main_mod, monkeypatch):
        """In the Docker layout (ZED_HOME=/opt/data, profiles under
        /opt/data/profiles/<name>) the reroute must pin the child to the
        machine root /opt/data â€” NOT drop ZED_HOME.

        Dropping it makes the child fall back to $HOME/.zed
        (= /opt/data/.zed), an empty auto-seeded home, so the dashboard
        ...
        # get_default_zed_root() strips the trailing profiles/<name>, so the
        # child binds /opt/data â€” where the real default/oracle/saga profiles
        # and the .install_method stamp actually live.
        assert env.get("ZED_HOME") == "/opt/data"

    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (ZED_DESKTOP=1) must NOT
        reroute into the machine dashboard. The reroute re-execs as the default
        profile and exits, so the desktop never sees a ready backend â†’ boot
