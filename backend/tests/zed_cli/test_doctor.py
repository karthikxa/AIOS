...
import zed_cli.doctor as doctor
import zed_cli.gateway as gateway_cli
from zed_cli import doctor as doctor_mod
from zed_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `zed doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        env_path.write_text(
            "OPENAI_API_KEY=sk-test  # em-dash here â€” should not crash\n",
            encoding="utf-8",
        )

        monkeypatch.setattr(doctor_mod, "ZED_HOME", zed_home)
        ...
        # Short-circuit the expensive tool-availability probe â€” we only
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("ZED_KANBAN_TASK", "probe")
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("ZED_INTERACTIVE")
        raise SystemExit(0)
...
# â”€â”€ Memory provider section (doctor should only check the *active* provider) â”€â”€


class TestDoctorMemoryProviderSection:
    """The â—† Memory Provider section should respect memory.provider config."""

    def _make_zed_home(self, tmp_path, provider=""):
        """Create a minimal ZED_HOME with config.yaml."""
        home = tmp_path / ".zed"
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_zed_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "ZED_HOME", home)
        ...
        try:
            from zed_cli import auth as _auth_mod
...
def test_run_doctor_accepts_zed_provider_ids_that_catalog_aliases(
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from zed_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("zed_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "zed_cli.doctor", raising=False)
        from zed_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
# ---------------------------------------------------------------------------
# â—† Auth Providers â€” xAI OAuth display in run_doctor()
# ---------------------------------------------------------------------------


class TestDoctorXaiOAuthStatus:
    """The â—† Auth Providers section must show xAI OAuth login state.
    ...
    def test_import_failure_does_not_crash_doctor(self, monkeypatch, tmp_path):
        ...
        with contextlib.redirect_stdout(buf):
            doctor_mod.run_doctor(Namespace(fix=False))
        out = buf.getvalue()
        # The â—† Auth Providers header must still appear â€” other providers unaffected.
    ...
    def test_function_returns_none_does_not_crash_doctor(self, monkeypatch, tmp_path):
        """None return is normalised to {} via `or {}` â€” must not AttributeError."""
        out = self._run(monkeypatch, tmp_path, xai_auth_fn=lambda: None)
        # None â†’ {} â†’ logged_in falsy â†’ shows not-logged-in warn
        assert "xAI OAuth" in out
        assert "(not logged in)" in out


# ---------------------------------------------------------------------------
# â—† Auth Providers â€” codex CLI import hint placement (issue #27975)
# ---------------------------------------------------------------------------
...
class TestDoctorCodexCliHintPlacement:
    ...
    def test_hint_never_attaches_to_minimax_row(self, monkeypatch, tmp_path):
        out = self._run(monkeypatch, tmp_path, codex_logged_in=False, codex_cli_present=False)
        # The MiniMax OAuth row and the hint must not be adjacent â€” the hint
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale ZED_MAX_ITERATIONS in .env shadows
    ...
    The detector reads the .env FILE directly, NOT os.environ â€” the gateway
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        zed_home = tmp_path / ".zed"
        zed_home.mkdir(parents=True)
        (zed_home / "config.yaml").write_text(
            f"agent:\n  max_turns: {cfg_turns}\n", encoding="utf-8"
        )
        env_lines = ["OPENAI_API_KEY=sk-test\n"]
        if ghost is not None:
            env_lines.append(f"ZED_MAX_ITERATIONS={ghost}\n")
        (zed_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "ZED_HOME", zed_home)
        monkeypatch.setattr(doctor_mod, "get_zed_home", lambda: zed_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("ZED_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("ZED_MAX_ITERATIONS", raising=False)

        # Short-circuit at the Tool Availability stage â€” the drift check runs
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale ZED_MAX_ITERATIONS" in out
        env_after = (zed_home / ".env").read_text(encoding="utf-8")
        assert "ZED_MAX_ITERATIONS" not in env_after
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    """`zed doctor` must not hand users `npm audit fix --workspace <name>`:
    ...
    # Only npm is "installed" â€” keeps the rest of run_doctor's external checks
    ...
    # left thinking a crashing command means a broken Zed install.
