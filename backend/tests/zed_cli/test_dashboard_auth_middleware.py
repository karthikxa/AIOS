...
without any external IDP.  Exercises:
  ...
  * Full /auth/login â†’ /auth/callback â†’ / round trip with the stub
...
def test_other_public_api_paths_are_public_under_gate(gated_app, path):
    ...
    dependency) â€” but NEVER 401, and NEVER a 302 to ``/login``.
    """
    r = gated_app.get(path, follow_redirects=False)
    assert r.status_code != 401, (
        f"{path} returned 401 under the OAuth gate â€” should be public"
    )
    if r.status_code == 302:
        location = r.headers.get("location", "")
        assert "/login" not in location, (
            f"{path} redirected to {location} â€” should be public, "
...
def test_gated_static_asset_path_is_public(gated_app):
    ...
    # 404 not 401 â€” proves middleware let the request through to the
...
def test_full_login_round_trip_unlocks_gated_api(gated_app):
    # 1) Click "Sign in with Stub IdP" â€” /auth/login redirects to the stub
    ...
    assert any("zed_session_at" in c for c in set_cookies)
    assert any("zed_session_rt" in c for c in set_cookies)

    # 3) A gated API route (``/api/sessions``) now succeeds because we
    #    have a valid session cookie. (We deliberately don't probe
    #    ``/api/status`` here â€” it's in the shared PUBLIC_API_PATHS
...
def _complete_stub_login(client) -> None:
    ...
    client's cookie jar holds ``zed_session_at`` / ``zed_session_rt``
...
def test_gated_require_token_endpoint_accepts_cookie_session(gated_app):
    ...
    ``_require_token`` directly â€” plugin install/enable/disable,
    ``/api/dashboard/plugins/hub``, and others â€” used to re-check the absent
    ...
    400. The assertion is simply "not 401" â€” proving auth succeeded without
...
def test_gated_require_token_endpoint_still_rejects_no_cookie(gated_app):
    ...
    The fix defers to the gate â€” it does not make these endpoints public. A
...
# 401'd â€” i.e. it cleared the auth layer and reached the handler. The
...
def test_gated_require_token_routes_accept_cookie_session(
    ...
    ``test_gated_require_token_endpoint_accepts_cookie_session`` â€” this just
    ...
    assert r.status_code != 401, (
        ...
        f"the OAuth gate â€” _require_token still rejecting a valid session. "
        f"Body: {r.text}"
    )
...
def test_callback_without_pkce_cookie_returns_400(gated_app):
    # No prior /auth/login â†’ no PKCE cookie.
    r = gated_app.get(
...
def test_logout_clears_cookies_and_redirects_to_login(gated_app):
    ...
    assert any(
        c.startswith("zed_session_at=") and "Max-Age=0" in c
        for c in set_cookies
    )
    assert any(
        c.startswith("zed_session_rt=") and "Max-Age=0" in c
        for c in set_cookies
    )
...
class _UnreachableProvider(StubAuthProvider):
    ...
    Models the real-world bug â€” a self-hosted-OIDC session hits the ``nous``
...
def _mint_stub_at(stub: StubAuthProvider) -> str:
    ...
    state = dict(
        seg.split("=", 1)
        for seg in ls.cookie_payload["zed_session_pkce"].split(";")
        if "=" in seg
    )["state"]
    verifier = dict(
        seg.split("=", 1)
...
def test_unreachable_first_provider_does_not_block_second(_gated_state):
    ...
    register_provider(_UnreachableProvider())  # registered first â†’ tried first
...
def test_all_providers_unreachable_returns_503(_gated_state):
    ...
    client = _gated_state()
    # Any non-empty cookie â€” the unreachable provider raises before parsing.
...
def test_unverifiable_token_with_reachable_providers_redirects(_gated_state):
    ...
    return None, none raises), the gate falls through to re-login â€” NOT 503."""
    register_provider(StubAuthProvider())
    client = _gated_state()
    client.cookies.set(SESSION_AT_COOKIE, "garbage-not-a-real-token")
    # API path â†’ 401; HTML would 302. Either way, NOT 503.
    r = client.get("/api/auth/me")
    assert r.status_code == 401
