...
Covers:
- Defense 1: gateway stop/restart refuse when _ZED_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "zed gateway stop",
        "zed gateway start",
        "zed  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "ZED GATEWAY RESTART",           # uppercase
    ])
    def test_zed_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.zed.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.zed.gateway.plist",
        "launchctl stop ai.zed.gateway",
        "systemctl restart zed-gateway",
        "systemctl stop zed-gateway.service",
        "systemctl start zed-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill zed gateway process",
        "pkill -f zed.*gateway",
    ])
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_zed_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade zed then run zed gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.zed.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_allow_empty_prompt(self, capsys):
        """Empty prompt (no lifecycle content) should pass the filter â€” the
        ...
        # The API rejects it for "requires prompt or skill" â†’ rc 1, but
...
class TestGatewaySelfTargetingGuard:
    """Verify zed gateway stop/restart refuse when _ZED_GATEWAY=1."""
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _ZED_GATEWAY=1.

    Issue #37453: systemctl --user restart zed-gateway runs as a child of the
    gateway process.  When systemd delivers SIGTERM the gateway kills its own
    restart command mid-execution â€” the service may never restart.  The guard
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart zed-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-zed systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        """Without _ZED_GATEWAY=1 the lifecycle guard must not fire."""
        import tools.terminal_tool as tt
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart zed-gateway"))

        # Outside the gateway the lifecycle guard doesn't block â€” the normal
