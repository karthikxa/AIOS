...
profile switcher can target any profile's ZED_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via ZED_HOME.
"""
import pytest
import yaml
...
def isolated_profiles(tmp_path, monkeypatch, _isolate_zed_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_zed_home", lambda: default_home)
...
class TestProfileScopedConfig:
    ...
    def test_config_query_param_equivalent_to_body(self, client, isolated_profiles):
        """The SPA's fetchJSON injects ?profile= â€” must scope like body.profile."""
        resp = client.put(
    ...
    def test_config_raw_path_reflects_requested_profile(self, client, isolated_profiles):
        """The Config page header shows /api/config/raw's ``path`` â€” it must
...
class TestProfileScopedMcp:
    ...
    def test_mcp_probe_runs_inside_profile_scope(
        ...
        import zed_cli.mcp_config as mcp_config
        from zed_constants import get_zed_home
...
class TestProfileScopedModel:
    ...
    def test_auxiliary_read_scoped_matches_write_target(
        ...
        /api/model/set wrote scoped â€” the Models page displayed the
    ...
    def test_model_options_scoped_to_profile(self, client, isolated_profiles):
        """The Models picker must read the SAME profile model/set writes â€”
    ...
    def test_model_info_unknown_profile_404(self, client, isolated_profiles):
        ...
        with empty model info ("no model set" â€” silently wrong)."""
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same ZED_HOME the rest
        ...
        monkeypatch.setattr(
            "zed_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid():
            seen_homes.append(str(get_zed_home()))
            return None
        ...
        assert resp.json()["zed_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_zed_home(self, isolated_profiles, monkeypatch):
        import zed_cli.web_server as web_server

        monkeypatch.setattr(
            "zed_cli.main._make_tui_argv",
            lambda root, tui_dev=False: (["cat"], None),
            raising=False,
        )
        argv, cwd, env = web_server._resolve_chat_argv(profile="worker_beta")
        assert env is not None
        assert env["ZED_HOME"] == str(isolated_profiles["worker_beta"])
        # Scoped chat must NOT attach to the dashboard's in-memory gateway.
        assert "ZED_TUI_GATEWAY_URL" not in env
    ...
    def test_chat_argv_unscoped_keeps_legacy_env(self, isolated_profiles, monkeypatch):
        ...
        assert env is not None
        assert env.get("ZED_HOME") != str(isolated_profiles["worker_beta"])
    ...
    def test_chat_argv_unknown_profile_raises(self, isolated_profiles, monkeypatch):
        ...
        # Reuse the HTTPException class web_server itself raises â€” avoids a
