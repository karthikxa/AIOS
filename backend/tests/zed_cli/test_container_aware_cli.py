...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ZED_HOME with .container-mode file."""
    zed_home = tmp_path / ".zed"
    zed_home.mkdir()
    monkeypatch.setenv("ZED_HOME", str(zed_home))
    monkeypatch.delenv("ZED_DEV", raising=False)

    container_mode = zed_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=zed-agent\n"
        "exec_user=zed\n"
        "zed_bin=/data/current-package/bin/zed\n"
    )
    return zed_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("zed_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "zed-agent"
    assert info["exec_user"] == "zed"
    assert info["zed_bin"] == "/data/current-package/bin/zed"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("zed_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_zed_dev(container_env, monkeypatch):
    """Returns None when ZED_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ZED_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_zed_dev_zero(container_env, monkeypatch):
    """ZED_DEV=0 does NOT trigger bypass â€” only '1' does."""
    monkeypatch.setenv("ZED_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        zed_home = Path(tmpdir) / ".zed"
        zed_home.mkdir()
        (zed_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("zed_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_zed_home": lambda: zed_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZED_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=zed-custom\n"
        "exec_user=myuser\n"
        "zed_bin=/opt/zed/bin/zed\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "zed-custom"
    assert info["exec_user"] == "myuser"
    assert info["zed_bin"] == "/opt/zed/bin/zed"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from zed_cli.main import _exec_in_container
