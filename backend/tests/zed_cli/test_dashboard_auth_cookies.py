...
# Cookie name resolution helpers used throughout â€” the bare name resolves
...
def test_session_cookies_use_host_prefix_on_https_direct():
    """HTTPS + no proxy prefix â†’ __Host- prefix (strongest spec
...
def test_session_cookies_use_secure_prefix_when_proxied():
    """HTTPS + /zed prefix â†’ __Secure- prefix (__Host- forbids
    Path != "/"; __Secure- keeps the Secure-required hardening)."""
    client = TestClient(_build_app(use_https=True, prefix="/zed"))
    r = client.get("/set")
    cookies = r.headers.get_list("set-cookie")
    at = next(c for c in cookies if c.startswith(f"__Secure-{SESSION_AT_COOKIE}="))
    assert "Path=/zed" in at
    assert "Secure" in at
...
def test_detect_https_via_scheme():
    ...
    """
    from zed_cli.dashboard_auth.cookies import detect_https
    http_req = Request({
