"""Tests for xAI Grok OAuth â€” tokens stored in Zed auth store (~/.zed/auth.json)."""
...
def _setup_zed_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the Zed auth store at the given root."""
...
def _patch_httpx_client(monkeypatch, response):
    ...
    monkeypatch.setattr("zed_cli.auth.httpx.Client", _factory)
    return holder
...
def test_xai_oauth_authorize_url_includes_referrer_zed_agent():
    """Attribution: xAI's OAuth server can identify Zed-originated logins
    ...
    params = _parse_authorize_url(url)
    assert params["referrer"] == "zed-agent"
...
def test_xai_callback_handler_returns_400_when_callback_url_lacks_code_and_error():
    ...
    out â€” leaving the user with a contradictory success page and a CLI error.
    """
    server, thread, result, redirect_uri = _xai_start_callback_server(preferred_port=0)
    try:
        ...
        assert "zed auth add xai-oauth" in body
...
def test_save_and_read_xai_oauth_tokens_roundtrip(tmp_path, monkeypatch):
    ...
    zed_home.mkdir(parents=True, exist_ok=True)
    (zed_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
...
def test_read_xai_oauth_tokens_missing_access_token(tmp_path, monkeypatch):
    zed_home = tmp_path / "zed"
    _setup_zed_auth(zed_home, access_token="")
    monkeypatch.setenv("ZED_HOME", str(zed_home))
...
def test_read_xai_oauth_tokens_missing_refresh_token(tmp_path, monkeypatch):
    zed_home = tmp_path / "zed"
    _setup_zed_auth(zed_home, refresh_token="")
    monkeypatch.setenv("ZED_HOME", str(zed_home))
...
def test_resolve_xai_runtime_credentials_returns_singleton_state(tmp_path, monkeypatch):
    zed_home = tmp_path / "zed"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_zed_auth(zed_home, access_token=fresh)
    monkeypatch.setenv("ZED_HOME", str(zed_home))
    monkeypatch.delenv("ZED_XAI_BASE_URL", raising=False)
    ...
    assert creds["source"] == "zed-auth-store"
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zed_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
def test_resolve_xai_runtime_credentials_honours_env_base_url(tmp_path, monkeypatch):
    ...
    _setup_zed_auth(zed_home, access_token=fresh)
    monkeypatch.setenv("ZED_HOME", str(zed_home))
    monkeypatch.setenv("ZED_XAI_BASE_URL", "https://custom.x.ai/v1/")
...
# ``XAI_BASE_URL`` / ``ZED_XAI_BASE_URL`` are a credential-leak vector
...
def _seed_xai_oauth_state(
    zed_home: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    zed_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
    ...
    (zed_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    zed_home = tmp_path / "zed"
    _seed_xai_oauth_state(zed_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="nous")
    monkeypatch.setenv("ZED_HOME", str(zed_home))
    ...
    monkeypatch.setattr("zed_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
    ...
    raw = json.loads((zed_home / "auth.json").read_text())
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    NOT trigger the quarantine path â€” tokens stay on disk for the next attempt.
    """
    zed_home = tmp_path / "zed"
    _seed_xai_oauth_state(zed_home, dict(_STALE_XAI_OAUTH_STATE))
    monkeypatch.setenv("ZED_HOME", str(zed_home))
    ...
    monkeypatch.setattr("zed_cli.auth._refresh_xai_oauth_tokens", _transient_refresh)
    ...
    # Tokens must be untouched â€” no quarantine on transient errors.
    raw = json.loads((zed_home / "auth.json").read_text())
    tokens = raw["providers"]["xai-oauth"]["tokens"]
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    Regression test for #26847 â€” xAI's backend has been seen to 403
    standard SuperGrok subscribers despite the in-app subscription
    being active. Re-running ``zed model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``zed model``
    is actively wrong â€” re-logging in won't change xAI's allowlist
    ...
    err = AuthError(
        ...
        "This OAuth account is not authorized for xAI API access â€” "
    ...
    assert "zed model" not in rendered.lower()
...
def test_refresh_xai_oauth_pure_keeps_refresh_token_when_response_omits_it(monkeypatch):
    """Some OAuth providers don't rotate refresh tokens â€” preserve the old one."""
    new_access = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    response = _StubHTTPResponse(
        200,
        {
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    ``json.JSONDecodeError`` escape â€” so the message reads as an auth
    problem instead of an internal parsing crash."""
    from zed_cli.auth import _xai_oauth_discovery
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    Zed that didn't validate) must not be silently honored on the refresh
    hot path. A non-HTTPS ``token_endpoint`` would leak the refresh_token in
    cleartext on every refresh; refuse before the POST."""
    # No HTTP stub installed â€” refresh must fail at validation, not at POST.
    with pytest.raises(AuthError) as exc:
        refresh_xai_oauth_pure(
...
def test_refresh_xai_oauth_pure_rejects_off_origin_token_endpoint(monkeypatch):
    ...
    infrastructure â€” every subsequent refresh would silently leak the
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    this attack worth defending against â€” one MITM = forever credential
    leak.)"""
    from zed_cli.auth import _xai_oauth_discovery
    ...
    monkeypatch.setattr("zed_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `zed model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `zed auth list` reflects truth and
    refreshes route through the pool consistently with codex."""
    from agent.credential_pool import load_pool

    zed_home = tmp_path / "zed"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_zed_auth(zed_home, access_token=fresh, refresh_token="rt-1")
    monkeypatch.setenv("ZED_HOME", str(zed_home))
...
def test_credential_pool_seed_respects_suppression(tmp_path, monkeypatch):
    """`zed auth remove xai-oauth <N>` for the seeded entry suppresses
    ...
    # Suppress the source â€” mimic `zed auth remove`.
    from zed_cli.auth import suppress_credential_source
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``zed auth remove xai-oauth 1`` for a
    singleton-seeded entry must clear auth.json providers.xai-oauth AND
    suppress further re-seeding â€” otherwise the next ``load_pool`` call
    ...
    and ``auth_remove_command`` fell through to the "unregistered source â€”
    ...
    # Act: the user runs `zed auth remove xai-oauth 1`.
    ...
    # nothing to import.
    raw_after = json.loads((zed_home / "auth.json").read_text())
    assert "xai-oauth" not in raw_after.get("providers", {}), (
        "auth.json providers.xai-oauth must be cleared â€” otherwise the "
    ...
    assert not pool_after.has_credentials(), (
        "Removal must stick across load_pool() calls â€” without the "
        "loopback_pkce RemovalStep, the seed function reads the singleton "
        "and rebuilds the entry on every Zed invocation."
    )
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_zed_auth(zed_home, access_token=expired, refresh_token="rt-old")
    monkeypatch.setenv("ZED_HOME", str(zed_home))
    ...
    monkeypatch.setattr("zed_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
    ...
    # Singleton must reflect refreshed tokens â€” otherwise the next process
...
def test_pool_entry_needs_refresh_when_jwt_within_skew(tmp_path, monkeypatch):
    ...
    is within the XAI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS window â€” otherwise a
    near-expired token will hit the API and 401 unnecessarily.  Mirrors the
    Codex skew-window behavior."""
    from agent.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    from zed_cli.auth import XAI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS
    import uuid
    ...
    # Token expires in 30s â€” well inside the proactive refresh skew window.
...
def test_pool_select_proactively_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    the API call â€” separate from the 401-reactive path."""
    from agent.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    import uuid
...
def test_pool_try_refresh_current_handles_xai_oauth(tmp_path, monkeypatch):
    ...
    must work for xai-oauth alongside openai-codex â€” otherwise mid-call
...
def test_pool_refresh_marks_entry_exhausted_on_failure(tmp_path, monkeypatch):
    ...
    failover path â€” _recover_with_credential_pool rotates to the next entry
    ...
    monkeypatch.setattr("zed_cli.auth.refresh_xai_oauth_pure", _fake_refresh_fail)
...
def test_pool_seeded_entry_sync_back_after_refresh(tmp_path, monkeypatch):
    ...
    zed_home = tmp_path / "zed"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_zed_auth(zed_home, access_token=near_expiry, refresh_token="rt-singleton")
    monkeypatch.setenv("ZED_HOME", str(zed_home))
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    """Multi-process race: another Zed process refreshed the singleton
    ...
    profiles + Zed processes."""
    from agent.credential_pool import load_pool

    zed_home = tmp_path / "zed"
    in_memory_at = _jwt_with_exp(int(time.time()) + 30)  # near-expiry
    _setup_zed_auth(zed_home, access_token=in_memory_at, refresh_token="rt-stale")
    monkeypatch.setenv("ZED_HOME", str(zed_home))
    ...
    (zed_home / "auth.json").write_text(json.dumps(raw))
    ...
    def _fake_refresh(access_token, refresh_token, **kwargs):
        ...
        # POSTing the refresh â€” otherwise it would replay the stale one.
...
def test_pool_refresh_recovers_when_other_process_already_refreshed(tmp_path, monkeypatch):
    ...
    _setup_zed_auth(zed_home, access_token=in_memory_at, refresh_token="rt-shared")
    monkeypatch.setenv("ZED_HOME", str(zed_home))
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    user runs ``zed model`` -> xAI Grok OAuth (or another process
    ...
    _setup_zed_auth(zed_home, access_token=stale_at, refresh_token="rt-stale")
    monkeypatch.setenv("ZED_HOME", str(zed_home))
    ...
    # Simulate the user re-running `zed model` -> xAI Grok OAuth: the
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    ``zed auth add xai-oauth``) own their own refresh-token lifecycle
    ...
    _setup_zed_auth(zed_home, access_token=singleton_at, refresh_token="rt-singleton")
    monkeypatch.setenv("ZED_HOME", str(zed_home))
    ...
    # Same object â€” no sync happened.
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`zed auth add xai-oauth` entries (source='manual:xai_pkce') are
    ...
    # Refresh the manual entry â€” singleton must be left alone.
    ...
    raw = json.loads((zed_home / "auth.json").read_text())
    tokens = raw["providers"]["xai-oauth"]["tokens"]
    # Singleton must be untouched â€” manual refresh shouldn't leak across.
...
def test_auxiliary_client_routes_xai_oauth_through_responses_api(tmp_path, monkeypatch):
    ...
    arm and returns ``(None, None)`` â€” silently re-routing every auxiliary
...
def test_auxiliary_client_xai_oauth_returns_none_when_unauthenticated(tmp_path, monkeypatch):
    """No xAI OAuth tokens in the auth store â†’ ``resolve_provider_client``
...
def test_auxiliary_client_xai_oauth_requires_explicit_model(tmp_path, monkeypatch):
    """xAI's Responses API has no safe "cheap aux model" default â€”
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    provider (visible to ``zed auth status``, ``zed setup``, and the
    ``zed`` no-arg dispatcher).  Pin the ``set_active=False`` contract so
    no future refactor regresses to the legacy semantic."""
    from agent.credential_pool import load_pool

    zed_home = tmp_path / "zed"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_zed_auth(zed_home, access_token=near_expiry, refresh_token="rt-xai")
    monkeypatch.setenv("ZED_HOME", str(zed_home))

    # Simulate a multi-provider user whose actual chosen provider is
    # OpenRouter â€” xai-oauth tokens exist in the singleton but are NOT
    # the active provider.
    raw = json.loads((zed_home / "auth.json").read_text())
    ...
    # The refresh wrote new tokens back into the singleton â€” the user's
