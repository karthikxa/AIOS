"""Tests for zed_cli.tools_config platform tool persistence."""
...
def test_gui_toolset_label_strips_leading_emoji():
    assert gui_toolset_label("ðŸ” Web Search & Scraping") == "Web Search & Scraping"
    assert gui_toolset_label("ðŸ‘ï¸  Vision / Image Analysis") == "Vision / Image Analysis"
    assert gui_toolset_label("ðŸ”Œ My Plugin") == "My Plugin"
...
def test_get_platform_tools_homeassistant_toolset_enabled_for_cron_when_hass_token_set(monkeypatch):
    ...
    When HASS_TOKEN is set, the user has explicitly opted in â€” _DEFAULT_OFF_TOOLSETS
    ...
    # moa must stay off â€” the original goal of #14798
...
def test_get_platform_tools_homeassistant_toolset_off_for_cron_when_hass_token_missing(monkeypatch):
    """Without HASS_TOKEN, HA stays off by default â€” preserves #14798's behavior
...
def test_get_platform_tools_x_search_auto_enabled_when_xai_oauth_present(monkeypatch):
    ...
    tokens are present, mirroring the HASS_TOKEN â†’ homeassistant rule.

    The user already authenticated via SuperGrok OAuth; they shouldn't have
    to also click through `zed tools` â†’ X (Twitter) Search to flip the
    toolset on. Tool's check_fn still gates schema registration if creds
    later go missing.
    """
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "zed_cli.tools_config._xai_credentials_present", lambda: True
    )
...
def test_get_platform_tools_x_search_auto_enabled_when_xai_api_key_present(monkeypatch):
    ...
    OAuth tokens â€” the API-key path is a supported credential source."""
...
def test_get_platform_tools_x_search_off_when_no_xai_credentials(monkeypatch):
    """Without any xAI credentials, x_search stays off â€” preserves the
    "don't ship the schema to users who can't use it" default."""
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "zed_cli.tools_config._xai_credentials_present", lambda: False
    )
...
def test_get_platform_tools_x_search_respects_explicit_config(monkeypatch):
    """Once the user has saved an explicit toolset list via `zed tools`,
    that list is authoritative â€” x_search auto-enable does NOT fire even
    ...
    # User explicitly opted into spotify but not x_search via `zed tools`.
    config = {"platform_toolsets": {"cli": ["zed-cli", "spotify"]}}
    enabled = _get_platform_tools(config, "cli")
...
def test_get_platform_tools_expands_composite_when_mixed_with_configurable():
    """``[zed-cli, spotify]`` (composite + configurable) must keep the full
    ``zed-cli`` toolset alongside the explicit Spotify opt-in. The
    has_explicit_config branch used to drop ``zed-cli`` on the floor,
    ...
    for ts in ("terminal", "file", "web", "browser", "memory", "delegation",
        ...
        assert ts in enabled, f"{ts} should be enabled when zed-cli is listed"
    # User explicitly opted into Spotify â€” must survive _DEFAULT_OFF_TOOLSETS subtraction.
    assert "spotify" in enabled


def test_get_platform_tools_composite_only_unchanged():
    """Composite-only config (no configurable in list) must still take the
    else-branch path and produce the full toolset â€” guards against the new
    code accidentally hijacking the composite-only case."""
    composite_only = _get_platform_tools(
        {"platform_toolsets": {"cli": ["zed-cli"]}},
        "cli",
...
def test_get_platform_tools_configurable_only_no_expansion():
    ...
    toolsets â€” guards against the expansion firing when ``composite_tools``
    is empty."""
...
def test_get_platform_tools_mixed_does_not_resurrect_default_off():
    ...
    pull-in. Without this, ``zed-cli`` expansion would re-enable
    ``moa`` / ``rl`` / ``homeassistant`` for users who never opted in."""
    config = {"platform_toolsets": {"cli": ["zed-cli", "terminal"]}}
...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # terminal, memory, â€¦). Non-configurable platform toolsets that ride
    # along on the platform's default composite (e.g. `kanban`, whose tools
    # live in _ZED_CORE_TOOLS but aren't user-toggleable) are still
    # auto-recovered by _get_platform_tools so saving via `zed tools`
...
def test_apply_toolset_change_from_default_does_not_enable_default_off_toolsets():
    ...
    with patch("zed_cli.tools_config.save_config"):
...
def test_get_platform_tools_handles_null_platform_toolsets():
    """YAML `platform_toolsets:` with no value parses as None â€” the old
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/NousResearch/zed-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (zed-cli, zed-telegram, etc.) must NOT
    ...
    ``zed tools``, but zed-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "zed-cli",  # <-- the culprit
    ...
    # zed-cli must NOT survive â€” it's a platform default, not an MCP server
    assert "zed-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_zed_telegram():
    """Same bug for Telegram â€” zed-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "zed-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "zed-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "zed-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_visible_providers_include_nous_subscription_when_logged_in(monkeypatch):
    ...
    monkeypatch.setattr(
        "zed_cli.nous_subscription.get_nous_portal_account_info",
    ...
    # The managed Nous row is listed (not necessarily first â€” "Local Browser"
...
def test_visible_providers_show_nous_subscription_when_paid_access_is_false(monkeypatch):
    ...
    The paid-access gate moved from visibility to selection time â€” the row is
...
def test_visible_providers_force_fresh_shows_nous_subscription_after_upgrade(monkeypatch):
    ...
    monkeypatch.setattr(
        "zed_cli.tools_config.get_nous_subscription_features",
    ...
    # longer asserted to be first â€” "Local Browser" sorts first by design.
    assert any(p["name"].startswith("Nous Subscription") for p in providers)
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("zed_cli.tools_config._run_post_setup", lambda key: None)
...
def test_fresh_install_browser_default_is_free_local_not_paid_nous():
    ...
    """
    from zed_cli.tools_config import _detect_active_provider_index

    providers = TOOL_CATEGORIES["browser"]["providers"]
    assert providers[0]["name"] == "Local Browser"
    assert providers[0]["browser_provider"] == "local"
    # Nothing active/configured â†’ cursor defaults to index 0 (the free local row).
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr("zed_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "zed_cli.tools_config._configure_tool_category_for_reconfig",
        lambda ts_key, cat, config, **kwargs: configured.append(ts_key),
    )
    monkeypatch.setattr("zed_cli.tools_config.save_config", lambda config: None)
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    monkeypatch.setattr("zed_cli.nous_subscription.managed_nous_tools_enabled", lambda: True)
    config = {
        "model": {"provider": "nous"},
        "platform_toolsets": {"cli": []},
    }
    ...
    monkeypatch.setattr(
        "zed_cli.tools_config._prompt_toolset_checklist",
    ...
    monkeypatch.setattr(
        "zed_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
        "zed_cli.nous_subscription.get_nous_portal_account_info",
    ...
    configured = []
    monkeypatch.setattr(
        "zed_cli.tools_config._configure_toolset",
...
def test_first_install_nous_auto_configures_video_gen(monkeypatch):
    ...
    # video_gen should NOT appear in the manual configure list â€” it's auto-configured
    assert "video_gen" not in configured

# â”€â”€ Platform / toolset consistency â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from zed_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """zed-gateway includes list should cover all messaging platforms."""
        from zed_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["zed-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from zed_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from zed_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/NousResearch/zed-agent/issues/6901
    """
    config = {
...
# â”€â”€â”€ Imagegen Backend Picker Wiring â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_computer_use_needs_configuration_when_cua_driver_post_setup_pending():
    ...
    Returning users enabling Computer Use through `zed tools` must reach the
...
def test_computer_use_respects_custom_cua_driver_command():
    """The setup gate should match runtime's ZED_CUA_DRIVER_CMD override."""
    def fake_which(name: str):
        return "/opt/bin/custom-cua" if name == "custom-cua" else None

    with patch.dict("os.environ", {"ZED_CUA_DRIVER_CMD": "custom-cua"}), \
...
def test_computer_use_blank_custom_driver_command_falls_back_to_default():
    ...
    with patch.dict("os.environ", {"ZED_CUA_DRIVER_CMD": "   "}), \
         patch("shutil.which", side_effect=fake_which):
        assert _toolset_needs_configuration_prompt("computer_use", {}) is False
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from zed_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        ...
        from zed_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from zed_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("zed_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        """GPT-Image quality is pinned to medium in the tool's defaults â€”
        ...
        # Only ONE picker call is expected (for model) â€” not two (model + quality).
        call_count = {"n": 0}
        ...
        config = {}
        with patch("zed_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("zed_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_save_platform_tools_clears_no_mcp_sentinel():
    """`zed tools` has no UI for no_mcp, so saving from the picker clears
    the sentinel unconditionally â€” otherwise a user who once set no_mcp by
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    the save â€” we only clear `no_mcp`, not every non-configurable entry.
    """
    config = {
        "platform_toolsets": {
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    fake_toolsets["zed-_test_platform"] = {
    ...
    test_platforms = {
        "_test_platform": {"label": "Test", "default_toolset": "zed-_test_platform"},
    }

    with mock_patch("zed_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}):
...
def test_get_platform_tools_second_pass_skips_fully_claimed_toolsets():
    ...
    be added by the second pass (prevents 'search', 'zed-acp' noise).
    """
...
def test_get_platform_tools_discord_both_off_by_default():
    """Both `discord` and `discord_admin` are opt-in via `zed tools`,
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    Telegram, etc. â€” not even as an opt-in."""
    from zed_cli.tools_config import _toolset_allowed_for_platform
...
def test_discord_toolsets_user_enabled_are_honored():
    """When the user opts in via `zed tools`, the toolset appears."""
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from zed_cli.tools_config import _save_platform_tools
    config = {}
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice â€” otherwise `zed tools` â†’ "reconfigure existing" shows
    the same toolset two rows in a row.
    """
    from zed_cli.tools_config import _get_effective_configurable_toolsets
    ...
    # Spotify specifically â€” the bug that motivated the dedupe.
    ...
    assert spotify_rows[0][1] == "ðŸŽµ Spotify"


@pytest.mark.parametrize("provider,config_key,expected", [
    # managed provider â†’ use_gateway True
    ...
    # self-hosted provider â†’ use_gateway False
...
def test_reconfigure_provider_syncs_use_gateway(monkeypatch, provider, config_key, expected):
    ...
    monkeypatch.setattr(
        "zed_cli.nous_subscription.ensure_nous_portal_access",
        lambda **kwargs: True,
    )
    config = {}
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    both env_vars and post_setup â€” parity with _configure_provider() line 2286."""
    called = []
    monkeypatch.setattr("zed_cli.tools_config._run_post_setup", lambda key: called.append(key))
    monkeypatch.setattr("zed_cli.tools_config.get_env_value", lambda k: None)
    monkeypatch.setattr("zed_cli.tools_config._prompt", lambda *a, **kw: "")
    monkeypatch.setattr("zed_cli.tools_config.save_env_value", lambda k, v: None)
...
def test_configure_managed_provider_blocks_when_not_entitled(monkeypatch):
    ...
    # No use_gateway / backend written â€” the gate returned before any mutation.
...
def test_configure_non_managed_provider_skips_portal_gate(monkeypatch):
    ...
    monkeypatch.setattr(
        "zed_cli.nous_subscription.ensure_nous_portal_access", _boom
    )
...
def test_apply_provider_selection_web_sets_backend():
    ...
    from zed_cli.tools_config import apply_provider_selection
...
# â”€â”€ Checklist diff scope: non-configurable toolsets (kanban) must not be
#    reported as added/removed by `zed tools` â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `zed tools` printed ``- kanban``
...
def test_real_configurable_changes_still_reported_in_diff():
    ...
    # User unticks 'terminal' (configurable) â€” must still report as removed.
    new_enabled = {t for t in current if t not in ("kanban", "terminal")}
    assert ((current - new_enabled) & universe) == {"terminal"}

    # User adds 'vision' (configurable) â€” must still report as added.
