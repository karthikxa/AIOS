...
The notice prints the most recent curator run summary on `zed update`,
...
session start) so users normally never see the rename map. `zed update`
...
def test_silent_when_no_curator_run_yet(curator_env):
    ...
    assert "Skill curator â€” last run" not in out
...
def test_prints_multiline_summary_with_rename_map(curator_env):
    ...
    curator_env["main"]._print_curator_recent_run_notice()
    out = curator_env["capsys"].readouterr().out
    assert "Skill curator â€” last run" in out
    assert "pdf-extraction â†’ document-tools" in out
    assert "docx-extraction â†’ document-tools" in out
...
def test_new_run_resets_show_once(curator_env):
    ...
    _set_state(
        ...
        last_run_summary=(
            "auto: no changes; llm: consolidated 1 into 1\n"
            "archived 1 skill(s):\n"
            "  â€¢ thing-b â†’ umbrella\n"
            "full report: zed curator status"
        ),
    )
    curator_env["main"]._print_curator_recent_run_notice()
    out = curator_env["capsys"].readouterr().out
    assert "thing-b â†’ umbrella" in out
    assert "thing-a" not in out  # only the newer run shows


def test_format_time_ago_buckets(curator_env):
    """Smoke test the time formatter â€” drives the `last run Xh ago` line."""
