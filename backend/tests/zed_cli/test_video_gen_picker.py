...
through Reconfigure tool â†’ Video Generation. The first-time configure
...
class TestReconfigureWritesProvider:
    ...
    tool â†’ Video Generation silently no-op'd: the env var was already
    ...
    def test_reconfigure_with_env_vars_already_set_writes_provider(
        ...
        """Env vars present and user accepts current value â†’ still writes
        ...
        # Picker prompts replaced â€” no TTY in tests.
    ...
    def test_reconfigure_with_no_env_vars_writes_provider(
        ...
        """No env vars at all (managed-style plugin) â†’ writes
...
class TestVideoPluginProviderActive:
    ...
    def test_detect_active_index_picks_video_plugin_match(self, monkeypatch):
        ...
        env-var fallback â€” so the picker visually defaulted to FAL even
