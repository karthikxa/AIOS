...
``save_env_value()`` â€” so a dashboard switched to a freshly created profile
...
profile's ZED_HOME, and the dashboard's own profile stays untouched.
"""
import pytest
import yaml
...
class TestProfileScopedMessagingReads:
    def test_scoped_read_does_not_show_root_credentials(
        ...
        # The worker profile has an empty .env â€” the root token must not leak.
    ...
    def test_scoped_read_returns_profile_path_command_and_startup_failure(
        ...
        assert payload["gateway_start_command"] == (
            "zed -p worker_alpha gateway start"
        )
...
class TestProfileScopedMessagingWrites:
    def test_scoped_write_lands_in_target_profile_env(
        ...
        # The dashboard's own .env must stay untouched â€” this was the bug.
