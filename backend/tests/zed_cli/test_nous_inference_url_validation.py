...
4. The env-var override path (``NOUS_INFERENCE_BASE_URL``) is NOT
   gated by the validator â€” that's the documented dev/staging escape
   hatch.
"""
...
class TestValidatorRules:
    def test_allowlisted_https_host_returned(self):
        url = "https://inference-api.zedteam.com/v1"
    ...
    def test_attacker_host_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="zed_cli.auth"):
            assert (
    ...
    def test_subdomain_of_allowlist_host_rejected(self):
        """*.zedteam.com is NOT in the allowlist â€” exact hostname only.

        A subdomain takeover or DNS hijack of *.zedteam.com would
        otherwise pass â€” keep the gate tight.
        """
        assert (
            _validate_nous_inference_url_from_network(
                "https://evil.inference-api.zedteam.com/v1"
            )
            is None
        )
    ...
    def test_http_scheme_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="zed_cli.auth"):
            assert (
                _validate_nous_inference_url_from_network(
                    "http://inference-api.zedteam.com/v1"
                )
                is None
            )
    ...
    def test_default_inference_url_is_in_allowlist(self):
        ...
        ``inference-api.zedteam.com``, they MUST update the allowlist
        in the same change â€” otherwise the allowlist would reject the
...
class TestCallSiteWiring:
    ...
    â€” that's what the unsafe pre-fix code looked like, and the only
    semantic difference between the safe and unsafe helpers is the
    host-allowlist check.
    """

    def _read_auth_source(self):
        import zed_cli.auth as _auth_mod
    ...
    def test_proxy_adapter_also_validates(self):
        ...
        boundary."""
        from pathlib import Path
        import zed_cli.proxy.adapters.nous_portal as _nous_adapter
...
class TestEnvOverrideNotGated:
    ...
    via ``os.getenv`` â€” that path doesn't pass through the validator
        ...
        for line in source.splitlines():
            if "NOUS_INFERENCE_BASE_URL" in line and "os.getenv" in line:
                assert "_validate_nous_inference_url_from_network" not in line, (
                    ...
                    "validator â€” it would break documented dev/staging use."
                )
