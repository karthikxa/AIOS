...
The terminal `zed model` flow already exposes `custom_providers`, but the
shared slash-command pipeline (`/model` in CLI/gateway/Telegram) historically
only looked at `providers:`.
"""

import zed_cli.providers as providers_mod
...
def test_list_authenticated_providers_includes_custom_providers(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "ZED_OVERLAYS", {})
...
def test_switch_model_accepts_explicit_bare_custom_current_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("zed_cli.models.validate_requested_model", lambda *a, **k: _MOCK_VALIDATION)
    monkeypatch.setattr("zed_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr("zed_cli.model_switch.get_model_capabilities", lambda *a, **k: None)
...
def test_list_enumerates_dict_format_models_alongside_default(monkeypatch):
    ...
    Regression: Zed's own writer stores configured models as a dict
...
def test_list_authenticated_providers_groups_same_endpoint(monkeypatch):
    ...
    providers = list_authenticated_providers(
        ...
        user_providers={},
        custom_providers=[
            {"name": "Ollama â€” MiniMax M2.7", "base_url": "http://localhost:11434/v1",
             "api_key": "ollama", "model": "minimax-m2.7"},
            {"name": "Ollama â€” GLM 5.1",      "base_url": "http://localhost:11434/v1",
             "api_key": "ollama", "model": "glm-5.1"},
            {"name": "Ollama â€” Qwen3-coder", "base_url": "http://localhost:11434/v1",
...
def test_list_authenticated_providers_current_endpoint_uses_current_slug(monkeypatch):
    ...
    credential pipeline â€” provided current_provider is a real slug, not
...
def test_list_authenticated_providers_same_url_different_keys_disambiguated(monkeypatch):
    ...
    picker â€” slug is suffixed to disambiguate."""
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr(providers_mod, "ZED_OVERLAYS", {})
    ...
    providers = list_authenticated_providers(
        user_providers={},
        custom_providers=[
            {"name": "OpenAI â€” key A", "base_url": "https://api.openai.com/v1",
             "api_key": "sk-AAA", "model": "gpt-5.4"},
            {"name": "OpenAI â€” key B", "base_url": "https://api.openai.com/v1",
...
def test_lmstudio_picker_probes_active_config_base_url(monkeypatch):
    ...
    LM_BASE_URL env var, the picker must probe the saved base_url â€” not
    ...
    monkeypatch.setattr("zed_cli.models.fetch_lmstudio_models", _fake_fetch)
