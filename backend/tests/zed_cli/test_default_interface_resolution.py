...
`zed` launches the classic prompt_toolkit REPL by default, but users can
flip ``display.interface: tui`` in config.yaml to make the modern Ink TUI the
default for bare ``zed`` / ``zed chat``. Explicit flags always win:

    --cli                forces the classic REPL (highest precedence)
    --tui / ZED_TUI=1 forces the TUI
    display.interface    the configured default
    (unset)              classic REPL

These tests pin that precedence at every layer that makes the decision:

  * ``_resolve_use_tui(args)``  â€” the canonical args-aware resolver used by
    ``cmd_chat`` and the Termux fast-TUI path.
  * ``_wants_tui_early(argv)``  â€” the dependency-free early resolver used by
    mouse-residue suppression and the Termux fast paths, before argparse and
    ``zed_cli.config`` are importable.
  * the argument parser   â€” both ``--cli`` and ``--tui`` parse at the top
...
def _reset_early_cache(monkeypatch):
    ...
    # a fresh value, and make sure no stray ZED_TUI leaks in.
...
# ---------------------------------------------------------------------------
# _resolve_use_tui â€” args-aware resolver
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# _wants_tui_early â€” dependency-free early resolver
# ---------------------------------------------------------------------------
class TestWantsTuiEarly:
    ...
    def test_missing_config_defaults_to_cli(self, tmp_path, monkeypatch):
        # ZED_HOME points at an empty dir â€” no config.yaml.
        monkeypatch.setenv("ZED_HOME", str(tmp_path))
        monkeypatch.setattr(m, "_EARLY_INTERFACE_CACHE", None)
        assert m._wants_tui_early([]) is False
...
# ---------------------------------------------------------------------------
# argument parser â€” flags exist at both levels and are relaunch-inherited
# ---------------------------------------------------------------------------
class TestParserFlags:
    ...
    def test_cli_and_tui_are_relaunch_inherited(self):
        from zed_cli.relaunch import _INHERITED_FLAGS_TABLE
...
# ---------------------------------------------------------------------------
# config default â€” shipped default preserves classic behavior
# ---------------------------------------------------------------------------
