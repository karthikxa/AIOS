"""Coverage for _run_with_idle_timeout â€” the streaming subprocess helper.
...
Added for issue #33788: ``zed update`` got stuck at "webui-build" because
``npm run build`` ran with ``capture_output=True`` and no timeout. The helper
fixes both halves â€” streams output AND idle-kills the process.
"""
...
def test_kills_process_on_idle_timeout(tmp_path):
    # Sleeps without printing â€” exactly the failure mode users see when
