...
* Be macOS-only â€” no-op silently on Linux/Windows so ``zed update`` can
...
class TestCheckCuaDriverAssetForArch:
    ...
    def test_x86_64_api_failure_returns_true(self):
        """Network failure should fail open â€” let the installer handle it."""
        from hermes_cli import tools_config
    ...
    def test_upgrade_x86_64_no_asset_returns_existing_status(self):
        ...
        # With binary installed â€” returns True (binary exists)
        with patch("platform.system", return_value="Darwin"), \
             patch.object(tools_config.shutil, "which",
                          side_effect=lambda n: "/usr/local/bin/" + n
                                                 if n in ("cua-driver", "curl") else None), \
        ...
        # Without binary â€” returns False
        with patch("platform.system", return_value="Darwin"), \
             patch.object(tools_config.shutil, "which",
                          side_effect=lambda n: "/usr/bin/curl" if n == "curl" else None), \
             patch("platform.machine", return_value="x86_64"), \
