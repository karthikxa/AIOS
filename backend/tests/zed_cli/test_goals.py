"""Tests for zed_cli/goals.py â€” persistent cross-turn goals."""
...
def zed_home(tmp_path, monkeypatch):
    """Isolated ZED_HOME so SessionDB.state_meta writes don't clobber the real one."""
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves ZED_HOME.
...
class TestParseJudgeResponse:
    def test_clean_json_done(self):
        from zed_cli.goals import _parse_judge_response
    ...
    def test_json_embedded_in_prose(self):
        """Some models prefix reasoning before emitting JSON â€” we extract it."""
        from zed_cli.goals import _parse_judge_response
    ...
    def test_malformed_json_fails_open(self):
        """Non-JSON â†’ not done, with error-ish reason (so judge_goal can map to continue)."""
        from zed_cli.goals import _parse_judge_response
...
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# judge_goal â€” fail-open semantics
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


class TestJudgeGoal:
    def test_empty_goal_skipped(self):
        from zed_cli.goals import judge_goal
    ...
    def test_api_error_continues(self):
        """Judge exception â†’ fail-open continue (don't wedge progress on judge bugs)."""
        from zed_cli import goals
...
class TestGoalManager:
    def test_no_goal_initial(self, zed_home):
    ...
    def test_set_then_status(self, zed_home):
        from zed_cli.goals import GoalManager
    ...
    def test_set_rejects_empty(self, zed_home):
        from zed_cli.goals import GoalManager
    ...
    def test_pause_and_resume(self, zed_home):
        from zed_cli.goals import GoalManager
    ...
    def test_persistence_across_managers(self, zed_home):
        """Key invariant: a second manager on the same session sees the goal.

        This is what makes /resume work â€” each session rebinds its
    ...
    def test_evaluate_after_turn_done(self, zed_home):
        """Judge says done â†’ status=done, no continuation."""
        from zed_cli import goals
        from zed_cli.goals import GoalManager
    ...
    def test_evaluate_after_turn_continue_under_budget(self, zed_home):
        from zed_cli import goals
        from zed_cli.goals import GoalManager
    ...
    def test_evaluate_after_turn_budget_exhausted(self, zed_home):
        ...
        with patch.object(goals, "judge_goal", return_value=("continue", "not yet", False)):
            ...
            # turns_used is now 2 which equals max_turns â†’ paused
            assert d2["should_continue"] is False
            assert mgr.state.status == "paused"
            assert mgr.state.turns_used == 2
            assert "budget" in (mgr.state.paused_reason or "").lower()

    def test_evaluate_after_turn_inactive(self, zed_home):
    ...
    def test_continuation_prompt_shape(self, zed_home):
        """The continuation prompt must include the goal text verbatim â€”
        ...
        mgr.set("port goal command to zed")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to zed" in prompt
...
def test_goal_command_dispatches_in_cli_registry_helpers():
    ...
    from zed_cli.commands import COMMANDS, COMMANDS_BY_CATEGORY
...
class TestJudgeParseFailureAutoPause:
    ...
    def test_empty_judge_reply_flagged_as_parse_failure(self):
        """End-to-end: judge returns empty content â†’ parse_failed=True."""
        from zed_cli import goals
    ...
    def test_auto_pause_after_three_consecutive_parse_failures(self, zed_home):
        """N=3 consecutive parse failures â†’ auto-pause with config pointer."""
        from zed_cli import goals
        from zed_cli.goals import GoalManager, DEFAULT_MAX_CONSECUTIVE_PARSE_FAILURES
    ...
    def test_parse_failure_counter_resets_on_good_reply(self, zed_home):
        """A single good judge reply resets the counter â€” transient flakes don't pause."""
        from zed_cli import goals
        from zed_cli.goals import GoalManager
        ...
        # â€¦then one clean reply resets the counter.
        with patch.object(
    ...
    def test_parse_failure_counter_not_incremented_by_api_errors(self, zed_home):
    ...
    def test_consecutive_parse_failures_persists_across_goalmanager_reloads(
        ...
        from zed_cli import goals
        from zed_cli.goals import GoalManager, load_goal
...
class TestGoalStateSubgoalsBackcompat:
    def test_old_state_meta_row_loads_without_subgoals(self):
        ...
        from zed_cli.goals import GoalState
...
class TestGoalManagerSubgoals:
    def test_add_subgoal(self, zed_home):
        from zed_cli.goals import GoalManager
    ...
    def test_add_subgoal_requires_active_goal(self, zed_home):
        import pytest
        from zed_cli.goals import GoalManager
    ...
    def test_add_empty_subgoal_rejected(self, zed_home):
        import pytest
        from zed_cli.goals import GoalManager
    ...
    def test_remove_subgoal(self, zed_home):
        from zed_cli.goals import GoalManager
    ...
    def test_remove_subgoal_out_of_range(self, zed_home):
        import pytest
        from zed_cli.goals import GoalManager
    ...
    def test_clear_subgoals(self, zed_home):
        from zed_cli.goals import GoalManager
    ...
    def test_subgoals_persist_across_reloads(self, zed_home):
...
class TestContinuationPromptWithSubgoals:
    def test_empty_subgoals_uses_original_template(self, zed_home):
        from zed_cli.goals import GoalManager
    ...
    def test_with_subgoals_includes_them(self, zed_home):
        from zed_cli.goals import GoalManager
...
class TestJudgeGoalWithSubgoals:
    def test_judge_uses_subgoals_template_when_provided(self, zed_home):
        """judge_goal switches templates when subgoals is non-empty.

        We don't actually call the model â€” we patch the aux client to
    ...
    def test_judge_uses_original_template_when_no_subgoals(self, zed_home):
        from unittest.mock import patch
        from zed_cli import goals
...
class TestStatusLineSubgoalCount:
    def test_status_line_no_subgoals(self, zed_home):
        from zed_cli.goals import GoalManager
    ...
    def test_status_line_with_subgoals(self, zed_home):
        from zed_cli.goals import GoalManager
