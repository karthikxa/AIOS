...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("hermes_cli.config.get_managed_system", return_value=None), \
         patch("hermes_cli.config.get_zed_home", return_value=tmp_path):
...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    ...
    with patch("hermes_cli.config.get_managed_system", return_value=None), \
         patch("hermes_cli.config.get_zed_home", return_value=home):
        from hermes_cli.config import detect_install_method
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    ...
    code.mkdir()
    home.mkdir()
    with patch("hermes_cli.config.get_zed_home", return_value=home):
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    with patch("hermes_cli.config.get_zed_home", return_value=hh), \
         patch("hermes_constants.get_zed_home", return_value=hh):
        buf = io.StringIO()
