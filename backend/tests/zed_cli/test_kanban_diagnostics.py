...
# ---------------------------------------------------------------------------
# Each rule â€” positive + negative + clearing
# ---------------------------------------------------------------------------
...
def test_prose_phantom_refs_fires_after_clean_completion():
    ...
    # must be valid hex â€” the scanner regex is ``t_[a-f0-9]{8,}``.
    task = _task(status="done")
    events = [
...
def test_repeated_failures_fires_on_timeout_loop():
    """The rule surfaces for timeout loops too â€” that's the point of
...
def test_repeated_crashes_counts_trailing_streak_only():
    ...
    # 2 consecutive crashes at the end â†’ default threshold 2 â†’ error severity.
    assert d.severity == "error"
...
def test_repeated_crashes_truncates_huge_tracebacks():
    ...
    line (â‰¤160 chars); the detail caps at 500 chars + ellipsis so the
    ...
    assert d.detail.endswith("â€¦") or len(d.detail) < 700
...
# ---------------------------------------------------------------------------
# Integration â€” runs through real kanban_db so sqlite.Row fields work
# ---------------------------------------------------------------------------
...
def test_stranded_in_ready_skips_claimed_tasks():
    """A live claim_lock means a worker is on it â€” even an old one. Don't
...
def test_stranded_in_ready_uses_latest_ready_transition():
    ...
    age-from the most recent â€” a task reclaimed 20 min ago is NOT
    ...
    events = [
        ...
        _event("reclaimed", ts=now - 20 * 60),      # 20 min ago â€” wins
    ]
    diags = kd.compute_task_diagnostics(task, events, [], now=now)
    assert [d for d in diags if d.kind == "stranded_in_ready"] == []
...
def test_stranded_in_ready_severity_escalates_with_age():
    """warning â†’ error â†’ critical at 2x and 6x threshold."""
    now = 100_000
    task = _task(status="ready", assignee="demo")
    # Default threshold = 1800s.
    cases = [
        (45 * 60, "warning"),    # 1.5x â†’ warning
        (90 * 60, "error"),      # 3x â†’ error
        (4 * 3600, "critical"),  # 8x â†’ critical
    ]
...
def test_stranded_in_ready_respects_config_override():
    ...
    # Default 30 min â€” wouldn't fire.
    diags = kd.compute_task_diagnostics(task, events, [], now=now)
    assert [d for d in diags if d.kind == "stranded_in_ready"] == []
    # Lower the threshold to 5 min â€” now it fires.
    diags = kd.compute_task_diagnostics(
        task, events, [], now=now,
...
def test_stranded_in_ready_works_on_real_db_row(kanban_home):
    """Round-trip through real kanban_db.connect() â€” confirms the rule
...
# ---------------------------------------------------------------------------
# triage_aux_unavailable rule â€” auto-decompose aware
# ---------------------------------------------------------------------------
...
def test_triage_aux_unavailable_silent_when_main_model_visible():
    """Default `provider: auto` falls back to the main model â€” no warning."""
    config = {
        "auxiliary": {},
...
def test_triage_aux_unavailable_silent_when_decomposer_explicit():
    """User explicitly configured decomposer â†’ no warning, even without main."""
    config = {
        "auxiliary": {
...
def test_triage_aux_unavailable_fires_auto_decompose_on_no_fallback():
    """auto_decompose=True, no decomposer, no main model â†’ warn about decomposer."""
    config = {
        "auxiliary": {},
        "kanban": {"auto_decompose": True},
    }
...
def test_triage_aux_unavailable_fires_auto_decompose_off_points_at_specifier():
    """auto_decompose=False â†’ primary is specifier, not decomposer."""
    config = {
        "auxiliary": {},
    ...
    assert any("zed kanban specify" in l for l in labels)
...
def test_triage_aux_status_recognises_auto_default_as_not_explicit():
    """Default `provider: auto` with empty fields â†’ not 'explicit'."""
