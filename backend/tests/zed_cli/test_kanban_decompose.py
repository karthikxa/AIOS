"""Tests for the decomposer module + `zed kanban decompose` CLI surface.

The auxiliary LLM client is mocked â€” no network calls. Tests exercise the
...
def _patch_list_profiles(names: list[str]):
    ...
    return [
        patch("zed_cli.profiles.list_profiles", return_value=fake_profiles),
        patch("zed_cli.profiles.profile_exists", side_effect=lambda x: x in names),
        patch("zed_cli.profiles.get_active_profile_name", return_value=names[0] if names else "default"),
    ]
...
def test_decompose_fanout_false_assigns_default_when_unassigned(kanban_home):
    ...
    try:
        with _patch_aux_client(llm_payload), _patch_extra_body(), patch(
            "zed_cli.kanban_decompose._load_config",
