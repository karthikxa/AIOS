"""Tests for the ``zed hooks`` CLI subcommand."""
...
from zed_cli import hooks as hooks_cli
...
class TestHooksList:
    ...
    def test_shows_configured_and_consent_status(self, tmp_path):
        ...
        assert "âœ“ allowed" in out
        assert "âœ— not allowlisted" in out
...
class TestHooksTest:
    def test_synthetic_payload_matches_production_shape(self, tmp_path):
        """`zed hooks test` must feed the script stdin in the same
        shape invoke_hook() would at runtime.  Prior to this fix,
        run_once bypassed _serialize_payload and the two paths diverged â€”
        scripts tested with `zed hooks test` saw different top-level
...
class TestHooksDoctor:
    def test_flags_missing_exec_bit(self, tmp_path):
        ...
        # No chmod â€” intentionally not executable
    ...
    def test_unallowlisted_script_is_not_executed(self, tmp_path):
        """Regression for M4: `zed hooks doctor` used to run every
        ...
        assert not sentinel.exists(), (
            "doctor executed an un-allowlisted script â€” "
            "M4 gate regressed"
        )
        assert "not allowlisted" in out.lower()
