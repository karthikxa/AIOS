"""Tests for the Command Installation check in zed doctor."""
...
import zed_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal ZED_HOME + PROJECT_ROOT for doctor tests."""
    home = tmp_path / ".zed"
    ...
    zed_bin = venv_bin_dir / "zed"
    zed_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    zed_bin.chmod(0o755)
...
class TestDoctorCommandInstallation:
    ...
    def test_missing_symlink_shows_fail(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Don't create the symlink â€” it should be missing

        out = _run_doctor(fix=False)
        assert "Command Installation" in out
        assert "Venv entry point exists" in out
        assert "not found" in out
        assert "zed doctor --fix" in out
    ...
    def test_fix_creates_missing_symlink(self, monkeypatch, tmp_path):
        ...
        cmd_link = tmp_path / ".local" / "bin" / "zed"
        assert cmd_link.is_symlink()
        assert cmd_link.resolve() == zed_bin.resolve()
    ...
    def test_wrong_target_symlink_shows_warn(self, monkeypatch, tmp_path):
        ...
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "zed"
        wrong_target = tmp_path / "wrong_zed"
    ...
    def test_dot_venv_dir_is_found(self, monkeypatch, tmp_path):
        ...
        out = _run_doctor(fix=False)
        assert "Venv entry point exists" in out
        assert ".venv/bin/zed" in out

    @pytest.mark.skipif(sys.platform == "win32", reason="Symlink check is Unix-only")
    def test_non_symlink_regular_file_shows_ok(self, monkeypatch, tmp_path):
        """If ~/.local/bin/zed is a regular file (not symlink), accept it."""
        home, project, zed_bin = _setup_doctor_env(monkeypatch, tmp_path)

        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "zed"
        cmd_link.write_text("#!/bin/sh\nexec python -m zed_cli.main \"$@\"\n")
