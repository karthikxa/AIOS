"""Tests for `zed memory setup [provider]` routing.
...
``zed memory setup honcho``) without the interactive picker â€” which matters
because the per-provider ``zed <provider>`` subcommand is only registered
...
class TestMemorySetupProviderRouting:
    ...
    def test_setup_with_missing_provider_attr_runs_picker(self):
        """A SimpleNamespace lacking `provider` must not crash â€” fall back to picker."""
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "zed memory setup" in out
