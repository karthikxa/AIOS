"""Regression tests for GitHub #36823 â€” the TUI must reset terminal input
...
sequence as its first step via ``_reset_terminal_input_modes_on_exit`` â€” gated
...
class TestResetTerminalInputModes(unittest.TestCase):
    ...
    def test_noop_when_tui_never_ran(self):
        """Non-TUI one-shot CLI runs share _run_cleanup via atexit â€” they must
    ...
    def test_noop_when_not_a_tty_and_no_dev_tty(self):
        """stdout redirected and /dev/tty unavailable â†’ nothing written, no raise."""
        cli_mod = _import_cli()
    ...
    def test_swallows_stdout_errors(self):
        ...
        with (
            ...
            # Cleanup runs at process teardown â€” it must never raise.
            cli_mod._reset_terminal_input_modes_on_exit()
    ...
    def test_flag_cleared_after_reset(self):
        """Once the modes are disabled they are no longer active â€” the flag must
...
class TestRunCleanupWiring(unittest.TestCase):
    ...
    â€” even if a later cleanup step raises."""
    ...
    def _run_cleanup_isolated(self, cli_mod, **extra_patches):
        ...
        try:
            with (
                ...
                patch("zed_cli.plugins.invoke_hook", lambda *a, **k: None),
            ):
    ...
    def test_reset_runs_even_when_a_cleanup_step_raises(self):
        ...
        it â€” covering the Ctrl+C / crash paths the issue is about."""
        cli_mod = _import_cli()
