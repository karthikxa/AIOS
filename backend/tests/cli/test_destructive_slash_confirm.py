"""Tests for cli.ZedCLI._confirm_destructive_slash.

Drives the helper directly via __get__ on a SimpleNamespace stand-in so we
don't have to construct a full ZedCLI (which requires extensive setup).
"""
...
def _make_self(prompt_response):
    ...
    self_._normalize_slash_confirm_choice = _bound(
        ZedCLI._normalize_slash_confirm_choice, self_,
    )
    return self_
...
def test_gate_off_returns_once_without_prompting():
    ...
    with patch(
        ...
        result = _bound(ZedCLI._confirm_destructive_slash, self_)(
            "clear", "detail",
        )
...
def test_gate_on_choice_cancel_returns_none():
    """When the user picks '3' (cancel), return None â€” caller must abort."""
    from cli import ZedCLI
...
def test_gate_on_unknown_choice_returns_none():
    """Garbage input is treated as cancel â€” fail safe, don't destroy state."""
    from cli import ZedCLI
...
def test_gate_on_choice_always_persists_and_returns_always():
    """User picks 'always' â†’ returns 'always' AND
...
def test_gate_default_true_when_config_missing():
    ...
    'gate on' (default safe) â€” must prompt."""
    from cli import ZedCLI
    ...
    # Got prompted (returned None from cancel) â€” meaning the gate was
...
def test_slash_confirm_modal_number_selection_submits_without_raw_input():
    ...
    _bound(ZedCLI._submit_slash_confirm_response, self_)("always")
...
def test_slash_confirm_display_fragments_include_choice_mapping():
    ...
    self_ = SimpleNamespace(
        _slash_confirm_state={
            "title": "âš ï¸  /new â€” destroys conversation state",
    ...
    fragments = _bound(ZedCLI._get_slash_confirm_display_fragments, self_)()
...
def test_split_destructive_skip_recognized_tokens():
    ...
    assert ZedCLI._split_destructive_skip("/reset now") == ("", True)
    assert ZedCLI._split_destructive_skip("/clear --yes") == ("", True)
    assert ZedCLI._split_destructive_skip("/undo -y") == ("", True)
...
def test_split_destructive_skip_strips_command_word():
    ...
    assert ZedCLI._split_destructive_skip("/new My title") == ("My title", False)
    assert ZedCLI._split_destructive_skip("/new --yes My title") == ("My title", True)
...
def test_split_destructive_skip_case_insensitive():
    ...
    assert ZedCLI._split_destructive_skip("/new NOW") == ("", True)
    # Substring match must NOT trigger â€” "Now-Title" is a literal title token.
    assert ZedCLI._split_destructive_skip("/new Now-Title") == ("Now-Title", False)
...
def test_split_destructive_skip_handles_empty_and_none():
    ...
    assert ZedCLI._split_destructive_skip(None) == ("", False)
    assert ZedCLI._split_destructive_skip("") == ("", False)
    assert ZedCLI._split_destructive_skip("   ") == ("", False)
...
def test_confirm_destructive_slash_now_skips_modal():
    ...
    # Build a prompt stub that fails the test if invoked â€” proving the modal
    # was never reached.
    def _explode(**_kw):
    ...
    self_._split_destructive_skip = ZedCLI._split_destructive_skip  # classmethod
...
def test_confirm_destructive_slash_yes_flag_skips_modal():
    ...
    self_._split_destructive_skip = ZedCLI._split_destructive_skip
...
def test_confirm_destructive_slash_no_skip_token_still_prompts():
    ...
    # Prompt was reached and returned cancel â†’ None.
    assert result is None
