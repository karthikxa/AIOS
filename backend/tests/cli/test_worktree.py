...
def _setup_worktree(repo_root):
    """Test version of _setup_worktree â€” creates a worktree."""
    import uuid
...
def _cleanup_worktree(info):
    ...
    if _has_unpushed_commits(wt_path, timeout=10):
        return False  # Did not clean up â€” has unpushed commits
...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("zed/zed-")
...
class TestWorktreeCleanup:
    ...
    def test_dirty_worktree_cleaned_when_no_unpushed(self, git_repo):
        ...
        # commit is seen as "pushed".  No unpushed commits â†’ cleanup proceeds.
        result = _cleanup_worktree(info)
    ...
    def test_worktree_with_unpushed_commits_kept(self, git_repo):
        ...
        result = _cleanup_worktree(info)
        assert result is False  # Kept â€” has unpushed commits
        assert Path(info["path"]).exists()
...
class TestWorktreeInclude:
    ...
    def test_ignores_comments_and_blanks(self, git_repo):
        ...
        info = _setup_worktree(str(git_repo))
        assert info is not None
        # Should not crash â€” just skip all lines
...
class TestMultipleWorktrees:
    ...
    def test_ten_concurrent_worktrees(self, git_repo):
        """Create 10 worktrees â€” simulating 10 parallel agents."""
        worktrees = []
        for _ in range(10):
            info = _setup_worktree(str(git_repo))
            assert info is not None
...
class TestStaleWorktreePruning:
    ...
    def test_keeps_recent_worktree(self, git_repo):
        ...
        # Don't modify mtime â€” it's recent
        worktrees_dir = git_repo / ".worktrees"
        cutoff = time.time() - (24 * 3600)
    ...
    def test_keeps_old_worktree_with_unpushed_commits(self, git_repo):
        ...
        # Make it old (25h â€” in the 24-72h soft tier)
        old_time = time.time() - (25 * 3600)
        os.utime(info["path"], (old_time, old_time))

        # Check for unpushed commits (simulates prune logic)
        has_unpushed = _has_unpushed_commits(info["path"])
        assert has_unpushed  # Has unpushed commits â†’ not pruned in soft tier
        assert Path(info["path"]).exists()
    ...
    def test_force_prunes_very_old_worktree(self, git_repo):
        ...
        # Make it very old (73h â€” beyond the 72h hard threshold)
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned zed/* and pr-* branches."""

    def test_prunes_orphaned_zed_branch(self, git_repo):
        """zed/zed-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "zed/zed-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "zed/zed-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "zed/zed-deadbeef" in result.stdout
        ...
        assert "zed/zed-deadbeef" in orphaned
        ...
        assert "zed/zed-deadbeef" not in result.stdout
