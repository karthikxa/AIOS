...
class TestHandleFastCommand(unittest.TestCase):
    ...
    def test_no_args_shows_status(self):
        ...
        with (
            ...
            cli_mod.ZedCLI._handle_fast_command(stub, "/fast")
    ...
    def test_normal_argument_clears_service_tier(self):
        ...
        with (
            ...
            cli_mod.ZedCLI._handle_fast_command(stub, "/fast normal")
...
class TestPriorityProcessingModels(unittest.TestCase):
    ...
    def test_all_documented_models_supported(self):
        ...
        # All OpenAI flagship models support Priority Processing â€” including
    ...
    def test_all_anthropic_models_supported(self):
        ...
        (Opus 4.8's fast offering is a separate ``â€¦-fast`` model id selected
        via the model field, not this parameter â€” see the adapter test.)
        """
        from zed_cli.models import model_supports_fast_mode
    ...
    def test_codex_models_excluded(self):
        ...
        for model in ["gpt-5-codex", "gpt-5.2-codex", "gpt-5.3-codex", "gpt-5.1-codex-max"]:
            assert not model_supports_fast_mode(model), f"{model} is codex â€” should not expose /fast"
...
class TestFastModeRouting(unittest.TestCase):
    def test_fast_command_exposed_for_model_even_when_provider_is_auto(self):
        ...
        assert cli_mod.ZedCLI._fast_command_available(stub) is True
    ...
    def test_turn_route_injects_overrides_without_provider_switch(self):
        ...
        route = cli_mod.ZedCLI._resolve_turn_agent_config(stub, "hi")
...
class TestAnthropicFastMode(unittest.TestCase):
    ...
    def test_anthropic_non_opus46_models_excluded(self):
        """The speed=fast parameter is gated to Opus 4.6 â€” others excluded.

        Per https://platform.claude.com/docs/en/build-with-claude/fast-mode,
        sending speed=fast to Opus 4.7, Sonnet, or Haiku returns HTTP 400.
        Opus 4.8 uses a separate ``â€¦-fast`` model id, not this parameter.
        """
        from zed_cli.models import model_supports_fast_mode
    ...
    def test_is_anthropic_fast_model(self):
        """The speed=fast parameter is Opus 4.6 only â€” other Claude excluded."""
        from zed_cli.models import _is_anthropic_fast_model
        ...
        # Unsupported â€” would 400 (4.7) or uses a separate model id (4.8)
    ...
    def test_fast_command_hidden_for_anthropic_sonnet(self):
        """Sonnet doesn't support fast mode (Opus 4.6 only) â€” /fast must be hidden."""
        cli_mod = _import_cli()
        stub = SimpleNamespace(
            provider="anthropic", requested_provider="anthropic",
            model="claude-sonnet-4-6", agent=None,
        )
        assert cli_mod.ZedCLI._fast_command_available(stub) is False

    def test_fast_command_hidden_for_anthropic_opus_47(self):
        """Opus 4.7 doesn't take the speed=fast parameter â€” /fast must hide."""
        cli_mod = _import_cli()
        stub = SimpleNamespace(
            provider="anthropic", requested_provider="anthropic",
