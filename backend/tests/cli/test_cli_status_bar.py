...
class TestCLIStatusBar:
    ...
    def test_input_height_counts_prompt_only_on_first_wrapped_row(self):
        ...
        # cells, "âš” " leaves one cell for the first input character, but
    ...
    def test_compression_count_in_wide_fragments(self):
        ...
        assert "ðŸ—œï¸ 7" in frag_texts
        frag_styles = {text: style for style, text in frags}
        assert frag_styles["ðŸ—œï¸ 7"] == "class:status-bar-warn"
    ...
    def test_compression_count_absent_from_fragments_when_zero(self):
        ...
        assert not any("ðŸ—œï¸" in t for t in frag_texts)
    ...
    def test_input_rules_hide_after_resize_until_next_input(self):
        ...
        See _recover_after_resize â€” column shrink reflows already-rendered
    ...
    def test_scrollback_box_width_returns_viewport_width(self):
        ...
        already printed Panel borders â€” that's a cosmetic artifact of
        stamped scrollback history, not a live-render bug.
        """
        from cli import ZedCLI

        # Floor at 32 â€” narrow terminals still get something usable
        # (avoids negative ``'â”€' * (w - 2)`` math).
        assert ZedCLI._scrollback_box_width(20) == 32
        assert ZedCLI._scrollback_box_width(32) == 32
        # Above the floor, return the actual viewport width â€” no cap.
        assert ZedCLI._scrollback_box_width(48) == 48
        assert ZedCLI._scrollback_box_width(80) == 80
        assert ZedCLI._scrollback_box_width(120) == 120
        assert ZedCLI._scrollback_box_width(200) == 200
    ...
    def test_voice_status_bar_compacts_on_narrow_terminals(self):
        ...
        assert fragments == [("class:voice-status", " ðŸŽ¤ Ctrl+B ")]
    ...
    def test_voice_status_bar_renders_configured_ctrl_letter(self):
        ...
        assert compact == [("class:voice-status", " ðŸŽ¤ Ctrl+O ")]
    ...
    def test_voice_recording_status_bar_renders_configured_named_key(self):
        ...
        assert fragments == [("class:voice-status-recording", " â— REC  Ctrl+Space to stop ")]
    ...
    def test_voice_status_bar_falls_back_to_ctrl_b_without_cache(self):
        ...
        cli_obj._voice_tts = False
        cli_obj._voice_continuous = False
        # No cache set â€” mirrors pre-startup state; fall back to
        # documented Ctrl+B default (Copilot round-13 review).

        compact = cli_obj._get_voice_status_fragments(width=50)

        assert compact == [("class:voice-status", " ðŸŽ¤ Ctrl+B ")]
...
class TestIdleSinceLastTurn:
    ...
    def test_hidden_before_first_turn(self):
        assert ZedCLI._format_idle_since(None, turn_live=False) == ""

    def test_hidden_while_turn_is_live(self):
        assert ZedCLI._format_idle_since(time.time() - 30, turn_live=True) == ""

    def test_shows_compact_idle_time_after_turn(self):
        label = ZedCLI._format_idle_since(time.time() - 42, turn_live=False)
        assert label.startswith("âœ“ ")
        assert label == "âœ“ 42s"

    def test_scales_to_minutes(self):
        label = ZedCLI._format_idle_since(time.time() - 3 * 60, turn_live=False)
        assert label == "âœ“ 3m"
    ...
    def test_snapshot_carries_idle_since(self):
        ...
        snapshot = cli_obj._get_status_bar_snapshot()
        assert snapshot["idle_since"].startswith("âœ“ ")
