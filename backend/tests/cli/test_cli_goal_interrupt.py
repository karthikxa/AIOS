...
minimal ``ZedCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def zed_home(tmp_path, monkeypatch):
    """Isolated ZED_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".zed"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("ZED_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves ZED_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal ZedCLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    def test_interrupted_turn_pauses_goal_and_skips_continuation(self, zed_home):
        ...
        # regressed â€” fail loudly instead of silently querying a mock.
        with patch("zed_cli.goals.judge_goal") as judge_mock:
    ...
    def test_interrupted_turn_is_resumable(self, zed_home):
        ...
        with patch("zed_cli.goals.judge_goal"):
            cli._maybe_continue_goal_after_turn()
...
class TestEmptyResponseSkip:
    def test_empty_response_does_not_invoke_judge(self, zed_home):
    ...
    def test_no_assistant_message_skipped(self, zed_home):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, zed_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, zed_home):
        ...
        src = inspect.getsource(ZedCLI.chat)
        ...
        assert "self._last_turn_interrupted = False" in head, (
            ...
            "runs â€” otherwise a prior turn's interrupt state leaks into the "
