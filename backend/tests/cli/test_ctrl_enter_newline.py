"""Regression tests for issue #22379 â€” Ctrl+Enter newline over SSH/WSL.
...
(native, WSL, and SSH-forwarded sessions) sends Ctrl+Enter as bare LF â€” same
...
def test_pure_local_linux_does_not_preserve():
    """A bare local Linux TTY (no SSH/WSL/WT/Ghostty) keeps c-j â†’ submit so docker exec
    style Enter-as-LF stays usable."""
    import cli as cli_mod
    # Stub out /proc reads â€” those are the WSL fallback signal.
    with patch.object(sys, "platform", "linux"):
...
# ---------------------------------------------------------------------------
# install_ctrl_enter_alias() â€” ANSI sequence mappings for enhanced terminals
# ---------------------------------------------------------------------------
...
def test_install_ctrl_enter_alias_maps_csi_u_sequences():
    ...
    from zed_cli.pt_input_extras import install_ctrl_enter_alias
