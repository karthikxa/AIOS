...
pulls from that queue and calls ``process_command()`` â€” but while the
...
running â€” matching the existing pattern for /model â€” so the steer
...
def _make_cli():
    """Create a ZedCLI instance with prompt_toolkit stubbed out."""
    _clean_config = {
        "model": {
...
class TestSteerBusyPathDispatch:
    ...
    def test_idle_path_queues_as_next_turn(self):
        """Control â€” when the agent is NOT running, /steer correctly falls
