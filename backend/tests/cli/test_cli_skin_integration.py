...
from cli import ZedCLI, _rich_text_from_ansi
...
class TestCliSkinPromptIntegration:
    ...
    def test_narrow_terminals_compact_voice_prompt_fragments(self):
        ...
        with patch.object(ZedCLI, "_get_tui_terminal_width", return_value=50):
            assert cli._get_tui_prompt_fragments() == [("class:voice-prompt", "ðŸŽ¤ ")]
    ...
    def test_narrow_terminals_compact_voice_recording_prompt_fragments(self):
        ...
        assert frags[0][1].startswith("â—")
        assert "â¯" not in frags[0][1]
