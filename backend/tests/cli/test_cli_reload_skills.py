"""Tests for the ``/reload-skills`` CLI slash command (``ZedCLI._reload_skills``).

The CLI handler prints the diff (name + description) for the user and â€”
when any skills were added or removed â€” queues a one-shot note on
...
def _make_cli():
    """Build a minimal ZedCLI shell exposing ``_reload_skills``."""
...
class TestReloadSkillsCLI:
    def test_reports_added_and_removed_and_queues_note(self, capsys):
        ...
        # Must NOT pollute conversation_history â€” alternation-safe.
