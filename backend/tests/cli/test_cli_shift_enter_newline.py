...
def _ensure_alias_installed():
    """Make every test idempotent â€” install the alias once per test run."""
...
def test_install_overwrites_stock_modifyotherkeys_shift_enter():
    """Stock prompt_toolkit maps `\\x1b[27;2;13~` to plain Keys.ControlM â€”
...
def test_install_returns_zero_when_already_correct():
    """Idempotency â€” running install twice should not report a second change."""
    install_shift_enter_alias()
...
def test_plain_enter_remains_distinct_from_alt_enter():
    ...
    Alt+Enter tuple â€” otherwise we would have broken submit."""
    enter = _parse("\r")
    alt_enter = _parse("\x1b\r")
