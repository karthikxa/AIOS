"""Regression tests for #15165 (CLI sibling site) â€” CLI exit cleanup must
...
``AIAgent`` has no ``conversation_history`` attribute â€” so the ``or []``
...
@patch("zed_cli.plugins.invoke_hook")
def test_cleanup_forwards_session_messages(mock_invoke_hook):
    ...
    transcript = [
        ...
        {"role": "assistant", "content": "Got it â€” Biscuit."},
    ]
...
def test_cleanup_non_list_attribute_falls_back_to_no_arg(mock_invoke_hook):
    ...
    agent = MagicMock()
    agent.session_id = "cli-session-id"
    # No explicit _session_messages â€” MagicMock synthesises one on access.
