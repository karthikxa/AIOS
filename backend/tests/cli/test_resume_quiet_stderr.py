...
``tool_progress_mode == "off"`` (set by ``zed chat --quiet``):

  * "Session not found: ..."
  * "â†» Resumed session ... (N user messages, M total messages)"
...
def _make_cli(quiet=False, session_id="20260524_111111_xyz", db=None):
    """Build a minimal ZedCLI bound to only what _init_agent needs for
    ...
    # we'll let raise â€” we only check stdout/stderr printed BEFORE that.
...
class TestResumeQuietStderr:
    def test_session_not_found_goes_to_stderr_in_quiet_mode(self, capsys):
        ...
        assert "zed sessions list" in captured.err
    ...
    def test_session_not_found_goes_to_stdout_in_full_mode(self, capsys):
        ...
        captured = capsys.readouterr()
        assert result is False
        # Interactive mode keeps the existing _cprint path â†’ stdout.
    ...
    def test_resumed_banner_goes_to_stderr_in_quiet_mode(self, capsys):
        ...
        with patch("cli._prepare_deferred_agent_startup"):
            try:
                ...
                # stubbed context (no API key, no real config) â€” we only
                # care about the printed banner that comes earlier.
                pass

        captured = capsys.readouterr()
        # Banner on stderr â€” stdout stays clean for automation.
        assert "â†» Resumed session" not in captured.out
        assert "â†» Resumed session" in captured.err
