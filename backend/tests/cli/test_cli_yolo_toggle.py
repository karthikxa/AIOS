...
Pre-fix bug (issue #33925): ``cli.ZedCLI._toggle_yolo`` mutated only
``os.environ["ZED_YOLO_MODE"]``. That env var is captured once at
...
prompt. Only ``zed --yolo`` (process-start env), ``ZED_YOLO_MODE=1``,
and ``zed config set approvals.mode off`` actually bypassed.
...
read (``session_id``). This avoids the heavy ``ZedCLI`` construction
path used in ``test_cli_init.py``, which is incompatible with this test
file's path layout â€” ``ZedCLI.__init__`` imports a lot of optional
...
def _make_stand_in(session_id: str = SESSION_KEY) -> SimpleNamespace:
    ...
    that only read ``self.session_id`` â€” no other CLI state is touched.
    Calling them as unbound functions against this stand-in is equivalent
    to invoking them on a fully-constructed ``ZedCLI`` for the
    behaviour under test, and avoids the brittle prompt_toolkit / config
    stubbing required to instantiate ``ZedCLI`` from this test file.
    """
...
class TestToggleYoloIsSessionScoped:
    ...
    def test_toggle_yolo_disables_session_bypass_on_second_call(self):
        stand_in = _make_stand_in()
        with patch("cli._cprint"):
            ZedCLI._toggle_yolo(stand_in)  # ON
            assert approval_module.is_session_yolo_enabled(SESSION_KEY) is True
            ZedCLI._toggle_yolo(stand_in)  # OFF
            assert approval_module.is_session_yolo_enabled(SESSION_KEY) is False

    def test_toggle_yolo_does_not_mutate_env_var(self):
        """Toggling /yolo must not write ``ZED_YOLO_MODE`` â€” that path is
        ...
        assert os.environ.get("ZED_YOLO_MODE") is None
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_reflects_toggle(self):
        ...
        assert ZedCLI._is_session_yolo_active(stand_in) is False

        with patch("cli._cprint"):
            ZedCLI._toggle_yolo(stand_in)

        assert ZedCLI._is_session_yolo_active(stand_in) is True
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``zed --yolo`` sets ``ZED_YOLO_MODE`` before tool imports, so
...
class TestToggleYoloEndToEnd:
    ...
    def test_toggle_yolo_bypasses_dangerous_command_check(self):
        ...
        try:
            with patch("cli._cprint"):
                ZedCLI._toggle_yolo(stand_in)  # YOLO ON
...
class TestIsSessionYoloActiveAttrSafety:
    ...
    (tests use ``ZedCLI.__new__(ZedCLI)`` to skip ``__init__``). It must
    not raise ``AttributeError`` when ``session_id`` is absent â€” the
    ...
    def test_helper_survives_missing_session_id_attr(self):
        ...
        assert ZedCLI._is_session_yolo_active(no_attr) is False
...
class TestSessionRotationTransfersYolo:
    ...
    the next turn â€” the same UX failure mode this PR set out to fix.
    ...
    def test_transfer_moves_yolo_to_new_session(self):
        ...
        try:
            ...
            ZedCLI._transfer_session_yolo(stand_in, "old-id", "new-id")
    ...
    def test_transfer_is_noop_when_ids_match(self):
        ...
        try:
            ...
            ZedCLI._transfer_session_yolo(stand_in, "same-id", "same-id")
            # Must NOT have been disabled â€” same-id == same-id is a no-op,
    ...
    def test_transfer_handles_empty_inputs_safely(self):
        ...
        # to transfer from "" / to "".
        ZedCLI._transfer_session_yolo(stand_in, "", "new")
        ZedCLI._transfer_session_yolo(stand_in, "old", "")
