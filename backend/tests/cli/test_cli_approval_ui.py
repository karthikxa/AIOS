...
class TestCliApprovalUi:
    ...
    def test_approval_display_preserves_command_and_choices_with_long_description(self):
        ...
        cli = _make_cli_stub()
        long_desc = (
            "Security scan â€” [CRITICAL] Destructive shell command with wildcard expansion: "
        ...
        # Every choice must render â€” this is the core bug: approve/deny were
        ...
        assert rendered.rstrip().endswith("â•¯")

        # The description gets truncated â€” marker should appear.
    ...
    def test_background_task_registers_thread_local_approval_callbacks(self):
        ...
        assert seen["approval"].__func__ is ZedCLI._approval_callback
        assert seen["sudo"].__self__ is cli
        assert seen["sudo"].__func__ is ZedCLI._sudo_password_callback
...
def _make_real_paint_cli_stub():
    ...
    be dropped under these conditions â€” _paint_now must paint regardless.
    """
    cli = ZedCLI.__new__(ZedCLI)
    cli._approval_state = None
    cli._approval_deadline = 0
    ...
    cli._paint_now = ZedCLI._paint_now.__get__(cli, ZedCLI)
    cli._invalidate = ZedCLI._invalidate.__get__(cli, ZedCLI)
...
class TestModalPaintNow:
    """Regression for #41098 â€” modal prompts must paint immediately.
    ...
    or while a resize is pending â€” so the prompt timed out unseen. They now use
...
class TestApprovalCallbackThreadLocalWiring:
    ...
    registered in the main thread â€” the dangerous-command prompt silently fell
    ...
    def test_child_thread_registration_is_visible_and_cleared_in_finally(self):
        ...
        # Finally block must clear both slots â€” otherwise a reused thread
...
class TestPersistPromptSummary:
    """display.persist_prompts â€” one-line scrollback record of resolved modals."""
