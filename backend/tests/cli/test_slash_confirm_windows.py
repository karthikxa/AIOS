...
``call_soon_threadsafe``, so it is safe on every platform â€” including native
Windows (#33961), where the earlier ``sys.platform == "win32"`` â†’ raw ``input()``
...
1. Daemon-thread confirm uses the modal via the app loop on Linux AND native
   Windows (#33961) â€” never the raw stdin fallback, never a hang.
...
def _make_cli():
    """Minimal ZedCLI shell exposing the prompt/modal helpers."""
    import cli as cli_mod

    obj = object.__new__(cli_mod.ZedCLI)
    obj._app = MagicMock()
...
def _run_on_daemon(call, cli, *, platform, response, schedule=None):
    """Invoke ``call`` on a daemon thread â€” as the process_loop does â€” answering
    ...
    assert not worker.is_alive(), "daemon thread hung â€” modal deadlocked"
    return outcome
...
class TestModal:
    ...
    def test_daemon_thread_uses_modal_via_app_loop(self, platform):
        ...
        call_soon_threadsafe on every platform â€” including native Windows, where
    ...
    def test_windows_no_app_falls_back_to_stdin(self):
        """win32 without a running app keeps stdin â€” the only case where the raw
    ...
    def test_daemon_thread_no_app_loop_uses_fallback(self, platform, expect_stdin, expect_result):
        ...
        circuits at the app_loop-is-None site (cli.py ~7260) â€” a distinct path
...
class TestConfirmDestructiveSlashWindows:
    ...
    def test_confirm_destructive_slash_uses_modal_on_windows(self, response, expected):
        ...
        and returns the chosen outcome â€” the bug #33961 froze this path."""
...
class TestNativeWindowsNoRawInputDeadlock:
    ...
    assert *routing* (modal vs. stdin) â€” they cannot observe the actual hang
    ...
    pre-#33961 code (win32 â†’ ``_prompt_text_input`` â†’ off-main ``input()``)
    ...
    def test_win32_daemon_thread_never_blocks_on_real_input(self):
        ...
        ``_prompt_text_input`` â†’ off-main ``input()`` â†’ permanent hang.
        """
        cli = _make_cli()
        ...
        assert not worker.is_alive(), (
            "daemon thread hung in real input() â€” native-Windows confirm "
            "deadlock regressed (#33961)"
        )
        # cancel â†’ None; the point is it RETURNED rather than blocking forever.
    ...
    def test_win32_scheduling_failure_cleanly_cancels_no_input(self):
        ...
        (scheduling failure) the off-main-thread path must cancel cleanly â€”
        ...
        worker.join(timeout=5.0)
        assert not worker.is_alive(), (
            "daemon thread hung â€” win32 scheduling-failure fallback used raw "
