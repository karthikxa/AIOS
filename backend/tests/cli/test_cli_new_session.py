...
def _prepare_cli_with_active_session(tmp_path):
    ...
    # Bypass the destructive-slash confirmation gate â€” these tests focus on
...
def test_new_command_rotates_zed_session_id_env_and_context(tmp_path):
    ...
    assert cli.session_id != old_session_id
    assert os.environ["ZED_SESSION_ID"] == cli.session_id
    assert get_session_env("ZED_SESSION_ID") == cli.session_id
...
def test_new_session_with_duplicate_title_surfaces_error(capsys):
    ...
    # the method's __globals__ dict is the one from the live module â€” patch
