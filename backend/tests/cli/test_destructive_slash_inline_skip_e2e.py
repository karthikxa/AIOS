...
Drives ``ZedCLI.process_command("/reset now")`` against a minimal stand-in
and verifies:
...
This is the regression test for issue #30768 â€” the inline-skip escape hatch
...
def _make_cli_stub():
    """Build a minimal ZedCLI-shaped object that can run ``process_command``
    ...
    self_._confirm_destructive_slash = ZedCLI._confirm_destructive_slash.__get__(
        self_, type(self_)
    )
    self_.process_command = ZedCLI.process_command.__get__(self_, type(self_))
...
def test_new_without_skip_token_still_consults_modal():
    ...
    self_._split_destructive_skip = ZedCLI._split_destructive_skip
    self_._normalize_slash_confirm_choice = ZedCLI._normalize_slash_confirm_choice.__get__(
        self_, type(self_)
    )
    self_._confirm_destructive_slash = ZedCLI._confirm_destructive_slash.__get__(
        self_, type(self_)
    ...
    assert not new_session_calls, "user cancelled â€” new_session must not run"
