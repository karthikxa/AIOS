...
objects â€” we're asserting the escape sequences the CLI sends, not that
...
@pytest.fixture
def bare_cli():
    """A ZedCLI with no __init__ â€” we only exercise the redraw helper."""
    cli = object.__new__(ZedCLI)
    return cli
...
class TestForceFullRedraw:
    ...
    def test_missing_app_attr_is_safe(self, bare_cli):
        # Simulate ZedCLI before the TUI has ever been constructed.
        bare_cli._force_full_redraw()  # must not raise
    ...
    def test_sends_full_clear_replays_then_invalidates(self, bare_cli, monkeypatch):
        ...
        # Must erase screen, home cursor, and flush â€” in that order.
    ...
    def test_resize_recovery_skips_clear_when_width_unchanged(self, bare_cli, monkeypatch):
        ...
        # Same width on this resize â†’ rows-only change.
        ...
        # Must NOT clear the screen or scrollback â€” those destroy the banner.
    ...
    def test_resize_recovery_clears_viewport_on_width_change(self, bare_cli, monkeypatch):
        ...
        It must be banner-safe â€” CSI 3J (write_raw) must NOT fire.
        """
        app = MagicMock()
        events = []
    ...
    def test_swallows_renderer_exceptions(self, bare_cli):
        ...
        # propagate â€” otherwise a stray Ctrl+L would crash the CLI.
        app = MagicMock()
