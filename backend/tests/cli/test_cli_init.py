"""Tests for ZedCLI initialization -- catches configuration bugs
...
def _make_cli(env_overrides=None, config_overrides=None, **kwargs):
    """Create a ZedCLI instance with minimal mocking."""
    import importlib
    ...
    clean_env = {"LLM_MODEL": "", "ZED_MAX_ITERATIONS": ""}
    if env_overrides:
...
class TestMaxTurnsResolution:
    ...
    def test_env_var_max_turns(self):
        ...
        cli_obj = _make_cli(env_overrides={"ZED_MAX_ITERATIONS": "42"})
        assert cli_obj.max_turns == 42

    def test_invalid_env_var_max_turns_falls_back_to_default(self):
        """Invalid env values should not crash CLI init."""
        cli_obj = _make_cli(env_overrides={"ZED_MAX_ITERATIONS": "not-a-number"})
...
class TestPromptToolkitTerminalCompatibility:
    def test_lf_enter_binds_to_submit_handler_posix(self):
        ...
        On a bare local POSIX TTY (no SSH/WSL/WT/Ghostty) we keep c-j â†’ submit so
...
class TestHistoryDisplay:
    ...
    def test_history_shows_recent_sessions_when_current_chat_is_empty(self, capsys):
        ...
        cli._session_db.list_sessions_rich.return_value = [
            ...
            },
            {
                ...
                "title": "Checking Running Zed Agent",
                "preview": "check running gateways for zed agent",
                "last_active": 0,
            },
        ]
        ...
        assert "Checking Running Zed Agent" in output
    ...
    def test_resume_updates_zed_session_id_env_and_context(self, tmp_path):
        ...
        os.environ["ZED_SESSION_ID"] = "current_session"
        _VAR_MAP["ZED_SESSION_ID"].set("current_session")

        try:
            cli._handle_resume_command("/resume target_session")

            assert cli.session_id == "target_session"
            assert os.environ["ZED_SESSION_ID"] == "target_session"
            assert get_session_env("ZED_SESSION_ID") == "target_session"
        finally:
            cli._session_db.close()
            os.environ.pop("ZED_SESSION_ID", None)
            _VAR_MAP["ZED_SESSION_ID"].set(_UNSET)

    def test_resume_list_shows_full_long_titles(self, capsys):
        """Long session titles render in full in the /resume table â€” not
    ...
    def test_sessions_with_target_delegates_to_resume(self):
        ...
        with patch.object(cli, "_handle_resume_command") as mock_resume:
            cli.process_command("/sessions Checking Running Zed Agent")

        mock_resume.assert_called_once_with(
            "/resume Checking Running Zed Agent"
        )
...
class TestRootLevelProviderOverride:
    ...
    def test_model_provider_wins_over_root_provider(self, tmp_path, monkeypatch):
        """model.provider takes priority â€” root-level provider is only a fallback."""
        import yaml
        ...
        import cli
        monkeypatch.setattr(cli, "_zed_home", zed_home)
    ...
    def test_root_provider_used_as_fallback_when_model_provider_missing(self, tmp_path, monkeypatch):
        ...
        config_path.write_text(yaml.safe_dump({
            ...
            "model": {
                "default": "google/gemini-3-flash-preview",
                # no explicit model.provider â€” defaults provide "auto"
            },
        }))
