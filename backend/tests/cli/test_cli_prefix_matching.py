"""Tests for slash command prefix matching in ZedCLI.process_command."""
...
class TestSlashCommandPrefixMatching:
    ...
    def test_exact_command_with_args_does_not_recurse(self):
        ...
        original_pc = ZedCLI.process_command
        ...
        # Mock show_config since the test is about recursion, not config display
        with patch.object(ZedCLI, 'process_command', guarded), \
             patch.object(cli_obj, 'show_config'):
            try:
    ...
    def test_ambiguous_prefix_shows_suggestions(self):
        """/re matches multiple commands â€” should show ambiguous message."""
        cli_obj = _make_cli()
    ...
    def test_skill_command_prefix_matches(self):
        ...
        # Should NOT show "Unknown command" â€” should have dispatched or attempted skill
    ...
    def test_shortest_match_preferred_over_longer_skill(self):
        ...
        import cli as cli_mod
        with patch.object(cli_mod, '_skill_commands', fake_skill):
            # /quit is caught by the exact "/quit" branch â†’ process_command returns False
    ...
    def test_tied_shortest_matches_still_ambiguous(self):
        """/re matches /reset and /retry (both 6 chars) â€” no unique shortest, stays ambiguous."""
        cli_obj = _make_cli()
        printed = []
        import cli as cli_mod
    ...
    def test_exact_typed_name_dispatches_over_longer_match(self):
        """/help typed with /help-extra skill installed â†’ exact match wins."""
        cli_obj = _make_cli()
