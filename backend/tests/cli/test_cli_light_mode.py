...
try to fake an OSC 11 reply â€” the env-override branch short-circuits
...
class TestLightModeDetection:
    def test_zed_light_env_true_forces_light(self, cli_mod, monkeypatch):
        monkeypatch.setenv("ZED_LIGHT", "1")
        assert cli_mod._detect_light_mode() is True

    def test_zed_light_env_false_forces_dark(self, cli_mod, monkeypatch):
        monkeypatch.setenv("ZED_LIGHT", "0")
        # Also blank out other signals so nothing else flips it light.
        monkeypatch.delenv("ZED_TUI_LIGHT", raising=False)
        monkeypatch.delenv("ZED_TUI_THEME", raising=False)
        monkeypatch.delenv("ZED_TUI_BACKGROUND", raising=False)
        monkeypatch.delenv("COLORFGBG", raising=False)
        assert cli_mod._detect_light_mode() is False

    def test_theme_hint_light(self, cli_mod, monkeypatch):
        monkeypatch.delenv("ZED_LIGHT", raising=False)
        monkeypatch.delenv("ZED_TUI_LIGHT", raising=False)
        monkeypatch.setenv("ZED_TUI_THEME", "light")
        assert cli_mod._detect_light_mode() is True
    ...
    def test_background_hex_hint_light(self, cli_mod, monkeypatch):
        ...
        monkeypatch.delenv("ZED_TUI_LIGHT", raising=False)
        monkeypatch.delenv("ZED_TUI_THEME", raising=False)
        monkeypatch.setenv("ZED_TUI_BACKGROUND", "#FFFFFF")
        assert cli_mod._detect_light_mode() is True
    ...
    def test_background_hex_hint_dark(self, cli_mod, monkeypatch):
        ...
        monkeypatch.delenv("ZED_TUI_LIGHT", raising=False)
        monkeypatch.delenv("ZED_TUI_THEME", raising=False)
        monkeypatch.setenv("ZED_TUI_BACKGROUND", "#1a1a2e")
        monkeypatch.delenv("COLORFGBG", raising=False)
        assert cli_mod._detect_light_mode() is False
...
class TestLightModeRemap:
    ...
    def test_remap_skips_statusbar_paired_colors(self, cli_mod, monkeypatch):
        ...
        remapped â€” otherwise they go dark-on-dark and disappear.
...
class TestSkinConfigHook:
    ...
    def test_hook_installed(self, cli_mod):
        ...
        assert getattr(SkinConfig, "_zed_light_mode_hook_installed", False) is True
