"""Tests for save_config_value() in cli.py â€” atomic write behavior."""
...
class TestSaveConfigValueAtomic:
    ...
    def test_preserves_readable_unicode_after_config_mutation(self, config_env):
        ...
        config_env.write_text(
            "agent:\n"
            "  system_prompt: ä½ å¥½ï¼Œä¿æŒä¸­æ–‡è¾“å‡º\n"
            "display:\n"
            "  skin: default\n",
            encoding="utf-8",
        )
        ...
        text = config_env.read_text(encoding="utf-8")
        result = yaml.safe_load(text)
        assert result["agent"]["system_prompt"] == "ä½ å¥½ï¼Œä¿æŒä¸­æ–‡è¾“å‡º"
        assert "ä½ å¥½ï¼Œä¿æŒä¸­æ–‡è¾“å‡º" in text
