"""Tests for /save â€” the conversation snapshot slash command.

Regression: the old implementation wrote ``zed_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``zed sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.zed/sessions/saved/`` and prints
...
def test_save_conversation_writes_under_zed_home(zed_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.zed/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the ZED_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "zed_constants"]:
    ...
    cli.ZedCLI.save_conversation(stub)

    # File must NOT be in CWD
    cwd_leak = list(work.glob("zed_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.zed/sessions/saved/
    saved_dir = zed_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("zed_conversation_*.json"))
    ...
    assert "zed --resume 20260101_120000_abc123" in out, out


def test_save_conversation_empty_history_does_nothing(zed_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "zed_constants"]:
        sys.modules.pop(mod, None)
    import cli
