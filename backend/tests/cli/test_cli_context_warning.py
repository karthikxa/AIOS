...
def _isolate(tmp_path, monkeypatch):
    """Isolate ZED_HOME so tests don't touch real config."""
    home = tmp_path / ".zed"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal ZedCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = ZedCLI.__new__(ZedCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["zed-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Zed' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Zed' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Zed' minimum context length."""
