...
The classic prompt_toolkit status bar shows `â–¶ N` when N tasks launched via
...
def _stub_thread() -> threading.Thread:
    """Return a Thread instance that's never started â€” pure dict-value stand-in."""
    return threading.Thread(target=lambda: None)


def _make_cli():
    """Bare-metal ZedCLI for snapshot/build tests (no __init__ side effects)."""
...
def test_snapshot_safe_when_background_tasks_attr_missing():
    """Older ZedCLI instances (tests with __new__, etc.) may lack the attr."""
    cli_obj = ZedCLI.__new__(ZedCLI)
    cli_obj.model = "x"
    cli_obj.agent = None
    cli_obj.session_start = datetime.now()
    # No _background_tasks at all â€” must not raise.
    snap = cli_obj._get_status_bar_snapshot()
    assert snap["active_background_tasks"] == 0
...
def test_narrow_width_omits_bg_indicator():
    """The narrow tier (<52) is already cramped â€” bg is secondary, drop it."""
    cli_obj = _make_cli()
    cli_obj._background_tasks = {"bg_a": _stub_thread()}
...
# â”€â”€ Background terminal-process indicator (âš™ N) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# Distinct from /background tasks above: â–¶ counts agent threads, âš™ counts
...
def test_fragments_include_proc_segment_when_active(monkeypatch):
    ...
    frags = cli_obj._get_status_bar_fragments()
    rendered = "".join(text for _style, text in frags)
    assert "âš™ 1" in rendered


def test_indicators_independent_agents_and_processes(monkeypatch):
    """â–¶ (agent tasks) and âš™ (shell processes) render side-by-side."""
    cli_obj = _make_cli()
    cli_obj._background_tasks = {"bg_a": _stub_thread()}
    _patch_process_registry(monkeypatch, 2)
    cli_obj._status_bar_visible = True
    ...
    frags = cli_obj._get_status_bar_fragments()
    rendered = "".join(text for _style, text in frags)
    assert "â–¶ 1" in rendered
    assert "âš™ 2" in rendered
