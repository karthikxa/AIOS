"""Regression test for #49287 â€” the CLI memory-provider ``on_session_end``
...
bare ``global _active_agent_ref`` â€” correct while the code lived in
...
assert that contract â€” the existing shutdown tests pass only because they
...
def test_mixin_writes_active_agent_ref_to_cli_module():
    ...
    ``_run_cleanup`` reads it â€” on the ``cli`` module, not the mixin module."""
    import cli as cli_mod
    from zed_cli import cli_agent_setup_mixin as mixin_mod
    ...
    try:
        ...
        # The cleanup path reads cli._active_agent_ref â€” it must see the value.
...
def test_mixin_does_not_use_bare_global_for_active_agent_ref():
    ...
    assert "global _active_agent_ref" not in code_lines, (
        ...
        "namespace â€” cli._active_agent_ref stays None and memory shutdown dies "
