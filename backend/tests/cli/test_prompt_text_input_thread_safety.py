"""Tests for ``ZedCLI._prompt_text_input`` thread-safe input dispatch.
...
def _make_cli():
    """Minimal ZedCLI shell exposing prompt fallback helpers."""
...
class TestPromptTextInputThreadSafety:
    ...
    def test_background_thread_cancels_instead_of_hanging(self):
        ...
        hang). The guard cancels to None instead of hanging â€” it must NOT call
        ...
        t.start()
        t.join(timeout=2.0)
        assert not t.is_alive(), "daemon thread hung â€” guard did not cancel cleanly"
