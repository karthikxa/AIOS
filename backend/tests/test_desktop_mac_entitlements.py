...
Zed Desktop signs with ``hardenedRuntime: true`` and points electron-builder
at two entitlement files (see ``apps/desktop/package.json``):

* ``entitlements`` â†’ ``electron/entitlements.mac.plist`` (the main app), and
* ``entitlementsInherit`` â†’ ``electron/entitlements.mac.inherit.plist`` (the
