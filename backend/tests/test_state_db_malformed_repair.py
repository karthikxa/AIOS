...
journal_mode in apply_wal_with_fallback), before _init_schema runs â€” so it
...
def test_auto_heal_attempted_once_per_process(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(zed_state, "repair_state_db_schema", fake_repair)
    ...
    monkeypatch.setattr(zed_state, "repair_state_db_schema", real_repair)
...
def test_strategy_b_rebuild_when_dedup_insufficient(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(zed_state, "_db_opens_cleanly", flaky_check)
    report = repair_state_db_schema(db_path)
    monkeypatch.undo()
...
def test_non_malformed_error_is_not_auto_repaired(tmp_path, monkeypatch):
    ...
    e.g. 'file is not a database' â€” those raise unchanged."""
    db_path = tmp_path / "state.db"
    ...
    monkeypatch.setattr(zed_state, "repair_state_db_schema", spy)
    with pytest.raises(sqlite3.DatabaseError):
        SessionDB(db_path=db_path)
