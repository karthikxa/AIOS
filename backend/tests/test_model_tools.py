...
class TestHandleFunctionCall:
    ...
    def test_tool_hooks_receive_session_and_tool_call_ids(self):
        with (
            ...
            patch("zed_cli.plugins.has_hook", return_value=True),
            patch("zed_cli.plugins.invoke_hook") as mock_invoke_hook,
        ):
    ...
    def test_no_listener_skips_post_and_transform_emit(self):
        ...
        ``has_hook`` and never build/dispatch a payload â€” so the
        ...
        with (
            patch("model_tools.registry.dispatch", return_value='{"ok":true}'),
            patch("zed_cli.plugins.has_hook", return_value=False),
            patch("zed_cli.plugins.invoke_hook") as mock_invoke_hook,
        ):
    ...
    def test_tool_request_and_execution_middleware_wrap_registry_dispatch(self, monkeypatch):
        ...
        monkeypatch.setattr("zed_cli.plugins.invoke_middleware", fake_invoke_middleware)
...
class TestPreToolCallBlocking:
    ...
    def test_blocked_tool_returns_error_and_skips_dispatch(self, monkeypatch):
        ...
        monkeypatch.setattr("zed_cli.plugins.invoke_hook", fake_invoke_hook)
        monkeypatch.setattr("zed_cli.plugins.has_hook", lambda name: True)
        monkeypatch.setattr("model_tools.registry.dispatch", fake_dispatch)
    ...
    def test_skip_flag_prevents_double_fire(self, monkeypatch):
        ...
        handle_function_call must NOT fire it a second time â€” that was
        ...
        # post_tool_call and transform_tool_result still fire â€” only the
    ...
    def test_run_agent_pattern_fires_pre_tool_call_exactly_once(self, monkeypatch):
        ...
        hook MUST fire exactly once across both calls â€” not twice as it
...
# =========================================================================
# _coerce_number â€” inf / nan must fall through to the original string
...
class TestCoerceNumberInfNan:
    ...
    def test_infinity_spelling_returns_original_string(self):
        from model_tools import _coerce_number
        # Python's float() parses "Infinity" too â€” still not JSON-safe.
    ...
    def test_normal_numbers_still_coerce(self):
        """Guard against over-correction â€” real numbers still coerce."""
        from model_tools import _coerce_number
