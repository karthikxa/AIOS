...
Each test calls build_kwargs twice â€” once with legacy flags, once with provider_profile â€”
...
class TestKimiProfileParity:
    ...
    def test_thinking_enabled(self, transport):
        # xor contract: explicit effort â†’ reasoning_effort only, no thinking.
    ...
    def test_reasoning_effort_default(self, transport):
        # xor contract: enabled w/o effort â†’ thinking-enabled only, no effort.
        rc = {"enabled": True}
        legacy = transport.build_kwargs(
            model="kimi-k2", messages=_msgs(), tools=None,
            provider_profile=get_provider_profile("kimi-coding"), reasoning_config=rc,
...
class TestNousProfileParity:
    def test_tags(self, transport):
        legacy = transport.build_kwargs(
            model="zed-3", messages=_msgs(), tools=None, provider_profile=get_provider_profile("nous"),
        )
        profile = transport.build_kwargs(
            model="zed-3", messages=_msgs(), tools=None,
