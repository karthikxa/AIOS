...
to replace flags, every assertion here must still pass â€” any failure is
...
class TestKimiParity:
    ...
    def test_thinking_enabled_without_effort(self, transport):
        # enabled but no effort â†’ fall back to the thinking toggle, no effort.
        kw = transport.build_kwargs(
            model="kimi-k2",
            messages=_simple_messages(),
            tools=None,
    ...
    def test_reasoning_effort_default_no_effort(self, transport):
        ...
        # "medium" alongside thinking â€” the pairing this fix removes).
        kw = transport.build_kwargs(
            model="kimi-k2",
            messages=_simple_messages(),
            tools=None,
...
class TestOpenRouterParity:
    ...
    def test_reasoning_omitted_for_mandatory_anthropic(self, transport):
        ...
        field â€” sending one makes OpenRouter emit thinking.type.disabled on
...
class TestCustomOllamaParity:
    """Custom/Ollama: num_ctx, thinking controls â€” now tested via profile."""
