...
Verifies that:
 ...
 2. User plugins at $ZED_HOME/plugins/model-providers/<name>/ override bundled
...
def _clear_provider_caches():
    ...
    for mod in list(sys.modules.keys()):
        if (
            ...
            or mod.startswith("_zed_user_provider")
        ):
            del sys.modules[mod]
...
def test_all_profiles_register():
    ...
    This is an invariant â€” the number of profiles matches the number of plugin
...
def test_general_plugin_manager_skips_model_provider_kind(tmp_path, monkeypatch):
    ...
    user_plugin = zed_home / "plugins" / "test-model-provider"
