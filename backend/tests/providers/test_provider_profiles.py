...
class TestKimiProfile:
    ...
    def test_thinking_enabled(self):
        ...
        # reasoning_effort ONLY â€” never paired with extra_body.thinking.
        p = get_provider_profile("kimi")
    ...
    def test_reasoning_effort_default(self):
        # enabled with no effort â†’ thinking toggle only, no top-level effort.
        p = get_provider_profile("kimi")
    ...
    def test_no_config_defaults(self):
        # No reasoning_config â†’ thinking on, server picks depth; no effort.
        p = get_provider_profile("kimi")
...
class TestOpenRouterProfile:
    ...
    def test_pareto_min_coding_score_emitted_for_pareto_model(self):
        """min_coding_score â†’ plugins block when model is openrouter/pareto-code."""
        p = get_provider_profile("openrouter")
        body = p.build_extra_body(
    ...
    def test_pareto_score_ignored_for_other_models(self):
        """Score has no effect on any other model â€” plugins block must not appear."""
        p = get_provider_profile("openrouter")
        body = p.build_extra_body(
    ...
    def test_pareto_score_unset_omits_plugins(self):
        """Empty/None score â†’ no plugins block (router uses its omission default)."""
        p = get_provider_profile("openrouter")
        for unset in (None, ""):
            body = p.build_extra_body(
                model="openrouter/pareto-code",
    ...
    def test_pareto_score_out_of_range_dropped(self):
        """Invalid scores are silently dropped â€” never forwarded to OR."""
        p = get_provider_profile("openrouter")
    ...
    def test_reasoning_disable_omitted_for_mandatory_anthropic(self):
        ...
        for model in (
            ...
            "anthropic/claude-some-future-7",    # unknown â†’ default mandatory
    ...
    def test_reasoning_disable_kept_for_non_anthropic(self):
        """Non-Anthropic models (DeepSeek, Qwen, â€¦) disable reasoning fine; the
    ...
    def test_reasoning_omitted_for_mandatory_anthropic_even_when_enabled(self):
        ...
        thinking â€” OpenRouter ignores reasoning.effort for them, and sending any
    ...
    # --- reasoning-mandatory Anthropic effort â†’ top-level verbosity (#43432) ---
    #
    ...
    # ``reasoning`` field (which would 400 â€” see #42991). Gate every fixture on
    ...
    def test_mandatory_anthropic_effort_routes_to_verbosity(self):
        """effort set + reasoning enabled â†’ top-level verbosity == effort,
    ...
    def test_mandatory_anthropic_verbosity_is_value_agnostic_passthrough(self):
        """The mapping passes the effort value through verbatim â€” it must NOT
    ...
    def test_mandatory_anthropic_no_verbosity_when_effort_absent(self):
        """No effort / none / disabled â†’ no verbosity emitted, so the model
    ...
    def test_non_mandatory_reasoning_model_unchanged_no_verbosity(self):
        ...
        ``reasoning`` in extra_body and never get a ``verbosity`` field â€” the
    ...
    def test_mandatory_anthropic_verbosity_coexists_with_grok_header(self):
        ...
        top-level dict must remain a single merged dict â€” verify the verbosity
        path doesn't clobber the extra_headers slot used by Grok affinity."""
        p = get_provider_profile("openrouter")
        # mandatory anthropic + effort â†’ verbosity, no extra_headers
        _, tl = p.build_api_kwargs_extras(
            reasoning_config={"enabled": True, "effort": "high"},
            supports_reasoning=True,
            model="anthropic/claude-fable-5",
