"""Shared fixtures for the zed-agent test suite.
...
2. **Isolated ZED_HOME.** ZED_HOME points to a per-test tempdir so
   code reading ``~/.zed/*`` via ``get_zed_home()`` can't see the
   real one. (We do NOT also redirect HOME â€” that broke subprocesses in
   CI. Code using ``Path.home() / ".zed"`` instead of the canonical
   ``get_zed_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ZED_SESSION_* inheritance** â€” the agent's current gateway
...
# â”€â”€ Per-file process isolation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# author's responsibility â€” if test A in foo.py mutates state that test B
...
# â”€â”€ Credential env-var filter â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#
...
# ZED_* vars that change test behavior by being set. Unset all of these
# unconditionally â€” individual tests that need them set do so explicitly.
_ZED_BEHAVIORAL_VARS = frozenset({
    "ZED_YOLO_MODE",
    "ZED_INTERACTIVE",
    "ZED_QUIET",
    "ZED_TOOL_PROGRESS",
    "ZED_TOOL_PROGRESS_MODE",
    "ZED_MAX_ITERATIONS",
    "ZED_SESSION_PLATFORM",
    "ZED_SESSION_CHAT_ID",
    "ZED_SESSION_CHAT_NAME",
    "ZED_SESSION_THREAD_ID",
    "ZED_SESSION_SOURCE",
    "ZED_SESSION_KEY",
    "ZED_GATEWAY_SESSION",
    "ZED_CRON_SESSION",
    ...
    "ZED_TIMEZONE",
    "ZED_REDACT_SECRETS",
    "ZED_BACKGROUND_NOTIFICATIONS",
    "ZED_EXEC_ASK",
    "ZED_HOME_MODE",
    "ZED_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.zed/kanban.db instead of the per-test ZED_HOME.
    "ZED_KANBAN_DB",
    "ZED_KANBAN_BOARD",
    "ZED_KANBAN_HOME",
    "ZED_KANBAN_WORKSPACES_ROOT",
    "ZED_KANBAN_LOGS_ROOT",
    "ZED_KANBAN_TASK",
    "ZED_KANBAN_WORKSPACE",
    "ZED_KANBAN_RUN_ID",
    "ZED_KANBAN_CLAIM_LOCK",
    "ZED_KANBAN_DISPATCH_IN_GATEWAY",
    "ZED_TENANT",
    ...
    "ZED_DASHBOARD_OAUTH_CLIENT_ID",
    "ZED_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
    ...
    "BROWSER_CDP_URL",
    "CAMOFOX_URL",
    # Platform allowlists â€” not credentials, but if set from any source
    ...
    # Platform gating â€” set by load_gateway_config() as a side effect when
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ZED_HOME to per-test tempdirs so code that
    reads ``~/.zed/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ZED_* vars that could change test semantics.
    for name in _ZED_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect ZED_HOME to a per-test tempdir. Code that reads
    #    ``~/.zed/*`` via ``get_zed_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.zed/*`` via
    #    ``Path.home() / ".zed"`` instead of ``get_zed_home()``
    #    is a bug to fix at the callsite.
    fake_zed_home = tmp_path / "zed_test"
    fake_zed_home.mkdir()
    (fake_zed_home / "sessions").mkdir()
    (fake_zed_home / "cron").mkdir()
    (fake_zed_home / "memories").mkdir()
    (fake_zed_home / "skills").mkdir()
    monkeypatch.setenv("ZED_HOME", str(fake_zed_home))
    ...
    #     out. Tests don't run on EC2 â€” IMDS is always unreachable here.
...
# Backward-compat alias â€” old tests reference this fixture name. Keep it
# as a no-op wrapper so imports don't break.
@pytest.fixture(autouse=True)
def _isolate_zed_home(_hermetic_environment):
    """Alias preserved for any test that yields this name explicitly."""
    return None


# â”€â”€ Module-level state reset â€” replaced by per-file process isolation â”€â”€â”€â”€â”€â”€
#
...
@pytest.fixture()
def mock_config():
    """Return a minimal zed config dict suitable for unit tests."""
    return {
...
# â”€â”€ Per-test timeout â€” handled by the isolation plugin â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#
...
# â”€â”€ Live-system guard â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#
...
# environment and finds the developer's live ``zed-gateway`` process
# via ``psutil`` â€” sending it SIGTERM mid-test. The shutdown forensics in
# PR #23285 caught this happening 5+ times in 3 days, every time
# correlated with a ``tests/zed_cli/`` pytest run starting up.
#
# This fixture makes the leak impossible by intercepting the two
# primitives that actually do damage:
#
#  â€¢ ``os.kill`` rejects any PID outside the test process subtree with
#    a hard ``RuntimeError`` so the offending test gets a stack trace
#    instead of silently murdering the real gateway.
#  â€¢ ``subprocess.run`` / ``subprocess.Popen`` / ``call`` / ``check_call`` /
#    ``check_output`` reject any ``systemctl ... <verb> zed-gateway``
...
# here â€” tests of those functions themselves need the real implementation.
...
def pytest_configure(config):  # noqa: D401 â€” pytest hook
    ...
    config.addinivalue_line(
        ...
        "behaviour â€” e.g. PTY tests that signal their own child).",
    )

    # The pyproject addopts pin ``--timeout-method=signal`` relies on
    # ``signal.SIGALRM``, which does not exist on Windows â€” pytest-timeout
...
def _live_system_guard(request, monkeypatch):
    ...
    terminate a foreign process):
      â€¢ os.kill, os.killpg (POSIX)
      â€¢ subprocess.run / Popen / call / check_call / check_output
      â€¢ subprocess.getoutput / getstatusoutput
      â€¢ os.system / os.popen
      â€¢ pty.spawn
      â€¢ asyncio.create_subprocess_exec / create_subprocess_shell
    Subprocess inspection looks at the WHOLE command string (not just
    tokens[0]), so ``bash -c "systemctl restart zed-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting zed/python patterns are also blocked.
    """
    ...
    test_pid = _os.getpid()
    # Capture the test process's existing children at fixture start â€”
    ...
    def _is_own_subtree(pid: int) -> bool:
        ...
        try:
            ...
            # Stale PID â€” kill would be a no-op anyway, allow it.
            return True
        try:
    ...
    def _guarded_kill(pid, sig, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"{pid}, {sig}) â€” PID is outside the test process subtree. "
    ...
    # ``os.killpg`` is the same risk class â€” sends a signal to every
    ...
    if hasattr(_os, "killpg"):
        ...
        def _guarded_killpg(pgid, sig, *args, **kwargs):
            ...
            raise RuntimeError(
                ...
                f"os.killpg({pgid}, {sig}) â€” PGID is outside the test "
                "process group. See _live_system_guard for the why."
            )

        monkeypatch.setattr(_os, "killpg", _guarded_killpg)

    # â”€â”€ Subprocess command-string inspection (whole-line) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    _ZED_TOKENS = (
        "zed-gateway",
        "zed.service",
    ...
    def _matches_zed_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ZED_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_zed_gateway(cmd_str):
            return False
        try:
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) â€” would mutate the "
                "live zed-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) â€” process-killer command "
                "targeting zed/python could hit the live gateway. "
        ...
        if "update" in low and (
            ...
            (".venv/bin/zed" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) â€” this command would run "
                "`zed update` against the real checkout, fetching "
    ...
    # os.system / os.popen â€” same risk class, completely unwrapped before.
    ...
    # asyncio.create_subprocess_* â€” bypasses subprocess module entirely.
    try:
