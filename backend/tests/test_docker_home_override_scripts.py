...
def test_main_wrapper_preserves_docker_workdir() -> None:
    ...
    # first exec / if-block.
    activate_idx = text.index("/opt/zed/.venv/bin/activate")
    restore_idx = text.index('cd "$_zed_orig_cwd"')
    exec_idx = text.index("if [ $# -eq 0 ]")
    assert activate_idx < restore_idx < exec_idx, (
        "cd $_zed_orig_cwd must appear after venv activation and "
...
def test_dashboard_run_resets_home_before_dropping_privileges() -> None:
    ...
    assert "exec s6-setuidgid zed zed dashboard" in text
...
def test_dashboard_run_does_not_derive_insecure_from_bind_host() -> None:
    ...
    ``ZED_DASHBOARD_HOST``. Doing so disables the OAuth auth gate on
    every non-loopback bind even when an auth provider is registered â€”
    the exact regression that exposed every wildcard-subdomain agent
    dashboard publicly until early 2026.

    The opt-in is now explicit: ``ZED_DASHBOARD_INSECURE=1`` (truthy).
    ...
    assert '127.0.0.1|localhost' not in text, (
        ...
        "is the authority now â€” opt in via ZED_DASHBOARD_INSECURE instead."
    )
    ...
    assert "ZED_DASHBOARD_INSECURE" in text, (
        "Explicit ZED_DASHBOARD_INSECURE opt-in is missing."
    )
    # Truthy values aligned with the rest of the s6 scripts
    # (e.g. ZED_DASHBOARD).
    for truthy in ("1", "true", "TRUE", "True", "yes", "YES", "Yes"):
        assert truthy in text, (
            f"ZED_DASHBOARD_INSECURE should accept truthy value {truthy!r}"
        )
...
def test_stage2_hook_repairs_profiles_and_cron_ownership_on_every_boot() -> None:
    ...
    assert 'if [ -d "$ZED_HOME/profiles" ]; then' in text
    assert 'chown -R zed:zed "$ZED_HOME/profiles" 2>/dev/null || true' in text

    assert 'if [ -d "$ZED_HOME/cron" ]; then' in text
    assert 'chown -R zed:zed "$ZED_HOME/cron" 2>/dev/null || true' in text
