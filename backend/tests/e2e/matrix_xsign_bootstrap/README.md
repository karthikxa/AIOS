# Matrix cross-signing bootstrap â€” E2E test
...
a fresh homeserver â€” important because Continuwuity's one-time admin
...
inline so it runs without importing the entire zed gateway and its
many dependencies. **If the source diverges from what's in
`_connect_with_bootstrap`, this test must be updated to match.** A
small price for not requiring the full zed-agent runtime in CI.
