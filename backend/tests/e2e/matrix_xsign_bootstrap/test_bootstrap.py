...
runs the patched ``MatrixAdapter.connect()`` against it, and asserts:
  ...
  1. cross-signing keys get published with **unpadded** base64 keyids
     (the bug this PR fixes â€” padded keyids are silently rejected by
...
class XsignBootstrapE2E(unittest.IsolatedAsyncioTestCase):
    ...
    def setUpClass(cls):
        ...
        # won't activate yet â€” burn the one-time admin token first to
    ...
    async def _connect_with_bootstrap(self, creds: dict, store_dir: Path) -> tuple[list[str], str | None]:
        ...
        # (copied so we can run it without importing the full zed
        ...
        # --- /end patched block ---
        # Clean teardown â€” without this the asyncio loop never exits.
    ...
    async def test_bootstrap_publishes_unpadded_keys(self):
        """Fresh bot â†’ bootstrap fires, keys published unpadded, device signed."""
    ...
    async def test_second_startup_skips_bootstrap(self):
        """Second startup with same crypto store â†’ no second recovery key."""
