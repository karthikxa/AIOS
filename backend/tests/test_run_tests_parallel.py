...
The leaker test always passes â€” its only job is to spawn a grandchild
...
_HANDOFF_DIR = Path(os.environ.get("TMPDIR", "/tmp")) / "zed-isolation-probe"
...
def _pid_alive(pid: int) -> bool:
    ...
    """
    if sys.platform == "win32":  # pragma: no cover â€” POSIX-only test
...
def test_grandchild_leak_is_killed_by_runner(tmp_path: Path) -> None:
    ...
    # never picks it up â€” only our explicit invocation does.
    ...
    probe_src = textwrap.dedent(f"""
        ...
        def test_spawns_grandchild_and_walks_away():
            ...
            # handoff, then walk away â€” don't close the pipe (would
