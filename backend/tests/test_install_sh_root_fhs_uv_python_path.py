...
and the shared ``/usr/local/bin/zed`` wrapper fails for non-root users with
...
def _resolve_install_layout_body() -> str:
    ...
    return between the marker and the export â€” both of which would leave the
...
def test_root_fhs_uv_python_export_is_inside_root_branch() -> None:
    ...
    assert export_idx < return_idx, (
        "Export must precede the branch's `return 0` â€” otherwise unreachable"
    )
