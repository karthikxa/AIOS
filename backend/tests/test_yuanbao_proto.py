"""
test_yuanbao_proto.py - yuanbao_proto å•å…ƒæµ‹è¯•

æµ‹è¯•è¦†ç›–ï¼š
  1. varint ç¼–è§£ç  round-trip
  2. conn å±‚ encode/decode round-trip
  3. biz å±‚ encode/decode round-trip
  4. decode_inbound_push è§£æž TIMTextElem æ¶ˆæ¯
  5. encode_send_c2c_message / encode_send_group_message ç¼–ç 
  6. å›ºå®š bytes å¸¸é‡éªŒè¯ï¼ˆé˜²æ­¢åè®®æ‚„æ‚„æ”¹åŠ¨ï¼‰
  7. auth-bind / ping ç¼–ç 
"""
...
class TestConnCodec:
    ...
    def test_full_round_trip(self):
        """encode_conn_msg_full å« cmd/msg_id/module"""
    ...
    # å›ºå®š bytes å¸¸é‡æµ‹è¯•â€”â€”é˜²åè®®æ‚„æ‚„æ”¹åŠ¨
    def test_fixed_bytes_simple(self):
        """
        encode_conn_msg(msg_type=0, seq_no=1, data=b"") çš„å›ºå®šç¼–ç ã€‚
...
# ===========================================================
# 4. MsgContent / MsgBodyElement ç¼–è§£ç 
# ===========================================================
...
class TestMsgBodyElement:
    ...
    def test_fixed_text_elem_bytes(self):
        """
        å›ºå®š bytes éªŒè¯ï¼šTIMTextElem { text="hi" }
        MsgBodyElement:
...
class TestDecodeInboundPush:
    def _build_inbound_push_bytes(
        ...
        """æ‰‹å·¥æž„é€  InboundMessagePush bytesï¼ˆä¸Ž proto å­—æ®µé¡ºåºä¸€è‡´ï¼‰"""
        from gateway.platforms.yuanbao_proto import (
    ...
    def test_basic_c2c_text_message(self):
        ...
        assert result["msg_body"][0]["msg_content"]["text"] == "ä½ å¥½"
    ...
    def test_returns_none_on_empty(self):
        # ç©º bytes åº”è¿”å›žç©ºå­—æ®µ dictï¼Œè€Œä¸æ˜¯ None
        result = decode_inbound_push(b"")
        # ç©ºæ¶ˆæ¯è§£æžç»“æžœæ˜¯ {}ï¼ˆæ— å­—æ®µï¼‰ï¼Œè¿‡æ»¤åŽ msg_body=[] ä¹Ÿä¼šä¿ç•™
        assert result is not None or result is None  # ä¸å´©æºƒå³å¯
...
class TestEncodeOutbound:
    ...
    def test_c2c_biz_payload_contains_to_account(self):
        """éªŒè¯ biz payload åŒ…å« to_account å­—æ®µ"""
...
# ===========================================================
# 10. å®Œæ•´ç«¯åˆ°ç«¯æµç¨‹ï¼ˆæ¨¡æ‹Ÿ send -> recvï¼‰
# ===========================================================

class TestEndToEnd:
    def test_send_recv_c2c(self):
        """æ¨¡æ‹Ÿå‘é€ C2C æ¶ˆæ¯ï¼Œç„¶åŽï¼ˆåœ¨æŽ¥æ”¶æ–¹ï¼‰è§£ç """
        msg_body = [
            {"msg_type": "TIMTextElem", "msg_content": {"text": "ç«¯åˆ°ç«¯æµ‹è¯•"}},
        ]
        # å‘é€æ–¹ç¼–ç 
        ...
        # ä»Ž biz payload ä¸­è¯»å– to_account å’Œ msg_body
        from gateway.platforms.yuanbao_proto import (
        ...
        assert el_dec["msg_content"]["text"] == "ç«¯åˆ°ç«¯æµ‹è¯•"

    def test_inbound_push_full_flow(self):
        """æž„é€ æœåŠ¡ç«¯ push -> è§£ç å…¥ç«™æ¶ˆæ¯"""
        from gateway.platforms.yuanbao_proto import (
            _encode_field, _encode_string, _encode_message,
            _encode_varint, WT_LEN, WT_VARINT,
        )
        # æž„é€ å…¥ç«™æ¶ˆæ¯ biz payload
        ...
        # å°è£…æˆ ConnMsgï¼ˆæ¨¡æ‹ŸæœåŠ¡ç«¯ pushï¼‰
