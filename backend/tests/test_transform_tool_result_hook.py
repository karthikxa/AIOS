...
def _run_handle_function_call(
    ...
    *,
    ...
    if invoke_hook is not _UNSET:
        ...
        # fire â€” make has_hook agree so the has_hook gate doesn't skip the
...
def test_result_unchanged_when_no_hook_registered(monkeypatch):
    ...
    monkeypatch.setenv("ZED_HOME", "/tmp/zed_no_plugins")
...
def test_transform_tool_result_integration_with_real_plugin(monkeypatch, tmp_path):
    """End-to-end: load a real plugin from ZED_HOME and verify it rewrites results."""
    import yaml
    ...
    # Plugins are opt-in â€” must be listed in plugins.enabled to load.
