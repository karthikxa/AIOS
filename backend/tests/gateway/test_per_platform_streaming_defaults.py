...
def test_resolver_telegram_on_discord_off_when_global_enabled():
    ...
    and Discord not â€” matching the platforms' actual streaming quality."""
    from zed_cli.config import DEFAULT_CONFIG
...
def test_user_override_wins_over_default():
    """A user who explicitly enables Discord streaming keeps their value â€” the
    default false must not clobber it (config deep-merge: user wins)."""
    from zed_cli.config import DEFAULT_CONFIG, _deep_merge
