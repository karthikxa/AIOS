...
class TestLoadBackgroundNotificationsMode:
    ...
    def test_defaults_to_all(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(gw, "_zed_home", tmp_path)
        monkeypatch.delenv("ZED_BACKGROUND_NOTIFICATIONS", raising=False)
    ...
    def test_env_var_overrides_config(self, monkeypatch, tmp_path):
        ...
        import gateway.run as gw
        monkeypatch.setattr(gw, "_zed_home", tmp_path)
        monkeypatch.setenv("ZED_BACKGROUND_NOTIFICATIONS", "off")
...
@pytest.mark.parametrize(
    ...
    [
        # all mode: running output â†’ sends update
        (
            "all",
            [
                SimpleNamespace(output_buffer="building...\n", exited=False, exit_code=None),
                None,  # process disappears â†’ watcher exits
            ],
            1,
            "is still running",
        ),
        # result mode: running output â†’ no update
        (
            "result",
            [
                SimpleNamespace(output_buffer="building...\n", exited=False, exit_code=None),
        ...
        # off mode: exited process â†’ no notification
        (
            "off",
        ...
        # error mode: exit 0 â†’ no notification
        (
            "error",
            [SimpleNamespace(output_buffer="done\n", exited=True, exit_code=0)],
            0,
...
async def test_agent_notification_no_message_id_is_tolerated(monkeypatch, tmp_path):
    ...
    still injects â€” message_id is simply None."""
    import tools.process_registry as pr_module
...
async def test_inject_watch_notification_ignores_foreground_event_source(monkeypatch, tmp_path):
    ...
    # The evt dict carries the correct session_key â€” NOT a foreground event
    evt = {
...
def test_build_process_event_source_returns_none_for_empty_evt(monkeypatch, tmp_path):
    """Missing session_key and no platform metadata â†’ None (drop notification)."""
    runner = _build_runner(monkeypatch, tmp_path, "all")
...
def test_build_process_event_source_returns_none_for_invalid_platform(monkeypatch, tmp_path):
    """Invalid platform string â†’ None."""
    runner = _build_runner(monkeypatch, tmp_path, "all")
...
def test_build_process_event_source_returns_none_for_short_session_key(monkeypatch, tmp_path):
    """Session key with <5 parts doesn't parse, falls through to empty metadata â†’ None."""
    runner = _build_runner(monkeypatch, tmp_path, "all")
...
def test_parse_session_key_with_extra_parts():
    """6th part in a group key may be a user_id, not a thread_id â€” omit it."""
...
def test_parse_session_key_with_user_id_part():
    """Group keys with per-user isolation have user_id as 6th part â€” don't return as thread_id."""
