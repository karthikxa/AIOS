...
def _set_dm_role_auth_guild(monkeypatch, guild_id=None):
    """Stub ``zed_cli.config.read_raw_config`` so ``_read_dm_role_auth_guild``
    resolves to ``guild_id`` (or None for the opt-out default).
    """
    cfg = {"discord": {"dm_role_auth_guild": guild_id if guild_id is not None else ""}}
    # Patch the attribute ``zed_cli.config.read_raw_config`` â€” that's
...
# ---------------------------------------------------------------------------
# Cross-guild DM bypass â€” MUST be rejected
# ---------------------------------------------------------------------------
...
def test_dm_role_auth_requires_explicit_guild_optin(monkeypatch):
    ...
    The user has the role in the opted-in guild â€” allowed.
    """
...
def test_dm_role_auth_optin_rejects_when_not_member(monkeypatch):
    """dm_role_auth_guild set but user isn't a member â†’ reject."""
...
# ---------------------------------------------------------------------------
# Guild messages â€” role check must be scoped to THIS guild only
# ---------------------------------------------------------------------------
...
def test_guild_message_role_check_scoped_to_originating_guild(monkeypatch):
    ...
    # No author object passed â†’ falls through to guild.get_member path
    assert (
...
def test_guild_message_role_check_allows_when_role_in_same_guild(monkeypatch):
    """Positive path: user has the role IN the message's guild â†’ allowed."""
    _set_dm_role_auth_guild(monkeypatch)
...
def test_guild_message_rejects_author_roles_from_different_guild(monkeypatch):
    ...
    message, the cached .roles on it must NOT be trusted â€” rely on the
...
# ---------------------------------------------------------------------------
# Backwards-compatibility â€” user-ID allowlist still works in both contexts
# ---------------------------------------------------------------------------
...
def test_slash_authorization_allows_in_scope_guild_role(monkeypatch):
    """Positive control: slash in guild B, user has role in guild B â†’ allowed."""
    _set_dm_role_auth_guild(monkeypatch)
