...
substrings like 'foo@zed_bot.example'.
...
a corresponding `MENTION` entity is NOT a mention â€” this correctly ignores
...
class TestRealMentionsAreDetected:
    """A real Telegram mention always comes with a MENTION entity â€” detect those."""

    def test_mention_at_start_of_message(self):
        adapter = _make_adapter()
        text = "@zed_bot hello world"
    ...
    def test_mention_mid_sentence(self):
        adapter = _make_adapter()
        text = "hey @zed_bot, can you help?"
        msg = _message(text=text, entities=[_mention_entity(text)])
        assert adapter._message_mentions_bot(msg) is True

    def test_mention_at_end_of_message(self):
        adapter = _make_adapter()
        text = "thanks for looking @zed_bot"
        msg = _message(text=text, entities=[_mention_entity(text)])
        assert adapter._message_mentions_bot(msg) is True

    def test_mention_in_caption(self):
        adapter = _make_adapter()
        caption = "photo for @zed_bot"
...
class TestSubstringFalsePositivesAreRejected:
    ...
    def test_email_like_substring(self):
        """bug #12545 exact repro: 'foo@zed_bot.example'."""
        adapter = _make_adapter()
        msg = _message(text="email me at foo@zed_bot.example")
        assert adapter._message_mentions_bot(msg) is False

    def test_hostname_substring(self):
        adapter = _make_adapter()
        msg = _message(text="contact user@zed_bot.domain.com")
        assert adapter._message_mentions_bot(msg) is False

    def test_superstring_username(self):
        """`@zed_botx` is a different username; Telegram would emit a mention
        entity for `@zed_botx`, not `@zed_bot`."""
        adapter = _make_adapter()
        msg = _message(text="@zed_botx hello")
        assert adapter._message_mentions_bot(msg) is False

    def test_underscore_suffix_substring(self):
        adapter = _make_adapter()
        msg = _message(text="see @zed_bot_admin for help")
        assert adapter._message_mentions_bot(msg) is False

    def test_substring_inside_url_without_entity(self):
        """@handle inside a URL produces a URL entity, not a MENTION entity."""
        adapter = _make_adapter()
        msg = _message(text="see https://example.com/@zed_bot for details")
        assert adapter._message_mentions_bot(msg) is False

    def test_substring_inside_code_block_without_entity(self):
        """Telegram doesn't emit mention entities inside code/pre entities."""
        adapter = _make_adapter()
        msg = _message(text="use the string `@zed_bot` in config")
        assert adapter._message_mentions_bot(msg) is False
    ...
    def test_email_substring_in_caption(self):
        adapter = _make_adapter()
        msg = _message(caption="foo@zed_bot.example")
        assert adapter._message_mentions_bot(msg) is False
...
class TestCaseInsensitivity:
    ...
    def test_uppercase_mention(self):
        adapter = _make_adapter()
        text = "hi @ZED_BOT"
        msg = _message(text=text, entities=[_mention_entity(text, mention="@ZED_BOT")])
        assert adapter._message_mentions_bot(msg) is True
