...
historically bypassed every gate ``on_message`` enforces â€” DISCORD_ALLOWED_USERS,
...
# ---------------------------------------------------------------------------
# Discord module mock â€” borrowed from test_discord_slash_commands.py so this
...
def adapter():
    ...
    a._client = SimpleNamespace(user=SimpleNamespace(id=99999, name="ZedBot"), guilds=[])
    return a
...
def _make_interaction(
    ...
    return SimpleNamespace(
        ...
        # has no members by default â€” tests exercising positive role paths
...
# ---------------------------------------------------------------------------
# Backwards-compat: empty allowlist â†’ everything passes (matches on_message)
# ---------------------------------------------------------------------------
...
async def test_no_allowlist_dm_also_allowed(adapter):
    """Same for DMs â€” no allowlist means no restriction, matching on_message."""
...
# ---------------------------------------------------------------------------
# Channel allowlist (DISCORD_ALLOWED_CHANNELS) parity â€” the gate prajer used
# ---------------------------------------------------------------------------
...
async def test_channel_allowlist_does_not_apply_to_dms(adapter, monkeypatch):
    """DMs aren't channel-gated â€” they go through on_message's DM lockdown."""
...
async def test_unauthorized_attempt_notifies_telegram(adapter):
    ...
    # Notify is fire-and-forget â€” let the scheduled task run.
...
def test_visibility_hide_off_by_default_is_noop(adapter, monkeypatch):
    """DISCORD_HIDE_SLASH_COMMANDS unset â†’ don't touch any command's permissions."""
    ...
    # When called directly the helper applies â€” env gating is at the call site,
...
async def test_missing_user_rejected_when_allowlist_configured(adapter):
    ...
    # Must not raise â€” must return False with an ephemeral rejection
    assert await adapter._check_slash_authorization(interaction, "/help") is False
    interaction.response.send_message.assert_awaited_once()
