...
async def test_rich_happy_path_sends_raw_markdown():
    ...
    # Raw markdown â€” NOT MarkdownV2-escaped. Table pipes still present.
...
async def test_expect_edits_metadata_keeps_preview_on_legacy_path():
    ...
    # rich messages until Zed wires rich_message edits directly.
    bot = adapter._bot
    assert bot is not None
    bot.do_api_request.assert_not_called()
    bot.send_message.assert_awaited()
...
async def test_rich_limit_is_characters_not_bytes():
    ...
    # is 20k chars / 60k UTF-8 bytes â€” over the byte count, under the char cap.
    cjk = "| a | b |\n|---|---|\n" + "æµ‹" * 20000  # 20k chars, ~60k UTF-8 bytes
...
async def test_per_message_bad_request_does_not_latch_off():
    """A parser/limit BadRequest is per-message â€” rich must stay enabled
...
async def test_reply_to_propagates_as_reply_parameters():
    ...
    # the legacy reply_to_message_id scalar â€” unknown params are silently
...
# â”€â”€ Streaming drafts: sendRichMessageDraft â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async def test_finalize_edit_uses_rich_for_table_content():
    ...
    existing message IN PLACE via editMessageText's rich_message param â€”
    ...
    assert api_kwargs["rich_message"]["markdown"] == RICH_CONTENT
    # No fresh send / delete â€” the whole point of the in-place rich edit.
...
async def test_finalize_edit_rich_not_modified_is_success_noop():
    """'Message is not modified' on a rich edit is a no-op success â€” must NOT
