...
opened in ``__init__`` â€” for ``APIServerAdapter`` that means 2 file
...
class _CountingAdapter(BasePlatformAdapter):
    ...
    ``APIServerAdapter`` shape â€” every constructor call opens 2 fds
...
class TestReconnectFDLeakRegression:
    ...
    async def test_nonretryable_failure_disposes_unowned_adapter(self):
        ...
        code with a chance to call disconnect() â€” and disconnect() is
        ...
        # exactly once on the unowned adapter" â€” not "at least once".
        ...
        assert adapter._disconnect_calls == 1, (
            ...
            "(#37011). More than one call would also be a bug â€” the "
...
class TestAPIServerDisconnectClosesResponseStore:
    ...
    test class above) is a no-op for ``APIServerAdapter`` â€” the
    ...
    async def test_disconnect_closes_response_store(self, tmp_path):
        ...
        to ``~/.zed/response_store.db`` (or :memory: as a fallback),
        ...
        # only passes when the close actually took effect â€” a generic
    ...
    async def test_disconnect_swallows_response_store_close_exception(self, tmp_path):
        ...
        # Must not raise â€” disconnect() swallows the close error and
