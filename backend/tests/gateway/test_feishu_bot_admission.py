...
def test_feishu_load_settings_ignores_extra_allow_bots(monkeypatch):
    # extra is ignored â€” env is single source of truth (yaml is bridged to env).
    from gateway.platforms.feishu import FeishuAdapter
...
#
# Covers the four-step admission pipeline (self_echo â†’ bot_policy â†’
# DM bypass â†’ group_policy + mention) as a single result-only matrix.
...
def test_hydrate_bot_identity_populates_self_ids_from_bot_v3_info(monkeypatch):
    ...
    def _fake_request(request):
        ...
        return SimpleNamespace(raw=SimpleNamespace(
            content=b'{"code":0,"bot":{"app_name":"Zed","open_id":"ou_hydrated"}}'
        ))
