...
`/restart` again and immediately restart â€” a self-perpetuating loop.
"""
import json
import time
...
async def test_redelivered_restart_with_same_update_id_is_ignored(tmp_path, monkeypatch):
    ...
    event = _make_restart_event(update_id=12345)  # same update_id â†’ redelivery
    result = await runner._handle_restart_command(event)
...
async def test_redelivered_restart_with_older_update_id_is_ignored(tmp_path, monkeypatch):
    ...
    event = _make_restart_event(update_id=12344)  # older update â€” shouldn't happen,
...
async def test_fresh_restart_with_higher_update_id_is_processed(tmp_path, monkeypatch):
    ...
    event = _make_restart_event(update_id=12346)  # strictly higher â†’ fresh
    result = await runner._handle_restart_command(event)
...
async def test_stale_marker_older_than_5min_does_not_block(tmp_path, monkeypatch):
    """A marker older than the 5-minute window is ignored â€” fresh /restart proceeds."""
    monkeypatch.setattr(gateway_run, "_zed_home", tmp_path)
    monkeypatch.delenv("INVOCATION_ID", raising=False)
...
async def test_corrupt_marker_file_is_treated_as_absent(tmp_path, monkeypatch):
    """Malformed JSON in the marker file doesn't crash â€” /restart proceeds."""
    monkeypatch.setattr(gateway_run, "_zed_home", tmp_path)
    monkeypatch.delenv("INVOCATION_ID", raising=False)
...
async def test_event_without_update_id_bypasses_dedup(tmp_path, monkeypatch):
    ...
    # No update_id â€” the dedup check should NOT kick in
...
async def test_different_platform_bypasses_dedup(tmp_path, monkeypatch):
    ...
    # /restart from Discord â€” not a redelivery candidate
