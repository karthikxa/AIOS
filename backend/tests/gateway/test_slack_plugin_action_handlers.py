...
from zed_cli.plugins import (  # noqa: E402
    PluginContext,
    PluginManager,
    PluginManifest,
)
...
# ---------------------------------------------------------------------------
# PluginContext.register_slack_action_handler â€” input validation + queuing
# ---------------------------------------------------------------------------
...
class TestRegisterSlackActionHandlerAPI:
    ...
    def test_regex_action_id_is_accepted(self):
        """slack_bolt accepts re.Pattern matchers â€” so should the plugin API."""
        import re as _re
        mgr, ctx = _make_ctx()
...
def _connect_with_recording_app(
    ...
    *,
    ...
    with patch.object(_slack_mod, "AsyncApp", return_value=mock_app), \
         ...
         patch("zed_cli.plugins.get_plugin_manager", return_value=fake_mgr), \
...
class TestSlackAdapterPluginActionWiring:
    ...
    def test_plugin_exception_does_not_propagate_to_slack(self):
        ...
        # Slack still got an ack â€” best-effort fallback after exception.
        ack.assert_awaited()
    ...
    def test_plugin_loader_failure_does_not_break_connect(self):
        ...
        with patch.object(_slack_mod, "AsyncApp", return_value=mock_app), \
             ...
             patch("gateway.status.acquire_scoped_lock", return_value=(True, None)), \
             patch("gateway.status.release_scoped_lock"), \
             patch("zed_cli.plugins.get_plugin_manager",
