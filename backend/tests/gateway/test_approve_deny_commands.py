...
mechanism â€” the agent thread blocks until the user responds with /approve
...
class TestBareTextNoLongerApproves:
    ...
    async def test_yes_does_not_execute_pending_command(self):
        ...
        # "yes" is not /approve â€” entry should still be pending
...
class TestBlockingApprovalE2E:
    """Test the full blocking flow: agent thread blocks â†’ user approves â†’ agent resumes."""
    ...
    def test_parallel_mixed_approve_deny(self):
        ...
        # _gateway_queues[session_key] â€” poll until we see 2 entries.
...
class TestFallbackNoCallback:
    ...
    def test_no_callback_returns_approval_required(self):
        ...
        os.environ["ZED_EXEC_ASK"] = "1"
        os.environ["ZED_SESSION_KEY"] = "no-callback-test"
        try:
