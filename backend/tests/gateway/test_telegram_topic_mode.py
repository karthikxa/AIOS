...
Telegram topics act as independent Zed session lanes.
"""
...
def _make_runner(session_db=None):
    ...
    # Bypass the destructive-slash confirm gate â€” these tests focus on
...
async def test_group_new_keeps_existing_reset_semantics_when_dm_topic_mode_enabled(
    ...
    assert "Started a new Zed session in this topic" not in result
...
async def test_new_inside_telegram_topic_resets_current_topic_with_parallel_tip(monkeypatch):
    ...
    assert "Started a new Zed session in this topic" in result
...
async def test_topic_binding_follows_compression_tip_on_read(tmp_path, monkeypatch):
    ...
    parent transcript and re-run preflight compression â€” sometimes in a loop.
...
async def test_topic_restore_inside_topic_binds_old_session_and_returns_last_assistant_message(
    ...
    assert "Last Zed message:" in result
...
def test_migration_rebuilds_v1_binding_table_with_cascade_fk(tmp_path):
    """v1 â†’ v2 migration rebuilds the bindings table when FK lacks ON DELETE CASCADE."""
    ...
    # Re-run migration â€” should upgrade to v2 shape.
    db.apply_telegram_topic_migration()
...
async def test_topic_help_subcommand_returns_usage(tmp_path):
    ...
    # No side effects â€” topic mode tables should not even exist yet.
    tables = {
        row[0]
...
def test_recover_returns_none_for_brand_new_topic(tmp_path):
    ...
    # the previous topic's session. The hijack was self-reinforcing â€” because
    ...
    # "99999" is non-lobby and not in the binding table â€” brand-new topic.
...
def test_list_telegram_topic_bindings_for_chat_no_table(tmp_path):
    # Missing topic-mode tables â†’ [] without auto-migrating.
    db = SessionDB(db_path=tmp_path / "state.db")
