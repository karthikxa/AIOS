...
Coverage targets:
  - Backward compat: no ``allow_admin_from`` set â†’ behaves exactly as before
  ...
  - User path: user not in admin list, but command in
    ``user_allowed_commands`` â†’ allowed.
  - User denied: command not in either list â†’ returns the â›” denial.
...
# ---------------------------------------------------------------------------
# /whoami response shape â€” proves the handler is reachable AND uses the
...
# ---------------------------------------------------------------------------
# Gate denial â€” admin-only command attempted by non-admin
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Gate ALLOW â€” admin and listed user
# ---------------------------------------------------------------------------
...
async def test_admin_runs_unlisted_command():
    ...
    # return the â›” denial. The /whoami handler is deterministic and doesn't
...
# ---------------------------------------------------------------------------
# Backward compatibility â€” no admin list set means no gating at all
# ---------------------------------------------------------------------------
...
async def test_group_only_gating_leaves_dm_unrestricted():
    runner = _make_runner(
        platform_extra={
            # Only group has an admin list â†’ DM scope stays in backward-compat mode
            "group_allow_admin_from": ["222"],
        }
    )
...
async def test_plugin_registered_command_is_gated(monkeypatch):
    ...
    from zed_cli import commands as cmd_mod
    ...
    # Non-admin tries to run the plugin command â†’ must be denied by the gate.
    result = await runner._handle_message(
...
# ---------------------------------------------------------------------------
# Running-agent fast-path gating â€” admin/user split must hold even when an
...
# We must apply the gate there too â€” otherwise non-admins could bypass
...
# ---------------------------------------------------------------------------
# Alias resolution â€” /h aliases to /help; the gate must canonicalize before
...
async def test_gate_uses_canonical_name_not_alias():
    ...
    from zed_cli.commands import COMMAND_REGISTRY
...
# ---------------------------------------------------------------------------
# Unknown / unregistered command â€” gate must NOT intercept (let the existing
...
async def test_gate_does_not_intercept_unknown_command():
    ...
    must not produce a denial message â€” the existing unknown-command path
    ...
    # The gate should pass through (no â›”) since canonical resolution
    # returns the raw command and is_gateway_known_command returns False.
    # We can only verify the gate didn't fire â€” downstream behavior may
...
# ---------------------------------------------------------------------------
# Scope independence â€” admin in DM scope is NOT auto-admin in group when
...
async def test_dm_admin_blocked_in_group_with_separate_admin_list():
    ...
    # run group_user_allowed_commands. /restart is not in that list â†’ denied.
...
# ---------------------------------------------------------------------------
# Multi-platform isolation â€” gating on Discord doesn't leak to Telegram.
# ---------------------------------------------------------------------------
...
async def test_gating_isolated_per_platform():
    ...
    # Same user_id on Telegram â†’ must be unrestricted (Telegram has no admin list).
