...
Precedence verified here:
    ZED_MAX_TOKENS env  >  model.max_tokens  >  per-provider
...
def isolated_home(tmp_path, monkeypatch):
    """Isolated ZED_HOME with a writable config.yaml and a clean module cache.

    These tests deliberately re-import ``zed_cli`` / ``gateway`` so each
    ...
    monkeypatch.delenv("ZED_MAX_TOKENS", raising=False)

    _saved = {
        k: v
        for k, v in sys.modules.items()
        if k.startswith(("zed_cli", "gateway"))
    }

    def write_cfg(body: str) -> None:
        (zed_home / "config.yaml").write_text(textwrap.dedent(body))

    def fresh_gateway():
        for mod in list(sys.modules.keys()):
            if mod.startswith(("zed_cli", "gateway")):
                del sys.modules[mod]
...
def test_env_override_beats_everything(isolated_home, monkeypatch):
    """ZED_MAX_TOKENS is the internal override mechanism (highest priority)."""
    write_cfg, fresh_gateway = isolated_home
    monkeypatch.setenv("ZED_MAX_TOKENS", "2048")
    write_cfg(
        """
        model:
          default: glm-5.1
