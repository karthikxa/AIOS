...
class TestFormatMessageCodeBlocks:
    ...
    def test_inline_code_no_double_escape(self, adapter):
        ...
        result = adapter.format_message(text)
        # \\ in input â†’ \\\\ in output (each \ escaped once)
...
class TestFormatMessageBoldItalic:
    ...
    def test_reload_mcp_summary_escapes_dynamic_server_names(self, adapter):
        content = (
            "ðŸ”„ **MCP Servers Reloaded**\n"
            "â™»ï¸ Reconnected: agent_one, tool[beta]\n"
            "âž• Added: alpha*prod\n"
            "ðŸ”§ 3 tool(s) available from 2 server(s)"
        )
...
# =========================================================================
# _strip_mdv2 â€” plaintext fallback
# =========================================================================
...
class TestWrapMarkdownTables:
    ...
    def test_basic_table_rewritten_as_row_groups(self):
        ...
        assert "**Alice**" in out
        # The heading IS the Player cell â€” don't repeat it as a bullet.
        assert "â€¢ Player: Alice" not in out
        assert "â€¢ Score: 150" in out
        assert "**Bob**" in out
        assert "â€¢ Score: 120" in out
    ...
    def test_bare_pipe_table_rewritten(self):
        ...
        # No duplicate first bullet â€” heading 'a' already shows the head1 value.
        assert "â€¢ head1: a" not in out
        assert "â€¢ head2: b" in out
        assert "**c**" in out
    ...
    def test_alignment_separators(self):
        ...
        assert "â€¢ Name: Ada" not in out
        assert "â€¢ Age: 30" in out
        assert "â€¢ City: NYC" in out
    ...
    def test_two_consecutive_tables_rewritten_separately(self):
        ...
        # Headings duplicate first cells (no row-label col) â€” skip those bullets.
        assert "â€¢ A: 1" not in out
        assert "â€¢ X: 9" not in out
        assert "â€¢ B: 2" in out
        assert "â€¢ Y: 8" in out
    ...
    def test_single_column_separator_not_matched(self):
        """Single-column tables (rare) are not detected â€” we require at
    ...
    def test_row_label_column_preserves_first_bullet(self):
        ...
        and is distinct from any header â€” so every headerâ†’value bullet is
        ...
        out = _wrap_markdown_tables(text)
        assert "**Alice**" in out
        # No header to duplicate against â€” both bullets stay.
        assert "â€¢ Score: 150" in out
        assert "â€¢ Rank: 1" in out
        assert "**Alice**\nâ€¢ Score: 150\nâ€¢ Rank: 1" in out
...
class TestFormatMessageTables:
    ...
    def test_table_rendered_as_bullets(self, adapter):
        ...
        assert "*A*" in out
        # Heading 'A' duplicates the Col1 value â€” skip that bullet.
        assert "â€¢ Col1: A" not in out
        assert "â€¢ Col2: B" in out
        assert "```" not in out
        assert "\\|" not in out
    ...
    def test_text_after_table_still_formatted(self, adapter):
        ...
        assert "\\!" in out
        assert "*1*" in out
        # Heading '1' is also the A-column value â€” skip the redundant bullet.
        assert "â€¢ A: 1" not in out
        assert "â€¢ B: 2" in out
...
# =========================================================================
# edit_message â€” streaming Markdown safety
# =========================================================================
...
class TestEditMessageStreamingSafety:
    ...
    async def test_message_too_long_splits_into_continuations_not_silent_truncation(self):
        ...
        the adapter best-effort truncated the content with 'â€¦' and returned
...
def _guest_test_adapter(*, guest_mode=True, require_mention=True, allowed_chats=None):
    ...
    adapter._bot = SimpleNamespace(id=999, username="zed_bot")
    ...
    # gate â€” they're exercising the guest-mode mention/allowed_chats
    # logic that runs after â€” so stub the user authz to always allow.
...
def _guest_mention_entity(text, mention="@zed_bot"):
...
class TestTelegramGuestMentionGating:
    ...
    def test_guest_mode_allows_text_mention_entity_outside_allowed_chats(self):
        """MessageEntity(type=text_mention) tags a user by ID â€” recognised as mention."""
        adapter = _guest_test_adapter(guest_mode=True, allowed_chats=["-100200"])
        message = _guest_group_message(
            "hey there",
            chat_id=-100201,
