...
# â”€â”€ _setup_dm_topics: load persisted thread_ids â”€â”€
...
# â”€â”€ _create_dm_topic: error handling â”€â”€
...
# â”€â”€ _persist_dm_topic_thread_id â”€â”€
...
def test_persist_dm_topic_thread_id_writes_config(tmp_path):
    ...
    config_file = tmp_path / ".zed" / "config.yaml"
...
# â”€â”€ _cache_dm_topic_from_message â”€â”€
...
# â”€â”€ _build_message_event: auto_skill binding â”€â”€
...
# â”€â”€ _build_message_event: group_topics skill binding â”€â”€
...
# telegram_mod.ChatType â€” not telegram_mod.constants.ChatType.  We must use
...
def test_group_topic_unmapped_thread_id():
    """Thread ID not in config should fall through â€” no skill, no topic name."""
    from gateway.platforms.base import MessageType
...
def test_group_topic_no_config():
    """No group_topics config at all should be fine â€” no skill, no topic."""
    from gateway.platforms.base import MessageType
...
def test_group_topic_chat_id_int_string_coercion():
    ...
    adapter = _make_adapter(group_topics_config=[
        {
            ...
            "topics": [
                {"name": "Dev", "thread_id": "7", "skill": "zed-agent-dev"},
            ],
        }
    ])
    ...
    assert event.auto_skill == "zed-agent-dev"
    assert event.source.chat_topic == "Dev"


# â”€â”€ _build_message_event: from_user=None fallback in DMs â”€â”€
...
def test_build_message_event_group_from_user_none_stays_none():
    ...
    # Groups should NOT fall back â€” anonymous senders stay None
...
def test_build_message_event_dm_from_user_present_uses_user():
    ...
    # Normal case â€” from_user is used directly
