...
The judge verdict message ("âœ“ Goal achieved", "â¸ budget exhausted", etc.)
must reach the user after each turn. Before this fix the code checked
``hasattr(adapter, "send_message")`` â€” but adapters expose ``send()``,
...
def _make_runner_with_adapter(session_id: str = None):
    ...
    # module-import time, defeating per-test ZED_HOME monkeypatches).
...
@pytest.mark.asyncio
async def test_goal_verdict_done_sent_via_adapter_send(zed_home):
    """When the judge says done, the 'âœ“ Goal achieved' message must reach
    ...
    with patch("zed_cli.goals.judge_goal", return_value=("done", "the feature shipped", False)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
            final_response="I shipped the feature.",
        )
        # fire-and-forget create_task â€” give the loop a tick
...
@pytest.mark.asyncio
async def test_goal_verdict_continue_enqueues_continuation(zed_home):
    ...
    with patch("zed_cli.goals.judge_goal", return_value=("continue", "still needs work", False)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
@pytest.mark.asyncio
async def test_goal_verdict_budget_exhausted_sends_pause(zed_home):
    """When the budget is exhausted, a 'â¸ Goal paused' message must be sent
    and no further continuation enqueued."""
    runner, adapter, session_entry, src = _make_runner_with_adapter()

    from zed_cli.goals import GoalManager, save_goal
    ...
    with patch("zed_cli.goals.judge_goal", return_value=("continue", "keep going", False)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
@pytest.mark.asyncio
async def test_goal_verdict_skipped_when_no_active_goal(zed_home):
    """No goal set â†’ the hook is a no-op. Nothing is sent, nothing enqueued."""
    runner, adapter, session_entry, src = _make_runner_with_adapter()
...
@pytest.mark.asyncio
async def test_goal_verdict_survives_adapter_without_send(zed_home):
    ...
    with patch("zed_cli.goals.judge_goal", return_value=("done", "ok", False)):
        # must not raise
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
