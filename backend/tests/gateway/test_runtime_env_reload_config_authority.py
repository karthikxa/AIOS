...
ZED_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env ZED_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_keeps_env_max_iterations_when_config_omits_key(
    ...
    zed_home = tmp_path / ".zed"
    zed_home.mkdir()
    (zed_home / "config.yaml").write_text(yaml.safe_dump({"agent": {}}), encoding="utf-8")
    (zed_home / ".env").write_text("ZED_MAX_ITERATIONS=123\n", encoding="utf-8")
    ...
    assert os.environ["ZED_MAX_ITERATIONS"] == "123"


def test_current_max_iterations_reloads_before_reading(monkeypatch) -> None:
    monkeypatch.setenv("ZED_MAX_ITERATIONS", "90")

    def _fake_reload() -> None:
        os.environ["ZED_MAX_ITERATIONS"] = "200"
