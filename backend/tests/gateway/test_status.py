...
class TestGatewayPidState:
    def test_write_pid_file_records_gateway_metadata(self, tmp_path, monkeypatch):
        ...
        assert payload["kind"] == "zed-gateway"
    ...
    def test_get_running_pid_cleans_stale_record_from_dead_process(self, tmp_path, monkeypatch):
        ...
        # able to unlink it so ``write_pid_file``'s O_EXCL create succeeds â€”
    ...
    def test_get_running_pid_accepts_script_style_gateway_cmdline(self, tmp_path, monkeypatch):
        ...
        pid_path.write_text(json.dumps({
            "pid": os.getpid(),
            "kind": "zed-gateway",
            "argv": ["/venv/bin/python", "/repo/zed_cli/main.py", "gateway", "run", "--replace"],
            "start_time": 123,
        }))
        ...
        monkeypatch.setattr(
            ...
            lambda pid: "/venv/bin/python /repo/zed_cli/main.py gateway run --replace",
        )
    ...
    def test_get_running_pid_cleans_stale_metadata_from_dead_foreign_pid(self, tmp_path, monkeypatch):
        ...
        # No live lock holder â†’ get_running_pid should clean both files.
        assert status.get_running_pid() is None
        assert not pid_path.exists()
...
class TestGatewayRuntimeStatus:
    ...
    def test_write_runtime_status_overwrites_stale_argv_on_restart(self, tmp_path, monkeypatch):
        ...
        state_path.write_text(json.dumps({
            ...
            "start_time": 1000.0,
            "kind": "zed-gateway",
            "argv": ["/old/path/zed", "gateway", "run"],
            "platforms": {},
            "updated_at": "2025-01-01T00:00:00Z",
        }))

        monkeypatch.setattr(status.sys, "argv", ["/new/path/zed", "gateway", "run"])
        monkeypatch.setattr(status, "_get_process_start_time", lambda pid: 2000)

        status.write_runtime_status(gateway_state="running")

        payload = status.read_runtime_status()
        assert payload["argv"] == ["/new/path/zed", "gateway", "run"]
        assert payload["pid"] == os.getpid()
...
class TestScopedLocks:
    ...
    def test_acquire_scoped_lock_rejects_live_other_process(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ZED_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
    ...
    def test_acquire_scoped_lock_replaces_pid_reused_by_unrelated_process(self, tmp_path, monkeypatch):
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "zed-gateway",
            "argv": ["/Users/user/.zed/zed-agent/zed_cli/main.py", "gateway", "run", "--replace"],
        }))
    ...
    def test_acquire_scoped_lock_keeps_lock_when_cmdline_unreadable_but_record_is_gateway(self, tmp_path, monkeypatch):
        ...
        lock.  Fall back to the lock record's own argv â€” written by the
        gateway at startup â€” before declaring the lock stale.
        """
        monkeypatch.setenv("ZED_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
        lock_path = tmp_path / "locks" / "telegram-bot-token-2bb80d537b1da3e3.lock"
        lock_path.parent.mkdir(parents=True, exist_ok=True)
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "zed-gateway",
            "argv": ["zed_cli/main.py", "gateway", "run"],
        }))
    ...
    def test_acquire_scoped_lock_replaces_reused_pid_even_with_matching_start_time(self, tmp_path, monkeypatch):
        ...
        but the live process is not a gateway â€” the lock must be evicted.
        """
        monkeypatch.setenv("ZED_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
        lock_path = tmp_path / "locks" / "telegram-bot-token-2bb80d537b1da3e3.lock"
        lock_path.parent.mkdir(parents=True, exist_ok=True)
...
class TestTakeoverMarker:
    ...
    def test_consume_returns_false_for_different_pid(self, tmp_path, monkeypatch):
        ...
        # and isn't relevant to us â€” leaving it around would grief a later
        # legitimate check).
        assert not (tmp_path / ".gateway-takeover.json").exists()
    ...
    def test_clear_takeover_marker_is_idempotent(self, tmp_path, monkeypatch):
        ...
        # Nothing to clear â€” must not raise
    ...
    def test_consume_ignores_marker_for_different_process_and_prevents_stale_grief(
        ...
        # We are not the target â€” must NOT consume as planned
        assert result is False
...
class TestPlannedStopMarker:
    ...
    def test_consume_returns_true_on_windows_when_start_time_unavailable(
        ...
        it would always return False â€” and ``zed gateway stop`` would be
    ...
    def test_consume_still_rejects_foreign_pid_when_start_time_unavailable(
        ...
        a PID check â€” a marker for a different process is never ours.
        """
        monkeypatch.setenv("ZED_HOME", str(tmp_path))
        monkeypatch.setattr(status, "_get_process_start_time", lambda pid: None)
    ...
    def test_consume_still_rejects_start_time_mismatch_when_both_known(
        ...
        still reject â€” otherwise PID reuse could resurrect a stale marker.
        """
        monkeypatch.setenv("ZED_HOME", str(tmp_path))
        monkeypatch.setattr(status, "_get_process_start_time", lambda pid: 100)
...
class TestReadProcessCmdlinePsFallback:
    ...
    def test_proc_cmdline_takes_priority_over_ps(self, monkeypatch):
        ...
        def fake_read_bytes(self):
            calls.append("proc")
            return b"python\x00zed_cli/main.py\x00gateway\x00"

        monkeypatch.setattr(status.Path, "read_bytes", fake_read_bytes)
        result = status._read_process_cmdline(12345)
        assert "zed_cli/main.py" in result
        assert calls == ["proc"]
    ...
    def test_ps_fallback_used_when_proc_returns_empty(self, monkeypatch):
        ...
        monkeypatch.setattr(
            status.subprocess, "run",
            lambda args, **kwargs: SimpleNamespace(returncode=0, stdout="python zed_cli/main.py gateway run\n"),
        )
        result = status._read_process_cmdline(12345)
        assert "zed_cli/main.py" in result
