...
Reported during the TUI v2 blitz retest:
  - typing `@folder:` (and `@folder` with no colon yet) surfaced files
    alongside directories â€” the gateway-side completion lives in
    `tui_gateway/server.py` and was never touched by the earlier fix to
    `zed_cli/commands.py`.
  - typing `@appChrome` required the full `@ui-tui/src/components/appâ€¦`
    path to find the file â€” users expect Cmd-P-style fuzzy basename
...
# â”€â”€ Fuzzy basename matching â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Users shouldn't have to know the full path â€” typing `@appChrome` should
...
def test_fuzzy_at_finds_file_without_directory_prefix(tmp_path, monkeypatch):
    """`@appChrome` â€” with no slash â€” should surface the nested file."""
    monkeypatch.chdir(tmp_path)
...
def test_fuzzy_ranks_exact_before_prefix_before_subseq(tmp_path, monkeypatch):
    ...
    # Exact `server.py` beats `tui_gateway/server.py` (prefix match) â€” both
...
def test_fuzzy_subsequence_catches_sparse_queries(tmp_path, monkeypatch):
    """`@uCo` â†’ `useCompletion.ts` via subsequence, last-resort tier."""
    monkeypatch.chdir(tmp_path)
    _nested_fixture(tmp_path)
...
def test_fuzzy_skipped_when_path_has_slash(tmp_path, monkeypatch):
    ...
    # It should only surface direct children of the named dir â€” not the
...
def test_fuzzy_skipped_when_folder_tag(tmp_path, monkeypatch):
    """`@folder:<name>` still lists directories â€” fuzzy scanner only walks
...
def test_fuzzy_paths_relative_to_cwd_inside_subdir(tmp_path, monkeypatch):
    ...
    completion paths must resolve under that cwd â€” not under the repo root.
    ...
    # Run from `apps/web/` â€” completions should be relative to here, and
