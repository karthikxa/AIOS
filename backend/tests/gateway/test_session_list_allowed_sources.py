...
- The original implementation hardcoded an allow-list of known gateway
  ...
  sources (``acp``, ``webhook``, user-defined ``ZED_SESSION_SOURCE``
  values, newly-added platforms) were silently dropped from the resume
  picker â€” users reported "lots of sessions are missing from browse
  but exist in .zed/sessions."
...
def test_session_list_surfaces_all_user_facing_sources(monkeypatch):
    ...
    # Every human-facing source â€” including previously-hidden acp, webhook,
    # and custom sources â€” must surface in the picker now.
    assert "tg-1" in ids
    assert "tui-1" in ids
    assert "cli-1" in ids
    ...
    assert "custom-1" in ids, "custom ZED_SESSION_SOURCE values were being hidden"
