...
def _mention_entity(text, mention="@zed_bot"):
...
def _bot_command_entity(text, command):
    ...
    client does NOT emit a separate ``mention`` entity â€” the whole span
...
def test_observed_group_context_uses_shared_source_and_prompt_for_later_mentions():
    async def _run():
        ...
        text = "@zed_bot what did Alice say?"
        msg = _group_message(
            text,
            from_user_id=222,
...
def test_observed_group_context_replays_as_current_message_context_not_user_turns():
    ...
    history = [
        ...
        {"role": "user", "content": "[Alice|111]\nAcha que dÃ¡ fazer estoque?", "observed": True},
    ...
    assert "Acha que dÃ¡ fazer estoque?" in api_message
...
def test_observed_group_context_preserves_slash_command_text_for_dispatch():
    ...
    event = MessageEvent(
        ...
        raw_message=_group_message(
            "/new@zed_bot",
            entities=[_bot_command_entity("/new@zed_bot", "/new@zed_bot")],
        ),
    )

    attributed = adapter._apply_telegram_group_observe_attribution(event)

    assert attributed.text == "/new@zed_bot"
...
def test_group_messages_can_require_direct_trigger_via_config():
    ...
    assert adapter._should_process_message(_group_message("hi @zed_bot", entities=[_mention_entity("hi @zed_bot")])) is True
    ...
    assert adapter._should_process_message(
        _group_message(
            "/status@zed_bot",
            entities=[_bot_command_entity("/status@zed_bot", "/status@zed_bot")],
        ),
        is_command=True,
    ) is True
    # A bot_command entity addressed at a different bot must not satisfy
    # the mention gate â€” Telegram groups can host multiple bots that
    ...
    # require_mention=True â€” Telegram delivers it only when the bot's
...
def test_raw_bot_mention_fallback_does_not_match_email_or_substring():
    adapter = _make_adapter(require_mention=True, bot_username="zed_bot")

    assert adapter._should_process_message(_group_message("email ops@zed_bot.example")) is False
    assert adapter._should_process_message(_group_message("prefix@zed_bot hi")) is False
    assert adapter._should_process_message(_group_message("hi @zed_bot")) is True
...
def test_guest_mode_allows_only_direct_mentions_outside_allowed_chats():
    ...
    mentioned = _group_message(
        "hi @zed_bot",
        chat_id=-201,
        entities=[_mention_entity("hi @zed_bot")],
    )
...
def test_guest_mode_mention_dropped_in_ignored_thread():
    """A guest mention in an ignored thread is still dropped â€” thread gate runs first."""
    adapter = _make_adapter(
        require_mention=True,
        allowed_chats=["-200"],
        guest_mode=True,
...
def test_allowed_topics_drop_other_forum_topics_before_other_gates():
    ...
    assert adapter._should_process_message(
        _group_message("hi @zed_bot", chat_id=-100, thread_id=11, entities=[_mention_entity("hi @zed_bot")])
    ) is False
...
def test_top_level_require_mention_bridges_to_telegram(monkeypatch, tmp_path):
    ...
    zed_home = tmp_path / ".zed"
    zed_home.mkdir()
    # Intentionally no "telegram:" section â€” keys are at the top level.
    (zed_home / "config.yaml").write_text(
        "require_mention: true\n"
