...
class FakeTextChannel:
    def __init__(self, channel_id: int = 1, name: str = "general", guild_name: str = "Zed Server"):
        self.id = channel_id
        self.name = name
...
class FakeForumChannel:
    def __init__(self, channel_id: int = 1, name: str = "support-forum", guild_name: str = "Zed Server"):
        self.id = channel_id
        self.name = name
        self.guild = SimpleNamespace(name=guild_name)
        self.type = 15
...
class FakeThread:
    def __init__(self, channel_id: int = 1, name: str = "thread", parent=None, guild_name: str = "Zed Server"):
        self.id = channel_id
        self.name = name
        self.parent = parent
...
async def test_discord_defaults_to_require_mention(adapter, monkeypatch):
    ...
    # Should be ignored â€” no mention, require_mention defaults to true
...
async def test_discord_forum_threads_are_handled_as_threads(adapter, monkeypatch):
    ...
    thread = FakeThread(channel_id=456, name="Can Zed reply here?", parent=forum)
    ...
    assert event.source.thread_id == "456"
    assert event.source.chat_type == "thread"
    assert event.source.chat_name == "Zed Server / support-forum / Can Zed reply here?"
...
def test_discord_free_response_channels_bare_int(adapter, monkeypatch):
    ...
    # returning an empty set.  Scalar â†’ str coercion makes single-channel
...
def test_discord_free_response_channels_int_list(adapter, monkeypatch):
    # YAML list form with bare numeric entries â€” each element should be coerced.
    monkeypatch.delenv("DISCORD_FREE_RESPONSE_CHANNELS", raising=False)
...
async def test_discord_free_response_channel_skips_auto_thread(adapter, monkeypatch):
    ...
    channel into a thread-spawning machine â€” see the docs at
...
async def test_discord_thread_require_mention_gates_followups(adapter, monkeypatch):
    ...
    message = make_message(channel=thread, content="ambient chatter â€” not for me")
    await adapter._handle_message(message)
...
async def test_discord_thread_require_mention_via_config_extra(adapter, monkeypatch):
    ...
    message = make_message(channel=thread, content="ambient â€” should be ignored")
    await adapter._handle_message(message)
...
async def test_fetch_channel_context_skips_self_improvement_boundary_message(adapter, monkeypatch):
    ...
    channel = FakeHistoryChannel(
        [
            ...
            make_history_message(
                author=codex,
                content="â™» Gateway restarted successfully. Your session continues.",
                msg_id=7,
            ),
            make_history_message(
                author=codex,
                content="ðŸ’¾ Self-improvement review: Memory updated",
                msg_id=6,
            ),
            make_history_message(author=human, content="question after reply", msg_id=5),
            make_history_message(
                author=adapter._client.user,
                content="ðŸ’¾ Self-improvement review: Skill 'zed-gateway-display-config' patched",
                msg_id=4,
            ),
...
async def test_fetch_channel_context_hydrates_around_reply_target(adapter, monkeypatch):
    ...
    channel = FakeHistoryChannel(
        [
            ...
            # Older exchange â€” behind the partition, only reachable via reply anchor
...
def test_nonconversational_fallback_requires_self_improvement_emoji():
    assert discord_platform._looks_like_nonconversational_history_message(
        "ðŸ’¾ Self-improvement review: Memory updated"
    )
...
async def test_fetch_channel_context_uses_cache_to_narrow_window(adapter, monkeypatch):
    ...
    # Seed the cache â€” bot's last message in this channel was ID 100
...
async def test_fetch_channel_context_ignores_stale_cache(adapter, monkeypatch):
    ...
    # Cache has a NEWER ID than the trigger â€” stale/invalid
    ...
    assert result == "[Recent channel messages]\n[Alice] hello"
    # Cache should have been ignored â€” after= should be None
...
async def test_discord_dm_does_not_backfill(adapter, monkeypatch):
    """DMs skip backfill â€” every DM triggers the bot, so there's no mention gap."""
    monkeypatch.setenv("DISCORD_REQUIRE_MENTION", "true")
    adapter.config.extra["history_backfill"] = True
    adapter._fetch_channel_context = AsyncMock(return_value="[Recent channel messages]\n[Alice] context")
...
async def test_discord_auto_thread_skips_backfill(adapter, monkeypatch):
    """Auto-created threads skip backfill â€” the thread is brand new with no prior context."""
    monkeypatch.setenv("DISCORD_REQUIRE_MENTION", "true")
...
async def test_discord_non_reply_free_channel_skips_backfill(adapter, monkeypatch):
    ...
    message â€” only replies (and the existing mention-gap / thread cases) should
    hydrate context.
    """
    monkeypatch.setenv("DISCORD_REQUIRE_MENTION", "false")
    monkeypatch.delenv("DISCORD_FREE_RESPONSE_CHANNELS", raising=False)
