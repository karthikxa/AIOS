...
- config.yaml â†’ env var bridging (via load_gateway_config) where applicable.
"""
...
class TestTelegramAllowedChats:
    ...
    def test_env_var_fallback(self, monkeypatch):
        ...
        adapter = _make_telegram_adapter()  # no extra â†’ falls back to env
        assert adapter._telegram_allowed_chats() == {"-100", "-200"}
    ...
    def test_mention_cannot_bypass_whitelist(self):
        ...
        adapter = _make_telegram_adapter(allowed_chats=["-100"])
        msg = _tg_group_message(-999, text="@zed_bot hello")
        msg.entities = [SimpleNamespace(
            type="mention", offset=0, length=len("@zed_bot"),
        )]
    ...
    def test_config_bridge(self, monkeypatch, tmp_path):
        """slack-style config.yaml â†’ env var bridge works."""
...
def _make_dingtalk_adapter(*, allowed_chats=None, require_mention=None):
    # Import lazily â€” DingTalk SDK may not be installed.
...
# ---------------------------------------------------------------------------
# Mattermost (env-var only â€” no config.yaml bridge)
# ---------------------------------------------------------------------------

class TestMattermostAllowedChannels:
    """Mattermost whitelist logic â€” replicated since the adapter reads config
...
class TestMatrixAllowedRooms:
    """Matrix whitelist behavior â€” tested via the env-var-initialized
    ...
    def test_block_logic(self):
        ...
        # Non-allowed room in group (is_dm=False) â†’ blocked
