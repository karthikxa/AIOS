...
async def test_start_gateway_verbosity_imports_redacting_formatter(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("zed_logging.setup_logging", lambda zed_home, mode: tmp_path)
    monkeypatch.setattr("zed_logging._add_rotating_handler", lambda *args, **kwargs: None)
...
async def test_start_gateway_replace_aborts_when_force_killed_pid_still_alive(
    ...
    file / scoped locks and start a fresh instance â€” that leaves two live
    ...
    # _pid_exists never goes False â€” the force-kill did not take.
...
async def test_start_gateway_replace_writes_takeover_marker_before_sigterm(
    ...
    planned takeover (â†’ exit 0) rather than an unexpected kill (â†’ exit 1).
...
async def test_runner_degrades_gracefully_when_all_adapters_missing(monkeypatch, tmp_path, caplog):
    ...
    the gateway should NOT return failure â€” it should warn and continue running for
    ...
    # missing credentials â€” no connection attempt ever made).
    ...
    # Must NOT return False â€” gateway should keep running for cron.
    assert ok is True
    assert runner.should_exit_cleanly is False
    assert runner.adapters == {}
