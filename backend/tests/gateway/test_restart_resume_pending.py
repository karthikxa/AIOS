...
1. When a gateway restart drain times out and agents are force-interrupted,
   the affected sessions are flagged ``resume_pending=True`` â€” not
   ``suspended`` â€” so the next user message on the same session_key
...
2. ``suspended=True`` (from ``/stop`` or stuck-loop escalation) still
   wins over ``resume_pending`` â€” the forced-wipe path is preserved.
...
4. The existing ``.restart_failure_counts`` stuck-loop counter from
   PR #7536 remains the single source of escalation â€” no parallel
...
def _build_agent_history(history: list) -> list:
    """Mirror gateway/run.py's ``history â†’ agent_history`` conversion.
...
def _simulate_note_injection(
    ...
    *,
    ...
    Tests pass the raw ``history`` â€” ``agent_history`` is derived from it
    ...
    if is_resume_pending:
        ...
        message = (
            ...
            f"run â€” do NOT re-execute or verify it. {resume_guidance} "
...
class TestMarkResumePending:
    ...
    def test_does_not_override_suspended(self, tmp_path):
        """suspended wins â€” mark_resume_pending is a no-op on a suspended entry."""
        store = _make_store(tmp_path)
        source = _make_source()
        entry = store.get_or_create_session(source)
...
class TestGetOrCreateResumePending:
    def test_resume_pending_preserves_session_id(self, tmp_path):
        """This is THE core behavioural fix â€” resume_pending â‰  new session."""
        store = _make_store(tmp_path)
        source = _make_source()
        ...
        # Flag is NOT cleared on read â€” only on successful turn completion.
        assert second.resume_pending is True

    def test_suspended_still_creates_new_session(self, tmp_path):
        """Regression guard â€” suspended must still force a clean slate."""
        store = _make_store(tmp_path)
        source = _make_source()
        first = store.get_or_create_session(source)
        original_sid = first.session_id
    ...
    def test_suspended_overrides_resume_pending(self, tmp_path):
        ...
        behave like ``suspended`` â€” forced wipe + auto_reset_reason."""
        store = _make_store(tmp_path)
        source = _make_source()
        first = store.get_or_create_session(source)
        original_sid = first.session_id
...
class TestSuspendRecentlyActiveSkipsResumePending:
    ...
    def test_non_resume_pending_gets_resume_pending(self, tmp_path):
        ...
        count = store.suspend_recently_active()
        # entry_a is already resume_pending â†’ skipped. entry_b gets marked.
        assert count == 1
...
class TestResumePendingSystemNote:
    ...
    def test_resume_pending_subsumes_tool_tail_note(self):
        """When BOTH conditions are true, the restart-resume note wins â€”
        no duplicate notes."""
        entry = self._pending_entry()
        history = [
    ...
    def test_stale_resume_pending_does_not_inject_restart_note(self):
        ...
        The transcript's last row is from an hour ago â€” well outside the
    ...
    def test_stale_tool_tail_does_not_inject_auto_continue_note(self):
        ...
        Uses window_secs=1800 (30 min) to verify the gate fires at 1h â€”
    ...
    def test_stale_tool_tail_with_production_data_shape(self):
        ...
        â€” which is always ``None`` at runtime because the gateway strips
        ``timestamp`` off tool/tool_call rows in ``history â†’ agent_history``.
        ...
        ``_build_agent_history`` conversion, then asserts:
          ...
          1. The stripped ``agent_history`` carries NO timestamp (protects
             ...
             stripped field â€” which would break the API contract).
        ...
        assert "timestamp" not in agent_history[-1], (
            "agent_history tool rows must NOT carry a timestamp â€” the "
    ...
    def test_legacy_history_without_timestamps_still_injects(self):
        ...
        behaviour â€” freshness unknown â†’ treat as fresh."""
        history = [
            {"role": "assistant", "content": None, "tool_calls": [
                {"id": "c1", "function": {"name": "x", "arguments": "{}"}},
            ]},
    ...
    def test_resume_pending_note_warns_against_reexecuting_restart(self):
        ...
        verified â€” the cognitive backstop to the source-level tail strip.
        """
        entry = self._pending_entry(reason="restart_timeout")
        result = _simulate_note_injection(
            history=[
...
class TestFreshnessHelpers:
    ...
    def test_is_fresh_unknown_is_fresh(self):
        """Legacy-compat: unknown timestamp â†’ fresh."""
    ...
    def test_last_transcript_timestamp_row_without_timestamp(self):
        """Legacy transcript row (no timestamp) returns None â†’ caller
        treats as fresh."""
        history = [
    ...
    def test_auto_continue_freshness_window_malformed_falls_back(self, monkeypatch):
        monkeypatch.setenv("ZED_AUTO_CONTINUE_FRESHNESS", "not-a-number")
        assert _auto_continue_freshness_window() == 3600.0

    def test_auto_continue_freshness_window_empty_falls_back(self, monkeypatch):
        monkeypatch.setenv("ZED_AUTO_CONTINUE_FRESHNESS", "")
        assert _auto_continue_freshness_window() == 3600.0
...
async def test_drain_timeout_skips_pending_sentinel_sessions():
    """Pending sentinels â€” sessions whose AIAgent construction hasn't
    produced a real agent yet â€” are skipped by
...
async def test_startup_auto_resume_schedules_fresh_pending_sessions():
    ...
    # Text is empty â€” the existing _is_resume_pending branch in
...
async def test_startup_auto_resume_skips_disallowed_reasons():
    ...
    existing _is_resume_pending branch â€” we just don't synthesize a turn
    for them at startup.
    """
    runner, adapter = make_restart_runner()
...
async def test_reconnect_reschedules_pending_after_late_platform_connect():
    ...
    user sent a fresh message â€” the documented startup auto-resume silently
    ...
    # Platform was not connected at gateway startup â†’ session skipped.
    runner.adapters = {}
    assert runner._schedule_resume_pending_sessions() == 0
    adapter.handle_message.assert_not_called()

    # Platform reconnects â†’ its pending session is retried.
...
async def test_auto_resume_skips_sessions_with_running_agent():
    ...
    again â€” guards against a double resume when a platform reconnects while a
...
async def test_restart_banner_uses_try_to_resume_wording():
    ...
    â€” the session-continuity fix is best-effort (stuck-loop counter can
...
async def test_restart_notifies_home_channel_even_without_active_sessions():
    ...
    assert adapter.sent == [
        "âš ï¸ Gateway restarting â€” Your current task will be interrupted. "
...
class TestStuckLoopEscalation:
    ...
    single source of terminal escalation â€” no parallel counter on
    ...
    def test_escalation_via_stuck_loop_counter_overrides_resume_pending(
        ...
        monkeypatch.setattr("gateway.run._zed_home", tmp_path)
...
async def test_auto_resume_sets_sentinel_before_task_execution():
    ...
    assert scheduled == 1
    # The sentinel must be set immediately â€” before the task starts executing.
...
async def test_auto_resume_runs_agent_exactly_once_through_full_path():
    ...
    only prove the sentinel is set/cleaned around a stub â€” they never
    ...
    must run once â€” never zero (regression) and never twice (the bug
    the fix targets).
    """
    runner, adapter = make_restart_runner()
    ...
    # Exactly one agent run for the resumed session â€” not zero (the
