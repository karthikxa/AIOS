"""Regression tests for #35809 â€” compression-exhaustion auto-reset loop.
...
(#20470 / #29712 / #33414):
    ...
    2. ``reset_session`` swaps in a clean, parentless session â€” but its return
    ...
    3. On the next inbound message in that topic, the binding-heal walk
       ...
       compression exhaustion re-fired â€” a new session id every loop.
...
* ``TestAutoResetBlockReSyncsBinding`` â€” an AST invariant on
...
* ``TestAutoResetLoadsCleanContext`` â€” a behavioral contract on the real
...
def _find_compression_exhausted_reset_block() -> ast.If:
    ...
    raise AssertionError(
        ...
        "in gateway/run.py â€” the structure changed or the AST walker is stale."
    )


class TestAutoResetBlockReSyncsBinding:
    def test_reset_session_return_is_captured(self):
        """``reset_session`` must be assigned, not called-and-discarded â€”
...
class TestAutoResetLoadsCleanContext:
    ...
    session_id â€” never the bloated compressed-child transcript."""
    ...
    def test_next_turn_transcript_is_empty_after_auto_reset(self, tmp_path):
        ...
        assert loaded == [], (
            ...
            f"messages â€” the bloated compressed child leaked into the new session."
        )
