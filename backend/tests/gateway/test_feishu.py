...
class TestFeishuAdapterMessaging(unittest.TestCase):
    ...
    def test_edit_message_updates_existing_feishu_message(self):
        ...
        with patch("gateway.platforms.feishu.asyncio.to_thread", side_effect=_direct):
            result = asyncio.run(
                adapter.edit_message(
                    ...
                    content="ðŸ“– read_file: \"/tmp/image.png\"",
                )
            )
        ...
        self.assertEqual(
            ...
            json.dumps({"text": "ðŸ“– read_file: \"/tmp/image.png\""}, ensure_ascii=False),
        )
    ...
    def test_edit_message_falls_back_to_text_when_post_update_is_rejected(self):
        ...
        self.assertEqual(
            ...
            json.dumps({"text": "å¯ä»¥ç”¨ ç²—ä½“ å’Œ æ–œä½“ã€‚"}, ensure_ascii=False),
        )
    ...
    def test_get_chat_info_uses_real_feishu_chat_api(self):
        ...
        class _ChatAPI:
            def get(self, request):
                ...
                return SimpleNamespace(
                    success=lambda: True,
                    data=SimpleNamespace(name="Zed Group", chat_type="group"),
                )
        ...
        self.assertEqual(info["name"], "Zed Group")
...
class TestAdapterBehavior(unittest.TestCase):
    ...
    def test_group_message_allowlist_and_mention_both_required(self):
        ...
        adapter = FeishuAdapter(PlatformConfig())
        # Mention without IDs â€” name fallback legitimately engages.
    ...
    def test_group_message_matches_bot_name_when_only_name_available(self):
        ...
        the mention and the bot carry open_ids, IDs are authoritative â€” a
    ...
    @patch.dict(
        os.environ,
        {
            "ZED_FEISHU_TEXT_BATCH_MAX_MESSAGES": "2",
        },
        clear=True,
    )
    ...
    def test_media_batch_merges_rapid_photo_messages(self):
        ...
        self.assertIn("ç¬¬äºŒå¼ ", event.text)
    ...
    def test_download_remote_document_reads_response_before_httpx_client_closes(self):
        """#18451 â€” snapshot Content-Type + body while the httpx.AsyncClient
        ...
        with tempfile.TemporaryDirectory() as tmp:
            with patch.dict(os.environ, {"ZED_HOME": tmp}, clear=False):
                adapter = FeishuAdapter(PlatformConfig())
        ...
        # content_read MUST happen before client_exit â€” otherwise we're
    ...
    def test_dedup_state_persists_across_adapter_restart(self):
        ...
        with tempfile.TemporaryDirectory() as temp_home:
            with patch.dict(os.environ, {"ZED_HOME": temp_home}, clear=False):
    ...
    def test_process_inbound_message_fetches_reply_to_text(self):
        ...
        adapter._fetch_message_text = AsyncMock(return_value="çˆ¶æ¶ˆæ¯å†…å®¹")
        message = SimpleNamespace(
            chat_id="oc_chat",
            thread_id=None,
            parent_id="om_parent",
        ...
        self.assertEqual(event.reply_to_text, "çˆ¶æ¶ˆæ¯å†…å®¹")
    ...
    def test_send_document_with_caption_uses_single_post_message(self):
        ...
        try:
            with patch("gateway.platforms.feishu.asyncio.to_thread", side_effect=_direct):
                result = asyncio.run(
                    adapter.send_document(chat_id="oc_chat", file_path=file_path, caption="æŠ¥å‘Šè¯·çœ‹")
                )
        finally:
            os.unlink(file_path)
        ...
        self.assertIn("æŠ¥å‘Šè¯·çœ‹", captured["message_request"].request_body.content)
    ...
    def test_send_image_file_with_caption_uses_single_post_message(self):
        ...
        try:
            with patch("gateway.platforms.feishu.asyncio.to_thread", side_effect=_direct):
                result = asyncio.run(
                    adapter.send_image_file(chat_id="oc_chat", image_path=image_path, caption="æˆªå›¾è¯´æ˜Ž")
                )
        finally:
            os.unlink(image_path)
        ...
        self.assertIn("æˆªå›¾è¯´æ˜Ž", captured["message_request"].request_body.content)
    ...
    def test_build_post_payload_wraps_markdown_in_md_tag(self):
        ...
        adapter = FeishuAdapter(PlatformConfig())
        payload = json.loads(
            adapter._build_post_payload("æ”¯æŒ **ç²—ä½“**ã€*æ–œä½“* å’Œ `ä»£ç `")
        )
        ...
        self.assertEqual(
            elements,
            [
                {"tag": "md", "text": "æ”¯æŒ **ç²—ä½“**ã€*æ–œä½“* å’Œ `ä»£ç `"},
            ],
        )
    ...
    def test_send_uses_post_for_inline_markdown(self):
        ...
        elements = payload["zh_cn"]["content"][0]
        self.assertEqual(elements, [{"tag": "md", "text": "å¯ä»¥ç”¨ **ç²—ä½“** å’Œ *æ–œä½“*ã€‚"}])
    ...
    def test_send_splits_fenced_code_blocks_into_separate_post_rows(self):
        ...
        content = (
            "ç¡®è®¤å·²å…¥åº“ âœ“\n"
            "æ–‡ä»¶è·¯å¾„ï¼š`/root/.zed/profiles/agent_cto/cron/jobs.json`\n"
            "**è§£ç åŽçš„å†…å®¹ï¼š**\n"
            "```json\n"
            '{"cron": "list"}\n'
            "```\n"
            "åŽç»­è¯´æ˜Žä»åº”ä¿ç•™ã€‚"
        )
        ...
        self.assertEqual(
            ...
            [
                ...
                [{"tag": "md", "text": "```json\n{\"cron\": \"list\"}\n```"}],
                [{"tag": "md", "text": "åŽç»­è¯´æ˜Žä»åº”ä¿ç•™ã€‚"}],
            ],
        )
...
class TestHydrateBotIdentity(unittest.TestCase):
    ...
    ``FEISHU_BOT_NAME`` are not configured â€” hydration populates them so
    ...
    def test_hydration_populates_open_id_from_bot_info(self):
        ...
        self.assertEqual(adapter._bot_open_id, "ou_zed_hydrated")
        self.assertEqual(adapter._bot_name, "Zed Bot")

    @patch.dict(
        os.environ,
        {
            "FEISHU_BOT_OPEN_ID": "ou_env",
            "FEISHU_BOT_NAME": "Env Zed",
        },
        clear=True,
    )
    def test_hydration_refreshes_env_values_when_bot_info_available(self):
        ...
        self.assertEqual(adapter._bot_name, "Hydrated Zed")
    ...
    def test_hydration_overwrites_stale_env_open_id(self):
        ...
        self.assertEqual(adapter._bot_name, "Zed Bot")  # filled in
    ...
    def test_hydration_preserves_env_values_when_bot_info_probe_fails(self):
        ...
        self.assertEqual(adapter._bot_name, "Env Zed")
    ...
    def test_hydration_tolerates_probe_failure_and_falls_back_to_app_info(self):
        ...
        # Primary probe failed â€” open_id stays empty, but bot_name came from app-info.
...
class TestDedupTTL(unittest.TestCase):
    ...
    def test_load_tolerates_malformed_timestamp_values(self):
        """Regression #13632 â€” a non-numeric timestamp in the persisted
        ...
        with tempfile.TemporaryDirectory() as temp_home:
            with patch.dict(os.environ, {"ZED_HOME": temp_home}, clear=True):
                adapter = FeishuAdapter(PlatformConfig())
...
class TestBotNameResolution(unittest.TestCase):
    ...
    def test_bot_absent_from_response_is_not_cached(self):
        """Bot not in ``data.bots`` (e.g. landed in ``failed_bots``) â†’ no
    ...
    def test_empty_name_in_response_is_negative_cached(self):
        """API returns name="" â†’ cache "" so repeat lookups short-circuit."""
...
class TestProcessingReactions(unittest.TestCase):
    """Typing on start â†’ removed on SUCCESS, swapped for CrossMark on FAILURE,
    ...
    def test_delete_failure_on_failure_outcome_skips_cross_mark(self):
        # Removing Typing is best-effort â€” but if it fails, we must NOT
...
class TestFeishuMentionMap(unittest.TestCase):
    def test_build_mentions_map_handles_at_all(self):
        ...
        result = _build_mentions_map(
            [mention],
            _FeishuBotIdentity(open_id="ou_bot", name="Zed"),
        )
    ...
    def test_build_mentions_map_marks_self_by_name_fallback(self):
        ...
        result = _build_mentions_map([mention], _FeishuBotIdentity(name="Zed"))
    ...
    def test_build_mentions_map_name_match_does_not_override_mismatching_open_id(self):
        ...
        result = _build_mentions_map(
            ...
            _FeishuBotIdentity(open_id="ou_bot", name="Zed Bot"),
        )
    ...
    def test_build_mentions_map_falls_back_to_name_when_bot_open_id_not_hydrated(self):
        ...
        carrying a real open_id should still match via name â€” otherwise
        ...
        result = _build_mentions_map(
            [bot_mention],
            _FeishuBotIdentity(open_id="", name="Zed Bot"),
        )
        self.assertTrue(result["@_user_1"].is_self)
...
class TestFeishuMentionHint(unittest.TestCase):
    ...
    def test_hint_filters_self_mentions(self):
        ...
        refs = [
            FeishuMentionRef(name="Zed", open_id="ou_bot", is_self=True),
            FeishuMentionRef(name="Alice", open_id="ou_alice"),
        ]
        self.assertEqual(
            _build_mention_hint(refs),
    ...
    def test_hint_returns_empty_when_only_self(self):
        ...
        refs = [FeishuMentionRef(name="Zed", open_id="ou_bot", is_self=True)]
...
class TestFeishuStripLeadingSelf(unittest.TestCase):
    def _make_refs(self, *, self_name="Zed", other_name=None):
    ...
    def test_strips_leading_self(self):
        ...
        result = _strip_edge_self_mentions("@Zed /help", self._make_refs())
        self.assertEqual(result, "/help")

    def test_strips_consecutive_leading_self(self):
        from gateway.platforms.feishu import _strip_edge_self_mentions

        result = _strip_edge_self_mentions("@Zed @Zed hi", self._make_refs())
    ...
    def test_stops_at_first_non_self_token(self):
        ...
        result = _strip_edge_self_mentions(
            "@Zed @Alice make a group", self._make_refs(other_name="Alice")
        )
    ...
    def test_preserves_mid_text_self(self):
        ...
        result = _strip_edge_self_mentions("check @Zed said yesterday", self._make_refs())
        self.assertEqual(result, "check @Zed said yesterday")

    def test_strips_trailing_self_at_end_of_text(self):
        from gateway.platforms.feishu import _strip_edge_self_mentions

        result = _strip_edge_self_mentions("look up docs @Zed", self._make_refs())
        self.assertEqual(result, "look up docs")

    def test_strips_trailing_self_with_terminal_punct(self):
        from gateway.platforms.feishu import _strip_edge_self_mentions

        # Terminal punct after the mention â€” strip the mention, keep the punct.
        result = _strip_edge_self_mentions("look up docs @Zed.", self._make_refs())
        self.assertEqual(result, "look up docs.")
    ...
    def test_preserves_trailing_self_before_non_terminal_char(self):
        ...
        # Non-terminal char (here a Chinese particle) follows â€” preserve.
        result = _strip_edge_self_mentions(
            "please don't @Zed anymore", self._make_refs()
        )
        self.assertEqual(result, "please don't @Zed anymore")

    def test_returns_input_when_refs_empty(self):
        from gateway.platforms.feishu import _strip_edge_self_mentions

        self.assertEqual(_strip_edge_self_mentions("@Zed /help", []), "@Zed /help")
...
class TestFeishuNormalizeText(unittest.TestCase):
    ...
    def test_renders_self_mention_with_name(self):
        ...
        refs = {"@_user_1": FeishuMentionRef(name="Zed", open_id="ou_bot", is_self=True)}
        self.assertEqual(
            _normalize_feishu_text("stop pinging @_user_1 please", refs),
            "stop pinging @Zed please",
        )
    ...
    def test_mention_for_missing_map_entry_degrades_to_space(self):
        ...
        # @_user_2 has no entry â€” should degrade to a space (legacy behavior)
        self.assertEqual(
...
class TestFeishuNormalizeWithMentions(unittest.TestCase):
    ...
    def test_text_message_marks_bot_self_mention(self):
        ...
        # self mention is still rendered â€” strip is a separate adapter-level pass
        self.assertEqual(normalized.text_content, "@Zed /help")
    ...
    def test_text_message_at_all_not_synthesized_if_absent_from_text(self):
        """No @_all in text â†’ no synthetic ref even if mentions_map is empty."""
        from gateway.platforms.feishu import normalize_feishu_message
    ...
    def test_post_message_marks_self_via_mentions_map_lookup(self):
        ...
        raw = json.dumps({
            "en_us": {
                "content": [
                    [
                        {"tag": "at", "user_id": "@_user_1", "user_name": "Zed"},
...
class TestFeishuProcessInboundMessage(unittest.TestCase):
    ...
    def test_mid_text_self_mention_preserved(self):
        ...
        event = adapter._dispatch_inbound_event.call_args.args[0]
        self.assertEqual(event.text, "stop pinging @Zed please")

    def test_pure_self_mention_message_is_ignored(self):
        """A message containing only '@Bot' (no body, no media) must not dispatch.

        Regression guard: the rendered '@Zed' slips past the pre-strip empty
...
class TestFeishuFetchMessageText(unittest.TestCase):
    ...
    def test_fetch_message_text_renders_mentions_without_hint_prefix(self):
        ...
        # No [Mentioned:] wrapper â€” reply-context path intentionally skips the hint.
    ...
    def test_fetch_message_text_marks_is_self_via_string_id_shape(self):
        ...
        adapter = self._build_adapter()
        # bot_name is empty â€” is_self must be detected via open_id alone
        adapter._bot_name = ""
        ...
        result = asyncio.run(adapter._fetch_message_text("m_parent"))
        self.assertEqual(result, "@Zed hi")
    ...
    def test_build_mentions_map_string_id_shape(self):
        ...
        detection â€” inbound mention payloads always include an open_id."""
        from gateway.platforms.feishu import _build_mentions_map, _FeishuBotIdentity
        ...
        bot_oid = SimpleNamespace(key="@_user_3", id="ou_bot", id_type="open_id", name="Zed")
        self.assertTrue(
...
class TestFeishuMentionEndToEnd(unittest.TestCase):
    """High-level scenarios from the design spec â€” verify the full pipeline."""
    ...
    def test_scenario_bot_plus_alice_plus_bob_build_group(self):
        ...
        event = self._run(
            ...
            [
                {"key": "@_user_1", "open_id": "ou_bot", "name": "Zed"},
        ...
        self.assertNotIn("@Zed", event.text)
    ...
    def test_scenario_trailing_self_mention_stripped(self):
        """Trailing @bot at the end of a message is routing noise, not content â€”
    ...
    def test_scenario_mid_text_self_mention_preserved(self):
        ...
        character) is meaningful content â€” preserve it."""
        adapter = self._build_adapter()
        event = self._run(
            adapter,
        ...
        self.assertEqual(event.text, "please don't @Zed anymore")
    ...
    def test_scenario_post_bot_plus_alice_filters_self_from_hint(self):
        ...
        self.assertIn("[Mentioned: Alice (open_id=ou_alice)]", event.text)
        self.assertNotIn("Zed (open_id=", event.text)
        # Body: leading @Zed stripped, Alice preserved, trailing text intact.
        self.assertIn("@Alice review the spec with Alice", event.text)
        self.assertNotIn("@Zed @Alice", event.text)
