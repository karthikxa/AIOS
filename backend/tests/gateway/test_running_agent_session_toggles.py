...
slash commands with "â³ Agent is running â€” /{cmd} can't run mid-turn"
(PR #12334). A small allowlist bypasses that and actually dispatches:

  * /yolo â€” toggles the session yolo flag; useful to pre-approve a
    pending approval prompt without waiting for the agent to finish.
  * /verbose â€” cycles the per-platform tool-progress display mode;
    affects the ongoing stream.

Commands whose handlers say "takes effect on next message" stay on the
catch-all by design:

  * /fast â€” writes config.yaml only
  * /reasoning â€” writes config.yaml only
...
def _make_runner():
    ...
    # compares seconds_since_activity against ZED_AGENT_TIMEOUT. Return a
...
async def test_yolo_dispatches_mid_run(monkeypatch):
    ...
    runner = _make_runner()
    runner._handle_yolo_command = AsyncMock(return_value="âš¡ YOLO mode **ON** for this session")

    result = await runner._handle_message(_make_event("/yolo"))

    runner._handle_yolo_command.assert_awaited_once()
    assert result == "âš¡ YOLO mode **ON** for this session"
...
async def test_fast_rejected_mid_run():
    """/fast mid-run must hit the busy catch-all â€” config-only, next message."""
    runner = _make_runner()
...
async def test_reasoning_rejected_mid_run():
    """/reasoning mid-run must hit the busy catch-all â€” config-only, next message."""
    runner = _make_runner()
...
async def test_btw_dispatches_mid_run():
    ...
    /btw is an alias of /background (see zed_cli/commands.py). Typing
    /btw mid-turn must spawn a parallel background task â€” that's the whole
    point of the command. Before the mid-turn bypass was added for
    /background, /btw fell through to the "Agent is running â€” wait or
    ...
    runner._handle_background_command = AsyncMock(
        return_value='ðŸš€ Background task started: "what module owns titles?"'
    )
