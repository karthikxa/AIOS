...
def _ensure_telegram_mock():
    ...
    if "telegram" in sys.modules and hasattr(sys.modules["telegram"], "__file__"):
        # Real library is installed â€” no mocking needed
        return
...
def _make_message(document=None, caption=None, media_group_id=None, photo=None):
    ...
    msg.caption = caption
    msg.date = None
    # Media flags â€” all None except explicit payload
    msg.photo = photo
    msg.video = None
    msg.audio = None
    msg.voice = None
...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document/video cache to tmp_path so tests don't touch ~/.zed."""
    monkeypatch.setattr(
...
class TestDocumentDownloadBlock:
    ...
    async def test_unicode_decode_error_handled(self, adapter):
        """Binary bytes that aren't valid UTF-8 in a .txt â€” content not injected but file still cached."""
        ...
        assert len(event.media_urls) == 1
        assert os.path.exists(event.media_urls[0])
        # Content NOT injected â€” text should be empty (no caption set)
...
# ---------------------------------------------------------------------------
# TestMediaGroups â€” media group (album) buffering
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# TestSendVoice â€” outbound audio delivery
# ---------------------------------------------------------------------------
...
class TestSendVoice:
    ...
    async def test_flac_falls_back_to_document(self, connected_adapter, tmp_path):
        """Telegram sendAudio does not accept FLAC â€” must fall back to sendDocument."""
    ...
    async def test_wav_falls_back_to_document(self, connected_adapter, tmp_path):
        """Telegram sendAudio does not accept WAV â€” must fall back to sendDocument."""
...
# ---------------------------------------------------------------------------
# TestSendDocument â€” outbound file attachment delivery
# ---------------------------------------------------------------------------

class TestSendDocument:
    """Tests for TelegramAdapter.send_document() â€” sending files to users."""
...
# ---------------------------------------------------------------------------
# TestSendVideo â€” outbound video delivery
# ---------------------------------------------------------------------------

class TestSendVideo:
    """Tests for TelegramAdapter.send_video() â€” sending videos to users."""
