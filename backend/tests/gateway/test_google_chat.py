...
# companion modules â€” it does not set ``__package__`` on the loaded
...
# (which targets bare ``import adapter`` / ``from adapter import â€¦`` and
...
def adapter(tmp_path):
    ...
    ~/.zed/google_chat_thread_counts.json.
    """
    ...
    # Replace the production store (which would write to ~/.zed/...)
...
# ===========================================================================
# _on_pubsub_message â€” event routing
# ===========================================================================
...
class TestOnPubsubMessage:
    ...
    def test_relay_flat_bot_sender_is_filtered_end_to_end(self, adapter):
        ...
        flows through ``_extract_message_payload`` â†’ ``_on_pubsub_message``
...
class TestExtractMessagePayload:
    ...
    def test_native_chat_api_format_extracts_msg_and_space(self):
        ...
        wrapper â€” events arrive directly from the Chat API publisher.
        """
        envelope = {
            "type": "MESSAGE",
            "message": {
    ...
    def test_native_chat_api_format_drops_non_message_events(self):
        """Format 2 with ``type != MESSAGE`` returns None â€” caller acks."""
        envelope = {
            "type": "ADDED_TO_SPACE",
...
# ===========================================================================
# _build_message_event â€” payload parsing
# ===========================================================================
...
class TestBuildMessageEvent:
    ...
    async def test_dm_first_message_in_thread_is_main_flow(self, adapter):
        ...
        is treated as 'main flow' â€” thread_id is NOT propagated to the
        ...
        # First message in this thread â†’ main-flow â†’ no thread_id on source.
    ...
    async def test_dm_second_message_in_same_thread_is_side_thread(self, adapter):
        ...
        Without this isolation the bug RamÃ³n reported reappears: he
        opens a new thread, says 'Hola!', asks 'dime los mensajes
        anteriores' and the bot answers with messages from OTHER
        threads â€” because all DM threads were sharing one session."""
        ...
        # Second time same thread = user re-engaged â†’ isolated session.
    ...
    async def test_dm_side_thread_caches_thread_for_outbound(self, adapter):
        ...
        # First message â†’ main flow (cache stays clear).
        ...
        # Second message in same thread â†’ side thread â†’ cache populated.
    ...
    async def test_dm_main_flow_after_side_thread_clears_cache(self, adapter):
        ...
        # Two messages in T_side â†’ side thread, cache populated.
        for _ in range(2):
        ...
        # User types in input box: NEW thread T_new (count goes 0â†’1, main flow).
    ...
    async def test_dm_different_top_level_threads_share_session(self, adapter):
        """Three separate top-level user messages â†’ three different
    ...
    async def test_group_keeps_thread_id_on_source(self, adapter):
        """In group spaces, threads are real conversational containers â€”
...
# ===========================================================================
# send() â€” text, patch-in-place, chunking, error handling
# ===========================================================================
...
class TestSend:
    ...
    async def test_create_message_passes_messageReplyOption_when_thread_set(self, adapter):
        ...
        # Capture the kwargs handed to .create() â€” this is what hits
    ...
    async def test_create_message_omits_messageReplyOption_when_no_thread(self, adapter):
        """No thread.name in body â†’ no messageReplyOption needed.
...
class TestTypingLifecycle:
    ...
    async def test_send_typing_inherits_inbound_thread(self, adapter):
        ...
        messages.patch cannot change thread â€” it's immutable). Regression
    ...
    async def test_send_typing_no_thread_when_cache_empty(self, adapter):
        ...
        without thread (Chat will assign a default). Defensive â€” first
    ...
    async def test_send_typing_concurrent_calls_create_only_one_card(self, adapter):
        ...
        guard the second call would create a duplicate card â†’ orphan
        'Zed is thinkingâ€¦' stuck in chat. Race fix via
    ...
    async def test_send_typing_survives_caller_cancellation(self, adapter):
        ...
        longer than 1.5s, wait_for cancels the awaiter â€” but the create
...
# ===========================================================================
# edit_message / delete_message â€” required by gateway tool-progress + streaming
# ===========================================================================
...
class TestEditMessage:
    ...
    async def test_edit_message_overrides_base_so_progress_pipeline_runs(self, adapter):
        ...
        ever shown to the user â€” so this test guards against a future
        accidental removal."""
...
class TestNativeAttachmentDelivery:
    ...
    async def test_send_file_posts_setup_notice_when_no_user_oauth(self, adapter, tmp_path):
        ...
        result = await adapter._send_file(
            "spaces/S", str(f), caption="AquÃ­ va el PDF",
    ...
    async def test_send_file_two_step_native_upload_when_user_oauth_ready(self, adapter, tmp_path):
        ...
        messages.create with the attachmentDataRef â€” both via the
...
class TestUserOAuthHelper:
    ...
    def test_scopes_are_minimal(self):
        """The OAuth flow should request ONLY chat.messages.create â€” no
    ...
    def test_load_user_credentials_per_email_returns_none_when_missing(
        ...
        ``None`` and never throws â€” same contract as the legacy path."""
        monkeypatch.setenv("ZED_HOME", str(tmp_path))
        from plugins.platforms.google_chat.oauth import load_user_credentials
    ...
    def test_pending_auth_path_is_per_user_when_email_given(
        ...
        clobber each other's PKCE verifier â€” the pending state file
...
class TestPerUserAttachmentRouting:
    ...
    async def test_send_file_uses_per_user_token_when_sender_known(
        ...
        """sender_email maps to a per-user file â†’ that user's API client
    ...
    async def test_send_file_falls_back_to_legacy_when_per_user_missing(
        ...
        """sender known but no per-user token â†’ legacy creds fill in.
        ...
        # Cache untouched â€” the per-user slot stays empty so the next
    ...
    async def test_send_file_no_creds_anywhere_posts_setup_notice(
        ...
        """Sender unknown AND no legacy fallback â†’ setup-instructions
...
class TestThreadCountStore:
    ...
    def test_corrupt_json_treated_as_empty(self, tmp_path):
        """A garbage file shouldn't crash the adapter â€” log warn, treat
    ...
    def test_incr_returns_pre_increment_value(self, tmp_path):
        """The PRE-increment count is the heuristic input â€” it answers
    ...
    async def test_outbound_thread_tracked_for_user_reply_in_bot_thread(self, adapter):
        """The bug RamÃ³n hit on the live mac-mini: when the bot replies
        ...
        # Stub _create_message's underlying create call â€” we want to
        ...
        # Bot sends a top-level reply (no thread.name in body â€” main flow).
        await adapter._create_message("spaces/S", {"text": "hola"})
        ...
        # Now user clicks "Reply in thread" on the bot's message â†’
    ...
    async def test_side_thread_detection_survives_restart(self, adapter, tmp_path):
        """End-to-end regression for the bug RamÃ³n hit across 4
        ...
        Flow:
          ...
          4. Adapter must STILL classify it as side thread (isolated
             session + outbound thread) â€” otherwise main-flow context
             leaks in.
        """
...
class TestAttachmentSSRFGuard:
    ...
    async def test_drive_picker_only_skipped_when_no_resource_name(self, adapter):
        ...
        cannot be downloaded with bot SA â€” skip silently."""
        attachment = {
            "source": "DRIVE_FILE",
    ...
    async def test_drive_file_with_resource_name_uses_bot_path(self, adapter, tmp_path, monkeypatch):
        ...
        come with attachmentDataRef.resourceName â€” bot media.download_media
        ...
        # â€” return some bytes directly, no need to walk the full
...
class TestOutboundRetry:
    ...
    async def test_retries_on_503_then_succeeds(self, adapter, monkeypatch):
        ...
        # Two execute() calls â€” initial + one retry.
    ...
    async def test_gives_up_after_max_attempts(self, adapter, monkeypatch):
        ...
        # _RETRY_MAX_ATTEMPTS = 3 â†’ 3 calls total.
        assert execute.execute.call_count == 3

    @pytest.mark.asyncio
    async def test_does_not_retry_on_400(self, adapter, monkeypatch):
        """A 400 (client error) is permanent â€” no retry, fails immediately."""
        from plugins.platforms.google_chat import adapter as gc_mod
        async def _no_sleep(*_a, **_kw):
            return None
        monkeypatch.setattr(gc_mod.asyncio, "sleep", _no_sleep)
        ...
        with pytest.raises(_FakeHttpError):
            await adapter._create_message("spaces/S", {"text": "hi"})
        # Only one attempt â€” 400 is not retryable.
...
class TestFormatMessage:
    """Markdownâ†’Chat dialect conversion + invisible Unicode stripping.
    ...
    def test_header_to_bold_at_line_start_only(self):
        """# Title â†’ *Title* but only at line-start; mid-line `#` untouched."""
    ...
    def test_url_with_parens_in_path(self):
        """`[txt](https://x.com/foo(bar))` â€” pin the documented limitation.
    ...
    def test_mixed_bold_italic_orderings(self):
        """**bold** _italic_ in the same line â€” both surface conversions."""
    ...
    def test_strips_zwj_and_variation_selector(self):
        ...
        These appear in composite emoji like ðŸ‘¨â€ðŸ‘©â€ðŸ‘§ (family) â€” Chat's
        ...
        src = "hello \U0001f468â€\U0001f469â€\U0001f467 world"
        out = GoogleChatAdapter.format_message(src)
        assert "â€" not in out  # ZWJ gone
    ...
    def test_strips_bom_and_bidi_marks(self):
        """BOM, LTR/RTL marks stripped â€” they break Chat's font rendering."""
        src = "ï»¿ hello â€Ž world â€"
        out = GoogleChatAdapter.format_message(src)
        assert "ï»¿" not in out
        assert "â€Ž" not in out
        assert "â€" not in out
    ...
    def test_empty_and_none_safe(self):
        ...
        The double-space collapser runs on every non-empty input â€” that's
...
class TestADCFallback:
    ...
    def test_load_credentials_uses_adc_when_no_sa_path(self, adapter, monkeypatch):
        """No SA path â†’ google.auth.default() is called."""
    ...
    def test_interactive_setup_uses_shared_cli_prompt_helpers(self, monkeypatch):
        ...
        answers = {
            ...
            "Pub/Sub subscription (projects/<proj>/subscriptions/<sub>)": (
                "projects/demo-project/subscriptions/zed-chat"
            ),
...
class TestSupervisorReconnect:
    ...
    async def test_fatal_after_max_retries(self, adapter, monkeypatch):
        ...
        # Keep the test fast â€” run supervisor until it exhausts retries.
...
class TestAuthorizationEmailMatch:
    ...
    def test_allowlist_matches_when_user_id_is_email(self, monkeypatch):
        """Email allowlist match â€” the canonical case.
    ...
    def test_allowlist_falls_back_to_resource_name_when_no_email(
        ...
        source = SessionSource(
            ...
            chat_id="spaces/S",
            chat_type="dm",
            user_id="users/77777",  # no email available â€” resource name wins
            user_name="System",
            user_id_alt=None,
        )
        assert runner._is_user_authorized(source) is True
...
# â”€â”€ _standalone_send (out-of-process cron delivery) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
