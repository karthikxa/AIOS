"""Tests for gateway.display_config â€” per-platform display/verbosity resolver."""
...
class TestResolveDisplaySetting:
    ...
    def test_platform_default_when_no_user_config(self):
        ...
        # Empty config â€” should get built-in defaults
        config = {}
        # Telegram is a mobile inbox by default â€” final-answer-first unless
        # explicitly configured otherwise.
        assert resolve_display_setting(config, "telegram", "tool_progress") == "off"
        # Email defaults to tier_minimal â†’ "off"
    ...
    def test_global_default_for_unknown_platform(self):
        ...
        config = {}
        # Unknown platform, no config â†’ global default "all"
...
class TestYAMLNormalisation:
    """YAML 1.1 quirks (bare off â†’ False, on â†’ True) are handled."""

    def test_tool_progress_false_normalised_to_off(self):
        """YAML's bare `off` parses as False â€” normalised to 'off' string."""
        from gateway.display_config import resolve_display_setting
    ...
    def test_tool_progress_true_normalised_to_all(self):
        """YAML's bare `on` parses as True â€” normalised to 'all'."""
        from gateway.display_config import resolve_display_setting
    ...
    def test_platform_override_false_tool_progress(self):
        """Per-platform bare off â†’ normalised."""
        from gateway.display_config import resolve_display_setting
...
class TestPlatformDefaults:
    ...
    def test_telegram_mobile_chatter_defaults(self):
        ...
        # Real model voice â€” keep on. Without this, Telegram users see
        ...
        # Periodic "Working â€” N min" heartbeat â€” keep on. Otherwise long
        # turns appear completely silent.
        assert resolve_display_setting({}, "telegram", "long_running_notifications") is True
        # Verbose iteration counter in busy-ack and heartbeat â€” off by
...
# ---------------------------------------------------------------------------
# Config migration: tool_progress_overrides â†’ display.platforms
# ---------------------------------------------------------------------------
...
class TestStreamingPerPlatform:
    ...
    def test_none_means_follow_global(self):
        ...
        config = {}
        # Telegram has no streaming override in defaults â†’ None
...
# ---------------------------------------------------------------------------
# cleanup_progress â€” opt-in deletion of temporary progress bubbles
# ---------------------------------------------------------------------------
...
class TestCleanupProgress:
    ...
    def test_default_off_for_all_platforms(self):
        """No config set â†’ cleanup_progress resolves to False everywhere."""
        from gateway.display_config import resolve_display_setting
    ...
    def test_yaml_off_string_normalises_to_false(self):
        """YAML 1.1 bare ``off`` becomes string 'off' â€” treat as False."""
        from gateway.display_config import resolve_display_setting
...
class TestToolProgressGrouping:
    ...
    def test_default_is_accumulate(self):
        """No config anywhere â†’ global default 'accumulate'."""
        from gateway.display_config import resolve_display_setting
