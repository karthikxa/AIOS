...
# ===========================================================================
# send_exec_approval â€” Block Kit buttons
# ===========================================================================
...
class TestSlackExecApproval:
    ...
    async def test_sends_blocks_with_buttons(self):
        ...
        assert "zed_approve_once" in action_ids
        assert "zed_approve_session" in action_ids
        assert "zed_approve_always" in action_ids
        assert "zed_deny" in action_ids
...
# ===========================================================================
# _handle_approval_action â€” button click handler
# ===========================================================================
...
class TestSlackApprovalAction:
    ...
    async def test_deny_action(self):
        ...
        action = {"action_id": "zed_deny", "value": "session-key"}
...
class TestSlackThreadContext:
    ...
    async def test_fetch_thread_context_includes_bot_parent(self):
        ...
        mock_client.conversations_replies = AsyncMock(return_value={
            "messages": [
                # Bot-posted parent (cron job)
                {
                    ...
                    "subtype": "bot_message",
                    "username": "cron",
                    "text": "ãƒ¡ãƒ¼ãƒ«è¦ç´„: æœ¬æ—¥ã®æ–°ç€3ä»¶",
                },
                # User reply that triggered the fetch
                {"ts": "1000.1", "user": "U1", "text": "è©³ç´°ã‚’æ•™ãˆã¦"},
            ]
        })
        ...
        assert "ãƒ¡ãƒ¼ãƒ«è¦ç´„: æœ¬æ—¥ã®æ–°ç€3ä»¶" in context
    ...
    async def test_fetch_thread_context_multi_workspace(self):
        ...
        mock_client.conversations_replies = AsyncMock(return_value={
            "messages": [
                ...
                # This has the *T1* bot's user id â€” from T2's perspective this
                ...
                # Self-bot for T2 â€” must be skipped
                {
                    "ts": "2000.2",
                    "bot_id": "B_SELF_T2",
                    "user": "U_BOT_T2",
...
# ===========================================================================
# _has_active_session_for_thread â€” session key fix (#5833)
# ===========================================================================
...
# ===========================================================================
# Thread engagement â€” bot-started threads & mentioned threads
# ===========================================================================
