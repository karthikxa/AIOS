...
runner â€” otherwise they are queued as user text and either:
  - leak into the conversation as agent input (/stop, /new), or
  - deadlock (/approve, /deny â€” agent blocks on Event.wait)
...
class TestAllResolvableCommandsBypassGuard:
    ...
    async def test_command_bypasses_guard(self, command_text, canonical):
        ...
        assert sk not in adapter._pending_messages, (
            f"{command_text} was queued as pending â€” it should bypass the guard"
        )
        assert len(adapter.sent_responses) > 0, (
            f"{command_text} produced no response â€” it should be dispatched, "
    ...
    def test_should_bypass_returns_false_for_unknown(self):
        """Unknown words don't bypass â€” they get queued as user text."""
        from hermes_cli.commands import should_bypass_active_session
...
class TestNonBypassStillQueued:
    ...
    async def test_regular_text_queued(self):
        ...
        assert sk in adapter._pending_messages, (
            "Regular text was not queued â€” it should be pending"
        )
        assert len(adapter.sent_responses) == 0, (
...
# ---------------------------------------------------------------------------
# Tests: no active session â€” commands go through normally
# ---------------------------------------------------------------------------
...
class TestNoActiveSessionNormalDispatch:
    ...
    async def test_stop_when_no_session_active(self):
        ...
        # No active session â€” _active_sessions is empty
...
class TestBypassWithBotnameSuffix:
    ...
    async def test_stop_with_botname(self):
        """/stop@MyZedBot must bypass the guard."""
        adapter = _make_adapter()
        sk = _session_key()
        adapter._active_sessions[sk] = asyncio.Event()

        await adapter.handle_message(_make_event("/stop@MyZedBot"))

        assert sk not in adapter._pending_messages, (
            "/stop@MyZedBot was queued instead of bypassing"
        )
        assert any("handled:stop" in r for r in adapter.sent_responses)

    @pytest.mark.asyncio
    async def test_new_with_botname(self):
        """/new@MyZedBot must bypass the guard."""
        adapter = _make_adapter()
        sk = _session_key()
        adapter._active_sessions[sk] = asyncio.Event()

        await adapter.handle_message(_make_event("/new@MyZedBot"))
