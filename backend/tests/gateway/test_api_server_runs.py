...
Covers:
- POST /v1/runs â€” start a run (202)
- GET /v1/runs/{run_id} â€” poll run status
- GET /v1/runs/{run_id}/events â€” SSE event stream
    ...
    async def test_stop_interrupt_exception_does_not_crash(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_create_agent") as mock_create:
                ...
                # â€” without this the agent thread blocks the full 10s
