"""Tests for gateway.shutdown_forensics â€” fast snapshot + async diag spawn."""
...
class TestSnapshotShutdownContext:
    ...
    def test_completes_quickly(self):
        """Snapshot must NOT block â€” it runs inside the asyncio signal handler."""
        start = time.monotonic()
        sf.snapshot_shutdown_context(signal.SIGTERM)
        elapsed = time.monotonic() - start
        # Generous bound; the function should be sub-millisecond in practice.
        assert elapsed < 0.5, f"snapshot took {elapsed:.3f}s â€” too slow"
...
class TestSpawnAsyncDiagnostic:
    ...
    def test_spawns_subprocess_and_writes_output(self, tmp_path):
        ...
        # Wait briefly for the subprocess to write â€” bounded by its own timeout.
