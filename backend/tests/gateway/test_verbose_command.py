...
class TestVerboseCommand:
    ...
    async def test_defaults_to_platform_default_when_no_tool_progress_set(self, tmp_path, monkeypatch):
        ...
        first ``/verbose`` invocation therefore cycles ``off â†’ new``, not
        ``all â†’ ...``.
        """
        zed_home = tmp_path / "zed"
        zed_home.mkdir()
        ...
        # Telegram platform default is "off" â†’ cycles to "new"
    ...
    async def test_per_platform_isolation(self, tmp_path, monkeypatch):
        ...
        default â€” Telegram = 'off' (tier-1 inbox override), Slack = 'off'
        (quiet Slack default). Both cycle to 'new' on first /verbose.
        """
        zed_home = tmp_path / "zed"
        zed_home.mkdir()
        config_path = zed_home / "config.yaml"
        # No global tool_progress â†’ built-in platform defaults apply
        config_path.write_text(
            "display:\n  tool_progress_command: true\n",
            encoding="utf-8",
        )
