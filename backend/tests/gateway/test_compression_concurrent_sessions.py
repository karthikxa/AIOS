...
def test_concurrent_compressions_do_not_alias_sessions(tmp_path: Path) -> None:
    ...
    assert len(set(new_ids)) == n, (
        ...
        "Two agents aliased to the same id â€” cross-session contamination."
    )
...
def test_concurrent_compressions_same_session_serialize(tmp_path: Path) -> None:
    ...
    # acquire â†’ compress â†’ rotate â†’ RELEASE the lock before the other even
    ...
    # condition this test means to assert â€” with zero timing dependency.
