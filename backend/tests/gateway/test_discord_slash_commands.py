...
def _ensure_discord_mock():
    if "discord" in sys.modules and hasattr(sys.modules["discord"], "__file__"):
        # Real discord is installed â€” nothing to do.
        return
    ...
    # need onto discord.app_commands â€” the flat /skill command uses
...
def adapter():
    ...
    adapter._client = SimpleNamespace(
        ...
        user=SimpleNamespace(id=99999, name="ZedBot"),
    )
    adapter._text_batch_delay_seconds = 0  # disable batching for tests
    # Slash auth is exercised in test_discord_slash_auth.py â€” bypass it here
...
async def test_registers_native_thread_slash_command(adapter):
    # The /thread slash closure now delegates ALL the work â€” including
    # defer() â€” to _handle_thread_create_slash so the auth gate can send
    ...
    # auth check passes â€” not by the closure.
...
async def test_auto_registers_missing_gateway_commands(adapter):
    ...
    # hardcoded registration list â€” they should be auto-registered.
...
async def test_auto_registered_command_dispatches_correctly(adapter):
    ...
    # /debug has no args â€” test parameterless dispatch
...
async def test_auto_registered_command_with_args(adapter):
    ...
    # /branch has args_hint="[name]" â€” test dispatch with args
...
async def test_plugin_command_name_conflict_skipped(adapter):
    ...
    assert callable(status_entry) and not hasattr(status_entry, "callback"), (
        "plugin registration overrode the built-in /status command â€” "
...
async def test_slash_command_registration_stays_under_discord_limit(adapter):
    ...
    # 200 plugin commands â€” way past Discord's limit on their own.
    many_plugins = {
        f"plug{i:03d}": {
            "handler": lambda _a: "ok",
    ...
    assert len(tree_names) <= _DISCORD_MAX_APP_COMMANDS, (
        f"registered {len(tree_names)} commands â€” exceeds Discord's "
    ...
    # the cap â€” they are the core UX, not droppable overflow.
    for native in ("status", "stop", "new", "model", "help"):
        assert native in tree_names, f"/{native} (native) was dropped by the cap"

    # The cap must actually have dropped overflow â€” not every plugin fit.
...
# ------------------------------------------------------------------
# _handle_thread_create_slash â€” success, session dispatch, failure
# ------------------------------------------------------------------
...
async def test_handle_thread_create_slash_dispatches_session_when_message_provided(adapter):
    ...
    await adapter._handle_thread_create_slash(interaction, "Planning", "Hello Zed", 1440)

    adapter._dispatch_thread_session.assert_awaited_once_with(
        interaction, "555", "Planning", "Hello Zed",
    )
...
# ------------------------------------------------------------------
# _dispatch_thread_session â€” builds correct event and routes it
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# _build_slash_event â€” preserve thread context for native slash commands
# ------------------------------------------------------------------
...
async def test_auto_create_thread_strips_mention_syntax_from_name(adapter):
    ...
    Regression guard for #6336 â€” previously a message like
...
@pytest.mark.asyncio
async def test_auto_create_thread_falls_back_to_zed_when_only_mentions(adapter):
    """If a message contains only mention syntax, the stripped content is
    empty â€” fall back to the 'Zed' default rather than ''."""
    thread = SimpleNamespace(id=999, name="Zed")
    message = SimpleNamespace(
...
async def test_auto_create_thread_falls_back_to_seed_message(adapter):
    ...
    result = await adapter._auto_create_thread(message)
    assert result is thread
    message.channel.send.assert_awaited_once_with("ðŸ§µ Thread created by Zed: **Hello**")
    seed_message.create_thread.assert_awaited_once_with(
        name="Hello",
        auto_archive_duration=1440,
...
import discord as _discord_mod  # noqa: E402 â€” mock or real, used below
...
class _FakeTextChannel:
    ...
    def history(self, *args, **kwargs):
        async def _empty():
            return
            yield  # pragma: no cover â€” make this an async generator

        return _empty()


class _FakeThreadChannel(_discord_mod.Thread):
    """isinstance(ch, discord.Thread) â†’ True."""

    def __init__(self, channel_id=200, name="existing-thread", guild_name="TestGuild", parent_id=100):
        # Don't call super().__init__ â€” mock Thread is just an empty type
        self.id = channel_id
        self.name = name
        self.guild = SimpleNamespace(name=guild_name, id=1)
        self.topic = None
...
def test_discord_auto_thread_config_bridge(monkeypatch, tmp_path):
    ...
    zed_dir.mkdir()
    config_path = zed_dir / "config.yaml"
    config_path.write_text(yaml.dump({
        "discord": {"auto_thread": True},
    }))

    monkeypatch.delenv("DISCORD_AUTO_THREAD", raising=False)
    monkeypatch.setenv("ZED_HOME", str(zed_dir))
...
def test_register_skill_command_is_flat_not_nested(adapter):
    ...
    flat layout sidesteps the limit â€” autocomplete options are fetched
    ...
    # Flat command â€” NOT a Group â€” so it has no _children of category subgroups
...
def test_register_skill_command_callback_dispatches_by_name(adapter):
    ...
    # gif-search â†’ /gif-search with no args
...
def test_register_skill_command_payload_fits_discord_8kb_limit(adapter):
    ...
    (20 categories Ã— 25 â€” the hard cap per category in the collector) and
    ...
    # 20 categories Ã— 25 skills each, with verbose 100-char descriptions.
    ...
    # Autocomplete options are NOT registered â€” they're fetched dynamically.
    payload = json.dumps({
        "name": skill_cmd.name,
    ...
    assert len(payload) < 500, (
        f"Flat /skill command payload is ~{len(payload)} bytes â€” the whole "
...
def test_register_skill_command_autocomplete_filters_by_name_and_description(adapter):
    ...
    skill_cmd = adapter._client.tree.commands["skill"]
    # The callback has been wrapped with @autocomplete(name=...) â€” in our mock
