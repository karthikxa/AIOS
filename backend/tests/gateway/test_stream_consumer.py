...
# â”€â”€ _clean_for_display unit tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestCleanForDisplay:
    ...
    def test_media_tag_with_space(self):
        ...
        text = "Audio generated\nMEDIA: /home/user/.zed/audio_cache/voice.mp3"
        result = GatewayStreamConsumer._clean_for_display(text)
        assert "MEDIA:" not in result
...
# â”€â”€ Integration: _send_or_edit strips MEDIA: â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestFinalizeCapabilityGate:
    ...
    Slack, Matrix, â€¦) should skip the redundant final edit when the
    ...
    *do* need it (DingTalk AI Cards) must always receive a finalize=True
    ...
    async def test_identical_text_skip_respects_adapter_flag(self):
        ...
        # Adapter without finalize requirement â€” should skip identical edit.
        plain = MagicMock()
        ...
        await c1._send_or_edit("hello", finalize=True)  # identical â†’ skip
        plain.edit_message.assert_not_called()

        # Adapter that requires finalize â€” must still fire the edit.
        picky = MagicMock()
...
class TestEditMessageFinalizeSignature:
    ...
    silently regress â€” existing tests use MagicMock which swallows any
...
# â”€â”€ Integration: full stream run â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestStreamRunMediaStripping:
    ...
    async def test_stream_with_media_tag(self):
        ...
        consumer.on_delta("MEDIA:/home/user/.zed/cache/images/abc123.png")
        consumer.finish()
...
# â”€â”€ Segment break (tool boundary) tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestSegmentBreakOnToolBoundary:
    ...
    async def test_segment_break_clears_failed_edit_fallback_state(self):
        ...
        assert sent_texts == ["Hello â–‰", "world", "Next segment"]
    ...
    async def test_segment_break_after_mid_stream_edit_failure_preserves_tail(self):
        ...
        delivered â€” not silently dropped by the segment reset."""
        adapter = MagicMock()
        ...
        # â€” simulating Telegram's "edit once then get rate-limited" pattern.
        edit_results = [
            SimpleNamespace(success=True),   # "Hello world â–‰"  â€” succeeds
            SimpleNamespace(success=False, error="flood_control:6.0"),  # "Hello world more â–‰" â€” flood triggered
    ...
    async def test_fallback_final_sends_full_text_at_tool_boundary(self):
        ...
        # would return empty (no prefix match â†’ full text returned, but if the
        ...
        post_tool_response = "â° Script timed out after 30s and was killed."
        consumer.finish()
        await task
        ...
        assert sent, (
            ...
            "_send_fallback_final â€” the #10807 fix should prevent this"
        )
    ...
    async def test_fallback_final_deletes_partial_after_full_resend(self):
        ...
        # final response â€” fallback re-sends the complete final text.
        consumer._message_id = "msg_partial"
        consumer._last_sent_text = "Let me check that for youâ€¦"
    ...
    async def test_fallback_final_keeps_partial_after_tail_only_send(self):
        ...
        answer â€” deleting it would leave the user with only the last part
        ...
        # Visible partial is a true prefix of the final response â€” the
    ...
    async def test_fallback_final_does_not_delete_when_no_chunks_reach_user(self):
        ...
        user has â€” must NOT be deleted."""
        adapter = MagicMock()
        adapter.send = AsyncMock(
...
class TestFinalResponseDeliveryGuard:
    """Regression coverage for #10748 â€” _final_response_sent must reflect
    ...
    async def test_split_overflow_failed_send_does_not_mark_final_sent(self):
        ...
        adapter = MagicMock()
        # Every send fails â€” _send_new_chunk returns the passed-in reply_to.
        adapter.send = AsyncMock(
            return_value=SimpleNamespace(success=False, error="network down"),
        )
        adapter.edit_message = AsyncMock(
        ...
        assert consumer._final_response_sent is False, (
            "_already_sent leaked into _final_response_sent â€” gateway will "
    ...
    async def test_split_overflow_partial_send_marks_final_sent(self):
        ...
        we did deliver the final answer â€” _final_response_sent must be True."""
        adapter = MagicMock()
        adapter.send = AsyncMock(side_effect=[
            SimpleNamespace(success=True, message_id="msg_1"),
            SimpleNamespace(success=True, message_id="msg_2"),
...
class TestFinalContentDeliveredGuard:
    """Regression coverage for #25010 â€” _final_content_delivered must only be
    ...
    async def test_mid_stream_edit_success_does_not_mark_content_delivered(self):
        ...
        assert consumer._final_content_delivered is False, (
            "_final_content_delivered was prematurely set to True â€” gateway "
    ...
    async def test_final_edit_success_does_mark_content_delivered(self):
        ...
        must be True â€” the normal happy path should still work."""
        adapter = MagicMock()
        adapter.edit_message = AsyncMock(return_value=SimpleNamespace(success=True))
        adapter.send = AsyncMock(
            return_value=SimpleNamespace(success=True, message_id="msg_1"),
    ...
    async def test_fallback_partial_send_does_not_mark_final_sent(self):
        ...
        assert consumer._final_response_sent is False, (
            "Partial fallback send must not set _final_response_sent â€” gateway "
...
# â”€â”€ Think-block filtering unit tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ buffer_only mode tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Cursor stripping on fallback (#7183) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestCursorStrippingOnFallback:
    ...
    fix this would leave the message permanently frozen with a visible â–‰.
    """
    ...
    async def test_cursor_stripped_when_continuation_empty(self):
        ...
        consumer = GatewayStreamConsumer(
            adapter, "chat-1",
            config=StreamConsumerConfig(cursor=" â–‰"),
        )
        consumer._message_id = "msg-1"
        consumer._last_sent_text = "Hello world â–‰"
    ...
    async def test_cursor_strip_edit_failure_handled(self):
        ...
        consumer._message_id = "msg-1"
        consumer._last_sent_text = "Hello â–‰"
        consumer._fallback_final_send = False
        ...
        assert consumer._last_sent_text == "Hello â–‰"


# â”€â”€ on_new_message callback (tool-progress linearization) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestOnNewMessageCallback:
    ...
    below the content â€” preserving chronological order in the chat.
    ...
    async def test_callback_fires_once_per_segment(self):
        ...
        # Three content bubbles â‡’ three reset notifications
    ...
    async def test_callback_fires_on_commentary(self):
        """Commentary messages are fresh bubbles too â€” fire the callback."""
        adapter = MagicMock()
        adapter.send = AsyncMock(return_value=SimpleNamespace(success=True, message_id="msg_1"))
        adapter.edit_message = AsyncMock(return_value=SimpleNamespace(success=True))
        adapter.MAX_MESSAGE_LENGTH = 4096
...
class TestUtf16OverflowDetection:
    """Regression coverage for #11170 â€” Telegram counts message length in
    ...
    async def test_emoji_text_exceeding_utf16_limit_triggers_overflow_split(self):
        ...
        # ðŸš€ is 1 codepoint = 2 UTF-16 units. 2200 of them = 2200 codepoints,
        ...
        # 2200 (under 4096) and no split would fire â€” Telegram would then
        # reject the send or render \x00 artifacts.
        adapter.truncate_message.assert_called(), (
            "UTF-16 overflow not detected â€” emoji text bypassed split path"
        )
    ...
    def test_codepoint_only_adapter_falls_back_to_len(self):
        ...
        # this file passing â€” they all use MagicMock adapters.
