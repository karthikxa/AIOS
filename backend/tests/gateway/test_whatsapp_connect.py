...
class TestBridgeRuntimeFailure:
    ...
    async def test_shutdown_suppresses_fatal_on_planned_bridge_exit(self, returncode):
        ...
        "âœ“ whatsapp disconnected" â€” because _check_managed_bridge_exit()
...
class TestNoCredsPreflight:
    ...
    Without this guard, every gateway boot:
      â€¢ spawned the bridge subprocess (npm install if needed)
      â€¢ waited 30s for status:connected (never happens without creds)
      â€¢ queued WhatsApp for indefinite retries that would just repeat
    ...
    async def test_connect_returns_false_when_no_creds(self, tmp_path):
        ...
        # bridge-missing check doesn't trip â€” we want to exercise the
    ...
    async def test_connect_proceeds_when_creds_present(self, tmp_path):
        ...
        simulate the bridge here â€” we just verify no fast-fail occurs.
        """
        from gateway.platforms.whatsapp import WhatsAppAdapter
        ...
        # Preflight passed â€” exits because we faked lock acquisition,
