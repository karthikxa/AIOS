...
def _fake_create_task(coro):
    ...
    by code that later cancels, awaits, or attaches ``add_done_callback`` â€”
...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.zed."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /zed, /btw, /stop,
        ...
        for expected in ("/zed", "/btw", "/stop", "/model", "/help"):
...
class TestSlackConnectCleanup:
    ...
    async def test_reconnect_closes_previous_handler_to_prevent_zombie_socket(self):
        ...
        every Slack event, producing double responses â€” the same bug that
        ...
        # asyncio.create_task â€” so the stub must return a real coroutine, not a
        # bare MagicMock.
...
class TestSlackSocketWatchdog:
    ...
    async def test_watchdog_cancellation_does_not_respawn(self):
        """Cancellation is the intentional-shutdown signal â€” no respawn allowed."""
        adapter = SlackAdapter(PlatformConfig(enabled=True, token="xoxb-fake"))
        adapter._socket_watchdog_interval_s = 0.01
        ...
        with contextlib.ExitStack() as stack:
            ...
            try:
                ...
                # to the same cancelled task â€” never a fresh respawn).
    ...
    async def test_watchdog_unexpected_exit_respawns_via_done_callback(self):
        ...
        with contextlib.ExitStack() as stack:
            ...
            try:
                ...
                # the done-callback directly â€” this is the exact signal the
    ...
    async def test_connect_replaces_prior_watchdog_atomically(self):
        ...
        with contextlib.ExitStack() as stack:
            ...
            try:
                ...
                # a brand new one â€” never observe a window with no watchdog.
                assert await adapter.connect() is True
...
class TestBangPrefixCommands:
    ...
    async def test_bang_known_command_is_rewritten_to_slash(self, adapter):
        """``!queue`` â†’ ``/queue`` and tagged as COMMAND."""
    ...
    async def test_bang_unknown_token_passes_through_unchanged(self, adapter):
        """``!nice work`` is just a casual message â€” must NOT be rewritten."""
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@zed`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@zed"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@zed")
        assert msg_event.message_type == MessageType.COMMAND

    @pytest.mark.asyncio
    async def test_plain_slash_still_works(self, adapter):
        """Sanity check â€” ``/queue`` (top-level channel/DM) still dispatches."""
        await adapter._handle_slack_message(self._make_event("/queue"))
...
class TestIncomingDocumentHandling:
    ...
    async def test_download_failure_is_surfaced_in_message_text(self, adapter):
        ...
        scope). No proactive files.info probe is made â€” the diagnostic
    ...
    async def test_rich_text_quotes_and_lists_are_extracted(self, adapter):
        ...
        assert "â€¢ First bullet" in msg_event.text
        assert "â€¢ Second bullet" in msg_event.text
    ...
    async def test_attachments_unfurl_text_is_appended_even_when_url_is_in_message(
        ...
        assert "ðŸ“Ž [Spec](https://example.com/spec)" in msg_event.text
    ...
    async def test_channel_routing_ignores_bot_mentions_inside_block_text(
        ...
        gating in channels â€” routing decisions use the original user text so
...
# ---------------------------------------------------------------------------
# TestSendTyping â€” assistant.threads.setStatus
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# TestFormatMessage â€” Markdown â†’ mrkdwn conversion
# ---------------------------------------------------------------------------
...
class TestFormatMessage:
    ...
    def test_message_only_code_block(self, adapter):
        """Entire message is a fenced code block â€” no conversion."""
        code = "```python\nx = 1\n```"
        assert adapter.format_message(code) == code
    ...
    def test_nested_bold_in_link(self, adapter):
        """Bold inside link label â€” label is stashed before bold pass."""
...
class TestEditMessageStreamingPipeline:
    ...
    async def test_edit_message_formats_streaming_updates(self, adapter):
        ...
        # Second streaming update â€” bold + link
    ...
    async def test_edit_message_formats_code_and_bold(self, adapter):
        """Streaming update with code block and bold â€” code must be preserved."""
        adapter._app.client.chat_update = AsyncMock(return_value={"ok": True})
    ...
    async def test_edit_message_formats_blockquote_in_stream(self, adapter):
        """Streaming update with blockquote â€” '>' marker must survive."""
        adapter._app.client.chat_update = AsyncMock(return_value={"ok": True})
...
# ---------------------------------------------------------------------------
# TestSlashCommands â€” expanded command set
# ---------------------------------------------------------------------------
...
class TestSlashCommands:
    ...
    # ------------------------------------------------------------------
    # Native slash commands â€” /btw, /stop, /model, ... dispatched directly
    # instead of as /zed subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /zed btw."""
        command = {
            "command": "/btw",
        ...
        # resolve_command() â€” our handler's job is just to deliver
    ...
    @pytest.mark.asyncio
    async def test_legacy_zed_prefix_still_works(self, adapter):
        """Backward compat: /zed btw foo must still route to /btw foo.

        Old workspace manifests only declared /zed as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_zed_freeform_question(self, adapter):
        """/zed <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/zed",
...
class TestMessageSplitting:
    ...
    async def test_send_preserves_blockquote_formatting(self, adapter):
        """Blockquote '>' markers must survive format â†’ chunk â†’ send pipeline."""
        adapter._app.client.chat_postMessage = AsyncMock(return_value={"ts": "ts1"})
...
class TestProgressMessageThread:
    ...
    async def test_dm_toplevel_progress_uses_message_ts_as_thread(self, adapter):
        ...
        event = {
            ...
            "text": "Hello bot",
            "ts": "1234567890.000001",
            # No thread_ts â€” this is a top-level DM
        }
        ...
        # The message_id should be the event's ts â€” this is what the gateway
    ...
    async def test_channel_mention_progress_uses_thread_ts(self, adapter):
        ...
        event = {
            ...
            # No thread_ts â€” top-level channel message
        }
...
class TestSlackReplyToText:
    ...
    async def test_slack_reply_to_text_set_on_thread_reply(self, adapter):
                ...
                "messages": [
                    {
                        "ts": "1000.0",
                        "bot_id": "B_CRON",
                        "text": "ãƒ¡ãƒ¼ãƒ«è¦ç´„: æ–°ç€ãƒ¡ãƒ¼ãƒ«3ä»¶ã‚ã‚Šã¾ã™",
                    },
                    {"ts": "1000.5", "user": "U_USER", "text": "è©³ç´°ã‚’æ•™ãˆã¦"},
                ]
            }
        )
        ...
        assert "ãƒ¡ãƒ¼ãƒ«è¦ç´„" in msg_event.reply_to_text
...
class TestSlashEphemeralAck:
    ...
    async def test_send_slash_ephemeral_fallback_on_post_failure(self, adapter):
        ...
        # Still success â€” the user saw the initial ack already
        assert result.success is True
    ...
    async def test_native_slash_stashes_context_and_dispatches(self, adapter):
        """Full flow: native /q slash â†’ stash + handle_message dispatch."""
        command = {
            "command": "/q",
            "text": "do something",
            "user_id": "U_Q",
    ...
    @pytest.mark.asyncio
    async def test_legacy_zed_slash_stashes_context(self, adapter):
        """Legacy /zed <subcommand> also stashes context."""
        command = {
            "command": "/zed",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_zed_question_does_not_stash_context(self, adapter):
        """Free-form /zed <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/zed",
        ...
        adapter.handle_message.assert_called_once()
        event = adapter.handle_message.call_args[0][0]
        # Free-form text â€” not a command
        assert event.message_type == MessageType.TEXT
        assert event.text == "what's the weather"
        # Context must NOT be stashed â€” agent reply should be public
        assert len(adapter._slash_command_contexts) == 0

    @pytest.mark.asyncio
    async def test_concurrent_users_same_channel_isolates_contexts(self, adapter):
        """Two users slash on the same channel â€” each gets their own context."""
        import time
        ...
        # Alice's send() â€” ContextVar set to Alice's user_id.
        ...
        # Bob's send() â€” ContextVar set to Bob's user_id.
    ...
    async def test_no_contextvar_does_not_match_any_context(self, adapter):
        ...
        # ContextVar is unset (default=None) â€” simulates a normal message send.
        assert _slash_user_id.get() is None
        ctx = adapter._pop_slash_context("C1")
        # Fallback scan still finds it (channel-only) â€” this is fine for
