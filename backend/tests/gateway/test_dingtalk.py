...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "Zed"
...
class TestWebhookDomainAllowlist:
    ...
    def test_unsanctioned_subdomain_rejected(self):
        from gateway.platforms.dingtalk import _DINGTALK_WEBHOOK_RE
        # Only api.* and oapi.* are allowed â€” e.g. eapi.dingtalk.com must not slip through
...
class TestExtractText:
    ...
    returns ``"TextContent(content=...)"`` â€” falling back to ``str(text)``
...
class TestExtractMedia:
    ...
    def test_voice_rich_text_item_classified_as_voice(self):
        ...
        path via MessageType.VOICE â€” the gateway skips STT for AUDIO."""
        from gateway.platforms.dingtalk import DingTalkAdapter
    ...
    def test_audio_rich_text_item_stays_audio(self):
        ...
        be auto-transcribed â€” they stay MessageType.AUDIO."""
...
# ---------------------------------------------------------------------------
# Group gating â€” require_mention + allowed_users (parity with other platforms)
# ---------------------------------------------------------------------------
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^zed"]}
        )
        assert adapter._message_matches_mention_patterns("zed please help") is True
        assert adapter._message_matches_mention_patterns("please zed help") is False

    def test_pattern_is_case_insensitive(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^zed"]}
        )
        assert adapter._message_matches_mention_patterns("ZED help") is True
...
class TestShouldProcessMessage:
    ...
    def test_group_accepted_when_text_matches_wake_word(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch,
            extra={"require_mention": True, "mention_patterns": ["^zed"]},
        )
        msg = MagicMock(is_in_at_list=False)
        assert adapter._should_process_message(msg, "zed help", is_group=True, chat_id="grp1") is True
...
# ---------------------------------------------------------------------------
# _IncomingHandler.process â€” session_webhook extraction & fire-and-forget
# ---------------------------------------------------------------------------
...
class TestIncomingHandlerProcess:
    ...
    async def test_process_returns_ack_immediately(self):
        """process() must not block on _on_message â€” it should return
...
# ---------------------------------------------------------------------------
# Text extraction â€” mention preservation + platform sanity
# ---------------------------------------------------------------------------
...
class TestExtractTextMentions:
    ...
    def test_preserves_at_mentions_in_text(self):
        ...
        cases = [
            ...
            ("@æœºå™¨äºº è½¬å‘ç»™ @è€çŽ‹", "@æœºå™¨äºº è½¬å‘ç»™ @è€çŽ‹"),
        ]
...
# ---------------------------------------------------------------------------
# Concurrency â€” chat-scoped message context
# ---------------------------------------------------------------------------
...
class TestCardLifecycle:
    ...
    async def test_final_reply_finalizes_card(self, adapter_with_card):
        ...
        # Not tracked as streaming â€” it's already closed.
    ...
    async def test_intermediate_send_stays_streaming(self, adapter_with_card):
        ...
        commentary / streaming first chunk).  No flicker closedâ†’streaming
        when edit_message follows."""
        a = adapter_with_card
        result = await a.send("chat-1", "ðŸ’» terminal: ls")
        assert result.success
        call = a._card_sdk.streaming_update_with_options_async.call_args
    ...
    async def test_done_fires_only_when_reply_to_is_set(self, adapter_with_card):
        ...
        # Tool-progress / commentary path: no reply_to â€” no Done.
        await a.send("chat-1", "tool line")
        assert fired == []

        # Final response path: reply_to set â€” Done fires.
    ...
    async def test_next_send_auto_closes_sibling_streaming_cards(
        ...
        a = adapter_with_card
        # First tool: intermediate send â€” card stays open.
        r1 = await a.send("chat-1", "ðŸ’» tool1")
        # Second tool: edit_message(finalize=False) â€” keeps streaming.
        await a.edit_message(
            chat_id="chat-1", message_id=r1.message_id,
            content="ðŸ’» tool1\nðŸ’» tool2", finalize=False,
        )
