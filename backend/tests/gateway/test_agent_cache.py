...
- Reuses agents across messages (same config â†’ same instance)
...
class TestAgentConfigSignature:
    ...
    def test_same_config_same_signature(self):
        ...
        sig1 = GatewayRunner._agent_config_signature("claude-sonnet-4", runtime, ["zed-telegram"], "")
        sig2 = GatewayRunner._agent_config_signature("claude-sonnet-4", runtime, ["zed-telegram"], "")
        assert sig1 == sig2
    ...
    def test_model_change_different_signature(self):
        ...
        runtime = {"api_key": "sk-test12345678", "base_url": "https://openrouter.ai/api/v1",
                    "provider": "openrouter"}
        sig1 = GatewayRunner._agent_config_signature("claude-sonnet-4", runtime, ["zed-telegram"], "")
        sig2 = GatewayRunner._agent_config_signature("claude-opus-4.6", runtime, ["zed-telegram"], "")
        assert sig1 != sig2
    ...
    def test_same_token_prefix_different_full_token_changes_signature(self):
        ...
        sig1 = GatewayRunner._agent_config_signature("gpt-5.3-codex", rt1, ["zed-telegram"], "")
        sig2 = GatewayRunner._agent_config_signature("gpt-5.3-codex", rt2, ["zed-telegram"], "")
        assert sig1 != sig2
    ...
    def test_provider_change_different_signature(self):
        ...
        sig1 = GatewayRunner._agent_config_signature("claude-sonnet-4", rt1, ["zed-telegram"], "")
        sig2 = GatewayRunner._agent_config_signature("claude-sonnet-4", rt2, ["zed-telegram"], "")
        assert sig1 != sig2
    ...
    def test_toolset_change_different_signature(self):
        ...
        sig1 = GatewayRunner._agent_config_signature("claude-sonnet-4", runtime, ["zed-telegram"], "")
        sig2 = GatewayRunner._agent_config_signature("claude-sonnet-4", runtime, ["zed-discord"], "")
        assert sig1 != sig2
    ...
    def test_reasoning_not_in_signature(self):
        ...
        runtime = {"api_key": "sk-test12345678", "base_url": "https://openrouter.ai/api/v1", "provider": "openrouter"}
        # Same config â€” signature should be identical regardless of what
...
class TestExtractCacheBustingConfig:
    ...
    def test_non_dict_section_treated_as_missing(self):
        ...
        # compression is a string â€” should not crash, all compression.* keys None
        out = GatewayRunner._extract_cache_busting_config(
...
class TestAgentCacheLifecycle:
    ...
    def test_cache_hit_returns_same_agent(self):
        ...
        runtime = {"api_key": "test", "base_url": "https://openrouter.ai/api/v1",
                    "provider": "openrouter", "api_mode": "chat_completions"}
        sig = runner._agent_config_signature("anthropic/claude-sonnet-4", runtime, ["zed-telegram"], "")

        # First message â€” create and cache
        agent1 = AIAgent(
    ...
    def test_cache_miss_on_model_change(self):
        """Model change produces different signature â†’ cache miss."""
        from run_agent import AIAgent
        ...
        old_sig = runner._agent_config_signature("anthropic/claude-sonnet-4", runtime, ["zed-telegram"], "")
        agent1 = AIAgent(
            model="anthropic/claude-sonnet-4", api_key="test",
            base_url="https://openrouter.ai/api/v1", provider="openrouter",
            max_iterations=5, quiet_mode=True, skip_context_files=True,
        ...
        # New model â†’ different signature
        new_sig = runner._agent_config_signature("anthropic/claude-opus-4.6", runtime, ["zed-telegram"], "")
        assert new_sig != old_sig

        with runner._agent_cache_lock:
            cached = runner._agent_cache.get(session_key)
        assert cached[1] != new_sig  # signature mismatch â†’ would create new agent
    ...
    def test_reasoning_config_updates_in_place(self):
        ...
        assert prompt1 is prompt2  # same object â€” not invalidated by reasoning change
    ...
    def test_system_prompt_frozen_across_cache_reuse(self):
        ...
        # Simulate second turn â€” prompt should be frozen
        prompt2 = agent._cached_system_prompt
...
class TestAgentCacheBoundedGrowth:
    ...
    def test_cap_evicts_lru_when_exceeded(self, monkeypatch):
        ...
        # Insert a 4th â€” oldest (s0) must be evicted.
        with runner._agent_cache_lock:
    ...
    def test_cap_respects_move_to_end(self, monkeypatch):
        ...
        # Touch s0 â€” it is now MRU, so s1 becomes LRU.
        ...
        assert "s1" not in runner._agent_cache  # now oldest â†’ evicted
        assert "s3" in runner._agent_cache
    ...
    def test_cap_triggers_cleanup_thread(self, monkeypatch):
        ...
        _cleanup_agent_resources â€” cache eviction must not tear down
        ...
        # Hard-cleanup path must NOT have fired â€” that's for session expiry only.
...
class TestAgentCacheActiveSafety:
    ...
    def test_cap_skips_active_lru_entry(self, monkeypatch):
        ...
        # Mark `active` as mid-turn â€” it's LRU, but protected.
    ...
    def test_cap_pending_sentinel_does_not_block_eviction(self, monkeypatch):
        ...
        runner._agent_cache["s1"] = (a1, "sig")
        runner._agent_cache["s2"] = (a2, "sig")
        # Another session is mid-creation â€” sentinel, no real agent yet.
    ...
    def test_eviction_does_not_close_active_agent_client(self, monkeypatch):
        ...
        This reproduces the original concern â€” if eviction fired while an
        ...
        # Real cleanup function, not mocked â€” we want to see whether close()
        ...
        assert active.client is not None, (
            "Active agent's client was closed by eviction â€” "
...
class TestAgentCacheSpilloverLive:
    ...
    def test_evicted_session_next_turn_gets_fresh_agent(self, monkeypatch):
        ...
        # Now sA's user sends another message â†’ a fresh agent goes in.
...
class TestAgentCacheIdleResume:
    ...
    The new agent built for the same session_id must inherit:
      - Conversation history (from SessionStore â€” outside cache concern)
      - Terminal sandbox (same task_id â†’ same _active_environments entry)
      - Browser daemon (same task_id â†’ same browser session)
      - Background processes (same task_id â†’ same process_registry entries)
    ...
    def test_release_clients_does_not_touch_process_registry(self, monkeypatch):
        ...
        # Spy on process_registry.kill_all â€” it MUST NOT be called.
        ...
        assert kill_all_calls == [], (
            f"release_clients() called process_registry.kill_all â€” would "
    ...
    def test_release_clients_does_not_touch_terminal_or_browser(self, monkeypatch):
        ...
        assert vm_calls == [], (
            f"release_clients() tore down terminal sandbox â€” user's cwd, "
            f"env, and bg shells would be gone on resume. Calls: {vm_calls}"
        )
        assert browser_calls == [], (
            f"release_clients() tore down browser session â€” user's open "
    ...
    def test_close_vs_release_full_teardown_difference(self, monkeypatch):
        ...
        (full teardown â€” session is done), cache-eviction path uses
        release_clients() (soft â€” session may resume).
        """
        from run_agent import AIAgent
        import run_agent as _ra

        # Agent A: evicted from cache (soft) â€” terminal survives.
        # Agent B: session expired (hard) â€” terminal torn down.
        agent_a = AIAgent(
            model="anthropic/claude-sonnet-4", api_key="test",
            base_url="https://openrouter.ai/api/v1", provider="openrouter",
            max_iterations=5, quiet_mode=True,
        ...
        # directly â€” so patch the ``run_agent`` reference.
    ...
    def test_idle_evicted_session_rebuild_inherits_task_id(self, monkeypatch):
        ...
        gets the same task_id â€” so tool state (terminal/browser/bg procs)
        ...
        # User comes back â€” new agent built for the SAME session_id.
        new_agent = AIAgent(
            model="anthropic/claude-sonnet-4", api_key="test",
            base_url="https://openrouter.ai/api/v1", provider="openrouter",
            max_iterations=5, quiet_mode=True,
...
class TestCachedAgentInactivityReset:
    ...
    def test_interrupt_turn_preserves_idle_clock(self):
        ...
        assert agent._last_activity_ts == old_ts, (
            ...
            "(interrupt_depth>0) â€” the watchdog needs the accumulated idle time"
        )

    def test_interrupt_turn_preserves_desc(self):
        """interrupt_depth=1: desc preserved â€” it is semantically paired with ts."""
        from gateway.run import GatewayRunner
    ...
    def test_interrupt_turn_preserves_flush_cursor(self):
        ...
        assert agent._last_flushed_db_idx == 42, (
            ...
            "turns â€” the flush cursor tracks in-progress writes"
        )
    ...
    def test_watchdog_accumulation_across_recursive_turns(self):
        """Scenario: stuck turn + user interrupt â†’ recursive turn.
        ...
        # That triggers an interrupt â†’ _run_agent recurses at depth=1.
        GatewayRunner._init_cached_agent_for_turn(agent, interrupt_depth=1)

        # Watchdog sees time.time() - _last_activity_ts â‰¥ STUCK_FOR.
...
class TestAgentConfigSignatureUserId:
    ...
    def test_signature_changes_with_user_id(self):
        ...
        sig_a = GatewayRunner._agent_config_signature(
            "claude-sonnet-4", runtime, ["zed-telegram"], "", user_id="7654321"
        )
        sig_b = GatewayRunner._agent_config_signature(
            "claude-sonnet-4", runtime, ["zed-telegram"], "", user_id="491827364"
        )
        assert sig_a != sig_b
    ...
    def test_signature_changes_with_user_id_alt(self):
        ...
        runtime = {"provider": "anthropic", "api_key": "k", "base_url": "", "api_mode": "chat_completions"}
        sig_a = GatewayRunner._agent_config_signature(
            "claude-sonnet-4", runtime, ["zed-telegram"], "",
...
class TestAgentCacheMessageCountRebaseline:
    ...
    next turn sees a mismatch and rebuilds the agent â€” every turn, for every
    conversation â€” silently destroying per-conversation prompt caching.
    ...
    def test_same_process_turns_preserve_cached_agent(self, tmp_path):
        ...
        runs â€” so the NEXT turn's guard sees no external change and reuses.
        """
        ...
        # All 5 follow-on turns must reuse â€” WITHOUT the re-baseline this is 0.
        assert reuses == 5
    ...
    def test_cross_process_write_still_invalidates(self, tmp_path):
        ...
        # to the SAME session in the shared DB â€” we have NOT re-baselined for it.
