...
async def test_gateway_stop_kills_tool_subprocesses_on_graceful_path(monkeypatch):
    ...
    exactly once via the final catch-all â€” regression guard against
    ...
    # No running agents â†’ drain returns immediately, no timeout, no eager cleanup.
    with patch("gateway.status.remove_pid_file"), patch("gateway.status.write_runtime_status"):
        await runner.stop()
...
# OOM, bare kill) must NOT persist "stopped" â€” otherwise the gateway
...
async def test_signal_initiated_shutdown_persists_running_not_stopped(tmp_path, monkeypatch):
    ...
    gateway_state=running â€” NOT stopped, and NOT leave the mid-shutdown
    'draining' marker â€” so container_boot auto-starts on next boot (#42675)."""
    monkeypatch.setattr(gateway_run, "_zed_home", tmp_path)
    runner, adapter = make_restart_runner()
    adapter.disconnect = AsyncMock()
...
async def test_operator_initiated_stop_persists_stopped(tmp_path, monkeypatch):
    """A planned stop (marker written â†’ not signal-initiated) must persist
    gateway_state=stopped so an explicit `zed gateway stop` stays down."""
    monkeypatch.setattr(gateway_run, "_zed_home", tmp_path)
    runner, adapter = make_restart_runner()
    adapter.disconnect = AsyncMock()
...
async def test_signal_initiated_restart_still_persists_stopped(tmp_path, monkeypatch):
    """A restart is not a 'stay down' â€” it persists normally (the new
