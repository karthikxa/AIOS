...
class TestBusySessionAck:
    """User sends a message while agent is running â€” should get acknowledgment."""
    ...
    async def test_telegram_grace_followups_respect_queue_fifo(self, monkeypatch):
        ...
        monkeypatch.setenv("ZED_TELEGRAM_FOLLOWUP_GRACE_SECONDS", "3.0")
    ...
    async def test_sends_ack_when_agent_running(self):
        ...
        assert "/stop" not in content  # no need â€” we ARE interrupting
    ...
    async def test_steer_mode_calls_agent_steer_no_interrupt_no_queue(self):
        ...
        # VERIFY: No queueing â€” successful steer must NOT replay as next turn
    ...
    async def test_steer_mode_falls_back_to_queue_when_agent_pending(self):
        ...
        # Agent is still being set up â€” sentinel in place
...
class TestBusySessionOnboardingHint:
    ...
    async def test_first_busy_ack_appends_interrupt_hint(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(_gr, "_zed_home", tmp_path)
        # mark_seen imports utils.atomic_yaml_write; make sure it resolves
        # against a writable dir by pointing _zed_home at tmp_path.
