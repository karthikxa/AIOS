...
class FakeAgent:
    def __init__(self, **kwargs):
        ...
        # freeze it â€” production now assigns tool_progress_callback after
...
class LongPreviewAgent:
    ...
    LONG_CMD = "cd /home/teknium/.zed/zed-agent/.worktrees/zed-d8860339 && source .venv/bin/activate && python -m pytest tests/gateway/test_run_progress_topics.py -n0 -q"
...
async def test_run_agent_progress_stays_in_originating_topic(monkeypatch, tmp_path):
    monkeypatch.setenv("ZED_TOOL_PROGRESS_MODE", "all")
...
def _run_long_preview_helper(monkeypatch, tmp_path, preview_length=0):
    ...
    that _run_agent reads â€” so the gateway picks it up the same way production does.
    """
    import asyncio
    import yaml
...
def test_all_mode_default_truncation_40_chars(monkeypatch, tmp_path):
    ...
    # The long command should be truncated â€” total preview <= 40 chars
    assert "..." in content
...
def test_all_mode_no_truncation_when_preview_fits(monkeypatch, tmp_path):
    ...
    # Set a generous cap â€” the LongPreviewAgent's command is ~165 chars
...
class BackgroundReviewAgent:
    ...
    def run_conversation(self, message, conversation_history=None, task_id=None):
        if self.background_review_callback:
            self.background_review_callback("ðŸ’¾ Skill 'prospect-scanner' created.")
        return {
            "final_response": "done",
            "messages": [],
            "api_calls": 1,
...
async def test_run_agent_matrix_streaming_omits_cursor(monkeypatch, tmp_path):
    ...
    assert all("â–‰" not in text for text in all_text)
...
async def test_transformed_response_edits_streamed_message_in_place(monkeypatch, tmp_path):
    ...
    assert result.get("already_sent") is True
    # The transformed final text reached the user â€” appended portion is present
...
async def test_base_processing_releases_post_delivery_callback_after_main_send():
    ...
    def _post_delivery_cb():
        ...
        adapter.sent.append(
            {
                "chat_id": "bg-review",
                "content": "ðŸ’¾ Skill 'prospect-scanner' created.",
                "reply_to": None,
                "metadata": None,
            }
        )
    ...
    sent_texts = [call["content"] for call in adapter.sent]
    assert sent_texts == ["done", "ðŸ’¾ Skill 'prospect-scanner' created."]
...
async def test_base_processing_stops_typing_before_hung_post_delivery_callback(
    ...
    # starts.  Don't pin the exact stop_typing call count â€” the shared
...
async def test_verbose_mode_does_not_truncate_args_by_default(monkeypatch, tmp_path):
    ...
    was 0 (default).  The user explicitly opted into verbose â€” show full detail.
    """
    adapter, result = await _run_with_agent(
        monkeypatch,
        tmp_path,
...
async def test_verbose_mode_respects_explicit_tool_preview_length(monkeypatch, tmp_path):
    ...
    all_content = " ".join(call["content"] for call in adapter.sent)
    all_content += " ".join(call["content"] for call in adapter.edits)
    # Should be truncated â€” full 300-char string NOT present
...
async def test_terminal_progress_renders_fenced_code_block(monkeypatch, tmp_path):
    ...
    renders a bare fenced code block â€” no language tag (Slack mrkdwn would print
    ...
    # Non-verbose collapses to the first line + truncation marker â€” the later
...
async def test_terminal_progress_verbose_shows_full_command(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("ZED_TOOL_PROGRESS_MODE", "verbose")
    ...
    assert "```" in all_content
    assert "```bash" not in all_content
    # Full command body present â€” verbose is uncapped.
...
class MultiTerminalCommandAgent:
    ...
    different tool, then terminal again â€” to exercise header collapsing."""
