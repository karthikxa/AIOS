...
class TestBlueBubblesHelpers:
    ...
    def test_default_mention_patterns_match_zed_variants(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter.require_mention is True
        assert adapter._message_matches_mention_patterns("Zed, summarize this")
        assert adapter._message_matches_mention_patterns("@Zed agent help")
        assert not adapter._message_matches_mention_patterns("casual family chatter")
        assert not adapter._message_matches_mention_patterns("antized should not match")
    ...
    def test_custom_mention_patterns_override_defaults(self, monkeypatch):
        ...
        assert not adapter._message_matches_mention_patterns("Zed what is next?")

    def test_clean_mention_text_strips_leading_wake_word(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter._clean_mention_text("Zed, summarize this") == "summarize this"
        assert adapter._clean_mention_text("Zed agent: summarize this") == "summarize this"
        assert adapter._clean_mention_text("please ask Zed about this") == "please ask Zed about this"
...
class TestBlueBubblesGuidResolution:
    def test_raw_guid_returned_as_is(self, monkeypatch):
        """If target already contains ';' it's a raw GUID â€” return unchanged."""
        adapter = _make_adapter(monkeypatch)
        import asyncio
...
class TestBlueBubblesAttachmentDownload:
    ...
    def test_download_returns_none_without_client(self, monkeypatch):
        """No client â†’ returns None gracefully."""
        adapter = _make_adapter(monkeypatch)
        adapter.client = None
        import asyncio
...
class TestBlueBubblesWebhookUrl:
    ...
    def test_default_host(self, monkeypatch):
        adapter = _make_adapter(monkeypatch)
        # Default webhook_host is 0.0.0.0 â†’ normalized to localhost
...
class TestBlueBubblesWebhookRegistration:
    ...
    def test_register_fresh(self, monkeypatch):
        """No existing webhook â†’ POST creates one."""
        import asyncio
        adapter = _make_adapter(monkeypatch)
        adapter.client = self._mock_client(
            get_response={"status": 200, "data": []},
    ...
    def test_register_accepts_201(self, monkeypatch):
        """BB might return 201 Created â€” must still succeed."""
        import asyncio
        adapter = _make_adapter(monkeypatch)
        adapter.client = self._mock_client(
            get_response={"status": 200, "data": []},
    ...
    def test_register_reuses_existing(self, monkeypatch):
        """Crash resilience â€” existing registration is reused, no POST needed."""
        import asyncio
        adapter = _make_adapter(monkeypatch)
    ...
    def test_unregister_removes_all_duplicates(self, monkeypatch):
        """Multiple orphaned registrations for same URL â€” all get removed."""
        import asyncio
        adapter = _make_adapter(monkeypatch)
        url = adapter._webhook_register_url
        deleted_ids = []
