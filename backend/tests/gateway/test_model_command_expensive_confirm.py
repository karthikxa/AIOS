...
previously bypassed the guard entirely â€” a user typing
``/model openai/gpt-5.5-pro`` switched silently while the picker warned.
These tests pin the typed path:

- warning fires â†’ handler returns the slash-confirm prompt, switch NOT applied
- confirm ("once") â†’ switch applies (session override set)
- cancel â†’ switch not applied, current model unchanged
- no warning (cheap model) â†’ switch applies immediately, no prompt
"""
...
def _fake_warning():
    return SimpleNamespace(
        message=(
            ...
            "openai/gpt-5.5-pro has known pricing above Zed' safety threshold.\n"
...
async def test_typed_model_expensive_prompts_instead_of_switching(tmp_path, monkeypatch):
    """Expensive model typed directly â†’ confirm prompt, no switch applied."""
...
async def test_typed_model_cheap_switches_without_prompt(tmp_path, monkeypatch):
    """No warning â†’ switch applies immediately; confirm primitive never invoked."""
