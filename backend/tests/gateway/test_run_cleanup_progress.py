...
tool-progress bubble, "â³ Working â€” N min" heartbeats, and status-callback
...
# ---------------------------------------------------------------------------
# Test fakes â€” mirror those in test_run_progress_topics.py but add a
...
class NoDeleteAdapter(CleanupCaptureAdapter):
    ...
    async def delete_message(self, chat_id, message_id) -> bool:  # type: ignore[override]
        ...
        raise AssertionError("should not be called â€” cleanup must skip this adapter")
...
def _install_fakes(monkeypatch, agent_cls, *, cleanup_on: bool):
    ...
    import tools.terminal_tool  # noqa: F401 â€” register tool emoji
...
async def test_cleanup_registers_callback_and_deletes_on_success(monkeypatch, tmp_path):
    ...
    cb()
    # delete_message is scheduled via run_coroutine_threadsafe â†’ give the
...
async def test_cleanup_skipped_on_failed_run(monkeypatch, tmp_path):
    """Failed runs skip cleanup registration â€” breadcrumbs stay."""
    adapter = CleanupCaptureAdapter()
    runner = _make_runner(adapter)
    ...
    # Whatever callback is registered should not trigger any deletion â€”
...
async def test_cleanup_noop_on_adapter_without_delete_support(monkeypatch, tmp_path):
    ...
    detected up front â€” the cleanup path never registers its callback so
...
async def test_cleanup_chains_with_existing_callback(monkeypatch, tmp_path):
    ...
    callback chains with it â€” both fire, neither clobbers the other."""
    adapter = CleanupCaptureAdapter()
    runner = _make_runner(adapter)
    gateway_run = _install_fakes(monkeypatch, ProgressAgent, cleanup_on=True)
    monkeypatch.setattr(gateway_run, "_zed_home", tmp_path)
    ...
    # (run_generation=None in this test path â€” matches the default slot).
