...
NEGATIVE_CASES = [
    "Silence is golden â€” here is the plan...",
    ...
    "silently",  # word boundary â€” trailing letters mean it isn't a bare token
...
def test_length_guard_rejects_long_strings():
    # Exactly 65 chars of dots â€” over the 64-char guard, so not treated as narration.
...
async def test_silence_narration_dropped_pre_send(tmp_path, monkeypatch):
    monkeypatch.setattr("gateway.delivery.get_zed_home", lambda: tmp_path)
    monkeypatch.delenv("ZED_FILTER_SILENCE_NARRATION", raising=False)
...
async def test_real_message_is_delivered(tmp_path, monkeypatch):
    ...
    result = await router._deliver_to_platform(
        target, "Silence is golden â€” here is the plan...", metadata=None
    )

    assert len(adapter.calls) == 1
    assert adapter.calls[0]["content"] == "Silence is golden â€” here is the plan..."
...
async def test_env_override_disables_filter(tmp_path, monkeypatch):
    monkeypatch.setattr("gateway.delivery.get_zed_home", lambda: tmp_path)
    monkeypatch.setenv("ZED_FILTER_SILENCE_NARRATION", "0")
    adapter = RecordingAdapter()
    # Config default is True, but env override wins.
    router = DeliveryRouter(GatewayConfig(), adapters={Platform.DISCORD: adapter})
    target = DeliveryTarget.parse("discord:99887766")

    result = await router._deliver_to_platform(target, "ðŸ”‡", metadata=None)
...
async def test_env_override_enables_filter_over_config(tmp_path, monkeypatch):
    monkeypatch.setattr("gateway.delivery.get_zed_home", lambda: tmp_path)
    monkeypatch.setenv("ZED_FILTER_SILENCE_NARRATION", "1")
    adapter = RecordingAdapter()
