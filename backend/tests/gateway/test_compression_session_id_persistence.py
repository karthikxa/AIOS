"""Regression tests for #29335 â€” gateway must persist ``session_entry.session_id``
...
def _session_id_assignments_followed_by_save(source: str) -> list[tuple[int, bool]]:
    ...
    return ``(lineno, saved_within_5_stmts)`` â€” True iff a
...
def test_every_post_compression_session_id_assignment_persists():
    ...
    Regression for #29335 â€” the assignment at the end of
    ...
    assert assignments, (
        "No ``session_entry.session_id = ...`` assignments found in gateway/run.py â€” "
...
class TestCompressionSessionPropagation:
    ...
    def test_gateway_session_entry_follows_compression_rotation(self) -> None:
        ...
        exactly once â€” the minimal contract that prevents the restart-loop bug.
        """
    ...
    def test_no_update_when_session_id_unchanged(self) -> None:
        ...
        session_entry must not be touched and _save must not be called â€” avoiding
    ...
    def test_contextvar_and_session_entry_agree_after_compression(self) -> None:
        ...
        contextvar_sid = get_session_env("ZED_SESSION_ID", "")
