...
DM thread sessions must start empty â€” no parent transcript seeding.
...
def store(tmp_path, monkeypatch):
    """SessionStore with SQLite â€” load_transcript reads from DB only.

    Pin DEFAULT_DB_PATH to tmp_path so SessionDB() can't write to the real
    ~/.zed/state.db. (DEFAULT_DB_PATH is a module-level constant computed
...
class TestDMThreadIsolation:
    """Thread sessions must start empty â€” no parent transcript seeding."""
...
class TestDMThreadIsolationEdgeCases:
    """Edge cases â€” threads always start empty regardless of context."""
