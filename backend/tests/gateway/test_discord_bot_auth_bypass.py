...
The bug had two sequential gates both rejecting bot messages:
  ...
  Gate 1 â€” `on_message` in gateway/platforms/discord.py ran the user-allowlist
  ...
  Gate 2 â€” `_is_user_authorized` in gateway/run.py rejected bots at the
...
def _make_bare_runner():
    ...
    Uses ``object.__new__`` to skip the heavy __init__ â€” many gateway tests
...
def test_discord_bot_authorized_when_allow_bots_mentions(monkeypatch):
    ...
    This is the exact scenario from #4466 â€” a Cloudflare Worker webhook
    posts Notion events to Discord, the Zed bot gets @mentioned, and
...
def test_discord_bot_authorized_when_allow_bots_all(monkeypatch):
    """DISCORD_ALLOW_BOTS=all is a superset of =mentions â€” should also bypass."""
    runner = _make_bare_runner()
...
def test_discord_bot_NOT_authorized_when_allow_bots_none(monkeypatch):
    ...
    in DISCORD_ALLOWED_USERS â€” preserves the original security behavior.
    """
    runner = _make_bare_runner()
...
def test_discord_human_still_checked_against_allowlist_when_bot_policy_set(monkeypatch):
    """DISCORD_ALLOW_BOTS=all must NOT open the gate for humans â€” they
    ...
    # Human NOT on the allowlist â†’ must be rejected.
    source = _make_discord_human_source(user_id="999999999")
    assert runner._is_user_authorized(source) is False

    # Human ON the allowlist â†’ accepted.
...
def test_bot_bypass_does_not_leak_to_other_platforms(monkeypatch):
    """The is_bot bypass is Discord-specific â€” a Telegram bot source with
...
# removed the shortcut â€” Discord auth now flows through the same allowlist
...
def test_discord_role_config_does_not_bypass_gateway_allowlist(monkeypatch):
    ...
    # DISCORD_ALLOWED_USERS deliberately NOT set â€” verifies the role
