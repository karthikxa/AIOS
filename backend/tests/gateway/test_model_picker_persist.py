...
*typing* the same model persisted â€” a contradiction the same PR introduced.
...
callback and assert ``config.yaml`` is (or isn't) updated â€” exercising the exact
closure the PR changed, against a real temp ``ZED_HOME``.
"""
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    # the network (OpenRouter model catalog). Stub it to a minimal list â€” these
...
async def test_picker_tap_persists_by_default(tmp_path, monkeypatch, seed_model):
    ...
    matching the typed ``/model`` default â€” this is the #49176 fix. The written
...
async def test_picker_tap_session_flag_does_not_persist(tmp_path, monkeypatch):
    """``/model --session`` then a picker tap stays in-memory only â€” config
    ...
    # But config.yaml is untouched â€” the override is in-memory only.
    written = yaml.safe_load(cfg_path.read_text(encoding="utf-8"))
