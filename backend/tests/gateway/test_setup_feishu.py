"""Tests for _setup_feishu() in zed_cli/gateway.py.
...
def _run_setup_feishu(
    *,
    ...
    # QR path: method(0), dm(0), group(0) â€” 3 choices (no connection mode)
    # Manual path: method(1), domain(0), connection(0), dm(0), group(0) â€” 5 choices
...
class TestSetupFeishuQrPath:
    ...
    def test_qr_success_does_not_persist_bot_identity(self):
        """Bot identity is discovered at runtime by _hydrate_bot_identity â€” not persisted
...
# ---------------------------------------------------------------------------
# Adapter integration: env vars â†’ FeishuAdapterSettings
# ---------------------------------------------------------------------------
...
class TestSetupFeishuAdapterIntegration:
    ...
    def test_qr_env_produces_valid_adapter_settings(self):
        """QR setup â†’ adapter initializes with websocket mode."""
    ...
    def test_open_dm_env_sets_correct_adapter_state(self):
        """Setup with 'allow all DMs' â†’ adapter sees allow-all flag."""
    ...
    def test_group_open_env_sets_adapter_group_policy(self):
        """Setup with 'open groups' â†’ adapter group_policy is 'open'."""
