"""Tests for gateway proxy mode â€” forwarding messages to a remote API server."""
    ...
    async def test_run_agent_skips_proxy_when_not_configured(self, monkeypatch):
        ...
        with patch("gateway.run._load_gateway_config", return_value={}):
            try:
                ...
                pass  # Expected â€” bare runner can't create a real agent
...
class TestRunAgentViaProxy:
    ...
    async def test_builds_correct_request(self, monkeypatch):
        ...
        assert session.captured_headers["X-Zed-Session-Id"] == "session-abc"
