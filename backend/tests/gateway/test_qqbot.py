...
class TestChunkedUploadHelpers:
    ...
    def test_compute_hashes_large_file_has_distinct_md5_10m(self, tmp_path):
        # File > 10,002,432 bytes â†’ md5_10m is truncated, so it differs from full md5.
...
class TestChunkedUploaderFlow:
    ...
    async def test_full_upload_two_parts_success(self, tmp_path):
        ...
        # Mock api_request â€” handles prepare, part_finish, complete based on URL.
        api_calls = []
        ...
        # Mock http_put â€” always returns 200.
        put_calls = []
    ...
    async def test_part_finish_retries_on_40093001_then_succeeds(self, tmp_path):
        """biz_code 40093001 is retryable â€” finish-with-retry must keep trying."""
        from gateway.platforms.qqbot.chunked_upload import ChunkedUploader
...
# ---------------------------------------------------------------------------
# Inline keyboards â€” approval + update-prompt flows
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Quoted-message handling (message_type=103 â†’ msg_elements)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Gateway-contract approval UX â€” send_exec_approval + default dispatcher
# ---------------------------------------------------------------------------
...
class TestDefaultInteractionDispatch:
    ...
    async def test_approval_click_once_maps_to_once(self):
        """'allow-once' button â†’ resolve_gateway_approval(session, 'once')."""
        adapter = self._make_adapter()
    ...
    async def test_update_prompt_click_writes_response_file(self, tmp_path, monkeypatch):
        """update_prompt:y click writes 'y' to ~/.zed/.update_response."""
        adapter = self._make_adapter()
        ...
        response = zed_home / ".update_response"
    ...
    async def test_unknown_button_data_is_harmless(self):
        """Unrecognised button_data is logged and dropped â€” no exception."""
        adapter = self._make_adapter()
...
class TestSendExecApproval:
    ...
    async def test_accepts_metadata_arg(self):
        """Gateway always passes metadata=â€¦; the adapter must accept + ignore it."""
        adapter = self._make_adapter()
...
class TestOp7ServerReconnect:
    ...
    def test_op7_closes_websocket(self):
        ...
        # But the task was created â€” verify via asyncio
