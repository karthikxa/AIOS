...
Context: #18451 â€” on macOS behind Cloudflare Warp, httpx's default
...
def _clear_env(monkeypatch):
    monkeypatch.delenv("ZED_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", raising=False)
    monkeypatch.delenv("ZED_GATEWAY_HTTPX_MAX_KEEPALIVE", raising=False)
...
def test_default_limits_tighten_keepalive_below_httpx_default():
    ...
    # httpx default keepalive_expiry is 5.0 â€” ours must be shorter so
...
def test_env_override_keepalive_expiry(monkeypatch):
    monkeypatch.setenv("ZED_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", "7.5")
    from gateway.platforms._http_client_limits import platform_httpx_limits
    limits = platform_httpx_limits()
...
def test_env_override_max_keepalive(monkeypatch):
    monkeypatch.setenv("ZED_GATEWAY_HTTPX_MAX_KEEPALIVE", "25")
    from gateway.platforms._http_client_limits import platform_httpx_limits
    limits = platform_httpx_limits()
...
def test_env_override_rejects_garbage(monkeypatch):
    ...
    monkeypatch.setenv("ZED_GATEWAY_HTTPX_KEEPALIVE_EXPIRY", "not-a-number")
    monkeypatch.setenv("ZED_GATEWAY_HTTPX_MAX_KEEPALIVE", "-3")
    from gateway.platforms._http_client_limits import platform_httpx_limits
    limits = platform_httpx_limits()
    # Non-positive / non-numeric â†’ fell back to defaults (not the override values)
...
class TestWhatsappTypingLeakFix:
    """#18451 â€” whatsapp.send_typing previously used a bare
    ...
    the coroutine â€” the test suite would otherwise need a live
