"""Tests for gateway session hygiene â€” auto-compression of large sessions.
...
The hygiene system uses the SAME compression config as the agent:
  compression.threshold Ã— model context length
...
class TestSessionHygieneThresholds:
    ...
    Thresholds are derived from model context length Ã— compression threshold,
    ...
    def test_under_threshold_no_trigger(self):
        ...
        # 250 short messages â€” lots of messages but well under token threshold
    ...
    def test_message_count_alone_does_not_trigger(self):
        """Message count alone should NOT trigger â€” only token count matters.
        ...
        """
        # 300 very short messages â€” old system would compress, new should not
...
class TestEstimatedTokenThreshold:
    ...
    limit â€” for both actual and estimated token counts.
    ...
    The fix removed the multiplier entirely â€” the 85% threshold already
    ...
    def test_no_multiplier_means_same_threshold_for_estimated_and_actual(self):
        ...
        # Both paths should use 170K â€” no inflation
    ...
    def test_overestimate_fires_early_but_safely(self):
        ...
        That's between the agent's 50% threshold and the model's limit â€”
        safe and harmless.
        """
        context_length = 200_000
...
async def test_session_hygiene_messages_stay_in_originating_topic(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway_run, "_zed_home", tmp_path)
    ...
    assert result == "ok"
    # Compression warnings are no longer sent to users â€” compression
...
async def test_session_hygiene_warns_user_when_compression_aborts(monkeypatch, tmp_path):
    ...
    ABORTS â€” returns messages unchanged, sets _last_compress_aborted=True,
    and drops nothing.  Gateway must surface a visible âš ï¸ warning to the
    ...
    class FakeCompressAgentWithSummaryFailure:
        ...
        def __init__(self, **kwargs):
            ...
            # and ABORTED â€” no fallback inserted, no messages dropped.
    ...
    assert result == "ok"
    # The compressor reported abort â†’ exactly one warning message must
...
async def test_session_hygiene_informs_user_when_aux_model_fails_but_recovers(monkeypatch, tmp_path):
    ...
    config is still broken.  Gateway hygiene must surface an â„¹ note so the
    user knows to fix ``auxiliary.compression.model`` â€” silent recovery
    ...
    assert result == "ok"
    # No âš ï¸ hard-failure warning (that's for dropped turns)
    ...
    # But an â„¹ note about the configured aux model must be delivered.
    aux_notes = [
...
async def test_session_hygiene_honors_configurable_hard_message_limit(
    ...
    # 12 messages: below 400 default â†’ no compression without override,
    # but above the configured limit of 10 â†’ should compress.
    ...
    # won't trigger for 12 short messages â€” hard-limit must be the ONLY
    ...
    assert result == "ok"
    # The compression agent was instantiated â†’ hard-limit fired on the
...
async def test_session_hygiene_default_hard_message_limit_does_not_fire_at_12_messages(
    ...
    # No config.yaml â€” use defaults (hard_limit=400)
    gateway_run = importlib.import_module("gateway.run")
    GatewayRunner = gateway_run.GatewayRunner
    ...
    assert result == "ok"
    # No compression agent instantiated â€” 12 messages well under 400 default.
