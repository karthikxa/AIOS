...
def test_validate_config_uses_env_or_extra():
    ...
    # Empty extra + no env â†’ invalid
    cfg = PlatformConfig(enabled=True)
    assert validate_config(cfg) is False
    # extra-only path â†’ valid
...
def test_corr_id_pending_set_self_trims():
    ...
    # logic â€” at most one trim window above the cap.
...
async def test_send_dm():
    ...
    Zed deployment has been using in production for months.
    """
    from gateway.config import PlatformConfig
    cfg = PlatformConfig(enabled=True, extra={"ws_url": "ws://localhost:5225"})
    adapter = SimplexAdapter(cfg)
...
async def test_send_group():
    ...
    â€” so messages to groups whose display name isn't literally the ID
...
async def test_send_when_ws_not_connected_does_not_crash():
    ...
    cfg = PlatformConfig(enabled=True, extra={"ws_url": "ws://localhost:5225"})
    adapter = SimplexAdapter(cfg)
    # No _ws assigned â€” _send_ws should drop quietly
    result = await adapter.send("contact-42", "hi")
    assert result.success is True  # send() always returns success â€” fire-and-forget
...
# ---------------------------------------------------------------------------
# 10. register() â€” plugin-side metadata
# ---------------------------------------------------------------------------
...
def test_register_calls_register_platform():
    ...
    # SimpleX uses opaque IDs only â€” no PII to redact.
