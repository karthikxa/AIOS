...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        runner = _make_runner()
        event = _make_event()
        monkeypatch.setenv("ZED_MANAGED", "homebrew")

        # Guard: prevent any accidental fall-through from spawning a real
        # `zed update --gateway` against the CI checkout. The managed-install
        ...
        assert "brew upgrade zed-agent" in result
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _zed_home to tmp_path and project_root to a dir without .git
        ...
        # Simpler approach â€” mock at method level using a wrapper
        runner = _make_runner()
        ...
        with patch("gateway.run._zed_home", tmp_path):
            ...
            # project_root will be the real zed-agent dir (which HAS .git).
    ...
    @pytest.mark.asyncio
    async def test_no_zed_binary(self, tmp_path):
        """Returns error when zed is not on PATH and zed_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
    ...
    async def test_fallback_to_sys_executable(self, tmp_path):
        ...
        with patch("gateway.run._zed_home", zed_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", return_value=None), \
    ...
    async def test_resolve_zed_bin_prefers_which(self, tmp_path):
        """_resolve_zed_bin returns argv parts from shutil.which when available."""
        from gateway.run import _resolve_zed_bin

        with patch("shutil.which", return_value="/custom/path/zed"):
            result = _resolve_zed_bin()

        assert result == ["/custom/path/zed"]

    @pytest.mark.asyncio
    async def test_resolve_zed_bin_fallback(self):
        """_resolve_zed_bin falls back to sys.executable argv when which fails."""
        import sys
        from gateway.run import _resolve_zed_bin
    ...
    async def test_resolve_zed_bin_returns_none_when_both_fail(self):
        """_resolve_zed_bin returns None when both strategies fail."""
        from gateway.run import _resolve_zed_bin
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._zed_home", zed_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", side_effect=lambda x: "/usr/bin/zed" if x == "zed" else "/usr/bin/setsid"), \
             patch("subprocess.Popen"):
            result = await runner._handle_update_command(event)

        pending_path = zed_home / ".update_pending.json"
        ...
        assert not (zed_home / ".update_exit_code").exists()
    ...
    async def test_writes_pending_marker_with_thread_id(self, tmp_path):
        ...
        data = json.loads((zed_home / ".update_pending.json").read_text())
    ...
    async def test_popen_failure_cleans_up(self, tmp_path):
        ...
        assert not (zed_home / ".update_pending.json").exists()
        assert not (zed_home / ".update_exit_code").exists()
...
class TestUpdateCommandPlatformGate:
    ...
    mattermost, teams, â€¦) are NOT in the frozenset and rely on the
    ...
    async def test_blocks_programmatic_interface(self, monkeypatch):
        ...
        monkeypatch.setenv("ZED_MANAGED", "")
    ...
    async def test_allows_plugin_platform_via_registry_fallback(self, monkeypatch):
        ...
        # The gate must NOT have rejected us â€” anything other than the
        # ``platform_not_messaging`` rejection string is acceptable here.
        # Later steps may legitimately return success ("Starting Zed
        # updateâ€¦") or fail for environment reasons.
    ...
    async def test_allows_builtin_platform_in_allowlist(self, monkeypatch):
        """``Platform.TELEGRAM`` is in the hardcoded allowlist â€” gate
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (zed_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = zed_home / ".update_pending.claimed.json"
        claimed_path.write_text(json.dumps({
            "platform": "telegram", "chat_id": "67890", "user_id": "12345",
        }))
        (zed_home / ".update_output.txt").write_text("done")
        (zed_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_notification_with_output(self, tmp_path):
        ...
        (zed_home / ".update_pending.json").write_text(json.dumps(pending))
        (zed_home / ".update_output.txt").write_text(
            "â†’ Found 3 new commit(s)\nâœ“ Code updated!\nâœ“ Update complete!"
        )
        (zed_home / ".update_exit_code").write_text("0")
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (zed_home / ".update_pending.json").write_text(json.dumps(pending))
        (zed_home / ".update_output.txt").write_text("x" * 5000)
        (zed_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (zed_home / ".update_pending.json").write_text(json.dumps(pending))
        (zed_home / ".update_output.txt").write_text("Traceback: boom")
        (zed_home / ".update_exit_code").write_text("1")
    ...
    async def test_cleans_up_files_after_notification(self, tmp_path):
        ...
        pending_path = zed_home / ".update_pending.json"
        output_path = zed_home / ".update_output.txt"
        exit_code_path = zed_home / ".update_exit_code"
        pending_path.write_text(json.dumps({
            "platform": "telegram", "chat_id": "111", "user_id": "222",
        }))
        output_path.write_text("âœ“ Done")
    ...
    async def test_no_adapter_for_platform_preserves_markers(self, tmp_path):
        ...
        assert result is False
        mock_adapter.send.assert_not_called()
        # Markers are preserved for a later retry â€” NOT cleaned up.
        assert pending_path.exists()
        assert output_path.exists()
        assert exit_code_path.exists()
        # The marker stays in its canonical pending location (claim restored).
        assert not (zed_home / ".update_pending.claimed.json").exists()
    ...
    async def test_deferred_notification_delivers_after_reconnect(self, tmp_path):
        ...
        exit_code_path = zed_home / ".update_exit_code"
        pending_path.write_text(json.dumps(pending))
        output_path.write_text("âœ“ Update complete!")
        exit_code_path.write_text("0")

        # First pass: target platform (discord) is still offline â†’ defer.
        with patch("gateway.run._zed_home", zed_home):
        ...
        # Now everything is cleaned up â€” no duplicate deliveries possible.
        assert not pending_path.exists()
        assert not output_path.exists()
        assert not exit_code_path.exists()
        assert not (zed_home / ".update_pending.claimed.json").exists()
