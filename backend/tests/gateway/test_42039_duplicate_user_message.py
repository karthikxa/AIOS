"""Tests for #42039 â€” user messages stored twice in state.db.
...
1. ``agent_failed_early`` path â€” transient 429/timeout failures
2. ``not new_messages`` path â€” edge case where ``history_offset`` exceeds
...
# â”€â”€ Test 1: agent_failed_early path uses skip_db=True â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async def test_agent_failed_early_skip_db_when_agent_has_session_db(
    ...
    runner._run_agent = AsyncMock(
        return_value={
            "failed": True,
            "final_response": None,
            "error": "429 Too Many Requests â€” rate limit exceeded",
            "messages": [],
            "history_offset": 0,
...
# â”€â”€ Test 2: agent_failed_early with no _session_db â†’ skip_db not True â”€
...
async def test_agent_failed_early_no_skip_db_when_no_session_db(
    ...
    runner = _bootstrap(monkeypatch, tmp_path)
    runner._session_db = None  # No agent DB â†’ agent_persisted=False
...
# â”€â”€ Test 3: not-new-messages path uses skip_db=True â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async def test_not_new_messages_skip_db_when_agent_has_session_db(
    ...
    # Agent succeeds but history_offset equals messages length â†’ no new messages
    runner._run_agent = AsyncMock(
        return_value={
            "final_response": "Hello!",
            "messages": [{"role": "user", "content": "hi"}],
            "tools": [],
            "history_offset": 1,  # equals len(messages) â†’ new_messages=[]
            "last_prompt_tokens": 0,
        }
    )
...
# â”€â”€ Test 4: normal path (new_messages found) uses skip_db=True â”€â”€â”€â”€â”€â”€â”€â”€
