...
async def test_internal_event_bypasses_authorization(monkeypatch, tmp_path):
    ...
    async def _raise(*_a, **_kw):
        raise RuntimeError("sentinel â€” stop here")
...
async def test_non_internal_event_without_user_triggers_pairing(monkeypatch, tmp_path):
    ...
    # import time from whichever ZED_HOME was set then. Per-test
    # ZED_HOME redirection in conftest doesn't retroactively move it.
    ...
    # module-level dotenv in gateway/run.py from the real ~/.zed/.env).
