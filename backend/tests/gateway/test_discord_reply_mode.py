...
def _make_discord_adapter(reply_to_mode: str = "first"):
    ...
    # ref_message.to_reference() â†’ a distinct sentinel: the adapter now wraps
...
class TestSendWithReplyToMode:
    ...
    async def test_invalid_mode_falls_back_to_first_behavior(self):
        """Invalid mode behaves like 'first' â€” only first chunk gets reference."""
...
# library is installed â€” this guarantees isinstance() checks pass in
...
class FakeDMChannel(_DMChannelBase):
    ...
    def __init__(self, channel_id: int = 100, name: str = "dm"):
        # Do NOT call super().__init__() â€” real DMChannel requires State
        self.id = channel_id
        self.name = name
...
def reply_text_adapter(monkeypatch):
    """DiscordAdapter wired for _handle_message â†’ handle_message capture."""
...
class TestReplyToText:
    ...
    async def test_reference_with_deleted_message(self, reply_text_adapter):
        """Deleted messages lack .content â€” getattr guard should return None."""
...
class TestYamlConfigLoading:
    ...
    def test_top_level_reply_to_mode_off(self, tmp_path, monkeypatch):
        """YAML 1.1 parses bare 'off' as boolean False â€” must map back to 'off'."""
        zed_home = self._write_config(tmp_path, "discord:\n  reply_to_mode: off\n")
