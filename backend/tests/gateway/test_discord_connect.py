...
class _FakeAllowedMentions:
    """Stand-in for ``discord.AllowedMentions`` â€” exposes the same four
...
def _ensure_discord_mock():
    ...
    Always force ``AllowedMentions`` onto whatever is in ``sys.modules`` â€”
...
class FakeBot:
    def __init__(self, *, intents, proxy=None, allowed_mentions=None, **_):
        ...
        self.user = SimpleNamespace(id=999, name="Zed")
        self._events = {}
        self.tree = FakeTree()
...
async def test_reconnect_closes_previous_client_to_prevent_zombie_websocket(monkeypatch):
    ...
    # First connect â€” fresh adapter, no prior client.
    assert await adapter.connect() is True
    assert len(created) == 1
    first_bot = created[0]
    assert first_bot._closed is False, "first bot should still be open after connect()"

    # Second connect WITHOUT disconnect â€” simulates an in-process reconnect.
...
async def test_connect_timeout_cancels_bot_task(monkeypatch):
    ...
    class NeverReadyBot(FakeBot):
        """Bot whose start() never fires on_ready â€” simulates a slow gateway handshake."""
        async def start(self, token):
...
async def test_safe_sync_slash_commands_only_mutates_diffs():
    ...
    desired_same = {
        "name": "status",
        "description": "Show Zed session status",
        "type": 1,
        "options": [],
        "nsfw": False,
        "dm_permission": True,
...
async def test_post_connect_initialization_skips_same_fingerprint_after_success(tmp_path, monkeypatch):
    adapter = DiscordAdapter(PlatformConfig(enabled=True, token="test-token"))
    monkeypatch.setattr("hermes_constants.get_zed_home", lambda: tmp_path)

    class _DesiredCommand:
        def to_dict(self, tree):
            return {
                "name": "status",
                "description": "Show Zed status",
                "type": 1,
                "options": [],
            }
...
async def test_post_connect_initialization_reraises_non_rate_limit_exceptions(tmp_path, monkeypatch):
    ...
    class _DesiredCommand:
        def to_dict(self, tree):
            return {"name": "status", "description": "Show Zed status", "type": 1, "options": []}
    ...
    # caught by the rate-limit handler â€” it has nothing to do with 429s.
    ...
    # that logs defensively â€” so we assert on state NOT being written.
    await adapter._run_post_connect_initialization()
...
async def test_safe_sync_reads_permission_attrs_from_existing_command():
    ...
    nsfw, dm_permission, or default_member_permissions â€” they live only
    ...
    class _ExistingCommand:
        """Mirrors discord.py's AppCommand â€” to_dict() omits nsfw/dm/perms."""
        ...
        def to_dict(self):
            # Match real AppCommand.to_dict() â€” no nsfw/dm_permission/default_member_permissions
            return {
                "id": self.id,
                "type": 1,
                "application_id": 999,
    ...
    # Existing command has matching attrs â€” should report unchanged, NOT falsely diff.
    existing = _ExistingCommand(
        42,
        "admin",
        "Admin-only command",
