"""Tests for gateway/platforms/base.py â€” MessageEvent, media extraction, message truncation."""
...
# ---------------------------------------------------------------------------
# MessageEvent â€” command parsing
# ---------------------------------------------------------------------------
...
class TestExtractMedia:
    ...
    def test_as_document_directive_stripped_from_cleaned_text(self):
        """[[as_document]] is a routing directive â€” strip it from
    ...
    def test_as_document_directive_alone_does_not_attach_voice_flag(self):
        """[[as_document]] is independent of [[audio_as_voice]] â€” combining
    ...
    # Windows path support â€” regression coverage for #34632
    ...
    def test_media_mixed_code_and_prose(self):
        ...
        # survive verbatim â€” masking is a locator, not a text rewrite.
        assert "```text\nMEDIA:/example/path.pdf\n```" in cleaned
...
class TestMediaInsideSerializedJson:
    """Regression coverage for #34375 â€” MEDIA: embedded in serialized JSON
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.zed/media/'
    ...
    def test_json_embedded_media_kept_verbatim_in_cleaned_text(self):
        ...
        # The JSON-embedded path must survive verbatim â€” not blanked to spaces.
...
class TestMediaExtensionAllowlistParity:
    """Regression coverage for issue #34517 â€” the MEDIA: extension black hole.
    ...
    the body before extract_local_files could pick up the bare path â€” the file
    ...
    def test_unknown_extension_not_black_holed_by_cleanup(self):
        ...
        body â€” it survives so extract_local_files can still see the bare path,
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("ZED_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("ZED_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_operator_configured_extra_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZED_MEDIA_ALLOW_DIRS", str(extra_root))
    ...
    def test_recency_trust_allows_freshly_produced_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("ZED_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("ZED_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("ZED_MEDIA_TRUST_RECENT_SECONDS", "600")
    ...
    def test_recency_trust_rejects_old_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("ZED_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("ZED_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("ZED_MEDIA_TRUST_RECENT_SECONDS", "60")
    ...
    def test_recency_trust_allows_pdf_in_project_dir(self, tmp_path, monkeypatch):
        ...
        files outside ~/.zed/cache/* were silently dropped, leaving the
...
class TestMediaDeliveryDefaultMode:
    """Default (non-strict) mode â€” denylist gates delivery, nothing else.
    ...
    def _patch_roots(self, monkeypatch, *roots):
        ...
        # Pin strict OFF â€” the public default. Tests that exercise the
        # strict path live in TestMediaDeliveryPathValidation.
        monkeypatch.delenv("ZED_MEDIA_DELIVERY_STRICT", raising=False)
        monkeypatch.delenv("ZED_MEDIA_ALLOW_DIRS", raising=False)

    def test_accepts_stale_file_outside_allowlist(self, tmp_path, monkeypatch):
        """The motivating case â€” agent says ``MEDIA:/home/user/notes.md``
        ...
        old_mtime = time.time() - 7200  # 2 hours ago â€” far outside any window
    ...
    def test_accepts_any_extension_not_on_denylist(self, tmp_path, monkeypatch):
        """No extension allowlist â€” .md, .txt, .json, .py all deliver."""
        self._patch_roots(monkeypatch)
    ...
    def test_denylist_still_blocks_credentials(self, tmp_path, monkeypatch):
        """Default mode is permissive but not naive â€” credential paths
    ...
    def test_denylist_blocks_zed_credentials(self, tmp_path, monkeypatch):
        """~/.zed/.env and ~/.zed/auth.json stay blocked even in
        ...
        fake_home = tmp_path / "home"
        zed_dir = fake_home / ".zed"
        zed_dir.mkdir(parents=True)
        env_file = zed_dir / ".env"
        env_file.write_text("OPENAI_API_KEY=sk-...")
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._ZED_HOME",
            zed_dir,
        )

        assert BasePlatformAdapter.validate_media_delivery_path(str(env_file)) is None

    def test_denylist_blocks_zed_config_in_active_profile(self, tmp_path, monkeypatch):
        ...
        zed_dir = fake_home / ".zed"
        zed_dir.mkdir(parents=True)
        config_file = zed_dir / "config.yaml"
    ...
    def test_denylist_blocks_shared_zed_root_config_for_profiles(self, tmp_path, monkeypatch):
        """Profile-mode gateways must still block the shared Zed root config."""
        self._patch_roots(monkeypatch)

        fake_home = tmp_path / "home"
        profile_home = fake_home / ".zed" / "profiles" / "work"
        profile_home.mkdir(parents=True)
        zed_root = fake_home / ".zed"
        config_file = zed_root / "config.yaml"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._ZED_ROOT",
            zed_root,
        )
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting ZED_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_strict_mode_truthy_aliases(self, monkeypatch, tmp_path):
        """``ZED_MEDIA_DELIVERY_STRICT=true|yes|on|1`` all enable strict mode."""
        self._patch_roots(monkeypatch)
        from gateway.platforms.base import _media_delivery_strict_mode

        for raw in ("1", "true", "TRUE", "yes", "on"):
            monkeypatch.setenv("ZED_MEDIA_DELIVERY_STRICT", raw)
    ...
    def test_root_home_deliverable_is_accepted(self, tmp_path, monkeypatch):
        ...
        deliver â€” the home itself is not a credential location.
        """
        self._patch_roots(monkeypatch)
    ...
    def test_root_home_credential_subdir_still_blocked(self, tmp_path, monkeypatch):
        ...
        separate, more-specific denylist entry â€” even when $HOME is itself a
        denied prefix.
        """
        self._patch_roots(monkeypatch)
    ...
    def test_root_home_zed_env_still_blocked(self, tmp_path, monkeypatch):
        """``~/.zed/.env`` stays blocked under the $HOME exception â€” it is a
        ...
        monkeypatch.setattr("gateway.platforms.base._ZED_HOME", zed_dir)
    ...
    def test_other_users_home_still_blocked_for_nonroot(self, tmp_path, monkeypatch):
        ...
        (``/root/...``) â€” that prefix stays denied because it isn't $HOME.
        """
        self._patch_roots(monkeypatch)
...
class TestGetHumanDelay:
    def test_off_mode(self):
        with patch.dict(os.environ, {"ZED_HUMAN_DELAY_MODE": "off"}):
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_default_is_off(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZED_HUMAN_DELAY_MODE", None)
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_natural_mode_range(self):
        with patch.dict(os.environ, {"ZED_HUMAN_DELAY_MODE": "natural"}):
            delay = BasePlatformAdapter._get_human_delay()
            assert 0.8 <= delay <= 2.5

    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "ZED_HUMAN_DELAY_MODE": "natural",
            "ZED_HUMAN_DELAY_MIN_MS": "oops",
            "ZED_HUMAN_DELAY_MAX_MS": "still-bad",
        }
    ...
    def test_custom_mode_uses_env_vars(self):
        env = {
            "ZED_HUMAN_DELAY_MODE": "custom",
            "ZED_HUMAN_DELAY_MIN_MS": "100",
            "ZED_HUMAN_DELAY_MAX_MS": "200",
        }
        with patch.dict(os.environ, env):
            delay = BasePlatformAdapter._get_human_delay()
...
# ---------------------------------------------------------------------------
# Ported from nearai/ironclaw#2304 â€” Telegram counts message length in UTF-16
...
class TestUtf16Len:
    ...
    def test_bmp_cjk(self):
        ...
        assert utf16_len("ä½ å¥½") == 2

    def test_emoji_surrogate_pair(self):
        # ðŸ˜€ (U+1F600) is outside BMP â†’ 2 UTF-16 code units
        assert utf16_len("ðŸ˜€") == 2

    def test_mixed(self):
        # "hiðŸ˜€" = 2 + 2 = 4 UTF-16 units
        assert utf16_len("hiðŸ˜€") == 4

    def test_musical_symbol(self):
        # ð„ž (U+1D11E) â€” Musical Symbol G Clef, surrogate pair
        assert utf16_len("ð„ž") == 2
...
class TestPrefixWithinUtf16Limit:
    ...
    def test_does_not_split_surrogate_pair(self):
        # "aðŸ˜€b" = 1 + 2 + 1 = 4 UTF-16 units; limit 2 should give "a"
        result = _prefix_within_utf16_limit("aðŸ˜€b", 2)
        assert result == "a"
        assert utf16_len(result) <= 2

    def test_emoji_at_limit(self):
        # "ðŸ˜€" = 2 UTF-16 units; limit 2 should include it
        result = _prefix_within_utf16_limit("ðŸ˜€x", 2)
        assert result == "ðŸ˜€"

    def test_all_emoji(self):
        msg = "ðŸ˜€" * 10  # 20 UTF-16 units
...
class TestTruncateMessageUtf16:
    ...
    def test_emoji_near_limit_triggers_split(self):
        ...
        # 2049 emoji = 2049 codepoints but 4098 UTF-16 units â†’ exceeds 4096
        msg = "ðŸ˜€" * 2049
    ...
    def test_each_utf16_chunk_within_limit(self):
        ...
        msg = ("Hello ðŸ˜€ world ðŸŽµ test ð„ž " * 200).strip()
        max_len = 200
    ...
    def test_all_content_preserved(self):
        ...
        words = ["emojiðŸ˜€", "musicðŸŽµ", "cjkä½ å¥½", "plain"] * 100
        msg = " ".join(words)
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"ZED_MEDIA_DELIVERY_STRICT": "1",
                                     "ZED_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZED_MEDIA_DELIVERY_STRICT", "0")
