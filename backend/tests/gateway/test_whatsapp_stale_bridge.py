...
``zed update``, serving pre-update bridge.js behavior forever (e.g.
no inbound media download â†’ images/voice notes arrive as placeholders).
...
class TestStaleBridgeHandshake:
    ...
    async def test_restarts_bridge_on_hash_mismatch(self, tmp_path):
        ...
        # Spawned bridge dies immediately â†’ connect() returns False, but the
    ...
    async def test_restarts_unversioned_bridge(self, tmp_path):
        """Bridges predating the handshake report no scriptHash â†’ stale."""
        bridge_dir = _setup_bridge_dir(tmp_path)
        _fresh_node_modules(bridge_dir)
        adapter = _make_adapter(
            bridge_script=str(bridge_dir / "bridge.js"),
...
class TestDepRefreshStamp:
    ...
    async def test_reinstalls_when_package_json_changed(self, tmp_path):
        bridge_dir = _setup_bridge_dir(tmp_path)
        _fresh_node_modules(bridge_dir)
        # Simulate `zed update` bumping the Baileys pin
        ...
        from gateway.platforms.whatsapp import _file_content_hash
        stamp = (bridge_dir / "node_modules" / ".zed-pkg-hash").read_text().strip()
...
class TestCacheDirEnvPassthrough:
    ...
    async def test_bridge_spawn_env_has_cache_dirs(self, tmp_path):
        ...
        assert env["ZED_IMAGE_CACHE_DIR"] == str(get_image_cache_dir())
        assert env["ZED_AUDIO_CACHE_DIR"] == str(get_audio_cache_dir())
        assert env["ZED_DOCUMENT_CACHE_DIR"] == str(get_document_cache_dir())
