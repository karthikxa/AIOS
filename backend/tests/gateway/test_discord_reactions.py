...
async def test_process_message_background_adds_and_swaps_reactions(adapter):
    ...
    assert raw_message.add_reaction.await_args_list[0].args == ("ðŸ‘€",)
    assert raw_message.remove_reaction.await_args_list[0].args == ("ðŸ‘€", adapter._client.user)
    assert raw_message.add_reaction.await_args_list[1].args == ("âœ…",)
...
async def test_reactions_enabled_by_default(adapter, monkeypatch):
    ...
    raw_message.add_reaction.assert_awaited_once_with("ðŸ‘€")
...
async def test_on_processing_complete_cancelled_removes_eyes_without_terminal_reaction(adapter):
    ...
    raw_message.remove_reaction.assert_awaited_once_with("ðŸ‘€", adapter._client.user)
    raw_message.add_reaction.assert_not_awaited()
