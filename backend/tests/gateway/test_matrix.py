...
class TestMatrixConfigLoading:
    ...
    def test_matrix_user_id_stored_in_extra(self, monkeypatch):
        monkeypatch.setenv("MATRIX_ACCESS_TOKEN", "syt_abc123")
        monkeypatch.setenv("MATRIX_HOMESERVER", "https://matrix.example.org")
        monkeypatch.setenv("MATRIX_USER_ID", "@zed:example.org")
        ...
        mc = config.platforms[Platform.MATRIX]
        assert mc.extra.get("user_id") == "@zed:example.org"
    ...
    async def test_canonical_alias_used_when_name_missing(self):
        ...
        async def get_state_event(room_id, event_type):
            ...
            if event_type == "m.room.canonical_alias":
                return {"content": {"alias": "#zed:ex.org"}}
        ...
        assert identity.display_name == "#zed:ex.org"
        assert identity.chat_type == "room"
...
class TestMatrixBangCommandAlias:
    """Matrix clients may reserve /commands, so Zed supports !commands."""
    ...
    def test_bang_alias_underscore_resolves_to_hyphen_form(self):
        ...
        not itself registered â€” the hyphenated alias set-home is."""
        from gateway.platforms.matrix import _normalize_matrix_bang_command
    ...
    async def test_slash_command_in_quoted_reply_normalizes(self):
        """Sanity: the slash equivalent already works post-strip â€” the bang
...
class TestMatrixModuleImport:
    def test_module_importable_without_mautrix(self):
        ...
        not installed â€” otherwise the gateway crashes for ALL platforms.
        ...
        classes don't share globals with the original â€” breaking patch.object
        in subsequent tests).
        """
        import subprocess
...
class TestMatrixRequirements:
    ...
    def test_check_e2ee_deps_requires_asyncpg(self, monkeypatch):
        """E2EE deps check must reject when asyncpg is missing â€” even if olm is present.
    ...
    def test_check_e2ee_deps_requires_aiosqlite(self):
        ...
        aiosqlite lazily â€” without it, connect fails at ``crypto_db.start()``.
        """
        from gateway.platforms.matrix import _check_e2ee_deps
        import builtins
        real_import = builtins.__import__
    ...
    def test_check_requirements_runs_lazy_install_when_partial(self, monkeypatch):
        ...
        # Simulate "mautrix installed, asyncpg missing" â†’ feature_missing
        # returns a non-empty tuple â†’ ensure_and_bind MUST be called.
...
class TestMatrixDeviceId:
    ...
    def test_device_id_from_config_extra(self):
        ...
        config = PlatformConfig(
            ...
            extra={
                "homeserver": "https://matrix.example.org",
                "device_id": "ZED_BOT_STABLE",
            },
        )
        adapter = MatrixAdapter(config)
        assert adapter._device_id == "ZED_BOT_STABLE"
...
class TestMatrixDeviceIdConfig:
    ...
    def test_device_id_in_config_extra(self, monkeypatch):
        monkeypatch.setenv("MATRIX_ACCESS_TOKEN", "syt_abc123")
        monkeypatch.setenv("MATRIX_HOMESERVER", "https://matrix.example.org")
        monkeypatch.setenv("MATRIX_DEVICE_ID", "ZED_BOT")
        ...
        mc = config.platforms[Platform.MATRIX]
        assert mc.extra.get("device_id") == "ZED_BOT"
...
# ---------------------------------------------------------------------------
# Self / bridge / system sender filtering â€” regression coverage for #15763
...
class TestMatrixSelfSenderFilter:
    ...
    def test_case_insensitive_match_is_self(self):
        ...
        # different API surfaces â€” a case-sensitive equality check lets
...
class TestMatrixSystemBridgeFilter:
    ...
    def test_regular_user_is_not_bridge(self):
        ...
        # bridge â€” the convention is a LEADING underscore.
        assert self.adapter._is_system_or_bridge_sender(
            "@alice_smith:example.org"
        ) is False

    def test_bot_account_is_not_bridge(self):
        # The Zed bot itself (no leading underscore) must not be
        # classified as a bridge â€” that filter is a pairing guard, not
        # a self-filter.
        assert self.adapter._is_system_or_bridge_sender(
...
class TestMatrixOnRoomMessageFilter:
    ...
    async def test_bridge_sender_dropped_before_pairing(self):
        ...
        # gateway â€” otherwise they trigger pairing (#15763).
        self.adapter._handle_text_message.assert_not_called()
    ...
    async def test_self_with_unresolved_user_id_dropped(self):
        # whoami has not resolved yet â†’ user_id empty â†’ drop ALL traffic
...
class TestMatrixClockSkewWarning:
    ...
    async def test_late_drops_emit_one_shot_clock_skew_warning(self, caplog):
        ...
        # Handler should never be invoked â€” all events failed the grace check.
        self.adapter._handle_text_message.assert_not_called()
    ...
    async def test_initial_sync_drops_do_not_warn(self, caplog):
        ...
        now = _t.time()
        # Startup was 1s ago â€” we're still in the initial-sync window.
        ...
        # Backfill drops are silent â€” no clock-skew warning fired.
    ...
    async def test_fewer_than_three_late_drops_do_not_warn(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="gateway.platforms.matrix"):
            for i in range(2):  # only 2 late drops â€” under the threshold
                ev = self._mk_event(
                    sender=f"@alice{i}:example.org", ts_ms=old_ts_ms
                )
                await self.adapter._on_room_message(ev)
    ...
    async def test_state_reset_allows_warning_to_fire_again(self, caplog):
        ...
        the top.  This test exercises the rearm path by:
          ...
          3. Tripping the warning a second time â€” the second warning should
