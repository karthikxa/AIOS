...
Covers the document branch in DiscordAdapter._handle_message() â€”
...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests never write to ~/.zed."""
    monkeypatch.setattr(
...
class TestIncomingDocumentHandling:
    ...
    async def test_oversized_document_skipped(self, adapter):
        """A document over 32MB should be skipped â€” media_urls stays empty."""
        msg = make_message([
            make_attachment(
                filename="huge.pdf",
...
class TestAllowAnyAttachment:
    ...
    async def test_unknown_type_skipped_by_default(self, adapter):
        ...
        to dispatch the event at all, so we don't assert on call_args here â€”
    ...
    async def test_unknown_type_cached_when_flag_on(self, adapter):
        ...
        assert event.message_type == MessageType.DOCUMENT
        # We deliberately do NOT inline arbitrary bytes â€” run.py emits the
    ...
    async def test_max_attachment_bytes_zero_means_unlimited(self, adapter):
        ...
        # 64 MiB â€” would normally exceed the historical 32 MiB hardcoded cap.
    ...
    async def test_allowlisted_doc_unchanged_when_flag_on(self, adapter):
        ...
        and the MIME should still be the canonical text/plain â€” not whatever
        discord guessed.
        """
        adapter.config.extra["allow_any_attachment"] = True
