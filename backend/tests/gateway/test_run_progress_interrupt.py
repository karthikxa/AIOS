...
tool calls, the gateway's progress_callback should stop queuing ðŸ” bubbles
...
response â€” making the interrupt feel ignored.
"""
...
class PreInterruptAgent:
    ...
    interrupted case â€” proves the harness renders events when no
    interrupt is active.
    """
...
class InterruptedAgent:
    """Fires tool.started events AFTER interrupt â€” all should be suppressed.
    ...
    def __init__(self, **kwargs):
        self.tool_progress_callback = kwargs.get("tool_progress_callback")
        self.tools = []
        # Start already interrupted â€” simulates stop having already landed
    ...
    def run_conversation(self, message, conversation_history=None, task_id=None):
        # Parallel tool batch â€” in production these come from one LLM
        # response with 5 tool_calls.  All are post-interrupt.
        self.tool_progress_callback("tool.started", "web_search", "cognee zed", {})
...
async def test_baseline_non_interrupted_agent_renders_progress(monkeypatch, tmp_path):
    ...
    assert "first search" in rendered, (
        "baseline agent should render its tool-progress event â€” "
...
async def test_progress_suppressed_when_agent_is_interrupted(monkeypatch, tmp_path):
    ...
    âš¡ Interrupting, but 5 more ðŸ” web_search bubbles still render because
    ...
    for leaked_query in (
        ...
        assert leaked_query not in rendered, (
            f"event '{leaked_query}' leaked into the UI after interrupt â€” "
