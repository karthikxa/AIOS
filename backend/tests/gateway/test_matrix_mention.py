...
# module-level mock installation â€” tests that call adapter methods
...
class TestIsBotMentioned:
    ...
    def test_full_user_id_in_body(self):
        assert self.adapter._is_bot_mentioned("hey @zed:example.org help")

    def test_localpart_in_body(self):
        assert self.adapter._is_bot_mentioned("zed can you help?")

    def test_localpart_case_insensitive(self):
        assert self.adapter._is_bot_mentioned("ZED can you help?")

    def test_matrix_pill_in_formatted_body(self):
        html = '<a href="https://matrix.to/#/@zed:example.org">Zed</a> help'
        assert self.adapter._is_bot_mentioned("Zed help", html)
    ...
    def test_partial_localpart_no_match(self):
        # "zedbot" should not match word-boundary check for "zed"
        assert not self.adapter._is_bot_mentioned("zedbot is here")

    # m.mentions.user_ids â€” MSC3952 / Matrix v1.7 authoritative mentions
    # Ported from openclaw/openclaw#64796

    def test_m_mentions_user_ids_authoritative(self):
        """m.mentions.user_ids alone is sufficient â€” no body text needed."""
        assert self.adapter._is_bot_mentioned(
            "please reply",  # no @zed anywhere in body
            mention_user_ids=["@zed:example.org"],
        )

    def test_m_mentions_user_ids_with_body_mention(self):
        """Both m.mentions and body mention â€” should still be True."""
        assert self.adapter._is_bot_mentioned(
            "hey @zed:example.org help",
            mention_user_ids=["@zed:example.org"],
        )

    def test_m_mentions_user_ids_other_user_only(self):
        """m.mentions with a different user â€” bot is NOT mentioned."""
        assert not self.adapter._is_bot_mentioned(
            "hello",
    ...
    def test_m_mentions_user_ids_empty_list(self):
        """Empty user_ids list â€” falls through to text detection."""
        assert not self.adapter._is_bot_mentioned(
            "hello everyone",
            mention_user_ids=[],
        )
    ...
    def test_m_mentions_user_ids_none(self):
        """None mention_user_ids â€” falls through to text detection."""
        assert not self.adapter._is_bot_mentioned(
            "hello everyone",
            mention_user_ids=None,
        )
...
class TestStripMention:
    ...
    def test_strip_full_user_id(self):
        result = self.adapter._strip_mention("@zed:example.org help me")
        assert result == "help me"

    def test_localpart_preserved(self):
        """Bare localpart (no @) is preserved â€” avoids false positives in paths."""
        result = self.adapter._strip_mention("zed help me")
        assert result == "zed help me"

    def test_localpart_in_path_preserved(self):
        """Localpart inside a file path must not be damaged."""
        result = self.adapter._strip_mention("read /home/zed/config.yaml")
        assert result == "read /home/zed/config.yaml"
    ...
    def test_does_not_strip_bare_localpart_word(self):
        # Regression: plain words like "Zed Agent" should not be mutated.
        result = self.adapter._strip_mention("Zed Agent")
        assert result == "Zed Agent"

    def test_strip_returns_empty_for_mention_only(self):
        result = self.adapter._strip_mention("@zed:example.org")
        assert result == ""
...
async def test_require_mention_default_processes_mentioned(monkeypatch):
    ...
    adapter = _make_adapter()
    event = _make_event("@zed:example.org help me")
...
async def test_require_mention_html_pill(monkeypatch):
    ...
    adapter = _make_adapter()
    formatted = '<a href="https://matrix.to/#/@zed:example.org">Zed</a> help'
    event = _make_event("Zed help", formatted_body=formatted)
...
async def test_require_mention_m_mentions_user_ids(monkeypatch):
    """m.mentions.user_ids is authoritative per MSC3952 â€” no body mention needed.
...
async def test_dm_preserves_localpart_in_body(monkeypatch):
    """DMs no longer strip bare localpart â€” only the full MXID is removed."""
    monkeypatch.delenv("MATRIX_REQUIRE_MENTION", raising=False)
    monkeypatch.delenv("MATRIX_FREE_RESPONSE_ROOMS", raising=False)
    monkeypatch.setenv("MATRIX_AUTO_THREAD", "false")
    ...
    adapter = _make_adapter()
    _set_dm(adapter)
    event = _make_event("zed help me")

    await adapter._on_room_message(event)
    adapter.handle_message.assert_awaited_once()
    msg = adapter.handle_message.await_args.args[0]
    assert msg.text == "zed help me"
...
async def test_bare_mention_passes_empty_string(monkeypatch):
    ...
    adapter = _make_adapter()
    event = _make_event("@zed:example.org")
...
async def test_require_mention_disabled_skips_stripping(monkeypatch):
    ...
    adapter.handle_message.assert_awaited_once()
    msg = adapter.handle_message.await_args.args[0]
    assert msg.text == "@zed:example.org help me"
...
async def test_dm_mention_thread_disabled_by_default(monkeypatch):
    ...
    adapter = _make_adapter()
    _set_dm(adapter)
    event = _make_event("@zed:example.org help me", event_id="$dm1")
...
async def test_dm_mention_thread_preserves_existing_thread(monkeypatch):
    ...
    event = _make_event("@zed:example.org help me", thread_id="$existing_thread")
...
async def test_dm_mention_thread_tracks_participation(monkeypatch):
    ...
    adapter = _make_adapter()
    _set_dm(adapter)
    event = _make_event("@zed:example.org help", event_id="$dm1")
