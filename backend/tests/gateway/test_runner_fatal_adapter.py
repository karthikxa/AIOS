...
class _FatalAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        self._set_fatal_error(
            "telegram_token_lock",
            "Another local Zed gateway is already using this Telegram bot token.",
            retryable=False,
        )
        return False
...
async def test_runner_queues_retryable_runtime_fatal_for_reconnection(monkeypatch, tmp_path):
    ...
    AND keep the gateway alive â€” the background reconnect watcher recovers
    ...
    # Gateway stays alive â€” watcher will retry in background
