...
command" response instead of the intended "disabled â€” enable with â€¦"
or "not installed â€” install with â€¦" hint.
...
* ``disabled`` membership is checked by the declared name, because that
  is what :func:`zed_cli.skills_config.save_disabled_skills` stores.
"""
...
def tmp_skills(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolated skills dir + ZED_HOME so the real user config is untouched."""
    home = tmp_path / ".zed"
    home.mkdir()
    (home / "skills").mkdir()
...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    # Config disables by declared name (matches what `zed skills config` writes).
    monkeypatch.setattr(
    ...
    assert "zed skills config" in msg
...
def test_slug_normalization_strips_non_alnum(
    ...
    """Frontmatter ``C++ Code Review`` â†’ slug ``c-code-review`` (``+`` stripped)."""
    from gateway import run as gateway_run
...
def test_optional_skill_uses_frontmatter_slug(
    ...
    # ``get_optional_skills_dir(repo_root / "optional-skills")`` â€” we
    # can't easily retarget ``repo_root``, so patch the resolver.
    monkeypatch.setattr(
        "zed_constants.get_optional_skills_dir",
