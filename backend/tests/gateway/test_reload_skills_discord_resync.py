...
autocomplete â€” new skills stayed invisible in the dropdown and deleted
...
options dynamically on every keystroke â€” we only need to rebind the
...
class TestRefreshSkillGroup:
    def test_refresh_repopulates_entries_after_catalog_change(
        ...
        skill to ~/.zed/skills/, runs /reload-skills, and expects
    ...
    def test_refresh_sorts_entries_alphabetically(self, monkeypatch) -> None:
        ...
        def fake_collector(*, reserved_names):
            # Intentionally unsorted â€” the fix must resort.
            return (
                {"zzz": [("zebra", "", "/zebra")]},
                [("alpha", "", "/alpha")],
                0,
...
class TestRegisterSkillGroupUsesInstanceState:
    ...
    method is useless â€” autocomplete will keep serving the stale list.
    ...
    def test_refresh_catalog_state_populates_instance_attrs(
        ...
        # Instance-level state populated â€” the autocomplete + handler
...
class TestHandleReloadSkillsCallsRefreshSkillGroup:
    ...
    def test_orchestrator_calls_refresh_skill_group_on_every_adapter(self):
        ...
        # Import without constructing a real runner â€” test the method
        ...
        class NoOpAdapter:
            name = "other"
            # No refresh_skill_group â€” must not crash.
        ...
        with patch(
            ...
            # _session_key_for_source may be called â€” make it safe.
