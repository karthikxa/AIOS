"""Regression guard for #15421 bug 1 â€” Slack channel session scoping.
...
across messages â€” even when the operator set ``reply_in_thread: false``
...
both modes â€” threading UX is unchanged when the operator actually
asks for it.
...
class TestChannelSessionScopeDefault:
    ...
    becomes its own threaded session â€” unchanged from the pre-#15421
    behaviour."""
    ...
    async def test_top_level_maps_to_ts_when_reply_in_thread_true(self, adapter):
        ...
        assert len(captured) == 1, (
            "handler dropped the top-level channel mention â€” "
        ...
        assert source.thread_id == "1700000000.000001", (
            ...
            "threaded session â€” regression guard"
        )
...
class TestChannelSessionScopeShared:
    ...
    async def test_top_level_reply_to_id_stays_none_when_shared(self, adapter):
        ...
        expression evaluates to ``thread_ts`` itself â€” which IS
    ...
    async def test_thread_reply_scopes_by_thread_even_when_shared(self, adapter):
        ...
        assert source.thread_id == "1700000000.000000", (
            ...
            "when reply_in_thread=false â€” only TOP-LEVEL messages share "
