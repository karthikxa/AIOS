...
i18n catalogs and render human strings â€” not raw key paths.
...
def test_installed_wheel_renders_i18n_strings(tmp_path):
    ...
    #    (no PYTHONPATH leaking the repo, no ZED_BUNDLED_LOCALES).
    probe = (
    ...
    env = {k: v for k, v in os.environ.items() if k not in ("PYTHONPATH", "ZED_BUNDLED_LOCALES")}
...
def test_built_sdist_ships_locale_catalogs(tmp_path):
    ...
    `graft locales` is what puts the catalogs in the tarball â€” a stale graft or
    ...
    with tarfile.open(tarballs[0]) as tf:
        ...
        # zed_agent-0.15.1/locales/en.yaml â€” match on the suffix.
    ...
    assert len(catalogs) == expected, (
        ...
        f"({len(SUPPORTED_LANGUAGES)} supported languages) â€” check `graft "
