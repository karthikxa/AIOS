...
Tests cover:
1. Bug #20001 â€” _finalize_session() uses stale session_key after compression rotation
2. Bug #20001 â€” _sync_session_key_after_compress called post-run_conversation
3. Bug #19029 â€” pending_title ValueError leaves title wedged
4. Bug #18765 â€” gateway surfaces null response when agent did work
5. Prune â€” finalize_orphaned_compression_sessions catches ghost continuations
"""
...
class TestFinalizeSessionUsesAgentSessionId:
    ...
    def test_finalize_targets_agent_session_id_not_stale_key(self, tmp_path):
        ...
        # Continuation is NOT ended â€” this is the bug state
...
class TestPendingTitleValueError:
    ...
    pending_title must be cleared â€” not left wedged forever."""
...
class TestGatewaySurfacesNullResponse:
    ...
    def test_partial_response_surfaces_error(self):
        """Agent returns partial=True with no response â†’ user sees error."""
    ...
    def test_interrupted_response_stays_empty(self):
        """Interrupted agent â†’ response stays empty (platform handles UX)."""
        from gateway.run import _normalize_empty_agent_response
    ...
    def test_failed_context_overflow(self):
        """Agent failed with context overflow â†’ specific guidance message."""
        from gateway.run import _normalize_empty_agent_response
    ...
    def test_failed_generic_error(self):
        """Agent failed with non-context error â†’ generic error message."""
        from gateway.run import _normalize_empty_agent_response
...
class TestFinalizeOrphanedCompressionSessions:
    ...
    def test_marks_ghost_continuation_with_compression_parent(self, tmp_path):
        """Ghost session with compression-ended parent + messages â†’ finalized."""
        db = _make_session_db(tmp_path)

        # Parent session (ended by compression â€” this is the key condition)
    ...
    def test_skips_session_without_parent(self, tmp_path):
        ...
        continuation â€” should not be touched by this prune."""
        db = _make_session_db(tmp_path)
    ...
    def test_skips_recent_sessions(self, tmp_path):
        ...
        # started_at is now() â€” within 7 days
    ...
    def test_skips_session_with_non_compression_parent(self, tmp_path):
        ...
        not be touched â€” it's not from the compression continuation path."""
        db = _make_session_db(tmp_path)
    ...
    def test_skips_sessions_without_messages(self, tmp_path):
        """Empty sessions (no messages) are NOT targeted by this prune â€”
    ...
    def test_titled_ghost_with_parent_is_caught(self, tmp_path):
        ...
        # Create parent first â€” ended by compression
        db.create_session(session_id="parent", source="tui", model="test")
