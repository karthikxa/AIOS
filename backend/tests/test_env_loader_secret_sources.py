...
don't see an unexplained "credentials âœ“" line when their .env is empty.
"""
...
def test_format_secret_source_suffix_empty_for_untracked():
    # Credentials from .env or the shell shouldn't add noise â€” the
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_zed_dotenv()`` is called at module-import time from several
    hot modules (cli.py, zed_cli/main.py, run_agent.py, ...).  The
    Bitwarden status line previously printed once per call â€” 3-5x per
    ...
    runs exactly once per ZED_HOME per process.
    """
    ...
    # cli.py, zed_cli/main.py, run_agent.py, trajectory_compressor.py,
    # gateway/run.py.  Only the first should actually call the backend.
    for _ in range(5):
        env_loader._apply_external_secret_sources(tmp_path)

    assert call_count["n"] == 1, (
        "Bitwarden backend was called {} time(s); expected exactly 1 â€” "
