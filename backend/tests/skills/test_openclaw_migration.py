...
"""
    ...
    assert "Tyler Williams > Active Projects: Zed Agent" in entries
...
def test_source_candidate_finds_files_in_custom_workspace(tmp_path: Path):
    ...
    # No workspace/ directory inside .openclaw â€” files live in custom workspace
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `zed cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # security scanner.  None represent actual threats â€” they are all legitimate
    # uses in a migration CLI tool:
    #
    # agent_config_mod   â€” references AGENTS.md to migrate workspace instructions
    # python_os_environ  â€” reads MIGRATION_JSON_OUTPUT to enable JSON output mode
    #                      (feature flag, not an env dump)
    # zed_config_mod  â€” print statements in the post-migration summary that
    #                      tell the user to *review* ~/.zed/config.yaml;
    #                      the script never writes to that file
    #
    # Accept "caution" or "safe" â€” just not "dangerous" from a *real* threat.
    assert result.verdict in {"safe", "caution", "dangerous"}, f"Unexpected verdict: {result.verdict}"
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "zed_config_mod"}
    for f in result.findings:
        assert f.pattern_id in KNOWN_FALSE_POSITIVES, f"Unexpected finding: {f}"


# â”€â”€ rebrand_text tests â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches â†’ capital-H ``Zed``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Zed prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Zed to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Zed config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Zed uses tools well"
    # All-lowercase matches â†’ lowercase ``zed``; this preserves the
    # real filesystem path ``~/.zed`` (Zed home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "zed should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Zed remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "zed prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Zed was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "zed uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Zed said to ask Zed about Zed settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches â€” especially ``.openclaw`` filesystem paths â€” must
    rewrite to lowercase ``.zed`` (the real Zed home), not the broken
    ``.Zed``.
    ...
    rewritten to ``~/.Zed/config.yaml`` â€” a path that doesn't exist â€”
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.zed/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .zed directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.zed'"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Zed" in result


# â”€â”€ migrate_model_config: alias resolution (issue #16745) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_migrate_model_config_resolves_alias_against_real_openclaw_schema(tmp_path: Path):
    """Regression for #16745 â€” OpenClaw's catalog is keyed by the full
...
def test_migrate_model_config_passes_through_existing_api_id(tmp_path: Path):
    ...
    a key in the catalog, it should be written verbatim â€” not double-rewritten."""
    parsed = _run_model_migration(
        tmp_path,
        {
            "agents": {
