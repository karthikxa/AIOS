...
class TestGoogleWorkspaceCredentialFiles:
    ...
    def test_entries_are_registered_when_files_exist(self, tmp_path):
        ...
        (zed_home / "google_client_secret.json").write_text("{}")
        ...
        try:
            ...
            assert "/root/.zed/google_token.json" in container_paths
            assert "/root/.zed/google_client_secret.json" in container_paths
        finally:
            clear_credential_files()

    def test_missing_token_is_reported(self, tmp_path):
        """google_token.json absent (first-time setup) â€” reported as missing, client secret still mounts."""
        zed_home = tmp_path / ".zed"
        zed_home.mkdir()
        (zed_home / "google_client_secret.json").write_text("{}")
        ...
        try:
            ...
            container_paths = {m["container_path"] for m in mounts}
            assert "/root/.zed/google_client_secret.json" in container_paths
            assert "/root/.zed/google_token.json" not in container_paths
        finally:
            clear_credential_files()
