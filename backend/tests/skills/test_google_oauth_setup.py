...
class TestZedConstantsFallback:
    """Tests for _zed_home.py fallback when zed_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_zed_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _zed_home.py with zed_constants blocked."""
        monkeypatch.setitem(sys.modules, "zed_constants", None)
        spec = importlib.util.spec_from_file_location("_zed_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_zed_home_env_var(self, monkeypatch, tmp_path):
        """When zed_constants is missing, ZED_HOME comes from env var."""
        monkeypatch.setenv("ZED_HOME", str(tmp_path / "custom-zed"))
        module = self._load_helper(monkeypatch)
        assert module.get_zed_home() == tmp_path / "custom-zed"

    def test_fallback_defaults_to_dot_zed(self, monkeypatch):
        """When zed_constants is missing and ZED_HOME unset, default to ~/.zed."""
        monkeypatch.delenv("ZED_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_zed_home() == Path.home() / ".zed"

    def test_fallback_ignores_empty_zed_home(self, monkeypatch):
        """Empty/whitespace ZED_HOME is treated as unset."""
        monkeypatch.setenv("ZED_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_zed_home() == Path.home() / ".zed"

    def test_fallback_display_zed_home_shortens_path(self, monkeypatch):
        """Fallback display_zed_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("ZED_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_zed_home() == "~/.zed"

    def test_fallback_display_zed_home_profile_path(self, monkeypatch):
        """Fallback display_zed_home() handles profile paths under ~/."""
        monkeypatch.setenv("ZED_HOME", str(Path.home() / ".zed/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_zed_home() == "~/.zed/profiles/coder"

    def test_fallback_display_zed_home_custom_path(self, monkeypatch):
        """Fallback display_zed_home() returns full path for non-home locations."""
        monkeypatch.setenv("ZED_HOME", "/opt/zed-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_zed_home() == "/opt/zed-custom"

    def test_delegates_to_zed_constants_when_available(self):
        """When zed_constants IS importable, _zed_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_zed_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
...
class TestInstallDeps:
    ...
    Regression coverage for the Zed Docker image, whose venv is built with
    `uv sync` and ships without pip â€” `sys.executable -m pip install` fails
    ...
    def test_uses_pip_when_available(self, monkeypatch):
        ...
        def fake_check_call(cmd, **kwargs):
            recorded.append(cmd)
            # pip path is the first attempt â€” succeed.
            return 0
    ...
    def test_falls_back_to_uv_when_pip_missing(self, monkeypatch):
        """No pip â†’ uv pip install --python <interpreter> is used."""
        module = _load_setup_module(monkeypatch)
        _force_deps_missing(monkeypatch)
    ...
    def test_returns_false_when_no_pip_and_no_uv(self, monkeypatch, capsys):
        """No pip AND no uv â†’ failure, with the [google] extra hint printed."""
        module = _load_setup_module(monkeypatch)
        _force_deps_missing(monkeypatch)
        ...
        out = capsys.readouterr().out
        assert "zed-agent[google]" in out

    def test_returns_false_when_uv_fallback_also_fails(self, monkeypatch, capsys):
        """uv present but its install fails â†’ failure surfaced (not swallowed)."""
        module = _load_setup_module(monkeypatch)
        _force_deps_missing(monkeypatch)
