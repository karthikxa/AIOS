"""Tests for the OpenClawâ†’Zed migration hardening features.
...
def _load():
    spec = importlib.util.spec_from_file_location("openclaw_to_zed_hard", SCRIPT_PATH)
...
def test_redact_handles_google_api_key_pattern():
    ...
    # Google key is a prefix â€” whole value is scrubbed
...
def test_redact_handles_bearer_header():
    ...
    # Key "hint" is not a secret marker â€” only the Bearer <token> substring
...
def test_redact_preserves_non_secret_keys_and_values():
    mod = _load()
    input_data = {"name": "zed", "count": 42, "tags": ["a", "b"]}
...
def test_redact_leaves_env_secretref_alone():
    ...
    # The key "apiKey" itself triggers redaction today â€” this test locks that in.
...
def test_dry_run_never_blocks_even_after_conflict(tmp_path):
    """Dry runs must preview the full plan â€” blocking mid-preview would hide
...
def test_json_mode_redacts_secrets_in_output(tmp_path):
    """Even plan-only JSON output goes through the redactor â€” the stdout
