"""Tests for cron/jobs.py â€” schedule parsing, job CRUD, and due-job detection."""
...
class TestComputeNextRun:
    ...
    def test_once_recent_past_within_grace_returns_time(self, monkeypatch):
        ...
        monkeypatch.setattr("cron.jobs._zed_now", lambda: now)
...
class TestUpdateJob:
    ...
    def test_update_rejects_id_change(self, tmp_cron_dir):
        """Job IDs are filesystem path components â€” must be immutable."""
...
class TestResolveJobRef:
    ...
    def test_resolve_ambiguous_name_raises(self, tmp_cron_dir):
        """Two jobs sharing a name â†’ refuse to pick, surface both IDs."""
...
class TestMarkJobRun:
    ...
    def test_delivery_error_tracked_separately(self, tmp_cron_dir):
        """Agent succeeds but delivery fails â€” both tracked independently."""
    ...
    def test_both_agent_and_delivery_error(self, tmp_cron_dir):
        """Agent fails AND delivery fails â€” both errors recorded."""
        job = create_job(prompt="Report", schedule="every 1h")
    ...
    def test_recurring_cron_not_disabled_when_croniter_missing(self, tmp_cron_dir, monkeypatch):
        ...
        `mark_job_run()` must NOT treat that as terminal completion â€” the job
    ...
    def test_recurring_interval_not_disabled_when_next_run_is_none(self, tmp_cron_dir, monkeypatch):
        """Defensive sibling of the cron test â€” any recurring schedule that
        somehow yields next_run_at=None must stay enabled with state=error.
        """
        job = create_job(prompt="Recurring", schedule="every 1h")
        assert job["schedule"]["kind"] == "interval"

        # Force compute_next_run to return None for this call â€” simulates
    ...
    def test_oneshot_still_completes_when_next_run_is_none(self, tmp_cron_dir):
        ...
        when next_run_at cannot be computed â€” the #16265 fix must not
...
class TestAdvanceNextRun:
    """Tests for advance_next_run() â€” crash-safety for recurring jobs."""
    ...
    def test_advances_interval_job(self, tmp_cron_dir):
        ...
        updated = get_job(job["id"])
        from cron.jobs import _ensure_aware, _zed_now
        new_next_dt = _ensure_aware(datetime.fromisoformat(updated["next_run_at"]))
        assert new_next_dt > _zed_now(), "next_run_at should be in the future after advance"
    ...
    def test_skips_oneshot_job(self, tmp_cron_dir):
        """One-shot jobs should NOT be advanced â€” they need to retry on restart."""
    ...
    def test_already_future_stays_future(self, tmp_cron_dir):
        ...
        from cron.jobs import _ensure_aware, _zed_now
        new_next_dt = _ensure_aware(datetime.fromisoformat(updated["next_run_at"]))
        assert new_next_dt > _zed_now(), "next_run_at should remain in the future"
...
class TestMarkJobRunConcurrency:
    ...
    Without _jobs_file_lock protecting the loadâ†’modifyâ†’save cycle in
    ...
    def test_three_concurrent_mark_job_run_no_overwrites(self, tmp_cron_dir):
        ...
        # Verify each job has the correct state â€” no overwrites
        a = get_job(job_a["id"])
        b = get_job(job_b["id"])
        c = get_job(job_c["id"])
