"""Tests for cron/scheduler.py â€” origin resolution, delivery routing, and error logging."""
...
class TestRoutingIntents:
    ...
    def test_all_with_no_connected_channels_returns_empty(self, monkeypatch):
        """deliver='all' with nothing connected returns [] â€” delivery is recorded as failed upstream."""
        from cron.scheduler import _resolve_delivery_targets
...
class TestRunJobSessionPersistence:
    ...
    def test_run_job_titles_cron_session_from_job_not_important_hint(self, tmp_path):
        # The cron session's first message is the injected "[IMPORTANT: â€¦]"
        # hint, which used to surface as the sidebar/history row label. run_job
        # must title the session from the job (name â†’ short prompt â†’ id).
        job = {
            "id": "test-job",
            "name": "Morning digest",
    ...
    def test_run_job_closes_agent_on_failure_to_prevent_fd_leak(self, tmp_path):
        ...
        # agent was previously leaked â€” over days of ticks this accumulated
    ...
    def test_run_job_disabled_toolsets_layer_user_config_on_baseline(self, tmp_path):
        """agent.disabled_toolsets must be honoured in cron â€” issue #25752.
    ...
    def test_run_job_enabled_toolsets_resolves_from_platform_config_when_not_set(self, tmp_path):
        ...
        resolves them from ``zed tools`` platform config for ``cron``
        (PR #14xxx â€” blanket fix for Norbert's surprise ``moa`` run).

        The legacy "pass None â†’ AIAgent loads full default" path is still
        ...
        kwargs = mock_agent_cls.call_args.kwargs
        # Resolution happened â€” not None, is a list.
        assert isinstance(kwargs["enabled_toolsets"], list)
        # The cron default is _ZED_CORE_TOOLS with _DEFAULT_OFF_TOOLSETS
    ...
    def test_run_job_per_job_toolsets_win_over_platform_config(self, tmp_path):
        ...
        over the platform-level ``zed tools`` config."""
        job = {
            "id": "override-job",
            "name": "test",
            "prompt": "hello",
        ...
        fake_db, patches = self._make_run_job_patches(tmp_path)
        # Even if the user has ``zed tools`` configured to enable web+file
    ...
    def test_run_job_sets_auto_delivery_env_from_dotenv_home_channel(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("TELEGRAM_HOME_CHANNEL", raising=False)
        monkeypatch.delenv("ZED_CRON_AUTO_DELIVER_PLATFORM", raising=False)
        monkeypatch.delenv("ZED_CRON_AUTO_DELIVER_CHAT_ID", raising=False)
        monkeypatch.delenv("ZED_CRON_AUTO_DELIVER_THREAD_ID", raising=False)
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen["platform"] = get_session_env("ZED_CRON_AUTO_DELIVER_PLATFORM") or None
                seen["chat_id"] = get_session_env("ZED_CRON_AUTO_DELIVER_CHAT_ID") or None
                seen["thread_id"] = get_session_env("ZED_CRON_AUTO_DELIVER_THREAD_ID") or None
        ...
        assert os.getenv("ZED_CRON_AUTO_DELIVER_PLATFORM") is None
        assert os.getenv("ZED_CRON_AUTO_DELIVER_CHAT_ID") is None
        assert os.getenv("ZED_CRON_AUTO_DELIVER_THREAD_ID") is None
        fake_db.close.assert_called_once()
    ...
    def test_run_job_clears_stale_auto_delivery_thread_id_between_jobs(self, tmp_path, monkeypatch):
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen.append(
                    {
                        "platform": get_session_env("ZED_CRON_AUTO_DELIVER_PLATFORM") or None,
                        "chat_id": get_session_env("ZED_CRON_AUTO_DELIVER_CHAT_ID") or None,
                        "thread_id": get_session_env("ZED_CRON_AUTO_DELIVER_THREAD_ID") or None,
                    }
                )
                return {"final_response": "ok"}
...
class TestRunJobConfigEnvVarExpansion:
    ...
    def test_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_ZED_TEST_CRON_MODEL}\n")
        monkeypatch.setenv("_ZED_TEST_CRON_MODEL", "gpt-4o-mini-cron-test")
    ...
    def test_fallback_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text(
            "fallback_providers:\n"
            "  - provider: openrouter\n"
            "    model: ${_ZED_TEST_CRON_FALLBACK}\n"
        )
        monkeypatch.setenv("_ZED_TEST_CRON_FALLBACK", "gpt-4o-fallback-test")
    ...
    def test_unexpanded_ref_passthrough_when_var_unset(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_ZED_TEST_CRON_UNSET_VAR}\n")
        monkeypatch.delenv("_ZED_TEST_CRON_UNSET_VAR", raising=False)
        ...
        assert success is True
        kwargs = mock_agent_cls.call_args.kwargs
        # Unresolved refs are kept verbatim â€” _expand_env_vars contract
        assert kwargs["model"] == "${_ZED_TEST_CRON_UNSET_VAR}"
...
class TestSilentDelivery:
    ...
    def test_silent_trailing_suppresses_delivery(self):
        """Agent appended [SILENT] after explanation text â€” must still suppress."""
...
class TestParseWakeGate:
    """Unit tests for _parse_wake_gate â€” pure function, no side effects."""
    ...
    def test_non_boolean_false_still_wakes(self):
        """Only strict ``False`` skips â€” truthy/falsy shortcuts are too risky."""
        from cron.scheduler import _parse_wake_gate
...
class TestRunJobWakeGate:
    ...
    def _stub_runtime_provider(self):
        ...
        in addition to ``AIAgent`` â€” otherwise in a hermetic CI env (no
    ...
    def test_script_failure_does_not_trigger_gate(self):
        ...
        # gate JSON â€” we must NOT honor it because ran_ok is False.
        agent = MagicMock()
        agent.run_conversation = MagicMock(return_value={
            "final_response": "ok", "messages": []
        })
...
class TestBuildJobPromptBumpUse:
    ...
    def test_bump_failure_does_not_break_prompt(self, caplog):
        """If bump_use raises, the prompt still builds â€” error is logged at DEBUG."""
...
class TestSendMediaViaAdapter:
    """Unit tests for _send_media_via_adapter â€” routes files to typed adapter methods."""
...
class TestParallelTick:
    ...
    def test_parallel_jobs_run_concurrently(self):
        ...
        def mock_run_job(job):
            """Each job hits a barrier â€” both must be active simultaneously."""
        ...
        assert result == 2
        # Both starts happened before both ends â€” proof of concurrency
    ...
    def test_parallel_jobs_isolated_contextvars(self):
        """Each job's ContextVars must be isolated â€” no cross-contamination."""
        from gateway.session_context import get_session_env
        seen = {}

        def mock_run_job(job):
            origin = job.get("origin", {})
            # run_job sets ContextVars â€” verify each job sees its own
            ...
            platform = get_session_env("ZED_SESSION_PLATFORM")
            chat_id = get_session_env("ZED_SESSION_CHAT_ID")
    ...
    def test_max_parallel_env_var(self, monkeypatch):
        """ZED_CRON_MAX_PARALLEL=1 should restore serial behaviour."""
        monkeypatch.setenv("ZED_CRON_MAX_PARALLEL", "1")
        call_times = []
...
class TestDeliverResultTimeoutCancelsFuture:
    ...
    def test_live_adapter_timeout_cancels_future_and_falls_back(self):
        ...
        # 2. The standalone fallback delivered â€” no double send, no silent drop
...
class TestSendMediaTimeoutCancelsFuture:
    ...
    def test_media_send_timeout_cancels_future_and_continues(self, tmp_path, monkeypatch):
        ...
        with patch("asyncio.run_coroutine_threadsafe", side_effect=fake_run_coro):
            # Should not raise â€” the except Exception clause swallows the timeout
        ...
        # 2. Second file still got dispatched â€” one timeout doesn't abort the batch
        adapter.send_video.assert_called_once()
...
class TestCronDeliveryTargets:
    ...
    def test_lists_configured_platforms_flagging_missing_home_channel(self, monkeypatch):
        ...
        # Configured but no home channel â†’ surfaced, flagged for the UI.
...
class TestHomeTargetEnvVarRegistry:
    ...
    entry means ``zed cron create --deliver=<platform>`` silently
    ...
    def test_whatsapp_cloud_registered(self):
        ...
        WHATSAPP_CLOUD_HOME_CHANNEL â€” added alongside the existing
