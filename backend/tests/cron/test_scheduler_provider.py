...
CronScheduler plan, .zed/plans/cron-scheduler-provider-interface.md). They
must pass unchanged on `main` now, and after every subsequent phase of the
refactor â€” they are the regression harness that proves the built-in firing
...
No production code is exercised beyond the two ticker entry points:
  ...
  - zed_cli/web_server.py::_start_desktop_cron_ticker  (desktop fallback)
...
# â”€â”€ Phase 1: CronScheduler ABC + InProcessCronScheduler â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


def test_cronscheduler_is_abstract():
    """name + start are abstract â€” the bare ABC can't be instantiated."""
    import pytest
...
def test_abc_growth_stays_additive():
    ...
    would force every provider â€” including the built-in â€” to implement it).
    """
    from cron.scheduler_provider import CronScheduler
...
def test_inprocess_provider_ticks_and_stops():
    ...
    and exits promptly when stop_event is set â€” same contract as the raw
...
# â”€â”€ Phase 2: config key, discovery, resolver â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def test_discover_cron_schedulers_returns_list():
    """Discovery returns a list. May be empty â€” the built-in is core, not
...
# â”€â”€ Phase 4B: additive hooks (on_jobs_changed / fire_due / reconcile) â”€â”€â”€â”€â”€â”€â”€â”€


def test_hooks_did_not_change_required_surface():
    """The additive hooks must NOT become abstractmethods â€” the Phase-1 guard
