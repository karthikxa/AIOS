...
- Path resolution (absolute, relative to ZED_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp ZED_HOME."""
    zed_home = tmp_path / ".zed"
    zed_home.mkdir()
    (zed_home / "cron").mkdir()
    (zed_home / "cron" / "output").mkdir()
    (zed_home / "scripts").mkdir()
    monkeypatch.setenv("ZED_HOME", str(zed_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "ZED_DIR", zed_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", zed_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", zed_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", zed_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Zed provider env (SECURITY.md Â§2.3)."""
        ...
        blocked_var = sorted(_ZED_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.zed/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        # (no valid model, no API key â€” will raise inside try block)
        job = {
            "id": "test-envleak",
            "name": "env-leak-test",
            "prompt": "test",
        ...
        assert os.environ.get("ZED_SESSION_PLATFORM") is None
        assert os.environ.get("ZED_SESSION_CHAT_ID") is None
        assert os.environ.get("ZED_SESSION_CHAT_NAME") is None
