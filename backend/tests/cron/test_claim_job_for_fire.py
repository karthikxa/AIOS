...
These exercise the real store against a temp ZED_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    """Isolated ZED_HOME so jobs.json doesn't touch the real store."""
    monkeypatch.setenv("ZED_HOME", str(tmp_path))
    # cron.jobs caches no home at import; get_zed_home() reads the env live.
    yield tmp_path
...
def test_stale_claim_is_reclaimable(temp_home, monkeypatch):
    """A claim older than the TTL is overwritten â€” the fire isn't stuck forever
...
def test_mark_job_run_clears_claim(temp_home):
    ...
    # â€¦and the re-armed recurring job is claimable again.
    assert claim_job_for_fire(jid) is True
