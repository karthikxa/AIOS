...
class TestCreateJobWorkdir:
    ...
    def test_workdir_none_preserves_old_behaviour(self, tmp_cron_dir):
        ...
        stored = get_job(job["id"])
        # Field is present on the dict but None â€” downstream code checks
...
class TestCronjobToolWorkdir:
    ...
    def test_create_without_workdir_hides_field_in_format(self, tmp_cron_dir):
        ...
        assert result["success"] is True
        # _format_job omits the field when unset â€” reduces noise in agent output.
...
class TestTickWorkdirPartition:
    ...
    def test_workdir_jobs_run_sequentially(self, tmp_path, monkeypatch):
        ...
        # Workdir jobs run on the persistent single-thread cron-seq pool â€”
        # NOT the main thread â€” so a long workdir job never blocks the ticker.
...
class TestRunJobTerminalCwd:
    ...
    is set, and restores the prior TERMINAL_CWD in finally â€” even on error.
    ...
    def _install_stubs(monkeypatch, observed: dict):
        ...
        # run_job calls load_dotenv(~/.zed/.env, override=True), which will
    ...
    def test_no_workdir_leaves_terminal_cwd_untouched(self, monkeypatch):
        """When workdir is absent, run_job must not touch TERMINAL_CWD at all â€”
        ...
        # Feature is OFF â€” skip_context_files stays True.
