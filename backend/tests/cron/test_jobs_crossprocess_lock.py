...
Background: ``zed cron pause`` runs in its own process (CLI â†’ cronjob tool â†’
``pause_job`` â†’ ``update_job`` â†’ ``save_jobs``), entirely separate from the
...
was live could be silently lost to a concurrent gateway write â€” the job kept
