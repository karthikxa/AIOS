...
- Unlimited timeout (ZED_CRON_TIMEOUT=0)
- Backward compat: ZED_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_idle_agent_triggers_timeout(self):
        ...
        assert result is None  # Never got a result â€” interrupted

    def test_unlimited_timeout(self):
        """ZED_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """ZED_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("ZED_CRON_TIMEOUT", "1200")
        raw = os.getenv("ZED_CRON_TIMEOUT", "").strip()
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """ZED_CRON_TIMEOUT=0 yields None (unlimited)."""
        monkeypatch.setenv("ZED_CRON_TIMEOUT", "0")
        raw = os.getenv("ZED_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
        _cron_inactivity_limit = _cron_timeout if _cron_timeout > 0 else None
    ...
    def test_timeout_invalid_value_falls_back_to_default(self, monkeypatch):
        """ZED_CRON_TIMEOUT=abc should fall back to 600s, not raise ValueError."""
        monkeypatch.setenv("ZED_CRON_TIMEOUT", "abc")
        raw = os.getenv("ZED_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
    ...
    def test_timeout_empty_string_uses_default(self, monkeypatch):
        """ZED_CRON_TIMEOUT='' (empty) should use the 600s default."""
        monkeypatch.setenv("ZED_CRON_TIMEOUT", "")
        raw = os.getenv("ZED_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
        assert _cron_timeout == 600.0
    ...
    def test_agent_without_activity_summary_uses_wallclock_fallback(self):
        ...
        pool.shutdown(wait=False)
        # Should NOT have timed out â€” bare agent has no get_activity_summary
        assert not _inactivity_timeout
...
class TestSysPathOrdering:
    ...
    def test_zed_time_importable(self):
        """zed_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _zed_now
        assert callable(_zed_now)
