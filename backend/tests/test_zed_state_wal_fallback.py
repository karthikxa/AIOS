"""Tests for the WALâ†’DELETE journal-mode fallback on NFS / SMB / FUSE.

When ``PRAGMA journal_mode=WAL`` raises ``OperationalError("locking protocol")``
(SQLITE_PROTOCOL â€” typical on NFS/SMB), Zed must fall back to
``journal_mode=DELETE`` so ``state.db`` / ``kanban.db`` remain usable.

Without this fallback, users on NFS-mounted ``ZED_HOME`` silently lose
``/resume``, ``/title``, ``/history``, ``/branch``, session search, and the
kanban dispatcher â€” because ``SessionDB()`` init propagates the error and
every caller swallows it, leaving ``_session_db = None``.

See: https://www.sqlite.org/wal.html â€” "WAL does not work over a network
filesystem".
"""
...
class TestApplyWalWithFallback:
    ...
    def test_falls_back_to_delete_on_locking_protocol(self, tmp_path, caplog):
        """NFS-style ``locking protocol`` error â†’ DELETE mode + one WARNING."""
    ...
    def test_does_not_downgrade_when_disk_says_wal(self, tmp_path):
        ...
        # And the file is STILL WAL on disk â€” nothing got rewritten
    ...
    def test_warning_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation â€” see zed_cli/kanban_db.py).
...
class TestGetLastInitError:
    def test_none_on_successful_init(self, tmp_path):
        ...
        """
        # Autouse fixture starts at None â€” success-path leaves it None
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        Simulates a filesystem where BOTH WAL and DELETE journal modes fail â€”
...
class TestSessionDbUsesWalFallback:
    def test_sessiondb_works_when_wal_unavailable(self, tmp_path):
        ...
        try:
            # WAL was attempted and rejected â€” fallback kicked in
            assert attempts[0] >= 1, (
                "WAL pragma was never executed â€” check the patch target"
            )
