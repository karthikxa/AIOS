...
We never hit GitHub or Bitwarden in tests â€” subprocess + urllib are
mocked so the suite stays fast and offline-safe.  The "live" pull and
binary download are exercised manually by `zed secrets bitwarden
...
def zed_home(tmp_path, monkeypatch):
    """Point Zed at an isolated home directory."""
    home = tmp_path / ".zed"
    home.mkdir()
...
# ---------------------------------------------------------------------------
# install_bws â€” fully mocked HTTP
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# _safe_extract_member â€” zip-slip containment
# ---------------------------------------------------------------------------
...
def test_install_bws_rejects_malicious_member(zed_home, monkeypatch):
...
def test_install_bws_happy_path(zed_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(zed_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(zed_home, monkeypatch):
...
def test_fetch_server_url_keyed_in_cache(monkeypatch, tmp_path):
    ...
    # US (default empty) â€” fresh fetch.
    bw.fetch_bitwarden_secrets(
        access_token="0.t", project_id="p",
        binary=fake_binary, cache_ttl_seconds=60,
    )
    # EU â€” different server_url, must NOT hit the US cache entry.
    bw.fetch_bitwarden_secrets(
        access_token="0.t", project_id="p",
        binary=fake_binary, cache_ttl_seconds=60,
        server_url="https://vault.bitwarden.eu",
...
# ---------------------------------------------------------------------------
# apply_bitwarden_secrets â€” the public entry point used by env_loader
# ---------------------------------------------------------------------------
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    """No config.yaml present â†’ no BSM call, no crash."""
    home = tmp_path / ".zed"
    home.mkdir()
    monkeypatch.setenv("ZED_HOME", str(home))
...
# ---------------------------------------------------------------------------
# Disk-persisted cache (cross-process â€” speeds up back-to-back CLI invocations)
# ---------------------------------------------------------------------------


def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under zed_home/cache/."""
    home = tmp_path / ".zed"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
    ...
    # Mode must be 0600 â€” disk cache contains plaintext secret values
...
def test_disk_cache_expires_with_ttl(monkeypatch, tmp_path):
    ...
    # Second call: stale disk â†’ refetch
    bw.fetch_bitwarden_secrets(
        access_token="0.t", project_id="proj-1", binary=fake_binary,
        cache_ttl_seconds=300, home_path=home,
    )
...
def test_disk_cache_corrupt_file_falls_through(monkeypatch, tmp_path):
    """A garbage cache file must NOT crash startup â€” we refetch."""
    home = tmp_path / ".zed"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
