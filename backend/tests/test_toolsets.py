"""Tests for toolsets.py â€” toolset resolution, validation, and composition."""
...
class TestResolveToolset:
    ...
    def test_cycle_detection(self):
        ...
        try:
            ...
            # Should not infinite loop â€” cycle is detected
            assert "t1" in tools
            assert "t2" in tools
        finally:
...
class TestToolsetConsistency:
    ...
    def test_zed_platforms_share_core_tools(self):
        """All zed-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on zed-discord, gated on DISCORD_BOT_TOKEN) are allowed on top â€”
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["zed-cli", "zed-telegram", "zed-discord", "zed-whatsapp", "zed-slack", "zed-signal", "zed-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_zed_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("zed-whatsapp")

    def test_zed_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("zed-api-server")
