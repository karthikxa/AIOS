...
only the latest descendant ends up with rows in the ``messages`` table â€”
the parent row has ``message_count = 0``. ``zed --resume <parent_id>``
used to load zero rows and show a blank chat.

``SessionDB.resolve_resume_session_id()`` walks the parent â†’ child chain
