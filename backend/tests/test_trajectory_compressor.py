...
def test_import_loads_env_from_zed_home(tmp_path, monkeypatch):
    home = tmp_path / ".zed"
    home.mkdir()
    (home / ".env").write_text("OPENROUTER_API_KEY=from-zed-home\n", encoding="utf-8")
    ...
    assert os.getenv("OPENROUTER_API_KEY") == "from-zed-home"


def test_generate_summary_kimi_omits_temperature():
    """Kimi models should have temperature omitted â€” server manages it."""
...
# ---------------------------------------------------------------------------
# TrajectoryCompressor â€” compression boundary must not split tool pairs
# ---------------------------------------------------------------------------
...
def _paired_trajectory():
    ...
    token-accumulation boundary stops at index 5 â€” i.e. between gpt#1's
    ...
    return [
        ...
        _tool_response("a", 12),
        _gpt_with_tool_call("b", 400),  # oversized â€” forces a mid-pair boundary
        _tool_response("b", 12),
...
class TestCompressionToolPairIntegrity:
    ...
    def test_sync_compression_does_not_orphan_tool_markers(self):
        ...
        # A kept 'tool' turn must always immediately follow its 'gpt' turn â€”
