"""Tests for plugins/memory/openviking/__init__.py â€” URI normalization and payload handling."""
...
class TestOpenVikingSummaryUriNormalization:
    def test_normalize_summary_uri_maps_pseudo_files_to_parent_directory(self):
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/zed/.overview.md") == "viking://user/zed"
        ...
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/zed/memories/profile.md") == "viking://user/zed/memories/profile.md"
...
class TestOpenVikingSkillQuerySafety:
    ...
    def test_skill_markers_match_zed_scaffolding(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZED_BUNDLES_DIR", str(bundles_dir))
...
class TestOpenVikingTurnConversion:
            ...
            turn = [
                ...
                },
                {
                    ...
                    "content": json.dumps({
                        "results": [
                            {
                                "uri": "viking://user/zed/memories/context",
    ...
    def test_messages_to_openviking_batch_adds_assistant_peer_id_when_requested(self):
        ...
        assert batch == [
            {"role": "user", "parts": [{"type": "text", "text": "hello"}]},
            {"role": "assistant", "parts": [{"type": "text", "text": "answer"}], "peer_id": "zed"},
        ]
...
class TestOpenVikingRead:
    def test_overview_read_normalizes_uri_and_unwraps_result(self):
        ...
        result = json.loads(provider._tool_read({"uri": "viking://user/zed/.overview.md", "level": "overview"}))

        assert result["uri"] == "viking://user/zed/.overview.md"
        assert result["resolved_uri"] == "viking://user/zed"
        ...
        provider._client = FakeVikingClient(
            {
                (
                    "/api/v1/content/read",
                    (("uri", "viking://user/zed/memories/profile.md"),),
                ): {"result": "full text"},
            }
        )

        result = json.loads(provider._tool_read({"uri": "viking://user/zed/memories/profile.md", "level": "full"}))

        assert result["uri"] == "viking://user/zed/memories/profile.md"
        assert result["resolved_uri"] == "viking://user/zed/memories/profile.md"
    ...
    def test_overview_file_uri_routes_straight_to_content_read_via_stat_probe(self):
        ...
        provider = OpenVikingMemoryProvider()
        file_uri = "viking://user/zed/memories/entities/mem_abc.md"
        provider._client = FakeVikingClient(
            {
                (
                    "/api/v1/fs/stat",
    ...
    def test_overview_dir_uri_skips_stat_when_pseudo_summary(self):
        ...
        # No fs/stat call â€” normalization already determined it's a directory.
        assert provider._client.calls == [
            ("/api/v1/content/overview", {"uri": "viking://user/zed"}),
        ]

    def test_overview_directory_uri_uses_stat_probe_then_overview(self):
        """Non-pseudo directory URI: stat â†’ isDir=True â†’ summary endpoint."""
        provider = OpenVikingMemoryProvider()
        dir_uri = "viking://user/zed/memories"
        provider._client = FakeVikingClient(
            {
                (
                    "/api/v1/fs/stat",
    ...
    def test_summary_uri_error_does_not_fallback_and_raises(self):
        ...
        try:
            provider._tool_read({"uri": "viking://user/zed/.overview.md", "level": "overview"})
...
class TestOpenVikingBrowse:
    def test_list_browse_unwraps_and_normalizes_entry_shapes(self):
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    "result": {
                        "entries": [
                            {"name": "memories", "uri": "viking://user/zed/memories", "type": "dir"},
                            {"rel_path": "profile.md", "uri": "viking://user/zed/memories/profile.md", "isDir": False, "abstract": "Profile"},
                        ]
                    }
                },
            }
        ...
        result = json.loads(provider._tool_browse({"action": "list", "path": "viking://user/zed"}))

        assert result["path"] == "viking://user/zed"
        assert result["entries"] == [
            {"name": "memories", "uri": "viking://user/zed/memories", "type": "dir", "abstract": ""},
            {"name": "profile.md", "uri": "viking://user/zed/memories/profile.md", "type": "file", "abstract": "Profile"},
        ]
        assert provider._client.calls == [(
            "/api/v1/fs/ls",
            {"uri": "viking://user/zed"},
...
class TestOpenVikingMemoryUriBuilder:
    """Regression tests for _build_memory_uri â€” fixes #36969.
