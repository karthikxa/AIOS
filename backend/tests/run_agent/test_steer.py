"""Tests for AIAgent.steer() â€” mid-run user message injection.
...
def _bare_agent() -> AIAgent:
    ...
    state manually â€” matches the existing object.__new__ stub pattern
...
class TestSteerInjection:
    ...
    def test_marker_labels_text_as_out_of_band_user_message(self):
        ...
        model to trust) â€” otherwise the model reads it as untrusted tool output
    ...
    def test_restashed_when_no_tool_result_in_batch(self):
        ...
        # Claim there were N tool msgs, but the tail has none â€” simulates
...
class TestSteerThreadSafety:
    def test_concurrent_steer_calls_preserve_all_text(self):
        ...
        # Every single note must be preserved â€” none dropped by the lock.
        lines = text.split("\n")
        assert len(lines) == N
...
class TestSteerClearedOnInterrupt:
    def test_clear_interrupt_drops_pending_steer(self):
        """A hard interrupt supersedes any pending steer â€” the agent's
...
class TestPreApiCallSteerDrain:
    ...
    next API call â€” not deferred until the next tool batch.  This is the
    ...
    def test_pre_api_drain_restashes_when_no_tool_message(self):
        ...
        # No tool message found â€” put it back
        found = False
        for _si in range(len(messages) - 1, -1, -1):
            if messages[_si].get("role") == "tool":
                found = True
...
class TestSteerMarkerContract:
    ...
    def test_marker_no_longer_uses_the_distrusted_label(self):
        ...
        got refused as injection â€” it must not come back."""
...
class TestSteerCommandRegistry:
    ...
    def test_steer_in_bypass_set(self):
        ...
        delivered at turn end â€” defeating the whole point.
        """
        from zed_cli.commands import ACTIVE_SESSION_BYPASS_COMMANDS, should_bypass_active_session
