...
The toggle suppresses notice EMISSION only â€” credits state capture and /usage
...
class TestCreditsNoticesToggle:
    def test_disabled_emits_nothing(self):
        ...
        with patch("zed_cli.config.load_config", return_value=_cfg(False)):
    ...
    def test_enabled_emits_depleted(self):
        ...
        received = []
        agent.notice_callback = received.append
        with patch("zed_cli.config.load_config", return_value=_cfg(True)):
            agent._emit_credits_notices()
        assert any(getattr(n, "key", None) == "credits.depleted" for n in received)

    def test_default_missing_key_emits(self):
        """Key absent from config â†’ fail-open True (current behaviour preserved)."""
        agent = _agent_with_state()
        received = []
        agent.notice_callback = received.append
        with patch("zed_cli.config.load_config", return_value={"display": {}}):
            agent._emit_credits_notices()
        assert any(getattr(n, "key", None) == "credits.depleted" for n in received)
    ...
    def test_toggle_cached_per_agent(self):
        ...
        with patch("zed_cli.config.load_config", return_value=_cfg(True)) as mock_load:
            agent._emit_credits_notices()
            agent._emit_credits_notices()
        assert mock_load.call_count == 1

    def test_disabled_state_still_cached_for_usage(self):
        """The gate stops emission only â€” get_credits_state still returns data."""
        agent = _agent_with_state()
        agent.notice_callback = lambda n: None
