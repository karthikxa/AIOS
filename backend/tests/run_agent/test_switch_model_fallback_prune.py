...
fallback chain â€” seeded from config at agent __init__ â€” kept entries for the
provider the user just moved away from.

Reported: "switched from openrouter provider to anthropic api key via zed
...
def _switch_to_anthropic(agent):
    with (
        ...
        patch("zed_cli.timeouts.get_provider_request_timeout", return_value=None),
    ):
        agent.switch_model(
...
def test_switch_drops_old_primary_from_fallback_chain():
    ...
    assert agent._fallback_model == {"provider": "nous", "model": "zed-4"}
...
def test_switch_within_same_provider_preserves_chain():
    ...
    with patch("zed_cli.timeouts.get_provider_request_timeout", return_value=None):
        agent.switch_model(
