...
Without this, every review rebuilds the system prompt from scratch â€” fresh
``_zed_now()`` timestamp, fresh ``session_id``, and a different skills
prompt under the (former) narrow toolset â€” and the prefix-cache miss costs
...
def _make_agent_stub(agent_cls):
    ...
    agent._cached_system_prompt = (
        "PARENT-SYSTEM-PROMPT-BYTES â€” must be inherited verbatim "
...
class _ReviewAgentRecorder:
    ...
    def run_conversation(self, *args, **kwargs):
        raise RuntimeError("stop after recording state â€” don't actually call the API")
...
def test_review_fork_pins_session_start_and_session_id():
    ...
    assert captured.get("session_start") == agent.session_start, (
        "Review fork did not inherit parent's session_start â€” "
        "system-prompt rebuild paths would diverge."
    )
    assert captured.get("session_id") == agent.session_id, (
        "Review fork did not inherit parent's session_id â€” "
        "system-prompt rebuild paths would diverge."
    )
...
def test_review_fork_inherits_parent_toolset_config():
    ...
    class _Recorder:
        ...
        def run_conversation(self, *args, **kwargs):
            raise RuntimeError("stop after recording â€” don't actually call the API")
