...
1. ``_extract_file_mutation_targets`` â€” pulls file paths from write_file /
   patch (replace + V4A) tool-call argument dicts.
2. ``AIAgent._record_file_mutation_result`` â€” builds the per-turn state
   dict, removing entries when a later success supersedes an earlier
   failure for the same path.
3. ``AIAgent._format_file_mutation_failure_footer`` â€” renders the dict
...
class TestExtractFileMutationTargets:
    ...
    def test_patch_default_mode_is_replace(self):
        # Mode omitted â€” schema default is ``replace``.
...
# ---------------------------------------------------------------------------
# _record_file_mutation_result â€” state transitions
# ---------------------------------------------------------------------------
...
def _bare_agent() -> AIAgent:
    ...
    the filesystem.  For these tests we only need the two methods â€”
...
class TestRecordFileMutationResult:
    ...
    def test_repeated_failure_keeps_first_error(self):
        ...
        # Keep the original error â€” swapping to the latest would obscure
    ...
    def test_no_state_dict_silent_noop(self):
        ...
        The record helper must never raise â€” a tool dispatched from, say,
    ...
    def test_missing_path_arg_recorded_nowhere(self):
        ...
        # No path â†’ nothing to key on, state stays empty.  The per-turn
...
class TestFormatFooter:
    ...
    def test_truncation_at_10_entries(self):
        ...
        assert "â€¦ and 5 more" in out
        # Ten file bullets + header + "and X more" line
        lines = out.split("\n")
        bullet_lines = [ln for ln in lines if ln.lstrip().startswith("â€¢")]
    ...
    def test_paths_are_backtick_wrapped(self):
        ...
        out = AIAgent._format_file_mutation_failure_footer(
            {"/home/u/.zed/config.yaml": {
                "tool": "patch",
                "error_preview": (
                    "Write denied: '/home/u/.zed/config.yaml' is a "
                    "protected system/credential file."
                ),
            }},
        )
        # Path still human-readable.
        assert "/home/u/.zed/config.yaml" in out
    ...
    def test_footer_path_not_extracted_by_gateway(self):
        ...
        tmp = tempfile.mkdtemp(prefix="zed_footer_")
        try:
...
# ---------------------------------------------------------------------------
# _file_mutation_verifier_enabled â€” env + config precedence
# ---------------------------------------------------------------------------
...
class TestVerifierEnabled:
    ...
    def test_env_disables(self, monkeypatch, value):
        monkeypatch.setenv("ZED_FILE_MUTATION_VERIFIER", value)
        agent = _bare_agent()
