...
These tests exercise:
  1. ``_format_turn_completion_explanation`` â€” the pure reasonâ†’message map.
  2. ``_turn_completion_explainer_enabled`` â€” the env/config seam.
...
def test_explainer_disabled_via_env():
    agent = _make_agent()
    with patch.dict(
        os.environ, {"ZED_TURN_COMPLETION_EXPLAINER": "0"}, clear=False
    ):
