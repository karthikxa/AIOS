...
def _isolate_zed(tmp_path, monkeypatch):
...
def _make_agent(monkeypatch):
    ...
    monkeypatch.setenv("ZED_INFERENCE_PROVIDER", "")
    # Avoid full AIAgent init â€” just import the class and build a stub
    ...
    # tool batch. Stub it as a no-op â€” this test exercises interrupt
...
def test_clear_interrupt_clears_worker_tids(monkeypatch):
    ...
    next turn's tools â€” which may be scheduled onto recycled tids â€” don't
    ...
    assert is_interrupted() is False, (
        ...
        "worker tid â€” stale interrupt can leak into the next turn"
    )
