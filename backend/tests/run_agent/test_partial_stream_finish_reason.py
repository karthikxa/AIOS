"""Regression tests for issue #30963 â€” partial-stream stub finish_reason.

Pins the contract:

- text-only partial stream â†’ stub.finish_reason == "length" so the
  conversation loop's existing length-continuation path can keep the
  agent moving against an unfinished goal.
- partial mid-tool-call â†’ stub.finish_reason == "length" so the loop
...
# â”€â”€ Helpers (mirrors test_streaming.py) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Stub finish_reason â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestPartialStreamStubFinishReason:
    ...
    def test_text_only_partial_returns_length(self, _mock_close, mock_create, monkeypatch):
        ...
        monkeypatch.setenv("ZED_STREAM_RETRIES", "0")
    ...
    def test_partial_tool_call_uses_length(self, _mock_close, mock_create, monkeypatch):
        ...
        conversation loop's continuation machinery fires â€” bounded 3-retry
...
# â”€â”€ Clean stream-end mid-tool-call (no exception, no finish_reason) â”€â”€â”€â”€â”€â”€â”€â”€â”€

class TestCleanStreamEndMidToolCall:
    """The upstream closes the SSE stream cleanly after delivering a tool
    name + the opening '{' of its arguments â€” NO exception, NO finish_reason,
    ...
    def test_no_finish_reason_partial_tool_args_routes_to_stub(
        ...
        def _clean_ending_stream():
            ...
            # generator simply RETURNS (StopIteration) â€” no raise, no
            ...
            # falls off the end â€” clean close, no terminator
        ...
        assert response.id == PARTIAL_STREAM_STUB_ID, (
            ...
            "truncation â€” otherwise the loop reports the false 'output "
            "length limit' error."
        )
        assert response.choices[0].finish_reason == FINISH_REASON_LENGTH
        assert response.choices[0].message.tool_calls is None, (
    ...
    def test_real_length_truncation_still_uses_uuid_id(
        ...
        partial tool args, it is a genuine output cap â€” keep the existing
...
# â”€â”€ Length-continuation prompt branching â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Integration: live conversation loop â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class TestConversationLoopPartialStreamContinuation:
    ...
    def test_partial_stream_stub_does_not_exit_loop_immediately(self, loop_agent):
        ...
        through length_continue_retries â€” the loop persists the partial
        ...
        # variant, not the "output length limit" lie â€” otherwise the model
