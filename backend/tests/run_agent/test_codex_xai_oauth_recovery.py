...
1. ``RuntimeError("Expected to have received `response.created` before
   ...
   ``response.completed`` postlude â€” fall back to a non-stream
...
3. Multi-turn replay of ``codex_reasoning_items`` (with
   ...
   reverted shortly after: xAI confirmed they explicitly want Zed to
...
# the wire and us â€” we read the wire directly.  When the chatgpt.com Codex
...
# strictly better than the old "SDK raises RuntimeError â†’ we retry â†’ fall
...
def test_codex_stream_retries_remote_protocol_error_once():
    ...
    # max_stream_retries=1 â†’ one retry + final attempt â†’ 2 create calls total.
...
def test_summarize_api_error_decorates_xai_entitlement_403():
    ...
    # The original xAI text must survive â€” it's still useful diagnostic info.
...
def test_summarize_api_error_does_not_accuse_subscribers():
    ...
    contradict him â€” leading with the X Premium+ gotcha gives subscribers
...
# lists, so it returned FailoverReason.unknown (retryable=True) â€” burning
...
def test_classify_api_error_stream_event_unrelated_not_reclassified():
    ...
    err = _StreamErrorEvent("Internal server error â€” try again later")
    result = classify_api_error(err, provider="xai-oauth", model="grok-4.3")
    assert result.reason != FailoverReason.auth
...
def test_codex_reasoning_replay_includes_encrypted_content_for_xai():
    ...
    want Zed to thread encrypted reasoning back across turns for
    cross-turn coherence â€” that's the whole point of the partnership
    integration.
    """
    from agent.codex_responses_adapter import _chat_messages_to_responses_input
    ...
    reasoning = [it for it in items if it.get("type") == "reasoning"]
    assert len(reasoning) == 1, (
        "xAI must receive replayed reasoning items â€” see docstring for the "
...
def test_recover_with_credential_pool_skips_refresh_on_entitlement_403():
    ...
    loop indefinitely: refresh â†’ 403 â†’ refresh â†’ 403, infinitely.  With
...
def test_recover_with_credential_pool_skips_refresh_on_bare_403_for_xai_oauth():
    ...
    Regression for #26847 â€” xAI's backend has been seen to 403 standard
    ...
    assert not AIAgent._is_entitlement_failure(error_context, 403), (
        ...
        "heuristic â€” otherwise this test isn't covering the defense-in-depth path."
    )
...
def test_recover_with_credential_pool_still_refreshes_genuine_auth_failure():
    ...
    class _FakePool:
        def try_refresh_current(self):
            refresh_calls["n"] += 1
            # Return a fake refreshed entry â€” semantically "refresh worked"
            entry = MagicMock()
    ...
    agent._credential_pool = _FakePool()
    # _swap_credential is called by the recovery path â€” stub it out
...
# xAI's authoritative disambiguator â€” when present, the body is an auth
...
def test_is_entitlement_failure_false_for_bad_credentials_wke_suffix():
    ...
    Verbatim shape from the #29344 reporter â€” the ``code`` text matches
...
@pytest.mark.parametrize("wke_variant", [
    # The headline variant â€” what xAI returns today.
...
def test_is_entitlement_failure_case_insensitive_wke_match():
    """Substring match is case-insensitive â€” the classifier lowercases
...
def test_recover_with_credential_pool_refreshes_on_xai_bad_credentials_403():
    ...
    but with the exact 403 body shape xAI ships for stale tokens â€”
    ...
    assert recovered is True, (
        "Stale OAuth token (bad-credentials 403) must trigger refresh â€” "
...
def test_recover_with_credential_pool_still_blocks_real_entitlement():
    ...
    #26847 protection â€” entitlement 403 must NOT refresh â€” must
    ...
    # Pure entitlement body â€” no WKE suffix, no OAuth2 phrase.
    error_context = {
        "reason": (
            "The caller does not have permission to execute the specified operation"
        ),
...
def test_grok_4_3_context_length_is_1m():
    ...
    Zed' substring-match fallback used to return 256k (from the
...
def test_cross_issuer_reasoning_is_dropped_on_replay():
    ...
    # Calling against codex_backend â€” the grok-issued blob must be dropped.
...
def test_transport_round_trip_drops_foreign_reasoning():
    ...
    messages = [
        ...
        {"role": "user", "content": "hi"},
        _stamped_assistant_msg("xai_responses", encrypted="grok_blob"),
        {"role": "user", "content": "ì—‘ìŠ¤ë‹¤ìž„ í”„ë¡œì íŠ¸ íŒŒì•…, ìŠ¤í‚¬ë¡œ ì •ë¦¬."},
    ]
    ...
    assert reasoning == [], (
        "Cross-issuer reasoning leaked through build_kwargs â€” this is the "
