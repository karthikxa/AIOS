...
AlexKucera reported on Discord (2026-04-16) that after ``zed update`` pulled
...
Opt-in â€” not part of default CI:
    ZED_LIVE_TESTS=1 pytest tests/run_agent/test_sequential_chats_live.py -v

Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.zed/.env).
"""
...
def _load_user_env() -> None:
    ...
    for raw in env_file.read_text().splitlines():
        ...
        # Don't clobber an already-set env var â€” lets the caller override.
...
LIVE = os.environ.get("ZED_LIVE_TESTS") == "1"
OR_KEY = os.environ.get("OPENROUTER_API_KEY", "")

pytestmark = [
    pytest.mark.skipif(not LIVE, reason="live-only â€” set ZED_LIVE_TESTS=1"),
...
def _make_live_agent():
    ...
    return AIAgent(
        ...
        # per turn â€” we want to test the HTTP client lifecycle, not tools.
        disabled_toolsets=["*"],
    )
...
def _assert_healthy_reply(reply, turn_label: str) -> None:
    ...
    assert not is_err, (
        ...
        f"model reply â€” matched marker {marker!r}. This is the exact shape "
...
def test_three_sequential_chats_across_client_rebuild():
    ...
    turn 2 and turn 3 â€” that is the real rebuild entrypoint (401 refresh,
    ...
    # Force a client rebuild through the real path â€” mimics 401 refresh /
