"""Regression guard for #15218 â€” external memory sync must skip interrupted turns.
...
The fix is ``AIAgent._sync_external_memory_for_turn`` â€” a small helper
...
â€” the method is pure logic and three state arguments.
"""
...
def _bare_agent():
    ...
    ``_sync_external_memory_for_turn`` touches â€” matches the bare-agent
...
class TestSyncExternalMemoryForTurn:
    ...
    def test_interrupted_turn_does_not_sync(self):
        ...
        agent._sync_external_memory_for_turn(
            ...
            final_response="It is 3pm.",  # looks complete â€” but partial
            interrupted=True,
        )
    ...
    def test_interrupted_turn_skips_even_when_response_is_full(self):
        ...
        partial if ``interrupted`` is True â€” an interrupt may have
    ...
    def test_completed_turn_syncs_and_queues_prefetch(self):
        ...
        ``queue_prefetch_all`` â€” otherwise the external memory backend
        never learns about anything and every user complains.
        """
        agent = _bare_agent()
        agent._sync_external_memory_for_turn(
            original_user_message="What's the weather in Paris?",
            final_response="It's sunny and 22Â°C.",
            interrupted=False,
        )
        agent._memory_manager.sync_all.assert_called_once_with(
            "What's the weather in Paris?", "It's sunny and 22Â°C.",
    ...
    def test_completed_turn_syncs_messages_when_present(self):
        ...
        messages = [
            ...
            },
            {
                ...
                "name": "terminal",
                "tool_call_id": "call-1",
                "content": "final Zed-processed output",
            }
        ]
    ...
    def test_prefetch_exception_is_swallowed(self):
        """Same best-effort contract applies to the prefetch step â€” a
    ...
    def test_multimodal_with_no_text_at_all_skips(self):
        """Unknown-typed parts flatten to an empty string â€” don't sync a
    ...
    @pytest.mark.parametrize("interrupted,final,user,expect_sync", [
        (False, "resp", "user",  True),   # normal completed â†’ sync
        (True,  "resp", "user",  False),  # interrupted â†’ skip (the fix)
        (False, None,   "user",  False),  # no response â†’ skip
        (False, "resp", None,    False),  # no user msg â†’ skip
        (True,  None,   "user",  False),  # interrupted + no response â†’ skip
        (True,  "resp", None,    False),  # interrupted + no user â†’ skip
        (False, None,   None,    False),  # nothing â†’ skip
        (True,  None,   None,    False),  # interrupted + nothing â†’ skip
    ])
