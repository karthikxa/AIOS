...
def test_aiagent_reuses_existing_errors_log_handler():
    ...
    error_log_path = (run_agent._zed_home / "logs" / "errors.log").resolve()
...
class TestStripThinkBlocks:
    ...
    # â”€â”€â”€ Tool-call XML block stripping (openclaw/openclaw#67318) â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def test_function_mention_in_prose_preserved(self, agent):
        """'Use <function> in JavaScript.' â€” no name attr, not at block boundary
...
class TestExtractReasoning:
    ...
    def test_content_list_thinking_blocks_extracted(self, agent):
        ...
        Without this branch thinking text is silently dropped â†’ HTTP 400 on
    ...
    def test_content_list_thinking_prefers_structured_field(self, agent):
        ...
        result = agent._extract_reasoning(msg)
        # structured field was found first â†’ content-list branch skipped
...
class TestSessionJsonSnapshotOptIn:
    ...
    files by default â€” that behavior caused multi-GB sessions directories
...
class TestSaveSessionLogRedactsSecrets:
    ...
    def _ensure_redaction_enabled(self, monkeypatch):
        """Force redaction on regardless of host ZED_REDACT_SECRETS state.
        ...
        monkeypatch.delenv("ZED_REDACT_SECRETS", raising=False)
...
class TestToolUseEnforcementConfig:
    ...
    def test_auto_injects_execution_guidance_for_grok(self):
        ...
        practice â€” claims completion without tool calls, suggests workarounds
...
class TestTaskCompletionGuidance:
    ...
    Unlike tool_use_enforcement, this block is model-family-agnostic â€” it
    ...
    def test_default_injects_for_claude(self):
        """The block must reach Claude by default â€” that's the
    ...
    def test_default_injects_for_deepseek(self):
        """And for DeepSeek â€” the other model that failed the Sarasota
    ...
    def test_default_injects_for_gpt(self):
        """Also reaches model families that already get enforcement â€”
    ...
    def test_no_tools_no_injection(self):
        """Same gate as tool_use_enforcement â€” no tools means no guidance.
...
class TestEnvironmentProbeIntegration:
    ...
    def test_probe_silent_on_clean_env(self, monkeypatch):
        """Clean environment â†’ probe emits nothing â†’ no line in prompt."""
        from tools import env_probe
        env_probe._reset_cache_for_tests()
        monkeypatch.setattr(env_probe, "_python_version_of",
...
class TestBuildApiKwargs:
    ...
    def test_reasoning_sent_for_nous_route(self, agent):
        agent.provider = "nous"
        agent.base_url = "https://inference-api.zedteam.com/v1"
        agent.model = "minimax/minimax-m2.5"
        messages = [{"role": "user", "content": "hi"}]
        kwargs = agent._build_api_kwargs(messages)
        assert kwargs["extra_body"]["reasoning"]["effort"] == "medium"
...
class TestBuildAssistantMessage:
    ...
    def test_streaming_only_reasoning_promoted_to_reasoning_content(self, agent):
        ...
        message â€” only ``reasoning`` (or the internal accumulator).
        ...
        # ``reasoning_content`` â€” the streaming-only provider shape.
    ...
    def test_no_reasoning_text_leaves_field_absent(self, agent):
        ...
        tiers at replay time â€” cross-provider leak guard (#15748),
        promote-from-``reasoning``, and DeepSeek/Kimi " "-pad â€” which
    ...
    def test_memory_context_in_stored_content_is_preserved(self, agent):
        ...
        containing literal <memory-context> markers â€” that's legitimate text
...
class TestExecuteToolCalls:
    ...
    def test_keyboard_interrupt_emits_cancelled_post_tool_hook(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("zed_cli.plugins.invoke_hook", _capture_hook)
        monkeypatch.setattr("zed_cli.plugins.has_hook", lambda name: True)
...
class TestConcurrentToolExecution:
    ...
    def test_invoke_tool_agent_level_tool_emits_terminal_post_tool_hook(self, agent, monkeypatch):
        ...
        hook_calls = []
        monkeypatch.setattr(
            "zed_cli.plugins.get_pre_tool_call_block_message",
            lambda *args, **kwargs: None,
        )
        monkeypatch.setattr(
            "zed_cli.plugins.invoke_hook",
    ...
    def test_sequential_agent_level_tool_execution_middleware_wraps_inline_dispatch(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("zed_cli.plugins.get_plugin_manager", lambda: manager)
        monkeypatch.setattr(
            "zed_cli.plugins.invoke_middleware",
...
class TestAgentRuntimePostHookOwnershipSync:
    ...
    def test_invoke_tool_dispatch_matches_inline_dispatch_chain(self):
        ...
        path) must cover the same set of agent-runtime tools â€” otherwise
...
class TestHandleMaxIterations:
    ...
    def test_summary_strips_strict_schema_foreign_fields(self, agent):
        ...
        Chat-Completions-schema-foreign keys â€” tool_name (SQLite FTS
        ...
        # Internal history is untouched â€” the path copies each message.
...
class TestRunConversation:
    ...
    def test_ollama_small_runtime_context_fails_before_api_call(self, agent, caplog):
        ...
        assert "Ollama runtime context too small for Zed tool use" in caplog.text
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            ...
            patch(
                "zed_cli.plugins.has_hook",
                side_effect=lambda name: name in {"pre_api_request", "post_api_request"},
            ),
            patch("zed_cli.plugins.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_api_request_error_hook_skips_payload_work_without_listener(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("zed_cli.plugins.has_hook", lambda name: False)
        monkeypatch.setattr("zed_cli.plugins.invoke_hook", _invoke_hook)
    ...
    def test_request_scoped_api_hooks_skip_payload_work_without_listeners(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("zed_cli.plugins.has_hook", _has_hook)
        ...
        with (
            patch("zed_cli.plugins.invoke_hook", return_value=[]),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_glm_prompt_exceeds_max_length_triggers_compression(self, agent):
        ...
        self._setup_agent(agent)
        agent.compression_enabled = True  # this test verifies overflowâ†’compression fires
        err_400 = Exception(
    ...
    def test_non_minimax_overflow_without_provider_limit_keeps_context(self, agent):
        ...
        rely on compression â€” see #33669 / PR #33826.
        """
        self._setup_agent(agent)
        agent.compression_enabled = True  # this test verifies overflowâ†’compression fires
        agent.provider = "openrouter"
        ...
        mock_compress.assert_called_once()
        # Context length preserved â€” no guessed probe-tier step-down.
    ...
    def test_stub_stall_mid_tool_call_recovers_within_3_retries(self, agent):
        ...
        retry up to 3 times rather than hard-failing after one â€” and recover
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("ZED_KANBAN_TASK", "t_test_task_123")
        ...
        summary_resp = _mock_response(
            content="Could not finish â€” budget exhausted.", finish_reason="stop",
        )
        agent.client.chat.completions.create.side_effect = [
        ...
        with (
            ...
            patch("zed_cli.kanban_db._record_task_failure",
                  mock_record_failure),
            patch("zed_cli.kanban_db.connect", mock_connect),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_no_kanban_block_when_not_in_kanban_mode(self, agent, monkeypatch):
        """The exhaustion bridge must NOT fire when ZED_KANBAN_TASK
...
class TestHookPayloadSanitizesSimpleNamespace:
    ...
    tool_call objects as ``types.SimpleNamespace`` â€” see
    ...
    def test_api_response_payload_for_hook_normalizes_simplenamespace_tool_calls(self, agent):
        ...
        # agent/codex_responses_adapter.py â€” raw SDK objects are SimpleNamespace.
...
class TestRetryExhaustion:
    ...
    def test_invalid_response_returns_error_not_crash(self, agent):
        ...
        # in time/jittered_backoff at module level â€” patch BOTH so the
    ...
    def test_content_filter_refusal_surfaced_not_retried(self, agent):
        ...
        # Crucial regression guard: a deterministic refusal is NOT retried â€”
...
class TestNousCredentialRefresh:
    ...
    def test_try_refresh_nous_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "zed_cli.auth.resolve_nous_runtime_credentials", _fake_resolve
        )
        ...
        assert (
            rebuilt["kwargs"]["base_url"] == "https://inference-api.zedteam.com/v1"
        )
...
class TestMaxTokensParam:
    ...
    # â”€â”€ Model-name fallback for non-openai.com endpoints serving newer families â”€â”€
    ...
    def test_returns_max_completion_tokens_for_gpt5_on_custom_endpoint(self, agent):
        ...
        max_completion_tokens â€” otherwise the server 400s on max_tokens."""
...
class TestGpt5ApiModeRouting:
    ...
    def test_azure_gpt5_stays_on_chat_completions(self, agent):
        """Azure serves gpt-5.x on /chat/completions â€” must not upgrade to codex_responses."""
        agent.base_url = "https://my-resource.openai.azure.com/openai/v1"
        agent.api_mode = "chat_completions"
    ...
    def test_nous_gpt5_stays_on_chat_completions(self, agent):
        """Nous serves gpt-5.x on /chat/completions â€” must not upgrade to codex_responses."""
        agent.provider = "nous"
        agent.base_url = "https://inference-api.zedteam.com/v1"
        agent.api_mode = "chat_completions"
    ...
    def test_is_azure_openai_url_detection(self, agent):
        ...
        # Path-embedded azure string should still detect â€” we're ~substring matching
        agent.base_url = "https://my-resource.openai.azure.com/openai/v1"
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "Zed Agent" in agent._cached_system_prompt
...
class TestStreamingApiCall:
    ...
    def test_truncated_tool_call_args_no_finish_reason_routes_to_stub(self, agent):
        ...
        # with no finish_reason (the SSE just stops â€” no terminator, no
    ...
    def test_truncated_tool_call_args_with_length_finish_reason_upgrades(self, agent):
        ...
        # existing behaviour â€” tool_calls preserved, finish_reason 'length' â€”
...
class TestMemoryContextSanitization:
    """sanitize_context() helper correctness â€” used at provider boundaries."""

    def test_user_message_is_not_mutated_by_run_conversation(self):
        """User input must reach run_conversation untouched â€” if a user types
