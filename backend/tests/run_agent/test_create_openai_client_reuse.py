...
That test pins "don't mutate input kwargs" at the syntactic level â€” it catches
...
AlexKucera's Discord report (2026-04-16): after ``zed update`` pulled
...
def test_second_create_does_not_wrap_closed_transport_from_first():
    ...
    with patch("run_agent.OpenAI", fake_openai):
        # Call 1 â€” what _replace_primary_openai_client does at init/rebuild.
        ...
        # Call 2 â€” the rebuild path. This is where #10933 crashed on the
        # next real request.
    ...
    # past the bug class â€” nothing to share, nothing to close. That's fine.
...
def test_replace_primary_openai_client_survives_repeated_rebuilds():
    ...
    _replace_primary_openai_client is the real rebuild entrypoint â€” it is
    ...
    with patch("run_agent.OpenAI", fake_openai):
        ...
        for label in ("rebuild_1", "rebuild_2", "rebuild_3"):
            ...
            assert not cur._closed, (
                f"after rebuild {label}, self.client is already closed â€” "
            ...
            if hc is not None:
                ...
                if is_closed_attr is not None:
                    assert not is_closed_attr, (
                        ...
                        "closed â€” reproduces #10933 (AlexKucera report, "
                        "Discord 2026-04-16)"
                    )
    ...
    assert len({id(c) for c in constructed}) == len(constructed), (
        ...
        "a teardown â€” rebuild is not producing fresh clients"
    )
...
def test_force_close_tcp_sockets_descends_httpcore_1_connection_wrapper():
    ...
    FD via ``sock.close()`` â€” that race recycled FDs into unrelated file
    ...
    # #29507: close() must NOT be called from this helper â€” the owning
