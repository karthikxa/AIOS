...
class TestStripNonAscii:
    ...
    def test_removes_non_ascii(self):
        assert _strip_non_ascii("hello âš• world") == "hello  world"

    def test_removes_emoji(self):
        assert _strip_non_ascii("test ðŸ¤– done") == "test  done"
...
class TestSanitizeMessagesNonAscii:
    ...
    def test_sanitizes_content_string(self):
        messages = [{"role": "user", "content": "hello âš• world"}]
    ...
    def test_sanitizes_content_list(self):
        messages = [{
            "role": "user",
            "content": [{"type": "text", "text": "hello ðŸ¤–"}]
        }]
        assert _sanitize_messages_non_ascii(messages) is True
        assert messages[0]["content"][0]["text"] == "hello "

    def test_sanitizes_name_field(self):
        messages = [{"role": "tool", "name": "âš•tool", "content": "ok"}]
        assert _sanitize_messages_non_ascii(messages) is True
    ...
    def test_multiple_messages(self):
        messages = [
            {"role": "system", "content": "âš• System prompt"},
...
class TestSurrogateVsAsciiSanitization:
    ...
    def test_ascii_codec_strips_all_non_ascii(self):
        ...
        messages = [{"role": "user", "content": "test âš•ðŸ¤–ä½ å¥½ end"}]
        assert _sanitize_messages_non_ascii(messages) is True
...
class TestApiKeyNonAsciiSanitization:
    ...
    Covers the root cause of issue #6843: a non-ASCII character (Ê‹ U+028B)
    ...
    def test_strip_non_ascii_from_api_key(self):
        """_strip_non_ascii removes Ê‹ from an API key string."""
        key = "sk-proj-abc" + "Ê‹" + "def"
        assert _strip_non_ascii(key) == "sk-proj-abcdef"

    def test_api_key_at_position_153(self):
        """Reproduce the exact error: Ê‹ at position 153 in 'Bearer <key>'."""
        key = "sk-proj-" + "a" * 138 + "Ê‹" + "bcd"
        auth_value = f"Bearer {key}"
        # This is what httpx does â€” and it fails:
        ...
        tools = [
            {
                ...
                "function": {
                    "name": "read_file",
                    "description": "Print structured output â”‚ with emoji ðŸ¤–",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "path": {
...
class TestSanitizeStructureNonAscii:
    def test_sanitizes_nested_dict_structure(self):
        payload = {
            "default_headers": {
                "X-Title": "Zed â”‚ Agent",
                "User-Agent": "Zed/1.0 ðŸ¤–",
            }
        }
        assert _sanitize_structure_non_ascii(payload) is True
        assert payload["default_headers"]["X-Title"] == "Zed  Agent"
        assert payload["default_headers"]["User-Agent"] == "Zed/1.0 "
...
class TestApiKeyClientSync:
    ...
    def test_client_api_key_updated_on_sanitize(self):
        ...
        bad_key = "sk-proj-abc\u028bdef"  # Ê‹ lookalike at position 11
...
class TestApiMessagesAndApiKwargsSanitized:
    ...
    'System encoding is ASCII â€” stripped...' log line appears.
    """
