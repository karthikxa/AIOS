...
â€¦before any HTTP request is issued.  This trips the
``openai-codex`` / ``gpt-5.5`` combo on ``chatgpt.com/backend-api/codex``
whenever the user runs Zed without external tools registered: the
...
never emit ``tools=None`` â€” only add the ``tools`` key when there are
...
# Stub optional deps the parent module imports at top level â€” keeps this
...
def test_build_kwargs_omits_tool_choice_and_parallel_when_no_tools(transport, codex_messages):
    ...
    tools â€” and some backends 400 on them.  Confirm we never set them."""
    kwargs = _build_kwargs_no_tools(transport, codex_messages)
...
def test_build_kwargs_keeps_required_codex_fields_without_tools(transport, codex_messages):
    ...
    (model / instructions / input / store) â€” otherwise we'd just be moving
...
def test_build_kwargs_drops_empty_tools_list(transport, codex_messages):
    """``tools=[]`` collapses to ``None`` inside ``_responses_tools`` â€”
...
def test_openai_sdk_raises_typeerror_on_tools_none():
    ...
    gracefully, this test will start failing â€” at which point the agent
