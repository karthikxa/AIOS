"""Tests for issue #860 â€” SQLite session transcript deduplication.
    ...
    def test_flush_writes_only_new_messages(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                # First flush â€” should write 2 new messages
                agent._flush_messages_to_session_db(messages, conversation_history)

                rows = db.get_messages(agent.session_id)
                assert len(rows) == 2, f"Expected 2 messages, got {len(rows)}"

                # Second flush with SAME messages â€” should write 0 new messages
                agent._flush_messages_to_session_db(messages, conversation_history)
    ...
    def test_flush_writes_incrementally(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                # Second flush â€” should write only 2 new messages
                agent._flush_messages_to_session_db(messages, conversation_history)
                rows = db.get_messages(agent.session_id)
