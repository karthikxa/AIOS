...
def test_build_api_kwargs_codex(monkeypatch):
    ...
    assert kwargs["instructions"] == "You are Zed."
...
# should deep-copy first" â€” see ``tools/schema_sanitizer.py``).  Until this
...
# ``pattern``/``format`` keywords from the per-agent tool registry â€” any
...
def test_build_api_kwargs_xai_strips_slash_enum_from_outgoing_request(monkeypatch):
    ...
    assert "enum" not in params["properties"]["accept"], (
        "outgoing xAI request must not carry slash-containing enums â€” "
...
def test_build_api_kwargs_xai_does_not_mutate_agent_tools(monkeypatch):
    ...
    non-xAI call from the same agent saw an already-stripped schema â€”
    ...
    assert accept_after.get("enum") == ["application/json", "*/*"], (
        "agent.tools mutated â€” slash-containing enum was stripped from the "
        "shared per-agent registry, will leak to non-xAI calls"
    )
    assert match_after.get("pattern") == "^[a-z]+$", (
        "agent.tools mutated â€” pattern stripped from shared registry"
    )
    assert match_after.get("format") == "regex", (
        "agent.tools mutated â€” format stripped from shared registry"
    )
...
def test_run_codex_stream_returns_collected_items_when_stream_ends_without_terminal(monkeypatch):
    ...
    returns whatever it collected â€” no retry, no separate fallback path.
    """
    agent = _build_agent(monkeypatch)
...
def test_run_codex_stream_parses_create_stream_events(monkeypatch):
    ...
    # items â€” we don't read those directly, we read the items via output_item.done,
...
def test_run_codex_stream_ignores_completed_response_with_null_output(monkeypatch):
    ...
    'NoneType' object is not iterable``. The Zed runtime consumes raw
...
def test_build_api_kwargs_xai_oauth_sends_cache_key_via_extra_body(monkeypatch):
    ...
    Responses-API cache key â€” see docs.x.ai prompt-caching/maximizing-
    cache-hits).
    ...
    # Top-level kwarg must NOT be set â€” that's the openai SDK
    ...
    assert extra_body.get("prompt_cache_key"), (
        ...
        "for /v1/responses â€” body field is the documented surface."
    )
...
def test_run_conversation_xai_oauth_refreshes_after_401_and_retries(monkeypatch):
    ...
    handler that fires for openai-codex must also fire for xai-oauth â€” the
...
def test_try_refresh_codex_client_credentials_handles_xai_oauth(monkeypatch):
    ...
    xai-oauth (both speak codex_responses) â€” covering both cases prevents
    ...
    monkeypatch.setattr(
        "zed_cli.auth.resolve_xai_oauth_runtime_credentials",
        _fake_resolve,
    )
...
def test_try_refresh_codex_client_credentials_skips_xai_oauth_when_singleton_differs(monkeypatch):
    ...
    def _fake_resolve(force_refresh=False, refresh_if_expiring=True, **_):
        ...
        # The pre-refresh guard read â€” return the singleton's view of the
    ...
    assert refresh_calls["count"] == 0, (
        "force_refresh must not run â€” that would mutate the singleton's "
...
def test_preflight_codex_api_kwargs_drops_invalid_timeout(monkeypatch):
    """Zero, negative, inf, and booleans are all dropped â€” not passed to SDK."""
    agent = _build_agent(monkeypatch)
    from agent.codex_responses_adapter import _preflight_codex_api_kwargs
...
def test_normalize_codex_response_ignores_tool_call_text_when_real_tool_call_present(monkeypatch):
    ...
    structured call â€” don't wipe content that came alongside a real tool use.
    """
    agent = _build_agent(monkeypatch)
    from agent.codex_responses_adapter import _normalize_codex_response
...
def test_interim_commentary_preserves_assistant_content(monkeypatch):
    ...
    literal <memory-context> markers â€” that's legitimate model output (docs,
...
def test_stream_delta_strips_leaked_memory_context_across_chunks(monkeypatch):
    """Regression for #5719 â€” the real streaming case.
    ...
    â€” only a stateful scrubber can.  None of the payload, system-note
...
def test_stream_delta_scrubber_resets_between_turns(monkeypatch):
    ...
    # Simulate a hung span carried over â€” directly populate the scrubber.
...
def test_stream_delta_preserves_mid_stream_leading_newlines(monkeypatch):
    """Mid-stream leading newlines must survive â€” they are legitimate
    ...
    # First delta delivers text â€” strips its own leading "\n" once.
    agent._fire_stream_delta("\nHere is a list:")
    # Second delta starts with "\n- item" â€” must NOT be stripped.
    agent._fire_stream_delta("\n- first")
    agent._fire_stream_delta("\n- second")
...
def test_dump_api_request_debug_redacts_request_and_error_secrets(monkeypatch, tmp_path, capsys):
    ...
    _patch_agent_bootstrap(monkeypatch)
    monkeypatch.setenv("ZED_DUMP_REQUEST_STDOUT", "1")
    agent = run_agent.AIAgent(
        model="gpt-4o",
        base_url="http://127.0.0.1:9208/v1",
...
def _codex_reasoning_only_response(*, encrypted_content="enc_abc123", summary_text="Thinking..."):
    """Codex response containing only reasoning items â€” no message text, no tool calls."""
    return SimpleNamespace(
        output=[
            SimpleNamespace(
                type="reasoning",
...
def test_run_conversation_codex_continues_after_reasoning_only_response(monkeypatch):
    """End-to-end: reasoning-only â†’ final message should succeed, not hit retry loop."""
    agent = _build_agent(monkeypatch)
    responses = [
        _codex_reasoning_only_response(),
...
def test_chat_messages_to_responses_input_deduplicates_reasoning_ids(monkeypatch):
    ...
    # IDs must be stripped â€” with store=False the API 404s on id lookups.
