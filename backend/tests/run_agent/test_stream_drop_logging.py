...
def test_log_stream_retry_works_without_diag(caplog):
    """diag is optional â€” older callers / unit tests still work."""
    agent = _make_agent()
...
def test_emit_stream_drop_ui_omits_suffix_without_diag():
    """When there's no diag, no suffix â€” line stays compact."""
    agent = _make_agent()
    agent.provider = "openrouter"
...
def test_quiet_mode_does_not_clobber_runagent_logger_level():
    """Regression guard for the parent fix â€” must persist across this PR."""
    _ = _make_agent()
    for name in ("run_agent", "tools", "trajectory_compressor", "cron", "zed_cli"):
