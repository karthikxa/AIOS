...
#   - cli.py, zed_cli/*      -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    if quiet_mode:
        ...
        cache_key = (
            ...
            cfg_fp,
            bool(os.environ.get("ZED_KANBAN_TASK")),
...
def _truncate_tool_descriptions(tools: List[Dict], max_desc_chars: int = 60, max_param_chars: int = 20):
    for td in tools:
        func = td.get("function", {})
        desc = func.get("description", "")
        if len(desc) > max_desc_chars:
            func["description"] = desc[:max_desc_chars].rsplit(" ", 1)[0] + "."
        params = func.get("parameters", {})
        for prop in params.get("properties", {}).values():
            if isinstance(prop, dict) and "description" in prop and len(prop["description"]) > max_param_chars:
                prop["description"] = prop["description"][:max_param_chars].rsplit(" ", 1)[0] + "."
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("ZED_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by ZED_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like zed-cli)
    ...
    # Truncate all descriptions to minimum viable length for token efficiency.
    # Tool descs: max 60 chars, param descs: max 20 chars.
    # Long descriptions (~1000+ chars for clarify/delegate_task/execute_code/memory
    # /session_search/cronjob/skill_manage) consume ~100-250 tokens each. The model
    # primarily relies on tool name + param names; descriptions are supplementary.
    _truncate_tool_descriptions(filtered_tools, max_desc_chars=60, max_param_chars=20)
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from zed_cli.config import load_config as _load
        cfg = _load() or {}
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from zed_cli.plugins import has_hook, invoke_hook
