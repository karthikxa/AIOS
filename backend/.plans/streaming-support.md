# Streaming LLM Response Support for Zed Agent
...
1. **Feature-flagged**: `streaming.enabled: true` in config.yaml. Off by default.
   When off, all existing code paths are unchanged â€” zero risk to current behavior.
...
```
  ...
  â”‚  LLM API    â”‚    â”‚      queue.Queue()          â”‚
  â”‚  (stream)   â”‚â”€â”€â”€â–ºâ”‚  thread-safe bridge between â”‚
  â”‚             â”‚    â”‚  agent thread & consumer    â”‚
  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜    â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                                   â”‚
                    â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
                    â”‚              â”‚              â”‚
              â”Œâ”€â”€â”€â”€â”€â–¼â”€â”€â”€â”€â”€â” â”Œâ”€â”€â”€â”€â”€â–¼â”€â”€â”€â”€â”€â” â”Œâ”€â”€â”€â”€â”€â–¼â”€â”€â”€â”€â”€â”
              â”‚    CLI     â”‚ â”‚  Gateway  â”‚ â”‚ API Serverâ”‚
              â”‚ print to   â”‚ â”‚ edit msg  â”‚ â”‚ SSE event â”‚
              â”‚ terminal   â”‚ â”‚ on Tg/Dc  â”‚ â”‚ to client â”‚
...
```
ZED_STREAMING_ENABLED=true    # Master switch via env
```
...
No other init changes. The callback is optional â€” when None, everything
...
def _run_streaming_chat_completion(self, api_kwargs: dict):
    ...
    try:
        ...
        for chunk in stream:
            ...
            # Text content â€” emit via callback
            if delta.content:
            ...
            # Tool call deltas â€” accumulate silently
            if delta.tool_calls:
...
try:
    ...
    pass
# Env var override
if os.getenv("ZED_STREAMING_ENABLED", "").lower() in ("true", "1", "yes"):
...
async def stream_preview():
    ...
    try:
        ...
        # Final edit â€” remove cursor, show complete text
...
This is the most delicate integration point â€” we need to communicate from
...
Recommended: **Option A** â€” cleanest separation. The result dict already
...
|----------|-------------|-------------|-------------------|
| Telegram | âœ… edit_message_text | ~20 edits/min | Edit every 1.5s |
| Discord | âœ… message.edit | 5 edits/5s per message | Edit every 1.2s |
| Slack | âœ… chat.update | Tier 3 (~50/min) | Edit every 1.5s |
| WhatsApp | âŒ no edit support | N/A | Skip streaming, use normal path |
| HomeAssistant | âŒ no edit | N/A | Skip streaming |
| API Server | âœ… SSE native | No limit | Real SSE events |
...
next API call may produce the final text response â€” streaming picks up again.
...
after. This isn't a problem in practice â€” compression happens between
API calls, not during streaming.

### 5. Interrupt during streaming

User sends a new message while streaming â†’ interrupt. The stream is killed
...
| `zed_cli/config.py` | 1 | +streaming config defaults |
| `cli-config.yaml.example` | 1 | +streaming section |
| `tests/test_streaming.py` | 1-4 | NEW â€” ~380 lines of tests |
