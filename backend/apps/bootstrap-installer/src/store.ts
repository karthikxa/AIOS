...
/*
 * Bootstrap state store â€” single source of truth for installer screens.
 *
...
// ---------------------------------------------------------------------------
// Types â€” mirror src-tauri/src/events.rs
// ---------------------------------------------------------------------------
...
/// desktop app handing off via `Zed-Setup.exe --update`.
...
export const $zedHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, zedHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_zed_home'),
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Zed" success screen. Update: this is a
        // hand-off â€” the installer relaunches the desktop and exits within a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Zed-Setup.exe --update);
...
export async function launchZedDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_zed_desktop', { installRoot })
}
