...
<tr><td><b>Voice</b></td><td>Talk to Zed and hear it back.</td></tr>
...
### Install with Zed (recommended)

Already have the Zed CLI? Just run:

```bash
zed desktop
```

It builds and launches the GUI against your existing install â€” same config, keys, sessions, and skills. On first launch Zed walks you through picking a provider and model; nothing else to configure.
...
The packaged app ships only the Electron shell. On first launch it installs the Zed Agent runtime into `ZED_HOME` (`~/.zed`, or `%LOCALAPPDATA%\zed` on Windows) â€” the **same layout a CLI install uses**, so the two are interchangeable. The renderer (React, in `src/`) talks to a `zed dashboard` backend over the standard gateway APIs and reuses the embedded TUI rather than reimplementing chat. The install, backend-resolution, and self-update logic all live in `electron/main.cjs`.
...
Boot logs land in `ZED_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) â€” check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\zed\zed-agent\.zed-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\zed\zed-agent\venv"
```

> The default Zed home on Windows is `%LOCALAPPDATA%\zed`. Set the `ZED_HOME` env var if you've relocated it.
