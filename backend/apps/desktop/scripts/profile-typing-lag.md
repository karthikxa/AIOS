...
```bash
# Terminal A â€” start dev server without HMR
cd apps/desktop
node scripts/dev-no-hmr.mjs

# Terminal B â€” start Electron with CDP exposed
cd apps/desktop
XCURSOR_SIZE=24 ZED_DESKTOP_DEV_SERVER=http://127.0.0.1:5174 \
...
All zero-dep â€” Node 24 built-in `WebSocket` + `fetch`.

### Typing latency â€” `measure-latency.mjs`

Per-keystroke `keypress â†’ next paint` latency, p50/p90/p99/max.
...
Roughly even on a quick session â€” patches don't fix typing latency
...
### Leak counters â€” `leak-typing.mjs`
...
**Real-session numbers (Phaser thread, 8 rounds Ã— 200 chars):**
...
| **forced layouts/char** | **7.02** | **2.35** (3Ã— fewer) |

The forced-layout count is the load-bearing number â€” typing into a real
...
### CPU profile + heap snapshot â€” `profile-typing.mjs`
...
node apps/desktop/scripts/profile-typing.mjs \
  --chars=400 --cps=30 --out=/tmp/zed-typing
# â†’ /tmp/zed-typing.cpuprofile  (open in Chrome DevTools Performance)
# â†’ /tmp/zed-typing.before.heapsnapshot
# â†’ /tmp/zed-typing.after.heapsnapshot
```

Loading the cpuprofile: Chrome DevTools â†’ Performance tab â†’ drag the file
in, or VS Code â†’ open the `.cpuprofile` directly.

For heap diff: Chrome DevTools â†’ Memory â†’ Load snapshot â†’ load "before",
then Comparison view â†’ load "after". Sort by `# Delta`. Stay alert for
detached DOM, FiberNodes (unmounted), and listener growth.

## Helpers

- `probe-renderer.mjs` â€” dump page state (URL, composer mounted?, body text)
- `click-session.mjs <title>` â€” click a sidebar session by partial title match
- `reload-renderer.mjs` â€” force Page.reload via CDP (no HMR available)
- `dump-state.mjs` â€” richer state dump (thread message count, sticky session, etc.)
- `probe-console.mjs` â€” dump recent console errors / exceptions
...
3. **Removed dead `$composerDraft` â†’ `aui.composer().setText` round-trip.**
   Nothing outside the composer subscribed to `$composerDraft` (verified
   via grep). The two useEffects that pushed draft â†’ store and store â†’
   ...
   stale-pruning step, not a correctness one â€” `terminalContextBlocksFromDraft`
...
The biggest win is the listener leak in (3) â€” without it, each round of
...
`enter â†’ composer-cleared â†’ user-message-rendered â†’ first-paint`. The
...
User complaint: "the streaming must bring fps to like 5? lol" â€” felt
hitches during assistant streaming on long threads.

### Tooling added

- **`src/app/chat/perf-probe.tsx`** â€” dev-only side-effect import (guarded by
  `import.meta.env.MODE !== 'production'` in `main.tsx`). Attaches two
  helpers to `window`:
  - `__PERF_PROBE__` â€” React `<Profiler>` recorder. Currently inert because
    Vite is serving the production React build (see "Vite dev-build issue"
    below); kept for when that's fixed.
  - `__PERF_DRIVE__` â€” synthetic stream driver. Pushes tokens through the
    ...
    pipeline all see the same workload they'd see from a real LLM stream â€”
    but with no LLM call (and no credit cost).
- **`scripts/measure-synthetic-stream.mjs`** â€” drives `__PERF_DRIVE__`,
...
- **`scripts/profile-synth-stream.mjs`** â€” CPU profile during a synthetic
  stream; writes a `.cpuprofile` (open in Chrome DevTools Performance panel)
  and a top-30 self-time table.
- **`scripts/measure-real-stream.mjs`** â€” same harness as the synthetic but
  fires a real LLM prompt. Use when you have credits and want to confirm
  the synthetic predictions hold.
- **`scripts/profile-real-stream.mjs`** â€” CPU profile over the duration of
  a real LLM stream.
...
| type-while-stream p95 latency (ms) | 17 | â€” |

A single real-LLM stream on Cloud Shadows (gpt-4o-mini, 39s window) saw
12 longtasks totalling 1.26 s â€” same cadence the synthetic predicted
...
|---|---|---|
| 260 | `bn$1` | `chunk-BO2Nâ€¦js:20003` (micromark tokenize) |
| 249 | `m$1` | `chunk-BO2Nâ€¦js:19949` (micromark) |
| 128 | `compile` | `chunk-BO2Nâ€¦js:21884` (mdast â†’ hast compile) |
| 73 | FadeText body | `components/ui/fade-text.tsx` |
| 62 | `parser` | `chunk-BO2Nâ€¦js:22680` |
...
`hast-util-sanitize`, etc. â€” i.e. **Streamdown's markdown pipeline,
re-parsing the entire growing assistant message on every token append**.
Cost scales linearly with message length.

Compare plain-text (no markdown) â€” the `chunk-BO2Nâ€¦` entries drop out
...
`useEffect([children])` that forced a `scrollWidth` layout read â€” even
...
(110 instances Ã— ~100 stream updates). Doesn't move the longtask needle
on its own â€” Streamdown dwarfs it â€” but eliminates a class of forced
...
The longtask CPU was unavoidable inside the block-memo pattern â€” the live
...
deferred value through the normal `useMessagePartText` hook â€” no fork,
...
- When the main thread has urgent work (typing, scroll, layout), the
  Streamdown render gets deprioritized â€” input stays responsive even
...
to the consumer text boundary, so the whole pipeline â€” preprocess,
block split, repair, parse, render â€” runs at low priority during streaming.
...
Longtask count and max LT are unchanged â€” `useDeferredValue` doesn't
...
clean run logged **MUTATIONS=0** â€” React skipped every intermediate
...
[Flowdown](https://github.com/Atomics-hub/flowdown) â€” process each
character exactly once, emit DOM ops directly, no re-parse of the prefix
on every token. Claimed ~2,000Ã— over `marked`. Trades: not a
...
1. **Coalesce/throttle Streamdown updates** â€” render at most every 32 ms
   instead of every set-state. Reduces parses but doesn't reduce
   per-parse cost; trades latency for smoothness.
2. **Memoize per-prefix** â€” diff the new text against the prior parsed
   version; only re-parse the changed suffix.
3. **Render in stable segments** â€” close-form historical paragraphs as
...
4. **Move parsing to a Web Worker** â€” main thread no longer blocks on
...
`react/cjs/react.production.js`, and `react-dom_client.js` â†’
...
new `optimizeDeps` defaults. Worth a separate fix pass â€” when it's
