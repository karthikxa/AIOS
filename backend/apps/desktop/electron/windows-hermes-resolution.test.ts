// Regression guards for Windows `zed` resolution in main.ts.
//
...
//      Git-Bash `zed` shim shadowed the real zed.cmd/zed.exe; the
...
//      --repair by checking ONLY venv\Scripts\zed.exe (the console-script
...
//      installs.
//   3. unwrapWindowsVenvzedCommand() returned the venv python with NO
...
test('findOnPath tries PATHEXT extensions before the bare (empty) name on Windows', () => {
  ...
  assert.doesNotMatch(
    source,
    /\['', \.\.\.\(process\.env\.PATHEXT/,
    'empty-extension-first order regressed: an extensionless shim can shadow zed.cmd/.exe'
  )
})
...
test('Windows bootstrap recovery chooses --update when any real-install signal is present', () => {
  ...
  // The old too-narrow check (only venv\Scripts\zed.exe) must not return.
  assert.doesNotMatch(
    source,
    /updaterArgs = fileExists\(venvzed\) \?/,
    'recovery regressed to gating only on the zed.exe shim, which forces destructive --repair'
  )
})

test('unwrapWindowsVenvzedCommand smoke-tests the venv python before trusting it', () => {
  const source = readMain()
  const fnStart = source.indexOf('function unwrapWindowsVenvzedCommand(')
  assert.notEqual(fnStart, -1, 'unwrapWindowsVenvzedCommand must exist in main.ts')
