...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'zed_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'zed_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'zed_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/zed.app/Contents/MacOS/zed', 'darwin'),
    '/Applications/zed.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/zed.app/Contents/MacOS/zed', 'darwin'),
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\zed\\zed.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\zed'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\zed-desktop\\zed.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\zed-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\zed.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_zedXXXX/zed', 'linux', { APPIMAGE: '/home/x/Apps/zed.AppImage' }),
    '/home/x/Apps/zed.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/zed/linux-unpacked/zed', 'linux', {}), '/opt/zed/linux-unpacked')
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/zed', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/zed.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/zed.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.zed/zed-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.zed/zed-agent',
    uninstallArgs: ['-m', 'zed_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/zed/linux-unpacked',
    zedHome: '/home/x/.zed'
  })
  ...
  assert.match(script, /'-m' 'zed_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/zed\/linux-unpacked'/)
  assert.match(script, /export zed_HOME='\/home\/x\/\.zed'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.zed/zed-agent',
    agentRoot: '/home/x/.zed/zed-agent',
    uninstallArgs: ['-m', 'zed_cli.uninstall', '--mode', 'full'],
    appPath: null,
    zedHome: '/home/x/.zed'
  })

  // System python + source on PYTHONPATH so import zed_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.zed\/zed-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'zed_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'zed_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    zedHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'zed_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\zed',
    uninstallArgs: ['-m', 'zed_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\zed',
    zedHome: 'C:\\Users\\x\\AppData\\Local\\zed'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import zed_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\zed;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "zed_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\zed" >nul 2>&1/)
