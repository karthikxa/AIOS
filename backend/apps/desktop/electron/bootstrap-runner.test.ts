...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'zed-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/zed-runner-test',
    sourceRepoRoot: null,
    zedHome: '/tmp/zed-runner-test',
    logRoot: '/tmp/zed-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'zed-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'zed-agent')
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/zed-agent', '--zed-home', '/tmp/zed', '--branch', 'main']
  )
})
