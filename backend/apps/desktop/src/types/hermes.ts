...
export interface EnvVarInfo {
  ...
  // this provider identity — the SAME one `zed model` uses — instead of
...
// folders. Mirrors zed_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with zed, 'hub' = installed. */
 ...
 *  not zed); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/zed/update/check` — the backend's own update state.
