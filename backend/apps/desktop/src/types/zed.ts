...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Zed doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in zed_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity â€” the SAME one `zed model` uses â€” instead of
...
export type ZedConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  yet â€” render these with a setup affordance instead of hiding them. */
  authenticated?: boolean
  /** Auth flow for an unconfigured provider: "api_key" can be activated inline
   *  by pasting `key_env`; anything else (oauth_*, external, aws_sdk, â€¦) needs
   *  the `zed model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
export interface SessionInfo {
   ...
   *  continuation tip. Stable across compressions â€” used as the durable id for
...
export interface ToolProvider {
   ...
   *  CLI `zed tools` active-provider detection). */
  is_active: boolean
}
...
export interface SessionSearchResult {
  ...
  model: string | null
  role: string | null
  /** Live compression tip of the matched conversation â€” resume by this id. */
  session_id: string
...
/** Shape of `GET /api/zed/update/check` â€” the backend's own update state.
...
export interface ModelAssignmentRequest {
   ...
   *  on the main slot â€” wires a self-hosted endpoint into runtime resolution. */
  base_url?: string
  model: string
  provider: string
