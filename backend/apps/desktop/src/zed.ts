...
// Profile that profile-scoped REST settings (config/env/skills/tools/model/â€¦)
...
// change is needed. Null â†’ primary, so single-profile users are unaffected.
...
// primary backend straight off each profile's state.db â€” no per-profile backend
...
// backend (remote pool or local primary) via request.profile â€” matching the
...
// 404s when the id isn't on that profile â€” so a cheap by-id lookup replaces the
...
// curation `zed model` does â€” for Nous it honors the free/paid tier so a
