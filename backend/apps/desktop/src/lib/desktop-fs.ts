import type {
  ZedConnection,
  ZedReadDirResult,
  ZedReadFileTextResult,
  ZedSelectPathsOptions,
  ZedWorktreeInfo
} from '@/global'
...
function connectionCacheKey(connection: ZedConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
function bridge() {
  ...
  if (!desktop) {
    throw new Error('Zed Desktop bridge is unavailable')
  }
  return desktop
}

export async function readDesktopDir(path: string): Promise<ZedReadDirResult> {
  const desktop = bridge()
  if (!isDesktopFsRemoteMode()) {
    return desktop.readDir(path)
  }
  return desktop.api<ZedReadDirResult>({ path: fsPath('list', path) })
}

export async function readDesktopFileText(path: string): Promise<ZedReadFileTextResult> {
  const desktop = bridge()
  if (!isDesktopFsRemoteMode()) {
    return desktop.readFileText(path)
  }
  return desktop.api<ZedReadFileTextResult>({ path: fsPath('read-text', path) })
}
...
// we can't resolve them here â€” callers fall back to the path-name heuristic.
export async function desktopWorktrees(cwds: string[]): Promise<Record<string, ZedWorktreeInfo | null>> {
  if (isDesktopFsRemoteMode()) {
    return {}
  }
...
export async function selectDesktopPaths(options?: ZedSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
  if (!isDesktopFsRemoteMode()) {
