...
describe('model-status-label', () => {
  ...
  it('always surfaces the effort (default medium) so the level is visible', () => {
    expect(formatModelStatusLabel('openai/gpt-5.5', { reasoningEffort: 'medium' })).toBe('GPT-5.5 Â· Med')
    expect(formatModelStatusLabel('openai/gpt-5.5')).toBe('GPT-5.5 Â· Med')
  })
  ...
  describe('currentPickerSelection', () => {
    ...
    const options = { model: 'zed-4', provider: 'nous' }
