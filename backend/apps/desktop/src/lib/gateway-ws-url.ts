 ...
 *   back to `conn.wsUrl` â€” that ticket is dead and the connect is guaranteed to
 ...
 *   ("session has expiredâ€¦ Sign in again").
 *
...
export interface ResolveGatewayWsUrlDeps {
  /** `window.zedDesktop.getGatewayWsUrl`, if the preload exposes it. The
   *  optional profile selects which backend to mint for â€” critical when swapping
...
export async function resolveGatewayWsUrl(
  ...
  conn: Pick<ZedConnection, 'authMode' | 'profile' | 'wsUrl'>
): Promise<string> {
  ...
  if (conn.authMode === 'oauth') {
    if (!mint) {
      ...
      throw new GatewayReauthRequiredError(
        'Your remote gateway session needs to be refreshed. Open Settings â†’ Gateway and click "Sign in" again.'
      )
    }
    ...
    try {
      ...
      throw new GatewayReauthRequiredError(
        'Your remote gateway session has expired. Open Settings â†’ Gateway and click "Sign in" again.',
        { cause: error }
      )
    }
  }
