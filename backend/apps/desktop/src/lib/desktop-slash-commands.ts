 ...
 * means adding a branch to a switch ladder â€” you add a row here + a handler
 * keyed by the id.
 */
...
/** Why a known Zed command has no desktop UI surface. */
 ...
 * dispatcher, popover, pills, and pickers all read â€” no parallel block-lists.
 *
 * - `action`     â†’ handled by a local client handler (new chat, branch, â€¦)
 * - `picker`     â†’ opens an overlay (`/model`, `/resume`); a typed arg is
 *                  resolved by that picker instead of falling through
 * - `exec`       â†’ runs on the backend via slash.exec / command.dispatch and
 *                  renders its text output inline
 * - `unavailable`â†’ a known command with genuinely no desktop UI (terminal-only,
 *                  messaging-only, â€¦); shows a reason instead of executing
 */
...
export interface DesktopCommandSpec {
   ...
   * that argument step instead of committing â€” mirroring typing `/<cmd> ` by hand.
   */
  args?: boolean
}
...
/**
 * THE source of truth for desktop slash commands. Everything below â€” execution
 * gating, popover suggestions, catalog filtering, pill grouping, and the
 * dispatcher's behavior â€” derives from this one table.
 */
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/yolo', description: 'Toggle YOLO â€” auto-approve dangerous commands', surface: action('yolo') },
  { name: '/handoff', description: 'Hand off this session to a messaging platform', surface: action('handoff'), args: true },
  { name: '/profile', description: 'Switch the active Zed profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Zed Agent version', surface: exec() },
...
// Known commands with no desktop surface (and no alias) â€” a flat name list
...
function isKnownZedSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Zed' built-in slash commands â€” i.e. skill commands (`/gif-search`,
 * `/codex`, â€¦) and user-defined quick commands. These are user-activated, so
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownZedSlashCommand(normalized)
}
