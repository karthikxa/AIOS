...
import type { SessionMessage, UsageStats } from '@/types/zed'
...
export type GatewayEventPayload = {
  ...
  question?: string
  choices?: string[] | null
  // approval.request (dangerous command / execute_code) â€” session-keyed
  command?: string
  description?: string
  ...
  start?: number
  count?: number
  // status.update (kind=process â†’ background process completion/watch-match)
  kind?: string
}
...
// tool call, image, â€¦). The opposite streaming channel (text <-> reasoning) is
// transparent, so a reasoning burst between two content deltas can't shred one
// sentence into text / Thinking / text â€” the fragmentation models that
