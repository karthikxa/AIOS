 ...
 *  global options query returns â€” else the checkmark snaps back to the default
 ...
 *  tag, so distinct ids (e.g. `â€¦-4.8` vs `â€¦-4.8-fast`) don't collapse. */
export function modelDisplayParts(model: string): { name: string; tag: string } {
  ...
  // Drop a trailing date-pin (`â€¦-20251101`) â€” snapshot noise, not a name.
...
/** Status bar trigger label â€” model name plus the live session state (effort/fast). */
export function formatModelStatusLabel(
  ...
  // active model is a `â€¦-fast` variant (fast via a separate model id).
  if (options?.fastMode || /-fast$/i.test(modelBaseId(model))) {
    parts.push('Fast')
  }

  // Always surface the effort (empty = Zed default of medium) so the
