...
const copy = {
  ...
  availableBody: 'A new version of Zed is ready to install.',
  availableTitleBackend: 'Backend update available',
  availableBodyBackend: 'A newer version of the connected Zed backend is ready to install.',
  availableBodyNoChangelog: 'A newer version is ready. Release notes arenâ€™t available for this install type.'
}
...
describe('resolveUpdateCopy', () => {
  it('client target with commits: client title + client body', () => {
    ...
    expect(r.body).toBe('A new version of Zed is ready to install.')
  })
  ...
  it('no changelog (pip/non-git backend): degrades honestly, still names backend target in title', () => {
    ...
    expect(r.title).toBe('Backend update available')
    // Body must NOT pretend there are notes â€” it states they're unavailable.
