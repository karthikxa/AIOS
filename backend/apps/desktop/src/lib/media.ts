...
// URL size cap and supports seeking. `path` may be a plain path or `file://â€¦`.
export function mediaStreamUrl(path: string): string {
  return `zed-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// expose GET /api/media (zed_cli/web_server.py).
export async function gatewayMediaDataUrl(path: string): Promise<string> {
  const file = filePathFromMediaPath(path)

  const result = await window.zedDesktop!.api<{ data_url: string }>({
