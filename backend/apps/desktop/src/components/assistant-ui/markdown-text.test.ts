...
describe('preprocessMarkdown', () => {
  ...
  it('demotes title/url blocks wrapped in malformed inline fences', () => {
    const input = [
      '**ðŸš¢ TOMORROW (Fajardo, crystal clear cays, pickup avail):**',
      '',
      'Icacos Full-Day Catamaran â€” 6hr, $140, small group, pickup```',
      'https://www.getyourguide.com/fajardo-l882/from-fajardo-icacos-island-full-day-catamaran-trip-t19891/',
      '```Sail Getaway Luxury Cat (Cordillera Cays, water slide, unlimited rum) â€” 6hr, $195```',
  ...
  it('autolinks urls glued to prices and removes orphan fence tails', () => {
    const input = [
      '**ðŸ¢ TODAY (from San Juan, no driving):**',
      '',
      'Sea Turtles & Manatees Snorkel + Free Rum â€” 1.5hr,',
      '~$56```https://www.getyourguide.com/san-juan-puerto-rico-l355/san-juan-snorkel-sea-turtles-manatees-free-video-rum-t879147/ Old San Juan Sunset Cruise w/ Drinks + Hotel Pickup â€” 1.5hr, ~$99 (drinks, no snorkel)```',
    ...
    // to the user â€” `\$` renders as a literal `$` in the final output.
    expect(output).toContain(
  ...
  it('does not swallow trailing emphasis asterisks into an autolinked url', () => {
    const input = '**PR opened: https://github.com/NousResearch/zed-agent/pull/12345**'
    ...
    expect(output).toContain('<https://github.com/NousResearch/zed-agent/pull/12345>')
