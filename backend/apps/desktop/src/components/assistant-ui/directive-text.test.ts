...
import { formatRefValue, zedDirectiveFormatter } from './directive-text'
...
describe('zedDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = zedDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = zedDirectiveFormatter.parse('@file:src/main.tsx the entry point')
