...
declare global {
  interface Window {
    zedDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<ZedConnection>
      ...
      // a spectator window (lazy resume â€” no agent build) for live-streaming
      ...
      profile: {
        ...
        // backend under the new ZED_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: ZedApiRequest) => Promise<T>
      notify: (payload: ZedNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<ZedReadFileTextResult>
      selectPaths: (options?: ZedSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<ZedPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<ZedPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: ZedTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<ZedReadDirResult>
      ...
      // checkout (or can't be read â€” e.g. a remote backend's path).
      worktrees?: (cwds: string[]) => Promise<Record<string, ZedWorktreeInfo | null>>
      terminal: {
        dispose: (id: string) => Promise<boolean>
        onData: (id: string, callback: (payload: string) => void) => () => void
        onExit: (id: string, callback: (payload: ZedTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<ZedTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: ZedWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: ZedPreviewFileChanged) => void) => () => void
...
export interface ZedTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface ZedTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `zed update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  zedRoot?: string
}
...
export interface ZedTitleBarTheme {
  background: string
  foreground: string
}

export interface ZedWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface ZedApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface ZedNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface ZedPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface ZedReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface ZedPreviewWatch {
  id: string
  path: string
}

export interface ZedWorktreeInfo {
  // Main repo root â€” the shared grouping key for a checkout and all its linked
  // worktrees.
  repoRoot: string
...
export interface ZedReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface ZedReadDirResult {
  entries: ZedReadDirEntry[]
  error?: string
}

export interface ZedPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface ZedSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
