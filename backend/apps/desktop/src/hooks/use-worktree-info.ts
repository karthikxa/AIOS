...
type WorktreeMap = Record<string, ZedWorktreeInfo | null>

/**
 * Probe the local filesystem for the git-worktree identity of each session cwd
 * and return a resolver the grouping uses to build `parent â†’ worktree`. Results
...
export function useWorktreeInfo(sessions: SessionInfo[], enabled: boolean): WorktreeResolver {
  ...
  useEffect(() => {
    ...
    void desktopWorktrees(missing)
      ...
      .catch(() => {
        // Bridge unavailable / probe failed â€” leave cwds unresolved so the
