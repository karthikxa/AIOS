...
import { getProfiles, setApiRequestProfile } from '@/zed'
...
import type { ProfileInfo } from '@/types/zed'

// Canonical key for a profile: trimmed, empty â†’ "default". Used everywhere we
...
// /api/profiles/active `current`). "default" is the root ~/.zed. This is the
...
// â”€â”€ Rail order â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// User-defined order for the named (non-default) profile squares in the rail.
// Names absent from the list fall back to alphabetical, appended at the tail â€”
// so a freshly created profile lands at the end until the user drags it.
const PROFILE_ORDER_STORAGE_KEY = 'zed.desktop.profileOrder'
...
// â”€â”€ Rail colors â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const PROFILE_COLORS_STORAGE_KEY = 'zed.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.zedDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new ZED_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.zedDesktop.profile.set(name)
}

// â”€â”€ Swap-minimal gateway routing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// Route profile-scoped REST settings (config/env/skills/tools/model/â€¦) to the
...
// immediately (no real change â†’ no invalidation), so single-profile users just
// get "default" (â†’ the primary backend) with no extra fetches.
...
// reconnect â€” so activating a *background* profile left $connection describing
...
// path it can't resolve ("image not found: C:\â€¦"), while the /api/fs/* file
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.zedDesktop?.getConnection
...
// their sockets â€” so their sessions keep streaming concurrently. A null/empty
// target means "no explicit profile" â†’ keep the current gateway (a plain new
...
export async function ensureGatewayProfile(profile: string | null | undefined): Promise<void> {
  ...
  gatewaySwitch = (async () => {
    ...
    // the active gateway at it â€” without closing the profile you came from.
...
// â”€â”€ Sidebar profile scope (the "workspace switcher" model) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'zed.desktop.showAllProfiles'
...
// whole sidebar with it â€” a real context switch, not a separate filter to keep
// in sync.
...
// $activeGatewayProfile â†’ name, so $profileScope follows).
...
// unified sidebar stays put â€” used by the per-profile "+" in the all-profiles
...
// â”€â”€ Hotkey-driven profile switching â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// when the slot is empty so unused âŒ˜N keys stay harmless.
...
// Switch to the default (root ~/.zed) profile â€” bound to âŒ˜1.
...
// (which can't see the direct rendererâ†”backend WS) spares it. No-op for the
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.zedDesktop?.touchBackend?.(target).catch(() => undefined)
}
