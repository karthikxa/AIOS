...
const checkZedUpdateSpy = vi.fn()
const updateZedSpy = vi.fn()
const getActionStatusSpy = vi.fn()

vi.mock('@/zed', () => ({
  checkZedUpdate: (...args: unknown[]) => checkZedUpdateSpy(...args),
  updateZed: (...args: unknown[]) => updateZedSpy(...args),
...
describe('maybeNotifyUpdateAvailable', () => {
  ...
  it('stays quiet for new commits once the toast was closed', () => {
    maybeNotifyUpdateAvailable(status())
    lastToast().onDismiss() // user closes it â†’ cooldown starts
    notifySpy.mockClear()
...
describe('reportBackendContract', () => {
  ...
  it('clears the snooze once the backend catches up, so a regression warns again', () => {
    ...
    reportBackendContract(2) // backend updated â†’ satisfied, snooze cleared
...
describe('checkBackendUpdates', () => {
  ...
  it('maps the backend /update/check onto the backend status, including commits', async () => {
    setRemote(true)
    checkZedUpdateSpy.mockResolvedValue({
      install_method: 'git',
      current_version: '0.16.0',
      behind: 2,
      update_available: true,
      can_apply: true,
      update_command: 'zed update',
      message: null,
    ...
    expect(checkZedUpdateSpy).toHaveBeenCalled()
  ...
  it('is a no-op in local mode (backend check only runs when remote)', async () => {
    ...
    expect(checkZedUpdateSpy).not.toHaveBeenCalled()
  })
})
...
describe('applyBackendUpdate recovery', () => {
  ...
  it('waits for the backend to return after the restart drops the connection, then clears the overlay', async () => {
    updateZedSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkZedUpdateSpy.mockResolvedValue({ install_method: 'git', current_version: '0.16.0', behind: 0, update_available: false, can_apply: true, update_command: 'zed update', message: null })
  ...
  it('surfaces an error when the backend never comes back after the restart', async () => {
    updateZedSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkZedUpdateSpy.mockRejectedValue(new Error('ECONNREFUSED'))
