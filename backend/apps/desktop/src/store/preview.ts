...
export interface PreviewTarget {
  binary?: boolean
  byteSize?: number
  /** Inline image bytes (a `data:` URL) when the renderer already holds them â€”
...
const REGISTRY_STORAGE_KEY = 'zed.desktop.sessionPreviews.v1'
...
// Manual/file-browser opens are "peeking at a file" â†’ source view in the file
// pane. Tool/explicit-link opens are runnable artifacts â†’ live preview pane.
...
function persistSessionPreviewRegistry(registry: SessionPreviewRegistry) {
  ...
  try {
    // Drop the inline image bytes before persisting â€” a screenshot data URL is
...
/** User clicked the close X â€” clear the target and persist dismissal for the current session. */
