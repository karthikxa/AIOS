...
import type { CronJob } from '@/types/zed'

// Cron *jobs* (not run sessions) power the sidebar "Cron jobs" section. Listing
// the job â€” schedule, state, live next-run countdown â€” makes the job the
...
// In-place edit so the cron overlay's mutations (create/edit/delete/pause/â€¦)
// land in the same atom the sidebar renders â€” no stale list until the next poll.
