...
import { checkZedUpdate, getActionStatus, updateZed } from '@/zed'
import { translateNow } from '@/i18n'
import { persistString, storedString } from '@/lib/storage'
import { dismissNotification, notify } from '@/store/notifications'
import { $connection } from '@/store/session'
import type { BackendUpdateCheckResponse } from '@/types/zed'
...
const UPDATE_TOAST_SNOOZE_KEY = 'zed:update-toast-snooze-until'
...
// value (or none â€” a pre-GUI checkout) means GUI<->backend skew.
...
const SKEW_TOAST_SNOOZE_KEY = 'zed:backend-skew-toast-snooze-until'
...
export function reportBackendContract(contract: number | undefined): void {
  if ((contract ?? 0) >= REQUIRED_BACKEND_CONTRACT) {
    ...
    // Backend caught up â€” forget any prior snooze so a future regression warns
  ...
  notify({
    action: {
      label: translateNow('notifications.updateZed'),
      onClick: () => {
        snoozeSkewToast()
 ...
 * Closing the toast â€” dismissing it or opening the updates window from it â€”
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.zedDesktop?.getVersion?.()
...
export async function checkBackendUpdates(): Promise<DesktopUpdateStatus | null> {
  ...
  try {
    const status = mapBackendCheck(await checkZedUpdate(true))
...
export async function checkUpdates(): Promise<DesktopUpdateStatus | null> {
  const bridge = window.zedDesktop?.updates
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  $updateApply.set({ ...IDLE, applying: true, stage: 'prepare', message: 'Starting updateâ€¦' })

  try {
    const result = await bridge.apply(opts)

    // CLI install with no staged updater: not an error â€” the user just runs
    // `zed update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'zed update',
        command: result.command ?? 'zed update'
      })
    }
...
export async function applyBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    const started = await updateZed()

    if (!started.ok) {
      const message = (started as { message?: string }).message || translateNow('updates.applyStatus.notAvailable')
      const command = (started as { update_command?: string }).update_command || 'zed update'
    ...
    for (let attempt = 0; attempt < 30; attempt += 1) {
      ...
      try {
        ...
        // The dashboard restarts mid-update, dropping this connection â€” expected, not a failure.
        $backendUpdateApply.set({
          ...$backendUpdateApply.get(),
          applying: true,
          stage: 'restart',
...
export function startUpdatePoller(): void {
  ...
  // The poller starts at mount, before the gateway connects â€” so the first
  // backend check above sees modeâ‰ remote and no-ops. Re-check once the
