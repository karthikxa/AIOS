...
export function setZoomPercent(percent: number): void {
  window.zedDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.zedDesktop?.zoom) {
  void window.zedDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.zedDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
