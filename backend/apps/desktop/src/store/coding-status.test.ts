...
import type { zedRepoStatus } from '@/global'

import { $repoStatus, refreshRepoStatus } from './coding-status'
import { $currentCwd } from './session'

const sampleStatus: zedRepoStatus = {
...
function stubProbe(impl: (cwd: string) => Promise<zedRepoStatus | null>) {
  ;(window as unknown as { zedDesktop?: unknown }).zedDesktop = { git: { repoStatus: impl } }
}
