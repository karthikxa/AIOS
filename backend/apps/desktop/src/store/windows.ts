...
// running subagent). It resumes lazily â€” the gateway registers history + a
...
export function canOpenSessionWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.zedDesktop?.openSessionWindow === 'function'
}
...
export async function openSessionInNewWindow(sessionId: string, opts?: { watch?: boolean }): Promise<void> {
  ...
  await openWindow(() => window.zedDesktop.openSessionWindow(sessionId, opts), 'Could not open chat in a new window')
}

// Open a fresh compact window on the new-session draft.
export async function openNewSessionInNewWindow(): Promise<void> {
  if (!canOpenSessionWindow() || typeof window.zedDesktop.openNewSessionWindow !== 'function') {
    return
  }

  await openWindow(() => window.zedDesktop.openNewSessionWindow(), 'Could not open new session window')
}
