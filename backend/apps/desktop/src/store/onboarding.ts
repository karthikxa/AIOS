...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/zed'
...
export type OnboardingFlow =
  ...
  // No code to paste and no user_code to show â€” just a waiting state.
  ...
  | {
      ...
      // /api/model/options â€” no need to cache the list here.
      currentModel: string
      label: string
      providerSlug: string
      saving: boolean
...
export interface DesktopOnboardingState {
   ...
   *  re-nags on subsequent launches â€” the user can connect a provider any time
   *  from Settings â†’ Providers (or the model picker's "Add provider"). Distinct
   ...
   *  custom OpenAI-compatible endpoint (e.g. from Settings â†’ Model's "Set up
...
const CONFIGURED_CACHE_KEY = 'zed-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'zed-onboarding-skipped-v1'
const POLL_MS = 2000
...
function writeCachedConfigured(value: boolean) {
  ...
  try {
    ...
    // localStorage unavailable â€” degrade silently.
  }
}
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Zed is ready', message: `${provider} connected.` })
}

// Human-friendly labels for tools auto-routed through the Nous Tool Gateway,
// mirroring zed_cli/nous_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
// Gateway, tell the user which ones â€” same information the CLI prints. Silent
...
function notifyGatewayTools(tools: string[] | undefined) {
  ...
  notify({
    durationMs: 8000,
    kind: 'info',
    message: `${list} now run through your Nous subscription â€” no separate API keys needed.`,
...
// card â€” the user gets the undefined-model auto-selection behaviour
...
async function fetchProviderDefaultModel(
  ...
  // Prefer the backend's recommended default â€” it mirrors the curation
  // `zed model` does (for Nous it honors the user's free/paid tier, so a
  ...
  try {
    ...
    // Endpoint unavailable â€” keep models[0]. Non-fatal: the confirm card still
...
// the caller can fold it into a user-facing error â€” same contract as
...
async function completeWithModelConfirm(
  ...
  // When true, a failing runtime check no longer blocks progression â€” the
  ...
  if (!defaults) {
    // Couldn't get a sensible default â€” proceed without confirm step.
  ...
  try {
    ...
    // Persistence failed â€” still show the confirm card so the user can
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Zed still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Zed still cannot resolve a usable provider.'
}
...
// app â€” e.g. the model picker's "Add provider" button. Reuses the entire
...
// (URL + optional API key), bypassing the OAuth picker. Used by Settings â†’
...
// re-show the picker â€” the original "booted back to the first screen" loop).
...
// (working) configuration. Only valid in the manual path â€” the unconfigured
...
export function completeDesktopOnboarding() {
  ...
  // moot â€” clear it so the flag never lingers in a configured install.
...
// (from Settings â†’ Providers or the model picker's "Add provider") â€” this only
...
export async function refreshOnboarding(ctx: OnboardingContext) {
  ...
  // auto-dismiss on runtime-ready â€” the whole point is to let them add /
...
async function openSignInUrl(url: string) {
  if (window.zedDesktop?.openExternal) {
    try {
      await window.zedDesktop.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Zed still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingApiKey(
  ...
  label: string,
  ctx: OnboardingContext,
  // Optional endpoint key â€” only meaningful for the "Local / custom endpoint"
  ...
  // base_url + model + api_key), not dropped into .env â€” runtime resolution
  ...
  try {
    ...
    // backend slug â€” e.g. OPENROUTER_API_KEY â†’ "openrouter" â€” is the
...
// Ollama, â€¦). Unlike API-key providers, a local endpoint is defined by its URL
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Zed still cannot reach ${url}.` }
    }
...
// â€” the model was already persisted by completeWithModelConfirm (or by
