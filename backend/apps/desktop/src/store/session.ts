...
import type { ZedConnection } from '@/global'
import type { ChatMessage } from '@/lib/chat-messages'
import { persistBoolean, persistString, storedBoolean, storedString } from '@/lib/storage'
import type { SessionInfo, UsageStats } from '@/types/zed'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'zed.desktop.workspace-cwd'

// The composer's model/effort/fast is sticky UI state, NOT the profile default
// (that lives in Settings â†’ Model). Persisting it in localStorage makes a pick
...
const COMPOSER_MODEL_KEY = 'zed.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'zed.desktop.composer.provider'
const COMPOSER_EFFORT_KEY = 'zed.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'zed.desktop.composer.fast'

let configuredDefaultProjectDir = ''

function workspaceCwdKey(connection: ZedConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.zedDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.zedDesktop?.sanitizeWorkspaceCwd
...
export function applyConfiguredDefaultProjectDir(dir: null | string | undefined): void {
  ...
  // Cache only â€” new chats read this via workspaceCwdForNewSession(). Do not
 ...
 *  row the server omitted that we still want visible â€” both still-"working"
 ...
 *     in-memory list â€” and because the Pinned section resolves pins against
...
export function mergeSessionPage(
  ...
  // id (old #4 â†’ new #5), the incoming page carries the new tip but the
...
export const $connection = atom<ZedConnection | null>(null)
...
// slice â€” separate from local recents â€” so each platform renders a
...
// is replaced ~30Ã—/s; components that only care about coarse facts (is the
// thread empty? is the tail a user message?) subscribe to these instead of
// $messages so per-token flushes don't re-render them â€” nanostores' `computed`
...
export const setConnection = (next: Updater<ZedConnection | null>) => updateAtom($connection, next)
...
// Watchdog tracking â€” when does a "working" session count as stuck?
...
// pinned, nor active â€” so mergeSessionPage() evicts it. Nothing re-fetches
...
export function setSessionWorking(sessionId: string | null | undefined, working: boolean) {
  ...
  if (working) {
    ...
    // Only grant grace on a real workingâ†’idle transition (updateSessionState
