...
const remoteConn = (over: Partial<ZedConnection> = {}): ZedConnection =>
  ({ baseUrl: 'https://zed-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as ZedConnection

const localConn = (over: Partial<ZedConnection> = {}): ZedConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as ZedConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<ZedConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { zedDesktop: { getConnection } })
})
...
describe('ensureGatewayProfile â†’ $connection sync (#46651)', () => {
  it('refreshes $connection to the remote descriptor when activating a remote pool profile', async () => {
    ...
    // "local". Activating the remote profile must flip it to "remote" â€” without
    // this, image attach uses path-based image.attach against the remote
    // gateway ("image not found: C:\\â€¦") instead of image.attach_bytes.
