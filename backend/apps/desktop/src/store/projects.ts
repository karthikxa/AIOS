...
import type { zedGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/zed'
...
const PROJECT_SCOPE_KEY = 'zed.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('zed gateway is not connected')
  }
...
// repo, returning where zed should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<zedGitBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.zedDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.zedDesktop?.writeClipboard?.(path)
  }
}
