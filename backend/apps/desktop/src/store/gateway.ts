import type { ConnectionState, GatewayEvent } from '@zed/shared'
import { atom } from 'nanostores'

import { ZedGateway } from '@/zed'
import { resolveGatewayWsUrl } from '@/lib/gateway-ws-url'
import { setGatewayState } from '@/store/session'

// â”€â”€ Multi-profile gateway routing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// socket as the PRIMARY (window) backend â€” owned by use-gateway-boot, with all
// its boot-progress / sleep-wake machinery â€” and add one persistent SECONDARY
...
const isOpen = (gateway: ZedGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<ZedGateway | null>(null)
...
// â”€â”€ Primary (window) backend â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
let primaryGateway: ZedGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: ZedGateway | null, profile = 'default'): void {
  primaryGateway = gateway
  primaryProfile = normKey(profile)
}

// â”€â”€ Secondary (pool) backends â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
interface Secondary {
  profile: string
  gateway: ZedGateway
  offEvent: () => void
...
export function activeGateway(): ZedGateway | null {
...
async function openSecondary(entry: Secondary): Promise<void> {
  const desktop = window.zedDesktop
...
function scheduleReconnect(entry: Secondary): void {
  ...
  // 1s, 2s, 4s â€¦ capped at 15s â€” same backoff shape as the primary.
...
async function reconnectSecondary(entry: Secondary): Promise<void> {
  ...
  try {
    ...
    // Transport failure â†’ fall through to the backoff below.
  } finally {
...
export async function ensureActiveGatewayOpen(): Promise<ZedGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
