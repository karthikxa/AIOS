...
import type { zedReviewFile, zedReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — zed' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'zed.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'zed.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'zed.desktop.reviewTreeMode'
const SELECTED_KEY = 'zed.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<zedReviewFile[]>([])
...
export const $reviewShipInfo = atom<zedReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['zedDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: zedReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.zedDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.zedDesktop?.openExternal?.(url)
    }
