...
// touches this â€” only ChatBar's scope swap reads/writes it. Text mirrors to
// localStorage; attachments are memory-only (blobs, upload state).
export const SESSION_DRAFTS_STORAGE_KEY = 'zed:composer-drafts:v3'
...
function persistDraftTexts() {
  try {
    ...
    // Best-effort only â€” quota/private-mode must never break typing.
  }
}
 ...
 * id is gone â€” e.g. the user removed the chip while an eager upload was still in
