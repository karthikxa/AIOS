 ...
 * app's bounds and stays visible while zed is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'zed.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'zed.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.zedDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.zedDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.zedDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.zedDesktop?.petOverlay?.close()
}
