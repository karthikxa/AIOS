...
describe('model visibility', () => {
  ...
  it('preserves hidden-provider sentinel without re-adding defaults', () => {
    // User explicitly hid all models for "nous" â€” sentinel marks this choice.
    const stored = new Set([emptyProviderSentinelKey('nous')])

    const visible = effectiveVisibleKeys(stored, [
      provider('nous', ['zed-3-llama-3.1-70b', 'zed-3-llama-3.1-8b']),
      provider('ollama', ['qwen3:latest'])
    ])

    expect(visible.has(modelVisibilityKey('nous', 'zed-3-llama-3.1-70b'))).toBe(false)
    expect(visible.has(modelVisibilityKey('nous', 'zed-3-llama-3.1-8b'))).toBe(false)
  ...
  it('restores model when toggling on after hiding all', () => {
    ...
    afterToggle.add(modelVisibilityKey('nous', 'zed-3-llama-3.1-70b'))
    ...
    expect(visible.has(modelVisibilityKey('nous', 'zed-3-llama-3.1-70b'))).toBe(true)
    expect(visible.has(modelVisibilityKey('nous', 'zed-3-llama-3.1-8b'))).toBe(false)
  })
