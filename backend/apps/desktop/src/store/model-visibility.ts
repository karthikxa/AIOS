...
import type { ModelOptionProvider } from '@/types/zed'

const STORAGE_KEY = 'zed.desktop.visible-models'
...
/** A model and its optional `â€¦-fast` sibling, collapsed into one logical row.
...
/** Collapse a provider's model list so a base model and its `â€¦-fast` variant
 *  become a single family (one row, one toggle). Order is preserved by the
 *  base model's position. A `â€¦-fast` model with no base stands on its own. */
export function collapseModelFamilies(models: readonly string[]): ModelFamily[] {
  ...
  for (const model of models) {
    ...
    if (/-fast$/i.test(model) && present.has(model.replace(/-fast$/i, ''))) {
      // Represented by its base entry â€” the base attaches it as `fastId`.
      continue
    }

    if (/-\d{8}$/.test(model) && present.has(model.replace(/-\d{8}$/, ''))) {
      // A date-pinned snapshot superseded by its rolling alias â€” drop the dupe.
      continue
    }
 ...
 *  hasn't customized â€” in which case the curated default applies. */
...
export function effectiveVisibleKeys(
  ...
  // Strip sentinel keys â€” they are bookkeeping, not real visibility entries.
