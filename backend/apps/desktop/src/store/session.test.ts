...
describe('sessionPinId', () => {
  ...
  it('uses the lineage root so a pin survives compression', () => {
    ...
    // the original root â€” pinning on the root keeps the pin stable.
...
describe('mergeSessionPage', () => {
  ...
  it('evicts an old compression tip when the incoming page has the new tip from the same lineage', () => {
    // Repro of #43483: after auto-compression rotates the tip (#4 â†’ #5),
...
describe('workspaceCwdForNewSession', () => {
  afterEach(() => {
    ...
    window.localStorage.removeItem('zed.desktop.workspace-cwd')
    window.localStorage.removeItem('zed.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-a.default')
    window.localStorage.removeItem('zed.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-b.default')
  })

  it('prefers the configured default over the sticky remembered workspace', () => {
    window.localStorage.setItem('zed.desktop.workspace-cwd', '/home/user/sticky')
  ...
  it('keeps remote workspace memory separate from local and other remotes', () => {
    window.localStorage.setItem('zed.desktop.workspace-cwd', '/local/project')
    $currentCwd.set('/live/session/path')
...
describe('getRecentlySettledSessionIds', () => {
  ...
  it('keeps a session for the grace window after its turn settles, then drops it', () => {
    ...
    // A turn starts then ends: the workingâ†’idle transition grants grace.
  ...
  it('clears the grace timer when the session goes busy again', () => {
    ...
    // A new turn for the same session is "working" again â€” drop it from the
