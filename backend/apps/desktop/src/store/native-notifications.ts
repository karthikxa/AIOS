...
// Blocking prompts â€” surface even while focused if they're for another session.
...
const STORAGE_KEY = 'zed:native-notifications'
...
// "Backgrounded" = the user isn't on Zed. `document.hidden` only flips when
...
function shouldFire(kind: NativeNotificationKind, sessionId?: null | string): boolean {
  ...
  // Completion kinds: only the active session, only while away â€” so a busy
...
// Run/Reject bar. Keyed by session id â€” a background approval has no local guard.
...
// Settings "send test" â€” bypasses gating. Returns whether the OS accepted it so
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.zedDesktop
