 ...
 * (`electron/vscode-marketplace.cjs`), reached via `window.zedDesktop.themes`.
 ...
 * is Comingâ€¦). Rather than install them as separate flat entries â€” which made
 * the light/dark toggle a no-op and let "install in dark mode" land on the light
 * variant â€” we map the first light variant onto `colors` and the first dark
...
export function buildThemeFromMarketplace(result: DesktopMarketplaceThemeResult): DesktopTheme {
  ...
  // (light â†’ terminal, dark â†’ darkTerminal); each falls back to the other so a
  ...
  return {
    ...
    description: `VS Code Â· ${result.extensionId}`,
...
export async function installVscodeThemeFromMarketplace(id: string): Promise<DesktopTheme> {
  ...
  const api = window.zedDesktop?.themes
