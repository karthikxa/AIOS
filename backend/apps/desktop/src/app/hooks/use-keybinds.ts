...
// for every rebindable hotkey: it runs the matched action, or â€” while capture
// mode is active (edit overlay / panel rebind) â€” records the pressed combo.
export function useKeybinds(deps: KeybindRuntimeDeps): void {
  ...
  handlersRef.current = {
    ...
    'session.new': () => {
      ...
      window.dispatchEvent(new CustomEvent('zed:new-session-shortcut'))
    },
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      // so e.g. âŒ˜K rebinds instead of opening the palette.
      ...
      // combo dispatch â€” âŒƒTab keeps stepping through the existing handler.
