...
import type { ZedReadDirResult } from '@/global'
...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { zedDesktop: { readDir: typeof readDir } }).zedDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): ZedReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { zedDesktop: unknown }).zedDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: ZedReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<ZedReadDirResult>(resolve => {
    ...
    // Mount load + a single folder fetch â€” duplicates were dropped.
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { zedDesktop: unknown }).zedDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.zedDesktop is missing', async () => {
    delete (window as unknown as { zedDesktop?: unknown }).zedDesktop
