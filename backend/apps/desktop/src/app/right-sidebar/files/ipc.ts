...
export type ProjectTreeEntry = ZedReadDirEntry
...
/** Repo-root â†’ repo-root/a â†’ repo-root/a/b â†’ â€¦ for every dir between root and `dir`. */
...
/** Read .gitignore at `dir` if it actually exists â€” never probe missing files. */
...
function ignoredBy(rules: GitignoreRule[], entry: ZedReadDirEntry) {
...
async function filterIgnored(entries: ZedReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<ZedReadDirResult> {
  if (!window.zedDesktop) {
