...
// âŒ˜/Ctrl+L is a global shortcut, so a text selection in the file preview pane
...
const ZED_PATHS_MIME = 'application/x-zed-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(ZED_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(ZED_PATHS_MIME)
    ...
    // Malformed in-app drag payload â€” fall through to OS files.
  }

  const getPath = window.zedDesktop?.getPathForFile
...
export function useTerminalSession({ cwd, onAddSelectionToChat }: UseTerminalSessionOptions) {
  ...
  // clicked switch) â€” a skin can keep a light surface in "dark" mode, and we
  ...
  // onSelectionChange, so trust xterm directly â€” fall back to the native
  // selection â€” rather than the cached ref / React state.
  ...
  const addSelectionToChat = useCallback(() => {
    ...
    // Terminal selection â†’ shell-anchored label; anything else came from the
    // preview pane sharing this global shortcut â†’ label it with the file.
  ...
  // Always listen â€” gating on the React selection state misses selections the
  // TUI redraw races. Only swallow âŒ˜/Ctrl+L when there's text to send, else it
  ...
  useEffect(() => {
    ...
    const terminalApi = window.zedDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      letterSpacing: 0,
      lineHeight: 1.12,
      // Full-screen TUIs (zed --tui, vim) grab the mouse, so a plain drag
      // can't select â€” âŒ¥-drag (macOS) / Shift-drag (else) forces a native
      // selection over mouse-mode apps, which âŒ˜/Ctrl+L then sends to chat.
      ...
      // saturated ANSI palette â€” vivid green/cyan on white reads as candy.
    ...
    // Coalesce ResizeObserver bursts through rAF â€” running fit.fit()
    ...
    const dataDisposable = term.onData(data => {
      ...
      if (id) {
        // Once the user submits a line, real output may follow â€” stop the
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[zed-terminal] WebGL unavailable; falling back to DOM', err)
      }
    ...
    // atlas init), so warm them up front â€” otherwise the WebGL atlas bakes a
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      void window.zedDesktop?.terminal?.write(id, `${command}\r`)
