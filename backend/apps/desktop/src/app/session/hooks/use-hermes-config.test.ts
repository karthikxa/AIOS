...
import { getzedConfig } from '@/zed'
import { persistString } from '@/lib/storage'
import { $currentCwd, setCurrentCwd } from '@/store/session'

import { usezedConfig } from './use-zed-config'

vi.mock('@/zed', () => ({
  getzedConfig: vi.fn(),
  getzedConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'zed.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getzedConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getzedConfig>>)

describe('usezedConfig refreshzedConfig', () => {
  beforeEach(() => {
  ...
  it('applies terminal.cwd from config even when localStorage has a stale value', async () => {
    ...
    await act(async () => {
      await result.current.refreshzedConfig()
    })
