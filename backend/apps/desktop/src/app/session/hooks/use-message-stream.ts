...
export function useMessageStream({
  ...
  const upsertToolCall = useCallback(
    (
      ...
      // The composer status stack owns todo display now (no inline panel) â€”
  ...
  const failAssistantMessage = useCallback(
    (sessionId: string, errorMessage: string) => {
      updateSessionState(sessionId, state => {
        ...
        const groupId = state.pendingBranchGroup ?? undefined
        const prev = state.messages
        const error = errorMessage.trim() || 'Zed reported an error'
  ...
  const handleGatewayEvent = useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        // Turn ended â€” drop any blocking prompt still open for THIS session
        ...
        if (sessionId) {
          ...
          // one is the clarify resolving â€” drop the "needs input" flag here so
          // the sidebar indicator clears as soon as it's answered, not only at
          // message.complete.
          updateSessionState(sessionId, state => (state.needsInput ? { ...state, needsInput: false } : state))

          // terminal/process tool calls are the only things that spawn or reap
          // background processes â€” sync the composer status stack right after.
        ...
        //
        // Store the request for whichever session raised it â€” even a background
        ...
        if (requestId && question) {
          ...
          // "needs input" indicator on its row â€” works for the active session
        ...
        // raise it and wait â€” the sidebar flags "needs input" and the inline bar
        ...
        if (sessionId && payload?.kind === 'compacting') {
          ...
          // completions / watch matches here â€” re-sync the status stack.
          void refreshBackgroundProcesses(sessionId)
        }
      } else if (event.type === 'review.summary') {
        // Self-improvement background review saved something to memory/skills
        // and emitted a persistent summary (Python formats it as
        // "ðŸ’¾ Self-improvement review: â€¦"). The CLI prints this via
        ...
        // in the transcript so the user is always informed â€” it must not be a
        ...
        const errorMessage = payload?.message || 'Zed reported an error'
        const looksLikeProviderSetup = isProviderSetupErrorMessage(errorMessage)

        // A turn that errors out has also ended â€” drop any open blocking prompt
