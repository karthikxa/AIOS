...
import { getProfiles, transcribeAudio } from '@/zed'
...
// user â€” the turn settles within the deadline and the call lands.
...
async function readImageForRemoteAttach(
  ...
  const dataUrl = await window.zedDesktop?.readFileDataUrl(filePath)
...
async function readFileDataUrlForAttach(filePath: string): Promise<string | null> {
  const reader = window.zedDesktop?.readFileDataUrl
 ...
 * and the message-edit composer drop â€” keep them in lockstep.
 */
...
export function usePromptActions({
  ...
  const appendSessionTextMessage = useCallback(
    (sessionId: string, role: ChatMessage['role'], text: string) => {
      ...
      // in the chat panel, so without this the `[1;31mâ€¦[0m` payload leaks as
      // literal text.
  ...
  const syncAttachmentsForSubmit = useCallback(
    async (
      ...
      for (const original of attachments) {
        ...
        // before deciding anything â€” otherwise submit and the eager task both
        ...
        // attachedSessionId) â€” don't re-upload it.
  ...
  // an eager upload would race that write â€” clobbering the thumbnail and
  ...
  const eagerlyUploadAttachment = useCallback(
    async (sessionId: string, attachment: ComposerAttachment) => {
      ...
      try {
        ...
        // don't resurrect it â€” just drop the staged result on the floor.
  ...
  const submitPromptText = useCallback(
    async (rawText: string, options?: SubmitTextOptions) => {
      ...
      // a (remote-mode 403-prone) /api/media fetch â€” see optimisticAttachmentRef.
      ...
      // Queue drains fire on the busyâ†’false settle edge, where busyRef (synced
      // from $busy by a separate effect) may still read true â€” honoring it would
      ...
      // Idempotent optimistic insert â€” re-running with the resolved sessionId
      ...
      const seedOptimistic = (sid: string) =>
        updateSessionState(
          ...
          state => ({
            ...
            // Fresh submit = new turn â€” clear any leftover interrupt flag, else
      ...
      try {
        ...
        // (Images keep their inline base64 preview â€” see optimisticAttachmentRef.)
        ...
        // "session busy" (4009). Don't surface an error bubble/toast â€” the entry
  ...
  // separate `zed gateway` process performs the actual transfer, so we
    ...
    async (rawCommand: string, options?: { sessionId?: string; recordInput?: boolean }) => {
      ...
      async function runExec(ctx: SlashActionCtx): Promise<void> {
        ...
        const handleDispatch = async (dispatch: NonNullable<ReturnType<typeof parseCommandDispatch>>): Promise<void> => {
          ...
          // send / prefill carry an optional `notice` (e.g. "âŠ™ Goal set â€¦")
          // that the backend wants shown as a system line before the message
          // is acted on. Mirrors the TUI's createSlashHandler â€” without it a
          ...
          if (dispatch.type === 'skill') {
            renderSlashOutput(`âš¡ loading skill: ${dispatch.name}`)
          }

          if (busyRef.current) {
            renderSlashOutput('session busy â€” /interrupt the current turn before sending this command')
      ...
      // registry row in desktop-slash-commands.ts plus an entry here â€” never a
      ...
      const actionHandlers: Record<DesktopActionId, (ctx: SlashActionCtx) => Promise<void>> = {
        ...
        // /yolo maps to the status-bar YOLO control â€” a per-session approval
        ...
        // /profile selects which profile new chats open in â€” no app relaunch.
        ...
        skin: async ({ arg, command, recordInput, sessionHint }) => {
          ...
          // No session to print into yet â€” surface it as a toast instead of
        ...
        // /title <name> renames via the gateway's session.title RPC â€” the same
        ...
        // BROWSER_CDP_URL (and may launch Chrome) in the gateway process â€” only
        ...
        browser: async ctx => {
          ...
          if ($connection.get()?.mode === 'remote') {
            renderSlashOutput(
              '/browser manages a Chromium-family browser on the gateway host â€” only available when connected to a local gateway.'
            )
          ...
          if (!['connect', 'disconnect', 'status'].includes(cmdAction)) {
            renderSlashOutput(
              'usage: /browser [connect|disconnect|status] [url] Â· persistent: set browser.cdp_url in config.yaml'
            )
          ...
          try {
            ...
            // progress lines into `messages` â€” flush them inline.
      ...
      const openPicker = async (pickerId: DesktopPickerId, ctx: SlashActionCtx): Promise<void> => {
        if (pickerId === 'model') {
          ...
          // Power users can still type `/model <name>` â€” run it on the backend.
          await runExec(ctx)

          return
        }

        // session picker â€” /resume, /sessions, /switch
      ...
      // its kind. No per-command ladder â€” behavior lives in the registry.
  ...
  const steerPrompt = useCallback(
    async (rawText: string): Promise<boolean> => {
      ...
      try {
        ...
        // Swallow â€” caller queues the text so nothing is lost.
      }
  ...
  // mechanism â€” `prompt.submit` with `truncate_before_user_ordinal` drops that
  ...
  // can fire while a turn is streaming, interrupt the live turn first â€” the
    ...
    async (sessionId: string, text: string, truncateOrdinal: number | undefined, wasRunning: boolean) => {
      if (wasRunning) {
        try {
          ...
          // Best-effort â€” the busy-retry below still gates the submit.
        }
      }
  ...
  const editMessage = useCallback(
    async (edited: AppendMessage) => {
      ...
      // new text. It can fire mid-turn, so capture the live state â€” the submit
      ...
      // Editing rewinds the conversation to this prompt â€” same as restore â€” so
      ...
      try {
        ...
        if (!isFailedTurn && isStaleTargetError(err)) {
          try {
            // Already interrupted on the first attempt â€” submit as a plain resend.
