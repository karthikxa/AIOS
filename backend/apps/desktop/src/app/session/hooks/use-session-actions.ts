...
import { deleteSession, getSession, getSessionMessages, setSessionArchived } from '@/zed'
...
import type { SessionCreateResponse, SessionInfo, SessionResumeResponse, SessionRuntimeInfo, UsageStats } from '@/types/zed'
...
async function resolveStoredSession(storedSessionId: string): Promise<SessionInfo | undefined> {
  ...
  // Direct by-id on the live backend â€” one row lookup, no list scan. Covers
  ...
  try {
    ...
    // Not on the active profile â€” fall through to the cross-profile probe.
  }
...
export function useSessionActions({
  ...
  const startFreshSessionDraft = useCallback(
    (replaceRoute = false) => {
      ...
      // localStorage) â€” a new chat FOLLOWS your last pick instead of snapping
      ...
      setCurrentBranch('')
      // Never clear the composer here â€” ChatBar's per-thread draft swap owns it.
  ...
  const createBackendSessionForSend = useCallback(
    async (preview: string | null = null): Promise<string | null> => {
      ...
      try {
        ...
        // chat on the launch (default) profile â€” the "rubberbands back to
        ...
        // shows â€” applied as per-session overrides, never written to the profile
        // default (that lives in Settings â†’ Model).
        ...
        // session existed â€” apply it to the freshly created session.
        if (yoloArmed) {
  ...
  const resumeSession = useCallback(
    async (storedSessionId: string, replaceRoute = false) => {
      ...
      // lingering until resume lands. A warm-cached target keeps its transcript â€”
      ...
      try {
        ...
        // â€” run them concurrently so a big session's wall time is
        ...
        // Watch windows skip the prefetch â€” lazy resume attaches the live mirror.
        ...
        // skip converting/reconciling the resume payload entirely â€” on a
        ...
        // wrapped in its own try â€” if it ALSO throws (wedged/unreachable backend,
        ...
        // forever (messagesEmpty && !activeSessionId) with no recovery path â€”
        ...
        if (isCurrentResume() && $messages.get().length === 0) {
          ...
          // window is readable â€” arming here would needlessly auto-retry and,
  ...
  const archiveSession = useCallback(
    async (storedSessionId: string) => {
      ...
      try {
        ...
        // that race after the mutation succeeds so right-click â†’ Archive does
