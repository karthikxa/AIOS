...
export const ZED_PATHS_MIME = 'application/x-zed-paths'

/**
 * Eagerly resolve files from a drop event into [File?, path, isDirectory?]
 * triples. Internal Zed sources (e.g. the project tree) ride on a custom
 * MIME and produce path-only entries; OS drops produce File-bearing entries.
 *
 * Must be called synchronously from inside the drop handler â€” `DataTransfer`
...
export function extractDroppedFiles(transfer: DataTransfer): DroppedFile[] {
  ...
  // In-app drags first â€” they carry richer metadata (isDirectory) than the
  // File-based fallback can provide, and produce no overlapping native files.
  try {
    const internalRaw = transfer.getData(ZED_PATHS_MIME)
    ...
    // Malformed payload â€” fall through to native files.
  }
 ...
 * An OS drop is an absolute path on *this* machine â€” the gateway can't read it
...
export function useComposerActions({ activeSessionId, currentCwd, requestGateway }: ComposerActionsOptions) {
  ...
  const pickContextPaths = useCallback(
    async (kind: 'file' | 'folder') => {
      const paths = await window.zedDesktop?.selectPaths({
  ...
  const attachImagePath = useCallback(async (filePath: string) => {
    ...
    try {
      const previewUrl = await window.zedDesktop?.readFileDataUrl(filePath)
  ...
  const attachImageBlob = useCallback(
    async (blob: Blob) => {
      ...
      try {
        ...
        const savedPath = await window.zedDesktop?.saveImageBuffer(data, blobExtension(blob))
  ...
  const pasteClipboardImage = useCallback(async () => {
    try {
      const path = await window.zedDesktop?.saveClipboardImage()
  ...
  const attachDroppedItems = useCallback(
    async (candidates: DroppedFile[]) => {
      ...
      for (const candidate of candidates) {
        ...
        const fallbackPath =
          !knownPath && window.zedDesktop?.getPathForFile ? window.zedDesktop.getPathForFile(file) : ''
