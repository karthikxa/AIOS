...
import type { ZedGateway } from '@/zed'
...
/** How many recent sessions to surface inline before the "Browse allâ€¦" entry. */
...
export function useSlashCompletions(options: {
  gateway: ZedGateway | null
  /** Desktop theme list â€” `/skin` is owned client-side, so its arg completions
  ...
  const fetcher = useCallback(
    async (query: string): Promise<CompletionPayload> => {
      ...
      // /resume (and its aliases) completes recent sessions inline â€” the same
      // client-side list the picker overlay shows â€” instead of the backend
      ...
      try {
        ...
        // Arg-completion items (replace_from > 1) carry just the arg stub â€”
        ...
        const decorated = (result.items ?? [])
          ...
          .map(item => ({
            ...
            // blurb). Only command rows get the registry description â€” looking
  ...
  const toItem = useCallback((entry: CompletionEntry, index: number): Unstable_TriggerItem => {
    ...
    const metadata: SlashItemMetadata = {
      ...
      // Provide rawText so zedDirectiveFormatter.serialize uses the
