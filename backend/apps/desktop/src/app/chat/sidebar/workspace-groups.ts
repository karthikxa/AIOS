...
import type { SessionInfo } from '@/zed'
 ...
 * path and prepending parent segments until each colliding label is unique â€”
 * e.g. `zed-agent/desktop` vs `zed-agent-wt-rtl/desktop`. Groups with a
...
export function workspaceGroupsFor(
  ...
  if (!options.preserveSessionOrder) {
    ...
    // creation time so they don't reshuffle every time a message lands â€” keeps
 ...
 * the worktree they ran in. `parent â†’ worktree â†’ sessions`.
 */
...
export type WorktreeResolver = (cwd: string) => ZedWorktreeInfo | null | undefined
...
function placeWorkspace(path: string, resolver?: WorktreeResolver): WorkspacePlacement | null {
  ...
  if (info?.repoRoot && info.worktreeRoot) {
    ...
    return {
      ...
      // The main checkout's branch is transient â€” it changes as you work, so a
...
/** Unique, non-empty session cwds â€” the batch to probe for worktree info. */
...
/**
 * Build the `parent â†’ worktree â†’ sessions` tree. Parents keep recency order
