...
import type { ZedWorktreeInfo } from '@/global'
import type { SessionInfo } from '@/types/zed'
...
describe('workspaceGroupsFor', () => {
  it('groups by full cwd, not by basename â€” same-named folders are separate groups', () => {
    const groups = workspaceGroupsFor(
      [makeSession('/a/zed-agent/apps/desktop'), makeSession('/a/zed-agent-wt-rtl/apps/desktop')],
      'No workspace'
    )
  ...
  it('disambiguates colliding basenames by walking up the path', () => {
    expect(
      labels([makeSession('/a/zed-agent/apps/desktop'), makeSession('/a/zed-agent-wt-rtl/apps/desktop')])
    ).toEqual(['zed-agent/apps/desktop', 'zed-agent-wt-rtl/apps/desktop'])
  })

  it('leaves a unique basename as its short label', () => {
    expect(labels([makeSession('/a/zed-agent/apps/desktop'), makeSession('/b/heval-py')])).toEqual([
      'desktop',
      'heval-py'
    ])
  })
...
const info = (over: Partial<ZedWorktreeInfo> & Pick<ZedWorktreeInfo, 'repoRoot' | 'worktreeRoot'>): ZedWorktreeInfo => ({
  branch: null,
...
describe('workspaceTreeFor', () => {
  it('heuristic nests `<repo>-wt-<branch>` under its sibling repo', () => {
    const tree = workspaceTreeFor(
      [makeSession('/www/zed-agent'), makeSession('/www/zed-agent-wt-rtl')],
      'No workspace'
    )

    expect(tree).toHaveLength(1)
    expect(tree[0].label).toBe('zed-agent')
    expect(tree[0].groups.map(g => g.label).sort()).toEqual(['zed-agent', 'rtl'])
  })

  it('git metadata is authoritative â€” worktrees group by repoRoot regardless of directory naming', () => {
    const resolver: WorktreeResolver = cwd => {
      if (cwd === '/www/zed-agent') {
        return info({ repoRoot: '/www/zed-agent', worktreeRoot: '/www/zed-agent', isMainWorktree: true, branch: 'main' })
      }

      if (cwd === '/elsewhere/ha-rtl') {
        return info({ repoRoot: '/www/zed-agent', worktreeRoot: '/elsewhere/ha-rtl', branch: 'rtl' })
      }
    ...
    const tree = workspaceTreeFor(
      [makeSession('/www/zed-agent'), makeSession('/elsewhere/ha-rtl')],
      'No workspace',
      resolver
    )
    ...
    expect(tree).toHaveLength(1)
    expect(tree[0].label).toBe('zed-agent')
    // The main checkout labels by directory (its branch is transient â€” using it
    // would misattribute old sessions to the currently checked-out branch);
    // linked worktrees label by branch.
    expect(tree[0].groups.map(g => g.label)).toEqual(['zed-agent', 'rtl'])
  })

  it('a standalone directory is its own parent (always parent â†’ worktree â†’ sessions)', () => {
  ...
  it('aggregates session counts across a repoâ€™s worktrees', () => {
    const tree = workspaceTreeFor(
