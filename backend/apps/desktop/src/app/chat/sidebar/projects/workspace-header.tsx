...
const branchActionLabel = (branch: zedGitBranch, copy: BranchActionCopy) => {
...
// inside it. Naming is explicit — no auto-generated `zed/work-<ts>` trees.
export function StartWorkButton({ repoPath, onStarted }: { repoPath: string; onStarted: (path: string) => void }) {
  ...
  const [branches, setBranches] = useState<zedGitBranch[]>([])
  ...
  const convert = async (branch: zedGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
