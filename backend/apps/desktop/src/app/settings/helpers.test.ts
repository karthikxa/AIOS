...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: ZedConfigRecord = { display: { theme: 'mono' } }
  ...
  describe('stripToolsetLabel', () => {
    it('removes leading emoji prefixes from registry labels', () => {
      expect(stripToolsetLabel('â° Cron Jobs')).toBe('Cron Jobs')
      expect(stripToolsetLabel('âš¡ Code Execution')).toBe('Code Execution')
      expect(stripToolsetLabel('â“ Clarifying Questions')).toBe('Clarifying Questions')
      expect(stripToolsetLabel('ðŸŒ Browser Automation')).toBe('Browser Automation')
      expect(stripToolsetLabel('ðŸŽ¨ Image Generation')).toBe('Image Generation')
    })
  ...
  describe('toolsetDisplayLabel', () => {
    it('strips emoji from toolset rows', () => {
      expect(toolsetDisplayLabel({ name: 'cronjob', label: 'â° Cron Jobs' })).toBe('Cron Jobs')
    })
  })
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // ZED_QWEN_ and ZED_GEMINI_ both share the ZED_ stem.
      expect(providerGroup('ZED_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
      expect(providerGroup('ZED_GEMINI_CLIENT_ID')).toBe('Gemini')
    })
  ...
  describe('enumOptionsFor â€” backend selector dropdowns', () => {
    const config: ZedConfigRecord = {}
