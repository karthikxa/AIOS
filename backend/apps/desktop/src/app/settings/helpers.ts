import type { ZedConfigRecord, ToolsetInfo } from '@/types/zed'
...
export const redactedValue = (v: string) => (v.length <= 8 ? 'â€¢â€¢â€¢â€¢' : `${v.slice(0, 4)}...${v.slice(-4)}`)
...
export function getNested(obj: ZedConfigRecord, path: string): unknown {
  let cur: unknown = obj
...
export function setNested(obj: ZedConfigRecord, path: string, value: unknown): ZedConfigRecord {
...
function personalityOptions(config: ZedConfigRecord): string[] {
