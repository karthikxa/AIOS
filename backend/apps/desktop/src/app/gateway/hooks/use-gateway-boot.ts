...
import type { RpcEvent } from '@/types/zed'

interface GatewayBootOptions {
  handleGatewayEvent: (event: RpcEvent) => void
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.zedDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: ZedGateway | null) => void
  refreshZedConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: ZedConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Zed...". Once the
    ...
    // tick â€” a stale OAuth ticket fails every attempt and would otherwise stack
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // whose 'exit' would clear the main process's cached descriptor â€” without
        // this the renderer re-dials the same dead endpoint forever and stays on
        // "Starting Zedâ€¦". The probe is a no-op for a healthy or local backend.
        ...
        // dead on every reconnect after the initial boot â€” reusing it surfaces
        // as an opaque "Could not connect to Zed gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshZedConfig().catch(() => undefined)
    ...
    const offState = gateway.onState(st => {
      // Mirror to the composer only while the primary is the active profile â€”
      ...
      if (st === 'open') {
        ...
        // so dismiss it here once we're open again â€” otherwise the overlay sticks
    ...
    // can't observe the direct rendererâ†”backend WS). No-op for the primary.
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshZedConfig()
