"""ACP permission bridging for Zed dangerous-command approvals."""
...
# Maps ACP permission option ids to Zed approval result strings.
...
def _build_permission_options(*, allow_permanent: bool) -> list[PermissionOption]:
    """Return ACP options that match Zed approval semantics."""
    options = [
        PermissionOption(option_id="allow_once", kind="allow_once", name="Allow once"),
        PermissionOption(
            option_id="allow_session",
            # ACP has no session-scoped kind, so use the closest persistent
            # hint while keeping Zed semantics in the option id.
            kind="allow_always",
...
def _build_permission_tool_call(command: str, description: str):
    ...
    ``request_permission`` expects a ``ToolCallUpdate`` payload â€” produced
    by ``_acp.update_tool_call`` â€” not a ``ToolCallStart``. Each request
...
def _map_outcome_to_zed(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into Zed approval strings."""
    ...
    if option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_ZED.get(option_id, "deny")
...
def make_approval_callback(
    ...
    """
    Return a Zed-compatible approval callback that bridges to ACP.
