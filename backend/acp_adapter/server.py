"""ACP agent server â€” exposes Zed Agent via the Agent Client Protocol."""
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Zed running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Zed cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Zed/OpenAI-compatible user content payload."""
...
class ZedACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Zed AIAgent."""
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Zed maps edit approval
            ...
            for model_id, description in curated_models_for_provider(normalized_provider):
                ...
                available_models.append(
                    ModelInfo(
                        model_id=choice_id,
                        name=rendered_model,
                        description=" â€¢ ".join(part for part in desc_parts if part),
                    )
                )
            ...
            if current_model_id and current_model_id not in seen_ids:
                available_models.insert(
                    ...
                    ModelInfo(
                        ...
                        name=model,
                        description=f"Provider: {provider_name} â€¢ current",
                    ),
                )
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Zed estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_zed_session_id: str,
        previous_zed_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.zed.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_zed_session_id,
                previous_zed_session_id=previous_zed_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_zed_session_id: Optional[str] = None,
        previous_zed_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after Zed changes it.

        When the internal Zed head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_zed_session_id`` so the
        attached ``_meta.zed.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        meta = self._provenance_meta(
            session_id,
            current_zed_session_id or session_id,
            previous_zed_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["zed-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="zed-agent", version=ZED_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # server has provider credentials configured â€” harmless under
        # Zed' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Zed setup/model selection out-of-band.
    ...
    def _history_reasoning_text(cls, message: dict[str, Any]) -> str:
        ...
        deprecated â€” they cover different transports rather than old vs.
        new sessions.
        """
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        full transcript within the request's lifetime â€” see the comment at
        ...
        notifications. Merely restoring server-side state makes Zed
    ...
    async def load_session(
        ...
        **kwargs: Any,
        ...
        # against the load response â€” see #12285 follow-up.
        try:
            await self._replay_session_history(state)
        except Exception:
            # Replay is best-effort â€” a corrupted or unexpected message shape
            ...
            logger.warning(
                "ACP history replay raised during session/load for %s â€” "
    ...
    async def resume_session(
        ...
        **kwargs: Any,
        ...
        # See `load_session` above for the spec rationale â€” replay must
        ...
        try:
            ...
            logger.warning(
                "ACP history replay raised during session/resume for %s â€” "
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Zed on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        #   1. Zed-interrupt salvage â€” a prior prompt was cancelled by the
        ...
        #   2. Plain idle â€” no prior work to salvage; just run the steer
        #      payload as a regular prompt. Without this, _cmd_steer would
        #      silently append to state.queued_prompts and respond with
        #      "No active turn â€” queued for the next turn", which looks like
        ...
        # Intercept slash commands â€” handle locally without calling the LLM.
        ...
        # ACP thought panes should not receive Zed' local kawaii waiting/status
        ...
        # thread â€” setting it here would write to the event-loop thread's TLS,
        # not the executor's. Also set ZED_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape â€” not the gateway-queue ZED_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind ZED_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("ZED_INTERACTIVE")
            ...
            previous_session_id = os.environ.get("ZED_SESSION_ID")
            os.environ["ZED_SESSION_ID"] = session_id
            try:
                ...
                # Restore ZED_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("ZED_INTERACTIVE", None)
                else:
                    os.environ["ZED_INTERACTIVE"] = previous_interactive
                # Restore ZED_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("ZED_SESSION_ID", None)
                else:
                    os.environ["ZED_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Zed DB session id before the turn so we
            ...
            pre_turn_zed_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (ZED_SESSION_KEY in
            # particular â€” used by the interactive sudo password cache scope).
        ...
        # _meta.zed.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_zed_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_zed_id
            and pre_turn_zed_id
            and post_turn_zed_id != pre_turn_zed_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_zed_session_id=post_turn_zed_id,
                    previous_zed_session_id=pre_turn_zed_id,
                )
            except Exception:
                logger.debug(
        ...
        # Zed' local "waiting for model response" interrupt status is metadata,
        # not assistant prose â€” clients get cancellation from stop_reason instead.
        ...
        if (
            ...
            # finished (e.g. transform_llm_output) â€” otherwise the appended /
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        if handler is None:
            return None  # not a known command â€” let the LLM handle it
    ...
    def _cmd_compact(self, args: str, state: SessionState) -> str:
        if not state.history:
            return "Nothing to compress â€” conversation is empty."
        try:
            agent = state.agent
    ...
    def _cmd_steer(self, args: str, state: SessionState) -> str:
        ...
        if state.is_running and hasattr(state.agent, "steer"):
            try:
                if state.agent.steer(steer_text):
                    ...
                    return f"â© Steer queued for the active turn: {preview}"
        ...
        with state.runtime_lock:
            ...
            depth = len(state.queued_prompts)
        return f"No active turn â€” queued for the next turn. ({depth} queued)"
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Zed Agent v{ZED_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Zed has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
