"""CLI entry point for the zed-agent ACP adapter.

Loads environment variables from ``~/.zed/.env``, configures logging
...
# caller â€” but the supervisor task that dispatches the request then surfaces
...
class _BenignProbeMethodFilter(logging.Filter):
    ...
    background-task error â€” including method_not_found for any non-probe
    method â€” visible in stderr.
    """
...
def _load_env() -> None:
    ...
    if loaded:
        ...
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", zed_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="zed-acp",
        description="Run Zed Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Zed version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Zed provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.zed/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import ZedACPAgent  # noqa: F401

    print("Zed ACP check OK")
...
def _run_setup() -> None:
    ...
    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "zed", "model"]
        zed_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``zed postinstall`` and the runtime lazy installer.
    ...
    try:
        ...
        if not node_ok:
            print("Node.js installation failed â€” cannot proceed with browser tools.",
                  file=sys.stderr)
            return 1
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting zed-agent ACP adapter")
    ...
    import acp
    from .server import ZedACPAgent

    # MCP tool discovery from config.yaml â€” run before asyncio.run() so
