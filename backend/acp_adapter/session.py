"""ACP session manager â€” maps ACP sessions to Zed AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.zed/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Zed itself is running in WSL.

    Windows ACP clients can launch ``zed acp`` inside WSL while still sending
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Zed from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["zed-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Zed agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Zed AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Zed runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.zed/state.db``) is lazily created.
        """
        self._sessions: Dict[str, SessionState] = {}
        self._lock = Lock()
        self._agent_factory = agent_factory
        self._db_instance = db  # None â†’ lazy-init on first use
    ...
    def _get_db(self):
        ...
        Note: we resolve ``ZED_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_zed_home``).
        """
