"""ACP tool-call helpers for mapping zed tools to ACP ToolKind and building content."""
...
# ---------------------------------------------------------------------------
# Map zed tool names -> ACP ToolKind
# ---------------------------------------------------------------------------
...
def get_tool_kind(tool_name: str) -> ToolKind:
    """Return the ACP ToolKind for a zed tool, defaulting to 'other'."""
...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    # Some Zed tools append a human hint after a JSON payload, e.g.
...
def _tool_result_failed(result: Optional[str], tool_name: str | None = None) -> bool:
    """Return True when a structured Zed tool result clearly failed.
    ...
    # produced by the wrapper itself â€” it cannot legitimately appear in
    ...
    # Zed core/polished tools commonly report tool-level failures as a
...
def _format_todo_result(result: Optional[str]) -> Optional[str]:
    ...
    for item in data["todos"]:
        ...
        if content:
            lines.append(f"- {icon.get(status, 'â€¢')} {content}")
    if summary:
...
def _format_read_file_result(result: Optional[str], args: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    if data.get("total_lines") is not None:
        header += f" â€” {data.get('total_lines')} total lines"
    # Zed read_file output is line-numbered with `|`. If we send it as raw
...
def _format_skill_manage_result(result: Optional[str], args: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    status = "âœ… Skill updated" if success is not False else "âœ— Skill update failed"
...
def _format_web_search_result(result: Optional[str]) -> Optional[str]:
    ...
    for item in web[:10]:
        ...
        lines.append(f"â€¢ {title}" + (f" â€” {url}" if url else ""))
        if desc:
            lines.append(f"  {desc}")
    return _truncate_text("\n".join(lines))
...
def _format_web_extract_result(result: Optional[str]) -> Optional[str]:
    ...
    for item in results[:10]:
        ...
        failures.append(
            f"- {title}" + (f" â€” {url}" if url and url != title else "") + f"\n  Error: {_truncate_text(error, limit=500)}"
        )
...
def _format_process_result(result: Optional[str], args: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    if isinstance(data.get("processes"), list):
        ...
        for proc in processes[:20]:
            ...
            lines.append(f"- `{sid}` â€” {', '.join(bits)}" + (f" â€” {cmd[:120]}" if cmd else ""))
        if len(processes) > 20:
...
def _format_delegate_result(result: Optional[str]) -> Optional[str]:
    ...
    icon = {"completed": "âœ…", "failed": "âœ—", "error": "âœ—", "timeout": "â±", "interrupted": "âš "}
    for item in results:
        if not isinstance(item, dict):
            lines.append(f"- {item}")
            continue
        ...
        header = f"{icon.get(status, 'â€¢')} Task {idx + 1 if isinstance(idx, int) else '?'}: {status}"
        bits = []
        if model:
...
def _format_session_search_result(result: Optional[str]) -> Optional[str]:
    ...
    for item in results:
        ...
        lines.append(f"- **{title}** (`{sid}`)" + (f" â€” {meta}" if meta else ""))
...
def _format_memory_result(result: Optional[str], args: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    if data.get("success") is False:
        lines = [f"âœ— Memory {action} failed ({target})", str(data.get("error") or "unknown error")]
        matches = data.get("matches")
        if isinstance(matches, list) and matches:
            lines.append("Matches:")
            lines.extend(f"- {_truncate_text(str(m), 160)}" for m in matches[:5])
        return "\n".join(lines)
    lines = [f"âœ… Memory {action} saved ({target})"]
    if data.get("message"):
...
def _format_edit_result(tool_name: str, result: Optional[str], args: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    if isinstance(data, dict):
        ...
        replacements = data.get("replacements") or data.get("replacement_count")
        lines = [f"âœ… {tool_name} completed" + (f" for `{path}`" if path else "")]
        if message:
    ...
    return f"âœ… {tool_name} completed" + (f" for `{path}`" if path else "")
        ...
        if isinstance(images, list):
            ...
            for img in images[:12]:
                if isinstance(img, dict):
                    ...
                    lines.append(f"- {alt or 'image'}" + (f" â€” {url}" if url else ""))
...
def _format_media_or_cron_result(tool_name: str, result: Optional[str]) -> Optional[str]:
    ...
    if data.get("success") is False or data.get("error"):
        return f"{tool_name} failed: {data.get('error', 'unknown error')}"
    lines = [f"âœ… {tool_name} completed"]
...
def _format_generic_structured_result(
    ...
    *,
    ...
    lines = [f"âœ… {tool_name} completed" if data.get("success") is True else f"{tool_name} result"]
    priority_keys = (
...
def build_tool_start(
    ...
    *,
    ...
    """Create a ToolCallStart event for the given zed tool invocation."""
