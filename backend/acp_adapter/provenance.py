...
This is an additive Zed extension surfaced under ACP ``_meta.zed`` so
...
compression rotates the internal Zed head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.zed.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_zed_session_id: The live internal Zed DB session id
            (``state.agent.session_id``).
        previous_zed_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"zed": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_zed_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    # toward depth â€” delegate/branch children share the parent_session_id
    # column but are not compaction boundaries.
    root_id = current_zed_session_id
    compression_depth = 0
    cursor_parent = parent_id
    seen = {current_zed_session_id}
    ...
    rotated = bool(
        ...
        and previous_zed_session_id != current_zed_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentZedSessionId": current_zed_session_id,
        "rootZedSessionId": root_id,
        "parentZedSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root",
        "compressionDepth": compression_depth,
    }
    if previous_zed_session_id:
        provenance["previousZedSessionId"] = previous_zed_session_id
    if rotated:
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"zed": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"zed": {"sessionProvenance": prov}}
