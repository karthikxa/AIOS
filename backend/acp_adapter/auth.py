"""ACP auth helpers â€” detect and advertise Zed authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "zed-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Zed runtime provider, or None if unavailable.

    Treats a ``Callable`` ``api_key`` (Azure Foundry Entra ID bearer
    token provider â€” see :mod:`agent.azure_identity_adapter`) as a valid
...
def has_provider() -> bool:
    """Return True if Zed can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Zed.
    ...
    not have Zed provider credentials configured yet, so Zed always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Zed using the currently configured "
