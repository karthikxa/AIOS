"""
zed fallback â€” manage the fallback provider chain.
...
Subcommands:
  zed fallback [list]   Show the current fallback chain (default when no subcommand)
  zed fallback add      Pick provider + model via the same picker as `zed model`,
                           then append the selection to the chain
  zed fallback remove   Pick an entry to delete from the chain
  zed fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.zed/config.yaml`` (top-level, list of
...
def _read_chain(config: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    always a fresh copy â€” callers can mutate without touching the config dict.
    """
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from zed_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        with _auth_store_lock():
            store = _load_auth_store()
            store["active_provider"] = value
            _save_auth_store(store)
    except Exception:
        # Best-effort â€” if auth.json can't be restored, the user's primary
        # provider may have been deactivated by the picker.  They can re-run
        # `zed model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  zed fallback add")
        print()
        return
    ...
    print("  Docs: https://zed-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `zed model`, then append the selection to the chain."""
    from zed_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `zed model` â€” select the provider + model you want as a fallback.")
    print()

    try:
        select_provider_and_model(args=args)
    except SystemExit:
        # Some provider flows exit on auth failure â€” restore state and re-raise.
    ...
    # Picker picked the same thing that's already the primary â†’ nothing changed,
    ...
    if primary_entry and primary_entry["provider"] == new_entry["provider"] \
        ...
        print("  A provider cannot be a fallback for itself â€” no change.")
        return
    ...
    for existing in chain:
        if existing.get("provider") == new_entry["provider"] \
            ...
            print()
            print(f"  {_format_entry(new_entry)} is already in the fallback chain â€” skipped.")
            return
    ...
    print()
    print("  Run `zed fallback list` to view, or `zed fallback remove` to delete.")
...
def cmd_fallback_remove(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        print()
        print("  No fallback providers configured â€” nothing to remove.")
        print()
        return
...
def cmd_fallback_clear(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        print()
        print("  No fallback providers configured â€” nothing to clear.")
        print()
        return
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``zed fallback [subcommand]``."""
