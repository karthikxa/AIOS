"""Profile describer â€” auto-generate ``description`` for a profile.

Used by ``zed profile describe <name> --auto`` and the dashboard's
...
- Reads at most ``MAX_SKILLS_FOR_PROMPT`` skill names to keep the
  prompt bounded. No skill body â€” names + categories are enough
...
# is per-category â€” see _collect_skills.
MAX_SKILLS_FOR_PROMPT = 60


_SYSTEM_PROMPT = """You are a profile-describer for the Zed Agent kanban board.

A user runs multiple "profiles" â€” distinct agent identities, each with their
...
def _collect_skills(profile_dir: Path) -> list[str]:
    ...
    # important â€” we'll let the LLM see a sample. Pick evenly-spaced
...
def describe_profile(
    ...
    *,
    ...
    API error, malformed response) â€” those surface via ``ok=False`` so
