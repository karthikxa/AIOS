...
This command:
  ...
  3. Writes ``ZED_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``ZED_DASHBOARD_PORTAL_URL`` into ``~/.zed/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      1. ``override`` â€” explicit ``--portal-url`` flag or
         ``ZED_DASHBOARD_PORTAL_URL`` env (used for testing against a
    ...
    try:
        from zed_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def _register_self_hosted_client(
    *,
    ...
    dashboard record in place instead of minting a duplicate â€” this is what
    makes re-running ``zed dashboard register`` idempotent. The portal
...
def _print_post_register_hint(
    *,
    ...
    from zed_cli.config import get_env_path

    env_path = get_env_path()
    _cid = client_id
    print()
    print(f"  Wrote to {env_path}:")
    print("    ZED_DASHBOARD_OAUTH_CLIENT_ID=" + str(_cid))
    if wrote_portal_url:
        print("    ZED_DASHBOARD_PORTAL_URL=" + str(portal_base_url))
    if public_url:
...
def cmd_dashboard_register(args) -> None:
    ...
    # stamped in by the orchestrator (NAS sets ZED_DASHBOARD_OAUTH_CLIENT_ID
    # via buildContainerEnvVars). Registering from inside such a container is a
    # mistake â€” and save_env_value refuses to write anyway.
    if is_managed():
        print(
            "âœ— `zed dashboard register` is not available in a managed/hosted "
            "install.\n"
    ...
    # ZED_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    #
    ...
    # client_id locally (ZED_DASHBOARD_OAUTH_CLIENT_ID). Re-send it so the
    ...
    try:
        existing_client_id = get_env_value("ZED_DASHBOARD_OAUTH_CLIENT_ID")
    except Exception:
        existing_client_id = None
    ...
    # every time â€” so leave `name` unset and let the portal preserve it.
    if explicit_name:
        name = explicit_name
    ...
    if updated_existing:
        print(f'âœ“ Updated dashboard "{registered_name}"')
    else:
        print(f'âœ“ Registered dashboard "{registered_name}"')

    # 3. Write env vars idempotently. Always set the client_id.
    try:
        save_env_value("ZED_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
        print(f"âœ— Failed to write ZED_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}")
        print(f"  Set it manually:  ZED_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)

    # Persist the portal URL. Two cases:
    #   a) The user explicitly supplied a custom portal (--portal-url flag or
    #      ZED_DASHBOARD_PORTAL_URL env). That's an intentional choice we
    #      always persist so it survives across sessions â€” overwriting any
    ...
    if should_write_portal:
        try:
            save_env_value("ZED_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    # the portal, e.g. https://zed.example.com/auth/callback. At serve time
    # the dashboard auth layer (dashboard_auth/routes._redirect_uri) reconstructs
    # that same callback by taking ZED_DASHBOARD_PUBLIC_URL and appending
    # "/auth/callback" verbatim. So the value the runtime actually consumes is
    # the ORIGIN (scheme://host[:port]), not the full callback path â€” persisting
    # the raw redirect URI would double up the path. We derive the origin from
    # the supplied redirect URI and persist it as ZED_DASHBOARD_PUBLIC_URL so
    ...
    if public_url:
        ...
        try:
            existing_public_url = get_env_value("ZED_DASHBOARD_PUBLIC_URL")
        except Exception:
            existing_public_url = None
        if existing_public_url != public_url:
            try:
                save_env_value("ZED_DASHBOARD_PUBLIC_URL", public_url)
