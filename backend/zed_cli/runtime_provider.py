...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    (or one of the local-server aliases that resolve to ``custom`` â€” ollama, vllm, llamacpp, â€¦),
    ...
    # (ollama, vllm, llamacpp, â€¦) should be trusted the same way "custom"
    # is, otherwise a legit LAN/WireGuard ollama endpoint silently falls
    # through to OpenRouter.
    try:
        from zed_cli.auth import resolve_provider as _resolve_provider
...
def _detect_api_mode_for_url(base_url: str) -> Optional[str]:
    ...
    - Third-party Anthropic-compatible gateways (MiniMax, Zhipu GLM,
      ...
      protocol under a ``/anthropic`` suffix â€” treat those as
...
def _host_derived_api_key(base_url: str) -> str:
    ...
    Examples:
        https://api.deepseek.com/v1   â†’ DEEPSEEK_API_KEY
        https://api.groq.com/openai/v1 â†’ GROQ_API_KEY
        https://api.mistral.ai/v1     â†’ MISTRAL_API_KEY
        https://generativelanguage.googleapis.com/v1beta/openai/ â†’ GOOGLEAPIS_API_KEY

    Returns the env value (stripped) or "". Never returns env vars whose names
    are already explicitly checked elsewhere â€” those are handled by their own
    ...
    (``api.deepseek.com`` â†’ ``deepseek``). Falls back to "" for hostnames
    ...
    if not hostname:
        return ""
    # Reject IPv4 / IPv6 / loopback â€” no meaningful vendor label.
    if any(ch.isdigit() for ch in hostname.split(".")[-1]):
        # Last label starts with a digit â†’ likely IP. (TLDs are never numeric.)
        return ""
    ...
    #   deepseek.com               â†’ labels[-2] = "deepseek"  âœ“
    #   api.groq.com â†’ groq.com    â†’ labels[-2] = "groq"      âœ“
    #   api.mistral.ai             â†’ labels[-2] = "mistral"   âœ“
    # Crucially, lookalike hosts pick the ATTACKER's label, not the spoofed
    # vendor:
    #   api.deepseek.com.attacker.test â†’ labels[-2] = "attacker"
...
def _auto_detect_local_model(base_url: str) -> str:
    ...
    try:
        ...
        # Log instead of silently swallowing â€” aids debugging when
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from zed_cli.models import copilot_model_api_mode
...
_VALID_API_MODES = {
    ...
    # instead of Zed' tool dispatch. Gated behind config key
...
def _maybe_apply_codex_app_server_runtime(
    *,
    ...
    """Optional opt-in: rewrite api_mode â†’ "codex_app_server" for OpenAI/Codex
    ...
    are eligible â€” other providers (anthropic, openrouter, etc.) cannot be
...
def _resolve_runtime_from_pool_entry(
    *,
    ...
    # longer matches the model actually being used â€” the bug that caused
    ...
    if provider == "openai-codex":
        ...
        # /chat/completions on these with 400 "operation unsupported" â€” see
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from zed_cli.models import azure_foundry_model_api_mode
        ...
        # matches this provider â€” same pattern as the Anthropic branch above.
        ...
        if provider in {"opencode-zen", "opencode-go"}:
            ...
            # Refs #16878.
            from zed_cli.models import opencode_model_api_mode
            ...
            # Kimi /coding, api.openai.com â†’ codex_responses, api.x.ai â†’
            # codex_responses).
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = _getenv("ZED_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    # Bare "custom" is normally an incomplete spec â€” the canonical form is
    # "custom:<name>" â€” and is otherwise owned by the model.base_url "bare
    ...
    # the global default (Codex) â€” the cause of cron jobs with
    ...
    # is exempt from the shadow check â€” it is not a built-in to defer to.
    ...
    if requested_norm != "custom" and not requested_norm.startswith("custom:"):
        try:
            ...
            # only an *alias* (``kimi`` â†’ built-in ``kimi-coding``) is the
            # user's intended target â€” alias rewriting would otherwise hijack
            # the request.  We only defer to the built-in when the raw name is
            # the canonical provider itself (``nous``, ``openrouter``, â€¦) so
            # accidentally shadowing a canonical provider still resolves to
            # the built-in. See tests/zed_cli/test_runtime_provider_resolution.py
        ...
        for ep_name, entry in providers.items():
            ...
            if requested_norm in {ep_name, name_norm, f"custom:{name_norm}"}:
                ...
                if base_url:
                    ...
                    # The v11â†’v12 migration writes the API mode under the new
                    # ``transport`` field, but hand-edited configs may still
                    # use the legacy ``api_mode`` spelling.  Accept both â€”
...
def has_named_custom_provider(requested_provider: str) -> bool:
    ...
    entry â€” without reaching into a private helper or duplicating the scan.
    """
    try:
...
def find_custom_provider_identity(base_url: str) -> Optional[str]:
    ...
    named custom endpoint that is the literal string ``"custom"`` â€” the entry
...
def canonical_custom_identity(
    *,
    ...
    every named ``providers:`` / ``custom_providers:`` entry â€” it is NOT a
    ...
    1. ``base_url`` â€” reverse-lookup the entry that owns the endpoint URL
       (the one fact that always survives the persistence round-trip when a
       URL was recorded).
    2. ``config_provider`` â€” the active ``config.model.provider`` (or its
       ``provider``/``ZED_INFERENCE_PROVIDER`` equivalent). When the agent
    ...
    ``None`` (caller keeps whatever it had â€” bare ``"custom"`` only as a last
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("ZED_INFERENCE_PROVIDER", "").strip()
...
def _resolve_named_custom_runtime(
    *,
    ...
    # from a `model_aliases:` direct-alias resolution) â€” build a runtime
    # directly so the alias's base_url actually takes effect.
    #
    # GitHub #27132: provider aliases that resolve to "custom" at runtime
    # (ollama, vllm, llamacpp, â€¦) are treated identically here, so a YAML
    ...
    if requested_norm == "custom" and explicit_base_url:
        ...
        # Check credential pool first â€” mirrors the named-custom-provider path
    ...
    if pool_result:
        # Propagate the model name even when using pooled credentials â€”
    ...
    api_key_candidates = [
        ...
        # Gate provider env keys on their authoritative hosts â€” sending
...
def _resolve_openrouter_runtime(
    *,
    ...
    # vllm, llamacpp, â€¦) follow the same base_url trust + routing rules
    # as a bare `provider: custom`. Normalising here keeps every check
    # below â€” `requested_norm == "custom"`, the trust check, the pool
    # gate up the stack â€” alias-aware without duplicating the alias map.
    if requested_norm and requested_norm != "custom":
        try:
            from zed_cli.auth import resolve_provider as _resolve_provider
    ...
    # OPENAI_BASE_URL env var is no longer consulted â€” config.yaml is
    ...
    # for key selection â€” if the user set OPENROUTER_BASE_URL or requested
    ...
    if _is_openrouter_context:
        ...
        # When the endpoint is Ollama Cloud, check OLLAMA_API_KEY â€” it's
        # the canonical env var for ollama.com authentication. Match on
        # HOST, not substring â€” a custom base_url whose path contains
        ...
        # Gate each provider key on its own host â€” sending OPENAI_API_KEY or
        # OPENROUTER_API_KEY to an unrelated custom endpoint (DeepSeek, Groq,
        # Mistral, â€¦) leaks credentials and causes 401s (issue #28660).
    ...
    # authentication â€” the OpenAI SDK requires a non-empty api_key string.
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'zed model' or "
    ...
    # â”€â”€ Entra ID (Microsoft Foundry recommended path) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    #
    # OpenAI-style endpoints use the OpenAI SDK's native callable
    # ``api_key=`` contract â€” the SDK mints a fresh JWT per request
    # automatically.
    #
    ...
    # are identical â€” return the callable api_key and let the
    # downstream SDK wrapper handle the contract difference.
    if cfg_auth_mode == "entra_id":
        if explicit_api_key:
            # User passed --api-key on the CLI while config says entra_id â€”
    ...
    # â”€â”€ Static API key (legacy / default) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    if provider == "nous":
        ...
        api_key = explicit_api_key or (
            ...
            if _agent_key_is_usable(
                state,
                max(60, env_int("ZED_NOUS_MIN_KEY_TTL_SECONDS", 1800)),
            )
            else ""
        )
        ...
        if not api_key:
            creds = resolve_nous_runtime_credentials(
                timeout_seconds=float(_getenv("ZED_NOUS_TIMEOUT_SECONDS", "15")),
            )
            api_key = creds.get("api_key", "")
    ...
    if pconfig and pconfig.auth_type == "api_key":
        ...
        if provider == "copilot":
            ...
            if configured_mode:
                ...
                # api.openai.com â†’ Responses, Kimi /coding, etc.).
                detected = _detect_api_mode_for_url(base_url)
                if detected:
                    api_mode = detected
...
def resolve_runtime_provider(
    *,
    ...
    if pool and pool.has_credentials():
        ...
        # non-runtime contexts like `zed auth list`).  If the key is
        # expired, clear pool_api_key so we fall through to
        # resolve_nous_runtime_credentials() which handles refresh.
        if provider == "nous" and entry is not None and pool_api_key:
            min_ttl = max(60, env_int("ZED_NOUS_MIN_KEY_TTL_SECONDS", 1800))
            nous_state = {
    ...
    if provider == "nous":
        try:
            ...
            if requested_provider != "auto":
                raise
            # Auto-detected Nous but credentials are stale/revoked â€”
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "zed-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
            if requested_provider != "auto":
                raise
            # Auto-detected Codex but credentials are stale/revoked â€”
            # fall through to env-var providers (e.g. OpenRouter).
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from zed_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        # For Microsoft Foundry endpoints, use ANTHROPIC_API_KEY directly â€”
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Zed canonical â€” matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Zed-compatible importers).
    ...
    if provider == "bedrock":
        ...
        # trust boto3's credential chain â€” it handles IMDS, ECS task roles,
        ...
        # Region priority: config.yaml bedrock.region â†’ env var â†’ us-east-1
        ...
        if is_anthropic_bedrock_model(_current_model):
            # Claude on Bedrock â†’ AnthropicBedrock SDK â†’ anthropic_messages path
            runtime = {
                "provider": "bedrock",
                "api_mode": "anthropic_messages",
            ...
            # Non-Claude (Nova, DeepSeek, Llama, etc.) â†’ Converse API
            runtime = {
                "provider": "bedrock",
    ...
    if pconfig and pconfig.auth_type == "api_key":
        creds = resolve_api_key_provider_credentials(provider)
        # Honour model.base_url from config.yaml when the configured provider
        # matches this provider â€” mirrors the Anthropic path above.  Without
        ...
        if provider == "copilot":
            ...
            if provider in {"opencode-zen", "opencode-go"}:
                ...
                # plus api.openai.com â†’ codex_responses and api.x.ai â†’ codex_responses.
                detected = _detect_api_mode_for_url(base_url)
                if detected:
                    api_mode = detected
