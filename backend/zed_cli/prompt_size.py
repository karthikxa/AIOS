"""Prompt-size diagnostic: ``zed prompt-size``.

Reports a byte/char breakdown of the system prompt the agent would build for
a fresh session â€” system prompt total, the ``<available_skills>`` index,
...
def compute_prompt_breakdown(platform: str = "cli") -> Dict[str, Any]:
    ...
    # Skills index â€” the <available_skills> block (the largest single block
    ...
    # Tool-schema JSON â€” the other half of the fixed per-call payload.
...
def cmd_prompt_size(args: Any) -> None:
    """Entry point for ``zed prompt-size``."""
