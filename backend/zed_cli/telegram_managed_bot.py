...
manual BotFather token copy-paste. Zed talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.zed-agent.nousresearch.com"
...
DEFAULT_MANAGER_BOT = "ZedSetupBot"

DEFAULT_BOT_NAME = "Zed Agent"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``zed_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``zed_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"zed_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Zed Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  âœ— Could not reach the Zed Telegram onboarding service.")
        print("    Try the manual setup instead, or check your network.")
        return None

    print("  âœ“ Pairing created")
    ...
    sys.stdout.write("\r  âœ— Timed out waiting for bot creation.                    \n")
    sys.stdout.flush()
    print("    The bot may still be created â€” check Telegram.")
