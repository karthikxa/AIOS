"""Managed scope â€” IT-pushed, user-immutable config & env layer.

A system-level directory (default ``/etc/zed``, root-owned and not
user-writable) supplies ``config.yaml`` and ``.env`` values that WIN over the
user's ``~/.zed/config.yaml`` and ``~/.zed/.env`` on a per-leaf-key basis.

This is DISTINCT from ``zed_cli.config.is_managed()`` / ``ZED_MANAGED``,
...
_DEFAULT_MANAGED_DIR = Path("/etc/zed")
...
def _under_pytest() -> bool:
    ...
    Used to ignore the system default ``/etc/zed`` during tests so a real
    managed scope on a developer/CI box can't leak policy into the suite. Tests
    that exercise managed scope set ``ZED_MANAGED_DIR`` explicitly, which is
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      1. ``$ZED_MANAGED_DIR`` â€” deployment/bootstrap path override (IT-only;
         never persisted to any .env). Honored only when set to a non-empty value
         AND the directory exists.
      2. ``/etc/zed`` â€” POSIX default, when it exists. Ignored under pytest so
    ...
    """
    override = os.environ.get("ZED_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
...
def _cached_read(path: Path, cache: Dict[str, tuple], parse):
    ...
    try:
        with open(path, encoding="utf-8") as f:
            parsed = parse(f)
    except Exception as exc:  # noqa: BLE001 â€” fail-open, but LOUD
        logger.warning(
            "managed scope: failed to parse %s: %s â€” IGNORING this managed file. "
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through zed_cli.config.load_config) to honor managed
    scope. Mirrors zed_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from zed_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
        ...
        # ``model.default`` â€” otherwise _deep_merge would replace the caller's
    ...
    except Exception:  # noqa: BLE001 â€” overlay must never break a caller
