"""zed claw â€” OpenClaw migration commands.

Usage:
    zed claw migrate              # Preview then migrate (always shows preview first)
    zed claw migrate --dry-run    # Preview only, no changes
    zed claw migrate --yes        # Skip confirmation prompt
    zed claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    zed claw migrate --no-backup  # Skip pre-migration snapshot
    zed claw cleanup              # Archive leftover OpenClaw directories
    zed claw cleanup --dry-run    # Preview what would be archived
"""
...
from zed_cli.config import get_zed_home, get_config_path, load_config, save_config
...
def _detect_openclaw_processes() -> list[str]:
    ...
    if sys.platform == "win32":
        try:
            ...
            # Node.js-hosted OpenClaw â€” tasklist doesn't show command lines,
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Zed may try to use the same token, causing disconnects."
    )
    ...
    if not sys.stdin.isatty():
        print_info("Non-interactive session â€” continuing to preview only.")
        return
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Zed gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Zed gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'zed gateway stop'.")
    print()
    if not auto_yes and not prompt_yes_no("Continue anyway?", default=False):
        print_info("Migration cancelled. Stop the gateway and try again.")
        sys.exit(0)

# State files commonly found in OpenClaw workspace directories â€” listed
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_zed.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_zed", script_path)
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: zed claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Zed")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'zed claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw â†’ Zed migration."""
    ...
    # Secrets are never included implicitly â€” they must be explicitly requested
    # via --migrate-secrets, even under --preset full.  This mirrors OpenClaw's
    # migrate-zed posture (two-phase: run once without secrets, rerun with
    ...
    print(
        color(
            "â”‚          âš• Zed â€” OpenClaw Migration                 â”‚",
            Colors.MAGENTA,
        )
    )
    print(
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: zed claw migrate --source /path/to/.openclaw")
        return
    ...
    # Check if OpenClaw is still running â€” migrating tokens while both are
    ...
    # â”€â”€ Phase 1: Always preview first â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    try:
    ...
    if preview_count > 0:
        ...
        print_header(
            f"Migration Preview â€” {preview_conflicts} conflict(s), nothing would be imported"
        )
    ...
    # â”€â”€ Phase 1b: Refuse if the plan has conflicts and --overwrite is not set â”€
    # Modelled on OpenClaw's assertConflictFreePlan() â€” apply is a safe no-op
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.zed/. "
    ...
    # â”€â”€ Phase 2: Confirm and execute â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    print()
    if not auto_yes:
        if not sys.stdin.isatty():
            print_info("Non-interactive session â€” preview only.")
            print_info("To execute, re-run with: zed claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # â”€â”€ Phase 2b: Pre-apply backup of the Zed home â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # Delegates to zed_cli.backup.create_pre_migration_backup(), which
    # shares implementation with the pre-update backup (same exclusion
    # rules, same SQLite safe-copy, zip format) so the archive is
    # restorable with `zed import`.  Mirrors OpenClaw's
    # createPreMigrationBackup posture â€” one atomic restore point before
    # any mutation, auto-pruned to the last 5 pre-migration zips.
    backup_archive: Optional[Path] = None
    if not no_backup:
        try:
            from zed_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(zed_home=zed_home)
            if backup_archive:
                size_str = _format_size(backup_archive.stat().st_size)
                print()
                print_success(f"Pre-migration backup: {backup_archive} ({size_str})")
                print_info(f"Restore with: zed import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Zed home."
            )
    ...
    # Source directory is left untouched â€” archiving is not the migration
    # tool's responsibility.  Users who want to clean up can run
    # 'zed claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    # Warn if OpenClaw is still running â€” archiving while the service is
    ...
    if running:
        ...
        if not auto_yes:
            if not sys.stdin.isatty():
                print_info("Non-interactive session â€” aborting. Stop OpenClaw and re-run.")
                return
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: zed claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info(f"Would archive: {source_dir} â†’ {archive_path}")
        elif not auto_yes and not sys.stdin.isatty():
            print_info(f"Non-interactive session â€” would archive: {source_dir}")
            print_info("To execute, re-run with: zed claw cleanup --yes")
        elif auto_yes or prompt_yes_no(f"Archive {source_dir}?", default=True):
            try:
                archive_path = _archive_directory(source_dir)
                print_success(f"Archived: {source_dir} â†’ {archive_path}")
                total_archived += 1
            except OSError as e:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if items:
        ...
        if conflict_items:
            print(color("  âš  Conflicts (skipped â€” use --overwrite to force):", Colors.YELLOW))
        ...
        if skipped_items:
            print(color("  â”€ Skipped:", Colors.DIM))
        ...
        if error_items:
            print(color("  âœ— Errors:", Colors.RED))
    ...
    if dry_run:
        ...
        print_info(f"  zed claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            print()
            print(color("  âš  API keys were NOT migrated (secrets migration is disabled by default).", Colors.YELLOW))
            ...
            print_info("  zed claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  zed config set OPENROUTER_API_KEY sk-or-v1-...")
