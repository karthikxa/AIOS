"""Windows ConPTY bridge for the `zed dashboard` chat tab.

Drop-in counterpart to ``zed_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``zed_cli.web_server`` consumes: ``spawn``, ``read``,
