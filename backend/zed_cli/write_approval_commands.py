...
and a chat message. Skill diffs are intentionally NOT inlined here â€” the
...
def handle_pending_subcommand(
    ...
    *,
    ...
    """
    if not args:
        # Bare /memory or /skills with no sub â†’ show pending + gate state.
    ...
    return None  # not ours â€” caller handles
...
def _set_approval(subsystem: str, rest: List[str], set_mode_fn) -> str:
    ...
    if set_mode_fn is None:
        ...
        return (f"To change the {subsystem} approval gate, run:\n"
                f"  zed config set {subsystem}.write_approval {val}")
    try:
        set_mode_fn(enabled)
    except Exception as e:
