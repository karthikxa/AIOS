"""Kanban decomposer â€” fan a triage task out into a graph of child tasks.

Invoked by ``zed kanban decompose [task_id | --all]`` and the
...
so when the whole graph completes the root wakes back up â€” its
...
* Mirrors the shape of ``zed_cli/kanban_specify.py``: lazy aux
  client import inside the function, lenient response parse, never
  raises on expected failure modes.

* The system prompt sees the *configured* profile roster â€” names plus
...
_SYSTEM_PROMPT = """You are the Kanban decomposer for the Zed Agent board.
...
Rules:
  ...
  - Each child task body is what a fresh worker will read with no other
    context â€” be specific about goal, approach, and acceptance criteria.
...
def _profile_author() -> str:
    """Mirror of ``zed_cli.kanban._profile_author``."""
    return (
...
def _format_roster(roster: list[dict]) -> str:
    if not roster:
        return "  (no profiles installed â€” decomposer cannot route work)"
    lines = []
    for entry in roster:
        tag = "" if entry["has_description"] else " âš  undescribed"
...
def decompose_task(
    ...
    *,
    ...
    fanout=true with empty task list) â€” those surface via ``ok=False``.
    """
    ...
    # task with assignee=None â€” the user explicitly does not want that.
    children: list[dict] = []
    for idx, entry in enumerate(raw_tasks):
        ...
        if (
            ...
            logger.info(
                "decompose: task %s child %d picked unknown assignee %r â€” "
