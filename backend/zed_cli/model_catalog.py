...
The Zed docs site hosts a JSON manifest of curated models for providers
we want to update without shipping a release (currently OpenRouter and
Zed Portal). This module fetches, validates, and caches that manifest,
falling back to the in-repo hardcoded lists when the network is unavailable.

Pipeline
--------
1. ``get_catalog()`` â€” returns a parsed manifest dict.
   - Checks in-process cache (invalidated by TTL).
   - Reads disk cache at ``~/.zed/cache/model_catalog.json``.
   - Fetches the master URL if disk cache is stale or missing.
   - On any fetch failure, keeps using the stale cache (or empty dict).

2. ``get_curated_openrouter_models()`` / ``get_curated_nous_models()`` â€”
...
Unknown fields are ignored â€” extra metadata can be added at either level
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/zedteam/zed-agent/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def _cache_path() -> Path:
    ...
    return get_zed_home() / "cache" / "model_catalog.json"
...
def _validate_manifest(data: Any) -> bool:
    ...
    if not isinstance(version, int) or version > SUPPORTED_SCHEMA_VERSION:
        # Future schema version we don't understand â€” refuse rather than
...
def get_catalog(*, force_refresh: bool = False) -> dict[str, Any]:
    ...
    â€” never raise from this function so the CLI keeps working offline.
    """
    ...
    # Disk is fresh enough â€” use it without a network hit.
...
def get_curated_nous_models() -> list[str] | None:
    """Return Zed Portal's curated list of model ids from the manifest.
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``zed update`` pulls the latest repo, so the freshly-pulled
    ``website/static/api/model-catalog.json`` IS the newest catalog â€” no
    ...
    to ``~/.zed/cache/model_catalog.json`` via the same atomic writer the
    ...
    as non-fatal â€” the network fetch path still applies on the next picker
    open).
    """
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``zed model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
