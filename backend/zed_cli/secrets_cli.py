"""CLI handlers for ``zed secrets bitwarden ...``.

Subcommands:
    setup    â€” interactive wizard: install bws, prompt for token + project, test fetch
    status   â€” show current config + binary version + last fetch outcome
    sync     â€” run a fetch right now and show what would be applied (dry-run friendly)
    disable  â€” flip ``secrets.bitwarden.enabled`` to False
    install  â€” just download the bws binary (no token / project required)
"""
...
# ---------------------------------------------------------------------------
# Argparse wiring â€” called from zed_cli.main
# ---------------------------------------------------------------------------
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    Called from ``zed_cli.main`` as part of building the top-level
    ``zed secrets`` parser.
    """
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        Panel.fit(
            ...
            "  Secrets Manager â†’ Machine accounts â†’ [your account] â†’\n"
            "  Access tokens â†’ Create access token\n\n"
            "Copy the token (starts with [cyan]0.[/cyan]â€¦) â€” it cannot be retrieved later.",
            border_style="cyan",
        )
    )
    ...
    try:
        ...
        if binary is None:
            console.print("  No bws on PATH â€” downloadingâ€¦")
            binary = bw.install_bws()
        version = _bws_version(binary)
        console.print(f"  [green]âœ“[/green] {binary}  ({version})")
    except Exception as exc:  # noqa: BLE001
        console.print(f"  [red]âœ— Could not install bws: {exc}[/red]")
        console.print(
            "  Manual install: "
    ...
    if not token.startswith("0."):
        console.print(
            "  [yellow]Warning: token doesn't start with '0.' â€” usually that means "
    ...
    console.print(f"  [green]âœ“[/green] stored in {get_env_path()} as {token_env}")
    ...
    if server_url is None:
        return 1
    if server_url:
        console.print(f"  [green]âœ“[/green] using {server_url}")
    else:
        console.print(
            "  [green]âœ“[/green] using bws default "
    ...
    if args.project_id and args.project_id.strip():
        ...
        if not projects:
            ...
            console.print(
                "  In the Bitwarden web app, open the machine account â†’ Projects tab "
        ...
        while True:
            ...
            if 1 <= idx <= len(projects):
                ...
                break
            console.print(f"  [red]Out of range â€” pick 1-{len(projects)}.[/red]")
    ...
    try:
        ...
        console.print(f"  [red]âœ— Fetch failed: {exc}[/red]")
        return 1
    ...
    if not secrets:
        ...
        for key in sorted(secrets):
            if key == token_env:
                status = "[dim]bootstrap token â€” never overrides itself[/dim]"
    ...
    console.print()
    console.print(
        "[green]âœ“ Bitwarden Secrets Manager is enabled.[/green]  "
        "Secrets will be pulled at the start of every Zed process."
    )
    console.print(
        "  Status:  [cyan]zed secrets bitwarden status[/cyan]\n"
        "  Refresh: [cyan]zed secrets bitwarden sync[/cyan]\n"
        "  Disable: [cyan]zed secrets bitwarden disable[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]zed secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set â€” Zed will skip BSM "
            "and warn on next startup.[/yellow]"
        )
    if not project_id:
        console.print(
            "\n  [yellow]Enabled but no project_id â€” nothing to fetch.[/yellow]"
        )
    return 0
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`zed secrets bitwarden setup` first.[/yellow]"
        )
        return 1
    ...
    if not args.apply:
        console.print(
            "\n  This was a dry-run â€” secrets are picked up automatically on the "
            "next [cyan]zed[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Zed invocation.\n"
        "  Your access token is left in .env â€” remove it manually if you also want "
...
def cmd_install(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        console.print(f"[green]âœ“[/green] {path}  ({_bws_version(path)})")
        return 0
    except Exception as exc:  # noqa: BLE001
    ...
    if res.returncode != 0:
        ...
        if "invalid_client" in lowered or "400 bad request" in lowered:
            console.print(
                ...
                "[cyan]zed secrets bitwarden setup[/cyan] and pick EU or "
...
# publishes â€” these are stable but if a third region appears, add it here
# and to the prompt below.
_REGION_PRESETS = [
    ("US Cloud  (https://vault.bitwarden.com â€” bws default)", ""),
...
def _resolve_server_url(
    ...
    if env_url:
        console.print(
            f"  Detected [cyan]BWS_SERVER_URL[/cyan]={env_url} in your shell â€” using it."
        )
        return env_url
        ...
        if idx == custom_idx:
            ...
            if not custom.startswith(("http://", "https://")):
                console.print(
                    ...
                    "https:// â€” bws may reject it.[/yellow]"
                )
            return custom
        console.print(f"  [red]Out of range â€” pick 1-{custom_idx}.[/red]")
