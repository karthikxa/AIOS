"""``zed plugins`` CLI subcommand â€” install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.zed/plugins/``.
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other Zed subprocess resolution: :func:`shutil.which` first,
...
def _resolve_git_url(identifier: str) -> tuple[str, Optional[str]]:
    ...
    - Browser URL: https://github.com/owner/repo/tree/main/path
      â†’  (https://github.com/owner/repo.git, "path")
    - Shorthand: owner/repo  â†’  https://github.com/owner/repo.git
    - Shorthand w/ subdir: owner/repo/path/to/plugin
      â†’  (https://github.com/owner/repo.git, "path/to/plugin")
    ...
    if identifier.startswith(("https://", "http://", "git@", "ssh://", "file://")):
        ...
        # Explicit ``#subdir`` fragment â€” unambiguous for any scheme.
        if "#" in identifier:
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.zed/.env``."""
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from zed_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_zed_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]âœ“[/green] Saved to {display_zed_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_zed_home()}/.env later)[/dim]")
...
def _display_removed(name: str, plugins_dir: Path) -> None:
    ...
    console = Console()
    console.print()
    console.print(f"[red]âœ—[/red] Plugin [bold]{name}[/bold] removed from {plugins_dir}")
    console.print()
...
def _install_plugin_core(identifier: str, *, force: bool) -> tuple[Path, dict, str]:
    """Clone Git plugin into ``~/.zed/plugins``.
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update Zed.",
                ) from None
        ...
        if target.exists():
            if not force:
                raise PluginOperationError(
                    ...
                    f"or run `zed plugins update {plugin_name}`.",
                )
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (
        ...
        console.print(
            ...
            f"or __init__.py. It may not be a valid Zed plugin.",
        )
    ...
    if should_enable:
        ...
        console.print(
            f"[green]âœ“[/green] Plugin [bold]{installed_name}[/bold] enabled.",
        )
    else:
        console.print(
            f"[dim]Plugin installed but not enabled. "
            f"Run `zed plugins enable {installed_name}` to activate.[/dim]",
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  zed gateway restart[/dim]")
    console.print()
...
def cmd_update(name: str) -> None:
    ...
    if "Already up to date" in out:
        console.print(
            f"[green]âœ“[/green] Plugin [bold]{name}[/bold] is already up to date."
        )
    else:
        console.print(f"[green]âœ“[/green] Plugin [bold]{name}[/bold] updated.")
...
def _resolve_plugin_key(name: str) -> Optional[str]:
    ...
    This is the single normalization point so ``zed plugins enable`` /
    ``disable`` write the same key that ``PluginManager`` matches against â€”
    nested category plugins (e.g. ``observability/nemo_relay``) included.
    """
    entries = _discover_all_plugins()
    # 1. Exact match on canonical key or manifest name â€” always unambiguous.
...
def cmd_enable(name: str) -> None:
    ...
    console = Console()
    # Discover the plugin â€” check installed (user) AND bundled, including
    # nested category plugins â€” and normalize to its canonical registry key.
    ...
    _save_disabled_set(disabled)
    console.print(
        f"[green]âœ“[/green] Plugin [bold]{key}[/bold] enabled. "
...
def _discover_all_plugins() -> list:
    ...
    every plugin the loader can see â€” user + bundled + project.
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``zed plugins list`` CLI filters."""
    filtered = entries
...
def cmd_list(args: Any | None = None) -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] zed plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Compact view:[/dim] zed plugins list --plain --no-bundled")
    console.print("[dim]Interactive toggle:[/dim] zed plugins")
    console.print("[dim]Enable/disable:[/dim] zed plugins enable/disable <name>")
    console.print("[dim]Plugins are opt-in by default â€” only 'enabled' plugins load.[/dim]")
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as Zed
    ...
    try:
        from zed_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def cmd_toggle() -> None:
    """Interactive composite UI â€” general plugins + provider plugin categories."""
    from rich.console import Console
...
def _run_composite_ui(curses, plugin_names, plugin_labels, plugin_selected,
    ...
    from zed_cli.curses_ui import flush_stdin
    ...
    def _draw(stdscr):
        ...
        while True:
            ...
            try:
                ...
                stdscr.addnstr(
                    1, 0,
                    "  â†‘â†“/j/k navigate  PgUp/PgDn page  SPACE toggle  ENTER configure/confirm  ESC done",
            ...
            if key in {curses.KEY_UP, ord("k")}:
                ...
                if cursor < n_plugins:
                    ...
                    # Provider category â€” launch sub-screen
                    ci = cursor - n_plugins
                ...
                if cursor < n_plugins:
                    # ENTER on a plugin checkbox â€” confirm and exit
                    result_holder["plugins_changed"] = True
                    return
                else:
                    # ENTER on a category â€” same as SPACE, launch sub-screen
                    ci = cursor - n_plugins
                    if 0 <= ci < n_categories:
                        curses.endwin()
                        _cat_name, _cat_cur, cat_fn = categories[ci]
...
def _get_plugin_toolset_key(name: str) -> Optional[str]:
    ...
    Queries the live tool registry â€” the plugin must already be loaded.
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.zed/plugins/<name>`` if it exists."""
    plugins_dir = _plugins_dir()
    try:
        target = _sanitize_plugin_name(name, plugins_dir, allow_subdir=True)
    except ValueError:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.zed/plugins/<name>``."""
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.zed/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def plugins_command(args) -> None:
    """Dispatch zed plugins subcommands."""
