"""
Zed Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.zed/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Zed PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing zed-agent or zed PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Zed Agent" comment and following line
                if '# Zed Agent' in line or '# zed-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('zed' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing zed
                if 'zed' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the zed wrapper script if it exists."""
    wrapper_paths = [
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains zed_cli reference)
                content = wrapper.read_text()
                if 'zed_cli' in content or 'zed-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(zed_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``zed`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Zed home's ``node``
    directory are removed â€” links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (zed_home / "node").resolve()
    removed = []
    ...
    for name in ("node", "npm", "npx"):
        for bin_dir in _node_symlink_candidate_dirs():
            link = bin_dir / name
            try:
                # Only act on symlinks â€” never delete a real binary the user put here.
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``zed gateway run`` processes
    ...
    try:
        from zed_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # Termux/Android has no systemd and no launchd â€” nothing left to do.
    ...
    if system == "Linux":
        try:
            ...
            for is_system in (False, True):
                ...
                try:
                    if is_system and os.geteuid() != 0:  # windows-footgun: ok â€” Linux systemd uninstall path, guarded by `if system == "Linux"` above
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from zed_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ZED_HOME`` and ``ZED_GIT_BASH_PATH``
#      via ``[Environment]::SetEnvironmentVariable(..., "User")``.  These
#      don't live in ~/.bashrc â€” they're in the Windows registry at
#      HKCU\Environment.
#   2. Prepends to User-scope ``PATH`` (same registry location) entries
#      like ``%LOCALAPPDATA%\zed\git\cmd``, ``%LOCALAPPDATA%\zed\git\bin``,
#      ``%LOCALAPPDATA%\zed\git\usr\bin``, ``%LOCALAPPDATA%\zed\node``.
#      Again not in any rc file â€” only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\zed\git\`` and Node to
#      ``%LOCALAPPDATA%\zed\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``zed dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\zed\gateway-service\`` and sometimes
#      ``%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup\`` â€” the
...
# be nicer but requires ctypes and buys us nothing â€” the user will log out
# or open a new terminal anyway).


def _zed_path_markers(zed_home: Path) -> list[str]:
    """Path-entry substrings that identify Zed-owned User-PATH entries."""
    root = str(zed_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare zed-agent install dir.
    markers = [root + "\\zed-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ZED_HOME was customised to somewhere else â€” find-and-nuke
    # any entry whose path component contains "zed".  We don't want to catch
    # unrelated entries like "czed-foo" or "ephermeral", so we look for
    # backslash-zed as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(zed_home: Path) -> list[str]:
    """Strip Zed-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _zed_path_markers(zed_home)
            kept: list[str] = []
...
def remove_zed_env_vars_windows() -> list[str]:
    """Delete ZED_HOME and ZED_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ZED_HOME", "ZED_GIT_BASH_PATH"):
                try:
...
def remove_portable_tooling_windows(zed_home: Path) -> list[Path]:
    """Delete PortableGit and Node installs the Windows installer created under
    ``%LOCALAPPDATA%\\zed\\``.  Only called on full uninstall; they're
...
def _is_default_zed_home(zed_home: Path) -> bool:
    """Return True when ``zed_home`` points at the default (non-profile) root."""
    try:
        from zed_constants import get_default_zed_root
        return zed_home.resolve() == get_default_zed_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ZED_HOME directory.

    We shell out to ``zed -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ZED_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m zed_cli.main` so we don't depend on a `zed`
    #    wrapper that may be half-removed mid-uninstall.
    zed_invocation = [_sys.executable, "-m", "zed_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                zed_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ZED_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``zed uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir â€” nothing under ``$ZED_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from zed_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(zed_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”", Colors.MAGENTA, Colors.BOLD))
    print(color("â”‚         âš• Zed Chat GUI Uninstaller                  â”‚", Colors.MAGENTA, Colors.BOLD))
    print(color("â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Zed Chat GUI installation was found.")
        print(f"  Checked: {zed_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Zed agent stays installed.", Colors.CYAN))
    print()
    ...
    if summary["userdata_exists"]:
        print(f"  â€¢ {summary['userdata_dir']}  (desktop app data)")
    print()
    if agent_is_installed(zed_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  â€¢ The Zed agent at {zed_home / 'zed-agent'}")
        print(f"  â€¢ Your config, sessions, and secrets under {zed_home}")
        print()
    ...
    print()
    uninstall_gui(zed_home)

    print()
    print(color("â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”", Colors.GREEN, Colors.BOLD))
    print(color("â”‚            âœ“ Chat GUI Uninstalled!                      â”‚", Colors.GREEN, Colors.BOLD))
    print(color("â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜", Colors.GREEN, Colors.BOLD))
    print()
    print("The Zed agent is still installed. Run 'zed' to use the CLI,")
    print("or 'zed uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.zed/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.zed/ for future reinstall
    """
    project_root = get_project_root()
    zed_home = get_zed_home()

    # Detect named profiles when uninstalling from the default root â€”
    # offer to clean them up too instead of leaving zombie ZED_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_zed_home(zed_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.zed data); otherwise keep-data. Named profiles
    # are NOT auto-removed here â€” that's a destructive, surprising default for
    ...
    print()
    print(color("â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”", Colors.MAGENTA, Colors.BOLD))
    print(color("â”‚            âš• Zed Agent Uninstaller                  â”‚", Colors.MAGENTA, Colors.BOLD))
    print(color("â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {zed_home / 'config.yaml'}")
    print(f"  Secrets: {zed_home / '.env'}")
    print(f"  Data:    {zed_home / 'cron/'}, {zed_home / 'sessions/'}, {zed_home / 'logs/'}")
    print()
    ...
    if named_profiles:
        ...
        for p in named_profiles:
            ...
            print(f"  â€¢ {p.name}{running}: {p.path}")
        print()
    ...
    # remove any named profiles â€” stopping their gateway services, unlinking
    # their alias wrappers, and wiping their ZED_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("âš ï¸  WARNING: This will permanently delete ALL Zed data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Zed code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    Steps: stop gateway â†’ strip PATH (rc files + Windows registry) â†’ remove the
    ``zed`` wrapper + node symlinks â†’ remove the desktop Chat GUI artifacts â†’
    delete the code checkout â†’ (Windows) remove PortableGit/Node â†’ optionally
    wipe ``$ZED_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in zed_home so the marker matching is
        # against fully resolved paths â€” installer writes literal strings
        # like C:\Users\<u>\AppData\Local\zed\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(zed_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Zed-owned PATH entries in User environment")

        log_info("Removing ZED_HOME / ZED_GIT_BASH_PATH User env vars...")
        removed_env = remove_zed_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Zed-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing zed command...")
    ...
    #     (only when they still point into this Zed home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Zed-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(zed_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Zed-managed node/npm/npx symlinks found")
    ...
    #     code, so the GUI â€” which is just another consumer of the same
    #     checkout â€” should go with it. uninstall_gui() never touches config /
    #     sessions / .env, so it's safe in keep-data mode; on full uninstall the
    #     step-5 rmtree(zed_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE ZED_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from zed_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(zed_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.zed/, just remove the zed-agent subdir
            if zed_home in project_root.parents or project_root.parent == zed_home:
    ...
    #     under ZED_HOME but they're install tooling, not config â€” safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(zed_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(zed_home)
    ...
    # 5. Optionally remove ~/.zed/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ZED_HOME dirs live under
        ...
        try:
            if zed_home.exists():
                shutil.rmtree(zed_home)
                log_success(f"Removed {zed_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {zed_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {zed_home}")
    
    # Done
    print()
    print(color("â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”", Colors.GREEN, Colors.BOLD))
    print(color("â”‚              âœ“ Uninstall Complete!                      â”‚", Colors.GREEN, Colors.BOLD))
    print(color("â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜", Colors.GREEN, Colors.BOLD))
    print()
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://zed-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://zed-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Zed Agent! âš•")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m zed_cli.uninstall --mode <gui|lite|full>``.
    ...
    venv that contains the running ``python.exe`` â€” and a running .exe is
    ...
    so ``import zed_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``zed_constants`` + ``zed_cli.colors``
    (and lazily ``zed_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m zed_cli.uninstall")
    parser.add_argument(
        "--mode",
