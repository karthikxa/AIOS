"""Boundary-aware partial compression â€” "summarize up to here".

Inspired by Claude Code's Rewind menu "Summarize up to here" action
(v2.1.139â€“v2.1.142, Week 20, May 2026):
https://code.claude.com/docs/en/whats-new/2026-w20

Zed already has ``/compress`` (full-history compaction) and an
...
the Claude Code feature â€” the user decides the compression boundary
...
* **Role alternation.** The compressed head ends with summary/handoff
  ...
  history keeps the userâ†”assistant alternation that providers validate.
...
#: into a no-op surprise â€” clamp instead.
MAX_KEEP_LAST = 100
...
def parse_partial_compress_args(
    ...
    * ``here``            â†’ partial compress, keep ``DEFAULT_KEEP_LAST``
    * ``here 4``          â†’ partial compress, keep 4 exchanges
    * ``--keep 4``        â†’ partial compress, keep 4 exchanges
    * ``up to here``      â†’ alias for ``here`` (matches Claude Code's
    ...
    * ``partial`` â€” True when a boundary-aware form was requested.
    * ``keep_last`` â€” exchanges to preserve verbatim (only meaningful
      when ``partial`` is True).
    * ``focus_topic`` â€” focus string for full compression, or None.
...
def split_history_for_partial_compress(
    ...
    userâ†”assistant alternation stays valid (the compressed head's
    ...
    ``(history, [])`` â€” signaling the caller to fall back to full
    ...
    if not user_starts:
        # No user turns at all (degenerate) â€” nothing sensible to keep
...
def rejoin_compressed_head_and_tail(
    ...
    the seam against an illegal userâ†’user / assistantâ†’assistant adjacency.
    ...
    recent window), which terminates on an ``assistant``/``tool`` turn â€”
    so ``assistant â†’ user`` at the seam is already valid. But the head
    ...
    ``tool`` messages are left alone â€” consecutive ``tool`` entries are
