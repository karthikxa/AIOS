"""
Zed Agent â€” Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m zed_cli.main web          # Start on http://127.0.0.1:9119
    python -m zed_cli.main web --port 8080
"""
...
try:
    ...
    # running `zed dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["ZED_WEB_DIST"]) if "ZED_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

# ---------------------------------------------------------------------------
# Per-channel subscriber registry used by /api/pub (PTY-side gateway â†’ dashboard)
# and /api/events (dashboard â†’ browser sidebar).  Keyed by an opaque channel id
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``zed gateway run`` â€” but the
    desktop app spawns a ``zed dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same ZED_HOME â€” whichever process grabs the lock
    first wins the tick.
    """
...
async def _lifespan(app: "FastAPI"):
    ...
    # event loop during lifespan startup â€” see _get_event_state's docstring.
    app.state.chat_argv_lock = asyncio.Lock()

    # Desktop-spawned backends (ZED_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `zed
    # dashboard` is unaffected â€” it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("ZED_DESKTOP") == "1":
...
app = FastAPI(title="Zed Agent", version=__version__, lifespan=_lifespan)

# ---------------------------------------------------------------------------
# Session token for protecting sensitive endpoints (reveal).
# The desktop shell mints the token and injects it via
# ZED_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("ZED_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Zed-Session-Token"

# In-browser Chat tab (/chat, /api/pty, /api/ws, â€¦).  Always enabled: the
...
#
# This list is defined in ``zed_cli.dashboard_auth.public_paths`` so the
# OAuth gate middleware can honour the same allowlist â€” keeping the two
...
#
# Keep the upstream list minimal â€” only truly non-sensitive, read-only
# endpoints belong there.
# ---------------------------------------------------------------------------
from zed_cli.dashboard_auth.public_paths import (
...
# can't be set. Kept narrow â€” same query-token tradeoff as the /api/pty WS.
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Zed-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
    * **Gated / OAuth mode** (``auth_required`` True): ``_SESSION_TOKEN`` is
      ...
      cookie before the request reached this handler â€” any non-public ``/api/``
...
# which resolves to 127.0.0.1 after a TTL flip â€” bypassing same-origin
...
def should_require_auth(host: str, allow_public: bool) -> bool:
    ...
    Truth table:
      host == loopback                              â†’ False (no auth)
      host != loopback AND allow_public (--insecure)â†’ False (legacy escape hatch)
      host != loopback AND NOT allow_public         â†’ True  (gate engages)

    "Loopback" matches the same set used by ``--insecure`` enforcement in
    ``start_server``: 127.0.0.1, localhost, ::1. RFC1918 / CGNAT / link-local
    are deliberately treated as PUBLIC â€” a hostile device on the same LAN is
...
def _is_accepted_host(host_header: str, bound_host: str) -> bool:
    ...
    h = host_header.strip()
    if h.startswith("["):
        # IPv6 bracketed â€” port (if any) follows "]:"
        close = h.find("]")
        if close != -1:
...
async def host_header_middleware(request: Request, call_next):
    ...
    TTL-flips to 127.0.0.1. CORS and same-origin checks don't help â€”
    the browser now treats the attacker origin as same-origin with the
    dashboard. Host-header validation at the app layer catches it.

    See GHSA-ppp5-vxwm-4cf7.
    """
    # Store the bound host on app.state so this middleware can read it â€”
...
# ---------------------------------------------------------------------------
# Dashboard OAuth auth gate â€” engaged only when start_server flags the
...
# auth_middleware so the order is: host check â†’ cookie auth â†’ token auth.
# ---------------------------------------------------------------------------


@app.middleware("http")
async def _dashboard_auth_gate(request: Request, call_next):
    from zed_cli.dashboard_auth.middleware import gated_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "stt.provider": {
        ...
        # "mistral" temporarily removed â€” mistralai PyPI package quarantined
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Zed (no terminal prompt), "
...
_CATEGORY_MERGE: Dict[str, str] = {
    ...
    "onboarding": "agent",
    # Only `telegram.reactions` currently lives under telegram â€” fold it in
...
class ModelAssignment(BaseModel):
    """Payload for POST /api/model/set â€” assign a provider/model to a slot.

    scope="main"        â†’ writes model.provider + model.default
    scope="auxiliary"   â†’ writes auxiliary.<task>.provider + auxiliary.<task>.model
    scope="auxiliary" with task=""  â†’ applied to every auxiliary.* slot
    scope="auxiliary" with task="__reset__"  â†’ resets every slot to provider="auto"
    """
    scope: str
    provider: str
    model: str
    ...
    # providers on the main slot â€” lets the GUI configure a self-hosted endpoint
    # (vLLM, llama.cpp, Ollama, â€¦) that needs no API key. The runtime resolver
    ...
    # endpoint that requires auth works from the GUI â€” mirrors the key the
    # ``zed model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Zed provider slug â€” fine.
    - The per-card "Use as â†’ Main model" menu sends ``entry.provider``
      ...
      ``default: anthropic/claude-opus-4.6`` to config â€” a vendor-prefixed
    ...
    1. Vendor-name â†’ Zed-provider mapping: when the provider string is
       not a known Zed provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    2. Model-format normalization for the resolved provider via
       ...
       on native anthropic â†’ ``claude-opus-4-6``).
    """
    from zed_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from zed_cli.models import _AGGREGATOR_PROVIDERS
    ...
    # Custom/user-config providers keep the model verbatim â€” the registry
...
def _apply_main_model_assignment(
    ...
    - Otherwise, a stale ``base_url`` is cleared ONLY when switching to a
      *different* provider â€” that URL belonged to the old provider. When the
      ...
      alive when they merely re-pick a model under the same provider â€” picking
...
try:
    ...
    _log.warning(
        "Invalid GATEWAY_HEALTH_TIMEOUT value %r â€” using default 3.0s",
...
# don't break.  Do not add new callers â€” wire new uses through the planned
# config surface.
...
def _probe_gateway_health() -> tuple[bool, dict | None]:
    ...
    - ``http://gateway:8642``                (base URL â€” recommended)
    - ``http://gateway:8642/health``         (explicit health path)
    - ``http://gateway:8642/health/detailed`` (explicit detailed path)

    This is a **blocking** call â€” run via ``run_in_executor`` from async code.
    """
...
_MANAGED_FILES_ROOT_ENV = "ZED_DASHBOARD_FILES_ROOT"
...
def _media_serve_roots() -> list[Path]:
    ...
    gateway host â€” its images dir and cache subtree. This stops an authenticated
...
async def get_media(path: str):
    ...
    â€” they can't read the gateway's local disk directly.
...
def _default_zed_root_is_opt_data() -> bool:
    raw = os.environ.get("ZED_HOME", "").strip()
    if not raw:
        return False
    try:
        from zed_constants import get_default_zed_root

        root = get_default_zed_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``zed update``.
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Zed root is actually /opt/data
    # (the container/hosted layout) or when ZED_DASHBOARD_FILES_ROOT is set.
    if _default_zed_root_is_opt_data():
...
async def download_managed_file(request: Request, path: str):
    ...
    managed-files route â€” ``auth_middleware`` additionally accepts the session
...
# atomically renames into place â€” constant memory, no base64 inflation.
...
async def upload_managed_file_stream(
    ...
    try:
        ...
        # BaseException paths the `except` clauses above don't catch â€” most
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_zed_home() at call time
    ...
    try:
        ...
        if not gateway_running and _GATEWAY_HEALTH_URL:
            ...
            if alive:
                ...
                # PID from the remote container (display only â€” not locally valid)
        ...
        # os.kill() probe, so it must only run against the LOCAL status file â€”
        ...
        # and which providers are registered so ``zed status`` and the
        # SPA's StatusPage can show "OAuth gate ON via Zed Team" or
        # "loopback only â€” no auth gate" with no extra round trips.
        auth_required = bool(getattr(app.state, "auth_required", False))
        auth_providers: list[str] = []
        try:
            from zed_cli.dashboard_auth import list_providers as _list_providers
            auth_providers = [p.name for p in _list_providers()]
        except Exception:
            # Module not importable yet (early startup) â€” leave as [].
            pass
        ...
        # bootstrap, and anyone who can curl the host â€” i.e. exactly the audience
        ...
        status = {
            ...
            "can_update_zed": not _dashboard_local_update_managed_externally(),
        ...
        # envelope â€” the same loopback/gated split ``should_require_auth`` draws.
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the zed home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_zed_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
# ---------------------------------------------------------------------------
# Curator endpoints â€” background skill-maintenance status + controls.
#
...
async def run_curator():
    ...
    try:
        proc = _spawn_zed_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from zed_cli.auth import get_nous_auth_status
...
async def run_prompt_size():
    try:
        proc = _spawn_zed_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_zed_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_zed_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
class DebugShareRequest(BaseModel):
    # Redaction is ON by default â€” force-mode scrubs credential-shaped tokens
...
# streamed to a per-action log file under ``~/.zed/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_zed_home() / "logs"

# Short ``name`` (from the URL) â†’ absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "gateway-start": "gateway-start.log",
    "gateway-stop": "gateway-stop.log",
    "zed-update": "zed-update.log",
...
# ``name`` â†’ most recently spawned Popen handle.  Used so ``status`` can
# report liveness and exit code without shelling out to ``ps``.
_ACTION_PROCS: Dict[str, subprocess.Popen] = {}
_ACTION_COMMANDS: Dict[str, Tuple[str, ...]] = {}

# ``name`` â†’ completed synthetic action result for actions the server handled
...
def _spawn_zed_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``zed <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``zed_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "zed_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "ZED_NONINTERACTIVE": "1"},
    }
    ...
    # parent's handle can be released immediately â€” otherwise we leak one
...
def _tail_lines(path: Path, n: int) -> List[str]:
    """Return the last ``n`` lines of ``path``.  Reads the whole file â€” fine
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["zed", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``zed gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``zed gateway restart`` children race each other on the
    ...
    return _spawn_zed_action(subcommand, "gateway-restart"), False
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``zed gateway restart`` in the background."""
    try:
...
@app.post("/api/zed/update")
async def update_zed():
    """Kick off ``zed update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Zed updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("zed-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "zed-update",
    ...
    try:
        proc = _spawn_zed_action(["update"], "zed-update")
    except Exception as exc:
        _log.exception("Failed to spawn zed update")
...
def _recent_upstream_commits(n: int = 20) -> List[Dict[str, Any]]:
    ...
    Logs the SAME range the behind-count uses (``HEAD..origin/main`` â€” see
...
@app.get("/api/zed/update/check")
async def check_zed_update(force: bool = False):
    """Report whether a Zed update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/zed/update`` actually runs ``zed update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Zed version string
        ...
        commits: for git/pip installs that are behind, a list of the commits
                 the local checkout is behind upstream by â€” each
    ...
    try:
        ...
        if force:
            try:
                (get_zed_home() / ".update_check").unlink()
            except OSError:
                pass
    ...
    if behind is None:
        payload["message"] = "Couldn't reach the update source â€” try again later."
    elif behind == 0:
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.zed/config.yaml`` under ``tts.``.
    """
...
async def get_profiles_sessions(
    ...
    from disk â€” it does NOT spawn a dashboard backend per profile. Each returned
    ...
    # the cron-jobs section passes source=cron â€” two independent lists so
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Zed surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Zed supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _memory_provider_config_path(provider: MemoryProvider) -> Path:
    return get_zed_home() / provider.name / "config.json"
...
# ---------------------------------------------------------------------------
# Model assignment â€” pick provider+model for main slot or auxiliary slots.
...
# in zed_cli/config.py â€” listed here for deterministic ordering in the UI.
...
def get_model_options(profile: Optional[str] = None, refresh: bool = False):
    ...
    re-fetches its live catalog â€” used by the picker's explicit "Refresh
    ...
    try:
        ...
        # surface fed by this endpoint (Settings â†’ Model, the first-run
        # onboarding picker) sees the SAME full provider universe `zed model`
        # exposes â€” not just the authenticated subset. Unconfigured providers
...
def get_recommended_default_model(provider: str = ""):
    ...
    if slug == "nous":
        try:
            ...
            from zed_cli.auth import get_provider_auth_state
...
def get_auxiliary_models(profile: Optional[str] = None):
    ...
    ``profile`` scopes the read â€” without it, the Models page would show
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.zed/config.yaml`` â€” applies to **new** sessions only.
...
def _apply_model_assignment_sync(
    ...
    HTTPException for validation errors â€” the async wrapper re-raises them.
    """
    cfg = load_config()
    ...
    if scope == "main":
        ...
        if provider.strip().lower() == "nous":
            try:
                from zed_cli.nous_subscription import apply_nous_managed_defaults
        ...
        # endpoint, mirroring the ``zed model`` custom flow
        # (_save_custom_provider). Without this the endpoint only lives in
        # ``model.*`` and the picker has no proper ready row for it â€” the
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from zed_cli.main import _auto_provider_name, _save_custom_provider
                ...
                # Never block the assignment on the bookkeeping write â€”
        ...
        # (they're independent, sticky per-task overrides â€” see
        ...
        # auto-clear them â€” pinning aux to a cheaper/different model is a
        # legitimate config â€” but we tell the caller so the UI can offer a
    ...
    if task == "__reset__":
        # Reset every slot to provider="auto", model="" â€” keeps other fields intact.
        for slot in _AUX_TASK_SLOTS:
...
def _catalog_provider_env_metadata() -> dict:
    """Map provider env vars â†’ desktop card metadata, derived from the catalog.
    ...
    (i.e. the ``zed model`` universe). This is what lets the desktop Keys tab
    render a card for a provider even when its env var was never hand-added to
    ``OPTIONAL_ENV_VARS`` â€” closing the drift where CLI-configurable providers
    (openai-api, kilocode, novita, tencent-tokenhub, copilot, â€¦) were missing
    from the GUI.
    ...
    try:
        from zed_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock â€” a `zed model` provider â€” would be
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict) -> dict:
        ...
        return {
            ...
            # CLI `zed model` picker uses (not desktop-only prefix guesses).
    ...
    # OPTIONAL_ENV_VARS â€” these are the providers that were CLI-configurable but
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    try:
        ...
        # on the denylist (LD_PRELOAD, PATH, PYTHONPATH, â€¦). Surface the
...
# read-only models/key call that 401s on a bad token â€” enough to catch a
...
async def validate_provider_credential(body: EnvVarUpdate, request: Request):
    ...
    # Local / custom endpoint: validate connectivity, not auth â€” any HTTP
    ...
    if not probe:
        # No probe for this provider â€” can't validate, don't block.
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Zed from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Zed to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Zed from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Zed to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Zed in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Zed through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Zed via Home Assistant.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Zed to DingTalk groups (é’‰é’‰).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Zed inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Zed through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Zed to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (å…ƒå®)",
        "description": "Connect Zed to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "HASS_TOKEN": {
        "description": "Long-lived access token from Home Assistant (Profile â†’ Security)",
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in zed gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in zed gateway setup",
...
# Cross-cutting gateway / relay knobs stay on the Keys â†’ Settings tab even though
# they use the ``messaging`` category in OPTIONAL_ENV_VARS. Platform-scoped vars
# (``DISCORD_*``, ``MATRIX_*``, â€¦) are owned by the Messaging UI instead.
...
def _messaging_platform_payload(
    ...
    for key in entry["env_vars"]:
        # When profile-scoped, judge only the profile's own .env â€” the
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Zed Agent").strip() or "Zed Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Zed appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's ZED_HOME.
...
def _truncate_token(value: Optional[str], visible: int = 6) -> str:
    ...
    bearer provider) â€” the callable is NEVER invoked here.
    """
    if not value:
        return ""
    if callable(value) and not isinstance(value, str):
        # Entra ID bearer provider â€” never reveal a minted token in the UI.
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.zed/.anthropic_oauth.json`` â€” Zed-managed PKCE flow (what
       this entry's Connect button writes)
    2. ``ANTHROPIC_API_KEY`` â†’ ``ANTHROPIC_TOKEN`` â†’ ``CLAUDE_CODE_OAUTH_TOKEN``
       env vars (registry order) â€” from ``.env``, the shell, or an external
       secret source like Bitwarden (whose keys are injected into the process
       env during ``load_zed_dotenv()``, so the same check covers them)

    Claude Code's ``~/.claude/.credentials.json`` is deliberately NOT read
    here â€” it has its own dedicated catalog entry (``claude-code`` â†’
    ...
    try:
        from agent.anthropic_adapter import (
            read_zed_oauth_credentials,
            _ZED_OAUTH_FILE,
        )
    except ImportError:
        read_zed_oauth_credentials = None  # type: ignore
        _ZED_OAUTH_FILE = None  # type: ignore

    zed_creds = None
    if read_zed_oauth_credentials:
        try:
            zed_creds = read_zed_oauth_credentials()
        except Exception:
            zed_creds = None
    if zed_creds and zed_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "zed_pkce",
            "source_label": f"Zed PKCE ({_ZED_OAUTH_FILE})",
            "token_preview": _truncate_token(zed_creds.get("accessToken")),
            "expires_at": zed_creds.get("expiresAt"),
            "has_refresh_token": bool(zed_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Zed even
    when they also have a separate Zed-managed PKCE login.
    """
    try:
...
def _gemini_cli_status() -> Dict[str, Any]:
    ...
    try:
        from zed_cli import auth as hauth
...
def _copilot_acp_status() -> Dict[str, Any]:
    """Status for copilot-acp â€” credentials are owned by the Copilot CLI.
    ...
    Zed never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "zed auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "zed auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "zed auth add minimax-oauth",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "zed auth add google-gemini-cli",
    ...
    # â”€â”€ Anthropic / Claude entries sit at the bottom: the API-key path
    # first, then the subscription OAuth path (which only works with extra
    # usage credits on top of a Claude Max plan â€” see disclaimer in name).
    {
        "id": "anthropic",
        "name": "Anthropic API Key",
        "flow": "pkce",
        "cli_command": "zed auth add anthropic",
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Zed, so the disconnect
    ...
    instead hand the GUI a command it can *run in the embedded terminal* â€” the
    user sees exactly what executes, and Zed then stops resolving the token.
    ...
    file â€” the two sources ``read_claude_code_credentials()`` consults. Returns
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Zed â€” run the disconnect command to remove it."
        return "Managed by that provider's CLI; remove it there."
    if status.get("source") == "env_var":
        return "Remove the API key from Settings â†’ Keys instead."
    return None
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      1. ``_OAUTH_PROVIDER_CATALOG`` â€” the explicit, hand-tuned cards that carry
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``zed model`` universe) â€” so any OAuth/external provider added as a
    ...
    # 2. Catalog accounts-providers not already covered â€” keeps the Accounts tab
    #    in lockstep with the `zed model` universe (zero-edit for new plugins).
    try:
        from zed_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"zed auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("zed_pkce", "claude_code", ...)
    ...
    sync with the `zed model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Zed-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _ZED_OAUTH_FILE
                if _ZED_OAUTH_FILE.exists():
                    _ZED_OAUTH_FILE.unlink()
                    cleared = True
            except Exception:
                pass
            # Also clear the credential pool entry if present.
            try:
                from zed_cli.auth import clear_provider_auth
        ...
        try:
            from zed_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          â†’ persists to ~/.zed/.anthropic_oauth.json AND credential pool
...
#          â†’ server binds a 127.0.0.1 callback listener, builds the xAI
#            authorize URL, spawns a background worker waiting on the redirect
#          â†’ returns { session_id, flow: "loopback", auth_url, expires_in }
#     2. UI opens auth_url in the browser. There is NO user_code/code to
#        paste â€” the redirect lands back on the loopback listener.
#     3. UI polls GET /api/providers/oauth/{provider}/poll/{session_id}
...
# Guarded so zed web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Zed file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``zed auth add anthropic``.
    """
    from agent.anthropic_adapter import _ZED_OAUTH_FILE
    payload = {
    ...
    _ZED_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _ZED_OAUTH_FILE.with_name(
        f"{_ZED_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _ZED_OAUTH_FILE)
        try:
            _ZED_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
async def _start_device_code_flow(
    ...
    if provider_id == "minimax-oauth":
        ...
        sess["region"] = "global"
        # `expired_in` from MiniMax is overloaded â€” could be a unix-ms
...
# the tokens exactly like `zed auth add xai-oauth`.
...
def _start_xai_loopback_flow(profile: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # Binding succeeded but URL construction failed â€” release the socket
...
def _xai_loopback_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = hauth._xai_validate_inference_base_url(
            os.getenv("ZED_XAI_BASE_URL", "").strip().rstrip("/")
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `zed auth add xai-oauth`'s credential-pool insert.
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from zed_cli.auth import persist_nous_credentials
...
def _minimax_poller(session_id: str) -> None:
    ...
    and persists via ``_minimax_save_auth_state`` â€” so the dashboard
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from zed_cli.auth import _save_codex_tokens
...
async def poll_oauth_session(
    ...
    """Poll a session's status (no auth â€” read-only state).
...
async def cancel_oauth_session(
    ...
    if sess.get("flow") == "loopback":
        ...
        # until the timeout â€” otherwise repeated cancel/retry piles up daemon
...
# CRITICAL â€” every literal-path route below MUST be declared BEFORE the
...
# pattern is unconstrained â€” it would otherwise swallow e.g.
...
async def bulk_delete_sessions_endpoint(body: BulkDeleteSessions):
    ...
    of IDs â€” Starlette accepts both, but POSTing a list keeps proxies,
    ...
    * Unknown IDs are silently skipped (the response ``deleted`` count
      ...
      deliberate â€” UI selection state can race against another tab's
    ...
    * Active and archived sessions ARE deleted when explicitly
      selected â€” unlike ``DELETE /api/sessions/empty``, the user
    ...
    * Like the other session-delete endpoints, this does NOT pass a
    ...
    delete (unknown IDs are silently skipped â€” see contract above)
...
async def count_empty_sessions_endpoint(profile: Optional[str] = None):
    ...
    Drives the dashboard's "Delete empty (N)" button â€” when N is 0 the
...
async def delete_empty_sessions_endpoint(profile: Optional[str] = None):
    ...
    * Archived sessions are skipped â€” the user explicitly chose to
      keep those rows.
    * Children of deleted parents are orphaned, not cascade-deleted.

    Like the single-session ``DELETE /api/sessions/{id}`` endpoint
    below, this doesn't pass a ``sessions_dir`` through â€” the on-disk
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `zed sessions stats`).
...
def _open_session_db_for_profile(profile: Optional[str]):
    ...
    ``profile`` None/empty â†’ this process's own ``state.db`` (the common,
...
async def delete_session_endpoint(session_id: str, profile: Optional[str] = None):
    ...
    # opening its state.db directly. Remote profiles never reach here â€” the
    ...
    try:
        ...
        # on any error â€” so a 404 on an already-absent row resurrected a ghost
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `zed sessions prune`)."""
    if body.older_than_days < 1:
        raise HTTPException(status_code=400, detail="older_than_days must be >= 1")
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_zed_home()
    db = _open_session_db_for_profile(body.profile)
    try:
...
async def get_logs(
    ...
    from zed_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_zed_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from zed_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, ZED_HOME)."""
    from zed_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process ZED_HOME at import time. The dashboard is a single
...
async def list_cron_job_runs(job_id: str, profile: Optional[str] = None, limit: int = 20):
    ...
    Backed by ``SessionDB.list_cron_job_runs`` â€” a bounded ``[prefix, hi)``
...
async def get_cron_delivery_targets():
    ...
    ``cron.scheduler.cron_delivery_targets()`` â€” no hardcoded platform list. A
...
def _fire_cron_job_for_profile(profile: str, job_id: str) -> bool:
    ...
    the shared lock â€” same mechanism as ``_call_cron_for_profile`` â€” so the
...
async def cron_fire_webhook(request: Request):
    ...
    Chronos fire-verifier), NOT the dashboard session cookie â€” so this path is
    ...
    if not profile:
        # Job is gone (cancelled / completed) â€” nothing to fire. 200 so NAS
...
# ---------------------------------------------------------------------------
# Automation Blueprints â€” parameterized automation blueprints. The dashboard renders the
...
async def instantiate_blueprint(body: AutomationBlueprintInstantiate, profile: str = "default"):
    ...
    try:
        ...
        try:
            ...
            # Field-level validation error â€” 422 so the form can show it inline.
...
# ---------------------------------------------------------------------------
# MCP server endpoints â€” list / add / remove / test.
#
# Wraps the same config data layer the CLI uses (zed_cli.mcp_config), so
# servers managed here show up under `zed mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from zed_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from zed_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from zed_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from zed_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # resolution during the probe â€” env-placeholder expansion in
        # _resolve_mcp_server_config reading the profile's .env â€” sees the
        ...
        # ZED_HOME override (see mcp_tool._wrap_with_home_override), so
        # OAuth token stores resolve against the selected profile as well.
        with _profile_scope(profile):
            return _probe_single_server(name, servers[name])

    try:
        # Probe blocks on a dedicated MCP event loop â€” run in a thread so the
...
async def set_mcp_server_enabled(
    ...
    Toggles the ``enabled`` key on the server's config.yaml entry â€” the same
    ...
    try:
        ...
        for entry in catalog_entries:
            ...
            entries.append({
                ...
                # installing â€” surface them rather than hiding them in the repo.
        ...
        # Unknown/invalid profile â†’ 404, not a silently-empty catalog.
        raise
    except Exception:
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # Git-bootstrap entries can take a while to clone â€” run via the background
    # action path so the request returns immediately and the UI can tail logs.
    # The -p subprocess rebinds ZED_HOME-derived paths in the child.
    if entry.install is not None:
        try:
            proc = _spawn_zed_action(
    ...
    # No git step â€” install synchronously via the catalog API. install_entry
    ...
    # to_thread the other way around â€” asyncio.to_thread copies context, so
...
# ---------------------------------------------------------------------------
# Pairing endpoints â€” approve / revoke / list messaging pairing codes.
#
# These are how a remote admin onboards messaging users (Telegram, Discord, â€¦)
...
# ---------------------------------------------------------------------------
# Webhook subscription endpoints â€” list / subscribe / remove.
#
# Wraps the same JSON store the CLI uses (zed_cli.webhook); the webhook
...
async def create_webhook(body: WebhookCreate):
    ...
    if body.deliver_only and body.deliver == "log":
        raise HTTPException(
            status_code=400,
            detail="Direct delivery requires a real target (telegram, discord, â€¦), not 'log'.",
        )
...
# ---------------------------------------------------------------------------
# Gateway lifecycle endpoints â€” start / stop.
#
...
# spawn the real `zed gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_zed_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_zed_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
# ---------------------------------------------------------------------------
# Credential pool endpoints â€” list / add / remove rotation keys.
#
...
async def list_credential_pool():
    ...
    from zed_cli.auth import read_credential_pool
...
# ---------------------------------------------------------------------------
# Memory provider endpoints â€” status / list providers / select / disable / reset.
#
...
# `zed memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `zed memory setup` to configure a new one.",
            )
...
# ---------------------------------------------------------------------------
# Operations endpoints â€” doctor / security audit / backup / import /
# checkpoints / hooks.
#
...
# /api/actions/{name}/status â€” same pattern as gateway restart and update.
...
async def run_doctor():
    try:
        proc = _spawn_zed_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_zed_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_zed_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `zed import`. The spawned action runs with
    # stdin=DEVNULL, so the CLI's interactive "Continue? [y/N]" overwrite
    # prompt hits EOF and auto-aborts ("Aborted.", exit 1) whenever the
    # target already has a config â€” which it always does when the dashboard
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_zed_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    try:
        from zed_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <zed_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_zed_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_zed_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
# ---------------------------------------------------------------------------
# Skills hub endpoints â€” search / install / uninstall / update.
#
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``zed`` subprocess, and
    ``_apply_profile_override()`` reads ``-p`` from argv in the child â€” the
...
async def list_skills_hub_sources(profile: Optional[str] = None):
    ...
    Gives the dashboard something to show BEFORE a search runs â€” which hubs
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from zed_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
async def scan_skill_hub(identifier: str = ""):
    ...
    `should_allow_install` pipeline the CLI installer uses â€” then cleans up the
...
# ---------------------------------------------------------------------------
# Profile management endpoints (minimal â€” list/create/rename/delete + SOUL.md)
# ---------------------------------------------------------------------------
...
class ProfileCreate(BaseModel):
    ...
    # Profile-builder additions â€” all optional, all applied best-effort AFTER
    ...
    # via a subprocess scoped to the profile (`zed -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the ZED_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_zed_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "zed setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local ZED_HOME override so the write lands in the target
    ...
    token = set_zed_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local ZED_HOME override (same mechanism as
    ...
    """
    from zed_constants import set_zed_home_override, reset_zed_home_override
    from zed_cli.mcp_security import validate_mcp_server_entry
    ...
    try:
        ...
        for server in servers:
            ...
            if not entry:
                # Nothing usable to write (neither url nor command) â€” skip
        ...
        if written:
            ...
            # We created an empty mcp_servers dict but wrote nothing â€” don't
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    Profiles manage skill activation via a *disabled* list â€” all installed
    ...
    install.) Scoped to the profile via the ZED_HOME override. Returns the
    number of skills newly disabled.
    """
    from zed_constants import set_zed_home_override, reset_zed_home_override
    from zed_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # whole create â€” the user can set the model later from the Models page
    # or `<profile> setup`.
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
    model_set = False
    ...
    # Optional "keep" skill selection â€” replace semantics. When the builder
    ...
    # profile's ZED_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``zed profile use`` â€”
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from ZED_HOME).
    """
    from zed_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``zed profile use``).

    Note: this does not retarget the already-running dashboard process â€”
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the ZED_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``zed profile describe
    <name> --auto``.

    A failed generation (no aux client, LLM error, â€¦) is returned as
...
# toggles would land in the activated profile â€” they silently wrote into the
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_zed_home()`` at call
       time â€” the context-local override from ``set_zed_home_override``
    ...
    ``profile`` of None/""/"current" means "the dashboard's own profile" â€”
    config resolution is untouched, but the skill-module globals are still
    retargeted to the *current* ``get_zed_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a ZED_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_zed_home_override`` contextvar â€” it does NOT swap the
    ...
    which is all endpoints that resolve ``get_zed_home()`` at call time
    (config, env, gateway status) actually need.

    None/""/"current" means the dashboard's own profile â€” no override.
    """
    requested = (profile or "").strip()
    if not requested or requested.lower() == "current":
        yield None
...
async def create_skill(body: SkillCreate):
    ...
    optional security scan) â€” but bypasses the agent write-approval gate:
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``zed tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``zed tools`` picker shows
...
async def select_toolset_provider(
    ...
    Delegates to ``apply_provider_selection`` â€” the shared, non-interactive
    core extracted from the CLI configurator â€” so the GUI and ``zed tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.zed/.env`` via ``save_env_value`` â€”
    the same store ``zed tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``zed tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``zed -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same ZED_HOME the rest of the
    drawer's writes targeted â€” so the scope is threaded for consistency.
    """
    from zed_cli.tools_config import (
        _get_effective_configurable_toolsets,
...
async def get_config_raw(profile: Optional[str] = None):
    ...
    shows the file the switched profile actually reads/writes â€” /api/status's
...
async def get_models_analytics(days: int = 30, profile: Optional[str] = None):
    ...
    try:
        ...
        # Models page used to show a duplicate "0 tokens / â€” API calls" card
...
# ---------------------------------------------------------------------------
# /api/pty â€” PTY-over-WebSocket bridge for the dashboard "Chat" tab.
#
# The endpoint spawns the same ``zed --tui`` binary the CLI uses, behind
...
# REST.  Localhost-only â€” we defensively reject non-loopback clients even
...
# expose the same public surface â€” spawn/read/write/resize/close/is_available â€”
# so the /api/pty WebSocket handler needs no platform guards.
if sys.platform.startswith("win"):
    try:
        from zed_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from zed_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_client_is_allowed(ws: "WebSocket") -> bool:
    ...
    Loopback bind: only loopback clients allowed â€” the legacy
    ...
    requires the Host header to match the bound interface â€” the same
    defence ``_is_accepted_host`` applies to non-loopback HTTP requests.

    Gated mode: any peer is allowed â€” uvicorn's ``proxy_headers=True``
...
def _ws_host_origin_reason(ws: "WebSocket") -> Optional[str]:
    ...
    machine-parseable token (``host_mismatch â€¦`` / ``origin_mismatch â€¦``)
...
def _ws_auth_mode() -> str:
    """Short label for the active WS auth mode â€” logged on every connection."""
    if getattr(app.state, "auth_required", False):
        return "gated"
    bound_host = (getattr(app.state, "bound_host", "") or "").strip().lower()
    if bound_host and bound_host not in _LOOPBACK_HOSTS:
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    Gated (public bind, no ``--insecure``): one of two credentials â€”

    * ``?ticket=<single-use>`` â€” a browser-minted, single-use, 30s-TTL ticket
      consumed against the dashboard-auth ticket store. This is what the SPA
      (and native clients) use.
    * ``?internal=<process-credential>`` â€” the process-lifetime internal
      ...
      injected into the SPA â€” see ``dashboard_auth.ws_tickets`` for the
      threat model.
    ...
    auth_required = bool(getattr(app.state, "auth_required", False))
    if auth_required:
        # Lazy import â€” keeps this function importable in test harnesses
        # that don't bring in the dashboard_auth layer.
        from zed_cli.dashboard_auth.audit import AuditEvent, audit_log
        ...
        # Server-spawned children (PTY child â†’ /api/ws, /api/pub) present the
...
# startup â€” see _get_event_state / _get_chat_argv_lock above.)
...
def _resolve_chat_argv(
    ...
    Default: whatever ``zed --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf â€¦'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``ZED_TUI_RESUME`` env var â€”
    matching what ``zed_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``ZED_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``ZED_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `profile` (when set) scopes the ENTIRE chat to that profile by pointing
    ``ZED_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_zed_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    â€” the same propagation ``zed -p <name>`` performs. The in-process
    ``ZED_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from zed_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("ZED_TUI_DISABLE_MOUSE", "1")
    env.setdefault("ZED_TUI_INLINE", "1")
    env["ZED_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["ZED_HOME"] = str(profile_dir)

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["ZED_TUI_RESUME"] = resume

    if sidecar_url:
        env["ZED_TUI_SIDECAR_URL"] = sidecar_url

    # Profile-scoped chats must NOT attach to the dashboard's in-memory
    # gateway â€” it runs under the dashboard's own profile. Without the
    # attach URL, gatewayClient spawns its own `tui_gateway.entry`, which
    # inherits the profile ZED_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["ZED_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from zed_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _build_sidecar_url(channel: str) -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        # Gated mode â€” use the internal credential so the WS upgrade survives
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Zed inside WSL2 to use the dashboard's /chat "
            "tab â€” the rest of the dashboard works here.\x1b[0m\r\n"
        )
    ...
    # --- reader task: PTY master â†’ WebSocket ----------------------------
    ...
    # --- writer loop: WebSocket â†’ PTY master ----------------------------
    try:
        while True:
...
# ---------------------------------------------------------------------------
# /api/ws â€” JSON-RPC WebSocket sidecar for the dashboard "Chat" tab.
#
...
# ---------------------------------------------------------------------------
# /api/pub + /api/events â€” chat-tab event broadcast.
#
# The PTY-side ``tui_gateway.entry`` opens /api/pub at startup (driven by
# ZED_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
async def events_ws(ws: WebSocket) -> None:
    ...
    try:
        while True:
            # Subscribers don't speak â€” the receive() just blocks until
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`zed_cli.dashboard_auth.prefix.normalise_prefix`
    â€” the single source of truth lives in the dashboard_auth package so
    the gate middleware, the OAuth routes, the cookie helpers, and the
    SPA mount all agree on validation rules.
    """
    from zed_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/zed/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /zed`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__ZED_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/zed``)
        or empty string when served at root.

        When the OAuth auth gate is active (``app.state.auth_required``),
        the legacy ``_SESSION_TOKEN`` is NOT injected â€” the SPA reads
        identity from ``/api/auth/me`` over cookie auth instead.  The
        ``__ZED_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__ZED_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ZED_BASE_PATH__="{prefix}";'
                f"window.__ZED_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__ZED_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__ZED_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ZED_BASE_PATH__="{prefix}";'
                f"window.__ZED_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/zed`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Zed backend. Intercept CSS asset
...
# Built-in dashboard themes â€” label + description only.  The actual color
...
_BUILTIN_DASHBOARD_THEMES = [
    ...
    {"name": "cyberpunk", "label": "Cyberpunk",      "description": "Neon green on black â€” matrix terminal"},
    {"name": "rose",      "label": "RosÃ©",           "description": "Soft pink and warm ivory â€” easy on the eyes"},
]
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # Color overrides â€” keep only valid keys with string values.
    ...
    # Assets â€” named slots + arbitrary user-defined keys.  Values must be
    ...
    # Custom CSS â€” raw CSS text the frontend injects as a scoped <style>
    # tag on theme apply.  Clipped to _THEME_CUSTOM_CSS_MAX to keep the
    # payload bounded.  We intentionally do NOT parse/sanitise the CSS
    # here â€” the dashboard is localhost-only and themes are user-authored
    # YAML in ~/.zed/, same trust level as the config file itself.
    custom_css_val = data.get("customCSS")
    custom_css: Optional[str] = None
    if isinstance(custom_css_val, str) and custom_css_val.strip():
        custom_css = custom_css_val[:_THEME_CUSTOM_CSS_MAX]

    # Component style overrides â€” per-bucket dicts of camelCase CSS
...
def _discover_user_themes() -> list:
    """Scan ~/.zed/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_zed_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.zed/dashboard-themes/*.yaml` ship with their full
...
# web/src/themes/fonts.ts â€” the frontend owns the stacks + webfont URLs;
...
def _safe_plugin_api_relpath(api_field: Any, *, dashboard_dir: Path) -> Optional[str]:
    ...
    execution by design â€” that's how plugins extend the backend).
    Pre-#29156 the field was used as-is, which meant:

    * An absolute path swallowed the plugin's dashboard directory
      entirely â€” ``Path('safe/dashboard') / '/tmp/evil.py'`` resolves
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as zed_cli.plugins):
    1. User plugins:    ~/.zed/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.zed/plugins/  (only if ZED_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from zed_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_zed_home() / "plugins", "user"),
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
    ]
    # GHSA-5qr3-c538-wm9j (#29156): the previous ``os.environ.get(...)``
    # check treated *any* non-empty string as truthy, so ``=0``, ``=false``,
    # and ``=no`` â€” all of which the agent loader and operators correctly
    # read as "disabled" â€” silently *enabled* the untrusted project source
    ...
    # ``zed_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("ZED_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".zed" / "plugins", "project"))
    ...
    for plugins_root, source in search_dirs:
        ...
        for child in sorted(plugins_root.iterdir()):
            ...
            try:
                ...
                # any absolute path or ``..`` traversal here â€” the
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_zed_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from zed_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from zed_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from zed_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from zed_cli.plugins_cmd import dashboard_remove_user_plugin
...
async def serve_plugin_asset(plugin_name: str, file_path: str):
    ...
    auth header â€” so this route stays unauthenticated to keep the SPA
    ...
    # files, ``.env.example`` templates, etc. â€” none of which the SPA
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.zed/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"zed_dashboard_plugin_{plugin['name']}"
...
# always mounted â€” the gate middleware decides whether to enforce auth,
# not whether the routes exist.
from zed_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    â€” used when a profile alias (``<profile> dashboard``) routes to the
    machine dashboard.
    """
    import uvicorn
    ...
    if app.state.auth_required:
        ...
        from zed_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing ZED_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Zed Agent exposes a
            ...
            try:
                ...
                if _nous_plugin.LAST_SKIP_REASON:
                    skip_reasons.append(
                        f"  â€¢ nous: {_nous_plugin.LAST_SKIP_REASON}"
                    )
            except Exception:
                pass
            ...
            if skip_reasons:
                raise SystemExit(
                    f"Refusing to bind dashboard to {host} â€” the OAuth auth "
        ...
        # --insecure path â€” no auth, loud warning.
        _log.warning(
            "Binding to %s with --insecure â€” the dashboard has no robust "
    ...
    # â”€â”€ Start uvicorn with direct Server API â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # bound â€” we read the OS-assigned port from the live socket, print
    # ZED_DASHBOARD_READY, open the browser, *then* serve.
    #
    # This eliminates the TOCTOU of the old pre-bind-then-close approach
    # (bind port 0 â†’ close â†’ uvicorn rebind): the socket is held by
    ...
    # OSError inside create_server() and exits with a clear error â€” no
    ...
    config = uvicorn.Config(
        ...
        # disconnectâ†’reap path.  20s stays under Cloudflare Tunnel's idle
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"ZED_DASHBOARD_READY port={actual_port}", flush=True)
            print(f"  Zed Web UI â†’ http://{host}:{actual_port}")
