...
- ``zed_cli.providers``        -- canonical provider identity + overlays
- ``zed_cli.model_normalize``  -- per-provider name formatting

Provider switching uses the ``--provider`` flag exclusively.
No colon-based ``provider:model`` syntax â€” colons are reserved for
...
_ZED_MODEL_WARNING = (
    "Zed Team Zed 3 & 4 models are NOT agentic and are not designed "
    "for use with Zed Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Zed Team Zed 3 / Zed 4 chat families.
# The previous substring check (`"zed" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``zed-brain:qwen3-14b-ctx16k`` that just
# happen to carry "zed" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Zed-3-Llama-3.1-70B, zed-4-405b, openrouter/zed3:70b
# Negative examples it must NOT match:
#   zed-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_ZED_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])zed[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_zed_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Zed 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_ZED_NON_AGENTIC_RE.search(model_name))


def _check_zed_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Zed 3/4 chat model."""
    if is_nous_zed_non_agentic(model_name):
        return _ZED_MODEL_WARNING
    return ""
...
# ---------------------------------------------------------------------------
# Direct aliases â€” exact model+provider+base_url for endpoints that aren't
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``zed config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from zed_cli.model_switch import
    DIRECT_ALIASES` references valid in callers â€” rebinding would leave them
...
def resolve_persist_behavior(is_global: bool, is_session: bool) -> bool:
    ...
    1. ``--session`` explicitly opts out â†’ ``False`` (this session only).
    2. ``--global`` explicitly opts in â†’ ``True``.
    3. Otherwise defer to ``model.persist_switch_by_default`` in
       ``config.yaml`` (defaults to ``True``, so a plain ``/model <name>``
       survives across sessions â€” the behavior users expect).
...
def _model_sort_key(model_id: str, prefix: str) -> tuple:
    ...
    Examples (with prefix ``"mimo"``)::
        ...
        mimo-v2.5-pro   â†’ (-2.5, 0, 'pro')     # highest version wins
        mimo-v2.5       â†’ (-2.5, 1, '')          # no suffix = lower than pro
        mimo-v2-pro     â†’ (-2.0, 0, 'pro')
        mimo-v2-omni    â†’ (-2.0, 1, 'omni')
        mimo-v2-flash   â†’ (-2.0, 1, 'flash')
    """
    ...
    # "v2.5-pro" â†’ version [2.5], suffix "pro"
    # "-omni"    â†’ version [],    suffix "omni"
    # State machine: start â†’ in_version â†’ between â†’ in_suffix
    nums: list[float] = []
    suffix_buf = ""
    state = "start"
    num_buf = ""
    ...
    for ch in rest:
        if state == "start":
            ...
            if ch.isdigit():
                ...
                if "." in num_buf:
                    # Second dot â€” flush current number, start new component
                    try:
    ...
    # Flush remaining buffer (strip trailing dots â€” "5.4." â†’ "5.4")
    ...
    # Negate versions so higher â†’ sorts first
...
def resolve_alias(
    ...
    # Sort by version descending â€” prefer the latest/highest version
...
def get_authenticated_provider_slugs(
    ...
    in-memory cache (1 hr TTL) â€” no extra network cost.
    """
    try:
...
def resolve_display_context_length(
    ...
    honored â€” this closes #15779 where ``/model`` switch ignored user-set
    overrides.
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'zed model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'zed doctor' â€” config issues detected:"
        ...
        from zed_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from zed_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
    # PATH B: No explicit provider â€” resolve from model input
    # =================================================================
    else:
        ...
        if alias_result is not None:
            ...
            if key in MODEL_ALIASES:
                ...
                # Only convert when there's no slash â€” a slash means the name
        ...
        # model â€” if so, step e must not second-guess and switch providers.
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from zed_cli.providers import resolve_user_provider as _ruser
                ...
                if slug == target_provider:
                    ...
                    if isinstance(cfg_models, list):
                        if any(m.get("name") == new_model for m in cfg_models if isinstance(m, dict)):
                            override = True
                            break
        # Also check custom_providers list â€” models declared there should be accepted
    ...
    # Mirrors the same logic in zed_cli.runtime_provider.resolve_runtime_provider;
    ...
    zed_warn = _check_zed_model_warning(new_model)
    if zed_warn:
        warnings.append(zed_warn)
...
# process â€” mirrors run_agent's _openrouter_prewarm_done. Without a guard a
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    Fully exception-isolated â€” a slow or offline provider can never affect the
    ...
    def _warm() -> None:
        try:
            from zed_cli.inventory import load_picker_context
            ...
            # Best-effort warmup â€” never surface errors into the session.
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from zed_cli/models.py (OPENROUTER_MODELS,
    _PROVIDER_MODELS) â€” NOT the full models.dev catalog.  These are hand-picked
    agentic models that work well as agent backends.

    Returns a list of dicts, each with:
      - slug: str â€” the --provider value to use
      - name: str â€” display name
      - is_current: bool
      - is_user_defined: bool
      - models: list[str] â€” curated model IDs (up to max_models)
      - total_models: int â€” total curated count
      - source: str â€” "built-in", "models.dev", "user-config"
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from zed_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by zed provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://zed-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Zed release. Falls back to the in-repo
    ...
    # LM Studio has no static catalog â€” probe its native /api/v1/models
    ...
    # --- 1. Check Zed-mapped providers ---
    from zed_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from zed_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for zed_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        # Skip vendor names that are merely aliases routing through an
        # aggregator (e.g. bare "openai" â†’ "openrouter"). These are NOT
        ...
        # resolve_provider_full() to the aggregator (OpenRouter) â€” silently
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(zed_id)
        if (
            _alias_target
            and _alias_target != zed_id
            and _alias_target in _AGG_PROVIDERS
        ):
            continue
        # Skip aliases that map to the same models.dev provider (e.g.
        # kimi-coding and kimi-coding-cn both â†’ kimi-for-coding).
        ...
        # Prefer auth.py PROVIDER_REGISTRY for env var names â€” it's our
        # source of truth.  models.dev can have wrong mappings (e.g.
        # minimax-cn â†’ MINIMAX_API_KEY instead of MINIMAX_CN_API_KEY).
        pconfig = PROVIDER_REGISTRY.get(zed_id)
        # Skip non-API-key auth providers here â€” they are handled in
        # section 2 (ZED_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from zed_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(zed_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `zed model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(zed_id)
        if not model_ids:
            model_ids = curated.get(zed_id, [])
            if zed_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(zed_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Zed-only providers (nous, openai-codex, copilot, opencode-go) ---
    from zed_cli.providers import ZED_OVERLAYS
    from zed_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID â†’ Zed provider ID.
    # ZED_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Zed IDs ("copilot").
    _mdev_to_zed = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ZED_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Zed slug â€” e.g. "github-copilot" â†’ "copilot"
        zed_slug = _mdev_to_zed.get(pid, pid)
        if zed_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(zed_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, zed_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or zed_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(zed_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", zed_slug, exc)
        ...
        # But the /model picker is discovery-oriented â€” we WANT to show
        # providers the user can switch to, even if they aren't currently
        # configured.
        if not has_creds and zed_slug == "anthropic":
            try:
                ...
                if (zed_creds and zed_creds.get("accessToken")) or \
        ...
        if zed_slug in {"openai-codex", "copilot", "copilot-acp"}:
            # Use live OAuth-backed discovery so the gateway /model picker
            # matches what the user's authenticated Codex/Copilot backend
            # actually serves â€” including ChatGPT-Pro-only Codex slugs
            ...
            model_ids = cached_provider_model_ids(zed_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(zed_slug)
                model_ids = _ids if _ids else (curated.get(zed_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(zed_slug, []) or curated.get(pid, [])
        elif zed_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `zed model` picker deliberately shows ONLY the curated agentic
            # list â€” augmented with the Portal's free/paid recommendations so
            # newly-launched models surface without a CLI release â€” in curated
            ...
            try:
                ...
                from zed_cli.auth import get_provider_auth_state as _nous_state
                ...
                # Portal recommendation fetch failed â€” fall back to the
                # curated list alone (still correct, just may lag newly
                # launched models, exactly like an offline CLI run).
                pass
        else:
            # Unified pathway â€” see Section 1 rationale. Fall back to the
            # curated dict (with models.dev merge for preferred providers)
            # when the live fetcher comes up empty.
            model_ids = cached_provider_model_ids(zed_slug)
            if not model_ids:
                model_ids = curated.get(zed_slug, []) or curated.get(pid, [])
                if zed_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(zed_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models] if max_models is not None else model_ids

        results.append({
            "slug": zed_slug,
            "name": get_label(zed_slug),
            "is_current": zed_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "zed",
        ...
        seen_slugs.add(zed_slug.lower())
        _record_builtin_endpoint(zed_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or ZED_OVERLAYS (keeps /model in sync
    # with `zed model`).
    try:
        from zed_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    for _cp in _canon_provs:
        ...
        # Special case: aws_sdk auth (bedrock) â€” no API key env vars,
        ...
        if _cp_config and getattr(_cp_config, "auth_type", "") == "aws_sdk":
            ...
            # Unified pathway â€” same as sections 1 and 2.
    ...
    # merges ``providers:`` into the list) â€” without this, the same endpoint
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Zed's canonical write key (matches
            ...
            # Zed writes ``models:`` as a dict keyed by model id
            # (see zed_cli/main.py::_save_custom_provider); older
            ...
            # explicit models: dict â€” avoid a misleading zero count in /model.
            if not models_list:
            ...
            # - Without an api_key but with explicit models: skip â€” the user
    ...
    # names ("Ollama â€” GLM 5.1", "Ollama â€” Qwen3-coder", ...) appear as one
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        # slug: names frequently differ per model ("Ollama â€” X") while the
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                # Strip per-model suffix so "Ollama â€” GLM 5.1" becomes
                # "Ollama" for the grouped row. Em dash is the convention
                # Zed's own writer uses; a hyphen variant is accepted
            ...
            # active model. Zed's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, zed_cli/config.py) already consume that dict.
        ...
        for grp in groups.values():
            ...
            # endpoints with the same cleaned name â€” e.g. two OpenAI-
            ...
            # ``providers:`` dict key â€” matches on (display_name, base_url).
            ...
            # auth wiring, and canonical slug â€” keep it and hide the shadow.
            ...
            # Ollama servers) â€” the /models endpoint often works without
            ...
            #   endpoint and live /models is the source of truth â€” replace
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`zed_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from zed_cli.models import fetch_openrouter_models
