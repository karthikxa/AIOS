...
prefix (e.g. ``mission-control.tilos.com/zed/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /zed`` so the backend can
reconstruct prefixed URLs (Location: headers, OAuth redirect_uri,
cookie Path attributes, SPA asset URLs).

This module is also the home of the ``ZED_DASHBOARD_PUBLIC_URL`` /
``dashboard.public_url`` resolution â€” when the operator declares a
...
# rather than try to sanitise â€” the operator can fix their config.
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    missing scheme (``zed.example.com`` instead of
    ``https://zed.example.com``) â€” the single most common cause of
    "I set ZED_DASHBOARD_PUBLIC_URL but the OAuth callback is still
    ...
    if not cleaned:
        return  # empty/unset is a legitimate "no override" â€” not malformed
    key = (source, cleaned)
    ...
    _log.warning(
        ...
        "absolute URL â€” it must include an http:// or https:// scheme "
        ...
        source,
        cleaned,
        cleaned.split("://")[-1] or "zed.example.com",
    )
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/zed"`` (no trailing slash) or ``""``
...
# ---------------------------------------------------------------------------
# ZED_DASHBOARD_PUBLIC_URL / dashboard.public_url
# ---------------------------------------------------------------------------
...
def _normalise_public_url(raw: Optional[str]) -> str:
    ...
    must treat ``""`` as "fall back to request reconstruction" â€” never
    ...
    # parse â€” urlparse is permissive enough to accept some hostile
...
def resolve_public_url() -> str:
    ...
    Precedence (mirrors ``dashboard.oauth.client_id``):
      ...
      1. ``ZED_DASHBOARD_PUBLIC_URL`` env var (when non-empty after
         strip â€” empty values are treated as unset so a provisioned-but-
         not-populated Fly secret can't shadow a valid config.yaml entry).
      2. ``dashboard.public_url`` in ``config.yaml``.
      3. Empty string â€” signals "no override, reconstruct from request"
         to the caller.
    ...
    """
    env_raw = os.environ.get("ZED_DASHBOARD_PUBLIC_URL", "")
    env_clean = _normalise_public_url(env_raw)
    if env_clean:
        return env_clean
    _warn_if_malformed("ZED_DASHBOARD_PUBLIC_URL env var", env_raw)
