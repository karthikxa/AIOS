...
Visual styling mirrors the Zed Team design system (the
...
``tests/zed_cli/test_dashboard_auth_401_reauth.py``.
"""
...
# bundle, which we deliberately do NOT load here â€” the login page must
...
<title>Sign in â€” Zed Agent</title>
<style>
  /* Brand fonts shipped by @nous-research/ui â€” same files the SPA loads. */
  @font-face {{
  ...
  *, *::before, *::after {{ box-sizing: border-box; }}
  ...
  /* Subtle dot-grid backdrop â€” DS idiom (see `.dither` in globals.css). */
  body {{
    background-image:
      radial-gradient(
        ellipse at top,
  ...
  /* Brand wordmark above the card â€” same uppercase + wide-tracking
  ...
  .card {{
    ...
    /* Hairline highlight + bevel shadow â€” matches DS Button SHADOW_DEFAULT
  ...
  /* Provider button â€” mirrors DS Button (default variant):
  ...
  .provider-btn {{
    ...
    border: 0;
    border-radius: 0;  /* DS Button is squared â€” no rounded corners. */
    cursor: pointer;
    box-shadow:
  ...
  /* Password provider form â€” same visual language as the OAuth buttons:
  ...
  /* Selection â€” DS uses midground bg + background text. */
  ::selection {{
    background: var(--midground);
    color: var(--background-base);
  }}
...
<main>
  ...
  <div class="card">
    <h1>Sign in</h1>
    <p class="subtitle">Choose a sign-in method to continue to the Zed Agent dashboard.</p>
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Sign-in unavailable â€” Zed Agent</title>
<style>
  @font-face {
    font-family: 'Collapse';
    font-style: normal;
...
#
# Plain string (NOT run through ``str.format``), so braces are literal â€”
...
def render_login_html(*, next_path: str = "") -> str:
    ...
    ``next_path`` â€” when set, the post-login landing path the user
