...
from zed_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
from zed_cli.dashboard_auth.cookies import read_session_cookies
...
# ``path.startswith(prefix)`` â€” so ``/assets/`` (with trailing slash)
...
def _path_is_public(path: str) -> bool:
    ...
    * :data:`PUBLIC_API_PATHS` â€” the shared ``/api/*`` allowlist that
    ...
    * :data:`_GATE_PUBLIC_PREFIXES` â€” auth-bootstrap routes and static
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    """API routes â†’ 401 JSON with ``login_url``; HTML routes â†’ 302 â†’ /login.
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /zed``, the
    ``login_url`` is prefixed (``/zed/login?next=...``) so the
...
def _safe_next_target(request: Request) -> str:
    ...
    string return means the caller produces a bare ``/login`` URL â€” fine,
    user lands at the dashboard root after re-auth.
    """
    path = request.url.path
    # Reject anything that doesn't start with "/" or starts with "//"
    # (protocol-relative URL â€” would open-redirect to an attacker host).
    if not path or not path.startswith("/") or path.startswith("//"):
        return ""
    # Don't redirect back to the auth routes themselves â€” that loops.
    if any(
    ...
    # the SPA's own ``sessionStorage["zed.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    if not at and not _rt:
        # Neither token present â€” no session at all. Nothing to verify or
        # refresh; force login.
    ...
    # lets multiple providers stack â€” the first one that recognises a
    # token wins.
    #
    ...
    # sends only ``zed_session_rt``. If we bailed on ``not at`` here we'd
    # bounce the user to /login on every expiry despite holding a perfectly
    # good refresh token â€” defeating the whole transparent-refresh feature.
    session = None
    if at:
        ...
        # multiple providers stacked, that MUST NOT abort the chain â€” the
        ...
        # tries to reach Zed Portal's JWKS; if that's unreachable it raises,
        ...
        # 503 â€” distinguishing "transient IDP outage" (don't force re-login)
        ...
        if session is None and unreachable_provider is not None:
            ...
            # reached â€” treat as a transient outage rather than forcing a
    ...
    if session is None:
        ...
        # cycle with cookies â†’ middleware at module load. Pass the active
        # prefix so the deletion's Path matches the set-Path (otherwise
        # the browser ignores it).
        from zed_cli.dashboard_auth.cookies import clear_session_cookies
        from zed_cli.dashboard_auth.prefix import prefix_from_request
...
def _attempt_refresh(request: Request, *, refresh_token):
    ...
    ``RefreshExpiredError`` (dead/revoked/reuse-detected RT â†’ force re-login).

    A ``ProviderError`` (Portal unreachable) is NOT swallowed into a re-login
    here â€” re-raising would 500 the request; instead we log and return None so
    ...
    for provider in list_providers():
        try:
            ...
            # This provider owns the RT but it's dead â€” stop trying others
