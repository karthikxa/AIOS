...
The routes:
  ...
  GET  /login              â†’ server-rendered login page
  GET  /auth/login?provider=N â†’ 302 to IDP, sets PKCE cookie
  GET  /auth/callback?code,state â†’ completes login, sets session cookies
  POST /auth/logout        â†’ clears cookies, best-effort revoke
  GET  /api/auth/providers â†’ list registered providers (login bootstrap)
  GET  /api/auth/me        â†’ current Session as JSON (auth-required)
"""
...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``ZED_DASHBOARD_PUBLIC_URL`` env var or
         ``dashboard.public_url`` in config.yaml â€” when set, this is
         ...
         the public URL â€” we no longer need to guess from proxy headers,
      ...
      2. ``X-Forwarded-Prefix: /zed`` (Mission Control deploys) â€” we
         prepend the prefix to the path FastAPI's ``url_for`` produces
         (it doesn't natively honour this header â€” it isn't part of the
         Starlette/uvicorn proxy_headers set).

      3. Bare ``request.url_for("auth_callback")`` â€” under uvicorn's
    ...
    from zed_cli.dashboard_auth.prefix import (
        prefix_from_request,
        resolve_public_url,
    )
...
def _prefix(request: Request) -> str:
    ...
    ``zed_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
    from zed_cli.dashboard_auth.prefix import prefix_from_request
...
async def login_page(request: Request) -> HTMLResponse:
    ...
    # callback applies (defence in depth â€” the gate already filters,
...
async def auth_login(request: Request, provider: str, next: str = ""):
    ...
    pkce = ls.cookie_payload.get("zed_session_pkce", "")
    ...
    # query-string transport would lose the value â€” the cookie is the
...
async def auth_callback(
    ...
    # Parse ``provider=...;state=...;verifier=...;next=...`` â€” the
    ...
    # cookie. We re-validate against the same-origin rules here â€” the
...
def _validate_post_login_target(raw: str) -> str:
    ...
    The ``next`` query param survives a full OAuth round trip â€” the gate
    ...
    # API endpoint renders raw JSON in the browser address bar â€” never
...
# proxy the IP is the proxy's unless X-Forwarded-For is set â€” which is why
...
def _password_rate_limited(ip: str) -> bool:
    ...
    discernible client) shares a single bucket â€” fail-safe toward
...
async def auth_password_login(request: Request, body: _PasswordLoginBody):
    ...
    session cookies and returns JSON ``{"ok": true, "next": <path>}`` â€”
    ...
    as a username oracle or a provider-enumeration oracle:
      * unknown provider / provider lacks password support â†’ 404
      * bad credentials â†’ 401 ("Invalid credentials")
      * backing store unreachable â†’ 503
      * too many attempts from this IP â†’ 429
    """
    ip = _client_ip(request)
    ...
    if p is None or not getattr(p, "supports_password", False):
        # Don't leak which providers exist or which support passwords â€”
    ...
    try:
        ...
        # Generic message â€” never distinguish unknown-user from wrong-password.
        raise HTTPException(status_code=401, detail="Invalid credentials")
    except NotImplementedError:
        # supports_password was True but the method isn't actually
        # implemented â€” a provider bug, not a client error.
...
async def auth_logout(request: Request):
    ...
    if rt:
        ...
        for provider in list_providers():
            ...
            except Exception as e:  # noqa: BLE001 â€” best-effort
                _log.warning(
...
async def api_auth_ws_ticket(request: Request):
    ...
    from zed_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
