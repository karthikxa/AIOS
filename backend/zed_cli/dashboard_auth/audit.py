...
Profile-aware location: ``$ZED_HOME/logs/dashboard-auth.log``.
...
This module deliberately keeps a minimal dependency surface â€” no imports
from ``zed_constants`` or other zed_cli modules â€” so it can be
...
def _resolve_log_path() -> Path:
    """``$ZED_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``zed_constants.get_zed_home`` semantics: env var wins,
    else ``~/.zed``. A local copy avoids an import cycle with the
    middleware which lives below ``zed_cli``.
    """
    home = os.environ.get("ZED_HOME") or str(Path.home() / ".zed")
...
def audit_log(event: AuditEvent, **fields: Any) -> None:
    ...
    Write failures are logged at WARNING but never raise â€” auth must not
