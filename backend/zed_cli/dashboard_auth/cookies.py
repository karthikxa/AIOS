...
Three cookies in play:
  - zed_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - zed_session_rt:   the OAuth refresh token
                         (HttpOnly, lifetime = 24h, ROTATING + reuse-detected)
                         Zed Portal issues a rotating refresh token for the
                         dashboard auth-code grant (Portal NAS #293 / zed
                         ...
                         degrades gracefully to access-token-only sessions â€”
                         the RT cookie is simply not written.
  - zed_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
ONLY when the dashboard was reached over HTTPS â€” detected via the
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Loopback HTTP â€” bare name. ``__Host-`` / ``__Secure-`` require
    ``Secure``, which is incompatible with HTTP.
  * Gated HTTPS, direct deploy (Path=/) â€” ``__Host-`` prefix. Binds the
    cookie to the exact origin (no Domain attribute) â€” strongest spec
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/zed) â€”
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/zed`` covers
    same-origin app isolation.

The setters and readers BOTH consult the active prefix because the
cookie *name* changes â€” a reader that looked up the bare name when the
setter wrote ``__Secure-zed_session_at`` would never find the value.
...
Refresh-token handling:
   ...
   live RT â†’ rotate server-side, else 401 â†’ /login") lives in
...
# Bare cookie names â€” the request-scoped ``_resolved_name`` helper
# decides whether to prepend ``__Host-`` / ``__Secure-`` based on the
# request's HTTPS + prefix combination.
SESSION_AT_COOKIE = "zed_session_at"
SESSION_RT_COOKIE = "zed_session_rt"
PKCE_COOKIE = "zed_session_pkce"
...
# practice â€” a single request emits exactly one variant).
...
# real authority â€” once the RT itself expires/rotates out, a refresh attempt
# returns 400 â†’ RefreshExpiredError â†’ clean re-login, regardless of how long
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /zed`` we want ``Path=/zed`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    ``refresh_token`` is written as the RT cookie when non-empty. Zed Portal
    issues a 24h rotating refresh token (zed #37247); a provider that
    omits it returns ``Session.refresh_token == ""`` and we simply don't
    persist the RT cookie â€” the session then behaves as access-token-only
    until the AT expires. No other branch changes between the two cases.

    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/zed``)
...
def detect_https(request: Request) -> bool:
    ...
    Reads ``request.url.scheme`` â€” under uvicorn's ``proxy_headers=True``
