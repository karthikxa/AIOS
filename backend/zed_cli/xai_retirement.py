...
Pure logic: walks a Zed config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies â€” testable in isolation
and reusable from both `zed doctor` and a future `zed migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Zed config."""
...
def _normalize(model_id: str) -> str:
    """Strip provider prefix (``x-ai/grok-4`` â†’ ``grok-4``) and lowercase."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Zed config and return retirement issues.

    Slots scanned:
      - ``principal.model``
      - ``auxiliary.<any>.model`` (introspective â€” covers future aux slots)
      - ``delegation.model``
      - ``tts.xai.model``
...
def format_issue(issue: RetirementIssue) -> str:
    ...
    parts = [
        f"{issue.config_path}: {issue.current_model!r} â†’ use {issue.replacement!r}"
    ]
...
def apply_migration(
    ...
    unless ``backup=False``.
    """
    from ruamel.yaml import YAML  # local import â€” avoid hard dep at module load
    ...
    for issue in issues:
        try:
            ...
            # Slot vanished between scan and apply â€” skip silently
            continue
