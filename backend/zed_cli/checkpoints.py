"""`zed checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.zed/checkpoints/``.  Actions:

    zed checkpoints               # same as `status`
    zed checkpoints status        # total size, project count, breakdown
    zed checkpoints list          # per-project checkpoint counts + workdir
    zed checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    zed checkpoints clear [-f]    # nuke the entire base (asks first)
    zed checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    zed checkpoints
    zed checkpoints prune --retention-days 3 --max-size-mb 200
    zed checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: zed checkpoints clear-legacy")
    return 0


def cmd_list(args: argparse.Namespace) -> int:
    # `list` is just a terser status â€” already covered.
...
def cmd_prune(args: argparse.Namespace) -> int:
    ...
    print("Pruning checkpoint storeâ€¦")
...
def cmd_clear(args: argparse.Namespace) -> int:
    ...
    if info["total_size_bytes"] == 0 and not Path(CHECKPOINT_BASE).exists():
        print("Nothing to clear â€” checkpoint base does not exist.")
        return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``zed checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `zed checkpoints` â†’ status
