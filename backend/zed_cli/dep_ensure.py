...
Detection and prompting live here in Python â€” not in install.sh â€” because:
...
zypper/Termux/â€¦).  Reimplementing that in Python would be huge duplication.

Deps that degrade gracefully (ripgrep â†’ grep fallback, ffmpeg â†’ skip conversion)
don't need ensure_dependency wired in â€” only hard-fail sites do (TUI needs node,
...
def _find_install_script(
    ...
    """Locate the install script â€” bundled in wheel or in git checkout.
...
def ensure_dependency(
    ...
    if check is None:
        # Unknown dep â€” don't silently forward to install script.
        return False
    if check():
        return True
