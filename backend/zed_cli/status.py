"""
Status command for zed CLI.

Shows the status of all Zed Agent components.
"""

import os
import sys
import subprocess  # noqa: F401 â€” re-exported for tests that monkeypatch status.subprocess to guard against regressions
...
from zed_cli.config import get_env_path, get_env_value, get_zed_home, load_config
...
from zed_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``zed config``'s
    output (previously this variant was missing the DIM color â€”
...
def show_status(args):
    """Show status of all Zed Agent components."""
    ...
    print(color("â”‚                 âš• Zed Agent Status                  â”‚", Colors.CYAN))
    ...
    keys: dict[str, str | tuple[str, ...]] = {
        ...
        "Browser Use": "BROWSER_USE_API_KEY",  # Optional â€” local browser works without this
        "Browserbase": "BROWSERBASE_API_KEY",  # Optional â€” direct credentials only
        "FAL": "FAL_KEY",
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: zed model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: zed auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: zed auth add xai-oauth)'}"
    )
    ...
    # =========================================================================
    print()
    print(color("â—† API-Key Providers", Colors.CYAN, Colors.BOLD))
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: zed model)"
        print(f"  {pname:<16} {check_mark(configured)} {label}")

    # LM Studio reachability â€” only probe when it's the active provider so
    ...
    if _effective_provider_label() == "LM Studio":
        from zed_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
        ...
        try:
            ...
            ok, msg = False, "auth rejected â€” set LM_API_KEY"
    ...
    try:
        from zed_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_zed_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_zed_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'zed doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'zed setup' to configure", Colors.DIM))
    print()
