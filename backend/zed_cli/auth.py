...
from zed_cli.config import get_zed_home, get_config_path, read_raw_config
from zed_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
# plugins/model-providers/<name>/ plugin â€” no edits to this file required.
try:
    ...
    for _pp in _list_providers_for_registry():
        ...
        # the registry â€” adding them here breaks runtime_provider resolution
...
# "/v1/messages" internally â€” so "/coding" + SDK suffix â†’ "/coding/v1/messages"
...
def _resolve_kimi_base_url(api_key: str, default_url: str, env_override: str) -> str:
    ...
    if env_override:
        return env_override
    # No key â†’ nothing to infer from.  Return default without inspecting.
    if not api_key:
        return default_url
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from zed_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.zed/.env file
...
# Each entry lists candidate models to try in order â€” newer coding plan accounts
...
def _resolve_zai_base_url(api_key: str, default_url: str, env_override: str) -> str:
    ...
    # No API key set â†’ don't probe (would fire NÃ—M HTTPS requests with an
    ...
    # at all â€” the caller discards the result immediately, so the probe is
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    These failures are transient â€” re-authenticating cannot resolve them â€” so
    callers should surface a "retry later" notice and prefer a fallback chain
    instead of prompting the operator to run ``zed auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    # Rate-limit / quota errors are not credential problems â€” never append the
    # "re-authenticate" remediation, which would mislead the operator.
    if is_rate_limited_auth_error(error):
        return str(error)

    if error.relogin_required:
        return f"{error} Run `zed model` to re-authenticate."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("ZED_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store â€” persistence layer for ~/.zed/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_zed_home() / "auth.json"
    # Seat belt: if pytest is running and ZED_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch ZED_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".zed" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set ZED_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom ZED_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_zed_root()
    except Exception:
        return None
    profile_home = get_zed_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.zed/auth.json`` even when ZED_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_zed_root()`` for a profile-shaped ZED_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".zed" / "auth.json"
            try:
...
def _file_lock(
    ...
    state â€” that would let one lock's reentrant acquisition silently skip
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            pass
        logger.warning(
            "auth: failed to parse %s (%s) â€” starting with empty store. "
...
def _save_auth_store(auth_store: Dict[str, Any], target_path: Optional[Path] = None) -> Path:
    ...
    # specific store â€” e.g. the global-root write-through for rotating xAI
    # OAuth grants (#43589) â€” reusing this function's atomic O_EXCL + 0o600
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global scope. Once the user runs ``zed auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``zed auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``auth.json`` are used as a read-only fallback â€” so workers spawned in a
    ...
    ``zed auth add <provider>`` inside the profile, profile entries
    ...
    # Profile has no entries for this provider â€” fall back to global.
...
def get_provider_auth_state(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global-root ``auth.json`` per-provider when the profile has no entry â€”
    ...
    are unchanged â€” they still target the profile only. This mirrors
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Zed.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `zed logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from zed_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected â€” run 'zed doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'zed model' for available providers, or run 'zed doctor' to diagnose config issues."
    ...
    # Auto-detect an OpenRouter credential added via `zed auth add openrouter`
    # (manual pool entry, no env var). Without this, a key that only lives in
    # the credential pool is invisible to auto-detection â€” the user sees
    # `zed auth list` showing the credential while requests go out with no
    ...
    # AWS Bedrock â€” detect via boto3 credential chain (IAM roles, SSO, env vars).
    ...
    try:
        ...
        pass  # boto3 not installed â€” skip Bedrock auto-detection

    raise AuthError(
        "No inference provider configured. Run 'zed model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.zed/.env.",
...
# (NOUS_INFERENCE_BASE_URL) bypass validation â€” that's the documented
...
def _validate_nous_inference_url_from_network(url: Optional[str]) -> Optional[str]:
    ...
    unexpected host â€” letting the caller fall back to the configured
    ...
    subsequent proxy request â€” bearing the user's inference JWT â€” to an
    ...
    this â€” env values come from the trusted OS user, not from the
...
def _save_qwen_cli_tokens(tokens: Dict[str, Any]) -> Path:
    ...
    # Create with 0o600 atomically via os.open(O_EXCL) â€” closes the TOCTOU
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("ZED_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `zed model` walks users into a broken Qwen setup flow.
...
# =============================================================================
# Google Gemini OAuth (google-gemini-cli) â€” PKCE flow + Cloud Code Assist.
#
# Tokens live in ~/.zed/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `zed auth list` / `zed status`."""
    try:
...
# Spotify auth â€” PKCE tokens stored in ~/.zed/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("ZED_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set ZED_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("ZED_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("ZED_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("ZED_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # Diagnostic logging â€” emits at INFO so reporters of loopback bugs
            # (#27385 â€” "callback received but Zed times out") can produce
            # actionable evidence without a code change.  Logged values are
            # fingerprints / booleans only; no actual code/state strings leak
            # into the log file.  Run with ``ZED_LOG_LEVEL=INFO`` (or check
            # ``~/.zed/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
            ...
            # Show an explicit "not received" page rather than the success page â€”
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>zed auth add xai-oauth</code> to retry.</p>"
                    "</body></html>"
                )
                self.wfile.write(body.encode("utf-8"))
                return
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `zed auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `zed auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `zed auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `zed auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.zed/.env, and return it.
    ...
    print("       App name:     anything (e.g. zed-agent)")
    ...
    # Persist so subsequent `zed auth spotify` runs skip the wizard.
    save_env_value("ZED_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("ZED_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved ZED_SPOTIFY_CLIENT_ID to ~/.zed/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "ZED_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Zed:")
    print(authorize_url)
    print()
...
def _is_remote_session() -> bool:
    ...
    **browser-only remote consoles** (GCP Cloud Shell, GitHub
    ...
    the exact same problem â€” the user has a browser on their laptop
...
# the terminal.  Opening one of these is worse than not opening anything â€”
...
_CONSOLE_BROWSER_NAMES: FrozenSet[str] = frozenset(
    {
        ...
        "elinks",
        "www-browser",
        "browsh",  # TUI browser â€” still hijacks the terminal
    }
)
...
def _can_open_graphical_browser() -> bool:
    ...
    Heuristics:
      * Respect ``$BROWSER`` â€” if it names a known console browser, refuse.
    ...
    try:
        ...
        # No browser resolvable at all â†’ definitely don't auto-open.
        return False
...
def _parse_pasted_callback(raw: str) -> dict:
    ...
    * bare code (no state, only used when the upstream omits state):
    ...
    #26923 â€” formalises the curl-the-callback-URL workaround the
...
def _prompt_manual_callback_paste(redirect_uri: str) -> dict:
    ...
    """
    print()
    print("â”€â”€â”€ Manual callback paste â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€")
    ...
    print("not on your laptop) â€” that is expected.  Copy the FULL URL")
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print()
    print(divider)
    print("Remote session detected â€” SSH tunnel required")
    print(divider)
    print(f"Zed is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth â€” tokens stored in ~/.zed/auth.json (not ~/.codex/)
#
# Zed maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Zed auth store (~/.zed/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `zed auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `zed auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `zed auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `zed auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    refresh token plus any stale error markers â€” so the next request spends a
    dead token and gets a 401 ``token_invalidated``.

    What gets refreshed:

    * ``device_code`` â€” the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``zed setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` â€” entries created by ``zed auth add openai-codex``
      ...
      ``zed auth add openai-codex`` and must not be overwritten by a
    ...
    * ``manual:api_key`` and any other non-device-code manual sources â€” those
    ...
    * ``manual:device_code`` entries whose access_token does NOT match the
      previous singleton â€” see above; these are independent accounts.
    ...
    if not isinstance(entries, list):
        return
    # Previous singleton access_token (before this re-auth overwrote it) â€”
    ...
    for entry in entries:
        ...
        if source == "device_code":
            # Singleton-seeded mirror â€” always refresh.
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Zed auth store (~/.zed/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        # ``zed auth add openai-codex`` created (which must not be
        # overwritten â€” see #39236).
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Zed auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Zed auth state."""
    ...
    if response.status_code == 429:
        ...
        # The stored refresh token is still valid here â€” re-authenticating
        ...
        # "run zed auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `zed auth` to re-authenticate."
            )
            relogin_required = True
        # A 401/403 from the token endpoint always means the refresh token
        # is invalid/expired â€” force relogin even if the body error code
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Zed auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. Zed keeps its OWN
        ...
        # the Codex CLI (or another Zed process) rotates the shared token,
        ...
        # keep relogin_required=False â€” the stored token is still valid there, so
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Zed's own Codex token store.
    ...
    fallback, a user whose tokens live only in the pool â€” for example after a manual
    pool seed, a partial re-auth, or pool-only restoration from a backup â€” gets a bare
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("ZED_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("ZED_CODEX_REFRESH_TIMEOUT_SECONDS", 20)

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Zed processes
...
# =============================================================================
# xAI Grok OAuth â€” tokens stored in ~/.zed/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `zed model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `zed model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `zed model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `zed model`.",
            provider="xai-oauth",
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    profile store (the caller already saved that). Swallows all errors â€” a
    ...
    # ~/.zed/auth.json even when ZED_HOME points at a profile path
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.zed/auth.json``. A single MITM during initial
    login could substitute a malicious ``token_endpoint``; that URL would
    then receive the refresh_token on every subsequent refresh â€” a permanent
    ...
    RFC 8414 Â§2 requires the issuer to be ``https://`` and SHOULD-keeps the
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `zed model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``ZED_XAI_BASE_URL``
    ...
    than raise â€” a bad env var should not deadlock authentication, but it
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        logger.warning(
            "Refusing xAI base_url override %r â€” host %r is not on the xAI origin "
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `zed model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Zed (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `zed model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``zed model``
        # won't fix that â€” surface a separate error code so
        ...
        if response.status_code == 403:
            raise AuthError(
                ...
                + " This OAuth account is not authorized for xAI API"
                  " access â€” xAI may be restricting API/OAuth use to"
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("ZED_XAI_REFRESH_TIMEOUT_SECONDS", 20)
            ...
            if should_refresh:
                ...
                try:
                    ...
                    if _is_terminal_xai_oauth_refresh_error(exc):
                        # Terminal failure (HTTP 400/401/403 â€” invalid_grant, token revoked).
...
# -----------------------------------------------------------------------------
# Shared Nous token store â€” lets OAuth credentials persist across profiles
# so a new `zed --profile <name> auth add nous --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${ZED_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<zed-root>/shared/nous_auth.json`` where ``<zed-root>`` is what
# ``get_default_zed_root()`` returns â€” ``~/.zed`` on Linux/macOS,
# ``%LOCALAPPDATA%\zed`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's ZED_HOME so named profiles (which
# typically live under ``<zed-root>/profiles/<name>/``) all see the
# same file.
#
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``ZED_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<zed-root>/shared/``, where ``<zed-root>`` is what
    :func:`zed_constants.get_default_zed_root` returns â€” so
    Linux/macOS classic installs land at ``~/.zed/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\zed\\shared\\``, and
    Docker / custom ``ZED_HOME`` deployments at
    ``<ZED_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("ZED_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from zed_constants import get_default_zed_root
    return get_default_zed_root() / "shared"


def _nous_shared_store_path() -> Path:
    path = _nous_shared_auth_dir() / NOUS_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's Zed root, refuse rather than silently corrupt cross-profile
    # state. Tests must set ZED_SHARED_AUTH_DIR to a tmp_path (conftest
    # does not do this automatically â€” mirror the _auth_file_path() guard
    # so forgetting to set it fails loudly instead of writing to the real
    # shared store).
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from zed_constants import get_default_zed_root
        real_home_shared = (
            get_default_zed_root() / "shared" / NOUS_SHARED_STORE_FILENAME
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No ZED_HOME yet (pre-setup): fall through without locking.
        yield
        return
...
def _read_shared_nous_state() -> Optional[Dict[str, Any]]:
    ...
    credentials available â€” fall through to device-code".
    """
    try:
        path = _nous_shared_store_path()
    except RuntimeError:
        # Test seat belt tripped â€” treat as missing
        return None
    if not path.is_file():
        return None
    try:
...
def _is_terminal_xai_oauth_refresh_error(exc: Exception) -> bool:
    ...
    token at all â€” retrying will never work.
...
def _try_import_shared_nous_state(
    *,
    ...
    etc.) â€” caller should then fall through to the normal device-code
    flow.
    """
    try:
...
def _refresh_access_token(
    *,
    ...
    # persisting the rotated token back to auth.json â€” the server then
    # retires the original RT, Zed's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "Nous refresh tokens are single-use â€” only Zed may call the "
            "refresh endpoint. For health checks, use `zed auth status` "
            "instead.\n"
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Zed models â€” they're not reliable for agentic tool-calling
            if "zed" in mid.lower():
                continue
    ...
    *,
    ...
    with _auth_store_lock():
        ...
        portal_base_url = (
            ...
            or os.getenv("ZED_PORTAL_BASE_URL")
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``zed auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``zed auth add nous --label <name>``).  It gets embedded in the
    ...
    seeding somehow produced no match â€” shouldn't happen).
    """
    from agent.credential_pool import load_pool
    ...
    # these credentials via `zed auth add nous --type oauth`. Best-
    ...
    *,
    ...
    with _auth_store_lock():
        ...
        def _persist_state(reason: str) -> None:
            ...
            # Best-effort â€” any failure is logged and swallowed inside
...
# `zed auth login/logout/add/remove` invalidate naturally on the next call.
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `zed auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first â€” this is where `zed auth` and
    # `zed model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("ZED_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("ZED_COPILOT_ACP_ARGS", "").strip()
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    # AWS SDK providers (Bedrock) â€” check via boto3 credential chain
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``zed doctor`` runs the live
    ...
    Never invokes the Entra credential chain â€” keeps CLI startup latency
    flat regardless of token-service / az login state.
    """
    info: Dict[str, Any] = {"provider": "azure-foundry"}
    try:
        from zed_cli.config import load_config, get_env_value
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Zed' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `zed doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set ZED_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from zed_cli.config import clear_model_endpoint_credentials
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `zed logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    """
    from zed_cli.models import _format_price_per_mtok
    ...
    try:
        ...
        if _unavailable:
            ...
            desc_lines.append(f"  â”€â”€ {unavailable_footer} â”€â”€")
    ...
    if _unavailable:
        ...
        unavailable_footer = unavailable_message.strip() or (
            f"Unavailable models (requires paid tier â€” upgrade at {_upgrade_url})"
        )
        print()
        print(f"  {_DIM}â”€â”€ {unavailable_footer} â”€â”€{_RESET}")
        for mid in _unavailable:
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from zed_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'zed model' or 'zed setup' instead."""
    print("The 'zed login' command has been removed.")
    print("Use 'zed auth' to manage credentials,")
    print("'zed model' to select a provider, or 'zed setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.zed/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Zed-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Zed auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Zed will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("ZED_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("Zed will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow â€” Zed gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Zed creates its own session â€” won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Zed auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Zed auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Zed creates its own local OAuth session)")
    print()
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=zed-agent` lets xAI attribute Zed-originated logins
...
def _xai_oauth_exchange_code_for_tokens(
    *,
    ...
    Sends ``code_verifier`` as required by RFC 7636 Â§4.5.  Also echoes
    ...
    strict RFC-compliant servers â€” RFC 7636 doesn't forbid additional
    parameters at the token endpoint â€” and decisively fixes the
    ...
    # state captured during the authorize step â€” without them, xAI
    ...
    if response.status_code != 200:
        body = response.text.strip()
        # See ``refresh_xai_oauth_pure`` â€” token-exchange 403 also
...
def _xai_oauth_loopback_login(
    *,
    ...
    URL into stdin (regression fix for #26923 â€” browser-only remote
    ...
    if manual_paste:
        # No HTTP listener â€” synthesize a redirect_uri matching what
        ...
        print("Open this URL to authorize Zed with xAI:")
        print(authorize_url)
    ...
    # bare-code paths â€” we substitute the locally generated state to keep
...
def _codex_device_code_login() -> Dict[str, Any]:
    ...
    # IP/account â€” retry with capped backoff (honoring ``Retry-After``)
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Zed auth store (~/.zed/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Zed login via MiniMax ({region}) OAuth...")
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    sending the same bearer until MiniMax's server returns 401 â€” typically
    ...
    calls :func:`_refresh_minimax_oauth_state` â€” that helper is a no-op
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `zed model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    sessions survive MiniMax's short access-token lifetime â€” see
    ...
    diagnostic call sites like ``zed status`` that just want to know
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Zed login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        ...
        # TUI gateway, whose stdout is a JSON-RPC pipe â€” a plain print() there is
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `zed model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def step_up_nous_billing_scope(
    *,
    ...
    targets the same deployment (incl. a preview via ``ZED_PORTAL_BASE_URL`` set
    at the original login). Persists to the auth store + shared store + pool, exactly
    like ``_login_nous`` â€” but WITHOUT the model picker (this is a scope upgrade, not
    a fresh login).
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if shared:
            ...
            if do_import in {"", "y", "yes"}:
                ...
                if auth_state is None:
                    print("Could not refresh shared credentials â€” falling back to device-code login.")
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Zed builds still see
        ...
        # preserve the user's previous provider â€” don't silently switch
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Zed will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `zed model` or configure an API key to use Zed.")
        else:
