"""``zed debug`` debug tools for Zed Agent.

Currently supports:
    zed debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.zed/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[zed debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.zed/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal â€” worst case the user has to run ``zed debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    the pending file and will be retried on the next sweep.  Silent â€”
    intended to be called from every ``zed debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
    ...
    for entry in entries:
        ...
        try:
            ...
            # Network hiccup, 404 (already gone), etc. â€” drop the entry
...
_PRIVACY_NOTICE = """\
âš ï¸  This will upload the following to a public paste service:
  â€¢ System info (OS, Python version, Zed version, provider, which API keys
    are configured â€” NOT the actual keys)
  â€¢ Recent log lines (agent.log, errors.log, gateway.log, gui.log, desktop.log
    â€” may contain conversation fragments and file paths)
  â€¢ Full agent.log, gateway.log, gui.log, and desktop.log (up to 512 KB each â€”
...
_GATEWAY_PRIVACY_NOTICE = (
    "âš ï¸ **Privacy notice:** This uploads system info + recent log tails "
    "(may contain conversation fragments) to a public paste service. "
    "Full logs are NOT included from the gateway â€” use `zed debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "zed-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    for 6 hours and then issued DELETE requests.  Those subprocesses leaked â€”
    every ``zed debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.zed/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``zed debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "zed-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from zed_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_zed_home() / "logs" / filename) if filename else None
...
def _capture_log_snapshot(
    ...
    *,
    ...
    try:
        ...
        if truncated:
            full_text = f"[... truncated â€” showing last ~{max_bytes // 1024}KB ...]\n{full_text}"
...
def _capture_dump() -> str:
    """Run ``zed dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``zed dump`` is run
        internally.
    ...
    # â”€â”€ Recent log tails (summary only) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    buf.write("\n\n")
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``zed debug share`` (CLI) and the
    dashboard ``POST /api/ops/debug-share`` endpoint. It performs blocking
    network I/O (paste uploads) â€” callers inside an event loop must run it in
    a worker thread.
    ...
    if redact:
        logger.info(
            "zed debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    # 1. Summary report (required â€” raises on failure so callers can fall back)
    urls["Report"] = upload_to_pastebin(report, expiry_days=expiry)

    # 2-4. Full logs (optional â€” failures are collected, not raised)
...
def run_debug_share(args):
    ...
    if local_only:
        # Local-only path never uploads â€” render the report to stdout and bail
    ...
    try:
        ...
        print("\nRun `zed debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print(f"\nâ±  Pastes will auto-delete in {hours} hours.")

    # Manual delete fallback
    print(f"To delete now:  zed debug delete <url>")

    print(f"\nShare these links with the Zed team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: zed debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'zed debug share'.")
        return
    ...
    for url in urls:
        try:
            ...
            if ok:
                ...
                print(f"  âœ— Failed to delete: {url} (unexpected response)")
        except ValueError as exc:
            print(f"  âœ— {exc}")
        except Exception as exc:
            print(f"  âœ— Could not delete {url}: {exc}")


def run_debug(args):
    """Route debug subcommands."""
    # Opportunistic sweep of expired pastes on every ``zed debug`` call.
    ...
    # best-effort â€” any failure is swallowed so ``zed debug`` stays
