"""Unified provider catalog â€” one source of truth for the provider universe.

The provider list shown by ``zed model`` (CLI/TUI) and the desktop Settings
â†’ Providers tabs (Accounts + API keys) **must be the same set**.  Historically
they were not: the CLI picker read :data:`zed_cli.models.CANONICAL_PROVIDERS`
...
GUI â€” e.g. GitHub Copilot showing up only under "tools", or ``openai-api`` being
...
the same universe ``zed model`` renders (``CANONICAL_PROVIDERS``), joining:

* ``auth_type`` / ``api_key_env_vars`` / ``base_url_env_var`` from
  :data:`zed_cli.auth.PROVIDER_REGISTRY` (credential truth), and
* ``display_name`` / ``description`` / ``signup_url`` from the provider's
  ...
  and four canonical providers have no profile at all â€” lmstudio, openai-api,
  tencent-tokenhub, xai-oauth â€” so the fallbacks are load-bearing).
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``zed model`` shows.
"""
...
# zed_cli.auth.PROVIDER_REGISTRY and providers.base.ProviderProfile.
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index â€” mirrors `zed model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``zed model`` universe.
