...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Zed'
...
The actual runtime resolution happens in zed_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    No args         â†’ return current state (value=None)
    'auto' / 'codex_app_server' / 'on' / 'off' â†’ return that value
    ...
    if raw in {"off", "default", "disable", "zed"}:
        return "auto", []
    ...
    *,
    ...
    if new_value is None:
        ...
        msg = (
            ...
            f"codex CLI: {'OK ' + ver if ok else 'not available â€” ' + (ver or 'install with `npm i -g @openai/codex`')}"
        )
    ...
    # If switching ON, verify codex CLI is installed before persisting â€”
    ...
    msg_lines = [
        f"openai_runtime: {current} â†’ {new_value}",
    ]
    if new_value == "codex_app_server":
        ok, ver = _check_binary_cached()
        if ok:
            msg_lines.append(f"codex CLI: {ver}")
        # Auto-migrate Zed' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Zed for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal â€” the runtime change still proceeds.
        try:
            from zed_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the zed-tools callback,
            # which is internal plumbing â€” surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "zed-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Zed tool callback are always-on production
            ...
            if "zed-tools" in mig_report.migrated:
                msg_lines.append(
                    "Zed tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Zed runtime â€” they need the "
                    "agent loop context.)"
                )
            msg_lines.append(f"  (config: {mig_report.target_path})")
            for err in mig_report.errors:
                msg_lines.append(f"âš  MCP migration: {err}")
        except Exception as exc:
            msg_lines.append(f"âš  MCP migration skipped: {exc}")
        msg_lines.append(
            "OpenAI/Codex turns now run through `codex app-server` "
            "(terminal/file ops/patching inside Codex; "
            "Zed tools available via MCP callback)."
        )
        msg_lines.append(
            "Effective on next session â€” current cached agent keeps "
            "the prior runtime to preserve prompt cache."
        )
    else:
        msg_lines.append("OpenAI/Codex turns will use the default Zed runtime.")
