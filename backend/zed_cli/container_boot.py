...
``$ZED_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-zed-setup
(the stage2 hook) has chowned the volume and seeded $ZED_HOME, but
...
# the down state and waits for explicit user action â€” this avoids the
...
# garbage post-restart â€” a numerically-equal PID in the new container
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$ZED_HOME``,
    not under ``profiles/``). The dispatcher in ``zed_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``zed gateway start`` (no ``-p``) targets. Without it,
    bare ``zed gateway start`` inside the container would land on
    ``s6-svc -u /run/service/gateway-default`` â†’ uncaught
    ``CalledProcessError`` â†’ traceback to the user (PR #30136 review).

    The default slot's prior state is read from
    ``$ZED_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        zed_home: The container's ZED_HOME (typically /opt/data).
            Profiles live under ``<zed_home>/profiles/<name>/``;
            the default profile lives at ``<zed_home>`` itself.
    ...
    # Default profile â€” always register, even if nothing has ever
    # populated the root profile dir. The slot exists so
    # ``zed gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_desired_state(zed_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(zed_home)
    ...
    profiles_root = zed_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `zed profile create` (config.yaml
            # is not â€” that comes later via `zed setup`). Use it as the
    ...
    if not dry_run:
        _write_reconcile_log(zed_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = zed_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("ZED_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off PID 1 argv, leaving the zed args.

    The container PID 1 argv looks like
    ``/init /opt/zed/docker/main-wrapper.sh <subcommand> [args...]`` and
    the wrapper re-execs ``zed <subcommand>``. Peel ``init`` â†’
    ``main-wrapper.sh`` â†’ ``zed`` so callers can match on the bare
    ...
    if args and args[0].endswith("main-wrapper.sh"):
        args = args[1:]
    if args and Path(args[0]).name == "zed":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``zed dashboard ...``) never spawns or
    supervises per-profile gateways â€” that is the gateway container's job.
    Reconciling profile gateway s6 slots there is not just wasted work: when
    the gateway and dashboard containers share a bind-mounted ZED_HOME,
    ...
    and a flag can be forgotten in a hand-written compose/k8s manifest â€”
...
def _cleanup_stale_runtime_files(profile_dir: Path) -> None:
    """Remove gateway.pid and processes.json â€” they reference PIDs in
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    s6-svscan starts scanning the dynamic scandir â€” the manager's
    ...
    review item O4) â€” even though cont-init.d runs before s6-svscan
    ...
    try:
        ...
        # Reuse the manager's run-script rendering â€” single source of
        ...
        # it up explicitly with `zed -p <profile> gateway start`
        ...
        # step in S6ServiceManager.register_profile_gateway â€” when
        # s6-svscan picks the published slot up, the s6-supervise it
        # spawns will EEXIST our dirs/FIFOs and inherit zed
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the zed user) won't hit EACCES. See
...
def _write_reconcile_log(
    zed_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $ZED_HOME/logs/container-boot.log.
    ...
    (defaults to 256 KiB â‰ˆ 3000 reconcile lines), the current file
    ...
    try:
        ...
        # Rotation failure is non-fatal â€” keep appending to the
...
def main() -> int:
    ...
    # gateways, so reconciling their s6 slots here is pure waste â€” and
    # actively harmful: when the gateway and dashboard containers share a
    # bind-mounted ZED_HOME, both race to flock() the same s6-log lock
    ...
    if _is_dashboard_container(_read_container_argv()):
        print(
            "reconcile: skipping (dashboard container â€” does not need "
    ...
    actions = reconcile_profile_gateways(
        zed_home=zed_home, scandir=scandir,
    )
    for a in actions:
        print(
