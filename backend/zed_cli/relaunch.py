"""
Unified self-relaunch for Zed CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``zed sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``zed`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
def _build_inherited_flag_table() -> list[tuple[str, bool]]:
    ...
    ``inherit_on_relaunch = True`` â€” set by ``_parser._inherited_flag``.
    """
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched zed."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_zed_bin() -> Optional[str]:
    """Find the zed entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("zed")`` on PATH.
      3. ``None`` â†’ caller should fall back to ``python -m zed_cli.main``.
    ...
    directly â€” CreateProcessW needs a real .exe, not a script associated
    ...
    ``zed.exe`` on PATH or the ``sys.executable -m zed_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
    ...
    # Relative path â€” resolve against CWD
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with zed.
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_zed_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "zed_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh zed invocation.

    On POSIX we use ``os.execvp`` which replaces the running process with
    the new one in place â€” same PID, no double-fork.  That's what the
    relaunch contract wants: "run zed again as if the user had typed
    the new argv".

    Windows has no native exec semantics â€” ``os.execvp`` on Windows
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``zed.exe`` (a Python console-script shim that wraps
    ``python -m zed_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent â€” the user sees "zed exited, then
    new zed started" â€” just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # Surface a helpful error rather than the raw OSError â€” the
            # caller used to see ``[Errno 8] Exec format error`` which is
            # cryptic.  Common causes: ``zed`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nZed relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run zed.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
