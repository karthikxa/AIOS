"""Session recap â€” summarize what's happened in the current session.
...
Differences from Claude Code:
    ...
    - Works unchanged on CLI and every gateway platform (Telegram,
      Discord, Slack, â€¦) because both call into the same ``build_recap``
      helper. Claude Code only shows this on the CLI.
    - Tailored to zed-agent's tool vocabulary (``terminal``, ``patch``,
      ``write_file``, ``delegate_task``, ``browser_*``, ``web_*``) â€” the
...
def _summarise_tool_activity(
    ...
    # files_seen is newestâ†’oldest which is what we want for display.
...
def build_recap(
    ...
    *,
    ...
    Inputs:
        ...
        platform: optional hint (``"cli"``, ``"telegram"``, â€¦). Does not
    ...
    if session_title:
        header_bits.append(f"â€” {session_title}")
    elif session_id:
        header_bits.append(f"â€” {session_id[:8]}")
    lines.append(" ".join(header_bits))

    if not messages:
        lines.append("  (nothing to recap â€” no messages yet)")
        return "\n".join(lines)
    ...
    if tool_counts:
        top = ", ".join(f"{name}Ã—{count}" for name, count in tool_counts[:5])
    ...
    if len(lines) == 2:
        # Only the header + scope line â€” nothing substantive to show.
