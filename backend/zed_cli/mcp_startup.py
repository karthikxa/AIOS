...
def _resolve_discovery_timeout(explicit: "float | None") -> float:
    ...
    and fail-safe â€” a missing/invalid value or a broken config falls back to a
    short safe bound so startup can never hang or crash.
    """
    if explicit is not None:
        return explicit
    try:
        from zed_cli.config import load_config, DEFAULT_CONFIG
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    only ever blocks for the real connect time of a still-pending server â€”
