...
configured for "cli" in `zed tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (ZED_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `zed chat`:
    ...
    - If only --provider given, error out (ambiguous â€” caller must pick a model).
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"zed -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "zed -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"zed -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "zed -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "zed -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to ZED_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("ZED_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "zed -z: --provider requires --model (or ZED_INFERENCE_MODEL). "
    ...
    # definition â€” a prompt would hang forever.
    ...
    if failure is not None:
        ...
        if isinstance(failure, (KeyboardInterrupt, SystemExit)):
            raise failure
        real_stderr.write(f"zed -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1

    if not (response or "").strip():
        real_stderr.write("zed -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``zed -z`` / oneshot mode.

    Oneshot bypasses ``ZedCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when zed is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from zed_cli.config import load_config
    from zed_cli.models import detect_provider_for_model
    ...
    # Resolve effective model: explicit arg â†’ env var â†’ config.
    ...
    env_model = os.getenv("ZED_INFERENCE_MODEL", "").strip()
    effective_model = (model or "").strip() or env_model or cfg_model

    # Resolve effective provider: explicit arg â†’ (auto-detect from model if
    # model was explicit) â†’ env / config (handled inside resolve_runtime_provider).
    #
    # When --model is given without --provider, auto-detect the provider that
    # serves that model â€” same semantic as `/model <name>` in an interactive
    ...
    if effective_provider is None and (model or env_model):
        ...
        # env var (not when it came from config â€” that's the "use my defaults"
        ...
        if explicit_model:
            ...
            try:
                from zed_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("ZED_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    agent = AIAgent(
        ...
        #   - clarify  â†’ returns a synthetic "pick a default" instruction
        #                so the agent continues instead of stalling on
        #                the tool's built-in "not available" error
        #   - sudo password prompt â†’ terminal_tool gates on
        #                ZED_INTERACTIVE which we never set
        #   - shell-hook approval â†’ auto-approved via ZED_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval â†’ bypassed via ZED_YOLO_MODE=1
        #   - skill secret capture â†’ returns gracefully when no callback set
...
def _oneshot_clarify_callback(question: str, choices=None) -> str:
    """Clarify is disabled in oneshot mode â€” tell the agent to pick a
