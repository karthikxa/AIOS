...
Each memory provider *declares* its configurable surface here â€” the fields, their
...
class ProviderField:
    ...
    * ``text`` / ``select`` â€” persisted to the provider's JSON config file
      (``<zed_home>/<provider>/config.json``) under ``key``.
    * ``secret`` â€” persisted to the env store under ``env_key`` and never read
...
HINDSIGHT = MemoryProvider(
    ...
    fields=(
        ProviderField(
            ...
            kind=KIND_SELECT,
            default="cloud",
            description="How Zed connects to Hindsight.",
            options=(
                ProviderFieldOption(
                    "cloud",
                    "Cloud",
