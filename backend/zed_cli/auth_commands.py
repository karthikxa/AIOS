...
def auth_add_command(args) -> None:
    ...
    # Clear ALL suppressions for this provider â€” re-adding a credential is
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_zed_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:zed_pkce",
    ...
    if provider == "nous":
        ...
        # <zed-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `zed --profile <name>
        ...
        if shared:
            ...
            if do_import in {"", "y", "yes"}:
                ...
                # â€” fall through to device-code flow.
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``zed auth add openai-codex`` overwrote
    ...
    raise SystemExit(f"`zed auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Zed reads from
    ...
    if step is None:
        # Unregistered source â€” e.g. "manual", which has nothing external
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `zed auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `zed auth` is called bare."""
    ...
    # Show AWS Bedrock credential status (not in the pool â€” uses boto3 chain)
    try:
        ...
        if has_aws_credentials():
            ...
            try:
                ...
                print(f"  Identity: (could not resolve â€” boto3 STS call failed)")
            print()
    except ImportError:
        ...
        if isinstance(_model_cfg, dict):
            ...
            if _cfg_provider == "azure-foundry" and _cfg_auth_mode == "entra_id":
                ...
                if not has_azure_identity_installed():
                    print("  Status: âš  azure-identity not installed "
                    ...
                    if _info.get("ok"):
                        ...
                        print(f"  Status: âœ“ token acquired ({_tag})")
                    else:
                        _err = _info.get("error") or "credential chain exhausted"
                        print(f"  Status: âš  {_err}")
...
def _interactive_strategy() -> None:
    ...
    for i, s in enumerate(strategies, 1):
        marker = " â†" if s == current else ""
        print(f"  {i}. {s:15s} â€” {descriptions.get(s, '')}{marker}")
...
def auth_command(args) -> None:
    ...
    if action == "spotify":
        ...
        return
    # No subcommand â€” launch interactive mode
    _interactive_auth()
