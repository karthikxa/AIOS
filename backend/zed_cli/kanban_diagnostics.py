"""Kanban diagnostics â€” structured, actionable distress signals for tasks.
...
# Severity rungs, ordered least â†’ most urgent. The UI colors them
...
class DiagnosticAction:
    ...
    * ``reclaim`` / ``reassign`` â€” POST to the matching /tasks/:id/*
      endpoint; dashboard wires into the existing recovery popover.
    * ``unblock`` â€” PATCH status back to ``ready`` (for stuck-blocked
      diagnostics).
    * ``cli_hint`` â€” print/copy a shell command (e.g.
      ``zed -p <profile> auth``). No HTTP side effect.
    * ``open_docs`` â€” deep-link to the docs URL named in ``payload.url``.
    * ``comment`` â€” nudge the operator to add a comment (for
...
# fallbacks regardless of kind â€” they're the two baseline recovery
...
def _aux_slot_explicit(slot: Any) -> bool:
    ...
    model/base_url/api_key â€” that path falls through to the main model. An
...
def triage_aux_status(config: Optional[dict]) -> Optional[dict]:
    ...
    returns a dict with:
      ...
      - ``auto_decompose``: bool â€” whether the dispatcher auto-runs decompose
      - ``decomposer_explicit``: bool â€” user-supplied decomposer slot
      - ``specifier_explicit``: bool â€” user-supplied specifier slot
      - ``main_model_visible``: bool â€” main model can serve as auto fallback
    """
...
def _rule_triage_aux_unavailable(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    With auto-decompose off, the user must run ``zed kanban specify``,
    which only needs ``auxiliary.triage_specifier``.

    The default slot is ``provider: auto`` â†’ auto-falls back to the main model,
    ...
    if auto_decompose:
        ...
        detail_path = (
            ...
            "`zed kanban specify`, which uses auxiliary.triage_specifier."
        )
    ...
    actions = [
        DiagnosticAction(
            ...
            payload={
                "command": (
                    f"zed config set {primary_slot}.provider auto"
                )
            },
            suggested=True,
        ),
    ...
    if not fallback_explicit and not main_visible:
        actions.append(DiagnosticAction(
            ...
            payload={
                "command": (
                    f"zed config set {fallback_slot}.provider auto"
                )
            },
        ))
    if not auto_decompose:
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Specify manually: zed kanban specify {task_id}",
            payload={"command": f"zed kanban specify {task_id}"},
        ))
...
def _rule_repeated_failures(task, events, runs, now, cfg) -> list[Diagnostic]:
    """Task's unified ``consecutive_failures`` counter is climbing â€”
    ...
    if most_recent_outcome == "spawn_failed" and assignee and assignee != "default":
        # Spawn is failing specifically â€” profile setup issue.
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Verify profile: zed -p {assignee} doctor",
            payload={"command": f"zed -p {assignee} doctor"},
            suggested=True,
        ))
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Fix profile auth: zed -p {assignee} auth",
            payload={"command": f"zed -p {assignee} auth"},
        ))
        ...
        if task_id:
            actions.append(DiagnosticAction(
                kind="cli_hint",
                label=f"Check logs: zed kanban log {task_id}",
                payload={"command": f"zed kanban log {task_id}"},
                suggested=True,
            ))
    ...
    err_snippet = err_text[:500] + ("â€¦" if len(err_text) > 500 else "") if err_text else ""
    outcome_label = {
        "spawn_failed": "spawn",
        "timed_out": "timeout",
        "crashed": "crash",
...
def _rule_repeated_crashes(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    Narrower than ``repeated_failures`` â€” fires earlier (2 crashes vs 3
    ...
    # Unified rule will catch this â€” let it handle to avoid double fire.
    ...
    for r in reversed(ordered):
        ...
        if outcome == "crashed":
            ...
            # aren't crash signals â€” don't count them, but they also
    ...
    # having to open the logs. Truncate defensively â€” these can be huge
    # (full tracebacks).
    err_text = (last_err or "").strip() if last_err else ""
    err_snippet = err_text[:500] + ("â€¦" if len(err_text) > 500 else "") if err_text else ""
    if err_snippet:
...
def _rule_stuck_in_blocked(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    return [Diagnostic(
        ...
        detail=(
            ...
            f"are waiting for human input â€” check the block reason and "
...
def _rule_block_unblock_cycling(task, events, runs, now, cfg) -> list[Diagnostic]:
    """Task has cycled through blocked â†’ unblocked many times â€” the
    ...
    gap 1). This rule complements that one by counting blockâ†’unblock
    ...
    # Walk events chronologically (arrival order â€” callers pre-sort by
    ...
    if task_id:
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Check block reasons: zed kanban events {task_id}",
            payload={"command": f"zed kanban events {task_id}"},
            suggested=True,
        ))
    return [Diagnostic(
        kind="block_unblock_cycling",
        severity="warning",
        title=f"Task blockâ†’unblock cycled {cycles}x in {int(window_seconds/3600)}h",
        detail=(
...
def _rule_stranded_in_ready(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    * Operator typo'd the assignee â€” no profile or external worker matches.
    ...
    * Dispatcher is misconfigured (wrong board, wrong ZED_HOME).

    Pre-rule, all of these silently rotted in ``skipped_nonspawnable`` â€”
    the dispatcher correctly skipped them (good â€” no respawn loop) but
    ...
    works for Zed profiles, registered lanes, external workers, and
    ...
    if status != "ready":
        return []
    # Skip tasks with a live claim â€” they're being worked on, even if
    ...
    # 2x â€“ 6x = error; beyond 6x = critical (something is clearly
    ...
    actions = [
        ...
        DiagnosticAction(
            ...
            payload={"command": "zed kanban diagnostics"},
        ),
    ]
...
# Registry â€” order matters: rules higher on the list render first when
...
DEFAULT_CONFIG = {
    ...
    # Stranded-task threshold. 30 min by default â€” below that, the
...
def config_from_runtime_config(raw_config: Optional[dict]) -> dict:
    """Build diagnostics config from the full Zed runtime config.
