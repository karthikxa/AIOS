"""Persistent session goals â€” the Ralph loop for Zed.

A goal is a free-form user objective that stays active across turns. After
each turn completes, a small judge call asks an auxiliary model "is this
goal satisfied by the assistant's last response?". If not, Zed feeds a
...
- The continuation prompt is just a normal user message appended to the
  ...
  swap â€” prompt caching stays intact.
...
- When a real user message arrives mid-loop it preempts the continuation
  ...
  will say ``done``).
- This module has zero hard dependency on ``cli.ZedCLI`` or the gateway
  runner â€” both wire the same ``GoalManager`` in.
...
# before emitting the visible JSON â€” and the first /goal turn's prompt is
...
# transport errors do NOT count toward this â€” those are transient. This guards
...
JUDGE_SYSTEM_PROMPT = (
    ...
    "Otherwise the goal is NOT done â€” CONTINUE.\n\n"
...
JUDGE_USER_PROMPT_WITH_SUBGOALS_TEMPLATE = (
    ...
    "met' or 'implying it was done' â€” require specific evidence (a "
    "file contents excerpt, an output line, a command result). If "
    "ANY criterion lacks specific evidence in the response, the goal "
    "is NOT done â€” return CONTINUE.\n\n"
...
def _get_session_db() -> Optional[Any]:
    """Return a SessionDB instance for the current ZED_HOME.

    SessionDB has no built-in singleton, but opening a new connection per
    /goal call would thrash the file. We cache one instance per
    ``zed_home`` path so profile switches still pick up the right DB.
...
def judge_goal(
    ...
    *,
    ...
    was unusable (empty or non-JSON). API/transport errors return False â€” they
    ...
    so a broken judge doesn't wedge progress â€” the turn budget and the
    consecutive-parse-failures auto-pause are the backstops.
    """
    if not goal.strip():
        return "skipped", "empty goal", False
    if not last_response.strip():
        # No substantive reply this turn â€” almost certainly not done yet.
    ...
    # Build the prompt â€” pick the with-subgoals variant when applicable.
    ...
    try:
        ...
        logger.info("goal judge: API call failed (%s) â€” falling through to continue", exc)
...
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# GoalManager â€” the orchestration surface CLI + gateway talk to
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class GoalManager:
    ...
    - ``set(goal)`` â€” start a new standing goal.
    - ``clear()`` â€” remove the active goal.
    - ``pause()`` / ``resume()`` â€” explicit user controls.
    - ``status()`` â€” printable one-liner.
    - ``evaluate_after_turn(last_response)`` â€” call the judge, update state,
      and return a decision dict the caller uses to drive the next turn.
    - ``next_continuation_prompt()`` â€” the canonical user-role message to
    ...
    def status_line(self) -> str:
        ...
        if s.status == "active":
            return f"âŠ™ Goal (active, {turns}{sub}): {s.goal}"
        if s.status == "paused":
            extra = f" â€” {s.paused_reason}" if s.paused_reason else ""
            return f"â¸ Goal (paused, {turns}{sub}{extra}): {s.goal}"
        if s.status == "done":
            return f"âœ“ Goal done ({turns}{sub}): {s.goal}"
    ...
    def render_subgoals(self) -> str:
        ...
        if not self._state.subgoals:
            return "(no subgoals â€” use /subgoal <text> to add criteria)"
    ...
    def evaluate_after_turn(
        ...
        *,
        ...
        Decision keys:
          ...
          - ``should_continue``: bool â€” caller should fire another turn
        ...
        if verdict == "done":
            ...
            return {
                ...
                "verdict": "done",
                "reason": reason,
                "message": f"âœ“ Goal achieved: {reason}",
            }
        ...
        if state.consecutive_parse_failures >= DEFAULT_MAX_CONSECUTIVE_PARSE_FAILURES:
            ...
            return {
                ...
                "reason": reason,
                "message": (
                    f"â¸ Goal paused â€” the judge model ({state.consecutive_parse_failures} turns) "
                    "isn't returning the required JSON verdict. Route the judge to a stricter "
                    "model in ~/.zed/config.yaml:\n"
                    "  auxiliary:\n"
                    "    goal_judge:\n"
                    "      provider: openrouter\n"
        ...
        if state.turns_used >= state.max_turns:
            ...
            return {
                ...
                "reason": reason,
                "message": (
                    f"â¸ Goal paused â€” {state.turns_used}/{state.max_turns} turns used. "
        ...
        return {
            ...
            "reason": reason,
            "message": (
                f"â†» Continuing toward goal ({state.turns_used}/{state.max_turns}): {reason}"
            ),
        }
...
# the goal â€” the worker already has the full task body in its first turn,
# so we keep this short and point it back at the lifecycle contract.
KANBAN_GOAL_CONTINUATION_TEMPLATE = (
    "[Continuing toward this kanban task â€” judge says it is not done yet]\n"
    "Reason: {reason}\n\n"
...
def run_kanban_goal_loop(
    *,
    ...
    (``zed -p <profile> chat -q "work kanban task <id>"``). The worker's
    ...
    1. Check whether the worker already terminated the task (called
       ``kanban_complete`` / ``kanban_block``). If so, stop â€” nothing to do.
    2. Otherwise judge the latest response against ``goal_text`` (the card's
       title + body). ``continue`` â†’ feed a continuation prompt and run
       another turn IN THE SAME SESSION via ``run_turn``. ``done`` but the
       task is still open â†’ one explicit "call kanban_complete" nudge.
    ...
    This function performs NO SessionDB persistence â€” a worker process is
    ...
    while True:
        ...
        if status not in ("running", "ready"):
            # Reclaimed / archived / unexpected â€” let the dispatcher own it.
            _log(f"kanban goal loop: task {task_id} status={status!r}; stopping")
            return {"outcome": "stopped", "turns_used": turns_used, "reason": f"status={status}"}

        # Still open â€” judge whether the latest response satisfies the card.
        ...
        if verdict == "done":
            if nudged_to_finalize:
                ...
                # didn't â€” block for review rather than spin.
