"""Managed uv â€” one path, no guessing.

Zed owns its own uv binary at ``$ZED_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$ZED_HOME/bin`` so the installer writes directly there â€” no PATH
...
def managed_uv_path() -> Path:
    """Return the path where Zed keeps *its* uv binary.

    ``$ZED_HOME/bin/uv`` on POSIX, ``$ZED_HOME\\bin\\uv.exe`` on
    Windows.  The directory may not exist yet â€” callers should use
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``zed update`` runs
    the call site from the *old*, already-imported ``zed_cli.main`` against
    ...
    POSIX only. This wrapper is **never** returned on Windows â€” see
...
def _ensure_uv_path() -> Optional[str]:
    ...
    print(f"  â†’ Installing managed uv into {target.parent} ...")

    try:
        _install_uv(target)
    except Exception as exc:
        logger.warning("Managed uv install failed: %s", exc)
        print(f"  âœ— Failed to install managed uv: {exc}")
        return None
    ...
    if result:
        ...
        print(f"  âœ“ Managed uv installed ({version})")
    else:
        print("  âœ— Managed uv install appeared to succeed but binary not found")
    return result
...
def ensure_uv():
    ...
    release â€” see :class:`_UvResult` for the update-boundary rationale.
    ...
    so a ``_UvResult`` â€” whose ``__iter__`` yields ``(path, fresh_bootstrap)``
    rather than characters â€” would inject the bool into the command line and
    ...
    On failure the result is falsy â€” never raises â€” so callers can fall back to
    pip gracefully.
    """
...
def update_managed_uv() -> Optional[str]:
    ...
    Call this during ``zed update`` so the managed copy stays current.
    Returns the managed path on success, ``None`` if uv isn't available or
    the self-update fails (non-fatal â€” the old version still works).
    """
    existing = resolve_uv()
    if not existing:
        # Not installed yet â€” ensure_uv() will handle that elsewhere.
        return None
    ...
    if result.returncode == 0:
        ...
        print(f"  âœ“ Managed uv updated ({version})")
    else:
        # Non-fatal â€” old uv still works fine.
...
def _install_uv(target: Path) -> None:
    ...
    ``$ZED_HOME/bin/`` instead of ``~/.local/bin/``.
    """
