...
# Deliberately specific â€” a bare "gateway ... restart" catch-all would block
...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(zed\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*zed)"
    r"|(systemctl\s+(-\S+\s+)*(restart|stop|start)\s+.*zed)"
    r"|(p?kill\s+.*zed.*gateway)"
)
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'zed cron create ...' or the /cron command in chat.", Colors.DIM))
        return

    print()
    print(color("â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”", Colors.CYAN))
    print(color("â”‚                         Scheduled Jobs                                  â”‚", Colors.CYAN))
    print(color("â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜", Colors.CYAN))
    print()
    ...
    for job in jobs:
        ...
        repeat_str = f"{repeat_completed}/{repeat_times}" if repeat_times else "âˆž"
        ...
        if delivery_err:
            print(f"    {color('âš  Delivery failed:', Colors.YELLOW)} {delivery_err}")

        print()

    from zed_cli.gateway import find_gateway_pids
    if not find_gateway_pids():
        print(color("  âš   Gateway is not running â€” jobs won't fire automatically.", Colors.YELLOW))
        print(color("     Start it with: zed gateway install", Colors.DIM))
        print(color("                    sudo zed gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        print(color("âœ“ Gateway is running â€” cron jobs will fire automatically", Colors.GREEN))
        print(f"  PID: {', '.join(map(str, pids))}")
    else:
        print(color("âœ— Gateway is not running â€” cron jobs will NOT fire", Colors.RED))
        print()
        print("  To enable automatic execution:")
        print("    zed gateway install    # Install as a user service")
        print("    sudo zed gateway install --system  # Linux servers: boot-time system service")
        print("    zed gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `zed gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: zed cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
