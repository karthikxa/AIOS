"""Implementation of the ``zed bundles`` CLI subcommand.

Mirrors the structure of ``zed_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `zed bundles list | grep â€¦` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  zed bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def _cmd_reload(args) -> None:
    ...
    if diff["added"]:
        ...
        for entry in diff["added"]:
            c.print(f"  + {entry['name']} â€” {entry.get('description', '')}")
    if diff["removed"]:
...
def register_cli(subparser) -> None:
    """Build the ``zed bundles`` argparse tree.

    Called from ``zed_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `zed bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``zed bundles <subcommand>`` to the right handler."""
    handler = getattr(args, "_bundles_handler", None)
    if handler is None:
        # No subcommand given â€” default to list.
        _cmd_list(args)
        return
    handler(args)
