...
read-only entitlement/balance) â€” this module owns the *write* side: buy credits,
poll a charge, configure auto-reload.

Design rules:

- **Money is decimal, never float.** The server emits decimal STRINGS
  (``"142.5"`` â€” not fixed 2dp). We parse with :class:`decimal.Decimal` and never
...
- **Auth** = the OAuth bearer JWT Zed already holds for inference
...
- **Portal base URL** resolves with the same precedence as the device-flow login
  (``auth.py``): ``ZED_PORTAL_BASE_URL`` â†’ ``NOUS_PORTAL_BASE_URL`` â†’ the
  stored auth-state ``portal_base_url`` â†’ the registry default. This is how the
...
# zed_cli.auth.NOUS_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
class BillingScopeRequired(BillingError):
    """``403 insufficient_scope`` â€” the held token lacks ``billing:manage``.
...
class BillingRateLimited(BillingError):
    ...
    NOT a payment failure. Carries ``retry_after`` (seconds) â€” back off and tell
...
class BillingAuthError(BillingError):
    """``401`` â€” missing/invalid bearer token (not logged in / expired)."""
...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    ``ZED_PORTAL_BASE_URL`` â†’ ``NOUS_PORTAL_BASE_URL`` â†’ stored auth-state
    ``portal_base_url`` â†’ registry default. Trailing slash stripped.
    """
    env = os.getenv("ZED_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
...
def _absolutize_portal_url(portal_url: Optional[str]) -> Optional[str]:
    ...
    â€” it doesn't know which deployment the client points at. Resolve it against the
...
def _resolve_token_and_base(*, use_cache: bool = True) -> tuple[str, str]:
    ...
    try:
        ...
        # auth module unavailable â€” fall back to the raw stored token.
...
def _retry_after_seconds(headers: Any) -> Optional[int]:
    """Parse a ``Retry-After`` header (integer seconds) â€” None if absent/bad."""
    if headers is None:
        return None
    try:
...
def _raise_for_error(
    ...
    if status in (429, 503):
        raise BillingRateLimited(
            message or "Rate limited â€” try again shortly.", **common
        )
...
def _request(
    ...
    *,
    ...
    try:
        ...
        # A 401 on a cached token â†’ drop the cache and retry once with a fresh
...
def get_billing_state(*, timeout: float = DEFAULT_TIMEOUT) -> dict[str, Any]:
    """``GET /api/billing/state`` â€” role-tiered overview (no scope required)."""
...
def patch_auto_top_up(
    *,
    ...
    """``PATCH /api/billing/auto-top-up`` â€” configure auto-reload (scope required).
...
def post_charge(
    *,
    ...
    """``POST /api/billing/charge`` â€” buy credits (scope required).
    ...
    Returns ``202 {chargeId}`` â€” money is NOT confirmed yet; poll with
...
def get_charge_status(
    ...
    """``GET /api/billing/charge/{id}`` â€” poll a charge (scope required).

    Returns ``{status: "pending"|"settled"|"failed", ...}``. An unknown or foreign
    id returns ``{status:"pending"}`` (never 404, never another org's data) â€” so a
