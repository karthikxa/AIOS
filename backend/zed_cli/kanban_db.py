...
``<root>`` is the **shared Zed root** (the parent of any active
...
``zed -p <profile>`` joins the same board as the dispatcher that
...
the profile's Zed home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  (explicit â€” used by the CLI ``--board`` flag and the dashboard
  ``?board=...`` query param).
* ``ZED_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on â€” workers cannot see other boards).
* ``ZED_KANBAN_DB`` env var (pins the DB file path directly â€” legacy
  override still honoured; highest precedence when the file path itself
  is what the caller wants to force).
* ``<root>/kanban/current`` â€” a one-line text file holding the slug of
  the "currently selected" board. Written by ``zed kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.zed``. In Docker / custom
deployments where ``ZED_HOME`` points outside ``~/.zed`` (e.g.
``/opt/zed``), ``<root>`` is ``ZED_HOME``. Legacy env-var
overrides still work:

* ``ZED_KANBAN_DB`` â€” pin the database file path directly.
* ``ZED_KANBAN_WORKSPACES_ROOT`` â€” pin the workspaces root directly.
* ``ZED_KANBAN_HOME`` â€” pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``ZED_KANBAN_DB``,
``ZED_KANBAN_WORKSPACES_ROOT``, and ``ZED_KANBAN_BOARD`` into
worker subprocess env so workers converge on the exact DB the
dispatcher used to claim their task â€” even under unusual symlink or
Docker layouts.

Schema is intentionally small: tasks, task_links, task_comments,
task_events.  The ``workspace_kind`` field decouples coordination from git
worktrees so that research / ops / digital-twin workloads work alongside
coding workloads.  See ``docs/zed-kanban-v1-spec.pdf`` for the full
...
The CAS coordination is **per-board** â€” each board is a separate DB,
...
# ``ZED_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
# claim in that state would spawn a duplicate alongside the surviving worker â€”
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``ZED_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("ZED_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
# fork() â†’ /proc-visibility window where liveness can transiently report
...
# transient throttle). 75 == BSD ``EX_TEMPFAIL`` (sysexits.h) â€” the
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``ZED_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("ZED_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``ZED_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    the next tick) â€” useful for tests that want to assert the task becomes
    spawnable again immediately.
    """
    raw = os.environ.get(
        "ZED_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "zed_kanban_current_board_override",
    default=None,
)
...
# Slug validator: lowercase alphanumerics, digits, hyphens; 1â€“64 chars.
# Strict enough to stop traversal (`..`) and embedded path separators, loose
# enough that kebab-case names like ``atm10-server`` or ``zed-agent``
...
def kanban_home() -> Path:
    """Return the shared Zed root that anchors the kanban board.

    Resolution order:

    1. ``ZED_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_zed_root()``, which already returns ``<root>``
       when ``ZED_HOME`` is ``<root>/profiles/<name>``, and returns
       ``ZED_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``ZED_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("ZED_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from zed_constants import get_default_zed_root
    return get_default_zed_root()


def boards_root() -> Path:
    """Return ``<root>/kanban/boards`` â€” the parent of non-default board dirs.

    ``default`` is intentionally NOT under this directory â€” its DB lives at
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``zed kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``ZED_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``zed kanban boards
       switch``), but only when that board still exists.
    3. ``DEFAULT_BOARD`` (``"default"``).

    A malformed or stale slug at any step falls through to the next layer
    with a best-effort warning â€” the dispatcher must never crash because a
    ...
    env = os.environ.get("ZED_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    exists first (via :func:`board_exists`) â€” this function does not â€”
    so that ``zed kanban boards switch <typo>`` returns an error
...
def board_dir(board: Optional[str] = None) -> Path:
    ...
    ``<root>/kanban.db`` for back-compat â€” see :func:`kanban_db_path`.
...
def board_exists(board: Optional[str] = None) -> bool:
    ...
    ``default`` is considered to always exist â€” its DB is created
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``ZED_KANBAN_DB`` env var â€” pins the path directly. Honoured for
       back-compat and for the dispatcherâ†’worker handoff (defense in
       depth: dispatcher injects this into worker env so workers are
       immune to any path-resolution disagreement).
    2. When ``board`` arg is None, the active board from
       :func:`get_current_board` is used.
    3. Board ``default`` â†’ ``<root>/kanban.db`` (back-compat path).
       Other boards â†’ ``<root>/kanban/boards/<slug>/kanban.db``.
    """
    override = os.environ.get("ZED_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``ZED_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    precedence) â€” the dispatcher injects this into worker env.

    ``default`` keeps the legacy path ``<root>/kanban/workspaces/`` so
    that existing scratch workspaces from before the boards feature are
    preserved. Other boards use ``<root>/kanban/boards/<slug>/workspaces/``.
    """
    override = os.environ.get("ZED_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``ZED_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    local terminal backend â€” the default for kanban â€” that path resolves
    directly. Remote backends (Docker/Modal) need this directory mounted;
    see the kanban docs.
    """
    override = os.environ.get("ZED_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board â€” makes ``zed kanban log`` unambiguous even when multiple
...
def _default_board_display_name(slug: str) -> str:
    ...
    ``atm10-server`` â†’ ``Atm10 Server``. Users can override via
...
def read_board_metadata(board: Optional[str] = None) -> dict:
    ...
    Never raises â€” a missing / malformed ``board.json`` falls back to a
    ...
    try:
        ...
        if p.exists():
            ...
            if isinstance(raw, dict):
                ...
                # its directory â€” trust the filesystem.
                raw["slug"] = slug
                meta.update(raw)
...
def write_board_metadata(
    ...
    *,
    ...
    # Preserve existing DB-derived fields â€” they get re-computed each
...
def create_board(
    ...
    *,
    ...
    board already exists â€” matching ``mkdir -p`` semantics.
    """
    normed = _normalize_board_slug(slug)
    if not normed:
        raise ValueError("board slug is required")
    ...
    if root.is_dir():
        for child in sorted(root.iterdir(), key=lambda p: p.name.lower()):
            if not child.is_dir():
                continue
            slug = child.name
            # Keep slug normalisation soft for discovery â€” but skip dirs
...
def remove_board(slug: str, *, archive: bool = True) -> dict:
    ...
    The ``default`` board cannot be removed â€” raises :class:`ValueError`.
...
class Task:
    ...
    # The value is the failure count at which the breaker trips â€” e.g.
    ...
    # judge agrees, the goal-turn budget is exhausted (â†’ kanban_block),
    ...
    # within an agent loop that propagated ``ZED_SESSION_ID``. NULL for
...
class Run:
    ...
    A run is one attempt to execute a task â€” created on claim, closed
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``ZED_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("ZED_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    change between attempts â€” e.g. a partial repair or further damage â€” the
    ...
    from caller-supplied directory segments â€” no traversal is possible.
    """
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    :func:`kanban_db_path` env-var chain, or the kanban-home default â€”
    all sources Zed treats as user-controlled-but-trusted on the
    ...
    try:
        ...
        # Lock contention, busy, transient IO â€” not corruption. Let it propagate.
        raise
...
def connect(
    ...
    * ``db_path`` explicit â†’ used as-is (legacy callers, tests).
    * ``board`` explicit â†’ resolves to that board's DB.
    * Neither â†’ :func:`kanban_db_path` resolves via
      ``ZED_KANBAN_DB`` env â†’ ``ZED_KANBAN_BOARD`` env â†’
      ``<root>/kanban/current`` â†’ ``default``.
    """
    ...
    with _cross_process_init_lock(path):
        # Cheap byte-level check first â€” catches the #29507 TLS-overwrite shape
        # and other invalid-header cases without opening a sqlite connection.
        _validate_sqlite_header(path)
        # Full integrity probe â€” catches corruption past the header (malformed
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See zed_state._WAL_INCOMPAT_MARKERS for detection logic.
...
def connect_closing(
    ...
    *,
    ...
    Use this instead of ``with kb.connect() as conn:`` â€” sqlite3's
    ...
    through ``connect()`` (e.g. ``run_slash`` dispatching ``/kanban â€¦``
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``zed kanban init`` and the
    ...
    may have drifted â€” tests that write legacy event kinds directly,
    external tools that upgrade an old DB file â€” can call this to
    force re-migration.
    """
    if db_path is not None:
        path = db_path
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    # the other additive-column indexes â€” see the block after the
    ...
    # Legacy column migration: ``spawn_failures`` â†’ ``consecutive_failures``
    # and ``last_spawn_error`` â†’ ``last_failure_error``.
    #
    ...
    if "session_id" not in cols:
        ...
        # ``ZED_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
    ...
    if runs_exist:
        with write_txn(conn):
            ...
            for row in inflight:
                ...
                # CAS fails we've got an orphan run_row â€” mark it
...
# neither can fix a drifted column type â€” the table must be rebuilt. See
# #35096.
#
...
def _table_has_drifted(conn: sqlite3.Connection, table: str) -> bool:
    ...
    if not info:
        return False  # table absent â€” nothing to rebuild
...
def _rebuild_drifted_tables(conn: sqlite3.Connection) -> None:
    ...
    rebuilt with the standard SQLite pattern â€” CREATE new â†’ INSERT shared
    columns â†’ DROP old â†’ RENAME â€” recreating its indexes too (DROP TABLE takes
    ...
    so the first post-migration tick replays a task's event history once â€”
    ...
    try:
        for table in drifted:
            ...
            if table == "kanban_notify_subs":
                # Cast the legacy TEXT cursor to INTEGER; NULL / non-numeric â†’ 0.
...
def write_txn(conn: sqlite3.Connection):
    ...
    try:
        ...
        # A discrepancy means a torn-extend â€” raise now rather than silently corrupt.
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from zed_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    parents â€” a specifier/triager is expected to promote the task to
    ...
    each name to ``zed --skills ...`` alongside the built-in
    ...
    # invisibly splatter a comma-joined string into one argv slot â€” the
    # `zed --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
    ...
    # Idempotency check â€” return the existing task instead of creating a
...
# Canonical sort-order mappings for ``zed kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        # Dependency edge removed â€” re-evaluate promotion eligibility for the
        ...
        # next dispatcher tick or a manual `zed kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``zed kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``zed kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** â€” a worker called
      ``kanban_block(reason="review-required: ...")`` (or somebody ran
      ``zed kanban block <id>``).  This is a deliberate handoff that
      should stay blocked until an operator unblocks it.  The block tool
      emits a ``"blocked"`` event row in ``task_events``.

    * **Circuit-breaker** â€” ``_record_task_failure`` tripped after
    ...
    DB manipulation) â€” preserves the pre-#28712 auto-recover semantics
    for that path.
    """
    row = conn.execute(
...
def recompute_ready(
    ...
    1. The most recent block event was a worker-initiated
       ``kanban_block`` â€” those stay blocked until an explicit
    ...
    with write_txn(conn):
        ...
        for row in todo_rows:
            ...
            if cur_status == "blocked" and _has_sticky_block(conn, task_id):
                # Worker / operator asked for human review â€” do not
            ...
            if all(p["status"] in ("done", "archived") for p in parents):
                if cur_status == "blocked":
                    ...
                    # block â†’ auto-recover â†’ respawn â†’ budget
                    # exhausted â†’ block â†’ â€¦  The counter must also
...
def claim_task(
    ...
    *,
    ...
    with write_txn(conn):
        ...
        # 'todo' here â€” recompute_ready will re-promote when the parents
...
def claim_review_task(
    ...
    *,
    ...
    does NOT check parent dependencies â€” the task already passed that
...
def release_stale_claims(
    ...
    *extended* (with a ``claim_extended`` event) instead of being
    ...
    been making no observable progress and we reclaim anyway â€” even if
...
def reclaim_task(
    ...
    *,
    ...
    if row["status"] != "running" and row["claim_lock"] is None:
        # Nothing to reclaim â€” already ready / blocked / done.
        return False
    ...
    # Operator intervention â€” they've looked at the task, so the
...
def _verify_created_cards(
    ...
    * The card is linked as a ``task_links.child`` of the completing
      task â€” i.e. the worker explicitly called ``kanban_create`` with
    ...
    if row is None:
        # Completing task not found â€” nothing resolves.
        return [], ordered
...
def _scan_prose_for_phantom_ids(
    ...
    empty return means "no suspicious references found" â€” either the
...
def complete_task(
    ...
    *,
    ...
    completion (``zed kanban complete <id>``) works without requiring
    ...
    (e.g. ``{"changed_files": [...], "tests_run": [...]}``) â€” workers
    ...
    with write_txn(conn):
        ...
        # blocked â†’ done with no run in flight), synthesize a
        ...
        # second SQL round-trip. First line only, 400 char cap â€” the
        ...
        # ``metadata["artifacts"]`` â€” we promote it onto the event so
    ...
    # not resolve. Advisory â€” does not block the completion. Runs in
    ...
    if scan_text:
        ...
        # above (shouldn't happen â€” verified means they exist â€” but
        # belt-and-suspenders).
    ...
    # Successful completion â€” wipe the consecutive-failures counter.
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    A managed root is exclusively a ``workspaces/`` directory â€” never the
    broader kanban home, a board root, or sibling subtrees like ``logs/`` or
    ``boards/<slug>/`` itself. Allowed roots:

    * ``ZED_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
      injected by the dispatcher).
    * ``<kanban_home>/kanban/workspaces`` â€” legacy default-board scratch root.
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Zed' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Zed-managed storage. A board ``default_workdir`` pointing at a
...
def _cleanup_workspace(conn: sqlite3.Connection, task_id: str) -> None:
    ...
    Best-effort â€” any error is swallowed so cleanup never blocks task completion.
    ...
    try:
        ...
        # tasks now have all children done â€” their deferred cleanup can
        # proceed (#33774).
        _try_cleanup_parent_workspaces(conn, task_id)
    except Exception:
        pass  # best-effort â€” never block completion
...
def _try_cleanup_parent_workspaces(conn: sqlite3.Connection, task_id: str) -> None:
    ...
    try:
        ...
        for (parent_id,) in parents:
            ...
            # All children done â€” safe to clean up parent workspace
            import shutil
...
# ---------------------------------------------------------------------------
#
# Scratch workspaces are intentionally ephemeral â€” ``_cleanup_workspace``
...
#      via ``zed kanban show <id>`` and the dashboard.
#   3. Touch a sentinel file under ``kanban_home() / '.scratch_tip_shown'``
#      so we don't repeat the tip â€” once you know, you know.
#
...
_SCRATCH_TIP_MESSAGE = (
    "scratch workspaces are ephemeral â€” they're deleted when the task "
...
def _scratch_tip_shown() -> bool:
    ...
    install. Best-effort â€” any error means we re-emit, which is the safer
...
def _maybe_emit_scratch_tip(
    ...
    try:
        ...
        # Best-effort â€” never block the spawn loop over a help message.
        pass
    finally:
...
def unblock_task(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    runs invariant (``current_run_id IS NULL`` â‡” run row in terminal
...
def specify_triage_task(
    ...
    *,
    ...
    False when the task is missing or not in the ``triage`` column â€” callers
    ...
    ``title`` / ``body`` / ``assignee`` actually changed â€” avoids noisy
    ...
    with write_txn(conn):
        ...
        if changed_fields and author and author.strip():
            ...
            # already inside this function's write_txn â€” nested BEGIN
    ...
    # Outside the write_txn above, so we don't nest BEGIN IMMEDIATE â€” the
...
def decompose_triage_task(
    ...
    *,
    ...
    The root task stays alive and becomes the parent of every child â€”
    ...
    # add_comment) from inside this block â€” see architecture.md
    ...
    with write_txn(conn):
        ...
        # Create children. Status is 'todo' regardless of parents â€” we
        ...
        # Link the ROOT task as a child of every leaf child â€” i.e. the
    ...
    # stay in 'todo' until the user manually promotes them â€” useful
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``dir:<path>``: the path stored in ``workspace_path``.  Created
      if missing.  MUST be absolute â€” relative paths are rejected to
    ...
    if kind == "scratch":
        if task.workspace_path:
            ...
            # same absolute-path guard as dir: â€” consistent with the
            # threat model.
...
# Legacy alias â€” callers / tests still reference the old name.
...
# These errors won't resolve by retrying immediately â€” auto-block instead.
...
# without thrashing. Overridable via ``ZED_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    Operator-actionable â€” usually a misfiled task waiting for routing."""
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Zed
    ...
    operator-actionable failure â€” the task will be picked up on a
    ...
    Reasons: ``"blocker_auth"`` (quota/auth error â€” also auto-blocked),
    ``"recent_success"`` (completed run within guard window),
    ``"active_pr"`` (GitHub PR URL in a recent comment)."""
    rate_limited: list[str] = field(default_factory=list)
    """Task ids whose workers bailed on a provider rate-limit / quota wall
    (EX_TEMPFAIL sentinel exit) and were released back to ``ready`` WITHOUT
    counting a failure. These never trip the circuit breaker â€” a long quota
...
def _classify_worker_exit(pid: int) -> "tuple[str, Optional[int]]":
    ...
    * ``"clean_exit"`` â€” ``WIFEXITED`` with ``WEXITSTATUS == 0``. When the
      task is still ``running`` in the DB, this is a protocol violation
      (worker exited without calling ``kanban_complete`` / ``kanban_block``)
      and should be auto-blocked immediately â€” retrying will just loop.
    * ``"rate_limited"`` â€” ``WIFEXITED`` with status
      ``KANBAN_RATE_LIMIT_EXIT_CODE``. The worker bailed because the
      provider rate-limited / exhausted quota, NOT because the task failed.
      ``detect_crashed_workers`` releases the task back to ``ready`` without
      counting a failure, so a long quota window can't trip the breaker.
    * ``"nonzero_exit"`` â€” ``WIFEXITED`` with non-zero status. Real error.
    * ``"signaled"`` â€” ``WIFSIGNALED`` (OOM killer, SIGKILL, etc). Real crash.
    * ``"unknown"`` â€” pid was not in the reap registry (either reaped by
...
def _pid_alive(pid: Optional[int]) -> bool:
    ...
    **DO NOT** use ``os.kill(pid, 0)`` directly on Windows â€” Python's
    ...
    **Zombie handling:** the existence check succeeds against zombie
    ...
    Popen handle abandonment, so init reaps them quickly â€” but during
    ...
    if not _pid_exists(int(pid)):
        return False
    # Still here â†’ process exists. Check for zombie on platforms
            ...
            with open(f"/proc/{int(pid)}/status", "r", encoding="utf-8") as f:
                for line in f:
                    if line.startswith("State:"):
                        # "State:\tZ (zombie)" â†’ dead
                        if "Z" in line.split(":", 1)[1]:
                            return False
                        break
        except (FileNotFoundError, PermissionError, OSError):
            # proc entry gone â†’ already reaped; treat as dead.
...
def _terminate_reclaimed_worker(
    ...
    *,
    ...
    try:
        ...
        # Process is already gone â€” that's a successful termination, not a
...
def _worker_survived_termination(termination: dict) -> bool:
    ...
    spawn a second worker while the first is still running â€” the duplication
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``zed kanban tail``. The next dispatch
...
def enforce_max_runtime(
    ...
    *,
    ...
    dispatcher tick re-spawns it â€” unless the spawn-failure circuit
    ...
    for row in rows:
        ...
        if kill is not None:
            try:
                ...
                pass
            # Short polling wait â€” no time.sleep on the write txn.
            for _ in range(10):
                if not _pid_alive(pid):
                    break
                time.sleep(0.5)
        ...
        # breaker trips, this flips the task ``ready â†’ blocked`` and
...
# Heartbeat staleness heartbeat gap â€” if a running task hasn't sent a
...
def detect_stale_running(
    ...
    *,
    ...
    2. Its ``last_heartbeat_at`` is older than
       ``_STALE_HEARTBEAT_GAP_SECONDS`` (or NULL â€” never sent a heartbeat).
    ...
    for row in rows:
        ...
        if hb_age is not None and hb_age < _STALE_HEARTBEAT_GAP_SECONDS:
            continue  # recent heartbeat â†’ still alive
...
def detect_crashed_workers(conn: sqlite3.Connection) -> list[str]:
    ...
    Only considers tasks claimed by *this host* â€” PIDs from other hosts
    ...
    on the first occurrence â€” retrying a worker whose CLI keeps
    ...
    with write_txn(conn):
        ...
        for row in rows:
            ...
            if kind == "clean_exit":
                ...
                # ``running`` in the DB â€” it exited without calling
                # ``kanban_complete`` / ``kanban_block``. Retrying won't
                # help.
                protocol_violation = True
                error_text = (
                    "worker exited cleanly (rc=0) without calling "
                    "kanban_complete or kanban_block â€” protocol violation"
                )
                ...
                # quota (EX_TEMPFAIL sentinel). This is NOT a task failure â€”
                ...
                error_text = (
                    f"pid {pid} exited rate-limited (quota wall) â€” "
            ...
            if cur.rowcount == 1:
                # Rate-limited requeues are a clean release, not a crash â€”
                ...
                if rate_limited_exit:
                    ...
                    # respawn until the window clears â€” WITHOUT touching
    ...
    # ready â†’ blocked with a ``gave_up`` event on top of the ``crashed``
    ...
    # Same side-channel for rate-limited requeues â€” these did NOT count a
...
def _record_task_failure(
    ...
    * ``release_claim=True, end_run=True`` â€” spawn-failure path.
    ...
    * ``release_claim=False, end_run=False`` â€” timeout/crash path.
      ...
      ``ready â†’ blocked`` and a ``gave_up`` event is emitted.
    ...
    with write_txn(conn):
        ...
        if failures >= effective_limit:
            ...
            # Below threshold.
            if release_claim:
                # Spawn path: transition running â†’ ready + clear claim.
                conn.execute(
                    "UPDATE tasks SET status = 'ready', claim_lock = NULL, "
                    "claim_expires = NULL, worker_pid = NULL, "
                    "consecutive_failures = ?, last_failure_error = ? "
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``zed kanban
...
def _clear_failure_counter(conn: sqlite3.Connection, task_id: str) -> None:
    ...
    Called from ``complete_task`` on successful completion â€” a fresh
...
def check_respawn_guard(conn: sqlite3.Connection, task_id: str) -> Optional[str]:
    ...
    ``"rate_limit_cooldown"``
        ...
        the cooldown elapses â€” then allow a cheap probe. This is checked
    ...
    ``"blocker_auth"``
        ...
        blocks via the normal path â€” but a transient 429 gets a few
    ...
    Stale / dead claim locks are NOT a guard reason â€” they are handled
    ...
    #    (quota wall) â€” defer while inside the cooldown window, then allow a
    ...
    #    the regex would otherwise match â†’ defer forever (no failure counter
    ...
    if (
        ...
        if rl_cooldown <= 0:
            # Cooldown disabled â€” respawn immediately, and skip the
        ...
        # Cooldown elapsed â€” allow the respawn. Return early so the
    ...
    # 3. Completed run within guard window â€” proof of recent success.
    ...
    # 4. GitHub PR URL in a recent comment â€” prior worker already opened a PR.
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Zed profile.
    ...
    importable (e.g. partial install) â€” preserves the old behavior so
    ...
    if not rows:
        return False
    try:
        from zed_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
        # Can't introspect â€” assume spawnable, preserve legacy behavior.
        return True
    for row in rows:
...
def has_spawnable_review(conn: sqlite3.Connection) -> bool:
    ...
    Mirror of :func:`has_spawnable_ready` for the review column â€”
...
def dispatch_once(
    ...
    *,
    ...
    failures the task is auto-blocked with the last error as its reason â€”
    ...
    at any time across the whole board" â€” matching the gateway's stated
    ...
    # Rate-limited requeues (quota wall, no failure counted) â€” surface for
    ...
    # board, since "running" tasks aren't reclaimed by completion alone â€”
    ...
    # skipped_unassigned â€” the operator-actionable signal is different:
    ...
    # Normalize default_assignee once: empty/whitespace string â†’ None so the
    ...
    if _default_assignee:
        try:
            from zed_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Zed profile.
        # `_default_spawn` invokes ``zed -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Zed
        # profile. Those task lanes are pulled by terminals via
        # ``claim_task`` directly and should NEVER auto-spawn â€” the
        ...
        if profile_exists is not None and not profile_exists(row_assignee):
            ...
            # intended owner â€” a terminal lane). Health telemetry uses
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `zed kanban tail` â€” without
    ...
    # sdlc-review skill) that verifies the PR and either merges (â†’ done)
    # or rejects (â†’ back to running for the worker to fix).
    #
...
def _module_zed_argv() -> list[str]:
    """Return the interpreter-bound Zed CLI invocation."""
    # ``zed_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``zed`` package â€” there is no
    # ``zed`` package to import.
    return [sys.executable, "-m", "zed_cli.main"]


def _absolute_zed_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Zed shim."""
...
def _zed_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Zed executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_zed_argv()
    return [_absolute_zed_path(path)]


def _resolve_zed_argv() -> list[str]:
    """Resolve the ``zed`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$ZED_BIN`` â€” explicit operator override. Path-like values are
    ...
    2. ``shutil.which("zed")`` â€” the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\zed.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m zed_cli.main`` â€” fallback for setups where
       Zed is launched from a venv and the ``zed`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_zed_bin`` for the same reason. Kept
    local (not imported from gateway) because ``zed_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("ZED_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _zed_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _zed_path_argv(resolved_env_bin)
        return _module_zed_argv()

    zed_bin = _safe_which_no_cwd("zed") if _IS_WINDOWS else shutil.which("zed")
    if zed_bin:
        return _zed_path_argv(zed_bin)
    return _module_zed_argv()


def _kanban_worker_skill_available(zed_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``zed -p <name>``), its ``SKILLS_DIR``
    becomes ``<profile_home>/skills`` â€” which on many profiles does NOT contain
    ...
    # An unset ZED_HOME means the worker falls back to the default root
    # home (``~/.zed``), which ships the bundled skill.
    base = _Path(zed_home) if zed_home else (_Path.home() / ".zed")
...
def _resolve_worker_cli_toolsets(zed_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``ZED_KANBAN_TASK`` is set.
    """
    if not zed_home:
        return None
    try:
        ...
        token = set_zed_home_override(zed_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for ZED_HOME=%r (%s)",
            zed_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``zed -p <profile> chat -q ...`` subprocess.
    ...
    ``ZED_KANBAN_DB`` / ``ZED_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject ZED_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `zed -p <name>` the
    # _apply_profile_override() runs *before* zed_constants is imported.
    # If ZED_HOME is absent from the child's env, get_zed_home() falls
    # back to Path.home() / ".zed" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from zed_cli.profiles import resolve_profile_env
    try:
        env["ZED_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist â€” defer resolution to the CLI's
        # _apply_profile_override() via ZED_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # ZED_HOME never had profiles created.
        pass
    if task.tenant:
        env["ZED_TENANT"] = task.tenant
    env["ZED_KANBAN_TASK"] = task.id
    env["ZED_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["ZED_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["ZED_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["ZED_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["ZED_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["ZED_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`zed -p <name>`
    # rewrites ZED_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_zed_root()`
    # resolution in `kanban_home()` â€” symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["ZED_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["ZED_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug â€” the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["ZED_KANBAN_BOARD"] = resolved_board
    # ZED_PROFILE is the author the kanban_comment tool defaults to.
    # `zed -p <assignee>` activates the profile, but the env var is
    # what the tool reads â€” set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["ZED_PROFILE"] = profile_arg

    cmd = [
        *_resolve_zed_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped ZED_HOME above,
    ...
    # at a different/additional skill via config if they want â€”
    ...
    # fatal at CLI startup. Omitting it is safe â€” the lifecycle
    # contract still ships via KANBAN_GUIDANCE.
    if _kanban_worker_skill_available(env.get("ZED_HOME")):
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("ZED_HOME"))
    if worker_toolsets:
    ...
    # `zed kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`zed` executable not found on PATH. "
            "Install Zed Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``zed kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown:
        ...
        for offset, run in enumerate(shown):
            ...
            lines.append(f"### Attempt {idx} â€” {outcome} ({profile}, {ts})")
    ...
    if task.assignee:
        ...
        if role_rows:
            ...
            for row in role_rows:
                ...
                lines.append(f"- {row['id']} â€” {row['title']} ({ts}): {first}")
            lines.append("")
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled ZED_PROFILE values like "zed-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Zed root exists

    Reads profile paths directly so this module has no import dependency on
    ``zed_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from zed_constants import get_default_zed_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``zed kanban assignees`` for the terminal.
