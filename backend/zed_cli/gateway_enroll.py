"""``zed gateway enroll`` â€” enroll a self-hosted gateway with a relay connector.

The connectorâ‡„gateway channel is authenticated (the gateway may be
...
``docs/connector-gateway-auth-design.md``:
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.zed/.env``. The per-gateway secret authenticates the WS upgrade;
...
â‰¥2 Class-1 platforms validate the contract.
"""
...
def _resolve_connector_url(override: Optional[str]) -> Optional[str]:
    ...
    if not raw:
        return None
    raw = raw.rstrip("/")
    # The relay dial URL is ws(s)://â€¦/relay; enrollment posts to http(s)://â€¦/relay/enroll.
...
def cmd_gateway_enroll(args) -> None:
    ...
    # from inside such a container is a mistake â€” and save_env_value refuses to
    # write anyway.
    if is_managed():
        print(
            "âœ— `zed gateway enroll` is not available in a managed/hosted install.\n"
    ...
    if not enrollment_token:
        print(
            "âœ— No enrollment token. Pass --token <token> (or set "
    ...
    if not connector_base_url:
        print(
            "âœ— No connector URL. Pass --connector-url <url> (or set GATEWAY_RELAY_URL "
    ...
    try:
        ...
        print(f"âœ— Enrollment failed: {exc}")
        sys.exit(1)
    ...
    #    save_env_value writes them to ~/.zed/.env (0600 dir) and never logs.
    to_write = {
    ...
    for key, value in to_write.items():
        ...
        try:
            ...
            print(f"âœ— Failed to write {key} to .env: {exc}")
            sys.exit(1)

    from zed_cli.config import get_env_path

    print(f'âœ“ Enrolled gateway "{resolved_gateway_id}"' + (f" for tenant {tenant}" if tenant else ""))
    print()
