...
- **DeepSeek** accepts ``deepseek-chat`` (V3), ``deepseek-reasoner``
  ...
  Zed revisions folded every non-reasoner input into
  ``deepseek-chat``, which on aggregators routes to V3 â€” so a user
...
# that users might copy from marketing docs â€” it only accepts
...
_DEEPSEEK_CANONICAL_MODELS: frozenset[str] = frozenset({
    ...
    "deepseek-v4-pro",     # V4 Pro â€” first-class model ID
    "deepseek-v4-flash",   # V4 Flash â€” first-class model ID
})
...
def _normalize_provider_alias(provider_name: str) -> str:
    """Resolve provider aliases to Zed' canonical ids."""
...
def normalize_model_for_provider(model_input: str, target_provider: str) -> str:
    ...
    Args:
        ...
        target_provider: The canonical Zed provider id, e.g.
            ``"openrouter"``, ``"anthropic"``, ``"copilot"``,
            ``"deepseek"``, ``"custom"``.  Should already be normalised
            via ``zed_cli.models.normalize_provider()``.
    ...
    #     â€” commonly copied from aggregator slugs into fallback_model lists â€”
    ...
    if provider in {"copilot", "copilot-acp"}:
        try:
            from zed_cli.models import normalize_copilot_model_id
