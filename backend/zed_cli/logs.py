"""``zed logs`` â€” view and filter Zed log files.
...
Usage examples::
    ...
    zed logs                    # last 50 lines of agent.log
    zed logs -f                 # follow agent.log in real time
    zed logs errors             # last 50 lines of errors.log
    zed logs gateway -n 100    # last 100 lines of gateway.log
    zed logs gui -f            # follow gui.log (dashboard/pty/ws)
    zed logs desktop -f        # follow desktop.log (Electron app boot/backend)
    zed logs --level WARNING    # only WARNING+ lines
    zed logs --session abc123   # filter by session ID substring
    zed logs --component tools  # only tool-related lines
    zed logs --since 1h         # lines from the last hour
    zed logs --since 30m -f     # follow, starting 30 min ago
"""
...
# Known log files (name â†’ filename)
LOG_FILES = {
    "agent": "agent.log",
    "errors": "errors.log",
    "gateway": "gateway.log",
...
# Log line timestamp regex â€” matches "2026-04-05 22:35:00,123" or
# "2026-04-05 22:35:00" at the start of a line.
_TS_RE = re.compile(r"^(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2})")

# Level extraction â€” matches " INFO ", " WARNING ", " ERROR ", " DEBUG ", " CRITICAL "
_LEVEL_RE = re.compile(r"\s(DEBUG|INFO|WARNING|ERROR|CRITICAL)\s")

# Logger name extraction â€” after level and optional session tag, the next
...
def tail_log(
    ...
    *,
    ...
    log_path = get_zed_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Zed runs â€” try 'zed chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_zed_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_zed_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
    ...
    # Follow mode â€” poll for new content
    try:
...
def _read_last_n_lines(path: Path, n: int) -> list:
    ...
    try:
        ...
        # For files up to 1MB, just read the whole thing â€” simple and correct.
        if size <= 1_048_576:
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_zed_home()}/logs/")
        return

    print(f"Log files in {display_zed_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet â€” run 'zed chat' to generate logs)")
