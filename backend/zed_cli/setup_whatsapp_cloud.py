...
Entry point: ``zed whatsapp-cloud`` (dispatched from
``cmd_whatsapp_cloud`` in ``zed_cli/main.py``).
...
- Putting the actual phone number in ``WHATSAPP_CLOUD_PHONE_NUMBER_ID``
  ...
  This is the #1 trap â€” caught us during Phase 3 live testing.
...
The wizard intentionally does NOT smoke-test the webhook itself â€” the
Zed gateway and the cloudflared tunnel both run in separate
...
# reject obviously-malformed input before saving â€” saves users a round
...
def _validate_phone_number_id(value: str) -> tuple[bool, Optional[str]]:
    ...
    phone number (e.g. ``15556422442``) into this field â€” that's only
...
def _validate_app_secret(value: str) -> tuple[bool, Optional[str]]:
    ...
    if not re.fullmatch(r"[0-9a-f]+", s.lower()):
        return False, (
            ...
            "Settings â†’ Basic, not some other token."
        )
    if len(s) != 32:
...
def _validate_access_token(value: str) -> tuple[bool, Optional[str]]:
    ...
    if not s.startswith("EAA"):
        ...
        return False, (
            ...
            "you're copying from the right place (API Setup â†’ 'Generate "
            "access token', or Business Settings â†’ System Users â†’ "
...
def _prompt(message: str, default: Optional[str] = None, secret: bool = False) -> str:
    ...
    on empty input â€” callers handle the "user kept existing" case
...
def _prompt_validated(
    ...
    *,
    ...
    while True:
        attempts += 1
        value = _prompt(f"  â†’ {message}", default=current, secret=secret)
        if not value:
            return None
...
def run_whatsapp_cloud_setup() -> int:
    ...
    print()
    print("âš• WhatsApp Business Cloud API Setup")
    print("=" * 50)
    print()
    print("This wizard configures Zed to talk to WhatsApp via Meta's")
    print("official Cloud API. It's the production-grade path:")
    print()
    print("  â€¢ No QR codes, no Node.js bridge subprocess")
    print("  â€¢ Stable connection â€” no account-ban risk")
    print("  â€¢ Business account required (not personal WhatsApp)")
    print("  â€¢ Public webhook URL required (Cloudflare Tunnel, ngrok,")
    ...
    print()
    print("  1. https://developers.facebook.com/apps â†’ Create App")
    print("     â†’ 'Connect with customers through WhatsApp'")
    print("  2. App Dashboard â†’ WhatsApp â†’ API Setup")
    ...
    print()
    print("â”€" * 50)
    print("STEP 1 â€” Phone Number ID")
    print("â”€" * 50)
    ...
    phone_id = _prompt_validated(
        ...
        help_text=(
            "Found in: App Dashboard â†’ WhatsApp â†’ API Setup, in the\n"
            "'Send and receive messages' section.\n"
            "Look BELOW the 'From' dropdown â€” there's a 'Phone number ID'\n"
    ...
    if not phone_id:
        if current_phone_id:
            ...
            print(f"  âœ“ Keeping existing: {phone_id}")
        else:
            print("\nâœ— Phone Number ID is required. Aborting.")
            return 1
    else:
        save_env_value("WHATSAPP_CLOUD_PHONE_NUMBER_ID", phone_id)
        print(f"  âœ“ Saved: {phone_id}")
    print()

    print("â”€" * 50)
    print("STEP 2 â€” Access Token")
    print("â”€" * 50)
    ...
    token = _prompt_validated(
        ...
        help_text=(
            ...
            "  (a) TEMP â€” App Dashboard â†’ WhatsApp â†’ API Setup â†’\n"
            "      'Generate access token' button. Lasts 24 hours.\n"
            "      Fine for testing today; you'll have to regenerate\n"
            "      tomorrow.\n\n"
            "  (b) PERMANENT (production) â€” System User token. One-time\n"
            "      setup, never expires:\n"
            "      â€¢ business.facebook.com â†’ Settings â†’ System users â†’\n"
            "        Add â†’ Admin role\n"
            "      â€¢ Assign Assets â†’ your app (Manage app), your\n"
            "        WhatsApp account (Manage WABAs)\n"
            "      â€¢ Generate token â†’ expiration: Never â†’ permissions:\n"
    ...
    if not token:
        if current_token:
            ...
            print("\nâœ— Access Token is required. Aborting.")
            return 1
    else:
        save_env_value("WHATSAPP_CLOUD_ACCESS_TOKEN", token)
        print("  âœ“ Saved (token hidden)")
    print()

    print("â”€" * 50)
    print("STEP 3 â€” App Secret (required for webhook signature verification)")
    print("â”€" * 50)
    ...
    app_secret = _prompt_validated(
        ...
        secret=True,
        help_text=(
            "Found in: App Dashboard â†’ Settings â†’ Basic â†’\n"
    ...
    if not app_secret:
        if current_secret:
            ...
            print("  âœ“ Keeping existing App Secret")
        else:
            print("\nâš  Skipping App Secret â€” inbound webhooks will be refused")
            print("   until you set WHATSAPP_CLOUD_APP_SECRET manually.")
    else:
        save_env_value("WHATSAPP_CLOUD_APP_SECRET", app_secret)
        print("  âœ“ Saved (secret hidden)")
    print()

    print("â”€" * 50)
    print("STEP 4 â€” App ID & WABA ID (optional, for analytics)")
    print("â”€" * 50)
    ...
    app_id = _prompt_validated(
        ...
        help_text=(
            "Found in: App Dashboard â†’ Settings â†’ Basic â†’ 'App ID' at the\n"
            "top of the page. Numeric, ~15-16 digits.\n"
            "Not required for messaging â€” useful only for analytics later."
        ),
    )
    if app_id:
        save_env_value("WHATSAPP_CLOUD_APP_ID", app_id)
        print(f"  âœ“ Saved: {app_id}")
    elif current_app_id:
        print(f"  âœ“ Keeping existing: {current_app_id}")
    ...
    waba_id = _prompt_validated(
        ...
        help_text=(
            "WhatsApp Business Account ID. Found in: App Dashboard â†’\n"
            "WhatsApp â†’ API Setup, near the top â€” 'WhatsApp Business\n"
            "Account ID'. Numeric, ~15+ digits.\n"
            "Not required for messaging â€” useful for analytics."
        ),
    )
    if waba_id:
        save_env_value("WHATSAPP_CLOUD_WABA_ID", waba_id)
        print(f"  âœ“ Saved: {waba_id}")
    elif current_waba_id:
        print(f"  âœ“ Keeping existing: {current_waba_id}")
    print()

    print("â”€" * 50)
    print("STEP 5 â€” Verify Token (auto-generated)")
    print("â”€" * 50)
    ...
    if current_verify:
        ...
        if regen in {"y", "yes"}:
            ...
            print(f"  âœ“ New verify token: {verify_token}")
        else:
            verify_token = current_verify
            print("  âœ“ Keeping existing verify token")
    else:
        verify_token = secrets.token_urlsafe(32)
        save_env_value("WHATSAPP_CLOUD_VERIFY_TOKEN", verify_token)
        print(f"  âœ“ Generated: {verify_token}")
    print()
    print("  â†’ COPY THIS TOKEN NOW. You'll paste it into Meta's webhook")
    print("    configuration dialog (next step).")
    print()

    print("â”€" * 50)
    print("STEP 6 â€” Recipient Allowlist")
    print("â”€" * 50)
    print()
    print("  Who is allowed to message the bot? (Comma-separated phone")
    print("  numbers with country code, no '+' / spaces / dashes. Use '*'")
    print("  to allow anyone â€” only safe if you've also configured Meta's")
    ...
    try:
        allowed = input(
            f"  â†’ Allowed users{' [' + allow_default + ']' if allow_default else ''}: "
        ).strip() or (allow_default or "")
    except (EOFError, KeyboardInterrupt):
        allowed = ""
    if allowed:
        # Light normalization â€” strip spaces and dashes from each entry.
        allowed = ",".join(
            re.sub(r"[\s\-+]", "", part) for part in allowed.split(",") if part.strip()
        )
        save_env_value("WHATSAPP_CLOUD_ALLOWED_USERS", allowed)
        print(f"  âœ“ Saved: {allowed}")
    else:
        print("  âš  No allowlist â€” every inbound message will be denied.")
        print("    Re-run this wizard or set WHATSAPP_CLOUD_ALLOWED_USERS manually.")
    print()

    print("â”€" * 50)
    print("SETUP COMPLETE â€” Next steps")
    print("â”€" * 50)
    print()
    print("  Zed needs a public HTTPS URL to receive WhatsApp messages.")
    ...
    print()
    print("    3. Start the Zed gateway in another terminal:")
    print("         zed gateway")
    print()
    ...
    print("         App Dashboard â†’ WhatsApp â†’ Configuration â†’ Edit webhook")
    print("         Callback URL: <tunnel-url>/whatsapp/webhook")
    print(f"         Verify Token: {verify_token}")
    print("         â†’ Click 'Verify and save'")
    print("         â†’ Then 'Manage' webhook fields â†’ subscribe to 'messages'")
    print()
    print("    6. Add your phone to Meta's recipient list:")
    print("         App Dashboard â†’ WhatsApp â†’ API Setup â†’ 'To' â†’")
    ...
    print("  Business Manager, not via this wizard â€” but here's where to do")
    print("  it once you're up and running:")
    print()
    effective_waba = waba_id or current_waba_id
    if effective_waba:
        print("    â€¢ Display name + profile picture:")
    ...
    print()
    print("    â€¢ About, description, website, hours, business category:")
    print("        Same page â†’ click your phone number â†’ 'Edit profile'.")
    print()
    print("    â€¢ Verified badge (the green check):")
