"""
Interactive setup wizard for Zed Agent.
...
# display_zed_home imported lazily at call sites (stale-module safety during zed update)
...
from zed_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from zed_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("âš• Zed Setup â€” Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Zed using environment variables or config commands:")
    print_info("  zed config set model.provider custom")
    print_info("  zed config set model.base_url http://localhost:8080/v1")
    print_info("  zed config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'zed setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'zed setup')")


def _print_setup_summary(config: dict, zed_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'zed setup' to configure"))

    # Mixture of Agents â€” requires OpenRouter specifically (calls multiple models)
    ...
    # Video generation â€” opt-in via `zed tools` â†’ Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from zed_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS â€” not installed)", False, "run 'zed setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS â€” not installed)", False, "run 'zed setup tts'"))
    else:
    ...
    # Spotify (OAuth via zed auth spotify â€” check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'zed setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {zed_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('zed setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('zed setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('zed setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('zed setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('zed setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('zed config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('zed config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('zed config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('zed', Colors.GREEN)}              Start chatting")
    print(f"   {color('zed gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('zed doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `zed tools` and `zed setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``zed model``)
    ...
    provider added to ``zed model`` is automatically available here.
    ...
    # Delegate to the shared zed model flow â€” handles provider picker,
    # credential prompting, model selection, and config persistence.
    from zed_cli.main import select_provider_and_model
    try:
    ...
    # on demand via `zed auth add`, `zed setup` vision, and
    # `zed setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``zed model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.zed/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Zed auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via zed setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'zed setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Zed runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `zed setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `zed setup terminal`.
        ...
        if use_managed_modal:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    print_warning("Install failed â€” run manually: pip install modal")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed â€” run manually: pip install daytona")
                if result.stderr:
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into ZED_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("ZED_MAX_ITERATIONS")
    ...
    print_info("  Run `zed setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # ZED_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from zed_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_zed_home(Path(get_zed_home()))
    except Exception:
        pass
...
def _profile_name_from_zed_home(zed_home) -> str | None:
    """Return the active profile name when ZED_HOME is a profile dir."""
    if zed_home.parent.name == "profiles":
        return zed_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("ðŸ“¬ Home Channel: where Zed delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `zed update`)",
                True,
            ):
    ...
    print_info("   1. Go to https://api.slack.com/apps â†’ Create New App")
    print_info("      Pick 'From an app manifest' â€” we'll generate one for you below.")
    print_info("   2. Enable Socket Mode: Settings â†’ Socket Mode â†’ Enable")
    print_info("      â€¢ Create an App-Level Token with 'connections:write' scope")
    ...
    print()
    print_info("ðŸ”’ Security: Restrict who can use your bot")
    print_info("   To find a Member ID: click a user's name â†’ View full profile â†’ â‹® â†’ Copy member ID")
    print()
    allowed_users = prompt(
    ...
    if allowed_users:
        ...
        print_warning("âš ï¸  No Slack allowlist set - unpaired users will be denied by default.")
    ...
    print_info("   the channel name â†’ Copy link â€” the ID starts with C (e.g. C01ABC2DE3F).")
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under ZED_HOME, and print
    paste-into-Slack instructions.

    Exposed as its own helper so both the initial setup flow and the
    "reconfigure? â†’ no" branch can refresh the manifest without the user
    re-entering tokens. Failures are non-fatal â€” if the manifest write
    fails for any reason, we print a warning and skip rather than abort
    the whole Slack setup.
    """
    try:
        from zed_cli.slack_cli import _build_full_manifest
        from zed_constants import get_zed_home

        manifest = _build_full_manifest(
            bot_name="Zed",
            bot_description="Your Zed agent on Slack",
        )
        ...
        print_info(
            "   Paste it into https://api.slack.com/apps â†’ your app â†’ Features "
            "â†’ App Manifest â†’ Edit, then Save.  Slack will prompt to "
            "reinstall if scopes or slash commands changed."
        )
        print_info(
            "   Re-run `zed slack manifest --write` anytime to refresh after "
            "Zed adds new commands."
        )
...
def _setup_matrix():
    ...
    if token:
        ...
        user_id = prompt("User ID (@bot:server â€” optional, will be auto-detected)")
        if user_id:
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        # asyncpg, aiohttp-socks) â€” not just mautrix itself.  The previous
        ...
        try:
            ...
            if _missing_before:
                ...
                try:
                    ...
                    print_warning(
                        f"Install failed â€” run manually: pip install "
                        f"'mautrix[encryption]' asyncpg aiosqlite Markdown "
                        f"aiohttp-socks"
                    )
                    print_info(f"  Error: {exc}")
        except ImportError:
            # tools.lazy_deps unavailable (extreme edge case â€” partial
        ...
        if allowed_users:
            ...
            print_info("âš ï¸  No allowlist set - anyone who can message the bot can use it!")

        print()
        print_info("ðŸ“¬ Home Room: where Zed delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Zed to iMessage via BlueBubbles â€” a free, open-source")
...
def _setup_qqbot():
    ...
    from zed_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from zed_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Zed from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'zed setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     zed config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_zed_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo zed gateway install --system")
                print_info("  Or run in foreground:  zed gateway")
        else:
            from zed_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   zed gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   zed gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `zed setup tools` and `zed tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``zed_cli.auth`` â€” lists every supported
    ...
    """
    try:
        from zed_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from zed_cli.config
    ...
    if section_key == "model":
        ...
        from zed_cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_zed migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_zed", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Zed.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "âš  Gateway/messaging â€” this will configure Zed to use your OpenClaw messaging channels",
    "telegram": "âš  Telegram â€” this will point Zed at your OpenClaw Telegram bot",
    "slack": "âš  Slack â€” this will point Zed at your OpenClaw Slack workspace",
    "discord": "âš  Discord â€” this will point Zed at your OpenClaw Discord bot",
    "whatsapp": "âš  WhatsApp â€” this will point Zed at your OpenClaw WhatsApp connection",
    "config": "âš  Config values â€” OpenClaw settings may not map 1:1 to Zed equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Zed config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Zed.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" â‰  Zed's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(zed_home: Path) -> bool:
    ...
    print_info("Zed can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: zed claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: zed claw migrate"
        )
        print_info(
    ...
    # Execute the migration â€” overwrite=False so existing Zed configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Zed config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Zed (use zed claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Zed session â€” model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub â€” without being told to run
    ``zed setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``zed model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``zed portal`` always offers a model picker,
    ...
    # setup calls, so `zed portal` == quick setup's Nous step.
    try:
    ...
    print_info("  Run `zed portal info` to inspect routing.")
    print_info("  Run `zed` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      zed setup           â€” full or quick (auto-detected)
      zed setup model     â€” just model/provider
      zed setup tts       â€” just text-to-speech
      zed setup terminal  â€” just terminal backend
      zed setup gateway   â€” just messaging platforms
      zed setup tools     â€” just tool configuration
      zed setup agent     â€” just agent settings
    """
    from zed_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"â”‚     âš• Zed Setup â€” {label:<34s} â”‚", Colors.MAGENTA))
                print(
                    color(
                        "â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "â”‚             âš• Zed Agent Setup Wizard                â”‚", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "â”‚  Let's configure your Zed Agent installation.       â”‚", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, zed_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Zed configured.")
        print_info("Running the full wizard â€” each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'zed setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(zed_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, zed_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {zed_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'zed config edit'")
    ...
    # Tune later with `zed setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, zed_home)
...
def _run_first_time_quick_setup(config: dict, zed_home, is_existing: bool):
    ...
    or switch providers with ``zed model``.
    """
    from zed_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip â€” set up later with 'zed setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    zed setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  zed setup gateway")
    print()

    _print_setup_summary(config, zed_home)


def _run_quick_setup(config: dict, zed_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'zed setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Zed to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'zed setup gateway'.")
