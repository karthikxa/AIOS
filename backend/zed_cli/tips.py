...
TIPS = [
    ...
    "/title \"my project\" names your session â€” resume it later with /resume or zed -c.",
    ...
    "Ctrl+Z suspends Zed to the background â€” run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "zed -c resumes your most recent CLI session. zed -c \"project name\" resumes by title.",
    "zed -w creates an isolated git worktree â€” perfect for parallel agent workflows.",
    "zed -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "zed chat -t web,terminal enables only specific toolsets for a focused session.",
    "zed chat -s github-pr-workflow preloads a skill at launch.",
    "zed chat -q \"query\" runs a single non-interactive query and exits.",
    "zed chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "zed chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "zed --yolo bypasses all dangerous command approval prompts for the entire session.",
    "zed chat --source telegram tags the session for filtering in zed sessions list.",
    "zed -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "zed doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "zed dump outputs a compact setup summary â€” great for bug reports.",
    "zed config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "zed config edit opens config.yaml in your default editor.",
    "zed config check scans for missing or stale configuration options.",
    "zed sessions browse opens an interactive session picker with search.",
    "zed sessions stats shows session counts by platform and database size.",
    "zed sessions prune --older-than 30 cleans up old sessions.",
    "zed skills search react --source skills-sh searches the skills.sh public directory.",
    "zed skills check scans installed hub skills for upstream updates.",
    "zed skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "zed skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "zed mcp add github --command npx adds MCP servers from the command line.",
    "zed mcp serve runs Zed itself as an MCP server for other agents.",
    "zed auth add lets you add multiple API keys for credential pool rotation.",
    "zed completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "zed logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "zed backup creates a zip backup of your entire Zed home directory.",
    "zed profile create coder creates an isolated profile that becomes its own command.",
    "zed profile create work --clone copies your current config and keys to a new profile.",
    "zed update syncs new bundled skills to ALL profiles automatically.",
    "zed gateway install sets up Zed as a system service (systemd/launchd).",
    "zed memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "zed webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: zed tools disables unused tools, zed skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) â€” faster, cheaper responses.",
    "zed models routes vision, compression, and aux tasks to cheaper models â€” cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as zed -w).",
    ...
    "ZED_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Zed tools programmatically â€” results stay out of context.",
    ...
    "Profile names become shell commands â€” 'zed profile create coder' creates the 'coder' command.",
    "zed profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Zed prints a resume command with session ID and stats.",
    "zed sessions export backup.jsonl exports all sessions for backup or analysis.",
    "zed -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "zed skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .zed/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: zed cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Zed runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "zed gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode â€” no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Zed via a local macOS server.",
    ...
    "Context files (.zed.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md is the agent's primary identity file â€” customize it to shape behavior.",
    "Zed loads project context from .zed.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "zed mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "zed mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "zed mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime â€” zed tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.zed/checkpoints/ â€” your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "zed chat -Q enables quiet mode for programmatic use â€” suppresses banner and spinner.",
    ...
    "zed plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "zed acp runs Zed as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "ZED_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "zed auth add nous or zed auth add openai-codex sets up OAuth-based providers.",
    ...
    "zed status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.zed/scripts/ and run before the agent â€” perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity â€” rewrite it to make Zed your own.",
    ...
    "Before a gateway session reset, Zed auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.zed/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Zed state via ZED_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "ZED_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in ZED_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording â†’ cancel prompts â†’ cancel picker â†’ interrupt agent â†’ exit.",
    "Every interrupt during an agent run is logged to ~/.zed/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries â€” 'zed profile create ls' would be rejected.",
    "zed profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "zed claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    '/goal <text> sets a standing Ralph-loop objective â€” Zed auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Zed config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Zed is working.',
    ...
    '/reload reloads ~/.zed/.env into the running session â€” pick up new API keys without restarting.',
    ...
    "ZED_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.zed/hooks/<name>/ with HOOK.yaml + handler.py â€” handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.zed/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'zed curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "zed curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'zed curator rollback restores skills from a pre-run snapshot â€” backups live under skills/.curator_backups/.',
    ...
    # --- TUI & Dashboard ---
    'ZED_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch â€” handy after SSH drops.',
    "ZED_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    'Ctrl+G or Ctrl+X Ctrl+E in the TUI opens the input buffer in $EDITOR for long multi-line prompts.',
    'The TUI renders LaTeX inline â€” $E=mc^2$ becomes Unicode math instead of raw TeX.',
    'zed dashboard launches a local web UI at 127.0.0.1:9119 â€” zero data leaves localhost.',
    'zed dashboard embeds the full Zed TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.zed/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.zed/dashboard-plugins/ â€” no npm build required.',
    ...
    'ZED_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'ZED_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'ZED_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'ZED_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU â€” voices auto-download to ~/.zed/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.zed/skills/ so drop-in skills appear without restarting the session.',
    ...
    'zed -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else â€” ideal for piping in scripts.',
    'zed chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'zed chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'zed chat --ignore-user-config skips the active user config â€” reproducible bug reports and CI runs.',
    "zed chat --source tool tags programmatic chats so they don't clutter zed sessions list.",
    'zed dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'zed sessions rename <ID> "new title" renames any past session; zed sessions delete <ID> removes one.',
    'zed import restores a session export or profile archive produced by sessions export or profile export.',
    'zed fallback manages the fallback_model chain interactively â€” no hand-editing config.yaml.',
    'zed pairing rotates the DM pairing token â€” the first messager after rotation claims access to the bot.',
    'zed setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'zed status --deep runs the full health sweep across every component; plain zed status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'ZED_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent â€” use for long research runs.',
    'ZED_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.zed/plugins/ â€” trust-gated by design.',
    "ZED_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'ZED_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks â€” off by default in gateway mode.',
    'ZED_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'ZED_BUNDLED_SKILLS points at a custom bundled-skill tree â€” used by Homebrew and Nix packaging.',
    'ZED_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'ZED_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'ZED_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'ZED_GATEWAY_BUSY_ACK_ENABLED=false silences the âš¡/â³/â© ack messages when a user messages a busy agent.',
    'ZED_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'ZED_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'ZED_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation â€” raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Zed block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from zed sessions list by default â€” set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ â€” drop files into ~/.zed/dashboard-plugins/.',
]
