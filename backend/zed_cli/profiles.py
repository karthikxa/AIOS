"""
Profile management for multiple isolated Zed instances.

Each profile is a fully independent ZED_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.zed/profiles/<name>/`` by default.

The "default" profile is ``~/.zed`` itself â€” backward compatible,
zero migration needed.

Usage::

    zed profile create coder          # fresh profile + bundled skills
    zed profile create coder --clone  # also copy config, .env, SOUL.md, skills
    zed profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    zed -p coder chat                 # or via flag
    zed profile use coder             # set as sticky default
    zed profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See zed_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.zed``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   zed-agent  â€” git repo checkout (~84 MB source + ~3 GB venv)
#   .worktrees    â€” git worktrees
#   profiles      â€” sibling named profiles (recursive copy never intended)
#   bin           â€” installed binaries (tirith etc., ~10 MB) shared per-host
#   node_modules  â€” npm packages (hundreds of MB)
#
...
# source profile.  A new profile is a fresh workspace â€” inheriting the source
...
#
# Rationale per item:
#   state.db (+wal/shm) â€” SQLite session store (can reach many GB)
#   sessions            â€” per-session transcript/data dirs
#   backups             â€” `zed backup` archives
#   state-snapshots     â€” quick-backup snapshot trees
#   checkpoints         â€” session checkpoint data
...
# Marker file written by `zed profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `zed
...
# `zed skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` â€” known
         Zed infrastructure directories that only the default profile
         (``~/.zed``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_zed_home().resolve()
    ...
    def _ignore(directory: str, names: List[str]) -> List[str]:
        ...
        for entry in names:
            ...
            try:
                ...
                # symlinks, missing parents).  Fail open â€” better to
...
# Directories/files to exclude when exporting the default (~/.zed) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "zed-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "zed", "default", "test", "tmp", "root", "sudo",
})

# Zed subcommands that cannot be used as profile names/aliases
_ZED_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the zed root, NOT to the current ZED_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ZED_HOME points outside
    ``~/.zed``, profiles live under ``ZED_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_zed_home() / "profiles"


def _get_default_zed_home() -> Path:
    """Return the default (pre-profile) ZED_HOME path.

    In standard deployments this is ``~/.zed``.
    In Docker/custom deployments where ZED_HOME is outside ``~/.zed``
    (e.g. ``/opt/data``), returns ZED_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_zed_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            f"Profile name {name!r} is reserved â€” it collides with either "
            f"the Zed installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ZED_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_zed_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, zed subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _ZED_SUBCOMMANDS:
        return f"'{canon}' conflicts with a zed subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            zed_exe = shutil.which("zed") or "zed"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(zed_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Zed. If we
    ...
    try:
        ...
        # not be migrated. The next `zed doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``zed -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``zed profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``zed profile list`` for an unrelated profile.
    """
...
# ---------------------------------------------------------------------------
# profile.yaml â€” per-profile metadata (description, role, etc.)
# ---------------------------------------------------------------------------
#
# We keep this file deliberately tiny and separate from the profile's
# ``config.yaml``. ``config.yaml`` is the user-facing Zed config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_zed_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, zed_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own ZED_HOME.
    ...
    The returned ``zed_home`` is the path to pass to
    ``set_zed_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_zed_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``zed update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' â€” it is the built-in profile (~/.zed)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.zed/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `zed -p <name> auth add`) had no file until first
    # write, and the profile silently inherited API keys from the shell
    # environment â€” users reasonably read that as "the new profile reads
    # the root .env". Skipped when --clone/--clone-all already copied one.
    env_path = profile_dir / ".env"
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Zed profile.\n"
            ...
            pass  # best-effort â€” save_env_value creates the file on demand
    ...
    # Write the opt-out marker so seed_profile_skills() and `zed update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`zed profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `zed update`.\n",
                encoding="utf-8",
            )
        except OSError:
            pass  # best-effort â€” the feature still works via the empty skills/ dir

    # Cloned configs can be older than the running Zed (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal â€” user can describe later with `zed profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `zed -p <profile> gateway start` can supervise it via
    # `s6-svc -u` instead of spawning a bare process. On host (systemd
    # / launchd / windows) this is a no-op â€” the existing per-profile
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ZED_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``zed profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ZED_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_zed_home() / ".env"
    ...
    for entry in sorted(profiles_root.iterdir()):
        ...
        try:
            ...
            if not quiet:
                print(f"âš  Could not seed .env for profile '{entry.name}': {e}")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.zed).\n"
            "To remove everything, use: zed uninstall"
        )
    ...
    try:
        def _make_writable(func, path, exc):
            ...
            #   onexc(func, path, exc_instance)   â€” 3.12+
            #   onerror(func, path, exc_info_tuple) â€” 3.11
            if isinstance(exc, tuple):
                exc = exc[1]  # exc_info â†’ actual exception object
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``ZED_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        if detect_service_manager() != "s6":
            return  # host path â€” silent, no registration needed
        from zed_cli.service_manager import get_service_manager
        mgr = get_service_manager()
    except RuntimeError:
        return  # no backend on this host â€” nothing to do
    except Exception:
    ...
    try:
        ...
        print(f"âš  Could not register s6 gateway service: {exc}")
...
def _maybe_unregister_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        print(f"âš  Could not unregister s6 gateway service: {exc}")
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ZED_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ZED_HOME")
    try:
        os.environ["ZED_HOME"] = str(profile_dir)
        from zed_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ZED_HOME"] = old_home
        elif "ZED_HOME" in os.environ:
            del os.environ["ZED_HOME"]
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    try:
        ...
        # primitive (taskkill / TerminateProcess) â€” raw os.kill with
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.zed/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: zed profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ZED_HOME.

    Returns ``"default"`` if ZED_HOME is not set or points to ``~/.zed``.
    Returns the profile name if ZED_HOME points into ``~/.zed/profiles/<name>``.
    Returns ``"custom"`` if ZED_HOME is set to an unrecognized path.
    """
    from zed_constants import get_zed_home
    zed_home = get_zed_home()
    resolved = zed_home.resolve()

    default_resolved = _get_default_zed_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.zed itself â€” its parent is ~/ and its
        # directory name is ".zed", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: zed profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.zed itself â€” disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' â€” that is the built-in root profile (~/.zed). "
            "Specify a different name: zed profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="zed_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"zed_{old_name}"
    legacy_old_host = f"zed.{old_name}"
    new_host = f"zed_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_zed_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if new_host in hosts:
            print(f"âš  Honcho host block not migrated: {new_host} already exists in {path}")
            continue

        block = hosts[source_host]
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("zed_"):
        ...
        print(f"âœ“ Honcho host updated: {source_host} â†’ {new_host}")
...
def rename_profile(old_name: str, new_name: str) -> Path:
    ...
    if not collision:
        ...
        print(f"âœ“ Alias updated: {new_canon}")
    else:
        print(f"âš  Cannot create alias '{new_canon}' â€” {collision}")

    # 5. Update active_profile if it pointed to old name
    try:
        if get_active_profile() == old_canon:
            set_active_profile(new_canon)
            print(f"âœ“ Active profile updated: {new_canon}")
    except Exception:
        pass
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ZED_HOME path string.

    Called early in the CLI entry point, before any zed modules
    are imported, to set the ZED_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
