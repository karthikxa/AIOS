...
the existing module-level functions in zed_cli.gateway and
zed_cli.gateway_windows directly. This protocol is a thin facade
...
``zed gateway start/stop/restart`` when running inside a container.
"""
...
# Profile name â†’ service directory mapping. Profile names must be safe
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``zed gateway start/stop/restart``).
    """
    ...
    # `zed gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged zed
    ...
    detection always fails. Since every Zed runtime call inside the
    container drops to zed via ``s6-setuidgid``, that silent failure
...
# in ``zed_cli.gateway`` (systemd/launchd) and ``zed_cli.gateway_windows``
# (Windows Scheduled Tasks). The protocol's ``name`` parameter is currently
# unused for host backends â€” they operate on whichever profile is currently
# active (set via the ``zed -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in zed_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from zed_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from zed_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from zed_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from zed_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in zed_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from zed_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from zed_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from zed_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from zed_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``zed_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `zed profile create`
# runs inside the container (Phase 4). Static services (main-zed, dashboard)
# live in /etc/s6-overlay/s6-rc.d/ and are NOT managed by this class â€” they're
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's ZED_HOME blindly;
    derive the shared profile root from the current ZED_HOME and map the
    ...
    zed_home = Path(os.environ.get("ZED_HOME", "/opt/data"))
    if zed_home.parent.name == "profiles":
        root = zed_home.parent.parent
    else:
        root = zed_home
...
# that runs via `docker exec` or any other out-of-tree entry point â€”
# notably our Phase 4 profile create/delete hooks â€” inherits the
...
# constant. We don't use `/usr/bin/s6-â€¦` symlinks because the
# s6-overlay-symlinks-noarch tarball only links a subset, and we
# want every s6 invocation to be guaranteed-findable.
_S6_BIN_DIR = "/command"


# UID/GID of the in-image ``zed`` user. Hardcoded to match what
# ``stage2-hook.sh`` enforces (the runtime invariant â€” see also
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the zed user.
    ...
    (the ``zed`` user â€” UID 10000 â€” running every Zed runtime
    ...
    with zed ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           zed:zed, 0755 (parent must already exist)
    ``svc_dir/event/``                     zed:zed, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 zed:zed, 0755
    ``svc_dir/supervise/event/``           zed:zed, 03730
    ``svc_dir/supervise/control``          zed:zed, 0660    (FIFO)
    ...
    slot's supervise/ was zed-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _ZED_UID, _ZED_GID)
        except PermissionError:
            # Running as the zed user already â€” directory is zed-
    ...
    # (e.g. the default 0022 on most dev hosts â†’ 0o660 becomes 0o640).
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _ZED_UID, _ZED_GID)
        except PermissionError:
            pass

    # If a log/ subdir is present (the canonical s6 logger pattern â€”
    ...
    # when the parent slot's supervise/ is zed-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _ZED_UID, _ZED_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``zed -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`zed profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-zed, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources ZED_HOME (and any extra env) via with-contenv â€”
             so e.g. ``-e ZED_HOME=/data/zed`` is honored at run
          ...
          4. Drops to the zed user and exec's
             ``zed -p <profile> gateway run`` (or just ``zed
             gateway run`` for the default profile â€” see below).

        Special case: ``profile == "default"`` emits ``zed gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root ZED_HOME profile" (the implicit profile that exists at
        the top of $ZED_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``zed_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$ZED_HOME/profiles/default/`` â€” a
        ...
        each supervised profile gateway loads its own ``ZED_HOME``,
        ...
        allocator in ``zed_cli.profiles._allocate_gateway_port``).
        ...
        # `_gateway_command_inner` with subcmd == "run" â€” without it the
        ...
        # guard.
        lines.append("export ZED_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's ZED_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``zed gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-ZED_HOME PID lock first;
        # the supervised slot then execs a bare ``gateway run``, hits the
        # "Another gateway instance is already running" guard, exits
        # non-zero, and s6 restarts it â€” a restart loop that floods the
        # log and never binds (NS-505). ``--replace``
        # instead reaps the stale holder (hardened takeover path: marker +
        # SIGTERMâ†’SIGKILL-with-confirmation + scoped-lock cleanup, see
        # gateway/run.py) so s6 always wins. The ZED_S6_SUPERVISED_CHILD
        # sentinel above prevents the runâ†’startâ†’run redirect recursion.
        # Each profile is scoped to its own ZED_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "zed gateway run --replace"
        else:
            gateway_cmd = f"zed -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID â†’
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid zed {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${ZED_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the ZED_HOME path is sourced from the runtime env
        via with-contenv â€” NOT Python-substituted at registration time
        â€” so a container started with ``-e ZED_HOME=/data/zed``
        gets its logs under /data/zed/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` â€” also write a timestamped copy to the
             ...
             files). This is what ``zed logs`` reads and what
        ...
        return (
            ...
            f': "${{ZED_HOME:=/opt/data}}"\n'
            f'log_dir="$ZED_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            # The gateways/ parent must be chowned too (non-recursively):
            # `mkdir -p` creates it root-owned on a root-context boot, and a
            # leaf-only chown leaves it that way â€” every profile registered
            # later then runs its log service as zed and crash-loops on
            ...
            f'chown zed:zed "$ZED_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R zed:zed "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid zed s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``zed gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``zed -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with zed ownership
            ...
            # the zed user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # Stop the service (best effort â€” service may already be down).
        subprocess.run(
        ...
        # "scan completed" handshake â€” the -a/-n trigger just sets a
        ...
        # by s6-supervise itself) â€” but the parent supervise/ directory
        # is zed-owned (see ``_seed_supervise_skeleton``), and on
