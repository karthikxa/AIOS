"""
Doctor command for zed CLI.

Diagnoses issues with Zed Agent setup.
"""
...
from zed_cli.config import get_project_root, get_zed_home, get_env_path
from zed_cli.env_loader import load_zed_dotenv
from zed_constants import display_zed_home

PROJECT_ROOT = get_project_root()
ZED_HOME = get_zed_home()
_DHH = display_zed_home()  # user-facing display path (e.g. ~/.zed or ~/.zed/profiles/coder)

# Load environment variables from ~/.zed/.env so API key checks work
_env_path = get_env_path()
load_zed_dotenv(zed_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from zed_cli.colors import Colors, color
from zed_cli.models import _ZED_USER_AGENT
from zed_constants import OPENROUTER_MODELS_URL
...
from zed_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.zed/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("ZED_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from zed_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from zed_cli.auth import get_minimax_oauth_auth_status
...
def _section(title: str) -> None:
    """Print a doctor section banner: blank line + bold cyan â—† title."""
    print()
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches zed_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``zed --version`` reporting a stale version while
    ...
    """
    try:
        from zed_cli import __version__ as init_version
    except Exception:
        return
    pyproject_version = _read_pyproject_version()
    if pyproject_version is None:
        # Installed wheel or unreadable pyproject â€” nothing to cross-check.
        return
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != zed_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'zed update', or set "
            "zed_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-zed and dashboard static services are up
    ...
    for static in ("main-zed", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet â€” create one with `zed profile create <name>`")
        return
...
def _check_gateway_service_linger(issues: list[str]) -> None:
    ...
    Skipped inside a container running under s6 â€” the linger concept
...
def _build_apikey_providers_list() -> list:
    ...
    already present â€” adding plugins/model-providers/<name>/ is sufficient to get into doctor.
    """
    _static = [
    ...
    try:
        ...
        try:
            from zed_cli.providers import normalize_provider as _normalize_provider
        ...
        for _pp in list_providers():
            ...
            if _candidates & _dedicated_canonical:
                continue
            # Separate API-key vars from base-URL override vars â€” the health-check
...
def managed_scope_check() -> None:
    ...
    resolved from the ZED_MANAGED_DIR override (rather than the system
    default), that is surfaced too â€” a redirected scope is the documented
    foot-gun (see docs/design/managed-scope.md Â§7) and an operator should see it.
    """
    try:
        from zed_cli import managed_scope
        managed_dir = managed_scope.get_managed_dir()
    except Exception:  # noqa: BLE001 â€” diagnostics must never crash
        return
    ...
    if os.environ.get("ZED_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via ZED_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `zed`.
    os.environ.setdefault("ZED_INTERACTIVE", "1")

    # Handle `zed doctor --ack <id>` as a fast path. Persist the ack and
    # return without running the rest of the diagnostics â€” the user has
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            print(color(
                f"  âœ“ Acknowledged advisory {ack_target}. "
                f"It will no longer trigger startup banners.",
                Colors.GREEN,
            ))
        else:
            print(color(
                f"  âœ— Failed to persist ack for {ack_target}. "
                f"Check ~/.zed/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("â”‚                 ðŸ©º Zed Doctor                        â”‚", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`zed doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and zed_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.zed/.env (primary location for user config)
    ...
    if env_path.exists():
        ...
        # defaults to the system locale â€” which crashes on non-UTF-8 Windows
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'zed setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                env_path.touch()
                # .env holds API keys â€” restrict to owner-only access from
                ...
                check_info("Run 'zed setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'zed setup' to create one")
                issues.append("Run 'zed setup' to create .env")
    
    # Check ~/.zed/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from zed_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from zed_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'zed config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            # provider â€” bare "custom" or a named "custom:<name>" that fronts an OpenAI-compatible
            ...
            # Limit to API-key providers in PROVIDER_REGISTRY â€” other provider
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from zed_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.zed/.env or run 'zed setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'zed setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'zed config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from zed_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from zed_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'zed setup' to migrate config")
                else:
                    issues.append("Run 'zed doctor --fix' or 'zed setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml â€” run 'zed doctor --fix'")
        except Exception:
            pass

        # Detect stale ZED_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives ZED_MAX_ITERATIONS from agent.max_turns
        ...
        # wrong budget â€” e.g. config says 400 but the activity line reads N/90.
        # Read the .env FILE directly (load_env), not get_env_value/os.environ,
        # which the startup bridge may already have overridden.
        try:
            import yaml
            from zed_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            env_ghost = load_env().get("ZED_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"ZED_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `zed setup` run)",
                )
                if should_fix:
                    if remove_env_value("ZED_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale ZED_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove ZED_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the ZED_MAX_ITERATIONS line from "
                            f"{_DHH}/.env â€” config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale ZED_MAX_ITERATIONS in .env shadows config.yaml â€” "
                        "run 'zed doctor --fix'"
                    )
        except Exception:
            pass
    ...
    try:
        ...
        if not retired_refs:
            ...
            manual_issues.append(
                ...
                f"in config.yaml â€” see {MIGRATION_GUIDE_URL}"
            )
    ...
    try:
        ...
        if nous_status.get("logged_in"):
            check_ok("Zed Portal auth", "(logged in)")
        else:
            check_warn("Zed Portal auth", "(not logged in)")
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Zed' own device-code flow â€” the Codex CLI is
            ...
            if not _safe_which("codex"):
                check_info(
                    ...
                    "(optional â€” only required to import tokens "
    ...
    # disrupt the already-printed Zed/Codex/Gemini/MiniMax rows above.
    try:
        from zed_cli.auth import get_xai_oauth_auth_status
    ...
    for subdir_name in expected_subdirs:
        subdir_path = zed_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Zed a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Zed Agent Persona\n\n"
                "<!-- Edit this file to customize how Zed communicates. -->\n\n"
    ...
    state_db_path = zed_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from zed_state import is_malformed_db_error, repair_state_db_schema

            if is_malformed_db_error(e):
                # sqlite_master itself is malformed (e.g. duplicate
                # messages_fts) â€” every statement fails before it runs, so
                ...
                if should_fix:
                    ...
                    if report.get("repaired"):
                        ...
                        issues.append(
                            "state.db schema malformed and auto-repair failed â€” "
                            "restore from the backup copy beside state.db"
                        )
                else:
                    issues.append(
                        "state.db schema malformed â€” run 'zed doctor --fix' "
                        "(or 'zed sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = zed_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file â€” run 'zed doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "zed"
        ...
        _cmd_link = _cmd_link_dir / "zed"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(zed not in venv/bin/ or .venv/bin/ â€” reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/zed â†’ correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/zed points to wrong target",
                        f"(â†’ {_target}, expected â†’ {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/zed â†’ {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/zed â€” run 'zed doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink â€” possibly a wrapper script
                check_ok(f"{_cmd_link_display}/zed exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/zed not found",
                    "(zed command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/zed â†’ {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/zed symlink â€” run 'zed doctor --fix'")
    ...
    try:
        from zed_constants import is_container as _is_container
    ...
    if running_in_container:
        ...
        if terminal_env != "docker":
            check_info(
                "Running inside a container â€” using local terminal backend "
    ...
    if _safe_which("node"):
        ...
        # Chromium presence â€” the browser tools silently fail to register when
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `zed doctor` invocation.
        ...
        for npm_dir, label, audit_extra in npm_audit_targets:
            ...
            try:
                ...
                if audit_extra and audit_extra[0] == "--workspace":
                    ...
                    # 'edgesOut')" â€” an arborist bug with workspace-filtered
                ...
                if total == 0:
                    ...
                    if fix_cmd:
                        vuln_detail = (
                            f"{critical} critical, {high} high, {moderate} moderate â€” run: {fix_cmd}"
                        )
                    else:
                        vuln_detail = (
                            f"{critical} critical, {high} high, {moderate} moderate â€” "
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree â€” in that case it is an npm bug, not a Zed one.
                        check_info(
                            "  ^ build-time tooling (not runtime); if manual npm remediation "
                            "errors with an arborist crash it's a known npm bug â€” clears "
                            "via a lockfile bump"
                        )
                    issues.append(
    ...
    # single probe â€” about 2s â€” without changing the output format.
    #
    ...
    def _probe_openrouter() -> _ConnectivityResult:
        ...
        if not key:
            return _ConnectivityResult(
                "OpenRouter API",
                [(color("âš ", Colors.YELLOW), "OpenRouter API",
        ...
        try:
            ...
            if r.status_code == 200:
                return _ConnectivityResult(
                    "OpenRouter API",
                    [(color("âœ“", Colors.GREEN), "OpenRouter API", "")],
                    [],
                )
            if r.status_code == 401:
                return _ConnectivityResult(
                    "OpenRouter API",
                    [(color("âœ—", Colors.RED), "OpenRouter API",
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    "OpenRouter API",
                    [(color("âœ—", Colors.RED), "OpenRouter API",
                      color("(out of credits â€” payment required)", Colors.DIM))],
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'zed config set model.provider <provider>' "
            ...
            if r.status_code == 429:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter rate limit hit â€” consider switching to "
    ...
    def _probe_anthropic() -> _ConnectivityResult:
        ...
        try:
            ...
            if r.status_code == 200:
                return _ConnectivityResult(
                    "Anthropic API",
                    [(color("âœ“", Colors.GREEN), "Anthropic API", "")],
                    [],
                )
            if r.status_code == 401:
                return _ConnectivityResult(
                    "Anthropic API",
                    [(color("âœ—", Colors.RED), "Anthropic API",
                      color("(invalid API key)", Colors.DIM))],
                    [],
                )
            return _ConnectivityResult(
                "Anthropic API",
                [(color("âš ", Colors.YELLOW), "Anthropic API",
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # Auto-detect Kimi Code keys (sk-kimi-) â†’ api.kimi.com/coding/v1
            ...
            # ``ACCESS_TOKEN_TYPE_UNSUPPORTED`` â€” that header is reserved for
            # OAuth 2 access tokens, not plain API keys. Plain keys use
            # ``x-goog-api-key`` (or ``?key=``). Without this, a perfectly valid
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``zed doctor``.
            ...
            if r.status_code == 401:
                return _ConnectivityResult(
                    pname,
                    [(color("âœ—", Colors.RED), label,
                      color("(invalid API key)", Colors.DIM))],
                    [f"Check {env_vars[0]} in .env"],
                )
            return _ConnectivityResult(
                pname,
                [(color("âš ", Colors.YELLOW), label,
                  color(f"(HTTP {r.status_code})", Colors.DIM))],
                [],
            )
        except Exception as e:
    ...
    def _probe_bedrock() -> _ConnectivityResult:
        ...
        try:
            ...
            return _ConnectivityResult(
                "AWS Bedrock",
                [(color("âš ", Colors.YELLOW), label,
                  color(f"(boto3 not installed â€” {sys.executable} -m pip install boto3)",
                        Colors.DIM))],
            ...
            return _ConnectivityResult(
                ...
                [f"AWS Bedrock: {err_name} â€” check IAM permissions for "
    ...
    def _probe_azure_entra() -> _ConnectivityResult:
        ...
        azure-foundry`` AND ``model.auth_mode: entra_id`` â€” we don't probe
    ...
    for _entry in _APIKEY_PROVIDERS_CACHE:
        ...
        # Capture loop vars by binding default args â€” without this, all closures
    ...
    print(f"  {color(f'Running {len(_probes)} connectivity checks in parallelâ€¦', Colors.DIM)}",
          end="", flush=True)
    ...
    # never the legitimate source for `zed doctor`.
    _imds_prev = os.environ.get("AWS_EC2_METADATA_DISABLED")
    os.environ["AWS_EC2_METADATA_DISABLED"] = "true"
    try:
        # 8 workers is plenty â€” each probe is a single HTTP call plus a TLS
    ...
    # Clear the "Running â€¦" line and print all results in submission order.
    print("\r" + " " * 70 + "\r", end="")
    for _r in _results:
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'zed setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = ZED_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: zed skills list)")
    ...
    if github_token:
        ...
        check_ok("GitHub authenticated via gh CLI", "(full API access â€” no GITHUB_TOKEN needed)")
    else:
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = ZED_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                # Config file missing â€” but env var fallback may have resolved it.
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: zed memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: zed memory setup",
                    "No Honcho API key â€” run 'zed memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run zed memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: zed memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: zed memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from zed_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
        ...
        if named_profiles:
            ...
            for p in named_profiles:
                ...
                if not (p.path / "config.yaml").exists():
                    parts.append("âš  missing config")
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "zed -p" in content:
                            _m = _re.search(r"zed -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'zed doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("â”€" * 60, Colors.GREEN))
