"""Helpers for loading Zed .env files consistently across entrypoints."""
...
# only env vars whose values we sanitize on load â€” we must not silently
# alter arbitrary user env vars, but credentials are known to require
# pure ASCII (they become HTTP header values).
_CREDENTIAL_SUFFIXES = ("_API_KEY", "_TOKEN", "_SECRET", "_KEY")

# Names we've already warned about during this process, so repeated
# load_zed_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name â†’ source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_zed_dotenv().
# Used by setup / `zed model` flows to label detected credentials so
# users understand WHERE a key came from when their .env doesn't contain it
# directly (otherwise the "credentials detected âœ“" line looks identical to
# the .env case and they don't know Bitwarden is wired up).
_SECRET_SOURCES: dict[str, str] = {}

# ZED_HOME paths we've already pulled external secrets for during this
# process.  ``load_zed_dotenv()`` is called at module-import time from
# several hot modules (cli.py, zed_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_zed_dotenv()`` call.  Returns
...
def reset_secret_source_cache() -> None:
    """Forget which ZED_HOME paths have already had external secrets applied.
...
def format_secret_source_suffix(env_var: str) -> str:
    ...
    # Generic fallback â€” future-proofing for additional secret sources
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            f"{'s' if stripped != 1 else ''} ({detail}) â€” stripped so the "
        ...
        print(
            ...
            "  provider's dashboard and run `zed setup` (or edit the "
...
def _load_dotenv_with_fallback(path: Path, *, override: bool) -> None:
    ...
    # that substitute Unicode lookalike glyphs (e.g. Ê‹ U+028B for v).
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``zed_cli.config._sanitize_env_lines`` which
    already knows all valid Zed env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from zed_cli.config import _sanitize_env_lines
    except ImportError:
        return  # early bootstrap â€” config module not available yet
    ...
    try:
        ...
        pass  # best-effort â€” don't block gateway startup


def load_zed_dotenv(
    *,
    zed_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load Zed environment files with user config taking precedence.

    Behavior:
    - `~/.zed/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(zed_home or os.getenv("ZED_HOME", Path.home() / ".zed"))
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of ZED_HOME / profile). v1
    enforcement is "applied last with override=True" â€” at the end of startup load
    ...
    ``docs/design/managed-scope.md`` Â§4.1).
    ...
    v2 item (design Â§8.1). v1 relies on filesystem permissions only.
    ...
    except Exception:  # noqa: BLE001 â€” managed scope must never block startup
        return
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of Zed reads
    ``os.environ`` for credentials.  Any failure here is logged and
    swallowed â€” external secret sources must never block startup.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_zed_dotenv()`` runs at import
    time from several hot modules (cli.py, zed_cli/main.py,
    run_agent.py, trajectory_compressor.py, ...), so without this guard
    the Bitwarden status line would print 3-5x per CLI startup.  Use
    ``reset_secret_source_cache()`` if you need to force a re-pull
    (tests, future ``zed secrets bitwarden sync`` from a long-running
    process).
    """
    ...
    except Exception:  # noqa: BLE001 â€” config errors must not block startup
        return
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `zed model`
        # flows can label detected credentials with "(from Bitwarden)" â€”
        # otherwise users see "credentials âœ“" with no hint that the value
