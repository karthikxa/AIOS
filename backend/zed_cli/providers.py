"""
Single source of truth for provider identity in Zed Agent.

Two data sources, merged at runtime:

1. **models.dev catalog** â€” 109+ providers with base URLs, env vars, display
   names, and full model metadata (context, cost, capabilities).  This is
   the primary database.

2. **Zed overlays** â€” transport type, auth patterns, aggregator flags,
   and additional env vars that models.dev doesn't track.  Small dict,
   maintained here.

3. **User config** (``providers:`` section in config.yaml) â€” user-defined
...
# -- Zed overlay ----------------------------------------------------------
# Zed-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class ZedOverlay:
    """Zed-specific provider metadata layered on top of models.dev."""
...
ZED_OVERLAYS: Dict[str, ZedOverlay] = {
    "openrouter": ZedOverlay(
    ...
    "openai-api": ZedOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": ZedOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": ZedOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="ZED_QWEN_BASE_URL",
    ),
    "google-gemini-cli": ZedOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
    ...
    "copilot-acp": ZedOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": ZedOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": ZedOverlay(
    ...
    "alibaba-coding-plan": ZedOverlay(
        transport="openai_chat",
...
@dataclass
class ProviderDef:
    """Complete provider definition â€” merged from all sources."""
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "zed", "user-config"
...
ALIASES: Dict[str, str] = {
    # openrouter
    "openai": "openrouter",     # bare "openai" â†’ route through aggregator
    ...
    # Local server aliases â†’ virtual "local" concept (resolved via user config)
    "lmstudio": "lmstudio",
    "lm-studio": "lmstudio",
    "lm_studio": "lmstudio",
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Zed overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Zed overlay
    ...
    overlay = ZED_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + zed extra
    ...
    if overlay is not None:
        # Zed-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    Resolution order:
      1. Known provider â†’ transport â†’ TRANSPORT_TO_API_MODE.
    ...
    # Direct provider checks for providers not in ZED_OVERLAYS
    if provider == "bedrock":
...
def resolve_custom_provider(
    ...
    # Self-heal: bare "custom" matched nothing â€” return first valid entry
...
def resolve_provider_full(
    ...
    """Full resolution chain: built-in â†’ models.dev â†’ user config.
    ...
    #    explicit intent for that name â€” it must not be hijacked by a legacy
    #    vendor alias (e.g. bare "openai" â†’ "openrouter"). Resolve the raw
