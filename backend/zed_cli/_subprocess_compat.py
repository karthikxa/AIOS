...
Zed is developed on Linux / macOS and tested natively on Windows too.
...
# bootstrap-installer) dies, the OS tears down the whole job â€” taking the
...
# Electron exits, so the gateway never gets respawned after a `zed
...
def windows_detach_flags() -> int:
    ...
    - ``CREATE_BREAKAWAY_FROM_JOB`` â€” escape any job object the parent is
    ...
    If a process is in a job that disallows breakaway (rare â€”
...
def windows_detach_flags_without_breakaway() -> int:
    ...
    Callers that want to recover â€” by retrying without the breakaway
    bit â€” can pair the two helpers symbolically rather than coding the
    ``& ~0x01000000`` magic at every site:

    .. code-block:: python

        try:
            subprocess.Popen(argv, creationflags=windows_detach_flags(), â€¦)
        except OSError:
            subprocess.Popen(
                argv,
