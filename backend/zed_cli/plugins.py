...
1. **Bundled plugins** â€“ ``<repo>/plugins/<name>/`` (shipped with zed-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded â€” they have their
   own discovery paths)
2. **User plugins**   â€“ ``~/.zed/plugins/<name>/``
3. **Project plugins** â€“ ``./.zed/plugins/<name>/`` (opt-in via
   ``ZED_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     â€“ packages that expose the ``zed_agent.plugins``
   entry-point group.
...
from zed_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``ZED_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("ZED_BUNDLED_PLUGINS")
    if env_override:
...
try:
    import yaml
except ImportError:  # pragma: no cover â€“ yaml is optional at import time
...
# ---------------------------------------------------------------------------
#
# Set ``ZED_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.zed/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("ZED_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When ZED_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Zed loggers.
    """
    ...
    logger.debug(
        "ZED_PLUGINS_DEBUG=1 â€” verbose plugin discovery logging enabled"
    )
...
ENTRY_POINTS_GROUP = "zed_agent.plugins"

_NS_PARENT = "zed_plugins"
...
class PluginManifest:
    ...
    # Plugin kind â€” see plugins.py module docstring for semantics.
    ...
    #              in ~/.zed/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key â€” path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``zed plugins list``. For a flat plugin at
...
# ---------------------------------------------------------------------------
# PluginContext  â€“ handed to each plugin's ``register()`` function
# ---------------------------------------------------------------------------
...
class PluginContext:
    ...
    def __init__(self, manifest: PluginManifest, manager: "PluginManager"):
        ...
        # Lazy-built host-owned LLM facade â€” see ctx.llm property below.
        self._llm: Any = None
    ...
    def inject_message(self, content: str, role: str = "user") -> bool:
        ...
        if getattr(cli, "_agent_running", False):
            # Agent is mid-turn â€” interrupt with the message
            cli._interrupt_queue.put(msg)
        else:
            # Agent is idle â€” queue as next input
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``zed honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``zed <subcommand>``
    ...
    def dispatch_tool(self, tool_name: str, args: dict, **kwargs) -> str:
        ...
        # In gateway mode _cli_ref is None â€” tools degrade gracefully
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`zed_cli.dashboard_auth.DashboardAuthProvider`. Used by
    ...
    def register_slack_action_handler(
        ...
        Zed' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``zed model â†’ Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: zed_cli.main imports plugins indirectly
        from zed_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS

        builtin_keys = {k for k, _name, _desc in _BUILTIN_AUX_TASKS}
        if key in builtin_keys:
            raise ValueError(
                f"Plugin '{self.manifest.name}' cannot register auxiliary task "
                f"{key!r} â€” that key is reserved for a built-in task. "
        ...
        if existing is not None and existing.get("plugin") != self.manifest.name:
            raise ValueError(
                f"Plugin '{self.manifest.name}' cannot register auxiliary task "
                f"{key!r} â€” already registered by plugin "
                f"'{existing.get('plugin')}'"
            )

        # Normalize defaults â€” plugin owns the schema, but we ensure routing
    ...
    def register_skill(
        ...
        ``~/.zed/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def __init__(self) -> None:
        ...
        # Plugin skill registry: qualified name â†’ metadata dict.
        self._plugin_skills: Dict[str, Dict[str, Any]] = {}
        # Plugin-registered auxiliary tasks: key â†’ {key, display_name,
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if env_var_enabled("ZED_SAFE_MODE"):
            logger.info("ZED_SAFE_MODE=1 â€” plugin discovery skipped")
        ...
        # registry" â€” callers swallow the exception and would otherwise be
    ...
    def _discover_and_load_inner(self) -> None:
        """The actual discovery sweep â€” see :meth:`discover_and_load`."""
        manifests: List[PluginManifest] = []

        # 1. Bundled plugins (<repo>/plugins/<name>/)
        #
        # Repo-shipped plugins live next to zed_cli/. Two layouts are
        ...
        # skipped at the top level â€” they have their own discovery systems
        ...
        # 2. User plugins (~/.zed/plugins/)
        user_dir = get_zed_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.zed/plugins/)
        if _env_enabled("ZED_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".zed" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set ZED_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        # Later sources override earlier ones on key collision â€” user
        ...
        for manifest in winners.values():
            ...
            if manifest.kind == "exclusive":
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "exclusive plugin â€” activate via <category>.provider config"
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
            ...
            # not import the module â€” a second import would create two
            ...
            # Built-in backends auto-load â€” they ship with zed and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform Zed ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `zed plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _parse_manifest(
        ...
        try:
            if yaml is None:
                logger.warning("PyYAML not installed â€“ cannot load %s", manifest_file)
                return None
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `zed plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``zed_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``zed_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def get_plugin_commands() -> Dict[str, dict]:
    """Return the full plugin commands dict (name â†’ {handler, description, plugin}).
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``zed tools`` TUI so plugin-provided toolsets appear
    ...
    for ts_key in sorted(toolset_tools):
        ...
        label = f"ðŸ”Œ {ts_key.replace('_', ' ').title()}"
