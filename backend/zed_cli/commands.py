"""Slash command definitions and autocomplete for the Zed CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Zed config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Zed works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Zed is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.zed/skills/ for newly installed or removed skills",
    ...
    CommandDef("billing", "Manage Nous terminal billing â€” buy credits, auto-reload, limits", "Info"),
    ...
    CommandDef("update", "Update Zed Agent to the latest version", "Info"),
    CommandDef("version", "Show Zed Agent version", "Info", aliases=("v",)),
...
def is_gateway_known_command(name: str | None) -> bool:
    ...
    ``command:<name>`` hooks â€” plugin commands get the same lifecycle
    events as built-ins.
    """
    if not name:
        return False
...
# "all resolvable commands" â€” which is the real bypass set (see
...
def should_bypass_active_session(command_name: str | None) -> bool:
    ...
    a "busy â€” wait or /stop first" response. In both paths the command
    ...
    the pending queue â€” which meant a mid-run /model (or /reasoning,
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`zed_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/zed`` subcommand mapping, and
...
def telegram_bot_commands() -> list[tuple[str, str]]:
    ...
    for cmd in COMMAND_REGISTRY:
        if not _is_gateway_available(cmd, overrides):
            continue
        # Built-in arg-taking commands are included â€” their handlers show
...
_TELEGRAM_MENU_PRIORITY = (
    ...
    "sessions",
    "model",
    # Maintenance / diagnostics â€” the ones that prompted this priority list.
    "debug",
    "restart",
    "update",
    "verbose",
...
Telegram only displays a small BotCommand menu in practice.  The full Zed
...
# Backward-compat alias â€” tests and external code may reference the old name.
...
def _sanitize_telegram_name(raw: str) -> str:
    ...
    Steps: lowercase â†’ replace hyphens with underscores â†’ strip all other
    invalid characters â†’ collapse consecutive underscores â†’ strip leading/
        ...
        if len(name) > _CMD_NAME_LIMIT:
            ...
            if candidate in used:
                ...
                for digit in range(10):
                    ...
                    # All 10 digit slots exhausted â€” skip entry
                    continue
            name = candidate
        if name in used:
            continue
...
def _collect_gateway_skill_entries(
    ...
    # Plugins have no cmd_key â€” use empty string as placeholder
    ...
    try:
        ...
        # ``zed skills list`` and the agent's ``/skill-name`` dispatch but
    ...
    # Skills fill remaining slots â€” only tier that gets trimmed
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    User-installed hub skills are excluded â€” accessible via /skills.
    Skills disabled for the ``"telegram"`` platform (via ``zed skills
    ...
    # Drop the cmd_key â€” Telegram only needs (name, desc) pairs.
...
def discord_skill_commands(
    ...
    - Hyphens are allowed in names (no ``-`` â†’ ``_`` sanitization)
    ...
    return _collect_gateway_skill_entries(
        platform="discord",
        max_slots=max_slots,
        reserved_names=set(reserved_names),  # copy â€” don't mutate caller's set
        desc_limit=100,
    )
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    ``skills.external_dirs`` â€” matching the widened filter applied to the
    flat ``discord_skill_commands()`` collector in #18741. Without this
    parity, external-dir skills are visible via ``zed skills list`` and
    ...
    The legacy 25-group Ã— 25-subcommand caps (from the old nested
    ``/skill <cat> <name>`` layout) are **not** applied â€” the live caller
    ...
    # Map clamped-32-char-name â†’ what it came from, so we can emit an
    ...
    # on the 32-char clamp" â€” the latter is the rename-worthy case.
    ...
    try:
        ...
        # becomes its own scan root for category derivation â€” a skill at
        ...
        for cmd_key in sorted(skill_cmds):
            ...
            if discord_name in _names_used:
                ...
                # meant skill authors had no way to discover the drop â€”
                ...
                if prior == "<reserved>":
                    logger.warning(
                        ...
                        "clamp with a reserved gateway command name %r â€” the "
                    ...
                    logger.warning(
                        ...
                        "Discord's 32-char command-name limit â€” only %r "
            ...
            # scan root. e.g. creative/ascii-art/SKILL.md â†’ ("creative", ...)
...
# unrelated command landed. These claim their slots right after /zed,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /zed <command>).
...
# gets clamped ("reset" was unpinned for exactly that â€” /new keeps its
# native slot, the alias spelling stays reachable via /zed reset).
...
# ``/zed <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional â€”
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/zed" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /zed credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /zed billing.
#   - debug: the log/report upload surface; reached via /zed debug on Slack.
_SLACK_VIA_ZED_ONLY = frozenset({"credits", "billing", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/zed <verb>`` subcommand.
    ...
    can still reach them via ``/zed <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/zed`` is always reserved as the first entry so the
    legacy ``/zed <subcommand>`` form keeps working for anything that
    ...
    # Reserve /zed as the catch-all top-level command.
    entries.append(("zed", "Talk to Zed or run a subcommand", "[subcommand] [args]"))
    seen.add("zed")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_ZED_ONLY:
            # Intentionally Slack-via-/zed only (see _SLACK_VIA_ZED_ONLY).
            return
    ...
    # everything except /zed, so a new canonical command can never silently
    ...
    for alias in _SLACK_PRIORITY_ALIASES:
        ...
        if cmd is not None:
            _add(alias, f"Alias for /{cmd.name} â€” {cmd.description}", cmd.args_hint or "")
...
def slack_app_manifest(request_url: str = "https://zed-agent.local/slack/commands") -> dict[str, Any]:
    ...
    The returned dict is the ``features.slash_commands`` portion only â€”
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /zed handler.

    Maps both canonical names and aliases so /zed bg do stuff works
    the same as /zed background do stuff.

    Plugin-registered slash commands are included so ``/zed <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    # - Adding space makes "/model" â†’ "/model " which blocks picker execution
    ...
    def _completion_text(cmd_name: str, word: str) -> str:
        ...
        NOT get a trailing space â€” the TUI would apply the completion on Enter
        and block the picker from opening.
        """
        if cmd_name != word:
            return cmd_name
        # Don't add space for picker commands â€” allows Enter to execute them
    ...
    def _extract_path_word(text: str) -> str | None:
        ...
        ``/`` â€” they are never useful local-path completions.
        """
        if not text:
            return None
        ...
        # https:// URL becomes a listdir of "https:") â€” pure latency, never a
    ...
    def _context_completions(self, word: str, limit: int = 30):
        ...
        # Bare @ or @partial â€” fuzzy project-wide file search
    ...
    def _fuzzy_file_completions(self, word: str, query: str, limit: int = 20):
        ...
        if not query:
            # No query â€” show recently modified files (already sorted by mtime)
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from zed_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _tools_completions(sub_text: str, sub_lower: str):
        """Yield completions for /tools â€” subcommand + toolset/MCP-server name.
    ...
    def _handoff_completions(sub_text: str, sub_lower: str):
        ...
        home channel is NOT required to list a platform â€” it's often learned at
        runtime â€” so the meta hints whether one is set yet. Completes only the
        ...
        for platform in platforms:
            ...
            try:
                ...
                home = None
            meta = f"â†’ {home.name}" if home and getattr(home, "name", None) else "send this session here"
            yield Completion(
                name,
                start_position=-len(partial),
                display=name,
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        ...
        try:
            # Resolve from the same source the runtime applies personalities â€”
        ...
        for cmd, info in self._iter_skill_bundles().items():
            ...
            if cmd_name.startswith(word):
                ...
                yield Completion(
                    ...
                    start_position=-len(word),
                    display=cmd,
                    display_meta=f"â–£ {short_desc} ({skill_count} skills)",
                )
            ...
            for cmd_name, cmd_info in get_plugin_commands().items():
                if cmd_name.startswith(word):
                    ...
                    yield Completion(
                        ...
                        start_position=-len(word),
                        display=f"/{cmd_name}",
                        display_meta=f"ðŸ”Œ {short_desc}",
                    )
        except Exception:
            pass
...
class SlashCommandAutoSuggest(AutoSuggest):
    ...
    def get_suggestion(self, buffer, document):
        ...
        if len(parts) == 1 and not text.endswith(" "):
            # Still typing the command name: /upd â†’ suggest "ate"
            word = text[1:].lower()
            for cmd in COMMANDS:
        ...
        # Command is complete â€” suggest subcommands
        sub_text = parts[1] if len(parts) > 1 else ""
        sub_lower = sub_text.lower()
