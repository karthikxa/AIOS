"""
Gateway subcommand for zed CLI.

Handles: zed gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_zed_home is imported lazily at call sites to avoid ImportError
# when zed_constants is cached from a pre-update version during `zed update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    at self â€” the caller's dedup / exclude logic then couldn't distinguish
    "zed CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
        ...
        return None
    # Fallback: shell out to ps (POSIX only â€” bare ``ps`` doesn't exist on Windows).
...
def _request_gateway_self_restart(pid: int) -> bool:
    ...
    if not _is_pid_ancestor_of_current_process(pid):
        return False
    try:
        os.kill(pid, signal.SIGUSR1)  # windows-footgun: ok â€” POSIX signal, guarded by hasattr(signal, 'SIGUSR1') above
...
def _graceful_restart_via_sigusr1(pid: int, drain_timeout: float) -> bool:
    ...
    try:
        ...
        # Already gone â€” nothing to drain.
        return True
    ...
    # Windows â€” Python's implementation calls ``TerminateProcess(handle, 0)``
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``zed gateway status`` from falsely counting the ``zed`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``zed gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_zed_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"zed_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. ZED_HOME
        # may be passed via env (not visible in wmic/CIM command line) so
        # its absence is NOT disqualifying â€” only a non-matching explicit
        # ZED_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "zed_home=" in command_lc
            and f"zed_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
    ...
    try:
        if is_windows():
            ...
            # removed as part of the WMIC deprecation â€” fall back to
    ...
    # behaviour â€” BUT it means every gateway run produces two pythonw
    ...
    # launcher stub â€” drop it, keep the child.
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``zed update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Zed profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Zed profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from zed_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "zed_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    #
    # POSIX â€” ``start_new_session=True`` (os.setsid in the child) detaches
    # from the parent's process group so Ctrl+C in the CLI doesn't
    # propagate and the watcher/gateway survive the CLI exiting.
    #
    # Windows â€” ``start_new_session`` is silently accepted but does NOT
    # detach.  The watcher stays attached to the CLI's console and dies
    # when the user closes the terminal, leaving ``zed update`` users
    # with no running gateway until they re-invoke ``zed gateway``
    ...
    watcher = textwrap.dedent(
        ...
        while time.monotonic() < deadline:
            # ``os.kill(pid, 0)`` is not a no-op on Windows â€” use the
        ...
        if sys.platform == "win32":
            ...
            try:
                ...
                # breakaway. Retry without it â€” DETACHED_PROCESS et al.
    ...
    # Same platform-aware detach for the watcher process itself â€” so
    ...
    try:
        ...
        # (Electron, Windows Terminal with restrictive job settings, â€¦).
        # Retry without it. POSIX never reaches this branch â€” there
        # ``start_new_session=True`` cannot raise OSError â€” so the
...
def _sync_zed_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``ZED_HOME``.

    Under ``sudo``, ``ZED_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_zed_home` falls back to ``/root/.zed`` â€” the wrong
    profile. The unit file pins ``ZED_HOME`` for the actual gateway
    ...
    unit_home = env.get("ZED_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("ZED_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["ZED_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
        ...
        if active_state == "active":
            if new_pid and (previous_pid is None or new_pid != previous_pid):
                ...
                if gateway_state == "running":
                    print(f"âœ“ {scope_label} service restarted (PID {new_pid})")
                    return True
                if gateway_state == "startup_failed":
                    ...
                    print(
                        f"âš  {scope_label} service process restarted (PID {new_pid}), but gateway startup failed: {reason}"
                    )
                    return False
                if not printed_runtime_wait:
                    print(
                        f"â³ {scope_label} service process started (PID {new_pid}); waiting for gateway runtime..."
                    )
    ...
    print(
        f"âš  {scope_label} service did not become active within {int(timeout)}s.\n"
        f"  Check status: {'sudo ' if system else ''}zed gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"â³ {scope_label} service is temporarily rate-limited by systemd.")
    print("  systemd is refusing another immediate start after repeated exits.")
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}zed gateway restart{scope_flag}"
    )
...
def _recover_pending_systemd_restart(
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        print("â³ Service restart already pending â€” waiting for systemd relaunch...")
    ...
    if active_state == "failed" and (
        ...
        scope_label = _service_scope_label(system).capitalize()
        print(
            f"â†» Clearing failed state for pending {scope_label.lower()} service restart..."
        )
        _run_systemctl(
            ["reset-failed", svc],
            system=system,
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from zed_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print()
    print(
        "âš  Gateway process is running for this profile, but the service is not active"
    )
    print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
    print("  This is usually a manual foreground/tmux/nohup run, so `zed gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``zed gateway status`` output so users with
    ...
    try:
        ...
        for proc in other_processes:
            print(f"  âœ“ {proc.profile:<16s} â€” PID {proc.pid}")
    except Exception:
        pass
...
def _gateway_list() -> None:
    ...
    """
    try:
        from zed_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
    ...
    for prof in profiles:
        marker = "âœ“" if prof.gateway_running else "âœ—"
        label = prof.name
...
def kill_gateway_processes(
    ...
    for pid in pids:
        try:
            ...
            print(f"âš  Permission denied to kill PID {pid}")
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (ZED_HOME-scoped).
    ...
    # a no-op â€” route through the cross-platform existence check.
    import time as _time
    from gateway.status import _pid_exists
...
from zed_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Zed Docker image â€” that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``zed gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``ZED_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("ZED_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "zed-gateway"
SERVICE_DESCRIPTION = "Zed Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current ZED_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (ZED_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from zed_constants import get_default_zed_root

    home = get_zed_home().resolve()
    default = get_default_zed_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary ZED_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(zed_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when ZED_HOME is a named profile.

    For ``~/.zed/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        zed_home: Optional explicit ZED_HOME path. Defaults to the current
            ``get_zed_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Zed root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_zed_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from zed_constants import get_default_zed_root

    home = Path(zed_home or str(get_zed_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_zed_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(zed_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".zed"
    try:
        Path(zed_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(zed_home, default_root=target_root)
    except ValueError:
        return _profile_arg(zed_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this ZED_HOME.

    Default ``~/.zed`` returns ``zed-gateway`` (backward compatible).
    Profile ``~/.zed/profiles/coder`` returns ``zed-gateway-coder``.
    Any other ZED_HOME appends a short hash for uniqueness.
    """
...
def _user_dbus_socket_path() -> Path:
    ...
    xdg = os.environ.get("XDG_RUNTIME_DIR") or f"/run/user/{os.getuid()}"  # windows-footgun: ok â€” POSIX systemd helper, never invoked on Windows
    return Path(xdg) / "bus"
...
def _ensure_user_systemd_env() -> None:
    ...
    """
    uid = os.getuid()  # windows-footgun: ok â€” POSIX systemd helper, never invoked on Windows
        ...
        try:
            ...
            if result.returncode == 0:
                if _wait_for_user_dbus_socket(timeout=5.0):
                    print(f"âœ“ Enabled linger for {username} â€” user D-Bus now available")
                    return
...
# Legacy service names from older Zed installs that predate the
# zed-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (zed-gateway-*.service) and unrelated third-party
# "zed" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("zed.service",)
...
def _legacy_unit_search_paths() -> list[tuple[bool, Path]]:
    """Return ``[(is_system, base_dir), ...]`` â€” directories to scan for legacy units.
...
def _find_legacy_zed_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Zed gateway units.

    Detects unit files installed by older Zed versions that used a
    different service name (e.g. ``zed.service`` before the rename to
    ``zed-gateway.service``). When both a legacy unit and the current
    ``zed-gateway.service`` are active, they fight over the same bot
    token â€” the PR #5646 signal-recovery change turns this into a 30-second
    SIGTERM flap loop.

    Safety guards:

    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``zed-gateway-coder.service`` and unrelated third-party
      ``zed-*`` services are never matched.
    * ExecStart content check â€” only flag units that invoke our gateway
      entrypoint. A user-created ``zed.service`` running an unrelated
    ...
    for is_system, base in _legacy_unit_search_paths():
        for name in _LEGACY_SERVICE_NAMES:
            ...
            if not any(marker in text for marker in _LEGACY_UNIT_EXECSTART_MARKERS):
                # Not our gateway â€” leave alone
                continue
            results.append((name, unit_path, is_system))
    return results


def has_legacy_zed_units() -> bool:
    """Return True when any legacy Zed gateway unit files exist."""
    return bool(_find_legacy_zed_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Zed gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_zed_units()
    if not legacy:
        return
    print_warning("Legacy Zed gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current zed-gateway service and")
    print_info("  cause SIGTERM flap loops â€” both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    zed gateway migrate-legacy")


def remove_legacy_zed_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Zed gateway unit files.

    Iterates over whatever ``_find_legacy_zed_units()`` returns â€” which is
    ...
    Returns:
        ``(removed_count, remaining_paths)`` â€” remaining includes units we
        couldn't remove (typically system-scope when not running as root).
    """
    legacy = _find_legacy_zed_units()
    if not legacy:
        print("No legacy Zed gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Zed gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok â€” Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            print()
            print_warning("System-scope legacy units require root to remove.")
            print_info("  Re-run with: sudo zed gateway migrate-legacy")
    ...
    if remaining:
        print_warning(
            f"{len(remaining)} legacy unit(s) still present â€” see messages above."
        )
    else:
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    zed gateway uninstall")
    print_info("    sudo zed gateway uninstall --system")


def _require_root_for_system_service(action: str) -> None:
    if os.geteuid() != 0:  # windows-footgun: ok â€” POSIX systemd helper, never invoked on Windows
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok â€” Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Zed can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo zed gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo zed gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo zed gateway start --system")
            return scope, False
...
def get_systemd_linger_status() -> tuple[bool | None, str]:
    ...
    if not username:
        try:
            ...
            username = pwd.getpwuid(os.getuid()).pw_name  # windows-footgun: ok â€” POSIX loginctl helper, never invoked on Windows
        except Exception:
...
def print_systemd_linger_guidance() -> None:
    ...
    linger_enabled, linger_detail = get_systemd_linger_status()
    if linger_enabled is True:
        print("âœ“ Systemd linger is enabled (service survives logout)")
    elif linger_enabled is False:
        print("âš  Systemd linger is disabled (gateway may stop when you log out)")
        print("  Run: sudo loginctl enable-linger $USER")
    else:
        print(f"âš  Could not verify systemd linger ({linger_detail})")
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Zed often sets ``HOME`` to a profile-scoped directory, but
    launchd user agents still live under the actual account home.
    """
    import pwd

    return Path(pwd.getpwuid(os.getuid()).pw_dir)  # windows-footgun: ok â€” POSIX launchd (macOS) helper, never invoked on Windows


def get_launchd_plist_path() -> Path:
    """Return the launchd plist path, scoped per profile.

    Default ``~/.zed`` â†’ ``ai.zed.gateway.plist`` (backward compatible).
    Profile ``~/.zed/profiles/coder`` â†’ ``ai.zed.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.zed.gateway-{suffix}" if suffix else "ai.zed.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.zed/zed-agent  -> /home/alice/.zed/zed-agent
      /opt/zed                 -> /opt/zed  (kept as-is)
...
def _zed_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ZED_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_zed_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.zed                    â†’ /home/alice/.zed
      /root/.zed/profiles/coder     â†’ /home/alice/.zed/profiles/coder
      /opt/custom-zed               â†’ /opt/custom-zed  (kept as-is)
    """
    current_zed = get_zed_home().resolve()
    current_default = (Path.home() / ".zed").resolve()
    target_default = Path(target_home_dir) / ".zed"

    # Default ~/.zed â†’ remap to target user's default
    if current_zed == current_default:
        return str(target_default)

    # Profile or subdir of ~/.zed â†’ preserve the relative structure
    try:
        relative = current_zed.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.zed) â€” keep as-is
        return str(current_zed)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    zed_node = zed_home / "node" / "bin"
    if _is_dir(zed_node):
        candidates.append(str(zed_node))
    zed_nm = zed_home / "node_modules" / ".bin"
    if _is_dir(zed_nm):
        candidates.append(str(zed_nm))
...
def _stable_service_working_dir() -> str:
    ...
    The gateway does NOT need its cwd to be the source checkout â€” ``ExecStart``
    uses an absolute python interpreter and ``-m zed_cli.main``, so module
    ...
    when the unit is generated from a transient checkout â€” a ``.worktrees/``
    dir, or a clone that ``zed update`` later relocates/removes â€” the path
    ...
    ``ZED_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if ZED_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    # SIGKILL on the cgroup â€” otherwise bash/sleep tool-call children left
    ...
    if system:
        ...
        zed_home = _zed_home_for_target_user(home_dir)
        profile_arg = _profile_arg_for_target_user(zed_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's ZED_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(zed_home) if zed_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m zed_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
"""
    ...
    zed_home = str(get_zed_home().resolve())
    profile_arg = _profile_arg(zed_home)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir ZED_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved ZED_HOME
    ...
    exported a throwaway ``ZED_HOME=/tmp/...`` â€” writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``zed gateway restart``
    with ``ZED_HOME=/tmp/zed-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="ZED_HOME=..."`` lines and launchd
    ``<key>ZED_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'ZED_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>ZED_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp ZED_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"âœ— Refusing to write the gateway {kind}: ZED_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported ZED_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # â”€â”€ Test-environment safety belt â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # The user-scope unit path resolves under ``Path.home()``, which is NOT
    # sandboxed by the test conftest (only ZED_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp ZED_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../zed_test`` path into
    # ``Environment="ZED_HOME=..."``. Writing that to the developer's
    ...
    print(
        f"â†» Updated gateway {_service_scope_label(system)} service definition to match the current Zed install"
    )
    return True
...
def _print_linger_enable_warning(username: str, detail: str | None = None) -> None:
    print()
    print("âš  Linger not enabled â€” gateway may stop when you close this terminal.")
    if detail:
...
def _ensure_linger_enabled() -> None:
    ...
    if result.returncode == 0:
        print("âœ“ Linger enabled â€” gateway will persist after logout")
        return
...
def _system_scope_wizard_would_need_root(system: bool = False) -> bool:
    ...
    """
    if os.geteuid() == 0:  # windows-footgun: ok â€” systemd scope wizard decision, never invoked on Windows
        return False
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    svc = get_service_name()
    print_warning(
        f"Gateway is installed as a system-wide service â€” " f"{action} requires root."
    )
    print_info("  Options:")
    ...
    print_info("         sudo zed gateway uninstall --system")
    print_info("         zed gateway install")
    print_info("         zed gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (zed.service from pre-rename installs)
    # before installing the new zed-gateway.service. If both remain, they
    ...
    # signature â€” profile units are never touched.
    if has_legacy_zed_units():
        print()
        print_legacy_unit_warning()
        print()
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_zed_units(interactive=False)
            print()
    ...
    if unit_path.exists() and not force:
        if not systemd_unit_is_current(system=system):
            print(
                f"â†» Repairing outdated {_service_scope_label(system)} systemd service at: {unit_path}"
            )
            refresh_systemd_unit_if_needed(system=system)
            if enable_on_startup:
                _run_systemctl(["enable", get_service_name()], system=system, check=True, timeout=30)
            print(f"âœ“ {_service_scope_label(system).capitalize()} service definition updated")
            return
    ...
    print(f"âœ“ {_service_scope_label(system).capitalize()} service {enable_label}!")
    print()
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}zed gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}zed gateway status{scope_flag}             # Check status"
    )
    print(
...
def systemd_uninstall(system: bool = False):
    ...
    _run_systemctl(["daemon-reload"], system=system, check=True, timeout=30)
    print(f"âœ“ {_service_scope_label(system).capitalize()} service uninstalled")
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        scope_flag = " --system" if system else ""
        print(f"âœ— Gateway service is not installed")
        print(f"  Run: {'sudo ' if system else ''}zed gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    print(f"âœ“ {_service_scope_label(system).capitalize()} service started")
...
def systemd_stop(system: bool = False):
    ...
    _sync_zed_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `zed gateway status` or logs for final shutdown state."
        )
        return
    print(f"âœ“ {_service_scope_label(system).capitalize()} service stopped")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None:
        ...
        print(f"â³ {scope_label} service restarting gracefully (PID {pid})...")
        ...
        print(
            f"âš  Graceful restart did not complete within {int(drain_timeout + 5)}s; "
        ...
        try:
            ...
            print(
                ...
                "check `zed gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        print("âš  Installed gateway service definition is outdated")
        print(
            f"  Run: {'sudo ' if system else ''}zed gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        print(
            f"âœ“ {_service_scope_label(system).capitalize()} gateway service is running"
        )
    else:
        print(
            f"âœ— {_service_scope_label(system).capitalize()} gateway service is stopped"
        )
        print(f"  Run: {'sudo ' if system else ''}zed gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        print("  â³ Restart pending: systemd is waiting to relaunch the gateway")
    elif _systemd_unit_is_start_limited(unit_props):
        print("  â³ Restart pending: systemd is temporarily rate-limiting starts")
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}zed gateway restart{scope_flag}"
        )
        print(
        ...
        print("  âš  Planned restart is stuck in systemd failed state (exit 75)")
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}zed gateway start{scope_flag}"
        )
    elif active_state == "failed" and result_code:
        print(f"  âš  Systemd unit result: {result_code}")

    if system:
        print("âœ“ System service starts at boot without requiring systemd linger")
    elif deep:
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.zed.gateway-{suffix}" if suffix else "ai.zed.gateway"


# Cached launchd domain result â€” probing is cheap but should only run once per
# process invocation (each ``zed gateway start/stop/status`` call).
...
def _launchd_domain() -> str:
    ...
    uid = os.getuid()  # windows-footgun: ok â€” POSIX launchd (macOS) helper, never invoked on Windows
    label = get_launchd_label()
    gui_domain = f"gui/{uid}"
    user_domain = f"user/{uid}"

    # 1. Probe gui/<uid> first â€” in Aqua sessions the service is loaded here.
    try:
        subprocess.run(
    ...
    # 2. Probe user/<uid> â€” in Background/SSH sessions this is the working domain.
    try:
        subprocess.run(
    ...
    # 3. Neither domain has the service loaded â€” use managername as heuristic.
    #    Aqua â†’ gui/<uid>, anything else (Background, loginwindow) â†’ user/<uid>.
    try:
        result = subprocess.run(
...
# (the documented `nohup zed gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m zed_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active ZED_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "zed_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup zed gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(f"âš  launchd cannot manage the gateway on this macOS version ({reason}).")
    if _spawn_detached_gateway():
        print("âœ“ Started gateway as a background process instead")
        print("  It will NOT auto-start at login or auto-restart on crash.")
        print(f"  Logs: {_dhh()}/logs/gateway.log")
        print("  Stop it with: zed gateway stop")
        return True
    print_error("Failed to start the gateway as a background process.")
    print(
        f"  Try manually: nohup zed gateway run --replace "
...
def generate_launchd_plist() -> str:
    python_path = get_python_path()
    # Stable cwd anchor â€” never the volatile source checkout. See
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    # `launchctl bootout` tears down the service's process group â€” which
    # includes THIS CLI â€” before the follow-up `bootstrap` can run. The gateway
    ...
    if (
        ...
        try:
            ...
            return False
        print(
            "â†» Updated gateway launchd service definition; reload deferred to a "
    ...
    print(
        "â†» Updated gateway launchd service definition to match the current Zed install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    if plist_path.exists() and not force:
        if not launchd_plist_is_current():
            ...
            print("âœ“ Service definition updated")
            return
    ...
    print()
    print("Next steps:")
    print("  zed gateway status             # Check status")
    from zed_constants import display_zed_home as _dhh
...
def launchd_start():
    ...
    try:
        ...
        if not _launchd_error_indicates_unloaded(e):
            raise
        # Job not loaded in this domain â€” re-bootstrap the plist and retry.
        ...
        try:
            ...
            # Even a fresh bootstrap can't manage the domain on this host â€”
...
def launchd_stop():
    ...
    # `zed gateway start` re-bootstraps when it detects the job is unloaded.
    try:
        subprocess.run(["launchctl", "bootout", target], check=True, timeout=90)
    except subprocess.CalledProcessError as e:
        # Job already unloaded (3/113/125), or the domain can't be managed at
        # all (5/125, macOS 26+ detached-fallback process, issue #23387) â€” in
...
def _wait_for_gateway_exit(
    ...
    while time.monotonic() < deadline:
        ...
        if (
            ...
            try:
                ...
                print(f"âš  Gateway PID {pid} did not exit gracefully; sent SIGKILL")
            except (ProcessLookupError, PermissionError, OSError):
    ...
    if remaining_pid is not None:
        print(
            f"âš  Gateway PID {remaining_pid} still running after {timeout}s â€” restart may fail"
        )
        return False
    return True
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and _request_gateway_self_restart(pid):
            print("âœ“ Service restart requested")
            return
        if pid is not None:
            try:
            ...
            if pid is not None:
                ...
                if not exited:
                    print(
                        f"âš  Gateway drain timed out after {drain_timeout:.0f}s â€” forcing launchd restart"
                    )
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("âœ“ Service definition matches the current Zed install")
    else:
        print("âš  Service definition is stale relative to the current Zed install")
        print("  Run: zed gateway start")
    ...
    if deep:
        log_file = get_zed_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``ZED_S6_SUPERVISED_CHILD``.
    """
    if os.environ.get("INVOCATION_ID"):
        return True
    if os.environ.get("ZED_S6_SUPERVISED_CHILD"):
        return True
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``zed gateway run`` is always a misconfiguration, so we
    ...
    try:
        ...
        return
    if not suffix:
        return  # default profile (or unrecognized) â€” this guard doesn't apply

    try:
        from zed_constants import get_default_zed_root
        default_root = get_default_zed_root()
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``zed gateway run [--replace]`` (or the manual-restart fallback)
    ...
    kanban DB â€” the documented root cause of multi-writer SQLite WAL corruption
    ...
    print(
        ...
        "  DB concurrently â€” which can corrupt it. Restart the supervised gateway\n"
        "  instead:"
    )
    print()
    print("    zed gateway restart")
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``zed gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other ZED_HOME roots.
    """
    ...
    print("  Use 'zed gateway restart' to replace it,")
    print("  or 'zed gateway stop' first.")
    print("  Or use 'zed gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("ZED_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Zed gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'zed' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/zed/docker/entrypoint.sh before the Zed command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$ZED_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set ZED_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `zed gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set ZED_GATEWAY_DETACHED=1; older wrappers
    ...
    if _absorb_windows_console_controls:
        try:
            ...
            # SetConsoleCtrlHandler not available (rare on Windows) â€”
        ...
        # â€” this tells the kernel to IGNORE all console control events
        ...
        try:
            ...
            # BOOL SetConsoleCtrlHandler(NULL, Add)  â€”  Add=TRUE means
    ...
    # /restart) rather than through `zed gateway restart` which already
    ...
    # `zed gateway start/restart` â€” leaving the gateway vulnerable to
    ...
    print("â”‚           âš• Zed Gateway Starting...                 â”‚")
    print("â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤")
    ...
    # â”€â”€ Exit-path diagnostics â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # ZED_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("ZED_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
...
_PLATFORMS = [
    ...
    # Discord moved to plugins/platforms/discord/ â€” its setup metadata is
    ...
    # entry registered by plugins/platforms/discord/adapter.py::register().
    {
        ...
        "setup_instructions": [
            ...
            "   âš  Without message.channels the bot will ONLY work in DMs!",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Zed will log in directly",
        ...
        "vars": [
            ...
            {
                "name": "MATRIX_USER_ID",
                "prompt": "User ID (@bot:server â€” required for password login)",
                "password": False,
                "help": "Full Matrix user ID, e.g. @zed:matrix.example.org",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. zed) and copy the bot token",
            ...
            "   âš  This is NOT your username â€” it's a 26-character alphanumeric ID.",
        ...
        "vars": [
            ...
            },
            {
                ...
                "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)",
                "password": False,
                "help": "Channel ID where Zed delivers cron results and notifications.",
            },
            {
                "name": "MATTERMOST_REPLY_MODE",
                "prompt": "Reply mode â€” 'off' for flat messages, 'thread' for threaded replies (default: off)",
                "password": False,
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Zed agent",
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address Zed will use (e.g., zed@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
    ...
    },
    {
        ...
        "vars": [
            ...
            {
                "name": "FEISHU_DOMAIN",
                "prompt": "Domain â€” feishu or lark (default: feishu)",
                "password": False,
                "help": "Use 'feishu' for Feishu China, or 'lark' for Lark international.",
            },
            {
                "name": "FEISHU_CONNECTION_MODE",
                "prompt": "Connection mode â€” websocket or webhook (default: websocket)",
                "password": False,
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Zed connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: zed pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. In the app, go to PAI â†’ My Bot and create a new bot",
            "3. After the bot is created, copy the App ID and App Secret",
            "4. Enter them below and Zed will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``zed setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run zed under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.zed/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if entry is not None:
        ...
        if entry.is_connected is not None:
            ...
            # No is_connected hook â€” fall back to check_fn as a coarse
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_zed_home() / "whatsapp" / "session" / "creds.json"
...
def _runtime_health_lines() -> list[str]:
    ...
    for platform, pdata in platforms.items():
        if pdata.get("state") == "fatal":
            ...
            lines.append(f"âš  {platform}: {message}")

    if gateway_state == "startup_failed" and exit_reason:
        lines.append(f"âš  Last startup issue: {exit_reason}")
        ...
        lines.append(f"âš  Last shutdown reason: {exit_reason}")
...
def _setup_standard_platform(platform: dict):
    ...
    print()
    print(color(f"  â”€â”€â”€ {emoji} {label} Setup â”€â”€â”€", Colors.CYAN))
    ...
    if platform.get("key") == "telegram":
        ...
        print_info("  [1] Automatic (scan QR â†’ confirm in Telegram â†’ done)")
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'zed pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_success(
                        "  DM pairing mode â€” users will receive a code to request access."
                    )
                    print_info(
                        "  Approve with: zed pairing approve <platform> <code>"
                    )
                else:
                    print_info(
                        "  Skipped â€” configure later with 'zed gateway setup'"
                    )
            continue
...
def _setup_whatsapp():
    ...
    from zed_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    """Configure DingTalk â€” QR scan (recommended) or manual credential entry."""
    from zed_cli.setup import (
        prompt_choice,
        prompt_yes_no,
        print_success,
    ...
    if method == 0:
        # â”€â”€ QR-code device-flow authorization â”€â”€
        try:
            from zed_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_wecom():
    """Interactive setup for WeCom â€” scan QR code or manual credential input."""
    print()
    print(color("  â”€â”€â”€ ðŸ’¬ WeCom (Enterprise WeChat) Setup â”€â”€â”€", Colors.CYAN))
    ...
    if method_idx == 0:
        ...
        if qr_scan_for_bot_info is not None:
            ...
            if credentials:
                ...
                print_success("  âœ” QR scan successful! Bot ID and Secret obtained.")
    ...
    # â”€â”€ Manual credential input â”€â”€
    if not bot_id or not secret:
        print()
        print_info(
            "  1. Go to WeCom Application â†’ Workspace â†’ Smart Robot -> Create smart robots"
        )
        print_info("  2. Select API Mode")
        print_info("  3. Copy the Bot ID and Secret from the bot's credentials info")
        print_info("  4. The bot connects via WebSocket â€” no public endpoint needed")
        print()
        bot_id = prompt("  Bot ID", password=False)
        if not bot_id:
            print_warning("  Skipped â€” WeCom won't work without a Bot ID.")
            return
        secret = prompt("  Secret", password=True)
        if not secret:
            print_warning("  Skipped â€” WeCom won't work without a Secret.")
            return

    # â”€â”€ Save core credentials â”€â”€
    save_env_value("WECOM_BOT_ID", bot_id)
    save_env_value("WECOM_SECRET", secret)

    # â”€â”€ Allowed users (deny-by-default security) â”€â”€
    print()
    print_info("  The gateway DENIES all users by default for security.")
    print_info("  Enter user IDs to create an allowlist, or leave empty.")
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("  Approve with: zed pairing approve <platform> <code>")
        elif access_idx == 2:
            save_env_value("WECOM_DM_POLICY", "disabled")
            print_warning("  Direct messages disabled.")
        else:
            print_info("  Skipped â€” configure later with 'zed gateway setup'")

    # â”€â”€ Home channel (optional) â”€â”€
    print()
    ...
    print()
    print_success("ðŸ’¬ WeCom configured!")
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from zed_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print(color("  â”€â”€â”€ ðŸ’¬ Weixin / WeChat Setup â”€â”€â”€", Colors.CYAN))
    print()
    print_info("  1. Zed will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Zed will store the returned account_id/token in ~/.zed/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `zed gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_zed_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `zed pairing approve`."
        )
    elif access_idx == 1:
    ...
    print_info(
        "  delivers group events for your account type â€” otherwise DM remains the only"
    )
...
def _setup_feishu():
    ...
    print()
    print(color("  â”€â”€â”€ ðŸª½ Feishu / Lark Setup â”€â”€â”€", Colors.CYAN))
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `zed pairing approve`."
        )
    elif access_idx == 1:
    ...
    print()
    print_success("ðŸª½ Feishu / Lark configured!")
...
def _setup_qqbot():
    """Interactive setup for QQ Bot â€” scan-to-configure or manual credentials."""
    print()
    print(color("  â”€â”€â”€ ðŸ§ QQ Bot Setup â”€â”€â”€", Colors.CYAN))
    ...
    if not credentials:
        ...
        app_id = prompt("  App ID", password=False)
        if not app_id:
            print_warning("  Skipped â€” QQ Bot won't work without an App ID.")
            return
        app_secret = prompt("  App Secret", password=True)
        if not app_secret:
            print_warning("  Skipped â€” QQ Bot won't work without an App Secret.")
            return
        credentials = {
            "app_id": app_id.strip(),
    ...
    print()
    print_success("ðŸ§ QQ Bot configured!")
...
def _setup_signal():
    ...
    print()
    print(color("  â”€â”€â”€ ðŸ“¡ Signal Setup â”€â”€â”€", Colors.CYAN))
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "ZedAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``zed_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from zed_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.zed/plugins/
    ...
    # Plugin with no setup helper â€” show env-var instructions.
    label = platform.get("label", platform["key"])
    emoji = platform.get("emoji", "ðŸ”Œ")
    print()
    print(color(f"  â”€â”€â”€ {emoji} {label} Setup â”€â”€â”€", Colors.CYAN))
    required = entry.required_env if entry else []
    if required:
        print_info(f"  Set these env vars in ~/.zed/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    print(
        color(
            "â”‚             âš• Gateway Setup                            â”‚", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "â”‚  Configure messaging platforms and the gateway service. â”‚",
            Colors.MAGENTA,
        )
    )
    print(
    ...
    # â”€â”€ Gateway service status â”€â”€
    print()
    ...
    if service_installed and service_running:
        ...
        if supports_systemd_services() and _system_scope_wizard_would_need_root():
            ...
            try:
                ...
                print_error("  Failed to start â€” user systemd not reachable:")
    ...
    # â”€â”€ Platform configuration loop â”€â”€
    while True:
        print()
    ...
    # â”€â”€ Post-setup: offer to install/restart gateway â”€â”€
    ...
    if any_configured:
        ...
        if service_running:
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: zed gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                    ...
                    try:
                        ...
                        if did_install and start_now:
                            try:
                                ...
                                print_error(
                                    "  Start failed â€” user systemd not reachable:"
                                )
                                for line in str(e).splitlines():
                                    print(f"  {line}")
                            except subprocess.CalledProcessError as e:
                        ...
                        print_info("  You can try manually: zed gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: zed gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo zed gateway install --system"
                        )
                    print_info("  Or run in foreground:  zed gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: zed gateway run")
                print_info(
                    "  For persistence:   tmux new -s zed 'zed gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: zed gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup zed gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'zed gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    otherwise â€” caller continues with the host-side code path.
    ...
    hook or by the container-boot reconciler (cont-init.d/02-â€¦). If it
    doesn't exist or s6 returns an error, the named errors from
    :mod:`zed_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from zed_cli.service_manager import (
    ...
    if profile is None:
        ...
        # ZED_HOME=<root>/profiles/<name>, "" for the default root,
        # or a hash for unrelated paths. Map "" â†’ "default" so the
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``zed gateway stop --all`` and ``... restart --all``
    ...
    crash and restarts each one ~1s later â€” so ``--all`` ends up
    *kicking* every gateway instead of *stopping* it. By iterating
    ...
    if not profiles:
        print("âœ— No profile gateways registered under s6")
        return True
    ...
    for profile in profiles:
        ...
        try:
            fn(service_name)
        except Exception as exc:  # noqa: BLE001 â€” report and continue
            errors.append((profile, exc))
    succeeded = len(profiles) - len(errors)
    verb = "stopped" if action == "stop" else "restarted"
    if succeeded:
        print(f"âœ“ {verb.capitalize()} {succeeded} profile gateway(s) under s6")
    for profile, exc in errors:
        print(f"âœ— Could not {action} gateway-{profile}: {exc}")
    return True
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``zed gateway install|uninstall|start|stop|restart``
        # path lands here when the user typed a system-scope action without
        # sudo. Same exit code as before â€” just gives the wizard a way to
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    old invocation transparently â€” the user gets the new behavior
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``zed gateway run`` are unaffected.
      2. ``ZED_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself â€” i.e. when s6-supervise execs ``zed gateway
         run --replace`` as a longrun, this guard short-circuits the
         redirect so the supervised gateway actually runs in
         foreground (otherwise we'd recurse: run â†’ start â†’ run â†’ start
         â†’ ...).
      3. ``--no-supervise`` (or ``ZED_GATEWAY_NO_SUPERVISE=1``) opts
         out for users who genuinely want pre-s6 semantics â€” CI smoke
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("ZED_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("ZED_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${ZED_HOME}/logs/gateways/<profile>/current,
    # so the user sees a clear sequence: this banner first, then the
    # gateway's own stdout/stderr from the supervisor.
    print(
        "â†’ gateway is now running under s6 supervision (auto-restart on crash,\n"
        "  dashboard supervised alongside if ZED_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image â€” the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or ZED_GATEWAY_NO_SUPERVISE=1) to opt out\n"
        "  and get the pre-s6 foreground behavior instead.",
        file=sys.stderr,
        flush=True,
    )
    # Keep the CMD process alive as a no-op heartbeat. The supervised
    # gateway's lifetime is independent of this process â€” s6-supervise
    ...
    #
    # Prefer `sleep infinity` (matches the static main-zed service's
    # pattern in docker/s6-rc.d/main-zed/run, and frees the Python
    # interpreter â€” the heartbeat is a tiny `sleep` process, not a
    ...
    # point â€” e.g. after user customizations rewrote PATH, or on minimal
    ...
    try:
        ...
        # land here.
        print(
            "â†’ `sleep` is unavailable; keeping the s6 CMD process alive "
...
def _block_until_terminated() -> None:
    ...
    (``sleep`` missing from PATH â€” issue #36208). Installs a SIGTERM
    ...
    platforms without ``signal.pause()`` (e.g. Windows) â€” although this
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: zed gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                    "WSL detected â€” systemd services may not survive WSL restarts."
                )
                print_info(
                    "  Consider running in foreground instead: zed gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s zed 'zed gateway run'"
                )
                print()
            ...
            print()
            print(
                "  zed gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s zed 'zed gateway run'         # persistent via tmux"
            )
            print(
                "  nohup zed gateway run > ~/.zed/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from zed_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  zed profile create <name>     # creates the s6 service slot")
                print("  zed -p <name> gateway start   # bring it up via s6")
                print("  zed status                    # see currently-supervised gateways")
                return
            # Fallback for pre-s6 containers or other container runtimes
            # we haven't taught about supervision (Podman without our
            # /init, k8s plain runs, etc.) â€” the historical guidance still
            # applies.
            print("Service installation is not needed inside a Docker container.")
            print(
                "The container runtime is your service manager â€” use Docker restart policies instead:"
            )
            print()
            print(
            ...
            print()
            print("To run the gateway: zed gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: zed gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: zed gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  zed profile delete <name>     # tears down the s6 service slot")
                print("  zed -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot â€” start them individually via `zed -p <name> gateway
        ...
        if start_all:
            ...
            if killed:
                print(
                    f"âœ“ Killed {killed} stale gateway process(es) across all profiles"
                )
                _wait_for_gateway_exit(timeout=10.0, force_after=5.0)
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: zed gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_ZED_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `zed gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if stop_all:
            ...
            if total:
                print(f"âœ“ Stopped {total} gateway process(es) across all profiles")
            else:
                print("âœ— No gateway processes found")
        else:
            ...
            if not service_available:
                # No systemd/launchd/schtasks service â€” use profile-scoped PID file
                if stop_profile_gateway():
                    print("âœ“ Stopped gateway for this profile")
                else:
                    print("âœ— No gateway running for this profile")
            else:
                print(f"âœ“ Stopped {get_service_name()} service")
        ...
        if os.getenv("_ZED_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `zed gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if supports_systemd_services():
                ...
                if linger_ok is not True:
                    ...
                    print()
                    print(
                        "âš  Cannot restart gateway as a service â€” linger is not enabled."
                    )
                    print(
            ...
            if service_configured:
                print()
                print("âœ— Gateway service restart failed.")
                print(
                    "  The service definition exists, but the service manager did not recover it."
                )
                print("  Fix the service, then retry: zed gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                print(f"âœ“ Gateway is running (PID: {', '.join(map(str, pids))})")
                ...
                if is_termux():
                    ...
                    print("  zed gateway install")
                else:
                    print("To install as a service:")
                    print("  zed gateway install")
                    print("  sudo zed gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  zed gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup zed gateway run > ~/.zed/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s zed 'zed gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  zed gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  zed gateway install  # Install as user service")
                    print(
                        "  sudo zed gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Zed gateway unit files from
        # pre-rename installs (e.g. zed.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_zed_units(interactive=not yes, dry_run=dry_run)
