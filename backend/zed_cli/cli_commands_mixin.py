...
``cli.py``'s ``ZedCLI`` class. ``ZedCLI`` inherits ``CLICommandsMixin`` so
every ``self.<handler>`` call resolves unchanged via the MRO â€” behavior-neutral.
...
Import discipline (mirrors gateway/slash_commands.py, PR #41886):
  ...
  * cli.py-internal symbols (the ``_cprint``/``_ACCENT``/``save_config_value``â€¦
    module-level helpers and constants) are imported LAZILY inside each handler
    via ``from cli import ...`` â€” that resolves at call time when ``cli`` is fully
...
from zed_constants import display_zed_home, is_termux as _is_termux_environment
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: zed --checkpoints")
        ...
        if result["success"]:
            if file_path:
                print(f"  âœ… Restored {file_path} from checkpoint {result['restored_to']}: {result['reason']}")
            else:
                print(f"  âœ… Restored to checkpoint {result['restored_to']}: {result['reason']}")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot â€” lightweight state snapshots for Zed config/state.
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_zed_home()}/state-snapshots/):\n")
    ...
    def _handle_stop_command(self):
        """Handle /stop â€” kill all running background processes and
        ...
        if n_async and interrupt_all is not None:
            ...
            print(f"  âœ… Interrupted {stopped} background delegation(s).")
    ...
    def _handle_agents_command(self):
        ...
        # Background (async) delegations â€” delegate_task(background=true)
        try:
        ...
        if delegations:
            ...
            for d in delegations:
                ...
                _cprint(
                    f"    {d.get('delegation_id', '?')} Â· "
                    f"{d.get('status', '?')} Â· {goal}"
                )
    ...
    def _handle_paste_command(self):
        ...
        from zed_cli.clipboard import has_clipboard_image
        if has_clipboard_image():
            if self._try_attach_clipboard_image():
                n = len(self._attached_images)
                _cprint(f"  ðŸ“Ž Image #{n} attached from clipboard")
            else:
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        _cprint(f"  ðŸ“Ž Attached image: {image_path.name}")
        if _remainder:
            _cprint(f"  {_DIM}Now type your prompt (or use --image in single-query mode): {_remainder}{_RST}")
        elif _is_termux_environment():
            _cprint(f"  {_DIM}Tip: type your next message, or run zed chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in zed_cli/colors.py
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        _cprint(f"  Queued handoff of '{session_title}' â†’ {platform_name} (home: {home.name}).")
        ...
        while _time.time() < deadline:
            ...
            if current == "completed":
                _cprint("")
                _cprint(f"  â†» Handoff complete. The session is now active on {platform_name}.")
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `zed gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `zed sessions list` to find sessions.")
            return
        ...
        if target.isdigit():
            ...
            from zed_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `zed sessions list` to see available sessions.")
            return
        ...
        if self.conversation_history:
            ...
            _cprint(f"  â†» Resumed session {target_id}{title_part} â€” no messages, starting fresh.")
    ...
    def _handle_suggestions_command(self, cmd: str):
        """Handle /suggestions â€” review/accept/dismiss suggested automations.
    ...
    def _handle_blueprint_command(self, cmd: str):
        """Handle /blueprint â€” set up an automation from a blueprint template.
        ...
        ``agent_seed``); ``/blueprint <name> slot=val â€¦`` creates the job
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to zed_cli.curator so the CLI and the `zed curator`
        ...
        try:
            from zed_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command â€” delegates to zed_cli.skills_hub."""
        ...
        if args and args[0].lower() in {"pending", "approve", "apply", "reject",
            ...
            if out is not None:
                print(out)
                return
        from zed_cli.skills_hub import handle_skills_slash
        handle_skills_slash(cmd, ChatConsole())

    def _handle_memory_command(self, cmd: str):
        """Handle /memory slash command â€” pending review + approval-gate toggle."""
        from zed_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        parts = cmd.strip().split()
        args = parts[1:] if len(parts) > 1 else []
    ...
    def _handle_background_command(self, cmd: str):
        ...
        _cprint(f"  ðŸ”„ Background task #{task_num} started: \"{prompt[:60]}{'...' if len(prompt) > 60 else ''}\"")
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``zed bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: zed bundles create "
        ...
        _cprint(
            ...
            f"Manage with `zed bundles`.{_RST}"
        )
    ...
    def _handle_browser_command(self, cmd: str):
        ...
        if sub.startswith("connect"):
            ...
            if _already_open:
                ...
                print("   Chromium-family browser isn't running with remote debugging â€” attempting to launch...")
                ...
                if _launched:
                    ...
                    for _wait in range(10):
                        ...
                        time.sleep(0.5)
                    if _already_open:
                        print(f"   âœ“ Chromium-family browser launched and listening on port {_port}")
                    else:
                        print(f"   âš  Browser launched but port {_port} isn't responding yet")
                        print("     Try again in a few seconds â€” the debug instance may still be starting")
                else:
            ...
            if not _already_open:
                print()
                print("Browser not connected â€” start a Chromium-family browser with remote debugging and retry /browser connect")
                print()
                return
            ...
            try:
                ...
                pass
            print()
            print("ðŸŒ Browser connected to live Chromium-family browser via CDP")
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Zed-managed isolated debug "
            ...
            if current:
                ...
                try:
                    ...
                    pass
                print()
                print("ðŸŒ Browser disconnected from live Chromium-family browser")
            ...
            if current:
                ...
                if provider is not None:
                    print(f"ðŸŒ Browser: {provider.provider_name()} (cloud)")
                else:
                    ...
                    if engine == "lightpanda":
                        print("ðŸŒ Browser: local Lightpanda (agent-browser --engine lightpanda)")
                        ...
                        print("ðŸŒ Browser: local headless Chromium (agent-browser --engine chrome)")
                    else:
                        print("ðŸŒ Browser: local headless Chromium (agent-browser)")
            print()
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Zed keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from zed_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_zed_home()}/skins/\n")
            return
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy â€” control what Enter does while Zed is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Zed is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Zed is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from zed_cli.models import _is_anthropic_fast_model
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update â€” update Zed Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``zed update`` so the user sees update output directly and gets
        ...
        if is_managed():
            print(f"  âœ— {format_managed_message('update Zed Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Zed Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="âš•  Update Zed Agent",
            detail="This will exit the current session and run `zed update`.",
            choices=choices,
        )
        if raw is None:
            print("  ðŸŸ¡ /update cancelled.")
            return False
