...
def build_tools_parser(subparsers, *, cmd_tools: Callable) -> None:
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'zed tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # zed tools disable <name...> [--platform cli]
    ...
    # zed tools enable <name...> [--platform cli]
    ...
    # zed tools post-setup <key>
    tools_postsetup_p = tools_sub.add_parser(
        "post-setup",
        help="Run a provider's post-setup install hook (npm/pip/binary)",
        description=(
            "Run the install/bootstrap hook a tool backend declares â€” the\n"
            "same step `zed tools` runs after you pick a provider that\n"
