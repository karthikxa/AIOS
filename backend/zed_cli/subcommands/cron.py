"""``zed cron`` subcommand parser.

Extracted verbatim from ``zed_cli/main.py:main()`` â€” same arguments, same
...
def build_cron_parser(subparsers, *, cmd_cron: Callable) -> None:
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.zed/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        ...
        default=False,
        help=(
            "Skip the LLM entirely â€” run --script on schedule and deliver "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.zed/scripts/. Pass empty string to clear. "
