...
def build_claw_parser(subparsers, *, cmd_claw: Callable) -> None:
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Zed",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Zed",
    ...
    claw_migrate.add_argument(
        "--dry-run",
        action="store_true",
        help="Preview only â€” stop after showing what would be migrated",
    )
    claw_migrate.add_argument(
        "--preset",
        choices=["user-data", "full"],
        default="full",
        help="Migration preset (default: full). Neither preset imports secrets â€” "
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.zed/ (by default a "
        "single restore-point archive is written to ~/.zed/backups/ "
        "before apply; restorable with 'zed import').",
    )
    claw_migrate.add_argument(
