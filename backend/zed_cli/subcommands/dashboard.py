"""``zed dashboard`` subcommand parser.
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Zed Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # Lifecycle flags â€” mutually exclusive with each other and with the
    ...
    # for `zed dashboard` cmdlines and SIGTERM them directly â€” the same
    # path `zed update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running zed dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running zed dashboard processes and exit",
    )
    # Backward-compat shim: older Zed desktop app shells (<= 0.15.x) spawn the
    # backend as `zed dashboard --no-open --tui --host ... --port ...`. The
    ...
    # --tui" and exit(2) â€” the backend died before becoming ready and the GUI just
    # showed "Zed couldn't start" with no actionable cause. Accept and silently
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "ZED_DASHBOARD_OAUTH_CLIENT_ID into ~/.zed/.env, and prints "
            "how to engage the login gate. Requires being logged in (zed setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://zed.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
