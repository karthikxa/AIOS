...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``zed login`` was removed in favor of ``zed auth`` / ``zed model``
    (the runtime handler in ``zed_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``zed login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``zed --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``zed login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``zed model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `zed auth` to manage credentials, "
            "`zed model` to select a provider, or `zed setup` for full setup."
        ),
    )
    # No ``choices=`` on purpose â€” the handler is a deprecation notice that
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored â€” see `zed model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: zed-cli)"
    )
