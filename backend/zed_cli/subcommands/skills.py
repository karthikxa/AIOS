"""``zed skills`` subcommand parser.

Extracted from ``zed_cli/main.py:main()`` (god-file Phase 2 follow-up).
...
def build_skills_parser(subparsers, *, cmd_skills: Callable) -> None:
    ...
    skills_reset = skills_subparsers.add_parser(
        "reset",
        help="Reset a bundled skill â€” clears 'user-modified' tracking so updates work again",
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.zed/skills/.bundled_manifest) "
            "so future 'zed update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_list_modified = skills_subparsers.add_parser(
        "list-modified",
        help="List bundled skills you've edited (which `zed update` keeps)",
        description=(
            "Show the bundled skills whose local copy differs from the version last "
            "synced, i.e. the ones `zed update` reports as user-modified and skips. "
            "Use `zed skills diff <name>` to see changes and `zed skills reset "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`zed update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `zed update`. Pass --sync to re-seed now."
        ),
    )
    ...
    skills_subparsers.add_parser(
        "config",
        help="Interactive skill configuration â€” enable/disable individual skills",
    )
