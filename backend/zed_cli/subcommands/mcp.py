...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Zed as an MCP server",
        description=(
            "Manage MCP server connections and run Zed as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'zed mcp add' to connect to a new server, or\n"
            "'zed mcp serve' to expose Zed conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Zed as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `zed mcp add ...` to fall through to
    # interactive chat.
    ...
    # â”€â”€ Catalog (Nous-approved MCPs shipped with the repo) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `zed mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `zed mcp install n8n`)",
    )
