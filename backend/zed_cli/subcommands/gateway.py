"""``zed gateway`` and ``zed proxy`` subcommand parsers.

Extracted verbatim from ``zed_cli/main.py:main()`` (god-file Phase 2).
...
from zed_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if ZED_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy zed.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Zed gateway unit files "
            "(e.g. zed.service) left over from older installs. Profile "
            "units (zed-gateway-<profile>.service) and unrelated "
    ...
    # gateway enroll â€” enroll a self-hosted gateway with a relay connector
    # (connectorâ‡„gateway auth). Redeems a single-use enrollment token for the
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "Authenticates as your Zed Portal account (the connector derives the "
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.zed/.env. "
            "Requires being logged in (zed setup). Not available in managed installs."
        ),
    )
    ...
    # =========================================================================
    # proxy command â€” local OpenAI-compatible proxy that attaches the user's
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "to an OAuth-authenticated provider (e.g. Zed Portal). External "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `zed proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
