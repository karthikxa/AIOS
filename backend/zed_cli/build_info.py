"""
Baked-in build metadata for Zed Agent.

Source installs report their git revision live via ``git rev-parse`` (see
``zed_cli/dump.py`` and ``zed_cli/banner.py``).  That doesn't work inside
...
To make ``zed dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$ZED_GIT_SHA``
arg into ``<project_root>/.zed_build_sha``.  This module is the single
read-side helper consumed by both callsites â€” keeping the lookup in one place
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``ZED_GIT_SHA`` build-arg fall through to live-git
...
# Path is resolved relative to this module so it works regardless of cwd â€”
# matches the pattern used by ``banner._resolve_repo_dir``.
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".zed_build_sha"


def get_build_sha(short: int = 8) -> Optional[str]:
    """Return the baked-in build SHA, truncated to ``short`` chars, or None.

    Reads ``<project_root>/.zed_build_sha`` if present.  The file is
    written by the Dockerfile's ``ZED_GIT_SHA`` build-arg and contains
