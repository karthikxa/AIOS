...
Add, remove, or reorder entries here â€” both `zed setup` and
`zed` provider-selection will pick up the change automatically.
"""
...
_ZED_USER_AGENT = f"zed-cli/{_ZED_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `zed model`
    flow without maintaining a separate static list.
    """
    from zed_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $ZED_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
#
# Models retired by xAI on May 15, 2026 are excluded â€” see
# https://docs.x.ai/developers/migration/may-15-retirement
# (grok-4, grok-4-0709, grok-4-fast{,-reasoning,-non-reasoning},
#  grok-4-1-fast{,-reasoning,-non-reasoning}, grok-code-fast-1 â†’ grok-4.3).
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Zed-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $ZED_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Zed refreshes the cache from
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    "nvidia": [
        ...
        # (map to OpenRouter defaults â€” users get familiar picks on NIM)
    ...
    # Alibaba DashScope Coding platform (coding-intl) â€” default endpoint.
    ...
    # Alibaba Coding Plan â€” same platform as alibaba (DashScope coding-intl),
    ...
    # Curated HF model list â€” only agentic models that map to OpenRouter defaults.
    "huggingface": [
        "moonshotai/Kimi-K2.5",
        "Qwen/Qwen3.5-397B-A17B",
    ...
    # AWS Bedrock â€” static fallback list used when dynamic discovery is
...
# ---------------------------------------------------------------------------
# Zed Portal free-model helper
# ---------------------------------------------------------------------------
# The Zed Portal models endpoint is the source of truth for which models
# are currently offered (free or paid). We trust whatever it returns and
# surface it to users as-is â€” no local allowlist filtering.
...
# ---------------------------------------------------------------------------
# Zed Portal account tier detection
# ---------------------------------------------------------------------------
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    models are free *right now* â€” independent of what the in-repo
    ...
    older Zed that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    which paid models are blessed *right now* â€” independent of what the
    ...
    picker even if the user is running an older Zed that doesn't ship
    ...
    * Portal paid recommendations missing from ``curated_ids`` are
      appended after the curated list (so the in-repo curated models
      show first and Portal-only picks follow).
    * ``pricing`` is left untouched â€” we deliberately do NOT synthesize
    ...
    Failures (network, parse, missing field) are silent and degrade to
    returning the inputs unchanged â€” never block the picker on a
    Portal-side hiccup.
    """
    try:
        payload = fetch_nous_recommended_models(
...
# ---------------------------------------------------------------------------
# TTL cache for free-tier detection â€” avoids repeated API calls within a
...
def check_nous_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current Zed Portal user is on a free (unpaid) tier.
    ...
    try:
        ...
        return False  # default to paid on error â€” don't block users


# ---------------------------------------------------------------------------
# Zed Portal recommended models
#
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_zed_home() / "cache" / "nous_recommended_cache.json"
...
def _write_nous_recommended_disk(base: str, data: dict[str, Any]) -> None:
    ...
    are non-fatal (logged at debug) â€” the in-process cache still works.
    """
    if not data:
        return
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    """Fetch the Zed Portal's curated recommended-models payload.

    Hits ``<portal>/api/nous/recommended-models``. The endpoint is public â€”
    ...
    (``$ZED_HOME/cache/nous_recommended_cache.json``) as last-known-good.
    When the live fetch fails (network, parse, non-2xx) and the in-process
    cache is empty, the disk copy is returned instead of ``{}`` â€” so a
    ...
    if data:
        # Live fetch succeeded â€” refresh both cache layers.
...
def get_nous_recommended_aux_model(
    ...
    * ``vision=True``  â†’ ``paidRecommendedVisionModel``  (paid tier) or
                         ``freeRecommendedVisionModel``  (free tier)
    * ``vision=False`` â†’ ``paidRecommendedCompactionModel`` or
    ...
    detection â€” useful for tests or when the caller already knows the tier.
    ...
    fails â€” callers should fall back to their own default (currently
    ``google/gemini-3-flash-preview``).
    """
    ...
    if free_tier is None:
        try:
            ...
            # On any detection error, assume paid â€” paid users see both fields
    ...
    # Preference order:
    #   free tier  â†’ free only
    #   paid tier  â†’ paid, then free (if paid field is null)
...
# ---------------------------------------------------------------------------
# Canonical provider list â€” single source of truth for provider identity.
# Every code path that lists, displays, or iterates providers derives from
# this list:  zed model, /model, list_authenticated_providers.
#
# Fields:
#   slug        â€” internal provider ID (used in config.yaml, --provider flag)
#   label       â€” short display name
#   tui_desc    â€” longer description for the `zed model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `zed model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("nous",           "Zed Portal",              "Zed Portal (Everything your agent needs, 300+ models with bundled tool use)"),
...
# downstream consumers â€” no edits to this file needed.
...
# Derived dicts â€” used throughout the codebase
_PROVIDER_LABELS = {p.slug: p.label for p in CANONICAL_PROVIDERS}
_PROVIDER_LABELS["custom"] = "Custom endpoint"  # special case: not a named provider


# ---------------------------------------------------------------------------
# Provider groups â€” DISPLAY ONLY
#
# Some vendors expose several Zed provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `zed model` / setup wizard /
...
# the ``--provider`` flag, ``/model <provider:model>``, or any typed path â€”
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``zed model``, the
    ...
    Rules:
      ...
      * A group reduced to a single present member degrades to a ``single``
        row â€” no pointless one-item submenu.
...
# sensible default, but Zed Portal is a per-token *metered aggregator* whose
# list is ordered best-/most-capable-first â€” entry [0] is the priciest flagship
...
# path. The *interactive* default (GUI onboarding / ``zed model``) uses the
# richer free/paid-tier-aware resolver â€” see ``get_recommended_default_model``
# in zed_cli/web_server.py and ``partition_nous_models_by_tier`` â€” which can
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``zed auth add openai-codex`` without
    ``zed model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` â€” the
    same model the ``zed model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    zed-agent is tool-calling-first â€” every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (Zed Portal, private mirrors, older catalog snapshots) don't populate
    ``supported_parameters`` at all. Treat that as "unknown capability â†’ allow"
    ...
    if not isinstance(params, list):
        # Field absent / malformed / None â€” be permissive.
        return True
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from zed_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support â€” zed-agent
...
def get_curated_nous_model_ids() -> list[str]:
    """Return the curated Zed Portal model-id list.
    ...
    """
    try:
        from zed_cli.model_catalog import get_curated_nous_models
...
# ---------------------------------------------------------------------------
# Pricing helpers â€” fetch live pricing from OpenRouter-compatible /v1/models
# ---------------------------------------------------------------------------

# Cache: maps model_id â†’ {"prompt": str, "completion": str} per endpoint
...
def _format_price_per_mtok(per_token_str: str) -> str:
    ...
    Examples:
        "0.000003"   â†’ "$3.00"      (per million tokens)
        "0.00003"    â†’ "$30.00"
        "0.00000015" â†’ "$0.15"
        "0.0000001"  â†’ "$0.10"
        "0.00018"    â†’ "$180.00"
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Zed Portal).
    """
...
_DEFAULT_NOUS_INFERENCE_BASE = "https://inference-api.zedteam.com"


def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for Zed Portal pricing.

    The Zed inference ``/v1/models`` endpoint exposes pricing without
    ...
    need this â€” pricing drives the free/paid partition, and silently
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    if normalized == "nous":
        ...
        if base_url:
            # Nous base_url typically looks like https://inference-api.zedteam.com/v1
...
def _fetch_novita_pricing(
    ...
    *,
    ...
    Results are cached in ``_pricing_cache`` keyed on the resolved base URL â€”
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``zed model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from zed_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        openrouter:anthropic/claude-sonnet-4.5  â†’  ("openrouter", "anthropic/claude-sonnet-4.5")
        nous:zed-3                           â†’  ("nous", "zed-3")
        anthropic/claude-sonnet-4.5             â†’  (current_provider, "anthropic/claude-sonnet-4.5")
        gpt-5.4                                 â†’  (current_provider, "gpt-5.4")
    ...
    if colon > 0:
        ...
        if provider_part and model_part and provider_part in _KNOWN_PROVIDER_NAMES:
            ...
            # providers.  ``custom:local:qwen`` â†’ ("custom:local", "qwen").
            # Single colon ``custom:qwen`` â†’ ("custom", "qwen") as before.
...
def _resolve_static_model_alias(
    ...
    try:
        from zed_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # Skip "custom" and "openrouter" â€” custom has no model catalog, and
    ...
    # Aggregators list other providers' models â€” never auto-switch TO them
...
def detect_provider_for_model(
    ...
    Returns ``(provider_id, model_name)`` â€” the model name may be remapped
    (e.g. bare ``deepseek-chat`` â†’ ``deepseek/deepseek-chat`` for OpenRouter).
    Returns ``None`` when no confident match is found.

    Priority:
    0. Bare provider name â†’ switch to that provider's default model
...
def _find_openrouter_slug(model_name: str) -> Optional[str]:
    ...
    Handles:
    - Exact match: ``anthropic/claude-opus-4.6`` â†’ as-is
    - Bare name: ``deepseek-chat`` â†’ ``deepseek/deepseek-chat``
    - Bare name: ``claude-opus-4.6`` â†’ ``anthropic/claude-opus-4.6``
    """
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Zed' canonical provider ids.

    Note: ``"auto"`` passes through unchanged â€” use
    ``zed_cli.auth.resolve_provider()`` to resolve it to a concrete
...
#
# Pattern-based matching â€” any OpenAI flagship model (gpt-*, o1*, o3*, o4*)
...
# (gpt-5-codex, gpt-5.3-codex, etc.) are excluded â€” they don't expose the
...
def _is_openai_fast_model(model_id: Optional[str]) -> bool:
    ...
    if not base:
        return False
    # Exclude Codex-series â€” they route through the Codex Responses API
...
#
# Pattern-based matching â€” any claude-* model is eligible. The anthropic
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Zed should expose the /fast toggle for this model."""
...
def _is_anthropic_fast_model(model_id: Optional[str]) -> bool:
    ...
    model id (``â€¦-opus-4.8-fast``) selected via the model field, not the speed
    parameter â€” see ``agent.anthropic_adapter._supports_fast_mode`` and its
...
def resolve_fast_mode_overrides(model_id: Optional[str]) -> dict[str, Any] | None:
    ...
    ``_build_api_kwargs`` in run_agent.py â€” each API path handles its own
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      1. ``resolve_api_key_provider_credentials("copilot")`` â€” env vars
         (``COPILOT_GITHUB_TOKEN`` / ``GH_TOKEN`` / ``GITHUB_TOKEN``) plus
         the ``gh auth token`` CLI fallback.
      2. ``read_credential_pool("copilot")`` â€” a token (typically a
         ``gho_*`` from device-code login, or a fine-grained PAT) stored in
         ``auth.json`` under ``credential_pool.copilot[]``. The pool is
         populated by ``zed auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.zed/.env``.
    ...
    entry, each candidate is exchanged via ``exchange_copilot_token`` â€”
    ...
    """
    try:
        from zed_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from zed_cli.copilot_auth import (
...
# stepfun, openai-codex) â€” those paths handle freshness themselves.
...
def _merge_with_models_dev(provider: str, curated: list[str]) -> list[str]:
    ...
    returned unchanged â€” this is the offline/CI fallback path.
    """
    try:
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Zed release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Zed release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        # Try live Zed Portal /models endpoint
        try:
            from zed_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # â€” NOT the in-repo _PROVIDER_MODELS["nous"] snapshot â€” so newly
        # added Portal models still surface without a Zed release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # may serve a small curated catalog â€” use the live list verbatim so
            ...
            # legacy chat models â€” none of which belong in the agent model picker.
            # For the default endpoint, intersect the live list with our curated
            # agentic catalog so ``/model`` matches what ``zed model`` shows.
            ...
            try:
                ...
                if live:
                    if is_default_openai:
                        ...
                        if filtered:
                            return filtered
                        # Account serves none of the curated models (rare â€”
    ...
    # below â€” bedrock is not expected to appear in that table.
    ...
    # â”€â”€ Profile-based generic live fetch (all simple api-key providers) â”€â”€
...
# ---------------------------------------------------------------------------
# Generic disk cache for provider_model_ids() â€” keeps /model picker fast.
# ---------------------------------------------------------------------------
#
...
#
# Cache strategy:
#   - One JSON file at $ZED_HOME/provider_models_cache.json
...
#     to a live fetch â€” the picker keeps working.
...
def _provider_models_cache_path() -> Path:
    from zed_constants import get_zed_home
    return get_zed_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    (codex, copilot, anthropic-via-claude-code, Zed Portal), the
    relevant tokens live in ``$ZED_HOME/auth.json`` and external
    ...
    # blake2b for cache-key fingerprinting only â€” not for credential storage.
    # We never reverse this hash; collisions are harmless (worst case: cache
    # miss â†’ live re-fetch). Use blake2b instead of sha256 here because
...
def _save_provider_models_cache(data: dict) -> None:
    """Persist the cache dict. Best-effort â€” silent on any error."""
    try:
...
def cached_provider_model_ids(
    ...
    *,
    ...
    # Cache miss / stale / forced refresh â€” call the live path.
    ...
    # SAME fingerprint, prefer it over an empty result â€” stale data
...
# â”€â”€â”€ Copilot catalog context-window helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def get_copilot_model_context(model_id: str, api_key: Optional[str] = None) -> Optional[int]:
    ...
    if _copilot_context_cache and (time.time() - _copilot_context_cache_time < _COPILOT_CONTEXT_CACHE_TTL):
        ...
        # Cache is fresh but model not in it â€” don't re-fetch
        return None
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _ZED_USER_AGENT}
...
def fetch_lmstudio_models(
    ...
    or wrong ``LM_API_KEY`` from an unreachable server â€” the most common
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Zed' default Claude IDs elsewhere use
...
# ``400 "The requested operation is unsupported."`` â€” the same payload Bob
...
def azure_foundry_model_api_mode(model_name: Optional[str]) -> Optional[str]:
    ...
    reasoning models).  Returns ``None`` otherwise â€” the caller should fall
    ...
    # the full gpt-5 family uniformly on Responses API â€” don't carve an
    # exception here.
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _ZED_USER_AGENT}
...
# ---------------------------------------------------------------------------
# Ollama Cloud â€” merged model discovery with disk cache
# ---------------------------------------------------------------------------
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from zed_constants import get_zed_home
    return get_zed_home() / "ollama_cloud_models_cache.json"
...
def fetch_ollama_cloud_models(
    ...
    *,
    ...
    Resolution order:
      ...
      2. Live ``/v1/models`` endpoint (primary â€” freshest source)
      3. models.dev registry (secondary â€” fills gaps for unlisted models)
      4. Merge: live models first, then models.dev additions (deduped)

    Returns a list of model IDs (never None â€” empty list on total failure).
    """
    # 1. Check disk cache
    ...
    # Total failure â€” return stale cache if available (ignore TTL)
    ...
    *,
        ...
        if not models:
            return {
                ...
                "message": (
                    ...
                    f"Load `{requested}` in LM Studio (Developer tab â†’ Load Model) and try again."
                ),
            }
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        if api_models is not None:
            ...
            if auto:
                return {
                    ...
                    "message": f"Auto-corrected `{requested}` â†’ `{auto[0]}`",
                }
        ...
        message = (
            ...
            f"Zed will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
    ...
    # Providers with non-standard catalog validation â€” /v1/models probing is not the right path.
    ...
    # MiniMax providers don't expose a /models endpoint â€” validate against
        ...
        if catalog_models:
            ...
            if auto:
                ...
                return {
                    ...
                    "message": f"Auto-corrected `{requested}` â†’ `{corrected}`",
                }
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Zed cannot verify the model name."
    ...
    if normalized == "anthropic":
        ...
        if anthropic_models is not None:
            ...
            if suggestions:
                suggestion_text = "\n  Similar models: " + ", ".join(f"`{s}`" for s in suggestions)
            # Accept anyway â€” Anthropic sometimes gates newer/preview models
        ...
        # _fetch_anthropic_models returned None â€” no token resolvable or
    ...
    if api_mode == "anthropic_messages":
        ...
        # Probe failed or model not found â€” accept anyway (proxy likely
    ...
    if api_models is not None:
        ...
        # prefixed with "models/" (e.g. "models/gemini-2.5-flash") â€” native
        ...
        if requested_for_lookup in set(api_models):
            ...
            # API responded but model is not listed.  Accept anyway â€”
            ...
            # Model not in live /v1/models â€” check the curated catalog
            ...
            # fetch) so we only accept models Zed actually ships.  (#46850)
            if _model_in_provider_catalog(
                ...
                return {
                    ...
                    "message": (
                        ...
                        f"but exists in the curated catalog â€” accepted."
                    ),
                }
    ...
    # api_models is None â€” couldn't reach API.  Accept and persist,
    # but warn so typos don't silently break things.

    # Bedrock: use our own discovery instead of HTTP /models endpoint.
    # Bedrock's bedrock-runtime URL doesn't support /models â€” it uses the
    ...
    if normalized == "bedrock":
        try:
            ...
            # Not in discovered list â€” still accept (user may have custom
    ...
    # validate against the curated list from provider_model_ids() â€” same
    ...
    # No catalog available â€” accept with a warning, matching the comment's
