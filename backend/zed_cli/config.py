...
- ~/.zed/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.zed/.env         - API keys and secrets

This module provides:
- zed config          - Show current configuration
- zed config edit     - Open config in editor
- zed config set      - Set a specific value
- zed config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    That file is still the user's only copy of their intended overrides â€” if
    they re-run the setup wizard or ``zed config set`` (which rewrites
    ...
    ``config.yaml`` in place: zed never silently mutates the user's config,
    and leaving it means a hand-fixed file is re-read on the next load. The
    backup is best-effort â€” any failure (permissions, symlink, disk full) is
        ...
        for existing in sibling_baks:
            try:
                if existing.stat().st_size == st.st_size:
                    # Same size as the current broken file â€” likely the same
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.zed/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``zed logs``
    ...
    msg = (
        ...
        f"Falling back to default config â€” every user override "
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"âš ï¸  zed config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
# Env var names that influence how the next subprocess executes â€”
# never writable through ``save_env_value``. Anything that controls
# the loader, interpreter, shell, or replacement editor counts:
#
# * ``LD_PRELOAD`` / ``LD_LIBRARY_PATH`` / ``LD_AUDIT`` â€” Linux dynamic
#   loader. ``DYLD_*`` â€” macOS equivalent. Planting a path here means
#   the next ``subprocess.run([...])`` Zed makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` â€” Python interpreter init. Zed itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` â€” Node interpreter; affects npm,
#   ``zed update``, the TUI build.
# * ``PATH`` â€” too broad to allow. The dashboard never needs to rewrite
...
# * ``GIT_SSH_COMMAND`` / ``GIT_EXEC_PATH`` â€” git rewrites that fire
#   on every plugin install / ``zed update``.
# * ``BROWSER`` / ``EDITOR`` / ``VISUAL`` / ``PAGER`` â€” commands the
#   shell or CLI invokes implicitly. Wrong values here = RCE on next
#   ``$EDITOR``.
# * ``SHELL`` â€” what subprocess uses with ``shell=True`` (we try to
#   avoid that, but defense in depth).
# * ``ZED_HOME`` / ``ZED_PROFILE`` / ``ZED_CONFIG`` /
#   ``ZED_ENV`` â€” Zed runtime location flags. Writing these into
...
#
# IMPORTANT: ``ZED_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ZED_GEMINI_CLIENT_ID,
# ZED_LANGFUSE_PUBLIC_KEY, ZED_SPOTIFY_CLIENT_ID, ...). The
# denylist is name-by-name on purpose so the gate stays narrow and
# doesn't accidentally break provider setup wizards.
#
# This is enforced on *write* only â€” values already in ``.env`` (set
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Zed runtime location â€” never via dashboard env writer.
    # NOT a ZED_* blanket: integration credentials (ZED_GEMINI_*,
    # ZED_LANGFUSE_*, ZED_SPOTIFY_*, ...) ARE allowed.
    "ZED_HOME", "ZED_PROFILE", "ZED_CONFIG", "ZED_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Zed runtime location "
            "(ZED_HOME, ZED_PROFILE, ...) cannot be persisted via "
...
# load repopulates automatically â€” no explicit invalidation hook.
# Cached tuple is (user_mtime_ns, user_size, managed_mtime_ns, managed_size,
# merged_value) â€” the managed-file signature is folded in so editing the
...
# _LOAD_CONFIG_CACHE but for read_raw_config() â€” used when callers want
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Deprecated tool-progress env vars â€” replaced by display.tool_progress in
    ...
    "ZED_TOOL_PROGRESS", "ZED_TOOL_PROGRESS_MODE",
    "WHATSAPP_MODE", "WHATSAPP_ENABLED",
    "MATTERMOST_HOME_CHANNEL", "MATTERMOST_HOME_CHANNEL_NAME", "MATTERMOST_REPLY_MODE",
    "MATRIX_PASSWORD", "MATRIX_ENCRYPTION", "MATRIX_DEVICE_ID", "MATRIX_HOME_ROOM",
    "MATRIX_REQUIRE_MENTION", "MATRIX_FREE_RESPONSE_ROOMS", "MATRIX_AUTO_THREAD", "MATRIX_DM_AUTO_THREAD",
    "MATRIX_RECOVERY_KEY",
    # Langfuse observability plugin â€” optional tuning keys + standard SDK vars.
    # Activation is via plugins.enabled (opt-in through `zed plugins enable
    # observability/langfuse` or `zed tools â†’ Langfuse`); credentials gate
    # the plugin at runtime.
    "ZED_LANGFUSE_ENV",
    "ZED_LANGFUSE_RELEASE",
    "ZED_LANGFUSE_SAMPLE_RATE",
    "ZED_LANGFUSE_MAX_CHARS",
    "ZED_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
from zed_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ZED_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_zed_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Zed is running in package-manager-managed mode.

    Two signals: the ZED_MANAGED env var (set by the systemd service),
    or a .managed marker file in ZED_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``zed_cli/`` â€” i.e. the git checkout for source
    installs, ``/opt/zed`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$ZED_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Zed was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) â€” the authoritative marker.
    2. Legacy home-scoped stamp ``$ZED_HOME/.install_method`` â€” read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. ZED_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.zed``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$ZED_HOME`` is a shared DATA directory â€” the Docker docs deliberately
    bind-mount it (``~/.zed:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$ZED_HOME``, a
    ...
    ``docker`` and ``zed update`` refuses to run ("doesn't apply inside the
    ...
    through to managed/.git/pip detection instead â€” so existing shared-home
    ...
    # 1. Code-scoped stamp â€” authoritative, immune to shared $ZED_HOME.
    try:
    ...
    # 2. Legacy home-scoped stamp â€” back-compat. Ignore a ``docker`` value
    #    when we are not actually containerised: that is the signature of a
    #    host install whose shared $ZED_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``zed update``.
    try:
        method = (
            (get_zed_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``zed_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$ZED_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/zed`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Zed lives in a ``uv tool`` layout.

    ``uv tool install zed-agent`` places the install at
    ``.../uv/tools/zed-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``zed-agent`` happens to be uv-tool-installed on the machine while
    the *active* Zed is a regular pip/venv install, causing
    ``zed update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/zed-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "pip":
        ...
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade zed-agent"
        return "pip install --upgrade zed-agent"
    return "zed update"
...
# Long-form text for ``zed update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# zed_cli/main.py; lives here so the wording stays consistent and we
...
#   - The pre-existing fallback message ("âœ— Not a git repository. Please
#     reinstall: curl ... install.sh") is actively misleading inside Docker
#     â€” that script installs a *new* host-side Zed, it doesn't update
...
Notes:
  ...
  â€¢ Running a fork?  Build your own image with this repo's ``Dockerfile``
    and replace the ``docker pull`` step with your build/push pipeline."""


def format_docker_update_message() -> str:
    """Return the user-facing message for ``zed update`` inside Docker.
...
def format_managed_message(action: str = "modify this Zed installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ZED_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Zed installation is managed by NixOS "
            f"(ZED_MANAGED={env_hint}).\n"
            "Edit services.zed-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Zed installation is managed by Homebrew "
            f"(ZED_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade zed-agent"
        )

    return (
        f"Cannot {action}: this Zed installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Zed."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ZED_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, zed_bin
    or None if container mode is not active, we're already inside the
    container, or ZED_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ZED_DEV") == "1":
        return None
    ...
    container_mode_file = get_zed_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "zed-agent")
    exec_user = info.get("exec_user", "zed")
    zed_bin = info.get("zed_bin", "/data/current-package/bin/zed")
...
# Re-export from zed_constants â€” canonical definition lives there.
from zed_constants import get_zed_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_zed_home() / "config.yaml"
...
def _resolve_zed_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ZED_UID / ZED_GID env vars set by Docker deployments.

    Docker containers running Zed commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level ZED_HOME once, but
    subdirectories created at runtime by ``ensure_zed_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ZED_UID", "").strip()
    gid_str = os.environ.get("ZED_GID", "").strip()
    try:
...
def _chown_to_zed_uid(path) -> None:
    """Chown ``path`` to ``ZED_UID:ZED_GID`` if those env vars are set.

    No-op when:
      - Either env var is unset/invalid
      - The current process isn't root (chown will EPERM â€” silently ignored)
      - On Windows (chown semantics don't apply)

    Used by :func:`_secure_dir` to keep ownership consistent across all
    directories created by :func:`ensure_zed_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_zed_uid_gid()
    ...
    try:
        ...
        # both of which are non-fatal â€” the dir is still created and
...
def _secure_dir(path):
    ...
    Skipped in managed mode â€” the NixOS module sets group-readable
    permissions (0750) so interactive users in the zed group can
    share state with the gateway service.

    The mode can be overridden via the ZED_HOME_MODE environment variable
    (e.g. ZED_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ZED_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``ZED_UID``/``ZED_GID``-based ownership when those env
    vars are set (#34107 â€” Docker deployments need this so profile subdirs
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ZED_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_zed_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Zed runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ZED_CONTAINER") or os.environ.get("ZED_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    Skipped in managed mode â€” the NixOS activation script sets
    group-readable permissions (0640) on config files.

    Skipped in containers â€” Docker/Podman volume mounts often need broader
    permissions.  Set ZED_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ZED_HOME if the user doesn't have one yet."""
...
def ensure_zed_home():
    """Ensure ~/.zed directory structure exists with secure permissions.
...
def _ensure_zed_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ZED_HOME {home} does not exist. "
    ...
    # Inside umask(0o007) scope â€” SOUL.md will be created as 0660
    _ensure_default_soul_md(home)
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # coding conversation mid-reasoning runs 60â€“150s per call, so a 60s
        ...
        # the Zed-level retry loop that wraps the whole call.  Lower
        ...
        # Values: "auto" (default â€” applies to gpt/codex models), true/false
        # (force on/off for all models), or a list of model-name substrings
        # to match (e.g. ["gpt", "codex", "gemini", "qwen"]).
        "tool_use_enforcement": "auto",
        # Universal "finish the job" guidance â€” short prompt block applied to
        ...
        # Universal parallel-tool-call guidance â€” short prompt block applied to
        ...
        # batch â€” cutting round-trips and the resent-context cost that
        # compounds over a long conversation.  Costs ~70 tokens in the cached
        # system prompt.  Set False to disable globally.
        "parallel_tool_call_guidance": True,
        # Local-environment toolchain probe â€” surfaces Python/pip/uv/PEP-668
        # state in the system prompt when something non-default is detected
        # (e.g. python3 has no pip module, pipâ†’python version mismatch, PEP
        ...
        # (docker/modal/ssh â€” they have their own probe).  Set False to
        # disable entirely.
        "environment_probe": True,
        # Embedder-supplied environment description appended to the system
        # prompt's environment-hints block. Lets a host that wraps Zed
        # (sandbox runner, managed platform) explain the runtime environment
        # â€” proxy, credential handling, mount layout â€” without editing the
        # identity slot (SOUL.md). Empty by default. The ZED_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture â€” on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Zed adds a coding operating brief
        # + a live git/workspace snapshot to the system prompt. See
        # agent/coding_context.py.
        #   "auto" (default) â€” prompt-only posture when the surface is
        #                      interactive AND cwd is a code workspace.
        #                      Toolsets are never touched; messaging platforms
        #                      unaffected.
        #   "focus"          â€” auto + collapse the toolset to the lean coding
        #                      set (+ enabled MCP servers) + demote non-coding
        #                      skill categories to names-only in the prompt's
        #                      skill index. Explicit opt-in.
        #   "on"             â€” force the prompt posture everywhere.
        #   "off"            â€” disable entirely.
        "coding_context": "auto",
        ...
        # interrupted â€” process the unfinished tool result(s) first]"
    ...
    "terminal": {
        ...
        #   auto    â€” host keeps the real OS-user HOME; containers use
        #             ZED_HOME/home for persistent state (default)
        #   real    â€” force the real OS-user HOME
        #   profile â€” force ZED_HOME/home when it exists (old strict
        ...
        # a bash login shell would skip â€” most commonly ``~/.bashrc``
        # (bash doesn't source bashrc in non-interactive login mode) or
        # zsh-specific files like ``~/.zshrc`` / ``~/.zprofile``.
        # Paths support ``~`` / ``${VAR}``. Missing files are silently
        # skipped. When empty, Zed auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour â€” disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Zed sources the user's shell rc files
        # (``~/.profile``, ``~/.bash_profile``, ``~/.bashrc``) in the
        # login shell used to build the environment snapshot. This
        # captures PATH additions, shell functions, and aliases â€” which a
        ...
        # docker_env lets you specify exact key-value pairs â€” useful when Zed
        ...
        # Container resource limits (docker, singularity, modal, daytona â€” ignored for local/ssh)
        "container_cpu": 1,
        "container_memory": 5120,       # MB (default 5GB)
        "container_disk": 51200,        # MB (default 50GB)
        "container_persistent": True,   # Persist filesystem across sessions
        # Docker volume mounts â€” share host directories with the container.
        ...
        # bundled Zed image, which drops to the `zed` user via
        # s6-setuidgid inside each supervised service).
        # When on, SETUID/SETGID caps are omitted from the container since
        # no privilege drop is needed.
        "docker_run_as_host_user": False,
        # Persistent shell â€” keep a long-lived bash shell across execute() calls
    ...
    "web": {
        "backend": "",           # shared fallback â€” applies to both search and extract
    ...
    "browser": {
        ...
        # "auto"       â€” use Chrome (default, don't pass --engine at all)
        # "lightpanda" â€” use Lightpanda (1.3-5.8x faster navigation, no screenshots)
        # "chrome"     â€” explicitly request Chrome
        # Also settable via AGENT_BROWSER_ENGINE env var.
        "engine": "auto",
        "auto_local_for_private_urls": True,  # When a cloud provider is set, auto-spawn local Chromium for LAN/localhost URLs instead of sending them to the cloud
        "cdp_url": "",  # Optional persistent CDP endpoint for attaching to an existing Chromium/Chrome
        # CDP supervisor â€” dialog + frame detection via a persistent WebSocket.
        ...
        "camofox": {
            # When true, Zed sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Zed should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Filesystem checkpoints â€” automatic snapshots before destructive file ops.
    ...
    # Opt in via ``zed chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.zed/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: zed sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .zed.md, or .cursorrules before Zed applies
    ...
    # 100K chars â‰ˆ 25â€“35K tokens across typical tokenisers.
    ...
    # case) or fast servers pay ~0s regardless of this value â€” the bound is
    ...
    # single read_file page exceeds these limits, Zed truncates the
    ...
    #                    (default 50_000 â‰ˆ 12-15K tokens).
    # - max_lines:       read_file pagination cap â€” the maximum `limit`
    ...
    "compression": {
        ...
        "codex_gpt55_autoraise": True,  # When True, gpt-5.5 on the ChatGPT Codex OAuth
                                      ...
                                      # exact route is affected â€” gpt-5.5 on OpenAI's
                                      # direct API, OpenRouter, and Copilot keep the
                                      # global threshold regardless.
    },

    # Kanban subsystem (orchestrator workers + dispatcher-driven child tasks).
    # See tools/kanban_tools.py and zed_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``zed dashboard`` -> Notifications).
    "kanban": {
        ...
        # behaviour â€” e.g. for a profile that prefers explicit
    ...
    "bedrock": {
        "region": "",  # AWS region for Bedrock API calls (empty = AWS_REGION env var â†’ us-east-1)
        "discovery": {
        ...
        "guardrail": {
            # Amazon Bedrock Guardrails â€” content filtering and safety policies.
    ...
    # Auxiliary model config â€” provider:model for each side task.
    ...
    #
    # Each aux task is independent â€” main-agent provider_routing and
    ...
    "auxiliary": {
        "vision": {
            ...
            "download_timeout": 30,  # seconds â€” image HTTP download timeout; increase for slow connections
        },
        "web_extract": {
            "provider": "auto",
            "model": "",
            "base_url": "",
            "api_key": "",
            "timeout": 360,        # seconds (6min) â€” per-attempt LLM summarization timeout; increase for slow local models
            "extra_body": {},
        },
        "compression": {
            "provider": "auto",
            ...
            "base_url": "",
            "api_key": "",
            "timeout": 120,        # seconds â€” compression summarises large contexts; increase for local models
            "extra_body": {},
        },
        # Note: session_search no longer uses an auxiliary LLM (PR #27590 â€”
        ...
        # Triage specifier â€” flesh out a rough one-liner in the Kanban
        # Triage column into a concrete spec, then promote it to ``todo``.
        # Invoked by ``zed kanban specify`` (single id or --all). Set a
        ...
        # Kanban decomposer â€” decomposes a triage task into a graph of
        # child tasks routed to specialist profiles by description.
        # Invoked by ``zed kanban decompose`` and the kanban
        ...
        # Profile describer â€” auto-generates a 1-2 sentence description
        # of what a profile is good at. Invoked by
        # ``zed profile describe <name> --auto`` and the dashboard's
        ...
        # Curator â€” skill-usage review fork. Timeout is generous because the
        # review pass can take several minutes on reasoning models (umbrella
        # building over hundreds of candidate skills). "auto" = use main chat
        # model; override via `zed model` â†’ auxiliary â†’ Curator to route
        ...
        # Monitor â€” urgency/importance classifier used by the important-mail
    ...
    "display": {
        ...
        # Which interface bare `zed` (and `zed chat`) launches by default:
        #   "cli" â€” the classic prompt_toolkit REPL (default, preserves prior behavior)
        #   "tui" â€” the modern Ink TUI (same as passing `--tui`)
        # Explicit flags always win over this setting: `--cli` forces the classic
        # REPL and `--tui` (or ZED_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `zed --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `zed -c` muscle memory.  Default off so existing
        # users aren't surprised.  ZED_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `zed --tui` drops a one-time hint
        # ("subagents working Â· /agents to watch live") the first time a turn
        ...
        #   "off"     â€” no chat notification (the review still runs and writes)
        #   "on"      â€” generic "ðŸ’¾ Memory updated" line (default)
        #   "verbose" â€” include a compact content preview of what changed
        ...
        # abnormally with no usable reply â€” empty content after retries, a
        ...
        # last-turn) ticking and keeps the bottom chrome alive during idle â€”
        ...
        # Auto-delete system-notice replies (e.g. "âœ¨ New session started!",
        # "â™» Restarting gatewayâ€¦", "âš¡ Stoppedâ€¦") after N seconds on platforms
        ...
        # replies wrapped with gateway.platforms.base.EphemeralReply â€” agent
        ...
        # platform ("telegram", "discord", "slack", â€¦) mapping to a dict of
        ...
        # streaming.enabled master switch still gates everything â€” these
        ...
        # e.g. `model Â· 68% Â· ~/projects/zed`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``ZED_DASHBOARD_PORTAL_URL`` respectively â€” and the env var
        ...
        # Username/password gate configuration â€” read by the bundled
        ...
        # is generated â€” fine for a single process, but sessions then
        ...
        "basic_auth": {
            "username": "",  # blank â†’ plugin no-op (no password provider)
            "password_hash": "",  # scrypt$... (preferred â€” no plaintext at rest)
            "password": "",  # plaintext fallback (hashed in-memory at load)
            "secret": "",  # token-signing key; blank â†’ random per-process
            "session_ttl_seconds": 0,  # 0 â†’ plugin default (12h)
        },
        # Public URL override (env: ``ZED_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority â€” scheme + host +
        # optional path prefix (e.g. ``https://example.com/zed``) â€”
        ...
        # the operator has declared the public URL â€” we no longer need
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Zed appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
            "voice_id": "eve",  # or custom voice ID â€” see https://docs.x.ai/developers/model-capabilities/audio/custom-voices
            "language": "en",
            "sample_rate": 24000,
            "bit_rate": 128000,
        },
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.zed/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.zed/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "memory": {
        ...
        #   false (default) â€” write freely; the gate is off (pre-gate behaviour)
        #   true            â€” require approval: foreground writes prompt inline
    ...
    # Subagent delegation â€” override the provider:model used by delegate_task
    # so child agents can run on a different (cheaper/faster) provider and model.
    # Uses the same runtime provider resolution as CLI/gateway startup, so all
    # configured providers (OpenRouter, Nous, Z.ai, Kimi, etc.) are supported.
    "delegation": {
        "model": "auto",       # "auto" = freellmapi routes each child independently across all
                               # available models (89+ across OpenRouter/Groq/NVIDIA/CF/Google/...).
                               # This gives sub-agents Kimi-style swarm load balancing: each child
                               # hits a different model, 429s auto-failover, rate limits spread.
        ...
        "base_url": "http://127.0.0.1:3002/v1",  # freellmapi loopback (no-auth). Each child request
                               # is routed by freellmapi across the full multi-model pool.
        "api_key": "no-auth",  # freellmapi requires no key (loopback-only, OS-level security).
        ...
        "max_spawn_depth": 1,        # depth (1 = flat [default], 2 = orchestratorâ†’leaf, 3+ = deeper)
        "orchestrator_enabled": True,  # kill switch for role="orchestrator"
        # When a subagent hits a dangerous-command approval prompt, the parent's
        # prompt_toolkit TUI owns stdin â€” a thread-local input() call from the
        ...
        #   false (default) â†’ auto-deny with a logger.warning audit line (safe)
        #   true             â†’ auto-approve "once" with a logger.warning audit line
    ...
    # Goals â€” persistent cross-turn goals (Ralph-style loop).
    ...
    # response. If not, Zed feeds a continuation prompt back into the
    ...
    # fail OPEN (continue) so a flaky judge never wedges progress â€” the
    # turn budget is the real backstop.
    "goals": {
        # Max continuation turns before Zed auto-pauses the goal and
    ...
    # Skills â€” external skill directories for sharing skills across tools/agents.
    # Each path is expanded (~, ${VAR}) and resolved.  Read-only â€” skill creation
    # always goes to ~/.zed/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ZED_SKILL_DIR} and ${ZED_SESSION_ID} in SKILL.md
        ...
        # state, detected tool versions, â€¦).  Off by default because any
        ...
        # security.  Turn on if you want the belt-and-suspenders â€” a
        ...
        #   false (default) â€” write freely; the gate is off (pre-gate behaviour)
        #   true            â€” require approval: stage the write for review
        ...
        #                     /skills diff <id> (full diff â€” CLI/dashboard/file,
        #                     never crammed into a chat bubble), apply with
        #                     /skills approve <id> or drop with /skills reject <id>.
        "write_approval": False,
    },

    # Curator â€” background skill maintenance.
    #
    ...
    # and patch drift. Runs inactivity-triggered from session start â€” no
    # cron daemon.
    #
    # See `zed curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # without use. Archived skills are recoverable â€” no auto-deletion.
        ...
        # aux-model review entirely â€” no umbrella-building, no aux-model cost.
        # Set to true to opt back into merging overlapping skills into
        # class-level umbrellas. `zed curator run --consolidate` overrides
        ...
        # normally restored on every `zed update`, so pruning them only
        # sticks because a suppression list tells the re-seeder to leave them
        # archived. Hub-installed skills are NEVER pruned here â€” they have an
        ...
        # skipped), snapshot ~/.zed/skills/ into
        # ~/.zed/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `zed curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for zed-specific overrides; everything else
    ...
    "discord": {
        ...
        "reactions": True,             # Add ðŸ‘€/âœ…/âŒ reactions to messages during processing
        "channel_prompts": {},         # Per-channel ephemeral system prompts (forum parents apply to child threads)
        # Opt-in DM role-based auth (#12136). By default, DISCORD_ALLOWED_ROLES
        # authorizes only guild messages in the role's own guild â€” DMs require
        ...
        # stop-and-swap â€” the Grok-voice-mode feel. discord.py ships no mixer;
        ...
        "voice_fx": {
            ...
            "ambient_gain": 0.18,     # idle bed loudness, 0.0â€“1.0
            "duck_gain": 0.06,        # ambient loudness while speech plays
            "speech_gain": 1.0,       # TTS / ack loudness, 0.0â€“1.0
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "âš• *Zed Agent*" header.
        # Set to "" (empty string) to disable the header entirely.
        # Supports \n for newlines, e.g. "ðŸ¤– *My Bot*\nâ”€â”€â”€â”€â”€â”€\n"
    },

    # Telegram platform settings (gateway mode)
    "telegram": {
        "reactions": False,            # Add ðŸ‘€/âœ…/âŒ reactions to messages during processing
    ...
    #   manual â€” always prompt the user (default)
    #   smart  â€” use auxiliary LLM to auto-approve low-risk commands, prompt for high-risk
    #   off    â€” skip all approval prompts (equivalent to --yolo)
    #
    # cron_mode â€” what to do when a cron job hits a dangerous command:
    #   deny    â€” block the command and let the agent find another way (default, safe)
    #   approve â€” auto-approve all dangerous commands in cron jobs
    "approvals": {
        "mode": "manual",
        "timeout": 60,
        "cron_mode": "deny",
        ...
        # prompt), so the next message re-sends full input tokens â€” this can
        ...
        # through tools.slash_confirm â€” native yes/no buttons on Telegram,
        ...
        # false.  TUI has its own modal overlay (ZED_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Zed' built-in platform hint for a single messaging platform
    ...
    #   { "append": "extra text" }   â€” keep the default hint, append text
    #   { "replace": "full text" }   â€” substitute the default hint entirely
    #   "extra text"                 â€” shorthand for { "append": ... }
    ...
    # Shell-script hooks â€” declarative bridge that invokes shell scripts
    ...
    # stored approval from ~/.zed/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ZED_ACCEPT_HOOKS=1.
    # Gateway / cron / non-interactive runs need this (or one of the other
    # channels) to pick up newly-added hooks.
    "hooks_auto_accept": False,
    # Custom personalities â€” add your own entries here
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `zed doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``zed_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Zed to lazy-install opt-in backend packages from PyPI
        ...
        # ``pip install`` for everything beyond the base set â€” appropriate
    ...
    "cron": {
        # Active cron SCHEDULER provider (Axis B â€” the trigger that decides
        ...
        # $ZED_HOME/plugins/<name>/) to relocate the trigger â€” e.g. "chronos",
        ...
        "chronos": {
            ...
            # Empty â†’ the fire endpoint refuses all tokens (no unsigned decode).
            "nas_jwks_url": "",
        },
        ...
        # Also overridable via ZED_CRON_MAX_PARALLEL env var.
        "max_parallel_jobs": None,
    },

    # Kanban multi-agent coordination â€” controls the dispatcher loop that
    # spawns workers for ready tasks. The dispatcher ticks every N seconds
    # (default 60), reclaims stale claims, promotes dependency-satisfied
    # todos to ready, and fires `zed -p <assignee> chat -q ...` for
    # each claimable ready task. One dispatcher per profile is sufficient;
    # running more than one on the same kanban.db will race for claims.
    "kanban": {
        # Run the dispatcher inside the gateway process. On by default â€”
        # the cost is ~300Âµs every `dispatch_interval_seconds` when idle,
        ...
        # one `zed` launches with no -p flag). This does not control the
        ...
        # Unset (None) means "no per-profile cap" â€” backward-compatible
        ...
        # decomposition is manual via `zed kanban decompose <id>` or
    ...
    # execute_code settings â€” controls the tool used for programmatic tool calls.
    "code_execution": {
        # Execution mode:
        #   project (default) â€” scripts run in the session's working directory
        #     with the active virtualenv/conda env's python, so project deps
        #     (pandas, torch, project packages) and relative paths resolve.
        #   strict            â€” scripts run in an isolated temp directory with
        #     zed-agent's own python (sys.executable). Maximum isolation
    ...
    # in the model-facing tools array with three bridge tools â€”
    # tool_search / tool_describe / tool_call â€” and surfaced on demand.
    #
    # Core Zed tools (terminal, read_file, write_file, patch,
    ...
    # Logging â€” controls file logging to ~/.zed/logs/.
    ...
    # Network settings â€” workarounds for connectivity issues.
    "network": {
    ...
    # Gateway settings â€” control how messaging platforms (Telegram, Discord,
    ...
    "gateway": {
        ...
        # sent. Off by default â€” when off, the model sees clean message text.
        ...
        # user. Bridged to ZED_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Zed
        # cache (~/.zed/cache/{documents,images,audio,video,screenshots})
        ...
        # to ZED_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # operator allowlist â€” useful for ``pandoc -o /tmp/report.pdf`` or
        ...
        # ZED_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ZED_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # the feature discoverable in config.yaml â€” it does not change behavior.
    #
    ...
    "streaming": {
        ...
        "enabled": False,
        # Transport selection:
        #   "auto"  â€” prefer native draft streaming where the platform
        ...
        #   "draft" â€” explicitly request native drafts; falls back to edit
        #             when the platform/chat doesn't support them.
        #   "edit"  â€” progressive editMessageText only (legacy behavior).
        #   "off"   â€” disable streaming entirely (same as enabled: false).
        "transport": "auto",
        # Minimum seconds between progressive edits â€” tuned for Telegram's
    ...
    # Session storage â€” controls automatic cleanup of ~/.zed/state.db.
    ...
    "sessions": {
        ...
        # Only touches ended sessions â€” active sessions are always preserved.
        # Default false: session history is valuable for search recall, and
        # silently deleting it could surprise users.  Opt in explicitly.
        "auto_prune": False,
        # How many days of ended-session history to keep.  Matches the
        # default of ``zed sessions prune``.
        "retention_days": 90,
        # VACUUM after a prune that actually deleted rows.  SQLite does not
        # reclaim disk space on DELETE â€” freed pages are just reused on
        # subsequent INSERTs â€” so without VACUUM the file stays bloated
        ...
        # rewrites ``~/.zed/sessions/session_{sid}.json`` on every turn
        ...
        # and token counts), so this is off by default â€” the snapshots had
    ...
    # ``zed update`` behaviour.
    "updates": {
        # Run a full ``zed backup``-style zip of ZED_HOME before every
        # ``zed update``.  Backups land in ``<ZED_HOME>/backups/`` and
        # can be restored with ``zed import <path>``.  Defaults to true
        # after the #48200 incident: a ``zed update --yes`` run that
        ...
        # floored to 1 â€” the backup just created is always preserved.  To
        # disable backups entirely, set ``pre_update_backup: false`` above
        # rather than ``backup_keep: 0``.
        "backup_keep": 5,
        # What `zed update` does with uncommitted local changes to the
        # source tree when it runs NON-interactively â€” i.e. triggered from
        ...
        # as they always have.
        #   "stash"   â€” auto-stash the changes, pull, then auto-restore them
        #               on top of the updated code (the safe default; nothing
        #               is ever lost â€” conflicts are preserved in a git stash).
        #   "discard" â€” auto-stash the changes and throw the stash away after
        ...
        #               ignored paths â€” node_modules, venv, build outputs â€”
        #               are never touched.
        "non_interactive_local_changes": "stash",
    },

    # Language Server Protocol â€” semantic diagnostics from real
    ...
    # tier â€” handy for Telegram/Discord chats where the cwd is the
    # user's home directory.
    "lsp": {
        # Master toggle.  Setting this to false disables the entire
        # subsystem â€” no servers spawn, no background event loop, no
        # cost.
        "enabled": True,
        ...
        # ``"auto"`` â€” try to install via npm/go/pip into
        #              ``<ZED_HOME>/lsp/bin/`` on first use.
        # ``"manual"`` â€” only use binaries already on PATH.
        # ``"off"`` â€” alias for ``manual``.
        ...
        #   disabled: true
        #     â€” skip this server even when its extensions match
        #   command: ["full/path/to/server", "--stdio"]
        #     â€” pin a custom binary path; bypasses auto-install
        #   env: {"KEY": "value"}
        #     â€” extra env vars passed to the spawned process
        #   initialization_options: {...}
        #     â€” merged into the LSP ``initializationOptions``
    ...
    # `zed tools`. These settings tune the backing Responses API call.
    "x_search": {
        ...
        # 60-120s for complex queries â€” the default is generous.
        "timeout_seconds": 180,
    ...
    "secrets": {
        "bitwarden": {
            ...
            # True because the point of using BSM is centralized rotation â€”
            ...
            # ~/.zed/bin/ on first use.  When False you must install
...
OPTIONAL_ENV_VARS = {
    ...
    "ZED_GEMINI_CLIENT_ID": {
        "description": "Google OAuth client ID for google-gemini-cli (optional; defaults to Google's public gemini-cli client)",
        "prompt": "Google OAuth client ID (optional â€” leave empty to use the public default)",
        "url": "https://console.cloud.google.com/apis/credentials",
        "password": False,
        "category": "provider",
        "advanced": True,
    },
    "ZED_GEMINI_CLIENT_SECRET": {
    ...
    "OLLAMA_API_KEY": {
        "description": "Ollama Cloud API key (ollama.com â€” cloud-hosted open models)",
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'zed model' for endpoint-specific config)",
    ...
    "BROWSERBASE_API_KEY": {
        "description": "Browserbase API key for cloud browser (optional â€” local browser works without this)",
    ...
    "BROWSERBASE_PROJECT_ID": {
        "description": "Browserbase project ID (optional â€” only needed for cloud browser)",
    ...
    "BROWSER_USE_API_KEY": {
        "description": "Browser Use API key for cloud browser (optional â€” local browser works without this)",
    ...
    # â”€â”€ Bundled skills (opt-in: only needed if the user uses that skill) â”€â”€
    # These use category="skill" (distinct from "tool") so the sandbox
    # env blocklist in tools/environments/local.py does NOT rewrite them â€”
    ...
    "SLACK_APP_TOKEN": {
        "description": "Slack app-level token (xapp-) for Socket Mode. Get from Basic Information â†’ "
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Zed, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @zed:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ZED_BOT)",
    ...
    "MATRIX_RECOVERY_KEY": {
        "description": "Matrix recovery key for cross-signing verification after device key rotation (from Element: Settings â†’ Security â†’ Recovery Key)",
    ...
    "BLUEBUBBLES_PASSWORD": {
        "description": "BlueBubbles server password (from BlueBubbles Server â†’ Settings â†’ API)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #zed)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: zed-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'zed-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Zed API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only â€” all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Zed API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Zed API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # NOTE: MESSAGING_CWD was removed here â€” use terminal.cwd in config.yaml
    ...
    # ZED_TOOL_PROGRESS and ZED_TOOL_PROGRESS_MODE are deprecated â€”
...
# Tool Gateway env vars are always visible â€” they're useful for
...
def _set_nested(config, dotted_key: str, value):
    ...
    Supports both dict and list navigation:
      _set_nested(c, "a.b.c", 1)     â†’ c["a"]["b"]["c"] = 1
      _set_nested(c, "a.0.b", 1)     â†’ c["a"][0]["b"] = 1
      _set_nested(c, "providers.1", "x") â†’ c["providers"][1] = "x"
    ...
    (we do not grow lists â€” the user is navigating into structure they
    wrote themselves).  If a segment targets a non-container leaf
    (scalar), the leaf is replaced with a fresh dict so the write can
    proceed â€” this preserves the pre-existing behavior for bare scalar
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.zed.config`` entries, then checks
    ...
    try:
        ...
        # should never break `zed update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _CAMEL_ALIASES: Dict[str, str] = {
        ...
        "apiMode": "api_mode",
        "keyEnv": "key_env",
        "apiKeyEnv": "key_env",  # alias â€” OpenClaw-compatible + docs variant
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Zed versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``zed_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_VALID_CUSTOM_PROVIDER_FIELDS = {
    ...
    # â€” include it here so the set accurately describes the supported schema.
    "key_env",
}
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'zed setup' to create a valid config")]

    issues: List[ConfigIssue] = []

    # â”€â”€ custom_providers must be a list, not a dict â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    cp = config.get("custom_providers")
    if cp is not None:
        if isinstance(cp, dict):
            issues.append(ConfigIssue(
                "error",
                "custom_providers is a dict â€” it must be a YAML list (items prefixed with '-')",
                "Change to:\n"
                "  custom_providers:\n"
                "    - name: my-provider\n"
                "      base_url: https://...\n"
    ...
    # â”€â”€ fallback_model: single dict OR list of dicts (chain) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    fb = config.get("fallback_model")
    if fb is not None:
        if isinstance(fb, list):
            # Chain fallback â€” validate each entry
            ...
            if not fb.get("provider"):
                issues.append(ConfigIssue(
                    "warning",
                    "fallback_model is missing 'provider' field â€” fallback will be disabled",
                    "Add: provider: openrouter (or another provider)",
                ))
            if not fb.get("model"):
                issues.append(ConfigIssue(
                    "warning",
                    "fallback_model is missing 'model' field â€” fallback will be disabled",
                    "Add: model: anthropic/claude-sonnet-4 (or another model)",
                ))

    # â”€â”€ Check for fallback_model accidentally nested inside custom_providers â”€â”€
    ...
    # â”€â”€ model section: should exist when custom_providers is configured â”€â”€
    model_cfg = config.get("model")
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section â€” Zed won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # â”€â”€ Root-level keys that look misplaced â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    for key in config:
        if key.startswith("_"):
            continue
        if key not in _KNOWN_ROOT_KEYS and key in _CUSTOM_PROVIDER_LIKE_FIELDS:
            issues.append(ConfigIssue(
                "warning",
                f"Root-level key '{key}' looks misplaced â€” should it be under 'model:' or inside a 'custom_providers' entry?",
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines = ["\033[33mâš  Config issues detected in config.yaml:\033[0m"]
    for ci in issues:
        marker = "\033[31mâœ—\033[0m" if ci.severity == "error" else "\033[33mâš \033[0m"
        lines.append(f"  {marker} {ci.message}")
    lines.append("  \033[2mRun 'zed doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    These env vars are deprecated â€” the canonical setting is terminal.cwd
    ...
    if messaging_cwd:
        lines.append(
            f"  \033[33mâš \033[0m MESSAGING_CWD={messaging_cwd} found in .env â€” "
            f"this is deprecated."
        )
    if terminal_cwd_env and not config_has_explicit_cwd:
        # TERMINAL_CWD in env but not from config bridge â€” likely from .env
        lines.append(
            f"  \033[33mâš \033[0m TERMINAL_CWD={terminal_cwd_env} found in .env â€” "
            f"this is deprecated."
        )
    if lines:
        hint_path = os.environ.get("ZED_HOME", "~/.zed")
        lines.insert(0, "\033[33mâš  Deprecated .env settings detected:\033[0m")
        lines.append(
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    # â”€â”€ Always: sanitize .env (split concatenated keys) â”€â”€
    try:
        fixes = sanitize_env_file()
        if fixes and not quiet:
            print(f"  âœ“ Repaired .env file ({fixes} corrupted entries fixed)")
    except Exception:
    ...
    # â”€â”€ Version 3 â†’ 4: migrate tool progress from .env to config.yaml â”€â”€
    if current_ver < 4:
        config = load_config()
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ZED_TOOL_PROGRESS")
            old_mode = get_env_value("ZED_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ZED_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ZED_TOOL_PROGRESS_MODE)")
            else:
            ...
            save_config(config)
            if not quiet:
                print(f"  âœ“ Migrated tool progress to config.yaml: {display['tool_progress']}")
    
    # â”€â”€ Version 4 â†’ 5: add timezone field â”€â”€
    if current_ver < 5:
        config = load_config()
        if "timezone" not in config:
            old_tz = os.getenv("ZED_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ZED_TIMEZONE)")
            else:
                config["timezone"] = ""
            ...
            if not quiet:
                ...
                print(f"  âœ“ Added timezone to config.yaml: {tz_display}")

    # â”€â”€ Version 8 â†’ 9: clear ANTHROPIC_TOKEN from .env â”€â”€
    ...
    if current_ver < 9:
        try:
            ...
            if old_token:
                ...
                if not quiet:
                    print("  âœ“ Cleared ANTHROPIC_TOKEN from .env (no longer used)")
        except Exception:
            pass

    # â”€â”€ Version 11 â†’ 12: migrate custom_providers list â†’ providers dict â”€â”€
    if current_ver < 12:
        config = load_config()
        ...
        if isinstance(custom_list, list) and custom_list:
            ...
            if migrated_count > 0:
                ...
                # Remove the old list â€” runtime reads via get_compatible_custom_providers()
                config.pop("custom_providers", None)
                save_config(config)
                if not quiet:
                    print(f"  âœ“ Migrated {migrated_count} custom provider(s) to providers: section")
                    for key in list(providers_dict.keys())[-migrated_count:]:
                        ep = providers_dict[key]
                        print(f"    â†’ {key}: {ep.get('api', '')}")

    # â”€â”€ Version 12 â†’ 13: clear dead LLM_MODEL / OPENAI_MODEL from .env â”€â”€
    ...
    # Stale entries cause user confusion â€” see issue report.
    if current_ver < 13:
        for dead_var in ("LLM_MODEL", "OPENAI_MODEL"):
            try:
                ...
                if old_val:
                    ...
                    if not quiet:
                        print(f"  âœ“ Cleared {dead_var} from .env (no longer used â€” config.yaml is source of truth)")
            except Exception:
                pass

    # â”€â”€ Version 13 â†’ 14: migrate legacy flat stt.model to provider section â”€â”€
        ...
        if isinstance(raw_stt, dict) and "model" in raw_stt:
            ...
            if provider in {"local", "local_command"}:
                ...
                if legacy_model in _local_models:
                    # Check raw config â€” only set if user didn't already
                ...
                # else: drop it â€” it was an OpenAI model name, local section
                # already defaults to "base" via DEFAULT_CONFIG
            else:
                # Cloud provider â€” put it in that provider's section only
            ...
            save_config(config)
            if not quiet:
                print(f"  âœ“ Migrated legacy stt.model to provider-specific config")

    # â”€â”€ Version 14 â†’ 15: add explicit gateway interim-message gate â”€â”€
    if current_ver < 15:
        ...
        if "interim_assistant_messages" not in display:
            ...
            save_config(config)
            if not quiet:
                print("  âœ“ Added display.interim_assistant_messages=true")

    # â”€â”€ Version 15 â†’ 16: migrate tool_progress_overrides into display.platforms â”€â”€
    if current_ver < 16:
        config = read_raw_config()
        display = config.get("display", {})
        if not isinstance(display, dict):
        ...
        if isinstance(old_overrides, dict) and old_overrides:
            ...
            if not quiet:
                ...
                print(f"  âœ“ Migrated tool_progress_overrides â†’ display.platforms: {migrated}")
            results["config_added"].append("display.platforms (migrated from tool_progress_overrides)")

    # â”€â”€ Version 16 â†’ 17: remove legacy compression.summary_* keys â”€â”€
    if current_ver < 17:
        config = read_raw_config()
        ...
        if isinstance(comp, dict):
            ...
            if migrated_keys or s_model is not None or s_provider is not None or s_base_url is not None:
                ...
                if not quiet:
                    if migrated_keys:
                        print(f"  âœ“ Migrated compression.summary_* â†’ auxiliary.compression: {', '.join(migrated_keys)}")
                    else:
                        print("  âœ“ Removed unused compression.summary_* keys")

    # â”€â”€ Version 20 â†’ 21: plugins are now opt-in; grandfather existing user plugins â”€â”€
    ...
    #
    # Bundled plugins (shipped in the repo itself) are NOT grandfathered â€”
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ZED_HOME/plugins/`` for currently installed user plugins.
            ...
            if not quiet:
                if grandfathered:
                    print(
                        f"  âœ“ Plugins now opt-in: grandfathered "
                        f"{len(grandfathered)} existing plugin(s) into plugins.enabled"
                    )
                else:
                    print(
                        "  âœ“ Plugins now opt-in: no existing plugins to grandfather. "
                        "Use `zed plugins enable <name>` to activate."
                    )

    # â”€â”€ Version 22 â†’ 23: seed curator defaults + create logs/curator/ â”€â”€
    ...
    # settings in their `config.yaml`, and `zed curator status` has no
    ...
    #      â€” only keys the user hasn't already overridden.
    #   2. Writes the `auxiliary.curator` aux-task slot (provider, model,
    #      base_url, api_key, timeout, extra_body) â€” canonical slot for
    #      routing the curator fork to a cheaper aux model.
    #   3. Creates `~/.zed/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_zed_home() â€” old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_zed_home() / "logs" / "curator"
        ...
        # (1) Top-level curator section â€” only add missing keys
        ...
        if touched:
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        "  âœ“ Seeded curator defaults in config.yaml: "
            ...
            if added_aux:
                ...
                if not quiet:
                    print(
                        "  âœ“ Seeded auxiliary.curator defaults in config.yaml: "
                        f"{', '.join(added_aux)}"
                    )

    # â”€â”€ Version 24 â†’ 25: lower model_catalog TTL 24h â†’ 1h â”€â”€
    ...
    # stale window. Only rewrite the OLD default (24) â€” never clobber a
    ...
    if current_ver < 25:
        ...
        if isinstance(raw_mc, dict) and raw_mc.get("ttl_hours") == 24:
            ...
            save_config(config)
            results["config_added"].append("model_catalog.ttl_hours 24â†’1")
            if not quiet:
                print("  âœ“ Lowered model_catalog.ttl_hours to 1 (hourly picker refresh)")

    # â”€â”€ Version 28 â†’ 29: rename memory/skills write_mode â†’ write_approval â”€â”€
    ...
    # it maps to true; everything else (on/off/unset) â†’ false. The old
    # "off = block all writes" mode is dropped â€” memory_enabled: false disables
    ...
    if current_ver < 29:
        ...
        for subsystem in ("memory", "skills"):
            ...
            touched = True
            results["config_added"].append(
                f"{subsystem}.write_mode â†’ write_approval={sub['write_approval']}"
            )
        if touched:
            save_config(config)
            if not quiet:
                print("  âœ“ Renamed write_mode â†’ write_approval (boolean gate)")

    # â”€â”€ Version 29 â†’ 30: seed curator.consolidate (default false) â”€â”€
    ...
    # Only add the key when a curator section exists and lacks it â€” never
    ...
    if current_ver < 30:
        ...
        if isinstance(raw_curator, dict) and "consolidate" not in raw_curator:
            ...
            if not quiet:
                print(
                    "  âœ“ Seeded curator.consolidate: false "
                    "(LLM consolidation is now opt-in; pruning stays on)"
                )

    # â”€â”€ Post-migration: disable exfiltration-shaped MCP stdio entries â”€â”€
        ...
        if _validate_mcp_server_entry:
            ...
            for server_name, entry in raw_mcp_servers.items():
                ...
                if not quiet:
                    for issue in issues:
                        print(f"  âš  {issue}")
                    print(f"  âš  Disabled MCP server '{server_name}' pending review")
            if mcp_touched:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            for name, info in new_and_unset:
                print(f"    â€¢ {name} â€” {info.get('description', '')}")
            print()
            try:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: zed config set <key> <value>")
    ...
    # â”€â”€ Skill-declared config vars â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # Skills can declare config.yaml settings they need via
    # metadata.zed.config in their SKILL.md frontmatter.
    ...
    if missing_skill_config and interactive and not quiet:
        ...
        for var in missing_skill_config:
            ...
            print(f"    â€¢ {var['key']} â€” {var['description']} (from skill: {skill_name})")
        print()
        try:
        ...
        if answer in {"y", "yes"}:
            ...
            for var in missing_skill_config:
                ...
                if value:
                    ...
                    print(f"  âœ“ Saved {var['key']} = {value}")
                else:
                    results["warnings"].append(
                        f"Skipped {var['key']} â€” skill '{var.get('skill', '?')}' may ask for it later"
                    )
                print()
            save_config(config)
        else:
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ``model.*`` key is empty â€” they never override an existing value.
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.zed/config.yaml as-is, without merging defaults or migrating.
    ...
    Cached on the config file's (mtime_ns, size) â€” same strategy as
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.zed/config.yaml.
    ...
    (which change ``ZED_HOME`` and therefore ``get_config_path()``)
    don't collide.

    Read-only callers should use ``load_config_readonly()`` to skip the
    defensive deepcopy â€” that path matters in agent-loop hot spots like
...
def load_config_readonly() -> Dict[str, Any]:
    ...
    caller** â€” only use this when you are absolutely sure your code path
    ...
    safety guarantee is purely documented, not enforced â€” be careful.
    """
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/zed/config.yaml invalidates the
        ...
        # keys the managed layer pins â€” see docs/design/managed-scope.md Â§4.1.
        ...
        if cache_sig is not None:
            ...
            # calls will see â€” keeps "two readonly calls return the same
        ...
        # file missing), it's also fine â€” callers get an isolated object.
        return expanded
...
# Secret redaction is ON by default â€” strings that look like API keys,
...
_FALLBACK_COMMENT = """
# â”€â”€ Fallback Model â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
#   minimax      (MINIMAX_API_KEY)     â€” MiniMax
#   minimax-cn   (MINIMAX_CN_API_KEY)  â€” MiniMax (China)
#   bedrock      (AWS IAM / boto3)     â€” AWS Bedrock (Converse API)
#
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.zed/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.zed/.env.
    ...
    interactive menu render (`zed tools`, `zed setup`, status
    panels). Sanitisation is O(lines Ã— known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `zed tools`
...
# Editing .env bumps mtime â†’ next load_env() rebuilds. invalidate_env_cache()
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Zed env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.zed/.env in place.
...
def _check_non_ascii_credential(key: str, value: str) -> str:
    ...
    API keys and tokens must be pure ASCII â€” they are sent as HTTP header
    ...
    try:
        value.encode("ascii")
        return value  # all ASCII â€” nothing to do
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.zed/.env."""
    if is_managed():
        managed_error(f"set {key}")
        return
    # Managed scope guard: a managed env key can't be set by the user â€” the
    ...
    value = value.replace("\n", "").replace("\r", "")
    # API keys / tokens must be ASCII â€” strip non-ASCII with a warning.
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.zed/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.zed/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Zed â€” OPTIONAL_ENV_VARS and
    _EXTRA_ENV_KEYS â€” to avoid clobbering unrelated environment).
    """
    env_vars = load_env()
    known_keys = set(OPTIONAL_ENV_VARS.keys()) | _EXTRA_ENV_KEYS
    count = 0
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Zed vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.zed/.env or environment."""
...
def redact_key(key: str) -> str:
    ...
    Thin wrapper over :func:`agent.redact.mask_secret` â€” preserves the
...
def show_config():
    ...
    print(color("â”‚              âš• Zed Configuration                    â”‚", Colors.CYAN))
    ...
    if _managed_keys or _managed_env:
        ...
        print()
        print(color(
            f"  âš· Some settings are managed by your administrator ({_managed_dir}) "
            f"and cannot be changed",
            Colors.YELLOW,
            Colors.BOLD,
        ))
    ...
    from zed_cli.auth import get_anthropic_key
    ...
    # Warn on stale ZED_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("ZED_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                âš  .env has stale ZED_MAX_ITERATIONS={_env_ghost} "
                f"(run 'zed doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    # Display
    print()
    print(color("â—† Display", Colors.CYAN, Colors.BOLD))
    display = config.get('display', {})
    ...
    # Timezone
    print()
    print(color("â—† Timezone", Colors.CYAN, Colors.BOLD))
    ...
    if has_overrides:
        print()
        print(color("â—† Auxiliary Models (overrides)", Colors.CYAN, Colors.BOLD))
    ...
    try:
        ...
        if skill_vars:
            ...
            print()
            print(color("â—† Skill Settings", Colors.CYAN, Colors.BOLD))
    ...
    print(color("  zed config edit     # Edit config file", Colors.DIM))
    print(color("  zed config set <key> <value>", Colors.DIM))
    print(color("  zed setup           # Run setup wizard", Colors.DIM))
    print()
...
def edit_config():
    ...
    if not editor:
        # Try common editors â€” order is platform-aware so Windows users
...
def set_config_value(key: str, value: str):
    ...
    # the user â€” the next load would override it anyway. Hard-reject and name the
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: zed config set <key> <value>")
            print()
            print("Examples:")
            print("  zed config set model anthropic/claude-sonnet-4")
            print("  zed config set terminal.backend docker")
            print("  zed config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'zed config migrate' to add them")
        ...
        print("  zed config           Show current configuration")
        print("  zed config edit      Open config in editor")
        print("  zed config set <key> <value>   Set a config value")
        print("  zed config check     Check for missing/outdated config")
        print("  zed config migrate   Update config with new options")
        print("  zed config path      Show config file path")
        print("  zed config env-path  Show .env file path")
        sys.exit(1)


# â”€â”€ Profile-driven env var injection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _inject_profile_env_vars() -> None:
    ...
    Called once at module load time. Idempotent â€” repeated calls are no-ops.
    """
...
# â”€â”€ Platform-plugin env var injection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# ``_inject_profile_env_vars`` surfaces them in ``zed config`` UI so users
