"""Shell completion script generation for zed CLI.

Walks the live argparse parser tree to generate accurate, always-up-to-date
completion scripts â€” no hardcoded subcommand lists, no extra dependencies.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_zed_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Zed Agent bash completion
# Add to ~/.bashrc:
#   eval "$(zed completion bash)"

_zed_profiles() {{
    local profiles_dir="$HOME/.zed/profiles"
...
_zed_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_zed_profiles)" -- "$cur"))
        return
    fi
...
complete -F _zed_completion zed
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef zed
# Zed Agent zsh completion
# Add to ~/.zshrc:
...
_zed_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.zed/profiles" ]]; then
        profiles+=($HOME/.zed/profiles/*(N/:t))
    fi
...
_zed() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_zed_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Zed Agent fish completion",
        "# Add to your config:",
        "#   zed completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __zed_profiles",
        "    echo default",
        "    if test -d $HOME/.zed/profiles",
        "        for d in $HOME/.zed/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c zed -f -s p -l profile"
        " -d 'Profile name' -xa '(__zed_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__zed_profiles)' -d 'Profile name'"
                )
