#!/usr/bin/env python3
"""
Zed CLI - Main entry point.

Usage:
    zed                     # Interactive chat (default)
    zed chat                # Interactive chat
    zed gateway             # Run gateway in foreground
    zed gateway start       # Start gateway as service
    zed gateway stop        # Stop gateway service
    zed gateway status      # Show gateway status
    zed gateway install     # Install gateway service
    zed gateway uninstall   # Uninstall gateway service
    zed setup               # Interactive setup wizard
    zed logout              # Clear stored authentication
    zed status              # Show status of all components
    zed cron                # Manage cron jobs
    zed cron list           # List cron jobs
    zed cron status         # Check if cron scheduler is running
    zed doctor              # Check configuration and dependencies
    zed honcho setup                    # Configure Honcho AI memory integration
    zed honcho status                   # Show Honcho config and connection status
    zed honcho sessions                 # List directory â†’ session name mappings
    zed honcho map <name>               # Map current directory to a session name
    zed honcho peer                     # Show peer names and dialectic settings
    zed honcho peer --user NAME         # Set user peer name
    zed honcho peer --ai NAME           # Set AI peer name
    zed honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    zed honcho mode                     # Show current memory mode
    zed honcho mode [hybrid|honcho|local]  # Set memory mode
    zed honcho tokens                   # Show token budget settings
    zed honcho tokens --context N       # Set session.context() token cap
    zed honcho tokens --dialectic N     # Set dialectic result char cap
    zed honcho identity                 # Show AI peer identity representation
    zed honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    zed honcho migrate                  # Step-by-step migration guide: OpenClaw native â†’ Zed + Honcho
    zed version             Show version
    zed update              Update to latest version
    zed uninstall           Uninstall Zed Agent
    zed acp                 Run as an ACP server for editor integration
    zed sessions browse     Interactive session picker with search

    zed claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: zed_bootstrap must be the very first import â€” it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``zed_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``zed update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``zed_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, zed crashes on import and the user can't run
# ``zed update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _set_process_title() -> None:
    """Set the process title to 'zed' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic â€” non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep â€” installed via ``zed tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``zed.exe``).
    """
    # Strategy 1: setproctitle (best â€” works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("zed")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"zed", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"zed")
        # Windows: the .exe name is already ``zed.exe`` â€” nothing to do.
    except Exception:
        pass
...
# that run *before* zed_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("ZED_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".zed", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``ZED_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("ZED_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``ZED_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("ZED_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`zed --tui â€¦ >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from zed_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Zed Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``zed --version`` before config/logging imports on Termux."""
    if os.environ.get("ZED_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from zed_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
...
from zed_cli.subcommands.model import build_model_parser
from zed_cli.subcommands.setup import build_setup_parser
from zed_cli.subcommands.postinstall import build_postinstall_parser
from zed_cli.subcommands.whatsapp import build_whatsapp_parser
from zed_cli.subcommands.slack import build_slack_parser
from zed_cli.subcommands.login import build_login_parser
from zed_cli.subcommands.logout import build_logout_parser
from zed_cli.subcommands.auth import build_auth_parser
from zed_cli.subcommands.status import build_status_parser
from zed_cli.subcommands.webhook import build_webhook_parser
from zed_cli.subcommands.hooks import build_hooks_parser
from zed_cli.subcommands.doctor import build_doctor_parser
from zed_cli.subcommands.security import build_security_parser
from zed_cli.subcommands.dump import build_dump_parser
from zed_cli.subcommands.debug import build_debug_parser
from zed_cli.subcommands.backup import build_backup_parser
from zed_cli.subcommands.import_cmd import build_import_cmd_parser
from zed_cli.subcommands.config import build_config_parser
from zed_cli.subcommands.version import build_version_parser
from zed_cli.subcommands.update import build_update_parser
from zed_cli.subcommands.uninstall import build_uninstall_parser
from zed_cli.subcommands.dashboard import build_dashboard_parser
from zed_cli.subcommands.gui import build_gui_parser
from zed_cli.subcommands.logs import build_logs_parser
from zed_cli.subcommands.prompt_size import build_prompt_size_parser
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (zed tools, zed setup, zed model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'zed {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override â€” MUST happen before any zed module import.
#
# Many modules cache ZED_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ZED_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.zed/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ZED_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `zed mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Zed' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo zed -p <name>` against the invoking user's home.
        ...
        candidate = home / ".zed" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`zed chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors zed_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ZED_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.zed/profiles/coder or
    # /opt/data/profiles/coder).  If ZED_HOME points to the zed root
    # instead (e.g. systemd hardcodes ZED_HOME=/root/.zed), we must
    # still read active_profile â€” the user may have switched profiles via
    # `zed profile use` and the gateway should honour that choice.
    # See issue #22502.
    zed_home_env = os.environ.get("ZED_HOME", "")
    if profile_name is None and zed_home_env:
        if Path(zed_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the zed root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as ZED_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``zed gateway run`` to mean
    # "the root ZED_HOME profile". If the reserved default child read
    # active_profile here, switching the active profile (e.g. via the dashboard)
    # would silently redirect the default gateway into that profile â€” yielding a
    # duplicate gateway for the active profile and no real default gateway. See
    # the "Docker & Profiles & Dashboard" report.
    if profile_name is None and not os.environ.get("ZED_S6_SUPERVISED_CHILD"):
        try:
            from zed_constants import get_default_zed_root

            active_path = get_default_zed_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ZED_HOME
    if profile_name is not None:
        try:
            from zed_cli.profiles import resolve_profile_env

            zed_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            zed_home = _resolve_sudo_user_profile_env(profile_name)
            if not zed_home:
            ...
            # A bug in profiles.py must NEVER prevent zed from starting
            print(
...
load_zed_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml â†’ ZED_REDACT_SECRETS env
# var BEFORE zed_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins â€” this is config.yaml fallback only.
#
# We also read network.force_ipv4 from the same yaml load to avoid two
# separate config.yaml reads (saves ~17ms on every CLI startup â€” the second
# `load_config()` was doing a full deep-merge for one boolean lookup).
_FORCE_IPV4_EARLY = False
try:
    import yaml as _yaml_early

    _cfg_path = get_zed_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "ZED_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["ZED_REDACT_SECRETS"] = str(_early_redact).lower()
    ...
    pass  # best-effort â€” redaction stays at default (enabled) on config errors

# Initialize centralized file logging early â€” all `zed` subcommands
...
try:
    from zed_logging import setup_logging as _setup_logging
    ...
    pass  # best-effort â€” don't crash the CLI if logging setup fails

# Apply IPv4 preference early, before any HTTP clients are created.
# We already determined whether to force IPv4 from the raw yaml read above â€”
# this just calls the toggle without a redundant load_config() round trip.
if _FORCE_IPV4_EARLY:
    try:
        from zed_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort â€” don't crash if zed_constants not importable yet
...
from zed_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to zed_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (zed_cli.main._model_flow_*) keep resolving unchanged.
from zed_cli.model_setup_flows import (
...
def _read_git_revision_fingerprint(repo_root: Path) -> str | None:
    ...
    try:
        ...
        if head.startswith("ref:"):
            ...
            # Ref name is known but unresolved â€” still stable across launches,
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_zed_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("ZED_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("ZED_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from zed_cli.config import get_env_path, get_zed_home, load_config
    from zed_cli.auth import get_auth_status

    # Determine whether Zed itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_zed_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts â€” local models (vLLM, llama.cpp, etc.)
    ...
    # Check config.yaml â€” if model is a dict with an explicit provider set,
    ...
    # Only count these if Zed has been explicitly configured â€” Claude Code
    # being installed doesn't mean the user wants Zed to use their tokens.
    if _has_zed_config:
        try:
    ...
    try:
        ...
        def _curses_browse(stdscr):
            ...
            while True:
                ...
                # Header line
                if search_text:
                    header = f"  Browse sessions â€” filter: {search_text}â–ˆ"
                    header_attr = curses.A_BOLD
                    if curses.has_colors():
                        header_attr |= curses.color_pair(3)
                else:
                    header = "  Browse sessions â€” â†‘â†“ navigate  Enter select  Type to filter  Esc quit"
                    header_attr = curses.A_BOLD
                    if curses.has_colors():
                ...
                if key in {curses.KEY_UP,}:
                    ...
                    # Printable character â†’ add to search filter
                    search_text += chr(key)
                    filtered = [s for s in sessions if _match(s, search_text)]
                    cursor = 0
                    scroll_offset = 0
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, zed_bin
        cli_args: the original CLI arguments (everything after 'zed')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    zed_bin = container_info["zed_bin"]
    ...
    # users â€” Podman uses per-user namespaces, Docker needs group access.
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"sudo for {backend} â€” the -n (non-interactive) flag is required\n"
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo zed {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo zed {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    try:
        ...
        if message_count == 0:
            return  # No real conversation â€” don't show resume info
    ...
    print(f"  zed --tui --resume {target}")
    if title:
        print(f'  zed --tui -c "{title}"')
    print()
...
on dev-dependencies that are *also* declared as peers â€” the canonical
...
def _workspace_root(dir: Path) -> Path:
    ...
    ``npm install`` cwd stay consistent â€” a single helper prevents
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @zed/ink is missing or node_modules is behind package-lock.json.

    Prebuilt bundle mode: when ``dist/entry.js`` exists and there is no
    ``package-lock.json`` (nix install layout only ships ``dist/`` +
    ``package.json``), skip reinstall entirely â€” the bundle is self-contained
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    For each entry in the root lock's ``packages`` map:
      - missing from hidden lock â†’ reinstall (unless the entry is marked
      ...
      - present but with differing fields (excluding npm-written runtime
        annotations like ``ideallyInert``) â†’ reinstall

    Extra entries that exist only in the hidden lock are ignored â€” stale
    ...
    ink = ws_root / "node_modules" / "@zed" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/zed-ink/package.json",
    "packages/zed-ink/index.js",
    "packages/zed-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``ZED_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("ZED_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    new binaries in this Python process â€” regardless of which version manager
    was used (nvm, fnm, proto, brew, or the bundled fallback).

    Idempotent no-op when node+npm are already discoverable. Set
    ``ZED_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ZED_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    zed_home = os.environ.get("ZED_HOME") or str(Path.home() / ".zed")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ZED_HOME": zed_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(zed_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(zed_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if zed_cli_dir is None:
        zed_cli_dir = Path(__file__).parent
    bundled = zed_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev â†’ tsx src; else node dist (ZED_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ZED_NODE")
        ...
        if not path and bin == "node":
            try:
                ...
                pass
        if not path:
            print(f"{bin} not found â€” install Node.js to use the TUI.")
            sys.exit(1)
        return path

    # Footgun: --dev against a prebuilt bundle that has no source/node_modules.
    ext_dir = os.environ.get("ZED_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ZED_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ZED_TUI_DIR (e.g. `unset ZED_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if (
        ...
        npm = _node_bin("npm")
        if not os.environ.get("ZED_QUIET"):
            print("Installing TUI dependenciesâ€¦")
    ...
    if tui_dev:
        # Keep the local @zed/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @zed/ink resolves through
        # packages/zed-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "zed-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from zed_cli.oneshot import _normalize_toolsets
...
def _read_cgroup_memory_limit() -> Optional[int]:
    ...
    OOM-killer SIGKILLs Node before V8's own heap monitor ever fires â€” which
    ...
    for path in candidates:
        ...
        # 0x7FFFFFFFFFFFF000 â‰ˆ 9.2 EB, sometimes PAGE_COUNTER_MAX). Anything
        # at/above ~1 PB is effectively unconstrained â€” treat as no limit.
        if limit >= (1 << 50):
            return None
        return limit
    return None
...
def _resolve_tui_heap_mb(default_mb: int = 8192) -> int:
    ...
    clamped to a sane floor (1536MB â€” below this V8 GC-thrashes and the TUI
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from zed_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="zed-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["ZED_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["ZED_PYTHON_SRC_ROOT"] = os.environ.get(
        "ZED_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("ZED_PYTHON", sys.executable)
    env.setdefault("ZED_CWD", os.getcwd())
    ...
    if worktree:
        try:
            ...
            print(f"âœ— Failed to create TUI worktree: {exc}", file=sys.stderr)
            wt_info = None
        if not wt_info:
            sys.exit(1)
        env["ZED_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["ZED_MODEL"] = model
        env["ZED_INFERENCE_MODEL"] = model
    if provider:
        env["ZED_TUI_PROVIDER"] = provider
        env["ZED_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ZED_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ZED_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ZED_TUI_SKILLS"] = value
    if query:
        env["ZED_TUI_QUERY"] = query
    if image:
        env["ZED_TUI_IMAGE"] = image
    if checkpoints:
        env["ZED_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ZED_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ZED_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ZED_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ZED_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["ZED_ACCEPT_HOOKS"] = "1"
    # Guarantee a generous V8 heap for the TUI. Default node cap is ~1.5â€“4GB
    ...
    # SIGKILLs Node â€” running no JS handler, writing no breadcrumb, leaving the
    ...
    # ZED_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `zed --tui` try to
    ...
    # still set ZED_TUI_RESUME themselves.
    env.pop("ZED_TUI_RESUME", None)
    if resume_session_id:
        env["ZED_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `zed update` so
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ZED_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``zed kanban â€¦``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``zed kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("ZED_KANBAN_BOARD"):
        return
    try:
        from zed_cli.kanban_db import get_current_board

        os.environ["ZED_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.zed/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Zed â€” chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Zed still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      1. ``--cli`` flag         â†’ always classic REPL
      2. ``--tui`` flag / ``ZED_TUI=1`` â†’ always TUI
      3. ``display.interface`` config value ("cli" | "tui")
      4. default â†’ classic REPL
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("ZED_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            # -c "session name" â€” resolve by title or ID
            resolved = _resolve_session_by_name_or_id(continue_val)
            if resolved:
                args.resume = resolved
            else:
                print(f"No session found matching '{continue_val}'.")
                print("Use 'zed sessions list' to see available sessions.")
                sys.exit(1)
        else:
            # -c with no argument â€” continue the most recent session
    ...
    if resume_val:
        ...
        if resolved:
            args.resume = resolved
        # If resolution fails, keep the original value â€” _init_agent will
        # report "Session not found" with the original input

    # xAI retirement warning â€” one-shot, non-blocking, never fails startup
    try:
        ...
        from zed_cli.config import load_config as _load_config_for_xai_check

        _retired_xai_refs = find_retired_xai_refs(_load_config_for_xai_check())
        if _retired_xai_refs:
            sys.stderr.write(
                f"\033[33mâš  xAI retires {len(_retired_xai_refs)} model(s) "
                f"in your config on {RETIREMENT_DATE}:\033[0m\n"
            )
            for _ref in _retired_xai_refs:
                sys.stderr.write(f"  \033[33mâš \033[0m {format_issue(_ref)}\n")
            sys.stderr.write(f"  \033[2mMigration guide: {MIGRATION_GUIDE_URL}\033[0m\n")
            sys.stderr.write("  \033[2mRun 'zed doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Zed isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  zed setup")
        print()

        from zed_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'zed setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from zed_cli.banner import prefetch_update_check
    ...
    # setup (config, rules files, plugins, MCP servers) or from Zed itself.
    # Implemented as a superset of --ignore-user-config + --ignore-rules plus
    # plugin/MCP discovery suppression (ZED_SAFE_MODE is checked by
    # zed_cli/plugins.py and tools/mcp_tool.py).
    if getattr(args, "safe_mode", False):
        os.environ["ZED_SAFE_MODE"] = "1"
        os.environ["ZED_IGNORE_USER_CONFIG"] = "1"
        os.environ["ZED_IGNORE_RULES"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.zed/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded â€” this flag only
    ...
    if getattr(args, "source", None):
        os.environ["ZED_SESSION_SOURCE"] = args.source
...
def cmd_proxy(args):
    ...
    # Lazy import â€” pulls in aiohttp, which is gated behind an extras install
    # for users who don't run the proxy or the messaging gateway.
    from zed_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from zed_constants import find_node_executable, with_zed_node_path

    print()
    print("âš• WhatsApp Setup")
    print("=" * 50)

    # â”€â”€ Step 1: Choose mode â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    current_mode = get_env_value("WHATSAPP_MODE") or ""
    if not current_mode:
        print()
        print("How will you use WhatsApp with Zed?")
        print()
        print("  1. Separate bot number (recommended)")
        print("     People message the bot's number directly â€” cleanest experience.")
        print(
        ...
        if choice == "1":
            ...
            print("  â”‚                                                 â”‚")
            print("  â”‚  Easiest: Install WhatsApp Business (free app)  â”‚")
            print("  â”‚  on your phone with a second number:            â”‚")
            print("  â”‚    â€¢ Dual-SIM: use your 2nd SIM slot            â”‚")
            print("  â”‚    â€¢ Google Voice: free US number (voice.google) â”‚")
            print("  â”‚    â€¢ Prepaid SIM: $3-10, verify once            â”‚")
            print("  â”‚                                                 â”‚")
            print("  â”‚  WhatsApp Business runs alongside your personal â”‚")
            print("  â”‚  WhatsApp â€” no second phone needed.             â”‚")
    ...
    # has no creds.json.  Every subsequent `zed gateway` then paid a 30s
    # bridge-bootstrap timeout and queued WhatsApp for indefinite retries.
    # Now: aborted setup leaves WHATSAPP_ENABLED unset â†’ gateway skips it.
    ...
    if not (bridge_dir / "node_modules").exists():
        print(
            "\nâ†’ Installing WhatsApp bridge dependencies (this can take a few minutes)..."
        )
        npm = find_node_executable("npm")
        if not npm:
            print("  âœ— npm not found on PATH â€” install Node.js first")
            return
        try:
            result = subprocess.run(
    ...
    session_dir = get_zed_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: zed gateway")
            return

    # â”€â”€ Step 6: QR code pairing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    print()
    print("â”€" * 50)
    if wa_mode == "bot":
        print("ðŸ“± Open WhatsApp (or WhatsApp Business) on the")
        print("   phone with the BOT's number, then scan:")
    else:
        print("ðŸ“± Open WhatsApp on your phone, then scan:")
    print()
    print("   Settings â†’ Linked Devices â†’ Link a Device")
    print("â”€" * 50)
    print()
    ...
    # â”€â”€ Step 7: Post-pairing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    print()
    if (session_dir / "creds.json").exists():
        ...
        # and `zed gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  zed gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with 'âš• Zed Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  zed gateway")
            print("    2. Open WhatsApp â†’ Message Yourself")
            print("    3. Type a message â€” the agent will reply")
            print()
            print("  Tip: Agent responses are prefixed with 'âš• Zed Agent'")
            print("  so you can tell them apart from your own messages.")
        print()
        print("  Or install as a service: zed gateway install")
    else:
        print("âš  Pairing may not have completed. Run 'zed whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``zed whatsapp`` (the Baileys bridge wizard) â€” the
    two adapters are complementary, not alternatives. See
    ``zed_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from zed_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_postinstall(args):
    ...
    from zed_cli.config import stamp_install_method
    from zed_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("âš• Zed post-install bootstrap")
    print()
    ...
    if not _has_any_provider_configured():
        ...
        print()
        print("âœ“ Post-install complete.")


def cmd_model(args):
    """Select default model â€” starts with provider selection, then model picker."""
    _require_tty("model")
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``zed model``) and the setup wizard
    ...
    from zed_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("ZED_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'zed model' for "
                "available providers, or run 'zed doctor' to diagnose config "
                "issues."
            )
    ...
    #
    # Canonical providers are folded into top-level groups (display only â€” see
    # PROVIDER_GROUPS in zed_cli/models.py). A multi-member group shows one
    # row ("Kimi / Moonshot â–¸"); picking it opens a member sub-picker that
    ...
    #   members == [] â†’ leaf row, key is a provider slug / action
    #   members != [] â†’ group row, key is "group:<gid>"
    ...
    for row in grouped_rows:
        if row["kind"] == "group":
            ...
            label = f"{row['label']} â–¸ ({group_desc})" if group_desc else f"{row['label']} â–¸"
            key = f"group:{gid}"
            ...
            members = []
        if is_active:
            ordered.append((key, f"{label}  â† currently active", members))
    ...
    for key, provider_info in _custom_provider_map.items():
        ...
        model_hint = f" â€” {saved_model}" if saved_model else ""
        label = f"{name} ({short_url}){model_hint}"
        if active and key == active:
            ordered.append((key, f"{label}  â† currently active", []))
            default_idx = len(ordered) - 1
        else:
    ...
    # Group row â†’ drill into a member sub-picker. Default to the active member
    ...
    if selected_members:
        ...
        group_label = ordered[provider_idx][1].split(" â–¸", 1)[0]
    ...
    # â”€â”€ Post-switch cleanup: clear stale OPENAI_BASE_URL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # When the user switches to a named provider (anything except "custom"),
    # a leftover OPENAI_BASE_URL in ~/.zed/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.zed/.env if the active provider is not 'custom'.
    ...
    """
    from zed_cli.config import get_env_value, save_env_value, load_config
...
#
# Zed uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `zed model` provider picker. It does NOT re-run credential setup â€” it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `zed model` flow first.
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`zed_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from zed_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    Only writes the four routing fields â€” timeout, download_timeout, and any
    ...
    for task, _name, _desc in _all_aux_tasks():
        ...
        for field in ("model", "base_url", "api_key"):
            if entry.get(field):
                entry[field] = ""
                changed = True
        # Preserve timeout/download_timeout â€” those are user-tuned, not routing
        if changed:
            count += 1
    save_config(cfg)
    return count
...
def _aux_config_menu() -> None:
    """Top-level auxiliary-model picker â€” choose a task to configure.
    ...
    while True:
        ...
        print()
        print("  Auxiliary models â€” side-task routing")
        print()
...
def _aux_select_for_task(task: str) -> None:
    ...
    inside the aux picker â€” users set up new providers through the normal
    ``zed model`` flow, then route aux tasks to them here.
    """
    from zed_cli.config import load_config
    ...
    # "auto" always first
    auto_marker = (
        "  â† current" if current_provider == "auto" and not current_base_url else ""
    )
    ...
    for p in providers:
        ...
        model_hint = f" â€” {total} models" if total else ""
        marker = (
            "  â† current" if slug == current_provider and not current_base_url else ""
        )
        entries.append((slug, f"{name}{model_hint}{marker}", list(models)))

    # Custom endpoint (raw base_url)
    custom_marker = "  â† current" if current_base_url else ""
    entries.append(("__custom__", f"Custom endpoint (direct URL){custom_marker}", []))
    entries.append(("__back__", "Back", []))

    print()
    print(f"  Configure {display_name} â€” current: {_format_aux_current(task_cfg)}")
    print()
    ...
    # Regular provider â€” pick a model from its curated list
...
def _aux_flow_provider_model(
    ...
    from zed_cli.auth import _prompt_model_selection
    from zed_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from zed_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Zed URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _save_custom_provider(
    ...
    Deduplicates by base_url â€” if the URL already exists, updates the
    ...
    # Check if this URL is already saved â€” update model/context_length if so
    ...
    cfg["custom_providers"] = providers
    save_config(cfg)
    print(f'  ðŸ’¾ Saved to custom providers as "{name}" (edit in config.yaml)')
...
def _remove_custom_provider(config):
    ...
    try:
        from zed_cli.curses_ui import curses_radiolist
    ...
    print(f'âœ… Removed "{removed_name}" from custom providers.')




# Lazy-export the model catalog at module level. Tests and a handful of
# downstream call sites read `zed_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `zed` invocation â€” including
# fast paths like `zed --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from zed_cli.models import _PROVIDER_MODELS
...
def _prompt_reasoning_effort_selection(efforts, current_effort=""):
    ...
    def _label(effort):
        if effort == current_effort:
            return f"{effort}  â† currently in use"
        return effort
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``zed setup`` / ``zed model``.
    ...
    recover from a malformed paste without editing ``~/.zed/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately â€” the user cancelled entry, declined to replace, or
    ...
    # First-time entry â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if not existing_key:
    ...
    # Already configured â€” offer K / R / C â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    from zed_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `zed setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from zed_constants import display_zed_home as _dhh_fn

            print(
                f"    Zed will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
    ...
    try:
        print()
        print("  Running 'claude setup-token' â€” follow the prompts below.")
        ...
        # Claude CLI not installed â€” guide user through manual setup
        print()
...
def cmd_login(args):
    """Authenticate Zed CLI with a provider."""
    from zed_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from zed_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from zed_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from zed_cli.status import show_status
...
def cmd_webhook(args):
    ...
    from zed_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``zed slack <subcommand>``. Currently supports:
      manifest â€” print or write a Slack app manifest with every gateway
                 command registered as a first-class slash.
    """
    sub = getattr(args, "slack_command", None)
    if sub in {None, ""}:
        # No subcommand â€” print usage hint.
        print(
            "usage: zed slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `zed slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from zed_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from zed_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from zed_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from zed_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `zed security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from zed_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from zed_cli.dump import run_dump
...
def cmd_config(args):
    ...
    from zed_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Zed home directory to a zip file."""
    if getattr(args, "quick", False):
        from zed_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from zed_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Zed backup from a zip file."""
    from zed_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    # ``import openai`` â€” the SDK drags in ~800ms of pydantic-backed type
    ...
    # Show update status (synchronous â€” acceptable since user asked for version info)
    try:
        from zed_cli.banner import check_for_updates
        from zed_cli.config import recommended_update_command

        behind = check_for_updates()
        if behind and behind > 0:
            commits_word = "commit" if behind == 1 else "commits"
            print(
                f"Update available: {behind} {commits_word} behind â€” "
...
def cmd_uninstall(args):
    """Uninstall Zed Agent (or just the Chat GUI with --gui)."""
    # Machine-readable install snapshot for the desktop app's uninstall UI.
    # Must run before any TTY gate â€” it's called from a non-interactive child.
    if getattr(args, "gui_summary", False):
        from zed_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from zed_cli.uninstall import run_gui_uninstall
    ...
    from zed_cli.uninstall import run_uninstall
...
# Critical files that every ``zed`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked â€” the user can't
# even run ``zed update`` again to roll forward. The post-pull syntax
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``zed`` startup; if any of them
    ...
    Python version. The pyc is discarded on function return either way â€”
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="zed-syntax-check-") as tmpdir:
        for relpath in _UPDATE_CRITICAL_FILES:
            path = root / relpath
            if not path.exists():
                # Missing file is suspicious but not necessarily fatal â€” a future
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``zed update --gateway`` so interactive prompts (stash restore,
    ...
    # Timeout â€” clean up and use default
    prompt_path.unlink(missing_ok=True)
    response_path.unlink(missing_ok=True)
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``zed_cli/web_dist/`` (per vite.config.ts
    outDir: "../zed_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "zed_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    reboot â€” leaving the editable install in a half-state with the
    ``zed`` launcher present but ``zed_cli`` not importable.
    ...
    stderr, and an integer returncode. Never raises on idle timeout â€”
    ...
    def _reader() -> None:
        ...
        for line in proc.stdout:
            try:
                ...
                # Windows cp1252 fallback â€” same pattern as _say().
    ...
    if idle_killed:
        msg = (
            f"\n  âš  Build produced no output for {idle_timeout_seconds}s â€” terminated.\n"
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    does a bare ``PATH`` lookup â€” which fails on NixOS.

    Two-tier resolution:
    1. Fast path â€” the zed venv's python3 (present in managed installs)
    2. Fallback â€” resolves the absolute python3 path via ``nix-shell``
    ...
    # python3 already on PATH â€” nothing to do
    if shutil.which("python3"):
        return None

    # Tier 1: fast path â€” zed venv python3, no nix-shell overhead
    for venv_name in ("venv", ".venv"):
        venv_python = PROJECT_ROOT / venv_name / "bin" / "python3"
        if venv_python.exists():
            return {**os.environ, "PYTHON": str(venv_python)}

    # Tier 2: nix-shell fallback â€” resolves the absolute python3 path once.
    # Slower (~2â€“5 s for the nix-shell eval) but always works, even without
    # a zed venv (pip / non-managed / bare-git installs).  The resolved
    ...
    try:
        ...
        pass  # nix-shell not available â€” caller will get None
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    sync on a WIP checkout).  Without this, ``npm install`` on npm â‰¥ 10 silently
    rewrites committed lockfiles (stripping ``"peer": true`` etc.), which leaves
    the working tree dirty and causes the next ``zed update`` to stash the
    lockfile â€” repeatedly.
    """
    # unicode-animations' postinstall animates to /dev/tty (bypasses
    # --silent/capture_output). It no-ops when CI is set â€” same as the TUI
    ...
    if lockfile.exists():
        ...
        if ci_result.returncode == 0:
            return ci_result
        # Fall through to `npm install` â€” lockfile may be out of sync on a
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``zed web``).
    ...
    # build path usable on a stock `py -m zed_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_zed_node_path()
    _say("â†’ Building web UI...")
    ...
    if r1.returncode != 0:
        _say(
            f"  {'âœ—' if fatal else 'âš '} Web UI npm install failed"
            + ("" if fatal else " (zed web will not be available)")
        )
        _relay(r1)
        if fatal:
            _say("  Run manually:  npm install --workspace web && npm run build -w web")
        return False
    # First attempt â€” stream output via idle-timeout helper (issue #33788).
    ...
    if r2.returncode != 0:
        # Retry once after a short delay â€” covers boot-time races on Windows
    ...
    if r2.returncode != 0:
        ...
        # (Windows Scheduled Tasks, CI) â€” issue #23817.
        if dist_index.exists():
            _say("  âš  Web UI build failed â€” serving stale dist as fallback")
            if stderr_tail:
                _say(f"  Build error:\n  {stderr_tail}")
            return True

        _say(
            f"  {'âœ—' if fatal else 'âš '} Web UI build failed"
            + ("" if fatal else " (zed web will not be available)")
        )
        _relay(r2)
        if fatal:
...
# ---------------------------------------------------------------------------
# Desktop build stamp â€” content-hash based skip logic
# ---------------------------------------------------------------------------
...
#   - ``zed update`` can unconditionally call ``zed desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``zed desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $ZED_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _compute_desktop_content_hash(project_root: Path) -> str:
    ...
    # Walk apps/desktop/ â€” prune ignored directories in-place
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $ZED_HOME."""
    from zed_constants import get_zed_home
    return get_zed_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``zed update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Zed.app/Contents/MacOS/Zed"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Zed.exe",
            release_dir / "win-ia32-unpacked" / "Zed.exe",
            release_dir / "win-arm64-unpacked" / "Zed.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "zed",
            release_dir / "linux-unpacked" / "Zed",
            release_dir / "linux-arm64-unpacked" / "zed",
            release_dir / "linux-arm64-unpacked" / "Zed",
        ]
...
def _electron_download_cache_dirs() -> list[Path]:
    ...
    corrupt zip here â€” not a bad workspace install â€” is what poisons the build.
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    Root cause of the ``ENOENT â€¦ rename 'â€¦/linux-unpacked/electron' ->
    'â€¦/linux-unpacked/Zed'`` desktop build failure: a corrupt zip in the
    ...
    most common corruption here â€” it reads from the end-of-central-directory
    ...
    them and decide whether a retry is worthwhile (empty list â‡’ nothing to
...
def _electron_pkg_staged_missing_dist(project_root: Path) -> bool:
    """electron staged (package.json + install.js) but dist missing â€” blocked postinstall."""
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Zed.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Zed.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove â€¦\\Zed.exe: Access is
    ...
    this desktop's ``release`` tree are stopped â€” a packaged install elsewhere or
    an unrelated "Zed" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Zed is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Zed.app/Contents/MacOS/Zed  ->  app bundle = .../Zed.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"âœ— Zed Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False

    # Reject symlinks â€” chown/chmod must not follow an attacker-controlled
    ...
    try:
        ...
        print(f"âœ— Cannot stat Electron's Linux sandbox helper: {sandbox}")
        return False
    if not stat.S_ISREG(sandbox_lstat.st_mode):
        print(f"âœ— Electron's Linux sandbox helper is not a regular file: {sandbox}")
        return False
    ...
    if not sudo:
        print("âœ— Zed Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False

    print("â†’ Configuring Electron Linux sandbox helper (sudo required)...")
    for command in ([sudo, "chown", "root:root", str(sandbox)], [sudo, "chmod", "4755", str(sandbox)]):
        if subprocess.run(command, check=False).returncode != 0:
            print(f"âœ— Failed to configure Electron's Linux sandbox helper: {sandbox}")
            return False
    return True
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from zed_logging import setup_logging as _setup_logging_gui
    ...
    # with_zed_node_path() copies os.environ when called with no arg.
    env = with_zed_node_path()
    if getattr(args, "fake_boot", False):
        env["ZED_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["ZED_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "zed_root", None):
        env["ZED_DESKTOP_ZED_ROOT"] = str(Path(args.zed_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["ZED_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  zed gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        if source_mode:
            if not _desktop_dist_exists(desktop_dir):
                print(f"âœ— --skip-build --source was passed but no desktop dist found at: {desktop_dir / 'dist'}")
            ...
            if not (_electron_dir(PROJECT_ROOT) / "package.json").exists():
                print("âœ— --skip-build --source requires existing desktop workspace dependencies.")
                print(f"  Install first:  cd {PROJECT_ROOT} && npm ci")
                print("  Or drop --skip-build to install dependencies and build automatically.")
                sys.exit(1)
            print(f"â†’ Skipping desktop source build (--skip-build --source); using dist at {desktop_dir / 'dist'}")
        elif packaged_executable is None:
            print(f"âœ— --skip-build was passed but no packaged desktop app was found at: {desktop_dir / 'release'}")
            print("  Pre-build first:  cd apps/desktop && npm run pack")
            print("  Or drop --skip-build to package automatically.")
            sys.exit(1)
        else:
            print(f"â†’ Skipping desktop package build (--skip-build); using {packaged_executable}")
    else:
        ...
        if not build_needed:
            ...
            if install_result.returncode != 0:
                if not _electron_pkg_staged_missing_dist(PROJECT_ROOT):
                    print("âœ— Desktop dependency install failed")
                    print(f"  Run manually:  cd {PROJECT_ROOT} && npm ci")
                    sys.exit(install_result.returncode or 1)
                repaired = _try_redownload_electron_dist(PROJECT_ROOT, env)
                if repaired:
                    print("  âš  Dependency install failed with a missing Electron dist; "
            ...
            build_label = "source build" if source_mode else "packaged app"
            print(f"â†’ Building desktop {build_label}...")
            build_script = "build" if source_mode else "pack"
            if not source_mode:
                # A running desktop instance launched from release/win-unpacked
                # holds Zed.exe locked on Windows, so the pack can't replace
                # it ("Access is denied" / ERR_ELECTRON_BUILDER_CANNOT_EXECUTE).
                # Stop it first so the rebuild â€” including the installer's
                # headless --update rebuild â€” succeeds instead of failing cryptically.
                stopped = _stop_desktop_processes_locking_build(desktop_dir)
                if stopped:
                    print(f"  âš  Stopped running desktop app to free the build output (pid {', '.join(map(str, stopped))})")
            build_result = subprocess.run([npm, "run", build_script], cwd=desktop_dir, env=env, check=False)
            if build_result.returncode != 0 and not source_mode:
                # Corrupt cached Electron zip â†’ partial unpack â†’ ENOENT on rename.
                ...
                if restored:
                    print("  âš  Desktop build failed; refreshed the Electron download and retrying once...")
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Zed.exe
                    # is still locked by a running instance; stop it before retry.
                    _stop_desktop_processes_locking_build(desktop_dir)
                    build_result = subprocess.run([npm, "run", build_script], cwd=desktop_dir, env=env, check=False)
            if build_result.returncode != 0 and not source_mode and not env.get("ELECTRON_MIRROR"):
                print("  âš  Desktop build still failing; the Electron download from "
            ...
            if build_result.returncode != 0:
                print("âœ— Desktop GUI build failed")
                print(f"  Run manually:  cd apps/desktop && npm run {build_script}")
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Zed.exe, close any")
                    print("  running Zed desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> zed desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Zed is
                # damaged"). No-op on non-macOS and on real-identity builds.
                _desktop_macos_relaunchable_fixup(desktop_dir)

            # Build succeeded â€” write the stamp so next run can skip
    ...
    # happen here â€” it would block the installer and, on Windows, the old exe
    ...
    if source_mode:
        print("â†’ Launching Zed Desktop from source build...")
        launch_result = subprocess.run([npm, "exec", "--", "electron", "."], cwd=desktop_dir, env=env, check=False)
        sys.exit(launch_result.returncode)

    if packaged_executable is None:
        print(f"âœ— Desktop package build completed but no launchable app was found at: {desktop_dir / 'release'}")
        print("  Expected an unpacked Electron app for the current OS.")
        sys.exit(1)
    ...
    print(f"â†’ Launching packaged Zed Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``zed dashboard`` processes other than ourselves.

    ``zed dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``zed update`` replaces files on disk, the running
    process keeps the old Python backend in memory while the JS bundle on
    disk is updated, causing a silent frontend/backend mismatch (e.g. new
    auth headers the old backend doesn't recognise â†’ every API call 401s).

    The dashboard has no service manager (systemd / launchd), no PID file,
    and we can't know the original launch args â€” so the only sane action
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Zed Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``zed dashboard`` as
    ...
    environment variable ``ZED_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "zed dashboard",
        "zed_cli.main dashboard",
        "zed_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "zed.*dashboard"`) keeps us consistent
            # with `zed_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `zed update`.
    ...
    if state.get("last_run_at"):
        # Curator has run before (real or already seeded) â€” no notice needed.
        return
    try:
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``zed update`` is a high-attention surface â€” surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``zed update`` invocations skip the block until a newer
    ...
    if not last_run_at:
        return  # no curator run yet â€” first-run notice handles this case
    ...
    # Only print when there's something interesting to show â€” i.e. the
    ...
    print()
    print(f"â„¹ Skill curator â€” last run {when}")
    for line in summary.splitlines():
        print(f"  {line}")
    print(
        "  (This message shows once per curator run. "
        "View anytime: zed curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``zed dashboard`` processes.

    Called at the end of ``zed update`` (default ``reason``) and also
    from ``zed dashboard --stop`` (which overrides ``reason``).  The
    ...
    recognise â†’ every API call 401s).
    ...
    """
    # When the Zed Desktop Electron app spawns this dashboard as a
    # backend child, it sets ZED_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("ZED_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    print()
    print(f"âŸ² Stopping {len(pids)} dashboard process(es) ({reason})")
    ...
    if sys.platform == "win32":
        ...
        # SIGTERM first â€” give each process a chance to shut down cleanly
        # (uvicorn closes its socket, flushes logs, etc.).
        for pid in pids:
            try:
                os.kill(pid, _signal.SIGTERM)
            except ProcessLookupError:
                # Already gone â€” count as killed.
                killed.append(pid)
...
def _update_via_zip(args):
    """Update Zed Agent by downloading a ZIP archive.
    ...
    # if the user asked for something else â€” exactly the silent-divergence
    # bug --branch was added to prevent. Refuse to proceed in that case
    # rather than lie.
    branch = _resolve_update_branch(args)
    if branch != "main":
        print(
            f"âœ— --branch={branch} is not supported on the Windows ZIP-fallback "
            "update path."
        )
        print(
            "  This path runs when git file I/O is broken on the system. "
            "Either resolve the git-side breakage (typically an antivirus "
            "or NTFS filter holding files open) and rerun `zed update "
            f"--branch {branch}`, or update against main with `zed update`."
        )
        sys.exit(1)
    zip_url = (
    ...
    tmp_dir = tempfile.mkdtemp(prefix="zed-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"zed-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for zed-agent itself
            # should never contain symlinks â€” they'd point outside the
        ...
        # GitHub ZIPs extract to zed-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"zed-agent-{branch}")
    ...
    from zed_cli.managed_uv import ensure_uv, update_managed_uv

    # Keep managed uv current â€” runs `uv self update` if we already have one.
    update_managed_uv()
    ...
    try:
        ...
        print("â†’ Syncing bundled skills...")
        result = sync_skills(quiet=True)
        if result["copied"]:
            print(f"  + {len(result['copied'])} new: {', '.join(result['copied'])}")
        if result.get("updated"):
            print(
                f"  â†‘ {len(result['updated'])} updated: {', '.join(result['updated'])}"
            )
        if result.get("user_modified"):
            print(f"  ~ {len(result['user_modified'])} user-modified (kept)")
            print(
                "    â†’ see them: zed skills list-modified  "
    ...
    try:
        from zed_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    if unmerged.stdout.strip():
        print("â†’ Clearing unmerged index entries from a previous conflict...")
        subprocess.run(git_cmd + ["reset"], cwd=cwd, capture_output=True)

    from datetime import datetime, timezone

    stash_name = datetime.now(timezone.utc).strftime(
        "zed-update-autostash-%Y%m%d-%H%M%S"
    )
    print("â†’ Local changes detected â€” stashing before update...")
    subprocess.run(
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        print()
        print("âš  Local changes were stashed before updating.")
        print(
            "  Restoring them may reapply local customizations onto the updated codebase."
        )
        print("  Review the result afterward if Zed behaves unexpectedly.")
    ...
    print("â†’ Restoring local changes...")
    ...
    # Check for unmerged (conflicted) files â€” can happen even when returncode is 0
    unmerged = subprocess.run(
    ...
    if restore.returncode != 0 or has_conflicts:
        print("âœ— Update pulled new code, but restoring local changes hit conflicts.")
        ...
        print("\nYour stashed changes are preserved â€” nothing is lost.")
        print(f"  Stash ref: {stash_ref}")

        # Always reset to clean state â€” leaving conflict markers in source
        # files makes zed completely unrunnable (SyntaxError on import).
        ...
        # Don't sys.exit â€” the code update itself succeeded, only the stash
    ...
    if stash_selector is None:
        print(
            "âš  Local changes were restored, but Zed couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "âš  Local changes were restored, but Zed couldn't drop the saved stash entry."
            )
    ...
    print("âš  Local changes were restored on top of the updated codebase.")
    print("  Review `git diff` / `git status` if Zed behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    ``updates.non_interactive_local_changes: discard`` â€” i.e. they've opted out
    ...
    explicitly captured) â€” ignored paths like node_modules/venv/build outputs
    ...
    if stash_selector is None:
        print(
            "âš  Configured to discard local changes on non-interactive update, "
            "but Zed couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "âš  Configured to discard local changes, but Zed couldn't drop "
    ...
    print("â†’ Discarded local source changes (updates.non_interactive_local_changes=discard).")
    return True


# =========================================================================
# Fork detection and upstream management for `zed update`
# =========================================================================
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_zed_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_zed_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                ...
                print("  âœ— Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print(
    ...
    # origin/main, so there's no reason to pull every upstream ref â€” and a bare
    # fetch drags in thousands of auto-generated branches.
    print()
    print("â†’ Fetching upstream...")
    try:
        subprocess.run(
        ...
        print("  âœ— Failed to fetch upstream. Skipping upstream sync.")
        return
    ...
    if origin_ahead < 0 or upstream_ahead < 0:
        print("  âœ— Could not compare branches. Skipping upstream sync.")
        return

    # If origin/main has commits not on upstream, don't trample
    if origin_ahead > 0:
        print()
        print(f"â„¹ Your fork has {origin_ahead} commit(s) not on upstream.")
    ...
    print()
    print(f"â†’ Fork is {upstream_ahead} commit(s) behind upstream")
    print("â†’ Pulling from upstream...")
    ...
    try:
        ...
        print(
            "  âœ— Failed to pull from upstream. You may need to resolve conflicts manually."
        )
        return
...
def _invalidate_update_cache():
    ...
    The git repo is shared across profiles â€” when one profile runs
    ``zed update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware â€” uses /opt/data in Docker)
    from zed_constants import get_default_zed_root

    default_home = get_default_zed_root()
...
# Install-scoped breadcrumb dropped right before ``zed update`` mutates the
...
# survives and the next ``zed`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $ZED_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``zed update`` left half-done.

    Triggered on launch when ``.update-incomplete`` is present â€” meaning the
    ...
    Output: everything â€” our status lines AND the streamed pip/uv install
    (which inherits fd 1) â€” is routed to stderr.  Launches whose stdout is a
    protocol stream (``zed acp`` speaks JSON-RPC on stdout) must never get
    ...
    # process holds it, skip â€” it is running the same reinstall into the same
    ...
    try:
        ...
        # Couldn't create the lock (read-only fs, perms). Proceed unlocked â€”
    ...
    try:
        ...
        print(
            "âš  A previous `zed update` was interrupted mid-install â€” "
            "finishing dependency installation now..."
        )

        try:
            from zed_cli.managed_uv import ensure_uv
            ...
            print("âœ— Could not auto-recover the interrupted install.")
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``zed update`` is still progressing even if pip/uv itself is silent.
    """
    ...
    def _heartbeat() -> None:
        ...
        while not done.wait(heartbeat_interval_seconds):
            ...
            print(
                f"  â€¦ still installing dependencies ({elapsed}s elapsed)"
                " â€” compiling Rust/C extensions can take several minutes",
                flush=True,
            )
...
def _zed_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    they're regular Python scripts which can be replaced atomically â€” no
    ...
    return [
        scripts_dir / "zed.exe",
        scripts_dir / "zed-gateway.exe",
    ]


def _detect_concurrent_zed_instances(
    scripts_dir: Path, *, exclude_pid: int | None = None
) -> list[tuple[int, str]]:
    """Find other live processes whose .exe is one of our entry-point shims.

    Windows blocks DELETE/REPLACE on a running .exe â€” and even RENAME on the
    same .exe when another process opened it without ``FILE_SHARE_DELETE``.
    The Zed Desktop Electron app spawns ``zed.EXE`` as a backend child,
    so during ``zed update`` the user-invoked process and the desktop's
    ...
    for shim in _zed_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated zed.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``zed update``
    # reports its own launcher as a concurrent instance â€” a false positive
    # (issues #29341, #34795).
    #
    # Two robustness points learned from the field:
    #   1. Use ``proc.parents()`` â€” it returns the WHOLE ancestor list in one
    ...
    #      zed.exe sitting *under* a non-Zed parent (e.g. a Zed
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "zed.exe"
    lines = ["âœ— Another zed.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Zed Desktop, exit any open `zed` REPLs, and")
    lines.append("  stop the gateway (`zed gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `zed update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_zed_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``zed.exe``.
    ...
    when ``zed update`` runs, ``zed.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``zed.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next zed invocation by ``_cleanup_quarantined_exes``.

    Rename can still fail when *another* process has opened the .exe without
    ``FILE_SHARE_DELETE`` â€” typically AV real-time scanners with transient
    handles (recovers in <1s), or the Zed Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Zed Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    where we used ``MOVEFILE_DELAY_UNTIL_REBOOT`` are NOT returned â€” they
    ...
    for shim in _zed_exe_shims(scripts_dir):
        ...
        if scheduled:
            print(
                f"  âš  {shim.name} is locked by another process; scheduled "
        ...
        # uv try its luck â€” sometimes uv's own retry handling pulls through.
        print(
            f"  âš  Could not quarantine {shim.name} ({last_exc.__class__.__name__}: "
            f"another process is holding it open)."
        )
        print(
            "    Close Zed Desktop, exit other `zed` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `zed update`."
        )
...
def _schedule_replace_on_reboot(shim: Path, quarantine_target: Path) -> bool:
    ...
    runs on next boot â€” at which point no process can hold the .exe.
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_zed_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``zed.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``zed.exe`` is the running process â€”
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``zed`` drops off PATH. This wraps
    ...
    install that touches the shims is protected â€” including the
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_zed_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``zed.exe.old.*`` left by prior updates.

    Called early on every zed invocation. The .old files are unlocked once
    ...
    try:
        for stale in scripts_dir.glob("*.exe.old.*"):
            try:
                ...
                pass  # still locked or in use â€” try again next run
    except OSError:
        pass
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``zed update`` runs ``uv pip install -e .[all]`` which
    leaves those packages untouched â€” so if we bump a pin in
    ...
    user never enabled stay quiet â€” no churn for cold backends.
    ...
    print()
    print(f"â†’ Refreshing {len(active)} active lazy backend(s)...")
    ...
    try:
        ...
        print(f"  âš  Lazy refresh failed unexpectedly: {exc}")
        return
    ...
    if failed:
        for feature, status in failed:
            ...
            if len(reason) > 200:
                reason = reason[:200] + "..."
            print(f"  âš  {feature} failed to refresh: {reason}")
        print("  Backends keep their previously-installed version; rerun")
        print("  `zed update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``zed.exe`` / ``zed-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_zed_exe`` for the rationale.
    """
    ...
    try:
        ...
        print(
            "  âš  Optional extras failed, reinstalling base dependencies and retrying extras individually..."
        )
    ...
    if failed_extras:
        print(
            f"  âš  Skipped optional extras that still failed: {', '.join(failed_extras)}"
        )
    ...
    # target venv. uv's incremental resolver has â€” in the wild â€” produced
    ...
    # hours later inside ``zed update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    update â€” but the warning makes the partial install visible at the spot
    that caused it, instead of hours later in a downstream subprocess.
    """
    try:
        import tomllib  # Python 3.11+
    except ImportError:  # pragma: no cover â€” Python < 3.11 unsupported but be safe
        return
    ...
    # Run the check inside the venv Python â€” sys.executable here may be the
    # outer Python that drove ``zed update``, not the venv we just wrote
    ...
    print(
        f"  âš  Verification: {len(missing)} declared dep(s) missing after install: "
        f"{', '.join(missing[:8])}{'...' if len(missing) > 8 else ''}"
    )
    print("  â†’ Reinstalling base group with --reinstall to repair...")
    ...
    # purpose â€” the missing dep is in *base* deps; rerunning the full all-
    ...
    #
    # Quarantine the running ``zed.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``zed`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  âš  Repair install failed; check `zed update` output above.")
        return
    ...
    print(
        f"  â†’ Force-installing remaining missing dep(s): {', '.join(specs)}"
    )
    try:
        ...
        print(
            f"  âš  Could not install: {', '.join(still_missing)}. "
            "Run `zed update --force` after closing other zed processes."
        )
        return
    ...
    if final_missing:
        print(
            f"  âš  Still missing after repair: {', '.join(final_missing)}. "
            "Run `zed update --force` after closing other zed processes."
        )
    else:
        print("  âœ“ All declared core dependencies now installed")
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from zed_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    if not _is_termux_env():
        return None
    try:
        print("  â†’ Termux detected: trying to install uv for faster dependency updates...")
...
def _update_node_dependencies() -> None:
    ...
    # workspaces â€” but apps/desktop pulls in Electron as a devDependency,
    # and its postinstall downloads a ~200MB binary.  Most users don't
    # need desktop during `zed update`, so we install root-only first
    ...
    nixos_env = with_zed_node_path(_nixos_build_env())
    ...
    if root_result.returncode != 0:
        print("  âš  npm install failed in repo root")
...
class _UpdateOutputStream:
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    def write(self, data):
        # Mirror to the log file first â€” it's the most reliable destination.
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``zed update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.zed/logs/update.log`` and to silently absorb
       ``BrokenPipeError`` when the terminal vanishes.

    ``SIGINT`` (Ctrl-C) and ``SIGTERM`` (systemd shutdown) are
    **intentionally left alone** â€” those are legitimate cancellation
    signals the user or OS sent on purpose.

    In gateway mode (``zed update --gateway``) the update is already
    ...
    if hasattr(_signal, "SIGHUP"):
        try:
            ...
            # Called from a non-main thread â€” not fatal.  The update still
    ...
    try:
        ...
        # zed_cli.config.get_zed_home to simulate setup failure.
        from zed_cli.config import get_zed_home as _get_zed_home

        logs_dir = _get_zed_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``zed update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from zed_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``zed update`` (apply
        # path) uses â€” telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from zed_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from zed_cli.config import recommended_update_command
        from zed_cli.banner import check_via_pypi
        if branch_explicit and branch != "main":
            print(f"âš  --branch is ignored for PyPI installs (would have checked '{branch}').")
        result = check_via_pypi()
        if result is None:
            print("âœ— Could not reach PyPI to check for updates.")
            sys.exit(1)
        elif result == 0:
            print("âœ“ Already up to date.")
        else:
            print("âš• Update available on PyPI.")
            print(f"  Run '{recommended_update_command()}' to install.")
        return

    git_dir = PROJECT_ROOT / ".git"
    if not git_dir.exists():
        print("âœ— Not a git repository â€” cannot check for updates.")
        sys.exit(1)
    ...
    if branch == "main":
        print("â†’ Fetching from upstream...")
    ...
    if fetch_result.returncode != 0:
        ...
        if "Could not resolve host" in stderr or "unable to access" in stderr:
            ...
            print("âœ— Authentication failed â€” check your git credentials or SSH key.")
        else:
    ...
    if verify_result.returncode != 0:
        print(f"âœ— Branch '{branch}' not found on {compare_branch.split('/', 1)[0]}.")
        sys.exit(1)
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``zed update`` without requiring a reinstall.  The
    ...
    ``zed`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/zed.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v zed'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
        if os.geteuid() != 0:  # windows-footgun: ok â€” Linux FHS helper, guarded by sys.platform == "linux" above + AttributeError catch
            return
    except AttributeError:
        return
    # Only act when this is actually an FHS-layout install (command link at
    # /usr/local/bin/zed, code at /usr/local/lib/zed-agent).
    fhs_link = Path("/usr/local/bin/zed")
    ...
    try:
        ...
        return  # no bash or probe hung â€” don't block update on this
    if probe.returncode == 0:
        return  # already on PATH, nothing to do

    path_line = 'export PATH="/usr/local/bin:$PATH"'
    path_comment = (
        "# Zed Agent â€” ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ZED_HOME before running the update.
    ...
    ZED_HOME directories.  The ``--backup`` flag on ``zed update``
    opts in for a single run; ``--no-backup`` forces it off when config
    has it enabled.  Never raises â€” a backup failure should not block the
    update itself.
    """
    # CLI flags win over config.  --no-backup beats --backup if both are set.
    if getattr(args, "no_backup", False):
        print("â—† Pre-update backup: skipped (--no-backup)")
        print()
        return
    ...
    # ``zed_cli/config.py``). Fall back to true if the key is missing
    # (e.g. a user has an older custom config without the field). The
    # ``False`` default from before #48200 caused silent data loss when
    # an update step computed a wrong path â€” the cost of a few minutes
    ...
    if not enabled and not force_backup:
        # Silent by default â€” the backup is off, most users don't need to
        # hear about it on every update.  They can opt in via --backup
        # or by flipping the config knob.
        return

    try:
        from zed_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
            f"âš  Pre-update backup: could not load backup module ({exc}); continuing update."
        )
        print()
        return

    print("â—† Creating pre-update backup...")
    t0 = _time.monotonic()
    try:
        out_path = create_pre_update_backup(keep=int(keep))
    except Exception as exc:  # defensive â€” helper already swallows, but just in case
        print(f"  âš  Backup failed: {exc}")
    ...
    if out_path is None:
        print("  âš  Backup skipped (no files found or write failed); continuing update.")
        print()
        return
    ...
    # Render path using display_zed_home so the user sees ~/.zed/...
    try:
        from zed_constants import get_zed_home, display_zed_home

        home = get_zed_home()
        try:
            display_path = f"{display_zed_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  zed import {out_path}")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    zed.exe concurrent-instance guard does not see them. They still import
    ...
    print("â†’ Stopping Windows gateway process(es) before updating Zed...")
    try:
    ...
    if unmapped_pids:
        print(
            f"  â†’ Stopped {len(unmapped_pids)} gateway process(es) without profile mapping"
        )
        print("    Restart manually after update: zed gateway run")
...
def _resume_windows_gateways_after_update(token: dict | None) -> None:
    ...
    try:
        from zed_cli.gateway import launch_detached_profile_gateway_restart
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``zed update`` sees a clean tree instead of autostashing every run.
    ...
    try:
        ...
        print(f"â†’ Discarded npm lockfile churn ({len(dirty)} file(s))")
    except Exception:
...
def cmd_update(args):
    """Update Zed Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Zed Agent")
        return

    # Docker users can't ``git pull`` â€” the image excludes ``.git`` from
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `zed update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Zed via pip (for PyPI installs)."""
    from zed_cli import __version__
    from zed_cli.config import is_uv_tool_install

    print(f"â†’ Current version: {__version__}")
    print("â†’ Checking PyPI for updates...")
    ...
    if is_uv_tool_install():
        if not uv:
            print("âœ— Detected a uv-tool install but managed uv install failed.")
            print("  Install uv manually: https://docs.astral.sh/uv/getting-started/installation/")
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "zed-agent"]
        ...
        cmd = [pipx, "upgrade", "zed-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "zed-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "zed-agent"]
    ...
    print("âœ“ Update complete! Restart zed to use the new version.")


def _cmd_update_impl(args, gateway_mode: bool):
    """Body of ``cmd_update`` â€” kept separate so the wrapper can always
    ...
    print("âš• Updating Zed Agent...")
    print()

    # On Windows, abort early if another zed.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_zed_instances(scripts_dir)
            if concurrent:
                print(_format_concurrent_instances_message(concurrent, scripts_dir))
                sys.exit(2)

    # Pre-update backup â€” runs before any git/file mutation so users can
    ...
    # Build git command once â€” reused for fork detection and the update itself.
    git_cmd = ["git"]
    if sys.platform == "win32":
        git_cmd = ["git", "-c", "windows.appendAtomically=false"]
    ...
    # tree dirty â€” forcing an autostash on every update and making branch
    ...
    try:
        ...
        # thousands of auto-generated branches â€” an unscoped fetch can stall for
        ...
        # the same thing â€” get HEAD onto the requested branch first, then
        # fast-forward.
        if current_branch != branch:
            ...
            print(f"  âš  Currently on {label} â€” switching to {branch} for update...")
            ...
            if checkout_result.returncode != 0:
                ...
                if track_result.returncode != 0:
                    ...
                    print(f"âœ— Branch '{branch}' does not exist locally or on origin.")
        ...
        # though `git pull` can't touch $ZED_HOME, this is cheap
        ...
        try:
            from zed_cli.backup import create_quick_snapshot

            pre_update_snapshot_id = create_quick_snapshot(label="pre-update", keep=1)
            if pre_update_snapshot_id:
                print(f"  âœ“ Pre-update snapshot: {pre_update_snapshot_id}")
        except Exception as exc:
        ...
        # orphan merge-conflict markers in zed_cli/config.py bricked
        # every user who ran ``zed update`` for the 7 minutes between
        ...
        try:
            ...
            if pull_result.returncode != 0:
                ...
                print(
                    "  âš  Fast-forward not possible (history diverged), resetting to match remote..."
                )
            ...
            # so the CLI stays bootable. The user can then retry ``zed
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``zed update`` again later once a fix lands.")
                    else:
            ...
            if auto_stash_ref is not None:
                ...
                if not update_succeeded:
                    print(
                        f"  â„¹ï¸  Local changes preserved in stash (ref: {auto_stash_ref})"
                    )
        ...
        # the old bytecode (e.g. get_zed_home added to zed_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  âœ“ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``zed`` launch finishes the
        ...
        if uv_bin:
            ...
            if _is_termux_env(uv_env):
                ...
                print("  â†’ Termux detected: using uv + curated termux-all optional profile...")
            if _is_termux_env(uv_env) and _is_android_python():
                print("  â†’ Termux/Android detected: prebuilding psutil with Linux source path compatibility...")
            ...
            if _is_termux_env():
                install_group = "termux-all"
                print("  â†’ Termux detected: using curated termux-all optional profile...")
        ...
        # breadcrumb now â€” the remaining steps (lazy refresh, node deps, web
        ...
        # build.  ``zed desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``zed desktop`` shouldn't be forced into a full
        # Electron build by ``zed update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        from zed_constants import find_node_executable

        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            print("â†’ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "zed_cli.main", "desktop", "--build-only"]
            # Stream the build output live (long Electron builds otherwise
            # look hung). On the rare nonzero exit, retry once after waiting
            # again for the venv â€” this covers a still-settling rebuild window
            # the first wait didn't fully catch.
            build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                print("  âš  Desktop build failed (non-fatal; run `zed desktop` to retry)")
        ...
        # current â€” so copy it straight over ~/.zed/cache/model_catalog.json
        ...
        # modules in this process.  Reload zed_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_zed_home() added since the last release.
        try:
            import importlib
            import zed_constants as _hc

            importlib.reload(_hc)
        except Exception:
            pass  # non-fatal â€” worst case a lazy import fails gracefully
        ...
        # seed_profile_skills() uses subprocess with an explicit ZED_HOME so
        # it is not affected by sync_skills()'s module-level ZED_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ZED_HOME env var points at the default or a named profile.
        try:
            ...
            if all_profiles:
                print()
                print("â†’ Syncing bundled skills to all profiles...")
                for p in all_profiles:
                    try:
                        ...
                        if r and r.get("skipped_opt_out"):
                            ...
                            if updated:
                                parts.append(f"â†‘{updated} updated")
                            if modified:
        ...
        try:
            from zed_cli.profiles import backfill_profile_envs

            backfilled = backfill_profile_envs(quiet=True)
            if backfilled:
                print()
                print(
                    f"â†’ Seeded .env for {len(backfilled)} profile(s) "
        ...
        if version_bump_only:
            # Nothing for the user to fill in â€” only the config format version
            ...
            try:
                ...
                print("     Run 'zed config migrate' to retry.")
            ...
            if missing_env:
                print(
                    f"  âš ï¸  {len(missing_env)} new required setting(s) need configuration"
                )
            ...
            if assume_yes:
                print(
                    "  â„¹ --yes: auto-applying config migration (skipping API-key prompts)."
                )
                response = "y"
            elif gateway_mode:
                response = (
                ...
                print("  â„¹ Non-interactive session â€” applying safe config migrations.")
                response = "auto"
            else:
                try:
                    response = (
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  â„¹ API keys require manual entry: zed config migrate")
            else:
                print()
                print("Skipped. Run 'zed config migrate' later to configure.")
        else:
        ...
        try:
            from zed_cli.backup import restore_cron_jobs_if_emptied

            cron_restore = restore_cron_jobs_if_emptied(pre_update_snapshot_id)
            if cron_restore:
                print()
                print(
                    "  âš ï¸  cron/jobs.json was emptied during this update â€” "
        ...
        # has never run â€” i.e. the window where the ticker would otherwise
        ...
        # check `zed curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``zed update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from zed_cli.tools_config import install_cua_driver

                print()
                print("â†’ Refreshing cua-driver (Computer Use)...")
                install_cua_driver(upgrade=True)
        except Exception as e:
            logger.debug("cua-driver refresh failed: %s", e)

        # Write exit code *before* the gateway restart attempt.
        # When running as ``zed update --gateway`` (spawned by the gateway's
        ...
        # the cgroup (KillMode=mixed â†’ SIGKILL to remaining processes),
        ...
        # timeout message.
        #
        # Writing the marker here â€” after git pull + pip install succeed but
        # before we attempt the restart â€” ensures the new gateway sees it
        # regardless of how we die.
        if gateway_mode:
            _exit_code_path = get_zed_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                10-15s timeout â€” the user sees the prompt flash and "exit
                ...
                non-interactive sudo (``sudo -n``) â€” first a blanket probe,
                then a targeted ``systemctl reset-failed`` probe so a
                least-privilege sudoers entry scoped to
                ``systemctl ... zed-gateway*`` also qualifies
                (``reset-failed`` is an idempotent no-op we run before every
                privileged restart anyway).  If neither works, return None â€”
                ...
                if (
                    ...
                    and os.geteuid() != 0  # windows-footgun: ok â€” systemd path, Linux-only
                ):
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            # Blanket sudo refused â€” a targeted sudoers entry
                            # (NOPASSWD for systemctl ... zed-gateway*)
            ...
            # Discover all zed-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. zed-gateway.service or zed-gateway-coder.service
                            ...
                            # None â‡’ no non-interactive privilege path; we
                            ...
                            # flash and "exit directly" â€” reported June 2026).
                            ...
                            # request_restart(via_service=True) â†’ drain â†’
                            ...
                            if _main_pid > 0:
                                print(
                                    f"  â†’ {svc_name}: draining (up to {int(_drain_budget)}s)..."
                                )
                            ...
                            if _graceful_ok:
                                ...
                                # passwordless sudo) skip it â€” systemd's own
                                ...
                                if _manage_cmd is None and _restart_sec > 5.0:
                                    print(
                                        f"  â†’ {svc_name}: waiting for systemd "
                                ...
                                print(
                                    f"  âš  {svc_name} drained but didn't relaunch â€” forcing restart"
                                )
                            ...
                            # â€” it flashes and dies before the user can
                            # answer.  Skip with clear instructions instead.
                            if _manage_cmd is None:
                                print(
                                    f"  âš  {svc_name} is a system service and restarting it needs root.\n"
                                    f"    Restart it manually to load the new version:\n"
                                    f"      sudo systemctl restart {svc_name}\n"
                                    f"    To let `zed update` restart it automatically, allow\n"
                            ...
                            if restart.returncode == 0:
                                ...
                                if _wait_for_service_active(
                                    ...
                                    # Retry once â€” transient startup failures
                                    ...
                                    # crash.
                                    print(
                                        f"  âš  {svc_name} died after restart, retrying..."
                                    )
                                    subprocess.run(
                                        _manage_cmd + ["reset-failed", svc_name],
                                        capture_output=True,
                                    ...
                                    if _wait_for_service_active(
                                        ...
                                        print(
                                            f"  âœ— {svc_name} failed to stay running after restart.\n"
                                ...
                                print(
                                    f"  âš  Failed to restart {svc_name}: {restart.stderr.strip()}"
                                )
                    except FileNotFoundError:
                        pass
                    except subprocess.TimeoutExpired as exc:
                        # Don't swallow this silently â€” a wedged systemctl
                        # call here used to make the whole restart phase
                        # vanish with no output (June 2026 report).
                        print(
                            f"  âš  systemctl timed out during the {scope}-scope "
                            f"gateway restart ({exc.cmd if exc.cmd else 'unknown command'}). "
                            f"Check the gateway with: zed gateway status"
                        )
                    ...
                    if plist_path.exists():
                        ...
                        if check.returncode == 0:
                            try:
                                ...
                                print(f"  âš  Gateway restart failed: {stderr}")
            ...
            for pid, proc in profile_processes.items():
                ...
                # the drain budget, fall back to SIGTERM â€” the watcher
                ...
                # via back-off retries â€” but a brief wait here reduces the
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    print(f"  â†’ Stopped {unmapped_count} manual gateway process(es)")
                    print("    Restart manually: zed gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: zed -p <profile> gateway run  for each profile)"
                        )

            if not restarted_services and not killed_pids:
                # No gateways were running â€” nothing to do
                pass
            ...
            # watchers wait 120s for the old PID to exit â€” if it never
            ...
            try:
                ...
                # started AFTER our restart attempt â€” respecting user
                # intent, we don't kill those.
                _stuck = [pid for pid in _surviving if pid in killed_pids]
                if _stuck:
                    print()
                    print(
                        f"  âš  {len(_stuck)} gateway process(es) ignored SIGTERM â€” force-killing"
                    )
                    from gateway.status import terminate_pid as _terminate_pid
                    for pid in _stuck:
                        try:
                            # Routes through taskkill /T /F on Windows,
                            # SIGKILL on POSIX â€” _signal.SIGKILL doesn't
        ...
        # Warn if legacy Zed gateway unit files are still installed.
        # When both zed.service (from a pre-rename install) and the
        # current zed-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `zed update` surfaces the issue until the user migrates.
        try:
            from zed_cli.gateway import (
                has_legacy_zed_units,
                _find_legacy_zed_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_zed_units():
                print()
                print("âš  Legacy Zed gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_zed_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (zed.service) fight the current")
                print("  zed-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    zed gateway migrate-legacy")
                print()
                print("  (add `sudo` if any are in system scope)")
        except Exception as e:
            logger.debug("Legacy unit check during update failed: %s", e)

        # Kill stale dashboard processes â€” the dashboard has no service
        ...
        print("  zed model              # Select provider and model")

    except subprocess.CalledProcessError as e:
        if sys.platform == "win32":
            print(f"âš  Git update failed: {e}")
            print("â†’ Falling back to ZIP download...")
            print()
            _update_via_zip(args)
        else:
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``zed -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    """Profile management â€” create, delete, list, switch, alias."""
    from zed_cli.profiles import (
        list_profiles,
        create_profile,
        delete_profile,
    ...
    if action is None:
        # Bare `zed profile` â€” show current profile status
        profile_name = get_active_profile_name()
        dhh = display_zed_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} â†’ zed -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        for p in profiles:
            ...
            name = p.name
            model = (p.model or "â€”")[:26]
            gw = "running" if p.gateway_running else "stopped"
            alias = (p.alias_name or p.name) if p.alias_path else "â€”"
            if p.is_default:
                alias = "â€”"
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.zed)")
            else:
        ...
        try:
            ...
            if not (clone_config or clone_all):
                ...
                if result and result.get("skipped_opt_out"):
                    ...
                    print(
                        "âš  Skills could not be seeded. Run `{} update` to retry.".format(
                            name
                        )
                    )
            ...
            if not no_alias:
                ...
                if collision:
                    print(f"\nâš  Cannot create alias '{name}' â€” {collision}")
                    print(
                        f"  Choose a custom alias:  zed profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     zed -p {name} chat")
                else:
                    wrapper_path = create_wrapper_script(name)
                    if wrapper_path:
                        print(f"Wrapper created: {wrapper_path}")
                        if not _is_wrapper_dir_in_path():
                            print(f"\nâš  {_get_wrapper_dir()} is not in your PATH.")
                            print(
            ...
            if clone or clone_all:
                ...
                print(
                    f"\n  âš  This profile has no API keys yet. Run '{name} setup' first,"
                )
        ...
        from zed_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from zed_constants import get_zed_home as _hh
        ...
        from zed_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `zed profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} â†’ zed -p {name}  ({wrapper})")
        print()
        ...
        from zed_cli.profiles import profile_exists
        ...
        from zed_cli.profiles import rename_profile

        try:
            new_dir = rename_profile(args.old_name, args.new_name)
            print(f"\nProfile renamed: {args.old_name} â†’ {args.new_name}")
        ...
        from zed_cli.profiles import export_profile
        ...
        from zed_cli.profiles import import_profile
        ...
        import tempfile
        from zed_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="zed_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  zed -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      zed -p {plan.manifest.name} chat")
        ...
        from zed_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `zed profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"zed -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from zed_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("zed_requires"):
            print(f"Requires:     Zed {data['zed_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from zed_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.zed_requires:
        print(f"  Requires: Zed {mf.zed_requires}")
    print(f"  Source:   {plan.provenance}")
    print(f"  Target:   {plan.target_dir}")
    if plan.existing:
        # Distinguish "updating an existing distribution" (well-understood
        # semantics â€” dist-owned overwritten, config preserved, user data
        ...
        if existing_is_distribution:
            print("  (profile exists â€” will overwrite distribution-owned files only)")
        else:
            print(
                "  âš  Profile exists but is NOT a distribution.  Installing here will\n"
    ...
    if plan.has_cron:
        print(
            "\n  âš  This distribution ships cron jobs.  They will NOT run "
            "automatically â€” review and enable manually."
        )


def _report_dashboard_status() -> int:
    """Print ``zed dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``zed dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No zed dashboard processes running.")
        return 0

    print(f"{len(pids)} zed dashboard process(es) running:")
    for pid in pids:
...
def _dashboard_listening(host: str, port: int) -> bool:
    ...
    Any listener counts â€” even a 401 response proves a dashboard is up.
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No zed dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `zed update`.
    ...
    # â”€â”€ Unified profile launch routing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # launches the dashboard (`worker dashboard` â†’ ZED_HOME points into
    # profiles/), default to the machine dashboard:
    #   - already running â†’ open the browser at ?profile=<name> and exit
    #   - not running     â†’ re-exec as the machine dashboard (pinned to the
    ...
    if (
        ...
        and os.environ.get("ZED_DESKTOP") != "1"
    ):
        ...
        # ZED_HOME.  We must resolve the root explicitly instead of just
        # dropping ZED_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV ZED_HOME=/opt/data`), so an unset
        # ZED_HOME falls back to $HOME/.zed = /opt/data/.zed â€” an
        ...
        # update-button guard also misfires).  get_default_zed_root()
        # returns the root for both layouts: ~/.zed for a standard install
        ...
        try:
            from zed_constants import get_default_zed_root
            env["ZED_HOME"] = str(get_default_zed_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop ZED_HOME) rather than block the reroute.
            env.pop("ZED_HOME", None)
        # On Windows, os.execvpe() does not truly replace the process â€” it
    ...
    # the same logs directory as every other Zed surface.
    try:
        from zed_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if "ZED_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["ZED_WEB_DIST"])
            if "ZED_WEB_DIST" in os.environ
            else PROJECT_ROOT / "zed_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            print(f"âœ— --skip-build was passed but no web dist found at: {_dist_root}")
    ...
    # providers (image_gen, web, dashboard_auth, â€¦).
    try:
        from zed_cli.plugins import discover_plugins
        discover_plugins()
    except Exception as exc:
        # Discovery failures must not block dashboard startup outright â€”
        # log and proceed; the gate's fail-closed branch will surface
        # the missing-provider state if it matters.
        print(f"âš  Plugin discovery failed: {exc}", file=sys.stderr)
    ...
    # tool registry â€” it never starts MCP discovery (the stdio TUI does that in
    ...
    from zed_cli.web_server import start_server

    # The in-browser Chat tab (the embedded TUI over PTY/WebSocket) is always
    # available â€” the desktop app and the dashboard's own Chat tab both rely on
...
def cmd_gateway_enroll(args):
    ...
    from zed_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from zed_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from zed_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Zed log files."""
    from zed_cli.logs import tail_log, list_logs
...
_BUILTIN_SUBCOMMANDS = frozenset(
    {
        ...
        # Help-ish invocations â€” plugin commands not being listed in
...
# so that in ``zed -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``zed_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``zed -m gpt5 --provider openai chat "msg"`` by skipping the
    values attached to known top-level flags.

    Does NOT fully simulate argparse â€” unknown ``--foo=bar`` / ``--foo
    ...
    while i < len(argv):
        ...
        if tok == "--":
            ...
            return None
        if tok.startswith("-"):
            # ``--flag=value`` carries its value inline â€” single token.
            if "=" in tok:
                i += 1
                continue
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``zed`` or only flags â†’ defaults to ``chat``.
        return False
    if first in _BUILTIN_SUBCOMMANDS:
        return False
    # Unknown token â€” could be a plugin subcommand, OR a chat prompt
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("ZED_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    # the TUI â€” explicit --tui/env or display.interface=tui. `--cli` forces this
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["ZED_DEFER_AGENT_STARTUP"] = "1"
            os.environ["ZED_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["ZED_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `zed --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        if not existing:
            print(
                f"\n  Nothing to reset â€” no memory files found in {display_zed_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_zed_home()}/memories/\n")
    else:
        from zed_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Zed Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from zed_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from zed_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from zed_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from zed_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from zed_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from zed_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from zed_cli.plugins_cmd import plugins_command
...
def cmd_claw(args):
    from zed_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for zed CLI."""
    # Cosmetic: make the process show up as 'zed' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal â€” just a nicer UX.
    _set_process_title()
    ...
    # Sweep stale ``zed.exe.old.*`` quarantine files left by previous
    # ``zed update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_zed_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``zed update``
    # (Ctrl-C, terminal close, WSL OOM mid-install). Skip when the user is
    # *running* update â€” that flow writes and clears its own marker, and we
    ...
    # ``zed skills install update``) merely defers recovery one launch;
    # under-matching (missing ``zed -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in zed_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    # =========================================================================
    # fallback command â€” manage the fallback provider chain
    # =========================================================================
    from zed_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `zed model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    # =========================================================================
    # secrets command â€” external secret managers (currently: Bitwarden)
    # =========================================================================
    ...
    # Lazy import â€” only pays for itself when this subcommand is actually used.
    from zed_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from zed_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in zed_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    try:
        ...
        # LSP is optional infrastructure â€” never let a registration
        # failure break the CLI overall.
        logger.debug("LSP CLI registration failed: %s", _lsp_err)

    # =========================================================================
    # setup command  (parser built in zed_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in zed_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in zed_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `zed whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in zed_cli/subcommands/slack.py)
    # =========================================================================
    build_slack_parser(subparsers, cmd_slack=cmd_slack)

    # =========================================================================
    # send command â€” pipe shell-script output to any configured platform
    # =========================================================================
    from zed_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in zed_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in zed_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in zed_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in zed_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in zed_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in zed_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    # kanban command â€” multi-profile collaboration board
    # =========================================================================
    from zed_cli.kanban import build_parser as _build_kanban_parser

    kanban_parser = _build_kanban_parser(subparsers)
    kanban_parser.set_defaults(func=cmd_kanban)

    # =========================================================================
    # hooks command â€” shell-hook inspection and management
    # =========================================================================
    # hooks command  (parser built in zed_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in zed_cli/subcommands/doctor.py)
    # =========================================================================
    build_doctor_parser(subparsers, cmd_doctor=cmd_doctor)

    # =========================================================================
    # security command â€” on-demand supply-chain audit
    # =========================================================================
    # security command  (parser built in zed_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in zed_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in zed_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in zed_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.zed/checkpoints/",
        description="Manage the filesystem checkpoint store â€” the shadow git "
        "repo zed uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from zed_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in zed_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in zed_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in zed_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in zed_cli/subcommands/skills.py)
    # =========================================================================
    build_skills_parser(subparsers, cmd_skills=cmd_skills)

    # =========================================================================
    # bundles command â€” skill bundles (alias /<name> for multiple skills)
    # =========================================================================
    ...
    from zed_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in zed_cli/subcommands/plugins.py)
    # =========================================================================
    build_plugins_parser(subparsers, cmd_plugins=cmd_plugins)

    # =========================================================================
    # Plugin CLI commands â€” dynamically registered by memory/general plugins.
    ...
    # subcommand â€” ``zed --help``, ``zed version``, ``zed logs``,
    ...
    # =========================================================================
    # curator command â€” background skill maintenance
    # =========================================================================
    curator_parser = subparsers.add_parser(
        "curator",
        help="Background skill maintenance (curator) â€” status, run, pause, pin",
        description=(
    ...
    try:
        from zed_cli.curator import register_cli as _register_curator_cli
    ...
    # =========================================================================
    # memory command  (parser built in zed_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in zed_cli/subcommands/tools.py)
    # =========================================================================
    build_tools_parser(subparsers, cmd_tools=cmd_tools)

    # =========================================================================
    # computer-use command â€” manage Computer Use (cua-driver) on macOS
    # =========================================================================
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `zed computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `zed tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: zed computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: zed computer-use install")
            return
    ...
    # =========================================================================
    # mcp command  (parser built in zed_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    sessions_browse = sessions_subparsers.add_parser(
        "browse",
        help="Interactive session picker â€” browse, search, and resume sessions",
    )
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_zed_home() / "sessions"
            ...
            # Launch zed --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in zed_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in zed_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in zed_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in zed_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in zed_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in zed_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in zed_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in zed_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Zed-Setup.exe success screen tells users to
    # run `zed desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in zed_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in zed_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in zed_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``zed -c Pokemon Agent Dev`` â†’ ``zed -c 'Pokemon Agent Dev'``
    # â”€â”€ Container-aware routing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    from zed_cli.config import get_container_exec_info
    ...
    # â”€â”€ Defensive subparser routing (bpo-9338 workaround) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # fails (e.g. 'zed -c model' where 'model' is consumed as the
    ...
    if _has_cmd_token:
        ...
        try:
            ...
            sys.stderr = _saved_stderr
            # Help/version flags (exit code 0) already printed output â€”
    ...
    # so introspection/management commands (zed hooks list, cron
