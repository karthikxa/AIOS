...
DEFAULT_CODEX_MODELS: List[str] = [
    ...
    # in zed_cli/models.py â€” only the openai-codex (OAuth) provider
    ...
    # was unsupported â€” that was wrong; restored here. Keep it in the
    ...
    # these slugs, which is why they survived previously â€” but those entries
    # describe the public OpenAI API, not the OAuth-backed Codex backend
    # Zed uses. Removed here. If OpenAI re-enables them on Codex backend,
...
def _read_cache_models(codex_home: Path) -> List[str]:
    ...
    if isinstance(entries, list):
        for item in entries:
            ...
            # public OpenAI API, while Zed openai-codex talks to the same
