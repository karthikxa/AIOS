...
into prompt_toolkit's event loop. Each function takes the ZedCLI instance
...
from zed_cli.banner import cprint, _DIM, _RST
...
def prompt_for_secret(cli, var_name: str, prompt: str, metadata=None) -> dict:
    ...
    The secret is stored in ~/.zed/.env and never exposed to the model.
    """
    if not getattr(cli, "_app", None):
        ...
        if not value:
            cprint(f"\n{_DIM}  â­ Secret entry skipped{_RST}")
            return {
                "success": True,
                "reason": "cancelled",
                "stored_as": var_name,
