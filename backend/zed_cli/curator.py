"""CLI subcommand: `zed curator <subcommand>`.
...
This module intentionally has no side effects at import time â€” main.py wires
...
def _cmd_status(args) -> int:
    ...
    # map gets appended as `name â†’ umbrella` lines). Indent continuation
...
def _cmd_run(args) -> int:
    ...
    if consolidate is None and not curator.get_consolidate():
        print(
            "curator: consolidation is off â€” running prune-only "
    ...
    if auto:
        if dry:
            print(
                ...
                "â€” no transitions applied in dry-run"
            )
        else:
            print(
    ...
    if not synchronous:
        print("llm pass running in background â€” check `zed curator status` later")
    if dry:
        if synchronous:
            print(
                "dry-run: no changes applied. Read the report with "
                "`zed curator status` and run `zed curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_pin(args) -> int:
    ...
    if not skill_usage.is_agent_created(args.skill):
        print(
            f"curator: '{args.skill}' is bundled or hub-installed â€” cannot pin "
...
def _cmd_unpin(args) -> int:
    ...
    if not skill_usage.is_agent_created(args.skill):
        print(
            f"curator: '{args.skill}' is bundled or hub-installed â€” "
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            f"curator: '{args.skill}' is pinned â€” unpin first with "
            f"`zed curator unpin {args.skill}`"
        )
        return 1
...
def _idle_days(record: dict) -> Optional[int]:
    ...
    can still be pruned â€” otherwise never-touched skills would be immortal.
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        print("curator: snapshot failed â€” check logs (backup disabled or IO error)")
        return 1
    print(f"curator: snapshot created at ~/.zed/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`zed curator backup` or wait for the next curator run."
            )
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.zed/skills/ tree (a safety "
    ...
    if ok:
        print(f"curator: {msg}")
        return 0
    print(f"curator: rollback failed â€” {msg}")
    return 1
...
# ---------------------------------------------------------------------------
# argparse wiring (called from zed_cli.main)
# ---------------------------------------------------------------------------
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_run.add_argument(
        ...
        help="Report only â€” no state changes, no archives, no consolidation "
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.zed/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.zed/skills/ from a curator snapshot "
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by zed_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="zed curator")
    register_cli(parser)
