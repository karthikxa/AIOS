"""Shared curses-based UI components for Zed CLI.

Used by `zed tools` and `zed skills` for interactive checklists.
...
def flush_stdin() -> None:
    ...
    but does NOT drain the OS input buffer â€” leftover escape-sequence bytes
...
def read_menu_key(stdscr) -> str:
    ...
    keys as raw CSI/SS3 byte sequences â€” ``getch()`` then returns ``27`` (ESC)
...
def _decode_menu_key(stdscr, key: int) -> str:
    ...
    if key == 27:  # ESC â€” could be a lone ESC (cancel) or an escape sequence.
        ...
        if nxt in (ord("["), ord("O")):  # CSI / SS3 introducer
            ...
            while 0x20 <= final <= 0x3F:  # CSI parameter/intermediate bytes
                final = stdscr.getch()
            return NAV_NONE
        # ESC followed by some other byte we don't handle â€” swallow it.
        return NAV_NONE
...
def _run_curses_menu(
    *,
    ...
    # so return the cancel value directly â€” matching the pre-refactor guard in
...
def curses_checklist(
    ...
    *,
    ...
    def _draw_row(stdscr, y, i, is_cursor, max_x):
        import curses
        check = "âœ“" if i in chosen else " "
        arrow = "â†’" if is_cursor else " "
    ...
    *,
    ...
    def _draw_header(stdscr, max_y, max_x, search=None):
        ...
        try:
            ...
            if searchable and search is not None and search.active:
                ...
                hint = "  â†‘â†“ navigate  ENTER confirm  / search  ESC/q cancel"
            else:
                hint = "  â†‘â†“ navigate  ENTER confirm  ESC/q cancel"
    ...
    def _on_action(action, cursor):
        ...
        if action == NAV_CANCEL:
            return None
        return _KEEP  # NAV_TOGGLE â€” no-op for this menu
...
def _numbered_fallback(
    ...
    while True:
        for i, label in enumerate(items):
            marker = color("[âœ“]", Colors.GREEN) if i in chosen else "[ ]"
            print(f"  {marker} {i + 1:>2}. {label}")
        if status_fn:
            status_text = status_fn(chosen)
            if status_text:
