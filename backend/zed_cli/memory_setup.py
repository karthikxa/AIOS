"""zed memory setup|status â€” configure memory provider plugins.
...
# ---------------------------------------------------------------------------
# Curses-based interactive picker (same pattern as zed tools)
# ---------------------------------------------------------------------------
...
def _install_dependencies(provider_name: str) -> None:
    ...
    if uv_path:
        ...
        if not pip_cmd:
            ...
            print(f"  Then re-run: zed memory setup")
            return
        print(f"  âš  uv not found. Falling back to standard pip...")
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if not match:
        ...
        print("  Run 'zed memory setup' to see available providers.\n")
        return
    ...
    if hasattr(provider, "post_setup"):
        ...
        provider.post_setup(zed_home, config)
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.zed/plugins/ and try again.\n")
        return
    ...
    if provider_config and hasattr(provider, "save_config"):
        try:
            provider.save_config(provider_config, zed_home)
        except Exception as e:
...
def _write_env_vars(env_path: Path, env_writes: dict) -> None:
    ...
    env_path.write_text("\n".join(new_lines) + "\n", encoding="utf-8")
    # Restrict permissions â€” .env holds API keys and tokens.
    try:
        import stat
...
def cmd_status(args) -> None:
    ...
    if provider_name:
        ...
        if provider:
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.zed/plugins/")
