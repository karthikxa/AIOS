...
needs â€” pick a blueprint by name and the agent asks you for what it needs, one
question at a time (the messaging-assistant model: pick a blueprint â†’ it asks you
a couple things â†’ done).
...
Subcommand shapes:
  ...
  /blueprint <name> slot=val â€¦    fill + create the cron job directly
                                    (the deterministic dashboard / docs / power-
                                    user shortcut â€” no agent turn)
...
closest. When it resolves, the handler returns an ``agent_seed`` â€” a natural-
language instruction built from the blueprint's typed slots + schedule/prompt
templates â€” that the calling surface feeds to the agent as a normal user turn
...
def match_blueprint(query: str) -> Tuple[Optional[Any], List[Any]]:
    ...
    Returns ``(blueprint, candidates)``:
      ...
      * ambiguous (2+ plausible) -> ``(None, [candidatesâ€¦])``
...
def build_blueprint_seed(blueprint) -> str:
    ...
    for s in blueprint.slots:
        ...
        if s.options:
            bits.append(f" â€” one of: {', '.join(map(str, s.options))}")
        if s.default not in (None, ""):
            bits.append(f" [default: {s.default}]")
        if s.optional:
            bits.append(" (optional)")
        if s.help:
            bits.append(f" â€” {s.help}")
...
def _fmt_catalog() -> str:
    ...
    lines = ["Automation Blueprints â€” `/blueprint <name>` and I'll ask you what I need:\n"]
    for r in CATALOG:
        lines.append(f"  â€¢ {r.key} â€” {r.title}")
...
def _fmt_candidates(query: str, candidates: List[Any]) -> str:
    lines = [f"'{query}' matches several blueprints â€” which one?\n"]
    for r in candidates:
        lines.append(f"  â€¢ {r.key} â€” {r.title}")
...
def handle_blueprint_command(
    ...
    *,
    ...
    (``"cli"`` | ``"gateway"``) picks the right wording for follow-up hints â€”
    ...
    if not values:
        ...
        text = (
            ...
            "I'll ask you a couple of thingsâ€¦"
        )
        return BlueprintCommandResult(text, agent_seed=seed)

    # `<name> slot=val â€¦` -> fill + create directly (deterministic shortcut).
    try:
