"""
Dump command for zed CLI.

Outputs a compact, plain-text summary of the user's Zed setup
that can be copy-pasted into Discord/GitHub/Telegram for support context.
No ANSI colors, no checkmarks â€” just data.
"""
...
from zed_cli.config import get_zed_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    that lookup always fails â€” we fall back to the baked-in build SHA written
    to ``<project_root>/.zed_build_sha`` by the Dockerfile's
    ``ZED_GIT_SHA`` build-arg (see ``zed_cli/build_info.py``).
...
def _get_git_commit_date(project_root: Path) -> str:
    ...
    image excludes ``.git``, so this returns '' there â€” the dump line simply
...
def _redact(value: str) -> str:
    ...
    an empty value (matches the historical behavior of this helper â€”
    ``zed dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from zed_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(zed_home: Path) -> int:
...
def _cron_summary(zed_home: Path) -> str:
...
def run_dump(args):
    ...
    # Terminal backend â€” report the EFFECTIVE backend, not just config.yaml.
    ...
    # intentionally NOT shown here â€” it reads like a wall-clock timestamp and
    ...
    lines.append(f"zed_home:      {display_zed_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `zed auth add openrouter` lives in the
        # credential pool, not as an env var â€” surface it so the dump doesn't
        # misleadingly read "not set" while `zed auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["zed-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(zed_home)}")
    lines.append(f"  skills:             {_count_skills(zed_home)}")
