"""
Security advisory checker for Zed Agent.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``zed doctor --ack <id>``; the ack is persisted to
...
1. ``zed doctor`` (and ``zed doctor --ack <id>``)
...
run in environments where the rest of Zed failed to import.
"""
...
#      â€” version strings must match what ``importlib.metadata.version()``
...
class Advisory:
    ...
    Attributes:
        ...
        compromised: tuple of ``(package_name, frozenset_of_versions)``
            ...
            considered suspect" â€” use sparingly.
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        title="Mini Shai-Hulud worm â€” mistralai 2.4.6 compromised on PyPI",
        summary=(
            ...
            "imported mistralai 2.4.6 â€” including zed when configured "
            "with provider=mistral for TTS or STT â€” assume those credentials "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.zed/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: zed doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 â€” Zed requires 3.10+ but defensive.
        return None
    try:
...
# advisory IDs. The list is the only state â€” no per-host data, no
...
def get_acked_ids() -> set[str]:
    ...
    just because config is broken â€” the advisory will keep firing until
...
def ack_advisory(advisory_id: str) -> bool:
    ...
    Idempotent â€” acking an already-acked ID is a no-op.
    """
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'zed doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.zed/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_zed_home()) / "cache"
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``zed doctor``.
    ...
    fresh = filter_unacked(hits)
    if not fresh:
        return False, ["No active security advisories.  âœ“"]
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `zed doctor` on the gateway host for details.")
