"""zed hooks â€” inspect and manage shell-script hooks.

Usage::

    zed hooks list
    zed hooks test <event> [--for-tool X] [--payload-file F]
    zed hooks revoke <command>
    zed hooks doctor

Consent records live under ``~/.zed/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.zed/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``zed hooks`` â€” dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: zed hooks {list|test|revoke|doctor}")
        print("Run 'zed hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.zed/config.yaml.")
        print("See `zed hooks --help` or")
    ...
    for event in sorted(by_event.keys()):
        ...
        for spec in by_event[event]:
            ...
            status = "âœ“ allowed" if is_approved else "âœ— not allowlisted"
            ...
            if is_approved:
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            f"      âš  script modified since approval "
                            f"(was {mtime_at}, now {mtime_now}) â€” "
                            f"run `zed hooks doctor` to re-validate"
                        )
        print()
...
# Synthetic kwargs matching the real invoke_hook() call sites â€” these are
...
# stdin a script sees under `zed hooks test` and `zed hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if result.get("timed_out"):
        print(f"      âœ— timed out after {result['elapsed_seconds']}s")
        return
    ...
    if parsed:
        print(f"      parsed (Zed wire shape): {json.dumps(parsed)}")
    else:
        print("      parsed: <none â€” hook contributed nothing to the dispatcher>")
...
def _cmd_doctor(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured â€” nothing to check.")
        return
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if shell_hooks.script_is_executable(spec.command):
        ...
        problems += 1
        print("      âœ— script missing or not executable "
              "(chmod +x the file, or fix the path)")

    # 2. Allowlist status
    entry = shell_hooks.allowlist_entry_for(spec.event, spec.command)
    if entry:
        print(f"      âœ“ allowlisted (approved {entry.get('approved_at', '?')})")
    else:
        problems += 1
        print("      âœ— not allowlisted â€” hook will NOT fire at runtime "
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            problems += 1
            print(f"      âš  script modified since approval "
                  f"(was {mtime_at}, now {mtime_now}) â€” review changes, "
                  f"then `zed hooks revoke` + re-approve to refresh")
        elif mtime_now and mtime_at and mtime_now == mtime_at:
            print("      âœ“ script unchanged since approval")

    # 4. Produces valid JSON for a synthetic payload â€” only when the entry
    # is already allowlisted.  Otherwise `zed hooks doctor` would execute
    ...
    if not entry:
        print("      â„¹ skipped JSON smoke test â€” not allowlisted yet. "
              "Approve the hook first (via TTY prompt or --accept-hooks), "
              "then re-run `zed hooks doctor`.")
        ...
        if result.get("timed_out"):
            ...
            if stdout:
                try:
                    json.loads(stdout)
                    print(f"      âœ“ produced valid JSON on synthetic payload "
                          f"(exit={rc}, {elapsed}s)")
                except json.JSONDecodeError:
                    problems += 1
                    print(f"      âœ— stdout was not valid JSON (exit={rc}, "
                          f"{elapsed}s): {_truncate(stdout, 120)}")
            else:
                print(f"      âœ“ ran clean with empty stdout "
                      f"(exit={rc}, {elapsed}s) â€” hook is observer-only")
