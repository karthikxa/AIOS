"""Kanban triage specifier â€” flesh out a one-liner into a real spec.

Used by ``zed kanban specify [task_id | --all]``. Takes a task that
lives in the Triage column (a rough idea, typically only a title), calls
the auxiliary LLM to produce:

  * A tightened title (optional â€” only replaces if the model proposes a
...
* This module intentionally mirrors ``zed_cli/goals.py`` â€” same aux
...
* The prompt is a short system + user pair. We ask for JSON with
  ...
  retry loop â€” one shot, keep cost bounded.
...
ZED_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("ZED_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Zed Agent board.
...
heading, in this order:
  ...
  **Goal** â€” one sentence, user-facing outcome.
  **Approach** â€” 2-5 bullets on how a worker should tackle it.
  **Acceptance criteria** â€” checklist of concrete, verifiable conditions.
  **Out of scope** â€” short list of things NOT to touch (omit if nothing
      obvious; never invent scope creep).

Rules:
  - Keep the tightened title close in meaning to the original idea â€” do
...
def _extract_json_blob(raw: str) -> Optional[dict]:
    """Lenient JSON extraction â€” tolerates fenced code blocks and
...
def _profile_author() -> str:
    """Mirror of ``zed_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("ZED_PROFILE")
...
def specify_task(
    ...
    *,
    ...
    error, malformed response) â€” those surface via ``ok=False`` so the
    ...
    except Exception as exc:  # pragma: no cover â€” import smoke test
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=ZED_KANBAN_SPECIFY_MAX_TOKENS,
            timeout=timeout or 120,
            extra_body=get_auxiliary_extra_body() or None,
        )
    except Exception as exc:
        logger.info(
            "specify: API call failed for %s (%s) â€” skipping",
            task_id, exc,
        )
        return SpecifyOutcome(
    ...
    if parsed is None:
        ...
        # Worst case the user edits afterward â€” still better than stranding
