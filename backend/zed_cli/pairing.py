...
Usage:
    zed pairing list              # Show all pending + approved users
    zed pairing approve <platform> <code>  # Approve a pairing code
    zed pairing revoke <platform> <user_id> # Revoke user access
    zed pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle zed pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: zed pairing {list|approve|revoke|clear-pending}")
        print("Run 'zed pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.zed/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'zed pairing list' to see pending codes.\n")
