"""Migrate Zed' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Zed' YAML and writes equivalent [mcp_servers.<name>]
  ...
  2. Queries codex's `plugin/list` for the openai-curated marketplace
     ...
     OpenClaw calls "migrate native codex plugins" â€” the YouTube-video-
...
What translates (MCP servers):
  Zed mcp_servers.<n>.command/args/env  â†’ codex stdio transport
  Zed mcp_servers.<n>.url/headers       â†’ codex streamable_http transport
  Zed mcp_servers.<n>.timeout           â†’ codex tool_timeout_sec
  Zed mcp_servers.<n>.connect_timeout   â†’ codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Zed-specific keys (sampling, etc.) â€” codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md â€” codex respects this file natively in its cwd. Zed' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Zed config.")
...
# Zed keys that codex's MCP schema doesn't support â€” dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Zed' sampling subsection â€” codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, zed_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Zed MCP server config to the codex inline-table dict
    ...
    if not isinstance(zed_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(zed_cfg.get("command"))
    has_url = bool(zed_cfg.get("url"))
    ...
    if has_command:
        # Stdio transport
        out["command"] = str(zed_cfg["command"])
        args = zed_cfg.get("args") or []
        if args:
            out["args"] = [str(a) for a in args]
        env = zed_cfg.get("env") or {}
        if env:
        ...
        out["url"] = str(zed_cfg["url"])
        headers = zed_cfg.get("headers") or {}
        if headers:
            out["http_headers"] = {str(k): str(v) for k, v in headers.items()}
        # Zed' transport: sse hint is informational; codex auto-negotiates
        if zed_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]

    # Timeouts
    if "timeout" in zed_cfg:
        try:
            out["tool_timeout_sec"] = float(zed_cfg["timeout"])
        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")
    if "connect_timeout" in zed_cfg:
        try:
            out["startup_timeout_sec"] = float(zed_cfg["connect_timeout"])
        except (TypeError, ValueError):
            skipped.append("connect_timeout (not numeric)")

    # Enabled flag (codex defaults to true so we only emit when explicitly false)
    if zed_cfg.get("enabled") is False:
        out["enabled"] = False
    ...
    for key in zed_cfg:
        ...
        elif key not in _KNOWN_ZED_KEYS:
            skipped.append(f"{key} (unknown Zed key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    We only emit strings, numbers, booleans, and tables of those â€” no nested
    ...
    if isinstance(value, str):
        ...
        # â€” passing them through would produce invalid TOML that codex
        # would refuse to load. Paths usually don't contain control
        # chars but env-var passthrough (ZED_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    Args:
        servers: dict of MCP server name â†’ translated codex inline-table
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Zed)")
    ...
    if default_permission_profile:
        ...
        # structured fields â€” not what we want.
        normalized = (
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Zed' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Zed' migrate has ever touched the file). When we
    ...
    headers â€” codex's strict TOML parser then refuses to load the file.
    ...
    invoke this strip when ``plugin/list`` succeeded â€” otherwise we'd lose
    ...
    for line in lines:
        ...
        # lstrip but are not headers â€” without this guard they would
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Zed
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    ([], error_message). Migration treats this as non-fatal â€” MCP
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="zed-migration")
    ...
    for marketplace in marketplaces:
        ...
        for plugin in plugins:
            ...
            # Cf. openclaw/openclaw#80815 â€” OpenClaw learned to gate
            ...
            if key in seen:
                continue
            seen.add(key)
            # Carry forward whatever 'enabled' codex reports â€” defaults to
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    macOS routes ``/tmp`` through ``/private/var/folders/<â€¦>/T`` which is
    what pytest's tempdir factory uses by default. If a ZED_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed zed-tools call fails silently once the directory is GC'd.

    We err on the side of refusing â€” losing a (very unlikely) real
    ``~/.zed`` symlink that happens to live under ``/private/var/folders``
...
def _build_zed_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Zed' own
    ...
    so a zed installed under /opt/, /usr/local/, or a venv all work.
    ZED_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # ZED_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_zed_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever ZED_HOME its launcher sets
    ...
    # â€” that would override the launcher's ZED_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("ZED_HOME", tmp_path) would
    ...
    zed_home = os.environ.get("ZED_HOME") or ""
    if zed_home and _looks_like_test_tempdir(zed_home):
        zed_home = ""
    if zed_home:
        env["ZED_HOME"] = zed_home
    # PYTHONPATH passes through so a worktree-launched zed finds the
    ...
    env["ZED_QUIET"] = "1"
    env["ZED_REDACT_SECRETS"] = env.get("ZED_REDACT_SECRETS", "true")
    ...
    if env:
        out["env"] = env
    # Generous timeouts â€” browser_navigate or delegate_task can take a
...
def migrate(
    ...
    *,
    ...
    expose_zed_tools: bool = True,
) -> MigrationReport:
    """Translate Zed mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        zed_config: full ~/.zed/config.yaml dict
    ...
    zed_servers = (zed_config or {}).get("mcp_servers") or {}
    if not isinstance(zed_servers, dict):
        report.errors.append(
            "mcp_servers in Zed config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in zed_servers.items():
    ...
    # Discover installed Codex curated plugins. Best-effort â€” never blocks
    ...
    # Inject Zed' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Zed for the tools codex
    # doesn't ship with â€” web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/zed_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_zed_tools:
        translated["zed-tools"] = _build_zed_tools_mcp_entry()
        if "zed-tools" not in report.migrated:
            report.migrated.append("zed-tools")
    ...
    if target.exists():
        ...
        # the entries we re-emit inside the managed block â€” producing
