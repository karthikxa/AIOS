"""PTY bridge for `zed dashboard` chat tab.

Wraps a child process behind a pseudo-terminal so its ANSI output can be
streamed to a browser-side terminal emulator (xterm.js) and typed
keystrokes can be fed back in.  The only caller today is the
``/api/pty`` WebSocket endpoint in ``zed_cli.web_server``.
...
* **Zero Node dependency on the server side.**  We use :mod:`ptyprocess`,
  ...
  to the same ``zed --tui`` binary it would launch from the CLI, so
...
# columns=131072) rather than a genuine ultrawide.  Lower bound is 1 â€” a
...
class PtyBridge:
        ...
        if not _PTY_AVAILABLE:
            if sys.platform.startswith("win"):
                raise PtyUnavailableError(
                    ...
                    "Zed Agent supports Windows only via WSL."
                )
    ...
    def resize(self, cols: int, rows: int) -> None:
        ...
        garbage window sizes â€” the motivating case is WSL2, where xterm.js
        ...
        leaving the TUI laid out for a one-row / absurdly-wide screen â€”
    ...
    def close(self) -> None:
        ...
        try:
            pgid = os.getpgid(self._proc.pid)  # windows-footgun: ok â€” POSIX-only module (imports fcntl/termios/ptyprocess at top)
        except Exception:
            pgid = None
        ...
        for sig in (signal.SIGHUP, signal.SIGTERM, signal.SIGKILL):  # windows-footgun: ok â€” POSIX-only module (imports fcntl/termios/ptyprocess at top)
            ...
            try:
                if pgid is not None:
                    os.killpg(pgid, sig)  # windows-footgun: ok â€” POSIX-only module (imports fcntl/termios/ptyprocess at top)
                else:
                    self._proc.kill(sig)
            except Exception:
                pass
