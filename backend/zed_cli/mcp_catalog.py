"""MCP catalog â€” curated, Nous-approved MCP servers shipped with the repo.
...
entries via ``zed mcp catalog`` or the interactive ``zed mcp picker``,
and install them with ``zed mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into zed-agent. Presence in the
...
- Manifests pin transport details (commands, args, refs). MCPs are never
  auto-updated; users explicitly re-run ``zed mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.zed/.env`` (the
...
from zed_constants import get_zed_home, get_optional_mcps_dir
...
from zed_cli.cli_output import prompt as _prompt_input
...
class InstallSpec:
    ...
    type: str  # "git"
    url: str
    ref: str  # commit/tag/branch â€” pinned, never floats
...
# â”€â”€â”€ Manifest loader â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Zed install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Zed understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Zed is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` â€” manifest_version is newer than this Zed
        understands. Update Zed to install this entry.
      - ``invalid`` â€” manifest is malformed in some other way (caught by
...
def _install_root() -> Path:
    ...
    root = get_zed_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.zed/mcp-installs/<name>`` and run
    ...
    if dest.exists():
        # Fresh checkout each install â€” manifest version is the source of truth,
        # so wipe + re-clone for determinism.
        print(color(f"  Removing existing install at {dest}", Colors.DIM))
        shutil.rmtree(dest)

    print(color(f"  Cloning {install.url} ({install.ref}) â†’ {dest}", Colors.CYAN))
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.zed/.env via save_env_value()."""
    collected: Dict[str, str] = {}
    for spec in specs:
        existing = get_env_value(spec.name)
        if existing:
            print(color(f"  âœ“ {spec.name} already set in .env", Colors.GREEN))
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    here â€” the fallback path in :func:`_apply_tool_selection` handles them.
    """
    servers = installed_servers()
    ...
    try:
        ...
        from zed_cli.mcp_config import _probe_single_server
...
def _write_tools_include(name: str, include: Optional[List[str]]) -> None:
    ...
    if include is None:
        # No filter â€” drop any existing tools block.
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      - If manifest declares ``tools.default_enabled`` â†’ apply directly.
      - Otherwise â†’ leave config with no filter (all on when reachable).
      - Either way, point the user at ``zed mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `zed mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `zed mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        # User unchecked everything; treat as "no tools" â€” write empty include
        # so the server is installed but contributes nothing until reconfigured.
        _write_tools_include(entry.name, [])
        print(color(
            f"  No tools selected. Run `zed mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        # Everything selected â€” clear filter for the cleanest config shape.
        ...
        # the user can re-run `zed mcp configure <name>` and unselect a
        # tool to switch back to include-mode.
        _write_tools_include(entry.name, None)
        print(color(
            f"  âœ“ All {len(probed)} tools enabled (no filter â€” new tools "
    ...
    print(color(
        f"  âœ“ {len(chosen_names)}/{len(probed)} tools enabled.",
        Colors.GREEN,
    ))
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `zed auth <provider>` flow. Surface guidance
            # here rather than auto-running it â€” keeps the catalog install
            # decoupled from provider-auth lifecycle.
            print(color(
                f"  This MCP uses {entry.auth.provider} OAuth. Run "
                f"`zed auth {entry.auth.provider}` if you have not "
    ...
    # â”€â”€ Preserve any prior user tool selection across reinstalls â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    from zed_cli.mcp_config import _save_mcp_server
    ...
    # â”€â”€ Probe + tool selection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    _apply_tool_selection(entry, prior_selection=prior_selection)

    print()
    print(color(
        f"  âœ“ Installed '{entry.name}' "
        f"({'enabled' if enable else 'disabled'}). "
        f"Start a new Zed session to load its tools.",
        Colors.GREEN,
    ))
