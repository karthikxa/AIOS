...
* **Push-to-talk** (``start_recording`` / ``stop_and_transcribe``) â€” single
  manually-bounded capture used when the caller drives the start/stop pair
  explicitly.
* **Continuous (VAD)** (``start_continuous`` / ``stop_continuous``) â€” mirrors
...
# ``_MOD_ALIASES`` table â€” the contract that removes the cross-runtime
...
# (``c-b``) for them â€” a silent fallback was preferred to a hard
...
# never fire push-to-talk â€” the same blocklist the TUI parser uses.
...
# lookup, and zed-ink reports Alt as ``key.meta`` on many terminals.
...
def normalize_voice_record_key_for_prompt_toolkit(raw: Any) -> str:
    ...
    * non-string / empty / typo'd / bare-char / multi-modifier / reserved
      ``ctrl+c|d|l`` â†’ documented default ``c-b``
    * single-char keys: ``ctrl+o`` â†’ ``c-o``
    * named keys: ``ctrl+space`` â†’ ``c-space`` (aliases collapse:
      ``ctrl+return`` â†’ ``c-enter``)
    * ``super`` / ``win`` / ``windows`` â†’ ``c-b`` (TUI-only modifiers â€”
    ...
    # in prompt_toolkit (a-c-r form) and zed-ink rejects them; collapse
    # to the documented default instead of silently diverging.
    if len(parts) > 2:
        return _DEFAULT_PT_KEY

    # Bare char / bare named key (no explicit modifier) â€” the CLI's
...
def format_voice_record_key_for_status(raw: Any) -> str:
    ...
    if normalized.startswith("c-"):
        ...
        # ``super+<key>`` / ``win+<key>`` â€” CLI won't bind them, but
...
def _debug(msg: str) -> None:
    """Emit a debug breadcrumb when ZED_VOICE_DEBUG=1.

    Goes to stderr so the TUI gateway wraps it as a gateway.stderr event,
    which createGatewayEventHandler shows as an Activity line â€” exactly
    ...
    broken stderr pipe must not kill the whole gateway â€” the main
    command pipe (stdin+stdout) is what actually matters.
    """
    if os.environ.get("ZED_VOICE_DEBUG", "").strip() != "1":
        return
    try:
...
def _play_beep(frequency: int, count: int = 1) -> None:
    ...
    Best-effort â€” sounddevice failures are silently swallowed so the
...
# â”€â”€ Push-to-talk state â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
_recorder = None
_recorder_lock = threading.Lock()

# â”€â”€ Continuous (VAD) state â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ TTS-vs-STT feedback guard â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# When TTS plays the agent reply over the speakers, the live microphone
# picks it up and transcribes the agent's own voice as user input â€” an
...
# â”€â”€ Push-to-talk API â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


def start_recording() -> None:
    """Begin capturing from the default input device (push-to-talk).

    Idempotent â€” calling again while a recording is in progress is a no-op.
    """
    global _recorder
...
def stop_and_transcribe() -> Optional[str]:
    ...
    # â€” matches cli.py:_voice_stop_and_transcribe's result.get("transcript").
...
# â”€â”€ Continuous (VAD) API â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def start_continuous(
    ...
    transcribing/cleaning up; otherwise returns True. Idempotent â€” calling while
    ...
    with _continuous_lock:
        if _continuous_active:
            _debug("start_continuous: already active â€” no-op")
            return True
        if _continuous_stopping:
            _debug("start_continuous: stop/transcribe in progress â€” busy")
            return False
    ...
    # CLI parity: single 880 Hz beep *before* opening the stream â€” placing
...
def stop_continuous(force_transcribe: bool = False) -> None:
    ...
    Idempotent â€” calling while not active is a no-op. If ``force_transcribe`` is
    ...
    if rec is not None:
        if force_transcribe and on_transcript:
            ...
            try:
                # cancel() (not stop()) discards buffered frames â€” the loop
    ...
    # Audible "recording stopped" cue (CLI parity: same 660 Hz Ã— 2 the
...
def _continuous_on_silence() -> None:
    """AudioRecorder silence callback â€” runs in a daemon thread.

    Stops the current capture, transcribes, delivers the text via
    ``on_transcript``, and â€” if the loop is still active â€” starts the
    ...
    with _continuous_lock:
        if not _continuous_active:
            _debug("_continuous_on_silence: loop inactive â€” abort")
            return
        rec = _continuous_recorder
        on_transcript = _continuous_on_transcript
        on_status = _continuous_on_status
    ...
    if rec is None:
        _debug("_continuous_on_silence: no recorder â€” abort")
        return
    ...
    # the VAD firing â€” tells us at a glance whether the mic was too quiet
    ...
    if wav_path:
        try:
            result = transcribe_recording(wav_path)
            # transcribe_recording returns {"success": bool, "transcript": str,
            # "error": str?} â€” NOT {"text": str}.  Using the wrong key silently
    ...
    with _continuous_lock:
        if not _continuous_active:
            # User stopped us while we were transcribing â€” discard.
            _debug("_continuous_on_silence: stopped during transcribe â€” no restart")
            return
        if transcript:
            _continuous_no_speech_count = 0
        else:
    ...
    if should_halt:
        _debug(f"_continuous_on_silence: {no_speech} silent cycles â€” halting")
        with _continuous_lock:
            _continuous_active = False
            _continuous_no_speech_count = 0
        if on_silent_limit:
    ...
    # loop becomes a feedback loop â€” the agent's spoken reply lands
...
# â”€â”€ TTS API â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


def speak_text(text: str) -> None:
    """Synthesize ``text`` with the configured TTS provider and play it.

    Mirrors cli.py:_voice_speak_response exactly â€” same markdown strip
    ...
    # Cancel any live capture before we open the speakers â€” otherwise the
    ...
    try:
        ...
        tts_text = re.sub(r'\[([^\]]+)\]\([^)]+\)', r'\1', tts_text)    # [text](url) â†’ text
