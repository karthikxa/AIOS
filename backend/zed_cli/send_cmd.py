"""CLI subcommand: ``zed send`` â€” pipe text from shell scripts to any
...
* No LLM, no agent loop â€” the subcommand just resolves arguments, reads the
...
* For platforms that send via bot token (Telegram, Discord, Slack, Signal,
  SMS, WhatsApp-CloudAPI, â€¦) no running gateway is required. The tool
  ...
  modes, â€¦) a live gateway is needed; the underlying tool surfaces that
  error to the caller.
* Exit codes follow the classic Unix convention:
    0 â€” delivery (or list) succeeded
    1 â€” delivery failed at the platform level
    2 â€” usage / argument / config error (argparse already uses 2)
"""
...
def _read_message_body(
    ...
    Returns ``None`` when nothing is available â€” callers must treat that as
    a usage error.
    """
    if positional:
        return positional
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"zed send: {file_path} is not a text file. --file reads the "
                ...
                f'  zed send --to telegram "MEDIA:{file_path}"\n'
                f'  zed send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  zed send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"zed send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
...
def _emit_result(
    ...
    *,
    ...
    try:
        ...
        # Shouldn't happen with the shared tool, but be defensive â€” pass the
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"zed send: {payload['error']}", file=sys.stderr)
            ...
            # Unknown shape â€” dump it so nothing is silently dropped.
            print(json.dumps(payload, indent=2))
    ...
    if payload.get("success"):
        return _SUCCESS_EXIT
    # Unknown / unexpected â€” treat as failure so scripts notice.
    return _FAILURE_EXIT
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    the send_message tool itself shows to the model â€” keeps the two surfaces
    identical.
    """
    try:
        ...
        print(f"zed send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"zed send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"zed send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `zed gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.zed/channel_directory.json.")
        return _SUCCESS_EXIT

    # Human display â€” when unfiltered, reuse the shared formatter the agent
...
def _load_zed_env() -> None:
    """Populate ``os.environ`` from ``~/.zed/.env`` AND bridge top-level
    ...
    startup that ``zed send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.zed/.env)`` â€” brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.zed/config.yaml`` into
    ...
    See ``gateway/run.py`` for the canonical version of this bridge â€” we
    intentionally reimplement the minimum needed here so ``zed send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.zed/.env and ~/.zed/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "zed send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  zed send --to telegram \"hello\"\n"
            "  zed send --to discord:#ops --file report.md\n"
            "  zed send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "zed send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `zed send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Zed "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.zed/.env + ~/.zed/config.yaml) â€” no LLM, "
        ...
        epilog=(
            ...
            "  zed send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
    ...
    # Legacy / convenience positional removed â€” use --to for clarity.
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `zed send --list telegram`.",
    )
