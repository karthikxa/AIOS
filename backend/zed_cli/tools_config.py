"""
Unified tool configuration for Zed Agent.

`zed tools` and `zed setup tools` both enter this module.
Select a platform â†’ toggle toolsets on/off â†’ for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.zed/config.yaml under
...
from zed_cli.nous_account import format_nous_portal_entitlement_message
...
# â”€â”€â”€ UI Helpers (shared with setup.py) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

from zed_cli.cli_output import (  # noqa: E402 â€” late import block
...
# â”€â”€â”€ Toolset Registry â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
CONFIGURABLE_TOOLSETS = [
    ...
    ("browser",         "ðŸŒ Browser Automation",       "navigate, click, type, scroll"),
    ("terminal",        "ðŸ’» Terminal & Processes",      "terminal, process"),
    ("file",            "ðŸ“ File Operations",           "read, write, patch, search"),
    ("code_execution",  "âš¡ Code Execution",            "execute_code"),
    ("vision",          "ðŸ‘ï¸  Vision / Image Analysis",  "vision_analyze"),
    ("video",           "ðŸŽ¬ Video Analysis",            "video_analyze (requires video-capable model)"),
    ("image_gen",       "ðŸŽ¨ Image Generation",          "image_generate"),
    ("video_gen",       "ðŸŽ¬ Video Generation",          "video_generate (text-to-video + image-to-video)"),
    ("x_search",        "ðŸ¦ X (Twitter) Search",        "x_search (requires xAI OAuth or XAI_API_KEY)"),
    ("moa",             "ðŸ§  Mixture of Agents",         "mixture_of_agents"),
    ("tts",             "ðŸ”Š Text-to-Speech",            "text_to_speech"),
    ("skills",          "ðŸ“š Skills",                    "list, view, manage"),
    ("todo",            "ðŸ“‹ Task Planning",             "todo"),
    ("memory",          "ðŸ’¾ Memory",                    "persistent memory across sessions"),
    ("context_engine",  "ðŸ§© Context Engine",            "runtime tools from the active context engine"),
    ("session_search",  "ðŸ”Ž Session Search",            "search past conversations"),
    ("clarify",         "â“ Clarifying Questions",      "clarify"),
    ("delegation",      "ðŸ‘¥ Task Delegation",           "delegate_task"),
    ("cronjob",         "â° Cron Jobs",                 "create/list/update/pause/resume/run, with optional attached skills"),
    ("homeassistant",    "ðŸ  Home Assistant",           "smart home device control"),
    ("spotify",          "ðŸŽµ Spotify",                  "playback, search, playlists, library"),
    ("discord",         "ðŸ’¬ Discord (read/participate)", "fetch messages, search members, create thread"),
    ("discord_admin",   "ðŸ›¡ï¸  Discord Server Admin",    "list channels/roles, pin, assign roles"),
    ("yuanbao",          "ðŸ¤– Yuanbao",                  "group info, member queries, DM"),
    ("computer_use",     "ðŸ–±ï¸  Computer Use (macOS)",     "background desktop control via cua-driver"),
]
...
def gui_toolset_label(label: str) -> str:
    ...
    Registry labels use ``<emoji> <title>``; plugin toolsets prefix with ``ðŸ”Œ``.
    CLI/TUI keeps the raw ``label`` â€” only HTTP APIs call this helper.
    """
    text = (label or "").strip()
    if not text:
        return text
...
# They're still in _ZED_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default â€” it's a niche, paid, slow feature. Users
# who want it opt in via `zed tools` â†’ Video Generation, which walks
...
# set â€” mirroring the HASS_TOKEN â†’ homeassistant auto-enable below. The
# `zed tools` â†’ X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    ``XAI_API_KEY``. Does NOT hit the network â€” only inspects the local
    ...
    """
    try:
        from zed_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `zed tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    already appears in ``CONFIGURABLE_TOOLSETS`` is skipped â€” bundled
    ...
    Without the dedupe, ``zed tools`` â†’ "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from zed_cli.plugins import discover_plugins, get_plugin_toolsets
        discover_plugins()  # idempotent â€” ensures plugins are loaded
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``zed tools`` checklist actually offers
    for ``platform``.
    ...
    time â€” ``kanban`` and other check_fn-gated toolsets, recovered platform
    composites, MCP server names â€” are NOT in this set because the checklist
    never shows them. Use this to scope the added/removed diff the UI prints,
    so ``zed tools`` never claims to add or remove a toolset the user was
    never given a checkbox for. The underlying config is unaffected â€” those
...
# Platform display config â€” derived from the canonical registry so every
# module shares the same data.  Kept as dict-of-dicts for backward
# compatibility with existing ``PLATFORMS[key]["label"]`` access patterns.
from zed_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
# â”€â”€â”€ Tool Categories (provider-aware configuration) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
TOOL_CATEGORIES = {
    "tts": {
        ...
        "providers": [
            ...
            {
                "name": "xAI TTS",
                "tag": "Grok voices â€” uses xAI Grok OAuth or XAI_API_KEY",
                "env_vars": [],
                "tts_provider": "xai",
                "post_setup": "xai_grok",
            },
            ...
            # Mistral Voxtral TTS â€” `mistralai` SDK lazy-installs on first use.
            {
    ...
    "web": {
        ...
        "setup_note": "A free DuckDuckGo search skill is also included â€” skip this if you don't need a premium provider.",
        "icon": "ðŸ”",
    ...
    "x_search": {
        ...
        "setup_note": (
            "Zed routes X searches through xAI's built-in x_search "
            "Responses tool. Both credential sources hit the same "
            "https://api.x.ai/v1/responses endpoint â€” pick whichever you "
        ...
        "providers": [
            {
                ...
                "badge": "subscription",
                "tag": "Browser login at accounts.x.ai â€” no API key required",
                "env_vars": [],
                "post_setup": "xai_grok",
            },
            {
        ...
        "providers": [
            {
                ...
                "tag": "PKCE OAuth â€” opens the setup wizard",
                "env_vars": [],
                "post_setup": "spotify",
            },
        ],
        ...
        "providers": [
            {
                ...
                "badge": "â˜… recommended Â· free Â· local",
                "tag": (
                    "macOS background computer-use via SkyLight SPIs â€” does "
                    "NOT steal your cursor or focus. Works with any model."
                ),
                "env_vars": [
                    # cua-driver reads HOME/TMPDIR from the process env, no
                    # extra keys required. ZED_CUA_DRIVER_VERSION is an
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "ZED_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "ZED_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "ZED_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "ZED_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "ZED_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# â”€â”€â”€ Post-Setup Hooks â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


def _cua_driver_cmd() -> str:
    """Return the cua-driver executable name/path, honoring non-empty overrides."""
    return os.environ.get("ZED_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def _pip_install(
    ...
    *,
    ...
    Strategy (in order):
    1. ``uv pip install`` if uv is on PATH â€” fast, doesn't need pip in the venv.
    2. ``python -m pip install`` â€” works on stdlib venvs.
    3. ``python -m ensurepip --upgrade`` then retry pip â€” covers ``uv venv``
    ...
    if uv_bin:
        try:
            ...
            if result.returncode == 0:
                return result
            # Fall through to pip â€” uv may have failed for an unrelated reason
...
def _check_cua_driver_asset_for_arch() -> bool:
    ...
    # x86_64 / Intel â€” probe the latest release for an architecture-specific
    ...
    try:
        ...
        # Network / API failure â€” proceed and let the installer handle it.
        pass
    return True
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=False`` â€” original post-setup behaviour: skip if already
      installed, install otherwise. Used by the toolset enable flow where
      we don't want to surprise the user with a network fetch.
    * ``upgrade=True`` â€” always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``zed update`` and
      by ``zed computer-use install --upgrade``.

    Returns True iff cua-driver is installed (or successfully refreshed)
    when the function returns. macOS-only â€” silently returns False on
    other platforms.
    """
    import platform as _plat
    import shutil
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS â€” `zed update` calls this for every
    ...
    # Not installed â†’ fresh install path (only when caller asked for it).
    if not binary and not upgrade:
        if not shutil.which("curl"):
            _print_warning("    curl not found â€” install manually:")
    ...
    # Already installed and caller didn't ask to upgrade â†’ just confirm.
    ...
    # upgrade=True path â€” refresh to the latest upstream release.
    if not shutil.which("curl"):
        _print_warning("    curl not found â€” cannot refresh cua-driver.")
        return bool(binary)
    ...
    if ok and before:
        try:
            ...
            if after and after != before:
                _print_success(f"    {driver_cmd} upgraded: {before} â†’ {after}")
            elif after:
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                _print_info("    IMPORTANT â€” grant macOS permissions now:")
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Zed process.")
            return True
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            # batch shims).  On POSIX npm_bin is the plain path â€” same
            # behaviour as before.
            result = subprocess.run(
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_zed_home()}/zed-agent && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in zed-agent directory)")
            return
        ...
        # use until the command timeout fires. Skip inside Docker â€” the
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.zed/config.yaml")
        ...
        # Run the full `zed auth spotify` flow â€” if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.zed/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from zed_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: zed auth spotify")
            return
        ...
        try:
            ...
            # User aborted the wizard, or OAuth failed â€” don't fail the
            # toolset enable; they can retry with `zed auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: zed auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_warning("    langfuse SDK install failed â€” run manually: uv pip install langfuse")
        ...
        try:
            from zed_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: zed plugins enable observability/langfuse")
        _print_info("    Restart Zed for tracing to take effect.")
        _print_info("    Verify: zed plugins list")
        ...
        # SuperGrok-tier OAuth bearer token (preferred â€” billed against the
        ...
        try:
            ...
            _print_info("    Run later: zed auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip â€” configure later via `zed auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                _print_success(
                    "    Logged in â€” xAI will use these OAuth credentials"
                )
            else:
                _print_warning(
                    "    xAI Grok OAuth login did not complete. "
                    "Run later: zed auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: zed auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``zed tools post-setup``
    ...
    for builder in (
        ...
        try:
            for prov in builder():
                ...
                if ps:
                    keys.add(ps)
        except Exception:  # pragma: no cover â€” defensive; plugins optional
            continue
    return keys


def run_post_setup_command(args) -> int:
    """``zed tools post-setup <key>`` â€” non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: zed tools post-setup <key>")
        return 2
...
# â”€â”€â”€ Platform / Toolset Helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    *,
    ...
    if toolset_names is None or not isinstance(toolset_names, list):
        ...
        if plat_info:
            ...
            # Plugin platform â€” derive toolset name from platform key
            default_ts = f"zed-{platform}"
    ...
    # has explicitly configured this platform â€” use direct membership.
    # This avoids the subset-inference bug where composite toolsets like
    # "zed-cli" (which include all _ZED_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[zed-cli, spotify]`` after enabling Spotify via ``zed
        ...
        # _DEFAULT_OFF_TOOLSETS only to the implicit expansion â€” anything the
        ...
        # No explicit config â€” fall back to resolving composite toolset names
        # (e.g. "zed-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # directly here, mirroring the HASS_TOKEN â†’ ``homeassistant`` rule
        # below: once you have working creds, you don't have to also click
        # through ``zed tools`` to flip the toolset on. Only fires when
        # the user has not yet saved an explicit toolset list â€” once they
        ...
        # platform-restricted toolsets â€” those are always opt-in even on
        ...
        # HASS_TOKEN, they've explicitly opted in â€” don't also strip it via
    ...
    # checklist or in a user-saved config.  Must run in BOTH branches â€”
    # otherwise saving via `zed tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"zed-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("zed-")}
    ...
    for ts_key, ts_def in TOOLSETS.items():
        ...
        # by agent/coding_context.py â€” not per-platform capabilities to recover.
    ...
    # unless the toolset is in _DEFAULT_OFF_TOOLSETS (e.g. spotify â€”
    # shipped as a bundled plugin but user must opt in via `zed tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `zed tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                # Explicitly listed in config â€” enabled
                enabled_toolsets.add(pts)
            elif pts in _DEFAULT_OFF_TOOLSETS:
                # Opt-in plugin toolset â€” stay off until user picks it
                continue
            elif pts not in known_for_platform:
                # New plugin not yet seen by zed tools â€” default enabled
                enabled_toolsets.add(pts)
    ...
    # Honor agent.disabled_toolsets from config.yaml â€” allows users to
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (zed-cli, zed-telegram, etc.)
    ...
    # Opening `zed tools` is the user's opt-in to reconfigure tools, so treat
...
# â”€â”€â”€ Token Estimation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _prompt_toolset_checklist(
    ...
    *,
    ...
    from zed_cli.curses_ui import curses_checklist
...
# â”€â”€â”€ Provider-Aware Configuration â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _plugin_image_gen_providers() -> list[dict]:
    ...
    plugin registry. Every image-gen backend is a plugin now â€” there
...
def _plugin_video_gen_providers() -> list[dict]:
    ...
    Mirrors ``_plugin_image_gen_providers`` exactly â€” every video backend
...
def _plugin_tts_providers() -> list[dict]:
    ...
    Issue #30398 â€” the ``register_tts_provider()`` plugin hook
    coexists alongside the 10 built-in TTS providers
    (``edge``/``openai``/``elevenlabs``/â€¦) and the
    ...
    are filtered out â€” even though the registry already rejects them
    ...
    for provider in providers:
        ...
        row = {
            ...
            "tag": schema.get("tag", ""),
            "env_vars": schema.get("env_vars", []),
            # Selecting this row writes ``tts.provider: <name>`` â€” the
...
def _visible_providers(
    ...
    *activates* the gateway once paid access is confirmed.
    ...
    # no paid access) get image gen but NOT video gen â€” the pool doesn't fund
    ...
    for provider in cat.get("providers", []):
        ...
        if (
            ...
            continue
        # Hide the managed video-gen row from pool-only users â€” their free tool
    ...
    # video_gen has NO hardcoded providers â€” every backend is a plugin.
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `zed tools` gets a silent no-op
...
def _post_setup_already_installed(post_setup_key: str) -> bool:
    ...
    if predicate is None:
        # No install-state check registered â†’ assume satisfied (don't
...
def _toolset_needs_configuration_prompt(
    ...
    *,
    ...
    if ts_key == "video_gen":
        ...
        # available â€” no in-tree fallback (every backend is a plugin).
        try:
            from agent.video_gen_registry import list_providers
            from zed_cli.plugins import _ensure_plugins_discovered
    ...
    *,
    ...
    if len(providers) == 1:
        ...
        for p in providers:
            ...
            tag = f" â€” {p['tag']}" if p.get("tag") else ""
            configured = ""
        ...
        # Add skip option
        provider_choices.append("Skip â€” keep defaults / configure later")
...
def _detect_active_provider_index(
    ...
    *,
    ...
    for i, p in enumerate(providers):
        if _is_provider_active(p, config, force_fresh=force_fresh):
            return i
        # Fallback: env vars present â†’ likely configured
        env_vars = p.get("env_vars", [])
...
# â”€â”€â”€ Image Generation Model Pickers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#
...
def _configure_imagegen_model(backend_name: str, config: dict) -> None:
    ...
    call even when stdin is not a TTY â€” curses_radiolist falls back to
...
def _plugin_image_gen_catalog(plugin_name: str):
    ...
    ``catalog_dict`` is shaped like the legacy ``FAL_MODELS`` table â€”
    ``{model_id: {"display", "speed", "strengths", "price", ...}}`` â€”
...
# â”€â”€â”€ Video Generation Model Pickers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    if managed_feature and managed_feature not in {"web", "tts", "browser"}:
        ...
        # User picked a non-gateway provider â€” find which category this
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    # model picker the CLI runs afterwards â€” model choice is a separate GUI
    # flow).
    plugin_name = provider.get("image_gen_plugin_name")
    if plugin_name:
        img_cfg = config.setdefault("image_gen", {})
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `zed model`).
    if managed_feature:
        from zed_cli.nous_subscription import (
    ...
    if not env_vars:
        ...
        if plugin_name:
            ...
            return
        # Plugin-registered video_gen provider â€” same flow, different
        # registry.
        video_plugin = provider.get("video_gen_plugin_name")
        if video_plugin:
...
def _configure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        choices = [
            "OpenRouter â€” uses Gemini",
            "OpenAI-compatible endpoint â€” base URL, API key, and vision model",
            "Skip",
        ]
        ...
        if idx == 0:
            ...
            if api_key and api_key.strip():
                ...
                # Save vision base URL to config (not .env â€” only secrets go there)
                _cfg = load_config()
    ...
    *,
    ...
    if not env_vars:
        ...
        if plugin_name:
            _select_plugin_image_gen_provider(plugin_name, config)
            return
        # Plugin-registered video_gen provider â€” same flow, different registry.
        video_plugin = provider.get("video_gen_plugin_name")
        if video_plugin:
            _select_plugin_video_gen_provider(video_plugin, config, use_gateway=bool(managed_feature))
            return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `zed tools` and `zed setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print(color("âš• Tool Summary", Colors.CYAN, Colors.BOLD))
        print()
        ...
        for pkey in enabled_platforms:
            ...
            if to_configure:
                ...
                for ts_key in to_configure:
                    label = next((l for k, l, _ in _get_effective_configurable_toolsets() if k == ts_key), ts_key)
                    print(color(f"    â€¢ {label}", Colors.DIM))
            ...
            save_config(config)
            print(color(f"  âœ“ Saved {pinfo['label']} tool configuration", Colors.GREEN))
            print()

        return

    # â”€â”€ Returning user: platform menu loop â”€â”€
    ...
    while True:
        ...
        if idx == _global_idx:
            ...
            if new_enabled != all_current:
                ...
                save_config(config)
                print(color("  âœ“ Saved configuration for all platforms", Colors.GREEN))
        ...
        if new_enabled != current_enabled:
            ...
            _save_platform_tools(config, pkey, new_enabled)
            save_config(config)
            print(color(f"  âœ“ Saved {pinfo['label']} configuration", Colors.GREEN))
        else:
    ...
    print()
    from zed_constants import display_zed_home
    print(color(f"  Tool configuration saved to {display_zed_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'zed' or gateway restart.", Colors.DIM))
    print()


# â”€â”€â”€ MCP Tools Interactive Configuration â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, zed mcp
        ...
        if len(chosen) == len(tools):
            # All tools enabled â€” clear filters (cleanest config shape; the
            ...
            # Drop any legacy exclude block â€” we\'re include-mode now.
            tools_cfg.pop("exclude", None)
    ...
    if any_changes:
        save_config(config)
        print()
        print(color("  âœ“ MCP tool configuration saved", Colors.GREEN))
    else:
        print(color("  No changes to MCP tools", Colors.DIM))


# â”€â”€â”€ Non-interactive disable/enable â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _print_tools_list(enabled_toolsets: set, mcp_servers: dict, platform: str = "cli"):
    ...
    for ts_key, label, _ in effective:
        if ts_key not in builtin_keys:
            continue
        status = (color("âœ“ enabled", Colors.GREEN) if ts_key in enabled_toolsets
                  else color("âœ— disabled", Colors.RED))
