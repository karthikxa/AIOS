#!/usr/bin/env python3
"""
Skills Hub CLI â€” Unified interface for the Zed Skills Hub.

Powers both:
  - `zed skills <subcommand>` (CLI argparse entry point)
...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    if len(exact) > 1:
        ...
        # so users can copy it for `zed skills install`.
    ...
    # No exact match â€” check if there are partial matches to suggest
    if results:
        c.print(f"[yellow]No exact match for '{name}'. Did you mean one of these?[/]")
        for r in results[:5]:
            c.print(f"  [cyan]{r.name}[/] â€” {r.identifier}")
        c.print()
        return ""
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.zed/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional â€” press Enter to install flat at ~/.zed/skills/<name>/)[/]"
        )
    try:
    ...
    if not _VALID_CATEGORY_RE.match(answer):
        c.print(f"[dim]Invalid category {answer!r} â€” installing flat.[/]")
        return ""
    return answer
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    if as_json:
        # Avoid Rich status spinner contaminating stdout â€” JSON consumers
    ...
    table = Table(title=f"Skills Hub â€” {len(results)} result(s)")
    ...
    # is part of the actual identifier â€” truncating it makes copy-paste
    # into `zed skills install` fail.
    ...
    c.print(table)
    c.print("[dim]Use: zed skills inspect <identifier> to preview, "
            "zed skills install <identifier> to install "
            "(--json for scripting)[/]\n")
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    # Per-source limits are generous â€” parallelism + 30s timeout cap prevents hangs.
    _TRUST_RANK = {"builtin": 3, "trusted": 2, "community": 1}
    # NOTE: when the centralized index is available, parallel_search_sources
    # skips the external API sources and serves everything from "zed-index".
    # That source MUST therefore carry a limit large enough to cover the whole
    # catalog, or browse silently caps the hub â€” it shipped at 50 (surfaced
    ...
    # Build header
    source_label = f"â€” {source}" if source != "all" else "â€” all sources"
    loaded_label = f"{total} skills loaded"
    if timed_out:
        loaded_label += f", {len(timed_out)} source(s) still loading"
    c.print(f"\n[bold]Skills Hub â€” Browse {source_label}[/]"
            f"  [dim]({loaded_label}, page {page}/{total_pages})[/]")
    if official_count > 0 and page == 1:
        c.print(f"[bright_cyan]â˜… {official_count} official optional skill(s) from Zed Team[/]")
    c.print()
    ...
    # The identifier is what you pass to `zed skills install`. Browse used
    ...
    for i, r in enumerate(page_items, start=start + 1):
        ...
        trust_label = "â˜… official" if r.source == "official" else r.trust_level
    ...
    nav_parts = []
    if page > 1:
        nav_parts.append(f"[cyan]--page {page - 1}[/] â† prev")
    if page < total_pages:
        nav_parts.append(f"[cyan]--page {page + 1}[/] â†’ next")
    ...
    if timed_out:
        c.print(f"  [yellow]âš¡ Slow sources skipped: {', '.join(timed_out)} "
                f"â€” run again for cached results[/]")

    c.print("[dim]Tip: 'zed skills inspect <identifier>' to preview, "
            "'zed skills install <identifier>' to install, "
            "'zed skills search <query>' to search deeper[/]\n")
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    if bundle.source == "url" and (not bundle.name or bundle_meta.get("awaiting_name")):
        if name_override and _is_valid_installed_skill_name(name_override):
            ...
            # prompt â€” emit an actionable error.
            ...
            c.print(
                ...
                f"  [bold]zed skills install {url} --name <your-name>[/]\n\n"
                "[dim]Or ask the SKILL.md's author to add a `name:` field to "
                "its YAML frontmatter.[/]\n"
            )
            return
        else:
            # Interactive TTY â€” prompt.
            url = bundle_meta.get("url") or identifier
    ...
    # caller didn't specify one (TTY only â€” non-interactive installs fall
    ...
    # Confirm with user â€” show appropriate warning based on source
    # skip_confirm bypasses the prompt (needed in TUI mode where input() hangs)
    if not force and not skip_confirm:
        c.print()
        if bundle.source == "official":
            c.print(Panel(
                "[bold bright_cyan]This is an official optional skill maintained by Zed Team.[/]\n\n"
                "It ships with zed-agent but is not activated by default.\n"
                "Installing will copy it to your skills directory where the agent can use it.\n\n"
                f"Files will be at: [cyan]{display_zed_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
    ...
    # metadata.zed.blueprint block, it is a runnable automation. Register it as
    # a Suggested Cron Job rather than auto-scheduling â€” installing never
    ...
    try:
        ...
        if spec is not None:
            ...
            if registered is not None:
                ...
                c.print(
                    "[dim]Added to your suggestions â€” run[/] [bold]/suggestions[/] "
                    "[dim]to schedule or dismiss it.[/]\n"
                )
            else:
                # Dropped: already offered/dismissed (latched) or the pending
                # list is at its cap. Say so instead of silently doing nothing â€”
                ...
                c.print(
                    ...
                    "list is full â€” run [bold]/suggestions[/] to review)."
                )
                c.print(
                    "[dim]You can still schedule it any time by asking the agent "
                    "or via[/] [bold]zed cron add[/][dim].[/]\n"
                )
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="zed skills install <id> to install"))
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    page_size = max(1, min(page_size, 100))
    _TRUST_RANK = {"builtin": 3, "trusted": 2, "community": 1}
    # "zed-index" must carry a high limit: when the index is available the
    ...
    _PER_SOURCE_LIMIT = {"zed-index": 5000, "official": 100, "skills-sh": 100,
    ...
    # source-skip logic â€” querying zed-index AND the external APIs at once
...
def do_list(source_filter: str = "all",
    ...
    config â€” ``zed -p <profile> skills list`` reads that profile's
    ``skills.disabled`` list because ``-p`` swaps ``ZED_HOME`` at process
    ...
    if enabled_only:
        summary += f" â€” {enabled_count} enabled shown"
    else:
        summary += f" â€” {enabled_count} enabled, {disabled_count} disabled"
    summary += "[/]\n"
    c.print(summary)
...
def do_audit(name: Optional[str] = None, console: Optional[Console] = None,
    ...
    files (review aid only â€” not a security gate; skills_guard.py verdicts
    are unchanged).
    """
    ...
    for entry in targets:
        ...
        if not skill_path.exists():
            c.print(f"[yellow]Warning:[/] {entry['name']} â€” path missing: {entry['install_path']}")
            continue
...
def do_list_modified(console: Optional[Console] = None,
    ...
    """List bundled skills the user has edited (which `zed update` keeps)."""
    ...
    if not modified:
        c.print("[dim]No user-modified bundled skills â€” everything tracks upstream.[/]\n")
        return

    c.print(f"\n[bold]{len(modified)} user-modified bundled skill(s)[/] "
            "[dim](kept as-is by `zed update`):[/]")
    for entry in modified:
        c.print(f"  [yellow]~[/] {entry['name']}")
    c.print()
    c.print("[dim]See changes:   zed skills diff <name>[/]")
    c.print("[dim]Resume updates: zed skills reset <name>          (keep your copy, re-baseline)[/]")
    c.print("[dim]Revert to stock: zed skills reset <name> --restore[/]\n")
...
def do_diff(name: str, console: Optional[Console] = None) -> None:
    ...
    c.print()
    c.print(f"[dim]Revert with: zed skills reset {name} --restore[/]\n")
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: zed skills tap add owner/repo\n")
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: zed skills tap remove owner/repo\n")
            return
        if mgr.remove(repo):
    ...
    if target == "github":
        if not repo:
            c.print("[bold red]Error:[/] --repo required for GitHub publish.\n"
                    "Usage: zed skills publish <path> --to github --repo owner/repo\n")
            return

        auth = GitHubAuth()
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_zed_home()}/.env or run 'gh auth login'.\n")
            return
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via Zed Skills Hub.\n\n"
                        f"This skill was scanned by the Zed Skills Guard before submission.",
...
def skills_command(args) -> None:
    """Router for `zed skills <subcommand>` â€” called from zed_cli/main.py."""
    ...
    if action == "browse":
        ...
        if snap_action == "export":
            ...
            _console.print("Usage: zed skills snapshot [export|import]\n")
    elif action == "tap":
        tap_action = getattr(args, "tap_action", None)
        repo = getattr(args, "repo", "") or getattr(args, "name", "")
        if not tap_action:
            _console.print("Usage: zed skills tap [list|add|remove]\n")
            return
        do_tap(tap_action, repo=repo)
    else:
        _console.print("Usage: zed skills [browse|search|install|inspect|list|list-modified|diff|check|update|audit|uninstall|reset|opt-out|opt-in|publish|snapshot|tap]\n")
        _console.print("Run 'zed skills <command> --help' for details.\n")
...
def handle_skills_slash(cmd: str, console: Optional[Console] = None) -> None:
    ...
    if action == "browse":
        ...
        # Always skip confirmation â€” the user typing the command is implicit consent.
        skip_confirm = True
        force = "--force" in args
        ...
        invalidate_cache = "--now" in args
        # Slash commands can't prompt â€” --restore in slash mode is implicit consent.
