"""CLI for the Zed Kanban board â€” ``zed kanban â€¦`` subcommand.

Exposes the full Kanban command surface documented in the design spec
(``docs/zed-kanban-v1-spec.pdf``).  All DB work is delegated to
``kanban_db``.  This module adds:
  ...
  * A short shared helper that parses a single slash-style string
    (used by ``/kanban â€¦`` in CLI and gateway) and forwards it to the
    argparse surface.
"""
...
from zed_cli import kanban_swarm as ks
from zed_cli.profiles import get_active_profile_name, get_profile_dir, seed_profile_skills
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this ZED_HOME and its
    ...
    Used by ``zed kanban create`` (and callers) to warn when a task
    will sit in ``ready`` because nothing is there to pick it up.
    Defensive against import failures and config-read errors â€” if the
    ...
    try:
        ...
        return (True, "")  # can't probe â€” silent
    try:
        pid = get_running_pid()
    except Exception:
        return (True, "")  # probe errored â€” silent
    ...
    try:
        ...
        dispatch_on = True  # can't tell â€” assume default
    ...
    if pid and not dispatch_on:
        return (
            ...
            "config.yaml â€” the task will sit in 'ready' until you flip it "
            "back on and restart the gateway, OR run the legacy "
            "standalone daemon (`zed kanban daemon --force`)."
        )
    return (
        False,
        "No gateway is running â€” the task will sit in 'ready' until you "
        "start it. Run:\n"
        "    zed gateway start\n"
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    # ZED_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `zed kanban boards switch <slug>` or the "
            "ZED_KANBAN_BOARD env var). Use `zed kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_boards = sub.add_parser(
        ...
        description=(
            ...
            "loop â€” tasks on one board cannot collide with tasks on "
    ...
    p_create.add_argument("--triage", action="store_true",
                          help="Park in triage â€” a specifier will flesh out the spec and promote to todo")
    ...
    p_create.add_argument("--max-retries", type=int, default=None,
                          ...
                          help="Per-task override for the consecutive-failure "
                               "circuit breaker. Trip on the Nth failure â€” "
    ...
    p_swarm = sub.add_parser(
        "swarm",
        help="Create a Kanban Swarm v1 graph (parallel workers â†’ verifier â†’ synthesizer)",
    )
    ...
    p_list.add_argument("--mine", action="store_true",
                        help="Filter by $ZED_PROFILE as assignee")
    ...
    p_comment.add_argument("--author", default=None,
                           help="Author name (default: $ZED_PROFILE or 'user')")
    ...
    p_unblock.add_argument(
        "--reason",
        default=None,
        help="Optional reason/note â€” recorded as a comment before unblocking. Quote multi-word reasons.",
    )
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED â€” dispatcher now runs in the gateway. Use `zed gateway start`.",
    )
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.zed/profiles/ and current assignees on the board)",
    )
    ...
    # --- specify --- (triage â†’ todo via auxiliary LLM)
    ...
    p_specify.add_argument(
        ...
        help="Author name recorded on the audit comment "
             "(default: $ZED_PROFILE or 'specifier')",
    )
    p_specify.add_argument(
        "--json",
        action="store_true",
    ...
    # --- decompose --- (triage â†’ fan-out via auxiliary LLM + orchestrator)
    ...
    p_decompose.add_argument(
        ...
        default=None,
        help="Author name recorded on the audit comment "
             "(default: $ZED_PROFILE or 'decomposer')",
    )
    p_decompose.add_argument(
        "--json",
        action="store_true",
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``zed kanban â€¦`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: zed kanban <action> [options]\n"
                "Run 'zed kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0
    ...
    # env-var pin for the duration of this call. Using ZED_KANBAN_BOARD
    # (rather than threading `board=` through 50+ kb.connect() sites)
    # keeps the patch small and inherits the exact same resolution the
    # dispatcher uses for workers â€” consistency is a feature here.
    ...
    if board_override:
        ...
        if not normed:
            ...
            return 2
        # Boards other than 'default' must already exist â€” typoed slugs
        # would otherwise silently create an empty board.
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                f"kanban: board {normed!r} does not exist. "
                f"Create it with `zed kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
    ...
    # ZED_HOME. Previously only `init` and `daemon` triggered
...
def _profile_author() -> str:
    ...
    for env in ("ZED_PROFILE_NAME", "ZED_PROFILE"):
...
# ---------------------------------------------------------------------------
# Boards management (zed kanban boards â€¦)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``zed kanban boards <action>``.
    ...
    DB, so a fresh ZED_HOME that has never called ``kanban init``
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    # Human table: marker (â€¢) for current, slug, display name, counts.
    if not boards:
        print("(no boards â€” create one with `zed kanban boards create <slug>`)")
        return 0
    print(f"{'':2s}  {'SLUG':24s}  {'NAME':28s}  COUNTS")
    for b in boards:
        marker = "â—" if b["is_current"] else " "
        counts = b["counts"] or {}
        counts_str = (
    ...
    if len(boards) > 1:
        print("Switch boards with `zed kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `zed kanban boards switch {meta['slug']}` to make it current.")
    return 0


def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `zed kanban boards delete <slug>` (alias), the
    ...
    if res["action"] == "archived":
        print(f"Board {res['slug']!r} archived â†’ {res['new_path']}")
...
def _parse_duration(val) -> Optional[int]:
    """Parse ``30s`` / ``5m`` / ``2h`` / ``1d`` or a raw integer â†’ seconds.
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    # the kanban dispatcher can use them without a separate `zed profile
    # create` step.  This is best-effort â€” a missing or broken profile is
    ...
    # dispatcher doesn't need to enumerate â€” we just pass the name
    # through to `zed -p <name>`.
    try:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.zed/profiles/.")
        print("Create one with `zed -p <name> setup` before assigning tasks.")
    print()
    print("Next step: start the gateway so ready tasks actually get picked up.")
    print("  zed gateway start")
    print()
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees â€” create a profile with `zed -p <name> setup`)")
        return 0
    # Header
...
def _cmd_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        # present. Only warn on ready+assigned tasks â€” triage/todo are
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    # never see this â€” the feature stays invisible until you opt in.
    try:
    ...
    if len(all_boards) > 1:
        ...
        print(
            f"Board: {current} "
            f"({other_count} other board{'s' if other_count != 1 else ''} â€” "
            f"`zed kanban boards list`)\n"
        )
    if not tasks:
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    # Diagnostics section â€” surface active distress signals at the top
    ...
    if diags:
        sev_marker = {"warning": "âš ", "error": "!!", "critical": "!!!"}
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``zed kanban show <id>``
    ...
    if runs:
        ...
        for r in runs:
            ...
            if r.summary:
                print(f"        â†’ {r.summary.splitlines()[0][:160]}")
            if r.error:
...
def _cmd_reassign(args: argparse.Namespace) -> int:
    ...
    if not ok:
        print(
            ...
            f"(unknown id, or still running â€” pass --reclaim to release first)",
            file=sys.stderr,
        )
        return 1
    print(
...
def _cmd_diagnostics(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        ...
        # Map task_id â†’ title/status/assignee for the table output.
        meta: dict[str, dict] = {}
        if diags_by_task:
...
def _worker_run_id_for(task_id: str) -> Optional[int]:
    if os.environ.get("ZED_KANBAN_TASK") != task_id:
        return None
    raw = os.environ.get("ZED_KANBAN_RUN_ID")
    if not raw:
        return None
    try:
        return int(raw)
...
def _cmd_complete(args: argparse.Namespace) -> int:
    ...
    # copy-pasted identically across N runs â€” almost always a footgun.
...
def _cmd_dispatch(args: argparse.Namespace) -> int:
    ...
    if res.skipped_nonspawnable:
        print(
            f"Skipped (non-spawnable assignee â€” terminal lane, OK): "
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    """Deprecated â€” the dispatcher now runs inside the gateway.
    ...
    # casually â€” intentional.
    if not getattr(args, "force", False):
        print(
            "zed kanban daemon: DEPRECATED â€” the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            "    zed gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
    ...
    # Legacy path â€” same logic as before, kept behind --force.
    ...
    # credential loss â€” cases where the per-task circuit breaker auto-blocks
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`zed kanban list --status ready` / "
                    f"`zed kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        """Cheap probe â€” is there at least one ready+assigned+unclaimed
        task whose assignee maps to a real Zed profile (i.e. one the
        ...
        terminals via ``claim_task`` directly â€” those are correctly idle
...
def _cmd_log(args: argparse.Namespace) -> int:
    ...
    if content is None:
        print(f"(no log for {args.task_id} â€” task may not have spawned yet)",
              file=sys.stderr)
        return 1
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    for tid in ids:
        ...
        if want_json:
            ...
            title_suffix = (
                f" â€” retitled: {outcome.new_title!r}"
                if outcome.new_title
                else ""
            )
            print(f"Specified {outcome.task_id} â†’ todo{title_suffix}")
        else:
            print(
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from zed_cli import kanban_decompose as decomp
    ...
    for tid in ids:
        ...
        if want_json:
            ...
            if outcome.fanout and outcome.child_ids:
                ...
                print(
                    f"Decomposed {outcome.task_id} â†’ {len(outcome.child_ids)} "
                ...
                print(
                    f"Specified {outcome.task_id} â†’ todo "
...
_SLASH_KANBAN_HELP = """\
**/kanban** â€” manage the shared task board.

Common subcommands:
  `list` (alias `ls`)   List tasks on the current board
  `show <id>`           Task details + comments + events
  `stats`               Per-status / per-assignee counts
  `create <title>â€¦`     Create a task (auto-subscribes you to events)
  `comment <id> <msg>`  Append a comment
  `complete <id>â€¦`      Mark task(s) done
...
def run_slash(rest: str) -> str:
    """Execute a ``/kanban â€¦`` string and return captured stdout/stderr.
    ...
    # the kanban_parser back out â€” then drive it directly so usage/error
    ...
    try:
        ...
        # Help dump (exit 0) â†’ return the captured help text directly.
        if exc.code in {0, None} and out:
            return out
        body = err or out
        return f"âš  /kanban usage error\n{body}" if body else "âš  /kanban usage error"
