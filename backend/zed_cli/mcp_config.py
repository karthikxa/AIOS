"""
MCP Server Management CLI â€” ``zed mcp`` subcommand.

Implements ``zed mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.zed/config.yaml under the ``mcp_servers`` key.
"""
...
from zed_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_zed_home,  # noqa: F401 â€” used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from zed_cli.cli_output import prompt as _shared_prompt
...
def _strip_bearer_prefix(token: str) -> str:
    ...
    server receives ``Bearer Bearer <jwt>`` â†’ 401. Normalize on save. (#37792)
    """
...
# â”€â”€â”€ Discovery (temporary connect) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.zed/.env`` into ``os.environ`` and recursively interpolate any
    ...
    auth-requiring servers (e.g. n8n) returned 401 â€” while runtime tool
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``zed mcp login`` to distinguish a genuine authentication
    ...
    """
    try:
        from tools.mcp_oauth import ZedTokenStorage
        return ZedTokenStorage(name).has_cached_tokens()
...
# â”€â”€â”€ zed mcp add â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def cmd_mcp_add(args):
    """Add a new MCP server with discovery-first tool selection."""
    name = args.name
    url = getattr(args, "url", None)
    # Read from `mcp_command` (set by --command via explicit dest) â€” see
    # mcp_add_p.add_argument("--command", dest="mcp_command", ...) in
    # zed_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  zed mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  zed mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  zed mcp add myserver --preset mypreset')
        return
    ...
    if url and auth_type == "oauth":
        ...
        try:
            ...
            if oauth_auth:
                ...
                _warning("OAuth setup failed â€” MCP SDK auth module not available")
        ...
        if not oauth_ok:
            ...
            if _confirm("Continue without authentication?", default=True):
                # Don't store auth: oauth â€” server doesn't support it
                pass
            else:
                _info("Cancelled.")
                return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_zed_home()}/.env as {env_key}")
    ...
    # â”€â”€ Discovery: connect and list tools â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: zed mcp test " + name)
        return
    ...
    # â”€â”€ Tool selection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    if choice in {"n", "no"}:
        _info("Cancelled â€” server not saved.")
        return
    ...
    if choice in {"s", "select"}:
        ...
        labels = [f"{t[0]}  â€”  {t[1]}" for t in tools]
        ...
        if not chosen:
            _info("No tools selected â€” server not saved.")
            return
        ...
        # Enable all (no filter needed â€” default behaviour)
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_zed_home()}/config.yaml ({tool_count}/{total} tools enabled)")
        _info("Start a new session to use these tools.")


# â”€â”€â”€ zed mcp remove â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def cmd_mcp_remove(args):
    ...
    # Clean up OAuth tokens if they exist â€” route through MCPOAuthManager so
    # any provider instance cached in the current process (e.g. from an
    # earlier `zed mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
# â”€â”€â”€ zed mcp list â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  zed mcp add <name> --url <endpoint>')
        _info('  zed mcp add <name> --command <cmd> --args <args...>')
        print()
        return
    ...
    for name, cfg in servers.items():
        ...
        status = color("âœ“ enabled", Colors.GREEN) if enabled else color("âœ— disabled", Colors.DIM)
...
# â”€â”€â”€ zed mcp test â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def cmd_mcp_test(args):
    ...
    # Show transport info
    if "url" in cfg:
        _info(f"Transport: HTTP â†’ {cfg['url']}")
    else:
        cmd = cfg.get("command", "?")
        _info(f"Transport: stdio â†’ {cmd}")
...
# â”€â”€â”€ zed mcp login â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def cmd_mcp_login(args):
    ...
    if not url:
        _error(f"Server '{name}' has no URL â€” not an OAuth-capable server")
        return
    if server_config.get("auth") != "oauth":
        _error(f"Server '{name}' is not configured for OAuth (auth={server_config.get('auth')})")
        _info("Use `zed mcp remove` + `zed mcp add` to reconfigure auth.")
        return
    ...
    try:
        ...
        # OAuth flow never completed â€” e.g. dynamic client registration
        # 400'd because the provider doesn't support RFC 7591. Reporting
        # "Authenticated â€” N tools" in that case is a false success: every
        ...
        if not _oauth_tokens_present(name):
            _warning(
                "Server responded, but no OAuth token was obtained â€” "
            ...
            print()
            _info("Then re-run `zed mcp login " + name + "`.")
            return
        if tools:
            _success(f"Authenticated â€” {len(tools)} tool(s) available")
        else:
...
# â”€â”€â”€ zed mcp configure â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'zed mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
    ...
    labels = [f"{t[0]}  â€”  {t[1]}" for t in all_tools]
    ...
    if len(chosen) == total:
        # All selected â†’ remove include/exclude (register all)
...
def mcp_command(args):
    """Main dispatcher for ``zed mcp`` subcommands."""
    ...
    if action == "picker":
        from zed_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from zed_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from zed_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # No subcommand â€” drop the user into the catalog picker. This is the
        # "try enabling and it flows you into setup" UX matching `zed plugin`.
        from zed_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("zed mcp                                    Open the catalog picker (default)")
        _info("zed mcp catalog                            List Nous-approved MCPs")
        _info("zed mcp install <name>                     Install a catalog MCP")
        _info("zed mcp serve                              Run as MCP server")
        _info("zed mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("zed mcp add <name> --command <cmd>         Add a stdio server")
        _info("zed mcp add <name> --preset <preset>       Add from a known preset")
        _info("zed mcp remove <name>                      Remove a server")
        _info("zed mcp list                               List configured servers")
        _info("zed mcp test <name>                        Test connection")
        _info("zed mcp configure <name>                   Toggle tools")
        _info("zed mcp login <name>                       Re-authenticate OAuth")
        print()
