...
# xAI's public API is OpenAI-compatible for the endpoints Zed commonly
# uses. The Responses endpoint is included because Zed' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Zed-managed OAuth credentials."""

    auth_hint = "zed auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`zed auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`zed auth reset xai-oauth` or re-authenticate with "
                    "`zed auth add xai-oauth --type oauth`."
                )
        ...
        *,
        ...
        with self._lock:
            ...
            if status_code == 429:
                ...
                # has no other key to offer â€” the 429 will flow back to the client.
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `zed auth add xai-oauth --type oauth`."
            )
