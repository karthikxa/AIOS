"""CLI handlers for the ``zed proxy`` subcommand."""
...
from zed_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "zed proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"zed auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Zed proxy for {adapter.display_name}\n"
        f"  Listening on:  http://{host}:{port}/v1\n"
        f"  Forwarding to: (resolved per-request from your subscription)\n"
        f"  Use any bearer token in the client â€” the proxy attaches your real credential.\n"
        f"\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Zed proxy upstream adapters\n")
    for name in sorted(ADAPTERS):
        adapter = get_adapter(name)
        if not adapter.is_authenticated():
            print(f"  [{name:8s}] {adapter.display_name} â€” not logged in")
            continue
        try:
            cred = adapter.get_credential()
        except Exception as exc:
            print(
                f"  [{name:8s}] {adapter.display_name} â€” credentials need attention "
                f"({exc})"
            )
            continue
        expires = f" (bearer expires {cred.expires_at})" if cred.expires_at else ""
        print(f"  [{name:8s}] {adapter.display_name} â€” ready{expires}")
    print(
        "\nStart the proxy with: zed proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy_list_providers(args: Any) -> int:
    ...
    for name in sorted(ADAPTERS):
        adapter = get_adapter(name)
        print(f"  {name}  â€” {adapter.display_name}")
    return 0


def cmd_proxy(args: Any) -> int:
    """Dispatch ``zed proxy <subcommand>``."""
    ...
    # No subcommand â†’ print short help.
    print(
        "zed proxy â€” local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  zed proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
