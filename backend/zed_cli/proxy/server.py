...
def _filter_response_headers(headers) -> dict:
    ...
    for key, value in headers.items():
        if key.lower() in _HOP_BY_HOP_HEADERS:
            continue
        # aiohttp recomputes Content-Encoding/Content-Length on stream â€” let it.
...
def create_app(adapter: UpstreamAdapter) -> "web.Application":
    ...
    if not AIOHTTP_AVAILABLE:
        raise RuntimeError(
            "aiohttp is required for `zed proxy`. Install with: "
            "pip install 'zed-agent[messaging]' or `pip install aiohttp`."
        )
    ...
    async def handle_proxy(request: "web.Request") -> "web.StreamResponse":
        ...
        # Forward body verbatim. Read into memory once â€” request bodies for
    ...
    # Catch-all under /v1 â€” forwards if the path is allowed.
    ...
    if shutdown_event is None:
        ...
        for sig in (signal.SIGINT, signal.SIGTERM):
            try:
                ...
                # Windows / restricted environments â€” Ctrl+C will still
