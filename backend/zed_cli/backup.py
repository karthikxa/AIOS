"""
Backup and import commands for zed CLI.

`zed backup` creates a zip archive of the entire ~/.zed/ directory
(excluding the zed-agent repo and transient files).

`zed import` restores from a backup zip, overlaying onto the current
ZED_HOME root.
"""
...
from zed_constants import get_default_zed_root, get_zed_home, display_zed_home
...
# ``zed-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/zed-agent/``
...
# ZED_HOME gets walked file-by-file, ballooning a backup to hundreds of
# thousands of entries that crawl for hours â€” the exact "backup stuck for
...
_EXCLUDED_DIRS = {
    "zed-agent",     # the codebase repo â€” re-clone instead
    "__pycache__",      # bytecode caches â€” regenerated on import
    ".git",             # nested git dirs (profiles shouldn't have these, but safety)
    "node_modules",     # js deps â€” reinstalled on demand
    "backups",          # prior auto-backups â€” don't nest backups exponentially
    "checkpoints",      # session-local trajectory caches â€” regenerated per-session,
                        # session-hash-keyed so they don't port to another machine anyway
    # Python dependency trees (plugin / MCP-server venvs under ZED_HOME) â€”
    # regenerated by reinstalling; never irreplaceable state.
    ".venv",
    "venv",
    "site-packages",
    # Tool / build caches â€” all regeneratable.
    ".cache",
    ".tox",
    ".nox",
    ".pytest_cache",
...
_EXCLUDED_SUFFIXES = (
    ".pyc",
    ".pyo",
    # SQLite sidecar files â€” the backup takes a consistent snapshot of ``*.db``
...
# File names that ``zed import`` must never overwrite, matched by basename so
...
# machine or container the backup was taken on* â€” PIDs in a dead process
...
#     stuck "starting"/"cooking", disconnecting it from the Zed Portal
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to zed root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``zed-agent`` only matches at the root level (first component).
        # Nested directories with the same name â€” e.g.
        # ``skills/autonomous-ai-agents/zed-agent/`` â€” must be preserved.
        if part == "zed-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside ZED_HOME.
...
def _safe_copy_db(src: Path, dst: Path) -> bool:
    ...
    Handles WAL mode â€” produces a consistent snapshot even while
...
def run_backup(args) -> None:
    """Create a zip backup of the Zed home directory."""
    zed_root = get_default_zed_root()

    if not zed_root.is_dir():
        print(f"Error: Zed home directory not found at {zed_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"zed-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"zed-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_zed_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(zed_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(zed_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``zed-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "zed-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(zed_root)
    ...
    print(f"\nRestore with: zed import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Zed backup.
    ...
    # Look for telltale files that a zed home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Zed backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.zed/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a zed dir name
        if prefix in {".zed", "zed"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_zed_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (zed_root / "config.yaml").exists()
        has_env = (zed_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Zed configuration.")
        ...
        for member in members:
            ...
            # Zed Portal. Matched by basename so both the root profile and
            ...
            try:
                target.resolve().relative_to(zed_root.resolve())
            except ValueError:
        ...
        profiles_dir = zed_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # zed_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: zed profile list  (after installing zed)")

        # Guidance
        print()
        if not (zed_root / "zed-agent").is_dir():
            print("Note: The zed-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: zed update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  zed -p {pname} gateway install")

        print("Done. Your Zed configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and zed backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ZED_HOME).
...
# â€” `zed update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(zed_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(zed_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside ZED_HOME. Kept in sync with
...
def _count_cron_jobs(path: Path) -> Optional[int]:
    ...
    Returns:
        ...
        the file is missing or cannot be parsed. ``None`` means "unknown" â€”
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``zed update``.
    ...
    The check is deliberately conservative â€” it only ever restores when there
    ...
    Args:
        ...
        zed_home: Override for the Zed home directory (tests).
    ...
    # unparseable) is intentionally left alone â€” that's a different failure
    ...
    if not snap_count:  # None or 0 â€” nothing worth restoring
        return None
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(zed_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for zed backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_zed_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, zed_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``zed_root`` to ``out_path``.
    ...
    or write error â€” caller should surface the outcome but not raise).
    """
...
def _pre_update_backup_dir(zed_home: Optional[Path] = None) -> Path:
    home = zed_home or get_zed_home()
...
def _prune_pre_update_backups(backup_dir: Path, keep: int) -> int:
    ...
    ``updates.pre_update_backup: false`` in config â€” that gates creation.
    """
    keep = max(keep, 1)
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ZED_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ZED_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises â€” the caller
    (``zed update``) should continue even if the backup fails.
    """
    zed_root = zed_home or get_default_zed_root()
    if not zed_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(zed_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, zed_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `zed claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ZED_HOME under ``backups/`` before a
    ``zed claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` â€” same exclusions, same SQLite safe-copy,
    restorable with ``zed import <archive>``.  Writes to
    ``<ZED_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    old pre-migration backups.

    Returns the path to the created zip, or ``None`` if nothing was found
    to back up (fresh install) or the write failed.  Never raises â€” the
    ...
    # Reuses the shared backups/ directory so `zed import` and the
