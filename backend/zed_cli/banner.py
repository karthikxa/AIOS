...
Pure display functions with no ZedCLI state dependency.
"""
...
from zed_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    try:
        ...
        pass  # Offline or timeout â€” use stale refs, that's fine
...
def _fetch_pypi_latest(package: str = "zed-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Zed update is available.

    Two paths: if ``ZED_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = zed_home / ".update_check"
    embedded_rev = os.environ.get("ZED_REVISION") or None

    # Docker images have no working tree to count commits against â€” the
    # published image excludes `.git` (see .dockerignore) and sets no
    # ZED_REVISION (that's nix-only). Without this guard the checks below
    ...
    # "1 commit behind" â€” even though no git repo or commit math is involved,
    # and `zed update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/zed/update/check`
    ...
    # Read cache â€” invalidate if the embedded rev OR installed version has
    ...
    if embedded_rev:
        ...
        # $ZED_HOME/zed-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = zed_home / "zed-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Zed git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$ZED_HOME/zed-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    the active checkout.  When no checkout is available â€” the canonical case
    is the published Docker image, which excludes ``.git`` from the build
    context â€” we fall back to the baked-in build SHA (see
    ``zed_cli/build_info.py``) and return it as a frozen
    ``upstream == local`` state with ``ahead=0``.  A built image is by
    definition pinned to one commit, so "ahead" is always zero and the
    banner correctly shows ``Â· upstream <sha>`` with no carried-commits
    annotation.
    """
    repo_dir = repo_dir or _resolve_repo_dir()
    if repo_dir is None:
        # No git checkout â€” try the baked build SHA (Docker image path).
        try:
            from zed_cli.build_info import get_build_sha
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    repo_dir = repo_dir or _resolve_repo_dir()
    if repo_dir is None:
        _latest_release_cache = ()  # falsy sentinel â€” skip future lookups
        return None
...
def format_banner_version_label() -> str:
    ...
    base = f"Zed Agent v{VERSION} ({RELEASE_DATE})"
    ...
    if ahead <= 0 or upstream == local:
        return f"{base} Â· upstream {upstream}"

    carried_word = "commit" if ahead == 1 else "commits"
    return f"{base} Â· upstream {upstream} Â· local {local} (+{ahead} carried {carried_word})"
...
def _format_context_length(tokens: int) -> str:
    """Format a token count for display (e.g. 128000 â†’ '128K', 1048576 â†’ '1M')."""
    if tokens >= 1_000_000:
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    # homeassistant) â€” they show as unavailable at banner time because the
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else ZED_CADUCEUS
    except Exception:
        _bskin = None
        _hero = ZED_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    ctx_str = f" [dim {dim}]Â·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]Â·[/] [dim {dim}]Zed Team[/]")

    if os.getenv("ZED_YOLO_MODE"):
        left_lines.append(f"[bold red]âš  YOLO mode[/] [dim {dim}]â€” all approval prompts bypassed[/]")
        ...
        for srv in mcp_status:
            ...
            if srv["connected"]:
                right_lines.append(
                    ...
                    f"[dim {dim}]â€”[/] [{text}]{srv['tools']} tool(s)[/]"
                )
            elif srv.get("disabled") or status == "disabled":
                right_lines.append(
                    f"[dim {dim}]{srv['name']}[/] [dim]({srv['transport']})[/] "
                    f"[dim {dim}]â€” disabled[/]"
                )
            elif status == "connecting":
                right_lines.append(
                    f"[dim {dim}]{srv['name']}[/] [dim]({srv['transport']})[/] "
                    f"[yellow]â€” connecting[/]"
                )
            elif status == "configured":
                right_lines.append(
                    f"[dim {dim}]{srv['name']}[/] [dim]({srv['transport']})[/] "
                    f"[dim {dim}]â€” configured[/]"
                )
            else:
                right_lines.append(
    ...
    try:
        from zed_cli.codex_runtime_switch import get_current_runtime
        from zed_cli.config import load_config as _load_cfg
    ...
    right_lines.append(f"[dim {dim}]{' Â· '.join(summary_parts)}[/]")

    # Update check â€” use prefetched result if available
    try:
        behind = get_update_result(timeout=0.5)
        if behind is not None and behind != 0:
            from zed_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                commits_word = "commit" if behind == 1 else "commits"
                right_lines.append(
                    f"[bold yellow]âš  {behind} {commits_word} behind[/]"
                    f"[dim yellow] â€” run [bold]{recommended_update_command()}[/bold] to update[/]"
                )
            else:
                # UPDATE_AVAILABLE_NO_COUNT: nix-built zed; we know an update
                # exists but not by how much, and we don't know how the user
                # installed it (nix run, profile, system flake, home-manager).
                managed_cmd = get_managed_update_command()
                line = "[bold yellow]âš  update available[/]"
                if managed_cmd:
                    line += f"[dim yellow] â€” run [bold]{managed_cmd}[/bold][/]"
                right_lines.append(line)
    except Exception:
        pass  # Never break the banner over an update check

    # Pip-install warning â€” `pip install zed-agent` is not the supported
    ...
    try:
        ...
        if detect_install_method() == "pip":
            right_lines.append(
                "[bold yellow]âš  pip install not officially supported[/]"
                "[dim yellow] â€” exists for reasons other than user install; "
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else ZED_AGENT_LOGO
        console.print(_logo)
        console.print()
