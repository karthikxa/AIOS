"""On-demand supply-chain audit for Zed Agent installs.

Scans three surfaces a Zed user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Zed venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.zed/plugins``
...
+ ``/v1/vulns/{id}``). Single-shot, on-demand, never daily â€” see the design
...
class Component:
    ...
    name: str
    version: str
    ecosystem: str  # "PyPI" | "npm" â€” exactly as OSV expects
...
# â”€â”€â”€ Component discovery â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _parse_requirements(text: str) -> list[tuple[str, str]]:
    ...
    is worse than missing a finding for an audit tool â€” false positives
...
def _discover_plugins(zed_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.zed/plugins``.
...
#   npx --yes pkg@1.2.3
#   npx pkg@1.2.3 [...args]
# We deliberately don't try to resolve unversioned names â€” that maps to
...
# â”€â”€â”€ OSV client â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _osv_severity_from_record(record: dict) -> str:
    ...
    if isinstance(raw, str) and raw.strip():
        ...
        if upper in SEVERITY_ORDER:
            return upper
    # Fall back to CVSS score â†’ tier
    score: Optional[float] = None
    for sev_entry in record.get("severity") or []:
        s = sev_entry.get("score")
        if isinstance(s, str):
            # CVSS vector strings look like "CVSS:3.1/AV:N/..." â€” we can't
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, zed_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(zed_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `zed security audit`."""
    home = Path(get_zed_home())
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, zed_home=home
    )
    if total == 0:
