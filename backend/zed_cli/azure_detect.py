...
Inspect a Microsoft Foundry / Azure OpenAI endpoint to determine:
  ...
  - Available models (best effort â€” Azure does not expose a deployment
...
Azure has no pure-API-key deployment-listing endpoint â€” per Microsoft,
...
still a useful hint â€” the user picks a familiar model name and we look
...
def _resolve_credential(api_key: Any,
    ...
    Returns ``(token_or_None, mode)`` where ``mode`` is:
      - ``"entra_id"`` when a callable token provider was supplied â€” the
        returned token is a freshly minted bearer JWT, sent ONLY in
        ``Authorization: Bearer``.
      - ``"api_key"`` when a string key was supplied â€” the returned token
...
def _http_get_json(url: str,
    ...
    req.add_header("User-Agent", "zed-agent/azure-detect")
    try:
...
def _probe_openai_models(base_url: str,
    ...
    # Azure OpenAI v1: {resource}.openai.azure.com/openai/v1 â€” no
    ...
    for url in candidates:
        ...
        if status == 200 and body is not None:
            ...
            # listed" â€” let the user proceed with manual entry.
...
def _probe_anthropic_messages(base_url: str,
    ...
    try:
        with urllib_request.urlopen(req, timeout=6.0) as resp:
            # Should never 200 â€” "probe" isn't a real deployment.  But
...
def detect(base_url: str,
    ...
    :class:`DetectionResult` as *advisory* â€” if ``api_mode`` is None,
    fall back to asking the user.

    ``api_key`` may be a string (legacy API-key auth â€” sends both
    ``api-key:`` and ``Authorization: Bearer``) or a callable returning
    a bearer JWT (Entra ID auth â€” sends ONLY ``Authorization: Bearer``).
    ...
    if _looks_like_anthropic_path(base_url):
        ...
        result.reason = "URL path ends in /anthropic â†’ Anthropic Messages API"
        return result
    ...
    if ok:
        ...
        result.reason = (
            f"GET /models returned {len(models)} model(s) â€” OpenAI-style endpoint"
            if models
            else "GET /models returned an OpenAI-shaped empty list â€” OpenAI-style endpoint"
        )
        return result
    ...
    result.reason = (
        ...
        "non-standard path) â€” falling back to manual API-mode selection"
    )
    return result
...
def lookup_context_length(model: str,
    ...
    only reads catalog metadata over HTTP â€” no SDK client is built â€” so
