...
on Japanese locales, etc.).  Zed's banners, tool output feed, and slash
command listings all contain Unicode: box-drawing characters (``â”€â”Œâ”â””â”˜â”œâ”¤``),
mathematical and geometric symbols (``â—† â—‡ â—Ž â–£ âš” âš– â†’``), and user-supplied
text in any language.  Printing those to a cp1252 console raises
``UnicodeEncodeError: 'charmap' codec can't encode characterâ€¦`` and kills the
...
Entry points (``cli.py`` ``main``, ``zed_cli/main.py`` CLI dispatch,
...
actually flip the console code page â€” they rely on their runtime (Node or
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent â€” if there's no attached console (e.g. Zed is running
    ...
    try:
        ...
        # ctypes import, missing kernel32, or non-Windows â€” any failure here
...
def configure_windows_stdio() -> bool:
    ...
    Idempotent â€” safe to call multiple times from different entry points.

    Returns ``True`` if anything was actually changed, ``False`` on
    non-Windows or on a repeat call.

    Set ``ZED_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    set â€” see :func:`_default_windows_editor`.
    """
    global _CONFIGURED
    ...
    if os.environ.get("ZED_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Windows â€” Ctrl+X Ctrl+E and ``/edit`` silently do nothing there
    ...
    # Augment PATH with the Zed-managed Git install directories so
    ...
    # already-running shells don't see the change â€” which means zed
    ...
    # install without Zed-managed PortableGit).
    ...
    # gets a ``?`` rather than crashing the interpreter â€” we prefer
    # degraded output over a stack trace.
    _reconfigure_stream(sys.stdout)
    _reconfigure_stream(sys.stderr)
    # stdin is re-configured for completeness; Zed's interactive
...
def _default_windows_editor() -> str:
    ...
    1. ``notepad`` â€” ships with every Windows install, no deps, works as a
       blocking editor (``subprocess.call(["notepad", file])`` blocks until
       the user closes the window).  This is the "always-works" default.

    The prompt_toolkit buffer's ``open_in_editor`` and Zed's
    ``zed config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Zed (User env var in Windows Settings, or
    export in a PowerShell profile) and Zed picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Zed-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\zed\\git\\bin`` to the User PATH via
    ``SetEnvironmentVariable(..., "User")``.  That write propagates to newly
    *spawned* processes only â€” already-running shells (including the one the
    user invokes ``zed`` from right after install) retain their old PATH.

    Any subprocess Zed spawns â€” bash, ``rg``, ``grep``, ``npm`` â€” inherits
    ...
    Patch-up strategy: add the known Zed-managed tool directories to our
    ...
    # the PATH entries that installer adds to User scope â€” the two lists
    # should match so this prefill fully mirrors what a fresh shell would
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "zed", "git", "cmd"),
        os.path.join(local_appdata, "zed", "git", "bin"),
        os.path.join(local_appdata, "zed", "git", "usr", "bin"),
        # Zed venv Scripts directory â€” host of the zed.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes zed, but harmless to include.
        os.path.join(local_appdata, "zed", "zed-agent", "venv", "Scripts"),
        # WinGet packages directory â€” where ``winget install`` drops CLI
