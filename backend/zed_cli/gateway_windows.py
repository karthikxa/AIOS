...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m zed_cli.main gateway run --replace`` invocation) and
...
_FALLBACK_PATTERNS = re.compile(
    r"(access is denied|acceso denegado|pÅ™Ã­stup byl odepÅ™en|schtasks timed out|schtasks produced no output)",
    re.IGNORECASE,
)
_ACCESS_DENIED_PATTERN = re.compile(r"(access is denied|acceso denegado)", re.IGNORECASE)

_TASK_NAME_DEFAULT = "Zed_Gateway"
_TASK_DESCRIPTION = "Zed Agent Gateway - Messaging Platform Integration"
...
# ---------------------------------------------------------------------------
# Quoting helpers (two DIFFERENT parsers â€” do not mix)
# ---------------------------------------------------------------------------
...
def _exec_schtasks(args: list[str]) -> tuple[int, str, str]:
    ...
    If schtasks wedges, returns code=124 with a synthetic stderr string â€”
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Zed profile."""
    from zed_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "zed_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
    ...
    try:
        ...
        print(f"âš  Could not launch elevated gateway {command} prompt: {exc}")
        return False
    if result <= 32:
        print(f"âš  Elevated gateway {command} prompt was not started (ShellExecuteW={result})")
        return False
    return True
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("ZED_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("ZED_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("ZED_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["ZED_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["ZED_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["ZED_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("ZED_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("ZED_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("ZED_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``Zed_Gateway``
    Named profile X: ``Zed_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during zed_cli boot.
    from zed_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\zed\\gateway-service\\<task_name>.cmd``
    (or ``<ZED_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Zed installs stay self-contained).
    """
    _assert_windows()
    from zed_cli.config import get_zed_home

    script_dir = Path(get_zed_home()) / "gateway-service"
...
def _startup_dir() -> Path:
    ...
    if not userprofile:
        raise RuntimeError("neither APPDATA nor USERPROFILE is set â€” cannot resolve Startup folder")
    return (
        Path(userprofile)
        / "AppData"
        / "Roaming"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``ZED_HOME`` whenever it
    ...
    source checkout only if ``ZED_HOME`` cannot be resolved yet.
    """
    from zed_cli.config import get_zed_home
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports ZED_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m zed_cli.main [--profile X] gateway run``
        directly so the wrapper cmd.exe exits without a visible gateway console

    We intentionally do NOT inline PATH overrides here â€” cmd.exe inherits
    ...
    lines.append(f'set "ZED_HOME={zed_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "ZED_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports zed_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    pythonw_path = _derive_venv_pythonw(python_path)
    prog_args = [pythonw_path, "-m", "zed_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    zed_home = str(Path(get_zed_home()).resolve())
    profile_arg = _profile_arg(zed_home)

    content = _build_gateway_cmd_script(python_path, working_dir, zed_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Zed builds and failed
...
def _derive_venv_pythonw(python_exe: str) -> str:
    ...
    isn't there â€” caller must still set CREATE_NO_WINDOW in that case.
    """
    p = Path(python_exe)
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    native argv for direct ``subprocess.Popen`` invocation â€” no cmd.exe
    layer in between.
    """
    _assert_windows()
    from zed_cli.config import get_zed_home
    ...
    argv = [python_exe, "-m", "zed_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m zed_cli.main gateway run``
    directly â€” NOT through a cmd.exe shim â€” because on Windows a cmd.exe
    ...
    but ignored â€” we don't need it now that we go direct.
    ...
    # DETACHED_PROCESS        0x00000008  â€” no console attached to child
    # CREATE_NEW_PROCESS_GROUP 0x00000200 â€” child gets its own group, won't
    #                                       receive Ctrl+C from our group
    # CREATE_NO_WINDOW         0x08000000 â€” belt-and-braces no-console flag
    # CREATE_BREAKAWAY_FROM_JOB 0x01000000 â€” escape any job object the
    ...
    # real gateway logs still land there â€” this just captures anything
    # that goes to print() or native stderr.
    from zed_cli.config import get_zed_home

    log_dir = Path(get_zed_home()) / "logs"
    ...
    try:
        ...
        # Terminal configs). Retry without the breakaway flag â€” in most
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("ZED_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("ZED_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from zed_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    print(f"â†» Scheduled Task install blocked ({detail.splitlines()[0]}) â€” using Startup folder fallback")
    entry = _install_startup_entry(script_path)
    print(f"âœ“ Installed Windows login item: {entry}")
    print(f"  Task script: {script_path}")

    # Re-running `zed -p <profile> gateway install` must be safe.
    ...
    from zed_cli.gateway import find_gateway_pids, _profile_arg

    running_pids = list(find_gateway_pids())
    if running_pids:
        print(f"âœ“ Gateway already running (PID: {', '.join(map(str, running_pids))})")
    elif start_now:
        pid = _spawn_detached()
        _report_gateway_start(f"direct spawn (PID {pid})")
    else:
        profile_arg = _profile_arg()
        start_cmd = f"zed {profile_arg} gateway start" if profile_arg else "zed gateway start"
        print("â„¹ Startup fallback installed; gateway not started now.")
...
def install(
    ...
    *,
    ...
    / ``systemd_install`` but isn't needed â€” we always reconcile.
    """
    _assert_windows()
    start_now, start_on_login = _prompt_install_choices(start_now, start_on_login)

    if not start_on_login:
        print("â„¹ Skipped Windows login auto-start install.")
        if start_now:
            ...
            print("â„¹ Gateway not started and no auto-start service installed.")
            print("  Run later with: zed gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        print("â†» Scheduled Task install may need administrator approval on this Windows account.")
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("âœ“ Launched elevated Zed gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: zed gateway status")
                return
            print("âš  Falling back to Startup folder because elevation was unavailable or cancelled.")
        else:
    ...
    if ok:
        print(f"âœ“ {detail}")
        print(f"  Task script: {script_path}")
        print("â„¹ Gateway auto-start installed for Windows login.")
        if start_now:
            running_pids = _gateway_pids()
            if running_pids:
                print(f"âœ“ Gateway already running (PID: {', '.join(map(str, running_pids))})")
            ...
            print("â„¹ Gateway not started now.")
            print("  Start manually with: zed gateway start")
        _print_next_steps()
        return
    ...
    if _is_access_denied(detail) and not _is_running_as_admin():
        ...
        print(f"â†» Scheduled Task install needs administrator approval ({detail.splitlines()[0]})")
        print("  UAC is Windows' admin approval prompt; it is needed to create/update the Scheduled Task.")
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("âœ“ Launched elevated Zed gateway install prompt.")
    ...
    # Unknown schtasks error â€” surface it and bail.
...
def _wait_for_gateway_ready(timeout_s: float = 6.0, interval_s: float = 0.4) -> list[int]:
    ...
    time â€” the caller should surface that to the user as a failed start.
    """
    from zed_cli.gateway import find_gateway_pids
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        print(f"âœ“ Gateway started via {via} (PID: {', '.join(map(str, pids))})")
    else:
        print(f"âš  Launched gateway via {via}, but no process detected after 6s.")
        print("  Check the log for startup errors:")
        from zed_cli.config import get_zed_home
        print(f"    type {Path(get_zed_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_zed_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from zed_cli.config import get_zed_home

    zed_home = Path(get_zed_home()).resolve()
    print()
    print("Next steps:")
    print("  zed gateway status                      # Check status")
    print(f"  type {zed_home}\\logs\\gateway.log       # View logs")
...
def uninstall() -> None:
    ...
    if is_task_registered():
        ...
        if code == 0:
            ...
            print(f"âœ“ Removed Scheduled Task {task_name!r}")
        elif _is_access_denied(detail) and not _is_running_as_admin():
            from zed_cli.setup import prompt_yes_no

            print(f"â†» Scheduled Task uninstall needs administrator approval ({detail or 'access denied'})")
            print("  UAC is Windows' admin approval prompt; it is needed to remove the Scheduled Task.")
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("âœ“ Launched elevated Zed gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: zed gateway status")
                    return
                print("âš  Elevated uninstall prompt was unavailable or cancelled.")
            else:
                print("  Skipped elevation. Scheduled Task was not removed.")
        else:
            print(f"âš  schtasks /Delete returned code {code}: {detail}")
    ...
    for path, label in [
        ...
        try:
            path.unlink()
            print(f"âœ“ Removed {label}: {path}")
        except FileNotFoundError:
            pass

    if is_task_registered() and not scheduled_task_removed:
        print(f"âš  Scheduled Task still registered: {task_name}")
...
def _print_deep_probes() -> None:
    ...
    âœ“ / âœ— line, which is great when they agree and confusing when they
    ...
    Probes:
      ...
      [4] _pid_exists(pid) â€” OS confirms the process is alive
    ...
    home = Path(get_zed_home()).resolve()
...
def status(deep: bool = False) -> None:
    ...
    if task_installed:
        print(f"âœ“ Scheduled Task registered: {task_name}")
        ...
        print(f"âœ“ Windows login item installed: {get_startup_entry_path()}")
    else:
        print("âœ— Gateway service not installed")

    if pids:
        print(f"âœ“ Gateway process running (PID: {', '.join(map(str, pids))})")
    else:
        print("âœ— No gateway process detected")
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: zed gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            print("âš  Gateway install did not complete in this process.")
            print("  If a UAC prompt opened, approve it, then run: zed gateway start")
            return
    ...
    if task_installed:
        ...
        if code == 0:
            ...
            return
        print(f"âš  schtasks /Run failed (code {code}): {err.strip()} â€” falling back to direct spawn")
...
def stop() -> None:
    ...
    in-flight agents and persist ``resume_pending`` before exit (the
    gateway's marker-watcher thread picks this up â€” Windows asyncio
    ...
    """
    _assert_windows()
    from zed_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
    from gateway.status import get_running_pid
    ...
    if is_task_registered():
        ...
        # schtasks returns nonzero when the task isn't currently running â€” don't treat that as an error.
        if code == 0:
            stopped_any = True
        elif "not running" not in (err or "").lower():
            print(f"âš  schtasks /End returned code {code}: {err.strip()}")
    ...
    if killed:
        stopped_any = True
        print(f"âœ“ Killed {killed} gateway process(es)")
    if stopped_any:
        if drained:
            print("âœ“ Gateway stopped (drained cleanly)")
        else:
            print("âœ“ Gateway stopped")
    else:
        print("âœ— No gateway was running")
...
def restart() -> None:
    ...
    """
    _assert_windows()
    from zed_cli.gateway import kill_gateway_processes

    stop()

    if not _wait_for_gateway_absent(timeout_s=30.0):
        print("âš  Gateway still present after stop; forcing termination before restart...")
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `zed gateway status`."
        )
