"""zed webhook â€” manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.zed/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _zed_home() / _SUBSCRIPTIONS_FILENAME
...
def _save_subscriptions(subs: Dict[str, dict]) -> None:
    ...
    # webhook_subscriptions.json contains per-route HMAC secrets â€” write
...
def _setup_hint() -> str:
  ...
  Then start the gateway: zed gateway run
"""
...
def webhook_command(args):
    """Entry point for 'zed webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: zed webhook {subscribe|list|remove|test}")
        print("Run 'zed webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    if getattr(args, "deliver_only", False):
        if route["deliver"] == "log":
            print(
                ...
                "(telegram, discord, slack, github_comment, etc.) â€” not 'log'."
            )
            return
    ...
    print(f"  The gateway must be running to receive events (zed gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: zed webhook subscribe <name>")
        return
    ...
    for name, route in subs.items():
        ...
        if route.get("deliver_only"):
            deliver = f"{deliver} (direct â€” no agent)"
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from zed webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (zed gateway run)")
