...
def format_nous_portal_entitlement_message(
    ...
    *,
    ...
    ``None`` means the account is entitled to use the capability â€” via paid
    ...
    if account_info.paid_service_access is None:
        ...
        if include_refresh_hint:
            detail += " Run `zed model` to refresh your session."
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `zed model` to refresh Zed."
        return message
...
def _fresh_account_info(
    *,
    ...
    try:
        from zed_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from zed_cli.auth import _decode_jwt_claims
    except Exception:
        return None
