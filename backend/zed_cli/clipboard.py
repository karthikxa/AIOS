...
True on success.  No external Python dependencies â€” uses only OS-level
CLI tools that ship with the platform (or are commonly installed).

Platform support:
  macOS   â€” osascript (always available), pngpaste (if installed)
  Windows â€” PowerShell via WinForms, Get-Clipboard, file-drop fallback
  WSL2    â€” powershell.exe via WinForms, Get-Clipboard, file-drop fallback
  Linux   â€” wl-paste (Wayland), xclip (X11)
"""
...
from zed_constants import is_wsl as _is_wsl
...
def has_clipboard_image() -> bool:
    ...
    Lighter than save_clipboard_image â€” doesn't extract or write anything.
    """
    if sys.platform == "darwin":
        return _macos_has_image()
    if sys.platform == "win32":
        return _windows_has_image()
    # Match _linux_save fallthrough order: WSL â†’ Wayland â†’ X11
...
def _macos_has_image() -> bool:
    ...
    try:
        ...
        return "Â«class PNGfÂ»" in info.stdout or "Â«class TIFFÂ»" in info.stdout
    except Exception:
        return False


def _macos_pngpaste(dest: Path) -> bool:
    """Use pngpaste (brew install pngpaste) â€” fastest, cleanest."""
    try:
        r = subprocess.run(
            ["pngpaste", str(dest)],
...
# â”€â”€ Shared PowerShell scripts (native Windows + WSL2) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

# .NET System.Windows.Forms.Clipboard â€” used by both native Windows (powershell)
...
def _powershell_has_image(exe: str, *, timeout: int, label: str) -> bool:
    for script in _POWERSHELL_HAS_IMAGE_SCRIPTS:
        try:
            ...
            logger.debug("%s not found â€” clipboard unavailable", exe)
            return False
        except Exception as e:
...
# â”€â”€ Native Windows â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _windows_save(dest: Path) -> bool:
    """Extract clipboard image on native Windows via PowerShell â†’ base64 PNG."""
    ps = _get_ps_exe()
    if ps is None:
        logger.debug("No PowerShell found â€” Windows clipboard image paste unavailable")
        return False
    return _powershell_save_image(ps, dest, timeout=15, label="Windows")


# â”€â”€ Linux â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def _linux_save(dest: Path) -> bool:
    """Try clipboard backends in priority order: WSL â†’ Wayland â†’ X11."""
    if _is_wsl():
        if _wsl_save(dest):
            return True
        # Fall through â€” WSLg might have wl-paste or xclip working
...
# â”€â”€ WSL2 (powershell.exe) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _wsl_save(dest: Path) -> bool:
    """Extract clipboard image via powershell.exe â†’ base64 â†’ decode to PNG."""
...
def _wayland_has_image() -> bool:
    ...
    try:
        ...
        logger.debug("wl-paste not installed â€” Wayland clipboard unavailable")
    except Exception:
        pass
    return False
...
def _convert_to_png(path: Path) -> bool:
    ...
    try:
        ...
        logger.debug("Pillow BMPâ†’PNG conversion failed: %s", e)
    ...
    try:
        ...
        if r.returncode == 0 and path.exists() and path.stat().st_size > 0:
            ...
            # Convert failed â€” restore the original file
            tmp.rename(path)
    except FileNotFoundError:
        logger.debug("ImageMagick not installed â€” cannot convert BMP to PNG")
        if tmp.exists() and not path.exists():
            tmp.rename(path)
    except Exception as e:
        logger.debug("ImageMagick BMPâ†’PNG conversion failed: %s", e)
        if tmp.exists() and not path.exists():
            tmp.rename(path)

    # Can't convert â€” BMP is still usable as-is for most APIs
...
def _xclip_save(dest: Path) -> bool:
    ...
    try:
        ...
        logger.debug("xclip not installed â€” X11 clipboard image paste unavailable")
        return False
    except Exception:
        return False
