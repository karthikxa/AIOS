...
# coverage category (zed_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `zed tools` picker scope its entitlement gate to the selected backend, so a
# free-tool-pool user is allowed image gen but denied video gen at select time â€”
...
def _local_browser_runnable() -> bool:
    ...
    Lightpanda engine is selected â€” text-only navigation needs no Chromium.
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("ZED_LOCAL_STT_COMMAND"):
        return True
    try:
...
def _resolve_browser_feature_state(
    *,
    ...
    ``browser_local_available`` means "the agent-browser CLI is present" â€” the
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # use_gateway flags â€” when True, the user explicitly opted into the
    # Tool Gateway via `zed model`, so direct credentials should NOT
    ...
    # OpenAI TTS â€” resolve_openai_audio_api_key() reads VOICE_TOOLS_OPENAI_KEY
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("ZED_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("ZED_LOCAL_STT_COMMAND"))
    ...
    # pool funds image and NOT video â€” so gate it on its own coverage category
    ...
    # tool â€” the gateway voice middleware calls it on every inbound voice
    # message â€” so toolset_enabled is N/A and we treat stt as always
...
def apply_nous_managed_defaults(
    ...
    *,
    ...
    # local command â€” strong intent signal that "local" was a choice, not
...
# ---------------------------------------------------------------------------
# Tool Gateway offer â€” single Y/n prompt after model selection
# ---------------------------------------------------------------------------
...
def _get_gateway_direct_credentials() -> Dict[str, bool]:
    ...
    return {
        ...
        # with TTS via resolve_openai_audio_api_key() â€” counting it here
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    # no direct keys exist â€” we only skip the prompt for tools where
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from zed_cli.setup import prompt_checklist
    except Exception:
        return set()
    ...
    labels += [
        f"{_GATEWAY_TOOL_LABELS[k]} â€” keep using your {_GATEWAY_DIRECT_LABELS[k]}"
        for k in has_direct
    ]
    ...
    if changed:
        ...
        for key in sorted(changed):
            ...
            print(f"  âœ“ {label}: enabled via {source_label}")
    return changed
...
def ensure_nous_portal_access(
    *,
    ...
    must cover that category specifically â€” so a pool user selecting video
    ...
    # Logged in but not entitled for this capability â€” surface neutral billing
...
def _run_nous_portal_login_only(*, capability: str) -> bool:
    ...
    try:
        ...
        with _auth_store_lock():
            ...
            # Preserve the user's existing inference provider â€” this login is
