"""Profile distributions â€” shareable, packaged Zed profiles via git.

A distribution is a Zed profile published as a git repository (or
...
* ``zed profile export/import`` â€” local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``zed skills install <url>`` â€” the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``zed profile``, not a parallel tree):

    zed profile install <source> [--name N] [--alias] [--force] [--yes]
...
* A git URL (``github.com/user/repo``, ``https://github.com/...``, ``git@...``,
  ...
  commit SHA.
* A local directory that already contains ``distribution.yaml`` â€” used
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "zed-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    # Tracked after install â€” where we pulled from, so ``update`` can re-pull.
    source: str = ""
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            zed_requires=str(data.get("zed_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.zed_requires:
            out["zed_requires"] = self.zed_requires
        if self.author:
...
def _load_yaml(text: str) -> Any:
    try:
        import yaml
    except ImportError as exc:  # pragma: no cover â€” pyyaml is a hard dep
...
def _parse_semver(v: str) -> Tuple[int, int, int]:
    """Very small semver parser â€” major.minor.patch only.  Extra labels stripped."""
...
def check_zed_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Zed {op}{target}, "
...
# ---------------------------------------------------------------------------
# Source staging â€” git clone or local directory
# ---------------------------------------------------------------------------
...
def _looks_like_git_url(s: str) -> bool:
    ...
    if s.startswith(("http://", "https://")):
        ...
        # tar.gz URLs â€” git is the only remote transport.
        return True
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``zed profile update`` can
    re-pull from the same place.

    Accepts:
      * A git URL (https / ssh / git@ / bare github.com shorthand) â€” cloned
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Zed profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            f"No {MANIFEST_FILENAME} found at the distribution root â€” "
            "this source is not a Zed distribution."
        )

    # Version check up-front so we fail fast
    check_zed_requires(manifest.zed_requires, zed_version)
    ...
    if canon == "default":
        raise DistributionError(
            "Cannot install a distribution as 'default' â€” that is the built-in "
            "root profile (~/.zed).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="zed_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `zed profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`zed profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="zed_dist_update_") as tmp:
        plan = plan_install(
...
# ---------------------------------------------------------------------------
# Info â€” render a manifest summary
# ---------------------------------------------------------------------------
