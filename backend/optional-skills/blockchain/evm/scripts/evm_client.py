#!/usr/bin/env python3
"""
evm_client.py â€” EVM blockchain CLI tool for the Zed Agent project.
...
def is_valid_address(s: str) -> bool:
    ...
    Does NOT validate EIP-55 checksum â€” RPC endpoints accept any-case hex.
...
def rpc_batch(chain: str, calls: List[Tuple[str, List[Any]]], batch_limit: int = 10) -> List[Any]:
    ...
    Base's public RPC caps batches at 10 â€” exceeding that returns a single error
    ...
    for start in range(0, len(items), batch_limit):
        ...
        if not isinstance(resp, list):
            # Single error response (e.g. batch-too-large) â€” leave this chunk as None
            continue
        for r in resp:
            rid = r.get("id")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="evm_client",
        description="EVM blockchain CLI â€” stdlib only, zero dependencies.",
    )
