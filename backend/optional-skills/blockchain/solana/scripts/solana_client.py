#!/usr/bin/env python3
"""
Solana Blockchain CLI Tool for Zed Agent
--------------------------------------------
Queries the Solana JSON-RPC API and CoinGecko for enriched on-chain data.
Uses only Python standard library â€” no external packages required.
...
# Well-known Solana token names â€” avoids API calls for common tokens.
# Maps mint address â†’ (symbol, name).
...
# Reverse lookup: symbol â†’ mint (for the `price` command).
...
def _http_get_json(url: str, timeout: int = 10, retries: int = 2) -> Any:
    ...
    for attempt in range(retries + 1):
        req = urllib.request.Request(
            url, headers={"Accept": "application/json", "User-Agent": "ZedAgent/1.0"},
        )
        try:
...
# Keep backward compat â€” the rest of the code uses `rpc()`.
rpc = _rpc_call
...
# ---------------------------------------------------------------------------
# Price & token name helpers (CoinGecko â€” free, no API key)
# ---------------------------------------------------------------------------
...
def fetch_prices(mints: List[str], max_lookups: int = 20) -> Dict[str, float]:
    ...
    so we do individual calls â€” capped at *max_lookups* to stay within
...
def cmd_wallet(args):
    ...
    # Sort: tokens with known USD value first (highestâ†’lowest), then unknowns
...
def cmd_whales(args):
    ...
    out = {
        ...
        "note":              "Scans latest block only â€” point-in-time snapshot.",
    }
    if sol_price:
        out["sol_price_usd"] = sol_price
    print_json(out)
...
def cmd_price(args):
    ...
    if mint in prices:
        ...
        out["price_usd"] = None
        out["note"] = "Price not available â€” token may not be listed on CoinGecko."
    print_json(out)
...
def main():
    parser = argparse.ArgumentParser(
        ...
        description="Solana blockchain query tool for Zed Agent",
    )
