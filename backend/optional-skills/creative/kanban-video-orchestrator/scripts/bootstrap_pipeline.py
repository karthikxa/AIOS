...
../assets/, and writes a setup.sh that creates Zed profiles and fires the
initial kanban task.

Profile-config patching, SOUL.md-per-profile, TEAM.md task-graph convention,
and the `zed kanban create --workspace dir:` initial-task pattern are
adapted from alt-glitch's ZedTeam/kanban-video-pipeline.

Usage:
    bootstrap_pipeline.py plan.json [--out setup.sh]

The plan.json schema is documented inline below â€” see the `validate_plan`
    ...
    if "team" in plan:
        if not isinstance(plan["team"], list) or not plan["team"]:
            ...
            for i, t in enumerate(plan["team"]):
                ...
                # Profile name must match Zed's regex (lowercase
                ...
                if "profile" in t:
                    if not PROFILE_NAME_RE.match(t["profile"]):
                        errors.append(
                            ...
                            f"[a-z0-9][a-z0-9_-]{{0,63}} per Zed profile rules"
                        )
...
def render_team_md(plan: dict) -> str:
    """Render TEAM.md from the team list + scene â†’ tool mapping."""
    lines = [f"# Team & Task Graph â€” {plan['title']}", "", "## Team", ""]
    for t in plan["team"]:
        skills = (
        ...
        lines.append(
            f"- `{t['profile']}` â€” {t['responsibilities']} ({skills})"
        )
    ...
    lines.append(f"T0  {director} â€” decompose")
    ...
    if "cinematographer" in profiles_by_role:
        cid = f"T{next_id}"
        lines.append(
            f"{cid:5} {profiles_by_role['cinematographer']} â€” visual spec for all scenes (parent: T0)"
        )
    ...
    if "music-supervisor" in profiles_by_role:
        cid = f"T{next_id}"
        lines.append(
            f"{cid:5} {profiles_by_role['music-supervisor']} â€” track analysis + beats.json (parent: T0)"
        )
        next_id += 1
        ms_id = cid
    else:
    ...
    for s in plan["scenes"]:
        ...
        lines.append(
            f"{cid:5} {renderer_profile} â€” scene {s.get('n', '?')}: "
    ...
    if "voice-talent" in profiles_by_role:
        vo_id = f"T{next_id}"
        lines.append(f"{vo_id:5} {profiles_by_role['voice-talent']} â€” narration (parent: T0)")
        next_id += 1
    else:
        vo_id = None
    ...
    if "audio-mixer" in profiles_by_role:
        ...
        lines.append(
            f"{am_id:5} {profiles_by_role['audio-mixer']} â€” mix audio (parents: {', '.join(am_parents)})"
        )
        next_id += 1
    else:
        am_id = None
    ...
    if "editor" in profiles_by_role:
        ...
        lines.append(
            f"{ed_id:5} {profiles_by_role['editor']} â€” assemble + mux (parents: {', '.join(ed_parents)})"
        )
        next_id += 1
    else:
        ed_id = None
    ...
    if "captioner" in profiles_by_role and ed_id:
        cap_id = f"T{next_id}"
        lines.append(
            f"{cap_id:5} {profiles_by_role['captioner']} â€” SRT + burn (parent: {ed_id})"
        )
        next_id += 1
        last = cap_id
    else:
    ...
    if "reviewer" in profiles_by_role and last:
        rv_id = f"T{next_id}"
        lines.append(
            f"{rv_id:5} {profiles_by_role['reviewer']} â€” final QA (parent: {last})"
        )
...
def render_setup_sh(plan: dict, brief_md: str, team_md: str) -> str:
    ...
    for key in plan.get("api_keys_required", []):
        key_checks.append(f'check_key {key} zed {key} || exit 1')
    ...
    for t in plan["team"]:
        profile_creates.append(
            f'zed profile create {t["profile"]} --clone 2>/dev/null || true'
        )

    # Profile config â€” emit JSON arrays so the bash function can pass them
    ...
    # SOUL writes â€” uses heredocs per profile
    soul_writes = []
    for t in plan["team"]:
        soul_writes.append(
            f'cat > "$HOME/.zed/profiles/{t["profile"]}/SOUL.md" <<\'SOUL_EOF\'\n'
            f"{render_soul_md(t, plan)}\n"
            f"SOUL_EOF\n"
            f'echo "  âœ“ SOUL.md for {t["profile"]}"'
        )
    ...
    taste_writes = (
        ...
        '# Emotional DNA\n\n'
        '_(What this piece should FEEL like â€” populate from the brief.)_\n'
        'DNA_EOF'
    )

    # Asset copies â€” leave empty by default; user fills in
...
def render_soul_md(team_member: dict, plan: dict) -> str:
    ...
    if role == "director":
        common_rules += (
            ...
            "- **Decompose, route, comment, approve â€” that's the whole job.**\n"
