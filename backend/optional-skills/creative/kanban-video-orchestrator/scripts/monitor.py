#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `zed kanban list` and
...
are detected. Designed to run alongside the kanban â€” kill with Ctrl-C when
...
def kanban_list(tenant: str) -> list[dict]:
    ...
    output isn't supported by the installed zed CLI."""
    try:
        out = subprocess.run(
            ["zed", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `zed kanban list`
    out = subprocess.run(
        ["zed", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def detect_issues(tasks: list[dict]) -> list[str]:
    ...
    for t in by_status.get("running", []) + by_status.get("RUNNING", []):
        ...
        if elapsed > max_rt:
            issues.append(
                f"OVERTIME: {t['id']} ({t.get('assignee', '?')}) â€” "
    ...
    for t in tasks:
        ...
        if retries and retries >= 2:
            issues.append(
                f"FLAPPING: {t['id']} ({t.get('assignee', '?')}) â€” "
                f"retried {retries}Ã— â€” fix root cause before next run"
            )
...
def print_snapshot(tasks: list[dict], issues: list[str]):
    ...
    for t in tasks:
        bar = "âœ“" if str(t.get("status", "")).lower() == "done" else \
              "â–¶" if str(t.get("status", "")).lower() == "running" else \
              "Â·" if str(t.get("status", "")).lower() == "ready" else \
              "âœ—" if str(t.get("status", "")).lower() == "failed" else "?"
        print(f"  {bar} {t.get('id', '?'):14} {t.get('assignee', '?'):20}  "
              f"{t.get('title', '')[:60]}")

    if issues:
        print("\n  âš   ISSUES:", file=sys.stderr)
        for i in issues:
...
def main():
    ...
    if not zed_available():
        print("ERROR: 'zed' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
