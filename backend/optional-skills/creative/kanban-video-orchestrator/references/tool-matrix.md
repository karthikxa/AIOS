# Tool Matrix â€” Skills + Toolsets per Role

Maps each role archetype to the Zed skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public zed-agent
...
## Zed skills relevant to video production

### Visual / rendering skills (`zed-agent/skills/creative/`)

| Skill | What it does | Best fit for |
|-------|--------------|--------------|
| `ascii-video` | Production pipeline for ASCII art video â€” generative, audio-reactive, video-to-ASCII | Renderer for ASCII / terminal / retro pixel content; cinematographer for ASCII projects |
| `ascii-art` | Static ASCII art generation | Concept artist for ASCII style frames; secondary tool for ASCII renderer |
| `manim-video` | Manim CE animations â€” math, algorithms, 3Blue1Brown-style explainers | Renderer for math, algorithm walkthroughs, technical concept explainers |
| `p5js` | p5.js sketches â€” generative art, shaders, interactive, 3D | Renderer for generative art, particle systems, organic motion, web-canvas content |
...
| `touchdesigner-mcp` | Control a running TouchDesigner instance â€” real-time visuals, audio-reactive installation art, VJ | Renderer for real-time/audio-reactive content; installation art; live performance |
| `blender-mcp` *(optional)* | Control Blender 4.3+ via MCP â€” 3D modeling, animation, rendering | Renderer for 3D scenes, photoreal environments, character animation |
...
### Design / pre-production skills (`zed-agent/skills/creative/`)
...
### Audio / media skills (`zed-agent/skills/creative/` + `skills/media/`)
...
| `spotify` | Spotify control â€” play, search, queue, manage playlists | Music supervisor sourcing existing tracks; reference research |
...
### Kanban infrastructure (`zed-agent/skills/devops/`)
...
| `kanban-worker` | Pitfalls, examples, edge cases for kanban workers (deeper than auto-injected guidance) | Any profile â€” load when handling tricky multi-step workflows |

The kanban plugin auto-injects baseline orchestration guidance into every
worker's system prompt â€” the `kanban_create` fan-out pattern, claim/handoff
...
These are **not** Zed skills but external CLIs / APIs that profiles invoke.
...
## Built-in Zed tools for media review

These are native Zed tools â€” not invoked via terminal but through their own
...
|------|---------|--------------|----------------------|
| `video_analyze` | `video` (opt-in â€” `zed tools enable video`) | Native video understanding â€” sends full clip to a multimodal LLM (Gemini via OpenRouter) for review without frame extraction. Supports mp4, webm, mov, avi, mkv. 50 MB cap. Model: `AUXILIARY_VIDEO_MODEL` env â†’ `AUXILIARY_VISION_MODEL` fallback. | reviewer, cinematographer, editor |
| `vision_analyze` | `vision` (core â€” enabled by default) | Image/frame analysis â€” review stills, thumbnails, exported frames. Already available to all profiles without opt-in. | reviewer, cinematographer, concept-artist |
...
No terminal â€” writers don't need it.
...
toolsets:
  ...
  - terminal
  - file
  - video               # video_analyze â€” review full clips natively
  - vision              # vision_analyze â€” review stills / exported frames
skills:
  always_load:
    - kanban-worker
...
skills:
  always_load:
    ...
    # - p5js                      (renderer-p5js)
    # - comfyui                   (renderer-comfyui â€” img/video AI gen)
...
contains `kanban-worker` â€” the role's work is API-driven and the API key +
terminal commands suffice.

For multi-skill renderer setups (rare â€” usually one variant per skill is
...
toolsets:
  ...
  - terminal
  - file
  - video              # video_analyze â€” editor reviews assembled cuts natively
  - vision             # vision_analyze â€” spot-check frames
skills:
  always_load:
    - kanban-worker
```
...
key is present in `${ZED_HOME:-~/.zed}/.env` (or macOS Keychain) before firing the kanban.
...
| OpenRouter | `OPENROUTER_API_KEY` | reviewer, cinematographer, editor (`video_analyze` routes through `AUXILIARY_VIDEO_MODEL` â†’ OpenRouter) |
...
| Anthropic | `ANTHROPIC_API_KEY` | every Zed profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain â†’ `${ZED_HOME:-~/.zed}/.env` â†’ environment variable.
...
When a new Zed-public video skill ships:
