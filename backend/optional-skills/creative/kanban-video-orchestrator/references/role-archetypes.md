...
- **Personality:** Tied to the brand voice â€” see `assets/soul.md.tmpl`
...
- **Toolsets:** kanban, terminal, file
- **Skills:** `kanban-worker` plus any project-specific design skill â€”
...
- **Toolsets:** kanban, file
- **Skills:** `kanban-worker` plus a diagram skill â€” `excalidraw` (sketch),
...
| `renderer-procedural` | (none â€” Python with PIL + ffmpeg directly) | Custom procedural content where no skill fits |
| `renderer-video` | (external image-to-video API: Runway / Kling / Luma) | Animating still images in narrative film |
| `renderer-motion-graphics` | (external â€” Remotion CLI) | Motion graphics, kinetic typography, UI animations |
...
- **Skills:** `kanban-worker`, `songsee` (audio visualization), plus one of:
  - `songwriting-and-ai-music` â€” when commissioning lyrics + Suno prompts
  - `heartmula` â€” when generating music with the open-source local model
  - `spotify` â€” when sourcing existing tracks
- **Outputs:** `audio/track.mp3`, `audio/beats.json`, optional
  `audio/track-spectrogram.png`

### voice-talent / narrator

Generates voiceover audio. Calls a TTS API directly; no Zed skill required
...
Color grading. Usually optional â€” if the renderers already produce
...
Reviews specifically for brand compliance â€” colors, typography, voice. Use
