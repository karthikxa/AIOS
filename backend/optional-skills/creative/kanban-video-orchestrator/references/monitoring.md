# Monitoring â€” Watch the Pipeline + Intervene
...
```bash
# Live event stream â€” task spawns, status changes, heartbeats, completions
zed kanban watch --tenant <project-slug>

# Snapshot of the board
zed kanban list --tenant <project-slug>
zed kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
zed kanban stats --tenant <project-slug>

# Visual dashboard (browser)
zed dashboard

# Inspect a specific task (includes comments + events)
zed kanban show <task-id>

# Follow a single task's event stream
zed kanban tail <task-id>
```

Verify available subcommands with `zed kanban --help` â€” the kanban CLI
...
- Tasks transition `READY â†’ RUNNING â†’ DONE` in roughly the expected order
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `zed kanban show <id>` â€” read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `zed kanban block <id>` then `zed kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `zed kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `zed kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway â€” raise `max-runtime` on a re-created task. |
| Child task READY but parents still RUNNING for >2Ã— expected | Cascade slow, dependency miswired | Check the dependency graph. Inspect the parent: sometimes it completed but its handoff fields (summary, metadata) were empty so the child has nothing to consume. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `zed profile list` (profile exists?), `zed status` (gateway/dispatcher up?), and verify tenant. |
| Specific renderer task fails â†’ review note â†’ renderer redoes â†’ fails again | Brief is asking for the impossible | Pivot the brief, not the renderer. |
...
zed kanban comment <renderer-task-id> "Scene 3 looks too sparse \
â€” increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
zed kanban create "Scene 3 â€” re-render with feedback" \
...
NEW_TASK_ID=$(zed kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
zed kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
zed kanban block <task-id>
zed kanban archive <task-id>

# Diagnose what happened
zed kanban show <task-id>      # task body, comments, recent events
zed kanban tail <task-id>      # follow the live event stream
zed kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `zed kanban create` for the director

Don't try to "edit while running" â€” the kanban's audit trail makes a clean
...
while true; do
    clear
    zed kanban list --tenant <slug>
    echo "---"
    zed kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `zed kanban watch --tenant <slug>` in a
separate terminal â€” it streams task lifecycle events as they happen.
...
notes. Do NOT delete the workspace â€” the user may want to iterate on a single
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `zed profile list` and start
...
- **Audio/visual sync.** The editor's clip stitching must match the
  ...
  the editor â€” read from the renderer's handoff metadata.
