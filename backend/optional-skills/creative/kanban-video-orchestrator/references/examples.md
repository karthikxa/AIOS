...
for that brief. **These are illustrative, not templates** â€” adapt to the
actual brief.

## Example 1 â€” Narrative short film (text-to-image â†’ image-to-video â†’ cut)

**Brief:** A 90-second noir-style short. A detective walks through a rainy
city. Voiceover narration. AI-generated visuals.

**Team:**
- `director` â€” vision, decomposition, approval
- `writer` â€” script + voiceover copy (loads `humanizer` for natural voice)
- `storyboarder` â€” beat-by-beat shot list (loads `excalidraw`)
- `image-generator` â€” generates each shot's still via local ComfyUI workflows
  (loads `comfyui`)
- `image-to-video-generator` â€” animates each still (Runway/Kling, OR
  ComfyUI's AnimateDiff/WAN workflows via `comfyui`)
- `voice-talent` â€” narration via ElevenLabs
- `audio-mixer` â€” VO + ambient pad
- `editor` â€” assembly + transitions
- `reviewer` â€” final QA
...
- Local ComfyUI via `comfyui` skill is preferred over external API for
  cost/control â€” but external APIs are fine if ComfyUI isn't installed
- `editor` profile is ffmpeg-only, no Zed skill required beyond
  `kanban-worker`
- Storyboarder produces `storyboard.excalidraw` alongside the markdown

## Example 2 â€” Product / marketing teaser
...
**Team:**
- `director`
- `copywriter` â€” taglines, voiceover script, CTA (loads `humanizer`)
- `concept-artist` â€” style frames (loads `claude-design` for UI mockups)
- `renderer-motion-graphics` â€” animated UI sequences (Remotion CLI)
- `renderer-ascii` â€” terminal-style demo scenes (loads `ascii-video`)
- `voice-talent` â€” VO via ElevenLabs
- `editor` â€” assembly + brand-color treatment
- `audio-mixer` â€” VO + light music bed
- `captioner` â€” burned subtitles for muted-autoplay platforms
- `masterer` â€” produces 1:1 + 9:16 + 16:9 variants
...
## Example 3 â€” Music video (synced to provided track)
...
**Team:**
- `director`
- `music-supervisor` â€” analyze track, emit `audio/beats.json` (loads `songsee`)
- `storyboarder` â€” beat-aligned shot list (loads `excalidraw`)
- `renderer-ascii` â€” ASCII scenes synced to bass kicks (loads `ascii-video`)
- `renderer-p5js` â€” generative particle scenes synced to highs (loads `p5js`)
- `editor` â€” beat-cut assembly using `beats.json`
- `reviewer` â€” sync QA

**Task graph:**
```
T0  director              decompose
T1  music-supervisor      analyze track â†’ beats.json + spectrogram  (parent: T0)
...
**Key choices:**
- `music-supervisor` runs FIRST â€” `beats.json` gates the renderers
- `editor` uses `beats.json` directly to align cuts to bass kicks
- No voice-talent â€” music is the audio
- Two specialized renderers (`ascii-video` + `p5js`) for visual variety

## Example 4 â€” Math/algorithm explainer
...
**Team:**
- `director`
- `writer` â€” narration script (loads `humanizer`)
- `cinematographer` â€” visual spec (loads `manim-video`)
- `renderer-manim` â€” all animated scenes (loads `manim-video`)
- `voice-talent` â€” narration via ElevenLabs
- `editor` â€” assembly + audio mux
- `captioner` â€” burned subtitles
...
## Example 5 â€” ASCII video, music-track-only
...
- `renderer-ascii` â€” all visuals (loads `ascii-video`)
- `editor` â€” assembly + audio mux
...
- No reviewer â€” short experimental piece, director approves directly
...
## Example 6 â€” Real-time / installation art
...
**Team:**
- `director`
- `cinematographer` â€” visual language spec (loads `touchdesigner-mcp`)
- `renderer-touchdesigner` â€” all visuals + record-to-disk
  (loads `touchdesigner-mcp`)
- `audio-mixer` â€” final loudness pass on the captured audio (optional if
  pre-mixed source)
- `editor` â€” assemble final clip from TouchDesigner recording
- `reviewer` â€” visual QA
...
**Key choices:**
- `touchdesigner-mcp` controls a running TouchDesigner instance â€” the
...
- **Plot, characters, scripted dialogue** â†’ Example 1 (narrative)
- **Specific product, CTA, brand colors, voiceover** â†’ Example 2 (marketing)
- **Track file provided, "synced to music"** â†’ Example 3 (music video)
- **"Explain how X works", math/algorithm/concept walkthrough** â†’ Example 4 (manim explainer)
- **Terminal aesthetic, ASCII, retro pixel** â†’ Example 5 (ASCII)
- **"Audio-reactive", "real-time", "installation"** â†’ Example 6 (TouchDesigner)
- **Comic-style narrative** â†’ use `renderer-comic` (`baoyu-comic` skill)
- **Retro game / pixel-art aesthetic** â†’ use `renderer-pixel` (`pixel-art` skill)
- **3D scene, photoreal environment** â†’ use `renderer-3d` (`blender-mcp`)
- **Generative art, particle system, shader** â†’ use `renderer-p5js` (`p5js`)
- **AI-generated photoreal stills + animation** â†’ use `renderer-comfyui`
  (`comfyui`) for both stills and image-to-video
- **"video about how the system works", recursive demo** â†’ composable from
  any of the above; the recursion is a rendering technique, not a style

The actual team should be derived from the specific brief â€” these examples are
