...
metadata:
  zed:
    ...
    credits: |
      ...
      https://github.com/zedteam/kanban-video-pipeline.
---
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${ZED_HOME:-~/.zed}/.env` or the user's secret store.
