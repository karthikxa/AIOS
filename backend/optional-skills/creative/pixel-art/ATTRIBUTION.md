...
- License: MIT
- Copyright: Â© Synero, MIT-licensed contributors

### What was ported

**`scripts/palettes.py`** â€” the `PALETTES` dict containing 23 named RGB
palettes (hardware and artistic). Values are reproduced verbatim from
`scripts/pixelart.py` of pixel-art-studio.

**`scripts/pixel_art_video.py`** â€” the 12 procedural animation init/draw pairs
(`stars`, `fireflies`, `leaves`, `dust_motes`, `sparkles`, `rain`,
`lightning`, `bubbles`, `embers`, `snowflakes`, `neon_pulse`, `heat_shimmer`)
and the `SCENES` â†’ layer mapping. Ported from `scripts/pixelart_video.py`
...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (Zed uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) â€” Zed has `image_generate`
...
- License: MIT (inherits from zed-agent repo)
- Original author of the skill shell: dodo-reach
- Expansion with palettes + video: Zed Agent contributors
