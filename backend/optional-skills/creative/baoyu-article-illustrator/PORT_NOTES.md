# Port Notes â€” baoyu-article-illustrator
...
| Change | Upstream | Zed |
|--------|----------|--------|
| Metadata namespace | `openclaw` | `zed` |
| Trigger | `/baoyu-article-illustrator` slash command + CLI flags | Natural language skill matching |
| User config | EXTEND.md (project/user/XDG paths) + first-time-setup | Removed â€” not part of Zed infra |
...
| Backend selection | User picks provider via CLI flags | Not agent-selectable â€” `image_generate` uses the user-configured FAL model. Removed hardcoded "nano banana pro" line from `prompts/system.md`. |
...
| File operations | Bash commands | Zed file tools: `write_file`/`read_file` for text, `terminal` for binaries and URL downloads, `vision_analyze` for reading images |
| Watermark | Driven by EXTEND.md `watermark.enabled` | Optional â€” user asks for it per-article |
...
- Type Ã— Style Ã— Palette three-dimension framework
- All style definitions (23 files, verbatim)
- All palette definitions (4 files, verbatim)
- Core reference files (workflow, prompt-construction, styles, style-presets) â€” adapted for Zed tooling
- Core principles and workflow structure (analyze â†’ confirm â†’ outline â†’ prompts â†’ generate)
...
# Diff style/palette files (safe to overwrite â€” unchanged from upstream)
...
`references/styles/*` and `references/palettes/*` can be overwritten directly. `SKILL.md`, `references/workflow.md`, `references/usage.md`, `references/style-presets.md`, `references/styles.md`, `references/prompt-construction.md`, and `prompts/system.md` must be manually merged since they contain Zed-specific adaptations (tool wiring, backend neutrality, removed EXTEND.md references).
