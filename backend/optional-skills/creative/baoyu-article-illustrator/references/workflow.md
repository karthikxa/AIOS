...
If the user provides reference images (local path or URL), the goal is to produce **textual descriptions** that can be embedded in prompts â€” `image_generate` doesn't accept reference-image inputs, and Zed' text file tools can't read or write binaries.
...
| Write the text description | `write_file` | Sidecar `.md` files only â€” never try to `write_file` a PNG/JPG. |
| (Optional) Keep a local copy of the binary | `terminal` | `cp "$src" "{output-dir}/references/NN-ref-{slug}.{ext}"` â€” purely for the record; the skill itself doesn't read the binary. |

| Input Type | Action |
|------------|--------|
| Image file path provided | `vision_analyze` â†’ write sidecar `.md`. Optional `terminal cp` for a local record. |
| Image URL provided | `vision_analyze` with the URL â†’ write sidecar `.md`. |
| Image in conversation (no path, no URL) | Ask via `clarify` for a path or URL, or for a verbal description. |
| User can't provide either | Extract style/palette verbally from the user â†’ write `references/extracted-style.md`. Do NOT add `references:` to prompt frontmatter. |
...
---
[vision_analyze description â€” colors, style, composition, subject]
```
...
|-------|------------------|------------------|
| Article file path | `{article-dir}/imgs/` (default) | â€” (read article via `read_file`) |
...
**CRITICAL**: If the article uses metaphors (e.g., "ç”µé”¯åˆ‡è¥¿ç“œ"), do NOT illustrate literally. Visualize the **underlying concept**.
...
Based on Step 2 content analysis, recommend a preset first (sets both type & style). Look up [style-presets.md](style-presets.md) "Content Type â†’ Preset Recommendations" table.

- [Recommended preset] â€” [brief: type + style + why]
- [Alternative preset] â€” [brief]
- Or choose type manually: infographic / scene / flowchart / comparison / framework / timeline / mixed

**If user picks a preset â†’ skip Q3** (type & style both resolved).
**If user picks a type â†’ Q3 is required.**

### Q2: Density

- minimal (1-2) â€” Core concepts only
- balanced (3-5) â€” Major sections
- per-section â€” At least 1 per section/chapter (Recommended)
- rich (6+) â€” Comprehensive coverage
...
Style selection based on Type Ã— Style compatibility matrix ([styles.md](styles.md)).
...
- `macaron` â€” soft pastel blocks on warm cream
- `warm` â€” warm earth tones, no cool colors
- `neon` â€” vibrant neon on dark backgrounds
...
|-----|----------|-------------------|
| 01 | 01-ref-diagram.png | direct â†’ Illustration 1, 3 |
| 02 | 02-ref-chart.png | palette â†’ Illustration 2 |
```
...
6. **Prompt quality requirements** (all REQUIRED):
   ...
   - `LABELS`: Use **actual numbers, terms, metrics, quotes from the article** â€” NOT generic placeholders
