...
# is always present â€” the engine auto-detects `HeadlessExperimental.beginFrame`
...
if ! command -v node >/dev/null 2>&1; then
  red "âœ— Node.js is not installed."
...
if [ "$node_major" -lt "$MIN_NODE_MAJOR" ]; then
  red "âœ— Node.js ${node_version} is too old. HyperFrames requires Node.js >= ${MIN_NODE_MAJOR}."
...
if ! command -v ffmpeg >/dev/null 2>&1; then
  red "âœ— FFmpeg is not installed."
  case "$(uname -s)" in
...
green "âœ“ FFmpeg $(ffmpeg -version 2>&1 | head -1 | awk '{print $3}')"

# --- 3. npm ------------------------------------------------------------------

if ! command -v npm >/dev/null 2>&1; then
  red "âœ— npm is not installed (should ship with Node.js)."
  exit 1
fi
...
if ! npm install -g "hyperframes@latest" >/dev/null 2>&1; then
  red "âœ— npm install -g hyperframes@latest failed."
...
green "âœ“ hyperframes ${installed_version} installed globally"

# Sanity-check minimum version.
version_ge() {
  # version_ge A B  â†’  true if A >= B
  [ "$(printf '%s\n%s\n' "$1" "$2" | sort -V | head -1)" = "$2" ]
}
if ! version_ge "$installed_version" "$MIN_HYPERFRAMES_VERSION"; then
  red "âœ— hyperframes ${installed_version} is below required minimum ${MIN_HYPERFRAMES_VERSION}."
...
# auto-detects and falls back to screenshot mode â€” but BeginFrame mode is
...
if ! npx --yes puppeteer browsers install chrome-headless-shell >/dev/null 2>&1; then
  yellow "âš  Could not pre-install chrome-headless-shell."
  ...
  green "âœ“ chrome-headless-shell installed"
fi
...
if hyperframes doctor; then
  ...
  yellow "âš  hyperframes doctor reported issues."
