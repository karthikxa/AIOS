...
FP8 (8-bit floating point) can provide significant time savings compared to FP16/BF16 while maintaining training stability. Axolotlâ€™s implementation uses PyTorchâ€™s TorchAO library with â€œtensorwiseâ€ scaling strategy.
...
Q: The trainer stopped and hasnâ€™t progressed in several minutes.
...
Q: AttributeError: â€˜DummyOptimâ€™ object has no attribute â€˜stepâ€™

Q: ModuleNotFoundError: No module named â€˜mpi4pyâ€™ using single GPU with deepspeed
...
A: This is likely due to vocab size mismatch. By default, Axolotl expands the modelâ€™s embeddings if the tokenizer has more tokens than the model. Please use the axolotl merge-lora command to merge the adapters instead of using your own scripts.

On the other hand, if the model has more tokens than the tokenizer, Axolotl does not shrink the modelâ€™s embeddings unless shrink_embeddings: true is set in the config.
...
A: This is the class name of the transformer layer to wrap with FSDP. For example, for LlamaForCausalLM, the value is LlamaDecoderLayer. To find this for a specific model, check the modelâ€™s PreTrainedModel definition and look for _no_split_modules variable in the modeling_<model_name>.py file within transformers library.
...
A: The specific turnâ€™s start/end could not be detected. Please ensure you have set the eos_token following your chat_template. Otherwise, this could be a chat_template which doesnâ€™t use proper boundaries for each turn (like system). On the rare occurrence, make sure your content is not [[dummy_message]]. Please let us know about this.
...
Q: â€œchat_template choice is tokenizer_default but tokenizerâ€™s chat_template is null. Please add a chat_template in tokenizer configâ€
...
Dataset pre-processing is the step where Axolotl takes each dataset youâ€™ve configured alongside the dataset format and prompt strategies to:
...
Letâ€™s say you are writing a custom prompt strategy or using a user-defined prompt template. Because the trainer cannot readily detect these changes, we cannot change the calculated hash value for the pre-processed dataset.
...
Multimodal support is limited and doesnâ€™t have full feature parity.

Here are the hyperparams youâ€™ll need to use to finetune a multimodal model.
...
The modelâ€™s initial loss and grad norm will be very high. We suspect this to be due to the Conv in the vision layers.
...
For multi-modal datasets, we adopt an extended chat_template format similar to OpenAIâ€™s Message format.
...
In the latest GRPO implementation, vLLM is used to significantly speedup trajectory generation during training. In this example, weâ€™re using 4 GPUs - 2 for training, and 2 for vLLM:

Make sure youâ€™ve installed the correct version of vLLM by including it as an extra when installing axolotl, e.g. pip install axolotl[vllm].

Your vLLM instance will now attempt to spin up, and itâ€™s time to kick off training utilizing our remaining two GPUs. In another terminal, execute:

Due to TRLâ€™s implementation with vLLM, the vLLM instance must use the last N GPUs instead of the first N GPUs. This is why in the example above, we use CUDA_VISIBLE_DEVICES=2,3 for the vLLM instance.

GRPO uses custom reward functions and transformations. Please have them ready locally.

For example, to load OpenAIâ€™s GSM8K and use a random reward for completions:
...
This matches the API of datasets.load_dataset, so if youâ€™re familiar with that, you will feel right at home.

For HuggingFaceâ€™s guide to load different dataset types, see here.
...
If path points to a file and ds_type is not specified, we will automatically infer the dataset type from the file extension, so you could omit ds_type if youâ€™d like.

If youâ€™re loading a directory, you can point the path to the directory.
...
If youâ€™re using a private dataset, you will need to enable the hf_use_auth_token flag in the root-level of the config file.
...
With the --use-ray CLI flag, Axolotl will use Ray Trainâ€™s TorchTrainer to run training.
...
Coming back to the example on a Ray cluster with 1 head node and 2 4xL40S worker nodes, letâ€™s say you want to make use of all 8 GPUs. You would be able to just set ray_num_workers: 8 and run the previous command. The Cluster tab will show the following:
...
Sequence parallelism is a technique that splits sequences across multiple GPUs, allowing you to train with very long sequences that wouldnâ€™t fit on a single GPU. Each GPU processes a different portion of the sequence, and the results are aggregated through a ring communication pattern.
...
Sequence parallelism is compatible with Axolotlâ€™s sample packing functionality. When using both features together:
...
Quantization Aware Training (QAT) is a technique for improving the accuracy of models which are quantized by applying â€œfakeâ€ quantizations to the modelâ€™s weights (and optionally, activations) during training. This fake quantization allows for the model to adjust for noise introduced by the quantization, so when the model is eventually quantized, the accuracy loss is minimized. We use the quantization techniques implemented in torchao to provide support for QAT and post-training quantization (PTQ) in axolotl.
...
If available memory is insufficient even after FSDPâ€™s CPU offloading, you can enable swap memory usage by setting cpu_offload_pin_memory: false alongside offload_params: true in FSDP config.
...
This was enabled by this work from the Answer.AI team.â†©ï¸Ž
...
Run the following command to install cut_cross_entropy[transformers] if you donâ€™t have it already.
...
Train with an example config (Llamaâ€‘3.2 1B): - Pretrain: axolotl train examples/llama-3/diffusion-3.2-1b-pretrain.yaml - SFT: axolotl train examples/llama-3/diffusion-3.2-1b-sft.yaml
...
Interactive controls (prefix the prompt with commands): - :complete N â†’ completion mode with N new masked tokens appended (default 64) - :mask R â†’ random masking mode with target mask ratio R in [0.0, 1.0]
...
Fine-tune sparsified models in Axolotl using Neural Magicâ€™s LLMCompressor.

This integration enables fine-tuning of models sparsified using LLMCompressor within the Axolotl training framework. By combining LLMCompressorâ€™s model compression capabilities with Axolotlâ€™s distributed training pipelines, users can efficiently fine-tune sparse models at scale.
...
Pre-sparsified checkpoints can be: - Generated using LLMCompressor - Downloaded from Neural Magicâ€™s Hugging Face page - Any custom LLM with compatible sparsity patterns that youâ€™ve created yourself
...
For more details on vLLMâ€™s capabilities and advanced configuration options, see the official vLLM documentation.
...
It is not necessary to place your integration in the integrations folder. It can be in any location, so long as itâ€™s installed in a package in your python env.
...
- ðŸŽ‰ Latest Updates
- âœ¨ Overview
- ðŸš€ Quick Start - LLM Fine-tuning in Minutes
  - Google Colab
  - Installation
    - Using pip
    - Using Docker
...
Thatâ€™s it! Check out our Getting Started Guide for a more detailed walkthrough.
...
Letâ€™s start by fine-tuning a small language model using LoRA. This example uses a 1B parameter model to ensure it runs on most GPUs. Assuming axolotl is installed (if not, see our Installation Guide)

Thatâ€™s it! Letâ€™s understand what just happened.

The YAML configuration file controls everything about your training. Hereâ€™s what (part of) our example config looks like:
...
Letâ€™s modify the example for your own data:
...
Example 3 (yaml):
```yaml
base_model: zedteam/Llama-3.2-1B
...
Example 4 (yaml):
```yaml
base_model: zedteam/Nous-Zed-llama-1b-v1
...
w packing ( note itâ€™s the same effective number of tokens per step, but a true bsz of 1)

cu_seqlens: [[ 0, 11, 17, 24, 28, 36, 41 44, 48, 51, 55, 60, 64]]

Multipack can still be achieved without Flash attention, but with lower packing efficiency as we are not able to join multiple batches into a single batch due to context length limits without flash attention. We can use either Pytorchâ€™s Scaled Dot Product Attention implementation or native Pytorch attention implementation along with 4d attention masks to pack sequences together and avoid cross attention.
...
Gradient accumulation means accumulating gradients over several mini-batches and updating the model weights afterward. When the samples in each batch are diverse, this technique doesnâ€™t significantly impact learning.

This method allows for effective training with larger effective batch sizes without needing proportionally larger memory. Hereâ€™s why:
...
Gradient Accumulation: With gradient accumulation, youâ€™re effectively simulating a larger batch size by accumulating gradients over several smaller batches (or micro-batches). However, at any given time, youâ€™re only forward and backward propagating a micro-batch. This means you only store activations for the micro-batch, not the full accumulated batch. As a result, you can simulate the effect of a larger batch size without the memory cost of storing activations for a large batch.
...
|----------------|----------------|----------------|
| â†’ (accumulate) | â†’ (accumulate) | â†’ (accumulate) |
|----------------|----------------|----------------|
| S10, S11, S12  | S13, S14, S15  | S16, S17, S18  |
| e10, e11, e12  | e13, e14, e15  | e16, e17, e18  |
|----------------|----------------|----------------|
| â†’ (apply)      | â†’ (apply)      | â†’ (apply)      |
...
Weight update for w1:
w1_new = w1_old - learning rate Ã— (Total gradient for w1 / 6)
```
...
While debugging itâ€™s helpful to simplify your test scenario as much as possible. Here are some tips for doing so:
...
Use a small dataset: Construct or use a small dataset from HF Hub. When using a small dataset, you will often have to make sure sample_packing: False and eval_sample_packing: False to avoid errors. If you are in a pinch and donâ€™t have time to construct a small dataset but want to use from the HF Hub, you can shard the data (this will still tokenize the entire dataset, but will only use a fraction of the data for training. For example, to shard the dataset into 20 pieces, add the following to your axolotl config):
...
[!Tip] To understand which containers are available, see the Docker section of the README and the DockerHub repo. For details of how the Docker containers are built, see axolotlâ€™s Docker CI builds.
...
The config actually mimics the command CUDA_VISIBLE_DEVICES=0 python -m accelerate.commands.launch -m axolotl.cli.train devtools/chat_template.yml, but this is the same thing.â†©ï¸Ž

Many of the below flags are recommended best practices by Nvidia when using nvidia-container-toolkit. You can read more about these flags here.â†©ï¸Ž
