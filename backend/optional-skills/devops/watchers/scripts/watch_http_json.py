...
Usage (via cron with --no-agent):
    ...
    zed cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ZED_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def _dig(obj, path: str):
    """Dotted-path lookup: _dig({'a':{'b':[1,2]}}, 'a.b') â†’ [1,2]."""
    if not path:
        return obj
    cur = obj
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Zed-Watcher/1.0"})
    ...
    # Keep only dicts â€” skip any bare strings / numbers so filter_new doesn't crash.
