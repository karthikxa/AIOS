...
Usage (via cron with --no-agent):
    ...
    zed cron create my-feed \\
      --schedule "*/15 * * * *" --no-agent \\
      --script "$ZED_HOME/skills/devops/watchers/scripts/watch_rss.py" \\
...
def _parse_feed(xml_bytes: bytes):
    ...
    for item in root.iter():
        ...
        if tag not in {"item", "entry"}:
            continue
        # ElementTree Elements without children are *falsy* â€” use `is not None`.
...
def main() -> int:
    ...
    # Empty stdout on no-new â€” cron treats that as silent.
    return 0
