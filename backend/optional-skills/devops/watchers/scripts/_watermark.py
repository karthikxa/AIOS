...
def _state_dir() -> Path:
    """Where watermark files live â€” respects WATCHER_STATE_DIR override."""
    override = os.environ.get("WATCHER_STATE_DIR")
    if override:
        return Path(override)
    # Default: $ZED_HOME/watcher-state/, falling back to ~/.zed/watcher-state/.
    zed_home = os.environ.get("ZED_HOME") or str(Path.home() / ".zed")
    return Path(zed_home) / "watcher-state"
    ...
    def load(cls, name: str, *, max_seen: int = 500) -> "Watermark":
        ...
        if wm._path.exists():
            try:
                ...
                # Corrupt state file â€” treat as a first run but don't crash.
...
def format_items_as_markdown(
    ...
    *,
    ...
    empty string when items is empty â€” cron will then treat stdout as
