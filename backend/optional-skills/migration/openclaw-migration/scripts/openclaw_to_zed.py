#!/usr/bin/env python3
"""OpenClaw -> Zed migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Zed Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Zed.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Zed memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Zed memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Zed-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Zed-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Zed command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.zed/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.zed/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Zed .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Zed .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Zed .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Zed .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Zed .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Zed config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Zed config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Zed.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Zed MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Zed config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'zed cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Zed config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Zed session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Zed custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Zed config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Zed config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Zed config.yaml approvals section.",
    },
    "memory-backend": {
...
# Item shape constants â€” kept stable for downstream consumers of report.json.
...
def resolve_secret_input(value: Any, env: Optional[Dict[str, str]] = None) -> Optional[str]:
    ...
    if isinstance(value, dict):
        ...
        # File/exec sources can't be resolved here â€” return None
    return None
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Zed config.yaml")
    ensure_parent(path)
    path.write_text(
...
# â”€â”€ Brand rewriting â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Replace OpenClaw brand names with Zed in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` â†’ ``Zed`` (prose), but lowercase matches
# like ``openclaw`` â†’ ``zed`` (so filesystem paths like ``~/.openclaw``
# become ``~/.zed`` â€” the real Zed home â€” not the broken ``~/.Zed``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Zed'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Zed'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Zed'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` â†’ ``Zed`` but maps ``openclaw`` â†’ ``zed`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.zed/config.yaml`` (the real Zed home) instead of the broken
    ``~/.Zed/config.yaml``.
    """
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with Zed.
...
# like a credential â€” by key name or by value shape â€” is replaced with
# "[redacted]" before the report is written.
#
# Modelled on OpenClaw's src/plugin-sdk/migration.ts so both migration tools
# redact consistently.  Pure function â€” safe to call on any plain-data dict.
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-zed/apply.ts "blocked by earlier
        ...
        if ws:
            ...
            if ws_path.is_dir():
                try:
                    ...
                    # ws_path is outside source_root â€” use it as custom workspace
    ...
    # Option ids that mutate the Zed config.yaml file.  Once any one of
    ...
    def migrate(self) -> Dict[str, Any]:
        ...
        # â”€â”€ v2 migration modules â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def run_if_selected(self, option_id: str, func) -> None:
        ...
        # write.  Dry-run mode never blocks â€” the user needs the full preview
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-zed/plan.ts warnings[].
        Keep the messages actionable â€” they show up in summary.md and the
        JSON report.
        """
        warnings: List[str] = []
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Zed session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Zed config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # Skip if the workspace points inside the OpenClaw source directory â€”
            # that path will be stale after migration and would cause the Zed
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Zed-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Zed-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: zed config set",
                        )
                    continue
        ...
        # Also check the OpenClaw .env file â€” many users store keys there
        ...
        for oc_key, zed_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and zed_key not in secret_additions:
                secret_additions[zed_key] = val

        # Check the openclaw.json "env" sub-object â€” some OpenClaw setups
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, zed_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and zed_key not in secret_additions:
                        secret_additions[zed_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        zed_config = load_yaml_file(destination)
        current_model = zed_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = zed_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                zed_config["model"] = {"default": model_str}
            dump_yaml_file(destination, zed_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; Zed still uses "edge"
        ...
        zed_config = load_yaml_file(destination)
        existing_tts = zed_config.get("tts", {})
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Zed destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Zed-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Zed-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    # â”€â”€ MCP servers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        zed_cfg_path = self.target_root / "config.yaml"
        zed_cfg = load_yaml_file(zed_cfg_path)
        existing_mcp = zed_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Zed config")
                continue

            zed_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                zed_srv["command"] = srv["command"]
                if srv.get("args"):
                    zed_srv["args"] = srv["args"]
                if srv.get("env"):
                    zed_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    zed_srv["cwd"] = srv["cwd"]
            ...
            if srv.get("url"):
                zed_srv["url"] = srv["url"]
                if srv.get("headers"):
                    zed_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    zed_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                zed_srv["enabled"] = False
            if srv.get("timeout"):
                zed_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                zed_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                zed_srv["tools"] = {}
                if tools_cfg.get("include"):
                    zed_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    zed_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(zed_cfg_path)
            zed_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(zed_cfg_path, zed_cfg)

    # â”€â”€ Plugins â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Cron jobs â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'zed cron' to recreate jobs manually.")
            else:
    ...
    # â”€â”€ Agent config â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = zed_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Zed reasoning_effort
        ...
        if compaction:
            compression = zed_cfg.get("compression") or {}
    ...
    # â”€â”€ Gateway config â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'zed gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("ZED_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")

    # â”€â”€ Session config â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        zed_cfg_path = self.target_root / "config.yaml"
        zed_cfg = load_yaml_file(zed_cfg_path)
        sr = zed_cfg.get("session_reset") or {}
        changes = False
    ...
    # â”€â”€ Full model providers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        zed_cfg_path = self.target_root / "config.yaml"
        zed_cfg = load_yaml_file(zed_cfg_path)
        custom_providers = zed_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(zed_cfg_path)
            zed_cfg["custom_providers"] = custom_providers
            dump_yaml_file(zed_cfg_path, zed_cfg)
    ...
    # â”€â”€ Deep channel config â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Zed config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            zed_cfg_path = self.target_root / "config.yaml"
            zed_cfg = load_yaml_file(zed_cfg_path)
            discord_zed = zed_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_zed["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_zed["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                zed_cfg["discord"] = discord_zed
                dump_yaml_file(zed_cfg_path, zed_cfg)
    ...
    # â”€â”€ Browser config â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        zed_cfg_path = self.target_root / "config.yaml"
        zed_cfg = load_yaml_file(zed_cfg_path)
        browser_zed = zed_cfg.get("browser") or {}
        changed = False

        # Map fields that have Zed equivalents
        if browser.get("cdpUrl"):
            browser_zed["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_zed["headless"] = browser["headless"]
            changed = True

        if changed:
            zed_cfg["browser"] = browser_zed
            if self.execute:
                self.maybe_backup(zed_cfg_path)
                dump_yaml_file(zed_cfg_path, zed_cfg)
    ...
    # â”€â”€ Tools config â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Approvals config â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            zed_mode = mode_map.get(mode, "manual")
            zed_cfg.setdefault("approvals", {})["mode"] = zed_mode
            if self.execute:
                self.maybe_backup(zed_cfg_path)
                dump_yaml_file(zed_cfg_path, zed_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{zed_mode}'")
    ...
    # â”€â”€ Memory backend â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Skills config â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ UI / Identity â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Logging / Diagnostics â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Helper: set env var â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Generate migration notes â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Zed Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Zed. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Zed Config Not Overwritten)",
                "",
                "These items already existed in your Zed config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Zed agent discovers these",
            "directories, it may read/write to them instead of the Zed state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `zed claw cleanup` to rename the OpenClaw",
            "directory to `.openclaw.pre-migration`. This prevents the agent from finding it.",
            "The directory is renamed, not deleted â€” you can undo this at any time.",
            "",
            "If you skip this step and notice the agent getting confused about workspaces",
            "or todo lists, run `zed claw cleanup` to fix it.",
            "",
            "## Zed-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `zed claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `zed setup` to configure any remaining settings",
            "- Run `zed mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `zed cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `zed cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `zed gateway install` if you need the gateway service",
            "- Review `~/.zed/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Zed Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("ZED_HOME") or str(Path.home() / ".zed"), help="Zed home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Zed targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Zed-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    # â”€â”€ Machine-readable JSON mode â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Human-readable terminal recap â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    s = report["summary"]
    items = report["items"]
    ...
    print(f"  â•‘   OpenClaw -> Zed Migration   [{mode_label:>8s}]   â•‘")
    print(f"  â• â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•£")
    print(f"  â•‘  Source:  {str(report['source_root'])[:42]:<42s}  â•‘")
    print(f"  â•‘  Target:  {str(report['target_root'])[:42]:<42s}  â•‘")
    print(f"  â• â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•£")
    print(f"  â•‘  âœ” Migrated:  {s.get('migrated', 0):>3d}    â—† Archived:  {s.get('archived', 0):>3d}        â•‘")
    print(f"  â•‘  âŠ˜ Skipped:   {s.get('skipped', 0):>3d}    âš  Conflicts: {s.get('conflict', 0):>3d}        â•‘")
    print(f"  â•‘  âœ– Errors:    {s.get('error', 0):>3d}    Total:       {total:>3d}        â•‘")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.zed/" + dest[len(str(report["target_root"])) + 1:]
            meta = MIGRATION_OPTION_METADATA.get(label, {})
            display = meta.get("label", label)
            print(f"    âœ” {display:<35s} -> {dest}")
    ...
    if archived:
        ...
        for item in archived:
            ...
            print(f"    â—† {display:<35s}  {short_reason}")
    ...
    if conflicts:
        ...
        for item in conflicts:
            print(f"    âš  {item['kind']}: {item.get('reason', '')}")
    ...
    if errors:
        ...
        print("  Errors:")
        for item in errors:
            print(f"    âœ– {item['kind']}: {item.get('reason', '')}")

    # PM2 reassurance
    print()
    print("  â„¹ PM2 processes (Discord/Telegram bots) are NOT affected.")

    # Next steps
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.zed/config.yaml")
        print("    2. Run: zed mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: zed cron")
        if report.get("output_dir"):
            print(f"    â†’ Full report: {report['output_dir']}/MIGRATION_NOTES.md")
