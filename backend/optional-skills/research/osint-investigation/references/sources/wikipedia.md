...
Together they're a high-precision entity-resolution layer â€” the bar for
...
Set `ZED_OSINT_UA` to something identifying (e.g. `your-app/1.0 (you@example.com)`).
...
facts â€” far more lenient on rate limits.
...
- High notability bar â€” most private individuals are not in Wikipedia

## 5. Cross-Reference Potential

- **All sources** â†” `label` (entity identity resolution)
- **SEC EDGAR** â†” `label` (public companies)
- **CourtListener** â†” `label` (parties to notable litigation)
...
- Notability filter â€” only notable entities (criteria vary by topic)
- Recency lag â€” current events take days to weeks to be reflected
- POV / vandalism â€” moderated, but edits between sweeps can be bad
- Living-persons biographies have stricter sourcing requirements
- Wikidata claims have qualifiers and references â€” the fetch script
...
The OpenSearch is fuzzy â€” `--limit 5` returns the top 5 Wikipedia article
