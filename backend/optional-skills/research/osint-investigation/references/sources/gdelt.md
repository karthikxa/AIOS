# GDELT â€” Global News Monitoring
...
~2015 â†’ present, ~1B+ articles indexed. Free anonymous access.
...
- **Auth:** None
- **Rate limit:** **1 request per 5 seconds** for the DOC API â€” strict
...
- ~2015 â†’ present (Events back to 1979 via a separate stream)
- Update frequency: 15 minutes
- Bias: heavily Anglophone in volume but very wide source list overall

## 5. Cross-Reference Potential

- **All sources** â†” `title` / `url` (news context for any subject)
- **Wikipedia** â†” event timeline for notable entities
- **Wayback Machine** â†” recover articles whose URLs have died
- **OFAC SDN** â†” news context for sanctions designations
- **SEC EDGAR** â†” news context for 8-K material events
...
fetcher â€” query GDELT directly for entity-level filtering.
...
- Source country is inferred from domain registration / TLD â€” can be
  wrong for international news sites with country-neutral domains
- Article URLs can rot â€” pair with Wayback Machine to preserve content
...
python3 SKILL_DIR/scripts/fetch_gdelt.py --query "Zed Team" \
...
- Article URLs link out to original publishers â€” copyright remains with
  the publisher
