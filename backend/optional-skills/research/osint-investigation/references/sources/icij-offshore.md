...
- **Note:** The previous Open Refine reconciliation endpoint at
  ...
  the ZIP locally (default `~/.cache/zed-osint/icij/`, refreshes after
...
- Earliest records: 1970s (Bahamas Leaks). Most data 1990â€“2018.
- NOT updated in real-time â€” new leaks added when ICIJ publishes them
- ~810,000 offshore entities + ~750,000 officers + ~150,000 intermediaries

## 5. Cross-Reference Potential

- **SEC EDGAR** â†” `name` (public companies with offshore arms)
- **USAspending** â†” `name` (federal contractors with offshore structure)
- **OFAC SDN** â†” `name` (sanctioned entities using offshore vehicles)
...
- The connections graph is incomplete â€” some relationships are documented in
...
First call downloads the ~70 MB ZIP under `~/.cache/zed-osint/icij/`
(or `$ZED_OSINT_CACHE/icij/`). Subsequent calls reuse the cache for 30 days.
