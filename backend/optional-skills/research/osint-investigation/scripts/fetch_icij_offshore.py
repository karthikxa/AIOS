...
404 â€” ICIJ has removed it. The remaining stable access path is the public
bulk download:
...
$ZED_OSINT_CACHE/icij/ (default: ~/.cache/zed-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("ZED_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "zed-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "zed-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $ZED_OSINT_CACHE/icij or ~/.cache/zed-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
