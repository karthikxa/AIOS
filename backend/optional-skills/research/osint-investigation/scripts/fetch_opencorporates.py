...
def _via_html(query: str, limit: int) -> list[dict]:
    ...
    params = {"q": query, "utf8": "âœ“"}
    url = f"{HTML_URL}?{urllib.parse.urlencode(params)}"
    body = get(url, user_agent="Mozilla/5.0 zed-osint").decode("utf-8", errors="replace")
...
def fetch(
    ...
    if token:
        ...
        print(
            "OPENCORPORATES_API_TOKEN not set â€” using HTML fallback (limited fields). "
