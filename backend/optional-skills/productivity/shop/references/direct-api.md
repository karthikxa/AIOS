...
device_name=<your name> - <device>   # e.g. Max - Mac Mini; name from IDENTITY.md (OpenClaw) / ~/.zed/SOUL.md (Zed)
```
...
returned under `payment.instruments`. Re-send each instrument verbatim â€”
including the merchant-issued `id` â€” with `selected: true` and `credential.token`
...
it did not complete â€” do not retry without re-verifying.
...
When the buyer enables purchasing without approval in [Shop â†’ Settings â†’ Connections](https://shop.app/account/settings/connections), Shop issues a budgeted wallet payment token. Read the remaining budget:
...
```json
{
  "payment_tokens": [
    {
      "id": "<wallet token â€” never log or persist>",
...
**`limit` and `remaining_amount` are minor units (cents)** â€” `remaining_amount: 5750` is $57.50. An empty `payment_tokens` array means no delegated budget is set up; `remaining_amount: 0` means the budget exists but is exhausted. (Stay tolerant: older shapes put the token at `.token`/`.id` and amounts at the root or `.display`.)

Never persist or surface the wallet token value itself â€” only report whether a budget is available and how much remains. The user can adjust or revoke the budget at any time in Shop â†’ Settings â†’ Connections.

**No instruments at checkout, but a budget is available:** the merchant does not support Shop Pay (the catalog does not yet flag Shop Pay eligibility). When a checkout returns no `payment.instruments`, GET this endpoint to disambiguate: if a token exists (budget available), hand off `continue_url` for manual checkout or suggest another store â€” do **not** re-prompt to set up a budget. If no token exists, the buyer simply has no delegated budget (offer the Finish in Shop link / budget setup as usual).
...
The response is `text/markdown` (a short summary), not JSON â€” there is no result cursor to page through. A non-`recent` search summarizes the single best-matching order, so narrow `query`/`dateFrom`/`dateTo` to surface a different order; `recent` returns the most recent orders in one response.
