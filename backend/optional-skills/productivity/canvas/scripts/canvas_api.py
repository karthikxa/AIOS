#!/usr/bin/env python3
"""Canvas LMS API CLI for Zed Agent.
...
def _check_config():
    ...
    if missing:
        zed_env = os.path.join(
            os.environ.get("ZED_HOME", os.path.expanduser("~/.zed")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {zed_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Zed Agent"
    )
