---
name: telephony
description: Give Zed phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
version: 1.0.0
author: Zed Team
license: MIT
...
- save provider credentials into `${ZED_HOME:-~/.zed}/.env`
...
3. Buy it and save it into `${ZED_HOME:-~/.zed}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `${ZED_HOME:-~/.zed}/.env`
