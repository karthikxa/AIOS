...
metadata:
  zed:
    ...
    category: security
setup:
  help: "Create a service account at https://my.1password.com â†’ Settings â†’ Service Accounts"
  collect_secrets:
...
Set `OP_SERVICE_ACCOUNT_TOKEN` in `${ZED_HOME:-~/.zed}/.env` (the skill will prompt for this on first load).
