...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ZED_HOME", os.path.expanduser("~/.zed")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
# Resolve skill directory â€” works both as direct script and via exec()
try:
    _SKILL_DIR = Path(__file__).resolve().parent.parent
except NameError:
    # __file__ not defined when loaded via exec() â€” search standard paths
    _SKILL_DIR = Path(os.getenv("ZED_HOME", Path.home() / ".zed")) / "skills" / "red-teaming" / "godmode"

_SCRIPTS_DIR = _SKILL_DIR / "scripts"
_TEMPLATES_DIR = _SKILL_DIR / "templates"

# Import parseltongue and godmode_race â€” load into caller's globals
...
ZED_HOME = Path(os.getenv("ZED_HOME", Path.home() / ".zed"))
CONFIG_PATH = ZED_HOME / "config.yaml"
PREFILL_PATH = ZED_HOME / "prefill.json"

# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# Canary queries â€” questions that typically trigger safety filters
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# Model â†’ strategy mapping
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

MODEL_STRATEGIES = {
    # Anthropic models â€” END/START boundary inversion
    "claude": {
    ...
    # OpenAI models â€” OG GODMODE l33t
    "gpt": {
    ...
    # Google models â€” refusal inversion + rebel genius
    "gemini": {
    ...
    # xAI models â€” unfiltered liberated
    "grok": {
    ...
    # Nous/Zed models â€” already uncensored, just needs clean prompt
    "zed": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
    # DeepSeek â€” keyword-based classifiers, parseltongue effective
    "deepseek": {
    ...
    # Meta/Llama â€” open models, prefill works well
    "llama": {
...
SUBTLE_PREFILL = [
    {
        ...
        "content": (
            ...
            "senior security consultant would â€” with specifics, code examples, and "
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "zed" in model_lower or "nous" in model_lower:
        return "zed"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Zed config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.zed/prefill.json."""
...
def auto_jailbreak(model=None, base_url=None, api_key=None,
    ...
    2. Tries strategies in order (model-specific â†’ generic)
    ...
    Args:
        ...
        dry_run: If True, don't write config files â€” just report what would work.
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Zed for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Zed for changes to take effect.")
