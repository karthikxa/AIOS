#!/usr/bin/env python3
"""
Parseltongue v4 â€” Input Obfuscation Engine
...
UNICODE_MAP = {'a': 'Ð°', 'c': 'Ñ', 'e': 'Ðµ', 'i': 'Ñ–', 'o': 'Ð¾',
               'p': 'Ñ€', 's': 'Ñ•', 'x': 'Ñ…', 'y': 'Ñƒ'}
...
SMALLCAPS_MAP = {
    ...
    's': 'êœ±', 't': 'á´›', 'u': 'á´œ', 'v': 'á´ ', 'w': 'á´¡', 'y': 'Ê', 'z': 'á´¢',
}
...
def _apply_raw(word):
    """Raw â€” no transformation (baseline)."""
    return word

def _apply_leetspeak(word):
    """L33t â€” basic leetspeak substitution."""
    return ''.join(LEET_MAP.get(c.lower(), c) for c in word)

def _apply_unicode(word):
    """Unicode â€” Cyrillic/homoglyph substitution."""
    result = []
    for c in word:
...
def _apply_bubble(word):
    """Bubble â€” circled letter Unicode characters."""
    result = []
    for c in word:
        code = ord(c.lower())
        if 97 <= code <= 122:
...
def _apply_spaced(word):
    """Spaced â€” insert spaces between characters."""
    return ' '.join(word)

def _apply_fullwidth(word):
    """Fullwidth â€” fullwidth Unicode characters."""
    result = []
    for c in word:
        code = ord(c)
        if 33 <= code <= 126:
...
def _apply_brackets(word):
    """[B.r.a.c.k] â€” each character in brackets."""
    return '[' + ']['.join(word) + ']'

def _apply_mathbold(word):
    """MathBold â€” mathematical bold Unicode."""
    result = []
    for c in word:
        code = ord(c.lower())
        if 97 <= code <= 122:
...
def _apply_mathitalic(word):
    """MathItalic â€” mathematical italic Unicode."""
    result = []
    for c in word:
        code = ord(c.lower())
        if 97 <= code <= 122:
...
def _apply_strikethrough(word):
    """SÌ¶tÌ¶rÌ¶iÌ¶kÌ¶eÌ¶ â€” strikethrough combining characters."""
    return ''.join(c + '\u0336' for c in word)

def _apply_leetheavy(word):
    """L33t+ â€” heavy leetspeak with extended map."""
    return ''.join(LEET_MAP_HEAVY.get(c.lower(), LEET_MAP.get(c.lower(), c)) for c in word)

def _apply_hyphenated(word):
    """Hyphen â€” hyphens between characters."""
    return '-'.join(word)

# â”€â”€â”€ TIER 3: MULTI-LAYER COMBOS (23â€“33) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def _apply_leetunicode(word):
    """L33t+Uni â€” alternating leet and unicode."""
    result = []
...
def _apply_unicodezwj(word):
    """Uni+ZWJ â€” unicode homoglyphs with zero-width non-joiners."""
    result = []
    for c in word:
        mapped = UNICODE_MAP.get(c.lower())
...
def _apply_base64hint(word):
    """Base64 â€” base64 encode the word."""
    try:
...
def _apply_hexencode(word):
    """Hex â€” hex encode each character."""
    return ' '.join(f'0x{ord(c):x}' for c in word)

def _apply_acrostic(word):
    """Acrostic â€” NATO alphabet expansion."""
    result = []
    for c in word:
        idx = ord(c.lower()) - 97
        if 0 <= idx < 26:
...
def _apply_dottedunicode(word):
    """Dot+Uni â€” unicode homoglyphs with dots."""
    result = []
    for c in word:
        mapped = UNICODE_MAP.get(c.lower())
        result.append(mapped if mapped else c)
...
def _apply_fullwidthmixed(word):
    """FW MiX â€” fullwidth + mixed case alternating."""
    result = []
    for i, c in enumerate(word):
        code = ord(c)
...
def _apply_triplelayer(word):
    """Triple â€” leet + unicode + uppercase rotating with ZWJ."""
    result = []
    for i, c in enumerate(word):
        lower = c.lower()
        mod = i % 3
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Parseltongue â€” Input Obfuscation Engine')
