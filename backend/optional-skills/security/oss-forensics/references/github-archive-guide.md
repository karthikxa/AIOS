...
GitHub Archive records every public event on GitHub as immutable JSON records. This data is accessible via Google BigQuery and is the most reliable source for forensic investigation â€” events cannot be deleted or modified after recording.
...
- `payload.distinct_size = 0` with `payload.size > 0` â†’ commits were erased
...
3. **Select only needed columns**: Avoid `SELECT *`. The `payload` column is large â€” only select specific JSON paths.
...
If BigQuery is unavailable, document this limitation in the report. Use the other 4 investigators (Git, GitHub API, Wayback Machine, IOC Enrichment) â€” they cover most investigation needs without BigQuery.
