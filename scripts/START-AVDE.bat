...
start "LLM Proxy" cmd /k "set PORT=3001&& set LOCAL_PORT=3002&& cd /d "%~dp0..\llm\llm-proxy" && npm run dev -w server"
timeout /t 3 /nobreak >nul
echo [2/5] Starting Zed Agent Backend (port 8642)...
start "Zed Agent Backend" cmd /k "set ZED_HOME=%USERPROFILE%\.hermes&& set ZED_PRO_BASE_URL=http://127.0.0.1:3002/v1&& cd /d "%~dp0..\backend" && .venv\Scripts\python.exe -m uvicorn server:app --host 0.0.0.0 --port 8642"
timeout /t 3 /nobreak >nul
