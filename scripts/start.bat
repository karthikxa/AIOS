...
start "Zed Agent Backend - Port 8642" cmd /k "set ZED_HOME=%USERPROFILE%\.hermes && cd /d "%~dp0..\backend" && .venv\Scripts\python.exe -m uvicorn server:app --host 0.0.0.0 --port 8642 --reload"
...
start "Computer Agent - Port 4000" cmd /k "cd /d "%~dp0..\desktop-agent\agent" && ..\..\backend\.venv\Scripts\python.exe -m uvicorn main:app --host 0.0.0.0 --port 4000"
ping 127.0.0.1 -n 5 > nul
